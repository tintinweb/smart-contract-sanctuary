...
contract Presale is ReentrancyGuard {
    ...
    // All Presale Holders are entitled to contribute 40% of their presale commitments
    // For Private Farming
    ...
    function buy (uint256 _amount, address beneficiary) public nonReentrant {
        ...
        whitelistAllowance[_buyer] = _amount.div(100).div(60);
    ...
    function changeWhitelistingAllowance (address _user, uint256 _amount) external view returns (uint256) {
        return whitelistAllowance[_user];
    }
    ...
    // Unsold BYIELD will be burned 2 weeks after Presale has ended to provide everyone a fair chance to claim 
    function withdrawUnsoldDOLLARS() external onlyOwner {
...
contract PrivateFarmMC is Ownable, ReentrancyGuard {
    ...
    // Total of 5,000 bYield Allocated for Private Farms 
    // Based on Period of 4 days (115200 blocks), equalivant to 0.043 bYield Per Block
    // Emission Rate will stay constant throughout
    uint256 public constant INITIAL_EMISSION_RATE = 43 finney;
    ...
    // Lockup Period for all Withdrawal and Harvest: 4 days = 115200 Blocks
    ...
    function updatePool(uint256 _pid) public {
        ...
        // Since there are no Mint, updatePool will not fail when there are no more BYIELD to mint 
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0 && block.number >= endBlock) {
        ...
        // Pid 0 and 1 will be Native LP that will have no allowance limitations
        if (_pid != 0 && _pid != 1) {
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        // In Line with our private farms policy, withdrawal are locked till end of block
        require(block.number >= endBlock, "Emergency Withdrawal locked until end block");
    ...
    function safeByieldTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > byieldBal && byieldBal != 0) {
