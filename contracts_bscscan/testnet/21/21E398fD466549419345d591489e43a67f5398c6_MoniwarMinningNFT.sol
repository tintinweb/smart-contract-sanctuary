...
contract MoniwarMinningNFT is Ownable, ReentrancyGuard {
    ...
    function calculator(uint256 _pid, uint256 date, uint256 hashrate) external view returns (uint256) {
        ...
        accMowaPerShare = accMowaPerShare.add(mowaReward.div(totalHahrate));
        return hashrate.mul(accMowaPerShare);
    }
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256)
    {
        ...
        accMowaPerShare = accMowaPerShare.add(mowaReward.div(pool.totalHashRate));
        pending = user.hashRate.mul(accMowaPerShare).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 mowaReward = multiplier.mul(pool.rewardPerBlock);
        pool.accMowaPerShare = pool.accMowaPerShare.add(mowaReward.div(pool.totalHashRate));
        pool.lastRewardBlock = block.number;
    }
...
//        user.rewardDebt = user.hashRate.mul(pool.accMowaPerShare);
    ...
    function harvest(uint256 _pid) public nonReentrant {
        ...
        if (user.hashRate == 0 || block.number >= pool.endBlock) {
            ...
            pending = user.hashRate.mul(pool.accMowaPerShare).sub(user.rewardDebt);
