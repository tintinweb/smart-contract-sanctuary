...
contract SWP {
    ...
    mapping(address => uint16) public targets;  // percentage weights of assets
    ...
    constructor (
        ...
        for (uint i=0; i<_tokens.length; i++) {
            addAsset(_tokens[i]);
            setTargetForToken(_tokens[i], _targets[i]);
        }
    ...
    function disinvest(uint256 _swp) public returns (bool success) {
        ...
        if (_bnb > CASH()) {
            rebalance(AUM().sub(_bnb));
        }
        
        // Transfer BASE
        // safeTransferFrom(BASE, address(this), msg.sender, _bnb);
        ERC20(BASE).transfer(msg.sender, _bnb);
    ...
    function checkWeights() private returns (bool _success) { 
        ...
        for (uint i=0; i<assets.length; i++) {
            total += targets[assets[i]];
        }
        require(total <= 100, "Sum of weights exceeds 100 %");
        targets[BASE] = 100 - total;
        return true;
    }
    
    function setTargetForToken(address _token, uint8 _weight) public onlyOwnerOrSelf returns (bool _success) { 
        targets[_token] = _weight;
        checkWeights();
        return true;
    }
    ...
    function getPortfolio() public view returns (Position[] memory _port) {
        Position[] memory portfolio  = new Position[](assets.length+1);
        
        portfolio[0].add    = BASE;
        portfolio[0].symbol = '_BASE_';
        portfolio[0].weight = Weight(BASE);
        portfolio[0].target = targets[BASE];
        
        for (uint i=0; i<assets.length; i++) {
            portfolio[i+1].add    = assets[i];
            portfolio[i+1].symbol = ERC20(assets[i]).symbol();
            portfolio[i+1].weight = Weight(assets[i]);
            portfolio[i+1].target = targets[assets[i]];
        }
        return portfolio;
    }
    ...
    function rebalance(uint256 _AUM) private returns (bool _success) {
        ...
        for (uint i=0; i<assets.length; i++) {
            ...
            if (_AUM==0) {
                ...
                amounts = PK(PK_ROUTER).getAmountsOut(1000, path); // Should change that "1000"
                uint256 H = desiredHoldings(_AUM, targets[assets[i]], amounts[0], amounts[1]);  // Desired holdings of token i
