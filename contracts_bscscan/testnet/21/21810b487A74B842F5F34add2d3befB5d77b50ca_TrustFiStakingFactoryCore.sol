...
import "./interface/ITrustFiStakingFactoryCreator.sol";
...
contract TrustFiStakingFactoryCore is ITrustFiStakingFactoryCore {
    ...
    */
    ...
    /**
        Pre-unlock procedures cost for platform
        Factory: Indicates the factory contract
        PoolId: indicates the ID of a pool
        Token: indicates the address of the token contract
        To: award transfer address
        FeeAmount: fee for processing
    */
    event UnStateRewardPlatformFee(
        address factory,
        uint256 poolId,
        address indexed token,
        address indexed to,
    ...
    */
    ...
    function _unStake(uint256 poolId, uint256 amount, address user) override onlyFactory external {
        ...
        if (block.number >= poolStakeInfo.startBlock &&  //The mining pool has begun
            ...
            uint256 unStakeFeePercent = ITrustFiStakingFactoryCreator(platform).getUnStakeFeePercent();
            uint256 platformFee = rewardFee.mul(unStakeFeePercent).div(100);
            IERC20(poolStakeInfo.token).safeTransfer(platform,platformFee);
            emit UnStateRewardPlatformFee(factory, poolId, poolStakeInfo.token, user, platformFee);

            IERC20(poolStakeInfo.token).safeTransfer(poolStakeInfo.feeAddress,rewardFee.sub(platformFee));
            emit UnStateRewardFee(factory, poolId, poolStakeInfo.token, user, rewardFee.sub(platformFee));
...
interface ITrustFiStakingFactoryCreator {
