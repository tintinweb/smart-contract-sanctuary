...
contract MacroPenis is ERC20, Ownable {
    ...
    uint256 public _tokenLiquidityThreshold = 5e18;
    ...
    // 7,878,334,000
    uint256 constant internal max_cap_supply = 7878334000000000000000000000; // 18 zeros are the decimals
    ...
    constructor() ERC20("MacroPenis", "MP") {
    ...
    function sendStuckContractCoinsToMarketing() public onlyOwner {
        // kcs
        uint256 kcs_balance = address(this).balance;
        if (kcs_balance > 0)
        {
            //_approve(address(this), marketingwallet, kcs_balance);
    ...
    function removeStuckContractLiquidity(uint256 amount) public onlyOwner {
        // approve token transfer to cover all possible scenarios
        //_approve(address(this), address(router), amount);
        //_approve(address(router), address(this), amount);
        
        router.removeLiquidity(
            address(this),
            router.WETH(), //KUCOIN: router.WKCS(),
            amount,
            0,
            0,
            address(this),
    ...
    function changeFees(uint16 _feeliq, uint16 _feeburn, uint16 _feemarketing) external onlyOwner returns (bool){
        ...
        // cannot set more than 20% total taxes
        require(feesum <= 20, "exceeds hardcap");
        return true;
    }
    ...
    function updateExemptTransferLimit(address _address, bool state) public onlyOwner returns (bool) {
        exemptTransferlimit[_address] = state;
        return true;
    }

    function updateExemptFee(address _address, bool state) public onlyOwner returns (bool) {
    ...
    function whitelistInitialLiquidityProviders(address _liqidityProvider) public onlyOwner {
        ...
        updateExemptTransferLimit(_liqidityProvider, true);
        updateExemptFee(_liqidityProvider, true);
    }
