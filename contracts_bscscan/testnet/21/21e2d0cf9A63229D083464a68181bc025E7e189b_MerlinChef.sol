...
import "./interfaces/IMerlinMinterV2.sol";
import "./interfaces/IMerlinChef.sol";
import "./interfaces/IStrategy.sol";
import "./token/MERL.sol";
...
contract MerlinChef is IMerlinChef, OwnableUpgradeable {
    ...
    IMerlinMinterV2 public minter;

    uint256 public startBlock;
    uint256 public override merlinPerBlock;
    ...
    modifier onlyVaults() {
        require(
            ...
            "MerlinChef: caller is not on the vault"
        );
        _;
    }
        ...
        if (block.number > vaultInfo.lastRewardBlock) {
            ...
            if (tokenSupply > 0) {
                ...
                uint256 rewards = multiplier
                    ...
                    .div(totalAllocPoint);
                vaultInfo.accMerlinPerShare = vaultInfo.accMerlinPerShare.add(
    ...
    event MerlinRewardPaid(
        address indexed user,
        address indexed vault,
        uint256 amount
    );
    event WBNBRewardPaid(
        address indexed user,
        address indexed vault,
        uint256 amount
    );
     ...
     */
    constructor(address merlin, address wbnb) public {
        MERL = MERLToken(merlin);
        WBNB = wbnb;
    }

    /* ========== INITIALIZER ========== */

    function initialize(uint256 _startBlock, uint256 _merlinPerBlock)
        external
        initializer
    {
        __Ownable_init();
    ...
    function pendingMerlin(address vault, address user)
        public
        view
        override
        returns (uint256)
    {
        ...
        uint256 accMerlinPerShare = vaultInfo.accMerlinPerShare;
        uint256 tokenSupply = tokenSupplyOf(vault);
        if (block.number > vaultInfo.lastRewardBlock && tokenSupply > 0) {
            ...
            uint256 merlinRewards = multiplier
                .mul(merlinPerBlock)
                .mul(vaultInfo.allocPoint)
                .div(totalAllocPoint);
            accMerlinPerShare = accMerlinPerShare.add(
                merlinRewards.mul(1e12).div(tokenSupply)
            );
        }
        return
            userInfo.pending.add(
                userInfo.balance.mul(accMerlinPerShare).div(1e12).sub(
                    userInfo.rewardPaid
                )
            );
    }
    ...
    function addVault(
        ...
        require(
            ...
            "MerlinChef: vault is already set"
        );
        bulkUpdateRewards();
    ...
    function setMinter(address _minter) external onlyOwner {
        require(
            ...
            "MerlinChef: setMinter only once"
        );
        minter = IMerlinMinterV2(_minter);
    }

    function setMerlinPerBlock(uint256 _merlinPerBlock) external onlyOwner {
        bulkUpdateRewards();
        merlinPerBlock = _merlinPerBlock;
    }
        ...
        updateRewards(msg.sender)
    {
        ...
        uint256 pending = userInfo
            .balance
            .mul(vaultInfo.accMerlinPerShare)
            .div(1e12)
    ...
    function safeMerlinTransfer(address user)
        external
        override
        onlyVaults
        updateRewards(msg.sender)
        returns (uint256)
    {
        ...
        uint256 bal = IBEP20(WBNB).balanceOf(address(this));
        if (amount > 0 && amount <= bal) {
            IBEP20(WBNB).safeTransfer(user, amount);
        }

        // minter.mint(amount);
        // minter.safeMerlinTransfer(user, amount);
        emit WBNBRewardPaid(user, msg.sender, amount);
        return amount;
    }
        ...
        onlyOwner
    {
        require(
            _token != address(MERL),
            "MerlinChef: cannot recover MERL token"
        );
...
interface IMerlinMinterV2 {
    ...
    function amountMerlinToMintForMerlinBNB(uint256 amount, uint256 duration)
        external
        view
        returns (uint256);
...
interface IMerlinChef {
    ...
    struct VaultInfo {
        address token;
        uint256 allocPoint; // How many allocation points assigned to this pool. MERLs to distribute per block.
        uint256 lastRewardBlock; // Last block number that MERLs distribution occurs.
        uint256 accMerlinPerShare; // Accumulated MERLs per share, times 1e12. See below.
    }

    function merlinPerBlock() external view returns (uint256);
    ...
    function safeMerlinTransfer(address user) external returns (uint256);
    // function safeMerlinTransferBehalf(address user, address to) external returns (uint);
}
...
interface IStrategy is IVaultController {
    ...
    function withdraw(uint256 _amount) external; // MERL STAKING POOL ONLY
    ...
    function withdrawableBalanceOf(address account)
        external
        view
        returns (uint256); // MERL STAKING POOL ONLY
...
/**
 * @dev Implementation of the {MERLToken}.
 */
contract MERLToken is BEP20("Merlin Token", "MERL") {
    constructor() public {
        mint(1e8 * 1e18);
    }
}
...
library PoolConstant {
    enum PoolTypes {
        ...
        CakeStake,
        FlipToFlip,
        FlipToCake, // CakeStake => CakeToCake
