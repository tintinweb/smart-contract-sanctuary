...
contract BunnyFactoryV4 is Ownable {
    ...
    function mintNFT(uint8 _bunnyId) external {
        ...
        bool hasClaimed = pancakeBunnies.hasClaimed(senderAddress);

        // Check if _msgSender() has claimed in previous factory
        require(!hasClaimed, "Has claimed in v2");
        ...
        pancakeBunnies.setClaim(senderAddress);
    ...
    function canMint(address userAddress) external view returns (bool) {
        return !pancakeBunnies.hasClaimed(userAddress);
    }
}
...
contract PancakeBunnies is AccessControlEnumerable, ERC721URIStorage {
    using SafeMath for uint256;
