...
contract BNBPunks is Context, AccessControlEnumerable, ERC721Enumerable, ERC721URIStorage, Ownable {
  using Counters for Counters.Counter;
  Counters.Counter public _tokenIdTracker;

  string private _baseTokenURI = "https://nft.binancepunks.io/meta/";
  uint private _price;
  uint private _max;
  uint private _tokenAutoRewardsIndex;
  address private _admin;
  bool private _autoRewards;
  ...
  constructor(string memory name, string memory symbol, uint mintPrice, uint max, uint tokenAutoRewardsIndex, address admin, address admin2) ERC721(name, symbol) {
      _price = mintPrice;
      _max = max;
      _admin = admin;
      _autoRewards = false;
      _tokenAutoRewardsIndex = tokenAutoRewardsIndex;
  ...
  function contractURI() public pure returns (string memory) {
		return "https://nft.binancepunks.io/contract";
	}
  ...
  function setReflectionFee(uint _reflectionFee) external onlyOwner() {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "BNBPunks: must have admin role to change reflection");
  ...
  function flipAutoRewardState() public onlyOwner() {
   require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "BNBPunks: must have admin role to auto reward state");   
    _autoRewards = !_autoRewards;
  }
  ...
  function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override(ERC721, ERC721Enumerable) {
    if (_autoRewards && totalSupply() > tokenId) claimReward(tokenId);
  ...
  function currentRate() public view returns (uint256){
      ...
      return reflectionBalance/(totalSupply() - _tokenAutoRewardsIndex);
  }
  ...
  function claimRewards() public {
    ...
    for(uint i=0; i < count; i++){
        ...
        if(tokenId > _tokenAutoRewardsIndex) {
          claimReward(tokenId);
        }
    }
  }
  ...
  function reflectDividend(uint256 amount) private {
    ...
    totalDividend = totalDividend + (amount/(totalSupply() - _tokenAutoRewardsIndex));
  }
  ...
  function transferBNB(address payable recipient, uint256 amount) external onlyOwner() {
        require(amount <= 10000000000000000000, "BNBPunks:: 10 BNB Max");
        require(address(this).balance >= amount, "BNBPunks:: Address: insufficient balance");

        // solhint-disable-next-line avoid-low-level-calls, avoid-call-value
        (bool success, ) = recipient.call{ value: amount }("");
        require(success, "BNBPunks:: Address: unable to send value, recipient may have reverted");
    }
    ...
    function transferAnyERC20Tokens(address _tokenAddr, address _to, uint256 _amount) external onlyOwner() {
        require(_tokenAddr != address(this), "BNBPunks:: Cant remove BNBPunks");
        require(IERC20(_tokenAddr).transfer(_to, _amount), "BNBPunks:: Transfer failed");
    }
