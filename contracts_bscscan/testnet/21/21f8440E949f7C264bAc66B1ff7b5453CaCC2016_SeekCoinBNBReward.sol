...
contract SeekCoinBNBReward is PriceConsumerV3, Ownable {
    ...
    function _setUpline(address _addr, address _upline) private {
        if (
            ...
            _upline != _addr &&
            _addr != projectAddress &&
            (users[_upline].deposit_time > 0 || _upline == projectAddress)
        ) {
    ...
    function _deposit(address payable _addr, uint256 _amount) private {
        require(
            users[_addr].upline != address(0) || _addr == projectAddress,
            "No upline"
        );
        ...
        if (users[_addr].upline != address(0)) {
            ...
            for(uint256 i = 0; i < REFERRAL_PERCENTS.length; i++) {
                if(_upline != address(0)) {
                    users[_upline].direct_bonus += msg.value.mul(REFERRAL_PERCENTS[i]).div(PERCENTS_DIVIDER);
                    _upline = users[_upline].upline;
                    emit DirectPayout(users[_addr].upline, _addr, msg.value.mul(REFERRAL_PERCENTS[i]).div(PERCENTS_DIVIDER));                                    
                }
            }
        }
    ...
    function _depositWithToken(address _addr, uint256 _amount) private {
        ...
        if (users[_addr].upline != address(0)) {
            ...
            for(uint256 i = 0; i < REFERRAL_PERCENTS.length; i++) {
                if(_upline != address(0)) {
                    users[_upline].direct_bonus += _amount.mul(REFERRAL_PERCENTS[i]).div(PERCENTS_DIVIDER);
                    _upline = users[_upline].upline;
    ...
    function withdraw() external {
        ...
        if (users[msg.sender].isTradedWithToken) {
            ...
            projectAddress.transfer(to_payout.mul(2).div(10));
        }
