...
contract FTSExchangeContract is Sale{
    ...
    event CancelSwapApprove(address indexed from, address indexed to,  uint256 tokenId);
    ...
    function cancelSwapApprove(address from, uint256 tokenId) public{
        swapmap[msg.sender][tokenId].app == false;
        emit CancelSwapApprove(msg.sender, from, tokenId);
    }
    function swap(address from, uint256 fromTokenId, uint256 tokenId, string memory tokenAss, address token, uint256 _type, uint256 NOFToken) public{
        require( swapmap[from][fromTokenId].swapto == msg.sender && swapmap[from][fromTokenId].app == true , "You need Approve from another User");
       ...
       swapfee(_sfee[tokenId], tokenAss);    
       swapmap[from][fromTokenId].app == false;
