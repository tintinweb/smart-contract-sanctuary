...
contract KickToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    KickTokenDividendTracker public dividendTracker;

    address public liquidityWallet;

    address payable public operations1Address =
        payable(0x621faC073b4356ccc92618fe7D6Bb037677DBDF2); // devi 1
    address payable public operations2Address =
        payable(0xBdb35d486cd16481FB93067Ef3C4EBA630b57e06); // dev 2
    ...
    constructor() ERC20("KickToken", "KKT") {
        dividendTracker = new KickTokenDividendTracker(payable(this));

        liquidityWallet = owner();
        // testnet Pancakeswap V2 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
		// Mainnet Pancakeswap V2 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        KickTokenDividendTracker newDividendTracker = KickTokenDividendTracker(
            payable(newAddress)
        );
...
contract KickTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "KickToken_Dividend_Tracker",
            "KickToken_Dividend_Tracker"
        )
    {
        kickToken = KickToken(mainContract);
    ...
    function _transfer(
        ...
        require(false, "KickToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "KickToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main KickToken contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(kickToken);

            uint256 prevBalance = kickToken.balanceOf(address(this));
            ...
            {
                uint256 received = kickToken.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    kickToken.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
