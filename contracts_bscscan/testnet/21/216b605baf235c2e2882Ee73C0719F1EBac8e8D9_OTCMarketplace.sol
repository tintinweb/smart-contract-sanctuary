...
contract OTCMarketplace is ERC721Holder, AccessControl, ReentrancyGuard {
  ...
  /**
   * @dev Stores the contract, and allows users with the admin role to
   * grant/revoke the admin role from other users. Stores treasury wallet.
   *
   * Params:
   * _MFRMTokenAddress: the address of the contract
   ...
   * Params:
   * label: listing label
   * nftTokenAddress: address of token to list
   * nftTokenId: id of token
   * nftNumTokens: number of tokens
   * mfrmPrice: listing mfrmPrice i.e. 10 MFRM
   */
  function listOTCTokens(
    string calldata listingTitle,
    address nftTokenAddress,
    address paymentTokenAddress,
    uint256 paymentAmount,
    uint256 nftTokenId,
