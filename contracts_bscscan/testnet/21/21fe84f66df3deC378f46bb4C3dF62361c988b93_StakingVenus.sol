...
contract StakingVenus {
    ...
    constructor(
        ...
        address[] memory vTokenAddresses = new address[](2);
        vTokenAddresses[0] = 0x488aB2826a154da01CC4CC16A8C83d4720D3cA2C;
        vTokenAddresses[1] = 0x08e0A5575De71037aE36AbfAfb516595fE68e5e4;
        comptroller = _comptroller;
        ComptrollerInterface(comptroller).enterMarkets(vTokenAddresses);
        entered = false;
        // Track the contract owner
        owner = msg.sender;
    }

    function enterToVTokenMarkets(address[] memory _vTokenAddresses) public onlyOwner {
        ComptrollerInterface(comptroller).enterMarkets(_vTokenAddresses);
    }

    // // delete if no need
    // function exitFromVTokenMarket(address _vTokenAddress) public onlyOwner {
    //     ComptrollerInterface(comptroller).exitMarket(_vTokenAddress);
    // }
     ...
     */
    function deposit(VBep20Interface _vToken, IBEP20 _token, uint256 _tokenAmount) public onlyOwner {
    ...
    function claimVTokens(VBep20Interface _vToken, IBEP20 _token, uint256 _vTokenAmount) public onlyOwner {
    ...
    function claimTokens(VBep20Interface _vToken, IBEP20 _token, uint256 _tokenAmount) public onlyOwner {
        require(!entered, "TurboSwap: reentrant call");
        entered = true;
        uint256 vbusdTotal = VTokenInterface(address(_vToken)).balanceOf(address(this));
