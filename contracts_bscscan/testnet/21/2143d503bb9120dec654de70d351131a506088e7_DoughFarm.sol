...
contract DoughFarm is Ownable {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate, uint256 _doughPerBlock) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function pendingDough(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 doughReward = multiplier.mul(pool.doughPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
