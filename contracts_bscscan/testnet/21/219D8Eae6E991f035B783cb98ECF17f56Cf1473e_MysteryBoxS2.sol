...
contract MysteryBoxS2 is RandomGenerator, Context {
    ...
    constructor(address KTA_, address Msbox_, address U_, address wallet_) {
        ...
        uint[7] memory cardIds_ = [uint(50001), uint(50002), uint(50003), uint(50004), uint(5005), uint(5006), uint(5007)];
        ...
        uint[4] memory secCardIds_ = [uint(50006), uint(50007), uint(50008), uint(0)];
    ...
    function secOpen(uint tokenId_, uint signtime_, bytes32 r, bytes32 s, uint8 v) external openBoxStage returns (uint) {
        uint interval = block.timestamp.sub(signtime_);
        require(interval <= expireTime, "Signature expired");
