    ...
    OwnableUpgradeable
{
        ...
        nonReentrant
    {
        ...
        uint256 amountInCWOLF = Variables(VariablesContractAddress).getDollarsInCWOLF(totalDollars * wolfsInWolfPack[_wolfPackId].length);
    ...
    function decreaseWolfPackLink(uint256 _wolfPackId, uint256 _amountToDecrease) public {
        wolfPackLinkDays[_wolfPackId] = wolfPackLinkDays[_wolfPackId] - _amountToDecrease;
        emit DecreasedLink(_wolfPackId, _amountToDecrease);
    }

    function checkWolfPackLink(uint256 _wolfPackId)
        public
        view
        returns (bool)
    {
    ...
    OwnableUpgradeable
{
        ...
        returns (uint256 commission)
    {
        if (_positionToAttack < 5) {
            ...
            return gasToMinter + ((gasToMinter * 3000) / 10000);
        } else if (_positionToAttack >= 10 && _positionToAttack <= 14) {
            return gasToMinter + ((gasToMinter * 6000) / 10000);
        } else if (_positionToAttack >= 15 && _positionToAttack <= 19) {
            return gasToMinter + ((gasToMinter * 8000) / 10000);
        } else if (_positionToAttack >= 20 && _positionToAttack <= 24) {
            return gasToMinter + ((gasToMinter * 11000) / 10000);
        } else if (_positionToAttack >= 25 && _positionToAttack <= 29) {
            return gasToMinter + ((gasToMinter * 14000) / 10000);
        }
    }
