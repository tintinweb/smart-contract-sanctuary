...
/*
   ...
   4% fee auto add to the burn LP  
   3% fee is reflected to all the holders
   
   Telegram: https://t.me/swiftinu
   Website: https://swiftinu.space/
   Twitter: https://twitter.com/swiftinu
...
contract NukeDoge is Context, IERC20, Ownable {
    ...
    //address payable public _marketingAddress = payable(0x102CFC75257661726d3938972847e8e71776AeAe); // Mainnet
    //address payable public _charityAddress = payable(0x832Abfa44b117AdA4970Ef6733645A8E4d57F86b); // Mainnet
    
    address payable public _marketingAddress = payable(0x3A99f527CC1BAAC09244f152a690B08cCcc9feA9); // Testnet
    address payable public _charityAddress = payable(0xe5505cA0b825d6681761fF42C2357eC75AA378e6); // Testnet
    ...
    bool inProgressSwapAndLiquify;
    bool public swapAndLiquifyEnabled = true;
    
    uint256 public _maxTxAmount = 10 * 10**6 * 10**9;
    uint256 private minimumTokensRequiredForLiquify = 5 * 10**6 * 10**9;
    ...
    modifier lockTheSwap {
        inProgressSwapAndLiquify = true;
        _;
        inProgressSwapAndLiquify = false;
    }
    ...
    /*
     * Reward and fee address management
     */
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!inProgressSwapAndLiquify) {
            _attemptSwapAndLiquify(from);
        }
        _tokenTransferWithNormalFees(from, to, amount);
    }
    ...
    uint8 _liquifyAction = 0; // Cycle through which wallet we attempt to liquify so the gas doesn't murder 1 person
    
    function _attemptSwapAndLiquify(address from) private lockTheSwap {
        bool exchangeHasToken = balanceOf(uniswapV2Pair) > 0;
        if (!swapAndLiquifyEnabled || !exchangeHasToken || from == uniswapV2Pair) {
            return;
        }
    ...
    function _handleLiquiditySwap() private {
        ...
        bool enoughTokensToSell = contractTokenBalance >= minimumTokensRequiredForLiquify;
    ...
    function _handleCharitySwap() private {
        ...
        bool enoughTokensToSell = charityBalance >= minimumTokensRequiredForLiquify;
        if (enoughTokensToSell) {
    ...
    function _handleMarketingSwap() private {
        ...
        bool enoughTokensToSell = marketingBalance >= minimumTokensRequiredForLiquify;
        if (enoughTokensToSell) {
    ...
    // Allow recieve of ETH from uniswapV2Router when swaping
    ...
    function updateActiveFee(address sender, address recipient) private {
        ...
        if (!takeFee) {
            ...
            if(sender == uniswapV2Pair){ // Buy Fees
                removeAllFee();
