...
contract PlayToWinGames {
    ...
    address payable public  owner = 0xC34B7Ac22D49259C98680b377AC4f435Df4A4df4;
    ...
    uint public maxStake = 500 * 10 ** 18;
    ...
    event ConfirmGame(address indexed player, string indexed game, uint profit, bytes choice, uint game_choice,  bool status, bool draw,uint bet);
    event ConfirmGameRolling(address indexed player, string indexed  game, uint profit, bytes min, bytes max, uint game_choice, bool status, bool draw,uint bet);
    ...
    function game_coin(bytes memory _choice, bytes32 seed) public payable returns(uint8) {
        ...
        if(msg.value >= jackpotMinBet){
            jackpotSize += msg.value.div(1000);
        }
        listGames[seed] = Game({
    ...
    function game_knb(bytes memory _choice, bytes32 seed) public payable {
    ...
    function game_dice(bytes memory _choice, bytes32 seed) public  payable {
    ...
    function game_rolling(bytes memory _choice1,bytes memory _choice2, bytes32 seed) public  payable {
        ...
        totalBet += msg.value;
        emit NewGame(msg.sender, seed, msg.value, _choice1, game_title);
    }
    ...
    function confirm(bytes32 seed, bytes32 _s) public onlyCroupier {
        ...
        if (game.game_title == Games.CoinFlip){
            ...
            if(game.result == user_choice){
                ...
                emit ConfirmGame(game.player, 'CoinFlip', profit, game.choice1, game.result, true, false,game.bet);
            }else{
                game.state = GameState.Lose;
                emit ConfirmGame(game.player, 'CoinFlip', 0, game.choice1, game.result, false, false,game.bet);
            }
        //KNB game algorithm
            ...
            if(game.result != user_choice){
                if (user_choice == 1 && game.result == 2 || user_choice == 2 && game.result == 3 || user_choice == 3 && game.result == 1) {
                    ...
                    game.profit = profit;
                    game.player.transfer(profit);
                    emit ConfirmGame(game.player, 'KNB', profit, game.choice1, game.result, true, false, game.bet);
                }
                else{
                    game.state = GameState.Lose;
                    emit ConfirmGame(game.player, 'KNB', 0, game.choice1, game.result, false, false,game.bet);
                }
            }else{
                ...
                emit ConfirmGame(game.player, 'KNB', profit, game.choice1, game.result,  false, true,game.bet);
            }
        //Dice game algorithm
            ...
            if(profit > 0){
                ...
                game.profit = profit;
                game.player.transfer(profit);
                emit ConfirmGame(game.player, 'Dice', profit, game.choice1, game.result, true, false,game.bet);
            }else{
                game.state = GameState.Lose;
                emit ConfirmGame(game.player, 'Dice', 0, game.choice1, game.result, false, false, game.bet);
            }
        }
            ...
            if(profit > 0){
                game.state = GameState.Win;
                game.profit = profit;
                game.player.transfer(profit + game.bet);
                emit ConfirmGameRolling(game.player, 'Rolling', profit, game.choice1, game.choice2, game.result,  true, false,game.bet);
            }else{
                game.state = GameState.Lose;
                emit ConfirmGameRolling(game.player, 'Rolling', 0, game.choice1, game.choice2, game.result, false, false,game.bet);
            }
        }
    ...
    function() external isNotContract contractIsOn payable {
    }

    function addMoney(uint amount, address payable sendTo) external {
