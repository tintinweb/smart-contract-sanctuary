...
contract Starlink is IStarlink, IDepositable, StarlinkComponent, PancakeSwapHelper {
    ...
    function unclaimedDividendsOf(address userAddress, address token) external view returns (uint256) {
        Pool memory pool = pools[poolIndexByTokenAddress[token]];
        UserStaking memory userStake = userPoolStake[userAddress][token];
        return userStake.unclaimedDividends + calculateReward(pool, userStake);
    }

    function totalUnclaimedDividendsOf(address userAddress) external view returns (uint256) {
        uint256 totalUnclaimed;
        for(uint i = 0; i < pools.length; i++) {
            Pool memory pool = pools[poolIndexByTokenAddress[pools[i].token]];
            UserStaking memory userStake = userPoolStake[userAddress][pools[i].token];

            totalUnclaimed += userStake.unclaimedDividends + calculateReward(pool, userStake);
        }
    ...
    function totalEarnedBy(address userAddress) external view returns (uint256) {
        ...
        for(uint i = 0; i < pools.length; i++) {
            ...
            totalEarned += userStake.earned + calculateReward(pool, userStake);
        }
        
        return totalEarned;
    }

    function calculateReward(Pool memory pool, UserStaking memory userStake) private view returns (uint256) {
