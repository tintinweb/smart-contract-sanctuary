...
contract LotteryPricePool is Ownable,Pausable{
    ...
    constructor(
        ...
        require(_prizePeriodStart > 0,"LotteryPricePool: _prizePeriodStart must be greater than zero");
        require(_rewardToken != address(0) && address(_ticket) != address(0),"LotteryPricePool: address must be not zero");
    ...
    function changePrizePeriod(uint _prizePeriodSeconds) public onlyOwner(){
        require(_prizePeriodSeconds > 0,"LotteryPricePool: _prizePeriodSeconds must be greater than zero");
        require(_prizePeriodSeconds != prizePeriodSeconds,"LotteryPricePool: _prizePeriodSeconds must be different");
        prizePeriodSeconds = _prizePeriodSeconds;
        emit ChangePrizePeriod(_prizePeriodSeconds);
    }
    ...
    function _distribute(uint256 randomNumber) internal  {
        require(winnerAmount != 0,"LotteryPricePool: Winner Amount is Zero");
        require(IERC20(address(ticket)).totalSupply() != 0,"LotteryPricePool: total supply of ticket is low");
    ...
    function changeClientSeeds(string memory _seed) public {
        require(prizePeriodStartedAt.add(prizePeriodSeconds) > block.timestamp,"LotteryPricePool: only before lottey distribution start");
        _clientSeeds = _seed;
    ...
    function notifyRewardAmount(uint256 reward) external onlyOwner() {
        ...
        winnerAmount = winnerAmount.add(reward);
    ...
    modifier requireCanStartAward() {
      require(_isPrizePeriodOver(), "LotteryPricePool: lottery prize-period-not-over");
      _;
    }
    ...
    event ChangePrizePeriod(uint256 _prizePeriodSeconds);
