...
contract Whot is ERC20Burnable, RandomNumberConsumer, CardEngine {
    ...
    mapping(address => RandReq) randRequests;

    address public gWhotAddr;
    uint256 public gWhotMintRate;
    ...
    event BurnForGWHOT(
        ...
        uint256 indexed amountWHOT,
        uint256 indexed amountGWHOT
    );
        ...
        )
    {
        ...
        gWhotAddr = _gWhotAddr;
        gWhotMintRate = _gWhotMintRate;
    ...
    /// @notice set the GWHOT mint rate
    /// @param rate is the conversion rate from WHOT to GWHOT.
    function setGWhotMintRate(uint256 rate) public onlyDeployer {
        gWhotMintRate = rate;
    }
    ...
    /// @notice burn WHOT to get GWHOT
    /// @param amount is the amount of WHOT to burn
    function burnForGWhot(uint256 amount) public {
        burn(amount);
        uint256 gWhotAmt = amount.mul(gWhotMintRate).div(10**decimals());
        GWhot gWhot = GWhot(gWhotAddr);
        gWhot.mint(msg.sender, gWhotAmt);
        emit BurnForGWHOT(msg.sender, amount, gWhotAmt);
    }
    ...
    /// - The player must have sufficient GWHOT to burn.
    function enter() public {
        require(playerPoolIndex[msg.sender] == 0, "Sender cant be in a pool");
        require(matchIndex[msg.sender] == 0, "Sender cant be in a match");

        // Burn the entry GWHOT
        if (matchEntryFee > 0) {
            GWhot gWhot = GWhot(gWhotAddr);
            gWhot.burnFrom(msg.sender, matchEntryFee);
        }

        playerPool.push(Player(block.number, msg.sender, 0, matchEntryFee));
    ...
    /// @dev exit the pool before matched with someone.
    ...
    function exit() public {
        ...
        gWhot.mint(
            player.addr,
            player.entryFee.sub(player.entryFee.mul(exitSlashPct).div(100))
        );
    ...
    function deleteMatch(uint256 matchIdx) internal {
        Match memory m = matches[matchIdx];
        if (matches.length > 1) {
            Match memory last = matches[matches.length - 1];
            matches[matchIdx] = last;
        }
        matches.pop();
        delete matchIndex[m.playerA.addr];
        delete matchIndex[m.playerB.addr];
    }
    ...
    /// @dev returns entry GWHOT fee back to loser.
    ...
    function endMatch(uint256 matchIdx) public {
        require(matchIdx <= matches.length, "Match must exist");
        Match memory m = matches[matchIdx - 1];
        require(
        ...
        // If `turn` is zero, return player's GWHOT - exit slash fee.
