...
contract SCU is Context, IERC20, Ownable {
	...
	address payable private _buybackWallet = payable(0x8392283922bB78EbD8767c9cF462eC8dEA62F0fa);

    string private _name = "Shark Cuban";
    string private _symbol = "SCU";
	...
	function _getbuybackWallet() public view virtual returns (address) {
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBuyback, uint256 tDead) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tBuyback, data.tDead);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tBuyback).sub(tDead);
        return (tTransferAmount, TData(0, tFee, tLiquidity, tBuyback, tDead, 0));
    }
    ...
    function _getRValues( TData memory _data) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rBuyback = _data.tBuyback.mul(_data.currentRate);
		uint256 rDead = _data.tDead.mul(_data.currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rBuyback).sub(rDead);
	...
	function _takeBuyback(uint256 tBuyback) private {
        ...
        if (_shouldSwapToBNB) {
            ...
            if(_isExcluded[_buybackWallet])
                _tOwned[_buybackWallet] = _tOwned[_buybackWallet].add(tBuyback);
        }
	}
