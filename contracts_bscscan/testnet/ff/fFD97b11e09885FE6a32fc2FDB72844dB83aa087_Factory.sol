...
import "./PlayingCard.sol";

contract Factory is Ownable {

    event CardTokenCreated(address tokenAddress);

    address[] gameCardAddresses;

    constructor() {
        
    }

    function deployNewGameCardContract(string memory _name, string memory _symbol,string memory _baseURI,uint[] memory _counters) onlyOwner()
        external
        returns (address)
    {
        require(bytes(_name).length != 0,"Factory: Use valid _name");
        require(bytes(_symbol).length != 0,"Factory: Use valid _symbol");
        Card playCard = new Card(_name, _symbol,_baseURI,_counters);
        playCard.transferOwnership(msg.sender);
        emit CardTokenCreated(address(playCard));
        gameCardAddresses.push(address(playCard));
        return address(playCard);
    }

    function getAllGameCardAddress() external view returns(address[] memory){
        return gameCardAddresses;
    }

    function getGameCardAddress(uint _gameId) external view returns(address){
        return gameCardAddresses[_gameId];
    }
}
