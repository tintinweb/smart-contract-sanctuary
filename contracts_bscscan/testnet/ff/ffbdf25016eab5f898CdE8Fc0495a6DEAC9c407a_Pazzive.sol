...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    address public immutable BUSD = address(0x8301F2213c0eeD49a7E28Ae4c3e91722919B8B47);
...
contract Pazzive is ERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x87500968B83f3f7091B85ea58dAaBc815935b553);

    // 1 million being the smallest antiwhale
    uint256 public MIN_MAX_SELL_AMOUNT = 1000000 * DECIMALS;
    uint256 public maxSellAmount = 0;
    uint256 public swapTokensAtAmount = 200000000 * DECIMALS;
    ...
    */
    ...
    constructor(address routerAddress, address busdAddress) ERC20("Pazzive", "PAZ") {
        dividendTracker = new PazDividendTracker(payable(this), routerAddress);
    ...
    */
    ...
    function setSwapPair(address pair, bool isSwapPair) public onlyOwner {
        require(swapPairs[pair] != isSwapPair, "Pazzive: already set");
        swapPairs[pair] = isSwapPair;
        ...
        emit SetSwapPair(pair, isSwapPair);
    }
    ...
    //    function updateFees(uint256 dev, uint256 liquidity, uint256 rewardBuy, uint256 rewardSell) public onlyOwner {
    //        require(dev <= 5, "operation fees can't exceed 5%");
    //        require(liquidity <= 5, "liquidity fees can't exceed 5%");
    //        require(rewardBuy >= 5 && rewardBuy <= 25, "Pazzive: reward fees must be between 5 and 25");
    //        require(rewardSell >= 5 && rewardSell <= 25, "Pazzive: reward fees must be between 5 and 25");
    //
    //        teamFee = dev;
    //        liquidityFee = liquidity;
    //        rewardBuyFee = rewardBuy;
    //        rewardSellFee = rewardSell;
    //
    //        totalSellFees = rewardSellFee.add(liquidityFee).add(teamFee);
    //        totalBuyFees = rewardBuyFee.add(liquidityFee).add(teamFee);
    //
    //        emit UpdateFees(dev, liquidity, rewardBuy, rewardSell);
    //    }
    ...
    */
    ...
    function antiWhaleCheck(address from, address to, uint256 amount) internal view {
        // 1. antiwhale on 2. is Sell 3. is not remove liquidity
        if (maxSellAmount != 0 && swapPairs[to] && routers[from]) {
            require(amount <= maxSellAmount, "Pazzive: amount greater than maxSellTransactionAmount");
        }
    }
    ...
    function transferWithFee(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        handleStaking(from);
        uint256 totalFees = getTotalFee(isSell);
        ...
        if (canSwap && !swapPairs[from]) {
            ...
            if (swapAndLiquifyEnabled) {
                uint256 amountToSwap = contractTokenBalance.mul(liquidityFee).div(totalBuyFee);
    ...
    function takeFee(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        uint256 feeAmount = amount.mul(getTotalFee(isSell(to))).div(feeDenominator);
...
//        return totalFee.add(feeIncrease.mul(remainingTime).div(buybackMultiplierLength));
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 teamAmount = tokens.mul(teamFee).div(feeDenominator);
        uint256 dividends = tokens.sub(teamAmount);
        ...
        emit SendDividends(dividends, teamAmount, success && successDev && successMarketing);
    }
...
contract PazDividendTracker is DividendPayingToken, Ownable {
    ...
    uint256 public DECIMALS = 10 ** 9;
    uint256 public minimumTokenBalanceForAutoDividends = 10000000 * DECIMALS;
    uint256 public minimumTokenBalanceForDividends = 1000 * DECIMALS;
    ...
    constructor(address payable pazziveAddress, address routerAddress)DividendPayingToken("PazDividendTracker", "PazDividendTracker"){
...
//    }
    ...
    function processAccount(address payable account, bool automatic) public onlyOwner returns (bool){
        ...
        // add restrictions
        // 1. only send if over 1 BUSD
        // 2. only send if over 1 hour
    ...
    function setUserRewardToken(address account, address token) public onlyOwner {
        require(allowedTokens[token] || token == address(0), "Token not in allow list");
    ...
    function isCustomRouter(address user) internal returns (bool) {
        return tokenRouter[userRewardToken[user]] != address(router);
    }

    // Customized function to send tokens to dividend recipients
    // todo: this will be changed to swapping BUSD for tokens
    function swapETHForTokens(address recipient, uint256 ethAmount) private returns (uint256) {
        bool swapSuccess;
        IERC20 token = IERC20(userRewardToken[recipient]);
        ISwapRouter swapRouter = isCustomRouter(recipient) ? router : ISwapRouter(tokenRouter[address(token)]);
    ...
    function _withdrawDividendOfUser(address payable user) internal override returns (uint256){
        ...
        if (_withdrawableDividend > 0) {
            ...
            if (!allowCustomTokens || tokenAddress == address(0) || !allowedTokens[tokenAddress]) {
                ...
                try
                router.swapExactETHForTokensSupportingFeeOnTransferTokens{value : _withdrawableDividend}(0, path, user, block.timestamp)
