...
contract CCL is Context, Ownable, ReentrancyGuard {
 ...
 //  uint256 private BNBCost; // Fees in BNB
    ...
    address[] public requestedAddrList;     // Applied addresses array [Public Access]
    address[] public verifiedWallets;   // Verified wallets array [Wallets verificaton only] [Public Access]
    address[] public verifiedContracts; // Verified contracts wallet [Contracts and businesses only] [Public Access]
    address[] public scamAddresses;     // Scam addresses list [Public Access]
    ...
    function payInLTS() private {
        require(LTSToken.balanceOf(_msgSender()) >= LTSCost * (10 ** 18), "Insufficient LTS Balance");
            TransferHelper.safeTransfer(address(LTSToken), ltsCCLTransactions, LTSCost * (10 ** 18));
    }

    function payInBNB() private {
        require(BNBToken.balanceOf(_msgSender()) >= BNBCost * (10 ** 18), "Insufficient BNB Balance");
            TransferHelper.safeTransfer(address(BNBToken), bnbCCLTransactions, BNBCost * (10 ** 18));
    } 
}