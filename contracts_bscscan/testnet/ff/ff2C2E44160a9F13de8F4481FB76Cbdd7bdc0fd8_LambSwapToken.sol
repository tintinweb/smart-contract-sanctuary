...
contract LambSwapToken is  Context, Ownable  {
	...
	uint256 public _maxSupply = 500e9 * 1e9;
    ...
    /* --------- max tx info --------- */
	uint public _limitTxAmount = 500e9 * 1e9;
	uint public numTokensSellToAddToLiquidity = 39e2 * 1e9;
    ...
    constructor (address _RouterAddress) public {
        ...
        _symbol = "LST";
        _decimals = 9;
        _totalSupply = 39e9*1e9; /// initial supply 1000,000,000,000,000
	...
	function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		// transfer 
		if((sender == PancakeSwapV2Pair || recipient == PancakeSwapV2Pair )&& !isWhiteList[sender])
		...
		if(!isWhiteList[sender]) {
			if(sender == PancakeSwapV2Pair){
                ...
                _burn(msg.sender,amount.mul(buyFees.burn).div(1000));
                ...
                _burn(msg.sender,amount.mul(sellFees.burn).div(1000));
