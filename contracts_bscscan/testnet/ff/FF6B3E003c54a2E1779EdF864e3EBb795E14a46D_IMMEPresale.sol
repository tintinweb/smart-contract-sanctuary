...
contract IMMEPresale is ReentrancyGuard {
	...
	// Price of presale IMME, 10 BUSD
	uint256 BUSDPerToken = 7 * 10 ** 15;
	// Amount of BUSD received in presale
	uint256 busdReceived = 0;

    uint256 HardCap = 14 * 10 ** 5 * 10 ** 18;

    uint256 public mincontributionLimit = 1000 * 10 ** 18;
    uint256 public maxcontributionLimit = 10000 * 10 ** 18;
	...
	function claimTokens() external nonReentrant {
		...
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed IMME tokens");
		require(IMME.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough IMME tokens to transfer.");
