...
// File: contracts/BSC/SigTokenBsc.sol
...
// File: contracts/BSC/SigMasterChefBsc.sol
...
contract SigMasterChefBsc is Ownable {
    using SafeMath for uint256;
    ...
    // The SUSHI TOKEN!
    SigTokenBsc public immutable SIG_TOKEN;
...
//    address public sharedVault;
    ...
    // because BSC rewards are coming from Ethereum mint of [email protected]
    // we have to know the part of the rewards got frozen on ethereum to re-mint it here.
    uint256 public bscMintFraction1000 = 200;
    ...
    function totalRewardAtBlock(uint256 atBlock) public view returns (uint256) {
        ...
        return SIG_TOKEN.maxRewardMintAfterBlocks(t);
    }
...
//        sharedVault = _sharedVault;
    ...
    function amountOfBscMint(uint256 amount) public view returns(uint256) {
        return amount.mul(bscMintFraction1000).div(1000);
    }

    function changeBscMintFraction1000(uint256 newBscMintFraction1000) external onlyOwner {
        bscMintFraction1000 = newBscMintFraction1000;
    }
    ...
    function pendingSushi(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return amountOfBscMint(userAmountBeforeTax);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        SIG_TOKEN.mint(address(this), amountOfBscMint(totalSushiRewardForPool));
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = amountOfBscMint(user.amount.mul(pool.accSushiPerShare).div(1e12));
    ...
    function safeSushiTransfer(address _to, uint256 _amount) internal {
        uint256 sushiBal = SIG_TOKEN.balanceOf(address(this));
        if (_amount > sushiBal) {
            SIG_TOKEN.transfer(_to, sushiBal);
        } else {
            SIG_TOKEN.transfer(_to, _amount);
        }
    }
