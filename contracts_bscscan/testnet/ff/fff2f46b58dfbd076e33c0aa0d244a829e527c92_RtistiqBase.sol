...
contract RtistiqBase is Ownable, Pausable {
    ...
    function burnToken(uint256 _tokenId) public onlyTokenContract whenNotPaused {
        ...
        _holderTokens[arts[_tokenId].owner].remove(_tokenId);
    ...
    */
    ...
    function getBalance(address owner) public view returns (uint256 count) {
 ...
 * @dev see https://eips.ethereum.org/EIPS/eip-721
 */
contract RtistiqToken is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable, Ownable, Pausable {
    using SafeMath for uint256;
    using Address for address;
    using EnumerableSet for EnumerableSet.UintSet;
    using EnumerableMap for EnumerableMap.UintToAddressMap;
...
/**
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual override returns (uint256) {
        return rtistiqBase.getTokenOfOwnerByIndex(owner, index);
    }
    ...
    function totalSupply() public view virtual override returns (uint256) {
        // _tokenOwners are indexed by tokenIds, so .length() returns the number of tokenIds
        return rtistiqBase.getTotalSupply();
    }
    ...
    function tokenByIndex(uint256 index) public view virtual override returns (uint256) {
        return rtistiqBase.getTokenByIndex(index);
    }

    function _mint(address to, string memory metadata) internal returns (uint256) {
        uint256 tokenId = rtistiqBase.mintToken(to, metadata);
...
contract RtistiqCore is RtistiqToken {
  ...
  constructor() public RtistiqToken("RtistiqDigitalTwin", "RDT", "https://gateway.pinata.cloud/ipfs/") {}
  ...
  function createArtworkToken(bytes24 _message,
    ...
    require(verifySignature(_isWeb3Signature,_signer,_message, _signature), "Signature Verification failed");

    uint256 tokenId = _mint(_recipient,_metadata);
    return tokenId;
  }
  ...
  function mint(
    ...
    uint256 tokenId = _mint(_to,_metadata);
    return tokenId;
  }
