...
interface IPreDoveToken {
  function burnFrom(address account_, uint256 amount_) external;
}

contract ExercisePreDove is Ownable {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  address public immutable pDOVE;
  ...
  // Allows wallet to redeem pDOVE for DOVE
  function exercise(uint256 _amount) external returns (bool) {
    ...
    IPreDoveToken(pDOVE).burnFrom(msg.sender, _amount);

    IERC20(USDC).approve(treasury, _amount);
    uint256 DOVEToSend = ITreasury(treasury).deposit(_amount, USDC, 0);

    terms[msg.sender].claimed = info.claimed.add(_amount);

    IERC20(DOVE).safeTransfer(msg.sender, DOVEToSend);
  ...
  function DOVECirculatingSupply() external view returns (uint256) {
    uint256 _totalSupply = IERC20(DOVE).totalSupply();
  ...
  function redeemable(Term memory _info) internal view returns (uint256) {
    return
      (this.DOVECirculatingSupply().mul(_info.percent).mul(1000)).sub(
        _info.claimed
      );
  }
}