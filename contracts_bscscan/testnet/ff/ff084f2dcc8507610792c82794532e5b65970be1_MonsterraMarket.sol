...
import "../interfaces/IMonsterraPayment.sol";
    ...
    ERC721HolderUpgradeable
{
    ...
    address public MonsterraPaymentContract;
    ...
    modifier validAmount(
        ...
        require(
            _validAmount(_orderId, _quantity, _paymentAmount)
        );
        _;
    }
    ...
    function _validAmount(
        ...
        uint256 buyAmount = order.price.mul(_quantity);
        return
            (_paymentAmount >= buyAmount)
                ? true
                : false;
    }
    ...
    function _match(
        ...
        amountToSeller = amountToSeller.sub(orderAmount.mul(ZOOM_FEE - contractFee).div(ZOOM_FEE));
    ...
    function createOrder(
        ...
        bool isSupportContract = IMonsterraPayment(MonsterraPaymentContract).isContractSupport(
            _tokenAddress
        );
        ...
        if (isERC721) {
            ...
            balance = IERC1155Upgradeable(_tokenAddress).balanceOf(
                msg.sender,
                _tokenId
            );
            require(balance >= _quantity, "Insufficient-token-balance");
        ...
        bool isSupportPayment = IMonsterraPayment(MonsterraPaymentContract).isTokenSupport(
            _tokenAddress,
            _paymentToken
        );
    ...
    function buy(uint256 _orderId, uint256 _quantity)
        external
        whenNotPaused
        returns (bool)
    {
        ...
        uint256 discount = IMonsterraPayment(MonsterraPaymentContract).getTokenDiscount(order.tokenAddress, order.paymentToken);
        exactPaymentAmount = orderAmount.mul(ZOOM_FEE - discount).div(ZOOM_FEE);
        ...
        uint256 contractFee = IMonsterraPayment(MonsterraPaymentContract).getContractFee(order.tokenAddress);
        emit Buy(_orderId, _quantity, order.tokenAddress, exactPaymentAmount);
        return
            _match(
                msg.sender,
                order.tokenAddress,
    ...
    function cancelOrder(uint256 _orderId) external whenNotPaused() {
		Order memory order = orders[_orderId];
		require(order.owner == msg.sender && order.isOnsale, "Wrong-order-owner-or-cancelled");
		if (order.isERC721) {
			IERC721Upgradeable(order.tokenAddress).safeTransferFrom(address(this), order.owner, order.tokenId);
		} else {
			IERC1155Upgradeable(order.tokenAddress).safeTransferFrom(
				address(this),
				order.owner,
    ...
    function setMonsterraPaymentContract(address _monsterraPaymentContract)
        external
        onlyOwner
    {
        MonsterraPaymentContract = _monsterraPaymentContract;
    }
}
...
interface IMonsterraPayment {
	function isTokenSupport(address _contractAddress, address _paymentAddress) external view returns (bool);
    function isContractSupport(address _contractAddress) external view returns (bool);
    function getContractFee(address _contractAddress) external view returns (uint256);
    function getTokenDiscount(address _contractAddress, address _paymentAddress) external view returns (uint256);
}
