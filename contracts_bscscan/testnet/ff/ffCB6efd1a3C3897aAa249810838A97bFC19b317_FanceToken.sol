...
/**
 * @title FanceToken
 * @dev Implementation of the FanceToken
 */
contract FanceToken is BEP20Capped, BEP20Mintable, BEP20Burnable {
    string private t_name = "Freedom Finance";
    string private t_symbol = "FANCE";
    uint8 private t_decimals = 18;
    uint256 private t_cap = 100000000000000000000000000;
    uint256 private t_initialBalance = 20000000000000000000000000;

    uint public constant duration = 2 hours;
    uint public end;
    address public tx_fee_wallet;
    address public dev_wallet = 0xcb9a1AF13e3F0D90cCc381eB1d1C47a0e502a8E9;
    address public marketing_wallet = 0x82DDc6c63a28Cb0FA5723Eb0a865e15f04c4f3c1;

    uint public max_fee = 90000000;
    uint public development_fee = 4000000;
    uint public marketing_fee = 6000000;
    ...
    constructor ()
        BEP20(t_name, t_symbol)
        BEP20Capped(t_cap)
        payable
    {
        _setupDecimals(t_decimals);
        _mint(_msgSender(), t_initialBalance);
        end = block.timestamp + 0 hours;
        tx_fee_wallet = address(msg.sender);
    }
    ...
    /**
     * @dev Function to set transaction fee wallet.
     * @param mfee max transaction fee in first 2 hours
     */
    function setMaximumFee(uint mfee) public onlyOwner {
        if(mfee > 100000000 - development_fee - marketing_fee) {
            max_fee = 100000000 - development_fee - marketing_fee;
        } else {
            max_fee = mfee;
        }
        if(max_fee < 0)
    ...
    /**
     * @dev Function to add address to _pairList
     * @param pairAddress address
     */
    function addPancakePairAddress(address pairAddress) public onlyOwner {
        _pairList[pairAddress] = true;
    }
     ...
     * @dev Function to set transaction fee wallet.
     */
    function startSaleLock() public onlyOwner {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        uint development_fee_amount = calculateDevFee(amount);
        uint marketing_fee_amount = calculateMarketingFee(amount);
        uint max_fee_amount = calculateMaxFee(amount);
        uint r_amount = amount - development_fee_amount - marketing_fee_amount;

        if(block.timestamp < end && _pairList[recipient]) {
            r_amount = amount - development_fee_amount - marketing_fee_amount - max_fee_amount;
            super.transferFrom(sender, recipient, r_amount);
            super.transferFrom(sender, tx_fee_wallet, max_fee_amount);
        } else {
            super.transferFrom(sender, recipient, r_amount);
        }
            
        super.transferFrom(sender, dev_wallet, development_fee_amount);
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if(block.timestamp < end && _pairList[recipient]) {
            ...
            super.transfer(tx_fee_wallet, max_fee_amount);
        } else {
            super.transfer(recipient, r_amount);
        }
        super.transfer(dev_wallet, development_fee_amount);
    ...
    function calculateDevFee(uint256 amount) internal view returns (uint256) {
        return amount * development_fee / (10 ** transaction_fee_decimal);
    }

    /**
     * @dev Function to set transaction marketing fee.
     * @param amount transaction amount
     */
    function calculateMarketingFee(uint256 amount) internal view returns (uint256) {
        return amount * marketing_fee / (10 ** transaction_fee_decimal);
    }

    /**
     * @dev Function to set transaction maximum fee.
     * @param amount transaction amount
     */
    function calculateMaxFee(uint256 amount) internal view returns (uint256) {
        return amount * max_fee / (10 ** transaction_fee_decimal);
    }
}
