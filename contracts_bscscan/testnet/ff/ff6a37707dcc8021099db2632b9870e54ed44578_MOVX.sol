...
contract MOVX is Context, IBEP20, Ownable {
    ...
    uint256 private taxFee = 1; // Fee that gets re-distributed to holders.
    ...
    uint256 public _sellMaxTxAmount2 = 400000 * 10**9;
    ...
    function setMaxTxAmount(uint256 buyTxAmount, uint256 sellTxAmount, uint256 sellTxAmount2, uint256 maxWalletAmount) external onlyOwner() {
    ...
    function _transfer(
        ...
        if(!(_isExcludeAddressFromMaxTx[from] || _isExcludeAddressFromMaxTx[to])) {
            ...
            if((from == pancakeSwapV2Pair) &&  from != owner() && to != owner()) {
            ...
            if (isSelling && from != owner() && to != owner()) {
                require(amount <= _sellMaxTxAmount2, "maxSellTransactionAmount.");
            }

            if (_sellMaxTxAmount != 0 && isSelling && from != address(pancakeSwapV2Router)) { //router -> pair is removing liquidity which shouldn't have max
                require(amount <= _sellMaxTxAmount, "MaxSellTransactionAmount.");
            }
        }
