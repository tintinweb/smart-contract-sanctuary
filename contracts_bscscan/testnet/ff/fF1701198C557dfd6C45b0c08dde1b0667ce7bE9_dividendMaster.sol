 ...
 contract dividendMaster is Ownable, ReentrancyGuard{
    ...
    */
    function distributeDividend() external nonReentrant onlyOwner {
        ...
        uint256 allowAmount = rewardToken.allowance(address(this),address(xdvdToken));
        if(allowAmount == 0){
            rewardToken.safeApprove(address(xdvdToken), _dvdtodist);
        }
        else{
            if(allowAmount < _dvdtodist){
                rewardToken.safeIncreaseAllowance(address(xdvdToken), _dvdtodist);
            }
        }
