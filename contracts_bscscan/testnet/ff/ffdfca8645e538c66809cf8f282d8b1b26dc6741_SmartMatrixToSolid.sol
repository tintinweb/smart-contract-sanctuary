...
contract SmartMatrixToSolid is SmartMatrixToSolidBasic {
    ...
    function checkLevelEligible(address user,address parent, uint8 level) internal {
        
        if(users[parent].activeX6Levels[level]){
            return sendETHDividends1(parent, user, 2, level);
        }else{
           address parentReferer = users[user].referrer;
           checkLevelEligible(user,parentReferer,level); 
        }
    }
    ...
    function updateX6Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if(users[referrerAddress].x6Matrix[level].referrals.length >2 && users[referrerAddress].x6Matrix[level].referrals.length <6){
            
            address parentReferer = users[userAddress].referrer;
            checkLevelEligible(userAddress,parentReferer, level);
            return;
        }
    ...
    function sendETHDividends1(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        depositToken.safeTransfer(receiver, 0.5e18);
