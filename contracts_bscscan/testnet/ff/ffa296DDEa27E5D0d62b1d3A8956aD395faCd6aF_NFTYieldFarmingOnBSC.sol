...
contract NFTYieldFarmingOnBSC is Ownable {
    ...
    struct NFTPoolInfo {
        IERC1155 nftToken;    /// NFT token as a target to stake
    ...
    function addNFTPool(
        IERC1155 _nftToken,   /// NFT token as a target to stake
    ...
    function updatePool(uint256 _pid) public {
        ...
        governanceToken.transfer(devaddr, governanceTokenReward.div(10));  /// [Note]: transfer method can be used by dev address
        governanceToken.transfer(address(this), governanceTokenReward);    /// [Note]: transfer method can be used by only owner
    ...
    function deposit(uint256 _pid, uint256 _amount, uint256 _ids) public {
        NFTPoolInfo storage pool = nftPoolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        uint256 balanceCheck = pool.nftToken.balanceOf(msg.sender, _ids);
        require(balanceCheck > 0, "Unkown NFT Token");
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending =
