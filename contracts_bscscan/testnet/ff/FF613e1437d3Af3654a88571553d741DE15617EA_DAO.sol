...
contract DAO is Ownable {
    ...
    uint256[] public resolutionList;
    mapping (uint256 => Resolution) resolutionContentList;
    ...
    function setMemberPermissions(uint256 _memberId, int _permissions) public onlyOwner {
        Member memory member = memberList[_memberId];
    ...
    function addResolution(int _executeType, string memory _resolutionTitle, string memory _resolutionContent, uint256 _voteValidTime, uint256 _executeValidTime) public onlyOwner {
        ...
        resolutionContentList[_resolutionId] = Resolution({
        ...
        resolutionList.push(_resolutionId);
    ...
    function memberVotes(uint256 _resolutionId, int _agree, string memory _comments) public returns (address) {
        Resolution storage resolution = resolutionContentList[_resolutionId];
        uint256 lastBlock = block.number;

        require(lastBlock <= resolution.voteValidTime, "[1]Voting time has expired");
        require(lastBlock <= resolution.executeValidTime, "[2]Voting time has expired");
    ...
    function executeResolution(uint256 _resolutionId) public onlyOwner {
        Resolution storage resolution = resolutionContentList[_resolutionId];
    ...
    function getResolutionId(uint _resolutionIndex) public view returns (uint256) {
        return resolutionList[_resolutionIndex];
    }

    function getResolutionList() public view returns (uint256[] memory) {
        return resolutionList;
    }

    function getResolutionContent(uint256 _resolutionId) public view returns (Resolution memory) {
