...
contract TESTX is Context, BEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public feeAddress = payable(0x0000000000000000000000000000000000000000);
    address payable public listingAddress = payable(0x0000000000000000000000000000000000000000);
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD; //Dead Address
    mapping (address => bool) private operators; // Allowing contract as operator to update TESTX rewards
    ...
    function minTESTXAmountBeforeSell() public view returns (uint256) {
        return minTokensBeforeSell;
    }
    
    function buyBackTESTXCap() public view returns (uint256) {
        return buyBackCap;
    }
    ...
    function _transfer(
        ...
        TESTXTransfer(from,to,amount,takeFee);
    }
    ...
    function transferFee(address from, address to,uint256 initialBalance) private lockedSwap{
        if(from != address(this) && (to !=address(feeAddress) || to !=address(0)) && initialBalance > 0 && (address(feeAddress) != address(0) && address(listingAddress) != address(0))){
            uint256 bnbToBeAddedToLiquidity = address(this).balance.sub(initialBalance);
            
            (bool sent,) = address(feeAddress).call{value : uint256(bnbToBeAddedToLiquidity).mul(marketingDenominator).div(_liquidityFee)}("");
            (bool sent2,) = address(listingAddress).call{value : uint256(bnbToBeAddedToLiquidity).mul(jackpotDenominator).div(_liquidityFee)}("");
            if(sent && sent2){
    ...
    function swapAndLiquifyTESTX(address from, address to) private lockedSwap{
        ...
        !(from == address(this) && to == address(uniswapV2Pair))
            ...
            transferFee(from,to,initialBalance);
            ...
            if (buyBackEnabled && balance > uint256(bnbToLiquidateTaxes)) {
                ...
                buyBackTESTX(balance.div(100));
            }

            emit SwapAndLiquify(piece, deltaBalance, otherPiece);
        }
    }
    /// @dev Buy back TESTX token
    function buyBackTESTX(uint256 amount) private lockedSwap {
    	if (amount > 0) {
    ...
    function TESTXTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        if(!takeFee)
            removeAllFee();
    ...
    function setMarketingAddress(address _marketingAddress) public onlyOperate {
        require(feeAddress != _marketingAddress,'Address unchanged!');
        feeAddress = payable(_marketingAddress);
    }
    ...
    function setJackpotAddress(address _jackpotContract) public onlyOperate {
        ...
        listingAddress = payable(_jackpotContract);
    }
    ...
    function activateLive() external onlyOperate {
        ...
        isExcludedFromFee(address(feeAddress));
        isExcludedFromFee(address(listingAddress));
        _holders = 2;
        _liquidityFee = 8;
        setMaxTxAmount(1);
    }
    ...
    function increaseSupply() public onlyOperate {
        ...
        require(balanceOf(deadAddress) >= _tTotal.mul(75).div(100),'TESTX token supply unable to increase yet!');
