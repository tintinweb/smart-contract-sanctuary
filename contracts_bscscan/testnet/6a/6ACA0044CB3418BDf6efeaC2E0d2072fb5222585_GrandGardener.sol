...
// S33DS with Governance.
contract S33DS is BEP20('A S33D', 'S33D') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (s33dgardener).
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "S33D::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "S33D::delegateBySig: invalid nonce");
        require(now <= expiry, "S33D::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "S33D::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying S33Ds (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "S33D::_writeCheckpoint: block number exceeds 32 bits");
