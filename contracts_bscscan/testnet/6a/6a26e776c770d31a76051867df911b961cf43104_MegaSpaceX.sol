...
contract MegaSpaceX is ERC20, Ownable {
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 liquiditytoken= tokens.mul(liquidityFee).div(2).div(totalFees) ; //Caculate the tokens for liquidity
        ...
        // has been manually sent to the contract
         ...
         uint256 initialBalance = address(this).balance;
          // swap tokens for ETH for liquidity ,marketing and dividens.
         swapTokensForEth(tokens.sub(liquiditytoken));
        
        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);
        
        uint256 liquidityeth = newBalance.mul(liquidityFee.div(2)).div(totalFees.sub(liquidityFee.div(2)));
        uint256 marketingeth = newBalance.mul(marketingDivisor).div(totalFees.sub(liquidityFee.div(2))); 
        uint256 dividends = newBalance.mul(BNBRewardsFee).div(totalFees.sub(liquidityFee.div(2)));
        
        //SEND dividends to DdividendTracker
        ...
        addLiquidity(liquiditytoken, liquidityeth);

        emit SwapAndLiquify(liquiditytoken, liquidityeth, liquiditytoken);
    }
    
    function withdrawbuybackfund () external onlyOwner {
        uint256 initialBalance = address(this).balance;
