...
contract LiquidArtNFTContract is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    constructor() ERC721('Liquid Art System NFT', 'LIQUID') {}
    ...
    function royaltyInfo(uint256 tokenId)external view returns (uint256 user_royalty)
    {
    //   address minter = minterOf(tokenId);
       uint256 royalty = royaltyOf(tokenId);
        return (royalty);
    }
    ...
    function mint(uint256 amount , uint32 royalty) public payable {
        require(saleOpen == true, 'Sale not started yet!');
        require(royalty <= 90000, "Pentas: Royalty should not be more than 90%");
        require(amount <= maxMint, 'max minting limit exceeded');
        require(msg.value >= price * amount, 'insufficient payment amount!');
