...
contract ZapPool is Ownable, ReentrancyGuard, Initializable {
    ...
    modifier onlyUserOrWhitelistedContracts() {
        require(msg.sender == tx.origin || whitelistContracts[msg.sender], "Allow non-contract only");
    _;
    }
    ...
    function unclaimed_pool_collateral() external pure returns (uint256) {
    ...
    function zapMint(uint256 _collateral_amount, uint256 _dollar_out_min) external onlyUserOrWhitelistedContracts nonReentrant {
        ...
        if (_tcr < COLLATERAL_RATIO_MAX) {
            ...
            // deduct dollar amount from swap slippage
            uint256 _received_amount = _received_amounts[_received_amounts.length - 1];
            uint256 _received_value = (_received_amount * _share_price) / PRICE_PRECISION;
            _actual_dollar_amount = _actual_dollar_amount - (_share_value - _received_value); 

            emit ZapSwapped(_swap_collateral_amount, _received_amount);
        }
