...
contract Handing is IERC721Receiver {
    ...
    uint256 public MAX_DRAW = 1000; //单轮开奖条件
    uint256 public MAX_RECORD = 10; //单次发奖上限
    uint256 public MAX_AWARD_PLAYER = 10; //单次开奖人数
    uint256 public RECORD_NUMBER = 2000; //单轮抽奖次数
    uint256 public MAX_TIME = 24*60*60; //单轮开奖时间
    uint256 public MAX_RECEIVE = 20;  //单次领取奖励数量
    uint256 public MAX_RECEIVE_QUERY = 20; //查询用户待领取奖励,一次返回上限
    ...
    mapping(uint256 => uint256[]) private cycle_card;
    ...
    function award(uint256 types)external returns(bool) {
        ...
        emit cycle_card_log(cycle_card[CYCLE],CYCLE);
        draw.last_time = now;
        draw.draw_count = 0;
        CYCLE++;
        return true;
    ...
    function award_box(uint256 _box,uint256 _series_id,address _user,uint256 rand)internal{
        ...
        for(uint i = 0; i < _token_ids.length; i++){
            ...
            cycle_card[CYCLE].push(_token_ids[i]);
        }
    }
    ...
    function queryCycleDrawNumber(uint256 _cycle) external view returns (uint256){
        return cycle_card[_cycle].length;
    }
