...
contract Wankit is Context, IBEP20, Ownable {
    ...
    mapping (Functions => uint256) public _timelock; //CHANGEME should be private
    uint256[2] public _timelockDuration = [0, 2 minutes]; //CHANGEME, should be 12 hours
    ...
    address[2] public _marketingWallet;
    address[2] public _platformWallet;
    address[2] public _charityWallet;
    ...
    event WalletAddressUpdated (bytes32 walletName, address walletAddress, bool changeEnacted, uint256 timestampToEnactChange);
    ...
    function setMarketingWallet (address newWallet) external onlyFeeSetter {
        ...
        emit WalletAddressUpdated (bytes32("Marketing"), newWallet, false, _timelock[Functions.MARKETING]);
    }
    ...
    function setPlatformWallet (address newWallet) external onlyFeeSetter {
        ...
        emit WalletAddressUpdated (bytes32("Platform"), newWallet, false, _timelock[Functions.PLATFORM]);
    }
    ...
    function setCharityWallet (address newWallet) external onlyFeeSetter {
        ...
        emit WalletAddressUpdated (bytes32("Charity"), newWallet, false, _timelock[Functions.CHARITY]);
    }
    ...
    function _setFeeFormula() private {
        if (_timelock[Functions.FEE_FORMULA] > block.timestamp) {
    ...
    function _setMinMaxBalancesForFeeFormula() private {
        if (_timelock[Functions.MIN_MAX_FEE_BALANCE] > block.timestamp) {
    ...
    function _setRouterAddress () private {
        if (_timelock[Functions.ROUTER] > block.timestamp) {
    ...
    function _setNewTimelock() private {
        if (_timelock[Functions.TIMELOCK] > block.timestamp) {
    ...
    function _setNewFeeSetter() private {
        if (_timelock[Functions.FEESETTER] > block.timestamp) {
