...
interface IOctiStakingBNB is IOctiStaking {
    function sendProfit() external payable;
}


interface IOctiStakingBEP20 is IOctiStaking {
...
// File: contracts/Pool/OctiBNBPool.sol
 ...
 */
contract OctiBNBPool is
    IBNBLiquidityPool,
    Ownable,
    BEP20("Octi BNB LP Token", "OBNB")
{
    using SafeMath for uint256;
    ...
    /*
     * @nonce A provider supplies BNB to the pool and receives OBNB tokens
...
// File: contracts/Options/OctiBNBOptions.sol
 ...
 */
contract OctiBNBOptions is Ownable, IOctiOptions {
    using SafeMath for uint256;

    IOctiStakingBNB public settlementFeeRecipient;
    ...
    OctiBNBPool public pool;

    /**
     * @param pp The address of ChainLink BNB/USD price feed contract
     */
    constructor(AggregatorV3Interface pp, IOctiStakingBNB staking) public {
        pool = new OctiBNBPool();
        priceProvider = pp;
     ...
     */
    function setSettlementFeeRecipient(IOctiStakingBNB recipient) external onlyOwner {
        require(block.timestamp < contractCreationTimestamp + 14 days);
