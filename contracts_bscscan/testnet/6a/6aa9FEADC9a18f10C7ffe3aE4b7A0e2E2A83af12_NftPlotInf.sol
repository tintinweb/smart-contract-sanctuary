...
contract NftPlotInf is NftPlotPojo, CoinFactoryAdminRole {
    ...
    //业务接口，（如：临时补充）改变原有系列盲盒剩余数量
    function updateBlindBox(uint256 plot_id, uint256 number) public onlyCoinFactoryAdmin returns (bool){
    ...
    //获取所有已发行nft数量
    function getAllTokenLen() public view returns (uint256) {return allTokenId.length;}

    //获取所有展示nft剩余数量
    function getPlotList() public view returns (uint256 singleLength, uint256[] memory elementList) {
        uint256 singleLength = 1;
        uint256 j = 0;
        uint256[] memory elementList = new uint256[](plotList.length * singleLength);
        for (uint256 i = 0; i < plotList.length; i++) {elementList[j] = plotList[i].lave_num;
            j = j + singleLength;}
        return (singleLength, elementList);
    }
