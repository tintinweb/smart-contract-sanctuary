...
abstract contract IThreeKingdomsNFT{
    
    function genesisBreed(uint256 maxAmount) virtual public; //mint a genesis egg, buy genesis egg
    
    function breed(uint256 tokenIdA,uint256 tokenIdB) virtual public; //mint egg by two pets
    
    function hatch(uint256 tokenId) virtual public;
    
    function statusChange(uint256 tokenId,uint8 status) virtual public;    //change pet stake status
    
    function feedEnergy(uint256 tokenId,uint256 energy) virtual public;  //potion contract run
    
    function feedPoint(uint256 tokenId, uint256 position, uint256 point, uint256 expire) virtual public; //potion contract run
...
contract teeFarm is Ownable,Depositable {
    ...
    function sweep() public onlyOwner{
        require(block.number>endBlock.add(3 days));
    ...
    function pending(address _user, uint256 _tokenId) external view returns (uint256) {
        (,,,uint256 energyLimit,uint256 energyPoint,,,,,,uint8 depositStatus) = IThreeKingdomsNFT(nftTokenAddress).petInfo(_tokenId);
        require(depositStatus==1,"pet is not staked in farm");
        require(IThreeKingdomsNFT(nftTokenAddress).ownerOf(_tokenId)==_user,"owner invalid");
    ...
    function claim(uint256 _tokenId) public{
        ...
        address petOwner = IThreeKingdomsNFT(nftTokenAddress).ownerOf(_tokenId);
    ...
    function stake(uint256 _tokenId) public {
        (uint8 form,,,uint256 energyLimit,,,,,,,uint8 depositStatus) = IThreeKingdomsNFT(nftTokenAddress).petInfo(_tokenId);
        ...
        IThreeKingdomsNFT(nftTokenAddress).statusChange(_tokenId,1); //change status
    ...
    function unStake(uint256 _tokenId) public {
        ...
        IThreeKingdomsNFT(nftTokenAddress).statusChange(_tokenId,0); //change status
        IThreeKingdomsNFT(nftTokenAddress).feedEnergy(_tokenId,energyRecovered);  //update energy
    ...
    function emergencyWithdraw(uint256 _tokenId) public {
        (,,,uint256 energyLimit,,,,,,,uint8 depositStatus) = IThreeKingdomsNFT(nftTokenAddress).petInfo(_tokenId);
        require(depositStatus==1,"pet is not staked in farm");
