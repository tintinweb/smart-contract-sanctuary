...
struct GeneralDetails {
    ...
    string _symbol;
    uint8 _decimals;
    uint256 _totalSupply; //6
    bool _isPaused;
    uint256 _totalStake;   
    uint256 _activeStakers;
    uint256 _refPaid; //10
    uint256 _totalRewardPaid;  //11
    
    address _validatorAdd;   //12
    string _validatorApi;
    uint256 _maxSupply;
    uint256 _maxTxLimit;  //15
    uint256 _rewardDuration;
    uint256 _refCom;
    uint256 _voteApproveRate;  //18  
    uint256 _propDuration; //19
    uint256 _changeDate;
    uint256 _swapOut;
    uint256 _swapIn;
    uint256 _supplyTime;
    uint256 _otherSupply;
    address _stakeAddress;  //25
}
