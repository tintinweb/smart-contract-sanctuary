...
import './FuzzyswapPoolDeployer.sol';
...
contract FuzzyswapFactory is IFuzzyswapFactory, FuzzyswapPoolDeployer, NoDelegateCall {
...
contract FuzzyswapPoolDeployer is IFuzzyswapPoolDeployer {
    struct Parameters {
        address factory;
        address token0;
        address token1;
    ...
    /// @inheritdoc IFuzzyswapPoolDeployer
    ...
    function deploy(
        ...
        pool = address(new FuzzyswapPool{salt: keccak256(abi.encode(token0, token1, fee))}());
        delete parameters;
    }
}
...
contract FuzzyswapPool is IFuzzyswapPool, NoDelegateCall {
    ...
    constructor() {
        int24 _tickSpacing;
        (factory, token0, token1, fee, _tickSpacing) = IFuzzyswapPoolDeployer(msg.sender).parameters();
...
library SqrtPriceMath {
    ...
    function getNextSqrtPriceFromInput(
        ...
        if (zeroForOne) {
            ...
            uint256 product;
            if ((product = amountIn * sqrtPX96) / amountIn == sqrtPX96) {
                uint256 denominator = numerator1 + product;
                if (denominator >= numerator1)
                    // always fits in 160 bits
                    return uint160(FullMath.mulDivRoundingUp(numerator1, sqrtPX96, denominator));
            }
            return uint160(UnsafeMath.divRoundingUp(numerator1, (numerator1 / sqrtPX96).add(amountIn)));
        } else { 
            uint256 quotient =
                (
                    amountIn <= type(uint160).max
                        ? (amountIn << FixedPoint96.RESOLUTION) / liquidity
    ...
    function getNextSqrtPriceFromOutput(
        ...
        if (zeroForOne) {
            uint256 quotient =
                (
                    amountOut <= type(uint160).max
                        ? UnsafeMath.divRoundingUp(amountOut << FixedPoint96.RESOLUTION, liquidity)
                        : FullMath.mulDivRoundingUp(amountOut, FixedPoint96.Q96, liquidity)
                );
            ...
            require((product = amountOut * sqrtPX96) / amountOut == sqrtPX96 && numerator1 > product);
            uint256 denominator = numerator1 - product;
