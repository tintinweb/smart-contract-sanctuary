...
contract HentaiholidaySingle is
    ERC721,
    ERC721Enumerable,
    ERC721URIStorage,
    ERC721Burnable,
    ...
    
{
    ...
    constructor(
        ...
        uint256 _transvalue
    ) ERC721("HentaiholidaySingle", "HentaiholidaySingle") {
 ...
 function getServiceFee() public view returns(uint256){
    ...
    function cursaleTokenTransfer(address payable from, uint256 tokenId, uint256 amount) public payable{
       new_saleToken(from, tokenId, amount, "ETH");
    //    saleTokenTransfer(from, tokenId, token, _type, NOFToken);
            tokenTrans(tokenId, from, msg.sender);
            // tokenTrans(tokenId, from, msg.sender);

    }
    // function saleTokenwithToken(address payable from, uint256 tokenId, uint256 amount, string memory tokenAss) public{
    //     new_saleToken(from, tokenId, amount, tokenAss);
    //     // saleTokenTransfer(from, tokenId, token, _type, NOFToken);
    //             tokenTrans(tokenId, from, msg.sender);
    // }


    function new_saleToken(
 ...
 t.transferFrom(from,owner(),_adminfee);
    ...
    if(keccak256(abi.encodePacked((currency))) == keccak256(abi.encodePacked(("ETH")))){   
        ...
        SalewithToken(tokenAss, toadd, amount,tokenId);
        saleTokenTransfer(from, tokenId);
    }
    ...
    //  function saleTokenwithToken(address payable from, uint256 tokenId, uint256 amount, string memory tokenAss) public{
     function saleTokenwithToken(address tokenAss,address from , uint256 tokenId, uint256 amount) public{
        // new_saleToken(from, tokenId, amount, tokenAss);
    ...
    function newtokenasbid(address tokenAss,address from, address admin, uint256 amount, uint256 tokenId) internal {
        (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], serviceValue);
        ERC20 t = ERC20(tokenAss);
