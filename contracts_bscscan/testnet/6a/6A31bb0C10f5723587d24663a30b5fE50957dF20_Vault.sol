...
contract Vault {
	...
	IBEP20 SAFU = IBEP20(0x890cc7d14948478c98A6CD7F511E1f7f7f99F397);

    uint256 public maximumLocked = 2000 * 10e9;
    ...
    function stake(uint256 amount) public {
		require(amount > 0 && totalLocked + amount <= maximumLocked, "The maximum amount of SAFUs have been staked in this pool.");
		//A TESTER!!!!!! est-ce que fail si pas allowed??
		...
		if (balance[msg.sender] > 0) {
			stakersRewards[msg.sender] += balance[msg.sender] * rewardsPerSAFUPerBlock * (block.number - _lastUpdate);
		} else {
    ...
    function withdraw(uint256 amount) public {
		...
		SAFU.transfer(msg.sender, amount + rewards);
    }
	...
	function modifyRewards(uint256 amount) public onlyOwner {
		...
		for (uint256 i = 0; i < stakers.length; i++) {
			...
			stakersRewards[stakers[i]] += balance[stakers[i]] * rewardsPerSAFUPerBlock * (block.number - _lastUpdate);
		}
