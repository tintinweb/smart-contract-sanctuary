...
contract FixStaking is AccessControl {
    ...
    event PoolFunded(uint256 fundAmount);
    event Deposit(
        ...
        isPoolEmpty(_poolIndex)
    {
        if (database.getPoolInfo(_poolIndex).reserve > 0) {
            database.withdrawReserve(_poolIndex);
        }
        ...
        isPoolExist(_poolIndex)
    {
        ...
        require(
            database.getStakerAmount(msg.sender, _poolIndex) + _amount <=
                maxStake,
        ...
        database.deposit(msg.sender, index, _amount, _poolIndex, duration);
    }
