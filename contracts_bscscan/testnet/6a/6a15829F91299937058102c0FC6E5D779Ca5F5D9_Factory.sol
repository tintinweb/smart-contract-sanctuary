...
// File: contracts/Token_generator.sol
...
//   event NewToken(address indexed _from, address indexed _token);
...
//   }
  ...
  function newToken(string memory _name, string memory _symbol, uint8 _decimals, uint256 _initialSupply) public returns(address) {
    address token = address(new Token(_name, _symbol, _decimals, _initialSupply, msg.sender));
    return token;
...
contract Token is ERC20 {
    ...
    constructor (string memory _name, string memory _symbol, uint8 _decimals, uint256 _initialSupply, address _sender) public ERC20(_name, _symbol) {
        _setupDecimals(_decimals);
        _mint(_sender, _initialSupply * (10 ** uint256(decimals())));
    }
}