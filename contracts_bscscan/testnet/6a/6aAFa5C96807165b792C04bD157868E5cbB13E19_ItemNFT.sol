...
abstract contract BaseNFT is NFTAuth, ERC721BurnableUpgradeable, EIP712Upgradeable {
  ...
  function updateMinPrice(uint _tokenId, uint _ownPrice) virtual external;
...
contract ItemNFT is BaseNFT {
  ...
  function mintItem(address _owner, uint8 _gene, uint16 _class, uint _price, uint _index) onlyMintAdmin public returns (uint) {
    ...
    _mint(_owner, newId);
    Item memory item = Item(_gene, _class, block.timestamp, _price, _price, 0);
    items[newId] = item;
    return newId;
  }
  ...
  function getItem(uint _tokenId) public view returns (uint8, uint16, uint, uint, uint, uint) {
    return (
      items[_tokenId].gene,
      items[_tokenId].class,
      items[_tokenId].createdAt,
      items[_tokenId].ownPrice,
      items[_tokenId].minPrice,
  ...
  function updateMinPrice(uint _tokenId, uint _minPrice) override onlyMainAdmin external {
    Item storage item = items[_tokenId];
    require(_minPrice > item.minPrice, "NFT: min price invalid");
