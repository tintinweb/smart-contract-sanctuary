...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    ...
    contract CakeMaxi is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingWalletAddress = payable(0x590b26612033FE5d679aee9cDC5bfB8F9fFaFeB4); // Marketing Address
    address payable private _liquidityWalletAddress = payable(0x000000000000000000000000000000000000dEaD); // Cake Address
    ...
    string private _name = "CakeMaxi";
    string private _symbol = "CMX";
    uint8 private _decimals = 9;

    uint256 private _burnFee = 1;
    uint256 public _liquidityFee = 3;
    uint256 public _marketingFee = 7;
    uint256 public _CakeFee = 5;
    ...
    constructor () {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); //Prod 0x10ED43C718714eb63d5aA57B78B54704E256024E     
        ...
        _totalTaxPercent = _burnFee.add(_liquidityFee).add(_marketingFee).add(_CakeFee);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _marketingFee.add(_CakeFee) > 0)
        {
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(forWallets);
            uint256 newBalance = address(this).balance.sub(initialBalance);
    
            uint256 marketingShare = newBalance.div(_marketingFee.add(_CakeFee)).mul(_marketingFee);
            uint256 CakeWalletShare = newBalance.sub(marketingShare);
            
            if(marketingShare > 0)
                transferToAddressETH(marketingWalletAddress, marketingShare);
            
            if(CakeWalletShare > 0)
                transferToAddressETH(_liquidityWalletAddress, CakeWalletShare);
        }
    ...
    function setTaxes(uint256 newBurnFee, uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newCakeTax) external onlyOwner() {
        _burnFee = newBurnFee;
    ...
    function setXrp_liquidityWalletAddress(address newAddress) external onlyOwner() {
        _liquidityWalletAddress = payable(newAddress);
    }
