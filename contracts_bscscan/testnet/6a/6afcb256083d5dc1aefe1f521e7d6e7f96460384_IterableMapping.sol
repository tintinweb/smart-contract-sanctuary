...
telegram:https://t.me/PCAKE

Tokenomics
12% fee
to holders Hold get Cake token Cake token reward
Sp 8%-12%
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  //Can be any token, currently using CAKE
  uint256 constant internal magnitude = 2**128;

  uint256 internal magnifiedDividendPerShare;
  uint256 internal lastAmount;
  
  address public dividendToken = 0x9C21123D94b93361a29B2C2EFB3d5CD8B17e0A9e;
...
/////////// Tokens /////////////
////////////////////////////////
//Payout token contract address listed here as well CAKE
contract PCAKE is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public _dividendToken = 0x9C21123D94b93361a29B2C2EFB3d5CD8B17e0A9e;
    ...
    PCAKEDividendTracker public dividendTracker;

    address private buyBackWallet;
    
    uint256 public maxBuyTranscationAmount = 2500000000 * (10**18); // 2,5% of total supply
    uint256 public maxSellTransactionAmount = 1500000000 * (10**18);// 1.5% of total supply
    uint256 public swapTokensAtAmount = 20000000 * (10**18);
    uint256 public maxWalletToken = 2500000000 * (10**18);// 2.5% of total supply
    ...
    uint256 private marketingDivisor = 30;
    ...
    constructor() ERC20("PCAKE", "PCAKE") {
        ...
        dividendTracker = new PCAKEDividendTracker();

       //wallet_currently using DEV
        buyBackWallet = 0x3A3ef933BD1e15e3C2667bb9AdB7Bc6975914AC2;
        
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
         // Create a uniswap pair for this new token on Pancakeswap
    ...
    function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
        require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "PCAKE: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
        sellFeeIncreaseFactor = multiplier;
    }
    
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        require(marketingDivisor >= 0 && marketingDivisor <= 100, "PCAKE: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "PCAKE: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "PCAKE: The dividend tracker already has that address");

        PCAKEDividendTracker newDividendTracker = PCAKEDividendTracker(payable(newAddress));
//Contract Owner
        require(newDividendTracker.owner() == address(this), "PCAKE: The new dividend tracker must be owned by the testCake token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "PCAKE: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PCAKE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PCAKE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PCAKE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PCAKE: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "PCAKE: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PCAKE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PCAKE: Cannot update gasForProcessing to same value");
...
contract PCAKEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("PCAKE_Dividend_Tracker", "PCAKE_Dividend_Tracker") {
        claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "PCAKE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "PCAKE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PCAKE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "PCAKE_Dividend_Tracker: claimWait must be updated to between 0.5 and 24 hours");
        require(newClaimWait != claimWait, "PCAKE_Dividend_Tracker: Cannot update claimWait to same value");
