...
contract IDOs is Adminable {
    ...
    function addIDO(address _address) external onlyOwnerOrAdmin {
        addresses.push(_address);
    }

    function getIDOs() public view returns (ILevelManager[] memory) {
        ILevelManager[] memory newArray = new ILevelManager[](addresses.length);
        for (uint256 i = 0; i < addresses.length; i++) {
            newArray[i] = ILevelManager(addresses[i]);
        }
        return newArray;
    }
