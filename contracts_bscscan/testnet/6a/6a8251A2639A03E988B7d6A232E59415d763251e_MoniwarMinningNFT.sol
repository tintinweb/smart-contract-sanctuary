...
contract MoniwarMinningNFT is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accMowaPerShare; // Accumulated MOWAs per share, times 1e18. See below.
        uint256 totalAmount;
    ...
    uint256 amountHashrate = 9;
    address public feeWallet;

    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event DepositNFT(address indexed user, uint256 indexed pid, uint256 tokenId);
    event UnstakeNFT(address indexed user, uint256 indexed pid, uint256 tokenId);
    ...
    constructor(
        ...
        poolNFT[8]  = PoolNFT(8, 10, 42 * 10**18);
        poolNFT[9]  = PoolNFT(9, 10, 42 * 10**18);
        poolNFT[10] = PoolNFT(10, 100, 420 * 10**18);
        poolNFT[11] = PoolNFT(11, 100, 420 * 10**18);
        poolNFT[12] = PoolNFT(12, 100, 420 * 10**18);
        poolNFT[13] = PoolNFT(13, 150, 630 * 10**18);
        poolNFT[14] = PoolNFT(14, 50, 210 * 10**18);
        poolNFT[15] = PoolNFT(15, 50, 210 * 10**18);
        poolNFT[16] = PoolNFT(16, 50, 210 * 10**18);
        poolNFT[17] = PoolNFT(17, 100, 420 * 10**18);
        poolNFT[18] = PoolNFT(18, 150, 630 * 10**18);
        poolNFT[19] = PoolNFT(19, 50, 210 * 10**18);
        poolNFT[20] = PoolNFT(20, 50, 210 * 10**18);
        poolNFT[21] = PoolNFT(21, 50, 210 * 10**18);
        poolNFT[22] = PoolNFT(22, 100, 420 * 10**18);
        poolNFT[23] = PoolNFT(23, 150, 630 * 10**18);
    }
    ...
    function set(
        ...
        if (_totalReward > 0) {
            mowaPoolInfoNFT[_pid].totalReward = _totalReward;
        }
    ...
    function getUserInfo(uint256 pid, address _userAddress) public view returns (User memory userInfo)
    {
        return userInfo = users[pid][_userAddress];
    }

    function getPriceUpgrade(uint256 _pid, uint256 uid, address _userAddress) public view returns (uint256 price)
    {
        User storage userAddPool = users[_pid][_userAddress];
        for (uint256 index = 0; index < userAddPool.userPools.length; index++) {
            if(userAddPool.userPools[index].uid == uid) {
                price = userAddPool.userPools[index].amountCalHashrate;
            }
        }
        return price;
    }
    ...
    function calculator(uint256 _pid, uint256 date, uint256 hashrate) external view returns (uint256) {
        ...
        accMowaPerShare = accMowaPerShare.add(mowaReward.mul(1e18).div(totalHahrate));
        return hashrate.mul(accMowaPerShare).div(1e18);
    }
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256)
    {
        ...
        accMowaPerShare = accMowaPerShare.add(mowaReward.mul(1e18).div(pool.totalHashRate));
        pending = user.hashRate.mul(accMowaPerShare).div(1e18).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 mowaReward = multiplier.mul(pool.rewardPerBlock);
        pool.accMowaPerShare = pool.accMowaPerShare.add(mowaReward.mul(1e18).div(pool.totalHashRate));
        pool.lastRewardBlock = block.number;
    }
    ...
    function upgrade(uint256 _pid, uint256 uid, uint256 number) public nonReentrant {
        require(number > 0, "Number Hashrate > 0");
        PoolInfo storage pool = mowaPoolInfoNFT[_pid];
        require(block.number < pool.endBlock, "pool has ended");
        User storage userAddPool = users[_pid][msg.sender];
        uint256 amountCalHashrate = 0;
        uint256 amountDeposit = 0;
        uint256 hashRate = 0;
        uint256 tokenId = 0;
        for (uint256 index = 0; index < userAddPool.userPools.length; index++) {
            if(userAddPool.userPools[index].uid == uid) {
                amountCalHashrate = userAddPool.userPools[index].amountCalHashrate;
                amountDeposit = userAddPool.userPools[index].amountDeposit;
                hashRate = userAddPool.userPools[index].hashRate;
                tokenId = userAddPool.userPools[index].tokenId;
            }
        }
        require(tokenId > 0, "nft not stake pool");
        uint256 amountUpgarade = amountCalHashrate.mul(number).mul(amountHashrate).div(1000);
        uint256 hashRateUpgrade = hashRate.mul(number);
        uint256 mowaBal = mowaToken.balanceOf(address(msg.sender));
        require(mowaBal >= amountUpgarade, "upgrade not good");
        mowaToken.transferFrom(address(msg.sender), address(this), amountUpgarade);
        amountDeposit = amountDeposit.add(amountUpgarade);
        amountCalHashrate = amountCalHashrate.add(amountUpgarade);
        for (uint256 index = 0; index < userAddPool.userPools.length; index++) {
            if(userAddPool.userPools[index].uid == uid) {
                userAddPool.userPools[index].amountDeposit = amountDeposit;
                userAddPool.userPools[index].amountCalHashrate = amountCalHashrate;
                userAddPool.userPools[index].hashRate = hashRateUpgrade;
            }
        }
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        payOrLockupPendingMowa(_pid);
        user.amount = user.amount.add(amountUpgarade);
        ...
        user.rewardDebt = user.hashRate.mul(pool.accMowaPerShare).div(1e18);
...
//        require(mowaNFT.ownerOf(_tokenId) == msg.sender, "not own");
        ...
        User storage userAddPool = users[_pid][msg.sender];
        uint256 tokenIdInPool = 0;
        for (uint256 index = 0; index < userAddPool.userPools.length; index++) {
            if(userAddPool.userPools[index].uid == uid) {
                tokenIdInPool = userAddPool.userPools[index].tokenId;
            }
        }
//        require(tokenIdInPool == 0, "already stake this nft");
        UserInfo storage user = userInfo[_pid][msg.sender];
        uint256 characterNFT = 1; // mowaNFTCore.getNFT(_tokenId).character;
...
//        mowaNFT.transferFrom(address(msg.sender), address(this), _tokenId);
        UserPool memory uPool = UserPool(amount,0,hashRate,_tokenId,uid);
        userAddPool.userPools.push(uPool);
    ...
    function harvest(uint256 _pid) public nonReentrant {
        ...
        if (user.hashRate == 0 || block.number >= pool.endBlock) {
            ...
            pending = user.hashRate.mul(pool.accMowaPerShare).div(1e18).sub(user.rewardDebt);
    ...
    function unStake(uint256 _pid, uint256 uid) public nonReentrant {
        ...
        for (uint256 index = 0; index < userAddPool.userPools.length; index++) {
            if(userAddPool.userPools[index].uid == uid) {
                amount = userAddPool.userPools[index].amountDeposit;
                amountCalHashrate = userAddPool.userPools[index].amountCalHashrate;
                hashRate = userAddPool.userPools[index].hashRate;
                _tokenId = userAddPool.userPools[index].tokenId;
            }
        }
        ...
        if (amount > 0) {
            ...
            pool.totalAmount = pool.totalAmount.sub(amountCalHashrate);
        }
        ...
        for (uint256 index = 0; index < userAddPool.userPools.length; index++) {
            if(userAddPool.userPools[index].uid == uid) {
                delete userAddPool.userPools[index];
            }
        }
        emit UnstakeNFT(msg.sender, _pid, _tokenId);
    }
    ...
    function setAmountHashrate(uint256 _fee) external onlyOwner {
        amountHashrate = _fee;
    }
