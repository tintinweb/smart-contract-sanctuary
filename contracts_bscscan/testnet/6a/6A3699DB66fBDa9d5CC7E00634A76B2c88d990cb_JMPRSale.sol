...
contract JMPRSale is Ownable {
    ...
    // investor withdraws
    function withdrawsOf(address investor) public view returns (uint256) {
        return _withdraws[investor];
    }

    // investor withdraws
    function availableOf(address investor) public view returns (uint256) {
        uint256 availablePercent = _unlock.mul(_percent);
        return _deposits[investor].div(100).mul(availablePercent).sub(_withdraws[investor]);
    }
    ...
    function withdrawIDO(uint256 value) public onlyOwner {
    ...
    function changeUnlock(uint256 value) public onlyOwner {
        _unlock = value;
    }
    ...
    function _withdrawTokens() internal {
        ...
        require(availableOf(msg.sender) > 0, "Zero tokens available");

        uint256 available = availableOf(msg.sender);
        uint256 tokens = available.mul(_rate);
        _withdraws[msg.sender] = _withdraws[msg.sender].add(available);
