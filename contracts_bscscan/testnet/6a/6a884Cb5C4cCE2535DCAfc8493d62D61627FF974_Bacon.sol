...
contract Bacon is IBacon, ERC721Enumerable, Pausable, Ownable, ReentrancyGuard, PaymentSplitter {
	...
	event BaconMinted(uint256 indexed tokenId);
	event CopMinted(uint256 indexed tokenId);
	...
	string public _contractBaseURIBacon = "ipfs://xxxxxx-bacon-xxxxx/metadata/";
	string public _contractBaseURICops = "ipfs://xxxxxx-cops-xxxxx/metadata/";
	...
	function dele21t1e1xxxxx321x933Me() internal pure {} //etherscan trick
	...
	function mint(uint256 qty, uint256 _seed) external payable whenNotPaused nonReentrant {
		...
		for (uint256 i = 0; i < qty; i++) {
			seed = randomizer.random(100, 2**256 - 1, _seed + _tokenIds.current());
			...
			if (isBaconInt >= 90) {
				isCop = true;
				emit CopMinted(_tokenIds.current());
			} else {
				emit BaconMinted(_tokenIds.current());
			}
	...
	function tokenURI(uint256 _tokenId) public view override returns (string memory) {
		...
		if (tokenTraits[_tokenId].isCop) {
			return string(abi.encodePacked(_contractBaseURICops, _tokenId.toString()));
		}
		return string(abi.encodePacked(_contractBaseURIBacon, _tokenId.toString()));
	}

	function setBaseURI(string memory _newCops, string memory _newBacon) external onlyDev {
		require(!locked, "locked functions");
		_contractBaseURIBacon = _newBacon;
		_contractBaseURICops = _newCops;
	}
