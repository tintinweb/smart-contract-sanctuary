...
import "./interfaces/IWebooReward.sol";
import "./interfaces/IWebooTokenForFarming.sol";
import "./interfaces/IWebooToken.sol";
...
contract WebooFarming is Context, Auth {
    ...
    mapping (address => uint256) public amountLPToken;
    ...
    constructor(address _tokenAddress, address _routerAddress, address _pairAddress, address _rewardAddress)  Auth(msg.sender) {
   ...
   function addLiquidity(uint256 amountToken, address recipient) external payable {
      // Transfer token to this contract
      uint256 balanceTokenBefore = IERC20(tokenAddress).balanceOf(address(this));
      IERC20(tokenAddress).transferFrom(_msgSender(), address(this), amountToken);
      uint256 balanceTokenAfter = IERC20(tokenAddress).balanceOf(address(this));
      ...
      IERC20(tokenAddress).approve(routerAddress, balanceForFarming);
       ...
       amountLPToken[recipient] = amountLPToken[recipient].add(liquidity);
       totalLPToken = totalLPToken.add(liquidity);
       setShare(recipient);
       calculateShare();
   }

   function removeLiquidity(uint256 amountRemove, address recipient) external {
       require(amountLPToken[_msgSender()] >= amountRemove,"Insufficient Amount");
        // Exclude From Fee for Pair
       IWebooToken(tokenAddress).setIsExcludeFromFee(pairAddress,true);

       // Approve LP Token for transfer to routerAddress
       IERC20(pairAddress).approve(routerAddress,amountRemove);

       // Get Amount Token and Eth Before
       uint256 amountTokenBefore = IERC20(tokenAddress).balanceOf(address(this));
       ...
       uint256 amountTokenAfter = IERC20(tokenAddress).balanceOf(address(this));
       ...
       IERC20(tokenAddress).transfer(recipient,amountTokenDiff);
       ...
       amountLPToken[_msgSender()] = amountLPToken[_msgSender()].sub(amountRemove);
       IWebooToken(tokenAddress).setIsExcludeFromFee(pairAddress,false);
    ...
    function setRewardShare() public {
        uint256 totalWeboo = IERC20(tokenAddress).balanceOf(pairAddress);
        IWebooReward(rewardAddress).setShare(address(this),totalWeboo);
    }
    ...
    function calculateShare() public {
        ...
        IWebooReward(rewardAddress).claimToWeth(address(this));
        uint256 balanceAfter = address(this).balance;
        uint256 balanceDiff = balanceAfter.sub(balanceBefore);
        totalDividends = totalDividends.add(balanceDiff);
        dividendsPerShare = dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(balanceDiff).div(totalLPToken));
    }
    ...
    function setShare(address account) internal {
        uint256 amount = amountLPToken[account];
    ...
    function _claimToWeboo(address account) internal {
        IUniswapV2Router02 router = IUniswapV2Router02(routerAddress);
        uint256 amount = dividendOf(account);
        if(amount > 0){
            IWETH(wethAddress).deposit{value:amount}();
            ...
            IERC20(tokenAddress).transfer(account,balanceTransfer);
