    ...
    function createVault(VaultData calldata data) external botOnly returns(uint vID) {
        ...
        if (vault.data.botToken != address(0)) {
            require(vault.data.botToken == data.botToken &&
                    ...
                    vault.data.bot == data.bot, "BotVault: vault existed");
            return vID;
        }
        vault.data = data;
...
abstract contract VaultBase is Context, Ownable {
    ...
    modifier botOnly() {
        require(data().botManager.isRegisteredBot(_msgSender()), 'VaultBase: caller is not a bot');
        _;
    }

    modifier ownedBotOnly(uint vID) {
        Vault storage vault = _vaultOf(vID);
        require(IBotToken(vault.data.botToken).owner() == _msgSender(),'VaultBase: caller is not the owner bot');
        _;
    }

    modifier _validVault(uint vID) {
        Vault storage vault = _vaultOf(vID);
        require(address(vault.data.botToken) != address(0), 
            string(abi.encodePacked('BotVault: invalid vault ID ', Strings.toHexString(vID, 32))));
        _;
    }
    ...
    function _vaultHandler(uint vID) internal view returns(address result) {
        ...
        require(address(result) != address(0), 'VaultBase: invalid vault type');
    }
