...
import "./interfaces/IWeapons.sol";
    ...
    AccessControlUpgradeable
{
    ...
    uint256 public constant BOX_OPEN_COOLDOWN = 2 weeks;
    ...
    uint256[4] guarantees = [20000e18, 10000e18, 5000e18, 2000e18];
    uint256[4] rarityQuantity = [10000e18, 5000e18, 2000e18, 1000e18];
    ...
    function initialize(IvBuni _vBuni, IWeapons _weapons, ICharacters _characters) public initializer {
        __ERC721_init("Buni Mystery Box", "BMB");
        __AccessControl_init_unchained();
    ...
    function changeWeapons(IWeapons _newWeapons) public restricted {
        weapons = _newWeapons;
    }

    function changeCharacters(ICharacters _characters) public restricted {
        characters = _characters;
    }
    ...
    function openChest(address _receiver) private {
        ...
        weapons.mintWeaponByElementWithStars(
            _receiver,
            2,
            rolled % 100,
            element
        );
        characters.mintCharacterWithElement(_receiver, rolled % 100, element);
    }
    ...
    function openEgg(address _receiver, uint256 _amount) private {
        ...
        uint256 rate = ((_amount - minimumQuantity) * 1e3) /
...
interface ICharacters {
    ...
    function mintCharacterWithElement(address minter, uint256 seed, uint256 element) external;
}
...
interface IWeapons {
    /*
        traits: 0-3 [0(fire) > 1(earth) > 2(lightning) > 3(water)
    */
    function mint(address minter, uint256 seed) external returns (uint256);

    function mintWeaponByElement(
        address minter,
        uint256 seed,
        uint256 element
    ) external returns (uint256);

    function mintWeaponWithStars(
        address minter,
        uint256 stars,
        uint256 seed
    ) external returns (uint256);

    function mintWeaponByElementWithStars(
        address minter,
        uint256 stars,
        uint256 seed,
        uint256 element
