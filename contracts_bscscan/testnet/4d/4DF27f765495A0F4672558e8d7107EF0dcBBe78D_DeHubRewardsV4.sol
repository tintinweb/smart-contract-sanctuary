 ...
 */
contract DeHubRewardsV4 is DeHubRewardsUpgradeable {
  ...
  uint256 public nextCycleResetTimestamp;
  // Staking contract address, which can act as a proxy for claiming rewards.
  address public stakingAddress;

  event ReceivedBNB(uint256 amount, address from);
  event ClaimedBNBRewards(uint256 bnbClaimed, address claimant);
  event SentBNBRewards(uint256 bnbSent, address holder);
  event PulledBNB(uint256 amount, address puller, address destination);
  ...
  modifier onlyStakingAddress() {
    require(msg.sender == stakingAddress, "Not a Staking Contract addr!");
    _;
  }
  ...
  function hasCyclePassed() public view returns (bool) {
    uint256 timeSinceReset = block.timestamp - lastCycleResetTimestamp;
  ...
  function hasAlreadyClaimed(address holderAddr) public view returns (bool) {
    ...
    bool didClaim = lastClaim >= lastCycleResetTimestamp &&
      lastCycleResetTimestamp != 0;
  ...
  function calcClaimableShare(
    ...
    uint256 _lpTokens,
    uint256 _claimableDistribution,
    uint256 _stakeAmount
  ) public view returns (uint256) {
    uint256 totalHoldingAmount = _circulatingSupply - _lpTokens;
    uint256 holderAmount = dehubToken.balanceOf(_holderAddr) + _stakeAmount;
    uint256 bnbShare = totalHoldingAmount.percent(holderAmount, 18);
    uint256 bnbToSend = bnbShare.percentOf(_claimableDistribution, 1 * 10**18);
  ...
  /**
   * @notice Calculates a share of BNB belonging to a holder based on his
   * holdings(or stake, or both), current circulating supply and tokens stored
   * in LP.
   */
  function calcCurrentClaimableShare(address holderAddr, uint256 stakeAmount)
    public
    view
    returns (uint256)
  {
    uint256 circulatingSupply = dehubToken.totalCirculatingSupply();
    uint256 lpTokens = dehubToken.balanceOf(uniswapV2Pair);
    uint256 bnbToSend = calcClaimableShare(
      holderAddr,
      circulatingSupply,
      lpTokens,
   ...
   */
  function resetClaimDistributionCycle(uint256 cycleHours, bool hardReset)
    internal
  {
    require(cycleHours > 0, "Cycle hours can't be 0.");
    ...
    // Reset time stamp
    uint256 adjustedResetTimestamp = block.timestamp;

    uint256 cycleSeconds = cycleHours * (60 * 60);
    if (lastCycleResetTimestamp > 0 && hardReset) {
      uint256 timeSinceReset = block.timestamp - lastCycleResetTimestamp;
      uint256 overtime = timeSinceReset > cycleSeconds
        ? timeSinceReset - cycleSeconds
        : 0;
  ...
  function _claimReward(address claimant, uint256 stakeAmount)
    internal
    returns (uint256)
  {
    ...
    if (stakeAmount == 0) {
      require(
        dehubToken.balanceOf(claimant) > 0,
        "Address has no DeHub tokens!"
      );
    }
    ...
    if (willReset) {
      ...
      require(
        !hasAlreadyClaimed(claimant),
        "Already claimed in the current cycle."
      );
    }

    uint256 bnbShare = calcCurrentClaimableShare(claimant, stakeAmount);
  ...
  function claimReward() external nonReentrant returns (uint256) {
    ...
    uint256 bnbShare = _claimReward(sender, 0);
  ...
  function sendReward(address holderAddr)
    ...
    returns (uint256)
  {
    uint256 bnbShare = _claimReward(holderAddr, 0);
   ...
   */
  function pullBnb(uint256 amount, address to) external onlyOwner nonReentrant {
    ...
    // Update claimable distribution with the new balance
    claimableDistribution = address(this).balance;
  ...
  function upgradeToV2() public {
    ...
    nextCycleResetTimestamp =
      lastCycleResetTimestamp +
      (claimCycleHours * (60 * 60));
  }

  function resetRewardDistribution(uint256 cycleHours)
    external
    onlyOwner
    returns (uint256)
  {
  ...
  /**
   * @notice Allows to change the staking contract address.
   */
  ...
  function sendRewardToStaker(address _stakerAddr, uint256 _stakeAmount)
    ...
    returns (uint256)
  {
    require(_stakerAddr != address(0), "Address can't be 0.");
    require(_stakeAmount > 0, "Stake amount can't be 0.");
    uint256 bnbShare = _claimReward(_stakerAddr, _stakeAmount);
    emit SentBNBRewards(bnbShare, _stakerAddr);
    return bnbShare;
  }
  ...
  function upgradeToV4() public {
    require(version < 4, "DeHubRewards: Already upgraded to version 4");
    version = 4;
