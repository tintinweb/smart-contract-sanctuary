...
contract MYTESTTT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public _maxTransactionAmount = 10000 * (10**9);
    uint256 public _maxWalletToken = 10000 * (10**9);  
    ...
    constructor() ERC20("MY TEST", "MYTEST") {
        buyFee.rewardFee = 2;
        buyFee.marketingFee = 4;
        buyFee.liquidityFee = 3;
        totalBuyFee = buyFee.rewardFee + buyFee.marketingFee + buyFee.liquidityFee;
        ...
        _marketingWallet = payable(address(0x291a65211b46C926Ef88B1A05c82F618E9E398f8)); 
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DTEST: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function addBotToBlackList(address[] calldata addresses) external onlyOwner {
    ...
    function updateBuyFees(uint16 rewardfee, uint16 marketing, uint16 liquidity) external onlyOwner {
    ...
    function updateSellFees(uint16 rewardfee, uint16 marketing, uint16 liquidity) external onlyOwner {
    ...
    function updateMaxWalletAmount(uint256 tokens) external onlyOwner {
        _maxWalletToken = tokens * (10**9);
    }  
    
    function updateMaxTx(uint256 tokens) external onlyOwner {
        _maxTransactionAmount = tokens * (10**9);
    }
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && to != uniswapV2Pair 
            ...
            //What if the buy transaction exceeds the max wallet amount?
            uint256 BalanceRecepient = balanceOf(to);
            require(BalanceRecepient + amount <= _maxWalletToken, "Exceeds max Wallet tokens amount.");
        }
        ...
        if(swapEnabled && !swapping && from != uniswapV2Pair && overMinimumTokenBalance) {
            ...
            uint256 marketingTokens = contractTokenBalance.mul(buyFee.marketingFee + sellFee.marketingFee).div(totalFee);
...
contract DTESTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DTEST_Dividend_Tracker", "DTEST_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**9); //must hold 1000 tokens
    }
 
    function _transfer(address, address, uint256) internal pure override {
        require(false, "DTEST_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() public pure override {
        require(false, "DTEST_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Token contract.");
    }
