...
contract Box{
    ...
    constructor() public {
       ...
       _setSaleMap(true, 30000000 * 10 ** 18, 100 *10 ** 18,1000 *10 ** 18, 1, 100, 1643112000 ,100, 3 ,1645804800,1648224000);
	}
    ...
    function plan(uint256 _usdtAmount, address _leader) public{
		...
		require(saleMap.maxUsdtAmount >= _usdtAmount,"too big");
        require(saleMap.minUsdtAmount <= _usdtAmount,"too small");
		...
		handling(msg.sender,_leader, pizzaAmount);
	...
	function handling(address _from, address _leader, uint256 _pizzaAmount) internal {
		...
		uint256 brokerage = _pizzaAmount.mul(saleMap.brokerageRate).div(1000);
			userMap[userMap[_from].leader].brokerage = userMap[userMap[_from].leader].brokerage.add(brokerage);
			userMap[userMap[_from].leader].invitations = userMap[userMap[_from].leader].invitations.add(1);
        }        
       
    }
	
	function installment() public{
		if(block.timestamp >= saleMap.twoPeriodTime && userMap[msg.sender].installmentTwo > 0){
			installmentForTwo();
		}
		if(block.timestamp >= saleMap.threePeriodTime && userMap[msg.sender].installmentThree > 0){
			installmentForThree();
		}
	}
	
	function unpaid(address _user)public view returns(uint256){
		uint256 amount = 0;
		if(block.timestamp >= saleMap.twoPeriodTime){
			amount = amount.add(userMap[_user].installmentTwo);
		}
		if(block.timestamp >= saleMap.threePeriodTime){
			amount = amount.add(userMap[_user].installmentThree);
		}
		return amount;
	}
    ...
    function setSaleMap( bool _status, 
						...
						uint256 _brokerageRate,
						uint256 _maxPurchases,
						uint256 _TwoPeriodTime,
						uint256 _threePeriodTime								
						)external onlyOwner{
			
		_setSaleMap(_status,_pizzaTotalAmount,_minUsdtAmount,_maxUsdtAmount,_usdtRate,_pizzaRate,_endSaletTime,_brokerageRate,_maxPurchases,_TwoPeriodTime,_threePeriodTime);
	}
    ...
    function _setSaleMap( bool _status, 
        ...
        saleMap.minUsdtAmount = _minUsdtAmount;
		...
		saleMap.brokerageRate = _brokerageRate;
