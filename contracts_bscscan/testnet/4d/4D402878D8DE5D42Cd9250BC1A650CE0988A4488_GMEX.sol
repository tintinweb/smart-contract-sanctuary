...
contract GMEX is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 portionForFees = contractTokenBalance.mul(5).div(8);
        uint256 halfOfLiquify = contractTokenBalance.mul(3).div(16);
        uint256 otherHalfOfLiquify = contractTokenBalance.mul(3).div(16);
        ...
        onlyOwner
    {
        ...
        uint256 amountToEach = amount.div(users.length);
        for (uint256 i = 0; i < users.length; i++) {
            _tokenTransfer(msg.sender, users[i], amountToEach, false);
        }
    }
