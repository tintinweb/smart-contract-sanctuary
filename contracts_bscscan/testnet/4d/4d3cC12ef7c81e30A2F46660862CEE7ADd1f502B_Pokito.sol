...
contract Pokito is Ownable, IZapper {
    ...
    constructor () public {
        ...
        //IBEP20(_PRNTR).safeApprove(address(ROUTER), uint(~0));
        //IBEP20(_BUSD).safeApprove(address(ROUTER), uint(~0));

        //approve to breakLP
        //IBEP20(_LPaddress).safeApprove(address(ROUTER), uint(~0));
        //IBEP20(_PRNTRBUSD).safeApprove(address(ROUTER), uint(~0));

        // set route pairs for our tokens
        //routePairAddresses[_PRNTR] = _BUSD;
    ...
    function zapTokenToLP(address _from, uint amount, address _to) public override {
        ...
        // PRNTR, PAPR, BUSD to create PRNTR-BUSD or PAPR-BUSD will hit this if
     ...
     * Not very useful for our current use case as bolt input currencies will only be BUSD, WBNB, PAPR, PRNTR
    ...
    function _swap(address _from, uint amount, address _to, address receiver) private returns (uint) {
        ...
        if (intermediate != address(0) && (_from == WBNB || _to == WBNB)) {
            ...
            // [PRNTR/PRNTR, BUSD, WBNB, xRoute, xToken]
            path = new address[](5);
            path[0] = _from;
            ...
            // E.g. [PRNTR/PAPR, BUSD, WBNB, xTokenWithWBNBLiquidity]
            path = new address[](4);
            path[0] = _from;
            path[1] = intermediate;
            path[2] = WBNB;
            ...
            // E.g. [xTokenWithWBNBLiquidity, WBNB, BUSD, PRNTR/PAPR]
            path = new address[](4);
            path[0] = _from;
            path[1] = WBNB;
            path[2] = intermediate;
