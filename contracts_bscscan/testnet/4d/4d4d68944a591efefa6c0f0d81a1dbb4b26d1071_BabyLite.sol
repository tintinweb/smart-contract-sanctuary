...
contract BabyLite is ERC20, Ownable {
    ...
    BabyLiteDividendTracker public babyLiteDividendTracker;
    ...
    event UpdateBabyLiteDividendTracker(address indexed newAddress, address indexed oldAddress);
    ...
    event ProcessedBabyLiteDividendTracker(
    	uint256 iterations,
    	uint256 claims,
    ...
    constructor() ERC20("BabyLite", "BLT") {
    	babyLiteDividendTracker = new BabyLiteDividendTracker();

    	marketingWallet = 0x5C1232C335c963a052F86A7949e6369dEacF9AdC;
    	teamWallet = 0xd6260481A3d19a209376663faed4876f1e91D63a;
        ...
        excludeFromDividend(address(babyLiteDividendTracker));
    ...
    receive() external payable {
  	...
  	function whitelistDxSale(address _presaleAddress, address _routerAddress) external onlyOwner {
        ...
        babyLiteDividendTracker.excludeFromDividends(_presaleAddress);
        excludeFromFees(_presaleAddress, true);

        babyLiteDividendTracker.excludeFromDividends(_routerAddress);
  	...
  	function prepareForPartherOrExchangeListing(address _partnerOrExchangeAddress) external onlyOwner {
  	    babyLiteDividendTracker.excludeFromDividends(_partnerOrExchangeAddress);
  	...
  	function updateEthDividendToken(address _newContract) external onlyOwner {
  	    ...
  	    babyLiteDividendTracker.setDividendTokenAddress(_newContract);
  	}
  	
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "BabyLite: The team wallet is already this address");
    ...
    function updateBabyLiteDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(babyLiteDividendTracker), "BabyLite: The dividend tracker already has that address");

        BabyLiteDividendTracker newBabyLiteDividendTracker = BabyLiteDividendTracker(payable(newAddress));

        require(newBabyLiteDividendTracker.owner() == address(this), "BabyLite: The new dividend tracker must be owned by the BabyLite token contract");

        newBabyLiteDividendTracker.excludeFromDividends(address(newBabyLiteDividendTracker));
        newBabyLiteDividendTracker.excludeFromDividends(address(this));
        newBabyLiteDividendTracker.excludeFromDividends(address(uniswapV2Router));
        newBabyLiteDividendTracker.excludeFromDividends(address(deadAddress));

        emit UpdateBabyLiteDividendTracker(newAddress, address(babyLiteDividendTracker));

        babyLiteDividendTracker = newBabyLiteDividendTracker;
    }
    
   
    function updateEthDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "BabyLite: Fee must be less than 6%");
    ...
    function excludeFromDividend(address account) public onlyOwner {
        babyLiteDividendTracker.excludeFromDividends(address(account));
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        ...
        if(value) {
            babyLiteDividendTracker.excludeFromDividends(pair);
        }
    ...
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        babyLiteDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }

    function updateClaimWait(uint256 claimWait) external onlyOwner {
        babyLiteDividendTracker.updateClaimWait(claimWait);
    }

    function getEthClaimWait() external view returns(uint256) {
        return babyLiteDividendTracker.claimWait();
    }
    
  
    function getTotalEthDividendsDistributed() external view returns (uint256) {
        return babyLiteDividendTracker.totalDividendsDistributed();
    }
    ...
    function withdrawableEthDividendOf(address account) external view returns(uint256) {
    	return babyLiteDividendTracker.withdrawableDividendOf(account);
  	}
 
	function ethDividendTokenBalanceOf(address account) external view returns (uint256) {
		return babyLiteDividendTracker.balanceOf(account);
	}
    ...
    function getAccountEthDividendsInfo(address account)
        external view returns (
            ...
            uint256,
            uint256) {
        return babyLiteDividendTracker.getAccount(account);
    }
	...
	function getAccountEthDividendsInfoAtIndex(uint256 index)
        external view returns (
            ...
            uint256,
            uint256) {
    	return babyLiteDividendTracker.getAccountAtIndex(index);
    }
    
  
	function processDividendTracker(uint256 gas) external onlyOwner {
		(uint256 ethIterations, uint256 ethClaims, uint256 ethLastProcessedIndex) = babyLiteDividendTracker.process(gas);
		emit ProcessedBabyLiteDividendTracker(ethIterations, ethClaims, ethLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function claim() external {
		babyLiteDividendTracker.processAccount(payable(msg.sender), false);
    }
    function getLastEthDividendProcessedIndex() external view returns(uint256) {
    	return babyLiteDividendTracker.getLastProcessedIndex();
    }

    function getNumberOfEthDividendTokenHolders() external view returns(uint256) {
        return babyLiteDividendTracker.getNumberOfTokenHolders();
    }
    ...
    function _transfer(
        ...
        try babyLiteDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try babyLiteDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}

        if(!swapping) {
	    	uint256 gas = gasForProcessing;

	    	try babyLiteDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
	    		emit ProcessedBabyLiteDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
	    	}
	    	catch {
    ...
    function swapAndSendEthDividends(uint256 tokens) private {
        ...
        transferDividends(ethDividendToken, address(babyLiteDividendTracker), babyLiteDividendTracker, ethDividends);
    }
...
contract BabyLiteDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BabyLite_Litecoin_Dividend_Tracker", "BabyLite_Litecoin_Dividend_Tracker", 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca) {
    	claimWait = 600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BabyLite_Litecoin_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BabyLite_Litecoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyLite contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "BabyLite_Litecoin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyLite_Litecoin_Dividend_Tracker: Cannot update claimWait to same value");
