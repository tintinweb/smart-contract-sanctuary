...
/*
    DROUGHT TOKEN - É um token criado juntamente com o projeto Fim da SECA, 
    que busca investir na produção de poços artesianos nas regiões mais afetadas 
    pela seca no Nordeste brasileiro, Muitas famílias hoje sofrem com a escassez de agua, 
    E vendo essa situação o Projeto se reuniu e criaram então o DROUGHT TOKEN.

    INFORMAÇÕES:

    SUPPLY: 250.000.000
    Taxas de Compra = 8%
    - Taxas essas divididas em:
            1% para os holders na xprópria moeda
            1% queima
            1% Liquidez
            5%  Carteira - Projetos e Mkt divididos em:
                2% projeto dos poços artesianos
                2% Marketing
                1% recompra
Taxas de Venda = 10%
    - Taxas essas divididas em:
            2% para os holders na própria moeda
            1% queima
            1% Liquidez
            5%  Carteira - Projetos e Mkt divididos em:
                2% projeto dos poços artesianos
...
is Context, IERC20, Ownable {
    ...
    address public  prjAddress = (0x2A86eDc88D76e93C99108B3542BD02b259Cdc146);

    uint256 private _tTotal = 250000000 * 10**_decimals;    // Supply do Token
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 public maxIn = 50000 * 10**_decimals;    // 50k - Initial max buy
    uint256 public maxOut = 20000 * 10**_decimals;   // 20k - Initial max sell
	uint256 public maxWallet = 200000 * 10**_decimals;     // 200k - Initial max Wallet	

    uint256 public numTokensToSwap = 20000 * 10**_decimals; // 20k
    /// PODE DAR MENSAGEM DE HONEYPOT SE VALOR ESTIVER > 0
	uint public OutPerSecond = 0; // 0 seconds - antiBot Sell > 0
    uint public InPerSecond = 0;	// 0 seconds  - antiBot Buy > 0
    ...
    string private constant _name = "DROUGHT TOKEN";
    string private constant _symbol = "DRG";

    struct feeRatesStruct {
        uint256 rfi; // reflection to holders
        uint256 project; // wallet that accumulates bnb
 ...
 //      IUniswapV2Router02 _PancakeSwapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //BSC mainnet
       IUniswapV2Router02 _PancakeSwapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //BSC Testnet
        ...
        projectAddress = payable(prjAddress); // wallet  project that accumulates bnb 
    ...
    function getFromProjectAddress() public view returns (address) {
        return prjAddress;
    }
    ...
    function setProjectAddress(address newAddress) public onlyOwner {
        prjAddress = newAddress;
    }
    
    
    function setBuyRates(uint256 rfi, uint256 project, uint256 liquidity, uint256 burn) public onlyOwner {
        buyRates.rfi = rfi;
        buyRates.project = project;
        buyRates.liquidity = liquidity;
        buyRates.burn = burn;
    }
    
    function setSellRates(uint256 rfi, uint256 project, uint256 liquidity, uint256 burn) public onlyOwner {
        sellRates.rfi = rfi;
        sellRates.project = project;
    ...
    // Set time between buy
    function setInPerSecond(uint timeForBuy) public onlyOwner {
        InPerSecond = timeForBuy;
    }
    // Set time between sell
    function setSellPerSecond(uint timeForSell) public onlyOwner {
        OutPerSecond = timeForSell;
    }		
    // Set tokens to swapTokensForBNB to  wallet projectAddress
    ...
    function TradingOn(bool enabled) public onlyOwner {
        Trading = enabled;
    }
    ...
    //set max buy tokens per transaction
    function setMaxInTokens(uint256 maxInTokens) public onlyOwner {
        maxIn = maxInTokens * 10**_decimals;
    }

    //set max sell tokens per transaction
    function setMaxOutTokens(uint256 maxOutTokens) public onlyOwner {
        maxOut = maxOutTokens * 10**_decimals;
    }
	
	//set max walllet tokens 
    function setMaxWallet(uint256 maxWalletPercent) public onlyOwner {
        maxWallet = maxWalletPercent * 10**_decimals;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount,to_return.rTransferAmount,to_return.rRfi,to_return.rProject,to_return.rLiquidity,to_return.rBurn) = _getRValues(to_return, tAmount, takeFee, _getRate());
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tProject = tAmount*appliedFees.project/1000;
        s.tLiquidity = tAmount*appliedFees.liquidity/1000;
        s.tBurn = tAmount*appliedFees.burn/1000;
        s.tTransferAmount = tAmount-s.tRfi -s.tProject -s.tLiquidity -s.tBurn; 
        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rProject, uint256 rLiquidity, uint256 rBurn) {
        ...
        rTransferAmount= rAmount- rRfi-rProject-rLiquidity-rBurn;

        return ( rAmount,  rTransferAmount,  rRfi,  rProject,  rLiquidity,  rBurn);
    }
    ...
    function _takeProject(uint256 rProject, uint256 tProject) private {
        contractBalance.project_balance+=tProject;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner() && to != address(1) && to != pancakeswapV2Pair){
            ...
            require(contractBalanceTo + amount <= maxWallet, "Transfer amount exceeds the maxWallet"); 
        }
        ...
        if (contractBalance.project_balance>= numTokensToSwap && !inSwapAndLiquify && from != pancakeswapV2Pair && swapAndLiquifyEnabled) {

            swapAndSendToProject(numTokensToSwap);
        }       
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        if(takeFee) {
            ...
            if(sender == pancakeswapV2Pair) {
                ...
                if(sender != owner() && recipient != owner() && recipient != address(1)){
					require(tAmount <= maxIn, "Transfer amount exceeds the maxIn per transaction.");//max buy
                    bool blockedPurchase = lockToBuyOrSellForTime(getFromLastPurchaseBuy(recipient), InPerSecond);
                ...
                if(sender != owner() && recipient != owner() && recipient != address(1)){
				    require(tAmount <= maxOut, "Transfer exceeds the maxOut per transaction.");
                    bool blockedSellTime = lockToBuyOrSellForTime(getFromLastSell(sender), OutPerSecond);
        ...
        emit Transfer(sender, address(this), s.tProject+s.tLiquidity);
    ...
    function swapAndSendToProject(uint256 tokenAmount) private lockTheSwap {
        ...
        contractBalance.project_balance-=tokenAmount;
