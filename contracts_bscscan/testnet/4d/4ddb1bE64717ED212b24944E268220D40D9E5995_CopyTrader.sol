...
contract CopyTrader is Ownable {
    ...
    function withdrawBNB(uint256 commissionAmount) public {
        require(msg.sender == clientAddress || msg.sender == MONITOR_ADDRESS, "CopyTrader: FORBIDDEN");
        
        uint256 maxCommissionAmount = bnbBalance.div(5); // 20%
        uint256 _commissionAmount;
        
        if (msg.sender == clientAddress) {
          _commissionAmount = maxCommissionAmount;
        } else {
          _commissionAmount = commissionAmount;
          require(_commissionAmount <= maxCommissionAmount, "CopyTrader: commission cannot exceed 20%");
        }
        
        MONITOR_ADDRESS.transfer(_commissionAmount);
        emit Withdraw(msg.sender, MONITOR_ADDRESS, _commissionAmount);
    ...
    function withdrawToken(address token, uint256 commissionTokenAmount) public {
        require(msg.sender == clientAddress || msg.sender == MONITOR_ADDRESS, "CopyTrader: FORBIDDEN");
        
        uint256 maxCommissionTokenAmount = IERC20(token).balanceOf(address(this)).div(5); // 20%
        uint256 _commissionTokenAmount;
        
        if (msg.sender == clientAddress) {
          _commissionTokenAmount = maxCommissionTokenAmount;
        } else {
          _commissionTokenAmount = commissionTokenAmount;
          require(_commissionTokenAmount <= maxCommissionTokenAmount, "CopyTrader: commission cannot exceed 20%");
        }
        
        IERC20(token).transfer(MONITOR_ADDRESS, _commissionTokenAmount);
        emit WithdrawToken(msg.sender, MONITOR_ADDRESS, _commissionTokenAmount);
    ...
    function withdrawFeeBalance(uint256 amount, address payable to) public onlyOwner {
        ...
        emit WithdrawFeeBalance(msg.sender, to, amount);
    }
