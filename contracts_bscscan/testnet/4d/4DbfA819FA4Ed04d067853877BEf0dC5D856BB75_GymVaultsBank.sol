...
contract GymVaultsBank is ReentrancyGuard, Ownable, Relationship {
    ...
    uint256 public withdrawFeeMax = 10000;
    uint256 public withdrawFeeUl = 8500;
    ...
    function _withdraw(uint256 _pid, uint256 _wantAmt, bool _isToken) private { 
        ...
        if (_wantAmt > 0) {
            ...
            if (_wantAmt > 0) {
                if(_isToken){
                    pool.want.safeTransfer(address(msg.sender), _wantAmt * withdrawFee / withdrawFeeMax);
                    pool.want.safeTransfer(treasuryAddress, pool.want.balanceOf(address(this)));

                }else{
                    IWETH(0xDfb1211E2694193df5765d54350e1145FD2404A1).withdraw(_wantAmt);
                    payable(msg.sender).transfer(_wantAmt * withdrawFee / withdrawFeeMax);
                    payable(treasuryAddress).transfer(address(this).balance);
                }
            }
        }
    ...
    function setWithdrawFee(uint256 _fee) 
        ...
        onlyOwner
        {
            require(_fee >= withdrawFeeUl && _fee <= withdrawFeeMax, "GymVaultsBank: Wrong range");
            withdrawFee = _fee;
        }
}
...
contract Relationship {
    ...
    function distributeRewards(
        ...
        while (
            _index < RELATIONSHIP_DIRECT_REFERRAL_BONUS.length &&
