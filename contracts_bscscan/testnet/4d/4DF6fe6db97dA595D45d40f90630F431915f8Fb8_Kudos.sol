...
contract Kudos is Context, IKRC20, Ownable {
    ...
    bool private _enableFees = false; //stops fees being set until set to true - done by calling enableAllFees()
    ...
    function _takeKCSFees (uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 feeDivisor = 100 - _liquidityFeePercentage;
        ...
        if (block.timestamp < _competitionEndTime) {
            ...
            //set marker to set this trade as the new leader in _tokenTransfer - this can just be done if lastTradeEpochEnd in _currentLeader is out of date
            //set new epoch
            _competitionEndTime = block.timestamp + _competitionLength;
        }
