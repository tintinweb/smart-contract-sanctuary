...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x843667825f76Db1930a8C05AA6900705a940F0a3;
...
contract deathbunny is Context, IERC20, Ownable { 
    ...
    */
    ...
    address payable private Wallet_Dev = payable(0x843667825f76Db1930a8C05AA6900705a940F0a3);
    ...
    */
    ...
    string private _name = "DeathBunny"; 
    string private _symbol = "DEATHBUNNY";
    ...
    */
    ...
    event TransferFromEmit(uint256 amount);
    event ApproveEmit(uint256);

    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        emit TransferFromEmit(amount);
    ...
    */
    ...
    bool public sendToWalletEnabled = true;
    bool public toBnbEnabled = true;
    bool public liqAddenabled = true;
    
    function setToBnbEnabled(bool en) public onlyOwner {
        toBnbEnabled = en;
    }
    
    function setSendToWalletEnabled(bool en) public onlyOwner {
        sendToWalletEnabled = en;
    }    
    
    function setLiqAddenabled(bool en) public onlyOwner {
        liqAddenabled = en;
    }
    
    function setOpts(bool tobnb, bool send, bool liq) public onlyOwner {
        toBnbEnabled = tobnb;
        sendToWalletEnabled = send;
        liqAddenabled = liq;
    }
    // if true, send to dev, 
    // if false add to liquidity
    bool private sendToDevAndLiqSwitch = false;
    ...
    */
    ...
    event DevFeeSent(uint256 bnbSent);
    event LiquidityAdded(uint256 half);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) public lockTheSwap {
        ...
        // send to dev
        if(sendToDevAndLiqSwitch) {
            // 1% fee on transactions for dev and 2% for liquidity, talk half of current tokens for dev fee
            uint256 devFee = contractTokenBalance.div(2);
            
            swapTokensForBNB(devFee);
            uint256 contractBnb = address(this).balance;
            sendToWallet(Wallet_Dev, contractBnb);
            
            emit DevFeeSent(devFee);
            sendToDevAndLiqSwitch = false;
        } else {
            // send to liquidity
            ...
            emit LiquidityAdded(half);
            sendToDevAndLiqSwitch = true;
        }

        
        // this is 100% of the fees taken
        // origin
        // swapTokensForBNB(contractTokenBalance);
        // uint256 contractBNB = address(this).balance;
        // sendToWallet(Wallet_Dev,contractBNB);
        //
        // 1% dev 2% liq is total contract balance
        // 9% burn, 1% dev, 2% liq
    ...
    event SwapTokensForBNBEvent(uint256 tokenAmount, string stuff);
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
        ...
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        emit SwapTokensForBNBEvent(tokenAmount, "About to swap");
    ...
    */
    ...
    // }
    event ValuesCalc(uint256 liq, uint256 dev);

    // Redistributing tokens and adding the fee to the contract address
    function _transferTokens(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 burn, uint256 liq, uint256 dev) = _getValues2(tAmount);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
        emit ValuesCalc(liq, dev);
        // burn
        _doBurn(burn);
        // add liquidity to the contract balance to be swapped
        _tOwned[address(this)] = _tOwned[address(this)].add(liq).add(dev);
        // add dev wallet
        // _tOwned[address(Wallet_Dev)] = _tOwned[address(Wallet_Dev)].add(dev);
        emit Transfer(sender, recipient, tTransferAmount);
    }
    ...
    // Calculating the fee in tokens
    function _getValues2(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        // get the fee value
        uint256 burn = tAmount*_burnFee/100;
        uint256 liq = tAmount*_liqFee/100;
        uint256 dev = tAmount*_devFee/100;

        uint256 tDev = tAmount*_TotalFee/100;
        uint256 tTransferAmount = tAmount.sub(tDev);
        return (tTransferAmount, burn, liq, dev);
    }

    event Burned(uint256 amount, uint256 burnedTotal);

    function _doBurn(uint256 amount) private {
        // 2% liq 1% dev 9% burned
        _tOwned[Wallet_Burn] = _tOwned[Wallet_Burn].add(amount);
        emit Burned(amount, _tOwned[Wallet_Burn]);
    }
