...
import "./ReactionTransferHelper.sol";

contract Reaction is ERC20, Ownable {
	...
	ReactionDividendTracker public dividendTracker;
	ReactionTransferHelper private transferHelper;

	address public deadWallet = 0x000000000000000000000000000000000000dEaD;

	address public immutable USD = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;
	...
	constructor(address routerAddress, address payable marketingWalletAddress) public ERC20("Reaction", "RTC") {
		dividendTracker = new ReactionDividendTracker();
		transferHelper = new ReactionTransferHelper(routerAddress);
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "Reaction: The dividend tracker already has that address");

		ReactionDividendTracker newDividendTracker = ReactionDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "Reaction: The new dividend tracker must be owned by the Reaction token contract");
	...
	function updateUniswapV2Router(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "Reaction: The router already has that address");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "Reaction: Account is already the value of 'excluded'");
	...
	function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
		require(pair != uniswapV2Pair, "Reaction: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
	...
	function setMaxWalletAmount(uint256 amount) public onlyOwner {
		require(amount <= totalSupply(), "Reaction: Amount cannot be over the total supply.");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "Reaction: Automated market maker pair is already set to that value");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "Reaction: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "Reaction: Cannot update gasForProcessing to same value");
	...
	function swapAndSendDividends(uint256 amountIn) private {
		uint256 dividends = swapExactETHForTokens(amountIn, USD);
...
contract ReactionDividendTracker is Ownable, DividendPayingToken {
	using SafeMath for uint256;
	...
	constructor() public DividendPayingToken("Reaction_Dividend_Tracker", "Reaction_Dividend_Tracker") {

	}

	function _transfer(address, address, uint256) internal override {
		require(false, "Reaction_Dividend_Tracker: No transfers allowed");
	}

	function withdrawDividend() public override {
		require(false, "Reaction_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Reaction contract.");
	}
	...
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "Reaction_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "Reaction_Dividend_Tracker: Cannot update claimWait to same value");
