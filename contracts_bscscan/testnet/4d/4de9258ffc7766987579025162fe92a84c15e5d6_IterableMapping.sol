...
//     Wen BUSD Smart Contract - https://t.me/WenBUSD
//
//     Hey!, wanna get free BUSD Token by just holdings a token?
//     check Real BUSD Rewards (WENBUSD) you can earn BUSD by just
//     holdings thiz token... Didn't believe me?, try it you will 
//     proof it by yourself
//
//     Tokenomics:
//     10% BUSD reflection
//     3% liquidity rereflection
//     2% Marketing Fee
//
...
// File: contracts/WENBUSDDividendPayingToken.sol
...
contract WENBUSD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    WENBUSDDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("Wen BUSD", "WENBUSD") {
    	...
    	dividendTracker = new WENBUSDDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "WENBUSD: The dividend tracker already has that address");

        WENBUSDDividendTracker newDividendTracker = WENBUSDDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "WENBUSD: The new dividend tracker must be owned by the WENBUSD token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "WENBUSD: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "WENBUSD: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "WENBUSD: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "WENBUSD: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "WENBUSD: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "WENBUSD: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "WENBUSD: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "WENBUSD: This account cannot send tokens until trading is enabled");
        }
...
contract WENBUSDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("WENBUSD_Dividend_Tracker", "WENBUSD_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "WENBUSD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "WENBUSD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WENBUSD contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "WENBUSD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "WENBUSD_Dividend_Tracker: Cannot update claimWait to same value");
