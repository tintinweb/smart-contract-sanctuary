    ...
    Treasury
{
    ...
    uint256 public totalAllocationAmount;
    uint256 public currentAllocationAmount;
    ...
    function initialize(
        ...
        totalAllocationAmount = _totalAllocationAmount;
    ...
    function deposit() public payable virtual whenNotPaused {
        ...
        currentAllocationAmount = currentAllocationAmount + msg.value;
        ...
        whenNotPaused
    {
        ...
        addTokenInfoAmount(currency, amount);
    ...
    function claim() public virtual whenNotPaused {
        ...
        require(
            allowanceToUserInfo[msg.sender].claimed == false,
            "You have already claimed"
        );
        require(
            allowanceToUserInfo[msg.sender].deposit > 0,
            "You don't have an allocation to clame"
        );
        ...
        uint256 claimAmount = _tokenInfo.pricePerOne *
            allowanceToUserInfo[msg.sender].deposit;
        IERC20(token).transfer(msg.sender, claimAmount);

        allowanceToUserInfo[msg.sender].claimed = true;
        claimCount++;
    ...
    function transferToken(address to) public onlyRole(MANAGER_ROLE) {
        uint256 idoBalance = address(this).balance;
        IERC20(token).transfer(to, idoBalance);
    }
    ...
    function addTokenInfoAmount(IERC20 _token, uint256 _amount) private {
        uint256 pid = getTokenInfoId(_token);
        tokenInfo[pid].amount = tokenInfo[pid].amount + _amount;
    }
    ...
    function _deposirRequire(address sender, uint256 amount) private view {
        ...
        require(amount >= minDeposit, "Value is less than min allowed");
        require(amount <= maxDeposit, "Value is more than max allowed");

        uint256 currentAllocationAmountValue = currentAllocationAmount + amount;

        require(
            currentAllocationAmountValue <= totalAllocationAmount,
