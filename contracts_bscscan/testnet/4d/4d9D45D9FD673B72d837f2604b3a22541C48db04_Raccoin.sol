/**
   ...
   + 10% fee auto add to the liqudity pool when selling above a certain amount in a given time period
...
contract Raccoin is ERC20("Raccoin", "RACN"), Ownable {
    ...
    //max amount of single transfer, 15k
    uint256 public _maxSellAmountTx = 15 * 10**3 * 10**_decimals;
    ...
    function _transfer(
        ...
        //selling token
        if (to == uniswapV2Pair) {
            //check to avoid selling in big chunks
            require(
                amount <= _maxSellAmountTx,
                "RACN: Sell amount exceeds the maximum allowed."
            );
            ...
            if (antiDump.count.add(amount) > numTokensDumpPenalty) {
                ...
                //`numTokensDumpPenalty` - `_maxSellAmountTx` + 1
                //if the next tx is `_maxSellAmountTx`, then this fee will activate
    ...
    function setMaxSellAmountTxPercent(uint256 maxSellPercent) external onlyOwner {
        _maxSellAmountTx = _tTotal.mul(maxSellPercent).div(100);
    }
