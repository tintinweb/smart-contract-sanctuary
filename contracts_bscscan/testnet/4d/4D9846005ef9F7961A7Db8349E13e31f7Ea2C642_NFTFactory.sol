...
contract NFTFactory is Context, ReentrancyGuard, VRFConsumerBase {
  ...
  mapping(uint256 => mapping(uint256 => uint256)) private editionMapping;
  ...
  function getNFTInfo(uint256 _id) public view returns(uint256 id, uint256 card, uint256 suit, uint256 edition) {
    NFTInfo storage nft = nfts[_id];
    require(nft.cardInfoId >= 0, "NFT doesn't exist on this id");
    CardInfo storage cardNft = deckInfo[nft.cardInfoId];
    return (_id, cardNft.num, cardNft.kind, nft.edition);
  }
   ...
   */
  function getEditionValue(uint256 _card, uint256 _suit) public view returns(uint256) {
    return editionMapping[_card][_suit];
  }
   ...
   */
  function upgradeCards(uint256[] memory _ids, bytes memory _data) external whenNotPaused {
    require(_msgSender().isContract() == false, "Factory.create: No contracts are permitted");
    require(_msgSender() != address(0), "Factory.create: sender address is ZERO");
    address creator = _msgSender();
    ...
    newNFT.edition = editionMapping[newCardInfo.num][newCardInfo.kind] + 1;
    editionMapping[newCardInfo.num][newCardInfo.kind]++;
    ...
    emit CardUpgraded(_ids, id, newNFT.edition, newCardInfo.num, newCardInfo.kind);
  }
  ...
  function generateNFTInfo(bytes32 _requestId, uint256 _randomness) private {
    ...
    nft.edition = editionMapping[cardInfo.num][cardInfo.kind] + 1;
    editionMapping[cardInfo.num][cardInfo.kind]++;

    emit NFTCreated(config.id, cardInfo.num, cardInfo.kind, nft.edition);
  }
  ...
  /**
    @notice get next edition id
    */
  function _getNextEditionID() private view returns(uint256) {
    return editionId + 1;
  }
  ...
  /**
    ...
    */
  function _incrementEditionId() private {
    editionId ++;
  }
