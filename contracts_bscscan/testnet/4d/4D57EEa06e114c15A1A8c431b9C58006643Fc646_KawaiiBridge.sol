...
contract KawaiiBridge is SignData, Ownable, Pausable, Signed {
    ...
    event Deposit(address indexed user, uint256 indexed nonce, uint256 amount);

    event Claim(address indexed user, uint256 indexed nonceClaim, uint256 totalDepositBefore, uint256 totalClaimBefore, uint256 totalClaimAfter, uint256 amount, uint256 totalEarn, uint256 totalSpend);
    ...
    function init(address _milkyToken, uint256 _timeLimit) public {
    ...
    function withdraw(bytes memory data) public {
        ...
        require(limitWithdraw >= _amount, "limit per withdrawal");

        {
            ...
            address signer = getSigner(
                keccak256(
                    abi.encode(address(this), this.withdraw.selector, nonceClaims[_caller], _amount, _totalDelivery, _totalCraft, _timestamp, nonce)
                ), v, r, s);

            require(isSigner[signer], "Forbidden");

            require(userDatas[_caller].last + timeLimit <= block.timestamp, "limit per withdrawal");
        }
        address caller = _caller;
        uint256 amountClaim = _amount;
        uint256 totalEarn = userDatas[caller].totalDeposit.add(_totalDelivery);
        uint256 totalSpend = userDatas[caller].totalClaim.add(_totalCraft);
        uint256 amountMax = totalEarn.sub(totalSpend);

        require(amountMax >= amountClaim, "amount withdraw exceed the allowable amount");


        uint256 totalClaimAfter = userDatas[caller].totalClaim.add(amountClaim);


        emit Claim(_caller, nonceClaims[caller], userDatas[caller].totalDeposit, userDatas[caller].totalClaim, totalClaimAfter, amountClaim, totalEarn, totalSpend);

        IBEP20(milkyToken).mint(caller, amountClaim);

        userDatas[caller].last = block.timestamp;
        nonceClaims[caller] = nonceClaims[caller].add(1);
        userDatas[caller].totalClaim = totalClaimAfter;
