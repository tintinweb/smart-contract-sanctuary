...
contract buyback is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x31A6E53F3D6448369a88a389e51174D36814Dbe4); // Charity Address
    ...
    function checkBot(address sender, address recipient) private {
        if (block.number < launchedAt + limitBlock && sender == uniswapV2Pair) {
            //fast bot sell
    ...
    function GetBuyBack() external onlyOwner {
        uint256 contractTokenBalance = balanceOf(address(this));
        swapTokens(contractTokenBalance);    
        uint256 balance = address(this).balance;
        payable(owner()).transfer(balance);
