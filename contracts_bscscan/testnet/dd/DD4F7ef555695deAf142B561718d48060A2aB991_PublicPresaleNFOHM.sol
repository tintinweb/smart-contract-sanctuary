...
contract NewOlypmusAccess is Context {
    ...
    modifier onlyAuthorized() {
        require(
            ...
            "PRESALE NFOHM: onlyAuthorized() - caller not authorized"
        );
        _;
    }
    ...
    modifier onlyPendingIceCreamMan() {
        require(
            ...
            "PRESALE NFOHM: onlyPendingIceCreamMan() - caller not pendingICM"
        );
        _;
    }
    ...
    modifier onlyCreamery() {
        require(
            creamery == _msgSender(),
            "PRESALE NFOHM: onlyCreamery() - caller not creamery"
        );
        _;
    }
    ...
    modifier onlyNewOlypmusToken() {
        require(
            ...
            "PRESALE NFOHM: onlyNewOlypmusToken() - caller not newOlypmusToken"
        );
        _;
    }
    ...
    modifier onlyIceCreamMan() {
        require(
            ...
            "PRESALE NFOHM: onlyIceCreamMan() - caller not iceCreamMan"
        );
        _;
    }
 }
...
contract NewOlypmusTimedPresale is NewOlypmusAccess {
    ...
    string internal startTimeString 
        = 'Tue Oct 19 2021 21:00:00 GMT-0700 (Pacific Daylight Time)';
    uint256 internal presaleTimestampSTART = 1634702400;
    ...
    function checkFluxCapacitor(uint256 start, uint256 end) internal pure {
        ...
        require(
            start < end,
            "PRESALE NFOHM: Hey, McFly! END must be after START!"
        );
    }
    ...
    function checkEndOrDurationAreZero(uint256 duration, uint256 end) internal pure {
        require(
            ...
            "PRESALE NFOHM: Can only set END or DURATION, not both"
        );
    }
    ...
    function checkTooLate() internal view {
        ...
        require(
            ...
            "PRESALE NFOHM: checkTooLate() - Too Late! Presale has ended."
        );
    }
    ...
    function checkTooEarly() internal view {
        ...
        require(
            ...
            "PRESALE NFOHM: checkToEarly() - Too Early! Presale has not started."
        );
    }
    ...
    function checkOverridesAllowedTimedPresale() internal view {
        require(
            ...
            "PRESALE NFOHM: Overrides After Contract Launch Are Disabled."
        );
    }
    ...
    function checkTimedPresaleIsNotInitialized() internal view {
        require(
            ...
            "PRESALE NFOHM: Presale has already been initialized."
        );
    }
...
contract PublicPresaleNFOHM is NewOlypmusTimedPresale {
    using SafeMath for uint256;
    
    // type declarations
    INewOlypmus internal NFOHM;
    ...
    // The amount of NFOHM the holder has claimed
    mapping(address => uint256) internal claimedNFOHM;
...
//         on all failures
    ...
    function getMaxClaimableNFOHM() internal view returns (uint256) {
        // multiply, then apply decimals => (a*b)/(10^d)
        return
            (maxHolderContribution.mul(nfOHMPerNativeCoin))
                .div(10**DECIMALS_BNB);
    }
    ...
    function getRemainingMaxClaimableNFOHM(address holder)
        internal
        view
        returns (uint256)
    {
        if (enforceWhitelist){
            if (whitelist[holder]) {
                // subtract => a-b
                return getMaxClaimableNFOHM().sub(claimedNFOHM[holder]);
            } else {
                return 0;
            }
        } else {
    ...
    function getHoldersClaimableNFOHM(address holder)
        internal
        view
        returns (uint256)
    {
        ...
        return (
            (contributions[holder].mul(nfOHMPerNativeCoin)
                .div(10**DECIMALS_BNB))
                .sub(claimedNFOHM[holder])
        );
    }
    ...
    function getMyInfo()
        ...
        returns (
            ...
            uint256 holdersClaimableNFOHM,
            uint256 holderContributions_,
            uint256 claimedNFOHM_,
        ...
        )
    {
        return (
            ...
            getHoldersClaimableNFOHM(holder),
    ...
    */
    ...
    function _contribute(address holder, uint256 amount) internal {
        ...
        emit ContributionReceived(
            ...
            getRemainingBNBcontribution(holder),
            globalTotal_contributions,
            "PRESALE NFOHM: Contribution Received!"
        );
    }
    ...
    function claim_OWL() external {
        ...
        if(softCapHit()){
            uint256 amount = checkHoldersClaimableNFOHM(holder);
    ...
    /**
        ...
        @notice requires that, if the transfer is successful, the holders
            claimedNFOHM does not exceed the holders total claimable NFOHM
    ...
    function _claim(address holder, uint256 amount) internal {
        require(
            amount <= getRemainingMaxClaimableNFOHM(holder),
            "PRESALE NFOHM: _claim() - claim exceeds remaining unclaimed NFOHM"
        );
        // update the values in the contract FIRST, then process the transfer
        globalTotal_claims = globalTotal_claims.add(amount);
        claimedNFOHM[holder] = claimedNFOHM[holder].add(amount);
        checkHoldersClaimedNFOHM(holder);
        
        if (getRemainingMaxClaimableNFOHM(holder) == 0) {
            completedClaims[holder] = true;
        }
        require(
            processClaim(holder, amount),
            "PRESALE NFOHM: _claim() - transfer of claimed tokens failed."
        );
    }
    ...
    /**
        @notice Handles the actual transfer of claimed NFOHM to the holder
        @dev Internal private function may be called by any function within
            this contract
        @param holder The holder address who will receive the NFOHM tokens
        @param amount The total amount of NFOHM to transfer to the holder
        ...
        returns (bool)
    {
        ...
        if (useTokensInContract) {
            return NFOHM.transfer(holder, amount);
        ...
        // transfers them to the holder. 
            ...
            NFOHM.presaleClaim(address(this), amount);
            return NFOHM.transfer(holder, amount);
        }
    }

    /**
        @notice used to set the addresses of NFOHM and Creamery
        @dev External function callable by onlyIceCreamMan
        @param nfOHM The NewOlypmus Contract address which we are contributing to
    ...
    */
    ...
    /**
        ...
        @param nfOHMPerNativeCoin_ The rate for calculating claimed tokens.
    ...
    function setRate_OICM(
        ...
        onlyIceCreamMan
    {
        nfOHMPerNativeCoin = nfOHMPerNativeCoin_.mul(10**DECIMALS_NFOHM);
    }
    ...
    function setAddressNFOHM(address newOlypmusToken_) internal {
        newOlypmusToken = newOlypmusToken_;
        NFOHM = INewOlypmus(newOlypmusToken);
    }
    ...
    */
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "PRESALE NFOHM: adminTokenWithdrawal() - insufficient balance"
        );
    ...
    /**
        ...
        @param claimedNFOHM_ the holders total claimed NFOHM
    ...
    */
        ...
        internal
    {
        ...
        claimedNFOHM[holder] = claimedNFOHM_;
    ...
    function checkHoldersClaimedNFOHM(address holder) internal view {
        require(
            claimedNFOHM[holder] <= getMaxClaimableNFOHM(),
            "PRESALE NFOHM: _claim() - Claim exceeds total claimable NFOHM"
        );
     }
    ...
    function checkBatchLength(uint256 batchLength) internal view {
        require(
            ...
            "PRESALE NFOHM: batchAddHolder() - list length exceeds max"
        );
     }
    ...
    function checkTransferSuccess(bool success) internal pure {
        require(
            success,
            "PRESALE NFOHM: checkTransferSuccess() - transferFailed"
        );
     }
    ...
    function checkRefundsEnabled() internal view {
        require(
            softCapMissedRefundEnabled,
            "PRESALE NFOHM: The missed soft cap refunds have not been enabled"
        );
     }
    ...
    function checkHoldersClaimableRefund(
        ...
        returns (uint256)
     {
        ...
        require(
            value > 0,
            "PRESALE NFOHM: checkHoldersClaimableRefund() - holder has no refund to claim"
        );
        return value;
     }

    function checkHoldersClaimableNFOHM(
        address holder
     )
        internal
        view
        returns (uint256)
     {
        uint256 amount = getHoldersClaimableNFOHM(holder);
        require(
            amount > 0,
            "PRESALE NFOHM: checkHoldersClaimableNFOHM() - holder has no tokens to claim"
        );
        return amount;
     }
    ...
    function checkHolderCompletedClaims(address holder) internal view {
        require(
            !completedClaims[holder],
            "PRESALE NFOHM: checkHolderCompletedClaims() - holder already hit max claims"
        );
     }
    ...
    function checkBlacklist(address holder) internal view {
        require(
            !blacklist[holder],
            "PRESALE NFOHM: checkBlacklist() - WALLET BLACKLISTED! What did you do?"
        );
     }
    ...
    function checkWhitelist(address holder) internal view {        
        if(enforceWhitelist){
            require(
                whitelist[holder],
                "PRESALE NFOHM: checkWhitelist() - NOT WHITELISTED! https://newOlypmusbsc.com/"
            );
        }
     }
    ...
    function checkHolderMaxContribution(
        ...
        require(
            ...
            "PRESALE NFOHM: value exceeds holder's remaining allowed contribution"
        );
     }
    ...
    function checkHolderMinContribution(uint256 value) internal view {
        require(
            ...
            "PRESALE NFOHM: value is less than the minimum contribution"
        );
     }
    ...
    function checkNotInitialized() internal view {
        require(
            !initialized,
            "PRESALE NFOHM: checkNotInitialized() - Already Initialized!"
        );
     }
    ...
    function checkContributionsEnabled() internal view {
        ...
        if(shouldCheckContributionsEnabled) {
            ...
            require(
                contributionsEnabled,
                "PRESALE NFOHM: checkContributionsEnabled() - Contributions not enabled."
            );
        }
     }
    function checkHardCapHit() internal view {        
        require(
            !hardCapHit(),
            "PRESALE NFOHM: Cap Reached! ...ask a dev to increase it"
        );
     }
    ...
    function checkNewHardCap(uint256 newHardCap) internal view {
        require(
            ...
            "PRESALE NFOHM: hardCap must be more than total contributions"
        );
     }
    ...
    function checkNewSoftCap(
        ...
        pure
     {
        require(
            ...
            "PRESALE NFOHM: checkNewSoftCap() - softCap must be less than hardCap"
        );
     }
    ...
    function checkClaimsEnabled() internal view {
        require(
            claimsEnabled,
            "PRESALE NFOHM: checkClaimsEnabled() - Claiming NFOHM is not enabled."
        );
     }
    ...
    function checkSoftCapHit() internal view {
        require(
            ...
            "PRESALE NFOHM: checkSoftCapHit() - Soft Cap Not Hit."
        );
     }
