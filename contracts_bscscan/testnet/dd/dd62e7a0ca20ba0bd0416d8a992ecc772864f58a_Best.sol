...
interface IAirdropFundWallet {
    function setLockSettings(uint256 _minTokenBalance, uint256 _duration) external;
    function sendToken(address bridgeAirdropAddress) external payable;
}
...
contract AirdropFundWallet is IAirdropFundWallet {
    ...
    function setLockSettings(uint256 _minTokenBalance, uint256 _duration) external override onlyToken {
        minTokenBalance = _minTokenBalance;
        duration = _duration * 1 days;
    }
    
    function sendToken(address bridgeAirdropAddress) external payable override onlyToken {
            require(temp_lock_time <= block.timestamp, "The lock is on");
            uint256 token_balance = bep_token.balanceOf(bridgeAirdropAddress);
            if(token_balance < minTokenBalance){
                uint256 tokenToSend = minTokenBalance.mul(5).sub(token_balance);
...
contract Best is IBEP20, Auth {
    ...
    string constant _name = "2Rey";
    string constant _symbol = "2rtt";    
    ...
    uint256 setPriceTimestampInterval = 10 days;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(volumeLoops){ try resender.resend() {} catch {} }
        
        if(launched()) { updateCurrentPrice(); }
        if(setPriceTimestamp <= block.timestamp){ updatePastPrice(); }

        try distributor.process(distributorGas) {} catch {}
        try airdropfund.sendToken(bridgeAirdropAddress) {} catch {}
...
//        emit Transfer(sender, recipient, amount);
    ...
    function updateCurrentPrice() internal {
       currentPrice = address(pair).balance.div(balanceOf(address(pair)));
    }
    ...
    function swapBack() internal swapping {
        ...
        payable(airdropFeeReceiver).transfer(amountBNBAirdrop);
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _airdropFeeReceiver) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        airdropFeeReceiver = _airdropFeeReceiver;
    }
    ...
    function setLockSettings(uint256 _minTokenBalance, uint256 _duration) external authorized {
        airdropfund.setLockSettings(_minTokenBalance, _duration);
    }
