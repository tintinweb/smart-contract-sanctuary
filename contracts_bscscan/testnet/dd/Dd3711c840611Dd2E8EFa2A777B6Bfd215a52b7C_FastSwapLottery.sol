...
contract FastSwapLottery is ReentrancyGuard, IFastSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInFast = 50 ether;
    uint256 public minPriceTicketInFast = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInFast;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] fastPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInFast;
        uint32 finalNumber;
    }
    ...
    constructor(address _fastTokenAddress, address _randomGeneratorAddress) {
        fastToken = IERC20(_fastTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of Fast to this contract
        uint256 amountFastToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInFast,
            _ticketNumbers.length
        );

        // Transfer fast tokens to this contract
        fastToken.safeTransferFrom(address(msg.sender), address(this), amountFastToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInFast += amountFastToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInFastToTransfer
        uint256 rewardInFastToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInFastToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        fastToken.safeTransfer(msg.sender, rewardInFastToTransfer);

        emit TicketsClaim(msg.sender, rewardInFastToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInFast) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in FAST for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].fastPerBracket[j] =
                ...
                // A. No FAST to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].fastPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInFast - amountToShareToWinners);

        // Transfer Fast to treasury address
        fastToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        fastToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInFast += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInFast >= minPriceTicketInFast) && (_priceTicketInFast <= maxPriceTicketInFast),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInFast: _priceTicketInFast,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            fastPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInFast: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(fastToken), "Cannot be FAST token");
    ...
    function setMinAndMaxTicketPriceInFast(uint256 _minPriceTicketInFast, uint256 _maxPriceTicketInFast)
        external
        onlyOwner
    {
        require(_minPriceTicketInFast <= _maxPriceTicketInFast, "minPrice must be < maxPrice");

        minPriceTicketInFast = _minPriceTicketInFast;
        maxPriceTicketInFast = _maxPriceTicketInFast;
    }
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].fastPerBracket[_bracket];
        } else {
            return 0;
        }
    }
