...
contract BunnyMintingStation is AccessControl {
    ...
    constructor(AnpansBunnies _anpansBunnies) public {
    ...
    function mintCollectible(
        ...
        uint256 tokenId =
            anpansBunnies.mint(_tokenReceiver, _tokenURI, _bunnyId);
        return tokenId;
    }
    ...
    function setBunnyName(uint8 _bunnyId, string calldata _bunnyName)
        ...
        onlyOwner
    {
        anpansBunnies.setBunnyName(_bunnyId, _bunnyName);
    }
...
// File: contracts/utils/AnpansProfile.sol
...
contract BunnySpecialV1 is Ownable {
    ...
    AnpansProfile public anpansProfile;
    ...
    function mintNFT(uint8 _bunnyId) external {
        ...
        (userId, , , , , isUserActive) = anpansProfile.getUserProfile(
            senderAddress
        );
        ...
        if (bunnyCharacteristics[_bunnyId].anpanCost > 0) {
            anpanToken.safeTransferFrom(
                senderAddress,
                address(this),
                bunnyCharacteristics[_bunnyId].anpanCost
            );
        }
    ...
    function addBunny(
        ...
        emit BunnyAdd(_bunnyId, _thresholdUser, _anpanCost);
    }
    ...
    function updateBunny(
        ...
        bunnyCharacteristics[_bunnyId].anpanCost = _anpanCost;
        bunnyCharacteristics[_bunnyId].isActive = _isActive;

        emit BunnyChange(_bunnyId, _thresholdUser, _anpanCost, _isActive);
    }
        ...
        returns (bool)
    {
        if (!anpansProfile.hasRegistered(_userAddress)) {
            ...
            (userId, , , , , userStatus) = anpansProfile.getUserProfile(
                _userAddress
            );
