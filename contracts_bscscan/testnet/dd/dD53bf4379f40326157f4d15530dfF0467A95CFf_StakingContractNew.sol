...
contract StakingContractNew is AccessControl {
    ...
    function changeRewardsPerBlock(uint256 _numberOfBlocks) public onlyRole(GOVERNANCE_ROLE) {
        massUpdatePools();
        uint256 rewardBal = rewardToken.balanceOf(address(this));
        rewardsPerBlock = rewardBal.div(_numberOfBlocks);
    }
        ...
        returns (uint256)
    {
        ...
        if (totalAllocPoint == 0) {
            ...
            return
                user.amount.mul(accRewardsPerShare).div(1e12).sub(
                    user.rewardDebt
                );
        }
        if (user.amount == 0) {
    ...
    function distributeReward(uint256 _pid, uint256 _amount) internal {
        ...
        UserInfo storage user = userInfo[_pid][msg.sender];
        uint256 rewardBal = rewardToken.balanceOf(address(this));
        require(_amount <= rewardBal, "Not enough Rewards");
        uint256 pending = user
            .amount
            .mul(pool.accRewardsPerShare)
            .div(1e12)
