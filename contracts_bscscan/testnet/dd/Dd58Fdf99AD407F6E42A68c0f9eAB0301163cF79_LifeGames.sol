...
contract LifeGames is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 1000000000000000000000000; // 1M for easy tests, need change on future
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyDevFee)
        external
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyDevFee <= 1500, "Must keep buy taxes below 15%");
        //emit SetBuyFee(buyMarketingFee, buyLiquidityFee, buyTaxFee);
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellDevFee)
        external
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee  + _sellDevFee <= 2000, "Must keep sell taxes below 20%");
