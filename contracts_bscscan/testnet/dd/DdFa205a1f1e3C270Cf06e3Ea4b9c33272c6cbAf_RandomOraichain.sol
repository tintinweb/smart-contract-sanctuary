...
import "./interfaces/IRandomOraichain.sol";
import "./oraichain/interfaces/IVRFOracleOraichain.sol";


contract RandomOraichain is IRandomOraichain, Ownable{

    using SafeERC20 for IERC20;
    uint internal fee;
    address public oraiToken;
    //address(0xE6487f7BdEB798e2457E44570f367a3059Ed9F93);
    address public oraichainOracle;
    //address(0xd962B50c33934890e3DC392a5e7517D314404cA7);
     uint256 public random;

    mapping (bytes32 => uint) randomResults;
    mapping (bytes32 => uint) heroIds;

    IKabyHero public kabyHero;
    
    modifier onlyKabyHero() {
        require(
            msg.sender == address(kabyHero),
            "RAMDOMIZE: Only call by kaby hero contract"
        );
        _;
    }
     ...
     * @param kabyHeroAddress new KabyHero contract address
     */
    function setKabyHero(IKabyHero kabyHeroAddress) external onlyOwner {
        require(address(kabyHeroAddress) != address(0));
        kabyHero = kabyHeroAddress;
    }

    function requestRandomNumberForStar(uint heroId, uint256 _seed) external override onlyKabyHero returns (bytes32 requestId) {

        IERC20(oraiToken).approve(oraichainOracle, IVRFOracleOraichain(oraichainOracle).getFee());

        bytes memory data = abi.encode(address(this), this.fulfillRandomness.selector);

        requestId = IVRFOracleOraichain(oraichainOracle).randomnessRequest(_seed, data);
        heroIds[requestId] = heroId;
    }

    function fulfillRandomness(bytes32 requestId) external {
        random = uint(requestId);
        kabyHero.setRandomStarByOraichain(
            heroIds[requestId],
            random
        );
...
interface IKabyHero {
    ...
    event UpdateRandomOraichain(address newRandomOraichain);
     ...
     */
    function setRandomStarByChainlink(uint heroId, uint randomness) external;

    /**
     * @notice Set random star
     */
    function setRandomStarByOraichain(uint heroId, uint randomness) external;

}

pragma solidity ^0.8.0;

interface IRandomOraichain {
    function requestRandomNumberForStar(uint heroId, uint256 _seed ) external returns (bytes32);
}
