...
contract MetaDNA is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    modifier onlyWhitelisted() {
        require(isWhitelisted(msg.sender), "The address is not in the whitelist");
        _;
    }
    ...
    function updateWhitelistSwitch(bool _whitelistSwitch) public onlyOwner {
    ...
    function getRich(uint256 tokenId) public pure returns (string memory) {
        return pluck(tokenId, "RICH");
    }
    
    function getLook(uint256 tokenId) public pure returns (string memory) {
        return pluck(tokenId, "APPEARNCE");
    }
    
    function getSpeed(uint256 tokenId) public pure returns (string memory) {
        return pluck(tokenId, "EQ");
    }

    function getSpirit(uint256 tokenId) public pure returns (string memory) {
        return pluck(tokenId, "SPIRIT");
    }
    
    function getWisdom(uint256 tokenId) public pure returns (string memory) {
        return pluck(tokenId, "IQ");
    }
    
    function pluck(uint256 tokenId, string memory keyPrefix) internal pure returns (string memory) {
        uint256 rand = random(string(abi.encodePacked(keyPrefix, toString(tokenId))));
        string memory output = toString(rand % 101);
