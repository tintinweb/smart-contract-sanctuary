...
contract EthToTokenExchangeContract {
    ...
    mapping (uint=> address ) private b_holders; //incremented for each address , add check if exsts
    mapping (address => uint) private b_shares; //incremented for each transaciont
