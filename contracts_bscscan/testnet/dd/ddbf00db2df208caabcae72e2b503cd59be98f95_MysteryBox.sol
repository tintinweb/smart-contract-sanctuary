...
contract MysteryBox is Initializable, AccessControlUpgradeable {
...
contract Trainers is Initializable, ERC721Upgradeable, AccessControlUpgradeable, ITransferCooldownable {
    ...
    */
    ...
    function _reforgeMinLevel(uint8 level) internal pure returns(uint8) {
        if (level <= 9) {
            return 0;
        }
        else if (level <= 49) {
    ...
    function _reforgeMaxLevel(uint8 level) internal pure returns(uint8) {
        if (level <= 9) {
            return 9;
        }
        else if (level <= 49) {
    ...
    function _levelMax(uint256 id) internal view returns(uint8) {
        ...
        // if trainer has reforged, move to next max level
        if (_trainerReforgedLevel[id] == maxLevel) {
            if (maxLevel == 9) {
                return 49;
            }
            else if (maxLevel == 249) {
                return 255;
            }
            else {
                return maxLevel + 50;
...
contract Bunicorns is Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    /*
        ...
        cosmetics: 0-255 but only 24 is used, may want to cap so future expansions dont change existing bunicorns
    */
    ...
    struct BunicornBurnPoints {
        uint8 lowStarBurnPoints;
        uint8 fourStarBurnPoints;
        uint8 fiveStarBurnPoints;
    }

    struct BunicornCosmetics {
        uint8 version;
        uint256 seed;
    }

    Bunicorn[] private tokens;
    BunicornCosmetics[] private cosmetics;
    mapping(uint256 => BunicornBurnPoints) burnPoints;
    ...
    function getCosmetics(uint256 id) internal view
        ...
        BunicornCosmetics memory wc = cosmetics[id];
    ...
    function _get(uint256 id) internal view
        ...
        BunicornBurnPoints memory wbp = burnPoints[id];
        _burnPoints =
    ...
    function mint(address minter, uint256 seed) public restricted returns(uint256) {
        ...
        return mintBunicornWithStars(minter, stars, seed);
    }

    function mintBunicornByElement(address minter, uint256 seed, uint256 element) public restricted returns(uint256) {
        uint256 stars;
        uint256 roll = seed % 100;
        // will need revision, possibly manual configuration if we support more than 5 stars
        if(roll < 1) {
        ...
        return mintBunicornByElementWithStars(minter, stars, seed, element);
    }

    function mintBunicornWithStars(address minter, uint256 stars, uint256 seed) public restricted returns(uint256) {
        require(stars < 8, "Stars parameter too high! (max 7)");
        (uint16 stat1, uint16 stat2, uint16 stat3) = getStatRolls(stars, seed);

        return performMintBunicorn(minter,
    ...
    function mintBunicornByElementWithStars(address minter, uint256 stars, uint256 seed, uint256 element) public restricted returns(uint256) {
        require(stars < 8, "Stars parameter too high! (max 7)");
        (uint16 stat1, uint16 stat2, uint16 stat3) = getStatRolls(stars, seed);

        return performMintBunicorn(minter,
            getRandomPropertiesWithoutElement(stars, seed, element),
            stat1,
            stat2,
            stat3,
            RandomUtil.combineSeeds(seed,3)
    ...
    function performMintBunicorn(address minter,
        uint16 properties,
        ...
        tokens.push(Bunicorn(properties, stat1, stat2, stat3, 0));
        cosmetics.push(BunicornCosmetics(0, cosmeticSeed));
        _mint(minter, tokenID);
        durabilityTimestamp[tokenID] = uint64(now.sub(getDurabilityMaxWait()));

        emit NewBunicorn(tokenID, minter);
        return tokenID;
    }
    ...
    function getRandomPropertiesWithoutElement(uint256 stars, uint256 seed, uint256 element) public pure returns (uint16) {
        return uint16((stars & 0x7) // stars aren't randomized here!
            | ((element & 0x3) << 3) // trait
            | ((RandomUtil.randomSeededMinMax(0,124,RandomUtil.combineSeeds(seed,2)) & 0x7F) << 5)); // statPattern
    }
    ...
    function getPowerMultiplier(uint256 id) public view noFreshLookup(id) returns (int128) {
        ...
        Bunicorn memory bunicorn = tokens[id];
        int128 powerPerPoint = ABDKMath64x64.divu(1, 400); // 0.25% or x0.0025
        int128 stat1 = bunicorn.stat1.fromUInt().mul(powerPerPoint);
        int128 stat2 = bunicorn.stat2.fromUInt().mul(powerPerPoint);
        int128 stat3 = bunicorn.stat3.fromUInt().mul(powerPerPoint);
    ...
    function getPowerMultiplierForTrait(
        ...
        // Does not include trainer trait to bunicorn trait match
        // Only counts arbitrary trait to bunicorn stat trait
    ...
    function _calculateBurnValues(uint256 burnID) public view returns(uint8[] memory) {
        ...
        // Carried burn points.
        BunicornBurnPoints storage burningbp = burnPoints[burnID];
        ...
        Bunicorn storage burning = tokens[burnID];
    ...
    function reforge(uint256 reforgeID, uint256 burnID) public restricted {
        ...
        BunicornBurnPoints storage wbp = burnPoints[reforgeID];
        emit Reforged(
            ownerOf(reforgeID),
            reforgeID,
            burnID,
    ...
    function getBonusPower(uint256 id) public view noFreshLookup(id) returns (uint24) {
        Bunicorn storage bunicorn = tokens[id];
        return getBonusPowerForFight(id, bunicorn.level);
    }

    function getBonusPowerForFight(uint256 id, uint8 level) public view noFreshLookup(id) returns (uint24) {
        BunicornBurnPoints storage wbp = burnPoints[id];
    ...
    function getFightData(uint256 id, uint8 trainerTrait) public view noFreshLookup(id) returns (int128, int128, uint24, uint8) {
        Bunicorn storage bunicorn = tokens[id];
        return (
            oneFrac.add(powerMultPerPointBasic.mul(
                    ABDKMath64x64.fromUInt(
                        bunicorn.stat1 + bunicorn.stat2 + bunicorn.stat3
                    )
            )),//targetMult
            getPowerMultiplierForTrait(bunicorn.properties, bunicorn.stat1, bunicorn.stat2, bunicorn.stat3, trainerTrait),
            getBonusPowerForFight(id, bunicorn.level),
            getTraitFromProperties(bunicorn.properties)
        );
    }

    function getFightDataAndDrainDurability(uint256 id, uint8 trainerTrait, uint8 drainAmount) public
