...
contract Farm is UnicornMint{
    ...
    uint256 constant internal THRESHOLD = 2000000000 * 1e18 ;
    ...
    function getRate(uint _month) public view returns (uint) {
        uint _mi = _month%12;
        return BASE_RATE.mul(MRATE**_mi)/(100**_mi);
    }
    ...
    function autoMint() internal returns( uint macMint, uint macPure, uint macBurn ) {
        ...
        macMint = multiplier.mul(macsPerBlock).div(BASE_INIT);
    ...
    function updateReward(bytes32 _to,address minter) internal {
        require(isMinter(minter),"error minter");
    ...
    function rewardPerToken(uint macactul) public view returns(uint256) {
        if (totalSupply == 0) {
            return rewardPerTokenStored;
        }
        return rewardPerTokenStored.add(
            macactul.mul(1e18).div(totalSupply)
        );
    }

    function earned(bytes32 account) public view returns (uint256) {
        (uint256 multiplier,uint256 curHash) = getMultiplier(lastUpdateBlock,block.number);
        uint macactul = multiplier.mul(macsPerBlock).mul(curHash).div(THRESHOLD).div(BASE_INIT);
        return
            balances[account].amount
                .mul(
                    rewardPerToken(macactul).sub(balances[account].rewardPerTokenPaid)
                )
                .div(1e18)
                .add(rewards[account]);
        ...
        returns (uint256 multiplier,uint256 curHash)
    {
        ...
        uint fromPeriod = month(_from);
        uint toPeriod   = month(_to);
        uint _startBlock = _from;
        
        for(;fromPeriod<=toPeriod;fromPeriod++){
            uint _endBlock = monthEndBlock(fromPeriod);
            if(_to<_endBlock) _endBlock = _to;
            multiplier = multiplier.add(
                _endBlock.sub(_startBlock).mul(getRate(fromPeriod))
            );
    ...
    function monthEndBlock(uint256 _period) public view returns (uint) {
        return startBlock.add((_period+1).mul(REDUCE_PERIOD));
    }

    function month(uint256 blockNumber) public view returns (uint _period) {
...
contract UnicornFarm is Ownable,IERC721Receiver {
    ...
    function withdraw(uint pid,uint pindex) public {
        ...
        uint reward = farm.withdraw(msg.sender,pawn.tokenId);
        mac.safeTransfer(msg.sender,reward);
    }
