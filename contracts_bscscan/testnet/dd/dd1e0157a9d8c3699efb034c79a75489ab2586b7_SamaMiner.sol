...
// File: contracts/multisender/SAMAMiner.sol
...
contract SamaMiner is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. SAMAs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that SAMAs distribution occurs.
        uint256 accSamaPerShare; // Accumulated SAMAs per share, times 1e12.
    ...
    // Set the number of sama produced by each block
    ...
    // Update the given pool's SAMA allocation point. Can only be called by the owner.
    ...
    function getSamaBlockReward(uint256 _lastRewardBlock) public view returns (uint256) {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 blockReward = getSamaBlockReward(pool.lastRewardBlock);
        if (blockReward <= 0) {
            return;
        }
        uint256 samaReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
        bool minRet = sama.mintMine(address(this), samaReward);
        if (minRet) {
            pool.accSamaPerShare = pool.accSamaPerShare.add(samaReward.mul(1e12).div(lpSupply));
        }
    ...
    // View function to see pending SAMAs on frontend.
    function pending(uint256 _pid, address _user) external view returns (uint256, uint256){
        uint256 samaAmount = pendingSama(_pid, _user);
        return (samaAmount, 0);
    }

    function pendingSama(uint256 _pid, address _user) private view returns (uint256){
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accSamaPerShare = pool.accSamaPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (user.amount > 0) {
            if (block.number > pool.lastRewardBlock) {
                uint256 blockReward = getSamaBlockReward(pool.lastRewardBlock);
                uint256 samaReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
                accSamaPerShare = accSamaPerShare.add(samaReward.mul(1e12).div(lpSupply));
                return user.amount.mul(accSamaPerShare).div(1e12).sub(user.rewardDebt);
            }
            if (block.number == pool.lastRewardBlock) {
                return user.amount.mul(accSamaPerShare).div(1e12).sub(user.rewardDebt);
            }
    ...
    // Deposit LP tokens to SAMAMiner for SAMA allocation.
    function deposit(uint256 _pid, uint256 _amount) public notPause {
        depositSama(_pid, _amount, msg.sender);
    }

    function depositSama(uint256 _pid, uint256 _amount, address _user) public notPause {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pendingAmount = user.amount.mul(pool.accSamaPerShare).div(1e12).sub(user.rewardDebt);
            if (pendingAmount > 0) {
                safeSamaTransfer(_user, pendingAmount);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accSamaPerShare).div(1e12);
        emit Deposit(_user, _pid, _amount);
    }

    // Withdraw LP tokens from SAMAMiner.
    function withdraw(uint256 _pid, uint256 _amount) public notPause {
        withdrawSama(_pid, _amount, msg.sender);
    }

    function withdrawSama(uint256 _pid, uint256 _amount, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        require(user.amount >= _amount, "withdrawSama: not good");
    ...
    function emergencyWithdraw(uint256 _pid) public notPause {
        emergencyWithdrawSama(_pid, msg.sender);
    }

    function emergencyWithdrawSama(uint256 _pid, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
    ...
    // Safe SAMA transfer function, just in case if rounding error causes pool to not have enough SAMAs.
    function safeSamaTransfer(address _to, uint256 _amount) internal {
        uint256 samaBal = sama.balanceOf(address(this));
        if (_amount > samaBal) {
            sama.transfer(_to, samaBal);
        } else {
            sama.transfer(_to, _amount);
        }
    }
