...
contract A4Sale is Ownable  {
    ...
    event TokenWithdrowed(address, uint256);


    constructor(IERC20 token, IERC20 currency, address bank,  uint256 cap, uint256 releaseTime) public
    {
        _token = token;
        _currency = currency;
        _bank = bank;
    ...
    function releaseTime() view public returns (uint256) {
        return _releaseTime;
    }
    ...
    function withdrawIDO( uint256 value ) onlyOwner public {
        require(_releaseTime <= block.timestamp, "IDO is active");
    ...
    {
        require(_releaseTime > block.timestamp, "IDO is stopped");
    ...
    {
        require(_releaseTime > block.timestamp, "IDO is stopped");
        require(value <= _deposits[msg.sender] && value > 0, "Error value");
    ...
    function _withdrawTokens() internal
    {
        ...
        _token.transferFrom(_bank, msg.sender, tokens);
