...
╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═╝╚═╝     ╚═╝╚═╝  ╚═╝
    ...
    Max wallet size : 1% (excluding DEV wallet 2%)
    Max Transaction size : Sells are restricted to less than 0.1% of the total supply + extra 3% fee is applied to all sells.
    BNB reflection : 10% auto distribution 
    LP fee : 5%
*/
...
contract Kurma is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public devWallet = 0xd93F24b0244D2CC691A35eF6FB3aAE40218c2C44;

    DividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 1 * 10 ** 12 * (10**9);
    uint256 public swapTokensAtAmount = 2 * 10 ** 10 * (10**9);
    uint256 public maxHoldAmount = 1 * 10 ** 13 * (10 ** 9);
    ...
    constructor() public ERC20("Kurma Token", "KURMA") {
	...
	function processDividendTracker(uint256 gas) external {
		...
		(uint256 iterations, uint256 claims, uint256 lastProcessedIndex) = dividendTracker.process{gas:gas}(gas);
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[to] && to != uniswapV2Pair){
            if (to == devWallet){
                require(balanceOf(to) < maxHoldAmount * 2, "Dev wallet hold amount can't exceeds 2% of total supply!");
            } else {
                require(balanceOf(to) < maxHoldAmount, "Wallet hold amount can't exceeds 1% of total supply!");
            }
        }
        ...
        if(!swapping) {
            ...
            require(gasleft() >= gas, "Out of gas, please increase gas limit and retry!");
	    	try dividendTracker.process{gas:gas}(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
