...
contract TokenCrowdsale is Ownable, Crowdsale, TimedCrowdsale {
    ...
    uint256 public maxBuy = 3 * 1e18;
    uint256 public maxBuyTokens;
    uint256 public totalSale;
    uint256 public totalContributors;
    mapping(address => uint256) public seedOrder;
    ...
    public
    {
        maxBuyTokens = _getTokenAmount(maxBuy);
        maxBuyTokens = maxBuyTokens.div(10**(18 - _token.decimals()));
    }
    ...
    function buyTokens(address beneficiary) public nonReentrant payable {
        ...
        if (seedOrder[beneficiary] == 0) {
            totalContributors = totalContributors.add(1);
        }

        // _processPurchase(beneficiary, tokens);
        seedOrder[beneficiary] = seedOrder[beneficiary].add(tokens);
        require(seedOrder[beneficiary] <= maxBuyTokens, 'Out of the max buy amount');
    ...
    function claim() public {
        ...
        uint256 tokenBalance = seedOrder[msg.sender];
        seedOrder[msg.sender] = 0;
        excluded(msg.sender);
        _processPurchase(msg.sender, tokenBalance);
    }

    function pushClaim(uint256 _amount) public onlyOwner {
        require(canClaim, 'can not claim now');
        ...
        for(uint256 i = 0; i < leftOver; i++) {
            ...
            uint256 tokenBalance = seedOrder[account];
            seedOrder[account] = 0;
            excluded(account);
    ...
    function updateWalletAddress(address payable _newWallet) public onlyOwner {
        _wallet = _newWallet;
    }
}