...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  //address public immutable ETHToken = address(0x2170Ed0880ac9A755fd29B2688956BD959F933F8); // MainNet
  //address public immutable ETHToken = address(0x8BaBbB98678facC7342735486C851ABD7A0d17Ca);  // TestNet
  address public  earnToken = address(0x8BaBbB98678facC7342735486C851ABD7A0d17Ca);  // TestNet
  ...
  function distributeBTCDividends(uint256 amount) public {
    require(totalSupply() > 0);
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
     ...
     // bool success = IERC20(ETHToken).transfer(user, _withdrawableDividend);
      bool success = IERC20(earnToken).transfer(user, _withdrawableDividend);
      if(!success) {
...
//   function updateEarnToken(address earnToken_) internal  {
//         earnToken = earnToken_;
//   }
}

// File: contracts/RewardSwitch.sol

pragma solidity ^0.7.6;

contract RewardSwitch is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    //address public marketingWallet = 0x21CC6B1E8868ae103d0659C933Fe4044580d6898; 
    address private marketingWallet = 0x7D9A3dA641a5dd9feE57DD768681F7FA36A5C602;

    bool private swapping;

    RewardSwitchDividendTracker public dividendTracker;

    address private liquidityWallet;

    uint256 public maxSellTransactionAmount = 10000 * (10**18);
    uint256 public swapTokensAtAmount = 3000 * (10**18);
    uint256 private maxWalletAmount=  40000 * (10**18);
    uint256 public BNBRewardsFee;
    uint256 public liquidityFee;
	uint256 public marketingFee;
    uint256 public totalFees;

    uint256 private sellFeeIncreaseFactor = 110;
    ...
    uint256 public tradingEnabledTimestamp = 1629536400;  // 	Sat Aug 21 2021 09:00:00 GMT+0000
    ...
    constructor()  ERC20("RewardSwitch", "RSE1") {
    	...
    	dividendTracker = new RewardSwitchDividendTracker();
        ...
        */
        ...
        //////////////////////
        _approve(owner(), address(uniswapV2Router), 1000000 * (10**18));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "RewardSwitch: The dividend tracker already has that address");

        RewardSwitchDividendTracker newDividendTracker = RewardSwitchDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "RewardSwitch: The new dividend tracker must be owned by the RewardSwitch token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "RewardSwitch: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "RewardSwitch: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "RewardSwitch: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "RewardSwitch: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "RewardSwitch: The liquidity wallet is already this address");
    ...
    function updateFeeInternal(uint256 feeLP_ ,uint256 feeMarketing_ ,uint256 feeDividends_) public onlyOwner {
        feeLP = feeLP_;
    ...
    function updateProjectFee(uint256 newLiquidityFee, uint256 newMarketingFee, uint256 newRewardsFee ) public onlyOwner {
    ...
    function updateEnabledTimestamp(uint256 newTradingEnabledTimestamp) public onlyOwner {
        tradingEnabledTimestamp = newTradingEnabledTimestamp;
    }
    
    function updateEnabledSwapMode(bool marketing_, bool liquidity_ , bool reward_ , bool enSwapFee_ ,bool enTakeFee_, bool enWalletLimit_,bool enMaxSellTransaction_) public onlyOwner {
    ...
    function updateTransactionAmount(uint256 maxSellTransactionAmount_ ,uint256 swapTokensAtAmount_ ,uint256 maxWalletAmount_) public onlyOwner {
        maxSellTransactionAmount = maxSellTransactionAmount_ * (10**18);
        swapTokensAtAmount = swapTokensAtAmount_ * (10**18);
        maxWalletAmount = maxWalletAmount_ * (10**18);
    }
   
    function updateEarnToken(address earnToken_) public onlyOwner {
        earnToken = earnToken_;
        dividendTracker.updateEarnToken(earnToken_);
    }
    
    function calulateFee(uint256 amount_, address to_) internal returns (uint256){
        	uint256 fees = amount_.mul(totalFees).div(100);

            if( automatedMarketMakerPairs[to_] ) {
                fees = fees.mul(sellFeeIncreaseFactor).div(100);
            }

            uint256 _feeDividends = fees.mul(BNBRewardsFee).div(totalFees);
            uint256 _feeMarketing = fees.mul(marketingFee).div(totalFees);
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "RewardSwitch: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "RewardSwitch: Cannot update gasForProcessing to same value");
    ...
    function getMaxSellTransactionAmount() external view returns (uint256,uint256,uint256) {
        return (maxSellTransactionAmount,swapTokensAtAmount,maxWalletAmount);
    }
   
    //function getEnabledSwapMode() external view returns (bool ,bool,bool,bool ,bool,bool ,bool) {
    function getEnabledSwapMode() external view returns (bool swapFee_ ,bool marketing_,bool LP_,bool dividends_,bool takeFee_,bool enWalletLimit_ ,bool enMaxSellTransaction_) {
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from_], "RewardSwitch: This account cannot send tokens until trading is enabled");
        }
        ...
        if(	!swapping &&
        	tradingIsEnabled &&
            !_isExcludedFromFees[to_]  &&//no max for those excluded from fees
            (automatedMarketMakerPairs[from_] || automatedMarketMakerPairs[to_]) && // sell only by detecting transfer to automated market maker pair
        ...
        if(canSwap && !swapping && !automatedMarketMakerPairs[from_] ){
            ...
            if(enSwapAndLiquify && (feeLP > swapTokensAtAmount )){
                uint256 _tAmount = balanceOf(address(this));
                if(_tAmount >= feeLP){
                    swapAndLiquify(swapTokensAtAmount);
                    feeLP = feeLP.sub(swapTokensAtAmount);
                }
                else{
            ...
            if(enSwapTokensForEth && (feeMarketing > swapTokensAtAmount) ){
                uint256 _bAmount = address(this).balance;
                uint256 _tAmount = balanceOf(address(this));
                if(_tAmount >= feeMarketing){
                    swapTokensForEth(swapTokensAtAmount);
                    feeMarketing = feeMarketing.sub(swapTokensAtAmount);
                }
                else{
            ...
            if(enSwapAndSendDividends && (feeDividends > swapTokensAtAmount) ){
                uint256 _tAmount = balanceOf(address(this));
                if(_tAmount >= feeDividends){
                    swapAndSendDividends(swapTokensAtAmount);
                    feeDividends = feeDividends.sub(swapTokensAtAmount);
                }
                else{  
        ...
        if(takeFee && enTakeFee) {
            uint256 fees = calulateFee(amount_ ,to_);
...
contract RewardSwitchDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()  DividendPayingToken("RewardSwitch_Dividend_Tracker", "RewardSwitch_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 5000 * (10**18); //must hold 5000+ tokens
    }

    function  _transfer(address, address, uint256) internal pure override {
        require(false, "RewardSwitch_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "RewardSwitch_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RewardSwitch contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "RewardSwitch_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "RewardSwitch_Dividend_Tracker: Cannot update claimWait to same value");
