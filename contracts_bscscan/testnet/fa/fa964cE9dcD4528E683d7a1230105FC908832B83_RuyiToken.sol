...
contract RuyiToken is Context, IERC20, Ownable, ITokensRecoverable {
    ...
    function _tokenTransferStandard(
        ...
        uint256 StakingPoolAmount = amount.mul(StakingPoolTax).div(100);
        uint256 marketingAmount = amount.mul(MarketingTax).div(100);
        uint256 AddToLiquidityAmount = amount.mul(LiquidityPoolTax).div(100);
        ...
        if (contractTokenBalance >= maxTxAmt) {
            contractTokenBalance = maxTxAmt;
        }
        ...
        emit Transfer(sender, address(this), AddToLiquidityAmount);
        emit Transfer(sender, recipient, amount);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 initialBalance = _busd.balanceOf(address(this));

        // swap tokens for ETH
        swapTokensForBUSD(half); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = _busd.balanceOf(address(this)).sub(initialBalance);
        ...
        onlyOwner
    {
        ...
        if (initialAdd) {
            initialAdd = false;
            lockTime = block.timestamp + 300 seconds;
        }
    }
