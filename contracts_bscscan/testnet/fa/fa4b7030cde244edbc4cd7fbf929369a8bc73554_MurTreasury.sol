...
contract MurTreasury is Ownable {

    address public murStakingManager;
    ...
    modifier onlyCounterParty {
        require(murStakingManager == msg.sender || insuranceFund == msg.sender, "not authorized");
        _;
    }

    constructor(IMurToken _mur) {
        mur = _mur;
    }

    function myBalance() public view returns (uint256) {
        return mur.balanceOf(address(this));
    }

    function mint(address recipient, uint256 amount) public onlyCounterParty {
        if(myBalance() < amount){
            mur.mint(address(this), calulateMintAmount(amount));
        }
        mur.treasuryTransfer(recipient, amount);
    }
    ...
    function setMurStakingManager(address _newAddress) public onlyOwner {
        murStakingManager = _newAddress;
    }
    ...
    function setPosition(IMurToken _newMur) public onlyOwner {
        mur = _newMur;
    }
    ...
    function calulateMintAmount(uint256 amount) private view returns (uint256 amountToMint) {
        uint256 baseAmount = mur.BASE_MINT();
