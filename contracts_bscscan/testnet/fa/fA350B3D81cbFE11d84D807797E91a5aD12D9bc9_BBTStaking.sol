...
interface IEACAggregatorProxy
{
  ...
  contract BBTStaking {
    ...
    =            WRITE FUNCTIONS            =
    ==========================================*/
    function buy_(address _user, uint256 tokenAmount) external returns(uint256)
    {
      ...
      uint tkn = purchaseTokens(_user, BBTToken);
      return tkn;
    }
    //address _referredBy,
    function buy( uint256 tokenAmount) public payable returns(uint256)
    {
      ...
      uint tkn = purchaseTokens(msg.sender, BBTToken);
      return tkn;
    }
