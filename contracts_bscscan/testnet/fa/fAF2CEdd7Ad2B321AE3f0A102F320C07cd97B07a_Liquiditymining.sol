...
contract Liquiditymining is ReentrancyGuard{
    ...
    constructor(address _owner, address _token, address routerAddress){
        owner = _owner;
        token = IBEP20(_token);
    ...
    //to receive ETH from external call
    ...
    function liquidityMining(uint256 tokenAmount)
        public
        payable
        nonReentrant
        isHuman
    {
        ...
        uint256 bonus = tokenAmount.mul(bonusPercent).div(1e3);
        token.transferFrom(owner, msg.sender, bonus);
    }
