    ...
    UUPSUpgradeable
{
  ...
  */
    ...
    function stackApe(uint256 apeNftId, uint256 buildingId) public {
        ...
        //Checks para building: owner, stackeado, no lleno, type
        

        addressToApesType[sender][apesInstance.getTypeIdByNftId(apeNftId)].push(
                apeNftId
            );
  ...
  */
    ...
    function claimRewardsWithoutUnStack(uint256 genericType, address sender)
        private
    {
        uint256 currentTimestamp = block.timestamp;
        require(
    ...
    function claimSpecialRewards(uint256 apeNftID) public returns (uint256) {
        ...
        require(
            ...
            "This APE has already claimed its reward"
        );
        require(isOwner, "You aren't the owner of this APE");
        require(
            apeInstance.getTypeIdByNftId(apeNftID) > 0,
            "Baby Ape doesn't generate rewards"
        );
        IAGold aGoldInstance = IAGold(aGoldAddress);
        uint256 apeAge = getApeAge(apeNftID);
        if (apeAge <= specialStartBlockTime) {
            uint256 totalRewards = ((specialEndBlockTime -
                specialStartBlockTime) / 3600) * rewardFactor; //Delta Hours
            aGoldInstance.transfer(address(msg.sender), totalRewards);
            NftIDClaimed[apeNftID] = 1;
            emit SpecialRewardClaimed(apeNftID, totalRewards);
            return totalRewards;
        } else {
            uint256 totalRewards = ((specialEndBlockTime - apeAge) / 3600) *
                rewardFactor;
            aGoldInstance.transfer(address(msg.sender), totalRewards);
            NftIDClaimed[apeNftID] = 1;
            emit SpecialRewardClaimed(apeNftID, totalRewards);
    ...
    function calculateSpecialRewards(uint256 apeNftID)
        public
        view
        returns (uint256)
    {
        ...
        if (apeAge <= specialStartBlockTime) {
            uint256 totalRewards = ((specialEndBlockTime -
                specialStartBlockTime) / 3600) * rewardFactor;
            return totalRewards;
        } else {
            uint256 totalRewards = ((specialEndBlockTime - apeAge) / 3600) *
                rewardFactor;
