...
contract DuckFactory is Ownable {
    ...
    uint128 public presaleStartDate;
    uint128 public presaleEndDate;
    mapping(address => uint256) public presaleAddressToToken;
    ...
    function transferDuckiesOwnership(address _newFactory) public onlyOwner() {
        Duckies(tokenAddress).transferOwnership(_newFactory);
    }
    ...
    function openTrading(uint256 _tokenAmount, uint256 _bnbAmount) external onlyOwner() {
        ...
        token.approve(address(uniswapV2Router), _tokenAmount);
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(tokenAddress, uniswapV2Router.WETH());
        uniswapV2Router.addLiquidityETH{value: _bnbAmount}(address(token),_tokenAmount,0,0,owner(),block.timestamp);
        swapEnabled = true;
        tradingOpen = true;
...
contract Duckies is ERC20, Ownable {

    constructor(uint256 __totalSupply) ERC20("Duckies", "DUCKIE") {
