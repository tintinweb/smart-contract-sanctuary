...
//SPDX-License-Identifier: Unlicensed 
    ...
    contract TENIS20 is IBEP20, Auth {
        using SafeMath for uint256;
        ...
        string constant _name = "TENIS20";
        string constant _symbol = "TENIS20";
        uint8 constant _decimals = 9;
    
        uint256 _totalSupply = 1000000000000 * (10 ** _decimals); // 1,000,000,000,000 (1M)
        ...
        function swapBack() internal swapping {
            if(liquidityFeeAccumulator >= swapThreshold && autoLiquifyEnabled){
                ...
                uint256 amountBNBProject = amountBNB.mul(projectFee).div(totalFee);
                ...
                (success, ) = payable(projectFeeReceiver).call{value: amountBNBDev, gas: 30000}("");
                if(success){ projectFees = projectFees.add(amountBNBProject); }
        ...
        function setFees(
            ...
            totalFee = buybackFee.add(reflectionFee).add(marketingFee).add(teamFee).add(projectFee);
            ...
            emit FeesUpdated(_enabled, _liquidityFee, _buybackFee, _reflectionFee, _marketingFee, _teamFee, _projectFee, _feeDenominator);
        }
    
        function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _teamFeeReceiver, address _projectFeeReceiver) external authorized {
            autoLiquidityReceiver = _autoLiquidityReceiver;
            marketingFeeReceiver = _marketingFeeReceiver;
            teamFeeReceiver = _teamFeeReceiver;
            projectFeeReceiver = _projectFeeReceiver;
            emit FeeReceiversUpdated(_autoLiquidityReceiver, _marketingFeeReceiver, _teamFeeReceiver, _projectFeeReceiver);
        }
        ...
        function getAccumulatedFees() external view returns (uint256, uint256,uint256) {
            return (marketingFees, teamFees, projectFees );
        }
        ...
        event FeesUpdated(bool enabled, uint256 liquidityFee, uint256 buybackFee, uint256 reflectionFee, uint256 marketingFee, uint256 teamFee,uint256 projectFee, uint256 feeDenominator);
        event FeeReceiversUpdated(address autoLiquidityReceiver, address marketingFeeReceiver, address teamFeeReceiver, address projectFeeReceiver);
