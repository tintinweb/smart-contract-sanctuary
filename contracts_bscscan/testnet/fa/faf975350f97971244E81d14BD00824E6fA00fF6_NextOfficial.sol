...
contract BNB_DISTRIBUTOR is IDividendDistributor { // TO-DO change contract name
    ...
    IBEP20 BNB = IBEP20(0x094616F0BdFB0b526bD735Bf66Eca0Ad254ca81F); // BNB address
    address WBNB = 0x094616F0BdFB0b526bD735Bf66Eca0Ad254ca81F; // BNB address
    ...
    uint256 public minholdforreward = 200000 * (10 ** 9); //minimum amount of tokens needed for holding to receive BNB reward
    ...
    function shouldDistribute(address shareholder) internal view returns (bool) {
        return shareholderClaims[shareholder] + minPeriod < block.timestamp
                && getUnpaidEarnings(shareholder) > minDistribution && token.balanceOf(shareholder) >= minholdforreward;
    }
...
contract NextOfficial is IBEP20, Auth { // TO-DO change contract name
    using SafeMath for uint256;

    address public BNB = 0x094616F0BdFB0b526bD735Bf66Eca0Ad254ca81F; //WBNB address
    address WBNB = 0x094616F0BdFB0b526bD735Bf66Eca0Ad254ca81F;
    address zeroAddress = 0x0000000000000000000000000000000000000000;
    address public marketingAddress = 0x0000000000000000000000000000000000000000; //TO-DO change external address
    address public autoLiquidityReceiver = 0x0000000000000000000000000000000000000000; //TO-DO change liquidty owner address

    string constant _name = "Artisan Token Test";
    string constant _symbol = "ARTSN";
    ...
    mapping (address => bool) public isBNBDividendExempt;
    ...
    // all fee values are upto 1 decimal points. so 5 is 0.5% and 50 is 1% and so on...
    ...
    uint256 private maxTransactiondivisor = 500;
    uint256 private maxTransactionPerAddress = 6;
    ...
    BNB_DISTRIBUTOR distributor; 
    uint256 distributorGas = 500000;

    bool public swapEnabled = true;
    uint256 public _maxTxAmount = _totalSupply.div(maxTransactiondivisor); // 0.2%
    uint256 public minTokensBeforeSwapThreshold = _totalSupply / 20000; // 0.005%
    uint256 public _maxTokensPerAddress         = _totalSupply.mul(maxTransactionPerAddress).div(feeDenominator); // Max number of tokens that an address can hold
 
    // structure to store the value of fees on buy and sell 
    struct BuySellFee{
        uint256 _reflectFee;
        uint256 _marketFee;
        uint256 _liquidFee;
    ...
    constructor () Auth(msg.sender) {
        ...
        distributor = new BNB_DISTRIBUTOR(address(router));

        isFeeExempt[msg.sender] = true;
        isTxLimitExempt[msg.sender] = true;
        
        isBNBDividendExempt[pair] = true;
        isBNBDividendExempt[address(this)] = true;
        isBNBDividendExempt[zeroAddress] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isBNBDividendExempt[sender]){ try distributor.setShare(sender, _balances[sender]) {} catch {} }
        if(!isBNBDividendExempt[recipient]){ try distributor.setShare(recipient, _balances[recipient]) {} catch {} }
    ...
    */
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        ...
        if(burnAmount > 0) {
            ...
            emit Transfer(sender, marketingAddress, burnAmount);
        }
    ...
    */
    function swapBack() internal swapping {
        ...
        try distributor.deposit{value: amountBNBReflection.add(balanceBefore)}() {} catch {}
            payable(marketingAddress).call{value: amountBNBMarketing, gas: 30000}("");
	...
	*/
     ...
     */
    function setMarketingFees(uint256 _marketingFee) external authorized {
    ...
    /**  
     * @dev set BNB distribution criteria
     * _minPeriod--> minimum time to wait before send
     * _minDistribution--> minimum amount of BNB reward before send to address
     */
    function setBNBDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external authorized {
        distributor.setDistributionCriteria(_minPeriod, _minDistribution);
    }

    /**  
     * @dev set BNB distributor's setting
     * gas--> gas fee to be used from sender's gas fee to send BNB reward
     */
    function setBNBDistributorSettings(uint256 gas) external authorized {
        require(gas < 750000);
        distributorGas = gas;
    }
	...
	*/
    ...
    /**  
     * @dev assigning the buy fees to variables
     */
    function setFeeOnBuy() private {
        reflectionFee = _buyFees._reflectFee;
        marketingFee = _buyFees._marketFee;
        liquidityFee = _buyFees._liquidFee;
    }

    /**  
     * @dev assigning the sell fees to variables
     */
    function setFeeOnSell() private {
        reflectionFee = _sellFees._reflectFee;
        marketingFee = _sellFees._marketFee;
        liquidityFee = _sellFees._liquidFee;
    }

    /**  
     * @dev set max transaction divisor 
     */
    function setmaxTransactiondivisor(uint256 _maxTransactiondivisor) external authorized {
        maxTransactiondivisor = _maxTransactiondivisor;   
    }

    /**  
     * @dev set max transaction per address 
     */
    function setmaxTransactionPerAddress(uint256 _maxTransactionPerAddress) external authorized {
        maxTransactionPerAddress = _maxTransactionPerAddress;   
    }
    ...
    function blacklistSingleWallet(address account) external authorized {
        ...
        setIsDividendExempt(account, true); // also remove BNB reward for address
    }
    ...
    function blacklistMultipleWallets(address[] calldata accounts) external authorized {
        ...
        for (uint256 i; i < accounts.length; ++i) {
            ...
            setIsDividendExempt(accounts[i], true); // also remove BNB reward for address
        }
    }
    ...
    function unBlacklistSingleWallet(address account) external authorized {
        ...
        setIsDividendExempt(account, false); // also includes in BNB reward for address
    }
    ...
    function unBlacklistMultipleWallets(address[] calldata accounts) external authorized {
        ...
        for (uint256 i; i < accounts.length; ++i) {
            ...
            setIsDividendExempt(accounts[i], false); // also includes in BNB reward for address
        }
    }
