...
interface IDividendDistributor {
    ...
    function getShareholders() external returns (uint256);
}
...
contract DividendDistributor is IDividendDistributor {
     ...
     */
    constructor (address _madlads) {
        router = IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        _token = msg.sender;
        BUSD = IBEP20(msg.sender);
        madlads = IERC721Enumerable(_madlads);
    }

    function initializeMints() external override {
        uint supply = madlads.totalSupply();
    
        if (supply > 1000) supply = 1; // max gas limit

        for (uint256 i = 0; i < supply; i++) {
            shareholders[i] = 1; // to distinguish samurai added before the first reflection from unregistered samurai
        }

        totalMadlads = supply;
    }

    function addMints() external override {
        uint supply = madlads.totalSupply();
        uint supplyDiff = supply - totalMadlads;

        if (supplyDiff > 1000) supply = 1000; // max gas limit

        for (uint i = shareholders[shareholders.length - 1]; i < supply; i++) {
            shareholders[i] = 1;
        }
    ...
    function getShareholders() public view override returns (uint256) {
        return totalMadlads;
    }
...
contract WatchdogProtocol is IBEP20, Auth {
    ...
    constructor () Auth(msg.sender) {
        ...
        distributor = new DividendDistributor(0x088b2621d7FfD6E1A4ca32b8282fF346a815A599);
    ...
    function getShareholders() external {
        distributor.getShareholders();
    }
