...
contract TitanTest is Context, IERC20, Ownable {
    ...
    function buyToken(uint256 amountOfToken) public payable{
        require(keccak256(abi.encodePacked(stage)) == keccak256(abi.encodePacked("seed")) ||
            keccak256(abi.encodePacked(stage)) == keccak256(abi.encodePacked("presale")),"SALE IS NOT STARTED");
   ...
   function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
    ...
    function _takeBurnFee(uint256 tBurn, address sender) private{
        uint256 currentRate = _getRate();
        uint256 rBurn = tBurn.mul(currentRate);
