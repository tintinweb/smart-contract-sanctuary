...
interface AFTS {
    ...
    function byToken(
	    bytes[] memory sig, //0-sender,1-validtor
	    uint256[] memory details, //0-method,1-fees,2-tier,3-qty,4-swapNonce,5-chainId,6-tier
	    address[] memory accounts, //sender, spender, refAdd
    ...
    function stake(uint256 tier, uint256 qty, address refAdd, uint256 method) external returns (bool);
    ...
    function mint(bytes32 txid, uint256 swapNonce, uint256 amount, uint256 fees, uint256 chainId, bytes memory sig) external returns (bool);   
...
contract Exosis is Context, AFTS {
	...
	function byToken(
        ...
        _sigValidate(sig[0], keccak256(abi.encodePacked(accounts[0], details[6], details[0], details[1], _user[accounts[0]].swapNonce + 1)), accounts[0]);
	    require(details[0] >= 0 && details[0] < 8, "Invalid Method");
	    if(details[6] == 0){
    	    if(details[0] == 1){
    	        _transfer(accounts[0], recipient[0], amount[0], 1);
    	    } else if(details[0] == 2){
                _transfer(accounts[0], recipient[0], amount[0], 1);
                _approve(accounts[0], accounts[1], _allowances[accounts[0]][accounts[1]] - amount[0]);
    	    } else if(details[0] == 3){
    	        _approve(accounts[0], accounts[1], amount[0]);
    	    } else if(details[0] == 4){
    	        _transferMulti(accounts[0], recipient, amount, 0, address(0));
    	    } else if(details[0] == 5){
    	        _transferMulti(accounts[0], recipient, amount, 1, accounts[1]);
    	    }
	    } else if(details[6] > 0 && details[6] < 4){
	        _stakeProcess(accounts[0], details[2], details[3], accounts[2], details[0]);
	    } else if(details[6] == 4){
	        _propose(accounts[0], info[0], info[1], amount[0], recipient[0]);
	    } else if(details[6] == 5){
	        _voteProcess(accounts[0], info[0]);
	    } else if(details[6] == 6){
	        _burn(accounts[0], amount[0], details[4], details[5]);
	    } else if(details[6] == 7){
	        _mint(accounts[0], txid, details[4], amount[0], details[1], details[5], sig[1]);
	    }
	    _user[accounts[0]].swapNonce += 1;
	    return true;
	}
	
    function stake(uint256 tier, uint256 qty, address refAdd, uint256 method) public virtual override returns (bool) {
        _stakeProcess(_msgSender(), tier, qty, refAdd, method);
        return true;
    }  
    ...
    function _stakeProcess(address sender, uint256 tier, uint256 qty, address refAdd, uint256 method) internal virtual {
        ...
        if(method < 2){
            ...
            if(_user[sender].refAdd != address(0)){
                ...
                _stake[_user[sender].refAdd][tier].refRew += refRew;
                _user[_user[sender].refAdd].refPaid += refRew;
            ...
            _stake[sender][tier].rewPaid += stakeReward;
    ...
    function _splitSignature(bytes memory sig) internal virtual returns (bytes32 r, bytes32 s, uint8 v) {
    ...
    function _sigValidate(bytes memory sig, bytes32 hash, address account) internal {
    ...
    function _burn(address sender, uint256 amount, uint256 swapNonce, uint256 chainId) internal {
        ...
        if(swapNonce > 0){
            _user[sender].swapNonce = swapNonce;
            _user[sender].chainId = chainId;
        }
        _general._totalSupply -= amount;
    }    
    
    
    function mint(bytes32 txid, uint256 swapNonce, uint256 amount, uint256 fees, uint256 chainId, bytes memory sig) public virtual override returns (bool) {
        _mint(_msgSender(), txid, swapNonce, amount, fees, chainId, sig);
        return true;
    }  
    
    function _mint(address sender, bytes32 txid, uint256 swapNonce, uint256 amount, uint256 fees, uint256 chainId, bytes memory sig) internal {
        require(!_general._isPaused, "Contract is Paused");
        require(_swapIn[sender][txid] == 0, "Already Swapped");
        _sigValidate(sig, keccak256(abi.encodePacked(sender, txid, swapNonce, amount, fees, chainId, true)), _general._validatorAdd);
