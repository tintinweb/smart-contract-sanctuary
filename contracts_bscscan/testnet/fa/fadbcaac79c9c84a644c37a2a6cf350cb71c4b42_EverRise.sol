...
contract EverRise is Context, IERC20, Ownable {
    ...
    uint256 public _fundingFee = (_marketingFee.add(_buyBackFee).add(_liquidityFee));
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && to == uniswapV2Pair) {
	        ...
	        uint256 balance = address(buyBackAddress).balance;
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 swapPiece = transferredBalance.div(_liquidityFee.mul(_marketingFee).mul(_buyBackFee));
    ...
    function setBuyBackUpperLimit(uint256 buyBackLimit) public onlyOwner {
        require(buyBackLimit > 1 * (10**17));
