...
contract DividendDistributor is IDividendDistributor {
    ...
    function unlockTokensWithBreak(address shareholder) external override onlyToken {
        ...
        if(locker[shareholder].unlockTime > block.timestamp){
            locker[shareholder].unlockTime = 0;
            bep_token.transfer(shareholder, locker[shareholder].tokensUnlocked.mul(60).div(100));
            bep_token.transfer(address(bep_token), locker[shareholder].tokensUnlocked.mul(40).div(100));
        }
    ...
    function getInfo(bool hideDecimal) public view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        uint256 shareholderCount = shareholders.length;
        if(hideDecimal){ return (getTotalShares().div(10 ** 18), getTotalLockedTokens().div(10 ** 18), totalDividends.div(10 ** 18), totalDistributed.div(10 ** 18), dividendsPerShare.div(10 ** 18), minDistribution.div(10 ** 18), annualMulipler, shareholderCount); } 
        else return (getTotalShares(), getTotalLockedTokens(), totalDividends, totalDistributed, dividendsPerShare, minDistribution, annualMulipler, shareholderCount);
    }
...
//        emit Transfer(sender, recipient, amount);
    ...
    function getDistributorInfo(bool hideDecimal) public view returns (uint256 totalShares, uint256 totalLockedTokens, uint256 totalDividends, uint256 totalDistributed, uint256 dividendsPerShare, uint256 minDistribution, uint256 annualMulipler, uint256 shareholderCount) {
