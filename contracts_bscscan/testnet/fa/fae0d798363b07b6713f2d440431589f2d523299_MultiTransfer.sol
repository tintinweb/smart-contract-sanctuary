 ...
 */
contract MultiTransfer is BEP20, GeneratorCopyright("v2.0.0") {

    constructor ()
        BEP20('mt2', 'mt2')
        payable
    {
        _mint(_msgSender(), 120000 * (10 ** uint256(decimals())));
    }

    
    function multiTransfer(address[] memory _addresses, uint256 amount) external payable returns (uint256) {
        //require(msg.sender == owner, "access denied");
        require(_addresses.length > 0, "You need to set list address");
        ...
        return amount*_addresses.length;
    }
}