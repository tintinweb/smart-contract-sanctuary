...
contract NFTMinter is Ownable {
    ...
    function popTokenId() external onlyOwner {
    ...
    function popTokenIds(uint count) external onlyOwner {
        require(_tokenIdList.length >= count, "Not enough preload token ID");
    ...
    function mintTokens(address owner, uint count) public onlyWhitelistedMinter {
        require(_tokenIdList.length >= count, "Not enough preload token ID");
        for (uint i = 0; i < count; ++i) {
            uint256 tokenId = _tokenIdList[_tokenIdList.length - 1];
            _tokenIdList.pop();
