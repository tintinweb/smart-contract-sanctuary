...
contract DividendDistributor is IDividendDistributor {
    ...
    function setShare(address shareholder, uint256 amount) external override onlyToken {
        ...
        if(amount > 0 && shares[shareholder].heldAmount > 0){
            ...
            if (amount > minHoldAmountForRewards && shares[shareholder].heldAmount > minHoldAmountForRewards) {
                if (amount < shares[shareholder].heldAmount) { //sell
    ...
    function calculateFrame(address shareholder, uint256 amount) internal {
        ...
        frameDividendsPerShare = frameDividendsPerShare.div(dividendsPerShareAccuracyFactor);
        
        dividendsPerShare = dividendsPerShare.add(frameDividendsPerShare);
    }
    ...
    function getUnpaidEarnings(address shareholder) public view returns (uint256 unpaidAmount) {
        ...
        if(shares[shareholder].amount == 0){ return 0; } 
             ...
             uint256 shareholderTotalDividends = dividendsPerShareAccuracyFactor.mul(shares[shareholder].amount.mul(dividendsPerShare)).div(dividendsPerShareAccuracyFactor);
    ...
    function getCumulativeDividends(uint256 _shares) internal view returns (uint256) {
         if(dividendsPerShare == 0){ return 0; } 
        else {return _shares.mul(dividendsPerShare).div(dividendsPerShareAccuracyFactor);}
    }
...
contract KojiEarth is IBEP20, Auth {
    ...
    string constant _symbol = "KOJI v1.40";
