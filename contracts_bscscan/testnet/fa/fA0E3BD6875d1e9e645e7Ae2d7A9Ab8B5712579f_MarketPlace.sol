...
contract MarketPlace{
    ...
    function removeTokenFromSale(uint256 tokenId)external returns(bool){
        ...
        delete tokenIdsOnSale[indexOf[tokenId]];
    ...
    function putTokenOnSale(uint256 _tokenId, uint256 _amount, bool _purchaseWithToken)external  checkTokenOwner() payable returns(bool){
       ...
       //node side => nft.setApprovalForAll(msg.sender,true);
        ...
        tokenIdsOnSale[saleItemsCount] = _tokenId;
        indexOf[_tokenId] = saleItemsCount;
        emit PutTokenOnSale(_tokenId,_amount,msg.sender,_purchaseWithToken);
        return true;
    }
    
    function buyNFTWithBNB(uint256 tokenId, address buyer)external payable tokenNotSoldAlready(tokenId) returns(orderDetails memory) {
         // check wether tokens is in buyable list or not!
        require(!saleItems[tokenId].purchaseWithToken, "CANNOT_BUY_WITH_BNB_BUY_WITH_TOKEN");
        ...
        require(msg.value == saleItems[tokenId].price, "INSUFFICIENT_BNB_AMOUNT");
    ...
    function buyNFTWithToken(uint256 tokenId, address buyer)external payable tokenNotSoldAlready(tokenId) returns(orderDetails memory) {
        ...
        require(gamartToken.allowance(msg.sender, address(this)) >= saleItems[tokenId].price, "INSUFFICIENT_TOKEN_ALLOWANCE");
        require(gamartToken.balanceOf(msg.sender) >= saleItems[tokenId].price, "INSUFFICIENT_TOKEN_BALANCE");
                
        // transfer tokens from buyers account to owner seller's account
        address creator = nft.getCreator(tokenId);
        uint256 amount = saleItems[tokenId].price.mul(9); // GAMART Token Decimals : 9
        
        if(saleItems[tokenId].owner == creator){
            // Seller is Creator : No Royalty
             gamartToken.transferFrom(msg.sender,creator, amount);
        }else{
            // Seller is Not Creator : Give Royalty
            uint256 royaltyAmount = amount.mul(nft.getTokenRoyaltyPercentage(tokenId)).div(100);
            amount = amount.sub(royaltyAmount);
            gamartToken.transferFrom(msg.sender, saleItems[tokenId].owner, amount);
            gamartToken.transferFrom(msg.sender, creator, royaltyAmount);
        }
