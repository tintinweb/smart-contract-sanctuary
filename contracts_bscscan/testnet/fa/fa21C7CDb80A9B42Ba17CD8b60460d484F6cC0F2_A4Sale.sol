...
contract A4Sale is Ownable  {
    ...
    uint256 private minValue = 1 * 10 ** 18;
    ...
    function withdrawIDO( uint256 value ) onlyOwner public {
        require(releaseTime <= block.timestamp, "IDO is active");
    ...
    function _deposit( uint256 value ) internal
    {
        ...
        require(value >= minValue, "Not enough value");
    ...
    {
        require(startTime <= block.timestamp &&  releaseTime > block.timestamp, "IDO is stopped");
        require(value <= _deposits[msg.sender] && value > 0, "Error withdraw value");
    ...
    {
        require(releaseTime <= block.timestamp && endTime > block.timestamp, "No time for claime");
