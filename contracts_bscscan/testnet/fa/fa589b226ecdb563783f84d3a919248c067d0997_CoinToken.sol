...
contract CoinToken is Context, IERC20, Ownable {
    ...
    address public _LPWalletAddress = 0x8303530E95C9Ee00371842dbf17298F16ceae972; 
    ...
    constructor (string memory _NAME, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, uint256 _txFee,uint256 _lpFee,uint256 _DexFee,address routerAddress,address feeaddress,address tokenOwner) public payable {
        ...
        _maxSellTxAmount = (_tTotal * 5 / 1000) * 10 ** _decimals;
        _maxBuyTxAmount = (_tTotal * 5 / 1000) * 10 ** _decimals;        
    ...
    function setMaxSellTxPercent(uint256 maxTxPercent) public onlyOwner {
        _maxSellTxAmount = maxTxPercent  * 10 ** _decimals;
    }
    function setMaxBuyTxPercent(uint256 maxTxPercent) public onlyOwner {
        _maxBuyTxAmount = maxTxPercent  * 10 ** _decimals;
    }
    ...
    function _transfer(
        ...
        bool isSell=from==uniswapV2Pair || to == routerAddress;
        bool isBuy=to==uniswapV2Pair || from == routerAddress;   
        bool excludedAccount = _isExcludedFromFee[from] || _isExcludedFromFee[to];        
        
        if(isSell && !excludedAccount){  
          require(amount <= _maxBuyTxAmount, "Transfer amount exceeds the maxTxAmount.");
        }else if(isBuy && !excludedAccount){
