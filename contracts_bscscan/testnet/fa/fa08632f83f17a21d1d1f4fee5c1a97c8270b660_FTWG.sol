...
contract FTWG is Context, IERC20, Ownable {
    ...
    string private _name = "Feed The World Global";
    string private _symbol = "FTWG";
    ...
    address payable buybackWallet = payable(0x513f25FB9b959bc45EE02ae75e166F817d6A5D97);
    
    uint256 public antiwhaleAmount = _tTotal * 1 / 100; // 1% of total supply
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && from != uniswapV2Pair) {
            ...
            if(amount >= antiwhaleAmount && !_isExcludedFromFee[from] && to == uniswapV2Pair ){
                handle_fees();
                uint256 buybackAmt = amount * _buybackFee / 100;
                swapTokens(buybackAmt);
                amount -= buybackAmt;
                restoreAllFee();
            }
        }
    ...
    function _takeCharity(uint256 tCharity) private {
        ...
        _rOwned[charityAddress] = _rOwned[charityAddress] + (rCharity);
        if(_isExcluded[charityAddress])
            _tOwned[charityAddress] = _tOwned[charityAddress] + (tCharity);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(!takeFee) {
            ...
            if(sender == uniswapV2Pair){ 
                handle_fees(); 
                uint256 buybackAmt = amount * _buybackFee / 100;
                amount = amount - buybackAmt;
                _transferToExcluded(sender, buybackWallet, buybackAmt);
                restoreAllFee();
            }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tCharity, uint256 rAmount, uint256 rTransferAmount, uint256 rFee) {
        ( tTransferAmount,  tFee, tLiquidity, tMarketing, tCharity) = _getTValues(tAmount);
        ( rAmount,  rTransferAmount,  rFee) = _getRValues(tAmount, tFee, tLiquidity, tMarketing, tCharity, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tMarketing, tCharity);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tCharity, uint256 tTransferAmount) {
         tFee = calculateTaxFee(tAmount);
         tLiquidity = calculateLiquidityFee(tAmount);
		 tMarketing = calculateMarketingFee(tAmount);
		 tCharity = calculateCharityFee(tAmount);
         tTransferAmount = tAmount - (tFee) - (tLiquidity) - (tMarketing) - (tCharity);
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tCharity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
		...
		uint256 rMarketing = tMarketing * (currentRate);
		uint256 rCharity = tCharity * (currentRate);
        uint256 rTransferAmount = rAmount - (rFee) - (rLiquidity) - (rMarketing) - (rCharity);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingFee == 0 && _charityFee == 0 && _buybackFee == 0) return;
