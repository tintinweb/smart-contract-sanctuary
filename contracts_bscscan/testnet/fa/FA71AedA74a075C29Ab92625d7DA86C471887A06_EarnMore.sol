...
/** 
 * EarnMore: SafeEarn/EarnHub v2/SafeVault forked with improvements and additions (Whale timer, whale fee multiplier, reflection pool mechanism, merchant fee)
 */
 ...
 *  Additional Whale Timer & Fee Multiplier Mechanism + Custom Transfer Fee + Custom Merchant Fee + Reward Pool Mechanism added by the EarnMore team
 ...
 */
contract EarnMore is IERC20, Context, Ownable {
    ...
    // token data
    string  constant _name = "EarnMore";
    ...
    constructor (
        ...
        // Liquidity Pool Address for BNB -> EarnMore
    ...
    /** Basic Transfer with no swaps for BNB -> EarnMore or EarnMore -> BNB */
    ...
    /** True if we should swap from EarnMore => BNB, only swapsback during buy/sell to save transfer gas fees */
    ...
    /**
     *  Swaps EarnMore for BNB if threshold is reached and the swap is enabled
    ...
    /** Should EarnMore buy/burn right now? */
    ...
    /**
     * Buys EarnMore with bnb in the contract and then sends to the dead wallet
     */ 
    ...
    /**
     * Buy and Burn EarnMore with bnb stored in contract
     */
    function triggerEarnMoreBuyback(uint256 amount) external onlyOwner {
        buyTokens(amount, DEAD);
        emit EarnMoreBuyBackAndBurn(amount);
    }
    ...
    /** Returns the Circulating Supply of EarnMore ( supply not owned by Burn Wallet ) */
    ...
    event EarnMoreBuyBackAndBurn(uint256 amountBNB);
