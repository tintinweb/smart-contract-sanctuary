...
// SEEK IS BROUGHT TO YOU BY DYOR TOKEN AND RUGSEEKERS IN EFFORTS TO STOP SCAMS AND GIVE INVESTORS A SAFE PLACE TO GROW WEALTH ALSO AUTO EARN DYOR TOKEN WHILE HOLDING SEEK
//RugSeekers is committed to do our part to rid cryptospace of corruption and fraud. In a time when investors are more afraid than excited, we intend to fight the good fight and make our decentralized space what it was intended to be. A safe and exciting alternative to fiat and all its limitations. 

//We will have a fully interactive website with plans to launch our own brand of crypto security software. 
...
contract DividendDistributor is IDividendDistributor {
    ...
    // DYOR contract address
    address Dyor = 0x6382e2D214CC4CacA4c665029826e2CfF7E46B1F;
    // bnb address
    ...
    function deposit() external override onlyToken {
        ...
        if (bnbBalance >= swapToDyorThreshold) {
            ...
            uint256 balanceBefore = IERC20(Dyor).balanceOf(address(this));
            ...
            uint256 amount = IERC20(Dyor).balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            IERC20(Dyor).transfer(shareholder, amount);
 ...
 *  This Contract Awards DYOR TOKEN Daily to holders, weighted by how much you hold
 * 
 *  
 *  Transfer Fee:  5%
 *  Buy Fee:       5%
 ...
 * 
 *  Fees Go Toward:
 *  22% DYOR Distribution
 *  5% Buyback and burn
 *  2% Auto Liquidity
 *  1% Marketing
 */
contract SEEKTOKEN is IERC20, Context, Ownable {
    ...
    // our burn wallet address - separate from DYOR TOKEN's
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    // token data
    string constant _name = "SEEK TOKEN";
    string constant _symbol = "SEEK";
    uint8 constant _decimals = 9;
    // 200 Trillion Max Supply
    ...
    address public marketingFeeReceiver = 0x56285287e520fC3EA41C33777a3f83BB6dB35d5b;
    ...
    constructor (
        ...
        // Liquidity Pool Address for BNB -> SEEK
    ...
    /** Takes Associated Fees and sets holders' new Share for the Dyor Token Distributor */
    ...
    /** Basic Transfer with no swaps for BNB -> SEEK or SEEK -> BNB */
    ...
    /**
     *  Swaps SEEK Token for BNB if threshold is reached and the swap is enabled
     *  Uses BNB retrieved to:
     *      fuel the contract for buy/burns
     *      provide distributor with BNB for Dyor Token
    ...
    /** Transfers BNB to Dyor Token Distributor and Marketing Wallet */
    function transferToDistributorAndMarketing(uint256 distributorBNB, uint256 marketingBNB) internal {
        ...
        if (success) {
            try distributor.deposit() {totalBNBDyorReflections = totalBNBDyorReflections.add(marketingBNB);} catch {}
        }
    ...
    /**
     * Buys SEEK with bnb in the contract, sending to target address
     */ 
    ...
    /**
     * Buy and Burn SEEK with bnb stored in contract
     */ 
