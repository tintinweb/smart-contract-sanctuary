...
contract KEY is ERC721Enumerable, Ownable {
    ...
    modifier onlyMinter() {
        require(minter[_msgSender()], "KEY: caller is not the minter");
        _;
    }
    event Lock();
    ...
    function mint(uint256 _type, address _to, uint256 _tokenId) external onlyMinter {
        require(totalSupply() < maxSupply, "KEY: Total supply reached");
        _mint(_to, _tokenId);
    ...
    function setType(string memory _name, string memory _hash) public onlyOwner {
        types.push(Type(_name, _hash));
    }
