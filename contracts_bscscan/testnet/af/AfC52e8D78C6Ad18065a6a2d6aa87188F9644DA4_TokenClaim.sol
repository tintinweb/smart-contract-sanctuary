...
contract TokenClaim {
  ...
  function refund(address _userAddr) external {
    require(_userAddr != address(0),'Invalid address');
    require(addressToAmount[_userAddr] > 0,'Invalid refund amount');
    require(block.number < refundBlockNumber, 'Refund is no longer allowed');
    require(userRefunded[_userAddr] == false, 'Already refunded');
    require(processedClaim[_userAddr] == false, 'Already distributed');

    userRefunded[_userAddr] = true;

    IERC20(refundToken).transfer(_userAddr, addressToAmount[_userAddr]);

    emit Refunded(
      _userAddr,
      addressToAmount[_userAddr],
      block.timestamp
    );
  }
  ...
  function claimTokens(
    ...
    uint256 amount = addressToAmount[recipient];
    
    require(recipient != address(0),'Invalid address');
    require(addressToAmount[recipient] > 0,'Not in whitelist or claimed');
    require(processedClaim[recipient] == false, 'Distribute already processed');
    require(userRefunded[recipient] == false,'Refunded');
    require(amount > 0, 'Invalid amount');
    require(currentDistributeAmount + amount <= maxDistributeAmount, 'Distributed 100% of the tokens');

    processedClaim[recipient] = true;
    addressToAmount[recipient] = addressToAmount[recipient] - amount;
    currentDistributeAmount = currentDistributeAmount + amount;

    IERC20(idoToken).transfer(recipient, amount);
