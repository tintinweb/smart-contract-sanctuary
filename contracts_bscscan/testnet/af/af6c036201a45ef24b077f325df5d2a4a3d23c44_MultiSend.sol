...
contract MultiSend is Context, Ownable {
   ...
   function transferBulk(address[] memory tokens, address[] memory _tos, uint256[] memory _values) public virtual returns(bool) {
      for(uint256 i=0; i < tokens.length; i++) {
         // If one fails, revert the tx, including previous transfers
         require(!IERC20(tokens[i]).transfer(_tos[i], _values[i]), "MultiSend Bulk: Transfer failed");
      }
      payable(SERVICE_ADDRESS).transfer(this.estFeeTransferBulk(tokens.length));
      return true;
   }
   
   function estFeeTransferBulk(uint256 noOfTxs) public virtual returns(uint256) {
       if(noOfTxs < 5) {
