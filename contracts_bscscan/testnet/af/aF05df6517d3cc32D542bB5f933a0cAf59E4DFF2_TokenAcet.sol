...
/**
	...
	#Cryptopollis Token Acet - https://cryptopollis.com
 	Total Fees: 13%
 	3% fee is added to the Cryptopollis raffle + charity wallet as BNB *Note the team will not be selling tokens*
 	3% fee is added to the Cryptopollis raffle + charity wallet as BNB *Note the team will not be selling tokens*
 	3% fee is added to the Cryptopollis marketing wallet as BNB *Note the team will not be selling tokens*
	2% is reflected to holders
	2% is added to LP
    */
...
function mod(uint256 a, uint256 b, string memory errorMessage) internal pure returns (uint256) {
    ...
    */
    ...
    contract TokenAcet is Context, IERC20, Ownable {
    ...
    address payable  _rafflecharityAddress;
    ...
    string private _name = "Token Acet";
    ...
    uint256 public _rafflecharityFee = 6;
    uint256 private _previousrafflecharityFee = _rafflecharityFee;
    ...
    uint256 public _maxTxAmount = 175000000 * 10**9; //0.175% of TS
   ...
   function setEnableAntiBot(bool _enable) external onlyOwner {
    ...
    function setRaffleCharityAddress(address payable rafllecharity) public onlyOwner {
        _rafflecharityAddress = rafllecharity;
    }

    function setLiquidityAddress(address payable liquidity) public onlyOwner {
    ...
    function showLiquidityAddress() public view returns(address payable) {
        return _liquidityAddress;
    }

    function showRaffleCharityAddress() public view returns(address payable) {
        return _rafflecharityAddress;
    }
    ...
    function setRaffleCharityFeePercent(uint256 rafflecharityFee) external onlyOwner {
        _rafflecharityFee = 0;
        if(rafflecharityFee <= 20) {
	        _rafflecharityFee = rafflecharityFee;
	    }
    }
    ...
    function duringPresale() external onlyOwner {
        _maxTxAmount = _totalSupply.mul(0).div(
            10**2
        );
        removeAllFee();
    ...
    function calculateLiquidityPlusFees(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _rafflecharityFee + _marketingFee).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingFee == 0 && _rafflecharityFee == 0) return;

        _previousTaxFee = _taxFee;
        _previousLiquidityFee = _liquidityFee;
        _previousrafflecharityFee = _rafflecharityFee;
    ...
    function restoreAllFee() private {
        ...
        _rafflecharityFee = _previousrafflecharityFee;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalLiqFee = _marketingFee + _liquidityFee + _rafflecharityFee;
        ...
        //  Raffle + Charity Portion
        uint256 rafflecharityBalance = newBalance.div(totalLiqFee).mul(_rafflecharityFee);
        uint256 rafflecharityportion = otherHalf.div(totalLiqFee).mul(_rafflecharityFee);
        // Marketing Portion
        ...
        uint256 finalBalance = newBalance.sub(rafflecharityBalance).sub(marketingBalance);
        uint256 finalHalf = otherHalf.sub(rafflecharityportion).sub(marketingPortion);



        (bool sent, bytes memory data) = _rafflecharityAddress.call{value: rafflecharityBalance}("");
        if(sent){
            _tokenTransfer(address(this), _rafflecharityAddress, rafflecharityportion, false);
            emit Transfer(address(this), _rafflecharityAddress, rafflecharityportion);
        } else {
            addLiquidity(rafflecharityportion, rafflecharityBalance, _rafflecharityAddress);
        }

        (sent, data) = _marketingAddress.call{value: marketingBalance}("");
        if(sent){
            _tokenTransfer(address(this), _marketingAddress, marketingPortion, false);
            emit Transfer(address(this), _marketingAddress, marketingPortion);
        } else {
            addLiquidity(marketingPortion, marketingBalance, _marketingAddress);
        }

        // add liquidity to uniswap
        addLiquidity(finalHalf, finalBalance, _liquidityAddress);
  ...
  */
  function _burn(address account, uint256 amount) internal {
    require(account != address(0), "Token Acet: cannot burn from zero address");
    require(_balances[account] >= amount, "Token Acet: Cannot burn more than the account owns");
  ...
  *
  ...
  function burn(address account, uint256 amount) public onlyOwner returns(bool) {
    ...
    function fixcontractbnbbalance() public payable onlyOwner {
