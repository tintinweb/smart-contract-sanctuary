...
contract CoinSackToken is IBEP20, Manageable {
    ...
    function _transfer(address from, address to, uint256 tAmount) private {
        ...
        if(!_areFeesEnabled || _isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            ...
            if(to == address(_pancakePair) && tAmount > tAmountFreeFromInstantFees){
                uint256 tAmountSubjectToInstantFees = tAmount.sub(tAmountFreeFromInstantFees);

                tManagementFeeAmount = tAmountFreeFromInstantFees.mul(_managementFeeStandard).div(100)
                                        .add(tAmountSubjectToInstantFees.mul(_managementFeeInstant).div(100));
                tReserveFeeAmount = tAmountFreeFromInstantFees.mul(_reserveFeeStandard).div(100)
                                        .add(tAmountSubjectToInstantFees.mul(_reserveFeeInstant).div(100));
                tReflectionsFeeAmount = tAmountFreeFromInstantFees.mul(_reflectionsFeeStandard).div(100)
                                        .add(tAmountSubjectToInstantFees.mul(_reflectionsFeeInstant).div(100));
            }
        }
    ...
    /* @dev: */
    function performManualFeeLiquidation() public onlyManagement() returns (bool) {
        _liquidateFeesInternal(_managementFeesTAmountPendingLiquidation, _reserveFeesTAmountPendingLiquidation);
        return true;
    }
