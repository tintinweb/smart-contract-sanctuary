    ...
    UUPSUpgradeable
{
    ...
    using AkoyaType for AkoyaType.CharacterType;
    ...
    struct CryptoAkoya {
        uint256 generation;
        AkoyaType.CharacterType akoyaCharacterType;
        uint256 exp;
        uint256 dna;
        uint256 farmTime;
        uint256 bornTime;
    ...
    mapping(AkoyaType.CharacterType => CharacterTypeInformation) public characterTypeInfo;
    mapping(uint256 => bool) public isEvolved;
    mapping(uint256 => CryptoAkoya) internal akoyalegends;
  
    event AkoyaMinted(address to, AkoyaType.CharacterType akoyaCharacterType, string data, uint256 newTokenId);
    ...
    function initialize(address _forwarder) public virtual initializer {
        ...
        // _bnbToAKOYA[0] = _uniswapV2Router.WETH();
        // _bnbToAKOYA[1] = address(this);
    }
    ...
    function mint(
        ...
        AkoyaMinted(recipient, akoyaCharacterType, data, newTokenId);
    ...
    */
    ...
    function getTokenPriceUSD(address token_address, address USDT_address) external view returns (uint256) {
        uint token_decimals = IERC20Metadata(token_address).decimals();
        uint min_amountIn = 1*10**token_decimals;
        ...
        uint price = uniswapV2Router.getAmountOut(min_amountIn, reserve0, reserve1);
        return price;
    }
    ...
    function getAkoyalegends(uint256 _tokenId) public view returns (CryptoAkoya memory) {
        return akoyalegends[_tokenId];
    }

    function evolve(uint256 _tokenId, uint256 _dna) public {
        require(ownerOf(_tokenId) == _msgSender(), "not own");
        CryptoAkoya storage akoyalegend = akoyalegends[_tokenId];
        require(!isEvolved[_tokenId], "require: not evolved");

        akoyalegend.bornTime = block.timestamp;
        akoyalegend.dna = _dna;
        isEvolved[_tokenId] = true;

        emit Evolve(_tokenId, _dna);
    }

    function exp(uint256 _tokenId, uint256 _exp) onlyRole(MINTER_ROLE) public {
        require(_exp > 0, "no exp");
        CryptoAkoya storage akoyalegend = akoyalegends[_tokenId];
        akoyalegend.exp = akoyalegend.exp.add(_exp);
