...
contract SpotBattle is DataLayout, Proxiable{
    ...
    function addMarket(address token) public {
        ...
        marketData[token].targetPrice = uint(getLatestPrice(token)); // get price from usdc quote
    ...
    function expireRound(address token, uint32 index) public {
        ...
        marketData[token].roundEnd = marketData[token].roundEnd.add(5 minutes);
        //reward function caller with amount of versus(maybe)

    }

    function getSpotInfo(address token, address user) public view returns(uint256[] memory, uint256[] memory, uint256[] memory, bool[] memory) {
        ...
        nextInfo[0] = marketData[token].longHistory[marketData[token].round-2];
        nextInfo[1] = marketData[token].shortHistory[marketData[token].round-2];
        nextInfo[2] = marketData[token].closingHistory[marketData[token].round-2];
    ...
    function getUserMarketHistory(address user, uint256 sIndex) public view returns(address[] memory,uint256[] memory,uint256[] memory,bool[] memory,bool[] memory) {
        address[] memory tokenHistory = new address[](10);
        uint256[] memory roundHistory = new uint256[](10);
        uint256[] memory BNBHistory = new uint256[](10);
        bool[] memory positionHistory = new bool[](10);
        bool[] memory winClaimed = new bool[](10);
        
        uint iCounter;
        for(uint i; i < 10; i++) {
            tokenHistory[i] = userTokenHistory[user][sIndex - iCounter];
            roundHistory[i] = userTokenRoundHistory[user][sIndex - iCounter][tokenHistory[i]];
            BNBHistory[i] = userBNBHistory[user][sIndex - iCounter][tokenHistory[i]][roundHistory[i]];
            positionHistory[i] = userPositionHistory[user][sIndex - iCounter][tokenHistory[i]][roundHistory[i]];
            //user => index => token address => round => bool
            winClaimed[i] = userClaimHistory[user][sIndex - iCounter][tokenHistory[i]][roundHistory[i]];
            iCounter = iCounter+1;
        }
        return(
            tokenHistory,
    ...
    function claim(address user, uint256 userIndex) public {
        ...
        if (!userClaimHistory[user][userCurrentIndex[msg.sender]][token][marketData[token].round]) {
            ...
            if (userPositionHistory[user][userIndex][token][round] != longWon) {
                ...
                // VersusToken(versusToken).updateUsesrWins(user, false);
                return;
            }
