...
contract SecretBox is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    uint256 public reforgeStatsFee;
    ...
    function setReforgeStatsFee(uint256 _fee) public onlyOwner {
        reforgeStatsFee = _fee;
    }
    ...
    function reforgeStats(uint256 wep)
        public
        onlyNonContract
        oncePerBlock(msg.sender)
        whenNotPaused
    ...
    {
        require(
            xBlade.balanceOf(msg.sender) >= reforgeStatsFee,
            "Not enough xBlade"
        );
        xBlade.transferFrom(msg.sender, address(this), reforgeStatsFee);
        weapons.reforgeWeaponStats(
            wep,
            uint256(
                keccak256(
                    abi.encodePacked(blockhash(block.number - 1), msg.sender)
...
contract Weapons is Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    */
    ...
    function mintWeaponWithStars(address minter, uint256 stars, uint256 seed) public canMintWeapon returns(uint256) {
        require(stars < 8, "Stars params max 7");
        (uint16 stat1, uint16 stat2, uint16 stat3) = getStatRolls(stars, seed);
    ...
    function _decrementDustSuppliesCheck(address playerAddress, uint32 amountLB, uint32 amount4B, uint32 amount5B) internal {
        ...
        require(supplies[0] >= amountLB, "Dust LB needed");
        require(supplies[1] >= amount4B, "Dust 4B needed");
        require(supplies[2] >= amount5B, "Dust 5B needed");
    }
    ...
    function _incrementDustSuppliesCheck(address playerAddress, uint32 amountLB, uint32 amount4B, uint32 amount5B) internal {
        uint32[] memory supplies = getDustSupplies(playerAddress);
        require(uint256(supplies[0]) + amountLB <= 0xFFFFFFFF, "Dust LB capped");
        require(uint256(supplies[1]) + amount4B <= 0xFFFFFFFF, "Dust 4B capped");
        require(uint256(supplies[2]) + amount5B <= 0xFFFFFFFF, "Dust 5B capped");
    }
    ...
    function _applyBurnPoints(uint256 reforgeID, uint8 amountLB, uint8 amount4B, uint8 amount5B) private {
        ...
        if(amount4B > 0) {
            require(wbp.fourStarBurnPoints < 25, "4 star burn points are capped");
        }
        if(amount5B > 0) {
            require(wbp.fiveStarBurnPoints < 10, "5 star burn points are capped");
        }
    ...
    function reforgeWeaponStats(uint256 id, uint256 seed) public canMintWeapon  {
        Weapon storage wep = tokens[id];
        uint8 stars = getStarsFromProperties(wep.properties);

        // Generate new stats
        (uint16 stat1, uint16 stat2, uint16 stat3) = getStatRolls(
            stars,
            RandomUtil.combineSeeds(
                seed,
                RandomUtil.randomSeededMinMax(0, 1000, seed)
            )
        );
        wep.properties = getRandomProperties(stars,seed);
        wep.stat1 = stat1;
        wep.stat2 = stat2;
        wep.stat3 = stat3;
    }
