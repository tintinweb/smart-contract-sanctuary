...
// Contract created by https://gentokens.com/ for Mansa Musa
...
Tokenomics
    ...
    Supply  1,000,000,000,000,000 (Mansa Musa is deflationary. The Total Supply will reduce over time whenever tokens are burned.)
    ...
    3%      Anti-Whale max wallet (after initial launch phase)
...
contract MAMU is Context, IERC20, Ownable { 
    ...
    address payable public BSC_Charity = payable(0x8B99F3660622e21f2910ECCA7fBe51d654a1517D); 
    ...
    string private _name = "Mansa Musa"; 
    string private _symbol = "MAMU";  
    ...
    uint256 public _FeeMaxPossible = 6;
    ...
    uint256 public _FeesTotal = _FeeLiquidity+_FeeReflection;
    ...
    */
    ...
    function _set_Fees(uint256 Liquidity, uint256 Reflection) external onlyOwner() {

        // Buyer protection - The fees can never be set above the max possible (6%)
        require((Liquidity+Reflection) <= _FeeMaxPossible, "Total fees set to high!");

        // Set the fees
          _FeeLiquidity = Liquidity;
          _FeeReflection = Reflection;

          _FeesTotal = _FeeLiquidity + _FeeReflection;
    ...
    */
    ...
    function removeAllFee() private {
        if(_FeeReflection == 0 && _FeeLiquidity == 0) return;
        ...
        uint256 amount
          ...
          if (launchPhase){
                ...
                if(TradeOpen){
                        ...
                        // People that tried to snipe and blocked for approx 30 seconds 
                        if (launchBlock + 10 > block.number){
                        require((!_isSnipe[to] && !_isSnipe[from]), 'You tried to snipe, now you need to wait.');
                        }

                        // Buy in first block = snipe (During the first block the max wallet and transaction are limited to 0.05% of supply)
                        ...
                        // From Block 2 the transaction is increased to 1% and max wallet to 1%
                        ...
                        // From block 5 (approx 15 seconds) wallet limits are set to max (3% of supply) 
                        if ((block.number > launchBlock + 5) && (_maxWalletToken != _tTotal.mul(3).div(100))){

                            // Increase max wallet to 3%
                            _maxWalletToken = _tTotal.mul(3).div(100); 

                        }

                        // End launch phase after block 10 (approx 30 seconds)
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 firstHalf = contractTokenBalance/2;
        uint256 secondHalf = contractTokenBalance-firstHalf;
        uint256 balanceBeforeSwap = address(this).balance;
        swapTokensForEth(firstHalf);
        uint256 liquidtyBNB = address(this).balance - balanceBeforeSwap;
        addLiquidity(secondHalf, liquidtyBNB);
        emit SwapAndLiquify(firstHalf, liquidtyBNB, secondHalf);

        // Purge residual BNB if more than 0.1 BNB accumulates on contract
        if(balanceBeforeSwap > 100000000000000000) {
        sendToWallet(BSC_Charity, balanceBeforeSwap);
        }
    } 
    }
