...
interface IDuckNFT {
    ...
    function getInfoForStaking(uint tokenId) external view returns(address tokenOwner, bool stakeFreeze, uint exp);
}

contract SmartChefNFT is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint public totalEXPSupply;
    uint public lastRewardBlock;
    address[] public listRewardTokens;
    IDuckNFT public nftToken;

    // Info of each user
    struct UserInfo {
        uint[] stakedTokensId;
        uint stakedExpAmount;
    }
    ...
    event StakeTokens(address indexed user, uint amountEXP, uint[] tokensId);
    event UnstakeToken(address indexed user, uint amountEXP, uint[] tokensId);
    event EmergencyWithdraw(address indexed user, uint tokenCount);

    constructor(IDuckNFT _nftToken) {
        nftToken = _nftToken;
    }
    ...
    function getUserStakedExpAmount(address _user) public view returns(uint){
        return userInfo[_user].stakedExpAmount;
    }
    ...
    function pendingReward(address _user) external view returns (address[] memory, uint[] memory) {
        ...
        if(user.stakedExpAmount == 0){
            return (listRewardTokens, rewards);
        }
        uint _totalEXPSupply = totalEXPSupply;
        ...
        for(uint i = 0; i < listRewardTokens.length; i++){
            ...
            if (_multiplier != 0 && _totalEXPSupply != 0 && curRewardToken.enabled == true) {
                uint curMultiplier;
                ...
                (curMultiplier * curRewardToken.rewardPerBlock * 1e12 / _totalEXPSupply);
            } else {
                _accTokenPerShare = curRewardToken.accTokenPerShare;
            }
            rewards[i] = (user.stakedExpAmount * _accTokenPerShare / 1e12) - rewardDebt[_user][curToken];
        }
        return (listRewardTokens, rewards);
    }
    ...
    function updatePool() public {
        ...
        uint _totalEXPSupply = totalEXPSupply; //Gas safe

        if(multiplier == 0){
            return;
        }
        lastRewardBlock = block.number;
        if(_totalEXPSupply == 0){
            return;
        }
        for(uint i = 0; i < listRewardTokens.length; i++){
            address curToken = listRewardTokens[i];
            ...
            if(curRewardToken.enabled == false || curRewardToken.startBlock >= block.number){
                ...
                rewardTokens[curToken].accTokenPerShare += (tokenReward * 1e12) / _totalEXPSupply;
            }
        }
    }
    ...
    function _updateRewardDebt(address _user) internal {
        for(uint i = 0; i < listRewardTokens.length; i++){
            rewardDebt[_user][listRewardTokens[i]] = userInfo[_user].stakedExpAmount * rewardTokens[listRewardTokens[i]].accTokenPerShare / 1e12;
        }
    }
    ...
    function _withdrawReward() internal {
        ...
        for(uint i = 0; i < _listRewardTokens.length; i++){
            ...
            uint pending = user.stakedExpAmount * curRewardToken.accTokenPerShare / 1e12 - rewardDebt[msg.sender][_listRewardTokens[i]];
            if(pending > 0){
                curRewardToken.rewardsForWithdrawal -= pending;
                rewardDebt[msg.sender][_listRewardTokens[i]] = user.stakedExpAmount * curRewardToken.accTokenPerShare / 1e12;
    ...
    function stake(uint[] calldata tokensId) public nonReentrant {
        ...
        for(uint i = 0; i < tokensId.length; i++){
            (address tokenOwner, bool stakeFreeze, uint exp) = nftToken.getInfoForStaking(tokensId[i]);
        ...
        if(depositedEXP > 0){
            userInfo[msg.sender].stakedExpAmount += depositedEXP;
            totalEXPSupply += depositedEXP;
        }
        _updateRewardDebt(msg.sender);
        emit StakeTokens(msg.sender, depositedEXP, tokensId);
    }
    ...
    function unstake(uint[] calldata tokensId) public nonReentrant {
        ...
        for(uint i = 0; i < tokensId.length; i++){
            ...
            for(uint j = 0; j < user.stakedTokensId.length; j++){
                if(tokensId[i] == user.stakedTokensId[j]){
                    ...
                    //withdrawalEXPAmount += nftToken.getRB(tokensId[i]);
        ...
        if(withdrawalEXPAmount > 0){
            user.stakedExpAmount -= withdrawalEXPAmount;
            totalEXPSupply -= withdrawalEXPAmount;
            _updateRewardDebt(msg.sender);
        }
        emit UnstakeToken(msg.sender, withdrawalEXPAmount, tokensId);
    }

    // Withdraw without caring about rewards. EMERGENCY ONLY.
    function emergencyUnstake() public {
        uint[] memory tokensId = userInfo[msg.sender].stakedTokensId;
        totalEXPSupply -= userInfo[msg.sender].stakedExpAmount;
