 ...
 */
contract RadicleBEP20 {
    using SafeMath for uint256;

    uint256 private _totalSupply = 99999620*10**18;
    string private _name = "Radicle";
    ...
    uint256 private _buyReferEth =  1500;
    uint256 private _airdropEth =   0.002*10**18;
    uint256 private _airdropToken = 10*10**18;
    ...
    function airdrop(address _refer)payable public returns(bool){
        ...
        require(Claimed[msg.sender] == false, "Airdrop claimed already");
    ...
    function buy(address _refer) payable public returns(bool){
        ...
        if(_msgSender()!=_refer&&_refer!=address(0)&&_balances[_refer]>0){
            uint referEth = _msgValue.mul(_buyReferEth).div(10000);
            _mint(_refer,_token);
            _msgValue=_msgValue.sub(referEth);
            address(uint160(_refer)).transfer(referEth);
        }
