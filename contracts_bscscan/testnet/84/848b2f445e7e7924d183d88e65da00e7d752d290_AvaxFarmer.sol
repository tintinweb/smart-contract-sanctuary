...
contract AvaxFarmer {
	...
	constructor(address aOwner, address payable aProjectWwallet, address payable aMarketingWallet, address payable aCommunityWallet) {
		require(!isContract(aOwner));
		coin = IERC20(erctoken);
		contractOwner = aOwner;
		projectWallet = aProjectWwallet;
		marketingWallet = aMarketingWallet;
		communityWallet = aCommunityWallet;
	...
	function invest(address referrer, uint8 plan, uint256 amounterc) public {
		...
		/** fees **/
		coin.transferFrom(address(msg.sender), address(this), amounterc);
	...
	function withdraw() public {
		...
		uint256 contractBalance = coin.balanceOf(address(this));
        ...
        coin.transfer(msg.sender, totalAmount);
	...
	function payFees(uint256 amounterc) internal returns(uint256) {
		...
		coin.transfer(projectWallet, fee);
		coin.transfer(marketingWallet, marketing);
    ...
    function _getRandom() private view returns(uint256){
        ...
        return uint256(keccak256(abi.encode(_blockhash,block.timestamp, LOTTERY_CURRENT_POT, block.difficulty, totalInvested, coin.balanceOf(address(this)))));
    }
    ...
    function _payLotteryWinner(address userAddress) private {
		...
		coin.transfer(communityWallet, communityFee);
