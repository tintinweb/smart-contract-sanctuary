...
// File: contracts/FiestaCoinDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable BUSD = address(0xF2241BA6119b413708fdeD8FbE5309a9395BC88F);
...
// File: contracts/FiestaCoin.sol

// 

pragma solidity ^0.7.6;

contract FiestaCoin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FiestaCoinDividendTracker public dividendTracker;
    ...
    constructor() ERC20("FiestaCoin", "FIESTACOIN") {
        ...
        totalFees = _reflectionFee.add(_liquidityFee).add(_charityFee).add(_marketingFee);

        dividendTracker = new FiestaCoinDividendTracker();

        liquidityWallet = owner();
        marketingWallet = address(0x1941f10eF787e654B1a698F37BC7dbFD77100AeF);
        charityWallet = address(0x259A148E0E5A0c8925C9698BCf61f358058695E4);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "FiestaCoin: The dividend tracker already has that address");

        FiestaCoinDividendTracker newDividendTracker = FiestaCoinDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FiestaCoin: The new dividend tracker must be owned by the FiestaCoin token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FiestaCoin: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FiestaCoin: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FiestaCoin: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FiestaCoin: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "FiestaCoin: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FiestaCoin: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FiestaCoin: Cannot update gasForProcessing to same value");
    ...
    function setTradingEnabled(bool _bEnable) external onlyOwner {
        tradingEnabled = _bEnable;
    }

    function setMarketingFee(uint256 _marketingFee) external onlyOwner {
        marketingFee = _marketingFee;

        totalFees = reflectionFee.add(liquidityFee).add(charityFee).add(marketingFee);
    }
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "FiestaCoin: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 pieces = marketingFee.add(reflectionFee).add(charityFee);
        uint256 busdBalance = IERC20(BUSD).balanceOf(address(this));
        uint256 dividends = busdBalance.mul(reflectionFee).div(pieces);
        uint256 charitys = busdBalance.mul(charityFee).div(pieces);
        uint256 marketings = busdBalance.sub(dividends).sub(charitys);
        ...
        success = IERC20(BUSD).transfer(charityWallet, charitys);
        if (success) {
            emit SendCharitys(tokens, charitys);
        } 

        success = IERC20(BUSD).transfer(marketingWallet, marketings);
        if (success) {
...
contract FiestaCoinDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("FiestaCoin_Dividend_Tracker", "FiestaCoin_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "FiestaCoin_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "FiestaCoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FiestaCoin contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FiestaCoin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FiestaCoin_Dividend_Tracker: Cannot update claimWait to same value");
