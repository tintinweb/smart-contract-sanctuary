...
contract Auction is ReentrancyGuard, Ownable, Pausable, IERC721Receiver {
        ...
        whenNotPaused
    {
        ...
        emit EvNewBid(id, msg.sender, offer, inv.netBidPrice, inv.endTimestamp);
    }
...
// File contracts/FeeController2.sol
...
contract FeeController2 is AccessControl, IFeeController2 {
    using SafeERC20 for IERC20;
    ...
    Tier[] public tiers;
    mapping(address => Partner) public nftPartners;
    ...
    constructor(
        ...
        updateSettings(maxFeeRate_, feeAddress_, bookkeeper_, farm_, farmPID_);
    ...
    modifier _onlyMarket() {
        require(hasRole(MARKET_ROLE, msg.sender), 'MARKET only');
        _;
    }
    ...
    function updateTiers(uint256 windowSize_, Tier[] calldata tiers_) public _onlyAdmin {
        volumeWindowSize = windowSize_;
    ...
    function partnerRate(address nft) public view returns (uint256) {
        Partner storage p = nftPartners[nft];
        if (p.feeRate > 0 && p.feeAddress != address(0)) {
            return p.feeRate;
        }
        return 0;
    }
    ...
    function feeRate(
        ...
        uint256 level = getLevel(user);
        uint256 prate = partnerRate(nft);
        if (level < tiers.length) {
            return tiers[level].rate + prate;
        } else {
            return defaultFeeRate + prate;
        }
    }

    function addPartner(address nft, address admin) external _onlyAdmin {
        nftPartners[nft].operator = admin;
    }

    function removePartner(address nft) external _onlyAdmin {
        delete nftPartners[nft];
    }
    ...
    function updatePartnerSettings(
        ...
        require(nftPartners[nft].operator == msg.sender, 'operator only');
        require(rate <= maxFeeRate, 'rate should be <= maxFeeRate');
        nftPartners[nft].feeAddress = feeAddr;
        nftPartners[nft].feeRate = rate;
    }
    ...
    function distributeFee(
        ...
        uint256 total,
        uint256 fee
    ) external override _onlyMarket {
        IERC20(currency).safeTransferFrom(msg.sender, address(this), fee);
        Partner storage p = nftPartners[nft];
        uint256 pfee = (total * partnerRate(nft)) / rateBase;
        if (pfee > fee) {
            pfee = fee;
        }
        if (pfee > 0) {
            _transfer(currency, p.feeAddress, pfee);
            fee -= pfee; // leftover
        }
        _transfer(currency, feeAddress, fee);
    }
...
contract Market2 is ReentrancyGuard, Ownable, Pausable {
    ...
    event EvSettingsUpdated(address feeController, address bookkeeper);
    event EvNFTBlacklistUpdate(IERC721 nft, bool blacklisted);
    event EvCurrency(IERC20 indexed token, bool allow);

    // variables

    IFeeController2 public feeController;
    IBookkeeper public bookkeeper;
    IWETH public weth;
    ...
    // settings
    constructor(
        IFeeController2 feeController_,
        IBookkeeper bookkeeper_,
        IWETH weth_
    ) {
        feeController = feeController_;
    ...
    function updateSettings(IFeeController2 feeController_, IBookkeeper bookkeeper_)
        public
        onlyOwner
    {
        feeController = feeController_;
        ...
        emit EvSettingsUpdated(address(feeController_), address(bookkeeper_));
    }
    ...
    function _acceptBuy(uint256 id) internal {
        ...
        _offer.nft.safeTransferFrom(msg.sender, _offer.user, _offer.tokenId);
        _distributePayment(_offer.currency, _offer.price, msg.sender, _offer.nft);
    ...
    function _acceptSell(uint256 id) internal {
        ...
        _offer.nft.safeTransferFrom(_offer.user, msg.sender, _offer.tokenId);
        _distributePayment(_offer.currency, _offer.price, _offer.user, _offer.nft);
    ...
    function _distributePayment(
        ...
        uint256 feeRate = feeController.feeRate(seller, address(nft), address(currency));
        uint256 fee = (totalAmount * feeRate) / feeController.rateBase();
        if (fee > 0) {
            currency.approve(address(feeController), fee);
            feeController.distributeFee(seller, address(nft), address(currency), totalAmount, fee);
        }
