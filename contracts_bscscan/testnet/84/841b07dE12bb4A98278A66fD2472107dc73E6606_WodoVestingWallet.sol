...
contract WodoVestingWallet is VestingWallet, Ownable {
    ...
    *      by passing the value 864000, which is 10 days in seconds, to extend cumulative lock period. 
    ...
    *    
    *  valiation: the given addedTimeSeconds value can not be smaller than the previous addedTimeSeconds value 
    *             to prevent moving the unlock date earlier in time. 
    *
     */
    function extendLockStartTime(uint64 addedTimeSeconds) external virtual onlyOwner {
        ...
        require(addedTimeSeconds > _additionalStartTime, "addedTimeSeconds should be greater than the current addedTimeSeconds .");
        require(uint64(block.timestamp) >= start(), "Vesting has already started.The lock time can not be extended.");
        _additionalStartTime = addedTimeSeconds;
    }
