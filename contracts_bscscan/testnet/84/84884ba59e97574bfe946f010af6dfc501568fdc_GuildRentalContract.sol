...
contract GuildRentalContract is Ownable, TokenRecover, ERC721Holder {
    ...
    function getGuildExpiredListingCount(string calldata _guild_id, address _account) public view returns (uint) {
        uint counter;
        for(uint i = 0; i < _listings.length; i++) {
            if ( (_listings[i].timestamp + duration) > block.timestamp || keccak256(bytes( _listings[i].guildId)) != keccak256(bytes(_guild_id)) || ( _listings[i].borrower != _account && _listings[i].owner != _account ) || ( _listings[i].isWithdraw ) ) continue;
            counter++;
        }
        return counter;
    }
