...
contract Presale is ReentrancyGuard, Context, Ownable {
    ...
    function _deliverTokens() external onlyOwner icoNotActive {
        require(_weiRaised >= softCap);
        uint arrayLength = contributors.length;
        
        for(uint i=0; i< 1000; i++)
        {
        _token.transfer(contributors[0], _getTokenAmount(_contributions[contributors[0]]));
        }
        
            uint leftoverTokens = _token.balanceOf(address(this));
            if(leftoverTokens>0)
            {
            _token.transfer(0x000000000000000000000000000000000000dEaD, leftoverTokens);
            }
        }
