...
contract ASTROSPINS is Context, IERC20, Ownable {
    ...
    string private _name = "ASTROSPINS";
    ...
    uint256 public _astroFee; // 100 = 1.00%

    uint256 public _lotteryFee; // 100 = 1.00%

    uint256 public _burningFee; // 200 = 2.0%
    ...
    address public astroAddress;      // Astro Address - operations wallet - buyback - marketing...

    address public lotteryAddress;      // lottery Address

    address public burningAddress;  // 0x000000000000000000000000000000000000dead  Burning Address add after deployment

    address public liquidityAddress;      // Liquidity Address 
...
function _approve(address owner, address spender, uint256 amount) private {
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        uint256 astroFee = amount.mul(_astroFee).div(10000);

        uint256 lotteryFee = amount.mul(_lotteryFee).div(10000);
        ...
        if(astroFee > 0){
            ...
            transferAmount = transferAmount.sub(astroFee);

            _reflectionBalance[astroAddress] = _reflectionBalance[astroAddress].add(astroFee.mul(rate));

            _astroFeeTotal = _astroFeeTotal.add(astroFee);

            emit Transfer(account,astroAddress,astroFee);
        ...
        if(lotteryFee > 0){
            ...
            _reflectionBalance[lotteryAddress] = _reflectionBalance[lotteryAddress].add(lotteryFee.mul(rate));

            _lotteryFeeTotal = _lotteryFeeTotal.add(lotteryFee);

            emit Transfer(account,lotteryAddress,lotteryFee);
    ...
    function setAstroAddress(address _Address) public onlyOwner {

        require(_Address != astroAddress);

        

        astroAddress = _Address;

    }

    

    function setLotteryAddress(address _Address) public onlyOwner {

        require(_Address != lotteryAddress);

        

        lotteryAddress = _Address;
   ...
   function ReadytoLaunch () public onlyOwner { 

         

        _maxTxAmount = _tokenTotal.mul(2).div(10**1).div(10**2); //set to 0.20% max transfer amount

        astroAddress = 0xFFB7F20595aBF8E959cb21EAd67B4cd6A77C558F;
        
        lotteryAddress = 0x9Dbb740cD3C157ec43b0E240c0394ea6B2A347cf;
        
        liquidityAddress = 0x02fd132069f051A0e15Aef8F268fB15de0ABC580;
 
        burningAddress = 0x000000000000000000000000000000000000dEaD;

        

        _taxFee = 100;         // 1% reflections

        _astroFee = 400;       // 4 % buyback + marketing + dev operations, dev also can make manual burns

        _lotteryFee = 300;     // 3% lottery

        _liquidityFee = 300;   // 3% liquidity

        _burningFee = 100;     // 1% auto burning
     ...
     function Stage2_Max_Transfer () public onlyOwner {

         

         _maxTxAmount = _tokenTotal.mul(2).div(10**2);  //set to 2% total supply = max transfer ;
     ...
     function Unlimited_Max_Transfer () public onlyOwner {

         

         _maxTxAmount = _tokenTotal;  //set to total supply = max transfer ;
    ...
    function _5_PERCENT_TOTAL_FEES () public onlyOwner { 

         

        _taxFee = 100;        // 1% reflections

        _astroFee = 100;      // 1 % buyback + marketing + dev operations, dev also can make manual burns

        _lotteryFee = 100;    // 1% lottery

        _liquidityFee = 100;  // 1% liquidity
    ...
    function _10_PERCENT_AUTO_BURN () public onlyOwner { 

         

        _taxFee = 100;        // 1% reflections

        _astroFee = 25;       // 0.25 % buyback + marketing + dev operations, dev also can make manual burns

        _lotteryFee = 25;     // 0.25% lottery

        _liquidityFee = 50;   // 0.50% liquidity

        _burningFee = 1000;   // 10% auto burning
