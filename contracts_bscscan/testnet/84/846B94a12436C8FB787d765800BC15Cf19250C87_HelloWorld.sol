...
contract HelloWorld is Ownable, ReentrancyGuard {
    ...
    function withdrawfromBridge(uint256 _amount, address _withdrawERC20, address _depositedERC20, uint256[2] memory chainIds, uint256 _nonce, bytes32 _txHash, bytes[] memory signatures, address[] memory _validators) external noContractsAllowed nonReentrant {
        ...
        require(_validators.length == signatures.length, "WithdrawfromBridge:: Invalid Length of Validators");
        require(checkDuplicateValidators(_validators) == 1, "WithdrawfromBridge:: Duplicate Validators Found");
        uint256 noOfApprovals = 0;
        for(uint8 i=0; i <= signatures.length; i++) {
            noOfApprovals.add(isValidTrade(_amount, _withdrawERC20, _depositedERC20, _nonce, _txHash, signatures[i], _validators[i]));
        }
    ...
    function isValidTrade(uint256 _amount, address _withdrawERC20, address _depositedERC20, uint256 _nonce, bytes32 _txHash, bytes memory _signature, address _validatedBy) internal view returns (uint8) {
        (uint8 v, bytes32 r, bytes32 s) = splitSignature(_signature);
        address validatedBy = StringLibrary.getAddress(abi.encodePacked(msg.sender, _amount, _withdrawERC20, _depositedERC20, homeChainId, foreignChainId, _nonce, _txHash, address(this)), v, r, s);
        require(isValidator[validatedBy], "Invalid Validator");
        require(validatedBy == _validatedBy, "Invalid Validator");
        return 1;
    }
