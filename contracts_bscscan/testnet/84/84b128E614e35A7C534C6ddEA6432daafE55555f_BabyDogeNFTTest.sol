...
contract BabyDogeNFTTest is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    bool internal withdrawIsLocked = true;
    ...
    function mintDoge(uint256 numberOfTokens) public payable {
        ...
        require(numberOfTokens <= maxDogePurchase,'numberOfTokens is greater than maxDogePurchase');
        require(totalSupply().add(numberOfTokens) <= MAX_DOGES,'numberOfTokens is greater than MAX_DOGES');
    ...
    function claimPool(uint256[] calldata nftIDs) external nonReentrant {
        require(iterationToClaim[currentIteration.current()] > 0,'iterationToClaim of cuurrent is 0');
        uint256 claimable;
        uint256 balance;
        if (ethPayout) {
        ...
        for (uint256 i = 0; i < nftIDs.length; i++) {
            require(ownerOf(nftIDs[i]) == msg.sender,'caller should be owner of token');
            require(
                iterationClaimed[currentIteration.current()][nftIDs[i]] == false,'iterationClaimed of the current iteration is true'
            );
        ...
        require(claimable > 0,'claimable is 0');
        if (ethPayout) {
    ...
    function claimLotto(uint256 _id) external nonReentrant {
        require(ownerOf(_id) == msg.sender,'caller should be owner of token');
        uint256 prize;
        ...
        require(prize > 0,'price is 0');
    }

    function nextRewardNonce() public {
        require(withdrawIsLocked == true,'withdrawIsLocked is false');
        uint256 teamPortion;
        require(
            block.timestamp >
                iterationTime[currentIteration.current()].add(ITERATION_PERIOD),'time does not reach'
        );
        // require(
    ...
    //     override
    // {
    //     uint256 randInt = randomness % totalSupply().sub(1000);
    //     winners = [
    //         randInt,
    //         randInt.add(1000),
    //         randInt.add(750),
