...
contract FcnFarm is Ownable {
    ...
    function toRandomNumber100() public returns (bool) {
        ...
        for(uint256 i=1;i<=100;i++){
            ...
            emit RandomNumberInt(msg.sender,randomNumberInt,i);
        }
        return true;
    }
    ...
    function toLuckyJackpot() public returns (bool) {
        ...
        if(luckyJackpot5billionCount<=0){
            ...
            for(uint256 j=1;j<=luckyJackpot5billionCount;j++){
                ...
                // Calculation 3
                if(joinFarmOrders[isJoinFarmSuccessIndex[luckyJackpotAddress[j]]].account==luckyJackpotAddress[j]){
                    joinFarmOrders[isJoinFarmSuccessIndex[luckyJackpotAddress[j]]].receiveLuckyJackpotTotal += nowLuckyJackpot.div(luckyJackpot5billionCount);
                }
            }
        }
        nowLuckyJackpot = 0;
