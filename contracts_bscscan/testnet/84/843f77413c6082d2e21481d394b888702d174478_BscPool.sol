...
contract BscPool is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. PNNs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that PNNs distribution occurs.
        uint256 accPnnPerShare; // Accumulated PNNs per share, times 1e12.
    ...
    // The PNN Token!
    IPnn public pnn;
    // PNN tokens created per block.
    uint256 public pnnPerBlock;
    // Info of each pool.
    ...
    // The block number when PNN mining starts.
    ...
    constructor(
        IPnn _pnn,
        uint256 _pnnPerBlock,
        uint256 _startBlock
    ) public {
        pnn = _pnn;
        pnnPerBlock = _pnnPerBlock;
    ...
    // Set the number of pnn produced by each block
    function setPnnPerBlock(uint256 _newPerBlock) public onlyOwner {
        massUpdatePools();
        pnnPerBlock = _newPerBlock;
    }
    ...
    // Update the given pool's PNN allocation point. Can only be called by the owner.
    ...
    function reward(uint256 blockNumber) public view returns (uint256) {
        ...
        return pnnPerBlock.div(2 ** _phase);
    }

    function getPnnBlockReward(uint256 _lastRewardBlock) public view returns (uint256) {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 blockReward = getPnnBlockReward(pool.lastRewardBlock);
        if (blockReward <= 0) {
            return;
        }
        uint256 pnnReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
        bool minRet = pnn.mint(address(this), pnnReward);
        if (minRet) {
            pool.accPnnPerShare = pool.accPnnPerShare.add(pnnReward.mul(1e12).div(lpSupply));
        }
        pool.lastRewardBlock = block.number;
    }

    // View function to see pending PNNs on frontend.
    function pending(uint256 _pid, address _user) external view returns (uint256, uint256){
        PoolInfo storage pool = poolInfo[_pid];
        if (isMultLP(address(pool.lpToken))) {
            (uint256 pnnAmount, uint256 tokenAmount) = pendingPnnAndToken(_pid, _user);
            return (pnnAmount, tokenAmount);
        } else {
            uint256 pnnAmount = pendingPnn(_pid, _user);
            return (pnnAmount, 0);
        }
    }

    function pendingPnnAndToken(uint256 _pid, address _user) private view returns (uint256, uint256){
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accPnnPerShare = pool.accPnnPerShare;
        ...
        if (user.amount > 0) {
            ...
            if (block.number > pool.lastRewardBlock) {
                uint256 blockReward = getPnnBlockReward(pool.lastRewardBlock);
                uint256 pnnReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
                accPnnPerShare = accPnnPerShare.add(pnnReward.mul(1e12).div(pool.totalAmount));
                return (user.amount.mul(accPnnPerShare).div(1e12).sub(user.rewardDebt), userPending);
            }
            if (block.number == pool.lastRewardBlock) {
                return (user.amount.mul(accPnnPerShare).div(1e12).sub(user.rewardDebt), userPending);
            }
    ...
    function pendingPnn(uint256 _pid, address _user) private view returns (uint256){
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accPnnPerShare = pool.accPnnPerShare;
        ...
        if (user.amount > 0) {
            if (block.number > pool.lastRewardBlock) {
                uint256 blockReward = getPnnBlockReward(pool.lastRewardBlock);
                uint256 pnnReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
                accPnnPerShare = accPnnPerShare.add(pnnReward.mul(1e12).div(lpSupply));
                return user.amount.mul(accPnnPerShare).div(1e12).sub(user.rewardDebt);
            }
            if (block.number == pool.lastRewardBlock) {
                return user.amount.mul(accPnnPerShare).div(1e12).sub(user.rewardDebt);
            }
    ...
    // Deposit LP tokens to BscPool for PNN allocation.
    function deposit(uint256 _pid, uint256 _amount) public notPause {
        PoolInfo storage pool = poolInfo[_pid];
        if (isMultLP(address(pool.lpToken))) {
            depositPnnAndToken(_pid, _amount, msg.sender);
        } else {
            depositPnn(_pid, _amount, msg.sender);
        }
    }

    function depositPnnAndToken(uint256 _pid, uint256 _amount, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pendingAmount = user.amount.mul(pool.accPnnPerShare).div(1e12).sub(user.rewardDebt);
            if (pendingAmount > 0) {
                safePnnTransfer(_user, pendingAmount);
            }
        ...
        user.rewardDebt = user.amount.mul(pool.accPnnPerShare).div(1e12);
        user.multLpRewardDebt = user.amount.mul(pool.accMultLpPerShare).div(1e12);
        emit Deposit(_user, _pid, _amount);
    }

    function depositPnn(uint256 _pid, uint256 _amount, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        updatePool(_pid);
        if (user.amount > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public notPause {
        PoolInfo storage pool = poolInfo[_pid];
        if (isMultLP(address(pool.lpToken))) {
            withdrawPnnAndToken(_pid, _amount, msg.sender);
        } else {
            withdrawPnn(_pid, _amount, msg.sender);
        }
    }

    function withdrawPnnAndToken(uint256 _pid, uint256 _amount, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        require(user.amount >= _amount, "withdrawPnnAndToken: not good");
        updatePool(_pid);
        uint256 pendingAmount = user.amount.mul(pool.accPnnPerShare).div(1e12).sub(user.rewardDebt);
        if (pendingAmount > 0) {
            safePnnTransfer(_user, pendingAmount);
    ...
    function withdrawPnn(uint256 _pid, uint256 _amount, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        require(user.amount >= _amount, "withdrawPnn: not good");
        updatePool(_pid);
        uint256 pendingAmount = user.amount.mul(pool.accPnnPerShare).div(1e12).sub(user.rewardDebt);
        if (pendingAmount > 0) {
            safePnnTransfer(_user, pendingAmount);
    ...
    function emergencyWithdraw(uint256 _pid) public notPause {
        PoolInfo storage pool = poolInfo[_pid];
        if (isMultLP(address(pool.lpToken))) {
            emergencyWithdrawPnnAndToken(_pid, msg.sender);
        } else {
            emergencyWithdrawPnn(_pid, msg.sender);
        }
    }

    function emergencyWithdrawPnnAndToken(uint256 _pid, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
    ...
    function emergencyWithdrawPnn(uint256 _pid, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 amount = user.amount;
        user.amount = 0;
    ...
    // Safe PNN transfer function, just in case if rounding error causes pool to not have enough PNNs.
    function safePnnTransfer(address _to, uint256 _amount) internal {
        uint256 pnnBal = pnn.balanceOf(address(this));
        if (_amount > pnnBal) {
            pnn.transfer(_to, pnnBal);
        } else {
            pnn.transfer(_to, _amount);
        }
    }
