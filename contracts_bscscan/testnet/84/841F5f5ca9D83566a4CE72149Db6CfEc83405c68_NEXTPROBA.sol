...
contract NEXTPROBA is ERC20, AccessControl, Pausable {
    // CONFIG START

    uint256 public denominator = 10000;
    
    // TOKEN
    string tokenName = "NEXT1";
    string tokenSymbol = "NEXT1";
    ...
    // BUY TAX
    uint256 public devTaxBuy = 500;

    // SELL TAX
    uint256 public devTaxSell = 500;
    ...
    constructor(address _owner, address payable _devWallet_a, address payable _devWallet_b, address payable _devWallet_c, address payable _devWallet_d, address payable _devWallet_e) ERC20(tokenName, tokenSymbol) {
    ...
    function handleFees(address sender, address recipient, uint256 amount) internal returns (uint256 fee) {
       ...
       // uint256 taxSum;
        ...
        if(isBuy) {
            ...
            fees = amount * 10**18 / denominator * devTaxBuy / 10**18;
            ...
            fees = amount * 10**18 / denominator * devTaxSell / 10**18;
            ...
            if(feeTokens > 0 && exchange == true) {
                ...
                Address.sendValue(payable(devWallet_a), distributeFees);
                Address.sendValue(payable(devWallet_b), distributeFees);
                Address.sendValue(payable(devWallet_c), distributeFees);
                Address.sendValue(payable(devWallet_d), distributeFees);
                Address.sendValue(payable(devWallet_e), distributeFees);
                ...
                uint256 tokenGained = balanceOf(address(this)) - startBalance;
                
                distributeFees = tokenGained / 6;
                ...
                Address.sendValue(payable(devWallet_a), distributeFees * 10**18);
                Address.sendValue(payable(devWallet_b), distributeFees * 10**18);
                Address.sendValue(payable(devWallet_c), distributeFees * 10**18);
                Address.sendValue(payable(devWallet_d), distributeFees * 10**18);
                Address.sendValue(payable(devWallet_e), distributeFees * 10**18);
            ...
            if(!isExcluded[sender] && !isExcluded[recipient]) {
                ...
                if(sender == address(_UniswapV2Pair) || recipient == address(_UniswapV2Pair)) {
                    ...
                    if(recipient == address(_UniswapV2Pair)) {
                        require(block.number >= nextBuyBlock[sender], "HFT: Cooldown");
                        require(amount <= maxSellTxAmount, "HFT: Max sell tx amount");

                        nextBuyBlock[sender] = block.number + 1;
                    }
    ...
    function setDevWalletAddress(address payable _devWallet_a, address payable _devWallet_b, address payable _devWallet_c, address payable _devWallet_d, address payable _devWallet_e) external onlyRole(OWNER_ROLE) {
        devWallet_a = _devWallet_a;
        devWallet_b = _devWallet_b;
        devWallet_c = _devWallet_c;
        devWallet_d = _devWallet_d;
    ...
    function setTaxesBuy(uint256 dev) external onlyRole(OWNER_ROLE) {
        devTaxBuy = dev;
    }

    function setTaxesSell(uint256 dev) external onlyRole(OWNER_ROLE) {
        devTaxSell = dev;
     }
