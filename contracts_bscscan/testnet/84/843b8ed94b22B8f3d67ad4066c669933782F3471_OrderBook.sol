...
contract OrderBook is Initializable, Ownable, ReentrancyGuard, IOrderBook {
  ...
  uint16 constant public _maxLoop = 1000;
  int24 constant public _maxTradeItem = 500;
  ...
  mapping(uint8 => int24) public _nodeCounts;
  ...
  function makeOrder(uint8 position, uint256 securityAmt, uint256 currencyAmt) external override {
    require(position == 1 || position == 2, "makeOrder: position invalid");
    require(securityAmt > 0 && currencyAmt > 0, "makeOrder: token amount should > 0");

    uint256 price = __getTradePrice(securityAmt, currencyAmt);
    ...
    if (headNodeOpAddr == address(0)) {
      ...
      if (pos == 1) {
        ...
        // matched oppo order, try trading
        __fillMatchedOrder(position, securityAmt, currencyAmt, currNodeOp.nodeAddr);
      } else {
        // price is more fit for order
        __fillOrder(position, price, securityAmt, currencyAmt);
      }
    }
    emit OrderBookMaked(position, price, securityAmt, currencyAmt, msg.sender);
  }
  ...
  function killOrder(address orderItemAddr) external override {
    ...
    require(msg.sender == orderCreator || msg.sender == owner(), "only order creator can kill order");
    ...
    // kill order use mark sweep
    __flushOrderItem(orderItemAddr);
  ...
  function findOrderNode(uint8 position, uint256 price) public view returns (bool inNode, address prevAddr, address nextAddr) {
    ...
    int24 loopCount = _nodeCounts[position];

    for (int24 i = 0; i <= loopCount; i++) {
  ...
  function viewOrderNode(uint8 position, uint16 count) external view override returns (OrderNode[50] memory) {
    require(count < 50, "view count should < 50");
  ...
  function viewOrderItem(address orderNodeAddr, uint16 count) external view override returns (OrderItem[50] memory) {
    require(count < 50, "view count should < 50");
    require(orderNodeAddr != address(0), "view address invalid");

    OrderItem[50] memory orderItems;

    OrderNode memory currNode = _orderNodeMap[orderNodeAddr];

    address currAddr = currNode.headItem;
  ...
  function __bookOrder(uint8 position, uint256 price, uint256 securityAmt, uint256 currencyAmt) private {
    require(securityAmt > 0 && currencyAmt > 0, "bookOrder: token amount should > 0");
    ...
    if (inNode) {
      ...
      currNode.balance = currNode.balance.add(currItem.balance);
      currNode.capacity = currNode.capacity.add(currItem.capacity);
  ...
  function __fillOrder(uint8 position, uint256 price, uint256 securityAmt, uint256 currencyAmt) private {
    ...
    int24 loopCount = _nodeCounts[positionOp];
    ...
    for (int24 i = 0; i < loopCount; i++) {
      ...
      if (position == 1) {
        leftAmt = __fillMatchedOrder(position, 0, leftAmt, currNodeAddr);
      } else if (position == 2) {
        leftAmt = __fillMatchedOrder(position, leftAmt, 0, currNodeAddr);
      }
      if (leftAmt <= 0) {
        break;
      }
    ...
    if (leftAmt > 0) {
      if (position == 1) {
        __bookOrder(position, price, 0, leftAmt);
      } else if (position == 2) {
        __bookOrder(position, price, leftAmt, 0);
      }
    }
  }

  function __fillMatchedOrder(uint8 position, uint256 securityAmt, uint256 currencyAmt, address orderNodeOpAddr) private returns (uint256 leftAmt) {
    leftAmt = 0;
    if (position == 1) {
      leftAmt = currencyAmt;
    } else if (position == 2) {
    ...
    OrderNode storage currNodeOp = _orderNodeMap[orderNodeOpAddr];
    address currItemAddr = currNodeOp.headItem;
    int24 loopCount = currNodeOp.itemCount;

    for (int24 i = 0; i < loopCount; i++) {
      if (currItemAddr == address(0)) {
        break;
      }

      OrderItem storage currItem = _orderItemMap[currItemAddr];
      if (currItem.status == 3 || currItem.status == 4) {
        currItemAddr = __cleanAndMoveOrderItem(currItemAddr);
        continue;
      }

      ITradeOrderV2 tradeItem = ITradeOrderV2(currItem.data);
      uint256 prevBalance = tradeItem.balance();
      uint256 prevCapacity = tradeItem.capacity();
      // partly fill order item
      if (leftAmt < currItem.capacity) {
        if (position == 1) {
          __makeTradeOrder(tradeItem, position, 0, leftAmt, 2);
        } else if (position == 2) {
          __makeTradeOrder(tradeItem, position, leftAmt, 0, 2);
        }

        currItem.balance = tradeItem.balance();
        currItem.capacity = tradeItem.capacity();
        currItem.status = 2;
        
        currNodeOp.balance = currNodeOp.balance.sub(prevBalance - currItem.balance);
        currNodeOp.capacity = currNodeOp.capacity.sub(prevCapacity - currItem.capacity);

        leftAmt = 0;
        break;
      } else {
        // fullfill order item
        leftAmt = leftAmt.sub(prevCapacity);
        if (position == 1) {
          __makeTradeOrder(tradeItem, position, 0, prevCapacity, 2);
        } else if (position == 2) {
          __makeTradeOrder(tradeItem, position, prevCapacity, 0, 2);
        }
        currItemAddr = __cleanAndMoveOrderItem(currItemAddr);
      }
    }
  ...
  function __cleanAndMoveOrderItem(address orderItemAddr) private returns (address) {
    ...
    currNode.balance = currNode.balance.sub(currItem.balance);
    currNode.capacity = currNode.capacity.sub(currItem.capacity);
    currNode.itemCount -= 1;
  ...
  // mark sweep order item
  // for kill order
  function __flushOrderItem(address orderItemAddr) private {
    OrderItem storage currItem = _orderItemMap[orderItemAddr];
    currItem.balance = 0;
    currItem.capacity = 0;
    currItem.status = 4;

    address orderNodeAddr = currItem.parent;
    OrderNode storage orderNode = _orderNodeMap[orderNodeAddr];
    orderNode.balance = orderNode.balance.sub(currItem.balance);
    orderNode.capacity = orderNode.capacity.sub(currItem.capacity);
  }
  ...
  function __approve(IERC20 token, address spender, uint256 amount) private {
  ...
  function __makeTradeOrder(ITradeOrderV2 tradeOrder, uint8 position, uint256 securityAmt, uint256 currencyAmt, uint8 orderType) private {
    ...
    if (amount1 > 0) {
      token1.safeTransfer(transferTo, amount1);
    }

    emit TokenTransfered(address(token0), amount0, msg.sender, address(token1), amount1, transferTo);
  }

  function __getTradePrice(uint256 security, uint256 currency) internal pure returns (uint256) {
    require(security > 0 && currency > 0, "trade amount should both > 0");
    return currency.mul(1e18).div(security);
  }
...
interface IOrderBook {
  event OrderBookMaked(uint8 position, uint256 price, uint256 securityAmt, uint256 currencyAmt, address orderMaker);
  ...
  event TokenTransfered(address token0, uint256 amount0, address account0, address token1, uint256 amount1, address account1);
  ...
  function makeOrder(uint8 position, uint256 securityAmt, uint256 currencyAmt) external;
  function killOrder(address orderItemAddr) external;
  function viewOrderNode(uint8 position, uint16 count) external view returns (OrderNode[50] memory);
  function viewOrderItem(address orderNodeAddr, uint16 count) external view returns (OrderItem[50] memory);
...
interface ITradeOrderV2 {
  ...
  event OrderBooked(uint8 position, address security, address currency, uint256 amount, address orderCreator);
