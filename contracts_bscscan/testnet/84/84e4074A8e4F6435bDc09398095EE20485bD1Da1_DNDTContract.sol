...
contract DNDTContract {
    ...
    event WithdrawDNDT(address, uint);
    ...
    function initialize(address _admin, address _teamAddr, address _usdtAddr, address _dndtAddr) external {
        require(!initialized,"initialized");
        admin = _admin;
        teamAddr = _teamAddr;
        USDT = IERC20Upgradeable(_usdtAddr);
        DNDT = IERC20Upgradeable(_dndtAddr);
        poolRate = 90;
        teamRate = 10;
        userRate = 97;
        team2Rate = 3;
    ...
    function withdrawDNDT(address _addr, uint _amount) external onlyAdmin {
        require(_addr!=address(0),"Can not withdraw to Blackhole");
        DNDT.transfer(_addr, _amount);
    }

    function batchAdminWithdrawDNDT(address[] memory _userList, uint[] memory _amount) external onlyAdmin {
        for (uint i = 0; i < _userList.length; i++) {
            DNDT.transfer(address(_userList[i]), uint(_amount[i]));
        }
    }

    function burnDNDT(uint _amount) external onlyAdmin {
        DNDT.transferFrom(msg.sender,address(0x000000000000000000000000000000000000dEaD), _amount);
