...
contract Ninneko is Initializable, ERC721Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    uint8 private constant MAX_BREED_COUNT = 9;
    uint256[9] public breed_cost;
    ...
    function initialize(
        ...
        breed_cost = [100, 200, 300, 600, 1000, 2000, 3500, 7200, 10000];
    }
    ...
    modifier validateBreed(uint256 _pet1Id, uint256 _pet2Id) {
        ...
        uint256 feeBreed = fee1.add(fee2);
        require(_getNINOBalance() > breedNINOFee, "Insufficient NINO");
        require(_getMATABalance() > feeBreed, "Insufficient MATA");
        _;
    }
    ...
    function _setAcceptedTokenContract(address addNINO, address addMATA) private {
        ...
        NINOToken = IERC20(addNINO);
        MATAToken = IERC20(addMATA);
    }
    ...
    function SetBreedCost(uint256[9] memory _newBreedCost) external onlyOwner {
        breed_cost = _newBreedCost;
    }
    ...
    function breed(uint256 _pet1Id, uint256 _pet2Id) external payable validateBreed(_pet1Id, _pet2Id) {
        NINOToken.transfer(owner(), _getNINOBalance());
        MATAToken.transfer(owner(), _getMATABalance());
    ...
    function putListPetOnSale(uint256[] memory _listPetId, uint256 _petPrice) external onlyListPetOwner(_listPetId) {
    ...
    function putOnSale(uint256 _petId, uint256 _price) external onlyPetOwner(_petId) {
    ...
    function cancelSale(uint256 _petId) external onlyPetOwner(_petId) {
    ...
    function _getBreedPrice(Pet memory _pet) private view returns (uint256) {
        if (_pet.generation == 1) {
            ...
            return breed_cost[_pet.breedCount];
        }
    }

    function buyPet(uint256 _petId) external payable nonReentrant {
        ...
        // hero cancle in _transfer function
    ...
    function burn(uint256 _petId) external onlyPetOwner(_petId) {
