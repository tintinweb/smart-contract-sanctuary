...
contract MaharajaDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Maharaja_Dividend_Tracker", "Maharaja_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000_00 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Maharaja_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Maharaja_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Maharaja contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Maharaja_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "Maharaja_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Maharaja is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    MaharajaDividendTracker public dividendTracker;

    uint256 public _totalSupply = 1_000_000_000_000_000 * (10**9);
    uint256 public maxSellTransactionAmount = _totalSupply.div(100);
    uint256 public swapThreshold = _totalSupply.div(1000_000_00);
    uint256 public maxLimit = _totalSupply.div(100).mul(6);
    ...
    constructor() ERC20("Maharaja", "Maharaja") 
    {
        BNBRewardsFee = 4;
        liquidityFee = 3;
        marketingFee = 2;
        ...
        marketingWallet = payable(0xc85bFE12db63992F2c0Bd32654c1A406766A1888);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new MaharajaDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Maharaja: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Maharaja: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Maharaja: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Maharaja: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Maharaja: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Maharaja: Cannot update gasForProcessing to same value");
