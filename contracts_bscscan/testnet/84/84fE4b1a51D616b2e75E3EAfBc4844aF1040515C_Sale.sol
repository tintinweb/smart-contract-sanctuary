...
//      initDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E); // mainnet bsc
    ...
    function claimable(address account) view external returns (uint256) {
        uint256 amount;
        for (uint256 i = 0; i < stages.length; i++) {
            amount += claimableByStageIndex(i, account);
        }
        return amount;
    }

    function claimableByStageIndex(uint256 stageIndex, address account) view public returns (uint256) {
        ...
        uint256 absolutePercent = _absolutePercentageByStageIndex(stageIndex);
    ...
    function _absolutePercentageByStageIndex(uint256 stageIndex) view private returns (uint256) {
        Stage memory stage = stages[stageIndex];

        uint256 totalPercent = stage.firstReleasePercent; // 100.00%

        if (stage.offsetTime < block.timestamp - tgeTimestamp) {
            uint256 deltaTime = block.timestamp - tgeTimestamp - stage.offsetTime;
    ...
    function claim() external {
        ...
        for (uint256 i = 0; i < stages.length; i++) {
            amountByStage = claimableByStageIndex(i, msg.sender);
            if (amountByStage > 0) {
