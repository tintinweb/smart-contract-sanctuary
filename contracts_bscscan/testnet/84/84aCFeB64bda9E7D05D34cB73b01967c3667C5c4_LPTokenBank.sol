...
contract LPTokenBank is Bank {
	address[] public stakingLP0ToStable;
	...
	constructor(
		...
		address[] memory _stakingLP0ToStable,
	...
	{
		stakingLP0ToStable = _stakingLP0ToStable;
		stakingLP1ToStable = _stakingLP1ToStable;
	}

	function _getStakedTokenPrice() internal view override returns (uint256) {
		return
			_getLPTokenPrice(
				router,
				stakingLP0ToStable,
...
import '../../common/PriceCalculator.sol';
import '../../common/Statistics.sol';

abstract contract Bank is AccessControlEnumerable, ReentrancyGuard, Statistics {
	...
	// dev reward tokens created per block.
	uint256 public devRewardPerBlock;
	...
	event NewDevRewardPerBlock(uint256 rewardPerBlock);
	event NewDevAddress(address indexed oldAddress, address indexed newAddress);
	event NewPoolLimit(uint256 poolLimitPerUser);
	event RewardsStop(uint256 blockNumber);
	event Withdraw(address indexed user, uint256 amount);

	function _getStakedTokenPrice() internal view virtual returns (uint256);
	...
	/*
	 * @notice Update dev reward per block
	 * @dev Only callable by owner.
	 * @param _devRewardPerBlock: the dev reward per block
	 */
	function updateDevRewardPerBlock(uint256 _devRewardPerBlock)
		external
		onlyRole(DEFAULT_ADMIN_ROLE)
	{
		devRewardPerBlock = _devRewardPerBlock;
		emit NewDevRewardPerBlock(_devRewardPerBlock);
	}
	...
	function APR() external view override returns (uint256) {
		...
		return
			TVL() > 0
				? (rewardPerBlock *
					10512000 *
					_getTokenPrice(router, rewardToStablePath)) / TVL()
				: 0;
	}
	...
	function _updatePool() internal {
		...
		uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
		uint256 reward = multiplier.mul(rewardPerBlock);
		uint256 devReward = multiplier.mul(devRewardPerBlock);
		...
		if (devReward > 0)
			IMintableToken(address(rewardToken)).mint(devAddress, devReward);
	}
