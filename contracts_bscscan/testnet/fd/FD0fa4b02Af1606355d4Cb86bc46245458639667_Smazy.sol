...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _transferToOffChainWallet(address sender, uint256 amount)
        internal
        virtual
    {
        require(sender != address(0), "ERC20: transfer from the zero address");
    ...
    function _transferToOnChainWallet(address recipient, uint256 amount)
        internal
        virtual
    {
        require(
...
contract Smazy is Ownable, ERC20 {
    ...
    function depositToken(address tokenAddress, uint256 amount) public {
        ...
        emit DepositToken(tokenAddress, _msgSender(), amount);
    }
    ...
    function deposit(uint256 amount) public {
        ...
        _transferToOffChainWallet(_msgSender(), amount);
        emit Deposit(_msgSender(), amount);
    }

    function withdraw(address recipient, uint256 amount) public onlyOwner {
        _transferToOnChainWallet(_msgSender(), amount);
    ...
    event Deposit(address indexed userAddress, uint256 indexed amount);
    event Withdraw(address indexed userAddress, uint256 indexed amount);
