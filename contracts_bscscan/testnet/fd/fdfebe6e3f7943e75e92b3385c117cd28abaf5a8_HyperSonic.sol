...
contract DividendPayingToken is BEP20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0x8A6E3213a3351A7F587894f84Fe07C7F86aC7130;
...
contract HyperSonic is BEP20, Ownable {
    ...
    address public _dividendToken = 0x8A6E3213a3351A7F587894f84Fe07C7F86aC7130;
    ...
    constructor() BEP20("HYPERSONIC", "HyperSonic") {
		...
		IUniswapV2Router02 _uniswapV1Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
    ...
    receive() external payable {
  	...
  	function whitelistDxSale(address _presaleAddress) public onlyOwner {
	    ...
	    _isExcludedFromMaxWalletToken[_presaleAddress] = true;
  	}
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(from != owner() && from != presaleAddress && !_isExcludedFromMaxWalletToken[to] && !automatedMarketMakerPairs[to]) 
		{
            uint256 contractBalanceRecepient = balanceOf(to);
            require(contractBalanceRecepient + amount <= maxWalletToken, "Exceeds maximum wallet token amount");
        }
		
		if(from != owner() && automatedMarketMakerPairs[to] && from != presaleAddress) 
		{
		...
		if(automatedMarketMakerPairs[from] || (from == presaleAddress && presaleAddress != address(0)))
		{
        ...
        if(takeFee) 
		{
			...
			{   
	            ...
	            uint256 tPercentage = 0;
			    if(block.timestamp > (lastbuytime + 7 days))
			    {
					tPercentage = 1500;
			    }
			    else if(block.timestamp > (lastbuytime + 5 days) && block.timestamp <= (lastbuytime + 7 days))
			    {
					tPercentage = 1800;
			    }
			    else if(block.timestamp > (lastbuytime + 3 days) && block.timestamp <= (lastbuytime + 5 days))
			    {
					tPercentage = 2000;
			    }
			    else if(block.timestamp > (lastbuytime + 2 days) && block.timestamp <= (lastbuytime + 3 days))
			    {
					tPercentage = 2500;
			    }
			    else if(block.timestamp > (lastbuytime + 1 days) && block.timestamp <= (lastbuytime + 2 days))
			    {
					tPercentage = 3000;
			    }
			    else if(lastbuytime > 0)
			    {
				    tPercentage = 4000;
			    }
