...
contract AlarmFactory is Context {
    ...
    RigelAlarmClock[] public ETHExchange;
    RigelAlarmClockTokens[] public SwapTokenForToken;
    ...
    function getUserSwapFromETHtoETHInfo(uint _index)
        public
        view
        returns (
            address _owner,
    ...
    {
        RigelAlarmClock alarm = ETHExchange[_index];

        return (alarm.callee(), address(alarm).balance);
    }

    function getUserSwapFromTokenToTokensInfo(uint _index)
        public
        view
        returns (
            address _owner,
    ...
    {
        RigelAlarmClockTokens alarm = SwapTokenForToken[_index];
