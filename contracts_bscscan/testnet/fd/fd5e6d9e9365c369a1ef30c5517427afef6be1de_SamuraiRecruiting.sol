 ...
 */
abstract contract RSunAuth is IRSunAuth {
...
interface IMysteriousCrates is IRSunAuth {
    function mintCrate(address user, uint[] memory samuraiIds) external;
...
contract MysteriousCrates is ERC721Enumerable, RSunAuth, IMysteriousCrates {
    ...
    function mintCrate(address user, uint[] memory samuraiIds) external override {
        authorizedFor(Permission.MintCrates);
        require(samuraiIds.length == 3, "Incorrect number of IDs");

        uint mintIndex = currentIndex;

        _safeMint(user, mintIndex);
        
        crates[mintIndex].ids = getIdsAsSingleUint(samuraiIds);
    }

    function openCrate(uint crateId) external override {
        require(ownerOf(crateId) == msg.sender, "Can only open one's own crates");

        samurai.teamMintPack(packId, msg.sender);
    }

    function getIdsAsSingleUint(uint[] memory samuraiIds) public pure returns(uint72 ids) {
        ids = uint72(samuraiIds[0]);
        ids |= uint72(samuraiIds[1] << 24);
        ids |= uint72(samuraiIds[2] << 48);
    }

    function getIdsAsArray(uint crateId) public view returns(uint[] memory arr) {
        uint72 ids = crates[crateId].ids;
...
contract SamuraiRecruiting is RSunAuth, IERC721Receiver {
    ...
    address public rsunAdr = 0xb4A9e6b7d1fb38404Cef32069CAe49F5d5317Da7;
    address public infAdr = 0xF5ccc7150A3Afe6d490B90AA47498Cf54EC6CF6A;
    address public samuraiAdr = 0x14a3Ee3771845cee9EA2D49Fcca8DDA58f5D5D8b; // 0xC3c3B849ED5164Fb626c4a4F78e0675907B2C94E Mainnet
    address public cratesAdr;

    uint public maxUsages = 10;
    uint public rsunCost = 15000 gwei; // 15k RSUN ~ 15-20$

    mapping(uint => uint) public usages; // how often a samurai has been used in recruiting
    mapping(address => uint[]) public stakeOwnerships;
    Stake[] public stakes;

    ISamuraiRising public samurai;
    IMysteriousCrates public crates;

    event DebugLog(string message, uint value);

    constructor() RSunAuth(msg.sender) {
        samurai = ISamuraiRising(samuraiAdr);
        crates = new MysteriousCrates();
        cratesAdr = address(crates);
        crates.authorizeForAllPermissions(msg.sender);
    }

    function recruit(uint[] memory ids) external {
        _recruit(ids, msg.sender);
    }

    function _recruit(uint[] memory ids, address user) internal {
        _checkSamurai(ids);
        require(IBEP20(rsunAdr).transferFrom(msg.sender, address(this), rsunCost), "RSUN payment failed");
        require(IBEP20(infAdr).transferFrom(msg.sender, address(this), getInfCost(ids)), "INF payment failed");

        for (uint256 i = 0; i < ids.length; i++) {
            uint id = ids[i];
            try samurai.safeTransferFrom(user, address(this), id) {
                usages[id]++;
            } catch {
                revert("Failed to transfer samurai to contract");
            }
        }

        stakeOwnerships[user].push(stakes.length);
        stakes.push(Stake(getIdsAsSingleUint(ids), uint40(block.timestamp), getLockDuration(ids), false));

        crates.mintCrate(user, ids);
    }

    function _checkSamurai(uint[] memory ids) internal view returns (uint8 num) {
        require(ids.length == 3, "Recruiting takes 3 samurai");
        for (uint256 i = 0; i < ids.length; i++) {
            require(usages[ids[i]] < maxUsages, "Recruitment limit of one or more samurai exceeded");
            if (ids[i] < 5000) num++;
        }
    }

    function retrieveSamurai(uint stakeId) external {
        Stake memory s = stakes[stakeId];
        require(block.timestamp >= s.startTimestamp + s.duration, "Staking period hasn't ended yet");
        require(s.retrieved == false, "Samurai have already been retrieved");
        
        uint[] memory ids = getIdsAsArray(stakeId);

        stakes[stakeId].retrieved = true;

        for (uint256 i = 0; i < ids.length; i++) {
            samurai.safeTransferFrom(address(this), msg.sender, ids[i]);
        }
    }

    function getLockDuration(uint[] memory ids) public view returns (uint32 dur) {
        for (uint256 i = 0; i < ids.length; i++) {
            dur += uint32(4 hours + usages[ids[i]] * 2 hours);
        }
    }

    function getUnlockTime(uint stakeId) public view returns (uint time) {
        Stake memory s = stakes[stakeId];
        return s.startTimestamp + s.duration;
    }

    function getInfCost(uint[] memory ids) public view returns (uint infCost) {
        for (uint256 i = 0; i < ids.length; i++) {
            uint timesUsed = usages[ids[i]];
            infCost += 1000 gwei + (timesUsed * 1000 gwei);
        }
    }
    ...
    function getIdsAsArray(uint stakeId) public view returns(uint[] memory arr) {
        uint72 ids = stakes[stakeId].cardIds;
    ...
    function getAllStakesOfUser(address user) public view returns(Stake[] memory stakes_) {
        uint[] memory stakeIds = stakeOwnerships[user];
        
        stakes_ = new Stake[](stakeIds.length);
        for (uint256 i = 0; i < stakeIds.length; i++) {
            stakes_[i] = stakes[stakeIds[i]];
        }
    }

    function setRsunAdr(address rsun_) external {
        authorizedFor(Permission.AdjustVariables);
        rsunAdr = rsun_;
    }
    ...
    function setRsunCost(uint newCost) external {
        authorizedFor(Permission.AdjustVariables);
        rsunCost = newCost;
    }
