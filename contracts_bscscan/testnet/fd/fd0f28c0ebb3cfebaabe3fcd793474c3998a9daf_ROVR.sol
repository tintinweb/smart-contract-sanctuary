...
contract ROVR is ERC20, Ownable, VestController {
    ...
    function updateSwapV2Router(address newAddress) external onlyOwner {
        require(
            ...
            "ROVR: The router already has that address"
        );
        ...
        onlyOwner
    {
        require(
            pair != swapV2Pair,
            "ROVR: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "ROVR: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function _transfer(
        ...
        if (!tradingIsEnabled) {
            revert(
                "ROVR: This account cannot send tokens until trading is enabled"
            );
        }
