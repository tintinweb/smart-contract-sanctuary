...
contract FarmageddonLottoKeeper is KeeperCompatibleInterface, Ownable {
    ...
    // start the lottery contract with endtime variable and other information from setup
     function StartLotteryAndKeeper(uint256 _LotteryEndTime) external onlyOwner {
        LotteryAddress.startLottery(_LotteryEndTime, fticketPrice, fdiscount, frewardsBreakdown, ftreasuryFee);
        fcurrentLotteryId = LotteryAddress.viewCurrentLotteryId();
        upKeepTime = _LotteryEndTime;
        step = 1;
    }
