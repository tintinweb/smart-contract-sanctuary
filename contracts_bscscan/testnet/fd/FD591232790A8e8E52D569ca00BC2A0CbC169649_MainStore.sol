 ...
 */
contract MainStore is AssetContract, ReentrancyGuard {
    // Migration contract address
    MainStore public migrationTarget;
    ...
    modifier creatorOnly(uint256 _id) {
        require(
            ...
            "MainStore#creatorOnly: ONLY_CREATOR_ALLOWED"
        );
        _;
    }
    ...
    modifier onlyFullTokenOwner(uint256 _id) {
        require(
            ...
            "MainStore#onlyFullTokenOwner: ONLY_FULL_TOKEN_OWNER_ALLOWED"
        );
        _;
    }
    ...
    constructor(
        ...
        migrationTarget = MainStore(_migrationAddress);
    }
    ...
    function disableMigrate() public onlyOwnerOrProxy {
        migrationTarget = MainStore(address(0));
    }
    ...
    function migrate(Ownership[] memory _ownerships) public onlyOwnerOrProxy {
        MainStore _migrationTarget = migrationTarget;
        require(
            _migrationTarget != MainStore(address(0)),
            "MainStore#migrate: MIGRATE_DISABLED"
        );
        ...
        for (uint256 i = 0; i < _ownerships.length; ++i) {
            ...
            require(
                owner != address(0),
                "MainStore#migrate: ZERO_ADDRESS_NOT_ALLOWED"
            );
    ...
    function batchMint(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            require(
                ...
                "MainStore#_batchMint: ONLY_CREATOR_ALLOWED"
            );
        }
        _batchMint(_to, _ids, _quantities, _data);
    }
    ...
    function setCreator(uint256 _id, address _to) public creatorOnly(_id) {
        require(_to != address(0), "MainStore#setCreator: INVALID_ADDRESS.");
    ...
    function _requireMintable(address _address, uint256 _id) internal view {
        require(
            ...
            "MainStore#_requireMintable: ONLY_CREATOR_ALLOWED"
        );
    }
