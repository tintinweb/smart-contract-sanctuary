...
contract Ninneko is Initializable, ERC721Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    uint256[9] public breed_cost; // unused

    Pet[] public pets;
    mapping(uint256 => uint256) public petsOnSale;
    mapping(address => bool) public operators; // unused
    ...
    event PetCreated(address indexed owner, uint256 indexed petId, uint256 matronId, uint256 sireId, uint8 generation);
    ...
    function initialize(
        ...
        pets.push(Pet(0, 0, 0, 0, 0)); // Pet #0 belongs to none
        serviceFee = 300;
    ...
    modifier onlyOperator() {
        require(msg.sender == operator || msg.sender == owner(), "You're not the operator or owner of this contract");
        _;
    }

    modifier onlyPetOwner(uint256 _petId) {
        require(ownerOf(_petId) == msg.sender, "You're not the owner of this one");
        _;
    }
    ...
    modifier onlyListPetOwner(uint256[] memory _listPetId) {
        for (uint256 i = 0; i < _listPetId.length; i++) {
            ...
            require(ownerOf(petId) == msg.sender, "You're not the owner of these");
        }
        _;
    }
    ...
    modifier validateBreed(uint256 _pet1Id, uint256 _pet2Id) {
        require(ownerOf(_pet1Id) == msg.sender && ownerOf(_pet2Id) == msg.sender, "You're not the owner of this one");
    ...
    function calculateServiceFee(uint256 _price) private view returns (uint256) {
        return _price.mul(serviceFee).div(FEE_RATIO);
    }
    ...
    function breed(uint256 _pet1Id, uint256 _pet2Id) external whenNotPausedBreed validateBreed(_pet1Id, _pet2Id) {
        ...
        uint256 breedMATAFee = fee1.add(fee2);
        ...
        MATAToken.safeTransferFrom(msg.sender, burnAddress, breedMATAFee);
        pets[_pet1Id].breedCount++;
        pets[_pet2Id].breedCount++;
        uint256 petId = _createPet(_pet1Id, _pet2Id, 2); // generation is always = 2
    ...
    function batchMint(uint8 quantity, uint8 generation) external onlyOperator {
        for (uint8 i = 0; i < quantity; i++) {
            uint256 petId = _createPet(0, 0, generation);
            _safeMint(msg.sender, petId);
        }
    }
    ...
    function _createPet(
        ...
        pets.push(Pet(generation, 0, uint32(block.timestamp), matronId, sireId));
        _petId = pets.length - 1;
        emit PetCreated(msg.sender, _petId, matronId, sireId, generation);
    }
    ...
    function _makeTransaction(
        ...
        (bool transferToSeller, ) = _seller.call{value: _price.sub(fee)}("");
