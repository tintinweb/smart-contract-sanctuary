...
contract StableCoinAdaptor is IStableCoinAdaptor, AccessControlEnumerableUpgradeable, ERC20CappedUpgradeable {
    mapping(address => uint256) private _balances;
    uint256 private _totalSupply;
    mapping(address => address) private _whiteList;

    IStableCoinBank private bank;
    ...
    function initialize(
        ...
        bank = IStableCoinBank(bankAddr_);
    }
    ...
    function _transfer(
        ...
        if (sender == vault && senderBalance < amount) {
            bank.updateVault();
        ...
        if (_whiteList[recipient] != address(0)) {
            _balances[address(bank)] += amount;
            bank.send(recipient, amount, _whiteList[recipient]);
    ...
    function addToWhiteList(address to, address token) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        _whiteList[to] = token;
    }
