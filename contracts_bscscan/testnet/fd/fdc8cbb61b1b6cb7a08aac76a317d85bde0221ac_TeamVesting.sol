...
contract TeamVesting is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
	...
	/* the cliff time of the team vesting */
    uint256 private cliffTime;

	/* the duration of the team vesting */
    uint256 private duration;

    /* the address of the token contract */
    IERC20Upgradeable private tokenReward;

	/* the balances (in INERY) of all team members */
	mapping(address => uint256) public balanceOfINERY;

	/* the whitelist of the team */
    ...
    function initialize(
        ...
        tokenReward = _token;
        tgeTime = _tge;
		cliffTime = tgeTime.add(60 * 60 * 24 * 365);	// 1 year cliff
		duration = 60 * 60 * 24 * 30 * 24; // 24 months (720 days)
    }
	...
	function _vestedAmount(address addr) private view returns (uint256) {
		...
		// after 1 year cliff + duration
			...
			return balanceOfINERY[addr];
		// linear part during 1 year cliff + 24 months
		} else {
