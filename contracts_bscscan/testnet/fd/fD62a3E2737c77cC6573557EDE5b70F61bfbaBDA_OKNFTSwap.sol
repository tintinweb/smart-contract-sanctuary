...
contract OKNFTSwap {
    ...
    function swap(MarketRegistry.TradeDetails[]   memory tradeDetails)  payable public{

        for(uint256 i = 0; i < tradeDetails.length; i++){
            (bool success, bytes memory returnData) = exchange.call{value:msg.value}(tradeDetails[i].tradeData);
