...
contract CoinToken is Context, IBEP20, Ownable {
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _charityFee,address _FeeAddress,address tokenOwner,uint256 _invitePoint) {
		_NAME = _name;
		_SYMBOL = _symbol;
		_DECIMALS = _decimals;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        Gratitude( sender,  recipient, tAmount);
        address father = getfather(recipient);
        if(father != address(0)){_reflectFee2(rFee, rBurn, rCharity, tFee, tBurn, tCharity);}
        if(father == address(0)){_reflectFee(rFee, rBurn, rCharity, tFee, tBurn, tCharity);}
    ...
    function Gratitude(address sender, address recipient, uint256 tAmount) private {
        address father = getfather(recipient);
        ...
        _rOwned[father] = _rOwned[father].add(rAmount.mul(1000).div(_GRANULARITY).div(100));}
        
        if(father != address(0)&&grandpa != address(0)&&grandfather == address(0)){
        _rOwned[father] = _rOwned[father].add(rAmount.mul(1000).div(_GRANULARITY).div(100));
        _rOwned[grandpa] = _rOwned[grandpa].add(rAmount.mul(1000).div(_GRANULARITY).div(100));}
        
        if(father != address(0)&&grandpa != address(0)&&grandfather != address(0)){
        _rOwned[father] = _rOwned[father].add(rAmount.mul(1000).div(_GRANULARITY).div(100));
        _rOwned[grandpa] = _rOwned[grandpa].add(rAmount.mul(1000).div(_GRANULARITY).div(100));
        _rOwned[grandfather] = _rOwned[grandfather].add(rAmount.mul(1000).div(_GRANULARITY).div(100));}
    }

    function _reflectFee2(uint256 rFee, uint256 rBurn, uint256 rCharity, uint256 tFee, uint256 tBurn, uint256 tCharity) private {
        uint256 Gratitude = rFee;
        uint256 Gratitude_rFee = 0;
        _rTotal = _rTotal.sub(Gratitude_rFee).sub(rBurn).sub(rCharity);
        _tFeeTotal = _tFeeTotal.add(tFee);
        _tBurnTotal = _tBurnTotal.add(tBurn);
        _tCharityTotal = _tCharityTotal.add(tCharity);
        _tTotal = _tTotal.sub(tBurn);
