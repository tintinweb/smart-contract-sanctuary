...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Update Deposit fee
    function updateDepositFee(uint16 _newDepositFee) public onlyOwner {
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            if(poolInfo[pid].depositFeeBP > 0){
				poolInfo[pid].depositFeeBP = _newDepositFee;
			}
        }
    }
	
    // updateEmissionRate
    function updateEmissionRate(uint256 _plumPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, plumPerBlock, _plumPerBlock);
        plumPerBlock = _plumPerBlock;
    }
