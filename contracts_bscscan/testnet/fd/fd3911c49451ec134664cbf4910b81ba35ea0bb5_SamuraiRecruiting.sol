...
contract SamuraiRecruiting is RSunAuth, IERC721Receiver {
    ...
    uint public maxUsages = 5;
    uint public maxUsagesGeneral = 20;
    uint public rsunCost = 15000 gwei; // 15k RSUN ~ 15-20$

    uint public lockDurationBase = 40;
    uint public lockDurationPerUsage = 40;

    uint public lockDurationBaseGeneral = 40;
    uint public lockDurationPerUsageGeneral = 20;

    uint public infCostBase = 1000 gwei;
    uint public infCostPerUsage = 1000 gwei;

    uint public infCostBaseGeneral = 1000 gwei;
    uint public infCostPerUsageGeneral = 500 gwei;
    ...
    function getLockDuration(uint[] memory ids) public view returns (uint32 dur) {
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            if (id < 5000) {
                dur += uint32(lockDurationBaseGeneral + usages[ids[i]] * lockDurationPerUsageGeneral);
            } else {
                dur += uint32(lockDurationBase + usages[ids[i]] * lockDurationPerUsage);
            }
        }
    }
