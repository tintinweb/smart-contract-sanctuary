...
contract MoonBear is IERC20, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    uint256 public amountToBuyback;
    ...
    function initialize(IUniswapV2Router02 _router, address _busd) external initializer {
        ...
        numTokensSellToAddToLiquidity = 10000 * 10**9; // 10,000 MBF min token before liquidity (0.001% of supply)
        amountToBuyback = 1 * 10**17; // 0.1 BNB min before buyback
    ...
    function setAmountToBuyback(uint256 _amountToBuyback) external onlyOwner {
        amountToBuyback = _amountToBuyback;
    }
    ...
    function _buyback() internal {
        if (!buybackEnabled || address(this).balance < amountToBuyback) {
            return;
        }

        _swapETHForTokens(amountToBuyback);
    }
