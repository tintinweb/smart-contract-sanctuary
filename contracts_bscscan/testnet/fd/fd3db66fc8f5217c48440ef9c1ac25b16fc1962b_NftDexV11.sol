...
contract ERC721MetadataMintable is ERC721, ERC721Metadata, MinterRole {
  ...
  // }
   ...
   function updateTokenURI(uint256 tokenIdT, string memory uriT) public{
        require(_exists(tokenIdT), "ERC721Metadata: URI query for nonexistent token");
...
contract NftDexV11 is ERC721Full, ERC721MetadataMintable, Ownable {
  IERC20 ALIA;
 ...
 // events
  ...
  function buyNFT(address nft_a,uint256 tokenId, address buyer) public returns (bool) {
        ...
        if(msg.sender == nonCryptoNFTVault){
          ...
          if(_tokenAuthors[tokenId] == 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859 && _saleTokens[tokenId].seller == 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859 && adminDiscount > 0){
            _saleTokens[tokenId].price = (_saleTokens[tokenId].price * adminDiscount) / 1000;
            _dollarPrice[tokenId] = (_dollarPrice[tokenId]  * adminDiscount) / 1000;
          }
          if(_saleTokens[tokenId].price > 0){
          price = _saleTokens[tokenId].price;
        } else {
  ...
  function blindBox(address to, string calldata tokenURI, uint256 quantity, bool flag, string calldata ownerId) external returns(bool){
    require(msg.sender==0x56C0E2Abf25546e09F91fc3461Ce6Ab9f7041360,"Not Authorized");
  uint256 tokenId;
  tokenId = mintWithTokenURI(to, tokenURI, 1, flag);
