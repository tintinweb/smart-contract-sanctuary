...
contract ElonGODL is IERC20, Ownable {
    ...
    uint256 private _liquiditySwapAmount = 1000000 * 10**_decimals;
    
    uint256 private _burnTaxRate = 1;
    address private _burnAddress = 0x000000000000000000000000000000000000dEaD;
    
    uint256 private _lotteryTaxRate = 2;
    address private _lotteryAddress = 0xA160DC6EEEd18B4Fd1bC034dA64cBDb530DABfBf; // for event transparency, not token balance tracking
    uint256 private _lotteryBalance = 0;
    uint256 _lotteryContestantMinAmount = 1000000 * 10**_decimals;
    ...
    uint256 private _marketingSwapAmount = 1000000 * 10**_decimals;
    ...
    function _doFirstEligibleTask(bool isSell) private lockTasks {
        // only do eth swaps during sells
        if (isSell) {
        ...
        if (_lotteryCurrentRound >= _lotteryDrawRate) {
            _drawLottery();
            return;
        }
    }
    ...
    function _validateLotteryContestant(address account) private {
        if (account == _uniswapV2Pair || _isExcluded[account] || balanceOf(account) < _lotteryContestantMinAmount) {
            _lotteryContestants.remove(account);
        } else {
            _lotteryContestants.set(account, balanceOf(account));
        }
    }
    ...
    function _drawLottery() private {
        ...
        emit LotteryDraw(winnerAddress, amount);
    }
