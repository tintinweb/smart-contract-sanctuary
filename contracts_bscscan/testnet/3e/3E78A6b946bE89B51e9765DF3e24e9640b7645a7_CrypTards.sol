 ...
 */
  ...
  function setTeamMintCount(uint256 _number) public onlyOwner {
    teamMintCount = _number;
  }
  ...
  function paymentSplitter(uint256 _amount) internal {
    ...
    uint256 GandMMints = _amount / 5; // 20%
    uint256 GandMMintsSplits = GandMMints / 4; // 25% each
    payable(gandmPaymentOne).transfer(GandMMintsSplits);
    payable(gandmPaymentTwo).transfer(GandMMintsSplits);
    payable(gandmPaymentThree).transfer(GandMMintsSplits);
    payable(gandmPaymentFour).transfer(GandMMintsSplits);
