...
contract UBTEST is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    COVENANTDividendTracker public dividendTracker;
    ...
    constructor() BEP20("UB TEST", "UBTEST") {
        dividendTracker = new COVENANTDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "COVENANT: The dividend tracker already has that address"
        );

        COVENANTDividendTracker newDividendTracker = COVENANTDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "COVENANT: The new dividend tracker must be owned by the COVENANT token contract"
        );
    ...
    function updatePancakeV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "COVENANT: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "COVENANT: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != pancakeV2Pair,
            "COVENANT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "COVENANT: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "COVENANT: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "COVENANT: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (canSwap && swapEnabled && !swapping && from != pancakeV2Pair) {
            swapping = true;
...
contract COVENANTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken(
            "COVENANT_Dividend_Tracker",
            "COVENANT_Dividend_Tracker"
        )
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100000 * (10**9); //must hold 100k tokens
    }
    ...
    function _transfer(
        ...
        require(false, "COVENANT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "COVENANT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main COVENANT contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            newClaimWait >= 3600 && newClaimWait <= 1296000,
            "COVENANT_Dividend_Tracker: claimWait must be updated to between 1 hr and 14 days"
        );
        require(
            newClaimWait != claimWait,
            "COVENANT_Dividend_Tracker: Cannot update claimWait to same value"
        );
