...
contract GyroBond is Ownable {
    ...
    function deposit(
        ...
        uint256 value = IReservoir(reservoir).gyroValueOf(principle, amount_);
        uint256 payout = payoutFor(value); // payout to bonder is computed

        require(payout >= 10000000, "Bond too small"); // must be > 0.01 gyro ( underflow protection )
        ...
        emit BondCreated(amount_, payout, block.number.add(terms.vestingTerm), priceInUSD);
        ...
        /**
            ...
            approved and
            deposited into the reservoir, returning (amount_ - profit) gyro
         */
        IERC20(principle).safeTransferFrom(msg.sender, address(this), amount_);
        IERC20(principle).approve(address(reservoir), amount_);
        IReservoir(reservoir).deposit(amount_, principle, profit);

        if (fee != 0) {
            // fee is transferred to treasury
            IERC20(gyro).safeTransfer(treasury, fee);
        }
    ...
    function redeem(address recipient_, bool stake_) external returns (uint256) {
        ...
        uint256 percentVested = percentVestedFor(recipient_); // (blocks since last interaction / vesting term remaining)

        if (percentVested >= 10000) {
            // if fully vested
            delete bondInfo[recipient_]; // delete user info
            emit BondRedeemed(recipient_, info.payout, 0); // emit bond data
            ...
            emit BondRedeemed(recipient_, payout, bondInfo[recipient_].payout);
            return stakeOrSend(recipient_, stake_, payout);
        }
    }
    ...
    function stakeOrSend(
        ...
        if (!stake_) {
            ...
            IERC20(gyro).approve(vault, amount_);
            IGyroVault(vault).stake(amount_, recipient_);
        }
        return amount_;
    }
     ...
     *  @return uint
     */
    function payoutFor(uint256 value_) public view returns (uint256) {
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or gyro) to the treasury
     *  @return bool
     */
