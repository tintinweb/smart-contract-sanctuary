...
contract InfomatixTimeLockVesting {
    ...
    //inherit the function from BokkyPooBahsDateTimeLibrary for addMonths 
    function addMonths(uint timestamp, uint _months) internal pure returns (uint newTimestamp) {
        newTimestamp = BokkyPooBahsDateTimeLibrary.addMonths(timestamp, _months);
    }

    //inherit the function from BokkyPooBahsDateTimeLibrary for addYears
    function addYears(uint timestamp, uint _years) internal pure returns (uint newTimestamp) {
        newTimestamp = BokkyPooBahsDateTimeLibrary.addYears(timestamp, _years);
    }

    //inherit the function from BokkyPooBahsDateTimeLibrary for addDays
    function addDays(uint timestamp, uint _days) internal pure returns (uint newTimestamp) {
    ...
    function release() public virtual {
        ...
        if(_currentRelease == 0) {
            ...
            _nextReleaseTime = addDays(_nextReleaseTime, 1);
        } else {
