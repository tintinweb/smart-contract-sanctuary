...
contract Transitory is Context, IERC20, Ownable 
{
    ...
    uint256 private _tTotal = 29_000_000_000_000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Transitory";
    string private _symbol = "TRAN";
    ...
    uint256 public _founderFee = 1;
    uint256 private _previousFounderFee = _founderFee;
    
    address public founderWallet =  0xd389835E5391640ED3AD1B6bf02c3658815ec449;
    ...
    uint256 public _maxTxAmount = 58_000_000_000_0 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 29_000_000_0 * 10**9;
    uint256 public  maxLimit = 58_000_000_000_0 * 10**9;
    ...
    constructor () 
    {
        ...
        _isExcludedFromWhale[founderWallet]=true;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private 
    {
       ...
       {
            uint256 allFees = _taxFee.add(_liquidityFee).add(_burnFee).add(_founderFee);
            uint256 fee = amount.mul(allFees).div(100);
            uint256 newBalance = balanceOf(recipient).add(amount).sub(fee);
            require(newBalance <= maxLimit, "Exceeding max tokens limit in the wallet");            
        }
        
        uint256 burnAmt = amount.mul(_burnFee).div(100);
        uint256 amtWalletS = amount.mul(_founderFee).div(100);
        ...
        if(amtWalletS>0)
        {
           _transferStandard(sender, founderWallet, amtWalletS);    
        }  
        restoreAllFee();
    }
    ...
    {
            maxLimit = amount;
            emit Log("maxLimit changed", amount);
    }
