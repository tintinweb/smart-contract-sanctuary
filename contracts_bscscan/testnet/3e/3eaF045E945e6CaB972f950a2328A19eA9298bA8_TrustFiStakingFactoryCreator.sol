...
contract TrustFiStakingFactoryCreator is BaseStruct{
    ...
    constructor(address _defaultPayToken,uint256 _defaultPoolCommAmount,uint256 _editFeeValue,uint256 _closeFeeValue) {
        admin = msg.sender;
    ...
    */
    function createPool(
        ...
        require(poolParams[3] > 0, "TrustFiStaking:MAX_STAKE_AMOUNT_MUST_GREATER_THAN_ZERO"); //maxStakeAmount
        ...
        require(poolParams[3] <= rewardTotals, "TrustFiStaking:MAX_STAKE_AMOUNT_MUST_LESS_THAN_REWARD_TOTALS");
        ...
        TrustFiStakingFactory(vars.pFactory).addPool(range, vars.token,poolParams, rewardToken, newTotals, rewardPerBlocks,poolViewParams,feeAddress,vars.commissionToken);
    }
    ...
    */
    function editPool(
        ...
        TrustFiStakingFactory(vars.pFactory).editPool(poolId, vars.token,poolParams, rewardToken, vars.newTotal, rewardPerBlocks,poolViewParams,vars.feeAddress);
    }
    ...
    function closePool(uint256 poolId) public {
        ...
        if (!whiteList[msg.sender]) {
            if (vars.poolStakeInfo.closeFeeValue > 0) {
                require(IERC20(address(vars.commissionToken)).balanceOf(address(this)) >= vars.poolStakeInfo.closeFeeValue,"TrustFiStaking:COMMISSIONTOKEN NOT ENOUGH");
                IERC20(address(vars.commissionToken)).safeTransferFrom(address(this),msg.sender, vars.poolStakeInfo.closeFeeValue);
            }
        }
    ...
    */
     ...
     */
    function getSupportCommTokens() external view returns (address[] memory supportCommTokenArrRet, uint256[] memory poolAmounts, bool[] memory states) {
