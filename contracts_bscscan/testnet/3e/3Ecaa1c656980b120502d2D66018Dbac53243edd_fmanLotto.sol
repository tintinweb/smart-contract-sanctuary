...
//        uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function swapFMANForBeans(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = fmanAddy;
        path[1] = uniswapV2Router.WETH();

        IERC20(fmanAddy).approve(address(uniswapV2Router), tokenAmount);
    ...
    function mintLotto(uint256 numberOfTokens) public {
        ...
        uint256 curr_balance = IERC20(fmanAddy).balanceOf(address(this));
        IERC20(fmanAddy).transferFrom(msg.sender, address(this), payableTokens);
        uint256 delta_balance = IERC20(fmanAddy).balanceOf(address(this)).sub(curr_balance);

        // 25% auto sold o bnb automatically
        uint256 balanceToSell = delta_balance.div(4);
        swapFMANForBeans(balanceToSell);
