...
contract CartographerElevation is Ownable, Initializable, ReentrancyGuard, ICartographerElevation {
    ...
    function getCurrentAccSummitPerShare(ElevationPoolInfo memory pool) internal view returns (uint256) {
        if (block.number > pool.lastRewardBlock && pool.lpSupply > 0) {
            ...
            return pool.accSummitPerShare.add(summitReward.mul(1e12).div(pool.lpSupply));
        }
        return pool.accSummitPerShare;
    }
    ...
    function getHypotheticalSummit(uint8 _pid, address _userAdd) external view poolExists(_pid) validUserAdd(_userAdd) override returns (uint256, uint256) {
        ...
        uint256 accSummitPerShare = getCurrentAccSummitPerShare(pool);
        uint256 rew = getUserHypotheticalReward(pool, user, accSummitPerShare);    
        if (rew == 0 || pool.totemRoundRewards[userTotem[_userAdd][pool.elevation]] == 0) { return (0, 0); }
        return (
            // CURRENT: How much the user has farmed currently
            rew,
            // IF WIN: User rewards * (total round rewards / totem rewards)
            rew.mul(pool.roundRewards).div(pool.totemRoundRewards[userTotem[_userAdd][pool.elevation]])
        );
    }
    ...
    function updatePool(uint8 _pid) public {
        ...
        for (uint8 i = 0; i < pool.totemRoundRewards.length; i++) {
            pool.totemRoundRewards[i] = pool.totemRoundRewards[i].add(summitReward.mul(pool.totemLpSupplies[i]).div(pool.lpSupply));
        }        
        pool.lastRewardBlock = block.number;
    }
    ...
    function totemRoundRewards(uint8 _pid) public view poolExists(_pid) returns (uint256[10] memory) {
        ElevationPoolInfo storage pool = elevationPoolInfo[_pid];
        if (pool.elevation == 1) {
            return [pool.totemRoundRewards[0], pool.totemRoundRewards[1], 0, 0, 0, 0, 0, 0, 0, 0];
        } else if (pool.elevation == 2) {
            return [pool.totemRoundRewards[0], pool.totemRoundRewards[1], pool.totemRoundRewards[2], pool.totemRoundRewards[3], pool.totemRoundRewards[4], 0, 0, 0, 0, 0];
        } else {
            return [pool.totemRoundRewards[0], pool.totemRoundRewards[1], pool.totemRoundRewards[2], pool.totemRoundRewards[3], pool.totemRoundRewards[4], pool.totemRoundRewards[5], pool.totemRoundRewards[6], pool.totemRoundRewards[7], pool.totemRoundRewards[8], pool.totemRoundRewards[9]];
        }
    }
}
