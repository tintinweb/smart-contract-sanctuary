...
contract Exchange is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function calculateLoanAmountAndExchangeRate(
        ...
        if (_col.collateralAddress == address(0)) {
            collateralToUSD =
                (uint256(RateBNBwithUSD()) * 10**10 * _pkg.loanToValue * _col.amount) / 100;
        } else {
            collateralToUSD =
                (uint256(getLatesPriceToUSD(_col.collateralAddress)) * 10**10 * _pkg.loanToValue * _col.amount) / 100;
        }
        ...
        if (_col.loanAsset == address(0)) {
            ...
            RateLoanAsset =
                uint256(getLatesPriceToUSD(_col.loanAsset)) * 10**10;
        }
        ...
        if (_pkg.repaymentAsset == address(0)) {
            ...
            RateRepaymentAsset =
                uint256(getLatesPriceToUSD(_pkg.repaymentAsset)) * 10**10;
        }
