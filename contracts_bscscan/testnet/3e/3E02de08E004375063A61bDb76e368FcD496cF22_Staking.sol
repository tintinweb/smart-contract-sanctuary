 ...
 *
 *  NOTES:
 *  Migration dans les toutes premieres transactions a analyser pour voir comment cree les contracts sans besoins de migration
 */
...
interface ISTOKEN {
  function rebase(uint256 hecProfit_, uint256 epoch_)
    external
    returns (uint256);
...
contract Staking is Ownable {
  ...
  /**
        @notice stake HEC to enter warmup
        @param _amount uint
        @return bool
     */
  function stake(uint256 _amount, address _recipient) external returns (bool) {
    ...
    warmupInfo[_recipient] = Claim({
      ...
      gons: info.gons.add(ISTOKEN(sHEC).gonsForBalance(_amount)),
  ...
  /**
        @notice retrieve sHEC from warmup
  ...
  function claim(address _recipient) public {
    ...
    if (epoch.number >= info.expiry && info.expiry != 0) {
      ...
      IWarmup(warmupContract).retrieve(
        _recipient,
        ISTOKEN(sHEC).balanceForGons(info.gons)
      );
    }
  }

  /**
        @notice forfeit sHEC in warmup and retrieve HEC
     */
  ...
  /**
        @notice returns the sHEC index, which tracks rebase growth
        @return uint
     */
  function index() public view returns (uint256) {
    return ISTOKEN(sHEC).index();
  }
  ...
  function rebase() public {
    if (epoch.endBlock <= block.number) {
      ISTOKEN(sHEC).rebase(epoch.distribute, epoch.number);
      ...
      uint256 staked = ISTOKEN(sHEC).circulatingSupply();
  ...
  /**
        @notice returns contract HEC holdings, including bonuses provided
        @return uint
     */
