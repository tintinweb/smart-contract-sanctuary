...
contract Issue is AccessControl, ReentrancyGuard {
    ...
    constructor( address minter, address _heroesNft, address _heroesToken ) {
        ...
        duckChance.push(ChanceTableDuck({level: 1, maxValue: 10, minValue: 5, chance: 350}));
...
//DUCKPART
    ...
    function _mintLevelUp(uint level, uint[] memory tokenId) private {
        ...
        heroesNFT.addDuckNumber(newTokenId, level + 1, heroes[level + 1].name, heroes[level + 1].category, newEXP * 1e18, level, block.timestamp, false );
