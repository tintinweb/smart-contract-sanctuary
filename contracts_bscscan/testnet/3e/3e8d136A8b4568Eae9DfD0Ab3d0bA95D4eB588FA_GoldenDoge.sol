...
contract GoldenDoge is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 private _liqudityAmount;
    address payable private _liqudityAddress;
    ...
    event ContractFee(bool ContractFee);

    event ContractLiqudityAmount(uint256 tLiqudity, uint256 rLiqudity);
    ...
    constructor(
        ...
        address payable _chrtAddress
    ) public {
        _rOwned[_msgSender()] = _rTotal;
        _liqudityAddress = _lpAddress;
        _charityAddress = _chrtAddress;
    ...
    function _takeLiquidity(uint256 tLiquidity, bool isSell) private {
        if (isSell) {
            _liqudityAmount = tLiquidity;
        }
        uint256 currentRate = _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        emit ContractLiqudityAmount(tLiquidity, rLiquidity);
    ...
    function _transfer(
        ...
        if (
            ...
            emit ContractBalance(address(this).balance, _liqudityAmount);
            ...
            if (
                ...
                //     _liqudityAmount
                // );
                swapTokensForEth(_liqudityAmount);
    ...
    function sendETHToFee(uint256 amount) private {
        _liqudityAddress.transfer(amount);
    }
    ...
    function setLiqudityAddress(address payable _address) public onlyOwner {
        _liqudityAddress = _address;
    }
