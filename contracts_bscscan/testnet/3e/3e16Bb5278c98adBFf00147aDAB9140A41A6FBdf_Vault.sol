...
contract Vault is ReentrancyGuard{
    ...
    function addTokenFactory(address factory) public nonReentrant {
    ...
    function createStandardToken(
        ...
        assignTokensToOwner(msg.sender, address(newToken), 0);
        emit TokenCreated(msg.sender, address(newToken), 0);
    }

    function addTokenFactories(address[] memory factories) external nonReentrant {
        for (uint256 i = 0; i < factories.length; i++) {
            addTokenFactory(factories[i]);
        }
    }

    function removeTokenFactory(address factory) external nonReentrant {
        tokenFactories.remove(factory);
    }

    function assignTokensToOwner(address owner, address token, uint8 tokenType) private nonReentrant {
