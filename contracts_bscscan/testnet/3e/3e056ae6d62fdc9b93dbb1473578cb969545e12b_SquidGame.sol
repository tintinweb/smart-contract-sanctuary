...
contract SquidGame is Ownable {
    ...
    uint public roundNumber = 0; // a.k.a round id
    ...
    function playTheRound(uint _roundID, uint nftIndex) public returns (bool)
    {
        require(gameIsActive && !gameEND, "The Squid Game is stopped");

        Round memory r = allRounds[roundNumber];
        bool validPlayTime = (block.timestamp > r.startTime) && (block.timestamp < r.endTime);
        ...
        bool alreadyPlayed = playedRoundNum == _roundID;
        require(alreadyPlayed, "You have already played this round.");

        bool alive = (_roundID == 1) || (sgSurvivor[nftID]);
        require(alive, "Sorry, you are dead.");

        
        sgPlayerRound[nftID] = _roundID; // to record the player who has played this round
        sgSurvivorCount++;
    ...
    function resetGame() public onlyOwner {
        ...
        gameEND = false;
        roundNumber = 0;
        delete allRounds; // TODO: test this
        sgSurvivorCount = 0;
    }
