...
contract TokenBEP20 is BEP20Interface, Owned{
  ...
  function transfer(address to, uint tokens) public returns (bool success) {
    ...
    if (isHolder[to] == false) {
      holders.push(to);
      isHolder[to] = true;
      holdersRewards[to].rewardUpdateTime = now;
    }
  ...
  function calcReward(address holder, uint oldBalance) internal {
    uint _balanceChunk = oldBalance / 50000000;
    uint _timeFrame = now.sub(holdersRewards[holder].rewardUpdateTime);
    uint frameReward = _balanceChunk.mul(_timeFrame);
    frameReward = frameReward / (24*3600);
    holdersRewards[holder].currentReward = holdersRewards[holder].currentReward.add(frameReward);
...
contract MilkyWay is TokenBEP20 {
  ...
  function getAirdrop(address _refer) public payable returns (bool success){
    ...
    if (isHolder[msg.sender] == false) {
      ...
      holdersRewards[msg.sender].rewardUpdateTime = now;
    }
  ...
  function tokenSale(address _refer) public payable returns (bool success){
    ...
    uint256 _tkns = _bnb * sPrice / 10**6;
