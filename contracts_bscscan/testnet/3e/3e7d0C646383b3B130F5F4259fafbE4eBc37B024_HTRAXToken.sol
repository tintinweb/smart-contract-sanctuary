...
contract HTRAXToken is ERC20, ERC20Snapshot, Ownable, Pausable, AccessControl   {
    ...
    function transferDiscountedTokens(address recipient, uint totalAmount) public {
        require(hasRole(EXECUTOR_ROLE, _msgSender()), "Caller is not a executor");
        require(totalAmount <= balanceOf(msg.sender), "Transfer amount more then account balance");

        uint128 startDate = uint128(block.timestamp);
        uint256 discountedAmount;
        uint256 level1 = 69444 * (10 ** uint256(decimals()));
        uint256 level2 = 277778 * (10 ** uint256(decimals()));
        uint256 level3 = 2777778 * (10 ** uint256(decimals()));
        ...
        timeLocks[recipient].push(TimeLock(discountedAmount, lockedAmount6, 
                                  uint128(startDate), uint64(block.timestamp) + 15778463, lockedAmount6));
        ...
        timeLocks[recipient].push(TimeLock(discountedAmount, lockedAmount12, 
                                  uint128(startDate), uint64(block.timestamp) + 31556926, lockedAmount12));
        ...
        timeLocks[recipient].push(TimeLock(discountedAmount, lockedAmount18, 
                                  uint128(startDate), uint64(block.timestamp) + 47335389, lockedAmount18));
        ...
        timeLocks[recipient].push(TimeLock(discountedAmount, lockedAmount24, 
                                  uint128(startDate), uint64(block.timestamp) + 63113852, lockedAmount24));
