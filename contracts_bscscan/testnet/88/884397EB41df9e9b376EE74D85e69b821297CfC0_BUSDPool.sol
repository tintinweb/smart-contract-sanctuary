...
contract BUSDPool {
    ...
    uint16 constant percent_divide_by = 1000; // 0.0%, one decimals

    uint24 constant day_secs = 86400;
    uint8 constant commission = 100; // 10%
    uint8 constant ref_lines = 5;
    uint8[ref_lines] public ref_bonuses = [50, 30, 20, 10, 5];

    uint8 constant min_investment_days = 7;
    uint8 constant max_investment_days = 120;
    uint16 constant initial_profit_percent = 1200; // 120%
    uint8 constant profit_percent_step = 50; // plus 5% for each more day
    ...
    function withdraw() external {
        ...
        uint256 revenue = this.revenueOf(msg.sender);
    ...
    function revenueOf(address _addr) external view returns (uint256 value) {
        ...
        )
    {
        ...
        uint256 revenue = this.revenueOf(_addr);
    ...
    function getUserHistory(address _addr)
        external
        view
