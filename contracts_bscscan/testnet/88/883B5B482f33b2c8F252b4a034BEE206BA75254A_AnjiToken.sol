    ...
    function swapBack() internal swapping {
        ...
        if (swapedBNBAmount > 0) {
            if (bnbSwapMarketingAmount > 0) {
                ...
                address(marketingFeeReceiver).call{value: bnbSwapMarketingAmount}("");
            }

            if (bnbSwapCharityAmount > 0) {
                // Send BNB for Charity
                address(charityFeeReceiver).call{value: bnbSwapCharityAmount}("");
            }

            if (bnbSwapBnbAmount > 0) {
                // Send BNB for Rewards
                address(bnbDistributor).call{value: bnbSwapBnbAmount}("");
