...
contract WithdrawRouterV2 is Ownable {
    ...
    function calcMinToken(uint reserveWeth, uint reserveToken, uint lpTotal) private view returns (uint wethAmount, uint tokenAmount, uint lpAmount) {
        ...
        // 上面两行结论是错的，因为_addLiquidity时不管前后顺序是什么，得到的结果都是固定的，假设eth在前，就是遇到的第一种情况，2 eth不够349 token，会触发用349 token计算eth，eth为1
        // 反过来之后相当于直接用349 token计算eth数量，得到的结果还是1，所以直接乘以3倍是靠谱的。考虑到小的可能是token，同时token转移时会舍入，会再次出现舍入
        // 详细舍入情况为：第一次transfer，减一个比例k；第二次addLiquidity，减比例k；第三次removeLiquidity，保证之前add时至少为2即可；即 amount * (1 - k) * (1 - k) = 2（更严谨一点其实应该是amount * (1 - k) * (1 - k) * (1 - k) = 1），考虑舍入情况amount，可得到amount与k的函数关系图像
        ...
        // 分别对应手续费99%到手续费1%，在手续费为25%前都是4，我们按4倍计算可以保证手续费在25%以内的完成交易
    }
