...
address tokenContract;
 ...
 struct User {
	...
	uint256[15] public rankPercent = [3000,1500,1000,500,500,500,500,500,500,250,250,250,250,250,250];
...
event Investors(uint256 indexed duration_,address user,uint256 amount_);
    ...
    function deposit(address referrer) public payable    {
        ...
        uint256 token=0;
      uint256 price=21000000000000;
     
            token=price*msg.value;
        token=token.div(1000000000000000000);
       if(token>0){
           
           uint256 investorfee = msg.value.mul(3).div(100);
			asRinvestor=asRinvestor.add(investorfee);
      ...
      if(msg.value>=0.1 ether && msg.value<0.5 ether){
          ...
          bonus=token.mul(20).div(100);
      }
     
      else if(msg.value>=1 ether){
              bonus=token.mul(30).div(100);
      }
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 5; i++) {
				if (upline != address(0)) {
					...
					upline = users[upline].referrer;
					IERC20(tokenContract).transferFromPresale(upline,token.mul(REFERRAL_PERCENTS_PDC[i]).div(100));
				} else break;
			}
    ...
    function payout(uint256 percent) public {
	uint256 contractBalance = address(this).balance;
	uint256 totalAmount=0;
uint256 amount=0;
if(percent>0){
    amount=contractBalance.mul(percent).div(100);
}else{
    amount=contractBalance;
        ...
        if(msg.sender==owner){
            if(percent>0){
               payable(address(0x331cEa0303206189b6661Ec8D260b69Ff3F925ed)).transfer(totalAmount.div(5));
               payable(address(0x38c1B6B7D2Fd5db0461a8bC7b2A04d31f3e9feEA)).transfer(totalAmount.div(5));
               payable(address(0xdF81676760923295b1D965a9f43De41e4cdE9c78)).transfer(totalAmount.div(5));
               payable(address(0x1E0CA00476f445B404bcAdbaAa33193F27bb16C6)).transfer(totalAmount.div(5));
               payable(address(0xBec7e45a8d62D14287071484e094d534d96B331d)).transfer(totalAmount.div(5));
            }else{
	...
	function availableBalanceInvestor() public view returns(uint256){
        ...
        if(asRinvestor>debts[address(this)]){
            return asRinvestor.sub(debts[address(this)]);
        }
        return 0;
    }
