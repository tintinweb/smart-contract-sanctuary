...
contract EncodeSide {
	...
	uint256 constant private CHAIN_ID = 97;
	...
	//signature
	function transferTo(uint256 _toChainId, IERC20 _token, uint256 _amount) external {
		//approve-->
		_token.safeTransferFrom(msg.sender, address(this), _amount);
		Transaction[] storage transactions = userTransactions[msg.sender][CHAIN_ID][_toChainId];
		Transaction memory transaction = Transaction({
			executeFunctionId: this.transfer.selector,
			sendToken: _token,
			receiveToken: toChainToken[_token][_toChainId],
			amount: _amount
		});
		...
		emit Transfer(
			...
			_toChainId, 
			_token, 
			toChainToken[_token][_toChainId], 
			_amount);
	}    
	...
	{
		Transaction memory transaction = userTransactions[_user][_fromChainId][_toChainId][_txIndex];
