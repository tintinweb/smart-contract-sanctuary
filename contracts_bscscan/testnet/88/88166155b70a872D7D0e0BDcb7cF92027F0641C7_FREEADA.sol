...
15-18% Buy tax, divided into:
8% ADA rewards
2% liquidity 
5% Marketing

25 - 30% Sell tax, divided into:
16% ADA rewards
4% liquidity 
5% Marketing


TG: https://t.me/freeada

Twitter: https://twitter.com/freeada
...
contract FREEADA is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0xD52389Bc32247eCA808288aDbAC4F2C8b10A46B5); // Marketing Address
    ...
    uint256 private _TFee = 2;
    uint256 private _previousTFee = _TFee;
    
    uint256 private _lp = 10;
    uint256 private _previouslp = _lp;
    
    uint256 private _buyFee = 17;
    uint256 private _buylp = 1;
    
    uint256 private _botNeedsPrevention = 29;
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(to == uniswapV2Pair){
                removeAllFee();
                _TFee = _botNeedsPrevention;
                _lp = _whaleNeedsPrevention;
            }
            
            // If send account has a special fee 
            if(_addressFees[from].enable){
                removeAllFee();
                _TFee = _addressFees[from]._TFee;
                _lp = _addressFees[from]._lp;
                
                // Sell
                if(to == uniswapV2Pair){
                    _TFee = _addressFees[from]._botNeedsPrevention;
                    _lp = _addressFees[from]._whaleNeedsPrevention;
                }
            }
            else{
                ...
                if(_addressFees[to].enable){
                    ...
                    removeAllFee();
                    if(from == uniswapV2Pair){
                        _TFee = _addressFees[to]._buyFee;
                        _lp = _addressFees[to]._buylp;
                    }
                }
            }
        }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 tFee = calculateTFee(tAmount);
        uint256 tLiquidity = calculatelp(tAmount);
    ...
    function calculateTFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_TFee).div(
            10**2
        );
    }
    
    function calculatelp(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_lp).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_TFee == 0 && _lp == 0) return;
