...
interface IJaxAdmin {
  ...
  function jtokens(address jtoken_address) external view returns (JToken memory);
...
contract JaxSwap is IJaxSwap, Initializable, JaxOwnable {
  ...
  function setTokenAddresses(address _wjxn, address _wjax, address _vrp, address _jusd) external {
  ...
  function swap_wjxn_wjax(uint amount) external onlyGovernor {
    ...
    require(wjax_lsc_ratio <= jaxAdmin.wjax_collateralization_ratio() * 110 / 100, "Low Collateral"); //Unable to withdraw as collateral is fine.
  ...
  function swap_wjax_wjxn(uint amount) external onlyGovernor {
    ...
    require(wjax_lsc_ratio >= jaxAdmin.wjax_collateralization_ratio(), "Low Reserves"); //Unable to withdraw as reserves are low.
  ...
  function swap_jusd_jtoken(address jtoken, uint amountIn) external isActive returns (uint) {
    IJaxAdmin.JToken memory token = jaxAdmin.jtokens(jtoken);
    uint ratio = token.jusd_ratio;
    ...
    uint256 jtoken_markup_fee = jtoken_amount * token.markup_fee / 1e8;
    ...
    IERC20(jtoken).mint(token.markup_fee_wallet, jtoken_markup_fee);
  ...
  function swap_jtoken_jusd(address jtoken, uint amountIn) external isActive returns (uint) {
    ...
    uint jusd_markup_fee = jusd_amountOut * token.markup_fee / 1e8;
    ...
    jusd.mint(token.markup_fee_wallet, jusd_markup_fee);
  ...
  function swap_jtoken_busd(address jtoken, uint amountIn) external isActive returns (uint) {
    IJaxAdmin.JToken memory token = jaxAdmin.jtokens(jtoken);
    require(token.jusd_ratio > 0, "Zero Ratio"); //ratio is not set for this token
    uint jusd_amountOut = amountIn * 1e8 / token.jusd_ratio;
    uint jusd_markup_fee = jusd_amountOut * token.markup_fee / 1e8;
    require(IERC20(jtoken).balanceOf(msg.sender) >= amountIn, "Insufficient Jtoken");
    IERC20(jtoken).burnFrom(msg.sender, amountIn);
    // The recipient has to pay fee. 
  ...
  function swap_busd_jtoken(address jtoken, uint amountIn) external isActive returns(uint) {
    IJaxAdmin.JToken memory token = jaxAdmin.jtokens(jtoken);
    require(token.jusd_ratio > 0, "Invalid JUSD-Ratio");
    ...
    uint jtoken_amount = jusd_amount * token.jusd_ratio / 1e8;
    // Calculate Fee on receiver side
    uint jtoken_markup_fee = jtoken_amount * token.markup_fee / 1e8;
    // jusd.burnFrom(msg.sender, jusd_amount);
    // The recipient has to pay fee. 
    uint amountOut = jtoken_amount-jtoken_markup_fee;
    IERC20(jtoken).mint(token.markup_fee_wallet, jtoken_markup_fee);
  ...
  function initialize(address _jaxAdmin, address pancakeRouter, address _busd) external initializer {
