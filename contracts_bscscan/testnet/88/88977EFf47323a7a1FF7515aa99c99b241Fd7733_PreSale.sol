...
contract PreSale is ContextSlave {
    ...
    function CharityBuyForLiquidity() public payable {
        uint256 bnbAmount = uint256(msg.value);
        require(checkAmountValidity(bnbAmount) == true, "Amount is not valide");

        uint amountOfToken = iRouter.getAmountsOut(bnbAmount, getPathForWBNBToToken())[1];
        ...
        iUserManagement.updateUserGiftStats(address(msg.sender), bnbAmount, amountOfToken);
    }
