...
contract UpDown is VRFConsumerBase, OwnableUpgradeable, ReentrancyGuard {
    ...
    function initialize(address _tiny, address _nft, address _stats, address _nftLocker, uint256[4] memory _bonusReward) external initializer {
        tiny = IERC20(_tiny);
        nft = _nft;
        stats = _stats;
    ...
    */
    ...
    Internal
     */
    function _settleBet(bytes32 requestId, uint256 randomNumber) internal {
        uint256 betId = betMap[requestId];
        Bet storage bet = bets[betId];

        // Validation checks.
        require(bet.amount >= MINIMUM_BET_AMOUNT, "Bet does not exist");
        require(!bet.isFinished, "Bet is settled already");

        uint256 luckyNum = randomNumber % 100;
        require(luckyNum > 0, 'invalid luckyNum');
    ...
    function setMinBet(uint256 value) external onlyOwner {
        require(value >= 0.05 ether && value <= 10000 ether);
