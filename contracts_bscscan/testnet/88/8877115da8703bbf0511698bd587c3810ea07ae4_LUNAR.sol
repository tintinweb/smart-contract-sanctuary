...
import "./SafeMath.sol";
import "./LUNARDividendTracker.sol";
...
contract LUNAR is ERC20, Ownable {
    ...
    LUNARDividendTracker public dividendTracker;
    
    address  payable public miningPool;
    ...
    uint256 public  miningPoolFee;
    
    uint256 public  totalFees = BNBRewardsFee.add(liquidityFee).add(buyBackFee).add(miningPoolFee);
    
    uint256 public buyBackUpperLimit = 1 * 10**18;
    bool public buyBackEnabled = true;
    bool public swapEnabled = false;

    uint256 public lastTransferTokenRewardsInterval = 1800; // Checks for BNB to be sent to Mining Pool.
    ...
    constructor() public ERC20("LNUARMINING", "LUNAR") {
        uint256 _BNBRewardsFee = 10;
        uint256 _liquidityFee = 2;
        uint256 _buyBackFee = 4;
        uint256 _miningPoolFee = 0;
        
        BNBRewardsFee = _BNBRewardsFee;
        liquidityFee = _liquidityFee;
        buyBackFee = _buyBackFee;
        miningPoolFee = _miningPoolFee;
        
        totalFees = BNBRewardsFee.add(liquidityFee).add(buyBackFee).add(miningPoolFee);

    	dividendTracker = new LUNARDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "LUNAR: The dividend tracker already has that address");

        LUNARDividendTracker newDividendTracker = LUNARDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "LUNAR: The new dividend tracker must be owned by the LUNAR token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "LUNAR: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "LUNAR: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "LUNAR: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "LUNAR: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "LUNAR: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "LUNAR: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "LUNAR: Cannot update gasForProcessing to same value");
    ...
    function setMiningPool(address payable miningPoolAddress) external onlyOwner{
    ...
    function setMiningPoolFee(uint256 value) external onlyOwner{
        miningPoolFee = value;
        totalFees = BNBRewardsFee.add(liquidityFee).add(buyBackFee).add(miningPoolFee);
    }
    ...
    function _transfer(
        ...
        if(block.timestamp>lastTransferTokenRewardsTimestamp.add(lastTransferTokenRewardsInterval) && address(this).balance> buy2rewthreshold) // Every hour check for funds available and send to Mining Pool.
        {
            lastTransferTokenRewardsTimestamp=block.timestamp;
            miningPool.transfer(address(this).balance.div(2));
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // Send BNB to miningPool
        uint256 rewardBNB = unitBalance.mul(2).mul(miningPoolFee);
        miningPool.transfer(rewardBNB);
    ...
    function manualSendFromBuybackToMiningPool(uint256 weiAmount) external onlyOwner{
        require(address(this).balance>=weiAmount);
        miningPool.transfer(weiAmount);
    }
