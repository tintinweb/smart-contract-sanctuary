...
contract Blacklister is Ownable {
  ...
  mapping(uint256 => bool) public simbaBlackList;

  event AddSimbaBlacklist(uint256 id);

  function addSimbaBackList(uint256[] memory ids) public onlyOwner {
    uint256 length = ids.length;
    for (uint256 index = 0; index < length; index++) {
      simbaBlackList[ids[index]] = true;
      emit AddSimbaBlacklist(ids[index]);
    }
  }

  event RemoveSimbaBlacklist(uint256 id);

  function removeSimbaBlackList(uint256[] memory ids) public onlyOwner {
    uint256 length = ids.length;
    for (uint256 index = 0; index < length; index++) {
      simbaBlackList[ids[index]] = false;
      emit RemoveSimbaBlacklist(ids[index]);
    }
  }

  function ensureTokenNotInBlacklist(uint256 id) public view {
    require(!simbaBlackList[id], "BLACKLISTED");
  }
