...
contract ASK_v04a is Context, IBEP20, Ownable {
   ...
   uint256 private MinBalanceATH ;
   uint256 public MaxBBackAmount = 1 * 10**15;
   uint256 public BBackThreshold = 1;
   //end skizzo
    ...
    string private _name = "AleSkizz_v04a";
    string private _symbol = "ASK4";
    uint8 private _decimals = 18;
    
	//normal sell
    uint256 public _taxFeeL0 = 2;
	uint256 private _taxFee = _taxFeeL0;
   	uint256 public _BNBFeeL0 = 2;
	uint256 private _BNBFee = _BNBFeeL0;
	...
	address payable public fundraisingAddress = payable(0x02b2bEe3AAA726e1ECBC2E76B3afeefb4782e103);
    
    uint256 public fundDivisor = 2;
    ...
    function viewMaxBalance() view external returns (string memory) {
        ...
        result = "MinBalanceATH: ";
        result = concatenate(result, uint2str(MinBalanceATH));
        result = concatenate(result, ", UniswapV2PairBalance: ");
        result = concatenate(result,uint2str(balanceOf(uniswapV2Pair)));
        result = concatenate(result, ", MaxBalance.Mul1.Div100: ");
        result = concatenate(result,uint2str( MinBalanceATH+MinBalanceATH.mul(1).div(100)) );
       return result;
	...
	//per un frontend MODDED
  	function _feeLevel(address account) public view returns(string memory feeLevel) {
        ...
        if ( _firstDeposit[account] == 0 || _firstDeposit[account].add(_feeDurationMax) <= block.timestamp ) {
			...
			result = concatenate(result, uint2str(_taxFeeL0 + _BNBFeeL0)); //slippage
...
//end modded
    ...
    function timedFees (address from) private {
		...
		function ApplyBBack() private {
			if (MinBalanceATH == 0) {
				MinBalanceATH = balanceOf(uniswapV2Pair);	
			   }
			if ( balanceOf(uniswapV2Pair) < MinBalanceATH) {
			MinBalanceATH = balanceOf(uniswapV2Pair);
			} else if ( balanceOf(uniswapV2Pair) > (MinBalanceATH + (MinBalanceATH.mul(1)).div(100)) ) {
					    uint BNBamount =  address(this).balance;
					    if (BNBamount >= MaxBBackAmount){
					        buyBackTokens(MaxBBackAmount);
					    }
					}
			}
    ...
    function setfundraisingAddress(address payable _fundraisingAddress) public onlyOwner {
        fundraisingAddress = _fundraisingAddress;
            ...
            function setBBackThreshold(uint256 BBthresh) external onlyOwner() {
    BBackThreshold = BBthresh;
    }
    
        function setMaxBBackAmount(uint256 BBamount) external onlyOwner() {
    MaxBBackAmount = BBamount;
    }
    ...
    function swaptoBNB(uint256 contractTokenBalance) private lockTheSwap {
        // swap tokens for BNB
        uint256 initialBalance = address(this).balance; //bnb balance
        swapTokensForEth(contractTokenBalance); //swap into bnb
        uint256 transferredBalance = address(this).balance.sub(initialBalance); //bnb gained
        
          //Send to development and funds address
        //_tBNBFeeTotal = _tBNBFeeTotal.add(address(this).balance); //transferredBalance

        TransferTofundraisingAddress(transferredBalance.div(fundDivisor));   // trasferisce L'X% dei BNB INCASSATI, non del wallet
        //emit SwaptoBNB(contractTokenBalance, _tBNBFeeTotal, fundraisingAddress);
        emit SwaptoBNB(contractTokenBalance, fundraisingAddress);
    }
