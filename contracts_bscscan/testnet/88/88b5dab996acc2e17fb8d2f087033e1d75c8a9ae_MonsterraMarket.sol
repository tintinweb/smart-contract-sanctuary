    ...
    ERC721HolderUpgradeable
{
    ...
    function _validAmount(
        ...
        return
            (_paymentAmount >= buyAmount.mul(ZOOM_FEE).div(ZOOM_FEE))
                ? true
                : false; //102.5%
    }
    ...
    function _match(
        ...
        amountToSeller = amountToSeller.sub(orderAmount.mul(1).div(ZOOM_FEE));
        ...
        returns (bool)
    {
        ...
        exactPaymentAmount = orderAmount;
        // } else {
        //     orderAmount = estimateToken(_paymentToken, orderAmount);
        //     exactPaymentAmount = orderAmount.mul(ZOOM_FEE).div(
        //         ZOOM_FEE
        //     );
        // }
        // uint256 exactPaymentAmount =
        ...
        return
            _match(
                ...
                _orderId,
                _quantity,
                estimateToken(order.tokenAddress, order.price),
                orderAmount
            );
    }
...
interface IMonsterraPayment {
    ...
    function ContractSupport(address _contractAddress) external view returns (uint256, bool, bool);
    function getTokenInfo(address _contractAddress, address _paymentAddress) external view returns (uint256, bool);
}
