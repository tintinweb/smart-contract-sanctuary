...
// Ownership for this contract, when the community or owner is ready this will help renounce ownership.
...
//LockToken Provided for special Whitelist function or VIP Users, this may not be used for any other reason. Open trade is the only function that will ever be used from LockToken. Removed Stop Trade for security reasons for the contract above to pass audit with no red flags.
...
contract Chameleon is Context, IERC20, Ownable, LockToken 
{
    ...
    uint256 public _taxFee = 2;  //2% Reflections on Buy
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 2; //2% Liquidity on Buy
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public _marketingFee = 3; //3% Marketing on Buy
    uint256 private _previousMarketingFee = _marketingFee;
	
	uint256 public _buybackFee = 3; //3% BuyBack Fee during Buy.
    ...
    uint256 public buybackDivisor = 3; //For Main Buy Back

    uint256 public _saleTaxFee = 3; //3% Reflection on Sale.
    uint256 public _saleLiquidityFee = 2; //2% Liquidity fee on Sale.
    uint256 public _saleMarketingFee = 4; //4% Marketing fee on Sale.
    uint256 public _saleBuyBackFee = 4; //4% BuyBack fee on Sale.
    ...
    address payable public marketingWallet =  payable(0xcbFA1ce0b8bFb9C09E713162771C31F176fB1ADE); //Marketing Wallet  Prod: 0x4fcfA45d1F0e27D749276421Af25dA556De525b8
	address payable public buybackWallet =  payable(0xcbFA1ce0b8bFb9C09E713162771C31F176fB1ADE); //Buy Back Wallet  Prod: 0xfc2b6E294BCfE0782FB324d1Be000f3D8DEfa052
...
// This function remove all fees during the Burn Event to avoid any taxes for the manual burn.
...
// This is an internal function to set sale fees = to buy fees in case of a special event, this is a private function.
...
// This function is to restore all fees in one function, is only used during the burn function to avoid tax or reflections during the burn event. 
...
// This function set new buy taxes for the contract, note that it cannot exceed 25% of the function will fail.
...
// This function set new sale taxes for the contract, note that it cannot exceed 25% or the function will fail.
...
// Before presale is scheduled you need to run this function to get taxes to 0% to avoid any problems with the presale.
...
// After Presale is completed you need to execute this function to get taxes back on the contract. 
    function afterPresale() external onlyOwner()   
    {
        _taxFee = 2;  //2% Reflections on Buy.
        _previousTaxFee = 2; //2% Reflections on Buy.
        _liquidityFee = 2; //2% Liquidity on Buy.
        _previousLiquidityFee = 2; //2% Liquidity on Buy.
        _marketingFee = 3; //3% Marketing fee on Buy.
        _previousMarketingFee = 3; //3% Marketing fee on Buy.
        _buybackFee = 3; //3% Buy Back fee for Buy Back Wallet.
        _previousBuyBackFee = 3; //3% Buy Back Fee for Buy Back Wallet.
        _saleTaxFee = 3; //3% Reflection on Sale.
        _saleLiquidityFee = 2; //2% Liquidity fee on Sale.
        _saleMarketingFee = 4; //4% Marketing fee on Sale.
        _saleBuyBackFee = 4; //4% Buy Back fee on Sale.
