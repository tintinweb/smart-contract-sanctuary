...
contract CumStarMinter is WorkerOwnable, ModelRegistry {
    ...
    event CumStarMinterMintedArtistSingleToken(address indexed tokenOwner, uint256 userProfileId, uint256 tokenId);
    ...
    function mintArtistSingleToken(
        uint256 userProfileId,
        uint256 artistFee,
        ...
        ProfileToken memory userProfile = UserProfileTokenInterface(_userProfileTokenContractAddress).getUserProfile(artistAddress, userProfileId);

        uint256 cumStarNFTTokenId = CumStarNFTTokenInterface(_cumStarNFTTokenContractAddress).mint(artistAddress, tokenUri);

        ArtistTokenInterface(_artistTokenContractAddress).mintArtistToken(
            artistAddress,
            userProfileId,
            artistFee,
            cumStarNFTTokenId,
        ...
        emit CumStarMinterMintedArtistSingleToken(artistAddress, userProfile.tokenId, cumStarNFTTokenId);
    }
    ...
    function mintArtist(string memory username, string memory tokenUri, uint256 payableNFTTokenId) external payable {
        address artistAddress = msg.sender;
        require(artistAddress != address(0), "Ownable: Artist address is the zero address");
    ...
    function mintCustomer(string memory username, string memory tokenUri) external payable {
