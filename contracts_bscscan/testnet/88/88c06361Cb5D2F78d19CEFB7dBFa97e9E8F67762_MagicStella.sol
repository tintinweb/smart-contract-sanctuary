...
contract MagicStella is BEP20, Ownable {
	...
	uint256 public maxSellTransactionAmount = (initTotalSupply * 5) / 1000; // 50 000 tokens
  ...
  address public constant BUSD = address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); 
  address public constant USDT = address(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);
	...
	function _transfer(
      ...
      if (_isSellerTransfer(to) && _priceImpactTax(amount)) {
        localFeesRation = localFeesRation.add(_sellFee);
      }

			uint256 fees = (amount.mul(localFeesRation)).div(10000);
    ...
    // Check for price impact before doing transfer
    function _priceImpactTaxV2(uint256 amount) public view returns (uint256, uint256, uint256) {
        ...
        if (_reserveA == 0 && _reserveB == 0) {
            ...
            if (amount >= _reserveA) return (2,2,2);

            uint256 _reserveA_new = _reserveA.sub(amount);
            uint256 _reserveB_new = _constant.div(_reserveA_new);

            if (_reserveB >= _reserveB_new) return (3,3,3);
            uint256 receivedBUSD = _reserveB_new.sub(_reserveB);
            ...
            return (_new_price, _delta_price, _priceImpact);
        }
    }
