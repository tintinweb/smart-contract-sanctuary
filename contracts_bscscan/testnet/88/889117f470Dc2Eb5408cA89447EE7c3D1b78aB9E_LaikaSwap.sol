...
contract LaikaSwap is Context, Ownable {
    ...
    address public newLaika = 0x5684A49D93915Cb550502964Fb4e52112eA7216E;
    ...
    function loadSeedAccounts (address[] memory _seedAccounts) public onlyAuthorised {
        ...
        for (uint256 i = 0; i < _seedAccounts.length; i++) {
            ...
            if (i == 0)
                prevRatio = IERC20(oldLaika).balanceOf (_seedAccounts[i]) * MAGNITUDE / snapshotBalance[_seedAccounts[i]];
            else
                require (IERC20(oldLaika).balanceOf (_seedAccounts[i]) * MAGNITUDE / snapshotBalance[_seedAccounts[i]] == prevRatio, "Seed balances have changed inconsistently since snapshot");
        }
