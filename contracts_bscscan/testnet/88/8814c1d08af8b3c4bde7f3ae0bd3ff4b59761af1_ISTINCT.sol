...
// File: contracts/ISTINCTDividendPayingToken.sol
...
// File: contracts/ISTINCT.sol

// 

pragma solidity ^0.7.6;

contract ISTINCT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ISTINCTDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("TEST1", "TEST1") {
        ...
        dividendTracker = new ISTINCTDividendTracker();
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "ISTINCT: The dividend tracker already has that address");

        ISTINCTDividendTracker newDividendTracker = ISTINCTDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ISTINCT: The new dividend tracker must be owned by the ISTINCT token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "ISTINCT: The router already has that address");
        uniswapV2Router = IUniswapV2Router02(newAddress);
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "ISTINCT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "ISTINCT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ISTINCT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ISTINCT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ISTINCT: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "ISTINCT: This account cannot send tokens until trading is enabled");
        }
...
contract ISTINCTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("ISTINCT_Dividend_Tracker", "ISTINCT_Dividend_Tracker") {
        claimWait = 3600;
...
// Developed by snipe.finance 
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "ISTINCT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "ISTINCT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ISTINCT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ISTINCT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ISTINCT_Dividend_Tracker: Cannot update claimWait to same value");
