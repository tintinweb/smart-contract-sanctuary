...
contract MPStaking is Ownable, ReentrancyGuard {
    ...
    // Avg no.of Blocks per day in BSC - 28800
    // Source - https://ycharts.com/indicators/binance_smart_chain_blocks_per_day
    uint256 public noOfBlocksPerDay = 28800;
        ...
        returns (uint256[] memory milestones)
    {
        ...
        uint256 _gapBlocks = _noOfBlocksPerDay.mul(30);   // gap between milestone[0] & milestone[1], 31 days, 1 day will include in the loop

        milestones[0] = _rewardStartBlock;
        milestones[1] = _rewardStartBlock.add(_gapBlocks).add(_noOfBlocksPerDay); // Make it 31 days
    ...
    function harvest() external nonReentrant {
        ...
        if (user.status[0] == false) {
            ...
            if (block.number > user.milestones[1]) {
                gapReward = user.amounts[0].mul(2).mul(apy.mul(1e12).div(1e12)).mul(30).div(noOfDaysYear).div(100);
            }
        ...
        require(currentLocked > 0, "Harvest:: Withdrawing too early");

        // Tokens allowed to claim
        IERC20(stakeToken).transfer(msg.sender, currentLocked);
        ...
        emit StakeHarvest(msg.sender, block.timestamp, currentLocked);
        emit RewardHarvest(
            msg.sender,
            block.timestamp,
            totalReward
    ...
    function pendingHarvest(address _to) external view returns (uint256, uint256) {
        ...
        return (currentLocked, totalReward);
    }
