...
contract Link is LinkInfo, Initialized, Ilink {
    ...
    event Getaddress(address indexed luca, address indexed wluca,  address indexed weth, address trader, address token);
    ...
    function _withdraw(address to, uint amount) internal{
        ...
        emit Getaddress(luca, wluca, weth, trader, token);
        if (true) return ;
        if (token == ETH){
            ...
            //if (true)revert("befor in withdraw");
