...
contract AmbassadorProgram is IAmbassadorProgram, Ownable {
    ...
    address[] public ambassadorsHistory; // contains deleted ambassadors
    address[] public deletedAmbassadors;
    ...
    address[] public ambassadorManagers;
    mapping(address => bool) public isAmbassadorManager;
    ...
    modifier onlyOwnerOrManagers() {
        require(
            owner() == _msgSender() || isAmbassadorManager[_msgSender()],
            "Caller is not owner nor manager"
        );
        _;
    }
        ...
        onlyOwnerOrManagers
    {
        for (uint256 index = 0; index < addrs.length; index++) {
            ...
            if (!isAmbassador(currentAddr)) {
                ...
                ambassadorsHistory.push(currentAddr);
            }
        }
        ...
        onlyOwnerOrManagers
    {
        for (uint256 index = 0; index < addrs.length; index++) {
            ...
            if (isAmbassador(currentAddr)) {
                ...
                deletedAmbassadors.push(currentAddr);
            }
        }
    ...
    function addAmbassadorManagers(address[] memory managers)
        public
        override
        onlyOwner
    {
        for (uint256 index = 0; index < managers.length; index++) {
            ambassadorManagers.push(managers[index]);
            isAmbassadorManager[managers[index]] = true;
        }
        emit AddedAmbassadorManagers(managers);
    }
    ...
    function hashFrom(address addr) public view override returns (bytes32) {
        require(
            owner() == _msgSender() ||
                isAmbassadorManager[_msgSender()] ||
                addr == _msgSender() ||
    ...
    function updateSaleContract(address newAddr) public override onlyOwner {
        require(
            newAddr != address(0) && newAddr != address(this),
            "Wrong sale address"
        );

        emit SaleContractUpdated(r1SaleContract, newAddr);

        r1SaleContract = newAddr;
    }
}
