...
contract PreSaleTier is ReentrancyGuard, Context, Ownable{
    ...
    uint public startDate = 1634906346; // Mon Oct 22 2021
    ...
    function isTierMapping(address user, uint256 amount) public view returns (bool){
        ...
        if(specialListed[msg.sender] ) {
            result = true;
        } else if (amount <= tier3BnbAmount && amountLocked >= tier3MilkyAmount) {
            result = true;
        } else if (amount <= tier2BnbAmount && amountLocked >= tier2MilkyAmount) {
            result = true;
        } else if (amount <= tier1BnbAmount && amountLocked >= tier1MilkyAmount) {
            result = true;
        }
        return result;
    }
