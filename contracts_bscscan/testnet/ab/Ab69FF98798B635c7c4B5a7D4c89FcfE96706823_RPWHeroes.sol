...
import "../interfaces/IRPWHeroes.sol";
import "../utils/AcceptedToken.sol";

contract RPWHeroes is IRPWHeroes, ERC721, ReentrancyGuard, AcceptedToken {
    using SafeERC20 for IERC20;

    uint private constant BPS = 10000;

    uint public inValueInBps = 200;
    uint public summonHeroInValueInBps = 1000;
    ...
    constructor(
        ...
        uint startTime,
        address accountant
    ) ERC721("RPWHeroes", "RPWHeroes") AcceptedToken(tokenAddress) {
        _uri = baseURI;
        versions.push(Version(0, maxSupply, salePrice, startTime, accountant));
    }

    modifier onlyHeroOwner(uint heroId) {
        require(ownerOf(heroId) == msg.sender, "RPWHeroes: not hero owner");
        _;
    }

    function setInValueAndMarketFeeInBps(uint inValue, uint marketFee) external onlyOwner {
        require(inValue + marketFee <= BPS);
        inValueInBps = inValue;
    ...
    function addNewVersion(
        ...
        uint latestVersionId = getLatestVersionId();
        versions.push(Version(0, maxSupply, salePrice, startTime, accountant));
        emit NewVersionAdded(latestVersionId + 1);
    }

    function getLatestVersionId() public view returns (uint) {
    ...
    function summonHero(
        ...
        uint str, 
        uint dex, 
        uint intl) external override payable 
    {
        Version storage version = versions[getLatestVersionId()];
        uint inValue = version.salePrice * summonHeroInValueInBps / BPS;

        require(block.timestamp >= version.startTime, "RPWHeroes: Sale has not started");
        require(version.currentSupply + 1 <= version.maxSupply, "RPWHeroes: sold out");
        require(msg.value == version.salePrice, "RPWHeroes: incorrect value");
        ...
        _heroes.push(Hero(
            ...
            dex, 
            intl));
        uint heroId = _heroes.length - 1 < 0 ? 0 : _heroes.length - 1;
        _safeMint(msg.sender, heroId);
        version.currentSupply += 1;

        (bool isSuccess,) = version.accountant.call{value: msg.value - inValue}("");
        require(isSuccess);

        emit HeroCreated(heroId, inValue);
    }

    function list(uint heroId, uint price) external override onlyHeroOwner(heroId) {
        require(price >= _heroes[heroId].inValue, "RPWHero: under inValue");
        heroesOnSale[heroId] = price;
        emit HeroListed(heroId, price);
    }

    function delist(uint heroId) external override onlyHeroOwner(heroId) {
        require(heroesOnSale[heroId] > 0, "RPWHero: not listed");
    ...
    function buy(uint heroId) external override payable nonReentrant {
        ...
        require(price > 0, "RPWHero: not on sale");
        require(msg.value == price, "RPWHero: incorrect value");
        require(buyer != seller, "RPWHero: cannot buy your own Hero");
    ...
    function offer(uint heroId, uint offerValue) external override nonReentrant payable {
        address buyer = msg.sender;
        uint currentOffer = heroesWithOffers[heroId][buyer];
        bool needRefund = offerValue < currentOffer;
        uint requiredValue = needRefund ? 0 : offerValue - currentOffer;

        require(buyer != ownerOf(heroId), "RPWHero: owner cannot offer");
        require(offerValue != currentOffer, "RPWHero: same offer");
        require(msg.value == requiredValue, "RPWHero: sent value incorrect");

        heroesWithOffers[heroId][buyer] = offerValue;
    ...
    function takeOffer(
        ...
        uint offeredValue = heroesWithOffers[heroId][buyer];
        address seller = msg.sender;

        require(offeredValue >= _heroes[heroId].inValue, "RPWHero: under inValue");
        require(offeredValue >= minPrice, "RPWHero: less than min price");
        require(buyer != seller, "RPWHero: cannot buy your own Hero");

        heroesWithOffers[heroId][buyer] = 0;
    ...
    function cancelOffer(uint heroId) external override nonReentrant {
        ...
        require(offerValue > 0, "RPWHero: no offer found");
    ...
    function levelUp(uint heroId, uint amount) external override onlyOperator {
        ...
        require(amount > 0);
        require(newLevel <= maxLevel, "RPWHero: max level reached");
    ...
    function getHero(uint heroId) external override view returns (
        string memory name, 
        uint level,
        uint inValue,
        uint armor, 
        ...
        name = hero.name;
        inValue = hero.inValue;
        level = hero.level;
        armor = hero.armor;
        magRes = hero.magRes;
        phyAtt = hero.phyAtt;
        spellInc = hero.spellInc;
        hp = hero.hp;
        manaRg = hero.manaRg;
        str = hero.str;
        dex = hero.dex;
    ...
    function releaseHero(uint heroId) external override nonReentrant onlyHeroOwner(heroId) {
        Hero storage hero = _heroes[heroId];
        uint amount = hero.inValue;
    ...
    function addInValueToHero(uint heroId) external override payable {
        Hero storage hero = _heroes[heroId];
        uint newInValue = hero.inValue + msg.value;

        require(msg.value > 0, "RPWHero: no value sent"); 
        require(newInValue <= 50 ether, "RPWHero: cannot add more");
        require(acceptedToken.balanceOf(msg.sender) >= serviceFeeInToken, "RPWHero: insufficient token balance");

        hero.inValue = newInValue;
        acceptedToken.safeTransferFrom(msg.sender, versions[getLatestVersionId()].accountant, serviceFeeInToken);

        emit HeroPriceIncreased(heroId, newInValue, serviceFeeInToken);
    }      

    function _makeTransaction(uint heroId, address buyer, address seller, uint price) private {
        Hero storage hero = _heroes[heroId];
        uint inValue = price * inValueInBps / BPS;
        uint marketFee = price * marketFeeInBps / BPS;
        uint newPrice = hero.inValue + inValue;

        heroesOnSale[heroId] = 0;
        hero.inValue = newPrice;

        (bool transferToSeller,) = seller.call{value: price - (inValue + marketFee)}("");
        require(transferToSeller);

        (bool transferToTreasury,) = versions[getLatestVersionId()].accountant.call{value: marketFee}("");
        require(transferToTreasury);

        _transfer(seller, buyer, heroId);

        emit HeroPriceIncreased(heroId, newPrice, inValue);
    }      
}
...
interface IRPWHeroes {
    ...
    event HeroCreated(uint indexed heroId, uint inValue);
    ...
    event HeroPriceIncreased(uint indexed heroId, uint inValue, uint increasedAmount);
    event HeroLeveledUp(uint indexed heroId, uint level, uint amount);
    event NewVersionAdded(uint versionId);

    /**
     * @dev Claims one or more heroes when it's on presale phase.
     */
    function summonHero( 
        string memory name, 
        uint armor, 
        ...
        uint str, 
        uint dex, 
        uint intl) external payable;

    /**
     * @dev Lists a hero on sale.
     *
     * Requirements:
     * - `price` cannot be under hero's `inValue`.
     * - Caller must be the owner of the hero.
     */
    function list(uint heroId, uint price) external;

    /**
     * @dev Delist a hero on sale.
     */
    function delist(uint heroId) external;

    /**
     * @dev Instant buy a specific hero on sale.
     *
     * Requirements:
    ...
    /**
     * @dev Owner take an offer to sell their heroes.
     *
     * Requirements:
     * - Cannot take offer under the hero's `inValue`.
     * - Offer value must be at least equal to `minPrice`.
     */
    function takeOffer(uint heroId, address offerAddr, uint minPrice) external;

    /**
     * @dev Cancels an offer for a specific hero.
     */
    function cancelOffer(uint heroId) external;

    function releaseHero(uint heroId) external; 

    function levelUp(uint knightId, uint amount) external;    

    function addInValueToHero(uint heroId) external payable;   
