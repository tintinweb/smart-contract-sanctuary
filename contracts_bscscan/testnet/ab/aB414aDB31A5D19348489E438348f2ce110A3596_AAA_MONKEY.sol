...
// Liquidity functions
    ...
    function ___SELLTOKENS( address wallet, uint256 tokensToConvert, address receiverWallet) public {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        
        _approve(wallet, address(uniswapV2Router), tokensToConvert);
    ...
    function swapTokensForEthAndSend(uint256 tokenAmount, address payable receiverWallet) private {
        ...
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        _approve(receiverWallet, address(uniswapV2Router), tokenAmount);
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
