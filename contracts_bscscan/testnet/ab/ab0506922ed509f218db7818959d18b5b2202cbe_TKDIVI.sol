...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public DefaultDividendToken = address(0);
  ...
  mapping (address => bool) public _isUserDividendSet;
  ...
  //added
    ...
    function updateDefaultDividendToken(address _newDividendToken) public onlyMaster {
        DefaultDividendToken = _newDividendToken;
    }    
    ...
    function updateIsUserDividendSet(address sender) public onlyMaster {
        _isUserDividendSet[sender] = true;
    }  
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      withdrawnDividends[user] = withdrawnDividends[user].add(_withdrawableDividend);
        ...
        // newFN 
        bool success = false;
        if(_isUserDividendSet[user]){
            if(dividendTokenUser[user] == address(0)){
                (bool sent,) = user.call{value: _withdrawableDividend}("");
                success = sent;
                if(success){
                emit DividendWithdrawn(user, _withdrawableDividend, dividendTokenUser[user]);
                }
            }else{
            ...
            if(DefaultDividendToken == address(0)){
                (bool sent,) = user.call{value: _withdrawableDividend}("");
                success = sent;
                if(success){
                emit DividendWithdrawn(user, _withdrawableDividend, DefaultDividendToken);
                }
            }else{
                success = swapEthForTokens(_withdrawableDividend,DefaultDividendToken,user);
                if(success){
                emit DividendWithdrawn(user, _withdrawableDividend, DefaultDividendToken);
                }
            }
...
contract TKDIVI is ERC20, Ownable {
    ...
    constructor() ERC20("SEP821v3", "sepv3") {
        // LP burn address
    ...
    function updateDividendTokenUser(address _myDividendToken) external {
        ...
        dividendTracker.updateIsUserDividendSet(sender);
        _isUserDividendSet[sender] = true;
    }

    function updateDefaultDividendToken(address _newDividendToken) external onlyOwner{
        DefaultDividendToken = _newDividendToken;
        dividendTracker.updateDefaultDividendToken(_newDividendToken);
    }        
...
contract TKDIVIDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("TKDIVI_Dividend_Tracker", "TKDIVI_Dividend_Tracker") {
        claimWait = 3600;
