...
contract PeaFusion is Ownable {
    ...
    function fusion(uint256 _mainTokenId, uint256 _foodTokenId, FusionClass _fClass) external {
        ...
        require(peaToken.balanceOf(_msgSender()) >= fusionFee, "Insufficient Token to fusion");
        ...
        require(_main_pean.champ > 0, "Can not fusion a Gem");
        require(_main_pean.champ < 5, "Champ already max level");
        ...
        if (rnd < sucRate) {
            ...
            peaNFT.levelUp(_mainTokenId);
            peaNFT.destroy(_foodTokenId);
        }
        ...
        peaNFT.transferFrom(address(this), peaRouter.feeAddress(), _foodTokenId);
