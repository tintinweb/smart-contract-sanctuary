...
contract ILO is Ownable { 
    ...
    uint256 public bnb_hardcap = 10 * 10 ** 6 * 10 ** 18;
    uint256 public muft_hardcap = 10 * 10 ** 9 * 10 ** 18;
    ...
    function commitMuft(uint256 _amount) MuftCommitable isNotBeingCommit isNotBeingWithdraw external {
        ...
        uint256 muft_swap_amount = _amount / muft_muftswap_rate;
        user.muft_swap = user.muft_swap + muft_swap_amount;

        emit CommitMUFT(msg.sender, _amount);
