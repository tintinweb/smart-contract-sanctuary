...
contract StakeToken is Ownable {
    ...
    address[] public bep20Profit = [0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee];
    address public takeBep20 = 0x2076A228E6eB670fd1C604DE574d555476520DB7;
    struct bag {
        uint start;
        uint amount;
    }
    ...
    event DepositProfit(address _bep20, uint _amount);
    ...
    function depositProfit(address _bep20, uint _amount) public {
        IBEP20 bep20pf = IBEP20(_bep20);
        require(bep20pf.transferFrom(msg.sender, address(this), _amount), 'insufficient-allowance');
        bool existed;
        for(uint i = 0; i < bep20Profit.length; i++) {
            if(_bep20 == bep20Profit[i]) existed = true;
        }
        if(!existed) bep20Profit.push(_bep20);
        emit DepositProfit(_bep20, _amount);
    }
    ...
    function reward(IBEP20 bep20pf, uint _stakeAmount) public view returns(uint _reward) {
        uint bep20Balance = getRemainingToken(bep20);
        uint bep20pfBalance = getRemainingToken(bep20pf);
        return bep20pfBalance.mul(_stakeAmount).div(bep20Balance);
    }
    function rewards(uint _stakeAmount) public view returns(uint[] memory _rewards) {
        uint bep20Balance = getRemainingToken(bep20);
        uint BNBBalance = address(this).balance;
        uint BNBReward = BNBBalance.mul(_stakeAmount).div(bep20Balance);
        _rewards[0] = BNBReward;
        for(uint i = 0; i < bep20Profit.length; i++) {
            IBEP20 bep20pf = IBEP20(bep20Profit[i]);
            _rewards[i+1] = reward(bep20pf, _stakeAmount);
        }
    }
    function refundReward(uint _stakeAmount) internal {
        uint bep20Balance = getRemainingToken(bep20);
        uint BNBBalance = address(this).balance;
        if(BNBBalance > 0) msg.sender.transfer(BNBBalance.mul(_stakeAmount).div(bep20Balance));
        for(uint i = 0; i < bep20Profit.length; i++) {
            IBEP20 bep20pf = IBEP20(bep20Profit[i]);
            uint bep20pfBalance = getRemainingToken(bep20pf);
            if(bep20Balance > 0 && bep20pfBalance > 0) {
                uint _reward = bep20pfBalance.mul(_stakeAmount).div(bep20Balance);
                bep20pf.transfer(msg.sender, _reward);
            }
        }
    }
    ...
    function stake(uint _amount) public {
        ...
        packages[msg.sender].bags[block.number] = bag(now, _amount);
        packages[msg.sender].bagLength.push(block.number);
        emit Stake(_amount, block.number);
    }
    function getStake(address _guy) public view returns(uint[] memory _bagLength) {
        _bagLength = packages[_guy].bagLength; 
    }
    function getStake(address _guy, uint index) public view returns(bag memory _bag) {
        _bag = packages[_guy].bags[index]; 
    }
