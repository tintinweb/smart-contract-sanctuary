...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    address private charityWallet = 0xbB91B72A1bbCAc905f0FC3fb51Fc09Cbbc4F8A3D;
    ...
    constructor(string memory name_, string memory symbol_, uint256 totalSupply_) {
        ...
        _owner = msg.sender;
        //by default the taxes will be on
        _toggleMWTax = true;
        _toggleCWTax = true;
        _toggleDWTax = true;
        _toggleBWTax = true;
        //intially the tax will be 2% each
        _MarketingWalletTax = 2;
        _CharityWalletTax = 2;
        _DevWalletTax = 2;
        _BurnWalletTax = 2;
        //by default the max of totalSupply will be enabled
        _toggleMPTS = true;
        //be default the max percent of total supply will be 40%
        _maxPercentTotalSupply = 40;
        //by default the max per tx will be enabled
        _toggleMPPT = true;
        //be default the max percent of total supply will be 10%
        _maxPercentPerTx = 10;
        ...
        address[5] memory addresses = [marketingWallet, charityWallet, devWallet, burnWallet, _owner];
    ...
    function _transfer(address sender,address recipient,uint256 amount) internal virtual whenNotPaused maxLimitPerTx(sender, amount) maxLimitTotalSupply(recipient, amount) {
        ...
        //if the charity tax is turned on and neither the recipient nor the sender is on the tax exempt whitelist, a tax while be calculated
        if (_toggleCWTax == true && noCharityTax[recipient] == false && noCharityTax[sender] == false) {
            charityTax = (amount * _CharityWalletTax) / 100;
        ...
        _balances[recipient] += (amount - marketingTax - charityTax - burnTax - devTax);
    ...
    function _pause() internal virtual whenNotPaused onlyOwner {
        _paused = true;
        //emitting a pause event so the exterior can know that the contract has been paused
    ...
    function _unpause() internal virtual whenPaused onlyOwner {
        _paused = false;
        //emitting a unpause event so the exterior can know that the contract has been unpaused
    ...
    //turn on & turn off the charityTax
    function toggleCWTax() public onlyOwner {
        //checks to see current state and goes to the other
        if (_toggleCWTax == true) {
            _toggleCWTax = false;
        }
        else {
            _toggleCWTax = true;
    ...
    //give the function a percentage and it will make it the new charityTax
    function changeCharityTax(uint256 _percent) public onlyOwner {
        require(_percent < 25, 'Tax Cannot be more than 25% of the transaction value');
        _CharityWalletTax = _percent;
    }
    ...
    function addToNoCharityTax(address newAddress) public onlyOwner {
        require(noCharityTax[newAddress] != true, 'Address is already part of Tax Exempt List');
        
        noCharityTax[newAddress] = true;
    }
    
    function removeFromNoCharityTax(address newAddress) public onlyOwner {
        require(noCharityTax[newAddress] != false, 'Address is not part of Tax Exempt List');
        
        noCharityTax[newAddress] = false;
    }
    ...
    function CharityTaxIsActive() public onlyOwner view returns(bool) {
        return _toggleCWTax;
    }
    ...
    function currentCharityTax() public onlyOwner view returns(uint256) {
        return _CharityWalletTax;
    }
