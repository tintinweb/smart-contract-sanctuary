...
import "../interfaces/IPlayLottery.sol";

/** @title Play Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract PlayLottery is ReentrancyGuard, IPlayLottery, Ownable {
    ...
    uint256 public maxPriceTicketInPlay = 50 ether;
    uint256 public minPriceTicketInPlay = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInPlay;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] playPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInPlay;
        uint32 finalNumber;
    }
     ...
     * @param _playTokenAddress: address of the PLAY token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _playTokenAddress, address _randomGeneratorAddress) {
        playToken = IERC20(_playTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of PLAY to this contract
        uint256 amountPlayToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInPlay,
            _ticketNumbers.length
        );

        // Transfer play tokens to this contract
        playToken.safeTransferFrom(address(msg.sender), address(this), amountPlayToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInPlay += amountPlayToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInPlayToTransfer
        uint256 rewardInPlayToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInPlayToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        playToken.safeTransfer(msg.sender, rewardInPlayToTransfer);

        emit TicketsClaim(msg.sender, rewardInPlayToTransfer, _lotteryId, _ticketIds.length);
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in PLAY per group, and make lottery claimable
     * @param _lotteryId: lottery id
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInPlay) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in PLAY for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                // B. If rewards at this bracket are > 0, calculate,
                // else, report the numberAddresses from previous bracket
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].playPerBracket[j] =
                ...
                // A. No PLAY to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].playPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInPlay - amountToShareToWinners);

        // Transfer PLAY to treasury address
        playToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in PLAY token
     * @dev Callable by owner or injector address
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        playToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInPlay += _amount;
     ...
     * @param _priceTicketInPlay: price of a ticket in PLAY
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInPlay >= minPriceTicketInPlay) && (_priceTicketInPlay <= maxPriceTicketInPlay),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInPlay: _priceTicketInPlay,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            playPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInPlay: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(playToken), "Cannot be PLAY token");
    ...
    /**
     * @notice Set PLAY price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInPlay: minimum price of a ticket in PLAY
     * @param _maxPriceTicketInPlay: maximum price of a ticket in PLAY
     */
    function setMinAndMaxTicketPriceInCake(uint256 _minPriceTicketInPlay, uint256 _maxPriceTicketInPlay)
        external
        onlyOwner
    {
        require(_minPriceTicketInPlay <= _maxPriceTicketInPlay, "minPrice must be < maxPrice");

        minPriceTicketInPlay = _minPriceTicketInPlay;
        maxPriceTicketInPlay = _maxPriceTicketInPlay;
    }
     ...
     * @param _priceTicket price of a ticket (in PLAY)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].playPerBracket[_bracket];
        } else {
            return 0;
        }
    }
