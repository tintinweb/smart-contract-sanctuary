...
contract PuzlUnpack is ERC1155 {
    ...
    function open(address who, uint256 tokenId) private {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            amounts[i] = 1;
            _mintAPart(who, ((100000*i) + tokenId));
        }
       // _mintBatch(who, ids, amounts, "");

       // emit CreatePart(ids[0]);
    ...
    function uri(uint256 _tokenId) override public view returns (string memory) {
     ...
     function tokenURI(uint256 _tokenId) public view returns (string memory) {
        ...
        parts[3] = '</text><text x="10" y="160" class="base"></text></svg>';

        string memory output = string(abi.encodePacked(parts[0], parts[1], parts[2], parts[3]));
        
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Part From Pack #', toString(_tokenId), '", "description": "PuzlPunks Parts are randomized parts for building puzlpunks stored on chain.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
