...
contract Farm is BaseFarm, IFarm {
    ...
    function init(
        ...
        uint256[2] memory _vestingParameters, // 0: percentForVesting, 1: vestingDuration
        address _owner
    ) public {
...
contract Vesting is ReentrancyGuard {
    ...
    function claimTotalVesting(uint256 _start, uint256 _end) external nonReentrant {
        ...
        for (uint256 _index = _start; _index <= _end; _index++) {
            if (_getVestingClaimableAmount(_user, _index) > 0) {
                _claimVesting(_index);
            }
        }
    }
    ...
    function _claimVesting(uint256 _index) internal {
        address _user = msg.sender;
        require(_index < _userToVestingList[_user].length, "Vesting: Invalid index");
        uint256 claimableAmount = _getVestingClaimableAmount(_user, _index);
        require(claimableAmount > 0, "Vesting: Nothing to claim");
        _userToVestingList[_user][_index].claimedAmount += claimableAmount;
        ...
        _totalClaimedAmount[_user] += claimableAmount;
        token.safeTransfer(_user, claimableAmount);
    }
