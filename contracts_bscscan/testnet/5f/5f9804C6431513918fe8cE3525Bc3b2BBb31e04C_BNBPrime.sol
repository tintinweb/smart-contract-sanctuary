...
contract BNBPrime {
    ...
    function compound(address referrer, uint8 plan) public{
        ...
        user.totalReinvested = uint128(uint256(user.totalReinvested).add(totalAmount));
    ...
    function withdraw() public {
        ...
        user.checkpoint = uint32(block.timestamp);
        user.withdrawn = uint128(uint256(user.withdrawn).add(totalAmount));
        ...
        if (user.referrer != address(0)) {
            ...
            for (uint256 i = 0; i < 5; i++) {
                if (upline != address(0)) {
                    ...
                    users[upline].totalBonus = uint128(uint256(users[upline].totalBonus).add(amount));
    ...
    function getUserInfo(address userAddress) public view returns(uint256 totalDeposit, uint128 totalReinvested, uint128 totalWithdrawn, uint128 totalBonus, uint64 bonus, uint256 available, uint32 checkpoint) {
        return(
