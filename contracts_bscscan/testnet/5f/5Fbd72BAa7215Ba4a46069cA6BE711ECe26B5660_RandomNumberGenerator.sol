...
contract RandomNumberGenerator is VRFConsumerBase, IRandomNumberGenerator, ReentrancyGuard, Ownable {
    ...
    uint32[] internal _randomResults;
    ...
    function clearRandomResults() external override {
        require(msg.sender == plearnLucky, 'Only PlearnLucky');
        delete _randomResults;
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override nonReentrant {
        ...
        require(_randomResults.length == 0, 'Already randomness');

        uint256 luckyId = IPlearnLucky(plearnLucky).getCurrentLuckyId();
        uint256 countTicketsInRound = IPlearnLucky(plearnLucky).getCountTicketsInRound(luckyId);

        uint32 countLuckyNumbersPerRound = IPlearnLucky(plearnLucky).getCountLuckyNumbersPerRound(luckyId);
        (uint32 firstTicketNumber, uint32 lastTicketNumber) = IPlearnLucky(plearnLucky)
            .getFirstAndLasTicketNumberInRound(luckyId);

        uint32[] memory luckyNumbers = new uint32[](countLuckyNumbersPerRound);
        uint32 countAdded = 0;
        uint256 countRandom = 0;

        if (countTicketsInRound > countLuckyNumbersPerRound) {
            while (countAdded < countLuckyNumbersPerRound) {
                uint256 randomNumber = uint256(keccak256(abi.encode(randomness, (countRandom + 1), block.timestamp)));
                uint32 luckyNumber = uint32(
                    (randomNumber % (lastTicketNumber - firstTicketNumber + 1)) + firstTicketNumber
                );

                bool luckyNumberIsNotExist = true;
                for (uint256 i = 0; i < countAdded; i++) {
                    uint32 existNumber = luckyNumbers[i];
                    if (luckyNumber == existNumber) {
                        luckyNumberIsNotExist = false;
                        break;
                    }
                }
                if (luckyNumberIsNotExist) {
                    luckyNumbers[countAdded] = luckyNumber;
                    countAdded++;
                }
                countRandom++;
            }
        } else {
            for (uint256 i = 0; i < countTicketsInRound; i++) {
                luckyNumbers[i] = firstTicketNumber++;
            }
        }

        //Set lucky numbers to random result
        _randomResults = luckyNumbers;

        //Update lasted lucky id after genarate randoom numbers finish
     ...
     */
    function getRandomResults() external view override returns (uint32[] memory) {
