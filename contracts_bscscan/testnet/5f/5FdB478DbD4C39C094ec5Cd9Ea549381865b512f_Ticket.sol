...
contract Ticket is ERC721, ITicket, PermissionRight {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenCounter;
    string internal _baseUri;
    event TicketMint(address indexed account, uint256 tokenId);

    constructor (address _owner, string memory name_, string memory symbol_) ERC721(name_, symbol_){
    ...
    function mint(
        ...
        for (uint i = 0; i < quantity; i++) {
            ...
            _safeMint(receiver, _tokenCounter.current());
            emit TicketMint(receiver, _tokenCounter.current());
        }
    }
...
contract PermissionRight is Ownable {
    ...
    mapping(address => bool) public operatorGroup;

    modifier onlyAdmin() {
        require(adminGroup[_msgSender()], "PermissionRight: You're not in the admin group");
        _;
    }

    modifier onlyOperator() {
        require(operatorGroup[_msgSender()], "PermissionRight: You're not in the operator group");
        _;
    }

    function inAdminGroup() internal view returns (bool) {
        return adminGroup[_msgSender()];
    }

    function inOperatorGroup() internal view returns (bool) {
        return operatorGroup[_msgSender()];
    }
