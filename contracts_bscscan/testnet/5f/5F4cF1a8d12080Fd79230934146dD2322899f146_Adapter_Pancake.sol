...
// kjill the main adapter assumes pancake so don't think this is needed

contract Adapter_Pancake is Adapter {
  constructor(
    ...
    // earnedToNUTSPath = _earnedToAUTOPath;
...
abstract contract Adapter is Ownable, ReentrancyGuard, Pausable {
  ...
  address public adminAddress; // timelock contract

  modifier onlyAdmin() {
    require(msg.sender == adminAddress, '!gov');
    _;
  }
  ...
  function deposit(
    ...
    uint256 _wantAmt,
    uint256 _pid
  ) public virtual onlyOwner whenNotPaused nonReentrant returns (uint256) {
  ...
  function _farm(uint256 _pid) internal virtual {
    ...
    if (isCAKEStaking) {
      ...
      IPancakeswapFarm(farmContractAddress).deposit(_pid, wantAmt);
    }
  }

  // leave staking / withdraw
  function _unfarm(uint256 _wantAmt, uint256 _pid) internal virtual {
    if (isCAKEStaking) {
      // withdraw CAKE tokens from STAKING
      IPancakeswapFarm(farmContractAddress).leaveStaking(_wantAmt); // Just for CAKE staking, we dont use withdraw()
    } else {
      // withdraw LP tokens from MasterChef
      IPancakeswapFarm(farmContractAddress).withdraw(_pid, _wantAmt);
    }
  }
