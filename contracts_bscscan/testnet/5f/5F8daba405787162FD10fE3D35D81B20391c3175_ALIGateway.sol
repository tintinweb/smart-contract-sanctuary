...
contract ALIGateway is OwnableUpgradeable {
    ...
    modifier onlyLottery() {
        require(_msgSender() == lottery, "ALIGateway: No permission");
        _;
    }

    modifier onlyManager() {
        require(_msgSender() == owner() || _msgSender() == lottery, "ALIGateway: No permission");
        _;
    }
    function initialize(address _lottery, IYieldSource _yieldSource, address _ali, address _priceFeed) public initializer {
        lottery = _lottery;
        erc20 = IERC20(_ali);
    ...
    function setLottery(address _lottery) public onlyOwner {
        require(_lottery != address(0), "ALIGateway: _lottery must not be zero address");
        emit ChangeLottery(lottery, _lottery);
        lottery = _lottery;
    }
    function setYieldSource(address _yieldSource) public onlyOwner {
        require(_yieldSource != address(0), "ALIGateway: _yieldSource must not be zero address");
        emit ChangeYieldSource(address(yieldSource), _yieldSource);
        yieldSource = IYieldSource(_yieldSource);
    }
    function setPriceFeed(address _priceFeed) public onlyOwner {
        require(_priceFeed != address(0), "ALIGateway: _priceFeed must not be zero address");
