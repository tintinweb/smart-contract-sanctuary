...
/** 
 * Forked Distributor from SafeVault, adjusted to Useless reflections
 */
...
contract DividendDistributor is IDividendDistributor {
    ...
    // SafeVault Contract
    address _token;
    // Share of the Useless Pie
    struct Share {
        uint256 amount;
        uint256 totalExcluded;
        uint256 totalRealised;
    }
    // Reflectiontoken contract address
    address RT = 0x2cd2664Ce5639e46c6a3125257361e01d0213657; //Useless
    // bnb address
    ...
    // 1 Million Useless Minimum Distribution
    uint256 public minDistribution = 1 * (10 ** 15);
    // BNB Needed to Swap to Useless
    ...
    function deposit() external override onlyToken {
        ...
        if (bnbBalance >= swapToSafemoonThreshold) {
            ...
            uint256 balanceBefore = IERC20(RT).balanceOf(address(this));
            ...
            uint256 amount = IERC20(RT).balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            IERC20(RT).transfer(shareholder, amount);
    ...
    function setSafemoonAddress(address nRTN) external onlyToken {
        RT = nRTN;
    }
 ...
 *  10% Useless Distribution
 *  5% Buyback and burn
 *  2% Auto Liquidity
 *  3% Marketing
 */
contract SuperUseless is IERC20, Context, Ownable {
    ...
    // our burn wallet address 
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    // token data
    string constant _name = "SuperUseless";
    ...
    address public marketingFeeReceiver = 0x14Bba80671D55768eB110285f93C50F2cA2237b1;
    address public RT = 0x2cd2664Ce5639e46c6a3125257361e01d0213657;
    ...
    constructor (
        ...
        // exempt this contract, the LP, and OUR burn wallet from receiving Useless Rewards
    ...
    /** Takes Associated Fees and sets holders' new Share for the Useless Distributor */
    ...
    /** Takes Proper Fee (5% buys / transfers, 20% on sells) and stores in contract */
    ...
    /**
     *  Swaps SuperUseless for BNB if threshold is reached and the swap is enabled
     *  Uses BNB retrieved to:
     *      fuel the contract for buy/burns
     *      provide distributor with BNB for Useless
    ...
    /** Transfers BNB to Useless Distributor and Marketing Wallet */
    ...
    function setReflectionContractAddress(address nRTN) public onlyOwner {
        distributor.setSafemoonAddress(nRTN);
        
    }
        
        function getReflectionContractAddress() external view returns (address) {
        return address(RT);
    }
