...
contract Referral {
    ...
    constructor(address _uniswapV2Pair, address _uniswapV2Router, address _usdt, address _n, address _erc1155) public{
    ...
    function addLiquidity(uint256 tokenAmount, uint256 usdtAmount, address _referrer) public {
        require(usdtAmount >= 100e6, "Referral: usdt must be more than 100");
        usdt.transfer(address(this), usdtAmount);
        n.transfer(address(this), tokenAmount);
        // approve token transfer to cover all possible scenarios
        usdt.approve(address(uniswapV2Router), tokenAmount);
        n.approve(address(uniswapV2Router), usdtAmount);
        ...
        if(referreeHasDeposit[msg.sender] != true){

            erc1155.mint(referrer[msg.sender], rewards[referralCount[referrer[msg.sender]]],1);
        }
        referreeHasDeposit[msg.sender] = true;
    }

    function addRewards(uint256[] memory _count, uint256[] memory _ids) public{
        require(_count.length == _ids.length, "Referral: address is not the same length as ids");
        for(uint256 i = 0 ; i < _count.length ; i++){
            rewards[_count[i]] = _ids[i];
        }
    }

    function _addRefereeToReferrer(address _referrer, address _referee) internal {
