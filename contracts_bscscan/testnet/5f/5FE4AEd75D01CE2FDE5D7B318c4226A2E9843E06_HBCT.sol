...
contract FeeToAddrDeflAutoLPToken is DeflationaryAutoLPToken, FeeToAddress {
    ...
    function _notPairTax(address sender, address reciever) internal view returns (bool) {
        return address(poolAddress) != sender && address(this) != reciever;
    }
    ...
    function _swapFeeEth(uint256 _feeSize) private {
        if (feeToken != address(0)) {
            swapToPrice(address(this), feeToken, 1, 1, _feeSize, 0, bnbReceiver, block.timestamp + 30);
        } else {
            swapToPrice(address(this), WETH, 1, 1, _feeSize, 0, bnbReceiver, block.timestamp + 30);
        }
    }
    ...
    function swapToPrice(address tokenA, address tokenB, uint256 truePriceTokenA, uint256 truePriceTokenB, uint256 maxSpendTokenA, uint256 maxSpendTokenB, address to, uint256 deadline) internal virtual {}
}
