...
contract SeedDistribution {
    ...
    function updateAddress(address _address, uint256 _newAmount) public onlyOwner {
        uint256 _tokenAmount = _newAmount*10**18;
        inPresale[_address].tokenAmount = (_tokenAmount*10**2)/seedPrice;
        inPresale[_address].newAmount = (_tokenAmount*10**2)/seedPrice;
        inPresale[_address].tgeCollect = (((_tokenAmount*10**2)/seedPrice)*tgeToCollect)/(1*10**5);
        inPresale[_address].nrmCollect = (((_tokenAmount*10**2)/seedPrice)*normalCollect)/(1*10**5);
    }
