...
contract MineralPool is Ownable {
    ...
    function getAccountTokenIdByIndex(address account ,uint256 index) public view returns (uint256) {
		return accountsTokens[account].tokens[index];
	}
    ...
    function calcCapacity(uint256 tokenId) public view returns (uint256) {
        ...
        uint256 ownerLength = IERC721(accountContract).balanceOf(ownerAccount);
        require(ownerLength > 0, "Calc capacity: owner has not registered");
        
        return _calcCapacity(ownerAccount, ownerLength, tokenId);
    }
    ...
    function estimatePerSecondsRewards(uint256 tokenId) public view returns (uint256) {
        ...
        if(isAccountStakeToken(ownerAccount, tokenId)){
            perSecondsRewards = calcPerSecondsRewards(tokenId);
        } else {
    ...
    function stake(uint256 tokenId) public updateRewards(tokenId) {
        ...
        uint256 ownerLength = IERC721(accountContract).balanceOf(msg.sender);
        require(ownerLength > 0,"Stake failed, you has not registered");
        ...
        uint256 capacity = _calcCapacity(ownerAccount, ownerLength, tokenId);
    ...
    function withdrawAllAndClaim() public {
        ...
        delete accountsTokens[msg.sender].tokens;
    }

    function _calcCapacity(address ownerAccount, uint256 ownerLength, uint256 tokenId) private view returns (uint256) {
        uint256 ownerTokenId = IERC721Enumerable(accountContract).tokenOfOwnerByIndex(ownerAccount, ownerLength - 1);
        AccountInfo memory ownerAccountInfo = IAccount(accountContract).getAccountInfo(ownerTokenId);
        bool isMatchPlace = false;
        if(ownerAccountInfo.birthPlace == poolPlace){
            isMatchPlace = true;
        }
    ...
    function _removeToken(address account, uint256 tokenId) private {
        ...
        uint256 arrayLength = accountsTokens[account].tokens.length;
        ...
        for(uint256 idx = tokenIndex; idx < arrayLength - 1; idx++){
