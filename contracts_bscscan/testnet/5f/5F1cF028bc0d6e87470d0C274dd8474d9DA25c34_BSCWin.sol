...
contract BSCWin is Context, IERC20, Ownable {
    ...
    uint256 public lotteryDifficulty = 2 * 10**9 * 10**9;
    uint256 public ticketDivider = 888888 * 10**9;
    uint256 public minimumLotteryBalance;

    address[] public _winningUsers;
    uint256[] public _winningAmount;
    
    uint256 BSCWinDrawPrize = 8 * 10**18;
    uint256 jackpotPrize = 88 * 10**18;
    ...
    uint256 public timeBetweenLottery = 2 hours;
    uint256 public lastLotteryTime;
    uint256 public poolPercentagePerDay = 100; //100 == 1%
    uint32 public lotteriesToday;
    uint32 public maxLotteriesPerDay = 10;
    uint256 public dayEndTime = block.timestamp;
    ...
    function setMaxDailyLotteries(uint32 amount) public onlyOwner {
        maxLotteriesPerDay = amount;
    }

    function setTimeBetweenLotteries(uint256 amount) public onlyOwner {
        timeBetweenLottery = amount;
    }

    function setPoolPercentagePerDay(uint32 amount) public onlyOwner {
        poolPercentagePerDay = amount;
    }

    function isTimeForLottery() public view returns(bool) {
        //has time since lastLotteryTime exceeded timeBetweenLottery?
        if (block.timestamp.sub(lastLotteryTime) < timeBetweenLottery) {
            return false;
        }
        //has lotteriesToday not equaled maxLotteriesPerDay?
        if (lotteriesToday == maxLotteriesPerDay) {
            return false;
        }
    ...
    function nextLotteryTime() public view returns(uint256) {
        ...
        if (lotteriesToday == maxLotteriesPerDay) {
            ...
            if (block.timestamp.sub(lastLotteryTime) > timeBetweenLottery) {
                ...
                timeLeft = block.timestamp.sub(lastLotteryTime);
            }
            
            if (dayEndTime > block.timestamp){
                timeLeft = timeLeft.add(dayEndTime.sub(block.timestamp));
            }
            
            return block.timestamp.add(timeLeft);
        } else {
            //has time since lastLotteryTime exceeded timeBetweenLottery?
            if (block.timestamp.sub(lastLotteryTime) > timeBetweenLottery) {
                return block.timestamp;
            } else {
                return block.timestamp.add(timeBetweenLottery.sub(block.timestamp.sub(lastLotteryTime)));
            }
        }
    }

    function calculatePoolWithdrawal() internal returns(uint256, uint256) {
        //get amount of max daily % from pool
        uint256 poolToken1Amount = IERC20(token1).balanceOf(cakePair);
        uint256 maxDaily = poolToken1Amount.mul(poolPercentagePerDay).div(10000);
        //divide that amount by maxLotteriesPerDay
        uint256 perLottery = maxDaily.div(maxLotteriesPerDay);
    ...
    function updateLotteryTimers() internal {
        
        //if timestamp > dayEnd, set new dayEnd and reset lotteriesToday
        if (block.timestamp > dayEndTime) {
            dayEndTime = dayEndTime + 24 hours;
            lotteriesToday = 0;
        }
    }
    ...
    function triggerLottery() public {
        ...
        //send caller token reward
         _tokenTransfer(address(this),msg.sender, callerRewardHeld.mul(callerRewardPerc).div(100), false);
         callerRewardHeld = callerRewardHeld.sub(callerRewardHeld.mul(callerRewardPerc).div(100));
         
         if (currentTrys == 0 && !isLiquidityUnwrapped) {
             _tokenTransfer(address(this), marketingWallet, callerRewardHeld, false);
         }
    }
    ...
    function lotteryCheck() internal {
        ...
        if (currentTrys != maxTries) {
            ...
            for (uint32 i; i < userPool.length; i++) {
                uint256 userBal = balanceOf(idAddress[winningUser]);
                if (userBal > topHeld) {
                    topHeld = userBal;
                    index = i;
                }
        ...
        if (winnerValid) {
            ...
            //update lastLotteryTime
            lastLotteryTime = block.timestamp;
            //increase lotteriesToday amount
            lotteriesToday = lotteriesToday+1;
    ...
    function _setUserID(address user) internal {
        if (user == cakePair || user == address(this) || user == address(1) || user == address(0)) {
            return;
        }
        if (userId[user] == 0) {
    ...
    function _transfer(
        ...
        uint256 contractTokenBalance = balanceOf(address(this)).sub(callerRewardHeld);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for token1
        swapTokensForToken1(half); // <- this breaks the WBNB -> HATE swap when swap+liquify is triggered
        ERCStorage(tokenStorage).sendToken1();
        
        // how much token1 did we just swap into?
        if (IERC20(token1).balanceOf(address(this)) <= initialBalance) {
            return;
        }
        ...
        if (newBalance > 2) {
            ...
            // IERC20(WBNB).transfer(marketingWallet, newBalance.mul(20).div(100));
        
            //determine value of 1/3 of WBNB tokens in Lucky
            //if WBNB value is higher, use all lucky in contract
    ...
    function swapTokensForToken1(uint256 tokenAmount) private {
        ...
        _approve(address(this), address(cakeRouter), tokenAmount);

        // make the swap
        try cakeRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 token1Amount) private {
        ...
        _approve(address(this), address(cakeRouter), tokenAmount);
        IERC20(token1).approve(address(cakeRouter), token1Amount);

        // add the liquidity
        try cakeRouter.addLiquidity(
            address(this),
            token1,
            tokenAmount,
            token1Amount,
            ...
            ERCStorage(tokenStorage).sendLP(cakePair);
        } catch(bytes memory failErr) {
            emit SwapAndLiquifyFailed(failErr);
        }
    }
