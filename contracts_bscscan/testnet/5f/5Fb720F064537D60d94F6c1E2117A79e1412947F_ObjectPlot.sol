...
contract ObjectPlot is ERC721URIStorage, CoinFactoryAdminRole {
    ...
    uint256[][] xySize;
    mapping(bytes32 => uint256) public xySizeUse;
    ...
    function makePlotNFT(address sender, string memory name, string memory image_suffix, uint256 c_x, uint256 c_y, uint256 block_width, uint256 block_height) public onlyCoinFactoryAdmin {
        ...
        // ################ 记载所属列表
        if (privateGlobalUint[5] == 1) {
            bytes32 sign = sha256(abi.encodePacked(c_x, c_y));
            require(xySizeUse[sign] == 0, "dev : xy used");
            xySizeUse[sign] = 1;
    ...
    //得到坐标参数
    function getLastXYParam() public returns (uint256[] memory, uint256, uint256, uint256){
        return (xySize[xySize.length - 1], xySize.length, privateGlobalUint[3], privateGlobalUint[4]);
    }
