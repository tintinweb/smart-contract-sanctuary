 ...
 *                                                   
 *
 * Fuck high taxes and fuck earning back your own fees the only taxes are for LP to stabilize the price.
 * - Supply:            1000 SUPER RARE
 * - Max wallet:        40 FREE (4%)  
 * - Max TX:            20 FREE (2%)
 * - Sell cooldown:     2 MINUTES (!)
 * - Buy cooldown:      8 SECONDS
 * - Fees:              8% liquidity // 0% reflections // 0% burn // 0% dev
 ...
 * COIN
 */
contract text3 is Context, IERC20, Ownable, ReentrancyGuard {
	...
	// @note: Pancake V2 testnet 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
    ...
    uint256 private _tTotal = 1111 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    // Max TX in tokens
    uint256 public _maxSellTxAmount = 20 * 10**9;
    uint256 public _maxBuyTxAmount = 20 * 10**9;
    ...
    uint256 private maxTotalFee = 40;
    ...
    function getAllBuyFees() private view returns (uint256 _liquidityFeeBuy, uint256 _burnFeeBuy, uint256 _rewardFeeBuy, uint256 _devFeeBuy){
        return (liquidityFeeBuy, burnFeeBuy, rewardFeeBuy, devFeeBuy);
    }

    // Get all the sell fees
    function getAllSellFees() private view returns (uint256 _liquidityFeeSell, uint256 _burnFeeSell, uint256 _rewardFeeSell, uint256 _devFeeSell){
        return (liquidityFeeSell, burnFeeSell, rewardFeeSell, devFeeSell);
    }

    // Get HODLr rewards
    function totalHODLrRewards() private view returns (uint256) {
        return _tHODLrRewardsTotal;
    }

    // Get amount of tokens burned
    function totalBurned() private view returns (uint256) {
    ...
    */
    ...
    // Resets the sell lock for the address calling it
    ...
    // TODO: We should find a contract using separate taxes (liquidity/reflection and burn) for 
    // buying and selling.
