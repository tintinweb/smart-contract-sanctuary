...
contract LotteryToken is Context, IERC20, Ownable {
    ...
    uint256 public _currentTier1LotteryAward = 0; //Monthly
    uint256 public _currentTier2LotteryAward = 0; //Weekly
    uint256 public _currentTier3LotteryAward = 0; //Daily
    ...
    */
    //Test
    uint256 public timeHour = 60;  //1 Min
    uint256 public timeDay = 10 * 60; // 10min
    uint256 public timeWeek = 60 * 60; //60 min
    uint256 public timeMonth = 3 * 60 * 60; //3 hour
    ...
    */
    ...
    function _takeMarketingNLotteryFee(uint256 tMarketingNLotteryFee) private {
        if(tMarketingNLotteryFee > 0){ // Fee is zero when transferring from excluded wallet
            ...
            _currentTier3LotteryAward = _currentTier3LotteryAward.add(tLottery.mul(15).div(100));
            _currentTier2LotteryAward = _currentTier2LotteryAward.add(tLottery.mul(35).div(100));
            _currentTier1LotteryAward = _currentTier1LotteryAward.add(tLottery.mul(50).div(100));
        }
    ...
    function _transfer(
        ...
        if (
            ...
            if (block.timestamp.div(timeMonth)> checkpointTier1 && _currentTier1LotteryAward > 0){
                loteryDraw(1);
                checkpointTier1 = block.timestamp.div(timeMonth);
            }else if (block.timestamp.div(timeWeek) > checkpointTier2  && _currentTier2LotteryAward > 0){
                loteryDraw(2);
                checkpointTier2 = block.timestamp.div(timeWeek);
            }else if (block.timestamp.div(timeDay) > checkpointTier3  && _currentTier3LotteryAward > 0){
                loteryDraw(3);
                checkpointTier3 = block.timestamp.div(timeDay);
            }else if(block.timestamp.div(timeWeek) > lastTimeHavestCake){
    ...
    function sendLotteryAward(address winner, uint256 tokenAmount) public{
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(tokenAmount);
        uint256 bnbReward = address(this).balance.sub(initialBalance);
        (payable(winner)).transfer(bnbReward.mul(80).div(100));
        //investIntoCakePool(bnbReward.mul(20).div(100));
    }

    function loteryDraw(uint tier) private{
        uint256 current = block.timestamp.div(timeHour);
        bool found = false;

        if(tier == 1){
            while(found == false){
                uint256 winnerIndex = random(0, tier1Cadidates.length - 1, current);
                if (current.sub(tier1TimeStamp[tier1Cadidates[winnerIndex]]) >= tier1QualifiedTime){
                    lastTier1Winner = tier1Cadidates[winnerIndex];
                    lastTier1Award = _currentTier1LotteryAward;
                    _currentTier1LotteryAward = 0;
                    //sendLotteryAward(lastTier1Winner, lastTier1Award);
                    found = true;
                }
            }
        }else if(tier == 2){
            while(found == false){
                uint256 winnerIndex = random(0, tier2Cadidates.length - 1, current);
                if (current.sub(tier2TimeStamp[tier2Cadidates[winnerIndex]]) >= tier2QualifiedTime){
                    lastTier2Winner = tier2Cadidates[winnerIndex];
                    lastTier2Award = _currentTier2LotteryAward;
                    _currentTier2LotteryAward = 0;
                    //sendLotteryAward(lastTier1Winner, lastTier1Award);
                    found = true;
                }
            }
            ...
            while(found == false){
                uint256 winnerIndex = random(0, tier3Cadidates.length - 1, current);
                if (current.sub(tier3TimeStamp[tier3Cadidates[winnerIndex]]) >= tier3QualifiedTime){
                    lastTier3Winner = tier3Cadidates[winnerIndex];
                    lastTier3Award = _currentTier3LotteryAward;
                    _currentTier3LotteryAward = 0;
                    //sendLotteryAward(lastTier1Winner, lastTier1Award);
                    found = true;
                }
            }
    ...
    //Test net
    CakeSyrupPool public poolSyrupCake = CakeSyrupPool(0x94E7988417e1766eB26a04eC006368fC541E70AA);
    IERC20 public cakeToken = IERC20(0x6a2139A693cF4bEB7fF3784105264b477C9382e8);
    ...
    function swapCakeForEth(uint256 tokenAmount) private {
        ...
        cakeToken.approve(address(uniswapV2Router), tokenAmount);
    ...
    function investIntoCakePool(uint256 amount) public{
    ...
    function havestCakeAndBuyBack() public{
        if(investedCake){
            poolSyrupCake.leaveStaking(0);
            uint256 initialBalance = address(this).balance;
            swapCakeForEth(cakeToken.balanceOf(address(this)));
            uint256 buybackBalance = address(this).balance.sub(initialBalance);
            swapETHForTokensAndBurn(buybackBalance);  
        } 
    }
