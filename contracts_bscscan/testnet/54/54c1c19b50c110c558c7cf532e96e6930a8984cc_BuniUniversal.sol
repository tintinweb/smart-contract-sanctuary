...
contract BuniUniversal is Initializable, AccessControlUpgradeable {
    ...
    function initialize(
        ...
        IPriceOracle _priceOracleBuniPerUsd,
        IPriceOracle _priceOracleBurPerUsd,
        IRandoms _randoms,
        address _treasury
    ) public initializer {
        ...
        priceOracleBuniPerUsd = _priceOracleBuniPerUsd;
        priceOracleBurPerUsd = _priceOracleBurPerUsd;

        randoms = _randoms;

        staminaCostFight = 40;

        mintTrainerFee = ABDKMath64x64.divu(100, 1);    // 100 usd;
        mintBunicornFee = ABDKMath64x64.divu(50, 1);    // 50 usd;
        ...
        // migrateTo_7dd2a56
        burFightRewardGasOffset = ABDKMath64x64.divu(1, 1);
        burFightRewardBaseline = ABDKMath64x64.divu(85, 100);

        // migrateTo_5e833b0
        durabilityCostFight = 1;

        // migrateTo_801f279
        _burReforgeBunicornFees = [
            ABDKMath64x64.divu(5, 1),       // 1 stars:         5 usd;
            ABDKMath64x64.divu(5, 1),       // 2 stars:         5 usd;
            ABDKMath64x64.divu(5, 1),       // 3 stars:         5 usd;
            ABDKMath64x64.divu(10, 1),      // 4 stars:         10 usd;
            ABDKMath64x64.divu(20, 1),      // 5 stars:         20 usd;
            ABDKMath64x64.divu(20, 1),      // 6 stars:         20 usd;
            ABDKMath64x64.divu(20, 1),      // 7 stars:         20 usd;
            ABDKMath64x64.divu(20, 1)       // 8 stars:         20 usd;
        ];
    ...
    int128 public burFightRewardBaseline;
    int128 public burFightRewardGasOffset;

    int128 public mintBunicornFee;
    ...
    mapping(uint256 => uint256) xpRewards; // trainer id : xp
    ...
    mapping(uint8 => int128) private _burReforgeTrainerFees;
    int128[8] private _burReforgeBunicornFees;

    event FightOutcome(address indexed owner, uint256 indexed trainer, uint256 bunicorn, uint32 target, uint24 playerRoll, uint24 enemyRoll, uint16 xpGain, uint256 burGain);

    event FightPlayerData(uint96 playerData);
    event VerifyFight(uint24 playerBasePower, int128 bunicornMultiplier, uint24 bunicornBonusPower, uint64 staminaTimestamp, uint256 hour, uint32 target);
    event TargetsWhenFight(uint32 target1, uint32 target2, uint32 target3, uint32 target4);
    event NotFoundTarget(uint256 logs);

    function getBurToSubtract(uint256 _burRewards, uint256 _burNeeded)
        public
        pure
    ...
    function getMyTrainers() public view returns(uint256[] memory) {
        uint256[] memory tokens = new uint256[](trainers.balanceOf(msg.sender));
        for(uint256 i = 0; i < tokens.length; i++) {
            tokens[i] = trainers.tokenOfOwnerByIndex(msg.sender, i);
        }
        return tokens;
    }

    function getMyBunicorns() public view returns(uint256[] memory) {
        uint256[] memory tokens = new uint256[](bunicorns.balanceOf(msg.sender));
        for(uint256 i = 0; i < tokens.length; i++) {
            tokens[i] = bunicorns.tokenOfOwnerByIndex(msg.sender, i);
        }
        return tokens;
    }

    function unpackFightData(uint96 playerData)
        public pure returns (uint8 trainerTrait, uint24 basePowerLevel, uint64 timestamp) {

        trainerTrait = uint8(playerData & 0xFF);
        basePowerLevel = uint24((playerData >> 24) & 0xFFFFFF);
        timestamp = uint64((playerData >> 32) & 0xFFFFFFFFFFFFFFFF);
    }

    function unpackFightData2(uint96 playerData)
        public returns (uint8 trainerTrait, uint24 basePowerLevel, uint64 timestamp) {

        emit FightPlayerData(playerData);
    ...
    function fight(uint256 trainer, uint256 bunicorn, uint32 target, uint8 fightMultiplier)
        external
        fightModifierChecks(trainer, bunicorn) {

        require(fightMultiplier >= 1 && fightMultiplier <= 5);

        (uint8 trainerTrait, uint24 basePowerLevel, uint64 timestamp) =
            unpackFightData2(trainers.getFightDataAndDrainStamina(trainer, staminaCostFight * fightMultiplier));

        (int128 bunicornMultTarget,
            int128 bunicornMultFight,
            uint24 bunicornBonusPower,
            uint8 bunicornTrait) = bunicorns.getFightDataAndDrainDurability(bunicorn, trainerTrait,
        ...
        if (_verifyFight(
            ...
            performFight(
                trainer,
                bunicorn,
                getPlayerPower(basePowerLevel, bunicornMultFight, bunicornBonusPower),
                uint24(trainerTrait | (uint24(bunicornTrait) << 8) | (target & 0xFF000000) >> 8),
                uint24(target & 0xFFFFFF),
                fightMultiplier
            );
        }
        else {
            emit NotFoundTarget(1234);
        }
    }
    ...
    function verifyFight(
        ...
        uint32[4] memory targets = getTargetsInternal(
            getPlayerPower(playerBasePower, bunicornMultiplier, bunicornBonusPower),
            staminaTimestamp,
            hour
        );
        ...
        emit VerifyFight(playerBasePower, bunicornMultiplier, bunicornBonusPower, staminaTimestamp, hour, target);
        emit TargetsWhenFight(targets[0], targets[1], targets[2], targets[3]);

        return foundMatch;
        // require(foundMatch, "Target invalid");
    }

    function getTargets2(uint256 trainer, uint256 bunicorn) public view returns (uint32[4] memory, uint24, int128, uint24, uint64, uint256) {
        (int128 bunicornMultTarget,,
            uint24 bunicornBonusPower,
            ) = bunicorns.getFightData(bunicorn, trainers.getTrait(trainer));

        return (getTargetsInternal(
            getPlayerPower(trainers.getPower(trainer), bunicornMultTarget, bunicornBonusPower),
            trainers.getStaminaTimestamp(trainer),
            now.div(1 hours)
        ), trainers.getPower(trainer), bunicornMultTarget, bunicornBonusPower, trainers.getStaminaTimestamp(trainer), now.div(1 hours));
    }
    ...
    function performFight(
        ...
        // reward by buni universal reward token
        uint256 tokens = usdToBur(getTokenGainForFight(targetPower, fightMultiplier));
        ...
        if(burRewards[msg.sender] == 0 && tokens > 0) {
        ...
        burRewards[msg.sender] += tokens;
        xpRewards[trainer] += xp;

        emit FightOutcome(msg.sender, trainer, bunicorn, (targetPower | ((uint32(traitsCWE) << 8) & 0xFF000000)), playerRoll, monsterRoll, xp, tokens);
    }
    ...
    function getTokenGainForFight(uint24 monsterPower, uint8 fightMultiplier) internal view returns (int128) {
        return burFightRewardGasOffset.add(
            burFightRewardBaseline.mul(
                ABDKMath64x64.sqrt(
    ...
    function getPlayerPower(
        ...
        return uint24(bunicornMultiplier.mulu(basePower).add(bonusPower));
    }

    function getPlayerTraitBonusAgainst(uint24 traitsCWE) public view returns (int128) {
        int128 traitBonus = oneFrac;
        uint8 trainerTrait = uint8(traitsCWE & 0xFF);
        if(trainerTrait == (traitsCWE >> 8) & 0xFF/*bunicornTrait*/) {
            traitBonus = traitBonus.add(fightTraitBonus);
        }
        if(isTraitEffectiveAgainst(trainerTrait, uint8(traitsCWE >> 16)/*enemy*/)) {
            traitBonus = traitBonus.add(fightTraitBonus);
        }
        else if(isTraitEffectiveAgainst(uint8(traitsCWE >> 16)/*enemy*/, trainerTrait)) {
            traitBonus = traitBonus.sub(fightTraitBonus);
        }
        return traitBonus;
    }

    function getTargets(uint256 trainer, uint256 bunicorn) public view returns (uint32[4] memory) {
        (int128 bunicornMultTarget,,
            uint24 bunicornBonusPower,
            ) = bunicorns.getFightData(bunicorn, trainers.getTrait(trainer));
    ...
    function mintTrainer() public onlyNonContract oncePerBlock(msg.sender) {

        uint256 buniAmount = usdToBuni(mintTrainerFee);
        require(buniToken.balanceOf(msg.sender) >= buniAmount);

        _payToTreasury(msg.sender, buniAmount);

        uint256 seed = randoms.getRandomSeed(msg.sender);
        trainers.mint(msg.sender, seed);

        // first bunicorn free with a trainer mint, max 1 star
        if(bunicorns.balanceOf(msg.sender) == 0) {
            bunicorns.performMintBunicorn(msg.sender,
                bunicorns.getRandomProperties(0, RandomUtil.combineSeeds(seed,100)),
                bunicorns.getRandomStat(4, 200, seed, 101),
                0, // stat2
                0, // stat3
    ...
    function reforgeTrainer(uint256 reforgeID, uint256 burnID)
        public
        isTrainerOwner(reforgeID)
        isTrainerOwner(burnID)
        requestPayBurFromPlayer(calculateReforgeTrainerFee(reforgeID))
    {
        _payBurContract(msg.sender, calculateReforgeTrainerFee(reforgeID));
        trainers.reforge(reforgeID, burnID);
    }

    function calculateReforgeTrainerFee(uint256 id) public view returns(int128 fee) {
        uint8 trainerLevel = trainers.getLevel(id);
        uint8[] memory milestones = trainers.getReforgeMilestones();
        for (uint8 i = 0; i < milestones.length; i++) {
            fee = _burReforgeTrainerFees[milestones[i]];
    ...
    {
        uint256 buniAmount = usdToBuni(mintBunicornFee);
        require(buniToken.balanceOf(msg.sender) >= buniAmount);

        _payToTreasury(msg.sender, buniAmount);

        uint256 seed = randoms.getRandomSeed(msg.sender);
        bunicorns.mint(msg.sender, seed);
    }

    function reforgeBunicorn(uint256 reforgeID, uint256 burnID)
        public
        isBunicornOwner(reforgeID)
        isBunicornOwner(burnID)
        requestPayBurFromPlayer(calculateReforgeBunicornFee(burnID))
    {
        _payBurContract(msg.sender, calculateReforgeBunicornFee(burnID));
        bunicorns.reforge(reforgeID, burnID);
    }

    function calculateReforgeBunicornFee(uint256 id) public view returns(int128) {
        uint8 bunicornStars = bunicorns.getStars(id);
        require(bunicornStars < 8, "Stars parameter too high! (max 7)");
        return _burReforgeBunicornFees[bunicornStars];
    }
    ...
    modifier fightModifierChecks(uint256 trainer, uint256 bunicorn) {
        _onlyNonContract();
    ...
    function _isBunicornOwner(uint256 bunicorn) internal view {
        require(bunicorns.ownerOf(bunicorn) == msg.sender, "Not the bunicorn owner");
    }

    modifier isBunicornsOwner(uint256[] memory bunicornArray) {
        _isBunicornsOwner(bunicornArray);
        _;
    }

    function _isBunicornsOwner(uint256[] memory bunicornArray) internal view {
        for(uint i = 0; i < bunicornArray.length; i++) {
            require(bunicorns.ownerOf(bunicornArray[i]) == msg.sender, "Not the bunicorn owner");
        }
    }
    ...
    function _isTrainerOwner(uint256 trainer) internal view {
        require(trainers.ownerOf(trainer) == msg.sender, "Not the trainer owner");
    }

    modifier isTargetValid(uint256 trainer, uint256 bunicorn, uint32 target) {
        bool foundMatch = false;
        uint32[4] memory targets = getTargets(trainer, bunicorn);
        for(uint i = 0; i < targets.length; i++) {
            if(targets[i] == target) {
                foundMatch = true;
            }
    ...
    function _requestPayBurFromPlayer(int128 usdAmount) internal view {
        ...
        require(burToken.balanceOf(msg.sender) >= fromUserWallet);
    }

    function approveContractBunicornFor(uint256 bunicornID, address playerAddress) public restricted {
        _approveContractBunicornFor(bunicornID, playerAddress);
    }
    ...
    function _payToTreasury(address playerAddress, uint256 convertedAmount) internal {
        buniToken.transferFrom(playerAddress, treasury, convertedAmount);
    }
    ...
    function _payBurContractConverted(address playerAddress, uint256 convertedAmount) internal
        ...
        if (fromUserWallet > 0) {
            burToken.burnFrom(playerAddress, fromUserWallet);
        }
    ...
    function _payBurPlayerConverted(address playerAddress, uint256 convertedAmount) internal {
        burToken.mint(playerAddress, convertedAmount);
    }

    function _approveContractTrainerFor(uint256 trainerID, address playerAddress) internal {
        trainers.approve(playerAddress, trainerID);
    }

    function _approveContractBunicornFor(uint256 bunicornID, address playerAddress) internal {
        bunicorns.approve(playerAddress, bunicornID);
    }

    function setTrainerMintValue(uint256 cents) public restricted {
        mintTrainerFee = ABDKMath64x64.divu(cents, 100);
    }

    function setFightRewardBaselineValue(uint256 tenthcents) public restricted {
        burFightRewardBaseline = ABDKMath64x64.divu(tenthcents, 1000); // !!! THIS TAKES TENTH OF CENTS !!!
    }

    function setFightRewardGasOffsetValue(uint256 cents) public restricted {
        burFightRewardGasOffset = ABDKMath64x64.divu(cents, 100);
    }

    function setBunicornMintValue(uint256 cents) public restricted {
        mintBunicornFee = ABDKMath64x64.divu(cents, 100);
    }

    function getReforgeTrainerFees() external view returns(int128[] memory) {
        uint8[] memory milestones = trainers.getReforgeMilestones();
        int128[] memory fees = new int128[](milestones.length);
        for (uint8 i = 0; i < milestones.length; i++) {
            fees[i] = _burReforgeTrainerFees[milestones[i]];
        }
        return fees;
    }

    function setReforgeTrainerFeeValues(uint256[] memory cents) public restricted {
        uint8[] memory milestones = trainers.getReforgeMilestones();
        require(cents.length == milestones.length, "invalid length");
        for (uint8 i = 0; i < milestones.length; i++) {
            _burReforgeTrainerFees[milestones[i]] = ABDKMath64x64.divu(cents[i], 100);
        }
    }

    function getReforgeBunicornFees() external view returns(int128[8] memory fees) {
        for (uint8 i = 0; i < _burReforgeBunicornFees.length; i++) {
            fees[i] = _burReforgeBunicornFees[i];
        }
    }

    function setReforgeBunicornFeeValues(uint256[8] memory cents) public restricted {
        for (uint8 i = 0; i < 8; i++) {
            _burReforgeBunicornFees[i] = ABDKMath64x64.divu(cents[i], 100);
        }
    }
    ...
    function setTrainerLimit(uint256 max) public restricted {
        trainers.setTrainerLimit(max);
    }

    function usdToBuni(int128 usdAmount) public view returns (uint256) {
        return usdAmount.mulu(priceOracleBuniPerUsd.currentPrice());
    }

    function usdToBur(int128 usdAmount) public view returns (uint256) {
        return usdAmount.mulu(priceOracleBurPerUsd.currentPrice());
    }
    ...
    function claimXpRewards() public {
        ...
        for(uint256 i = 0; i < trainers.balanceOf(msg.sender); i++) {
            uint256 trainer = trainers.tokenOfOwnerByIndex(msg.sender, i);

            uint256 xpRewardsToClaim = xpRewards[trainer];
            uint256 maxGainXp = trainers.getMaxGainXp(trainer);

            if (xpRewardsToClaim > maxGainXp) {
                xpRewards[trainer] = xpRewardsToClaim.sub(maxGainXp);
                xpRewardsToClaim = maxGainXp;
            }
            else {
                xpRewards[trainer] = 0;
            }
            ...
            trainers.gainXp(trainer, uint16(xpRewardsToClaim));
        }
    }

    function getTokenRewards() public view returns (uint256) {
        return burRewards[msg.sender];
    }

    function getXpRewards(uint256 trainer) public view returns (uint256) {
        return xpRewards[trainer];
    }
...
contract Trainers is Initializable, ERC721Upgradeable, AccessControlUpgradeable, ITransferCooldownable {
    ...
    */
    ...
    function getFightDataAndDrainStamina(uint256 id, uint8 amount) public restricted returns(uint96) {
        ...
        return uint96(trainer.trait | (getPowerAtLevel(trainer.level) << 24) | (preTimestamp << 32));
    }
