...
contract NurseryManagement is Nursery {
    ...
    uint256[4] public maturities = [3 days,7 days,15 days,30 days];
   ...
   struct MintedData {
    ...
    mapping(uint256 => MintedData) _tempCounter;
    ...
    function setMaturityDurations(uint256 _index, uint256 _numberOfDays) external onlyOwner{
        ...
        maturities[_index]= _numberOfDays.mul(1 days); 
    }
    
    function _canMint(uint256 _tokenId) internal view returns (bool) {
        EggsPrices memory e = eggsPrices[_tokenId];
        bool result;
        if (
    ...
    function _updateNextMint(uint256 _tokenId, uint256 state) internal {
        ...
        MintedData storage temp = _tempCounter[_tokenId];
        MintedData storage total = nurseryMintedDatas[_tokenId];
        uint256 _next;

        if(temp.bronzeMinted < 5){
            temp.bronzeMinted = temp.bronzeMinted.add(1);
            total.bronzeMinted = total.bronzeMinted.add(1);
            if(temp.bronzeMinted == 5){
                _next = 1;
                nextUnlock[_tokenId] = block.timestamp.add(1 days);
            }
            ...
            temp.silverMinted = temp.silverMinted.add(1);
            total.silverMinted = total.silverMinted.add(1);
            temp.bronzeMinted = 0;

        }else if(temp.goldMinted < 5){
            temp.goldMinted = temp.goldMinted.add(1);
            total.goldMinted = total.goldMinted.add(1);
            temp.bronzeMinted = 0;
            temp.silverMinted = 0;
        }else {
            temp.platinumMinted = temp.platinumMinted.add(1);
            total.platinumMinted = total.platinumMinted.add(1);
            temp.bronzeMinted = 0;
            temp.silverMinted = 0;
            temp.goldMinted = 0;
        }
    ...
    function setPrice(
        ...
        EggsPrices storage e = eggsPrices[_tokenId];
    ...
    function buyEgg(uint256 _tokenId, uint256 state) external returns(uint256){
        ...
        require(BZAI.transferFrom(
            msg.sender,
            _paymentContract,
            _price
        ));

        nurseryRevenues[_tokenId] = nurseryRevenues[_tokenId].add(_price);

        IPayments(_paymentContract).payOwner(ownerOf(_tokenId), _price);

        return IEggs(gameAddresses.getEggsAddress()).mintEgg(msg.sender, state, block.timestamp.add(maturities[state]));
    }
...
contract Nursery is ERC721, Ownable {
    ...
    constructor() ERC721("Nursery_NFT", "NURS") {}
    ...
    function mintNursery(address _to)
        external
        onlyAuth
        returns (uint256)
    {
