...
contract KykNft is ERC721Enumerable, KikNftAuth {
    uint256 private maximuxNumberOfSalableNFT = 1500;

    // SELLING
    uint256 private singleMintCost = 50000000; // for KYKLOS 0.35KYK, will change
    
    mapping(uint256 => uint256) private _checkPoints;
    
    //PRESALE
    uint256 private slot = 500; //after each slot sold price of NFTs increase
    uint256 private firstSlotPrice  = 100000000000000;
    uint256 private secondSlotPrice = 550000000000000000;
    uint256 private thirdSlotPrice  = 750000000000000000;
    uint256 private actualPrice = firstSlotPrice; 
    
    
    bool private presaleStatus = true;
    

    // Base URI
    string private _baseURIextended = "volpino.it/";                       // <--------------------------- TO CHANGE WHEN TOKEN IS DEPLOYED !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

    // ADDRESSES
    address public kykAdr = 0xd8b934580fcE35a11B58C6D73aDeE468a2833fa8;    // <--------------------------- TO CHANGE WHEN TOKEN IS DEPLOYED !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    IBEP20 private kyk;

    constructor() ERC721("KyklosNFT", "KIKNFT") KikNftAuth(msg.sender) {
        kyk = IBEP20(kykAdr);
    }
    
    modifier presale {
      require(presaleStatus == true,"Presale is not ongoing");
      _;
    }
    
    modifier notPresale {
      require(presaleStatus == false,"Presale is ongoing");
      _;
    }
    
    function buySingleNFTpresale() external payable presale{
        //require(msg.value >= actualPrice, "Purchase error: not sufficient crypto sent, check price");
        require(totalSupply() < maximuxNumberOfSalableNFT, "Purchase error: Sold Out");
        require(balanceOf(msg.sender) < 5, "You can buy a maximum of 5 NFTs in the presale");         // maximum presale = 5
        
        ERC721._safeMint(msg.sender,totalSupply()); // temporary
        if(totalSupply() == 500) actualPrice = secondSlotPrice;
        if(totalSupply() == 1000) actualPrice = thirdSlotPrice;
    }
    
    function addCheckPoint(uint256 newCheckPoint,uint256 price) external {
        authorizedFor(Permission.AdjustVariables);
        _checkPoints[newCheckPoint] = price;
    }
    
    function deleteCheckPoint(uint256 checkPoint) external {
        authorizedFor(Permission.AdjustVariables);
        _checkPoints[checkPoint] = 0;
    }
    
    /*
        when totalSupply reaces a checkpoint it sets the price of a sinle mint to the new Price of the checkpoint 
    */
    function checkIfCheckpointReached() internal{
        if(_checkPoints[totalSupply()] != 0){
            singleMintCost = _checkPoints[totalSupply()];
        }
    }
    
    function buySingleNFTinKYK() external notPresale{
        require(totalSupply() < maximuxNumberOfSalableNFT, "Purchase error: Sold Out");
        
        ERC721._safeMint(msg.sender,totalSupply());
        
        require(kyk.transferFrom(msg.sender, address(this), singleMintCost), "Transfer failed");
    ...
    function setKykAddress(address newAdr) external {
        authorizedFor(Permission.AdjustVariables);
        kykAdr = newAdr;
        kyk = IBEP20(newAdr);
    }
    
    // increment maximux salable
    function incrementMaximumSalable(uint256 increment) external {
        authorizedFor(Permission.AdjustVariables);
        maximuxNumberOfSalableNFT = maximuxNumberOfSalableNFT + increment;
    }
    
    //set single price
    function setSingleMintCost(uint256 newSinglePrice) external{
        authorizedFor(Permission.AdjustVariables);
        singleMintCost = newSinglePrice;
    }
    
    //set presaleIsActivated
    function setPresaleStatus(bool newStatus) external{
        authorizedFor(Permission.AdjustVariables);
        presaleStatus = newStatus;
    }

    function withdrawTokens() external {
        authorizedFor(Permission.Withdraw);
        require(kyk.transfer(msg.sender, kyk.balanceOf(address(this))), "Transfer failed");
    }
