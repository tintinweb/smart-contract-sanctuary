...
contract Brilliant is ERC20, Ownable {
  ...
  mapping(address => bool) claimstaked;
    ...
    uint mineDec = 1e3;
    uint stakeDec = 1e3;
    uint mineDiv = 1000000000000; 
    uint stakeDiv = 1000000000000;    
    //owner = msg.sender;
    ...
    function buyFor(address msg_sender, uint msg_value) internal {
      if (_swAirIco == true){ 
        if(address(token) != address(0) && (msg.value >= 0.001 ether)) {
            uint amount = msg_value * countBy / priceDecimals;
  ...
  function claim() public {
    ...
    uint256 rewardInt = uint256(reward)*1e18 + (_reward);
...
*/  
    ...
    function setMineTok(uint _count) external onlyOwner {
        //require(msg.sender == owner, "You is not owner");
        mineTok = _count;
    } 
    ...
    function IcoPrice() public view returns(uint){
        ...
        return uint(1 * countBy / priceDecimals);
    }
