...
// File: contracts/NFTCharacters.sol
...
contract NFTCharacters is EIP712, ERC721Enumerable, Ownable {
    using Strings for uint256;

    bytes32 public constant PRESALE_TYPEHASH = keccak256("Presale(address buyer,uint256 maxCount)");

    uint256 public constant NFT_GIFT = 150;
    uint256 public constant NFT_PRIVATE = 8738;
    uint256 public constant NFT_MAX = NFT_GIFT + NFT_PRIVATE;
    ...
    constructor(
        ...
        ERC721("NFT Characters", "NFT")  
        EIP712("NFT Characters", "1.0.0") 
    {
    ...
    function presaleBuy(uint256 tokenQuantity, uint256 maxCount, bytes memory signature) external payable {
        ...
        require(totalSupply() < NFT_MAX, "All NFTs are minted");
        require(privateAmountMinted + tokenQuantity <= NFT_PRIVATE, "Minting would exceed the presale allocation");
        require(presalerListPurchases[msg.sender] + tokenQuantity <= maxCount, "You can not mint exceeds maximum NFT");
        require(NFT_PRICE * tokenQuantity <= msg.value, "Insufficient ETH sent");
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= NFT_MAX, "MAX_MINT");
        require(giftedAmount + receivers.length <= NFT_GIFT, "GIFTS_EMPTY");
