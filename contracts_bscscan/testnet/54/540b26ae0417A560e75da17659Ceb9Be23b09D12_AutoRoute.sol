...
contract AutoRoute is OwnableUpgradeable {
    ...
    event AmountOut(uint256[] amounts);
        ...
        returns (uint256[] memory amounts)
    {
        ...
        //     IERC20(paths[0]).transferFrom(msg.sender, address(this), ammAmountsIn[0]);
        //     uint256[] memory ammAmountIn = IEvryRouter2(ammRouterAddress).swapTokensForExactTokens(
        //         ammAmountsIn[ammAmountsIn.length - 1], 
        //         MAXUINT, 
        //         ammPaths, 
        //         address(this), 
        //         block.timestamp
        //     );
        //     uint256[] memory dmmAmountIn = IDMMRouter02(dmmRouterAddress).swapTokensForExactTokens(
        //         amountOut, 
        //         MAXUINT, 
        //         dmmPoolAddresses, 
        //         dmmPaths, 
        ...
        //     amounts = mergeAmount(ammAmountIn, dmmAmountIn);
        // } else {
        //     (uint256[] memory dmmAmountsIn,) = getAmountsInDmmAmm(amountOut, ammPaths, dmmPaths, dmmPoolAddresses);
        //     require(amountInMax >= dmmAmountsIn[0], "AutoRoute: EXCESSIVE_INPUT_AMOUNT");
        //     IERC20(paths[0]).transferFrom(msg.sender, address(this), dmmAmountsIn[0]);
        //     uint256[] memory dmmAmountIn = IDMMRouter02(dmmRouterAddress).swapTokensForExactTokens(
        //         dmmAmountsIn[dmmAmountsIn.length - 1], 
        //         MAXUINT, 
        //         dmmPoolAddresses, 
        //         dmmPaths, 
        //         address(this), 
        ...
        //         block.timestamp
        //     );
        //     amounts = mergeAmount(dmmAmountIn, ammAmountIn);
        // }
    }
    ...
    {
        require(compareStrings(poolType[0], "AMM"), 'AutoRoute: Invalid paths');
        uint256[] memory amountsOut = getAmountsOut(msg.value, paths, poolType, dmmPoolAddresses);
        require(amountsOut[amountsOut.length - 1] >= amountOutMin, 'AutoRoute: INSUFFICIENT_OUTPUT_AMOUNT');
        (address[] memory ammPaths, IERC20[] memory dmmPaths) = splitPath(paths, poolType);
        uint256[] memory ammAmountOut = IEvryRouter2(ammRouterAddress).swapExactETHForTokens{value:msg.value}(
            0, 
            ammPaths, 
            address(this), 
            block.timestamp
        );
        uint256[] memory dmmAmountOut = IDMMRouter02(dmmRouterAddress).swapExactTokensForTokens(
            ammAmountOut[ammAmountOut.length - 1], 
            amountOutMin, 
            dmmPoolAddresses, 
            dmmPaths, 
            to, 
        ...
        amounts = mergeAmount(ammAmountOut, dmmAmountOut);
    }
        ...
        returns (uint[] memory amounts)  
    {
        ...
        (uint256[] memory ammAmountsIn,) = getAmountsInAmmDmm(amountOut, ammPaths, dmmPaths, dmmPoolAddresses);
        require(msg.value >= ammAmountsIn[0], "AutoRoute: EXCESSIVE_INPUT_AMOUNT");
        uint256[] memory ammAmountOut = IEvryRouter2(ammRouterAddress).swapETHForExactTokens{value: msg.value}(
            ammAmountsIn[ammAmountsIn.length - 1], 
            ammPaths, 
            address(this), 
            block.timestamp
        ...
        if (msg.value > ammAmountOut[0]) {
            TransferHelper.safeTransferETH(msg.sender, msg.value - ammAmountOut[0]);
        }

        uint256[] memory dmmAmountOut = IDMMRouter02(dmmRouterAddress).swapTokensForExactTokens(
            amountOut, 
            MAXUINT, 
            dmmPoolAddresses, 
            dmmPaths, 
    ...
    function swapExactTokensForNative(
        ...
        returns (uint[] memory amounts)
    { 
        require(compareStrings(poolType[0], "DMM"), 'AutoRoute: Invalid paths');
        uint256[] memory amountsOut = getAmountsOut(amountIn, paths, poolType, dmmPoolAddresses);
        require(amountsOut[amountsOut.length - 1] >= amountOutMin, 'AutoRoute: INSUFFICIENT_OUTPUT_AMOUNT');
        IERC20(paths[0]).transferFrom(msg.sender, address(this), amountIn);
        (address[] memory ammPaths, IERC20[] memory dmmPaths) = splitPath(paths, poolType);
        ...
        uint256[] memory ammAmountOut = IEvryRouter2(ammRouterAddress).swapExactTokensForETH(
            dmmAmountOut[dmmAmountOut.length - 1],
            amountOutMin, 
            ammPaths, 
            to, 
            block.timestamp
        );
        amounts = mergeAmount(dmmAmountOut, ammAmountOut);
    }
    ...
    { 
        ...
        uint256[] memory ammAmountIn = IEvryRouter2(ammRouterAddress).swapTokensForExactETH(
            amountOut, 
            amountInMax, 
            ammPaths, 
            to, 
    ...
    function getAmountsInAmmDmm(
        uint256 _amountOut,
        address[] memory ammPaths, 
        IERC20[] memory dmmPaths,
        address[] memory dmmPoolAddresses
    )
        private
        view
        returns (uint256[] memory amm, uint256[] memory dmm) 
    {
        uint256[] memory dmmAmountIn = IDMMRouter02(dmmRouterAddress)
            .getAmountsIn(_amountOut, dmmPoolAddresses, dmmPaths);
        uint256[] memory ammAmountIn = IEvryRouter2(ammRouterAddress)
            .getAmountsIn(dmmAmountIn[0], ammPaths);
        return (ammAmountIn, dmmAmountIn);
    }

    function getAmountsInDmmAmm(
        uint256 _amountOut,
        address[] memory ammPaths, 
        IERC20[] memory dmmPaths,
        address[] memory dmmPoolAddresses
    ...
    {
        uint256[] memory ammAmountIn = IEvryRouter2(ammRouterAddress)
            .getAmountsIn(_amountOut, ammPaths);
        uint256[] memory dmmAmountIn = IDMMRouter02(dmmRouterAddress)
            .getAmountsIn(ammAmountIn[0], dmmPoolAddresses, dmmPaths);
        return (dmmAmountIn, ammAmountIn);
    }

    function mergeAmount(uint256[] memory a, uint256[] memory b) 
        private 
        pure 
        returns (uint256[] memory) 
    {
            
        uint256[] memory amounts = new uint256[](a.length + b.length);
        uint256 amountIndex = 0;
        for (uint256 i = 0; i < a.length; i++) {
            amounts[amountIndex] = a[i];
            amountIndex += 1;
        }
        for (uint256 i = 0; i < b.length; i++) {
            amounts[amountIndex] = b[i];
            amountIndex += 1;
        }
        return amounts;
    }

    function splitPath(address[] memory paths, string[] memory poolType) 
        private 
        pure 
        returns (address[] memory, IERC20[] memory) 
    {
        uint256 ammPathSize = 0;
        uint256 dmmPathSize = 0;
        for (uint256 i = 0; i < poolType.length; i++) {
            if (compareStrings(poolType[i], "AMM")) {
                ammPathSize += 1;
            } else if (compareStrings(poolType[i], "DMM")) {
                dmmPathSize += 1;
        ...
        address[] memory ammPaths = new address[](ammPathSize);
        IERC20[] memory dmmPaths = new IERC20[](dmmPathSize);
        uint256 ammIndex = 0;
        uint256 dmmIndex = 0;
        for (uint256 i = 0; i < paths.length; i++) {
            if (compareStrings(poolType[i], "AMM")) {
                ammPaths[ammIndex] = paths[i];
                ammIndex += 1;
            } else if (compareStrings(poolType[i], "DMM")) {
                dmmPaths[dmmIndex] = IERC20(paths[i]);
                dmmIndex += 1;
            } else {
                ammPaths[ammIndex] = paths[i];
                dmmPaths[dmmIndex] = IERC20(paths[i]);
    ...
    // function swap(address _tokenIn, address _tokenOut, uint _amountIn, uint _amountOutMin) external payable {
        
        
    //     Route memory route = pairToRoute[_tokenIn][_tokenOut];

    //     uint256[] memory amounts = IEvryRouter2(ammRouterAddress).getAmountsOut(_amountIn, route.ammRoute);
    //     require(amounts[amounts.length - 1] >= _amountOutMin, 'AutoRoute: INSUFFICIENT_OUTPUT_AMOUNT');
    //     if (route.ammRoute[0] == WBNB) {
    //         IEvryRouter2(ammRouterAddress).swapExactETHForTokensSupportingFeeOnTransferTokens{value: msg.value}(
    //             amounts[amounts.length - 1],
    //             route.ammRoute,
    //             address(this),
    //             block.timestamp
    //         );    
    //     } else if (route.ammRoute[1] == WBNB && route.ammRoute.length == 2) {
    //         IERC20(_tokenIn).transferFrom(msg.sender, address(this), _amountIn);
    //         IEvryRouter2(ammRouterAddress).swapExactTokensForETHSupportingFeeOnTransferTokens(
    //             _amountIn,
    //             amounts[amounts.length - 1], 
    //             route.ammRoute, 
    //             address(this), 
    ...
    //     } else {
    //         IERC20(_tokenIn).transferFrom(msg.sender, address(this), _amountIn);
    //         IEvryRouter2(ammRouterAddress).swapExactTokensForTokens(
    //             _amountIn, 
    //             amounts[amounts.length - 1], 
    //             route.ammRoute, 
    //             address(this), 
    ...
    //     IDMMRouter02(dmmRouterAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(
    //         amounts[0], 
    //         _amountOutMin, 
    //         poolAddresses, 
    //         route.dmmRoute, 
...
interface IDMMRouter01 is IDMMExchangeRouter, IDMMLiquidityRouter {
    ...
    function weth() external view returns (IWBNB);
}
