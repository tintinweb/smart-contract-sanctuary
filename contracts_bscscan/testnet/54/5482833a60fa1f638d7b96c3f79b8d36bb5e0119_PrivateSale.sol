...
contract PrivateSale is Context, Ownable {
    ...
    function updateSaleName(string calldata saleName) external onlyAdmin() {
        name = saleName;   
    }
    ...
    function start(
        ...
        require(_maxPurchase > 0 && _maxPurchase <= _availableTokens, 'Max purchase must be greater than zero & less than available amount!');
