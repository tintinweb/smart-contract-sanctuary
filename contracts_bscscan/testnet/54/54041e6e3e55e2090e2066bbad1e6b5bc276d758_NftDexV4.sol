...
interface LPInterface {
     ...
     * @dev Returns the amount of tokens in existence.
     */
    function getResevers() external view returns (uint256, uint256, uint256);
  ...
  function buyNFT(address nft_a,uint256 tokenId, address buyer) public returns (bool) {
        ...
        if(_saleTokens[tokenId].price > 0){
          price = _saleTokens[tokenId].price;
            ...
            uint256 aliaPanReserve;
            uint256 bnbPanReserve;
            uint256 aliaTime;
            (aliaPanReserve, bnbPanReserve, aliaTime) = LPAlia.getResevers();

            price = _dollarPrice[tokenId]/ ((busdLpReserve/bnbLpReserve) * (bnbPanReserve /aliaPanReserve));
        }
