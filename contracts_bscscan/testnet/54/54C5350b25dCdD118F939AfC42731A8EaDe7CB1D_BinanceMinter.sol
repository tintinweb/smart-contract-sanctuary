...
contract BinanceMinter is Ownable, Pausable, ReentrancyGuard {
    ...
    address public rootNativeToken;//NativeToken
    ...
    modifier onlyActivatedChains(uint256 chainID) {
        ...
        _;
    }//chek if he chainid is acttive or not

    modifier notContract() {
        require(!_isContract(msg.sender), "contract not allowed");
        require(msg.sender == tx.origin, "proxy contract not allowed");
        _;
    }//check it is the contract or not
    ...
    function swapEndForNative(
        ...
        IERC20(rootNativeToken).safeMint(to, amount);
        emit SwapEndForNative(receivedHash, depositCount, fromChainID, from, to, amount);
    }
    ...
    function setRootTokenForNative(address _tokenAddr)public onlyOwner returns(bool)
    {
        require(_tokenAddr != address(0),"Token address should not be zero");
        rootNativeToken = _tokenAddr;
    }
}