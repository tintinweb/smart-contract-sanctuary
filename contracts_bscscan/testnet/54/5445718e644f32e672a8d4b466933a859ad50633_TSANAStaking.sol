...
contract TSANAStaking is Ownable, ReentrancyGuard {
  ...
  function deposit(uint256 _amount) external nonReentrant {
    require(depositWhitelistOn == false || depositWhitelist[msg.sender] == true, "Address not in whitelist");
   ...
   */
  function canHarvest(address _user)
      public
      view
      returns (bool)
  {
      UserInfo storage user = userInfo[_user];
      return harvestLockOn == false || block.timestamp >= user.nextHarvestUntil;
  }
  ...
  {
      UserInfo storage user = userInfo[_user];
      return withdrawLockMethod == 0 || withdrawLockMethod == 2 || block.timestamp >= user.nextWithdrawUntil;
  }
  ...
  function withdraw(uint256 _amount) external nonReentrant {
    ...
    if (_amount > 0) {
      if (canWithdraw(msg.sender)) {
        ...
        uint256 feeAmount = 0;
        if (withdrawLockMethod == 2 && withdrawFeeMethod != 0) {
          feeAmount = _amount.mul(withdrawFee).div(10000);
          if (withdrawFeeMethod == 1) {     // 100% to marketing addresss
            stakedToken.safeTransfer(marketingAddress, feeAmount);
          } else if (withdrawFeeMethod == 2) {    // 100% burn
            stakedToken.safeTransfer(BURN_ADDRESS, feeAmount);
          } else if (withdrawFeeMethod == 3) {    // 50% marketing address 50% burn
            stakedToken.safeTransfer(BURN_ADDRESS, feeAmount.div(2));
            stakedToken.safeTransfer(marketingAddress, feeAmount.sub(feeAmount.div(2)));
          } else if (withdrawFeeMethod == 4) {    // add to liquidity
   ...
   * @dev Only callable by owner
   */
  function updateDepositWhitelist(address _address, bool _on) external onlyOwner {
  ...
  function updateWithdrawLockMethod(uint8 _method) external onlyOwner {
    require(_method == 0 || _method == 1 || _method == 2, "Invalid withdraw lock method set");
  ...
  function updateWithdrawFeeMethod(uint8 _method) external onlyOwner {
    require(_method == 0 || _method == 1 || _method == 2 || _method == 3 || _method == 4, "Invalid withdraw fee method set");
