...
contract GymVaultsStrategyAlpaca is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function initialize(
        ...
        uniRouterAddress = address(0x367633909278A3C91f4cB130D8e56382F00D1071);
        notPublic = false;
        controllerFee = 0;
    ...
    function withdraw(address, uint256 _wantAmt) public onlyOwner nonReentrant returns (uint256) {
        ...
        IERC20Upgradeable(wantAddress).safeTransfer(address(msg.sender), _wantAmt);
    ...
    function pendingHarvest() public view returns (uint256) {
        uint256 _earnedBal = IERC20Upgradeable(earnedAddress).balanceOf(address(this));
    ...
    function _farm() internal {
        ...
        IERC20Upgradeable(wantAddress).safeIncreaseAllowance(vaultContractAddress, wantAmt);
        IVault(vaultContractAddress).deposit(wantAmt);
        // add ibToken to farm contract
        uint256 ibWantAmt = IERC20Upgradeable(vaultContractAddress).balanceOf(address(this));
        IERC20Upgradeable(vaultContractAddress).safeIncreaseAllowance(farmContractAddress, ibWantAmt);
    ...
    function distributeFees(uint256 _earnedAmt) internal returns (uint256 _fee) {
        if (_earnedAmt > 0) {
            ...
            if (controllerFee > 0) {
                ...
                IERC20Upgradeable(earnedAddress).safeTransfer(operator, _fee);
