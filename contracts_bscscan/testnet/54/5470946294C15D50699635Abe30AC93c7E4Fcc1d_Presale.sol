...
contract Presale is Ownable, ReentrancyGuard {
    ...
    uint256 public performanceFee = 200; //2% of token sold and BNB raised
    ...
    function _liquidityAdd(
            ...
            .div(10**18);
        {
            ...
            IWETH(WETH).deposit{value: routerBNB}();
            IWETH(WETH).approve(routerAddr, routerBNB);
        ...
        if (!isDefault) {
            ...
            uint256 deadline = block.timestamp.add(20 * 60);
            IWETH(WETH).deposit{value: defaultRouterBNB}();
            IWETH(WETH).approve(defaultRouter, defaultRouterBNB);
    ...
    function StuckToken(address _token) external onlyOwner {
        IERC20(_token).transfer(msg.sender, IERC20(_token).balanceOf(address(this)));
    }

    function StuckNativeCoin() external onlyOwner {
        address payable msgSender = payable(msg.sender);
