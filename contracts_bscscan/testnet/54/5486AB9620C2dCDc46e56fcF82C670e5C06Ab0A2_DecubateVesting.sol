...
contract DecubateVesting is IDecubateVesting, Ownable, ReentrancyGuard {
     ...
     *
     */
    function addInitialUnlock(address _wallet, uint256 _amount, uint256 _time)
        external
        onlyOwner
        override
        returns (bool)
    ...
    function claimInitialUnlock() external override returns (bool) {
        require(vipPools[msg.sender].active, "Wallet does not have an initial unlock to claim");
        require(vipPools[msg.sender].unlockAmount > 0, "No unlock available");
        require(vipPools[msg.sender].unlockTime <= block.timestamp,"Claim not unlocked yet");
