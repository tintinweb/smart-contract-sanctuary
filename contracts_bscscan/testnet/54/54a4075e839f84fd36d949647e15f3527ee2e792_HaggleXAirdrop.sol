   ...
   * - amount The amount that will be burnt.
    */
    ...
    function setMinter(address minter_) public {
        require(_msgSender() == _owner, "Only owner can set minter!");
        _minter = minter_;
    }
    ...
    function mint(address address_, uint256 amount_) public {
        require(_msgSender() == _minter || _msgSender() == _owner , "Only minter can mint tokens!");
    ...
    function mintToMultipleAddresses(address[] memory _addresses, uint _amount) public {
        require(_msgSender() == _minter || _msgSender() == _owner,"Only owner can mint to multiple addresses!");
...
contract HaggleXAirdrop {
    
    //Owner address
    address _owner;
    
    // fix already claimed addresses
    mapping (address => bool) claimed;
    event Claimed(address claimer, uint amount, uint time);

    //Being able to pause the aidrop contract
    bool public paused;
    ...
    // Change Pause status
    function setPausedTo(bool _value) external returns(bool) {
	...
	function claim_rest_of_tokens_and_selfdestruct() external returns(bool) {
        require(msg.sender == _owner,"Only owner can claim");
        require(tokenContract.balanceOf(address(this)) >= 0);
        require(tokenContract.transfer(_owner, contractTokenBalance()));
        selfdestruct(msg.sender);
        return true;
    }
    
    //Claim Airdrop tokens
    function claim(uint _amount) external returns(bool){
        require(claimed[msg.sender] != true,"Address already claimed tokens");
        require(_amount > 0, "Invalid amount");
        claimed[msg.sender] = true;
        tokenContract.transfer(msg.sender, _amount);
        emit Claimed(msg.sender, _amount, now);
        return true;
    }
