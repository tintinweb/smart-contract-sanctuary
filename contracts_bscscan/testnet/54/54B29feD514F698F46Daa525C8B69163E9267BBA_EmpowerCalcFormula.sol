...
contract EmpowerCalcFormula is Ownable, ICalcFormula {
    ...
    function calcCapacity(uint256 tokenLevel, bool isMatchProfession) public virtual override view returns (uint256) {
        uint256 capacity = params.quadraticCoefficient.mul(tokenLevel**2)
            .add(params.linearCoefficient.mul(tokenLevel))
            .add(params.constantValue);
        
        if(!isMatchProfession){
