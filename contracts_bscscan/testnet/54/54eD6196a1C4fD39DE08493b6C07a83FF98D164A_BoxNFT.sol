...
contract BoxNFT is ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable{
    ...
    // mapping(uint256 => uint256) boxStarter;

    // DiceNFT public diceNFT;
    // BuildingNFT public buildingNFT;
    // using BoxsUpgradeable for Box;
    ...
    mapping(address => mapping(uint256 => uint256)) public boxes;// address - id - details // cach lay details = boxes[address][boxId]
    
    mapping(uint256 => uint256) prices;

    mapping(uint256 => uint256) limits;

    mapping(address => uint256) starterBoxes;

    DiceNFT public diceNFT;

    event randomData(uint256 characterId, uint256 diceId, uint256 randomCharacter,uint256 randomDice);

    function setConfig (uint256 _rank, uint256 _price) public onlyOwner {
        prices[_rank]=_price;
    }
    ...
    function initByOwner(CharacterNFT _characterNFT, DiceNFT _diceNFT) public  onlyOwner {
        ...
        diceNFT = _diceNFT;
        prices[1]=0.016 ether;    // D Starter Box
        prices[2]=0.08 ether;     // C Bronze Box
        prices[3]=0.4 ether;      // B Silver Box
        prices[4]=2 ether;        // A Golden Box
        prices[5]=10 ether;       // S Diamond Box
        prices[6]=50 ether;       // SS Legendary Box
        prices[7]=250 ether;      // SSS Dragon Box
        prices[8]=999 ether;      // SSSS Mystic Box
  ...
  function createBox(address owner,uint256 rank) public onlyOwner returns (uint256) {
        ...
        boxes[owner][newBoxId] = encode(Box(newBoxId,rank));
        return newBoxId;
    }

 function createRandomThreeBox(address owner) public {
        for (uint i=0; i<3; i++) {
        _tokenIds.increment();
        uint256 newBoxId = _tokenIds.current();
        //we can call mint from the ERC721 contract to mint our nft token
        // _safeMint(msg.sender, newBoxId);
        _safeMint(owner, newBoxId);
        uint256 value = uint256(newBoxId*block.timestamp);
        uint256 rankRandom = (value << (16+i))%2 +1;

        boxes[owner][newBoxId] = encode(Box(newBoxId,rankRandom));
        }
    }


  function getBox(address owner, uint256 id) public view returns (Box memory _box) {
    uint256 details= boxes[owner][id];
    _box.id = uint256(uint48(details>>100));
    _box.rank = uint256(uint16(details>>116));
  }
...
function getBoxPublic(address _owner, uint256 _id) public view returns (
    ...
    Box memory _box= getBox(_owner,_id);
    id=_box.id;
    rank=_box.rank;

  }

  function encode(Box memory box) public pure returns (uint256) {
  // function encode(Box memory box)  external view returns  (uint256) {
    uint256 value;
    value = uint256(box.id);
    value |= box.id << 100;
    value |= box.rank << 116;
    return value;
  }
...
function transfer(uint256 _nftId, address _target)
        external
    {
        ...
        Box memory box= getBox(ownerOf(_nftId),_nftId);
        // star will start = 1, exp will start = 0

        boxes[_target][_nftId] = encode(box);
        boxes[ownerOf(_nftId)][_nftId]= encode(Box(0,0));
        _transfer(ownerOf(_nftId), _target, _nftId);
        
    }
  function approveMarketPlace(address to, uint256 tokenId) external onlyBoxMarketPlaceOrOwner {
...
function buyBox(uint256 rank) public payable returns (uint256 characterId,uint256 diceId,uint256 randomCharacter,uint256 randomDice) {
        require(msg.value!=0,'msg.value must be greater than 0');
        require(msg.value==prices[rank],'Not equal value to buy');
        require(rank<=2,'Rank must be less than 2');
            uint256 value = uint256(block.timestamp);
            uint256 randomHaveDice =  (value << 10) % 100+1;
            bool haveDice=false;
            uint256 typeDice=0;
            uint256 typeCharacter=1;
            if(1<= randomHaveDice && randomCharacter<=80){
                haveDice=true;
            }else{
              diceId=0;
            }
            ...
            if(rank==1){
              ...
              characterId=characterNFT.createCharacter(msg.sender,typeCharacter,rank,1,0);
...
function openBox(uint256 tokenId) external returns (uint256 characterId,uint256 diceId,uint256 randomCharacter,uint256 randomDice){
        address owner = ownerOf(tokenId);
        Box memory box= getBox(owner,tokenId);
        uint256 value = uint256(tokenId*block.timestamp);
        require(box.rank<=2,'Rank must be less than 2');
        uint256 randomHaveDice =  (value << 10) % 100+1;
        bool haveDice=false;
        uint256 typeDice=0;
        uint256 typeCharacter=1;
            ...
            if(box.rank==1){
              ...
              characterId=characterNFT.createCharacter(msg.sender,typeCharacter,box.rank,1,0);
              randomDice = (value << 30)%100+1;
              if(haveDice==true){
