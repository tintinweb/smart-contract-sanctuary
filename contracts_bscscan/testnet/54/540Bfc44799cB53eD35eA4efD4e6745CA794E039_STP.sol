...
contract STP is Context, IERC20, AccessControl {
    ...
    mapping (address => bool) private _isExcludedFromTimeLimit;
    ...
    uint256 private _tTotal = 300 * 10**6 * 1 ether;
    uint256 private _rTotal = _tTotal;
    uint256 private _tFeeTotal;

    uint256 public maxHoldAmount = _tTotal / 1000;
    ...
    uint256 public _manualBurnFee = 5;
    uint256 public _charityFee = 5;
    uint256 public _devFee = 10;

    address public _manualBurnAddress = 0xa61c5EB7C6166adeBc41DF862c51E97ED3A9FB40;
    address public _charityAddress = 0xa61c5EB7C6166adeBc41DF862c51E97ED3A9FB40;
    address public _devFeeAddress = 0xa61c5EB7C6166adeBc41DF862c51E97ED3A9FB40;


    address public _owner = msg.sender;
    
    // liq + manual burn + charity + devFee
    ...
    uint256 public _maxTxAmount = _tTotal / 10000;
    ...
    constructor () public {
        ...
        _isExcludedFromHoldingLimit[owner()] = true;
        _isExcludedFromHoldingLimit[_msgSender()] = true;
        _isExcludedFromHoldingLimit[address(this)] = true;

        _isExcludedFromTimeLimit[owner()] = true;
        _isExcludedFromTimeLimit[_msgSender()] = true;
        _isExcludedFromTimeLimit[address(this)] = true;
    ...
    function excludeFromTimeLimit(address account) public onlyAdmin {
        _isExcludedFromTimeLimit[account] = true;
    }

    function includeFromTimeLimit(address account) public onlyAdmin {
        _isExcludedFromTimeLimit[account] = false;
    }

    function excludeFromHoldingLimit(address account) public onlyAdmin {
        _isExcludedFromHoldingLimit[account] = true;
    }

    function includeInHoldingLimit(address account) public onlyAdmin {
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        uint256 tCharityFee = tLiquidity.mul(_charityFee).div(
            _liquidityFee
        );
        uint256 rCharityFee = rLiquidity.mul(_charityFee).div(
            _liquidityFee
        );

        _rOwned[_charityAddress] = _rOwned[_charityAddress].add(rCharityFee);
        if(_isExcluded[_charityAddress])
            _tOwned[_charityAddress] = _tOwned[_charityAddress].add(tCharityFee);
        ...
        _rOwned[address(this)] = _rOwned[address(this)].add(rLiquidity.sub(rdevFee).sub(rCharityFee).sub(rmanualBurnFee));
        if(_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tLiquidity.sub(tdevFee).sub(tCharityFee).sub(tmanualBurnFee));
    }
    ...
    function isExcludedFromTimeLimit(address account) public view returns(bool) {
        return _isExcludedFromTimeLimit[account];
    }
    ...
    function _transfer(
        ...
        if (!_isExcludedFromHoldingLimit[to])
            require(balanceOf(to) + amount <= maxHoldAmount, 'Holding Amount Limit');

        // uint256 oneHourLater = block.timestamp + periodLimitation;

        if(!_isExcludedFromTimeLimit[from])
            require(lastTransfer[from] + periodLimitation < block.timestamp, 'Time Transfer Limit');
