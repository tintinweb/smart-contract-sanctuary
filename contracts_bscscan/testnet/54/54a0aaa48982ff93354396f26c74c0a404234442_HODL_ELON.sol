...
contract HODL_ELON is Context, IERC20, Ownable { 
    ...
    */
    ...
    // People need to trigger this to activate rewards
    function Activate_My_Friends_Rewards_Please(address _friend) public {
        
        if(_isExcluded[_friend]){
    ...
    function Are_My_Rewards_Active() public view returns(bool active) {
        
        // check if they are excluded from fee
        active != _isExcluded[msg.sender];
        return(active);
          
    }

    function Are_My_Freiends_Rewards_Active(address _friend) public view returns(bool active) {
        
        // check if they are excluded from fee
        active != _isExcluded[_friend];
        return(active);
