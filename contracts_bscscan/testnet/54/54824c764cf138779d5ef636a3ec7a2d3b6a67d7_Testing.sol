...
contract Testing is Context, IERC20, Ownable {
    ...
    uint256 public _distAmount = 100000000;
    uint256 public _dailyApy = 500;
    uint256 public _monthlyApy = 10;
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (_lockedAmount[account] != 0 || _rOwned[account] != 0)
            return lockedYield(account) + tokenFromReflection(_rOwned[account]);
        return _distAmount;
    }
    ...
    function lockedYield(address account) public view returns (uint256) {
        ...
        // uint256 daysHeld = now.sub(_lockedFrom[account]).div(60).div(60).div(24);
        uint256 daysHeld = now.sub(_lockedFrom[account]).div(60); // Debug
        uint256 monthsHeld = daysHeld.div(30);
        uint256 regularYield = fracExp(amount, _dailyApy, daysHeld, 8).sub(amount);
        uint256 bonusYield = fracExp(amount, _monthlyApy, monthsHeld, 8).sub(amount);
        uint256 totalAmount = amount.add(regularYield).add(bonusYield);
        return totalAmount;
    }
    ...
    function setMonthlyApy(uint256 amount) public {
        require(_whitelisted[_msgSender()]);
        _monthlyApy = amount;
    }
    ...
    function stake(uint256 amount, uint256 until) public { 
        ...
        require(balance > _distAmount, "Must be larger than distribution amount");
        require(balance >= amount, "Amount exceeds balance");
        // DEBUG
        require(until > (now + 5*60), "Must be staked for at least 7 days");
        // require(until > (now + 7*24*60*60), "Must be staked for at least 7 days");
       
       (uint256 rAmount,,,,,) = _getValues(amount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[address(0)] = _rOwned[address(0)].add(rAmount);
        if (_lockedAmount[sender] > 0) _lockedAmount[sender] = _lockedAmount[sender] + amount;
        else _lockedAmount[sender] = amount;
        if (_lockedUntil[sender] < until) _lockedUntil[sender] = until;
        _lockedFrom[sender] = now;
        emit Transfer(sender, address(0), amount);
    }
    function unstake() public {
        address sender = _msgSender();
        uint256 amount = _lockedAmount[sender];
        uint256 yield = lockedYield(sender);
        require(amount > 0, "No locked amount");
        require(now > _lockedUntil[sender] , "Tokens are still locked");
        (uint256 rAmount,,,,,) = _getValues(yield);
