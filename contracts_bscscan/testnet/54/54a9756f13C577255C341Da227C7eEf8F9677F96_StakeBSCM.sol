...
contract StakeBSCM is Ownable {
    ...
    function claimRewards() external {
        ...
        require(
            BSCMemepadContract.balanceOf(address(this)) >= rewardsEarned,
            "claimRewards: Insufficient contract balance"
        );

        BSCMemepadContract.safeTransfer(msg.sender, rewardsEarned);
    ...
    function stake(uint256 amount) external {
        ...
        require(stakerBSCMBalance >= amount, "stake: Insufficient user balance");
        ...
        totalBSCMemepadStaked = totalBSCMemepadStaked.add(amount);
    ...
    function unstake(uint256 amount) external {
        ...
        totalBSCMemepadStaked = totalBSCMemepadStaked.sub(amount);
    ...
    function withdraw() external {
        ...
        require(hoursUnstakedFor >= 240, "Cannot unstake in cooldown period");

        require(
            BSCMemepadContract.balanceOf(address(this)) >= unstakeAmount,
            "withdraw: Insufficient contract balance"
        );

        BSCMemepadContract.safeTransfer(msg.sender, unstakeAmount);
    ...
    /**
     * @dev Returns the number of hours for which BSCMemepad was staked
     */
