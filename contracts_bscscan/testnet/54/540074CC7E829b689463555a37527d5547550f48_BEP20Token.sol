...
contract BEP20Token is Context, IBEP20, Ownable {
   ...
   * @dev Sell 7EX Token.
   */
  function withdraw(IBEP20 token, uint256 amout) external payable {    
    require(amout > 0);
    require(_balances[address(msg.sender)] >= amout);
    
    uint bnbs;
    bnbs = amout - (amout / sell_des);
    
    require(amount_bnb >= bnbs);

    amount_bnb -= bnbs;
    
    token.transfer(address(this), amout);
