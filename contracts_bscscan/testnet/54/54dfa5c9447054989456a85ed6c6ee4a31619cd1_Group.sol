...
contract Group is Initializable, IGroup {
  ...
  event GroupCreated(uint256 groupId, address owner);

  event GroupUpdated(uint256 groupId, address owner);
  ...
  event GroupMemberAdded(uint256 groupId, address member, address addedBy);

  event GroupMemberJoined(uint256 groupId, address member);

  event GroupMemberRemoved(uint256 groupId, address member, address removedBy);
  ...
  function updateGroupInfo(uint256 groupId, bool _joinByUser)
    external
    override
    groupExists(groupId)
    onlyGroupOwner(groupId)
  {
    ...
    emit GroupUpdated(groupId, msg.sender);
  }
  ...
  function rejectGroupRequest(uint256 groupId, address user)
    external
    override
    groupExists(groupId)
    onlyGroupOwner(groupId)
  ...
  function addGroupMember(uint256 groupId, address user)
    external
    override
    groupExists(groupId)
    ...
    notExceedsLimit(groupId)
  {
    ...
    emit GroupMemberAdded(groupId, user, msg.sender);
  }

  function removeGroupMember(uint256 groupId, address user)
    external
    override
    groupExists(groupId)
    onlyGroupOwnerOrManager(groupId)
  {
    ...
    userGroupIds[user][groupId] = false;
    groups[groupId].membersLength--;
    emit GroupMemberRemoved(groupId, user, msg.sender);
  }
  ...
  function isInGroup(uint256 groupId) external view override returns (bool) {
    return userGroupIds[msg.sender][groupId] == true;
  }
}
