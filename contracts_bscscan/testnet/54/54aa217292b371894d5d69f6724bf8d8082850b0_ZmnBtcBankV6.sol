...
contract ZmnBtcBankV6 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    modifier onlyAuthority() {
        require(authorityAddress == _msgSender(), "Not authorized");
        _;
    }
    ...
    function manualEditHistoryRate(
        ...
        require(_rid < rates.length - 1, "Only history rate");
        rates[_rid].rate = _rate;
        rates[_rid].circulateZmn = _circulateZmn;
        rates[_rid].accIncomingBtcb = _accIncomingBtcb;
        rates[_rid].accBorrowingBtcb = _accBorrowingBtcb;
        rates[_rid].accRepaidBtcb = _accRepaidBtcb;
        rates[_rid].ongoingBorrowingBtcb = _ongoingBorrowingBtcb;
        rates[_rid].accRedeemFeeZmn = _accRedeemFeeZmn;
        rates[_rid].accTransactionFeeZmn = _accTransactionFeeZmn;
        rates[_rid].accOverdueCollateralZmn = _accOverdueCollateralZmn;
    }
    */
    ...
    function setProfile(
        ...
        profiles[_pid].transactionFee = _transactionFee;
    ...
    function _createLoanAgreement(
        uint256 _pid,
        uint256 _amountCollateralZmn,
        address _borrower
    ) internal view returns (LoanAgreement memory) {
        ...
        LoanAgreement memory _loan = LoanAgreement({
            loanId: _loanId,
            borrower: _borrower,
            profile: _pid,
            ...
            amountTransactionFeeZmn: transactionFee,
    ...
    function borrow(
        ...
        LoanAgreement memory _loan = _createLoanAgreement(
            _pid,
            _amountCollateralZmn,
            address(msg.sender)
        );
        require(_loan.amountBorrowBtcb >= _minAmountOut, "Min amountOut");
        ...
        if (_loan.amountRedeemFeeZmn > 0) {
            zmn.safeTransferFrom(
                address(msg.sender),
                address(redeemFeeAddress),
                _loan.amountRedeemFeeZmn
            );
        }
        if (_loan.amountTransactionFeeZmn > 0) {
            zmn.safeTransferFrom(
                address(msg.sender),
                address(transactionFeeAddress),
                _loan.amountTransactionFeeZmn
            );
        }

        // send btcb to borrower
        btcb.safeTransfer(address(msg.sender), _loan.amountBorrowBtcb);

        // add to list
        loanAgreements.push(_loan);

        // add to user list
        uint256[] storage _loanAgreementsByUser = loanAgreementsByUser[
            msg.sender
        ];
        _loanAgreementsByUser.push(_loan.loanId);

        // update calc information
        if (_loan.amountRedeemFeeZmn > 0) {
            calcRate.accRedeemFeeZmn = calcRate.accRedeemFeeZmn.add(
                _loan.amountRedeemFeeZmn
            );
        }
        if (_loan.amountTransactionFeeZmn > 0) {
        ...
        calcRate.accBorrowingBtcb = calcRate.accBorrowingBtcb.add(
            _loan.amountBorrowBtcb
        );
        ...
        if (_status == 1) {
            ...
            // add to ongoing list
            loanAgreementsWithOngoingStatus.add(_loan.loanId);
    ...
    // transactionFee
    // loopCnt
    function _previewBulkBorrow(uint256 _pid, uint256 _amountCollateralZmn)
        internal
        view
        returns (
            uint256,
    ...
    {
        // bulk variable
        uint256 remainingCollateralZmn = _amountCollateralZmn;
        uint256 accLoopAmountOut = 0;
        uint256 accLoopRedeemFee = 0;
        uint256 accLoopTransactionFee = 0;

        // Loop
        for (uint16 i = 0; i < loopLimit; i++) {
            // amount on this iteration
            uint256 _loopCollateralZmn = remainingCollateralZmn;
            if (_loopCollateralZmn > maxCollateralZmnAmount) {
                _loopCollateralZmn = maxCollateralZmnAmount;
            }
            ...
            (
                ...
                uint256 redeemFee,
                uint256 transactionFee
            ) = _previewBorrow(_pid, _loopCollateralZmn);

            // accumulate
            if (amountOut > 0) {
                accLoopAmountOut = accLoopAmountOut.add(amountOut);
            }
            if (redeemFee > 0) {
                accLoopRedeemFee = accLoopRedeemFee.add(redeemFee);
            }
            if (transactionFee > 0) {
                accLoopTransactionFee = accLoopTransactionFee.add(
                    transactionFee
                );
            }

            // end loop?
            remainingCollateralZmn = remainingCollateralZmn.sub(
                _loopCollateralZmn
            );
            if (remainingCollateralZmn == 0) break;
        }

        return (accLoopAmountOut, accLoopRedeemFee, accLoopTransactionFee);
    }

    function previewBulkBorrow(uint256 _pid, uint256 _amountCollateralZmn)
        public
        view
        returns (
            uint256,
        ...
        )
    {
        if (
            _amountCollateralZmn < minCollateralZmnAmount ||
            _amountCollateralZmn > (maxCollateralZmnAmount.mul(loopLimit))
        ) {
            return (0, 0, 0);
        }
        ...
        return _previewBulkBorrow(_pid, _amountCollateralZmn);
    }

    // perform loop borrow
    // for ZmnStakeAuto only
    // ZmnStakeAuto sent btcb on behaveof user
    function bulkBorrowFor(
        uint256 _pid,
        uint256 _amountCollateralZmn,
        uint256 _minAmountOut,
        address _to
    ) public whenNotPaused onlyAuthority {
        require(
            _amountCollateralZmn >= minCollateralZmnAmount,
            "Minimum amount of collateral ZMN"
        );
        ...
        Profile memory profile = profiles[_pid];
        require(profile.enable, "Lending profile has been suspended");
        // require(profile.period == 0, "Only borrow without repayment");
        uint16 _status = (profile.period == 0) ? 1 : 0;
        ...
        for (uint16 i = 0; i < loopLimit; i++) {
            ...
            // accumulate
            if (_loan.amountBorrowBtcb > 0) {
                accLoopAmountOut = accLoopAmountOut.add(_loan.amountBorrowBtcb);
            }
            if (_loan.amountRedeemFeeZmn > 0) {
                accLoopRedeemFee = accLoopRedeemFee.add(
                    _loan.amountRedeemFeeZmn
                );
            }
            if (_loan.amountTransactionFeeZmn > 0) {
        ...
        require(accLoopAmountOut >= _minAmountOut, "Min amountOut");
        ...
        if (accLoopTransactionFee > 0) {
            zmn.safeTransferFrom(
                address(msg.sender),
                address(transactionFeeAddress),
                accLoopTransactionFee
            );
        }

        // send btcb to caller
        btcb.safeTransfer(address(msg.sender), accLoopAmountOut);
    ...
    function setLoopLimit(uint256 _loopLimit) public onlyOwner {
