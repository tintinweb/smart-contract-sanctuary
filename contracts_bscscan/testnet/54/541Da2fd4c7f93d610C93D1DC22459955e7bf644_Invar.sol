...
contract Invar {
    ...
    function work(address playerAddress) public returns (bool) {
        ...
        uint256 timeSinceLastWorked = block.timestamp -
            players[playerAddress].lastWorked;
        uint256 workTimeout = 120; // 2 minutes

        if (timeSinceLastWorked >= workTimeout) {
            // uint256 reward = 25 * (10**decimals) * playerEconomicLevel;
            uint256 reward = 25 * (10**decimals);
            accumulatedRewards[playerAddress] += reward;
            players[playerAddress].lastWorked = block.timestamp;
    ...
    function claimReward(address playerAddress) public returns (bool) {
