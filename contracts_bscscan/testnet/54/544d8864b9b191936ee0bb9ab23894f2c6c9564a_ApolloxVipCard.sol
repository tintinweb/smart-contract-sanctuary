...
contract ApolloxVipCard is ApolloxERC721EnumerableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function _oncePerBlock() internal view {
        require(lastMintedBlock < block.number, "Invalid id");
    }
    ...
    function _ownerOnly(uint256 tokenId) internal view{
        ...
        require(owner == msg.sender, "Permission denied: not owner");
    }
    ...
    function mint(address recipient, uint8 round) public oncePerBlock minterOnly{
        ...
        uint8 level = 0; //Todo random
        string memory memo;

        properties.push(Property(block.number, block.timestamp, round, level, memo));
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        require(tokenId < properties.length, "Invalid token id");
        Property memory property = properties[tokenId];
        //Todo enhance random key
        string memory output = SvgBuilder.buildSvg(tokenId, property.level, property.mintTimestamp, property.mintTimestamp, property.memo);
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Bag #', tokenId.toString(), '", "description": "This is a description.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
...
//    }
    ...
    function testMemo(string memory memo) public view returns (bool){
        ...
        require(valid, "Invalid characters");
        return valid;
    }
}
