...
interface IANN is IERC20 {
//   function ANNtransfer(address account_, uint256 amount_) external;
...
contract AnnexTreasury is AnnexAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    constructor(
        ...
        totalReserves = 3000000000000000000000000000;
    }
    ...
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        ANN.transfer(0x000000000000000000000000000000000000dEaD, value);
    ...
    /**
     * @notice mint new ANN using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        ANN.transfer(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or ANN from the treasury, using sANN as collateral. This might allow an
     * sANN holder to provide ANN liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(ANN)) {
            require(permissions[STATUS.ANNDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sANN.changeDebt(value, msg.sender, true);
        require(sANN.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(ANN)) {
            ANN.transfer(msg.sender, value);
            annDebt = annDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sANN.changeDebt(value, msg.sender, false);
     ...
     * @param _amount uint256
     */
    function repayDebtWithANN(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.ANNDEBTOR][msg.sender], notApproved);
        ANN.transfer(0x000000000000000000000000000000000000dEaD, _amount);
        sANN.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        annDebt = annDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(ANN), _amount, _amount);
    }
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SANN) {
            // 9
            sANN = IsANN(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(ANN.totalSupply().sub(totalDebt));
    }
    /**
    ...
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(ANN)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return ANN.totalSupply() - annDebt;
    }
}