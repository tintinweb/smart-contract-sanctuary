...
contract PresaleContract is Context, Ownable, ReentrancyGuard{
    ...
    function claimRewards() public nonReentrant{
        ...
        // We try to add liquidity directly - if it fails, we send the eth to the owner to add it manually
        try kupcakeRouter.addLiquidityETH{value: ethToLiquify}(
            address(rewardToken),
            ethToLiquify,
            0,
            0,
            ...
            block.timestamp + 60
                ...
                // If it fails, we send the eth so we can add liquidity manually
                payable(owner()).transfer(ethToLiquify);
            }
