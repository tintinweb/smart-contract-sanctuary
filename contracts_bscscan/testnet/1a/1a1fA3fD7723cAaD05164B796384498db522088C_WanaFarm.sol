...
contract WanaFarm is BaseFarm, ReentrancyGuard, Ownable, Initializable, ITokenFarm {
    ...
    event AddedLocking(address indexed account, uint256 amount);
    event ClaimedLocking(address indexed account, uint256 amount);
    ...
    function claimLockReward(uint256 _index) external nonReentrant {
        ...
        emit ClaimedLocking(_user, _claimableAmount);
    }
    ...
    function claimTotalReward(uint256 _start, uint256 _end) external nonReentrant {
        ...
        if (_totalClaimableAmount > 0) {
            _safeTokenTransfer(_user, _totalClaimableAmount);
            emit ClaimedLocking(_user, _totalClaimableAmount);
        }
    }
