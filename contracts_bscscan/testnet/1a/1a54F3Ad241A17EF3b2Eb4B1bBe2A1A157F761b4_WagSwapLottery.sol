...
// File: contracts/interfaces/IWagSwapLottery.sol

pragma solidity ^0.8.4;

interface IWagSwapLottery {
    /**
     ...
     * @param _priceTicketInWag: price of a ticket in CAKE
...
// File: contracts/WagSwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title WagSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract WagSwapLottery is ReentrancyGuard, IWagSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInWag = 50 ether;
    uint256 public minPriceTicketInWag = 0.005 ether;
     ...
     * @param _wagTokenAddress: address of the CAKE token
        ...
        nonReentrant
    {
        ...
        uint256 amountWagToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInWag,
            _ticketNumbers.length
        );

        // Transfer wag tokens to this contract
        wagToken.safeTransferFrom(address(msg.sender), address(this), amountWagToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInWag += amountWagToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInWagToTransfer
        uint256 rewardInWagToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInWagToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        wagToken.safeTransfer(msg.sender, rewardInWagToTransfer);

        emit TicketsClaim(msg.sender, rewardInWagToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInWag) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInWag - amountToShareToWinners);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        _lotteries[_lotteryId].amountCollectedInWag += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInWag >= minPriceTicketInWag) && (_priceTicketInWag <= maxPriceTicketInWag),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInWag: _priceTicketInWag,
            ...
            amountCollectedInWag: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(wagToken), "Cannot be CAKE token");
     ...
     * @dev Only callable by owner
     * @param _minPriceTicketInWag: minimum price of a ticket in CAKE
     * @param _maxPriceTicketInWag: maximum price of a ticket in CAKE
     */
    function setMinAndMaxTicketPriceInWag(uint256 _minPriceTicketInWag, uint256 _maxPriceTicketInWag)
        external
        onlyOwner
    {
        require(_minPriceTicketInWag <= _maxPriceTicketInWag, "minPrice must be < maxPrice");

        minPriceTicketInWag = _minPriceTicketInWag;
        maxPriceTicketInWag = _maxPriceTicketInWag;
    }
