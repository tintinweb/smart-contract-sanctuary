...
contract NFTStaking_V2 is IERC721Receiver, ReentrancyGuard {
    ...
    uint256 public farmingStartBlock = block.number;
    uint256 public farmingEndBlock = farmingStartBlock + 576000; // No more rewards after 20 days;
    ...
    event FarmingEndBlock(uint256 _endTimeBlock);
    ...
    function _unStakeNFT(uint256 tokenId) internal onlyStaker(tokenId) requireTimeElapsed(tokenId) returns (bool) {
        ...
        require(block.number >= farmingEndBlock, 'Unstake: Farming end block has not been passed yet');
    ...
    function _stakeNFT(uint256 tokenId) internal returns (bool) {
        ...
        require(block.number < farmingEndBlock, 'Stake: Farming has ended');
    ...
    // Setters
    function setFarmingEndBlock(uint256 _endTimeBlock) external onlyDao {
        require(_endTimeBlock != 0, 'setfarmingEndBlock: Cannot be 0');
        farmingEndBlock = _endTimeBlock;

        emit FarmingEndBlock(_endTimeBlock);
    }
