...
import "./ITwinciStaking.sol";

interface IRewardToken {
    function mint(address recipient_, uint256 amount_) external returns (bool);
}

// TwinciStacking is a smart contract for distributing Twinci by asking user to stake the ERC20-based token.
contract FoxciStaking is ITwinciStaking, Ownable ,ReentrancyGuard{
    ...
    // The block number when Foxci allowance starts.
    ...
    function pendingReward(uint256 _pid, address _user) external override view returns (uint256, address) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint accRewardPerShare = pool.accRewardPerShare.add(foxciReward);
            uint _reward = pool.accRewardPerShare.mul(_stakerWeight).div(pool.totalStakingWeight);
            accFoxciPerShare = accRewardPerShare.mul(_stakerWeight).div(pool.totalStakingWeight);
        }
        return (accFoxciPerShare, pool.stakeToken);
    }
    ...
    function updatePool(uint256 _pid) public override {
        ...
        uint256 rewardAllowance = IERC20(rewardToken).balanceOf(address (this));
        pool.accRewardPerShare = pool.accRewardPerShare.add(reward);
        pool.lastRewardBlock = block.number;
    }


    // Deposit Staking tokens to StakingToken for Foxci allocation.
    ...
    function _harvest(address _to, uint256 _pid) internal {
        ...
        require(_reward <= IERC20(rewardToken).balanceOf(address(this)), "Not enough Twinci");
        user.lastBlock = block.number;
    ...
    // Safe Twinci transfer function, just in case if rounding error causes pool to not have enough Twincis.
    function safeTransfer(address _to, uint256 _amount) internal {
        uint256 twinciBal = IERC20(rewardToken).balanceOf(address(this));
        if (_amount > twinciBal) {
            IERC20(rewardToken).transfer(_to, twinciBal);
        } else {
    ...
    function cleanPools(uint[] calldata pids, bool allPools) override public onlyOwner {
        require(!active, "Staking in process");
...
interface ITwinciStaking {
    ...
    function cleanPools(uint[] calldata pids, bool allPools) external;
