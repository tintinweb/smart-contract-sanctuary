...
contract preSale {
    ...
    uint256 public minAmountInBUSD;
    uint256 public maxAmount;
    uint256 public maxAmountInBUSD;
    uint256 public totalSupply;
    uint256 public preSaleStartTime;
    uint256 public preSaleEndTime;
    uint256 public soldToken;
    uint256 public amountRaisedBusd;
    uint256 public amountRaisedInBnb;
    ...
    constructor(
        ...
        tokenPerBUSD = 33 * (10**token.decimals());
        tokenPerBnb = 16500 * (10**token.decimals());
        minAmount = 0.001 ether;
        maxAmount = 2 ether;
        minAmountInBUSD = 60 ether;
        maxAmountInBUSD = 1000 ether;
        totalSupply = 28000000 * (10**token.decimals());
    ...
    function buyTokenWithBNB() public payable { 
        ...
        amountRaisedInBnb = amountRaisedInBnb + (msg.value);
        if (tokenBalance[msg.sender] == 0) {
            totalParticipants += 1;
        }
    ...
    // to buy token during preSale time => for web3 use
    function buyTokenWithBUSD(uint256 _amountBusd) public { 
        require(
            block.timestamp >= preSaleStartTime &&
                block.timestamp < preSaleEndTime,
            "PRESALE: PreSale time not met"
        ...
        require(
            coinBalance[msg.sender] + (_amountBusd) <= maxAmountInBUSD,
            "PRESALE: Amount exceeds max limit"
        );
        require(
            _amountBusd >= minAmountInBUSD,
    ...
    function busdToToken(uint256 _amount) public view returns (uint256) {
        uint256 numberOfTokens = _amount * (tokenPerBUSD);
    ...
    function changePriceInBUSD(uint256 _price) external onlyOwner {
        tokenPerBUSD = _price;
    }
       // to change Price in bnb of the token
    function changePriceInBNB(uint256 _price) external onlyOwner {
        tokenPerBnb = _price;
    } 

    // to change preSale amount limits
    function setPreSaletLimitsInBUSD(
        uint256 _minAmount,
        uint256 _maxAmount 
    ) external onlyOwner {
        minAmountInBUSD = _minAmount;
        maxAmountInBUSD = _maxAmount;
     
    }

    function setPreSaletLimitsInBNB(
        uint256 _minAmount,
        uint256 _maxAmount
    ...
    function changeOwner(address payable _newOwner) external onlyOwner {
     ...
     // transfer ownership
    function changeTotalSupply(uint256 _totalSupply) external onlyOwner {
