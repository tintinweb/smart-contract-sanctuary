...
contract ProSmartSplitter {
  ...
  constructor(address[] memory _payees, uint256[] memory _shares, IERC20 _depositTokenAddress) {
  ...
  function releaseAll() public {
    require(shares[tx.origin] > 0, "Caller has no shares");
    ...
    for (uint256 i = 0; i < payees.length; i++) {
      ...
      uint256 amount = (currentBalance * shares[payee]) / totalShares;

      depositToken.safeTransfer(payee, amount);
    }
  }
