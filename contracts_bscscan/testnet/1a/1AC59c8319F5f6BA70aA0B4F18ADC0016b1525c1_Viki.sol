...
* Telegram:  https://t.me/MoonVikings_Main/
* Twitter:   https://twitter.com/moon_vikings
* Website:   https://www.moon-vikings.com/
...
contract Viki is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 38440100000 * 10**_decimals;
    ...
    address public marketingAddr = 0x2914308cF65100E6E16dEF88674fA72E413b81a3;
    address public operationsAddr = 0x38088052B57e8faA7D82f9980243BD95549f4adB;
    address public constant deadAddr = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "Moon Vikings";
    string private constant _symbol = "Viki";
    ...
    feeRatesStruct public sellFee = feeRatesStruct(
    {rfi: 20,
     operations: 20,
     marketing: 40,
     liquidity: 20,
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[operationsAddr] = true;
   ...
   function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
    ...
    function setsellFee(uint256 _rfi, uint256 _operations, uint256 _marketing, uint256 _liquidity, uint256 _buyback) external onlyOwner{
        sellFee.rfi = _rfi;
        sellFee.operations = _operations;
        sellFee.marketing = _marketing;
        sellFee.liquidity = _liquidity;
    ...
    function _takeMarketing(uint256 rMarketing, uint256 tMarketing) private {
        ...
        if(_isExcluded[marketingAddr])
        {
            _tOwned[marketingAddr]+=tMarketing;
        }
        _rOwned[marketingAddr] +=rMarketing;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            s.tRfi = tAmount*sellFee.rfi/1000;
            s.tOperations = tAmount*sellFee.operations/1000;
            s.tMarketing = tAmount*sellFee.marketing/1000;
            s.tLiquidity = tAmount*sellFee.liquidity/1000;
            s.tBuyback = tAmount*sellFee.buyback/1000;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        ...
        emit Transfer(sender, marketingAddr, s.tMarketing);
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        if(operationsAmt > 0){
          payable(operationsAddr).transfer(operationsAmt);
        }
    }
    ...
    function setMarketingWallet(address newWallet) external onlyOwner{
        require(marketingAddr != newWallet ,'Wallet already set');
        marketingAddr = newWallet;
        _isExcludedFromFee[marketingAddr];
    }

    function setOperationsWallet(address newWallet) external onlyOwner{
        require(operationsAddr != newWallet ,'Wallet already set');
        operationsAddr = newWallet;
        _isExcludedFromFee[operationsAddr];
    }
    ...
    function setRouterAddr(address newRouter) external onlyOwner {
