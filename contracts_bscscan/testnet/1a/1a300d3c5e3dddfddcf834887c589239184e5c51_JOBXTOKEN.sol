...
// update 1 = FEE BUY/SELL OK, MAX WALLET OK, MAX BUY/SELL OK, LIMITBY DAY OK, 
// MIN WALLET NO, TRANSFER MIN/MAX WALLET NO, TRANSFER KE BURN DARI OWNER KENA FEE, MANUAL swap & SWEEP ERROR
...
contract JOBXTOKEN is Context, IBEP20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludeAddressFromMaxTx[from] && !_isExcludeAddressFromMaxTx[to]) {
            ...
            if(!_isExcludeAddressFromMaxTx[to] && (from == pancakeSwapV2Pair) && from != owner() && to != owner()) {
		        require(
                    amount <= _buyMaxTxAmount,
            ...
            if (!_isExcludedFromTransactionlock[from] && limitByDay && (to == pancakeSwapV2Pair) && (from != pancakeSwapV2Pair) && from != owner() && to != owner()) {
                ...
                if(!_isExcludeAddressFromMaxTx[from] && (to == pancakeSwapV2Pair) && from != owner() && to != owner()) {
		            require(
                        amount <= _sellMaxTxAmount2,
                        "Transfer amount exceeds the maxTxAmount"
                    );

                    uint256 contractBalanceRecipient = balanceOf(from);
                    require(
                        contractBalanceRecipient + amount >= _minWalletAmount,
                        "Exceeds minimum wallet amount"
                    );
