...
contract FomoMarketplace is Ownable, ReentrancyGuard {
    ...
    function buyItem(
        ...
        if (
            ...
            uint256 royaltyFee = msg.value.sub(feeAmount).mul(royalties[_nftAddress][_tokenId]).div(
                100
            );
            (bool royaltyTransferSuccess, ) = payable(minters[_nftAddress][_tokenId]).call{
                value: royaltyFee
            }("");
    ...
    function registerRoyalty(
        ...
        if (_supportERC721(_nftAddress)) {
            require(IERC721(fomo721).ownerOf(_tokenId) == _msgSender(), "Not owning the item.");
        } else if (_supportERC1155(_nftAddress)) {
            require(
                IFomoNFT1155(fomo721).creators(_tokenId) == _msgSender(),
                "Not owning the item."
            );
        } else {
            revert("Invalid NFT address.");
    ...
    function updateFomo721(address _fomo721) external onlyOwner {
        require(IERC165(_fomo721).supportsInterface(INTERFACE_ID_ERC721), "Not ERC721");
        fomo721 = _fomo721;
    }
