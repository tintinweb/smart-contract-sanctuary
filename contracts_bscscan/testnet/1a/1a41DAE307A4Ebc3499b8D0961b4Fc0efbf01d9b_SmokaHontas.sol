...
contract SmokaHontas is IERC20, EIP712, Ownable, TransactionThrottler {
    ...
    uint256 private constant MAX_FEE_RANGE = 100;
    uint256 private constant DIVIDER = 10000;

    uint256 public transferTax = 5;
    uint256 public bTax = 5;
    uint256 public sTax = 10;
    ...
    constructor(address _admin, address feecontract)
        EIP712(Constants.getName(), "1")
    {
    ...
    function _transfer(
        ...
        if (
            ...
            (bTax > 0 || sTax > 0 || transferTax > 0) &&
            !_excludedFromFees[sender] &&
            !_excludedFromFees[recipient]
        ) {
            fee = (((amount * _getTax(sender, recipient)) * 100) / DIVIDER);
    ...
    function setTransferTax(uint256 fee) external onlyOwner {
        require(fee <= MAX_FEE_RANGE, "Fee is outside of range 0-100");
        transferTax = fee;
        emit FeeBPS(transferTax);
    }
...
contract TransactionThrottler is Ownable {
    ...
    bool private _restrictionActive = false;
    uint256 private _tradingStart;
    uint256 private constant _delayBetweenTx = 5;
    ...
    function initAntibot() external onlyOwner {
        ...
        _tradingStart = block.timestamp + 100;
