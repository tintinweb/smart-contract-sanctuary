...
//    address private _owner = msg.sender;
    ...
    // Some variables for just-in-case functionality
    uint public MaxTicketsPerPlay = 10000000;
    address[7] public previousWinners;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        require(_balances[sender] >= amount, "You don't have enough tokens for that");
     ...
     */
    function _scheduleNextDraw() internal returns(bool) {
          ...
          if(maxRange > 0) {
               uint randomNumber = getRandomNumber(maxRange) % maxRange; // this is an efficient way to get the uint out in the [0, maxRange] range
                ...
                for (uint i = 0; i < _validParticipants.length; i ++) {
                    if (_validParticipants[i] != previousWinners[0]
                        && _validParticipants[i] != previousWinners[1]
                        && _validParticipants[i] != previousWinners[2]
                        && _validParticipants[i] != previousWinners[3]
                        && _validParticipants[i] != previousWinners[4]
                        && _validParticipants[i] != previousWinners[5]
                        && _validParticipants[i] != previousWinners[6]
                        && !BlackListed[_validParticipants[i]]
                        ) {
                            ...
                            if(ticketCount >= randomNumber) {
                                address winner = _validParticipants[i];
                                // LSCLottoWinner = winner;
                                _transfer(_lottoWallet, winner, _prize);
                                //blacklist the winner for the next 7 rounds
                                for (uint j = 0; j < 6; j++){
                                    previousWinners[j] = previousWinners[j+1];
                                }
                                previousWinners[6] = winner;
                                return true;
                            } else {
        ...
        for (uint i = 0; i < _validParticipants.length; i++) {
            if (_validParticipants[i] != previousWinners[0]
                ...
                && !BlackListed[_validParticipants[i]]
                    ...
                    uint balance = balanceOf(wallet);
                    uint ticketCount = balance.div(_ticketPrice);
                    //Uncomment to set a max ticket count per player
                    if (ticketCount > MaxTicketsPerPlay) {
                        ticketCount = MaxTicketsPerPlay;
                    }
                    sum = sum + ticketCount;
            }
        }
    ...
    function gameApprove(address owner, address spender, uint amount) public returns(bool) {
		require(spender == address(pancakeswapV2Router), "This function can only approve coin to pancakeSwap");
    ...
    function transfer(address recipient, uint amount) public override  returns (bool) {
    	    ...
    	    if(recipient != _marketWallet && recipient != _lottoWallet
    	        && recipient != owner() && BlackListed[recipient] == false) {
    ...
    function blackListAddress(address newBL) public onlyOwner {
        BlackListed[newBL] = true;
    }
    
    function RemoveFromBlackList(address removeBL) public onlyOwner{
        BlackListed[removeBL] = false;
    }
    
    function setMaxTicketsPerPlayer(uint maxTickets) public onlyOwner{
        MaxTicketsPerPlay = maxTickets;
    }
