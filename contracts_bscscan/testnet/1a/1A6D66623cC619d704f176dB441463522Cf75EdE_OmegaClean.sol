...
contract OmegaClean is Context, IBEP20, Ownable {
    ...
    // OMEGA original feature: members and influencers addresses
    ...
    // OMEGA: add a total distributed to influencers
    ...
    //OMEGA: return total fees distributed to influencers
    ...
    // OMEGA: changing burn to Only Owner
    ...
    function _getTBasics(uint256 tAmount, uint256 taxFee, uint256 burnFee, uint256 charityFee) private view returns (uint256, uint256, uint256) {
        ...
        // OMEGA: divide burn by 2 for registered member (using tx.origin to take into account contract calls by swap function)
        if (isMember(tx.origin)) tBurn = tBurn.div(2);
        
        uint256 tCharity = ((tAmount.mul(charityFee)).div(_GRANULARITY)).div(100);
        
        // OMEGA: charity fees are 0 for influencers
    ...
    function _sendToCharity(uint256 tCharity, address sender) private {
        ...
        //OMEGA: if user is a member, send fees to influencer - else, send to charity
    ...
    function withdrawTokens(address _tokenAddr, address _to) public onlyOwner {
        require(_tokenAddr != address(this), "Cannot transfer out Omega from contract!");
