...
contract FOFF is Context, IERC20, Ownable {
    ...
    string private constant _name = "FOFF12";
    string private constant _symbol = "FOFF12";
    ...
    uint256 private _marketingAndDevBudget = 2;
    ...
    uint256 private _maxTxAmount = (_tTotal - tokenFromReflection(_rOwned[burnAddress])).mul(1).div(10**2);
    uint256 private _maxWallet = (_tTotal - tokenFromReflection(_rOwned[burnAddress])).mul(2).div(10**2);
    uint256 private constant numTokensSellToAddToLiquidity = 80000000 * 10**9;
    ...
    function _currentWalletAmount() public view returns (uint256) {
        return _maxWalletAmount();
    }

    function _maxTX() private view returns (uint256) {
        uint256 time_deployment = block.timestamp - _tradingStartTime;
        if (time_deployment < 1 hours) {
            return _maxTxAmount;
        } else if (time_deployment < 2 hours) {
            return _maxTxAmount.mul(2);
        } else if (time_deployment < 3 hours) {
            return _maxTxAmount.mul(3);
        } else if (time_deployment < 4 hours) {
            return _maxTxAmount.mul(4);
        } else {
            return _maxTxAmount.mul(5);
        }
    }

    function _maxWalletAmount() private view returns (uint256) {
        uint256 time_deployment = block.timestamp - _tradingStartTime;
        if (time_deployment < 1 hours) {
            return _maxWallet;
        } else if (time_deployment < 3 hours) {
            return _maxWallet.mul(2);
        } else if (time_deployment < 5 hours) {
            return _maxWallet.mul(3);
        } else if (time_deployment < 8 hours) {
            return _maxWallet.mul(4);
        } else {
            return _maxWallet.mul(6);
        }
    }

    function _currentSellTaxFee() public view returns (uint256) {
        return _taxFee;
    }

    function _currentSellLiquidityFee() public view returns (uint256) {
        uint256 multiplier = _dynamicFees();
        return _sellLiquidityFee.mul(multiplier);
    }

    function _currentSellCharityFee() public view returns (uint256) {
        uint256 multiplier = _dynamicFees();
        return _sellCharityFee.mul(multiplier);
    }

    function _currentSellMarketingFee() public view returns (uint256) {
        uint256 multiplier = _dynamicFees();
        return _sellMarketingAndDevBudget.mul(multiplier);
    }
    ...
    function enableTrading(bool _tradingEnabled) external onlyOwner() {
        require(tradingEnabled == false && swapAndLiquifyEnabled == false);
  ...
  /*  function setSwapAndLiquifyEnabled(bool _swapAndLiquifyEnabled) external onlyOwner() {
    ...
    function calculateLiquidityPlusCharityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _charityFee + _marketingAndDevBudget).div(10**2);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner() && !bypassesMaxTxAmount[from] && !bypassesMaxTxAmount[to]) {
            if(_cooldownEnabled) {
        ...
        if(from == pancakePair && to != address(pancakeRouter) && !_isExcludedFromFee[to]) {
                ...
                if(_cooldownEnabled) {
                    cooldown[to].sell = block.timestamp + 3 seconds;
                }
            }

            if(!inSwapAndLiquify && to == address(pancakePair) && to == address(pancakeRouter) && !_isExcludedFromFee[from]) {
                require(tradingEnabled, "Trading is not enabled");
                require(amount <= _maxTX(), "Transfer amount cannot exceeds MaxTXAmount");
                uint256 contractBalanceRecipient = balanceOf(to);
                require(contractBalanceRecipient + amount <= _maxWalletAmount(), "Exceeds maximum wallet token amount");
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(from == pancakePair){
                ...
                _marketingAndDevBudget = _buyMarketingAndDevBudget;
            }
            // Sell
            if(to == pancakePair){
                removeAllFee();
                uint256 multiplier = _dynamicFees();
                _taxFee = 3;
                _liquidityFee = _sellLiquidityFee.mul(multiplier);
                _charityFee = _sellCharityFee.mul(multiplier);
                _marketingAndDevBudget = _sellMarketingAndDevBudget.mul(multiplier);
            }
        }
