...
contract HowlsCastle is Ownable, ReentrancyGuard {
    ...
    function pendingCalciferTotal(uint256 _pid, address _user) public view returns (uint256) {
    ...
    function pendingCalciferBoosted(uint256 _pid, address _user) public view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accCALCIFERPerShare = pool.accCALCIFERPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
    ...
    function payOrLockupPendingCALCIFER(uint256 _pid, bool _boost, uint256 _amount) internal {
        ...
        if (pending > 0 || user.rewardLockedUp > 0) {
            ...
            if (_boost || _amount == 0) { // User wanna boost or harvest
                ...
                CALCIFER.mint(address(this), boostAmount.add(user.boostLockedUp));
