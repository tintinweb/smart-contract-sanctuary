...
contract LoveDoge is ERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x6E0BD35c71E1D48e8f972c2118eb59F2aFF6e398); // Marketing Address
    address payable public buybackAddress = payable(0x753F0aa94873463517443c89d2ED2a72dac36123); // Buyback Address
    ...
    function swapTokens(uint256 contractTokenBalance) private {
        ...
        uint256 transferredBalance = afterBalance.sub(initialBalance);

        if(transferredBalance > min){
            transferredBalance = transferredBalance - min;
            uint256 remain = transferredBalance.div(liquidityFee).mul(marketingDivisor);
            // Send to Marketing address
            transferToAddressETH(marketingAddress, remain);  
            // Send to Buyback address
            transferToAddressETH(buybackAddress, transferredBalance.sub(remain));  
        }
    }
