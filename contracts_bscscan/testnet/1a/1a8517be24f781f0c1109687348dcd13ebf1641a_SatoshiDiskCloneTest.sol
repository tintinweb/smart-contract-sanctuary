...
contract SatoshiDiskCloneTest is Context, Ownable {
    ...
    function getFileInfos() public view returns (FileInfo[] memory) {
        return orderbook[_msgSender()];
    }
    
    function getFileInfo(uint index) public view returns (FileInfo memory) {
        return orderbook[_msgSender()][index];
    }
    
    function getFileName(uint index) public view returns (string memory) {
        return orderbook[_msgSender()][index].fileName;
    }
    
    function getFileHash(uint index) public view returns (string memory) {
        return orderbook[_msgSender()][index].fileHash;
    }
    
    function getPrice(uint index) public view returns (uint256) {
        return orderbook[_msgSender()][index].price;
    }
    ...
    function setAdminFee(string memory fee) external onlyOwner() {
        adminEmail = fee;
    }
