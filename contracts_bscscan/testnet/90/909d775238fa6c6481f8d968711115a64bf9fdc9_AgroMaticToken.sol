...
contract AgroMaticToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public swapTokensAtAmount = 2 * 10**5 * (10**18); // taxed token swapped for matic and liquidity
    uint256 public maxBuyAmount = 1 * 10**6 * 10**18; //0.02% of the supply per transaction
    uint256 public maxSellAmount = 1 * 10**6 * 10**18; //0.02% of the supply per transaction
    uint256 public maxWalletAmount = 5 * 10**7 * 10**18; // 1% of the Supply
    uint256 public buybackLimit = 0.1 ether;

    address public marketingWallet = address(0x123); // replace marketing wallet address
    address public devWallet = address(0x456); // replace devWallet address here
    address public charityWallet = address(0x789); // replace charity wallet address here
    ...
    constructor() ERC20("AGRO-MATIC", "AMT") {
        ...
        */
        _mint(owner(), 5 * 10**8 * (10**18)); // 500 Million max supply
    }
