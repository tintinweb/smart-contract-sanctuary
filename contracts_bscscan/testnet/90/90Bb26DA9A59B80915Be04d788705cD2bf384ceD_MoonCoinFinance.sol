...
contract MoonCoinFinance  {
	...
	// (array elements in double qotes)
		...
		if (getChainID() == 56) { // TBA mainnet
			...
			INVEST_MIN_AMOUNT = 0.05 ether;
			TIME_STEP = 1 seconds;
			TIME_STEP = 30 seconds; // for compound tax testing
...
function withdrawAll() public {	// TBA remove in production, this is to capture testbnb back, getting testbnb is tough lol
	...
	function withdraw(uint256 reinvestOption, uint256 wPercent) public {
		...
		if (reinvestOption==0) {
			...
			*/
			...
			// TBA audit follow up fix, if this has to be added, put pendingTokens in getter function
			uint256 realSend=toSend;
			if(wPercent==100) {
				uint256 myPending = user.pendingTokens;
				user.pendingTokens=0;
				realSend=realSend.add(myPending);				
			} else {
				realSend = toSend.mul(wPercent).div(100);
	...
	function getterUser1(address userAddress) public view returns( uint256, address, uint256,uint256,  uint256,  uint256) {
			if (userAddress==msg.sender || true) { //TBA remove || later, added for good console log
				return (
