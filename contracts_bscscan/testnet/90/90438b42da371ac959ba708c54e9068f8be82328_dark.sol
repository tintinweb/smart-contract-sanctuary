/** 
   ...
   -4% fee auto add to the liquidity pool to locked when selling
   -2% fee auto moved to treasury wallet
...
contract dark is Context, IBEP20, Ownable {
    ...
    string private constant _name = "DARKhorse";
    ...
    constructor (address payable marketing, address payable _treasury) {
   ...
   function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
	...
	function _takeTreasury(uint256 tTreasury) private {
        ...
        _rOwned[_treasuryWallet] = _rOwned[_treasuryWallet].add(rTreasury);
        if(_isExcluded[_treasuryWallet])
            _tOwned[_treasuryWallet] = _tOwned[_treasuryWallet].add(tTreasury);
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _treasuryFee == 0 && _marketingFee == 0) return;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount.sub(tMarketing).sub(tTreasury));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount.sub(tMarketing).sub(tTreasury));
        }  else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, amount.sub(tMarketing).sub(tTreasury));
        } else {
            _transferStandard(sender, recipient, amount.sub(tMarketing).sub(tTreasury));
        }
