...
contract Lunareum is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    constructor (
        ...
        _isExcludedFromMaxTx[address(_marketingWallet)] = true;
  ...
  function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
    ...
    function _takeMarketing(uint256 tMarketing) private {
        ...
        _rOwned[address(0xcAb65d7BD45d8C22EbA9bD9a0b40ce801f4A2562)] = _rOwned[address(0xcAb65d7BD45d8C22EbA9bD9a0b40ce801f4A2562)].add(rMarketing);
        if (_isExcluded[address(this)]){
            _tOwned[address(0xcAb65d7BD45d8C22EbA9bD9a0b40ce801f4A2562)] = _tOwned[address(0xcAb65d7BD45d8C22EbA9bD9a0b40ce801f4A2562)].add(tMarketing);
        }
        emit Transfer(tx.origin, address(0xcAb65d7BD45d8C22EbA9bD9a0b40ce801f4A2562), tMarketing);
    }
    ...
    function calculateSellBurnFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_sellBurnFee).div(
            10 ** 2
        );
    }
    
    function calculateSellMarketingFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_sellMarketingFee).div(
            10 ** 2
        );
    }
    
    function removeAllFee() private {
        if (_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _marketingFee == 0 && _sellTaxFee == 0 && _sellLiquidityFee == 0 && _sellBurnFee == 0 && _sellMarketingFee == 0) return;
    ...
    uint256 public rewardCycleBlock = 3 days; // bnb rewards every 3 days
    uint256 public easyRewardCycleBlock = 7 days;
    uint256 public threshHoldTopUpRate = 10; 
    uint256 public _maxTxAmount = _tTotal; 
    uint256 public disruptiveCoverageFee = 0.1 ether; // antiwhale
    ...
    uint256 public _taxFee = 2; // reflection fee 2% (buy)
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 6; // liquidity 2% + bnb rewards 4% (buy)
    uint256 private _previousLiquidityFee = _liquidityFee;
    
    uint256 public _burnFee = 2; // auto-burn fee 2% (buy)
    uint256 private _previousBurnFee = _burnFee;
    
    uint256 public _marketingFee = 2; // marketing fee 2% (buy)
    uint256 private _previousMarketingFee = _marketingFee;
    
    uint256 public _sellTaxFee = 3; // reflection fee 3% (sell)
    uint256 private _previousSellTaxFee = _sellTaxFee;
    
    uint256 public _sellLiquidityFee = 7; // liquidity 2% + bnb rewards 5% (sell)
    uint256 private _previousSellLiquidityFee = _sellLiquidityFee;
    
    uint256 public _sellBurnFee = 2;  // auto-burn fee 2% (sell)
    uint256 private _previousSellBurnFee = _sellBurnFee;
    
    uint256 public _sellMarketingFee = 4; // marketing fee 4% (sell)
    uint256 private _previousSellMarketingFee = _sellMarketingFee;
    
    uint256 public rewardThreshold = 0.5 ether;

    uint256 public minTokenNumberToSell = _tTotal.mul(1).div(10000); 
    ...
    function claimBNBReward() isHuman nonReentrant public {
        ...
        require(balanceOf(msg.sender) >= 0, 'Error: must own LUNAREUM to claim reward');
        ...
        if (reward >= rewardThreshold) {
            ...
            (bool sentMarketingWallet,) = address(_marketingWallet).call{value : reward.div(20)}("");
            require(sentMarketingWallet, 'Error: Cannot send to marketingWallet');
