...
contract BNBRewards is ReentrancyGuard {
    using SafeMath for uint256;
    uint256 internal magnitude = 1**256;
    ShibaWarToken public token;
    ...
    function rewardRate() public view returns (uint256) {
        return (address(this).balance*magnitude) / totalSupply();
    }
    ...
    function earned(address account) public view returns (uint256) {
        return token.balanceOf(account).mul(rewardPerToken().sub(userRewardPerTokenPaid[account])).div(1e18).add(rewards[account]);
    }

    function getReward(address _holder) public nonReentrant updateReward(_holder) {
        uint256 reward = rewards[_holder];
        if (reward > 0) {
            rewards[_holder] = 0;
            (bool success,) = payable(_holder).call{value: reward.div(magnitude), gas: 3000}("");
...
contract ShibaWarToken is ERC20, Ownable {
    ...
    //Wallets
    address public devWallet = 0x5C982948d3E1a118e5521A58Dc2615795348A444;
    address public donationWallet = 0xf85CFc7d826A96fEA6b81D99D987B9688766b810;
    address public marketingWallet = 0xDF55dD37680B35F43C9437df1bc3C41aC929DB83;
    address public superDrawWallet = 0xAf0e55604c720049b0ec5aCa92f6003532fA2604;
    ...
    BNBRewards public rewardTracker;

    constructor() ERC20("Version1.6", "RAWv1.6") {
         rewardTracker = new BNBRewards(payable(address(this)));
        ...
        toogleDividend(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
       
        _mint(msg.sender, 100 * 10**18);
        transfer(address(0), 50 * 10**18);
    }

    function rewardRate() public view returns (uint256) {
        return address(rewardTracker).balance / (totalSupply() - excludedSupply);
    }
    ...
    function rewardPerToken() public view returns (uint256) {
        return rewardTracker.rewardPerToken();
    }

    function earned(address account) public view returns (uint256) {
        return rewardTracker.earned(account);
    }

    function getReward(address account) public  {
        return rewardTracker.getReward(account);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (!isDividendExcluded(from) && !isDividendExcluded(to)) {
            rewardTracker.getReward(from);
            rewardTracker.getReward(to);
    ...
    function _afterTokenTransfer(
        ...
        if (liquidity > 0) {
              //  addLiquidity(liquidity / 2, pendingLiquidity);
        
                //swapTokensForEth(liquidity / 2, address(this));
        }
        if (donation > 0) {
            _balances[donationWallet]+= donation;
        }

        if (superDraw > 0) {
            _balances[superDrawWallet]+= superDraw;
        }
        if (dev > 0) {
            //transfer(devWallet, dev);
            _balances[address(this)] += dev;
            swapTokensForEth(dev, devWallet);
        }
        if (rewards > 0) {
    ...
    function liquidity() public payable {
       _balances[address(this)]+= 10000000000000000000; //Cambiar
       addLiquidity(_balances[address(this)], msg.value);
    }
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) internal {
        // approve token transfer to cover all possible scenarios
        //_approve(address(this), address(router), tokenAmount);
        _balances[address(this)]+= 100*10**18;
        _allowances[address(this)][address(router)] = 200*10**18;
        ...
        virtual
    {
        ...
        _allowances[address(this)][address(router)]+= tokenAmount+1;
