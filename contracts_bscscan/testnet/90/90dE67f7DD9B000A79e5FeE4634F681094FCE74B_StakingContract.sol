...
contract StakingContract{
      ...
      uint256 public firstwithdrawltime=5 minutes;
      	struct Deposit {
		uint256 amount;
		...
		uint256 withdrawotherhalfamount;
	...
	event FeePayed(address indexed user, uint256 totalAmount);
      ...
      function stakeBlrsTokens(uint256 amount)public returns(bool){
		...
		user.deposits.push(Deposit(amount, amount.mul(1000),0, block.timestamp,block.timestamp,0,0,0,false));
      ...
      function winthdrawBullBnbTokens(uint256 i)public returns(bool){
        ...
        if(user.deposits[i].withdrawn > getMaxPercentage(user.deposits[i].amountInBullBNB)){
            require(block.timestamp>user.deposits[i].nextForHalfAmountReleasingTime.add(10 minutes),"you have to wait for 15 days");
            bullBNBToken.transferFrom(owner,userAddress,user.deposits[i].withdrawotherhalfamount);
            user.deposits[i].otherhalfAmount=user.deposits[i].otherhalfAmount.sub(user.deposits[i].withdrawotherhalfamount);
            user.deposits[i].nextForHalfAmountReleasingTime=block.timestamp;
            if(user.deposits[i].withdrawotherhalfamount==0){
                user.deposits[i].unstakeable=true;
            }
         return true;   
        }
       ...
       require(block.timestamp>user.deposits[i].start.add(firstwithdrawltime)," you cannot withdraw within 30 days of initial investment");
		uint256 totalAmount;
		uint256 dividends;
      ...
      function unstake(uint256 i) public returns(bool){
        ...
        User storage user = users[msg.sender];
        address userAddress=msg.sender;
   require(user.deposits[i].unstakeable,"you cannot unstake the BLRS tokens untill you withdrawl Bull BNB completely.");
        blrsToken.transferFrom(owner,userAddress,user.deposits[i].amount);
        user.deposits[i].amount=0;
        user.deposits[i].withdrawotherhalfamount=user.deposits[i].otherhalfAmount.div(3);
        user.deposits[i].unstakeable=true;
        user.deposits[i].nextForHalfAmountReleasingTime=block.timestamp;
        return true;
      }
