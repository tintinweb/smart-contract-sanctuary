...
contract IBO {
    ...
    function initialize(address rasing, address offering, uint256 _price, address _adminAddress, address _feeAddress, uint256 _feeRate) external {
        require(msg.sender == factory, 'BenSwap: FORBIDDEN'); // sufficient check
        require(_feeRate < 1e12, 'Fee rate cannot be more than 100%!');
    ...
    function swap(uint256 _amount) public {
        require(!isConcluded, 'Has concluded!');

        uint256 result = _amount.mul(price).div(1e28);
        require(offeringToken.balanceOf(address(this)) >= result, 'Not enough balance!');

        uint256 fee = _amount.mul(feeRate).div(1e12);
       
        raisingToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        raisingToken.safeTransfer(feeAddress, fee);

        offeringToken.safeTransfer(address(msg.sender), result);
    ...
    function conclude() public {
        ...
        isConcluded = true;
        raisingToken.safeTransfer(address(msg.sender), raisingToken.balanceOf(address(this)));
...
contract IBOFactory {
    ...
    constructor(address _admin, IBEP20 _feeToken) public {
        adminAddress = _admin;
        feeToken = _feeToken;
    }
    ...
    function createIBO(address raisingToken, address offeringToken, uint256 _price, uint256 amount) external returns (address ibo) {
        bytes memory bytecode = type(IBO).creationCode;
        ...
        IBO(ibo).initialize(raisingToken, offeringToken, _price, msg.sender, adminAddress, feeRate);

        // creation fee
        feeToken.safeTransferFrom(address(msg.sender), address(feeToken), creationFee);

        // initial token amount, can be added simply by sending more to the ibo contract
        TransferHelper.safeTransferFrom(offeringToken, msg.sender, ibo, amount);
