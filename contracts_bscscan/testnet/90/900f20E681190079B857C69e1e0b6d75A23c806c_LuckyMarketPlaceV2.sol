...
contract LuckyMarketPlaceV2 is IBidNFT ,ISwapNFT, ERC721Holder, Ownable, Pausable {
   ...
   struct ListNFT {
    ...
    event ReadyToBidToken(address indexed user, uint256 indexed tokenId);
    event RevokeBidToken(address indexed user, uint256 indexed tokenId);    
    ...
    function readyToBidToken(uint256 _tokenId, uint256 _price, uint256 _step, uint256 _startDate, uint256 _endDate) public override whenNotPaused {
        ...
        _userBiddingTokens[address(_msgSender())].add(_tokenId);
         emit ReadyToBidToken(_msgSender(), _tokenId);
    }
    ...
    function revokeBidToken(uint256 _tokenId) public override whenNotPaused {
        ...
        delete _tokenBidders[_tokenId];
         emit RevokeBidToken(_msgSender(), _tokenId);    
    }
   ...
   function getOffersLength(uint256 _tokenId) public view returns (uint256) {
    ...
    function checkTokenIdListed(uint256 _tokenId) public view returns (bool){
        if(_listMap[_tokenId] == 0x0000000000000000000000000000000000000000){
            return false;
        }
        return true;
    } 
