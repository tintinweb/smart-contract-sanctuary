...
// File: contracts/src/libs/IERC165.sol
...
// File: contracts/src/libs/IERC721.sol
...
// File: contracts/src/libs/IERC721Receiver.sol
...
// File: contracts/src/libs/IERC721Metadata.sol
...
// File: contracts/src/libs/Address.sol
...
// File: contracts/src/libs/Strings.sol
...
// File: contracts/src/libs/ERC165.sol
...
// File: contracts/src/libs/ERC721.sol
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        // If there is no token URI, return tokenID as string.
...
// File: contracts/src/token/CumStarNFTToken/CumStarNFTTokenState.sol
...
contract CumStarNFTTokenState is CumStarNFTTokenModel, TokenOwnable, ERC721 {
    mapping (uint256 => Token) internal newBalances;
    mapping (address => Token[]) internal balancesMap;

    uint256 public nextTokenId = 1;

    // Token state Events
    event CumStarNFTTokenMinted(address indexed tokenOwner, uint256 tokenId);
    event CumStarNFTTokenNewStateAdded(address indexed tokenOwner, uint256 tokenId);

    /**
     * @dev Initializes CumStar NFT token state contract
     */
    constructor() ERC721("HUSH NFT", "HUSH") {}
    ...
    function mintToken(address owner, string memory tokenUri) external onlyToken returns (uint256) {
        uint256 tokenId = nextTokenId;
        _mint(owner, tokenId);
        _setTokenURI(tokenId, tokenUri);
        nextTokenId++;

        emit CumStarNFTTokenMinted(owner, tokenId);

        return tokenId;
    }

    function addNewNFTState(Token memory token) external onlyToken {
        newBalances[token.tokenId] = token;
        balancesMap[token.owner].push(token);
        emit CumStarNFTTokenNewStateAdded(token.owner, token.tokenId);
    }
}