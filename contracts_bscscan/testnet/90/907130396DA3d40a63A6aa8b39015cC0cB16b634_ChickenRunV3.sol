...
contract ChickenRunV3 is ERC721Upgradeable {
    ...
    function _random() internal view returns (uint) {
        uint256 _next_newItemId = _tokenIds.current();
        uint rand = uint(keccak256(abi.encodePacked(_next_newItemId)));   
        uint newItemId = rand % dna_modulus;
        return newItemId;
    }    
