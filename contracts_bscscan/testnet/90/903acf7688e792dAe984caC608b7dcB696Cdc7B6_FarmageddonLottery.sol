...
// File: contracts/interfaces/IFarmageddonLottery.sol
...
interface IPancakeSwapLottery {
     ...
     * @param _autoBurnDivisor: dividor for how much to burn 0=none, 1=all, 2=half, 3=1/3rd
     * @dev Callable by operator
     */
    function drawFinalNumberAndMakeLotteryClaimable(uint256 _lotteryId, bool _autoInjection, uint256 _autoBurnDivisor) external;
 ...
 */
contract FarmageddonLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInCake = 10000000000000000000000000000000000000 ether;
    uint256 public minPriceTicketInCake = 0.00000000000001 ether;

    uint256 public pendingInjectionNextLottery;

    uint256 public constant MIN_DISCOUNT_DIVISOR = 100;
    uint256 public constant MIN_LENGTH_LOTTERY = 1 hours - 59 minutes; // 1 hours
    uint256 public constant MAX_LENGTH_LOTTERY = 32 days + 15 minutes; // 31 days
    ...
    event NewOperatorAndTreasuryAndInjectorAddresses(address operator, address treasury, address burn, address injector);
        ...
        nonReentrant
    {
        ...
        _lotteries[_lotteryId].amountCollectedInCake = cakeToken.balanceOf(address(this));
     ...
     * @dev Callable by operator
     */
    function drawFinalNumberAndMakeLotteryClaimable(uint256 _lotteryId, bool _autoInjection,  uint256 _autoBurnDivisor)
        external
        override
        onlyOperator
        nonReentrant
    {
        ...
        // Transfer CAKE to treasury address and Burn
        amountToBurn = (amountToWithdrawToTreasury / _autoBurnDivisor);
        amountToSendToTreasury = (amountToWithdrawToTreasury - amountToBurn);

        if (amountToSendToTreasury > 0) {
            cakeToken.safeTransfer(treasuryAddress, amountToSendToTreasury);
        }

        if (amountToBurn > 0){
            cakeToken.safeTransfer(burnAddress, amountToBurn);
        }
     ...
     * @param _burnAddress: address of the Burn address ( token owner )
    ...
    function setOperatorAndTreasuryAndInjectorAddresses(
        ...
        require(_burnAddress != address(0), "Cannot be zero address");
        ...
        emit NewOperatorAndTreasuryAndInjectorAddresses(_operatorAddress, _treasuryAddress, _burnAddress, _injectorAddress);
    }
