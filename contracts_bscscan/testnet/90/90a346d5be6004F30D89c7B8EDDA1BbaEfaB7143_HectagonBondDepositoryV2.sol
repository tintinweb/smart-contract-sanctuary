...
interface INoteKeeper {
  ...
  function redeem(address _user, uint256[] memory _indexes, bool _sendgHECTA) external returns (uint256);
...
abstract contract FrontEndRewarder is HectagonAccessControlled {
  ...
  constructor(
    IHectagonAuthority _authority, 
    IERC20 _hecta
  ) HectagonAccessControlled(_authority) {
    hecta = _hecta;
  }
...
/* ========== REDEEM ========== */
   ...
   */
  function redeem(address _user, uint256[] memory _indexes, bool _sendgHECTA) public override returns (uint256 payout_) {
