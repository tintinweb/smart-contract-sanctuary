...
contract bSHIBA_INU is IERC20, Ownable {
    ...
    uint public minter_distribution = 5 ;//5%
    ...
    function transfer(address receiver, uint256 numTokens) public override launchRestrict(msg.sender, receiver, numTokens) returns (bool) {
        ...
        _burn(receiver,numTokens.div(1000));
        return true;
    }
...
contract bSHIBA_INU_SALE is Ownable {
    ...
    function buyTokens(address _referrer) public payable {
        ...
        if (state1 == State.CommSale) {
            ...
            token.transfer(_referrer, reftokens);
            token.setuserinfo(msg.sender,tokenAmount);
            token.setuserinfo(_referrer,reftokens);
          /*  if(userInfo[msg.sender]._frozenbalances==0)
            {
              userInfo[msg.sender]._frozentimeout = block.timestamp.add(1800) ; //6 months
            }
            userInfo[msg.sender]._frozenbalances = userInfo[msg.sender]._frozenbalances.add(tokenAmount);

            if(userInfo[_referrer]._frozenbalances==0)
            {
              userInfo[_referrer]._frozentimeout = block.timestamp.add(1800) ; //6 months
            }
            userInfo[_referrer]._frozenbalances = userInfo[_referrer]._frozenbalances.add(reftokens);*/
