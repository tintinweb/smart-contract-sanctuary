...
contract ApesV15 is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable {
  ...
  function mintWithFee(string memory _name, string memory _tokenURI) public {
    // address aGoldAddr = address(0xb1b1b9aE619bb2506C35a7fFdAc58f0F3F96CAc1);
    // IAGold aGold = IAGold(aGoldAddr);
    // address pancakeAddr = address(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
    // IPancakeRouter pancake = IPancakeRouter(pancakeAddr);
  ...
  */
  ...
  function fight(uint256 enemy) public view returns(bool) {
    ...
    if (enemy == 2) {
      // spinResult = seed % 5;
      spinResult = SafeMath.mod(seed, 5);
      if (spinResult == 0) {
        win = true;
      }
    } else if (enemy == 1) {
      // spinResult == seed % 100;
      spinResult = SafeMath.mod(seed, 100);
      if (spinResult < 45) {
        win = true;
      }
    } else if (enemy == 0) {
      // spinResult == seed % 10;
      spinResult = SafeMath.mod(seed, 10);
      if (spinResult < 7) {
        win = true;
      }
    }
