...
contract GENIVesting is Ownable {
    ...
    uint private constant SECONDS_PER_MONTH = 300;
    ...
    function setGeni(IERC20 newGeni) external onlyOwner {
    ...
    function distribute() external {
        ...
        if(amountForSeedSale > 0)
            geni.transfer(seedSales, amountForSeedSale);
        if(amountForPrivateSale > 0)
            geni.transfer(privateSales, amountForPrivateSale);
        if(amountForPublicSale > 0)
            geni.transfer(publicSales, amountForPublicSale);
        if(amountForAdvisorsAndPartners > 0)
            geni.transfer(advisorsAndPartners, amountForAdvisorsAndPartners);
        if(amountForTeamAndOperations > 0)
            geni.transfer(teamAndOperations, amountForTeamAndOperations);
        if(amountForMktAndCommunity > 0)
            geni.transfer(mktAndCommunity, amountForMktAndCommunity);
        if(amountForGameTreasury > 0)
            geni.transfer(gameTreasury, amountForGameTreasury);
        if(amountForFarmingAndStaking > 0)
            geni.transfer(farmingAndStaking, amountForFarmingAndStaking);
        if(amountForLiquidity > 0)
            geni.transfer(liquidity, amountForLiquidity);
        
        lastestDistributeMonth = month + 1;
    }
    
    function getAmountForSeedSale(uint month) internal view returns (uint amount) {
        uint maxAmount = 5000000 * 10 ** ERC20(address(geni)).decimals();
        uint publicSaleAmount = 250000 * 10 ** ERC20(address(geni)).decimals();
    ...
    function getAmountForPrivateSales(uint month) internal view returns (uint amount) {
        uint maxAmount = 10000000  * 10 ** ERC20(address(geni)).decimals();
        uint publicSaleAmount = 500000 * 10 ** ERC20(address(geni)).decimals();
        uint linearAmount = (maxAmount - publicSaleAmount) / 12;
        if (month == 0)
            amount = publicSaleAmount;
        else if (month >= 1 && month <= 6 || month > 18)
    ...
    function getAmountForPublicSales(uint month) internal view returns (uint amount) {
        uint maxAmount = 2000000  * 10 ** ERC20(address(geni)).decimals();
        uint linearAmount = maxAmount / 3;
        if (month >= 0 && month < 2)
            amount = linearAmount;
        else if (month == 2)
            amount = maxAmount - linearAmount * 2;
        else if (month > 2)
            amount = 0;
    }
    ...
    function getAmountForAdvisorsAndPartners(uint month) internal view returns (uint amount) {
        ...
        if (month >= 0 && month <= 6 || month > 18)
            amount = 0;
        else if (month >= 7 && month < 18)
            amount = linearAmount;
        else if (month == 18)
    ...
    function getAmountForTeamAndOperations(uint month) internal view returns (uint amount) {
        uint maxAmount = 15000000 * 10 ** ERC20(address(geni)).decimals();
        uint publicSaleAmount = 1500000 * 10 ** ERC20(address(geni)).decimals();
        uint linearAmount = (maxAmount - publicSaleAmount) / 18;
        if  (month >= 0 && month <= 5 || month > 24)
            amount = 0;
        else if (month == 6)
            amount = publicSaleAmount;
            ...
            amount = maxAmount - publicSaleAmount - linearAmount * 17;
    }
    
    function getAmountForMktAndCommunity(uint month) internal view returns (uint amount) {
        uint maxAmount = 20000000 * 10 ** ERC20(address(geni)).decimals();
    ...
    function getAmountForGameTreasury(uint month) internal view returns (uint amount) {
        uint maxAmount = 23000000 * 10 ** ERC20(address(geni)).decimals();
        uint linearAmount = maxAmount / 36;
        if (month > 35)
            amount = 0;
        else if (month >= 0 && month < 35)
