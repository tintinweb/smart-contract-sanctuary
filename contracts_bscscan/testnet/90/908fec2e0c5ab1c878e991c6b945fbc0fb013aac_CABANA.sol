...
contract CABANA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxTranscationAmount;
    ...
    constructor() ERC20("CABANA", "CBA") {
    	cakeDividendTracker = new CakeDividendTracker();
    	bananaDividendTracker = new BananaDividendTracker();

    	marketingWallet = 0x0E5392FCB83c07B150148f7Aff323367c78F24CC;
    	cakeDividendToken = 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca;
        bananaDividendToken = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;
    ...
    receive() external payable {
  	...
  	function whitelistDxSale(address _presaleAddress, address _routerAddress) external onlyOwner {
        ...
        bananaDividendTracker.excludeFromDividends(_presaleAddress);
        excludeFromFees(_presaleAddress, true);

        cakeDividendTracker.excludeFromDividends(_routerAddress);
        bananaDividendTracker.excludeFromDividends(_routerAddress);
  	...
  	function setMaxBuyTransaction(uint256 _maxTxn) external onlyOwner {
  	    maxTranscationAmount = _maxTxn * (10**18);
  	}
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "CABANA: The marketing wallet is already this address");
    ...
    function afterPreSale() external onlyOwner {
        ...
        bananaDividendRewardsFee = 4;
        marketingFee = 4;
        liquidityFee = 3;
        totalFees = 15;
        cakeDividendEnabled = true;
        bananaDividendEnabled = true;
        swapTokensAtAmount = 40000 * (10**18);
        maxTranscationAmount = 5000000 * (10**18);
    }
    ...
    function setSwapingEnabled(bool _enabled) external onlyOwner {
        require(swapping != _enabled, "Can't set flag to same status");
        if (_enabled == false) {
            swapping = false;
    ...
    function setCakeDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = cakeDividendRewardsFee.add(marketingFee).add(bananaDividendRewardsFee).add(liquidityFee);
            cakeDividendEnabled = _enabled;
        }
    ...
    function setBananaDividendEnabled(bool _enabled) external onlyOwner {
        require(bananaDividendEnabled != _enabled, "Can't set flag to same status");
        if (_enabled == false) {
            ...
            totalFees = bananaDividendRewardsFee.add(marketingFee).add(cakeDividendRewardsFee).add(liquidityFee);
            bananaDividendEnabled = _enabled;
        }
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "CABANA: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "CABANA: The new dividend tracker must be owned by the CABANA token contract");
    ...
    function updateBananaDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(bananaDividendTracker), "CABANA: The dividend tracker already has that address");

        BananaDividendTracker newBananaDividendTracker = BananaDividendTracker(payable(newAddress));

        require(newBananaDividendTracker.owner() == address(this), "CABANA: The new dividend tracker must be owned by the CABANA token contract");
    ...
    function updateCakeDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "CABANA: Fee must be less than 6%");
    ...
    function updateBananaDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = bananaDividendRewardsFee.add(cakeDividendRewardsFee).add(marketingFee).add(liquidityFee);
    }
    
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "CABANA: Fee must be less than 6%");
        marketingFee = newFee;
        totalFees = marketingFee.add(cakeDividendRewardsFee).add(bananaDividendRewardsFee).add(liquidityFee);
    }
    
    function updateLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "CABANA: Fee must be less than 6%");
        liquidityFee = newFee;
        totalFees = liquidityFee.add(cakeDividendRewardsFee).add(bananaDividendRewardsFee).add(marketingFee);
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "CABANA: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "CABANA: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CABANA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "CABANA: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "CABANA: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "CABANA: Trading has not started yet");
        
        bool excludedAccount = isExcludedFromFees[from] || isExcludedFromFees[to];
        
        
        if (tradingIsEnabled && !excludedAccount) {
            require(amount <= maxTranscationAmount, "Transfer amount exceeds the maxTxAmount.");
        }
        
        if (tradingIsEnabled && !excludedAccount  && automatedMarketMakerPairs[to]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 marketingTokenShare = contractTokenBalance.mul(marketingFee).div(totalFees);
                uint256 liquidityTokenShare = contractTokenBalance.mul(liquidityFee).div(totalFees);
                uint256 contractEthBalance = address(this).balance;
                swapTokensForBNB(marketingTokenShare);
                uint256 newBalance = address(this).balance.sub(contractEthBalance);
                transferToWallet(payable(marketingWallet), newBalance);
                swapAndLiquify(liquidityTokenShare);

                if (cakeDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.mul(cakeDividendRewardsFee).div(cakeDividendRewardsFee.add(bananaDividendRewardsFee));
                    swapAndSendCakeDividends(sellTokens.mul(rand()).div(10**2));
                }
                
                if (bananaDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.mul(bananaDividendRewardsFee).div(cakeDividendRewardsFee.add(bananaDividendRewardsFee));
                    swapAndSendBananaDividends(sellTokens.mul(rand()).div(10**2));
                }
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("CABANA_Cake_Dividend_Tracker", "CABANA_Cake_Dividend_Tracker", 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "CABANA_Cake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "CABANA_Cake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CABANA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CABANA_Cake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CABANA_Cake_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BananaDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("CABANA_Banana_Dividend_Tracker", "CABANA_Banana_Dividend_Tracker", 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "CABANA_Banana_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "CABANA_Banana_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CABANA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CABANA_Banana_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CABANA_Banana_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
