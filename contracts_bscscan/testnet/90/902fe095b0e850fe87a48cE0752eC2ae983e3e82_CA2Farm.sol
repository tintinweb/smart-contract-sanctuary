...
contract CA2Farm is Ownable{
    ...
    uint public Counter;
    uint public TotalReward = 100000000000000000;//0.1
    address public CA2LPFarm = 0x2D9Cb484df3B25396F213EA15394d1B744D562A9; //test
    ...
    function OwnerDeposit(uint amount, address _userAddress) public checkPhase onlyOwner{
        CA2Token.safeTransferFrom(_userAddress, address(this), amount);
        Insert(_userAddress, amount, false);
    }
    
    function Insert(address _userAddress, uint amount, bool _restake) internal{
        if(!_restake){
            if(UserRewards[_userAddress] > 0){
            Claim();
            }
        }
    ...
    function ExitOwner(address _userAddress) public checkPhase onlyOwner{
        ...
        CA2Token.safeTransfer(_userAddress, UserPoolStakeAmount[_userAddress][PoolId]);
    ...
    function Claim() public{
        ...
        if(UserPoolStakeAmount[msg.sender][PoolId - 1] > 0){
            Insert(msg.sender, UserPoolStakeAmount[msg.sender][PoolId - 1], false);
            UserPoolStakeAmount[msg.sender][PoolId - 1] = 0;
        }
        else{
            Insert(msg.sender, UserPoolStakeAmount[msg.sender][UserLastStakePool[msg.sender]], false);
    ...
    function ClaimCA2() external{
        ...
        CA2Token.safeTransfer(msg.sender, CA2UserReward);
    ...
    function OwnerClaim(address _userAddress) public onlyOwner{
        require(UserRewards[_userAddress] > 0, "No reward to claim");
        BUSD.safeTransfer(_userAddress, UserRewards[_userAddress]);
        ...
        if(UserPoolStakeAmount[_userAddress][PoolId - 1] > 0){
            Insert(msg.sender, UserPoolStakeAmount[_userAddress][PoolId - 1], false);
            UserPoolStakeAmount[_userAddress][PoolId - 1] = 0;
        }
        else{
            Insert(msg.sender, UserPoolStakeAmount[_userAddress][UserLastStakePool[_userAddress]], false);
            UserPoolStakeAmount[_userAddress][UserLastStakePool[_userAddress]] = 0;
        }
    }

    function OwnerClaimCA2(address _userAddress) external onlyOwner{
        require(UserRewards[_userAddress] > 0, "No reward to claim");
        uint balanceInContrat = CA2Token.balanceOf(address(this));
        convertPancake(UserRewards[_userAddress], address(BUSD), address(CA2Token));
        uint CA2UserReward = CA2Token.balanceOf(address(this)).sub(balanceInContrat);

        CA2Token.safeTransfer(_userAddress, CA2UserReward);
        emit RewardPaidInCA2(_userAddress, CA2UserReward);
        UserRewards[_userAddress] = 0;
        if(UserPoolStakeAmount[_userAddress][PoolId - 1] > 0){
            Insert(_userAddress, UserPoolStakeAmount[_userAddress][PoolId - 1], false);
            UserPoolStakeAmount[_userAddress][PoolId - 1] = 0;
        }
        else{
            Insert(_userAddress, UserPoolStakeAmount[_userAddress][UserLastStakePool[_userAddress]], false);
            UserPoolStakeAmount[_userAddress][UserLastStakePool[_userAddress]] = 0;
        }
    }
    ...
    function RestakeCA2LP() external{
        require(UserRewards[msg.sender] > 0, "No reward to claim");
        uint balanceInContrat = CA2Token.balanceOf(address(this));
    ...
    function OwnerRestakeCA2LP(address _userAddress) external onlyOwner{
        require(UserRewards[_userAddress] > 0, "No reward to claim");
        uint balanceInContrat = CA2Token.balanceOf(address(this));
        uint half = UserRewards[_userAddress];
        convertPancake(half, address(BUSD), address(CA2Token));
        convertPancake(half, address(BUSD), address(WBNB));
        uint CA2UserReward = CA2Token.balanceOf(address(this)).sub(balanceInContrat);
