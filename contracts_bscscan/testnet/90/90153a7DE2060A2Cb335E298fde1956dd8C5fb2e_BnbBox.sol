...
contract BnbBox is IERC20, Ownable {
    ...
    string constant _name = "BNBBox";
    string constant _symbol = "$BNBBOX";
    ...
    // sell fees
    uint256 public sellBnbBoxFee = 5;
    ...
    address public marketingFeeReceiver =
        0x046F1458e75C16141ea35eff17735aFf9ae7A3e7;
    address public lotteryWallet = 0x75dF5847982Bf7Ee6244799Fa69bD9022124515d;
    address public devFeeReceiver = 0x046F1458e75C16141ea35eff17735aFf9ae7A3e7;
    ...
    function updateBuyFees(
        ...
        buyDevFee = dev;
        buyBurnFee = burn;
        buyTotalFees = boxFee.add(marketing).add(liquidity).add(burn).add(dev);
    }
    ...
    function updateSellFees(
        ...
        sellBnbBoxFee = boxFee;
        sellMarketingFee = marketing;
        sellLiquidityFee = liquidity;
        sellDevFee = dev;
        sellBurnFee = burn;
        sellTotalFees = boxFee.add(marketing).add(liquidity).add(burn).add(dev);
    }
    ...
    function swapBackInBnb() internal swapping {
        ...
        uint256 boxBnb = swappedBnbAmount.mul(buyBnbBoxFee).div(buyTotalFees);
        uint256 marketingFeeBnb = swappedBnbAmount.mul(buyMarketingFee).div(
            buyTotalFees
        );
        uint256 devFeeBnb = swappedBnbAmount.sub(boxBnb).sub(marketingFeeBnb);
        // calculate reward bnb amount
        if (boxBnb > 0) {
            (bool boxBnbSuccess, ) = payable(lotteryWallet).call{
                value: boxBnb,
                gas: 30000
            }("");
            boxBnbSuccess = false;
    ...
    function changeLotteryWallet(address newWallet) external onlyOwner {
