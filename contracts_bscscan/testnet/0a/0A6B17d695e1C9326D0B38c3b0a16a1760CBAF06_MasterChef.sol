...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    modifier validatePoolById(uint256 _pid) {
        require(_pid < poolInfo.length, "MasterChef::validatePoolById::INVALID");
        _;
    }
    ...
    function add(
        ...
        require(poolId1[address(_lpToken)] == 0, "MasterChef::add::ALREADY_ADDED");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function set(
        ...
        uint256 _allocPoint,
        bool _withUpdate
    ) public validatePoolById(_pid) onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public validatePoolById(_pid) nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function withdraw(uint256 _pid, uint256 _amount) public validatePoolById(_pid) nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "MasterChef::withdraw::EXCESSIVE_AMOUNT");
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.rewardPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            safeTransferReward(msg.sender, pending);
    ...
    function emergencyWithdraw(uint256 _pid) public validatePoolById(_pid) nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function pendingReward(uint256 _pid, address _user) external view validatePoolById(_pid) returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
