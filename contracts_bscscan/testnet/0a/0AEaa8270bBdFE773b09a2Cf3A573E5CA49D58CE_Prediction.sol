...
contract Prediction is Pausable {
    ...
    constructor(
        ...
        addAdmin(address(0x178AE1346a6c8aE31F7da930D263499d54E3677b));
...
//        require(_startBlock >= block.number, "Cannot set start block");
    ...
    function getPredictionHistory(uint256 epoch, address user) public view returns
        (bool, Position, uint256, address[] memory, uint256[] memory, uint256[] memory) {
        Round storage round = rounds[epoch];
        ...
        uint256 predictAmount = 0;

        if (!info.participated) {
            return (false, Position.UP, 0, round.listToken, upAmount, downAmount);
        }
        ...
        return (info.claimed, info.position, predictAmount, round.listToken, upAmount, downAmount);
    }
    ...
    function isRoundClose(uint256 epoch) public view returns (bool) {
        return rounds[epoch].binanceCalled;
    }
