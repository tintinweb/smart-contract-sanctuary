...
contract FarmAuction is Ownable, ReentrancyGuard {
    ...
    modifier onlyOperator() {
        require(msg.sender == operatorAddress, "Management: Not operator");
        _;
    }
    ...
    event AuctionStart(uint256 indexed auctionId, uint256 startBlock, uint256 endBlock, uint256 leaderboard);
    event NewInitialBidAmount(uint256 initialBidAmount);
    ...
    function bid(uint256 _amount) external nonReentrant {
        ...
        if (auctionBids[currentAuctionId][msg.sender].totalAmount == 0) {
            require(_amount >= auctions[currentAuctionId].initialBidAmount, "Bid: Initial amount incorrect");
        }
     ...
     * @dev Callable by (currently, and previously whitelisted) bidders
     */
     ...
     * @param _accounts: addresses of the bidders
     * @dev Callable by operator.
     */
    function addWhitelist(address[] calldata _accounts) external onlyOperator {
     ...
     * @dev Callable by operator.
     */
    function removeWhitelist(address[] calldata _accounts) external onlyOperator {
        require((currentAuctionId == 0) || (auctions[currentAuctionId].status == Status.Close), "Auction: In progress");
     ...
     * @param _startBlock: start block of a round
     * @param _endBlock: end block of a round
    ...
    function startAuction(
        ...
        emit AuctionStart(currentAuctionId, _startBlock, _endBlock, _leaderboard);
    }

    /**
     * @notice Close an auction round, and store leaderboard
     ...
     * @param _auctionId: auction id
     * @param _winningBidders: array of addresses to claim
     * @dev Callable by owner.
     */
    function claimAuctionLeaderboard(uint256 _auctionId, address[] calldata _winningBidders)
        external
        onlyOwner
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < _winningBidders.length; i++) {
            BidInfo memory bidInfo = auctionBids[_auctionId][_winningBidders[i]];
            ...
            auctionBids[_auctionId][_winningBidders[i]].hasClaimed = true;
        }
    ...
    /**
     * @notice Allows an owner to set the Operator address (to run Auction rounds)
    ...
    /**
     * @notice Allows an owner to change the default bid on any auction
     * @param _initialBidAmount: amount of $Cake Token, for a minimal bid
     * @dev Callable by owner.
     */
    function setInitialBidAmount(uint256 _initialBidAmount) external onlyOperator {
        require((currentAuctionId == 0) || (auctions[currentAuctionId].status == Status.Close), "Auction: In progress");
        require(_initialBidAmount % uint256(10**19) == uint256(0), "Bid: Initial amount incorrect");

        initialBidAmount = _initialBidAmount;

        emit NewInitialBidAmount(_initialBidAmount);
    }

    /**
     * @notice Allows an owner to change the maximal length (expressed in block number) on any auction
