        ...
        ERC20("zz", "xx") 
{
    ...
    event NewRewardToken(ERC20 token);
    ...
    function getNftItemContract(address nftAddress)
        ...
        returns (ERC1155)
    {
        ERC1155 candidateContract = ERC1155(nftAddress);
    ...
    function depositChest(
        ...
        require(
            ...
            "Marketplace: token and amount not match"
        );
        
        ERC1155 nftContract = getNftItemContract(nftAddress);
        ...
        user.amountChest.add(amount.length);
    ...
    function withdraw() external nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        require(user.amount >= 125000000000000000000, "User does not stake enough BUSD");
        require(user.amountChest >= 10, "User does not stake enough Chest");
        require(
        ...
        uint256 lfwAmount = 500000000000000000000;
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            ERC20(rewardTokens[i]).transfer(address(msg.sender), lfwAmount);
        }
     ...
     * @param _amount: amount to withdraw (in rewardToken)
     */
    function claimNFT(address nftAddress) external validNFTContractAddress(nftAddress) {
        UserInfo storage user = userInfo[msg.sender];
        require(user.amount >= 125000000000000000000, "Address does not stake enough BUSD");
        require(user.amountChest >= 10, "Address does not stake enough Chest");
        require(
            unStakingBlock <= block.number,
            "Unstaking has not started"
        );
        ...
        uint256 randomSlot = random(0, 300);
        uint256 nftTokenId = nftIdRewards[randomSlot];
        nft.safeTransferFrom(address(this), msg.sender, nftTokenId);
        nftIdRewards[randomSlot] = nftIdRewards[nftIdRewards.length - 1];
        nftIdRewards.pop();
     ...
     * @dev Only callable by owner. Needs to be for emergency.
     */
    function ownerStakeWithdraw(uint256 _amount) external onlyOwner {
    ...
    function getUserAmountChest(address _usr) 
        external
        view
        returns (uint256)
    {
        UserInfo storage user = userInfo[_usr];
        return (user.amountChest);   
    }

    function getUserBUSDAmount(address _usr) 
        external
        view
        returns (uint256)
    {
