...
contract Fund is Watcher {
    ...
    bytes public priceEncodedData;

    constructor() {}

    function query() onlyOwner external {
        requestCounter.increment();
        watcherQuery(abi.encodePacked("bsc.", requestCounter.current()), QueryType.UpdatePrice, "");
    }
    
    function watcherCallback(bytes memory _reqId, QueryType _type, bytes calldata _calldata) external override {
        require(msg.sender == resolverAddress, "invalid resolver");
        if (_type == QueryType.UpdatePrice) {
            priceEncodedData = _calldata;
        }
        emit WatcherCallback();
    }

    function parsePriceData() public returns (PriceData memory priceData) {
        Obi.Data memory data = Obi.from(priceEncodedData);
        priceData.requestId = data.decodeString();
        priceData.aggregatedAt = data.decodeU256();
    } 
...
abstract contract Watcher is IWatcher, Ownable {

    address resolverAddress;

    event WatcherQuery(bytes _reqId, address _caller, QueryType _type, bytes _calldata);
    event WatcherCallback();

    function setResolverAddress(address _resolverAddress) public onlyOwner {
        resolverAddress = _resolverAddress;
    }

    function watcherQuery(bytes memory _reqId, QueryType _type, bytes memory _calldata) internal {
        emit WatcherQuery(_reqId, address(this), _type, _calldata);
    } 
...
interface IWatcher {
    function watcherCallback(bytes memory _reqId, QueryType _type, bytes calldata _calldata) external;
}
