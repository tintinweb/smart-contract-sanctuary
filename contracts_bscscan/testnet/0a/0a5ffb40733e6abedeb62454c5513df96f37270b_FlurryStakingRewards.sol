...
contract FlurryStakingRewards is IFlurryStakingRewards, BaseRewards {
     ...
     */
    function initialize(address flurryTokenAddr) public initializer {
    ...
    function claimReward() external override whenNotPaused nonReentrant claimerNotZeroAddr(_msgSender()) {
        claimRewardInternal(_msgSender());
    }

    function claimAllRewards() external override whenNotPaused nonReentrant claimerNotZeroAddr(_msgSender()) {
        lpStakingRewards.claimAllReward(_msgSender());
        rhoTokenRewards.claimAllReward(_msgSender());
        claimRewardInternal(_msgSender());
    }
    ...
    function grantFlurryInternal(address addr, uint256 amount) internal returns (uint256) {
        require(
    ...
    function isStakeholder(address addr) external view returns (bool) {
    ...
    function setLPRewardsContract(address lpRewardsAddr) external override onlyRole(DEFAULT_ADMIN_ROLE) whenNotPaused {
