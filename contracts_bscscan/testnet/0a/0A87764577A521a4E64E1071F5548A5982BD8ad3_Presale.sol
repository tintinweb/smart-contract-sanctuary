...
contract Presale is ReentrancyGuard, Context, Ownable {
    ...
    constructor (uint256 rate, address payable wallet, IERC20 token, uint256 tokenDecimals, address _pancakePair)  {
    ...
    //Start Pre-Sale
    function startICO(uint endDate, uint _maxPurchase, uint _softCap, uint _hardCap) external onlyOwner icoNotActive() {
        require(!presaleStarted);
        uint amountToBeTranferred = _hardCap.mul(_rate);
        availableTokensICO = amountToBeTranferred;
        _token.transferFrom(_msgSender(),address(this), amountToBeTranferred);
    ...
    function stopICO() external onlyOwner {
        require( presaleStarted);
        endICO = 0;
        presaleSuccess = _weiRaised >= softCap;
        if(presaleSuccess) {
    ...
    function claim() external icoNotActive {
        ...
        require(_token.balanceOf(pancakePair)>0, "Liquidity not added yet!");
    ...
    function refundMe() public icoNotActive{
        ...
        if(!startRefund && _weiRaised < softCap && presaleStarted)
        {
            startRefund = true;
        }
