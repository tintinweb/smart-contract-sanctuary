...
contract Faucet is Ownable {
    uint256  public tokenAmount = 500_000_000 * 10 ** 9;
    using SafeBEP20 for IBEP20;
    IBEP20 public token;

    mapping(address => bool) addressWithdrawn;
    ...
    function updateAmount(uint256 _tokenAmount) public onlyOwner {
        tokenAmount =  _tokenAmount;
    }

    function  claim() external {
        (bool ableToWithdraw, string memory message) = canWithdraw(msg.sender);
        require(ableToWithdraw, message);
        token.safeTransfer(address(msg.sender), tokenAmount);
        addressWithdrawn[msg.sender] = true;
    }

    function canWithdraw(address _address) public view returns (bool, string memory) {
        if(!whitelist[_address]) {
            return (false,"Not in whitelist");
        } else if(addressWithdrawn[_address]) {
