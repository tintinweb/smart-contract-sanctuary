...
contract lending_no_lock is Ownable {
    ...
    constructor(address dataFeedProxy_, address busd_, address usdt_, address usdc_) {
        ...
        returns (uint256)
    {
        ...
        return
            (priceUSDDecimal8 * _stakeAmount * 10**decimals * covertRatio) /
            10**8 /
            10**18 /
            10000;
    }
    ...
    function borrow(
        ...
        require(
            _stakeToken == BUSD || _stakeToken == USDT || _stakeToken == USDC,
            "not a valid stake"
        );
        borrowProd memory b = bProds[_prodId];
        ...
        IERC20(b.token).safeTransfer(_msgSender(), out);
        emit Borrow(
            _msgSender(),
