...
// File: contracts/interfaces/IAnpansProfile.sol

pragma solidity ^0.6.0;

/** @title IAnpansProfile.
 */
interface IAnpansProfile {
     ...
     * Callable only by owner admins.
     */
    function updateNumberAnpan() external;
...
/** @title AnpansBunnies.
 * @notice It is the contracts for AnpanSwap NFTs.
 */
...
contract BunnyMintingStation is AccessControl {
    ...
    /**
     * @notice Mint NFTs from the AnpansBunnies contract.
    ...
    function mintCollectible(
        ...
        uint256 tokenId = anpansBunnies.mint(_tokenReceiver, _tokenURI, _bunnyId);
        return tokenId;
    }
...
contract TradingCompV1 is Ownable {
    ...
    IAnpansProfile public anpansProfile;
    ...
    struct CompetitionRewards {
        ...
        uint256[5] anpanRewards; // anpan rewards per group
     ...
     * @param _anpansProfileAddress: AnpansProfile address
     * @param _bunnyStationAddress: BunnyMintingStation address
     * @param _anpanTokenAddress: the address of the ANPAN token
     */
    constructor(
        address _anpansProfileAddress,
        address _bunnyStationAddress,
        address _anpanTokenAddress
    ) public {
        anpansProfile = IAnpansProfile(_anpansProfileAddress);
        bunnyMintingStation = BunnyMintingStation(_bunnyStationAddress);
        anpanToken = IBEP20(_anpanTokenAddress);
    ...
    function claimReward() external {
        ...
        if (userRewardGroup > 0) {
            anpanToken.safeTransfer(senderAddress, userRewards.anpanRewards[userRewardGroup]);
        ...
        anpansProfile.increaseUserPoints(
            senderAddress,
     ...
     * @dev Only callable if the user has an active AnpansProfile.
     */
    function register() external {
        ...
        (, , userTeamId, , , isUserActive) = anpansProfile.getUserProfile(senderAddress);
    ...
    /**
     * @notice It allows the owner to claim the ANPAN remainder
     * @dev Only callable by owner.
     * @param _amount: amount of ANPAN to withdraw (decimals = 18)
     */
     ...
     * @param _anpanRewards: ANPAN rewards for each user group for teamId
     ...
     * @return userAnpanRewards: the ANPAN to claim/claimed
        ...
        )
    {
        ...
        if ((currentStatus != CompetitionStatus.Claiming) && (currentStatus != CompetitionStatus.Over)) {
            ...
            uint256 userAnpanRewards = _rewardCompetitions[userTeamId].anpanRewards[userRewardGroup];
            ...
            return (hasUserRegistered, hasUserClaimed, userRewardGroup, userAnpanRewards, userPointRewards, canClaimNFT);
        }
    }
