...
contract PIZZA is ERC721, Ownable {
    ...
    address public teamWallet = 0xB63D493d2F28a76296C63Bb4F10F1E818F97f9A6;
    address _forbuytoken = 0x39b3EEC7a4c71e0537292423372545840666F21c;

    address payable public _contractOwner;

    uint256 public teamWalletRate = 25;
    ...
    constructor() ERC721("THE PIZZACALLIONS", "PIZZA") {
        _contractOwner = msg.sender;
        token = IERC20(_forbuytoken);
    }

    function mint(string memory _tokenURI, uint _price) public returns (uint) {
        require(bytes(_tokenURI).length > 0, "tokenURI is not exist!");
        require(token.balanceOf(address(this)) >= mintPrice, "Not enought price!");
        uint256 teamWalletAmount = mintPrice * teamWalletRate / 1000;
        uint256 ownerAmount = mintPrice - teamWalletAmount;
        
        token.transferFrom(msg.sender, teamWallet, teamWalletAmount);
        token.transferFrom(msg.sender, owner(), ownerAmount);
    ...
    function teamWalletUpdate(address newAddr) public onlyOwner returns(bool){
        require(teamWallet != newAddr, "The team wallet address already exists!");
        teamWallet = newAddr;
        return true;
    }

    function teamWalletRateUpdate(uint256 newRate) public onlyOwner returns(bool){
        require(teamWalletRate != newRate, "There is already a value equal to the team percentage!");
        teamWalletRate = newRate;
        return true;
    }

    function mintPriceUpdate(uint256 newPrice) public onlyOwner returns(bool){
        require(newPrice != mintPrice, "There is already a value equal to the price!");
        mintPrice = newPrice;
        return true;
    }
}