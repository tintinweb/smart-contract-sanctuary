...
contract SquareToken is ERC20Capped, Ownable, AccessControlEnumerable {
    using SafeMath for uint256;
    using Address for address;

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    uint256 private constant SUPPLY = 5 * 10**6 * 10**9;
    uint256 private constant COMPANY_TOKEN = 50 * 10**4 * 10**9;
    uint256 private constant VESTING_TOKEN = 1 * 10**5 * 10**9;
    uint256 private constant SALE_TOKEN = 25 * 10**5 * 10**9;
    uint256 private constant ADDRESS_BUY_LIMIT = 50 * 10**9;

    bool private COMPANY_SHARE_CLAIMED = false;

    mapping(address=>uint256) private tokensSold;
    uint256 private TOTAL_SOLD = 0;

    address payable public immutable COMPANY_ADDRESS;
    address payable public immutable VESTING_ADDRESS;
    uint256 public immutable VESTING_END;

    uint256 private LAST_VESTED;

    //events
    event VEST_CLAIMED(address indexed _to, uint256 _amount );
    event TOKENS_BOUGHT(address indexed _to, uint256 _amount);
    event COMPANYSHARE_CLAIMED(address indexed _to, uint256 _amount);
    ...
    */
    constructor (string memory _name, string memory _ticker, address payable _company, address payable _vesting)
        ERC20(_name, _ticker) ERC20Capped(SUPPLY) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(MINTER_ROLE, _msgSender()); //the contract creator can mint

        COMPANY_ADDRESS = _company;
        VESTING_ADDRESS = _vesting;

        VESTING_END = block.timestamp + 157680000; //5 years
    ...
    function getTokensPurchased(address _by) public view returns(uint256){
        return tokensSold[_by];
    }
    ...
    function claimCompanyShare() external onlyOwner{
        require(COMPANY_SHARE_CLAIMED == false, "SquareToken: Company Share has already been claimed");
        //transfer company share
        _mint(COMPANY_ADDRESS, COMPANY_TOKEN);
        _moveDelegates(address(0), _delegates[COMPANY_ADDRESS], COMPANY_TOKEN);

        COMPANY_SHARE_CLAIMED = true;
        emit COMPANYSHARE_CLAIMED(COMPANY_ADDRESS, COMPANY_TOKEN);

    }

    function buy(address _to, uint256 _tokens) external onlyOwner {
        require(TOTAL_SOLD <= SALE_TOKEN, "SquareToken: Sale cap has been reached. You cannot buy more tokens.");

        require(tokensSold[_to] + _tokens <= ADDRESS_BUY_LIMIT, "SquareToken: You cannot buy more than 50 tokens.");

        TOTAL_SOLD = TOTAL_SOLD.add(_tokens);
        tokensSold[_to] = tokensSold[_to].add( _tokens);

        _mint(_to, _tokens);
        _moveDelegates(address(0), _delegates[_to], _tokens);

        emit TOKENS_BOUGHT(_to, _tokens);
    }

    function claimVest() external {
        require(_msgSender() == VESTING_ADDRESS || _msgSender() == owner(), "SquareToken: Invalid caller.");
        require(block.timestamp < VESTING_END, "SquareToken: Vesting period has ended");
        require(block.timestamp > (LAST_VESTED +  7890000) , "SquareToken: You cannot claim vest before 3 months have been completed.");

        LAST_VESTED = block.timestamp;
        _mint(VESTING_ADDRESS, VESTING_TOKEN);
        _moveDelegates(address(0), _delegates[VESTING_ADDRESS], VESTING_TOKEN);
        emit VEST_CLAIMED(VESTING_ADDRESS, VESTING_TOKEN);
    }
    ...
    */
    function mint(address _to, uint256 _amount) external {
        require(hasRole(MINTER_ROLE, _msgSender()), "SquareToken: must have minter role to mint");
        _mint(_to, _amount);
