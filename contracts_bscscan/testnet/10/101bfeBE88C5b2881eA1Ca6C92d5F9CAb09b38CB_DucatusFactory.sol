...
import "./DucatusPair.sol";

contract DucatusFactory is IDucatusFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(DucatusPair).creationCode));
    ...
    {
        require(tokenA != tokenB, "Ducatus: IDENTICAL_ADDRESSES");
        (address token0, address token1) =
            tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), "Ducatus: ZERO_ADDRESS");
        require(
            getPair[token0][token1] == address(0),
            "Ducatus: PAIR_EXISTS"
        ); // single check is sufficient
        bytes memory bytecode = type(DucatusPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IDucatusPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external override {
        require(msg.sender == feeToSetter, "Ducatus: FORBIDDEN");
        feeTo = _feeTo;
    }
