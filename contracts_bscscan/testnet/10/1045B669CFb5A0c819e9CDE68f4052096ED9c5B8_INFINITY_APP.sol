...
contract INFINITY_APP is Owned {
    struct User {
        uint id;
        address referrer;
        uint matrices_cnt;
        uint current_matrix;
        uint last_matrix;
        uint infinity_cooldown_time;
        uint infinity_cooldown_num;
    ...
    uint public lastInfinityId = 1;
    bool public useTokenPayment = true;

    uint public skippedInfinitysOffset;
    ...
    mapping(uint => InfinityLine) public infinity;
    ...
    constructor(address ownerAddress, address _tokenAddress, address[] memory _founders) Owned (ownerAddress) public {
        ...
        users[ownerAddress] = User({
            ...
            current_matrix: 0,
            last_matrix: 0,
            infinity_cooldown_time: 0,
            infinity_cooldown_num: 0,
            direct_referrals: 0
            });
        ...
        infinity[lastInfinityId] = InfinityLine({
    ...
    function receiveTransfer(address from, uint tokens, bytes memory data) public returns (string memory status) {
        ...
        if (data.length == 0) { //registration, purchase or renew cost
            ...
            if (!isUserExists(from)) {
                ...
                require(users[from].matrices_cnt < 150 || paymentQueue[from].matrix_to_renew != 0, "max 150 infinity allowed");

                if (!paymentQueue[from].is_eth_paid) {
                    paymentQueue[from].is_tokens_paid = true;
                    result = "Tokens accepted. Waiting ETH payment for Infinity purchase/renew";
                } else {
                    paymentQueue[from].is_eth_paid = false;
    ...
    function purchaseInfinityPosition() public payable {
        require(msg.value == regCost, "not correct purchase cost"); 
        require(isUserExists(msg.sender), "user not exists");
        require(users[msg.sender].matrices_cnt < 150, "max 150 Infinity allowed");
    ...
    function registration(address userAddress, address referrerAddress) private {
        ...
        joininfinity(lastMatrixId, userAddress, false);
    ...
    function purchase(address userAddress) private  {
        if (users[userAddress].infinity_cooldown_time < now - 86400) {
            users[userAddress].infinity_cooldown_time = now;
            users[userAddress].infinity_cooldown_num = 1;
        } else {
            if (users[userAddress].infinity_cooldown_num < 3) {
                users[userAddress].infinity_cooldown_num++;
            } else {
    ...
    function renew(address userAddress, uint matrixToRenew) private  {
        payUser(users[userAddress].referrer, regCost * 10 / 100); 
        payFounders(regCost * 10 / 100); 
        joininfinity(matrixToRenew, userAddress, true); 
        payForMatrix(matrices[matrixToRenew].matrix_referrer);
    }

    function joininfinity(uint matrixId, address matrixOwner, bool isRenew) private {
        infinity[lastInfinityId] = InfinityLine({
            matrix_id: matrixId,
            owner: matrixOwner
            });
        ...
        if (lastInfinityId % 2 == 0) {
            if (lastInfinityId > 2) {
                uint currentHp = skippedInfinitysOffset + lastInfinityId / 2 - 1;
                
                while (matrices[infinity[currentHp].matrix_id].bunch_cycles >= bunchCyclesLimit
                        && (lastInfinityId / 2 + skippedInfinitysOffset < lastInfinityId - 2)) { 
                    emit SkipMatrix(infinity[currentHp].matrix_id);
                    skippedInfinitysOffset++;
                    currentHp++;
                }

                payinfinity(infinity[currentHp].owner);
                joininfinity(infinity[currentHp].matrix_id, infinity[currentHp].owner, false);
                payForMatrix(matrices[infinity[currentHp].matrix_id].matrix_referrer);
            } else {
                payinfinity(owner);
                payForMatrix(0);
            }
    ...
    function fillMatrix(address user, address referrer, uint from_Infinity) private {
    ...
    function payinfinity(address user) private {
