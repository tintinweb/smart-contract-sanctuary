...
contract D3xRouter is IPancakeRouter02 {
    using SafeMath for uint;
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'D3xRouter: EXPIRED');
        _;
    }

    modifier allowAdminOnly(){
        require(msg.sender == swapAdmin, 'D3xRouter: ADMIN ACCESS ONLY');
        _;
    }
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'D3xRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'D3xRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'D3xRouter: INVALID_PATH');
    ...
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'D3xRouter: INVALID_PATH');
        amounts = PancakeLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'D3xRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        ...
        returns (uint[] memory amounts)
    {
        ...
        uint amountIn = _feeETHBeforeSwap(msg.value);
        amounts = PancakeLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountIn, 'D3xRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'D3xRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        uint amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'D3xRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function _getSwapFee() internal view returns (uint swapFee){
        address[] memory swapPath = new address[](2);
        swapPath[0] = BUSD;
        swapPath[1] = WETH;
        uint[] memory amounts = PancakeLibrary.getAmountsOut(factory, swapBUSDFee, swapPath);
        swapFee = amounts[1];
        return swapFee;
    }

    function _feeETHBeforeSwap(uint amountETHIn) internal view returns (uint amountETHOut) {
        uint swapFee = _getSwapFee();
        amountETHOut = amountETHIn.sub(swapFee);
        return amountETHOut;
    }

    function swapETHFee() public view returns (uint swapFee){
        return swapFee = _getSwapFee();
    }
