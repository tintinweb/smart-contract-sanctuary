...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
  ...
  // mapping for token creator
    ...
    */
    ...
    function _safeMint(address to, uint256 tokenId, address[] memory _royaltyAddress, uint256[] memory _royaltyfee, bytes memory _data) internal virtual {
        _mint(to, tokenId, _royaltyAddress, _royaltyfee);
        require(_checkOnERC721Received(address(0), to, tokenId, _data), "ERC721: transfer to non ERC721Receiver implementer");
    }

    function _mint(address to, uint256 tokenId, address[] memory _royaltyAddress, uint256[] memory _royaltyfee) internal virtual {
    ...
    function mintAndTransfer(address from, address to, uint256 itemId, address[] memory _royaltyAddress, uint256[] memory _royaltyfee, string memory _tokenURI, bytes memory data) public virtual override returns(uint256){
...
contract MobieNft721V2 is ERC721 {
    address private owner;
     ...
     * @param royalty address of the token to be minted.
     * @param royaltyFee uint256 royalty of the token to be minted.
     */

    function createCollectible(string memory tokenURI, address[] memory royalty, uint256[] memory royaltyFee, Sign memory sign) public returns (uint256) {
        require(!usedNonce[sign.nonce],"Nonce : Invalid Nonce");
        usedNonce[sign.nonce] = true;
        uint256 newItemId = tokenCounter;
        verifySign(tokenURI, msg.sender, sign);
        _safeMint(msg.sender, newItemId, royalty, royaltyFee);
