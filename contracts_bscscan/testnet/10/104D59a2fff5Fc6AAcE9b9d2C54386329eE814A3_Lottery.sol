...
contract Lottery is Ownable, Pausable {
    ...
    event AddDraw(uint256 drawNumber, Levels nftLevel);
    event AddTicketPoints(Levels nftLevel, uint256 points);
    event BuyTickets(uint256 drawNumber, uint256 ticketNumber, uint256 pointsSpent);
    ...
    function addDraw(uint256 _drawNumber, Levels _nftLevel) external onlyOwner returns(bool){
        ...
        emit AddDraw(_drawNumber, _nftLevel);
        return true;
    }
    ...
    function addTicketPoints(Levels _nftLevel, uint256 _points) external onlyOwner returns(bool){
        ...
        emit AddTicketPoints(_nftLevel, _points);
        return true;
    }
    ...
    function buyTicket(uint256 draw_number) external returns(bool){
        ...
        emit BuyTickets(draw_number, ticketCounter, ticketPoints[drawInfo[draw_number].nftLevel]);
