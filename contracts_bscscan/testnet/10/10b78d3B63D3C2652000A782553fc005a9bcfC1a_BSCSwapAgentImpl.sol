...
contract  BSCSwapAgentImpl is Context, Initializable {
    ...
    function initialize(address bep20Fac, uint256 fee, address payable ownerAddr, address _tlist) public initializer {
        bep20Factory = bep20Fac;
        swapFee = fee;
        owner = ownerAddr;
        tlist = _tlist;
    }
    ...
    */
    ...
    function createSwapPair(bytes32 ethTxHash, address erc20Addr, string calldata name, string calldata symbol, uint8 decimals) onlyOwner external returns (address) {
        ...
        ERC20Token token = ERC20TokenFactory(bep20Factory).createCloneToken(ERC20Token(0x0), 0, name, decimals, symbol, true, tlist);
