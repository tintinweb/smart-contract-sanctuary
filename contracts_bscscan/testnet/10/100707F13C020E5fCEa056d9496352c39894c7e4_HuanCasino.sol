...
contract HuanCasino is ERC1155Holder {
  ...
  function depositUyang(uint amount) public onlyOwner {
    require(amount > 0, "You deposit nothing!");
    IERC20 token = IERC20(uyangStone);
    require(token.transferFrom(tx.origin, address(this), amount), "Uyang transfer failed");
  ...
  function withdraw(uint amount) public {
    ...
    function bet_1(uint256 numberSelected) public payable {
        ...
        require(uyangPendingWithdrawals[address(this)] >= uyangAmout, "contract do not have enough uyang");
    ...
    function distributePrizes_1(uint256 numberWin) private {
        ...
        for (uint256 i = 0; i < players_1.length; i++) {
            ...
            if (playerInfo_1[playerAddress].numberSelected == numberWinner_1) {
                ...
                sendNftTo(
                    playerAddress,
                    numberMapNftId_1[(block.number % 5) + 1],
                    1
                );
                countWin++;
            } else {
    ...
    function pullFundsLowb(uint256 amout) public onlyOwner {
        ...
        require(
            lowb.transfer(owner, amout),
            "Lowb transfer failed"
        );
    }

    function pullFundsUyang(uint256 amout) public onlyOwner {
        IERC20 uyang = IERC20(uyangStone);
        require(
            uyang.transfer(owner, amout),
            "uyang transfer failed"
        );
    }
