...
contract GoldFurnace is ERC721Holder, Ownable, Divestor {
    ...
    mapping(uint8 => LevelInfo) levelInfoes;
    ...
    // constructor (address kmt_, address book_) {
    constructor () {
        meta.KMT = IERC20(0x7063B8CE05627301bC0bdBB390F0CB1C6B46d9A3);
        meta.KTA = IKTA(0x3D7bDcF5e0Bb389DE1c4D12Ee6a88B90E14c6486);
        levelInfoes[0] = LevelInfo({
            cardId: 110001,
            amount: 10 ether,
            orderNum: 0
        });
        levelInfoes[1] = LevelInfo({
            cardId: 110002,
            amount: 20 ether,
            orderNum: 0
        });
        levelInfoes[2] = LevelInfo({
            cardId: 110003,
            amount: 30 ether,
            orderNum: 0
        });
    ...
    function setLevelInfo(uint8 index_, uint cardId_, uint amount_) public onlyOwner {
        levelInfoes[index_].cardId = cardId_;
        levelInfoes[index_].amount = amount_;
    }

    function setMeta(address kmt_, address kta_) public onlyOwner {
        meta.KMT = IERC20(kmt_);
        meta.KTA = IKTA(kta_);
    }

    event Swap(address indexed user, uint8 indexed mode,  uint indexed tokenId, uint amount);
    // --------------------- onlyOwner end -----------------
    function swap(uint8 mode_, uint amount_) public onlyOpen returns (uint) {
        require(levelInfoes[mode_].amount != 0, "wrong amount");
        require(levelInfoes[mode_].amount == amount_, "wrong amount");
        
        uint cardId = levelInfoes[mode_].cardId;
        meta.KMT.safeTransferFrom(_msgSender(), address(this), amount_);
        uint tokenId = meta.KTA.mint(_msgSender(), cardId);
        
        meta.cardAmount += 1;
        meta.tokenAmount += amount_;
        levelInfoes[mode_].orderNum += 1;

        emit Swap(_msgSender(), mode_, tokenId, amount_);
        return tokenId;
    }

    function batchSwap(uint8[] calldata modes_, uint[] calldata amounts_) public  onlyOwner returns (bool) {
        for (uint i = 0; i < modes_.length; i++) {
            swap(modes_[i], amounts_[i]);
        }
        return true;
    }
