...
contract HIVE is Context, IERC20, Ownable ,Pausable{
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
     if(sender != owner() &&  recipient == uniswapV2Pair && !_isExcluded[sender] ) {
            _transferStandard(sender, recipient, amount);
        }else if(_isExcluded[sender]){
        _transExcluded(sender, recipient, amount);
        }
        else{
    ...
    function _transExcluded(address sender, address recipient, uint256 amount) internal {
        uint256 senderBalance = _balances[sender];
        unchecked {
            _balances[sender] = senderBalance.sub(amount);
        }
    ...
    function _getCalculatedFees(uint256 amount) internal view returns( uint256, uint256, uint256) {
        Fees memory fee;
        fee.HiveFee = amount.mul(_HiveFee).div(100);
        fee.TreasuryFee = amount.mul(_TreasuryFee).div(100);
    ...
    function _HiveTransfer(address sender, uint256 HiveFee) internal {
        if(HiveFee != 0) {
            _balances[Hive] = _balances[Hive].add(HiveFee);
            emit Transfer(sender, Hive, HiveFee);
        }
    }
