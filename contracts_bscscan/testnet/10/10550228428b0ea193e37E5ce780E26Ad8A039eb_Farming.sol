...
contract Farming is Ownable {
    ...
    bool public hasPoolLimit;
     ...
     // The requirePriceStaking
    uint256 public requirePriceStaking;
    ...
    constructor(uint256 _emissionRate, IERC20 _lpToken, uint256 _poolLimitPerUser, uint256 _poolCap, uint256 _requirePriceStaking) public {
        emissionRate = _emissionRate;
        lpToken = _lpToken;
        requirePriceStaking = _requirePriceStaking;
    ...
    function addMultiNFT(address contractAddress, uint256[] calldata ids, uint256 price) external onlyOwner {
        for(uint i=0; i< ids.length; i++){
            addNFT(contractAddress, ids[i], price);
        }
    }
    ...
    function stake(uint256 amount) external {
        ...
        require(amount == requirePriceStaking, "User must be requirePriceStaking");
     ...
     * @param _newRequirePriceStaking: RequirePriceStaking
     */
    function updateRequireStaking(uint256 _newRequirePriceStaking) external onlyOwner {
        requirePriceStaking = _newRequirePriceStaking;
    }
