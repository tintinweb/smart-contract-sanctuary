...
contract FlavorDripper0 is Context{
    ...
    function setMaxIterations_OAD(uint32 maxIterations_) public onlyAdmin{ maxIterations = maxIterations_;}

    function holdersListLength() public view returns (uint256) { return (holdersList.length);}
    function process_OFT(uint256 gas) external payable onlyFlavorsToken {
       ...
       emit procesSTART(holderCount, gasUsed, gasLeft, gas);
            while(gasUsed < gas && iterations < holderCount && iterations < maxIterations) {
                ...
                currentIndex++;
                iterations++;
                emit processEndLoop(holderCount,gasUsed,iterations,gasLeft,gas,currentIndex);
            }
        delete holderCount;
        delete gasUsed;
        delete gasLeft;
    ...
    event processEndLoop(uint256 holderCount, uint256 gasUsed, uint256 iteration, uint256 gasLeft, uint256 gas, uint256 currentIndex);
