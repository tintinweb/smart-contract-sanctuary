...
// will be transferred to a governance smart contract once BISON is sufficiently
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;         // How many allocation points assigned to this pool. BISONs to distribute per block.
        uint256 lastRewardBlock;    // Last block number that BISONs distribution occurs.
        uint256 accLifePerShare;    // Accumulated BISONs per share, times 1e12. See below.
    ...
    // BISON tokens created per block.
    uint256 public tokenPerBlock;
    // Bonus muliplier for early bison makers.
    ...
    // The block number when BISON mining starts.
    ...
    // Update the given pool's BISON allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending BISONs on frontend.
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 bisonReward = multiplier.mul(tokenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accLifePerShare = accLifePerShare.add(bisonReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 bisonReward = multiplier.mul(tokenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        bison.mint(devaddr, bisonReward.div(10));
        bison.mint(address(this), bisonReward);
        pool.accLifePerShare = pool.accLifePerShare.add(bisonReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for BISON allocation.
    ...
    // Safe bison transfer function, just in case if rounding error causes pool to not have enough BISONs.
    function safeLifeTransfer(address _to, uint256 _amount) internal {
        uint256 bisonBal = bison.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > bisonBal) {
            transferSuccess = bison.transfer(_to, bisonBal);
        } else {
            transferSuccess = bison.transfer(_to, _amount);
        }
    ...
    function setMinterStatus( address _minter, bool _status) external onlyOwner{
        bison.setMinterStatus(_minter, _status);
    }
