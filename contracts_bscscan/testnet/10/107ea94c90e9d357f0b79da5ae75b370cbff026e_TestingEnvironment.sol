...
contract TestingEnvironment is Context, Ownable {
    ...
    function addTrust(address account) public onlyOwner{

        _isTrusted[account] = true;
        
    }
    function removeTrust(address account) public onlyOwner{
    ...
    function UpdateWithdrawableBalanceIfNeeded() external onlyOwner{
    ...
    function callPercentage(address payable recipient) external {
        require(_isTrusted[recipient],"Cant withdraw, unauthorized account");
        uint256 transferAmount = currentWithrawableReference / _divisor;
        recipient.transfer(transferAmount);

    
    }
    function updatePercentage(uint256 newInt) external onlyOwner{
        _divisor = newInt;
