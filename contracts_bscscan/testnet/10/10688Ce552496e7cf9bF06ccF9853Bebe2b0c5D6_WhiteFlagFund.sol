...
/**  
    ...
    White Flag Fund Campaign - SCOP Community                                                                                                    
    Website: https://WhiteFlagFund.io
	Telegram: https://t.me/whiteflagfundWFF
	Twitter:  https://twitter.com/WhiteFlagFund
        Email:    [email protected]
    
   This WhiteFlagFund token created for charity during MCO. Govern by SCOP Community.  
   Code edited by @LemanCrypto	
   KitajagaKita AllahJagaKita

   
   Great features:
    -3% of each transaction contributes toward automatically generating liquidity locked inside PancakeSwap LP
    -5% of each transaction is converted to BNB and used for charity
    -2% each transaction is distributed to the holders automatically
...
contract WhiteFlagFund is Context, IBEP20, Ownable {
    ...
    string private _name = "WhiteFlagFund";
    string private _symbol = "WFF";
	...
	// (Charity + Liquidity).
    uint256 private totalLiqFee = liquidityFee.add(charityFee);
    ...
    constructor (address payable charityWallet, address pancakeSwapRouter) {
    ...
    function setLiquidityFeePercent(uint256 LiquidityFee) external onlyOwner() {
        ...
        totalLiqFee = liquidityFee.add(charityFee);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 ethForCharity = balanceDiff.mul(charityFee).div(totalLiqFee);
        uint256 ethForLiquidity = balanceDiff.sub(ethForCharity);
        
        addLiquidity(otherHalf, ethForLiquidity);
		emit SwapAndLiquify(half, ethForLiquidity, otherHalf);
		
		_charityWallet.transfer(ethForCharity);
    }
