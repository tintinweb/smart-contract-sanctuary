...
contract EviLPStaking is Ownable {
  using SafeMath for uint256;
    ...
    struct PoolInfo {
        IERC20 lpToken;
        uint256 perBlockEviAllocated;
        uint256 lastRewardBlock;
        uint256 accEviPerShare;
    }

    IERC20 public EviTokenContract;
    ...
    constructor(
        IERC20 _EviContractAddress,
        uint256 _startBlock
    ) {
        EviTokenContract = _EviContractAddress;
    ...
    function add(uint256 _EviPerBlock, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        poolInfo.push(PoolInfo({
            lpToken: _lpToken,
            perBlockEviAllocated: _EviPerBlock,
            lastRewardBlock: lastRewardBlock,
            accEviPerShare: 0
            }));
    }

    function set(uint256 _poolId, uint256 _EviPerBlock, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
        poolInfo[_poolId].perBlockEviAllocated = _EviPerBlock;
    }
    ...
    function fund(uint256 _amount) public {
        ...
        require(EviTokenContract.balanceOf(_from) >= _amount, 'fund: expected an address that contains enough Evi for Transfer');
        EviTokenContract.transferFrom(_from, address(this), _amount);
    ...
    function pendingEviReward(uint256 _poolId) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 EviReward = multiplier.mul(pool.perBlockEviAllocated);
    ...
    function harvest(uint256 _poolId) external returns (uint256) {
        ...
        if (user.amount > 0) {
            ...
            safeEviTransfer(msg.sender, pending);
            return pending;
        }
        return 0;
    }

    function getLockedEviView() external view returns (uint256) {
        return EviTokenContract.balanceOf(address(this));
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        safeEviTransfer(address(msg.sender), pending);
    ...
    function safeEviTransfer(address _to, uint256 _amount) internal {
        ...
        uint256 EviBal = EviTokenContract.balanceOf(_from);
        if (_amount > EviBal) {
            EviTokenContract.transfer(_to, EviBal);
        } else {
            EviTokenContract.transfer(_to, _amount);
        }
    }
}