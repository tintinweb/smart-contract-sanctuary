...
contract ElonCoin is IBEP20, Auth {
    ...
    string constant _name = "ElonCoin";
    string constant _symbol = "ELONCOIN";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 100_000_000_000 * (10**_decimals);
    uint256 public _maxTransactionAmount = _totalSupply.div(400); // 0.25%
    ...
    uint256 buybackMultiplierDuration = 30 minutes;
    ...
    function _transferFrom(
        ...
        if (sender == preSaleContract || inSwap) {
    ...
    function checkTransactionLimit(address sender, uint256 amount)
        internal
        view
    {
        require(
            amount <= _maxTransactionAmount || isTransactionLimitExempt[sender],
    ...
    function getMultipliedFee() public view returns (uint256) {
        if (launchedAtTimestamp + 1 days > block.timestamp) {
            ...
            buybackMultiplierTriggeredAt.add(buybackMultiplierDuration) >
            block.timestamp
        ) {
            uint256 remainingTime = buybackMultiplierTriggeredAt
                .add(buybackMultiplierDuration)
                .sub(block.timestamp);
            ...
            return
                totalFee.add(
                    feeIncrease.mul(remainingTime).div(
                        buybackMultiplierDuration
                    )
                );
        }
        return totalFee;
        ...
        authorized
    {
        ...
        if (triggerBuybackMultiplier) {
            ...
            emit BuybackMultiplierActive(buybackMultiplierDuration);
        }
    }
    ...
    function triggerAutoBuyback() internal {
        ...
        if (autoBuybackAccumulator > autoBuybackCapacity) {
    ...
    function setBuybackMultiplierSettings(
        ...
        buybackMultiplierDuration = duration;
    }
    ...
    function setIsTransactionLimitExempt(address holder, bool exempt)
        external
        authorized
    {
    ...
    function setPreSaleContract(address _preSaleContract) external authorized {
