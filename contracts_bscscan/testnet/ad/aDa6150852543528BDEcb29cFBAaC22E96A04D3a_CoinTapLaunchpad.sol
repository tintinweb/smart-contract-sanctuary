...
contract CoinTapLaunchpad is ICoinTapLaunchpad, Ownable, Pausable, Whitelist {
    ...
    address constant BURN_ADDRESS = address(0x000000000000000000000000000000000000dEaD); // 90%
    ...
    function finalize() public override onlyOwner {
      ...
      uint256 amountBNB = capacityBNB.mul(98).div(100);
      payable(owner()).transfer(amountBNB);

      // Get remain BNB include fee emergency withdraw and 2% capacityBNB
      payable(FACTORY_ADDRESS).transfer(address(this).balance);
     ...
     */
    function updatePresaleType(SharedConstants.PresaleType _presaleType) public override onlyOwner {
     ...
     * @dev Buy token
     */
    function buyToken() public override payable checkWhoCanBuy {
    ...
    function _handleUnsoldToken() internal {
      IERC20 token = IERC20(launchpadData.token);
      uint256 remainBNBCap = launchpadData.hardCap.sub(capacityBNB);
      uint256 remainToken = remainBNBCap.mul(launchpadData.presaleRate).div(SharedConstants.DECIMAL_OF_NATIVE_CURRENCY);
      if(launchpadData.unsoldToken == SharedConstants.UnsoldToken.REFUND) {
        token.transfer(owner(), remainToken);
      }
    ...
    modifier checkWhoCanBuy() {
      // public
      if(launchpadData.presaleType == SharedConstants.PresaleType.PUBLIC) {
        _;
        return;
      }

      // whitelist
      if(launchpadData.presaleType == SharedConstants.PresaleType.WHITELIST) {
        require(isWhitelisted(_msgSender()),"Whitelist: address does not exist");
        _;
        return;
      }
    }
...
interface ICoinTapLaunchpad { 
  ...
  function updatePresaleType(SharedConstants.PresaleType _presaleType) external;
  /* Paticipant */
