...
contract ConsoleStakeV2 is Ownable {
  ...
  uint256 constant DECADE_PERIOD = 2; // IN DAYS
  uint256 constant INIT_TIME = 13813445; // GAME TOKEN DEPLOYMENT TIMESTAMP
  ...
  constructor(IMintableERC20 _gameToken, ConsoleStake _oldStake) { 
  ...
  function calcReward(uint256 _tokenId) public view returns (uint256, uint256) {
    ...
    (lastClaim, ) = oldStake.stakes(_tokenId);
