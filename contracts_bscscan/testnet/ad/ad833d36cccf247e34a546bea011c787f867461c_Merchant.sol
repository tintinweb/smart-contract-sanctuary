...
contract Merchant is Pausable {
    ...
    uint16 public transactionFee = 500; // Transaction fee multiplied by 0.5%
    ...
    // How to process fee
    //0 - 100% to marketing address
    //1 - 50% of staked fee is added to WPAY-BNB liquidity and sent to the marketing address, 50% of staked fee is swapped to WPAY tokens to be sent to the marketing address

    uint8 public feeProcessingMethod = 0;
    ...
    function handleFee() internal {
        ...
        if (feeProcessingMethod == 0) {
            marketingWallet.transfer(etherBalance);
        }
        if (feeProcessingMethod == 1) {
            require(
    ...
    function swapTokenToUsdtAndTransferToMerchantWallet(
        ...
        require(
         ...
         token.approve(address(pancakeV2Router),_amountIn);
    ...
    function updatefeeProcessingMethod(uint8 _method) external onlyOwner {
        require(
            _method == 0 || _method == 1,
            "Invalid withdraw fee method set"
        );
        require(_method != feeProcessingMethod, "Already set");
    ...
    {
        uint256 finalFee = 0;
        uint256 feeAmount = amountIn.mul(transactionFee).div(10000);
        uint256 wpayBalanceOfCaller = WPAY_TOKEN.balanceOf(_msgSender());
        if (wpayBalanceOfCaller >= wpayBalanceForFreeTransaction || feeAmount == 0) {
            return finalFee;
        } else {
