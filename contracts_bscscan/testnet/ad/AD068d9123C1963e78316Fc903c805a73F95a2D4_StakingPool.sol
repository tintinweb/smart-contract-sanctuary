...
contract StakingPool is IStakingPool, Ownable {
    ...
    uint[10] expValue = [0, 2080, 15474, 83592, 330918, 965034, 2334044, 5289635, 10577966, 18405989];
    uint[10] earnExpToUps = [100, 516, 3194, 14093, 43772, 94502, 204022, 440470, 652003, 965124];
    ...
    /**
     * @dev Owner set KABY_TO_EXP value
     */
    function setPayAmountForEarnedExp(uint amount) external onlyOwner{
        KABY_TO_EXP = amount;
    }
     ...
     */
    function searchLevelIndexByExp(uint[10] memory levelExp, uint totalExp) private returns(uint){
        uint levelIndex = 1;
        uint maxLevelIndex = 90;
        if(totalExp >= levelExp[9] * 1e18)
        {
    ...
    function getMaxLevelToUp(
        ...
        uint[10] memory _levelExp,
        uint[10] memory _earnExpToUps) private returns(uint){
        ...
        if (totalExp >= 100 * 1e18 && totalExp < 220 * 1e18)
        {
            maxLevel = 2;
            heroCurrentExp[heroId] = 100 * 1e18;
            heroEarnedExp[heroId] = totalExp - heroCurrentExp[heroId];
            return maxLevel;
        }
        if (totalExp >= (expValue[currentStar]) * 1e18) {            
            maxLevel = currentStar * 10;            
            heroEarnedExp[heroId] = 0;
            heroCurrentExp[heroId] = expValue[currentStar] * 1e18;
            return maxLevel;
        }
        if (levelIndex < 20)
        {
        ...
        uint minExp = _levelExp[levelIndex / 10] * 1e18;
        uint minExpToLevelUp = _earnExpToUps[levelIndex / 10] * 1e18;
        
        for (uint i = levelIndex; i < levelIndex + 10; i++)
        {
            if (totalExp >= (minExp + minExpToLevelUp))
            {
                maxLevel++;
    ...
    function earned(uint heroId, address account) public view override returns (bool canUpgradeStar, uint expEarned, uint tokenEarned) {
        ...
        expEarned = heroLevel[heroId] < maxHeroLevel ? stakingDuration * BASE_APY * KABY_TO_EXP / ONE_YEAR_IN_SECONDS / 100 : 0;
        tokenEarned = stakingDuration * BASE_APY / ONE_YEAR_IN_SECONDS / 100;
    }
