...
//  // added nftContract param in function to support generalization
 ...
 // added nftContract param in function to support generalization
  ...
  function buyNFT(address nft_a,uint256 tokenId, string memory ownerId, uint256 currencyType) isValid(nft_a) public{
        ...
        uint256 price;
        uint256 mainPerecentage;
        uint256 authorPercentage;
        if(nft_a == address(this) && _tokenAuthors[tokenId][address(this)]._address == admin && adminOwner[temp.seller] && adminDiscount > 0){ // getAuthor() can break generalization if isn't supported in Collection.sol. SOLUTION: royality isn't paying for user-defined collections
        temp.price = temp.price- ((temp.price * adminDiscount) / 1000);
        }
        price = temp.price;
        IERC20 token = currencyType == 1 ? BUSD : ALIA;
        if(currencyType == 0 && temp.currencyType == 1){
         (uint112 _reserve0, uint112 _reserve1,) =LPBNB.getReserves();
         (uint112 reserve0, uint112 reserve1,) =LPAlia.getReserves();
         price = SafeMath.div(SafeMath.mul(SafeMath.mul(temp.price,reserve1), _reserve1),SafeMath.mul(_reserve0,reserve0));
         token = ALIA;
        } else if(currencyType == 1 && temp.currencyType == 0){
          (uint112 _reserve0, uint112 _reserve1,) =LPBNB.getReserves();
         (uint112 reserve0, uint112 reserve1,) =LPAlia.getReserves();
         price = SafeMath.div(SafeMath.mul(SafeMath.mul(temp.price,reserve0), _reserve0),SafeMath.mul(_reserve1,reserve1));
         token = BUSD;
        } else if (currencyType == 0 && temp.currencyType == 2) {
          (uint112 reserve0, uint112 reserve1,) =LPAlia.getReserves();
         price = SafeMath.div(SafeMath.mul(temp.price,reserve0),reserve1);
         token = ALIA;
        }else if (currencyType == 1 && temp.currencyType == 2) {
          (uint112 reserve0, uint112 reserve1,) =LPBNB.getReserves();
         price = SafeMath.div(SafeMath.mul(temp.price,reserve1),reserve0);
         token = BUSD;
        }
  ...
  function buyNFTBnb(address nft_a,uint256 tokenId, string memory ownerId) isValid(nft_a) payable public{
        ...
        if(nft_a == address(this) && author._address == admin && adminOwner[temp.seller] && adminDiscount > 0){ // getAuthor() can break generalization if isn't supported in Collection.sol. SOLUTION: royality isn't paying for user-defined collections
        temp.price = temp.price- ((temp.price * adminDiscount) / 1000);
        }
         price = temp.price;
        ...
        if(_tokenAuthors[tokenId][nft_a].royalty > 0 && nft_a == address(this)) { // royality for XNFT only (non-user defined collection)
          mainPerecentage = SafeMath.sub(SafeMath.sub(1000,author.royalty),platformPerecentage); //50
          authorPercentage = _tokenAuthors[tokenId][nft_a].royalty;
        } else {
          mainPerecentage = SafeMath.sub(1000, platformPerecentage);
        }
        address  blindRAddress =getBoxAddress(tokenId);
        if(blindRAddress == address(0x0)){
          blindRAddress=author._address;
