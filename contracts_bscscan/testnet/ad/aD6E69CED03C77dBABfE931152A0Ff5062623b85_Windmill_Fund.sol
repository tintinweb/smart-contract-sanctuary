...
event GasLog_Windmill_Fund(uint256 line, uint256 gas);
    ////
    ////
    ////
    //////////////// Public variables ////////////////
...
emit GasLog_Windmill_Fund(58, gasleft());
        updaterAddress = addr;
emit GasLog_Windmill_Fund(59, gasleft());
    }
    
    function setWithdrawFees(uint256 numerator, uint256 denominator) external onlyRole(DAO_ROLE){
emit GasLog_Windmill_Fund(62, gasleft());
        require(denominator>0, "Windmill_Fund: Denominator cannot be null");
emit GasLog_Windmill_Fund(63, gasleft());
        withdrawFees.numerator = numerator;
emit GasLog_Windmill_Fund(64, gasleft());
        withdrawFees.denominator = denominator;
emit GasLog_Windmill_Fund(65, gasleft());
    }

    function getWithdrawFees() external returns(Fraction memory){
emit GasLog_Windmill_Fund(68, gasleft());
        return withdrawFees;
emit GasLog_Windmill_Fund(69, gasleft());
    }
    
    function migrateFunds(address newfund) external onlyRole(DAO_ROLE){
emit GasLog_Windmill_Fund(72, gasleft());
        uint256 balance = BUSDToken.balanceOf(thisAddr);
emit GasLog_Windmill_Fund(73, gasleft());
        BUSDToken.transfer(newfund, balance);
emit GasLog_Windmill_Fund(74, gasleft());
    }

    function getBNBForGasRefund(uint256 amountBNB) external onlyRole(UPDATER_ROLE){
emit GasLog_Windmill_Fund(77, gasleft());
        address[] memory path = new  address[](2);
emit GasLog_Windmill_Fund(78, gasleft());
        path[0] = address(BUSDToken);
emit GasLog_Windmill_Fund(79, gasleft());
        path[1] = address(WBNBToken);
emit GasLog_Windmill_Fund(80, gasleft());

        uint256 amountBUSD = pancakeRouter.getAmountsIn(amountBNB, path)[0];
emit GasLog_Windmill_Fund(82, gasleft());

        require(BUSDToken.balanceOf(thisAddr) >= amountBUSD, "Windmill_Fund: BUSD reserve too small");
emit GasLog_Windmill_Fund(84, gasleft());

        BUSDToken.approve(address(pancakeRouter), amountBUSD);
emit GasLog_Windmill_Fund(86, gasleft());
        pancakeRouter.swapTokensForExactETH(amountBNB, amountBUSD, path, thisAddr, block.timestamp);
emit GasLog_Windmill_Fund(87, gasleft());

        payable(updaterAddress).transfer(amountBNB);
emit GasLog_Windmill_Fund(89, gasleft());
    }

    function getFundBUSD() public returns (uint256){
emit GasLog_Windmill_Fund(92, gasleft());
        uint256 nbBUSD = BUSDToken.balanceOf(thisAddr);
emit GasLog_Windmill_Fund(93, gasleft());
        
        uint256 l = tradeManager.getNbOpenTrades();
emit GasLog_Windmill_Fund(95, gasleft());
        for(uint256 i=0; i<l; i++){
emit GasLog_Windmill_Fund(96, gasleft());
            (I_Windmill_Trade_Manager.TradeData memory data,) = tradeManager.getOpenTrade(i);
emit GasLog_Windmill_Fund(97, gasleft());
            nbBUSD += data.trade.estimateBUSDBalance();
emit GasLog_Windmill_Fund(98, gasleft());
        }

        return nbBUSD;
emit GasLog_Windmill_Fund(101, gasleft());
    }

    function sendBUSDToTrade(I_Windmill_Trade_Abstract trade, uint256 nbBUSD) external onlyRole(TRADE_MANAGER_ROLE){
emit GasLog_Windmill_Fund(104, gasleft());
        address addr = address(trade);
emit GasLog_Windmill_Fund(105, gasleft());
        BUSDToken.transfer(addr, nbBUSD);
emit GasLog_Windmill_Fund(106, gasleft());
    }
    
    function getAvailableBUSD() public returns (uint256){
emit GasLog_Windmill_Fund(109, gasleft());
        uint256 nbBUSD = BUSDToken.balanceOf(thisAddr);
emit GasLog_Windmill_Fund(110, gasleft());

        return nbBUSD;
emit GasLog_Windmill_Fund(112, gasleft());
    }
    
    function getTotalPWR() public returns (uint256){
emit GasLog_Windmill_Fund(115, gasleft());
        return PWRToken.totalSupply();
emit GasLog_Windmill_Fund(116, gasleft());
    }

    function getPWRAmountFromBUSD(uint256 amountBUSD) public returns (uint256){
emit GasLog_Windmill_Fund(119, gasleft());
        uint256 nbBUSD = getFundBUSD();
emit GasLog_Windmill_Fund(120, gasleft());
        uint256 PWRSupply = getTotalPWR();
emit GasLog_Windmill_Fund(121, gasleft());

        uint256 nbPWRToGet = amountBUSD;
emit GasLog_Windmill_Fund(123, gasleft());
        if (PWRSupply > 0 && nbBUSD > 0){
emit GasLog_Windmill_Fund(124, gasleft());
            nbPWRToGet = amountBUSD * PWRSupply / nbBUSD;
emit GasLog_Windmill_Fund(125, gasleft());
        }

        return nbPWRToGet;
emit GasLog_Windmill_Fund(128, gasleft());
    }

    function getBUSDAmountFromPWR(uint256 amountPWR) public returns (uint256){
emit GasLog_Windmill_Fund(131, gasleft());
        uint256 nbBUSD = getFundBUSD();
emit GasLog_Windmill_Fund(132, gasleft());
        uint256 PWRSupply = getTotalPWR();
emit GasLog_Windmill_Fund(133, gasleft());

        uint256 nbBUSDToGet = 0;
emit GasLog_Windmill_Fund(135, gasleft());
        if (PWRSupply > 0 && nbBUSD > 0){
emit GasLog_Windmill_Fund(136, gasleft());
            nbBUSDToGet = amountPWR * nbBUSD / PWRSupply;
emit GasLog_Windmill_Fund(137, gasleft());
        }

        return nbBUSDToGet;
emit GasLog_Windmill_Fund(140, gasleft());
    }

    function buyPWR(uint256 amountBUSD) external{
emit GasLog_Windmill_Fund(143, gasleft());
        require(BUSDToken.balanceOf(msg.sender) >= amountBUSD, "Windmill_Fund: Not enough BUSD");
emit GasLog_Windmill_Fund(144, gasleft());

        uint256 nbPWRToMint = getPWRAmountFromBUSD(amountBUSD);
emit GasLog_Windmill_Fund(146, gasleft());

        require(nbPWRToMint > 0, "Windmill_Fund: Too small amount of BUSD sent");
emit GasLog_Windmill_Fund(148, gasleft());

        (uint256 nbFreePwr, uint256 nbLockedPwr, uint256 flatLockPercentNumerator,,) = getDataPWRLock(msg.sender);
emit GasLog_Windmill_Fund(150, gasleft());
        _updateLockedPWR(msg.sender, nbLockedPwr, nbFreePwr + nbLockedPwr + nbPWRToMint, flatLockPercentNumerator);
emit GasLog_Windmill_Fund(151, gasleft());
        
        BUSDToken.transferFrom(msg.sender, thisAddr, amountBUSD);
emit GasLog_Windmill_Fund(153, gasleft());
        PWRToken.mintTo(msg.sender, nbPWRToMint);
emit GasLog_Windmill_Fund(154, gasleft());
    }
    
    function beforePWRTransfer(address from, address to, uint256 amount) external onlyRole(PWR_TOKEN_ROLE){
emit GasLog_Windmill_Fund(157, gasleft());
        (uint256 nbFreePwrFrom, uint256 nbLockedPwrFrom, uint256 flatLockPercentNumeratorFrom,,) = getDataPWRLock(from);
emit GasLog_Windmill_Fund(158, gasleft());
        
        require(nbFreePwrFrom >= amount, "Windmill_Fund: Cannot transfer locked PWR tokens.");
emit GasLog_Windmill_Fund(160, gasleft());
        
        (uint256 nbFreePwrTo, uint256 nbLockedPwrTo, uint256 flatLockPercentNumeratorTo,,) = getDataPWRLock(to);
emit GasLog_Windmill_Fund(162, gasleft());
        
        
        _updateLockedPWR(from, nbLockedPwrFrom, nbFreePwrFrom + nbLockedPwrFrom - amount, flatLockPercentNumeratorFrom);
emit GasLog_Windmill_Fund(165, gasleft());
        _updateLockedPWR(to, nbLockedPwrTo, nbFreePwrTo + nbLockedPwrTo + amount, flatLockPercentNumeratorTo);
emit GasLog_Windmill_Fund(166, gasleft());
    }
    
    function getFreePWR(address addr) external returns(uint256){
emit GasLog_Windmill_Fund(169, gasleft());
        (uint256 nbFreePWR,,,,) = getDataPWRLock(addr);
emit GasLog_Windmill_Fund(170, gasleft());
        
        return nbFreePWR;
emit GasLog_Windmill_Fund(172, gasleft());
    }
    
    function getLockedPWR(address addr) external returns(uint256){
emit GasLog_Windmill_Fund(175, gasleft());
        (, uint256 nbLockedPWR,,,) = getDataPWRLock(addr);
emit GasLog_Windmill_Fund(176, gasleft());
        
        return nbLockedPWR;
emit GasLog_Windmill_Fund(178, gasleft());
    }
    
    function sellPWR(uint256 amountPWR) external{
emit GasLog_Windmill_Fund(181, gasleft());
        (uint256 nbFreePwr, uint256 nbLockedPwr, uint256 flatLockPercentNumerator,,) = getDataPWRLock(msg.sender);
emit GasLog_Windmill_Fund(182, gasleft());
        
        require(nbFreePwr >= amountPWR, "Windmill_Fund: Not enough PWR.");
emit GasLog_Windmill_Fund(184, gasleft());

        uint256 feesPWR = (amountPWR * withdrawFees.numerator) / (withdrawFees.denominator);
emit GasLog_Windmill_Fund(186, gasleft());
        uint256 PWRToWithdraw = amountPWR - feesPWR;
emit GasLog_Windmill_Fund(187, gasleft());

        PWRToken.burnFrom(msg.sender, feesPWR);
emit GasLog_Windmill_Fund(189, gasleft());
        
        uint256 nbBUSDTowithdraw = getBUSDAmountFromPWR(PWRToWithdraw);
emit GasLog_Windmill_Fund(191, gasleft());

        require(nbBUSDTowithdraw > 0, "Windmill_Fund: Too small amount of PWR sent.");
emit GasLog_Windmill_Fund(193, gasleft());
        require(nbBUSDTowithdraw >= BUSDToken.balanceOf(thisAddr), "Windmill_Fund: All BUSD are locked in trade.");
emit GasLog_Windmill_Fund(194, gasleft());
        
        _updateLockedPWR(msg.sender, nbLockedPwr, nbFreePwr + nbLockedPwr - amountPWR, flatLockPercentNumerator);
emit GasLog_Windmill_Fund(196, gasleft());

        PWRToken.burnFrom(msg.sender, PWRToWithdraw);
emit GasLog_Windmill_Fund(198, gasleft());
        BUSDToken.transfer(msg.sender, nbBUSDTowithdraw);
emit GasLog_Windmill_Fund(199, gasleft());
    }
    
    function removeLockedPWRTokens(address addr, uint256 amountPWR) external onlyRole(WITHDRAWER_ROLE){
emit GasLog_Windmill_Fund(202, gasleft());
        (uint256 nbFreePwr, uint256 nbLockedPwr, uint256 flatLockPercentNumerator,,) = getDataPWRLock(addr);
emit GasLog_Windmill_Fund(203, gasleft());
        
        require(nbLockedPwr >= amountPWR, "Windmill_Fund: Not enough locked PWR.");
emit GasLog_Windmill_Fund(205, gasleft());
        require(nbFreePwr + nbLockedPwr >= amountPWR, "Windmill_Fund: Not enough total PWR.");
emit GasLog_Windmill_Fund(206, gasleft());
        
        _updateLockedPWR(addr, nbLockedPwr - amountPWR, nbFreePwr + nbLockedPwr - amountPWR, flatLockPercentNumerator);
emit GasLog_Windmill_Fund(208, gasleft());
    }
    
    function getDataPWRLock(address addr) public returns (uint256, uint256, uint256, uint256, uint256[2][] memory){
emit GasLog_Windmill_Fund(211, gasleft());
        uint256 nbPWRUser = PWRToken.balanceOf(addr);
emit GasLog_Windmill_Fund(212, gasleft());
        
        LockData storage dataLock = userLockData[addr];
emit GasLog_Windmill_Fund(214, gasleft());
        uint256 lockPercentNumerator;
emit GasLog_Windmill_Fund(215, gasleft());
        uint256 flatLockPercentNumerator;
emit GasLog_Windmill_Fund(216, gasleft());
        
        uint256 l = tradeManager.getNbOpenTrades();
emit GasLog_Windmill_Fund(218, gasleft());
        
        uint256[2][] memory lockDatas = new uint256[2][](l);
emit GasLog_Windmill_Fund(220, gasleft());
        
        for(uint256 i=0; i<l; i++){
emit GasLog_Windmill_Fund(222, gasleft());
            (I_Windmill_Trade_Manager.TradeData memory data, uint256 id) = tradeManager.getOpenTrade(i);
emit GasLog_Windmill_Fund(223, gasleft());
            
            flatLockPercentNumerator += data.percentPWRLockedNumerator;
emit GasLog_Windmill_Fund(225, gasleft());
            
            uint256 n;
emit GasLog_Windmill_Fund(227, gasleft());
            if (data.blockStart >= dataLock.startBlock){
emit GasLog_Windmill_Fund(228, gasleft());
                n = data.percentPWRLockedNumerator;
emit GasLog_Windmill_Fund(229, gasleft());
            }else if (dataLock.factor.denominator != 0){
emit GasLog_Windmill_Fund(230, gasleft());
                n = (data.percentPWRLockedNumerator * dataLock.factor.numerator) / dataLock.factor.denominator;
emit GasLog_Windmill_Fund(231, gasleft());
            }
            
            lockDatas[i][0] = n;
emit GasLog_Windmill_Fund(234, gasleft());
            lockDatas[i][1] = id;
emit GasLog_Windmill_Fund(235, gasleft());
            lockPercentNumerator += n;
emit GasLog_Windmill_Fund(236, gasleft());
        }
        
        uint256 nbLockedPWR = (nbPWRUser * lockPercentNumerator) / 1e20;
emit GasLog_Windmill_Fund(239, gasleft());
        uint256 nbFreePWR = 0;
emit GasLog_Windmill_Fund(240, gasleft());
        
        if (nbPWRUser >= nbLockedPWR){
emit GasLog_Windmill_Fund(242, gasleft());
            nbFreePWR = nbPWRUser - nbLockedPWR;
emit GasLog_Windmill_Fund(243, gasleft());
        }else{
emit GasLog_Windmill_Fund(244, gasleft());
            nbLockedPWR = nbPWRUser;
emit GasLog_Windmill_Fund(245, gasleft());
        }
        
        return (nbFreePWR, nbLockedPWR, flatLockPercentNumerator, lockPercentNumerator, lockDatas);
emit GasLog_Windmill_Fund(248, gasleft());
    }
...
emit GasLog_Windmill_Fund(255, gasleft());
        LockData storage dataLock = userLockData[addr];
emit GasLog_Windmill_Fund(256, gasleft());
        
        if (currNbPWR == 0 || prevNbLockedPWR == 0){
emit GasLog_Windmill_Fund(258, gasleft());
            dataLock.factor.numerator = 0;
emit GasLog_Windmill_Fund(259, gasleft());
            dataLock.factor.denominator = 0;
emit GasLog_Windmill_Fund(260, gasleft());
        }else{
emit GasLog_Windmill_Fund(261, gasleft());
            uint256 currLockPercentNumerator = ((prevNbLockedPWR+1) * 1e20) / currNbPWR;
emit GasLog_Windmill_Fund(262, gasleft());
            
            dataLock.factor.numerator = currLockPercentNumerator;
emit GasLog_Windmill_Fund(264, gasleft());
            dataLock.factor.denominator = flatLockPercentNumerator;
emit GasLog_Windmill_Fund(265, gasleft());
        }
        
        dataLock.startBlock = block.number;
emit GasLog_Windmill_Fund(268, gasleft());
    }
}