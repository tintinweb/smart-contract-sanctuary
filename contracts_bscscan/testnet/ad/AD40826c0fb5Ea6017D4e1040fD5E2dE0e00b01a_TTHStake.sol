...
contract TTHStake is Owned {


    using SafeMath for uint;
    uint8 public demical = 9;
    ...
    event OnRegisterAndStake(address stakeholder, uint amount, uint totalTax, uint countDays);
    ...
    function registerAndStake(uint _amount ,  uint _days) external onlyUnregistered() whenActive() {
        ...
        Staker[msg.sender].push(stakerini({
                  ...
                  stakeDayss: _days,
                  totalStakeds:totalStaked,
                  idRandomer:block.timestamp+15
              }));
        // Staker[msg.sender].lastClocks = block.timestamp;
        // Staker[msg.sender].stakeDayss = 20;
        // Staker[msg.sender].totalStakeds = totalStaked;
        // Staker[msg.sender].idRandomer = Staker[msg.sender].idRandomer + 1;


        stakes[msg.sender] = (stakes[msg.sender]).add(finalAmount).sub(stakingTax);

        emit OnRegisterAndStake(msg.sender, _amount, registrationTax.add(stakingTax),_days);
    }
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        uint totalReward = (stakeRewards[msg.sender]).add(calculateEarnings(msg.sender));
    ...
    function getStakeResult() public view returns(uint){
      return Staker[msg.sender].length;
    }
