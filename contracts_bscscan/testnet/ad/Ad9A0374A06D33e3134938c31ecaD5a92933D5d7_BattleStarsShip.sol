...
contract BattleStarsShip is Ownable, ERC721Enumerable, ERC721Burnable, ERC721Pausable, InitialValue {
  ...
  function _createBattleStarsEndurance(
    ...
    emit NewBattleStarSuccess(randId, _owner, factions[0], collections);
  }
  ...
  function _createBattleStarsJaeger(
    ...
    emit NewBattleStarSuccess(randId, _owner, factions[1], collections);
  }
  ...
  function _createBattleStarsPrometeus(
    ...
    emit NewBattleStarSuccess(randId, _owner, factions[2], collections);
  }
  ...
  // set collections
  function setCollections(string memory _newCollections) public onlyOwner {
  ...
  function tokenURI(uint256 _tokenId) public
    ...
    string memory currentBaseURI = string(abi.encodePacked(_baseURI(), BattleStars[_tokenId].faction));

    return string(abi.encodePacked(currentBaseURI, "/", Strings.toString(_tokenId), baseExtension));
  }
...
contract InitialValue {
  ...
  string public baseURI = "https://api.battlestars.io/v1/";
  string public baseExtension = ".json";
  string public notRevealedUri;

  // Price of each battle star
  uint256 public initialPrice = 0.04 ether;
  ...
  uint256 public totalAmountOfSupplies = 10000;
  uint256 public maxMintAmountPerTransaction = 3;
  uint256 public ntfPerAddressLimit = 3;

  // number of ships for sale Endurance
  uint256 public shipsForSaleEndurance = 3300;
  // number of ships for sale Jaeger
  uint256 public shipsForSaleJaeger = 3300;
  // number of ships for sale Prometeus
  uint256 public shipsForSalePrometeus = 3300;
  // Reserved for the team, customs, giveaways, collabs and so on.
  uint256 public reserved = 100;

  // count type faction
  uint256 public countShipEndurance = 0;
  uint256 public countShipJaeger = 0;
  uint256 public countShipPrometeus = 0;

  // ship out of stock true or false
  bool public shipEnduranceOutOfStock = false;
  bool public shipJaegerOutOfStock = false;
  bool public shipPrometeusOutOfStock = false;

  // List of addresses that have a number of reserved tokens for whitelist
  address[] public whitelistedAddresses;
  mapping(uint256 => address) public battlestarsToOwner;
  mapping(address => uint256) public addressMintedBalanceBattleStars;
  ...
  BattleStar[] public BattleStars;

  // Type of faction
  string[] public factions = ["Endurance", "Jaeger", "Prometeus"];
  string public collections = "Origin";

  uint256 public battlestarsDigits = 16;
  uint256 public battlestarsModulus = 10**battlestarsDigits;
}
 ...
 */
abstract contract ERC721Pausable is ERC721, Pausable, Ownable {
    /**
