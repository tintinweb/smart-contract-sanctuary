...
contract FarmageddonLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    uint256 public minPriceTicketInCake = 0.000000000001 ether;

    uint256 public pendingInjectionNextLottery;

    uint256 public constant MIN_DISCOUNT_DIVISOR = 300;
    uint256 public constant MIN_LENGTH_LOTTERY = 1 hours - 5 minutes; // 1 hours
    uint256 public constant MAX_LENGTH_LOTTERY = 32 days + 5 minutes; // 31 days
...
contract FarmageddonLottoKeeper is KeeperCompatibleInterface, Ownable {
    ...
    uint256 public interval; // how long should the lottery last before drawing
    ...
    uint256 public fburnDivisor;
        
    FarmageddonLottery f = FarmageddonLottery(address(LotteryAddress));
    ...
    function checkUpkeep(bytes calldata) view external override returns (bool upkeepNeeded, bytes memory) {
        ...
        upkeepNeeded = block.timestamp >= upKeepTime;
    }

    function StartKeeper(uint256 _fCurrentLotteryId, address _lotteryAddress, uint256 _upKeepTime, uint256 _step) external onlyOwner {
        fcurrentLotteryId = _fCurrentLotteryId;
        upKeepTime = _upKeepTime;
        step = _step;
    ...
    function setupLottery(
        ...
        uint256[6] calldata _rewardsBreakdown,
        uint256 _treasuryFee,
        uint256 _burnDivisor
        
    ) external onlyOwner {
        fburnDivisor = _burnDivisor;
        interval = _interval;
        fticketPrice = _priceTicketInCake;
    ...
    function performUpkeep(bytes calldata /* performData */) external override {
        if (step == 1) {
            f.closeLottery(fcurrentLotteryId);
            step = 2;
            upKeepTime += 300;
            ...
            f.drawFinalNumberAndMakeLotteryClaimable(fcurrentLotteryId, true, fburnDivisor);
            step = 3;
            upKeepTime += 300;
            ...
            upKeepTime += (interval - 600);
            f.startLottery(upKeepTime, fticketPrice, fdiscount, frewardsBreakdown, ftreasuryFee);
            step = 1;
            fcurrentLotteryId += 1;
