...
contract WooRouter is Ownable {
        ...
        returns (uint256 realToAmount)
    {
        ...
        if(fromToken == quoteToken) {
            realToAmount = pool.sellQuote(toToken, fromAmount, minToAmount, address(this), to);
        }
        else if (toToken == quoteToken) {
            realToAmount = pool.sellBase(fromToken, fromAmount, minToAmount, address(this), to);
        }
        else {
            uint256 quoteAmount = pool.sellBase(fromToken, fromAmount, 0, address(this), address(this));
            IERC20(quoteToken).safeApprove(address(pool), quoteAmount);
            realToAmount = pool.sellQuote(toToken, quoteAmount, minToAmount, address(this), to);
        }
        emit WooRouterSwap(
            address(fromToken),
            address(toToken),
        ...
        returns (uint256 realQuoteAmount)
    {
        ...
        realQuoteAmount = pool.sellBase(baseToken, baseAmount, minQuoteAmount, address(this), to);
        emit WooRouterSwap(
            address(baseToken),
            address(quoteToken),
            baseAmount,
        ...
        returns (uint256 realBaseAmount)
    {
        ...
        IERC20(quoteToken).safeApprove(address(pool), quoteAmount);
        realBaseAmount = pool.sellQuote(baseToken, quoteAmount, minBaseAmount, address(this), to);
        emit WooRouterSwap(
            address(quoteToken),
            address(baseToken),
            quoteAmount,
    ...
    function fallbackSwap(
        ...
        IERC20(fromToken).approve(swapTarget, fromTokenAmount);
