...
contract Order is Owner {
    ...
    function fillOrder(uint256 _tokenId, uint256 _price) public nonContract {
        ...
        if (inviter != address(0)) {
            fee2 = (itemSale.price * invitefee) / 100;
            GTOKEN20.transferFrom(msg.sender, inviter, fee2);
            templar.setUserReward(inviter,  fee2);
        } else {
            fee2 = (itemSale.price * invitefee) / 100;
            GTOKEN20.transferFrom(msg.sender, feeAccount, fee2);
        }

        GTOKEN20.transferFrom(
            msg.sender,
            itemSale.owner,
            itemSale.price - feeAmount - deadAmount - lpAmount - fee2
        );
    ...
    function changeInviteFee(uint256 _invitefee) public onlyOwner {
