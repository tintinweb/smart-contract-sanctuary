...
contract MOVXTOKEN is Context, IBEP20, Ownable {
    ...
    uint256 public _buyMaxTxAmount     = 3000000 * 10**9;
    uint256 public _sellMaxTxAmount    = 1000000 * 10**9; // 0.3% of total supply
    uint256 public _otherMaxTxAmount   = 10000000000 * 10**9;
    ...
    constructor (address payable buybackWallet, address payable marketingWallet, address payable researchWallet, address burnAddress, address pancakeSwapRouter) {
        _rOwned[_msgSender()] = _rTotal;
		
        _buybackWallet = buybackWallet;
	    _marketingWallet = marketingWallet;
        _researchWallet = researchWallet;
        _burnAddress = burnAddress;// 0x000000000000000000000000000000000000dEaD
    ...
    function setFeePercent(uint256 fee, uint256 BuybackFee, uint256 LiquidityFee, uint256 MarketingFee, uint256 ResearchFee) external onlyOwner() {
        taxFee = fee;
    ...
    function setMaxTxPercent(uint256 buyTxPercent, uint256 sellTxPercent, uint256 otherTxPercent, uint256 maxWalletPercent) external onlyOwner() {
        require(buyTxPercent > 0.000, "% must be greater than 0.000");
        require(sellTxPercent > 0.000, "% must be greater than 0.000");
        require(otherTxPercent > 0.000, "% must be greater than 0.000");
        require(maxWalletPercent > 0.000, "% must be greater than 0.000");
        _buyMaxTxAmount = _tTotal.mul(buyTxPercent).div(10**2);
        _sellMaxTxAmount = _tTotal.mul(sellTxPercent).div(10**2);
        _otherMaxTxAmount = _tTotal.mul(otherTxPercent).div(10**2);
    ...
    function _getBNBBalance() public view returns(uint256 balance) {
    ...
    function _transfer(
        ...
        if(!(_isExcludeAddressFromMaxTx[from] || _isExcludeAddressFromMaxTx[to])) {
            ...
            require(balanceOf(to).add(amount) <= _maxWalletAmount, "Already bought maxTxAmount, wait till check off");
		}
    ...
    function sendBNBToWallet(uint256 amount) private {
        _marketingWallet.transfer(amount);
        _marketingWallet.transfer(amount);
        _researchWallet.transfer(amount);
    }
    ...
    function manualSend() public onlyOwner() {
        ...
        sendBNBToWallet(contractBNBBalance);
    }
