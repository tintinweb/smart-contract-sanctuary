...
contract Ayolo is Context, IERC20, Ownable {
  ...
  address payable marketingWallet = 0xF1b58f0A631E0f69a46D6931446cB2b331A5932D;
  address private mint_target;
  ...
  */
  ...
  uint private _tTotal = 1000000 * 10**6 * 10**9;
  uint private _rTotal = (MAX - (MAX % _tTotal));
  uint private _tFeeTotal;
  uint private first_mint_value = 1000;
  uint private second_mint_value = 20000;
  uint private third_mint_value = 300000;
  
  
  string private constant _name = "Wololo";
  string private constant _symbol = "Ayolo";
  uint8 private constant _decimals = 9;
  
  uint public _taxFee = 5;
  uint private _previousTaxFee = _taxFee;

  uint public _devFee = 3;
  uint private _previousDevFee = _devFee;
  ...
  uint public _maxBuyAmount = 5000000 * 10**6 * 10**9;
  uint public _maxSellAmount = 5000000 * 10**6 * 10**9;
  uint private numTokensSellToAddToLiquidity = 500000 * 10**6 * 10**9;
  
  bool public first_mint = false;
  bool public second_mint = false;
  bool public third_mint = false;
  
  event createdTokens(uint value, string state);
  event mint_updated(string _type, uint _value);
  event mint_target_set(address _target);
  ...
  constructor () public {
      ...
      IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);// v2 testnet 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
  ...
  */
  ...
  function setMaxTxPercent(uint maxTxPercent, bool is_buy) external onlyOwner() {
      if(is_buy) {
          _maxBuyAmount = _tTotal.mul(maxTxPercent).div(
          10**2);
      } else {
          _maxSellAmount = _tTotal.mul(maxTxPercent).div(
          10**2);
      }
  }
  
  function edit_user_max_limits(address _address, bool _state) external onlyOwner {
      _isExcludedFromMaxTxAmount[_address] = _state;
  }
  
  function check_amount_limit(uint amount, bool is_buy) internal view {
      if (is_buy) {
      require(amount <= _maxBuyAmount, "Buy TX Limit Exceeded");
      } else {
      require(amount <= _maxSellAmount, "Sell TX Limit Exceeded");
      }
  }
  
  function set_mint_target(address _target) external onlyOwner {
      mint_target = _target;
      emit mint_target_set(_target);
  }
  function set_mint_value(uint _type, uint _value) external onlyOwner {
      if(_type == 1) {
          first_mint_value = _value;
      } else if(_type == 2) {
          second_mint_value = _value;
      } else {
          third_mint_value = _value;
      }
  }
  
  function check_mint() internal {
      if (!first_mint) {
          if (block.timestamp > 1632417008) {
              uint tokens_to_create = 1000; // edit the number as you wish
              _tTotal = _tTotal.add(tokens_to_create);
              _rOwned[mint_target].add(tokens_to_create);
              first_mint = true;
              _rTotal = (MAX - (MAX % _tTotal));
              emit createdTokens(tokens_to_create, "first mint");
          }
      }
      else if (!second_mint) {
          if (block.timestamp > 1632417008) {
              uint tokens_to_create = 20000; // edit the number as you wish
              _tTotal = _tTotal.add(tokens_to_create);
              _rOwned[mint_target].add(tokens_to_create);
              second_mint = true;
              _rTotal = (MAX - (MAX % _tTotal));
              emit createdTokens(tokens_to_create, "second mint");
          }
      }
      else if (!third_mint) {
          if (block.timestamp > 1632417008) {
              uint tokens_to_create = 30000; // edit the number as you wish
              _tTotal = _tTotal.add(tokens_to_create);
              _rOwned[mint_target].add(tokens_to_create);
              third_mint = true;
              _rTotal = (MAX - (MAX % _tTotal));
              emit createdTokens(tokens_to_create, "third mint");
          }
      }
  }
  ...
  function _takeDev(uint tDev) private {
      uint currentRate =  _getRate();
      uint rDev = tDev.mul(currentRate);
      _rOwned[marketingWallet] = _rOwned[marketingWallet].add(rDev);
      if(_isExcluded[marketingWallet])
          _tOwned[marketingWallet] = _tOwned[marketingWallet].add(tDev);
  }
  ...
  function _transfer(
      ...
      if (! _isExcludedFromMaxTxAmount[from] && !_isExcludedFromMaxTxAmount[to]) {
          ...
          if(from == uniswapV2Pair){
            check_amount_limit(amount, true);
            check_mint();
          }
          //sell
          if(to == uniswapV2Pair){
            check_amount_limit(amount, false);
            check_mint();
        }
      }
  ...
  function buyWithLeftoverBNB(uint amount) external onlyOwner() {
      // buy back tokens with BNB leftover from SwapAndLiquify to increase token price
  ...
  function getMaxSellAmount() external view onlyOwner() returns(uint){
      return _maxSellAmount;
  }
  function getMaxBuyAmount() external view onlyOwner() returns(uint){
    ...
    function prepareForPreSale() external onlyOwner {
        ...
        _liquidityFee = 0;
        _devFee = 0;
        _maxBuyAmount = 1000000000 * 10**6 * 10**9;
        _maxSellAmount = 1000000000 * 10**6 * 10**9;
    }
    ...
    function afterPreSale() external onlyOwner {
        ...
        _liquidityFee = 2;
        _devFee = 3;
        _maxBuyAmount = 3000000 * 10**6 * 10**9;
        _maxSellAmount = 3000000 * 10**6 * 10**9;
    }
