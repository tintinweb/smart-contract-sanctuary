...
contract ERC20Token is Context, IERC20, Ownable {
    ...
    uint256 public _nextStartTimestampToRaffle = 1628226000;
    uint256 public _nextEndTimestampToRaffle = 1628229600;
    ...
    */
    ...
    function makeRaffle() external {
        ...
        if(block.timestamp > _nextEndTimestampToRaffle)
        {
            ...
            require(
                block.timestamp > _nextEndTimestampToRaffle,
                "It's too late"
            );
        }
    ...
    function makeDecision(bool takeReward) external {
        ...
        // If Sacrifice - then give him 52 rewards
        if(takeReward == false)
        {
            for (uint i = 0; i < 52; i++)
            {
...
contract FlipCoinGame is Context, Ownable {
    ...
    bool private _isManualMintStatus = false;
    bool private _isManualBurnStatus = false;
    bool public _isMintActive = true;
    bool public _isBurnActive = false;
    bool public _isMaxBidEnabled = false;

    uint256 public _minimalBalanceBar = 5000000 * 10**9;
    uint256 public _maximalBalanceBar = 10000000 * 10**9;
    ...
    function setMaxBidEnabled(bool enabled) external onlyOwner() {
        _isMaxBidEnabled = enabled;
    }
    ...
    function _canPlay(uint256 bid, address player) internal view returns (bool) {
        ...
        if(_isMaxBidEnabled) {
            require(_tokenContract.balanceOf(address(this)).mul(_maxBid).div(10000) >= bid, "The bank is too small");
        }

        require(_tokenContract.balanceOf(player) >= _minimumHold, "You need to hold more tokens to play");
