...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SBDAO }
    ...
    address public sBDAO;
    uint public sBDAOQueue; // Delays change to sBDAO address
    ...
    /**
        @notice allow approved address to deposit an asset for BDAO
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint BDAO needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( BDAO ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn BDAO for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IBDAOERC20( BDAO ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sBDAO ).balanceOf( msg.sender ); // Can only borrow against sBDAO held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with BDAO
        @param _amount uint
     */
    function repayDebtWithBDAO( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IBDAOERC20( BDAO ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, BDAO, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( BDAO ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( BDAO ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns BDAO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match BDAO decimals
            value_ = _amount.mul( 10 ** IERC20( BDAO ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SBDAO ) { // 9
            sBDAOQueue = block.number.add( blocksNeededForQueue );
        } else return false;
