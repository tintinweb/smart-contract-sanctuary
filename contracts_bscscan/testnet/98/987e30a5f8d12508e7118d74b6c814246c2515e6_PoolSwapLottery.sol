...
interface IPoolSwapLottery {
     ...
     * @param _priceTicketInPS: price of a ticket in PS
...
// File: contracts/PoolSwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title PoolSwapLottery Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract PoolSwapLottery is ReentrancyGuard, IPoolSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInPS = 50 ether;
    uint256 public minPriceTicketInPS = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInPS;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] psPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInPS;
        uint32 finalNumber;
    }
     ...
     * @param _psTokenAddress: address of the PS token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _psTokenAddress, address _randomGeneratorAddress) {
        psToken = IERC20(_psTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of PS to this contract
        uint256 amountPSToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInPS,
            _ticketNumbers.length
        );

        // Transfer ps tokens to this contract
        psToken.safeTransferFrom(address(msg.sender), address(this), amountPSToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInPS += amountPSToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInPSToTransfer
        uint256 rewardInPSToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInPSToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        psToken.safeTransfer(msg.sender, rewardInPSToTransfer);

        emit TicketsClaim(msg.sender, rewardInPSToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInPS) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;
        ...
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].psPerBracket[j] =
                ...
                _lotteries[_lotteryId].psPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInPS - amountToShareToWinners);

        // Transfer CAKE to treasury address
        psToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        psToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInPS += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInPS >= minPriceTicketInPS) && (_priceTicketInPS <= maxPriceTicketInPS),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInPS: _priceTicketInPS,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            psPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInPS: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(psToken), "Cannot be PS token");
    ...
    /**
     * @notice Set PS price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInPS: minimum price of a ticket in PS
     * @param _maxPriceTicketInPS: maximum price of a ticket in PS
     */
    function setMinAndMaxTicketPriceInPS(uint256 _minPriceTicketInPS, uint256 _maxPriceTicketInPS)
        external
        onlyOwner
    {
        require(_minPriceTicketInPS <= _maxPriceTicketInPS, "minPrice must be < maxPrice");

        minPriceTicketInPS = _minPriceTicketInPS;
        maxPriceTicketInPS = _maxPriceTicketInPS;
    }
     ...
     * @param _priceTicket price of a ticket (in PS)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].psPerBracket[_bracket];
        } else {
            return 0;
        }
    }
