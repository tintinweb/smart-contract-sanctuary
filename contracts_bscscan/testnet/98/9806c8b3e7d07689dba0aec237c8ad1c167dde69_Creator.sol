...
contract priceFrame{
    ...
    function pairInfo() public onlyNotExpiry view returns(string memory, uint256, string memory, uint256){
        
        (uint112 token0v, uint112 token1v,) =  IPancakePair(lpToken).getReserves();

        return (pair0, token0v, pair1, token1v);
...
contract Creator{
  ...
  function preview(address tokenAddress, address lpToken, uint256 exp) public view returns(
      address,
      address,
      uint256,
      uint8,
  ...
  function testDAI() public payable{
      newContract(0x8a9424745056Eb399FD19a0EC26A14316684e274, 0xAE4C99935B1AA0e76900e86cD155BFA63aB77A2a, 1);
  }
