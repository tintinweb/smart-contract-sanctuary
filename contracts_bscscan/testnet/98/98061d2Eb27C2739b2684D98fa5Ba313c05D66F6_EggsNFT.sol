...
contract EggsNFT is ERC721Enumerable, Ownable {
    ...
    modifier onlyAuth() {
        require(
            msg.sender == gameAddresses.getNurseryAddress() ||
            msg.sender == gameAddresses.getGameAddress() ||
            msg.sender == gameAddresses.getAlchemyAddress() ||
            msg.sender == gameAddresses.getChickenAddress()
            , "Only game allowed");
        _;
    }
    ...
    function mintEgg(
        ...
        _maturityTimestamp[_newItemId] = block.timestamp.add(_maturityDuration);
        return (_newItemId);
    }
    ...
    function claimMatureMonster(
        ...
        _burn(_tokenId);
        uint256 _monsterId = _generateRandomMonster(_path,_name, msg.sender, _stateIndex[_tokenId], _signature);
        return (_monsterId);
    }
    ...
    function getState(uint256 _tokenId) external view returns (string memory){
            return _states[_stateIndex[_tokenId]];
    } 
