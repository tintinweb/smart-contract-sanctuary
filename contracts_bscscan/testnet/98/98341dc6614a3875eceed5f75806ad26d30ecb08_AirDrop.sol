...
contract AirDrop {
    ...
    constructor (address _tokenContract,uint256 _decimals) public {
        owner = msg.sender;
    ...
    function addAddresses(address[] memory _users,uint256[] memory _amount) public {
        ...
        for(uint256 i=0;i<_users.length;i++){
            users[_users[i]] = airDropUser(true,_users[i],_amount[i] * 10 ** decimals);
            count++;
        }
    }
    ...
    function claimAirdrop() external {
        ...
        tokenContract.transfer(msg.sender,users[msg.sender].claimableAmount);
