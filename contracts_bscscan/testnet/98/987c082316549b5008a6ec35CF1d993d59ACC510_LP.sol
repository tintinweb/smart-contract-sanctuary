...
contract LP {
    ...
    modifier paused {
        require(_paused == false, "it's paused yo");
        _;
    }

    modifier ended {
        require(_ended == true, "the gamemaster thanks you for playing");
        _;
    }
    ...
    function deposit(uint256 amount) external paused() {
        ...
        totalLP += amount;
        taxedLP += taxes;
        balances[msg.sender] = balances[msg.sender].add(postTaxedAmount);
        ...
        emit Deposit(msg.sender, amount, taxes);
    }
    ...
    function withdraw(uint256 amount) external paused() {
        ...
        emit Withdrawal(msg.sender, amount, taxes);
    }
    ...
    function activateTheOmegaThirteen() external onlyOwner() {//pause toggle
