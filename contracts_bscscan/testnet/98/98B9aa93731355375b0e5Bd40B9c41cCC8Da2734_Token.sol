...
contract Token is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"TOKENTEST";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner()) {
            ...
            if(from == pcsV2Pair && to != address(pcsV2Router) && !_isExcludedFromFee[to]) {
                ...
                if(_cooldownEnabled) {
                    if(buyLimitEnd > block.timestamp) {
                        ...
                        cooldown[to].buy = block.timestamp + 2 seconds;
                    }
                }
                if(_cooldownEnabled) {
            ...
            // sell
            if(!inSwap && from != pcsV2Pair && tradingOpen) {
                ...
                if(_useImpactFeeSetter) {
                    ...
                    feeBasis = feeBasis.div(balanceOf(pcsV2Pair).add(amount));
                    setFee(feeBasis);
                }
    ...
    function addLiquidity() external onlyOwner() {
        ...
        _maxBuyAmount = 3000000 * 10**9;
        _launchTime = block.timestamp;
        IERC20(pcsV2Pair).approve(address(pcsV2Router), type(uint).max);
    }

    function openTrading() public onlyOwner {
        tradingOpen = true;
        buyLimitEnd = block.timestamp + (2 seconds);
    }
