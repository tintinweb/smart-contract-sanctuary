...
contract OpenAndCloseCenter is Ownable {
    ...
    uint256 maturityHousesDuration = 300;  //1 days
    uint256 closingHousesDuration = 300;  //3 days

    uint256 public nurseryMinted;
    uint256 public laboratoryMinted;
    uint256 public trainingCenterMinted;
    ...
    function createNursery(string memory _name, uint256 _bronzePrice, uint256 _silverPrice, uint256 _goldPrice, uint256 _platinumPrice) external returns (uint256) {
        ...
        nurseryMinted = nurseryMinted.add(1);
        lockedInNurseryID[nurseryId] = nurseryPrice;
        nurseryMaturityTime[nurseryId] = block.timestamp.add(maturityHousesDuration);
    ...
    function createTrainingCenter(string memory _name) external returns (uint256) {
        ...
        uint256 trainingId = ITraining(gameAddresses.getTrainingCenterAddress()).mintTrainingCenter(
            msg.sender
        );
        trainingCenterMinted = trainingCenterMinted.add(1);
        lockedInTrainingCenterID[trainingId] = trainingCenterPrice;
        trainingCenterMaturityTime[trainingId] = block.timestamp.add(maturityHousesDuration);
    ...
    function createLaboratory(string memory _name) external returns (uint256) {
        ...
        uint256 laboId = ILaboratory(gameAddresses.getLaboratoryAddress()).mintLaboratory(
            msg.sender
        );
        laboratoryMinted = laboratoryMinted.add(1);
        lockedInLaboID[laboId] = laboratoryPrice;
        laboratoryMaturityTime[laboId] = block.timestamp.add(maturityHousesDuration);
    ...
    function closeNursery(uint256 _tokenId) external {
        ...
        c.isClosing = true;
        c.timestampClosedActed = block.timestamp.add(closingHousesDuration);
    }
    ...
    function getBZAIBackFromClosingLabo(uint256 _tokenId) public {
        ...
        ILaboratory(gameAddresses.getLaboratoryAddress()).burn(_tokenId);
        c.isClosing = false;
        c.destructed = true;
    }
}
