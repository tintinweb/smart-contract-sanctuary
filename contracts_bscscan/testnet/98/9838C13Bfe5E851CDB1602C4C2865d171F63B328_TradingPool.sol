...
contract TradingPool is Ownable{
    ...
    uint256 constant MAX_BLOCK = 28000; //10 days
    ...
    function totalHashRate(address _pair) external view returns(uint256 _supply){
        PoolInfo memory _pool = poolInfo[_pair];
        _supply = _pool.totalHashRate.sub(_pool.totalHashRate.mul(min(MAX_BLOCK, block.number - _pool.lastRewardBlock)) / MAX_BLOCK).mul(poolInfo[_pair].allocPoint) / totalAllocPoint;
    }

    function userHashRate(address _pair, address _user) external view returns(uint256 _userHashRate) {
        UserInfo memory _user = userInfo[_pair][_user];
        _userHashRate = _user.amount.sub(_user.amount.mul(min(MAX_BLOCK, block.number.sub(_user.lastDepositBlock))) / MAX_BLOCK).mul(poolInfo[_pair].allocPoint) / totalAllocPoint;
    }
    ...
    function pendingReward(address _pair, address _user) public view returns (uint256 _pending) {
        PoolInfo memory _pool = poolInfo[_pair];
        UserInfo storage _user = userInfo[_pair][_user];

        uint256 _supply = _pool.totalHashRate;
        uint256 _accRewardPerShare = _pool.accRewardPerShare;
        if (block.number > _pool.lastRewardBlock && _supply != 0) {
            ...
            _accRewardPerShare = _accRewardPerShare.add(_reward.mul(ACC_PAN_PRECISION) / _supply);
        }
        _pending = _user.amount.mul(_accRewardPerShare.sub(_user.debt)) / ACC_PAN_PRECISION;

        uint256 _deltaBlock = block.number.sub(_user.lastDepositBlock);

        if (_deltaBlock >= MAX_BLOCK) {
            _pending = _pending.mul(MAX_BLOCK) / (_deltaBlock );
        } else {
            uint256 _curAmount = _user.amount.mul(_deltaBlock) / MAX_BLOCK;
            _pending = _pending.sub(_curAmount.mul(_accRewardPerShare.sub(_user.debt)) / ACC_PAN_PRECISION);
        }
        _pending = _pending.add(_user.pending);
    ...
    function updatePool(address _pair) public {
        if (addedPairs[_pair]) {
            ...
            if (block.number > _pool.lastRewardBlock) {
                uint256 _supply = _pool.totalHashRate;
                _supply = _supply.sub(_supply.mul(min(MAX_BLOCK, block.number - _pool.lastRewardBlock)) / MAX_BLOCK);
                if (_supply > 0) {
                    uint256 _blocks = block.number.sub(_pool.lastRewardBlock);
                    uint256 _reward = _blocks.mul(rewardPerBlock).mul(_pool.allocPoint) / totalAllocPoint;
                    _pool.accRewardPerShare = _pool.accRewardPerShare.add((_reward.mul(ACC_PAN_PRECISION) / _supply).to128());
                }
                _pool.lastRewardBlock = block.number.to64();
                _pool.totalHashRate = _supply;
                emit LogUpdatePool(_pair, _pool.lastRewardBlock, _supply, _pool.accRewardPerShare);
            }
        }
    }
    ...
    function estimationHashRate(uint256 _amountIn, address[] memory _path) external view returns(uint256 _total) {
        ...
        for (uint256 i = 0; i < _amounts.length - 1; i++) {
            ...
            if (addedPairs[_pair]) {
                ...
                _total += _amount * poolInfo[_pair].allocPoint;
            }
        }
...
//        }
        if (addedPairs[_pair]) {
            ...
            uint256 _curAmount = _user.amount.sub(_user.amount.mul(min(MAX_BLOCK, block.number - _user.lastDepositBlock) / MAX_BLOCK));
            uint256 _acc = _pool.accRewardPerShare;
            updatePool(_pair);
            _user.pending = pendingReward(_pair, _account);
            _user.debt = _acc;
            _user.amount = _curAmount;
            _user.lastDepositBlock = block.number;

            if (oracles[_output] != address(0)) {
                _amount = IOracle(oracles[_output]).consult().mul(_amount);
            } else {
                _amount = 0;
            }
            _user.amount = _user.amount.add(_amount);
            _pool.totalHashRate = _pool.totalHashRate.add(_amount);
            emit Deposit(_account, _pair, _amount * _pool.allocPoint / totalAllocPoint);
            return true;
        }
        return false;
    }
    ...
    function harvest(address _pair, address _to) public {
        if (addedPairs[_pair]) {
            ...
            PoolInfo storage _pool = poolInfo[_pair];
            uint256 _pendingReward = pendingReward(_pair, msg.sender);
            uint256 _currentAmount = _user.amount.sub(_user.amount.mul(min(MAX_BLOCK, block.number - _user.lastDepositBlock)) / MAX_BLOCK);
            _pool.totalHashRate = _pool.totalHashRate.sub(_currentAmount);

            _user.amount = 0;
            _user.debt = 0;
            _user.lastDepositBlock = 0;
            _user.pending = 0;

            // Interactions
            if (_pendingReward != 0) {
                PAN.mint(_to, _pendingReward);
            }
            emit Harvest(msg.sender, _pair, _pendingReward);
        }
    }
