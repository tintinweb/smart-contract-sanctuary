...
contract SkippyToken is Context, IERC20, Ownable {
    ...
    uint256 public liquidityThreshold = 30000 * 10 ** 18;
    ...
    mapping (address => uint) public _timeLock;
    mapping (address => bool) public _isToLock;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        require(_timeLock[sender] <= block.timestamp, "Transfer from address is locked");
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(!takeFee)
            restoreAllFee();
        _isTimeLimit[sender] = block.timestamp.add(1 * 60);
    ...
    function _getTValues(uint256 tAmount) private view returns (Tvalues memory) {
        ...
        tValues.tTransferAmount = tAmount.sub(tValues.tCharity).sub(tValues.tLiquidity).sub(tValues.tRandD).sub(tValues.tReward).sub(tValues.tMarketing);
        return tValues;
    }
    ...
    function _getRValues(uint256 tAmount, Tvalues memory tValues, uint256 currentRate) private pure returns (Rvalues memory) {
        ...
        rValues.rTransferAmount = rValues.rAmount.sub(rValues.rCharity).sub(rValues.rMarketing).sub(rValues.rRandD).sub(rValues.rReward).sub(rValues.rLiquidity);
        return rValues;
    }
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        _charityTransfer(tValues.tCharity);
        _marketingTransfer(tValues.tMarketing);
        _rAndDTransfer(tValues.tRandD);
    ...
    function _charityTransfer(uint256 tCharity) internal {
        if(tCharity != 0) {
    ...
    function _rAndDTransfer(uint256 tRandD) internal {
        if(tRandD != 0) {
            uint currentRate = _getRate();
    ...
    function _marketingTransfer(uint256 tMarketing) internal {
        if(tMarketing != 0) {
            uint currentRate = _getRate();
