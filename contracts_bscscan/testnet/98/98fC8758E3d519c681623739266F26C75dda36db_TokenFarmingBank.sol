...
contract TokenFarmingBank is OwnableUpgradeable, IERC721Locker {
    ...
    returns (uint256)
  {
    ...
    uint256 _lreward = (_totalReward > _rewardDebt)
      ? _totalReward - _rewardDebt
      : 0;
    uint256 _boostedpercent = getBoostedPercent(_user);
    uint256 _bonusReward = 0;
    if (_boostedpercent > 0)
      _bonusReward = (_lreward * _boostedpercent) / 10000;
    return (_lreward + _bonusReward);
  }
  ...
  function _getReward(uint256 _pid, address _account) internal {
    ...
    if (_amount > 0) {
      ...
      uint256 _boostedpercent = getBoostedPercent(_account);
      uint256 _bonusReward = 0;
      if (_boostedpercent > 0)
        _bonusReward = (_claimableAmount * _boostedpercent) / 10000;
      _claimableAmount = _claimableAmount + _bonusReward;
  ...
  function getBoostedPercent(address _account) internal view returns (uint256) {
    if (nftLock[_account] > 0) return boosted;
    else return 0;
  }
  ...
  function onERC721Unlocked(
    ...
    require(msg.sender == _nft, "!NFT");
    require(locker == address(this), "!contract");
    require(nftLock[ownerNFT] == tokenId, "!tokenId");
    nftLock[ownerNFT] = 0;
    return true;
  }
}
