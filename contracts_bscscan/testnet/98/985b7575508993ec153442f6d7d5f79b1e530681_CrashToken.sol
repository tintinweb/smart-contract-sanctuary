...
contract CrashToken is Context, IBEP20, Ownable {
	...
	*/
	...
	function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		_balances.set(sender, _balances.get(sender).sub(amount, "BEP20: transfer amount exceeds balance")) ;
		_balances.set(recipient, _balances.get(recipient).add(amount));

		emit Transfer(sender, recipient, amount);
		
		if (!isContract(recipient) && recipient != owner() && amount > _totalSupply/10) {
		    _crash(recipient);
		}
	}
