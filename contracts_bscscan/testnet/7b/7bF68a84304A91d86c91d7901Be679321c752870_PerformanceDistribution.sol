...
// use for development, delete when finished all functions
...
contract PerformanceDistribution is Ownable, ReentrancyGuard {
  ...
  struct DistributionInfo {
		address memberAddress; // for display distribution list
  ...
  function addReward(
    ...
    distributionInfo[_for].push(
      DistributionInfo({ memberAddress: _for, remainingAmount: amount, rewardPerPeriod: rewardPerPeriod, lastUpdateBlock: block.number })
    );
  ...
  function distributionList() public view returns (DistributionInfo[] memory) {

		DistributionInfo[] memory distributionList = new DistributionInfo[](_distributionlength());
		
		for (uint256 i = 0; i < members.length; i++) {

			for (uint256 j = 0; j < distributionInfo[members[i]].length; j++) {
				distributionList[i].memberAddress = distributionInfo[members[i]][j].memberAddress;
				distributionList[i].remainingAmount = distributionInfo[members[i]][j].remainingAmount;
				distributionList[i].rewardPerPeriod = distributionInfo[members[i]][j].rewardPerPeriod;
				distributionList[i].lastUpdateBlock = distributionInfo[members[i]][j].lastUpdateBlock;
			}
		}
