...
contract Greyhound is Context, IERC20, Ownable {
    ...
    constructor (address CharityWallet) public {
		...
		//_isExcluded[_CharityWallet]  = true;    // manually
        //_isExcludedFromFee[uniswapV2Router]    = true;    // manually
		//_isExcluded[uniswapV2Router] = true;    // manually
    ...
    function _getTValues1(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        (uint256 tFee, uint256 tLiquidity, uint256 tCharity, uint256 tBurn) = calculateFees(tAmount);
    ...
    function _getTValues2(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tCharity) private pure returns (uint256) { // uint256 tBurn
        return tAmount.sub(tFee).sub(tLiquidity).sub(tCharity); //Adress.this? burn?
    }
    ...
    function _getRValues2(uint256 rAmount, uint256 rFee, uint256 tLiquidity, uint256 tCharity, uint256 currentRate) private pure returns (uint256) {
        ...
        // Burning is done from supply, it is not a fee
    ...
    function _sendToCharity(uint256 tCharity, address sender) private {
     ...
     function _reflectFee(uint256 tBurn, uint256 rBurn, uint256 tFee, uint256 tCharity) private {
        ...
        // since we exclude the BurnWallet from rewards, tBurn is used.
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        _reflectFee(tBurn, rBurn, tFee, tCharity);
   ...
   function _transferStandard(address sender, address recipient, uint256 tAmount) private {
    ...
    function _prepareForPreSale() external onlyOwner {
    ...
    function _afterPreSale() external onlyOwner {
    ...
    function _burnFromSupply(uint256 TxPercent) private {
        (, uint256 tSupply) = _getCurrentSupply();
        uint256 txAmount = tSupply.mul(TxPercent).div(10**2);
        //uint256 rtxAmount = txAmount.mul(_getRate());
        require(txAmount <= tSupply, "Amount must be less than supply");
        emit Transfer(address(this), _BurnWallet, txAmount);
        _rTotal         = _rTotal.sub(txAmount);
        //_rOwned[_BurnWallet] = _rOwned[_BurnWallet].add(rtxAmount); 
        _tOwned[_BurnWallet] = _tOwned[_BurnWallet].add(txAmount);
    }
    function burnFromSupply(uint256 tAmount) external onlyOwner() {
        _burnFromSupply(tAmount);
    }   
}