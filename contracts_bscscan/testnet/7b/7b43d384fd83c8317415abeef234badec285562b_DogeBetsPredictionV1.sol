...
// V1 testnet contract for DogeBets. Ta-da!
// Originally forked from CandleGenie. Who originally forked it from PancakeSwap.
...
contract DogeBetsPredictionV1 is Ownable, Pausable, ReentrancyGuard {
    ...
    function ChangePriceSource(string memory _priceSource) external onlyOwner 
    {
    ...
    function RoundExecute(int256 price, uint32 timestamp, uint256 betOnBull, uint256 betOnBear) external onlyOwnerOrOperator whenNotPaused 
    {
        ...
        require(block.timestamp >= Rounds[currentEpoch].lockTimestamp, "Can only execute after current round lock timestamp");
        require(block.timestamp <= Rounds[currentEpoch].closeTimestamp, "Can only execute before current round close timestamp");
