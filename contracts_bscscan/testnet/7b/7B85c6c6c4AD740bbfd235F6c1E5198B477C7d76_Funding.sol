...
contract Funding {
  ...
  enum Status { FUG, PRS, FID, PEG, CAL, RED }
  struct Phase {
    uint    duration;         
  ...
  mapping(string  => Project)                     private _pPro;
  mapping(string  => Status)                      private _pSta;
  mapping(string  => Phase[])                     private _pPhs;
  mapping(string  => Result[])                    private _pRes;

  mapping(string  => mapping(uint => address[]))  private _denied;
  mapping(string  => uint)                        private _budget;
  mapping(string  => mapping(address => uint256)) private _funds;         // backer address => projectId => block date
  mapping(string  => uint)                        private _widwable;      // creator can withdraw amount
  mapping(string  => uint)                        private _bkAmt;         // amount each banker refundable
  mapping(string  => mapping(address => uint256)) private _refunds;       // projectCode => backer address => amount
  uint                                            private _getTax;        // get TAX
  
  event EAction(string action, string indexed name, string project, address creator, uint bakNum, uint256 bakAmt, uint256 amount);//action: [create,fund,Withdraw,Refund,Kickoff,Deny]
  event EFinal(string action, string indexed name, string project, address actor, string nft, uint widwable, uint backAmount);   //action: [commit,release,cancel]
  ...
  function createProject( string memory name_, address creator_, 
    ...
    // require(_pSta[name_] != 0, "exist project");
    require(_budget[name_]    <  1, "project is fundraising");  // can not update when already fund
    ...
    for(uint i = 0; i < duration_.length; i++) {
      ...
      pha.dateEnd       = block.timestamp + duration_[i];
      pha.widwable      = widwable_[i];
      _pPhs[name_].push(pha);
    }
    _widwable[name_]    = 0;
    emit EAction("Create", name_, name_, creator_, bakNum_, bakAmt_, pro.budget);
  }
  function fund(string memory name_) public payable {
    ...
    require(_pPro[name_].bakAmt       == msg.value, "amount incorrect");
    require(_funds[name_][msg.sender]  < 1, "already fundraising");
    require(_pPro[name_].creator      != msg.sender, "invalid backer");
    _token.safeTransferFrom(msg.sender, address(this) , msg.value);
    _budget[name_]                        = _budget[name_] + msg.value;
    _funds[name_][msg.sender]             = msg.value;
    emit EAction("Fund", name_, name_, msg.sender, msg.value, _budget[name_], _pPro[name_].budget);
  }
  function _next(string memory name_, string memory file_) private {
    ...
    require(_pPhs[name_][phN].dateEnd     >= block.timestamp,"invalid phase time"); // check min
  ...
  function kickoff(string memory name_) public {
    ...
    emit EAction("Kickoff", name_, name_, msg.sender, _pPro[name_].tax, _widwable[name_], _budget[name_]);
  }
  function deny(string memory name_) public {
    ...
    emit EAction("Deny", name_, name_, msg.sender, phN, _budget[name_]/_pPro[name_].bakNum, _budget[name_]);
  }
  ...
  function release(string memory name_, string memory nft_) public {
    ...
    uint phN                       =   _pRes[name_].length;
    // require(phN + 1                       ==  _pPro[name_].phNum, "invalid phase");
    require(_denied[name_][phN].length < _pPro[name_].deniedMax, "backers denied");
    Result memory res;
    res.pass              = true;
    res.file              = nft_;
  ...
  function cancel(string memory name_, string memory note_) public owner {
  ...
  function refund(string memory name_) public {
    ...
    _token.safeTransfer(msg.sender, _bkAmt[name_]);
    _refunds[name_][msg.sender]         = _bkAmt[name_];
    _budget[name_]                      =  _budget[name_] - _bkAmt[name_];
    emit EAction("Refund", name_, name_, msg.sender, _pRes[name_].length, _widwable[name_], _bkAmt[name_]);
  }
  function withdraw(string memory name_) public {
    require(_pPro[name_].creator   == msg.sender, "invalid creator");
    require(_widwable[name_]        >   0, "invalid widwable");
    uint withdrawed                 = _widwable[name_];
    _token.safeTransfer(msg.sender, _widwable[name_]);
    _widwable[name_]                = 0;
    emit EAction("Withdraw", name_, name_, msg.sender, _pRes[name_].length, withdrawed, 0);
  }
  function getTax() public owner {
    _token.safeTransfer(msg.sender, _getTax);
    uint backup = _getTax;
    _getTax     = 0;
