...
contract MetaPirateToken is AccessControlEnumerable, ERC20, IMetaPirateToken {
    ...
    constructor(address poolAddress_) ERC20("Metapirate", "MEP") {
...
// import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.4.1/contracts/utils/structs/EnumerableSet.sol";


library EnumerableUintToUintMap {
     ...
     * Returns true if the key was removed from the map, that is if it was present.
     */
    function remove(UintToUnitMap storage map, uint256 key) internal returns (bool) {
     ...
     * @dev Returns true if the key is in the map. O(1).
     */
    function contains(UintToUnitMap storage map, uint256 key) internal view returns (bool) {
     ...
     */
    function length(UintToUnitMap storage map) internal view returns (uint256) {
     ...
     * - `index` must be strictly less than {length}.
     */
    function at(UintToUnitMap storage map, uint256 index) internal view returns (uint256, uint256) {
     ...
     */
    function tryGet(UintToUnitMap storage map, uint256 key) internal view returns (bool, uint256) {
     ...
     * - `key` must be in the map.
     */
    function get(UintToUnitMap storage map, uint256 key) internal view returns (uint256) {
...
interface IMetaPirateToken {
    ...
    function release(address beneficiaryAddress_) external returns (bool);
    ...
    event Locked(address indexed _of, uint256 _releaseTime, uint256 _amount);

    event Unlocked(address indexed _of, uint256 _releaseTime, uint256 _amount);
}
