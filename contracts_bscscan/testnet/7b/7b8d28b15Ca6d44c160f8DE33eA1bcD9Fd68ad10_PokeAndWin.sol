...
contract PokeAndWin is TransferOwnable {   
    ...
    // Token struct and function 
    event log_tokens_create(bytes _symbol, address _tokens_address);
    event log_tokens_set(uint256 _tokensId, bytes _symbol, address _tokens_address);
    struct TOKENS {   
        uint256 tokensId;
        bytes  tokens_symbol; 
        address tokens_address; 
        uint256 tokens_cake_fee1; // domainsInfo[0]
    ...
    function tokens_create(bytes memory _symbol, address _tokens_address) public onlyAdmin {   
        ...
        emit log_tokens_create( _symbol, _tokens_address);
    }
    function tokens_set(uint32 _tokensId, bytes memory _symbol, address _tokens_address) public onlyAdmin {   
        ...
        tk.updateTime = block.timestamp; 
        emit log_tokens_set( _tokensId, _symbol, _tokens_address);
    }
    ...
    struct GROUPS {  
        ...
        uint32 indexLength;
        mapping(uint32 => uint32) indexInfo; // point to boxsInfo[boxIndex]
        uint32 timeCreate;
        uint8 isRecycle;
    } 
    struct BOXS {  
    ...
    function pokeETH(uint256 _groupId, uint256 _index, uint256 _cellId) external payable {
        ...
        uint32 boxId = g.indexInfo[uint32(_index)];
    ...
    function setCURRENCY(address _tokenAddress, address _pirAddress) public onlyPartner {
        ...
        if(c.token0==WBNB && c.token1==_tokenAddress) { 
            c.bnb_rate = c._reserve1/c._reserve0;
        } else if(c.token1==WBNB && c.token0==_tokenAddress) { 
