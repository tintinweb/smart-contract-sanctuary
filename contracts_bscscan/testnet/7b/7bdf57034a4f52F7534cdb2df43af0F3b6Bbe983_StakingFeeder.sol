...
contract StakingFeeder  {
	...
	function setFeedPercent(uint256 _FEED_PERCENT) external { 
	...
	function setApps(iMoonCoinApp _MoonCoin, iMoonCoinFinanceApp _MoonCoinFinanceApp,  pancakeInterface _pancakeRouter, address[] memory _path,address[] memory _reversePath) external {
		MoonCoinApp = _MoonCoin;
	...
	function getStakingAppTokens() public view returns (uint256) {
		...
		return stakingAppBalance.mul(FEED_PERCENT).div(PERCENTS_DIVIDER).mul(getStakingAppRate());
	}
	...
	function getExchangeAppTokens() public  returns (uint256) {
		return getExchangeAppRate(getterPoolBalance().mul(FEED_PERCENT).div(PERCENTS_DIVIDER));
	}
