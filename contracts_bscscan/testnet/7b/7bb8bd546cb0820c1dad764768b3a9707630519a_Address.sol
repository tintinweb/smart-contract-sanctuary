...
contract VeldoraTest is Context, IERC20, Ownable {
    ...
    string private _name = "VeldoraTest";
    string private _symbol = "VELDTBSC";
    
    // Target addresses for fees
    address public _burn_address = 0x000000000000000000000000000000000000dEaD;
    address public _marketing_address = 0x89ff795cd04e4AdFbf78C89189B6bB7c565Aa1aE;
    ...
    uint256 private _liqAddBlock = 10000;
    mapping (address => bool) private _liquidityHolders;
    mapping (address => bool) private _isSniper;
    uint256 private snipersCaught = 0;
    uint256 private snipeBlockAmount = 0;
    
    uint256 _purchaseLimitBlocks = 0;
    uint256 _purchaseLimit = 10000;
    uint256 public _maxTxAmount = 5e12 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 5e11 * 10**9;
	...
	event SniperTimeChanged(uint256 newBlocks);
    ...
    event TransferLiquidity(address indexed from, address indexed contractAddress, uint256 value);
    ...
    function _getRValues(uint256 tAmount, ExtraValues memory values, uint256 currentRate, bool takeFee) private pure {
        ...
        if (takeFee) {
            ...
            values.rTransferAmount = values.rAmount.sub(values.rFee).sub(rLiquidity).sub(rMarketing).sub(rBurn);
        } else {
            values.rFee = 0;
    ...
    function _transferBurn(uint256 tBurn) private {
		...
		_rOwned[_burn_address] = _rOwned[_burn_address].add(rBurn);
		if(isExcludedFromReward(_burn_address))
			_tOwned[_burn_address] = _tOwned[_burn_address].add(tBurn);
	}
	
	function _transferMarketing(uint256 tMarketing) private {
		uint256 currentRate = _getRate();
		uint256 rMarketing = tMarketing.mul(currentRate);		
		_rOwned[_marketing_address] = _rOwned[_marketing_address].add(rMarketing);
		if(isExcludedFromReward(_marketing_address))
			_tOwned[_marketing_address] = _tOwned[_marketing_address].add(tMarketing);
	}
    ...
    function _checkLiquidityAdd(address from, address to) private {
        if (!_hasLimits(from, to) && to == uniswapV2Pair) {
            // if (snipeBlockAmount == 0 || snipeBlockAmount > 5) {
	...
	function _checkSniper(address from, address to) private {
		if (_liqAddBlock == 0) {
			_checkLiquidityAdd(from, to);
			if (!_hasLiqBeenAdded && _hasLimits(from, to)) {
				revert("Only owner can transfer at this time.");
			}
		} else if (_liqAddBlock > 0  && from == uniswapV2Pair && _hasLimits(from, to)) {
			if (block.number - _liqAddBlock < snipeBlockAmount) {
				_isSniper[to] = true;
				snipersCaught++;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (_exceedsPurchaseLimit(sender, recipient, amount)) revert("Transfer exceeds temporary purchase limit");
    ...
    function _completeTransfer(address sender, address recipient, ExtraValues memory values) private {
        if (values.tLiquidity > 0) {
            ...
            emit TransferLiquidity(sender, address(this), values.tLiquidity);
        }
        if (values.tBurn > 0) {
            _transferBurn(values.tBurn);
            emit TransferBurn(sender, _burn_address, values.tBurn);
        }
        if (values.tMarketing > 0) {
            _transferMarketing(values.tMarketing);
            emit TransferMarketing(sender, _marketing_address, values.tMarketing);
        }
