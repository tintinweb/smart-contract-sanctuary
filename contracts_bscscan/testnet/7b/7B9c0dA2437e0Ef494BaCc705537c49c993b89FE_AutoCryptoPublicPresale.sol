...
contract AutoCryptoPublicPresale is Initializable, UUPSUpgradeable {
    ...
    function getMaxContribution(address _contributor) public view returns (uint) {
        ...
        uint _max = maxContribution - getContribution(msg.sender);
        if (hardCap - totalContribution > _max) {
            return _max;
        }
        if (hardCap - totalContribution < minContribution && contributor[_contributor].contribution < minContribution)
            return 0;
        return hardCap - totalContribution;
    }
