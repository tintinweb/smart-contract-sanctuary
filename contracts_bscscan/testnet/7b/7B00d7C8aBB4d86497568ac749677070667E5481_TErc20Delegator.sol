...
import "../interfaces/TTokenInterfaces.sol";

/**
 * @title TukTuk's TErc20Delegator Contract
 * @notice CTokens which wrap an EIP-20 underlying and delegate to an implementation
 * @author TukTukFinance
 */
contract TErc20Delegator is TTokenInterface, TErc20Interface, rDelegatorInterface {
    /**
     * @notice Construct a new money market
     * @param underlying_ The address of the underlying asset
     * @param tuktukController_ The address of the TukTukController
    ...
    constructor(address underlying_,
                TukTukControllerInterface tuktukController_,
    ...
    function liquidateBorrow(address borrower, uint repayAmount, TTokenInterface TTokenCollateral) external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("liquidateBorrow(address,uint256,address)", borrower, repayAmount, TTokenCollateral));
        return abi.decode(data, (uint));
    }
     ...
     * @dev This is used by tuktukController to more efficiently perform liquidity checks.
   ...
   /**
    ...
    /**
      * @notice Sets a new tuktukController for the market
      * @dev Admin function to set a new tuktukController
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setTukTukController(TukTukControllerInterface newTukTukController) public returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("_setTukTukController(address)", newTukTukController));
        return abi.decode(data, (uint));
    }
...
import "./TukTukControllerInterface.sol";
...
contract TTokenStorage {
     ...
     */
    TukTukControllerInterface public tuktukController;
...
contract TTokenInterface is TTokenStorage {
    ...
    /**
     * @notice Event emitted when tuktukController is changed
     */
    event NewTukTukController(TukTukControllerInterface oldTukTukController, TukTukControllerInterface newTukTukController);
...
contract TukTukControllerInterface {
    /// @notice Indicator that this is a TukTukController contract (for inspection)
    bool public constant isTukTukController = true;
    ...
    function transferAllowed(address TToken, address src, address dst, uint transferTTokens) external returns (uint);
    function transferVerify(address TToken, address src, address dst, uint transferTTokens) external;
