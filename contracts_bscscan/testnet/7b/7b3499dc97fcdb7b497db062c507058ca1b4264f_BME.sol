...
contract BME is ERC20, Ownable {
    ...
    function _transfer(
        ...
        if(antiWhaleRestriction[from].add(oneDayRestriction) >= block.timestamp ){
        ...
        if(!swapping) {
         ...
         CalculetHoldingDays(); // calculet holding days for holder after 5 trades
         SendBMEDividends(); // send BME rewads to all holders
    ...
    function setonMap(address from ,address to ) private onlyOwner{
    ...
    function SendBMEDividends() public  onlyOwner  {
        uint256 numberOfTokenHolders = tokenHoldersMap.keys.length;
        uint256 iterations = 0;
        ...
        function CalculetHoldingDays() private onlyOwner {
          ...
          uint256 iterations = 0;
            ...
            if( addOneDay && PassedMinimumTrades[account] && accountBalance >= minimumCycelTkoens && !onCycel[account]){
