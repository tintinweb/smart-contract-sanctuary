...
import "./interfaces/IRevisionHistory.sol";

contract LaunchPadTierFcfsPoolV2 is Ownable {
    ...
    function setRevisionHistory(address _historyContract, uint256 _gameCode)
        public
        onlyOwner
    {
    ...
    function buylaunchPadTokens() public {
        require(
            inputTokenAmount > 0 && outputTokenAmount > 0,
            "LaunchPad: token exchange rate has not set yet."
        );
        ...
        require(
            currentCapAmount.add(inputTokenAmount) <= maxCapAmount,
            "LaunchPad: maximum volume reached."
        );

        require(
            paymentToken.balanceOf(_msgSender()) >= inputTokenAmount,
        ...
        uint256 upfrontAmount = outputTokenAmount
            .mul(launchPadUpfrontPercent)
            .div(100);

        uint256 leftOverAmount = outputTokenAmount.sub(upfrontAmount);
        ...
        vestingInfo[_msgSender()] = LaunchPadVestingInfo({
            ...
            paymentAmount: inputTokenAmount,
        ...
        currentCapAmount = currentCapAmount.add(inputTokenAmount);
        historyContract.addFcfsHistory(
            gameCode,
            _msgSender(),
            tierName,
            address(this)
        );
        emit LaunchPadTokenBought(_msgSender(), outputTokenAmount);
    }
