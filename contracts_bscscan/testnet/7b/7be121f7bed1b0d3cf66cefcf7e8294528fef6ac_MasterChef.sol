...
contract TokenToken is BEP20 {
    ...
    // Burn rate % of transfer tax. (default 30% x 10% = 3% of total amount).
    ...
    // The trading pair
    address public tokenSwapPair = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
    // In swap and liquify
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "LUFFYS::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
     ...
     */
    constructor() public BEP20("LuffySwap", "LUFFYS") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "LUFFYS::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "LUFFYS::transfer: Tax value invalid");
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "LUFFYS::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "LUFFYS::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "LUFFYS::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
    ...
    function updateTokenSwapRouter(address _router) public onlyOperator {
        ...
        require(tokenSwapPair != address(0), "LUFFYS::updateTokenSwapRouter: Invalid pair address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "LUFFYS::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "LUFFYS::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "LUFFYS::delegateBySig: invalid nonce");
        require(now <= expiry, "LUFFYS::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "LUFFYS::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "LUFFYS::_writeCheckpoint: block number exceeds 32 bits");
