...
contract GrinchDividendDistributor is IDividendDistributor {
    ...
    //Address for Bitcoin Binance (BSC) 
    IERC20 REWARD = IERC20(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c); 
...
contract MetaGrinch is ERC20, Ownable {
    ...
    GrinchDividendDistributor public dividendDistributor;
    ...
    constructor() ERC20("MetaGrinch", "METAGRINCH") {
        ...
        dividendDistributor = new GrinchDividendDistributor(address(_uniswapV2Router), owner());
    ...
    function updateDividendDistributor(address newAddress) public onlyOwner {
        require(newAddress != address(dividendDistributor), "Grinch: The dividend distributor already has that address");

        GrinchDividendDistributor newDividendDistributor = GrinchDividendDistributor(payable(newAddress));

        require(newDividendDistributor._owner() == address(this), "Grinch: The new dividend distributor must be owned by the Test token contract");
