...
contract BSCBridge is BridgeBase {
    ...
    function depositTokens(
        ...
        bytes32 txHash = keccak256(abi.encode(transAmount, nonce_count, recipient, msg.sender));

        bytes memory _transactionID = abi.encode(transAmount, nonce_count, recipient, msg.sender);
        emit TokenDeposit(
             txHash,
            _transactionID
        );
