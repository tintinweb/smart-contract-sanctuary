...
8% pays reflections to holders
2% pays marketing fee
2% pays technology and development wallet
3% pays liquidity fee
...
contract Aware is Context, BEP20, Ownable {
    ...
    uint256 private _tTotal = 5000 * 10**3 * 10**6;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

// dev address
    address private _devAddress = 0xA97b397503c702F0D601fC86F3dCda80B50B820b;
    
    // marketing address
    address private _marketingAddress = 0x89A227d1eb3B22b04d049C639C7988D5264aE984;
    ...
    uint256 public _maxTxAmount = 5 * 10**3 * 10**6;
    uint256 private numTokensSellToAddToLiquidity = 5 * 10**3 * 10**6;
    ...
    constructor () public {
        ...
        // exclude owner, dev wallet,marketing wallet and this contract from fee
    ...
    function burnAddress() private view returns (address) {
        return _burnAddress;
    }
    ...
    function deliver(uint256 tAmount) public {
        ...
        (,uint256 tFee, uint256 tLiquidity, uint256 tDev, uint256 tMarketing,uint256 tBurn) = _getTValues(tAmount);
        (uint256 rAmount,,) = _getRValues(tAmount, tFee, tLiquidity, tDev,tMarketing, tBurn, _getRate());
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        (,uint256 tFee, uint256 tLiquidity, uint256 tDev,uint256 tMarketing, uint256 tBurn) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount,) = _getRValues(tAmount, tFee, tLiquidity, tDev,tMarketing, tBurn, _getRate());
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tDev,uint256 tMarketing, uint256 tBurn) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tDev,tMarketing, tBurn, _getRate());
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (tTransferAmount, tFee, tLiquidity, tDev,tMarketing,tBurn);
    }
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tDev, uint256 tMarketing, uint256 tBurn, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
