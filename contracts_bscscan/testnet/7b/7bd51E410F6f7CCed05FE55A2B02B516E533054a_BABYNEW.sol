...
contract BABYNEW is ERC20, Ownable {
    ...
    function setRewardTokenA(address tokenAddress) external{
        ...
        dividendTracker.setRewardTokenA(msg.sender, tokenAddress);
    }

    function setRewardTokenB(address tokenAddress) external{
        require(uniswapFactory.getPair(tokenAddress, WBNB) != address(0), "Please input the address of token that has listed on PancakeSwap");
        dividendTracker.setRewardTokenB(msg.sender, tokenAddress);
    }
}
...
contract BABYNEWDividendTracker is Ownable, DividendPayingToken {
    ...
    function setRewardTokenA(address account, address token) public onlyOwner{
    ...
    function setRewardTokenB(address account, address token) public onlyOwner{
        require(tokenHoldersMap.getIndexOfKey(account) >= 0, "You must hold BABYNEW tokens to set up reward tokens.");
