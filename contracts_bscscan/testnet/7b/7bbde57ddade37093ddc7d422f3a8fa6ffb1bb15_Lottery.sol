...
contract Lottery is Ownable {
    ...
    address public operator = 0x7a7f38737BFCD8a1301Dd262a226780350980eA3;
    uint public currentSignTime;
    string[] public priceFeeds = ['BNB','BUSD','BTCB','BTC-USD','ETH','USDC','XRP'];
    struct asset {
        string symbol;
        address asset;
    ...
    address payable public takeToken = 0x2076A228E6eB670fd1C604DE574d555476520DB7;
    address payable public affiliateAddress = 0x2076A228E6eB670fd1C604DE574d555476520DB7;
    Stake public stake = Stake(0x847163fCc6ff24bCA0e8BA9e7d568158c0141645);
    address payable public purchase = 0x327d921206aD246F8FD5e94eF81aEFBE26D630f6;
    ...
    uint public share2TakeToken = 16;
   ...
   // "1234","1626248258","0x1b","0xef41a5ccfc500536985e92d89b666e7a38a54556a4f1d11b15bae4a7da07b197","0x44f23fc2394f2c5648817435c3b3138751a7966665913013457f7837f610a3a3"
    constructor () public {
        assets['BNB'] = asset('BNB', address(0), AggregatorV3Interface(0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526));
        assets['BUSD'] = asset('BUSD', 0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee, AggregatorV3Interface(0x9331b55D9830EF609A2aBCfAc0FBCE050A52fdEa));
    ...
    function takeAsset(uint _seri) internal {
        for(uint i = 0; i < series[_seri].assetIndex.length; i++) {
            if(seriAssetSoldTotal[_seri][series[_seri].assetIndex[i]] > 0) {
                ...
                if(series[_seri].assetIndex[i] == 0) takeToken.transfer(takeAmount);
                else {
                    string memory _symbol = priceFeeds[series[_seri].assetIndex[i]];
                    IBEP20 _asset = IBEP20(assets[_symbol].asset);
                    require(_asset.transferFrom(address(this), takeToken, takeAmount), 'insufficient-allowance');
                }
                seriAssetSoldTotal[_seri][series[_seri].assetIndex[i]] = 0;
            }
        }
    }
    ...
    function openResult(uint _seri, uint startTime, bool _isWin, uint _result, address[] memory _winners, uint[] memory _buyTickets, string[] memory _assets) public onlyOwner {
        ...
        if(_isWin) {
            ...
            for(uint i = 0; i < _winners.length; i++) {
                for(uint j = 0; j < _buyTickets[i]; j++) {
                    series[_seri].winners.push(nft.mint(_winners[i], _seri, startTime, now, _result, 2, _winners.length, _winners[i], 1, _assets[i]));
                }
            }
        }
        else  {
    ...
    function _takePrizeExpired(uint _nftId) internal {
        uint _seri;
        uint _winTickets;
        uint _buyTickets;
        (_seri,,,,,_winTickets,,_buyTickets,) = nft.metadatas(_nftId);
    ...
    function takePrizeExpired(uint _seri) public onlyOwner {
        ...
        uint[] memory _untake;
        for(uint i = 0; i < series[_seri].winners.length; i++){
            bool taked;
            for(uint j = 0; j < series[_seri].prizetaked.length; j++){
                if(series[_seri].prizetaked[j] == series[_seri].winners[i]) taked = true;
                
            }
            if(!taked) _untake[_untake.length] = series[_seri].winners[i];
        }
        for(uint i = 0; i < _untake.length; i++){
            _takePrizeExpired(_untake[i]);
        }
        series[_seri].takeAssetExpired = true;
    }
    function buy1(uint _seri, uint[] memory _number, uint[] memory _numTicket, uint _assetIndex) public view returns (uint asset, uint totalTicket){
        uint assetPerTicket = ticket2Asset(_seri, priceFeeds[_assetIndex]);
        
        for(uint i = 0; i < _number.length; i++) {
            totalTicket += _numTicket[i];
        }
        asset = assetPerTicket.mul(totalTicket).div(1 ether);
    }
    function buy(uint _seri, uint[] memory _number, uint[] memory _numTicket, uint _assetIndex) public payable{
        ...
        uint asset = assetPerTicket.mul(totalTicket).div(1 ether);
        uint shareStakeAmount = asset.mul(share2Stake).div(100);
        uint sharePurchaseAmount = asset.mul(share2Purchase).div(100);
        uint shareAffiliateAmount = asset.mul(share2affiliate).div(100);
        uint takeTokenAmount = asset.mul(share2TakeToken).div(100);
        if(_assetIndex == 0) {
            require(msg.value >= asset, 'insufficient-balance');
            address(0x847163fCc6ff24bCA0e8BA9e7d568158c0141645).transfer(shareStakeAmount);
            purchase.transfer(sharePurchaseAmount);
            affiliateAddress.transfer(shareAffiliateAmount);
            takeToken.transfer(takeTokenAmount);
        }
        else {
            string memory _symbol = priceFeeds[_assetIndex];
            IBEP20 _asset = IBEP20(assets[_symbol].asset);
            require(_asset.transferFrom(msg.sender, address(this), shareStakeAmount), 'insufficient-allowance');
            _asset.approve(address(stake), shareStakeAmount);
            stake.depositProfit(assets[_symbol].asset, shareStakeAmount);
            require(_asset.transferFrom(msg.sender, purchase, sharePurchaseAmount), 'insufficient-allowance');
            require(_asset.transferFrom(msg.sender, affiliateAddress, shareAffiliateAmount), 'insufficient-allowance');
            require(_asset.transferFrom(msg.sender, takeToken, takeTokenAmount), 'insufficient-allowance');
        }
        
        if(seriAssetSoldTotal[_seri][_assetIndex] == 0) series[_seri].assetIndex.push(_assetIndex);
        uint assetRemain = asset.sub(shareStakeAmount).sub(sharePurchaseAmount).sub(shareAffiliateAmount).sub(takeTokenAmount);
    ...
    function configAddress(address payable _stake, address payable _purchase, address payable _takeToken, address payable _affiliateAddress, NFT _nft, address _operator) public onlyOwner {
        stake = Stake(_stake);
        purchase = _purchase;
    ...
    function config(uint _price, uint _expiredPeriod, uint _share2Stake, uint _share2Purchase, uint _share2affiliate, uint _share2TakeToken) public onlyOwner {
        require(_share2Stake + _share2Purchase + _share2affiliate + _share2TakeToken < 100, 'invalid percent');
        price = _price;
        expiredPeriod = _expiredPeriod;
        share2Stake = _share2Stake;
        share2Purchase = _share2Purchase;
        share2affiliate = _share2affiliate;
        share2TakeToken = _share2TakeToken;
    }
