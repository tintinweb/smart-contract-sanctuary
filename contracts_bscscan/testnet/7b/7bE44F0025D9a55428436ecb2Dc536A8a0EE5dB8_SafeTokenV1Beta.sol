    ...
    Tokenomics
{
    ...
    function isDeserver(address _address) public view returns (bool, uint256) {
        for (uint256 s = 0; s < vestingAddresses.length; s += 1) {
            if (_address == vestingAddresses[s]) return (true, s);
        }
        return (false, 0);
    }
    ...
    function withdraw(address _beneficiary, uint256 _amount) external {
        ...
        (bool _isDeserver, uint256 s) = isDeserver(_beneficiary);
        if (_isDeserver) {
    ...
    // function stakesList() external pure returns (VestingItem[] memory) {
    //     return deservers;
    // }

    function getDeservers()
        public
        view
        returns (
            address[] memory,
    ...
    {
        address[] memory addrs = new address[](vestingAddresses.length);
        uint256[] memory amount = new uint256[](vestingAddresses.length);
        uint256[] memory lockExpires = new uint256[](vestingAddresses.length);

        for (uint256 i = 0; i < vestingAddresses.length; i++) {
            VestingItem storage vestingData = deservers[vestingAddresses[i]];

            addrs[i] = vestingAddresses[i];
            amount[i] = vestingData.amount;
            lockExpires[i] = vestingData.lockExpires;
        }
        return (addrs, amount, lockExpires);
    }
