...
contract LottoGame {
    ...
    constructor() public {
        _tokenAddress = 0x42656606d10953a028Bc61a4B3cB33f36814C2c5; //kill this off later if not needed
    ...
    function buyTicket(address payable buyer, uint amount) public {
        ...
        //token.transfer(payable(address(this)), amount); //old method depreciated
        potValue += amount;
    }
    ...
    function endGame() private {
        ...
        if (amountToMarketingAddress > 1) {
            ...
            //token.transfer(payable(_marketingAddress), (amountToMarketingAddress - 1));
        }
    ...
    function setPayoutAmount() private returns(uint){
        ...
        for (uint i; i < numWinners; i++){
            ...
            if (winnerProfits[i] > 0) {
                bnbProfits[i] = swapProfitsForBNB(winnerProfits[i]);  //this is the breaking line
            }
