...
contract PeopleFutures {
    ...
    function balanceOf(address owner) public virtual onlyAdmin view returns (uint256[] memory) {
        uint256[] memory my_balances = new uint256[](3);
        my_balances[0] = balances[owner]["BTC"];
        my_balances[1] = balances[owner]["ETH"];
        my_balances[2] = balances[owner]["USD"];
        return my_balances;
    }
