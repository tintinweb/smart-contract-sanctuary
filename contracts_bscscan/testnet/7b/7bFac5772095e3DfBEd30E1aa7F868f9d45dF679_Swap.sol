...
contract Swap {
    ...
    uint256 taxRateNom = 100;
    uint256 taxRateDenom = 100;
    ...
    function swap(address _tokenIn, address _tokenOut, uint _amountIn, uint _amountOutMin, address _to) external {
      ...
      if(IERC20(_tokenOut).balanceOf(msg.sender) < (this.getAmountOutMin(_tokenIn, _tokenOut, _amountIn)) * taxRateNom / taxRateDenom ) {
        revert("Tax is greater than 20%");
      } 
    }
    ...
    function setTaxCheck(uint256 _taxNom, uint256 _taxDenom) external {
      taxRateNom = _taxNom;
      taxRateDenom = _taxDenom;
    }
}