...
contract StakingUSDC is Staking {
    ...
    function stakingBalanceOf(address account) public override view returns (uint256){
        ...
        if (usdcBalance > 0)
            return denominator * usdcBalance / SHARE_PRECISION;
        else
            return 0;
    }
    ...
    // ------------------------------------------------------------------------
    // User staking entrance - USDC
    // ------------------------------------------------------------------------
    function stake(uint256 amount) public override {
        require(amount > 0, "amount can't be 0!");
        require(USDC.allowance(msg.sender, address(this)) >= amount, "allowance is not enough!");
    ...
    function withdraw(uint256 shareAmount) public override {
        ...
        emit WithdrawFromUSDC(msg.sender, shareAmount);
    }
    ...
    function stakingYield() public onlyGovernance override {
        ...
        uint256 yieldAmount = USDC.balanceOf(address(this)) * yieldPercent / CERES_PRECISION;
        coinCRS.stakingMint(address(this), yieldAmount);

        lastInterval += yieldInterval * intervalCount;
        emit YieldFromUSDC(yieldAmount);
    }

    //****************
    // EVENTS
    //****************
    event StakeUSDC(address indexed from, uint256 amount);
    event WithdrawFromUSDC(address indexed to, uint256 shareAmount);
    event YieldFromUSDC(uint256 yieldAmount);
}
