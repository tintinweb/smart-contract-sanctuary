...
contract Voting is Ownable {
    ...
    //debug
    function getVoteResult(uint256 _voteId) external view voteExist(_voteId) returns (string memory) {
    ...
    function executeVote(uint256 _voteId) external voteExist(_voteId) {
        ...
        if (_canExecute(_voteId)) {
            if (votes[_voteId].requireExecutor)
...
contract DAO is Ownable {
    ...
    constructor(string memory _daoName, uint256 _minimumDuration, uint256 _minimumSupport, uint256 _minimumTokenHolds, ERC20 _voteToken) {
        daoName = _daoName;
