...
contract Matrix is Context, IBEP20, Ownable {
    ...
    uint256 private POWER_DEC = 10**_DECIMALS;
    
    // Reflection
    uint256 public _maxTxAmount = 500 * 10**6 * POWER_DEC;
    uint256 private _tTotal = 100000 * 10**6 * POWER_DEC;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    // Swap & Liquify
    uint256 private numTokensSellToAddToLiquidity = 5 * 10**6 * POWER_DEC;
    bool public swapAndLiquifyEnabled = false;
    bool inSwapAndLiquify;

    // Fee
    struct Fee { uint256 liquidity; uint256 retrib; uint256 burn; }
    Fee public _fee = Fee(0, 0, 0);
    Fee private origFee;  // Track Original Fee
    ...
    // Constructor
    constructor (address tokenOwner, address router) {
    ...
    function _reflectFee(uint256 rFee, uint256 rBurn, Fee memory tFee) private {
    ...
    function _getTValues(uint256 tAmount, Fee memory fee) private pure returns (Fee memory, uint256) {
        ...
        uint256 tTransferAmount =  tAmount.sub(tFee.liquidity).sub(tFee.retrib).sub(tFee.burn);
    ...
    function _getRValues(uint256 tAmount, Fee memory tFee, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rFee = tFee.retrib.mul(currentRate);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, Fee memory) {
        ...
        (uint256 rAmount, uint256 rFee, uint256 rTransferAmount) = _getRValues(tAmount, tFee, _getRate());
    ...
    function _removeFee() private {
        if (_fee.liquidity == 0 && _fee.retrib == 0 && _fee.burn == 0) return;
    ...
    function _transferAction(address sender, address recipient, uint256 tAmount, ExcludedFromFee excludedFromFee) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, Fee memory tFee) = _getValues(tAmount);
        uint256 rBurn = tFee.burn.mul(_getRate());
        
        _rOwned[sender] = _rOwned[sender].sub(rAmount);

        if (excludedFromFee == ExcludedFromFee.TOEXCLUDED) {
        ...
        _takeLiquidity(tFee.liquidity);
        _reflectFee(rFee, rBurn, tFee);
    ...
    function updateFee(uint256 _liquidityFee, uint256 _retribFee, uint256 _burnFee) public onlyOwner {
		require(_liquidityFee < 6 && _retribFee < 6 && _burnFee < 6, "Fee value needs to be less than 6");

        _fee.liquidity = _liquidityFee * 100;
        _fee.retrib = _retribFee * 100;
        _fee.burn = _burnFee * 100;
    ...
    function setNumTokensSellToAddToLiquidity(uint256 swapNumber) public onlyOwner {
        numTokensSellToAddToLiquidity = swapNumber * POWER_DEC;
    }


    function setMaxTxPercent(uint256 maxTxPercent) public onlyOwner {
        _maxTxAmount = maxTxPercent * POWER_DEC;
    }
