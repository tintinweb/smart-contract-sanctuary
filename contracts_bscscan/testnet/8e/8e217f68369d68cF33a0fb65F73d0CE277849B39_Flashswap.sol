...
contract Flashswap {
    ...
    address payable public immutable owner1 =
        0x05Ca3813809c6cd8cd9Ee90f8a216bC2A6c0f8a4;
    address payable public immutable owner2 =
        0x84F33e797364bdFC4511bf5AB35b0372Aa96ba69;
    modifier onlyOwner() {
    ...
    function checkByPair(
        ...
        uint256 amountOut = _getAmountOutByPair(
            _amountWBNB,
            _pairAddress0,
            _wbnbAddress,
            _fee1
        );
        uint256 amountRepay = _getAmountOutByPair(
            amountOut,
            _pairAddress1,
            _wbnbAddress,
            _fee2
        ...
        return (
            int256(amountRepay - _amountWBNB), // our profit or loss; example output: BNB amount
            amountOut // the amount we get from our input "_amountWBNB"; example: BUSD amount
        );
    }
    ...
    function getLiquidity(
        ...
        address _pairAddress1,
        address _wbnbAddress
    ) public view returns (uint112, uint112, uint112, uint112) {
        (uint112 token0Reserves, uint112 token1Reserves, ) = IUniswapV2Pair(
            _pairAddress0
        ).getReserves();
    ...
    function execute(
        ...
        require(amountRepay - amountIn > 0, "no profit");
        ...
        IUniswapV2Pair(_pairAddress0).swap(
            token0 == _wbnbAddress ? 0 : amountOut,
            token0 == _wbnbAddress ? amountOut : 0,
            _pairAddress1,
            new bytes(0)
        );
        ...
        IUniswapV2Pair(_pairAddress1).swap(
            token0p == _wbnbAddress ? amountRepay : 0,
            token0p == _wbnbAddress ? 0 : amountRepay,
            address(this),
            new bytes(0)
        );
    }
