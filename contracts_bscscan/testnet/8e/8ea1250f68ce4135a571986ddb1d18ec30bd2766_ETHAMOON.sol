...
contract DividendDistributor is IDividendDistributor {
    ...
    //address DEXROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E; //mainet
    address DEXROUTER = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; // TESTNET
...
contract ETHAMOON is IBEP20, Auth {
    using SafeMath for uint256;

    string constant _name = "Ethamoon";
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0xa8d3a174a00aA34FD9CC76AfF79166C0819bD704;
        ecosystemfeeReceiver = 0xa1bd8215Ab781724c7A9c1D3cD54613EDd080329;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if(_allowances[sender][msg.sender] != uint256(-1)){
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Ethamoon: Insufficient Allowance");
        }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != ecosystemfeeReceiver  && recipient != autoLiquidityReceiver && recipient != burnFeeReceiver){
            ...
            require((heldTokens + amount) <= _maxWalletToken, "Ethamoon: Total Holding is currently limited, you can not buy that much.");}
        
        if (sender == pair &&
            buyCooldownEnabled &&
            !isTimelockExempt[recipient]) {
            require(cooldownTimer[recipient] < block.timestamp, "Ethamoon: Please wait for 1min between two buys");
        ...
        //Exchange tokens
        _balances[sender] = _balances[sender].sub(amount, "Ethamoon: Insufficient Balance");
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "Ethamoon: TX Limit Exceeded");
    }
    ...
    function setIsFeeExemption(address holder, bool exempt) external authorized {
        isFeeExempt[holder] = exempt;
    }

    function setIsDividendExemption(address holder, bool exempt) external authorized {
    ...
    function setIsTimelockExemption(address holder, bool exempt) external authorized {
        isTimelockExempt[holder] = exempt;
    }

    function setIsTxLimitExemption(address holder, bool exempt) external authorized {
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _ecosystemfee, uint256 _burnFee, uint256 _feeDenominator) external authorized {
        ...
        require(totalFee < feeDenominator/2, "Ethamoon: Fees cannot be more than 50%");
    }
