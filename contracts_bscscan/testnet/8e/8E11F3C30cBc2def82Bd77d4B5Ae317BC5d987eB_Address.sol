...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    /**
     * recipient as LP-pair to be called to initialise timestamp for WhitelistRound
     * 
     */
...
contract LGEWhitelisted is Context {
    ...
    function resetTimestamp() external onlyWhitelister() {
        _lgeTimestamp = 0;
    }
...
// ForceToken with Governance.
contract ForceCoin is BEP20('TheForce Coin', 'FOC'), LGEWhitelisted {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (JediMaster).
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "FORCE::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "FORCE::delegateBySig: invalid nonce");
        require(now <= expiry, "FORCE::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "FORCE::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying FORCEs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "FORCE::_writeCheckpoint: block number exceeds 32 bits");
