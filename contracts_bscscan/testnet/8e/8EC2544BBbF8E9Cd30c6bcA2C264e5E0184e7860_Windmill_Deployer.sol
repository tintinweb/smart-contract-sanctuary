...
interface I_Windmill_Deployer is I_With_PancakeRouter, I_With_BUSDToken, I_With_WBNBToken{
    struct Windmill{
        ...
        I_Windmill_Trade_Deployer[] tradeDeployers;
        bytes tradeDeployerBytecode;
        bool initialized;
    }
    ...
    function setTradeDeployerBytecode(bytes memory bytecode) external;
    
    function deployTrade(bytes calldata bytecode, string calldata name) external returns (address payable);
...
contract Windmill_Deployer is Adressable, Ownable, With_PancakeRouter, With_BUSDToken, With_WBNBToken, I_Windmill_Deployer{
    ...
    function setTradeDeployerBytecode(bytes memory bytecode) external onlyOwner{
        windmill.tradeDeployerBytecode = bytecode;
    }
    
    function deployTrade(bytes calldata bytecode, string calldata name) external onlyOwner returns (address payable){
        address payable addr = payable(Create2.deploy(0, bytes32(salt), windmill.tradeDeployerBytecode));
        salt += 1;
        
        I_Windmill_Trade_Deployer deployer = I_Windmill_Trade_Deployer(addr);
        deployer.setDeployerCode(bytecode, name);
    ...
    function initWindmill(uint256[11] calldata data,
                          bool[] calldata isProposalSuper) external onlyOwner{
        ...
        windmill.fund.setDAO(windmill.DAO);
        ...
        for (uint256 i=0; i<windmill.tradeDeployers.length; i++){
            windmill.tradeDeployers[i].setBUSDToken(BUSDToken);
