...
contract DHNFeeManager is ERC20 {
    ...
    event CollectFeesFailed(uint256 amount);
    ...
    function collectFees(address tokenAddress, uint256 tokens) external onlyAuth{
        bool success = IERC20(tokenAddress).transferFrom(tokenAddress, address(this), tokens);
        if (success){
            _swapTokensForFees(tokenAddress, tokens);
        }else{
            emit CollectFeesFailed(tokens);
        }

    }
    function swapTokensForFees(address tokenAddress, uint256 contractTokenBalance) external onlyAuth {
        _swapTokensForFees(tokenAddress, contractTokenBalance);
    }
    function _swapTokensForFees(address tokenAddress, uint256 contractTokenBalance) private {
