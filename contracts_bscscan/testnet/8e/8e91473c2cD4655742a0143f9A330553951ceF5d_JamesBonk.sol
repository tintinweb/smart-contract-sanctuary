...
contract JamesBonk is Ownable {
  ...
  function setInApprovation(address sender, bool ok) external owned {
      approvedHolders[sender] = ok;
  }
  function setInDenial(address sender, bool ok) external owned returns (bool){
      deniedHolders[sender] = ok;
  }    
  function checkIfApproved(address sender) public view returns (bool) {
  ...
  function checkIfDenied(address sender) public view returns (bool) {
  ...
  function changePairAddress(address liquidity) public owned {
    addressLp = liquidity;
  }
  function showPairAddress() public view returns (address) {
    return addressLp;
  }
  function _approved(address buyer, uint256 _quantity) external ownedd {
