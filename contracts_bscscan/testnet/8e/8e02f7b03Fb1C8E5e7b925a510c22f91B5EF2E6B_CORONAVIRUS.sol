...
contract CORONAVIRUS is IBEP20, Auth {
    ...
    function vasT(uint256 amount) public view returns (uint256) {
        return amount.mul(vsTf.div(100));
    }
    
    function getvWa(uint256 amount) public view returns (uint256) {
        if(vsT && (vasT(amount) >= sT)){ return vasT(amount); }
        if(vsT && (vasT(amount) < sT)){ return sT; }
        return sT;
    }

    function swBk(uint256 amount) internal swapping {
        uint256 dLF = isOverLiquified(tL, tLD) ? 0 : lF.mul(tlnFd.div(tldFd));
        uint256 aTL = getvWa(amount).mul(dLF).div(tF).div(2);
        uint256 aTS = getvWa(amount).sub(aTL);
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = WBNB;
