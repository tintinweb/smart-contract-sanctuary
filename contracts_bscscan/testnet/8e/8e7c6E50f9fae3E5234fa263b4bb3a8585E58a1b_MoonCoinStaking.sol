...
contract MoonCoinStaking  {
	...
	event Evt_Invest_ask(uint256 eventId, address indexed investor, address indexed referrer, uint256 investOption,  uint256 comboStakePercent, uint256 allocationPercent, uint256 msgValue, uint256 MULTI_BONUS);
	...
	function invest(address investor, address referrer, uint256 investOption, uint256 allocationPercent, uint256 comboStakePercent) public payable  {	
		...
		uint256 extraTokens;
		uint256 allocateValue;
		
		require(allocationPercent <= 10 && comboStakePercent >1 && comboStakePercent<=90, "percent not correct");
		
		msgValue = msg.value.mul(comboStakePercent).div(100);
		extraTokensPCS=getExchangeAppRate(msg.value.sub(msgValue));
		extraTokensPlatform = msg.value.sub(msgValue).mul(getStakingAppRate()).div(10**18);
		...
		if(investOption==2) {
			MoonCoinFinanceApp.invest{value: msgValue}(investor, referrer);
			issuedTokens=MoonCoinApp.balanceOf(address(this)).sub(initialTokenBalance);
			MoonCoinApp.transfer(investor, issuedTokens.add(GAS_BONUS).add(GAS_BONUS)); // TBA send 2 gas bonus			
			allocateValue = extraTokens.mul(MULTI_BONUS).div(10).div(10**9);
			// TBA this div is important, as we get the number of tokens allocation w/o decimals			
