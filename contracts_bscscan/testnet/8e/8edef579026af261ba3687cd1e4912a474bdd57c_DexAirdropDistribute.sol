...
contract DexAirdropDistribute {
     ...
     */
    function executeDistribute(address token, uint256 amount) public {
        require(token != address(0), "token address can not be empty");
        address ownerAddress = msg.sender;
        require(distributeMap[ownerAddress] >= amount && IERC20(token).balanceOf(address(this)) >= amount, "balance is not enough");
        // IERC20(token).transferFrom(address(this), msg.sender, amount);
        IERC20(token).transfer(ownerAddress, amount);
        distributeMap[ownerAddress] -= amount;
        emit DidExecuteDistribute(ownerAddress, amount);
    }
