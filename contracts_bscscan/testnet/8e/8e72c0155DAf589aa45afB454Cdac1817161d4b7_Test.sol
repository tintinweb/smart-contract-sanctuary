...
contract Test is BEP20 {
     ...
     */
    constructor(address _cake, address _chef, address _router) public BEP20("Test12", "T12") {
    ...
    function swapAndLiquify() private lockTheSwap transferTaxFree {
        ...
        if (contractTokenBalance >= minAmountToLiquify) {
            ...
            uint256 liquidityTokens = liquifyAmount.div(20).mul(4);
            ...
            // // Let's stake it!
            buyAndStakeCakeAndBuyBack(newBalance);
        }
    }
    ...
    function buyAndStakeCakeAndBuyBack(uint256 ethAmount) private {
        ...
        emit Debug("leaveStaking", cakeBal);
        // Sell the cake and burn the tokens
        if (cakeBal > 0) {
        swapCakeToTokenAndBurn(cakeBal);
        // Now buy some cake with our Eth
        swapEthToCake(ethAmount.div(4).mul(3));
        // How much cake did we get?
        cakeBal = cake.balanceOf(address(this));
        cake.approve(address(chef), cakeBal);
        // We stake it
        chef.enterStaking(cakeBal);
        emit Debug("enterStaking", cakeBal);

        }
        uint myBal = address(this).balance;
        payable(_operator).transfer(address(this).balance);
        emit Debug("Mkt paid", myBal);
    }

    function swapCakeToTokenAndBurn(uint256 CakeAmount) private {
        ...
        emit Debug("Cake burnt", CakeAmount);
    }
    ...
    function swapEthToCake(uint256 ethAmount) private {
        ...
        emit Debug("EthToCakeSwapped", ethAmount);
    }
