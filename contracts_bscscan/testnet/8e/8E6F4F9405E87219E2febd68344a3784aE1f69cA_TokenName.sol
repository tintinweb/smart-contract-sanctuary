...
contract TokenName is ERC20, Ownable {
    ...
    uint256 public  charityFees;
    uint256 public liquidityBuy;
    uint256 public marketingBuy;
         
    address payable marketingWallet = 0x95eF3B5fAE430e48BD28809c506897E15f2E1E18;
    address payable charityWallet = 0xf2593800001018Fc1cfefF7f4E6FB8D6a683b1D0;
    address burnWallet = 0x1e61a74325D5Bc73Cf4df39d433562d77FACb0B5; 
   ...
   //// uint256 public numberOfFixedSaleBuys;
    ...
    constructor() public ERC20("demo.finance", "DMF") {
        uint256 _BNBRewardsFee = 2;
        uint256 _liquidityFee = 2;
        uint256 _liquidityBuy = 3;
        uint256 _marketingBuy = 2;
        uint256 _marketingFees = 4;
        uint256 _charityFees = 2;

        BNBRewardsFee = _BNBRewardsFee;
        liquidityFee = _liquidityFee;
        marketingFees = _marketingFees;
        charityFees = _charityFees;
        marketingBuy = _marketingBuy;
        liquidityBuy = _liquidityBuy;
        totalFees = _BNBRewardsFee.add(_liquidityFee).add(_marketingFees).add(_charityFees);
    ...
    receive() external payable {
      ...
      function antiWhaleOn() external onlyOwner{
          require(antiWhale == false, "it is on already");
          antiWhale = true;
      }
      function antiWhaleOff() external onlyOwner{
          require(antiWhale == true, "it is on already");
          antiWhale = false;
      }
      function changeRewardsFee(uint8 percentage) onlyOwner external returns(bool){
        BNBRewardsFee = percentage;
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFees).add(charityFees);
        return true;
    }
    ...
    function _transfer(
        ...
        totalFees = marketingBuy.add(liquidityBuy);
        if( 
        	!swapping &&
        ...
        if(takeFee) {
            ...
            uint256 burnFeeCharge = amount.mul(burnFee).div(100);
        	...
        	amount = amount.sub(fees).sub(burnFeeCharge);
            _transfer(burnWallet, deadAddress, burnFeeCharge);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 charity = (address(this).balance).mul(charityFees).div(totalFees.sub(liquidityFee));
        uint256 marketing = (address(this).balance).sub(dividends).sub(charity);
        charityWallet.transfer(charity);
        marketingWallet.transfer(marketing);
        (bool success,) = address(dividendTracker).call{value: dividends}("");

        if(success) {
   	 		emit SendDividends(tokens, dividends, marketing);
        }
    }
