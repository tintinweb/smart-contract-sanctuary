...
interface NFT {
    function boxRandomMint(address to, uint256 sourceId) external returns(uint256);
}
...
contract BlindBox is ERC721, Ownable{
    ...
    event Open(address indexed user, uint256 indexed boxId, uint256 indexed NFTId);
   ...
   */
    ...
    function setNFTAddress(address _nftAddr) external onlyOwner{
        require(isContract(_nftAddr), "is not contract Address");
        NFTAddress = _nftAddr;
    }
    ...
    function open(uint256 id) external {
        require(ownerOf(id) == _msgSender(), "open: caller is not the nft owner");
        uint256 nftId = NFT(NFTAddress).boxRandomMint(_msgSender(), id);
        _burn(id);
        emit Open(_msgSender(), id, nftId);
    }
