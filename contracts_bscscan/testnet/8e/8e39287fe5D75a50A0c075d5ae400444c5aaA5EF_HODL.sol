...
contract HODL is Context, IBEP20, Ownable, ReentrancyGuard {
        ...
        initializer
    {
        ...
        sellAllocation = SellAllocation(75, 125, 30, 20);
        buyAllocation = BuyAllocation(60, 20, 10, 10);
        transferAllocation = TransferAllocation(60, 20, 10, 10);

        selltax = sellAllocation.bnbReserve.add(
            sellAllocation.bnbReward.add(
                sellAllocation.liquidity.add(sellAllocation.reflection)
            )
        );
        buytax = buyAllocation.bnbReward.add(
            buyAllocation.liquidity.add(
                buyAllocation.marketing.add(buyAllocation.reflection)
            )
        );
        transfertax = transferAllocation.bnbReward.add(
            transferAllocation.liquidity.add(
                transferAllocation.marketing.add(transferAllocation.reflection)
            )
        );
        ...
        teamwallet = 0xb52C7EA77Dd1F5B374B6A6bFc19a6665753D64F6;
    ...
    function _transfer(
        ...
        if (
            ...
            _taxFee = sellAllocation.reflection;
            _liquidityFee = sellAllocation.bnbReserve.add(
                sellAllocation.bnbReward.add(sellAllocation.liquidity)
            );
        ...
        // take buy fee
            ...
            if (!checkTaxAndLiquidityFees()) {
                _taxFee = buyAllocation.reflection.div(2);
                _liquidityFee = (buyAllocation.bnbReward.add(
                    buyAllocation.liquidity.add(buyAllocation.marketing)
                )).div(2);
            } else {
                _taxFee = buyAllocation.reflection;
                _liquidityFee = buyAllocation.bnbReward.add(
                    buyAllocation.liquidity.add(buyAllocation.marketing)
                );
            }
        ...
        // take transfer fee
            ...
            if (takeFee && from != owner() && from != address(this)) {
                _taxFee = transferAllocation.reflection;
                _liquidityFee = transferAllocation.bnbReward.add(
                    transferAllocation.liquidity.add(
                        transferAllocation.marketing
                    )
                );
            }
        }
