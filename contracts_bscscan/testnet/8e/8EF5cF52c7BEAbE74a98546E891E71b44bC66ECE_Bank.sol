...
contract Bank is Crowdsale, Ownable {

	using SafeMath for uint256;

	uint256 public minimumInvest = 0.01 ether;
	uint256 public maximumInvest = 5 ether;
	uint256 public crowdsaleCap = 100 ether;
	...
	enum CrowdsaleStage { PREPARE, SALING, SNAPPING, FUNDING }
	CrowdsaleStage public stage = CrowdsaleStage.PREPARE;
  ...
  address public partnersTimelock;
	...
	function setMinimumInvest (uint256 value) external onlyOwner {
		require(minimumInvest > 0);
		minimumInvest = value;
	}

	function setMaximumInvest (uint256 value) external onlyOwner {
		maximumInvest = value;
	}

	function setCrowdCap (uint256 _cap) external onlyOwner {
	...
	function _processPurchase(address _beneficiary, uint256 _tokenAmount) internal override {
		claims[_beneficiary] = claims[_beneficiary].add(_tokenAmount);
	}
	...
	function getUserTokensToBeClaim (address _beneficiary) external view returns (uint256) {
	...
	function claimToken () external view {
		require(closingTime <= block.timestamp, "Claim: Not this time");
	}

	function buyTokens(address _beneficiary) public override payable {
		require(openingTime < block.timestamp, "Buy Token: Not open");
		require(closingTime >= block.timestamp, "Buy Token: Ended");
		super.buyTokens(_beneficiary);
	}

	function _preValidatePurchase(address _beneficiary, uint256 _weiAmount) internal override view {
		super._preValidatePurchase(_beneficiary, _weiAmount);
		require(contributions[_beneficiary].add(_weiAmount) >= minimumInvest, "Minimum invest required");
		require(contributions[_beneficiary].add(_weiAmount) <= maximumInvest, "Over cap");
	}
