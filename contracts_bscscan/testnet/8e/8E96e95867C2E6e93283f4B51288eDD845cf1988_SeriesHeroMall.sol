...
contract SeriesHeroMall is Ownable {
    ...
    constructor(address _heroAddress){
    ...
    function recruit(uint256 priceIndex) public payable {
        ...
        string memory _name = price.name;
        address whiteListContract = price.whiteListContract;
        ...
        string memory name = string(abi.encodePacked(_name, "#", uint2str(sellPrice[priceIndex].sellAmount)));
    ...
    function addPrice(address _exchangeToken,uint256 _rarity,uint256 _mineralLevel,uint256 _series,uint256 _race,uint256 _price,uint256 _totalSupply,string memory _name,address _whiteListContract) public onlyController {
    ...
    function updatePrice(uint256 index,address _exchangeToken,uint256 _rarity,uint256 _mineralLevel,uint256 _series,uint256 _race,uint256 _price,uint256 _totalSupply,string memory _name,address _whiteListContract) public onlyController {
        ...
        sellPrice[index].name = _name;
        sellPrice[index].whiteListContract = _whiteListContract;
        sellPrice[index].race= _race;
    }

    function setWhiteContract(uint256 index,address _whiteListContract) public onlyController {
         sellPrice[index].whiteListContract = _whiteListContract;
    }
    ...
    function getPriceByIndex(uint256 index) public view returns (CurrentPrice memory) {
        ...
        CurrentPrice memory currentPrice = CurrentPrice({
           ...
           name:price.name,
           whiteListContract:price.whiteListContract
        });
        return currentPrice;
    }
    ...
    function accountIsInWhiteList(uint256 priceIndex,address account) public view returns(bool) {
        address whiteListContract = sellPrice[priceIndex].whiteListContract;
        if(whiteListContract == address(0)){
            return true;
        }else{
