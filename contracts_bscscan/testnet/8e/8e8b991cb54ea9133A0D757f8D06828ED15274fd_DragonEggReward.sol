...
contract DragonEggReward is Ownable, ReentrancyGuard, IDragonEggReward {
    ...
    constructor(address _signer, address _beneficiaryAddress) {
        signer = _signer;
        beneficiaryAddress = _beneficiaryAddress;
    }

    function claim(uint salt, address eggToken, uint price, Sig calldata sig) override public nonReentrant {
        require(price > 0, "price must be greater than 0");
        require(!usedSalts[salt], "Already claim ");
        validateSig(salt, eggToken, price, sig);
        usedSalts[salt] = true;
        IERC20(twinciToken).safeTransferFrom(msg.sender, beneficiaryAddress, price);
        uint tokenId =  IDragonEgg(eggToken).mint(msg.sender);
        emit RewardClaimed(msg.sender, eggToken, tokenId);
    }

    function claimTest(address eggToken, uint price) public onlyOwner {
        IERC20(twinciToken).safeTransferFrom(msg.sender, beneficiaryAddress, price);
        uint tokenId =  IDragonEgg(eggToken).mint(msg.sender);
        emit RewardClaimed(msg.sender, eggToken, tokenId);
    }
    function validateSig(uint salt, address egg, uint price, Sig memory sig) internal view {
        require(keccak256(abi.encode(this, salt, egg, price, msg.sender)).toString().recover(sig.v, sig.r, sig.s) == signer, "incorrect reward signature");
    }
    ...
    function changeBeneficiary(address newBeneficiaryAddress) override public onlyOwner {
        require(newBeneficiaryAddress != address (0), "Wrong beneficiaryAddress");
        require(beneficiaryAddress != newBeneficiaryAddress, "beneficiaryAddress existed");
...
interface IDragonEggReward  {
    function claim(uint salt, address eggToken, uint price, Sig calldata sig) external;
