...
contract LevelManager is Adminable, ILevelManager, WithLevels, WithPools {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            ...
            if (poolEnabled[poolAddr]) {
                ...
                try pool.userInfo(account) returns (
                    ...
                    totalAmount += (userInfo.amount * multiplier) / DEFAULT_MULTIPLIER;
                    continue;
                } catch {}
            }
        }
    ...
    function getAlwaysRegister() external view override returns (address[] memory, string[] memory, uint256[] memory) {
...
abstract contract WithLevels is Adminable, ILevelManager {
    ...
    function getTierForAmount(uint256 amount) internal view returns (Tier memory) {
        return tiers[getTierIdxForAmount(amount)];
    }

    function getTierIdxForAmount(uint256 amount) internal view returns (uint256) {
        if (amount == 0) {
            return 0;
        }
        uint256 maxTierK = 0;
