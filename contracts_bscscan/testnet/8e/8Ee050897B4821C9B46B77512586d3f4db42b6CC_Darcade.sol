...
contract Darcade is ReentrancyGuard, Ownable {
    ...
    */
    ...
    /**
     * @dev {_credits} The wallet to credit mapping
    */
     ...
     */
    mapping (string => address) public userGameSession;


    /**
     * @dev {GameSession} Fires when coin is inserted
     */
    event GameSession(address wallet, uint256 amount, string gameId);

    /**
     * @dev {SessionComplete} Fires when session is complete
     */
    event SessionComplete(string session);
    ...
    */
     ...
     */
    function insertCoin(string memory gameId) public nonReentrant payable {
        address beneficiary = _msgSender();
        uint256 weiAmount = msg.value;
        _preValidatePurchase(beneficiary, weiAmount);

        updateUserGameSession(gameId);
        _forwardFunds();

        emit GameSession(beneficiary, _credits[beneficiary], gameId);
    }

    /**
     * @dev Adds a game in session value
     */
    function updateUserGameSession(string memory gameId) internal {
        userGameSession[gameId] = _msgSender();
    }
    ...
    */
     ...
     */
    function resetUserGameSession(string memory gameId) public {
        require(userGameSession[gameId] == _msgSender(), "You must be the owner of this session");
        delete userGameSession[gameId];
        emit SessionComplete(gameId);
    }
