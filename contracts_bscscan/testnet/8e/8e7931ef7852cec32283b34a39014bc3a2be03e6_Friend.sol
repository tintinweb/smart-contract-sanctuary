...
contract Friend is Initializable, IFriend, Managed {
  ...
  function requestFriend(address receiver) external {
    ...
    // check sender's tags
    for (uint256 i = 0; i < friendRequestTags[receiver].length; i++) {
      require(
        oracleTag().hasTag(msg.sender, friendRequestTags[receiver][i]),
        "Not has tag"
      );
    }
    friendRequests[receiver][msg.sender] = true;
   ...
   * @param gift Some RNT as gift to receiver. Can be zero.
   */
  function requestFriendWithGift(address receiver, uint256 gift) external {
    ...
    friendRequests[receiver][msg.sender] = true;
    if (gift > 0) {
      relationToken().transferFrom(msg.sender, address(this), gift);
      friendRequestGifts[receiver][msg.sender] = gift;
    }
  ...
  function receiveFriendRequest(address inviter) external {
    ...
    uint256 gift = friendRequestGifts[msg.sender][inviter];
    relationToken().transfer(msg.sender, gift);
  ...
  /** block some user that he/she can not send friend request */
...
import "../interfaces/IOracleTag.sol";

/**
 * @title Relation Managed contract
...
contract Managed {
    ...
    function _onlyGovernor() internal view {
        require(
            ...
            "Caller must be Controller governor"
        );
    }
    ...
    /**
     * @dev Return RelationToken interface.
     * @return Relation token contract registered with Controller
     */
    function relationToken() internal view returns (IRelationToken) {
        return IRelationToken(_resolveContract(keccak256("RelationToken")));
    }
    ...
    /**
     * @dev Return Group interface.
     * @return Relation Group contract registered with Controller
     */
    function group() internal view returns (IGroup) {
        return IGroup(_resolveContract(keccak256("Group")));
    }

    /**
     * @dev Return Friend interface.
     * @return Relation Friend contract registered with Controller
     */
    function friend() internal view returns (IFriend) {
        return IFriend(_resolveContract(keccak256("Friend")));
    }

    /**
     * @dev Return OracleTag interface.
     * @return Relation OracleTag contract registered with Controller
     */
    function oracleTag() internal view returns (IOracleTag) {
        return IOracleTag(_resolveContract(keccak256("OracleTag")));
    }
     ...
     */
    function _resolveContract(bytes32 _nameHash)
        internal
        view
        returns (address)
    {
...
interface IStaking is IStakingData {
    function setMinimumNodeStake(uint256 _minimumNodeStake) external;
    ...
    function stakeTo(address _node, uint256 _tokens) external;
    ...
    function isOperator(address _operator, address _node)
        external
        view
        returns (bool);
    ...
    function getNodeCapacity(address _node) external view returns (uint256);
...
interface IGroup {
  ...
  function updateGroupInfo(uint256 groupId, bool _joinByUser) external;
  ...
  function rejectGroupRequest(uint256 groupId, address user) external;
  ...
  function addGroupMember(uint256 groupId, address user) external;

  function removeGroupMember(uint256 groupId, address user) external;
  ...
  function chargeToExpandLimit(uint256 groupId) external;
}

// SPDX-License-Identifier: MIT
pragma solidity ^0.7.6;

interface IOracleTag {
  function hasTag(address user, bytes32 tag) external returns (bool);
}
...
interface IStakingData {
    ...
    struct Delegation {
        ...
        uint256 tokensLockedUntil; // Block whe
    }
}
