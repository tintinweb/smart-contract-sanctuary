...
contract HERAPrivateSale is Ownable, ReentrancyGuard {
    ...
    uint256 public min;
    uint256 public max;
    uint256 public tgeClaimPercentage; // 2 decimal places
    uint256 public price; // 3 decimal places
    ...
    mapping(address => uint256) private locks; // HERA
    ...
    function buy(uint256 _amount) public onlyWhitelister canBuy nonReentrant {
        require(_amount >= min, "Amount too small");
        require(boughts[_msgSender()] + _amount <= max, "Exceed max");

        // Calculate
        uint256 heraAmount = (_amount * 1000) / price;

        require(HERA.balanceOf(address(this)) >= heraAmount, "HERA insufficient");
        ...
        locks[_msgSender()] += heraAmount;
        totalLocked += heraAmount;
        totalFunded += _amount;

        emit Buy(_msgSender(), _amount, heraAmount, block.timestamp);
    }
    ...
    */
    ...
    function setWhitelisters(
        ...
        bool[] calldata _isWhitelisters
    ) public onlyOwner {
        require(_users.length == _isWhitelisters.length, "Input invalid");
        for (uint256 i = 0; i < _users.length; i++) {
            whitelist[_users[i]] = _isWhitelisters[i];
        }
    }
