...
contract LootBox is ERC1155, AccessControl {
    ...
    mapping(address => mapping(uint256 => uint256)) _rewardToBoxId;
    ...
    function mint(
        ...
        for (uint256 i = 0; i < rewardIds.length; i++) {
            ...
            _rewards[newBoxId].push(rewardIds[i]);
            _rewardToBoxId[rewardAddress][rewardIds[i]] = newBoxId;
        }

        _rewards[newBoxId].shuffle();
    ...
    function mint(
        ...
        for (uint256 rewardId = rewardIdStart; rewardId < rewardIdEnd; rewardId++) {
            ...
            _rewards[newBoxId].push(rewardId);
            _rewardToBoxId[rewardAddress][rewardId] = newBoxId;
        }
    ...
    function addRewards(uint256 boxId, uint256 rewardIdStart, uint256 rewardIdEnd) external onlyMinter {
        require(boxId < _tokenIds.current(), "Error: No box id");
    ...
    function withdrawERC20(address _token) external onlyAdmin {
        require(IERC20(_token).transfer(_msgSender(), IERC20(_token).balanceOf(address(this))), "Transfer failed");
    }

    function withdrawERC721(address _token, uint256[] memory ids) external onlyAdmin {
        for (uint256 i = 0; i < ids.length; i++) {
            IERC721(_token).transferFrom(address(this), _msgSender(), ids[i]);

            uint256 boxId =_rewardToBoxId[_token][ids[i]];

            _rewards[boxId].remove(ids[i]);
            delete _rewardToBoxId[_token][ids[i]];
        }
    }
    ...
    function withdrawERC1155(
        ...
        IERC1155(_token).safeBatchTransferFrom(address(this), _msgSender(), ids, amounts, data);
    }
}
