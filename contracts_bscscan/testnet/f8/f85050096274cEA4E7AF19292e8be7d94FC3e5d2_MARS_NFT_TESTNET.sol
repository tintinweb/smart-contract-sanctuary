...
contract MARS_NFT_TESTNET is ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    CountersUpgradeable.Counter private _tokenIds;

    mapping (uint256 => TokenMeta) private _tokenMeta;
    address public token_address = 0xf01Ea78b608DA9367d8Ab2C27982B4AD138f3222;
    address public dev_address = 0xe4E44b2C3A9298391524b244C42825e3E2cd2454;
    IERC20 public token;
    ...
    function initialize() public initializer{
        ...
        ERC721Upgradeable.__ERC721_init("MarsCollectible", "MARS_NFT");
        setBaseURI("https://nftmars.finance/nft/");
    ...
    function getAllTokenOnWallet() public view virtual returns( TokenMeta[] memory ) {
        TokenMeta[] memory tokensOnWallet = new TokenMeta[](_tokenIds.current());
        uint256 counter = 0;

        for(uint i = 1; i < _tokenIds.current() + 1; i++) {
            if(ownerOf(_tokenMeta[i].id) == _msgSender()) {
                tokensOnWallet[counter] = _tokenMeta[i];
                counter++;
            }
        }
        return tokensOnWallet;
    }
     ...
     * @param _tokenId uint256 token ID (token number)
     */
    function purchaseNFT(uint256 _tokenId)  public {
        require(msg.sender != address(0) && msg.sender != ownerOf(_tokenId),"Adress error");
        //check current token balanceOf
        require(token.balanceOf(msg.sender) >= _tokenMeta[_tokenId].price,"Not enough MARS token");
        
        uint256 price = _tokenMeta[_tokenId].price;
        address tokenSeller = ownerOf(_tokenId);
        token.transferFrom(msg.sender, address(this), price + price*fee/100);
        token.transferFrom(address(this), tokenSeller, price);
    ...
    function getTokenPrice(uint256 _tokenId)  public returns (uint256){

        uint256 price = _tokenMeta[_tokenId].price;
        uint256 token_price = price + price*fee/100;
        return token_price;
    }
        ...
        returns (uint256)
    {
        ...
        TokenMeta memory meta = TokenMeta(newItemId, _price, _name, _tokenURI, _backupURI, _sale);
        _setTokenMeta(newItemId, meta);

        return newItemId;
    }
    
    function setTokenAdresss(address _tokenAddres) public onlyOwner{
        token = IERC20(_tokenAddres);
        
    }
    
    function setDevAdresss(address _devAddres) public onlyOwner{
        dev_address = _devAddres;
    }
    ...
    function createNFT( 
        ...
        string memory _name, 
        uint256 _price, 
        bool _sale) external payable {

        require(_price > 0, "Please set NFT price");
        _tokenIds.increment();
    ...
    function setNFTOwner( 
        ...
        require(_tokenId > 0,"Token id Error");
        address tokenSeller = ownerOf(_tokenId);
        _transfer(tokenSeller, new_owner, _tokenId);
    }
        ...
        onlyOwner
    {
        ...
        token.transfer(dev_address, token.balanceOf(address(this)));
