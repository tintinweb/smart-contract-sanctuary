...
contract CTC  {
   ...
   mapping(address => uint256) public boughtOf;
    ...
    constructor(address ownerAddress,IBEP20 _ctcToken,address _comissionWallet) public 
    {
        owner = ownerAddress;
    ...
    function buyToken(uint tokenQty,address referrer) public payable
	{
		 ...
		 emit TokenDistribution(address(this), msg.sender, tokenAmount, priceLevel[priceIndex]);					
	 }
	        ...
	        while(i>=1 && tokenQty>0)
	        {
	            ...
	            else
	            {
	                ...
	                { 
	                    ...
	                    if(buyLevel[i]>=tokenQty)
	                    newPriceGap=buyLevel[i]-tokenQty;
	                    else
	                    newPriceGap=0;
	                    tokenQty=0;
	                }
	                ...
	                {
	                   amt=(buyLevel[i]/1e18)*priceLevel[newPriceIndex]; 
	                   tokenQty=tokenQty-buyLevel[i];
	                   newPriceGap=0; 
	                }
