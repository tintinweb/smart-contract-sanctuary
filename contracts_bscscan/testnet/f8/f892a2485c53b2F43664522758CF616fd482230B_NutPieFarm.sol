...
// NutToken with Governance.
contract NutToken is BEP20 {
    ...
    // Min amount to liquify. (default 10 NUTs)
    uint256 public minAmountToLiquify = 10 ether;
    // The swap router, modifiable. Will be changed to Nut's router when our own AMM release
    IPancakeRouter02 public nutRouter;
    // The trading pair
    address public nutPair;
    // In swap and liquify
    ...
    event NutRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "NUT::antiWhale: Transfer amount exceeds the maxTransferAmount");
                require(swapEnabled == true, "NUT::swap: Cannot transfer at the moment");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the NutToken contract.
     */
    constructor() public BEP20("NutPie Token", "NUT") {
    ...
    /// @dev overrides transfer function to meet tokenomics of NUT
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true
            && _inSwapAndLiquify == false
            && address(nutRouter) != address(0)
            && nutPair != address(0)
            && sender != nutPair
            && sender != owner()
        ) {
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "NUT::transfer: Burn value invalid");

            // default 100% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "NUT::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the nut pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = nutRouter.WETH();

        _approve(address(this), address(nutRouter), tokenAmount);
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "NUT::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "NUT::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "NUT::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateNutRouter(address _router) public onlyOperator {
        nutRouter = IPancakeRouter02(_router);
        nutPair = IPancakeV2Factory(nutRouter.factory()).getPair(address(this), nutRouter.WETH());
        require(nutPair != address(0), "NUT::updateNutRouter: Invalid pair address.");
        emit NutRouterUpdated(msg.sender, address(nutRouter), nutPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "NUT::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "NUT::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "NUT::delegateBySig: invalid nonce");
        require(now <= expiry, "NUT::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "NUT::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying NUTs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "NUT::_writeCheckpoint: block number exceeds 32 bits");
...
// NutPieFarm is the master of NutPie Token (NUT). He can make NUT and he is a fair guy.
//
...
// contract once $NUT is sufficiently distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract NutPieFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of NUTs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accNutPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accNutPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. NUTs to distribute per block.
        uint256 lastRewardBlock; // Last block number that NUTs distribution occurs.
        uint256 accNutPerShare; // Accumulated NUTs per share, times 1e12. See below.
    ...
    // NUT tokens created per block.
    uint256 public nutPerBlock;
    // Deposit Fee address
    ...
    // The block number when NUT mining starts.
    ...
    // Initial emission rate: 1 NUT per block.
    ...
    constructor(
        ...
        nutPerBlock = INITIAL_EMISSION_RATE;
    ...
    // Update the given pool's NUT allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    // View function to see pending NUTs on frontend.
    function pendingNut(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accNutPerShare = pool.accNutPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 nutReward = multiplier.mul(nutPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accNutPerShare = accNutPerShare.add(
                nutReward.mul(1e12).div(lpSupply)
            );
        }
        uint256 pending = user.amount.mul(accNutPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 nutReward =
            multiplier.mul(nutPerBlock).mul(pool.allocPoint).div(
                totalAllocPoint
            );
        nut.mint(devAddr, nutReward.div(10));
        nut.mint(address(this), nutReward);
        pool.accNutPerShare = pool.accNutPerShare.add(
            nutReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to NutPieFarm for NUT allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);		
		payOrLockupPendingNut(_pid);
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.lpToken) == address(nut)) {
                uint256 transferTax = _amount.mul(nut.transferTaxRate()).div(10000);
        ...
        user.rewardDebt = user.amount.mul(pool.accNutPerShare).div(1e12);
        emit Deposit(msg.sender, _pid, _amount);
    }	

    // Deposit LP tokens to NutPieFarm for NUT allocation with referral.
    ...
    // Withdraw LP tokens from NutPieFarm.
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
	    ...
	    // Pay or lockup pending NUTs.
    function payOrLockupPendingNut(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];

        uint256 pending = user.amount.mul(pool.accNutPerShare).div(1e12).sub(user.rewardDebt);
        ...
        if (block.number >= startBlockHarvest && block.number <= lastBlockHarvest) {
            if (pending > 0 || user.rewardLockedUp > 0) {        
                ...
                // send rewards
                safeNutTransfer(msg.sender, totalRewards);
    ...
    // Safe nut transfer function, just in case if rounding error causes pool to not have enough NUTs.
    function safeNutTransfer(address _to, uint256 _amount) internal {
        uint256 nutBal = nut.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > nutBal) {
            transferSuccess = nut.transfer(_to, nutBal);
        } else {
            transferSuccess = nut.transfer(_to, _amount);
        }
        require(transferSuccess, "safeNutTransfer: transfer failed.");
    }
    ...
    // updateEmissionRate
    function updateEmissionRate(uint256 _nutPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, nutPerBlock, _nutPerBlock);
        nutPerBlock = _nutPerBlock;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            nut.mint(referrer, refBonusEarned);
