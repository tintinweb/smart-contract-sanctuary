...
contract Staking {
    ...
    function stakingDetails(address addr) public view returns(bool, uint, uint, uint[] memory){
        ...
        return (staked, lastStakeTime, totalStake_, stakedAmount);
    }
        ...
        if(user[sender].tier == 1){
            if(sale.showSaleStatus() == true){
                if(sale.userBuyStatus(sender) == true){
                    ...
                    amount += user[sender].totalStaked * (endTime - since) * 25 / 365 days / 10000;
            ...
            if(sale.showSaleStatus() == true){
                if(sale.userBuyStatus(sender) == true){
                    ...
                    since = user[sender].lastStakeTime ;
                    amount += user[sender].totalStaked * (endTime - since) * 100 / 365 days / 1000;
                    amount += user[sender].totalStaked;
                }
                else{
                    uint saleEndTime = sale.viewSaleEndTime();
