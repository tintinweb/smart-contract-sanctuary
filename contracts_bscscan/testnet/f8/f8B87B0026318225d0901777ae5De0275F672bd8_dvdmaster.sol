...
import "./interfaces/IDVI.sol";

import "./libs/TransferHelper.sol";
import "./base/BaseDividends.sol";
import "./base/SafeBEP20.sol";

 contract dvdmaster is BaseDividends,Context,Ownable {
    ...
    function approve(uint256 amount) external   {
      dvifi.safeApprove(address(this), amount);

    }

    function stake(uint256 amount) external   {
        //UserInfo storage user = userInfo[msg.sender];
        dvifi.transferFrom(_msgSender(),address(this), amount);
        //IDVI(dvifi).deposit{value: amout}();

        //user.amount.add(shares_);
        //_totalShares.add(shares_);
        emit SharesAdded(_msgSender(), amount);
    }

    function remove(uint256 amount) external  {
        //UserInfo storage user = userInfo[msg.sender];
        //require(user.amount >= shares_, "withdraw: Amount to withdraw too high");
        //IDVI(dvifi).withdraw(amout);

        dvifi.safeTransfer(_msgSender(), amount);
        //user.amount.sub(shares_);
        //_totalShares.sub(shares_);
        emit SharesRemoved(_msgSender(), amount);
    }
    ...
    function stakingbalance() public  view returns (uint256) {
        return dvifi.balanceOf(address(this));
    }
     function stakable(address acc) public  view returns (uint256) {
        return dvifi.balanceOf(acc);
    }
  ...
  function distribute() external onlyOwner {
    uint256 amountDVD = rewardbalance();
