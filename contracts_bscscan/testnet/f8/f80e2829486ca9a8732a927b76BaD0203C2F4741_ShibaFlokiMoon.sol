...
contract ShibaFlokiMoon is ERC20Detailed, Ownable {
  ...
  // TOTAL_SBM is a multiple of INITIAL_FRAGMENTS_SUPPLY so that _SBMPerFragment is an integer.
  // Use the highest value that fits in a uint256 for max granularity.
  uint256 private constant TOTAL_SBM = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);

  // MAX_SUPPLY = maximum integer < (sqrt(4*TOTAL_SBM + 1) - 1) / 2
  uint256 private constant MAX_SUPPLY = ~uint128(0);  // (2^128) - 1

  uint256 private _totalSupply;
  uint256 private _SBMPerFragment;
  mapping(address => uint256) private _SBMBalances;
  ...
  ERC20Detailed("ShibaFlokiMoon", "SBM", uint8(DECIMALS))
  payable
  public
  {
    ...
    _SBMBalances[msg.sender] = TOTAL_SBM;
    _SBMPerFragment = TOTAL_SBM.div(_totalSupply);
...
*/
  ...
  returns (uint256)
  {
    return _SBMBalances[who].div(_SBMPerFragment);
  }
  ...
  function _transferStandard(address sender, address recipient, uint256 amount) private {
    ...
    uint256 FOLKIDeduct = amount.mul(_SBMPerFragment);
    uint256 FOLKIValue = tTransferAmount.mul(_SBMPerFragment);
    _SBMBalances[sender] = _SBMBalances[sender].sub(FOLKIDeduct);
    _SBMBalances[recipient] = _SBMBalances[recipient].add(FOLKIValue);
    _takeFee(sender, tFee);
    emit Transfer(sender, recipient, amount);
  }

  function _transferExcluded(address sender, address recipient, uint256 amount) private {
    uint256 FOLKIValue = amount.mul(_SBMPerFragment);
    _SBMBalances[sender] = _SBMBalances[sender].sub(FOLKIValue);
    _SBMBalances[recipient] = _SBMBalances[recipient].add(FOLKIValue);
    emit Transfer(sender, recipient, amount);
  }
  ...
  function _takeFee(address sender, uint256 tFee) private {
    uint256 rFee = tFee.mul(_SBMPerFragment);
    _SBMBalances[address(this)] = _SBMBalances[address(this)].add(rFee);
