...
contract BADCustomize is Ownable{
    struct MetaInfo {
        address banker;
        bool isOpen;
        IBAD BAD;
    ...
    // cardId => amount
    mapping(uint => uint) public mintAmount;
    ...
    event Mint(address indexed player, uint indexed cardId, uint indexed tokenId, string coreNo);
    ...
    function mint(uint cardId_, uint tokenId_,  string calldata coreNo_, bytes32 r, bytes32 s, uint8 v) public returns (bool) {

        // bytes32 hash1 =  keccak256(abi.encodePacked(cardId_, tokenId_));
        // bytes32 hash2 = keccak256(abi.encodePacked(coreNo_));
        // bytes32 hash = keccak256(abi.encodePacked(hash1, hash2));
        bytes32 hash = keccak256(abi.encodePacked(coreNo_));
        address a = ecrecover(hash, v, r, s);
        // require(a == meta.banker, "invalid signature");
        // require(!usedCoreNo[coreNo_], "used coreNo");
        // require(whiteList[_msgSender()], "not premied");

        // usedCoreNo[coreNo_] = true;

        // meta.BAD.mintWithId(player_, cardId_, tokenId_);
        // mintAmount[cardId_] += 1;
