...
contract TOOSWAPNFTMarketV2Test is ERC721Holder, Ownable, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    // WBNB Addresses are updated
    event NewWBNBAddresses(address indexed wbnb);
     ...
     * @notice Create ask order
     * @param _collection: contract address of the NFT
     * @param _startTokenId: start tokenId of the NFT
     * @param _endTokenId: end tokenId of the NFT
     * @param _askPrice: price for listing (in wei)
     */
    function createMultipleAskOrder(
        address _collection,
        ...
        // Verify end tokenId is greater than or equal to start tokenId
        require(_endTokenId >= _startTokenId, "Order: end tokenId must greater than start tokenId");

        for(uint j = _startTokenId; j <= _endTokenId; j++){
            
            if (_canTokenBeListed(_collection, j) == true){
                // Transfer NFT to this contract
                IERC721(_collection).safeTransferFrom(address(msg.sender), address(this), j);

                // Adjust the information
                _tokenIdsOfSellerForCollection[msg.sender][_collection].add(j);
                _askDetails[_collection][j] = Ask({seller: msg.sender, price: _askPrice});

                // Add tokenId to the askTokenIds set
                _askTokenIds[_collection].add(j);

                // Emit event
                emit AskNew(_collection, msg.sender, j, _askPrice);
            }
        }
    }
     ...
     * @dev Only callable by owner
     * @param _WBNBAddress: address of the WBNB token
     */
    function setWBNBAddresses(address _WBNBAddress) external onlyOwner {
        require(_WBNBAddress != address(0), "Operations: WBNB address cannot be zero");

        WBNB = _WBNBAddress;
    
        emit NewWBNBAddresses(_WBNBAddress);
    }
