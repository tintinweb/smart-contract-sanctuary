...
contract HIREX is Context, IBEP20, Ownable {
    ...
    string private _symbol  = "HRX";
    uint8 private _decimals = 9;  
         
    uint256 public taxFee           = 2; // Fee that gets re-distributed to holders.
	uint256 public liquidityFee     = 2;   
    uint256 private _previousTaxFee = taxFee;

    uint256 private buymarketingFee    = 5;
    uint256 private buyresearchFee     = 5;
    
    uint256 private sellmarketingFee   = 10;
    uint256 private sellresearchFee    = 10;

    address payable private _marketingWallet;
    address payable private _researchWallet;
   
	// This is the total fees for what should be sold into BNB.
	// (Marketing + charity + research + Liquidity).
    uint256 private totalLiqFee = liquidityFee;
    uint256 private totalBuyFee = buymarketingFee.add(buyresearchFee);
    uint256 private totalSellFee = sellmarketingFee.add(sellresearchFee);
    uint256 private _previousTotalLiqFee = totalLiqFee;
    uint256 private _previousTotalBuyFee = totalBuyFee;
    uint256 private _previousTotalSellFee = totalSellFee;
    ...
    function setBuyResearchFeePercent(uint256 BuyResearchFee) external onlyOwner() {
    ...
    function setSellResearchFeePercent(uint256 SellResearchFee) external onlyOwner() {
    ...
    function setResearchWallet(address payable researchWallet) external onlyOwner() {
         _researchWallet = researchWallet;
    }
    ...
    function PrivateAndPreSale() external onlyOwner {
    ...
    function removeAllFee() private {
        if(taxFee == 0 && totalLiqFee == 0 && totalBuyFee == 0 && totalSellFee == 0) return;
        
        _previousTaxFee = taxFee;
        _previousTotalLiqFee = totalLiqFee;
        _previousTotalBuyFee = totalBuyFee;
        _previousTotalSellFee = totalSellFee;
    ...
    function restoreAllFee() private {
        ...
        totalBuyFee = _previousTotalBuyFee;
        totalSellFee = _previousTotalSellFee;
    }
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(from == pancakeSwapV2Pair){
                removeAllFee();
                taxFee = buymarketingFee;
                totalLiqFee = buyresearchFee;
            }
            // Sell
            if(to == pancakeSwapV2Pair){
                removeAllFee();
                taxFee = sellmarketingFee;
                totalLiqFee = sellresearchFee;
            }
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 ethForMarketing = balanceDiff.mul(buymarketingFee).add(sellmarketingFee).div(totalLiqFee);
        uint256 ethForResearch = balanceDiff.mul(buyresearchFee).add(sellresearchFee).div(totalLiqFee);
