    ...
    UUPSUpgradeable
{
    ...
    function initialize() public initializer {
        __ERC721_init("Placeveres NFT Token", "PVSN");
    ...
    function _mint(
        ...
        require(!blackListAccounts[_to], "Address is in blackList.");
        require(placeIdPerToken[_placeId] == 0, "PlaceId is exist");

        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();

        tokenDetails[tokenId] = PlaceDetails.encode(
            PlaceDetails.Details(tokenId, _rarity, 1)
        );
    ...
    function addMiner(address _miner) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_miner != address(0), "Address zero");
        _grantRole(MINTER_ROLE, _miner);
        emit AddMiner(_miner);
    }

    function removeMiner(address _miner) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_miner != address(0), "Address zero");
        _revokeRole(MINTER_ROLE, _miner);
        emit RemoveMiner(_miner);
    }

    function setMaxRarity(uint256 _rarity) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _maxRarity = _rarity;
        emit SetMaxRarity(_rarity);
    }

    function setMaxLevel(uint256 _level) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _maxLevel = _level;
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        require(details > 0, "Not Found tokenId.");

        uint256 level = PlaceDetails.decodeLevel(details).add(1);
        require(level <= _maxLevel, "Level upgrade is over max.");
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        require(rarity <= _maxLevel, "Level upgrade is over max.");
    ...
    function _beforeTokenTransfer(
        ...
        require(!blackListAccounts[_from], "Address is in blackList.");
        require(!blackListTokens[_tokenId], "Token NFT is in blacklist.");
