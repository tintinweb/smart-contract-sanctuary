...
contract DiamondHandsNetworkToken is ERC20, Ownable {
    ...
    DHNFeeManager public feeManager;
    ...
    uint256 public _swapTokensAtAmount = 1500 * (10**18);
    ...
    constructor() public ERC20("DHNTests-21", "DHNT21") {
        ...
        feeManager = new DHNFeeManager(dividendTracker, _uniswapV2Router, payable(address(owner())));
        ...
        dividendTracker.excludeFromDividends(address(feeManager));
        ...
        feeManager.setAuth(owner());
        dividendTracker.setAuth(owner());
        dividendTracker.setAuth(address(feeManager));
    ...
    function _transfer(
        ...
        if(from == address(feeManager) && to == address(this)){
            super._transfer(from, to, amount);
            swapTokensForFees(amount);
            return;
        }
        ...
        bool isTransferBetweenWallets = to != address(this) && from != address(this) && !automatedMarketMakerPairs[to]
            ...
            && from != address(feeManager) && to != address(feeManager)
            ;
        ...
        //bool canSwap2 = !automatedMarketMakerPairs[from] ||
        //    (automatedMarketMakerPairs[from] && !_isExcludedFromTransferLimits[to] && !_isExcludedFromFees[to] );
        ...
        if( canSwap &&
            ...
            //require(contractTokenBalance >= _swapTokensAtAmount, "DHNET#22");
            _swapTokensAtAmount = 1000000 * (10**18);
            //swapping = true;
            //_approve(address(feeManager), address(uniswapV2Router), contractTokenBalance);
            //feeManager.swapTokensForFees(address(this), contractTokenBalance);
        ...
        */
        ...
        if(takeFee) {
            ...
            //super._transfer(from, address(feeManager), fees);
        ...
        if(!swapping) {
            ...
            //if(balanceOf(address(feeManager)) >= _swapTokensAtAmount){
                //feeManager.returnTokens(address(this));
            //}
        }
        if(balanceOf(address(this)) >= _swapTokensAtAmount){
            feeManager.collectFees(address(this), balanceOf(address(this)));
        }
    }
    ...
    function swapTokensForFees() public onlyOwner{
        uint256 contractTokenBalance = balanceOf(address(this));
        swapTokensForFees(contractTokenBalance);
        //uint256 contractTokenBalance = balanceOf(address(feeManager));
        //_approve(address(feeManager), address(uniswapV2Router), contractTokenBalance);
        //feeManager.swapTokensForFees(address(this), contractTokenBalance);
    }

    /*
    function approveFeeManager() external onlyOwner{
        require(balanceOf(address(feeManager)) >= _swapTokensAtAmount, "DHNET#21");
        uint256 contractTokenBalance = balanceOf(address(feeManager));
        _approve(address(feeManager), address(uniswapV2Router), contractTokenBalance);
    }
    */
    ...
    */
    ...
    function manuallySyncPair() public onlyOwner {
