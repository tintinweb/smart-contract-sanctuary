...
contract HungerToken is Context, IERC20, Ownable {
    ...
    function includeInReward(address account) external onlyOwner {
        ...
        if (tokenBalance > 0) {
            _reflectionBalance[account] = tokenBalance * _getReflectionRate();
    ...
    function excludeFromReward(address account) external onlyOwner {
        ...
        if (reflectionBalance > 0) {
            _tokenBalance[account] = reflectionBalance / _getReflectionRate();
     ...
     */
    function spendAvailableFunds(uint256 amount) external onlyOwner {
        if (!_lockSwap) {
            uint256 tokensMinusInitialSupport;
            unchecked {
                tokensMinusInitialSupport =
    ...
    function useContractBNB(
        ...
        if (!_lockSwap) {
            ...
            if (buybackRate > 0) {
                _buybackAndBurn(buybackRate, amount);
            }
    ...
    function _buybackAndBurn(uint128 buybackRate, uint256 amount)
        private
        LockSwap
    {
    ...
    function _addLiquidity(uint128 tokensAvailable, uint256 bnbAmount)
        private
        LockSwap
    {
