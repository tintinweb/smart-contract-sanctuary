...
contract IssueAndUpgradeHero is AccessControl {
    ...
    string stringNull = "";
    uint256 public feeBnb = 2000000000000000; // 0.02 BNB
    ...
    function requestIssueHero(uint256 _tokenId, uint256 _numberHero, string memory _tier) public payable{
    ...
    function issueHero(address _owner, uint256 _tokenId, uint256 _numberHero, string memory _tier) public {
        require(hasRole(CREATOR_ADMIN_SERVER, address(msg.sender)), "Caller is not a admin");
        require(queryTierName(_tier) == 1, "Tier not found");
        require(queryNumberTier(_tier) == 0 || queryNumberTier(_tier) == 1 || queryNumberTier(_tier) == 3, "tier is not allowed to issue");
        require(keccak256(bytes(heroes[_numberHero].name)) != keccak256(bytes(stringNull)), "Heroes not found");
        heroesNFT.safeMint(_owner, _tokenId);
    ...
    function getTierTes(uint256 _tokenId) public view returns(string memory) {
       return heroesNFT.getHeroesNumber(_tokenId).tier;
    }
    ...
    function upgradeLimitBreak( uint256 tokenId, uint256 level) public {
        ...
        heroesToken.transfer(msg.sender, amountLimit);
        emit limitBreak(
            msg.sender,
            tokenId,
            level
