...
contract PinkLock is Ownable, ReentrancyGuard {
  ...
  modifier feePaid() {
    if (!poolManager.isPoolGenerated(msg.sender)) {
  ...
  function addRouters(address[] memory routers) external onlyOwner {
    for (uint256 i = 0; i < routers.length; i++) {
      address factory = IUniswapV2Router02(routers[i]).factory();
      require(factory != address(0), "Invalid router");
      factoryToRouter[factory] = routers[i];
      _routers.add(routers[i]);
    }
  }

  function removeRouters(address[] memory routers) external onlyOwner {
    for (uint256 i = 0; i < routers.length; i++) {
      address factory = IUniswapV2Router02(routers[i]).factory();
      require(factory != address(0), "Invalid router");
      delete factoryToRouter[factory];
      _routers.remove(routers[i]);
    }
  }

  function getRouters() public view returns (address[] memory) {
    uint256 length = _routers.length();
    address[] memory routers = new address[](length);
    for (uint256 i = 0; i < length; i++) {
      routers[i] = _routers.at(i);
    }
    return routers;
  }
  ...
  function lock(
    ...
    uint256 amount,
    uint256 unlockDate
  ) external payable feePaid {
    require(
      unlockDate > block.timestamp,
      "Unlock date should be after current time"
    );
    (bool isLp, address router) = isLpToken(token);
    if (isLp) {
      require(router != address(0), "Router not supported");
      _lockLpToken(owner, token, router, amount, unlockDate);
    } else {
      _lockNormalToken(owner, token, amount, unlockDate);
    }
    IERC20(token).safeTransferFrom(msg.sender, address(this), amount);
    emit LockAdded(locks.length - 1, token, owner, amount, unlockDate);
  }
  ...
  function _lockNormalToken(
    ...
    if (tokenInfo.token == address(0)) {
      tokenInfo.token = token;
      tokenInfo.router = address(0);
    }
    tokenInfo.amount = tokenInfo.amount + amount;
  }

  function unlock(uint256 lockId) external {
    require(lockId < locks.length, "Invalid id");
    ...
    // As new router can be added or removed, we cannot rely on `cumulativeLockInfo`
    // mapping to determine that this is a LP or normal token unlock, so it
    // is safer to perform removal on both id sets
  ...
  function editLock(
    ...
    if (newAmount > 0) {
      ...
      CumulativeLockInfo storage tokenInfo = cumulativeLockInfo[userLock.token];
      tokenInfo.amount = tokenInfo.amount - userLock.amount + newAmount;
    ...
    returns (Lock[] memory)
  {
    ...
    Lock[] memory userLocks = new Lock[](length);
    for (uint256 i = 0; i < length; i++) {
      userLocks[i] = locks[_userNormalLockIds[user].at(i)];
    }
    return userLocks;
  }
  ...
  // Returns router if given token is a pair token
  function isLpToken(address token)
    public
    view
    returns (bool isLp, address router)
  {
    address potentialFactoryAddress;
    try IUniswapV2Pair(token).factory() returns (address factory) {
      potentialFactoryAddress = factory;
    } catch {}

    if (
      potentialFactoryAddress != address(0) &&
      _isValidLpToken(token, potentialFactoryAddress)
    ) {
      isLp = true;
      // router should not be address(0) if it was whitelisted
      router = factoryToRouter[potentialFactoryAddress];
    }
    // Not a pair token
  }
