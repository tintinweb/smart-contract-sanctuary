...
// Root file: contracts/AnyPadStakingPool.sol
...
contract AnyPadStakingPool is Configurable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public purchasingStartTime;
    uint256 public purchasingEndTime;
    ...
    event Staked(address indexed acct, uint256 amount);

    event Unstaked(address indexed acct, uint256 amount, uint256 volume);
    ...
    modifier purchasingInProgress {
        require(
            block.timestamp >= purchasingStartTime &&
                block.timestamp < purchasingEndTime,
            "ANYPAD: It is not time to purchase"
        );
        _;
    }

    modifier notPurchased {
        require(purchasedCurrencyOf[msg.sender] == 0, "Already purchased");
        _;
    }
    ...
    function initialize(
        ...
        uint256 purchasingStartTime_,
        uint256 purchasingEndTime_
    ) external initializer {
        require(
            purchasingStartTime_ > stakingStartTime_,
            "ANYPAD: Purchase start time should be after stake start time"
        );
        require(
            purchasingEndTime_ > purchasingStartTime_,
            "ANYPAD: Purchase end time should be after start time"
        );
        require(
            underlyingToken_ != address(0),
            "ANYPAD: Invalid underlying token"
        );
        require(stakingToken_ != address(0), "ANYPAD: Invalid staking token");
        require(
            underlyingToken_ != currencyToken_,
            "ANYPAD: currency and underlying token cannot be the same"
        );
        require(
            stakingToken_ != currencyToken_,
            "ANYPAD: currency and stakingToken_ token cannot be the same"
        );
        require(
            stakingToken_ != underlyingToken_,
            "ANYPAD: staking and underlying token cannot be the same"
        );
        ...
        purchasingStartTime = purchasingStartTime_;
        purchasingEndTime = purchasingEndTime_;
    }
    ...
    function setDate(
        ...
        purchasingEndTime = purchaseEndTime;
    }
        ...
        notPurchased
    {
        ...
        uint256 volume = amount.mul(1e18).div(price);
        require(volume > 0, "ANYPAD: Purchasing volume too small");
        require(
        ...
        purchasedVolumeOf[msg.sender] = purchasedVolumeOf[msg.sender].add(
            volume
        );
        ...
        notPurchased
    {
        ...
        uint256 amount = allocationOf(msg.sender);
        require(amount > 0, "ANYPAD: Cannot purchase 0");
        require(
            amount == msg.value,
            "ANYPAD: Can only purchase with exact allocation"
        );
        uint256 volume = amount.mul(1e18).div(price);
        require(volume > 0, "ANYPAD: Purchasing volume too small");
        purchasedCurrencyOf[msg.sender] = purchasedCurrencyOf[msg.sender].add(
    ...
    function unstakeAndWithdraw() external nonReentrant {
        require(
            block.timestamp >= purchasingEndTime,
            "ANYPAD: It is not time to unstake"
        );
        require(stakingOf[msg.sender] > 0, "ANYPAD: No stake");
        require(claimedOf[msg.sender] == 0, "ANYPAD: Already claimed");

        uint256 amount = stakingOf[msg.sender];
        stakingOf[msg.sender] = 0;
        uint256 purchased = purchasedCurrencyOf[msg.sender];
        ...
        emit Unstaked(msg.sender, amount, volume);
    }

    function allocationOf(address user) public view returns (uint256) {
        if (totalStaking == 0) return 0;
        uint256 staked = stakingOf[user];
        uint256 rate = FullMath.mulDiv(staked, 1e18, totalStaking);
        uint256 allocation = FullMath.mulDiv(totalAllocation, rate, 1e18);
        // uint256 volume = FullMath.mulDiv(allocation, 1e18, price);
        return allocation;
    }

    function volumeOf(address user) public view returns (uint256) {
        uint256 allocation = allocationOf(msg.sender);
        return allocation.mul(1e18).div(price);
    }
    ...
    {
        if (block.timestamp < purchasingEndTime) return (0, 0);
