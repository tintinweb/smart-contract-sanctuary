...
contract LaunchFactory {
    ...
    IERC20 public token = IERC20(0xB2902B3b56C59c21f332786Fb87c952962A3F7df); //TODO: MUSD, change to BUSD when in the mainnet
    uint public price = 300;

    PreSale public presale;

    event launchCreated(address add);

    function buyPreSale (uint price_, address saleToken_, address buyToken_,
    uint hardcap_, uint softCap_, uint minTx_, uint maxTx_, uint startTime_,
    uint endTime_, address operator_, uint liquidityPercentage_) external returns(address){
        require(price_ > 0, "Launch Factory: Price need to be higher than zero.");
        require(maxTx_ >= minTx_, "Launch Factory: maximum transaction can't be lower than minimum.");
        require(hardcap_ >= softCap_, "Launch Factory: hardcap can't be lower than softcap.");
        require(endTime_ > startTime_, "Launch Factory: End time needs to be higher than start time");
        require(liquidityPercentage_ > 50, "Launch Factory: Liquidity percentage needs to be higher than 50%");
        require(token.balanceOf(msg.sender) >= price, "Launch Factory: You don't have enough tokens");
        require(liquidityPercentage_ <= 100, "Launch Factory: Can't add more than 100% liquidity");
        token.transferFrom(msg.sender, address (this), price);
        presale = new PreSale(price_, saleToken_, buyToken_, hardcap_, softCap_, minTx_,
    maxTx_, startTime_, endTime_, operator_, liquidityPercentage_);

        emit launchCreated(address(presale));
