...
contract KYC is Auth {
    ...
    bool payToSee;
    uint256 feeToAdd = 500000000000000000; //=> represent 0.5BNB or 0.5ETH decimals
    ...
    function addParticipant(string memory _fName, string memory _lName, string memory _documentNumber) public payable {

        require(!participants[msg.sender].valid, "This user exists. You cannot add it again!");

        // If you are not a contract owner, you need to pay
        if (msg.sender != owner) {
