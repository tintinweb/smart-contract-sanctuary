...
contract Bingo is Ownable {
  ...
  address public monsta = address(0x342d3664f3F2Ea9146c86a0547033B2199aa2909);
  uint256 public cardPrice = 1e19;
  bool public isPaused = false;
  mapping(address => bool) hasBought;
  mapping(address => string) bingoData;

  mapping(address => bool) existAddress;
  address[] addresses;
  ...
  function buy(string memory _data) public canGenerate {
    ...
    if (!existAddress[msg.sender]) {
      existAddress[msg.sender] = true;
      addresses.push(msg.sender);
    }

    hasBought[msg.sender] = true;
    bingoData[msg.sender] = _data;
    ERC20(monsta).transferFrom(_msgSender(), wallet, cardPrice);
