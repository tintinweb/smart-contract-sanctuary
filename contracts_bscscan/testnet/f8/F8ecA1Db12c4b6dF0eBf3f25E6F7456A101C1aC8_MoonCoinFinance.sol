...
function withdrawAll() public {	// TBA remove in production, this is to capture testbnb back, getting testbnb is tough lol
	...
	function getterUser2(address userAddress) public view returns(uint256, uint256,    uint256, uint256,uint256, uint256) {
			if (userAddress==msg.sender) {
				return (
	...
	function getterUser3(address userAddress) public view returns(uint256, uint256,uint256,   uint256, uint256, uint256) {
			if (userAddress==msg.sender) {
			return (
			users[userAddress].tokensIssued,users[userAddress].promoTokens,users[userAddress].tokensBurned, users[userAddress].withdrawMisses, users[userAddress].myBurnPercent, users[userAddress].myBurnPercentMax
			);	
			}
