    ...
    PausableUpgradeable
{
    ...
    CryptiaNFT public _cryptiaNFTContract;
    ...
    modifier onlyOwnerToken (uint256 tokenId) {
        require(address(this) == _cryptiaNFTContract.ownerOf(tokenId), "token-not-sell");
        require(msg.sender == _informationOf[tokenId].seller, "not-owner-token");
        _;
    }

    function getSaleNFTs() public view returns (uint256[] memory) {
        return _cryptiaNFTContract.getOwnedTokenIds(address(this));
    }
    ...
    function setCryptiaNFTAddr(address cryptiaNFTAddr) public onlyOwner {
        require(
            cryptiaNFTAddr != address(0) && cryptiaNFTAddr != address(this)
        );
        _cryptiaNFTContract = CryptiaNFT(cryptiaNFTAddr);
    }
    ...
    function sellNFT(
        ...
        require(msg.sender == _cryptiaNFTContract.ownerOf(tokenId), "not-owner-token");
        require(_cryptiaNFTContract.getApproved(tokenId) == address(this), "not-approval");
        ...
        _cryptiaNFTContract.safeTransferFrom(
            msg.sender,
            address(this),
            tokenId
        );
    ...
    function purchaseNFT(uint256 tokenId) public payable whenNotPaused {
        ...
        if (_informationOf[tokenId].currency == address(0)) {
            ...
            _informationOf[tokenId].seller.transfer(msg.value);
            emit ProductSold(
                tokenId,
                msg.sender,
...
contract CryptiaNFT is ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    event MintNFT(address recipient, uint256 tokenId, uint256 properties, uint256 fullData);
