...
// File: contracts/src/token/PayableUserToken/PayableUserToken.sol
...
contract PayableUserToken is PayableUserTokenModel, TokenOwnable {
    address _stateContractAddress;

    // Token state Events
    event PayableUserTokenMinted(address indexed tokenOwner, uint256 tokenId);
    event PayableUserTokenUsed(address indexed tokenOwner, uint256 tokenId);

    /**
     * @dev Initializes CumStar Payable User Token
     */
    constructor(address stateContractAddress_) {
        require(stateContractAddress_ != address(0), "Ownable: state contract address can't be the Zero Address");

        _stateContractAddress = stateContractAddress_;
    }

    function mint(address artistAddress, uint256 cumStarNFTTokenId) external onlyMinter {
        require(artistAddress != address(0), "Ownable: Artist address is the zero address");

        uint256 tokenId = PayableTokenStateInterface(_stateContractAddress).getNextTokenId();

        PayableUserToken memory payableUserToken = PayableUserToken({
            cumStarNFTTokenId: cumStarNFTTokenId,
            owner: artistAddress,
            tokenId: tokenId,
            used: false
        });

        PayableTokenStateInterface(_stateContractAddress).addTokenToState(payableUserToken);

        emit PayableUserTokenMinted(artistAddress, tokenId);
    }

    function useToken(uint256 tokenId, address tokenOwner) external onlyMinter {
        require(tokenOwner != address(0), "Ownable: Token owner address is the zero address");
        PayableTokenStateInterface(_stateContractAddress).useToken(tokenId, tokenOwner);

        emit PayableUserTokenUsed(tokenOwner, tokenId);
    }

    function getPayableTokens(address userAddress) external onlyReader returns (PayableUserToken[] memory) {
        require(userAddress != address(0), "Ownable: User address is the zero address");

        return PayableTokenStateInterface(_stateContractAddress).getUserPayableTokens(userAddress);
    }
}