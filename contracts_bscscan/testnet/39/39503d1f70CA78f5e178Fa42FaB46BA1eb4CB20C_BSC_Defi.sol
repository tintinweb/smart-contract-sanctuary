...
/**
 ...
 Lottery:
    ...
    ~ Pot money will not be deducted to the user, it will be given as a bonus.
    ~ lottery will run every 4 hours / max 60 participants.
    ~ 50 tickets max per user.
    ~ number of tickets per user will depend on the amount of deposit or compound. 0.008 bnb = 1 per ticket.
    ~ rewards for the winner will be available for withdrawal.
...
contract BSC_Defi {
    ...
    uint256 public LOTTERY_TICKET_PRICE = 8 * 1e15; /** 0.008 ether **/
    ...
    function hatchEggs(address ref, bool isCompound) public {
        ...
        if (user.referrer != address(0)) {
            ...
            if (upline != address(0)) {
                uint256 amount = eggsUsed.mul(REFERRAL).div(PERCENTS_DIVIDER);
    ...
    function payFees(uint256 eggValue) internal {
        (uint256 projectFee, uint256 marketingFee) = getFees(eggValue);
    ...
    function _payLotteryWinner(address userAddress) private {
        ...
        uint256 insure = eggValue.mul(INSURANCE).div(PERCENTS_DIVIDER);
        insurance.transfer(insure);
    ...
    function getLotteryInfo() public view returns (uint256 lotteryStartTime,  uint256 lotteryStep, /**uint256 minerPerPoint,**/ uint256 lotteryCurrentPot,
	  uint256 lotteryParticipants, uint256 maxLotteryParticipants, uint256 totalLotteryTickets, uint256 lotteryTicketPrice, uint256 maxLotteryTicket, uint256 lotteryPercent, uint256 round){
		lotteryStartTime = LOTTERY_START_TIME;
		lotteryStep = LOTTERY_STEP;
		lotteryTicketPrice = LOTTERY_TICKET_PRICE;
		//minerPerPoint = MINER_PER_POINT;
    ...
    /** setup **/
    function hatchEggs(address _addr) external {
    ...
    function CHANGE_INSURANCE(address value) external {
        require(msg.sender == owner, "Admin use only.");
    ...
    function PRC_INSURANCE(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value < 50); /** 5% max **/
        INSURANCE = value;
    }
