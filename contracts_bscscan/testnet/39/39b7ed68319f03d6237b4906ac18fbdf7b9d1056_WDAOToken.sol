...
contract WDAOToken is IERC20, Ownable {
    ...
    // 分红
    mapping(uint256 => address) public _tOwnedList; // 可领分红的地址
    uint256 public constant A_BOUNDS_CONDITION = 100 ether; // 分红最低限制， >= 100 代币
    uint256 public _OwnerIndex = 0;
    ...
    function _tokenTransfer(
        ...
        if (balance >= A_BOUNDS_CONDITION) {
            _tOwnedList[_OwnerIndex] = recipient;
            _OwnerIndex = _OwnerIndex.add(1);
        }
    ...
    function _takeAbonus(address sender, uint256 tAmount, uint256 currentRate) private {
        ...
        uint256 amount = tAmount.mul(currentRate).div(_OwnerIndex);
        for(uint256 i = 0; i < _OwnerIndex; i++) {
            address recipient = _tOwnedList[i];
            uint256 balance = balanceOf(recipient);
            if (balance >= A_BOUNDS_CONDITION) {
