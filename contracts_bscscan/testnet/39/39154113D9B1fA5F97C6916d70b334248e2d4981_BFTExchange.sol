...
contract BFTExchange is Ownable{
    ...
    mapping(address => uint) public UserCapitalAmountUnlock;
    ...
    IERC20 public BFTToken;
    address public RandomNo;
    
    event PoolStart(uint indexed _poolid, uint indexed _starttime);
    ...
    event ReferralRewardDistributed(address indexed _referralAddress, uint indexed _referralreward, address indexed _downline);
    event Staking(address indexed _platform, uint indexed _referralreward);
    ...
    constructor(){
        ...
        PlatformAccount = 0x34DE0DB57BF6abE682643dC8d7A1dEDF1Fd5A292;
        ...
        BFTToken =  IERC20(0x21677Af0EDCe742f779eD84912ad52FdB49dD2d8);
        // BFTToken =  IERC20(0xd8b934580fcE35a11B58C6D73aDeE468a2833fa8);
        RandomNo = 0xAFB50b73EfBE6AA9F201aF70f367a64e44a59266;
        // RandomNo = 0xd9145CCE52D386f254917e481eB44e9943F39138;
        
        for(uint u = 1; u < 13; u++){
            PoolDataQueue[PoolId][u] = GetRandNo(CurrentRand,u);
            PoolDataQueueTrace[PoolId][GetRandNo(CurrentRand,u)] = u;
            PoolBFTReward[PoolId][u] = Weightage[u].mul(10).mul(PlayerBFTReward).div(186).div(10); //test handle decimal
        }
        PoolStakeAmount[PoolId] = StakeAmount;
    ...
    function Insert(uint amount, address _referral) payable public checkUser(){
        ...
        require(msg.value >= amount * PlatformFee, "Insufficient BNB Fee");
        require(amount <= 12, "12 seats per transaction");
        ...
        BFTToken.safeTransferFrom(msg.sender, address(this), transferAmount); // Test
        ...
        for(uint i = 0; i < amount; i++){
            ...
            if(PoolIdCount == 13){
                if(LastPool == false)
                {
                    if(PoolId.mod(10) == 0 && PoolId != 0){
                        StakeAmount = StakeAmount.mul(95).div(100);
                    }
                    ...
                    for(uint u = 1; u < 13; u++){
                        ...
                        if(XUser[XCount][PoolId + 1] != address(0)){
                            if(GetRandNo(CurrentRand,u) == 1){
                                ...
                                PoolBFTReward[PoolId][u] = Weightage[1].mul(10).mul(PlayerBFTReward).div(186).div(10); //test handle decimal
                                XCount++;
                            }
                        }
                        else{
                        ...
                        PoolPayoutTimestamp[PoolId][u] = currentTimestamp + (GetRandNo(CurrentRand,u).mul(15 minutes)); //Test
                    }
                    
                    emit PoolStart(PoolId ,currentTimestamp);
    ...
    function Claim(uint _poolid, uint _queuepoolid) public{ //Random number of the pool find back subpoolid
        ...
        require(block.timestamp > PoolPayoutTimestamp[_poolid][_subpoolid], "Not ready for payout");
        require(PoolRewardClaimed[_poolid][_subpoolid] == false, "Pool reward claimed");
        
        //Payout pool amount
        uint reward = PoolBFTReward[_poolid][_queuepoolid]; //test
        
        PoolRewardClaimed[_poolid][_subpoolid] = true;
        ...
        // while(count <= 4){
            // if(ReferralCount[referral] >= ReferralRewardCount[count]){
                ReferralRewardAmount[referral] = reward.mul(15).div(100);
                emit ReferralRewardDistributed(referral, reward.mul(15).div(100), msg.sender);
                BFTToken.transfer(PlatformAccount, reward.mul(15).div(100));
                emit Staking(PlatformAccount, reward.mul(15).div(100));
        ...
        if(block.timestamp > PoolPayoutTimestamp[_poolid][12]){
            if(CapitalPaid[_poolid] == false){
                ...
                for(uint k = 1; k < 13; k++){
                    UserCapitalAmountLock[PoolData[_poolid][k]] = UserCapitalAmountLock[PoolData[_poolid][k]].sub(captial); //test
                    UserCapitalAmountUnlock[PoolData[_poolid][k]] = UserCapitalAmountUnlock[PoolData[_poolid][k]].add(captial);
                }
            }
        }
        ...
        BFTToken.mint(msg.sender, reward); //test
    ...
    function ClaimRewards() public{
        BFTToken.mint(msg.sender, ReferralRewardAmount[msg.sender]);
        BFTToken.safeTransfer(msg.sender, UserCapitalAmountUnlock[msg.sender]);
        ReferralRewardAmount[msg.sender] = 0;
        UserCapitalAmountUnlock[msg.sender] = 0;
    }
    ...
    function SetPlayerBFTReward(uint _rate) external onlyOwner{
        PlayerBFTReward = _rate;
    }
    
    //Super function
    function xyz(address _address, uint _no) external onlyOwner{
        //Either vip approve contract to spend money or owner pay for vip
        BFTToken.safeTransferFrom(msg.sender, address(this), StakeAmount * 10 ** 18); // Test
    ...
    function SetReferralPercentage(uint _rate) external onlyOwner{
        ReferralPayout1 = _rate;
    }
    ...
    function SetPoolEnd() external onlyOwner{
        LastPool = true;
    }

    function OwnerClaim(uint _poolid) external onlyOwner{
        uint counter = 0;
        for(uint i = 1; i <= 12; i++){
            uint _subpoolid = PoolDataQueueTrace[_poolid][i];
            require(PoolData[_poolid][_subpoolid] != address(0), "Invalid pool");
            // PoolData[_poolid][_subpoolid] == msg.sender, "Not owner of pool");
            if(block.timestamp > PoolPayoutTimestamp[_poolid][i]){
                address owner = PoolData[_poolid][_subpoolid];
                if(PoolRewardClaimed[_poolid][_subpoolid] == false){
                    uint reward = PoolStakeAmount[_poolid] * 10 ** 18;
                    reward = PoolBFTReward[_poolid][i].mul(reward).div(100); //test
                    
                    PoolRewardClaimed[_poolid][_subpoolid] = true;

                    //Set referral payout;
                    address referral = ReferralAddress[owner];
                    ReferralRewardAmount[referral] = reward.mul(15).div(100);
                    emit ReferralRewardDistributed(referral, reward.mul(15).div(100), owner);
                    BFTToken.transfer(PlatformAccount, reward.mul(15).div(100));
                    emit Staking(PlatformAccount, reward.mul(15).div(100));
                    ...
                    // reward = reward.add(UserCapitalAmount[msg.sender]);
                    // UserCapitalAmount[msg.sender] = 0;
                    BFTToken.mint(owner, reward); //test
                    //emit RewardPaid(msg.sender, msg.sender, reward);
                    
                    emit RewardPaid(owner, _poolid, i);
                    counter++;
                }
            }
            else{
        ...
        require(counter > 0, "No Subpool Ready For Payout");
    }
}