...
contract InstantBuy
{
    ...
    function setMarginSign(uint newValue) external returns (bool success)
    {
        require(msg.sender == owner, 'FN'); //Forbidden
    ...
    function setMarginPercent(uint256 newValue) external returns (bool success)
    {
        require(msg.sender == owner, 'FN'); //Forbidden
    ...
    function getInstantBuyPriceQuote(address source, address destination) public view returns (uint256 value)
    {
        ...
        result = getPriceQuoteWithMargin(result);

        return result;
    }

    function getPriceQuoteWithMargin(uint256 amount) internal view returns (uint256 value)
    {        
        uint256 margin = 0;
        ...
        if(marginPercent > 0)
        {
            margin = SafeMath.safeDiv(SafeMath.safeMul(amount, marginPercent), ONE_HUNDRED);            
        }

        if(marginSign == 0) //marginSing positive
        {
            calculatedAmount = SafeMath.safeAdd(amount, margin);
        }
        else if(marginSign == 1)
        {
            if(amount > margin)
            {
