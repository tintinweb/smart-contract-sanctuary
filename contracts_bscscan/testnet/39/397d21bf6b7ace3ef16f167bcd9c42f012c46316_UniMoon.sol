...
UNIMOON.io
t.me/unimoonshitcoinkilla


The New Blockchain Built to Decentralize Social Media
...
contract UniMoon is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x4E23980bf2f4AF53de77FC90FD072cc621A8E6e5); // Marketing Address
    ...
    constructor () {
        ...
        // PCS Testnet - 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
        // PCS Testnet V1 - 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
    ...
    function _transfer(
        ...
        require(!_isLocked[from] && !_isLocked[to], "The addresses are locked");
        if(from != owner())
        {
        require(allowTrade == true, "Trading is prohibited");
        }
    ...
    function lockAddress(address add, uint256 timeLimit) public onlyOwner
    {
        timelocked[add] = timeLimit; //time in epoch seconds
        _isLocked[add] = true;
    }
    
    function checkLockDuration(address add) public view returns (uint256)
    {
        return timelocked[add];
    }
    
    function isLocked(address add) external view returns(bool)
    {
        return _isLocked[add];
    }
    
    function unLock(address add) public onlyOwner
    {
        if(block.timestamp >= checkLockDuration(add))
        _isLocked[add] = false;
    }

       function manageAntibots(address[] calldata addresses, bool status) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            _isBlacklisted[addresses[i]] = status;
        }
    }
    
    function addSingleToAntibotList(address wallet) public onlyOwner
    {
        _isBlacklisted[wallet] = true;
    }
    
    function removeSingleFromAntibotList(address wallet) public onlyOwner
    {
        _isBlacklisted[wallet] = false;
    }
    
    function turnTheContractON() public onlyOwner
    {
        allowTrade = true;
    }
    
    function turnTheContractOFF() public onlyOwner
    {
        allowTrade = false;
    }
