...
contract EpicMarket is AccessControl, EpicWhiteList {
    ...
    function closeOffer(uint256 offerId, uint256 marketId) public onlyExistItem(marketId){
        ...
        uint256 fee = getFee(offers[itemOwner][offerId]._price);
    ...
    function cancelItem(uint256 marketId) public onlyItemOwner(marketId)  {
        ...
        require(IERC20(itemData._token).balanceOf(msg.sender) >=  getFee(itemData._price), "Balance is not enough to cancel");
        IERC20(itemData._token).transferFrom(msg.sender, _wallet, getFee(itemData._price));
