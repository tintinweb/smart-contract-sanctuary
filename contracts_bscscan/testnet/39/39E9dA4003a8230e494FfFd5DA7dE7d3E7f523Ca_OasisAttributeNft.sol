...
// 
contract OasisAttributeNft is ERC721Enumerable, VRFConsumerBase, Ownable {
    using Strings for uint256;
    using ECDSA for bytes32;

    uint256 public constant Oasis_GIFT = 88;
    uint256 public constant Oasis_PRIVATE = 800;
    uint256 public constant Oasis_PUBLIC = 8000;
    uint256 public constant Oasis_MAX = Oasis_GIFT + Oasis_PRIVATE + Oasis_PUBLIC;
    uint256 public constant Oasis_PRICE = 0.08 ether;
    uint256 public constant Oasis_PER_MINT = 5;
    ...
    address private _artistAddress = 0x690F6f1327c7565326D80cB4b3234006d4854630;
    address private _signerAddress = 0x690F6f1327c7565326D80cB4b3234006d4854630;
  ...
  mapping(uint256 => uint256) public indices;
    ...
    ERC721("Oasis Attribute Nft", "Oasis")
    {
        s_keyHash = keyHash;
        s_fee = fee;
    }
    ...
    function buy(bytes32 hash, bytes memory signature, string memory nonce, uint256 tokenQuantity) external payable {
        ...
        require(totalSupply() < Oasis_MAX, "OUT_OF_STOCK");
        require(publicAmountMinted + tokenQuantity <= Oasis_PUBLIC, "EXCEED_PUBLIC");
        require(tokenQuantity <= Oasis_PER_MINT, "EXCEED_Oasis_PER_MINT");
     //   require(Oasis_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
        ...
        function setPricePerMint(uint _pricePerMint) external {
            pricePerMint = _pricePerMint;
        }


  function getRandomTokenId(uint256 random) private returns (uint256) {
    uint256 totalSize = Oasis_MAX - totalSupply();
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        uint256 randomTokenid = getRandomTokenId(randomness);

        s_results[requestId] = randomTokenid;
        address receiver = s_rollers[requestId];
       
       privateAmountMinted++;
            presalerListPurchases[receiver]++;
            _safeMint(receiver, randomTokenid);
    }

    function presaleBuy(uint256 tokenQuantity) external  {
        require(!saleLive && presaleLive, "PRESALE_CLOSED");
        require(presalerList[msg.sender], "NOT_QUALIFIED");
        require(totalSupply() < Oasis_MAX, "OUT_OF_STOCK");
        require(privateAmountMinted + tokenQuantity <= Oasis_PRIVATE, "EXCEED_PRIVATE");
        require(presalerListPurchases[msg.sender] + tokenQuantity <= presalePurchaseLimit, "EXCEED_ALLOC");
     //   require(Oasis_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
        uint256 totalPayable = pricePerMint;
       ...
       // s_results[requestId.sender] = ROLL_IN_PROGRESS;

    }
    
    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= Oasis_MAX, "MAX_MINT");
        require(giftedAmount + receivers.length <= Oasis_GIFT, "GIFTS_EMPTY");
