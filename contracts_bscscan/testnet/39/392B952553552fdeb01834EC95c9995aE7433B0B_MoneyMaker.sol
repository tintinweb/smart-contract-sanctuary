...
contract MoneyMaker {
    ...
    function withdrawMonthlyGain(uint stakeIndex) public returns(bool)
    {
      require(!isContract(msg.sender),  'No contract address allowed to withdraw');
      uint tim2=stakeInfo[msg.sender][stakeIndex].lastWithdrawTime ;
      require(tim2 + oneDay  < block.timestamp, "You can withdraw once in a day");
      address _customerAddress = msg.sender;
      uint256 amt = withdrawble(stakeIndex);
      require( amt > 0, "nothing staked");
      require( amt >= minWithdraw, "Does not reach to withdraw min limit");
      require(tokenInterface(tokenAddress).balanceOf(address(this)) >=  amt , "Insufficient fund");
      ...
      for(uint i =1 ; i < percent_.length;i++)
      {
        ...
        if(refCount[ref] >= eligiblerefcount){
          sponsordailyGain[ref]+= amt * perc/100;
        }
      }
      if(checklimit(_customerAddress,amt)){
        userInfo[_customerAddress].total_payouts += amt ;
        tokenInterface(tokenAddress).transfer(_customerAddress, amt);
        emit Transfer(address(this) , _customerAddress , amt);
        emit Withdraw(_customerAddress, amt);
      }
      else
      {
        userInfo[_customerAddress].total_payouts = 0;
        userInfo[_customerAddress].isActiveStake = false;
      }
      return true;
    }
