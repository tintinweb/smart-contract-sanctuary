...
contract NftPads is Ownable {
     ...
     * - projectToRegisteredAddress: launchPadA => userA => isRegistered? (whether userA registered in launchPadA or not)
     * - launchpadExisted: Currently not being used ==> allow override the existing launchpad
     * todo: keep track of how much user deposit ??? wait for pm
     */

    // mapping(string => mapping(address => uint256)) public userStakeBalance; // TVK token
    // mapping(string => mapping(address => uint256)) public userDepositBalance;

    mapping(string => mapping(address => bool))
        public projectToRegisteredAddress;

    // Hold information either if the launchpad existed
    ...
    event LaunchPadAdded(string indexed launchpad);

    /**
     * @dev Launchpad hold information of a launchpad
     * StartTime - endTime: Time when the pool start end end
     * Min Allocation - Max Allocation: Amount of token user can deposit after being registered
     *
     */
    ...
    function createLaunchPad(
        ...
        uint256 startTime_,
        uint256 endTime_,
        uint256 refundDuration_, // in hours
        ...
        /**
         * @dev currently allow override the contract
         * currently allow override the existing contract
         */
        //require(!launchPadExisted[launchPad], "launchpad is already existed");
        require(
        ...
        Launchpad memory launchpad = Launchpad({
            ...
            endTime: endTime_,
            refundDuration: refundDuration_,
        ...
        emit LaunchPadAdded(launchPad_);
    }

    /**
     * @dev Calculate what entry level it is of a specific launchpad
     * returns: enterable or not, what level it is
     */
        ...
        )
    {
        ...
        if (block.timestamp < launchpad.entryDates[0]) {
            return (false, 0);
        }
        ...
        // last point
        if (
            launchpad.ticketSolds[totalLength - 1] <
            launchpad.numberOfTickets[totalLength - 1]
        ) {
            return (true, totalLength - 1);
        }
    ...
    function register(string memory launchPad_) public {
        ...
        require(stakingToken != address(0), "Invalid staking token"); //note: check if token is ERC20

        require(registerable, "No entries are available!");
        require(
            !projectToRegisteredAddress[launchPad_][msg.sender],
            "You'are already registered"
        );

        Launchpad storage launchpad = projectToLaunchpads[launchPad_];

        //Case?: Many user request at the same time?
        ...
        Register memory registerUser = Register({
            ...
            amountOfStakingToken: launchpad.ticketPrices[level]
        });

        projectToRegisterUser[launchPad_][msg.sender] = registerUser;

        //update launchpad
        projectToRegisteredAddress[launchPad_][msg.sender] = true;
    ...
    function refund(string memory launchpad_) public {
        ...
        require(
            registerUser.amountOfStakingToken > 0,
            "pads: you have no token to withdraw"
        );
        require(
            block.timestamp <
        ...
        IERC20(stakingToken).safeTransfer(
            msg.sender,
            registerUser.amountOfStakingToken
        );
    }
    ...
    function withdrawAll() public onlyOwner {
        uint256 totalBalance = IERC20(stakingToken).balanceOf(address(this));
        IERC20(stakingToken).safeTransfer(owner(), totalBalance);
    }

    /**
     * @dev Get num of ticket solds, total ticket at level of launchpad
     */
