...
contract Ballot2 {
	...
	mapping(bytes32 => uint256) public diceCount;//每种点数的数量,string是英文，uint是数量
	...
	function play(bytes32[] memory itemData, uint128[] memory amountArr) public returns(uint256,uint256,uint256){
		...
		uint256 diceSum = dice1 + dice2 + dice3;//总数
		...
		//遍历传过来的数据，进行相关处理（itemData长度和amountArr的长度应该是一样的）
		...
		emit GuangBo(dice1,dice2,dice3,totalAmount,reward);
		
		
		//遍历处理，也不知会不会发瘟
		for(uint k = 0 ;k < itemMap.length; k ++){
			bytes32 key = itemMap[k];
			if(itemInvolved[key] > 0 ){//是否有参与
				if(diceCount[key] > 0){//这个点数,如果有超过0个
