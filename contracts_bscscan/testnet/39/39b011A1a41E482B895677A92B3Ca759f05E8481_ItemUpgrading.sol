...
contract ItemUpgrading is MarketAuth, EIP712Upgradeable {
  ...
  function upgradeItem(
    ...
    (uint ownPriceFeeWhenSuccess, uint willKeepMaterialIdIfFailed, uint fee) = _validateAndTakeMaterials(_itemClass, _materials, _tokenIdFormula, _usdCurrency, _acceptedRatio);
    UpgradingInfo memory upgradingInfo = UpgradingInfo(_itemClass, _materials, _tokenIdFormula);
  ...
  function _validateAndTakeMaterials(
    ...
    (uint16 formulaGene,,,uint formulaOwnPrice,,) = itemNft.getItem(_tokenIdFormula);
    require(formulaGene == 0, "Item Upgrading: invalid formula item");
    address approved2 = itemNft.getApproved(_tokenIdFormula);
    require(approved2 == address(this), "Item Upgrading: please approve formula token first");
  ...
  function _finishUpgrade(
    ...
    itemNft.burn(_upgradingInfo.tokenIdFormula);
  }
}
