...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    {    
        if (amount > 0) {
            require(tokenTransferProxy.transferFrom(token, from, to, amount), 'cannot transfer token');
        }
    }
        ...
        returns (uint)
    {
        ...
        /* Require price cross. */
        require(buyPrice >= sellPrice, 'price is not right');
        ...
        returns (uint)
    {
        ...
        if (sell.paymentToken != address(0)) {
            require(msg.value == 0, 'sell has payment token must not have value');
        }
        ...
        if (sell.feeRecipient != address(0)) {
            ...
            require(sell.takerRelayerFee <= buy.takerRelayerFee, 'taker fee is not valid');

            if (sell.feeMethod == FeeMethod.SplitFee) {
                /* Assert taker fee is less than or equal to maximum fee specified by buyer. */
                require(sell.takerProtocolFee <= buy.takerProtocolFee, 'taker fee is not valid 2');
            ...
            require(buy.takerRelayerFee <= sell.takerRelayerFee, 'taker fee is not valid 3');

            if (sell.feeMethod == FeeMethod.SplitFee) {
                /* The Exchange does not escrow Ether, so direct Ether can only be used to with sell-side maker / buy-side taker orders. */
                require(sell.paymentToken != address(0), 'payment token is not valid');

                /* Assert taker fee is less than or equal to maximum fee specified by seller. */
                require(buy.takerProtocolFee <= sell.takerProtocolFee, 'taker fee is not valid 4');
        ...
        reentrancyGuard
    {
        ...
        if (buy.maker == msg.sender) {
            require(validateOrderParameters(buy), 'Buy order not valid!');
        } else {
        ...
        /* Must be matchable. */
        require(ordersCanMatch(buy, sell), 'Order not match');
        ...
        require(ArrayUtils.arrayEq(buy.calldata, sell.calldata), 'call data not match');
        ...
        require(proxy.proxy(sell.target, sell.howToCall, sell.calldata), 'execute proxy fail!');
        /* Static calls are intentionally done after the effectful call so they can check resulting state. */

        /* Handle buy-side static call if specified. */
        if (buy.staticTarget != address(0)) {
            require(staticCall(buy.staticTarget, sell.calldata, buy.staticExtradata), 'execute buy staticCall fail');
        }

        /* Handle sell-side static call if specified. */
        if (sell.staticTarget != address(0)) {
            require(staticCall(sell.staticTarget, sell.calldata, sell.staticExtradata), 'execute sell staticCall fail');
        }
