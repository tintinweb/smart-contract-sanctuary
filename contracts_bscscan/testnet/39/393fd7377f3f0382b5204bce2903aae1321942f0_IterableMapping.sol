...
contract Test is ERC20, Ownable {
    ...
    address public ethDividendToken = 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca;
    address public dogeBackDividendToken = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;
    ...
    constructor() ERC20("Test", "Test") {
    	...
    	teamWallet = 0xBbc14dCDcFEcac5fF3C013415c40dE1123b6D761;
        ...
        // exclude from receiving eth dividends
        ethDividendTracker.excludeFromDividends(address(ethDividendTracker));
        ethDividendTracker.excludeFromDividends(address(this));
        ethDividendTracker.excludeFromDividends(address(_uniswapV2Router));
        ethDividendTracker.excludeFromDividends(address(deadAddress));
        
        // exclude from receiving dogeback dividends
        dogeBackDividendTracker.excludeFromDividends(address(dogeBackDividendTracker));
        dogeBackDividendTracker.excludeFromDividends(address(this));
        dogeBackDividendTracker.excludeFromDividends(address(_uniswapV2Router));
        dogeBackDividendTracker.excludeFromDividends(address(deadAddress));
    ...
    function afterPreSale() external onlyOwner {
        ...
        maxBuyTranscationAmount = 600000000 * (10**18);
    ...
    function updateEthDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(ethDividendTracker), "Shiback: The dividend tracker already has that address");

        EthDividendTracker newEthDividendTracker = EthDividendTracker(payable(newAddress));

        require(newEthDividendTracker.owner() == address(this), "Shiback: The new dividend tracker must be owned by the Shiback token contract");
    ...
    function updateDogeBackDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dogeBackDividendTracker), "Shiback: The dividend tracker already has that address");

        DogeBackDividendTracker newDogeBackDividendTracker = DogeBackDividendTracker(payable(newAddress));

        require(newDogeBackDividendTracker.owner() == address(this), "Shiback: The new dividend tracker must be owned by the Shiback token contract");
    ...
    function updateEthDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee > 0 && newFee <=6, "EtherBack: Fee must be greater than 0% but less than 6%");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "Shiback: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "Shiback: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Shiback: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newMarketingWallet) external onlyOwner {
        require(newMarketingWallet != marketingWallet, "Shiback: The liquidity wallet is already this address");
...
contract EthDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("Shiback_Dividend_Tracker", "Shiback_Dividend_Tracker", 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca) {
    	claimWait = 3600;
...
contract DogeBackDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("Shiback_Dividend_Tracker", "Shiback_Dividend_Tracker", 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }
