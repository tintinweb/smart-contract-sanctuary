...
interface IDividendDistributor {
    ...
    function deposit(uint256 shares) external payable;
...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public totalSharesFrame;
    ...
    function deposit(uint256 _shares) external payable override onlyToken {
        ...
        totalSharesFrame = totalShares.sub(_shares);
    ...
    function setShare(address shareholder, uint256 amount) external override onlyToken {
        ...
        if(amount > 0 && shares[shareholder].heldAmount > 0){
            ...
            if (amount > minHoldAmountForRewards && shares[shareholder].heldAmount > minHoldAmountForRewards) {
                if (amount < shares[shareholder].heldAmount) { //sell //transfer out

                    //get all divs belonging to the shareholder up until now
                    shares[shareholder].unpaidDividends = shares[shareholder].unpaidDividends.add(getUnpaidEarnings(shareholder));

                    //set new excluded amounts
                    shares[shareholder].totalExcluded = getCumulativeDividends(amount);

                    //set new holder amounts
                    shares[shareholder].amount = amount;
                    shares[shareholder].heldAmount = amount;
                    
                    if (frameDividends > 0) { //sell
                    //calculate divs from this sell only
                    //shares[shareholder].unpaidDividends = shares[shareholder].unpaidDividends.add(calculateFrame(shareholder, amount));
                    }

                    //set totals for next sell
                    totalShares = totalShares.sub(shares[shareholder].heldAmount).add(amount);
                }    


                if (amount > shares[shareholder].heldAmount) { //bought more //transfer in
                    shares[shareholder].unpaidDividends = shares[shareholder].unpaidDividends.add(getUnpaidEarnings(shareholder));
             ...
             //User bought/received more to qualify for rewards
            if (amount > minHoldAmountForRewards && shares[shareholder].heldAmount < minHoldAmountForRewards) {
            ...
            //User had enough for rewards previously but now dropped below from sell/transfer out
            if (amount < minHoldAmountForRewards && shares[shareholder].heldAmount > minHoldAmountForRewards) {
    ...
    /*function calculateFrame(address shareholder, uint256 amount) public returns (uint256 unpaidAmount) {
        if (frameDividends == 0) {return 0;}
        if (totalShares == 0) {return 0;}
        
        uint256 totalthisframe;

        frameDividendsPerShare = dividendsPerShareAccuracyFactor.mul(frameDividends).div(totalSharesFrame);

        totalthisframe = amount.mul(frameDividendsPerShare).div(dividendsPerShareAccuracyFactor);
        
        frameDividends = 0;

        return totalthisframe;
    }*/
    ...
    function getUnpaidEarnings(address shareholder) public view returns (uint256 unpaidAmount) {
        if(shares[shareholder].amount == 0 || shares[shareholder].heldAmount < minHoldAmountForRewards){ return 0; } 
        if(totalDividends == 0){ return 0; } 
        else {
             
             uint256 tempdivspershare = dividendsPerShareAccuracyFactor.mul(totalDividends).div(totalSharesFrame);
    ...
    function mathInfo() external view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        return (totalShares, totalSharesFrame, netDividends, totalDistributed, totalReinvested, totalWithdrawn, frameDividends, totalDividends);
    }
...
contract KojiEarth is IBEP20, Auth {
    ...
    string constant _symbol = "KOJI v0.152";
    ...
    function swapBack(uint256 _shares) internal swapping {
        ...
        if (distributorDeposit) {
            ...
            try distributor.deposit{value: amountBNBReflection}(_shares) {} catch {}
        }
    ...
    function ViewMathInfo() external view returns (uint256 totalshares, uint256 totalsharesframe, uint256 netdividends, uint256 totaldistributed, uint256 totalreinvested, uint256 totalwithdrawn, uint256 framedivs, uint256 totaldividends) {
