...
contract THC_NAZA is ERC721, Ownable {
    ...
    uint256 public constant totalSupply = 501;

    // //Prices
    // //0.25 BNB
    // uint256 immutable _priceStandard = 250000000000000000;
    // //0.5 BNB
    // uint256 immutable _priceRare = 500000000000000000;
    // //10 BNB
    // uint256 immutable _priceGenesis = 10000000000000000000;

        // //Prices
    // //0.025 BNB
    uint256 immutable _priceStandard = 25000000000000000;
    // //0.05 BNB
    uint256 immutable _priceRare = 50000000000000000;
    // //0.01 BNB
    uint256 immutable _priceGenesis = 10000000000000000;
    ...
    //Picks randomly and nft from each respective collection (note uses block number)
    function randomNumber(uint256 offset, uint256 range) internal view returns (uint) {
        //range sets range, e.g random numbers 1-100
        //offset sets where range start e.g 100-200
        // with the two we can pick say any number between 45-98 and so on..
        return uint(blockhash(block.number - 1)) % range + offset;
    }
    ...
    */
    function mintGenesis() external payable {
        ...
        require(numMinted < totalSupply, "Sale has already ended");
    ...
    */
    function mintRareNFT() external payable {
        require(numMinted < totalSupply, "Sale has already ended");
        require(_priceRare < msg.value, "Ether value sent is not correct");
    ...
    */
    function mintStandardNFT() external payable {
        require(numMinted < totalSupply, "Sale has already ended");
        require(_priceStandard < msg.value, "Ether value sent is not correct");
        // Incrementing ID to create new tokenn
