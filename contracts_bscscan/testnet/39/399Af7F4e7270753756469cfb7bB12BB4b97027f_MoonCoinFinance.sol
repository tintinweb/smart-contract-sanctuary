...
contract MoonCoinFinance  {
	...
	function getterUser1() public view returns( uint256, address, uint256,uint256,  uint256, uint256) {
			return (
			users[msg.sender].checkpoint,
			users[msg.sender].referrer,			
			users[msg.sender].totalBonus,
			users[msg.sender].promoMultiplier,
			users[msg.sender].teamSize	, users[msg.sender].roiBonusPercent		
			);	
	}
	function getterUser2() public view returns(uint256, uint256,  uint256,  uint256, uint256,uint256, uint256) {
			return (
			users[msg.sender].totalDepositByUser,users[msg.sender].totalReinvestsByUser,
			users[msg.sender].deposits.length,
			users[msg.sender].totalWithdrawnByUser,users[msg.sender].actualWithdrawnByUser, users[msg.sender].lastWithdrawTimeByUser, users[msg.sender].lastDepositTimeByUser
			);	
	}
	function getterUser3() public view returns(uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
			
			return (
			users[msg.sender].tokensIssued,users[msg.sender].promoTokens,users[msg.sender].tokensUnclaimed,users[msg.sender].tokensBurned, users[msg.sender].withdrawMisses, users[msg.sender].myBurnPercent, users[msg.sender].refBonus
			);	
	...
	function getterUserLevels1() public view returns(uint256,uint256,uint256,uint256,uint256,uint256,uint256) {
			return (
			users[msg.sender].levels[0],users[msg.sender].levels[1],users[msg.sender].levels[2],users[msg.sender].levels[3],users[msg.sender].levels[4],users[msg.sender].levels[5],users[msg.sender].levels[6]
			);	
	}
	function getterUserLevels2() public view returns(uint256,uint256,uint256) {
			return (
			users[msg.sender].levels[7],users[msg.sender].levels[8],users[msg.sender].levels[9]
			);	
	}
