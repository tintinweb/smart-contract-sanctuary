...
contract DSMarket is ReentrancyGuard {
    ...
    function getNftContract() view external onlyOwner returns (DSNFT) {
        return nftContract;
    }
    ...
    function getItemPrice(uint256 itemId) external view returns (uint256) {
        return (tokenIdToMarketItem[itemId].priceDBZ);
    }
 
    /* Change price for the market item */
    function setItemPrice(uint256 itemId, uint256 priceDBZ) external onlyOwner {
        tokenIdToMarketItem[itemId].priceDBZ = priceDBZ;
    }
    ...
    function createMarketSale(
        ...
        uint256 priceDBZ = tokenIdToMarketItem[itemId].priceDBZ;
        ...
        IERC20(dbzToken).safeTransferFrom(msg.sender, tokenIdToMarketItem[itemId].seller, amount);
