...
interface IDividendDistributor {
    function setDistributionSettings(uint256 _minPeriod, uint256 _minDistribution, uint maxGas) external;
...
contract DividendDistributor is IDividendDistributor {
    ...
    function setDistributionSettings(uint256 _minPeriod, uint256 _minDistribution, uint _maxGas) external override onlyToken {
    ...
    function setShare(address shareholder, uint256 amount) external override onlyToken {
        ...
        shares[shareholder].totalExcluded = uint128(getCumulativeDividends(share.amount));
    }
    ...
    function process() external override onlyToken {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            if(shouldDistribute(holders[currentIndex])){
                distributeDividend(holders[currentIndex]);
            }
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            try BUSD.transfer(shareholder, amount) {
                ...
                shares[shareholder].totalRealised = uint128(share.totalRealised.add(amount));
                shares[shareholder].totalExcluded = uint128(getCumulativeDividends(share.amount));
            } catch { }
        }
    }
...
contract Watchdog is IBEP20, Auth {
    ...
    uint pushThreshold = 200 * 10 ** 18; // 200 BUSD
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(inSwap || isTxHookExempt[sender] || isTxHookExempt[recipient]){ return _basicTransfer(sender, recipient, amount); }
        
        checkTxLimit(sender, amount);

        if(shouldSwapBack()){ swapBack(); }

        uint senderBal = _balances[sender];
        uint recipientBal = _balances[recipient];
        ...
        if(!isDividendExempt[sender]){ try distributor.setShare(sender, senderBal) {} catch {} }
        if(!isDividendExempt[recipient]){ try distributor.setShare(recipient, recipientBal) {} catch {} }
    ...
    function shouldTakeFee(address sender, address recipient) internal view returns (bool) {
        ...
        if (isSell(recipient)) return true;
    ...
    function swapBack() internal swapping {
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            uint256 amountBNBToBUSD = totalBNBFee.sub(amountBNBLiquidity).sub(amountBNBBuyback);

            uint busdDiff = swapToBusd(amountBNBToBUSD);

            uint totalFeeBUSD = utilityFee.add(reflectionFee);
            uint busdToDistributor = busdDiff.mul(reflectionFee).div(totalFeeBUSD);
    ...
    function swapToBusd(uint amount) internal returns (uint resultingAmt) {
    ...
    function pushFees() public {
        uint toBeDistributed = IBEP20(BUSD).balanceOf(address(this));
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _utilityFee, uint256 _feeDenominator, uint256 _totalSellFee) external {
        ...
        require(_utilityFee <= feeDenominator / 40, "Utility fee too high");
    }
    ...
    function setDistributionSettings(uint256 _minPeriod, uint256 _minDistribution, uint _maxGas) external {
        authorizedFor(Permission.AdjustVariables);
        
        require(_maxGas <= 1500000);
        distributor.setDistributionSettings(_minPeriod, _minDistribution, _maxGas);
    }
