...
contract Kaby is IKaby, ERC721, AcceptedToken, ReentrancyGuard {
    ...
    function buy(uint heroId) external override nonReentrant {
        ...
        bool transferToKaby = IERC20(kabyToken).transferFrom(buyer, address(this), price);
        require(transferToKaby);
    ...
    function offer(uint heroId, uint offerValue) external override nonReentrant {
        ...
        if (needRefund) {
            ...
            bool returnSuccess = IERC20(kabyToken).transferFrom(address(this), buyer, returnedValue);
    ...
    function cancelOffer(uint heroId) external override nonReentrant {
        ...
        bool success = IERC20(kabyToken).transferFrom(address(this), sender, offerValue);
        require(success);
    ...
    function _makeTransaction(uint heroId, address buyer, address seller, uint price) private {
        ...
        bool transferToSeller = IERC20(kabyToken).transferFrom(address(this), seller, price - marketFee);
        require(transferToSeller);

        bool transferToTreasury = IERC20(kabyToken).transferFrom(address(this), owner(), marketFee);
