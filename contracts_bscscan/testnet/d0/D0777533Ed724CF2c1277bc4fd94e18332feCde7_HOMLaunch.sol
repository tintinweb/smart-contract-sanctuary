...
interface HOMMint {
    ...
    function rareSupply() external view returns (LevelSpec memory);

    function epicSupply() external view returns (LevelSpec memory);

    function legendarySupply() external view returns (LevelSpec memory);

    function mythicSupply() external view returns (LevelSpec memory);
}
...
contract HOMLaunch is Launch, ReentrancyGuard {
    ...
    function getPrices() public view returns (uint256[4] memory) {
        return [
            _nft.rareSupply().price,
            _nft.epicSupply().price,
            _nft.legendarySupply().price,
            _nft.mythicSupply().price
        ];
    }

    function getSupplies() public view returns (uint256[4] memory) {
        return [
            _nft.rareSupply().quantity - _nft.rareArtifactsPurchased(),
            _nft.epicSupply().quantity - _nft.epicArtifactsPurchased(),
            _nft.legendarySupply().quantity - _nft.legendaryArtifactsPurchased(),
            _nft.mythicSupply().quantity - _nft.mythicArtifactsPurchased()
        ];
    }
}
