...
contract HIREX is Context, IBEP20, Ownable {
    ...
    uint256 private sellresearchFee  = 2;
    uint256 private sellbuybackFee   = 2;
	...
	// (Liquidity + Marketing + Research + Buyback).
    uint256 private totalLiqFee = liquidityFee.add(marketingFee).add(researchFee).add(buybackFee);
    ...
    constructor (address payable marketingWallet, address payable researchWallet, address payable buybackWallet, address burnAddress, address pancakeSwapRouter) {
    ...
    function setBuyFees(uint256 TaxFee, uint256 LiquidityFee, uint256 MarketingFee, uint256 ResearchFee, uint256 BuybackFee) external onlyOwner {
        taxFee = TaxFee;
        liquidityFee = LiquidityFee;
        marketingFee = MarketingFee;
        researchFee = ResearchFee;
        buybackFee = BuybackFee;
        totalLiqFee = liquidityFee.add(marketingFee).add(researchFee).add(buybackFee);
    }

    function setSellFees(uint256 SellTaxFee, uint256 SellLiquidityFee, uint256 SellMarketingFee, uint256 SellResearchFee, uint256 SellBuybackFee) external onlyOwner {
        selltaxFee = SellTaxFee;
        sellliquidityFee = SellLiquidityFee;
        sellmarketingFee = SellMarketingFee;
        sellresearchFee = SellResearchFee;
        sellbuybackFee = SellBuybackFee;
    }
	...
	function setWallets(address payable marketingWallet, address payable researchWallet, address payable buybackWallet) external onlyOwner() {
        _marketingWallet = marketingWallet;
        _researchWallet = researchWallet;
        _buybackWallet = buybackWallet;
    }
    ...
    function _transfer(
        ...
        if(!(_isExcludeAddressFromMaxTx[from] || _isExcludeAddressFromMaxTx[to])) {
            ...
            if (!_tradingOpen){
                    require(to != pancakeSwapV2Pair, "Trading is not enabled");
            }

            if (block.timestamp < _launchTime + antiSnipingTime && from != address(pancakeSwapV2Router)) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 ethForLiquidity = balanceDiff.sub(ethForMarketing).sub(ethForResearch).sub(ethForBuyback);
