...
contract ChefBEP20Token is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 rewardbps; // percentage of the share, decimals 12
        uint256 maxPoolAmount;
        uint256 minAmount;
        uint256 amount;
        uint256 lpLockinPeriod; // blocks
    }
        ...
        returns (uint256)
    {
        ...
        uint256 deadlineOfUser = user.initBlock + poolInfo[_pid].lpLockinPeriod;
        uint256 endBlock;

        if (deadlineOfUser > block.number) {
            endBlock = block.number;
        } else {
            endBlock = deadlineOfUser;
        }

        if (user.lastRewardBlock >= endBlock) {
            return 0;
        }

        uint256 reward = user.rewardDebt.add(user.amount.mul(endBlock.sub(user.lastRewardBlock)).mul(pool.rewardbps).div(1e12));
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        require(_amount >= pool.minAmount, "deposit:: cannot deposit less than min");
