...
contract Zap is IZap, OwnableUpgradeable {
    ...
    function _swapBNBToLpToken(address lpToken, uint256 amount, address receiver) private {
        if (!isLpToken(lpToken)) {
            ...
            if (token0 == WBNB || token1 == WBNB) {
				...
				_addLiquidityBNB(token, amount, receiver);
            } else {
				_addLiquidity(token0, token1, amount, receiver);
            }
        }
    }
    ...
    */
	function _addLiquidityBNB(address token, uint amount, address receiver ) private {
	
		uint256 tokenBalanceBefore = IBEP20(token).balanceOf(address(this));
		emit LogUint('tokenBalanceBefore', tokenBalanceBefore);
		...
		emit LogUint('tokenBalanceAfter', tokenBalanceAfter);
    ...
    * This function to avoid CompilerError: Stack too deep, try removing local variables on Solidity 
    */
    function _addLiquidity(address token0, address token1, uint amount, address receiver ) private {
