...
contract PositionToken is Context, IERC20, Ownable, Pausable {
    ...
    function _mint(address receiver, uint256 amount) private {
        ...
        _rTotal = _currentRate.mul(tSupply).add(_getRExcluded());
    ...
    function _getRExcluded() private view returns (uint256 rExcluded) {
        for (uint256 i = 0; i < _excluded.length; i++) {
            rExcluded = rExcluded.add(_rOwned[_excluded[i]]);
        }
    }
}
