...
contract Lottery is Ownable {
    ...
    constructor(uint256 _hardCapOfNextPool, uint256 _feePrizePool1, uint256 _feeNextPool1, uint256 _feePrizePool2, uint256 _numberOfWinners, IERC20 _chipz) {
    ...
    function enter(uint256 _amount) external payable inState(STATE.OPEN){
        require(_amount >= 1000, 'Minimum entry is 1000 CHPZ');
        require(endingTimeStamp.sub(block.timestamp).add(5) < 18000 ,'Prize Hard Cap time has reached out');
        uint256 entryPrize;
        uint256 entryNext;
        chipz.transferFrom(msg.sender, address(this), _amount);
        (entryPrize, entryNext) = _splitEntry(_amount);
        balanceOfPrizePool = balanceOfPrizePool + entryPrize;
        balanceOfNextPool = balanceOfNextPool + entryNext;
        
        chasers.push(msg.sender);
        putIns.push(_getPutin(_amount, msg.sender));
    ...
    function deliverPrize() external inState(STATE.OPEN) onlyOwner(){
        ...
        for(uint i = 0; i < _candsOfWin.length; i++){
            if(_candsOfWin[i] != address(0)){
                safeChipzTransfer(_candsOfWin[i], _willWins[i]);
            }     
        }
        balanceOfPrizePool = 0;
        currentState = STATE.IDLE;
    ...
    function withdrawMarketing(uint256 amount) external onlyOwner(){
        ...
        safeChipzTransfer(msg.sender, amount);
    }
    ...
    function sendToThisMarketing(address marketingAddress) external onlyOwner(){
        safeChipzTransfer(marketingAddress, (address(this).balance).sub(balanceOfPrizePool).sub(balanceOfNextPool));
    }
