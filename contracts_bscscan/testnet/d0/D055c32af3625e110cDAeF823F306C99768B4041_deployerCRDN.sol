...
contract deployerCRDN is ReentrancyGuard {
    ...
    mapping(address => mapping(address => uint256)) public getPreSaleCount;
    ...
    function createPreSaleBNB(
        ...
        getPreSale[msg.sender][address(token)].push(preSaleContract);
    ...
    function setPreSaleBNB(
        ...
        require(getPreSale[msg.sender][address(_token)][getPreSaleCount[msg.sender][address(_token)]] != address(0),"CRDN: No preSale found");
        require(!isInitialized[address(_token)],"CRDN: Already initialized");
        ...
        token.transferFrom(msg.sender, getPreSale[msg.sender][address(_token)][getPreSaleCount[msg.sender][address(_token)]], tokenAmountToSend);
        IPreSale(getPreSale[msg.sender][address(_token)][getPreSaleCount[msg.sender][address(_token)]]).initializeRemaining(
            _tokenPrice,
            _presaleTime,
            _hardCap,
            _softCap,
        ...
        isInitialized[address(_token)] = true;
        getPreSaleCount[msg.sender][address(token)]++;
    }
    ...
    function getTotalNumberOfTokens(
        ...
        uint256 _hardCap = _tokenAmount.div(_tokenPrice).mul(1000);
        ...
        return (tokensForSell.add(tokensForListing).div(1000), _hardCap);
    }
