...
// File contracts/interfaces/IOAXDEX_RestrictedLiquidityProvider.sol

// SPDX-License-Identifier: GPL-3.0-only
pragma solidity =0.6.11;

interface IOAXDEX_RestrictedLiquidityProvider {
...
// File contracts/OAXDEX_RestrictedLiquidityProvider.sol


pragma solidity =0.6.11;
contract OAXDEX_RestrictedLiquidityProvider is IOAXDEX_RestrictedLiquidityProvider {
    using SafeMath for uint256;
    ...
    constructor(address _factory, address _WETH) public {
        factory = _factory;
        WETH = _WETH;
        oaxToken = IOAXDEX_Governance(IOAXDEX_RestrictedFactory(_factory).governance()).oaxToken();
    }
    ...
    function _getPair(address tokenA, address tokenB, uint256 pairIndex) internal returns (address pair) {
        uint256 pairLen = IOAXDEX_RestrictedFactory(factory).pairLength(tokenA, tokenB);
        if (pairIndex == 0 && pairLen == 0) {
            pair = IOAXDEX_RestrictedFactory(factory).createPair(tokenA, tokenB);
        } else {
            require(pairIndex <= pairLen, "OAXDEX_Router: Invalid pair index");
    ...
    function addLiquidity(
        ...
        if (feeIn > 0)
            TransferHelper.safeTransferFrom(oaxToken, msg.sender, pair, feeIn);
        if (amountIn > 0)
        ...
        _offerIndex = IOAXDEX_RestrictedPair(pair).addLiquidity(msg.sender, /*direction*/addingTokenA, offerIndex, feeIn, locked, restrictedPrice, startDate, expire);
    }
    function addLiquidityETH(
        address tokenA,
        bool addingTokenA,
        ...
        _offerIndex = IOAXDEX_RestrictedPair(pair).addLiquidity(msg.sender, direction, offerIndex, feeIn, locked, restrictedPrice, startDate, expire);
    }
        ...
        returns (address pair, uint256 offerIndex) 
    {
        ...
        IOAXDEX_RestrictedPair(pair).addMultipleApprovedTrader(b, offerIndex, trader, allocation);
    }
    ...
    function removeLiquidity(
        ...
        IOAXDEX_RestrictedPair(pair).removeLiquidity(msg.sender, direction, offerIndex, amountOut, receivingOut);
    ...
    function removeAllLiquidity(
        ...
        address pair = pairFor(tokenA, tokenB, pairIndex);
        (uint256 amount0, uint256 amount1) = IOAXDEX_RestrictedPair(pair).removeAllLiquidity(msg.sender);
        // (uint256 amount0, uint256 amount1) = IOAXDEX_RestrictedPair(pair).removeAllLiquidity1D(msg.sender, false);
        // (uint256 amount2, uint256 amount3) = IOAXDEX_RestrictedPair(pair).removeAllLiquidity1D(msg.sender, true);
        // amount0 = amount0.add(amount3);
        // amount1 = amount1.add(amount2);
    ...
    function pairFor(address tokenA, address tokenB, uint256 index) internal view returns (address pair) {
        ...
        pair = address(uint256(keccak256(abi.encodePacked(
                ...
                /*restricted*/hex'a4f3963b8e4e0847eb25c8e2b9016038085bc6126a1527868829edffef60bf06' // restricted init code hash
            ))));
    }
