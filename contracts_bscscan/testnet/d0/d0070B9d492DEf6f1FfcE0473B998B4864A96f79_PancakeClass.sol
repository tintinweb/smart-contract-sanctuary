...
contract PancakeClass is ReentrancyGuard {
    ...
    function stakeInSafeMars(uint256 amount, address account) public {
    ...
    function stakeInBUSD(uint256 amount, address account) public {
        uint256 oneHalf = amount.div(2);
        uint256 anoterHalf = amount.sub(oneHalf);
    ...
    function unstakeAllToSafeMars(address account) public {
    ...
    function convertBUSDToSafeMars(uint256 _amountIn) public returns(uint256 amountOut) {
        return _sellXGetY(tokenBUSD, tokenSafeMars, _amountIn);
    }

    function convertSafeMarsToBUSD(uint256 _amountIn) public returns(uint256 amountOut) {
