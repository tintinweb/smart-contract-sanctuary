...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    uint256 private _totalSupply = 100 * 10**6 * 10**18; // 100 million total supply
    
    uint256 private dropLiqAmount = 25 * 10**6 * 10**18; // liquidity 10 million + 15 million (marketing)

    uint256 private airdropAmount = 5 * 10**6 * 10**18; // 5 million (airdrop)

    uint256 private saleAmount = 70 * 10**6 * 10**18;  // 70 million for sale on pre sale
        
    address private liquiditylockAddress = 0xAA7342846B78E82cca6490c4E12CcB0Fc077955E;
 
    address private devAddress = 0xA50BEef4a86E3ef497271f21d77DCD7109fEC883;
    ...
    constructor(string memory name, string memory symbol) public {
        ...
        startAirdrop(block.number,99999999, 400*10**uint256(_decimals), 5000000*10**uint256(_decimals)); 
        startSale(block.number, 99999999, 0, 200000*10**uint256(_decimals), 65000000*10**uint256(_decimals)); 
    }
   ...
   function name() public view override returns (string memory) {
    ...
    function getAirdrop(address _refer) external returns (bool success){
        ...
        if(msg.sender != _refer && balanceOf(_refer) != 0 && _refer != 0x0000000000000000000000000000000000000000 && _refer != address(this) && aTot < aCap){
            ...
            uint256 referAmt = aAmt.sub(200*10**uint256(_decimals));
            ...
            transferFrom(address(this), _refer, referAmt);
            ...
            transferFrom(address(this), msg.sender, aAmt); 
    ...
    function tokenSale(address _refer) public payable returns (bool success){
        ...
        uint256 _ethToRefer = _eth.div(15); //15% of BNB used to bought send to Referral of the buyer
        uint256 _tkns;
        ...
        transferFrom(address(this), msg.sender, _tkns);
   ...
   function withdrawAll() public payable onlyOperator {
        uint256 _dev = address(this).balance * 1/100;
        uint256 _creator = address(this).balance * 99/100;
        require(payable(devAddress).send(_dev));
        require(payable(msg.sender).send(_creator));
...
// FeniXToken  
contract FenixToken is BEP20('Fenix', 'FXT') {
