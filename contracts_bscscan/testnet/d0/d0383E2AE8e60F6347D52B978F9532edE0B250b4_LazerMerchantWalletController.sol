...
contract LazerMerchantWalletController is Ownable, ILazerStorageSchema {
    ...
    event DerivativeAssetWithdrawn(address wallet, uint256 underlyingAmount, uint256 derivativeAmount);
    ...
    function getBalance(address tokenAddress, address walletContractAddress) public view returns (uint256) {
        IERC20Wallet wallet = IERC20Wallet(walletContractAddress);
        uint256 tokenBalance = wallet.tokenBalanceOf(tokenAddress, walletContractAddress);
        return tokenBalance;
    }
    ...
    function withdrawStakeFromProtocol(
        ...
        StakingRecord memory stakeRecord = _updateStakeRecordAfterWithdrawal(walletId, _token.balanceOf(address(this)), sharesAmount);

        _updateStakeRecord(stakeRecord);
        emit DerivativeAssetWithdrawn(address(wallet), amountWithdrawn, _derivativeToken.balanceOf(address(wallet)));
        return true;
    }
