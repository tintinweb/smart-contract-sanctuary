...
contract BloqBallLottery is ReentrancyGuard, IBloqBallSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInCake = 50 * 10 ** 18;       // 50 BQB
    uint256 public minPriceTicketInCake = 1 * 10 ** 18; // 1 BQB

    uint256 public pendingInjectionNextLottery;

    uint256 public constant MIN_DISCOUNT_DIVISOR = 300;
    uint256 public constant MIN_LENGTH_LOTTERY = 12 hours - 5 minutes; // 4 hours
    uint256 public constant MAX_LENGTH_LOTTERY = 15 days + 5 minutes; // 4 days

    uint256 public constant MAX_TREASURY_FEE = 500; // 5%
    uint256 public constant MAX_STAKEBQB_FEE = 500; // 5%

    IERC20 public BQBToken;
     ...
     * @param _BQBTokenAddress: address of the CAKE token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _BQBTokenAddress, address _randomGeneratorAddress) {
        BQBToken = IERC20(_BQBTokenAddress);
    ...
    /**
     * @notice Generate tickets for the current buyer
     * @param _lotteryId: lotteryId
     * @dev Callable by users
     */
    function generateTicketNumber(uint256 _lotteryId, uint256[] memory _ticketNumbers) public {
        uint256 ticketNumber;
        uint256 _itemNumber;

        for (uint j = 0; j<_ticketNumbers.length; j++)
        {
            ticketNumber = 0;
            for (uint256 i = 0; i < 6; i++)
            {
                randomGenerator.getRandomNumber(uint256(keccak256(abi.encodePacked(_lotteryId, i))));
                _itemNumber = (randomGenerator.viewRandomResult() % 21) + 1;
                ticketNumber += _itemNumber * uint256(10)**(i*2);
            }
            _ticketNumbers[j] = ticketNumber;
        }
    }
     ...
     * @dev Callable by users
     */
    function generateTicketNumber(uint256 _lotteryId) public returns (uint256){
        uint256 ticketNumber;
        uint _itemNumber;
        ...
        nonReentrant
    {
        ...
        BQBToken.safeTransferFrom(address(msg.sender), address(this), amountCakeToTransfer);

        // Get tickets numbers
        for (uint j = 0; j<_ticketNumbers.length; j++)
        {
            _ticketNumbers[j] = generateTicketNumber(_lotteryId);
        }
        ...
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            ...
            _numberTicketsPerLotteryId[_lotteryId][1 + (thisTicketNumber / 10000000000)]++;
            _numberTicketsPerLotteryId[_lotteryId][11 + (thisTicketNumber / 100000000)]++;
            _numberTicketsPerLotteryId[_lotteryId][111 + (thisTicketNumber / 1000000)]++;
            _numberTicketsPerLotteryId[_lotteryId][1111 + (thisTicketNumber / 10000)]++;
            _numberTicketsPerLotteryId[_lotteryId][11111 + (thisTicketNumber / 100)]++;
            _numberTicketsPerLotteryId[_lotteryId][111111 + (thisTicketNumber / 1)]++;
    ...
    function claimTickets(
        ...
        BQBToken.safeTransfer(msg.sender, rewardInCakeToTransfer);
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = _lotteries[_lotteryId].amountCollectedInCake;
...
//      ) / 10000;
        ...
        // Initializes the amount to withdraw to masterchef
        uint256 amountToWithdrawToMasterchef;

        // Calculate prizes in CAKE for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            uint32 j = 5 - i;
            uint256 transformedWinningNumber = _bracketCalculator[j] + (finalNumber % (uint32(10)**((j + 1)*2)));
...
//      amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInCake - amountToShareToWinners);
        amountToWithdrawToTreasury += amountToShareToWinners * 500 / 10000;      // send 5% to treasury

        // Transfer CAKE to treasury address
        BQBToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
        
        amountToWithdrawToMasterchef = amountToShareToWinners * 500 / 10000;      // send 5% to masterchef
        
        // Transfer CAKE to masterchef address
        BQBToken.safeTransfer(masterchefAddress, amountToWithdrawToMasterchef);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        BQBToken.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function startLottery(
        ...
        require(
            (_rewardsBreakdown[0] +
                ...
                _rewardsBreakdown[3] +
                _rewardsBreakdown[4] +
                _rewardsBreakdown[5]) == 9000,
            "Rewards must equal 9000"               // 45%, 20%, 15%, 10%
        );
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(BQBToken), "Cannot be CAKE token");
    ...
    function setOperatorAndTreasuryAndInjectorAddresses(
        ...
        require(_masterchefAddress != address(0), "Cannot be zero address");
