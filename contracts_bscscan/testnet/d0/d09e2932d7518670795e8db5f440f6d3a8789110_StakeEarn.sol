...
contract StakeEarn is Ownable, ReentrancyGuard {
	...
	function getUserStats(address userAddress, uint256 id) public view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
	    UserInfo storage user = userInfo[userAddress];
		return (user.deposits[id].amount, user.deposits[id].withdrawn, user.deposits[id].startTime, user.deposits[id].endTime, user.deposits[id].status, user.deposits[id].withdrawnTime);
    }
