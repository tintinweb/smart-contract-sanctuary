...
contract TokenWithdrawable is Ownable {
    ...
    function withdrawToken(IERC20 token, uint256 amount, address to) external onlyOwner {
        ...
        if(address(token) == address(this)) {
            require(token.balanceOf(address(this)) - amount >= swapFund + burnFund, "sufficient funds");
        }
        token.safeTransfer(to, amount);
        emit TokenWithdrawn(address(token), amount, to);
    }
}
// File: contracts/WastedLands.sol
...
contract WastedLands is Context, Ownable, IERC20, TokenWithdrawable {
    ...
    function transferToBurnFund(uint amount) public onlyOwner returns (bool) {
    ...
    function _transfer(address fromAddress, address toAddress, uint256 amount) private {
        ...
        if(exceptionAddress[fromAddress] || exceptionAddress[toAddress]) {
            ...
            if ( swapFee > 0 && fromAddress != owner() && (routerAddresses[toAddress] || routerAddresses[fromAddress])){
