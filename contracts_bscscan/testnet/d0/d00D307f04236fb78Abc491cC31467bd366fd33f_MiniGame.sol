...
contract MiniGame is SamoyedGame, ReentrancyGuard {
    ...
    mapping(address => bool) public canDrawWinner;
    ...
    function addDrawWinner(address winnerDrawer) external onlyOwner {
        canDrawWinner[winnerDrawer] = true;
    }

    function removeDrawWinner(address winnerDrawer) external onlyOwner {
        canDrawWinner[winnerDrawer] = false;
    }
    ...
    function sitWithSmoy(uint8 sitQuantity, uint8[] memory sitPosition) external {
        collectFee(smoyToken, cashier, sitQuantity * smoyCostPerSeat);
    ...
    function drawWinner() external nonReentrant {
        require(canDrawWinner[msg.sender], "MiniGame: UnAuthorized Drawer");
        require(nextWinnerDrawRoomNo <= currentRoomNo - 1, "MiniGame: WinnerDraw cannot exceed currentRoomNo");
        winner[nextWinnerDrawRoomNo] = random();
        emit wonEvent(
