...
contract StakeEarn is Ownable, ReentrancyGuard {
	...
	function pendingreward(address _user) public view returns (uint256) {
		...
		return treward.add(remaining).sub(user.rewardDebt);
    }
    ...
    function withdraw() public nonReentrant {
		...
		user.rewardRemaining = user.rewardRemaining.add(pending);
		user.amount=0;
	...
	function getUserInfo(address userAddress) public view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        UserInfo storage user = userInfo[userAddress];
        return (user.amount, user.rewardRemaining, user.startTime, user.endTime, user.minTime, user.rewardPerStake, user.decimal, user.rewardDebt);
    }
