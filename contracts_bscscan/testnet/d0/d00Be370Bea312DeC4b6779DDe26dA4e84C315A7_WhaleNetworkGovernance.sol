...
contract WhaleNetworkGovernance is Context, Ownable {
    ...
    function voteOnProposal(uint256 proposalId, uint256 vote)external returns(bool){
        //vote = 0 - NO
        //vote = 1 - YES
        require(
        ...
        if(vote==1){
            _votes[proposalId].push(Voter(_msgSender(), Vote.YES));
        }else{
            _votes[proposalId].push(Voter(_msgSender(), Vote.YES));
        }
    ...
    function getVote(uint256 proposalId, address addr)public view returns(Vote){
        require( _proposalVotingStatus[proposalId][addr], "You have not voted on this proposal");
        for(uint256 i =0; i< _votes[proposalId].length; i++){
            if(_votes[proposalId][i].voter == addr){
                return _votes[proposalId][i].vote;
            }
        }
