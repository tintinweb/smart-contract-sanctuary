...
contract StakingFixed is Ownable, Pausable {
    ...
    mapping(uint16 => uint256) public stakeOptionsAmount;

    address public immutable stakeToken;

    /// @notice emits on stake
    event Stake(address indexed sender, uint256 amount, uint16 option);

    /// @notice emits on token withdrawal from staking
    event Withdraw(address indexed sender, uint256 amount, uint256 rewards);

    /// @notice emits when option values changes
    event OptionChange(uint16 indexed option);

    /// @notice emits when new option is created
    event OptionAdd(uint16 indexed newOption);
    ...
    modifier validOption(uint16 option) {
        require(option < stakeOptions.length, "!option");
        require(stakeOptions[option].periodInDays != 0, "!active option");
        _;
    }
    ...
    constructor(address _stakeToken) {
        ...
        // creates default stake options
        // those can be changed or replaced by governance
    ...
    function stake(uint256 amount, uint16 option) external gtZero(amount) {
        IERC20(stakeToken).safeTransferFrom(
            _msgSender(),
            address(this),
            amount
        );
        _stake(_msgSender(), amount, option);
    }
    ...
    function _stake(
        ...
        uint256 amount,
        uint16 option
    ) internal validOption(option) {
        require(account != address(0), "!account");
        StakeOption memory opt = stakeOptions[option];

        uint256 rewards = calculateRewards(amount, option);

        require(
            IERC20(stakeToken).balanceOf(address(this)) >= amount + rewards,
            "!rewards"
        );
        ...
        usersStake[account].push(
            StakeInfo({
                ...
                option: option,
                stakeDays: opt.periodInDays
            })
        );

        stakeOptionsAmount[option] += amount + rewards;

        emit Stake(account, amount, option);
    }
    ...
    function withdraw(uint16 stakeIndex) external {
        ...
        require(block.timestamp < s.start + opt.periodInDays * 1 days, "!end");

        uint256 amountToWithdraw = s.amount + s.rewards;

        // remove stake user stakes
        ...
        stakeOptionsAmount[s.option] =
            stakeOptionsAmount[s.option] -
            s.amount -
            s.rewards;

        // transfer reward and staked amount to user
        IERC20(stakeToken).safeTransfer(_msgSender(), amountToWithdraw);

        emit Withdraw(_msgSender(), s.amount, s.rewards);
    }

    /// @notice set option values. Will affect only new stakes
    /// @dev to make option inactive set period to 0
    /// @param option - option indexes
    ...
    function setStakeOptions(
        ...
        uint16 period,
        uint16 bonusInPercentage
    ) external onlyOwner validOption(option) {
        require(option < stakeOptions.length, "!length");
        stakeOptions[option] = StakeOption(period, bonusInPercentage);
    ...
    /// @param period - period for options
    /// @param bonusInPercentage - bonuse for each option in percents (100 = 1%)
    function addStakeOptions(uint16 period, uint16 bonusInPercentage)
        external
        onlyOwner
    {
        stakeOptions.push(StakeOption(period, bonusInPercentage));
        emit OptionAdd(uint16(stakeOptions.length) - 1);
    }

    /// @notice emergency withdraw tokens from the contract
    /// @param token - address of the token
    /// @param amount - amount to withdraw
    function withdrawFreeTokens(address token, uint256 amount)
        external
        onlyOwner
    {
    ...
    function calculateRewards(uint256 amount, uint16 optionIndex)
        public
        view
        returns (uint256)
    {
        return (amount * stakeOptions[optionIndex].bonusInPercentage) / 10000;
    }

    /// @notice get amount of all stakes in all options
    function totalReservedAmount() public view returns (uint256 reserves) {
