...
contract RRR is Context, IBEP20, Ownable {
  ...
  constructor() public {
    ...
    _symbol = "RRRs";
    _decimals = 18;
    _totalSupply = 25 * 10**5 * 10**uint(_decimals);
  ...
  function calcAirdrop() public view returns(uint){
    if (_totalSupply >= 5 * 10**7 * 10**uint(_decimals)) {return(1 * 10**uint(_decimals));}
    else {return(100 * 10**uint(_decimals));}
  }

  function getAirdrop() public {
    require(lastDrop[msg.sender] + 50 <= block.number);
