...
/**
 * @dev The official TiggerCointest smart contract
 */
contract TiggerCoin is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    // General Info
    string private _name     = "TiggerCointest";
    string private _symbol   = "TGRT";
    uint8  private _decimals = 9;
    
    // Liquidity Settings
    IUniswapV2Router02 public _pancakeswapV2Router; // The address of the PancakeSwap V2 Router
    address public _pancakeswapV2LiquidityPair;     // The address of the PancakeSwap V2 liquidity pairing for TiggerCointest/WBNB
    ...
    // Addresses
    address payable public _marketingAddress = payable(0xFf8844183873405f41716Ae2FEA5348E5E96a87c); // Marketing address used to pay for marketing
    ...
    uint256 public  _sellTaxFee       = 9; // 9% tax 
    uint256 public  _whaleSellTaxFee  = 21; // 21% tax 
    uint256 private _previousTaxFee;
    
    // Token Limits
    uint256 public _maxTxAmount        = 1 * 10**6 * 10**2;  // 1 million
    uint256 public _tokenSwapThreshold = 1* 10**9 * 10**9; // 100000
    
    // Timer Constants 
    uint private constant DAY = 86400; // How many seconds in a day
    
    // Anti-Whale Settings 
    uint256 public _whaleSellThreshold = 5 * 10*4 * 10**4; // 10000
    ...
    uint256 public _buybackUpperLimit    = 1 * 10**5 * 10**5; // 100 Thousand $TiggerCointest will be used to buy at maximum in any one trade
    ...
    uint256 public  _lotteryThreshold    = 10 * 10**2 * 10**2; // 100 tokens required to be in the pool before lottery can be triggered
    uint256 public  _lotteryMinimumSpend = 10 * 10**2 * 10**2; // 100 tokens required to buy before before lottery can be triggered
    ...
    /**
     * @notice Withdraws non-TiggerCointest tokens that are stuck as to not interfere with the liquidity
     */
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // Gets the contracts TiggerCoin balance for buybacks, charity, liquidity and marketing
    ...
    /**
     * @notice Burns TiggerCointest tokens straight to the burn address
     */
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockSwapping {
        ...
        // Swap 7/8ths of TiggerCointest tokens for BNB
        ...
        // We now have 6/7ths left of BNB (converted from TiggerCointest)
    ...
    function swapBNBForTokens(uint256 amount) private {
        ...
        _pancakeswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}(
            0, // Accept any amount of TiggerCointest
            path,
