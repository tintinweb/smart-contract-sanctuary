...
contract pool {
    ...
    address[] BUSD_BUST_path = [address(BUSD), address(BUST)];
    address[] WBNB_BUST_path = [address(WBNB), address(BUST)];
    ...
    function BUSD_DAO(uint _amount, address addr) internal{
     ...
     // case 1 and case 3 part 2 
      ...
      function getBUST_BNBLP(uint _amount, address addr) internal{
        ...
        uint tok0min = _reserve0.div(tSupply).mul(rmAmount).mul(rnum).div(rden);
        uint tok1min = _reserve1.div(tSupply).mul(rmAmount).mul(rnum).div(rden);
        uint256 time = block.timestamp + 1120;
