...
contract SamoyedMarketplace is Pausable, SamoyedMarketBase, ReentrancyGuard, ISamoyedMarketplace {
    ...
    function withdrawBalance() external onlyOwner {
        ...
        IERC20(ERC20Token).transfer(owner(), getBalance());
    }

    function changeFee(uint256 _feeRate) external onlyOwner {
    ...
    function cancelListingWhenPaused(uint256 _tokenId) external whenPaused onlyOwner nonReentrant {
        Listing memory listing = getEntryByKey(_tokenId);
        require(_isOnListing(listing), "Marketplace: TokenID is a must on listing");
...
abstract contract SamoyedMarketBase is ListingEnumerator {
    ...
    event ListingCreated(uint256 tokenId, address seller, uint256 startingPrice, uint256 endingPrice, uint256 duration);
    event ListingSuccessful(uint256 tokenId, uint256 totalPrice, address winner);
    ...
    function _purchase(uint256 _tokenId, uint256 _offerPrice) internal returns (uint256) {
        ...
        emit ListingSuccessful(_tokenId, price, msg.sender);
