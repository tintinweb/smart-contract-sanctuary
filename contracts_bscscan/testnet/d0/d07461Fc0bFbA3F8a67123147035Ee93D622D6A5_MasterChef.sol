...
contract MasterChef is Ownable {
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            uint256 sushiReward =  IChaChaLP(address(sushi)).getMultiplierForLp(pool.lastRewardTime, block.timestamp).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
        ...
        returns (uint256,uint256,uint256)
    {
        ...
        UserInfo storage user = userInfo[_pid][_user];
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        uint256 sushiReward =  IChaChaLP(address(sushi)).getMultiplierForLp(block.timestamp - 1 days, block.timestamp);
        if(sushiReward==0||lpSupply == 0){
            return (user.amount,lpSupply,0);
        }else{
            uint256 lpSushiReward =  sushiReward.mul(pool.allocPoint).div(totalAllocPoint);
            uint256 daySushiReward =  lpSushiReward.div(lpSupply).mul(sushiReward);
            return (user.amount,lpSupply,daySushiReward.mul(1 ether).div(lpSushiReward).mul(365));
        }
    }   
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 sushiReward = 
            IChaChaLP(address(sushi)).getMultiplierForLp(pool.lastRewardTime, block.timestamp).mul(pool.allocPoint).div(
                totalAllocPoint
            );
