...
contract SpaceMelody is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x97bF4F880aa48D1957255cD944f3881B2A31de50); // Marketing Address
    ...
    uint256 private _tTotal = 2100000000 * 10**2 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "testpop";
    ...
    uint256 public _maxTxAmount = 2100000 * 10**2 * 10**9;
    uint256 private minimumTokensBeforeSwap = 200000 * 10**2 * 10**9; 
    uint256 public buyBackSellLimit = 2 * 10**10;
    ...
    function _transfer(
        ...
        if(_isExcludedFromSwapAndLiquify[to] || _isExcludedFromSwapAndLiquify[from]){
            doSwapAndLiquify = false;
        }

        // Sell tokens for ETH
        if (doSwapAndLiquify && !inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
    ...
    function prepareForPreSale() external onlyOwner {
        ...
        _taxFee = 0;
        _liquidityFee = 0;
        _maxTxAmount = 2100000000 * 10**2 * 10**9;
    }
    ...
    function afterPreSale() external onlyOwner {
        ...
        _taxFee = 2;
        _liquidityFee = 10;
        _maxTxAmount = 2100000 * 10**2 * 10**9;
    }
