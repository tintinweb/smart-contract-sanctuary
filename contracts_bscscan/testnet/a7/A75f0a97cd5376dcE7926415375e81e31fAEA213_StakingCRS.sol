...
abstract contract Staking is IStaking, ReentrancyGuard, Pausable {
    ...
    function earned(address account) public view returns (uint256) {
        return _shareBalances[account].mul(rewardPerToken().sub(userRewardPerTokenPaid[account])).div(1e18).add(rewards[account]);
    }
...
contract StakingCRS is Staking {
    ...
    function stake(uint256 amount) external override nonReentrant whenNotPaused updateReward(msg.sender) {
        ...
        _totalShare += amount;
        _shareBalances[msg.sender] += amount;
    ...
    function _withdraw(uint256 amount) internal nonReentrant updateReward(msg.sender) {
        require(amount > 0 && amount <= _shareBalances[msg.sender], "check your share amount!");

        uint256 withdrawAmount = amount * _totalStaking / _totalShare;
