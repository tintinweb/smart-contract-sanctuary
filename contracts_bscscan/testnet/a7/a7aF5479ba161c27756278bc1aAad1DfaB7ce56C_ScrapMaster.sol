...
contract ScrapMaster is Ownable {
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFee) external onlyOwner {
        massUpdatePools();
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 tokenReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        shitiumToken.mint(address(this), tokenReward);
    ...
    function setTokenPerBlock(uint256 _tokenPerBlock) external onlyOwner {
        require(_tokenPerBlock > 0,"Token per block must be greather than zero");
        massUpdatePools();
        tokenPerBlock = _tokenPerBlock;
    }
