...
contract LoanTokenBase is ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 public firstOmega;
    uint256 public secondOmega;
    uint256 public utilTarget;
...
contract GasTokenUser {
    ...
    ITokenHolderLike public constant tokenHolder = ITokenHolderLike(0x23eFB940FC04824a32ABe09e05089794DC172024);
...
contract LoanTokenLogicForward is AdvancedToken, GasTokenUser {
    ...
    address public constant bZxContract = 0xFE2E1555303d34c06d668C75cc3a650e873bcE57; // bsc testnet
    ...
    function _nextBorrowInterestRate3(uint256 newBorrowAmount, uint256 assetSupply) internal view returns (uint256 nextRate) {
        ...
        uint256 thisUtilTarget = utilTarget;
        uint256 thisFirstOmega = firstOmega;
        uint256 thisSecondOmega = secondOmega;
        uint256 thisMaxRate = thisBaseRate.add(thisFirstOmega).add(thisSecondOmega);
        
        nextRate = thisBaseRate;
        
        if (utilRate > 0) {
            if (utilRate <= thisUtilTarget){
                uint256 firstOmegaFactor = utilRate.mul(thisFirstOmega).div(thisUtilTarget);
                nextRate = nextRate.add(firstOmegaFactor);
            }
            if (utilRate > thisUtilTarget){
                uint256 secondOmegaFactor = utilRate.sub(thisUtilTarget).mul(thisSecondOmega).div(SafeMath.sub(WEI_PERCENT_PRECISION,thisUtilTarget));
                nextRate = nextRate.add(thisFirstOmega).add(secondOmegaFactor);

                if (nextRate > thisMaxRate) {
