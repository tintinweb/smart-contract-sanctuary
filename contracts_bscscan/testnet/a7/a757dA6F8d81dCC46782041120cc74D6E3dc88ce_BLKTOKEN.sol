...
contract BLKTOKENDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("BLKTOKEN_Dividend_Tracker", "BLKTOKEN_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BLKTOKEN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BLKTOKEN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BLKTOKEN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BLKTOKEN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "BLKTOKEN_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BLKTOKEN is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    BLKTOKENDividendTracker public dividendTracker;
    ...
    event SeBLKtomatedMarketMakerPair(address indexed pair, bool indexed value);
...
/////---BUYBACK----////    
    ...
    constructor() ERC20("BLKTOKEN", "BLK") {
        ...
        marketingFee = 6;
        buybackFee = 1;
        marketingWallet = payable(0x5bd8E9709da6c9C7B0912EC927373e012027b848);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee); // total fee transfer and buy

    	dividendTracker = new BLKTOKENDividendTracker();
        ...
        _seBLKtomatedMarketMakerPair(_uniswapV2Pair, true);
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BLKTOKEN: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BLKTOKEN: Account is already the value of 'excluded'");
    ...
    function seBLKtomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BLKTOKEN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _seBLKtomatedMarketMakerPair(pair, value);
    }

    function _seBLKtomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BLKTOKEN: Automated market maker pair is already set to that value");
        ...
        emit SeBLKtomatedMarketMakerPair(pair, value);
    }

    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BLKTOKEN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BLKTOKEN: Cannot update gasForProcessing to same value");
