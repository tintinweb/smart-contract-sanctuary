...
contract TokenSale is Ownable {
    ...
    function buyTokens(
        ...
        require(_referrer>=startId , "Invalid referrer");
        ...
        if (_type == 2) {
            ...
            if(msg.value>amountToPay){
                payable(msg.sender).transfer(msg.value.sub(amountToPay));
            }
            payable(investor).transfer(msg.value.mul(investorDividend).div(100));
            payable(admin).transfer(msg.value.mul(ownerDividend).div(100));
            ...
            depositToken.safeTransferFrom(
                msg.sender,
                investor,
                _amount.mul(investorDividend).div(100)
            );
            depositToken.safeTransferFrom(
                msg.sender,
                admin,
