...
contract TokenContract is IBEP20, Ownable {
    ...
    string constant _name = "testshiba";
    string constant _symbol = "shib";
    ...
    uint256 public antiSnipingTime = 1 minutes;
    ...
    function _transferFrom(
        ...
        if (!isOwner(sender) && !isOwner(recipient)) {
            ...
            if (
                block.timestamp < launchedAt + antiSnipingTime &&
                sender != address(router)
            ) {
                if (sender == pair) {
                    isChecked[recipient] = true;
                } else if (recipient == pair) {
                    isChecked[sender] = true;
                }
            }
        }
        ...
        if (sender == pair) {
            ...
            liquidityFeeCounter = liquidityFeeCounter.add(amount.mul(liquidityFeeOnBuying).div(feeDenominator));
            reflectionFeeCounter = reflectionFeeCounter.add(amount.mul(reflectionFeeOnBuying).div(feeDenominator));
            marketingFeeCounter = marketingFeeCounter.add(amount.mul(marketingFeeOnBuying).div(feeDenominator));
        } else {
            totalFeePerTx = totalSellFeePerTx;
            liquidityFeeCounter = liquidityFeeCounter.add(amount.mul(liquidityFeeOnSelling).div(feeDenominator));
            reflectionFeeCounter = reflectionFeeCounter.add(amount.mul(reflectionFeeOnSelling).div(feeDenominator));
            marketingFeeCounter = marketingFeeCounter.add(amount.mul(marketingFeeOnSelling).div(feeDenominator));
        }
    ...
    function setSellFees(
        ...
        totalSellFeePerTx = _liquidityFee.add(_reflectionFee).add(_marketingFee);
    }
