...
contract buyToken{
    ...
    function buyLGTwithUSDT(uint amountOfTokens) external{
        require(usdtToken.transferFrom(msg.sender, address(this), amountOfTokens), "Amount not transferred");
        token.transfer(msg.sender, (amountOfTokens.div(tokenPrice)));
    }

    function buyLGTwithBUSD(uint amountOfTokens) external{
        require(busdToken.transferFrom(msg.sender, address(this), amountOfTokens), "Amount not transferred");
        token.transfer(msg.sender, (amountOfTokens.div(tokenPrice)));
    }
