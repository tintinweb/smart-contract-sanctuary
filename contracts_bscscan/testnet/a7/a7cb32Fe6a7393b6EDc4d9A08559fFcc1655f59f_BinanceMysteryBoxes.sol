...
contract BinanceMysteryBoxes is Ownable {
    ...
    event Claim(uint256 indexed orderId, uint256 indexed bnftTokenId, uint256 indexed tokenId, uint8 star, uint8 nftType);
    ...
    function claim(
        ...
        uint256 genes;
        if (nftType == 0) {
            genes = sharkGenes.born(star);
        } else {
            genes = sharkGenes.bornSkin(star);
        }
