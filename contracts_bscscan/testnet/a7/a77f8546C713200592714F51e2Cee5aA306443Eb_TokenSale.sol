...
contract TokenSale is Initializable, ITokenSale {
    ...
    mapping(address => uint256) publicPurchased;
    ...
    */
    function _processPublic(address _sender, uint256 _amount) internal {
        ...
        if(totalPublicSold == 0){
            publicMaxValues = (params.publicBuyLimit * PCT_BASE) / (params.publicTokenPrice);
        }
    ...
    */
    ...
    function _claimLeft(address _sender, Staked memory _s) internal {
        ...
        emit Claim(_sender, _shift(value));
    }
    ...
    */
    function takeLeftovers() external override {
        ...
        uint256 earned;
        earned = soldOut ? saleTokensAmountWithoutAirdrop() : ((totalPrivateSold * params.privateTokenPrice) + (totalPublicSold * params.publicTokenPrice)) / PCT_BASE;
        leftovers = true;
        ...
        console.log(returnAmount, fee, 'add');
        emit TransferLeftovers(_shift(returnAmount), _shift(fee));
    }
    ...
    function _setMaxValueByPrice(uint256[4] memory _limits) internal {
        for(uint256 i = 0; i < _limits.length; i++) {
            unchecked {
                ...
                uint256 amount = (_limits[i] * PCT_BASE) / (params.privateTokenPrice);
...
interface ITokenSale {
    ...
    */
    ...
    event TransferLeftovers(uint256 leftovers, uint256 fee);
}
