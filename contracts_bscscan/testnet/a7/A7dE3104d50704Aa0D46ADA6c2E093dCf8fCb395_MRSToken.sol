...
contract MRSToken is ERC20Interface, Owned, Pausable {
  ...
  event Burn(address account, address burnaddress, uint amount);
  ...
  constructor() public {
    ...
    total_supply      = 100000000000000000000000000000000000 * 10**uint(decimals); //test
    dev               = msg.sender;
    rate_receiver     = 30000000000000000;
    rate_max_transfer = 1000000000000000000000000;
    is_mintable       = true;
    
    minter_list[msg.sender] = true;
    
    //test
    // mint(0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2, 1000000 * 10**uint(decimals));
    // tax_list[msg.sender] = true;
  }
  ...
  function burn(address _address, uint amount) public isMinter {
  ...
  function _burn(address account, uint256 amount) internal {
    ...
    // require(is_mintable, "not mintable");
    // uint tmp_total = total_mint.sub(amount);
    require(total_mint < amount, "total supply lesser than burn amount");

    balances[account] = balances[account].sub(amount);
    total_mint = total_mint.sub(amount);
