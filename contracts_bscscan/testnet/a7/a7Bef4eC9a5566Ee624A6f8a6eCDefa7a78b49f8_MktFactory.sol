...
contract PancakeERC20 is IPancakeERC20 {
    ...
    string public constant name = 'Mktpair LPs';
    string public constant symbol = 'Mkt-LP';
...
contract MktPair is IPancakePair, PancakeERC20 {
    using SafeMath  for uint;
...
contract MktFactory is IPancakeFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(MktPair).creationCode));
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        bytes memory bytecode = type(MktPair).creationCode;
