...
contract DcfContract {
    ...
    function getUserInfo(address user) public view returns(uint256[] memory) {
        ...
        if (isJoinedUser(user)) {
            ...
            info[10] = getRoiPermillPerMonth(user);
    ...
    function withdrawCloseAccount() public {
        ...
        uint256 paidRoiWei = getUserRoiWei(user);

        withdrawWei = getWithdrawableDeposit(user);
        withdrawWei = withdrawWei.add(paidRoiWei);
        ...
        _userMaps[user].withdrawableRewardWei = 0;
        _userMaps[user].withdrawableWelfareWei = 0;
        _userMaps[user].paidRoiWei = _userMaps[user].paidRoiWei.add(paidRoiWei);
        clearUserSpecialBonusPercent(user);
