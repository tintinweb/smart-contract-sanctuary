...
contract HeroToTokenPool is Ownable {
    address private heroAddress;
    bool private isOpen = true;
    mapping(uint256 => uint256) stakeRecord;
    event Stake(address indexed owner,uint256 indexed heroId,uint256 indexed poolIndex);
    event Withdraw(address indexed owner,uint256 indexed heroId,uint256 indexed poolIndex);

    constructor(address _heroAddress){
        heroAddress = _heroAddress;
    }

    function stake(uint256 heroId,uint256 poolIndex) public {
        require(isOpen,"can not stake now");
        require(IERC721(heroAddress).ownerOf(heroId) == msg.sender,"the hero is not onwer");
        uint256 status = IERC721(heroAddress).getStatus(heroId);
        require(status == 0,"the hero is busy");
        IERC721(heroAddress).updateStatus(heroId,2);
        stakeRecord[heroId] = poolIndex;
        emit Stake(msg.sender,heroId,poolIndex);
    }

    function withdraw(uint256 heroId) public {
        require(IERC721(heroAddress).ownerOf(heroId) == msg.sender,"the hero is not onwer");
        require(stakeRecord[heroId]!=0,"the hero is not stake in this pool");
        IERC721(heroAddress).updateStatus(heroId,0);
        uint256 poolIndex = stakeRecord[heroId];
        delete stakeRecord[heroId];
        emit Withdraw(msg.sender,heroId,poolIndex);
    }
    ...
    function getStakePoolIndex(uint256 heroId) public view returns(uint256) {
        return stakeRecord[heroId];
    }
}