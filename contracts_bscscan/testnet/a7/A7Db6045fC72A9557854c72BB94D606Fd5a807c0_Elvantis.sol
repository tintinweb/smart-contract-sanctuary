...
contract Elvantis is Context, Ownable, ERC20 {
    ...
    function _transfer(
        ...
        if(!taxless[sender] && !taxless[recipient]) {
            ...
            if (burnFee != 0) {
                uint256 _burnFee = amount * burnFee / 10000;
    ...
    function setFeeReciever(IFeeReceiver _feeRecipient) onlyOwner external {
...
contract ElvantisFeeReceiver is Ownable, IFeeReceiver {
    ...
    constructor(address _elvantis, address _router, address _owner) {
        require(address(_elvantis) != address(0) && address(_router) != address(0), "ElvantisFeeReceiver: zero address");
    ...
    function onFeeReceived(address token, uint256 amount) payable external override onlyElvantis {
