...
//        updateRewardDebt(_pid);
    ...
    function updateRewardDebt(uint256 _pid) internal {
        ...
        PoolInfo storage pool = mowaPoolInfoNFT[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        uint256 amount = user.hashRate.mul(pool.accMowaPerShare.div(pool.totalHashRate)).add(user.rewardLockedUp);
...
//        payOrLockupPendingMowa(_pid);
    ...
    function payOrLockupPendingMowa(uint256 _pid) internal {
        ...
        if (user.hashRate == 0 || block.number >= pool.endBlock) {
            ...
            if(amount < user.rewardDebt){
                amount = amount.mul(1e12).sub(user.rewardDebt);
                pending = amount.div(1e12);
            } else{
                pending = amount.sub(user.rewardDebt);
            }
