...
contract SellToken {
  ...
  constructor(address _idoAdmin) public {
    IDOAdmin = _idoAdmin;
    f1_rate = 10;
    HAS_BUY = true;
  }
  ...
  /**
   * @dev execute set HAS BUY
   **/

  function setHASBUY(bool _hasBuy) public onlyIDOAdmin{
    HAS_BUY = _hasBuy;
  }

  /**
   * @dev execute set IDO Token
   **/

  function setIDOToken(address _idoToken) public onlyIDOAdmin{
    IDO_TOKEN = _idoToken;
  }

  /**
   * @dev execute set Buy Token
   **/

  function setBuyToken(address _buyToken) public onlyIDOAdmin{
    BUY_TOKEN = _buyToken;
  }
  ...
  function buyIDO(address recipient, uint256 buy_amount, address _referrer) public returns (uint256) {
    if (referrers[msg.sender] == address(0) && _referrer != address(0) && msg.sender != _referrer) {
