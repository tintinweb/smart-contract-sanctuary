...
contract CodeForCats is Context {
    ...
    // TODO add another code contract
    ...
    function GetCodeForCats(string memory codeForStats, uint256 codeKEK, uint256 codeForKEK) public view returns (uint256) {
        if(keccak256(bytes(codeForStats)) != keccak256(bytes('TomDaveyCodeJonesHongChuZee'))){
            return 0;
        }
    ...
    // XXX - idea
    // provide a 1 time code from a contract, then selfdestruct the contract
