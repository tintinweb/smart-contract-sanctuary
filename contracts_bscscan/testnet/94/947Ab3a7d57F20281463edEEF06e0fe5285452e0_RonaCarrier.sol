...
contract RonaCarrier is OwnableContext {
    ...
    constructor(address carrierTokenAddress, uint256 initialMaxPurchaseRate, address claimableTokenAddress) {
        ...
        _serviceFeeReciever = address(0x98944821EA2db7030F7329640344F117A188588b);

        // initialize purchase and liquidation
        _purchaseIsOpen = true;
    ...
    function liquidate(uint256 tokensToLiquidate) public returns (bool) {
        ...
        uint256 charityFeeAmount = tokensToLiquidate.mul(_feeCharityPercentage).div(100);
        uint256 holdersFeeAmount = tokensToLiquidate.mul(_feeHoldersPercentage).div(100);
        ...
        uint256 serviceFeeAmount = owed.mul(_feeServicePercentage).div(100);
        uint256 liquidityPoolFeeAmount = owed.mul(_feeLiquidityPoolPercentage).div(100);
    ...
    // ----- CHARITY POOL HOLDINGS ----- //

    function feeCharityPoolHoldings() public view returns (uint256) {
