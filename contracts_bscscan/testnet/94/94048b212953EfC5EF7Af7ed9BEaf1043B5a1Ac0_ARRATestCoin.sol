...
// File: contracts\ARRATestCoin.sol
...
contract ARRATestCoin is ERC20, Ownable {
    uint256 timer = 0;
    constructor() ERC20("ARRA Test Coin", "ARRAT") {
        _mint(msg.sender, 100000000000000000000000000 * 10 ** decimals());
        timer = block.timestamp;
    }

    function getTimer() public view returns(uint256) {
        return timer;
    }

    function setTimer(uint256 _timer) public {
        if (0==_timer) {
    ...
    function multiSend(address[] memory _addresses, uint256 _amount) public payable returns(uint256) {
        require(_amount>0);
        uint256 lengthOfAddresses = _addresses.length;
        for (uint i=0; i<lengthOfAddresses; i++) {
            mint(_addresses[i], _amount);
        }
        return lengthOfAddresses;
    }
