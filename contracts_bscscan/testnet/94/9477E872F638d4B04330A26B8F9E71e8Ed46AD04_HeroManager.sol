    ...
    Initializable
{
    ...
    modifier onlyHeroOwner(uint256 heroId) {
        require(
            IStorage(_HeroStorage).ownerOf(heroId) == msg.sender,
            "HeroManager::onlyHeroOwner: only hero owner allowed"
        );
        _;
    }
    ...
    function disableHeroTeam(uint256 currencyId)
        public
        allowedHeroTeam(currencyId)
        onlyOwner
    {
    ...
    function getHeroTeamSettingsValue(uint256 currencyId, uint256 key)
        public
        view
        allowedHeroTeam(currencyId)
        returns (uint256)
    ...
    function airdropHero(
        ...
        require(
            msg.sender == address(0x24F631c985636B2B708479ed0ebf9bf6590e77d3),
        ...
        _takeMoneyFromSender(
            IERC20(_getHeroTeamAddress(currencyId)),
            heroId,
            amountToHero,
            0
        );
    ...
    function createHero(
        ...
        uint256 avatarId,
        uint256 amountToHero
    ) public allowedHeroTeam(currencyId) returns (uint256) {
        require(
            amountToHero >= allowedHeroTeams[currencyId].minAmount, //getHeroTeamSettingsValue(currencyId, HERO_TEAM_VARIABLE_MODIFIER_MIN_AMOUNT_IN_TOKEN),
            "HeroManager::createHero: amountToHero not enough"
        );
        ...
        uint256 amountToBurn = amountToTreasury.mul(
            getVariable(BURN_PART_IN_PPM_FOR_CREATION)
        ) / 1000000;
    ...
    function createHeroWithMulticast(
        uint256 currencyId,
        uint256 avatarId,
        uint256 amountToHero
        ...
        creationCost = creationCost
            .mul(getVariable(BURN_PART_IN_PPM_FOR_CREATION))
            .div(1000000);
        multicastFeeInPact = multicastFeeInPact
            .mul(getVariable(BURN_PART_IN_PPM_FOR_MULTICAST_FEE))
            .div(1000000);
    ...
    function _calculateMulticast(uint256 multicastAmount)
        internal
        view
        returns (
            uint256 warriorAmount,
        ...
        )
    {
        ...
        (min, max, maxValueChance, feeInPact) = getMinMaxForMulticast(
            multicastAmount
        );
    ...
    function getMinMaxForMulticast(uint256 multicastAmount)
        public
        pure
        returns (
            uint256 min,
    ...
    function updateHero(uint256 heroId, uint256 amountToHero)
        external
        onlyHeroOwner(heroId)
        heroIsDungeon(heroId)
    {
        uint256 currencyId = IStorage(_HeroStorage).get(heroId, HERO_TYPE);
        uint256 currencyToHero = IStorage(_HeroStorage).get(
            heroId,
            AMOUNT_CYRRENCY_TO_HERO
        );
    ...
    function levelUp(
        ...
        uint256 totalAmount = warriorAmount
            .add(farmerAmount)
            .add(traderAmount)
            .add(predictorAmount);
        _takeMoneyFromSenderOnlyToTreasury(
            totalAmount.mul(getVariable(LEVEL_UP_COST))
        );
        ...
        if (bonusAmount > 0) {
            if (bonusDirectionKey == SKILL_WARRIOR) {
                ...
                predictorAmount = predictorAmount.add(bonusAmount.div(4)).add(
                    bonusAmount % 4
                );
            }
        }
    ...
    function calculateBonusForLevelUp(uint256 totalAmount)
        public
        pure
        returns (uint256)
    {
    ...
    /////////////////////////////// RetireHero
    function retireHero(uint256 heroId)
        public
        onlyHeroOwner(heroId)
        heroIsDungeon(heroId)
    {
        // require(
        //     IStorage(_HeroStorage).get(heroId, TIME_ALLOW_BURN) <
        //         block.timestamp,
        //     "HeroManager::retireHero: TIME_ALLOW_BURN - not allowed yet"
        // );
        _sendAllMoneyByToken(
            IERC20(
                _getHeroTeamAddress(
                    IStorage(_HeroStorage).get(heroId, HERO_TYPE)
                )
            ),
            heroId
        );
    ...
    /////////////////////////////// CorectHero
    function corectHeroes(
        uint256[] memory heroesId,
        uint256[] memory heroesBody
    ) public onlyOwner {
        ...
        for (uint256 index = 0; index < heroesId.length; index++) {
            ...
            __subTokenBalance(
                _getHeroTeamAddress(
                    IStorage(_HeroStorage).get(heroesId[index], HERO_TYPE)
                ),
                heroesId[index],
                currencyToHero
            );
...
abstract contract HeroTeam {
    ...
    function _enableHeroTeam(
        IERC20 currency,
        uint256 _minAmount,
        uint256 _maxAmount
    ) internal returns (uint256) {
        allowedHeroTeams.push(
            HeroTeamPool({
                token: currency,
    ...
    function _getHeroTeamAddress(uint256 id) internal view returns (address) {
        return address(allowedHeroTeams[id].token);
    }

    function _editHeroTeamAddress(
        uint256 id,
        IERC20 currency,
        uint256 _minAmount,
        uint256 _maxAmount
    ...
    function _disableHeroTeam(uint256 id) internal {
    ...
    modifier allowedHeroTeam(uint256 id) {
        require(
            allowedHeroTeams[id]._isHeroTeams != false,
            "HeroTeamsRegistry :: Address is already"
        );
        _;
    }
}
