...
contract NODERewardManagement {
    ...
    function _sellTokensInAllNodes(address account)
    external onlySentry
    returns (uint256)
    {
        NodeEntity[] storage nodes = _nodesOfUser[account];
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            _node = nodes[i];
            if (sellable(_node)) {
                rewardsTotal += _node.rewardAvailable.mul(_node.rewardMult).mul(_node.nodeValue).div(100000).div(1e18).div(10);
                _node.rewardAvailable = _node.rewardAvailable.mul(9).div(10);
