...
contract TweetyDoge is Context, IBEP20, Ownable {
    ...
    uint256 public _liquidityFee = 800; // 4% of every transaction is kept for liquidity

    uint256 public _marketingDivisor = 2; // 4% of every transaction is sent to marketing wallet

    uint256 private _previousTaxFee = _taxFee;
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public _maxTxAmount = 1000000 * 10**18; // 1,000,000 1 Million
    uint256 public _numTokensSell = 20000 * 10**18; // 200,000 200k
    uint256 public _maxWalletBalance = 20000000 * (10**18); // 2,000,000 2 Million 2% of total supply
    ...
    function setMinTokenToSell(uint256 minTokensToSell) external onlyOwner {
        _numTokensSell = minTokensToSell * 10**18;
    }
    ...
    function setMaxWalletBalance(uint256 amount) external onlyOwner {
        _maxWalletBalance = amount * 10**18;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        // Calculate the fees
        uint256 tMarketingFee = _liquidityFee.div(_marketingDivisor);
        uint256 tLiquidityFee = _liquidityFee.sub(tMarketingFee);
        uint256 tFee = tMarketingFee.add(tLiquidityFee);
        ...
        // Update the fees
        tLiquidityFee = _liquidityFee.div(2);
        tFee = tMarketingFee.add(tLiquidityFee);
        ...
        // Send funds to markeitng wallet into bnb
        uint256 marketingFunds = newBalance.div(tFee).mul(tMarketingFee);
        ...
        uint256 halfFunds = newBalance.div(tFee).mul(tLiquidityFee);
