...
/*
	Website: http://chameleontoken.app
	Telegram: https://t.me/Chameleon
	Donate: 0xEcC075cB2926564568F0b7C9a98ac0DC496817D1
	Contract Presale will be in https://www.pinksale.finance/#/.
    Contract Name: BladeReflections
	Contract Version: 1.04
    ...
    Manual BuyBack Function Defined.
    Deployed on Testnet Under: https://testnet.bscscan.com/address/0xc8Ae659B1029C6e0f6D884D4Fe71cd9fFDdF8225
    Fees cannot be higher than 25% for both buy and sale fees.
...
contract Camo is Context, IERC20, Ownable, LockToken 
{
	...
	uint256 public totalBuyFee = _taxFee.add(_liquidityFee).add(_marketingFee).add(_charityFee).add(_devFee); //12% Total Buy Fee
	...
	uint256 public _saleCharityFee = 0;
    uint256 public _saleDevFee = 4;

    uint256 public totalSaleFee = _saleTaxFee.add(_saleLiquidityFee).add(_saleMarketingFee).add(_saleCharityFee).add(_saleDevFee); //13% Total Sale Fee
    

    address payable public marketingWallet =  payable(0xcbFA1ce0b8bFb9C09E713162771C31F176fB1ADE);
	address payable public charityWallet =  payable(0xcbFA1ce0b8bFb9C09E713162771C31F176fB1ADE);
	address payable public devWallet =  payable(0xcbFA1ce0b8bFb9C09E713162771C31F176fB1ADE);
	address public PinkSaleAddr =  0xcbFA1ce0b8bFb9C09E713162771C31F176fB1ADE; //Define PInksale Presale //Prod  0x4650C6f8A56B1bC8Db534BD0Cb5feD505398b89c
    ...
    uint256 public _maxSaleAmount = _tTotal; //Total Supply
    uint256 public _maxBuyAmount = _tTotal; //Total Supply
    uint256 private numTokensSellToAddToLiquidity = _tTotal.div(100).mul(1);
    uint256 public maxLimit = maxLimit = _tTotal; //max allowed tokens in one wallet. 
    ...
    constructor () 
    {
        ...
        _isExcludedFromWhale[charityWallet]=true;
        _isExcludedFromWhale[devWallet]=true;
        _isExcludedFromWhale[PinkSaleAddr]=true;
    ...
    function setAllBuyFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 charityFee, uint256 devFee) public onlyOwner() 
    {
        _taxFee = taxFee;
        ...
        uint256 totalBuyFees = _taxFee.add(_liquidityFee).add(_marketingFee).add(_charityFee).add(_devFee);
        require(totalBuyFees <= 25, "Total fee is over 25%");
	
    }

    function setAllSaleFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 charityFee, uint256 devFee) public onlyOwner() 
    {
        _saleTaxFee = taxFee;
        ...
        _saleDevFee = devFee;
        uint256 totalSaleFees = _saleTaxFee.add(_saleLiquidityFee).add(_saleMarketingFee).add(_saleCharityFee).add(_saleDevFee);
    ...
    {  
        return _amount.mul(_liquidityFee.add(_marketingFee).add(_charityFee).add(_devFee)).div(100);
    }
    ...
    {
        uint256 allFee = _liquidityFee.add(_marketingFee).add(_charityFee).add(_devFee);
        ...
        charityWallet.transfer(newBalance.div(allFee).mul(_charityFee));
    ...
    {
        charityWallet = payable(newWallet);
    }
	...
	function setPinkSaleAddr(address newPinkSale) external onlyOwner() 
    {
        PinkSaleAddr = newPinkSale;
    }	
