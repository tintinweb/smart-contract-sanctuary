...
contract ElemonInfo is Ownable {
    ...
    function setBodyParts(uint256 tokenId, uint256[6] memory bodyParts) external onlyOperator {
    ...
    function setQuality(uint256 tokenId, uint256 quality) external onlyOperator {
        _tokenInfos[tokenId].quality = quality;
        emit ElemonQualityUpdated(tokenId, quality);
    }

    function setClass(uint256 tokenId, uint256 class) external onlyOperator {
        _tokenInfos[tokenId].class = class;
        emit ElemonClassUpdated(tokenId, class);
    }

    function setStar(uint256 tokenId, uint256 star) external onlyOperator {
        Info storage info = _tokenInfos[tokenId];
        info.star = star;
