...
contract TKDIVI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public proccessDividendTime;
    ...
    constructor() public ERC20("DiviPlan", "DVPL") {
        uint256 _tokenRewardsFee = 8;
        uint256 _liquidityFee = 3;
        uint256 _marketFee = 2;
        uint256 _charityFree = 1;
        uint256 _buyBackFee = 3;
        uint256 _dividendTime = 3600;

        tokenRewardsFee = _tokenRewardsFee;
        liquidityFee = _liquidityFee;
        marketFee = _marketFee;
        charityFee = _charityFree;
        buyBackFee = _buyBackFee;
        dividendTime = _dividendTime;
        totalFees = _tokenRewardsFee.add(_liquidityFee).add(_marketFee).add(_charityFree).add(_buyBackFee);
    ...
    // added
    function restoreFees() internal onlyOwner {
    ...
    function updateFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _charityFree, uint256 _buyBackFee2aqQ) external onlyOwner {
        tokenRewardsFee = _tokenRewardsFee;
        liquidityFee = _liquidityFee;
        marketFee = _marketFee;
        charityFee = _charityFree;
        buyBackFee = _buyBackFee2aqQ;
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketFee).add(charityFee).add(buyBackFee);
    }
    //
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "TKDIVI: The dividend tracker already has that address");

        TKDIVIDividendTracker newDividendTracker = TKDIVIDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TKDIVI: The new dividend tracker must be owned by the TKDIVI token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "TKDIVI: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "TKDIVI: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "TKDIVI: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TKDIVI: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TKDIVI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TKDIVI: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(to != address(0), "ERC20: transfer to the zero address");
            ...
            tokenRewardsFee = 8;
            liquidityFee = 3;
            totalFees = tokenRewardsFee.add(marketFee).add(charityFee).add(buyBackFee).add(liquidityFee);
        }
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "TKDIVI: This account cannot send tokens until trading is enabled");
        }
        ...
        bool canDividend = block.timestamp >= proccessDividendTime;

        if(!swapping && canDividend) {
            uint256 gas = gasForProcessing;
            proccessDividendTime += dividendTime;
...

{
  ...
  "evmVersion": "istanbul",
  "libraries": {
    "/Users/fly/Desktop/dev/safeFlight/contracts/diviToken.sol": {
      "IterableMapping": "0x1e9FBEE40E6E4632055E18649Df9A9A0a6fC956B"
    }
  },
  "outputSelection": {
    "*": {
