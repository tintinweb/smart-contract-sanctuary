...
contract TradingPool is Ownable{
    ...
    uint256 constant MAX_BLOCK = 270000; //10 days

    event Deposit(address indexed user, address indexed pair, uint256 amount, address indexed to);
    ...
    function pendingReward(address _pair, address _user) public view returns (uint256 pending) {
        ...
        uint256 _newTotalTradingValue = pool.totalTradingValue - pool.totalTradingValue * min(MAX_BLOCK, block.number - pool.lastRewardBlock) / MAX_BLOCK;
        uint256 supply = pool.totalTradingValue - (pool.totalTradingValue - _newTotalTradingValue) / 2;
        ...
        uint256 _newAmount = user.amount - user.amount * min(MAX_BLOCK, block.number - user.lastDepositBlock) / MAX_BLOCK;
        uint256 _delta = (user.amount - _newAmount) / 2;
        uint256 _amount = user.amount - _delta;
        int256 _rewardDebt = user.rewardDebt.sub(int256(_delta * _accRewardPerShare));
        pending = int256(_amount.mul(_accRewardPerShare) / ACC_PAN_PRECISION).sub(_rewardDebt).toUInt256();
    }
    ...
    function enter(address account, address input, address output, uint256 amount) public onlySwapRouter returns(bool) {
        ...
        if (addedPairs[pair]) {
            ...
            uint256 _currentAmount = user.amount - user.amount * min(MAX_BLOCK, block.number - user.lastDepositBlock) / MAX_BLOCK;
            uint256 _deltaAmount = (user.amount - _currentAmount) / 2;
            ...
            // Effects
            uint256 _amount = IOracle(pool.oracle).consult() * amount;
            user.amount = user.amount.add(_amount);
            user.rewardDebt = user.rewardDebt.add(int256(_amount.mul(pool.accRewardPerShare) / ACC_PAN_PRECISION));

            poolInfo[pair].totalTradingValue += _amount;
            user.lastDepositBlock = block.number;
            emit Deposit(msg.sender, pair, _amount, account);
            return true;
        }
        return false;
    }
    ...
    function harvest(address pair, address to) public {
        if (addedPairs[pair]) {
            ...
            uint256 _currentAmount = user.amount - user.amount * min(MAX_BLOCK, block.number - user.lastDepositBlock) / MAX_BLOCK;
            poolInfo[pair].totalTradingValue -= _currentAmount;
            user.amount = 0;
            user.rewardDebt = 0;
