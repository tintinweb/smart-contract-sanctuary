 ...
 */
contract VNSC is Context, Ownable, ERC165, IERC721Metadata {
    ...
    mapping (address => address[]) public _userFirst;  //用户的直推列表
    mapping (address => address[]) public _userSec;    //用户的间推列表
    ...
    function userFirstList(address user) public view returns(address[] memory) {
        return _userFirst[user];
    }

    function userSecondList(address user) public view returns(address[] memory) {
    ...
    function buyCoin(uint256 numberOfNfts, uint256 coinType, address invitor) public payable {
        ...
        if (invitor != address(0)) {
          ...
          _userFirst[invitor].push(msg.sender);
          _reword[invitor] = _reword[invitor].add(msg.value.mul(2).div(100));
          if (_firstInvitors[invitor] != address(0)) {  //二级邀请者
            _secInvitors[msg.sender] = _firstInvitors[invitor];
            _userSec[_firstInvitors[invitor]].push(msg.sender);
            _reword[_firstInvitors[invitor]] = _reword[_firstInvitors[invitor]].add(msg.value.div(100));
          }
        }

        for (uint i = 0; i < numberOfNfts; i++) {
            uint mintIndex = totalSupply() + 1;
            _safeMint(_msgSender(), mintIndex, coinType);
        }
    }
    ...
    function claim() public returns(uint256){
        ...
        addr.transfer(_reword[msg.sender]);
        return _reword[msg.sender];
    }

    function tokenURI(uint256 tokenId) public view returns (string memory) {
        if (!(totalSupply() == MAX_NFT_SUPPLY || block.timestamp >= revealTimestamp)) {
            return "";
        }
    
        require(_coinTypeMap[tokenId]>0, "can not find coin!");
        if (_coinTypeMap[tokenId] == 3) {
          return coinThird;
        }else if (_coinTypeMap[tokenId] == 2) {
          return coinSec;
        }else {
          return coinMost;
        }
    ...
    /**
     * @dev Gets current Neural Punks Price
     */
    ...
    function userProfit() public view returns (uint256) {
      return _reword[msg.sender];
    }
     ...
     */
    function setRevealTimestamp(uint256 newRevealTimestamp) public onlyOwner returns(bool) {
    ...
    */
     ...
     * Emits a {Transfer} event.
     */
    function _safeMint(address to, uint256 tokenId, uint256 coinType) internal virtual {
        _safeMint(to, tokenId, "", coinType);
    }
     ...
     */
    function _safeMint(address to, uint256 tokenId, bytes memory _data, uint256 coinType) internal virtual {
        _mint(to, tokenId, coinType);
     ...
     * Emits a {Transfer} event.
     */
    function _mint(address to, uint256 tokenId, uint256 coinType) internal virtual {
