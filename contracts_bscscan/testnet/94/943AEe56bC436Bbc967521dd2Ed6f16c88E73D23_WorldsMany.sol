...
interface IHolderRewarderDistributor {
    ...
    function process(address term, uint256 gas) external;
}
...
contract HolderRewarderDistributor is IHolderRewarderDistributor {
      ...
      if (tx_datetime + 5 minutes < blockTimestampLast)
      {
        if (tx_datetime + 10 minutes < blockTimestampLast)
        {
          ...
          if (amount < (longTerm + midTerm + standardTerm))
          {
              ...
              if (remaining == 0)
              {
                removeReward(shareholder, tIndex);
              }
              else
              {
  ...
  function removeReward(address shareholder, uint index) internal {
  ...
  function calculateShareOfShareHolder(address shareholderAddress, bool checkReward, bool claimReward) private returns (uint256, uint256) {
    ...
    for (uint256 i = 0; i < _shareHoldersTxShares[shareholderAddress].length; i++) {
      ...
      uint256 blockTimestampLast = getBlockTimestamp();
      if (txDateTime + 1 minutes < blockTimestampLast) {
        bool rewardCheck = checkReward ? rewardDateTime - 7 days < txDateTime : true;
        if (txDateTime + 5 minutes < blockTimestampLast && rewardCheck)
        {
  ...
  function process(address term, uint256 gas) external {
    ...
    while(iterations < shareholders.length) 
    {
  ...
  function distributeMidDividend(address shareholder, uint256 midTermHolds) internal {
      if (midTermHolds > 0) {
        ...
        BUSD.transferFrom(_midAddress, shareholder, (busdBalance.mul(perc)));
      }
  }
  ...
  */
  ...
  function processRewards() public onlyRewards {
    if (msg.sender == _longTermAddress) {
      BUSD.approve(address(distributor), BUSD.balanceOf(_longTermAddress));
      distributor.process(_longTermAddress, distributorGas);
    }
    else{
      BUSD.approve(address(distributor), BUSD.balanceOf(_midTermAddress));
      distributor.process(_midTermAddress, distributorGas);
    }
    emit ProcessRewards();
  }
