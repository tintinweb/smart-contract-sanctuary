...
contract LFGVesting is ILFGVesting, Ownable, ReentrancyGuard {
    ...
    /**
     *
     * @dev whitelistCount Record the number of white list address.
     *
     */
    ...
    /**
     *
     * @dev vestingPoolCount Record the number of vesting pool.
     *
     */
    uint256 public vestingPoolCount;
     ...
     * @param {uint256} duration in days of the cliff in which tokens will begin to vest
     * @param {uint256} vesting start date
     * @param {uint256} duration in days of the period in which the tokens will vest
     *
    ...
    function claimDistribution() external override nonReentrant returns (bool) {
        ...
        if (releaseAmount > 0) {
            ...
            // Sanity check, the distrubuted amount cannot exceed total amount, otherwise revert the transaction
            require(whitelistPools[msg.sender].distributedAmount <= whitelistPools[msg.sender].lfgAmount,
                "Cannot claim more than the total amount");

            emit ClaimDistribution(msg.sender, releaseAmount);
        } else {
            return false;
        }
    ...
    function calculateReleasableAmount(address _wallet) public view returns (uint256) {
        if (whitelistPools[_wallet].active) {
    ...
    function calculateVestAmount(address _wallet) public view returns (uint256) {
        // If the address is deactivated, then return 0
        if (block.timestamp < vestingPools[whitelistPools[_wallet].vestingOption].cliff ||
            !whitelistPools[_wallet].active) {
            return 0;
        } else if (
            block.timestamp >=
            vestingPools[whitelistPools[_wallet].vestingOption].cliff.add(
            ...
            return
                whitelistPools[_wallet]
                    ...
                    .mul(
                        block.timestamp.sub(
                            vestingPools[whitelistPools[_wallet].vestingOption].cliff
                        )
                    )
    ...
    function decafLists(
        ...
        for (uint256 i = 0; i < _wallet.length; i++) {
            ...
            emit DeactivateWhitelist(_wallet[i]);
        }
...
interface ILFGVesting {
     ...
     *
     */
    event DeactivateWhitelist(address wallet);

    /**
     *
     * @dev this event calls when user claims distribution
     *
     */
