...
function
authorize(address adr) public onlyOwner
...
// File: contracts/BLY_Contract.sol
...
/*
    ReflectFeeCoin template token tokenomics:
        ...
        - 2% fee goes to a "developer address"
*/
...
contract Bailey is Context, IERC20, Ownable {
    ...
    /* Naming */
    string private _name = "BaileyV2";
    string private _symbol = "BLYV2";
    ...
    uint256 public _maxSellTxAmount = _tTotal / (100) * (1); // 1%
    uint256 public _maxWalletToken = _tTotal / (100) * (4); // 4%
    ...
    /* Sell Fees */
    uint256 public _taxSellFee = 4;
    uint256 public _previousTaxSellFee = _taxSellFee;

    uint256 public _liqSellFee = 4;
    uint256 public _previousLiqSellFee = _liqSellFee;

    uint256 public _marketingSellFee = 1;
    uint256 public _previousMarketingSellFee = _marketingSellFee;

    uint256 public _developerSellFee = 2;
    uint256 public _previousDeveloperSellFee = _developerSellFee;
...
/*
    address Dev_address = 0x0d27f6D821aF54C6e85eAd4b733a879BCe576753;
    address Marketing_address = 0xEe5243074420EE6550da4543555388eCfF68E72B;
    address payable _developerAddress = payable(Dev_address);
    address payable _marketingAddress = payable(Marketing_address);
*/
    /* PancakeSwap */
...
// Avoid airdropped from ADD LP before launch     
        if(!tradingOpen && recipient == uniswapV2Pair && sender == uniswapV2Pair)
...
// Transfer Amount checks            
        ...
        if (recipient != owner()  && recipient != address(DEAD) && recipient != uniswapV2Pair && recipient != _marketingAddress && recipient != _developerAddress){
        ...
        if (takeFee && sender != uniswapV2Pair && !takeFeesMutexLock) 
        {
            //if Buy
            ...
            uint256 totalFee = _developerSellFee.add(_marketingSellFee).add(_liqSellFee);
            uint256 totalFeeAmount = amount.mul(totalFee).div(100);
            uint256 oldAmount = amount;
            amount = amount.sub(totalFeeAmount);
...
if (time_since_start < 4 minutes ) {
            ...
            _maxSellTxAmount = _tTotal / (1000) * (2); 
            _maxWalletToken = _tTotal / (100) * (5);
        } 
        
if (time_since_start < 2 minutes ) { 
            _maxTxAmount = _tTotal / (1000) * (3); 
            _maxSellTxAmount = _tTotal / (10000) * (2); 
            _maxWalletToken = _tTotal / (1000) * (5);
        } 
        
if (time_since_start < 1 minutes ) { 
            _maxTxAmount = _tTotal / (10000) * (3); 
            _maxSellTxAmount = _tTotal / (10000) * (2); 
            _maxWalletToken = _tTotal / (10000) * (5); 
        } 
        
else { 
            _maxTxAmount = _tTotal / (100) * (3); 
            _maxSellTxAmount = _tTotal / (100) * (2); 
            _maxWalletToken = _tTotal / (100) * (5); 
        } 
    }
...
//sellFees
    function calculateTaxSellFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_taxSellFee).div(100);
    }

    function calculateLiquiditySellFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liqSellFee).div(100);
    }

    function calculateMarketingSellFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_marketingSellFee).div(100);
    }

    function calculateDeveloperSellFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_developerSellFee).div(100);
    }
    ...
    function setCanTransferBeforeLaunch(address holder, bool exempt) external 
    {
    ...
    {
        autoLimits = false;
        _maxSellTxAmount = _tTotal / (1000) * (maxSellTXPercentage_base1000);
    }
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        ...
        function setMinTxPercent(uint256 minTxPercent) external onlyOwner {
        _minTxAmount = _tTotal.mul(minTxPercent).div(
            10**2
        );
    }
...
//sell
    function setTaxSellFeePercent(uint256 taxSellFee) external onlyOwner {
        _previousTaxSellFee = _taxSellFee;
        _taxSellFee = taxSellFee;
    }

    function setLiqSellFeePercent(uint256 liqSellFee) external onlyOwner {
        _previousLiqSellFee = _liqSellFee;
        _liqSellFee = liqSellFee;
    }

    function setDeveloperSellFeePercent(uint256 developerSellFee) external onlyOwner {
        _previousDeveloperSellFee = _developerSellFee;
        _developerSellFee = developerSellFee;
    }

    function setMarketingSellFeePercent(uint256 marketingSellFee) external onlyOwner {
        _previousMarketingSellFee = _marketingSellFee;
