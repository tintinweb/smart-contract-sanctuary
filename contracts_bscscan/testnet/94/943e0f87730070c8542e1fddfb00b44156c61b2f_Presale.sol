...
contract Presale is Ownable, Whitelisted, ReentrancyGuard {
    ...
    uint256 public novaStage = 600; //1800 = 30 minutes for nova/snova holders to buy first
    uint256 public lockPeriod; //time to lock liquidity
    ...
    function swap() public payable onlyWhitelisted nonReentrant{
        uint256 quota = token.balanceOf(address(this)) * (100 - liqPercent) / 100;
    ...
    function claim() external onlyWhitelisted nonReentrant {
        uint256 quota = token.balanceOf(address(this)) * liqPercent / 100;
    ...
    function addLiquidity () external onlyOwner nonReentrant {
        ...
        uint256 bnbLiquidity = totalSold / swapRate * liqPercent / 100;
   ...
   function withdrawBNB() public nonReentrant{
