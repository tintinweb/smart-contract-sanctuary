...
contract GatewayPool is OwnableUpgradeable, ReentrancyGuard {
    ...
    mapping(address => uint256) public adminCaps;
    ...
    uint256 public inFee;  // rate amount (10000)
    uint256 public outFee; // fixed amount
    ...
    function initialize(address _token, address _feeAddress, uint256 _inFee, uint256 _outFee, uint256 _mininumOutAmount, uint256 _mininumInAmount) public initializer {
    ...
    modifier checkMinterCap(uint256 _amount) {
        require(adminCaps[msg.sender] >= _amount, "!minter");
        _;
    }
    ...
    function setInFeeRate(uint256 _inFee) external onlyOwner {
        inFee = _inFee;
    }

    function setOutFee(uint256 _outFee) external onlyOwner {
        outFee = _outFee;
    }
    ...
    function outNetwork(
        uint256 _networkId,
        address _receiver
    ) public payable whenNotPaused notContract nonReentrant {
        require(msg.value >= mininumOutAmount, "_amount less than mininumOutAmount");
        require(msg.value >= outFee, "_amount less than fee");
        // move fee
        payable(feeAddress).transfer(outFee);
        uint256 _amount = msg.value - outFee;
    ...
    function inNetwork(
        ...
        bytes32 _nHash,
        bytes memory _sig
    ) public payable whenNotPaused checkMinterCap(msg.value) notContract nonReentrant {
        require(msg.value >= mininumInAmount, "_amount less than mininumInAmount");
        // Verify signature
        bytes32 signMessage = hashForSignature(_networkId, _txHash, msg.value, _receiver, _nHash);
        require(status[signMessage] == false, "Gateway: nonce hash already spent");
        bytes32 ethSignMessageHash = getEthSignedMessageHash(signMessage);
        require(verifySignature(ethSignMessageHash, _sig), "Gateway: invalid sig");

        status[signMessage] = true;
        adminCaps[msg.sender] = adminCaps[msg.sender].sub(msg.value);
        uint256 transferAmount = msg.value;
        uint256 fee = (msg.value * inFee) / 100000;
        require(msg.value >= fee, "not enough fee");
        // move fee
        payable(feeAddress).transfer(fee);
        emit InNetwork(_networkId, _txHash, _receiver, msg.value);
    }
}