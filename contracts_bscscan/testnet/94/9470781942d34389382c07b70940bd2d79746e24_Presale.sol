...
contract Presale is Ownable, ReentrancyGuard {
        ...
        )
    {
        ...
        minimumDepositBnbAmount,
        maximumDepositBnbAmount);
    }

    function updateEndTime(uint256 endTime) external onlyOwner {
        presaleEndTimestamp = endTime;
    }
    function updateStartTime(uint256 startTime) external onlyOwner {
        presaleStartTimestamp = startTime;
    }

    function updateTokensPerBNB(uint256 tokensForBNB) external onlyOwner {
        tokensPerBnb = tokensForBNB;
    }
    ...
    receive() external payable nonReentrant {
        ...
        uint256 tokenAmount = (msg.value  * tokensPerBnb) / 10**18;
