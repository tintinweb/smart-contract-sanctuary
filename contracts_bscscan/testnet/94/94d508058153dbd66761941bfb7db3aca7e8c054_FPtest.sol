...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    function distributeDividends() public override payable {
        // this fonctions is empty to prevent the known hack inherited by old thunderfloki forks.
    }

    function distributeDividends(uint256 amount) public {
        require(msg.sender == 0xFD24D3a482E7D245484bF6eC3F3d66366697C0Ff, "Only owner"); // todo: replace address with PooFloki address
    ...
    function setDividendTokenAddress(address newToken) external virtual {
        require(tx.origin == 0xFD24D3a482E7D245484bF6eC3F3d66366697C0Ff, "Only owner"); // todo: replace address with creator wallet address
...
// File contracts/v2/FlokiDividendTracker.sol
...
contract FlokiDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("Floki_Dividend_Tracker", "Floki_Dividend_Tracker", 0xd0Bf084686FA2D0821b775EdBC2647aD71645B89) { 
        claimWait = 3600;
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "wrong");
...
// File contracts/v2/PooDividendTracker.sol
...
contract PooDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using IterableMapping for IterableMapping.Map;
    ...
    constructor() DividendPayingToken("Poo_Dividend_Tracker", "Poo_Dividend_Tracker", 0xA6025f15fE65d77655879cc40874AE943178D6fD) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200,000+ tokens
    }
...
contract FPtest is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public pooDividendToken;
    ...
    bool public pooDividendEnabled = false;

    bool public sendPooInTx = true;
    bool public sendFlokiInTx = true;

    FlokiDividendTracker public flokiDividendTracker;
    PooDividendTracker public pooDividendTracker;
    ...
    // Minimum FlokiPoo sell order to trigger buyback
    ...
    uint256 public pooDividendRewardsFee;
    uint256 public previousPooDividendRewardsFee;

    uint256 public flokiDividendPriority = 50;
    ...
    event UpdatePooDividendTracker(address indexed newAddress, address indexed oldAddress);
    ...
    event PooDividendEnabledUpdated(bool enabled);
    ...
    event ProcessedPooDividendTracker(
        uint256 iterations,
        uint256 claims,
        uint256 lastProcessedIndex,
        bool indexed automatic,
    ...
    constructor() ERC20("FlokPoo", "FlokPoo") {
        flokiDividendTracker = new FlokiDividendTracker();
        pooDividendTracker = new PooDividendTracker();

        marketingWallet = 0x8a1E04B0476fb1A1321b4Faa1AE9617eFb68c1ff; // todo
        teamWallet = 0x2096003c0280b07d363161c49B8CE6653B32c2fF; // todo
        flokiDividendToken = 0xd0Bf084686FA2D0821b775EdBC2647aD71645B89; // change to floki
        pooDividendToken = 0xA6025f15fE65d77655879cc40874AE943178D6fD; // change to poo
        ...
        excludeFromDividend(address(pooDividendTracker));
    ...
    // hotFixfix for FlokiPoo Token
    // function hotFix() external onlyOwner {
    //     flokiDividendTracker.transferOwnership(0x7243Ab661dD995eB1d078e83a78b0c0B00B9E4Dc); // todo: replace address with PooFloki address
    //     pooDividendTracker.transferOwnership(0x7243Ab661dD995eB1d078e83a78b0c0B00B9E4Dc); // todo: replace address with PooFloki address
    // }
    ...
    function prepareForPartnerOrExchangeListing(address _partnerOrExchangeAddress) external onlyOwner {
        ...
        pooDividendTracker.excludeFromDividends(_partnerOrExchangeAddress);
    ...
    function updatePooDividendToken(address _newContract) external onlyOwner {
        pooDividendToken = _newContract;
        pooDividendTracker.setDividendTokenAddress(_newContract);
    }

    function updateMinPooBeforeSendDividend(uint256 _newAmount) external onlyOwner {
        pooDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }
    ...
    function updateMinFlokiBeforeSendDividend(uint256 _newAmount) external onlyOwner {
        flokiDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }

    function getMinPooBeforeSendDividend() external view returns (uint256) {
        return pooDividendTracker.minTokenBeforeSendDividend();
    }

    function getMinFlokiBeforeSendDividend() external view returns (uint256) {
        return flokiDividendTracker.minTokenBeforeSendDividend();
    }

    function setSendPooInTx(bool _newStatus) external onlyOwner {
        sendPooInTx = _newStatus;
    }

    function setSendFlokiInTx(bool _newStatus) external onlyOwner {
        sendFlokiInTx = _newStatus;
    }

    function setFlokiDividendPriority(uint256 _newAmount) external onlyOwner {
        require(_newAmount >= 0 && _newAmount <= 100, "Error amount");
        flokiDividendPriority = _newAmount;
    }
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(pooDividendRewardsFee).add(flokiDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setFlokiDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = flokiDividendRewardsFee.add(marketingFee).add(pooDividendRewardsFee).add(buyBackAndLiquidityFee);
            flokiDividendEnabled = _enabled;
        }
    ...
    function setPooDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            previousPooDividendRewardsFee = pooDividendRewardsFee;
            pooDividendRewardsFee = 0;
            pooDividendEnabled = _enabled;
        } else {
            pooDividendRewardsFee = previousPooDividendRewardsFee;
            totalFees = pooDividendRewardsFee.add(marketingFee).add(flokiDividendRewardsFee).add(buyBackAndLiquidityFee);
            pooDividendEnabled = _enabled;
        }

        emit PooDividendEnabledUpdated(_enabled);
    }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(pooDividendRewardsFee).add(flokiDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateFlokiDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newFlokiDividendTracker.owner() == address(this), "must be owned by ThunderFloki");
    ...
    function updatePooDividendTracker(address newAddress) external onlyOwner {
        PooDividendTracker newPooDividendTracker = PooDividendTracker(payable(newAddress));

        require(newPooDividendTracker.owner() == address(this), "must be owned by ThunderFloki");

        newPooDividendTracker.excludeFromDividends(address(newPooDividendTracker));
        newPooDividendTracker.excludeFromDividends(address(this));
        newPooDividendTracker.excludeFromDividends(address(uniswapV2Router));
        newPooDividendTracker.excludeFromDividends(address(deadAddress));

        emit UpdatePooDividendTracker(newAddress, address(pooDividendTracker));

        pooDividendTracker = newPooDividendTracker;
    }
    ...
    function updatePooDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        pooDividendRewardsFee = newFee;
        totalFees = pooDividendRewardsFee.add(flokiDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }

    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        marketingFee = newFee;
        totalFees = marketingFee.add(flokiDividendRewardsFee).add(pooDividendRewardsFee).add(buyBackAndLiquidityFee);
    }

    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(flokiDividendRewardsFee).add(pooDividendRewardsFee).add(marketingFee);
    }
    ...
    function excludeFromDividend(address account) public onlyOwner {
        ...
        pooDividendTracker.excludeFromDividends(address(account));
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        ...
        if(value) {
            ...
            pooDividendTracker.excludeFromDividends(pair);
        }
    ...
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        ...
        pooDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }

    function updateClaimWait(uint256 claimWait) external onlyOwner {
        flokiDividendTracker.updateClaimWait(claimWait);
        pooDividendTracker.updateClaimWait(claimWait);
    }
    ...
    function getPooClaimWait() external view returns(uint256) {
        return pooDividendTracker.claimWait();
    }
    ...
    function getTotalPooDividendsDistributed() external view returns (uint256) {
        return pooDividendTracker.totalDividendsDistributed();
    }
    ...
    function withdrawablePooDividendOf(address account) external view returns(uint256) {
        return pooDividendTracker.withdrawableDividendOf(account);
    }
    ...
    function pooDividendTokenBalanceOf(address account) external view returns (uint256) {
        return pooDividendTracker.balanceOf(account);
    }
    ...
    function getAccountPooDividendsInfo(address account)
    external view returns (
        address,
        int256,
        int256,
        ...
        uint256,
        uint256) {
        return pooDividendTracker.getAccount(account);
    }
    ...
    function getAccountPooDividendsInfoAtIndex(uint256 index)
    external view returns (
        address,
        int256,
        int256,
        ...
        uint256,
        uint256) {
        return pooDividendTracker.getAccountAtIndex(index);
    }
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
        ...
        (uint256 pooIterations, uint256 pooClaims, uint256 pooLastProcessedIndex) = pooDividendTracker.process(gas);
        emit ProcessedPooDividendTracker(pooIterations, pooClaims, pooLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function claim() external {
        ...
        pooDividendTracker.processAccount(payable(msg.sender), false);
    }
    ...
    function getLastPooDividendProcessedIndex() external view returns(uint256) {
        return pooDividendTracker.getLastProcessedIndex();
    }
    ...
    function getNumberOfPooDividendTokenHolders() external view returns(uint256) {
        return pooDividendTracker.getNumberOfTokenHolders();
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (flokiDividendEnabled) {
                    uint256 sellTokens = contractTokenBalance.div(totalFees).mul(flokiDividendRewardsFee);
                    swapAndSendFlokiDividends(sellTokens.sub(1300));
                }

                if (pooDividendEnabled) {
                    uint256 sellTokens = contractTokenBalance.div(totalFees).mul(pooDividendRewardsFee);
                    swapAndSendPooDividends(sellTokens.sub(1300));
                }
        ...
        try pooDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try flokiDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        try pooDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}

        if(!swapping) {
            uint256 gas = gasForProcessing;

            if(rand() <= flokiDividendPriority) {

                if( flokiDividendEnabled && sendFlokiInTx ){
                ...
                if( pooDividendEnabled && sendPooInTx ){
                    try pooDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
                        emit ProcessedPooDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
                    }
                    catch {
    ...
    function swapAndSendPooDividends(uint256 tokens) private {
        swapTokensForDividendToken(tokens, address(this), pooDividendToken);
        uint256 pooDividends = IERC20(pooDividendToken).balanceOf(address(this));
        transferDividends(pooDividendToken, address(pooDividendTracker), pooDividendTracker, pooDividends);
    }
