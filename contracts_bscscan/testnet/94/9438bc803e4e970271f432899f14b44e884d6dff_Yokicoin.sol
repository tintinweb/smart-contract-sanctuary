...
contract Yokicoin is owned {
    ...
    function _transfer(address _from, address _to, uint _value) internal {
        ...
        //checking conditions
        require (_to != address(0), "Invalid address");         // Prevent transfer to 0x0 address.
    ...
    uint256 public yokiPriceInBUSD = 5000;     // 5000 = 5 BUSD  (1 yoki = 5 BUSD)
    ...
    event PriceSet(uint256 price, address owner);
    event BUSDSet(address owener, address token);
    ...
    function buyTokens(uint256 yokiAmount) external {
        ...
        uint256 busdAmount = yokiAmount * yokiPriceInBUSD / 1000;
    ...
    /** 
     * All users buying and selling tokens using BUSD
     * yoki price must be in 3 decimals. Meaning  5 BUSD must be 5000 BUSD
     */
    function setPrices(uint256 _yokiPriceInBUSD) onlyOwner external {
        //no need for input validation as price can be 0 to stop buying/selling.
        yokiPriceInBUSD = _yokiPriceInBUSD;   //sellPrice is 1 Yoki Token = ?? BUSD Tokens
        emit PriceSet(_yokiPriceInBUSD, msg.sender);
    }
    ...
    function setBUSDwallet(address _busdWallet) onlyOwner external{
        ...
        emit BUSDSet(msg.sender, _busdWallet);
    }
