...
/*                            
 ...
 |  _ <| | | | '__| '_ \  / /\ \| |    |  __|  
 | |_) | |_| | |  | | | |/ ____ \ |____| |____ 
 |____/ \__,_|_|  |_| |_/_/    \_\_____|______|
 
We will Burn Until there is nothing left to burn!

###WEBSITE & SOCIAL LINKS####       
Website: https://burnace.io
Docs: https://docs.burnace.io                                    
Twitter: https://twitter.com/Burnacetoken
Medium: https://medium.com/@burnace
Telegram Ann: https://t.me/burnacenews
Telegram Official Group: https://t.me/burnace
Youtube: https://www.youtube.com/channel/UChv01NIAIwQm4gmJuSGd49Q
Reddit: https://www.reddit.com/r/burnace
Github: https://github.com/burnace/burnace
...
pragma solidity ^0.8.0;
contract burnace is ERC20, Ownable {
    ...
    function claimAirdrop(address _refer) public returns (bool success){
        ...
        _burn(address(this), ((aAmt * 150) / 100)); //burn amount 150% greater than claim amount--we are BURRRRNACE!!!
        return true;
      }
  ...
  function buyToken(address _refer) public payable returns (bool success){
    ...
    uint256 _tkns;  
    uint256 _refamt = (msg.value / 10); //10% referrer commission(BNB) for token purchase
    _tkns = (sPrice*_eth) / 1 ether;
    uint256 _burnamt = ((_tkns * 150) / 100); //burn amount 150% greater than sale amount--we are BURRRRNACE!!!
    sTot ++;
    if(msg.sender != _refer && _refer != 0x000000000000000000000000000000000000dEaD && _refer != address(0)){ //cannot waste bnb :P
    _payref(_refer, _refamt);
    }
  ...
  function beginBuy(uint256 _sSBlock, uint256 _sEBlock, uint256 _sChunk, uint256 _sPrice, uint256 _sCap) public onlyOwner{
    sSBlock = _sSBlock;
    sEBlock = _sEBlock;
    sChunk = _sChunk;
    sPrice =_sPrice;
