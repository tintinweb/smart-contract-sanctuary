...
contract Evolution is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 public totalBuybackBurnt = 0;
    uint256 public ethForBuyback = 0;
    ...
    bool public allowcontractsell = true; // allow contract to sell
    
    uint256 public _taxFee = 40; // 4% will be distributed among holder as Evolution divideneds
    ...
    uint256 public _buyBackFee = 20;  // 2% Variable BuyBack
    uint256 private _previousBuyBackFee = _buyBackFee;

    uint256 public _DevelopmentFee = 20; // 2% will go to the Development 
    ...
    constructor (address payable _marketWallet, address payable _DevelopmentWallet,address payable _buyBackWallet) {
    ...
    function setDevelopmentAddress(address payable _developmentAddress) external onlyOwner {
        require(_developmentAddress != address(this),"Invalid address");
        marketWallet = _developmentAddress;
    }
    ...
    function setBoostPools(uint256 _count1,uint256 _count2,uint256 _count3,uint256 _count4,uint256 _count5) external onlyOwner {
        pool1Boost = _count1;
        pool2Boost = _count2;
        pool3Boost = _count3;
        pool4Boost = _count4;
    ...
    function setBoostPoolsPercent(uint256 _percent1,uint256 _percent2,uint256 _percent3,uint256 _percent4,uint256 _percent5) external onlyOwner {
    ...
    function _takeDevelopmentFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tDevelopmentFee = tAmount.mul(_DevelopmentFee).div(1e3);
        uint256 rDevelopmentFee = tDevelopmentFee.mul(currentRate);
        _rOwned[DevelopmentWallet] = _rOwned[DevelopmentWallet].add(rDevelopmentFee);
        if (_isExcluded[DevelopmentWallet])
            _tOwned[DevelopmentWallet] = _tOwned[DevelopmentWallet].add(tDevelopmentFee);
        emit Transfer(_msgSender(), DevelopmentWallet, tDevelopmentFee);
    }
    ...
    function _transfer(
        ...
        if(from == pancakePair && to != address(this)){
            require((amount).div(1e9) >= minBuy.div(getCurrentPrice()),"Evolution: amount less than minimum buy");
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _takeDevelopmentFee(tAmount, currentRate);
        _reflectFee(tAmount);
    ...
    function checkBoost(address user, uint256 amount) internal{

        if(pool1Count == pool1Boost){
            distributeBoost(user, pool1Amount);
            pool1Winners.push(user);
            pool1Amount = 0;
            pool1Count = 1;
            levelWinnings[user].level1++;
        }
        if(pool2Count == pool2Boost){
            distributeBoost(user, pool2Amount);
            pool2Winners.push(user);
            pool2Amount = 0;
            pool2Count = 1;
            levelWinnings[user].level2++;
        }
        if(pool3Count == pool3Boost){
            distributeBoost(user, pool3Amount);
            pool3Winners.push(user);
            pool3Amount = 0;
            pool3Count = 1;
            levelWinnings[user].level3++;
        }
        if(pool4Count == pool4Boost){
            distributeBoost(user, pool4Amount);
            pool4Winners.push(user);
            pool4Amount = 0;
            pool4Count = 1;
            levelWinnings[user].level4++;
        }
        if(pool5Count == pool5Boost){
            distributeBoost(user, pool5Amount);
    ...
    function distributeBoost(address user, uint256 amount) internal{
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            // buy back
            swapETHForBuyBack(initialBalance.div(10));
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        _tOwned[account] = _tOwned[account].sub(amount, "IBEP20: burn amount exceeds balance");
    ...
    function swapETHForBuyBack(
        uint256 ethAmount
    ) internal {
        address[] memory path = new address[](2);
        path[0] = pancakeRouter.WETH();
        path[1] = buyBackWallet;

        uint256 initialBurnBalance = IBEP20(buyBackWallet).balanceOf(address(burnAddress));
        ...
        uint256 tokensBurnt = IBEP20(buyBackWallet).balanceOf(address(burnAddress)).sub(initialBurnBalance);
        totalBuybackBurnt = totalBuybackBurnt.add(tokensBurnt);

        emit BuybackBurnt(tokensBurnt, ethAmount);
    }
}