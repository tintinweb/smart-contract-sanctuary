...
contract DynamicStakingRewardsWithLock is IStakingRewards, Ownable, Readable {
    ...
    uint immutable public WITHDRAW_TIMEOUT;
    IERC20 immutable public override rewardsToken;
    IERC20 immutable public override stakingToken;
    ...
    constructor(address _owner, IERC20 _rewardsToken, IERC20 _stakingToken, uint _withdrawTimeout) {
        transferOwnership(_owner);
        rewardsToken = _rewardsToken;
        stakingToken = _stakingToken;
        WITHDRAW_TIMEOUT = _withdrawTimeout;
    ...
    function balanceOf(address account) external override view returns (uint) {
        return uint(users[account].stake).sub(availableToWithdraw(account));
    }
    ...
    function earned(address account) public override view returns (uint) {
        return uint(users[account].stake).mul(rewardPerToken().sub(userRewardPerTokenPaid[account])).div(1e18).add(rewards[account]);
    }
    ...
    function stake(uint amount) external override updateReward(msg.sender) {
        ...
        require(_totalSupply <= stakingCap, 'Staking cap reached');
        users[msg.sender].stake = uint(users[msg.sender].stake).add(amount).toUInt88();
    ...
    function requestWithdrawal(uint amount) external {
        ...
        uint available = availableToWithdraw(msg.sender);
        if(available > 0) {
            withdraw(available);
        }
        User storage user = users[msg.sender];
        ...
        require(user.stake >= newRequested, 'Insufficient balance to withdraw');
    ...
    function withdraw() public {
        withdraw(availableToWithdraw(msg.sender));
    }
    ...
    function withdraw(uint amount) public override updateReward(msg.sender) {
        ...
        require(withdrawable >= amount, 'Not enough withdrawable');
        User storage user = users[msg.sender];
        user.stake = uint(user.stake).sub(amount).toUInt88();
        user.requested = uint(user.requested).sub(amount).toUInt88();
    ...
    function exit() external override {
        withdraw(users[msg.sender].stake);
        getReward();
    }

    function setStakingCap(uint newCap) public onlyOwner {
        stakingCap = newCap;
        emit StakingCapSet(newCap);
    }
    ...
    function addRewardAmount(uint reward) public onlyOwner updateReward(address(0)) {
        require(_totalSupply > 0, 'Stake something first');
    ...
    event StakingCapSet(uint newCap);
}

contract BULKStakingRewards is DynamicStakingRewardsWithLock(
    0xE6f9422826DFdd43382d754e4111408242b39355,
    IERC20(0xeEcd13862ce534a04692d013b19C6706Bde76f1C),
    IERC20(0xc55b1FeEdf19C3aA951aD77129AaaC28cFb9D640),
    1 hours) {}
