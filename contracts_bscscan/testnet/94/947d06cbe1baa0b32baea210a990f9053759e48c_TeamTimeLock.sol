...
contract TeamTimeLock {
    ...
    IERC20 public token;
    uint constant public PERIOD = 2592000;
    ...
    /**
     * @notice Withdraw other tokens which are unexpectedly sent to the timelock!
     **/
     
    function withdrawOtherTokensThatAreStuck(address _otherToken) external {
        require(_otherToken != address(token), "Token cannot be KSF");
        uint256 amount = IERC20(_otherToken).balanceOf(address(this));
        IERC20(_otherToken).safeTransfer(beneficiary, amount);
    }
