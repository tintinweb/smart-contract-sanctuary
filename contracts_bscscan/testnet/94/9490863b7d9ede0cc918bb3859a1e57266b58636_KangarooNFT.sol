...
contract KangarooNFT is ERC721URIStorage, Ownable {
    ...
    address KG = 0xEA551129eCa6EA7A7113521895c23Cda7eE78F3f;
    ...
    function openBlindBox() public returns (uint256 types){
        ...
        //钱转钱包地址,不然咋提取?
        ERC20(KG).transferFrom(msg.sender, address(this), boxPrice * 10 ** 8);

        if(isCaptain[recommender]){
            recommendReward[recommender] = boxPrice * captainRewardRate / 100;//????用户如何入金????
        }else{
    ...
    function userWithdrawRecommendReward() public {
        ...
        require(reward > 0,"This user dont hava enough cards");

        ERC20(KG).transfer(msg.sender, reward * 10 ** 8);
