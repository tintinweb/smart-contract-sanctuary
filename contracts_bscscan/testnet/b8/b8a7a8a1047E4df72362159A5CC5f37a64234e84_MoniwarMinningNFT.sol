...
contract MoniwarMinningNFT is Ownable, ReentrancyGuard {
    ...
    function set(
        ...
        if (_remainingReward > 0) {
            mowaPoolInfoNFT[_pid].remainingReward = _remainingReward.mul(1e18);
        }
    ...
    function getEndBlock(uint256 _pid) external view returns (uint256) {
        if(mowaPoolInfoNFT[_pid].endBlock > block.number){
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256)
    {
        ...
        uint256 amount = user.hashRate.mul(accMowaPerShare).div(1e12).sub(user.rewardDebt);
        return amount.add(user.rewardLockedUp);
    }
    ...
    function harvest(uint256 _pid) public nonReentrant {
        ...
        if (user.hashRate == 0 || block.number >= pool.endBlock) {
            ...
            uint256 amount = user.hashRate.mul(pool.accMowaPerShare).div(1e12).sub(user.rewardDebt);
            pending = amount.add(user.rewardLockedUp);
        }
        if (pending > 0) {
