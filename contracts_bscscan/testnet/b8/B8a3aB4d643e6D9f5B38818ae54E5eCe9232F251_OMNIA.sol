...
import "./OMNIABase.sol";
import "./claim/AutoClaimDetails.sol";

// Implements rewards
contract OMNIA is OMNIABase, AutoClaimDetails {
    ...
    constructor()
        ...
        OMNIABase()
    {
        EnumerableSet.add(_addressesExcludedFromFees, OMNIA_REFLECTION_WALLET);
    ...
    function _transfer(
        ...
        address recipient,
        uint256 amount
    ) internal override(Transfers, ERC20) {
        ...
        Transfers._transfer(sender, recipient, amount);
...
abstract contract OMNIABase is Transfers {
    ...
    constructor()
        SwapsManager(
            ...
            0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c, // BTCB: https://www.binance.com/en/blog/421499824684901264/chain/experience-btcb--bitcoin-on-binance-smart-chain
        ...
        )
    {
        ...
        EnumerableSet.add(_addressesExcludedFromFees, DX_SALE_LOCK);

        // Initialize PancakeSwap V2 router and Omnia <-> BNB pair.
        setPancakeSwapRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
    ...
    // If any BEP20 token is sent by error the owner can retrieve it
    // This function excludes OMNIA claiming as:
    // 15% tax in OMNIA > swapped into BNB if neded > autoClaim swap to BNB to coins diretly to users wallets
    function transferAnyBEP20(
        address tokenAddress,
        address to,
        uint256 amount
    ) public onlyOwner {
        require(tokenAddress != address(this), "Cannot withdraw OMNIA");
...
import "./swaps/SwapExecutorDetails.sol";

// Base class that implements: tranfers
abstract contract Transfers is SwapExecutorDetails {
    uint256 public immutable DAILY_SELL_PERCENTAGE = 10**3; // 0.1% = 0.0001 = 10^-3
    ...
    function _transfer(
        ...
        // if (isPancakeSwapPair(sender) && block.timestamp < LISTING_DATE) {
        //     revert("Buying not allowed before September 5th 7:33 CET");
        // }
        // if (isPancakeSwapPair(recipient) && block.timestamp < LISTING_DATE) {
        //     revert("Selling are not allowed before September 5th 7:33 CET");
        // }
...
abstract contract SwapExecutorDetails is SwapExecutor {
        ...
        onlyOwner
    {
        if (isExcluded)
            require(
                EnumerableSet.add(_addressesExcludedFromFees, addr),
                "Fees: excluded"
            );
