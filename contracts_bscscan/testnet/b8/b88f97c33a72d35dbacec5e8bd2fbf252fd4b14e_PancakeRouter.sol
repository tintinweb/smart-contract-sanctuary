  ...
  factory = _factory;
    ...
    /* create Pair*/
     function createInitialPair(
        ...
        address to,
        uint deadline
    ) external override virtual  ensure(deadline) onlyAdmin(msg.sender) returns (uint amountA, uint amountB, uint liquidity) {
        require(IPancakeFactory(factory).getPair(tokenA, tokenB) == address(0)," Pair Already Exist"); 
           IPancakeFactory(factory).createPair(tokenA, tokenB);
    ...
    function addLiquidity(
        ...
        require(IPancakeFactory(factory).getPair(tokenA, tokenB) != address(0),"pair not found");
    ...
    function removeLiquidity(
        ...
        address pair = PancakeLibrary.pairFor(factory, tokenA, tokenB);
        IPancakePair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair for redemption
...
//     }
    function swapTokensForExactTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PancakeLibrary.pairFor(factory, token0, token1), amounts[0]
        );
        _swap(amounts, path, to);
    }
    function swapExactETHForTokens(uint amountOutMin, address token0,
...
function getPairData(address pairaddress)  public returns(address _T0Address,string memory _T0Symbol,uint _T0Decimals,uint _t0Reserves){
...
address  token0=pair.token0();
  ...
  return (token0,t0Symbol,t0Decimal,reserve0);
 }
