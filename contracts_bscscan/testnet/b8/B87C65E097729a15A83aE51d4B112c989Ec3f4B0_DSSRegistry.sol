...
contract DSSRegistry is Ownable,ERC1155Base {
    ...
    uint256 public totalExtenions = 0;
    ...
    function mint(bytes32 mNHash,string memory uri,string memory label,uint256 royalites) public payable {
      ...
      if(mNHash == nHash){
        ...
        totalExtenions = totalExtenions.add(1);
      } else {
    ...
    function _redistribute(uint256 amount,uint256 id) private  {
        ...
        if(registry[registry[id].motherId].owner == this.owner() || registry[id].motherId == 0){
            ...
            for(uint256 i = 0;i<scDevs.length;i++){
              payable(address(scDevs[i])).transfer(partDev);
            ...
            payable(this.owner()).transfer(partOwner);
            total = total.sub(partOwner);
        }
        if(registry[id].motherId > 0 ){
            payable(registry[registry[id].motherId].owner).transfer(total);
        } else {
    ...
    function getTotalSupply() external view returns (uint256){
        return(totalExtenions.add(totalDomains).add(totalSubdomains));
    }
