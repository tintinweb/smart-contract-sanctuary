...
contract BinamonLand is BinamonCollectionV2 {

    constructor () {
        _name = 'Binamon Lands Collection';
        _symbol = 'BLANDC';
        _paramNames = ["Level", "Element", "X", "Y", "Perk1", "Perk2", "Perk3", "Perk4", "Perk5"];
    }

    function getCoordinates(uint256 tokenId) public view returns (uint256, uint256) {
        return (tokens[tokenId].paramValues[2], tokens[tokenId].paramValues[3]);
    }
}
