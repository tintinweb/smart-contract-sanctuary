 ...
 *
 */
contract EPay is IERC20, Context, Ownable, TokenRecover {
    ...
    uint256 public sustainingFee                = 100;
    uint256 public megaPayFee                   = 350;
    // total fees
    ...
    */
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(_enableMegaPay &&  ((!isDividendExempt[recipient] && (amount >= _megaPayMinimumSpend && _isBuy)) || (!isDividendExempt[sender] && (amount >= _megaPayMinimumTransfer && _isTransfer && _balances[sender] >= _megaPayMinimumHODL)))){
    ...
    function getExemptions(address holder) external view returns (bool, bool, bool) {
        return (isFeeExempt[holder], isDividendExempt[holder], isTxLimitExempt[holder]);
    }
    ...
    */
    ...
    function calculatePoolReward(bool _transferring, uint chanceMultiplier) private returns (uint256) {
        ...
        if (poolTokens >= _megaPayThreshold) {
            ...
            _lastRoll = random(); 
            if(_lastRoll <= _cPoolChance.mul(chanceMultiplier)) {
                reward = poolTokens;
            }
        } 
        return reward;
    ...
    function getUserQualifiesForMegaPay(address holder) external view returns (bool) {
        return  _enableMegaPay && !isDividendExempt[holder] && (_balances[holder] >= _megaPayMinimumHODL);
    }
    ...
    function manualSwapBack(bool customRewardsEnabled) external {
        if(shouldSwapBack(customRewardsEnabled)) { 
            swapBack(customRewardsEnabled);
        }
    }
    ...
    function sendToBlackHole(uint256 tokenAmount, bool blackHoleMechanismChoice) external {
        ...
        require(_balances[msg.sender] >= tokenAmount && _balances[msg.sender] >=_megaPayThreshold, 'user does not own enough tokens');
        uint256 amountReceived = !isFeeExempt[msg.sender] ? takeFee(pair, msg.sender, tokenAmount) : tokenAmount;
        //uint256 amountReceived = tokenAmount;
        //uint256 feeAmount = tokenAmount.mul(totalFeeBuys).div(feeDenominator);
        //uint256 amountReceived = tokenAmount.sub(feeAmount);
        _totalSupply = _totalSupply.sub(amountReceived, 'total supply cannot be negative');
        _balances[msg.sender] = _balances[msg.sender].sub(tokenAmount, 'cannot have negative tokens');
        _spaceDustBurned = _spaceDustBurned.add(amountReceived);
        if(blackHoleMechanismChoice)
        {
            ...
            require(tokenAmount >= blackHoleMinimumMP, 'amount not sufficient');
            uint256 poolReward = calculatePoolReward(false, bHchanceMultiplier);
            if (poolReward > 0) {
                _megaPayTransfer(msg.sender, poolReward); 
            }
            if (_buyBonus > 0 && (random() <= _bonusChance.mul(bHchanceMultiplier))) {
                _bonusTransfer(msg.sender, _buyBonus); //Awards bonus pool if won
            }
        ...
        emit SentToBlackHole(amountReceived);
    }
    ...
    event SetBlackHole(uint256 _blackHoleMinimumSD, uint256 _blackHoleMinimumMP, uint _bHchanceMultiplier);
