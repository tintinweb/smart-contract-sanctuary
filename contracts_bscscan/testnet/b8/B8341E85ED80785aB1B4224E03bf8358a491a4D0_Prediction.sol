...
contract Prediction is Ownable, Pausable {
     ...
     */
    function genesisLockRound(address tokenAddress)
        external
        onlyOperator
        whenNotPaused
    {
        ...
        int256 currentPrice = _getPriceFromOracle(tokenAddress);
     ...
     */
    function executeRound(address tokenAddress)
        external
        onlyOperator
        whenNotPaused
    {
    ...
    /**
     * @dev Return rounds for a particular epoch
     */
     ...
     */
    function _getPriceFromOracle(address _oracle) internal returns (int256) {
        oracle = AggregatorV3Interface(_oracle);

        uint256 leastAllowedTimestamp =
            block.timestamp.add(oracleUpdateAllowance);
