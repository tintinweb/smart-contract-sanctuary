...
// File: ..\prj10\contracts\libraries\SafeMath.sol
...
// File: ..\prj10\contracts\interfaces\IBEP20.sol
...
// File: ..\prj10\contracts\interfaces\IMigratorChef.sol
...
// File: ..\prj10\contracts\libraries\Address.sol
...
// File: ..\prj10\contracts\libraries\SafeBEP20.sol
...
// File: ..\prj10\contracts\libraries\Strings.sol
...
// File: ..\prj10\contracts\libraries\ECDSA.sol
...
// File: ..\prj10\contracts\Context.sol
...
// File: ..\prj10\contracts\Ownable.sol
...
// File: ..\prj10\contracts\MasterChef.sol
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accCakePerShare; // Accumulated token per share, times 1e12. See below.
    }
    ...
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount, uint256 rewards);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount, uint256 rewards);
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate) public onlyExecutor {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function set(uint256 _pid, uint256 _allocPoint, bool _withUpdate) public onlyExecutor validatePoolByPid(_pid) {
        require(!poolInfo[_pid].disabled, "pool disabled!");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public validatePoolByPid(_pid) {
        require (_pid < poolInfo.length, 'deposit token by staking');
        require (!poolInfo[_pid].disabled, 'pool disabled!');
        PoolInfo storage pool = poolInfo[_pid];
        ...
        uint256 pending = 0;
        if (user.amount > 0) {
            pending = user.amount.mul(pool.accCakePerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
    ...
    function getPoolList() public view returns(PoolItem[] memory _pools){
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            _pools[pid].pid = pid;
            _pools[pid].disabled = poolInfo[pid].disabled;
            _pools[pid].lpToken = address(poolInfo[pid].lpToken);
            _pools[pid].allocPoint = poolInfo[pid].allocPoint;
            _pools[pid].accCakePerShare = poolInfo[pid].accCakePerShare;
            lpToken = poolInfo[pid].lpToken;
            _pools[pid].stakeAmount = lpToken.balanceOf(address(this));
        }
    }
    
    function setPoolStatus(uint256 _pid, bool _disabled) public onlyExecutor validatePoolByPid(_pid) {
        poolInfo[_pid].disabled = _disabled;
    }
}