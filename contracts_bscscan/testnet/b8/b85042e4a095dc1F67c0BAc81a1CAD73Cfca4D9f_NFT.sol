...
contract NFT is ERC721, Ownable {
  ...
  constructor()  ERC721("BANANAMONKEYFUNDME", "BANANA") {
  	_owner = msg.sender;
  }

  
  function mint(string memory _tokenURI, uint _price, uint256 numOfTokens) public returns(bool) {
    require (numOfTokens <= 40,"Tokens must be less than 40");
    totalSupply().add(numOfTokens);
    for (uint i = 0; i < numOfTokens; i++) {
        uint mintIndex = totalSupply();
        _safeMint(msg.sender, mintIndex);
        price[mintIndex] = _price;
  ...
  function changePrice(uint256 newPrice, uint _id) external {
      require(msg.sender == ownerOf(_id), "Caller must be tokenId owner");
      price[_id] = newPrice;
      sold[_id] = false;
  }
  ...
  function _trade(uint _id) internal {
     payable(ownerOf(_id)).transfer(msg.value); //eth to owner
  	_transfer(ownerOf(_id), msg.sender, _id); //nft to user
