...
// File contracts/ETBurnPoolv2.sol
...
contract ETBurnPoolv2 {
    ...
    mapping(address => EnumerableSet.UintSet) private burnEpoch;
    ...
    function getETHRewards() public view returns (uint256) {
        ...
        uint256 ethRewards;
        for (uint256 i = 0; i < burnEpoch[msg.sender].length(); i++) {
            uint256 epoch = burnEpoch[msg.sender].at(i);
            if (epoch < currentEpoch) {
                Round storage round = rounds[epoch];
                if (round.ethAmount > 0) {
                    ethRewards +=
                        (round.ethAmount * round.userBurnET[msg.sender]) /
                        round.etBurnAmount;
                }
            }
        }
    ...
    function withdrawETH() external {
        uint256 ethRewardsAmount = getETHRewards();
        require(ethRewardsAmount > 0, "You have no ETH to withdraw.");
        uint256 currentEpoch = getCurrentEpoch();
        for (uint256 i = 0; i < burnEpoch[msg.sender].length(); i++) {
            uint256 epoch = burnEpoch[msg.sender].at(i);
            if (epoch < currentEpoch) {
                burnEpoch[msg.sender].remove(epoch);
            }
        }
