...
contract cryptoparty{
    ...
    function buyTicketsBNB(address ref,uint256 ticket) public payable {
        ...
        require(customers[ref].alreadyUser,"refrer must be a customer first");
        ...
        customers[user.refrer].refferdUsers = customers[user.refrer].refferdUsers.add(1);
        }
        user.alreadyUser = true;
        user.refrer = ref;
        customers[user.refrer].refbonusbnb = customers[user.refrer].refbonusbnb.add(msg.value.mul(20).div(100));
        customers[user.refrer].buybackWorthbnb = customers[user.refrer].buybackWorthbnb.add(msg.value.mul(30).div(100));
