...
contract TestingSolidity is Context, IBEP20, Ownable {
    ...
    uint256 public  _liquidityFee = 1; // auto lp
    uint256 public  _marketingAndDevelopmentFee  = 7; // MarketingAndDevelopment wallet
    ...
    function transferForeignToken(address _token, address _to) public onlyOwner returns(bool _sent){
        // with this condition LSE token can not be moved but others junks can be removed to clear the contract.
        // other projects send their junk token for promotion. This function will be used to remove those junks.
        // This function can not remove LSE token from contract.
        require(_token != address(this), "Clear contract from junk tokens.  This tokens are sent to spam this contract.");
    ...
    function prepareForPresale() public onlyOwner {
        ...
        _liquidityFee = 0; // auto lp
        _marketingAndDevelopmentFee  = 0; // MarketingAndDevelopment wallet
        _burnFee      = 0; // burn address
        _totalFees      = _taxFee.add(_liquidityFee).add(_marketingAndDevelopmentFee).add(_burnFee);
    ...
    function activateContractAfterPresale() public onlyOwner {
        ...
        _liquidityFee = 1; // auto lp
        _marketingAndDevelopmentFee  = 7; // MarketingAndDevelopment wallet
        _burnFee      = 1; // burn address
        _totalFees      = _taxFee.add(_liquidityFee).add(_marketingAndDevelopmentFee).add(_burnFee);
    }
