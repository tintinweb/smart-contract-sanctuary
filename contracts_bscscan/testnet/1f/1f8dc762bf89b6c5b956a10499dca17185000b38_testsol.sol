...
/**
 ...
 @WealthyADA proposes an innovative feature in its contract.

 DIVIDEND YIELD PAID IN ADA! Without claim it.
 Simply hold WADA$ and you'll receive ADA token automatically in your wallet.
 
 Hold WADA$ and get rewarded in ADA token on every transaction!

 telegram: https://t.me/WealthyADA
 
 webside: https://WealthyADA.club
...
contract testsol is ERC20, Ownable {
    ...
    bool private marketingEnabled = true;
    ...
    constructor() ERC20("testsol", "testsol$") {
    	firstDividendTracker = new FIRSTDividendTracker();
 
    	marketingWallet = 0x9BD2305C44d54B20fdd0c87E3a8AfeE06A983C76;
    	firstDividendToken = 0x277C2b5B334118615c8fB4483278170FEC527C4A;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "WealthyADA: The marketing wallet is already this address");
    ...
    function updatefirstDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(firstDividendTracker), "WealthyADA: The dividend tracker already has that address");
 
        FIRSTDividendTracker newfirstDividendTracker = FIRSTDividendTracker(payable(newAddress));
 
        require(newfirstDividendTracker.owner() == address(this), "WealthyADA: The new dividend tracker must be owned by the $BSWISS token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "WealthyADA: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "WealthyADA: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "WealthyADA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "WealthyADA: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "WealthyADA: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "WealthyADA: Trading has not started yet");
...
contract FIRSTDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("WealthyADA_Dividend_Tracker", "WealthyADA_Dividend_Tracker", 0x277C2b5B334118615c8fB4483278170FEC527C4A) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1 * (10**9); //must hold 1+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "WealthyADA_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "WealthyADA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main $BSWISS contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "WealthyADA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "WealthyADA_Dividend_Tracker: Cannot update claimWait to same value");
