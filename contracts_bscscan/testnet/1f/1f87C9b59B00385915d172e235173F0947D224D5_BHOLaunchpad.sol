...
contract BHOLaunchpad is Ownable, Pausable, Whitelist, SharedConstants {
     ...
     * amount
     */
    function registration(uint256 _amount) external payable checkWhoCanBuy {
        require(_amount > 0, "Amount must the greater than 0");
        // check start, end time
        require(block.timestamp > launchpadData.startAt, "Launchpad do not start yet");
        // check hard cap
        require(capacity.add(_amount) <= launchpadData.hardCap, "The capacity has exceeded the limit");
        ...
        returns (uint256 amount, uint256 claimAt)
    {
        ...
        uint256 amountOfUser = amountUserStaked.mul(launchpadData.presaleRate).div(10 ** decimalsOfToken);
        ...
        VestingHistory memory history = VestingHistory(index, amountWillVesting, block.timestamp);
