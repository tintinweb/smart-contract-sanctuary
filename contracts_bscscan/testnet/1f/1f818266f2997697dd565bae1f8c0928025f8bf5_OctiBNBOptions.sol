...
contract OctiBNBOptions is Ownable, IOctiOptions {
    ...
    OctiBNBOptions private oldOctiBNBOptions;
    OctiBNBPool public pool;

    constructor(IOctiStakingBNB staking, OctiBNBPool _pool) public {
        pool = _pool;
     ...
     * @param optionID ID of your option
     */
    function exercise(uint256 optionID,uint256 price) external {
        Option storage option = options[optionID];
        ...
        uint256 profit = payProfit(optionID,price);
    ...
    function migrate(uint count) external onlyOwner {
        ...
        require(address(oldOctiBNBOptions) != address(0));
        for (uint i = 0; i < count; i++){
            uint optionID = options.length;
            OctiBNBOptions.Option memory option;
            (
                option.state,
                option.holder,
                option.strike,
                ...
                option.expiration,
                option.optionType
            ) = oldOctiBNBOptions.options(optionID);
    ...
    function setOldOctiBNBOptions(address oldAddr) external onlyOwner {
        require(address(oldOctiBNBOptions) == address(0));
        oldOctiBNBOptions = OctiBNBOptions(oldAddr);
    }
        ...
        )
    {
        ...
        uint256 currentPrice = latestPrice;
     ...
     */
    function payProfit(uint optionID,uint256 latestprice)
        internal
        returns (uint profit)
    {
