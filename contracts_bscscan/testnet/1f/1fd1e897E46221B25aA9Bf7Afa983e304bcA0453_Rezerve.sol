...
contract Rezerve is Context, IERC20, Ownable {
    ...
    uint256 public _liquidityFeeOnSale = 0;
    uint256 private _previousLiquidityFeeOnSale = _liquidityFee;
    ...
    function setSellFeePercent(uint256 liquidityFee) external onlyOwner() {
        _liquidityFeeOnSale = liquidityFee;
    }
    ...
    function calculateLiquiditySaleFee(uint256 _amount) private view returns (uint256) {
          if( action == 2 )
        return _amount.mul(_liquidityFeeOnSale).div(
            10**2
        );
