...
contract MultiPad is IERC20 {
    ...
    uint256 private ReserveAmount = 25000000;
    uint256 private MarketingAmount = 20000000;
    uint256 private Team1Amount = 18000000;
    uint256 private Team2Amount = 2000000;
    uint256 private LiquidityAmount = 10000000;
    uint256 private IDOAmount = 20000000;
    uint256 private PartnersAmount = 10000000;
    ...
    address private Liquidity = 0xf708212AaCEb256bf9933a37c121069AF9890277;
    ...
    */
    ...
    modifier validateClaim(address _userAddress, uint256 _lockNumber)
    {
        ...
        if(block.timestamp > saleEndTime && block.timestamp < saleEndTime +1209600 && _lockNumber == 1){
            require(reEntrance[_userAddress][1] != true, "Insufficient Unlocked Tokens");
        }
        if(block.timestamp > saleEndTime + 1209600 && block.timestamp < saleEndTime +2419200){
            require(reEntrance[_userAddress][2] != true , "Insufficient Unlocked Tokens");
        }
        if(block.timestamp > saleEndTime + 2419200 && block.timestamp < saleEndTime +3628800){
            require(reEntrance[_userAddress][3] != true ,  "Insufficient Unlocked Tokens");
        }
        if(block.timestamp > saleEndTime + 3628800 && block.timestamp < saleEndTime +4838400){
            require(reEntrance[_userAddress][4] != true,  "Insufficient Unlocked Tokens");
        }
        else{
    ...
    function withdrawSpecialLocked(address _toAddress, uint256 _amount) checkLockingRoles(msg.sender, _amount) external returns(bool){
        ...
        require(_amount != 0, "addresses should not be 0");
       ...
       if(_add == Team1){
         ...
         if(_add == Liquidity){
             if(block.timestamp > deploymentTime && block.timestamp < deploymentTime + 2629743){                                                     //One to Two Month
               require(LiquidityAmount.sub(_amountRequested) >= 80000000000000000000000000, "Amount exceded lock 1");
           }
            if(block.timestamp > deploymentTime + 2629743 && block.timestamp < deploymentTime + 5259486){                                            //Two to Three week
               require(LiquidityAmount.sub(_amountRequested) >= 60000000000000000000000000, "Amount exceded lock 2");
           }
            if(block.timestamp > deploymentTime + 5259486 && block.timestamp < deploymentTime + 7889229){                                            //Three to Four week
               require(LiquidityAmount.sub(_amountRequested) >= 40000000000000000000000000, "Amount exceded lock 3");
           }
            if(block.timestamp > deploymentTime + 7889229 && block.timestamp < deploymentTime + 6048000){                                            //Four to Fifth week
               require(LiquidityAmount.sub(_amountRequested) >= 20000000000000000000000000, "Amount exceded lock 4");
           }
         }
        _;
     }
