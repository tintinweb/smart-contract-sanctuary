...
contract MetaSpheres is ERC721Sale{
        //maxTokens = "1024"  maxMint = "25" maxPresaleMint = "2" price = "90000000000000000"; // 0.09 ETH 
        constructor() ERC721Sale("METASPHERES_TESTNET", "MSP", 1024,  25, 2, 90000000000000000) {
        _pause();        
        }
...
contract ERC721Sale is ERC721Pausable, Ownable { 
    ...
    uint256 public  _maxPresaleMint;
    ...
    mapping (address => User) private _whitelist;
    ...
    constructor(string memory name, string memory ticker, uint256 maxTokens, uint256  maxMint, uint256 maxPresaleMint, uint256 price) ERC721(name, ticker) {
        _maxTokens = maxTokens; 
        _maxMint = maxMint;
        _maxPresaleMint = maxPresaleMint;
        _price = price;
    }
    ...
    modifier isWhitelisted() {
        require(_whitelist[msg.sender].isWhitelist, "Only whitelist");
        _;
    }

    function addToWhitelist(address[] memory addrs) public onlyOwner {
        for (uint256 i = 0; i < addrs.length; i++) {
            User storage user = _whitelist[addrs[i]];
    ...
    function presaleMintItems(uint256 amount) isWhitelisted() public payable {
        User storage user = _whitelist[msg.sender];
        require(amount <= _maxPresaleMint, "maximum unit  mint for pre-sale address");
        require(user.presaleMints + amount <= _maxPresaleMint, "maximum unit  mint for pre-sale address");
        require(_presaleActive, "pre-sale closed");
        _sale(amount);
