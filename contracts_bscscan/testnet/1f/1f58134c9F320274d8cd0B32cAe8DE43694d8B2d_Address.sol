...
contract MasterChef is Ownable {
    ...
    // Deposited amount DRG in MasterChef
    uint256 public depositedDRG;
    ...
    function pendingDRG(uint256 _pid, address _user) external view returns (uint256){
        ...
        if (_pid == 0){
            lpSupply = depositedDRG;
        }
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 DRGReward = multiplier.mul(DRGPerBlock).mul(pool.allocPoint).div(totalAllocPoint).mul(stakingPercent).div(percentDec);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit DRG by staking');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw DRG by unstaking');
        ...
        // Stake DRG tokens to MasterChef
    function enterStaking(uint256 _amount) public {
        ...
        if(_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            user.amount = user.amount.add(_amount);
            depositedDRG = depositedDRG.add(_amount);
        }
        user.rewardDebt = user.amount.mul(pool.accDRGPerShare).div(1e12);
        emit Deposit(msg.sender, 0, _amount);
    }

    // Withdraw DRG tokens from STAKING.
    function leaveStaking(uint256 _amount) public {
        ...
        if(_amount > 0) {
            user.amount = user.amount.sub(_amount);
            pool.lpToken.safeTransfer(address(msg.sender), _amount);
            depositedDRG = depositedDRG.sub(_amount);
        }
        user.rewardDebt = user.amount.mul(pool.accDRGPerShare).div(1e12);
    ...
    function updateDRGPerBlock(uint256 newAmount) public onlyOwner {
        require(newAmount <= 30 * 1e18, 'Max per block 30 DRG');
        require(newAmount >= 1 * 1e18, 'Min per block 1 DRG');
