...
contract DinolandNFTUpgradeable is ERC721Upgradeable, DinosAccessControl {
    ...
    string private dnlStorageURI;
    // @dev Dino's information data type, default is json
    string private dnlStorageExtenstion = "json";
    // @dev Ready to breed period
    uint256 private readyToBreedPeriod = 7 days;
    ...
    function initialize() public initializer {
        ...
        readyToBreedPeriod = 7 days;
    }

    /// @dev An internal helper method to remove dino id from an array.
    function removeDinoFromArray(uint256 _dinoId, uint256[] memory _array)
        public
        pure
        returns (uint256[] memory)
    {
        ...
        for (; i < _array.length; i++) {
            if (_array[i] == _dinoId) {
                ...
                uint256[] memory newArray = new uint[]( _array.length - 1);
    ...
    /// @dev An exterval method that allow CLevel to set readyToBreedPeriod
    /// @param _readyToBreedPeriod - New Ready to breed period
    function setReadyToBreedPeriod(uint256 _readyToBreedPeriod) external onlyCLevel{
        readyToBreedPeriod = _readyToBreedPeriod;
    }
    ...
    function transferFrom(
        ...
        userOwnedDinos[_from] = removeDinoFromArray(
            _dinoId,
    ...
    function createDino(
        ...
        require(_genes >= 0, "Dino genes is not valid");
        require(_owner != address(0), "Address is not valid");
        require(_gender == 1 || _gender == 2, "Gender should be 1 or 2");
        require(_generation > 0, "Generation number should be greater than 0");
        uint256 dinoId = _createDino(
            _genes,
            _owner,
            block.timestamp + readyToBreedPeriod,
            _gender,
            _generation
        );
    ...
    function retireDino(uint256 _dinoId, bool _rip) external onlySpawner {
        if (_rip) {
            ...
            userOwnedDinos[ownerOf(_dinoId)] = removeDinoFromArray(
                _dinoId,
    ...
    function evolveDino(uint256 _dinoId, uint256 _newGenes)
        ...
        onlySpawner
    {
        require(_newGenes >= 0, "Dino genes is not valid");
