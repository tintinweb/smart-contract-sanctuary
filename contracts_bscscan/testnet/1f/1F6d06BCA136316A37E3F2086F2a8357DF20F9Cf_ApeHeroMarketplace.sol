...
contract ApeHeroMarketplace is ReentrancyGuard, Pausable, IERC721Receiver, Ownable, AccessControlEnumerable {
        ...
        for (uint i = 0; i < collCount; i++) {
            ...
            if (_tokens.length > 0) {
                for (uint j = 0; j < _tokens.length; j++) {
                    _tEarned = _rHoldersBalance - _rHolderClaimedRewardsByCollection[_royaltiesCollections.at(i)][_tokens[j]];
                    if (_tEarned > 0) {
                        _allEarned += _tEarned;
                    }
                }
            ...
            if (_listed.length > 0) {
                for (uint k = 0; k < _listed.length; k++) {
                    _lEarned = _rHoldersBalance - _rHolderClaimedRewardsByCollection[_royaltiesCollections.at(i)][_listed[k].id];
                    if (_lEarned > 0) {
                        _allEarned += _lEarned;
                    }
                }
        ...
        for (uint i = 0; i < collCount; i++) {
            ...
            if (_listed.length > 0) {
                for (uint k = 0; k < _listed.length; k++) {
                    ...
                    if (_lEarned > 0) {
                        _allEarned += _lEarned;
                        _rHolderClaimedRewardsByCollection[_royaltiesCollections.at(i)][_listed[k].id] += _tEarned;
                    }
                }
            }
        }
    ...
    function addCollection(address collection, uint supply, uint rHolders, uint rMinters, uint rTeam, bool earnRoyalties) public onlyOwner {
        ...
        _allCollections[collection] = Collection(collection, true, 0, 0, 0, 0, supply, rHolders, rMinters, rTeam, earnRoyalties);
    ...
    function listItem(address collection, uint tokenId, uint price) public payable nonReentrant whenNotPaused {
        ...
        if ((_allCollections[collection].floor == 0) || (price < _allCollections[collection].floor)) {
            _allCollections[collection].prevFloor = _allCollections[collection].floor;
            _allCollections[collection].floor = price;
        }
    ...
    function delistItem(address collection, uint tokenId) public payable nonReentrant whenNotPaused {
        ...
        // update collection data
        if (_item.price <= _allCollections[collection].floor) {
            _allCollections[collection].floor = _allCollections[collection].prevFloor;
        }
        _allCollections[collection].items -= 1;
    ...
    function buyItem(address collection, uint tokenId) public payable nonReentrant whenNotPaused {
        ...
        _rHoldersBalance += _sellShare.holders / _totalEarningHoldersRewards();
