...
contract KeanuBSC is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public councilWalletAddress = payable (0x4C2B43eb496b812E3B750f07F4279D74798b5126); //Team Fee
    address payable public treasuryWalletAddress = payable (0x41eB4256Ca05cA0d424ED163f54a152f6c2b73Ef); //Marketing Fee
    ...
    address public _councilAddress;
    ...
    constructor () public {
        ...
        _totalTaxPercent = _taxFee.add(_liquidityFee).add(_councilFee).add(_treasuryFee);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tOtherFee = tLiquidity.add(tCouncil).add(tTreasury);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        tTransferAmount = tTransferAmount.sub(tCouncil).sub(tTreasury);
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tCouncil, uint256 tTreasury, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rBurn = tCouncil.mul(currentRate);
    ...
    function _takeCouncil(uint256 tCouncil) private {
        uint256 currentRate =  _getRate();
        uint256 rCouncil = tCouncil.mul(currentRate);
        _rOwned[_councilAddress] = _rOwned[_councilAddress].add(rCouncil);
        if(_isExcluded[_councilAddress])
            _tOwned[_councilAddress] = _tOwned[_councilAddress].add(tCouncil);
    }
    ...
    function calculateCouncilFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_councilFee).div(
            10**2
        );
    }    
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 forLiquidity = contractTokenBalance.div(_totalTaxPercent).mul(_liquidityFee);
        uint256 forWallets = contractTokenBalance.sub(forLiquidity);
        ...
        if(forWallets > 0 && _councilFee.add(_treasuryFee) > 0)
        {
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(forWallets);
            uint256 newBalance = address(this).balance.sub(initialBalance);
    
            uint256 marketingShare = newBalance.div(_councilFee.add(_treasuryFee)).mul(_councilFee);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        uint256 tCouncil = tOtherFee.mul(_councilFee).div(otherFee);
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
       ...
       function _getOtherFee() private view returns (uint256) {
        uint256  otherFee = _previousLiquidityFee.add(_previousCouncilFee).add(_previousTreasuryFee);
        return otherFee;
    }
