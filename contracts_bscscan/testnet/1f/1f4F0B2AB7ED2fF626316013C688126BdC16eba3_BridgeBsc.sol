...
contract BridgeBase is Ownable{
    ...
    constructor(address tk_addr) public {
        tendie = ITendie(tk_addr);
        admin = msg.sender;
    }
    ...
    function get_fee(uint amount) public returns(uint) {
        return amount.div(1000);
    } 
    function lock(address to, uint amount) public {
        tendie.transferFrom(msg.sender, address(this), amount);
        emit Transfer(msg.sender, to, amount, block.timestamp, nonce, Step.Burn);
        nonce++;
    }
    function unlock(address to, uint amount, uint otherChainNonce) public {
        require(msg.sender == admin, 'BridgeBase: only admin can mint');
        require(processedNonce[otherChainNonce] == false, 'transfer already processed');
        processedNonce[otherChainNonce] = true;
        tendie.transferFrom(address(this), to, amount);
        uint fee = get_fee(amount);
        emit Transfer(msg.sender, to, amount, block.timestamp, otherChainNonce, Step.Mint);
    }
    function approve(address spender, uint amount) public returns(bool) {
        tendie.approve(spender, amount);
        return true;
    }
}
