...
contract FarmUser is FarmHashrate, IFarmUser, Initializable, Ownable {
    ...
    // 初始化
    function init(IAppData _appData, address _usdtToken) public onlyOwner {
        appData = _appData;

        usdtToken = _usdtToken;
        usdtDecimals = ERC20(usdtToken).decimals();
    ...
    function upgradeLevel(uint8 levelNo) public override needInit {
        ...
        uint256 usdtRebateAmount = SafeMath.div(SafeMath.mul(deltaAmount, appData.getUsdtRebateRate()), 100);
        uint256 totalUsdtCommission = 0;
        uint256 totalHashrateCommission = 0;
        Model.User storage genUser = userMap[user.inviterAddr];
        ...
        for (uint8 gen = 1; gen <= 20; gen++) {
            ...
            if (genUser.levelNo > 0 && hc.usdtRebate == 1) {
                uint256 usdtCommission = SafeMath.div(SafeMath.mul(usdtRebateAmount, commissionRate), 100);  
                IERC20(usdtToken).safeTransferFrom(user.addr, genUser.addr, usdtCommission);      
                totalUsdtCommission = SafeMath.add(totalUsdtCommission, usdtCommission);  
            }
            ...
            if (hc.rebate == 1) {
                ...
                recordUserHashrate(Model.CATEGORY_LEVEL, genUser.addr, levelHashrateMap[genUser.addr]);
                totalHashrateCommission = SafeMath.add(totalHashrateCommission, hashrateCommission);
            }
            
            genUser = userMap[genUser.inviterAddr];
        }

        totalHashrate = SafeMath.add(totalHashrate, totalHashrateCommission);

        // 记录算力
        recordHashrate(Model.CATEGORY_LEVEL, user.addr, levelHashrateMap[user.addr], totalHashrate);

        IERC20(usdtToken).safeTransferFrom(user.addr, appData.getCoolAddr(), SafeMath.sub(deltaAmount, totalUsdtCommission));
    ...
    function setUserLevel(address userAddr, uint8 levelNo) public onlyOwner {
        ...
        // 更新算力
        uint256 oldHashrate = levelHashrateMap[userAddr];
        levelHashrateMap[userAddr] = SafeMath.div(level.price, (10 ** 18));
        totalHashrate = SafeMath.add(SafeMath.sub(totalHashrate, oldHashrate), levelHashrateMap[userAddr]);

        // 记录算力
        recordHashrate(Model. CATEGORY_LEVEL, userAddr, levelHashrateMap[userAddr], totalHashrate);
    }
    ...
    function setUserHashrate(address userAddr, uint256 hashrate) public onlyOwner {
        // 更新算力
        uint256 oldHashrate = levelHashrateMap[userAddr];
        levelHashrateMap[userAddr] = hashrate;
        totalHashrate = SafeMath.add(SafeMath.sub(totalHashrate, oldHashrate), levelHashrateMap[userAddr]);
...
abstract contract FarmHashrate is IFarmHashrate {
    ...
    function recordHashrate(uint8 category, address userAddr, uint256 userTotalHashrate, uint256 networkHashrate) internal {
    ...
    function recordUserHashrate(uint8 category, address userAddr, uint256 userTotalHashrate) internal {
        userHashrateRecords[userAddr].push(Model.HashrateRecord({
            category: category,
            blockNumber: block.number,
            timestamp: block.timestamp,
