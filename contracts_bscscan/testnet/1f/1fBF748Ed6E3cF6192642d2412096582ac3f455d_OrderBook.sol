...
contract OrderBook is Initializable, Ownable, ReentrancyGuard, IOrderBook {
  ...
  function makeOrder(uint8 position, uint256 price, uint256 securityAmt, uint256 currencyAmt) external override {
    ...
    // mapping(address => mapping(address => uint256)) memory tokenTransferTo;
  ...
  function viewOrderNode(uint8 position) external view override returns (OrderNodeView[50] memory) {
    OrderNodeView[50] memory orderNodeViews;
    ...
    for (uint16 i = 0; i < _viewCount; i++) {
      ...
      (uint256 balance, uint256 capacity) = __getOrderBalance(currAddr);
      orderNodeViews[i] = OrderNodeView({
        nodeAddr: currNode.nodeAddr,
        position: currNode.position,
        price: currNode.price,
        // to next orderNode by using orderNodeMap
        next: currNode.next,
        // to head orderItem
        headItem: currNode.headItem,
        // to last orderItem, fast append node
        lastItem: currNode.lastItem,
        itemCount: currNode.itemCount,
        balance: balance,
        capacity: capacity
      });
      currAddr = currNode.next;
    }
    return orderNodeViews;
  }

  function viewOrderItem(address orderNodeAddr) external view override returns (OrderItemView[50] memory) {
    require(orderNodeAddr != address(0), "view address invalid");

    OrderItemView[50] memory orderItemViews;
    ...
    for (uint16 i = 0; i < _viewCount; i++) {
      ...
      orderItemViews[i] = OrderItemView({
  ...
  function __bookOrder(
    ...
    if (inNode) {
      ...
      // book order records
      __makeTradeOrder(currItemAddr, position, securityAmt, currencyAmt, 1);
      ...
      // book order records
      __makeTradeOrder(currNode.headItem, position, securityAmt, currencyAmt, 1);
  ...
  function __fillOrder(uint8 position, uint256 price, uint256 securityAmt, uint256 currencyAmt) private {
    ...
    while(currNodeAddr != address(0) && (securityLeft > 0 || currencyLeft > 0)) {
      // use storage but not memory because need to sync item status
      OrderNode storage currNode = _orderNodeMap[currNodeAddr];
      ...
      emit OrderNodeFilled(position, securityLeft, currencyLeft, currNodeAddr, currNode.itemCount);
  ...
  function __fillMatchedOrder(uint8 position, uint256 securityAmt, uint256 currencyAmt, address orderNodeOpAddr) private returns (uint256 securityLeft, uint256 currencyLeft) {
    ...
    while(currItemAddr != address(0) && (securityLeft > 0 || currencyLeft > 0)) {
      ...
      if (
        ...
        __makeTradeOrder(currItemAddr, position, securityLeft, currencyLeft, 2);
        securityLeft = 0;
        currencyLeft = 0;
        break;
      } else {
        ...
        if (position == 1) {
          __makeTradeOrder(currItemAddr, position, 0, currencyInItem, 2);
        } else if (position == 2) {
          __makeTradeOrder(currItemAddr, position, securityInItem, 0, 2);
        }
  ...
  function __makeTradeOrder(address tradeOrderAddr, uint8 position, uint256 securityAmt, uint256 currencyAmt, uint8 orderType) private {
    ...
    ITradeOrderV2 tradeOrder = ITradeOrderV2(tradeOrderAddr);
  ...
  function __getOrderBalance(address orderNodeAddr) internal view returns (uint256 balance, uint256 capacity) {
    require(orderNodeAddr != address(0), "view address invalid");
...
interface IOrderBook {
  ...
  event OrderNodeFilled(uint8 position, uint256 securityLeft, uint256 currencyLeft, address orderNodeAddr, uint16 nodeItemLeft);
  ...
  function viewOrderNode(uint8 position) external view returns (OrderNodeView[50] memory);
  function viewOrderItem(address orderNodeAddr) external view returns (OrderItemView[50] memory);
