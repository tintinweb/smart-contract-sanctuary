...
contract Exchange is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function calcLoanAmountAndExchangeRate(
        ...
        if (collateralAddress == address(0)) {
            (, uint256 rateMul) = SafeMathUpgradeable.tryMul(uint256(RateBNBwithUSD()), 10**10);
            (, uint256 ltvAmount) = SafeMathUpgradeable.tryMul(loanToValue, amount);
            (, uint256 coll) = SafeMathUpgradeable.tryMul(rateMul, ltvAmount);
            (, uint256 collToUSD) = SafeMathUpgradeable.tryDiv(coll, 100);

            // collateralToUSD = (uint256(RateBNBwithUSD()) * 10**10 * loanToValue * amount) / 100;
            collateralToUSD = collToUSD;
        } else {
            (, uint256 rateMul) = SafeMathUpgradeable.tryMul(uint256(getLatesPriceToUSD(collateralAddress)), 10**10);
            (, uint256 ltvAmount) = SafeMathUpgradeable.tryMul(loanToValue, amount);
            (, uint256 coll) = SafeMathUpgradeable.tryMul(rateMul, ltvAmount);
            (, uint256 collToUSD) = SafeMathUpgradeable.tryDiv(coll, 100);

            // collateralToUSD = (uint256(getLatesPriceToUSD(collateralAddress)) * 10**10 * loanToValue * amount) / 100;
            collateralToUSD = collToUSD;
        }

        if (loanAsset == address(0)) {
            (, uint256 rateLA) = SafeMathUpgradeable.tryMul(uint256(RateBNBwithUSD()), 10**10);
            // rateLoanAsset = uint256(RateBNBwithUSD()) * 10**10;
            rateLoanAsset = rateLA;
        } else {
            (, uint256 rateLA) = SafeMathUpgradeable.tryMul(uint256(getLatesPriceToUSD(loanAsset)), 10**10);
            // rateLoanAsset = uint256(getLatesPriceToUSD(loanAsset)) * 10**10;
            rateLoanAsset = rateLA;
        }
        ...
        (, uint256 lAmountDecimal) = SafeMathUpgradeable.tryDiv(lAmount, 10**5);

        loanAmount = lAmountDecimal;

        require(false, "1");

        if (repaymentAsset == address(0)) {
            (, uint256 rateRP) = SafeMathUpgradeable.tryMul(uint256(RateBNBwithUSD()), 10**10);
            // rateRepaymentAsset = uint256(RateBNBwithUSD()) * 10**10;
            rateRepaymentAsset = rateRP;
        } else {
            (, uint256 rateRP) = SafeMathUpgradeable.tryMul(uint256(getLatesPriceToUSD(repaymentAsset)), 10**10);
            // rateRepaymentAsset = uint256(getLatesPriceToUSD(repaymentAsset)) * 10**10;
            rateRepaymentAsset = rateRP;
        }

        (, uint256 rLoan) = SafeMathUpgradeable.tryMul(rateLoanAsset, 10**5);
        (, uint256 eRate) = SafeMathUpgradeable.tryDiv(rLoan, rateRepaymentAsset);
        // exchangeRate = (10**5 * rateLoanAsset) / rateRepaymentAsset;
        exchangeRate = eRate;
    }
