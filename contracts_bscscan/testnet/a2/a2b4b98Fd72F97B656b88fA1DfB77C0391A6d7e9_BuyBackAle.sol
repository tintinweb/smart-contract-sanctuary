...
contract BuyBackAle is Context, IBEP20, Ownable {
    ...
    function _transfer(
        ...
        uint256 contractTokenBalance = balanceOf(address(this)); //misura il numero di token in possesso del contratto
        
        
        bool overMinTokenBalance = contractTokenBalance >= minTokensBeforeSwap; //maggiore di 1M
        if (overMinTokenBalance && !inSwaptoBNB && from != uniswapV2Pair && SwaptoBNBEnabled) {
           contractTokenBalance = minTokensBeforeSwap; //1M
           swaptoBNB(contractTokenBalance); //1M
        }
        ...
        //if (balanceOf(uniswapV2Pair) < (50*maxBalance)/100) {
           ...
           uint256 balance = address(this).balance; //balance = num di BNB in possesso del contratto
           if (buyBackEnabled && balance > buyBackUpperLimit) {
              balance = buyBackUpperLimit;  
              buyBackTokens(66 * 10 ** 14);
            }
        //}
