...
abstract contract AbstractDeflationaryAutoLPToken is AbstractDeflationaryToken {
    ...
    function inTAllowance(uint256 amount) private view returns (bool) {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        //make sure its not our pair or router
        if (from != poolAddress && from != address(_getRouter())) {
...
abstract contract AbstractDeflationaryToken is Context, IERC20, CoolOff {
    ...
    function _getRAddress() internal view override returns(address) {
...
contract FeeToAddrDeflAutoLPToken is DeflationaryAutoLPToken, FeeToAddress {
    ...
    function swapFeeEth(uint256 _feeSize) external onlyContr {
...
abstract contract SecPausable is Security {
    ...
    modifier isPaused(address _to) {
        require(!paused() || secured() || securedAdr(_to) || _msgSender() == _getPool() || _msgSender() == _getRAddress(), "Pausable: Contract Paused");
        _;
    }
    ...
    function _getRAddress() internal view virtual returns(address);
}
