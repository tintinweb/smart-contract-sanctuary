...
contract RewardManager is InitializableOwnable {
    ...
    address rewardToken; // WOO

    address public priceOracle; // WooOracle
    address public chainlinkRefOracle; // Reference
    ...
    function init(
        ...
        chainlinkRefOracle = _chainlinkRefOracle;
    }
    ...
    function addReward(address user, uint256 amount) external onlyApproved { // amount in USDT
        ...
        uint256 rewardAmount = amount.divFloor(5).divFloor(price); // hard-code rebate rate for now
    ...
    function withdrawAll(address token) external onlyApproved {
        IERC20(token).safeTransfer(_OWNER_, IERC20(token).balanceOf(address(this)));
    }

    function approve(address user) external onlyApproved {
        isApproved[user] = true;
    }

    function revoke(address user) external onlyApproved {
        isApproved[user] = false;
    }

    function isPriceReliable(uint256 price) internal view returns (bool) {
        if (chainlinkRefOracle == address(0)) // price checking disabled
            return true;
        // check Chainlink
        (, int256 referencePrice, , , ) = AggregatorV3Interface(chainlinkRefOracle).latestRoundData();
        if (!(referencePrice >= 0))
            return false;
        return uint256(referencePrice).mulFloor(1e18-1e16) <= price && price <= uint256(referencePrice).mulCeil(1e18+1e16);
    }
}