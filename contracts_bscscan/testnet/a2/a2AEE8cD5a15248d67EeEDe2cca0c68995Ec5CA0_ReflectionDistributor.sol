...
contract ReflectionDistributor is Allowable, IReflectionDistributor {
    ...
    receive() external payable {
        ...
        _dividendsPerShare = _totalShares > 0
            ? _dividendsPerShare.add(ethAmount.div(_totalShares).div(_dividendsPerShareAccuracyFactor))
            : 0;
    }
    ...
    function _getCumulativeDividends(uint256 share) private view returns (uint256) {
        return share.mul(_dividendsPerShare).mul(_dividendsPerShareAccuracyFactor);
    }
    ...
    function _setShare(address shareholder, uint256 amount) private {
        ...
        _totalShares = _totalShares.add(amount).sub(previousAmount);
