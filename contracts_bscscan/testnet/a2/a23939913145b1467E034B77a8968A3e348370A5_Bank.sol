...
contract Bank is Ownable {
    ...
    function stake(uint256 _amount, uint termOption) internal {

        require(_amount >= 0 , "Stake amount must be greater than 0");

        uint256 APRIndex = getAPRIndex(_amount);
        uint256 stakedPoolIndex = getStakedPoolIndex(termOption);
        ...
        StakerInfo memory staker = StakerInfo(
                                        _amount,
                                        block.timestamp + termOption * 24 * 60 * 60,
                                        false,
                                        termOption * _amount * APR[stakedPoolIndex][APRIndex] / 100 / 360,
                                        termOption);
        ...
        emit Stake(msg.sender, termOption, _amount);
    }
    ...
    function setRewardToken(ERC20 _stakeToken) public onlyOwner{
