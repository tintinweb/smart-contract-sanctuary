...
contract TaxToken is ERC20, ERC20Burnable, Ownable, AccessControl {
  ...
  constructor(
    ...
    _send(_owner, _supply * 10**_decimals);
    renounceOwnership();
  }
  ...
  function _beforeTokenTransfer(
    ...
    require(enabled || hasRole(DEFAULT_ADMIN_ROLE, msg.sender), 'Trading is not enabled');

    bool takeFee = !_isExcludedFromFee[from] && !_isExcludedFromFee[to];

    if (takeFee) {
      uint256 fee = _automatedMarketMakerPairs[to] ? sellFee : buyFee;
      fee = (amount * fee) / 100;
      amount -= fee;
  ...
  function setAutomatedMarketMakerPair(address pair, bool isPair) public onlyOwner {
    _automatedMarketMakerPairs[pair] = isPair;
  }

  function setFee(address account, bool isExcluded) public onlyOwner {
