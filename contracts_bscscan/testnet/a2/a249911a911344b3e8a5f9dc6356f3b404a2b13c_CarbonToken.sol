...
interface IERC20 {
    ...
    function transferAndLock(address receiver, uint256 amount, uint256 releaseDate) external returns (bool);
...
contract CarbonToken is ERC20, Pausable, ERC20Detailed {
    ...
    mapping (uint => uint) public quarterMapProjectOwner;
    address [] private lockedAddressList; // list of addresses that have some fund currently or previously locked
    
	constructor() public ERC20Detailed("Carbon-4", "CO2", 9) {  
        totalCoins = 1000000000 * 10 ** uint256(decimals());
        _mint(owner(), totalCoins); // total supply fixed at 1 billion tokens

        ERC20.transfer(co2CorpWallet, 200000000 * 10 ** uint256(decimals()));
        ERC20.transfer(presaleIDOWallet, 50000000 * 10 ** uint256(decimals()));

        quarterMapProjectOwner[1]=1654041600;       // Wed Jun 01 2022 00:00:00 GMT
        quarterMapProjectOwner[2]=1656633600;
        quarterMapProjectOwner[3]=1659312000;
        quarterMapProjectOwner[4]=1661990400;
        quarterMapProjectOwner[5]=1664582400;
        quarterMapProjectOwner[6]=1667260800;
        quarterMapProjectOwner[7]=1669852800;
        quarterMapProjectOwner[8]=1672531200;
        quarterMapProjectOwner[9]=1675209600;
        quarterMapProjectOwner[10]=1677628800;      // Wed Mar 01 2023 00:00:00 GMT
        for(uint i = 1; i<= 10;i++) {
            transferAndLock(projectOwner, 65000000 * 10 ** uint256(decimals()), quarterMapProjectOwner[i]);
        }

        transferAndLock(privateSaleWallet, 25000000 * 10 ** uint256(decimals()), 1666396800);   // Sat Oct 22 2022 00:00:00
        transferAndLock(workingTeamWallet, 25000000 * 10 ** uint256(decimals()), 1666396800);   // Sat Oct 22 2022 00:00:00
        transferAndLock(boardOfAdvisory, 50000000 * 10 ** uint256(decimals()), 1666396800);     // Sat Oct 22 2022 00:00:00
    }
