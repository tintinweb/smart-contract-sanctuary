...
contract Hypersonic is BEP20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(tradingIsEnabled && !_isExcludedFromMaxWalletToken[to]) 
		{
            ...
            require(contractBalanceRecepient.add(amount) <= maxWalletToken, "Transfer amount exceeds the maxWalletToken");
        }
		...
		if(tradingIsEnabled && automatedMarketMakerPairs[to]) 
		{
			...
			require(amount <= maxSellToeknAmount, "Transfer amount exceeds the maxSellToeknAmount");
