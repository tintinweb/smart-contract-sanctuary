...
contract MagicStella is BEP20, Ownable {
    ...
    address public  operationsWallet;       // Optionnal 
    address private buyBackWallet;          // sell team wallet 
    ...
    // 10M
    uint256 constant _totalSupply = 1 * 10**7 * (10**_decimals); 

    // thresold for transaction 0.1%
    uint256 public maxSellTransactionAmount = (_totalSupply * 1) / 1000;    
    // Min amount to transform token in contract to dividends & liquify 0.01%
    uint256 public swapTokensAtAmount = (_totalSupply * 1) / 10000;         
    ...
    constructor() BEP20("MagicStella", "MagicStella", 9) {
        ...
        IStellaRouter02 _stellaRouter = IStellaRouter02(0xCc7aDc94F3D80127849D2b41b6439b7CF1eB4Ae0); // Testnet 
    ...
    function _diverseSupplyInAllEcoSystemWallets() private{
        ...
        */
        ...
        _mint(teamAddress, totalForTeamWallet);
        _mint(marketingFeeReceiver, totalForMarketingWallet);
        _mint(devAddress, totalForDevWallet);
        _mint(owner(), totalForSellTeamWallet);
        _mint(liquidityWallet, totalForLiquidityWallet);
    }

    function _setAllEcoSystemWallets() private {

        marketingFeeReceiver = owner();//0xad2805b8113066b4f9859689cB8a3fa360fcA1b3; //TODO Replace by mainnet contract
        devAddress = owner();//0x1C7CB4E5171434cdF832F0A57b7B240dB789DD5A; //TODO Replace by mainnet contract
        teamAddress = owner();//0x1C7CB4E5171434cdF832F0A57b7B240dB789DD5A; //TODO Replace by mainnet contract

        //TODO Set different wallet for each
        liquidityWallet = owner();
        operationsWallet = owner();
        buyBackWallet = owner();
    }

    function _removeRestrictionForEcoSystemWallets(IStellaRouter02 _stellaRouter) private {
        ...
        isTimelockExempt[address(_stellaRouter)] = true;
        isTimelockExempt[operationsWallet] = true;
        isTimelockExempt[buyBackWallet] = true;
        
        _isAllowedDuringDisabled[address(this)] = true;
        _isAllowedDuringDisabled[owner()] = true;
        _isAllowedDuringDisabled[teamAddress] = true; // To give a part of wallet for core team partners
    ...
    function updateHolderSellFactorSetup(
        ...
        require(holderSellFactorPeriod > 1, "MagicStella: holderSellFactorPeriod must be greater than 1.");
        require(holderSellFactorMaxOfPeriod > 1, "MagicStella: holderSellFactorMaxOfPeriod must be greater than 1.");
        require(holderSellFactorMinPourcent > 10, "MagicStella: holderSellFactorMinPourcent must be greater than 10%.");
        require(holderSellFactorReducePourcentByPeriod <= 20, "MagicStella: holderSellFactorReducePourcentByPeriod must be smaller or equal than 20%.");
    ...
    function updateDividendTokensMinimum(uint256 minimumToEarnDivs) external authorized {
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external authorized {
    ...
    function updateOperationsWallet(address newOperationsWallet) external authorized {
    ...
    function updateFees(uint256 bnbRewardPerc, uint256 liquidityPerc, uint256 operationsPerc, uint256 buyBackPerc) external authorized {
    ...
    function setIgnoreToken(address tokenAddress, bool isIgnored) external authorized returns (bool){
    ...
    function getHolderSellFactor(address holder) public view returns (uint256){
        ...
        if(!_isHolderSellFactorActived)
            return sellFeeIncreaseFactor;

        // get time since last sell measured in 2 week increments
        uint256 timeSinceLastSale = (block.timestamp.sub(_holderLastSellDate[holder])).div(_holderSellFactorPeriod);
        ...
        return sellFeeIncreaseFactor-(timeSinceLastSale.mul(_holderSellFactorReducePourcentByPeriod));
    }
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        stellaRouter.swapExactBNBForTokensSupportingFeeOnTransferTokens{value: bnbAmount}( //try to swap for tokens, if it fails (bad contract, or whatever other reason, send BNB)
    ...
    function _coolDownTimer(address to) private {
    ...
    ) internal override 
    {
        ...
        // convert token in contract to liquidity & dividends
        swapTokenInContractForLiquidityAndDividends(from, to);
    ...
    function swapTokenInContractForLiquidityAndDividends(address from, address to) private {
	...
	function recoverContractBNB(uint256 recoverRate) public authorized{
