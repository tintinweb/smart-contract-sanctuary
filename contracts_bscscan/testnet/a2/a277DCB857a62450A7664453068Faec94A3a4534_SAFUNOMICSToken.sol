...
contract SAFUNOMICSRewardTracker is RewardPayingToken, Ownable {
    ...
    constructor() RewardPayingToken("SAFUNOMICS_Reward_Tracker", "SAFUNOMICS_Reward_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForRewards = 10000 * (10**9); 
    }

    function _transfer(address, address, uint256) internal override pure {
        require(false, "SAFUNOMICS_Reward_Tracker: No transfers allowed");
    }

    function withdrawReward() public override  pure {
        require(false, "SAFUNOMICS_Reward_Tracker: withdrawReward disabled. Use the 'claim' function on the main contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "SAFUNOMICS_Reward_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SAFUNOMICS_Reward_Tracker: Cannot update claimWait to same value");
...
contract SAFUNOMICSToken is BEP20, Ownable {
    
    using SafeMath for uint256;

  
uint256 private constant INITIAL_SUPPLY = 30 ** 8 * 10 ** DECIMALS;
 ...
 uint256 private _totalSupply=INITIAL_SUPPLY;
  ...
  uint256 private constant Final_SUPPLY = ~uint128(0); 
    ...
    SAFUNOMICSRewardTracker public RewardTracker;

    address public walletMarket = 0xC0b703061a0C15401e38DF62A0BA712a3347512a;
...
address public lp;
  ILP public liquidutyReciever;
    ...
    constructor() BEP20("SAFUNOMICS", "SFN") {

    	RewardTracker = new SAFUNOMICSRewardTracker();
 ...
 function rebaseToken(uint256 epoch, int256 supplyDelta)
  ...
  returns (uint256)
  {
    ...
    function updateIDEXV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(IDEXV2Router), "SAFUNOMICS: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SAFUNOMICS: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 750000, "SAFUNOMICS: gasForProcessing must be between 200,000 and 750,000");
        require(newValue != gasForProcessing, "SAFUNOMICS: Cannot update gasForProcessing to same value");
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 bnbForSAFUNOMICSBuyBack = bnbBalance.mul(BuyBackCharges).div(bnbFeeFactor);
      
        uint256 bnbForMarketing = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection).sub(bnbForSAFUNOMICSBuyBack);
        ...
        if(bnbForSAFUNOMICSBuyBack > 0)    
            payable(_walletBack).transfer(bnbForSAFUNOMICSBuyBack);
