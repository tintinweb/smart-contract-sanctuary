...
// 	uint256 constant public CONTRACT_BALANCE_STEP = 1000000 trx;
        ...
        for (uint i = 1; i <= 15; i++) { // For matching bonus
            ...
            if (i==5 && i==6) {
                    if(users[up].referrals >= 5){
                        ...
                        if(!(users[up].deposits[i].withdrawn >= (users[up].deposits[i].amount.mul(210)).div(PERCENTS_DIVIDER))){
	...
	function payoutOf(address userAddress) public view returns (uint256) {
	...
	function isActive(address userAddress) public view returns (bool) {
		...
		if (user.deposits.length > 0) {
			if (user.deposits[user.deposits.length-1].withdrawn < (user.deposits[user.deposits.length-1].amount.mul(250)).div(PERCENTS_DIVIDER)) {
				return true;
			}
		}
		return false;
