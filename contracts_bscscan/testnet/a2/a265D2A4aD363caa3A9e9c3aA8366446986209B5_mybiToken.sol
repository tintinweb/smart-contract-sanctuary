...
contract mybiToken is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 82000000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "My Bingo v4";
    ...
    address payable public marketingWallet =
        payable(0xE8Ddc4325c9548fa27aC78218352e4d370E9C3eD);
    uint256 private _previousmarketingFee = _marketingFee;

    uint256 public _develomentFee = 3;
    uint256 public _develomentLiquifyFee = 3;

    address payable public develomentWallet =
        payable(0x611C17DA7943DF1F059A8BdAB5f7284cabB19cE5);
    uint256 private _previousdevelomentFee = _develomentFee;
	
    uint256 public _stakingFee = 2;
    uint256 public _stakingLiquifyFee = 3;

    address payable public stakingWallet =
        payable(0x97F94B70126cd6A016f0308d4ee9c6511cEe2d7A);
    uint256 private _previousstakingFee = _stakingFee;
	
    uint256 public _rewardFee = 3;
    uint256 public _rewardLiquifyFee = 3;

    address payable public rewardWallet =
        payable(0x030f7e2dFa7ff91586a478a6CE9e46564b57E037);
    ...
    constructor() {
        ...
        //_rOwned[_msgSender()] = _tTotal * 80 / 100;
        //_rOwned[develomentWallet] = _tTotal * 5 / 100;
        //_rOwned[marketingWallet] = _tTotal * 15 / 100;
        ...
        _isExcludedFromFee[develomentWallet] = true;
        _isExcludedFromFee[marketingWallet] = true;        

        emit Transfer(address(0), _msgSender(), _tTotal);
        //emit Transfer(address(0), _msgSender(), _tTotal * 80 / 100);
        //emit Transfer(address(0), develomentWallet, _tTotal * 5 / 100);
        //emit Transfer(address(0), marketingWallet, _tTotal * 15 / 100);
    }
    ...
    function removeAllFee() private {
		...
		_previousdevelomentFee = _develomentFee;
    ...
    function restoreAllFee() private {
		...
		_develomentFee = _previousdevelomentFee;
    ...
    function swapAndSendBNBToDeveloment(uint256 tokenAmount) private {
        swapTokensForBnb(tokenAmount, develomentWallet);
    }

    function swapAndSendBNBToStaking(uint256 tokenAmount) private {
        swapTokensForBnb(tokenAmount, stakingWallet);
    }
	
    function swapAndSendBNBToReward(uint256 tokenAmount) private {
        swapTokensForBnb(tokenAmount, rewardWallet);
    }
    ...
    function _transferStandard(
        ...
        (tTransferAmount, rTransferAmount) = takeDeveloment(
            sender,
            tTransferAmount,
            rTransferAmount,
            tAmount
        );
        (tTransferAmount, rTransferAmount) = takeStaking(
            sender,
            tTransferAmount,
            rTransferAmount,
            tAmount
        );
        (tTransferAmount, rTransferAmount) = takeReward(
            sender,
            tTransferAmount,
            rTransferAmount,
            tAmount
    ...
    function takeDeveloment(
        ...
        uint256 tDeveloment = tAmount.div(100).mul(_develomentFee);
        uint256 rDeveloment = tDeveloment.mul(_getRate());
        rTransferAmount = rTransferAmount.sub(rDeveloment);
        tTransferAmount = tTransferAmount.sub(tDeveloment);
        _rOwned[develomentWallet] = _rOwned[develomentWallet].add(rDeveloment);
        emit Transfer(sender, develomentWallet, tDeveloment);
        return (tTransferAmount, rTransferAmount);
    }
	
    function takeStaking(
        address sender,
        uint256 tTransferAmount,
        uint256 rTransferAmount,
        uint256 tAmount
        ...
        uint256 tStaking = tAmount.div(100).mul(_stakingFee);
        uint256 rStaking = tStaking.mul(_getRate());
        rTransferAmount = rTransferAmount.sub(rStaking);
        tTransferAmount = tTransferAmount.sub(tStaking);
        _rOwned[stakingWallet] = _rOwned[stakingWallet].add(rStaking);
    ...
    function setDevelomentWallet(address payable newWallet) external onlyOwner {
        develomentWallet = newWallet;
    }
	
    function setStakingWallet(address payable newWallet) external onlyOwner {
    ...
    function setDevelomentFeePercent(uint256 newFee) external onlyOwner {
        _develomentFee = newFee;
    }
    ...
    function setRewardFeePercent(uint256 newFee) external onlyOwner {
        _rewardFee = newFee;
    }
    ...
    function setDevelomentFeePreviousPercent(uint256 newFee) external onlyOwner {
        _previousdevelomentFee = newFee;
    }
	
    function setStakingFeePreviousPercent(uint256 newFee) external onlyOwner {
        _previousstakingFee = newFee;
    }
	
    function setRewardFeePreviousPercent(uint256 newFee) external onlyOwner {
        _previousrewardFee = newFee;
    }
