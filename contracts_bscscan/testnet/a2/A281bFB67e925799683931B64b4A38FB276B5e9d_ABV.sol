...
contract ABV is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable private feeAddress = payable(0x3aeDdD106a585F761a5D1ff65008BbcC89761e4C); // Marketing Address
    ...
    string private _name = "ABV";
    string private _symbol = "ABV";
    ...
    uint256 private buyBackSell = 1 * 10**14;
    ...
    uint256 private _buyBackTimeInterval = 5 minutes;
    uint256 private _buyBackMaxTimeForHistories = 24 * 60 minutes;
    ...
    bool private _isEnabledBuyBackAndBurn = true;
    ...
    function buyBackSellAmount() public view returns (uint256) {
        return buyBackSell;
    }
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
                ...
                if (buyBackEnabled) {
                    ...
                    uint256 _bBSLimitMax = buyBackSell;
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(feeAddress, transferredBalance.mul(marketingDivisor).div(10));
    ...
    function GetBuyBackTimeInterval() private view returns(uint256) {
        return _buyBackTimeInterval.div(60);
    }

    function GetBuybackMinutes() public view returns(uint256) {
        return _intervalMinutesForSwap.div(60);
    }

    function SetBuybackMinutes(uint256 newMinutes) external onlyOwner {
        _intervalMinutesForSwap = newMinutes * 1 minutes;
    }
    
    function setBuyBackAmount(uint256 buyBackAmount) external onlyOwner {
        buyBackSell = buyBackAmount;
    }
    ...
    function setMarketingAddress(address _feeAddress) external onlyOwner {
