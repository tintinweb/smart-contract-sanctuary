...
contract Ibnb_presale is Ownable {
  ...
  uint256 public presale_token_per_BNB = 345;  //pre-sale price (345K/1BNB) AKA (345*10**18)/(10**9*10**6)
...
// -- post sale --
  ...
  function concludeAndAddLiquidity(uint256 portion_for_reward_in_percent, uint256 emergency_slippage, bool correcting_ratio, address left_over_address) external onlyOwner ongoingSale {
    ...
    if(address(pair).balance > 0 && correcting_ratio) { // pair contract spammed with BNB ?
      uint256 to_add = address(pair).balance * public_token_per_BNB/10**6;
    ...
    if(balance_token *10**6 / balance_BNB >= public_token_per_BNB) { // too much token for BNB
        balance_token = public_token_per_BNB * balance_BNB/10**6;
      }
      else { // too much BNB for token left
        balance_BNB = balance_token *10**6 / public_token_per_BNB;
      }
