...
contract Presale{
    ...
    uint256 launchDays= 20 minutes;
    uint256 round1Days = 20 minutes;
    uint256 round2or3Days = 30 minutes;
    uint256 gapDays = 20 minutes;
    uint256 withdrawDays = 15 minutes;
    	struct User {
...
address owner;
  ...
  function availableClaimAmount(address userAdress) public view returns (uint256){
      	...
      	if(block.timestamp>=presSaleStart+launchDays){
      	    claimAmount = claimAmount.add(balance.mul(20).div(100));
      	}
      	if(block.timestamp>=presSaleStart+launchDays+withdrawDays){
      	    claimAmount = claimAmount.add(balance.mul(50).div(100));
      	}
      		if(block.timestamp>=presSaleStart+launchDays+withdrawDays+withdrawDays){
