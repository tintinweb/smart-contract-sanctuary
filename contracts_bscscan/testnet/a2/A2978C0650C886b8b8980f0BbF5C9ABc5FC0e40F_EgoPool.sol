...
contract EgoPool is BasePool {
    ...
    function _unstakeAsset(address celebrity, StakerInfo memory stakerInfo, CelebrityInfo memory celebrityInfo, uint256 assetStakeAmount) internal virtual override {
        if(assetStakeAmount > 0)
            require(_assetToken.transfer(msg.sender, assetStakeAmount), '_unstakeAsset: fail transfer asset' );
...
abstract contract BasePool is Ownable {
    ...
    uint256 private _lastAPYChangeTime;
     // vEGO Rate
    uint256 public CurrentRate;
    uint256 public APYCum;
    uint256 public Rate0;

    uint256 public TotalStaked;
    uint256 public TotalStakedEgo;
    uint256 public TotalVEgo;
    uint256 public RewardPerSeccond = 31688738506811400;
    ...
    mapping(address => uint256) public CelerbrityPendingReward;
    ...
    constructor(
        ...
        _egoToken = egoToken;
        _lastAPYChangeTime = block.timestamp;
        APYCum = 0;

        AssetPriceProvider = assetPriceProvider;
        CurrentRate = initialVEGORate * DEFAULT_BASE / 10;
        Rate0 = CurrentRate;
    ...
    function stake(
        ...
        stakerInfo.egoBalanceInfoOriginal.vBalance = stakerInfo.egoBalanceInfoOriginal.amount * RewardPerShare / DEFAULT_BASE;

        stakerInfo.egoBalanceInfoShare.amount += egoAmount * (DEFAULT_BASE - celebrityRewardPercent) / DEFAULT_BASE; 
        stakerInfo.egoBalanceInfoShare.vBalance = stakerInfo.egoBalanceInfoShare.amount * RewardPerShare / DEFAULT_BASE;
        
        uint256 pendingCelebrity =  celebrityInfo.egoBalanceInfo.amount * RewardPerShare / DEFAULT_BASE - celebrityInfo.egoBalanceInfo.vBalance;
        celebrityInfo.egoBalanceInfo.amount += egoAmount * celebrityRewardPercent / DEFAULT_BASE;
        celebrityInfo.egoBalanceInfo.vBalance = celebrityInfo.egoBalanceInfo.amount * RewardPerShare / DEFAULT_BASE;

        TotalStaked += assetStakeAmount;
        TotalStakedEgo += egoAmount;
        TotalVEgo += vEgoAdded;
        //----------------- calc --------------- 
        ...
        CelerbrityPendingReward[celebrity] += pendingCelebrity;
        //----------------- store --------------- 
    ...
    function celebrityClaim(address celebrity) nonReentrant public {
        ...
        ClaimAmount memory egoClaim = _calculateCelebrityEgoClaimAmount(celebrity);

        uint256 egoFee = 0;
        if(_assetToken == _egoToken)
            egoFee = PLATFORM_FEE;

        uint256 egoClaimAmount =  egoClaim.claimAmount * (DEFAULT_BASE - egoFee) / DEFAULT_BASE;
        uint256 egoFeeAmount =  egoClaim.claimAmount * egoFee / DEFAULT_BASE;
        if (egoClaimAmount > 0) {
            celebrityInfo.egoBalanceInfo.vBalance = celebrityInfo.egoBalanceInfo.amount * RewardPerShare / DEFAULT_BASE;
            if(_egoToken.balanceOf(address(RewardVault)) > egoClaimAmount)
                require(_egoToken.transferFrom(
                    address(RewardVault),
                    celebrity,
                    egoClaimAmount
                ), 'celebrityClaim: fail transfer ego');
            if(TotalVEgo > egoClaim.vClaimAmount)
                TotalVEgo -= egoClaim.vClaimAmount;
            else {
                TotalVEgo = 0;
            }
        }
        ...
        if (egoFeeAmount > 0) {
            if(_egoToken.balanceOf(address(RewardVault)) > egoFeeAmount)
                require(_egoToken.transferFrom(
                    address(RewardVault),
                    _platformFeeAddress,
                    egoFeeAmount
                ), 'celebrityClaim: fail transfer fee ego');
            if(TotalVEgo > egoClaim.vClaimAmount)
                TotalVEgo -= egoClaim.vClaimAmount;
            else {
                TotalVEgo = 0;
        ...
        CelerbrityInfo[celebrity] = celebrityInfo;
        CelerbrityPendingReward[celebrity] = 0;
    ...
    function calculateCelebrityEgoClaimAmount(address celebrity) view public returns (ClaimAmount memory result){
        result = _calculateCelebrityEgoClaimAmount(celebrity);
        result.claimAmount = result.claimAmount * (DEFAULT_BASE - PLATFORM_FEE) / DEFAULT_BASE;
    }
    

    // calculates amount of EGO available for the Celebrity to claim
    function _calculateCelebrityEgoClaimAmount(address celebrity) view internal returns (ClaimAmount memory result){
        CelebrityInfo memory celebrityInfo = CelerbrityInfo[celebrity];
        uint256 rewardPerShare = _calculateRewardPerShare();
        result.claimAmount = celebrityInfo.egoBalanceInfo.amount * rewardPerShare / DEFAULT_BASE - celebrityInfo.egoBalanceInfo.vBalance;
        result.claimAmount += CelerbrityPendingReward[celebrity];
    }
    ...
    function calculateStakerEgoClaimAmount(address owner, address celebrity) view public returns (ClaimAmount memory result) {
        ...
        result.claimAmount =  stakerInfo.egoBalanceInfoShare.amount * rewardPerShare / DEFAULT_BASE - stakerInfo.egoBalanceInfoOriginal.vBalance;
    }
    ...
    function unstake(address celebrity, uint256 assetStakeAmount) nonReentrant public {
        ...
        uint256 originalAssetBalance = stakerInfo.assetBalanceInfo.amount;
        // update rate inside
        _stakerClaim(msg.sender, celebrity);
        _unstakeAsset(celebrity, stakerInfo, celebrityInfo, assetStakeAmount);
        
        uint256 percentUnstake  = assetStakeAmount * DEFAULT_BASE / originalAssetBalance;
        uint256 egoBurn = stakerInfo.egoBalanceInfoOriginal.amount * percentUnstake / DEFAULT_BASE;
        stakerInfo.egoBalanceInfoOriginal.amount -= egoBurn;
        stakerInfo.egoBalanceInfoOriginal.vBalance = stakerInfo.egoBalanceInfoOriginal.amount * RewardPerShare / DEFAULT_BASE;
       
        stakerInfo.egoBalanceInfoShare.amount -= stakerInfo.egoBalanceInfoShare.amount * percentUnstake * (DEFAULT_BASE - stakerInfo.celebrityRewardPercent) / DEFAULT_BASE / DEFAULT_BASE;
        stakerInfo.egoBalanceInfoShare.vBalance = stakerInfo.egoBalanceInfoShare.amount * RewardPerShare / DEFAULT_BASE;
        
        uint256 pendingCelebrity =  celebrityInfo.egoBalanceInfo.amount * RewardPerShare  / DEFAULT_BASE - celebrityInfo.egoBalanceInfo.vBalance;
        celebrityInfo.egoBalanceInfo.amount -= celebrityInfo.egoBalanceInfo.amount * percentUnstake * stakerInfo.celebrityRewardPercent / DEFAULT_BASE / DEFAULT_BASE;
        celebrityInfo.egoBalanceInfo.vBalance = celebrityInfo.egoBalanceInfo.amount  * RewardPerShare / DEFAULT_BASE;
       
        uint256 vEgoBurnForCelebrity = egoBurn * DEFAULT_BASE / CurrentRate;
        ...
        if(TotalVEgo > vEgoBurnForCelebrity)
            TotalVEgo -= vEgoBurnForCelebrity;
        else
            TotalVEgo = 0;
    ...
    function _updateRate() private {
        RewardPerShare = _calculateRewardPerShare();
        LastRewardTime = block.timestamp;
    }

    function _calculateRewardPerShare()  internal view returns(uint256){
        if(TotalStakedEgo > 0){
            uint256 limitedRewardPerSeccond = RewardPerSeccond;
            if(limitedRewardPerSeccond * DEFAULT_BASE * SECONDS_IN_YEAR / TotalStakedEgo > MAXapr) {
               limitedRewardPerSeccond = TotalStakedEgo * MAXapr / SECONDS_IN_YEAR / DEFAULT_BASE;
            }
            uint256 multiplier = getMultiplier(LastRewardTime, block.timestamp);
            uint256 reward = multiplier * limitedRewardPerSeccond;
            uint256 result = RewardPerShare + reward * DEFAULT_BASE / TotalStakedEgo;
            return result;
        }
    ...
    function calculateAPR() public view returns (uint256) {
        if(TotalStakedEgo == 0)
            return MAXapr;
        if(RewardPerSeccond * DEFAULT_BASE * SECONDS_IN_YEAR / TotalStakedEgo > MAXapr)
          return MAXapr;
        return RewardPerSeccond * DEFAULT_BASE * SECONDS_IN_YEAR / TotalStakedEgo;
    }
    ...
    function setRewardPerSeccond(uint256 rewardPerSeccond) external onlyOwner {
        RewardPerSeccond = rewardPerSeccond;
    }
