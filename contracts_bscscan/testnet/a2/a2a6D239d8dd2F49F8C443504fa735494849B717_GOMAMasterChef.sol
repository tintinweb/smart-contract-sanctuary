...
contract GOMAMasterChef is Ownable {
    ...
    constructor(
        IBEP20 _rewardToken,
        uint256 _maxTokensPerSecond,
        uint256 _tokenPerSecond,
        uint256 _startTime
    ) {
        rewardToken = _rewardToken;
        maxTokensPerSecond = _maxTokensPerSecond;
	...
	// 
    function supplyRewardsTokens(uint256 _amount) public {
    ...
    function addPool(uint256 _allocPoint, IBEP20 _stakeToken) external onlyOwner {
        require(_allocPoint <= maxAllocPoint, "addPool: too many alloc points!");
		require(pausedTime == 0, "addPool: you can't add while paused!");
    ...
    function setPoolAllocationPoints(uint256 _pid, uint256 _allocPoint) external onlyOwner {
        require(pausedTime == 0, "setPoolAllocationPoints: you can't set while paused!");
		require(_allocPoint <= maxAllocPoint, "setPoolAllocationPoints: too many alloc points!");
    ...
    function massUpdatePools() public {
        require(pausedTime == 0, "massUpdatePools: you can't update while paused!");
        _massUpdatePools();
    }

    // Update reward variables of the given pool to be up-to-date.
    function updatePool(uint256 _pid) public {
        require(pausedTime == 0, "updatePool: you can't update while paused!");
        _updatePool(_pid);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
		require(pausedTime == 0, "deposit: you can't deposit while paused!");
        require(_amount > 0, "deposit: you can't deposit 0!");
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {  
		require(pausedTime == 0, "withdraw: you can't withdraw while paused!");
    ...
    function balanceOfStakeToken(IBEP20 _stakeToken) public view returns (uint256) {
