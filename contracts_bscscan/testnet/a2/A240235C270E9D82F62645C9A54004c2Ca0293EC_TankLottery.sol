...
contract TankLottery is IReceivesBogRandV2, Initializable, OwnableUpgradeable {
  ...
  uint256 private paidFeeInBNB;

  mapping(bytes32 => Play) private plays;
  ...
  event PlayRequest(address indexed player, bytes32 indexed hash);
  event PlayResult(address indexed player, bytes32 indexed hash, uint256 indexed point, uint256 random);
  ...
  function initialize(
    ...
    rng = IBogRandOracleV2(0x0eCb31Afe9FE6a10f9A173843aE7957Df39D8236);
  ...
  function getPlay(bytes32 hash) external view returns(Play memory) {
    return plays[hash];
  }
  ...
  function play(uint8 mode) external {
    ...
    require(contractBalance >= fee, "Insufficient fee to get random");
    ...
    paidFeeInBNB = paidFeeInBNB.add(fee);
    (bytes32 hash, ) = rng.requestRandomnessBNBFee{value: fee}();
    plays[hash] = Play(_msgSender(), false, 0);
    emit PlayRequest(_msgSender(), hash);
  }

  function receiveRandomness(bytes32 hash, uint256 random) external override {
    require(_msgSender() == address(rng));
  ...
  function _result(bytes32 hash, uint256 random) internal {
    uint256 contractBal = IERC20(BUSD).balanceOf(address(this));
    uint256 point = plays[hash].prizeId;
    ...
    emit PlayResult(plays[hash].player, hash, point, random);
  }
