            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"db5b860ff68bde408d682fa6fd5f4c2d1a1f4c2a60f773b26fda4b2741219722" // init code hash
                    )
                )
            )
        );
...
contract Router is IUniswapV2Router02 {
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (tokenA == medexToken || tokenB == medexToken) {
                 uint256 medPriceFinal = medaxPrice;
                if (tokenA == bnbAddress || tokenB == bnbAddress) {
                     medPriceFinal = bnbMedaxPrice;
                } 
                if (amountBDesired == 0) {
                    if (tokenA == medexToken) {
                        uint256 amountBOptimal = amountADesired.mul(
                            medPriceFinal
                        );
                        (amountA, amountB) = (amountADesired, amountBOptimal);
                    } else {
                        uint256 unitPrice = 1;
                        uint256 perbusd = unitPrice / medPriceFinal;
                        uint256 amountBOptimal = amountADesired.mul(perbusd);
                        (amountA, amountB) = (amountADesired, amountBOptimal);
                    }
                } else {
                    if (tokenA == medexToken) {
                        uint256 amountAOptimal = amountBDesired.mul(
                            medPriceFinal
                        );
                        ...
                        uint256 unitPrice = 1;
                        uint256 perbusd = unitPrice / medPriceFinal;
                        uint256 amountAOptimal = amountBDesired.mul(perbusd);
                        (amountA, amountB) = (amountAOptimal, amountBDesired);
                    }
                }
            } else {
