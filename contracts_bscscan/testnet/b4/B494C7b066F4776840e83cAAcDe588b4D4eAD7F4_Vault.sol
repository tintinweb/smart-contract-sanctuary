...
contract Vault is VaultStorage {
    ...
    function updateVault() internal {
        uint256 lpBalance = stakedToken.balanceOf(address(this));
        if (lpBalance == 0) { // avoids division by 0 errors
            return;
        }

        accAtlantisPerShare = accAtlantisPerShare.add(pendingRewards.mul(1e18).div(lpBalance));
        pendingRewards = 0;
    }
...
contract VaultProxy is VaultAdminStorage, VaultErrorReporter {
    ...
    /**
      * @notice Emitted when pendingVaultImplementation is accepted, which means Vault implementation is updated
      */
    ...
    /**
    * @notice Accepts new implementation of Vault. msg.sender must be pendingImplementation
