...
contract FanDungeonCaller is HeroInfo, LandInfo{
    ...
    function viewHeroes(address _address, uint256[] calldata tokenIds) external view returns(Hero[] memory heroes) {
    ...
    function viewLands(address _address, uint256[] calldata tokenIds) external view returns(Land[] memory lands) {
        lands = new Land[](tokenIds.length);
        for (uint256 i =0;i< tokenIds.length;i++) {
            Land memory land = IFanDungeonLand(_address).viewLand(tokenIds[i]);
            lands[i] = land;
        }
    }
