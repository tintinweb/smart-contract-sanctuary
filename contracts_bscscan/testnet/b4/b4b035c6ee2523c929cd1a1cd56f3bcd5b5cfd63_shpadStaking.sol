...
contract shpadStaking {
    ...
    */
    ...
    function userDetails(address addr) public view returns(uint amt, uint time, bool stakedBool) {
        amt = user[addr].stakedAmount;
        time = user[addr].stakedTime;
        stakedBool = user[addr].staked;
    }
