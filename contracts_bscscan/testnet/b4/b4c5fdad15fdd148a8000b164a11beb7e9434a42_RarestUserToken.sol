...
contract RarestToken is Ownable, SignerRole, ERC1155Base {
    string public name;
    ...
    function mint(uint256 id, uint8 v, bytes32 r, bytes32 s, Fee[] memory fees, uint256 supply, string memory uri) public {
        require(isSigner(StringLibrary.getAddress(abi.encodePacked(this, id), v, r, s)), "signer should sign tokenId");
...
contract RarestUserToken is RarestToken {
    event CreateERC1155_v1(address indexed creator, string name, string symbol);

    constructor(string memory name, string memory symbol, string memory contractURI, string memory tokenURIPrefix, address signer) RarestToken(name, symbol, signer, contractURI, tokenURIPrefix) public {
