...
contract Presale {
    ...
    constructor(address _bscsDevAddress) {
    ...
    modifier isValidClaimPeriod() {
        ...
        if (
            ...
            block.timestamp < cakeLiquidityAddingTime + claimCycle
        ) {
            currentPeriod = 1;
        }
        ...
        if (
            block.timestamp >= cakeLiquidityAddingTime + claimCycle &&
            block.timestamp < cakeLiquidityAddingTime + claimCycle * 2
        ) {
            currentPeriod = 2;
        }
        ...
        if (
            block.timestamp >= cakeLiquidityAddingTime + claimCycle * 2 &&
            block.timestamp < cakeLiquidityAddingTime + claimCycle * 3
        ) {
            currentPeriod = 3;
        }

        if (block.timestamp >= cakeLiquidityAddingTime + claimCycle * 3) {
            currentPeriod = 4;
        }
