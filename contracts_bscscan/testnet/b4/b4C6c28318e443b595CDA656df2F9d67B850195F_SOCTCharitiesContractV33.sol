...
pragma solidity ^0.8.4;
    ...
    // Charities Contract test Version 33 for the SOCT37 token. Only for testing purposes on the BSC testnet
...
contract SOCTCharitiesContractV33 is Context, Ownable {
    ...
    pancakePair public LPpairAddress; 
    ...
    constructor () {
        ...
        pancakePair _pair = pancakePair(0xE7e82B259aaD65684f2D2bbAAfa327d8BDAAfaf4); // the Liquidity Pool contract presented on the BSC testNet Pancake Swap dummy DEX
        LPpairAddress = _pair;
    ...
    function _setPancakeSwapLPContractAddress(address newPancakeLP, address newPancakeRouter) public onlyOwner() {
        ...
        pancakeRouter = _pancakeRouter;
        pancakePair _pair = pancakePair(newPancakeLP); // the Liquidity Pool contract presented on the BSC testNet Pancake Swap dummy DEX
        LPpairAddress = _pair;
    }
    
    function getPancakeLP() public view returns (pancakePair pancakeLP) {
    ...
    function _checkIfReadyToSwap() public view returns (uint256 minToSellinSOCT, uint256 tFundsInBNB) {
        ...
        (uint256 soct, uint256 bnb,) = LPpairAddress.getReserves();
        ...
        uint256 B = SOCTnoD.div(bnb.div(10**18));
        uint256 MinToSell = B.div(10); // 0.1BNB
        return (MinToSell, FundsInBNB);  
    }

    function _getHalfBNB() private view returns (uint256 HBNB) {
        (uint256 soct, uint256 bnb,) = LPpairAddress.getReserves();
        uint256 SOCTnoD = soct.div(10**8);
        uint256 hB = SOCTnoD.div(bnb.div(10**18));
