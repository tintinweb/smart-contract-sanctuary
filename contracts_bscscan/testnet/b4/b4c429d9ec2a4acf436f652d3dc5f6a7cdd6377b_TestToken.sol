...
contract TestToken is FinanceBase {
	...
	function setFirstToken(address user, string memory token) public nonReentrant {
	...
	function setSecondToken(address user, string memory token) public nonReentrant {
	    require(msg.sender == user, "You are not allowed to set this user first token");
	    address selectedToken = _tokensList[token];
    	require(selectedToken != address(0) && selectedToken != BURN_WALLET, "The selected token does not exist");
    	require(_tokensAllowance[selectedToken], "The selected token is not allowed");
	...
	function getFirstToken() public view returns (address) {
	    return _firstRewardToken[msg.sender];
	}
	
	function getSecondToken() public view returns (address) {
	    return _secondRewardToken[msg.sender];
	}
...
abstract contract FinanceBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	...
	function doTransfer(address sender, address recipient, uint256 amount) internal virtual {
		...
		if (tokensAvailableForSwap >= _tokenSwapThreshold) {
			...
			bool isSelling = isUniswapPair(recipient);
			if (isSelling) {
	            require((block.timestamp >= (_sellsAllowance[sender] + 1 minutes)), "Your last sell was less than 1 minute ago, wait a bit.");
	...
	function executeSwap(uint256 amount) private {
	    address fft = address(this);
		// Allow uniswap to spend the tokens of the address
		doApprove(fft, address(_uniswapV2Router), amount);
		...
		uint256 initialBalance = fft.balance;
		require(tokensToSwap > 0, "Tokens to swap amount is zero");
		require(tokensToSwap <= amount && balanceOf(fft) >= amount, "Sell amount output not sufficient");
		swapTokensForBNB(fft, tokensToSwap);
		uint256 currentBalance = fft.balance;
 		require(currentBalance < initialBalance, "BNB was not correctly swapped, try again");
		...
		uint256 bnbToBeAddedToLiquidity = (currentBalance * tokensToSwapForLiquidity) / tokensToSwap;
		uint256 bnbToBeSentToMarketing = (currentBalance * tokensReservedForMarketing) / tokensToSwap;
		...
		emit Swapped(tokensToSwap, currentBalance, tokensToAddAsLiquidity, bnbToBeAddedToLiquidity, successSentMarketing);
	}


	// This function swaps a {tokenAmount} of Future tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(address token, uint256 amount) internal {
		...
		// Swap
		_uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(amount, 0, path, token, block.timestamp + 360);
	}
