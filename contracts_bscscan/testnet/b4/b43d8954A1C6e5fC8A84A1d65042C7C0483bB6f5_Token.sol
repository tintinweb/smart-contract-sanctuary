...
contract Token is Context, IERC20, Ownable {
    ...
    */  
    ...
    /*  
        ...
        4: marketingFee
        5: maxTXPercentage
        6: earlyBotTaxMultiplier
        7: initialSellTaxMultiplier
    */  
    ...
    */  
    ...
    constructor (address marketing, string[] memory strings, uint8[] memory smallNumbers, uint256[] memory bigNumbers, address[] memory tradeTriggerer) {
        _name = strings[0];
        _symbol = strings[1];
        _decimals = smallNumbers[0];

        _initialsupply = bigNumbers[0];
        _tTotal = _initialsupply * 10 ** _decimals; 
        _rTotal = (MAX - (MAX % _tTotal));

        taxFee = smallNumbers[2];
        _previousTaxFee = taxFee;

        liquidityFee = smallNumbers[3];
        _previousLiquidityFee = liquidityFee;

        marketingFee = smallNumbers[4];
        _previousMarketingFee = marketingFee;
        
        numTokensSellToAddToLiquidity = _tTotal / 1000; // 0.01%

        _skipBlock = smallNumbers[1];

        for(uint i = 0; i < tradeTriggerer.length; i++) {
            _tradeTriggerer[tradeTriggerer[i]] = true;
        }

        setMaxTxPercent(smallNumbers[5]);

        // Tax
        _earlyBotTaxMultiplier = smallNumbers[6];
    ...
    function checkCooldown() public view returns(bool, uint256) {
        bool isCooldown = false;
        if(accountCooldown[msg.sender] > block.timestamp) {
            isCooldown = true;
        }

        return (isCooldown, accountCooldown[msg.sender]);
    }
    ...
    function setSellFeeMultiplier(uint8 _multiplier) external onlyOwner() {
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
            ...
            // Sell cooldown   
            if(to == uniswapV2Pair) {
                require(accountCooldown[from] < block.timestamp, "Transfer still on cooldown");
                accountCooldown[from] = block.timestamp + (2 minutes);
            } else {
                // Buy (trigger) trade
                if(from == uniswapV2Pair && !launched) {
                    require(_tradeTriggerer[to], "Trade not open yet");
