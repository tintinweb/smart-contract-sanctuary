...
contract StakingPool is IStakingPool, Ownable {
    ...
    mapping(uint => uint) public heroExp;
    ...
    function convertExpToLevels(uint heroId) private {
        uint currentLevel = heroLevel[heroId];
        uint currentExp = heroExp[heroId];
        
        if(currentLevel == 1){
            requiredExp[currentLevel + 1] = requiredExp[currentLevel] * 120 / 100;
        }else if(currentLevel > 1 && currentLevel <= 20 ){
            requiredExp[currentLevel + 1] = requiredExp[currentLevel] * 120 / 100;
        }else if(currentLevel > 20 && currentLevel <= 30 ){
            requiredExp[currentLevel + 1] = requiredExp[currentLevel] * 116 / 100;
        }else if(currentLevel > 30 && currentLevel <= 40 ){
            requiredExp[currentLevel + 1] = requiredExp[currentLevel] * 112 / 100;
        }else if(currentLevel > 40 && currentLevel <= 70 ){
            requiredExp[currentLevel + 1] = requiredExp[currentLevel] * 108 / 100;
        }else if(currentLevel > 70 && currentLevel < 100 ){
            requiredExp[currentLevel + 1] = requiredExp[currentLevel] * 104 / 100;
        }

        if(currentExp >= requiredExp[currentLevel + 1]){
           levelUp(heroId, currentLevel);
        }
    }

     function levelUp(uint heroId, uint currentLevel) private {
        uint newLevel = heroLevel[heroId] + 1;
        uint maxHeroLevel = kabyHeroContract.getHeroStar(heroId) * 10;
        require(newLevel <= maxHeroLevel, "SatakingPool: max level reached");
        heroExp[heroId] -= requiredExp[currentLevel];
        heroLevel[heroId] = newLevel;
    }
    ...
    function claim(uint heroId) external override {
        ...
        if(stakingHero.lockedTime != 0){
            if(block.timestamp >= stakingHero.lockedTime){
                uint amount = stakingHero.lockedAmount;
                stakingHero.lockedAmount = 0;
                acceptedToken.safeTransfer(account, amount);
            }
        }
    }
    ...
    function upgradeStarForHero(uint heroId, UpgradeStarType upgradeType) external override {
        require(heroId < kabyHeroContract.totalSupply(), "StakingPool: invalid hero");
        address account = msg.sender;
        StakingData storage stakingHero = stakingData[account][heroId];
        ...
        if (upgradeType == UpgradeStarType.payTokenUpgrade) {
            acceptedToken.safeTransferFrom(account, address(this), amount);
            kabyHeroContract.upgradeStar(heroId, 1);
        } else {
            kabyHeroContract.upgradeStar(heroId, 1);
            acceptedToken.safeTransferFrom(account, address(this), 10 * amount);
            stakingHero.lockedTime = block.timestamp + THREE_MONTHS_IN_SECONDS;
            stakingHero.lockedAmount = 10 * amount;
        }
    }
    ...
    function _harvest(uint heroId, address account) private {
        ...
        uint maxHeroLevel = kabyHeroContract.getHeroStar(heroId) * 10;
        if(heroLevel[heroId] <= maxHeroLevel){
            heroExp[heroId] += expEarned;
        }
...
interface IStakingPool {
    ...
    enum UpgradeStarType {
        payTokenUpgrade, lockTokenUpgrade
    }
     ...
     */
    function upgradeStarForHero(uint heroId, UpgradeStarType ) external;
