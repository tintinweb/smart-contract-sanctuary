...
contract TestContract is IBEP20, Auth {
    ...
    uint256 totalProvidedLiquidity = 0; 
    uint256 numberOfLiqProvider = 0; 
    ...
    mapping (address => uint256) public liquidityAmountProvided;
    mapping (address => uint256) public liquidityProvidedShare;
    address[] public liqProviders;

    //Buyfees
    uint256 liquidityFee = 5;
    uint256 marketingFee = 3;
    uint256 liqProviderFee = 2;
    ...
    function getSwapFees() internal {
        ...
        tempLiqProviderFee = liqProviderFee.mul(sellLiqProviderFee).div(2);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBLiqProvider = amountBNB.mul(tempLiqProviderFee).div(totalBNBFee);
        
        for (uint256 i = 0; i< numberOfLiqProvider; i +=1){
            address tempLiqProvider = liqProviders[i];
            uint256 sharePercentage = liquidityProvidedShare[tempLiqProvider];
            uint256 transferAmountLiqProvider = amountBNBLiqProvider.mul(sharePercentage).div(100);
            payable(tempLiqProvider).transfer(transferAmountLiqProvider);
        }
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function setBuyFees(uint256 _liquidityFee,  uint256 _marketingFee,uint256 _liqProviderFee, uint256 _stakingFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        stakingFee = _stakingFee;
        liqProviderFee = _liqProviderFee;
        totalFee = _liquidityFee.add(_marketingFee).add(_stakingFee).add(_liqProviderFee);
        feeDenominator = _feeDenominator;
    }
    
    function setSellFees(uint256 _liquidityFee,  uint256 _marketingFee,uint256 _liqProviderFee, uint256 _stakingFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        sellStakingFee = _stakingFee;
        sellLiqProviderFee = _liqProviderFee;
        totalSellFee = _liquidityFee.add(_marketingFee).add(_stakingFee).add(_liqProviderFee);
        feeDenominator = _feeDenominator;
    }
    ...
    function setLiquidityAmountProvided(address _address, uint256 _amountInUSD) public onlyOwner {
        liquidityAmountProvided[_address]= _amountInUSD;
        totalProvidedLiquidity = totalProvidedLiquidity.add(_amountInUSD);
        (bool _isLiqProvider, ) = isStakeholder(_address);
        if(!_isLiqProvider) {
            liqProviders.push(_address);
        }
        
        if(liquidityAmountProvided[_address]== 0){
            numberOfLiqProvider += 1; 
        }
    }
    ...
    {
        for (uint256 s = 0; s < liqProviders.length; s += 1){
            if (_address == liqProviders[s]) return (true, s);
        }
        return (false, 0);
    }
    
    function getLiquidityProvidedShare() public onlyOwner {
        for (uint256 i = 0; i< numberOfLiqProvider; i +=1){
            uint256 sharePercentage;
            address tempLiqProvider = liqProviders[i];
            sharePercentage = liquidityAmountProvided[tempLiqProvider].div(totalProvidedLiquidity);
            liquidityProvidedShare[tempLiqProvider] = sharePercentage.mul(100);
        }
    }
    ...
    function getLiqProviders() public view returns(address [] memory){
        return liqProviders; 
    }
