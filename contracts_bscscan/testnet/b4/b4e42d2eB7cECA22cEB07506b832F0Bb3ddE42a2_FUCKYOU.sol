 ...
 * --- NOTES ---
 * triggerManualBuyback: 1 = 0.01 BNB
 * setTxLimitInPercent:  1 = 0.01% 
 * setMaxWalletPercent:  1 = 0.01%
 * setSwapBackSettings:  1 = 0.01%
 */
...
/* Token contract */
contract FUCKYOU is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    function triggerManualBuyback(uint256 amount, bool triggerBuybackMultiplier) external authorized {
        uint256 amountToBuy = amount.div(100);
        uint256 amountWithDecimals = amountToBuy * (10 ** 18);
        buyTokens(amountWithDecimals, DEAD);
