...
contract NFT is ERC721URIStorage, Ownable {
    ...
    function _setSigleTokenURI(uint256 tokenId_, string memory tokenURI_) public returns (bool){
        require(!setTokenURLOnce[tokenId_], "TokenURI has been set");
...
contract SNS is NFT {
    ...
    modifier shortNameAllowed(address addr_) {
        require(_shortNameAllowedlist[addr_], "Address is not in shortNameAllowedlist");
        _;
    }
     ...
     */
    constructor(string memory name_, string memory symbol_,address payable feeTo_)   {
    ...
    function mint(string memory name_) payable external virtual {
        ...
        require(name_.lenOfChars() >= STANDARD_LENGTH, "Name length must less than 4");

        require(msg.value == 0.2 ether, "msg.value should be 0.2 ether");
        ...
        require(_registerName(name_, _msgSender()), "Name register fail");
    ...
    function shortNameMint(string memory name_) external virtual shortNameAllowed(_msgSender()){
        require(name_.lenOfChars() == SHORT_LENGTH, "Name length error");
        //NFT
        uint256 tokenId = _addrMint();
        name_ = name_.toLowercase();
        name_ = name_.concat(END_STR);
     ...
     * @param to_ SNS owner
     */
    function _managerMint(string memory name_, address to_) internal virtual returns (bool){
        name_ = name_.toLowercase();
        name_ = name_.concat(END_STR);
        ...
        if(_nameRegistered[name_] || _registered[to_]){
            ...
            require(_registerName(name_, to_), "Name register fail");
            _nameOfTokenId[tokenId] = name_;
            _tokenIdOfName[name_] = tokenId;
    ...
    function _registerName(string memory name_, address to_) internal virtual returns (bool){
        require(_defaultResolverAddress != address(0), "Please set defaultResolverAddress");
        require(!_nameRegistered[name_], "Name has been registered");
        require(!_registered[to_],"The address has registered");
    ...
    function setResolverInfo(string memory name_, address resolverAddress_) external virtual {
        require(_resolverInfo[name_].owner == _msgSender(), "OnlyOwner can setNewResolverInfo");
    ...
    * @dev NFT transfer
    ...
    function transfer(address to, string memory name_) public virtual {
        ...
        //ENS
        require(_transferName(_msgSender(), to, name_), "TransferName fail");
    }
    ...
    function transferFrom(address from, address to, uint256 tokenId) public virtual override {
        ...
        //ENS
        require(_transferName(from, to, _nameOfTokenId[tokenId]), "TransferName fail");
    }
    ...
    function _transferName(address form_, address to_, string memory name_) internal virtual returns (bool){
        require(!_registered[to_], " To_ has a name"); 
        require(_nameOfOwner[form_].equal(name_), "Form_ is not the owner of name");
        _resolverInfo[name_].owner = to_;
        _nameOfOwner[form_] = "";
        _nameOfOwner[to_] = name_;
