...
interface I_Windmill_Trade_Abstract is IAccessControlEnumerable, I_With_DAORole, I_With_UpdaterRole, I_With_TradeManagerRole, I_With_BUSDToken{
    function initTrade(address _owner, address _manager, address _fund, address _updater) external;
...
abstract contract Windmill_Trade_Abstract is Adressable, AccessControlEnumerable, With_DAORole, With_UpdaterRole,
                                             With_TradeManagerRole, With_BUSDToken, I_Windmill_Trade_Abstract{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    ////
    //////////////// Public functions ////////////////
function initTrade(address _owner, address _manager, address _fund, address _updater) public onlyRole(DAO_ROLE) virtual{
        owner = _owner;
        manager = _manager;
        fund = _fund;
        
        grantRole(OWNER_ROLE, owner);
        grantRole(TRADE_MANAGER_ROLE, manager);
    ...
    function endTrade() public onlyRole(TRADE_MANAGER_ROLE) virtual{
        ...
        BUSDToken.transfer(fund, balance);
    }
    
    function updateNeeded() external view virtual returns (bool){
        return false;
    }
    
    function update() external virtual onlyRole(UPDATER_ROLE){
    }
    
    
    function estimateBUSDBalance() external view virtual returns (uint256){
        uint256 balance = BUSDToken.balanceOf(thisAddr);
        return balance;
    }
    ////
...
interface I_Windmill_Trade_Deployer_Abstract is IAccessControlEnumerable, I_With_DAORole, I_With_UpdaterRole, I_With_TradeManagerRole, I_With_BUSDToken{
    ...
    function deployNewTrade(address owner, address manager, address fund, address updater) external returns (I_Windmill_Trade_Abstract);
...
contract Windmill_Trade_Swap_Long is Windmill_Trade_Abstract, With_PancakeRouter, I_Windmill_Trade_Swap_Long{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    I_Windmill_Trade_Deployer_Swap_Long public deployerSwapLong;
    ...
    //////////////// Constructor & Modifiers ////////////////
    constructor(){
        deployerSwapLong = I_Windmill_Trade_Deployer_Swap_Long(deployer);
    }
    ...
    function initTrade(address _owner, address _manager, address _fund, address _updater) public onlyRole(DAO_ROLE) override(I_Windmill_Trade_Abstract, Windmill_Trade_Abstract){
        super.initTrade(_owner, _manager, _fund, _updater);
    }
    
    function endTrade() public onlyRole(TRADE_MANAGER_ROLE) override(I_Windmill_Trade_Abstract, Windmill_Trade_Abstract){
        deployerSwapLong.renounceRole(deployerSwapLong.TRADE_ROLE(), thisAddr);
        
        uint256 nbToken = token.balanceOf(thisAddr);
        if (nbToken>0){
            _swap(nbToken, reverseRoute, token);
        }
        
        super.endTrade();
    }
    
    function setRoute(uint256 routeId) external onlyRole(OWNER_ROLE){
        require(route.length == 0, "Windmill_Trade_Swap_Long: Route have already been set.");
        
        address[] memory _route = deployerSwapLong.getRoute(routeId);
        ...
        token = IERC20(_route[l-1]);
    }
    
    function placeMarketOrder(bool increaseSize, uint256 sizeIn) external onlyRole(OWNER_ROLE){
        require(route.length != 0, "Windmill_Trade_Swap_Long: Route have not been set.");
        
        if (increaseSize){
            uint256 nbToken = BUSDToken.balanceOf(thisAddr);
            require(sizeIn <= nbToken, "Windmill_Trade_Swap_Long: Not enough BUSD");
            
            _swap(sizeIn, route, BUSDToken);
        }else{
            uint256 nbToken = token.balanceOf(thisAddr);
            require(sizeIn <= nbToken, "Windmill_Trade_Swap_Long: Not enough token");
            
            _swap(sizeIn, reverseRoute, token);
        }
    }
    
    function getNbLimitOrders() external view returns (uint256){
        return limitOrders.length;
    }

    function getLimitOrder(uint256 id) external view returns (LimitOrder memory){
        uint256 l = limitOrders.length;
        
        require(l < nbMaxLimitOrders, "Windmill_Trade_Swap_Long: Maximum of 10 limit orders reached.");
        require(id < l, "Windmill_Trade_Swap_Long: Limit order id not found.");
        
        return limitOrders[id];
    }
    
    function deleteLimitOrder(uint256 id) external onlyRole(OWNER_ROLE){
        require(id < limitOrders.length, "Windmill_Trade_Swap_Long: Limit order id not found.");
        
        _deleteLimitOrder(id);
    }
    
    function placeLimitOrder(bool increaseSize, uint256 sizeIn, uint256 triggerPrice) external onlyRole(OWNER_ROLE){
        require(route.length != 0, "Windmill_Trade_Swap_Long: Route have not been set.");
        
        LimitOrder memory order;
        order.increaseSize = true;
        order.sizeIn = sizeIn;
    ...
    function updateNeeded() external view override(I_Windmill_Trade_Abstract, Windmill_Trade_Abstract) returns (bool){
        uint256 l = limitOrders.length;
        
        for(uint256 i=0; i<l; i++){
            LimitOrder storage order = limitOrders[i];
            if (_oneUpdateNeeded(order)){
                return true;
            }
        }
    ...
    function estimateBUSDBalance() external view override(I_Windmill_Trade_Abstract, Windmill_Trade_Abstract) returns (uint256){
    ...
    function getTokenPrice(uint256 sizeBUSD) public view returns (uint256){
    ...
    function getTokenPriceFromTokenSize(uint256 sizeToken) public view returns (uint256){
    ...
    function update() external override(I_Windmill_Trade_Abstract, Windmill_Trade_Abstract) onlyRole(UPDATER_ROLE){
        uint256 l = limitOrders.length;
    ...
    ////
    //////////////// Private functions ////////////////
    function _deleteLimitOrder(uint256 i) internal{
        uint256 lastI = limitOrders.length - 1;
        if (i != lastI){
            limitOrders[i] = limitOrders[lastI];
        }
        limitOrders.pop();
    }
    
    function _oneUpdateNeeded(LimitOrder storage order) internal view returns (bool){
    ...
    function _swap(uint256 sizeIn, address[] storage _route, IERC20 token) internal{
        uint256[] memory sizeOut = pancakeRouter.getAmountsOut(sizeIn, _route);
        uint256 sizeOutMin = sizeOut[sizeOut.length-1];
        
        token.approve(address(pancakeRouter), sizeIn);
