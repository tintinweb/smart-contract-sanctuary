...
contract InfomatixTimeLockVesting {
    ...
    //current release
    uint256 private _currentRelease;

    //tokens to be released on first release
    uint256 private _tokensToBeReleasedFirstTerm;
    ...
    constructor(
        ...
        uint256 termsToGo_,
        uint256 firstTermReleaseTokens_
    ) {
        ...
        _currentRelease = 0;
        _tokensToBeReleasedFirstTerm = firstTermReleaseTokens_;
    }
    ...
    function tokensToBeReleasedOnNexReleaseDate() public view virtual returns (uint256) {
        if(_currentRelease == 0) {
            return _tokensToBeReleasedFirstTerm;
        } else {
    ...
    function release() public virtual {
        ...
        require(token().balanceOf(address(this)) > 0, "TokenTimelock: no tokens to release");
        if(_currentRelease == 0) {
            amount = _tokensToBeReleasedFirstTerm;
        } else {
            amount = token().balanceOf(address(this)).div(_termsToGo);
        }

        token().safeTransfer(beneficiary(), amount);

        _nextReleaseTime = addYears(_nextReleaseTime, 1);

        //_nextReleaseTime = _nextReleaseTime.add(300);

        _termsToGo = _termsToGo.sub(1);

        _currentRelease = _currentRelease.add(1);
