...
contract RastaNFT {
    ...
    function NFTOfOwner(address to) public view returns (NFTInfo[] memory) {
        ...
        NFTInfo[] memory NFTs = new NFTInfo[](len);
        for(uint i = 0; i < tokenIds.length; i ++) {
            if(address(_NFTs[tokenIds[i]].owner) == address(to)) {
    ...
    function buy(uint256 tokenId) external {
        NFTInfo storage nft = _NFTs[tokenId];
        require(nft.owner == nftAddr, "!You can't buy this NFT");
        ...
        stakingToken.safeTransferFrom(msg.sender, address(_owner), nft.price);

        emit Buy(msg.sender, nft.price);
    }
    ...
    function unstake(uint256 tokenId) external updateReward(tokenId) {
        NFTInfo storage nft = _NFTs[tokenId];
