...
// Selling without auction
 ...
 function _safeTransferETH(address to, uint256 value) internal returns (bool) {
    ...
    function acceptOffer(uint256 _id) external payable _validateTokenOwner(_id){
        _trade(_id);
address payable _owner = payable(ownerOf(_id));
   ...
   emit AuctionEnded(_owner,bidders[_id],price[_id],_id,tokenURI(_id));
     ...
     function _trade(uint _id) internal {
        ...
        _transfer(_owner,bidders[_id], _id);

        // Fee Cut
        uint _commissionValue = price[_id].mul(feePercent).div(100);
        uint _sellerValue = price[_id] - _commissionValue;
        require(msg.value == price[_id], "Sent ETH Value does not match specified bid amount");
        IWETH(wbnb).withdraw(_sellerValue);
