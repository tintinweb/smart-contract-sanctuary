...
abstract contract BELTToken is ERC20 {
...
contract MasterBelt is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public BELTPerBlock = 1000000000000000000;
    uint256 public startBlock = 5559747;
    ...
    constructor(address _belt) public {
        BELT = _belt;
    }
    ...
    returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && sharesTotal != 0) {
            ...
            uint256 BELTReward =
            multiplier.mul(BELTPerBlock).mul(pool.allocPoint).div(
                totalAllocPoint
            );
    ...
    function updatePool(uint256 _pid) public {
        ...
        BELTToken(BELT).mint(
            owner(),
            BELTReward.mul(ownerBELTReward).div(1000)
        );
        BELTToken(BELT).mint(address(this), BELTReward);
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (user.shares > 0) {
            uint256 pending =
            user.shares.mul(pool.accBELTPerShare).div(1e12).sub(
                user.rewardDebt
            );
            if (pending > 0) {
    ...
    function safeBELTTransfer(address _to, uint256 _BELTAmt) internal {
        ...
        if (_BELTAmt > BELTBal) {
            IERC20(BELT).transfer(_to, BELTBal);
        } else {
            IERC20(BELT).transfer(_to, _BELTAmt);
        }
    }
