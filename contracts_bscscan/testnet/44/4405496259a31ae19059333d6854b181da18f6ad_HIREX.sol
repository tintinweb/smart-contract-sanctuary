...
contract HIREX is Context, IBEP20, Ownable {
	...
	// (Marketing + Liquidity).
    uint256 private totalLiqFee   = liquidityFee.add(buymarketingFee).add(buyresearchFee);
    uint256 private totalBuyFee   = buyliquidityFee.add(marketingFee).add(researchFee);
    uint256 private totalSellFee  = sellliquidityFee.add(sellmarketingFee).add(sellresearchFee);
