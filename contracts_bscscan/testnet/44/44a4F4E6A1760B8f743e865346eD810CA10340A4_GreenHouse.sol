...
contract GreenHouse is Ownable {
    ...
    uint256 constant internal MONYHLY_POOL_TIMER = 900;
    ...
    uint256 constant internal BONUS_POOL_NEW_STAKEHOLDER_TIME_ADDITION = 60;
    uint256 constant internal BONUS_POOL_TIMER_INITIAL = 900;
    ...
    function unstake(uint256 amount) external AttemptToDistrubuteBonusPools {
        ...
        // bool success = trustedToken.transfer(msg.sender, net);
        // require(success, "GreenHouse: unstake transfer");
    ...
    function _bonusPoolLeaderboardPush(address value) internal {
        ...
        _bonusPoolLeaderboardPositionsCount[value]++;
    ...
    function _bonusPoolLeaderboardKick(address stakeholder, uint256 positions) internal {
        ...
        uint256 positionsLeft = positions;
        address[] memory leaderboard = new address[](_bonusPoolLeaderboardUsersCount() - positions);
        for (
            uint256 i = _bonusPoolLeaderboardLast;
            i >= _bonusPoolLeaderboardFirst;
            --i
        ) {
            if (positions == 0)
                break;
            if (_bonusPoolLeaderboard[i] == stakeholder) 
                --positions;
            else
                leaderboard[i - (positions - positionsLeft)] = _bonusPoolLeaderboard[i];
        }
