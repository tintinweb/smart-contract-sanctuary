...
contract AutoReflect is BEP20, Ownable {
    using SafeMath for uint256;
   ...
   AutoReflectDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 public constant MAX_SELL_TRANSACTION_AMOUNT = 5000000 * (10**9);

    uint256 public constant TOKEN_REWARDS_FEE = 5;
    uint256 public constant LIQUIDITY_FEE = 7;
    uint256 public constant TOTAL_FEES = TOKEN_REWARDS_FEE + LIQUIDITY_FEE;
    ...
    // liquidate tokens for ETH when the contract reaches 150k tokens by default
    uint256 public liquidateTokensAtAmount = 150000 * (10**9);

    // whether the token can already be traded
    bool public tradingEnabled;

    function activate() public onlyOwner {
        require(!tradingEnabled, "AutoReflect: Trading is already enabled");
        _swapEnabled = true;
        tradingEnabled = true;
    }
    ...
    event UpdatedpancakeswapV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    constructor() BEP20("Ethereum Vault", "AutoReflect") {
        dividendTracker = new AutoReflectDividendTracker();
    ...
    receive() external payable {
      ...
      function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "AutoReflect: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "AutoReflect: Automated market maker pair is already set to that value");
    ...
    function excludeFromFees(address account) public onlyOwner {
        require(!_isExcludedFromFees[account], "AutoReflect: Account is already excluded from fees");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        ...
        require(newValue != gasForProcessing, "AutoReflect: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (!tradingIsEnabled) {
            ...
            if(!openForPresale){
                require(canTransferBeforeTradingIsEnabled[from], "AutoReflect: This account cannot send tokens until trading is enabled");
            }
        }

            if ((from == pancakeswapV2Pair || to == pancakeswapV2Pair) && tradingIsEnabled) {
                //require(!antiBot.scanAddress(from, pancakeswapV2Pair, tx.origin),  "Beep Beep Boop, You're a piece of poop");
    ...
    function swapAndSendToDev(uint256 tokens) private {
        ...
        address payable _devAndMarketingAddress = payable(0xB2Bd3acfD02b8eDB49d6899a2E37FFfc8855270D);
...
contract AutoReflectDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("AutoReflect_Dividend_Tracker", "AutoReflect_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "AutoReflect_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "AutoReflect_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AutoReflect contract.");
    }
    ...
    function excludeFromDividends(address account) external onlyOwner {
      ...
      function updateGasForTransfer(uint256 newGasForTransfer) external onlyOwner {
        require(newGasForTransfer != gasForTransfer, "AutoReflect_Dividend_Tracker: Cannot update gasForTransfer to same value");
        emit GasForTransferUpdated(newGasForTransfer, gasForTransfer);
        gasForTransfer = newGasForTransfer;
    }

    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "AutoReflect_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "AutoReflect_Dividend_Tracker: Cannot update claimWait to same value");
