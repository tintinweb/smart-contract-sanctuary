...
contract Reflection is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1 * 10**10 * 10**9;
    ...
    address public _communityWallet = payable(address(0x456));
    ...
    constructor(address pinkBot) {
        _rOwned[_msgSender()] = _rTotal;

        buyFee.marketingFee = 2;
        buyFee.communityFee = 1;
        buyFee.taxFee = 2;

        sellFee.marketingFee = 2;
        sellFee.communityFee = 2;
        sellFee.taxFee = 3;
        ...
        pinkAntiBot = IPinkAntiBot(pinkBot);
    ...
    function isExcludedFromCommunity(address account) public view returns (bool) {
    ...
    function excludeFromCommunity(address account) public onlyOwner {
    ...
    function includeInCommunity(address account) external onlyOwner {
        require(_isExcluded[account], "Account is already excluded");
        ...
        )
    {
        ...
        tTransferAmount = tTransferAmount.sub(tCommunity).sub(tMarketing);
        return (tTransferAmount, tFee, tCommunity, tMarketing);
    }
        ...
        )
    {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rCommunity).sub(
            rMarketing
        );
    ...
    function _takeCommunityAndMarketing(uint256 tCommunity, uint256 tMarketing)
        private
    {
        uint256 currentRate = _getRate();
        uint256 rCommunity = tCommunity.mul(currentRate);
        ...
        _rOwned[_communityWallet] = _rOwned[_communityWallet].add(rCommunity);
        _tOwned[_communityWallet] = _tOwned[_communityWallet].add(tCommunity);
        ...
        if (tCommunity > 0 && tMarketing > 0) {
            emit Transfer(msg.sender, _communityWallet, tCommunity);
    ...
    function calculatecommunityFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(_communityFee).div(10**2);
    }
    ...
    function setBuy() private {
        ...
        _communityFee = buyFee.communityFee;
        _marketingFee = buyFee.marketingFee;
    }

    function setSell() private {
        _taxFee = sellFee.taxFee;
        _communityFee = sellFee.communityFee;
    ...
    function _transferStandard(
        ...
        _takeCommunityAndMarketing(
            calculatecommunityFee(tAmount),
