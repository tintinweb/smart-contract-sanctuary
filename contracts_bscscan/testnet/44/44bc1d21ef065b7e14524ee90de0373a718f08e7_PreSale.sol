...
contract PreSale is Ownable {
    ...
    constructor() {
        saleToken = IERC20(0xeB0ce5545E222258656eCe467B0DDA04C22607A4);
        nucleusToken = IERC20(0xD6f81aEBdA7509f074eF8231613fc7D3F2cDe8eD);
        
        totalTokenForSeed    = saleToken.totalSupply().mul(10).div(100);
        totalTokenForVip     = saleToken.totalSupply().mul(4).div(100);
        totalTokenForHolder  = saleToken.totalSupply().mul(4).div(100);
        totalTokenForPublic  = saleToken.totalSupply().mul(2).div(100);
    }
    ...
    function ownerWithdrawToken() public onlyOwner{    
        ...
        saleToken.transfer(msg.sender, withdrawable);
    }
    ...
    function getMemberType(address account) public view returns(uint256){
        ...
        if(listSeed[account]){
            ...
            memType = _TYPE_VIP;
        }else if(nucleusToken.balanceOf(account) > 0){
    ...
    function deposite() public payable {
        require(isSaleStarted, "Not during sale");
        ...
        require(_TOTAL_SOLD_TOKEN.add(numToken) <= saleToken.balanceOf(address(this)), "Do not enough token in contract");
    ...
    function addingVestToken(address account, uint256 amount, uint256 vType) private {
        ...
        if(vType == _TYPE_SEED){
            ...
            saleToken.transfer(account, amount);
            return;
        }
        ...
        saleToken.transfer(account, claimNow);
    }
}