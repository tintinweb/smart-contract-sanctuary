...
contract MoonBear is IERC20Upgradeable, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    address public swapAndDistributor;
    ...
    function initialize(IUniswapV2Router02 _router, address _busd) external initializer {
        ...
        // Initial tax configuration (4% buy and 15% sell fee)
        // Buy fee 4%
        buyFee = 40; //2% marketing , 2% team

        // Sell fee 15%
        burnFee = 30; // 3% BNB for burn
        reflectionFee = 0; // 0% BNB for reflection
        liquidityFee = 10; // 1% BNB for liquidity
        buybackFee = 30; // 3% BNB for buyback
        stableFee = 80; // 8% BUSD (3% dev/marketing, 3% staking, 1% charity, 1% team)
    ...
    function excludeFromBlackList(address account) external onlyOwner {
        require(blackList[account], "Account is already excluded");
        blackList[account] = false;
    }

    function includeInBlackList(address account) external onlyOwner {
        require(!blackList[account], "Account is already included");
    ...
    function setSwapAndDistributor(address _swapAndDistributor) external onlyOwner {
        swapAndDistributor = _swapAndDistributor;
    }
    ...
    function withdrawLP(address _to) external onlyOwner {
        IERC20Upgradeable(bnbPair).safeTransfer(_to, IERC20Upgradeable(bnbPair).balanceOf(address(this)));
    }
    ...
    function _transfer(
        ...
        require(!blackList[from] && !blackList[to], "Blocked");
        ...
        // Is it buy, sell or transfer?
        if (from == bnbPair || from == busdPair) {
            //buy
            taxWhitelist[to] = true;

            if (hotWallets[to] || to == owner() || to == address(this)) {
                _transferNoFee(from, to, amount);
            } else {
                _transferBuyFee(from, to, amount);
            }
        } else {
            // Sell
        ...
        if (
            swapAndDistributor != address(0) && (!hotWallets[from] || !hotWallets[to]) && balanceOf(address(this)) > 0
        ) {
            uint256 sellFeeCharge = balanceOf(address(this));
            _transferNoFee(address(this), swapAndDistributor, sellFeeCharge);
            ISwap(swapAndDistributor).chargeWithSellFee(sellFeeCharge);
        }
    }
    ...
    function _transferBuyFee(
        ...
        uint256 tBuyFee = (tAmount * buyFee) / 1000;
        ...
        uint256 transferedAmount = tAmount - tBuyFee;
        rOwned[recipient] = rOwned[recipient] + rAmount - tBuyFee * currentRate;
        rOwned[swapAndDistributor] = rOwned[swapAndDistributor] + tBuyFee * currentRate;

        ISwap(swapAndDistributor).chargeWithBuyFee(tBuyFee);

        emit Transfer(sender, recipient, transferedAmount);
        emit Transfer(sender, swapAndDistributor, tBuyFee);
    }
    ...
    function withdrawToken(address recipient, uint256 amount) external onlyOwner {
        ...
        require(amount > 0 && amount <= balanceOf(address(this)));
...
interface ISwap {
    function chargeWithBuyFee(uint256 _amount) external;

    function chargeWithSellFee(uint256 _amount) external;
}
