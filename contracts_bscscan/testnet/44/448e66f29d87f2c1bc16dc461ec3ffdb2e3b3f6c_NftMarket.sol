...
contract NftMarket is IERC721Receiver, ReentrancyGuard, Ownable {
    ...
    /// @notice change loot box contract6
    function setLootbox(address lootbox_) external onlyOwner {
        require(lootbox_ != address(0), "Loot box address can not be zero");
        lootbox = lootbox_;
    }

    /// @notice change loot box price
    ...
    /// @param lootboxType type
    function buyOfficial(uint256 lootboxType) public nonReentrant payable {
        require(msg.value >= lootboxPrice, "your price is too low");
        require(salesEnabled, "sales are closed");
        feeReceiver.transfer(lootboxPrice);
        ILootbox lootboxContract = ILootbox(lootbox);
        uint256 tokenId = lootboxContract.buyOfficial(msg.sender, lootboxType);
        
        string memory tokenUri;
        string memory tokenName;
        (tokenUri, tokenName) = ILootbox(lootbox).uniqueTokenInfo(tokenId);
