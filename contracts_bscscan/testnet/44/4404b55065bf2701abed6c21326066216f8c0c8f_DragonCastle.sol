...
contract DragonCastle is
    Ownable,
    ReentrancyGuard,
    ERC20("Dragon Castle", "gDRAGON")
{
    using SafeMath for uint256;
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            for (uint256 i = 0; i < rewardTokens.length; i++) {
                ...
                if (pending > 0) {
                    pending = harvestFee(pending, rewardTokens[i]);
    ...
    function harvestFee(uint256 _amount, ERC20 _token)
        internal
        returns (uint256)
    {
        if(rewardFee <= 0){
        ...
        uint256 harvestedAmt = _amount.mul(rewardFee).div(10000);
        ERC20(_token).transfer(feeCollector, harvestedAmt);
        return _amount.sub(harvestedAmt);
    }
    ...
    function updateUnstakingFee(uint256 _newFee) external onlyOwner {
        require(_newFee >= 0, "Fee must be equal or greater than 0");
    ...
    function updateHarvestFee(uint256 _newFee) external onlyOwner {
        require(_newFee >= 0, "Fee must be equal or greater than 0");
        rewardFee = _newFee;
    }
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            for (uint256 i = 0; i < rewardTokens.length; i++) {
                ...
                adjustedTokenPerShare = accTokenPerShare[rewardTokens[i]].add(
                    dragonReward.mul(PRECISION_FACTOR[rewardTokens[i]]).div(
                        stakedTokenSupply
                    )
                );
        ...
        returns (uint256)
    {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 adjustedTokenPerShare = accTokenPerShare[_token].add(
                dragonReward.mul(PRECISION_FACTOR[_token]).div(
                    stakedTokenSupply
                )
            );
