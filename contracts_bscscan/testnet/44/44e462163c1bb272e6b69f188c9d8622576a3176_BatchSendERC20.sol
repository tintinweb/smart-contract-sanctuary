...
contract BatchSendERC20 {
    ...
    function multiSendFixedToken(IERC20 token, address[] memory recipients, uint256 amount, uint256 decimals) public {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            token.safeTransferFrom(from, recipients[i], amount *10** decimals);
        }
        
    }  
    
    //batch send different token amount from sender, require approval of contract as spender
    function multiSendDiffToken(IERC20 token, address[] memory recipients, uint256[] memory amounts, uint256 decimals) public {
    ...
    function multiSendFixedTokenFromContract(IERC20 token, address[] memory recipients, uint256 amount, uint256 decimals) public onlyOwner {
        require(recipients.length > 0);
        require(amount > 0);
        require(recipients.length * amount <= token.balanceOf(address(this)));
        
        for (uint256 i = 0; i < recipients.length; i++) {
            token.safeTransfer(recipients[i], amount *10** decimals);
        }
    }
    
    //batch send different token amount from contract
    function multiSendDiffTokenFromContract(IERC20 token, address[] memory recipients, uint256[] memory amounts, uint256 decimals) public onlyOwner {
