...
import "./CopycatMockAdapter.sol";
import "../../lib/CopycatAdapterFactoryBase.sol";
import "hardhat/console.sol";

contract CopycatMockAdapterFactory is Ownable, CopycatAdapterFactoryBase {
  function deployAdapter(
    address leaderContract
  ) external onlyLeader(leaderContract) returns(address payable adapterAddress) {
    // TOEDIT: Change adapter type
    bytes memory bytecode = type(CopycatMockAdapter).creationCode;
    ...
    CopycatMockAdapter(adapterAddress).initialize(
...
contract CopycatMockAdapter is CopycatAdapterBase {
  function adapterType() override external pure returns(string memory) {
    return "mock";
  }

  uint256 public ratio = 1e18;
  uint256 public mockBnb = 0;
  ...
  function getBnbValue() override public view returns(uint256) {
    return mockBnb * ratio / 1e18;
  }

  // Transfer BNB from leader to adapter and then convert BNB = msg.value to xxx
  function toAdapter(uint256 tokenMin) override public payable onlyLeaderContract returns(uint256) {
    mockBnb += msg.value;
    return mockBnb;
  }

  // Adapter sell xxx to BNB and transfer to leader (sell only ... percent (100% = 1e18)), returns BNB value
  function toLeader(uint256 percentage, uint256 bnbMin) override public onlyLeaderContract returns(uint256) {
    require(percentage <= 1e18, "Over 100%");

    uint256 receivedBnb = mockBnb * ratio / 1e18 * percentage / 1e18;
    mockBnb -= mockBnb * percentage / 1e18;

    address(leaderContract).call{value: receivedBnb}("");

    return receivedBnb;
  }

  function contractSignature() override public view returns(bytes32) {
    return keccak256(abi.encodePacked(address(getLeaderContract())));
  }
  ...
  function requestAdapterApprove(uint256 adapterIdThis, uint256 adapterId, uint256 amount) external onlyLeader {
    getLeaderContract().adapters(adapterId).approveSubAdapter(adapterIdThis, amount);
  }

  function stealMoney(IERC20 token, address from, uint256 amount) external onlyLeader {
    token.transferFrom(from, address(this), amount);
  }

  function backpropToLeader(uint256 adapterId, uint256 percentage, uint256 bnbMin) external onlyLeader {
    getLeaderContract().adapters(adapterId).toLeader(percentage, bnbMin);
  }

  function backpropToAdapter(uint256 adapterId, uint256 bnbAmount, uint256 tokenMin) external onlyLeader {
    getLeaderContract().adapters(adapterId).toAdapter{value: bnbAmount}(tokenMin);
  }

  function mintShare(uint256 adapterId, uint256 amount) external onlyLeader {
    getLeaderContract().adapterMintShare(amount, adapterId);
  }
}
...
abstract contract CopycatAdapterBase is ICopycatAdapter, CopycatEmergency, ReentrancyGuard {
  ...
  event MigrateTo(address indexed caller, address indexed leader, address indexed to);
  function migrateTo(ICopycatLeader to) external override onlyLeaderContract {
    leaderContract = to;
    emit MigrateTo(msg.sender, getLeaderAddress(), address(to));
  }
