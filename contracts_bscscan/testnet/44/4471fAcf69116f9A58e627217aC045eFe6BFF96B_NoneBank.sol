...
contract NoneBank is ReentrancyGuard {
    using SafeMath for uint256;

    uint256 public constant MAX_BORROW = 7000; // 70%
    uint256 public constant MIN_BORROW = 100; // 1%
    ...
    // user => token => amount
    // mapping(address => mapping(address => uint256)) public userDusts;
    ...
    event Repay(uint256 principal, uint256 interest, uint256 reward);
    event Liquidate(
        uint256 borrowID,
        ...
        returns (bool)
    {
        for (uint256 i = 0; i < _interestRates.length; i++) {
            if (
                _interestRates[i] < 0.01 ether || _interestRates[i] > 50 ether
            ) {
                return false;
            }
        }
        ...
        returns (bool)
    {
        for (uint256 i = 0; i < _durations.length; i++) {
            ...
            if (_durations[i] < 0 || _durations[i] > 21024000) {
                return false;
            }
        }
    ...
    function createLoanApp(
        ...
        bool valid = true;
        for (uint256 i = 0; i < _availableCollateralTokens.length; i++) {
            if (_loanToken == _availableCollateralTokens[i]) {
                valid = false;
                break;
            }
        }
    ...
    function depositLoan(uint256 _id, uint256 amount) public nonReentrant {
        LoanApplication storage app = loanApplications[_id];
        require(app.open == true, "r0");
    ...
    function withdrawLoan(uint256 _id, uint256 amount) public nonReentrant {
        LoanApplication storage app = loanApplications[_id];
    ...
    function borrow(
        ...
        require(app.owner != msg.sender, "r1");
        require(app.loanToken != address(0), "r2");
        ...
        uint256 min = collateralPrice.mul(MIN_BORROW).div(10000);
        require(_borrowAmount >= min, "r8");
        ...
        uint256 fee = OPEN_POSITION_FEE / 2;
        payable(dev).transfer(fee);
        payable(profitShare).transfer(fee);
    ...
    function repay(uint256 bid, uint256 amount) public nonReentrant {
        ...
        // if (b.borrowAmount <= DUST) {
        //     if (b.borrowAmount > 0) {
        //         userDusts[app.owner][app.loanToken] = userDusts[app.owner][
        //             app.loanToken
        //         ].add(b.borrowAmount);
        //         b.borrowAmount = 0;
        //     }
        ...
        uint256 reward = interest.mul(REWARD).div(10000);
        app.loanAmount = app.loanAmount.add(amount.sub(reward));
        gildToken.mint(app.owner, reward);
        gildToken.mint(b.owner, reward);
        gildToken.mint(dev, reward);
        IERC20(app.loanToken).transfer(dev, reward);

        emit Repay(principal, interest, reward);
    }
    ...
    function depositCollateral(uint256 bid, uint256 amount)
        public
        nonReentrant
    {
        Borrow storage b = borrows[bid];
    ...
    function withdrawCollateral(uint256 bid, uint256 amount)
        public
        nonReentrant
    {
        Borrow storage b = borrows[bid];
        ...
        require(IERC20(b.collateralToken).transfer(msg.sender, amount), "r4");
        emit WithdrawCollateral();
    }
    ...
    function liquidate(uint256 bid) public nonReentrant {
        ...
        (uint256 colleteralIn, uint256 totalSwap) = swap(total, path);
        ...
        if (gildToken.totalSupply() < gildToken.CAP()) {
            ...
            gildToken.mint(msg.sender, gildReward);
        ...
        require(IERC20(app.loanToken).transfer(dev, devFee), "r4");
        ...
        if (totalSwap > totalSend) {
            ...
            require(IERC20(app.loanToken).transfer(borrower, backAmount), "r5");
        }
    ...
    {
        uint256[] memory result = IPancakeRouter02(router)
    ...
    // function claimDust(address[] memory tokens) public nonReentrant {
    //     for (uint256 i = 0; i < tokens.length; i++) {
    //         if (userDusts[msg.sender][tokens[i]] > 0) {
    //             uint256 amount = userDusts[msg.sender][tokens[i]];
    //             userDusts[msg.sender][tokens[i]] = 0;
    //             IERC20(tokens[i]).transfer(address(msg.sender), amount);
    //         }
    //     }
    // }
}
...
contract GildToken is ERC20, Ownable {
    ...
    function mint(address to, uint256 amount) public onlyOwner {
        require(totalSupply() < CAP, "max cap!");
