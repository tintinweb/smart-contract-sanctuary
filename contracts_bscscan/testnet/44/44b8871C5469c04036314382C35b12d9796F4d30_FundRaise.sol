...
contract FundRaise is Ownable, ReentrancyGuard {
    ...
    function createPool( PoolInfo memory _poolDetails) external onlyCreator {
        ...
        emit CreatePool(msg.sender, PoolID.sub(1));
    }
    ...
    function claim(uint256 _poolID) external onlyCreator nonReentrant {
        PoolInfo storage pool = poolInfo[_poolID];
    ...
    function transferToHolders(uint256 poolID,IBEP20 _token, address[] calldata recipients, uint256[] calldata amounts) external onlyCreator {
        require(recipients.length == amounts.length,"given details are invalid.");
        rewards[poolID].claimed = true;
