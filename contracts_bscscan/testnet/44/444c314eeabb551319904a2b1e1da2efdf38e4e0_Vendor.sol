...
contract Context {
  ...
  function _msgSender() view public returns (address) {
    return msg.sender;
  }
...
contract Vendor is BEP20Token {
  ...
  */
  function buyTokens() public payable returns (uint256 tokenAmount) {
    ...
    uint256 vendorBalance = bep20Token.balanceOf(address(this));
    require(vendorBalance >= amountToBuy, "Vendor contract has not enough tokens in its balance");

    // Transfer token to the msg.sender
    (bool sent) = bep20Token.transfer(msg.sender, amountToBuy);
  ...
  */
  function withdraw() public onlyOwner {
    ...
    function getDescLength() public view returns(uint count) {
        return desc.length;
    }

    function getResults() public view returns (uint firstPropCount, uint secondPropCount, uint blankVoteCount) {
          return (bep20Token.balanceOf(props.firstPropAddress) ,bep20Token.balanceOf(props.secondPropAddress),bep20Token.balanceOf(props.blankVoteAddress)) ;
    }
