...
contract DmiraiMLM is Ownable {
    ...
    constructor() {
        ...
        usersInfo[0x5f2cFa351B7d4b973d341fdB2cB154794c0a899c].id = 1;
    ...
    modifier onlyBank() {
        require(msg.sender == bankAddress, "DmiraiMLM:: Only bank");
        _;
    }
    ...
    function _addUser(address _user, address _referrer) private {
        usersInfo[_user].id = currentId;
        usersInfo[_referrer].directReferralCount += 1;
    ...
    /**
     * @notice  Function to add DmiraiMLM
     * @param _user Address of user
     * @param _referrerId Address of referrer
     */
    function addDmiraiMLM(address _user, uint256 _referrerId) external onlyBank {
        ...
        require(_user != address(0), "DmiraiMLM::user is zero address");

        require(_referrer != address(0), "DmiraiMLM::referrer is zero address");

        require(
            userToReferrer[_user] == address(0) || userToReferrer[_user] == _referrer,
            "DmiraiMLM::referrer is zero address"
        );

        // If user didn't exsist before
        if (usersInfo[_user].id == 0) {
    ...
    function distributeRewards(
        ...
        if (_wantAddr != 0xDfb1211E2694193df5765d54350e1145FD2404A1) {
            while (index < length && usersInfo[userToReferrer[_user]].id != 1) {
                address referrer = userToReferrer[_user];

                if ((index < usersInfo[referrer].directReferralCount) || (index == 0)) {
                    reward = (_wantAmt * directReferralBonuses[index]) / 100;
