...
contract Friends is Ownable {
    ...
    function invest(uint256 _packetId) public payable {
        ...
        Referral memory _newReferral = Referral({
            user: msg.sender,
            level: 1,
            amount: _earn,
        ...
        referrals[owner()].push(_newReferral);
    ...
    function investByRef(uint256 _packetId, address _referrer) public payable {
        ...
        referrals[_referrer].push(_newReferral);
        ...
        if(_sInvestor != address(0)){
            ...
            Referral memory _newSReferral = Referral({
                user: msg.sender,
                level: 2,
                amount: _sEarn,
                timestamp: block.timestamp
            ...
            referrals[_sInvestor].push(_newSReferral);
            ...
            if(_tInvestor != address(0)){
                ...
                Referral memory _newTReferral = Referral({
                    user: msg.sender,
                    level: 3,
                    amount: _tEarn,
                    timestamp: block.timestamp
                ...
                referrals[_tInvestor].push(_newTReferral);
            }
        }
    ...
    function takeInvestment(uint256 _packetId) public{
       ...
       uint256 _total = investAmount[msg.sender][_packetId].add(investAmount[msg.sender][_packetId].mul(DAILY_REWARD).mul(_endTime.sub(_lastUpdateTime)).div(1 days).div(100));
    ...
    function totalClaimable(address _user, uint256 _packetId) public view returns(uint256) {
        uint256 _endTime = (block.timestamp >= packets[_packetId].finishTime) ? packets[_packetId].finishTime : block.timestamp;
        ...
        return _totalPay.add(refRewards[_user][_packetId]);
    }
    ...
    function getReferrals(address _user) public view returns(Referral[] memory) {
