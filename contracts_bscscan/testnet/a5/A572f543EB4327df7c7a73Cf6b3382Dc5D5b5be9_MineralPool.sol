...
contract MineralPool is Ownable {
    ...
    function stake(uint256 tokenId) public updateRewards(tokenId) {
        ...
        require(tokenStatus == idleStatus, "Stake failed, token not idle");
        ...
        Parmas memory tokenParmas = INFTToken(stakeToken).getNFTParams(tokenId);
        uint256 capacity = ICalcFormula(empowerContract).calcCapacity(tokenParmas.mineralLevel, isMatchPlace);
    ...
    function claimReward(uint256 tokenId) public {
        require(isAccountStakeToken(msg.sender, tokenId), "Claim failed, not stake the token");
    ...
    function withdrawAndClaim(uint256 tokenId) public {
        require(isAccountStakeToken(msg.sender, tokenId), "Withdraw failed, not stake the token");
