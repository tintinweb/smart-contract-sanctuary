...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  string private constant _symbol = "TSWIFTY15";
  ...
  event TaxSwapToBnBFailed(string reason);
  ...
  function swapTokensForBNB(address sourceOfTokens, address receiverOfBnb, uint256 tokenAmount) private {
      ...
      catch Error(string memory _err) { // I want the trade to complete even if the swap fails so i will just emit the event.           
        emit TaxSwapToBnBFailed(_err);
      }
  }
