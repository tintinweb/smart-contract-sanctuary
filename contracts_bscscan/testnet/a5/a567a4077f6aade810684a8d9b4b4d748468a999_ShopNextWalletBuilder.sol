...
contract ShopNextToken is BEP20Detailed, BEP20 {
    using SafeMath for uint256;
    ...
    constructor() BEP20Detailed("TESTNEXT", "TESTNEXT", 18) {
    ...
    function transferFrom(
        ...
        address _receiver,
        uint256 _amount
    ) public override(BEP20, IBEP20) returns (bool) {
...
contract MultiOwner is Ownable {
    ...
    modifier IsOwner() {
       if (!_isSecondaryPromoting)  {
           require(owner() == _msgSender(), "MultiOwner: Sender is not a primary owner");
       }
       if (_isSecondaryPromoting) {
           require(_secondaryOwner == _msgSender(), "MultiOwner: Sender is not a seconday owner");
       }
        _;
    }

    modifier isSecondayOwner() {
        require (_secondaryOwner == _msgSender(), "MultiOwner: Sender is not a seconday owner");
        _;
    }

    modifier IsMultiOwner() {
        require (owner() == _msgSender() || _secondaryOwner == _msgSender(), "MultiOwner: Sender is not a owner");
        _;
    }
    ...
    function changeSecondaryOwner(address _newSecondaryOwner) IsMultiOwner external returns(bool) {
        require(_newSecondaryOwner != address(0), "MultiOwner: address is not a address 0");
        require(owner() != _newSecondaryOwner, "MultiOwner: address is not a current primary owner");
...
contract ShopNextWallet is MultiOwner, IShopNextWallet {
    ...
    function multiSendToken(
        ...
        uint256 total = 0;
        require(receivers.length <= 200, "ShopNextWallet: Limit size of reciever to 200");
        require(amounts.length <= 200, "ShopNextWallet: Limit size of amount to 200");
        require(receivers.length ==amounts.length, "ShopNextWallet: Diff length of array");
         BEP20 token = BEP20(tokenContract);
        uint8 i = 0;
...
import "../util/MultiOwner.sol";
import "../token/ShopNextToken.sol";
...
contract ShopNextWalletBuilder is MultiOwner {
    ...
    mapping(string => string) _lastReleasedTime;
    ...
    function newShopNextWallets() public payable IsOwner returns (bool) {
        ...
        wallets[owner()].push(teamAdvisorWallet);
        _walletCreationDateTime[Constant.SHOPNEXTWALLETYPETEAMADVISOR] = block
            .timestamp;

        // Constant.SHOPNEXTWALLETYPECOMPANY
        address companyWallet = createWallet(Constant.SHOPNEXTWALLETYPECOMPANY);
        wallets[owner()].push(companyWallet);
        _walletCreationDateTime[Constant.SHOPNEXTWALLETYPECOMPANY] = block
            .timestamp;
        ...
        wallets[owner()].push(marketingWallet);
        _walletCreationDateTime[Constant.SHOPNEXTWALLETYPEMARKETING] = block
            .timestamp;
        ...
        wallets[owner()].push(communityWallet);
        _walletCreationDateTime[Constant.SHOPNEXTWALLETYPECOMMUNITY] = block
            .timestamp;

        // Constant.SHOPNEXTWALLETYPESALE
        address saleWallet = createWallet(Constant.SHOPNEXTWALLETYPESALE);
        wallets[owner()].push(saleWallet);
        _walletCreationDateTime[Constant.SHOPNEXTWALLETYPESALE] = block
            .timestamp;
    ...
    function releaseTokenForWallet() public payable IsOwner returns (bool) {
        ...
        releasingTotalAmount += releaseTokenForMarketingWallet(currentTimestamp);

        emit ReleaseTokenForWallets(_tokenAddress, releasingTotalAmount);
        return true;
    }

    function releaseTokenForWalletCustomTime(uint256 currentTimestamp) public payable IsOwner returns (bool) {
        // daily trigger
        // DateTime creationDateTime = DateTime.parseTimestamp(block.timestamp);
        uint256 releasingTotalAmount = 0;
        releasingTotalAmount += releaseTokenForTeamAdvisorWallet(
        ...
        returns (uint256 releasingAmount)
    {
        ...
        uint256 walletCreationDateTime = _walletCreationDateTime[
            Constant.SHOPNEXTWALLETYPETEAMADVISOR
        ];
        uint256 diffMonth = dateTime.diffMonth(
            currentTimestamp,
            walletCreationDateTime
        );
        uint256 dayCurrentDate = dateTime.getDay(currentTimestamp);
        uint256 dayCreationDate = dateTime.getDay(walletCreationDateTime);

        require(dayCurrentDate == dayCreationDate, "");
        ...
        require(
            _totalTokenSize[Constant.SHOPNEXTWALLETYPETEAMADVISOR] >=
                _releasedTokenSize[Constant.SHOPNEXTWALLETYPETEAMADVISOR].add(
                    releasingAmount
                ),
            "ShopNextWalletBuilder: Current releasing token for team advisor wallet is exceeded total "
        );
        _releasedTokenSize[
            Constant.SHOPNEXTWALLETYPETEAMADVISOR
        ] = _releasedTokenSize[Constant.SHOPNEXTWALLETYPETEAMADVISOR].add(
        ...
        address factoryAddress = address(this);
        address walletAddress = _walletByType[
            Constant.SHOPNEXTWALLETYPETEAMADVISOR
        ];
        ...
        _releaseTokenHistory[Constant.SHOPNEXTWALLETYPETEAMADVISOR].push(info);
    }
        ...
        returns (uint256 releasingAmount)
    {
        ...
        require(
            _totalTokenSize[Constant.SHOPNEXTWALLETYPESALE] >=
                _releasedTokenSize[Constant.SHOPNEXTWALLETYPESALE].add(
                    releasingAmount
                ),
            "ShopNextWalletBuilder: Current releasing token for sale advisor wallet is exceeded total "
        );
        _releasedTokenSize[Constant.SHOPNEXTWALLETYPESALE] = _releasedTokenSize[
            Constant.SHOPNEXTWALLETYPESALE
        ].add(releasingAmount);
        address factoryAddress = address(this);
        address walletAddress = _walletByType[Constant.SHOPNEXTWALLETYPESALE];
        bep20.increaseAllowance(factoryAddress, releasingAmount);
        bep20.transferFrom(factoryAddress, walletAddress, releasingAmount);
        ReleaseTokenInfo info = new ReleaseTokenInfo(
            currentTimestamp,
        ...
        _releaseTokenHistory[Constant.SHOPNEXTWALLETYPESALE].push(info);
    }
        ...
        returns (uint256 releasingAmount)
    {
        ...
        require(
            _totalTokenSize[Constant.SHOPNEXTWALLETYPECOMMUNITY] >=
                _releasedTokenSize[Constant.SHOPNEXTWALLETYPECOMMUNITY].add(
                    releasingAmount
                ),
            "ShopNextWalletBuilder: Current releasing token for community wallet is exceeded total "
        );
        _releasedTokenSize[
            Constant.SHOPNEXTWALLETYPECOMMUNITY
        ] = _releasedTokenSize[Constant.SHOPNEXTWALLETYPECOMMUNITY].add(
        ...
        _releaseTokenHistory[Constant.SHOPNEXTWALLETYPECOMMUNITY].push(info);
    }
        ...
        returns (uint256 releasingAmount)
    {
        ...
        if (dayCurrentDate != dayCreationDate) {
            return 0;
        }
        ...
        require(
            _totalTokenSize[Constant.SHOPNEXTWALLETYPECOMPANY] >=
                _releasedTokenSize[Constant.SHOPNEXTWALLETYPECOMPANY].add(
                    releasingAmount
                ),
            "ShopNextWalletBuilder: Current releasing token for company wallet is exceeded total "
        );
        _releasedTokenSize[
            Constant.SHOPNEXTWALLETYPECOMPANY
        ] = _releasedTokenSize[Constant.SHOPNEXTWALLETYPECOMPANY].add(
        ...
        _releaseTokenHistory[Constant.SHOPNEXTWALLETYPECOMPANY].push(info);
    }
        ...
        returns (uint256 releasingAmount)
    {
        ...
        require(
            _totalTokenSize[Constant.SHOPNEXTWALLETYPEMARKETING] >=
                _releasedTokenSize[Constant.SHOPNEXTWALLETYPEMARKETING].add(
                    releasingAmount
                ),
            "ShopNextWalletBuilder: Current releasing token for marketing wallet is exceeded total "
        );
        _releasedTokenSize[
            Constant.SHOPNEXTWALLETYPEMARKETING
        ] = _releasedTokenSize[Constant.SHOPNEXTWALLETYPEMARKETING].add(
        ...
        _releaseTokenHistory[Constant.SHOPNEXTWALLETYPEMARKETING].push(info);
    }

    function buildTime( uint16 year,
        uint8 month,
        uint8 day) public  returns (uint256 builtTimestamp) {
         DateTime dateTime = new DateTime();
         builtTimestamp = dateTime.toTimestamp(year, month, day);
    }
