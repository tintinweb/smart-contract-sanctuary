...
contract Star_Cattle is ERC721Enumerable, Ownable {
    ...
    uint randomSeed = 1;
    event BornNormalBull(address indexed sender_, uint indexed tokenId, uint life_ , uint energy_,uint star,uint[2] parents,uint deadTime, uint attack_,uint defense_ ,uint stamina_);
    
    event BornNormalCow(address indexed sender_,uint indexed tokenId,uint life_,uint energy_,uint star,uint[2] parents,uint deadTime , uint milk_,uint milkRate_);
    
    event BornCreationBull(address indexed sender_, uint indexed tokenId, uint life_ , uint energy_,uint deadTime, uint attack_,uint defense_ ,uint stamina_);
    
    event BornCreationCow(address indexed sender_,uint indexed tokenId,uint life_,uint energy_,uint deadTime,uint milk_,uint milkRate_ );
    
    event AddDeadTime(uint indexed tokenId,uint indexed time);
    constructor() ERC721('Cattle', 'Cattle') {
        myBaseURI = '123456';
    ...
    function safeTransferFrom(
        ...
        if (!isCreation[tokenId]){
            require(!cowInfoes[tokenId].isAdult,'can not transfer adult Cattle');
    ...
    function mintNormall(address player, uint gender_, uint life_, uint energy_,uint star,uint[2] memory parents) public {
        require(gender_ == 1 || gender_ == 2, 'wrong gender');
        ...
        if (cowInfoes[currentId].gender == 1){
            ...
            emit BornNormalBull(player, currentId, cowInfoes[currentId].life, cowInfoes[currentId].energy,star,parents,deadTime[currentId],cowInfoes[currentId].attack,cowInfoes[currentId].defense,cowInfoes[currentId].stamina);
            

        }else{
            cowInfoes[currentId].milk = 60 + rand(26);
            cowInfoes[currentId].milkRate = 60 + rand(26);
            emit BornNormalCow(player, currentId, cowInfoes[currentId].life, cowInfoes[currentId].energy,star,parents,deadTime[currentId],cowInfoes[currentId].milk,cowInfoes[currentId].milkRate);
        }
        currentId ++;
    ...
    function mint(address player) public {
        ...
        cowInfoes[currentId].life = 86 + rand(14);
        cowInfoes[currentId].bornTime = block.timestamp;
        cowInfoes[currentId].energy = 86 + rand(14);
        cowInfoes[currentId].checkTime = block.timestamp;
        cowInfoes[currentId].isAdult = true;
        deadTime[currentId] = block.timestamp + 3600 days;
        if (cowInfoes[currentId].gender == 1){
            cowInfoes[currentId].attack = 86 + rand(14);
            cowInfoes[currentId].stamina = 86 + rand(14);
            cowInfoes[currentId].defense = 86 + rand(14);
            emit BornCreationBull(player, currentId, cowInfoes[currentId].life, cowInfoes[currentId].energy,deadTime[currentId],cowInfoes[currentId].attack,cowInfoes[currentId].defense,cowInfoes[currentId].stamina);
            
        }else{
            cowInfoes[currentId].milk = 86 + rand(14);
            cowInfoes[currentId].milkRate = 86 + rand(14);
            emit BornCreationCow(player, currentId, cowInfoes[currentId].life, cowInfoes[currentId].energy,deadTime[currentId],cowInfoes[currentId].milk,cowInfoes[currentId].milkRate);
        }
        isCreation[currentId] = true;
        currentId ++;
    }

    function checUserCowList(address player) public view returns (uint[] memory){
    ...
    function addDeadTime(uint tokenId,uint time_) external {
        ...
        deadTime[tokenId] += time_;
        emit AddDeadTime(tokenId,time_);
    }


    function tokenURI(uint256 tokenId_) override public view returns (string memory){
        require(_exists(tokenId_), "nonexistent token");
        return string(abi.encodePacked(myBaseURI, "/", tokenId_.toString(), '.jpeg'));
    }
