...
contract SC is ERC20, ERC20Burnable, Ownable {
    using SafeMath for uint256;

    uint256 MAX_TOTAL_SUPPLY         = 69000000000000000000000  * 10 ** 18;
    uint256 MAX_TOKENS_FOR_REWARDS   = 0 * 10 ** 18;
    uint256 MAX_LOCK   = 69000000000000000000000  * 10 ** 18;
    ...
    constructor() ERC20("SamuraiCheems", "SC") {
        addressForMarketing =address(0x2DaE360B994530a9CA0219348b61eD3d32592A2e);
        mintContract=address(this);
        
        _mint(_msgSender(), MAX_TOTAL_SUPPLY.sub(tokensForRewards));
        _mint(mintContract, tokensForRewards);

        unlockTime = block.timestamp + 1*30 minutes;
