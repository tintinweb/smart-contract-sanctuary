...
contract MagicStella is BEP20, Ownable {
	...
	address public buyBackWallet; // sell team wallet & feed algos
    ...
    // Override the system of balance on this contact
	...
	uint256 public maxSellTransactionAmount = (_totalSupply * 1) / 1000; // 10 000 tokens
	// Min amount to transform token in contract to dividends, feed algo & liquify 0.01%
	uint256 public swapTokensAtAmount = (_totalSupply * 1) / 10000; // 1 000 tokens
	...
	address BUSD = 0xE0dFffc2E01A7f051069649aD4eb3F518430B6a4; //TODO Replace by mainnet contract
	...
	constructor() BEP20("MagicStella", "MagicStella", 9) {
		...
		IStellaRouter02 _stellaRouter = IStellaRouter02(
		...
		address _stellaSwapV2Pair = IStellaSwapFactory(_stellaRouter.factory())
			.createPair(address(this), BUSD); //TODO update WBNB to BUSD whereas is possible
	...
	function _setAllEcoSystemWallets() private {
		...
		marketingWallet = 0xFc3a5413c1bbD21a6e50a47568689AF4c122888e; 
		devWallet = 0xC9687FaC5d27bAe82412564032938b9c763F0c5E; 
		teamWallet = 0xad2805b8113066b4f9859689cB8a3fa360fcA1b3; 
		liquidityWallet = 0x1C7CB4E5171434cdF832F0A57b7B240dB789DD5A;
		buyBackWallet = 0xcFB06bAc22107e9Cb741cf6CB94ad2302038742d;
	}

	function _removeRestrictionForEcoSystemWallets(
		IStellaRouter02 _stellaRouter
	) private {
		...
		authorized
	{
		require(
			...
			"MagicStella: setWhiteListAMM:: AMM is a wallet, not a contract"
		);
	...
	function updateDividendTracker(address newAddress) public authorized {
		require(
			...
			"MagicStella: The dividend tracker already has that address"
		);
		...
		require(
			...
			"MagicStella: The new dividend tracker must be owned by the MagicStella token contract"
		);
	...
	function updateHolderSellFactorSetup(
		...
		require(
			holderSellFactorPeriod > 1,
			"MagicStella: holderSellFactorPeriod must be greater than 1."
		);
		require(
			holderSellFactorMaxOfPeriod > 1,
			"MagicStella: holderSellFactorMaxOfPeriod must be greater than 1."
		);
		require(
			holderSellFactorMinPourcent > 10,
			"MagicStella: holderSellFactorMinPourcent must be greater than 10%."
		);
		require(
			holderSellFactorReducePourcentByPeriod <= 20,
			"MagicStella: holderSellFactorReducePourcentByPeriod must be smaller or equal than 20%."
		);
	...
	function updatestellaRouter(address newAddress) external authorized {
		require(
			newAddress != address(stellaRouter),
			"MagicStella: The router already has that address"
		);
	...
	{
		require(
			pair != stellaSwapV2Pair,
			"MagicStella: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
		);
		...
		authorized
	{
		require(
			...
			"MagicStella: The liquidity wallet is already this address"
		);
	...
	function updateBuyBackWallet(address newBuyBackWallet) external authorized {
		require(
			newBuyBackWallet != buyBackWallet,
			"MagicStella: The buyback wallet is already this address"
		);
	...
	function updateFees(
		...
		require(
			localTotalFee > 2500,
			"MagicStella: updateFees:: Liquidity Perc must be equal to or higher than operations and buyback combined."
		);
		...
		emit FeesUpdated(buyBackFee, liquidityFee, dividendFee, _totalFee);
	}
	...
	function updateGasForProcessing(uint256 newValue) external authorized {
		require(
			...
			"MagicStella: gasForProcessing must be between 200,000 and 500,000"
		);
		require(
			newValue != gasForProcessing,
			"MagicStella: Cannot update gasForProcessing to same value"
		);
	...
	function setRewardToken(address rewardTokenAddress) public returns (bool) {
		require(
			...
			"MagicStella: setRewardToken:: Address is a wallet, not a contract."
		);
		require(
			rewardTokenAddress != address(this),
			"MagicStella: setRewardToken:: Cannot set reward token as this token due to Router limitations."
		);
		require(
			!isIgnoredToken(rewardTokenAddress),
			"MagicStella: setRewardToken:: Reward Token is ignored from being used as rewards."
		);
	...
	function setRewardTokenWithCustomAMM(
		...
		require(
			ammContractAddress != address(stellaRouter),
			"MagicStella: setRewardToken:: Use setRewardToken to use default Router"
		);
		require(
			rewardTokenAddress != address(this),
			"MagicStella: setRewardToken:: Cannot set reward token as this token due to Router limitations."
		...
		require(
			...
			"MagicStella: setRewardToken:: AMM is not whitelisted!"
		);
		dividendTracker.setRewardToken(
			msg.sender,
			rewardTokenAddress,
	...
	function buyBackTokensWithNoFees() external payable returns (bool) {
		...
		require(
			...
			"MagicStella: buyBackTokensWithNoFees:: Cannot Spend more than earned."
		);
		...
		stellaRouter.swapExactBNBForTokensSupportingFeeOnTransferTokens{
			value: bnbAmount
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(
			...
			"MagicStella: Automated market maker pair is already set to that value"
		);
		automatedMarketMakerPairs[pair] = value;
	...
	function _checkPrerequisites(
		...
		require(
			...
			"MagicStella: To/from address is ignored"
		);
	...
	function _transfer(
		...
		require(
			...
			"MagicStella: transfer amount exceeds balance"
		);
		...
		if (takeFee) {
			uint256 fees = (amount * _totalFee) / 10000;
			amount -= fees;
	...
	function _executeTransfer(
		...
		require(
			sender != address(0),
			"MagicStella: transfer from the zero address"
		);
		require(
			recipient != address(0),
			"MagicStella: transfer to the zero address"
		);
	...
	function _executeSwap(uint256 tokens, uint256 native) private {
		...
		uint256 swapTokensTotal = swapTokensBuyBack +
			swapTokensDividends +
			swapTokensLiquidity;
	...
	function swapTokensForNative(uint256 tokens) private {
		...
		stellaRouter.swapExactTokensForBNBSupportingFeeOnTransferTokens(
			tokens,
	...
	function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private {
		...
		// add the liquidity
		stellaRouter.addLiquidityBNB{ value: bnbAmount }(
			address(this),
			tokenAmount,
...
contract DividendPayingToken is BEP20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function swapETHForTokens(
        ...
        // make the swap
        try swapRouter.swapExactBNBForTokensSupportingFeeOnTransferTokens{value: bnbAmount}( //try to swap for tokens, if it fails (bad contract, or whatever other reason, send BNB)
