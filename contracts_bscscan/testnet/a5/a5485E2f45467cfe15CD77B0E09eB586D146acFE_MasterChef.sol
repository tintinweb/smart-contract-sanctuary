...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 rewardStartBlock; // Block at which pool is added onto chef
        uint256 rewardEndBlock;   // Block till which rewards will be distributed.
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner {
        ...
        require(lpTokensStatus[_lpToken] != true, "Token already added");
        if (_withUpdate) {
            massUpdatePools();
        }
        uint256 rewardStartBlock = block.number;
        uint256 rewardEndBlock = rewardStartBlock.add(86400);
        uint256 lastRewardBlock = block.number > rewardStartBlock ? block.number : rewardStartBlock;
        totalAllocPoint = totalAllocPoint.add(_allocPoint);
        poolInfo.push(PoolInfo({
            lpToken: _lpToken,
            allocPoint: _allocPoint,
            rewardStartBlock: rewardStartBlock,
            rewardEndBlock: rewardEndBlock,
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 numerator = pool.rewardEndBlock.sub(block.number);
            uint256 denominator = pool.rewardEndBlock.sub(pool.rewardStartBlock);
            uint256 newRewardPerBlock = numerator.mul(TACHIPerBlock).div(denominator);
            if (newRewardPerBlock <= 2) { newRewardPerBlock = 2; }
    ...
    function updatePool(uint256 _pid) public {
        ...
        TACHI.mint(devaddr, TACHIReward.div(10));
