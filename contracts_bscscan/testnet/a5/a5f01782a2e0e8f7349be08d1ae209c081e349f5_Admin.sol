...
interface IXBEP20 {
   ...
   /**
    ...
    function add_to_blacklist(address _addr) external returns (uint256);

    function check_blacklist(address _addr) external view returns (uint256);

    function remove_from_blacklist(address _addr) external returns (uint256);
    ...
    function set_active_status(uint flag) external returns (uint256);
