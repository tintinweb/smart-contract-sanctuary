...
contract ConvertDopple is Ownable {
    ...
    constructor(
        address _dopple,
        address _superDopple,
        address _burnTo
    ) public {
        dopple = _dopple;
        superDopple = _superDopple;
        burnTo = _burnTo;
    }
    ...
    function adminEmergencyWithdraw(uint256 _amount) external onlyOwner {
        ...
        IERC20 _dopple = IERC20(superDopple);
        uint256 _balance = _dopple.balanceOf(address(this));

        require(_balance > 0, "Nothing to withdraw");
        require(_amount <= _balance, "Exceed balance");

        _dopple.transfer(msg.sender, _amount);
    }
    ...
    function convert(uint256 _amount) external {
        ...
        IERC20 _superDopple = IERC20(superDopple);
        ...
        require(
            _superDopple.balanceOf(address(this)) >= _amount,
            "convert: Contract has insufficient SuperDop amount"
        );

        _dopple.safeTransferFrom(msg.sender, burnTo, _amount);
        _superDopple.transfer(msg.sender, _amount);
