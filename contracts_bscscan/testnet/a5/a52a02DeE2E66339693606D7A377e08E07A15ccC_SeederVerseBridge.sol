...
contract SeederVerseBridge is OwnableUpgradeable, ERC721Holder, BridgeOracleClient {
    ...
    function createDownloadRequest(uint256 amount) external payable {
        ...
        requestDownloadSeed(ticketId, amount, msg.sender, msg.value);
    ...
    function processWithdraw(uint256 ticketId, uint256 tokenId, uint256 exp, bool isSuccess) internal override {
    ...
    function processDownload(uint256 requestId, uint256 tokenBalance, bool isSuccess) internal override {
...
abstract contract BridgeOracleClient is IBridgeClient {
    ...
    function requestWithdrawSeedmon(uint256 requestId, uint256 tokenId, uint256 providedGas) internal {
        ...
        oracle.requestWithdrawSeedmon{value: providedGas} (requestId, tokenId, msg.sender);
    }

    function requestDownloadSeed(uint256 requestId, uint256 requestAmount, address downloader, uint256 providedGas) internal {
        require(requestId > 0, "OracleClient: Invalid request Id for requesting download seed token");
        oracle.requestDownloadSeed{value: providedGas} (requestId, requestAmount, downloader);
    }
    ...
    // =========== Process ====================
    function processWithdraw(uint256 requestId, uint256 tokenId, uint256 exp, bool isSuccess) internal virtual {
    }

    function processDownload(uint256 requestId, uint256 tokenBalance, bool isSuccess) internal virtual {
    }
    ...
    // ============ Prophet method ============
    function onWithdrawSeedmonReceived(
        uint256 requestId, 
        uint256 tokenId, 
        uint256 exp, 
        bool isSuccess
        ...
        onlyOracle 
    {
        processWithdraw(requestId, tokenId, exp, isSuccess);
    }

    function onDownloadSeedReceived(
        uint256 requestId, 
    ...
    {
        processDownload(requestId, tokenBalance, isSuccess);
        isCompleted = true;
    }
...
interface IBridgeClient {
    ...
    function onDownloadSeedReceived(uint256 requestId, uint256 tokenBalance, bool isSuccess) external returns (bool isCompleted);
