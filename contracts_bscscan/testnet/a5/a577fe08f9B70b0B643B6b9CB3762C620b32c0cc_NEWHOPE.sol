...
contract NEWHOPE is Context, IBEP20, Ownable {
    ...
    uint256 private _tSionixTotal;

    string private _name = "NEWHOPE";
    string private _symbol = "NEWHOPE";
    ...
    uint256 public _sionixFee = 6;
    uint256 private _previousSionixFee = _sionixFee;
    
    uint256 public _maxTxAmount = 1000000 * 10**6 * 10**9;
    uint256 private minimumTokensBeforeSwap = 1 * 10**4 * 10**9; 
    
    address payable public sionixpoolAddress = 0x98c11ee2346f1Ab63185B92BC83B022feaF4552d; // Sionix Pool Address
    ...
    function totalSionixBNB() public view returns (uint256) {
        // BNB has  18 decimals!
        return _tSionixTotal;
    }
    ...
    function excludeFromReward(address account) public onlyOwner() {
        ...
        require(!_isExcluded[account], "Account is excluded from reward mechanism");
    ...
    function includeInReward(address account) external onlyOwner() {
        require(_isExcluded[account], "Account is included in reward mechanism");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _tSionixTotal = _tSionixTotal.add(address(this).balance);
        TransferSionixBNB(sionixpoolAddress, address(this).balance);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_sionixFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _sionixFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _previousSionixFee = _sionixFee;
    ...
    function setSionixFeePercent(uint256 SionixFee) external onlyOwner() {
        _sionixFee = SionixFee;
    }
    ...
    function TransferSionixBNB(address payable recipient, uint256 amount) private {
