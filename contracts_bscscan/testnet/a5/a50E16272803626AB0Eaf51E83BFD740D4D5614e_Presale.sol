...
// TO DO: withdraw unsold tokens? review liquidity creation to make sure it creates liquidity with only sold token amount (I think fixed)?
// first test - router: EXPIRED - fix deadline? - changed to block.timestamp instead of block.number
// add requirements for functions if presale is failed - added to addliquidity
// fix emergency withdraw - does not set spent to 0, so you can withdraw multiple times - possibly fixed
...
contract Presale is Ownable, Whitelisted, ReentrancyGuard {
    ...
    uint256 public novaStage = 300; //1800 = 30 minutes for nova/snova holders to buy first
    uint256 public lockPeriod; //time to lock liquidity 2592000 = 30 days
    ...
    uint256 public minBuy = 100000000000000; // .1 bnb 100000000000000000
    uint256 public minSNova = 1000000000000000000; // 100 sNova 100000000000000000000
    uint256 public minNova = 5000000000000000000; // 500 Nova 500000000000000000000
    ...
    function addLiquidity () external onlyOwner nonReentrant {
        ...
        require(canClaim != false, 'Liquidity already added');
        require(presaleFailed != false, 'Presale has been Failed');
        ...
        (address(token), tokenLiquidity, tokenLiquidity, bnbLiquidity, address(this), deadline);

        unlockTime = block.timestamp + lockPeriod;
        
        TransferHelper.safeTransferFrom(address(token), address(this), address(feeManager), feeToken);
    ...
    function widthdrawLiquidity () external nonReentrant {
        ...
        TransferHelper.safeTransferFrom(address(liqPair), address(this), address(Payee), liqAmount);

    }

 

    // review this to determine if we should define parameters. For example, is there an issue if you set the new start after the old end?
    function updateStart(uint256 newStart) public onlyOwner {
        startBlock = newStart;  
    }

    function updateEnd(uint256 newEnd) public onlyOwner { // can only end presale early, not extend, but may need to fix this
