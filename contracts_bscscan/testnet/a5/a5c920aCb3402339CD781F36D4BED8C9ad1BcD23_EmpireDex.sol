...
contract EmpireDex is ERC20, Ownable {
    ...
    IERC20 public immutable empirev1;
    ...
    constructor(IERC20 _empirev1, address _WBNB) public ERC20("EmpireDEX", "EMPIRE") Ownable() {
        empirev1 = _empirev1;
        WBNB = _WBNB;
    }

    function createPair(IEmpireFactory _factory) external onlyOwner() correctPhase(MigrationPhase.SETUP) {
        PairType pairType =
            address(this) < WBNB
    ...
    function replayDeposits(address[] calldata contributors, uint256[] calldata amounts) external onlyOwner() correctPhase(MigrationPhase.SETUP) {
        require(contributors.length == amounts.length, "EmpireV2::replayDeposits: Incorrect Arguments");
    ...
    function setTotalRaised(uint256 _totalRaised) external onlyOwner() correctPhase(MigrationPhase.SETUP) {
        totalRaised = _totalRaised;
    }

    function complete() external payable onlyOwner() correctPhase(MigrationPhase.SETUP) {
        uint256 _available = msg.value;

        IWBNB(WBNB).deposit{value: _available}();

        _mint(empireWbnbPair, 6000 * 1 ether); //  60% / 6000 for LGE

        IERC20(WBNB).safeTransfer(empireWbnbPair, _available);
    ...
    function extractLegacyEmpire(address to, uint256 amount) external onlyOwner() {
        empirev1.safeTransfer(to, amount);
    }
    ...
    function claim() correctPhase(MigrationPhase.STARTED) external {
        require(
            contributions[msg.sender] > 0,
            "EmpireV2::claim: No contribution detected!"
        );
        uint256 approval = empirev1.allowance(msg.sender, address(this));
        require(approval > 0, "EmpireV2::claim: Previous contract was not claimed!");
        ...
        empirev1.safeTransferFrom(msg.sender, address(this), approval);
    }
