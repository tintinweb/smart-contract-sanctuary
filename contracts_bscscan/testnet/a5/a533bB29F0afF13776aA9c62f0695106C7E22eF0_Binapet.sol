...
contract Binapet is Context, IERC20, IERC20Metadata, Ownable {
    ...
    uint256 private _previousCharityFee = dexTaxFee;
    ...
    function restoreAllFee() private {
        ...
        dexTaxFee = _previousCharityFee;
    }
    
        function calculateCharityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(dexTaxFee).div(
            10**2
        );
    }
        ...
        _rOwned[taxAddress] = _rOwned[taxAddress].add(rCharity);
        if(_isExcluded[taxAddress])
            _tOwned[taxAddress] = _tOwned[taxAddress].add(tCharity);
    }
    
        function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tCharity) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount) = _getRValues(tAmount, tCharity, _getRate());
    ...
    function _getRValues(uint256 tAmount, uint256 tCharity, uint256 currentRate) private pure returns (uint256, uint256) {
    ...
    function _getCurrentSupply() private view returns(uint256, uint256) {
        uint256 rSupply = _initSupply;
        uint256 tSupply = _totalSupply;      
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_rOwned[_excluded[i]] > rSupply || _tOwned[_excluded[i]] > tSupply) return (_initSupply, _totalSupply);
            rSupply = rSupply.sub(_rOwned[_excluded[i]]);
            tSupply = tSupply.sub(_tOwned[_excluded[i]]);
        }
        if (rSupply < _initSupply.div(_totalSupply)) return (_initSupply, _totalSupply);
