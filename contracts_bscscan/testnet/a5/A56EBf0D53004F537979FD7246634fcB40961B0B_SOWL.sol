...
contract SOWL is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    function liquidityMining(uint256 amount) public payable{
        ...
        Utils.addLiquidity(address(pancakeRouter), _msgSender(), amount, msg.value);
    ...
    function getSellFee() public view returns (uint256) {
        uint256 remainingTime = sellFeeMultiplierTriggeredAt[_msgSender()].add(sellFeeMultiplierLength).sub(block.timestamp);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if (!takeFee)
            ...
            if(sellFeeMultiplierTriggeredAt[_msgSender()].add(sellFeeMultiplierLength) > block.timestamp){
        ...
        if (contractTokenBalance >= _maxTxAmount) {
            ...
            if (buyBackEnabled && initialBalance > uint256(buyBackLowerLimit)) {
                ...
                if(currentBuyBackBurn <= buyBackBurnLimit){
                    ...
                    currentBuyBackBurn = currentBuyBackBurn.add(afterBalance.sub(beforeBalance));

                    emit BuyBack(deadAddress, initialBalance.div(10));
                }else{
                    Utils.swapETHForTokens(address(pancakeRouter), address(this), initialBalance.div(10));
