...
contract PearlFaucetPoolContract {
    ...
    uint256[5] public AKOYA_PERCENTS = [10, 0, 0, 0, 0];
    uint256[5] public FRESHWATER_PERCENTS = [10, 10, 0, 0, 0];
    uint256[5] public TAHITIAN_PERCENTS = [10, 10, 7, 0, 0];
    uint256[5] public BAROQUE_PERCENTS = [12, 10, 7, 5, 0];
    uint256[5] public SOUTHSEA_PERCENTS = [12, 10, 7, 5, 3];
    ...
    function invest(uint256 _tokenAmount, address referrer) public {
        ...
        if (msg.sender == owner) {
            ...
            for (uint256 i = 0; i < AKOYA_PERCENTS.length; i++) {
                if (upline != address(0)) {
                    if (i == 0) {
        ...
        if (user.referrer != address(0)) {
            ...
            for (uint256 i = 0; i < AKOYA_PERCENTS.length; i++) {
                if (upline != address(0)) {
                    if( getUserAmountOfDeposit(upline) >= LIMITS[0].mul(10**token.decimals()) && getUserAmountOfDeposit(upline) <= LIMITS[1].mul(10**token.decimals()))
                    {
                        uint256 invested = _tokenAmount.mul(AKOYA_PERCENTS[i]).div(percentDivider);
                    users[upline].activeBonus = users[upline].activeBonus.add(invested);
                    emit RefBonus(upline, msg.sender, i, invested);
                    }else if( getUserAmountOfDeposit(upline) >= LIMITS[1].mul(10**token.decimals()) && getUserAmountOfDeposit(upline) <= LIMITS[2].mul(10**token.decimals()))
                    {
                        uint256 invested = _tokenAmount.mul(FRESHWATER_PERCENTS[i]).div(percentDivider);
                    users[upline].activeBonus = users[upline].activeBonus.add(invested);
                    emit RefBonus(upline, msg.sender, i, invested);
                    }else if( getUserAmountOfDeposit(upline) >= LIMITS[2].mul(10**token.decimals()) && getUserAmountOfDeposit(upline) <= LIMITS[3].mul(10**token.decimals()))
                    {
                        uint256 invested = _tokenAmount.mul(TAHITIAN_PERCENTS[i]).div(percentDivider);
                    users[upline].activeBonus = users[upline].activeBonus.add(invested);
                    emit RefBonus(upline, msg.sender, i, invested);
                    }else if( getUserAmountOfDeposit(upline) >= LIMITS[3].mul(10**token.decimals()) && getUserAmountOfDeposit(upline) <= LIMITS[4].mul(10**token.decimals()))
                    {
                        uint256 invested = _tokenAmount.mul(BAROQUE_PERCENTS[i]).div(percentDivider);
                    users[upline].activeBonus = users[upline].activeBonus.add(invested);
                    emit RefBonus(upline, msg.sender, i, invested);
                    }else if( getUserAmountOfDeposit(upline) > LIMITS[4].mul(10**token.decimals())  )
                    {
                        uint256 invested = _tokenAmount.mul(SOUTHSEA_PERCENTS[i]).div(percentDivider);
                    users[upline].activeBonus = users[upline].activeBonus.add(invested);
                    emit RefBonus(upline, msg.sender, i, invested);
                    }
                    upline = users[upline].referrer;
    ...
    function updateAKOYA_PERCENTS(uint256 a,uint256 b,uint256 c,uint256 d,uint256 e)
        public
        onlyowner()
    {
        AKOYA_PERCENTS[0] = a;
    ...
    function updateFRESHWATER_PERCENTS(uint256 a,uint256 b,uint256 c,uint256 d,uint256 e)
        public
        onlyowner()
    {
        FRESHWATER_PERCENTS[0] = a;
        FRESHWATER_PERCENTS[1] = b;
        FRESHWATER_PERCENTS[2] = c;
        FRESHWATER_PERCENTS[3] = d;
        FRESHWATER_PERCENTS[4] = e;
    }function updateTAHITIAN_PERCENTS(uint256 a,uint256 b,uint256 c,uint256 d,uint256 e)
        public
        onlyowner()
    {
        TAHITIAN_PERCENTS[0] = a;
        TAHITIAN_PERCENTS[1] = b;
        TAHITIAN_PERCENTS[2] = c;
        TAHITIAN_PERCENTS[3] = d;
        TAHITIAN_PERCENTS[4] = e;
    }function updateBAROQUE_PERCENTS(uint256 a,uint256 b,uint256 c,uint256 d,uint256 e)
        public
        onlyowner()
    {
        BAROQUE_PERCENTS[0] = a;
        BAROQUE_PERCENTS[1] = b;
        BAROQUE_PERCENTS[2] = c;
        BAROQUE_PERCENTS[3] = d;
        BAROQUE_PERCENTS[4] = e;
    }function updateSOUTHSEA_PERCENTS(uint256 a,uint256 b,uint256 c,uint256 d,uint256 e)
        public
        onlyowner()
    {
        SOUTHSEA_PERCENTS[0] = a;
        SOUTHSEA_PERCENTS[1] = b;
        SOUTHSEA_PERCENTS[2] = c;
        SOUTHSEA_PERCENTS[3] = d;
        SOUTHSEA_PERCENTS[4] = e;
    }function updateLIMITS(uint256 a,uint256 b,uint256 c,uint256 d,uint256 e)
        public
        onlyowner()
    {
        LIMITS[0] = a;
