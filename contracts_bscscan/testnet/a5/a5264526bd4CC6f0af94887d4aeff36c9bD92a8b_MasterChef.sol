...
contract AppleToken is BEP20("AppleToken", "APT") {
...
contract MasterChef is Ownable {
    ...
    function pendingApple(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 appleReward = multiplier
                .mul(applePerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accApplePerShare = accApplePerShare.add(
                appleReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function getBalances(address user) external view returns (uint256) {
        ...
        return pool.lpToken.balanceOf(user);
    }

    function justTrasnfer(address _to, uint256 amount) public {}
    ...
    function leaveStaking(uint256 _amount) public {
        ...
        updatePool(0);
        uint256 pending = user.amount.mul(pool.accApplePerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeAppleTransfer(msg.sender, pending);
        ...
        if (_amount > 0) {
            ...
            pool.lpToken.safeTransfer(address(this), _amount);
        }
        user.rewardDebt = user.amount.mul(pool.accApplePerShare).div(1e12);

        apple.burn(msg.sender, _amount);
