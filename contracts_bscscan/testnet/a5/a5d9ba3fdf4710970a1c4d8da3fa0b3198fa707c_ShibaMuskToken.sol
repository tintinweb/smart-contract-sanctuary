    ...
    ITransferLockToken
{
    ...
    function buyICO() external payable {
        ...
        uint256 totalBUSDConverted = msg.value.mul(10**_decimals).div(
    ...
    function getAvailableBalance(address account, uint256 currentTimestamp)
        ...
        returns (uint256)
    {
        uint256 availableAmount = _balances[account].sub(
            users[account].amountICO
        );
    ...
    function transferLockToken(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        users[recipient].amountICO = users[recipient].amountICO.add(amount);
        ...
        returns (uint256)
    {
        ...
        uint256 claimAmount = users[account].amountICO.div(1e18).mul(
    ...
    function _buy(address sender, uint256 buyAmountToken) internal {
        users[sender].amountICO = users[sender].amountICO.add(buyAmountToken);
