...
contract ZYC is Context, IBEP20, Ownable {
    ...
    address payable private _liquidityAddress =
        payable(0xFFeB0aC54576fd1658f68C93A167e72ef05EeF32);

    address payable private _buybackAddress =
        payable(0x2FD3dc1D2879E0a056761CA3f9F038e5a9fEd458);
    ...
    string private _name = "ZiYang Coin";
    string private _symbol = "ZYC";
    ...
    uint256 private _intervalMinutes = 10;
    uint256 private _intervalSecondsForSwap = _intervalMinutes * 1 minutes;

    uint256 public _maxTxAmount = 10000000000 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 100000000 * 10**9;
    uint256 private numTokensBurn = 9999500000 * 10**9;
    ...
    function setIntervalMinutes(uint256 intervalMinutes) external onlyOwner {
        _intervalMinutes = intervalMinutes;
        _intervalSecondsForSwap = _intervalMinutes * 1 minutes;
    }
    ...
    function numTokensSellToAddToLiquidityAmount()
        public
        view
        returns (uint256)
    {
    ...
    function _transfer(
        ...
        if (_startTimeForSwap == 0) {
            if (from == _liquidityAddress) {
                _startTimeForSwap = block.timestamp;
            } else if (!_isExcludedFromFee[from] && to == pancakeSwapV2Pair) {
                revert("Don't sell at the current time");
            }
        } else {
            if (
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(_buybackAddress, buybackBalance);
    ...
    function addressChange(
        ...
        require(
            ...
            "cant set marketing address 0"
        );
        require(buybackAddress != address(0), "cant set buyback address 0");
        require(
            liquidityAddress != address(0),
            "cant set liquidityAddress address 0"
        );
    ...
    {
        return (_liquidityAddress, _buybackAddress, _marketingWalletAddress);
    }
