...
contract OneBuy {
    ...
    function setConfig(uint256[] memory nums) onlyOwner public {
        usdtAmount = nums[0];
        starttime = nums[1];
        endtime = nums[2];
    }
    
    function setReceiver(address addr) onlyOwner public {
        receiveAddr = addr;
    }
    ...
    function getMyConfig(address addr) public view returns (address[] memory, uint256[] memory) {
        ...
        uint256[] memory nums = new uint256[](8 + nftBalances);
        // usdtBalance usdtAllowance usdtDecimals NftPrice,
        ...
        // NftBalance -> every...
        nums[6] = OneNft(nftContract).totalSupply();
        nums[7] = nftBalances;
        for (uint256 i=0; i<nftBalances; i++) {
            nums[8 + i] = OneNft(nftContract).tokenOfOwnerByIndex(addr, i);
        }
        return (addrs, nums);
    }
