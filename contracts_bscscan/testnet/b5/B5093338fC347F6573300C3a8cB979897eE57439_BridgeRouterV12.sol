...
contract BridgeRouterV12 is Ownable {
    ...
    function swap(
		...
		IBridgePool(pool).swap(msg.sender,
			token, targetNetwork,
			targetToken, swapTargetTokenTo,
			targetAddress, token);
	}
	...
	external {
		IBridgePool(pool).withdrawSigned(
			token, payee, amount, swapToToken, sourceChainId, swapTxId, multiSignature);
	}
	...
	function withdrawSignedAndSwap(
		...
		bytes memory multiSignature)
	external {
		IBridgePool(pool).withdrawSigned(path[0],
			address(this),
			amountIn,
			path[path.length - 1],
			sourceChainId,
    ...
    function _swapAndCross(
		...
		IBridgePool(pool).swap(to, crossToken,
			crossTargetNetwork, crossTargetToken, crossSwapTargetTokenTo,
			crossTargetAddress,
			path[0]);
	}
