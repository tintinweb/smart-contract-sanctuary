...
contract Exchange is EIP712, Ownable {
    using ECDSA for bytes32;

    string public constant name = "Exchange";
    address public constant WEKTA = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    ...
    bytes4 constant public AUCTION_SALE_CLASS = bytes4(keccak256("AUCTION"));
    ...
    bytes32 constant ORDER_TYPEHASH = keccak256(
        "Order(address seller,bytes4 saleType,address tokenAddress,uint256 tokenId,uint256 amount,uint256 startTime,uint256 endTime,uint256 nonce)"
    );
    bytes32 constant BID_ORDER_TYPEHASH = keccak256(
        "BidOrder(address buyer,uint256 bidAmount)"
    );

    event OrderCreated(Order order, bytes32 hash, address signer);
    event Buy(address seller, address tokenAddress, uint256 tokenId);
    event Signer(address signer, address buyer);
     ...
     */
    function createOrder(Order memory order, bytes calldata signature) external {
        // verify signature
        bytes32 hashTypedData = genOrderHash(order);
        ...
        // get unique key
        bytes32 hashKey = genHashKey(order);
        // update order status
     ...
     */
    function completeFixedSale(Order memory order) external payable {
        // get unique key
        bytes32 hashKey = genHashKey(order);
        ...
        require(msg.value >= order.amount, "Exchange: Wrong amount");

        // transfer ETH from buyer to seller
        (bool success, ) = payable(order.seller).call{value: order.amount}("");
        require(success, "Exchange: ETH transfer failed");
        ...
        emit Buy(order.seller, order.tokenAddress, order.tokenId);
    }
     ...
     * Emits a {Buy} event, indicating the seller, token address and token id.
     */
    function completeBidding(BidOrder memory bidOrder, bytes calldata signature) external onlyOwner{
        // verify signature
        bytes32 hashTypedData = genBidOrderHash(bidOrder);
        address signer = verifySignature(hashTypedData, signature);

        emit Signer(signer, bidOrder.buyer);
    }

    receive() external payable {}

    function genOrderHash(Order memory order) internal view returns(bytes32) {
        bytes32 structHash = keccak256(
            abi.encode(ORDER_TYPEHASH, order.seller, order.saleType, order.tokenAddress, order.tokenId, order.amount, order.startTime, order.endTime, order.nonce)
        );
        return _hashTypedDataV4(structHash);
    }

    function genBidOrderHash(BidOrder memory bidOrder) internal view returns(bytes32) {
        bytes32 structHash = keccak256(
            abi.encode(BID_ORDER_TYPEHASH, bidOrder.buyer, bidOrder.bidAmount)
        );
        return _hashTypedDataV4(structHash);
    }
    ...
    function genHashKey(Order memory order) internal pure returns(bytes32) {
        bytes32 hashKey = keccak256(
            abi.encode(order.seller, order.saleType, order.tokenAddress, order.tokenId, order.amount, order.startTime, order.endTime, order.nonce)
        );
        return hashKey;
    }
