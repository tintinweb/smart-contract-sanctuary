...
// File: contracts/PaladinsPresale.sol
...
contract PaladinsPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	// Price of presale Paladins, 10 BUSD
	uint256 BUSDPerToken = 10 ** 18;
	// Amount of BUSD received in presale
	uint256 busdReceived = 0;

    uint256 maxHardCap = 100000000 * 10 ** 18 * 10 ** 9;

    uint256 public minimumDepositEthAmount = 10 * 10 ** 18 * 10 ** 9;
    uint256 public maximumDepositEthAmount = 1000 * 10 ** 18 * 10 ** 9;
	...
	function buy(uint256 _amount) public nonReentrant {
		...
		uint256 tokens = _amount.div(BUSDPerToken).mul(10 ** 9);
	...
	function getPaladinsTokensLeft() external view returns (uint256) {
		return Paladins.balanceOf(address(this));
	}

	function claimTokens() external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[msg.sender] > 0, "User should own some Paladins tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed Paladins tokens");
		require(Paladins.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough Paladins tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
		Paladins.safeTransfer(msg.sender, callerTokensUnclaimed);
	...
	function withdrawUnsoldPaladins() external onlyOwner {
		uint256 amount = Paladins.balanceOf(address(this)) - totalTokensSold;
		Paladins.safeTransfer(msg.sender, amount);
	}

	function withdrawAllPaladins() external onlyOwner {
		Paladins.safeTransfer(msg.sender, Paladins.balanceOf(address(this)));
	}
}