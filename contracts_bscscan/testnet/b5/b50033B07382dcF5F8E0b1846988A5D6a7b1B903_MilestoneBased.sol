...
contract MilestoneBased {
  struct PartialVotingInitializationSettings {
  ...
  /// @notice Returns the address of an upgradable beacon proxy for a roadmap contract
  /// @return Address of an upgradable beacon proxy for a roadmap contract
  UpgradeableBeacon public immutable roadmapBeacon;
  /// @notice Returns the address of an upgradable beacon proxy for a voting contract
  /// @return Address of an upgradable beacon proxy for a voting contract
  UpgradeableBeacon public immutable votingBeacon;
  /// @notice Returns if a particular address is a roadmap created by this factory
  ...
  constructor(UpgradeableBeacon _roadmapBeacon, UpgradeableBeacon _votingBeacon)
  {
  ...
  /// uint256 id - id of a deployed roadmap.
  /// address fundingToken - ERC20 contract to be used for a funding functionality.
  /// address refunding - address that is used for storing funds in a case of roadmap refusal.
  /// Roadmap.FundsReleaseType fundsReleaseType - chooses a funds release type logic.
  /// from a MilestoneStartDate(2), MilestoneEndDate(3) set.
  ...
  function createRoadmap(
    Roadmap.InitializationSettings calldata roadmapSettings,
    PartialVotingInitializationSettings calldata votingSettings
  ) external {
    BeaconProxy roadmap = new BeaconProxy(address(roadmapBeacon), "");
    BeaconProxy voting = new BeaconProxy(address(votingBeacon), "");

    Roadmap(address(roadmap)).initialize(address(voting), roadmapSettings);
    Voting.InitializationSettings memory fullVotingSettings = Voting
      .InitializationSettings({
        votingStrategy: votingSettings.votingStrategy,
        ipfsVotingDetails: votingSettings.ipfsVotingDetails,
        timeLockBeforeVoting: 0,
        votingDuration: votingSettings.votingDuration,
        timeLockBeforeExecution: 0,
        minConsensusVotersCount: 0,
        minConsensusVotingPower: votingSettings.minConsensusVotingPower
      });
    Voting(address(voting)).initialize(address(roadmap), fullVotingSettings);
...
contract Voting is Initializable, ReentrancyGuardUpgradeable {
  ...
  struct InitializationSettings {
    IVotingStrategy votingStrategy;
    bytes ipfsVotingDetails;
    uint64 timeLockBeforeVoting;
    uint64 votingDuration;
    uint64 timeLockBeforeExecution;
  ...
  /// @notice Returns a roadmap this voting is related to
  /// @return Address of a roadmap this voting is related to
  address public roadmap;
  /// @notice Returns duration of a window after the proposal is created before the voting stage starts
  ...
  /// @notice Returns duration of a voting stage
  /// @return Seconds that are the duration of a voting stage
  uint64 public votingDuration;
  /// @notice Returns duration of a window after the voting stage before the execution stage starts
    ...
    initializer
  {
    ...
    timeLockBeforeVoting = settings.timeLockBeforeVoting;
    votingDuration = settings.votingDuration;
    timeLockBeforeExecution = settings.timeLockBeforeExecution;
    minConsensusVotersCount = settings.minConsensusVotersCount;
