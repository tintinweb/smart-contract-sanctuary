...
contract Minter is Ownable {
    ...
    ///////addresses///////////////////////
    address payable public benefeciary;
    ///////////////////////////////////////
    ...
    // IBSCswapRouter01 public router = IBSCswapRouter01(0xbd67d157502A23309Db761c41965600c2Ec788b2);
    //////////////////////////////////////////////////////////////////////////////////////
    ...
    /////////////////////////////////events////////////////////////////////////////////////////////
    event NFT721Minted(IERC721 _collection, uint256 _id, address payable _creator);
    event NFT1155Minted(IERC721 _collection, uint256 _id, uint256 _value, address payable _creator);
    event FeesPaid(address payable from, address payable _to, uint256 _value, IERC20 _paidIn);
    ///////////////////////////////////////////////////////////////////////////////////////////////

    modifier getPayments(bool isBNB){
        uint256[2] memory fees = requiredFee(mallFee,bnbFee);
        if(isBNB){
            require(msg.value>=fees[1],"You must pay a the fee to mint tokens");
            benefeciary.transfer(fees[1]);
            payable(msg.sender).transfer(msg.value.sub(fees[1]));
        }
        else{
            erc20TransferProxy.erc20safeTransferFrom(mall,_msgSender(),benefeciary,fees[0]);
        }
        _;
    }

    receive() external payable {
        benefeciary.transfer(msg.value);
    }

    function setReceivers(address payable _benefeciary) external onlyOwner{
    ...
    function mint721(IERC721Mintable _collection, string memory token_uri, uint256 _royalty, bool isBNB) external payable getPayments(isBNB) {
        uint256 _id = _collection.mintToken(_msgSender(), token_uri);
        /**if(!_collection.isApprovedForAll(transferProxy)){
            _collection.setApprovalForAll(transferProxy());
        }
        transferProxy.erc721safeTransferFrom(_collection,address(this),_msgSender(),_id);**/
        nftStorage.addNFT(address(_collection), _id, payable(_msgSender()), _royalty);
    }

    // function mint1155(IERC1155Mintable _collection,string memory token_uri,uint256 _supply,uint256 _royalty, bool isBNB) external payable getPayments(isBNB){
    //     uint256 _id = _collection.mintToken(_msgSender(),token_uri,_supply,"");
    //     /**if(!_collection.isApprovedForAll(transferProxy)){
    //         _collection.setApprovalForAll(transferProxy());
    //     }
    //     transferProxy.erc1155safeTransferFrom(_collection,address(this),_msgSender(),_id,_supply);**/
    //     nftStorage.addNFT(address(_collection),_id,_msgSender(),_royalty);
    // }

    function generate721(ITokenFactory _factory, string memory contract_uri, string memory name, string memory symbol, bool isBNB) external payable getPayments(isBNB) {
        _factory.createERC721(_msgSender(), address(this), contract_uri, name, symbol);
    }

    // function generate1155(ITokenFactory _factory, string memory uri,string memory contract_uri,bool isBNB) external payable getPayments(isBNB){
    //         _factory.createERC1155(_msgSender(),address(this),uri,contract_uri);
    // }
