...
contract CAKEMiner{
  ...
  using SafeBEP20 for IBEP20;
    ...
    function sellEggs() public {
        ...
        cake.transferFrom(address(this),ceoAddress,fee);
        cake.transferFrom(address(this),msg.sender,(eggValue.sub(fee)));

    }
    function buyEggs(address ref, uint256 amountCAKE) public {
        require(initialized);
        cake.transferFrom(msg.sender,address(this),amountCAKE);
        uint256 eggsBought=calculateEggBuy(amountCAKE,cake.balanceOf(address(this)).sub(amountCAKE));
        eggsBought=eggsBought.sub(devFee(eggsBought));
        uint256 fee=devFee(amountCAKE);
        cake.transferFrom(address(this),ceoAddress,fee);
    ...
    function calculateEggBuy(uint256 _cake,uint256 contractBalance) public view returns(uint256){
        return calculateTrade(_cake,contractBalance,marketEggs);
    }
    function calculateEggBuySimple(uint256 _cake) public view returns(uint256){
        return calculateEggBuy(_cake,cake.balanceOf(address(this)));
    }
