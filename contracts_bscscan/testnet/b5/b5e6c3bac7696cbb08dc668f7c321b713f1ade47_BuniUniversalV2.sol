...
contract BuniUniversalV2 is Initializable, AccessControlUpgradeable {
    ...
    ITrainerRoller public constant TRAINER_ROLLER_CONTRACT = ITrainerRoller(0x09D9557C8a98c91F3A5737E132Cb8E863Db41D4d);
    IBunicornRoller public constant BUNICORN_ROLLER_CONTRACT = IBunicornRoller(0x29598B3EB06fdC462b083ED2b1BeAB348E562fc8);
    ...
    function requestPayOnlyBuniAmount(address _player, int128 _feeInBuni) public view {
        // check buni balance
        require(BUNI_TOKEN_CONTRACT.balanceOf(_player) >= usdToBuni(_feeInBuni), "insufficient buni");
    }

    function requestPayOnlyBurAmount(address _player, int128 _feeInBur) public view {
        // check bur balance
        _requestPayBurFromPlayer(_player, _feeInBur);
    }

    function requestPayMixedAmounts(address _player, int128 _feeInBuni, int128 _feeInBur)
        public
        view
    {
        // check buni balance
    ...
    function payOnlyBuniAmount(address _player, int128 _buniAmount)
        public
        onlyFundSpender
        oncePerBlock(_player)
    {
        _payToTreasury(_player, usdToBuni(_buniAmount));
    }

    function payOnlyBurAmount(address _player, int128 _burAmount)
        public
        onlyFundSpender
        oncePerBlock(_player)
    {
    ...
    function payMixedAmounts(address _player, int128 _buniAmount, int128 _burAmount)
        public
        onlyFundSpender
        oncePerBlock(_player)
    {
        ...
        oncePerBlock(msg.sender)
    {
        ...
        requestPayMixedAmounts(msg.sender, buniAmountToPay, burAmountToPay);

        // pay
        payMixedAmounts(msg.sender, buniAmountToPay, burAmountToPay);

        // mint
        for (uint256 i = 0; i < _times; i++) {
            uint256 _seed = uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), msg.sender, nonce + i)));
            BUNICORN_ROLLER_CONTRACT.mintOneRandomBunicornWithStarsProbSpecs(msg.sender, _starsProbSpecs, _seed);
        }
    ...
    function updateGameSettingsV3() public restricted {
        // Baseline rewards 0.4
        BATTLE_REWARDS_BUR_BASELINE = ABDKMath64x64.divu(400, 1000);
        // Element efficiency bonus 0.52%
        BATTLE_BONUS_ELEMENT_EFFICIENCY = ABDKMath64x64.divu(52, 1000);
    }
