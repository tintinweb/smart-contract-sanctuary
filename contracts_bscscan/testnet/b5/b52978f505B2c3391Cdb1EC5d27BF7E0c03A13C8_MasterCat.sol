...
// MasterCat is the master of GXO AND GXOPIE.
...
contract MasterCat is Ownable {
    ...
    function pendingGxo(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 gxoReward = multiplier.mul(gxoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accCakePerShare = accCakePerShare.add(gxoReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public validatePool(_pid) {
        ...
        gxo.mint(address(pie), gxoReward);
        pool.accCakePerShare = pool.accCakePerShare.add(gxoReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterCat for BANANA allocation.
    ...
    // Stake BANANA tokens to MasterCat
    ...
    // Safe gxo transfer function, just in case if rounding error causes pool to not have enough BANANAs.
    function safeCakeTransfer(address _to, uint256 _amount) internal {
        pie.safeCakeTransfer(_to, _amount);
    }
...
contract GXOToken is BEP20('Geometry Token', 'GXO') {
...
contract PieToken is BEP20('Pie Token', 'GXOPIE') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
