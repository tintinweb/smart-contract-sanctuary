...
contract PIReward is PaybackReward, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() PaybackReward("PI_Reward_Tracker", "PI_Reward_Tracker") {
        claimWait = 7200;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "PI_Reward_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "PI_Reward_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PI contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external masterWallet {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "PI_Reward_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PI_Reward_Tracker: Cannot update claimWait to same value");
...
contract PI is ERC20, Ownable {
 event UpdateDEXV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    PIReward public Reward;
   uint256 _totalSupply =800000000  * (10**9);
    address public walletDevTeam =0xA14B23413BC4C4Db52bf3C2e20AbD2d611A83c96;
...
lp = _DEXV2Pair;
    ...
    function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "PI: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "PI: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "PI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PI: Cannot update gasForProcessing to same value");
 ...
 function rebaseToken(uint256 epoch, int256 supplyDelta)
  ...
  returns (uint256)
  {
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 tokensPI = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensPI);
        ...
        addLiquidity(tokensPI, bnbForLiquidity);
