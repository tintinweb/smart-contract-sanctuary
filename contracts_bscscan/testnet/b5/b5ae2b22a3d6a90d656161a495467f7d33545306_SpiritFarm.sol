...
contract SpiritFarm is Ownable {
    ...
    event FarmYield(address indexed _account, uint256 _addEra, uint256 _dailyReward, uint256 _farmAccountTotalCount); */
    ...
    /* function toFarmYield() public returns (bool) {
        ...
        for(uint256 i=1;i<=farmAccountTotalCount;i++){
            if(farmAccounts[orderAccounts[i]].nowJoinAmount>0){
                profitAmount = dailyReward.mul(farmAccounts[orderAccounts[i]].nowJoinAmount).div(farmNowTotalAmount);
                farmAccounts[orderAccounts[i]].nowProfitAmount += profitAmount;// update farmAccountProfit
            }
        }
        nextFarmYieldTime += dayTime;// update nextFarmYieldTime

        emit FarmYield(msg.sender,addEra,dailyReward,farmAccountTotalCount);// set log
    ...
    /* function claim() public returns (bool) {
        ...
        // Transfer
        dtuTokenContract.safeTransfer(address(msg.sender), claimProfitAmount);// Transfer dtu to farm address
    ...
    /* function exitFarm() public returns (bool) {
        ...
        farmAccounts[msg.sender].nowJoinAmount -= account.nowJoinAmount;

        // Transfer
        lpTokenContract.safeTransfer(address(msg.sender), account.nowJoinAmount);// Transfer pangolin to farm address
    ...
    function getFarmBasic() public view returns (Invite InviteContract,ERC20 SpiritFragmentContract,ERC20 DtuTokenContract,
      bool FarmSwitchState,uint256 FarmStartTime,uint256 NextFarmYieldTime,uint256 NowDailyReward,uint256 FarmJoinTotalCount,uint256 FarmNowTotalCount)
    {
        return (inviteContract,spiritFragmentContract,dtuTokenContract,
          farmSwitchState,farmStartTime,nextFarmYieldTime,nowDailyReward,farmJoinTotalCount,farmNowTotalCount);
    }
