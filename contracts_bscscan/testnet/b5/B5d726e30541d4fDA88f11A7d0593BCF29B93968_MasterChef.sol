...
// MasterChef is the master of Yerbamate. He can make Yerbamate and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once YERBAMATE is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        uint256 rewardLockedUp;  // Reward locked up. Harvest code.
        uint256 nextHarvestUntil; // When can the user harvest again. Harvest code.
        //
    ...
    struct PoolInfo {
        ...
        uint256 harvestInterval;  // Harvest interval in seconds. Harvest code.
    ...
    // Bonus muliplier for early yerbamate makers.
    uint256 public constant BONUS_MULTIPLIER = 1;
    // Deposit Fee address
    address public feeAddress;
    // Max harvest interval: 14 days. Harvest code.
    ...
    // Total locked up rewards. Harvest code.
    ...
    event UpdateEmissionRate(uint256 previousAmount, uint256 newAmount);
    event RewardLockedUp(address indexed user, uint256 indexed pid, uint256 amountLockedUp); // Harvest code.
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint256 _depositFeeBP, uint256 _harvestInterval, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
        require(_depositFeeBP <= 400, "add: invalid deposit fee basis points"); //Deposit Shield: Deposit Fees Max 4%
    ...
    // View function to see if user can harvest. Harvest Code.
    ...
    function _updatePool(uint256 _pid) internal {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 yerbamateReward = multiplier.mul(yerbamatePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        yerbamate.mint(devaddr, yerbamateReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            // To handle correctly the transfer tax tokens 
    ...
    function updateEmissionRate(uint256 _yerbamatePerBlock) public onlyOwner {
        ...
        yerbamatePerBlock = _yerbamatePerBlock;
        emit UpdateEmissionRate(previousYerbamatePerBlock, yerbamatePerBlock);
    }
...
// YerbamateToken with Governance.
contract YerbamateToken is BEP20('Yerba Mate', 'YERBAMATE') {
    ...
    /// @dev Capped
    uint256 private _cap = 3000000e18; // 3M
    ...
    /// @dev Addresses that excluded from AntiWhale
    ...
    /// @dev Max transfer rate: 5%.
    uint16 public constant MAXIMUM_TRANSFER_RATE = 500;
    
    /// @dev Min transfer rate: 0.5%.
    uint16 public constant MINIMUM_TRANSFER_RATE = 50;

    /// @dev AntiWhale Max transfer amount rate in basis points. 2.5%
    uint16 public maxTransferAmountRate = 250;

    /// @dev Transfer FEE Rate: 0%.
    uint16 public constant FEE_RATE = 0;

    /// @dev Transfer Burn Rate: 2%.
    uint16 public BURN_RATE = 20;

    /// @dev DEV address
    address public constant DEV_ADDRESS = 0x872cD1874B38afC1D73b32C672A05Be9b0d460f4;

    /// @dev Fee address
    address public constant FEE_ADDRESS = 0x872cD1874B38afC1D73b32C672A05Be9b0d460f4;

    /// @dev Burn address
    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;

    /// @dev The operator can only update the transfer tax rate
    address private _operator;

    /// @dev Events
    event DelegateChanged(address indexed delegator, address indexed fromDelegate, address indexed toDelegate); /// @dev An event thats emitted when an account changes its delegate
    event DelegateVotesChanged(address indexed delegate, uint previousBalance, uint newBalance); /// @dev An event thats emitted when a delegate account's vote balance changes
    event OperatorTransferred(address indexed previousOperator, address indexed newOperator);
    event MaxTransferAmountRateUpdated(address indexed operator, uint256 previousRate, uint256 newRate);

    /// @dev Constructor for the AntiWhale and Operator
    constructor() public {
    ...
    /// @dev AntiWhale Returns the max transfer amount.
    ...
    /// @dev Update the max transfer amount rate AntiWhale.
    /// Can only be called by the current operator.
    /// Maximum Transfer Amount rate is hardcoded to 5%.
    /// Minimum Transfer Amount rate is hardcoded to 0.5%.
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= MAXIMUM_TRANSFER_RATE, "YERBAMATE::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(_maxTransferAmountRate >= MINIMUM_TRANSFER_RATE, "YERBAMATE::updateMaxTransferAmountRate: Max transfer amount rate can not be bleow minimum rate.");
        emit MaxTransferAmountRateUpdated(msg.sender, maxTransferAmountRate, _maxTransferAmountRate);
        maxTransferAmountRate = _maxTransferAmountRate;
    }

    /// @dev Exclude from the AntiWhale
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        if (recipient == BURN_ADDRESS || sender == DEV_ADDRESS || recipient == FEE_ADDRESS ) {
            ...
            // 0% of every transfer ist sent to FEE_ADDRESS
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "YERBAMATE::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "YERBAMATE::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "YERBAMATE::delegateBySig: invalid nonce");
        require(now <= expiry, "YERBAMATE::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "YERBAMATE::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying YERBAMATE (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "YERBAMATE::_writeCheckpoint: block number exceeds 32 bits");
    ...
    // Update BURN_RATE
    function updateBurnRate(uint16 _newBurnRate) public onlyOperator {
        BURN_RATE = _newBurnRate;
    }
}
