...
contract PZilla is ERC20, Ownable {
    ...
    PZillaDividendTracker public dividendTracker;
    ...
    uint256 public swapTokensAtAmount = 20_000_000 * 10**9;
    uint256 public maxWalletBalance = 500_000_000_000 * 10**9;
    ...
    Taxes public buyTaxes = Taxes(7,4,2,1,1,1);
    Taxes public sellTaxes = Taxes(7,4,2,1,1,1);
    ...
    constructor() ERC20("PowerZilla", "PZilla") {

    	dividendTracker = new PZillaDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        PZillaDividendTracker newDividendTracker = PZillaDividendTracker(payable(newAddress));
   ...
   ///////////////////////
    ...
    function setBuyTaxes(uint256 _rewards, uint256 _marketing, uint256 _liquidity, uint256 _operations,  uint256 _dev,uint256 _lottery) external onlyOwner{
        buyTaxes = Taxes(_rewards, _marketing, _liquidity, _operations, _dev, _lottery);
    }

    function setSellTaxes(uint256 _rewards, uint256 _marketing, uint256 _liquidity,uint256 _operations, uint256 _dev, uint256 _lottery) external onlyOwner{
        sellTaxes = Taxes(_rewards, _marketing, _liquidity, _operations, _dev, _lottery);
    }
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(automatedMarketMakerPairs[newPair] != value, "PZilla: Automated market maker pair is already set to that value");
    ...
    function setGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PZilla: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PZilla: Cannot update gasForProcessing to same value");
   ...
   ////////////////////////
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        uint256 swapTax = sellTaxes.rewards + sellTaxes.marketing + sellTaxes.liquidity + sellTaxes.dev + sellTaxes.operations;
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[to]){
                ...
                swapAmt = amount * (buyTaxes.rewards + buyTaxes.marketing + buyTaxes.liquidity + buyTaxes.dev + buyTaxes.operations) / 100;
...
contract PZillaDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor()  DividendPayingToken("PZilla_Dividen_Tracker", "PZilla_Dividend_Tracker") {
    	claimWait = 3600;
    	minimumTokenBalanceForDividends = 100_000_000_000 * (10**decimals()); //must hold 100,000,000,000 tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "PZilla_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PZilla_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PZilla_Dividend_Tracker: Cannot update claimWait to same value");
