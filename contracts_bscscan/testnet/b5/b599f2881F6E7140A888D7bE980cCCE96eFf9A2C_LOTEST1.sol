...
contract LOTEST1 is Context, IERC20, Ownable { 
    ...
    string private _name = "LOTEST1";
    string private _symbol = "LOTEST1";
    ...
    uint256 public _whatTimeIsIt = block.number;
    uint256 public antiSniperBlock;
    bool public antiSniperArmed;
    uint256 public snipeBalance;
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
     ...
     function setTradingIsEnabled(bool _enabled, uint256 sniperBlocks) external onlyOwner {
        antiSniperBlock = (block.number + sniperBlocks);
...
//FIX PUT IN SNIPER BOT MEASURES
        ...
        _whatTimeIsIt = block.number;
        if (tradingIsEnabled && antiSniperArmed && (antiSniperBlock <= _whatTimeIsIt)) {
            _totalBNBFees = 90;
            } else if (tradingIsEnabled && antiSniperArmed && (antiSniperBlock >= _whatTimeIsIt)){
                    //FIX ADD ANTISBIPER BLOCK TO LAUNCH FUNCTION 
            antiSniperArmed = false;
                    //THIS IS WHERE I NEED IT TO CASH OUT THE CONTRACT BALANCE FOR 70/30 AND TURN ON NORMAL FEES
            snipeBalance = balanceOf(address(this));
            swapAndLiquify(snipeBalance);
        }
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (snipeBalance > 0) {
        uint256 toBNB = (snipeBalance / 70);
        swapTokensForEth(toBNB);
        uint256 marketingBNB = address(this).balance;
        _marketingWalletAddress.transfer(marketingBNB);
        uint256 snipeLPTokens = address(this).balance / 2;
        swapTokensForEth(snipeLPTokens);
        addLiquidity(snipeLPTokens, address(this).balance);
        }
