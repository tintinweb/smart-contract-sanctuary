...
contract PRESALEPresale is Ownable {
    ...
    event LogBuyWithBUSD(
        ...
        uint256 updatedRemainPRESALEQuota,
        uint256 datetime
    );

    constructor(IERC20 _token, IERC20 _busd) {
        require(
    ...
    function buyWithBUSD(uint256 amount) external {
        ...
        uint256 tokenToReceived = amount.mul(tokenPerUsd);
        require(tokenToReceived <= remainPRESALEQuota, "over max sale amount");
        remainPRESALEQuota = remainPRESALEQuota.sub(tokenToReceived);
        buyerAmount[msg.sender] = tokenToReceived;
        ...
        onlyOwner
    {
        ...
        uint256 presaleAmount = uint256(3).mul(buyerAmount[buyerAddress]).div(100);
    ...
    function withdrawPRESALE(uint256 amount) external onlyOwner {
        token.safeTransfer(msg.sender, amount);
    }
    ...
    function balanceRemainPRESALEQuota() external view returns (uint256) {
        return remainPRESALEQuota;
    }
