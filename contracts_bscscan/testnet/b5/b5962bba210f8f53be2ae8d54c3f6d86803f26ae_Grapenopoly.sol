...
contract Grapenopoly
{
    ...
    function buyPropertyFromBank(uint256 _tokenId, address selectedTokenToPay) external
    {
        ...
        require(vaultBalances[msg.sender][selectedTokenToPay] >= safeMul(nftPrice[_tokenId], getQuote(usdToken, selectedTokenToPay)), "NB"); //No balance for purchase
        ...
        {
            //Withdraw token
            ERC20(selectedTokenToPay).transfer(feeandtaxreceiver, safeMul(nftPrice[_tokenId], getQuote(usdToken, selectedTokenToPay)));
        }
        else
        {
            //Withdraw Network Coin
            payable(feeandtaxreceiver).transfer(safeMul(nftPrice[_tokenId], getQuote(usdToken, selectedTokenToPay)));
        }

        //Subtract amount of user balance
        vaultBalances[msg.sender][selectedTokenToPay] = safeSub(vaultBalances[msg.sender][selectedTokenToPay], safeMul(nftPrice[_tokenId], getQuote(usdToken, selectedTokenToPay)) );

        //Transfer Property Ownership
        _transfer(feeandtaxreceiver, msg.sender, _tokenId);
    }

    function rollDices(address selectedTokenToPayRent) external returns (DiceResult memory)
    {
