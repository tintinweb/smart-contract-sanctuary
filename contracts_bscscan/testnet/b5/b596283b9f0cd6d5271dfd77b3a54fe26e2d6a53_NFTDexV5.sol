...
contract NFTDexV5 is ERC721Full, ERC721MetadataMintable, Ownable {
  IERC20 ALIA;
...
//   event SetValue(address indexed from, uint256 main, uint256 _author, uint256 _platform);
  ...
  function init5() public{
    // admin = 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859;
    // revenueShare = IERC20(0xb1b008a2009CeB738f6cD4de87D14C713B831Cf9);
  ...
  function buyNFT(address nft_a,uint256 tokenId, address buyer) public{
        fixedSell storage temp = _saleTokens[tokenId];
        require(msg.sender == buyer, "101");
        require(temp.price > 0 || _dollarPrice[tokenId] > 0 , "108");
        uint256 price;
        if(_tokenAuthors[tokenId] == admin && adminOwner[temp.seller] && adminDiscount > 0 && msg.sender != nonCryptoNFTVault){
        ...
        if(revenueShare.checkTokenAuthorIsArtist(tokenId)){
          ...
          if(revenueShare.checkArtistOwner(_nonCryptoOwners[tokenId], tokenId)){
           ...
           _artistPrice = (price  / 1000) * authorPercentage;
           ALIA.transferFrom(buyer, temp.seller, (price  / 1000) * mainPerecentage);
           ALIA.transferFrom(buyer, platform, _artistPrice);
          }
          ALIA.transferFrom(buyer, aliaHolder, _artistPrice);
          revenueShare.tranferAliaArtist(aliaHolder, _artistPrice, nonCryptoNFTVault, tokenId);
        }else{
          ALIA.transferFrom(buyer, temp.seller, (price  / 1000) * mainPerecentage);
  ...
  function buyNftNonCrypto(address  nft_a, uint256 tokenId, string memory ownerId) public {
    fixedSell storage temp = _saleTokens[tokenId];
    uint256 tempPrice;
     ...
     if(temp.price > 0){
          tempPrice = temp.price;
           ...
           (uint112 reserve0, uint112 reserve1,) =LPAlia.getReserves();
           tempPrice = SafeMath.div(SafeMath.mul(_dollarPrice[tokenId],reserve1),SafeMath.mul(reserve0,1000000000000));
        }
  ...
  require(msg.sender == 0xb1b008a2009CeB738f6cD4de87D14C713B831Cf9 || msg.sender == 0x0160b796792D38B98cd19f718F8dF6c143E5c529 || msg.sender == 0x7BB820E46Fd67BfEBA474B934ff48B9316bec219,"Not Authorized");
  _;
  }
  ...
  function checkPricing(uint256 tokenId) public view returns (uint112 reserve0, uint112 reserve1, uint256 dollarPrice) {
   (reserve0, reserve1,) =LPAlia.getReserves();
   dollarPrice = SafeMath.div(SafeMath.mul(_dollarPrice[tokenId],reserve1),SafeMath.mul(reserve0,1000000000000));
  }
}
