...
contract OLY11 is IBEP20, Auth {
    ...
    mapping (address => uint256) public lastBuyBlockNum; //greenwall2
...
*/
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
     ...
     function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!isSell(recipient)) {
            lastBuyBlockNum[recipient] = block.number;
        }
   ...
   //     if(shouldgreenwall(recipient)){triggergreenwall();} //greenwall
      ...
      //  if(!launched() && recipient == PCSpair){ require(_balances[sender] > 0); launch(); }
        ...
        if (block.number < lastBuyBlockNum[recipient].add(greenwallDuration)) {
            amountReceived = amountReceived.sub(greenwallNumerator);
                }
