...
contract Templar is Owner {
    ...
    function multMint(uint256 nftAmount, uint256 supplyId, string[] memory tokenUrls)
        external
        payable
        checkContractCall
        checkPaused
    {
        ...
        for (uint256 index = 0; index < nftAmount; index++) {
            uint256 tokenId = GTOKEN721.getNextTokenId();
            tokenInfo[tokenId].supplyId = supplyId;
            random.requestRandomness(tokenId, block.timestamp + index, false);

            open(tokenId, inviter, tokenUrls);
        }
    }

    function open(uint256 tokenId, address inviter, string[] memory tokenUrls) private {
        ...
        GTOKEN20Miner.mint(msg.sender, levelInfo[tokenInfo[tokenId].levelCode/100].reReward);
        tokenInfo[tokenId].energy = maxEnergy;
        GTOKEN721.mint(msg.sender, tokenUrls[tokenInfo[tokenId].levelCode/100-1]);
    ...
    function claim() external checkContractCall checkPaused {
        ...
        if (reward > 0) {
            ...
            if (inviter != address(0)) {
                ...
                inviterPart += (reward * PVE_REWARD_RATE) / 100;
                GTOKEN20Miner.mint(feeAccount, (reward * PVE_REWARD_RATE) / 100);
            }
    ...
    function setLevelInfo(
        ...
        levelInfo[level].reReward = reReward;
    ...
    function setUserReward(address user, uint256 reward) external {
        ...
        accountInfo[user].dgeReward += reward;
    }
