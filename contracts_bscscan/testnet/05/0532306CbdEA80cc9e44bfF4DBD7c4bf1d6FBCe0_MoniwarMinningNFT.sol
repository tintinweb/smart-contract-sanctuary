...
contract MoniwarMinningNFT is Ownable, ReentrancyGuard {
    ...
    function calculator(uint256 _pid, uint256 date, uint256 hashrate) external view returns (uint256) {
        ...
        uint256 totalHashrate = hashrate.add(pool.totalHashRate);
        if(pool.totalHashRate > 0){
            return hashrate.mul(mowaReward.div(totalHashrate));
        } else{
            return mowaReward;
        }
    }
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256)
    {
        ...
        accMowaPerShare = accMowaPerShare.add(mowaReward);

        uint256 amount = user.hashRate.mul(accMowaPerShare.div(pool.totalHashRate));
        if(amount < user.rewardDebt){
            amount = amount.mul(1e12).sub(user.rewardDebt);
            pending = amount.div(1e12).add(user.rewardLockedUp);
        } else{
            pending = amount.add(user.rewardLockedUp).sub(user.rewardDebt);
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accMowaPerShare = pool.accMowaPerShare.add(mowaReward);
        pool.lastRewardBlock = block.number;
    }
    ...
    function upgrade(uint256 _pid, uint256 uid, uint256 number) public nonReentrant {
        ...
        user.rewardDebt = user.hashRate.mul(pool.accMowaPerShare);
...
//        }
    ...
    function harvest(uint256 _pid) public nonReentrant {
        ...
        if (user.hashRate == 0 || block.number >= pool.endBlock) {
            ...
            uint256 amount = user.hashRate.mul(pool.accMowaPerShare.div(pool.totalHashRate));
            if(amount < user.rewardDebt){
                amount = amount.mul(1e12).sub(user.rewardDebt);
                pending = amount.div(1e12).add(user.rewardLockedUp);
            } else{
    ...
    function unStake(uint256 _pid, uint256 uid) public nonReentrant {
        ...
        uint256 amountTranfer = list.amountDeposit;
        uint256 hashRate = list.hashRate;
        ...
        if (amountTranfer > 0) {
            safeMowaTransfer(address(msg.sender), amountTranfer);
            user.amount = user.amount.sub(amountTranfer);
        }
    ...
    function updateRewardDebt(uint256 _pid) internal {
        updatePool(_pid);
        PoolInfo storage pool = mowaPoolInfoNFT[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        user.rewardDebt = user.hashRate.mul(pool.accMowaPerShare);
