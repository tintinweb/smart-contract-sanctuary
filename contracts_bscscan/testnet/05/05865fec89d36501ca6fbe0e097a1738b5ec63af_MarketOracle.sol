...
/**
 * @title KAKAO price Oracle
 *      This Oracle calculates the average USD price of KAKAO based on the BNB-KAKAO and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private kakaoBnbPrice0CumulativeLast;
    uint private kakaoBnbPrice1CumulativeLast;
    uint32 private kakaoBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _kakao_bnb;
    ...
    constructor(
        address __kakao_bnb,   // Address of the kakao-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _kakao_bnb = IUniswapV2Pair(__kakao_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        kakaoBnbPrice0CumulativeLast = _kakao_bnb.price0CumulativeLast();
        kakaoBnbPrice1CumulativeLast = _kakao_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, kakaoBnbBlockTimestampLast) = _kakao_bnb.getReserves();
    ...
    // Get the average price of 1 kakao in the smallest BNB unit (18 decimals)
    function getkakaoBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_kakao_bnb));

        require(_blockTimestamp != kakaoBnbBlockTimestampLast, "KAKAO Last and current are equal");

        FixedPoint.uq112x112 memory kakaoBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - kakaoBnbPrice0CumulativeLast) / (_blockTimestamp - kakaoBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, kakaoBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        uint kakaoBnbAverage;
        uint busdBnbAverage;

        (kakaoBnbPrice0CumulativeLast, kakaoBnbPrice1CumulativeLast, kakaoBnbBlockTimestampLast, kakaoBnbAverage) = getkakaoBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, kakaoBnbAverage) = getkakaoBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = kakaoBnbAverage*(1e6) / busdBnbAverage;
