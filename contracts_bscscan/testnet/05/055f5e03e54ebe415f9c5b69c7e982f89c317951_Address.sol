...
contract MoonstarNFT is ERC721, Ownable, Whitelist {
    ...
    constructor() public  ERC721("MoonstarNFT", "MOONNFT")  {
    ...
    function mint(string memory _tokenURI, address _toAddress, bool _currency,  uint256 _price, bool _isListOnMarketplace, uint256 royalties) public returns (uint) {
        ...
        if (royalties > 0) {
            uint256 allowance = IBEP20(paymentTokenAddress).allowance(msg.sender, address(this));
            uint256 token_bal = IBEP20(paymentTokenAddress).balanceOf(msg.sender); //how much MST buyer has
            ...
            require(IBEP20(paymentTokenAddress).transferFrom(msg.sender, address(this), royalties));
        }
    ...
    function buy(uint _id) external payable {
        ...
        if (currency[_id]) {
            ...
            require(IBEP20(paymentTokenAddress).transferFrom(_newOwner, address(this), _commissionValue));
        }
    ...
    function gift(uint _id) public returns (bool) {
        address payable _buyer = payable(msg.sender);
        address payable _owner = payable(ownerOf(_id));
        require(_exists(_id), "Error, wrong tokenId");
        require(msg.sender == ownerOf(_id), "you are not owner of this NFT");
  ...
  * @dev setter/getter for ERC20 linked to exchange (current) smartcontract
      */
    function setPaymentToken(address newBEP20Contract) external onlyOwner returns(bool){
    ...
    function withdrawTokens(address to_wallet, uint256 realAmountTokens) external onlyOwner {
        ...
        uint256 contractTokenBalance = IBEP20(paymentTokenAddress).balanceOf(address(this));
