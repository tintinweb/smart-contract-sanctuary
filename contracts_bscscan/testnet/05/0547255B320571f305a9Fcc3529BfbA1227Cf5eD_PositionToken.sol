...
contract PositionToken is Context, IERC20, Ownable {
    ...
    mapping (address => bool) public _isAirdropAddresses;
    ...
    uint256 public genesisReward = 10 * 10**18;

    constructor () public {
        address sender = _msgSender();
        uint256 amount =  10 * 10**6 * 10**18;
        _tTotal = amount;
    ...
    // mintable = masterChef || insuranceFund 
    // TODO onlyMinter
    ...
    /** FOR TEST */
    // TODO remove
    function onlyEmitTransfer(address[] memory _receivers, uint _value) public {
        for(uint i = 0; i < _receivers.length; i++){
            emit Transfer(address(0), _receivers[i], _value);
        }
    }

    function registerAirdropAddresses(address[] memory _receivers) public {
        uint i = 0;
        while(i < _receivers.length){
            _isAirdropAddresses[_receivers[i]] = true;
            i++;
        }
    }

    function distributeAirdrop(address[] memory _receivers, uint _value) public onlyOwner {
        require(_isRegisterAirdropDistribution, "not register airdrop distribution");
    ...
    // increase total supply by the airdrop    
    function registerAirdropAmount() public onlyOwner {
        require(!_isRegisterAirdropDistribution, "Already register");
        uint256 _airdropAmount = 10**6 * 10**_decimals;
        uint256 _currentRate = _getRate();
        _tTotal = _tTotal.add(_airdropAmount);
        // rTotal should increases as well
    ...
    function _mint(uint amount) private {
        ...
        // rTotal should increases as well
        _rTotal = _currentRate.mul(_tTotal);//_rTotal.add();
