...
// The ChainLink Price Feed is used to fetch current price of BNB used to show SOCT37 marketcap and price straight from this contract

/*
 *  This is just a Test Contract intended ONLY for the BSC TestNet. This testing token will be temporarily called “Secret OC Test Token V21” ($SOCT37).
 ...
 *  SOCT37 is a fork of TAMARIN which improved the original SAFEMOON contract.
 *  
 *  SOCT37 total Supply is 100.000.000.000 (100B).
 ...
 *  
 *  #SOCT37 10% Taxes are applied to all the token transactions, which is divided as follows:
 *
 ...
 *  any SOCT37 OR BNB tokens being raised on the CC. Thus, preventing any kind of donations scam.
 *
 */

contract SecretOCTestV37 is Context, IERC20, Ownable {
	...
	address private CHAINLINK_PRICE_FEED_BNB_USD = 0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526;
	...
	string private _name = "SecretOCTest V37";
	string private _symbol = "SOCT37";
	...
	// temporal pair to be able to compile, constructor creates the real one and changes it on deployment
	IPancakeSwapV2Pair private PancakeSwapV2Pair = IPancakeSwapV2Pair(0x1f814921c047e40bB3090332F6395559FF86f04d);
	
  /* By setting the _maxTxAmount to 1 since contract deployment, it will prevent trades bigger than 1 SOCT37 token.
   * Done this way to prevent snipping bots + pre-sale buyers from trading big ammounts the second liquidity is provided.
   ...
   */
	...
	function _getSOCT37Price_Marketcap() public view returns (uint256 marketcap, string memory price_USD, string memory price_BNB) {
       	(uint256 SOCT, uint256 BNB,) = PancakeSwapV2Pair.getReserves();
        uint256 SOCTnoD = SOCT / 10**8;
        uint256 rawPriceBNB = BNB.mul(10**18).div(SOCTnoD);
