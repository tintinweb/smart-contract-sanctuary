...
contract DEX is Ownable {
    ...
    event BnbPriceUpdate(uint256 amount);
    ...
    function buy() payable external {
        ...
        uint256 amountTobuy = (msg.value * bnbPrice * 100) / 25;
        require(amountTobuy > 0, "You need to send some BNB");

        uint256 dexBalance = token.balanceOf(address(this));
        require(amountTobuy <= dexBalance, "Not enough tbc in the reserve");
    ...
    modifier onlyUpdater() {
        require(updater == msg.sender || owner() == msg.sender, "Caller is not the updater");
        _;
    }

    function updateBnbPrice(uint256 price) external onlyUpdater {
        bnbPrice = price;
        emit BnbPriceUpdate(bnbPrice);
    }
    ...
    function close() external onlyOwner {
        ...
        uint256 dexBalance = token.balanceOf(address(this));
        token.safeTransfer(receiver, dexBalance);
