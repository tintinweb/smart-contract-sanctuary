...
Website: http://betterthensanta.fun/

Telegram: https://t.me/BetterThenSanta

Twitter: https://twitter.com/BetterThenSanta
...
contract BetterThenSanta is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public charityAddress =
        payable(0x23BA3d7e60B6039f70a40C798909614f0db9E342); // Charity Address
        
    address payable public marketingAddress =
        payable(0xDd2759BAa3B5E75304fA4c9aD56a0F890EBc8fCA); // Marketing Address
        
    address payable public liquidityAddress =
        payable(0x2f13883Bb6010315575a093D3AD689Ba30eB71D7); // Liquidity Address
    ...
    uint256 private constant _tTotal = 777777777 * 1e18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    bool public limitsInEffect = true;
    
    string private constant _name = "BetterThenSanta";
    ...
    event SetBuyFee(uint256 CharityFee, uint256 liquidityFee, uint256 reflectFee, uint256 buybackFee);
    
    event SetSellFee(uint256 CharityFee, uint256 liquidityFee, uint256 reflectFee, uint256 buybackFee);
    ...
    event UpdatedmarketingAddress(address buyback);
    ...
    function _transfer(
     ...
     if(limitsInEffect){
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _charityTokensToSwap + _marketingTokensToSwap;
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                removeAllFee();
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyCharityFee + _buyMarketingFee;
                buyOrSellSwitch = BUY;
            } 
            // Sell
                ...
                removeAllFee();
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellCharityFee + _sellMarketingFee;
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap.add(_marketingTokensToSwap).add(_charityTokensToSwap);
        ...
        uint256 bnbForLiquidity = bnbBalance.sub(bnbForCharity).sub(bnbForMarketing);
        ...
        (success,) = address(charityAddress).call{value: bnbForCharity}("");
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyCharityFee, uint256 buyMarketingFee)
        external
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyCharityFee + _buyMarketingFee <= 20, "Must keep taxes below 20%");
        emit SetBuyFee(buyCharityFee, buyLiquidityFee, buyTaxFee, buyMarketingFee);
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellCharityFee, uint256 sellMarketingFee)
        external
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellCharityFee + _sellMarketingFee <= 30, "Must keep taxes below 30%");
        emit SetSellFee(sellCharityFee, sellLiquidityFee, sellTaxFee, sellMarketingFee);
    }
    ...
    function setmarketingAddress(address _marketingAddress) external onlyOwner {
        ...
        _isExcludedFromFee[marketingAddress] = true;
        emit UpdatedmarketingAddress(marketingAddress);
    }
