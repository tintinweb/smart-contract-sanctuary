...
contract DFLICO  {
   ...
   IBEP20 private busdToken; 
    ...
    function getInvestmentPlanByUID(address _user) public view returns (uint256[] memory, uint256[] memory, uint256[] memory, uint256[] memory, uint256[] memory,uint256[] memory, bool[] memory) 
    {
        ...
        for (uint256 i = 0; i < investor.planCount; i++) {
            ...
            if (investor.plans[i].isExpired) {
                isExpireds[i] = true;
                newDividends[i] = 0;
                interests[i] = users[msg.sender].currentPercent;
            } else {
                isExpireds[i] = false;
                if (investmentPlans_[investor.plans[i].planId].term > 0) {
                    if (block.timestamp >= investor.plans[i].investmentDate.add(investmentPlans_[investor.plans[i].planId].term)) {
                        newDividends[i] = _calculateDividends(investor.plans[i].investment, users[msg.sender].currentPercent, investor.plans[i].investmentDate.add(investmentPlans_[investor.plans[i].planId].term), investor.plans[i].lastWithdrawalDate, users[msg.sender].currentPercent);
                        isExpireds[i] = true;
                        interests[i] = users[msg.sender].currentPercent;
                    }
                    else{
                        newDividends[i] = _calculateDividends(investor.plans[i].investment, users[msg.sender].currentPercent, block.timestamp, investor.plans[i].lastWithdrawalDate, users[msg.sender].currentPercent);
                       interests[i] = users[msg.sender].currentPercent;
                    }
                } else {
                    newDividends[i] = _calculateDividends(investor.plans[i].investment, users[msg.sender].currentPercent, block.timestamp, investor.plans[i].lastWithdrawalDate, users[msg.sender].currentPercent);
	...
	function getStakingPercent(address _user) public view returns(uint16)
	{
	    ...
	    if(users[_user].totalStakingBusd>=2000)
	    return 58;
	    else if(users[_user].totalStakingBusd>=1000)
	    return 50;
	    else if(users[_user].totalStakingBusd>=500)
	    return 42;
	    else
	    return 33;
	}
