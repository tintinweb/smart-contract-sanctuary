...
// File: contracts/BSCSFarm.sol

pragma solidity ^0.6.12;


contract BSCSFarm is Ownable {
    using SafeMath for uint256;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 reward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        safeTokenTransfer(address(this), reward);
        pool.accRewardPerShare = pool.accRewardPerShare.add(reward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to BSCSFarm for reward allocation.
    ...
    // Withdraw LP tokens from BSCSFarm.
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 rewardBal = bscsToken.balanceOf(address(this));
        if (_amount > rewardBal) {
            bscsToken.transfer(_to, rewardBal);
        } else {
            bscsToken.transfer(_to, _amount);
        }
    }
    ...
    function setStartBlock(uint256 _startBlock) public onlyDev {
        startBlock = _startBlock;
    }
}