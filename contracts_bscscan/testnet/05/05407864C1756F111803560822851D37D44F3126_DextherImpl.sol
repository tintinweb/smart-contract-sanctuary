...
contract DextherImpl is OwnableUpgradeable, ERC721Holder, ERC1155Holder {
    enum Status {
        Available,
        Swapped,
        Canceled
    ...
    function updateOffer(
        ...
        require(offers[offerId].creator == msg.sender, "Not creator");
        require(offers[offerId].offerTokensValues.length == _offerTokensValues.length, "Not creator");
        require(
        ...
        for (uint256 i = 0; i < _offerTokensValues.length; i++) {
          if (offers[offerId].offerTokensValues[i] > _offerTokensValues[i]){
              ...
              reduce[i] = SafeMath.sub(offers[offerId].offerTokensValues[i],_offerTokensValues[i]);
          } 
          if (offers[offerId].offerTokensValues[i] < _offerTokensValues[i]) {
              if(!isAdd) isAdd = true;
              addmore[i] = SafeMath.sub(_offerTokensValues[i], offers[offerId].offerTokensValues[i]);
          }
        }
    ...
    function swap(uint256 offerId) external payable {
        ...
        if (offers[offerId].estimateTokenAddress == address(0x0)) {
            require(msg.value >= offers[offerId].estimateAmount, "invalid amount");
            TransferHelper.safeTransferETH(feeTo, fee);
            TransferHelper.safeTransferETH(
                offers[offerId].creator,
                estimateAmountMinusFee
            );
            // if(msg.value > offers[offerId].estimateAmount) {
            //   TransferHelper.safeTransferETH(
            //     msg.sender,
            //     SafeMath.sub(
            //         msg.value,
