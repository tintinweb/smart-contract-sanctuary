...
abstract contract HasSecondarySaleFees is ERC165 {
    ...
    constructor() {
        super.supportsInterface(_INTERFACE_ID_FEES);
    }
...
contract HasContractURI is ERC165 {
    ...
    constructor(string memory _contractURI)  {
        contractURI = _contractURI;
         super.supportsInterface(_INTERFACE_ID_CONTRACT_URI);
    }
...
contract BLEUFINFT is ERC721Enumerable, Ownable {
    constructor() ERC721("BLEUFI NFT", "BLEUFINFT") {}
    ...
    mapping(address => uint256[]) public tokensPerCreator;
    ...
    function getTokensPerCreator(address creator)
        public
        view
        returns (uint256[] memory)
    {
    ...
    function burn(uint256 tokenId) public  {
        require(ownerOf(tokenId)==msg.sender,"caller is not owner of NFT ");
        _burn(tokenId);
    }
    ...
    function mintAll(
        ...
        require(
            _ids.length == _tos.length,
            "tokenIDs and creators are not mismatched"
        );
        require(
            _ids.length == _newTokenURIs.length,
            "tokenIDs and tokenURI are not mismatched"
        );
