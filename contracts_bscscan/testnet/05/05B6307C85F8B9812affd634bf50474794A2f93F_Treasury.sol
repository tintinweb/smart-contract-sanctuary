...
contract MarketPlace is Auth, PausableUpgradeable {
  ...
  function makeOrder(
    ...
    if (_startPrice != _endingPrice) {
      require(_auctionDuration >= 1 minutes && _auctionDuration <= 365 days, "duration is invalid"); //todo DEV to minutes
    }
    if (_type == IMarketPlace.OrderType.renting) {
      require(_rentingDuration >= 7 minutes && _rentingDuration <= 365 days, "renting duration is invalid"); //todo DEV to minutes
    }
  ...
  function setGameContract(address _gameProxy) external onlyMainAdmin {
...
contract Treasury is Auth {
  ...
  function initialize(address _mainAdmin, address _heroToken, address _itemToken) public initializer {
    ...
    if (block.chainid == 97) {
      // TODO testnet
      fotaToken = IFOTAToken(0x0A4E1BdFA75292A98C15870AeF24bd94BFFe0Bd4);
      busdToken = IFOTAToken(0xD8aD05ff852ae4EB264089c377501494EA1D03C9);
      usdtToken = IFOTAToken(0xa1D3c78f0f98f6ba7b08ec15df48E22880a0024c);
    }
  }
