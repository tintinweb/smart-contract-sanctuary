...
contract Presale is Ownable, Whitelisted, ReentrancyGuard {
    ...
    IERC20 public token;
    address public Payee = 0x71a63fC6Fd4caC5dAB150A97B733F4A149A38951; // wallet that recieves the bnb and liquidity
    ...
    uint256 public novaStage = 86400; //1800 = 30 minutes for nova/snova holders to buy first
    ...
    uint256 public liqPercent = 50; //amount of bnb used for liquidity div 100
    uint256 public liqTokens = 39; // amount of tokens used for liquidity div 100
    uint256 public fee = 1; // launchpad fee div 100
    
    uint256 public maxBuy = 3000000000000000000; // 2000000000000000000 = 2 bnb make sure to add 1 to avoid overflow on UI add 1 for overflow on ui
    ...
    uint256 public novaSwapRate = 100000; // tokens per bnb snova stage
    uint256 public swapRate = 80000; // tokens per bnb normal stage
    uint256 public publicLaunchRate = 66666; // tokens per bnb for liquidity
    uint256 public yourRate; // variable to get specific swap rate
    uint256 public nineDecimals = 1000000000;
    // uint256 public quota; // tokens to be purchased
    ...
    // function setQuota (uint256 _value) public onlyOwner {
    //    quota = _value *99/100;
    // }

    function swap() public payable nonReentrant{
        uint256 quota = token.balanceOf(address(this)) * ((100 - fee)/100) * (100 - liqTokens) / 100; // tokens available for purchase = total deposited mins 1% usage fee and liquidityTokens
        uint256 outAmount;
        ...
        if (IERC20(sNova).balanceOf(msg.sender) >= minSNova ||
    ...
    function burn(uint256 _amount) public onlyOwner {
        IERC20(token).transfer(burnAddress, _amount);
    }
    ...
    function setLiqRate ( uint256 newLiqRate) public onlyOwner {
        liqPercent = newLiqRate;
    }
