...
//    address referrer
  ...
  /// @dev The maximum possible deposit amount.
  ...
  canDeposit(to, controlledToken, amount)
  nonReentrant
  {
  ...
  external {
    address operator = _msgSender();
    emit Blacklist(operator, blacklist);
  }
  ...
  onlyControlledToken(controlledToken)
  {
    ...
    if (depositLimit == uint256(- 1)) {
      ...
      /// 保证不会转多钱给用户
      if (tokenBalance > amount) tokenBalance = amount;
      _token().safeTransfer(to, tokenBalance);
    }
  ...
  function setDepositLimit(uint256 _depositLimit) external override onlyOwner {
  ...
  /// @dev Checks if the Prize Pool can deposit amount based on the current limit
  /// @param _amount The amount of limit to be deposit by the User
  /// @return True if the User can receive the specified amount of limit
  function _canDeposit(address _to, address _controlledToken, uint256 _amount) internal view returns (bool) {
    uint256 balance = ControlledToken(_controlledToken).balanceOf(_to);
    return (balance.add(_amount) <= depositLimit);
  }
  ...
  /// @dev Function modifier to ensure the deposit amount does not exceed the deposit limit (if set)
  modifier canDeposit(address _to, address _controlledToken, uint256 _amount) {
    require(_canDeposit(_to, _controlledToken, _amount), "PrizePool/deposit-amount-exceeds-pool-limit");
    _;
  }
