...
contract DowgSale2 is Ownable {
  using SafeMath for uint256;
  TeamToken token;
  uint256 public constant RATE = 500000; // Number of tokens per Ether
  ...
  function isActive() public view returns (bool) {
    return (
        block.timestamp > 1639250983 && block.timestamp < 1673801698 && initialized == true// Goal must not already be reached
    );
  }
  ...
  function buyTokens() public payable whenSaleIsActive {
    ...
    token.transfer(msg.sender, tokens*(10**8)); // Send tokens to buyer
