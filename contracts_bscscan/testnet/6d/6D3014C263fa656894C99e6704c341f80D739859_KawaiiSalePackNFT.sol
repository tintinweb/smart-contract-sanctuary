...
contract SignData {
    ...
    constructor() internal {
        ...
        BUY_PACK_HASH = keccak256("Data((address sender,string memory pack,uint256 amount,uint256 nonce)");
    }
...
contract KawaiiSalePackNFT is Ownable, SignData {
    ...
    function setField(uint256 _fieldId) public onlyOwner {
        fieldId = _fieldId;
    }
    ...
    function setTokenPayment(IERC20 token) public onlyOwner {
        tokenPayment = token;
    }
    ...
    function createPack(uint256 numberTree, uint256 numberAnimal, uint256 numberField, uint256 price) public onlyOwner {
        require(numberAnimal < numberTree, "Animal must < tree");
        PackData memory packData;

        packData.tree = numberTree;
        packData.animal = numberAnimal;
        packData.field = numberField;
    ...
    function updatePack(uint256 pack, uint256 numberTree, uint256 numberAnimal, uint256 numberField, uint256 price) public onlyOwner {
        require(numberAnimal < numberTree, "Animal must < tree");
        packs[pack] = PackData(numberTree, numberAnimal, numberField, price);
    }

    function getLenPack(uint256 pack) public view returns (uint256){
        return packs.length;
    }

    function buyPack(IERC1155 nftRegister, address sender, uint256 pack, uint256 amount, uint8 v, bytes32 r, bytes32 s) public {
        ...
        require(packData.price != 0, "pack not sale");

        uint256 amountPay = packData.price.mul(amount);
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            _mintItem(nftRegister, sender, packData, animalsInPack, treesInPack);
        }

    }

    function _mintItem(IERC1155 nftRegister, address sender, PackData memory packData, uint256[] memory animalsInPack, uint256[] memory treesInPack) internal {
        for (uint256 i = 0; i < packData.animal; i++) {
            nftRegister.mint(sender, animalsInPack[i], 1);
        }
        for (uint256 i = 0; i < packData.tree; i++) {
            nftRegister.mint(sender, treesInPack[i], 1);
        }
        nftRegister.mint(sender, fieldId, packData.field);
    }
    ...
    function claim(IERC20 token, address to, uint256 amount) public onlyOwner {
