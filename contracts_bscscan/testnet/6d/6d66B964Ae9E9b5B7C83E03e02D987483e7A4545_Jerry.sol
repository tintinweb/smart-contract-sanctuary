...
contract JerryDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Jerry_Dividend_Tracker", "Jerry_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Jerry_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Jerry_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Jerry contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Jerry_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "Jerry_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Jerry is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    JerryDividendTracker public dividendTracker;
    ...
    uint256 public prevsellBNBRewardsFee;
    uint256 public prevBNBRewardsFee;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee) public onlyOwner {
        ...
        prevBNBRewardsFee = _bnbRewardFee;
    ...
    function setsellFee(uint256 _sellbnbRewardFee, uint256 _sellliquidityFee, uint256 _sellmarketingFee) public onlyOwner {
        sellBNBRewardsFee = _sellbnbRewardFee;
        prevsellBNBRewardsFee = _sellbnbRewardFee;
        sellliquidityFee = _sellliquidityFee;
        sellmarketingFee = _sellmarketingFee;
        
        totalFees = sellBNBRewardsFee.add(sellliquidityFee).add(sellmarketingFee); // total fee on sell
    }
    ...
    constructor() ERC20("SAFEJERRY", "JERRY") {
        BNBRewardsFee = 0;
        prevBNBRewardsFee = BNBRewardsFee;
        
        sellBNBRewardsFee = 0;
        prevsellBNBRewardsFee = sellBNBRewardsFee;
        ...
        marketingWallet = payable(0x4055afE72a69c2B7654970da1eBc6A217FC563F3);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy
        totalsellFees = sellBNBRewardsFee.add(sellliquidityFee).add(sellmarketingFee); // total fee on sell
        
    	dividendTracker = new JerryDividendTracker();
    ...
    function setSwapTokens(uint256 newAmount)public onlyOwner{
        swapTokensAtAmount = newAmount;    
    }
    
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Jerry: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Jerry: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Jerry: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Jerry: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Jerry: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Jerry: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function _transfer(
        ...
        BNBRewardsFee = prevBNBRewardsFee;
