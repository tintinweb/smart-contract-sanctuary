...
contract OHMICRON {
    ...
    uint totalStaked;
    uint noStaked;
    uint DPY = 1; //Daily percentage yield (% of supply)
    ...
    function rewardStakers() public{ //This function needs to be called once a day
        ...
        for(uint i = 1; i <= noStaked; i++){
            ...
            uint amount_dist = supply * DPY / 100; //Daily token distribution (percentage of supply)
            uint share = stakedBalance[user] * 100 / totalStaked; //User's stake share (%)
            stakedBalance[user] += amount_dist * share / 100; //Distribute rewards to user
            }
        }
    }
    ...
    function swapAndBurn(uint amountETH) internal{
