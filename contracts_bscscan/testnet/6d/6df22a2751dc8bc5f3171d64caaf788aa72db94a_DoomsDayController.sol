...
contract DoomsDayController is Ownable {
    ...
    function _getRandom(uint256 num) internal virtual returns (uint) {
        return uint256(keccak256(abi.encodePacked(block.timestamp, randNum++, msg.sender))) % num;
    }
    ...
    function getHeroCard() public{
        ...
        heros[totalHeros] = Hero(num1, num2, num3, num4, num5, num6, totalHeros, types, rarity, 1, 0, 0, msg.sender);
    ...
    function taskRewardWithdrew(uint256 heroId) public {
        ...
        if (tasks[heroId].taskTypes < 10) {
            total = miningRatio * (10 + (mainAttr - 85) * 5) * number * heroPublicAttributes[heros[heroId].level].profitMagnification * overtimeRatio / 100000000 ;
        }
    ...
    function fightMonster(uint256 areaId, uint256 heroId) public {
        ...
        uint[] memory monsterTypes = _getMonsterTypes(areaId, monsterNum);
        ...
        for (i = 0; i < monsterNum; i++) {
            ...
            uint256 monsterType = monsterTypes[i];
    ...
    function setAdjustLow(uint256 num1,uint256 num2,uint256 num3,uint256 num4,uint256 num5) public onlyOwner {adjustLow = [num1, num2, num3, num4, num5];}
    function setAdjustUp(uint256 num1,uint256 num2,uint256 num3,uint256 num4,uint256 num5) public onlyOwner {adjustUp = [num1, num2, num3, num4, num5];}
    ...
    //获取每次遇见怪物类型
    function _getMonsterTypes(uint256 areaId, uint256 monsterNum) internal returns (uint256[] memory monsterTypes) {
        ...
        uint256[5] memory monsters;

        for(uint256 j = 0; j < monsterNum; j++) {

            uint256 types = _getMonsterType(areaId);

            for(uint256 i = 0; i < 5; i++){
                if(monsters[i] == types){
                    types = _getMonsterType(areaId);
            
                    i = 0;
                }
            }

            monsters[types - 1] = types;
        }
    ...
    function _sort(uint256[] memory data) internal pure {
