...
//        require(block.timestamp.sub(lastRewardTime) >= Period, "not over 24hours");
    ...
    function stake7(uint256 _amount) public checkMinStake(_amount) {
        ...
        if (user.amount7 > 0) {
            user.earned7 = user.amount7.mul(accTokenPerShare7).div(PRECISION_FACTOR).sub(user.rewardDebt7);
    ...
    function stake30(uint256 _amount) public checkMinStake(_amount) {
        ...
        if (user.amount30 > 0) {
            user.earned30 = user.amount30.mul(accTokenPerShare30).div(PRECISION_FACTOR).sub(user.rewardDebt30);
            //already earned before this stake.
        }       
    ...
    function stake90(uint256 _amount) public checkMinStake(_amount) {
        ...
        if (user.amount90 > 0) {
            user.earned90 = user.amount90.mul(accTokenPerShare90).div(PRECISION_FACTOR).sub(user.rewardDebt90);
            //already earned before this stake.
        }  
    ...
    function stake360(uint256 _amount) public checkMinStake(_amount) {
        ...
        if (user.amount360 > 0) {
            user.earned360 = user.amount360.mul(accTokenPerShare360).div(PRECISION_FACTOR).sub(user.rewardDebt360);
            //already earned before this stake.
        }  
