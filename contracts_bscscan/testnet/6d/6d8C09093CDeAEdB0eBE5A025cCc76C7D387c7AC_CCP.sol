...
/*
All rights reserved Social Credit $CCP
 */
...
contract CCP is Context, IERC20, Ownable {
    ...
    string private _name = "Social Credit";
    string private _symbol = "CCP";
	...
	function _gettreasuryWallet() public view virtual returns (address) {
        return _treasuryWallet;
	}
	
	function _getprojectWallet() public view virtual returns (address) {
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tTreasury, uint256 tProject) = _getValues(tAmount);
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
    	...
    	function setWallets(address payable project, address payable treasury) public onlyOwner() {
        _projectWallet = project;
        _treasuryWallet = treasury;
	}
    	function setProjectAddress(address payable project) public onlyOwner() {
        _projectWallet = project;
	}
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tTreasury, data.tProject);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tTreasury).sub(tProject);
        return (tTransferAmount, TData(0, tFee, tLiquidity, tTreasury, tProject, 0));
    }
    ...
    function _getRValues( TData memory _data) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTreasury = _data.tTreasury.mul(_data.currentRate);
		uint256 rProject = _data.tProject.mul(_data.currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rTreasury).sub(rProject);
	...
	function _takeProject(uint256 tProject) private {
        ...
        if (_shouldSwapToBNB) {
            swapTokensForEth(rProject);
            _projectWallet.transfer(address(this).balance);
        } else {
            _rOwned[_projectWallet] = _rOwned[_projectWallet].add(rProject);
            if(_isExcluded[_projectWallet])
                _tOwned[_projectWallet] = _tOwned[_projectWallet].add(tProject);
        }
    }
