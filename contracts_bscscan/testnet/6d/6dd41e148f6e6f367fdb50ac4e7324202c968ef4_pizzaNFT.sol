...
contract pizzaNFT is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    function bakePizzaAndMint(address from, string memory metadata, uint256 base, uint256 sauce, uint256 cheese, uint256[] memory meats, uint256[] memory toppings ) public {
        ...
        require(baseType == 1 || baseType == 0, "Please select valid base type");
        require(ownerOf(base) == msg.sender || baseType == 0, "You are not owner of base you selected"); 
        // Checking sauce type
        uint256 sauceType = mintIngredientTypes[sauce];
        require(sauceType == 2 || sauceType == 0, "Please select valid sauce type");
        ...
        require(cheeseType == 3 || cheeseType == 0, "Please select valid cheese type");
        require(ownerOf(cheese) == msg.sender || cheeseType == 0, "You are not owner of cheeze you selected");

        uint256 meatType = 0;
        for(uint256 x = 0; x < meats.length; x++) {
            meatType = mintIngredientTypes[meats[x]];
            require(meatType == 4 || meatType == 0, "Please select valid meat type");
            require(ownerOf(meats[x]) == msg.sender || meatType == 0, "You are not owner of meet you selected");
        }
        uint256 toppingType = 0;
        for(uint256 x = 0; x < toppings.length; x++) {
            toppingType = mintIngredientTypes[toppings[x]];
            require(toppingType == 5 || toppingType == 0, "Please select valid topping type");
            require(ownerOf(toppings[x]) == msg.sender || toppingType == 0, "You are not owner of toppings you selected");
        }
        _nftIds.increment();
        uint256 _nftId = _nftIds.current();
        _mint(owner(), _nftId);
    ...
    //called on bake and random bake
    function randomBakePizzaAndMint(address from, string memory metadata, uint256 base, 
    ...
    modifier isRandomBakePizzaMint() {
        ...
        require(block.timestamp <= randomBakeEndTime, "Random bake is ended now you cannot bake the Pizza now");
        _;
    }
}