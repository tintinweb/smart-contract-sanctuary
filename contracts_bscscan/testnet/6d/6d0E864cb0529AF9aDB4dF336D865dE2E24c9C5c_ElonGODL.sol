...
contract ElonGODL is IERC20, Ownable {
    ...
    function setLaunched(bool value) public onlyOwner {
        _isLaunched = value;
    }
    ...
    function forceLiquiditySwap() public onlyOwner {
        require(_liquidityBalance >= _liquiditySwapAmount, "Liquidity balance too low.");
        
        _swapAndLiquify();
    }
    
    function forceDrawLottery() public onlyOwner {
        require(_lotteryBalance > 0, "Lottery balance too low.");
        
        _drawLottery();
    }
    
    function forceMarketingSwap() public onlyOwner {
        require(_marketingBalance >= _marketingSwapAmount, "Marketing balance too low.");
        
        _swapAndTransferToMarketing();
    }
    ...
    function changeUniswapRouter(address routerAddress) public onlyOwner {
    ...
    function _doFirstEligibleTask() private lockTasks {
        if (_swapAndLiquify()) return;
        if (_swapAndTransferToMarketing()) return;
        if (_drawLottery()) return;
    }
    
    function _swapAndLiquify() private returns(bool) {
        // dont swap for peanuts
        if (_liquidityBalance < _liquiditySwapAmount) return false;
        
        // dont swap more than max transaction amount
        uint256 tokenAmount = _clampValue(_liquidityBalance, _maxTransactionAmount);
        ...
        // swap tokens for ETH
        if (!_swapTokensForEth(half)) return true;
        ...
        // add liquidity
        if (!_addLiquidity(otherHalf, ethAmount)) return true;
    ...
    function _swapAndTransferToMarketing() private returns(bool) {
        // dont swap for peanuts
        if (_marketingBalance < _marketingSwapAmount) return false;
        
        // dont swap more than max transaction amount
        uint256 tokenAmount = _clampValue(_marketingBalance, _maxTransactionAmount);
        
        // capture the contract's current ETH balance.
        uint256 initialBalance = address(this).balance;
        
        // swap tokens for ETH
        if (!_swapTokensForEth(tokenAmount)) return true;
    ...
    function _validateLotteryContestant(address account) private {
        if (_isEligibleForLottery(account)) {
    ...
    function _isEligibleForLottery(address account) private view returns(bool) {
        if (account == uniswapPair) return false;
        if (_isExcluded[account]) return false;
        if (balanceOf(account) < _lotteryContestantMinAmount) return false;
        return true;
    }
    
    function _drawLottery() private returns(bool) {
        ...
        // dont transfer more than max transaction amount
        uint256 tokenAmount = _clampValue(_lotteryBalance, _maxTransactionAmount);
        
        // first, give the lottery address the tokens
        _transferTokens(address(this), _lotteryAddress, tokenAmount);
        
        // then, give the winner their prize
        _transferTokens(_lotteryAddress, winnerAddress, tokenAmount);
        
        // remove token amount from the lottery balance
        _lotteryBalance -= tokenAmount;
        
        // reset the round
        _lotteryCurrentRound = 0;
        
        emit DrawLottery(winnerAddress, tokenAmount);
        
        return true;
    }
    
    function _takeTaxes(address sender, address recipient, uint256 transactionAmount, bool isSell) private returns(uint256) {
        // dont tax excluded accounts
        if (_isExcluded[sender] || _isExcluded[recipient]) return 0;
    ...
    function _swapTokensForEth(uint256 tokenAmount) private returns(bool) {
        ...
        try uniswapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            emit ErrorHandled("swapTokensForEth failed");
            return false;
        }
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 ethAmount) private returns(bool) {
        ...
        try uniswapRouter.addLiquidityETH{value: ethAmount}(
            ...
            emit ErrorHandled("addLiquidity failed");
            return false;
        }
    ...
    function _clampValue(uint256 value, uint256 max) private pure returns(uint256) {
