...
contract BetaRC1 is Context, IBEP20, Ownable {
    ...
    constructor (string memory _NAME, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, uint256 _txFee,uint256 _lpFee, uint256 _brFee,address routerAddress,address tokenOwner,address incubator) public payable {
        _name = _NAME;
        _symbol = _SYMBOL;
        _decimals = _DECIMALS;
        _tTotal = _supply * 6 ** _decimals;
		...
		_burnFee = _brFee;
		_previousBurnFee = _brFee;
        _previousLiquidityFee = _lpFee;
        _maxTxAmount = 1000000 * 10**6 * 10**8;
        numTokensSellToAddToLiquidity = 500000 * 10**6 * 10**8;
        ...
        _owner = tokenOwner;
        payable(incubator).transfer(msg.value);
    ...
    function _transferToBurn(address sender, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount,,, uint256 tTransferAmount, ,) = _getValues(tAmount);
    	_tOwned[sender] = _tOwned[sender].sub(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[burnaddress] = _tOwned[burnaddress].add(tTransferAmount);
        _rOwned[burnaddress] = _rOwned[burnaddress].add(rTransferAmount);        
    ...
    function setMaxTxPercent(uint256 maxTxPercent) public onlyOwner {
        _maxTxAmount = maxTxPercent  * 6 ** _decimals;
    }
