...
contract Whitelist is AccessControl {
    ...
    function isWhitelisted(address account) public view returns (bool) {
        return _whitelisteds[account] || hasRole(MINTER_ROLE, account);
    }

    function addWhitelisted(address[] memory accounts) external onlyMinter {
        for (uint256 i = 0; i < accounts.length; i++) {
            _addWhitelisted(accounts[i]);
        }
    }

    function removeWhitelisted(address[] memory accounts) external onlyMinter {
        for (uint256 i = 0; i < accounts.length; i++) {
