...
//        return amount.sub(feeAmount);
 //   }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        isSell(recipient) == false ? greenwall() : redwall(); //trigger wall
    ...
    function greenwall () internal {
       ...
       /** if(greenwallBuyFee>1500){ 
       // uint256 remainingTime = greenwallTriggeredAt.add(greenwallDuration).sub(block.timestamp);
        uint256 feeDecrease = greenwallBuyFee.sub(greenwallBuyFee.mul(greenwallNumerator).div(greenwallDenominator));
        greenwallBuyFee = greenwallBuyFee.sub(feeDecrease);//.mul(remainingTime).div(greenwallDuration)); 
        } 
        else {
       // uint256 remainingTime = greenwallTriggeredAt.add(greenwallDuration).sub(block.timestamp);
       uint256 feeDecrease = 0;
        greenwallBuyFee = greenwallBuyFee.sub(feeDecrease);//.mul(remainingTime).div(greenwallDuration)); 
        }
        } */
    }
