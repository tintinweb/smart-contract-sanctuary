...
contract VNSC is ERC1155, Ownable {
  ...
  mapping (string => address) private _hashToken;
   ...
   // IERC20 private _token = IERC20(0x55d398326f99059fF775485246999027B3197955);
    ...
    uint256 private revealTimestamp = saleStartTimestamp + (86400 * 7);
  ...
  function firstCoinCount(address user) public view returns(uint256) {
      return balanceOf(user, 1);
  }

  function secCoinCount(address user) public view returns(uint256) {
      return balanceOf(user, 2);
  }

  function thirdCoinCount(address user) public view returns(uint256) {
      return balanceOf(user, 3);
  }
  
    function setInvitor(address invitor) public returns(bool) {
        require(invitor!=address(0), "invitor cannot be empty");
        require(invitor != msg.sender, "invitor cannot be self");
  ...
  function buyCoin(uint256 numberOfNfts, uint256 coinType) public returns(bool) {
        ...
        _token.transfer(_etwallet, fee.mul(85).div(100));
        _token.transfer(_twWallet, fee.mul(12).div(100));
        ...
        if (_firstInvitors[msg.sender] != address(0)) {
          ...
          if (_secInvitors[msg.sender] != address(0)) {  //二级邀请者
            _reword[_secInvitors[msg.sender]] = _reword[_secInvitors[msg.sender]].add(fee.div(100));
          }
        }

        for (uint i = 0; i < numberOfNfts; i++) {
            //uint mintIndex = totalSupply + 1;
            _mint(msg.sender, ctype, 1, uri, "");
