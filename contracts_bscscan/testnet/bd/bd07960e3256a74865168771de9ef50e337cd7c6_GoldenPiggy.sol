...
contract GoldenPiggyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("GoldenPiggy_Dividend_Tracker", "GoldenPiggy_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "GoldenPiggy_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "GoldenPiggy_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GoldenPiggy contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GoldenPiggy_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "GoldenPiggy_Dividend_Tracker: Cannot update claimWait to same value");
...
contract GoldenPiggy is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    GoldenPiggyDividendTracker public dividendTracker;

    uint256 public maxTransactionAmount = 100000000000000 * (10**18);
    ...
    address payable public  marketingWallet = payable(0xD60a24B863e78eb10149025a129A55c3c304B991);
    address payable public  buyBackWallet = payable(0x881bE2f62Dfd02B126Df981CC85737EaFc54e66e);
    ...
    constructor() ERC20("Golden Piggy", "Gpiggy") {
        BNBRewardsFee = 3;
        liquidityFee = 5;
    	...
    	dividendTracker = new GoldenPiggyDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GoldenPiggy: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GoldenPiggy: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GoldenPiggy: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GoldenPiggy: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GoldenPiggy: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GoldenPiggy: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(marketingFee-buyBackDivisor);
        marketingWallet.transfer(marketingAmount);
        
        // worthy buyBack fee
        uint256 buyBackAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(buyBackDivisor);
