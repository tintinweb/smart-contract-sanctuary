...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 private frameDividends;
    uint256 private frameDividendsPerShare;
    ...
    function setShare(address shareholder, uint256 amount) external override onlyToken {
        ...
        if(amount == 0 && shares[shareholder].heldAmount > 0) {          
                ...
                if (shares[shareholder].heldAmount > minHoldAmountForRewards) {
                        ...
                        calculateFrame(shareholder, amount);
                }

                if(shares[shareholder].unpaidDividends == 1){
                    ...
                    if (!shareholderAdded[shareholder] && shares[shareholder].unpaidDividends > 0) {
                        addShareholder(shareholder); //add back into the index so we can sweep the divs if not taken within 30 days
        ...
        if(amount > 0 && shares[shareholder].heldAmount > 0){
            ...
            //User bought/sold/transferred more but still qualifies for rewards
            if (amount > minHoldAmountForRewards && shares[shareholder].heldAmount > minHoldAmountForRewards) {
                if (amount < shares[shareholder].heldAmount) { //sell/transfer
                ...
                if (amount > shares[shareholder].heldAmount) { //buy
                    shares[shareholder].unpaidDividends = shares[shareholder].unpaidDividends.add(getUnpaidEarnings(shareholder));
    ...
    function calculateFrame(address shareholder, uint256 amount) internal {
        if (frameDividends == 0) {return;}
        if (totalShares == 0) {return;}
        frameDividendsPerShare = dividendsPerShareAccuracyFactor.mul(frameDividends).div(totalShares);

        shares[shareholder].unpaidDividends = shares[shareholder].unpaidDividends.add(amount.mul(frameDividendsPerShare).div(dividendsPerShareAccuracyFactor));
        
        dividendsPerShare = dividendsPerShare.add(frameDividends);
    ...
    function getUnpaidEarnings(address shareholder) public view returns (uint256 unpaidAmount) {
        if(dividendsPerShare == 0){ return 0; } 
        if(shares[shareholder].amount == 0){ return 0; } 
        else {
             
             uint256 tempdivspershare = dividendsPerShareAccuracyFactor.mul(dividendsPerShare).div(totalShares);
             uint256 shareholderTotalDividends = shares[shareholder].amount.mul(tempdivspershare).div(dividendsPerShareAccuracyFactor);
    ...
    function getCumulativeDividends(uint256 _shares) internal view returns (uint256) {
         if(dividendsPerShare == 0){ return 0; } 
         
        else {
            uint256 tempdivspershare = dividendsPerShareAccuracyFactor.mul(dividendsPerShare).div(totalShares);
            return _shares.mul(tempdivspershare).div(dividendsPerShareAccuracyFactor);
            }
    }
    ...
    function holderInfo(address _holder) external view returns (uint256, uint256, uint256, uint256, uint256) {
        return (shares[_holder].amount, shares[_holder].heldAmount, shares[_holder].unpaidDividends, shares[_holder].totalRealised, shares[_holder].totalExcluded);
    }

    function mathInfo() external view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        return (totalShares, totalDividends, netDividends, totalDistributed, totalReinvested, totalWithdrawn, frameDividends, dividendsPerShare);
    }
...
contract KojiEarth is IBEP20, Auth {
    ...
    string constant _symbol = "KOJI v1.47";
    ...
    function ViewHolderInfo(address _address) external view returns (uint256 amount, uint256 held, uint256 unpaid, uint256 realised, uint256 excluded) {
        return distributor.holderInfo(_address);
    }
    
    function ViewMathInfo() external view returns (uint256 totalshares, uint256 totaldividends, uint256 netdividends, uint256 totaldistributed, uint256 totalreinvested, uint256 totalwithdrawn, uint256 framedivs, uint256 dividendspershare) {
