...
contract AG1Master is Ownable {
    ...
    // Market oracle provides the AntiG/USD exchange rate as an 18 decimal fixed point number.
    ...
    constructor(address _AntiG) public {
        deviationThreshold = 100 * 10 ** (DECIMALS-2);

        targetRate = (2 * 10**(DECIMALS-4));

        ceiling = 13370 * 10 ** DECIMALS;        //not 13337000..
    ...
    function rebase() public {
        ...
        uint256 supplyAfterRebase = AntiG.rebase(epoch, supplyDelta);
    ...
    function getRebaseValues() public view returns (uint256, int256) {
        ...
        if (supplyDelta > 0 && AntiG.totalSupply().add(uint256(supplyDelta)) > MAX_SUPPLY) {
            supplyDelta = (MAX_SUPPLY.sub(AntiG.totalSupply())).toInt256Safe();
        }
        ...
        returns (int256)
    {
        ...
        return AntiG.totalSupply().toInt256Safe()
