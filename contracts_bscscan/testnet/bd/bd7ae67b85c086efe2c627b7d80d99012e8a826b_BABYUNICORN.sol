...
contract BABYUNICORN is ERC20, Ownable {
    ...
    uint256 public maxSellTransactionAmount = 1000000000000000 * (10**18);  //10^15 to be changed to 10^12 after creating LP
    ...
    function updateMinimumTokenBalanceForDividends(uint256 newMinimumTokenBalanceForDividends) external onlyOwner {
        dividendTracker.updateMinimumTokenBalanceForDividends(newMinimumTokenBalanceForDividends);
    }
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        IERC20(UNI).transfer(_buybackWalletAddress, newBalance);
    }
...
contract BABYUNICORNDividendTracker is Ownable, DividendPayingToken {
    ...
    function updateMinimumTokenBalanceForDividends(uint256 newMinimumTokenBalanceForDividends) external onlyOwner {
        require(newMinimumTokenBalanceForDividends >= 0, "BABYUNICORN_Dividend_Tracker: newMinimumTokenBalanceForDividends must be greater than 0");
        emit UpdateMinimumTokenBalanceForDividends(newMinimumTokenBalanceForDividends, minimumTokenBalanceForDividends);
