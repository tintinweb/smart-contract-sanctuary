...
contract PositionToken is Context, IERC20, Ownable, Pausable {
    ...
    uint256 public constant BASE_MINT = 5 * 10**5 * 10**18;
    ...
    function viewRates() public view returns (uint256 r, uint256 t, uint256 rate) {
        (uint256 rSupply, uint256 tSupply) = _getCurrentSupply();
        return (rSupply, tSupply, rSupply.div(tSupply));
    }
    event CurrentRate(uint256 indexed r, uint256 indexed t, uint256 indexed rate);

    function _debugRates() private {
        (uint256 r, uint256 t, uint256 rate) = viewRates();
        emit CurrentRate(r, t, rate);
    }
    event MintDebug(uint256 indexed currentRate, uint256 indexed amount, uint256 indexed rTotal, uint256 tTotal);
    event AfterMint(uint256 indexed newRate);
    function _mint(address receiver, uint256 amount) private {
        ...
        _rTotal = _currentRate.mul(tSupply).add(_rTotal.sub(rSupply));
        emit MintDebug(_currentRate, amount, _rTotal, _tTotal);
        uint256 _newRate = _getRate();
        emit AfterMint(_newRate);
