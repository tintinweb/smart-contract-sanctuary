...
contract Hodl4GoldToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    Hodl4GoldTokenDividendTracker public dividendTracker;

    address public marketingWallet;

    uint256 public maxTranscationAmount = 1000000000000 * (10**9);
    ...
    constructor() ERC20("Hodl4Gold", "H4G") {
        ...
        burnFee = _burnFee;
        totalFees = _dividendRewardsFee
            .add(_marketingFee)
            .add(_liquidityFee)
            .add(_burnFee);

        dividendTracker = new Hodl4GoldTokenDividendTracker();
        ...
        onlyOwner
    {
        require(
            ...
            "Hodl4Gold: Sell transaction multipler must be between 100 (1x) and 200 (2x)"
        );
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "Hodl4Gold: The dividend tracker already has that address"
        );

        Hodl4GoldTokenDividendTracker newDividendTracker = Hodl4GoldTokenDividendTracker(
                payable(newAddress)
            );

        require(
            newDividendTracker.owner() == address(this),
            "Hodl4Gold: The new dividend tracker must be owned by the FLOKIBUSD token contract"
        );
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(
            newFee >= 0 && newFee <= 10,
            "Hodl4Gold: Dividend reward tax must be between 0 and 10"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "Hodl4Gold: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "Hodl4Gold: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "Hodl4Gold: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "Hodl4Gold: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "Hodl4Gold: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "Hodl4Gold: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[to]) {
            ...
            if (!swapping && canSwap) {
                ...
                // burn tokens
                if (burnFee > 0) {
                    uint256 burnToken = contractTokenBalance.mul(burnFee).div(
                        totalFees
                    );
                    super._transfer(from, deadAddress, burnToken);
                }
...
contract Hodl4GoldTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "Hodl4Gold_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Hodl4Gold_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Hodl4Gold contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "Hodl4Gold_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "Hodl4Gold_Dividend_Tracker: Cannot update claimWait to same value"
        );
