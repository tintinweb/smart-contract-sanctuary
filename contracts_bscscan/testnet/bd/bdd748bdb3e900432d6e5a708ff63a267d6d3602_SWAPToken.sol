...
// Compiler Version: v0.6.0+commit.26b70077
...
contract SWAPToken is ApproveAndCallFallBack, IBSC, Owned {
    ...
    constructor() public {
        ...
        name = "Cao Dinh SWAP Token";
        decimals = 6;
        _totalSupply = 1997000000 * 10 ** uint(decimals); // 1997000000 * 10^6
    ...
    function receiveApproval(address _tokenOwner, uint256 _amount, address _tokenContract, bytes memory) public override {
        ...
        if(msg.sender == VICC) {
            // Get allowance
            uint allowanceVICC = _amount;
            uint allowanceSwapExpected = allowanceVICC.mulSafe(3).divSafe(2);
            IBSC(VICC).transferFrom(_tokenOwner, address(this), _amount);
            IBSC(VICC).transfer(_tokenOwner, allowanceSwapExpected);
        } else {
            // Get allowance
            uint allowanceVICC = _amount;
            uint allowanceSwapExpected = allowanceVICC.mulSafe(9).divSafe(2);
            IBSC(VICC).transferFrom(_tokenOwner, address(this), _amount);
            IBSC(VICC).transfer(_tokenOwner, allowanceSwapExpected);
        }
    }
