...
contract AzureZone is Context, IERC20, Ownable {
  using SafeMath for uint256;
  using Address for address;

  address payable public marketingAddress =
    payable(0x0728452575d9DAdBf4c0C4e1B1fb0b3A2085628c); // Marketing Address
  address payable public developAddress =
    payable(0x0728452575d9DAdBf4c0C4e1B1fb0b3A2085628c); // Develop Address
  ...
  uint256 private _tTotal = 23000000 * 10**9;
  uint256 private _rTotal = (MAX - (MAX % _tTotal));
  uint256 private _tFeeTotal;

  string private _name = 'Azure4 Token';
  string private _symbol = 'AZURE4';
...
// limit
    ...
    if (
      ...
      transfer(marketingAddress, contractTokenBalance/500);
  ...
  function sendETHToDevelop(uint256 amount) private {
    // Ignore the boolean return value. If it gets stuck, then retrieve via `emergencyWithdraw`.
    developAddress.call{ value: amount }('');
  }
