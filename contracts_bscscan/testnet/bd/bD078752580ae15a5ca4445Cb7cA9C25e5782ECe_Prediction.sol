...
contract Prediction is Owned, Pausable {
    ...
    event NewTicket(address indexed buyer, bool label, uint256 indexed pid, uint256 indexed epoch, uint8 tid, uint256 index_by_label, uint256 price);
    ...
    function _calculateRewards(uint256 _pid, uint256 _epoch) internal {
        ...
        if (rewardAmount == 0) {
            ...
            Pool memory pool = pools[_pid];
            treasuryAmount = treasuryAmount.add(
                rewardAmount.mul(pool.ticketNumberInit)
            ); 
        }
