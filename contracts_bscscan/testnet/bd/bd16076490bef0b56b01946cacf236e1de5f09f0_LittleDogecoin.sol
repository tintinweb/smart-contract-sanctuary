...
abstract contract MerchantObject{
    ...
    uint256 public _refundPerTransaction;
    uint256 public _minHoldingForRefund;
    uint256 public _minMerchantHoldingForRefund;
    uint256 public _minSupplyToRefund;
    ...
    function addUpdateMerchant(address merchantAddress, uint duration, bool allowLifeTime, uint256 allocatedRate, uint256 maxRewardPerAddress, uint maxSubAccount, uint256 transactionRefund, string calldata meta) public onlyAdmin returns(uint code){
        ...
        _merchants[merchantAddress].expiry += duration > 0 ? block.timestamp.add(duration) : 0;
    ...
    function addUpdateSubAccount(address childAddress)public onlyMainMerchant returns(uint code){
        require((getParentAccount(childAddress) == msg.sender) &&
        (_merchants[msg.sender].exist == true) &&
        (_merchants[msg.sender].isSubAccount == true) &&
        (_merchants[msg.sender].maxSubAccounts < _merchants[msg.sender].maxSubAccounts.add(1)) &&
        (_merchants[childAddress].exist == false), '');
    ...
    /**
     * @dev add member, add and update membership.
     */
    function addUpdateMembership(address memberAddress, uint256 rewardRate, uint duration, uint256 reward, string calldata campaign, string calldata membershipMeta)public onlyMerchant returns(uint code){
        ...
        require((_members[memberAddress].exist && _members[memberAddress].locked == false) && // don't allowed to update if the member choose to lock their account.
        ...
        (_merchants[merchAddress].expiry >= time) && // merchant account already expired;
        ...
        //if(_members[memberAddress].exist == false && _merchants[merchAddress].allowLifeTime == false && duration == 0) return 10; // don't allow to use 0 duration if not set for main account
        ...
        (_merchants[merchAddress].allowLifeTime == false && _memberships[merchAddress][memberAddress].exist==false && duration != 0),''); //merchant is not allowed to set lifetime reqward;
        ...
        if (_memberships[merchAddress][memberAddress].exist==false){
            _memberships[merchAddress][memberAddress].expiry = duration!=0? time.add(duration):0;
            _memberships[merchAddress][memberAddress].membershipMeta = membershipMeta;
            _memberships[merchAddress][memberAddress].startDate = time;
    ...
    function updateLifeTimeMembership(address memberAddress, uint256 rewardRate, string calldata campaign, string calldata updateMeta)public onlyMerchant returns(uint code){
        ...
        (_memberships[merchAddress][memberAddress].rewardRate <= rewardRate) &&
        (_merchants[merchAddress].expiry >= block.timestamp),'E33');
        _memberships[merchAddress][memberAddress].rewardRate = rewardRate;
        //
        ...
        emit NewMembership(memberAddress, merchAddress, msg.sender, rewardRate, 0e9, 0, campaign, updateMeta);
        return 0;
    }
    ...
    function getClaimable(address merchantAddress, address memberAddress) public view returns(uint code, uint256 unclaimed){
        address merchAddress = getParentAccount(merchantAddress);
        uint time = block.timestamp;
        require((_members[memberAddress].exist == false) &&
        (_memberships[merchAddress][memberAddress].exist == true) &&
        (_memberships[merchAddress][memberAddress].expiry < time && _memberships[merchAddress][memberAddress].rewardRate != 0),'E34');
        uint span = 0;
