...
abstract contract FinanceBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	...
	string private constant NAME = "Ever Ada";
	string private constant SYMBOL = "EVA";
	...
	uint256 private constant _totalTokens = 50000000 * 10**DECIMALS;	//10 million total supply
	...
	// This function swaps a {tokenAmount} of EVAITY tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for EVA -> WBNB
	...
	// Returns how many more $`EVA tokens are needed in the contract before triggering a swap
...
contract EverAda is FinanceBase {
	// REWARD CYCLE
	...
	uint256 private _maxClaimAllowed = 500 ether; // Can only claim up to 500 bnb at a time.
	...
	uint256 private _mainBnbPoolSize = 1000 ether; // Any excess BNB after the main pool will be used as reserves to ensure consistency in rewards
	...
	bool private _reimburseAfterTokenClaimFailure; // If true, and EVA reward claim portion fails, the portion will be given as BNB instead
	...
	constructor () FinanceBase(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3) {
	...
	function claimReward(address user) private {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough EVA balance");
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        //user didn't set any token, give rewards in EVA 
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		// Depending on the % of $EVA tokens transferred, relative to the balance, we might need to extend the period
