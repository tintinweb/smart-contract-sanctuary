...
contract Lottery is Ownable, Initializable {
    ...
    modifier onlyAdmins(){
        require(admins[msg.sender] == true, 'you are not admin');
        _;
    }
    ...
    function withdrawx22(uint256 _amount) external onlyOwner {
        require(allLotteries[lotteryId].lotteryStatus == Status.Completed," Current lottery is not completed yes!");
    ...
    function buyTickets(
        ...
        userPurchase[_lotteryId][msg.sender].push(totalCost);
        ...
        if(containsNFT[msg.sender]  == true){
            _numberOfTickets = _numberOfTickets.mul(timesTicket).div(100);
        }
        ...
        bool gotBenefit = containsNFT[msg.sender];
    ...
    function drawWinners(uint256 _lotteryId) external onlyAdmins {
        ...
        if(ticketsSold[lotteryId]>=allLotteries[lotteryId].minTicketsToBuy){
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            uint256 ticketId = AllIndex[i];
            winnersList[_lotteryId].push(
                lotteryTicketOwner[_lotteryId][ticketId]
            );
    ...
    function GetBackX22(uint256 _lotteryId) external {
        ...
        require(allLotteries[_lotteryId].closingTimestamp < block.timestamp,"Lottery is not finished yet");
            uint256 totalBuy = userPurchase[_lotteryId][msg.sender].length;
        require(userTickets[msg.sender][_lotteryId].length > 0, "you did not buy anything");
            for(uint256 i = 1; i<=totalBuy;i++){
                uint256 X22Paid = userPurchase[_lotteryId][msg.sender][i];
                _x22.transfer(msg.sender,X22Paid);
            }
            hasWithdrawn[lotteryId][msg.sender] = true;
        if(allLotteries[_lotteryId].lotteryStatus != Status.Completed){
            allLotteries[_lotteryId].lotteryStatus = Status.Completed;
        }
    }
    ...
    function cancel() external onlyAdmins {
        require(
    ...
    function SaveNFTContainer(address user) external onlyAdmins {
            containsNFT[user] = true;
    }
    
    function RemoveNFTContainer(address user) external onlyAdmins {
            containsNFT[user] = false;
    }
    ...
    function ChangeBenefits(uint256 _timesTicket) external onlyAdmins {
    ...
    function makeAdmin(address _admin) external onlyOwner(){
        require(_admin != address(0),'can not make owner to zero address');
        admins[_admin] = true;
    }
    ...
    function randomGeneratorAddress() external view returns(address){
