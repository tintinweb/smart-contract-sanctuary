...
import "./ERC2981MultiAddressOnly.sol";
import "./interface/IMAX721.sol";
import "./modules/PaymentSplitter.sol";

contract FantomDeFiVerse is ERC721, ERC2981MultiAddressOnly, IMAX721, ERC165Storage, PaymentSplitter, Developer, Ownable {
  using Counters for Counters.Counter;
  Counters.Counter private _tokenIdCounter;
  Counters.Counter private _teamMintCounter;
  Counters.Counter private _pauseCounter;
  ...
  event UpdatedRoyalties(uint256 _new);
  ...
  bytes4 private constant _INTERFACE_ID_ERC2981MultiAddressOnly = 0xf0c93038;
  ...
  constructor() ERC721("Fantom De-Fi Verse (By Cyberstoned)", "FDV") {

    // ECR165 Interfaces Supported
    _registerInterface(_INTERFACE_ID_ERC721);
    _registerInterface(_INTERFACE_ID_IERC2981);
    _registerInterface(_INTERFACE_ID_ERC2981MultiAddressOnly);
 ...
 */
  ...
  function publicMint(uint256 amount) public payable {
    ...
    for (uint i = 0; i < amount; i++) {
      ...
      _setRoyaltyAddress(_tokenIdCounter.current(), msg.sender);
 ...
 */
  ...
  function setRoyaltyPercent(uint256 _percentage) public onlyOwner {
    _setRoyaltyPercentage(_percentage);
    emit UpdatedRoyalties(_percentage);
  }
...
abstract contract ERC2981MultiAddressOnly is IERC2981 {

  // ERC165
  // _setRoyaltyAddress(uint256,address) => 0x2a46c32d
  // _setRoyaltyPercentage(uint256) => 0xf0dad34f
  // royaltyInfo(uint256,uint256) => 0x2a55205a
  // ERC2981MultiAddressOnly => 0xf0c93038

  // Mapping
  mapping(uint256 => address) royaltyAddress;
  uint256 private percentage;

  // Set to be internal function _setRoyaltyAddress(uint256,address)
  // _setRoyalties(uint256,address,uint256) => 0x48417f4f
  function _setRoyaltyAddress(uint256 _tokenId, address _receiver) internal {
    royaltyAddress[_tokenId] = _receiver;
  }

  // Set to be internal function _setRoyaltyPercentage(uint256)
  // _setRoyaltyPercentage(uint256) => 0xf0dad34f
  function _setRoyaltyPercentage(uint256 _percent) internal {
    percentage = _percent;
  }
  ...
  function royaltyInfo(
    ...
    royaltyAmount = _salePrice * percentage / 100;
  }
}
