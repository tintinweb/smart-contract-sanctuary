...
contract BNBPOWER is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public marketingWallet = 0x878C9bC4e1e9ce5c93E6745456300D3F2822Aa6c;
    address public autoBoostWallet = 0x1AEfB144350368daF9dE68D3b8A556B298c8c7bf;
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "BNBPOWER: The router already has that address");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BNBPOWER: The dividend tracker already has that address");

        BNBPOWERDividendTracker newDividendTracker = BNBPOWERDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BNBPOWER: The new dividend tracker must be owned by the BNBPOWER token contract");
    ...
    function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromPeriodLimit[account] != excluded, "BNBPOWER: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxTxLimit[account] != excluded, "BNBPOWER: Account is already the value of 'excluded'");
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BNBPOWER: This account cannot send tokens until trading is enabled");
        }
...
contract BNBPOWERDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BNBPOWERDividendTracker", "Honey_DividendTracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 500000000 * (10**decimals()); //must hold 500000000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BNBPOWERDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BNBPOWERDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main Honey contract.");
    }
