...
contract PROBAA is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using SafeBEP20 for IBEP20;
	
    address public _burnAddress;
    address payable public _taxFeeWallet;
    address payable public _charityWallet;
	
    mapping (address => uint256) private _bBackOwned;
    mapping (address => uint256) private _dDevOwned;
    ...
    string private _name    = "PROBA-A";
    string private _symbol  = "PROBA-A";
    uint8 private _decimals = 9;
    
    bool public tradingEnabled = true;

    uint256 private taxFee          = 6; // Fee that gets re-distributed to holders.
    ...
    uint256 private _taxFee = taxFee;
    uint256 private _charityFee = charityFee;
    uint256 private totalFee = _taxFee.add(_charityFee);	
    ...
    constructor (address payable taxFeeWallet, address payable charityWallet, address pancakeSwapRouter) {
       
		
        _taxFeeWallet = taxFeeWallet;
        ...
        _isExcludeAddressFromMaxTx[_taxFeeWallet] = true;
    ...
    function setFeePercent(uint256 charity_Fee, uint256 tax_Fee) external onlyOwner() {
	...
	function setWalletAddress(address payable charityWallet, address payable taxFeeWallet) external onlyOwner() {
         _charityWallet = charityWallet;
         _taxFeeWallet = taxFeeWallet;
    }
	...
	function _takeCharityFee(uint256 tCharity) private {
        if (tCharity > 0 ) {
            emit Transfer(_msgSender(), _charityWallet, tCharity);
        }
    }
    ...
    function _takeTaxFee(uint256 tTax) private {
        if (tTax > 0 ) {
            emit Transfer(_msgSender(), _taxFeeWallet, tTax);
        }
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        //fee calculate
        //charity calculate
        uint256 taxFeeAmount = calculateTaxFee(tAmount);
        uint256 charityFeeAmount = calculateCharityFee(tAmount);
        uint256 remainedAmount;
        if (takeFee) {
            remainedAmount =  tAmount.sub(taxFeeAmount.add(charityFeeAmount));
        }
        else {
            remainedAmount =  tAmount;
        }
        _takeCharityFee(taxFeeAmount);
        _takeTaxFee(charityFeeAmount);
        emit Transfer(sender, recipient, remainedAmount);
    }
