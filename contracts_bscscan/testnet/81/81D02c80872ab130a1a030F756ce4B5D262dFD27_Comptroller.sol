...
contract Comptroller is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function setAtlantisSpeedInternal(AToken aToken, uint atlantisSpeed) internal {
        ...
        if (currentAtlantisSpeed != 0) {
            ...
            Market storage market = markets[address(aToken)];
            require(market.isListed == true, "atl market is not listed");
    ...
    function updateContributorRewards(address contributor) public {
        uint atlantisSpeed = atlContributorSpeeds[contributor];
        uint blockNumber = getBlockNumber();
    ...
    /**
     * @notice Claim all the atl accrued by holder in all markets
    ...
    /**
     * @notice Claim all the atl accrued by holder in the specified markets
     * @param holder The address to claim Atlantis for
    ...
    /**
     * @notice Claim all atl accrued by the holders
    ...
    function grantAtlantisInternal(address user, uint amount) internal returns (uint) {
        Atlantis atl = Atlantis(getAtlantisAddress());
        uint atlantisRemaining = atl.balanceOf(address(this));
        if (amount > 0 && amount <= atlantisRemaining) {
            atl.transfer(user, amount);
            return 0;
        }
        return amount;
    }
    ...
    function _grantAtlantis(address recipient, uint amount) public {
        require(adminOrInitializing(), "only admin can grant atl");
        uint amountLeft = grantAtlantisInternal(recipient, amount);
        require(amountLeft == 0, "insufficient atl for grant");
    ...
    function _setAtlantisSpeed(AToken aToken, uint atlantisSpeed) public {
        require(adminOrInitializing(), "only admin can set atl speed");
    ...
    function _setContributorAtlantisSpeed(address contributor, uint atlantisSpeed) public {
        ...
        atlContributorSpeeds[contributor] = atlantisSpeed;
    ...
    function getAtlantisAddress() public view returns (address) {
        return 0xC246DC80926609A290A0f9A3Bc7A36FDAF2154AF;
    }
}
...
contract ComptrollerV5Storage is ComptrollerV4Storage {
    ...
    mapping(address => uint) public atlContributorSpeeds;
