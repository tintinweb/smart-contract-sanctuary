...
// File: contracts/EarnDexTDividendPayingToken.sol
...
contract EarnDexT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable BUSD = address(0x8BaBbB98678facC7342735486C851ABD7A0d17Ca);
    address private _liqAdd = address(0xcA374305971114B028E679c0B69e43Cc95c6A6f7);
    address private _LPadd = address(0xa338E9E47bBAd592fF2D6D3573f3965AFe8B01FA);

    bool private swapping;
    bool public airdrop;

    EarnDexTDividendTracker public dividendTracker;

    address public liquidityWallet;
    address public liqAdd;
    address public LPadd = _LPadd;

    uint256 public maxBuyTranscationAmount = 5000000000 * (10**18); // No limit
    uint256 public maxSellTransactionAmount = 5000000000 * (10**18); // No limit
    ...
    constructor() public ERC20("EarnBNB", "EarnBNB") {
    	...
    	dividendTracker = new EarnDexTDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "EarnDexT: The dividend tracker already has that address");

        EarnDexTDividendTracker newDividendTracker = EarnDexTDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "EarnDexT: The new dividend tracker must be owned by the EarnDexT token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "EarnDexT: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "EarnDexT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "EarnDexT: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "EarnDexT: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "EarnDexT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "EarnDexT: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "EarnDexT: This account cannot send tokens until trading is enabled");
        }
        ...
        if(
            ...
            uint256 bnbrewardstoken = contractTokenBalance.mul(6).div(totalFees);
            swapTokensForBNB(bnbrewardstoken);
            
            uint256 liqtokens = contractTokenBalance.mul(5).div(totalFees);
            swapTokensForBNB2(liqtokens);
            
            uint256 liqtax = contractTokenBalance.mul(5).div(totalFees);
            swapTokensForBNB3(liqtax);
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
     ...
     function swapTokensForBNB3(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        ...
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            0, // accept any amount of ETH
            path,
            0xf514d8758f153cbc3af9B99D765129832f141a81,
            block.timestamp
        );
...
contract EarnDexTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("EarnDexT_Dividend_Tracker", "EarnDexT_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "EarnDexT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "EarnDexT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EarnDexT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "EarnDexT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EarnDexT_Dividend_Tracker: Cannot update claimWait to same value");
