...
PLANS:
  plan 1 6% 45 days 180% ROI with 25% tax = 270% ROI
  plan 2 7% 35 days 250% ROI with 25% tax = 245% ROI
  plan 3 8% 25 days 161% ROI with 25% tax = 200% ROI
  plan 4 9% 15 days 160% ROI with 25% tax = 135% ROI
...
ANTI WHALE/CUT OFF TIME FEATURES:
 32hrs cut off time, user should re-invest or withdraw within the cut off period for the timer to reset.
 Users can only withdraw a maximum amount of $2,000. Excess dividends are sent back to the user's account available for the next withdrawal.
...
contract moneyPrinterFarm {
	...
	/*! anti-whale */
    uint256 public MAX_WITHDRAW_PERCENTAGE = 1000; // initially set to get full amount. would lessen this if needed.
    uint256 public CONTRACT_PERCENTAGE_CHECK = 50;
	uint256 public CUTOFF_STEP = 32 hours;
	...
	constructor(address payable owner, address payable projectAddress1, address payable projectAddress2, address payable projectAddress3, address payable marketingAddress, address payable communityAddress) {
        ...
        **/
        ...
        plans.push(Plan(45, 60, 10000000000000000000, 0, 0, 0, 0, true));   /** $10 **/                
        plans.push(Plan(35, 70, 10000000000000000000, 0, 0, 0, 0, true));   /** $50 **/                  
        plans.push(Plan(25, 80, 10000000000000000000, 0, 0, 0, 0, true));  /** $200 **/                  
        plans.push(Plan(15, 90, 10000000000000000000, 0, 0, 0, 0, true)); /** $300 **/
	}
	...
	function reinvest(uint8 plan) public {
	    ...
	    token.transfer(communityWallet, community);
	...
	function withdraw() public {
        ...
        /** if users withdrawable amount is more than the percentage set in CONTRACT_PERCENTAGE_CHECK of the contract balance, 
        less withdrawable amount by MAX_WITHDRAW_PERCENTAGE **/
        if(totalAmount > contractBalance.mul(CONTRACT_PERCENTAGE_CHECK).div(PERCENTS_DIVIDER)) {
            totalAmount = totalAmount.mul(MAX_WITHDRAW_PERCENTAGE).div(PERCENTS_DIVIDER);
    ...
    function addAdditionalPlans(uint256 time, uint256 percent, uint256 mininvest, bool planActivated) external {
    ...
    function _MAX_WITHDRAW_PERCENTAGE(uint256 value) external {
        require(msg.sender == contractAddr);
        MAX_WITHDRAW_PERCENTAGE = value;
    }
    
    function _CONTRACT_PERCENTAGE_CHECK(uint256 value) external {
        require(msg.sender == contractAddr);
        CONTRACT_PERCENTAGE_CHECK = value;
    }
