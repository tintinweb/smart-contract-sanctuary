...
contract Fidometa is Context, IERC20, Ownable {
   ...
   /* * @dev Burns a specific amount of tokens.
    ...
    // event LockDetail(uint startTime, uint lockedToken,uint unlockDate);
    ...
    function reduceLockTime(address target_, uint256 timeindays) external onlyOwner{
     ...
     event HighestBidIncreased(uint startTime, uint millis_days,uint expiry);
     function unlockToken(address target_, uint256 amount) external onlyOwner{
        ...
        require(lockedToken >= 0, "No tokens lock found");
        uint  timeInDays = locks[target_].timeInDays;
        uint  startTime = locks[target_].startTime;
        uint millis_days = timeInDays * 1 days;
        uint expiry = startTime + millis_days;
        require(block.timestamp >= expiry, "Token is in locking period");
        locks[target_].lockedToken = locks[target_].lockedToken - amount;
       emit  HighestBidIncreased(startTime,millis_days,expiry );
    }
    ...
    function _transfer(
        ...
        if(lockedToken > 0){
            ...
            require(amount <= withdrawable, "transfer amount is greater than unlocked amount you have"); 
        }
