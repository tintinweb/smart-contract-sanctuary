...
contract MockChainCashTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public immutable CCASH;
  ...
  address public sCCASH;
  uint256 public sCCASHQueue; // Delays change to sCCASH address
  ...
  constructor(
    ...
    // isLiquidityToken[ _CCASHDAI ] = true;
    // liquidityTokens.push( _CCASHDAI );

    blocksNeededForQueue = _blocksNeededForQueue;
  }

  /**
        @notice allow approved address to deposit an asset for CCASH
        @param _amount uint
  ...
  function deposit(
    ...
    (_token, _amount);
    // mint CCASH needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(CCASH).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn CCASH for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOfToken(_token, _amount);
    ICCASHERC20(CCASH).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sCCASH).balanceOf(msg.sender); // Can only borrow against sCCASH held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with CCASH
        @param _amount uint
     */
  function repayDebtWithCCASH(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");

    ICCASHERC20(CCASH).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, CCASH, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(CCASH).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(CCASH).totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns CCASH valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  {
    if (isReserveToken[_token]) {
      // convert amount to match CCASH decimals
      value_ = _amount.mul(10**IERC20(CCASH).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SCCASH) {
      // 9
      sCCASHQueue = block.number.add(blocksNeededForQueue);
    } else return false;
