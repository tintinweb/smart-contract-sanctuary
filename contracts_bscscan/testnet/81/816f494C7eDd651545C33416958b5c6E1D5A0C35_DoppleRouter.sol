...
import "./DoppleRouterOwnable.sol";
...
contract DoppleRouter is DoppleRouterOwnable, Initializable, ReentrancyGuardUpgradeable {
    ...
    function swap(
        ...
        require(_router != address(0), "DoppleRouter::swap: Address 0");
        require(isSupported[_router], "DoppleRouter::swap: router is not supported");

        IERC20 _tokenA = getToken(_router, _tokenIndexFrom);
        IERC20 _tokenB = getToken(_router, _tokenIndexTo);



        require(
            _tokenA.transferFrom(msg.sender, address(this), _dx),
            "DoppleRouter::swap: transferFrom _tokenA failed"
        );

        uint256 dy = ISwap(_router).swap(_tokenIndexFrom, _tokenIndexTo, _dx, _minDy, _deadline);

        uint256 fee = dy.mul(protocolFee).div(1e18);
        uint256 actualDy = dy.sub(fee);

        require(_tokenB.transfer(msg.sender, actualDy), "DoppleRouter::swap: transfer _tokenB failed");

        emit SwapWithDoppleRouter(_router, address(_tokenA), _dx, address(_tokenB), actualDy, fee);
    }
    ...
    function approveTokenForRouter(address _router, address _token) public onlyOwner {
        IERC20(_token).approve(_router, 2**256 - 1);
    }

    function removeRouter(address _router) external onlyOwner {
        require(_router != address(0), "DoppleRouter::removeRouter: Address 0");
        require(isSupported[_router] != false, "DoppleRouter::setRouter: router is already removed");

        for (uint8 i; i < tokens[_router].length; i++) {
            require(IERC20(tokens[_router][i]).approve(_router, 0), "DoppleRouter::setRouter: approve failed");
        }
    ...
    function calculateSwap(
        ...
        minDY = ISwap(_router).calculateSwap(_tokenIndexFrom, _tokenIndexTo, dx);
    }

    // pool data view functions
    function getA(address _router) external view returns (uint256) {
        return ISwap(_router).getA();
    }

    function getAllowlist(address _router) external view returns (IAllowlist) {
        return ISwap(_router).getAllowlist();
    }
    ...
    function getTokenIndex(address _router, address _tokenAddress) external view returns (uint8) {
        return ISwap(_router).getTokenIndex(_tokenAddress);
    }

    function getTokenBalance(address _router, uint8 _index) external view returns (uint256) {
        return ISwap(_router).getTokenBalance(_index);
    }

    function getVirtualPrice(address _router) external view returns (uint256) {
        return ISwap(_router).getVirtualPrice();
    }

    function isGuarded(address _router) external view returns (bool) {
        return ISwap(_router).isGuarded();
    }
    ...
    function calculateTokenAmount(
        ...
        return ISwap(_router).calculateTokenAmount(_amounts, _deposit);
    }

    function calculateRemoveLiquidity(address _router, uint256 _amount) external view returns (uint256[] memory) {
        return ISwap(_router).calculateRemoveLiquidity(_amount);
    }
    ...
    function calculateRemoveLiquidityOneToken(
        ...
        return ISwap(_router).calculateRemoveLiquidityOneToken(_tokenAmount, _tokenIndex);
    }

    function getSomeText() external pure returns (string memory _msg) {
        _msg = "function number 20";
    }
}
