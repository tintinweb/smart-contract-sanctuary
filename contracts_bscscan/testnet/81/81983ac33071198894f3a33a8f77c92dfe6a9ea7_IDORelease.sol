...
contract IDORelease is Ownable {
    ...
    function swapAccountOf(address _account) public view returns (uint256 TotalSwapDtuAmount,uint256 [] memory ReleaseOrdersIndex){
        SwapAccount storage account = swapAccounts[_account];
        return (account.totalSwapDtuAmount,account.releaseOrdersIndex);
    }
