...
contract KojiOracle {
    ...
    function getKojiUSDPrice() public view returns (uint256) {

      uint256 bnbusdprice = getLatestPrice();
      bnbusdprice = bnbusdprice.mul(10);
      ...
      uint256 pooledBNBUSD = pooledBNB.mul(bnbusdprice);
      uint256 kojiUSD = pooledKOJI.div(pooledBNBUSD);

      return kojiUSD;
    }

    function getMinKOJITier1Amount() public view returns (uint256) {
      return minTier1Amount.div(getKojiUSDPrice());

    }

    function getMinKOJITier2Amount() public view returns (uint256) {
      return minTier2Amount.div(getKojiUSDPrice());
    }
}