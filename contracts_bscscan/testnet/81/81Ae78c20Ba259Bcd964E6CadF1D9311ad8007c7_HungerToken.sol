...
contract HungerToken is Context, IERC20, Ownable {
    ...
    struct Reflections {
        ...
        uint256 reflectionsCirculating;
    ...
    function includeInReward(address account) external onlyOwner {
        ...
        if (balance > 0) {
            ...
            _reflections.reflectionsCirculating += balance * reflectionRate;
            _reflections.reflectionSupply += balance * reflectionRate;
        }
    }
    ...
    function _transfer(
        ...
        if (!isExcludedTo.fromReward) {
            _reflectionBalance[to] += (amount - tokenFees) * reflectionRate;
    ...
    function _handleReflections(
        ...
        if (isExcludedFrom.fromReward && !isExcludedTo.fromReward) {
            reflections.reflectionsCirculating += (amount * reflectionRate);
            reflections.reflectionSupply += amount;
    ...
    function _getReflectionRate() private view returns (uint256) {
        if (
            _reflections.reflectionsCirculating > 0 &&
            _reflections.reflectionsCirculating >= _reflections.reflectionSupply
        ) {
            return
                _reflections.reflectionsCirculating /
                _reflections.reflectionSupply;
        } else {
