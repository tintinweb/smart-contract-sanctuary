...
interface RandomNumberGenerator {
    ...
    function fetchNumber() external returns (uint256);
}
...
contract Lottery is Ownable, Initializable {
    ...
    event TicketPurchased(address buyer, uint256 lotteryId, uint256 numberOfTickets);
    event AmountWithdrawn(uint256 amount);
    event WinnerRewarded(address user);
    event RandomNumber(uint256 value);
    ...
    mapping (uint256 => uint256) public lotteryX22Amount;
    ...
    uint256 public _lotteryIdCounter;
    ...
    function createNewLottery(uint8[] calldata _prizeDistribution,
        string[] calldata _nftlinks, uint256 _costPerTicket, uint256 _minTicketsToBuy,
        uint256 _closingTimestamp, uint256 _noOfWinners) external onlyOwner()
    {
        // checking for starting time is strictly greater than current time
        require(_costPerTicket != 0, "cost cannot be zero");
        require(LotteryStart != 0 && LotteryStart < _closingTimestamp && LotteryStart > block.timestamp, "Timestamps for lottery Invalid");
        require(_nftlinks.length == _noOfWinners, "NFTs ID Invalid");
        require(_prizeDistribution.length == _noOfWinners, "Prize Distribution Invalid");
        require(_minTicketsToBuy >= _noOfWinners, "Min tickets to purchase should be more than or equal to No. of winners!");
        require(_noOfWinners > 0 , "There should some winners for lottery!");
        uint256 prizeDistributionTotal = 0;
        for (uint256 i=0; i<_prizeDistribution.length; i++) {
            prizeDistributionTotal = prizeDistributionTotal.add(uint256(_prizeDistribution[i]));
        }
        ...
        if (_lotteryIdCounter == 0) {
            ...
            require(allLotteries[_lotteryIdCounter].lotteryStatus == Status.Completed, "Past lottery is still in progress, you can't create New!");
            _ticketIdCounter = 0;
        }
        ...
        for(uint8 i=0; i< _noOfWinners; i++) {
        ...
        allLotteries[lotteryId].noOfWinners = _noOfWinners;
    ...
    function costToBuyTickets(uint256 _lotteryId, uint256 _numberOfTickets) public view  returns (uint256 totalCost) {
        uint256 costPerTicket = allLotteries[_lotteryId].costPerTicket;
        totalCost = costPerTicket.mul(_numberOfTickets);
    }
    function buyTickets(uint256 _lotteryId, uint256 _numberOfTickets, bool benefits) external  {
        // Ensure valid timings for the lottery
        require(block.timestamp >= allLotteries[_lotteryId].startingTimestamp, "Invalid time to start");
        require(block.timestamp < allLotteries[_lotteryId].closingTimestamp, "Invalid time: lottery time up");

        if (allLotteries[_lotteryId].lotteryStatus == Status.NotStarted) {
                allLotteries[_lotteryId].lotteryStatus = Status.Open;
        }
        require(allLotteries[_lotteryId].lotteryStatus == Status.Open, "lottery not in open state");
        ...
        for (uint256 i=0; i<_numberOfTickets; i++) {
            TicketInfo memory newTicket = TicketInfo(msg.sender, _ticketIdCounter , _lotteryId);
        ...
        lotteryX22Amount[_lotteryId] = lotteryX22Amount[_lotteryId].add(totalCost);
        
        // posting user's request for extra benefits
        if(benefits == true){
            requesters.push(msg.sender);
            requests[_lotteryId][msg.sender] = _numberOfTickets;
        }
        emit TicketPurchased(msg.sender, _lotteryId, _numberOfTickets);
    }

    function drawWinners(uint256 _lotteryId, uint256 seed, uint256 seedDifference) external onlyOwner()  {
        ...
        require(allLotteries[_lotteryId].lotteryStatus == Status.Open, "Lottery status is not open");
        require(allLotteries[_lotteryId].closingTimestamp <= block.timestamp, "Lottery time is not up!");
        ...
        if (ticketsSold[_lotteryId] < allLotteries[_lotteryId].minTicketsToBuy) {
        ...
        uint256 noOfWinners = allLotteries[lotteryId].noOfWinners;
        for (uint i=0;i<noOfWinners;i++) {
            _randomGenerator.getRandomNumber(seed);
            seed = seed.add(seedDifference);
            uint256 index = _randomGenerator.fetchNumber();
            index = index.mod(ticketsSold[_lotteryId]);
            // uint256 randomResult = index;
            index++;
            emit RandomNumber(index);
            winnersList[_lotteryId].push(lotteryTicketOwner[_lotteryId][index]);
        }
        // draw three winning numbers
        uint256 balance = _x22.balanceOf(address(this));
        for (uint i=0;i<noOfWinners;i++) {
                address user = winnersList[_lotteryId][i];
                uint256 amount = balance.mul(allLotteries[_lotteryId].prizeDistribution[i]).div(100);
                if(amount>0){
                    _x22.transfer(user, amount);
                    emit WinningAmount(amount);
                }
                emit WinnerRewarded(user);
        }
    ...
    function setLotteryTiming(uint256 _time) external onlyOwner(){
        require(_time > block.timestamp, 'Start timestamp for lottery is not valid!');
        LotteryStart = _time;
    }
    ...
    function fullFillRequests()external onlyOwner(){
        ...
        for(uint256 i=0;i<requesters.length;i++){
            ...
            if(containsNFT[temp] == true){
                ...
                TicketInfo memory newTicket = TicketInfo(msg.sender, _ticketIdCounter , lotteryId);
                _allTickets[_ticketIdCounter] = newTicket;
                userTickets[msg.sender][lotteryId].push(_ticketIdCounter);
                ticketsToALottery[lotteryId].push(_ticketIdCounter);
                lotteryTicketOwner[lotteryId][_ticketIdCounter] = msg.sender;
                _ticketIdCounter++;
                }
    ...
    function cancel() external onlyOwner(){
        require((allLotteries[lotteryId].startingTimestamp < block.timestamp)&&(allLotteries[lotteryId].closingTimestamp > block.timestamp), "The lottery is started but not finished yet you can't cancel it!");
        if(allLotteries[lotteryId].closingTimestamp < block.timestamp){
            require(ticketsSold[lotteryId] == 0,'Users bought tickets so you are not able to cancel the lottery!');
        }
    ...
    function fetchLottery(uint256 _LotteryId)external view returns(LotteryInfo memory info){
         return (allLotteries[_LotteryId]);
    }

    function SaveNFTContainers(address[] memory user, bool[] memory _contains) external onlyOwner(){
        for(uint256 i =0; i< user.length; i++){
         containsNFT[user[i]] = _contains[i];
        }
    }
