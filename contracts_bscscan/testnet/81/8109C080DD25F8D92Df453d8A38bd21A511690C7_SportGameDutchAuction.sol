...
contract SportGameDutchAuction is SportGameAuctionBase {
    ...
    event NFTForSaleEvent(address owner, uint256 tokenId, uint256 peakPrice, uint256 floorPrice, string symbol, uint256 deadline);
    event PurchaseNFTEvent(address to, uint256 tokenId, string symbol, uint256 amount, uint256 deadline);
    ...
    function nftForSale(uint256 tokenId, uint256 peakPrice, uint256 floorPrice, string memory symbol, uint256 deadline) public {
        ...
        _tokenPrices[tokenId] = NFTSaleModel(tokenId, peakPrice, floorPrice, symbol, deadline);
        
        emit NFTForSaleEvent(_msgSender(), tokenId, peakPrice, floorPrice, symbol, deadline);
    }
    ...
    function purchaseNFT(uint256 tokenId) public payable {
        ...
        require(_tokenContract.balanceOf(_msgSender()) >= model.peakPrice, "Err: Balance-Insufficient");
        require(_tokenContract.allowance(_msgSender(), address(this)) >= model.peakPrice, "Err: Allowance-Insufficient");
        ...
        require(_tokenContract.transferFrom(_msgSender(), _nftContract.ownerOf(tokenId), model.peakPrice), "Err: Transfer-Failure");
        
        delete _tokenPrices[tokenId];

        emit PurchaseNFTEvent(_msgSender(), tokenId, model.symbol, model.peakPrice, block.timestamp);
    }
    ...
    function acceptOffer(address _signer, uint256 offerPrice, uint256 tokenId, string memory _message, uint nonce, bytes memory signature) public {
        ...
        NFTSaleModel memory model = _tokenPrices[tokenId];
        require(offerPrice < model.peakPrice && offerPrice > model.floorPrice, "Err: Offer-Price-Invalid");
