...
contract Test is IBEP20, Ownable
{
    ...
    uint256 public constant _totalSupply = 1000 * 10**6 * 10**9;//equals 100,000,000,000
    ...
    bool SlaughterFestActive = false;
    ...
    address public constant TeamWallet=0x40eA6A1471202FFBa9a051973e6eB75531291F1e;
    ...
    //TODO: Change to Mainnet
    //address tokenToClaim=0xe9e7cea3dedca5984780bafc599bd69add087d56;//BUSD
    //Testnet BUSD
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            require(SlaughterFestActive); //Must be after slaughtertime for selling to be enabled.
            tax=_getsellTax();
    ...
    function _getsellTax() public view returns (uint8 tax)
    {
    ...
    function _calculateSlaughterTax() private view returns (uint8){
        ...
        if(block.timestamp >= StartOfSlaughter && block.timestamp <= EndOfSlaughter){
            uint8 Tax=uint8(20+(SlaughterMaxTax*(EndOfSlaughter-block.timestamp)/SlaughterTime));
            return Tax; 
        }else{
        return _sellTax;
        }
    ...
    bool public noDiamondHandsFeeOn=false;
    ...
    function getSlaughterfest() public returns(bool FestActive){
        if(block.timestamp>(launchTimestamp+SlaughterTime)){ 
            SlaughterFestActive = true;
        }
        return(SlaughterFestActive);
    }
    ...
    function getTaxes() public view returns(uint256 liquidityTax,uint256 StakingTax, uint256 buyTax, uint256 sellTax, uint256 transferTax){
        sellTax = _getsellTax();
        return (_liquidityTax,_stakingTax,_buyTax,_sellTax,_transferTax);
    }
    ...
    function SetupCreateLPandLaunch(uint8 TeamTokenPercent) public payable onlyTeam{
