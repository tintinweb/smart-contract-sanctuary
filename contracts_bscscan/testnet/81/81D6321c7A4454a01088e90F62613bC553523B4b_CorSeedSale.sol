...
contract CorSeedSale is Ownable {
    ...
    struct Seeder {
        uint256 realBuyAmount; // số lượng đã mua thực tế
        uint256 hadWithdraw; // số lượng đã rút
    }
    ...
    // thiết lập lượng token cho mỗi seeder
    ...
    // kiểm tra Thông tin seeder
    function getSeederInfo(address addr) public view returns (Seeder memory) {
        return seeders[addr];
    }

    // tính số tháng tính từ thời điểm release đến block hiện tại
    ...
    // tính số lượng token đã realease tính đến tháng hiện tại
    ...
    // kiểm tra số dư có thể rút hiện tại
    ...
    // seeder clainToken để nhận về token Corgi đã hết hạn release
    // tính vơi lượng release hàng tháng là 5%, số tháng release hết là 20 tháng
    ...
    // Trường hộp khẩn cấp 7 admin chọn vote để mở kích hoạt trạng thái cho phép claim khẩn cấp toàn bộ lượn token còn lại của mỗi seeder
