...
contract KabyStakeSummon is Ownable, Pausable {
  using SafeERC20 for IERC20;

  mapping(uint256 => mapping(address => bool)) public users;
  mapping(uint256 => mapping(address => bool)) public claimed;
  mapping(uint256 => uint256) amountClaimed;
  mapping(uint256 => uint256) amountRemaning;
  uint256 public listActive;

  IKabyHero public immutable kabyHero;
  ...
  event Active(uint256 listId, uint256 timestamp);
  ...
  modifier claimable() {
    require(
        !claimed[listActive][msg.sender] && 
        users[listActive][msg.sender], "Cannot claim");
    _;
  }

  function setVersion(uint256 _versionId) external onlyOwner {
    versionId = _versionId;
  }

  function setUsers(uint256 _listId, address[] memory _users, bool _value) external onlyOwner {
    for (uint256 i = 0; i < _users.length; i++) {
      address account = _users[i];
      if (_value != users[_listId][account]) {
        users[_listId][_users[i]] = _value;
        if (_value) {
          amountRemaning[_listId]++;
        } else {
          amountRemaning[_listId]--;
        }
      }
  ...
  function setListActive(uint256 _listId) external onlyOwner {
    listActive = _listId;
    emit Active(listActive, block.timestamp);
  }
  ...
  function updatePrice(IERC20 _acceptedToken, uint256 _price) external onlyOwner {
    acceptedToken = _acceptedToken;
    price = _price;
    emit UpdatePrice(price, block.timestamp);
  }

  function claim(uint256 allowancePrice) external claimable whenNotPaused {
    require(price <= allowancePrice, "Price over");
    amountClaimed[listActive]++;
    amountRemaning[listActive]--;
    claimed[listActive][msg.sender] = true;
    if (price > 0) {
      acceptedToken.safeTransferFrom(msg.sender, owner(), price);
    }
    kabyHero.mintHero(versionId, 1, msg.sender);
    emit Claimed(msg.sender, versionId, listActive, price, block.timestamp);
  }

  function canClaim(address user) external view returns(bool) {
    return users[listActive][user] && !claimed[listActive][user];
  }

  function getClaimedAndRemaining() external view returns(uint256 claimed, uint256 remaining) {
    return (amountClaimed[listActive], amountRemaning[listActive]);
  }
}