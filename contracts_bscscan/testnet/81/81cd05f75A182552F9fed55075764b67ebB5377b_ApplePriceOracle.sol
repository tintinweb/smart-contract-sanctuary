...
contract ApplePriceOracle is PriceOracle {
    ...
    address public tokenApple;
    address public tokenABC;
    ...
    function setTokenApple (address _tokenAddress) external {
        tokenApple = _tokenAddress;
    }

    function setTokenABC (address _tokenAddress) external {
        tokenABC = _tokenAddress;
    }
    ...
    function getUnderlyingPrice(AToken aToken) public view returns (uint) {
        if (compareStrings(aToken.symbol(), "abcBNB")) {
            IStdReference.ReferenceData memory data = ref.getReferenceData("BNB", "USD");
            return data.rate;
        } else if (compareStrings(aToken.symbol(), "abcAPPLE")) {
            return getAmountsOut(1000000000000000000,tokenApple, tokenBUSD);
        } else if (compareStrings(aToken.symbol(), "abcABC")) {            
            return getAmountsOut(1000000000000000000,tokenABC, tokenBUSD);
        } else {
            uint256 price;
