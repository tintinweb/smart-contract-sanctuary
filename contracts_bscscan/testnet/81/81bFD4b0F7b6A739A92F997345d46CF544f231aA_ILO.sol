...
contract Oppa is BEP20("Oppa", "OPPA") {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "OPPA::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "OPPA::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "OPPA::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "OPPA::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying OPPA (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "OPPA::_writeCheckpoint: block number exceeds 32 bits"
        );
...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    function _transfer(
        ...
        /**
         * @dev calculates tax and adds it to the tax address
         */
        _balances[sender] -= amount; // subtract the full amount
        // _balances[taxaddress] += fee; // add the fee to the admin balance
