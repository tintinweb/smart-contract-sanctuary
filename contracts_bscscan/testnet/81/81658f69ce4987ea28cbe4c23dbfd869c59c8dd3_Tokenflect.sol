...
// 15%~ Slippage required: 8% ADA Auto-reflect /5% Dev & Marketing Wallet/2% Auto-LP
    ...
    contract Tokenflect is IERC20, Ownable {
        using SafeMath for uint256;
        ...
        function setTradingO(bool _enabled) public onlyOwner {
        ...
        function swapAndSendToFee(uint256 tokens) private  {
            ...
            IERC20(BUSD).transfer(_teamWalletAddress, newBalance);
        }
        ...
        function swapAndSendToAlgo(uint256 tokens) private  {
            ...
            swapTokensForBUSD(tokens);
            uint256 newBalance = (IERC20(BUSD).balanceOf(address(this))).sub(initialBUSDBalance);
            IERC20(BUSD).transfer(_algoWalletAddress, newBalance);
        }
        ...
        function swapAndSendDividends(uint256 tokens) private{
            ...
            bool successPartners = IERC20(BUSD).transfer(address(dividendTracker), dividendsPartners);
            bool successHolders  = IERC20(BUSD).transfer(address(dividendTracker), dividendsHolders);
