...
contract NftpadFarmingPool is ReentrancyGuard {
    ...
    function notifyRewardAmount(uint256 reward, uint256 rewardsDuration) external updateReward(address(0)) {
        require(msg.sender == rewardsDistribution, "Caller is not Reward distribution contracts");
...
contract NftPad is Ownable, Pausable {
	...
	function setStakeToken(address token, uint256 ratePoint) external onlyOwner {
	...
	function stake(address token, uint256 amount) external whenNotPaused {
		...
		userStakedBalance[msg.sender][token] = userStakedBalance[msg.sender][token].add(amount);
	...
	function unStake(address token, uint256 amount) external whenNotPaused {
		require(amount > 0, 'Oops!Invalid-amount');
		require(stakeToken[token] != 0, 'Oops!Invalid-token');
		require(userStakedBalance[msg.sender][token] >= amount, 'Oops!Not-enough-balance-to-unstake');
		IERC20(token).transfer(msg.sender, amount);
		uint256 point = stakeToken[token].mul(amount);
		userPoints[msg.sender] = userPoints[msg.sender].sub(point);
		userStakedBalance[msg.sender][token] = userStakedBalance[msg.sender][token].sub(amount);
