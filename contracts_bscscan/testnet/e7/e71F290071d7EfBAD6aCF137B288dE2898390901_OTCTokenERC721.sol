...
contract OTCTokenERC721 is ERC721Pausable, AccessControl {
  ...
  event OTCTokenERC721Mint(
    uint256 tokenId,
    uint256 timestamp,
    uint256 otcTokenAmount,
  ...
  function retrieve(uint256 tokenId) public {
    ...
    address otcTokenAddress = nftsToTokenData[tokenId].tokenAddress;
    if(otcTokenAddress == address(0)) {
      msg.sender.transfer(amount);
    } else {
      IERC20 token = IERC20(otcTokenAddress);
  ...
  function mint(
    ...
    nftsToTokenData[tokenId] = TokenData(paymentTokenAddress, paymentTokenAmount);

    emit OTCTokenERC721Mint(tokenId, block.timestamp, paymentTokenAmount, paymentTokenAddress, msg.sender, tokenData);
  }
