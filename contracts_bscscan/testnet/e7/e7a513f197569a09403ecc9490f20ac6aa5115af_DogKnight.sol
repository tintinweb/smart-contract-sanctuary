...
contract DogKnight is Context, IERC20, IERC20Metadata,Ownable {
    ...
    string private _name = "Dog Knight";
    string private _symbol = "DK";
    uint256 private _initSupply = 100000000*10**18;
    
    //2021-09-15 22:30:00
	uint256 constant startTime = 1631284200;
	uint256 constant endTime = 1636554600;
    
    uint256 public dexTaxFee = 600; //take fee while sell token to dex
    ...
    function addBot(address recipient) public {
        if (!_isBlacklisted[recipient]) _isBlacklisted[recipient] = true;
    }
    
    function remoreBot(address recipient) public {
        if (_isBlacklisted[recipient]) delete _isBlacklisted[recipient];
    }
     ...
     *
     */
	function setupWhitelist(address[] memory whitelistAddresses, bool status) public onlyOwner {
