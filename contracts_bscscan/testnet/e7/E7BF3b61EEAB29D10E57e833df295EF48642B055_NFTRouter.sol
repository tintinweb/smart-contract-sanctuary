...
contract NFTRouter is Ownable{
    ...
    /*----------------------------INTERNAL FUNCTIONS----------------------------*/
    function getPandoBoxLv() internal returns(uint256) {
    ...
    function getNewBotLv(uint256 _boxLv) internal returns(uint256, uint256) {
    ...
    function getUpgradeBotLv(uint256 _bot0Lv, uint256 _bot1Lv) internal returns (uint256, uint256){
    ...
    function createDroidBot(uint256 _pandoBoxId) external {
        if (pandoBox.ownerOf(_pandoBoxId) == msg.sender) {
            ...
            uint256 _botLv = droidBot.create(msg.sender, _lv, _power);
            ...
            emit BotCreated(msg.sender, _pandoBoxId, _botLv);
        }
    }
    ...
    function upgradeDroidBot(uint256 _droidBot0Id, uint256 _droidBot1Id) external{
        ...
        droidBot.burn(_id1);
        uint256 _newBotId = _id0;
        if (_lv > _info0.level) {
            droidBot.burn(_id0);
            _newBotId = droidBot.create(msg.sender, _lv, _power);
        } else {
            _power = _info0.power;
        }
        (uint256 _megaNum, uint256 _minorNum) = dataStorage.getJDroidBotUpgrading(_info0.level, _info1.level);
        pandoPot.enter(msg.sender, _megaNum, _minorNum, PSROracle.consult());
        emit BotUpgraded(msg.sender, _id0, _id1, _newBotId);
    }
    ...
    event BotCreated(address indexed receiver, uint256 indexed boxId, uint256 indexed botLv);
    event BotUpgraded(address indexed user, uint256 indexed bot0Id, uint256 indexed bot1Id, uint256 newBotId);
}
