...
contract PresaleContract is Ownable {
    ...
    uint256 VEST_CLIFF = 1 * VEST_MONTH;
    ...
    struct PublicRound {
      ...
      /* Giveaway */
    uint256 public GiveawayTotalCap;
    uint256 public GiveawayTransferred;
    uint256 public GiveawayClaimed;


    mapping (address => uint256) public GiveawayVestingAmount;
    ...
    function claimTokenPublic() public {
        if ((publicVestingAmount[msg.sender] > 0 || GiveawayVestingAmount[msg.sender] > 0) && block.timestamp > claimTimePublic) {
            /*tokenPCA.transfer(msg.sender, publicVestingAmount[msg.sender]);*/
            tokenPCA.transfer(msg.sender, publicVestingAmount[msg.sender].add(GiveawayVestingAmount[msg.sender]));
            GiveawayClaimed = GiveawayClaimed.add(GiveawayVestingAmount[msg.sender]);
            publicVestingAmount[msg.sender] = 0;
            GiveawayVestingAmount[msg.sender] = 0;
            
        }
    }

    function ownerSetupNewGiveaway(uint256 totalAmount) public onlyOwner{
        /*PublicRound storage round = publicRound[currentPublicRound];*/
        GiveawayTotalCap = GiveawayTotalCap.add(totalAmount);
        /*require(tokenPCA.balanceOf(address(this)) >= round.hardcap + privateSoldAmount + GiveawayTotalCap, "Check PCA token balance");*/
    }

    function giveawayPCATransfer(uint256 pcaAmount,address account) public onlyOwner {
        require(tokenPCA.balanceOf(address(this)) >= pcaAmount, "Check PCA token balance");
        GiveawayVestingAmount[account] = GiveawayVestingAmount[account].add(pcaAmount);
        GiveawayTransferred = GiveawayTransferred.add(pcaAmount);
        require(GiveawayTransferred  <= GiveawayTotalCap,"Exceed total amount for this giveaway");
    }
    ...
    function ownerWithdrawUnsoldPCA() public onlyOwner {
        ...
        tokenPCA.transfer(msg.sender, tokenPCA.balanceOf(address(this)) - totalPublicSold - privateSoldAmount - GiveawayTotalCap - GiveawayTransferred);
    }

    function ownerWithdrawUnsoldGiveawayPCA() public onlyOwner {
        require(GiveawayTotalCap - GiveawayTransferred > 0, "Total amount transferred");
        tokenPCA.transfer(msg.sender, GiveawayTotalCap - GiveawayTransferred);
        GiveawayTotalCap = GiveawayTotalCap - GiveawayTransferred;
    }
