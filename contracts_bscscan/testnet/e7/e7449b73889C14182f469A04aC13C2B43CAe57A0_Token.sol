...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "DOGEFATHER COIN";
    string private _symbol = "TEST2DGF";
    ...
    mapping (address => SendLimit) sended;
    ...
    function setWhaleProtection(bool _whaleProtection) external onlyOwner() {
        whaleProtection = _whaleProtection;
    }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner())
            ...
            if(whaleProtection) {
                if (block.timestamp >= sended[from].lastCheck + 86400) {
                    sended[from].sendA = 0;
                    sended[from].limit = _tOwned[from] / 2;
                    sended[from].lastCheck = block.timestamp;
                }
                require(sended[from].sendA <= sended[from].limit, "Anti-whale protection");
                sended[from].sendA.add(amount);
            }
