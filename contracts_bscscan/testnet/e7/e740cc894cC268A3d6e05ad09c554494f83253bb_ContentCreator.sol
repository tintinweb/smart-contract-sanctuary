...
contract Auth is Ownable {
    ...
    constructor( address _owner, address _creatorAuth) public Ownable( _owner) {
...
contract Content is ERC721, Auth {
    ...
    constructor ( address _owner, address _creator, string memory _name, string memory _symbol, string memory _url) public ERC721(_name, _symbol) Auth( _owner, _creator) {
        _setBaseURI(_url);
    }    
...
contract ContentCreator is Ownable {
    ...
    event CreatedContent( uint _contentID, address indexed _content, address indexed _creator);
    ...
    function createContent( string memory _name, string memory _symbol, string memory _url) public {
        ...
        bytes memory bytecode = type(Content).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_name, _symbol));
        address _content;
        
        assembly {
            _content := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        
        contents[_content].push(
            contentStruct(
                _content,
                msg.sender,
                _name,
        ...
        totalContents.push(_content);
        getCreatorByContent[_content] = msg.sender;
        emit CreatedContent( totalContents.length.sub(1),_content, msg.sender);
    }
    ...
    function _transferPrice(address _owner, address _token, uint _price, address _creator) internal {
        ...
        if(_token != address(0x00)){
            ...
            uint _tFee = _price.mul(taxFee).div(100);
