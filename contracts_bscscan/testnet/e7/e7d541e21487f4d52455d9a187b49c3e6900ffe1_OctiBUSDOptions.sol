...
contract OctiBUSDOptions is Ownable, IOctiOptions {
        ...
        returns (uint256 optionID)
    {
        ...
        require(token >= total, "Wrong value");
        busdToken1.transferFrom(msg.sender,address(this),token);
        if (token > total) busdToken1.transfer(msg.sender,token - total);
        ...
        options.push(option);
        busdToken1.transfer(settlementFeeRecipient,settlementFee);
        ...
        )
    {
        ...
        periodFee = (getPeriodFee(amount, period, strike, currentPrice, optionType).mul(currentPrice)).div(10**8);
        ...
        returns (uint256 fee)
    {
        ...
        return ((getpercent / 100).mul(currentPrice)).div(10**8);
    }
    ...
    function getStrikeFee(
        ...
        if (strike > currentPrice && optionType == OptionType.Put)
            return ((strike.sub(currentPrice).mul(amount).div(currentPrice)).mul(currentPrice)).div(10**8);
        if (strike < currentPrice && optionType == OptionType.Call)
            return ((currentPrice.sub(strike).mul(amount).div(currentPrice)).mul(currentPrice)).div(10**8);
        return 0;
    }
