...
contract DecubateStaking is Ownable,InterestHelper {
   ...
   * @param {isNFTMultiplierUsed} If the pool uses multiplier or not
  ...
  function set(
    ...
    poolInfo[_pid].nftMultiplier = _multiplier;
    poolInfo[_pid].lockPeriodInDays = _lockPeriodInDays;
    poolInfo[_pid].isNFTMultiplierUsed = _isUsed;
  ...
  function claim(uint256 _pid) public returns (bool) {
  ...
  /**
   *
   * @dev Reinvest accumulated DCB reward for a single pool
   *
   * @param {_pid} pool identifier
   *
  ...
  function reinvest(uint256 _pid) public returns (bool) {
  ...
  function canClaim(uint256 _pid, address _addr) public view returns (bool) {
  ...
  function _payout(uint256 _pid, address _addr)
    public
    view
    returns (uint256 value)
  {
    ...
    if(pool.isNFTMultiplierUsed && nftContract.balanceOf(_addr) > 0){
      multiplier = pool.nftMultiplier;
    }  

    if (from < to) {
      uint256 rayValue = yearlyRateToRay((pool.apy * multiplier * 10**18) / 1000);
      value = (accrueInterest(user.total_invested, rayValue, to.sub(from))).sub(user.total_invested);
    }
