...
contract BabyFlokiX is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public shibDividendRewardsFee;
    uint256 public previousShibDividendRewardsFee;

    uint256 public shibDividendPriority = 60;
    ...
    constructor() BEP20("Baby FlokiX", "$BFLOKIX") {
        shibDividendTracker = new ShibDividendTracker();

        marketingWallet = 0x0897b6f79Fb000Fa4BF6c7E8B972f09Fc95293f6;
        divident = 0x9934049f097330C426344a745db469f6920764Cb;
    ...
    function updateMinShibBeforeSendDividend(uint256 _newAmount) external onlyOwner {
        shibDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }

    function getMinShibBeforeSendDividend() external view returns (uint256) {
        return shibDividendTracker.minTokenBeforeSendDividend();
    }

    function setSendShibInTx(bool _newStatus) external onlyOwner {
        sendShibInTx = _newStatus;
    }

    function setShibDividendPriority(uint256 _newAmount) external onlyOwner {
        require(_newAmount >= 0 && _newAmount <= 100, "Error amount");
        shibDividendPriority = _newAmount;
    }
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(shibDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setShibDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = shibDividendRewardsFee.add(marketingFee).add(buyBackAndLiquidityFee);
            shibDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(shibDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateShibDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newShibDividendTracker.owner() == address(this), "must be owned by BabyFlokiX");
    ...
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = buyBackAndLiquidityFee.add(shibDividendRewardsFee).add(marketingFee);
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (shibDividendEnabled) {
                    uint256 sellTokens = contractTokenBalance.div(totalFees).mul(shibDividendRewardsFee);
                    swapAndSendShibDividends(sellTokens.sub(1300));
                }
        ...
        if(!swapping) {
            ...
            if(rand() <= shibDividendPriority) {

                if( shibDividendEnabled && sendShibInTx ){
    ...
    function swapAndSendShibDividends(uint256 tokens) private {
        ...
        uint256 shibDividends = IBEP20(shibDividendToken).balanceOf(address(this));
...
contract ShibDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("Shib_Dividend_Tracker", "Shib_Dividend_Tracker", 0x2859e4544C4bB03966803b044A93563Bd2D0DD4D) {
        claimWait = 3600;
