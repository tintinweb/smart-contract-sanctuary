...
library Constants {
    ...
    /* Oracle */
    address private constant BNB = address(0x7FF0a7Ff82Cb094C9B745392cd6Bcee0e0328130); // BNB Coin
...
contract Oracle is IOracle {
    ...
    constructor(address SGO) public {
        _dao = msg.sender;
        _SGO = SGO;
    }

    function setup() public onlyDao {
        _pair = IUniswapV2Pair(IUniswapV2Factory(PANCAKE_FACTORY).createPair(_SGO, BNB()));

        (address token0, address token1) = (_pair.token0(), _pair.token1());
        _index = _SGO == token0 ? 0 : 1;

        Require.that(_index == 0 || _SGO == token1, FILE, "SGO not found");
    }
    ...
    function updateOracle() private returns (Decimal.D256 memory, bool) {
        ...
        // bool isBlacklisted = IBNB(BNB()).isBlacklisted(address(_pair)); // not used for BEP-20
