...
contract HeroAirport is Ownable {
    ...
    constructor(address _heroAddress,string memory _name){
        heroAddress = _heroAddress;
        name = _name;
    }

    function airport(Data [] memory datas) public onlyController {
        uint256 length = datas.length;
        for(uint256 i=0;i<length;i++){
            index +=1;
            string memory _name = string(abi.encodePacked(name, "#", uint2str(index)));
            INFT(heroAddress).mint(datas[i].to,_name,1,datas[i].rarity,datas[i].series,datas[i].race,datas[i].mineralLevel);
        }
    }
