...
contract SPKTokenReward is PaybackReward, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using Iterablemap for Iterablemap.Map;

    Iterablemap.Map private tokenHoldersMap;
    ...
    constructor() PaybackReward("SPKToken_Reward_Tracker", "SPKToken_Reward_Tracker") {
        claimWait = 7200;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "SPKToken_Reward_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "SPKToken_Reward_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SPKToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external masterWallet {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "SPKToken_Reward_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SPKToken_Reward_Tracker: Cannot update claimWait to same value");
...
contract SPKToken is ERC20, Ownable {
 event UpdateDEXV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    SPKTokenReward public Reward;
   uint256 _totalSupply =500000000  * (10**9);
    address public walletDevTeam =0xca9023B5A7f3b18B57e4A9C15Ab2C2a59BB2C9c3;
...
uint256 public  liquidityFee = 2;
  ...
  ILP public adressLiquidity;
    ...
    constructor() ERC20("Spacex King", "SPK") {

        Reward = new SPKTokenReward();
        TotalFees = rewardFee.add(liquidityFee).add(DevPercent);
...
lp = _DEXV2Pair;
    ...
    function setDevWallet(address payable wallet) public masterWallet {
        walletDevTeam = wallet;
    }

    function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "SPKToken: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "SPKToken: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "SPKToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SPKToken: Cannot update gasForProcessing to same value");
    ...
    function getTotalPaybackDistributed() external view returns (uint256) {
        return Reward.totalPaybackDistributed();
    }

    function withdrawableDividendOf(address account) public view returns(uint256) {
        return Reward.withdrawableDividendOf(account);
    }

    function dividendTokenBalanceOf(address account) public view returns (uint256) {
        return Reward.balanceOf(account);
    }
 function rebaseToken(uint256 epoch, int256 supplyDelta)
  ...
  returns (uint256)
  {
    ...
    function setMaxTxAMount(uint256 amount) external masterWallet{
        maxTxAmount = amount;
    }

    function changeWalletLimit(uint256 newLimit) external masterWallet {
        walletMax  = newLimit;
    }

    function enableDisableWalletMax(bool newValue) external masterWallet {
       restrictWhales = newValue;
    }

    function changeIsWalletLimit(address holder, bool Limited) external masterWallet {
        isWalletLimit[holder] = Limited;
    }

    function changeSwapThreshold(uint256 newSwapBackLimit) external masterWallet {
        swapTokensAtAmount = newSwapBackLimit;
    }

  
    function changeCooldownSettings(bool newStatus, uint8 newInterval) external masterWallet {
        require(newInterval <= 10 minutes, "Exceeds the limit");
        cooldownEnabled = newStatus;
        cooldownTimerInterval = newInterval;
    }

    function setIsTimelockLimited(address holder, bool Limited) external masterWallet {
        isTimelockLimited[holder] = Limited;
    }
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 tokensTSPKToken = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTSPKToken);
        ...
        addLiquidity(tokensTSPKToken, bnbForLiquidity);

        payable(walletDevTeam).transfer(bnbForDev);
