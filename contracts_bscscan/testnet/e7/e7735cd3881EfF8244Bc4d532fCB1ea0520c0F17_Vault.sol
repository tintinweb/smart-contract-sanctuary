...
contract Vault is IERC20, ReentrancyGuard, Ownable {
	...
	event SwapInProcessed(address sender, uint256 amount, uint256 fee);
	event SwapOutProcessed(address sender, uint256 amount);
	event RefundProcessed(address sender, uint256 amount, uint256 fee);
	...
	/// @param amount the amount user wants to receive after swap
	...
	function swapIn(
		...
		require(amount <= _maxAmountForSwap(), "Exceed maximum swap amount");
		IERC20Metadata(token).transferFrom(msg.sender, address(this), amount + fee);
	...
	function refund(
		...
		IERC20Metadata(token).transfer(account, amount + fee);
		uint256 redistribution = (fee * lpFee) / BASIS_POINT;
