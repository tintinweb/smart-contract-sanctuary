...
contract Babylion is Context, IERC20, Ownable {
    ...
    string private _name = "Baby Lion";
    string private _symbol = "BLN";
    ...
    function setMarketingDevFeePercent(uint256 marketingBudget) external onlyOwner {
        _marketingBudget = 0;
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateLiquidityPlusDevFee(tAmount);
    ...
    function calculateLiquidityPlusDevFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _devFee + _marketingBudget).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingBudget == 0 && _devFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalLiqFee = _marketingBudget + _liquidityFee + _devFee;
        ...
        // calculate the portions of the liquidity to add to dev
        ...
        if(sent){
            _tokenTransfer(address(this), 0x0000000000000000000000000000000000000001, devPortion, false);
            emit Transfer(address(this), 0x0000000000000000000000000000000000000001, devPortion);
        } else {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount, address dev) private {
