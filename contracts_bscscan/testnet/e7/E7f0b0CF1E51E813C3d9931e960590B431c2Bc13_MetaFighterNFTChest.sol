...
contract MetaFighterNFTChest is IChest, Ownable, ReentrancyGuard {
    ...
    uint32 public constant COMMON_CHEST = 0;
    uint32 public constant DELUXE_CHEST = 1;
    uint32 public constant PREMIUM_CHEST = 2;
    ...
    function setChestPrice(uint256 chestPrice_) external override onlyOwner {
        require(chestPrice_ >= 0, "MetaFighterNFTChest: Chest price must be greater than or equal to 0");
        chestPrice = chestPrice_;
    }

    function buyChest(uint32 chestType) external override nonReentrant {
        // uint256 timestamp = block.timestamp;
        require(chestType < 3, "MetaFighterNFTChest: Wrong chest type");
        require(_totalClaimedChestCounter.current() < totalSupply, "MetaFighterNFTChest: Chest currently out of stock");
        ...
        IMetaFighterNFT(mef721).mintGenesis(4, msg.sender, chestPrice);

        string memory chestName;
        if (chestType == 0) chestName = 'Common Chest';
        if (chestType == 1) chestName = 'Deluxe Chest';
        if (chestType == 2) chestName = 'Premium Chest';
...
interface IChest {
    ...
    function buyChest(uint32 chestType) external;
