...
/*    4% fee
 	...
 	100% of total supply is divided into 5 Wallets as defined below:
 
	Developer Wallet: 15,000,000 GART Tokens
	charity Wallet: 15,000,000 GART Tokens
	marketing: 15,000,000 GART Tokens
	Presale: 15,000,000 GART Tokens
	Owner: 40,000,000 GART Tokens
	...
	Every dev transaction will be documented and tracked via BSCScan
*/
...
contract GART is Context, IERC20, Ownable {
    ...
    address public _rfiWallet;
    address public _gtWallet;
 
    uint8 private _decimals = 18;
 
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 100000000 * 10**18; // 100 billion
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
 
    string private _name = "GART";
    string private _symbol = "GART";
    ...
    uint256 private minTokensBeforeSwap = 0 * 10**_decimals; // 30m
        ...
        if (nukeTheWhales) {
            ...
            uint256 threshold = (totalSupply()) * (5) / (10**3);
                ...
                if (fromBalance > threshold) {
                    ...
                    require(amount < fromBalance / (5), "Max sell is 20% if you hold 0.5% or more of supply.");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the BNB -> STEAMX swap when swap+liquify is triggered
 
        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);
 
        payable(_gtWallet).transfer(newBalance);
        ...
        _rOwned[_rfiWallet] = _rOwned[_rfiWallet].add(rTransferAmount);
        emit Transfer(address(this), _rfiWallet, tTransferAmount);
        restoreAllFee();
  ...
  // Created with the help of 0f0crypto
  ...
  // Check out 0f0crypto's solidity support channel for all your troubleshooting needs https://discord.gg/k3rewGKYmD
   ...
   function calculateLiquidityFee(uint256 amount, address to) internal view returns (uint256) {
        if (feesEnabled == true) {
            if ( to == address(uniswapV2Pair) )  {
                ...
                return amount * tax / 400;
            }
            return amount * _liquidityFee / 200;
        }
        return 0;
    }
   ...
   function calculateRfiFee(uint256 amount) internal view returns (uint256){ 
    ...
    function excludeFromBoth(address account)external onlyOwner {
    ...
    function setDeveloperWallet(address _address, address account) public onlyOwner() {
        _DeveloperWallet = _address;
        excludeFromFee(account);
        excludeFromReward;
    }
 
    function setcharityWallet(address _address, address account) public onlyOwner() {
    ...
    function setrfiWallet(address _address, address account) public onlyOwner() {
     ...
     function setgtWallet(address _address, address account) public onlyOwner() {
        _gtWallet = _address;
        excludeFromFee(account);
    }
