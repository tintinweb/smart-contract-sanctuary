...
contract Farming is Ownable, ReentrancyGuard {
    ...
    function autoDeposit(uint256 _pid, uint256 _amount, address _from) public poolExists(_pid) {
        ...
        poolInfo[_pid].lpToken.approve(ROUTER_ADDRESS, _amount/2);
        IPancakeRouter02(ROUTER_ADDRESS).swapExactTokensForETHSupportingFeeOnTransferTokens(_amount/2, 0, path, address(this), 0);
        IPancakeRouter02(ROUTER_ADDRESS).addLiquidityETH(address(rewardToken), _amount/2, 0, 0, address(this), 0);
        _autoDeposit(_pid, _amount/2, _from);
    }
