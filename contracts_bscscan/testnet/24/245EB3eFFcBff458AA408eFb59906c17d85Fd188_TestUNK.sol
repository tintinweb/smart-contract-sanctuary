...
contract TestUNK {
    ...
    function purchase(uint8 _matrix, uint8 _slotId) external {
        ...
        _activateSlots(msg.sender, _slotId, _matrix, false);
        emit Purchase(msg.sender, _slotId, _matrix);
    }
    
    function purchaseMatrix(uint8 _pool) external {
        require(isUserExists(msg.sender), "User not registered!");
        require(_pool >= 1 && _pool <= 40, "Invalid Level");
    ...
    function _createAccount(address _addr, address _referrer) internal {
        ...
        _activateSlots(_addr, 1, 1, true);
        _activateMatrix(1);
        emit Registration(_addr, _referrer, lastId);
    }
    
   function _activateSlots(address _addr, uint8 _slotId, uint8 _matrix, bool _isRegistration) internal {
        uint256 _flushs;
        uint256 _fees;
        if (_isRegistration == true || _matrix == 1) {
            ...
            tokenAddress.safeTransfer(address(owner), (_fees + _flushs));
        }
    ...
    function _activateMatrix(uint8 _pool) internal {
        matrixIds[_pool]++;
        ...
        emit EarnedProfit(_referrer, msg.sender, 3, 0, _pool, msg.value);
    ...
    function _sendLevelCommission(address _addr, uint8 _slotId) internal returns (uint256) {
        ...
        if (_referrer != address(0)) {
            (uint _earning, uint _maxLimit,,,) = _getLevelDistribution(_referrer, _slotId, 2, 1);
            uint256 commission = (slots[_slotId].price * 50) / 100;
    ...
    function _getLevelDistribution(address _referrer, uint8 _slotId, uint8 _matrix, uint8 _level) internal view returns(uint, uint, uint, uint, uint) {
        uint8 currentSlot;
        uint256 bonus;
        uint256 maxLimit;
        uint256 earning;
        if (_matrix == 1) {
            ...
            currentSlot = users[_referrer].workingPool[_referrer].currentSlot;
            ...
            currentSlot = users[_referrer].autoPool[_referrer].currentSlot;
            for (uint8 i = 1; i <= currentSlot; i++) {
                maxLimit += slots[i].limit;
            }
        }
