...
contract CCL is Context, Ownable, ReentrancyGuard {
 ...
 //  uint256 private BNBCost; // Fees in BNB
    ...
    event AddressCondition(address indexed addressQuery, bool condition);
    ...
    event WalletVerificationRequers(address indexed requestedAddress);      // Announce Wallet newRequestes
    ...
    function getAddressState(address _addr, bool _condition) external LTSHOLDER nonReentrant {
        ...
        if(_condition) { 
                // IF TRUE [CHECK ONGOING REQUEST]
            bool ongoingRequest = _isRequestedAddress(_addr);
            emit AddressCondition(_addr, ongoingRequest);
        } else {
                // IF FALSE [CHECK SUSPECTION]
            bool suspectedAdd = _isSusAddress(_addr);
            emit AddressCondition(_addr, suspectedAdd);
        }         
    }
    ...
    function ltsHOLDINGS(address _addr) private view returns (bool) { 
        ...
        // Array checkers
    function verifiedWalletsTable() external view returns (address[] memory) { 
