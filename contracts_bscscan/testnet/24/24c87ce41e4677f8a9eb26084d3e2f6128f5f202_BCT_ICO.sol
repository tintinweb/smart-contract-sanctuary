...
contract BCT_ICO is ReentrancyGuard, Context, Ownable {
  ...
  /*  require(_rate > 0, "Pre-Sale: rate is 0");
    ...
    rate = 1000000000000000; // _rate;
    refPrecent = 3000000000000000000; //_refPrecent;
    payWallet = payable(address(0x70d1e22A154a0101823f2D4b0F97e1F13D2B7513)); // _wallet;
    usdtWallet = payable(address(0x70d1e22A154a0101823f2D4b0F97e1F13D2B7513));  //_usdtwallet;
    token = IERC20(0x793ba712f810Bc41C0Eeb6fA2DeDf0bbB73E6E9D); //_token
	  usdt  = IERC20(0x337610d27c682E347C9cD60BD4b3b107C9d34dDd);

    //startICO delete on dev
    endICO = 1639467960;
    minPurchase = 10000000000000000;
    maxPurchase = 15000000000000000000000;
    softCap = 10000000000000000000000;
  ...
  function addWhitelist(address wallet) external onlyOwner {
   ...
   function _buyTokens(address beneficiary) internal {
    ...
    require(tokenAmt > tokens, "Not enough BEP20 tokens on contract");
    weiRaised = weiRaised.add(weiAmount);
    availableTokensICO = availableTokensICO - tokens;
    //contributions[beneficiary] = contributions[beneficiary].add(weiAmount);
    //emit TokensPurchased(_msgSender(), beneficiary, weiAmount, tokens);
    //ERC20(usdt).safeTransferFrom(beneficiary, address(this), weiAmount);
   /* uint256 allowance = usdt.allowance(beneficiary, address(this));
    if (allowance == 0) {
      usdt.approve(address(this),weiAmount);
      allowance = usdt.allowance(beneficiary, address(this));
    }
    require(allowance >= weiAmount, "Check the token allowance"); */
    //usdt.transferFrom(beneficiary, address(this), weiAmount); 
   // usdt.transfer(address(this), weiAmount);
    token.transfer(beneficiary, tokens);
    
   // emit TokensPurchased(payWallet, beneficiary, weiAmount, tokens);
   ...
   // address payable feeAddress = address(uint160(usdtWallet)); //cast the address to payable
   // address payable feeAddress = address(uint160(usdtWallet)); //cast the address to payable
   // ERC20(usdt).safeTransferFrom(beneficiary, usdtWallet, weiAmount);
  }

  function getCurSender() public view returns (address) {
    return _msgSender();
  }
  ...
  function getBalanceUSDT() public view returns (uint256) {
    uint256 bl = usdt.balanceOf(msg.sender); //balanceOf(address(this));
    return bl;
  }


  function getAllowanceApprove() public payable returns (uint256) {
    uint256 weiAmount = msg.value;
    bool val = usdt.approve(address(this), weiAmount);
	require(val, "bad approve");
    uint256 allowance = usdt.allowance(msg.sender, address(this));
    return allowance;
  }
  

  function getTokenNeed(uint val) public view returns (uint256) {
