...
contract SHIBAQUEEN is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    address payable public vaultWalletAddress = payable(0xFdEB18B3b65a5ef249a2479917e3cbD69603f401); // Marketing Address
    address payable public treasuryWalletAddress = payable(0xF17d535EE94C5E472b277F71DDB9C1ae6F8eAEe8); // MemeNFT Address
    ...
    constructor () {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); //Prod
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _vaultFee.add(_treasuryFee) > 0)
        {
            ...
            uint256 MemeNFTWalletShare = newBalance.sub(marketingShare);
            
            if(marketingShare > 0)
                transferToAddressETH(vaultWalletAddress, marketingShare);
            
            if(MemeNFTWalletShare > 0)
                transferToAddressETH(treasuryWalletAddress, MemeNFTWalletShare);
        }
    ...
    function setTaxes(uint256 newBurnFee, uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTreasuryTax) external onlyOwner() {
        _burnFee = newBurnFee;
    ...
    function setvaultWalletAddress(address newAddress) external onlyOwner() {
        vaultWalletAddress = payable(newAddress);
    }
