...
contract BSCPlayGroundPresale is Ownable, OwnerPausable
{
    ...
    uint256 public maxContribution = 15 * 10**17;  // 1.5 bnb
    ...
    mapping(address => bool) private _usersClaimed;
    mapping(address => uint256) private _usersDeposite;
    ...
    function depositedFrom(address owner) external view returns(uint256){
        return _usersDeposite[owner];
    }

    function claimedFrom(address owner) external view returns(bool){
        return _usersClaimed[owner];
    }
    ...
    function getAmountTokensByContributionValue(address sender) private view returns(uint256){        
        uint256 deposite = _usersDeposite[sender];
        return tokensPerBNB.mul(deposite);
    }
    ...
    function whitelistedAddress() public view returns(bool isWhitelisted){
    ...
    function deposit() external payable returns(bool success){
        ...
        if(whiteListEnabled) 
            require(whitelistedAddress(), 'Whitelist hasn`t this address. ');

        address sender = msg.sender;
        uint256 value = msg.value;
        uint256 amount = _usersDeposite[sender].add(value);
    ...
    function reclaimTokens() external returns(bool success){
        ...
        require(_usersDeposite[msg.sender] > 0 ,'This address did not feel in the presale.');
        require(!_usersClaimed[msg.sender], 'This address already claimed tokens.');

        address sender = msg.sender;
        uint256 tokens = getAmountTokensByContributionValue(sender);

        uint256 allTokensAmount = token.balanceOf(address(this));
        if( allTokensAmount < tokens)
            tokens = allTokensAmount;

        _usersClaimed[sender] = true;
    ...
    function reclaimNotPurchasedTokens() external onlyOwner{
        ...
        uint256 allTokensAmount = token.balanceOf(address(this));
            if( allTokensAmount < leftTokens)
                leftTokens = allTokensAmount;
