...
contract MasterStake is Ownable {
    ...
    struct coinToStake{
        address _address;
        uint256 _decimals;
        uint256 _stakeLimitWithDecimal;
    }
    ...
    mapping (uint256 => mapping (address => bool)) public userFinalClaim;
...
// contract fn
    function getRewardPaidView(uint256 _poolID,address _sender) public view isPoolExist(_poolID) returns(uint256) {
        ...
        if(userFinalClaim[_poolID][_sender]){
            return 0;
        }
        ...
        if((cr._poolAmount - currentPool._rewardClaimed) <= 0){
            return 0;
        }
...
// user functions
    function stake(uint256 _poolID, uint256 _stakeAmountWithDecimal) external isPoolExist(_poolID) isPoolEndTime(_poolID){
        ...
        StakePool storage currentPool = stakePoolFromPoolID[_poolID];
        coinToStake storage cs = currentPool._coinToStake;
        require(_stakeAmountWithDecimal <= cs._stakeLimitWithDecimal || cs._stakeLimitWithDecimal == 0, 'You are over the stake limit!');
        uint256 newStakeBal;
        uint256 stakeFee = currentPool._stakeFee; 
        uint256 feeAmount = (_stakeAmountWithDecimal * stakeFee / 100);
        
        if(isUserStaked[_poolID][sender]){
            UserStake storage currentUserStake = userStakeFromPoolID[_poolID][sender];
            require(_stakeAmountWithDecimal + currentUserStake._amountStaked <= cs._stakeLimitWithDecimal || cs._stakeLimitWithDecimal == 0, 'You are over the stake limit!');
    ...
    function harvest(uint256 _poolID) external isPoolExist(_poolID) {
        address sender = _msgSender();
        require(isUserStaked[_poolID][sender], 'You have no stake in this pool!');
        require(!userFinalClaim[_poolID][sender], 'No more claims left for this pool!');
        ...
        if(!isActiveFromPoolID[_poolID] || endDateFromPoolID[_poolID] < block.timestamp){
            // ...
            userFinalClaim[_poolID][sender] = true;
        }else{
            currentUserStake._startTime = block.timestamp;
        }
...
//admin actions
    ...
    function endStakePool(uint256 _poolID) external onlyAdmin isPoolExist(_poolID) isPoolActive(_poolID){
        ...
        if(cs._address == cr._address){
            withdrawBal = int256(availableBal - currentPool._amountStaked - (rewardPaid - currentPool._rewardClaimed));
        }else{
            withdrawBal = int256(availableBal - (rewardPaid - currentPool._rewardClaimed));
        }
        if(withdrawBal <= 0){
            withdrawBal = 0;
        }
        ...
        IERC20(cr._address).transfer(sender,uint256(withdrawBal));
