...
contract Crowdsale is Ownable {
        ...
        returns (bool) 
    {
        ...
        require(
            IBEP20(_tokenContract).allowance(_msgSender(), address(this)) >= quantity_, 
            "CRW: allowance of contract is not enough"
        );
        require(
            IBEP20(_tokenContract).transferFrom(_msgSender(), address(this), quantity_), 
            "CRW: error during transfer from"
        );
    ...
    function buy(
        ...
        returns (bool)
    {
        Sale memory c = _sales[_salesIndex];
        require(msg.value == amount_ * c.price, "Price doesn't match quantity");
        require(block.timestamp > c.startDate, "Sale didn't start yet.");
        require(block.timestamp < c.endDate, "Sale is already closed.");
        require(amount_ < c.quantity, "Amount over the limit");
        ...
        returns (bool)
    {
        ...
        require(_sales[_salesIndex].endDate > block.timestamp, "CRW: sale is not open");

        _sales[_salesIndex].endDate = block.timestamp;
        if ( _sales[_salesIndex].quantity > 0 ) {
            require(IBEP20(_tokenContract).transfer(_msgSender(), _sales[_salesIndex].quantity));
            _sales[_salesIndex].quantity = 0;
        }
        ...
        returns (bool)
    {
        ...
        IBEP20(_tokenContract).transfer(to, c.quantity);
        return true;
    }
    ...
    function getInfoLastSale() public view returns(uint256, uint256, uint256, uint256){
        uint256 startDate = _sales[_salesIndex].startDate;
        uint256 endDate = _sales[_salesIndex].endDate;
        uint256 quantity = _sales[_salesIndex].quantity;
        uint256 price = _sales[_salesIndex].price;

        return (startDate,endDate,quantity,price);
    }
}
