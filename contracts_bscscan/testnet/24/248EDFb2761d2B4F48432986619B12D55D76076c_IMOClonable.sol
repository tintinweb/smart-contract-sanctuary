...
contract IMOClonable is IIMOClonable, Initializable, OwnableUpgradeable{
    ...
    function claim() public onClaimEvent{
        require(userCommit[msg.sender].commitAmount > 0, "You haven't commit any funding or you have claimed.");
        uint256 _offerAmount = getOfferAmount(msg.sender);
        uint256 _refundAmount = getRefundAmount(msg.sender);
        offerToken.safeTransfer(msg.sender, _offerAmount);
        if(_refundAmount > 0){
            fundToken.safeTransfer(msg.sender, _refundAmount);
        }
        userCommit[msg.sender].commitAmount = 0;
        emit Claim(msg.sender, offerToken, _offerAmount, fundToken, _refundAmount, userCommit[msg.sender].tier);
    }
