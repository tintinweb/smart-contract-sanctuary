...
contract ULDRASAFEExchangeContract is Sale{
    ...
    function transferOwnershipForColle(address newOwner, address token721, address token1155) public onlyOwner {
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        ULS721 tok= ULS721(token721);
        ULS1155 tok1155= ULS1155(token1155);
        tok._transferOwnership(newOwner);
        tok1155._transferOwnership(newOwner);
    }
    //mint function
    function mint(address token ,string memory tokenuri, uint256 value, uint256 tokenId, uint256 royal, uint256 _type, uint256 supply) public onlyOwner{
       require(_creator[tokenId] == address(0), "Token Already Minted");
       if(_type == 721){
           ULS721 tok= ULS721(token);
    ...
    function tokenTransfer(address to,uint256 tokenId, address token, string memory tokenAss, uint256 fee, uint256 _type, uint256 NOFToken) public{
    ...
    function swap(address from, uint256 fromTokenId, uint256 tokenId, uint256 fee, string memory tokenAss, address token, uint256 _type, uint256 NOFToken) public{
