...
// Lic: MIT
library Logger {
    event logTransferTry(address indexed from, address indexed to, uint256 amount);
}
...
*/
contract TokenV5 is Context, IBEP20, Ownable {
    ...
    string private constant _name = 'TokenV5';
    string private constant _symbol = 'TOKV5';
    ...
    constructor() {
        ...
        pancakeV2Router = IPancakeV2Router(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        ...
        // anti bot/snipe launch
        _isWhitelisted[owner()] = true;
        _isWhitelisted[address(this)] = true;
        _isWhitelisted[pancakeV2Pair] = true;
        _isWhitelisted[deadAddress] = true;
    ...
    function _transfer(
        ...
        emit Logger.logTransferTry(from, to, amount);
        ...
        if (!_isWhitelisted[from]) {
            require(_lastTx[from] + _txDelay <= block.timestamp, 'Please wait delay next tx');
            _lastTx[from] = block.timestamp;
        }

        if (!_isWhitelisted[to]) {
            require(_lastTx[to] + _txDelay <= block.timestamp, 'Please wait delay next tx');
            _lastTx[to] = block.timestamp;
        }

        if (antiBot && launchedAt + 10 > block.number) {
            revert('Too soon boi');
        }
    ...
    function defuseAntibot() external onlyOwner {
        antiBot = false;
    }
