...
contract Lottery is Ownable, Initializable {
        ...
        uint256 _closingTimestamp) external onlyOwner()
    {
        ...
        LotteryInfo memory newLottery = LotteryInfo(lotteryId, lotteryStatus, _costPerTicket, _prizeInNFT,
             _nftIDs, _nftQuantity, _prizeDistribution, LotteryStart, _closingTimestamp, _minTicketsToBuy);
    ...
    function drawWinners(uint256 _lotteryId, uint256 seed, uint256 seedDifference) external onlyOwner()  {
        ...
        if (allLotteries[_lotteryId].prizeInNFT) {
            // give rewards in NFTs
            for (uint i=0;i<3;i++) {
            ...
            // give rewards in tokens
            for (uint i=0;i<3;i++) {
                address user = winnersList[_lotteryId][i];
                // _x22 transfer
                uint256 amount = allLotteries[_lotteryId].prizeDistribution[i];
                amount = amount * prizePool[_lotteryId] / 100;
