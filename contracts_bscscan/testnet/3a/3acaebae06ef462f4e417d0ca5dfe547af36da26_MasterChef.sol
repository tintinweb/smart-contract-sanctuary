...
contract MasterChef is Ownable, ReentrancyGuard {
        ...
        if (_amount > 0) {
            ...
            if (pool.withdrawFeeBP > 0) {
                uint256 withdrawFee = _amount.mul(pool.withdrawFeeBP).div(10000);
                 total = _amount.sub(withdrawFee);
                pool.lpToken.safeTransfer(feeAddress, withdrawFee);
            }
            else{
                 total = _amount;
            }
            pool.lpToken.safeTransfer(address(msg.sender), total);
        }
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        uint256 total = user.amount;
        if (pool.withdrawFeeBP > 0) {
            uint256 withdrawFee = total.mul(pool.withdrawFeeBP).div(10000);
             total = total.sub(withdrawFee);
            pool.lpToken.safeTransfer(feeAddress, withdrawFee);
        }
       
        pool.lpToken.safeTransfer(address(msg.sender), total);
       
        emit EmergencyWithdraw(msg.sender, _pid, total);
        user.amount = 0;
        user.rewardDebt = 0;
    }
