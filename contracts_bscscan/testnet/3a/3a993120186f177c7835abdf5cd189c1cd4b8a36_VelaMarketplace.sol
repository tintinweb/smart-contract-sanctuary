...
contract VelaMarketplace is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => mapping(uint256 => uint256)) public historyLength; // erc721 address => token id => length
    mapping(address => mapping(uint256 => mapping(uint256 => History))) public histories; // erc721 address => token id => history id -> History
    ...
    function listingPrice(address erc721, address erc20, uint256 tokenId, uint256 price) public whenNotPaused nonReentrant inWhitelist(erc721, erc20){
        ...
        if (info.seller == address(0)) {
            ...
            require(info.auction == false, "Marketplace: can not change the auction item");
        ...
        _history("listingPrice", erc721, tokenId, msgSender, address(0), erc20, price, false, 0,0, block.timestamp);
    }

    function buy(address erc721, uint256 tokenId) public payable whenNotPaused nonReentrant {
        address msgSender = _msgSender();
        NftInfo memory info = nfts[erc721][tokenId];
        require(info.auction == false, "Marketplace: can not buy the auction item");
        require(info.price > 0, "Marketplace: token is not for sale");
        require(info.seller != msgSender, "Marketplace: can not buy");
        require(erc721Whitelist[erc721], "Marketplace: erc721 must be in whitelist");
        if (info.erc20 == address(0)) {
            require(msg.value == info.price, "Marketplace: deposit amount is not enough");
        } else {
            require(erc20Whitelist[info.erc20], "Marketplace: erc20 must be in whitelist");
        ...
        _history("buy", erc721, tokenId, info.seller, msgSender, info.erc20, info.price, false, 0,0, block.timestamp);
    }
    ...
    function cancelListing(address erc721, uint256 tokenId) public whenNotPaused nonReentrant{
        ...
        require(info.auction == false, "Marketplace: can not to cancel listing the auction item");
        require(info.seller == msgSender, "Marketplace: can not to cancel listing if sender has not made one");
        IERC721(erc721).transferFrom(address(this), msgSender, tokenId);
        emit ListingCanceled(erc721, info.erc20, msgSender, info.price, tokenId);
        delete nfts[erc721][tokenId];
        _history("cancelListing", erc721, tokenId, msgSender, address(0), info.erc20, info.price, false, 0,0, block.timestamp);
    }
    ...
    function auctionListingPrice(address erc721, address erc20, uint256 tokenId, uint256 price, uint256 beginAuction, uint256 endAuction) public whenNotPaused nonReentrant inWhitelist(erc721, erc20){
        address msgSender = _msgSender();
        require(price > 0, "Marketplace: price must be greater than 0");
        require(beginAuction > 0, "Marketplace: beginAuction must be greater than 0");
        require(endAuction > 0, "Marketplace: endAuction must be greater than 0");
        require(endAuction > beginAuction, "Marketplace: endAuction must be greater than beginAuction");
        NftInfo memory info = nfts[erc721][tokenId];
        if (info.seller == address(0)) {
            IERC721(erc721).transferFrom(msgSender, address(this), tokenId);
            emit AuctionListed(erc721, erc20, msgSender, price, tokenId);    
        } else {
            require(info.auction == true, "Marketplace: can not change the not auction item");
            require(info.seller == msgSender, "Marketplace: can not change sale if sender has not made one");
    ...
    // request: add bid history
    function bid(address erc721, address erc20, uint256 tokenId, uint256 price) public payable whenNotPaused nonReentrant inWhitelist(erc721, erc20) {
        require(price > 0, "Marketplace: price must be greater than 0");
        address msgSender = _msgSender();
        address nftOwner = IERC721(erc721).ownerOf(tokenId);
        NftInfo memory info = nfts[erc721][tokenId];
        Bid memory oldBid = bids[erc721][tokenId];
        require(info.price <= price, "Marketplace: do not allow new bid price lower than open bid price");
        require(oldBid.price < price, "Marketplace: don not allow new bid price lower than current bid price");
        require(info.auction == true, "Marketplace: can not bid the not auction item");
        ...
        emit BidCreated(erc721, erc20, msgSender, price, tokenId);
        if (oldBid.bidder != address(0)) {
            _cancelBid(erc721, tokenId);
        }

        bids[erc721][tokenId] = Bid(erc20, msgSender, price);
    }
    ...
    function acceptBid(address erc721, uint256 tokenId) public whenNotPaused nonReentrant {
        ...
        if (nftInfo.erc20 != address(0)) {
            require(erc20Whitelist[nftInfo.erc20], "Marketplace: erc20 must be in whitelist");
        }
        require(bidInfo.bidder != address(0), "Marketplace: can not accept a bid when there is none");
        address nftOwner = IERC721(erc721).ownerOf(tokenId);
        
        require(nftInfo.auction == true, "Marketplace: can not accept bid the not auction item");
        require(bidInfo.bidder != address(0), "Marketplace: there is not bid");
        require(nftInfo.seller == msgSender || nftOwner == msgSender, "Marketplace: sender is not token owner");
    ...
    function cancelAuctionListing(address erc721, uint256 tokenId) public whenNotPaused nonReentrant{
        ...
        require(info.auction == true, "Marketplace: can not to cancel listing the not auction item");
        require(info.seller == msgSender, "Marketplace: can not to cancel listing if sender has not made one");
        IERC721(erc721).transferFrom(address(this), msgSender, tokenId);
    ...
    function _history(string memory action,  address erc721, uint256 tokenId, address seller, address buyer, address erc20, uint256 price, bool auction, uint256 beginAuction, uint256 endAuction, uint256 timestamp) internal{
        uint256 length = historyLength[erc721][tokenId];
        if (length == 0){
            length = 1;
        }else{
            length++;
        }
        historyLength[erc721][tokenId] = length;
        histories[erc721][tokenId][length] = History(action, erc721, tokenId, seller, buyer, erc20, price, auction, beginAuction, endAuction, timestamp);
    }
