...
interface IPokemonStats {
    ...
    /**
     * @dev Returns Pokemon stats from the token ID
     */
    ...
    /**
     * @dev Returns Pokemon card details from the token ID
     */
    function details(uint256 tokenId)
        external
        view
        returns (
            PokeInfo[] memory info,
            PokePurity memory purity,
            PokeBody memory body
        );

    /**
     * @dev Returns expected price (in PokeCash) from the token ID
     */
    function expectedPrice(uint256 tokenId) external view returns (uint256);

    function setInfo(uint256 tokenId, PokeInfo[] memory newInfo) external;

    function setPurity(uint256 tokenId, PokePurity memory newPurity) external;

    function setBody(uint256 tokenId, PokeBody memory newBody) external;

    function setDetails(
        uint256 tokenId,
        PokeInfo[] memory newInfo,
        PokePurity memory newPurity,
        PokeBody memory newBody
    ) external;
}
...
contract PokemonNFT is BNFT, IPokemonNFT, ReentrancyGuard {
    using Address for address;
    ...
    address private _pokeStats;

    constructor() BNFT(_msgSender(), "https://", "P NFT", "Pokemon") {}

    function setPokeStats(address newPokeStats) external onlyAdmin {
        _pokeStats = newPokeStats;
    }
    ...
    {
        if (_pokeStats != address(0)) {
            (pvpStats, luckStat, productivityStat, otherStats) = IPokemonStats(_pokeStats).stats(tokenId);
        }
    }
    ...
    function lock(uint256 tokenId, address locker) external override nonReentrant {
        require(_lockedBy[tokenId] == address(0), "PokemonNFT: already locked");

        //solhint-disable-next-line max-line-length
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");

        _lockedBy[tokenId] = locker;
        require(_checkOnERC721Locked(ownerOf(tokenId), locker, tokenId, ""), "PokemonNFT: lock failed");
    }
    ...
    function unlock(uint256 tokenId) external override {
        ...
        //solhint-disable-next-line max-line-length
        require(_locker == _msgSender() || admin() == _msgSender(), "PokemonNFT: caller is not locker nor admin");
        require(_checkOnERC721Unlocked(ownerOf(tokenId), _locker, tokenId, ""), "PokemonNFT: unlock failed");
    ...
    function _checkOnERC721Locked(
        ...
        if (locker.isContract()) {
            try IERC721Locker(locker).onERC721Locked(_msgSender(), from, tokenId, _data) returns (bytes4 retval) {
                ...
                if (reason.length == 0) {
                    revert("PokemonNFT: lock by non ERC721Locker implementer");
                } else {
                    // solhint-disable-next-line no-inline-assembly
                    assembly {
                        revert(add(32, reason), mload(reason))
    ...
    function _checkOnERC721Unlocked(
        ...
        if (locker.isContract()) {
            try IERC721Locker(locker).onERC721Unlocked(_msgSender(), from, tokenId, _data) returns (bytes4 retval) {
                ...
                if (reason.length == 0) {
                    revert("PokemonNFT: unlock from a non ERC721Locker implementer");
                } else {
                    // solhint-disable-next-line no-inline-assembly
                    assembly {
                        revert(add(32, reason), mload(reason))
    ...
    function _beforeTokenTransfer(
        ...
        if (_lockedBy[tokenId] != to) {
            require(_lockedBy[tokenId] == address(0), "PokemonNFT: token transfer while locked");
        } else {
            delete _lockedBy[tokenId];
        }
    }
