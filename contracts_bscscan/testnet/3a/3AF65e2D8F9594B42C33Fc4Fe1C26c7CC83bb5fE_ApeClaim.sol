...
contract ApeClaim is Ownable {
    ...
    function removeLP(address[] calldata lps_) external whenWhiteListNotConfirmed {
        require(! whiteListConfirmed, "ApeClaim: whitelist has already been finalized");
        for (uint i = 0; i < lps_.length; i++) {
    ...
    function releaseYXRewards(uint finalizedYXRewards_) external whenWhiteListConfirmed whenYXRewardsNotReleased {
    ...
    function getClaimableYXRewards() public view returns(uint) {
        uint totalAmount = (WETH.balanceOf(address(this)) + totalClaimed);
        uint yxQuotaAmount = totalAmount * YX_REWARDS / (LPS_COUNT + YX_REWARDS);
