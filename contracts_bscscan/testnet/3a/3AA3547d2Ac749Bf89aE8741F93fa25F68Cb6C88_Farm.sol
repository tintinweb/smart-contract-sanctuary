...
contract Farm is AccountChangable {
  ...
  struct User {
    ...
    address presenter;
    uint256 lockAmount;
    uint256 lastLockUpdatedAt;
    uint256 additionalLockAmount;
  }
  ...
  constructor(
    ...
    sifToken = IERC20(config.sifToken);
    lockFrom = config.lockFrom;
    lockTo = config.lockTo;
  ...
  */
  ...
  uint256 PAY_REASON_LEGEND_COMMISSION = 3;
  uint256 PAY_REASON_SYSTEM_COMMISSION = 4;
  ...
  function harvest(uint256 poolId) public {
    ...
    uint256 rate = ONE_HUNDRED_PERCENT.sub(DIRECT_COMMISSION_RATE_FOR_LEGEND).sub(SYSTEM_COMMISSION_RATE).sub(DEV_COMMISSION);
    payWithLock(sender, pendingReward.mul(rate).div(100), PAY_REASON_FARM_REWARD);
    payCommission(sender, pendingReward);
    ticketToken.mint(dev, pendingReward.mul(DEV_COMMISSION).div(100));
  }
  ...
  function payCommission(address account, uint256 totalReward) public {
    CommissionPayment[3] memory commissionPayments = getCommissionPayments(account);
  ...
  uint256 public DIRECT_COMMISSION_RATE_FOR_MEMBER = 7;
  uint256 public DIRECT_COMMISSION_RATE_FOR_LEGEND = 12;
  uint256 public SYSTEM_COMMISSION_RATE = 3;
  ...
  uint256 public MAX_LEVEL_TO_PAY_COMMISSION = 10;
  
  function setMaxLevelToPayCommission(uint256 _maxLevelToPayCommission) public onlyRoot {
    MAX_LEVEL_TO_PAY_COMMISSION = _maxLevelToPayCommission;
  }

  function getCommissionPayments(address fromAccount) public view returns(CommissionPayment[3] memory result) {
    User memory user = users[fromAccount];
    address presenter = user.presenter;
    uint256 minPresenterLevel = user.level < MAX_LEVEL_TO_PAY_COMMISSION ? 1 : user.level - MAX_LEVEL_TO_PAY_COMMISSION;
    if (getRank(presenter) == RANK_MEMBER) {
      result[0] = CommissionPayment({
        receiver: presenter,
        rate: DIRECT_COMMISSION_RATE_FOR_MEMBER,
        reason: PAY_REASON_DIRECT_COMMISSION
      });
      result[1] = CommissionPayment({
        receiver: getNextLegend(presenter, minPresenterLevel),
        rate: DIRECT_COMMISSION_RATE_FOR_LEGEND - DIRECT_COMMISSION_RATE_FOR_MEMBER,
        reason: PAY_REASON_LEGEND_COMMISSION
      });
      result[2] = CommissionPayment({
        receiver: getNextLegend(result[1].receiver, minPresenterLevel),
        rate: SYSTEM_COMMISSION_RATE,
        reason: PAY_REASON_SYSTEM_COMMISSION
      });
      return result;
    }

    if (getRank(presenter) == RANK_LEGEND) {
      result[0] = CommissionPayment({
        receiver: presenter,
        rate: DIRECT_COMMISSION_RATE_FOR_LEGEND,
        reason: PAY_REASON_DIRECT_COMMISSION
      });
      result[1] = CommissionPayment({
        receiver: getNextLegend(presenter, minPresenterLevel),
  ...
  function getNextLegend(address fromAccount, uint256 minLevel) public view returns(address result) {
    address presenterAddress = users[fromAccount].presenter;
    while (true) {
      User memory presenter = users[presenterAddress];
      if (presenter.level == 0 || presenter.level < minLevel) return root;
      if (getRank(presenterAddress) == RANK_LEGEND) return presenterAddress;
      presenterAddress = presenter.presenter;
    }
  }

  uint256 public LOCK_RATE = 75;

  function payWithLock(address account, uint256 value, uint256 reason) public {
    uint256 toLock = value.mul(LOCK_RATE).div(100);
    uint256 toPay = value.sub(toLock);
    ticketToken.mint(account, toPay);
    emit Pay(account, value, reason);
    lock(account, value);
  }

  function lock(address account, uint256 toLock) public {
    User storage user = users[account];
    uint256 additionalLockAmount = getUnlockable(account);
    user.additionalLockAmount = additionalLockAmount;
    user.lockAmount = user.lockAmount.add(toLock).sub(additionalLockAmount);
    user.lastLockUpdatedAt = getNow();
  }
  ...
  uint256 public lockFrom;
  uint256 public lockTo;

  function getUnlockable(address account) public view returns(uint256) {
    User memory user = users[account];
    uint256 lastLockUpdatedAt = user.lastLockUpdatedAt;
    uint256 unclaimedTime = getCollapse(lastLockUpdatedAt, getNow(), lockFrom, lockTo);
    if (lastLockUpdatedAt > lockTo) return 0;
    uint256 total = lockTo.sub(lastLockUpdatedAt);
    return user.lockAmount.mul(unclaimedTime).div(total).add(user.additionalLockAmount);
  }

  function unlock() public {
    address sender = _originalMsgSender();
    User storage user = users[sender];
    uint256 unlockable = getUnlockable(sender);
    ticketToken.mint(sender, unlockable);
    user.lockAmount = user.lockAmount.sub(unlockable).add(user.additionalLockAmount);
    user.additionalLockAmount = 0;
    user.lastLockUpdatedAt = getNow();
  }
  ...
  uint256 RANK_NORMAL = 0;
  uint256 RANK_MEMBER = 1;
  uint256 RANK_LEGEND = 2;

  IERC20 public sifToken;
  uint256 memberRankRequired = 100 ether;
  uint256 legendRankRequired = 500 ether;

  function getRank(address account) public view returns(uint256) {
    uint256 balance = sifToken.balanceOf(account);
    if (balance >= legendRankRequired) return RANK_LEGEND;
    if (balance >= memberRankRequired) return RANK_MEMBER;
    return RANK_NORMAL;
  }
