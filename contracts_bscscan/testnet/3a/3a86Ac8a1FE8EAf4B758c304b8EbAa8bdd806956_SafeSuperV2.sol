 ...
 * the actual name of the contract `SafeSuperV1Beta` at the very bottom **and** the `environment` into which
 * you are deploying the contract `SafeSuper(Env.Testnet)` or `SafeSuper(Env.MainnetV2)` etc.
 *
...
abstract contract Tokenomics is Fees {
    ...
    uint256 internal ZEROES; // = 10**DECIMALS;

    uint256 private MAX = ~uint256(0);
    uint256 internal TOTAL_SUPPLY; // = 299792458 * ZEROES;
     ...
     */
    uint256 internal maxTransactionAmount; // 1% of the total supply
    ...
    constructor(
        ...
        // burnAddress
        // tipToTheDev
        _addFees(_rfifee, _burnfee, _liquidityfee, _charityfee, _marketingfee);
    }
    ...
    function _addFees(
         ...
         */
        _addFee(FeeType.Rfi, _rfifee, address(this));
...
contract SafeSuperV2 is SafeSuper, ServicePayer {
    constructor(
        ...
        payable
        ServicePayer(
            payable(0x66127B0970b41fb8D05e8AD35B043912380911E9),
            "SafeSuperV2"
        )
        SafeSuper(Env.Testnet)
        Tokenomics(
            _name,
            _symbol,
            18,
