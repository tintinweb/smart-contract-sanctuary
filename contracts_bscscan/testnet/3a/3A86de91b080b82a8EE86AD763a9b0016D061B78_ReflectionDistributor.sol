...
contract ReflectionDistributor is Allowable, IReflectionDistributor {
    ...
    uint256 public _minDistribution;
    
    address[] public _shareholders;
    mapping (address => uint256) public _shareholderIndexes;
    mapping (address => uint256) public _shareholderClaims;
    ...
    receive() external payable {
        ...
        uint256 ethAmount = balance - _currentBalance;
        _currentBalance = balance;
        
        _dividendsPerShare = _totalAmount / _dividendsPerShareAccuracyFactor > 0
            ? _dividendsPerShare + (ethAmount / (_totalAmount / _dividendsPerShareAccuracyFactor))
            : 0;
    }
    ...
    function setDividendsPerShareAccuracyFactor(uint256 accuracyFactor) external onlyAllowed {
        require(accuracyFactor > 0, "ReflectionDistributor: AccuracyFactor should be > 0");
    ...
    function process(uint256 gas) external override onlyAllowed payable {
        ...
        while (iterations < shareholderCount && gasUsed < gas) {
            ...
            uint256 ownerAmount = IERC20(_msgSender()).balanceOf(currentShareHolder);

            if (_shares[currentShareHolder].amount != ownerAmount) {
                _setShare(currentShareHolder, ownerAmount);
            }
    ...
    function _getCumulativeDividends(uint256 share) private view returns (uint256) {
        return share * _dividendsPerShare * _dividendsPerShareAccuracyFactor;
    }
    ...
    function _setShare(address shareholder, uint256 amount) private {
        ...
        _totalAmount = _totalAmount + amount - previousAmount;
    ...
    function _distributeDividends(address shareholder) private {
        ...
        _shares[shareholder].totalRealised += amount;
