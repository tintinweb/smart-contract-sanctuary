...
contract Checklist {
    ...
    Task tasks = Task(0xc645148A9364dFe3C97D56b7CBA8B28D84D0523F);
    ...
    struct ChecklistItem {
        ...
        string status; // default: todo, checklist_item status: complete / ready / todo / progress
    ...
    function addUserToChecklistItem(uint item_id, uint assignee) public {
        ...
        for (uint i = 0; i < checklistItemUsers.length; i++) {
            for (uint j = 0; j < checklistItemUsers.length; j++) {
                if ((checklistItemUsers[i].item_id == item_id) && (checklistItemUsers[j].user_id == assigner)) validUser = true;
            }
        }
        if (validUser == true) {
            checklistItemUsers.push(ChecklistItemUser(item_id, assignee, 'n', 'y', block.timestamp, block.timestamp));
        } else revert('Only user in checklist item can add another user');
    }
    ...
    function removeUserFromChecklistItem(uint item_id, uint user_in_item) public {
        ...
        if (validUser == true) {
            for (uint i = 0; i < checklistItemUsers.length; i++) {
                if (checklistItemUsers[i].user_id == user_in_item) {
                    delete checklistItemUsers[i];
                }
            }
        } else revert('Only user in checklist item can remove user');
    }
}