...
contract Pledge is Ownable {
    ...
    function claimStatic(uint slot_) isStart checkTime checkHolder public {
        ...
        if (block.timestamp >= userSlot[msg.sender][slot_].endTime && userSlot[msg.sender][slot_].stakeTime < userSlot[msg.sender][slot_].endTime) {
            ...
            reward = tempDebt * userSlot[msg.sender][slot_].power;
    ...
    function claimToWallet() isStart checkMounth checkTime checkHolder external {
    ...
    function claimCommunity(uint total_, uint amount_, uint timestamp_, bytes32 r, bytes32 s, uint8 v) external {
    ...
    function claimNode(uint total_, uint amount_, uint timestamp_, bytes32 r, bytes32 s, uint8 v) external returns (uint out){
        ...
        if (amount_ > userClaim[msg.sender].quota) {
            ...
            userClaim[msg.sender].nodeTotal += userClaim[msg.sender].quota;
            require(userClaim[msg.sender].nodeTotal <= total_, '20');
            out = userClaim[msg.sender].quota;
            emit ClaimNode(msg.sender, userClaim[msg.sender].quota);
        } else {
            SpeToken.transfer(msg.sender, amount_);
