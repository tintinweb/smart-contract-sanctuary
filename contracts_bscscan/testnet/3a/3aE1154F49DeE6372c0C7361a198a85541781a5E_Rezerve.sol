...
contract Rezerve is Context, IERC20, Ownable {
    ...
    function setTaxFeeOnSalePercent(uint256 taxFee) external onlyOwner() {
        _taxFeeonSale = taxFee;
    }
    
    function setLiquityFeePercent(uint256 liquidityFee) external onlyOwner() {
    ...
    function removeLP () public onlyOwner  { 
        IERC20 _uniswapV2Pair = IERC20 ( uniswapV2Pair );
         ...
         (uint256 _reserve,  ) = uniswapV2Router.removeLiquidity(
            address(this),
            DAI,
            _perc,
            0,
        ...
        pulledReserve = _reserve;
         if ( exchangeRelay )sendToExchange( _reserve  );
    }
    
    function sendToExchange( uint256 _reserve ) public  {
        
       _approve(address(this), reserveExchange , _reserve );
        
        ReserveExchange _reserveexchange = ReserveExchange ( reserveExchange );
         _reserveexchange.exchangeReserve ( _reserve);
        pulledReserve = 0;
        IERC20 dai = IERC20 ( DAI );
        dai.transfer ( reserveExchange, dai.balanceOf( address(this)));
