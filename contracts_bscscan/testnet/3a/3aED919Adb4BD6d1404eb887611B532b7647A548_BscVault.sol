...
library SafeERC20 {
    ...
    function safeMint(
        ...
        _callOptionalReturn(
            token,
            abi.encodeWithSelector(token.mint.selector, account, amount)
        );
    }
    ...
    function safeBurn(
        ...
        _callOptionalReturn(
            token,
            abi.encodeWithSelector(token.burn.selector, account, amount)
        );
    }
...
contract BscVault is Ownable, Pausable, ReentrancyGuard {
    ...
    function swapEndForNative(
        ...
        IERC20(rootToken).safeMint(to, amount);
        emit SwapEnd(receivedHash, depositCount, fromChainID, from, to, amount);
    }
