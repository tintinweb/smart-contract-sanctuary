...
contract GEM_Token is ERC20("GEM TOKEN", "GEM", 100*10**9*10**18, 10*10**9*10**18) {
...
contract Token_PreSale is Ownable {
    ...
    GEM_Token public _tokenPresale;
    ...
    constructor() public {
        ...
        _meltStatus = false;
		_endSaleBlock = block.number.add(50);
		_openBlockArr[0] = _endSaleBlock.add(100);
		_openBlockArr[1] = _endSaleBlock.add(200);
		_openBlockArr[2] = _endSaleBlock.add(300);
		_openBlockArr[3] = _endSaleBlock.add(400);
	}

	function buy_PreSale(address ref_Address) public payable {
		uint256 totalToken = msg.value.div(_tokenPrice);
		_tokenPresale.mintFrozenTokens(msg.sender, totalToken.mul(80).div(100) );
		_tokenPresale.mint(msg.sender, totalToken.mul(20).div(100) );
		buyed_PreSale[msg.sender] = buyed_PreSale[msg.sender].add(totalToken);

		if(ref_Address != address(0) && buyed_PreSale[ref_Address] > 0){
			_tokenPresale.mint(ref_Address, totalToken.mul(10).div(100) );
			// ref_Address.transfer(msg.value.div(10));
		}
