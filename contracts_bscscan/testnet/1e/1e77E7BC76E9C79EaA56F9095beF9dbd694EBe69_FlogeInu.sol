pragma solidity ^0.8.11;
  ...
  contract FlogeInu is Context, IERC20, Ownable {
      ...
      Fees private _buyFees = Fees(20, 20, 10, 25, 5);
      Fees private _sellFess = Fees(30, 30, 15, 40, 5);
      Fees private _currentFees;
      ...
      function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
          ...
          tTransferAmount = tTransferAmount.sub(tBurn).sub(tCharity).sub(tMarketing);
      ...
      function calculateTaxFee(uint256 _amount) private view returns (uint256) {
          return _amount.mul(_currentFees.reflection).div(
              10**3
          );
      }
  
      function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
          return _amount.mul(_currentFees.liquidity).div(
              10**3
          );
      }

      function calculateMarketingFee(uint256 _amount) private view returns (uint256) {
          return _amount.mul(_currentFees.marketing).div(
              10**3
          );
      }

      function calculateBurnFee(uint256 _amount) private view returns (uint256) {
          return _amount.mul(_currentFees.burn).div(
              10**3
          );
      }

      function calculateCharityFee(uint256 _amount) private view returns (uint256) {
          return _amount.mul(_currentFees.charity).div(
              10**3
          );
      }
      
      function removeAllFee() private {
          _currentFees = Fees(0, 0, 0, 0, 0);
      }
      ...
      function _transfer(
          ...
          if (from != owner() && to != owner() && from != airdropWallet && to != airdropWallet)
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");

          //AirdropLock check
          if (_airdropped[from] && _airdropLog[from].releaseTime > block.timestamp)
            require(balanceOf(from).sub(amount) >= _airdropLog[from].amount, "Cannot sell/transfer airdropped tokens: still locked.");
      ...
      function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
          ...
          //taking charity, marketing and burn fee
          uint256 burnFee = calculateBurnFee(amount);
          _tOwned[address(0)] = _tOwned[address(0)].add(burnFee);
          _rOwned[address(0)] = _rOwned[address(0)].add(burnFee.mul(_getRate()));
          ...
          _rOwned[charityWallet] = _rOwned[charityWallet].add(charityFee.mul(_getRate()));
          emit Transfer(sender, charityWallet, charityFee);

          uint256 marketingFee = calculateMarketingFee(amount);
          _tOwned[marketingWallet] = _tOwned[marketingWallet].add(marketingFee);
          _rOwned[marketingWallet] = _rOwned[marketingWallet].add(marketingFee.mul(_getRate()));
      ...
      function isAddressAirdropped (address addr) view external returns (bool) {
          return _airdropped[addr];
      }
