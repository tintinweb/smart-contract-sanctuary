...
contract BuybackAgent is IBuyback, EmergencyWithdraw, ReentrancyGuardUpgradeable, CoinDexTools {
  ...
  // buyback config
  struct Config {
    uint configId; // config id
    address fromToken; // from token address
    address buybackToken; // buyback token address
    address burnAddress; // burn address
    address configOwner; // config owner
  }
  // buybackTokens[config id] => config info
  mapping(uint => Config) public buybackConfigs;
  uint public nextConfigId;

  // buybackPendingByTokens[from token][buyback token] => pending buyback amount
  mapping(address => mapping(address => uint)) public buybackPendingByTokens;
  // Buyback statistic
  struct Statistic {
    uint totalPending;
    uint totalSwapped;
  ...
  mapping(address => Statistic) public buybackTokenStatistics;
  address[] public buybackTokens;
  ...
  /**
   * @dev Register buyback config
   * _DEAD_ADDRESS = 0x000000000000000000000000000000000000dEaD
   */
  function registerBuyback(
    address _fromToken,
    address _buybackToken,
    address _burnAddress
  ) external override nonReentrant returns (uint _configId) {
    _configId = nextConfigId;
    _setBuybackConfig(_configId, _fromToken, _buybackToken, _burnAddress, msg.sender);
    emit ERegisterBuyback(_configId, _fromToken, _buybackToken, _burnAddress, msg.sender);
    nextConfigId++;
  }

  /**
   * @dev Update buyback config
   */
  function updateBuyback(
    ...
    _setBuybackConfig(_configId, _fromToken, _buybackToken, _burnAddress, msg.sender);
    emit EUpdateBuyback(_configId, _fromToken, _buybackToken, _burnAddress, msg.sender);
  }

  /**
   * @dev Update buyback config private
   */
  function _setBuybackConfig(
    uint _configId,
    address _fromToken,
    address _buybackToken,
    address _burnAddress,
    address _configOwner
  ) private {
    if (msg.sender != owner() && buybackConfigs[_configId].configOwner != address(0)) {
      require(_configId < nextConfigId, "Invalid id");
      require(buybackConfigs[_configId].configId == _configId, "Id does not match");
      require(buybackConfigs[_configId].configOwner == _configOwner, "Only owner");
    }
    buybackConfigs[_configId].configId = _configId;
    buybackConfigs[_configId].fromToken = _fromToken;
    buybackConfigs[_configId].buybackToken = _buybackToken;
    buybackConfigs[_configId].burnAddress = _burnAddress;
    buybackConfigs[_configId].configOwner = _configOwner;
  }
   ...
   */
  function depositTax(uint _buybackId, uint _amount) external override {
    Config storage config_ = buybackConfigs[_buybackId];
    require(config_.configOwner != address(0), "Invalid config");

    IERC20 token_ = IERC20(config_.fromToken);
    ...
    Statistic storage statistic_ = buybackTokenStatistics[config_.buybackToken];
    if (!statistic_.registered) {
      statistic_.registered = true;
      statistic_.index = buybackTokens.length;
      buybackTokens.push(config_.buybackToken);
    }
    buybackTokenStatistics[config_.buybackToken].totalPending += addedAmount_;
    buybackPendingByTokens[config_.fromToken][config_.buybackToken] += addedAmount_;
  }
   ...
   */
  function solvePendingTax(uint _buybackId, uint _debt) external override {
    Config storage config_ = buybackConfigs[_buybackId];
    if (getLP(config_.fromToken) == address(0)) return;
    require(config_.configOwner != address(0), "Invalid config");

    uint bal_ = IERC20(config_.fromToken).balanceOf(address(this)) - _debt;
    uint pending_ = buybackPendingByTokens[config_.fromToken][config_.buybackToken];
    buybackTokenStatistics[config_.buybackToken].totalPending -= pending_;
    if (pending_ > bal_) {
      pending_ = bal_;
    }
    bal_ -= pending_;
    if (pending_ > 0) {
      IERC20 buybackToken_ = IERC20(config_.buybackToken);
      uint previousBalance_ = buybackToken_.balanceOf(address(this));
      _swapTokensForTokens(config_.fromToken, config_.buybackToken, pending_, address(this), true);
      uint newBalance_ = buybackToken_.balanceOf(address(this));
      uint buybackAmount_ = newBalance_ - previousBalance_;
      // solhint-disable reentrancy
      buybackToken_.transfer(config_.burnAddress, buybackAmount_);
      buybackTokenStatistics[config_.buybackToken].totalSwapped += buybackAmount_;
      buybackPendingByTokens[config_.fromToken][config_.buybackToken] = 0;
    }
  }

  /**
   * @dev Solve all pending tax by owner
   */
  function solveAllPendingTax(address _fromToken, address _buybackToken) external onlyOwner {
    if (getLP(_fromToken) == address(0)) return;
    uint bal_ = IERC20(_fromToken).balanceOf(address(this));
    uint pending_ = buybackPendingByTokens[_fromToken][_buybackToken];
    buybackTokenStatistics[_buybackToken].totalPending -= pending_;
    if (pending_ > bal_) {
      pending_ = bal_;
    }
    bal_ -= pending_;
    if (pending_ > 0) {
      ...
      uint buybackAmount_ = newBalance_ - previousBalance_;
      // solhint-disable reentrancy
      buybackToken_.transfer(owner(), buybackAmount_);
      buybackTokenStatistics[_buybackToken].totalSwapped += buybackAmount_;
      buybackPendingByTokens[_fromToken][_buybackToken] = 0;
    }
  }
}
...
interface IBuyback {
  ...
  struct Info {
    ...
    uint configId; // buyback config id
    uint tax; // buyback tax
  }

  event ERegisterBuyback(
    uint _configId,
    address _fromToken,
    address _buybackToken,
    address _burnAddress,
  ...
  function registerBuyback(
    ...
    address _buybackToken,
    address _burnAddress
  ) external returns (uint _configId);
   ...
   * @dev Deposit taxed amount
   */
  function depositTax(uint _configId, uint _amount) external;

  /**
   * @dev Solve pending tax
   */
  function solvePendingTax(uint _configId, uint _debt) external;
}
