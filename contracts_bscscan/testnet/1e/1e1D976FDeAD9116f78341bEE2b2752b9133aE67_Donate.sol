...
contract Donate is EIP20Interface, Ownable, Pausable {
    ...
    uint256 public mixDonate = 100000000;
    ...
    event NewDonate(address indexed from, uint256 value, uint256 value1);
    ...
    /* start next index */
    function stratNextIndex() public onlyOwner whenNotPaused returns (bool success){
    ...
    /* set Lock contract */
    function setLockAddr(address _addr) public onlyOwner whenNotPaused returns (bool success){
    ...
    /* setnode */
    function setNode(address _addr, bool _set) public onlyOwner whenNotPaused returns (bool success){
    ...
    /* setRule */
    function setRule(uint256 _airdrop, uint256 _reward, uint32 _inviterProp, uint32 _nodeProp, uint32 _priceProp, uint8 _isAdd, uint8 _index) public onlyOwner whenNotPaused returns (bool success){
    ...
    /* setMinDonate */
    function setMinDonate(uint256 _value) public onlyOwner whenNotPaused returns(bool success) {
        mixDonate = _value;
        return true;
    }
    ...
    /* setAddress */
    function setInCoin(address _inCoin) public onlyOwner whenNotPaused returns(bool success) {
        inCoin = EIP20Interface(_inCoin);
        return true;
    }

    /* setReceiver */
    function setReceiver(address _receiver) public onlyOwner whenNotPaused returns(bool success){
        receiver =_receiver;
        return true;
    }
    
    /* setPlatform */
    function setPlatform(address _platform) public onlyOwner whenNotPaused returns(bool success){
        platform =_platform;
        return true;
    }
    ...
    function doDonate(uint256 _value, address _inviterAddr) public whenNotPaused returns (bool success){
        ...
        require((_inviterAddr != address(0) && users[_inviterAddr].isUsed) || (_inviterAddr == address(0)), 'iviter does not exist !' );
        ...
        if(remainReward >= ownReward) {
            remainReward -= ownReward;
        } else {
            ownReward = remainReward;
            remainReward = 0;
        }
        
        if(_value >= mixDonate && ownReward > 0) {
            uint256 inviterReward = ownReward.safeMul(rules[index].upProp).safeDiv(PERCENTS_DIVIDER);
            ...
            if(inviterReward > 0) {
                if(user.inviter != address(0)) {
                    ...
                    uint256 nodeReward = ownReward.safeMul(rules[index].nodeProp).safeDiv(PERCENTS_DIVIDER);
        ...
        emit NewDonate(msg.sender, _value, ownReward);
