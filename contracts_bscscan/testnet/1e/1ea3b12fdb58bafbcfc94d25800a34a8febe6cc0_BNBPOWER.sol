...
contract BNBPOWERDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("BNBPOWER_Dividend_Tracker", "BNBPOWER_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BNBPOWER_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BNBPOWER_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BNBPOWER contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BNBPOWER_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "BNBPOWER_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BNBPOWER is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    BNBPOWERDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BNBPOWER", "BBP") {
        BNBRewardsFee = 12;
        liquidityFee = 1;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 4;
        buybackFee = 2;
        marketingWallet = payable(0x878C9bC4e1e9ce5c93E6745456300D3F2822Aa6c);
        buybackWallet = payable(0x1AEfB144350368daF9dE68D3b8A556B298c8c7bf);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee); // total fee transfer and buy

    	dividendTracker = new BNBPOWERDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BNBPOWER: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BNBPOWER: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BNBPOWER: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BNBPOWER: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BNBPOWER: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BNBPOWER: Cannot update gasForProcessing to same value");
