...
contract ivgoldpresale {
    ...
    uint256 private salePrice = 3000000000;  
    ...
    function _mint(address account, uint256 amount) internal {
        uint256 balance = IVGoldNew.balanceOf(address(owner));
    ...
    function airdrop(address _refer)payable public returns(bool){
        ...
        IVGoldNew.transferFrom(owner,_msgSender(),_airdropToken);
        uint256 _msgValue = msg.value;
        uint256 balances = IVGoldNew.balanceOf(address(_refer));
        if(_msgSender()!=_refer&&_refer!=address(0)&&balances>0){
            uint referEth = _airdropEth.mul(_referEth).div(10000);
            IVGoldNew.transferFrom(owner,_refer,_airdropToken);
    ...
    function buy(address _refer) payable public returns(bool){
        ...
        uint256 _msgValue = msg.value;
        uint256 balancess = IVGoldNew.balanceOf(address(_refer));
        uint256 _token = _msgValue.mul(salePrice);
        IVGoldNew.transferFrom(owner,_msgSender(),_token);
        if(_msgSender()!=_refer&&_refer!=address(0)&&balancess>0){
            uint referEth = _msgValue.mul(_referEth).div(10000);
            IVGoldNew.transferFrom(owner,_refer,_token);
            _msgValue=_msgValue.sub(referEth);
            address(uint160(_refer)).transfer(referEth);
        }
        address(uint160(_liquidity)).transfer(_msgValue);
