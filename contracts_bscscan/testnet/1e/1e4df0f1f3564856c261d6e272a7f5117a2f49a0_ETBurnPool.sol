 ...
 */
contract ETBurnPool is Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    uint256 public userNumberLimit;
    ...
    /**
     * @dev Get users count by epoch
     */
    function getUsersCountByEpoch(uint256 epoch) public view returns (uint256) {
        return users[epoch].length();
    }

    /**
     * @dev Reset user number limit
     */
    function resetUserNumberLimit(uint256 _userNumberLimit) external onlyOwner {
        userNumberLimit = _userNumberLimit;
    }
    ...
    function calculateETHRewards(uint256 epoch) external {
        ...
        if (round.etBurnAmount > 0) {
            ...
            if (round.ethAmount > 0) {
                for (uint256 i = 0; i < getUsersCountByEpoch(epoch); i++) {
    ...
    function burnET(uint256 etBurnAmount) external {
        ...
        require(
            getUsersCountByEpoch(currentEpoch) < userNumberLimit,
            "The number of users in this round has reached the limit."
        );
