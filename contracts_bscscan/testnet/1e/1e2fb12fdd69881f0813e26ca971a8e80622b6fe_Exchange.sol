...
contract Exchange is Ownable {
    ...
    uint256 public limit = 1000000e9; 
    ...
    constructor (IERC20 _oldWOW, IERC20 _newWOW) public {
        wow = _oldWOW;
        newWow = _newWOW;
    }
    ...
    function exchange(uint256 _amount) external isPause {
        require(_amount >= minimumTokenToExchange, "_amount to exchange must be atleast one");
        
        uint256 value = _amount.mul(1e12).mul(1e9).div(limit); //amountIn *1e12 *decimals /limit 
        
        value = value.div(1e12); // value /1e12
        
        require(wow.transferFrom(msg.sender, address(this), _amount), "wow transer Failed");
