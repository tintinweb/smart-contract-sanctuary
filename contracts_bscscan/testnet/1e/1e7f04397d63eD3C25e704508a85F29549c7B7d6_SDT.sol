...
contract SDT is Context, IERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
   ...
   // mapping(address => uint256) public nextAvailableClaimDate;
    ...
    string private _name = "Super Digital Token";
    string private _symbol = "SDT";
    uint8 private _decimals = 9;
    uint256 private _totalSupply = 100 * 1e9 * 1e9;
  ...
  //  uint256 public rewardCycleBlock = 24 hours;
   ...
   // bool public reflectionFeesdiabled; // fees are enabled by default
    ...
    uint256 public _LiquidityFee = 20;   // 2% will be added to pool for ETH reward
    uint256 public _marketFee = 30; // 3% will go to the market address
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "SDT: transfer amount exceeds allowance"
            )
        );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "SDT: decreased allowance below zero"
            )
        );
        return true;
    }
    ...
    function setFeePercent(uint256 LiquidityFee, uint256 marketFee) external onlyOwner {
        _LiquidityFee = LiquidityFee;
        _marketFee = marketFee;
        _totalFeePerTx = LiquidityFee.add(marketFee);
       
    }

    function enableTrading() external onlyOwner {
        require(!_tradingOpen,"SDT: Already enabled");
        _tradingOpen = true;
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns (uint256) {
        uint256 percentage = tAmount.mul(_LiquidityFee.add(_marketFee)).div(1e3);
        return percentage;
    }
    ...
    function _approve(
        ...
        require(owner != address(0), "SDT: approve from the zero address");
        require(spender != address(0), "SDT: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "SDT: transfer from the zero address");
        require(to != address(0), "SDT: transfer to the zero address");
        require(amount > 0, "SDT: transfer amount must be greater than zero");
        require(balanceOf(from) >= amount,"SDT: not enough balance");
        ...
        if (
            ...
            require(
                amount <= _maxTxAmount,
                "SDT: Transfer amount exceeds the maxTxAmount."
            );

            if (!_tradingOpen) {
                require(
                    from != uniSwapV2Pair && to != uniSwapV2Pair,
                    "SDT: Trading is not enabled"
                );
            } 
        }
    ...
    function _takeBothFee(uint256 tAmount) internal {
        uint256 totalFee = tAmount.mul(_marketFee.add(_LiquidityFee)).div(1e3);
        uint256 marketfeee = tAmount.mul(_marketFee).div(1e3);
        _balances[address(this)] = _balances[address(this)].add(totalFee);
        _balances[address(marketWallet)]=_balances[address(marketWallet)].add(marketfeee);
        _balances[address (this)]=_balances[address(this)].sub(marketfeee);
       // _balances[address(address(this))]=_balances[address(address(this))].add(_LiquidityFee);

        emit Transfer(_msgSender(), address(this), totalFee);
        emit Transfer(_msgSender(), address(marketWallet), marketfeee);
        //emit Transfer(_msgSender(), address(this), _LiquidityFee);
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            uint256 totalPercent = _marketFee.add(_LiquidityFee);
