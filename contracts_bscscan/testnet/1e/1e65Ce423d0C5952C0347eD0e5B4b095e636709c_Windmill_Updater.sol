...
event GasLog_Gas_Refundable(uint256 line, uint256 gas);
    ...
    function getRefundableGas(address addr) public returns (uint256){
emit GasLog_Gas_Refundable(39, gasleft());
        return _refundData[addr].usedGas * refundBNBBonusRatio.numerator / refundBNBBonusRatio.denominator;
emit GasLog_Gas_Refundable(40, gasleft());
    }

    function isRefundAvailable(address addr) public returns (bool){
emit GasLog_Gas_Refundable(43, gasleft());
        return block.number >= _refundData[addr].refundLastBlock + refundNbBlockDelay;
emit GasLog_Gas_Refundable(44, gasleft());
    }

    function refundGas() external nonReentrant{
emit GasLog_Gas_Refundable(47, gasleft());
        uint256 BNBToRefund = getRefundableGas(msg.sender);
emit GasLog_Gas_Refundable(48, gasleft());

        require(BNBToRefund >= refundBNBMin, "Gas_Refundable: Too small amount to refund");
emit GasLog_Gas_Refundable(50, gasleft());
        require(isRefundAvailable(msg.sender), "Gas_Refundable: Refund delay not passed");
emit GasLog_Gas_Refundable(51, gasleft());

        beforeRefundGas(BNBToRefund);
emit GasLog_Gas_Refundable(53, gasleft());

        _refundData[msg.sender].usedGas = 0;
emit GasLog_Gas_Refundable(55, gasleft());
        _refundData[msg.sender].refundLastBlock = block.number;
emit GasLog_Gas_Refundable(56, gasleft());
        address payable sender = payable(msg.sender);
emit GasLog_Gas_Refundable(57, gasleft());
        sender.transfer(BNBToRefund);
emit GasLog_Gas_Refundable(58, gasleft());
    }
...
event GasLog_Payable(uint256 line, uint256 gas);
    receive() external payable {}
}
// Version: 1.0.0
...
event GasLog_With_DAOContract(uint256 line, uint256 gas);
    I_Windmill_DAO public DAO;

    function setDAO(address _DAO) external onlyRole(DAO_ROLE){
emit GasLog_With_DAOContract(15, gasleft());
        DAO = I_Windmill_DAO(_DAO);
emit GasLog_With_DAOContract(16, gasleft());
    }
}
...
event GasLog_Windmill_Updater(uint256 line, uint256 gas);
    ////
    ////
    ////
    //////////////// Public variables ////////////////
...
emit GasLog_Windmill_Updater(61, gasleft());
        currentDAOCycleEndBlock = block.number + DAOCycleDurationNbBlock;
emit GasLog_Windmill_Updater(62, gasleft());
        currentRoyaltyCycleEndBlock = block.number + royaltyCycleDurationNbBlock;
emit GasLog_Windmill_Updater(63, gasleft());
        currentStackingCycleEndBlock = block.number + stackingCycleDurationNbBlock;
emit GasLog_Windmill_Updater(64, gasleft());
    }
    
    function setDAOCycleDuration(uint256 duration) external onlyRole(DAO_ROLE){
emit GasLog_Windmill_Updater(67, gasleft());
        DAOCycleDurationNbBlock = duration;
emit GasLog_Windmill_Updater(68, gasleft());
    }

    function setStackingCycleDuration(uint256 duration) external onlyRole(DAO_ROLE){
emit GasLog_Windmill_Updater(71, gasleft());
        stackingCycleDurationNbBlock = duration;
emit GasLog_Windmill_Updater(72, gasleft());
    }

    function setRoyaltyCycleDuration(uint256 duration) external onlyRole(DAO_ROLE){
emit GasLog_Windmill_Updater(75, gasleft());
        royaltyCycleDurationNbBlock = duration;
emit GasLog_Windmill_Updater(76, gasleft());
    }
    
    function setUserGovernorStatus(address user, bool _isGovernor) external onlyRole(DAO_ROLE){
emit GasLog_Windmill_Updater(79, gasleft());
        isGovernor[user] = _isGovernor;
emit GasLog_Windmill_Updater(80, gasleft());
    }
    
    function updateRoyaltyCycleNeeded() public returns (bool){
emit GasLog_Windmill_Updater(83, gasleft());
        return (block.number >= currentRoyaltyCycleEndBlock);
emit GasLog_Windmill_Updater(84, gasleft());
    }

    function updateOneRoyaltyCycle() external onlyGovernor gasRefundable{
emit GasLog_Windmill_Updater(87, gasleft());
        require(updateRoyaltyCycleNeeded(), "Windmill_DAO: A Royalty cycle update is not required. Gas will not be refunded.");
emit GasLog_Windmill_Updater(88, gasleft());
        _updateOneRoyaltyCycle();
emit GasLog_Windmill_Updater(89, gasleft());
    }

    function updateRoyaltyCycle() external onlyGovernor gasRefundable{
emit GasLog_Windmill_Updater(92, gasleft());
        require(updateRoyaltyCycleNeeded(), "Windmill_DAO: A Royalty cycle update is not required. Gas will not be refunded.");
emit GasLog_Windmill_Updater(93, gasleft());

        do{
emit GasLog_Windmill_Updater(95, gasleft());
            _updateOneRoyaltyCycle();
emit GasLog_Windmill_Updater(96, gasleft());
        }while(updateRoyaltyCycleNeeded());
emit GasLog_Windmill_Updater(97, gasleft());
    }

    function updateStackingCycleNeeded() public returns (bool){
emit GasLog_Windmill_Updater(100, gasleft());
        return (block.number >= currentStackingCycleEndBlock);
emit GasLog_Windmill_Updater(101, gasleft());
    }

    function updateOneStackingCycle() external onlyGovernor gasRefundable{
emit GasLog_Windmill_Updater(104, gasleft());
        require(updateStackingCycleNeeded(), "Windmill_DAO: A Stacking cycle update is not required. Gas will not be refunded.");
emit GasLog_Windmill_Updater(105, gasleft());
        _updateOneStackingCycle();
emit GasLog_Windmill_Updater(106, gasleft());
    }

    function updateStackingCycle() external onlyGovernor gasRefundable{
emit GasLog_Windmill_Updater(109, gasleft());
        require(updateStackingCycleNeeded(), "Windmill_DAO: A Stacking cycle update is not required. Gas will not be refunded.");
emit GasLog_Windmill_Updater(110, gasleft());

        do{
emit GasLog_Windmill_Updater(112, gasleft());
            _updateOneStackingCycle();
emit GasLog_Windmill_Updater(113, gasleft());
        }while(updateStackingCycleNeeded());
emit GasLog_Windmill_Updater(114, gasleft());
    }

    function updateStackingUpdateNeeded() public returns (bool){
emit GasLog_Windmill_Updater(117, gasleft());
        return stacking.updateStackingNeeded();
emit GasLog_Windmill_Updater(118, gasleft());
    }
    
    function updateStackingUpdate() external onlyGovernor gasRefundable{
emit GasLog_Windmill_Updater(121, gasleft());
        require(stacking.updateStackingNeeded(), "Windmill_DAO: A Stacking update is not required. Gas will not be refunded.");
emit GasLog_Windmill_Updater(122, gasleft());

        _updateStackingUpdate();
emit GasLog_Windmill_Updater(124, gasleft());
    }
    
    function updateOneStackingUpdateNeeded(uint256 groupId) public returns (bool){
emit GasLog_Windmill_Updater(127, gasleft());
        return stacking.updateOneStackingNeeded(groupId);
emit GasLog_Windmill_Updater(128, gasleft());
    }
    
    function updateOneStackingUpdate(uint256 groupId) external onlyGovernor gasRefundable{
emit GasLog_Windmill_Updater(131, gasleft());
        require(stacking.updateOneStackingNeeded(groupId), "Windmill_DAO: A Stacking update is not required. Gas will not be refunded.");
emit GasLog_Windmill_Updater(132, gasleft());

        _updateOneStackingUpdate(groupId);
emit GasLog_Windmill_Updater(134, gasleft());
    }
    
    function updateTradeNeeded() public returns (bool){
emit GasLog_Windmill_Updater(137, gasleft());
        uint256 l = tradeManager.getNbOpenTrades();
emit GasLog_Windmill_Updater(138, gasleft());
        
        for (uint256 i=0; i<l; i++){
emit GasLog_Windmill_Updater(140, gasleft());
            (I_Windmill_Trade_Manager.TradeData memory data,) = tradeManager.getOpenTrade(i);
emit GasLog_Windmill_Updater(141, gasleft());
            if (data.trade.updateNeeded()){
emit GasLog_Windmill_Updater(142, gasleft());
                return true;
emit GasLog_Windmill_Updater(143, gasleft());
            }
        }
        
        return false;
emit GasLog_Windmill_Updater(147, gasleft());
    }

    function updateOneTrade(uint256 tradeId) external onlyGovernor gasRefundable{
emit GasLog_Windmill_Updater(150, gasleft());
        I_Windmill_Trade_Manager.TradeData memory data = tradeManager.getTrade(tradeId);
emit GasLog_Windmill_Updater(151, gasleft());
        
        require(data.trade.updateNeeded(), "Windmill_DAO: A trade update is not required. Gas will not be refunded.");
emit GasLog_Windmill_Updater(153, gasleft());
        
        data.trade.update();
emit GasLog_Windmill_Updater(155, gasleft());
    }

    function updateTrade() external onlyGovernor gasRefundable{
emit GasLog_Windmill_Updater(158, gasleft());
        bool updateApplied = false;
emit GasLog_Windmill_Updater(159, gasleft());
        uint256 l = tradeManager.getNbOpenTrades();
emit GasLog_Windmill_Updater(160, gasleft());
        
        for (uint256 i=0; i<l; i++){
emit GasLog_Windmill_Updater(162, gasleft());
            (I_Windmill_Trade_Manager.TradeData memory data,) = tradeManager.getOpenTrade(i);
emit GasLog_Windmill_Updater(163, gasleft());
            if (data.trade.updateNeeded()){
emit GasLog_Windmill_Updater(164, gasleft());
                updateApplied = true;
emit GasLog_Windmill_Updater(165, gasleft());
                data.trade.update();
emit GasLog_Windmill_Updater(166, gasleft());
            }
        }
        
        require(updateApplied, "Windmill_DAO: A trade update is not required. Gas will not be refunded.");
emit GasLog_Windmill_Updater(170, gasleft());
    }
    
    function updateDAOCycleNeeded() public returns (bool){
emit GasLog_Windmill_Updater(173, gasleft());
        return (block.number >= currentDAOCycleEndBlock);
emit GasLog_Windmill_Updater(174, gasleft());
    }

    function updateOneDAOCycle() external onlyGovernor gasRefundable{
emit GasLog_Windmill_Updater(177, gasleft());
        require(updateDAOCycleNeeded(), "Windmill_DAO: A DAO cycle update is not required. Gas will not be refunded.");
emit GasLog_Windmill_Updater(178, gasleft());
        _updateOneDAOCycle();
emit GasLog_Windmill_Updater(179, gasleft());
    }

    function updateDAOCycle() external onlyGovernor gasRefundable{
emit GasLog_Windmill_Updater(182, gasleft());
        require(updateDAOCycleNeeded(), "Windmill_DAO: A DAO cycle update is not required. Gas will not be refunded.");
emit GasLog_Windmill_Updater(183, gasleft());

        do{
emit GasLog_Windmill_Updater(185, gasleft());
            _updateOneDAOCycle();
emit GasLog_Windmill_Updater(186, gasleft());
        }while(updateDAOCycleNeeded());
emit GasLog_Windmill_Updater(187, gasleft());
    }

    function updateDAONeeded() public returns (bool){
emit GasLog_Windmill_Updater(190, gasleft());
        uint256 l = DAO.getNbOpenProposalIds();
emit GasLog_Windmill_Updater(191, gasleft());
        for (uint256 i=0; i<l; ++i){
emit GasLog_Windmill_Updater(192, gasleft());
            if (DAO.updateOpenProposalNeeded(i)){
emit GasLog_Windmill_Updater(193, gasleft());
                return true;
emit GasLog_Windmill_Updater(194, gasleft());
            }
        }
        return false;
emit GasLog_Windmill_Updater(197, gasleft());
    }

    function updateOneDAO(uint256 i) external onlyGovernor gasRefundable{
emit GasLog_Windmill_Updater(200, gasleft());
        _updateOneDAO(i);
emit GasLog_Windmill_Updater(201, gasleft());
    }

    function updateDAO() external onlyGovernor gasRefundable{
emit GasLog_Windmill_Updater(204, gasleft());
        require(updateDAONeeded(), "Windmill_DAO: A DAO update is not required. Gas will not be refunded.");
emit GasLog_Windmill_Updater(205, gasleft());

        uint256 i = DAO.getNbOpenProposalIds();
emit GasLog_Windmill_Updater(207, gasleft());
        while(i>0){
emit GasLog_Windmill_Updater(208, gasleft());
            i--;
emit GasLog_Windmill_Updater(209, gasleft());
            _updateOneDAO(i);
emit GasLog_Windmill_Updater(210, gasleft());
        }
    }
...
emit GasLog_Windmill_Updater(218, gasleft());
        currentRoyaltyCycle += 1;
emit GasLog_Windmill_Updater(219, gasleft());
        currentRoyaltyCycleEndBlock += royaltyCycleDurationNbBlock;
emit GasLog_Windmill_Updater(220, gasleft());

        royalty.processRoyalties();
emit GasLog_Windmill_Updater(222, gasleft());
    }

    function _updateOneStackingCycle() internal{
emit GasLog_Windmill_Updater(225, gasleft());
        currentStackingCycle += 1;
emit GasLog_Windmill_Updater(226, gasleft());
        currentStackingCycleEndBlock += stackingCycleDurationNbBlock;
emit GasLog_Windmill_Updater(227, gasleft());
        
        stacking.updateCycle(currentStackingCycle);
emit GasLog_Windmill_Updater(229, gasleft());
    }

    function _updateOneStackingUpdate(uint256 groupId) internal{
emit GasLog_Windmill_Updater(232, gasleft());
        stacking.updateOneStacking(groupId);
emit GasLog_Windmill_Updater(233, gasleft());
    }
    
    function _updateStackingUpdate() internal{
emit GasLog_Windmill_Updater(236, gasleft());
        stacking.updateStacking();
emit GasLog_Windmill_Updater(237, gasleft());
    }
    
    function _updateOneDAOCycle() internal{
emit GasLog_Windmill_Updater(240, gasleft());
        currentDAOCycle += 1;
emit GasLog_Windmill_Updater(241, gasleft());
        currentDAOCycleEndBlock += DAOCycleDurationNbBlock;
emit GasLog_Windmill_Updater(242, gasleft());
        DAO.setDAOCycle(currentDAOCycle);
emit GasLog_Windmill_Updater(243, gasleft());
    }

    function _updateOneDAO(uint256 i) internal{
emit GasLog_Windmill_Updater(246, gasleft());
        DAO.updateOneOpenProposal(i);
emit GasLog_Windmill_Updater(247, gasleft());
    }

    function beforeRefundGas(uint256 BNBToRefund) internal override{
emit GasLog_Windmill_Updater(250, gasleft());
        fund.getBNBForGasRefund(BNBToRefund);
emit GasLog_Windmill_Updater(251, gasleft());
    }

    function setRefundGasDefaultPrice(uint256 val) external{
emit GasLog_Windmill_Updater(254, gasleft());
        refundGasDefaultPrice = val;
emit GasLog_Windmill_Updater(255, gasleft());
    }
    
    function setRefundNbBlockDelay(uint256 val) external{
emit GasLog_Windmill_Updater(258, gasleft());
        refundNbBlockDelay = val;
emit GasLog_Windmill_Updater(259, gasleft());
    }
    
    function setRefundBNBBonusRatio(uint256 numerator) external{
emit GasLog_Windmill_Updater(262, gasleft());
        refundBNBBonusRatio.numerator = numerator;
emit GasLog_Windmill_Updater(263, gasleft());
    }
    
    function setRefundNbBNBMin(uint256 val) external{
emit GasLog_Windmill_Updater(266, gasleft());
        refundBNBMin = val;
emit GasLog_Windmill_Updater(267, gasleft());
    }
}