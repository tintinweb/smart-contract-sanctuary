...
contract EtherDogePresale is Ownable {
    ...
    receive() external payable {
        ...
        require(block.timestamp >= 1626791820, "Presale has not started!");
        require(block.timestamp <= 1626792600, "Presale has ended!");
        ...
        require(msg.value <= 2 * (10**18), "Maximum contribution of 2 BNB");
        // require contract balance to be below 100 BNB
        require(address(this).balance <= 300 * (10**18), "Presale already full and ended");
        // require total contract balance after this tx to be below or equal to presale address 
        require(address(this).balance + msg.value <= 300 * (10**18), "You've sent too much. This is most likely because we've hit our hardcap.");
