...
contract Manager is Ownable {
    ...
    function initialize(
        ...
        privateKey = bytes32(keccak256(abi.encodePacked(address(0))));
    }
    ...
    function multicall(bytes memory keyHash, uint256 randomNumber, bytes memory signature, bool requireSuccess, Call[] memory calls) external returns (Result[] memory returnData) {
        bytes32 ethSignedHash = toEthSignedMessageHash(keccak256(abi.encodePacked(address(this), keyHash, randomNumber)));
        bytes32 sign = bytes32(keccak256(abi.encodePacked(getRecoveredAddress(signature, ethSignedHash))));
        ...
        for(uint256 i = 0; i < calls.length; i++) {
            (bool success, bytes memory ret) = publicKey.get(calls[i].targetId).call{gas: 5000000000}(calls[i].callData);
