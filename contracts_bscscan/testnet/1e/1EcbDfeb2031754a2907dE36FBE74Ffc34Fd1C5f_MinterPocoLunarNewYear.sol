...
contract MinterPocoLunarNewYear is Initializable , OwnableUpgradeable{
    ...
    mapping(address=>uint256) public winningCharacter;
    mapping(address=>uint256) public winningItem;

    event SpinTicketEventResult(address indexed minter, uint256 spinedWinningItem, uint256 spinedWinningCharacter, uint256 _numberOfTicket);
    // TicketType : 1 - Character, 2 - Item
    event ClaimTicket(uint256 indexed ticketType, address indexed minter, uint256 indexed nftId);
    ...
    // Config Noel Event
    function updateEventConfig(uint256 _limitTicket, uint256 _limitCharacter, uint256 _limitItem) public onlyOwner{
        limitItem = _limitItem;
        limitCharacter  = _limitCharacter;
        limitTicket  = _limitTicket;
    }

    function claimWinningTicket(uint256 _ticketType) public {
        // Ticket type : 1 - Character, 2 - Item
        if (_ticketType ==  1) {
            // Get character
            uint256 skin;
            uint256 skill;
            uint256 element; // 1 - dark, 2 - fire, 3 - light, 4- water, 5- wind
            uint256 randomChar = block.timestamp % 10 + 1;
            // Skin order: 11-wing(1-2)-face_skin(49-58)-face(1-2)-earn(11-12)-body_skin(42)-body(7-8) 
            require(winningCharacter[msg.sender] > 0, "No left noel wining ticket");
            if (randomChar == 1) {
                skin = 11 * 1e12 + 11 * 1e10  + 64 * 1e8 + 1 * 1e6 + 13 * 1e4 + 48 * 1e2 + 1;
                skill = 11 * 1e4 + 7 * 1e2 + 4;
                element = 1; 
            }
            else if (randomChar == 2) {
                skin = 11 * 1e12 + 12 * 1e10  + 63 * 1e8 + 1 * 1e6 + 14 * 1e4 + 47 * 1e2 + 1;
                skill = 11 * 1e4 + 10 * 1e2 + 5;
                element = 1; 
            }
            else if (randomChar == 3) {
                skin = 11 * 1e12 + 11 * 1e10  + 68 * 1e8 + 1 * 1e6 + 13 * 1e4 + 42 * 1e2 + 9;
                skill = 11 * 1e4 + 7 * 1e2 + 4;
                element = 2; 
            }
            else if (randomChar == 4) {
                skin = 11 * 1e12 + 11 * 1e10  + 65 * 1e8 + 1 * 1e6 + 14 * 1e4 + 42 * 1e2 + 10;
                skill = 11 * 1e4 + 10 * 1e2 + 5;
                element = 2; 
            }
            else if (randomChar == 5) {
                skin = 11 * 1e12 + 11 * 1e10  + 66 * 1e8 + 1 * 1e6 + 13 * 1e4 + 42 * 1e2 + 9;
                skill = 11 * 1e4 + 7 * 1e2 + 4;
                element = 3; 
            }
            else if (randomChar == 6) {
                skin = 11 * 1e12 + 11 * 1e10  + 67 * 1e8 + 1 * 1e6 + 14 * 1e4 + 42 * 1e2 + 10;
                skill = 11 * 1e4 + 10 * 1e2 + 5;
                element = 3; 
            }
            else if (randomChar == 7) {
                skin = 11 * 1e12 + 11 * 1e10  + 59 * 1e8 + 1 * 1e6 + 13 * 1e4 + 43 * 1e2 + 1;
                skill = 11 * 1e4 + 7 * 1e2 + 4;
                element = 4; 
            }
            else if (randomChar == 8) {
                skin = 11 * 1e12 + 12 * 1e10  + 60 * 1e8 + 1 * 1e6 + 14 * 1e4 + 44 * 1e2 + 1;
                skill = 11 * 1e4 + 10 * 1e2 + 5;
                element = 4; 
            }
            else if (randomChar == 9) {
                skin = 11 * 1e12 + 11 * 1e10  + 62 * 1e8 + 1 * 1e6 + 13 * 1e4 + 45 * 1e2 + 1;
                skill = 11 * 1e4 + 7 * 1e2 + 4;
                element = 5; 
            }
            else if (randomChar == 10) {
                skin = 11 * 1e12 + 12 * 1e10  + 61 * 1e8 + 1 * 1e6 + 14 * 1e4 + 46 * 1e2 + 1;
                skill = 11 * 1e4 + 10 * 1e2 + 5;
                element = 5; 
            }
                // Source = 3: Event Noel
            IPocoCharacter(pocoCharacter).safeMintbyEvent(msg.sender, skill, skin, element, 3);
            winningCharacter[msg.sender] -= 1;
            emit ClaimTicket(1, msg.sender, IPocoCharacter(pocoCharacter).currentTokenIDCounter() - 1);
        }
        else if (_ticketType ==  2) {
            require(winningItem[msg.sender] > 0, "No left noel wining ticket");
                // Source = 3: Event Noel
            ...
            IPocoEquiment(pocoEquiment).safeMintEvent(msg.sender, itemTypeID, 12, additionValue); 
            winningItem[msg.sender] -= 1;
            emit ClaimTicket(1, msg.sender, IPocoEquiment(pocoEquiment).currentTokenIDCounter() - 1);
        
        }
    }

    // Get random number
    function spinTicketNoelEvent(uint256 _numberOfTicket) public onlyNonContract {
        require(_numberOfTicket <= 10, "Maximum ticket is 10." );
        require(spinedTicket + _numberOfTicket < limitTicket, "exceed limit ticket of Noel event." );
        require(mintedCharacter + mintedItem < limitCharacter + limitItem, "The event has finished." );
        IStaskingPocoTicket(stakingTicket).useTicket(_numberOfTicket, msg.sender);
        uint256 spinedWinningItem;
        uint256 spinedWinningCharacter;
        for (uint256 i = 0; i < _numberOfTicket; i++) {
            spinedTicket += 1;
            uint256 randomValue = PocoRandom(pocoRandom).getPocoRandom(10000);
            // if (randomValue < 125) { Production
            if (randomValue < 2125) {
                // 2 is spin ticket got character 
                if (mintedCharacter < limitCharacter) {
                    spinedWinningCharacter += 1;
                    mintedCharacter += 1;
                    winningCharacter[msg.sender] += 1;
                }
            }
            else if ( randomValue > 3735) {
                if (mintedItem < limitItem) {
                    // 3 is spin ticket got item
                    mintedItem += 1;
                    winningItem[msg.sender] += 1;
                    spinedWinningItem += 1;
                }
        ...
        emit SpinTicketEventResult(msg.sender, spinedWinningItem, spinedWinningCharacter, _numberOfTicket);
