...
interface IEACAggregatorProxy
{
  ...
  contract Xchange_Tokens {
    ...
    function buy( uint256 tokenAmount) public returns(uint256)
    {
      ...
      if(tokenInterface1(tokenMainAddress).balanceOf(address(this))>=token)
      {
        tokenInterface1(tokenMainAddress).transfer(msg.sender, token);
      }
      else
      {
         tokenInterface1(tokenMainAddress).mint(msg.sender, token);
      }
      // fire event
    ...
    function sell(uint256 _amountOfTokens ) external
    {
      require(!safeguard);
        ...
        address _customerAddress = msg.sender;
        uint256 userbalance = tokenInterface1(tokenMainAddress).balanceOf(_customerAddress);
        require(userbalance >= _amountOfTokens ,"Not enough balance");

        tokenInterface1(tokenMainAddress).transferFrom(_customerAddress,address(this),_amountOfTokens);

        uint256 _busd = (_amountOfTokens * sellPrice).div(10 ** decimals) ;
    ...
    function sendToOnlyExchangeContract() public onlyAdministrator returns(bool)
    {
        ...
        uint MaintokenBalance = tokenInterface1(tokenMainAddress).balanceOf(address(this));
        tokenInterface1(tokenMainAddress).transfer(terminal, MaintokenBalance);
        return true;
    }
