...
contract sTrapeza_v2 is IsFIDL, ERC20Permit {
  ...
  constructor(uint256 _prevTotalSupply) ERC20("Staked OHM", "sOHM", 9) ERC20Permit("Staked OHM") {
    initializer = msg.sender;
    _totalSupply = INITIAL_FRAGMENTS_SUPPLY;
    _gonsPerFragment = TOTAL_GONS.div(_prevTotalSupply);
  }
  ...
  function prevCirculatingSupply() public view returns (uint256) {
    return
      balanceForGons(prevsFIDL.gonsForBalance((prevsFIDL.circulatingSupply())));
  }
  ...
  function circulatingSupply() public view override returns (uint256) {
    return
      _totalSupply
        ...
        .add(
          balanceForGons(
            prevsFIDL.gonsForBalance((prevsFIDL.circulatingSupply()))
          )
        )
        .add(gFIDL.balanceFrom(IERC20(address(gFIDL)).totalSupply()))
