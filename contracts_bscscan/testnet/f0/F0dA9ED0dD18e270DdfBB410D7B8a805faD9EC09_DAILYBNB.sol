...
contract DAILYBNB is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DAILYBNBDividendTracker public dividendTracker;

    address public liquidityWallet;
    
    //to change
    address payable public operations1Address =
        payable(0xaAf51C29064d1Ee98c81FA9F173FCf94202618eb); 
    address payable public operations2Address =
        payable(0x414d6D61c7A527c2e23A5A2f463e9A47a433a1eA);

    uint256 public maxSellTransactionAmount = 1 * 10**13 * (10**9);
    uint256 public maxSellTransactionAmount2 = 1 * 10**13 * (10**9);
    uint256 public maxSellTransactionAmount3 = 2 * 10**12 * (10**9);
    uint256 public maxBuyTransactionAmount = 10**13 * (10**9);
    uint256 public swapTokensAtAmount = 1000000000 * (10**9);
    uint256 public maxWalletToken = 41 * 10**12 * (10**9);  //41000000000
    ...
    constructor() ERC20("DAILYBNB", "DAILYBNB") {
        dividendTracker = new DAILYBNBDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        DAILYBNBDividendTracker newDividendTracker = DAILYBNBDividendTracker(
            payable(newAddress)
        );
...
contract DAILYBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "DAILYBNB_Dividend_Tracker",
            "DAILYBNB_Dividend_Tracker"
        )
    {
        dAILYBNB = DAILYBNB(mainContract);
        minimumTokenBalanceForAutoDividends = 5000000000 * (10**9);
        minimumTokenBalanceForDividends = 1000000000 * (10**9);
    ...
    function _transfer(
        ...
        require(false, "DAILYBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DAILYBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DAILYBNB contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(dAILYBNB);

            uint256 prevBalance = dAILYBNB.balanceOf(address(this));
            ...
            {
                uint256 received = dAILYBNB.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    dAILYBNB.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
