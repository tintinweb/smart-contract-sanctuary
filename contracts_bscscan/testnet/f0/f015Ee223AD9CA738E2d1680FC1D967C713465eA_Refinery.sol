...
contract Refinery is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function generateScore(uint256 tokenId, string memory keyPrefix) public view returns (uint256) {
        if (tokenId <= 16000) {
            return 2;
        }
    ...
    function getLootContract(uint256 tokenId) public view returns (LootInterface) {
        if (originalLoot[tokenId] <= 8000) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Refined Bag #', toString(tokenId), '", "description": "Loot is randomized adventurer gear generated and stored on chain. Stats, images, and other functionality are intentionally omitted for others to interpret. Feel free to use Loot in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
