...
contract MFCExchangeCap is Treasury, BackendAgent {
  ...
  address private _mfcExchangeFloorAddress;
  ...
  constructor(
    ...
    _busdTreasuryAddress = busdTreasuryAddress_;
    _busdComptrollerAddress = busdComptrollerAddress_;
    _mfcExchangeFloorAddress = mfcExchangeFloorAddress_;
  }
  ...
  function tradeOffer(uint256 id, uint256 quantity) external {
    ...
    require(quantity >= MINIMUM_SWAP_IN_BUSD, "Minimum swap not reached");
    ...
    if (_offers[id].quantity * _offers[id].price / MULTIPLIER < MINIMUM_SWAP_IN_BUSD) {
      _closeOffer(id);
    }
  ...
  function _tradeOffer(uint256 quantity, uint256 price) private returns (uint256) {
    ...
    _mfc.transfer(_mfcTreasuryAddress, takerFee);
    _mfcExchangeFloor.increaseMfcCirculation(takerReceives);
