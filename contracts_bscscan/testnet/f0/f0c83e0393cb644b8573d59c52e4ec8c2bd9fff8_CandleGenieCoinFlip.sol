...
contract CandleGenieCoinFlip is Ownable, ReentrancyGuard 
{
    ...
    function Flip(int position) external payable nonReentrant notContract 
    {
        ...
        // Brute Check
        Bet storage lastUserBet = Bets[UserBets[msg.sender][currentBetId - 1]];
        if (lastUserBet.betAmount != 0)
        {
            require(block.timestamp > lastUserBet.timestamp + bruteTimeout, "Wait time delay to make a new bet. No brute plis");
        }
