        ...
        ERC20("zz", "xx") 
{
    ...
    struct UserInfo {
        ...
        uint256 amountChest;
        uint256 noOfClaimLFW;
        uint256 noOfClaimNFT;
...
//        require(user.noOfDepositNFT <= 4, "User exceed number of deposit");
        ...
        user.noOfClaimNFT = user.noOfClaimNFT + 1;
    }
    ...
    function withdraw() external nonReentrant {
        ...
        require(user.noOfClaimLFW > 0, "User does not stake anything");
        require(user.noOfClaimNFT > 0, "User does not stake anything");
        require(user.amount >= busdIn, "User does not stake enough BUSD");
        require(user.amountChest >= depositChestMax, "User does not stake enough Chest");
        require(user.rewardStakingBlockLFW[0] >= block.number, "Unstaking for user not started");
        require(user.rewardStakingBlockNFT[0] >= block.number, "Unstaking for user not started");

        _updatePool();

       // for (uint256 i = 0; i <= user.noOfDeposit)
        ...
        user.lastStakingBlockLFW[0] = user.lastStakingBlockLFW[user.lastStakingBlockLFW.length - 1];
        user.lastStakingBlockLFW.pop();
        user.rewardStakingBlockLFW[0] = user.rewardStakingBlockLFW[user.rewardStakingBlockLFW.length - 1];
        user.rewardStakingBlockLFW.pop();
        //
        user.lastStakingBlockNFT[0] = user.lastStakingBlockNFT[user.lastStakingBlockNFT.length - 1];
        user.lastStakingBlockNFT.pop();
        user.rewardStakingBlockNFT[0] = user.rewardStakingBlockNFT[user.rewardStakingBlockNFT.length - 1];
        user.rewardStakingBlockNFT.pop();
        // Update number of claim if transfer successful
        user.noOfClaimLFW -= 1;
    }
  ...
  //      require(block.number < unStakingBlock, "Unstaking has started");
    ...
    function claimNFT(address nftAddress) external validNFTContractAddress(nftAddress) {
        ...
        require(user.noOfClaimLFW > 0, "User does not stake anything");
        require(user.noOfClaimNFT > 0, "User does not stake anything");
        require(user.amount >= busdIn, "Address does not stake enough BUSD");
        require(user.amountChest >= depositChestMax, "Address does not stake enough Chest");
        require(user.rewardStakingBlockLFW[0] > block.number, "Unstaking for user not started");
        require(user.rewardStakingBlockNFT[0] > block.number, "Unstaking for user not started");
    ...
    function getNoOfDepositBUSD(address _usr) 
        external
        view
        returns (uint256)
    {
    ...
    function getNoOfClaimLFW(address _usr) 
        external
        view
        returns (uint256)
    {
        UserInfo storage user = userInfo[_usr];
        return (user.noOfClaimLFW);   
    }

    function getNoOfClaimNFT(address _usr) 
        external
        view
        returns (uint256)
    {
        UserInfo storage user = userInfo[_usr];
        return (user.noOfClaimNFT);   
    }
