...
function withdrawAll() public {	// TBA remove in production, this is to capture testbnb back, getting testbnb is tough lol
	...
	function setPromoMultiplier(address referrer) public { 
		...
		if(totalBonus >= 1 &&  tokenBalanceFactor >= 1) {user.promoMultiplier=50;} 
			...
			if(totalBonus >= 10 && tokenBalanceFactor >= 10) {user.promoMultiplier=70;}
			if(totalBonus >= 20 && tokenBalanceFactor >= 20) {user.promoMultiplier=80;}
	...
	function resetDataForTesting() public {
		for (uint256 i = 1; i <= 20; i++) {
			address userAddress; 
			(userAddress,) = getterIdToAddr(i);
	...
	function addMyLead(address investor, address referrer) public  {
		...
		uint256 secPassed = now.sub(user.depositTime);
		if (secPassed > 0 && user.depositTime > 0) {
            uint256 calculateProfit = (user.bnbDeposit.mul(secPassed.mul(116000))).div(1000000000000);
	...
	function invest(address investor, address referrer) public payable {
		...
		if (getChainID() == 97) { // TBA testnet // remove in prod
				if(buyBackAmount > MIN_BUYBACK_AMOUNT ) {
					// buy back possibility upon new deposits
					if (buyBackTriggerTime < block.timestamp) { 
						tokenBurn();
	...
	function withdraw(uint256 reinvestOption, uint256 wPercent) public {
		...
		uint256 totalAmount = user.profitGained;
		user.profitGained = 0;
		...
		//TBA temporarily disabled this penalty
	...
	function getDailyProfitsAvailable(address userAddress) public view returns (uint256) {
		...
		if(user.depositTime > 0){
			...
			if (secPassed > 0) {
				calculateProfit = (user.bnbDeposit.mul(secPassed.mul(116000))).div(1000000000000);
			}
			if (calculateProfit.add(user.profitGained) >= user.bnbDeposit.mul(3)){
				return user.bnbDeposit.mul(3);
			}
			else{
				return calculateProfit.add(user.profitGained);
			}
		} else {
			return 0;
		}
	...
	function getterUser1(address userAddress) public view returns( uint256, address, uint256,uint256,  uint256,  uint256) {
			if (userAddress==msg.sender) {
				return (
				users[userAddress].depositTime, //TBA in place of checkpoint
	...
	function getterUser2(address userAddress) public view returns(uint256, uint256,  uint256,  uint256, uint256,uint256, uint256) {
			if (userAddress==msg.sender) {
				...
				0, // TBA we do not have deposits array now
	...
	/* // TBA commented this to do testing for referral bonus, reset data, later uncomment in prod, and remove that resetdata test function in prod
