...
contract MarketPlace is MarketAuth, PausableUpgradeable {
  ...
  function takeOrder(OrderKind _kind, uint _tokenId, PaymentCurrency _paymentCurrency) external whenNotPaused {
    ...
    require(!locked[_tokenId], "MarketPlace: locked");
    ...
    emit OrderTaken(_kind, orderType, _tokenId, msg.sender, paymentType, currentPrice);
  }
  ...
  function getCurrentPrice(OrderKind _kind, uint _tokenId) external view returns(uint) {
    Order storage tradingOrder = tradingOrders[_kind][_tokenId];
    if (_isActive(tradingOrder)) {
  ...
  function _getCurrentPrice(Order storage _order) private view returns(uint) {
    ...
    return paymentType == PaymentType.fota ? currentPrice * decimal3 / fotaPrice : currentPrice;
  }
  ...
  function _marketMakeOrder(
    ...
    _type == OrderType.trading ? tradingOrders[_kind][_tokenId] = order : rentingOrders[_kind][_tokenId] = order;
    emit OrderCreated(_type, _kind, _tokenId, address(this), _startPrice, _endingPrice, _duration);
  }
  ...
  function listHeroes(
    ...
    for(uint i = 0; i < _quantity; i++) {
      uint _tokenId = nftTokens[OrderKind.hero].mintHero(address(this), _gene, _class, _price, i);
      _marketMakeOrder(_type, OrderKind.hero, _tokenId, _price, _price, 365 days);
    }
  }
  ...
  function listItems(
    ...
    for(uint i = 0; i < _quantity; i++) {
      uint _tokenId = nftTokens[OrderKind.item].mintItem(address(this), _gene, _class, _price, i);
      _marketMakeOrder(_type, OrderKind.item, _tokenId, _price, _price, 365 days);
    }
  }
  ...
  function updateLockStatus(uint _tokenId, bool _locked) external onlyMainAdmin {
    locked[_tokenId] = _locked;
    emit NFTLocked(_tokenId, _locked, block.timestamp);
  }
