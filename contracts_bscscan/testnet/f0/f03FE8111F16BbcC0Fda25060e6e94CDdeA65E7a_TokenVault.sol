...
contract TokenVault is AccessControlEnumerable, ITokenVault {
    ...
    modifier onlyFarm() {
        require(hasRole(FARM_ROLE, _msgSender()), "WanaVault: Not Farm");
        _;
    }

    constructor(address _tokenAddress, address _farm) {
    ...
    // Safe tokenAddress transfer function, just in case if rounding error causes pool to not have enough WANAs.
    function safeTokenTransfer(address _to, uint256 _amount) external override onlyFarm {
    ...
    function rescueFunds(
        ...
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "WanaVault: You are not admin role");
