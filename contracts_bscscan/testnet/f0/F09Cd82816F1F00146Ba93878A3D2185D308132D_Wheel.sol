...
import "./libs/RandomizerReceiver.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/SafeERC20.sol";
import "@openzeppelin/contracts/math/SafeMath.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract Wheel is GambleBase, Ownable, RandomizerReceiver {
    ...
    event GameRequest(address indexed account, bytes32 gameId);
    event Result(address indexed account, bytes32 indexed gameId, uint256 spin, uint256 multiplier, uint256 reward);

    constructor(IERC20 _token, address _oracle) public RandomizerReceiver(_oracle) {
        token = _token;
    }
    ...
    function _spin(uint256 seed) public view returns (uint256) {
        return rand(seed).mul(52).div(1000);
    }
    ...
    function bet(uint256 _1, uint256 _3, uint256 _5, uint256 _10, uint256 _20, uint256 _50) public payable returns (bytes32) {
        ...
        bytes32 gameId = keccak256(abi.encodePacked(gameCount));
        uint256 request = requestRandom(gameId);
        ...
        emit GameRequest(msg.sender, gameId);

        return gameId;
    }

    event Debug(Game game, bytes32 ref);

    function receiveRandom(uint256 seed, bytes32 ref) external override virtual onlyOracle {
        Game storage game = games[ref];
        emit Debug(game,ref);
        return;
        game.seed = seed;

        // Spin it!
        game.spin = _spin(seed);
        game.multiplier = wheel[game.spin];
        ...
        if (game.multiplier > 0) {
            ...
            game.reward = winningBet.mul(game.multiplier.add(1));
        }

        uint256 betTotal = game._1+game._3+game._5+game._10+game._20+game._50;
        if (betTotal > game.reward && burnPerc > 0) {
            uint256 burnAmount = betTotal.sub(game.reward).mul(burnPerc).div(10000);
            if (burnAmount > 0)
                token.safeTransfer(burnAddress, burnAmount);
        }
        
        if (game.reward > 0)
            token.safeTransfer(game.account, game.reward);

        lastResult[game.account] = BetResult({
            spin: game.spin,
            multiplier: game.multiplier,
            reward: game.reward
        });
        emit Result(game.account, ref, game.spin, game.multiplier, game.reward);
    }
}
