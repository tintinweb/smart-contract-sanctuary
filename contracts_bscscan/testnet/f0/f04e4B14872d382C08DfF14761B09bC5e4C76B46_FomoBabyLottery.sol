...
interface IPancakeSwapLottery {
     ...
     * @param _priceTicketInEth: price of a ticket in BNB
...
// File: contracts/FomoBabyLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title $FOMOBABY Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract FomoBabyLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInEth = 5 ether;
    uint256 public minPriceTicketInEth = 1 ether;

    uint256 public pendingInjectionNextLottery;

    uint256 public constant MIN_DISCOUNT_DIVISOR = 300;
    uint256 public constant MIN_LENGTH_LOTTERY = 12 hours - 5 minutes; // 12 hours
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInEth;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] ethPerBracket;
     ...
     * @param _ethTokenAddress: address of the BNB token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _ethTokenAddress, address _randomGeneratorAddress) {
        ...
        nonReentrant
    {
        ...
        uint256 amountEthToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInEth,
            _ticketNumbers.length
        );

        // Transfer BNB tokens to this contract
        ethToken.safeTransferFrom(address(msg.sender), address(this), amountEthToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInEth += amountEthToTransfer;
    ...
    function claimTickets(
        ...
        ethToken.safeTransfer(msg.sender, rewardInEthToTransfer);

        emit TicketsClaim(msg.sender, rewardInEthToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInEth) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;
        ...
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].ethPerBracket[j] =
                ...
                _lotteries[_lotteryId].ethPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInEth - amountToShareToWinners);

        // Transfer BNB to treasury address
        ethToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        ethToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInEth += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInEth >= minPriceTicketInEth) && (_priceTicketInEth <= maxPriceTicketInEth),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            ethPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInEth: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(ethToken), "Cannot be BNB token");
     ...
     * @dev Only callable by owner
     * @param _minPriceTicketInEth: minimum price of a ticket in BNB
     * @param _maxPriceTicketInEth: maximum price of a ticket in BNB
     */
    function setMinAndMaxTicketPriceInEth(uint256 _minPriceTicketInEth, uint256 _maxPriceTicketInEth)
        external
        onlyOwner
    {
        require(_minPriceTicketInEth <= _maxPriceTicketInEth, "minPrice must be < maxPrice");

        minPriceTicketInEth = _minPriceTicketInEth;
        maxPriceTicketInEth = _maxPriceTicketInEth;
    }
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].ethPerBracket[_bracket];
        } else {
            return 0;
        }
    }
