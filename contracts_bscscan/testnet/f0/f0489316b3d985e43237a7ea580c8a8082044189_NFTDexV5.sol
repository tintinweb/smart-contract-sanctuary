...
//   event SetValue(address indexed from, uint256 main, uint256 _author, uint256 _platform);
  ...
  function buyNFT(address nft_a,uint256 tokenId, address buyer) public{
        ...
        if(isArtistAuthor){
          ...
          if(revenueShare.checkArtistOwner(_nonCryptoOwners[tokenId], tokenId)){
           ...
           ALIA.transferFrom(buyer, platform, (_artistPrice * platformPerecentage) / 100);
          }
          ALIA.transferFrom(buyer, aliaHolder, (_artistPrice * 975)/1000);
        ...
        _transferFrom(address(this), buyer, tokenId);
         ...
         if(!isArtistAuthor) _nonCryptoWallet[_nonCryptoOwners[tokenId]] += (price / 1000) * mainPerecentage;
  ...
  function buyNftNonCrypto(address  nft_a, uint256 tokenId, string memory ownerId) public {
    ...
    //       (uint112 _reserve0, uint112 _reserve1,) =LPBNB.getReserves();
    //        (uint112 reserve0, uint112 reserve1,) =LPAlia.getReserves();
    //        tempPrice = SafeMath.div(SafeMath.mul(SafeMath.mul(_dollarPrice[tokenId],reserve0), _reserve0),SafeMath.mul(_reserve1,reserve1));
    //     }
