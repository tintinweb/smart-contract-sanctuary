...
contract UmblNFT is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {    
    ...
    uint public constant MAX_CRATE_TOKENS = 100;
    ...
    */
    ...
    function mintToken(uint256 _numTokens, uint256 _category, uint256 _rarity, uint256 _price) 
        public 
        onlyOwner 
        nonReentrant
    {
        require(_numTokens > 0 && _numTokens <= MAX_MINTING_TOKENS, "Must mint from 1 to 100000 NFTs");

        require(_category >= uint256(TokenCategory.WEAPONS) && _category <= uint256(TokenCategory.BADGES), "Must category from WEAPONS to BADGES");

        require(_rarity >= uint256(TokenRarity.COMMON) && _rarity <= uint256(TokenRarity.BLACK_DIAMOND), "Must rarity from COMMON to BLACK_DIAMOND");
        ...
        require(_price > 0, "Must price is greater than zero");
    ...
    // Mint crates
    function mintCrate(uint256 _numTokens, uint256[] memory _rarities, uint256 _price) 
        public
        onlyOwner
        nonReentrant
    {
        require(_numTokens > 0 && _numTokens <= MAX_CRATE_TOKENS, "Must include from 1 to 100 tokens");
        ...
        // create a new token struct and pass it new values
        UmblCrate memory newUmblCrate = UmblCrate(
            nextCrateId,
            _numTokens,
            new TokenRarity[](0),
            _price            
        ...
        // add the token id and it's struct to all tokens mapping
        crateUmblData.push(newUmblCrate);
    ...
    function getCrateData(uint i) 
        public 
        view 
        returns (uint256[] memory)
    {
        uint256[] memory result = new uint256[](crateUmblData[i].rarities.length);

        for(uint j=0; j<crateUmblData[i].rarities.length; j++) {
            result[j] = uint256(crateUmblData[i].rarities[j]);
        }
        return result;
    }
    ...
    // by a crate
    function buyCrate(uint256 _crateId, uint256[] memory _tokens)
        public payable
    {
        ...
        // check passed token count is equal to crate's token count
        require(crateData.tokenCount == _tokens.length);
