...
// File: DragonSlayer.sol

pragma solidity ^0.7.6;

contract DragonSlayer is
    ERC20,
    Ownable,
    ReentrancyGuard
{
    ...
    IManagableObject public manager;
    IUniswapV2Router02 public uniswapV2Router;

    address public uniswapV2Pair;
    address public addressForMarketing;
    
    uint256 public maxSupply    = 1000000000 * 10 ** 18; // Total supply: 1000000000
    uint256 public maxMarketing = 30000000   * 10 ** 18; // 3% for marketing

    // Tokens for rewards
    uint256 public maxRewards   = 200000000  * 10 ** 18; // 20% for rewards
    
    // Tokens for airdrop
    uint256 public maxAirdrop   = 20000000   * 10 ** 18; // 2% for airdrop
    uint256 public availableAirdrop;
    ...
    constructor() ERC20("Dragon Slayer", "DRS") {
    ...
    function setManager(address _manager) public onlyOwner {
        manager = IManagableObject(_manager);
    }
        ...
        onlyOwner
    {
        ...
        for (uint256 index = 0; index < to.length; index++) {
            airdropWhitelist[to[index]] = amount[index];
        }
    }
    ...
    function sweepTokenForMarketing() public onlyOwner {
        uint256 contractTokenBalance = balanceOf(address(this));
        if (contractTokenBalance >= maxMarketing) {
            swapTokensForEth(maxMarketing);
        }
    }
     ...
     */
    function rewards(address recipient, uint256 amount) external onlyOwner {
        require(recipient != address(0), "0x is not accepted here");
        require(maxRewards > 0, "Rewards not available");
        require(amount > 0, "not accept 0 value");

        if (maxRewards >= amount) {
            _mint(recipient, amount);
            maxRewards = maxRewards.sub(amount);
        } else {
            _mint(recipient, maxRewards);
            maxRewards = 0;
        }
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (
            ...
            uint256 _feeSplited = _fee.div(2);

            super._transfer(sender, address(this), _feeSplited);
            super._transfer(sender, addressForMarketing, _feeSplited);

            amount = amount.sub(_fee);
            maxRewards = maxRewards.add(_feeSplited);
        }
