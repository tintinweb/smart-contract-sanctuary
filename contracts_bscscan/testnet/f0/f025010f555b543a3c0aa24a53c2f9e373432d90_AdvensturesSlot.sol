...
contract AdvensturesSlot is Context, PartnerOwnable { 
    ...
    event bet_out(bytes result,address sender, uint256 tot_bet, uint256 win, uint256 loss, WINS wins);
    ...
    function tokens_create(address _tokens_address) public onlyPartner {   
    ...
    function tokens_set(uint256 _tokensId, address _tokens_address) public onlyPartner {   
	...
	constructor()   { 
        ...
        tokens_create(0x4ECfb95896660aa7F54003e967E7b283441a2b0A); // FRUIT
        tokens_create(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82); // CAKE
    ...
    function bet(uint[] calldata bets,uint domainId, uint tokensId) external  
        ...
        if(amount > 0) {
            ...
            emit bet_out('loss',msg.sender, tot_bet, 0, amount, wins);
        } else if(amount < 0) {
            //win
            amount = 0 - amount;
            TransferHelper.safeTransfer(FRUIT, msg.sender, amount); 
            emit bet_out('win',msg.sender, tot_bet, amount, 0, wins);
        } else {
            emit bet_out('tie',msg.sender, tot_bet, 0, 0, wins);
        }
        _wins = wins; 
