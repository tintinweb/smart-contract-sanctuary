...
import "../libs/PairOracle.sol";

contract LtyBusdPairOracle is Ownable, IPairOracle, PairOracle {
    /**
     * @dev Initializes contract setting up liquidity pool pair's address. Also, sets up last cumulative prices.
     *
     * Reverts if at least one of the liquidiy pool pair reserves is empty.
     */
    constructor(address pairAddress)PairOracle(pairAddress) {}
}
...
abstract contract PairOracle is Ownable, IPairOracle {
  ...
  /**
    * @dev Sets new period which specifies time interval between `update` calls. Can be called only by the owner.
    */
  ...
  /**
    * @dev Updates average and cumulative prices.
    * Reverts if `PERIOD` seconds not elapsed yet since last update.
    */
  ...
  /**
    * @dev Returns the price of the transferred token to the second token in the pair.
    * Note, this will always return 0 before update has been called successfully for the first time.
    * @param token The address of one of the two tokens of the pair.
    * @param amountIn Number of tokens for which the price will be returned.
  ...
  /**
    * @dev Returns `block.timestamp` as `uint32`.
    */
