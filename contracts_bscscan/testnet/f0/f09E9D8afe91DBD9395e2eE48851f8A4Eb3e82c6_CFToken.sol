...
/// @title CFToken
///
/// @dev This is the contract for the Carbon utillity token usd.
///
...
contract CFToken is ERC20Upgradeable,AccessControlUpgradeable{  
  ...
  event Paused(address carbonFinanceAddress, bool isPaused);
  
  function initialize() public  initializer {
     __ERC20_init("Carbon", "cbUSD");
  ...
  modifier onlyWhitelisted() {
    require(whiteList[msg.sender], "cbUSD: CarbonFinance is not whitelisted");
    _;
  }
  ...
  function mint(address _recipient, uint256 _amount) external onlyWhitelisted {
    require(!blacklist[msg.sender], "cbUSD: CarbonFinance is blacklisted.");
    uint256 _total = _amount.add(hasMinted[msg.sender]);
    require(_total <= ceiling[msg.sender],"cbUSD: CarbonFinance's ceiling was breached.");
    require(!paused[msg.sender], "cbUSD: user is currently paused.");
  ...
  function setBlacklist(address _toBlacklist,bool _state) external onlySentinel {
    blacklist[_toBlacklist] = _state;
  }
  /// This function reverts if the caller does not have the admin role.
  function pauseCarbonFinance(address _toPause, bool _state) external onlySentinel {
