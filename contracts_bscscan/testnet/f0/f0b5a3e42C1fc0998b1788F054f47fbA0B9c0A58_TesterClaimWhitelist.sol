...
contract TesterClaimWhitelist is AccessControl {
    ...
    function addWhiteList(address[] memory _address) public {
        ...
        for(uint256 i = 0; i < _address.length; i++){
            whitelistAddress[address(_address[i])] = true;
        }
    }
    function setTimeOpen(uint256 _start) public {
        require(hasRole(CREATOR_ADMIN, msg.sender), "Caller is not a admin");
    ...
    function claimBusd() public {
        ...
        require(!claimed[msg.sender],"You have claimed tokens");
        require(block.timestamp >= start, "Please wait until opening time");
