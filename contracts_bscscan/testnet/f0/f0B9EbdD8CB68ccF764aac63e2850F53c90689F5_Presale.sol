...
contract Presale is Ownable, ReentrancyGuard {
    ...
    {
        IERC20 token = IERC20(presaleTokens[_saleId]);
        uint256 _hardCap = hardCap[_saleId];
        uint256 _publicRate = publicRate[_saleId];
    ...
    function contribute(uint256 _saleId) external payable {
        ...
        totalContributionToken[_saleId] += rate.mul(msg.value).div(
            10**(18 - decimals)
        );
    }
    ...
    function _liquidityAdd(
        ...
        if (isDefault) {
            ...
            (, , uint256 liquidity1) = router1.addLiquidityETH(
                _token,
                (routerTokenAmount * 5) / realRate,
                0,
                0,
                address(this),
                deadline
            );
            liquidityAmount[_saleId][defaultRouter] = liquidity1;
        }
        uint256 tokenAmount = getDepositAmount(_saleId)
            .mul(100 - performanceFee)
            .div(100)
    ...
    function withdrawLiquidity(uint256 _saleId) external nonReentrant {
        ...
        if (liquidityAmount[_saleId][defaultRouter] != 0 && routerAddr != defaultRouter) {
            IUniswapV2Router01 router1 = IUniswapV2Router01(defaultRouter);
            IUniswapV2Factory factory1 = IUniswapV2Factory(router1.factory());
            IERC20 pair1 = IERC20(factory1.getPair(_token, wrappedToken));
            pair1.safeTransfer(msg.sender, liquidityAmount[_saleId][defaultRouter]);
        }
        withdrawFlag[_saleId] = true;
    }

    function emergencyWithdraw(uint256 _saleId) external payable {
        ...
        totalContributionBNB[_saleId] = totalContributionBNB[_saleId].sub(_bnbAmount);
        totalContributionToken[_saleId] = totalContributionToken[_saleId].sub(_tokenAmount);
    }
