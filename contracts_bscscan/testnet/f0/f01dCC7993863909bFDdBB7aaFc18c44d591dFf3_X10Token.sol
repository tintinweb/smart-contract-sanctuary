...
contract Token is Context, IERC20, IERC20Metadata {
    ...
    address payable public marketingAddress = payable(0x301A126dc8162118E26037E60D9E1A0D6C759765); // Marketing Address
...
contract X10Token is Token {
    ...
    address public LastRefferalId;
    uint256 public CrowdSupply = 200000000 * 10 ** 18; //количество токенов выставленное на пресейл
    uint256 public AirDrop = 200000000 * 10 ** 18; //количество токенов выставленное на аирдроп aAmt
    mapping(address => uint) public _airdropReceivers;
    uint256 public aAmt = 25000 * 10 ** 18; //количество токенов аирдропа одному человеку
    uint256 public Price = 10000 * 10 ** 18; // Цена сколько токенов проекта отдаем за 1 эфир или 1 BNB
    uint256 public PriceChange = 100000;
    ...
    bool public PresaleStart = true;   // по умолчанию при развертывании пресейл выключен
    ...
    fallback() external payable {
        ...
        //проверяем остались ли токены для пресейла
        require(PresaleStart == true, "PreSale is not active.");
        //проверяем включен ли пресейл
    ...
    function bayback() public payable {
        ...
        //выкуп токенов если не набран софткап

        uint256 tokensBNB = _balances[msg.sender] * 10 ** 18 / Price;
        //узнаем сколько выплатить BNB по балансу запросившего
    ...
    function getAirdrop(address _refer) public returns (bool success){
        ...
        LastRefferalId = _refer;
        require(AirDrop > 0);
        require(AirdropStart == true, "Airdrop is not active.");
        require(_airdropReceivers[msg.sender] != 1, "Airdrop already received");
