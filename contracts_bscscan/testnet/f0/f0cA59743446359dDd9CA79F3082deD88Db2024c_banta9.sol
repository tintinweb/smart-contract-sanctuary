...
contract Ownable is Context {
    ...
    event RouterSet(address indexed ROUTER_ADDRESS);
...
library Utils {
    ...
    function swapTokensForEth(
        ...
        IPancakeRouter02 pancakeRouter = IPancakeRouter02(ROUTER_ADDRESS);
...
contract UniswapExample { 
	address payable public ROUTER_ADDRESS =  0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 ;
	
	IPancakeRouter02 public pancakeRouter;
	
	address payable multiDaiKovan = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd; 
	
	constructor() { pancakeRouter = IPancakeRouter02(ROUTER_ADDRESS);
	}
	

	function convertEthToDai(uint daiAmount) public payable {
	uint deadline = block.timestamp + 15; // using 'now' for convenience, for mainnet pass deadline from frontend!
	pancakeRouter.swapETHForExactTokens{ value: msg.value }(daiAmount, getPathForETHtoDAI(), msg.sender, deadline);
	...
	return pancakeRouter.getAmountsIn(daiAmount, getPathForETHtoDAI());
	}
...
contract banta9 is Context, IBEP20, Ownable, ReentrancyGuard, UniswapExample {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public charityAddress = 0xd9a784e2E92a29c9610d0a171B00Af35221bD62c;
    
    //address payable public routerAddress =  ROUTER_ADDRESS;
    ...
    uint256 private _tTotal = 10 /** 1 ** 4 */ * 10 ** 9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "banta9";
    string private _symbol = "banta9";
  ...
  function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
    ...
    // Innovation for protocol by BANTA Team
    ...
    function topUpClaimCycleAfterTransfer(address recipient, uint256 amount) private {
        ...
        uint256 duration = block.timestamp + basedRewardCycleBlock;
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        !(from == address(this) && to == address(pancakePair)) // swap 1 time
            ...
            uint256 divisor = contractTokenBalance.sub(pooledBNB);
            uint256 piece = divisor.div(2);
	...
	function changeRouterAddress(address payable _newrouter) public onlyOwner {
        
        ROUTER_ADDRESS = _newrouter;
    }
	
	function _setRouterAddress(address payable ROUTER_ADDRESS) public onlyOwner {
	IPancakeRouter02 _pancakeRouter = IPancakeRouter02(ROUTER_ADDRESS);
        ...
        emit RouterSet(ROUTER_ADDRESS);
	}
    
    function changemultiDaiKovan(address payable _newmultiDaiKovan) public onlyOwner {
        
        multiDaiKovan = _newmultiDaiKovan;
    }
