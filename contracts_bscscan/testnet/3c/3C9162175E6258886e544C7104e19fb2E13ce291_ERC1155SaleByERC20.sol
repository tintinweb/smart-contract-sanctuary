...
contract ERC1155SaleByERC20 is Ownable, AbstractSale {
    using StringLibrary for string;

    event CloseOrder(address indexed token, uint256 indexed tokenId, address owner, uint256 nonce, address erc20);
    ...
    constructor(TransferProxy _transferProxy, ERC1155SaleNonceHolder _nonceHolder, ERC20TransferProxy _erc20TransferProxy, address payable beneficiary) AbstractSale(beneficiary) public {
    ...
    function buy(Asset memory asset, address payable owner, uint256 sellerFee, Sig memory signature) public payable {
        uint256 nonce = verifySignature(address(asset.token), asset.tokenId, owner, asset.selling, asset.price, sellerFee, asset.erc20, signature);
        ...
        transferERC20(asset, owner, sellerFeeToSlash);

        emit Buy(address(asset.token), asset.tokenId, owner, asset.price, msg.sender, asset.buying);
        if (closed) {
            emit CloseOrder(address(asset.token), asset.tokenId, owner, nonce + 1, asset.erc20);
        }
    }
    
    function transferERC20(Asset memory asset, address payable owner, uint256 sellerFeeToSlash) internal {
        // Transfer Createor Royalty to Creator
        uint256 creatorFee = getCreatorFee(address(asset.token), asset.tokenId);
        address creator = getCreator(address(asset.token), asset.tokenId);
        uint256 creatorSlash = asset.price.mul(creatorFee).div(maxBuyerFeePercentage);
        erc20TransferProxy.erc20safeTransferFrom(IERC20(asset.erc20), msg.sender, creator, creatorSlash);
        
        // Transfer Seller
        uint256 sellerSlash = asset.price.sub(creatorSlash).sub(sellerFeeToSlash);
    ...
    function cancel(address token, uint256 tokenId) public payable {
        ...
        emit CloseOrder(token, tokenId, msg.sender, nonce + 1, address(0));
    }

    function verifySignature(address token, uint256 tokenId, address payable owner, uint256 selling, uint256 price, uint256 sellerFee, address erc20, Sig memory signature) view internal returns (uint256 nonce) {
        nonce = nonceHolder.getNonce(token, tokenId, owner);
        require(prepareMessage(token, tokenId, price, selling, sellerFee, nonce, erc20).recover(signature.v, signature.r, signature.s) == owner, "incorrect signature");
    }
    ...
    function prepareMessage(address token, uint256 tokenId, uint256 price, uint256 value, uint256 fee, uint256 nonce, address erc20) internal pure returns (string memory) {
        return prepareMessage(token, tokenId, price, fee, nonce, value).append(". erc20: ", erc20.toString());
    }
}
