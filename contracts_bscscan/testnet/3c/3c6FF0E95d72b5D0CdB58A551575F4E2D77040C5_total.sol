...
contract total{
    ...
    function reverseBusdPool0(address account) view public returns (uint256){
        (address tokenAddr, , , , , , , ,) = kingDeFiFarm.poolInfo(2);
        (uint256 amount, ) = kingDeFiFarm.userInfo(2, account);
        IPancakeLPToken tokenLP = IPancakeLPToken(tokenAddr);
    ...
    function tvlBUSD(address account) view public returns (uint256) {
        return (prezzoKrown() * reverseBusdPool0(account) / 10**18) + (prezzoKrown() * reverseBusdPool0(account) / 10**18);
    }
    
    function totalDeposit(address account) view public returns(uint256){
        uint256 tt;
        (uint256 amount, ) = kingDeFiFarm.userInfo(1, account);
        for(uint256 i = 0; i < kingDeFiFarm.poolLength(); i++){
            if(i == 1){
                tt = tt + ((amount * prezzoKrown()) / 10**18);
            } 
            if(i == 2){
                tt = tt + tvlBUSD(account);
            }
            if(i != 1 && i != 2 && i != 6){
                tt = tt + tvl(i, account);
            }
    ...
    function calculatePending(uint256 _pid, address account) view public returns(uint256){
        ( , , , , ,uint256 totalSupply ,uint256 krwPerBlock ,uint256 lastRewardBlock ,uint256 rewardsPerShare) = kingDeFiFarm.poolInfo(_pid);
        (uint256 amount, uint256 rewardMinus) = kingDeFiFarm.userInfo(_pid, account);
        uint256 rewardToDeliver;
        rewardToDeliver = krwPerBlock * (block.number - lastRewardBlock);
        rewardsPerShare = rewardsPerShare + ((rewardToDeliver * 1e18) / totalSupply);
        return ((amount * rewardsPerShare) / 10**18) - rewardMinus;
    }
}