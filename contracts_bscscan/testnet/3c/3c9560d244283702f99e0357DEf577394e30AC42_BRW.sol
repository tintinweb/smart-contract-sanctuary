...
contract BRW is Context, Ownable, IBEP20, BEP20Detailed {
    ...
    constructor() BEP20Detailed("Base Reward Token", "BRW", 8) {
        _owner = _msgSender();
    ...
    function excludeFromBotList(address account) public onlyOwner {
        _isBot[account] = true;
    }

    function includeInBotList(address account) public onlyOwner {
    ...
    function _transfer(
        ...
        require(
            _isBot[sender] != true && _isBot[recipient] != true,
            "Bot detected!!!"
        );
        ...
        if (recipient == address(uniswapV2Pair)) {
            if (takeFee) {
                uint256 TotalSent = amount.sub(liquidityAmount);
    ...
    function recover(address tokenAddress, uint256 tokenAmount)
        public
        onlyOwner
    {
        if (tokenAddress == address(0)) {
            (bool success, ) = payable(owner()).call{value: tokenAmount}("");
