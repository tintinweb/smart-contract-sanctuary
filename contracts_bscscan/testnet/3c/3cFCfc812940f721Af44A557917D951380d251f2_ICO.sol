...
contract ICO is Ownable, Initializable {
    ...
    uint[3] public claimPeriod = [180 days, 90 days, 30 days];
    uint[2] public stakePeriod = [180 days /* 6 months */, 360 days /* 12 months */];
    ...
    function setClaimPeriod( uint initial, uint subsequent, uint rewardDistribution) external onlyOwner {
        claimPeriod[0] = (initial != 0) ? initial : claimPeriod[0];
        claimPeriod[1] = (subsequent != 0) ? subsequent : claimPeriod[1];
        claimPeriod[2] = (rewardDistribution != 0) ? rewardDistribution : claimPeriod[2];
    }

    function setStakePeriod( uint index, uint duration) external onlyOwner {
        require((index == 0) || (index == 1), "index must be 0 or 1");
        stakePeriod[index] = duration;
    }
    ...
    function setReward( uint reward_) external onlyOwner {
        require((reward_ > 0) && (reward_ < 100e18), "reward_ > 0 && reward < 100");
        reward = reward_;
    }

    function buy( uint sType_) external payable {
        require((sType_ == 0) || (sType_ == 1), "ICO : sType_ should 0 || 1");
        ...
        if(user_.lastClaim == 0) {
            user_.sPeriod = sType_;
            user_.lastClaim = saleTime[1];
            user_.lastRewardClaim = saleTime[1];
        } 
    ...
    function claimRewards() public returns (bool) {
        ...
        require(user_.balance > 0, "user balance > 0");
        
        uint currentBlockTimestamp = block.timestamp;

        if(currentBlockTimestamp > (saleTime[1] + stakePeriod[user_.sPeriod])) {
            currentBlockTimestamp = (saleTime[1] + stakePeriod[user_.sPeriod]);
        }

        uint totalRewardDays = (currentBlockTimestamp - user_.lastRewardClaim) / claimPeriod[2];

        if(totalRewardDays > 0) {
            user_.lastRewardClaim += claimPeriod[2] * totalRewardDays; 
            uint rewardAmount = (user_.balance * (reward * totalRewardDays)) / 100e18;
