...
contract NFTMasterV2 is Ownable, IERC721Receiver {
    ...
    //    IERC20 public baseToken; // 0xe9e7cea3dedca5984780bafc599bd69add087d56（BUSD）
    //    IERC20 public blesToken; // 0x393d87e44c7b1f5ba521b351532c24ece253b849（BLES）
    ...
    // 未发布的集合
    //    function collectionsUnpublished() public view returns (Collection[] memory){
    //        Collection[] memory cList = new Collection[](nextCollectionId);
    //        uint256 count = 0;
    //        for (uint256 i = 0; i < nextCollectionId; i++) {
    //            if (allCollections[i + 1].publishedAt == 0) {
    //                cList[i] = allCollections[i + 1];
    //                count++;
    //            }
    //        }
    //        if (count > 0) {
    //            Collection[] memory cList2 = new Collection[](count);
    //            for (uint256 j = 0; j < count; j++) {
    //                cList2[j] = cList[j];
    //            }
    //            return cList2;
    ...
    //
    //    // 已发布的集合（未售完）
    //    function collectionsPublished() public view returns (Collection[] memory){
    //        Collection[] memory cList = new Collection[](nextCollectionId);
    //        uint256 count = 0;
    //        for (uint256 i = 0; i < nextCollectionId; i++) {
    //            if (allCollections[i + 1].publishedAt != 0) {
    //                if (allCollections[i + 1].size != allCollections[i + 1].soldCount) {
    //                    cList[i] = allCollections[i + 1];
    //                    count++;
    //                }
    //            }
    ...
    //
    //    // 已售完的集合
    //    function collectionsPast() public view returns (Collection[] memory){
    //        Collection[] memory cList = new Collection[](nextCollectionId);
    //        uint256 count = 0;
    //        for (uint256 i = 0; i < nextCollectionId; i++) {
    //            //            if (allCollections[i + 1].publishedAt != 0) {
...
//            require(msg.value == cost, string(abi.encodePacked(('not enough token '), uint2str(cost))));

        } else {
            require(IERC20(collection.coin).allowance(_msgSender(), address(this)) >= cost, 'not enough erc20 token');
//            require(IERC20(collection.coin).allowance(_msgSender(), address(this)) >= cost, string(abi.encodePacked(('not enough token '), uint2str(cost))));
