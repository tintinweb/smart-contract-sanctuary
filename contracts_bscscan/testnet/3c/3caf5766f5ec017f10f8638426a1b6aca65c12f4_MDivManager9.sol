...
contract MDivManager9 is RewManagerInterface, Ownable {
  using SafeMath for uint256;
  using SafeMathUint for uint256;
  using SafeMathInt for int256;

  // The currency in which we pays dividends.
  RewDoge public REWDOGE = RewDoge(0xaFbe9C9A2edc98cE96C896696F332e1C848b6D18);
  ...
  event MDIV_BalanceAtWithdraw(uint256 balance);
  event MDIV_RewTransferComplete(address _user, uint256 _withdrawableDividend);
  event MDIV_RewTransferExceptionReason(address _user, uint256 _withdrawableDividend, string reason);
  event MDIV_RewTransferExceptionNoReason(address _user, uint256 _withdrawableDividend);
 ...
 function _withdrawDividendOfUser(IERC20 _client, address payable _user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      emit MDIV_BalanceAtWithdraw(REWDOGE.balanceOf(address(this)));
      
      bool success = false;
      try REWDOGE.transfer(_user, _withdrawableDividend) returns (bool tx_success) {
          success = tx_success;
          emit MDIV_RewTransferComplete(_user, _withdrawableDividend);
      } catch Error(string memory reason) {
          emit MDIV_RewTransferExceptionReason(_user, _withdrawableDividend, reason);
      } catch {
          emit MDIV_RewTransferExceptionNoReason(_user, _withdrawableDividend);
      }
