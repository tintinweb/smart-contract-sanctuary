...
contract TokenServiceExample is Context, IERC20, Ownable {
    ...
    string private _name = "Token Service Example";
    string private _symbol = "TSEK";
    ...
    address payable sustainableAddress = 0x79613956Bb34406e08F1EF4a54bCF91a13266B58;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(sustainableAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(sustainableAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(sustainableAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(sustainableAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(sustainableAmt)));
        }
    ...
    function setSustainableWallet(address payable newSustainable) external onlyOwner(){
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        require(maxTxPercent <=  5, "Cannot set transaction amount higher than 5 percent!");
    ...
    //checks for bots and declines their transactions
