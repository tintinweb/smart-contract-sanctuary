...
contract Momcoin is Context, IERC20, Ownable, Pausable 
{
    ...
    address payable public charityWallet =  payable(0x0572689b9Cb91789325C32E5f4b3d2b6A4e7D526);
    address payable public marketingWallet =  payable(0xdb995a5e8564Af80C411374fAf951cE267558DAA);
    ...
    uint256 private swapThreshold = _tTotal.div(1000_000);
    uint256 public maxLimit = _tTotal.div(50); //2%;
    ...
    function setAllFees(uint256 taxFee, uint256 liquidityFee, uint256 charityFee) 
    external onlyOwner() 
    {
        _taxFee = taxFee;
    ...
    {
        uint256 allFee = _liquidityFee.add(_charityFee);
        ...
        uint256 walletsBal = newBalance.sub(ethForLiquidity);
        uint256 marketingShare  = walletsBal.mul(marketingDivisor).div(_charityFee); 
        marketingWallet.transfer(marketingShare);
        charityWallet.transfer(walletsBal-walletsBal);
    }
    ...
    function setCharityAndMarketingWalletsAddresses(address _charityWallet, address _marketingWallet) external onlyOwner() 
    {
    ...
    function  setSwapThreshold(uint256 amount) public onlyOwner 
    {
            ...
            emit Log("SwapThreshold changed", amount);
    }

    function  setWalletsMaxLimit(uint256 amount) public onlyOwner 
    {
            maxLimit = amount;
