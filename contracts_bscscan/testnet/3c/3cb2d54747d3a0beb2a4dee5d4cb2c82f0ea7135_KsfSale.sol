...
contract FinalBondingCurve {
    ...
    uint constant private bisectionPrecision = 0.5 ether; 
    int constant private displacement = 5e8;
    int constant private B = 4*3.2000e40;
    int constant private A = 500000000;
...
abstract contract ERC777Receiver is IERC777Recipient {
    ...
    //  @dev: This hook is called when the contract receives KsfTokens.
    //  The contract can only receive KsfTokens from the owner
    function tokensReceived(
        address /*operator*/,
        address from,
        address /*to*/,
        ...
        require(msg.sender == getToken(), "KsfPreSale.tokensReceived() - Wrong token");
        require(from == getOwner(), "KsfPreSale.tokensReceived() - Only owner");
    }
...
contract KsfSale is 
    ...
    uint constant private PEAK_PRICE = 500000000; // WEI / GAGEO

    address public token;

    uint public startSaleTimestamp; // Unix timestamp
    uint public duration; // Seconds
    uint public price = 500000000; // WEI / GAGEO
    uint public tokensSold; // in nanoKsfs (gaKsfs)

    event PurchasedKsfTokens(
    ...
    event RemainingKsfTokensWithdrawn(
        address sender,
        uint amount
    );
    ...
    fallback() external {
        revert("KsfPreSale - Fallback function called");
    }
    ...
    // @dev: allows the owner to withdraw KsfTokens that have not been sold out
    // after the sale has ended
    function withdrawKsf() external onlyOwner {
        require(saleIsOver(), "KsfSale.withdrawKsf() - Sale is not over");
        ...
        emit RemainingKsfTokensWithdrawn(
            msg.sender,
            remainingTokens
        );
    }

    function swapEtherForKsf() public payable returns (bool) {
        require(saleStarted(), "KsfSale.swapEtherForKsf() - Sale has not started");
        require(!saleIsOver(), "KsfSale.swapEtherForKsf() - Sale is over");
        ...
        // Up to this moment, computations were done in nanoKsfs.
        // So we perform a conversion to attoKsfs by multiplying by 10^9
        ...
        require(
            success,
            "KsfSale.swapEtherForKsf - Could not return eth"
        );

        emit PurchasedKsfTokens(
            msg.sender,
            finalExpenditure,
            tokensBought,
            returnedEth
