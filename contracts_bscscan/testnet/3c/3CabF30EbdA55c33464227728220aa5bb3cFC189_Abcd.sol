...
interface Cashier {
    ...
    function giveMeMoneyPlease(address hobo) external;
...
contract Abcd is IERC20 {
    ...
    uint256 private constant startingSupply = 1_000_000_000_000; // ,DEV
    
    uint8 private _decimals = 18;
    uint256 private _decimalsMul = _decimals;
    uint256 private _tTotal = startingSupply * (10 ** _decimalsMul);

    string constant _name = "Abcd"; // ,DEV
    string constant _symbol = "ABCD"; // ,DEV

    uint256 private _founderFee = 0; // Adjusted by buys and sells ,DEV
    uint256 private _reflectFee = 0; // Adjusted by buys and sells.
    uint256 private _liqFee = 0; // Adjusted by buys and sells.
    uint256 private _charityFee = 0; // Adjusted by buys and sells ,DEV
    uint256 private _marketingFee = 0; // Adjusted by buys and sells.
    uint256 private _totalFee = _founderFee + _liqFee + _reflectFee + _charityFee + _marketingFee + _charityFee; // ,DEV
    uint256 public masterTaxDivisor = 10000;

    uint256 public _buyFounderFee = 300; // ,DEV
    uint256 public _buyReflectionsFee = 0;
    uint256 public _buyLiqFee = 100;
    uint256 public _buyMarketingFee = 500;
    uint256 public _buyCharityFee = 300; // ,DEV

    uint256 public _sellFounderFee = 0; // ,DEV
    uint256 public _sellReflectionsFee = 1400;
    uint256 public _sellLiqFee = 0;
    uint256 public _sellMarketingFee = 0;
    uint256 public _sellCharityFee = 0; // ,DEV

    uint256 constant public maxFounderFee = 500; // ,DEV
    uint256 constant public maxReflectionsFee = 1400;
    uint256 constant public maxLiqFee = 1000;
    uint256 constant public maxMarketingFee = 500;
    uint256 constant public maxCharityFee = 1000; // ,DEV
    ...
    address payable private _founderWallet = payable(0xc3278fB86f2Aa67efF4552e1b06c3669447e74D8); // ,DEV
    address payable private _charityWallet = payable(0xf7BBC61176EBc759d83DE3af38403A14f2B6c7dE); // ,DEV

    // Max TX amount is 1% of the total supply.
    uint256 private maxTxPercent = 1; // Less fields to edit ,DEV
...
//===============================================================================================================
    ...
    function setBuyTaxes(uint256 founderFee, uint256 liqFee, uint256 reflectFee, uint256 marketingFee, uint256 charityFee) external onlyOwner { // ,DEV
        require( founderFee <= maxFounderFee // ,DEV
                && liqFee <= maxLiqFee
                && reflectFee <= maxReflectionsFee
                && marketingFee <= maxMarketingFee
                && charityFee <= maxCharityFee // ,DEV
                );
        require(founderFee + liqFee + reflectFee + marketingFee + charityFee <= 5000); // ,DEV
        _buyFounderFee = founderFee; // ,DEV
        _buyLiqFee = liqFee;
        _buyReflectionsFee = reflectFee;
        _buyMarketingFee = marketingFee;
        _buyCharityFee = charityFee; // ,DEV
    }

    function setSellTaxes(uint256 founderFee,uint256 liqFee, uint256 reflectFee, uint256 marketingFee, uint256 charityFee) external onlyOwner { // ,DEV
        require( founderFee <= maxFounderFee
                && liqFee <= maxLiqFee
                && reflectFee <= maxReflectionsFee
                && marketingFee <= maxMarketingFee
                && charityFee <= maxCharityFee
                );
        require(founderFee + liqFee + reflectFee + marketingFee + charityFee <= 5000);
        _sellFounderFee = founderFee; // ,DEV
        _sellLiqFee = liqFee;
        _sellReflectionsFee = reflectFee;
        _sellMarketingFee = marketingFee;
        _sellCharityFee = charityFee; // ,DEV
    }

    function setFounderWallet(address payable newWallet) external onlyOwner {
        require(_founderWallet != newWallet, "Wallet already set!");
        _founderWallet = payable(newWallet);
    }
    ...
    function setMarketingWallet(address payable newWallet) external onlyOwner {
        ...
        require(_charityWallet != newWallet, "Wallet already set!");
    ...
    function giveMeMoneyPlease() external {
        reflector.giveMeMoneyPlease(msg.sender);
    }
    ...
    function adjustTaxes(address from, address to) internal {
        if (lpPairs[to]) {
            _founderFee = _sellFounderFee; // ,DEV
            _reflectFee = _sellReflectionsFee;
            _liqFee = _sellLiqFee;
            _marketingFee = _sellMarketingFee;
            _charityFee = _sellCharityFee; // ,DEV
        } else if (lpPairs[from]) {
            _founderFee = _buyFounderFee; // ,DEV
            _reflectFee = _buyReflectionsFee;
            _liqFee = _buyLiqFee;
            _marketingFee = _buyMarketingFee;
            _charityFee = _buyCharityFee; // ,DEV
        } else {
            _founderFee = 0; // ,DEV
            _reflectFee = 0;
            _liqFee = 0;
    ...
    function getTotalFee() public view returns (uint256) {
        return _charityFee + _reflectFee + _liqFee + _marketingFee + _founderFee;
    }
    ...
    function swapBack(uint256 numTokensToSwap) internal swapping {
        uint256 dynamicLiquidityFee = isOverLiquified(targetLiquidity, targetLiquidityDenominator) ? 0 : _liqFee;
        ...
        uint256 amountBNBReflection = amountBNB * _reflectFee / totalBNBFee;
        uint256 amountBNBFounder = amountBNB - (amountBNBLiquidity + amountBNBReflection); // ,DEV
        uint256 amountBNBMarketing = amountBNB - (amountBNBLiquidity + amountBNBReflection);
        uint256 amountBNBCharity = amountBNB - (amountBNBLiquidity + amountBNBReflection); // ,DEV
        
        _founderWallet.transfer(amountBNBFounder); // ,DEV
        _marketingWallet.transfer(amountBNBMarketing);
        _charityWallet.transfer(amountBNBCharity); // ,DEV
