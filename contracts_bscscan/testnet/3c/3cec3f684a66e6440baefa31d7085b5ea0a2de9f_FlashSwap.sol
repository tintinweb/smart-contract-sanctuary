...
contract FlashSwap is IPancakeCallee {
    ...
    function startFlashLoan(uint amountIn, address[] calldata path, address baseToken) external {
        ...
        if(allowance == 0)
        {
        require(TOKEN.approve(_router, MAX_INT), 'approve failed.');
        }

        address pairAddress = PancakeLibrary.pairFor(_factory, path[0], baseToken);

        // address pairAddress = IPancakeFactory(_factory).getPair(path[0], baseToken);
    ...
    function pancakeCall(address sender, uint amount0, uint amount1, bytes calldata data) external override {
        ...
        require(msg.sender == PancakeLibrary.pairFor(_factory, token0, token1), "FlashSwap: msg.sender is not a pair");
        ...
        IPancakeRouter02(_router).swapExactTokensForTokens(
            amounIn,
            0, // Amount that will return back has to be greater than start amount
            _path,
            address(this),
            now + 10 minutes
        );
    ...
    function withdrawAllFromToken(IERC20 token) external OnlyManager {
