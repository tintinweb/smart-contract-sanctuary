...
contract BEP20 is Context, IBEP20, IBEP20Metadata, Ownable, Pausable {
    ...
    function _getTValues(uint256 amount, address recipient) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tBurn = 0;
        if (recipient == pancakeswapV2Pair) tBurn = calculateTransactionBurn(tAmount);
    ...
    function _getRValues(uint256 amount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tMarketingFee, address recipient) private view returns (uint256, uint256, uint256) {
        ...
        uint256 rBurn = 0;
        if(recipient == pancakeswapV2Pair) rBurn = tBurn.mul(currentRate);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        if(!takeFee || (sender != pancakeswapV2Pair && recipient != pancakeswapV2Pair))
            removeAllFee();
    ...
    function removeLiquidityFromPlatform(uint256 liquidity) external virtual {
        ...
        IPancakePair(pancakeswapV2Pair).transferFrom(msg.sender, address(this), liquidity);
        // approve router address
        IPancakePair(pancakeswapV2Pair).approve(PANCAKEV2ROUTER, liquidity);

        // remove the liquidity
        pancakeswapV2Router.removeLiquidity(
            address(this),
