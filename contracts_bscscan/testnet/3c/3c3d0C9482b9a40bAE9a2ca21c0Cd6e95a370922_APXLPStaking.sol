...
contract APXLPStaking is Ownable, Pausable, ReentrancyGuard {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    /* ========== STRUCT DEFINITIONS ========== */

    // Info of staking pool.
    struct PoolInfo {
        IERC20 lpToken;            // Address of LP token contract.
        uint256 totalStaked;       // Total supply of LP token
    ...
    // APX tokens per block.
    uint256 public apxPerBlock;
    // Bonus muliplier
    uint256 public BONUS_MULTIPLIER;
    uint256 constant public MAX_MULTIPLIER = 100;
    // Mining start block
    ...
    modifier checkStart() {
        require(block.number >= startBlock, "Mining not started yet");
        _;
    }
    ...
    function pendingApx(address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfo[_user];
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 apxReward = multiplier.mul(apxPerBlock);
    ...
    function updateMultiplier(uint256 multiplierNumber) external onlyOwner {
        require(multiplierNumber <= MAX_MULTIPLIER, "Multiplier greater than max");
        updatePool();
        BONUS_MULTIPLIER = multiplierNumber;
    }

    // Deposit LP tokens for APX allocation.
    function deposit(uint256 _amount) external nonReentrant checkStart whenNotPaused oncePerBlock(msg.sender) {
    ...
    // Withdraw LP tokens
    function withdraw(uint256 _amount) external nonReentrant whenNotPaused oncePerBlock(msg.sender) {
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfo[msg.sender];
    ...
    function claim() external nonReentrant whenNotPaused oncePerBlock(msg.sender) {
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfo[msg.sender];
        ...
        if (pending > 0) {
            ...
            safeApxTransfer(msg.sender, pending);
            user.rewardDebt = user.amount.mul(pool.accAPXPerShare).div(1e12);
    ...
    // Safe apx transfer function, just in case if rounding error causes pool to not have enough APXs.
    function safeApxTransfer(address _to, uint256 _amount) internal {
