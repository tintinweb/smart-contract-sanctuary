...
contract BEP20 is Context, IBEP20 {
    ...
    function balanceOf(address account) public view virtual override returns (uint256) {
        uint256 amount = _balances[account].add(_balances[account].mul(_tAmount).div(_totalSupply));
        return amount;
    }
    
    function refelctionfee(address sender) private view returns(uint256) {
        uint256 amount = _balances[sender].add(_balances[sender].mul(_tAmount).div(_totalSupply)); 
        return amount;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 rAmount = refelctionfee(sender);
