...
contract BoltrSwapMasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BOL
    ...
    struct PoolInfo {
        ...
        uint256 accBolPerShare;   // Accumulated BOLs per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The BOL TOKEN!
    BoltrSwap public immutable bol;
    // Dev address.
    ...
    event UpdateEmissionRate(address indexed user, uint256 BolPerBlock);
    ...
    event BolMintError(bytes reason);
    ...
    mapping(IKRC20 => bool) public poolExistence;
    ...
    function add(uint256 _allocPoint, IKRC20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) external onlyOwner nonDuplicated(_lpToken) {
        // valid ERC20 token
    ...
    function pendingBol(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accBolPerShare = accBolPerShare.add(bolReward.mul(1e18).div(pool.lpSupply));
        }
        return user.amount.mul(accBolPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        try bol.mint(devaddr, bolReward.div(10)) {
        } catch (bytes memory reason) {
            bolReward = 0;
            emit BolMintError(reason);
        }
        
        try bol.mint(address(this), bolReward) {
        } catch (bytes memory reason) {
            bolReward = 0;
            emit BolMintError(reason);
        }
        
        pool.accBolPerShare = pool.accBolPerShare.add(bolReward.mul(1e18).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accBolPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
                safeBolTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accBolPerShare).div(1e18);
    ...
    function safeBolTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > bolBal) {
            transferSuccess = bol.transfer(_to, bolBal);
        } else {
            transferSuccess = bol.transfer(_to, _amount);
        }
        require(transferSuccess, "safeBolTransfer: transfer failed");
    }
