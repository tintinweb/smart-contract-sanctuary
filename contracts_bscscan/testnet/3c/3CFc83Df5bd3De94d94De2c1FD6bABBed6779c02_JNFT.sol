...
contract JNFT is AccessControlUpgradeable, ERC1155SupplyUpgradeable
{
    ...
    ) internal returns (uint256 _tokenId)
    {
        ...
        extraInfo[_tokenId] = ExtraInfo(block.timestamp, 0, 0, 1);
    ...
    ) external onlyMaintainer
    {
        ...
        ExtraInfo storage _extraInfo = extraInfo[_tokenId];
        _extraInfo.rarity = _rarity;
        emit MintWithRarity(_creator, _receiver, _tokenId, _collectionId, _amount, _loyaltyFeePercent, _rarity);
    }
    ...
    function _jnftMintBatch(
        ...
        for (uint256 i; i < _amounts.length; i++) {
            ...
            if (_rarities[i] == 0) {
                extraInfo[_tokenIds[i]] = ExtraInfo(block.timestamp, 0, 0, 1);
            } else {
                extraInfo[_tokenIds[i]] = ExtraInfo(block.timestamp, 0, 0, _rarities[i]);
            }  
            _tokenIdTracker.increment();
        }
    ...
    function mintBatch(
        ...
        uint256[] memory _tokenIds = _jnftMintBatch(msg.sender, msg.sender, _loyaltyFeePercent, _amounts, _collectionId, _rarities);
        emit MintBatch(msg.sender, _tokenIds, _collectionId, _amounts, _loyaltyFeePercent);
    }    

    function mintBatchWithRarities(
        address _creator,
        address _receiver,
        uint256 _loyaltyFeePercent, 
        uint256[] calldata _amounts, 
        ...
        uint256[] memory _tokenIds = _jnftMintBatch(_creator, _receiver, _loyaltyFeePercent, _amounts, _collectionId, _rarities);
        emit MintBatchWithRarities(_creator, _receiver, _tokenIds, _collectionId, _amounts, _loyaltyFeePercent, _rarities);
    }
    ...
    function setExtraRarity(uint256 _tokenId, uint256 _rarity)
        external onlyMaintainer
    {
        require(exists(_tokenId), "token does not exist");
        ExtraInfo storage _extraInfo = extraInfo[_tokenId];
        require(_extraInfo.rarity == 0, "Rarity is already set");
        _extraInfo.rarity = _rarity;   
    }
        ...
        external view returns (uint256)
    {
        ...
        return extraInfo[_tokenId].rarity == 0 ? 1 : extraInfo[_tokenId].rarity;
    }
