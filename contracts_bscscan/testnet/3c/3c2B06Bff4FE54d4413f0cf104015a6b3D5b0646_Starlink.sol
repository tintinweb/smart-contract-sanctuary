...
import "./pools/IStarlinkPool.sol";
import "./IDepositable.sol";

contract Starlink is AccessControlled  {
    ...
    uint256 public rewardPoolAllocationMagnitude = 100;
    ...
    /**
    * @dev Mainly called by the shop contract to deposit funds
    */
    function deposit(address tokenAddress, uint256 amount) external payable onlyAdmins {
        if (amount > 0) {
            IBEP20(tokenAddress).transferFrom(msg.sender, address(this), amount); //Trust that transferFrom transfers exactly 'amount' tokens
            tokensPendingDisburse[tokenAddress] += amount;
        }
    }
    ...
    function processFunds(uint256 gas) external onlyAdmins {
        ...
        xld.transfer(address(this).balance * rewardPoolAllocationMagnitude / 1000);
        ...
        while(gasUsed < gas && iteration < pools.length) {
            ...
            address outTokenAddress = pool.outTokenAddress();
            pool.deposit{value: fundsPerPool}(tokensPendingDisburse[outTokenAddress]);

            // Cheaper to wrap in an if statement than to delete a zero
            if (tokensPendingDisburse[outTokenAddress] > 0) {
                delete tokensPendingDisburse[outTokenAddress];
            }
    ...
    function poolInfos(uint256 index) external view returns(PoolInfo memory) {
        ...
        return PoolInfo(address(pool), pool.inTokenAddress(), pool.outTokenAddress(), pool.amountOut(), pool.amountIn(), pool.totalDividends(), pool.totalDividendPoints(), pool.starlinkPointsPerToken(), pool.isStakingEnabled());
    }
    ...
    function setRewardPoolAllocationMagnitude(uint256 magnitude) external onlyOwner {
        require(magnitude <= 1000, "Starlink: Invalid value");
        rewardPoolAllocationMagnitude = magnitude;
    }
}
