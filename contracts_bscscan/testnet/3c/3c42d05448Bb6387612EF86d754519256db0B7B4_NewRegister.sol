...
contract NewRegister {
  ...
  function use(address player, uint id, uint amount, uint kind, string memory metaData) public {
    ...
    if (registerContract.balanceOf(player).length > id) {
        uint oldBalance_ = registerContract.balanceOf(player)[id];
        if (oldBalance_ > oldBalance[player][id]) {
            accountInfo[player].balance[id] += (oldBalance_ - oldBalance[player][id]);
            oldBalance[player][id] = oldBalance_;
        }
    }
