...
contract VaultFactory is Ownable {
     ...
     * @param _pid: Farm pid
     * @param _isLp: Farm isLP
     * @param _withdrawFeePeriod: withdrawFeePeriod
...
contract VaultInitializable is Ownable, Pausable {
    ...
    function harvest() external notContract whenNotPaused {
        IMasterChef(masterchef).deposit(pid, 0, deadAddress);
        ...
        if (bal > 0 && address(token) != address(reward)) {
            reward.approve(address(swapRouter), bal);
            ...
            if (isLp) {
                ...
                if (address(token0) != address(reward)) {
                    ...
                    token0.approve(address(swapRouter), half0);
                }
                ...
                if (address(token1) != address(reward)) {
                    ...
                    token1.approve(address(swapRouter), half1);
                }

                swapRouter.addLiquidity(address(token0), address(token1), half0, half1, 0, 0, address(this), block.timestamp);
                ...
                swapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(bal, 0, path, address(this), block.timestamp);
            }
        }
    ...
    function calculateHarvestRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingOberon(pid, address(this));
    ...
    function _earn() internal {
        uint256 bal = availableToken();
        if (bal > 0) {
            IMasterChef(masterchef).deposit(pid, bal, deadAddress);
        }
    }
...
interface IMasterChef {
    ...
    function pendingOberon(uint256 _pid, address _user) external view returns (uint256);
