...
contract DandyDragons is ERC721Enumerable, Ownable {
  ...
  constructor(address payable _treasury1, address payable _treasury2)
    ERC721("Dandy Dragons", "DD")
  {
    treasury1 = _treasury1;
    treasury2 = _treasury2;

    // premint 200 dragons for giveaway
    for (uint256 i = 0; i < 200; i ++) {
      _safeMint(_treasury1, i + 1);
    }
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    uint256 fee = msg.value.mul(75).div(100); // 75%
    require(treasury1.send(fee));
    require(treasury2.send(msg.value.sub(fee)));
  }
