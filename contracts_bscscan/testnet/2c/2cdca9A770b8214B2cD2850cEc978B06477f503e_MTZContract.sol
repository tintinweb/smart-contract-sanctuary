...
contract MTZContract is Context, IBEP20, Ownable {
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    uint256 exceptBurnAmount = amount;
    ...
    if (_isExceptFeeMember[sender] == false && _isExceptFeeMember[recipient] == false) {
      ...
      if (_burnStopped == false) {
        ...
        exceptBurnAmount = exceptBurnAmount.sub(burnFeeAmount);
    ...
    _balances[sender] = _balances[sender].sub(exceptBurnAmount, "MONET: transfer amount exceeds balance");
