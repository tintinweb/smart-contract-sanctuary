...
ability to change the wallets 


functions to set fees *
change all fees to 0 *
burn at 1% *
send bnb to a contract or wallet and process it 
set max hold and max trans limits

add is excluded from fees to all wallets when they are updated (when change dev/mark.ani wall)
...
contract D2E_5 is Context, IERC20, Ownable {
    ...
    string private _name = "D2E_5";
    string private _symbol = "D2E_5";
    ...
    uint256 private _previousFeeAnimator_X100 = _FeeAnimator_X100;    


    uint256 _promoFee = (_FeeMarketing_X100+_FeeDev_X100+_FeeAnimator_X100)/100;
    uint256 _promoFee_X100 = _FeeMarketing_X100+_FeeDev_X100+_FeeAnimator_X100;
    uint256 private _previousPromoFee = _promoFee;

    uint256 public _FeesTotal = (_FeeMarketing_X100+_FeeDev_X100+_FeeAnimator_X100)/100+_FeeLiquidity+_FeeBurn;
    uint256 private _previousFeesTotal = _FeesTotal;
      

    //fee for the auto LP and the marketing wallet 
    uint256 private _liquidityAndPromoFee = _FeeLiquidity+_promoFee;
    ...
    uint256 public _numTokensSellToAddToLiquidity = 1000000 * 10**9;
    ...
    //set fee for the marketing (BNB) wallet (X100)
    ...
    //set fee for the animator (BNB) wallet (X100)
    function fees_setFeeAnimator_X100(uint256 FeeAnimator_X100) external onlyOwner() {
        _FeeAnimator_X100 = FeeAnimator_X100;
    }
    ...
    function removeAllFee() private {
        ...
        _previousFeeAnimator_X100 = _FeeAnimator_X100;
    ...
    function restoreAllFee() private {
         ...
         _FeeAnimator_X100 = _previousFeeAnimator_X100;
         _FeeReflection = _previousFeeReflection;
         _FeeBurn = _previousFeeBurn;
         _liquidityAndPromoFee = _previousLiquidityAndPromoFee;
    }
    ...
    function precDiv(uint a, uint b, uint precision) internal pure returns ( uint) {
     ...
     function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
       ...
       uint256 __splitM = 0;
       uint256 __tokensToM = 0;
       uint256 __splitA = 0;
       uint256 __tokensToA = 0;
       uint256 __splitD = 0;
       uint256 __tokensToD = 0;
        ...
        //split this amount and send to the corect wallets - Marketing/Dev/Animator

        __splitM = precDiv(_FeeMarketing_X100,_promoFee_X100,2);
        __tokensToM = balanceToSendPromo*__splitM/100;
        sendToWallet(Wallet_Marketing, __tokensToM);

        __splitD = precDiv(_FeeDev_X100,_promoFee_X100,2);
        __tokensToD = balanceToSendPromo*__splitD/100;
        sendToWallet(Wallet_Dev, __tokensToD);

        __splitA = precDiv(_FeeAnimator_X100,_promoFee_X100,2);
        __tokensToA = balanceToSendPromo*__splitA/100;
        sendToWallet(Wallet_Animator, __tokensToA);
        }
