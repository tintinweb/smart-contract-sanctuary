...
contract Grapenopoly
{
    ...
    mapping(uint256 => uint) public nftHouseAmount; //0 = empty | maxHouseAmount + 1 = hotel
    
    mapping(address => uint) public playerAccumulatedStepsFromLastTax;
    mapping(address => uint) public playerAccumulatedStepsFromLastPrize;
    ...
    function rollDices(address selectedTokenToPayRent) public returns (DiceResult memory)
    {
        require(vaultBalances[msg.sender][selectedTokenToPayRent] > 0, "NB"); //No balances for moving
        ...
        if( safeAdd(playerPosition[msg.sender], walkSteps) > id )
        {
            //Went around the board
            playerPosition[msg.sender] = safeSub(safeAdd(playerPosition[msg.sender], walkSteps), id);
        }
        else
        {
            playerPosition[msg.sender] = safeAdd(playerPosition[msg.sender], walkSteps);
        }
        ...
        if(prizeWalkAmount > 0 && playerAccumulatedStepsFromLastPrize[msg.sender] >= prizeWalkAmount)
        {
            ...
            //Collect prize
            uint256 prizeValue = uint256(keccak256(abi.encodePacked(safeAdd(block.timestamp, diceNonce), block.difficulty, msg.sender))) % prizeLandMaxPay;
        ...
        if(taxesWalkAmount > 0 && playerAccumulatedStepsFromLastTax[msg.sender] >= taxesWalkAmount)
        {
            ...
            //Pay tax
            uint256 taxValue = uint256(keccak256(abi.encodePacked(safeAdd(block.timestamp, diceNonce), block.difficulty, msg.sender))) % taxesLandMaxPay;
        ...
        if(nftOwners[ playerPosition[msg.sender] ] != msg.sender && nftOwners[ playerPosition[msg.sender] ] != owner)
        {
            ...
            if(rentValueInUSD > 0)
            {
                uint256 rentValueInToken = safeMul(rentValueInUSD, getQuote(usdToken, selectedTokenToPayRent));
