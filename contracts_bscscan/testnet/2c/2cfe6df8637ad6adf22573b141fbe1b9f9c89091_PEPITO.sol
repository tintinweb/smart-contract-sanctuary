...
contract PEPITO is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxAmount = 50_000_000 * 10**9; // 
    uint256 private constant SWAP_TOKENS_AT_AMOUNT = 100_000 * 10**9; //
    string private constant _name = "PEPITO GONZALES"; // 
    string private constant _symbol = "PEPITO"; //    
    uint8 private constant _decimals = 9; // 
    
    uint256 public _projectFee = 10;
    uint256 public _liquidityFee = 2;
    address public  _projectWallet = 0xa1ed8416F0368C29BAF93684fa82F2e47abeE40F;
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        uint256 projectFee = amount * _projectFee / 100; 
        uint256 liquidityFee = amount * _liquidityFee / 100; 
        _tOwned[address(this)] += projectFee + liquidityFee;
        emit Transfer (from, address(this), projectFee + liquidityFee);
        return (amount - projectFee - liquidityFee);
    }
    ...
    function swapAndLiquify() private {
        ...
        uint256 liquidityTokens = balanceOf (address(this)) * _liquidityFee / (_projectFee + _liquidityFee);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (ethAmount - ethFromLiquidity > 0)
            payable(_projectWallet).sendValue (ethAmount - ethFromLiquidity);
    }
}