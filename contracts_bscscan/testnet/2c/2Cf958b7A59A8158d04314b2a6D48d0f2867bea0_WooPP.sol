...
library DecimalMath {
    ...
    function to64x64(uint256 target) internal pure returns (int128) {
        require(target <= type(uint192).max, "OVERFLOW");
        uint256 res = (target << 64).div(10**18);
        require(res <= uint256(int256((type(int128).max))));
        return int128(uint128(res));
    }

    function from64x64(int128 x) internal pure returns (uint256) {
        require(x >= 0, "UNDERFLOW");
        return uint256(int256(x)).mul(10**18).div(1 << 64);
    }

    function ln(uint256 target) internal pure returns (uint256) {
        return from64x64(ABDKMath64x64.ln(to64x64(target)));
    }

    function exp(uint256 target) internal pure returns (uint256) {
        return from64x64(ABDKMath64x64.exp(to64x64(target)));
    }

    function negexp(uint256 target) internal pure returns (uint256) {
        return from64x64(ABDKMath64x64.exp(ABDKMath64x64.neg(to64x64(target))));
    }
}
...
contract WooPP is InitializableOwnable, ReentrancyGuard {
    ...
    struct TokenInfo {
        ...
        uint64 R;
        bool isValid;
        address referenceOracle1; // Chainlink
    }
    ...
    address public quoteReferenecOracle1; // Chainlink
    ...
    function setQuoteReferenecOracle1(address newQuoteReferenecOracle1) external preventReentrant onlyStrategist {
        require(newQuoteReferenecOracle1 != address(0), "INVALID_REFERNECE_ORACLE");
        quoteReferenecOracle1 = newQuoteReferenecOracle1;
    }
    ...
    function addBaseToken(
        ...
        info.R = uint64(R);
        info.referenceOracle1 = referenceOracle1;
        tokenInfo[baseToken] = info;
    }
    ...
    function removeBaseToken(
        ...
        info.lpFeeRate = 0;
        info.R = 0;
        info.referenceOracle1 = address(0);
        tokenInfo[baseToken] = info;
    }
    ...
    function ensurePriceReliable(uint256 p0, TokenInfo memory info) internal view {
        // check Chainlink
        if (info.referenceOracle1 != address(0)) {
            (, int256 rawReferencePrice, , , ) = AggregatorV3Interface(info.referenceOracle1).latestRoundData();
            require(rawReferencePrice >= 0, "INVALID_CHAINLINK_PRICE");
            (, int256 quoteReferenecPrice, , , ) = AggregatorV3Interface(quoteReferenecOracle1).latestRoundData();
            require(quoteReferenecPrice >= 0, "INVALID_CHAINLINK_QUOTE_PRICE");
            // referencePrice = rawReferencePrice * 10 ** (18 - decimals);
            require(AggregatorV3Interface(info.referenceOracle1).decimals() == AggregatorV3Interface(quoteReferenecOracle1).decimals(),
