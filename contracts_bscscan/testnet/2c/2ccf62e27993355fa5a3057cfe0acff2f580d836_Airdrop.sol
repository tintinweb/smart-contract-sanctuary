...
contract Airdrop is AccessControlUpgradeable {
    ...
    MysteryCapsule public mysterycapsule;
    ...
    mapping(address => Claimed) public whitelist;
    ...
    function setMysteryCapsule(address spaceinvadersAddr) public restricted {
        mysterycapsule = MysteryCapsule(spaceinvadersAddr);
    }
    ...
    function addWhitelist(address addr) public restricted {
        require(!whitelist[addr].isWhitelist, "AWL");
        whitelist[addr] = Claimed(true, false, 0);
        _totalWhitelist += 1;

        emit AddWhitelist(addr);
    }

    function addWhitelists(address[] memory addrs) public restricted {
        for (uint256 i = 0; i < addrs.length; i++) {
            address addr = addrs[i];

            if (!whitelist[addr].isWhitelist) {
                whitelist[addr] = Claimed(true, false, 0);
                _totalWhitelist += 1;
                emit AddWhitelist(addr);
            }
    ...
    function inWhitelist(address addr) public view returns (bool) {
        return whitelist[addr].isWhitelist;
    }

    function claim() public onlyNonContract oncePerBlock(msg.sender) returns (uint256) {
        address addr = msg.sender;
        require(whitelist[addr].isWhitelist, "NWL");
        require(!whitelist[addr].claimed, "Claimed");

        uint256 length = spaceship.totalSupply();

        require(length <= mysterycapsule.spaceshipTotalLimit(), "FT");
        require(mysterycapsule.spaceshipTotalByRate(0) < mysterycapsule.spaceshipAvailability(0), "MOR");

        uint256 seed = randoms.getRandomSeed(msg.sender);
        uint8 class = uint8(RandomUtil.randomSeededMinMax(0, mysterycapsule.maxClass(), seed));
        uint8 element = uint8(RandomUtil.randomSeededMinMax(0, 3, seed));

        mysterycapsule.increaseCountAtRate(0);
        uint256 shipId = length;
        spaceship.mint(addr, shipId, 0, class, element, seed);

        whitelist[addr].claimed = true;
        whitelist[addr].time = block.timestamp;
    ...
    function withdraw(address addr) external restricted {
        require(token.balanceOf(address(this)) > 0, "BZ"); // Current of contract balance must be > 0
 ...
 * Author: Mikhail Vladimirov <mikhail.vladimir[email protected]>
 */
pragma solidity ^0.8.0;
