...
Altruvaults combines modern tokenomics with the use case of adding Defi features to existing BSC tokens.
Every vault benefits the Altruvault token
    
Token features include:
    Externally Boosted Tokenomics Through The Vaults System (app.altruvaults.com)
    Altruboost: Dynamic Auto Buy Back
    12 % Buy Fee:
        2% to Hodlers
        5% to Buy Back
        1% to Charity
    ...
    18% Sell Fee:
        ...
        2% to Marketing
        2% to Development
    Multisignature Protocol Wallets (Increased Security)
...
contract Altruvault is Context, IERC20, Ownable {
    ...
    address payable public developmentAddress = payable(0x17d782117bf626150c109c25e8bCfe9FBb212090); 
    address payable public charityAddress = payable(0x17d782117bf626150c109c25e8bCfe9FBb212090); 
    address payable public vaultRewardAddress = payable(0x17d782117bf626150c109c25e8bCfe9FBb212090); 
    ...
    string private constant _name = "Altruvault";
    string private constant _symbol = "Altruvault";
    ...
    uint256 public _tempTaxFee = 0;
    uint256 public _tempVaultFee = 0;
    uint256 public _tempLiquidityFee = 0;
    ...
    uint256 public _bMaxTxAmount = 0;
    uint256 public _sMaxTxAmount = 0;
    uint256 public _trigger = 0;
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner()) {
            if(_abmode == true){
                require(!bots[from], "No bot transactions during antibotmode.");
            }
                ...
                if (buyBackEnabled) {
                    ...
                    if (_isAutoBuyBack) {
                        ...
                        if (cnt > 0 && _buyBackDivisor > 0) {
                            // Max = 10% of the average amount sold
    ...
    function prepareForPreSale() external onlyOwner {
        ...
        _taxFee = 0;
        _liquidityFee = 0;
        _bMaxTxAmount = 1000000000 * 10**6 * 10**9;
        _sMaxTxAmount = 1000000000 * 10**6 * 10**9;
    }
    ...
    function afterPreSale() external onlyOwner {
        ...
        _taxFee = 2;
        _liquidityFee = 10;
        _bMaxTxAmount = 6000000 * 10**6 * 10**9;
        _sMaxTxAmount = 3000000 * 10**6 * 10**9;
    }
    ...
    function manualSwapAndLiquifyBNB(uint256 bnbLiquifyAmount) private lockTheSwap onlyOwner {
    ...
    //Create additional liquidity using Altruvault tokens in contract
    function manualSwapAndLiquifyTokens(uint256 tokenLiquifyAmount) private lockTheSwap {
        // split the contract balance into halves
    ...
    // For stuck tokens in case of dex change or update. Can be used to create liquidity on other Dexs
