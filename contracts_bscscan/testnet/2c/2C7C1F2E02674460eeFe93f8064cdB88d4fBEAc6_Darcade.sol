...
contract Darcade is ReentrancyGuard, Ownable {
    ...
    */
    ...
    /**
     * @dev {SessionComplete} Fires when session is complete
     */
    event SessionComplete(string session, string userId);

    /**
     * @param receivingFeesWallet Address where collected funds will be forwarded to
     */
    constructor (address payable receivingFeesWallet) {
    ...
    */
    ...
    /**
     * @dev Resets the game session
     */
    function resetUserGameSession(string memory gameId, string memory userId) public {
        require(userGameSession[userId][gameId] == _msgSender(), "You must be the owner of this session");
        delete userGameSession[userId][gameId];
        emit SessionComplete(gameId, userId);
    }
