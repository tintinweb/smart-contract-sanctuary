...
abstract contract TransferManager is OwnableUpgradeable, TransferManagerBase
{
    ...
    function transferRoyalties(
        ...
        if (
            ...
            uint256 amount = abi.decode(counterAssetExtraData, (uint256));
            LibPart.Part[] memory fees = data.collection.royalties;
            if (fees.length == 0) fees = data.collection.tokens[0].royalties;
            rest = transferRoyalties(
                transferData,
                transferData.asset.value / amount,
                rest,
                fees,
                assetExtraData
            );
    ...
    function transfer(
        ...
        if (transferData.asset.assetClass == LibAsset.ETH_ASSET_CLASS) {
            ...
            (address token, LibERC1155Lazy.MintData memory data) = abi
                .decode(transferData.asset.data, (address, LibERC1155Lazy.MintData));
            bytes memory payload = abi.encodeWithSignature(
                "mintNFTs((address,(uint256,string,(address,uint256)[],(address,uint256)[],(uint256,string,uint256,(address,uint256)[],(address,uint256)[])[]),bytes[]),address,uint256)", 
                data,
                to,
