...
contract VaultVenusBridgeOwner is WhitelistUpgradeable {
  ...
  VaultVenusBridge private constant VENUS_BRIDGE =
    VaultVenusBridge(0x4a994ED8dF9E68c046d236453aB0f61e6357B17D);
  ...
  function harvestBehalf(address vault) public returns (uint256 XVSBalance) {
    ...
    uint256 xvsBefore = XVS.balanceOf(address(msg.sender));
    VENUS_UNITROLLER.claimVenus(address(msg.sender), vTokens);
    // VAIVAULT.claim();
    // uint256 xvsBalance = XVS.balanceOf(address(VENUS_BRIDGE)).sub(xvsBefore);
    uint256 xvsBalance = XVS.balanceOf(address(msg.sender)).sub(xvsBefore);
