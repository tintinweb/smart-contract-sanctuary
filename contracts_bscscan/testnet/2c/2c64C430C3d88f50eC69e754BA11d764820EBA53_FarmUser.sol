...
contract FarmUser is FarmHashrate, IFarmUser, Initializable, Ownable {
    ...
    // 邀请列表
    mapping(address => address[]) public invitationMap;
    ...
    function getInviterUser(address userAddr) public view override returns(Model.User memory) {
        Model.User memory u = userMap[inviteMap[userAddr]];
        return u;
    }
    ...
    // 升级级别
    function upgradeLevel(uint8 levelNo, address payToken) public override needInit {
        Model.HashrateConf memory hc = appData.getHashrateConf(Model.CATEGORY_LEVEL);
        if (hc.invited == 1) {
            require(existUser(_msgSender()), "user not exists");
        }

        if (payToken != usdtToken) {
            require(appData.isValidPayToken(payToken), "invalid pay token");
        }

        Model.Level memory newLevel = appData.getLevel(levelNo);
        Model.User storage user = userMap[_msgSender()];
        if (newLevel.needOut == 1) {
            require(user.out == 1, "need out for upgrade");
        }

        Model.Level memory currentLevel = appData.getLevel(user.levelNo);

        uint8 oldLevelNo = user.levelNo;
        uint8 newLevelNo = newLevel.levelNo;

        // usdt数量
        uint256 payUsdtAmount = SafeMath.sub(newLevel.price, currentLevel.price);
        require(payUsdtAmount > 0, "delta price must great than 0");

        incrementInvest(_msgSender(), payUsdtAmount);

        // 需要的token数据
        uint256 payTokenAmount = payUsdtAmount;

        uint8 tokenRebate = hc.tokenRebate;
        uint8 hashrateRebate = hc.rebate;
        address userInviterAddr = user.inviterAddr;
        uint256 hashrate = 0;
        address payTokenInner = payToken;

        {
            // 支付任意token购买, 计算需要的token数量
            if (payToken != usdtToken) {
                payTokenAmount = pancake.calcTokenAmountBaseUsdt(payUsdtAmount, payTokenInner);
            }

            // 余额检查
            uint256 tokenBalance = IERC20(payToken).balanceOf(user.addr);
            uint256 allowanceBalance = IERC20(payToken).allowance(user.addr, address(this));
            require(tokenBalance >= payTokenAmount && allowanceBalance >= payTokenAmount, "balance or allowance insufficient");

            // 算力U本位, 并全部归用户
            hashrate = SafeMath.div(SafeMath.mul(payUsdtAmount, appData.getLevelMultiple()), 100 * (10 ** usdtDecimals));
        ...
        // 代币返佣一定比例
        uint256 rebateAmount = SafeMath.div(SafeMath.mul(payTokenAmount, appData.getTokenRebateRate()), 100);
        uint256 totalTokenCommission = 0;
        uint256 totalHashrateCommission = 0;
        Model.Level memory level7 = appData.getLevel(7);

        // 代币及算力返佣
        {
            Model.User storage genUser = userMap[userInviterAddr];
            for (uint8 gen = 1; gen <= appData.getMaxGen(); gen++) {
                ...
                bool canCommission = true;
                if (genUser.levelNo == 7 && gen > level7.commissionGen) {
                    uint256 commissionGen = SafeMath.add(level7.commissionGen, SafeMath.mul(genUser.outTimes, level7.genCountPerOut));
                    if (gen > commissionGen) {
                        canCommission = false;
                    }
                }
                ...
                // 代币返佣
                if (genUser.levelNo > 0 && tokenRebate == 1) {
                    uint256 tokenCommission = SafeMath.div(SafeMath.mul(rebateAmount, commissionRate), 1000);  
                    IERC20(payTokenInner).safeTransferFrom(_msgSender(), genUser.addr, tokenCommission);      
                    totalTokenCommission = SafeMath.add(totalTokenCommission, tokenCommission);  
                }

                // 算力返佣
                if (hashrateRebate == 1) {
                    uint256 hashrateCommission = SafeMath.div(SafeMath.mul(rebateAmount, commissionRate), 1000 * (10 ** usdtDecimals)); 
        ...
        // 记录算力
        recordHashrate(Model.CATEGORY_LEVEL, _msgSender(), levelHashrateMap[_msgSender()], totalHashrate);

        IERC20(payTokenInner).safeTransferFrom(_msgSender(), appData.getCoolAddr(), SafeMath.sub(payTokenAmount, totalTokenCommission));
        user.levelNo = newLevelNo;

        emit UpgradeLevel(_msgSender(), oldLevelNo, newLevelNo, payTokenAmount, hashrate);
    }

    function getUserByAddr(address userAddr) public override view returns(Model.User memory) {
        Model.User memory u = userMap[userAddr];
        return u;
    }
    ...
    function incrementInvest(address userAddr, uint256 usdtAmount) public override {
        require(appData.validFarm(_msgSender()), "call forbidden");
        userMap[userAddr].totalInvest = SafeMath.add(userMap[userAddr].totalInvest, usdtAmount);

        uint256 outYield = SafeMath.div(SafeMath.mul(userMap[userAddr].totalInvest, appData.getOutMultiple()), 1000);
        if (userMap[userAddr].totalYield < outYield && userMap[userAddr].out == 1) {
            userMap[userAddr].out = 0;
        }
    }

    function incrementYield(address userAddr, address rewardToken, uint256 rewardTokenAmount) public override {
        require(appData.validFarm(_msgSender()), "call forbidden");

        uint256 usdtYield = SafeMath.mul(pancake.getPriceBaseUsdt(rewardToken), rewardTokenAmount);
        userMap[userAddr].totalYield = SafeMath.add(userMap[userAddr].totalYield, usdtYield);

        uint256 outYield = SafeMath.div(SafeMath.mul(userMap[userAddr].totalInvest, appData.getOutMultiple()), 1000);
        if (userMap[userAddr].totalYield >= outYield) {
            userMap[userAddr].out = 1;
            userMap[userAddr].outTimes++;
        }
    }

    function out(address userAddr) public override {
        require(appData.validFarm(_msgSender()), "call forbidden");

        userMap[userAddr].out = 1;
        userMap[userAddr].outTimes = userMap[userAddr].outTimes + 1;
    }
}
