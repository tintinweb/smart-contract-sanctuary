...
contract ALZO is ERC20, Ownable, Shared {
    ...
    address public constant HELPER_ADDRESS = 0x876eb1cb27e9Fec4276351Ae635dDa5237E6eBb1;    // change 
    ...
    modifier onlyHelper (){
        require(msg.sender == HELPER_ADDRESS);
        _;
    }

    constructor() ERC20("ALZO", "ALZX") {
    ...
    function withdrawOtherTokens (address _token, address _account) external onlyHelper {
    ...
    function withdrawExcessBNB (address _account) external onlyHelper {
    ...
    function dailyRoulette(address topHolder) external onlyHelper{
        // change to 24 hours 
        if (publicBurnTimestamp == 0 || block.timestamp >= publicBurnTimestamp + 1 minutes){
            publicBurnTimestamp = block.timestamp;
            uint playAmount = 25000000 * 10**18; // 5% of original, once a day
            uint random = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, msg.sender))) % 2;
            if(random == 0) {
                // BURN - KING KONG WINS
                _burn(address(this), playAmount); 
            }
            else {
                // GIVEAWAY - INU WINS
                _transfer(address(this), topHolder, playAmount);
            }
        }
    }
    ...
    function _transfer(
        ...
        uint256 contractTokenBalance = balanceOf(address(this));    // CHECK
