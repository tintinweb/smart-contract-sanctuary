...
contract ExoticMonsterController is Ownable {
    ...
    //用户英雄
    mapping(uint256 => Hero) public myHero;
    
    //用户投资池
    mapping(uint256 => Pool) public myPool;
    ...
    uint256 public myHeroLength = 0;
    uint256 public myPoolLength = 0;
    ...
    function inPool(uint256 poolid,uint256 amount,uint256 userHeroId) public {
        ...
        // msg.sender 为客户（调用合约）的地址 
        // Hero memory userHero = users[msg.sender].myHeros[userHeroId];
        Hero memory userHero = myHero[users[msg.sender].myHeros[userHeroId]];
        ...
        //更新用户矿池映射数据
        users[msg.sender].myPools[users[msg.sender].poolIds] =  myPoolLength + 1;
        
         //定期 
        if( 1 != poolid){
            myPool[users[msg.sender].myPools[users[msg.sender].poolIds]] = Pool(
                pools[poolid].id,
                userHeroId,
                amount,
                //时间固定，直接计算利息
        ...
        //加入矿池的英雄
        myHero[users[msg.sender].myHeros[userHeroId]].isPooled = true;
    ...
    function outPool(uint256 poolId) public {
        ...
        Pool memory myPool = myPool[users[msg.sender].myPools[poolId]];
        ...
        //解锁加入矿池的英雄
        myHero[users[msg.sender].myHeros[myPool.heroId]].isPooled = false;

        EMC20.transfer(msg.sender,reward);
        
        //删除对应矿池
        // delete myP[users[msg.sender].myPools[poolId]];
    ...
    function selectMonster(uint256 monsterID,uint256 userHeroId) public {
        ...
        if(userHero.usedTimes < userHero.totalTimes){
            ...
            //更新用户英雄数据
            myHero[users[msg.sender].myHeros[userHeroId]] = userHero;
    ...
    //   users[msg.sender].myPools[users[msg.sender].poolIds] =  myPoolLength + 1;
       ...
       users[user].myHeros[users[user].heroIds] = myHeroLength + 1;
       
       myHero[users[user].myHeros[users[user].heroIds]] = heros[heroLevel];
