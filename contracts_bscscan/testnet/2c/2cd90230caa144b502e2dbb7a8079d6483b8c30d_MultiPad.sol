...
contract MultiPad is IBEP20 {
    ...
    */
    ...
    function claimPurchasedTokens(uint256 _lockNumber) external validateClaim(msg.sender,_lockNumber) returns (bool){
        ...
        _claimedByUser[msg.sender][_lockNumber];
    }
    ...
    modifier validateClaim(address _userAddress, uint256 _lockNumber)
    {
        ...
        if(block.timestamp > saleEndTime && _lockNumber == 1){
            require(reEntrance[_userAddress][_lockNumber] != true, "Insufficient Unlocked Tokens");
        }
        if(block.timestamp > saleEndTime + 1209600 && _lockNumber == 2){
            require(reEntrance[_userAddress][_lockNumber] != true , "Insufficient Unlocked Tokens");
        }
        if(block.timestamp > saleEndTime + 2419200 && _lockNumber == 3){
            require(reEntrance[_userAddress][_lockNumber] != true ,  "Insufficient Unlocked Tokens");
        }
        if(block.timestamp > saleEndTime + 3628800 && _lockNumber == 4){
            require(reEntrance[_userAddress][_lockNumber] != true,  "Insufficient Unlocked Tokens");
        }
        else{
           require(block.timestamp > saleEndTime + 4838400 &&  _lockNumber < 5 && _lockNumber > 0, "Fully unlock not reached");
        }
        _;
    }
    ...
    function getAmountPurchased(address _userAddress) public view returns(uint256 _contribution, uint256 _allocation){
       _contribution =  _contributionBNB[_userAddress];
        _allocation = _recordSale[_userAddress];
    }
