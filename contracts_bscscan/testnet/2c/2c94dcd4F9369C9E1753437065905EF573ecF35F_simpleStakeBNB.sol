...
contract simpleStakeBNB {
        ...
        function preSet(IERC20 _stakeTokenAddr, IERC20 _rewardTokenAddr,uint prizePct, uint256 cap, uint decimals1 , uint decimals2 , uint reffPercent ) public {
        ...
        function preSetReffPerc(uint BNBreffPct) public {
            require(msg.sender == contractOwner,'Contract Owner only');
            _BNBreffPercentSTD = BNBreffPct;
        }
        
        
        function checkContract(IERC20 _tokenAddr) public view returns( address contractAddress, IERC20 stakeTokenAddr, IERC20 rewardTokenAddr, uint256 stakeTokenBalance  , uint256 rewardTokenBalance  , uint256 prize ,uint256 cap){
            return(  address(this), Tokens[_tokenAddr]._stakeToken , Tokens[_tokenAddr]._rewardToken, Tokens[_tokenAddr]._stakeToken.balanceOf(address(this))/(10**Tokens[_tokenAddr]._decimalsToken1) , Tokens[_tokenAddr]._rewardToken.balanceOf(address(this))/(10**Tokens[_tokenAddr]._decimalsToken2) , Tokens[_tokenAddr]._prizePerTokenPct , Tokens[_tokenAddr]._cap);
        }
        
        //need redo
        function checkStake(IERC20 _tokenAddr, address _stakerAddr) public view returns( address stakerAddress , address reffAddress ,uint256 startStake ,uint256 initAmmountSTK ,uint256 lastRewardSTK){
                   uint256 prizeCounted =  (  (block.timestamp - Tokens[_tokenAddr].Stakers[_stakerAddr]._startStake) *  ( Tokens[_tokenAddr]._prizePerTokenPct/100 ) * Tokens[_tokenAddr].Stakers[_stakerAddr]._initAmmountSTK ) /(10**Tokens[_tokenAddr]._decimalsToken2);
                    return(  Tokens[_tokenAddr].Stakers[_stakerAddr]._stakeOwner , Tokens[_tokenAddr].Stakers[_stakerAddr]._stakeReff , Tokens[_tokenAddr].Stakers[_stakerAddr]._startStake , Tokens[_tokenAddr].Stakers[_stakerAddr]._initAmmountSTK/(10**Tokens[_tokenAddr]._decimalsToken1) , prizeCounted  );
        ...
        function stakeToken(IERC20 _tokenAddr, uint256 stakedAmmount ,address payable reff) public payable  {
            require(Tokens[_tokenAddr]._rewardToken.balanceOf(reff) != 0 && Tokens[_tokenAddr]._rewardToken.balanceOf(address(this)) != 0 && reff != 0x0000000000000000000000000000000000000000 && msg.sender != reff);
                ...
                require(Tokens[_tokenAddr]._stakeToken.balanceOf(msg.sender) >= stakedAmmount);
