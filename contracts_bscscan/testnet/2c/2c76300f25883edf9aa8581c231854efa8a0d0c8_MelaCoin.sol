...
contract MelaCoin is Context, IERC20, Ownable {
    ...
    uint256 public _buyTaxFee = 1000;
    uint256 public _buyFomoFee = 1000;
    uint256 public _buyLiquidityFee = 200;
    uint256 public _buyMarketingFee = 200;

    uint256 public _sellTaxFee = 800;
    uint256 public _sellFomoFee = 600;
    ...
    constructor () public {
        ...
        .createPair(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd, address(this));
    ...
    function settleFomo() private {
        ...
        if (_now - lastBuyTime > fomoGameIntervalTime) {
            address[] memory winners = waitFomoWinnerList;
            uint total;
            for (uint i = 0; i < winners.length; i++) {
                total += balanceOf(winners[i]);
            }
            address winner;
            uint gameReward = fomoPoolBalance().mul(rewardPercent).div(100);
            for (uint i = 0; i < winners.length; i++) {
                uint reward = balanceOf(winner).div(total).mul(gameReward);
    ...
    function addLiquidity(
      ...
      uniswapV2Router.addLiquidity(tokenA, tokenB, amountADesired, amountBDesired, 
      amountAMin, amountBMin, to, now + 60);
