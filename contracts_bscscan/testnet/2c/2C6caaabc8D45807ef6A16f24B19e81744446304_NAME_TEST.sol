...
// declaration of the library of basic mathematical functions
...
library Address {
    ...
    function sendValue(address payable recipient, uint256 amount) internal {                                        // send token function
...
// funtion who create contract (prototyper)
...
// pragma solidity >=0.5.0;
// interface for V2 on pancakeswap
...
contract NAME_TEST is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public burnAddress = payable(0x0000000000000000000000000000000000000000); // burn address
    ...
    uint256 private _tTotal = 100000  * (10 ** _decimals);                                  // totalSupply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;                                                               

    string private _name = "NAME_TESTd";                                                     // token name
    string private _symbol = "TEST";                                                         // token symbol
    uint8 private _decimals = 10;                                                            // token decimals

    bool public tradingOpen = true;
    struct AddressFee {                                                                      // definition of fees in a struct
        bool enable;
    ...
    uint256 public _maxTxAmount = _tTotal* (10 ** _decimals);
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(burnAddress, transferredBalance.mul(marketingDivisor).div(100));
    ...
    // transfer standard token from sender to recipient
    ...
    // transfert token to a excluded address
    ...
    // transfert token from excluded sender to recipient
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        return (tTransferAmount,1, tLiquidity);
    }
    ...
    function setBuyFee( uint256 buyLiquidityFee) external onlyOwner {
        _buyLiquidityFee = buyLiquidityFee;
    }
   
    function setSellFee( uint256 sellLiquidityFee) external onlyOwner {
    ...
    function setMarketingAddress(address _marketingAddress) external onlyOwner {
        burnAddress = payable(_marketingAddress);
    }
    ...
    // change the address in anticipation of a V3 on pancakeswap
