...
// Welcome to B9 (BNINE) – a social project which came to life in ninety minutes based on a fork and other tokenomics enhancements.
// 
// The token applies a tax on every transaction so to achieve at least:
// 	9% to be added as liquidity;
// 	9% to be used for a reward pool;
// 	2% fee to apply on sell transactions; Half of it will go to liquidity, other half to rewards.
// 
// The crypto market carries an intense volatility and is a high risk environment; DYOR.
// 
// 9,999,999 Total Supply 
// 
// Name: B9
// Symbol: BNINE
// Decimals: 9
...
contract BNINE is BNINEBase {
	...
	// REWARD CYCLE
	uint256 private _rewardCyclePeriod = 9 days; // The duration of the reward cycle (e.g. can claim rewards every nine days)
	...
	uint256 private _gradualBurnTimespan = 9 days; //Burn every 9 days by default
	uint256 private _lastBurnDate; //The last burn date
	uint256 private _minBnbPoolSizeBeforeBurn = 9 ether; //The minimum amount of BNB that need to be in the pool before initiating gradual burns
...
abstract contract BNINEBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	...
	uint256 private constant _totalTokens = 9999999 * 10**DECIMALS;	//9,999,999 total supply
	...
	constructor (address routerAddress) {
		...
		// Initially set values are 9% liquidity fee, 9% reward fee, 2% additional sell fee
		setFees(9, 9, 2);
	...
	function activate() public onlyOwner {
		...
		setTransactionLimit(10000); // only 0.01% of the total supply can be sold at once
		activateBuying();
		onActivated();
	}
	...
	function setFees(uint8 liquidityFee, uint8 rewardFee, uint8 additionalSellFee) public onlyOwner {
		...
		require(rewardFee >= 1 && rewardFee <= 9, "Reward fee must be between 1% and 9%");
		require(additionalSellFee <= 9, "Additional sell fee cannot exceed 9%");
		require(liquidityFee + rewardFee + additionalSellFee <= 27, "Total fees cannot exceed 27%");
