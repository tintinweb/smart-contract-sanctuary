...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accTest111PerShare; // Accumulated TNDRs per share, times 1e12. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
    }

    // The Test111Swap Token!
    TestToken public test111;
    // Dev address.
    address public devAddr;
    // TNDR tokens created per block.
    uint256 public test111PerBlock;
    // Deposit Fee address
    ...
    // The block number when TNDR mining starts.
    ...
    // Initial emission rate: 1 TNDR per block.
    uint256 public constant INITIAL_EMISSION_RATE = 1 ether;
    // Minimum emission rate: 0.5 TNDR per block.
    ...
    constructor(
        ...
        test111PerBlock = INITIAL_EMISSION_RATE;
    ...
    function pendingTest111(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accTest111PerShare = pool.accTest111PerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier =
                getMultiplier(pool.lastRewardBlock, block.number);
            uint256 test111Reward =
                multiplier.mul(test111PerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accTest111PerShare = accTest111PerShare.add(
                test111Reward.mul(1e12).div(lpSupply)
            );
        }
        return user.amount.mul(accTest111PerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        test111.mint(devAddr, test111Reward.div(10));
        test111.mint(address(this), test111Reward);
        pool.accTest111PerShare = pool.accTest111PerShare.add(
            test111Reward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accTest111PerShare).div(1e12).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
                safeTest111Transfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accTest111PerShare).div(1e12);
    ...
    function safeTest111Transfer(address _to, uint256 _amount) internal {
        uint256 test111Bal = test111.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > test111Bal) {
            transferSuccess = test111.transfer(_to, test111Bal);
        } else {
            transferSuccess = test111.transfer(_to, _amount);
        }
        require(transferSuccess, "safeTest111Transfer: transfer failed.");
    }
    ...
    function updateEmissionRate() public {
        ...
        require(test111PerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = test111PerBlock;
        ...
        if (newEmissionRate >= test111PerBlock) {
            return;
        }
        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = test111PerBlock;
        test111PerBlock = newEmissionRate;
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            test111.mint(referrer, refBonusEarned);
