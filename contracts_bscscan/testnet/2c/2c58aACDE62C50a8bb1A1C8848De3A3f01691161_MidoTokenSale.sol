...
import {
    TokenVesting
} from './utils/TokenVesting.sol';


contract MidoTokenSale is IMidoTokenSale, Whitelist, TokenVesting {
    ...
    function claim() external {
        ...
        uint _vestedAmount = vesting(account);
        midoToken.transfer(account, _vestedAmount);
    }
...
contract TokenVesting is AccessControl  {
    ...
    function setVestingStartTime(uint64 _vestingStartTime) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_vestingStartTime > block.timestamp, "Cannot set an vesting start time in past!");
    ...
    */
    function addVesting(address user, uint64 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(vestingStartTime !=0, "vesting time has not been set!");
        totalTokenVesting[user]+=amount;
        lastClaimedTime[user]=vestingStartTime;
    }
    /**
