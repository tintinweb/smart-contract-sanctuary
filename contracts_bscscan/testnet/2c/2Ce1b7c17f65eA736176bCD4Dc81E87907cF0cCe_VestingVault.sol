...
contract VestingVault is Ownable {
        ...
        if (allAllocations.length > 0) {
            ...
            for (uint256 i; i < allAllocations.length; i++) {
                ...
                for (uint256 j; j < groupAllocation.allocations.length; j++) {
                    ...
                    // console.log('allocation', allocation.receiver, allocation.amountPerMille, allocation.revocable);
                    ...
                    if (initial > 0)
                        require(
                            token.transferFrom(msg.sender, allocation.receiver, initial),
                            'could not transfer token'
                        );

                    addAllocation(allocation.receiver, remainder, allocation.revocable);
                }
            }
        }
    }
    ...
    //     uint256 amount,
    //     bool _revocable
    // ) internal onlyOwner onlyBeforeStart {
    ...
    function withdrawToken(IBEP20 _token) external onlyOwner {
        require(address(_token) != address(token), 'cannot withdraw vault token');
