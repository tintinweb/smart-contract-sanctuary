...
contract PolkaMonsterNFT is ERC721 {
    ...
    mapping(uint256 => PolkaMonster) internal moners;
    ...
    function working(uint256 _tokenId, uint256 _time) public onlyFarmer {
        ...
        PolkaMonster storage moner = moners[_tokenId];
        moner.farmTime = moner.farmTime.add(_time);
    ...
    function evolve(
        ...
        moner.bornTime = block.timestamp;
        moner.dna = _dna;
    ...
    function _layEgg(address receiver, Tribe tribe) internal {
        ...
        moners[nextTokenId] = PolkaMonster({
    ...
    function getMoner(uint256 _tokenId)
        public
        view
        returns (PolkaMonster memory)
    {
        return moners[_tokenId];
    }

    function monerLevel(uint256 _tokenId) public view returns (uint256) {
        return getLevel(getMoner(_tokenId).exp);
    }

    function getRare(uint256 _tokenId) public view returns (uint256) {
        uint256 dna = getMoner(_tokenId).dna;
        if (dna == 0) return 0;
    ...
    function fillOrder(uint256 _tokenId) public {
        ...
        monerERC20.transferFrom(_msgSender(), manager.feeAddress(), feeMarket);
        monerERC20.transferFrom(
            _msgSender(),
            itemSale.owner,
