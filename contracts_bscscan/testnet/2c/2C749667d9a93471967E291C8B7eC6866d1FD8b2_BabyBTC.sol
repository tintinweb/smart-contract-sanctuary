...
/*
 * @BabyBTC DevTeam Provides Information:
 * BTC is a cryptocurrency that consumes a lot of electricity. In the future, BabyBTC can ban the status of BTC,
 * change the environment, make the world greener, reduce pollution, and reduce carbon emissions. Each of us should
 * serve the world. Global warming contributed to the withdrawal of BTC from its dominant position. BabyBTC is more
 * environmentally friendly and benefits the world.
 *
...
contract BabyBTC is ERC20, Ownable {
    ...
    uint256 public maxSellTransactionAmount = 2100000000000 * (10**18);
    uint256 public swapTokensAtAmount = 200000000 * (10**18);
    uint256 public maxWalletToken = 21000000000000 * (10**18); // 1.5% of total supply
    ...
    constructor() ERC20("BabyBTC", "BabyBTC") {
        ...
        buyBackWallet = 0xBD74b11b87339291046593eB412C05aaB6D9c00f;
    ...
    function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
        require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "BabyBTC: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
        sellFeeIncreaseFactor = multiplier;
    }
    
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        require(marketingDivisor >= 0 && marketingDivisor <= 100, "BabyBTC: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function prepareForPreSale() external onlyOwner {
        ...
        marketingFee = 0;
        maxBuyTranscationAmount = 2000000000000 * (10**18);
        maxWalletToken = 21000000000000 * (10**18);
    }
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "BabyBTC: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "BabyBTC: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "BabyBTC: Dividend reward tax must be between 0 and 10");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "BabyBTC: The liquidity wallet is already this address");
...
contract BabyBTCDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BabyBTC_Dividend_Tracker", "BabyBTC_Dividend_Tracker") {
        claimWait = 1800;
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "BabyBTC_Dividend_Tracker: claimWait must be updated to between 0.5 and 24 hours");
