...
contract Apollo11 is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 public _maxHoldAmount = _totalSupply.mul(5).div(100); // should be 0.5% percent per transaction
    uint256 public minTokenToSell = _totalSupply.div(100000); // 0.001% max tx amount will trigger swap and add liquidity
    ...
    function setMaxTxPercent(uint256 maxTxAmount) external onlyOwner {
        _maxHoldAmount = _totalSupply.mul(maxTxAmount).div(10000);
    }
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            if (buybackEnabled && address(this).balance > 0) {
