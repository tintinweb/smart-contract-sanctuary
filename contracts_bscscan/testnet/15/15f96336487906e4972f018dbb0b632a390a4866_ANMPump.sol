...
contract ANMPump is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingWalletAddress = payable(0x26C0EF61F77877c34a11c898FaAc652908BDDf79); // Marketing Address
    address public ANM = 0xFc951fe2eba091dA25CB4c8C9079baBC1dD54217; // ANM Address
    ...
    string private _name = "ANMPump";
    string private _symbol = "ANMPump";
    uint8 private _decimals = 9;


    uint256 public _liquidityFee = 4;
    uint256 public _marketingFee = 2;
    uint256 public _ANMbuyBackFee = 6;
    ...
    uint256 public _maxSellTransactionAmount = 5000000 * 10**9;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner() && to != uniswapV2Pair) {
            require(amount <= _maxBuyTransactioAmount, "Transfer amount exceeds the maxBuyTxAmount.");
        }
        ...
        {
            require(amount <= _maxSellTransactionAmount, "Transfer amount exceeds the maxSellTxAmount.");
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forMarketing > 0 )
        {
         ...
         if(forANMbuyBack > 0) {
                swapTokensForANM(forANMbuyBack);
        }
        
    }
    
    function swapTokensForANM(uint256 tokenAmount) private {
    ...
    function setTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newBuyBackFee) external onlyOwner() {
        _liquidityFee = newLiquidityTax;
        _marketingFee = newMarketingTax;
        _ANMbuyBackFee = newBuyBackFee;
        _totalTaxPercent = _liquidityFee.add(_marketingFee).add(_ANMbuyBackFee);
        _prevTotalTaxPercent = _totalTaxPercent;
    }
    ...
    function prepareForPreSale() external onlyOwner {
        ...
        _maxBuyTransactioAmount = 1000000000 * 10**9;
        _maxSellTransactionAmount = 1000000000 * 10**9;
    }
