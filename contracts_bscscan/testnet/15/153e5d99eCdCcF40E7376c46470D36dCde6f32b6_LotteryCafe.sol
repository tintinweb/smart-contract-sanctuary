...
contract LotteryCafe is OwnableUpgradeable {
    ...
    function getRewards(address _user) external view returns (uint256 rewards) {
        ...
        if (block.number > lastRewardsBlock) {
            ...
            uint256 multiplier = lastRewardsBlock - block.number;
    ...
    function getApr() external view returns (uint256 apr) {
        ...
        uint256 rewardsPerYearInStable = rewardsPerYear.mul(reserveStable).div(reserveLotto);
    ...
    function stake(uint256 _amount) external {
        ...
        if (user.shareAmount > 0) {
            uint256 pending = user
                ...
                .sub(user.rewardDebt);
            if (pending > 0) {
                lotto_.mint(msg.sender, pending);
            }
        }
        if (_amount > 0) {
