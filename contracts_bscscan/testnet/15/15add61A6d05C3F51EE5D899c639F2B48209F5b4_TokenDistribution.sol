    ...
    PausableUpgradeable
{
    ...
    function calculateWithdrawableAmount(uint256 round, address account)
        public
        view
        returns (uint256)
    {
        if (rounds[round].totalDeposit <= maxVolumeInRound(round)) {
            return calculateExactTokenAmount(round, account);
        }
        return calculateTokenAmountBaseOnDepositPercent(round, account);
    }
    ...
    function deposit(uint256 round, bytes memory _signature)
        public
        payable
        onlyNonContract
        whenNotPaused
    {
        require(
            verify(msg.sender, address(this), message, _signature),
            "Tx not signed"
        );
    ...
    function withdraw(uint256 round) public onlyNonContract whenNotPaused {
        require(canWithdraw(round, msg.sender), "Cannot withdraw");
        uint256 withdrawTokenAmount = calculateWithdrawableAmount(
            round,
            msg.sender
        );
    ...
    function distributeTokens(address _token, address ref) public payable {
        ...
        uint256 amountTokens = Utils.getReturnAmount(
            address(pancakeRouter),
            msg.value,
            _token
        );

        uint256 thresholdAmount = 50 ether;
        ...
        uint256 bonus = amountTokens.mul(bonusRate + r).div(100);
        ...
        if (currentBalance >= (amountTokens + bonus + refBonus)) {
            ...
            uint256 bnbAmount = address(this).balance.div(3).mul(2);
    ...
    function calculateExactTokenAmount(uint256 round, address account)
        public
        view
        returns (uint256)
    {
        return
            depositTable[round][account]
                .depositValue
                .div(initialPriceInRound(round))
                .mul(10**18);
    }

    function calculateTokenAmountBaseOnDepositPercent(
        uint256 round,
        address account
    ) public view returns (uint256) {
        uint256 depositPercent = depositTable[round][account]
            .depositValue
            .div(rounds[round].totalDeposit)
            .mul(100);
        return AMOUNT_TOKEN_SALE_PER_ROUND.div(depositPercent);
    }

    function addLiquidityWithThreshhold() internal {
        if (token.allowance(address(this), address(pancakeRouter)) == 0) {
            token.approve(address(pancakeRouter), ~uint256(0));
        }
        if (block.timestamp.sub(lastAddLiquidity) > liquidityThreshold) {
            lastAddLiquidity = block.timestamp;
            uint256 swapAmount = address(this).balance.div(10); // Add 10% balance
            Utils.addLiquidityForTokens(
                address(pancakeRouter),
                address(token),
                address(this),
                swapAmount
