...
contract ElpisBattle is BEP20("Elpis Battle", "EBA") {
    ...
    modifier capNotExceeded(uint256 _amount) {
        require(BEP20.totalSupply() + _amount <= cap(), "EBA::cap exceeded");
        _;
    }
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "EBA::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "EBA::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "EBA::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "EBA::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying EBAs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "EBA::_writeCheckpoint: block number exceeds 32 bits"
        );
