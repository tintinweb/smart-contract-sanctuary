...
contract GOO is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public _minimumTokenHold = 2.5 * 10**9 * 10**2;
    ...
    function claimReward() public nonReentrant {
        ...
        require(
            balanceOf(msg.sender) >= _minimumTokenHold,
    ...
    function _transfer(
        ...
        bool flagfrom=false;
        bool flagto=false;
        if(balanceOf(from) >= _minimumTokenHold||from!=owner()){
            flagfrom=true;
        }
        if(balanceOf(to) < _minimumTokenHold||to!=owner()){
            flagfrom=true;
        }

        _tokenTransfer(from, to, amount, takeFee);
        if (to != deadAddress) {
            if (flagto && to != uniswapV2Pair && balanceOf(to) >= _minimumTokenHold) {
                nHolders = nHolders + 1;
            }
        }

        if (flagfrom && from != uniswapV2Pair && balanceOf(from) < _minimumTokenHold) {
            nHolders = nHolders - 1;
        }
    }
    ...
    function setMinimumTokenHold(uint256 minimumTokenHold) external onlyOwner {
        _minimumTokenHold = minimumTokenHold;
    }
