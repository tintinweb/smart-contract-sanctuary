...
contract LaveToken is BEP20 {
     ...
     */
    constructor() public BEP20("Lave Token", "LAVE11") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
		...
		if (limitSwap == true && _excludedPoolsContracts[sender] == false && _excludedPoolsContracts[recipient] == false) {	
			...
			if (userAddress != address(0) && delaySell == 0){
			...
			else if (userAddress != address(0) && delaySell > 0){
				uint256 lastSwap = _userInfo[userAddress];
				uint256 checkLastSwap = block.number.sub(lastSwap);
				if(checkLastSwap >= timeLimitSwap){
					_userInfo[userAddress] = block.number.sub(timeLimitSwap.sub(delaySell));
				}
			}			
		}
