...
contract Mercenary is ERC721Enumerable,Ownable {
    ...
    bool public _reward = false;
    bool public _wiliam = false;
    uint256 public _abReward = 100;
    uint256 public _abOpen = 10;
    event OpenEgg(
    ...
    function setLidis(bool reward) public onlyOwner{
        _reward = reward;
    }
    function setWiliam(bool wiliam) public onlyOwner{
        _wiliam = wiliam;
    }
    function setAbReward(uint256 abReward) public onlyOwner{
        _abReward = abReward;
    }
    function setAbOpen(uint256 abOpen) public onlyOwner{
        _abOpen = abOpen;
    }
    ...
    function openEggAndAward() public returns (uint256 newItemId)  {
        ...
        uint256 numrd = random(msg.sender, _abOpen);
        if(numrd != 9){
            if(!_reward){
                return numrd;
            }
            uint256 reward = random(msg.sender, _abReward)  * 1e18;
            _token.transfer(msg.sender, reward);
            emit OpenEggReward(msg.sender,reward);
            return reward;
        }
    ...
    function random(address send, uint256 mod) internal returns (uint) {
        return uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, send))) % mod;
    }

    // function openEggsAndAwards(uint quantityEggs) public returns (uint[] memory _ids)  {
    //     uint256 numrd = random(msg.sender, 10);
    //     if(numrd !== 9){
    //         return (numrd);
    //     }
