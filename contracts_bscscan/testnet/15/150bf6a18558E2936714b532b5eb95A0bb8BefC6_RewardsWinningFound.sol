...
contract RewardsWinningFound is Ownable { 
    ...
    function _updateRewards() private {
        address reserve = gameAddresses.getReserveWinAddress();
        if(IReserveForWinRewards(reserve).getRewardFinished()){
            reserveFinished = true;
            return;
        } else if(block.timestamp >= IReserveForWinRewards(reserve).getNextUpdateTimestamp()){
             IReserveForWinRewards(reserve).updateRewards();
        } else {
            return;
        }
    }
    ...
    function getFightingWinRewards() external onlyGame returns(uint256){
        _updateRewards();
        uint256 _balance = BZAI.balanceOf(address(this)).div(1000);
        uint256 _toSend = _balance.div(1000); // 0.1%
