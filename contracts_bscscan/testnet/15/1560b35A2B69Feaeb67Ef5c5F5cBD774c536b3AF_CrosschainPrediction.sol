...
contract CrosschainPrediction {
    ...
    constructor (address _pye,address _owner) {
        pye = Pye(_pye);
        owner = _owner;
    }
    ...
    function polling(
      ...
      uint _flag
         ...
         pye.transferFrom(msg.sender,address(this),_amount);
    ...
    function claim (
        ...
        pye.transfer(msg.sender,getResult(_poolid,msg.sender));
        }
    ...
    function failSafe(address  _toUser, uint _amount) public onlyOwner returns(bool) {
        ...
        require(pye.balanceOf(address(this)) >= _amount, "PICNIC: Insufficient balance");
        pye.transfer(_toUser,_amount);
