...
contract MiraiUniverse is Context, IERC20, Ownable {
    ...
    string private _name = "Mirai Universe";
    string private _symbol = "MUNIT";
	...
	function _getdevWallet() public view virtual returns (address) {
        return _devWallet;
	}
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tMarketing, data.tDev);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tMarketing).sub(tDev);
        return (tTransferAmount, TData(0, tFee, tLiquidity, tMarketing, tDev, 0));
    }
