...
contract PMPTSOL is Context,Ownable {
    ...
    uint256 public token_decimals = 8;
     ...
     modifier lock_period() {
        require(block.timestamp > (balance[msg.sender].timestamp + withdraw_interval),"You cannot withdraw token right now");
        _;
    }
    ...
    function withdrawToken(address receipient) public payable lock_period(){        
