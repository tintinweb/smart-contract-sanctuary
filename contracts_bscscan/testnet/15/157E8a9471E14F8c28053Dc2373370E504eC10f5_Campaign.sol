...
contract Campaign is Ownable, Initializable, Pausable, ERC1155Holder {
    ...
    uint256 private specialBoxSize = 15;

    mapping(uint256 => CampaignInfo) private campaigns;
    mapping(address => bool) private whiteListUsers;
    mapping(uint256 => address) private mappingQuoteErc20s; // 0 - BNB, >0 QuoteToken
    uint256 private currentCampaignId = 0;
    mapping(address => bool) public mappingBuySpecialBox;
    ...
    function setSpecialBoxSize(uint256 _specialBoxSize) external onlyOwner {
        require(_specialBoxSize >= 4, "Special Box Size Must Be Greater Or Equal 4");
        specialBoxSize = _specialBoxSize;
    }
    ...
    function createSpecialBox(
        ...
        require(_mysteryBoxIds.length == _mysteryBoxQuantities.length, 'Invalid Input');
        require(_start < _end && _start > block.timestamp, 'Invalid Time');
        require(_quoteErc20Key == 0 || (mappingQuoteErc20s[_quoteErc20Key] != address(0)), 'Invalid Quote Token');
    ...
    function buySpecialBox(uint256 _campaignId) public payable whenNotPaused {
        CampaignInfo storage campaign = campaigns[_campaignId];
        require(campaign.start <= block.timestamp && campaign.end >= block.timestamp, 'Can Not Buy At This Time!');
        ...
        for (uint256 i = 0; i < campaign.mysteryBoxIds.length; i++) {
            require(campaign.mysteryBoxInfo[campaign.mysteryBoxIds[i]].quantity >= specialBoxSize - 3, 'Out Of Box');
        }
        ...
        mappingBuySpecialBox[_msgSender()] = true;

        uint256 randNum1 = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, msg.sender)));
        uint256 randNum2 = uint256(keccak256(abi.encodePacked(block.number, block.timestamp, msg.sender)));
        uint256 randNum3 = uint256(keccak256(abi.encodePacked(block.coinbase, block.timestamp, msg.sender)));

        uint256 numBox1 = randNum1.mod(specialBoxSize - 3) + 1;
        uint256 numBox2 = randNum2.mod(specialBoxSize - numBox1 - 2) + 1;
        uint256 numBox3 = randNum3.mod(specialBoxSize - numBox1 - numBox2 - 1) + 1;
        uint256 numBox4 = specialBoxSize - numBox1 - numBox2 - numBox3;

        uint256[] memory quantities = new uint256[](4);
        quantities[0] = numBox1;
        quantities[1] = numBox2;
        quantities[2] = numBox3;
        quantities[3] = numBox4;

        for (uint256 i = 0; i < campaign.mysteryBoxIds.length; i++) {
            campaign.mysteryBoxInfo[campaign.mysteryBoxIds[i]].quantity = campaign.mysteryBoxInfo[campaign.mysteryBoxIds[i]].quantity.sub(quantities[i]);
        }
    ...
    function withdrawCampaign(uint256 campaignId) public onlyWhiteListUser {
        ...
        emit WithdrawRedundantMysteryBox(_msgSender(), campaignId, withdrawIds, withdrawQuantities);
    }
