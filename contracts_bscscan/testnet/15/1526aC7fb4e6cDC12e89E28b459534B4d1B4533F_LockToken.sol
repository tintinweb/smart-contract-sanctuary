...
contract LockToken is Ownable {
    ...
    function checkClaim(address _userAddress, address _tokenAddress)
        public
        view
        returns (
            uint256 balance,
    ...
    {
        Token[] storage tokens = userBalance[_userAddress];
    ...
    function claim(address _tokenAddress) external {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (t.tokenAddress == _tokenAddress) {
                ...
                t.tokenAmount = t.tokenAmount.sub(claimAmount);
                t.lastClaim = block.number;
                tokenExist = true;
            }
        }
