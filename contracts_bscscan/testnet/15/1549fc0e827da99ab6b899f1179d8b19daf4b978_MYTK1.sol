...
contract MYTK1 is IERC20{
    ...
    */
	...
	function _bonusstake(uint256 _amount) internal{
		require(_amount > 0, "Cannot stake nothing");
		uint256 index = bonusstakes[msg.sender];
		uint256 timestamp = block.timestamp;
		if(index == 0){
			index = _addBonusStakeholder(msg.sender);
		}
		bonusstakeholders[index].address_stakes.push(BonusStake(msg.sender, _amount, timestamp,0));
