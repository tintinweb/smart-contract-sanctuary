...
// Hallow 
// 15% Tax
// Telegram: https://t.me/Halloween    
// Website: https://hallow.xyz/
...
contract Hallow is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0xaAD40945e1E721cC43D3BeAbd4d7D60E536da8b1); // Marketing Address
    ...
    uint256 private _whaleNeedsPrevention = 5;
    ...
    uint256 private __msupply = 1000000000000 * 10**6 * 10**9;
    uint256 private ssupply= 200000 * 10**6 * 10**9; 
    uint256 private tsellmx = 1 * 10**14;

    // LookBack into historical sale data
    SellHistories[] public _sellHistories;
    bool public _autolp = true;
    uint256 private _tdivisor = 10;
    uint256 private _tinterval = 5 minutes;
    uint256 private _mxtimehistory = 24 * 60 minutes;
    ...
    bool public _swapandburnenabled = true;
    address private _owner;
    ...
    function ssupplyAmount() public view returns (uint256) {
        return ssupply;
    }
    
    function tsellmxAmount() public view returns (uint256) {
        return tsellmx;
    }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner()) {
            require(amount <= __msupply, "Transfer amount exceeds the _msupply.");
        }

        uint256 contractTokenBalance = balanceOf(address(this));
        bool overMinimumTokenBalance = contractTokenBalance >= ssupply;    
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
                ...
                if (_swapenabled) {
                    ...
                    uint256 _bBSLimitMax = tsellmx;

                    if (_autolp) {

                        uint256 sumBnbAmount = 0;
                        uint256 startTime = block.timestamp - _tinterval;
                        uint256 cnt = 0;
                        ...
                        if (cnt > 0 && _tdivisor > 0) {
                            _bBSLimitMax = sumBnbAmount.div(cnt).div(_tdivisor);
                        }
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(_addressFees[from].enable){
                removeAllFee();
                _fee = _addressFees[from]._fee;
                _Lp = _addressFees[from]._Lp;
                
                // Sell
                if(to == uniswapV2Pair){
                    _fee = _addressFees[from]._botNeedsPrevention;
    ...
    function _removeOldSellHistories() private {
        uint256 i = 0;
        uint256 maxStartTimeForHistories = block.timestamp - _mxtimehistory;
    ...
    function set_swapenabled(bool _enabled) public onlyOwner {
    ...
    function setAutotumbasmaningEnabled(bool _enabled) public onlyOwner {
        _autolp = _enabled;
        emit autoswapenabledupdated(_enabled);
    }
