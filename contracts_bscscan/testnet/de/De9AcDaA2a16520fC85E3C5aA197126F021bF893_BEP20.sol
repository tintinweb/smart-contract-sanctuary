...
contract BEP20 is Context, IBEP20, IBEP20Metadata, Ownable, Pausable {
    ...
    uint256 public rewardLimit = 25000 * 10**18;
    ...
    function bridgeBurn(address from, uint amount) external virtual returns (bool) {
        ...
        require(amount > 0, "Transfer amount must be greater than zero");
        require(_rOwned[from] >= amount, "BEP20: burn amount exceeds balance");
    ...
    function _transfer(
        ...
        if(amount >= transactionLimit && from != pancakeswapV2Pair){
    ...
    function _swapAndLiquify(address from) internal {
        if(_marketingFeeBalance >= maxTokensToSell && from != pancakeswapV2Pair) {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual {
        ...
        require(from != SUPPLY_HOLDER, "BEP20: transfer from the supply address");
        require(to != SUPPLY_HOLDER, "BEP20: transfer to the supply address");
    ...
    function removeLiquidityFromPlatform(uint256 liquidity) external virtual {
        ...
        bool isTransfered = IPancakePair(pancakeswapV2Pair).transferFrom(msg.sender, address(this), liquidity);
        require(isTransfered == true, "PancakePair: transferFrom failed");

        // approve router address
        bool isApproved = IPancakePair(pancakeswapV2Pair).approve(PANCAKEV2ROUTER, liquidity);
        require(isApproved == true, "PancakePair: approve failed");
