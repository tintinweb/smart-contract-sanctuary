...
contract BabyUnicorn is ERC20, Ownable {
    ...
    constructor() ERC20("BabyUnicorn", "BabyUnicorn") {
    ...
    function setTokenSwapThreshold(uint256 amount) external onlyOwner {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "BabyUnicorn: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "BabyUnicorn: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "BabyUnicorn: A swapping process is currently running, wait till that is complete");
    ...
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "BabyUnicorn: Dividend reward tax must be between 0 and 10");
        marketingFee = newFee;
    }
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "BabyUnicorn: The liquidity wallet is already this address");
