...
interface ComptrollerLensInterface {
    ...
    function claimApple(address) external;
...
contract AppleLens {
    ...
    struct AppleBalanceMetadata {
        uint balance;
        uint votes;
        address delegate;
    }

    function getAppleBalanceMetadata(APPLE apple, address account) external view returns (AppleBalanceMetadata memory) {
        return AppleBalanceMetadata({
            balance: apple.balanceOf(account),
            votes: uint256(apple.getCurrentVotes(account)),
            delegate: apple.delegates(account)
        });
    }

    struct AppleBalanceMetadataExt {
        uint balance;
        uint votes;
        address delegate;
        uint allocated;
    ...
    function getAppleBalanceMetadataExt(APPLE apple, ComptrollerLensInterface comptroller, address account) external returns (AppleBalanceMetadataExt memory) {
        uint balance = apple.balanceOf(account);
        comptroller.claimApple(account);
        uint newBalance = apple.balanceOf(account);
        uint accrued = comptroller.venusAccrued(account);
        uint total = add(accrued, newBalance, "sum apple total");
        uint allocated = sub(total, balance, "sub allocated");

        return AppleBalanceMetadataExt({
            balance: balance,
            votes: uint256(apple.getCurrentVotes(account)),
            delegate: apple.delegates(account),
            allocated: allocated
    ...
    function getAppleVotes(APPLE apple, address account, uint32[] calldata blockNumbers) external view returns (AppleVotes[] memory) {
        AppleVotes[] memory res = new AppleVotes[](blockNumbers.length);
        for (uint i = 0; i < blockNumbers.length; i++) {
            res[i] = AppleVotes({
                blockNumber: uint256(blockNumbers[i]),
                votes: uint256(apple.getPriorVotes(account, blockNumbers[i]))
            });
        }
        return res;
    }
...
contract ABep20 is AToken, ABep20Interface {
     ...
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param receiver the account which is receiving the aTokens
     * @param mintAmount The amount of the underlying asset to supply
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
...
contract GovernorAlpha {
    ...
    string public constant name = "Apple Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public pure returns (uint) { return 600000e18; } // 600,000 = 2% of APPLE

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public pure returns (uint) { return 300000e18; } // 300,000 = 1% of APPLE
    ...
    /// @notice The address of the Apple Protocol Timelock
    TimelockInterface public timelock;

    /// @notice The address of the Apple governance token
    APPLEInterface public xvs;
