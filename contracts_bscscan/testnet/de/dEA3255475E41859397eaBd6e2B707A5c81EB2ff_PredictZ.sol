...
contract PredictZ is Ownable {
    ...
    constructor (address token_, address operatorAddress_,address treasuryAddress_) {
        token = IERC20(token_);
    ...
    function setFee(uint256 fee_) external onlyOwner {
        require(fee_>= 0 && fee_ <= 20,"invalid fee");
        fee = fee_;
    }

    function setOperatorAndTreasuryAddress(address operatorAddress_,address treasuryAddress_) external onlyOwner {
        require(operatorAddress_ != address(0), "Cannot be zero address");
        require(treasuryAddress_ != address(0), "Cannot be zero address");
        operatorAddress = operatorAddress_;
        treasuryAddress = treasuryAddress_;
    }
    ...
    function closeMatch(uint256 matchId,Result result, uint256 scoreTeamA, uint256 scoreTeamB) external onlyOperator{
        ...
        require(result!=Result.Pending,"PredictZ: results cannot be pending");
        matches[matchId].result = result;
        ...
        if (result!=Result.Cancelled) {
            ...
            IERC20(token).safeTransfer(treasuryAddress,(totalBet*fee)/100);
        }
    ...
    function bet(uint256 matchId, uint256 teamId,uint256 amount) external {
        ...
        emit Bet(msg.sender,matchId,teamId,amount);
    }
    ...
    function viewUserMatches(address _user,uint256 _cursor, uint256 _size) external view returns (
        ...
        uint256[] memory teamABets_ = new uint256[](length);
        uint256[] memory teamBBets_ = new uint256[](length);
        uint256[] memory drawBets_ = new uint256[](length);
        bool[] memory cliams_ = new bool[](length);

        for (uint256 i = 0; i < length; i++) {
            uint256 newCursor = i + _cursor;
            teamABets_[i] = userBets[matches[newCursor].id][_user][matches[newCursor].teamAId];
            teamBBets_[i] = userBets[matches[newCursor].id][_user][matches[newCursor].teamBId];
            drawBets_[i] = userBets[matches[newCursor].id][_user][0];
            cliams_[i] = userClaims[matches[newCursor].id][_user];
        }

        return (teamABets_,teamBBets_,drawBets_,cliams_);
    }
}