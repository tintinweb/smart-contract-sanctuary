...
abstract contract Windmill_Trade_Deployer_Abstract is Base, Initializable, Adressable, AccessControlEnumerable, With_DAORole, With_UpdaterRole,
    ...
    ////
    //////////////// Public functions ////////////////
	function setTradeCode(bytes memory _bytecode, string memory _name) external onlyRole(DAO_ROLE) onlyNotInitialized(0){
        name = _name;
...
abstract contract Windmill_Trade_Deployer_Borrow is Base, Math, Windmill_Trade_Deployer_Swap, I_Windmill_Trade_Deployer_Borrow{
	...
	function applyProposal(uint256 id, uint256 paramsUint256, address paramsAddress) public override(I_Windmill_Trade_Deployer_Abstract, Windmill_Trade_Deployer_Swap) onlyRole(DAO_ROLE){
		...
		if (id == 10){
			...
			uint256 lastI = authorizedTokens.length-1;
			
			if (paramsUint256 != lastI){
				authorizedTokens[paramsUint256] = authorizedTokens[lastI];
			}
