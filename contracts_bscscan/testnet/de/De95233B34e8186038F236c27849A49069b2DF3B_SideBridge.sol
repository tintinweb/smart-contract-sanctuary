...
contract SideBridge {
    ...
    function bridgeTokens (address _requester, uint _bridgedAmount, bytes32 _mainDepositHash) verifyInitialization onlyGateway  external {
        ...
        if (bridgeFee > 0) {
          // automatically swap some of the fee to be used as gas
          // for future transactions, the rest goes to the fee collector
          if (autoLiquidateForGasEnabled) {
              ...
              if (autoLiqAmount > maxAutoLiqAmount) {
                autoLiqAmount = maxAutoLiqAmount;
              }
              if (autoLiqAmount > 0) {
                sideToken.mint(gateway, autoLiqAmount);
                swapTokensForEth(autoLiqAmount);
              }
