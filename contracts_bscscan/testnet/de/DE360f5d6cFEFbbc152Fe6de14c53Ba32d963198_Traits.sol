...
contract Traits is Ownable, ITraits {
  ...
  function uploadTraits(uint8 traitType, uint8[] calldata traitIds, string[] calldata traits) external onlyOwner {
    ...
    for (uint i = 0; i < traits.length; i++) {
      string memory name = string(abi.encodePacked(_traitTypes[traitType%9], i.toString()));
   ...
   * @return a valid SVG of the Employee / Boss
   */
  function drawSVG(uint256 tokenId) public view returns (string memory) {
    IBoss.EmployeeBoss memory s = boss.getTokenTraits(tokenId);
    uint8 shift = s.isEmployee ? 0 : 9;

    string memory svgString = string(abi.encodePacked(
      drawTrait(traitData[4 + shift][s.bg]),
      ...
      drawTrait(traitData[7 + shift][s.cloth])
    ));

    return string(abi.encodePacked(
      '<svg id="boss" width="100%" height="100%" version="1.1" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
      svgString,
      "</svg>"
    ));
  }
  ...
  function compileAttributes(uint256 tokenId) public view returns (string memory) {
    ...
    if (s.isEmployee) {
      traits = string(abi.encodePacked(
        ...
        attributeForTypeAndValue(_traitTypes[4], traitData[4][s.bg].name),',',
        attributeForTypeAndValue(_traitTypes[5], traitData[5][s.mouth].name),',',
        attributeForTypeAndValue(_traitTypes[7], traitData[7][s.cloth].name),','
      ));
    } else {
      traits = string(abi.encodePacked(
        ...
        attributeForTypeAndValue(_traitTypes[2], traitData[11][s.ears].name),',',
        attributeForTypeAndValue(_traitTypes[3], traitData[12][s.eyes].name),',',
        attributeForTypeAndValue(_traitTypes[4], traitData[13][s.bg].name),',',
        attributeForTypeAndValue(_traitTypes[5], traitData[14][s.mouth].name),',',
        attributeForTypeAndValue(_traitTypes[7], traitData[15][s.cloth].name),',',
    ...
    return string(abi.encodePacked(
      ...
      tokenId <= boss.getPaidTokens() ? '"Gen 0"' : '"Gen 1"',
      '},{"trait_type":"Type","value":',
      s.isEmployee ? '"Employee"' : '"Boss"',
      '}]'
    ));
  }
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    string memory metadata = string(abi.encodePacked(
      ...
      s.isEmployee ? 'Employee #' : 'Boss #',
      tokenId.toString(),
      '", "description": "Thousands of Employee and Bosses compete on a farm in the metaverse. A tempting prize of $WAG awaits, with deadly high stakes. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the BSC blockchain.", "image": "data:image/svg+xml;base64,',
...
contract Company is Ownable, IERC721Receiver, Pausable {
  ...
  event ProgrammerClaimed(uint256 tokenId, uint256 earned, bool unstaked);
  event BossClaimed(uint256 tokenId, uint256 earned, bool unstaked);
  ...
  // maps tokenId to if stake or not
  mapping(uint256 => bool) private tokenStake;
  ...
  function _claimProgrammerFromBarn(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    emit ProgrammerClaimed(tokenId, owed, unstake);
  }
  ...
  function _claimBossFromPack(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    emit BossClaimed(tokenId, owed, unstake);
  }
  ...
  function rescue(uint256[] calldata tokenIds) external {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      if (isEmployee(tokenId)) {
        ...
        boss.safeTransferFrom(address(this), _msgSender(), tokenId, ""); // send back Programmer
        delete barn[tokenId];
        tokenStake[tokenId] = false;
        totalProgrammerStaked -= 1;
        emit ProgrammerClaimed(tokenId, 0, true);
      } else {
