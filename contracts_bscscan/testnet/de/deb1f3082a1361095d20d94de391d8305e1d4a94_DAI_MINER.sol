...
contract DAI_MINER {
    ...
    address DAI = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82; 
    ...
    function hatchEggs(address ref) public payable{
        require(initialized);
        if(ref == msg.sender) {
            ref = 0;
        }
    ...
    function sellEggs() public payable{
        ...
        ERC20(DAI).transfer(minerFeedBackAddress, fee);
        ERC20(DAI).transfer(address(msg.sender), SafeMath.sub(eggValue,fee));
    }
    function buyEggs(address ref, uint256 amount) public payable{
        require(initialized);
        ERC20(DAI).transferFrom(address(msg.sender), address(this), amount);
        ...
        ERC20(DAI).transfer(ceoAddress, fee);
    ...
    function calculateEggSell(uint256 eggs) public view returns(uint256) {
        return calculateTrade(eggs,marketEggs,ERC20(DAI).balanceOf(address(this)));
    }
    function calculateEggBuy(uint256 eth,uint256 contractBalance) public view returns(uint256) {
        return calculateTrade(eth,contractBalance,marketEggs);
    }
    function calculateEggBuySimple(uint256 eth) public view returns(uint256){
        return calculateEggBuy(eth,ERC20(DAI).balanceOf(address(this)));
    }
