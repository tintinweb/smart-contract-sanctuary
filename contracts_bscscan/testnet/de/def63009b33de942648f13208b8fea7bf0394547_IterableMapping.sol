...
contract RoarTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("RoarToken_Dividend_Tracker", "RoarToken_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "RoarToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "RoarToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RoarToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "RoarToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "RoarToken_Dividend_Tracker: Cannot update claimWait to same value");
...
Symbol: ROAR
6% buy tax
8% sell tax - 2% burn wallet 4% automatic rewards
2% liquidityfee
100 billion total supply 100,000,000,000
*/
contract RoarToken is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    RoarTokenDividendTracker public dividendTracker;
    ...
    function setBurnWallet(address payable _newBuybakWallet) public onlyOwner {
        BurnWallet = _newBuybakWallet;
    }
/**
                Buy	ActualSell      Sell (After Add) 		Additon on Sell
Marketing 	    2	2	            3	                    4
Burn 	        3	3	            4	
Liquidity 	    1	1	            2	
Distribtions 	2	2	            3	
...
*/
    constructor() ERC20("Roar", "ROAR") {
        ...
        BurnFee = 3;
/**
Pump and burn-     0xbe091dfaC7261a8aD69f9AB16B98BA986DB4848F
Marketing and dev- 0x4aeC8248AA9416E12F3e468fCb3FD4B2E94ac747
*/

        marketingWallet = payable(0x4aeC8248AA9416E12F3e468fCb3FD4B2E94ac747);
        BurnWallet = payable(0xbe091dfaC7261a8aD69f9AB16B98BA986DB4848F);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(BurnFee); // total fee transfer and buy

    	dividendTracker = new RoarTokenDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "RoarToken: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "RoarToken: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "RoarToken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "RoarToken: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "RoarToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "RoarToken: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee).sub(BurnFee)).mul(marketingFee);
        marketingWallet.transfer(marketingAmount);
        
         uint256 buybackAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee).sub(marketingFee)).mul(BurnFee);
        BurnWallet.transfer(buybackAmount);
    ...
    function swapAndSendBNBToBuybak(uint256 tokenAmount) private {
        swapTokensForBnb(tokenAmount, BurnWallet);
    }
