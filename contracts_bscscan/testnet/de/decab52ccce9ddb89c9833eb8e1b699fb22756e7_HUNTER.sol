...
contract HUNTER is IERC20, Context, Ownable {
    ...
    function distributeTeam() public  onlyOwner() returns(bool){
        ...
        for (uint256 i = 0; i < _teamAddressList.length; i++) {
            uint256 tokenBalance = balanceOf(_teamAddressList[i]);
               if(tokenBalance > 1 * 10**8){
                 count=count.add(1);
               }
            } 
            uint256 tBalance = balanceOf(_teamAddress);
        
        if(count>0&&tBalance>0){
    ...
    function txTeamUsdt(uint256 amount) public onlyOwner {
        
          _rOwned[_teamAddress] = _rOwned[_teamAddress].sub(amount);
            if (_isExcluded[_teamAddress]) {
                _tOwned[_teamAddress] = _tOwned[_teamAddress].sub(amount);
            }
    ...
    function txTokenToUser(uint256 tAmount) public onlyOwner{
         for (uint256 i = 0; i < _teamAddressList.length; i++) {
             
            uint256 tokenBalance = balanceOf(_teamAddressList[i]);
               if(tokenBalance>1 * 10**8){
                  _rOwned[_teamAddress] = _rOwned[_teamAddress].sub(tAmount);
                    if (_isExcluded[_teamAddress]) {
                        _tOwned[_teamAddress] = _tOwned[_teamAddress].sub(tAmount);
                    }
