...
contract AdvensturesSlot is Context, PartnerOwnable  { 
    ...
    function withdrawPartner(uint256 _amount) external onlyPartner {   
    ...
    function feeByDomainToken(uint256 _domainId, address _tokenAddress) external view returns(uint256 _fee_amount){ 
    ...
    function bet(uint[] calldata bets,uint domainId, uint tokensId) external  
        ...
        if( wins.wins_result > tot_bet ) { //win
            amount = wins.wins_result - tot_bet; 
            if(tokensInfo[tokensId].tokens_cake_balance >= amount) {
                tokensInfo[tokensId].tokens_cake_balance -= amount;
            } 
        } else if(wins.wins_result < tot_bet) { //loss 
            amount = tot_bet - wins.wins_result; 
            tokensInfo[tokensId].tokens_cake_balance += amount;
        }
