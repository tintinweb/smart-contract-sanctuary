...
contract GEMUNIMarketplace is IGEMUNIMarketplace, PermissionGroupUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IERC721 public gemuniItem;
    IERC721 public geniPass;
    IERC20 public geni;
     ...
     */
    function setGemUniItem(address _gemuniItem) external onlyOwner {
        require(_gemuniItem != address(0), "GEMUNIMarketplace: new address must be different address(0)");
        gemuniItem = IERC721(_gemuniItem);
    }   

    function setFeeRate(uint value) external onlyOwner {
        require(value < 100, "GEMUNIMarketplace: invalid rate");
        feeRate = value;
    }
    ...
    function getPricePass(uint passId, uint price, IGENIPass.PassType passType) public override view returns(uint pricePass) {
        bool isExistedToken = IGENIPass(address(geniPass)).exists(passId);
        require(isExistedToken, "GEMUNIMarketplace: invalid tokenId");
        if(price != 0) {
            pricePass = price;
        } else {
    ...
    function putOnSale(address token, uint tokenId, uint price, uint startTime, uint expirationTime) external override whenNotPaused {
        ...
        if(token == address(geniPass)) {
            IGENIPass.PassType passType = IGENIPass(address(geniPass)).getPass(tokenId).passType;

            geniPass.transferFrom(msg.sender, address(this), tokenId);
            saleInfo.price = getPricePass(tokenId, price, passType);
    ...
    function purchase(address token, uint tokenId, uint buyPrice) external override whenNotPaused nonReentrant {
        ...
        uint geniPrice = buyPrice * (100 - feeRate) / 100;
    ...
    function takeOffer(address token, uint tokenId, address buyer, uint takeOfferPrice) external override whenNotPaused nonReentrant {
        ...
        if(saleInfo.seller == address(0)) {
            require(buyer != msg.sender, "GEMUNIMarketplace: seller can not buy");
        } else {
            require(saleInfo.seller != buyer, "GEMUNIMarketplace: seller can not buy");
        ...
        uint price = priceOffer * (100 - feeRate) / 100;
...
interface IGENIPassSale {
    event PassPutOnSale(uint indexed passId, uint price, uint priceType, address seller);
    event PassUpdateOnSale(uint indexed passId, uint newPrice);
    event PassRemoveOnSale(uint indexed passId);
    event PassBought(
        uint indexed passId,
        address buyer,
        address seller,
    ...
    event PassOffered(uint indexed passId, address buyer, uint price);
    event PassOfferCanceled(uint indexed passId, address buyer);
    event PassOfferUpdated(uint indexed passId, address buyer, uint newOfferPrice);

    function getPricePass(uint passId, uint price, IGENIPass.PassType passType, IGENIPass.PriceType priceType) external view returns(uint);
}
...
interface IGENIPass {
    ...
    function permit(address owner, address spender, uint tokenId, uint _nouce, bytes memory _signature) external;
...
interface IGEMUNIMarketplace {
    ...
    function getPricePass(uint passId, uint price, IGENIPass.PassType passType) external view returns(uint);
}

pragma solidity ^0.8.0;

interface IGEMUNIItem {
...
contract GENIPassSale is IERC721Receiver, PermissionGroup, ReentrancyGuard, Pausable {
    using SafeERC20 for IERC20;
    
    IERC721 public immutable geniPass;
    IERC20 public immutable geni;
    
    uint public discountRate = 10;
    uint public referalBonusRate = 20;
    address public treasury;
    ...
    mapping(address => bool) public isReceiveReferalBonus;
    mapping(address => bool) public isEnterReferal;
    ...
    event PassBoughtWithReferal(uint indexed passId, address buyer, address seller, address referal, uint256 price, uint discountedPrice);
    event WithdrawGeniPass(address seller, address to, uint indexed tokenId);
    event WithdrawToken(address token, address to, uint amount);

    constructor(
        IERC20 geniAddr,
        IERC721 geniPassAddr,
        address _treasury,
        address _server,
        address _exchange,
        address _busd,
    ...
    function setDiscountRate(uint value) external onlyOwner {
        require(value < 100, "GENIPassSale: invalid rate");
        discountRate = value;
    }
    
    function setReferalBonusRate(uint value) external onlyOwner {
        require(value < 100, "GENIPassSale: invalid rate");
        referalBonusRate = value;
    }
    ...
    function putOnSale(uint passId, uint price, IGENIPass.PriceType priceType, uint startTime, uint expirationTime) public whenNotPaused onlyOperator {
        require(startTime >= block.timestamp, "GEMUNIMarketplace: invalid start time");
        require(expirationTime > startTime, "GEMUNIMarketplace: invalid end time");
        IGENIPass.GeniPass memory pass =  IGENIPass(address(geniPass)).getPass(passId);
        ...
        saleInfo.price = getPricePass(passId, price, passType, priceType);
        saleInfo.startTime = startTime;
        saleInfo.expirationTime = expirationTime;
    
        emit PassPutOnSale(passId, saleInfo.price, uint(priceType), treasury);
    }
    
    function mintForSale(string memory serialNumber, uint price, IGENIPass.PassType passType, IGENIPass.PriceType priceType, uint startTime, uint expirationTime) public whenNotPaused onlyOperator {
        require(startTime >= block.timestamp, "GEMUNIMarketplace: invalid start time");
        require(expirationTime > startTime, "GEMUNIMarketplace: invalid end time");
        uint passId = IGENIPass(address(geniPass)).mint(address(this), serialNumber, passType);
        SaleInfo storage saleInfo = saleInfos[passId];
        saleInfo.priceType = priceType;
        saleInfo.price = getPricePass(passId, price, passType, priceType);
        saleInfo.startTime = startTime;
    ...
    function updateSale(uint passId, uint price) external onlyOperator whenNotPaused {
        SaleInfo storage saleInfo = saleInfos[passId];
        require(block.timestamp <= saleInfo.expirationTime, "GEMUNIMarketplace: expired");
    ...
    function removeFromSale(uint passId) external onlyOperator whenNotPaused nonReentrant {
        SaleInfo storage saleInfo = saleInfos[passId];
        require(saleInfo.price > 0, "GENIPassSale: not for sale");
    ...
    function getPricePass(uint passId, uint price, IGENIPass.PassType passType, IGENIPass.PriceType priceType) public view returns(uint pricePass) {
        bool isExistedToken = IGENIPass(address(geniPass)).exists(passId);
    ...
    function putOnSaleBatch(CreateSalePassesWithoutMint[] memory input) external whenNotPaused onlyOperator {
        for (uint i = 0; i < input.length; i++) {
            putOnSale(input[i].passId, input[i].price, input[i].priceType, input[i].startTime, input[i].expirationTime);
        }
    }
    
    function mintForSaleBatch(CreateSalePasses[] memory input) external whenNotPaused onlyOperator {
        for (uint i = 0; i < input.length; i++) {
    ...
    function purchase(uint passId, uint buyPrice) external payable whenNotPaused nonReentrant {
        SaleInfo storage saleInfo = saleInfos[passId];
        ...
        uint salePrice = saleInfo.price;
        uint payPrice = geniPass.balanceOf(buyer) > 0 ? (salePrice - (salePrice * discountRate / 100)) : salePrice;
    ...
    function purchaseWithReferal(uint passId, uint buyPrice, address referalAddress, uint nonce, bytes memory signature) external payable whenNotPaused nonReentrant {
        SaleInfo storage saleInfo = saleInfos[passId];
        ...
        bytes32 hash = VerifySignature.getEthSignedMessageHash(getMessageHash(referalAddress, nonce));
        
        address recoverAddress = VerifySignature.recoverSigner(hash, signature);
        
        require(recoverAddress == server, "GENIPassSale: invalid signature");
        require(referalAddress != buyer, "GENIPassSale: invalid buyer");
        require(!isReceiveReferalBonus[referalAddress], "GENIPassSale: can only receive referal bonus 1 time");
        require(!isEnterReferal[buyer], "GENIPassSale: can only enter referal code 1 time");
        
        salePrice = payPrice * (100 - referalBonusRate) / 100;
        if (saleInfo.priceType == IGENIPass.PriceType.GENI) {
            geni.safeTransferFrom(buyer, treasury, salePrice);
            geni.safeTransferFrom(buyer, referalAddress, payPrice - salePrice);
        } else {
            treasury.call{value: salePrice}("");
            referalAddress.call{value: payPrice - salePrice}("");
        }

        geniPass.transferFrom(address(this), buyer, passId);
        isReceiveReferalBonus[referalAddress] = true;
        isEnterReferal[buyer] = true;
        emit PassBoughtWithReferal(passId, buyer, treasury, referalAddress, saleInfo.price, payPrice);
        delete saleInfos[passId];
    }
