...
contract PocoEquiment is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, OwnableUpgradeable, ERC721BurnableUpgradeable {
    ...
    mapping(uint256 => EquimentInfo) public properties;     
    ...
    event NewEquiment(uint256 indexed equiment, address indexed minter);

    constructor() initializer {}

    function initialize() initializer public {
        __ERC721_init("POCO Equiment", "POCOE");
    ...
    function _getAdditionValue(uint256 rarityItem, uint256 itemTypeID) internal view returns (uint256){
        uint256 additionValue;
        // 5 : Mythic item
        if (rarityItem == 5){
            if (itemTypeID ==  5) {
        ...
        else if ( rarityItem == 4) {
            if (itemTypeID ==  5) {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(3) + 2;
            } else if (itemTypeID == 4) {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(6) + 9;
            }
            else if (itemTypeID == 3) {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(3) + 3;
            }
            else if (itemTypeID == 2) {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(31) + 59;
            }
            else {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(6) + 9; 
            }
        }
        else if ( rarityItem == 3) {
            if (itemTypeID ==  5) {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(2) + 1;
            } else if (itemTypeID == 4) {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(6) + 6;
            }
            else if (itemTypeID == 3) {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(3) + 2;
            }
            else if (itemTypeID == 2) {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(41) + 39;
            }
            else {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(6) + 6; 
            }
        }
        else if ( rarityItem == 2) {
            if (itemTypeID ==  5) {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(2);
            } else if (itemTypeID == 4) {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(5) + 3;
            }
            else if (itemTypeID == 3) {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(2) + 1;
            }
            else if (itemTypeID == 2) {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(26) + 24;
            }
            else {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(5) + 3; 
            }
        }
        else if ( rarityItem == 1) {
            if (itemTypeID ==  5) {
                additionValue = 1;
            } else if (itemTypeID == 4) {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(4) + 1;
            }
            else if (itemTypeID == 3) {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(2);
            }
            else if (itemTypeID == 2) {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(11) + 19;
            }
            else {
                additionValue = PocoRandom(pocoRandom).getPocoRandom(4) + 1; 
            }
    ...
    function _getProperty() private view returns (uint256, uint256, uint256) {
        uint256 itemTypeID = PocoRandom(pocoRandom).getPocoRandom(5);
        uint256 itemID;
        uint256 additionValue;
        
        uint256 randomItemID = PocoRandom(pocoRandom).getPocoRandom(100);
        
        if (randomItemID > 99) {
            itemID = 10;   
            (additionValue) = _getAdditionValue(5, itemTypeID);
        } else if (randomItemID > 98) {
            itemID = 9;
            (additionValue) = _getAdditionValue(5, itemTypeID);
        }
        else if (randomItemID > 94) {
            itemID = 8;
            (additionValue) = _getAdditionValue(4, itemTypeID);
        }
        else if (randomItemID > 90) {
            itemID = 7;
            (additionValue) = _getAdditionValue(4, itemTypeID);
        }
        else if (randomItemID > 83) {
            itemID = 6;
            (additionValue) = _getAdditionValue(3, itemTypeID);
        }
        else if (randomItemID > 76) {
            itemID = 5;
            (additionValue) = _getAdditionValue(3, itemTypeID);
        }
        else if (randomItemID > 63) {
            itemID = 4;
            (additionValue) = _getAdditionValue(2, itemTypeID);
        }
        else if (randomItemID > 50) {
            itemID = 3;
            (additionValue) = _getAdditionValue(2, itemTypeID);
        }
        else if (randomItemID > 25) {
            itemID = 2;
            (additionValue) = _getAdditionValue(1, itemTypeID);
        }
        else{
            itemID = 1;
            (additionValue) = _getAdditionValue(1, itemTypeID);
        ...
        return (itemTypeID, itemID, additionValue);
    }

    function safeMint(address to, uint256 number) public onlyWhitelistMinter {
        for (uint256 i = 0; i < number; i++) {
            _safeMint(to, _tokenIdCounter.current());
            emit NewEquiment(_tokenIdCounter.current(), to);
            // Skill order : 11-acitve_skill(9)-passive_skill(5)
            PocoRandom(pocoRandom).updateCounter(uint256(gasleft() % 10));
            (uint256 itemTypeID, uint256 itemID, uint256 additionValue) = _getProperty();
            properties[_tokenIdCounter.current()].itemTypeID = itemTypeID;
            properties[_tokenIdCounter.current()].itemID = itemID;
            properties[_tokenIdCounter.current()].additionValue = additionValue;
