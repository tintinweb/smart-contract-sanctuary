...
contract WishItem is AccessControl {
    ...
    constructor( address minter, address _heroesNft, address _heroesToken, address _blackList) {
    ...
    function addMinAmount(uint256[] memory _amount, uint256[] memory _amountSpecial) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, address(msg.sender)), "Caller is not a owner");
        require(_amount.length == _amountSpecial.length, 'Input not true');
    ...
    function changeMinAmount(uint256 tier, uint256 amount, uint256 amountSpecial) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, address(msg.sender)), "Caller is not a owner");
        require(amount > 0 && amountSpecial > 0, "amount > 0");
        minAmount[tier][1] = amountSpecial;
        minAmount[tier][2] = amount;
    }
    function addHero(uint256[] memory heroId, uint256[] memory tier, uint256[] memory _raceNumber)public {
        require(hasRole(DEFAULT_ADMIN_ROLE, address(msg.sender)), "Caller is not a owner");
        require(heroId.length == tier.length && heroId.length == _raceNumber.length, 'Input not true');
    ...
    function matchTransactionWithId(uint256 id, uint256 tokenId) public {
        ...
        require(!blackList.getTokenId(tokenId), "TokenId in blacklist");
        uint256 heroesNumber = itemInfo[id].numberHero;
    ...
    function getMaxValue(uint256[] memory arrayHero) internal pure returns(uint256) {
        uint256 largest = 0; 
        uint256 numberMax = 0;
