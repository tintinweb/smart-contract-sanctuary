...
contract IDO is Ownable {
    ...
    event AddWhiteList(address indexed _account, uint256 _witeListCount);
    ...
    /* function release(uint256 _orderIndex) public returns (bool) {
        ...
        SwapOrder storage order =  swapAccounts[_orderIndex];
        require(order.isRelease,"isRelease: Your deposit order does not in Release");
        require(block.timestamp.sub(order.lastReleaseTime)>=dayTime,"-> daily: Your time deposit is not due.");

        uint256 diffTime = block.timestamp.sub(releaseStartTime);
        uint256 dayRelease = diffTime.dvi(dayTime).add(1);
    ...
    function addWhiteList() public onlyOwner returns (bool) {
        whiteListCount += 1;
        whiteListAccounts[msg.sender] = true;
        emit AddWhiteList(msg.sender,whiteListCount);
        return true;
    }
