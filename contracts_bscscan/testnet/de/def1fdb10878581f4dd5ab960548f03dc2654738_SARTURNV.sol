...
contract STFVDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("STFV_Dividend_Tracker", "STFV_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100000000 * (10**9); //must hold 10000000+ tokens
    }

    function _transfer(address, address, uint256) internal override pure {
        require(false, "STFV_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "STFV_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SURF contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "STFV_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "STFV_Dividend_Tracker: Cannot update claimWait to same value");
...
contract SARTURNV is ERC20, Ownable {
    ...
    STFVDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SARTURNV", "STFV") {

        dividendTracker = new STFVDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "STFV: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "STFV: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "STFV: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "STFV: Cannot update gasForProcessing to same value");
    ...
    function prepareForLaunch() external onlyOwner {
        updateBuyFees(4,2,3,0);
        updateSellFees(6,2,5,0);
        restrictWhales = true;
