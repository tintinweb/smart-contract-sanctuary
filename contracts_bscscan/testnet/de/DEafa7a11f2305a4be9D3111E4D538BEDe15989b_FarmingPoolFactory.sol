...
import "../../pools/BlockRewardFarmingPool.sol";
import "../../libraries/Dictionary.sol";

contract FarmingPoolFactory is Ownable {
  BlockRewardFarmingPool[] private childrenBlockRewardFarmingPool;

  Dictionary private config;

  event BlockRewardFarmingPoolDeployed(address owner, address sc);
  ...
  function createBlockRewardFarmingPool(
    ...
    require(
      msg.value >= config.getFee("farming"),
      "PoolFactory:Not enough value"
    );

    BlockRewardFarmingPool child = new BlockRewardFarmingPool(
      msg.sender,
      stakingToken,
      rewardToken,
      lockDuration,
    ...
    childrenBlockRewardFarmingPool.push(child);
    emit BlockRewardFarmingPoolDeployed(msg.sender, address(child));
  }
