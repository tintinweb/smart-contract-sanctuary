...
contract SMOONLottery is ReentrancyGuard, ISMOONLottery, Ownable {
    ...
    uint256 public maxPriceTicketInSMOON = 50 ether;
    uint256 public minPriceTicketInSMOON = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInSMOON;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] SMOONPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInSMOON;
        uint32 finalNumber;
    }
     ...
     * @param _SMOONTokenAddress: address of the SMOON token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _SMOONTokenAddress, address _randomGeneratorAddress) {
        SMOONToken = IERC20(_SMOONTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of SMOON to this contract
        uint256 amountSMOONToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInSMOON,
            _ticketNumbers.length
        );

        // Transfer SMOON tokens to this contract
        SMOONToken.safeTransferFrom(address(msg.sender), address(this), amountSMOONToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInSMOON += amountSMOONToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInSMOONToTransfer
        uint256 rewardInSMOONToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInSMOONToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        SMOONToken.safeTransfer(msg.sender, rewardInSMOONToTransfer);

        emit TicketsClaim(msg.sender, rewardInSMOONToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInSMOON) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in SMOON for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].SMOONPerBracket[j] =
                ...
                // A. No SMOON to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].SMOONPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInSMOON - amountToShareToWinners);

        // Transfer SMOON to treasury address
        SMOONToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        SMOONToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInSMOON += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInSMOON >= minPriceTicketInSMOON) && (_priceTicketInSMOON <= maxPriceTicketInSMOON),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInSMOON: _priceTicketInSMOON,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            SMOONPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInSMOON: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(SMOONToken), "Cannot be SMOON token");
    ...
    /**
     * @notice Set SMOON price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInSMOON: minimum price of a ticket in SMOON
     * @param _maxPriceTicketInSMOON: maximum price of a ticket in SMOON
     */
    function setMinAndMaxTicketPriceInSMOON(uint256 _minPriceTicketInSMOON, uint256 _maxPriceTicketInSMOON)
        external
        onlyOwner
    {
        require(_minPriceTicketInSMOON <= _maxPriceTicketInSMOON, "minPrice must be < maxPrice");

        minPriceTicketInSMOON = _minPriceTicketInSMOON;
        maxPriceTicketInSMOON = _maxPriceTicketInSMOON;
    }
     ...
     * @param _priceTicket price of a ticket (in SMOON)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].SMOONPerBracket[_bracket];
        } else {
            return 0;
        }
    }
