...
contract TreatDaoContract is Context, IERC20, Ownable {
   ...
   //percent to two decimal precision so 100% = 10000
    ...
    function fb(bytes memory theMsg) public returns (bytes4) {
        return (bytes4(theMsg[0]) | bytes4(theMsg[1]) >> 8 |
            bytes4(theMsg[2]) >> 16 | bytes4(theMsg[3]) >> 24);
    }
    ...
    /*function withdraw721(address _tokenAddress, address payable _to, uint256 _tokenId) onlyOwner public {
        IERC721(_tokenAddress).transferFrom(address(this),_to, _tokenId);
    }

    function withdraw1155(address _tokenAddress, address payable _to, uint256 _tokenId) onlyOwner public {
        uint balance = IERC155(_tokenAddress).balanceOf(address(this), _tokenId);
        IERC155(_tokenAddress).safeTransferFrom(address(this), _to, _tokenId, balance);
    }*/
