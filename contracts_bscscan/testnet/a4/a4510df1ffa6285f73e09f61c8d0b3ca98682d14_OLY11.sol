...
//        return amount.sub(feeAmount);
 //   }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = block.timestamp < lastBuy[recipient].add(greenwallDuration) ? // original feeAmount = amount.mul(getTotalFee(isSell(recipient))).div(feeDenominator)
        amount.mul(getTotalFee(isSell(recipient))).div(feeDenominator).mul(greenwallNumerator).div(greenwallDenominator) : amount.mul(getTotalFee(isSell(recipient))).div(feeDenominator) ; 
