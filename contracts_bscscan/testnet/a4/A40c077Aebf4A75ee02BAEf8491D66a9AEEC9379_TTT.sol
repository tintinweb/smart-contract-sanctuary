...
contract TTT is Context, IERC20, Ownable {
    ...
    mapping(address => uint256) private _winnerAmounts;
    address[] private _currentWinners;
    ...
    event LuckyShotWinner(address indexed winner, uint256 amount);
    ...
    function luckyShotDistributeRewards() external onlyOwner {
        require(
            _currentWinners.length > 0,
            "No winners to distribute tokens to"
        );
        address[] memory winnerAddresses = _currentWinners;

        for (uint256 index = 0; index < winnerAddresses.length; index++) {
            address winner = winnerAddresses[index];
            uint256 winnings = _winnerAmounts[winner];
            _winnerAmounts[winner] = 0;

            if (winnings > 0) {
                if (!_isExcluded[winner].fromReward) {
                    _reflectionBalance[winner] += (winnings *
                        __getReflectionRate());

                    _reflections.inCirculation += (winnings *
                        __getReflectionRate());
                    _reflections.totalSupply += winnings;
                } else {
                    _tokenBalance[winner] += winnings;
                }

                emit Transfer(address(this), winner, winnings);
                emit LuckyShotWinner(winner, winnings);
            }
        }

        delete _currentWinners;
    }
    ...
    function __luckyShot(address to) private {
        ...
        if (luckyShot.pool >= luckyShot.minimumPoolThreshold) {
            ...
            luckyShot.rolls++;
            uint256 roll = __roll(timestamp);
            luckyShot.lastRoll = roll;

            if (roll <= luckyShot.chanceToWin)
        ...
        if (luckyShotRewardsCollected > 0) {
            ...
            _winnerAmounts[to] += luckyShotRewardsCollected;
            _currentWinners.push(to);
        }
    }
