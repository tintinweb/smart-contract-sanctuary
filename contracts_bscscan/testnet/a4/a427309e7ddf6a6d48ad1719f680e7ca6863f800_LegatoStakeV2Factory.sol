...
interface StakePool {
	struct Pool {
		...
		address WBNB;
    	address BUSD;
    	address LegatoStakeV2V1Address;
...
contract LegatoStakeV2 is Context, Ownable, ReentrancyGuard, StakePool {
	...
	function hasMinimumLdogeHolding(address account) public view returns (bool) {
		...
		balance = balance.add(LegatoStakeV2V1(_pool.LegatoStakeV2V1Address).getUserStakes(1,account));
		balance = balance.add(LegatoStakeV2V1(_pool.LegatoStakeV2V1Address).getUserStakes(2,account).mul(oneLp));
...
contract LegatoStakeV2Factory is Ownable {
	using SafeMath for uint256;
	using Address for address;
	
	function createNewPool(StakePool.Pool memory pool) external onlyOwner returns (address) {
		return address(new LegatoStakeV2(pool));
	}
}


contract LegatoStakeV2Creator is Context, Ownable, ReentrancyGuard {
	using SafeMath for uint256;
	using Address for address;
	...
	address public _LegatoStakeV2V1Address;
	...
	LegatoStakeV2Factory public _factory;
	...
	constructor (address vault,address ldogeToken,address WBNB,address BUSD,address LegatoStakeV2V1Address,address LDOGE_WBNB_pair,address BUSD_WBNB_pair,uint256 minimumLDOGEHoldingsInDollars,uint256 minimumLDOGEHoldingsInLDOGE,uint256 creationFeePrice) {
		_vault = vault;
		_ldogeToken = ldogeToken;
		_WBNB = WBNB;
		_BUSD = BUSD;
		_LegatoStakeV2V1Address = LegatoStakeV2V1Address;
		_LDOGE_WBNB_pair = LDOGE_WBNB_pair;
		_BUSD_WBNB_pair = BUSD_WBNB_pair;
		_factory = new LegatoStakeV2Factory();
	...
	function _createNewPool(NewPoolInfo memory info) private returns (address) {
		...
		pool.WBNB = _WBNB;
		pool.BUSD = _BUSD;
		pool.LegatoStakeV2V1Address = _LegatoStakeV2V1Address;
	...
	function createNewPoolAndPayBNBAndAddReward(NewPoolInfo memory info,uint256 rewardAmount) external payable nonReentrant returns (address)  {
		require(msg.value >= _creationFeePrice,"You must first pay the pool creation fee.");
		(bool sent,) = address(_vault).call{value : msg.value}("");
		require(sent, 'Error: Cannot withdraw BNB');
		creationFeesReceived[_msgSender()].add(msg.value);
		address newPoolAddress = _createNewPool(info);
