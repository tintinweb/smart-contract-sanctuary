...
contract DNPToken is Context, IBEP20, Ownable {
    ...
    string private     _name = "Deflationary Test Token";
    ...
    address private _teamWallet = 0xC87300Cbc73bb1582C4d3140Aa4656ce39795eC6;

    address private _devWallet = 0xbdbd5c0dCA0Ba566Ac0c0534E4fc3afA53fB5C26;

    uint256 private numTokensSellToAddToLiquidity = 10000000000 * 10 **_decimals;
    ...
    function setTaxFee(uint256 _fee) external onlyOwner {
        require(_fee < uint256(100).sub(_liquidityFee.add(_teamFee).add(_burnFee)), "Tax fee too high");
        _previousTaxFee = _taxFee;
        _taxFee = _fee;
    }

    function setLiquidityFee(uint256 _fee) external onlyOwner {
        require(_fee < uint256(100).sub(_taxFee.add(_teamFee).add(_burnFee)), "Liquidity fee too high");
    ...
    function setTeamFee(uint256 _fee) external onlyOwner {
        require(_fee < uint256(100).sub(_liquidityFee.add(_taxFee).add(_burnFee)), "Team fee too high");
    ...
    function setBurnFee(uint256 _fee) external onlyOwner {
        require(_fee < uint256(100).sub(_liquidityFee.add(_teamFee).add(_taxFee)), "Burn fee too high");
        ...
        (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256)
    {
        ...
        uint256[6] memory rvalues;

        tvalues[0] = calculateTaxFee(tAmount);
        tvalues[1] = calculateLiquidityFee(tAmount);
        tvalues[2] = calculateTeamFee(tAmount);
        tvalues[3] = calculateBurnFee(tAmount);
        tvalues[4] = tAmount.sub(tvalues[0].add(tvalues[1]).add(tvalues[2]).add(tvalues[3]));
        ...
        rvalues[4] = tAmount.mul(currentRate);
        rvalues[5] = rvalues[4].sub(rvalues[0]).sub(rvalues[1]).sub(rvalues[2]).sub(rvalues[3]);

        return (rvalues[4], rvalues[5], rvalues[0], rvalues[3], tvalues[4], tvalues[0], tvalues[3], tvalues[2], tvalues[1]);
    }
    ...
    function calculateTeamFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_teamFee).div(100);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rBurn,
         uint256 tTransferAmount, uint256 tFee, uint256 tBurn, uint256 tTeam, uint256 tLiquidity) = _getValues(tAmount);
        _rOwned[sender]    = _rOwned[sender].sub(rAmount);
