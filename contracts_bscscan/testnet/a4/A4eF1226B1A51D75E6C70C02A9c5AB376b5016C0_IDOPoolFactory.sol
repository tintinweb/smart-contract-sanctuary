...
contract IDOPoolFactory is PoolCreator{
    ...
    function createPoolProxy(
        ...
        if (_variables[7] == 0) {
            _variables[7] = stakingPoolTaxRate;
        }
        ...
        idoTokenBank.addIDOPredictionWithToken(idoPoolProxyAddr, idoToken);
    ...
    // Call this in case you want to upgrade a working pool which inherits from
    // the original implementation
    // FIXME: This does not work because of onlyPoolCreator, not sure why
    // so should upgrade within the proxy itself for now
    function upgradeIDOPool(
        IDOPoolProxy _idoPoolProxy, 
        address _newIDOPoolImplementation
    ) external onlyPoolCreator {
        _idoPoolProxy.upgradeTo(_newIDOPoolImplementation);
    }
...
contract IDOPoolProxy is IDOPoolStorageStructure {
    ...
    function upgradeTo(address _newIDOPoolImplementation) external onlyPoolCreator {
        require(idoPoolImplementation != _newIDOPoolImplementation);
