...
contract TestcreateMultiNFT is Ownable, ReentrancyGuard {
    ...
    event Sale(uint256 indexed tokenId, address buyer, uint256 price, uint256 typeBox, uint256 randNumber);
    ...
    function createNFT() public payable nonReentrant
    {
        for (uint256 index = 0; index <= total; index++) {
            uint256 star = 1;
            if (index == 10 || index == 11 || index == 12 || index == 17 || index == 22) {
                star = 2;
            } else if (index == 13 || index == 18 || index == 23) {
                star = 3;
            }
            uint256 char = index;
            ...
            emit Sale(tokenId, _msgSender(), 1, 1, 1);
        }
    }
}
