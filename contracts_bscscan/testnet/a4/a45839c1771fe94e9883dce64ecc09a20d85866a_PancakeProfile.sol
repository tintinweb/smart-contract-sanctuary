...
contract PancakeProfile is AccessControl, ERC721Holder {
    ...
    uint256 public referrerCommision = 45e18;
    uint256 public tierCommisionrate = 2000;
    ...
    function createProfile(
        ...
        if(address(wedexReferral) != address(0)&&referrer!=address(0)){
            if(wedexReferral.getReferrer(_msgSender())!=address(0)){
    ...
    function payRefCommision(address _user, uint256 level, uint256 amount) internal{
        if(address(wedexReferral) != address(0) && referrerCommision>0){
    ...
    function updareRefferalSystem(
    uint256  _referrerCommision,
    uint256  _tierCommisionrate,
    uint256  _referDepth
    ) external onlyOwner{
        referrerCommision = _referrerCommision;
