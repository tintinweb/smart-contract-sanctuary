...
// File contracts/interfaces/IArtFactory.sol
...
interface IArtFactory {
    ...
    /**
     * @dev Returns the seller of the `tokenId` token.
     *
     * Requirements:
     *
     * - `tokenId` must exist.
     ...
     * - `to` cannot be the zero address.
     */
    function mint(address _to, address _creator, string calldata _itemName, string calldata _tokenURI, uint256 _price) external returns (uint256);

    /**
     * @dev Burn a NFT token. Callable by token owner only.
     *
     * Requirements:
     *
     * - `tokenId` must exist.
...
contract ArtKitchen is ERC721, Ownable, AccessControl, IArtFactory {
    ...
    function setCommissionAddress(address payable _address) external virtual onlyAdmin {
     ...
     */
    function getListed(uint256 _tokenId) external view returns (bool) {
    ...
    function creatorOf(uint256 _tokenId) external view virtual override returns (address) {
        require(_exists(_tokenId), "ERC721Metadata: creatorOf query for nonexistent token");
    ...
    function sellerOf(uint256 _tokenId) external view virtual override returns (address) {
        require(_exists(_tokenId), "ERC721Metadata: sellerOf query for nonexistent token");
        address seller = items[_tokenId].seller;
        require(seller != address(0), "Art Kitchen: seller is not set");
        return seller;
    }

    function priceOf(uint256 _tokenId) external view virtual returns (uint256) {
        return items[_tokenId].price;
    }

    function _getTokenURI(uint256 _tokenId) internal virtual view returns (string memory) {
        require(_exists(_tokenId), "ERC721Metadata: tokenURI query for nonexistent token");
     ...
     * @dev Get ItemName for a specific tokenId.
     */
    function getItemName(uint256 _tokenId) external view override returns (string memory) {
        require(_exists(_tokenId), "ERC721Metadata: itemName query for nonexistent token");
    ...
    function mint(
        ...
        require(_creator != address(0), "Art Kitchen: creator cannot be the zero address");
        require(bytes(_itemName).length > 0, "Art Kitchen: itemName cannot be empty");
        require(bytes(_tokenURI).length > 0, "Art Kitchen: tokenURI cannot be empty");

        uint256 newId = _tokenIds.current();
        _tokenIds.increment();
        _mint(_to, newId);
        arts[newId] = Art({ itemName: _itemName, creator: _creator, tokenURI: _tokenURI });
        items[newId] = Item({ listed: false, seller: address(0), price: 0 });
    ...
    /**
     * @dev Burn NFTs. Only the token owner can call it.
     */
    function burn(uint256 _tokenId) external override {
        address owner = ownerOf(_tokenId);
        require(_msgSender() == owner, "Art Kitchen: burn caller is not owner");
        _burn(_tokenId);
    ...
    function _sell(uint256 _tokenId, uint256 _price) internal virtual {
        ...
        require(_msgSender() == ownerOf(_tokenId), "Art Kitchen: Sender does not have token");
...
//        bytes32 _message = prefixed(keccak256(abi.encodePacked(_msg)));
//        require(recoverSigner(_message, signature) == _msgSender(), 'Art Kitchen: Invalid signature');
//
...
//        require(ownerOf(_tokenId) == address(this), "Art Kitchen: Item is not deposited");
//        _trade(_tokenId);
//    }
//
    ...
    function _trade(uint256 _tokenId) internal {
        Item memory _item = items[_tokenId];
        Art memory _art = arts[_tokenId];
        address payable _owner = payable(ownerOf(_tokenId));
        address payable _seller = payable(_item.seller);
        address payable _buyer = payable(_msgSender());
        address payable _creator = payable(_art.creator);
        address payable _commission = payable(commissionAddress);

        uint256 _percentValue = 10000;
//        uint256 _commissionValue = div(_item.price, div(mul(100, 100), commissionFeeRate));
        uint256 _commissionValue = _item.price.div(_percentValue.div(commissionFeeRate));
        if (_commissionValue < commissionFeeMin) {
            _commissionValue = commissionFeeMin;
        }
//        uint256 _creatorFeeValue = div(_item.price, div(mul(100, 100), creatorFeeRate));
        uint256 _creatorFeeValue = _item.price.div(_percentValue.div(creatorFeeRate));
        if (_creatorFeeValue < creatorFeeMin) {
            _creatorFeeValue = creatorFeeMin;
        }
        require(_commissionValue + _creatorFeeValue < _item.price, "Art Kitchen: Item price is lower");
//        uint256 _sellerValue = sub(_item.price, add(_commissionValue, _creatorFeeValue));
        uint256 _sellerValue = _item.price.sub(_commissionValue.add(_creatorFeeValue));
...
//            _buyer.transfer(sub(msg.value, _item.price));
            _buyer.transfer(msg.value.sub(_item.price));
        }
