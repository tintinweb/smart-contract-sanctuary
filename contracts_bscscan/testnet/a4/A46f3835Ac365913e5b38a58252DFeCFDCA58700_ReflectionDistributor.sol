...
contract ReflectionDistributor is Allowable, IReflectionDistributor {
    ...
    uint256 public _previousBalance;
    uint256 private _currentIndex;



    uint256 public _ethAmount;

    receive() external payable {
        uint256 balance = address(this).balance;
        uint256 ethAmount = balance.sub(_previousBalance);
    ...
    function setShare(address shareholder, uint256 amount) external override onlyAllowed {
        ...
        if (previousAmount == 0 && amount >= _minDistribution) {
            _addShareholder(shareholder);
        }
        else if (previousAmount >= _minDistribution && amount < _minDistribution) {
            _removeShareholder(shareholder);
        }

        _shares[shareholder].amount = amount;
        _totalShares = _totalShares.sub(previousAmount).add(amount);
