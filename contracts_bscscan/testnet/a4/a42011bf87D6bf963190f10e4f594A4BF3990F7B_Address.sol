...
/**
   ...
   Welcome to Bitcoin Smart Chain BTCSC 
   Auto Mining Rewards for Every 12 Hours to Holders.
   Liquidity Burned on Pancakeswap
   
   =====================================
   TOKENOMICS
   =====================================
   
   Total Supply: 21,000,000 BTCSC
   
   Circulating Supply: 2,100,000 BTCSC
   
   Maximum Supply: 21,000,000 BTCSC
   
   
   SET SLIPPAGE TO 0.1% or higher
   
   * Daily reward = 2000 interval per 12 hours
   * Weekly reward = 20000 interval per 7 days
   * Monthly reward = 30000 interval per 30 days
   * Buy Limit = No Max.
   * Sell Limit = 2,100 BTCSC
...
contract Ownable is Context {
    ...
    constructor () internal {
        ...
        _owner = 0xed290966dD5272ff255C478C6368ec17125F3A2b;
...
contract BitcoinSmartChain is Context, IERC20, Ownable {
    ...
    // Constants
    uint256 private constant TOTAL_SUPPLY = 21000000 * 10**9;
    uint256 private DEPLOYMENT_TIME;
    address[2] private SWAP;
    uint8 private constant DECIMALS = 9;
    string private constant SYMBOL = "BTCSC";
    string private constant NAME = "Bitcoin Smart Chain";
    ...
    uint256 private taxFees = 1; // 0.1% fee
    uint256 private burnFees = 1;
    uint256 private maxTxAmount = 2100 * 10**9;
    ...
    uint256 private cSupply = 2100000 * 10**9;
    ...
    function halving () private {
        ...
        if (
            ...
            daily.reward = 1800 * 10**9;
            weekly.reward = 15000 * 10**9;
            monthly.reward = 25000 * 10**9;
        }else if (
            ct >= mr[1] + DEPLOYMENT_TIME &&
            ct < mr[2] + DEPLOYMENT_TIME
        ) {
            daily.reward = 1600 * 10**9;
            weekly.reward = 10000 * 10**9;
            monthly.reward = 15000 * 10**9;
        }else if (
            ct >= mr[2] + DEPLOYMENT_TIME &&
            ct < mr[3] + DEPLOYMENT_TIME
        ) {
            daily.reward = 1400 * 10**9;
            weekly.reward = 5000 * 10**9;
            monthly.reward = 10000 * 10**9;
            stopHalving = true;
        }
    }
    
    function init () private {
        daily.reward = 2000 * 10**9;
        daily.interval = 12 hours;

        weekly.reward = 20000 * 10**9;
        weekly.interval = 7 days;

        monthly.reward = 30000 * 10**9;
    ...
    function initReward (uint256 r1, uint256 r2, uint256 r3) external onlyOwner {
        daily.reward = r1 * 10**9;
        weekly.reward = r2 * 10**9;
        monthly.reward = r3 * 10**9;
    }
