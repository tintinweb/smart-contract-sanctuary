...
*/
contract ShroomTaxOracle is Ownable {
    using SafeMath for uint256;

    IERC20 public mshroom;
    IERC20 public cake;
    address public pair;
    ...
    constructor(
        ...
        require(_mshroom != address(0), "mshroom address cannot be 0");
        require(_cake != address(0), "cake address cannot be 0");
        require(_pair != address(0), "pair address cannot be 0");
        mshroom = IERC20(mshroom);
        cake = IERC20(cake);
        pair = _pair;
    }

    function consult(address _token, uint256 _amountIn) external view returns (uint144 amountOut) {
        require(_token == address(mshroom), "token needs to be mshroom");
        uint256 tombBalance = mshroom.balanceOf(pair);
        uint256 cakeBalance = cake.balanceOf(pair);
        return uint144(tombBalance.div(cakeBalance));
    }

    function setMshroom(address _mshroom) external onlyOwner {
        require(_mshroom != address(0), "mshroom address cannot be 0");
        mshroom = IERC20(_mshroom);
    }

    function setCake(address _wftm) external onlyOwner {
