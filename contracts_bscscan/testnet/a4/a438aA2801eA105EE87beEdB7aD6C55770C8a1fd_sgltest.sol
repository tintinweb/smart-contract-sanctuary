...
contract sgltest is Context, IERC20, Ownable {
    ...
    string private _name = "sgltest";
    string private _symbol = "SGLT";
    ...
    uint256 public _convertBNBFee = 0;
    ...
    mapping (address => uint256) public bnbPaid;
    ...
    uint256 public supplyAmountCompareForBNBRewards = 35 * 10**13 * 10**9;
    ...
    // uint256 public bnbMaxUntilCharityCut = 100000000000000000;     // minimum 1 BNB        // CHANGEIT - make sure it's set to 1 BNB of course
    uint256 public percentOfCharityCut = 10;

    uint256 public maxNumOfTransfersToDoForReward = 5;

    uint256 public minimumAmountToHoldForRewards = 5000000000 * 10**9;      // must hold at least  0.0005% of the token to get rewards


    uint256 public hoursToAddForReward = 30 minutes; // CHANGEIT - make sure this is properly set
    // uint256 public hoursToAddForReward = 5 minutes;

    bool public isRewardSystemEnabled = false;



    address public charityWallet = 0xDEcA5624CBC3D389f933559040fB17D370a25d45;      // CHANGEIT - charity
    ...
    address public deadAddressThree = 0x000000000000000000000000000000000000dEaD;
    ...
    uint256 public _maxTxAmount = 1000000000000 * 10**9; //0.1% of totalSupply
    ...
    uint256 public numTokensSellToAddToLiquidity = 300000000000 * 10**9;      // 0.03% - safemoon's is 0.05%
    ...
    constructor () {
        ...
        address ownerWallet = 0xB76Fe6a8d4BCa89b2187224aCAC15e312Efd4aB4;
        _rOwned[ownerWallet] = _rTotal.div(1000).mul(700);
        emit Transfer(address(0), ownerWallet, _rTotal.div(1000).mul(700));

        _rOwned[deadAddressThree] = _rTotal.div(1000).mul(200);  // burn
        emit Transfer(address(0), deadAddressThree, _rTotal.div(1000).mul(200));

        _rOwned[0xbc01D9B1aEC6832350e798dFcb6a2D60A131e432] = _rTotal.div(1000).mul(50);
        emit Transfer(address(0), 0xbc01D9B1aEC6832350e798dFcb6a2D60A131e432, _rTotal.div(1000).mul(50));

        _rOwned[0x588BAAE1A207c736E0Da4B1e820Ea7F31cc08037] = _rTotal.div(1000).mul(40);
        emit Transfer(address(0), 0x588BAAE1A207c736E0Da4B1e820Ea7F31cc08037, _rTotal.div(1000).mul(40));

        _rOwned[0xaC04435CB4800ba235D291131e74349584893fd3] = _rTotal.div(1000).mul(10);
        emit Transfer(address(0), 0xaC04435CB4800ba235D291131e74349584893fd3, _rTotal.div(1000).mul(10));
        ...
        //For PCS TESTNET: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
        //For PCS TESTNET PAIR: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
        ...
        _isAdminAccount[routerDEXAddress] = true;
    ...
    function excludeFromReward(address account) public onlyOwner() {
        ...
        require(_excluded.length < 256, "Too long excluded array");
    ...
    function setConvertBNBFeePercent(uint256 newConvertBNBFee) external onlyOwner() {
        require(newConvertBNBFee <= 30, "Percentage too high. Please use a lower fee.");
        _convertBNBFee = newConvertBNBFee;
    }

    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 5, "Percentage too high. Please use a lower fee.");
    ...
    function _transfer(
        ...
        if(!inSwapAndLiquify){
            ...
            if(_isExcluded[address(this)]){
                amountToSellForLIQ = tliquidityAmountTrackingSell;
                amountToSellForBNB = tconvertBNBAmountTrackingSell;
            }
        ...
        if(!isAddressExemptFromBNBRewards[to]){
            if(balanceAfterTransfer > 0 && balanceAfterTransfer >= minimumAmountToHoldForRewards){
                ...
                if(from == uniswapV2Pair){        // if it was a buy....
                    ...
                    if(  ( balanceAfterTransfer.mul(100).div(balanceBeforeTransfer) ) > bnbBuyResetThresholdPercent ){
    ...
    function swapTokensForEthAndSwapAndLiquify(uint256 amountToSellForLIQ, uint256 amountToSellForBNB, address fromAddress) private lockTheSwap() {
        ...
        if(contractTokenBalance >= totalToSell && totalToSell > 0){
            ...
            if(halfOfLIQamount > 0){    // now if we are greater than zero on the half liq amount we know we sold for BNB now we need to do the pairing.
                ...
                if(deltaBalance.mul(percentOfLIQ).div(100) > 0){

                    uniswapV2Router.addLiquidityETH{value: deltaBalance.mul(percentOfLIQ).div(100) }(     // Add liquidity
                        address(this),
                    ...
                    emit SwapAndLiquify(halfOfLIQamount, deltaBalance, otherHalfOfLIQamount);
                }
            }
        }
    }
    ...
    function distributeBNBReward() private {
        ...
        if(BNBbalanceInContract >= bnbMinimumAmountInContractToReward){     // check to make sure we have a minimum amount of BNB to do the distribution
            ...
            if(numberOfTransfersToDo > addressesToReward.length){
                numberOfTransfersToDo = addressesToReward.length; //8      // in case there are a small number of holders I need to check this and set it
            }
            uint256 count = 0;
            uint256 i = 0;

            while (i < addressesToReward.length && count < numberOfTransfersToDo) {
                if (block.timestamp >= timeToReward[addressesToReward[i]]) {         // the soonest people should be close to the start, there will be some slight variation but it should all work
                    ...
                    if (tokenBalanceOfRewarded >= minimumAmountToHoldForRewards) {     // as long as the user has the minimum then it's good
                        ...
                        if(bnbToReward > bnbMaxUntilCharityCut){        // if the BNB reward amount is over bnbMaxUntilCharityCut then we are giving 30% to charity wallet

                            uint256 oneHundred = 100;

                            uint256 otherPercent = oneHundred.sub(percentOfCharityCut);

                            uint256 cutBNBToCharity = bnbToReward.mul(percentOfCharityCut).div(100);
                            bnbToReward = bnbToReward.mul(otherPercent).div(100);

                            payableAddress(charityWallet).transfer(cutBNBToCharity);
                        }

                        payableAddress(addressesToReward[i]).transfer(bnbToReward);     // pay the address their bnb

                        bnbPaid[addressesToReward[i]] = bnbPaid[addressesToReward[i]].add(bnbToReward);
                        totalBNBPaid = totalBNBPaid.add(bnbToReward);
                        lastPayoutTime[addressesToReward[i]] = block.timestamp;
    ...
    function addExemptWalletFromBNBRewards(address addAddress) external onlyOwner() {
    ...
    function removeExemptWalletFromBNBRewards(address removeAddress) external onlyOwner() {
    ...
    function setMinimumBNBInContractToReward(uint256 newBNBMinimum) external onlyOwner() {
        bnbMinimumAmountInContractToReward = newBNBMinimum;
    }

    function setBNBMaxUntilCharityCut(uint256 newBNBMaxUntilCharityCut) external onlyOwner() {
    ...
    function setMaxNumOfTransersToDoForReward(uint256 newMaxNumOfTransfers) external onlyOwner() {
    ...
    function setHoursForRewardTimer(uint256 newNoursToAddForReward) external onlyOwner() {
        hoursToAddForReward = newNoursToAddForReward;
    }

    function setNewPercentOfCharityCut(uint256 newPercentOfCharityCut) external onlyOwner() {
        percentOfCharityCut = newPercentOfCharityCut;       // new charity wallet
    }

    function setNewCharityWallet(address newCharityWallet) external onlyOwner() {
    ...
    function getData(address _user) public view returns(uint256, uint256, uint256, uint256, uint256, bool) {
        uint256 _balance = balanceOf(_user);
        uint256 _bnbPaid = bnbPaid[_user];
        uint256 _lastPayoutTime = lastPayoutTime[_user];
        uint256 _timeToReward = timeToReward[_user];
        uint256 _nextPayoutLoading;

        for(uint256 i=0; i < addressesToReward.length; i++) {
            if(addressesToReward[i] == _user) {
                _nextPayoutLoading = (block.timestamp.sub(_timeToReward.sub(hoursToAddForReward)).mul(10**2))
                .div(hoursToAddForReward);
                break;
            }
        }

        bool eligible;
        if (_balance < minimumAmountToHoldForRewards) {
            eligible = false;
        } else {
            eligible = true;
        }

        if (_nextPayoutLoading > 100) {
            _nextPayoutLoading = 100;
        }

        return (_balance, _bnbPaid, _lastPayoutTime, _nextPayoutLoading, totalBNBPaid, eligible);
    }
