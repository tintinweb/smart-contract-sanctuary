...
interface IBEP21{
    ...
    function GetStarNumbers() external view  returns(uint256 ,uint256,uint256,uint256);
    function GetoneDayReward() external view returns(uint256);
    function GetBlockNumber(address account) external view returns(uint256);
    
    function GetStartUpdade(address account) external view returns(uint32 star1update,uint32 star2update,uint32 star3update,uint32 star4update);
    function GetStarPower(address account) external view returns(uint256);
...
contract Voting1 {
    ...
    function _010_SystemInfo_010_ () external view returns(uint256 TotalPower, uint256 OneDayNFTWOutPut,uint256 TotalVoter,uint256 TotalStar1 ,uint256 TotalStar2,uint256 TotalStar3,uint256 TotalStar4){
       OneDayNFTWOutPut=Voters.GetoneDayReward();
       TotalVoter=Voters.VotersAll();
       TotalPower=Voters.totalSupply();
       (TotalStar1 ,TotalStar2,TotalStar3,TotalStar4)=Voters.GetStarNumbers();
       
      return(TotalPower,OneDayNFTWOutPut,TotalVoter,TotalStar1 ,TotalStar2,TotalStar3,TotalStar4); }

    function _010_UseInfo_010_ (address account) external view returns(uint256 YouAreStar,uint256 starActiveDays,uint256 NewGade4Invite,uint256 WithdrawalDays,uint256 StarBonusNow,address Father){
    Father=Voters.GetRelationShip(account);
    WithdrawalDays=Voters.GetBlockNumber(account);
    StarBonusNow=Voters.GetStarPower(account);
    ( YouAreStar, starActiveDays, NewGade4Invite)=starInfo(account);
    return (YouAreStar,starActiveDays,NewGade4Invite,WithdrawalDays,StarBonusNow,Father);}
   
    function starInfo (address account) internal view returns(uint256 YouAreStar,uint256 starActiveDays,uint256 NewGade4Invite){
    (bool YouAreStar1,bool YouAreStar2,bool YouAreStar3,bool YouAreStar4,uint256 NewGade4)=Voters.GetWhatStarYouAre(account);   
    (uint256 star1update,uint256 star2update,uint256 star3update,uint256 star4update)=Voters.GetStartUpdade(account);
    NewGade4Invite=NewGade4;
    if(YouAreStar1==true){YouAreStar=1;starActiveDays=star1update;}
    if(YouAreStar2==true){YouAreStar=2;starActiveDays=star2update;}
    if(YouAreStar3==true){YouAreStar=3;starActiveDays=star3update;}
    if(YouAreStar4==true){YouAreStar=4;starActiveDays=star4update;}
    return (YouAreStar,starActiveDays,NewGade4Invite);}
