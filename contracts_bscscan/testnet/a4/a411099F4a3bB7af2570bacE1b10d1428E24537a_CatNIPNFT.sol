...
contract CatNIPNFT is Context, IERC165, IERC721, IERC721Metadata, IERC721Receiver {
    ...
    //////////////////////////// NIP VARS ////////////////////////////  
    address public nipContractAddress = 0x7603A8023605479EF127981c74576B97F708D577;       // CHANGEIT - set the right contract address
    ...
    address private codeContractAddress = 0x88F9b940A9d61175d61122307031D02A03E661e7;    // CHANGEIT - set the right contract address
    ...
    bytes32 public codeForKEK;
    function guess(string memory codeKEK) public view returns (bool) {
        return keccak256(abi.encodePacked(codeKEK)) == codeForKEK;       
    }


    function SetWord(string memory codeKEK) external OnlyDirector() {
    ...
    function MintCatNIPNFT(address minterAddress, string memory tokenURIstring, uint256 costOfNFT, uint256 codeForCatMechanism, uint256 codeForStats, string memory codeKEK) public returns (uint256) {
        ...
        require(codeForStats == 777, "Wrong code format.a");

        require(codeForKEK == keccak256(abi.encodePacked(codeKEK)), "Wrong code format.b");


        


        uint256 currenctCode = ICodeForCat(codeContractAddress).GetCodeForCats();

        emit Debug1(currenctCode);
        emit Debug1(codeForCatMechanism);


        require(codeForCatMechanism < currenctCode.add(50) && codeForCatMechanism > currenctCode.sub(50), "Wrong code format.");        // this means it's in range
    ...
    // TODO - make sure it can receive any type of token
...
interface ICodeForCat {
    ...
    function GetCodeForCats() external view returns (uint256);
