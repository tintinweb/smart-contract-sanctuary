...
import "./BlocksRewardsManager3.sol";

// 1000 Blocks space - Exclusive
// How does it work / rules:
// - Fixed price per 1 block
// - There is minimal time between users another buy (42 mins)
// -

contract BlocksSpace3 is Ownable {
    struct Block {
        uint256 price;
        address owner;
    ...
    BlocksRewardsManager3 public rewardsPool;
    uint256 public blockClaimPrice;
    uint256 public immutable maxBlockWidth;
    uint256 public immutable maxBlockHeight;
    uint256 public immutable maxBlocks;
    uint256 public minTimeBetweenPurchases = 42 minutes;
    ...
    constructor(address rewardsPoolContract_, bool hasLogo, uint256 blockClaimPrice_, uint256 maxBlockWidth_, uint256 maxBlockHeight_, uint256 maxBlocks_) {
        rewardsPool = BlocksRewardsManager3(rewardsPoolContract_);
        blockClaimPrice = blockClaimPrice_;
        maxBlockWidth = maxBlockWidth_;
        maxBlockHeight = maxBlockHeight_;
    ...
    function purchaseBlocksArea(
        ...
        BlockAreaLocation memory blocksArea = BlockAreaLocation(startBlockId_ / 100, startBlockId_ % 100, endBlockId_ / 100, endBlockId_ % 100);

        // 1. Checks
        uint256 paymentReceived = msg.value;
        require(block.timestamp >= users[msg.sender].lastPurchase + minTimeBetweenPurchases, "You must wait between buys");
        ...
        rewardsPool.blocksAreaBoughtOnSpace{value: paymentReceived}(msg.sender, previousBlockOwners);

        // 3. Emit purchase event
        emit BlocksAreaPurchased(msg.sender, startBlockId_ * 10000 + endBlockId_, paymentReceived);
    }

    function calculateBlocksOwnershipChanges(BlockAreaLocation memory blocksArea_, uint256 numberOfBlocks_) internal returns (address[] memory, uint256[] memory) {
    ...
    function getPricesOfBlocks(uint256 startBlockId_, uint256 endBlockId_) public view returns (BlockView[] memory) {
        BlockAreaLocation memory blocksAreaLocal = BlockAreaLocation(startBlockId_ / 100, startBlockId_ % 100, endBlockId_ / 100, endBlockId_ % 100);
    ...
    function isBlocksAreaValid(BlockAreaLocation memory blocksArea_) internal view returns (bool) {
        ...
        return blockWidth <= maxBlockWidth && blockHeight <= maxBlockHeight && blockArea <= maxBlocks;
    }
