...
contract CoinToken is Context, IBEP20, Ownable {
    ...
    uint256 public     ORIG_TAXMEDIUM_FEE;
    uint256 public     ORIG_TAXHIGH_FEE;

    uint256 private ORIG_BURN_FEE;
    uint256 private ORIG_STARTUP_FEE;

    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _txmediumFee, uint256 _txhighFee, uint256 _burnFee,uint256 _startupFee,address _FeeAddress,address tokenOwner) {
		_NAME = _name;
		_SYMBOL = _symbol;
		_DECIMALS = _decimals;
		...
		_TAX_FEE = _txFee* 100; 
		_TAXMEDIUM_FEE = _txmediumFee* 100; 
		...
		ORIG_TAXMEDIUM_FEE = _TAXMEDIUM_FEE;
	...
	function updateFee(uint256 _txFee,uint256 _txmediumFee,uint256 _txhighFee, uint256 _burnFee,uint256 _startupFee) onlyOwner() public{
        _TAX_FEE = _txFee* 100; 
        _TAXMEDIUM_FEE = _txmediumFee* 100; 
        _TAXHIGH_FEE = _txhighFee* 100; 
...
//implements high tax for higher transactions        
...
//implements high tax for higher transactions        
    ...
    function removeAllFee() private {
        if(_TAX_FEE == 0 && _TAXMEDIUM_FEE==0 && _TAXHIGH_FEE==0 && _BURN_FEE == 0 && _STARTUP_FEE == 0) return;
...
_TAXMEDIUM_FEE = ORIG_TAXMEDIUM_FEE;
    ...
    function _getTaxmediumFee() private view returns(uint256) {
        return _TAXMEDIUM_FEE;
    }
