...
contract TestVerify is EIP712 {
  ...
  function verify(
    ...
    require(signer == admin, "verify: invalid signature");
...
//    require(block.number < _deadline, "MyFunction: signed transaction expired");
  ...
  function getKeccak(address _address) external view returns (bytes32) {
    return keccak256(abi.encode(_address));
  }

  function getKeccakEncodePacked(address _address) external view returns (bytes32) {
