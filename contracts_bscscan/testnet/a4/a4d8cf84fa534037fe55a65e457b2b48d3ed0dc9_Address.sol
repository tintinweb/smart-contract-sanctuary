...
contract DividendDistributor is IDividendDistributor {
    ...
    constructor (address _router) {
        router = _router != address(0)
            ? IRouter(_router)
            : IRouter(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
        _token = msg.sender;
    }
...
contract Test is Context, IERC20, Ownable {
    ...
    function setFeR(uint256 _rfi, uint256 _mark, uint256 _liq, uint256 _rew) external onlyOwner {
        feeRates.rfi = _rfi;
    ...
    function setSFeR(uint256 _rfi, uint256 _mark, uint256 _liq, uint256 _rew) external onlyOwner{
    ...
    function setrecadd(address _mkwa, address _spz, address _niz, address _jac, address _newra) external onlyOwner {
        mkwA = _mkwa;
        spzN = _spz;
        nizK = _niz;
        jacZ = _jac;
