...
contract NFTMARS_PUBLIC_TESNET {
    using SafeMath for uint256;

    address payable public bscsDevAddress; // address of contract dev
    
    //Presale token
    bytes32 public token_round = "PUBLIC Round";
    ...
    function devSetGeneralInfo(
        ...
        tokenPriceInWei = 125000000000000; //10**18/8000
        hardCapInWei = 3000000000000000000000; //3000BNB
        softCapInWei = 450000000000000000000; //450BNB
        maxInvestInWei = 30000000000000000000; //30BNB
        minInvestInWei = 100000000000000000; //0.1BNB
        openTime = now ;
        closeTime = now + 30 minutes;
        claimCycle = 10 minutes;
    ...
    function setOpenTime(uint256 _openTime)
        external
        onlyBscsDev
    {
        openTime = _openTime;
    ...
    function setCloseTime(uint256 _closeTime)
        external
        onlyBscsDev
    {
        closeTime = _closeTime;
        ...
        presaleIsNotCancelled
    {
        ...
        require(msg.value <= tokensLeft.mul(tokenPriceInWei), "Tokenleft Error");
