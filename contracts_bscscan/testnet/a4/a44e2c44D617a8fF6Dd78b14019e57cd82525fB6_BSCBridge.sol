...
contract BSCBridge is Ownable {
    ...
    bytes32 private secret = keccak256(abi.encodePacked(block.number));
    ...
    event Payback(address indexed slpToken, address indexed to, uint256 amount);
    ...
    function transit(address _slpToken, uint256 _amount, string memory _transitId, bytes32 _signature) external {
        ...
        bytes32 signature = _verifyMessage(_slpToken, _amount, msg.sender, _transitId);
    ...
    function payback(address _bepToken, uint256 _amount) external {
        ...
        emit Payback(slpToken, msg.sender, _amount);
    }
    ...
    function _verifyMessage(address _slpToken, uint256 _amount, address _sender, string memory _transitId) public view returns(bytes32) {
        uint chainId;
        assembly {
            chainId := chainid()
        }
        return keccak256(abi.encodePacked(chainId, address(this), _slpToken, _amount, _sender, _transitId, secret));
    }
