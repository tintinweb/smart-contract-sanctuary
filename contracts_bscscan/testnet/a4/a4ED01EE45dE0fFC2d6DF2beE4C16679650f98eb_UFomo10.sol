...
contract UFomo10 is Context, IERC20, Ownable {
    ...
    string private _name = "UFomo10";
    string private _symbol = "UFO10";
    uint8 private  _decimals = 18;
   
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 3 * (10**10) * (10**18);
    ...
    uint256 public _healthLevel = 0;
    uint256 private _previousHealthLevel = _healthLevel;
    ...
    //  uint256[6] public feeFomo = [0, 8, 11, 14, 17, 20];    
    // uint256[6] public feeFomo = [0, 8, 9, 10, 11, 13];  //7
    //    uint256[6] public feeFomo = [0, 1, 1, 1, 1, 1];  //8
      uint256[6] public feeFomo = [ 0, 8, 9, 10, 11, 13]; //17
        
    uint32 public _maxTxAmount = 20000000;
    uint32 public _tokenSwapThreshold = 20000000;
    uint16 public _initTimeLeft = 60 * 10;
    uint32 public _maxTimeLeft = 60 * 60 * 24;
    uint8 public _eliglblePlayers = 3;
    uint8 public _potPayoutPercent = 60;
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        // uint256 tLiquidityAndPot = tAmount.mul(_liquidityFee.add(feeFomo[_healthLevel])).div(100);
        uint256 tLiquidityAndPot = tAmount.mul(_liquidityFee).div(100);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _healthLevel == 0 && _devFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousLiquidityFee = _liquidityFee;
        _previousHealthLevel = _healthLevel;
    ...
    function restoreAllFee() private {
        ...
        _healthLevel = _previousHealthLevel;
    ...
    function getWinner(uint256 roundIdx)
        ...
        for(uint256 i = 0; i < _eliglblePlayers; i--) {
