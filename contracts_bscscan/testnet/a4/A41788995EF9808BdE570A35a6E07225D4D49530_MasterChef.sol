...
contract THORToken is ERC20("THORToken", "THOR"), Ownable {
    ...
    /**
     * @dev transfer with the voting power of the delegatee moved from token sender to token recipient.
     */
    ...
    /**
     * @dev transferFrom with the voting power of the delegatee moved from token sender to token recipient.
     */
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 pledgeTotalAmount;// the amount of pledge Token access into Pool.every pool has own pledge total amount
        uint256 poolType;//the type of pool// 1 =>LP, 2=>single Token ,//3 => Minter Token
    ...
    // The SUSHI TOKEN!
    THORToken public immutable thor;
    ...
    uint256 public allEndBlock;//10980*28800
    ...
    constructor(
        ...
        require(address(_thor) != address(0),'Non-zero address');
        thor = _thor;
        WBNB = _wbnb;
        //31104192388293170000 per block,895800.7407828433 per day//31.104192388293170000
        sushiPerBlock = _sushiPerBlock;
        startBlock = block.number;

        allEndBlock = startBlock + endDays * ONE_DAY_BLOCKS;
        deflationRate_[1] = 1e12;
        for(uint256 i=2; i<= 730; i++) {
            deflationRate_[i] = deflationRate_[i-1].mul(998).div(1000);
        }
    ...
    function setDeflationRate_(uint256 _startIndex, uint256 _endIndex) external onlyOwner {
        require(_startIndex >= 2, "_startIndex must GT 2");
        require(_endIndex.sub(_startIndex) <=1000,'lenght must be LT 1000');
        // deflationRate_ decline 0.2% daily
    ...
    function setAllDeflaRate(uint256 _startIndex,uint256 _endIndex) external onlyOwner returns(bool) {
        require(_startIndex >= 2, "_startIndex must GT 2");
        require(_endIndex.sub(_startIndex) <=1000,'lenght must be LT 1000');
    ...
    function setAllEndBlock(uint256 _allEndBlocks) external onlyOwner {
        allEndBlock = _allEndBlocks;
    }
    ...
    function add(uint256 _allocPoint, uint256 _poolType, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function set(uint256 _pid, uint256 _allocPoint,uint256 _poolType, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function pendingSushi(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pledgeTotalAmount != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 sushiReward = multiplier.mul(sushiPerBlock).mul(pool.allocPoint).div(totalAllocPoint).div(1e12);
            accSushiPerShare = accSushiPerShare.add(sushiReward.mul(1e12).div(pledgeTotalAmount));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        thor.mint(address(this), sushiReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) public payable nonReentrant{
        ...
        if(_pid == 0 && msg.value > 0) {
            pool.pledgeTotalAmount = pool.pledgeTotalAmount.add(msg.value);
            ...
            require(msg.value == 0, "This pool cannot receive BNB!");
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant{
        ...
        UserInfo storage user = userInfo[_pid][msg.sender];
        if(_pid == 0) {
            safeTransferBNB(address(msg.sender), user.amount);
        } else{
    ...
    function safeThorTransfer(address _to, uint256 _amount) internal {
    ...
    function transferBMintTokenOwnership (address _newThorOwner) public onlyOwner returns(bool) {
        require(_newThorOwner != address(0), "Ownable: new owner is the zero address");
        thor.transferOwnership(_newThorOwner);
        return true;
    }

    // return the days of xxx block number from startBlock
    function getGivenBlockDay(uint256 blocknumber) external view returns (uint256) {
