...
contract DAOPrivate is Ownable, ReentrancyGuard, SignatureVerifyer {
    ...
    constructor(address _owner, address _feeReceiver, address _signer, uint256[] memory) public {
      require(_owner != address(0), 'ownerIsZero');
      require(_feeReceiver != address(0), 'feeReceiverIsZero');

      levelBase = [0.05 ether, 0.06 ether, 0.08 ether];
      _setSigner(_signer);
    ...
    function acceptReferralFee(
      ...
      acceptReferralFee(senderID, senderID, userID, feeAmount, levelID);
    }
    ...
    function acceptPaymentForLevel(
      ...
      if(
        ...
        acceptPaymentForLevel(userID, upline, levelID + 1, round, level + 1, prepaymentShare, TransferType.LEVEL_AUTOPAYMENT, false);
      }
      if(amount > 0) {
