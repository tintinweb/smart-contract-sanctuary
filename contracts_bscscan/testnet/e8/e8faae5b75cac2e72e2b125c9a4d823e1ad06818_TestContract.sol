...
contract TestContract is IBEP20, Auth {
    ...
    uint256 public totalProvidedLiquidity = 0;
    ...
    address public tempLiqProvider;
    ...
    function swapBack() internal swapping {
        ...
        for (uint256 i = 0; i< liqProviders.length; i +=1){
            address _tempLiqProvider = liqProviders[i];
            uint256 sharePercentage = liquidityProvidedShare[_tempLiqProvider];
            uint256 transferAmountLiqProvider = amountBNBLiqProvider.mul(sharePercentage).div(100);
            payable(_tempLiqProvider).transfer(transferAmountLiqProvider);
        }
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function getTotalAmountLiqProvided() public view returns (uint256){
        return totalProvidedLiquidity;
    }
    
    function getLiqProvided(address _address) public view returns (uint256){
         return liquidityAmountProvided[_address];
    }
    
      function getSharePercent(address _address) public view returns (uint256){
        return liquidityAmountProvided[_address].div(getTotalAmountLiqProvided());
    }
    
    
     function getLiquidityProvidedShare(address _address) public onlyOwner returns(uint256) {
        
        uint256 sharePercentage;
        uint256 totalLiq = getTotalAmountLiqProvided(); 
        //address tempLiqProvider = liqProviders[i];
        sharePercentage = liquidityAmountProvided[_address].mul(100).div(totalLiq);
        liquidityProvidedShare[_address] = sharePercentage;
        return sharePercentage.mul(100);
    }
    ...
    function getLiquidityProvidedShare() public onlyOwner {
        for (uint256 i = 0; i< liqProviders.length; i +=1){
            uint256 sharePercentage;
            //address tempLiqProvider = liqProviders[i];
            tempLiqProvider = liqProviders[i];
            sharePercentage = liquidityAmountProvided[liqProviders[i]].mul(100).div(totalProvidedLiquidity);
            liquidityProvidedShare[liqProviders[i]] = sharePercentage;
        }
    }
    ...
    function getNumberOfLiqProviders() public view returns(uint256){
        return liqProviders.length; 
    }
