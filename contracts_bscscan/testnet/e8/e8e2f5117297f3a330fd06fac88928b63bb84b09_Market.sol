...
interface IMarket {
    function buyToken(uint256 _tokenId, uint256 _maximumPrice) external;

    function buyTokenTo(uint256 _tokenId, address _to, uint256 _maximumPrice) external;
    ...
    function sellTokenTo(uint256 _tokenId, address _to, uint256 _minimumPrice) external;
...
contract Market is IMarket, ERC721HolderUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function buyToken(uint256 _tokenId, uint256 _maximumPrice) public override whenNotPaused {
        buyTokenTo(_tokenId, _msgSender(), _maximumPrice);
    }

    function buyTokenTo(uint256 _tokenId, address _to, uint256 _maximumPrice) public override whenNotPaused {
        require(_msgSender() != address(0) && _msgSender() != address(this), 'Wrong msg sender');
        require(_asksMap[_tokenId] > 0, 'Token not in sell book');
        require(_userBids[_msgSender()][_tokenId] == 0, 'You must cancel your bid first');
        nft.safeTransferFrom(address(this), _to, _tokenId);
        uint256 price = _asksMap[_tokenId];
    ...
    function bidToken(uint256 _tokenId, uint256 _price) public override whenNotPaused {
        ...
        require(_userBids[_to][_tokenId] == 0, 'Bidder already exists');
        quoteErc20.safeTransferFrom(address(_msgSender()), address(this), _price);
        _userBids[_to][_tokenId] = _price;
    ...
    function updateBidPrice(uint256 _tokenId, uint256 _price) public override whenNotPaused {
        require(_userBids[_msgSender()][_tokenId] > 0, 'Only Bidder can update the bid price');
        require(_price != 0, 'Price must be granter than zero');
    ...
    // NFT에 대한 Bidding이 얼마인지 가지고 오는 함수
    ...
    // NFT Bidding 리스트에서 특정 Bidding을 삭제
    function delBidByTokenIdAndIndex(uint256 _tokenId, uint256 _index) private {
        _userBids[_tokenBids[_tokenId][_index].bidder][_tokenId] = 0;
        // delete the bid
    ...
    // NFT Bidding을 NFT 소유자가 판매
    // tokenId: NFT 토큰 아이디
    // to: 판매하는 주소
    // minimumPrice: 최소 판매 가격
    function sellTokenTo(uint256 _tokenId, address _to, uint256 _minimumPrice) public override whenNotPaused {
        address _seller = _tokenSellers[_tokenId];
        address _owner = address(_msgSender());
        require(_seller == _owner || nft.ownerOf(_tokenId) == _owner, 'Only owner can sell token');
        // find  bid and the index
        (BidEntry memory bidEntry, uint256 _index) = getBidByTokenIdAndAddress(_tokenId, _to);
        uint256 price = bidEntry.price;
        require(price != 0, 'Bidder does not exist');
        require(_minimumPrice <= price, 'invalid price');
        // transfer token to bidder
        nft.safeTransferFrom(_seller == _owner ? address(this) : msg.sender, _to, _tokenId);
        uint256 feeAmount = price.mul(feePercent).div(100);
        if (feeAmount != 0) {
            quoteErc20.transfer(feeAddr, feeAmount);
        }
        quoteErc20.transfer(msg.sender, price.sub(feeAmount));
        _asksMap[_tokenId] = 0;
        _userSellingTokens[_tokenSellers[_tokenId]].remove(_tokenId);
    ...
    function cancelBidToken(uint256 _tokenId) public override whenNotPaused {
        require(_userBids[_msgSender()][_tokenId] > 0, 'Only Bidder can cancel the bid');
    ...
    // NFT 소유자가 판매하려는 가격
    function getAskPrice(uint256 tokenId) external view returns (uint256) {
        return _asksMap[tokenId];
    }

    // 특정 유저가 NFT에 어느 가격에 Bidding을 했나 알려주는 기능
    function getUserBidPrice(address user, uint256 tokenId) external view returns (uint256) {
        return _userBids[user][tokenId];
    }
}