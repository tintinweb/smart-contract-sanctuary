 ...
 */
contract LazerMerchantWallet is Ownable, ILazerStorageSchema {
    ...
    event WalletCreationEvent(address contractWalletAddress, uint256 merchantsID, uint256 createdAt);
    event TokenWithdrawalEvent(uint256 amount, uint256 commissionFee, address destinationAddress,
     address contractWalletAddress, uint256 merchantsID);
    ...
    function createMerchantsWallet(
        uint256 _merchantsID,
        uint256 createdAt,
        address tokenAddress
    ) external returns (address) {
        ERC20Wallet wallet = new ERC20Wallet(owner, WalletAdmin);
        storageContract.createWalletMapping(_merchantsID, address(wallet), createdAt, tokenAddress);
        storageContract.createWalletMappingToWalletID(_merchantsID, address(wallet), createdAt, tokenAddress);
    ...
    function withdrawFunds(
        ...
        require(wallet.tokenBalanceOf() > 0, "MerchantWallet: Wallet is empty");
        ...
        emit TokenWithdrawalEvent(amountToWithdraw, commissionFee, recipient, address(wallet), merchantsId);
        return true;
    }
}
...
contract ERC20Wallet is Ownable {
    ...
    function tokenBalanceOf() public view returns (uint256 amount) {
