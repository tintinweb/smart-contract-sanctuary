...
contract BidderTreasury is Ownable {
    ...
    constructor (
        ...
        isReserveToken[ _MMT ] = true;
        reserveTokens.push( _MMT );
    ...
    /**
        @notice allow approved address to deposit an asset for BID
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( bool ) {
        require( isReserveToken[ _token ] || isLiquidityToken[ _token ], "ReserveToken Or LiquidityToken Not accepted" );
        IERC20( _token ).safeTransferFrom( msg.sender, address(this), _amount );

        if ( isReserveToken[ _token ] ) {
            require( isReserveDepositor[ msg.sender ], "ReserveDepositor Not approved" );
        } else {
            require( isLiquidityDepositor[ msg.sender ], "LiquidityDepositor Not approved" );
        }

        uint value = valueOf(_token, _amount);
        // mint BID needed and store amount of rewards for distribution
        uint send_ = value.sub( _profit );
        IERC20Mintable( Bidder ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn BID for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IOHMERC20( Bidder ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( SBidder ).balanceOf( msg.sender ); // Can only borrow against sOHM held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with BID
        @param _amount uint
     */
    function repayDebtWithOHM( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Debtor Not approved" );

        IOHMERC20( Bidder ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Bidder, _amount, _amount );
    }
    ...
    function manage( address _token, uint _amount ) external {
        if( isLiquidityToken[ _token ] ) {
            require( isLiquidityManager[ msg.sender ], "LiquidityManager Not approved" );
        } else {
            require( isReserveManager[ msg.sender ], "ReserveManager Not approved" );
        }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        require( isRewardManager[ msg.sender ], "RewardManager Not approved" );
        require( _amount <= excessReserves(), "Insufficient reserves" );
        IERC20Mintable( Bidder ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Bidder ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns BID valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match BID decimals
            value_ = _amount.mul( 10 ** IERC20( Bidder ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
