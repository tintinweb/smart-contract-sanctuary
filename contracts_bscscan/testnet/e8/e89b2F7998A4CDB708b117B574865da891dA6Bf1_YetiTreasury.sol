...
contract YetiTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SYETI }
    ...
    address public sYETI;
    uint public sYETIQueue; // Delays change to sYETI address
    ...
    constructor (
       ...
       /* isLiquidityToken[ _YETIBUSD ] = true;
       liquidityTokens.push( _YETIBUSD ); */

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for YETI
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint YETI needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( YETI ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn YETI for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IYETIERC20( YETI ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sYETI ).balanceOf( msg.sender ); // Can only borrow against sYETI held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with YETI
        @param _amount uint
     */
    function repayDebtWithYETI( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IYETIERC20( YETI ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, YETI, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( YETI ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( YETI ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns YETI valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match YETI decimals
            value_ = _amount.mul( 10 ** IERC20( YETI ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SYETI ) { // 9
            sYETIQueue = block.number.add( blocksNeededForQueue );
        } else return false;
