...
contract Pazzive is ERC20, Ownable {
    ...
    mapping(address => uint256) public userStakingBonus;
    mapping(address => uint256) public userStakeEndTime;
    mapping(uint256 => uint256) public stakingDurationToBonus;
    ...
    event SetUserRewardToken(address account, address token);
    event SetAllowedTokens(address token, bool allow);
...
//    }
    ...
    function setStakingDurationToBonus(uint256 duration, uint256 bonus) public onlyOwner {
        require(stakingDurationToBonus[duration] != bonus, "Pazzive: Bonus already set for duration");
        require(bonus <= 100, "Pazzive: Staking bonus can't exceed 100");

        stakingDurationToBonus[duration] = bonus;
    ...
    */
    ...
    function stake(uint256 duration) public {
        ...
        require(stakingDurationToBonus[duration] != 0, "Pazzive: Invalid staking duration");
        require(userStakeEndTime[_msgSender()] < block.timestamp.add(duration), "Pazzive: Already staked for a longer duration");

        userStakingBonus[_msgSender()] = stakingDurationToBonus[duration];
        userStakeEndTime[_msgSender()] = block.timestamp.add(duration);
    ...
    function getStakingInfo(address account) external view returns (uint256, uint256){
        return (userStakeEndTime[account], userStakingBonus[account]);
    }
...
//    // todo: i think we can remove this and just do a check normally instead of keeping a list
//    function setExcludedFromAutoClaim(bool value) external {
//        dividendTracker.setExcludedFromAutoClaim(msg.sender, value);
//    }

    // todo: also think we can remove this as we don't need to make a distinction between reinvesting in paz or ts
    ...
    function handleStaking(address from) internal {
        if (!swapPairs[from] && stakingEnabled) {
            require(userStakeEndTime[from] <= block.timestamp, "Pazzive: Tokens are locked during staking period");
            if (userStakeEndTime[from] != 0) {
                userStakeEndTime[from] = 0;
                userStakingBonus[from] = 0;
            }
        }
    }
...
//        return totalFee.add(feeIncrease.mul(remainingTime).div(buybackMultiplierLength));
    ...
    function getBalance(address account) public view returns (uint256) {
        return stakingEnabled ? balanceOf(account).mul(userStakingBonus[account].add(100)).div(100) : balanceOf(account);
    }
    ...
    function setUserRewardToken(address token) public {
        ...
        emit SetUserRewardToken(msg.sender, token);
    }
...
//        (bool successMarketing,) = address(marketingAddress).call{value: marketingShare}("");
    ...
    */
    ...
    function sweepTracker() external onlyOwner{
        dividendTracker.sweep();
    }

    function recoverTrackerTokens(address _token, address _to) external onlyOwner {
        dividendTracker.recoverTokens(_token, _to);
    }
}
...
//  function owner () external view returns ( address );
  ...
  function stakingDurationToBonus ( uint256 ) external view returns ( uint256 );
...
//  function symbol () external view returns ( string );
  ...
  function userStakeEndTime ( address ) external view returns ( uint256 );
  function userStakingBonus ( address ) external view returns ( uint256 );
