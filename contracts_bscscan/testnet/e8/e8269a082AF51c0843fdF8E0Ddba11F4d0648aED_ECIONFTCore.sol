...
contract ECIONFTCore is Initializable, ERC721Upgradeable, ERC721BurnableUpgradeable, ERC721URIStorageUpgradeable, AccessControlUpgradeable {
    ...
    mapping(uint256 => string)  public partCodes;
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://metadata.ecio.space/nft-card/";
    }

     function _setTokenURI(uint256 tokenId, string memory _tokenURI) internal virtual override {
        require(_exists(tokenId), "ERC721URIStorage: URI set of nonexistent token");
        _tokenURI = string(abi.encodePacked(_tokenURI, "?partCode=", partCodes[tokenId]));
        super._setTokenURI(tokenId,_tokenURI);
    }

    function safeMint(address to, string memory partCode) public onlyRole (MINTER_ROLE) {
        _safeMint(to, _tokenIdCounter.current());
        partCodes[_tokenIdCounter.current()] = partCode;
        createdAt[_tokenIdCounter.current()] = block.timestamp;
