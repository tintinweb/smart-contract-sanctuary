...
contract SpeedsterNft is ERC721URIStorage, Ownable {
    ...
    uint constant MAX_MINT = 25;
    uint public ENERGY_REGEN_RATE;
    uint public BASE_FLIGHT_COST;
    /* Only Minted */
    ...
    constructor () ERC721("YoloShips", "Y_SHIP"){
        tokenCounter = 0;
        tokenSupply = 0;
        ENERGY_REGEN_RATE = 1; //per hour
        BASE_FLIGHT_COST = 1; //Energy
    }
    ...
    modifier ensurePaid() {
        ...
        _;
        require(address(this).balance - balBef >= msg.value, "Didn't receive BNB");
    }
    
    uint mintPrice = 1e17;

    function create(address _account) external payable ensurePaid() {
        //Must check if has 0!
    ...
    // uint speedIncreasePL = 5;
    
    function levelUP(uint _amountOfLevels) external returns(uint newLvl){
    ...
    function explorationFlight(uint _energySpent) external returns(uint returnBlock) {
        ShipInfo memory nftInfo = allShips[msg.sender];
        require(_energySpent == 5 || _energySpent == 2 || _energySpent == 5, "Incorrect energy input!");
        require(_energySpent <= nftInfo.energy, "Insufficient Energy!");
        // (uint chestType, uint reward) = generateChest(_energySpent);
    ...
    function getRandomDefender(address _attacker) internal view returns(address defenderAddress) {
       ...
       _structHash = keccak256(
        ...
        randomNumber = uint256(_structHash) % this.totalSupply();

        for(uint i = 0; i < this.totalSupply(); i++){
            if(idToAccount[randomNumber] != _attacker) {
               return idToAccount[randomNumber];
            } else {
                 randomNumber = (uint256(_structHash) + i) % this.totalSupply();
            }
        }
    }
    ...
    function fightingSimulator(address _attacker, address _defender) internal view returns(address winner){
        ...
        //MAY THE FUN BEGIN
        for(uint i = 0; i < 10; i++){
            bool hitOrMissAttacker = randomHit(i, 1);
            bool hitOrMissDefender = randomHit(i, 2);
    ...
    // 1 = attacker
    // 2 = defender
    function randomHit(uint _round, uint _attackerOrDefender) internal view returns (bool didHitOrMiss) {
       bytes32 _structHash;
       uint256 randomNumber;
       bytes32 _blockhash = blockhash(block.number-1);
       ...
       outherFactor[0] = block.difficulty;
       outherFactor[1] = gaslefted;
       outherFactor[2] = _round;
       outherFactor[3] = allShips[idToAccount[_round]].energy + _attackerOrDefender; 
