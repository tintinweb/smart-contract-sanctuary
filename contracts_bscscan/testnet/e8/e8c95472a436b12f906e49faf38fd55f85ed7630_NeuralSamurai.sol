...
contract NeuralSamurai is ERC721Enumerable, RSunAuth {
    ...
    uint public increaseEvery = 100;

    // Fees
    FeeReceiver[] feeReceivers;
    uint totalWeight;
    bool pushAutomatically = true;
    uint pushThreshold = 1 * 10 ** 18; // 1 BNB
    ...
    function purchasePack(uint8 packId) external payable {
        Pack memory pack = packTypes[packId];
        require(pack.saleRunning == true, "Sale of pack has not started.");
        require(msg.value == getPrice(pack), "Incorrect BNB");
    ...
    function getPrice(Pack memory pack) public view returns (uint price) {
    ...
    function addNewPack(uint240 basePrice_, uint8 numberOfCards_) external authorizedFor(Permission.ManagePacks) {
