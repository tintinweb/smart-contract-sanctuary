...
contract Vault is Context, Ownable, ReentrancyGuard {
	...
	bool constant _nativeToken = false;
	...
	function lock(uint256 lockAmount, uint256 lockPeriod) external payable nonReentrant returns (bool) {
		...
		if (_nativeToken) {
		    require( msg.value >= lockAmount, "Lock amount exceed balance");
		} else {
...
// 		uint lockPeriodinSecond = lockPeriod.mul(86400); //day
	...
	function unlock(uint id) external nonReentrant returns (bool) {
		...
		if (_nativeToken) {
            payable(sender).transfer(lockAmount);
		} else {
