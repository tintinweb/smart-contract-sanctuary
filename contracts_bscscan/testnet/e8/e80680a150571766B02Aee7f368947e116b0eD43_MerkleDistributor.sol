...
contract MerkleDistributor is IMerkleDistributor {
    ...
    uint256 public constant totalCompensate = 96645350000000000000000000;
    uint256 public constant totalBEARN = 1000000000000000000000000;
    ...
    function claim(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) external override notPaused {
        ...
        _setClaimed(index);
        uint256 _amtBEARN = (amount.mul(totalBEARN)).div(totalCompensate);
        require(IERC20(token).transfer(account, _amtBEARN), "MerkleDistributor: Transfer failed.");

        emit Claimed(index, account, amount);
    }

    function governanceRecoverUnsupported(address _to, address _token, uint256 _amount) external onlyOperator {
        IERC20(_token).transfer(_to, _amount); // In case of emergency, we pull back token to the origin (old strategy)
    }
}