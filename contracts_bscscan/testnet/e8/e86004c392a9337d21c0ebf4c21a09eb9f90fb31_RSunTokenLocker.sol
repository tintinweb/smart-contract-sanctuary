...
contract RSunTokenLocker is RSunAuth {
    ...
    event TokensLocked(address owner_, uint112 amount, uint48 duration);
    ...
    function lockTokens(address tokenAdr, uint112 amount, uint48 duration, bool isVested, address owner_) public {
        ...
        emit TokensLocked(owner_, amount, duration);
    }
    ...
    function withdrawTokens(uint lockIndex) public {
        ...
        uint amount = lock.isVested ? lock.amount * (block.timestamp - lock.lastWithdrawn) / lock.duration : lock.amount;
