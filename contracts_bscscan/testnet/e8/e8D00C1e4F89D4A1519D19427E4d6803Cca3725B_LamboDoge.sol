...
contract LamboDogeVault is Ownable, ReentrancyGuard {
	using SafeMath for uint256;
	...
	function withdrawBNB(uint256 _index) external authorizedCallers {
        ...
        (bool sent,) = address(op.account).call{value : toRetrieve}("");
	...
	function withdrawToken(uint256 _index) external authorizedCallers {
		...
		op.amount = 0;
		op.account = address(0);
		bool sent = IBEP20(op.tokenAddress).transfer(op.account,toRetrieve);
...
contract LamboDoge is Context, IBEP20, Ownable, ReentrancyGuard {
	...
	LamboDogeVault public immutable vault;
	...
	constructor (
		...
		vault = new LamboDogeVault(_msgSender());
...

{
  ...
  "libraries": {
    "contracts/LamboDoge.sol": {
      "Utils": "0xb43b622159fa607812e51c4581ca4ac2156a463e"
    }
  }
}