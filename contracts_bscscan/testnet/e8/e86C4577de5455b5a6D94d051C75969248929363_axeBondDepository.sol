...
contract axeBondDepository is AccessControlled {
  ...
  ) external onlyGovernor returns (uint256 id_) {
   ...
   * @param _id uint
   */
  function deprecateBond(uint256 _id) external onlyGovernor {
  ...
  function deposit(
    ...
    info.principal.transfer(address(treasury), _amount); // send payout to treasury
