...
contract ShaCoin is Context, IERC20, Ownable {
    ...
    string private _name = "ShaCoin"; 
    string private _symbol = "SHAB"; 
    ...
    uint256[5] public feeFee = [0, 5, 7, 9, 15];
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tDev, uint256 tFee) {
        if (!feeIt) {
            return (tAmount, 0,  0);
        }
        uint256 healthLevel = 4;
        if (healthLevel == 0) {
            tDev = 0;
        } else {
            tDev = tAmount.mul(3).div(100);
        }
        tFee = tAmount.mul(feeFee[healthLevel]).div(100);
        tTransferAmount = tAmount.sub(tDev).sub(tFee);
    }
    ...
    function _takeTax(uint256 tDev) private {
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount,  uint256 tDev, uint256 tFee) = _getTValues(tAmount);
