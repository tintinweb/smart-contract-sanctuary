...
contract WenDAOTreasury is Ownable {
    ...
    address public sWenDAO;
    uint public sWDAOQueue; // Delays change to sWDAO address
    ...
    //    isLiquidityToken[ _WDAOBUSD ] = true;
    //    liquidityTokens.push( _WDAOBUSD );
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        IERC20Mintable( WenDAO ).mint( msg.sender, send_ );
    ...
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IWDAOERC20( WenDAO ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sWenDAO ).balanceOf( msg.sender ); // Can only borrow against sWDAO held
    ...
    function repayDebtWithWDAO( uint _amount ) external {
        ...
        IWDAOERC20( WenDAO ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, WenDAO, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( WenDAO ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( WenDAO ).totalSupply().sub( totalDebt ) );
    }
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = _amount.mul( 10 ** IERC20( WenDAO ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SWDAO ) { // 9
            sWDAOQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
