...
contract Vault is VaultStorage {
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        if(_amount > 0) {
            vaultAsset.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function _withdraw(address account, uint256 _amount) internal {
        ...
        if(_amount > 0) {
            ...
            vaultAsset.safeTransfer(address(account), _amount);
        }
        user.rewardDebt = user.amount.mul(accAtlantisPerShare).div(1e18);
    ...
    function updateVault() internal {
        ...
        if (vaultAssetBalance == 0) { // avoids division by 0 errors
            return;
        }

        accAtlantisPerShare = accAtlantisPerShare.add(pendingRewards.mul(1e18).div(vaultAssetBalance));
        pendingRewards = 0;
    }
    ...
    /**
     * @notice Set Atlantis info for this Vault
     * @param _atlantis Atlantis token address
     * @param _vaultAsset single or LP contract address
     */
    function setAtlantisInfo(address _atlantis, address _vaultAsset) public onlyAdmin {
        atlantis = IBEP20(_atlantis);
        vaultAsset = IBEP20(_vaultAsset);
