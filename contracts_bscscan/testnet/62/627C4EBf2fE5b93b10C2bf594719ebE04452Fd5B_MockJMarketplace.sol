...
contract JMarketplace is AccessControlUpgradeable, ERC1155HolderUpgradeable
{
    ...
    function initialize(address _admin, address _feeManager, address _jnft) 
        public initializer
    {
        __AccessControl_init_unchained();
        __ERC1155Holder_init_unchained();
        __JMarketplace_init_unchained(_admin, _feeManager, _jnft);
    }

    function __JMarketplace_init_unchained(address _admin, address _feeManager, address _jnft)
        ...
        external 
    {
        for(uint256 i; i < _amount; i++) {
            sell(_tokenId, _acceptedToken, _price, _startTime, _endTime);
        }
    }
    ...
    {
        require(jnft.balanceOf(msg.sender, _tokenId) != 0, "need a token");
        ...
        jnft.safeTransferFrom(msg.sender, address(this), _tokenId, 1, "");
        ...
        external
    {
        ...
        jnft.safeTransferFrom(address(this), msg.sender, _listing.tokenId, 1, "");
        ...
        external
    {
        ...
        uint256 _platformFee = feeManager.calculateFee(_listing.price);
        uint256 _royaltyFee = jnft.calculateRoyaltyFee(_listing.tokenId, _listing.price);
        uint256 _finalAmount = _listing.price.sub(_platformFee).sub(_royaltyFee);
        address _creator = jnft.creatorOf(_listing.tokenId);
    ...
    {
        require(jnft.exists(_tokenId), "not exitst");
        require(_endTime > _startTime, "invalid time");
        require(_price != 0, "invalid price");
        require(_acceptedToken != address(0), "invalid accepted token");
        ...
        IERC20(_acceptedToken).safeTransferFrom(msg.sender, address(this), _price);

        emit Buy(_offerId, msg.sender, _tokenId, _acceptedToken, _price, _startTime, _endTime);
        ...
        external
    {
        ...
        IERC20(_offer.acceptedToken).safeTransfer(msg.sender, _offer.price);

        emit CancelOrder(_offerId, msg.sender);
    }
        ...
        external
    {
        ...
        uint256 _platformFee = feeManager.calculateFee(_offer.price);        
        uint256 _royaltyFee = jnft.calculateRoyaltyFee(_offer.tokenId, _offer.price);
        uint256 _finalAmount = _offer.price.sub(_platformFee).sub(_royaltyFee);
        address _creator = jnft.creatorOf(_offer.tokenId);
        address _feeCollector = feeManager.getFeeCollector();
        IERC20(_offer.acceptedToken).safeTransfer(_feeCollector, _platformFee);
        IERC20(_offer.acceptedToken).safeTransfer(_creator, _royaltyFee);
        IERC20(_offer.acceptedToken).safeTransfer(msg.sender, _finalAmount);

        jnft.safeTransferFrom(msg.sender, _offer.owner, _offer.tokenId, 1, "");
...
import "../JMarketplace.sol";

contract MockJMarketplace is JMarketplace
{
