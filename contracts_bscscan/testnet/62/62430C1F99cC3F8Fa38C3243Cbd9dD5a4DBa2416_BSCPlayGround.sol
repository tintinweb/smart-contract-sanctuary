...
contract BSCPlayGround is ERC20, Ownable
{
    ...
    uint8 constant public BUSD_REWARDS_FEE = 3;
    uint8 constant public PRIZE_POOL_FEE = 2;
    uint8 constant public MARKETING_BUYBACK_FEE = 2;
    uint8 constant public TOTAL_FEES = 7; // Sum of all fees

    address public marketingWallet = 0x1664a715B15345C50bdd3b92Bf3EB7a4E4d39B4A;
    address public partnersWallet = 0xd962c2cB2B006049B2f7B2e2a142a4020804d413;
    address public airdropWallet = 0xa3D296669d4a7268fc85Ac6261b31Ca8E6AEebc8;
    ...
    modifier airdrop()
    {
     ...
     modifier partners()
    {
        _inPartnersTransfer = true;
        _; 
        _inPartnersTransfer = false; 
    }

    receive() external payable {}

    constructor() ERC20("PlayGroundToken", "PGT"){
        ...
        _excludeFromFeesAndDividends(partnersWallet, true);
        _excludeFromFeesAndDividends(marketingWallet, true);
        _excludeFromFeesAndDividends(airdropWallet, true);
        _excludeFromFeesAndDividends(prizePoolWallet, true);
    ...
    function maxApproveTournament(address tournament) external {
        require(_tournaments[tournament], 'IT IS NOT TOURNAMENT ADDRESS');
        _approve(msg.sender, tournament, type(uint128).max);
    }
    ...
    function airdropTransfer(address[] memory receivers, uint256[] memory amounts) public onlyOwner airdrop  
    {
        ...
        uint256 amount = _operationTransfer(receivers, amounts, airdropWallet);
        emit AirdropTransfered(amount);
    }

    function partnersTransfer(address[] memory receivers, uint256[] memory amounts) public onlyOwner partners  
    {
        require(receivers.length == amounts.length, 'Receivers length not equals amounts');
        uint256 amount = _operationTransfer(receivers, amounts, partnersWallet);
        emit PartnersTransfered(amount);
    }

    function _operationTransfer(address[] memory receivers, uint256[] memory amounts, address sender) private returns(uint256)
    {
        uint256 amount = 0;
        for(uint256 index = 0; index < receivers.length; index++)
        {
            require(amounts[index] <= balanceOf(sender));
            require(receivers[index] != address(0));

            _transfer(sender, receivers[index], amounts[index]);
    ...
    function registerTournament(address tournament) external onlyOwner {
        ...
        _approve(prizePoolWallet, tournament, type(uint128).max);
    }
    ...
    function unregisterTournament(address tournament) external onlyOwner{
        ...
        _approve(prizePoolWallet, tournament, 0);
    }
    ...
    function transferFrom(
        ...
        {
            require(_tournaments[recipient] || _tournaments[msg.sender], 'Tokens from prizePoolWallet can sends only for tournaments.');
            if( allowance(sender, msg.sender) < amount )
                _approve(sender, msg.sender, type(uint128).max);   
        }
    ...
    function _transfer(
        ...
        uint256 amount)
    internal override  {
        if(sender == airdropWallet) require(_inAirdropTransfer, 'Tokens from airdropWallet can sends only for special function.');   
        if(sender == partnersWallet) require(_inPartnersTransfer, 'Tokens from partnersWallet can sends only for special function.');
        if(sender == prizePoolWallet) require(_tournaments[recipient] || _tournaments[msg.sender], 'Tokens from prizePoolWallet can sends only for tournaments.');

        if(_inSwap || _inAirdropTransfer || _inPartnersTransfer) 
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        uint256 _amountFee = amount.mul(TOTAL_FEES).div(100);
    ...
    function swapBack() internal swapping {
        ...
        uint256 marketingAmount = amount.mul(MARKETING_BUYBACK_FEE).div(TOTAL_FEES);
        uint256 prizePoolAmount = amount.mul(PRIZE_POOL_FEE).div(TOTAL_FEES);
        uint256 rewardsAmount = amount.sub(marketingAmount).sub(prizePoolAmount);

        swapTokensForDividends(rewardsAmount);
        swapTokensForMarketing(marketingAmount);
        sendToPrizePool(prizePoolAmount);
    }

    function swapTokensForDividends(uint256 amount) private{
        uint256 amountBNB = swapTokens(amount);
        try _distributor.deposit{value: amountBNB}() {} catch {}
        emit SwapTokensForDividends(amount);
    }

    function swapTokensForMarketing(uint256 amount) private returns (bool){
        uint256 amountBNB = swapTokens(amount);
        (bool success, ) = payable(marketingWallet).call{value: amountBNB, gas: 30000}("");
        return success;
    }
