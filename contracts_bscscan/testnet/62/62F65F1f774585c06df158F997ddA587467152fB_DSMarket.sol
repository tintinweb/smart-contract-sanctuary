...
contract DSMarket is ReentrancyGuard {
    ...
    function getNftContract() view public onlyOwner returns (DSNFT) {
        return nftContract;
    }
    
    function setNftContract(address _nftContract) public onlyOwner {
        nftContract = DSNFT(_nftContract);
    }
    ...
    function getMarketItem(uint256 _itemId) view public onlyOwner returns (
    ...
    function getInitialSoldTicket(uint256 tokenId) view public returns (bool) {
    ...
    function initialTicketSale(
        ...
        require(!tokenSold, "DSMarketplace: This tokenId is already sold from initial ticket sale");
        require(amount >= initialPrice, "DSMarketplace: Please submit the asking price in order to complete the purchase");
    ...
    function createMarketItem(
        ...
        require(msg.sender == IERC721(nftContract).ownerOf(tokenId), "DSMarketplace: Should be the owner of token");
    ...
    function createMarketSale(
        ...
        uint256 priceDBZ = idToMarketItem[itemId].priceDBZ;
    ...
    function cancelMarketItem(uint256 itemId) public nonReentrant {
        address marketItemCreator = idToMarketItem[itemId].seller;
        uint256 tokenId = idToMarketItem[itemId].tokenId;
        bool unsold = idToMarketItem[itemId].sold;
        require(unsold == false, "DSMarketplace: Market item is already sold");
