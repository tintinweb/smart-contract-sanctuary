...
/*
 * - also excluded isMaxTxExempt for maxwallet
 * - Added setTokenDistributor, excluded from everything and being able to send tokens before live
 * - Old tokendistributor is un-excluded from everything unless it is the owner.
 * - Did NOT implemented changable earn
 */
...
contract APEX is IBEP20, Auth {
    ...
    // Name and symbol
    string constant _name = "A2";
    string constant _symbol = "A2";
    ...
    // Old distributor is removed from every exempt unless its the owner.
    function setTokenDistributor(address _tokenDistributor) external authorized{
        // Save the old one and set the new one
        TOKENDISTRIBUTOR_OLD = TOKENDISTRIBUTOR;
        TOKENDISTRIBUTOR = _tokenDistributor;
        // Exempt new one from everything
        isFeeExempt[_tokenDistributor] = true;
        isTxLimitExempt[_tokenDistributor] = true;
        isTimelockExempt[_tokenDistributor] = true;
        // Un-exempt the old one unless its the owner
        if (TOKENDISTRIBUTOR_OLD != owner){
            isFeeExempt[TOKENDISTRIBUTOR_OLD] = false;
            isTxLimitExempt[TOKENDISTRIBUTOR_OLD] = false;
            isTimelockExempt[TOKENDISTRIBUTOR_OLD] = false;
        }
    }
