...
contract CharacterNFT is ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable{
    ...
    function initialize() public initializer {
        __ERC721_init("NFT BPLUS Token", "NBT");
        __Ownable_init();
        version=1;
    }
    address public academyUpStar;
    modifier onlyAcademyUpStar {
      require(msg.sender == academyUpStar);
      _;
    }

    address public evolutionUpRank;
    modifier onlyEvolutionUpRank {
      require(msg.sender == academyUpStar);
      _;
    }

    // address public characterMarketPlace;
    // modifier onlyCharacterMarketPlace {
    //   require(msg.sender == characterMarketPlace);
    //   _;
    // }
  ...
  function createCharacter(address owner,uint256 characterType, uint256 rank, uint256  star,uint256  exp) public onlyOwner returns (uint256) {
        _tokenIds.increment();
 ...
 function createRandomThreeCharacter(address owner) public {
  ...
  function upStar(address _owner,uint256 _nftId) public onlyAcademyUpStar {
  ...
  function upRank(address _owner,uint256 _mainNftId, uint256 _materialNftId) public onlyEvolutionUpRank {
    ...
    delete characters[_owner][_materialNftId];
  }

  function upExp(address _owner,uint256 _nftId,uint256 _exp) public onlyAcademyUpStar  {
    Character memory character= getCharacter(_owner,_nftId);
    character.exp=character.exp + _exp;
    characters[_owner][_nftId] = encode(character);
  }
  // function upRank(address owner,uint256 nftId, Characters.Character memory character) public payable returns (bool) {
  //   characters[owner][nftId] = Characters.encode(Characters.upRank(character));
  //   return true;
  // }
...
function getCharacterPublic(address _owner, uint256 _id) public view returns (
  ...
  function updateAddress(address _academyUpStar, address _evolutionUpRank) public onlyOwner{
    academyUpStar=_academyUpStar;
    evolutionUpRank=_evolutionUpRank;
  }

  function tokensOfOwner(address _owner) external view returns(Character[] memory ) {
        uint256 tokenCount = balanceOf(_owner);
        Character[]  memory _characters= new Character[](tokenCount);
        if (tokenCount == 0) {
            // Return an empty array
            return new Character[](0);
        } else {
            uint256 totalChars = _tokenIds.current();
            // We count on the fact that all cats have IDs starting at 1 and increasing
            // sequentially up to the totalCat count.
            uint32 x=0;
            for (uint i = 0; i < totalChars; i++) {
                if (characterIndexToOwner[i+1] == _owner) {
                    _characters[x]=getCharacter(_owner,i+1);
                    x=x+1;
                }
            }
...
function charactersOfOwner(address _owner) external view returns(uint256[] memory ) {
        uint256 tokenCount = balanceOf(_owner);
        uint256[]  memory _characters= new uint256[](tokenCount);
        if (tokenCount == 0) {
  ...
  function random() internal view returns (uint) {
    uint randomnumber = uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty))) % 4;
...
function transfer(uint256 _nftId, address _target)
        external
    {
        ...
        characters[_target][_nftId] = characters[ownerOf(_nftId)][_nftId];
        characters[ownerOf(_nftId)][_nftId]= encode(Character(0,0,0,0,0));
        _transfer(ownerOf(_nftId), _target, _nftId);
        
    }

function approveMarketPlace(address to, uint256 tokenId) external {
