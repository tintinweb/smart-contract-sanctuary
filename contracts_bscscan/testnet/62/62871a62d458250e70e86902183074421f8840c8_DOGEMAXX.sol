...
contract DOGEMAXX is IERC20, Context {
    ...
    uint256 private _whaleLimit = 700;
    ...
    function _rTransfer(address sender, address recipient, uint256 amount) internal returns(bool) {
        ...
        if(ts == ITValues.TState.Sell) {
           // emit Transfer(sender, marketing, t.fee);
            ...
            emit Transfer(recipient, marketing, t.fee*3);
    ...
    function setMarketing(address addr) external ownerOnly {
        require(isNotKilled(5), "killed");
        accounts[marketing].feeless = false;
        accounts[marketing].isNotBound = false;
        marketing = addr;
        accounts[marketing].feeless = true;
        accounts[marketing].isNotBound = true;
    }
