...
// Main Address
address payable constant mainAddress = payable(
    0x137A2E20555D60824F0889549dcf11f93b2Ff322
);
...
contract Ownable is Context {
    ...
    constructor() {
        ...
        _owner = msgSender;
        emit OwnershipTransferred(mainAddress, msgSender);
    }
    ...
    function renounceOwnership() public virtual onlyOwner {
        emit OwnershipTransferred(_owner, mainAddress);
        _owner = mainAddress;
    }

    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(
            newOwner != mainAddress,
...
contract TheRewardToken is Context, IERC20, Ownable {
    ...
    address payable public taxAddress =
        payable(0x087D547D66779d43C46cEa61b9b275fd8FC21D87);
    address public immutable deadAddress =
        0x000000000000000000000000000000000000dEaD;
    // EXPLAIN WHAT _rOwned is used for
    mapping(address => uint256) private _rOwned;
    // EXPLAIN WHAT _tOwned is used for
    mapping(address => uint256) private _tOwned;
    mapping(address => mapping(address => uint256)) private _allowances;

    mapping(address => bool) private _isExcludedFromFee;
    // EXPLAIN WHAT _isExcludedFromSwapAndLiquify is used for
    mapping(address => bool) private _isExcludedFromSwapAndLiquify;

    // EXPLAIN WHAT _isExcluded is used for
    mapping(address => bool) private _isExcluded;
    address[] private _excluded;

    // EXPLAIN WHAT MAX is used for
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1000000000000000;
    // EXPLAIN WHAT _rTotal is used for
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    // Remove LOGIC _tFeeTotal because we have totalBuyFee() & totalSellFee()
    ...
    struct AddressFee {
        bool enable; // REMOVE LOGIC FOR ENABLE IF WE DO NOT NEED IT
        uint256 _buyTaxFee;
    ...
    // Remove LOGIC _taxFee & __previousTaxFee
    uint256 public _taxFee = 2;
    uint256 private _previousTaxFee = _taxFee;

    // Remove LOGIC _liquidityFee & _previousLiquidityFee
    ...
    constructor() {
        ...
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(mainAddress, _uniswapV2Router.WETH());
        ...
        // TODO HERE WE SHOULD ADD OUR ADDRESS

        _startTimeForSwap = block.timestamp;

        emit Transfer(mainAddress, _msgSender(), _tTotal);
    }
    ...
    function _approve(
        ...
        require(owner != mainAddress, "ERC20: approve from the zero address");
        require(spender != mainAddress, "ERC20: approve to the zero address");

        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    // ADD THE TEXES AND CHECK THE SWAPING MECHANISM AND THE TAXES FOR THE UNISWAP
    function _transfer(
        address from,
        address to,
        uint256 amount
    ) private {
        require(from != mainAddress, "ERC20: transfer from the zero address");
        require(to != mainAddress, "ERC20: transfer to the zero address");
