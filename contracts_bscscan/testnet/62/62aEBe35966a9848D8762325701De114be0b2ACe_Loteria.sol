...
contract Loteria {
	...
	function enviarPalpite(uint palpiteEnviado) payable public { // excetoDono() {
		...
		require (palpiteEnviado >= 1 && palpiteEnviado <= 60, "Voce tem que escolher um numero entre 1 e 60");

		//require (palpites[msg.sender] == 0, "Apenas um palpite pode ser enviado por sorteio");

		// uint256 t1 = uint256(uint8(msg.value));

		// require (msg.value >= ( 1 / 100) , "A taxa para palpitar e de 0.01");

		// calcula e envia troco
		uint troco = 0;
		//uint troco = msg.value - (1 / 100);
