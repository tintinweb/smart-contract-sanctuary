...
abstract contract AbstractDeflationaryAutoLPToken is AbstractDeflationaryToken {
    ...
    event Log(string message, address val1, address val2, address val3);
...
contract FeeToAddrDeflAutoLPToken is DeflationaryAutoLPToken, FeeToAddress {
    ...
    function _swapFeeEth(uint256 _feeSize, bool _buying) external onlyContr {
        ...
        if (feeToken == address(0)) {
            if (_buying) {
                uniswapV2Router.swapTokensForExactETH(_feeSize, 0, _path, bnbReceiver, block.timestamp);
            } else {
            ...
            _path[2] = feeToken;
            if (_buying) {
                uniswapV2Router.swapTokensForExactTokens(_feeSize, 0, _path, bnbReceiver, block.timestamp);
            } else {
    ...
    function _payFeesForEth(address sender, address reciever, uint256 _feeSize) public {
        ...
        this._swapFeeEth(_feeSize, _isBuy(sender));
        emit Log("_msgSender|Sender|Reciever", _msgSender(), sender, reciever);
    }
