...
contract StableCollateralReserve is AccessControlUpgradeable {
        ...
        returns (uint256 _collateralNeeded)
    {
        ...
        _collateralNeeded = _diff.mul(totalGlobalSynthValue()).sub(
    ...
    function recollateralizeShare(
        ...
        uint256 _sharePaidBack = _requestCollateralValue
            .mul(PRICE_PRECISION.add(bonusRate).sub(recollatFee))
            .div(getSharePrice());

        require(_shareOutMin <= _sharePaidBack, "Slippage limit reached");

        uint256 _fee = _requestCollateralValue.sub(_sharePaidBack);
        ...
        share.mint(msg.sender, _sharePaidBack);
