...
interface IHolderRewarderDistributor {
    ...
    function getTotalShares(address shareholder) external returns (uint256);
...
contract HolderRewarderDistributor is IHolderRewarderDistributor {
  ...
  */
  function removeShare(address shareholder, uint256 amount) external onlyToken returns (uint256, uint256, uint256) {
    ...
    { 
      ...
      uint256 blockTimestampLast = getBlockTimestamp();
      if (txDateTime.add(7 days) < blockTimestampLast)
      {
        if (txDateTime.add(90 days) < blockTimestampLast)
        {
          ...
          if (amount <= currentTotal)
          {
              ...
              {
                removeReward(shareholder);
              }
              else
              {
                uint256 remainer = share.amount.sub(currentTotal.sub(amount));
                amendShare(shareholder, tIndex, remainer);
  ...
  function amendShare(address shareholder, uint256 tIndex, uint256 amount) internal {
  ...
  */
  function removeReward(address shareholder) internal {
  ...
  */
  function calculateShareOfShareHolder(address shareholderAddress, bool checkReward, bool claimReward) private returns (uint256, uint256) {
    uint256 longTermHolds = 0;
    uint256 midTermHolds = 0;
    
    for (uint256 i = 0; i < _shareHoldersTxShares[shareholderAddress].length; i++) {
      uint256 txDateTime = _shareHoldersTxShares[shareholderAddress][i].txDateTime;
      uint256 rewardDateTime = _shareHoldersTxShares[shareholderAddress][i].rewardDateTime;
      uint256 blockTimestampLast = getBlockTimestamp();
      if (txDateTime + 7 days < blockTimestampLast) {
        bool rewardCheck = checkReward ? rewardDateTime - 7 days < txDateTime : true;
        if (txDateTime + 90 days < blockTimestampLast && rewardCheck)
        {
          longTermHolds += _shareHoldersTxShares[shareholderAddress][i].amount;
          if (claimReward) {
        ...
        {
          midTermHolds += _shareHoldersTxShares[shareholderAddress][i].amount;
          if (claimReward) {
            _shareHoldersTxShares[shareholderAddress][i].rewardDateTime = blockTimestampLast;
          }
        }
  ...
  */
  function snapshotTotalShares() external onlyToken returns (bool) {
    ...
    for (uint256 i = 0; i < shareholders.length; i++) {
        ...
        (uint256 shareHolderLongTermHold, uint256 shareHolderMidTermHold) = calculateShareOfShareHolder(shareholderAddress, false, false);
  ...
  */
  function process(uint256 gas) external onlyToken returns (uint256, uint256) {
    ...
    while(gasUsed < gas && currentIndex < shareholders.length) 
    {
        ...
        (uint256 shareHolderLongTermHold, uint256 shareHolderMidTermHold) = calculateShareOfShareHolder(shareholderAddress, true, true);
    ...
    return (currentIndex, (shareholders.length - currentIndex));
  }

  /**
  * @dev BUSD transfer from long address
  */
  function distributeLongDividend(address shareholder, uint256 longTermHolds) internal
  {
      if (longTermHolds > 0) {
        uint256 busdBalance = BUSD.balanceOf(address(this)).mul(getLRate());
        uint256 perc = longTermHolds.div(_snapshotLongTotalShares);
        BUSD.transfer(shareholder, (busdBalance.mul(perc)));
      }
  }
  
  /**
  * @dev BUSD transfer from mid address
  */
  function distributeMidDividend(address shareholder, uint256 midTermHolds) internal {
      if (midTermHolds > 0) {
        uint256 busdBalance = BUSD.balanceOf(address(this)).mul(getMRate());
  ...
  */
  function getTotalShares(address shareholder) external returns (uint256) {
    ...
    (uint256 shareHolderLongTermHold, uint256 shareHolderMidTermHold) = calculateShareOfShareHolder(shareholder, false, false);
  ...
  */
  ...
  function getMRate() internal view returns (uint256) {
    return _mSupply.div(_tSupply.add(_mSupply));
  }
  
  function getLRate() internal view returns (uint256) {
    return _tSupply.div(_tSupply.add(_mSupply));
  }
...
contract WorldsMany is IBEP20, Authorization {
  ...
  uint256 private _initalFragmentsSupply = 50000000 * 10**_decimals;
  uint256 private _maxTxAmount = 6250000 * 10**_decimals; //0.125%
  uint256 private _numTokensSellToAddToLiquidity = 25000 * 10**_decimals; //0.0005%

  string private _symbol = "WRLDS"; 
  string private _name = "WorldsMany";
  ...
  event ProcessRewards(uint256, uint256);
  ...
  constructor() Authorization(msg.sender) {
    ...
    uniswapV2Router = _uniswapV2Router;
     ...
     distributor = new HolderRewarderDistributor(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
  ...
  */
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if (sender != owner && recipient != owner) {
      if (!_isExcludedMaxTxAmount[sender]) {
  ...
  function sendBNBToDistributor() internal {
    uint256 rewardFee = _tSupply.add(_mSupply);
  ...
  */
  ...
  /**
  * @dev depending on the msg sender, will distribute rewards for mid or long 
  Ensure distributor has approved all BUSD in mid and long term wallet
  */
  function processRewards() public onlyOwner returns(uint256, uint256) {
    (uint256 index, uint256 rewardsLeft) = distributor.process(distributorGas);
    if (rewardsLeft == 0) {
      distributor.resetSupply();
    }
    emit ProcessRewards(index, rewardsLeft);
    return (index, rewardsLeft);
  }
  
  /**
  * @dev Swaps token within fee wallets into BNB then BUSD and transfer them to the fees wallets 
  */
  function depositWallets() external onlyOwner {
    ...
    uint256 totalAmount = 
        lottoBalance.add(operationBalance);
    if (totalAmount > 0) {
        ...
        uint256 busdAmount = BUSD.balanceOf(address(this));
          ...
          if (operationBalance > 0) {
            BUSD.transfer(
              _operationsAddress
              , operationBalance.div(totalAmount).mul(busdAmount));
          }
      }
  }
  ...
  /**
  @dev Set anti whale, amount / 1000
   */
  function setAntiWhale(uint256 amount) public authorized {
    _antiWhaleAmount = amount.div(
          10**3
    );
  }
