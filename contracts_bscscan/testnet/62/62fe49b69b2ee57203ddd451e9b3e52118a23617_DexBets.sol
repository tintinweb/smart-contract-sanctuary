...
abstract contract UpgradedContract {
    ...
    function newbet(uint256[] calldata amounts, uint256[] calldata bet_ids)
        public
        virtual
        returns (uint256[] memory return_bets);
...
contract DexBets is Ownable, Pausable {
    ...
    {
        if (deprecated) {
            UpgradedContract(upgradedAddress).newbet(amounts, bet_ids);
        } else {
            uint256 total_amount = 0;
            uint256[] memory newBets = new uint256[](amounts.length);
            for (uint256 i = 0; i < amounts.length; i++) {
                ...
                emit NewBet(newBetID);
            }
            require(total_amount <= max_amount);
