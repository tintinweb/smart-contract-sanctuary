...
contract Lottery is Ownable {
  using SafeMath for uint256;
    ...
    uint public upperLimit = 500000000000000000000;
    
    uint firstPrizes = 1;
    uint secondPrizes = 1;
    uint thirdPrizes = 1;
    uint fourthPrizes = 50;
    uint nonce=1;

    bool public IsAutomaticPickWay = true;
    struct rewardInfo {
        address addr;
        uint prizes;
    }
    ...
    function setPickWay(bool _flag) public onlyOwner {
        IsAutomaticPickWay=_flag;
    }
    function participate() public {
        ...
        if(IsAutomaticPickWay && roundAccumulatedAmount[round]>=upperLimit){
             pickWinnerByAutomatic();
        }
        emit Participate(msg.sender, perEggAmount,round);
    }

    function winnerNumber() private returns(uint) {
        return uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, nonce++,round)));
    }

    function pickWinnerByManual() public onlyOwner {
        require(roundAccumulatedAmount[round]>=upperLimit,"Participation has not reached the limit");
        require(IsAutomaticPickWay == false,"it is automatic pick ways");
        _openFirstPrize();
        _openSecondPrize();
        _openThirdPrize();
        _openFourthPrize();
    ...
    function pickWinnerByAutomatic() private {
        _openFirstPrize();
        _openSecondPrize();
        _openThirdPrize();
        _openFourthPrize();
