...
contract MoniwarMinningNFT is Ownable, ReentrancyGuard {
    ...
    function upgrade(uint256 _pid, uint256 uid, uint256 number) public nonReentrant {
        ...
        if(number == 2){
            amountUpgarade = amountCalHashrate.mul(number).mul(95).div(10000);
        } else if(number == 5){
            amountUpgarade = amountCalHashrate.mul(number).mul(9).div(1000);
        } else {
            amountUpgarade = amountCalHashrate.mul(number).mul(75).div(10000);
        }
        ...
        uint256 _amountDeposit = list.amountDeposit.add(amountUpgarade).sub(amoutsendwithfee);
        list.amountDeposit = _amountDeposit;
        ...
        user.amount = user.amount.add(_amountDeposit);
