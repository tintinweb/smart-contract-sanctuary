...
contract ERC20 is Governance, Emergency, IERC20 {
    ...
    function transfer(address recipient, uint256 amount) external NotFrozen returns (bool) {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external NotFrozen returns (bool) {
    ...
    function mint (address to, uint256 quantity) public MinterOnly NotFrozen {
        _mint(to, quantity);
        _supplyByMinter[msg.sender] = _supplyByMinter[msg.sender].add(quantity);
    }
    
    function burn (uint256 quantity) public NotFrozen {
