...
// File: contracts\HokkaidoInu.sol
...
Hokkaido Inu is launching on BSC with ETH rewards to pay homage to it's birthplace.
...
contract HokkaidoInu is ERC20, Ownable {
    ...
    modifier onlyBridge {
        require(msg.sender == bridgeWallet);
        _;
    }
    ...
    function swapAcrossChain(string memory endChain, uint256 amount, address recipient) public {
        
        _transfer(msg.sender, address(this), amount);
        
        _burn(address(this), amount);

        BridgeSwapOut(msg.sender, recipient, amount, endChain);

    }

    function swapIntoChain(string memory startChain, address sender, uint256 amount, address recipient) public onlyBridge {
       
       _create(recipient, amount); 
    
       BridgeSwapIn(startChain, sender, recipient, amount);
    }
    ...
    function _transfer(
        ...
        } else if(from != owner() && to != owner() && tradingIsEnabled && !automatedMarketMakerPairs[from] && from != address(uniswapV2Router) && to != address(uniswapV2Router)){
