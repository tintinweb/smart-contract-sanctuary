...
contract ANMPump is Context, IERC20, Ownable {
    ...
    uint256 public _maxBuyTrancactionAmount = 1000000000 * 10**9;
    uint256 public _maxSellTrancactionAmount = 1000000000 * 10**9;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner() && to != uniswapV2Pair) {
            require(amount <= _maxBuyTrancactionAmount, "Transfer amount exceeds the _maxBuyTrancactionAmount.");
        }
        ...
        if (overMinimumTokenBalance && !inSwapAndLiquify && from != uniswapV2Pair && swapAndLiquifyEnabled &&
            from != owner() && to != owner())
            require(amount <= _maxSellTrancactionAmount, "Transfer amount exceeds the _maxSellTrancactionAmount.");
        {
    ...
    function setMaxBuyTxAmount(uint256 maxBuyTxAmount) external onlyOwner() {
        _maxBuyTrancactionAmount = maxBuyTxAmount;
    }
    
    function setMaxSellTxAmount(uint256 maxSellTxAmount) external onlyOwner() {
        _maxSellTrancactionAmount = maxSellTxAmount;
    }
    ...
    function prepareForPreSale() external onlyOwner {
        ...
        _maxBuyTrancactionAmount = 1000000000 * 10**9;
        _maxSellTrancactionAmount = 1000000000 * 10**9;
    }
    ...
    function prepareForLaunch() external onlyOwner {
        ...
        _totalTaxPercent = _liquidityFee.add(_marketingFee).add(_ANMbuyBackFee);
        _prevTotalTaxPercent = _totalTaxPercent;
        _maxBuyTrancactionAmount = 100000000 * 10**9;
        _maxSellTrancactionAmount = 10000000 * 10**9;
    }
