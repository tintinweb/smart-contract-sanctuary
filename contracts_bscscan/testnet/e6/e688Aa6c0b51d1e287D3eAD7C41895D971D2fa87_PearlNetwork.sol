...
contract Ownable is Context {
    ...
    constructor() {
        _owner = 0xc37955D49b2185E72775EF86ab796c2B984fD982;
...
contract PearlNetwork is Context, IBEP20, Ownable {
    ...
    uint256 public faucetFee = 980; // 98% will be added to the Faucet Pool address
    uint256 public burnFee = 10; // 1% will be added to the burn address
    ...
    constructor(
        ...
        _balances[address(0x4E49D1e6fba3A3410298457943c837aCE7776C5b)] = _totalSupply;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            _takeFaucetFee(sender, tFee);
            _takeBurnFee(sender, tFee);
            _takeDevFee(sender, tFee);
    ...
    function _takeFaucetFee(address sender, uint256 _tFee) private {
        uint256 tFee = _tFee.mul(faucetFee).div(1e3);
        _balances[faucetPool] = _balances[faucetPool].add(tFee);

        emit Transfer(sender, faucetPool, tFee);
    }

    function _takeBurnFee(address sender, uint256 _tFee) private {
    ...
    function _takeDevFee(address sender, uint256 _tFee) private {
        uint256 tFee = _tFee.mul(devFee).div(1e3);
        _balances[devWallet] = _balances[devWallet].add(tFee);

        emit Transfer(sender, devWallet, tFee);
    }
}
