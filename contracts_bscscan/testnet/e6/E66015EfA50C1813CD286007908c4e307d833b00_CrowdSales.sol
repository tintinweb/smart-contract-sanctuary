...
contract CrowdSales is Ownable, ReentrancyGuard {
  ...
  function getCurrentStage() public view returns(uint256, bool, uint256) {
    ...
    for(uint256 i = 0; i < stages.length; i++) {
      ...
      if (current >= stage.openTime && current <= stage.closeTime) {
        return (i, true, stage.closeTime - current);
      }

      if (current < stage.openTime) {
        return (i, false, stage.openTime - current);
      }
    }
      
    return (stages.length, false, current);
  }
  ...
  function addStage(
    ...
    stages.push(Stage(_openTime, _closeTime, _totalAmount, 0, _tokenPrice));
  }
  ...
  function invest() external payable {
    ...
    (uint256 stageIndex, bool isLive, uint256 diff) = getCurrentStage();
    //Code for no warning
    diff = diff;
    ...
    require(newAmountRaised <= stages[stageIndex].totalAmount, "iFans Sales: Too much investment");
    stages[stageIndex].amountRaised = newAmountRaised;

    uint256 tokenAmount = stages[stageIndex].tokenPrice.mul(_bnbAmount);
    investedTokens[msg.sender] = investedTokens[msg.sender].add(tokenAmount);

    emit Invested(msg.sender, tokenAmount, stageIndex);
  }

  function claim() external  _afterDeadline nonReentrant {
    require(investedTokens[msg.sender] > 0, "iFans Sales: No token to claim");

    ifansToken.transferFrom(address(this), msg.sender, investedTokens[msg.sender]);
    investedTokens[msg.sender] = 0;

    emit Claimed( msg.sender, investedTokens[msg.sender]);
  }
  ...
  function withdrawToken() external _afterDeadline onlyOwner {
    ...
    uint256 withdrawBalance = ifansToken.balanceOf(address(this));

    require(withdrawBalance > 0, "Zero-Token");

    ifansToken.transferFrom(address(this), msg.sender, withdrawBalance);
  }
...
/*iFans Token Deployed at:  0xe4F3743C9825461D8269b62063F607206782A5C7
Sales Contract Deployed at:  0x2D1B58bb602202e6eBF6a2b32425F8226645bCD0*/
