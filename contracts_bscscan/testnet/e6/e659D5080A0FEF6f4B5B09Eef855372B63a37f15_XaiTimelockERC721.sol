...
contract XaiTimelockERC721 {
    ...
    function deposit(address owner, address minter, uint256 itemID) public returns (uint256) {
        ERC721 _minter = ERC721(minter);
        // check via minter if msg.sender has itemID
        require(_minter.ownerOf(itemID) == owner, "Sender is not the owner of itemID");
        // deposit into contract
        // _minter.transferFrom(msg.sender, address(this), itemID);
        // set Deposited Item data in array
        depositedTime[owner] = block.timestamp;
        depositedItemID[owner] = itemID;
        return depositedTime[owner];
    }
    
    function withdraw(address owner) public view returns (uint256) {
        // if currentTime >= withdraw time (deposited time + 7 days)
        require(depositedTime[owner] != 0, "No deposit to withdrawn");
        require(getCurrentTime() >= getWithdrawTime(owner), "Timelock not complete");
        return getWithdrawTime(owner);
    }
    
    function getWithdrawTime(address owner) public view returns (uint256) {
        require(depositedTime[owner] != 0, "No deposit to withdrawn");
        return SafeMath.add(depositedTime[owner], 604800);
    }
