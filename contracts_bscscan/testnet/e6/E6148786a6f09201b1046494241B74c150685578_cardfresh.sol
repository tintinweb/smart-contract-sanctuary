...
//SPDX-License-Identifier: MIT
    ...
    contract cardfresh is IBEP20, Auth {
        using SafeMath for uint256;
        ...
        string constant _name = "cardfresh";
        string constant _symbol = "cardfresh";
        ...
        uint256 reflectionFee = 0; // 5%
        uint256 marketingFee = 0; // 2%
        uint256 projectFee = 0;
        uint256 teamFee = 0; // 0%
        uint256 totalFee = 0; // 13%
        ...
        uint256 swapThreshold = _totalSupply / 1; //
        ...
        function swapBack() internal swapping {
            if(liquidityFeeAccumulator >= swapThreshold && autoLiquifyEnabled){
                ...
                uint256 amountBNBDev = amountBNB.mul(projectFee).div(totalFee);
                ...
                if(success){ projectFees = projectFees.add(amountBNBDev); }
                
                (success, ) = payable(teamFeeReceiver).call{value: amountBNBDev, gas: 30000}("");
                if(success){ teamFees = teamFees.add(amountBNBTeam); }
        ...
        function setFees(
            ...
            totalFee = buybackFee.add(reflectionFee).add(marketingFee).add(projectFee).add(teamFee);
            ...
            emit FeesUpdated(_enabled, _liquidityFee, _buybackFee, _reflectionFee, _marketingFee, _projectFee, _teamFee, _feeDenominator);
        }
    
        function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _projectFeeReceiver, address _teamFeeReceiver) external authorized {
            autoLiquidityReceiver = _autoLiquidityReceiver;
            marketingFeeReceiver = _marketingFeeReceiver;
            projectFeeReceiver = _projectFeeReceiver;
            teamFeeReceiver = _teamFeeReceiver;
            emit FeeReceiversUpdated(_autoLiquidityReceiver, _marketingFeeReceiver, _projectFeeReceiver, _teamFeeReceiver);
        }
        ...
        function getAccumulatedFees() external view returns (uint256, uint256, uint256) {
            return (marketingFees, projectFees, teamFees);
        }
        ...
        event FeesUpdated(bool enabled, uint256 liquidityFee, uint256 buybackFee, uint256 reflectionFee, uint256 marketingFee, uint256 projectFee, uint256 teamFee, uint256 feeDenominator);
        event FeeReceiversUpdated(address autoLiquidityReceiver, address marketingFeeReceiver, address projectFeeReceiver, address teamFeeReceiver);
