...
contract TxFeeWallet {
    ...
    IJaxAdmin public admin;

    IPancakeRouter01 pancakeRouter;

    modifier onlyAdmin() {
        require(admin.userIsAdmin(msg.sender), "Only Admin can perform this operation.");
        _;
    }

    modifier onlyGovernor() {
        require(admin.userIsGovernor(msg.sender), "Only Governor can perform this operation.");
        _;
    }

    constructor(address _admin_address, address _pancakeRouter, address _rewardToken) {
        admin = IJaxAdmin(_admin_address);
    ...
    function set_yield_tokens(address[] calldata newYieldTokens) public {
        delete yieldTokens;
    ...
    function swap_tokens() public {
        uint deadline = block.timestamp * 1000 + 1000 * 60 * 10;
