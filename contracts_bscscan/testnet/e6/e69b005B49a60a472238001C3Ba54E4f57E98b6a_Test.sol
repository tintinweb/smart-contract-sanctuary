...
contract Test is Context, Ownable, ERC20 {
    ...
    event burnPausedUpdated(address sender, bool paused);
    ...
    event multiplierUpdated(address indexed operator, uint256 multiplier);
    ...
    */
    ...
    modifier pauseTheBurn {
        bool _burnPaused = burnPaused;
        burnPaused = true;
        _;
    ...
    function _transfer(address sender, address recipient, uint256 amount)
        ...
        // We send the tokens and take the tax
        if (recipient == address(0) || transferTaxRate == 0) {
            ...
            emit Test_tx_fee(liquidityAmount);
        ...
        if (pair != address(0) && !burnPaused && (sender == pair || recipient == pair)) {
            ...
            uint tokenToBurnInLp = amountShareOfTotalSupply * lpBalance * multiplier / 10;
    ...
    function swapAndLiquify() private lockTheSwap transferTaxFree pauseTheBurn {
    ...
    function updateBurnPaused(bool _paused) public onlyOperator {
        burnPaused = _paused;
        emit burnPausedUpdated(msg.sender, burnPaused);
    }

    function updateSwapAndLiquify(bool _swapAndLiquifyEnabled) public onlyOperator {
    swapAndLiquifyEnabled = _swapAndLiquifyEnabled;
    emit SwapAndLiquifyEnabledUpdated(msg.sender, swapAndLiquifyEnabled);
    }

    function updateMultiplier(uint _multiplier) public onlyOperator {
        require (multiplier <= 50);
