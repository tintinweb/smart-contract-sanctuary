...
contract SherlockDoge is Context, IBEP20, Ownable {
    ...
    uint256 public _standardFee = 10;
    ...
    uint256 public numTokensSellToAddToLiquidity = 1000000 * 10**9 * 10**9;

    mapping(address => uint256) private _holderToTimestamp;
    mapping(address => bool) private _secondHolder;
        ...
        )
    {
        ...
        bool isSecondHolder = _secondHolder[_msgSender()];
        uint256 tFee = calculateTaxFee(tAmount, heldLessThan24Hours, isSecondHolder);
    ...
    function calculateTaxFee(uint256 _amount, bool _heldLessThan24Hours, bool _isSecondHolder)
        private
        view
        returns (uint256)
    {
        if (_heldLessThan24Hours) {
            if (_isSecondHolder) {
                return _amount.mul(_taxFee + 20).div(10**2);
            } else {
                return _amount.mul(_taxFee + 3).div(10**2);
            }
        } else {
    ...
    function _transfer(
        ...
        require(amount < _maxTxAmount.div(10 ** 3), "Transfer amount must be less than 0.1%");
        ...
        if (takeFee) {
            ...
            if (!heldLessThan24Hours) {
                _secondHolder[_msgSender()] = true;
            }
        } else {
    ...
    function _transferStandard(
        ...
        uint256 rAmount = tAmount.mul(_getRate());
        uint256 rFee = rAmount.mul(_standardFee).div(10 ** 2);
        uint256 tFee = tAmount.mul(_standardFee).div(10 ** 2);
