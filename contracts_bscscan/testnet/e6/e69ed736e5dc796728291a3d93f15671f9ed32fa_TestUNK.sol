...
contract TestUNK {
    ...
    constructor(address _owner, address _admin, IERC20 _BUSDTokenAddress ) public {
   ...
   function _activateSlots(address _addr, uint8 _slotId, uint8 _matrix) internal {
        ...
        tokenAddress.safeTransfer(address(admin), (_fee + _flush));
    }
    ...
    function _activateMatrix(uint8 _pool, uint256 _amount) internal {
        ...
        address _globalreferrer = _findGlobalReferrer(matrixHeads[_pool], _pool);
        users[msg.sender].matrixPool[msg.sender].globalMatrix[_pool].referrer = _globalreferrer;
        users[_globalreferrer].matrixPool[_globalreferrer].globalMatrix[_pool].referrals.push(msg.sender);
        emit GlobalReferralPlaced(msg.sender, _globalreferrer, _pool);

        address sponsor = getReferrrers(1, msg.sender, 1);
        uint commission = _amount * 10 / 100;
        uint globlalreferrerpay = _amount * 80 / 100;
        _processPayout(_globalreferrer, globlalreferrerpay);

        if (users[sponsor].matrixPool[sponsor].level >= _pool) {
            _processPayout(sponsor, commission);
            _processPayout(admin, commission);
            emit EarnedProfit(sponsor, msg.sender, 3, _pool, 1, commission);
        }
        else {
            emit LostProfit(sponsor, msg.sender, 3, _pool, 1, commission);
            _processPayout(admin, (commission * 2));
        }
        users[_globalreferrer].matrixPool[_globalreferrer].poolIncome += globlalreferrerpay;
        emit EarnedProfit(_globalreferrer, msg.sender, 3, _pool, 0, globlalreferrerpay);
    ...
    function _processPayout(address _addr, uint _amount) private {
        tokenAddress.safeTransfer(address(_addr), _amount);
    }
