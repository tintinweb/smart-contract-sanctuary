        ...
        if (pnl.reward > 0) {
            ...
            for(uint i = 0; i < fund.benefitciaries.length; i++) {
                if (fund.benefitciaries[i] != address(0)) {
                    ...
                    emit AwardBenefitciary(fund.benefitciaries[i], address(asset), pnl.reward * (shareScheme & 0xffff) / total, (shareScheme & 0xffff), total);
                }
                shareScheme = shareScheme >> 16;
            }
        }
...
interface IDABotFundManagerModuleEvent {
    ...
    event AwardBenefitciary(address indexed benefitciary, address indexed asset, uint amount, uint share, uint totalShare);
