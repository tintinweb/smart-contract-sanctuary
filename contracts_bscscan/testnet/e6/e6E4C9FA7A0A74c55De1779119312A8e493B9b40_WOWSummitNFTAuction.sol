...
contract WOWSummitNFTAuction {
    ...
    uint256 limitTopBidCount = 5; // limited top bid data count
    ...
    struct Auction {
        ...
        mapping(address => uint256[]) transactions; // mapping of all transactions with amount for user
        Transaction[] topBids; // mapping 5 bid rate with address, amount
    }
    ...
    function bid(uint256 _auctionId, uint256 _amt) public {
        ...
        uint256 recentBidAmount =  auction.transactions[msg.sender].length > 0 ? auction.transactions[msg.sender][auction.transactions[msg.sender].length - 1] : 0;
        if(recentBidAmount > 0){
            auction.auctionToken.transfer(msg.sender, recentBidAmount);
        }
        ...
        auction.transactions[msg.sender].push(_amt);
        ...
        // add top bid history
        uint256 focus = 0;
        uint256 len = limitTopBidCount;
        Transaction memory topBid = Transaction(msg.sender, _amt);
        if (auction.totalBidders > limitTopBidCount){
            for (focus = 0; focus < len -1; focus++){
                auction.topBids[focus] = auction.topBids[focus + 1];
            }
            auction.topBids[len-1] = topBid;
        }else{
            auction.topBids.push(topBid);
        }
    ...
    function claim(uint256 _auctionId) public {
        ...
        uint256 recentBidAmount =  auction.transactions[msg.sender].length > 0 ? auction.transactions[msg.sender][auction.transactions[msg.sender].length - 1] : 0;
        require(recentBidAmount > 0, "auction: invalid user");
    ...
    /**
     * Returns the transactions provided by user
     * @param _auctionId id of the auction
     * @param _user address of the user
     */
    function getTransactions(uint256 _auctionId, address _user)
        public
        view
        returns (uint256[] memory)
    {
        Auction storage auction = auctions[_auctionId];
        return auction.transactions[_user];
    }
    /**
    ...
    {
        Auction storage auction = auctions[_auctionId];
        uint256 recentBidAmount =  auction.transactions[_user].length > 0 ? auction.transactions[_user][auction.transactions[_user].length - 1] : 0;
        return recentBidAmount;
    }
    /**
     * Returns the bid amount provided by user
     * @param _auctionId id of the auction
     */
    function getTopBids(uint256 _auctionId)
        public
        view
        returns  (Transaction[] memory topbids) 
    {
        Auction storage auction = auctions[_auctionId];
