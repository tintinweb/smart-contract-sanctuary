...
contract RewardsBunny is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 public _liquidityFeeTransfer = 800; // 3% of every buy / sell is kept for liquidity, 5% for BNB reward pool
    uint256 public _marketingFeeTransfer = 800; // 8% of every buy / sell is sent to marketing wallet
    uint256 public _percentageOfLiquidityForBnbReward = 0;
    uint256 public _percentageOfLiquidityForMarketing = 99;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // split contract balance into halves
           ...
           uint256 nohalf = contractTokenBalance;
        ...
        */
        ...
        // swap tokens for BNB
        if (_percentageOfLiquidityForBnbReward < 100) {
           swapTokensForBnb(half);
        } else {
           swapTokensForBnb(nohalf);
        }
        ...
        uint256 bnbForMarketingWallet = newBalance * _percentageOfLiquidityForMarketing / 100; 
        uint256 bnbForLiquidity = newBalance - (bnbForMarketingWallet + bnbForRewardPool);
        
        sendMarketing (bnbForMarketingWallet);
    ...
    function sendMarketing(uint256 tokenAmount) private {
    // approve token transfer to cover all possible scenarios
    _approve(address(this), address(_uniswapV2Router), tokenAmount);
