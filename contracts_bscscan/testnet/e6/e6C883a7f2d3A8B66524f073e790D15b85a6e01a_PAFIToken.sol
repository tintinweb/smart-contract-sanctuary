 ...
 * 
  ...
  50% Burn of Total Supply, 5,000,000,000
  1% of every trade goes to existing holders
  2% of every trade is burned
  2% of every trade is auto-locked in liquidity
  1% of every trade goes to marketing
  Burning on every trade makes PAFI TRULY HYPER-Deflationary - to a final total supply of 5,000,000,000 PAFI
  
  Website: https://paficoin.com
  Twitter: https://twitter.com/Paficoin
  Telegram Global: https://t.me/Paficoin
  Telegram Announcements: https://t.me/PaficoinNews
  Reddit: https://www.reddit.com/r/Paficoin
  Github: https://github.com/Paficoin  
  Gitbook Document: https://docs.paficoin.com
  Medium: https://paficoin.medium.com
*/
...
contract PAFICOIN is Context, IBEP20, Ownable {
    ...
    address private _walletAddressMarketing = 0x461792257e2aF0a8dBdE7D994B6f3361cc810aC7;
    address private _burnWalletAddress = 0x000000000000000000000000000000000000dEaD;
   
    uint8 private _decimals = 18; // 0.000000000000000000
    uint256 private _zeroes = 10**_decimals;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 10 * 10**9 * _zeroes;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Paficoin";
    string private _symbol = "PAFI";
    ...
    uint256 public _maxTxAmount = 100000000 * _zeroes;
    uint256 private numTokensSellToAddToLiquidity = 100000 * _zeroes;
    uint256 private numTokensBurn = 9999500000 * _zeroes;
  ...
  function setMarketingWalletAddress(address marketingWalletAddress) external onlyOwner {
         _walletAddressMarketing = marketingWalletAddress;
    }
    ...
    function _takeMarketing(uint256 tMarketing) private {
        uint256 currentRate =  _getRate();
        uint256 rMarketing = tMarketing.mul(currentRate);
        _rOwned[_walletAddressMarketing] = _rOwned[_walletAddressMarketing].add(rMarketing);
        if(_isExcluded[_walletAddressMarketing])
            _tOwned[_walletAddressMarketing] = _tOwned[_walletAddressMarketing].add(tMarketing);
    }
  ...
  function calculateBurnFee(uint256 _amount) private view returns (uint256) {
    ...
    function _setRouterAddress(address router) private {
        IPancakeSwapV2Router02 _newPancakeRouter = IPancakeSwapV2Router02(router);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        (uint256 tokenAmountSent, uint256 ethAmountSent, uint256 liquidity) = pancakeSwapV2Router
         ...
         * of the contract. The contract does not contain a method to withdraw these funds,
         * and the BNB will be locked in the contract forever.
         */
    ...
    /**
     * @dev The owner can withdraw BNB(BNB) collected in the contract from `swapAndLiquify`
     * or if someone (accidentally) sends BNB/BNB directly to the contract.
     *
     */
...
contract PAFIToken is PAFICOIN {
    constructor() PAFICOIN(Env.Testnet) {
        // pre-approve the initial liquidity supply (to safe a bit of time)
        _approve(owner(), address(pancakeSwapV2Router), ~uint256(0));
    }
}