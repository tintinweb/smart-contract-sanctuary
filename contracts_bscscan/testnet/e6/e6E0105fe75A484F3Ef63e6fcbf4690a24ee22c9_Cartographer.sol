...
contract Cartographer is Ownable, Initializable, ReentrancyGuard {
    ...
    uint256 public devSummitPerSecond;
    uint256 public referralsSummitPerSecond;
    uint256 private constant sInH = 3600;
    address public devAdd;
    address public expedAdd;
    
    ICakeMasterChef passthroughMasterChef;
    IBEP20 cakeToken;
    ...
    event Elevate(address indexed user, uint8 indexed currentPid, uint8 indexed newPid, uint8 totem, uint256 amount);
    ...
    constructor(
        ...
        devAdd = _devAdd;
        expedAdd = _expedAdd;
        passthroughMasterChef = ICakeMasterChef(_cakeMasterChef);
    ...
    external initializer onlyOwner {
        ...
        setTotalSummitPerSecond(1e18);
    }


    function enable(SummitToken _trueSummit) external onlyOwner {
        require(!isTrueSummit, "Already enabled");
        summit = _trueSummit;
        isTrueSummit = true;
        summitGenesisTime = block.timestamp;
        elevationHelper.enable(summitGenesisTime);
        summitReferrals.enable(_trueSummit);
        cartographerOasis.enable(summitGenesisTime, address(_trueSummit));
        cartographerExpedition.enable(summitGenesisTime, address(_trueSummit));
    }
    ...
    function setTotalSummitPerSecond(uint256 _amount) public onlyOwner {
        summitPerSecond = _amount.mul(900).div(1000);
        devSummitPerSecond = _amount.mul(80).div(1000);
        referralsSummitPerSecond = _amount.mul(20).div(1000);
    }
    ...
    modifier correctKeyword(string memory keyword) {
        require(
            (elevationHelper.getKeywordRoundTimeInFailsafe()) ||
            (summitKeywords.checkKeyword(keyword, getKeywordRound())),
            "Bad keyword"
        );
        _;
    }
    ...
    function createTokenSharedAlloc(IBEP20 _lpToken, uint256 _allocPoint) public onlyOwner nonDuplicatedTokenAlloc(_lpToken) {
        ...
        _lpToken.approve(address(passthroughMasterChef), 1e50);
    ...
    function add(IBEP20 _lpToken, bool _live, uint16 _depositFeeBP, bool _withUpdate, uint8 _elevation, uint256 _passthroughPid) public onlyOwner tokenAllocExists(_lpToken) oasisOrElevation(_elevation) nonDuplicated(_lpToken, _elevation) {
        ...
        subCartographer(_elevation).add(pid, _elevation, _live, _lpToken, _depositFeeBP, _withUpdate, _passthroughPid);
    ...
    function addExpedition(IBEP20 _rewardToken, uint256 _rewardAmount, uint256 _rounds, bool _withUpdate) public onlyOwner nonDuplicated(_rewardToken, EXPEDITION) {
        ...
        cartographerExpedition.addExpedition(pid, _rewardToken, _rewardAmount, _rounds, _withUpdate);
    ...
    function set(uint8 _pid, bool _live, uint16 _depositFee, bool _withUpdate) public onlyOwner oasisOrElevation(poolElevation[_pid]) poolExists(_pid) {
        ...
        subCartographer(poolElevation[_pid]).set(_pid, _live, _depositFee, _withUpdate);
    ...
    function rewards(uint8 _pid, address _userAdd) public view poolExists(_pid) validElevation(_pid) returns (uint256, uint256, uint256, uint256) {
        return subCartographer(poolElevation[_pid]).rewards(_pid, _userAdd);
    }
    
    function hypotheticalRewards(uint8 _pid, address _userAdd) public view poolExists(_pid) validElevation(_pid) returns (uint256, uint256) {
    ...
    function rollover(uint8 _elevation, string memory _keyword) public elevationOrExpedition(_elevation) correctKeyword(_keyword) {
    ...
    function elevModifiedAllocPoint(uint8 _pid) public view returns (uint256) {
        if (!isLive(_pid)) { return 0; }
        return elevationHelper.getElevationModifiedAllocPoint(tokenBaseAllocPoint[lpToken(_pid)], poolElevation[_pid]);
    }
    function tokenElevationEmissionMultiplier(IBEP20 _lpToken, uint8 _elevation) public view returns (uint256) {
        uint256 totalLpTokenSupply = lpSupply(tokenElevationPid[_lpToken][OASIS]).mul(elevModifiedAllocPoint(tokenElevationPid[_lpToken][OASIS]))
            .add(lpSupply(tokenElevationPid[_lpToken][TWOTHOUSAND]).mul(elevModifiedAllocPoint(tokenElevationPid[_lpToken][TWOTHOUSAND])))
            .add(lpSupply(tokenElevationPid[_lpToken][FIVETHOUSAND]).mul(elevModifiedAllocPoint(tokenElevationPid[_lpToken][FIVETHOUSAND])))
            .add(lpSupply(tokenElevationPid[_lpToken][TENTHOUSAND]).mul(elevModifiedAllocPoint(tokenElevationPid[_lpToken][TENTHOUSAND])));
        if (totalLpTokenSupply == 0) { return 0; }
        return lpSupply(tokenElevationPid[_lpToken][_elevation])
            .mul(1e12).mul(elevModifiedAllocPoint(tokenElevationPid[_lpToken][_elevation]))
            .div(totalLpTokenSupply);
    }

    // Fraction of the total emissions for this token at elevation (pool)
    function poolEmissionMultiplier(uint256 _lastRewardTimestamp, IBEP20 _lpToken, uint8 _elevation) internal view returns (uint256) {
        if (totalAllocPoint == 0) { return 0; }
        return getMultiplier(_lastRewardTimestamp, block.timestamp).mul(1e12)
            .mul(tokenSharedAllocPoint[_lpToken]).div(totalAllocPoint)
            .mul(tokenElevationEmissionMultiplier(_lpToken, _elevation)).div(1e12);
    }

    // Purely the yield generation fraction of the pool's emissions
    function poolSummitEmission(uint256 lastRewardTimestamp, IBEP20 _lpToken, uint8 _elevation) external view onlySubCartographer() returns (uint256) {
        if (lastRewardTimestamp == block.timestamp) { return 0; }
        return poolEmissionMultiplier(lastRewardTimestamp, _lpToken, _elevation)
            .mul(summitPerSecond).div(1e12);
    }

    // Mints the total emission of pool, split respectively
    function mintPoolSummit(uint256 lastRewardTimestamp, IBEP20 _lpToken, uint8 _elevation) external onlySubCartographer() returns (uint256) {
        uint256 emissionMultiplier = poolEmissionMultiplier(lastRewardTimestamp, _lpToken, _elevation);
        summit.mint(devAdd, emissionMultiplier.mul(devSummitPerSecond).div(1e12));
        summit.mint(address(summitReferrals), emissionMultiplier.mul(referralsSummitPerSecond).div(1e12));
        summit.mint(address(this), emissionMultiplier.mul(summitPerSecond).div(1e12));
    ...
    function switchTotem(uint8 _elevation, uint8 _newTotem) public nonReentrant elevationOrExpedition(_elevation) {
        ...
        subCartographer(_elevation).switchTotem(_elevation, _newTotem, msg.sender);
    ...
    function deposit(uint8 _pid, uint256 _amount, uint8 _totem) public nonReentrant poolExists(_pid) {
        ...
        uint256 trueDepositAmount = subCartographer(elevation).deposit(_pid, _amount, _totem, msg.sender);
        stakedSummitRandAccum += (_amount + trueDepositAmount);
    ...
    function withdraw(uint8 _pid, uint256 _amount) public nonReentrant poolExists(_pid) {
        subCartographer(poolElevation[_pid]).withdraw(_pid, _amount, msg.sender);
        emit Withdraw(msg.sender, _pid, _amount);
    }

    function emergencyWithdraw(uint8 _pid) public nonReentrant poolExists(_pid) {
        uint256 amount = subCartographer(poolElevation[_pid]).emergencyWithdraw(_pid, msg.sender);
        emit EmergencyWithdraw(msg.sender, _pid, amount);
    }

    function elevate(uint8 _currentPid, uint8 _newPid, uint256 _amount, uint8 _totem) public nonReentrant poolExists(_currentPid) poolExists(_newPid) {
        ...
        (address _lpToken, uint256 _transferAmount, uint256 _passthroughPid) = subCartographer(currentElev).elevateWithdraw(_currentPid, _amount, msg.sender);

        subCartographer(newElev).validateElevate(_newPid, _lpToken, _passthroughPid, _totem, msg.sender);

        _transferAmount = subCartographer(newElev).elevateDeposit(_newPid, _transferAmount, _totem, msg.sender);

        emit Elevate(msg.sender, _currentPid, _newPid, _totem, _transferAmount);          
    }
    ...
    function enableTokenAtElevation(IBEP20 _lpToken, uint8 _elevation) external onlySubCartographer() {
        uint256 modAllocPoint = elevationHelper.getElevationModifiedAllocPoint(tokenBaseAllocPoint[_lpToken], _elevation);
        tokenSharedAllocPoint[_lpToken] = tokenSharedAllocPoint[_lpToken].add(modAllocPoint);
        totalAllocPoint = totalAllocPoint.add(modAllocPoint);
        tokenElevationLive[_lpToken][_elevation] = true;
    }
    function disableTokenAtElevation(IBEP20 _lpToken, uint8 _elevation) external onlySubCartographer() {
        uint256 modAllocPoint = elevationHelper.getElevationModifiedAllocPoint(tokenBaseAllocPoint[_lpToken], _elevation);
        tokenSharedAllocPoint[_lpToken] = tokenSharedAllocPoint[_lpToken].sub(modAllocPoint);
        totalAllocPoint = totalAllocPoint.sub(modAllocPoint);
        tokenElevationLive[_lpToken][_elevation] = false;
    }
    ...
    function takeDepositFee(IBEP20 _lpToken, uint256 _depositFeeBP, uint256 amount) internal returns (uint256) {
        if (_depositFeeBP > 0) {
            uint256 _depositFee = amount.mul(_depositFeeBP).div(10000);
            uint256 _depositFeeHalf = _depositFee.div(2);
            _lpToken.safeTransfer(expedAdd, _depositFeeHalf);
            _lpToken.safeTransfer(devAdd, _depositFeeHalf);
            return amount.sub(_depositFee);
        } 
        return amount;
    }

    function depositTokenManagement(address userAdd, IBEP20 _lpToken, uint256 depositFeeBP, uint256 _amount, uint256 _passthroughPid) external onlySubCartographer() returns (uint256) {
        _lpToken.safeTransferFrom(userAdd, address(this), _amount);
        uint256 amountAfterFee = takeDepositFee(_lpToken, depositFeeBP, _amount);
        if (_passthroughPid != 0) {
            passthroughMasterChef.deposit(_passthroughPid, amountAfterFee);
            transferPassthroughRewards();
        }
        return amountAfterFee;
    }
    function withdrawalTokenManagement(address userAdd, IBEP20 _lpToken, uint256 _amount, uint256 _passthroughPid) external onlySubCartographer() {
        if (_passthroughPid != 0) {
            passthroughMasterChef.withdraw(_passthroughPid, _amount);
            transferPassthroughRewards();
        }
        _lpToken.safeTransfer(userAdd, _amount);
    }
    function setPassthroughTokenManagement(uint256 _passthroughPid, IBEP20 _lpToken, uint256 _initialAmount) external onlySubCartographer() {
        _lpToken.approve(address(passthroughMasterChef), 1e50);
        if (_initialAmount == 0) { return; }
        passthroughMasterChef.deposit(_passthroughPid, _initialAmount);
    }
    function disablePassthroughTokenManagement(uint256 _amount, uint256 _passthroughPid) external onlySubCartographer() {
        passthroughMasterChef.withdraw(_passthroughPid, _amount);
        transferPassthroughRewards();
    }
    ...
    function lpSupply(uint8 _pid) public view poolExists(_pid) returns (uint256) {
        return subCartographer(poolElevation[_pid]).lpSupply(_pid);
    }
    function lpToken(uint8 _pid) public view returns (IBEP20) {
        return subCartographer(poolElevation[_pid]).lpToken(_pid);
    }
    function depositFee(uint8 _pid) public view poolExists(_pid) returns (uint256) {
        return subCartographer(poolElevation[_pid]).depositFee(_pid);
    }
    function isLive(uint8 _pid) public view returns (bool) {
        return subCartographer(poolElevation[_pid]).isLive(_pid);
    }
    function userTotem(uint8 _elevation, address _userAdd) public view validElevation(_elevation) returns (bool, uint8) {
        return (
...
contract ElevationHelper is Ownable {
    ...
    uint256 baseRoundDuration = 3600;
    uint256[5] public durationMult = [0, 1, 2, 4, 2];
    uint256[5] public updatedDurationMult = [0, 1, 2, 4, 4];
    ...
    mapping(uint256 => uint8) public expeditionDeityDivider;
    mapping(uint8 => mapping(uint256 => uint8)) public winningTotem;
    ...
    function updateElevationDuration(uint8 elevation, uint8 multiplier) public onlyOwner allElevations(elevation) {
        require(multiplier > 0 && multiplier < 48, "Invalid elevation duration, 0-48hrs");
        updatedDurationMult[elevation] = multiplier;
    }

    // INITIALIZER
    function enable(uint256 genesisTime) external onlyCartographer() {
        uint256 nextHourTimestamp = genesisTime + (3600 - (genesisTime % 3600));
        elevationUnlock = [
        ...
        referralGenesis = nextHourTimestamp.add(1 days);    // burn unclaimed referral rewards
        referralRoundEndTime = referralGenesis;
        keywordRoundStart = nextHourTimestamp - (1800);
    }
        
    // FUNCTIONALITY  
    function rolloverElevation(uint8 elevation) external onlyCartographer() {
        uint256 roundsToAdd = ((block.timestamp - elevationRoundEndTime[elevation]) / (durationMult[elevation] * baseRoundDuration));
        elevationRound[elevation] += 1;
        elevationRoundEndTime[elevation] += ((durationMult[elevation] * baseRoundDuration) * roundsToAdd);

        if (updatedDurationMult[elevation] != durationMult[elevation]) {
            durationMult[elevation] = updatedDurationMult[elevation];
        }
        elevationRoundEndTime[elevation] += (durationMult[elevation] * baseRoundDuration);
    }
    ...
    function selectWinningTotem(uint8 elevation, uint256 rand) external elevationOrExpedition(elevation) onlyCartographer() {
        ...
        if (elevation == TWOTHOUSAND) { selectedWinner = uint8(rand % 2); }
        if (elevation == FIVETHOUSAND) { selectedWinner = uint8(rand % 5); }
        if (elevation == TENTHOUSAND) { selectedWinner = uint8(rand % 10); }
        if (elevation == EXPEDITION) { selectedWinner = uint8(rand % 100) < expeditionDeityDivider[elevationRound[elevation]] ? 0 : 1; }
        
        poolWinCounter[elevation][selectedWinner] += 1;
        winningTotem[elevation][elevationRound[elevation]] = selectedWinner;

        // Set Next Round Expedition Totem Divider
        if (elevation == EXPEDITION) {
            expeditionDeityDivider[elevationRound[elevation] + 1] = uint8(50 + (uint256(keccak256(abi.encode(elevation, rand, block.timestamp, block.number))) % 50)); // Rand 50 - 99
        }
    }
    function expeditionCurrentRoundDeityDivider() external view returns (uint8) {
        return expeditionDeityDivider[elevationRound[EXPEDITION]];
    }
    function historicalWinningTotems(uint8 elevation) public view allElevations(elevation) returns (uint256[20] memory) {
        if (elevation == OASIS) {
        ...
        return [
            ...
            winningTotem[elevation][round - 1],
            winningTotem[elevation][round - 2],
            winningTotem[elevation][round - 3],
            winningTotem[elevation][round - 4],
            winningTotem[elevation][round - 5],
            winningTotem[elevation][round - 6],
            winningTotem[elevation][round - 7],
            winningTotem[elevation][round - 8],
            winningTotem[elevation][round - 9],
            winningTotem[elevation][round - 10]
        ];
    }


    // HELPERS (OTHER CONTRACTS)
    ...
    function getElevationStartRound(uint8 elevation) external view returns (uint256) {
        return block.timestamp <= elevationUnlock[elevation] ? 0 : elevationRound[elevation].add(1);
    }
    ...
    function getElevationFracRoundRemaining(uint8 elevation) external view returns (uint256) {
        return internalGetElevationTimeRemainingInRound(elevation)
            .mul(1e12).div((durationMult[elevation] * baseRoundDuration));
    }


    // INTERFACE
    function elevationDuration(uint8 elevation) public view returns (uint256) {
        return (durationMult[elevation] * baseRoundDuration);
    }
    function getKeywordRoundEndTime() public view returns (uint256) {
        uint256 keywordRoundDuration = durationMult[TWOTHOUSAND] * baseRoundDuration;
        return block.timestamp + (keywordRoundDuration / 2) - (block.timestamp % keywordRoundDuration);
    }
    function getKeywordRound() external view returns (uint256) {
        return keywordRoundStart >= block.timestamp ? 0 : ((block.timestamp - keywordRoundStart) / (durationMult[TWOTHOUSAND] * baseRoundDuration));
    }
    function getKeywordRoundTimeInFailsafe() external view returns (bool) {
        return block.timestamp >= (getKeywordRoundEndTime() - ((durationMult[TWOTHOUSAND] * baseRoundDuration) / 2)) + (5 minutes);
    }
    function getElevationFracRoundProgress(uint8 elevation) external view returns (uint256) {
        return (durationMult[elevation] * baseRoundDuration).sub(internalGetElevationTimeRemainingInRound(elevation))
            .mul(1e12).div((durationMult[elevation] * baseRoundDuration));
    }
    function getElevationCurrentRoundStartTime(uint8 elevation) external view returns(uint256) {
        return elevationRoundEndTime[elevation].sub((durationMult[elevation] * baseRoundDuration));
    }
...
interface ISummitReferrals {
    function enable(SummitToken _summit) external;
...
contract CartographerOasis is ISubCart, Ownable, Initializable, ReentrancyGuard {
    ...
    struct OasisPoolInfo {              // EXISTS FOR ALL POOLS
        ...
        uint256 passthroughPid;         // PID of the passthrough masterChef
    }
    ...
    function initialize(address _ElevationHelper) external override initializer onlyCartographer() {}

    function enable(uint256 _genTime, address) external override onlyCartographer() {
    ...
    // Internal Helpers
    function safePool(uint8 _pid) internal view returns (OasisPoolInfo storage) {
        require(pidExistence[_pid], "Pool doesnt exist");
        return oasisPoolInfo[_pid];
    }

    // Add a new lp to the pool. Can only be called by the owner.
    function add(uint8 _pid, uint8,  bool _live, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate, uint256 _passthroughPid) external override onlyCartographer() nonDuplicated(_lpToken) {
        if (_withUpdate) {
        ...
        oasisPoolInfo[_pid] = OasisPoolInfo({
            ...
            passthroughPid: _passthroughPid
        });
    ...
    function addExpedition(uint8 _pid, IBEP20 _rewardToken, uint256 _rewardAmount, uint256 _rounds, bool _withUpdate) external override onlyCartographer() {}

    // Update the given pool's SUMMIT allocation point and deposit fee. Can only be called by the owner.
    function set(uint8 _pid, bool _live, uint16 _depositFeeBP, bool _withUpdate) external override onlyCartographer() {
        require(_depositFeeBP <= 400, "Invalid deposit fee");
        if (_withUpdate) {
            cartographer.massUpdatePools();
        }
        OasisPoolInfo storage pool = safePool(_pid);
    ...
    function massUpdatePools() external override onlyCartographer() {
    ...
    function updatePool(uint8 _pid) public {
        ...
        uint256 summitReward = cartographer.mintPoolSummit(pool.lastRewardTimestamp, pool.lpToken, OASIS);
        pool.accSummitPerShare = pool.accSummitPerShare.add(summitReward.mul(1e12).div(pool.lpSupply));
        pool.lastRewardTimestamp = block.timestamp;
    }
    
    function disablePassthrough(uint8 _pid) public onlyOwner {
        // SAFETY VALVE
        OasisPoolInfo storage pool = safePool(_pid);
    ...
    function setPassthrough(uint8 _pid, uint256 _passthroughPid) public onlyOwner {
        require(_passthroughPid != 0, "Passthrough pid invalid");
        // Withdraw from previous passthrough pool if applicable
        disablePassthrough(_pid);
        
        // Send lp supply to new passthroughPool
        OasisPoolInfo storage pool = safePool(_pid);
        pool.passthroughPid = _passthroughPid;
    ...
    // INTERFACE
    function rewards(uint8 _pid, address _userAdd) external view override validUserAdd(_userAdd) returns (uint256, uint256, uint256, uint256) {
        OasisPoolInfo storage pool = safePool(_pid);
        ...
        if (block.timestamp > pool.lastRewardTimestamp && pool.lpSupply != 0) {
            uint256 poolSummitEmission = cartographer.poolSummitEmission(pool.lastRewardTimestamp, pool.lpToken, OASIS);
            accSummitPerShare = accSummitPerShare.add(poolSummitEmission.mul(1e12).div(pool.lpSupply));
        }
        return (
            staked.mul(accSummitPerShare).div(1e12).sub(user.debt),
            0, 0, 0
        );
    }
    ...
    // YIELD GAMBLING STUBS

    function rollover(uint8 _elevation) external override {}
    function hypotheticalRewards(uint8, address) external view override returns (uint256, uint256) { return (uint256(0), 0); }
    function switchTotem(uint8 elevation, uint8 totem, address userAdd) external override {}
    function isTotemInUse(uint8 elevation, address userAdd) external view override returns (bool) {}



    // I/O

    function validateElevate(uint8 _pid, address _lpToken, uint256 _passthroughPid, uint8, address _userAdd) external override nonReentrant onlyCartographer() validUserAdd(_userAdd) {
        require(_lpToken == address(oasisPoolInfo[_pid].lpToken), "Different lpToken");
        require(_passthroughPid == oasisPoolInfo[_pid].passthroughPid, "Different passthrough targets");
    }

     function deposit(uint8 _pid, uint256 _amount, uint8, address _userAdd) external override nonReentrant onlyCartographer() validUserAdd(_userAdd) returns (uint256) {
        OasisPoolInfo storage pool = safePool(_pid);
        UserInfo storage user = userInfo[_pid][_userAdd];
        return unifiedDeposit(pool, user, _amount, _userAdd, false);
    }

    function elevateDeposit(uint8 _pid, uint256 _amount, uint8, address _userAdd) external override nonReentrant onlyCartographer() returns (uint256) {
        OasisPoolInfo storage pool = oasisPoolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_userAdd];        
        return unifiedDeposit(pool, user, _amount, _userAdd, true);
    }

    function unifiedDeposit(OasisPoolInfo storage pool,  UserInfo storage user, uint256 amount, address userAdd, bool isElevate) internal returns (uint256) {
        updatePool(pool.pid);
    ...
    function withdraw(uint8 _pid, uint256 _amount, address _userAdd) external override nonReentrant onlyCartographer() validUserAdd(_userAdd) returns (uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];
        require(_amount > 0 && user.staked > 0 && user.staked >= _amount, "Bad withdrawal");
        
        OasisPoolInfo storage pool = safePool(_pid);

        return unifiedWithdraw(pool,  user, _amount, _userAdd, false);
    }

    function elevateWithdraw(uint8 _pid, uint256 _amount, address _userAdd) external override nonReentrant onlyCartographer() validUserAdd(_userAdd) returns (address, uint256, uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];       
        require(_amount > 0 && user.staked > 0 && user.staked >= _amount, "Bad transfer");
        OasisPoolInfo storage pool = safePool(_pid);

        unifiedWithdraw(pool, user, _amount, _userAdd, true);
        return (address(pool.lpToken), _amount, pool.passthroughPid);
    }

    function unifiedWithdraw(OasisPoolInfo storage pool, UserInfo storage user, uint256 amount, address userAdd, bool elevationTransfer) internal returns (uint256) {
        updatePool(pool.pid);
    ...
    function emergencyWithdraw(uint8 _pid, address _userAdd) external override nonReentrant onlyCartographer() validUserAdd(_userAdd) returns (uint256) {
        OasisPoolInfo storage pool = safePool(_pid);
        UserInfo storage user = userInfo[_pid][_userAdd];
    ...
    // POOL FETCHERS
    function lpSupply(uint8 _pid) external view override returns (uint256) {
        return oasisPoolInfo[_pid].lpSupply;
    }
    function lpToken(uint8 _pid) external view override returns (IBEP20) {
        return oasisPoolInfo[_pid].lpToken;
    }
    function depositFee(uint8 _pid) external view override returns (uint256) {
        return oasisPoolInfo[_pid].depositFeeBP;
    }
    function isLive(uint8 _pid) external view override returns (bool) {
...
contract CartographerElevation is ISubCart, Ownable, Initializable, ReentrancyGuard {
    ...
    struct ElevationPoolInfo {
        ...
        uint256 passthroughPid;                     // PID of the passthrough masterchef contract
    }
    ...
    function enable(uint256 _genesis, address _trueSummit) external override {}
    ...
    modifier poolExists(uint8 _pid) {
        require(pidExistence[_pid], "Pool doesnt exist");
        _;
    }

    modifier poolExistsAndEnabled(uint8 _pid) {
        require(pidExistence[_pid], "Pool doesnt exist");
        require(elevationPoolInfo[_pid].enabled, "Pool not available yet");
        _;
    }
    ...
    function add(uint8 _pid, uint8 _elevation, bool _live, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate, uint256 _passthroughPid) external override onlyCartographer() nonDuplicated(_lpToken, _elevation) {
        if (_withUpdate) {
            cartographer.massUpdatePools();
        }
    ...
    function setPassthrough(uint8 _pid, uint256 _passthroughPid) public onlyOwner poolExists(_pid) {
        require(_passthroughPid != 0, "Passthrough info required");
        // Withdraw from previous passthrough pool if applicable
        disablePassthrough(_pid);
    ...
    // Update the given pool's SUMMIT allocation point and deposit fee. Can only be called by the owner.
    function set(uint8 _pid, bool _live, uint16 _depositFeeBP, bool _withUpdate) external override onlyCartographer() poolExists(_pid) {
        require(_depositFeeBP <= 400, "Invalid deposit fee");
        if (_withUpdate) {
            cartographer.massUpdatePools();
        }
    ...
    function rollover(uint8 _elevation) external override onlyCartographer() {
        ...
        for (uint8 i = 0; i < poolsAtElev.length; i++) {
            ...
            if (elevationPoolInfo[pidAtElev].enabled) {
                rolloverPool(pidAtElev, currRound);

            } else if (nextRound >= elevationPoolInfo[pidAtElev].startRound && elevationPoolInfo[pidAtElev].live) {
                elevationPoolInfo[pidAtElev].enabled = true;
                cartographer.enableTokenAtElevation(elevationPoolInfo[pidAtElev].lpToken, elevationPoolInfo[pidAtElev].elevation);
            }
        }
    }
    
    function rolloverPool(uint8 _pid, uint256 _currRound) internal {
        updatePool(_pid);
        ElevationPoolInfo storage pool = elevationPoolInfo[_pid];

        uint256 accSummitPerShare = pool.accSummitPerShare;
        uint256 deltaAccSummitPerShare = accSummitPerShare.sub(poolRoundInfo[_pid][_currRound - 1].endAccSummitPerShare);
        uint256 winningTotemRoundRewards = pool.totemRoundRewards[elevationHelper.winningTotem(pool.elevation, _currRound)];
        uint256 winningsMultiplier = winningTotemRoundRewards == 0 ? 0 : pool.roundRewards.mul(1e12).div(winningTotemRoundRewards);
        poolRoundInfo[_pid][_currRound] = RoundInfo({
    ...
    //        P E N D I N G       //
    ////////////////////////////////
    function rewards(uint8 _pid, address _userAdd) external view override onlyCartographer() poolExists(_pid) validUserAdd(_userAdd) returns (uint256, uint256, uint256, uint256) {
        ElevationPoolInfo storage pool = elevationPoolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_userAdd];
        (uint256 vestingAmount, uint256 vestingDuration, uint256 currentVestingPeriodStart) = winningsToReVest(pool, user, _userAdd);
        return (harvestableWinnings(pool, user, _userAdd), vestingAmount, currentVestingPeriodStart, vestingDuration);
    }
    ...
    function reVestedWinnings(UserInfo storage user) internal view returns (uint256) {
    ...
    function liveAccSummitPerShare(ElevationPoolInfo memory pool) internal view returns (uint256, uint256) {
        if (block.timestamp > pool.lastRewardTimestamp && pool.lpSupply > 0) {
            uint256 poolSummitEmission = cartographer.poolSummitEmission(pool.lastRewardTimestamp, pool.lpToken, pool.elevation);
            return (pool.accSummitPerShare.add(poolSummitEmission.mul(1e12).div(pool.lpSupply)), poolSummitEmission);
        }
        return (pool.accSummitPerShare, 0);
    }

    function liveRoundRewards(uint8 _pid, uint8 _totem, uint256 _emissionToBringCurrent) internal view poolExists(_pid) returns (uint256, uint256) {
        ElevationPoolInfo storage pool = elevationPoolInfo[_pid];
        return (
    ...
    function hypotheticalYield(ElevationPoolInfo memory pool, UserInfo storage user, uint256 accSummitPerShare) internal view returns (uint256) {
    ...
    function hypotheticalRewards(uint8 _pid, address _userAdd) external view override poolExists(_pid) validUserAdd(_userAdd) returns (uint256, uint256) {
        ...
        return (
            yield,
            userTotemRoundRewards > 0 ? yield.mul(roundRewards).div(userTotemRoundRewards) : yield
        );
    }

    function userFirstInteractedRoundWinnings(RoundInfo memory round, UserInfo storage user) internal view returns (uint256) {
        return user.staked
    ...
    function innateVestedRoundWinnings(uint256 amount, uint8 elevation) internal view returns (uint256) {
        return amount.mul(elevationHelper.getElevationFracRoundProgress(elevation)).div(1e12);
    }

    function userRoundWinnings(ElevationPoolInfo storage pool, UserInfo storage user, uint256 roundIndex, uint256 prevRoundIndex, address userAdd) internal view returns (uint256) {
        if (userTotem[userAdd][pool.elevation] != elevationHelper.winningTotem(pool.elevation, roundIndex)) { return 0; }
        
        RoundInfo memory round = poolRoundInfo[pool.pid][roundIndex];

        uint256 roundWinnings = user.prevInteractedRound == roundIndex ?
            userFirstInteractedRoundWinnings(round, user) :
            user.staked.mul(round.precomputedFullRoundMult).div(1e12);

        return roundIndex == prevRoundIndex ?
            innateVestedRoundWinnings(roundWinnings, pool.elevation) :
            roundWinnings;
    } 
    
    function harvestableWinnings(ElevationPoolInfo storage pool, UserInfo storage user, address userAdd) internal view returns (uint256) {
        uint256 currRound = elevationHelper.elevationRound(pool.elevation);
        if (currRound <= pool.startRound) { return 0; } // return this value when no previous rounds available to be harvested

        uint256 avail = reVestedWinnings(user);
    ...
    function winningsToReVest(ElevationPoolInfo storage pool, UserInfo storage user, address userAdd) internal view returns (uint256, uint256, uint256) {
        ...
        uint256 roundWinnings = userRoundWinnings(
            pool,
            user,
    ...
    function updatePool(uint8 _pid) public poolExists(_pid) {
        ElevationPoolInfo storage pool = elevationPoolInfo[_pid];
        if (pool.lastRewardTimestamp == block.timestamp) { return; }
        ...
        uint256 summitReward = cartographer.mintPoolSummit(pool.lastRewardTimestamp, pool.lpToken, pool.elevation);
        pool.accSummitPerShare = pool.accSummitPerShare.add(summitReward.mul(1e12).div(pool.lpSupply));
    ...
    function harvestAndUpdateVesting(ElevationPoolInfo storage pool, UserInfo storage user, address userAdd) internal {
        uint256 harvestable = harvestableWinnings(pool, user, userAdd);
        if (harvestable > 0) {
            cartographer.redeemRewards(userAdd, harvestable);
        }
        (uint256 vestAmount, uint256 vestDur,) = winningsToReVest(pool, user, userAdd);
    ...
    function switchTotem(uint8 _elevation, uint8 _newTotem, address _userAdd) external override nonReentrant onlyCartographer() validTotem(_elevation, _newTotem) validUserAdd(_userAdd) elevationNotLockedUntilRollover(_elevation) {
        // Switch users totem and harvest available for all pools at elevation
        ...
        for (uint8 i = 0; i < poolsAtElevation[_elevation].length; i++) {
            ...
            if (elevationPoolInfo[pidAtElev].enabled && userInfo[pidAtElev][_userAdd].staked > 0) {
                switchTotemForPool(pidAtElev, prevTotem, _newTotem, _userAdd);
            }
        }
        userTotem[_userAdd][_elevation] = _newTotem;
    }

    function switchTotemForPool(uint8 pidAtElev, uint8 _prevTotem, uint8 _newTotem, address _userAdd) internal {
        ...
        harvestAndUpdateVesting(pool, user, _userAdd);
    ...
    function validateElevate(uint8 _pid, address _lpToken, uint256 _passthroughPid, uint8 _totem, address _userAdd) external override nonReentrant onlyCartographer() poolExistsAndEnabled(_pid) validTotem(elevationPoolInfo[_pid].elevation, _totem) elevationNotLockedUntilRollover(elevationPoolInfo[_pid].elevation) {
        ...
        require(_passthroughPid == elevationPoolInfo[_pid].passthroughPid, "Different passthrough targets");
    }
    ...
    function deposit(uint8 _pid, uint256 _amount, uint8 _totem, address _userAdd) external override nonReentrant onlyCartographer() poolExistsAndEnabled(_pid) validTotem(elevationPoolInfo[_pid].elevation, _totem) validUserAdd(_userAdd) elevationNotLockedUntilRollover(elevationPoolInfo[_pid].elevation) returns (uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];
        ElevationPoolInfo storage pool = elevationPoolInfo[_pid];
        ...
        if (_amount > 0) {
            amountAfterFee = depositTokenManagement(_pid, _amount, _userAdd);
    ...
    function elevateDeposit(uint8 _pid, uint256 _amount, uint8 _totem, address _userAdd) external override poolExists(_pid) nonReentrant onlyCartographer() returns (uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];
        ElevationPoolInfo storage pool = elevationPoolInfo[_pid];
    ...
    function withdraw(uint8 _pid, uint256 _amount, address _userAdd) external override nonReentrant onlyCartographer() poolExists(_pid) validUserAdd(_userAdd) elevationNotLockedUntilRollover(elevationPoolInfo[_pid].elevation) returns (uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];
        require(_amount > 0 && user.staked > 0 && user.staked >= _amount, "Bad withdrawal");
        ElevationPoolInfo storage pool = elevationPoolInfo[_pid];
    ...
    function elevateWithdraw(uint8 _pid, uint256 _amount, address _userAdd) external override nonReentrant onlyCartographer() poolExists(_pid) validUserAdd(_userAdd) elevationNotLockedUntilRollover(elevationPoolInfo[_pid].elevation) returns (address, uint256, uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];       
        require(_amount > 0 && user.staked > 0 && user.staked >= _amount, "Bad transfer");
        ElevationPoolInfo storage pool = elevationPoolInfo[_pid];
    ...
    function unifiedWithdraw(ElevationPoolInfo storage pool, UserInfo storage user, uint256 amount, address userAdd, bool elevationTransfer) internal returns (uint256) {
        updatePool(pool.pid);
        
        harvestAndUpdateVesting(pool, user, userAdd);
    ...
    // Withdraw without caring about rewards. EMERGENCY ONLY.
    function emergencyWithdraw(uint8 _pid, address _userAdd) external override nonReentrant onlyCartographer() validUserAdd(_userAdd) poolExistsAndEnabled(_pid) elevationNotLockedUntilRollover(elevationPoolInfo[_pid].elevation) returns (uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];
    ...
    function lpSupply(uint8 _pid) external view override returns (uint256) { return elevationPoolInfo[_pid].lpSupply; }
    function lpToken(uint8 _pid) external view override returns (IBEP20) { return elevationPoolInfo[_pid].lpToken; }
    function depositFee(uint8 _pid) external view override returns (uint256) { return elevationPoolInfo[_pid].depositFeeBP; }
    function isLive(uint8 _pid) external view override returns (bool) { return elevationPoolInfo[_pid].live; }
    ...
    function totemLpSupplies(uint8 _pid) public view poolExists(_pid) returns (uint256[10] memory) {
        ElevationPoolInfo storage pool = elevationPoolInfo[_pid];
        return [
            pool.elevation >= 1 ? pool.totemLpSupplies[0] : 0,
            pool.elevation >= 1 ? pool.totemLpSupplies[1] : 0,
            pool.elevation >= 2 ? pool.totemLpSupplies[2] : 0,
            pool.elevation >= 2 ? pool.totemLpSupplies[3] : 0,
            pool.elevation >= 2 ? pool.totemLpSupplies[4] : 0,
            pool.elevation >= 3 ? pool.totemLpSupplies[5] : 0,
            pool.elevation >= 3 ? pool.totemLpSupplies[6] : 0,
            pool.elevation >= 3 ? pool.totemLpSupplies[7] : 0,
            pool.elevation >= 3 ? pool.totemLpSupplies[8] : 0,
            pool.elevation >= 3 ? pool.totemLpSupplies[9] : 0
        ];
    }
    ...
    function totemRoundRewards(uint8 _pid) public view poolExists(_pid) returns (uint256[11] memory) {
        ...
        uint256 emissionToBringCurrent = cartographer.poolSummitEmission(pool.lastRewardTimestamp, pool.lpToken, pool.elevation);
...
contract CartographerExpedition is ISubCart, Ownable, Initializable, ReentrancyGuard {
    ...
    struct ExpeditionPoolInfo {
        ...
        uint256 rewardEmission;                     // How much reward is released per second
        ...
        uint256 roundRewards;                       // Rewards of entire pool accum over round
        uint256[] totemRoundRewards;                // Rewards of each totem accum over round
        uint256 startTimestamp;                     // When rewards begin emitting
    ...
    function enable(uint256, address _trueSummit) external override onlyCartographer() {
        summit = SummitToken(_trueSummit);
    }
    ...
    // Add a new lp to the pool. Can only be called by the owner.
    function add(uint8 _pid, uint8 _elevation, bool _live, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate, uint256 _passthroughPid) external override onlyCartographer() {}

    function addExpedition(uint8 _pid, IBEP20 _rewardToken, uint256 _rewardAmount, uint256 _rounds, bool _withUpdate) external override onlyCartographer() nonDuplicated(_rewardToken) {
        require(_rewardToken.balanceOf(address(this)) >= _rewardAmount, "Must have funds to cover expedition");
        if (_withUpdate) {
            cartographer.massUpdatePools();
        }
        uint256 roundDuration = elevationHelper.elevationDuration(EXPEDITION);
        
        registerPool(_pid, _rewardToken);

        uint256 rewardEmission = _rewardAmount.div(roundDuration.mul(_rounds));
    ...
    function set(uint8, bool, uint16, bool) external override onlyCartographer() {
    ...
    function extendExpeditionPool(uint8 _pid, uint256 _additionalRewardAmount, uint256 _additionalRounds, bool _withUpdate) public onlyOwner poolExistsAndEnabled(_pid) {
        ...
        uint256 timeElapsed = block.timestamp.sub(pool.startTimestamp);
        uint256 rewardRemaining = pool.totalRewardAmount.sub(timeElapsed.mul(pool.rewardEmission));
        ...
        uint256 roundDuration = elevationHelper.elevationDuration(EXPEDITION);
        uint256 blocksRemainingWithAdditional = pool.totalRoundsCount.add(_additionalRounds).mul(roundDuration).sub(timeElapsed);
    ...
    function rollover(uint8) external override onlyCartographer() {
        uint256 currRound = elevationHelper.elevationRound(EXPEDITION);
        uint256 nextRound = currRound + 1;
        for (uint8 i = 0; i < expeditionPIDs.length; i++) {
            uint8 expeditionPID = expeditionPIDs[i];

            // If expedition currently running, rollover round
            if (expeditionPoolInfo[expeditionPID].enabled) {
                endRoundAndStartNext(expeditionPID, currRound, nextRound);

            // Start expedition if nextRound matches expedition startRound
            } else if (nextRound >= expeditionPoolInfo[expeditionPID].startRound) {
                expeditionPoolInfo[expeditionPID].enabled = true;
                expeditionPoolInfo[expeditionPID].startTimestamp = block.timestamp;
            }
        }
    }
    ...
    function endRoundAndStartNext(uint8 _pid, uint256 currRound, uint256 nextRound) internal poolExistsAndEnabled(_pid) {
        ...
        uint256 winningTotemRoundRewards = pool.totemRoundRewards[elevationHelper.winningTotem(EXPEDITION, currRound)];
        uint256 winningsMultiplier = winningTotemRoundRewards == 0 ? 0 : pool.roundRewards.mul(1e12).div(winningTotemRoundRewards);
    ...
    function rewards(uint8 _pid, address _userAdd) external view override onlyCartographer() poolExists(_pid) validUserAdd(_userAdd) returns (uint256, uint256, uint256, uint256) {
        ...
        UserInfo storage user = userInfo[_pid][_userAdd];
        return (
            harvestableWinnings(pool, user, _userAdd),
            0, 0, 0
        );
    }
    ...
    function liveAccRewPerShare(ExpeditionPoolInfo memory pool) internal view returns (uint256, uint256) {
        if (block.timestamp > pool.lastRewardTimestamp && pool.summitSupply > 0) {
            uint256 expeditionReward = getMultiplier(pool.lastRewardTimestamp, block.timestamp).mul(pool.rewardEmission);
    ...
    function liveRoundRewards(uint8 _pid, uint8 _totem, uint256 _expeditionReward) internal view poolExists(_pid) returns (uint256, uint256) {
        ExpeditionPoolInfo storage pool = expeditionPoolInfo[_pid];
        return (
    ...
    function hypotheticalYield(ExpeditionPoolInfo memory pool, UserInfo storage user, uint256 accRewPerShare) internal view returns (uint256) {
        uint256 currRound = elevationHelper.elevationRound(EXPEDITION);
        return user.prevInteractedRound == currRound ?
    ...
    function hypotheticalRewards(uint8 _pid, address _userAdd) external view override poolExists(_pid) validUserAdd(_userAdd) returns (uint256, uint256) {
        ...
        (uint256 accRewPerShare, uint256 expeditionReward) = liveAccRewPerShare(pool);
        (uint256 roundRewards, uint256 userTotemRoundRewards) = liveRoundRewards(_pid, userTotem[_userAdd], expeditionReward);
        uint256 rew = hypotheticalYield(pool, user, accRewPerShare);    
        if (rew == 0 || userTotemRoundRewards == 0) { return (0, 0); }
        return (
            rew,
            userTotemRoundRewards > 0 ? rew.mul(roundRewards).div(userTotemRoundRewards) : rew
        );
    }
    ...
    function userRoundWinnings(ExpeditionPoolInfo storage pool, UserInfo storage user, uint256 roundIndex, address userAdd) internal view returns (uint256) {
        if (userTotem[userAdd] != elevationHelper.winningTotem(EXPEDITION, roundIndex)) { return 0; }
    ...
    function harvestableWinnings(ExpeditionPoolInfo storage pool, UserInfo storage user, address userAdd) internal view poolExists(pool.pid) returns (uint256) {
        uint256 currRound = elevationHelper.elevationRound(EXPEDITION);
        ...
        uint256 avail = 0;
        for (uint256 roundIndex = user.prevInteractedRound; roundIndex < currRound; roundIndex++) {
            avail += userRoundWinnings(pool, user, roundIndex, userAdd);
        }
    ...
    function harvestWinnings(ExpeditionPoolInfo storage pool, UserInfo storage user, address _userAdd) internal {
        uint256 pending = harvestableWinnings(pool, user, _userAdd);
    ...
    function switchTotem(uint8, uint8 _newTotem, address _userAdd) external override nonReentrant onlyCartographer() validTotem(_newTotem) validUserAdd(_userAdd) expeditionNotLockedUntilRollover() {
        ...
        for (uint8 i = 0; i < expeditionPIDs.length; i++) {
            ...
            if (expeditionPoolInfo[expeditionPid].enabled && userInfo[expeditionPid][_userAdd].staked > 0) {
                switchTotemForExpedition(expeditionPid, _newTotem, _userAdd);
            }
        }
        userTotem[_userAdd] = _newTotem;
    }

    function switchTotemForExpedition(uint8 _pid, uint8 _newTotem, address _userAdd) internal {
        UserInfo storage user = userInfo[_pid][_userAdd];
        if (user.staked == 0) { return; }
    ...
    function validateElevate(uint8, address, uint256, uint8, address) external override nonReentrant onlyCartographer() {
        require(false, "No exped elev switch");
    }
   
    function deposit(uint8 _pid, uint256 _amount, uint8 _totem, address _userAdd) external override nonReentrant onlyCartographer() poolExistsAndEnabled(_pid) validTotem(_totem) validUserAdd(_userAdd) expeditionNotLockedUntilRollover() returns (uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];
    ...
    function elevateDeposit(uint8 _pid, uint256 _amount, uint8 _totem, address _userAdd) external override returns (uint256) {}

    function withdraw(uint8 _pid, uint256 _amount, address _userAdd) external override nonReentrant onlyCartographer() poolExists(_pid) validUserAdd(_userAdd) expeditionNotLockedUntilRollover() returns (uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];
        require(_amount > 0 && user.staked > 0 && user.staked >= _amount, "Bad withdrawal");
        ExpeditionPoolInfo storage pool = expeditionPoolInfo[_pid];
        updatePool(_pid);
    ...
    function elevateWithdraw(uint8 _pid, uint256 _amount, address _userAdd) external override returns (address, uint256, uint256) {}

    // Withdraw without caring about rewards. EMERGENCY ONLY.
    function emergencyWithdraw(uint8 _pid, address _userAdd) external override nonReentrant onlyCartographer() poolExistsAndEnabled(_pid) validUserAdd(_userAdd) expeditionNotLockedUntilRollover() returns (uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];
        require(user.staked > 0, "Nothing to emergency withdraw");
        ExpeditionPoolInfo storage pool = expeditionPoolInfo[_pid];
        updatePool(_pid);
    ...
    function remainingRewards(uint8 _pid) public view poolExists(_pid) returns (uint256) {
        ExpeditionPoolInfo storage pool = expeditionPoolInfo[_pid];
        if (pool.startTimestamp == 0 || pool.startTimestamp >= block.timestamp) { return pool.totalRewardAmount; }
        uint256 timeElapsed = block.timestamp.sub(pool.startTimestamp);
        return pool.totalRewardAmount.sub(timeElapsed.mul(pool.rewardEmission));
    }
    function lpSupply(uint8 _pid) external view override poolExists(_pid) returns (uint256) {
        return expeditionPoolInfo[_pid].summitSupply;
    }
    function lpToken(uint8 _pid) external view override poolExists(_pid) returns (IBEP20) { return summit; }
    function depositFee(uint8) external view override returns (uint256) { return uint256(0); }
    function isLive(uint8) external view override returns (bool) { return true; }
    function selectedTotem(uint8, address _userAdd) external view override returns (uint8) {
        return userTotem[_userAdd];
    }
    function currentRoundDeityDivider(uint8 _pid) public view poolExists(_pid) returns (uint8) {
        return elevationHelper.expeditionCurrentRoundDeityDivider();
    }
}
...
interface ISubCart {
    ...
    function enable(uint256, address) external;
    function add(uint8, uint8, bool, IBEP20, uint16, bool, uint256) external;
    function addExpedition(uint8, IBEP20, uint256, uint256, bool) external;
    function set(uint8, bool, uint16, bool) external;
    function massUpdatePools() external;

    function rollover(uint8) external;

    function rewards(uint8, address) external view returns (uint256, uint256, uint256, uint256);
    function hypotheticalRewards(uint8, address) external view returns (uint256, uint256);

    function switchTotem(uint8, uint8, address) external;
    function isTotemInUse(uint8, address) external view returns (bool);
    
    function validateElevate(uint8, address, uint256, uint8, address) external;
    function deposit(uint8, uint256, uint8, address) external returns (uint256);
    function elevateDeposit(uint8, uint256, uint8, address) external returns (uint256);
    function withdraw(uint8, uint256, address) external returns (uint256);
    function elevateWithdraw(uint8, uint256, address) external returns (address, uint256, uint256);
    function emergencyWithdraw(uint8, address) external returns (uint256);
 
    function lpSupply(uint8) external view returns (uint256);
    function lpToken(uint8) external view returns (IBEP20);
    function depositFee(uint8) external view returns (uint256);
    function isLive(uint8) external view returns (bool);
