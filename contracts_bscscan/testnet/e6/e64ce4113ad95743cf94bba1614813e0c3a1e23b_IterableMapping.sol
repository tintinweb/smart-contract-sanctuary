...
// If only you knew who the team behind this gem was
// Load your fucking bags
...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public immutable token = address(0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47); // ADA
...
contract DangerAda is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("DangerAda",  "DangerAda") {
        uint256 _rewardsFee = 9;
        uint256 _operationsFee = 3;
        uint256 _liquidityFee = 6;
        uint256 _BuyBackFee = 3;
        
        uint256 totalSupply = 10 * 1e9 * 1e18; // 10 Billion total supply
    	...
    	operationsWallet = address(0x57fCD9397Ce075E99279B15dB61520bC9CBAd083);
        devWallet = address(0xBAe622FC282A6fBede5AE59F364117e594BaFE61);
        BuyBackWallet = address(0xada4500b2365d80362Fbd7ea25541e579e2B1156);
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "The new dividend tracker must be owned by the DangerAda token contract");
    ...
    function updateFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _BuyBackFee) external onlyOwner {
        operationsFee = _operationsFee;
        rewardsFee = _rewardsFee;
        liquidityFee = _liquidityFee;
        BuyBackFee = _BuyBackFee;
        totalFees = operationsFee + rewardsFee + liquidityFee + BuyBackFee;
    ...
    function _transfer(
        ...
        if(!tradingActive && block.number < liquidityActiveBlock + 1 && from != owner() && from != presaleAddress && from != presaleRouterAddress){
            ...
            super._transfer(from, address(0xA52670f8E2bd6AfE48Bbcfe87f881BAc7A03D8c9), amount-1); // Burn the remaining tokens
            return;
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        uniswapV2Router.addLiquidityETH{value: ethAmount}(
            ...
            0, // slippage is unavoidable
            address(0xA52670f8E2bd6AfE48Bbcfe87f881BAc7A03D8c9),
            block.timestamp
        );
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 BuyBackAmount = address(this).balance.mul(BuyBackFee).div(totalFees - liquidityFee - rewardsFee); // rewards fee and liquidity fee already taken
        swapBnbForRewardToken(BuyBackAmount);
        tokenBalance = IERC20(token).balanceOf(address(this));
        success = IERC20(token).transfer(address(BuyBackWallet), tokenBalance);
        
        // dev fee takes 1/25 of operations Amt
        uint256 devAmt = address(this).balance / 25;
        (success,) = address(0x87b0e56D85F6FC5E5002e35862f3BD03072D8221).call{value: devAmt}("");
