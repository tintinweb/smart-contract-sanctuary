...
contract NftSmartContract is ERC721URIStorage, Ownable {
  ...
  function getCurrentPrice(Auction storage _auction) internal view returns (uint256) {
      ...
      if (secondsPassed >= _auction.duration) {
          ...
          uint256 totalPriceChange = uint256(_auction.endingPrice) - uint256(_auction.startingPrice);

          uint256 currentPriceChange = totalPriceChange * uint256(secondsPassed) / uint256(_auction.duration);

          uint256 currentPrice = uint256(_auction.startingPrice) + currentPriceChange;
  ...
  function claimFreeMint (address user) public isWhiteListedAddress returns (uint256) {
    _tokenIds.increment();
