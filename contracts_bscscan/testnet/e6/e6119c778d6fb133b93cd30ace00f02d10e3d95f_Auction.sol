...
contract Auction is ReentrancyGuard, Ownable, Pausable, IERC721Receiver {
    ...
    event EvNFTBlacklisted(IERC721 nft, bool blacklisted);
    event EvCurrencyBlacklisted(IERC20 indexed token, bool blacklisted);
    event EvNewAuction(
        uint256 indexed id,
    ...
    function blacklistCurrency(IERC20 cur) public onlyOwner {
        currencyBlacklist[cur] = true;
        emit EvCurrencyBlacklisted(cur, true);
    }

    function unblacklistCurrency(IERC20 cur) public onlyOwner {
        delete currencyBlacklist[cur];
        emit EvCurrencyBlacklisted(cur, false);
    }

    function blacklistNFT(IERC721 nft) public onlyOwner {
        nftBlacklist[nft] = true;
        emit EvNFTBlacklisted(nft, true);
    }

    function unblacklistNFT(IERC721 nft) public onlyOwner {
        delete nftBlacklist[nft];
        emit EvNFTBlacklisted(nft, false);
    }
...
// File @openzeppelin/contracts/utils/introspection/[email protected].1.0
