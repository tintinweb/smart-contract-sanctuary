...
contract Vault is AccessControl {
    ...
    constructor(IERC20 _token, address team) {
        multiSig = team;
        token = _token;
    ...
    modifier onlyGrantRecipient(uint256 _grantId) {
        ...
        require(_msgSender() == tokenGrant.recipient, "not grant recipient");
        _;
    }
    ...
    function deposit(uint256 amount) public onlyRole(OWNER_ROLE) {
    ...
    function addTokenGrant(
        ...
        require(_vestingCliffInDays >= 30, "cliff must be >=30 days");
        require(
            _vestingDurationInDays >= 1 * 365,
            "vest period must be >=1 year"
        );
        require(
            _vestingDurationInDays >= _vestingCliffInDays,
            "vest period must be more than cliff"
        );

        uint256 amountVestedPerDay = _amount.div(_vestingDurationInDays);
        require(amountVestedPerDay > 0, "must be more than 0");
    ...
    function removeTokenGrant(uint256 _grantId) external onlyRole(OWNER_ROLE) {
        Grant storage tokenGrant = tokenGrants[_grantId];
    ...
    function changeMultiSig(address _multiSig)
        external
        onlyRole(OWNER_ROLE)
        onlyValidAddress(_multiSig)
    {
        multiSig = _multiSig;
        emit ChangedMultisig(_multiSig);
    }
}
