...
contract CsACs is IERC20, IERC20Metadata, Context, Ownable {
    ...
    uint256 private _sellDividendFee = 2; // 2%
    ...
    function getTotalRealeasedByToken(address token) external view returns(uint256) {
    ...
    function getFees() external view returns(uint256,uint256,uint256,uint256,uint256,uint256) {
        uint256 buyDev = _buyDevFee;
        uint256 buyDiv = _buyDividendFee;
        uint256 buyTotal = _buyTotalFee;
        uint256 sellDev = _sellDevFee;
        uint256 sellDiv = _sellDividendFee;
        uint256 sellTotal = _sellTotalFee;
        return (buyDev,buyDiv,buyTotal,sellDev,sellDiv,sellTotal);
    }
    ...
    function setBuyFees(uint256 buyDevFee, uint256 buyDividendFee, uint256 buyTotalFee) external onlyOwner {
        _buyDevFee = buyDevFee;
        _buyDividendFee = buyDividendFee;
        _buyTotalFee = buyTotalFee;
    }

    function setSellFees(uint256 sellDevFee, uint256 sellDividendFee, uint256 sellTotalFee) external onlyOwner {
        _sellDevFee = sellDevFee;
        _sellDividendFee = sellDividendFee;
        _sellTotalFee = sellTotalFee;
    }
