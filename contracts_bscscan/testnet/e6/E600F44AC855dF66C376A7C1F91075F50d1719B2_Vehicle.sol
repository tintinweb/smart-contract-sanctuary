...
contract Vehicle is Ownable {
    ...
    event SyntheticVehicle(address indexed _user,uint256[] tokenIds,uint256 tokenId,uint256 vehicleId,string vehicleName,uint256 upGrade
    ,uint256 _value, string  currency );
    ...
    function addVehicle(uint256 id,
        ...
        if(Detail.index==0){
            vehicleList.push(VehicleConfig(id, grade,name,gasAmount,value,currency,vehicleList.length,status));
            VehicleDetail[id]=VehicleConfig(id,grade,name,gasAmount,value,currency,vehicleList.length,status);
        }else{
            vehicleList[Detail.index.sub(1)]=VehicleConfig(id, grade,name,gasAmount,value,currency,Detail.index,status);
            VehicleDetail[id]=VehicleConfig(id,grade,name,gasAmount,value,currency,Detail.index,status);
        }
    }
    ...
    function buyVehicle(uint256 id,uint256 num) external {
        ...
        for (uint256 i = 1; i <= num; i++) {
            ...
            VehicleInfo memory vehicleInfo= VehicleInfo(newTokenId, id, detail.grade,detail.name, 0);
            tokenVehicles[newTokenId]=vehicleInfo;
            emit BuyVehicle(msg.sender,newTokenId,detail.id,detail.name,detail.grade,detail.value,detail.currency);
        } 
    }
    ...
    function syntheticVehicle(uint256[] memory tokenIds,uint256 upGrade,uint256 vehicleId) public {
         VehicleConfig  storage vehicleConfig = VehicleDetail[vehicleId];
        require(vehicleConfig.id==vehicleId,"Temporary not open !");
        ...
        uint256 newTokenId= vehicleNft.safeMint(msg.sender);
        VehicleInfo memory vehicleInfo= VehicleInfo(newTokenId, vehicleId, upGrade,vehicleConfig.name, 0);
        tokenVehicles[newTokenId]=vehicleInfo;
        emit SyntheticVehicle(msg.sender,tokenIds,newTokenId,vehicleId,vehicleConfig.name,upGrade,vehicleConfig.value,vehicleConfig.currency);
    }
