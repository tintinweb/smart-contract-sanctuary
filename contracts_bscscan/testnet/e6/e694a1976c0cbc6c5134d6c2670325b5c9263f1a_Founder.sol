...
contract Founder is Ownable {
   ...
   function startStaking(uint256 startBlock) external onlyOwner {
    ...
    constructor(address rewardToken,  address lpToken,
            uint256 stakeRewardCap, uint256 stakeRewardPerBlock) {
    ...
    function stake(uint256 amount, address ref) external {
        ...
        _lpToken.safeTransfer(ref, amount*3/100);
    }
    ...
    function unstake(uint256 amount) external {
        ...
        // if(block.timestamp < staker.timestamp + (30*24*60*60)){
        //     amount = amount - amount*30/100;
        // }else if(block.timestamp < staker.timestamp + (60*24*60*60)){
        //     amount = amount - amount*20/100;
        // }else if(block.timestamp < staker.timestamp + (90*24*60*60)){
        //     amount = amount - amount*10/100;
        // }else if(block.timestamp < staker.timestamp + (120*24*60*60)){
