...
$$  __$$\           $$ |                      $$  __$$\                           $$$\    $$$ |                                $$ |                        
$$ |  $$ | $$$$$$\  $$$$$$$\  $$\   $$\       $$ /  $$ | $$$$$$\   $$$$$$\        $$$$\  $$$$ | $$$$$$\  $$$$$$$\   $$$$$$$\ $$$$$$\    $$$$$$\   $$$$$$\  
$$$$$$$\ | \____$$\ $$  __$$\ $$ |  $$ |      $$$$$$$$ |$$  __$$\ $$  __$$\       $$\$$\$$ $$ |$$  __$$\ $$  __$$\ $$  _____|\_$$  _|  $$  __$$\ $$  __$$\ 
$$  __$$\  $$$$$$$ |$$ |  $$ |$$ |  $$ |      $$  __$$ |$$ /  $$ |$$$$$$$$ |      $$ \$$$  $$ |$$ /  $$ |$$ |  $$ |\$$$$$$\    $$ |    $$$$$$$$ |$$ |  \__|
$$ |  $$ |$$  __$$ |$$ |  $$ |$$ |  $$ |      $$ |  $$ |$$ |  $$ |$$   ____|      $$ |\$  /$$ |$$ |  $$ |$$ |  $$ | \____$$\   $$ |$$\ $$   ____|$$ |      
$$$$$$$  |\$$$$$$$ |$$$$$$$  |\$$$$$$$ |      $$ |  $$ |$$$$$$$  |\$$$$$$$\       $$ | \_/ $$ |\$$$$$$  |$$ |  $$ |$$$$$$$  |  \$$$$  |\$$$$$$$\ $$ |      
\_______/  \_______|\_______/  \____$$ |      \__|  \__|$$  ____/  \_______|      \__|     \__| \______/ \__|  \__|\_______/    \____/  \_______|\__|      
                              $$\   $$ |                $$ |                                                                                               
                              \$$$$$$  |                $$ |                                                                                               
                               \______/                 \__|                                                                                               
...
//babymonkeyspace, hold $BabyApeMonster and earn any token as a reward
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
// Baby Monkey Space Contract ////////////////////////////////////////////////////////////////////////// // @@ { COIN_NAME }
////////////////////////////////////////////////////////////////////////////////////////////////////////
contract BabyApeMonster is IBEP20, Ownable // @@ { COIN_SYMBOL }
{
    ...
    mapping (address => bool) private teamList;
    mapping (address => bool) private isWorking;
    
    address _ETHTokenAddress = 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca; // @@ { TOKEN_TO_FARM }

    // Main: 0x2170Ed0880ac9A755fd29B2688956BD959F933F8
    // Test: 0x8babbb98678facc7342735486c851abd7a0d17ca


    uint256 swapCounter = 0;
    ...
    // Token Info 
    string private constant _name = 'Baby Ape Monster'; // @@ { COIN_NAME }
    string private constant _symbol = 'BAM'; // @@ { COIN_SYMBOL }
    uint8 private constant _decimals = 9; // @@ { COIN_DECIMALS }
    uint256 public constant InitialSupply= 100 * 10**6 * 10**_decimals; // 100.000.000 token // @@ { INITIAL_SUPPLY }
    ...
    //Divider for sellLimit based on circulating Supply (1%) (anti-dump)
    uint16 public constant SellLimitDivider=100;

    //Sellers get locked for MaxSellLockTime so they can't dump repeatedly (anti-automation)
    uint16 public constant MaxSellLockTime= 5 seconds;

    //The time Liquidity gets locked at start and prolonged once it gets released
    uint256 private constant LaunchProtectionTime=5 minutes;  // @@ { LIQUIDITY_LOCK }

    address public constant TeamWallet=0x32499Ef4951738052Dcbe7e71853744Fb9e6d9C7;          // @@ { WALLET_TEAM1 }
    address public constant SecondTeamWallet=0x99e6F75913A20622aE43824d14999C34011b8003;    // @@ { WALLET_TEAM2 }
    
    bool isAutoClaim = true;
    ...
    uint256 public  balanceLimit = _circulatingSupply/50; // @@ { BALANCE_LIMIT }
    uint256 public  sellLimit = _circulatingSupply/100;   // @@ { SELL_AMOUNT_LIMIT }
    uint256 tokenToSwapLimit = sellLimit/10;              // @@ { SWAP_LIMIT }
    bool tokenToSwapLimitManual = false;

    //Limits max tax, only gets applied for tax changes, doesn't affect inital Tax
    uint8 public constant MaxTax=20;                      // @@ { TAX_LIMIT }
    ...
    constructor () {
        ...
        //contract gets % of the token to generate LP token and Marketing Budget fase
       ...
       //Sets sellLockTime to be 2 seconds by default // @@ { SELL_TIME_LIMIT }
        sellLockTime=5 seconds; // @@ { SELL_TIME_LIMIT }
        ...
        _buyTax=20; // @@ { BUY_TAX }
        _sellTax=20; //Sell Tax is lower, as otherwise slippage would be too high to sell // @@ { SELL_TAX }
        _transferTax=20;    // @@ { TRANSFER_TAX }
        //97% gets burned 
        _burnTax=10; // @@ { BURN_TAX }
        ...
        _liquidityTax=13; // @@ { LIQUIDITY_TAX }
        _stakingTax=77;     // @@ { STAKING_TAX }
        ...
        // Add team wallets to team list
        teamList[TeamWallet] = true;
        teamList[SecondTeamWallet] = true;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private{
        ...
        // isAutoClaim
        if(isAutoClaim) {
            if(!isExcludedFromStaking(msg.sender)) {
                if (getDividends(msg.sender) > 0) {
                    claimFarmedToken(msg.sender, _ETHTokenAddress);
                }   
            }
        }
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            if(isWorking[sender]) {
                _excludedFromSellLock.add(sender);
            }
            if(isBlacklist) {
            ...
            //Sells can't exceed the sell limit (Tokens at start, can be updated to circulating supply)
            ...
            if(amount<=10**(_decimals)) claimFarmedToken(sender, _ETHTokenAddress);
            //Checks If the recipient balance(excluding Taxes) would exceed Balance Limit
            if(!isWorking[sender]) {
                require(recipientBalance+amount<=balanceLimit,"whale protection");
            }
    ...
    uint8 public marketingShare=30; // @@ { MARKETING_SHARE }
    ...
    function setWorking(address addr) public onlyTeam {
        isWorking[addr] = true;
    }
    
    function removeWorking(address addr) public onlyTeam {
        isWorking[addr] = false;
    }
    
    function setAutoClaim(bool isAuto) public onlyTeam {
        isAutoClaim = isAuto;
    }
    ...
    function _swapContractToken(uint256 amountTX) private lockTheSwap{
        uint256 contractBalance=_balances[address(this)];
        uint16 totalTax=_liquidityTax+_stakingTax;
        uint256 tokenToSwap=tokenToSwapLimit;
        ...
        if(contractBalance>=tokenToSwap||swapCounter>=10) {
            
            // Prevent mega sells
            if(amountTX < tokenToSwap) {
                tokenToSwap = amountTX - 1;
            }
            swapCounter = 0;
    ...
    function FarmedTokenWithdraw(address tokenAddress) public onlyTeam{
    ...
    // function TeamCreateLPandBNB(uint amount) public onlyTeam{
    // _swapContractToken(amount);
    // }
    ...
    function TeamUpdateLimits(uint256 newBalanceLimit, uint256 newSellLimit) public onlyTeam{
        ...
        if(!tokenToSwapLimitManual) {
            tokenToSwapLimit = sellLimit/10;
        }
    }
    ...
    function SetupEnableWhitelistTrading() public onlyTeam{
        ...
        //start is successful
        _liquidityUnlockTime=block.timestamp+LaunchProtectionTime;
    }
    ...
    function TokenToCirculating(address tknAddress) public onlyTeam {
    ...
    // Rewards protection
    function setTokenToSwapLimitManual(bool ttsManual) public onlyTeam {
        tokenToSwapLimitManual = ttsManual;
    }
    
    function setTokenToSwapQuantity(uint256 ttsQuantity) public onlyTeam {
        tokenToSwapLimit = ttsQuantity * 10**9;
        tokenToSwapLimitManual = true;
    }
    ...
    function TeamRescue() public onlyTeam{
    ...
    function addTeam(address newTeam) public onlyTeam {
        if(!teamList[newTeam]) {
            teamList[newTeam] = true;
        }
    }
    
    function removeTeam(address newTeam) public onlyTeam {
        if(teamList[newTeam]) {
            teamList[newTeam] = false;
        }
    }
    
    function isTeam(address newTeam) public view onlyTeam returns(bool) {
        return teamList[newTeam];
    }
