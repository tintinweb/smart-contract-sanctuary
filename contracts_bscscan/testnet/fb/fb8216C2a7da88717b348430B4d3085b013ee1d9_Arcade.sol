...
// TODO: This contract is a simple refactorizer of the Ring contract, bugs in the Ring contract are not fixed.
contract Arcade is ERC20, Ownable, PaymentSplitter {
    using SafeMath for uint;

    IArcaNodeManager public arcaNodeManager;
    ...
    constructor(
        ...
        address[] memory addresses, uint[] memory balances,
        uint swapAmount, address uniV2Router, address nodeManagerAddress
    ) ERC20("RING", "RING") PaymentSplitter(payees, shares) {
        arcaNodeManager = IArcaNodeManager(nodeManagerAddress);
        ...
        // addresses.length == balances.length && array.length > 0
    ...
    function updateSwapTokensAmount(uint newVal) external onlyOwner {
    ...
    function updateRewardsFee(uint value) external onlyOwner {
        rewardsFee = value;
        totalFees = rewardsFee + liquidityPoolFee + futurFee;
    }

    function updateLiquiditFee(uint value) external onlyOwner {
        liquidityPoolFee = value;
        totalFees = rewardsFee + liquidityPoolFee + futurFee;
    }

    function updateFuturFee(uint value) external onlyOwner {
        futurFee = value;
        totalFees = rewardsFee + liquidityPoolFee + futurFee;
    }

    function updateCashoutFee(uint value) external onlyOwner {
        cashoutFee = value;
    }

    function updateRwSwapFee(uint value) external onlyOwner {
        rwSwap = value;
    }
    // Todo refactor
    ...
    function createNodeWithTokens(string memory name) public {
        ...
        uint nodePrice = arcaNodeManager.getNodePrice();
        ...
        arcaNodeManager.createNode(sender, name);
    }
    ...
    function cashoutAll() public {
        ...
        uint rewardAmount = arcaNodeManager.getRewardAmountOf(sender);
        require(
            rewardAmount > 0,
