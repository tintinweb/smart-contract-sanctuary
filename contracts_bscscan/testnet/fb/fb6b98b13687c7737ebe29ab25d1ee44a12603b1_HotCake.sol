...
contract HotCake is ERC20, Ownable {
    ...
    constructor() ERC20("HotCake", "HotCake") {
        ...
        bpetherDividendToken = 0x2170Ed0880ac9A755fd29B2688956BD959F933F8;
    ...
    function updateBpetherDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(bpetherDividendTracker), "HotCake: The dividend tracker already has that address");

        BpetherDividendTracker newBpetherDividendTracker = BpetherDividendTracker(payable(newAddress));

        require(newBpetherDividendTracker.owner() == address(this), "HotCake: The new dividend tracker must be owned by the HotCake token contract");
        ...
        newBpetherDividendTracker.excludeFromDividends(address(uniswapV2Router));
...
contract BpetherDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("HotCake_Bpether_Dividend_Tracker", "HotCake_Bpether_Dividend_Tracker", 0x2170Ed0880ac9A755fd29B2688956BD959F933F8) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "HotCake_Bpether_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "HotCake_Bpether_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main HotCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "HotCake_Bpether_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
