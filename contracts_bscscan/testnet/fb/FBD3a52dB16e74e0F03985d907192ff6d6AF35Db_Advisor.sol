...
contract Advisor is AccessControl {
    ...
    uint256 public vestingPeriod = 31536000; // 365 days
    uint256 public lockPeriod = 1800 ; // 90m ,15724800; // 182 days
    uint256 public totalPoolLock = 50000000000000000000000000; // 50,000,000 HE
    bool public lockBool = false; 
    mapping(address => uint256) public totalAmount;
    mapping(address => uint256) public withdrawnAmount;
    constructor(address minter, address _tokenHeroes, uint256 _decimalHeroes){
    ...
    function addAdvisor(address[] memory _account, uint256[] memory _amount) public {
    ...
    function availableAmount(address _address) public view returns( uint256 ) {
        ...
        if(block.timestamp > lockTime){
            ...
            amount = totalAmount[address(_address)].mul(claimPeriod).div(vestingPeriod) > withdrawnAmount[address(_address)] ? (totalAmount[address(_address)].mul(claimPeriod).div(vestingPeriod) - withdrawnAmount[address(_address)]) : 0;
        }
        return amount;
    }
