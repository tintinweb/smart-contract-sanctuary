...
contract HBSCStake {
    ...
    IBEP20 hbscToken = IBEP20(_hbscAddress);
    address public _wbnbAddress;
    IBEP20 wbnbToken = IBEP20(_wbnbAddress);
    ...
    */
        ...
        external 
    {
        ...
        for (uint256 s = 0; s < stakeholders.length; s += 1){
            ...
            bnbDividends[stakeholder] += tier0.mul(token0StakedBalances[stakeholder])
                                              .div(total0stakeholders);

            if(!isStakeFinished(stakeholder, 180))
            {
                bnbDividends[stakeholder] += tier180.mul(token180StakedBalances[stakeholder])
                                                    .div(total180stakeholders);
            } 
            if(!isStakeFinished(stakeholder, 270))
            {
                bnbDividends[stakeholder] += tier270.mul(token270StakedBalances[stakeholder])
                                                    .div(total270stakeholders);
            }
            if(!isStakeFinished(stakeholder, 365))
            {
                bnbDividends[stakeholder] += tier365.mul(token365StakedBalances[stakeholder])
                                                    .div(total365stakeholders);
            }
        }
    }
    ...
    */
        ...
        internal
    {
        ...
        for (uint256 s = 0; s < stakeholders.length; s += 1){
           ...
           hbscDividends[stakeholder] += tier0.mul(token0StakedBalances[stakeholder])
                                              .div(total0stakeholders);

            if(!isStakeFinished(stakeholder, 180))
            {
                hbscDividends[stakeholder] += tier180.mul(token180StakedBalances[stakeholder])
                                                     .div(total180stakeholders);
            }
             if(!isStakeFinished(stakeholder, 270))
            {
                hbscDividends[stakeholder] += tier270.mul(token270StakedBalances[stakeholder])
                                                     .div(total270stakeholders);
            }
            if(!isStakeFinished(stakeholder, 365))
            {
                hbscDividends[stakeholder] += tier365.mul(token365StakedBalances[stakeholder])
                                                     .div(total365stakeholders);
            }
        }
    }
    ...
    */
    function StakeTokens(uint256 amount, uint256 dayLength)
        public
    {
      //  require(hbscToken.allowance(msg.sender, address(this)) >= amount, 
      //      "Please first approve HBSC");
      //  require(amount > 0, "Stake amount can not be 0");
      //  require(hbscToken.balanceOf(msg.sender) >= amount, "Error: insufficient balance");
      ...
      //  require(hbscToken.transferFrom(msg.sender, address(this), amount),
       //         "Transfer failed");  
