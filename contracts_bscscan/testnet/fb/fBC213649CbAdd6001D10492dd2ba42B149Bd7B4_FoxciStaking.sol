...
// FoxciStaking is a smart contract for distributing Foxci by asking user to stake the ERC20-based token.
contract FoxciStaking is IFoxciStaking, Ownable ,ReentrancyGuard{
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Foxcis to distribute per block.
        uint256 lastRewardBlock; // Last block number that Foxcis distribution occurs.
        uint256 accRewardPerShare; // Accumulated Foxci per share
        uint timestamp; // timestamp of the last interaction with the pool
    ...
    // Foxci tokens created per block.
    uint256 public rewardPerBlock;
    // Bonus muliplier for early Foxci makers.
    uint256 public bonusMultiplier;
    // Block number when bonus Foxci period ends.
    ...
    // The block number when Foxci mining starts.
    ...
    // Update the given pool's Foxci allocation point. Can only be called by the owner.
    function setPool(
        uint256 _pid,
        uint256 _allocPoint
    ) public override onlyOwner {
    ...
    function pendingReward(uint256 _pid, address _user) external override view returns (uint256, address) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 foxciReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accFoxciPerShare = (pool.accRewardPerShare.add(foxciReward)).mul(_stakerWeight).div(pool.totalStakingWeight);
        }
        return (user.amount.mul(accFoxciPerShare).div(GLO_VAL).sub(user.rewardDebt), pool.stakeToken);
    }
    ...
    function updatePool(uint256 _pid) public override {
        ...
        uint _blocks = block.number - pool.lastRewardBlock;
        uint _seconds = block.timestamp - pool.timestamp;
        if (_seconds > _blocks * 25) //if time goes far in the future, then use new time as 25 second * blocks.
        {
            _seconds = _blocks * 25;
        }
        pool.totalStakingWeight += _seconds.mul(pool.totalStakingAmount);
        pool.timestamp += _seconds;
    ...
    // Deposit Staking tokens to FoxciStakingToken for Foxci allocation.
    function deposit(uint256 _pid, uint256 _amount) nonReentrant public override {
    ...
    // Withdraw Staking tokens from FoxciStakingToken.
    ...
    // Harvest Foxci earn from the pool.
    ...
    function _harvest(address _to, uint256 _pid) internal {
        ...
        require(_reward <= IERC20(rewardToken).balanceOf(address(this)), "Not enough Foxci");
        user.lastBlock = block.number;
        safeFoxciTransfer(_to, _reward);
    }
    ...
    // Safe Foxci transfer function, just in case if rounding error causes pool to not have enough Foxcis.
    function safeFoxciTransfer(address _to, uint256 _amount) internal {
        uint256 foxciBal = IERC20(rewardToken).balanceOf(address(this));
        if (_amount > foxciBal) {
            IERC20(rewardToken).transfer(_to, foxciBal);
        } else {
