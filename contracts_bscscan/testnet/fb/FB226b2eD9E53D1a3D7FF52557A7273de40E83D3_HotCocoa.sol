...
contract HotCocoa is Context, IERC20, Ownable {
    ...
    /// Set the minimum tokens to burn during sells when the Price Impact is triggered
        ...
        returns (uint128 reserveA, uint128 reserveB)
    {
        ...
        (address token0, ) = sortTokens(tokenA, PancakeRouter.WETH());
        (uint128 reserve0, uint128 reserve1, ) = IPancakePair(PancakePair)
            .getReserves();
    ...
    function maybeBuybackAndBurn(
        ...
        bool burnNow;
        uint128 tokensToBurn;
        uint128 initialTokens = _tokenBalance[address(this)];
        ...
        if (bnbForBuy > 0 && bnbBalance > bnbForBuy) {
            ...
            if (_tokenBalance[address(this)] > initialTokens) {
                emit BuybackTokens(bnbForBuy, uint64(_tokenBalance[address(this)]) -
                    initialTokens);
            }
        }
    }
    ...
    function getFees(uint128 amount, bool liquidatingFullAmount)
        private
        view
        returns (
            uint128 charityFee,
        ...
        )
    {
        ...
        uint128 commonDenominator = charityRate + marketingRate + supportStreamRate;

        /// Break down rates to give a percent based off 100% total
        if (liquidatingFullAmount) {
            if (charityRate > 0)
