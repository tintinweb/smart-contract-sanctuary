    ...
    function withdrawTokenToOwner(address tokenAddress, uint256 amount) external onlyOwner {
        ...
        if (tokenAddress == address(this)) {
            ...
            if (remainingTokens < tokensToSwap) {
                uint256 liquidityRate = tokensToSwap.mul(10**(_feeDecimal + 2)).div(_liquidityTokensToSwap);
                uint256 marketingRate = tokensToSwap.mul(10**(_feeDecimal + 2)).div(_marketingTokensToSwap);
                uint256 charityRate = tokensToSwap.mul(10**(_feeDecimal + 2)).div(_charityTokensToSwap);
