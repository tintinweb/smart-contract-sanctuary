...
contract COKEDOGEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("COKEDOGE_Dividend_Tracker", "COKEDOGE_Dividend_Tracker", 9) {
    	claimWait = 3600; // in seconds
        minimumTokenBalanceForDividends = 1000000 * (10**9); //must hold 1 000 000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "COKEDOGE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "COKEDOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main COKEDOGE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "COKEDOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "COKEDOGE_Dividend_Tracker: Cannot update claimWait to same value");
...
contract COKEDOGEToken is BEP20, Ownable {
    using SafeMath for uint256;

    IPancakeswapV2Router02 public pancakeswapV2Router;
    address public immutable pancakeswapV2Pair;
    address public marketingWallet = 0x0D5B49456485a3B894afD1D4d734eEDfEb34B032;

    bool private swapping;

    COKEDOGEDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 public maxSellTransactionAmount = 20000000000 * (10**9); // max 0.2% of total supply per transaction
    uint256 public swapTokensAtAmount = 30000000 * (10**9);// treshold to swap tokens for rewards
    ...
    constructor() public BEP20("COKEDOGE", "COKEDOGE", 9) {
    	...
    	dividendTracker = new COKEDOGEDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "COKEDOGE: The dividend tracker already has that address");

        COKEDOGEDividendTracker newDividendTracker = COKEDOGEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "COKEDOGE: The new dividend tracker must be owned by the COKEDOGE token contract");
    ...
    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "COKEDOGE: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "COKEDOGE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "COKEDOGE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "COKEDOGE: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "COKEDOGE: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "COKEDOGE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "COKEDOGE: Cannot update gasForProcessing to same value");
