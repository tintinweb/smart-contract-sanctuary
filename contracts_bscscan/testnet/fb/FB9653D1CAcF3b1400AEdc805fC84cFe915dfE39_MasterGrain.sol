...
import "./GrainToken.sol";

// MasterChef is the master of GRAIN. He can make GRAIN and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once GRAIN is sgrainiciently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract MasterGrain is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of GRAINs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accGRAINPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accGRAINPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. GRAIN to distribute per block.
        uint256 lastRewardBlock;  // Last block number that GRAIN distribution occurs.
        uint256 accGrainPerShare;   // Accumulated GRAIN per share, times 1e12. See below.
    ...
    // GRAIN tokens created per block.
    uint256 public grainPerBlock;
    // Bonus muliplier for early GRAIN makers.
    ...
    // The block number when GRAIN mining starts.
    ...
    event EmissionRateUpdated(address indexed user, uint256 grainPerBlock);

    constructor(
        GrainToken _grain,
        address _feeAddDev,
        address _feeAddBb,
        uint256 _grainPerBlock,
        uint256 _startBlock
    ) public {
        grain = _grain;
        feeAddDev = _feeAddDev;
        feeAddBb = _feeAddBb;
        grainPerBlock = _grainPerBlock;
    ...
    // Update the given pool's GRAIN allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingGrain(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 grainReward = multiplier.mul(grainPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accGrainPerShare = accGrainPerShare.add(grainReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accGrainPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 grainReward = multiplier.mul(grainPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        if (convertRewards) {
            grain.mint(address(this), grainReward.mul(1500).div(10000));
            convertAndSendRewards(grainReward.mul(1500).div(10000));
        } else {
            grain.mint(feeAddDev, grainReward.mul(1500).div(10000));
        }
        grain.mint(address(this), grainReward);
        pool.accGrainPerShare = pool.accGrainPerShare.add(grainReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for GRAIN allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant poolExists(_pid) {
        ...
        user.rewardDebt = user.amount.mul(pool.accGrainPerShare).div(1e12);
    ...
    function convertAndSendRewards(uint256 tokenAmount) private {
        ...
        path[0] = address(grain);
        path[1] = router.WETH();

        grain.approve(address(router), tokenAmount);
    ...
    function safeGrainTransfer(address _to, uint256 _amount) internal {
        uint256 grainBal = grain.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > grainBal) {
            transferSuccess = grain.transfer(_to, grainBal);
        } else {
            transferSuccess = grain.transfer(_to, _amount);
        }
        require(transferSuccess, "safeGrainTransfer: transfer failed");
    }
    ...
    function harvest(uint256 _pid) internal {
        ...
        uint256 pending = user.amount.mul(pool.accGrainPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            // send rewards
            safeGrainTransfer(msg.sender, pending);
        }
    }
    ...
    function updateEmissionRate(uint256 _grainPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, _grainPerBlock);
        grainPerBlock = _grainPerBlock;
    }

      // allow owner to finalize the presale once the presale is ended
    function updateGRAINOwner(address newOwner) public onlyOwner {
        grain.transferOwnership(newOwner);
    }
