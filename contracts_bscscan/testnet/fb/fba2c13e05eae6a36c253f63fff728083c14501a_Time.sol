...
contract Time is BEP20TokenImplementation {
    ...
    function _computePriceInfo(uint256 timeStamp) internal view returns (PriceInfo memory) {
        ...
        uint256 secondInDay = SafeMath.mod(timeStamp, _secondsInDay());

        uint256 timeStampFrom = SafeMath.sub(timeStamp, secondInDay);
        uint256 timeStampTo = SafeMath.add(timeStampFrom, _secondsInDay());
    ...
    function proofOfLapse(uint256 capitalGain, uint256 releasedCoins, uint256 newTimeStamp, uint256 expense) external onlyOwner returns (bool) {
        require(SafeMath.mod(newTimeStamp, _secondsInDay()) == 0, "TimeStamp should be divisible by 86400!");
        uint256 previousPrice = historicalPrice(SafeMath.sub(newTimeStamp, _secondsInDay()));
