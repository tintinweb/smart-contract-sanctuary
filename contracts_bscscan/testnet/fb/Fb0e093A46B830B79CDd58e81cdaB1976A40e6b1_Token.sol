...
contract Token is ERC20 {
    mapping(address => bool) private _requests;

    uint256 testnetTokens = 100000000000000000000000;
    ...
    function requestTestnetTokens() public payable {
        require(!_requests[msg.sender], "Token: You cant make more requests.");

        require(
            balanceOf(address(this)) > testnetTokens,
            "Not enough more testnet tokens"
        );

        _requests[msg.sender] = true;
        _transfer(address(this), msg.sender, testnetTokens);
    }

    function canMakeTestnetRequest(address owner) public view returns (bool) {
