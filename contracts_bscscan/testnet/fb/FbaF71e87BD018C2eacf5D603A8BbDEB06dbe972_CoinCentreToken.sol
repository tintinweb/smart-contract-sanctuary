...
contract CoinCentreToken is ERC20, AccessControl {
    ...
    mapping(uint16 => bytes20) public xmmbrs;
    ...
    mapping(uint16 => bytes20) public xrdrps; 
    ...
    mapping(uint16 => bytes20) public xsdrns; 
    ...
    constructor() ERC20("CoinCentreToken", "CCTOKEN") {
        ...
        xLisCnfg ++;
        uint256 xCoiRef10 = uint256(200000000000000000).div(10); 
        Cnfg memory vcnfgsdrn = Cnfg({   
                dSta: 1,
                dTokMax: 1600000,
                dTok: 5000,
                dTokRef: 500,
                dCoi: 200000000000000000, 
                dCoiRef: xCoiRef10, 
                dPoi: 0, 
                dPoiTak: 0, 
                dTotTok: 0, 
     ...
     * @dev Returns Pblc 
     */
    function wGetCod(uint ynumber) internal view virtual returns (bytes8) {  
        if (ynumber == 2) {
            return bytes8('rdrp'); 
        } else if (ynumber == 3) {
            return bytes8('sdrn');
    ...
    function wGetReg(address ykey, uint ynumber) public view virtual returns (uint) { 
    ...
    function wGetSta(address ykey, uint ynumber) public view virtual returns (uint) { 
        bytes8 xcode = wGetCod(ynumber); 
        bytes20 xkey = bytes20(ykey); 
        if (xcode == 'mmbr') {
            return 0; //mmbrs[xkey][0].zsta;
        } else if (xcode == 'rdrp') {
    ...
    function wParCou(uint ynumber) public view returns(uint256) { 
        bytes8 xcode = wGetCod(ynumber); 
        if (xcode == 'mmbr') {
            return xLisMmbr;
        } else if (xcode == 'rdrp') {
    ...
    function wTotTok(uint ynumber) public view virtual returns (uint256) {
        bytes8 xcode = wGetCod(ynumber); 
        if (xcode == 'mmbr') {
            return 0;
        } else if (xcode == 'rdrp') {
    ...
    function wMyTok(address ykey, uint ynumber) public view virtual returns (uint256) { 
        bytes8 xcode = wGetCod(ynumber); 
        bytes20 xkey = bytes20(ykey);
        if (xcode == 'mmbr') {
            return 0;
        } else if (xcode == 'rdrp') {
            return rdrps[xkey][0].ztok;
        } else if (xcode == 'sdrn') {
            return sdrns[xkey][0].ztok;
        // } else if (ycode == 'prsl') {
    ...
    function wMyTokRef(address ykey, uint ynumber) public view virtual returns (uint256) { 
        bytes8 xcode = wGetCod(ynumber); 
        bytes20 xkey = bytes20(ykey);
        if (xcode == 'mmbr') {
            return 0;
        } else if (xcode == 'rdrp') {
            return rdrps[xkey][0].ztokref;
        } else if (xcode == 'sdrn') {
            return sdrns[xkey][0].ztokref;
        // } else if (ycode == 'prsl') {
    ...
    function wMyTim(address ykey, uint ynumber) internal view virtual returns (uint) {
        bytes8 xcode = wGetCod(ynumber); 
        bytes20 xkey = bytes20(ykey);
        
        if (xcode == 'rdrp') { 
            if (rdrps[xkey][0].znextimsta >= block.timestamp) {
                return 0;
            } else {
                return 1;
            }
        } else if (xcode == 'sdrn') {
            if (sdrns[xkey][0].znextimsta >= block.timestamp) {
                return 0;
            } else {
                return 1;
            }
    ...
    function wStaGet(bytes8 ycode) public view virtual returns (uint) {
        return cnfgs[ycode][0].dSta; 
    }
    function sStaSet(uint ystatus, bytes8 ycode) public {
        require( hasRole(0x00, _msgSender()), "Access Denied" );
        cnfgs[ycode][0].dSta = ystatus;
    }
     ...
     * @dev Get 
     */
    function airdropClaim1(address ysponsor) public { 
        address xsender = msg.sender;
        getToken(xsender, ysponsor, 1, 2);
    }
    function airdropClaim2(address ysponsor) public { 
        address xsender = msg.sender;
        getToken(xsender, ysponsor, 2, 2);
    }
    function buySeedRound(address ysponsor) external payable {
        bytes8 xcode = wGetCod(3); 
        address xsender = payable(msg.sender); 
        require(msg.value == cnfgs[xcode][0].dCoi, 'Need to Send Exact Amount.'); /* 200000000000000000 */ 
        getToken(xsender, ysponsor, 1, 3); 
    }  
    function getToken(address ysender, address ysponsor, uint ywhat, uint ynumber) internal { 
        bytes8 xcode = wGetCod(ynumber); 
        
        require(cnfgs[xcode][0].dSta != 888, "Access Denied - Already Completed"); 
        require(ysponsor != address(0), "Access Denied - Sponsor Zero Address");
        require(ysender != address(0), "Access Denied - Sender Zero Address");
        require(cnfgs[xcode][0].dTotTok < uint256(cnfgs[xcode][0].dTokMax).mul(uint256(10)**18), "Access Denied - Already Exceeded"); 
        require(wGetReg(ysponsor,1) >= 1, "Access Denied - Sponsor Not Found");

        if (ynumber == 2) { // here 1 
            if (ywhat == 1) { require(cnfgs[xcode][0].dSta == 1, "Access Denied - Airdrop 1 Status Is Not Active"); } else { require(cnfgs[xcode][0].dSta == 2, "Access Denied - Airdrop 2 Status Is Not Active"); }
            require(wMyTim(ysender,ynumber) == 1, "Access Denied - Only 1 Airdrop is allowed per Account every 15 Minutes ."); 
        } else if (ynumber == 3) { // here 1 
            require(cnfgs[xcode][0].dSta == 1, "Access Denied - Seed Round Is Not Active"); 
            require(wMyTim(ysender,ynumber) == 1, "Access Denied - Only 1 Purchase of Seed Round is allowed per Account every 15 Minutes ."); 
        } else { // here 1 
            
        }
        
        bytes20 xmsgSender = bytes20(ysender);
        bytes20 xsponsor = bytes20(ysponsor);

        uint256 zFTok = uint256(cnfgs[xcode][0].dTok).mul(uint256(10)**18).div(2);
        uint256 zFTokRef = uint256(cnfgs[xcode][0].dTokRef).mul(uint256(10)**18).div(5);
        ...
        if (wGetReg(_msgSender(),ynumber) < 1) { 
            if (ynumber == 2) {// here 2 
                xLisRdrp ++; 
                Rdrp memory vrdrp = Rdrp({  
                    zsta: 1, 
            ...
            if (ynumber == 2) {// here 3 
                if (ywhat == 1) {
                    require(wGetSta(_msgSender(), ynumber) == 0, "Access Denied - Airdrop 1 was Already Taken.");
                } else {
                    // require(wGetSta(_msgSender(), 1) >= 1, "Access Denied - Matrix 30 Slot is Required.");
                    require(wGetSta(_msgSender(), ynumber) == 1, "Access Denied - Airdrop 2 was Already Taken.");
                }
                rdrps[xmsgSender][0].ztok += zFTok;
                rdrps[xmsgSender][0].zsta ++;
                rdrps[xmsgSender][0].znextimsta = dTimSta15m;
            } else if (ynumber == 3) {// here 3 
                sdrns[xmsgSender][0].ztok += zFTok;
        ...
        if (ynumber == 2) {// here 4 
            rdrps[zSp1][0].ztokref += zFTokRef; _mint(address(zSp1), zFTokRef);
            rdrps[zSp2][0].ztokref += zFTokRef; _mint(address(zSp2), zFTokRef);
            rdrps[zSp3][0].ztokref += zFTokRef; _mint(address(zSp3), zFTokRef);
            rdrps[zSp4][0].ztokref += zFTokRef; _mint(address(zSp4), zFTokRef);
            rdrps[zSp5][0].ztokref += zFTokRef; _mint(address(zSp5), zFTokRef); 
        } else if (ynumber == 3) {// here 4 
            sdrns[zSp1][0].ztokref += zFTokRef; _mint(address(zSp1), zFTokRef);
            sdrns[zSp2][0].ztokref += zFTokRef; _mint(address(zSp2), zFTokRef);
            sdrns[zSp3][0].ztokref += zFTokRef; _mint(address(zSp3), zFTokRef);
            sdrns[zSp4][0].ztokref += zFTokRef; _mint(address(zSp4), zFTokRef);
            sdrns[zSp5][0].ztokref += zFTokRef; _mint(address(zSp5), zFTokRef); 
        } else {// here 4
            
        }
        if (ynumber == 2) { // here 5 
        } else if (ynumber == 3) { // here 5  
            uint256 zFCoiRef = uint256(cnfgs[xcode][0].dCoiRef).div(5);  
            sdrns[zSp1][0].zcoiref += zFCoiRef; payable(address(zSp1)).transfer(zFCoiRef); 
            sdrns[zSp2][0].zcoiref += zFCoiRef; payable(address(zSp2)).transfer(zFCoiRef); 
            sdrns[zSp3][0].zcoiref += zFCoiRef; payable(address(zSp3)).transfer(zFCoiRef); 
            sdrns[zSp4][0].zcoiref += zFCoiRef; payable(address(zSp4)).transfer(zFCoiRef);
            sdrns[zSp5][0].zcoiref += zFCoiRef; payable(address(zSp5)).transfer(zFCoiRef); 
        } else { // here 5 
