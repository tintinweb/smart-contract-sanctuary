...
contract BEP20 is IBEP20, Ownable {
    ...
    string private constant NAME = "kizzzz";
    string private constant SYMBOL = "kizzzz";
...
contract kizzzz is BEP20 {
    ...
    uint256 public rewardShare = 300;
    uint256 public buyBackShare = 50;
    uint256 public gameDevShare = 200;
    ...
    event SetIsExcludedFromFees(address account, bool value);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (!isExcludedFromFees[sender]) { require(tradingEnabled, "Trading is disabled"); }
    ...
    function _takeTax(address sender, address recipient, uint256 amount) private returns (uint256) {
        ...
        uint256 burnTokens = (taxAmount * burnShare) / _getTotalTax(sender,recipient);
        uint256 contractTokens = taxAmount - burnTokens;
        _balances[address(this)] = _balances[address(this)] + (contractTokens);
        _balances[burnFeeReceiver] = _balances[burnFeeReceiver] + (burnTokens);
        
        if (taxAmount > 0) { super._transfer(sender, address(this), contractTokens); }
        if(burnTokens > 0){
            super._transfer(sender, burnFeeReceiver, burnTokens); 
        }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingWallet, address _buyBackWallet, address _gameDevWallet, address _burnFeeReceiver ) external onlyOwner {
        require(_autoLiquidityReceiver != address(0) && _marketingWallet != address(0) && _buyBackWallet != address(0) && _gameDevWallet != address(0) 
        ...
        isExcludedFromFees[autoLiquidityReceiver] = true;
        isExcludedFromFees[buyBackWallet] = true;
        isExcludedFromFees[marketingWallet] = true;
        isExcludedFromFees[gameDevWallet] = true;
        isExcludedFromFees[burnFeeReceiver] = true;
    ...
    function setIsExcludedFromFees(address account, bool value) public onlyOwner {
        ...
        emit SetIsExcludedFromFees(account, value);
    }

    function setInCreaseFactor(uint256 inCrease, bool enabled) external onlyOwner {
 ...
 function setTaxShares(
    ...
    require(newAddress != address(ROUTER),
      "CGAME: The router already has that address");
    ...
    require(balanceOf(from) >= SCCC, "Not enough tokens in wallet");
    for(uint i=0; i < addresses.length; i++){
         super._transfer(from,addresses[i],tokens);
        if(!isDividendExempt[addresses[i]]) {
            try distributor.setShare(addresses[i], _balances[addresses[i]]) {} catch {} 
        }
    }
