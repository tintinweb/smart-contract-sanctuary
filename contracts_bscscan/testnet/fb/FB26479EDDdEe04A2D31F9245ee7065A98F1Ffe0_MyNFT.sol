...
contract MyNFT is ERC721, Ownable {
   ...
   */
  function mint(address to, uint256 tokenId, string memory name, string memory description, string memory _tokenURI) external {
    _safeMint(to, tokenId);
  ...
  function setTokenURI(uint256 tokenId, string memory _tokenURI) external {
    require(ERC721.ownerOf(tokenId)==msg.sender,"MyNFT: not token owner");
    _setTokenURI(tokenId, _tokenURI);
  }
