...
contract StakingKill {
  ...
  function testCalcDividends(uint256 amount, uint256 percent, uint256 accrualPeriod, uint256 periods) public pure returns(uint256) {
    uint256 year = 365 days;
    uint256 dividends = (amount).mul(periods).mul(percent / year.div(accrualPeriod) / 100);
    return dividends;
  }
