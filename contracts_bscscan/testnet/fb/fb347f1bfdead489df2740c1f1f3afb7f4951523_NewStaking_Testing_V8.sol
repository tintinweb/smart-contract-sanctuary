...
contract NewStaking_Testing_V8 is Ownable {
    ...
    */
    ...
    function getAPY(address _staker) public view returns(uint){
        uint apy = rewardRate;
        if(block.timestamp.sub(firstTime[_staker]) > unstakeTime && alreadyProgUnstaked[_staker] == 0){
    ...
    */
    ...
    function getTimeToWithdraw(address _staker) public view returns (uint){
        ...
        uint _res = 0;
        uint _time = stakingTime[_staker];
        if(alreadyProgUnstaked[_staker] == 0 && !ended && block.timestamp <= _time.add(unstakeTime)){
