...
contract JAuction is AccessControlUpgradeable, ERC1155HolderUpgradeable
{
        ...
        public initializer
    {
        __AccessControl_init_unchained();
        __ERC1155Holder_init_unchained();
        __JAuction_init_unchained(_admin, _feeManager, _jnft);
    }

    function __JAuction_init_unchained(address _admin, address _feeManager, address _jnft)
    ...
    {
        require(jnft.balanceOf(msg.sender, _tokenId) >= _amount, "not enough tokens");
        ...
        jnft.safeTransferFrom(msg.sender, address(this), _tokenId, _amount, "");

        _auctionIdTracker.increment();

        emit Start(_auctionId, msg.sender, _tokenId, _acceptedToken, _amount, _price, _startTime, _endTime);
    }

    function bid(uint256 _auctionId, uint256 _price)
        external returns (uint256 _bidId)
    {
        Auctions.Auction memory _auction = auctionMap[_auctionId];
        ...
        if (_lastBid.price > 0) {
            IERC20(_auction.acceptedToken).safeTransfer(_lastBid.owner, _lastBid.price);
        }
        ...
        emit Bid(msg.sender, _auctionId, _price);
    }
        ...
        external
    {
        ...
        uint256 _platformFee = feeManager.calculateFee(_auction.price);
        uint256 _royaltyFee = jnft.calculateRoyaltyFee(_auction.tokenId, _auction.price);
        uint256 _finalAmount = _auction.price.sub(_platformFee).sub(_royaltyFee);
        address _creator = jnft.creatorOf(_auction.tokenId);
        ...
        jnft.safeTransferFrom(address(this), msg.sender, _auction.tokenId, _auction.amount, "");
        ...
        external
    {
        ...
        if (_lastBid.bidId == 0) { // no bidder
            jnft.safeTransferFrom(address(this), _auction.owner, _auction.tokenId, _auction.amount, "");
        } else {            
            uint256 _platformFee = feeManager.calculateFee(_lastBid.price);
            uint256 _royaltyFee = jnft.calculateRoyaltyFee(_auction.tokenId, _lastBid.price);
            ...
            IERC20(_auction.acceptedToken).safeTransfer(_creator, _royaltyFee);
            IERC20(_auction.acceptedToken).safeTransfer(_auction.owner, _finalAmount);
            jnft.safeTransferFrom(address(this), _lastBid.owner, _auction.tokenId, _auction.amount, "");
        }
        ...
        internal pure returns (Auction[] memory result) 
    {
        ...
        for (uint256 i = 0; i < limit; i++) {
            if (page * limit + i >= auctions.length) {
                result[i] = Auction(0, address(0), 0, 0, address(0), 0, 0, 0, 0, 0, false);
            } else {
...
contract MockJAuction is JAuction
{
