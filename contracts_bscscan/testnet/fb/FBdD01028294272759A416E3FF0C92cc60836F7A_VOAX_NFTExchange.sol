...
contract VOAX_NFTExchange is OrderState, TransferSafe {
    ...
    INFTMintableStore private NFTMintableStore;
    address public weth;
    ...
    constructor (address payable beneficiary, address buyerfeesigner, uint256 beneficiaryfee, address NFTStore, address wethAddr) public {
        beneficiaryAddress = beneficiary;
        buyerFeeSigner = buyerfeesigner;
        beneficiaryFee = beneficiaryfee;
        NFTMintableStore = INFTMintableStore(NFTStore);
        weth = wethAddr;
    }
    ...
    function mintableStoreExchange(Order calldata order, Sig calldata sig, Sig calldata buyerFeeSig, address buyer,uint256 royaltyFee, address payable royaltyReceipt, uint256 expiry, uint8 v, bytes32 r, bytes32 s) external payable  {
        ...
        NFTMintableStore.mintWithSignature(royaltyReceipt, buyer,order.key.sellAsset.tokenId, expiry, v, r, s);
        emitBuy(order, buyer);
    }
