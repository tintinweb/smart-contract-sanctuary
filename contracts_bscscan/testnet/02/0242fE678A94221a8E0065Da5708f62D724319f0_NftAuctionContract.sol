...
interface IMintableERC721 is IERC721Upgradeable {
    ...
    function use(uint256 _tokenId) external;
    function unUse(uint256 _tokenId) external; */
}
    ...
    OwnableUpgradeable
{
    ...
    struct CAMPAIGN_INFO {
        uint32 startId; // Start tokenID
        uint32 endId; // End tokenID
        uint256 startTime;
        uint256 endTime;
        ...
        bool isFlatPrice;
        uint16 maxBuyNftPerAddress; // Max allow buy nft
        uint16 maxBuyBidPerAddress; // Max allow bid
    }
    ...
    mapping(uint16 => CAMPAIGN_INFO) public campaigns;
    // other stats
    // mapping(uint16 => uint256) public highestPrice; // campaignId => highest price
    // mapping(uint16 => uint256) public lowestPrice; // campaignId => highest price
    mapping(uint16 => uint32) public totalBid; // campaignId => Total bid in campaign
    mapping(uint16 => uint32) public totalBidNft; // campaignId => Total bid NFT in campaign
    mapping(uint16 => uint32) public totalSold; // campaignId => Sold total

    mapping(uint16 => BID_INFO[]) public bids; // campaignId => bids
    ...
    // Buyer record
    mapping(uint16 => mapping(address => uint32)) public buyersNftTotal; // campaignId => buyer => total
    mapping(uint16 => mapping(address => uint32[])) public buyerBid; // campaignId => bid index
    ...
    function placeBid(
        ...
        ); // The campaign have not started / Expired / Locked

        // Only an address with 1 bidding
        // require(buyersNftTotal[_campaignId][_msgSender()] == 0, "Bidded");

        if (campaign.isFlatPrice) {
            uint256 totalNFTs = campaigns[_campaignId].endId -
                campaigns[_campaignId].startId +
                1;
            require(
                totalNFTs >= (totalBidNft[_campaignId] + _quantity),
                "Invalid quantity / sold out"
            ); // The campaign locked
        }

        require(
            _quantity > 0 && _priceEach >= campaign.startPrice,
            "Required valid quantity/price"
        ...
        require(
            allowToPayAmount >= campaign.startPrice,
            "Invalid token allowance"
        );

        require(
            campaign.maxBuyNftPerAddress >
                buyersNftTotal[_campaignId][_msgSender()] &&
                campaign.maxBuyBidPerAddress >
                buyerBid[_campaignId][_msgSender()].length,
            "Got limited"
        );
        ...
        BID_INFO memory bidding = BID_INFO({
            ...
            quantity: _quantity,
            winQuantity: campaign.isFlatPrice ? _quantity : 0 // Flat price
        });
        bids[_campaignId].push(bidding);

        // transfer BUSD

        // Update Stats
        /* if (highestPrice[_campaignId] < _priceEach) {
            highestPrice[_campaignId] = _priceEach;
        }
        if (
            lowestPrice[_campaignId] == 0 ||
            lowestPrice[_campaignId] > _priceEach
        ) {
            lowestPrice[_campaignId] = _priceEach;
        } */
        buyerBid[_campaignId][_msgSender()].push(totalBid[_campaignId]);
        buyersNftTotal[_campaignId][_msgSender()] += _quantity;
        totalBidNft[_campaignId] += _quantity;
        totalBid[_campaignId]++;

        emit PlaceBid(_msgSender(), _campaignId, _quantity, _priceEach);
    }
    ...
    function increaseBid(
        ...
        BID_INFO storage bidding = bids[_campaignId][_bidIndex];
        ...
        emit IncreaseBid(_msgSender(), _campaignId, _quantity, _priceEach);
    }

    function handleEndAuction(
        uint16 _campaignId,
        uint16[] calldata bidsIndex,
        uint16[] calldata quantityWin
    ) external onlyAdmin {
        ...
        for (uint256 i = 0; i < quantityWin.length; i++) {
            sum = sum + quantityWin[i];
        }
        require(totalNFTs >= sum, "Wrong quantity");

        for (uint16 i = 0; i < bidsIndex.length; i++) {
            if (bids[_campaignId][i].quantity >= quantityWin[i])
                bids[_campaignId][i].winQuantity = quantityWin[i];
        }
    }
     ...
     */
    function claim(uint16 _campaignId, uint256 _bidIdx) external whenNotPaused {
        BID_INFO memory bid = bids[_campaignId][_bidIdx];
        require(
            bid.winQuantity > 0 && bid.creator == _msgSender(),
            "Invalid claim"
        );
        require(
            campaigns[_campaignId].ended || campaigns[_campaignId].isFlatPrice,
            "Campaign not end"
        );

        uint256 tokenId;
        uint256 remainBusd = (bid.quantity - bid.winQuantity) * bid.priceEach;

        for (uint8 i = 0; i < bid.winQuantity; i++) {
            tokenId = campaigns[_campaignId].startId + totalSold[_campaignId];

            totalSold[_campaignId]++;
        ...
        emit Claim(_msgSender(), _campaignId, _bidIdx, tokenId, remainBusd);
    }
    ...
    function createCampaign(uint16 _campaignId, uint256 _startPrice)
        external
        onlyAdmin
    {
        require(
            campaigns[_campaignId].startPrice == 0 && _startPrice > 0,
            "Invalid"
        );

        CAMPAIGN_INFO memory campaign;
        campaign.startPrice = _startPrice;
    ...
    function updateCampaign(
        ...
        bool _locked,
        uint256 _startPrice,
        uint16 _maxBuyNftPerAddress,
        uint16 _maxBuyBidPerAddress,
        bool _requireWhitelist,
        bool _isFlatPrice
    ) external onlyAdmin {
        require(
            campaigns[_campaignId].startPrice > 0 &&
                _startId > 0 &&
                _endId > _startId,
            "Invalid"
        );
        ...
        // do update
        campaigns[_campaignId].startId = _startId;
        campaigns[_campaignId].endId = _endId;
        campaigns[_campaignId].startTime = _startTime;
        campaigns[_campaignId].endTime = _endTime;
        campaigns[_campaignId].startPrice = _startPrice;
        campaigns[_campaignId].locked = _locked;
        campaigns[_campaignId].maxBuyNftPerAddress = _maxBuyNftPerAddress;
        campaigns[_campaignId].maxBuyBidPerAddress = _maxBuyBidPerAddress;
        campaigns[_campaignId].requireWhitelist = _requireWhitelist;
        campaigns[_campaignId].isFlatPrice = _isFlatPrice;
    }

    /* GETTER */
    function buyerBidCount(uint16 _campaignId) external view returns (uint256) {
        return buyerBid[_campaignId][_msgSender()].length;
    }
