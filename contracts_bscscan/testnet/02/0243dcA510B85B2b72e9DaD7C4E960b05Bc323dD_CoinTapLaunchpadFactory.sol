    ...
    Ownable
{
    ...
    function _transferTokenSupply(address _token, uint256 _presaleRate, uint256 _hardCap, address _launchpadAddr) internal {
      uint256 tokenAmount =  _presaleRate.mul(_hardCap).div(SharedConstants.DECIMAL_OF_NATIVE_CURRENCY);
...
interface ICoinTapLaunchpad { 
  ...
  function getVestingHistories(address _addr) external view returns(SharedConstants.VestingHistory[] memory);
  /* Owner */
...
contract CoinTapLaunchpad is ICoinTapLaunchpad, Ownable, Pausable, Whitelist {
    ...
    mapping(address => SharedConstants.VestingHistory[]) private vestingHistories;
    ...
    function claim() public override {
      ...
      uint256 orderIndex = vestingHistories[buyer].length;
      require(orderIndex < vestingTimeline.percents.length, "CoinTapLaunchpad: Address claimed enough token");
      
      uint256 percent = vestingTimeline.percents[orderIndex];
      uint256 duration = vestingTimeline.durations[orderIndex];

      // check duration
      require(block.timestamp > launchpadData.endAt.add(duration), "CoinTapLaunchpad: It is not time to next claim");
      
      // transfer amount for participant
      uint256 totalAmount = amountBNB.mul(launchpadData.presaleRate).div(SharedConstants.DECIMAL_OF_NATIVE_CURRENCY);
      ...
      SharedConstants.VestingHistory memory history = SharedConstants.VestingHistory(amount, block.timestamp);
    ...
    function getVestingHistories(address _addr) public view override returns (SharedConstants.VestingHistory[] memory) {
