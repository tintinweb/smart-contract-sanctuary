...
contract UnicornFarm is Ownable,IERC721Receiver {
    ...
    IERC721 public unicorn;
    
    mapping(address=>mapping(uint=>uint)) public process;
    ...
    function stake(uint tokenId,uint amount,address account) internal {
...
contract UnicronAuction is IERC721Receiver, WhiteList, UnicornFee {
    
    using SafeERC20 for IERC20;
    using AuctionLib for AuctionLib.Asset;
    using AuctionLib for AuctionLib.Auction;
    using AuctionLib for mapping(bytes32=>AuctionLib.Auction);
    
    constructor(IERC20 _DOM,uint _feeRate,address _feeOwner) UnicornFee(_DOM,_feeRate,_feeOwner) {}
    
    mapping(bytes32=>AuctionLib.Auction) public auctions;
    mapping(bytes32=>mapping(address=>uint)) public bidFunds;
    
    event AuctionCreate(address indexed _token,address indexed _seller,uint _tokenId,uint _price);
    event AuctionCancel(address indexed _token,uint _tokenId);
    event AuctionBid(address indexed _token,address indexed _buyer,uint _tokenId,uint bidPrice);
    ...
    function onERC721Received(
        ...
        (uint startBlock,uint endBlock,uint price) = abi.decode(data,(uint,uint,uint));
        AuctionLib.Asset memory asset = AuctionLib.Asset({
            tokenId: tokenId,
            token: msg.sender
        });
        createAuction(asset,from,startBlock,endBlock,price);
        return this.onERC721Received.selector;
    }
    
    function createAuction(AuctionLib.Asset memory asset,address account, uint startBlock, uint endBlock, uint price) private {
        require(price>0,"invalid price");
        bytes32 assetId = asset.hash();
        auctions[assetId].seller = account;
        auctions[assetId].startBlock = startBlock;
        auctions[assetId].endBlock = endBlock;
        auctions[assetId].buyer.price = price;
        emit AuctionCreate(asset.token,account,asset.tokenId,price);
    }
    
    function bid(AuctionLib.Asset memory asset,uint bidPrice) public {
        require(bidPrice>0,"invalid bidPrice");
        bytes32 assetId = asset.hash();
        require(auctions.exist(assetId),"not exist");
        AuctionLib.Auction storage auction = auctions[assetId];
        require(auction.startBlock>block.number,"not started");
        require(auction.endBlock<block.number,"ended");
        require(auction.checkBid(bidPrice),"error bidPrice");

        DOM.safeTransferFrom(msg.sender,address(this),bidPrice);
        
        auction.buyer.account = msg.sender;
        auction.buyer.price = bidPrice;
        uint bidFund = bidFunds[assetId][msg.sender];
        if(bidFund>0) {
            DOM.safeTransfer(msg.sender,bidFund);
        }
        bidFunds[assetId][msg.sender] = bidPrice;

        emit AuctionBid(asset.token,msg.sender,asset.tokenId,bidPrice);
    }
    
    function redeem(AuctionLib.Asset memory asset) public {
        ...
        AuctionLib.Auction storage auction = auctions[assetId];
        require(auction.buyer.account != msg.sender,"not allowed");
    ...
    function finishAuction(address buyer, AuctionLib.Asset memory asset) public {
        ...
        uint bidFund = bidFunds[assetId][buyer];
        require(auction.buyer.account == buyer,"not allowed");
        
        IERC721(asset.token).safeTransferFrom(address(this),buyer,asset.tokenId);
        DOM.safeTransfer(auction.seller,bidFund);
        
        delete auctions[assetId];
...
contract UnicornCore is UnicornToken, UnicornFee {
    ...
    constructor(string memory _name, string memory _symbol, IERC20 _DOM, uint _feeRate, address _feeOwner) ERC721(_name,_symbol) UnicornFee(_DOM,_feeRate,_feeOwner) {}
    ...
    function _isReadyToBreed(UnicornLib.Unicorn memory _unicorn) internal view returns (bool) {
        return (_unicorn.edd <= block.timestamp) && (_unicorn.children < breeds.getChildLimit(_unicorn.birth.generation));
    }
    ...
    {
        require(_tokenId > 0);
        UnicornLib.Unicorn storage unicorn = unicorns[_tokenId];
        return _isReadyToBreed(unicorn);
    }

    function breed(uint256 _matronId) public {

        require(_owns(msg.sender, _matronId));

        UnicornLib.Unicorn storage matron = unicorns[_matronId];

        require(_isReadyToBreed(matron));
        
        chargeFee(msg.sender,breeds.getFee(matron.birth.generation));

        _breed(_matronId);
    }


    function _breed(uint256 _matronId) internal {
        UnicornLib.Unicorn storage matron = unicorns[_matronId];

        uint cooldown = breeds.getCooldown(matron.birth.generation);
        matron.edd = uint64(block.timestamp + cooldown);
        matron.children++;
        
        emit Pregnant(ownerOf(_matronId), _matronId);
    }
    
    function getUnicorns(address account, uint _from, uint _to) public view returns(uint256[] memory tokenIds,UnicornLib.Unicorn[] memory unicorns) {
        
        uint size = _from.sub(_to);
        uint start = _from;
        
        tokenIds = new uint256[](size);
        unicorns = new UnicornLib.Unicorn[](size);
        
        for(;start<_to;start++) {
            uint tokenId = tokenOfOwnerByIndex(account,start);
            tokenIds[start - _from] = tokenId;
            unicorns[start - _from] = unicorns[tokenId];
        }
    }
    
    
    function feed(uint256 _matronId) public {
        
        UnicornLib.Unicorn storage _matron = unicorns[_matronId];
        
        require(_matron.edd > block.timestamp, "feed over");
        
        (uint feedFee,uint feeBoost) = breeds.getFeed(_matron.birth.generation,feeds[_matronId]);
        
        chargeFee(msg.sender, feedFee);
        
        _matron.edd = uint64(uint256(_matron.edd).sub(feeBoost));
        
        feeds[_matronId]++;
        
        emit Feed(ownerOf(_matronId),_matronId,feeds[_matronId]);
    }
    ...
    function giveBirth(MintLib.MintData memory mintData) public {
        ...
        require(unicorns.isExist(birth.matronId),"matronId is not exist");
        require(unicorns.isReadyToGiveBirth(birth.matronId),"isn't Ready");
        
        mint(mintData);
        
        delete feeds[birth.matronId];
...
abstract contract UnicornToken is ERC721URIStorage,ERC721Enumerable,Ownable {
    ...
    mapping(uint=>uint) public feeds;
    ...
    function mint(MintLib.MintData memory mintData) internal virtual returns(uint tokenId)  {
        
        require(bytes(mintData.tokenURI).length > 0, "uri should be set");
        
        tokenId = mintData.tokenId;
        
        require(tokenId <= 4294967295,"too big");
        
        require(tokenId > 0,"not allowed");
        
        require(!unicorns.isExist(tokenId),"tokenId existed");
        
        require(msg.sender==mintData.owner,"error address");
        ...
        minters[tokenId] = mintData.owner;
...
library BreedLib {
    ...
    function getFeed(Breed[] storage breeds,uint generation,uint count) internal view returns(uint feedFee, uint feedBoost) {
        uint baseFeedFee = breeds[generation].feedFee;
        
        feedFee = (2**count)*baseFeedFee;
        feedBoost =  breeds[generation].feedBoost;
    }
}
...
library UnicornLib {
    ...
    function add(mapping(uint256=>Unicorn) storage unicorns,Unicorn memory unicorn,uint tokenId) internal {
        ...
        unicorn.birth.birthTime = uint64(block.timestamp);
        unicorn.edd = 0;
        unicorn.children = 0;
    ...
    function isReadyToGiveBirth(mapping(uint256=>Unicorn) storage unicorns,uint tokenId) internal view returns(bool _ready) {
         _ready = unicorns[tokenId].edd <= block.timestamp;
    }
    ...
    bytes32 constant BIRTH_TYPEHASH = keccak256(
        "Birth(uint64 birthTime,uint32 matronId,uint16 generation,uint16 race)"
    );
    
     bytes32 constant UNICORN_TYPEHASH = keccak256(
        "Unicorn(Trait trait,Birth birth)"
    );
...
contract UnicornFarm is Ownable, ERC20 {
...
library PriceLib {
    ...
    function hash(Pair memory pair) internal pure returns (bytes32) {
    ...
    function setPrice(mapping(bytes32=>Price) storage prices, Pair memory pair, Price memory price) internal {
        ...
        prices[hash(pair)] = price;
    }
        

    function getPrice(mapping(bytes32=>Price) storage prices, uint amountIn, address _factory, Pair memory pair) internal view returns(uint amountOut) { 
        ...
        if(_factory == address(0)) {
            ...
            Price memory price = prices[hash(pair)];
            ...
            address _pair = IPancakeFactory(_factory).getPair(pair.token0, pair.token1);
...
contract PriceController is Ownable {
    ...
    function getPrice(uint256 amountIn, address tokenIn, address tokenOut) public view returns(uint256) {
