...
/* 
* $CRATER is the governance token of LaLuna.Finance
* This is not like your experiemental token.
* Website : https://www.laluna.finance
* Twitter : https://twitter.com/LalunaFinance
* Telegram: https://t.me/LalunaFinance
*
* Availble in BSC
*
* Tokenomics:
*   Non-Deflationary
*   No Built-In Taxation
*   Initial Supply              : 109,000 CRATER
*   Decimals                    : 18
*   Mint freqency               : 1 Token/Month
*   
* Initial Distribution (2.752%)
*   Development Wallet   2000  1.376%
*   Marketing Wallet     2000  1.376%
*    *for more transparency, we suggest to setup your notifications for both wallet
*
*          _/_/_/  _/_/_/      _/_/    _/_/_/_/_/  _/_/_/_/  _/_/_/    
*       _/        _/    _/  _/    _/      _/      _/        _/    _/   
*      _/        _/_/_/    _/_/_/_/      _/      _/_/_/    _/_/_/      
*     _/        _/    _/  _/    _/      _/      _/        _/    _/     
*      _/_/_/  _/    _/  _/    _/      _/      _/_/_/_/  _/    _/   Token
*
* At Laluna, We love our investors and we will listen to them.
* We will provide them a mulitple platforms to trade safely.
* We will support them to help build the community.
* We will not ask them to keep working (shill) for us, we will work for them. 
* With that said, we want peaceful, vibrant and balance community, the community that doesnt push us.
* We will handle and address FUDs one by one and all social channels possible.
*
* Token ownershipt will not be renounced to support future upgrades.
*
*/
...
interface MerchantInt{
    function transferMechant(address to, uint256 amount) external;
...
abstract contract Merchant is MerchantInt{
    ...
    function setRewardAddress() public onlyAdmin{
    ...
    function addUpdateMember(address MemberAddress, string calldata MemberMeta)public onlyReseller returns(bool result){
        if(_members[MemberAddress].locked == true) return false;
    ...
    function addHashRate(address MemberAddress, uint256 hashrate, uint duration, uint256 reward, string calldata hashMeta)public onlyReseller returns(uint code){
        ...
        if(_hashRates[msg.sender][MemberAddress].exist == false){
            ...
            _resellers[msg.sender].Members.push(MemberAddress);
            transferMechant(MemberAddress, reward);
    ...
    function updateHashRate(address MemberAddress, uint256 hashrate, uint duration, uint256 reward, string calldata hashMeta)public onlyReseller returns(uint code){
        if(_members[MemberAddress].exist && _members[MemberAddress].locked == true) return 1;
        if(_resellers[msg.sender].usedHashRate + hashrate > _resellers[msg.sender].maxHashRate) return 2;
    ...
    function rewardAddress(address to, uint256 rewardAmount, string calldata hashMeta) public returns(uint code){
        transferMechant(to, rewardAmount);
    ...
    function extendAndHashRateRewardMember(address MemberAddress, uint duration, uint256 hashrate, uint256 reward, string calldata extendRewardMeta)public onlyReseller returns(uint code){
        ...
        if(_hashRates[msg.sender][MemberAddress].hashRate !=0) return 4;
        if(_hashRates[msg.sender][MemberAddress].expiry == 0) return 5;
        if(_resellers[msg.sender].usedHashRate + hashrate > _resellers[msg.sender].maxHashRate) return 6;
        if(duration==0) return 7;
        _hashRates[msg.sender][MemberAddress].expiry += duration;
        _hashRates[msg.sender][MemberAddress].hashRate += hashrate;
        transferMechant(MemberAddress, reward);
    ...
    //why?
    function getClaimable(address reseller, address memberAddress) public view returns(uint256 unclaimed){
        if(_members[memberAddress].exist == false) return 0;
        if(_hashRates[reseller][memberAddress].exist == false) return 0;
        if(_hashRates[reseller][msg.sender].expiry < block.timestamp && _hashRates[reseller][msg.sender].hashRate ==0) return 0;
        uint time = _hashRates[reseller][msg.sender].expiry > block.timestamp? block.timestamp - _hashRates[msg.sender][reseller].startDate : _hashRates[msg.sender][reseller].expiry - _hashRates[msg.sender][reseller].startDate;
        return (time - (_hashRates[reseller][msg.sender].startDate)) * (_hashRates[msg.sender][reseller].hashRate);
    }
    
    function getHashRateInfo(address reseller, address memberAddress) public view returns(uint256 hashRate, uint expiry, uint startDate, string memory meta, uint256 claimable, bool locked) {
        return (_hashRates[reseller][memberAddress].hashRate, _hashRates[reseller][memberAddress].expiry, _hashRates[reseller][memberAddress].startDate, _hashRates[reseller][memberAddress].meta, getClaimable(reseller, memberAddress), _hashRates[reseller][memberAddress].locked);
    }
    /**
    ...
    */
    function pay(address to, uint256 amount, string calldata payMeta)public returns(bool success){
        transferMechant(to, amount);
        emit Paid(to, msg.sender, amount, payMeta);
        return true;
    }
    
    function claim(address reseller) public onlyMember returns(uint totalClaimed){
        if(_members[msg.sender].exist == false) return 1;
        if(_hashRates[reseller][msg.sender].exist == false) return 3;
        if(_hashRates[reseller][msg.sender].locked == true) return 4;
        if(_hashRates[reseller][msg.sender].hashRate !=0) return 5;
        uint time = _hashRates[reseller][msg.sender].expiry > block.timestamp? block.timestamp : _hashRates[msg.sender][reseller].expiry;
        uint256 total = (time - (_hashRates[reseller][msg.sender].startDate)) * (_hashRates[msg.sender][reseller].hashRate);
       
        if(balanceOfMerchant(_rewardAddress) > total * (_burnRate)) burnMerchant(_rewardAddress, total * (_burnRate));
        if(_hashRates[reseller][msg.sender].expiry > block.timestamp){
    ...
    function transferMechant(address to, uint256 amount) public virtual override;
...
// File: contracts/LavaToken.sol

pragma solidity 0.6.12;


// LavaToken with Governance.
contract LittleDogecoin is BEP20, Merchant {
    ...
    // Min amount to liquify. (default 109000 LilDOGE)
    uint256 public minAmountToLiquify = 109000 ether;
    ...
    function updateLavaRouter(address _router) public onlyOperator {
        ...
        require(lavaPair != address(0), "E08");//LilDOGE::updateLavaRouter: Invalid pair address.
    ...
    */
    ...
    function transferMechant(address to, uint256 amount) public override {
        transfer(to, amount);
    }
