...
contract UFOMO10 is Context, IERC20, Ownable {
    ...
    string private _name = "UFOMOLD10";
    string private _symbol = "UFOLD1";
    ...
    //  uint256[6] public feeFomo = [ 8, 11, 14, 17, 20];
    //   uint256[6] public feeFomo = [ 8, 9, 10, 11, 13]; //5
   //         uint256[6] public feeFomo = [ 4, 5, 6, 7, 7]; //6
        //   uint256[6] public feeFomo = [ 3, 3, 3, 3, 3]; //7  8(+_isExcludedFromFee[address(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3)] = true;)
     //   uint256[6] public feeFomo = [ 1, 1, 1, 1, 1];
            //   uint256[6] public feeFomo = [ 8, 9, 10, 11, 13]; //9
                  uint256[6] public feeFomo = [ 8, 11, 14, 17, 20];//10
    ...
    struct GameSettings {
        ...
        uint256 level1balance; //when weth balance lower than level1balance, pot fee will add level1Percent
        uint256 level2balance; //when weth balance lower than level2balance but more than level1balance, sell pot fee is level2Percent
        uint256 level3balance; //when weth balance lower than level3balance but more than level2balance, sell pot fee is level3Percent
        uint256 level4balance; //when weth balance lower than level4balance but more than level3balance, sell pot fee is level4Percent
                                //when weth balance more than level4balance, sell pot fee is level5Percent
    }
    ...
    event GameSettingsUpdated(
        ...
        uint256 level1balance,
        uint256 level2balance,
        uint256 level3balance,
        uint256 level4balance
    );
    ...
    constructor () public {
        gameSettings = GameSettings(
            ...
            100, //level1balance is 100 
            300, //level2balance is 300
            500, //level3balance is 500
            1000 //level4balance is 1000
        );

        liquidityAddress = _msgSender();
        gameSettingsUpdaterAddress = _msgSender();
        _marketWallet = _msgSender();
        
        _rOwned[_marketWallet] = _rTotal;
...
//   _isExcludedFromFee[address(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3)] = true;
    ...
    function setMarketAddress(address payable market) public onlyOwner() {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidityAndPot, data.tMarket);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 tLiquidityAndPot = tAmount.mul(_liquidityFee + feeFomo[healthLevel]).div(100);
        uint256 tMarket = tAmount.mul(_marketFee).div(100);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidityAndPot).sub(tMarket);
        return (tTransferAmount, TData(0, tFee, tLiquidityAndPot, tMarket, 0));
    }
    ...
    function _getRValues(TData memory _data) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rMarket = _data.tMarket.mul(_data.currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidityAndPot).sub(rMarket);
    ...
    function _takeMarket(uint256 tMarket) private {
        ...
        _rOwned[_marketWallet] = _rOwned[_marketWallet].add(rMarket);
        if(_isExcluded[_marketWallet])
            _tOwned[_marketWallet] = _tOwned[_marketWallet].add(tMarket);
    }

    function getHealthLevel() private view returns(uint256) {
        uint256 bal = bnb.balanceOf(uniswapV2Pair);
        if (bal <= gameSettings.level1balance * 10**18) {
            return 4;
        } else if (bal <= gameSettings.level2balance * 10**18) {
            return 3;
        } else if (bal <= gameSettings.level3balance * 10**18) {
            return 2;
        } else if (bal <= gameSettings.level4balance * 10**18) {
            return 1;
        }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _potFee == 0 && _marketFee == 0) return;
    ...
    function handleBuyer(address buyer, uint256 amount) private {
        ...
        int256 newTimeLeft = oldTimeLeft + int256(gameSettings.extraTimeEachPot); // eeach buy can increase
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidityAndPot, uint256 tMarket) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
