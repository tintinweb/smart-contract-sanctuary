...
contract DragonFireTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("DragonFireToken_Dividend_Tracker", "DragonFireToken_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "DragonFireToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "DragonFireToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DragonFireToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DragonFireToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "DragonFireToken_Dividend_Tracker: Cannot update claimWait to same value");
...
contract DragonFireToken is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    DragonFireTokenDividendTracker public dividendTracker;
    ...
    uint256 public maxWalletToken = 25000000000000 * (10**18);
    ...
    constructor() ERC20("DragonFire Token", "Drgnfr") {
        BNBRewardsFee = 10;
        liquidityFee = 2;
    	...
    	dividendTracker = new DragonFireTokenDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DragonFireToken: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DragonFireToken: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DragonFireToken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DragonFireToken: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DragonFireToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DragonFireToken: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        marketingWallet.transfer(marketingAmount);
         ...
         uint256 devAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee).sub(marketingFee)).mul(devFee);
