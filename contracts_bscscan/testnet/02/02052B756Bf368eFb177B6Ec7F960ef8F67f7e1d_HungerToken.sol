...
contract HungerToken is Context, IERC20, Ownable {
    ...
    event PrizePoolWon(uint64 amount, uint64 indexed blockTime, address indexed winner);
    ...
    function setContractRate(uint16 index, uint16 value) external onlyOwner {
        ...
        if(index == 0 || index == 3) {
            ...
            require(value <= 6000, "Reflection/Prize Pool Fee Rate must be equal to or lower than 6% (6000)");
        }
    ...
    // Enable or disable the lottery functionality, must turn fees to 0 or POT will still accumulate
    ...
    /// Minimum number of tokens accumulated in the POT before the lottery can be triggered
    function setLotteryThreshold(uint128 minimumTokensAccumulated) external onlyOwner {
        _prizePool.triggerThreshold = minimumTokensAccumulated;
    }

    /// Amount of tokens needed to be spend by BUYER for CHANCE to win.
    function setLotteryMinimumSpend(uint64 minimumSpendAmount) external onlyOwner {
        _prizePool.minimumSpendAmount = minimumSpendAmount;
    }
    ...
    function prizePoolTokens() external view returns (uint256) {
        return _prizePool.collectedPot / 10**9;
    }

    function tokensAvailableToSell() external view returns (uint256) {
    ...
    function _transfer(
        ...
        if (!_lockSwap) {
            ...
            if (!(isExcludedFrom.fromFee || isExcludedTo.fromFee)) {
                ...
                tokenFees = tokensForSelling + reflectionFee + prizePoolFee;
            }
    ...
    function _handlePrizePool(
        ...
        if (
            ...
            uint256 reward;
            uint256 lotteryRewardsCollected = prizePool.collectedPot;
            ...
            if (reward > 0) {
                ...
                prizePool.previousWinAmount = uint64(lotteryRewardsCollected);
                prizePool.previousWinTime = uint64(block.timestamp);
                emit PrizePoolWon(
                    uint64(lotteryRewardsCollected),
