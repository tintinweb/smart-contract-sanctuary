...
contract SharkGenes is AccessControl {
    uint8 constant GENES_VERSION = 1;
    ...
    // numerators
    uint256 constant hundredNumerator = 100;
    ...
    event SetSkinMaxStar(uint8 indexed starLimit);
    ...
    function setSkinMaxStar(uint8 starLimit) external onlyOwner {
        require(starLimit != SKIN_MAX_STAR);
        SKIN_MAX_STAR = starLimit;

        emit SetSkinMaxStar(starLimit);
    }
    ...
    function bornSkin(uint8 star) external view onlyRole(WHITELIST_ROLE) returns (uint256) {
        require (star >= 1 && star <= SKIN_MAX_STAR, "bornSkin: star error");

        uint8[32] memory _genes;
        // version
        _genes[POS_VERSION] = _formatVersion(GENES_VERSION, TYPE_SKIN);
        //star
        _genes[POS_STAR] = star;
        // ethnic
        _genes[POS_ETHNIC] = uint8(_rand(RANGE_ETHNIC, 0));
