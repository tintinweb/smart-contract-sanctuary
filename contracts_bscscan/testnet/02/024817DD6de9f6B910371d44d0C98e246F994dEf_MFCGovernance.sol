...
contract MFCGovernance is AdminAgent {
  ...
  modifier hasProposal() {
    require(_busdTreasuryAddress != address(0), "Treasury address not set");
    require(_migration.destination != address(0), "No proposal");
    _;
  }
  ...
  function startMigrationProposal(address destination) external onlyAdminAgents {
    require(_migration.destination == address(0), "Proposal already active");
    _migration.endsAt = block.timestamp + VOTING_PERIOD;
    _migration.destination = destination;
  }
  ...
  function executeMigrationProposal() external hasProposal onlyAdminAgents {
    ...
    _busdt.setMigration(_migration.destination);
  }
