...
contract Prediction is IPrediction, Ownable, Pausable {
     ...
     */
    uint256 private bufferTimestampPrice=300;

    /**
     * @dev Allowed delay time when getting randomness in seconds after round expiration.
     */
    uint256 private bufferTimestampRandomness=600;
    ...
    mapping(address => uint256) slippagePrice;
     ...
     * @param _lotteryRatio Lottery ratio (real value * 1e5)
    ...
    function setRandomService(address _random) public onlyOwner {
        require(address(randomService) == address(0x0), "PREDICTION: Only set random service 1 time");
    ...
    function setPriceService(address _price) public onlyOwner {
        require(address(priceService) == address(0x0), "PREDICTION: Only set price service 1 time");
    ...
    function setAchievementService(address _achievement) public onlyOwner {
        require(address(achievementService) == address(0x0), "PREDICTION: Only set achievement service 1 time");
     ...
     * @param _pid id of pool
     */
    function genesisLockRound(uint256 _pid) external poolExisted(_pid) onlyOwner whenNotPaused(_pid) {
        (int price, , , ,) = priceService.getLastPrice(pools[_pid].oraclePriceCaller);
     ...
     * @param _pid id of pool
     */
    function executeRound(uint256 _pid) external poolExisted(_pid) onlyOperator whenNotPaused(_pid) {
        require(
     ...
     */
    function numbersDrawn(uint256 _pid, bytes32 _requestId, uint256 _randomNumber) external override onlyRandomService() {
        ...
        require(
            ...
            "PREDICTION: Required lock pool first"
        );
    ...
    function claimable(uint256 _pid, uint256 _epoch, uint256 _tid, address _user) public view returns (bool) {
        ...
        return 
            specification.oracleCalled && (
                ...
                (specification.lotteryWinningNumber == bought.lotteryNumber)
            );
    }
    ...
    function claim(uint256 _pid, uint256 _epoch, address _to, uint256[] memory _tids) external poolExisted(_pid) {
        ...
        for (uint256 i = 0; i < _tids.length; i++) {
            ...
            require(bought.lotteryNumber >= 2 * pools[_pid].ticketNumberInit, "PREDICTION: Cannot claim initialized ticket");
            ...
            if (specifications[_pid][_epoch].oracleCalled == true) {
                ...
                if ( specification.lotteryWinningNumber == bought.lotteryNumber) {
    ...
    function _startRound(uint256 _pid, uint256 _epoch, uint256 _price) internal {
        ...
        emit StartRound(_pid, _epoch, round.startTime, round.endTime);
        _initTicket(_pid);
    }
    ...
    function _safeLockRound(uint256 _pid, uint256 _epoch, uint256 _price) internal {
        ...
        require(block.timestamp <= rounds[_pid][_epoch].endTime + bufferTimestampPrice, "PREDICTION: Can only lock round within bufferTimes");
    ...
    function _safeEndRound(uint256 _pid, uint256 _epoch) internal {
        require(block.timestamp <= rounds[_pid][_epoch].endTime + bufferTimestampRandomness, "PREDICTION: Can only end round within bufferTimes");
    ...
    function _calculateRewards(uint256 _pid, uint256 _epoch) internal {
        ...
        if (lotteryReward > 0 && specification.lotteryWinningNumber > 0) {
            achievementService.increaseWinningLotteryCount(lotteryNumberOwner[_pid][_epoch][specification.lotteryWinningNumber]);
        }
    ...
    function _buyTicket(uint256 _pid, bool _label, address _buyer, uint256 _quantity) internal returns(uint256 price, uint256 fee) {
        ...
        require(_bettable(_pid, _currentEpoch), "PREDICTION: Round not bettable");
        ...
        achievementService.updateHistory(_buyer, _pid, _currentEpoch, price, _label, _quantity);
    }
    ...
    function _nextTicket(address _buyer, uint256 _pid, bool _label, uint256 _quantity) internal view returns(uint256 price, uint256 fee, uint256 newPool, uint256 reduceRate) {
        ...
        if (_label) {
            newPool = 
                (specification.bluePool / specification.blueTicketNumber + improvementDelta * _quantity) * specification.blueTicketNumber + 
                pool.ticketPriceStart * _quantity +
                improvementDelta * (_quantity - 1) * _quantity / 2;
            price = newPool - specification.bluePool;
        } else {
            newPool = 
                (specification.redPool / specification.redTicketNumber + improvementDelta * _quantity) * specification.redTicketNumber + 
                pool.ticketPriceStart * _quantity +
                improvementDelta * (_quantity - 1) * _quantity / 2;
    ...
    function _initTicket(uint256 _pid) internal {
        ...
        _storgeTicket(_buyer, _pid, _currentEpoch, true, pool.ticketNumberInit, price, pool.ticketFeeStart);
        _storgeTicket(_buyer, _pid, _currentEpoch, false, pool.ticketNumberInit, price, pool.ticketFeeStart);
    }
    ...
    function _split(uint256 _randomNumber, uint256 _pid, uint256 _epoch) internal view returns(uint256) {
        ...
        // Calculation range
        uint256 range = lotteryNumbers[_pid][_epoch] - (2 * pool.ticketNumberInit) - 1;
