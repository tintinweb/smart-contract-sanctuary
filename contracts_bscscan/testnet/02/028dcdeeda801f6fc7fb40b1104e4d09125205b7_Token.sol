...
contract Token is Context, IERC20, Ownable {
    ...
    string private constant _NAME = 'TONNY TOKEN';
    string private constant _SYMBOL = 'TONNY';
    ...
    function _transferStandard(
        ...
        uint256 rBurn = tBurn.mul(currentRate*2).div(uint256(100));
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
...
function _setBuyTaxFees(address sender, address recipient, uint256 tAmount) public {
     ...
     function _takeLiquidity(uint256 tLiquidity) public {
        uint256 currentRate =  _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate*2).div(uint256(100));
...
function _setSellTaxFees(address sender, address recipient, uint256 tAmount) public {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(tAmount);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
