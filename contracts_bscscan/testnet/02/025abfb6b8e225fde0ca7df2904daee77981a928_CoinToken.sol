...
contract CoinToken is Context, IERC20, Ownable {
    ...
    constructor (string memory _NAME, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, uint256 _txFee,uint256 _lpFee,uint256 _DexFee,address routerAddress,address feeaddress,address tokenOwner,address service) payable {
        _name = _NAME;
        _symbol = _SYMBOL;
        _decimals = _DECIMALS;
        ...
        _maxWalletBalance = (_tTotal * 10 / 1000) * 10 ** _decimals;
    ...
    function setMaxWalletPercent(uint256 maxWalletPercent) public onlyOwner {
        _maxWalletBalance = maxWalletPercent * 10 ** _decimals;
    }
    ...
    function _transfer(
        ...
        if (_maxWalletBalance > 0 && !_isExcludedFromFee[to] && !_isExcludedFromFee[from]) {
