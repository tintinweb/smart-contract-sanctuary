...
contract MyDiamondTeam {
    ...
    struct UsernameAdrress{
        address depositorAddress; 
        string depositorName;
    }
    
    mapping(string  => UsernameAdrress) public username;
    ...
    uint8 public MAX_CHARACTER_LENGTH = 10;
    ...
    function deposit(string memory name) payable external {
        _deposit(msg.sender, msg.value, name);
    }

    //deposit with upline
    function deposit(address _upline, string memory name) payable external {
    ...
    //invest
    function _deposit(address _addr, uint256 _amount, string memory name) private {
        if (!started) {
        ...
        require(bytes(name).length <= MAX_CHARACTER_LENGTH, "max length reached.");
        //assign username for depositor
        users[_addr].username.push(UsernameAdrress(_addr, name));
    ...
    function createTeam() external returns(uint256 teamId){
        ...
        uint256 nextTeamid = total_teams_created++;
    ...
    */
    ...
    function getUsernameAddress(string memory _username) view external returns(UsernameAdrress memory _user) {
