...
contract pixelSquidObserver is  IERC721,Ownables {
  ...
  mapping (address => uint256) userApprove;
  ...
  function buyObserver(address userAddress,address tokenAddress,uint256 _amount,string memory _token_name ,uint256 _quality,uint256 _lv, string memory _url, string memory _animation_url,string memory _background,uint256 _watchLimit) public {
      require(_amount == unitPrice);
      ...
      require(!isContract(userAddress),"contract cant buy observer");

      IBEP20 payment = IBEP20(tokenAddress);

      uint256 allowToPayAmount = payment.allowance(userAddress, address(this));
      ...
      observers.push(observer(_token_name,_quality,_lv,_url,_animation_url,_background,ability,0));
  ...
  function ConvertToV2(uint256 _id, string memory _tokenName,uint256 _quality,uint256 _lv, string memory _url, string memory _animation_url,string memory _background,uint256 _watchLimit) public onlyOwner {
     Ability memory ability = Ability(_watchLimit);
     observer memory cvObserver = observer(_tokenName,_quality,_lv,_url,_animation_url,_background,ability,0);
     observerToOwner[_id] = msg.sender;
     ownerObserverCount[msg.sender]++;
     observers[_id] = cvObserver;
  ...
  function seeAllowance()public view returns (uint256){
    return busd.allowance(msg.sender,receiver);
  }
   ...
   function isContract(address _address) private view returns (bool) {
    ...
    function buybox (address userAddress , uint256 price, address paymentErc20) external {
            IBEP20 paymentToken = IBEP20(paymentErc20);
        ...
        emit Paid(userAddress,price,address(this));
