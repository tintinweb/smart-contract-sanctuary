...
contract Ico is Crowdsale, CappedCrowdsale, TimedCrowdsale, PostDeliveryCrowdsale, AllowanceCrowdsale, CapperRole {
    ...
    {
        _individualDefaultCap = investorMaxCap;
    }
    ...
    function _preValidatePurchase(address beneficiary, uint256 weiAmount) internal view {
        super._preValidatePurchase(beneficiary, weiAmount);
        // solhint-disable-next-line max-line-length
        require(_contributions[beneficiary].add(weiAmount) <= investorMaxCap && _contributions[beneficiary].add(weiAmount) >= investorMinCap, "New Horizons ICO: beneficiary's cap exceeded");
    }
