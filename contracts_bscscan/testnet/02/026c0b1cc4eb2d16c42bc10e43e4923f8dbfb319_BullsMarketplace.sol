...
contract BullsMarketplace is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    uint256 public constant COLLECTION_SIZE = 7777; // number of bulls
    ...
    constructor(
     ...
     function reflectDividend(uint256 amount) private {
        ...
        totalDividend = totalDividend + (amount/COLLECTION_SIZE);
    }
    ...
    function reflectToMinters() public payable {
     ...
     function currentMintRate() public view returns (uint256){
      return reflectionMintBalance/COLLECTION_SIZE;
    }
    ...
    function claimRewards() external nonReentrant {
        uint count = bullContract.balanceOf(msg.sender);
        uint256 balance = 0;
        for(uint i=0; i < count; i++) {
            uint tokenId = bullContract.tokenOfOwnerByIndex(msg.sender, i);
  ...
  function getReflectionBalances() public view returns(uint256) {
        ...
        )
    {
        ...
        tradingFee = (_askPrice * _collections[_collection].tradingFee) / COLLECTION_SIZE;
        creatorFee = (_askPrice * _collections[_collection].creatorFee) / COLLECTION_SIZE;
