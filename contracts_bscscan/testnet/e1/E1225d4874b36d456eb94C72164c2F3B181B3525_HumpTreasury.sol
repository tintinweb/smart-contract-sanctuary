...
contract HumpTreasury is WhaleAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice allow approved address to deposit an asset for HUMP
    ...
    function deposit(
        ...
        // mint HUMP needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        HUMP.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn HUMP for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        HUMP.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new HUMP using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        HUMP.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or HUMP from the treasury, using sHUMP as collateral. This might allow an
     * sHUMP holder to provide HUMP liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(HUMP)) {
            require(permissions[STATUS.HUMPDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sHUMP.changeDebt(value, msg.sender, true);
        require(sHUMP.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(HUMP)) {
            HUMP.mint(msg.sender, value);
            humpDebt = humpDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sHUMP.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with HUMP
     * @param _amount uint256
     */
    function repayDebtWithHUMP(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.HUMPDEBTOR][msg.sender], notApproved);
        HUMP.burnFrom(msg.sender, _amount);
        sHUMP.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        humpDebt = humpDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(HUMP), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SHUMP) {
            sHUMP = IsHUMP(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SHUMP) {
            // 9
            sHUMP = IsHUMP(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(HUMP.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns HUMP valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(HUMP)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return HUMP.totalSupply() - humpDebt;
    }
}
