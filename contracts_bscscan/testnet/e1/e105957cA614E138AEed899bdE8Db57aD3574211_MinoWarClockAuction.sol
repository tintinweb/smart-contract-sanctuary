...
contract ClockAuctionBase {
    ...
    uint256 feeBid = 0.01 ether;
    uint256 minAuctionPrice = 1 ether;
        ...
        returns (uint256)
    {
        ...
        if (price > 0) {
            ...
            require(IERC20(minoWarToken).transfer(seller, sellerProceeds));
        }

        uint256 bidExcess = _bidAmount - price;

        require(IERC20(minoWarToken).transfer(msg.sender, bidExcess));
...
contract MinoWarClockAuction is ClockAuction {
        ...
        override external
    {
        ...
        require(_endingPrice == uint256(uint128(_endingPrice)));
        require(_duration == uint256(uint64(_duration)));
        require(minAuctionPrice >= _startingPrice, "minimum price not enough");
    ...
    function setStepBid(uint256 _percent) public onlyOwner {
        stepBid = _percent;
    }

    function setMinAuctionPrice(uint256 _price) public onlyOwner {
