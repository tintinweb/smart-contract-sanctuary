...
contract PerformanceDistribution is Ownable, ReentrancyGuard {
  ...
  function distributionList(address _for) public view returns (uint256[] memory, uint256[] memory) {
    DistributionInfo[] storage distribution = distributionInfo[_for];

    uint256[] memory amount = new uint256[](distribution.length);
    uint256[] memory remainingAmount = new uint256[](distribution.length);

    for (uint256 i = 0; i < distribution.length; i++) {
      amount[i] = distribution[i].amount;
