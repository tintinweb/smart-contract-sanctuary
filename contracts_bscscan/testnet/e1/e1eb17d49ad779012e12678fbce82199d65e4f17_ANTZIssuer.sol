...
contract ANTZIssuer is Ownable {
  ...
  event AddQueen(address indexed queenAddress, address indexed chequebook);
  event RemoveQueen(address indexed queenAddress, address indexed chequebook);
  ...
  function addQueen(address queenAddress, address chequebook) public onlyOwner {
      ...
      q.chequebook = chequebook;

      queenIndex[queenAddress] = queens.length;

      emit AddQueen(queenAddress, chequebook);
  }
  ...
  function removeQueen(address queenAddress) public onlyOwner {
     ...
     address chequebook = queens[idx].chequebook;
     ...
     emit RemoveQueen(queenAddress, chequebook);
  }
  ...
  function queenWithdraw(address queenAddress) public {
      ...
      require(idx > 0, "queenAddress not found");
      ...
      require(token.transfer(q.chequebook, ant_amount), "failed to transfer token");
