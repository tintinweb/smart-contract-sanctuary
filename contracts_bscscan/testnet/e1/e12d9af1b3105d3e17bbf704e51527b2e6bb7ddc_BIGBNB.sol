...
contract BIGBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("BIGBNB_Dividend_Tracker", "BIGBNB_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BIGBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BIGBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BIGBNB contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BIGBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "BIGBNB_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BIGBNB is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    BIGBNBDividendTracker public dividendTracker;
    ...
    function setBuyFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _buybackFee) 
    public onlyOwner 
    {
    ...
    function setSellFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _buybackFee) 
    public onlyOwner 
    {
        sellBNBRewardsFee = _bnbRewardFee;
        sellLiquidityFee = _liquidityFee;
        sellMarketingFee = _marketingFee;
        sellBuybackFee = _buybackFee;
        sellTotalFees = sellBNBRewardsFee.add(sellLiquidityFee).add(sellMarketingFee).add(sellBuybackFee); // total fee transfer and buy
    }
    ...
    constructor() ERC20("BIGBNB", "BIGBNB") 
    {
        BNBRewardsFee = 4;
        liquidityFee = 1;
        marketingFee = 1;
        ...
        sellMarketingFee = 2;
        sellBuybackFee = 5;
        sellTotalFees = sellBNBRewardsFee.add(sellLiquidityFee).add(sellMarketingFee).add(sellBuybackFee); 
        
        
        marketingWallet = payable(0xdb995a5e8564Af80C411374fAf951cE267558DAA);
        buybackWallet = payable(0x00A267A6451d6E5E6fBBdcEb7122852B130d6EC3);
        
        

    	dividendTracker = new BIGBNBDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BIGBNB: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BIGBNB: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BIGBNB: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BIGBNB: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BIGBNB: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BIGBNB: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 tokensToAddLiquidityWith = contractTokenBalance.mul(sellLiquidityFee).div(sellTotalFees).div(2);
        ...
        uint256 bnbToAddLiquidityWith = deltaBalance.mul(sellLiquidityFee).div(sellTotalFees).div(2);
        
        // add liquidity to uniswap
        addLiquidity(tokensToAddLiquidityWith, bnbToAddLiquidityWith);
        // worthy marketing fee

        uint256 marketingAmount = deltaBalance.mul(sellMarketingFee).div(sellTotalFees);
        marketingWallet.transfer(marketingAmount);

        uint256 buybackAmount = deltaBalance.mul(sellBuybackFee).div(sellTotalFees);
