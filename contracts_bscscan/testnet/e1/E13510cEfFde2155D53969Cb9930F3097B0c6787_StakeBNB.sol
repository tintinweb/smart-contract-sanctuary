...
contract StakeBNB is IBEP20, Ownable
{
    ...
    uint256 private constant DefaultLiquidityLockTime= 1 minutes;
    
    //A carteira de sorteio é 
    address public constant TeamWallet=0x80D37F14864568A351312b4F5dC77316604d1FDA;
    
    //Mudar para TestNet
    address private constant PancakeRouter=0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    
    //Mudar para Router PancakeRouter
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            ...
            // As vendas não podem exceder o limite de venda (10.000 tokens no início, podem ser atualizados para o estoque circulante)
       ...
       // Remove token e lida com stake
        ...
        // Adiciona token e lida com stake
    ...
    function _calculateFee(uint256 amount, uint8 tax, uint8 taxPercent) private pure returns (uint256) {
     ...
     //BNB Autostake/////////////////////////////////////////////////////////////////////////////////////////
      //Autostake usa os saldos de cada titular para redistribuir o BNB gerado automaticamente.
    ...
    // o pagamento total por meio de stake, para fins de rastreamento
...
// distribui bnb entre ações de marketing e dividendos
    ...
    // Contrato de trocar de Tokens //////////////////////////////////////////////////////////////////////////////////

    // rastreia o BNB gerado automaticamente, útil para ticker etc
    uint256 public totalLPBNB;
	
    // Bloqueia a troca se já estiver trocando
    ...
    // troca o token no contrato para Marketing BNB e LP Token.
    // sempre troca o limite de venda do token para evitar um grande impacto no preço
    function _swapContractToken() private lockTheSwap{
        uint256 contractBalance=_balances[address(this)];
        uint16 totalTax=_liquidityTax+_stakingTax;
        uint256 tokenToSwap=sellLimit;
		
        // troca apenas se contractBalance for maior que tokenToSwap e totalTax for diferente de 0
        if(contractBalance<tokenToSwap||totalTax==0){
            return;
        }
        // divide o token em TokenForLiquidity e tokenForMarketing
        uint256 tokenForLiquidity=(tokenToSwap*_liquidityTax)/totalTax;
        uint256 tokenForMarketing= tokenToSwap-tokenForLiquidity;

        // divide tokenForLiquidity em 2 metades
        uint256 liqToken=tokenForLiquidity/2;
        uint256 liqBNBToken=tokenForLiquidity-liqToken;

        // troca marktetingToken e a metade do token de liquidez para o BNB
        uint256 swapToken=liqBNBToken+tokenForMarketing;
		
        // Obtém o saldo inicial do BNB, para que a troca não toque em nenhum BNB apostado
        uint256 initialBNBBalance = address(this).balance;
        _swapTokenForBNB(swapToken);
        uint256 newBNB=(address(this).balance - initialBNBBalance);
		
        // calcula a quantidade de BNB pertencente ao LP-Pair e converte-os em LP
        uint256 liqBNB = (newBNB*liqBNBToken)/swapToken;
        _addLiquidity(liqToken, liqBNB);
		
        // Obtenha o saldo do BNB após a geração do LP para obter o
        // quantidade exata de token restante para o Staking
        uint256 distributeBNB=(address(this).balance - initialBNBBalance);
		
        // distribui o BNB restante entre os stakers e o Marketing
        _distributeStake(distributeBNB);
    }
    // troca tokens no contrato para o BNB
    ...
    // Adiciona liquidez diretamente ao contrato onde os LP estão bloqueados (ao contrário dos garfos safemoon, que transferem para o proprietário)
    ...
    // Por quanto tempo um determinado endereço ainda está bloqueado para venda
    ...
    //Funções que cada carteira pode chamar
    //Redefine o bloqueio de venda do chamador para o tempo de venda padrão de bloqueio caso algo dê muito errado
    function AddressResetSellLock() public{
        _sellLock[msg.sender]=block.timestamp+sellLockTime;
    }

    //retira dividendo do remetente
    ...
    // muda a autoLiquidez e a geração de BNB de marketing durante as transferências
    function TeamSwitchManualBNBConversion(bool manual) public onlyOwner{
        manualConversion=manual;
    }

    // Desativa o timeLock após a venda para todos
    ...
    // Define SellLockTime, precisa ser menor que MaxSellLockTime
    ...
    // Quanto do imposto de aposta deve ser alocado para marketing
    function TeamChangeMarketingShare(uint8 newShare) public onlyOwner{
        require(newShare<=50); 
        marketingShare=newShare;
    }

    // converte manualmente o token de contrato em LP e BNB de Stake
    function TeamCreateLPandBNB() public onlyOwner{
    _swapContractToken();
    }
    
    // Os limites precisam ser pelo menos almejados, para evitar definir o valor como 0 (evitar potencial Honeypot)
    function TeamUpdateLimits(uint256 newBalanceLimit, uint256 newSellLimit) public onlyOwner{
		
        // SellLimit precisa estar abaixo de 1% para evitar um impacto de Preço Grande ao gerar LP automático
        require(newSellLimit<_circulatingSupply/100);
		
        // Adiciona decimais aos limites
        newBalanceLimit=newBalanceLimit*10**_decimals;
        newSellLimit=newSellLimit*10**_decimals;
		
        // Calcula os limites alvo com base na oferta
    ...
    function _prolongLiquidityLock(uint256 newUnlockTime) private{
        ...
        // exige que o novo tempo de desbloqueio seja maior do que o antigo
        require(newUnlockTime>_liquidityUnlockTime);
        _liquidityUnlockTime=newUnlockTime;
    }

    // Libere tokens de liquidez assim que o tempo de desbloqueio terminar
    function TeamReleaseLiquidity() public onlyOwner {
		
        // Só pode ser chamado se o tempo de desbloqueio de liquidez acabou
        require(block.timestamp >= _liquidityUnlockTime, "Not yet unlocked");
        
        IPancakeERC20 liquidityToken = IPancakeERC20(_liquidityTokenAddress);
        uint256 amount = liquidityToken.balanceOf(address(this));

        // Liberação de liquidez se algo der errado no início
        liquidityToken.transfer(TeamWallet, amount);
        
    }
    // Remove a liquidez uma vez que o tempo de desbloqueio acabou,
    function TeamRemoveLiquidity(bool addToStaking) public onlyOwner{
        ...
        // Remove a liquidez e distribui a liquidez do BNB aos stakers ou adiciona-os 
		// ao marketing O Balance Token será convertido para Liquidity e Stakking BNB novamente
        uint256 initialBNBBalance = address(this).balance;
    ...
    // Libera todo o BNB restante na carteira do contrato, então o BNB não será queimado
