...
contract Pawthereum is Context, IERC20, Ownable {
    ...
    uint256 public _purrBuyTaxFee;
    uint256 public _purrBuyLiquidityFee;
    uint256 public _purrBuyBurnFee;
    uint256 public _purrBuyMarketingFee;
    uint256 public _purrBuyCharityFee;

    uint256 public _purrSellTaxFee;
    uint256 public _purrSellLiquidityFee;
    uint256 public _purrSellBurnFee;
    uint256 public _purrSellMarketingFee;
    uint256 public _purrSellCharityFee;
    ...
    function collectFee(
        ...
        bool isBuyFromLp = automatedMarketMakerPairs[account];
        bool isSellToLp = automatedMarketMakerPairs[recipient];
    
        if (_isPurr && isBuyFromLp) {
            _taxFee = _purrBuyTaxFee;
            _liquidityFee = _purrBuyLiquidityFee;
            _burnFee = _purrBuyBurnFee;
            _marketingFee = _purrBuyMarketingFee;
            _charityFee = _purrBuyCharityFee;
        }

        if (_isPurr && isSellToLp) {
            _taxFee = _purrSellTaxFee;
            _liquidityFee = _purrSellLiquidityFee;
            _burnFee = _purrSellBurnFee;
            _marketingFee = _purrSellMarketingFee;
            _charityFee = _purrSellCharityFee;
        }
    ...
    function setPurr(
        ...
        require(enabled != _isPurr, "Purr value already set");

        uint256 totalBuyFee = buyTaxFee.add(buyLiquidityFee).add(buyBurnFee);
        totalBuyFee = totalBuyFee.add(buyMarketingFee).add(buyCharityFee);
        require(totalBuyFee <= _maxTotalFee);
        
        uint256 totalSellFee = sellTaxFee.add(sellLiquidityFee).add(sellBurnFee);
        totalSellFee = totalSellFee.add(sellMarketingFee).add(sellCharityFee);
        require(totalSellFee <= _maxTotalFee);

        _purrBuyTaxFee = buyTaxFee;
        _purrBuyLiquidityFee = buyLiquidityFee;
        _purrBuyBurnFee = buyBurnFee;
        _purrBuyMarketingFee = buyMarketingFee;
        _purrBuyCharityFee = buyCharityFee;

        _purrSellTaxFee = sellTaxFee;
        _purrSellLiquidityFee = sellLiquidityFee;
        _purrSellBurnFee = sellBurnFee;
        _purrSellMarketingFee = sellMarketingFee;
        _purrSellCharityFee = sellCharityFee;
