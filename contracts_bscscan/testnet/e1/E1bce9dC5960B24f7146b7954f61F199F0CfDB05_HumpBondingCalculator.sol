...
contract HumpBondingCalculator is IBondingCalculator {
    using FixedPoint for *;
    using SafeMath for uint256;

    IERC20 internal immutable HUMP;

    constructor(address _HUMP) {
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == address(HUMP)) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair(_pair).token1() == address(HUMP), "Invalid pair");
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**IERC20Metadata(address(HUMP)).decimals())).div(getTotalValue(_pair));
    }
}
