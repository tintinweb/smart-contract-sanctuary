...
contract ClockAuctionBase {
    ...
    address public NBMToken = 0xc8FdA3fF1dcD6aCDD4c8AcB209ce3d489f98784d;
        ...
        returns (uint256)
    {
        ...
        if (price > 0) {
            ...
            require(IERC20(NBMToken).transfer(seller, sellerProceeds));
        }

        uint256 bidExcess = _bidAmount - price;

        require(IERC20(NBMToken).transfer(msg.sender, bidExcess));
...
contract ClockAuction is Pausable, ClockAuctionBase, IClockAuction {
    ...
    constructor(address _nftAddress, uint256 _cut) public {
        ...
        //require(candidateContract.supportsInterface(InterfaceSignature_ERC721), "ABC");
        nonFungibleContract = IERC721(_nftAddress);
    }

    function withdrawBalance() external onlyOwner {
        address(uint160(owner)).transfer(address(this).balance);
        IERC20(NBMToken).transfer(owner, getBalance());
    }
    ...
    function getBalance() view public returns(uint256) {
        return IERC20(NBMToken).balanceOf(address(this));
    }
    ...
    function bid(uint256 _tokenId, uint256 _amount)
        ...
        whenNotPaused
    {
        // require(IERC20(NBMToken).transfer(address(this), _amount));
...
/// @title Clock auction modified for sale
contract SaleClockAuction is ClockAuction {

    bool public isSaleClockAuction = true;

    // Tracks last 5 sale price of gen0 sales
    ...
    function bid(uint256 _tokenId, uint256 _amount)
        override external
    {
        require(IERC20(NBMToken).transferFrom(msg.sender, address(this), _amount));
