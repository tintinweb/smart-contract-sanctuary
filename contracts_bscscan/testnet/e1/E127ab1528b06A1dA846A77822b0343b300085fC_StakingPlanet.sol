...
contract StakingPlanet is IERC721Receiver, Ownable {
     ...
     * add reward per reward token, only callable when the target nft token is disabled and not staked
     * @param tokenId The Nft to add rewards.
     * @param amounts The reward amounts as the same sequence as rewardsTokenAddresses.
     */
    function addBatchRewardsForNft( uint256 tokenId, uint256[] memory amounts ) 
        external 
        onlyOwner 
        notStaked(tokenId) 
    {
        require(whitelistedNftIds.contains(tokenId), "This Nft token is not allowed");
        require(amounts.length==rewardsTokenAddresses.length, "Invalid reward amount array");
        RewardPool storage pool = rewardPools[tokenId];
        for(uint i=0; i<rewardsTokenAddresses.length; i++){
            pool.rewards[rewardsTokenAddresses[i]] = pool.rewards[rewardsTokenAddresses[i]].add(amounts[i]);
            require(
                IERC20(rewardsTokenAddresses[i]).transferFrom(_msgSender(), address(this), amounts[i]),
                "NftStaking: failed to add funds to the reward pool"
            );
        }
    }
    ...
    function updatePeriod(uint256 tokenId, uint16 period) 
        external 
        onlyOwner
    {
        RewardPool storage pool = rewardPools[tokenId];
    ...
    /**
     * get unclaimed rewards array of a staker as the same sequence as the rewardsTokenAddresses array
     */
    function getRewardsOf(address staker)
        external
        view
        returns (uint256[] memory)
    {
        ...
        for(uint i=0; i<stakedTokens[staker].length(); i++){
            ...
            for(uint j=0; j<rewardsTokenAddresses.length; j++){
                rewards[j] = rewards[j] + computed[j].mul(uint256(100).sub(fee)).div(100);
            }
        }
        return rewards;
    }
    ...
    function claimRewardsOfNft(uint256 tokenId) external {
        ...
        uint256[] memory rewards = _computeRewardsOfNft(tokenId);
        uint256 lastClaimed = tokenInfos[tokenId].lastClaimedTimestamp;
    ...
    function _transferRewardsWithFees(address to, uint256[] memory rewards) private {
        ...
        for(uint i=0; i<rewards.length; i++){
            if(rewards[i]>0){
                ...
                remain = rewards[i].sub(reward);
                if(galaxyAddress != address(0)){
                    require(IERC20(rewardsTokenAddresses[i]).transfer(galaxyAddress, remain.mul(galaxyTax).div(fee)), "NftStaking: failed to transfer galaxy tax");
                }
                if(transportationAddress != address(0)){
                    require(IERC20(rewardsTokenAddresses[i]).transfer(transportationAddress, remain.sub(remain.mul(galaxyTax).div(fee))), "NftStaking: failed to transfer galaxy tax");
                    IStakingCargo(transportationAddress).onRewardReceived(rewardsTokenAddresses[i], remain.sub(remain.mul(galaxyTax).div(fee)));
                }
            }
        }
