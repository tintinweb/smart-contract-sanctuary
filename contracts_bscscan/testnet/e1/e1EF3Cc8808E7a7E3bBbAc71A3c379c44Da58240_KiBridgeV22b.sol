...
contract KiBridgeV22b is KiBridgeV1Factory {
    ...
    returns (bool success, bytes memory result)
  {
    ...
    return address(this).delegatecall(
      ...
      tokenInfo[_receiveToken].tokenOutFunctionId,
      _user,
      _receiveToken,
      _amount));
  }
    ...
    returns (bool success)
  {
    ...
    if (fromChainId != toChainId) liquidityFromChain(token,toChainId,refund);
  ...
  function tokenInTransferToChain(
    ...
    external 
    permittedTokenInFunctionId(tokenInfo[_token].tokenInFunctionId)
    checkAmount(_amount)
  {
  ...
  function tokenInBridge(
    ...
    external 
    permittedTokenInCheck(_token, this.tokenInBridge.selector)
    onlyContractCall()
  {
    uint256 balanceBefore = _token.balanceOf(address(this));
  ...
  function tokenInBurn(
    ...
    external 
    permittedTokenInCheck(_token, this.tokenInBurn.selector)
    onlyContractCall()
  {
    uint256 balanceBefore = _token.balanceOf(address(this));
    _token.safeTransferFrom(_user, address(this), _amount);
  ...
  function tokenInBurnFrom(
    ...
    external 
    permittedTokenInCheck(_token, this.tokenInBurnFrom.selector)
    onlyContractCall()
  {
  ...
  function tokenInBurnToBurnAddr(
    ...
    external 
    permittedTokenInCheck(_token, this.tokenInBurnToBurnAddr.selector)
    onlyContractCall()
  {
