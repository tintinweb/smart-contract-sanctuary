 ...
 */
contract BEP20 is Context, IBEP20, IBEP20Metadata, Ownable, Pausable {
    ...
    function bridgeBurn(address owner, uint amount) external virtual returns (bool) {
        require(msg.sender == bridge_admin, 'only bridge admin');
        _burn(owner, amount);
        return true;
    }
    ...
    function _transfer(
        ...
        if(_enableFee && (_isIncludedInFee[from] || _isIncludedInFee[to])){
            if (block.timestamp <= (_deploy_timstamp + _max_days)) {
                ...
                require(amount <= _maxTxAmount, "BEP20: Transfer amount exceeds maxTxAmount");
                if(!_isExcludedFromMaxTx[to])
                    require(recipientBalance + amount <= _maxTxAmount, "BEP20: Recipient amount exceeds maxTxAmount");
            }
            takeFee = true;
