...
contract ProductCar is Ownable,Initializable,Permission{
    ...
    //event evtMintMyCar(address indexed,string carType,uint256 tokenId);
    ...
    function mintMyCar(address to,uint256 tokenId,string calldata _carType, Model.carProperty[] memory carProps)  public onlyRole(MINTER_ROLE) virtual{
         nftCar.mint(to, tokenId);
         bool b = appConf.mintMyCar(tokenId,_carType,carProps);
         if(b){
             evtMintCar(to,_carType,tokenId);
         }
    }
