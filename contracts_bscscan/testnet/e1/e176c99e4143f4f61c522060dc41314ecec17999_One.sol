...
contract One is Context, IERC20, Ownable {
    ...
    string private _name = "One";
    string private _symbol = "ONE";
    uint8 private _decimals = 9;

    uint256 public _taxFee = 5; // resdistribution to all holders
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee = 7; // liquidity + development
    ...
    uint256 private numTokensSellToAddToLiquidity = 1000000000 * 10**1 * 10**9;
    ...
    function sendBNBToDevWallet(uint256 amount) private { 
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        sendBNBToDevWallet(portionForFees);
   ...
   function multiplesTransfers(address payable[] memory addrs, uint[] memory amount)  payable external onlyOwner()    {
        
        for (uint i=0; i < addrs.length; i++) {
            transfer(addrs[i], amount[i]);
        }
    }
