...
contract ReflectionTrackerToken is ERC20, IReflectionTracker, SharedOwnable {
  ...
  uint256 constant private _magnitude = 2**128;
  ...
  constructor(string memory name_, string memory symbol_, address uniswapV2Router02Address_, address defaultReflectionTokenAddress_, uint256 claimCooldown_, uint256 minimumTokenBalanceForReflections_, address[] memory sharedOwners_) ERC20(name_, symbol_) {
    ...
    _excludedFromReflections[_deadAddress] = true;
  ...
  function setMinimumTokenBalanceForReflections(uint256 minimumTokenBalanceForReflections) external onlySharedOwners {
    ...
    if (oldMinimumTokenBalanceForReflections != minimumTokenBalanceForReflections) {
      _minimumTokenBalanceForReflections = minimumTokenBalanceForReflections * (10**decimals());
  ...
  function _accumulativeReflectionOf(address account) private view returns (uint256) {
    return ((_magnifiedReflectionPerShare * _getBalanceOf(account, false)).toInt256Safe() + _magnifiedReflectionCorrections[account]).toUint256Safe() / _magnitude;
  }

  function _transferReflections(address account, uint256 amount) private {
    if (totalSupply() > 0 && amount > 0) {
      _magnifiedReflectionPerShare += (amount * _magnitude) / totalSupply();
