...
import "./ICartographer.sol";
import "./ICartographerOasis.sol";
import "./ICakeMasterChef.sol";

import "hardhat/console.sol";

contract CartographerOasis is Ownable, Initializable, ReentrancyGuard, ICartographerOasis {
    ...
    constructor(address _ICartographer) public {
        require(_ICartographer != address(0), "Cartographer required");
        cartographer = ICartographer(_ICartographer);
    }

    function enableSummit(uint256 _genTime) external onlyCartographer() override {
    ...
    function add(uint8 _pid, bool _live, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate, uint256 _cakeChefPid) external onlyCartographer() nonDuplicated(_lpToken) override {
        if (_withUpdate) {
    ...
    function set(uint8 _pid, bool _live, uint16 _depositFeeBP, bool _withUpdate) external onlyCartographer() poolExists(_pid) override {
    ...
    // INTERFACE
    function getPendingSummit(uint8 _pid, address _userAdd) public view poolExists(_pid) validUserAdd(_userAdd) override returns (uint256) {
        OasisPoolInfo storage pool = oasisPoolInfo[_pid];
    ...
    function getLpSupply(uint8 _pid) external view poolExists(_pid) override returns (uint256) {
        return oasisPoolInfo[_pid].lpSupply;
    }
    function getLpToken(uint8 _pid) external view poolExists(_pid) override returns (IBEP20) {
        return oasisPoolInfo[_pid].lpToken;
    }
    function getDepositFee(uint8 _pid) external view poolExists(_pid) override returns (uint256) {
        return oasisPoolInfo[_pid].depositFeeBP;
    }
    function getIsLive(uint8 _pid) external view poolExists(_pid) override returns (bool) {
        return oasisPoolInfo[_pid].live;
    }
    
    function massUpdatePools() external onlyCartographer() override {
    ...
    function deposit(uint8 _pid, uint256 _amount, address _userAdd) external nonReentrant onlyCartographer() poolExists(_pid) validUserAdd(_userAdd) override returns (uint256) {
        OasisPoolInfo storage pool = oasisPoolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_userAdd];
    ...
    function switchElevationValidate(uint8 _pid, address _lpToken, uint256 _cakeChefPid, address _userAdd) external nonReentrant onlyCartographer() poolExists(_pid) validUserAdd(_userAdd) override {
    ...
    function switchElevationDeposit(uint8 _pid, uint256 _amount, address _userAdd) external nonReentrant onlyCartographer() override returns (uint256) {
        OasisPoolInfo storage pool = oasisPoolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_userAdd];        
    ...
    function withdraw(uint8 _pid, uint256 _amount, address _userAdd) external nonReentrant onlyCartographer() poolExists(_pid) validUserAdd(_userAdd) override returns (uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];
    ...
    function switchElevationWithdraw(uint8 _pid, uint256 _amount, address _userAdd) external nonReentrant onlyCartographer() poolExists(_pid) validUserAdd(_userAdd) override returns (address, uint256, uint256) {
        UserInfo storage user = userInfo[_pid][_userAdd];       
    ...
    function emergencyWithdraw(uint8 _pid, address _userAdd) external nonReentrant onlyCartographer() poolExists(_pid) validUserAdd(_userAdd) override returns (uint256) {
        OasisPoolInfo storage pool = oasisPoolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_userAdd];
...
interface ICartographer {
    function summitPerBlock() external view returns (uint256);
    function totalAllocPoint() external view returns (uint256);
    function enableTokenAtElevation(IBEP20 lpToken, uint8 elevation) external;
    function disableTokenAtElevation(IBEP20 lpToken, uint8 elevation) external;
    function getSummitReward(uint256 lastRewardBlock, IBEP20 lpToken, uint8 elevation) external view returns (uint256);

    function depositTokenManagement(address userAdd, IBEP20 lpToken, uint256 depositFeeBP, uint256 amount, uint256 cakeChefPid) external returns (uint256);
    function withdrawalTokenManagement(address userAdd, IBEP20 lpToken, uint256 amount, uint256 cakeChefPid) external;
    function setPassthroughTokenManagement(uint256 cakeChefPid, IBEP20 lpToken, uint256 initialAmount) external;
    function disablePassthroughTokenManagement(uint256 amount, uint256 cakeChefPid) external;
    function updatePoolMint(uint256 lastRewardBlock, IBEP20 _lpToken, uint8 _elevation) external returns (uint256);
    function redeemRewards(address userAdd, uint256 amount) external;
...
interface ICartographerOasis {
    function enableSummit(uint256 _genTime) external;
    
    function add(uint8 _pid, bool _live, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate, uint256 _cakeChefPid) external;
    function set(uint8 _pid, bool _live, uint16 _depositFeeBP, bool _withUpdate) external;
    function getPendingSummit(uint8 _pid, address _user) external view returns (uint256);
    function getLpSupply(uint8 _pid) external view returns (uint256);
    function getLpToken(uint8 _pid) external view returns (IBEP20);
    function getDepositFee(uint8 _pid) external view returns (uint256);
    function getIsLive(uint8 _pid) external view returns (bool);
    function massUpdatePools() external;
    function deposit(uint8 _pid, uint256 _amount, address userAdd) external returns (uint256);
    function switchElevationWithdraw(uint8 _currentPid, uint256 _amount, address _userAdd) external returns (address, uint256, uint256);
    function switchElevationValidate(uint8 _pid, address _lpToken, uint256 _cakeChefPid, address _userAdd) external;
    function switchElevationDeposit(uint8 _newPid, uint256 _amount, address _userAdd) external returns (uint256);
    function withdraw(uint8 _pid, uint256 _amount, address userAdd) external returns (uint256);
    function emergencyWithdraw(uint8 _pid, address userAdd) external returns (uint256);
}
