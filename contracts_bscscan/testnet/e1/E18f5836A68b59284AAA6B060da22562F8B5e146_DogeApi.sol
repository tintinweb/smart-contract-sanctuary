...
contract DogeApi{
  ...
  function stakedDetails(address owner) public view returns(Detail[] memory sheeps, Detail[] memory doges){
      (uint256 amount, uint256 sheepCount, uint256 dogeCount) = getStakedCount(owner);
      sheeps = new Detail[](sheepCount);
      doges = new Detail[](dogeCount);
      uint sheepIndex;
      uint dogeIndex;
      for(uint i = 0; i < amount; i++){
          uint256 _id = barn.tokenOfOwnerByIndex(owner, i);
          Detail memory animal = Detail({
              tokenId: _id,
              tokenURI : doge.tokenURI(_id)
            });
          if(isSheep(_id)) {
            sheeps[sheepIndex] = animal;
            sheepIndex++;
          } else {
            doges[sheepIndex] = animal;
            dogeIndex++;
          }
      }
  }
  ...
  function stakedDogeIds(address owner) public view returns(uint256[] memory ids){
      (,, uint256 doges) = getStakedCount(owner);
      if(doges == 0){
        return ids;
      }
      ids = new uint256[](doges);
      for(uint i = 0; i < doges; i++){
        uint256 _id = barn.tokenOfOwnerByIndex(owner, i);
  ...
  function stakedSheepIds(address owner) public view returns(uint256[] memory ids){
      (, uint256 sheeps,) = getStakedCount(owner);
      if(sheeps==0) {
        return ids;
      }
