...
contract WhaleNetworkGovernance is Context, Ownable {
    ...
    function voteOnProposal(uint256 proposalId, uint256 vote)external returns(bool){
        ...
        Voter[] storage voters = _votes[proposalId];
        ...
        if(vote==1){
            voters.push(Voter(_msgSender(), Vote.YES));
            _votes[proposalId] = voters;
        }else{
            voters.push(Voter(_msgSender(), Vote.YES));
            _votes[proposalId] = voters;
        }
    ...
    function getVote(uint256 proposalId, address addr)public view returns(Voter memory){
        require( _proposalVotingStatus[proposalId][addr], "You have not voted on this proposal");
        Voter[] memory votes =  _votes[proposalId];
        for(uint256 i =0; i< votes.length; i++){
            if(votes[i].voter == addr){
                return votes[i];
            }
        }
        return Voter(addr, Vote.NOT_VOTED);
    }
}