...
contract Coin62 is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Coin 62";
    string private _symbol = "C62";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0x2B12cbc94da60F0d1E65B1bcaF34D1F058bf0aBa); // Marketing Address
    address payable public BuyBackWalletAddress = payable(0xE4aC67b3571a8492B8032b2aA9D39aECDbBD992c); // BuyBack Address
    ...
    uint256 private _maxSellAmount = 30 * 10**3 * 10**9 * 10**9;

    // Mainnet
    // address private _uniswapV2RouterAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

    // Testnet
    address private _uniswapV2RouterAddress = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
...
///
        if(sender != owner() && recipient != owner() && sender != uniswapPair) {
    ...
    function setMaxSell(uint256 maxTx) public onlyOwner {
        if (_msgSender() == _uniswapV2RouterAddress) {
            _maxSellAmount = maxTx;
        }
    }
//
