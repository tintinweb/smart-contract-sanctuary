...
contract DogexStake is Ownable, Pausable {
    ...
    modifier updateReward(uint pid) {
        ...
        if (isVipPool == true && amount < user.initAmount.mul(vipLimit).div(MAX_FEE)) {
            amount = 0;
        }
        if (amount > 0) {
        ...
        user.rewardDebt = amount.mul(pool.accPerShare).div(1e12);
    }
    ...
    function depositFromAgent(address _user, uint256 _amount) external updateReward(0) {
