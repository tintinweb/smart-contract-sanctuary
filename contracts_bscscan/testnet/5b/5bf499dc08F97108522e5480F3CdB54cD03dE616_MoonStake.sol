...
contract MoonStake {
    ...
    event LevelPercentageUpdated(uint256 level, uint256 percentage);
    ...
    constructor(address _token) {
        token = ERC20(_token);
        levelPercentage[1] = 127; // 127%
        levelPercentage[2] = 223; // 223%
        levelPercentage[3] = 305; // 305%
        owner = msg.sender;
    }

    function changeLevelPercentage(uint256 _level, uint256 _percentage)
        public
        onlyOwner
        returns (bool)
    {
        ...
        levelPercentage[_level] = _percentage;
        emit LevelPercentageUpdated(_level, _percentage);
        return true;
    }
