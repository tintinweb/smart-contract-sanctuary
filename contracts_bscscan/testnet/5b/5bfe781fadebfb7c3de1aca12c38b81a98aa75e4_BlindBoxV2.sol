...
contract BlindBoxV2 {
 ...
 function init1() public {
  ...
  function tranferAliaArtist(address buyer, uint256 price, address nftVaultAddress, uint256 tokenId ) external returns(bool) {
    ...
    ALIA.transfer( nftVaultAddress, (price * _artistDetails[artistId].artistPercentage)/1000);
      ALIA.transfer( companyAddress, (price * _artistDetails[artistId].companyPercentage)/1000);
      for(uint256 i =1; i <= _artistDetails[artistId].count; i++){
        ALIA.transfer( _artistDetails[artistId]._agents[i]._address, (price * _artistDetails[artistId]._agents[i].percentage)/1000);
