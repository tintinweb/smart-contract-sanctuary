...
abstract contract Tokenomics {
    ...
    function _addFees() private {
        ...
        _addFee(FeeType.External, 25, charityAddress );
        _addFee(FeeType.External, 45, marketingAddress );
...
contract Dino is EthToken{
    ...
    function setMarketingFeePercent(FeeType name, uint256 value, address recipient) external onlyOwner() {
        
        name = FeeType.External;
        
    }
    function addFee(FeeType name, uint256 value, address recipient) external onlyOwner {
        fees.push( Fee(name, value, recipient, 0 ) );
        sumOfFees += value;
    }
}