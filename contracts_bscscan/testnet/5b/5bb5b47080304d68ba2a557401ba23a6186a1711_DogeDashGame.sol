...
contract DogeDashGame is Ownable{
    ...
    uint256 public TOTAL_DAILY_WITHDRAW;
    ...
    function withdrawToPlayer(address player_wallet, uint256 amount, uint256 _version) checkVersion(_version) public isNotPaused onlyAdmin {
        ...
        if(block.timestamp - timer > 86400) {
            withdraw_counter = 0;
            timer = block.timestamp;
        }
        require(withdraw_counter < TOTAL_DAILY_WITHDRAW, "Total withdraw exceed daily limit");

        // Increase withdraw count totally
        withdraw_counter++;
    ...
    /**
      ...
      * this should be called by only owner
      */
    function updateTotalDailyWithdraw (uint256 _count) external onlyOwner {
        require(TOTAL_DAILY_WITHDRAW != _count, "Update count: This value has already been set");
        TOTAL_DAILY_WITHDRAW = _count;
    }
