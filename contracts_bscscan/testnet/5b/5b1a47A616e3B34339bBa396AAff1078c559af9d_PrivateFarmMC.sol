...
// bYield Token with Governance.
contract BYieldToken is BEP20('BYield Token', 'bYield') {
...
contract Presale is Ownable, ReentrancyGuard {
    ...
    // All Presale Holders are entitled to 60% of their presale commitments
    // For Private Farming
    ...
    event WithdrawUnsoldBYIELD(uint256 blockNumber, uint256 amount);
    event WithdrawFunds(address indexed owner, uint256 amount);

    constructor (address _BYIELD, address _BUSD, uint256 _startBlock, uint256 _endBlock) public {
    ...
    function buy (uint256 _amount) external nonReentrant {
        ...
        // Require that Presale Contract has 50,000 bYield.
        require(BYIELD.balanceOf(address(this)) == 50000 ether, "Request Owner to have sufficiently transfer enough tokens to Presale Contract");
        ...
        require (tokensOwned[_buyer].add(tokens) <= 1000 ether, "Max limit of 10,000 BUSD reached");
        
        // Ensure 500,000 BUSD Hardcap is not exceeded
        require (busdReceived.add(_amount) <= 500000 ether, "Presale hardcap reached");

        // Ensure that Presale has began
        require(block.number >= startBlock, "Presale has not started");

        BUSD.safeTransferFrom(_buyer, address(this), _amount);
    ...
    function claimTokens (address claimer) external {
        require (block.number >= endBlock, "Presale has not end yet!");
    ...
    // Unsold BYIELD will be burned 1 week after Presale has ended to provide everyone a reasonable chance to claim 
    function withdrawUnsoldBYIELD() external onlyOwner {

        // Presale Holders given 7days to Claim before all unclaimed all burned 
        // 7 days is equal to 201600 Blocks
        uint256 burnBlock = endBlock.add(201600);
        require(block.number >= burnBlock, "Burning tokens too prematurely!");
        uint256 amount = BYIELD.balanceOf(address(this));
        BYIELD.safeTransfer(BURN_ADDRESS, amount);

        emit WithdrawUnsoldBYIELD(block.number, amount);
    }
...
contract PrivateFarmMC is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accByieldPerShare;   // Accumulated BYIELD per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The BYIELD TOKEN!
    BYieldToken public immutable byield;
    // Dev address.
    ...
    // Maximum Emission Rate set as 10 per block
    ...
    uint256 public constant PRIVATE_FARM_LOCKUP_PERIOD = 200;
    ...
    function pendingByield(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accByieldPerShare = accByieldPerShare.add(byieldReward.mul(1e18).div(pool.lpSupply));
        }
        return user.amount.mul(accByieldPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accByieldPerShare = pool.accByieldPerShare.add(byieldReward.mul(1e18).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0 && block.number >= harvestUnlockBlock) {
            uint256 pending = user.amount.mul(pool.accByieldPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accByieldPerShare).div(1e18);
    ...
    function updateEmissionRate(uint256 _byieldPerBlock) public onlyOwner {
        massUpdatePools();
        require(_byieldPerBlock <= MAX_EMISSION_RATE,"Too high");
