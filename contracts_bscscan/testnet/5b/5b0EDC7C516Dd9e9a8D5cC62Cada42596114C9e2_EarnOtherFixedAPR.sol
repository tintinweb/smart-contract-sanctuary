...
contract EarnOtherFixedAPR is Ownable, ReentrancyGuard {
  ...
  /// @dev accumulate stake token from all users
  ...
  constructor(
    ...
    cap = _cap;
    apr = _apr;
    tokenRatio = _tokenRatio; // (stakeToken/reward)
  ...
  */
  ...
  /// @dev 1. Validate amount > 0, cannot deposit after end block and remaining still > 0
  /// @dev 2. Auto Harvest if position existed
  /// @dev 3. If remaining less than input amount, use remaining instead
  function deposit(uint256 _amount) external nonReentrant {
    require(block.number < endBlock, "EarnOther::deposit:: not allow after end period");
    require(_amount > 0, "EarnOther::deposit:: not allow zero amount");
    require(totalStaked < cap, "EarnOther::deposit:: pool capacity is reached");
    ...
    rewardDebt = rewardDebt.add(
      _actualAmount.mul((endBlock.sub(_lastRewardBlock())).mul(rewardPerBlock)).div(REWARD_PER_BLOCK_PRECISION_FACTOR)
    );
  ...
  /// @notice calculate the reward ready to claim
  ...
  /// @notice Withdraw available once distribution reward is ended, so there're no partially withdraw
  ...
  */
  ...
  ///-------------------------------------------------------------------///
  */
  function _calculateReward(uint256 _amount, uint256 _calculateFromBlock) internal view returns (uint256 rewards) {
