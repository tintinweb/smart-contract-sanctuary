...
contract ExchangeRateMechanism is OwnableUpgradeable {
    ...
    bool public addressesInitialized;
    bool public sharesInitialized;
    ...
    function initializeAddresses(
        ...
        require(!addressesInitialized, "Addresses already initialized");
    ...
    function initializeShares(
        ...
        require(!sharesInitialized, "Shares already initialized");
    ...
    function _increaseRate(uint256 reservesDiff) private {
        uint256 busdToSwap = reservesDiff / 2;
    ...
    function _swapTokens(address tokenA, address tokenB, uint256 amountIn) private returns (uint256) {
        ...
        if (amountToSwap > balanceA) {
            amountToSwap = balanceA;
        }

        address[] memory path = new address[](2);
        path[0] = tokenA;
        path[1] = tokenB;
        IERC20(tokenA).approve(address(apeRouter), type(uint256).max);
        ...
        return IERC20(tokenB).balanceOf(address(this)) - initialBalanceB;
    }
