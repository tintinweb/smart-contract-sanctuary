...
contract TreatDAOContract is ERC20, Ownable {
    ...
    address[] public nontaxedContracts;
    mapping(address => bool) public isAddressNotTaxed;
    ...
    */
    function calculateTokenFee(
        ...
        bool isTaxed = true;
        for(uint i=0; i < nontaxedContracts.length; i++) {
            if(nontaxedContracts[i] == _from) {
                isTaxed = false;
            }
        }
        if(!isTaxed) {
    ...
    function addNonTaxedContract(address nontaxed) public {
        bool isAddedAlready = false;
        for(uint i=0; i< nontaxedContracts.length; i++) {
            if(nontaxedContracts[i] == nontaxed) {
                isAddedAlready = true;
            }
        }
        require(!isAddedAlready, "Address is already not taxed");
        nontaxedContracts.push(nontaxed);
        isAddressNotTaxed[nontaxed] = true;
    }

    function removeNonTaxedContract(address taxed) public {
        for (uint256 i=0; i < nontaxedContracts.length; i++) {
            if(nontaxedContracts[i] == taxed) {
                nontaxedContracts[i] = nontaxedContracts[nontaxedContracts.length - 1];
                nontaxedContracts.pop();
                isAddressNotTaxed[taxed] = false;
            }
        }
    }
