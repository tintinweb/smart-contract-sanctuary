...
// BTCC
 ...
 function addNewEmploy(address newEmploy, string memory employType) public onlyOwner {
      ...
      emit addEmploy(newEmploy, employType);
  }
  ...
  function swapLuni(uint256 amountLuni) public {
    ...
    uint256 swapUSDTAmountValid = _getValidSwap(msg.sender, true);
  ...
  function getLuniAvailableToSwap(address _employ) public returns (uint256) {
    if (bytes(employ[_employ].employType).length == 0) {
      return (0);
    }
    uint256 swapUSDTAmountValid = _getValidSwap(_employ, false);
    return(swapUSDTAmountValid.mul(usdtRate).div(1e18));
  }

  function _getValidSwap(address _employ, bool isReset) private returns (uint256) {
    ...
    if (lastSwapTimeStamp == 0) {
      vaildAmount = swapAmountMax;
        ...
        if (currentMonth == lastMonthSwap) {
          vaildAmount = swapAmountMax.sub(employ[_employ].amountSwap);
        } else {
          if (isReset) {
            employ[_employ].amountSwap = 0;
          }
          vaildAmount = swapAmountMax;
        }
    }
