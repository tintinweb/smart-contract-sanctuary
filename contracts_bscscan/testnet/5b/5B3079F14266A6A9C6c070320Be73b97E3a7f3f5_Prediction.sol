...
contract Prediction is Owned, Pausable {
    ...
    event NewTicket(uint256 indexed pid, uint256 indexed epoch, uint8 _tid, bool indexed label, uint256 price);
    ...
    function claimable(uint256 _pid, uint256 _epoch, uint8 _tid, address _user) public view returns (bool) {
        ...
        return specification.oracleCalled &&
                ((round.closePrice > round.lockPrice && ticket.label == true) ||
                (round.closePrice < round.lockPrice && ticket.label == false) );
    }
    ...
    function _safeStartRound(uint256 _pid, uint256 _epoch, uint256 _price) internal {
        ...
        require(rounds[_pid][_epoch - 1].status == Status.Lock, "PREDICTION: Can only start round after round n-1 has lock");
    ...
    function _lockRound(uint256 _pid, uint256 _epoch, uint256 _price) internal {
        ...
        if (
            round.lockPrice == round.closePrice ||  // House win
            (round.lockPrice > round.closePrice && specification.blueTicketNumber == pool.ticketNumberInit) || // Bule win but haven't blue ticket
            (round.lockPrice < round.closePrice && specification.redTicketNumber == pool.ticketNumberInit)  // Red win but haven't red ticket
        ) {
            round.status == Status.Lock;
            specifications[_pid][_epoch].oracleCalled = true;
        } else {
    ...
    function _buyTicket(uint256 _pid, bool _label, address _buyer, bool _initial, address _referral) internal returns(uint256 _price) {
        ...
        tokenReward.transferFrom(_buyer, address(this), price);
        ...
        emit NewTicket(_pid, _currentEpoch, tid, _label, price);
        return (price);
    }
    
    function _split(uint256 _randomNumber, uint256 _roundId, uint256 _epoch) internal view returns(uint16) {
        Round memory round = rounds[_roundId][_epoch];
        Specification memory specification = specifications[_roundId][_epoch];
        // Encodes the random number with its position in loop
        bytes32 hashOfRandom = keccak256(abi.encodePacked(_randomNumber, _roundId));
        // Casts random number hash into uint256
        uint256 numberRepresentation = uint256(hashOfRandom);
        
        bool result = round.lockPrice > round.closePrice;
        // Calculation range
