...
contract Locked is Pausable, AccessControl {
    ...
    IERC20 public token;
    uint256 public basePoint = 500 * 1e18;
    uint256 public maxTime = 10 minutes;
    ...
    function getNumerTicket(address _user) public view returns (uint256) {
    ...
    function locked(uint256 amount) public whenNotPaused {
        ...
        if (user.amount > 0) {
            user.lastTicket = getNumerTicket(msg.sender);
        }
    ...
    function unLock() public whenNotPaused {
        ...
        require(user.timeLocked + maxTime >= block.timestamp, "Locked");
        if (user.amount > 0) {
            user.lastTicket = getNumerTicket(msg.sender);
        }
    ...
    function useTicket(address account, uint256 amount)
        ...
        returns (bool)
    {
        uint256 ticket = getNumerTicket(account);
