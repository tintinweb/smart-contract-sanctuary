...
contract NewEraFloki is IERC20, Ownable {
    using SafeMath for uint256;

    address constant mainnetRouter = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; // TestNet
    //address constant mainnetRouter = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1; // TestNet
    //address constant mainnetRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address constant WBNB          = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address constant DEAD          = 0x000000000000000000000000000000000000dEaD;
    address constant ZERO          = 0x0000000000000000000000000000000000000000;

    string _name = "New Era Floki";
    string _symbol = "NewEraFloki";
    uint8 constant _decimals = 18;

    uint256 private _decimalFactor = 10 ** _decimals;
    uint256 public _totalSupply = 100000000000 * _decimalFactor;    // 100,000,000,000
    uint256 public _maxTxn = 2 * (_totalSupply / 100);       // 2% of total supply
    uint256 public _maxWalletSize = 2 * (_totalSupply / 100); // 2% of total supply
    ...
    uint256 marketingFee = 0;  // 8%
    uint256 liquidityFee = 0;  // 2%
    uint256 buybackFee = 0;   // 4%
    uint256 totalFee = 0;     // 14%
    ...
    constructor () {
        ...
        _maxWalletSize = _totalSupply / 400; // 0.25% of total supply at launch to prevent whales
    ...
    function triggerManualBuyback(uint256 amountBNB) external onlyOwner {
        buyTokens(amountBNB, liquidityFeeWallet);
    }
    ...
    function changeMaxTxn(uint256 percent) external onlyOwner { 
        require(percent >= 2, "Max tx must be 2% or greater");
        _maxTxn = _totalSupply.mul(percent).div(100);
    }
    
    function changeMaxWallet(uint256 percent) external onlyOwner {
        require(percent >= 2, "Max wallet must be 2% or greater");
    ...
    function AntiSnipeOn() internal {
        liquidityFee = 0;
        buybackFee = 0;
        marketingFee = 99; // 99% antisnipe fee during launch 
        totalFee = 99;
    }
