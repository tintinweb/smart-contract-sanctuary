...
contract MinoWarBox is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Strings for uint256;
    using EnumerableSet for EnumerableSet.UintSet;
    IMinoCore public minoNFT;
    IERC20 minoToken;
    ...
    constructor(
        ...
        minoNFT = IMinoCore(_minoNFT);
        feeWallet = _feeWallet;
        saleWallet = _saleWallet;
        randManager = RandInterface(_randManager);
        minoToken = _minoToken;
        saleBoxs[0] = SaleBox(0, 40 * 10**18, 1629813629, 1661349629, 5, 25);
        saleBoxs[1] = SaleBox(1, 30 * 10**18, 1629813629, 1661349629, 2, 20);
        saleBoxs[2] = SaleBox(2, 20 * 10**18, 1629813629, 1661349629, 1, 15);
    }
    ...
    function setPercentBox(uint256 typeBox, uint256 thirdStarPercent, uint256 secondStarPercent) public onlyOwner {
    ...
    function _buyNFT(uint256 _typeBox, uint256 _amount) internal {
        ...
        require(
            ...
            "Amount of MINO sent is not correct."
        );
        require(
            minoToken.allowance(msg.sender, address(this)) >= _amount,
            "Token allowance too low"
        );
        require(msg.value == feeSale, "Amount of BNB sent is not correct.");
        minoToken.transferFrom(msg.sender, saleWallet, _amount);
        if (feeSale > 0) {
            feeWallet.transfer(feeSale);
        }
    }

    function _randBlindbox(uint256 _typeBox, uint256 _amount) internal {
        randManager.randMod(_msgSender());
        uint256 starRand = randManager.currentRandMod();
        uint256 starBox;
        uint256 characterRand;
        if (starRand <= saleBoxs[_typeBox].thirdStarPercent) {
            starBox = 3;
            characterRand = 13;
        }

        if (starRand > saleBoxs[_typeBox].thirdStarPercent && starRand <= saleBoxs[_typeBox].secondStarPercent) {
            starBox = 2;
            characterRand = 10;
        }

        if (starRand > saleBoxs[_typeBox].secondStarPercent) {
            starBox = 1;
            randManager.randModEight(_msgSender());
            characterRand = randManager.currentRandMod();
            if (characterRand <= 15) {
                characterRand = 0;
            }
            if (characterRand > 15 && characterRand <= 25) {
                characterRand = 1;
            }
            if (characterRand > 25 && characterRand <= 35) {
                characterRand = 2;
            }
            if (characterRand > 35 && characterRand <= 45) {
                characterRand = 3;
            }
            if (characterRand > 45 && characterRand <= 55) {
                characterRand = 4;
            }
            if (characterRand > 55 && characterRand <= 65) {
                characterRand = 5;
            }
            if (characterRand > 65 && characterRand <= 75) {
                characterRand = 6;
            }
            if (characterRand > 75) {
                characterRand = 7;
        ...
        uint256 tokenId = minoNFT.getNextNFTId();
        minoNFT.safeMintNFT(_msgSender(), tokenId);
        Mino memory mino = Mino(
            tokenId,
            1,
            1,
            starBox,
        ...
        minoNFT.setNFTFactory(mino, tokenId);
        minoNFT.setNFTForUser(mino, tokenId, _msgSender());
        emit Sale(tokenId, _msgSender(), _amount, _typeBox, randManager.currentRandMod());
    }
...
interface RandInterface {
    ...
    function randModEight(address userAddress) external returns(uint);
}
