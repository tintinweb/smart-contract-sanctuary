...
contract TransmuterB is ContextUpgradeable {
    ...
    address public rewards;
     ...
     address public alchemistAddress;
    ...
    event StabletokenAddressUpdated(
        address updatedtoken
    );
	function setAlTokenAddress(address _alToken) public  onlyGov{    
        alToken = _alToken;     
        emit  AlTokenAddressUpdated(_alToken);
    }

    function setStabletokenAddress(address _token) public  onlyGov{     
        token = _token;
        emit  StabletokenAddressUpdated(_token);
    }
    ...
    function setAlchemistAddress(address _alchemistaddress) public onlyGov(){
        alchemistAddress = _alchemistaddress;
    ...
    function distribute(address origin, uint256 amount) public onlyWhitelisted() runPhasedDistribution() {
        require(!pause, "emergency pause enabled");
        require(msg.sender == alchemistAddress,"Caller is not an AlchemistAddress");
...
contract YearnVaultAdapterWithIndirection is YearnVaultAdapter {
    ...
    using SafeMathUpgradeable for uint256;
    //address public override governence;
    
    // function Initialize()public  override initializer{
    //   governence = msg.sender;    
    // }

    function setAdapter(IyVaultV2 _vault, address _admin) public  {     
      YearnVaultAdapter.setter(_vault, _admin);
    }
...
contract YearnVaultAdapter is IVaultAdapter,Initializable {
  ...
  function setter(IyVaultV2 _vault, address _admin) public {
    require(msg.sender==governence,"YVA:caller is not an governence");
    vault = _vault;
    admin = _admin;
    updateApproval();
  ...
  function withdraw(address _recipient, uint256 _amount) external override onlyAdmin {
	...
	address busdToken =vault.token();
	 // transfer all the busd in adapter to yum
    IDetailedERC20(busdToken).transfer(_recipient,IDetailedERC20(busdToken).balanceOf(address(this)));
  }
