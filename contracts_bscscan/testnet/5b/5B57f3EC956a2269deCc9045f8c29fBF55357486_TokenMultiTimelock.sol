...
contract TokenMultiTimelock is Ownable {
    ...
    function infoWallet(address wallet) external view virtual returns (string memory unlockedText, string memory claimedText, string memory claimableText) {
        ...
        unlockedText = string(abi.encodePacked(whole, ".", fract, " / "));
        ...
        claimedText = string(abi.encodePacked(whole, ".", fract, " ", _token.symbol(), " Claimed already"));        

        whole = _uintToStr(claimable.div(10 ** decimals));
        fract = _uintToStr(claimable.sub(claimable.div(10 ** decimals).mul(10 ** decimals)));
        
        claimableText = string(abi.encodePacked(whole, ".", fract, " ", _token.symbol(), " Claimable now"));         
