...
contract Advisor is AccessControl {
    ...
    function lockToken() public {
        ...
        lockBool = true;
        tokenImba.safeTransferFrom(address(msg.sender), address(this), lockAmount);
    }

    function addStrategic(address _account, uint256 _amount) public {
        require(hasRole(CREATOR_ADMIN, address(msg.sender)), "Caller is not a admin");
        require(totalAdd.add(_amount) <= lockAmount, "Exceeded the allowed amount");
        remainAmount[address(_account)] = remainAmount[address(_account)].add(_amount);
        totalAdd = totalAdd.add(_amount);
        emit addMember(
            _account,
            _amount,
            block.timestamp
    ...
    function getBalanceToken(address _address) public view returns( uint256 ) {
        ...
        if(block.timestamp > lockTime){
            ...
            amount = remainAmount[address(_address)].mul(claimPeriod).div(vestingPeriod) > withdrawalAmount[address(_address)] ? (remainAmount[address(_address)].mul(claimPeriod).div(vestingPeriod) - withdrawalAmount[address(_address)]) : 0;
        }
        return amount;
    }

    function safeImbaTransfer(address _to, uint256 _amount) internal {
        uint256 ImbaBal = tokenImba.balanceOf(address(this));
        if (_amount > ImbaBal) {
            tokenImba.transfer(_to, ImbaBal);
        } else {
            tokenImba.transfer(_to, _amount);
        }
    }
}