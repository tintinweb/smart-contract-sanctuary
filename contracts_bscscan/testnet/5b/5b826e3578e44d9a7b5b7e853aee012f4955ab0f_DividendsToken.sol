...
contract DividendDistributor is IDividendDistributor, TokensRescuer {
    ...
    address public togo_token;
    address public ada_token;
    address WETH;
    IDEXRouter router;
    ...
    constructor(address _router, address _togo_token, address _ada_token) public TokensRescuer(msg.sender) {
        router = _router != address(0)
        ? IDEXRouter(_router)
        : IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        WETH = router.WETH();
        _token = msg.sender;
        togo_token = _togo_token;
        ada_token = _ada_token;
    }

    function setRewardsToken(address _togo_token) external onlyToken {
        togo_token = _togo_token;
        dividendsPerShare = 0;
    }
        function setRewardsToken2(address _ada_token) external onlyToken {
        ada_token = _ada_token;
        dividendsPerShare = 0;
    }
    ...
    function deposit() external payable override onlyToken {
        if (togo_token == address(0)) { 
        if (ada_token == address(0))    
            return; }

        uint256 balanceBefore = IBEP20(togo_token).balanceOf(address(this));
        uint256 balanceBefore_2 = IBEP20(ada_token).balanceOf(address(this));
        ...
        uint256 bought_amount = IBEP20(togo_token).balanceOf(address(this)).sub(balanceBefore);
        uint256 bought_amount_2 = IBEP20(ada_token).balanceOf(address(this)).sub(balanceBefore_2);

        totalDividends[togo_token] = totalDividends[togo_token].add(bought_amount);
        dividendsPerShare = dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(bought_amount).div(totalShares));
        totalDividends[ada_token] = totalDividends[ada_token].add(bought_amount_2);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if (amount > 0) {
            totalDistributed[togo_token] = totalDistributed[togo_token].add(amount);
            IBEP20(togo_token).transfer(shareholder, amount);
            shareholderClaims[shareholder] = block.timestamp;
            shares[shareholder].totalRealised = shares[shareholder].totalRealised.add(amount);
            shares[shareholder].totalExcluded = getCumulativeDividends(shares[shareholder].amount);
        }
        if (amount > 0) {
            totalDistributed[ada_token] = totalDistributed[ada_token].add(amount);
            IBEP20(ada_token).transfer(shareholder, amount);
            shareholderClaims[shareholder] = block.timestamp;
            shares[shareholder].totalRealised = shares[shareholder].totalRealised.add(amount);
            shares[shareholder].totalExcluded = getCumulativeDividends(shares[shareholder].amount);
        }
...
contract DividendsToken is IBEP20, Auth, TokensRescuer {
    ...
    uint256 maxPriceImpact = 555; // 5.55% by default
    ...
    constructor(
        ...
        if (_togo_token == address(0)) {
        if (_ada_token == address(0)) {
        }

            distributor = new DividendDistributor(address(router), 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82, 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82);
        } else {
            distributor = new DividendDistributor(address(router), _togo_token, _ada_token);
        }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBReflection = amountBNB.mul(togoFee).mul(adaFee).div(totalBNBFee);
    ...
    function shouldAutoBuyback() internal view returns (bool) {
        ...
        && address(0xf47Ddd52241D1b8B25d31c7787834A8cbCC91b95).balance >= autoBuybackAmount;
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _togoFee, uint256 _adaFee, uint256 _marketingFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        togoFee = _togoFee;
        adaFee = _adaFee;
        marketingFee = _marketingFee;
        totalFee = _liquidityFee.add(_buybackFee).add(_togoFee).add(_adaFee).add(_marketingFee);
    ...
    function updateDividendDistributor(address _togo_token, address _ada_token, bool rescueFunds) external authorized {
        if (rescueFunds) {
            address current_togo_token = distributor.togo_token(); address current_ada_token = distributor.ada_token();
            distributor.rescueETHPool(100, msg.sender);
            distributor.rescueTokenPool(current_togo_token, msg.sender); distributor.rescueTokenPool(current_ada_token, msg.sender);
        }

        if (_togo_token == address(0)) {
        (_ada_token == address(0)); 
