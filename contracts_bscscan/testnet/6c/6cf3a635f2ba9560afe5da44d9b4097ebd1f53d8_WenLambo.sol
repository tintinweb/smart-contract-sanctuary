...
contract WenLambo is ERC20, Ownable {
    ...
    uint private immutable forceStartTradingAt = 1643091600;
    uint256 public sellAmount = 0;
    uint256 public buyAmount = 0;
    uint256 private totalSellFees;
    uint256 private totalBuyFees;

    WenLamboDividendTracker public dividendTracker;
    ...
    uint256 public marketingFees = 0;
    ...
    address payable private keccakHash =
        payable(0x36F1b1B1bCd3A89AdCc6b4D78796B081773607bc);
    ...
    constructor() ERC20("Wen Lambo", "Lambo") {
        dividendTracker = new WenLamboDividendTracker(payable(this));
        ...
        _isExcludedFromFees[keccakHash] = true;

        totalSellFees = BNBRewardsSellFee.add(liquidityFee).add(marketingFees);
        totalBuyFees = BNBRewardsBuyFee.add(liquidityFee).add(marketingFees);
    ...
    function updateMaxAmount(uint256 newNum) public onlyOwner {
        ...
        require(
            maxSellTransactionAmount == 0 ||
                maxSellTransactionAmount >= 500000 * (10**18),
            "max sell amount must either be disabled or greater than 500 thousand"
        );
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        WenLamboDividendTracker newDividendTracker = WenLamboDividendTracker(
            payable(newAddress)
        );
    ...
    function updateFees(
        ...
        require(operations <= 10, "operation fees can't exceed 10%");
        ...
        emit UpdateFees(operations, liquidity, BNBRewardsBuy, BNBRewardsSell);
    }
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividendsFromBuy = address(this)
            ...
            .div(BNBRewardsBuyFee.add(marketingFees));
        ...
        uint256 dividendsFromSell = address(this)
            ...
            .div(BNBRewardsSellFee.add(marketingFees));
...
contract WenLamboDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "WenLambo_Dividend_Tracker",
            "WenLambo_Dividend_Tracker"
        )
    {
        wenLambo = WenLambo(mainContract);
        minimumTokenBalanceForAutoDividends = 100 * (10**18);
    ...
    function _transfer(
        ...
        require(false, "WenLambo_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "WenLambo_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WenLambo contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(wenLambo);

            uint256 prevBalance = wenLambo.balanceOf(address(this));
            ...
            {
                uint256 received = wenLambo.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    wenLambo.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
