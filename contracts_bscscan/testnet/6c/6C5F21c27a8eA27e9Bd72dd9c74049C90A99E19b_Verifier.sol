 ...
 * @dev Verifier contract alow to verify signature
 */
contract Verifier is IVerifier, AdminableUpgradeable {

    address public signWallet;

    function __Verifier_init(address _signWallet) public initializer {
        __Adminable_init();

        signWallet = _signWallet;
    }

    function changeSigner(address _wallet) external onlyOwnerOrAdmin {
        require(_wallet != address(0), "changeSigner: singWallet cannot be zero");
        signWallet = _wallet;
    }

    function verify(bytes32 _message, bytes memory _signature) external view override returns (bool) {
