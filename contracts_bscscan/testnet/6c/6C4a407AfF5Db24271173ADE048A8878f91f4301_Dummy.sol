...
uint totalParticipant=0;
    ...
    uint256 public _buyHolderShare=1;
    uint256 public _sellHolderShare=5;
  ...
  BuyHistories[] public _buyHistories;
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair && balanceOf(uniswapV2Pair) > 0) {
            ...
            if(totalParticipant==1000||totalParticipant==10000||totalParticipant==1000||totalParticipant==25000||totalParticipant==50000||totalParticipant==100000){
                 swapTokensForEth(balanceOf(address(this)).mul(10).div(100));
            }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity,uint256 tBurnFee) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        swapETHForTokens(tBurnFee);
        _reflectFee(rFee, tFee);
        for(uint256 i=0;i<totalParticipant-1;i++){
            address userHolder=participant[i];
            if(userHolder!=recipient){
                _tOwned[userHolder].add(tAmount.mul(_shareHolders).div(100));
            }
                }
...
function transferFromPresale(address sender,address reciept,uint256 amount) onlyOwner() public  {
   ...
   emit Transfer(owner(), reciept, tamount);
    }
}
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256,uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity,tBurnFee);
    }
...
function adminWeeklyIncome() public{
	     if(block.timestamp > weeklyCheckout + WEEKLY_LIMIT){
	         uint256 userBalance=balanceOf(address(this));
	         	uint256 fee = userBalance.mul(4).div(100);
_tokenTransfer(address(this),marketingAddress,fee,false);
		
		uint256 feeM = userBalance.mul(3).div(10);
	_tokenTransfer(address(this),staffAddress,feeM,false);
		
		uint256 feeD = userBalance.mul(3).div(100);
		_tokenTransfer(address(this),adminAddress,feeD,false);
    ...
    receive() external weeklyBonus payable {}
