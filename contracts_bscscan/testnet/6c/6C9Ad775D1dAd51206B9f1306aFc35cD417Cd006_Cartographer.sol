...
import "./ICartographerExpedition.sol";
...
contract Cartographer is Ownable, Initializable, ReentrancyGuard, ICartographer {
    ...
    uint256 public override summitPerBlock = 9e17;
    ...
    ICartographerOasis cartographerOasis;
    ICartographerElevation cartographerElevation;
    ICartographerExpedition cartographerExpedition;
    ...
    function initialize(
        ...
        address _IElevationHelper,
        address _IRandomNumberGenerator,
        address _ISummitReferrals,
        address _ISummitKeywords,
        address _ICartographerOasis,
        address _ICartographerElevation,
        address _ICartographerExpedition)
    external initializer onlyOwner {
        require(
            _IElevationHelper != address(0) &&
            _IRandomNumberGenerator != address(0) &&
            _ISummitReferrals != address(0) &&
            _ISummitKeywords != address(0) &&
            _ICartographerOasis != address(0) &&
            _ICartographerElevation != address(0) &&
            _ICartographerExpedition != address(0),
            "Contract is zero"
        );
        summit = fakeSummit;
        ...
        cartographerOasis = ICartographerOasis(_ICartographerOasis);
        cartographerElevation = ICartographerElevation(_ICartographerElevation);
        cartographerExpedition = ICartographerExpedition(_ICartographerExpedition);
    ...
    function getElevationRoundEndTime(uint8 _elevation) public view elevationOrExpedition(_elevation) returns(uint256) {
        return elevationHelper.getElevationRoundEndTime(_elevation);
    }

    function getReferralBurnTime() public view returns(uint256) {
        return elevationHelper.getReferralBurnTime();
    }
    ...
    function rolloverElevation(uint8 _elevation, string memory _keyword) public elevationOrExpedition(_elevation) correctKeyword(_keyword) {
        ...
        bytes32 randomHash = randomGenerator.getRandomNumber(_keyword, msg.sender, elevationHelper.getElevationRound(_elevation), _elevation, stakedSummitRandAccum);
    ...
    function deposit(uint8 _pid, uint256 _amount, uint8 _totem) public nonReentrant poolExists(_pid) {
        uint8 elevation = poolElevation[_pid];
        require(elevation == OASIS || _totem < elevationHelper.getElevationTotemCount(poolElevation[_pid]), "Invalid totem");
    ...
    function switchTotem(uint8 _elevation, uint8 _newTotem) public nonReentrant elevationOrExpedition(_elevation) {
        require(_newTotem < elevationHelper.getElevationTotemCount(_elevation), "Invalid totem");
        if (_elevation == EXPEDITION) {
    ...
    function enableTokenAtElevation(IBEP20 lpToken, uint8 elevation) external onlySubCartographer() override {
    ...
    function disableTokenAtElevation(IBEP20 lpToken, uint8 elevation) external onlySubCartographer() override {
        uint256 modAllocPoint = elevationHelper.getElevationModifiedAllocPoint(tokenBaseAllocPoint[lpToken], elevation);
    ...
    function depositTokenManagement(address userAdd, IBEP20 lpToken, uint256 depositFeeBP, uint256 amount, uint256 cakeChefPid) external onlySubCartographer() override returns (uint256) {
    ...
    function withdrawalTokenManagement(address userAdd, IBEP20 lpToken, uint256 amount, uint256 cakeChefPid) external onlySubCartographer() override {
        if (cakeChefPid != 0) {
    ...
    function setPassthroughTokenManagement(uint256 cakeChefPid, IBEP20 lpToken, uint256 initialAmount) external onlySubCartographer() override {
        lpToken.approve(address(cakeChef), 1e50);
        if (initialAmount == 0) { return; }
        cakeChef.deposit(cakeChefPid, initialAmount);
    }
    function disablePassthroughTokenManagement(uint256 amount, uint256 cakeChefPid) external onlySubCartographer() override {
        cakeChef.withdraw(cakeChefPid, amount);
        transferPassthroughRewards();
    }
    ...
    function getSummitReward(uint256 lastRewardBlock, IBEP20 lpToken, uint8 elevation) external view onlySubCartographer() override returns (uint256) {
    ...
    function updatePoolMint(uint256 lastRewardBlock, IBEP20 _lpToken, uint8 _elevation) external onlySubCartographer() override returns (uint256) {
    ...
    function redeemRewards(address userAdd, uint256 amount) external onlySubCartographer() override {
...
interface ICartographerExpedition {
    function userTotem(address userAdd) external view returns (uint8);
    function getIsTotemInUse(address _userAdd) external view returns (bool);

    function initialize(address _IElevationHelper) external;
    function enableSummit(SummitToken _trueSummit) external;
    
    function add(uint8 _pid, IBEP20 _rewardToken, uint256 _rewardAmount, uint256 _rounds, bool _withUpdate) external;
    function rolloverExpedition() external;
    function getPendingReward(uint8 _pid, address _userAdd) external view returns (uint256);
    function getHypotheticalReward(uint8 _pid, address _userAdd) external view returns (uint256, uint256);
    function massUpdatePools() external;
    function switchAllTotems(uint8 _newTotem, address _userAdd) external;
    function getLpSupply(uint8 _pid) external view returns (uint256);
    function deposit(uint8 _pid, uint256 _amount, uint8 _totem, address _userAdd) external returns (uint256);
    function withdraw(uint8 _pid, uint256 _amount, address _userAdd) external returns (uint256);
    function emergencyWithdraw(uint8 _pid, address _userAdd) external returns (uint256);
