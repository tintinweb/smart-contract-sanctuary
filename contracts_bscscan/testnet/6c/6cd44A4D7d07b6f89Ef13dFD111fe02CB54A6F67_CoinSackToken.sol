...
contract CoinSackToken is IBEP20, Manageable {
    ...
    function _transfer(address from, address to, uint256 tAmount) private {
        ...
        if(_areFeesEnabled && !(_isExcludedFromFees[to] || _isExcludedFromFees[from])){
            if(_isAutoFeeLiquifyEnabled && !_isInternallySwapping && to == address(_pancakePair) && _totalFeesTAmountPendingLiquidation >= _minPendingFeesForAutoLiquify) {
            ...
            _totalTAmountInEscrow = balanceOf(_contextAddress()).sub(_totalFeesTAmountPendingLiquidation);
    ...
    function setManagementFeeBuy(uint256 managementFeeBuy) public onlyManagement() returns (bool) {
        require(_buyFeesTotal - _managementFeeBuy + managementFeeBuy <= 20, "total buy fees cannot exceed 20");
    ...
    function setManagementFeeSell(uint256 managementFeeSell) public onlyManagement() returns (bool) {
        require(_sellFeesTotal - _managementFeeSell + managementFeeSell <= 20, "total sell fees cannot exceed 20");
    ...
    function setReserveFeeBuy(uint256 reserveFeeBuy) public onlyManagement() returns (bool) {
        require(_buyFeesTotal - _reserveFeeBuy + reserveFeeBuy <= 20, "total buy fees cannot exceed 20");
    ...
    function setReserveFeeSell(uint256 reserveFeeSell) public onlyManagement() returns (bool) {
        require(_sellFeesTotal - _reserveFeeSell + reserveFeeSell <= 20, "total sell fees cannot exceed 20");
    ...
    function setReflectionsFeeBuy(uint256 reflectionsFeeBuy) public onlyManagement() returns (bool) {
        require(_buyFeesTotal - _reflectionFeeBuy + reflectionsFeeBuy <= 20, "total buy fees cannot exceed 20");
    ...
    function setReflectionsFeeSell(uint256 reflectionsFeeSell) public onlyManagement() returns (bool) {
        require(_sellFeesTotal - _reflectionFeeSell + reflectionsFeeSell <= 20, "total sell fees cannot exceed 20");
    ...
    function performManualBuyback(uint256 buybackFactor) public onlyManagement() returns (bool) {
        require(buybackFactor <= 100, "buyback factor cannot exceed 100");
        _buybackTokens(_contextAddress().balance.mul(buybackFactor).div(100));
        return true;
    }
...
contract Callable {
    ...
    receive() external payable { 
        emit RecieveETH(msg.value);
    }


    event RecieveETH(uint256 amountETHRecieved);
