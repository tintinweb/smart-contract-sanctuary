...
import "./Ownable2of3.sol";


contract CRT is Context, IERC20, Ownable2of3 {
    ...
    string private _name = "Crypto Race Token";
    ...
    uint256 public _antiDumpFee = 300;
    uint256 private _previousAntiDumpFee = _antiDumpFee;
    uint256 public antiDumpTime;

    uint256 public constant DENOMINATOR = 10000;

    address public marketingWallet = 0xBC3C2C6e7BaAeB7C7EA2ad4B2Fa8681a91d47Ccd;
    ...
    uint256 public _maxTxAmount = 4 * 10 ** 6 * 10 ** uint256(_decimals);
    uint256 private _numTokensSellToAddToLiquidity = 4 * 10 ** 6 * 10 ** uint256(_decimals);
    ...
    constructor(address founder1, address founder2, address founder3) Ownable2of3(founder1, founder2, founder3){
        _rOwned[_msgSender()] = _rTotal;

//        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); // main PCS V2
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); // test PCS V2
        ...
        _isExcludedFromFee[founder1] = true;
        _isExcludedFromFee[founder2] = true;
        _isExcludedFromFee[founder3] = true;
    ...
    function _getValues(uint256 tAmount, bool isDump) private view returns (ValuesStruct memory) {
        TValuesStruct memory tvs = _getTValues(tAmount, isDump);
    ...
    function _getTValues(uint256 tAmount, bool isDump) private view returns (TValuesStruct memory) {
        uint256 tFee = calculateTaxFee(tAmount);
        uint256 tMarketing = calculateMarketingFee(tAmount, isDump);
    ...
    function calculateMarketingFee(uint256 _amount, bool isDump) private view returns (uint256) {
        return _amount * (isDump ? (_marketFee + _antiDumpFee): _marketFee) / DENOMINATOR;
    }
    ...
    function removeAllFee() private {
        ...
        _previousAntiDumpFee = _antiDumpFee;
        _taxFee = 0;
        _liquidityFee = 0;
        _marketFee = 0;
        _antiDumpFee = 0;
    ...
    function restoreAllFee() private {
        ...
        _antiDumpFee = _previousAntiDumpFee;
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        bool isDump = (recipient == uniswapV2Pair && block.timestamp < antiDumpTime);
        ValuesStruct memory vs = _getValues(amount, isDump);
    ...
    function setAntiDumpFee(uint256 antiDumpFee) external onlyOwner {
        _antiDumpFee = antiDumpFee;
    }

    function setAntiDumpTime(uint256 newTime) external onlyOwner {
        antiDumpTime = newTime;
    }
    ...
    function clearStuckTokenBalance(address to, address tokenAddress, uint256 amount) external onlyOwner {
        require(tokenAddress != address(this), "Can't claim native tokens");
...
abstract contract Ownable2of3 {

    mapping(address => bool) public isFounder;
    address[3] public founders;
//          founder index   => new address => voted address
    mapping(uint256 => mapping(address => address)) updateFounderVotes;
//        new owner =>    end time   => voter
    mapping(address => mapping(uint256 => address)) addOwnerVotes;

    //          owner   => end timestamp
    mapping(address => uint256) public ownerEndTime;

    event OwnershipSet(address indexed newOwner, uint256 endTime);

    constructor(address address1, address address2, address address3) {
        setFounder(0, address1);
        setFounder(1, address2);
        setFounder(2, address3);
    }
    ...
    modifier onlyOwner() {
        require(ownerEndTime[msg.sender] > block.timestamp, "Ownable: caller is not the owner");
        _;
    }


    function updateFounder(uint256 index, address newFounder) external onlyFounder {
        address voter = msg.sender;
        require(newFounder != address(0), "Zero address provided");
        require(updateFounderVotes[index][newFounder] != voter, "Already voted for this address");
        address prevFounder = founders[index];
        if (isFounder[updateFounderVotes[index][newFounder]]) {
            isFounder[prevFounder] = false;
            setFounder(index, newFounder);
        } else {
            updateFounderVotes[index][newFounder] = voter;
        }
    }

    function setFounder(uint256 index, address founder) private {
        isFounder[founder] = true;
        founders[index] = founder;
    }

    function setOwner(address newOwner, uint256 endTime) external onlyFounder {
        address voter = msg.sender;
        require(newOwner != address(0), "Zero address provided");
        require(addOwnerVotes[newOwner][endTime] != voter, "Already voted for this owner");
        if (isFounder[addOwnerVotes[newOwner][endTime]]) {
            ownerEndTime[newOwner] = endTime;
        } else {
            addOwnerVotes[newOwner][endTime] = voter;
        }
    }
