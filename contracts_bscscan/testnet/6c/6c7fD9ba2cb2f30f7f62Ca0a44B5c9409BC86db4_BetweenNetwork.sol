    ...
    ReentrancyGuardUpgradeable
{
    ...
    bytes32[] public DepositTxHashes;
    ...
    event UpdateRewardToken(address oldRewardToken, address newRewardToken);
    ...
    function initialize(
        ...
        require(
            ...
            "ValidateInitialize:: _governance can not be Zero Address"
        );
        require(
            _beneficiary != address(0),
            "ValidateInitialize:: _beneficiary can not be Zero Address"
        );
        governance = _governance;
        beneficiary = _beneficiary;
        maxPlatformFee = 10000;
        ...
        validatorExists(_validator)
    {
        ...
        if (requiredValidation > validators.length) {
    ...
    function tapOut(
        ...
        uint256 _validatorCount;
        address[] memory _validatedBy;
        uint256 _index;
        uint256 _signatureLength = signature.length;
        address _proxy = address(this);
        for (uint256 i = 0; i < _signatureLength; i++) {
            if (
                verifySignature(
                    _proxy,
                    _amount,
                ...
                _validatorCount.add(1);
                _validatedBy[_index] = _validators[i];
                _index.add(1);
            }
        }
        require(
            _validatorCount >= requiredValidation,
            "TapOut: Your Transaction has not been validated"
        );
        ...
        internal
    {
        if (depositFee > 0) {
            IERC20Upgradeable(_homeERC20).safeTransferFrom(
                msg.sender,
                beneficiary,
                _amount.mul(depositFee).mul(multiplier).div(maxPlatformFee)
            );
        }
        if (
    ...
    function withdrawTokensAndPlatformFee(
        ...
        if (withdrawFee > 0) {
            IERC20Upgradeable(_withdrawERC20).safeTransfer(
                beneficiary,
                _amount.mul(withdrawFee).mul(multiplier).div(maxPlatformFee)
            );
        }
        if (
            rewardToken != address(0) &&
    ...
    function updateWithdraw(
        ...
        withdrawalWallets[msg.sender]._validators = _validators;
    ...
    function updateDepositFee(uint256 _newDepositFee) external onlyGovernance {
        ...
        require(
            _newDepositFee <= maxPlatformFee,
            "UpdateDepositFee:: Invalid Deposit fee "
        );
        require(
        ...
        onlyGovernance
    {
        ...
        require(
            _newWithdrawFee <= maxPlatformFee,
            "UpdateWithdrawFee:: Invalid withdraw fee "
        );
        require(
    ...
    {
        require(
            _newRewardToken != address(0),
            "UpdateRewardToken:: _newRewardToken can not be Zero address"
        );
        rewardToken = _newRewardToken;
        emit UpdateRewardToken(rewardToken, _newRewardToken);
    }
    ...
    function renounceRewardToken() external onlyGovernance {
        rewardToken = address(0);
        emit UpdateRewardToken(rewardToken, address(0));
    }
