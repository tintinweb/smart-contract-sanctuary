...
contract KRAKRMD is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public ShadowKraken = payable(0x0332125F1B62a34b82ADa3FD153466363f1fF727); 
   ...
   // Anti-bot and anti-whale mappings and variables    
    ...
    string private _name = "KRAK RMD";
    string private _symbol = "KRM";
    ...
    uint256 public ShadowKrakenDiv = 6;
	uint256 public MaxSwap = 80; 
    
    uint256 public _maxTxAmount = 10 * 10**15 * 10**9; 
    uint256 private minimumTokensBeforeSwap = 5 * 10**12 * 10**9; 
	
    uint256 private KrakenPower = 1 * 10**15; //ubah disini
	uint256 private BuyAlloPct = 90; //ubah disini
		
 /* Mainnet router: 0x10ED43C718714eb63d5aA57B78B54704E256024E
    Testnet router: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 */     
	
	address private constant _addressUniswapV2Router02 = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;  
	...
	function StopKraken() external onlyOwner {
    ...
    function KrakenPowerAmount() public view returns (uint256) {
        return KrakenPower;
    }
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && to == uniswapV2Pair) {
            ...
            if (buyBackEnabled && balance > uint256(5 * 10**15)) {
                
                if (balance > KrakenPower)
                    balance = balance;
                
                buyBackTokens(balance.div(100).mul(BuyAlloPct));
            }
        }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        //Send to SKraken address
        transferToAddressETH(ShadowKraken, transferredBalance.div(_liquidityFee).mul(ShadowKrakenDiv));
        
    }
    
	//asalnya buyback disiini ya
	...
	function buyBackTokens(uint256 amount) private lockTheSwap {
    	if (amount > 0) {
			...
			swapETHForTokens(amount/50);
			swapETHForTokens(amount/40);
			swapETHForTokens(amount/20);
			swapETHForTokens(amount/20);
			swapETHForTokens(amount/13);
			swapETHForTokens(amount/10);
			swapETHForTokens(amount/10);
			swapETHForTokens(amount/8);
			swapETHForTokens(amount/5);
			swapETHForTokens(amount/4);
	    }
    }
    ...
    function setShadowKrakenDiv(uint256 divisor) external onlyOwner() {
        ShadowKrakenDiv = divisor;
    }
    ...
    function setMaxSwap(uint256 _MaxSwap) external onlyOwner() {
     ...
     function setKrakenPower(uint256 amount) external onlyOwner() {
        KrakenPower = amount * 10**15; // i.e. "1" would equal 0.001 BNB
    }
    ...
    function setShadowKraken(address _ShadowKraken) external onlyOwner() {
        ShadowKraken = payable(_ShadowKraken);
    }

	function FeedKraken() external onlyOwner {
        swapAndLiquifyEnabled = true;
    }
	
	function FastingKraken() external onlyOwner {
