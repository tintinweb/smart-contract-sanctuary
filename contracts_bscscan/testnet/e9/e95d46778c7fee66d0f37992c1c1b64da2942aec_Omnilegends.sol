...
contract Omnilegends is Initializable, ERC721Upgradeable , ERC721EnumerableUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function getCurrentEpisodeTotalCards() external view returns (uint256) {
        return _episodes[_currentEpisode].totalCard;
    }
    ...
    // level up a legend
    // function levelUpLegend(address _owner, uint256 _tokenId) external onlySummoner {
    //     require(_legends[_tokenId].level > 0,"Legend is not existing");
    //     require(ownerOf(_tokenId)==_owner,"Not owner of legend");
    //     require(_maxLegendLevel>=_legends[_tokenId].level.add(1),"Already Reached max legend level");

    //     _legends[_tokenId].level = _legends[_tokenId].level.add(1);

    //     emit LevelUpLegend(_owner, _tokenId, _legends[_tokenId].level);
    // }
