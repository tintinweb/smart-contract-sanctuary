...
contract CMSToken is ERC20Capped, Ownable, AccessControlEnumerable {
    using SafeMath for uint256;
    using Address for address;

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    uint256 private constant SUPPLY = 12 * 10**7 * 10**18;
    uint256 private constant SALE_TOKEN = 6 * 10**5 * 10**18;
    ...
    */
    ...
    /**
     * @dev Destroys `_tokens` tokens from the address '_from'.
     *
     * See {ERC20-_burn}.
     */
    function burnTokens(address _from, uint256 _tokens) external onlyOwner {
        CURRENT_SUPPLY = CURRENT_SUPPLY.sub(_tokens);
    ...
    function buy(address _to, uint256 _tokens) external onlyOwner {
        require(TOTAL_SOLD <= SALE_TOKEN, "CMSToken: Sale cap has been reached. You cannot buy more tokens.");

        TOTAL_SOLD = TOTAL_SOLD.add(_tokens);
        tokensSold[_to] = tokensSold[_to].add( _tokens);
        CURRENT_SUPPLY = CURRENT_SUPPLY.add(_tokens);
    ...
    */
    function mint(address _to, uint256 _amount) external {
        require(hasRole(MINTER_ROLE, _msgSender()), "CMSToken: must have minter role to mint");
        _mint(_to, _amount);
