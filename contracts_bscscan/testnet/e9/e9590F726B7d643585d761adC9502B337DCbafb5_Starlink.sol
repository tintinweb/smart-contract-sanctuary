...
contract Starlink is IStarlink, IDepositable, StarlinkComponent, PancakeSwapHelper {
    ...
    function unclaimedDividendsOf(address userAddress, address token) external view returns (uint256) {
        ...
        UserStaking storage userStake = userPoolStake[userAddress][token];
        return calculateUnclaimedDividends(pool, userStake);
    }

    function calculateUnclaimedDividends(Pool storage pool, UserStaking storage userStake) private view returns (uint256) {
        uint256 totalDividends = pool.totalDividends;
        uint256 newDividends = totalDividends - userStake.lastDividends;
        uint256 reward = newDividends * userStake.amount / availableSupply();

        return userStake.unclaimedDividends + reward;
    }
    ...
    function updateStakingOf(address userAddress, address token) private {
        ...
        userStake.unclaimedDividends = calculateUnclaimedDividends(pool, userStake);
