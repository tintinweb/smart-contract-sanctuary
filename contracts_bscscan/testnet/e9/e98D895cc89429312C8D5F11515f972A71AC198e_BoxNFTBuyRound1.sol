...
contract BoxNFTBuyRound1 is OwnableUpgradeable, PausableUpgradeable  {
    ...
    mapping(uint256 => uint256) public limitBuyOfUsers; // Not use
    mapping(uint256 => CountersUpgradeable.Counter) public _totalBuyOfAllUsers;
    

    // ------ Start New Variable  ------ //
    uint256 totalDays;
    mapping(uint256 => mapping(uint256 => DailyRankConfig)) public dailyRankConfigs; // Day Rank DailyConfig
    struct DailyRankConfig {
      uint256 limitBuyOfUsers; // = 0 No limit 
      uint256 totalBuyOfAllUsers;
    }
    ...
    function initialize() public initializer {
        ...
        totalDays=0;
        isActive=true;
        priceSales[1]=0.09 ether;       // D Starter Box
        priceSales[2]=0.32 ether;       // C Bronze Box
        priceSales[3]=0.81 ether;       // B Silver Box
    ...
    function initByOwner(BoxNFT _boxNFT,uint256 _startTime, uint256 _totalDays) public onlyOwner {
        require(_totalDays <= 1000,'_totalDays can not greater than 1000 days');
        boxNFT = _boxNFT;
        startTime=_startTime;
        totalDays=_totalDays;
        endTime=_startTime + 86400 + _totalDays * 86400;  // 1 day = 86400s
        for(uint i = 0; i< _totalDays; i++){
          dailyRankConfigs[i][1].limitBuyOfUsers=100;
          dailyRankConfigs[i][2].limitBuyOfUsers=30;
          dailyRankConfigs[i][3].limitBuyOfUsers=10;
        }
    }

    /**
     * @dev called by the owner to init
     * startTime should start day 2022-01-15 00:00:00 1642179600
     *
     */
    function initByOwnerDailyConfig(uint256 _totalDays,uint256[] memory _limitBuyOfUsers) public onlyOwner {
        for(uint256 i = 0; i < _totalDays; i++){
          for(uint256 x = 0; x < _limitBuyOfUsers.length; x++){
          dailyRankConfigs[i][x+1] = DailyRankConfig(_limitBuyOfUsers[x],0);
          }
        }
    }
...
function buyBox(uint256 rank) external payable whenNotPaused returns (uint256) {
        ...
        require(_totalBuyOfAllUsers[rank].current()<=limits[rank]-1,'Not buy box, because maximum boxes are selled');
        _totalBuyOfAllUsers[rank].increment();
        
        uint256 day = 0;
        bool dayCheck = false;
        for(uint i=0 ;i< totalDays; i++){
          uint256 startTimeCheck=startTime+86400*i;
          uint256 endTimeCheck=startTimeCheck+86400;
          if(startTimeCheck<=current&&current<=endTimeCheck){
            day=i;
            dayCheck=true;
            break;
          }
        }
        require(dayCheck==true,'Not find daily config for sale');
        require(dailyRankConfigs[day][rank].totalBuyOfAllUsers <= dailyRankConfigs[day][rank].limitBuyOfUsers-1,
        'Not buy box, because maximum boxes are selled in day');
        dailyRankConfigs[day][rank].totalBuyOfAllUsers=dailyRankConfigs[day][rank].totalBuyOfAllUsers + 1;
        // uint numberBuyBox=0;
