...
contract MultiRewards is ReentrancyGuard, Pausable {
    ...
    function unlockedStake(address account) public view returns (uint256) {
	    uint actualAmount = 0;
	    uint thisTest = lastTimeRewardApplicable(rewardTokens[0]);
    ...
    function stake(uint256 amount) external nonReentrant notPaused updateReward(msg.sender) {
        ...
        userStakes[msg.sender].push(Stake(block.timestamp.add(lockDuration), amount));
    ...
    function notifyRewardAmount(address _rewardsToken, uint256 reward, uint256 max_supply) external updateReward(address(0)) {
        ...
        rewardData[_rewardsToken].rewardRate = reward.div(rewardData[_rewardsToken].rewardsDuration).mul(1e18).div(_maxSupply);
    ...
    modifier updateReward(address account) {
        for (uint i; i < rewardTokens.length; i++) {
            ...
            if (_totalSupply < _maxSupply)
                rewards[rewardData[token].rewardsDistributor][token] = lastTimeRewardApplicable(token).sub(rewardData[token].lastUpdateTime).mul(rewardData[token].rewardRate).mul(_maxSupply.sub(_totalSupply)).div(1e18).add(rewards[rewardData[token].rewardsDistributor][token]);   
