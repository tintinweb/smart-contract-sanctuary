...
contract Dummy1DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Dummy1_Dividend_Tracker", "Dummy1_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "Dummy1_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "Dummy1_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Dummy1 contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "Dummy1_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Dummy1_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Dummy1 is ERC20, Ownable {
    ...
    Dummy1DividendTracker public dividendTracker;

    address public _marketingWallet = 0xca9023B5A7f3b18B57e4A9C15Ab2C2a59BB2C9c3;
...
uint256 _totalSupply =1000000000 * (10**9);
    constructor() ERC20("Dummy1", "Dummy1") {

        dividendTracker = new Dummy1DividendTracker();
...
lp = _uniswapV2Pair;
 ...
 function rebaseToken(uint256 epoch, int256 supplyDelta)
  ...
  returns (uint256)
  {
    ...
    function setFeeBuy(uint256 newBuyRewardFee, uint256 newBuyLiqFee, uint256 newBuyMarketingFee) public onlyOwner {
    ...
    function setUpPanCake(uint256 tokenMax) external onlyOwner {
       
        restrictWhales = true;
        cooldownEnabled = true;
        maxTxAmount = tokenMax * (10**9);
        walletMax = tokenMax * (10**9);
         setFeeBuy(4,2,3);
        updateSellFees(6,2,5);
    }

    function setUpPinSale(uint256 tokenMax) external onlyOwner {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Dummy1: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Dummy1: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Dummy1: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Dummy1: Cannot update gasForProcessing to same value");
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        if(success) {
            emit transferDividends(tokensToLiquify.mul(SellRewardsFee).div(SellTotalFees), bnbForReflection);
        }
    }
