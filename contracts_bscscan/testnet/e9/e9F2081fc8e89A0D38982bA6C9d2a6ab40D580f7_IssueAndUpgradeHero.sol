...
contract IssueAndUpgradeHero is AccessControl {
    ...
    modifier conditionOfCommon(uint256 tokenId) {
        uint256 tierMain = queryNumberTier(heroesNumber[tokenId].tier);
        uint256 tierBasicMain = queryNumberTier(heroesNumber[tokenId].tierBasic);
        if(tierBasicMain == 0){
    ...
    function issueHero(uint256 _tokenId, uint256 _numberHero, string memory _tier) public {
        ...
        heroesNumber[_tokenId].name = heroes[_numberHero].name;
        heroesNumber[_tokenId].race = heroes[_numberHero].race;
        heroesNumber[_tokenId].class = heroes[_numberHero].class;
        heroesNumber[_tokenId].tier = _tier;
        heroesNumber[_tokenId].tierBasic = _tier;
        emit issue(
            msg.sender,
            _tokenId,
    ...
    function upgradeHero(uint256[] memory listHeroId) public conditionOfCommon(listHeroId[0]){
        ...
        require(keccak256(bytes( heroesNumber[heroesTierMain].tier )) != keccak256(bytes( tierName[tierName.length - 1].tierName )), "You are already at the highest level");
        require(queryNumberTier(heroesNumber[heroesTierMain].tier) != 100, "Tier not found");
        uint256 tierMain = queryNumberTier(heroesNumber[heroesTierMain].tier);    
        ...
        if(requestHeroOrRace == 0){
            ...
            for(uint256 i = 1; i < listHeroId.length ; i++){
                ...
                require(queryNumberTier(heroesNumber[listHeroId[i]].tier) == heroesTierFood, "Tier food not is incorrect");
        ...
        emit ascend(
            msg.sender,
            heroesTierMain,
            heroesNumber[heroesTierMain].tier
        );
    }
}