...
contract XXXTEST is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    event BuybackUpdated(address indexed newBuyback, address indexed oldBuyback);
    ...
    constructor() ERC20("XXXTEST", "XXXTEST") {
    	busdDividendTracker = new BusdDividendTracker();

    	marketingWallet = 0x38dB3c23AE2869c9AEd528B7524a6d4e1c3B876F;
    	buyback = 0x8321FcAa4A81e5b4b8A9997471F840C8A5099893;
        ...
        */
        _create(owner(), 100000000000000 * (10**18));
    }
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "XXXTEST: The marketing wallet is already this address");
    ...
    function afterPreSale() external onlyOwner {
        ...
        maxWalletToken = 100000000000000 * (10**18);
    }
    ...
    function updateBusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "XXXTEST: The dividend tracker already has that address");

        BusdDividendTracker newBusdDividendTracker = BusdDividendTracker(payable(newAddress));

        require(newBusdDividendTracker.owner() == address(this), "XXXTEST: The new dividend tracker must be owned by the XXXTEST token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "XXXTEST: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "XXXTEST: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "XXXTEST: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "XXXTEST: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "XXXTEST: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(SwapAndLiquifyIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "XXXTEST: SwapAndLiquify has not started yet");
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (marketingEnabled) {
                    ...
                    uint256 buybackPortion = address(this).balance.div(10**2).mul(33);
                    uint256 marketingPortion = address(this).balance.sub(buybackPortion);
                    transferToWallet(payable(marketingWallet), marketingPortion);
                    transferToWallet(payable(buyback), buybackPortion);
                }
...
contract BusdDividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("XXXTEST_Busdereum_Dividend_Tracker", "XXXTEST_Busdereum_Dividend_Tracker", 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 20000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "XXXTEST_Busdereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "XXXTEST_Busdereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main XXXTEST contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "XXXTEST_Busdereum_Dividend_Tracker: Cannot update claimWait to same value");
