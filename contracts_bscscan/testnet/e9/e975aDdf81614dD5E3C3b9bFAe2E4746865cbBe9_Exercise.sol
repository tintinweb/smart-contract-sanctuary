...
interface ICirculatingSNAKE {
    function SNAKECirculatingSupply() external view returns ( uint );
}

/**
 *  Exercise contract for unapproved sellers prior to migrating pSNAKE.
...
contract Exercise {
    ...
    address immutable pSNAKE;
    address immutable SNAKE;
    address immutable BUSD;
    address immutable treasury;
    address immutable circulatingSNAKEContract;
    ...
    constructor( address _pSNAKE, address _snake, address _busd, address _treasury, address _circulatingSNAKEContract ) {
        owner = msg.sender;
        require( _pSNAKE != address(0) );
        pSNAKE = _pSNAKE;
        require( _snake != address(0) );
        SNAKE = _snake;
        require( _busd != address(0) );
        BUSD = _busd;
        require( _treasury != address(0) );
        treasury = _treasury;
        // require( _circulatingSNAKEContract != address(0) );
        circulatingSNAKEContract = _circulatingSNAKEContract;
    }
    ...
    function setTerms(address _vester, uint _rate, uint _claimed, uint _max ) external {
        ...
        // require( !IPOLY( pSNAKE ).isApprovedSeller( _vester ) );
    ...
    // Allows wallet to redeem pSNAKE for SNAKE
    function exercise( uint _amount ) external {
        ...
        IERC20( pSNAKE ).safeTransferFrom( msg.sender, address(this), _amount );
        
        IERC20( BUSD ).approve( treasury, _amount );
        uint SNAKEToSend = ITreasury( treasury ).deposit( _amount, BUSD, 0 );

        terms[ msg.sender ].claimed = info.claimed.add( _amount );

        IERC20( SNAKE ).safeTransfer( msg.sender, SNAKEToSend );
    }
    ...
    function redeemable( Term memory _info ) internal view returns ( uint ) {
        return ( ICirculatingSNAKE( circulatingSNAKEContract ).SNAKECirculatingSupply().mul( _info.percent ).mul( 1000 ) ).sub( _info.claimed );
    }
