...
contract LotteryToken is Context, IERC20, Ownable {
    ...
    */
    ...
    function havestCakeAndBuyBackStep1() public{
        if(investedCake){
    ...
    function havestCakeAndBuyBackStep2() public{
        if(investedCake){
            uint256 initialBalance = address(this).balance;
            swapCakeForEth(cakeToken.balanceOf(address(this)));
            buybackBalance = address(this).balance.sub(initialBalance);
        } 
    }

    function havestCakeAndBuyBackStep3() public{
        if(investedCake){
