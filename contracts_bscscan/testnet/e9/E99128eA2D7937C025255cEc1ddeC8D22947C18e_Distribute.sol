...
contract Distribute is Ownable, IDistribute {
    ...
    function _earnedPool(
        ...
        if (unoinsPools[unionId].length == 2 && lastClaimTime < poolStartTime) {
            ...
            if (pools[0].startTime == poolStartTime) {
                ...
                trueRewards = amount.mul(pools[0].rewardsAmount.add(pools[1].rewardsAmount)).div(unionAmount);
    ...
    function unionWarRewards(
        ...
        uint256 managerRewardsAmount = rewardsAmount.mul(maxBonusPercent).div(1e18);
        uint256 poolRewardsAmount = rewardsAmount.sub(managerRewardsAmount);

        _unionWarManagerRewards(unionId, managerRewardsAmount);
    ...
    function _unionWarManagerRewards(
        ...
        for(uint256 idx = 0; idx < managers.length; idx++) {
            ...
            if(identity == 2) {
                uint256 managerBonusAmount = managerRewardsAmount.mul(unoinsManagersBonusPercent[unionId][accountId]).div(1e18);
        ...
        unionManagersRewards[unionId][leaderAccountId] = unionManagersRewards[unionId][leaderAccountId].add(managerRewardsAmount.sub(bonusRewardsAmount));
    }
