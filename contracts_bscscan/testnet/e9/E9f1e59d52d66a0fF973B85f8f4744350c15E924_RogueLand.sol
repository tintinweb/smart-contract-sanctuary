...
contract RogueLand {
    ...
    function getEvents(int x1, int y1, int x2, int y2, uint t) public view returns (Event[] memory) {
        ...
        for (int x=x1; x<=x2; x++) {
          for (int y=y1; y<=y2; x++) {
            selectEvents[uint((x-x1)*(y2-y1+1)+y)] = events[t][x][y];
          }
        }
        return selectEvents;
    }
    ...
    function _addStillPunk(uint id, int x, int y) private {
