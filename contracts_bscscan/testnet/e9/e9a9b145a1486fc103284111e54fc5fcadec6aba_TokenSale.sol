...
abstract contract BonusHandler {
    ...
    function _bonusInit() internal virtual {

        bonusLimits[0] = 100000;
    ...
    function _bonusUpdate(uint[] memory _bonusLimits, uint[] memory _bonusRates) internal virtual {

        require(_bonusLimits.length == _bonusRates.length);

        require(_bonusRates.length == 3);
    ...
    function calculateBonusRate(uint256 _usdCents) view public returns(uint256) {

        _usdCents = _usdCents / 10 ** 18;

        for(uint8 i = 0; i < bonusLimits.length; i++) {



            if(_usdCents >= bonusLimits[i]) {
...
contract BuntToken is BuntERC20, Pausable, AccessControl {
    ...
    address private taxWallet = 0x9F398eb7139eD8c491da9Fb3A4Da585e0eD4A58F;
    
    constructor() BuntERC20("Bunt Chain", "BUNT", TAX_RATE, taxWallet) {
...
contract VestingVault is AccessControl, Initializable {
    ...
    function addTimedVesting(address _beneficiary, uint256 _amount, uint256 _cliff, address _eventProvider) internal onlyRole(VAULT_ADMIN_ROLE) vestingEventInitialized(_eventProvider) {
        ...
        require(cliff >= block.timestamp, "TIMESTAMP_CANNOT_BE_IN_THE_PAST");

        require(cliff <= (block.timestamp + 365 days), "TIMESTAMP_CANNOT_BE_MORE_THAN_A_365_DAYS_IN_FUTURE");
    ...
    function withdrawUnallocatedTokens(address _recipient) external onlyRole(VAULT_ADMIN_ROLE) {

        uint256 amount = unallocatedTokensAmount();

        require(amount > 0, "No Unallocated tokens");
    ...
    function unallocatedTokensAmount() public view returns (uint256) {

        uint256 debt = vaultTotalAmount - vaultTotalReleased;
...
contract TokenSale is AccessControl, Whitelist, Finalizable, BonusHandler, Initializable {
    ...
    function initialize(address _buntToken, address _usdToken, address _wallet, address _priceFeedAddress, address _vestingVault) initializer public {
        ...
        buntToken = BuntToken(_buntToken);
    ...
    function initializeTokenSale(uint256 _USDRate, uint256 _minPurchase, uint256 _maxPurchase, uint256 _softcap, uint256 _hardcap) onlyRole(ADMIN_ROLE) public isInitialized hasRequiredRole {
    ...
    function initializeQUICK() public {

        initialize(0x929256647B5A8151f8e1097D02a87e85F99B217e, 0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee, 0x9e7eC99a816b3Adc3B2a2777F115cb1fc25D43a1, 0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526, 0x171098335a8E5E2A5930e1B7e96D48a3908d50a8);

        initializeTokenSale(400, 1 * 10 ** 18, 100 * 10 ** 18, 1 * 10 ** 18, 480000 * 10 ** 18);

        initVestingEvent("seed", block.timestamp + (1 days), 10, 24, 3 minutes);
    ...
    function bonusUpdate(uint[] memory _bonusLimits, uint[] memory _bonusRates) public isInitialized onlyRole(ADMIN_ROLE) {

        _bonusUpdate(_bonusLimits, _bonusRates);

    }



    function purchase(address beneficiary,uint256 amount,bytes32 paymentOption) public payable isTokenSaleInitialized isOpen isWhitelisted(beneficiary){

        if(paymentOption == keccak256("USD")){

            purchaseInUSD(beneficiary, amount);
    ...
    function purchaseInUSD(address beneficiary, uint256 purchaseAmount) public isTokenSaleInitialized isOpen isWhitelisted(beneficiary) {
        ...
        uint256 bonus = calculateBonusRate(purchaseAmount);
        ...
        _USDRaised = _USDRaised + purchaseAmount;
        ...
        //Mint and Lock Tokens in vesting vault instead of transfering



        //buntToken.mint(address(vestingVault), tokens);

        vestingVault.addVesting(beneficiary, tokens, address(this));



        emit TokensPurchased(msg.sender, beneficiary, purchaseAmount, tokens);
    ...
    function purchaseInBNB(address beneficiary) public payable isTokenSaleInitialized isOpen isWhitelisted(beneficiary) {
        ...
        uint256 bonus = calculateBonusRate(USDperTokens(tokens));
        ...
        _BNBRaised = _BNBRaised + purchaseAmount;
        ...
        emit TokensPurchased(beneficiary, beneficiary, purchaseAmount, tokens);

    }



    function withdrawTokens(address _token) internal {
    ...
    function withdrawOtherERC20(address _token) public isTokenSaleInitialized onlyRole(ADMIN_ROLE) {

        

        //This stops admins from stealing the allocated tokens of the investors.

        ///BUNT tokens should remain in this contract.

        require(_token != address(buntToken), "Withdrawing Bunt tokens is not allowed");



        //USD can only be withdrawn from withdrawUSD() method

        require(_token != address(usdToken), "Withdrawing USD tokens is not allowed");
    ...
    function tokensRemaining() public view returns (uint256) {
        ...
        return tokensAllocated() - tokensPerUSD(_amountRaisedInUSD);
    ...
    function contribution(address _purchaser) public view returns (uint256) {
    ...
    function _prePurchaseValidate(address beneficiary, uint256 purchaseAmount, uint256 tokensUSDPrice) internal view {
        ...
        require (purchaserList[beneficiary].amount + tokensUSDPrice <= maxPurchase, "Amount is larger than allowed maxPurchase price");
