...
contract Witty {
    ...
    function createEscrow(address _buyer,uint _tradeid,uint _sellAmt,uint8 _choice) public isLock {
        ...
        if (_choice == 0) {
            require(_tradingDetail.sellerAmt >= _sellAmt.add(_refPercent),"Seller not have enough money");
        }
        else {
        ...
        inrCount[_tradeid]++;
        _tradingDetail.sellerAmt = _tradingDetail.sellerAmt.sub(_sellAmt);
    ...
    function _buyerSellerConfimationFiat(uint _tradeId,uint _amt,address _buyer,uint _time,uint8 sellchoice) public isLock {
        ...
        if (_Trade.types == 1) {
            ...
            if (sellchoice == 0) {
            _tradingDetail.sellerAmt = _tradingDetail.sellerAmt.sub(
                _refPercent);
            }
