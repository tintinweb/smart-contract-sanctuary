...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    /**
     * @dev Initializes the contract by sting a `name` and a `symbol` to the token collection.
     */
    constructor(string memory name_, string memory symbol_) {
        _name = name_;
        _symbol = symbol_;
...
contract SharkieTicket is ERC721Enumerable, Ownable {
  ...
  constructor(ESHK token) ERC721('eShark Token', 'ESHK') {
    _token = token;
  }
  ...
  function mintNFT() public onlyOwner returns (uint256) {
    require(bytes(baseTokenURI).length > 0, 'Must set BaseURI before mint NFT');
