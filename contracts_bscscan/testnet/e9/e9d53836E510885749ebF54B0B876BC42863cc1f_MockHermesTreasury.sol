...
contract MockHermesTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public sHERMES;
  uint256 public sHERMESQueue; // Delays change to sHERMES address
  ...
  constructor(
    ...
    isReserveToken[_BAI] = true;
    reserveTokens.push(_BAI);

    // isLiquidityToken[ _HERMESBAI ] = true;
    // liquidityTokens.push( _HERMESBAI );

    blocksNeededForQueue = _blocksNeededForQueue;
  }

  /**
        @notice allow approved address to deposit an asset for HERMES
        @param _amount uint
  ...
  function deposit(
    ...
    (_token, _amount);
    // mint HERMES needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(HERMES).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn HERMES for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOfToken(_token, _amount);
    IHERMESERC20(HERMES).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sHERMES).balanceOf(msg.sender); // Can only borrow against sHERMES held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with HERMES
        @param _amount uint
     */
  function repayDebtWithHERMES(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");

    IHERMESERC20(HERMES).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, HERMES, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(HERMES).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(HERMES).totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns HERMES valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  {
    if (isReserveToken[_token]) {
      // convert amount to match HERMES decimals
      value_ = _amount.mul(10**IERC20(HERMES).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SHERMES) {
      // 9
      sHERMESQueue = block.number.add(blocksNeededForQueue);
    } else return false;
