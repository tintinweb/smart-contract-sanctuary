...
contract WatchdogMigrator is Auth {
    ...
    address public whitelistAdr = 0x4a502f5c906769E45c7fDD67A56879927E2eeaad;
    ...
    event Deposited(address adr, uint amount);
    event Claimed(address adr, uint amount);
    ...
    function getBonusNumerator(uint timestamp) public view returns (uint) {
    ...
    function getAllDepositsOfAdr(address user) external view returns (Deposit[] memory) {
    ...
    function editDeposit(address user, uint depositId, uint216 amount, uint40 timestamp) external authorizedFor(Permission.AdjustVariables) {
        deposits[user][depositId].amount = amount;
        deposits[user][depositId].timestamp = timestamp;
    }
