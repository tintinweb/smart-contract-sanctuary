...
/**
 * @dev Interface of the IWedding.
 */
interface IWedding {
    
    function getWyAmout(address player) external view returns(uint);
}
...
contract LoserUyang is ERC1155Holder {
    ...
    address public weddingContract;
    ...
    uint256 public basePerValue = 100000 ether;
    ...
    mapping(address => uint256) public uyangPendingWithdrawals;

    mapping(address => uint256) public uyangClaimed;
    ...
    function cliamUyang() public {
        IERC20 uyang = IERC20(uyangStone);
        IWedding wedding = IWedding(weddingContract);
        require(claimSwitch, "cliam close");
        require(uyangClaimed[msg.sender] == 0, "you have claim");
        uint amount = wedding.getWyAmout(msg.sender);
        require(amount > 0, "you can not claim uyang");
        require(uyang.transfer(msg.sender, amount), "uyang transfer failed");
        uyangClaimed[msg.sender] = amount;
    }
    ...
    function depositUyang(uint256 amount) public onlyOwner {
        require(amount > 0, "You deposit nothing!");
    ...
    function setCliamSwitch(bool open_) public onlyOwner {
        claimSwitch = open_;
    }
    ...
    function setBasePerValue(uint256 basePerValue_) public onlyOwner {
        if (basePerValue_ >= 0) {
            basePerValue = basePerValue_;
        }
    }
    ...
    function bet_1(uint256 numberSelected) public payable {
        uint256 betValue1 = basePerValue;
        ...
        treasury.sendAllTreasuryBalance(betValue1 / 2);
        pendingWithdrawals[address(this)] += betValue1 - (betValue1 / 2);
    ...
    function distributePrizes_1(uint256 numberWin) private {
        ...
        //only one can receive nft
        for (uint256 i = 0; i < players_1.length; i++) {
            address playerAddress = players_1[i];
            if (countWin <=1 && playerInfo_1[playerAddress].numberSelected == numberWinner_1) {
                winners[countWin] = playerAddress;
                sendNftTo(playerAddress, numberMapNftId_1[numberWinner_1], 1);
                countWin++;
            } else {
    ...
    function pullFundsNft(uint256 id) public onlyOwner {
        ...
        uint256 amount = nftToken.balanceOf(address(this), id);
    ...
    function getContractFee() public onlyOwner {
        IERC20 lowb = IERC20(lowbTokenAddress);
        require(lowb.transfer(owner, pendingWithdrawals[address(this)]), "Lowb transfer failed");
        pendingWithdrawals[address(this)] = 0;
    }

    function receiveUyang(uint256 amout) public onlyOwner {
        IERC20 uyang = IERC20(uyangStone);
        require(uyang.transfer(owner, amout), "uyang transfer failed");
    }
    ...
    function sendNftTo(
        ...
        IERC1155 nftToken = IERC1155(uyangTonesGates);
        if (nftToken.balanceOf(address(this), id) >= nftAmout) {
