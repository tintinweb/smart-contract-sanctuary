 ...
 * $DWAC
 *
 * DWAC stands for transparent, high-quality and healthy financial plans
...
contract DWAC is Context, Ownable, IERC20 {
    ...
    string private _name = "SPAC1";
    string private _symbol = "DWAC1";
    ...
    feeRatesStruct public buyFees = feeRatesStruct(
     {taxFee: 5000,
      ...
      giveawaysFee: 663,
    //   iSPACFee: 2000,
      swapFee: 5000, // devFee+marketingFee+giveawaysFee
      totFees: 13
    });
    ...
    feeRatesStruct public sellFees = feeRatesStruct(
     {taxFee: 5000,
      ...
      giveawaysFee: 663,
    //   iSPACFee: 2000,
      swapFee: 5000, // devFee+marketingFee+giveawaysFee+iSPACFee
      totFees: 13
    });
    ...
    address payable public marketingWallet = payable(0xe1824AEA5A32C8F696650F56bb867de927176b5B);
    address payable public devWallet = payable(0xCA6458F7AE71B055a7523c4Ff45CCf48FB12DB4E);
    address payable public giveawaysWallet = payable(0x8b4c0DB402a79E0E5919BF5B96adD58d2f6c2938);
    // address payable public iSPACFee = payable(0x075bb56673f9341640d32f89045cfbe729dd77b2);
    ...
    constructor () {
        ...
        // _isExcludedFromFee[iSPACFee] = true;
   ...
   function setMarketingWallet(address payable _address) external onlyOwner returns (bool){
    ...
    // function setISPACPOOL(address payable _address) external onlyOwner returns (bool){
    //     iSPACFee = _address;
    //     _isExcludedFromFee[iSPACFee] = true;
    //     return true;
    // }
    
    // function setBuyFees(uint256 taxFee, uint256 devFee, uint256 marketingFee, uint256 giveawaysFee, uint256 iSPACFee) external onlyOwner{
    //     buyFees.taxFee = taxFee;
    //     buyFees.devFee = devFee;
    //     buyFees.marketingFee = marketingFee;
    //     buyFees.giveawaysFee = giveawaysFee;
    //     buyFees.iSPACFee = iSPACFee;
    //     buyFees.swapFee = devFee+marketingFee+giveawaysFee+iSPACFee;
    //     require(buyFees.swapFee+buyFees.taxFee == 10000, "sum of all percentages should be 10000");
    // }
    
       function setBuyFees(uint256 taxFee, uint256 devFee, uint256 marketingFee, uint256 giveawaysFee) external onlyOwner{
        buyFees.taxFee = taxFee;
        buyFees.devFee = devFee;
        buyFees.marketingFee = marketingFee;
        buyFees.giveawaysFee = giveawaysFee;
        buyFees.swapFee = devFee+marketingFee+giveawaysFee;
        require(buyFees.swapFee+buyFees.taxFee == 10000, "sum of all percentages should be 10000");
    }
    
    // function setSellFees(uint256 sellTaxFee, uint256 sellDevFee, uint256 sellMarketingFee, uint256 sellGiveawaysFee, uint256 sellISPACFee) external onlyOwner{
    ...
    //     sellFees.iSPACFee = sellISPACFee;
    //     sellFees.swapFee = sellDevFee+sellMarketingFee+sellGiveawaysFee+sellISPACFee;
    //     require(sellFees.swapFee+sellFees.taxFee == 10000, "sum of all percentages should be 10000");
    // }
    
    function setSellFees(uint256 sellTaxFee, uint256 sellDevFee, uint256 sellMarketingFee, uint256 sellGiveawaysFee) external onlyOwner{
        sellFees.taxFee = sellTaxFee;
        sellFees.devFee = sellDevFee;
        sellFees.marketingFee = sellMarketingFee;
        sellFees.giveawaysFee = sellGiveawaysFee;
        sellFees.swapFee = sellDevFee+sellMarketingFee+sellGiveawaysFee;
        require(sellFees.swapFee+sellFees.taxFee == 10000, "sum of all percentages should be 10000");
    }
    ...
    function swapAndSendToFees(uint256 tokens) private {
        ...
        // iSPACFee.sendValue(transferBalance*appliedFees.iSPACFee/appliedFees.swapFee);
