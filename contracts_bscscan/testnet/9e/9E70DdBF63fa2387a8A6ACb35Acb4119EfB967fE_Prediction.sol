...
contract Prediction is Pausable {
     ...
     * callable by admin
     */
    function setFundWallet(address _fundWallet) external onlyAdmin {
     ...
     * callable by admin
     */
    function setDividendWallet(address _dividendWallet) external onlyAdmin {
     ...
     * callable by admin
     */
    function setRefRates(uint256[] memory _refRates) external onlyAdmin {
     ...
     * callable by admin
     */
    function setOperationFee(uint256 rate, address token) external onlyAdmin {
        if (rate == 0) {
            rate = defaultFeeRate;
        }
     ...
     * callable by admin
     */
    function setMinPredictionAmount(uint256 _minPredictionAmount) external onlyAdmin {
        minPredictionAmount = _minPredictionAmount;
     ...
     */
    function genesisStartRound(uint256 price) public onlyAdmin {
        startBlock = block.number;
        uint256 epoch = getCurrentEpoch();
        rounds[epoch].startBlock = startBlock;
        rounds[epoch].endBlock = rounds[epoch].startBlock + roundDuration;
        rounds[epoch].openPrice = price;
    }

    /**
     * @dev Start the next round n, lock price for round n-1, end round n-2
     */
    function endRound(uint256 epoch, uint256 currentPrice, string memory metadata) external onlyOperator whenNotPaused {
    ...
    function predict(uint256 epoch, uint256 side, uint256 amount, address token, address parent) external payable whenNotPaused {
        if (token == address(0)) {
            amount = msg.value;
        }
        ...
        require(epoch == getCurrentEpoch(), "Round not matched");
    ...
    function claim(uint256 epoch) external {
        ...
        //        require(predictionHistory[epoch][msg.sender].amount > 0, "User has not predicted");
        ...
        for (uint256 index = 0; index < round.listToken.length; index++) {
            ...
            round.endBlock + 20 * roundDuration <= block.number) {
                info.claimed = true;
                _claimFund(amount, msg.sender, token);
                emit Claim(msg.sender, epoch, token, amount);
                continue;
     ...
     * callable by admin
     */
    function withdraw(uint256 amount, address token) external onlyAdmin {
    ...
    function getCurrentEpoch() public view returns (uint256) {
        if (roundDuration == 0 || startBlock == 0) {
            return 0;
        }
