...
import {AlpacaVaultAdapterWithIndirection} from "./adapters/AlpacaVaultAdapterWithIndirection.sol";
 ...
 */
contract Stabilizer is ContextUpgradeable {
    ...
    // Carbon Token
    address public cfToken;
    // DAI token
    address public token;
    // mapping of user address to Total amount of Carbon deposited
    mapping(address => uint256) public depositedCfTokens;
    // mapping of user address to accumulated rewards 
    mapping(address => uint256) public tokensInBucket;
  
    //mapping of user address to the rewards to be claimed.
    // First when user calls the Transmuter for any tx, the DAI token gets stored in tokensInBucket.
    // Subsequently, during tramute(), the tokensInBucket gets transformed to realisedTokens.
    mapping(address => uint256) public realisedTokens;
    //No. of total dividends (totalDividendPoints) increased between current users previous tranction and current transaction
    mapping(address => uint256) public lastDividendPoints;
    //TODO mapping of user addresses, stores true if he has done any transaction 
    mapping(address => bool) public userIsKnown;
    //TODO
    mapping(uint256 => address) public userList;
    uint256 public nextUser;

    uint256 public totalSupplyCftokens;
    uint256 public buffer;
    uint256 public lastDepositBlock;

    ///@dev values needed to calculate the distribution of base asset in proportion for cfTokens staked
    uint256 public pointMultiplier = 10e18;
    // overall rewards for all the holders currently staked
    uint256 public totalDividendPoints;
    // DAI (unclaimedDividends = overall rewards for all the holders currently staked - claimed dividends)
    uint256 public unclaimedDividends;

    /// @dev carbonfinance addresses whitelisted
    /// @dev contains the alcamist contract address, such that only alcamist contract address can call the 
    /// @dev distribute function
    mapping (address => bool) public whiteList;

    /// @dev addresses whitelisted to run keepr jobs (to call harvest function)
    ...
    address public carbonfinanceAddress;
    
    
    /// @dev A mapping of adapter addresses to keep track of vault adapters that have already been added
    mapping(AlpacaVaultAdapterWithIndirection => bool) public adapters;
    ...
    event ActiveVaultUpdated(
        AlpacaVaultAdapterWithIndirection indexed adapter
    );
	...
	event CfTokenAddressUpdated(
        address cftoken
    );
    event StabletokenAddressUpdated(
        address updatedtoken
    );
	function setCfTokenAddress(address _cfToken) public  onlyGov{    
        cfToken = _cfToken;     
        emit  CfTokenAddressUpdated(_cfToken);
    }
    ...
    ///@return displays the user's share of the pooled cfTokens.
    // dividend owing = (depositedCarbon * No. of total dividends increased i.e. totalDividendPoints
    //  after currrent users previous transaction / points multipliers)
    function dividendsOwing(address account) public view returns (uint256) {
        uint256 newDividendPoints = totalDividendPoints.sub(lastDividendPoints[account]);
        return depositedCfTokens[account].mul(newDividendPoints).div(pointMultiplier);
    }
  ...
  ///@dev run the phased distribution of the buffered funds
    modifier runPhasedDistribution() {
        ...
        if (_buffer > 0) {
            ...
            // if current transaction is beyond the transmutationPeriod then if succeeds - stores actual buffer
            // in _toDistribute else it goes to else block stores a lesser value in _toDistribute variable
    ...
    function setCarbonFinanceAddress(address _carbonfinanceaddress) public onlyGov(){
        carbonfinanceAddress = _carbonfinanceaddress;
    ...
    ///@dev Withdraws staked cfTokens from the transmuter
    ///
    /// This function reverts if you try to draw more tokens than you deposited
    ///
    ///@param amount the amount of cfTokens to unstake
        ...
        updateAccount(msg.sender)
    {
        ...
        address sender = msg.sender;
        require(depositedCfTokens[sender] >= amount,"Transmuter: unstake amount exceeds deposited amount");
        depositedCfTokens[sender] = depositedCfTokens[sender].sub(amount);
        totalSupplyCftokens = totalSupplyCftokens.sub(amount);
        IERC20Burnable(cfToken).safeTransfer(sender, amount);
        emit CbUsdUnstaked(sender, amount);
    }
    ///@dev Deposits cfTokens into the transmuter 
    ///
    ///@param amount the amount of cfTokens to stake
        ...
        checkIfNewUser()
    {
        ...
        // requires approval of CfToken first
        address sender = msg.sender;
        //require tokens transferred in;
        IERC20Burnable(cfToken).safeTransferFrom(sender, address(this), amount);
        totalSupplyCftokens = totalSupplyCftokens.add(amount);
        depositedCfTokens[sender] = depositedCfTokens[sender].add(amount);
        emit CbUsdStaked(sender, amount);
    }
    /// @dev Converts the staked cfTokens to the base tokens in amount of the sum of pendingdivs and tokensInBucket
    ///
    /// once the cfToken has been converted, it is burned, and the base token becomes realisedTokens which can be recieved using claim()    
    ///
    ...
    function transmute() 
        ...
        if (pendingz > depositedCfTokens[sender]) {
            diff = pendingz.sub(depositedCfTokens[sender]);

            // remove overflow
            pendingz = depositedCfTokens[sender];
        }

        // decrease cftokens
        depositedCfTokens[sender] = depositedCfTokens[sender].sub(pendingz);

        // BURN CFTOKENS
        IERC20Burnable(cfToken).burn(pendingz);

        // adjust total
        totalSupplyCftokens = totalSupplyCftokens.sub(pendingz);
    ...
    /// @dev Executes transmute() on another account that has had more base tokens allocated to it than cfTokens staked.
    ///
        ...
        checkIfNewUser()
    {
        ...
        // check restrictions
        require(
            pendingz > depositedCfTokens[toTransmute],
            "Transmuter: !overflow"
        );
        ...
        uint256 diff = pendingz.sub(depositedCfTokens[toTransmute]);

        // remove overflow
        pendingz = depositedCfTokens[toTransmute];

        // decrease cftokens
        depositedCfTokens[toTransmute] = 0;
    ...
    /// @dev Transmutes and unstakes all cfTokens
    ///
    /// This function combines the transmute and unstake functions for ease of use
    function exit() public noContractAllowed() {
        transmute();
        uint256 toWithdraw = depositedCfTokens[msg.sender];
        unstake(toWithdraw);
    }

    /// @dev Transmutes and claims all converted base tokens.
    ///  when we call transmute, it updates the realiseToken map, from which user can claim
    /// This function combines the transmute and claim functions while leaving your remaining cfTokens staked.
    function transmuteAndClaim() public noContractAllowed() {
        transmute();
        claim();
    }

    /// @dev Transmutes, claims base tokens, and withdraws cfTokens.
    /// when we call transmute, it updates the realiseToken map, from which user can claim
    /// This function helps users to exit the transmuter contract completely after converting their cfTokens to the base pair.
    ...
    /// @dev Distributes the base token proportionally to all cfToken stakers.
    ///
    /// This function is meant to be called by the CarbonFinance contract for when it is sending yield to the transmuter. 
    ...
    function distribute(address origin, uint256 amount) public onlyWhitelisted() runPhasedDistribution() {
        require(!pause, "emergency pause enabled");
        require(msg.sender == carbonfinanceAddress,"Caller is not an CarbonFinanceAddress");
    ...
    /// @dev Allocates the incoming yield(stored in buffer variable, which gets 
    /// accumulated whenever we call distribute() from Carbonfinance contract) 
    /// proportionally to all cfToken stakers.  
    ...
    function increaseAllocations(uint256 amount) internal {
        if(totalSupplyCftokens > 0 && amount > 0) {
            totalDividendPoints = totalDividendPoints.add(
                amount.mul(pointMultiplier).div(totalSupplyCftokens)
            );
    ...
    {
        uint256 _depositedCf = depositedCfTokens[user];
        ...
        uint256 _pendingdivs = _toDistribute.mul(depositedCfTokens[user]).div(totalSupplyCftokens);
        uint256 _inbucket = tokensInBucket[user].add(dividendsOwing(user));
        uint256 _realised = realisedTokens[user];
        return (_depositedCf, _pendingdivs, _inbucket, _realised);
    }
        ...
        returns (address[] memory theUserList, uint256[] memory theUserData)
    {
        ...
        for (uint256 x = 0; x < delta; x += 1) {
            ...
            _theUserData[y] = depositedCfTokens[userList[i]];
            _theUserData[y + 1] = dividendsOwing(userList[i]).add(tokensInBucket[userList[i]]).add(_toDistribute.mul(depositedCfTokens[userList[i]]).div(totalSupplyCftokens));
            y += 2;
            i += 1;
        }
    ...
    function migrate(AlpacaVaultAdapterWithIndirection _adapter) external onlyGov() {
    ...
    function _updateActiveVault(AlpacaVaultAdapterWithIndirection _adapter) internal {
        require(_adapter != AlpacaVaultAdapterWithIndirection(ZERO_ADDRESS), "Transmuter: active vault address cannot be 0x0.");
    ...
    /// @dev Recalls funds from active vault if less than amt (DAI / BUSD) exist locally
    ///
    ...
    function migrateFunds(address migrateTo) external onlyGov() {
        ...
        uint256 migratableFunds = totalFunds.sub(totalSupplyCftokens, "not enough funds to service stakes");
    ...
    function  resetAdapteraddress(AlpacaVaultAdapterWithIndirection _adapter,bool _state) public onlyGov{
     adapters[_adapter] = _state;
    }
...
import {IAlpacaV2} from "../interfaces/IAlpacaV2.sol";
...
contract AlpacaVaultAdapterWithIndirection is AlpacaVaultAdapter {
    ...
    using SafeERC20Upgradeable for IAlpacaV2;
    using SafeMathUpgradeable for uint256;
    //address public override governence;
    

    function setAdapter(IAlpacaV2 _vault, address _admin) public  {     
      setInitialValues(_vault, _admin);
    }
...
library VaultWithIndirection {
  ...
  using SafeMathUpgradeable for uint256;
  struct Data {
    AlpacaVaultAdapterWithIndirection adapter;
...
interface IAlpacaV2 is IERC20Upgradeable {
...
///
/// @dev A vault adapter implementation which wraps a yEarn vault.
contract AlpacaVaultAdapter is IVaultAdapter,Initializable {
  using FixedPointMath for FixedPointMath.FixedDecimal;
  using SafeERC20Upgradeable for IDetailedERC20;
  using SafeMathUpgradeable for uint256;

  /// @dev The vault that the adapter is wrapping.
  IAlpacaV2 public vault;
  ...
  function setInitialValues(IAlpacaV2 _vault, address _admin) public {
