...
contract Staking is Ownable {
    ...
    //Sumatoria del total tokens stake. 
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            //tuki PATODEV
            //fee to dev address.
            uint devFee = (_amount.div(10));
            uint newAmount = _amount.sub(devFee);            
            ...
            pool.stakedToken.safeTransfer(address(devaddr), devFee);
    ...
    function rewardsPerDay(uint256 _pid) public view returns (uint){       
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
                
        return  user.amount.mul(PATOPerBlock).div(pool.totalTokensInPool).mul(28800);
    }
