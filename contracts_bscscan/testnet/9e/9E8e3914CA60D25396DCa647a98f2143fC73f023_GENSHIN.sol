...
contract GENSHIN is Context, IERC20, Ownable {
    ...
    address public marketingAddress = 0xbd4aF599EC5A936592FeCae0F383D541d707eb74;
    address public destroyAddress = 0x506Fd90B90F7858A85FDCA2B01628eC9e371bf6F;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "Genshin NFT";
    string private constant _symbol = "GENSHIN";
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[destroyAddress] = true;
   ...
   function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
    ...
    function setBuyFeeRates(uint256 _rfi, uint256 _destroy, uint256 _marketing, uint256 _liquidity, uint256 _buyback) external onlyOwner {
        buyFeeRates.rfi = _rfi;
        buyFeeRates.destroy = _destroy;
        buyFeeRates.marketing = _marketing;
        buyFeeRates.liquidity = _liquidity;
        buyFeeRates.buyback = _buyback;
        emit FeesChanged();
    }

    function setSellFeeRates(uint256 _rfi, uint256 _destroy, uint256 _marketing, uint256 _liquidity, uint256 _buyback) external onlyOwner{
        sellFeeRates.rfi = _rfi;
        sellFeeRates.destroy = _destroy;
    ...
    function _takeDestroy(uint256 rDestroy, uint256 tDestroy) private {
        totFeesPaid.destroy +=tDestroy;
        if(_isExcluded[address(this)]){
             _tOwned[address(this)]+=tDestroy;
        }
        _rOwned[address(this)] +=rDestroy;
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rDestroy, to_return.rMarketing, to_return.rLiquidity, to_return.rBuyback) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tDestroy = tAmount*sellFeeRates.destroy/1000;
            ...
            s.tTransferAmount = tAmount-s.tRfi-s.tDestroy-s.tMarketing-s.tLiquidity-s.tBuyback;
        }
        else{
            s.tRfi = tAmount*buyFeeRates.rfi/1000;
            s.tDestroy = tAmount*buyFeeRates.destroy/1000;
            s.tMarketing = tAmount*buyFeeRates.marketing/1000;
            s.tLiquidity = tAmount*buyFeeRates.liquidity/1000;
            s.tBuyback = tAmount*buyFeeRates.buyback/1000;
            s.tTransferAmount = tAmount-s.tRfi-s.tDestroy-s.tMarketing-s.tLiquidity-s.tBuyback;
        }
        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rDestroy,uint256 rMarketing, uint256 rLiquidity, uint256 rBuyback) {
        ...
        rDestroy = s.tDestroy*currentRate;
        rMarketing = s.tMarketing*currentRate;
        rLiquidity = s.tLiquidity*currentRate;
        rBuyback = s.tBuyback*currentRate;
        rTransferAmount =  rAmount-rRfi-rDestroy-rMarketing-rLiquidity-rBuyback;
        return (rAmount, rTransferAmount, rRfi,rDestroy,rMarketing,rLiquidity, rBuyback);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        ...
        _takeDestroy(s.rDestroy,s.tDestroy);
        _takeLiquidity(s.rLiquidity,s.tLiquidity);
        _takeMarketing(s.rMarketing, s.tMarketing);
        _takeBuyback(s.rBuyback, s.tBuyback);
        emit Transfer(sender, recipient, s.tTransferAmount);
        emit Transfer(sender, address(this), s.tLiquidity + s.tDestroy + s.tBuyback);
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        uint256 denominator= (buyFeeRates.liquidity + buyFeeRates.buyback + buyFeeRates.destroy) * 2;
        uint256 tokensToAddLiquidityWith = tokens * buyFeeRates.liquidity / denominator;
        ...
        uint256 unitBalance= deltaBalance / (denominator - buyFeeRates.liquidity);
        uint256 bnbToAddLiquidityWith = unitBalance * buyFeeRates.liquidity;
        ...
        // Send BNB to destroyWallet
        uint256 destroyAmt = unitBalance * 2 * buyFeeRates.destroy;
        if(destroyAmt > 0){
          payable(destroyAddress).transfer(destroyAmt);
        }
    }
    ...
    function updateDestroyWallet(address newWallet) external onlyOwner{
        require(destroyAddress != newWallet ,'Wallet already set');
        destroyAddress = newWallet;
        _isExcludedFromFee[destroyAddress];
    }
