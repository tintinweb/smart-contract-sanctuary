...
contract PandoBox is ERC721Burnable, Ownable
{
    ...
    EnumerableSet.AddressSet private whitelistTransferable;
    uint256 public totalSupply;
    mapping (uint256 => NFTLib.info) public nftInfo;
    string baseURI;

    /*----------------------------CONSTRUCTOR----------------------------*/
    constructor(address _dataStorage, string memory _baseURI) ERC721("PandoBox NFT Token", "PBOX")
    {
    ...
    function isApprovedForAll(address _owner, address _operator) public view override returns (bool isOperator) {
        ...
        if (isWhitelistTransferable(_operator) == true) {
            return true;
        }
        return ERC721.isApprovedForAll(_owner, _operator);
    }

    function create(address _receiver) external onlyMinter{
        ...
        uint256 _randSeed = Random.computerSeed() % PRECISION + 1;
        uint256 _cur = 0;
        for (uint256 i = 0; i < _creatingProbability.length; i++) {
            ...
            if (_cur >= _randSeed) {
                ...
                emit PandoBoxCreated(_receiver, _tokenId, i);
                break;
            }
        }
    }
    ...
    function setDataStorageContract(address _newAddress) external onlyOwner {
        require(_newAddress != address(0), 'PandoBoxNFT: _newAddress is the zero address');
    ...
    function addWhitelistTransferable(address _addr) external onlyOwner returns (bool){
        require(_addr != address(0), "Token: _addr is the zero address");
        return EnumerableSet.add(whitelistTransferable, _addr);
    }

    function delWhitelistTransferable(address _addr) external onlyOwner returns (bool){
        require(_addr != address(0), "Token: _addr is the zero address");
        return EnumerableSet.remove(whitelistTransferable, _addr);
    }

    function getWhitelistTransferableLength() public view returns (uint256) {
        return EnumerableSet.length(whitelistTransferable);
    }

    function isWhitelistTransferable(address account) public view returns (bool) {
        return EnumerableSet.contains(whitelistTransferable, account);
    }

    function getWhitelistTransferable(uint256 _index) external view onlyOwner returns (address) {
        require(_index <= getWhitelistTransferableLength() - 1, "Token: index out of bounds");
        return EnumerableSet.at(whitelistTransferable, _index);
    }
...
library NFTLib {
    ...
    function getPower(uint256[] memory tokenIds, IDroidBot droidBot) external returns (uint256) {
...
interface IDataStorage {
    ...
    function getDroidBotCreatingProbability(uint256 _pandoBoxLevel) external view returns (uint256[] memory);
    function getDroidBotEvolvingProbability(uint256 _droidBot0Level, uint256 _droidBot1Level) external view returns(uint256[] memory);
    function getDroidBotPower(uint256) external view returns(uint256);
    function getJDroidBotCreating(uint256) external view returns(uint256);
    function getJDroidBotEvolving(uint256, uint256) view external returns(uint256);
}
...
interface IDroidBot is IERC721{
    function create(address receiver, uint256 pandoBoxLevel) external;
    function evolve(address receiver, uint256 droidBot0Level, uint256 droidBot1Level) external;
