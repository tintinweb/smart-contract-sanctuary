...
contract TokenCarrier is BuildableContext {
    ...
    IBEP20 private _liquidityToken;
    uint256 private _liquidityTokenDecimalFactor;

    constructor(address carrierToken, address liquidityToken) {
        _carrierToken = IBEP20(carrierToken);
        _carrierTokenDecimalFactor = 10**_carrierToken.decimals();

        _liquidityToken = IBEP20(liquidityToken);
        _carrierTokenDecimalFactor = 10**_liquidityToken.decimals();
    }
    ...
    function liquidityToken() public view returns (address) {
    ...
    function _purchaseRate() internal view returns (uint256) {
        return _carrierToken.totalSupply().mul(_liquidityTokenDecimalFactor).div(_liquidityToken.balanceOf(address(this)));
    }
    ...
    function purchase(uint256 liquidityToSwap) public returns (bool) {
        uint256 allowance = _liquidityToken.allowance(_msgSender(), address(this));
        uint256 avalible = _carrierToken.balanceOf(address(this));

        require(allowance >= liquidityToSwap, "Purchase: not enough liquidity allowance provided");

        uint256 owed = liquidityToSwap.mul(_purchaseRate()).div(_liquidityTokenDecimalFactor);

        require(avalible >= owed, "Purchase: not enough carrier token avalible");

        _liquidityToken.transferFrom(_msgSender(), address(this), liquidityToSwap);
    ...
    function _liquidationRate() internal view returns (uint256) {
        return _liquidityToken.balanceOf(address(this)).mul(_carrierTokenDecimalFactor).div(_carrierToken.totalSupply());
    }
    ...
    function liquidate(uint256 carriedToSwap) public returns (bool) {
        ...
        uint256 avalible = _liquidityToken.balanceOf(address(this));
        ...
        _liquidityToken.transfer(_msgSender(), owed);
