...
//SPDX-License-Identifier: MIT
    ...
    contract DividendDistributor is IDividendDistributor {
        ...
        uint256 public minDistribution = (1 / 100) * (10 ** 8);
        ...
        event Deposit(uint256 amountBNB, uint256 BNBAmount);
    ...
    contract THEBSCDAOPROJECT is IBEP20, Auth {
        using SafeMath for uint256;
    
        address WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
        address DEAD = 0x000000000000000000000000000000000000dEaD;
    
        string constant _name = "BSCDAO";
        string constant _symbol = "BSCD";
        uint8 constant _decimals = 9;
        uint256 _initialSupply = 1_000_000_000;
        ...
        uint256 private constant cooldownLimit = (60 seconds);
        ...
        uint256 devFee = 50;
        uint256 totalFee = buybackFee.add(reflectionFee).add(marketingFee).add(devFee).add(liquidityFee);
        uint256 feeDenominator = 10000;

        uint256 constant private maxReflectionFee = 1500;
        uint256 constant private maxLiquidityFee = 300;
        uint256 constant private maxMarketingFee = 300;
        uint256 constant private maxDevFee = 200;
        uint256 constant private maxBuybackFee = 200;
        ...
        uint256 private _maxTxMinimum = (_totalSupply * 1) / 1000;
        ...
        uint256 swapThreshold = _totalSupply.div(400);
        bool inSwap;
        modifier swapping() { inSwap = true; _; inSwap = false; }
    
        constructor (address d, address m, address c) Auth(msg.sender) {
            router = IDEXRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
            pair = IDEXFactory(router.factory()).createPair(WBNB, address(this));
            _allowances[address(this)][address(router)] = uint256(-1);
            distributor = new DividendDistributor(address(router));
            marketingFeeReceiver = payable(m);
            devFeeReceiver = payable(d); 
            charityReceiver = payable(c);
            ...
            autoLiquidityReceiver = payable(address(this));
            _balances[DEAD] = _totalSupply.mul(25).div(100);
            _balances[msg.sender] = _totalSupply.mul(70).div(100);
            _balances[devFeeReceiver] = _totalSupply.mul(2).div(100);              
            _balances[marketingFeeReceiver] = _totalSupply.mul(2).div(100);                      
            _balances[charityReceiver] = _totalSupply.mul(1).div(100);           
            emit Transfer(address(0), DEAD, _totalSupply.mul(25).div(100));            
            emit Transfer(address(0), devFeeReceiver, _totalSupply.mul(2).div(100));                   
            emit Transfer(address(0), charityReceiver, _totalSupply.mul(1).div(100));            
            emit Transfer(address(0), marketingFeeReceiver, _totalSupply.mul(2).div(100));            
            emit Transfer(address(0), msg.sender, _totalSupply.mul(70).div(100));
        }
        ...
        function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
            ...
            if (sender == pair && recipient != address(router) && !isFeeExempt[recipient] && cooldownEnabled) {
                require(buycooldown[recipient] < block.timestamp);
                buycooldown[recipient] = block.timestamp.add(_cooldown);        
            } else if (sender != pair && !isFeeExempt[sender] && cooldownEnabled){
        ...
        function clearStuckToken(address recipient) external onlyOwner {
            uint256 _balance = balanceOf(address(this));
            _basicTransfer(address(this), recipient, _balance);
        }

        function clearStuckTokens(address _token, address recipient) external onlyOwner returns (bool _sent){
            require(_token != address(0));
            uint256 _contractBalance = IBEP20(_token).balanceOf(address(this));
            _sent = IBEP20(_token).transfer(recipient, _contractBalance);
        }
        ...
        function setTxLimit(uint256 percent, uint256 divisor) external authorized {
            ...
            _maxTxAmount = _totalSupply.mul(percent).div(divisor);
            emit TxLimitUpdated(percent, divisor);
        }
        ...
        function _isFeeExemt(address holder) public view returns(bool) {
            return isFeeExempt[holder];
        }

        function _isDividendExempt(address holder) public view returns(bool) {
        ...
        function setFees(
            ...
            require(_liquidityFee <= maxLiquidityFee && _reflectionFee <= maxReflectionFee && _marketingFee <= maxMarketingFee && _devFee <= maxDevFee && _buybackFee <= maxBuybackFee);
            require(_liquidityFee.add(_reflectionFee).add(_marketingFee).add(_devFee).add(_buybackFee) <= 2500);
        ...
        function setSwapBackSettings(bool _enabled, uint256 _amount) external authorized {
            ...
            uint256 amountSwap = _totalSupply.mul(_amount).div(1000);
            swapThreshold = amountSwap;
        ...
        event TxLimitUpdated(uint256 percent, uint256 divisor);
