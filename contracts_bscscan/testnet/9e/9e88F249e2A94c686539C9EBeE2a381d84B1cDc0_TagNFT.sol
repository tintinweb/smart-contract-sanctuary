...
contract TagNFT is AccessControl , ERC721URIStorage , Pausable {
    ...
    modifier isNotMinted( string memory Name ){
        ...
        require(isMint[hash] == false);
        _;
    }

    event mintEvent( string  _id , bytes32 _hash , uint256 _counter );

    constructor(string memory name_, string memory symbol_) AccessControl() Pausable() ERC721URIStorage( ) ERC721( name_ , symbol_ ){
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setRoleAdmin(PAUSE_ROLE, DEFAULT_ADMIN_ROLE);
        _setRoleAdmin(MINTER_ROLE, DEFAULT_ADMIN_ROLE);
    }

    function pauseToken () onlyRole(PAUSE_ROLE) external returns (bool){
        _pause();
        return true;
    }

    function unpauseToken () onlyRole(PAUSE_ROLE) external returns (bool){
        _unpause();
        return true;
    }

    function mint( address to , string memory tokenDetails ) isNotMinted(tokenDetails) onlyRole(MINTER_ROLE) external returns ( bool ){
        tokenID = tokenID + 1 ;
