...
/**
** 
 * South of heaven - The "compound" between heaven and hell
 * 
 *      
 *  Transfer Fee:  5%
 *  Buy Fee:       5%
 ...
 * 
 *  Fees Go Toward:
 *  15% Reflection Distribution in Binance-PEG compound coin
 * (add 0x52ce071bd9b1c4b00a0b92d298c512478cad67e8 in your trustwallet)
 * 
 *  5% Buyback and burn
 *  2% Auto Liquidity
 *  3% Marketing
...
interface IDividendDistributor {
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution, uint256 _bnbToReflectionThreshold) external;
...
contract DividendDistributor is IDividendDistributor {
    ...
    // Southofheaven Contract
    address _token;
    // Share of the Reflection Pie
    struct Share {
        uint256 amount;
        uint256 totalExcluded;
        uint256 totalRealised;
    }
    // reflection contract address
    address RT = 0x52CE071Bd9b1C4B00A0b92D298c512478CaD67e8; //compound
        // binance-peg compound: 0x52ce071bd9b1c4b00a0b92d298c512478cad67e8
    // bnb address
    ...
    // 1 Million Reflection Minimum Distribution
    uint256 public minDistribution = 1 * (10 ** 15);
    // BNB Needed to Swap to Reflection
    uint256 public swapToReflectionThreshold = 1 * (10 ** 18);
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution, uint256 _bnbToReflectionThreshold) external override onlyToken {
        minPeriod = _minPeriod;
        minDistribution = _minDistribution;
        swapToReflectionThreshold = _bnbToReflectionThreshold;
    }
    ...
    function deposit() external override onlyToken {
        ...
        if (bnbBalance >= swapToReflectionThreshold) {
            ...
            router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: swapToReflectionThreshold}(
                0,
                path,
                address(this),
                block.timestamp
    ...
    function setReflectionAddress(address nRTN) external onlyToken {
        RT = nRTN;
            }
...
/** 
 * Contract: Southofheaven 
 * 
 *  This contract is forked from SafeEarn (credit woofydev) with some small tweaks / improvements to gas
 *  This Contract Awards Reflection Daily to holders, weighted by how much you hold
 *  Reflection Burn Wallet Gains 2% of the Distribution (contributing to asynchronous burning)
 *  This is due to the fact that we sent 2% of the supply to Reflection's Burn Wallet on Launch
 *  
 *  Transfer Fee:  5%
 *  Buy Fee:       5%
 *  Sell Fee:     25%
...
contract SouthofHeaven is IERC20, Context, Ownable {
    ...
    // our burn wallet address - separate from Reflection's
    ...
    uint256 public totalBNBReflectionReflections = 0;
    ...
    constructor (
        ...
        // Liquidity Pool Address for BNB -> SOH
        ...
        // exempt this contract, the LP, and OUR burn wallet from receiving Reflection Rewards
    ...
    /** Takes Associated Fees and sets holders' new Share for the Reflection Distributor */
    ...
    /** Basic Transfer with no swaps for BNB -> SOH or SOH -> BNB */
    ...
    /** True if we should swap from SOH => BNB */
    ...
    /**
     *  Swaps SOH for BNB if threshold is reached and the swap is enabled
     *  Uses BNB retrieved to:
     *      fuel the contract for buy/burns
     *      provide distributor with BNB for Reflection
    ...
    /** Transfers BNB to Reflection Distributor and Marketing Wallet */
    function transferToDistributorAndMarketing(uint256 distributorBNB, uint256 marketingBNB) internal {
        ...
        if (success) {
            try distributor.deposit() {totalBNBReflectionReflections = totalBNBReflectionReflections.add(marketingBNB);} catch {}
        }
    ...
    /**
     * Buys Southofheaven with bnb in the contract, sending to target address
     */ 
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution, uint256 _bnbToReflectionThreshold) external onlyOwner {
        distributor.setDistributionCriteria(_minPeriod, _minDistribution, _bnbToReflectionThreshold);
    }
    ...
    /**
     * Buy and Burn SOH with bnb stored in contract
     */ 
    function triggerSohBuyback(uint256 amount) public onlyOwner {
        buyTokens(amount, DEAD);
        emit SohBuyBackAndBurn(amount);
    }
    ...
    function setReflectionContractAddress(address nRTN) public onlyOwner {
        distributor.setReflectionAddress(nRTN);
    }
    ...
    /** Returns the Circulating Supply of SOH ( supply not owned by Burn Wallet ) */
    ...
    event SohBuyBackAndBurn(uint256 amountBNB);
}