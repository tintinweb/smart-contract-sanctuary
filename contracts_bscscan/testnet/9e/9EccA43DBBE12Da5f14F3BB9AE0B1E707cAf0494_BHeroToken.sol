  ...
  IRandomRequester
{
  ...
  event TokenCreateRequested(address to, bytes32 requestId);
  event TokenCreateFulfilled(address to, bytes32 requestId);
  ...
  // Mapping from owner address to fulfilled token request IDs.
  mapping(address => bytes32[]) public fulfilledTokenRequests;

  // Mapping from request ID to create token requests.
  mapping(bytes32 => CreateTokenRequest) public createTokenRequests;
  ...
  function setUpgradeCost(uint256[] memory value) external onlyRole(DESIGNER_ROLE) {
    upgradeCost = value;
  }
  ...
  function balanceOwnerOf(address to) external view returns (uint256) {
  ...
  function increaseClaimableTokens(address to, uint256 count) external onlyRole(CLAIMER_ROLE) {
    require(to != address(0), "Zero address");
    claimableTokens[to] += count;
  }

  /** Claims tokens. */
  function claim(uint256 count) external returns (uint256) {
    address to = msg.sender;
    uint256 size = tokenIds[to].length;
    require(size < tokenLimit, "Cannot claim anymore token");

    uint256 claimableCount = Math.min(count, claimableTokens[to]);
    uint256 mintCount = Math.min(claimableCount, tokenLimit - size);

    require(mintCount > 0, "No token to claim");
    claimableTokens[to] -= mintCount;

    requestCreateToken(to, mintCount);
    return mintCount;
  }
  ...
  /** Legacy. */
  function mintMultiple(uint256 count) external {
    mint(count);
  }
  ...
  function mint(uint256 count) public {
    ...
    requestCreateToken(to, count);
  }

  /** Requests a create token request. */
  function requestCreateToken(address to, uint256 count) internal {
    require(to != address(0), "Zero address");
    require(address(randomGenerator) != address(0), "Random generator is not initialized");
    bytes32 requestId = randomGenerator.requestRandom();
    createTokenRequests[requestId] = CreateTokenRequest(
      to,
      uint16(count),
      true, // Exist.
      0 // Seed, will be calculated later.
    );
    emit TokenCreateRequested(to, requestId);
  }

  /** Called by random generator. */
  function requestRandomFulfilled(bytes32 requestId, uint256 randomness)
    external
    override
    onlyRole(RANDOM_GENERATOR_ROLE)
  {
    CreateTokenRequest storage request = createTokenRequests[requestId];
    require(request.exist, "Request not exist");
    request.seed = randomness;
    fulfilledTokenRequests[request.to].push(requestId);
    emit TokenCreateFulfilled(request.to, requestId);
  }

  /** Processes fulfilled token request. */
  function processFulfilledTokenRequest() external {
    address to = msg.sender;
    bytes32[] storage requests = fulfilledTokenRequests[to];
    require(requests.length > 0, "No request to process");
    for (uint256 i = 0; i < requests.length; ++i) {
      bytes32 id = requests[i];
      CreateTokenRequest storage request = createTokenRequests[id];
      require(request.exist, "Request not exist");
      createToken(request.to, request.count, request.seed);
      delete createTokenRequests[id];
    }
    delete fulfilledTokenRequests[to];
  }
  ...
  function createToken(
    ...
    for (uint256 i = 0; i < count; ++i) {
      ...
      (seed, details) = createRandomToken(seed, id);
  ...
  function createRandomToken(uint256 seed, uint256 id) internal returns (uint256 nextSeed, uint256 encodedDetails) {
    ...
    nextSeed = seed;
    encodedDetails = addToken(details);
  }

  /** Adds a token. */
  function addToken(BHeroDetails.Details memory details) internal returns (uint256 encodedDetails) {
    address to = msg.sender;
    uint256[] storage ownedTokens = tokenIds[to];
    details.index = ownedTokens.length;
    ownedTokens.push(details.id);
    encodedDetails = tokenDetails[details.id] = details.encode();
    tokenOwners[details.id] = to;
  }

  /** Removes the specified token ID. */
  function removeToken(uint256 id) internal {
    address to = msg.sender;
    require(tokenOwners[id] == to, "Can only remove owned token");
  ...
  function upgrade(uint256 baseId, uint256 materialId) external {
    ...
    address to = msg.sender;
    require(tokenOwners[baseId] == to, "Can only upgrade owned token");
    require(tokenOwners[materialId] == to, "Can only upgrade owned token");
...
interface IRandomGenerator {
  ...
  function getResult(bytes32 requestId) external returns (uint256);
}

interface IRandomRequester {
  function requestRandomFulfilled(bytes32 requestId, uint256 randomness) external;
}

contract RandomGenerator is VRFConsumerBase, IRandomGenerator {
  event RequestRandom(bytes32 requestId);
  event RequestRandomFulfilled(bytes32 requestId);

  IRandomRequester public randomRequester;

  // Mapping from request ID to its result.
  ...
  /** Requests randomness */
  function requestRandom() public override returns (bytes32) {
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    ...
    emit RequestRandomFulfilled(requestId);
    fulfillRandomRequest(requestId);
  }

  /** Fulfill random for the requester. */
  function fulfillRandomRequest(bytes32 requestId) public {
    randomRequester.requestRandomFulfilled(requestId, results[requestId]);
  }

  function getResult(bytes32 requestId) external view override returns (uint256) {
