...
contract MetaFarm is  ERC165, IERC721 {
   ...
   function Set_FramerName(uint256 tokenId,string memory _farmername) public onlyOwnerOf(tokenId){
    ...
    //农民升级
    function _farmerLevelUP(uint256 tokenId)public onlyOwnerOf(tokenId){
      _farmer storage farmer = MetaFarmer[tokenId];
      uint256 farmer_upexp=60*farmer.level*farmer.level-60*farmer.level+15;
      uint256 farmer_upcoin=(576*farmer.level*farmer.level-1008*farmer.level+360)/(20+farmer.level);
