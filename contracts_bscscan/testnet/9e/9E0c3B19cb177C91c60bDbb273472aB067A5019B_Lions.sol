...
contract YieldToken is ERC20("SexyToken", "SEXY") {
	...
	constructor(address _contract) {
        ...
        rewardsEnd[_contract] = block.timestamp + 600; //631152000;
	...
	function getTotalClaimable(address _user) external view isValidAddress isContract returns(uint256) {
		...
		uint256 _time = min(block.timestamp, _rewardTime);
		uint256 _pending = ILions(msg.sender).balanceOG(_user).mul(BASE_RATE.mul((_time.sub(lastUpdates[_contract][_user])))).div(REWARD_SEPARATOR);
		return rewards[_contract][_user] + _pending;
	}
