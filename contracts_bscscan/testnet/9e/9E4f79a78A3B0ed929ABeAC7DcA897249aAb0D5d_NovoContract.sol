  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    //burn 50% of fee until totalSupply touches targetSupply
    {
      if (_burnStopped == false) {
        ...
        emit log_value("burnAmount", burnAmount);

        fee = fee.sub(burnAmount);
      }
    }

    emit log_value("burntFee", fee);
    ...
    //send treasury fee and liquidity fee
    {
      ...
      emit log_value("treasuryFee", treasuryFee);
      emit log_value("liquidityFee", liquidityFee);
    ...
    emit log_value("balanceOwner", _balances[owner()]);
    ...
    //divide fee to holders
    {
      ...
      emit log_value("divideFee", divideFee);
