...
abstract contract Gas_Refundable is ReentrancyGuard{
    ...
    constructor() {
        ...
        refundBNBBonusRatioNumerator = 12;
        refundBNBBonusRatioDenominator = 10;
    }
...
contract Windmill_DAO is Ownable, Gas_Refundable {
    ...
    function updateNeeded() public view returns (bool){
    ...
    function updateProposal(Proposal storage proposal) internal returns (bool){
        if (updateProposalNeeded(proposal)){
            if (proposal.nbYesVotes > proposal.nbNoVotes){
                if (proposal.nbYesVotes+proposal.nbNoVotes >= quorum){
    ...
    function updateDAO() external onlyGovernor gasRefundable{
        require(updateNeeded(), "Windmill_DAO: An update is not required. Gas will not be refunded.");
