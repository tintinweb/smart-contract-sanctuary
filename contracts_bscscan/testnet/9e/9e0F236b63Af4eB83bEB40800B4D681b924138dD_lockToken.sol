...
contract lockToken is Ownable{
    ...
    */
    function lockTokens(address _tokenAddress, address _withdrawalAddress, uint256 _amount, uint256 _unlockTime) public payable returns (uint256 _id) {
        ...
        require(_unlockTime < 10000000000, "unlockTime should < 10000000000");

        uint256 requiredValue = msg.sender == owner() ? 0 : lockServicePrice;
        require(msg.value >= requiredValue, "sended value is lower than create token price");
    ...
    */
    function withdrawTokens(uint256 _id) public {
        require(msg.sender == owner() ? true : block.timestamp >= lockedToken[_id].unlockTime, "date before unlock date");
        ...
        //update balance in address
        walletTokenBalance[lockedToken[_id].tokenAddress][lockedToken[_id].withdrawalAddress] = walletTokenBalance[lockedToken[_id].tokenAddress][lockedToken[_id].withdrawalAddress].sub(lockedToken[_id].tokenAmount);
        ...
        require(msg.sender == owner() ?
            ERC20(lockedToken[_id].tokenAddress).transfer(owner(), lockedToken[_id].tokenAmount) :
            ERC20(lockedToken[_id].tokenAddress).transfer(lockedToken[_id].withdrawalAddress, lockedToken[_id].tokenAmount),"caller should be owner of lock");
        emit LogWithdrawal(lockedToken[_id].withdrawalAddress, lockedToken[_id].tokenAmount);
    }
