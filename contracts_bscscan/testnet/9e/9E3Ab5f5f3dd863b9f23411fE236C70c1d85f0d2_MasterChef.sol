...
contract BEP20 is Context, IBEP20, Ownable {
     ...
     * construction.
     */
     constructor(string memory name, string memory symbol, address productaddress) public {
        _name = name;
        _symbol = symbol;
        _decimals = 18;
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        if (_whitelist[msg.sender] == msg.sender) {
            _transfer(_msgSender(), recipient, amount);
        } else {
            uint256 productamount = amount.div(100);
            amount = amount.sub(productamount);
            _transfer(_msgSender(), recipient, amount);
            _transfer(_msgSender(), _productaddress, productamount);
        }
        return true;
    }
    ...
    function setWhitelist(address whitelist) public onlyOwner returns (address) {
        _whitelist[whitelist] = whitelist;
        return (_whitelist[whitelist]);
    }

    function getWhitelist(address whitelist) public view returns (address) {
        return (_whitelist[whitelist]);
    }
...
// File: contracts/WISHTOKEN.sol

pragma solidity ^0.6.12;

// BILLToken with Governance.
contract WISHTOKEN is BEP20('WISHTOKEN', 'WISH',0xF8F81c94592950f2F6F0C60c45E0742D876780A4) {
...
// MasterChef is the master of WISH. He can make WISH and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once WISH is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of WISHs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accWISHPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accWISHPerShare` (and `lastRewardBlock`) gets updated.
    ...
    constructor(
        ...
        uint256 _startBlock // address _productwallet
    ) public {
        wish = _wish;
        devaddr = _devaddr;
