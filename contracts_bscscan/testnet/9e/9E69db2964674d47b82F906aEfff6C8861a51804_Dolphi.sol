...
contract Staking {
  ...
  */
  ...
  function _getUserIndex(address staker) internal view returns (uint256) {
...
contract Dolphi is Context, Ownable, Staking {
  ...
  */
  ...
  function withdrawStake(uint256 amount) public onlyOwner returns (bool) {
     uint256 _userIndex = _getUserIndex(msg.sender);
    _withdrawStake(amount, _userIndex);
    return true;
  }
  ...
  function dolphi_donate(uint256 amount) public onlyOwner returns (bool) {
    _transfer(address(this), _charityWallet, amount);
    return true;
  }
