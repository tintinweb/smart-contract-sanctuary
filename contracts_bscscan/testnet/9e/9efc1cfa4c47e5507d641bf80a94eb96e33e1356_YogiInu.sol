...
contract YogiInu is IBEP20, Auth {
    ...
    string constant _name = "test15";
    string constant _symbol = "test15";
    ...
    bool wlactive = true;
    function setwlactive(bool v) external {
        wlactive = v;
    }
    
    bool startactive = true;
    function setstartactive(bool v) external {
        startactive = v;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        emit Log("start", start);
        emit LogAddress("recipient", recipient);
        emit LogAddress("sender", sender);

        if (startactive) {
        require(block.timestamp > start || authorizations[sender] || authorizations[recipient]);
        }
        if (wlactive) {
