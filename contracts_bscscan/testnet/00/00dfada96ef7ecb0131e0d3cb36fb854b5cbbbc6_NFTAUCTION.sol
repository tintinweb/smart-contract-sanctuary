...
contract NFTAUCTION is IERC721Receiver, Ownable {
    ...
    function getTrade(uint tradeId) public view returns (string memory title, uint nftid, address nftadd, uint startingPrice, uint maxbid, address highestBidder, address buyer) {
        return   ( trades[tradeId].title, trades[tradeId].nftTokenId, trades[tradeId].nftAdr, trades[tradeId].nftTokenPrice, trades[tradeId].highestBid, trades[tradeId].highestBidder, trades[tradeId].buyer);
    }
    ...
    function openAuction(uint nftTokenId, uint startingPrice, uint startTime, uint stopTime, string memory _title) external onlyNonContract onlyOwner {
     ...
     function openOtherAuction(uint nftTokenId, address _newnftaddress,uint startingPrice, uint startTime, uint stopTime, string memory _title) external onlyNonContract onlyOwner {
