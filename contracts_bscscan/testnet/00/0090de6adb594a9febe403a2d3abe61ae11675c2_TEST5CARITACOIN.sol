...
interface IPreSale {
    ...
    function charityBuyForLiquidity() external payable;
...
contract PreSale {
    ...
    function charityBuyForLiquidity() public payable {
        ...
        uint amountOfToken = pancakeRouter.getAmountsIn(msg.value, getPathForTokenToBNB())[1];

        require(tokenContract.balanceOf(address(this)) >= amountOfToken, "There is not enought tokens");

        emit Sold(msg.sender, amountOfToken);
        tokensSold += amountOfToken;

        require(tokenContract.transfer(msg.sender, amountOfToken));
        userAdministration.updateUserGiftStats(address(msg.sender), msg.value, amountOfToken);
    }
    ...
    function changeToken (address _newTokenAddress, address _newPairAddress) public {
        require(userAdministration.isAuthorized(msg.sender) == true);
        tokenAddress = _newTokenAddress;
        pairAddress = _newPairAddress;
    }

    function changeRouter (address _newRouterAddress) public {
        require(userAdministration.isAuthorized(msg.sender) == true);
