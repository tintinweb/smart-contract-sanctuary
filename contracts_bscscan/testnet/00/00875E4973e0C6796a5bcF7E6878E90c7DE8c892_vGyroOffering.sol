...
contract vGyroOffering is Ownable {
    ...
    address public vGyro;
    address public tokenIn;
    address public treasury;

    uint256 public rate;
    uint256 public totalWhiteListed;
    uint256 public offeringEndBlock;
    ...
    modifier initialized() {
        require(offeringStarted, "Offering not started");
        _;
    }

    modifier notInitialized() {
        require(!offeringStarted, "Already initialized");
        _;
    }

    function whiteListBuyers(address[] memory _buyers) external onlyOwner() notInitialized() returns (bool) {
    ...
    function initialize(
        ...
        address vGyro_,
        uint256 rate_,
        uint256 offeringPeriod_
    ) external onlyOwner() notInitialized() returns (bool) {
        vGyro = vGyro_;
        tokenIn = tokenIn_;
        rate = rate_;
        offeringEndBlock = block.number.add(offeringPeriod_);
    ...
    function buyToken(uint256 tokenInAmount_) external initialized() returns (bool) {
        require(whiteListed[msg.sender], "Not whitelisted");
        require(!participants[msg.sender], "Already participated");
        require(block.number < offeringEndBlock, "Offering is over");
    ...
    function claimRemaining(address recipient_) external onlyOwner() initialized() returns (bool) {
        require(block.number >= offeringEndBlock, "Offering not ended");
