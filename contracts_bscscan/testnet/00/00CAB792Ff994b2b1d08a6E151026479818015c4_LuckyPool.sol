...
import "./LuckyStickNFT.sol";
import "./LuckyPoolOwnable.sol";
...
// 4 numbers
contract LuckyPool is LuckyPoolOwnable {
    ...
    // The TOKEN to buy lucky stick
    IERC20 public fat;
    // The LuckyStick NFT for tickets
    LuckyStickNFT public luckyStickNFT;
    // adminAddress
    ...
    // issueId => [tokenId]
    mapping (uint256 => uint256[]) public luckyPoolInfo;
    ...
    IStdReference public oracle; // IStdReference(0xDA7a001b254CD22e46d3eAB04d937489c93174C3)
    ...
    function _getBNBPriceAt(uint256 _decimal) internal view returns (uint256) {
        IStdReference.ReferenceData memory _data = oracle.getReferenceData("BNB", "BUSD");
        uint256 _BNB_price = _data.rate;
        return _BNB_price.div(10 ** (18 - _decimal)).mod(10);
    }

    function sumArray(uint8[4] memory input) internal pure returns (uint8) {
        uint8 S = 0;
        for (uint8 i = 0; i < 4; i++) {
            S = S + input[i];
        }
    ...
    function drawed() public view returns (bool) {
        return sumArray(winningNumbers) != 0;
    }
    ...
    function drawing() external onlyAdmin {
        ...
        require(block.timestamp > lastTimestamp + 5 minutes, "not cheating");

        winningNumbers[0]=uint8(_getBNBPriceAt(1));
        winningNumbers[1]=uint8(_getBNBPriceAt(2));
        winningNumbers[2]=uint8(_getBNBPriceAt(3));
        winningNumbers[3]=uint8(_getBNBPriceAt(4));
        require(sumArray(winningNumbers) != 0, "not zero");
    ...
    function internalBuy(uint256 _price, uint8[4] memory _numbers) internal {
        ...
        uint256 tokenId = luckyStickNFT.newLuckyStick(address(this), _numbers, _price, issueIndex);
        luckyPoolInfo[issueIndex].push(tokenId);
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        uint256 tokenId = luckyStickNFT.newLuckyStick(msg.sender, _numbers, _price, issueIndex);
        luckyPoolInfo[issueIndex].push(tokenId);
        ...
        fat.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        for (uint i = 0; i < _numbers.length; i++) {
            for (uint j = 0; j < 4; j++) {
                require (_numbers[i][j] <= maxNumber, 'exceed number scope');
            }
            uint256 tokenId = luckyStickNFT.newLuckyStick(msg.sender, _numbers[i], _price, issueIndex);
            luckyPoolInfo[issueIndex].push(tokenId);
            userInfo[msg.sender].push(tokenId);
            totalAmount = totalAmount.add(_price);
        ...
        lastTimestamp = block.timestamp;
        fat.safeTransferFrom(address(msg.sender), address(this), totalPrice);

        emit MultiBuy(msg.sender, totalPrice);
    }

    function claimReward(uint256 _tokenId) external {
        require(msg.sender == luckyStickNFT.ownerOf(_tokenId), "not from owner");
        require (!luckyStickNFT.getClaimStatus(_tokenId), "claimed");
        uint256 reward = getRewardView(_tokenId);
        luckyStickNFT.claimReward(_tokenId);
        if(reward>0) {
            fat.safeTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        for (uint i = 0; i < _tickets.length; i++) {
            require (msg.sender == luckyStickNFT.ownerOf(_tickets[i]), "not from owner");
            require (!luckyStickNFT.getClaimStatus(_tickets[i]), "claimed");
            luckyStickNFT.claimReward(_tickets[i]);
        ...
        if(totalReward>0) {
            fat.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function getRewardView(uint256 _tokenId) public view returns(uint256) {
        uint256 _issueIndex = luckyStickNFT.getLuckyStickIssueIndex(_tokenId);
        uint8[4] memory luckyPoolNumbers = luckyStickNFT.getLuckyPoolNumbers(_tokenId);
        uint8[4] memory _winningNumbers = historyNumbers[_issueIndex];
        require(sumArray(_winningNumbers) != 0, "not drawed");

        uint256 matchingNumber = 0;
        for (uint i = 0; i < luckyPoolNumbers.length; i++) {
            if (_winningNumbers[i] == luckyPoolNumbers[i]) {
                matchingNumber = matchingNumber + 1;
            }
        }
        uint256 reward = 0;
        if (matchingNumber > 1) {
            uint256 amount = luckyStickNFT.getLuckyStickAmount(_tokenId);
    ...
    function adminWithdraw(uint256 _amount) public onlyAdmin {
        fat.safeTransfer(address(msg.sender), _amount);
    ...
    // Add new guidance 's quote
    function addQuote(string memory _quote, string memory _author) external onlyAdmin {
        luckyStickNFT.addQuote(_quote, _author);
    }

    // Set existing guidance 's quote
    function setQuote(uint256 _quoteId, string memory _quote, string memory _author) external onlyAdmin {
        luckyStickNFT.setQuote(_quoteId, _quote, _author);
    }

    // Set base URI
    function setBaseURI(string memory _baseURI) external onlyAdmin {
        luckyStickNFT.setBaseURI(_baseURI);
    }
}

pragma solidity 0.6.12;

contract LuckyPoolOwnable {
...
contract LuckyStickNFT is ERC721, Ownable {
    ...
    Quotes[] private quotes;

    mapping (uint256 => uint8[4]) public luckyPoolInfo;
    mapping (uint256 => uint256) public luckyStickShake;
    mapping (uint256 => uint256) public luckyStickAmount;
    mapping (uint256 => uint256) public issueIndex;
    mapping (uint256 => bool) public claimInfo;
    mapping (address => mapping (uint256 => EnumerableSet.UintSet)) private _holderTokensByQuotes;

    constructor() public ERC721("Lucky Stick", "STICK") {}

    function totalQuotes() external view returns (uint256) {
        return quotes.length;
    }

    function addQuote(string memory _quote, string memory _author) external onlyOwner {
        quotes.push(Quotes({
          quote : _quote,
          author : _author,
          counter : 0
    ...
    function setQuote(uint256 _quoteId, string memory _quote, string memory _author) external onlyOwner {
        require(_quoteId < quotes.length, "Can not set nonexistent");
        quotes[_quoteId].quote = _quote;
        quotes[_quoteId].author = _author;
    }

    function getQuote(uint256 _quoteId) external view returns (string memory quote, string memory author) {
        require(_quoteId < quotes.length, "Can not get nonexistent");
        return (quotes[_quoteId].quote, quotes[_quoteId].author);
    }

    function getLuckyStickShake(uint256 tokenId) public view returns (uint256){
        return luckyStickShake[tokenId];
    }

    function shake(uint256 tokenId) internal view returns (uint256) {
        bytes32 _structHash;
        uint256 _randomNumber;
        uint256 _maxNumber = quotes.length;
    ...
    function newLuckyStick(address player, uint8[4] memory _luckyNumbers, uint256 _amount, uint256 _issueIndex)
        public onlyOwner
        returns (uint256)
    {
        _tokenIds.increment();
        ...
        uint256 _randomNumber = shake(newItemId);
        _mint(player, newItemId);
        luckyStickShake[newItemId] = _randomNumber;
        _holderTokensByQuotes[player][_randomNumber].add(newItemId);
        quotes[_randomNumber].counter += 1;

        luckyPoolInfo[newItemId] = _luckyNumbers;
        luckyStickAmount[newItemId] = _amount;
    ...
    function getLuckyPoolNumbers(uint256 tokenId) external view returns (uint8[4] memory) {
        return luckyPoolInfo[tokenId];
    }
    function getLuckyStickAmount(uint256 tokenId) external view returns (uint256) {
        return luckyStickAmount[tokenId];
    }
    function getLuckyStickIssueIndex(uint256 tokenId) external view returns (uint256) {
    ...
    function burn(uint256 tokenId) external onlyOwner {
        _burn(tokenId);
        uint256 stickNumber = getLuckyStickShake(tokenId);
        _holderTokensByQuotes[msg.sender][stickNumber].remove(tokenId);
        quotes[stickNumber].counter -= 1;
    }
    ...
    function balanceOfQuote(address owner, uint256 quoteId) public view returns (uint256) {
        require(owner != address(0), "ERC721: balance query for the zero address");
        return _holderTokensByQuotes[owner][quoteId].length();
    }

    function tokenOfOwnerByIndexQuote(address owner, uint256 quoteId, uint256 index) public view returns (uint256) {
        return _holderTokensByQuotes[owner][quoteId].at(index);
    }

    function tokenOfQuote(uint256 _quoteId) external view returns (uint256) {
        require(_quoteId < quotes.length, "Can not get nonexistent");
        return quotes[_quoteId].counter;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        uint256 _shakeNumber = luckyStickShake[tokenId];
        string memory _base = baseURI();
        return string(abi.encodePacked(_base, _shakeNumber.toString()));
    }

    function _transfer(address from, address to, uint256 tokenId) internal virtual override {
        super._transfer(from, to, tokenId);
        uint256 stickNumber = getLuckyStickShake(tokenId);
        _holderTokensByQuotes[from][stickNumber].remove(tokenId);
        _holderTokensByQuotes[to][stickNumber].add(tokenId);
    }
}
