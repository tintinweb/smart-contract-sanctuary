...
contract Funding {
  ...
  event EAction(string action, string indexed name, address creator, uint bakNum, uint256 bakAmt, uint256 amount);//action: [create,fund,Withdraw,Refund,Kickoff,Deny]
  event EFinal(string action, string indexed name, address actor, string nft, uint widwable, uint backAmount);   //action: [commit,release,cancel]
  ...
  function createProject( string memory name_, address creator_, 
    ...
    _widwable[name_]    = 0;
    emit EAction("Create", name_, creator_, bakNum_, bakAmt_, pro.budget);
  }
  function fund(string memory name_) public payable {
    ...
    emit EAction("Fund", name_, msg.sender, msg.value, _budget[name_], _pPro[name_].budget);
  }
  ...
  function kickoff(string memory name_) public {
    ...
    emit EAction("Kickoff", name_, msg.sender, _pPro[name_].tax, _widwable[name_], _budget[name_]);
  }
  function deny(string memory name_) public {
    ...
    emit EAction("Deny", name_, msg.sender, phN, _budget[name_]/_pPro[name_].bakNum, _budget[name_]);
  }
  function commit(string memory name_, string memory file_) public {
    ...
    emit EFinal("Commit", name_, msg.sender, file_, phN, _widwable[name_]);
  }
  function release(string memory name_, string memory nft_) public {
    ...
    _budget[name_]        = 0;    
    emit EFinal("Release", name_, msg.sender, nft_, phN, _widwable[name_]);
  }
  function cancel(string memory name_, string memory note_) public owner {
    ...
    emit EFinal("Cancel", name_, msg.sender, note_, _widwable[name_], _bkAmt[name_]);
  }
  function refund(string memory name_) public {
    ...
    emit EAction("Refund", name_, msg.sender, _pRes[name_].length, _widwable[name_], _bkAmt[name_]);
  }
  function withdraw(string memory name_) public {
    ...
    _widwable[name_]                = 0;
    emit EAction("Withdraw", name_, msg.sender, _pRes[name_].length, withdrawed, 0);
  }
