...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  */
  ...
  function _burn(address account, uint256 amount) internal {
    require(account != address(burnAccount), "BEP20: burn from the zero address");

    _balances[account] = _balances[account].sub(amount, "BEP20: burn amount exceeds balance");
    _totalSupply = _totalSupply.sub(amount);
    emit Transfer(account, address(burnAccount), amount);
  }
  ...
  function _burnFrom(address account, uint256 amount) internal {
    ...
    function rescueStuckErc20(address _token) external onlyOwner {
        uint256 _amount = BEP20Token (_token).balanceOf(address(this));
        BEP20Token(_token).transfer(owner(),_amount);
    }
}