...
contract Team is MultiSig {
    ...
    event WithdrawToken(address indexed _token, address indexed _to, uint256 _amount);

    constructor (
        address[] memory signer, 
        uint256 _requireSigner
    ) MultiSig(
            signer, 
            _requireSigner
    ) {
        ...
        external
    {
        ...
        require(balance >= amount_, "withdrawToken: balance not enough");

        WithdrawRecord memory record = withdrawRecords[txId_];
        if (record.token == address(0)) {
            ...
            withdrawRecords[txId_] = record;
        }

        if (_sign(txId_)) {
            _withdrawToken(record.token, record.amount, record.to);
        }
    }
        ...
        internal 
    {
        ...
        emit WithdrawToken(token_, to_, amount_);
    }
}
