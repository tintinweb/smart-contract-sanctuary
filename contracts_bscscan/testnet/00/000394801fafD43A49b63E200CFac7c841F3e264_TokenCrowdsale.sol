...
contract TokenCrowdsale is Ownable, Crowdsale, TimedCrowdsale {
    bool public canClaim;
    uint256 public minBuy = 1 * 1e17;
    uint256 public maxBuy = 5 * 1e18;
    uint256 public totalSale;
    uint256 public totalContributors;
    mapping(address => bool) private whiteList;
    mapping(address => uint256) public preOrder;
    ...
    function lengthInWhiteList() public view returns (uint256) {
    ...
    function setBuyAmount(uint256 _minBuy, uint256 _maxBuy) public onlyOwner {
        minBuy = _minBuy;
        maxBuy = _maxBuy;
    }

    function excluded(address _account) internal {
    ...
    function setAddressToWhiteList(address _account) public onlyOwner {
        whiteList[_account] = true;
        addresses.push(_account);
    }

    function removeAddressFromWhiteList(address _account) public onlyOwner {
        whiteList[_account] = false;
        excluded(_account);
    }

    function setAddressesToWhiteList(address[] memory _accounts) public onlyOwner {
        for(uint256 i = 0; i < _accounts.length; i++) {
            setAddressToWhiteList(_accounts[i]);
        }
    }

    function removeAddressesFromWhiteList(address[] memory _accounts) public onlyOwner {
        for(uint256 i = 0; i < _accounts.length; i++) {
            removeAddressFromWhiteList(_accounts[i]);
        }
    }
    ...
    function buyTokens(address beneficiary) public nonReentrant payable {
        require(whiteList[beneficiary], 'Account not in the white list');
        require(preOrder[beneficiary] == 0, 'Account already pre order');
        require(msg.value >= minBuy && msg.value <= maxBuy, 'Out of purchase');
        ...
        preOrder[beneficiary] = tokens;
        totalSale = totalSale.add(tokens);
    ...
    function claim() public {
        ...
        uint256 tokenBalance = preOrder[msg.sender];
        preOrder[msg.sender] = 0;
        excluded(msg.sender);
        _processPurchase(msg.sender, tokenBalance);
    }

    function pushClaim(uint256 _amount) public {
        require(canClaim, 'can not claim now');
        uint256 leftOver = addresses.length;
        if (leftOver > _amount) {
            leftOver = _amount;
        }
        for(uint256 i = 0; i < leftOver; i++) {
            address account = addresses[i];
            uint256 tokenBalance = preOrder[account];
            preOrder[account] = 0;
            excluded(account);
            _processPurchase(account, tokenBalance);
        }
    }

    function remainingTokens() public view returns (uint256) {
        return token().balanceOf(address(this)) - totalSale;
    }
}