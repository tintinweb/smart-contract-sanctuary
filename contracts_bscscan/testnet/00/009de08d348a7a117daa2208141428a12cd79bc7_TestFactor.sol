...
contract TestFactor {
	...
	uint256[] public SEED_PERCENTS 		= [80, 75, 70, 65, 60, 55, 50, 45, 40, 30];
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		if (user.referrer == address(0)) {
			...
			for (uint256 i = 0; i < 10; i++) {
				...
				distributeRef(upline, msg.sender, _newReferral);
		}
	...
	/* function getUserSeedIncome(address referrer, address userAddress) public view returns (uint256){
		User storage user = users[userAddress];
				...
				if (user.checkpoint < finish) {
					...
					//seed income
							...
							for (uint256 i = 0; i < 10; i++) {
								...
								seedshare = share.mul(SEED_PERCENTS[i]).div(PERCENTS_DIVIDER);
