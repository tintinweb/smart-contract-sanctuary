...
abstract contract MintableBoxConfigHandler is Ownable, IMintableBoxConfigHandler {
  uint16 public constant TOTAL_CHANCES = 10000;
  ...
  uint16[] internal _wearableChances = new uint16[](3);
  uint16[] internal _handChances = new uint16[](5);
  ...
  constructor(IMintableBox.InitialConfig memory _initialConfig) {
    ...
    _setWearableChances(_initialConfig.wearableChances);
    _setHandChances(_initialConfig.handChances);
  }

  /// @inheritdoc IMintableBoxConfigHandler
  function wearableChances() external view returns (uint16[] memory) {
    return _wearableChances;
  }

  /// @inheritdoc IMintableBoxConfigHandler
  function handChances() external view returns (uint16[] memory) {
    return _handChances;
  }
  ...
  /// @inheritdoc IMintableBoxConfigHandler
  function setWearableChances(uint16[] calldata _chances) external onlyOwner {
    _setWearableChances(_chances);
    emit NewWearableChances(_chances);
  }

  /// @inheritdoc IMintableBoxConfigHandler
  function setHandChances(uint16[] calldata _chances) external onlyOwner {
    _setHandChances(_chances);
    emit NewHandChances(_chances);
  }

  function _setWearableChances(uint16[] memory _chances) internal {
    if (_chances.length != 3) revert InvalidAmountOfChances();
    uint16 _totalChances;
    for (uint256 i; i < 3; i++) {
      _wearableChances[i] = _chances[i];
      _totalChances += _chances[i];
    }
    if (_totalChances != TOTAL_CHANCES) revert InvalidChancesDistribution();
  }

  function _setHandChances(uint16[] memory _chances) internal {
    if (_chances.length != 5) revert InvalidAmountOfChances();
    uint16 _totalChances;
    for (uint256 i; i < 5; i++) {
      _handChances[i] = _chances[i];
      _totalChances += _chances[i];
    }
    if (_totalChances != TOTAL_CHANCES) revert InvalidChancesDistribution();
  }

  function _areStringsTheSame(string memory _text1, string memory _text2) internal pure returns (bool) {
    return keccak256(abi.encodePacked((_text1))) == keccak256(abi.encodePacked((_text2)));
  }
}
...
abstract contract MintableBoxOpeningHandler is MintableBoxERC721Handler, RandomNumberConsumer, IMintableBoxOpeningHandler {
  ...
  mapping(bytes32 => uint256) public tokenByRequest; // requestId => boxTokenId

  /// @inheritdoc IMintableBoxOpeningHandler
  mapping(uint256 => bytes32) public requestByToken; // boxTokenId => requestId

  mapping(HandSubtype => mapping(uint16 => uint256)) public nextHandTokenId; // hand subtype => rarity => next token id
  mapping(HandSubtype => mapping(uint16 => uint256)) public maxHandTokenId; // hand subtype => rarity => max token id

  mapping(uint16 => uint256) public nextWearableTokenId; // wearable subtype => next token id
  mapping(uint16 => uint256) public maxWearableTokenId; // wearable subtype next token id

  constructor(IMintableBox.InitialConfig memory _initialConfig) RandomNumberConsumer(_initialConfig.generator) {
    // Rock/human
    nextHandTokenId[HandSubtype.ROCK][0] = 1;
    maxHandTokenId[HandSubtype.ROCK][0] = 55000;
    // Paper/human
    nextHandTokenId[HandSubtype.PAPER][0] = 55001;
    maxHandTokenId[HandSubtype.PAPER][0] = 110000;
    // Scissors/human
    nextHandTokenId[HandSubtype.SCISSORS][0] = 110001;
    maxHandTokenId[HandSubtype.SCISSORS][0] = 165000;
    // Rock/skeleton
    nextHandTokenId[HandSubtype.ROCK][1] = 165001;
    maxHandTokenId[HandSubtype.ROCK][1] = 190000;
    // Paper/skeleton
    nextHandTokenId[HandSubtype.PAPER][1] = 190001;
    maxHandTokenId[HandSubtype.PAPER][1] = 215000;
    // Scissors/skeleton
    nextHandTokenId[HandSubtype.SCISSORS][1] = 215001;
    maxHandTokenId[HandSubtype.SCISSORS][1] = 240000;
    // Rock/zombie
    nextHandTokenId[HandSubtype.ROCK][2] = 240001;
    maxHandTokenId[HandSubtype.ROCK][2] = 250000;
    // Paper/zombie
    nextHandTokenId[HandSubtype.PAPER][2] = 250001;
    maxHandTokenId[HandSubtype.PAPER][2] = 260000;
    // Scissors/zombie
    nextHandTokenId[HandSubtype.SCISSORS][2] = 260001;
    maxHandTokenId[HandSubtype.SCISSORS][2] = 270000;
    // Rock/alien
    nextHandTokenId[HandSubtype.ROCK][3] = 270001;
    maxHandTokenId[HandSubtype.ROCK][3] = 278000;
    // Paper/alien
    nextHandTokenId[HandSubtype.PAPER][3] = 278001;
    maxHandTokenId[HandSubtype.PAPER][3] = 286000;
    // Scissors/alien
    nextHandTokenId[HandSubtype.SCISSORS][3] = 286001;
    maxHandTokenId[HandSubtype.SCISSORS][3] = 294000;
    // Rock/robot
    nextHandTokenId[HandSubtype.ROCK][4] = 294001;
    maxHandTokenId[HandSubtype.ROCK][4] = 296000;
    // Paper/robot
    nextHandTokenId[HandSubtype.PAPER][4] = 296001;
    maxHandTokenId[HandSubtype.PAPER][4] = 298000;
    // Scissors/robot
    nextHandTokenId[HandSubtype.SCISSORS][4] = 298001;
    maxHandTokenId[HandSubtype.SCISSORS][4] = 300000;
    // Ring
    nextWearableTokenId[0] = 300001;
    maxWearableTokenId[0] = 335000;
    // Bracelet
    nextWearableTokenId[1] = 335001;
    maxWearableTokenId[1] = 345000;
    // Watch
    nextWearableTokenId[2] = 345001;
    maxWearableTokenId[2] = 350000;
  }

  /// @inheritdoc IMintableBoxOpeningHandler
  function openBoxes(uint256[] calldata _tokenIds) external {
    for (uint256 i; i < _tokenIds.length; i++) {
      ...
      bytes32 _requestId = _requestRandomNumber();
      tokenByRequest[_requestId] = _tokenId;
      requestByToken[_tokenId] = _requestId;
    }
    emit OpeningRequested(_tokenIds);
  }

  /// @inheritdoc IMintableBoxOpeningHandler
  function replayBoxOpenings(uint256[] calldata _tokenIds) external onlyOwner {
    // Note: when a user starts the opening process, they can't start it again, nor can they transfer the token. Since we rely on
    // an external actor to finish the opening process, something might fail along the way. In order to get out of this situation, the
    // contract owner has the ability to replay the opening of boxes. By doing so, a number will be requested again to the generator
    for (uint256 i; i < _tokenIds.length; i++) {
      if (!_isBoxBeingOpened(_tokenIds[i])) revert BoxIsNotBeingOpened();
      bytes32 _requestId = _requestRandomNumber();
      tokenByRequest[_requestId] = _tokenIds[i];
      requestByToken[_tokenIds[i]] = _requestId;
    }
    emit OpeningsReplayed(_tokenIds);
  }
  ...
  function _consumeRandomNumber(bytes32 _requestId, uint256 _randomness) internal override {
    ...
    IRPSNFT.TokensToMint[] memory _tokens = new IRPSNFT.TokensToMint[](1);
    _tokens[0].owner = ownerOf(_tokenId);
    if (_areStringsTheSame(_nftType, 'hand')) {
      _tokens[0].tokenIds = _getRandomHands(_randomness);
    } else if (_areStringsTheSame(_nftType, 'wearable')) {
      _tokens[0].tokenIds = _getRandomWearable(_randomness);
    } else {
      revert ShouldNotReachHere();
    }

    // Burn box and perform clean up
    delete _tokenType[_tokenId];
    delete requestByToken[_tokenId];
    delete tokenByRequest[_requestId];
    _burn(_tokenId);

    // Mint NFTs
    nftMintingContract.mint(_tokens);
  }
  ...
  function _beforeTokenTransfer(
    ...
    if (_from != address(0) && _isBoxBeingOpened(_tokenId)) revert CannotTransferUntilBoxIsOpened();
  ...
  function _pickOneOfChances(uint16[] storage _chances, uint256 _randomness) internal view returns (uint16) {
    uint16 _randomChance = _getRandomChance(_randomness);
    uint16 _accum;
    for (uint16 i; i < _chances.length; i++) {
      _accum += _chances[i];
      if (_randomChance < _accum) return i;
    }
    revert ShouldNotReachHere();
  }

  function _getRandomWearable(uint256 _randomness) internal virtual returns (uint256[] memory _nftTokenIds) {
    _nftTokenIds = new uint256[](1);
    _nftTokenIds[0] = _calculateNextTokenId(_wearableChances, nextWearableTokenId, maxWearableTokenId, _randomness);
  }

  function _getRandomHands(uint256 _randomness) internal virtual returns (uint256[] memory _nftTokenIds) {
    uint256[] memory _randomNumbers = _generateMultipleNumbers(3, _randomness);
    _nftTokenIds = new uint256[](3);
    _nftTokenIds[0] = _calculateNextTokenId(
      _handChances,
      nextHandTokenId[HandSubtype.ROCK],
      maxHandTokenId[HandSubtype.ROCK],
      _randomNumbers[0]
    );
    _nftTokenIds[1] = _calculateNextTokenId(
      _handChances,
      nextHandTokenId[HandSubtype.PAPER],
      maxHandTokenId[HandSubtype.PAPER],
      _randomNumbers[1]
    );
    _nftTokenIds[2] = _calculateNextTokenId(
      _handChances,
      nextHandTokenId[HandSubtype.SCISSORS],
      maxHandTokenId[HandSubtype.SCISSORS],
      _randomNumbers[2]
    );
  }

  function _calculateNextTokenId(
    uint16[] storage _chances,
    mapping(uint16 => uint256) storage _nextTokenIdForOption,
    mapping(uint16 => uint256) storage _maxTokenIdForOption,
    uint256 _randomness
  ) internal returns (uint256) {
    uint16 _pick = _pickOneOfChances(_chances, _randomness);
    uint256 _nextTokenId = _nextTokenIdForOption[_pick];
    if (_nextTokenId <= _maxTokenIdForOption[_pick]) {
      _nextTokenIdForOption[_pick] += 1;
      return _nextTokenId;
    }
    // If we reach this point, it's because we randomly picked one of the chances and it was already full. In that case, the plan
    // is to go over the different options, from less rare to more rare, and pick the first that isn't full
    for (uint16 i; i < _chances.length; i++) {
      uint256 _tokenId = _nextTokenIdForOption[i];
      if (_tokenId <= _maxTokenIdForOption[i]) {
        _nextTokenIdForOption[i] += 1;
        return _tokenId;
      }
    }
  ...
  /// @notice Returns a random number R that is: 0 <= R < TOTAL_CHANCES
  function _getRandomChance(uint256 _randomness) internal pure returns (uint16) {
    return uint16(_randomness % TOTAL_CHANCES);
  }

  function _generateMultipleNumbers(uint256 n, uint256 _randomness) internal view virtual returns (uint256[] memory _randomNumbers) {
    _randomNumbers = new uint256[](n);
    for (uint256 i; i < n; i++) {
      // We are using this method suggested by Chainlink to generate multiple random numbers from a random seed
      _randomNumbers[i] = uint256(keccak256(abi.encode(_randomness, i)));
    }
  }
}
...
abstract contract MintableBoxMintingHandler is MintableBoxERC721Handler, IMintableBoxMintingHandler {
  ...
  mapping(string => uint32) public maxAmountOfBoxesWithType;
  mapping(string => uint32) public boxesWithType;

  constructor() {
    maxAmountOfBoxesWithType['hand'] = 100000;
    maxAmountOfBoxesWithType['wearable'] = 50000;
  }

  /// @inheritdoc IMintableBoxMintingHandler
  function purchaseBoxes(AmountPerType[] calldata _types, address _owner) external {
    if (block.timestamp < activationDate) revert PurchasingNotAllowedYet();

    // Calculate how much needs to be paid in USD
    uint256 _amountInUSD;
    for (uint256 i; i < _types.length; i++) {
      uint256 _priceInUSD = mintPriceInUSD[_types[i].nftType];
      if (_priceInUSD == 0) revert UnsupportedNFTType();
      _amountInUSD += _priceInUSD * _types[i].amount;
    }
    ...
    uint256 _amountRequired = _getPriceInToken(_amountInUSD, _paymentToken);
    ...
    // Mint tokens
    for (uint256 i; i < _types.length; i++) {
      string memory _nftType = _types[i].nftType;
      if (boxesWithType[_nftType] + _types[i].amount > maxAmountOfBoxesWithType[_nftType]) revert MaxAmountOfBoxes();
      for (uint256 j; j < _types[i].amount; j++) {
        _mint(_owner, _types[i].nftType);
      }
      boxesWithType[_nftType] += _types[i].amount;
    }
  }
  ...
  function _getPriceInToken(uint256 _amountInUSD, IERC20Metadata _paymentToken) internal view virtual returns (uint256) {
    ...
    return (_amountInUSD * (10**_paymentToken.decimals())) / uint256(_answer);
  }
}
...
interface IMintableBoxConfigHandler {
  ...
  event NewWearableChances(uint16[] newChances);

  /// @notice Emitted when hand chances are modified
  /// @param newChances The new minting chances
  event NewHandChances(uint16[] newChances);
  ...
  /// @notice Thrown when an invalid amount of chances is given
  error InvalidAmountOfChances();
  ...
  function setWearableChances(uint16[] calldata chances) external;

  /// @notice Sets new hand minting chances
  /// @param chances The new hand minting chances
  function setHandChances(uint16[] calldata chances) external;
}

/// @notice Handles everything in regard to the NFT part of the box
interface IMintableBoxERC721Handler is IERC721Enumerable {
  /// @notice Thrown when trying to mint more boxes than possible
  error MaxAmountOfBoxes();
...
interface IMintableBoxOpeningHandler is IRandomNumberConsumer {
  ...
  /// @notice Emitted when a user requests to open their boxes
  /// @param tokenIds The ids of the boxes being opened
  event OpeningRequested(uint256[] tokenIds);

  /// @notice Emitted when the opening of certain boxes is replayed
  /// @param tokenIds The ids of the boxes that were replayed
  event OpeningsReplayed(uint256[] tokenIds);
  ...
  function requestByToken(uint256 tokenId) external view returns (bytes32);
  ...
  function tokenByRequest(bytes32 tokenId) external view returns (uint256);

  /// @notice Opens the boxes with the given ids
  ...
  /// Will revert with:
  /// `OnlyOwnerCanOpenBox` if the caller is not the owner all the given boxes
  /// `BoxIsAlreadyBeingOpened` if one of the boxes is already being opened
  /// @param tokenIds The ids of the boxes to open
  function openBoxes(uint256[] calldata tokenIds) external;

  /// @notice Replays the opening of boxes so that a random number is be requested again
  ...
  function replayBoxOpenings(uint256[] calldata tokenIds) external;
}
...
interface IMintableBoxMintingHandler {
  ...
  /// @notice A way to specify a certain amount of NFTs per type
  struct AmountPerType {
    string nftType;
    uint16 amount;
  }
  ...
  /// @notice Mints one or more new boxes with the given types, amounts, and owner
  /// @dev Will revert with:
  /// `PurchasingNotAllowedYet` if the activation date has not passed yet
  /// `UnsupportedNFTType` if any of the given `nftType` is not supported
  /// @param types The types and amounts of boxes to purchase
  /// @param owner The address that will own the box
  function purchaseBoxes(AmountPerType[] calldata types, address owner) external;
...
abstract contract RandomNumberConsumer is Ownable, ERC165, IRandomNumberConsumer {
  ...
  function _requestRandomNumber() internal returns (bytes32 _requestId) {
  ...
  /// @param requestId The id of the request
  /// @param randomNumber The random number
  function _consumeRandomNumber(bytes32 requestId, uint256 randomNumber) internal virtual;
}
