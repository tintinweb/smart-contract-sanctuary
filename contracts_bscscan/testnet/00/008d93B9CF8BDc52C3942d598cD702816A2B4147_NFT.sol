    ...
    ERC721Pausable
{
    ...
    {
        require(initialCap_ > 0, "ERC721Preset: cap is 0");
    ...
    modifier onlySupportedBoxType(uint8 boxType_) {
        require(
            ...
            "NFT: unsupported artifact type"
        );
        _;
    }

    modifier onlyTokenOwner(uint256 tokenId_) {
        require(ownerOf(tokenId_) == _msgSender(), "NFT: caller is not owner");
        _;
    }

    modifier onlyMysteryBox(uint256 tokenId_) {
        require(_exists(tokenId_));
        require(items[tokenId_].dna == 0, "NFT: token is already unboxed");
        _;
    }
    ...
    function increaseCap(uint256 amount_) public onlyOwner {
        require(amount_ > 0, "ERC721Preset: amount is 0");
    ...
    function addSupportedItemType(uint256 boxType_) external onlyOwner {
        bool success = _supportedBoxTypes.add(boxType_);
        require(success, "NFT: item type is already supported");
    }

    function supportedArtifactTypes() external view returns (uint256[] memory) {
        ...
        returns (string memory)
    {
        ...
        string memory baseURI = _baseURI();
        return
            items[tokenId_].dna != 0 && bytes(baseURI).length > 0
                ? string(
                    abi.encodePacked(
                        baseURI,
                        bytes32(tokenId_),
                        "/",
                        bytes32(items[tokenId_].artifacts)
                    )
                )
                : "";
    }

    function updateItemFactory(address itemFactory_) public onlyOwner {
        require(
            itemFactory_ != address(0),
            "NFT: itemFactory_ is the zero address"
        );

        _updateItemFactory(itemFactory_);
    }

    function updateRandomGenerator(address randomGenerator_) public onlyOwner {
        require(
            randomGenerator_ != address(0),
            "NFT: randomGenerator_ is the zero address"
        );
        ...
        onlySupportedBoxType(boxType_)
    {
        ...
        Item storage _info = items[_tokenId];

        // Add box type: size 1 byte
        _info.artifacts = _addArtifactValue(
            _info.artifacts,
            0,
            8,
            uint256(boxType_)
        ...
        _info.artifacts = _addArtifactValue(_info.artifacts, 8, 8, uint256(1));
    }
    ...
    {
        Item storage _info = items[tokenId_];
        _info.dna = bytes32(keccak256(abi.encodePacked(tokenId_, randomness_)));

        uint256 _boxType = _getArtifactValue(_info.artifacts, 0, 8);
        (uint256 _itemId, uint256 _itemType) = itemFactory.getRandomItem(
            randomness_,
            _boxType
        );
        _info.artifacts = _addArtifactValue(_info.artifacts, 16, 16, _itemId); // add itemId
        _info.artifacts = _addArtifactValue(_info.artifacts, 32, 8, _itemType); // add itemType
