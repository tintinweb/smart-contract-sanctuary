...
contract XXX is Context, IERC20, Ownable {
  ...
  function setDaoAddress(address _daoContractAddress) public onlyOwner() {
   ...
   function setDaoFee(uint8 _daoFee) public onlyOwner() {
    daoFee = _daoFee;
  }
  ...
  function _getValues(
    ...
    TransactionValues memory transactionValues = _getTValues(transferAmount, daoFee);
    uint256 currentRate = getRate();
  ...
  function _getTValues(
    uint256 transferAmount, uint8 daoFee
  ) private  view  returns (TransactionValues memory) {
    TransactionValues memory transactionValues;
    // 5% fee to all XXX Token holders.
     transactionValues.transactionFee = transferAmount.mul(daoFee).div(200);
    // 4.5% fee to the XXX DAO contract.
    transactionValues.daoTax = transferAmount.mul(daoFee).div(200);
    // 0.5% royalty.
