...
contract Presale is Ownable, Whitelist {
  ...
  */
  function join() external payable {
    ...
    require(!isWhitelistEnabled || isWhitelisted[msg.sender], "You should become whitelisted to continue.");
  ...
  */
  ...
  function createLiquidityAndLock(uint256 amountETH, uint256 amountToken) private {
    // approve router
    tokenX.approve(router, amountToken);
    // create liquidity
    ...
    // sanity check
    require(tokenX.balanceOf(pair) == amountToken, "liquidity pool: token amount not met");
    // create locker
