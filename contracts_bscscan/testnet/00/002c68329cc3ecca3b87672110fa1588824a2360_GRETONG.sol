...
contract GRETONG is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    string private _name = "GRETONG";
    string private _symbol = "GRTNG";
    ...
    uint256 private _buyResearchFee    = 5;
	...
	// (Buyback + Marketing + Research + Liquidity).
    ...
    uint256 public _buyMaxTxAmount     = 4000000 * 10**9;
    uint256 public _sellMaxTxAmount    = 2000000 * 10**9; // 0.2% of total supply
    uint256 private _otherMaxTxAmount  = 4000000 * 10**9;
    uint256 public _maxWalletAmount    = 6000000 * 10**9;   
    ...
    function safeIncreaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].add(addedValue));
        return true;
    }

    function safeDecreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
    ...
    function setMaxTxAmount(uint256 buyTxAmount, uint256 sellTxAmount, uint256 otherTxAmount, uint256 maxWalletAmount) external onlyOwner() {
        _buyMaxTxAmount = buyTxAmount.mul(10**_decimals);
        _sellMaxTxAmount = sellTxAmount.mul(10**_decimals);
        _otherMaxTxAmount = otherTxAmount.mul(10**_decimals);
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && !_isExcludeAddressFromMaxTx[from] && !_isExcludeAddressFromMaxTx[to]) {
            // set _maxTxAmount to buy or sell action
            if (from == pancakeSwapV2Pair) { // Buy
                _maxTxAmount = _buyMaxTxAmount;
            } else if (to == pancakeSwapV2Pair) { // Sell
        ...
        if(from != owner() && to != owner() && to != address(0) && to != pancakeSwapV2Pair){
