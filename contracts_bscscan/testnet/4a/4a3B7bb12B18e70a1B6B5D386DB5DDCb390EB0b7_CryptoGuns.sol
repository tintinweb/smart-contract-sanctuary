...
function initialize() public initializer {
     ...
     _acceptedToken = 0x0FE6Ed84bd640660c8f0389CfcA0821204369858;
...
function addMember(address account) public virtual {
...
//assigns URI to swat based on name & rarity. Unity Engine reads the URI to pull character attributes for in-game stats.
  ...
  //  require(hasRole(MINTER_ROLE, _msgSender()), "Must be assigned Minter Role");
  //  require(_owner == _msgSender() , "Address owner is not the message sender");
  ...
  function _rollrandom() internal returns (uint) {
...
//overrides duplicate functions begins here
  ...
  function  _burn(uint256 tokenId) internal virtual override(ERC721Upgradeable, ERC721URIStorageUpgradeable) {
          delete userOwnedSwats[_msgSender()][swatIsAtIndex[tokenId]];
...
//level ups the swat
  function levelUp(uint _swatId) public{
  //  require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Not game admin");
    swats[_swatId].level = swats[_swatId].level + 1;
  }

  //swat properities getters
  function getLevel(uint _swatId) public view returns (uint) {
  ...
  function buyNFT(address _buyer) public {
    ...
    // require(acceptedToken.balanceOf(_buyer) >= swatPrice, "You must pay the full price of the NFT");
    require(acceptedToken.approve(address(this), swatPrice), "Approval to spend declined");
     require(acceptedToken.transferFrom(_msgSender(), address(this), swatPrice), "Transferring the sale amount failed");
     //safeTransferFrom(_buyer, address(this), swatPrice);
