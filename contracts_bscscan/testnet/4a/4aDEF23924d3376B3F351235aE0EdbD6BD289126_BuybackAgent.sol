...
contract BuybackAgent is IBuyback, EmergencyWithdraw, ReentrancyGuardUpgradeable, CoinDexTools {
  ...
  function depositTax(
    ...
    Statistic storage statistic_ = buybackTokenStatistics[_buybackToken];
    if (!statistic_.registered) {
      statistic_.registered = true;
      statistic_.index = buybackTokens.length;
      buybackTokens.push(_buybackToken);
    }
    buybackTokenStatistics[_buybackToken].totalPending += addedAmount_;
    buybackPendingByTokens[_fromToken][_buybackToken] += addedAmount_;
  }
