...
contract Presale {
  ...
  function invest(uint256 amount) public {
    ...
    uint256 approveAmount = stableToken.allowance(msg.sender, address(this));
    uint256 investAmount = amount;

    /** ! Make sure the user approved. */
    if (investAmount > approveAmount) {
      investAmount = approveAmount;
    }
    ...
    if (presaleTokensToBuy > presaleTokensLeftToSell) {
      investAmount = investAmount * precision * presaleTokensLeftToSell / presaleTokensToBuy / precision;
      presaleTokensToBuy = presaleTokensLeftToSell;
    }

    stableToken.transferFrom(msg.sender, address(this), investAmount);
    toRedeem[msg.sender] += presaleTokensToBuy;
    presaleTokensSold += presaleTokensToBuy;
    stableTokensRaised += investAmount;
    currentStableTokens += investAmount;
    emit Invest(msg.sender, investAmount, presaleTokensToBuy);
  }
