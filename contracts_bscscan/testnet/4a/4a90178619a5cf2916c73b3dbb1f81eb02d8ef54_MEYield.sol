...
contract MEYield    is Context, Ownable {
    ...
    uint256 public StartBlock = 28800;  // 1day eaual 86400secs or 28800 blocks
...
//        accTokenPerShare360 = accTokenPerShare30.add(
    ...
    function GetReward7(address _address) public view returns(uint256){
        UserInfo storage user = userInfo[_address];
        if (block.number > lastRewardBlock7days && totalStaked7days != 0) {
            uint256 multiplier = _getMultiplier(lastRewardBlock7days, block.number);
            uint256 EarnTokenReward = multiplier.mul(GetRewardPerBlock7days());
            uint256 adjustedTokenPerShare =
                accTokenPerShare7.add(EarnTokenReward.mul(PRECISION_FACTOR).div(totalStaked7days));
            return user.amount7.mul(adjustedTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt7).add(user.earned7);
        } else {
            return user.amount7.mul(accTokenPerShare7).div(PRECISION_FACTOR).sub(user.rewardDebt7).add(user.earned7);
        }
    }
    
    function GetReward30(address _address) public view returns(uint256){
        UserInfo storage user = userInfo[_address];
        if (block.number > lastRewardBlock30days && totalStaked30days != 0) {
            uint256 multiplier = _getMultiplier(lastRewardBlock30days, block.number);
            uint256 EarnTokenReward = multiplier.mul(GetRewardPerBlock30days());
            uint256 adjustedTokenPerShare =
                accTokenPerShare30.add(EarnTokenReward.mul(PRECISION_FACTOR).div(totalStaked30days));
            return user.amount30.mul(adjustedTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt30).add(user.earned30);
        } else {
            return user.amount30.mul(accTokenPerShare30).div(PRECISION_FACTOR).sub(user.rewardDebt30).add(user.earned30);
        }
    }
    
    function GetReward90(address _address) public view returns(uint256){
        UserInfo storage user = userInfo[_address];
        if (block.number > lastRewardBlock90days && totalStaked90days != 0) {
            uint256 multiplier = _getMultiplier(lastRewardBlock90days, block.number);
            uint256 EarnTokenReward = multiplier.mul(GetRewardPerBlock90days());
            uint256 adjustedTokenPerShare =
                accTokenPerShare90.add(EarnTokenReward.mul(PRECISION_FACTOR).div(totalStaked90days));
            return user.amount90.mul(adjustedTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt90).add(user.earned90);
        } else {
            return user.amount90.mul(accTokenPerShare90).div(PRECISION_FACTOR).sub(user.rewardDebt90).add(user.earned90);
        }
    }
    ...
    function GetReward360(address _address) public view returns(uint256){
        UserInfo storage user = userInfo[_address];
        if (block.number > lastRewardBlock360days && totalStaked360days != 0) {
            uint256 multiplier = _getMultiplier(lastRewardBlock360days, block.number);
            uint256 EarnTokenReward = multiplier.mul(GetRewardPerBlock360days());
            uint256 adjustedTokenPerShare =
                accTokenPerShare360.add(EarnTokenReward.mul(PRECISION_FACTOR).div(totalStaked360days));
            return user.amount360.mul(adjustedTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt360).add(user.earned360);
        } else {
            return user.amount360.mul(accTokenPerShare360).div(PRECISION_FACTOR).sub(user.rewardDebt360).add(user.earned360);
        }
    }
    
    function HarvestReward7() external {
        UserInfo storage user = userInfo[msg.sender];
        if (block.number > lastRewardBlock7days) {
            uint256 multiplier = _getMultiplier(lastRewardBlock7days, block.number);
            uint256 EarnTokenReward = multiplier.mul(GetRewardPerBlock7days());
            uint256 adjustedTokenPerShare =
                accTokenPerShare7.add(EarnTokenReward.mul(PRECISION_FACTOR).div(totalStaked7days));
            uint256 pending7=user.amount7.mul(adjustedTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt7).add(user.earned7);
            EarnToken.transfer(address(msg.sender), pending7);
            user.rewardDebt7 = user.amount7.mul(adjustedTokenPerShare).div(PRECISION_FACTOR);
            user.earned7 = 0;
    ...
    function HarvestReward30() external {
        UserInfo storage user = userInfo[msg.sender];
        if (block.number > lastRewardBlock30days) {
            uint256 multiplier = _getMultiplier(lastRewardBlock30days, block.number);
            uint256 EarnTokenReward = multiplier.mul(GetRewardPerBlock30days());
            uint256 adjustedTokenPerShare =
                accTokenPerShare30.add(EarnTokenReward.mul(PRECISION_FACTOR).div(totalStaked30days));
            uint256 pending30=user.amount30.mul(adjustedTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt30).add(user.earned30);
            EarnToken.transfer(address(msg.sender), pending30);
            user.rewardDebt30 = user.amount30.mul(adjustedTokenPerShare).div(PRECISION_FACTOR);
            user.earned30 = 0;
    ...
    function HarvestReward90() external {
        UserInfo storage user = userInfo[msg.sender];
        if (block.number > lastRewardBlock90days) {
            uint256 multiplier = _getMultiplier(lastRewardBlock90days, block.number);
            uint256 EarnTokenReward = multiplier.mul(GetRewardPerBlock90days());
            uint256 adjustedTokenPerShare =
                accTokenPerShare90.add(EarnTokenReward.mul(PRECISION_FACTOR).div(totalStaked90days));
            uint256 pending90=user.amount90.mul(adjustedTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt90).add(user.earned90);
            EarnToken.transfer(address(msg.sender), pending90);
            user.rewardDebt90 = user.amount90.mul(adjustedTokenPerShare).div(PRECISION_FACTOR);
            user.earned90 = 0;
    ...
    function HarvestReward360() external {
        UserInfo storage user = userInfo[msg.sender];
        if (block.number > lastRewardBlock360days) {
            uint256 multiplier = _getMultiplier(lastRewardBlock360days, block.number);
            uint256 EarnTokenReward = multiplier.mul(GetRewardPerBlock30days());
            uint256 adjustedTokenPerShare =
                accTokenPerShare360.add(EarnTokenReward.mul(PRECISION_FACTOR).div(totalStaked360days));
            uint256 pending360=user.amount360.mul(adjustedTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt360).add(user.earned360);
            EarnToken.transfer(address(msg.sender), pending360);
            user.rewardDebt360 = user.amount360.mul(adjustedTokenPerShare).div(PRECISION_FACTOR);
            user.earned360 = 0;
