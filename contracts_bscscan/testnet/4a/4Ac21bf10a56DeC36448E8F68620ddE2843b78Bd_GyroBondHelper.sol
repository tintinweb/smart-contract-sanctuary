...
contract GyroBondHelper is Ownable {
    ...
    constructor(
        ...
        router = router_;
        require(token0_ != address(0), "Token cannot be zero");
        token0 = token0_;

        require(token1_ != address(0), "Token cannot be zero");
        token1 = token1_;
    }

    struct DepositVars {
        IERC20 lpToken;
        uint256 prevAmount0Bal;
        uint256 currAmount0Bal;
        uint256 prevAmount1Bal;
    ...
    function deposit(
        ...
        vars.lpToken = IERC20(IGyroBond(bond).tokenIn());
        require(address(vars.lpToken) != address(0), "Bond principle undefined");

        // save previous balances

        vars.prevAmount0Bal = IERC20(token0).balanceOf(address(this));
        vars.prevAmount1Bal = IERC20(token1).balanceOf(address(this));

        vars.prevLiquidity = vars.lpToken.balanceOf(address(this));
        ...
        vars.currLiquidity = vars.lpToken.balanceOf(address(this));
        require(liquidity == vars.currLiquidity.sub(vars.prevLiquidity), "Not enough liquidity");

        vars.lpToken.safeIncreaseAllowance(bond, liquidity);
        IGyroBond(bond).deposit(liquidity, maxPrice_, recipient_, referralCode_);

        // return the remaining tokens to the depositor

        vars.currAmount0Bal = IERC20(token0).balanceOf(address(this));
        vars.currAmount1Bal = IERC20(token1).balanceOf(address(this));
        if (vars.currAmount0Bal > vars.prevAmount0Bal) {
            IERC20(token0).safeTransfer(msg.sender, vars.currAmount0Bal.sub(vars.prevAmount0Bal));
        }
        if (vars.currAmount1Bal > vars.prevAmount1Bal) {
            IERC20(token1).safeTransfer(msg.sender, vars.currAmount1Bal.sub(vars.prevAmount1Bal));
        }
    }
