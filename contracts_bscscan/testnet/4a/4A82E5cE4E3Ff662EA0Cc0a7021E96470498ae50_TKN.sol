...
abstract contract VestController is ERC20 {
    ...
    uint256 private _totalTokensVesting;
    ...
    function totalTokensVesting() public view virtual returns (uint256) {
    ...
    function _vestingSchedule(uint64 timestamp) internal view virtual returns (uint256) {
        if (timestamp < start()) {
            ...
            return (_totalTokensVesting * (timestamp - start())) / duration();
        }
    }
    ...
    function releaseVesting() public virtual {
        require(_vestingReleased < _totalTokensVesting, "No more tokens left to vest!");
