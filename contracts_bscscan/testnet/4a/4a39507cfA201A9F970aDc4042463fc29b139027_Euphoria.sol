...
contract Euphoria is IBEP20, Ownable {
    ...
    mapping (address => bool) private addressChecker;

    mapping (address => uint) private _euphoria_addresses;
    
    mapping (address => bool) private _full_withdrawal_euphoria_address;
    ...
    uint public _allowed_euphoria_address_spending;
    address[] private _addresses;  // list of holders
    
    
    constructor(address marketing, address airdrop, address operationAndInfrastructure, address privateSale, address presale, address liquidity) {
        _name = "Euphoria";
        _symbol = "EPR";
        _decimals = 8;
        ...
        _allowed_euphoria_address_spending = 20;
        ...
        _balances[privateSale] = (_totalSupply / 100) * 30;
        _balances[presale] = (_totalSupply / 100) * 15;
        _balances[liquidity] = (_totalSupply / 100) * 32;

        // euphoria wallet lock time
        _euphoria_addresses[marketing] = block.timestamp + 12 weeks;
        // _euphoria_addresses[msg.sender] = block.timestamp + 48 weeks;
        _euphoria_addresses[operationAndInfrastructure] = block.timestamp + 32 weeks;  // lock wallet for 8 months
        _euphoria_addresses[airdrop] = block.timestamp + 16 weeks;
        _euphoria_addresses[privateSale] = block.timestamp + 10 days;
        _euphoria_addresses[presale] = block.timestamp + 18 days;
        _euphoria_addresses[liquidity] = block.timestamp + 30 days;
        
        // initial addresses hodling token
        _insertAddress(marketing);
        _insertAddress(msg.sender);
        _insertAddress(operationAndInfrastructure);
        _insertAddress(airdrop);
        
        // euphoria addresses permitted to perform full withdrawal
        _full_withdrawal_euphoria_address[privateSale] = true;
        _full_withdrawal_euphoria_address[presale] = true;
        _full_withdrawal_euphoria_address[liquidity] = true;
    ...
    */
     modifier checkWithdrwalAddressTime (address userAddress) {
         require(_euphoria_addresses[userAddress] < block.timestamp, "Address withdrawal time hasn't reached.");
         _;
     }
    ...
    */
    function transfer(address recipient, uint256 amount) public override checkWithdrwalAddressTime(msg.sender) returns (bool) {
        ...
        // addresses as list of holders;
        _insertAddress(msg.sender);
        _insertAddress(recipient);
        return true;
    }
    
    
    /**
     * @dev function is used in executing transactions
     * on the betting platform
    */
    function betTransfer(uint256 amount) external checkWithdrwalAddressTime(tx.origin) {
        _transfer(tx.origin, msg.sender, amount);  // sends amount to contract calling it
    }
    ...
    */
    function _burn(uint256 amount) public checkWithdrwalAddressTime(msg.sender) returns (bool success) {
    ...
    */
    function _burnFrom(address account, uint256 amount) public checkWithdrwalAddressTime(msg.sender) returns (bool success) {
     ...
     /**
      * @dev function gets a list of current holders.
     */
     ...
     /**
      * @dev function inserts a new holder to the list
      * of token holders.
      * 
      * Requirements:
      * [address] must not have a zero balance
      * [address] must be not already exist in the list
     */
     
    function _insertAddress(address newAddress) private returns (bool) {
         if (addressChecker[newAddress] != true && _balances[newAddress] > 0) {
            addressChecker[newAddress] = true;
