...
contract DemoGreen is Context, IERC20, Ownable {
    ...
    event TransferStandard(uint256 tAmount, uint256 rTransferAmount);
    event _getTValuesEvent(uint256 tTransferAmount, uint256 tFee, uint256 tSwap);
    event _getRValuesEvent(uint256 rAmount, uint256 rTransferAmount, uint256 rFee);
    event getValuesEvent(uint256 rAmount, uint256 rTransferAmount, uint256 rFee,uint256 tTransferAmount, uint256 tFee, uint256 tSwap);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        emit TransferStandard(tAmount, rTransferAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _getValues(uint256 tAmount) private  returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        emit _getTValuesEvent(tTransferAmount, tFee, tSwap);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tSwap, _getRate());
        emit _getRValuesEvent(rAmount, rTransferAmount, rFee);
        emit getValuesEvent(rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tSwap);
    ...
    function sAsBlacklisted(address account) public onlyOwner {
