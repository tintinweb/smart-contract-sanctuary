...
contract MoonCoinFinance  {
	...
	event Custom_RefBonus1(uint256 eventId, uint256 tmpEarn,uint256 tmpMiss,uint256 tokensEarned,uint256 tokensMissed, address indexed upline, uint256 i);
			...
			for (uint256 i = 0; i < 10; i++) {
				if (upline != address(0) && upline!= msg.sender) {				
					...
					if(tokenBalanceFactor > i+1 || tokenBalanceFactor >= 5) { // token hold good					
						...
						eventId++;
						emit Custom_RefBonus1(eventId,  tmpEarn, tmpMiss, users[upline].tokensEarned, users[upline].tokensMissed, upline,i);
