...
contract Launchpad {
    ...
    function showBool(address user) public view returns (bool[] memory, bool[] memory, bool[] memory, bool[] memory,bool[] memory) {
        bool[] memory firstClaim = new bool[](buyer[user].count-1);
        bool[] memory secondClaim = new bool[](buyer[user].count-1);
        bool[] memory thirdClaim = new bool[](buyer[user].count-1);
        bool[] memory fourthClaim = new bool[](buyer[user].count-1);
        bool[] memory fifthClaim = new bool[](buyer[user].count-1);
        
        for(uint j = 0; j < buyer[user].count-1; j++){
