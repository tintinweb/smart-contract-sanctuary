...
contract LittleDogecoin is Token, ISO20022ForPaymentV1{
    ...
    function addUpdateSubAccount(address childAddress, address payto)public onlyMainMerchant{
        ...
        _merchants[childAddress].payto = payto;
    }
    ...
    */
    function pay(address stationAddress, uint256 amount, string memory transactionId, string memory  campaignCode, uint expiry, string memory payMeta)public override{
        ...
        swapTokensForBusd(amount, _merchants[stationAddress].payto);
    ...
    function payInBUSD( address payto, uint256 amount)internal{
        address[] memory path = new address[](2);
        path[0]=address(this);
        path[1]=_busd;
        _approve(address(this), address(LilDOGERouter), amount);
        ...
        _balances[msg.sender] = _balances[msg.sender].sub(amount, "E69");
    }
    function swapTokensForBusd(uint256 tokenAmount, address recipient) private {
        ...
        // make the swap
        LilDOGERouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    **/
    ...
    function swapAndLiquify() private lockTheSwap{
        uint256 minAmountToLiquify = 100e9;
        if (balanceOf(address(this)) >= minAmountToLiquify) {
