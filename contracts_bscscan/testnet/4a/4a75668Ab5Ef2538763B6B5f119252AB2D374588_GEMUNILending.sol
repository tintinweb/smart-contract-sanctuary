...
contract GEMUNILending is PermissionGroup {
    ...
    uint public BASE_APY ;
    uint public mortgagePercent;
    ...
    mapping(address => mapping(uint => LendingInfo)) public userLendingInfs;

    event RequestLending(uint indexed passId, address borrower, uint amountMortgage, uint startTime);
    event IncreaseMortgage(uint indexed passId, uint newAmount);
    event UserReturnPass(uint indexed passId, address borrower);
    event AdminRevokePass(uint indexed passId, address borrower);
    event WithdrawPass(uint indexed passId);
    ...
    constructor(
        ...
        uint _BASE_APY,
        uint _mortgagePercent,
        address _treasury,
        address _geniPassSale
    ) {
        geni = geniAddr;
        geniPass = geniPassAddr;
        BASE_APY = _BASE_APY;
        mortgagePercent = _mortgagePercent;
        treasury = _treasury;
        geniPassSale = IGENIPassSale(_geniPassSale);
    }

    function setMortgagePercent(uint _mortgagePercent) external {
        mortgagePercent = _mortgagePercent;
    }

    function setBaseAPY(uint _apy) external {
        BASE_APY = _apy;
    }

    function setTreasury(address _treasury) external {
        require(_treasury != address(0),"GEMUNILending: invalid treasury");
        treasury = _treasury;
    }
    ...
    function requestLending(uint passId, bytes memory sig) external {
        bool isExistedToken = IGENIPass(address(geniPass)).exists(passId);
        require(isExistedToken, "GEMUNILending: invalid tokenId");

        LendingInfo storage lendingInf = userLendingInfs[msg.sender][passId];

        require(lendingInf.amount == 0,"GEMUNILending: already borrowed this pass");
        uint mortgagePrice = geniPassSale.getPricePass(passId, 0, IGENIPass.PriceType.GENI) * mortgagePercent / decimalRate / 100;
        
        geni.transferFrom(msg.sender, address(this), mortgagePrice);

        geniPass.transferFrom(address(this), msg.sender, passId);
        
        IGENIPass(address(geniPass)).permit(msg.sender, address(this), passId, sig);
        ...
        lendingInf.amount = mortgagePrice;
        lendingInf.startLending = block.timestamp;

        emit RequestLending(passId, msg.sender, lendingInf.amount, lendingInf.startLending);
    }

    function getInterestPayable(uint passId, address user) public returns (uint){
        bool isExistedToken = IGENIPass(address(geniPass)).exists(passId);
        require(isExistedToken, "GEMUNILending: invalid tokenId");
        LendingInfo memory lendingInf = userLendingInfs[user][passId];
        require(lendingInf.amount > 0,"GEMUNILending: this pass has not been borrowed yet");

        uint interestPayable;
        IGENIPass.PassType passType = IGENIPass(address(geniPass)).getPass(passId).passType;  
        interestPayable = geniPassSale.getPricePass(passId, 0, IGENIPass.PriceType.GENI) * BASE_APY / decimalRate / 100 * (block.timestamp - lendingInf.startLending) / ONE_YEAR_IN_SECONDS;

        return interestPayable ;
    }

    function increaseMortgage(uint amount, uint passId) external {
        bool isExistedToken = IGENIPass(address(geniPass)).exists(passId);
        require(isExistedToken, "GEMUNILending: invalid tokenId");
        LendingInfo storage lendingInf = userLendingInfs[msg.sender][passId];
        require(lendingInf.amount > 0,"GEMUNILending: not found lending info");
        require(amount > 0,"GEMUNILending: invalid amount");

        uint increaseAmount = amount * 10 ** ERC20(address(geni)).decimals();
        geni.transferFrom(msg.sender, address(this), amount);

        lendingInf.amount += increaseAmount;

        emit IncreaseMortgage(passId, lendingInf.amount);
    }

    function userReturnPass(uint passId) external {
        bool isExistedToken = IGENIPass(address(geniPass)).exists(passId);
        require(isExistedToken, "GEMUNILending: invalid tokenId");
        LendingInfo memory lendingInf = userLendingInfs[msg.sender][passId];
        ...
        uint currentInterest = getInterestPayable(passId, msg.sender);

        if (lendingInf.amount > currentInterest) {
            uint returnAmount = lendingInf.amount - currentInterest;
            bool returnSuccess = geni.transfer(msg.sender, returnAmount);
            require(returnSuccess);
        } else if (lendingInf.amount < currentInterest) {
            uint debtAmount = currentInterest - lendingInf.amount;
            bool returnSuccess = geni.transferFrom(msg.sender, address(this), debtAmount);
            require(returnSuccess);
        }

        geni.transfer(treasury, currentInterest);
        delete userLendingInfs[msg.sender][passId];

        emit UserReturnPass(passId, msg.sender);
    }

    function adminRevokePass(uint passId, address user, address to) external onlyOperator {
        bool isExistedToken = IGENIPass(address(geniPass)).exists(passId);
        require(isExistedToken, "GEMUNILending: invalid tokenId");
        LendingInfo memory lendingInf = userLendingInfs[user][passId];
        require(lendingInf.amount > 0,"GEMUNILending: not found lending info");

        uint currentInterest = getInterestPayable(passId, user);
        require(lendingInf.amount < currentInterest, "GEMUNILending: can not be revoked");

        IGENIPass(address(geniPass)).unLockPass(passId);
        IGENIPass(address(geniPass)).setActive(passId, false);

        geniPass.transferFrom(user, to, passId);
        geni.transfer(treasury, lendingInf.amount);
        delete userLendingInfs[msg.sender][passId];

        emit AdminRevokePass(passId, user);
    }

    function withdrawPass(uint passId) external onlyOperator {
        bool isExistedToken = IGENIPass(address(geniPass)).exists(passId);
        require(isExistedToken, "GEMUNILending: invalid tokenId");
        require(geniPass.ownerOf(passId) == address(this), "GEMUNILending: not found token in contract");
