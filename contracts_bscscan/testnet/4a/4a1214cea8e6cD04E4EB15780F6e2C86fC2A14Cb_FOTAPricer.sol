...
contract FOTAPricer is PriceAuth {
  ...
  function initialize(
    ...
    _updateMinMaxPrice(_fotaPrice);
  }

  function syncFOTAPrice(uint _fotaPrice) external onlyNormalPriceAdmin {
    require(_fotaPrice >= minPrice && _fotaPrice <= maxPrice, "Price is invalid");
    fotaPrice = _fotaPrice;
    emit FOTAPriceSynced(fotaPrice, block.timestamp);
  }

  function updateMinMaxPrice(uint _fotaPrice) external onlyMinMaxPriceAdmin {
    _updateMinMaxPrice(_fotaPrice);
  }
  ...
  function _updateMinMaxPrice(uint _fotaPrice) private {
    uint tempMinPrice = fotaPrice * 90 / 100;
    uint tempMaxPrice = _fotaPrice * 105 / 100;
    if (tempMinPrice < absMinPrice || tempMaxPrice > absMaxPrice) {
      emit Warning(_fotaPrice, tempMinPrice, maxPrice, absMinPrice, absMaxPrice);
    } else {
