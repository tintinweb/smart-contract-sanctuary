...
contract PresaleTwo is AccessControl {
    ...
    uint256 public buyPeriod = 1800 ; // 7776000 90 days
    uint256 public lockAmount = 15000000000000000000000000; // 15,000,000 DEB
    uint256 public price = 20000000000000000;
    address public receivePresale = 0x7D3c5455BDF24fc411F906f44DD6EFdbFD483Bf1;
    bool public lockBool = false; 
    mapping(address => uint256) public remainAmount;
    mapping(address => uint256) public withdrawalAmount;
    constructor(address minter, address _tokenDebird, address _tokenUsdt){
        ...
        buyTime = buyPeriod.add(block.timestamp);
    }
    ...
    function lockToken() public {
        ...
        lockBool = true;
        tokenDebird.safeTransferFrom(address(msg.sender), address(this), lockAmount);
    }

    function buyPresale(uint256 _amount) public {
        require(hasRole(CREATOR_ADMIN, address(msg.sender)), "Caller is not a admin");
        require(block.timestamp < buyTime, "Presale End");
        uint256 amountDebird = _amount.div(price);
        require(totalAdd.add(amountDebird) <= lockAmount, "Exceeded the allowed amount");
        remainAmount[address(msg.sender)] = remainAmount[address(msg.sender)].add(amountDebird);
        totalAdd = totalAdd.add(amountDebird);
        emit addMember(
            msg.sender,
            amountDebird,
            block.timestamp
    ...
    function getBalanceToken(address _address) public view returns( uint256 ) {
        ...
        if(block.timestamp > lockTime){
            ...
            uint256 remainAmount80Percent = remainAmount[address(_address)].mul(8).div(10);
            uint256 remainCurrent = remainAmount80Percent.mul(claimPeriod).div(vestingPeriod) + remainAmount[address(_address)].div(10);
            amount = remainCurrent > withdrawalAmount[address(_address)] ? remainCurrent.sub(withdrawalAmount[address(_address)]) : 0;
        }
        return amount;
    }

    function safeImbaTransfer(address _to, uint256 _amount) internal {
        uint256 DebirdBal = tokenDebird.balanceOf(address(this));
        if (_amount > DebirdBal) {
            tokenDebird.transfer(_to, DebirdBal);
        } else {
            tokenDebird.transfer(_to, _amount);
        }
    }
}