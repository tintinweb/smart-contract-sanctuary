...
contract Airdrop is OwnableUpgradeable  {
    ...
    function getInfoAirdrop(address sender) external view returns (uint256 timeRegister,uint256 timeClaim,uint256 amount,address receiver,uint256 index) {
        index=indexOfAirdrops[sender];
        timeRegister=infoAirdrops[index].timeRegister;
        amount=infoAirdrops[index].amount;
        timeClaim=infoAirdrops[index].timeClaim;
        receiver=infoAirdrops[index].receiver;
    }
