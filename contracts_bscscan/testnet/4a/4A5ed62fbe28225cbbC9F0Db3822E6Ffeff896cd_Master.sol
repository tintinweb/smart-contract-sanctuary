...
contract Master is IPlatformMaster {
    ...
    event ReBuyNft(address indexed contractAddress, address buyer, uint256 nftId, uint256 tokenId, uint256 mintCap, uint256 minted, string modelName);
    ...
    // The cut of each NFT sale that is sent to the platform
    uint256 public platformFee = 15;

    // The percentage cut of sales that go to the referrer from a referrers sales
    ...
    constructor(address _modelContractBase, address _mainToken, address _wrapBNB, address _cummies) {
        masterOwner = msg.sender;
        admins[msg.sender] = true;
        wrapBNB = _wrapBNB;
        paymentTokens.push(wrapBNB);
    ...
    function logReBuyNft(address buyer, uint256 nftId, uint256 tokenId, uint256 mintCap, uint256 minted, string memory modelName) external override onlyChildContract {
        emit ReBuyNft(msg.sender, buyer, nftId, tokenId, mintCap, minted, modelName);
    }
...
contract ModelCollection is ERC721 {
    ...
    function reBuyNft(uint256 tokenId) public {
        ...
        master.logReBuyNft(msg.sender, nftId, tokenId, nft.mintCap, nft.numberMinted, modelName);
    }
...
interface IPlatformMaster {
    ...
    function logReBuyNft(address buyer, uint256 nftId, uint256 tokenId, uint256 mintCap, uint256 minted, string memory modelName) external;
