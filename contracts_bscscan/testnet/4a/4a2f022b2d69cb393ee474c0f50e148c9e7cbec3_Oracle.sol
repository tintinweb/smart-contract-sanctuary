...
contract Oracle is IOracle, Prophetable {
    ...
    event RequestRandomSeedmon(address requester, uint256 requestId, uint256 minNumber, uint256 maxNumber, uint256 provideGas);
    ...
    function requestRandomSeedmon(uint256 requestId, uint256 minBonusStat, uint256 maxBonusStat) onlyOpen requireGas payable override external {
        require(requestId > 0, "Oracle: Invalid request is zero");
        ...
        emit RequestRandomSeedmon(requester, requestId, minBonusStat, maxBonusStat, providedGas);
    }
...
interface IOracle {
    ...
    function requestRandomSeedmon(uint256 requestId, uint256 minBonusStat, uint256 maxBonusStat) payable external;
}
