...
contract MarketPlace is IMarketPlace, ReentrancyGuard, AccessControlEnumerable, IERC721Receiver {
    ...
    mapping(address => bool) private _whitelistAccount; // whitelist
    mapping(address => AirdropWhitelist) private _airdropWhitelist; // available amount to claim niimfer
    mapping(address => uint256) private _tokenRate;
    ...
    constructor(address nftContract, address nimf) {
        ...
        setTokenRate(nimf, 1e18);
        setTokenRate(address(0), 1e18);
    }
    ...
    function setTokenRate(address _token, uint256 _rate) public onlyRole(MARKETPLACE_GOVERNANCE) {
        _tokenRate[_token] = _rate;
        emit OnTokenRateUpdate(_token, _rate);
    }
    ...
    function setWhitelistAccount(address account, bool value) external onlyRole(MARKETPLACE_GOVERNANCE) {
        require(account != address(0), "zero");
        _whitelistAccount[account] = value;
        emit OnAddWhiteListAccount(account, value);
    }

    function addAirdropWhitelistAccount(address account, uint256 limit) external onlyRole(MARKETPLACE_GOVERNANCE) {
        require(account != address(0), "zero");
        _airdropWhitelist[account] = AirdropWhitelist(limit, 0);
        emit OnAddAirdropWhiteListAccount(account, limit);
    }

    function updateAirdropWhitelistAccount(address account, uint256 limit) external onlyRole(MARKETPLACE_GOVERNANCE) {
        _airdropWhitelist[account].airdropLimit = limit;
        emit OnUpdateAirdropWhiteListAccount(account, limit);
    }
    ...
    function updateSalesVersion(
        ...
        emit OnUpdateSalesVersion(versionId, newSalePrice, newStartTime, whitelisted, forAirDrop);
    }
    ...
    function purchase(
        ...
        uint256 _rate = _tokenRate[_payingToken];
        require(_rate > 0, "no rate");
        
        _purchase(nftContract, msg.sender, _payingToken, tokenId, _rate, 0, 0); 
    }
    ...
    function addSalesVersion(
        ...
        sales_version.push(SalesVersion(startTokenId, 0, totalSupply, salePrice, startTime, whitelisted, forAirDrop));
    ...
    function claimNiimfer(uint256 versionId, uint256[] memory tokenId) external payable override {
        ...
        if (sales_version[versionId].whitelisted) {
            require(_whitelistAccount[msg.sender], "not in whitelist");
        }
    ...
    function claimNiimfersforAirdrop(uint256 versionId, uint256 amount) external payable override {
        SalesVersion storage version = sales_version[versionId];
        AirdropWhitelist storage list = _airdropWhitelist[msg.sender];
    ...
    function offer(uint256 tokenId, uint256 offerValue) external payable override nonReentrant {
        SaleInfo storage sale = idToMarketItem[_toItemId[tokenId]];
        address buyer = msg.sender;
        uint256 currentOffer = offers[tokenId][buyer];
        ...
        offers[tokenId][buyer] = offerValue;
        ...
        emit Offered(tokenId, buyer, offerValue);
    }
    ...
    function cancelOffer(uint256 tokenId) external override nonReentrant {
        address buyer = msg.sender;
        uint256 offerValue = offers[tokenId][buyer];
    ...
    function acceptOffer(
        ...
        uint256 offeredPrice = offers[tokenId][buyer];
        ...
        uint256 _rate = _tokenRate[payingToken];
        require(_rate > 0, "no rate");

        _purchase(_nftContract, buyer, payingToken, tokenId, _rate, _extraBurn, offeredPrice);
    }
    ...
    function _transactionFee(
        ...
        if (tradingFee > 0 && communityFund != address(0)) {
            ...
            IERC20(_payingToken).safeTransfer(communityFund, _tradingFee);
            emit TradingFee(tokenId, _price, _tradingFee);
        }
        return amount;
    }
    ...
    function _purchase(
        ...
        uint256 _price =  ((soldPrice * tokenRate) / 1e18);
        uint256 _amount;
        ...
        emit Purchase(saleInfo, buyer, soldPrice);
    }
...
contract nERC721 is IERC721, IERC721Metadata, AccessControlEnumerable {
    using Address for address;
    ...
    bytes32 public constant ERC721_MANAGER = keccak256("ERC721_MANAGER");

    bytes32 public constant ERC721_MINTER = keccak256("ERC721_MINTER");
    ...
    constructor(string memory name_, string memory symbol_) {
        ...
        _symbol = symbol_;
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(ERC721_MANAGER, msg.sender);

        _setupRole(ERC721_MINTER, msg.sender);
    }
...
*/
    ...
    function _mintIfNotExist(uint256 tokenId) private {
        if (!_exists(tokenId)) {
            require(hasRole(ERC721_MINTER, msg.sender), "not minter");
    ...
    */
    ...
    function mintToken(address to, uint256 tokenId) external onlyRole(ERC721_MINTER) {
        _mint(to, tokenId);
    }
    ...
    */
    ...
    function safeMintToken(address to, uint256 tokenId) external onlyRole(ERC721_MINTER) {
        _safeMint(to, tokenId);
    }
...
interface IMarketPlace {
    ...
    event Purchase(SaleInfo saleInfo, address indexed buyer, uint256 price);
    event BurnFee(uint256 indexed tokenId, uint256 price, uint256 fee);
    event TradingFee(uint256 indexed tokenId, uint256 price, uint256 fee);
    ...
    event Offered(uint256 indexed tokenId, address indexed buyer, uint256 price);
    event OfferCanceled(uint256 indexed tokenId, address indexed buyer);
 
    event OnAddWhiteListAccount(address indexed account, bool value);
    event OnAddAirdropWhiteListAccount(address indexed account, uint256 limit);
    event OnUpdateAirdropWhiteListAccount(address indexed account, uint256 limit);
    event OnUpdateAirdropClaimedAmount(address indexed account, uint256 versionId, uint256 amount, uint256 totalClaimed);
     ...
     * - Owner cannot offer.
     */
    function offer(uint256 tokenId, uint256 offerValue) external payable;
