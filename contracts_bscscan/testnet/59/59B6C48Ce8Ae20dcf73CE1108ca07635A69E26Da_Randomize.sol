...
contract Randomize is VRFConsumerBase, Ownable, IRandom {
    
    bytes32 internal keyHash;
    uint256 internal fee;
    mapping (bytes32 => uint256) randomResults;
    mapping (bytes32 => uint256) pids;
    mapping (bytes32 => uint256) epochs;

    address public prediction;

    bool public isCallOracle = true;
    ...
    function setIsCallOracle(bool _isCallOracle) public onlyOwner {
        isCallOracle = _isCallOracle;
    }

    /** 
     * @dev Requests randomness from lottery for a pool, only by lottery contract
     * @param pid pool id in lotery contract
    ...
    function getRandomNumber(
        ...
        override
        onlyPrediction()
        returns (bytes32 requestId, uint256 randomness, bool isWating)
    {
        if (isCallOracle) {
            requestId = _getRandomNumberFromChainLink();
            pids[requestId] = pid;
            epochs[requestId] = epoch;
            return (requestId, 0, true);
        } else {
            uint256 random = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, pid, epoch)));
            return (bytes32(""), random, false);
        }
    }

    function _getRandomNumberFromChainLink() internal returns (bytes32 requestId) {
