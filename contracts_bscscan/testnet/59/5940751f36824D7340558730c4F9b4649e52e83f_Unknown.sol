...
contract Unknown is IBEP20, Ownable, Rewardable {
    ...
    uint8 public _totalFee = _marketingFee + _giveawayFee + _reflectionFee + _liquidityFee + _devFee;
    ...
    function _transferTokens(
        ...
        addLiquidity(computedLiquidityFees);
    ...
    function addLiquidity(uint256 tokenAmountToLiquify) private {
      require(tokenAmountToLiquify > 0, "No token to liquify");

      uint256 initialETHBalance = address(this).balance; // WETH balance before swapping tokens for WETH
      uint256 halfTokenAmount = tokenAmountToLiquify / 2; // Split the tokens in two parts
      _balances[address(this)] += halfTokenAmount; // Add half of the token to liquify to the contract
      
      swapTokensForEth(halfTokenAmount); // Swap half of the tokens for WETH, this contract will receive WETH

      uint256 wethAddedFromSwap = address(this).balance - initialETHBalance; // The ETH that has been added to the contract
      addLiquidityETH(halfTokenAmount, wethAddedFromSwap);
    }
    ...
    function addLiquidityETH(uint256 tokenAmount, uint256 ethAmount) public {
        _approve(address(this), address(_router), tokenAmount);
