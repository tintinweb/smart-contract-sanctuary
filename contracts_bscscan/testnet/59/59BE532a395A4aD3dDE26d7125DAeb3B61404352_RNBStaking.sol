...
contract RNBStaking {
    ...
    function modifyTxFee(uint256 newFee) public{
        if(msg.sender==owner){
            txFee=newFee;
        }
    }
    ...
    event newStake(address staker, uint256 stakeAmount);
    event requestReward (address staker);
    event removeAndRequestReward (address staker,string txid);
    ...
    function addStake(uint256 amount) public {
        ...
        pendingStakeDeposit[msg.sender]=amount;
    }

    function payFees() public payable{
        address payable oracleAddr=payable(oracle);
        oracleAddr.transfer(txFee); 
        emit newStake(msg.sender,pendingStakeDeposit[msg.sender]);
        delete pendingStakeDeposit[msg.sender];
    ...
    function getRewardAndRemoveLP(string memory txid) public {
        ...
        delete timestampForUnlock[txid];
        RNB.transfer(msg.sender,((totalClaimable/100)*15));
        emit removeAndRequestReward(msg.sender,txid);
        }
