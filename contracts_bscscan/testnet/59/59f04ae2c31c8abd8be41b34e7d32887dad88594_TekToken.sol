...
// ----------------------------------------------------------------------------
contract SafeMath {
    function Add(uint256 O, uint256 b) external pure returns (uint256 c) {
        c = O + b;
        require(c >= O);
    }
    function Sub(uint256 O, uint256 b) external pure returns (uint256 c) {
        require(b <= O);
        c = O - b;
    }
    function Mul(uint256 O, uint256 b) external pure returns (uint256 c) {
        c = O * b;
        require(O == 0 || c / O == b);
    }
    function Div(uint256 O, uint256 b) external pure returns (uint256 c) {
        require(b > 0);
        c = O / b;
    }
}
...
contract TekToken is Context, IERC20, IERC20Metadata, Ownable, SafeMath  {
    ...
    uint256 _TEK = 6*9**12 * 10**9;
    uint256 uniBurn = 400000000000 * 10**9;
