...
contract OctiBUSDOptions is Ownable, IOctiOptions {
        ...
        )
    {
        ...
        periodFee = (getPeriodFee(amount, period, strike, currentPrice, optionType)).mul(currentPrice);
        ...
        returns (uint256 fee)
    {
        ...
        return (getpercent / 100).mul(currentPrice);
    }
    ...
    function getStrikeFee(
        ...
        if (strike > currentPrice && optionType == OptionType.Put)
            return (strike.sub(currentPrice).mul(amount).div(currentPrice)).mul(currentPrice);
        if (strike < currentPrice && optionType == OptionType.Call)
            return (currentPrice.sub(strike).mul(amount).div(currentPrice)).mul(currentPrice);
        return 0;
    }
