...
contract GreenHouseV4 is OwnableUpgradeable {
    using SafeMath for uint256;

    event Planted(address beneficiary, uint256 fertilizedToken, uint256 tree);
    event PlantTransfer(address from, address to, uint256 amount);
    ...
    function transferPlantedAccumulatePoint(address to, uint256 amount) external {
        require(plantAccumulated[msg.sender] >= amount, "GreenHouse: Insufficient amount");

        plantAccumulated[msg.sender] = plantAccumulated[msg.sender].sub(amount);
        plantAccumulated[to] = plantAccumulated[to].add(amount);

        emit PlantTransfer(msg.sender, to, amount);
    }
