...
// File: contracts/TomorrowTokenDividendPayingToken.sol
...
// File: contracts/TomorrowToken.sol

// 

pragma solidity ^0.7.6;

contract TomorrowToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    TomorrowTokenDividendTracker public dividendTracker;
    ...
    uint256 public XRPRewardsFee;
    uint256 public liquidityFee;
    uint256 public marketingFee;
    uint256 public marketingXRPFee;
    ...
    constructor() ERC20("TomorrowToken", "ROW") {
        uint256 _XRPRewardsFee = 4;
        uint256 _liquidityFee = 3;
        uint256 _marketingFee = 1;
        uint256 _marketingXRPFee = 6;

        XRPRewardsFee = _XRPRewardsFee;
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        marketingXRPFee = _marketingXRPFee;
 
        totalFees = _XRPRewardsFee.add(_liquidityFee).add(_marketingFee).add(_marketingXRPFee);

        dividendTracker = new TomorrowTokenDividendTracker();

        liquidityWallet = owner();
        marketingWallet = address(0x868367d6107426bbb2795D6C2C376d19bc3D182b);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "TomorrowToken: The dividend tracker already has that address");

        TomorrowTokenDividendTracker newDividendTracker = TomorrowTokenDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TomorrowToken: The new dividend tracker must be owned by the TomorrowToken token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TomorrowToken: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TomorrowToken: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TomorrowToken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TomorrowToken: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "TomorrowToken: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TomorrowToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TomorrowToken: Cannot update gasForProcessing to same value");
    ...
    function setMarketingFee(uint256 _tokenFee, uint256 _xrpFee) external onlyOwner {
        marketingFee = _tokenFee;
        marketingXRPFee = _xrpFee;

        totalFees = XRPRewardsFee.add(liquidityFee).add(marketingFee).add(marketingXRPFee);
    }

    function setXRPRewardFee(uint256 _xrpRewardFee) external onlyOwner {
        XRPRewardsFee = _xrpRewardFee;
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "TomorrowToken: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 pieces = marketingXRPFee.add(XRPRewardsFee);
        uint256 xrpBalance = IERC20(XRP).balanceOf(address(this));
        uint256 dividends = xrpBalance.mul(XRPRewardsFee).div(pieces);
        uint256 marketings = xrpBalance.sub(dividends);
        ...
        success = IERC20(XRP).transfer(marketingWallet, marketings);
        if (success) {
...
contract TomorrowTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("TomorrowToken_Dividend_Tracker", "TomorrowToken_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "TomorrowToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "TomorrowToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TomorrowToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TomorrowToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TomorrowToken_Dividend_Tracker: Cannot update claimWait to same value");
