...
contract CoinTapLaunchpad is ICoinTapLaunchpad, Ownable, SharedConstants, Pausable, Whitelist {
    ...
    address constant BURN_ADDRESS =
        address(0x000000000000000000000000000000000000dEaD); // 90%

    mapping(address => uint256) private amountsStaked;
    mapping(address => VestingHistory[])
        private vestingHistories;
    ...
    constructor(
        ...
        /* Pre check data */
        require(
            _launchpadData.unsoldToken == UnsoldToken.BURN ||
                _launchpadData.unsoldToken ==
                UnsoldToken.REFUND,
            "Presale must be BURN or REFUND"
        );
        // require(
    ...
    function finalize() public override onlyOwner {
        ...
        require(
            capacityBNB >= launchpadData.softCap,
            "Capacity BNB user bought less than soft cap"
        );
     ...
     * @dev cancel launchpad
     */
    function updateInfo(Information memory _information)
        public
        override
        onlyOwner
    {
     ...
     * @dev Buy token
     */
    function buyToken() public payable override checkWhoCanBuy {
        // require(
        ...
        require(
            capacityBNB.add(amount) <= launchpadData.hardCap,
            "The capacity has exceeded the limit"
        );

        address buyer = _msgSender();
        uint256 amountOfUser = amountsStaked[buyer];

        require(
            amountOfUser.add(amount) <= launchpadData.maxBuy,
            "Cannot buy more token because you bought more than max amount"
        );
        require(
            buyer != address(0),
            "Buyer is the zero address"
        );
        require(amount > 0, "Amount must be greater than 0");

        amountsStaked[buyer] = amountsStaked[buyer].add(amount);
    ...
    function emergencyWithdraw() public override {
        ...
        uint256 amountOfUser = amountsStaked[buyer];
        // Fee 10%
        uint256 amountFinal = amountOfUser.mul(EMERGENCY_WITHDRAW_PORTION).div(
            100
        );
    ...
    function withdraw() public override {
        require(
            status == Statuses.CANCELLED,
            "Launchpad do not cancelled yet"
        );
     ...
     */
    function amountStakedOf(address _addr) public view override returns (uint256) {
     ...
     * @dev Claim token
     */
    function claim(uint8 _index) public override returns (uint256 amount, uint256 claimAt) {
        ...
        uint256 amountOfUser = amountUserStaked
            ...
            .div(DECIMAL_OF_NATIVE_CURRENCY);
    ...
    function _handleUnsoldToken() internal {
        ...
        uint256 remainToken = remainBNBCap.mul(launchpadData.presaleRate).div(
            DECIMAL_OF_NATIVE_CURRENCY
        );
        if (launchpadData.unsoldToken == UnsoldToken.REFUND) {
            token.transfer(owner(), remainToken);
        }

        if (launchpadData.unsoldToken == UnsoldToken.BURN) {
    ...
    modifier checkWhoCanBuy() {
        ...
        // whitelist
        if (
            launchpadData.presaleType == PresaleType.WHITELIST
        ) {
            require(
...
interface ICoinTapLaunchpad { 
  ...
  function claim(uint8 _index) external returns (uint256 amount, uint256 claimAt);
  function amountStakedOf(address _addr) external returns (uint256);
}
