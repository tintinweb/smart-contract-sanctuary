...
contract LockToken is Context {
    ...
    AddressesLib.Addresses private _lockerAddresses;

    event NewLock(address user, uint256 amount, uint256 cliff);
    event UpdateLock(address user, uint256 amount, uint256 cliff);
    ...
    function lockToken(uint256 _amount, uint256 _cliff) external {
        ...
        require(!_locks[_msgSender()].isActive, "Duplicate lock");
        ...
        _locks[_msgSender()] = lockInfo;
        _totalSupply = _totalSupply + _amount;
        _lockerAddresses.push(_msgSender());

        emit NewLock(_msgSender(), _amount, _cliff);
    }

    function updateLockInfo(uint256 _amount, uint256 _cliff) external {
        LockInfo memory lockInfo = _locks[_msgSender()];
        ...
        require(token.transferFrom(_msgSender(), address(this), _amount - lockInfo.amount), "transfer failed");

        _locks[_msgSender()].amount = _amount;
        _locks[_msgSender()].cliff = _cliff;

        emit UpdateLock(_msgSender(), _amount, _cliff);
    }
    ...
    function withdraw() external {
        ...
        _locks[_msgSender()].isActive = false;
        _totalSupply = _totalSupply - lockInfo.amount;

        _lockerAddresses.remove(_msgSender());

        emit Withdraw(_msgSender(), lockInfo.amount);
    }
    ...
    function getLockerAddresses() external view returns (address[] memory) {
        return _lockerAddresses.getAllAddresses();
    }

    function balanceOf(address _user) public view returns (uint256) {
        return _locks[_user].amount;
    }
