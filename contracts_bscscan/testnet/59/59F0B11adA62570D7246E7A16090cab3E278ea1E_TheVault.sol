...
contract TheVault is ERC20, ReentrancyGuard, Pausable, Ownable {
    ...
    uint256 public _arbitrageShare = 70; // 70%
    ...
    uint256 public constant ARBITRAGE_SHARE_MAX = 90; // 90%
    ...
    function arbitrage(
        ...
        if (arbitrageAmountOut > amountIn) {
            ...
            if (profitShare != 0) {
                IERC20(_stal).safeTransfer(msg.sender, profitShare);
            }
            uint256 profitToTheVault = profit.sub(profitShare);

            emit Arbitrage(profitToTheVault, profitShare);
        }
    }
