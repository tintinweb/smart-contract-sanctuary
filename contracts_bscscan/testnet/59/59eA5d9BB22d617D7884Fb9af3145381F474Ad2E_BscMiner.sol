...
contract BscMiner {
    ...
    function getMyAllBarrels() public view returns(uint[] memory, uint32[] memory, bool[] memory) {
        uint len = pools.length;
        uint[] memory barrels = new uint[](len);
        uint32[] memory lastHatchs = new uint32[](len);
        bool[] memory hasHatched = new bool[](len);
        for(uint i = 0; i < len; i++) {
            barrels[i] = poolxUser[i][msg.sender].claimedEggs.add(getEggsSinceLastHatch(i));
            lastHatchs[i] = poolxUser[i][msg.sender].lastHatch;
            hasHatched[i] = poolxUser[i][msg.sender].hatcheryMiners > 0;
        }
        return (barrels, lastHatchs, hasHatched);
    }
