...
contract Brilliant is ERC20, Ownable {
  ...
  mapping(address => bool) claimstaked;
    ...
    uint mineDec = 1e3;
    uint stakeDec = 1e3;
    uint mineDiv = 10000000000000; 
    uint stakeDiv = 10000000000000;    
    //owner = msg.sender;
  ...
  function AddStake() public {
   ...
   // balances[address(0)] = balances[address(0)].add(rewardInt);
    ...
    function balanceOf(address account) public view returns (uint256) {
        return _balanceOf(account) + (claimed[account] ? 0 : balanceMine(account)) + (claimstaked[account] ? 0 : balanceStake(account));
    }
