...
contract RastaNFT {
    ...
    function burn(uint256 tokenId, uint256 index) external onlyOwner returns (bool) {
        require(_NFTs[tokenId].owner == nftAddr, "!Not allowed");
        require(tokenId == tokenIds[index], "!Index is not correct");
    ...
    function NFTOfOwner(address to) public view returns (NFTInfo[] memory) {
        NFTInfo[] memory NFTs = new NFTInfo[](tokenIds.length);
        for(uint i = 0; i < tokenIds.length; i ++) {
            if(address(_NFTs[tokenIds[i]].owner) == address(to)) {
                NFTs[i] = _NFTs[tokenIds[i]];
            }
        }
        return NFTs;
    }
    ...
    function stake(uint256 tokenId) external {
        ...
        require(balance >= nft.price, "!Your balance is not enough");
    ...
    function nonOwnerNFTs() public view returns (NFTInfo[] memory) {
        NFTInfo[] memory NFTs = new NFTInfo[](tokenIds.length);
        for(uint i = 0; i < tokenIds.length; i ++) {
            if(_NFTs[tokenIds[i]].owner == nftAddr) {
                NFTs[i] = _NFTs[tokenIds[i]];
            }
        }
        return NFTs;
    ...
    function AllNFTs() public view returns (NFTInfo[] memory) {
        NFTInfo[] memory NFTs = new NFTInfo[](tokenIds.length);
        for(uint i = 0; i < tokenIds.length; i ++) {
            NFTs[i] = _NFTs[tokenIds[i]];
        }
