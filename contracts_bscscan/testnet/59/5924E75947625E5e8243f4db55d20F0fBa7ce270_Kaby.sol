...
import "./interfaces/IKaby.sol";
import "./interfaces/IEquipment.sol";
import "./utils/AcceptedToken.sol";

contract Kaby is IKaby, ERC721, AcceptedToken, ReentrancyGuard {
    ...
    ) ERC721("Kaby", "HERO") AcceptedToken(tokenAddress) {
    ...
    modifier onlyHeroOwner(uint heroId) {
        require(ownerOf(heroId) == msg.sender, "Kaby: not hero owner");
        _;
    }

    /**
     * @dev owner set new equipment contract
     * @param equipmentAddress new equipment contract address
     */
     ...
     * @param marketFee new market fee value
     */
     ...
     * @param value new service fee value
     */
    ...
    @param baseURI new baseURI value
     */
     ...
     * @param maxSupply new maxSupply value
     * @param salePrice new salePrice  value
     * @param startTime new startTime value
     * @param revealTime new revealTime value
     * @param provenance new provenance value
     */
    ...
    function claimHero(uint versionId, uint amount) external override {
        ...
        require(amount > 0 && amount <= 50, "Kaby: amount out of range");
        require(block.timestamp >= version.startTime, "Kaby: Sale has not started");
        require(version.currentSupply + amount <= version.maxSupply, "Kaby: sold out");
    ...
    function equipItems(uint heroId, uint[] memory itemIds) external override onlyHeroOwner(heroId) {
        _setHeroEquipment(heroId, itemIds, false);
    ...
    function removeItems(uint heroId, uint[] memory itemIds) external override onlyHeroOwner(heroId) {
        _setHeroEquipment(heroId, itemIds, true);
    ...
    function list(uint heroId, uint price) external override onlyHeroOwner(heroId) {
        require(price > 0, "Kaby: price is zero");
    ...
    function delist(uint heroId) external override onlyHeroOwner(heroId) {
        require(herosOnSale[heroId] > 0, "Kaby: not listed");
    ...
    function buy(uint heroId) external override nonReentrant {
        ...
        require(price > 0, "Kaby: not on sale");
        require(buyer != seller, "Kaby: cannot buy your own Hero");
    ...
    function offer(uint heroId, uint offerValue) external override nonReentrant {
        ...
        require(buyer != ownerOf(heroId), "Kaby: owner cannot offer");
        require(offerValue != currentOffer, "Kaby: same offer");
    ...
    function takeOffer(
        ...
        require(offeredValue > 0, "Kaby: no offer found");
        require(offeredValue >= minPrice, "Kaby: less than min price");
        require(buyer != seller, "Kaby: cannot buy your own Hero");
    ...
    function cancelOffer(uint heroId) external override nonReentrant {
        ...
        require(offerValue > 0, "Kaby: no offer found");
     ...
     */
    function upgradeStar(uint heroId, uint amount) external override {
        Hero storage hero = _heros[heroId];
        uint newStar = hero.star + amount;

        require(amount > 0);
        require(newStar <= maxStar, "Kaby: max Star reached");

        hero.star = newStar;

        emit HeroStaredUp(heroId, newStar, amount);
    }
     ...
     * @return equipment return detail info of hero. Current is a array has info about equipments 
     */
    function getHero(uint heroId) external view override returns (
        uint star,
        uint[4] memory equipment
    ) {
    ...
    function _createHero() private returns (uint heroId) {
        _heros.push(Hero(1, 0, 0, 0, 0));
        heroId = _heros.length - 1;
        emit HeroCreated(heroId);
    }

    function _setHeroEquipment(uint heroId, uint[] memory itemIds, bool isRemove) private {
        require(herosOnSale[heroId] == 0, "Kaby: cannot change items while on sale");
        require(itemIds.length > 0, "Kaby: no item");

        Hero storage hero = _heros[heroId];
        bool[] memory itemSet = new bool[](4);
        ...
        for (uint i = 0; i < itemIds.length; i++) {
            ...
            require(itemId != 0, "Kaby: invalid itemId");
            require(!itemSet[uint(itemType)], "Kaby: duplicate itemType");

            if (itemType == IEquipment.ItemType.WEAPON) {
                require(isRemove ? hero.weapon == itemId : hero.weapon == 0, "Kaby : invalid weapon");
                hero.weapon = updatedItemId;
                itemSet[uint(IEquipment.ItemType.WEAPON)] = true;
            } else if (itemType == IEquipment.ItemType.ARMOR) {
                require(isRemove ? hero.armor == itemId : hero.armor == 0, "Kaby : invalid armor");
                hero.armor = updatedItemId;
                itemSet[uint(IEquipment.ItemType.ARMOR)] = true;
            } else if (itemType == IEquipment.ItemType.FOOTWEAR) {
                require(isRemove ? hero.footwear == itemId : hero.footwear == 0, "Kaby : invalid footwear");
                hero.footwear = updatedItemId;
                itemSet[uint(IEquipment.ItemType.FOOTWEAR)] = true;
            } else if (itemType == IEquipment.ItemType.RING) {
                require(isRemove ? hero.ring == itemId : hero.ring == 0, "Kaby : invalid ring");
                hero.ring = updatedItemId;
                itemSet[uint(IEquipment.ItemType.RING)] = true;
            }
        }
    }
...
interface IEquipment {
    enum ItemType { WEAPON, ARMOR, FOOTWEAR, RING}
...
interface IKaby {
    ...
    event HeroStaredUp(uint indexed heroId, uint newStar, uint amount);
