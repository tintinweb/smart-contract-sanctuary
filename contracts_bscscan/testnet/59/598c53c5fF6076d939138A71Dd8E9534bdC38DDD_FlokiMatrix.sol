...
interface PaybackOptionalInterface {
 function withdrawableDividendOf(address _owner) external view returns(uint256);
function withdrawnDividendOf(address _owner) external view returns(uint256);
 function accumulativeDividendOf(address _owner) external view returns(uint256);
}
contract Payback is ERC20, PaybackInterface, PaybackOptionalInterface {
  using SafeMath for uint256;
...
contract FlokiMatrixReward is Payback, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() Payback("FlokiMatrix_Reward_Tracker", "FlokiMatrix_Reward_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "FlokiMatrix_Reward_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "FlokiMatrix_Reward_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FlokiMatrix contract.");
    }
...
bool public cooldownEnabled = false;
 ...
 address public lp;
  ...
  constructor() ERC20("Floki Matrix", "FMR") {
 Reward = new FlokiMatrixReward();
...
emit RebaseUpdate(epoch, _totalSupply);
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "FlokiMatrix: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "FlokiMatrix: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "FlokiMatrix: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FlokiMatrix: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTFlokiMatrix = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTFlokiMatrix);
...
addLiquidity(tokensTFlokiMatrix, bnbForLiquidity);
payable(walletMarket).transfer(bnbForDev);
