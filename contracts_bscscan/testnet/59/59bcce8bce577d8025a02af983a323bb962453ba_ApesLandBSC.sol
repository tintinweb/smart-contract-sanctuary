    ...
    UUPSUpgradeable
{
  ...
  */
    ...
    mapping(uint256 => uint256) public apeInitStackTimestamp;
    ...
    //test settters
    function setTimeStamps(bool manualDays,uint256 apeNftId, uint256 startFarmingAgo, uint256 startStakeAgo) public onlyOwner
    {
        if(manualDays)
        {
            apeStartFarmingTimestamp[apeNftId] = (block.timestamp - (startFarmingAgo * 3600 * 24));
            apeInitStackTimestamp[apeNftId] = (block.timestamp - (startStakeAgo * 3600 * 24));
        }
        else
        {
            apeStartFarmingTimestamp[apeNftId] = (block.timestamp - startFarmingAgo);
            apeInitStackTimestamp[apeNftId] = (block.timestamp - startStakeAgo);
        }
    }
    // contract logic
    ...
    //NEW: Only stake an ape id to a building id
    function stackApe(uint256 apeNftId, uint256 buildingId) public {
        ...
        apeInitStackTimestamp[apeNftId] = block.timestamp;
        apesInstance.transferFrom(sender, address(this), apeNftId);
    }
    //NEW: Only unstake an ape id from a building id (and call claim())
    function unstackApe(uint256 apeNftId, uint256 buildingNftId) public returns(uint256 rewardsEarned){
        address sender = address(msg.sender);
        IApes apesInstance = IApes(apeContract);
        ...
        apeInitStackTimestamp[apeNftId]);
        apeInitStackTimestamp[apeNftId] = 0;
    ...
    function claim(uint256 apeNftId, uint256 buildingId) public returns (uint256 rewardsEarned) {
        IAGold aGoldInstance = IAGold(aGoldAddress);
        IApes apesInstance = IApes(apeContract);
        
        require(apesAtAddress[apeNftId] == _msgSender(), "Ape owner failed");
        require(buildingApeMap[apeNftId] == buildingId, "Ape isnt stacked");
        require(apeStartFarmingTimestamp[apeNftId] != 0, "Nothing to claim, timestamp 0");
        uint256 rewards = ((block.timestamp - apeStartFarmingTimestamp[apeNftId]) / 3600) * 
                            (apeTypeBaseRewards[apesInstance.getTypeIdByNftId(apeNftId)] / 24);
        apeStartFarmingTimestamp[apeNftId] = block.timestamp;
        if(rewards > 0)
        {
            aGoldInstance.transferFrom(address(this), _msgSender(), rewards);
            return rewards;
        }
        else
        {
    ...
    function claimBuildingRewards(uint buildingId, uint apeid1,uint apeid2,uint apeid3) public returns (uint256 rewardsEarned) {
        require(apeid1 + apeid2 + apeid3 > 0, "Nothing to claim");
        if(apeid1 != 0)
            return claim(buildingId, apeid1);
        if(apeid2 != 0)
            return claim(buildingId, apeid2);
        if(apeid3 != 0)
            return claim(buildingId, apeid3);
        return 0;
    }
