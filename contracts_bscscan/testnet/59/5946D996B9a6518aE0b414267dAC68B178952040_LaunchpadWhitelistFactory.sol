...
contract LaunchpadWhitelistFactory is EIP712, ILaunchpadWhitelistFactory {
    ...
    function deployWithTypedSignature(
        ...
        _deploy(implementation, uuid, name, msg.sender, signature);
    }
    ...
    function _deploy(
        ...
        require(
            verifyTypedSignature(implementation, uuid, name, owner, signature),
            "LaunchpadWhitelistFactory: invalid signature"
        );

        bytes memory saltdata = _encodeSaltData(owner, uuid, name);
        bytes32 salt = keccak256(abi.encodePacked(saltdata, owner));
        ...
        emit Deployed(implementation, deployedContract, uuid, name, owner);
    }
