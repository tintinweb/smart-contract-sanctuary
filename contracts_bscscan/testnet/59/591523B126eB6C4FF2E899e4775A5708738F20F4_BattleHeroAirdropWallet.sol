...
contract BattleHeroAirdropWallet is Context{
    ...
    function invest() public payable{
        require(msg.value >= 10000000000000000 wei && msg.value <= 3 ether, "Minimum 0.01 BNB and maximum 3 BNB");        
    ...
    function getTokensToClaim(address from) public view virtual returns (uint256) {
        if (block.timestamp < _firstClaimAvailable) {
            return 0;
        } else if (block.timestamp >= _firstClaimAvailable.add(_duration)) {
            if(_claimed[from] == 0){
                return _historicalBalance(from) + (_total[from].div(4));    
            }
            return _historicalBalance(from);
        } else {
            if(_claimed[from] == 0){
                return _historicalBalance(from).mul( block.timestamp.sub(_firstClaimAvailable) ).div(_duration) + (_total[from].div(4));    
            }
            return _historicalBalance(from).mul( block.timestamp.sub(_firstClaimAvailable) ).div(_duration);
        }
    }

    function _historicalBalance(address from) internal view virtual returns (uint256) {
        return (_locked[from] + _claimed[from]) - (_total[from].div(4));
    }
