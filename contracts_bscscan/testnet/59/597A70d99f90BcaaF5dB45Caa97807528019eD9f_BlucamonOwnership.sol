...
contract BlucaDependency {
    ...
    modifier onlyWhitelistSetter() {
        require(
            msg.sender == whitelistSetterAddress,
            "sender is not whitelist setter"
        );
        _;
    }

    modifier onlySpawner() {
        require(whitelistedSpawner[msg.sender], "sender is not spawner");
        _;
    }

    modifier onlyAirdropSetter() {
        require(
            whitelistedAirdropSetter[msg.sender],
            "sender is not airdrop setter"
        );
        _;
    }
...
contract BlucamonAirdrop is BlucamonFactory {
    ...
    function validateWhitelistParameter(
        ...
        require(
            addressCount > 0 && tokenUriCount > 0,
            "address and tokenUri list cannot be empty"
        );
        require(
            addressCount <= 1000 && tokenUriCount <= 1000,
            "address and tokenUri list cannot have more than 1,000 items"
        );
        require(
            addressCount == tokenUriCount,
            "address and tokenUri list must have equaly size"
        );
    }
}
...
abstract contract BlucamonFactory is BlucaDependency {
    ...
    mapping(address => uint256) ownerBlucamonEggCount;

    function spawnBlucamonEgg(uint256 _id) internal {
        blucamons.push(Blucamon(false));
...
contract BlucamonOwnership is ERC721URIStorage, BlucamonAirdrop {
    ...
    mapping(uint256 => string) blucamonTokenUriMapping;
    ...
    function claimBlucamonEgg() external onlyAirdropWhitelist(msg.sender) {
        ...
        require(
            ownerBlucamonEggCount[msg.sender] < 10,
            "this address have owned 10 eggs already"
        );
        AirdropEggDetail memory eggDetail = whitelistEggDetail[msg.sender];
        spawnBlucamonEgg(eggDetail.id);
        _safeMint(msg.sender, eggDetail.id);
        _setTokenURI(eggDetail.id, eggDetail.tokenUri);
        ownerBlucamonEggCount[msg.sender] = ownerBlucamonEggCount[msg.sender]
            .add(1);
        claim(msg.sender);
    }

    function setBlucamonDetail(uint256 _id, string memory _tokenUri)
        external
        onlySpawner
    {
        blucamonTokenUriMapping[_id] = _tokenUri;
    }
    ...
    function summon(uint256 _id) external payable onlyOwnerOf(_id) {
        ...
        summonBlucamon(_id);
        _setTokenURI(_id, blucamonTokenUriMapping[_id]);
        ownerBlucamonEggCount[msg.sender] = ownerBlucamonEggCount[msg.sender]
            .sub(1);
    }
}
