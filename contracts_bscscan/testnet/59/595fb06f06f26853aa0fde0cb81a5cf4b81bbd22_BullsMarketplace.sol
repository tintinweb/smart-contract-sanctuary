...
contract BullsMarketplace is ERC721Holder, Ownable, ReentrancyGuard {
  ...
  mapping(address => uint256) public holderDividentBalance;
  mapping(address => uint256) public totalHoldersDivident;
  mapping(address => uint256) public mintersDividentBalance;
  mapping(address => uint256) public totalMintersDivident;
  mapping(address => mapping(uint256 => uint256)) lastHolderDividendAt;
  mapping(address => mapping(address => uint256)) lastMinterDividendAt;
  mapping(address => mapping(address => uint256)) minterCount; // Number mints per address for a collection
  ...
  struct Collection {
    ...
    uint256 reflectionMintRoyalty;
    uint256 reflectionHolderRoyalty;
    uint256 collectionSize;
  }

  event AddMinterDataForAccounts(address indexed collection, address[] accounts, uint256[] amounts);
  ...
  event CollectionNew(address indexed collection, address indexed creator, address indexed whitelistChecker, uint256 tradingFee, uint256 creatorFee, uint256 reflectionMintRoyalty, uint256 reflectionHolderRoyalty, uint256 collectionSize);

  // Existing collection is updated
  event CollectionUpdate(address indexed collection, address indexed creator, address indexed whitelistChecker, uint256 tradingFee, uint256 creatorFee, uint256 reflectionMintRoyalty, uint256 reflectionHolderRoyalty, uint256 collectionSize);
  ...
  event Trade(address indexed collection, uint256 indexed tokenId, address indexed seller, address buyer, uint256 askPrice, uint256 netPrice, bool withBNB);
  ...
  function addCollection(
    ...
    uint256 _tradingFee,
    uint256 _creatorFee,
    uint256 _reflectionMintRoyalty,
    uint256 _reflectionHolderRoyalty,
    ...
    holderDividentBalance[_collection] = 0;
    totalHoldersDivident[_collection] = 0;
    mintersDividentBalance[_collection] = 0;
    totalMintersDivident[_collection] = 0;
  ...
  function addDividendForHolders(address _collection, uint256 amount) private {
    holderDividentBalance[_collection] = holderDividentBalance[_collection] + amount;
    totalHoldersDivident[_collection] = totalHoldersDivident[_collection] + (amount / _collections[_collection].collectionSize);
  }

  function addReflectionForHolders(address _collection) public payable {
    require(_collectionAddressSet.contains(_collection), "Operations: Collection not listed");
    addDividendForHolders(_collection, msg.value);
  }

  function currentHoldersRate(address _collection) public view returns (uint256) {
    require(_collectionAddressSet.contains(_collection), "Operations: Collection not listed");
    return holderDividentBalance[_collection] / _collections[_collection].collectionSize;
  }

  function addDividendForMinters(address _collection, uint256 amount) private {
    mintersDividentBalance[_collection] = mintersDividentBalance[_collection] + amount;
    totalMintersDivident[_collection] = totalMintersDivident[_collection] + (amount / _collections[_collection].collectionSize);
  }

  function addReflectionForMinters(address _collection) public payable {
    require(_collectionAddressSet.contains(_collection), "Operations: Collection not listed");
    addDividendForMinters(_collection, msg.value);
  }

  function currentMintersRate(address _collection) public view returns (uint256) {
    require(_collectionAddressSet.contains(_collection), "Operations: Collection not listed");
    return mintersDividentBalance[_collection] / _collections[_collection].collectionSize;
  }

  function addMinterDataForAccounts(address _collection, address[] calldata accounts, uint256[] calldata mints) public onlyOwner {
    require(_collectionAddressSet.contains(_collection), "Operations: Collection not listed");
  ...
  function getMintCount(address _collection, address minterAddress) public view returns (uint256) {
    require(_collectionAddressSet.contains(_collection), "Operations: Collection not listed");
  ...
  function claimRewards() external nonReentrant {
    ...
    for (uint256 i = 0; i < _collectionAddressSet.length(); i++) {
      address _collection = _collectionAddressSet.at(i);
      allBalance += _getDividentBalancesForCollection(_collection, true);
    }
   ...
   * @param _isClaiming: if claiming rewards instead of checking balance
   * @notice Return total divident balance and individuals too
   */
  function _getDividentBalancesForCollection(address _collection, bool _isClaiming) 
    private
    returns (uint256)
  {
    // current bulls count
    uint256 count = bullContract.balanceOf(msg.sender);

    uint256 holderBalance = 0;
    for (uint256 i = 0; i < count; i++) {
      uint256 bullId = bullContract.tokenOfOwnerByIndex(msg.sender, i);
      holderBalance += (totalHoldersDivident[_collection] - lastHolderDividendAt[_collection][bullId]);  
      if(_isClaiming){
        lastHolderDividendAt[_collection][bullId] = totalHoldersDivident[_collection];
      }
    }
    
    uint256 mintBalance = (totalMintersDivident[_collection] - lastMinterDividendAt[_collection][msg.sender]) * getMintCount(_collection, msg.sender);
    if(_isClaiming){
      lastMinterDividendAt[_collection][msg.sender] = totalMintersDivident[_collection];
    }

    return (holderBalance + mintBalance);
  }
    ...
    returns (uint256)
  {
    ...
    for (uint256 i = 0; i < _collectionAddressSet.length(); i++) {
      address _collection = _collectionAddressSet.at(i);
      allBalance += _getDividentBalancesForCollection(_collection, false);
    }
    ...
    )
  {
    ...
    holdersFee = (_askPrice * _collections[_collection].reflectionHolderRoyalty) / 100;
    minterFee = (_askPrice * _collections[_collection].reflectionMintRoyalty) / 100;

    tradingFee = (_askPrice * _collections[_collection].tradingFee) / _collections[_collection].collectionSize;
    creatorFee = (_askPrice * _collections[_collection].creatorFee) / _collections[_collection].collectionSize;

    netPrice = _askPrice - tradingFee - creatorFee - holdersFee - minterFee;

    return (netPrice, tradingFee, creatorFee, holdersFee, minterFee);
  }
  ...
  function _buyToken(
    ...
    (uint256 netPrice, uint256 tradingFee, uint256 creatorFee, uint256 holdersFee, uint256 minterFee) = _calculatePriceAndFeesForCollection(_collection, _price);

    addDividendForHolders(_collection, holdersFee);
    addDividendForMinters(_collection, minterFee);
