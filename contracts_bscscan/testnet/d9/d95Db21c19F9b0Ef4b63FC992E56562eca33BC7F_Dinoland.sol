...
contract Dinoland is ERC20, Ownable {
    ...
    address public constant RESERVE_ADDRESS = 0xb40eA1DA6A55bDf2b5BFbd054cC7dDB53F8ac615;
    
    constructor() ERC20("Dinoland Metaverse", "DNL") {
        _maxTotalSupply = 10000000000 ether;
        TimelockFactory timelockFactory = new TimelockFactory();

        mint(SEED_ADDRESS, 700000 ether);
        address seedERC20Lock = timelockFactory.createTimelock(this, SEED_ADDRESS, block.timestamp + 30 days, 775000 ether, 30 days);
        mint(seedERC20Lock, 9300000 ether);
        
        mint(PRIVATE_SALE_ADDRESS, 5000000 ether);
        address privateERC20Lock = timelockFactory.createTimelock(this, PRIVATE_SALE_ADDRESS, block.timestamp + 30 days, 5000000 ether, 30 days);
        mint(privateERC20Lock, 45000000 ether);
        
        mint(IDO_ADDRESS, 6000000 ether);
        address idoERC20Lock = timelockFactory.createTimelock(this, IDO_ADDRESS, block.timestamp, 3500000 ether, 30 days);
        mint(idoERC20Lock, 14000000 ether);
        
        mint(LIQUIDITY_ADDRESS, 100000000 ether);

        address gameIncentiveERC20Lock = timelockFactory.createTimelock(this, GAME_INCENTIVES_ADDRESS, block.timestamp, 12500000 ether, 30 days);
        mint(gameIncentiveERC20Lock, 300000000 ether);
        
        address farmingERC20Lock = timelockFactory.createTimelock(this, FARMING_ADDRESS, block.timestamp, 8333334 ether, 30 days);
        mint(farmingERC20Lock, 200000000 ether);
        
        address marketingERC20Lock = timelockFactory.createTimelock(this, MARKETING_ADDRESS, block.timestamp, 0, 0);
        mint(marketingERC20Lock, 100000000 ether);
        
        address teamERC20Lock = timelockFactory.createTimelock(this, TEAM_ADDRESS, block.timestamp + 180 days, 10000000 ether, 30 days);
        mint(teamERC20Lock, 120000000 ether);
        
        address advisorERC20Lock = timelockFactory.createTimelock(this, ADVISOR_ADDRESS, block.timestamp + 180 days, 1666667 ether, 30 days);
        mint(advisorERC20Lock, 20000000 ether);
        
        address airdropERC20Lock = timelockFactory.createTimelock(this, AIRDROP_ADDRESS, block.timestamp, 0, 0);
        mint(airdropERC20Lock, 30000000 ether);

        address reserveERC20Lock = timelockFactory.createTimelock(this, RESERVE_ADDRESS, block.timestamp, 0, 0);
        mint(reserveERC20Lock, 50000000 ether);
    }
