...
contract DragonXStaking is AccessControl {
    ...
    function setRate(uint256 _newAPY) public onlyRole(ADMIN_ROLE) {
        require(_newAPY < 1000, "Hard limit 1,000");
        APY = _newAPY;
    }
    
    function setBond(uint256 _bondingPeriod) public onlyRole(ADMIN_ROLE) {
    ...
    function timeStaked() public view returns(uint256) {
        ...
        uint256 timeElapsed = currentTime - s.timestamp; 
        return timeElapsed;
    }
    ...
    function stake(uint256 _amount) public {
        ...
        stakes[msg.sender] = Stake(msg.sender, _amount + alreadyStaked, block.timestamp, block.timestamp); // add stake to stake array
        totalStaked += _amount; // increment total staked in pool (TVL)
        DAX_TOKEN.transferFrom(msg.sender, address(this), _amount);
    }
    
    function bondRemaining() public view returns(uint256) {
        Stake memory s = myStake();
        if (block.timestamp > s.bonded) { // prevent negative overflow
            if (bondingPeriod > (block.timestamp - s.bonded)) {
                return bondingPeriod - (block.timestamp - s.bonded);
            }
            else {
                return 0;
            }
    ...
    function pullStake() public {
        ...
        uint256 timeElapsed = currentTime - s.bonded; // calculate time elapsed since stake created or rewards last withdrawn
    ...
    function reward() public view returns(uint256)  {
        ...
        if (s.amount > 0) { // staking
            uint256 currentTime = block.timestamp; // get current time in seconds
            uint256 timeElapsed = currentTime - s.timestamp; // calculate time elapsed since stake created or rewards last withdrawn
            uint256 r = (s.amount * timeElapsed) / 31557600; // calculate rewards relative to number of seconds in a year (31557600)
            r = r * APY / 100; // calculate rewards according to APY
            return r;
        }
        else { // not staking
            return 0;
