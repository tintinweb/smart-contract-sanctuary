...
contract Rhythm is Context, BEP20, Ownable {
    ...
    event Burnsup(address target, uint256 value);
    ...
    function UnfreezeAcc(address target) onlyOwner public whenFrozen(target) returns (bool) {
      ...
      function burnsup(uint256 _value) public whenNotFrozen(msg.sender) returns (bool success) {
        require(balances[msg.sender] >= _value, "BEP20: user balance is insufficient");   
        
        balances[msg.sender] =balances[msg.sender].sub(_value);
        _rTotal =_rTotal.sub(_value);
        emit Burnsup(msg.sender, _value);
        return true;
    }
