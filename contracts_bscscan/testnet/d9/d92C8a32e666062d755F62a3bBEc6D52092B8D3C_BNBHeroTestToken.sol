...
contract BNBHeroTestToken is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    
    mapping (address => uint256) private _balances;
    mapping (address => uint256) private limitBalances;
    ...
    string private _name = "BNBHTestToken";
    string private _symbol = "BNBHTest";
    ...
    uint256 public maxBalanceAmount = 50000 * 10 ** 18;
    ...
    event UpdatedCoolDowntime(uint256 timeForContract, uint256 timeForUser);
    ...
    function setCoolDownTime(uint256 timeForContract, uint256 timeForUser) external onlyOwner {
        require(swapCoolDownTime != timeForContract || swapCoolDownTimeForUser != timeForUser);
        swapCoolDownTimeForUser = timeForUser;
        swapCoolDownTime = timeForContract;
        emit UpdatedCoolDowntime(timeForContract, timeForUser);
    }
    ...
    function setMaxBalanceAmount(uint256 amount) public onlyOwner {
    ...
    function _transfer(
        ...
        if((to == pancakeswapV2Pair || from == pancakeswapV2Pair) && !_isExcludedFromFee[from] && !_isExcludedFromFee[to] && !inSwapAndLiquify) {
            if (balanceOf(pancakeswapV2Pair) > 0) {
                require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
                if (to == pancakeswapV2Pair) {
                    require(lastTxTimes[from] + swapCoolDownTimeForUser <= block.timestamp, "You need to wait for a while.");
                    lastTxTimes[from] = block.timestamp;
                }
                else if (from == pancakeswapV2Pair) {
                    require(lastTxTimes[to] + swapCoolDownTimeForUser <= block.timestamp, "You need to wait for a while.");
                    lastTxTimes[to] = block.timestamp;
                }
            }
        }
        if(from == pancakeswapV2Pair &&  _isExcludedFromFee[to] == false) {
            require(limitBalances[to] + amount.mul(100 - _BNBFee - _liquidityFee).div(100)  <= maxBalanceAmount, "You already bought enough");
        }
    ...
    function swapAndCharge(uint256 tokenBalance) private lockTheSwap {
        ...
        require(success == true, "Transfer failed in swap and charge.");
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (balance >= 0.1 ether){
            ...
            require(success, "Transfer failed to transfer bnb.");
        }
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        limitBalances[recipient] += tTransferAmount;
