...
contract DXPresale is Owned {
  ...
  function claimTokens() public{
    ...
    function _addLiquidityToken() internal{
        ...
        if(contractBalance >= tokenAmount3){
            addLiquidity(tokenAmount3,hardCap);
        }else if(contractBalance >= tokenAmount2){
    ...
    function checkTokentoAddLiquidty() public view returns(uint256) {
         uint256 contractBalance = IToken(tokenAddress).balanceOf(address(this)).sub(soldTokens.add(totalSold));
        if((softCap.mul(tokenRatePerEthPancake)) > contractBalance){
            return (earnedCap.mul(tokenRatePerEthPancake).div(10e18)).sub(contractBalance).div(10 ** tokenDecimals);
        }else{
            return uint256(0);
        }
    }
