...
contract Presale is Ownable, ReentrancyGuard {
    ...
    // uint256 public tt_rate = 15 * 100000000;
    ...
    function buyTokens(address _des) payable external nonReentrant {
        ...
        require(usdAmount>=40, "BuyFail: MinBuyAmount");
    ...
    function getAmountToGive(uint256 usdAmount) view public returns(uint256) {
        uint256 amountToGive = usdAmount.mul(comFiAllocated[currentPhase]).div(usdForPhases[currentPhase]).mul(IERCBurn(token).decimals());
        return amountToGive;
    }
