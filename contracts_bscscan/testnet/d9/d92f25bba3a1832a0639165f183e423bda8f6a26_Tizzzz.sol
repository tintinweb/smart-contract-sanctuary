...
contract BEP20 is IBEP20, Ownable {
    ...
    string private constant NAME = "Tizzzz";
    string private constant SYMBOL = "Tizzzz";
...
contract Tizzzz is BEP20 {
    ...
    uint256 public gameDevShare = 100;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (!isExcludedFromFees[sender] || !isExcludedFromFees[recipient]) { require(tradingEnabled, "Trading is disabled"); }
    ...
    // cooldown timer, so a bot doesnt do quick trades! 1min gap between 2 trades.
        ...
        uint256 amountAfterTaxes = _shouldTakeTaxes(sender) || _shouldTakeTaxes(recipient) ? _takeTax(sender, recipient, amount) : amount;
    ...
    function _takeTax(address sender, address recipient, uint256 amount) private returns (uint256) {
        ...
        if(burnTokens > 0){
            super._transfer(sender, burnFeeReceiver, taxAmount); 
        }
