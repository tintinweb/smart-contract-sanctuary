...
contract VRP is IVRP, Initializable, JaxOwnable {
    ...
    function get_latest_apy() external view returns(uint) {
        ...
        uint period = (last1Epoch.timestamp - last2Epoch.timestamp) / 3600 / 24;
        return 1e8 * 365 * last1Epoch.reward / IERC20(rewardToken).balanceOf(IJaxAdmin(jaxAdmin).jaxSwap()) / period;
    }
