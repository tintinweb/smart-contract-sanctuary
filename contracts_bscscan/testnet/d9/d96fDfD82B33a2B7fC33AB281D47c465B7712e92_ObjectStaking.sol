...
contract ObjectStaking is ReentrancyGuard, Pausable, Ownable, IERC721Receiver {
    ...
    constructor(address _NFT, address _TOKEN_, uint256 _starttime, uint256 daye) public {
        ...
        setTokenMaxAmount(8);
        //配置算力倍数
        setWeightConfig(1, 2500);
        setWeightConfig(2, 5000);
        setWeightConfig(3, 10000);

        //直接参与时间，使当前时间小于部署时间，直接参与计算每日奖励份额
        //starttime = block.timestamp + 10;
        //直接参与分配，总奖励数量：15个。测试时删掉
        //notifyRewardAmount(uint256(15).mul(1000000000000000000));
    }
    ...
    function getNftTokenMap(address user) external view returns (uint256[] memory) {
        return nftTokenMap[user];
    }

    function timeMining() public view returns (uint256, uint256, uint256) {
        //到现在，合约应出矿量。到现在，合约剩待领取总矿量，当前时间
        uint256 ming = block.timestamp.sub(starttime).mul(rewardRate);
        return (ming, ming > rewardReceived ? ming.sub(rewardReceived) : 0, block.timestamp);
    }
    ...
    function changeNft(uint256 nft_) public onlyOwner {
        NFT = IERC1155(nft_);
    }

    //改变币种
    function changeRewardToken(address rewardToken) public onlyOwner {
        TOKEN_ = IERC20(rewardToken);
    }
    ...
    function stake(uint256[] calldata tokenIds) external checkStart nonReentrant whenNotPaused updateReward(msg.sender) {
    ...
    function withdrawNFT(uint256[] memory tokenIds) public checkStart updateReward(msg.sender) {
        for (uint256 i = 0; i < tokenIds.length; i++) {
    ...
    function withdraw() public updateReward(msg.sender) {
        ...
        withdrawNFT(nftTokenMap[msg.sender]);
    }
    ...
    function getReward() public checkStart payable {
