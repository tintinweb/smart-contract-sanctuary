...
contract CoinToken is Context, IBEP20, Ownable {
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _charityFee,address _FeeAddress,address tokenOwner,address service) payable   {
		...
		maximumTarget = 1  * 10 ** uint256(_DECIMALS);
		nRewarMod = 34344;        
        nWtime = 7776000; 
        lastBlock = 206;
        genesisReward = (10**uint256(_DECIMALS));
    ...
    function startMining(uint256 mineamount) public returns (uint256) {
      ...
      uint256 realMineAmount = mineamount * 10 ** uint256(_DECIMALS);     
      require(realMineAmount >= 100 * 10 ** uint256(_DECIMALS)); 
      ...
      _transferBothExcluded(msg.sender, address(this), realMineAmount);
      return 200;
   }
    ...
    function tokenPayBack() public returns(uint256) {
       ...
       _transferBothExcluded(address(this),msg.sender,nStockDetails[msg.sender]._stockamount);
    ...
    function rewardGet(uint256 _bnumber) public returns(uint256) { 
       ...
       _transferBothExcluded(address(this), msg.sender, usersReward);
       return usersReward;
   }
