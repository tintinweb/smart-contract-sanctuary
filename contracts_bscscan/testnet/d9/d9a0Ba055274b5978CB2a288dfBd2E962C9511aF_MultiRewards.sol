...
contract MultiRewards is ReentrancyGuard, Pausable {
    ...
    function checkTotalReward(address account, address _rewardsToken) public view returns (uint256, uint256, uint256, uint256, uint256 /*, uint256, uint256 , uint256 */) {
        ...
        return (reward, actualAmount, stake0R, stake1R, stake2R /* , stake3R, stake4R, stake5R */);
    }  
    
        // the discrepancy should be 0 in theory   
    function checkTotalReward2(address account, address _rewardsToken) public view returns (uint256, uint256, uint256, uint256, uint256 /*, uint256, uint256 , uint256 */) {
        uint256 reward = earned( account,  _rewardsToken );  // this function does not rely on the state variables
    ...
    // the discrepancy should be 0 at all time  
    function checkTotalStake(address account) public view returns (uint256, uint256, uint256, uint256, uint256 /*, uint256, uint256, uint256 */) {
        uint256 stakeAll = _balances[account];  
        ...
        //uint256 stake5 = 0;
        for (uint j; j < userStakes[account].length; j++) {
            uint256 outAmount = userStakes[account][j].remainingBalance; // this function does not rely on state variables  
    	    actualAmount = actualAmount.add(outAmount);
    	    if (j ==0)
    	        stake0 = outAmount;
    	    else if (j==1)
        ...
        return (stakeAll, actualAmount, stake0, stake1, stake2 /* , stake3, stake4, stake5 */);
    } 
    
    // the discrepancy should be 0 at all time  
    function checkTotalStake2(address account) public view returns (uint256, uint256, uint256, uint256, uint256 /*, uint256, uint256, uint256 */) {
        uint256 stakeAll = _balances[account];  
