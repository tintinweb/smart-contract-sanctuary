...
contract ElvantisLandPresale is Ownable, ERC721Holder, ERC1155Holder {
    ...
    function addPack(address nft,  uint256[] memory tokenIds, uint8 nftType, address paymentToken, uint256 price, bool enabled) public onlyOwner {
        require(price > 0, "ElvantisLandPresale: Invalid Price!");
        require(tokenIds.length > 0, "ElvantisLandPresale: Items must not be empty");
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            pack.tokenIds.push(tokenIds[i]);
        }

        emit PackAdded(price, enabled, tokenIds);
    }

    function setPackEnabled(uint256 packId, bool enabled) external onlyOwner {
        require(packs.length > packId, "ElvantisLandPresale: Invalid Pack Id");
    ...
    function buyPack(uint256 packId, uint256 quantity) external payable {
        ...
        require(pack.enabled, "ElvantisLandPresale: Pack disabled!");
        require(quantity > 0, "ElvantisLandPresale: Pack Quantity should not be zero");

        if(address(pack.paymentToken) == address(0)) {
            require(pack.price * quantity == msg.value, 'ElvantisLandPresale: Invalid amount');
            feeRecipient.transfer(pack.price * quantity);
        } else {
            require(msg.value == 0, "ElvantisLandPresale: No eth required");
        ...
        for (uint256 k = 0; k < quantity; k++) {
            for (uint256 i = 0; i < pack.tokenIds.length; i++) {
                nftTransfer(pack.nft, uint8(pack.nftType), address(this), msg.sender, pack.tokenIds[i]);
            }
        }
    ...
    function nftTransfer(address nft, uint8 nftType, address from, address to, uint256 tokenId) internal {
        if(TokenStandard(nftType) == TokenStandard.ERC721){
            ...
            require(false, "ElvantisLandPresale: Unsupported TokenStandard");
        }
    }
}
