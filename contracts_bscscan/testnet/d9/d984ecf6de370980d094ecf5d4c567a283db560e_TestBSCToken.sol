...
contract TestBSCToken is ERC20 {
    ...
    modifier onlySigner {
        require(signers[msg.sender] == true, "You are not the signer of this smart contract.");
        _;
    }


    constructor(address gr, address _signer1, address _signer2) ERC20("TestBSCToken", "TBT"){
    ...
    function getPendingTransaction(uint _id) public view returns(address, uint, string memory, uint) 
    {
        ...
        {
            revert("You are not the owner or signer of this smart contract.");
        }
    ...
    function setTotalSupply(string memory operation, uint _amount) onlyOwner public returns(uint) 
    {
        ...
        {
            require(currentTotalSupply >= _amount, "The amount is greater than the total supply.");
    ...
    function _transferFromContract(address _to, uint256 _value) internal {
        require(_to != address(0), "Error with the buyer address.");
    ...
    function getBuyer(address _address) public view returns(address, uint, uint, uint, uint, uint, uint)
    {
        ...
        {
            revert("Cannot find this address.");
        }
    }
    
    function transfer(address _to, uint256 _amount) override public returns(bool)
    {
        require(!freezeAccount[msg.sender], "Your address has been hold from transfering the token.");
        ...
        else
        {
            ...
            if(block.timestamp > unlockDate12months)
            {
                ...
                {
                    revert("The amount you want to transfer is higher than you're allowed.");
                }
            ...
            {
                revert("You can't transfer these tokens.");
            }
    ...
    function transferFrom(address sender, address _to, uint256 _amount) override onlyOwner public returns(bool)
    {
        if(_to == owner)
        {
    ...
    function approvePendingTransaction(uint _id) onlySigner public
    {
        uint id = _id;
        bool check = checkPendingTransactionList(id);
        require(check, "This ID is not in the transaction pending list.");
        require(result, "Did not have permission from signer.");
        ...
        {
            revert("This transaction is already already successful .");
        }
        else
        {
