...
contract TokenFarmer is Ownable {
	...
	function setupFarm(
			address newTokenToFarm, 
			address newProjectWwallet, 
			address newMarketingWallet) public onlyOwner {
    ...
    function set_projectWallet(address addr) external onlyOwner {
        projectWallet = addr;
    }
    function set_marketingWallet(address addr) external onlyOwner {
        marketingWallet = payable(addr);
    }
    function set_INVEST_MIN_AMOUNT(uint256 value) external onlyOwner {
    ...
    function set_PROJECT_FEE(uint256 value) external onlyOwner {
        require(value < 120); // 12 max
        PROJECT_FEE = value;
    }   
    function set_CONTRACT_LONGEVITY_FEE(uint256 value) external onlyOwner {
        require(value < 250); // 25 max
        CONTRACT_LONGEVITY_FEE = value;
    }
    function set_MARKETING_FEE(uint256 value) external onlyOwner {
        require(value < 100); // 10 max
        MARKETING_FEE = value;
    }
    function set_PlanPercent(uint8 plan, uint256 value) external onlyOwner {
        plans[plan].percent = value;
    }
    function set_PlanTime(uint8 plan, uint256 value) external onlyOwner {
        plans[plan].time = value;
    }
    function set_PlanMinInvest(uint8 plan, uint256 value) external onlyOwner {
        plans[plan].mininvest = value;
    }
    function setAdditionalPercent_Plan1(uint256 value) external onlyOwner {
        require(value < 100); // 100 = 10%
        ADDITIONAL_PERCENT_PLAN_1 = value;
    }
    function setAdditionalPercent_Plan2(uint256 value) external onlyOwner {
        require(value < 100); // 100 = 10%
        ADDITIONAL_PERCENT_PLAN_2 = value;
    }
    function setAdditionalPercent_Plan3(uint256 value) external onlyOwner {
        require(value < 100); // 100 = 10%
        ADDITIONAL_PERCENT_PLAN_3 = value;
    }
    function setAdditionalPercent_Plan4(uint256 value) external onlyOwner {
        require(value < 100); // 100 = 10%
        ADDITIONAL_PERCENT_PLAN_4 = value;
    }
    function setCUTOFF_STEP(uint256 value) external onlyOwner {
        CUTOFF_STEP = value;
    }
}