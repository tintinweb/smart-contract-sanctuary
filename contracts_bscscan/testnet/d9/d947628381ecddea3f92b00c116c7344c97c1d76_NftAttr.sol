...
contract NftAttr is Ownable, Context {
 ...
 constructor() {
  ...
  // 根据模版id获取所有卡牌模版列表
  function getAllLuckyNftInfoTypeIdMapById(uint256 typeId, uint256 templateId)public view returns(LuckNftInfoTemplate memory template){
  ...
  */
  ...
  function genearteNftInfo(uint256 typeId) external  onlyOwnerOrAdminAddress returns(NftInfo memory){
      ...
      uint256 length = LuckyNftInfoTypeIdMap[typeId].length - 1;
