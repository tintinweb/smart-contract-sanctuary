...
contract ShieldNetworkToken is IERC20, Ownable {
    ...
    uint256 _swapAndLiquifyThreshold = 500_000 * 10**9; // 500k
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled) {
                uint256 teamAmount = balanceOf(address(this)).mul(_feeTeam).div(
                    _feeTeam + _feeLiquidity
                );
                swapTokensForEth(teamAmount);
                uint256 currentBalance = address(this).balance;
                if (currentBalance > 0) sendETHToFee(currentBalance);
                if (balanceOf(address(this)) > _swapAndLiquifyThreshold) {
                    swapAndLiquify();
                }
            }
        }
    ...
    function _takeTaxes(uint256 tTaxes) private {
        ...
        if (rTaxes > 0) emit Transfer(_msgSender(), address(this), rTaxes);
    }
