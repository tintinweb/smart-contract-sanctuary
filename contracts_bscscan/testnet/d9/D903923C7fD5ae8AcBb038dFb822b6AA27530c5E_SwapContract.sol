...
contract SwapContract is AccessControlEnumerable, Pausable, ECDSAOffsetRecovery
{
    ...
    ) external whenNotPaused onlyValidToken(token)
    {
        ...
        require(
            amount >= feeAmountOfBlockchain[blockchain][token],
        ...
        onlyValidToken(token)
    {
        ...
        uint256 fee = feeAmountOfBlockchain[numOfThisBlockchain][token];
        uint256 amountWithoutFee = amountWithFee - fee;
        TransferHelper.safeTransfer(token, user, amountWithoutFee);
        TransferHelper.safeTransfer(token, feeAddress, fee);
    ...
    /**
      ...
      * @param token address of token contract to remove from list
      */
...
{
    function getHashPacked(address token, address user, uint256 amountWithFee, bytes32 originalTxHash) public pure returns (bytes32)
    {
