...
contract BuniUniversalV2 is Initializable, AccessControlUpgradeable {
    ...
    int128 public constant BATTLE_REWARDS_GAS_OFFSET_IN_USD = 9223372036854775808; // gas offset: 0.5 USD
    int128 public constant BATTLE_REWARDs_BUR_BASELINE = 5534023222112865484; // baseline: 0.3

    int128 public constant BATTLE_BONUS_ELEMENT_EFFICIENCY = 1383505805528216371; // 7.5%
    ...
    int128 public constant TRAINER_MINT_FEE_BY_BUNI_MIXED_USD = 1291272085159668613120; // 70 USD
    int128 public constant TRAINER_MINT_FEE_BY_BUR_MIXED_USD = 553402322211286548480; // 30 USD

    // Fee to mint trainer, paid in buni only
    //int128 public constant TRAINER_MINT_FEE_BY_BUNI_ONLY_USD = 1844674407370955161600; // 100 USD

    // Fee to mint trainer, paid in bur only
    //int128 public constant TRAINER_MINT_FEE_BY_BUR_ONLY_USD = 1844674407370955161600; // 100 USD

    // Fee to hunt bunicorn, paid in buni only
    int128 public constant BUNICORN_MINT_FEE_BY_BUNI_ONLY_USD = 922337203685477580800; // 50 USD

    // Fee to hunt bunicorn, paid in bur only
    int128 public constant BUNICORN_MINT_FEE_BY_BUR_ONLY_USD = 922337203685477580800; // 50 USD
    ...
    event BattleOutcome(address indexed owner, uint256 indexed trainer, uint256 bunicorn, uint32 target, uint24 playerRoll, uint24 enemyRoll, uint16 expGain, uint256 burGain, int128 usdGain);

    function initialize(
        IERC20 _buniToken,
        IBurToken _burToken,
        TrainersV2 _trainers,
        BunicornsV2 _bunicorns,
        ...
        _setupRole(ROLE_GAME_CONTRACT, msg.sender);
    ...
    function battle(uint256 trainer, uint256 bunicorn, uint32 enemy, uint8 battleMultiplier)
        ...
        drainGlobalStamina(msg.sender, TRAINER_STAMINA_PER_BATTLE * battleMultiplier);

        uint8 drainedTrainerStamina = TRAINER_STAMINA_PER_BATTLE * battleMultiplier;
        (uint8 trainerElement, uint24 basePowerLevel, uint64 timestamp) =
            unpackPlayerData(trainers.getBattleDataAndDrainStamina(trainer, drainedTrainerStamina));

        uint8 drainedBunicornStamina = (BUNICORN_BASE_STAMINA_PER_BATTLE - bunicorns.getStars(bunicorn)) * battleMultiplier;
        (int128 powerMultiplier,, uint8 bunicornElement) = bunicorns.getBattleDataAndDrainStamina(bunicorn, trainerElement, drainedBunicornStamina);
        ...
        performBattle(
            trainer,
            bunicorn,
            getPlayerPower(basePowerLevel, powerMultiplier),
    ...
    function checkEnemy(
        ...
        uint32[4] memory enemies = getEnemiesInternal(
            getPlayerPower(playerBasePower, powerMultiplier),
            staminaTimestamp,
            hour
        );
        ...
        require(foundMatch, "Enemy invalid");
    }
    ...
    function performBattle(
        ...
        uint24 playerRoll = getPlayerPowerRoll(playerBattlePower,traitsTBE,seed);
        uint24 enemyRoll = getEnemyPowerRoll(enemyPower, RandomUtil.combineSeeds(seed,1));

        uint16 exp = _calculateExpGainedForBattle(playerBattlePower, enemyPower) * battleMultiplier;
        int128 rewardsInUSD = _calculateRewardsGainedForBattleInUSD(enemyPower, battleMultiplier, bunicorns.getStars(bunicorn));
        ...
        emit BattleOutcome(msg.sender, trainer, bunicorn, (enemyPower | ((uint32(traitsTBE) << 8) & 0xFF000000)), playerRoll, enemyRoll, exp, rewardsInBUR, rewardsInUSD);
    }
    ...
    function _calculateRewardsGainedForBattleInUSD(uint24 monsterPower, uint8 battleMultiplier, uint8 star) internal pure returns (int128) {
        return BATTLE_REWARDS_GAS_OFFSET_IN_USD.add(
            BATTLE_REWARDs_BUR_BASELINE.mul(
                ABDKMath64x64.sqrt(
                    ABDKMath64x64.divu(uint24(monsterPower * star), 1000)
                )
    ...
    function getPlayerPowerRoll(
        ...
        return uint24(getPlayerElementBonusForBattle(traitsTBE).mulu(playerPower));
    }
    ...
    function getPlayerElementBonusForBattle(uint24 traitsTBE) public pure returns (int128) {
        int128 elementBonus = ABDKMath64x64.fromUInt(1);
        ...
        if(isElementEffective(trainerElement, uint8(traitsTBE >> 16)/*enemy*/)) {
            elementBonus = elementBonus.add(BATTLE_BONUS_ELEMENT_EFFICIENCY);
        }
        else if(isElementEffective(uint8(traitsTBE >> 16)/*enemy*/, trainerElement)) {
    ...
    function getEnemies(uint256 trainer, uint256 bunicorn) public view returns (uint32[4] memory) {
        (int128 powerMultiplier,,) = bunicorns.getBattleData(bunicorn, trainers.getElement(trainer));

        return getEnemiesInternal(
            getPlayerPower(trainers.getTrainerPower(trainer), powerMultiplier),
    ...
    function mintTrainer()
        ...
        if (globalStaminaTimestamp[msg.sender] == 0) {
            globalStaminaTimestamp[msg.sender] = uint64(now.sub(getMaxGlobalStaminaCooldown()));
        }

        uint256 seed = uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), msg.sender, nonce)));
        trainerRoller.mintOneRandomTrainer(msg.sender, seed);
    ...
    function fuseTrainer(uint256 fuseID, uint256 burnID)
        ...
        requestPayBurFromPlayer(calculateFuseTrainerFee(fuseID))
    {
        _payBurContract(msg.sender, calculateFuseTrainerFee(fuseID));
        trainers.fuse(fuseID, burnID);
        nonce = nonce + 1;
    }
    ...
    function _getFuseTrainerFee(uint8 levelIndex) internal pure returns(int128 fee) {
        ...
        if (levelIndex == 0) {
            return ABDKMath64x64.divu(1000, 100);
        }
        ...
        oncePerBlock(msg.sender)
    {
        ...
        uint256 seed = uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), msg.sender, nonce)));
        bunicornRoller.mintOneRandomBunicorn(msg.sender, seed);
        nonce = nonce + 1;
    }
    ...
    function _restricted() internal view {
        require(hasRole(ROLE_GAME_CONTRACT, msg.sender), "NGA");
    }
    ...
    function claimExpRewards() public {
        ...
        for(uint256 i = 0; i < trainers.balanceOf(msg.sender); i++) {
            ...
            uint256 maxClaimExp = trainers.getMaxClaimExp(trainer);
            ...
            trainers.claimExp(trainer, uint16(expRewardsToClaim));
        }
    ...
    // =========================== GLOBAL STAMINA ===========================
    function getMaxGlobalStaminaCooldown() public pure returns (uint64) {
        return uint64(MAXIMUM_GLOBAL_STAMINA * GLOBAL_STAMINA_RECOVER_SPEED_IN_SECONDS);
    }

    function getGlobalStaminaTimestamp(address _playerAddress) public view returns (uint64) {
    ...
    function getGlobalStaminaPoints(address _playerAddress) public view returns (uint16) {
        return getGlobalStaminaPointsFromTimestamp(globalStaminaTimestamp[_playerAddress]);
    }

    function getGlobalStaminaPointsFromTimestamp(uint64 _timestamp) public view returns (uint16) {
        if(_timestamp  > now) {
            return 0;
        }
    ...
    function drainGlobalStamina(address _playerAddress, uint8 _globalStamina) internal {
        uint16 globalStaminaPoints = getGlobalStaminaPointsFromTimestamp(globalStaminaTimestamp[_playerAddress]);
        require(globalStaminaPoints >= _globalStamina, "Not enough stamina!");
        ...
        if(globalStaminaPoints >= MAXIMUM_GLOBAL_STAMINA) {
            globalStaminaTimestamp[_playerAddress] = uint64(now - getMaxGlobalStaminaCooldown() + drainTime);
        }
        else {
    ...
    function setGlobalStaminaByMigrator(address _playerAddress) external onlyMigratorContract {
        if (globalStaminaTimestamp[_playerAddress] == 0) {
            globalStaminaTimestamp[_playerAddress] = uint64(now.sub(getMaxGlobalStaminaCooldown()));
        }
    }
    ...
    function setTokenRewardsByMigrator(address _playerAddress, uint256 _amount, uint256 _timerStart) external onlyMigratorContract {
        ...
        if (_timerStart < _burRewardsClaimTaxTimerStart[_playerAddress]) {
...
contract BunicornsV2 is IBunicornsV2, Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    uint256 public constant ENHANCED_STAR_MULTIPLIER_BONUS = 15;

    uint256 public constant MAXIMUM_STAMINA = 20;
    uint256 public constant STAMINA_RECOVER_SPEED_IN_SECONDS = 3000; // 50 mins 1 STA

    uint256 public constant BUNICORN_STATS_BASELINE = 500; // It's 500, and the factor is in percentage -> 500 * 100 = 50000
    uint256 public constant ELEMENT_NEUTRAL_FACTOR_PERCENT = 105;
    uint256 public constant ELEMENT_UNMATCHED_FACTOR_PERCENT = 100;
    uint256 public constant ELEMENT_MATCHED_FACTOR_PERCENT = 110;
    ...
    function _get(uint256 id) internal view
        ...
        _bonusPower = getBonusPowerForBattle(id);
    }
    // =========================== GETTER, SETTER ===========================
    ...
    function getStars(uint256 id) public view noFreshLookup(id) returns (uint8) {
        return getStarsFromProps(getProperties(id));
    }

    function getStarsFromProps(uint16 _props) public pure returns (uint8) {
        return uint8(_props & 0x7); // first two bits for stars
    }

    function getElement(uint256 id) public view noFreshLookup(id) returns (uint8) {
        return getElementFromProps(getProperties(id));
    }

    function getElementFromProps(uint16 _props) public pure returns (uint8) {
    ...
    function getAttrPattern(uint256 id) public view noFreshLookup(id) returns (uint8) {
        return getAttrPatternFromProps(getProperties(id));
    }

    function getAttrPatternFromProps(uint16 _props) public pure returns (uint8) {
        return uint8((_props >> 5) & 0x7F); // 7 bits after star(3) and element(2) bits
    }

    function getAttr1Element(uint8 _attrPattern) public pure returns (uint8) {
        return uint8(uint256(_attrPattern) % 5); // 0-3 regular elements, 4 = elementless (NEUTRAL)
    }

    function getAttr2Element(uint8 _attrPattern) public pure returns (uint8) {
        return uint8(SafeMath.div(_attrPattern, 5) % 5); // 0-3 regular elements, 4 = elementless (NEUTRAL)
    }

    function getAttr3Element(uint8 _attrPattern) public pure returns (uint8) {
    ...
    function getPowerMultiplier(uint256 id) public view noFreshLookup(id) returns (int128) {
        ...
        int128 multiplierPerPoint = ABDKMath64x64.divu(1, BUNICORN_STATS_BASELINE);
        int128 attr1 = bunicorn.attr1.fromUInt().mul(multiplierPerPoint);
        int128 attr2 = bunicorn.attr2.fromUInt().mul(multiplierPerPoint);
        int128 attr3 = bunicorn.attr3.fromUInt().mul(multiplierPerPoint);

        return ABDKMath64x64.fromUInt(1).add(attr1).add(attr2).add(attr3);
    }
    ...
    function getPowerMultiplierForElement(
        ...
        uint8 attrPattern = getAttrPatternFromProps(_props);
        int128 unmatchedElementFactor = ABDKMath64x64.divu(ELEMENT_UNMATCHED_FACTOR_PERCENT, BUNICORN_STATS_BASELINE * 100);
        int128 neutralElementFactor = ABDKMath64x64.divu(ELEMENT_NEUTRAL_FACTOR_PERCENT, BUNICORN_STATS_BASELINE * 100);
        int128 matchedElementFactor = ABDKMath64x64.divu(ELEMENT_MATCHED_FACTOR_PERCENT, BUNICORN_STATS_BASELINE * 100);
        int128 result = ABDKMath64x64.fromUInt(bonusPower);

        if (getAttr1Element(attrPattern) == _element) {
            result = result.add(_attr1.fromUInt().mul(matchedElementFactor));
        }
        else if(getAttr1Element(attrPattern) == 4) { // NEUTRAL, elementless
            result = result.add(_attr1.fromUInt().mul(neutralElementFactor));
        }
        else {
            result = result.add(_attr1.fromUInt().mul(unmatchedElementFactor));
        }

        if (getAttr2Element(attrPattern) == _element) {
            result = result.add(_attr2.fromUInt().mul(matchedElementFactor));
        }
        else if(getAttr2Element(attrPattern) == 4) { // NEUTRAL, elementless
            result = result.add(_attr2.fromUInt().mul(neutralElementFactor));
        }
        else {
            result = result.add(_attr2.fromUInt().mul(unmatchedElementFactor));
        }

        if (getAttr3Element(attrPattern) == _element) {
            result = result.add(_attr3.fromUInt().mul(matchedElementFactor));
        }
        else if(getAttr3Element(attrPattern) == 4) { // NEUTRAL, elementless
            result = result.add(_attr3.fromUInt().mul(neutralElementFactor));
        }
        else {
            result = result.add(_attr3.fromUInt().mul(unmatchedElementFactor));
        }
    ...
    function _calculateEnhancedCounters(uint256 _burnID) private view returns(uint8[3] memory values, uint8 totalStars) {
        ...
        uint8 stars = getStarsFromProps(bunicorn.props);
        if(stars < 3) {
            values[0] += (stars + 1);
        }
        else if(stars == 3) {
    ...
    // =========================== BATTLE ===========================
    function getBonusPowerForBattle(uint256 id) public view noFreshLookup(id) returns (uint24) {
        EnhancementCounters storage counters = enhancementCounters[id];
        return uint24(
            uint256(counters.bronzeEnhanced + counters.sliverEnhanced * 7 + counters.goldEnhanced * 14)
                .mul(ENHANCED_STAR_MULTIPLIER_BONUS)
        );
    }

    function getBattleData(uint256 id, uint8 _trainerElement)
        public
        view
        noFreshLookup(id)
        returns (int128, uint24, uint8) {

        Bunicorn storage bunicorn = bunicorns[id];

        uint24 bonusPower = getBonusPowerForBattle(id);
        int128 powerMultiplierForElement = getPowerMultiplierForElement(bunicorn.props, bunicorn.attr1, bunicorn.attr2, bunicorn.attr3, _trainerElement, bonusPower);
        uint8 element = getElementFromProps(bunicorn.props);
        return (
            powerMultiplierForElement,
            bonusPower,
            element
        );
    }

    function getBattleDataAndDrainStamina(uint256 id, uint8 _trainerElement, uint8 _stamina)
        public
        onlyGameContract
        noFreshLookup(id)
        returns (int128, uint24, uint8) {
        ...
        return getBattleData(id, _trainerElement);
    }
    // =========================== BATTLE ===========================
