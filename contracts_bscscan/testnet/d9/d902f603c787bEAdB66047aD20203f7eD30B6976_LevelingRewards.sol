...
contract LevelingRewards is ILevelingRewards, StarlinkComponent {
    ...
    mapping(uint256 => Reward[]) public rewardsPerLevel;

    event LevelRewardClaimed(address user, uint256 id);
    event RewardAdded(uint256 level, uint128 typeId, uint128 amount);
    ...
    function addReward(uint256 level, uint128 typeId, uint128 amount) external onlyAdmins {
        Reward memory reward = Reward(rewardsPerLevel[level].length, typeId, amount, block.timestamp);
        rewardsPerLevel[level].push(reward);

        emit RewardAdded(level, typeId, amount);
    }

    function unclaimedRewards(address user, uint256 level) external override view returns(uint256) {
        uint256 rewardDate = rewardDatesPerLevel[user][level];
        Reward[] memory rewards = rewardsPerLevel[level];

        uint256 unclaimedCount = 0;
        for(uint i = 0; i < rewards.length; i++) {
            if (rewards[i].dateAdded > rewardDate) {
                unclaimedCount++;
            }
        }
    ...
    function doClaimRewards(address user, uint256 level) private {
        uint256 currentLevel = leveling.levelOf(user);
        require(level <= currentLevel, "LevelingRewards: Cannot claim yet");
    ...
    function doGiveReward(address user, Reward memory reward) private {
        if (reward.typeId == 0) {
            taxFreeTransfers.increaseTaxFreeTransferVouchers(user, uint8(reward.amount));
        } else if (reward.typeId == 1) {
            leveling.increaseNameChangeVouchers(user, uint8(reward.amount));
        } else if (reward.typeId == 2) {
            leveling.grantRestXp(user, uint8(reward.amount));
        }

        emit LevelRewardClaimed(user, reward.id);
    }
