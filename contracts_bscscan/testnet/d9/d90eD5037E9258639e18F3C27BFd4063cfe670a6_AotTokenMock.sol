...
contract AotTokenMock is SafeMath {
    ...
    uint256 public blocknumberLastAcceptMinter;
        ...
        returns (bool success)
    {
        /* Send coins */
        require(
            _to != address(0x0) && !blacklist[msg.sender],
            "AotTokenMock: zero address"
        ); // Prevent transfer to 0x0 address. Use burn() instead
        require(_value >= 0, "AotTokenMock: negative value");
        require(
    ...
    function transferFrom(
        ...
        require(
            _to != address(0x0) && !blacklist[_from],
            "AotTokenMock: zero address"
        ); // Prevent transfer to 0x0 address. Use burn() instead
        require(_value >= 0, "AotTokenMock: negative value");
        require(
    ...
    function setOwner(address payable _add) public {
        require(
            msg.sender == owner && _add != address(0x0),
            "AotTokenMock: zero address"
        );
        ownerTemp = _add;
