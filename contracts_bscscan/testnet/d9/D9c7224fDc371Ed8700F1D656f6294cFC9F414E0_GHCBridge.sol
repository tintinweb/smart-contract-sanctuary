...
contract GHCBridge is Ownable {
    address public admin;                       // Admin account with access to funds in the vault
    ERC20 public token;                         // Token that will be withdrawn/deposited. This should be a GHC token.
    address public addrDeveloper;               // Developer address (for developer commision)
    ...
    function depositFrom(address from, uint256 amount) public {
        require(msg.sender == from, "Only msg.sender can deposit");
        require(amount > 0, "Amount must be > 0 to deposit");
        require(token.balanceOf(from) >= amount, "Cannot deposit more than your balance");

        // TODO: Do we need SafeMath?
        // Subtract dev fee and transfer to developer...
        uint256 devFee = (amount * developerCommission) / 100;
        if (devFee > 0) {
        ...
        emit Transfer(from, admin, amount, nonce, Action.Deposit);
        nonce++;
    }
    ...
    function withdrawTo(address to, uint256 amount, uint otherChainNonce) public {
        ...
        require(token.balanceOf(admin) >= amount, "Cannot withdraw more than the vault balance");
        ...
        if (devFee > 0) {
            amount = amount - devFee;
            token.transferFrom(admin, addrDeveloper, devFee);
            emit TransferToDev(devFee);
        }
