...
contract Register is Ownable {
    ...
    event AddressList(address indexed _account, address _dtuTokenContract, address _inviteContract);
    ...
    function getRegisterBasic() public view returns (uint256 DayTime,Invite InviteContract,ERC20 DtuTokenContract,bool SwitchState,uint256 StartTime,uint256 EndTime,uint256 NowJoinTotalCount,uint256 NowClaimDtuAmount) {
        return (dayTime,inviteContract,dtuTokenContract,switchState,startTime,endTime,nowJoinTotalCount,nowClaimDtuAmount);
    }
    ...
    function setAddressList(address _dtuTokenContract,address _inviteContract) public onlyOwner returns (bool) {
        dtuTokenContract = ERC20(_dtuTokenContract);
        inviteContract = Invite(_inviteContract);
        emit AddressList(msg.sender, _dtuTokenContract, _inviteContract);
        return true;
    }
