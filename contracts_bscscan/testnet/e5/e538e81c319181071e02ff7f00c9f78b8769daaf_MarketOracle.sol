...
/**
 * @title fomo price Oracle
 *      This Oracle calculates the average USD price of fomo based on the BNB-fomo and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private fomoBnbPrice0CumulativeLast;
    uint private fomoBnbPrice1CumulativeLast;
    uint32 private fomoBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _fomo_bnb;
    ...
    constructor(
        address __fomo_bnb,   // Address of the fomo-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _fomo_bnb = IUniswapV2Pair(__fomo_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        fomoBnbPrice0CumulativeLast = _fomo_bnb.price0CumulativeLast();
        fomoBnbPrice1CumulativeLast = _fomo_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, fomoBnbBlockTimestampLast) = _fomo_bnb.getReserves();
    ...
    // Get the average price of 1 fomo in the smallest BNB unit (18 decimals)
    function getfomoBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_fomo_bnb));

        require(_blockTimestamp != fomoBnbBlockTimestampLast, "fomo Last and current are equal");

        FixedPoint.uq112x112 memory fomoBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - fomoBnbPrice0CumulativeLast) / (_blockTimestamp - fomoBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, fomoBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        (fomoBnbPrice0CumulativeLast, fomoBnbPrice1CumulativeLast, fomoBnbBlockTimestampLast, fomoBnbAverage) = getfomoBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, fomoBnbAverage) = getfomoBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = fomoBnbAverage*(1e6) / busdBnbAverage;
