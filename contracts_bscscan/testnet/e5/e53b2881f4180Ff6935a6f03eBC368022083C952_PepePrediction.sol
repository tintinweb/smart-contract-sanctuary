...
contract PepePrediction is OwnableUpgradeable,  PausableUpgradeable {
    ...
    function betBull() external payable whenNotPaused notContract {
        emit Logi(1,currentEpoch);
        uint256 _currentEpoch = currentEpoch;
        if (_automatedLockRoundWithBetInBufferBlock(currentEpoch)) {
            _currentEpoch = _currentEpoch + 1;
        }
        emit Logi(3,_currentEpoch);
        require(_bettable(_currentEpoch), "Round not bettable");
        require(msg.value >= minBetAmount, "Bet amount must be greater than minBetAmount");
        require(ledger[_currentEpoch][msg.sender].amount == 0, "Can only bet once per round");

        // Update round data
        uint256 amount = msg.value;
        Round storage round = rounds[_currentEpoch];
        round.totalAmount = round.totalAmount.add(amount);
        round.bullAmount = round.bullAmount.add(amount);

        // Update user data
        BetInfo storage betInfo = ledger[_currentEpoch][msg.sender];
        betInfo.position = Position.Bull;
        betInfo.amount = amount;
        userRounds[msg.sender].push(_currentEpoch);

        emit BetBull(msg.sender, _currentEpoch, amount);
    }
