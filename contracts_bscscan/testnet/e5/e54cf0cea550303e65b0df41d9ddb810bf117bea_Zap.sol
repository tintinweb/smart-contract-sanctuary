...
contract Zap is IZap, OwnableUpgradeable {
    ...
    */
     ...
     */
    function initialize(address _router, address _wbnb, address ali, address usdt, address busd, address eth, address btcb , address baby, address raca, address beta) public initializer {
        __Ownable_init();
    ...
    function zapInToken(address fromToken, uint amountIn, address toToken, uint amountOutMin) external override notPaused {
        uint fromTokenBalanceBefore = IBEP20(fromToken).balanceOf(address(this));

        IBEP20(fromToken).safeTransferFrom(msg.sender, address(this), amountIn);
        ...
        if (isLpToken(toToken)) {
            ...
            if (fromToken == token0 || fromToken == token1) {
                ...
                uint bnbAmount = fromToken == wbnb? _safeSwapToBNB(amountIn) : _swapTokenForBNB(fromToken, amountIn, address(this));
    ...
    function zapIn(address toToken, uint amountOutMin) external payable override notPaused {
        uint bnbBalanceBefore = address(this).balance.sub(msg.value);
        uint amountOut = _swapBNBToLpToken(toToken, msg.value, msg.sender, bnbBalanceBefore);
    ...
    function zapOut(address fromToken, uint amountIn, uint amountOutMinA, uint amountOutMinB) external override notPaused {
        IBEP20(fromToken).safeTransferFrom(msg.sender, address(this), amountIn);
        _approveTokenIfNeeded(fromToken);

        if (!isLpToken(fromToken)) {
            uint amountOut = _swapTokenForBNB(fromToken, amountIn, msg.sender);
            ...
            if (token0 == wbnb || token1 == wbnb) {
                ...
                (uint amountToken, uint amountETH) = ROUTER.removeLiquidityETH(token, amountIn, amountOutMinA, amountOutMinB, msg.sender, block.timestamp);
                emit ZapOutBNB(fromToken, token, amountIn, amountToken, amountETH);
            } else {
                (uint amountA, uint amountB) =  ROUTER.removeLiquidity(token0, token1, amountIn, amountOutMinA, amountOutMinB, msg.sender, block.timestamp);
...
interface IZap {
    function zapOut(address fromToken, uint amountIn, uint amountOutMinA, uint amountOutMinB) external;
    function zapIn(address toToken, uint amountOutMin) external payable;
    function zapInToken(address fromToken, uint amountIn, address toToken, uint amountOutMin) external;
