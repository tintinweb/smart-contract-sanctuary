...
contract Balloons{
    ...
    function changeTaxRate(uint256 newRate) public returns(bool success){
        assert(msg.sender == admin && newRate < 5000);
        taxRate = newRate;
        return true;
    }
    ...
    function withdraw(uint amount, bytes32 user, address userAddress, uint newBalance) public returns(bool success){
        ...
        //send the rest amount to player
        payable(userAddress).transfer(amount.sub(taxAmount));
        return true;
    }
}