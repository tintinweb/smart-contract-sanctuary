...
import "./CointapLaunchpad.sol";
import "./interfaces/ICointapLaunchpadFactory.sol";
import "./constants/SharedConstants.sol";

contract CoinTapLaunchpadFactory is
    ICoinTapLaunchpadFactory,
    Ownable
{
    ...
    /* events */
    event NewCoinTapLaunchpad(address indexed launchpadAddr);
    ...
    function createLaunchpad(
        ...
        CoinTapLaunchpad launchpad = new CoinTapLaunchpad(
            _launchpadData,
            _vestingTimeline,
            _information,
            _ownerAddr
        ...
        // Transfer token to launchpad
        _transferTokenSupply(_launchpadData.token, _launchpadData.presaleRate, _launchpadData.hardCap, address(launchpad));

        emit NewCoinTapLaunchpad(address(launchpad));

        return address(launchpad);
    }

    /******************************************* Internal function below *******************************************/
    function _transferTokenSupply(address _token, uint256 _presaleRate, uint256 _hardCap, address _launchpadAddr) internal {
      uint256 tokenAmount =  _presaleRate.mul(_hardCap);

      IERC20 token = IERC20(_token);
      token.safeTransferFrom(_msgSender(), _launchpadAddr, tokenAmount);
    }
}
...
interface ICoinTapLaunchpadFactory {
    function createLaunchpad(
        SharedConstants.LaunchpadData memory _launchpadData,
        SharedConstants.VestingTimeline memory _vestingTimeline,
        SharedConstants.Information memory _information,
...
contract CoinTapLaunchpad is ICoinTapLaunchpad, Ownable, Pausable, Whitelist {
    ...
    function buyToken() public override payable {
      ...
      require(status != SharedConstants.Statuses.CANCELLED || status != SharedConstants.Statuses.FINZALIZE, "CoinTapLaunchpad: Launchpad is cancelled or finalized");
    ...
    function claim() public override {
      ...
      uint256 currentOrder = vestingHistories[buyer].length;
      uint256 nextOrder = currentOrder + 1;
      ...
      IERC20 token = IERC20(launchpadData.token);
      uint256 totalAmount = amountBNB.mul(launchpadData.presaleRate);
      uint256 amount = totalAmount.mul(percent).div(100);
      token.transfer(buyer, amount);

      // set participant claimed
      vestingHistories[buyer][nextOrder] = true;
    }
