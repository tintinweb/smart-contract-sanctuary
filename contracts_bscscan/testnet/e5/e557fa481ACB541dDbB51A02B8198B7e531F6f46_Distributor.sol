...
// This Contract Allows Users to Stake PROFIT tokens and receive ETH as reward for staking PROFIT.
//
// Note that it's ownable and only the owner has access to some restricted functions.
//  The ownership will be transferred to a governance smart contract once
// the community can show to govern itself.
//
...
contract Distributor is Ownable {
    ...
    // WETH tokens unlocked per block.
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 WETHReward =
                WETHPerBlock.mul(pool.allocPoint).div(
                    totalAllocPoint
                );
    ...
    * The Pool to be updated is chosen by specifying the ID of the pool (_pid).
    **/
    function updatePool(uint256 _pid) public {
        ...
        WETH.safeTransfer(devaddr, WETHReward.div(10));
        
        WETH.safeTransfer(address(this), WETHReward);
    ...
    // Deposit PROFIT tokens to Distributor to get rewarded with ETH.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accWETHPerShare).div(1e12).sub(
                    user.rewardDebt
                );
    ...
    // Withdraw PROFIT tokens from Distributor.
