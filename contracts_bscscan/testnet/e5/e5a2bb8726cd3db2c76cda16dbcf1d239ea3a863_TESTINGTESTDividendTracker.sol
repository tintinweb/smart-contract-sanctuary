...
contract TESTINGTEST is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    TESTINGTESTDividendTracker public dividendTracker;
    ...
    // sell fees
    uint256 public sellDividendRewardsFee = 2;
    ...
    constructor() ERC20("TESTINGTEST", "TEST") {
        dividendTracker = new TESTINGTESTDividendTracker();

        teamWallet = 0x169c329026afc6E7f896c6d98857977106c23981;
        devWallet = 0xB1D5Ed758F3fa0efE370Cc9252Bd54980FB2e485;
        marketingWallet = 0xc1625FD052aF443d2313341Dd81de1049Ded82Df;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "TEST: The dividend tracker already has that address"
        );

        TESTINGTESTDividendTracker newDividendTracker = TESTINGTESTDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "TEST: The new dividend tracker must be owned by the TEST token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "TEST: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "TEST: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "TEST: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "TEST: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "TEST: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[to]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 bnbSwap = buyDevFee.add(buyTeamFee).add(
                    buyMarketingFee
                );
    ...
    function swapAndSendBnb(uint256 tokenAmount, uint256 totalFee) private {
        ...
        transferToWallet(
            payable(devWallet),
            address(this).balance.div(totalFee).mul(buyDevFee)
        );
        transferToWallet(
...
contract TESTINGTESTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("TEST_Dividend_Tracker", "TEST_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "TEST_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TEST contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "TEST_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "TEST_Dividend_Tracker: Cannot update claimWait to same value"
        );
