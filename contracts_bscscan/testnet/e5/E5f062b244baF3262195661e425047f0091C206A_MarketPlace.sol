...
import "./interfaces/IFormulaTransporter.sol";
...
contract MarketPlace is MarketAuth, PausableUpgradeable {
  ...
  IFormulaTransporter public formulaTransporter;
  ...
  function initialize(
    ...
    formulaTransporter = IFormulaTransporter(_formulaTransporter);
  ...
  function makeOrder(
    ...
    if (_kind == IMarketPlace.OrderKind.hero) {
      _validateHero(_kind, _tokenId, _startPrice, _endingPrice);
    } else if (_kind == IMarketPlace.OrderKind.item) {
      _validateItem(_kind, _tokenId, _startPrice, _endingPrice);
    }
    if (_startPrice != _endingPrice) {
      require(_auctionDuration >= 7 days && _auctionDuration <= 365 days, "duration is invalid");
    }
  ...
  function takeOrder(IMarketPlace.OrderKind _kind, uint _tokenId, IMarketPlace.PaymentCurrency _paymentCurrency) external whenNotPaused {
    ...
    if (currentPrice > 0) {
      uint sharingAmount = currentPrice * (referralShare + creativeShare + treasuryShare) / 100000;
  ...
  function getCurrentPrice(IMarketPlace.OrderKind _kind, IMarketPlace.PaymentCurrency _paymentCurrency, uint _tokenId) external view returns(uint) {
    Order storage tradingOrder = tradingOrders[_kind][_tokenId];
    if (_isActive(tradingOrder)) {
  ...
  function getOrder(IMarketPlace.OrderType _type, IMarketPlace.OrderKind _kind, uint _tokenId) external view returns(address, uint, uint, uint, uint, bool) {
    Order storage order = _type == IMarketPlace.OrderType.trading ? tradingOrders[_kind][_tokenId] : rentingOrders[_kind][_tokenId];
    return (
      order.maker,
      order.startingPrice,
      order.endingPrice,
  ...
  function getItemToken() public view returns (IGameNFT) {
    return nftTokens[IMarketPlace.OrderKind.item];
  }
  ...
  function _shareOrderValue(IMarketPlace.OrderKind _kind, uint _tokenId, address _maker, uint _totalShare, IMarketPlace.PaymentCurrency _paymentCurrency) private {
  ...
  function _marketMakeOrder(
    ...
    emit OrderCreated(IMarketPlace.OrderType.trading, _kind, _tokenId, address(this), price, price, duration, duration);
  }
  ...
  function _initItemPrices() private {
    ...
    for(uint i = 0; i < classes.length; i++) {
      ...
      itemMinPrices[classes[i]] = 100e18;
    }
  }
  ...
  function _initHeroPrices() private {
    ...
    for(uint i = 0; i < classes.length; i++) {
      ...
      heroMinPrices[uint16(classes[i])] = 100e18;
    }
  }
  ...
  function _validateHero(IMarketPlace.OrderKind _kind, uint _tokenId, uint _startPrice, uint _endingPrice) private view {
  ...
  function _validateItem(IMarketPlace.OrderKind _kind, uint _tokenId, uint _startPrice, uint _endingPrice) private view {
    (uint gene, uint16 _class,,,,) = nftTokens[_kind].getItem(_tokenId);
    if (formulaTransporter.openFormulaItem()) {
  ...
  function listHeroes(
    ...
    require(heroPrices[_classId] > 0);
    for(uint i = 0; i < _quantity; i++) {
      uint _tokenId = nftTokens[IMarketPlace.OrderKind.hero].mintHero(address(this), _classId, heroPrices[_classId], i);
      _marketMakeOrder(IMarketPlace.OrderKind.hero, _tokenId);
    }
  }
  ...
  function listItems(
    ...
    for(uint i = 0; i < _quantity; i++) {
      require(_class >= 100);
      require(itemPrices[_class] > 0);
      uint8 gene = uint8(_class / 100);
      uint _tokenId = nftTokens[IMarketPlace.OrderKind.item].mintItem(address(this), gene, _class, itemPrices[_class], i);
      _marketMakeOrder(IMarketPlace.OrderKind.item, _tokenId);
    }
  }
  ...
  function burn(IMarketPlace.OrderKind _kind, uint _tokenId) external onlyMainAdmin {
    require(nftTokens[_kind].ownerOf(_tokenId) == address(this), "401");
    Order storage tradingOrder = tradingOrders[_kind][_tokenId];
    Order storage rentingOrder = rentingOrders[_kind][_tokenId];
    if (_isActive(tradingOrder)) {
      _removeTradingOrder(_kind, _tokenId);
  ...
  function setFormulaTransporter(address _formulaTransporter) external onlyMainAdmin {
    formulaTransporter = IFormulaTransporter(_formulaTransporter);
  }
...
interface IFormulaTransporter {
  function openFormulaItem() external view returns (bool);
}
