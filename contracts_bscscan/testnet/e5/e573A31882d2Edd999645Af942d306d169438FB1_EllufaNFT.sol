...
contract EllufaNFT is NFTokenMetadata, Ownable {
    ...
    {
        requireBalance = currentPrice.mul(totalMinted).div(
            this.getLatestPrice()
        );

        requireBalance = requireBalance.mul(bnb_multiplier);

        currentBalance = address(this).balance;

        if (currentBalance >= requireBalance) status = true;
        else status = false;
    }
    ...
    function sellAll() external payable {
        ...
        (uint256 requireBalance, uint256 currentBalance, bool status) = this
            .checkSellAllBalance();

        require(status == true, "Didnt Have Enough BNB");
        ...
        companyaddress.transfer(serviceamount);

        uint256 newreceive = requireBalance.sub(serviceamount);

        address payable receiver_user = payable(msg.sender);

        receiver_user.transfer(newreceive);

        contractStatus = false;

        uint256 moneydiff = currentBalance.sub(requireBalance);

        companyaddress.transfer(moneydiff);
    }
}