...
contract CCL is Context, Ownable, ReentrancyGuard {
 ...
 //  uint256 private BNBCost; // Fees in BNB
    ...
    function _isRequestedAddress(address _addr) external view LTSHOLDER() returns (bool) { return Requested[_addr]; } // IF THIS ADDRESS HAS AN ONGOING REQUEST 
    function _isVerfiedAddress(address _addr) external view LTSHOLDER() returns (bool) { return isVerified[_addr]; } // IF THIS ADDRESS IS VERIFIED BY THE CCL 
    ...
    function payInBNB() private {
        ...
        function payInLTS() private {
        require(LTSToken.balanceOf(_msgSender()) >= LTSCost * (10 ** 18), "Insufficient LTS Balance");
            TransferHelper.safeTransfer(address(LTSToken), address(this), LTSCost * (10 ** 18));
    }
        function payInBNB() private {
        require(BNBToken.balanceOf(_msgSender()) >= BNBCost * (10 ** 18), "Insufficient BNB Balance");
            TransferHelper.safeTransfer(address(BNBToken), address(this), BNBCost * (10 ** 18));
    } 
