...
contract MOVXToken is Context, IERC20, Ownable {
    ...
    uint256 private _otherMaxTxAmount   = 5000000000 * 10**_decimals;
    uint256 private _buyTaxFee          = 1; // 1% reflection fee for every holder
    uint256 public _buyLiquidityFee     = 2; // 2% Fee to LP on buy
    uint256 public _buyMarketingFee     = 4; // 4% Fee to Marketing wallet on buy
    uint256 private _buyResearchFee     = 4; // 4% Fee to Research wallet on buy
    uint256 private _sellTaxFee         = 2;  // 2% reflection fee for every holder
    uint256 public _sellLiquidityFee    = 3;  // 4% Fee to LP on sell
    uint256 public _sellMarketingFee    = 10; // 10% Fee to Marketing wallet on sell
    uint256 private _sellResearchFee    = 10; // 10% Fee to Research wallet on sell
    uint256 private _otherTaxFee        = 1;
	uint256 private _otherLiquidityFee  = 2;
    uint256 private _otherMarketingFee  = 4;
    uint256 private _otherResearchFee   = 4;
    ...
    address payable private _researchWalletAddress;
    ...
    constructor (address payable MarketingWallet, address payable ResearchWallet, address DeadAddress, address UniswapV2Router) {
        
        _marketingWalletAddress = MarketingWallet;
        _researchWalletAddress = ResearchWallet;
        ...
        _isExcludedFromFee[_researchWalletAddress] = true;
		...
		_isExcludedFromMax[deadAddress] = true;
		_isExcludedFromMax[_marketingWalletAddress] = true;
		_isExcludedFromMax[_researchWalletAddress] = true;
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _marketingFee == 0 && _researchFee == 0 && _liquidityFee == 0) return;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        require(!_isLockedWallet[sender], "Locked addresses cannot call this function");

        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");

        if(sender != owner() && recipient != owner() && !_isExcludedFromMax[sender] && !_isExcludedFromMax[recipient]) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 ethForMarketing = balanceDiff.mul(_marketingFee).div(_liquidityFee.add(_researchFee));
        uint256 ethForResearch = balanceDiff.mul(_researchFee).div(_liquidityFee.add(_marketingFee));
        uint256 ethForLiquidity = balanceDiff.sub(ethForMarketing.add(ethForResearch));
        ...
        _researchWalletAddress.transfer(ethForResearch);
    }
    ...
    function sendBNBToResearch(uint256 amount) private {
        _researchWalletAddress.transfer(amount);
    }
    ...
    function manualSend() public onlyOwner() {
        uint256 marketingETHBalance = address(this).balance;
        uint256 researchETHBalance = address(this).balance;
        sendBNBToMarketing(marketingETHBalance);
        sendBNBToResearch(researchETHBalance);
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tMarketingLiquidityFee) = _getTValues(tAmount, _taxFee, _liquidityFee.add(_marketingFee).add(_researchFee));
        uint256 currentRate = _getRate();
    ...
    function _setMaxTxAmount(uint256 buyTxValue, uint256 sellTxValue, uint256 otherTxValue) external onlyOwner() {
	...
	function setBuyFee(uint256 buyTaxFee, uint256 buyLiqFee, uint256 buyMarketingFee, uint256 buyResearchFee) external onlyOwner() {
    ...
    function setSellFee(uint256 sellTaxFee, uint256 sellLiqFee, uint256 sellMarketingFee, uint256 sellResearchFee) external onlyOwner() {
	...
	function setOtherOtherFee(uint256 otherTaxFees, uint256 otherLiqFee, uint256 otherMarketingFee, uint256 otherResearchFee) external onlyOwner() {
        _otherTaxFee = otherTaxFees;
