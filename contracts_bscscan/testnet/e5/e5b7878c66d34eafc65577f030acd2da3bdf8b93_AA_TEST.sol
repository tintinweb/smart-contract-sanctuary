...
contract AA_TEST is Context, IERC20, Ownable { 
    ...
    uint256 private swapTrigger = 4;               // Number of transactions (with fees) before processing XXX


    // FEES - Initial Settings
  
    uint256 public _FeeBuy_Total        = 10;       // Total Fee on Buys - Includes Reflection
    uint256 public _FeeSell_Total       = 20;       // Total Fee on Sells - Includes Reflection

    uint256 public _FeeBuy_Reflection   = 4;        // RFI to holders on Buys
    uint256 public _FeeSell_Reflection  = 4;        // RFI to holders on Sells

    // Fee Splits, Total Must = 100

    uint256 public _Split_BNB           = 25;       // Percent of fee that goes to external BNB wallet
    ...
    uint256 public _Split_Burn          = 25;        // Percent of fee sent to deflationary burn
...
//    uint256 public _min_Swap = _tTotal / 10000; // 0.01% of total supply XXXX
    ...
    */
    ...
    function _set_Fees(
                        ...
                        require(Buy_Total >= Buy_Reflection, "Total Fee Must Inlcude Reflection Fee");
                        require(Sell_Total >= Sell_Reflection, "Total Fee Must Inlcude Reflection Fee");
    ...
    */
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (_Split_Tokens != 0){
            ...
            uint256 tokens_to_Wallet = contractTokenBalance * _Split_Tokens / 100;
        ...
        if (_Split_Burn != 0){
            ...
            uint256 tokens_to_Burn = contractTokenBalance * _Split_Burn / 100;
   ...
   function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        tFees       = tAmount * (_FeeSell_Total - _FeeSell_Reflection) / 100;
        txCount++;

        } else if (_isPair[sender]){
       
        tReflect    = tAmount * _FeeBuy_Reflection / 100;
        tFees       = tAmount * (_FeeBuy_Total - _FeeBuy_Reflection) / 100;
        txCount++;
