...
interface IPokMonsterStatsStatic {
    struct PokBody {
        uint8 id;
        string Name;
        string Type_1;
    ...
    function getEvolutionIndex(uint256 tokenId)
        external
        view
        returns (uint256);
    ...
    function setBody(PokBody memory newBody) external;
}
...
interface IPokMonsterFactory {
    ...
    function mintToken(
        address receiver,
        uint256 tokenId,
        IPokMonsterStatsStatic.PokBody memory body
    ) external;

    function burnToken(uint256 tokenId) external;
}

contract PokMonsterStaticStats is IPokMonsterStatsStatic, OwnableUpgradeable {
    mapping(address => bool) private _admin;

    mapping(uint256 => PokBody) private body_;

    mapping(uint256 => uint256) private evolutionIndex_;
    ...
    function getBody(uint256 index)
        external
        view
        override
        returns (PokBody memory body)
    ...
    function getEvolutionIndex(uint256 index)
        external
        view
        override
        returns (uint256)
    {
        return evolutionIndex_[index];
    }

    /* ========== MUTATIVE FUNCTIONS ========== */

    function setBody(PokBody memory newBody) public override onlyAdmin {
        delete body_[newBody.id];
        body_[newBody.id] = newBody;
    }

    function setEvolutionIndex(uint256 index, uint256 _evolutionIndex)
        public
        onlyAdmin
    {
        evolutionIndex_[index] = _evolutionIndex;
    }
