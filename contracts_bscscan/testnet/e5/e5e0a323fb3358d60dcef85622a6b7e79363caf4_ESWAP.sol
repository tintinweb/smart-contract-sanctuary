...
contract ESWAP is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1350000000 *  10**18;
    ...
    string private _name = "ExoSwap Token";
    string private _symbol = "EXOSWAP";
    ...
    uint256 public _maxTxAmount = 1350000000 *  10**18;
	...
	function _getburnwallet() public view virtual returns (address) {
        return _burnwallet;
	}
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
		...
		uint256 tCharity = calculateburnfee(tAmount);
	...
	function _takeCharity(uint256 tCharity) private {
        ...
        if (_shouldSwapToBNB) {
            ...
            _burnwallet.transfer(address(this).balance);
        } else {
            _rOwned[_burnwallet] = _rOwned[_burnwallet].add(rCharity);
            if(_isExcluded[_burnwallet])
                _tOwned[_burnwallet] = _tOwned[_burnwallet].add(tCharity);
        }
    }
