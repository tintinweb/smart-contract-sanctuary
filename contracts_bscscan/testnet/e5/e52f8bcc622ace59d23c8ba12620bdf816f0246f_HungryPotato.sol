...
contract HungryPotato is ERC721Metadata {
    ...
    modifier periodStarted() {
        require(block.timestamp >= startTime && startTime != 0, "Hungry Potato: Period not started");
        _;
    }
    ...
    constructor(
        string memory baseURI_,
        address admin_
    ) ERC721Metadata("Hungry Potato", "HUPO", baseURI_) {
        _admin = admin_;
    }

    function setStartTime(uint256 _startTime) external onlyOwner {
        require(_startTime > 0, "Hungry Potato: invalid _startTime");
        require(_startTime > block.timestamp, "Hungry Potato: old start time");
    ...
    function purchase(uint256 numberOfNfts) external payable periodStarted {
        require(pendingCount > 0, "Hungry Potato: All minted");
        require(numberOfNfts > 0, "Hungry Potato: numberOfNfts cannot be 0");
        require(numberOfNfts <= 20, "Hungry Potato: You may not buy more than 20 NFTs at once");
        require(totalSupply() + (numberOfNfts) <= MAX_NFT_SUPPLY,"Hungry Potato: sale already ended");
        if(tax_freeUsers[msg.sender] == 0) require(MINT_PRICE * (numberOfNfts) == msg.value, "Hungry Potato: invalid ether value");
        else if (tax_freeUsers[msg.sender] < numberOfNfts){
            require(MINT_PRICE * (numberOfNfts -  tax_freeUsers[msg.sender]) == msg.value, "Hungry Potato: invalid ether value");
    ...
    function claimReward(uint256 tokenId) public {
        require(
            ...
            "Hungry Potato: Only owner or approved can claim rewards"
        );
    ...
    function _splitBalance(uint256 amount) internal {
        ...
        uint256 adminShare = ((amount - reflectionShare) * 60) / 100;
    ...
    function _randomMint(address _to) internal returns (uint256) {
        require(totalSupply() < MAX_NFT_SUPPLY, "Hungry Potato: max supply reached");
