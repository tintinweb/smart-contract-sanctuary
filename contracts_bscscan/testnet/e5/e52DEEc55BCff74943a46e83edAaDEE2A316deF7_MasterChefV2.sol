...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accGovTokenPerShare;   // Accumulated EGGs per share, times 1e12. See below.
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 govTokenReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accGovTokenPerShare = accGovTokenPerShare.add(govTokenReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 govTokenReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        govToken.mint(devaddr, govTokenReward.div(10));
        govToken.mint(address(this), govTokenReward);
        pool.accGovTokenPerShare = pool.accGovTokenPerShare.add(govTokenReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function safeGovTokenTransfer(address _to, uint256 _amount) internal {
        ...
        require(transferSuccess, "safeGovTokenTransfer: transfer failed");
    }
