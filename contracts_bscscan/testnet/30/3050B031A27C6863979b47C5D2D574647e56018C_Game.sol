...
contract Game is Ownable, ERC721Token("Game" ,"GAME"), ERC721Holder {
    ...
    function mintCharacter(uint256 _amount, string _ipfsHash, string _name, string _description) public {
        ...
        require(addressPaymentToken.transfer(address(this), _amount), "Transfer for mint error.");
    ...
    function addPower(uint256 _tokenId) public onlyOwnerOf(_tokenId) returns (bool){
        ...
        if(powerC == 200){
            require(addressPaymentToken.transfer(address(this), levelChar1), "Transfer error.");
            characters[_tokenId].power = 300;
            return true;
        }

        if(powerC == 300){
            require(addressPaymentToken.transfer(address(this), levelChar2), "Transfer error.");
            characters[_tokenId].power = 400;
            return true;
        }

        if(powerC == 400){
            require(addressPaymentToken.transfer(address(this), levelChar3), "Transfer error.");
            characters[_tokenId].power = 500;
            return true;
        }

        if(powerC == 500){
            require(addressPaymentToken.transfer(address(this), levelChar4), "Transfer error.");
            characters[_tokenId].power = 600;
            return true;
        }

        if(powerC == 600){
            require(addressPaymentToken.transfer(address(this), levelChar5), "Transfer error.");
            characters[_tokenId].power = 700;
            return true;
        }

        if(powerC == 700){
            require(addressPaymentToken.transfer(address(this), levelChar6), "Transfer error.");
            characters[_tokenId].power = 800;
            return true;
        }

        if(powerC== 800){
            require(addressPaymentToken.transfer(address(this), levelChar7), "Transfer error.");
            characters[_tokenId].power = 900;
            return true;
        }

        if(powerC == 900){
            require(addressPaymentToken.transfer(address(this), levelChar8), "Transfer error.");
