...
contract LgtbiPreSale is Ownable {
...
constructor(
  ...
  function _get_Balance() private returns(uint balance) {
    ...
    function buyTokens(bytes calldata _agreementSignature,string memory _UserSignature,  uint256 amount)  public view  returns(bool) 
    {
      bytes32 message = prefixed(keccak256(abi.encodePacked(
        msg.sender,
        amount,
