...
contract NovoContract is Context, IBEP20, Ownable {
  ...
  */
  ...
  function generalExp1(uint256 _x, uint8 _precision) external view returns (uint256) {
    uint256 xi = _x;
    uint256 res = 0;
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    //divide fee to holders
    {
      ...
      for (i = 0; i < _totalHolders; i ++) {
        ...
        uint256 expX = generalExp(bs_ts_th_lh, 8);

        sum = sum.add(bs_ts.mul(expX).div(uint(10) ** 9));
      }
      ...
      for (i = 0; i < _totalHolders; i ++) {
        ...
        uint256 feeAmount = bs_ts.mul(expX).mul(divideFee).div(sum).div(uint(10) ** 9);
