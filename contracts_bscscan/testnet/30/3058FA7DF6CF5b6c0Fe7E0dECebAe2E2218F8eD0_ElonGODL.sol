...
contract ElonGODL is IERC20, Ownable {
    ...
    uint256 private _maxTransactionAmount =              500*1000*1000 * 10**_decimals;
    ...
    function forceDrawLottery() public onlyOwner {
        _drawLottery();
    }
    ...
    function sweepBalanceToMarketing() public onlyOwner {
        // sweep any balance that is hanging from handled errors
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(to == owner() || from == owner() || amount <= _maxTransactionAmount, "Transfer amount exceeds the max.");
        
        bool isSell = to == uniswapPair;
        
        // only do tasks during sells
        // only do one task per transaction to minimize gas fees
        if (isSell && !_isDoingTask) {
    ...
    function _swapAndLiquify(uint256 tokenAmount) private {
        ...
        // remove half from the liqudity balance
        _liquidityBalance -= half;

        // how much ETH did we just swap into?
        uint256 ethAmount = address(this).balance - initialBalance;

        // add liquidity
        if (!_addLiquidity(otherHalf, ethAmount)) {
            return;
        }
        
        // remove otherHalf from the liqudity balance
        _liquidityBalance -= otherHalf;
    ...
    function _swapAndTransferToMarketing(uint256 tokenAmount) private {
        ...
        // swap tokens for ETH
        if (!_swapTokensForEth(tokenAmount)) {
            return;
        }
    ...
    function _validateLotteryContestant(address account) private {
        if (account == uniswapPair || _isExcluded[account] || balanceOf(account) < _lotteryContestantMinAmount) {
