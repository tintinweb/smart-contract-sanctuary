...
contract NFTMarketplace is ReentrancyGuard, Ownable, ERC1155Holder {
  ...
  function createMarketItem(
    ...
    uint256 listingFee = calculateListingFee(amount, price);
    require(price.mul(amount) >= listingFee, "Price must => listing fee");
  ...
  function buyMarketItem(
    ...
    _currency.transferFrom(msg.sender, owner(), calculateListingFee(amount, price));
  ...
  function calculateListingFee(uint256 amount, uint256 price)
    public
    view
    returns (uint256 listingFee)
  {
    return price.mul(amount).mul(_listingFee).div(FEE_DENOMINATOR);
  }
