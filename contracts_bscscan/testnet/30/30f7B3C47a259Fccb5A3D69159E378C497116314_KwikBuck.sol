...
contract KwikBuck is Context, IBEP20, Ownable , ReentrancyGuard {
    ...
    constructor (address payable _charityWallet, address payable _devWallet)  {
    ...
    function setPancakeRouter(IPancakeV2Router02 _pancakeRouter) external onlyOwner {
        pancakeRouter = _pancakeRouter;
    }
    ...
    function _transfer(
        ...
        require(balanceOf(from) >= amount.add(fee),"BEP20: Insufficient balance");
        ...
        if(from != owner()){
            ...
            if (block.timestamp < _launchTime + 120 seconds) {
                if ((from == pancakePair || to == pancakePair)&& from != address(pancakeRouter)) {
                    _isSniper[from] = true;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,uint256 fee) private {
