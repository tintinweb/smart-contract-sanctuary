...
contract arbbot {
    ...
    function swap(
        ...
        TransferHelper.safeTransferFrom(
            WBNB, msg.sender, pairs[0], amountIn
        );
        uint256 balanceBefore = IERC20(WBNB).balanceOf(to);
        //
         _swapSupportingFeeOnTransferTokens(WBNB,pairs, to);
        require(
            IERC20(WBNB).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    function _swapSupportingFeeOnTransferTokens(address input, address[] memory pairs, address _to) internal virtual {
        for (uint i; i < pairs.length - 1; i++) {
            ...
            address to = i < pairs.length - 2 ? pairs[i+1] : _to;
