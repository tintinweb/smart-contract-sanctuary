...
contract MATAToken is ERC20, Ownable {
    ...
    function lastClaim(address _addr) external view returns (uint256) {
        return lastClaims[_addr];
    }
    ...
    function claimMATA(
        ...
        // require(_nonce <= _now && _now <= _nonce + 1 hours, "Claim request expired");
        // require(_now >= lastClaims[_user] + 1 days, "The gap between 2 claims is 1 day"); // TODO
        bytes32 _hash = keccak256(abi.encodePacked(_user, _amount, _nonce));
        ...
        _mint(_user, _amountInt);
        lastClaims[_user] = _now;
        emit Claim(_user, _amountInt);
    }
