...
contract TSC is Context, IBEP20, Ownable {
    ...
    function transferFrom(
        ...
        _registerAddress(recipient);
        // affiliate
        _grantAffiliateReward(transactionPoolAddress, amount);
        return true;
    }
    ...
    /**
     * @dev grant affiliate reward.
     */
    function _grantAffiliateReward(address child, uint256 amount)
        private
        returns (bool)
    {
        if (child != address(0) && child != transactionPoolAddress)
            return false;

        address _parent = _affiliateParent[child];
        if (_parent == address(0x0)) return false;

        uint256 _rewardAmount = (amount.mul(_affiliateRewardPercent)).div(100);
        if (_rewardAmount == 0) _rewardAmount = _affiliateRewardMinAmount;

        _transfer(transactionPoolAddress, _parent, _rewardAmount);
    ...
    function burnAndDrop() external onlyOwner returns (bool) {
        ...
        _burn(transactionPoolAddress, _poolBalance.div(2));
        ...
        for (uint256 index = 0; index < _holderAddressess.length; index++) {
            ...
            if (_currentBalance != 0) {
                _transfer(
                    ...
                    _poolBalance.div(_notNullCount)
                );
            }
        }
    ...
    function _registerAddress(address _address) private {
        if (_isRegisteredAddress(_address)) {
