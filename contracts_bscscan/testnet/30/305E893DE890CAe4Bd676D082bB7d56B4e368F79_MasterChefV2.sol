...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    uint256 public constant RevolutionMaximumSupply = 500000 * (10 ** 18);

    RevolutionToken public revolution;
    uint16 public constant referralCommissionRate = 300; // 3%

    // tokens created per block.
    uint256 public revolutionPerBlock = 1 * (10 ** 18);
    // Deposit Fee address
    address public feeAddress;

    IReferralSystem revolutionReferral;
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (revolution.totalSupply() > RevolutionMaximumSupply)
            revolutionReward = 0;
        else if ((revolution.totalSupply() + revolutionReward) > RevolutionMaximumSupply)
            revolutionReward = RevolutionMaximumSupply - revolution.totalSupply();
...
//        if (revolution.totalSupply() >= RevolutionMaximumSupply && emissionEndBlock == type(uint256).max)
//            emissionEndBlock = block.number;
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            uint256 amountRemoved = pool.strategy.withdraw(_amount);
            if (amountRemoved > user.amount) {
                user.amount = 0;
            } else {
                user.amount = user.amount - amountRemoved;
            }
    ...
    function emergencyWithdraw(uint256 _pid) external nonReentrant {
        ...
        amount = pool.strategy.withdraw(amount);

        user.amount = 0;
        user.rewardDebt = 0;

        uint256 wantBal = pool.lpToken.balanceOf(address(this));
        pool.lpToken.safeTransfer(address(msg.sender), wantBal);
...
contract RevolutionToken is ERC20('Revolution', 'REV') {
    ...
    constructor() {
        _mint(address(msg.sender), uint256(37500000000000000000000));
    }
