...
contract Exchange is EIP712, Ownable {
    ...
    bytes32 constant BID_ORDER_TYPEHASH = keccak256(
        "Bid(Order order,address buyer,uint256 amount)"
    );
     ...
     * Emits a {Buy} event, indicating the seller, token address and token id.
     */
    function completeBidding(Bid memory bidOrder, bytes calldata signature) external onlyOwner{
        // verify signature
        ...
        // get unique key
        bytes32 hashKey = genHashKey(bidOrder.order);
        ...
        emit Buy(bidOrder.order.seller, bidOrder.order.tokenAddress, bidOrder.order.tokenId);
    }
    ...
    function genBidOrderHash(Bid memory bidOrder) internal view returns(bytes32) {
        bytes32 structHash = keccak256(
            abi.encode(BID_ORDER_TYPEHASH, bidOrder.order, bidOrder.buyer, bidOrder.amount)
        );
        return _hashTypedDataV4(structHash);
    }
