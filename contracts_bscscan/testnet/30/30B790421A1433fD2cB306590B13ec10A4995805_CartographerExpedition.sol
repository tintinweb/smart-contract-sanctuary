...
contract CartographerExpedition is Ownable, Initializable, ReentrancyGuard, ICartographerExpedition {
    ...
    function getCurrentAccRewPerShare(ExpeditionPoolInfo memory pool) internal view returns (uint256) {
        if (block.number > pool.lastRewardBlock && pool.summitSupply > 0) {
            ...
            return pool.accRewPerShare.add(expeditionReward.mul(1e12).div(pool.summitSupply));
        }
        return pool.accRewPerShare;
    }
    ...
    function getHypotheticalReward(uint8 _pid, address _userAdd) external view poolExists(_pid) validUserAdd(_userAdd) override returns (uint256, uint256) {
        ...
        uint256 accRewPerShare = getCurrentAccRewPerShare(pool);
        uint256 rew = getUserHypotheticalReward(pool, user, accRewPerShare);    
        if (rew == 0 || pool.totemRoundRewards[userTotem[_userAdd]] == 0) { return (0, 0); }
        return (
            rew,                                                                // CURRENT: How much the user has farmed currently
            rew.mul(pool.roundRewards).div(pool.totemRoundRewards[userTotem[_userAdd]])    // IF WIN: User rewards * (total round rewards / totem rewards)
        );
    }
    ...
    function withdraw(uint8 _pid, uint256 _amount, address _userAdd) external nonReentrant onlyCartographer() poolExists(_pid) validUserAdd(_userAdd) override expeditionNotLockedUntilRollover() returns (uint256) {
        ...
        pool.totemRoundRewards[totem] = pool.totemRoundRewards[totem].sub(user.roundRew);
    ...
    function totemRoundRewards(uint8 _pid) public view poolExists(_pid) returns (uint256[2] memory) {
        return [
            expeditionPoolInfo[_pid].totemRoundRewards[0],
            expeditionPoolInfo[_pid].totemRoundRewards[1]
        ];
    }
