...
//        address currency;   // 交易币
    ...
    // 拿取回（打开）用户购买的盲盒：NFT所有权转移至购买者、本合约将购买所得费用扣除手续费及佣金后转移至NFT原所有者（如果未索取收益）
    ...
    // 创建系列集合（集合名、可容纳NFT数、佣金比例、是否支持生态币、支持创作者列表）
    ...
    function publishCollection(uint256 collectionId_, address[] calldata path, uint256 amountInMax_, uint256 deadline_) external {
        ...
        uint256 times = actualSize.add(count).sub(1).div(count);  // Math.ceil （向上取整）
