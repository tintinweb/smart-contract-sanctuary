...
contract TheOneToken is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 1000 * 10**6 * 10**9;  // Swap tokens to convert and add liquidity
    uint256 public buyBackUpperLimit = 1 * 10**18;


    address public operationsAddress = 0x7620558B540DB0d959d8C826063a7049a9465cC2;  // Operations Wallet Address
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "The One Token"; // Token Name
    string private constant _symbol = "ONETOKEN"; // Token Symbol
    ...
    function setFeeRates(uint256 _rfi, uint256 _operations, uint256 _liquidity) external onlyOwner {
        feeRates.rfi = _rfi;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _operations, uint256 _liquidity) external onlyOwner{
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rOperations, to_return.rLiquidity, to_return.rBuyback) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tTransferAmount = tAmount-s.tRfi-s.tOperations-s.tLiquidity-s.tBuyback;
        }
        else{
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rOperations, uint256 rLiquidity, uint256 rBuyback) {
        ...
        rTransferAmount =  rAmount-rRfi-rOperations-rLiquidity-rBuyback;
        return (rAmount, rTransferAmount, rRfi,rOperations,rLiquidity, rBuyback);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(!_isBot[from] && !_isBot[to], "You have no power here");
