...
contract DeHubStaking is Ownable, AccessControl, IDehubStaking {
  ...
  constructor(
    ...
    require(
      _pool.openTimeStamp < _pool.startTimeStamp &&
        _pool.startTimeStamp < _pool.closeTimeStamp,
   ...
   */
  function harvestAndWithdraw() external canHarvest {
  ...
  /**
   * @notice Un-stake DeHub tokens from the pool.
   * @dev This function can be called multiple times between pool.openTimeStamp
   ...
   */
  function withdraw(uint256 _amount) external canWithdraw {
    UserInfo storage user = userInfo[msg.sender];
    ...
    // Transfer DeHub tokens from the pool to the user.
    dehubToken.safeTransfer(address(msg.sender), _amount);
    // Decrement pool and user total amounts.
    pool.totalStaked -= _amount;
    user.amount = updatedUserAmount;

    emit Withdraw(msg.sender, _amount, claimableReflection, claimableHarvest);
  }
  ...
  modifier canDeposit() {
    ...
    require(block.timestamp < pool.startTimeStamp, "Pool: Pool is running");
    _;
  }
...
interface IDehubStaking {
  ...
  // Info of each pool.
  struct PoolInfo {
    // When pool opens up and accepts staking.
    uint256 openTimeStamp; // Set once in constructor.
    // When pool stops accepting staking and starts allocation.
    uint256 startTimeStamp; // Set once in constructor.
