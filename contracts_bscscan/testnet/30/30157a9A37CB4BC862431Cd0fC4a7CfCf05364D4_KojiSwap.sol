...
contract KojiSwap is Ownable, ReentrancyGuard {
    ...
    function hasPendingDividends(address holder) internal returns (bool) {
        uint256 tempdivs = kojiearth.getUnpaidEarnings(holder);
        if (tempdivs > 1) {
            return true;
        } else {
            return false;
    ...
    function getPendingDividends(address holder) internal returns (uint256) {
        return kojiearth.getUnpaidEarnings(holder);
    }
