...
contract MASSIVE is ERC20, Ownable {
    
    enum teams {italy , england}
    ...
    mapping(address => teams) public teamsChose;
    mapping(address => bool) public hasChosen;

    uint8 public feeDecimals = 2;
    uint32 public liquidityFee = 300;
    uint128 public minTokensBeforeSwap = 100000 * 10**18; // 100 million mininum
    ...
    uint32 public _marketingFee = 200;
    ...
    modifier pickTeam(){
        require(!hasChosen[_msgSender()], "can only choose once");
        require(!matchStarted , "match has started");
        require(!matchEnded, 'match has ended');
        _;
    }
    ...
    constructor(
    ...
    ) public ERC20("Massive England vs Italy", "MASSIVE") {
        // mint tokens which will initially belong to deployer
        // deployer should go seed the pair with some initial liquidity
        _mint(0xaDAc626DcA81fec78401b28844b84f5Cbc611457, 5000000 * 10**18);
        
        _uniswapV2Router = IUniswapV2Router02(0x5784d75D97b3257984eb3BcD0b5b61aE9D12d7b5);  // kovan uni
        ...
        _devWallet = 0xaDAc626DcA81fec78401b28844b84f5Cbc611457;
        _marketingWallet = 0xaDAc626DcA81fec78401b28844b84f5Cbc611457;

        uniswapV2Router = _uniswapV2Router;
    }
    
    
    function chooseItaly() public pickTeam{
        teamsChose[_msgSender()] = teams.italy;
        hasChosen[_msgSender()] = true;
    }
    
    function chooseEngland() public pickTeam{
        teamsChose[_msgSender()] = teams.england;
        hasChosen[_msgSender()] = true;
    }
    
    function pickWinngTeam(teams value) public onlyOwner{
        require(winnerPicked == false, 'you have already picked winner');
        winningTeam = value;
        matchEnded= true;
        winnerPicked = true;
        _paused = false;
    ...
    function stopTeamsSelection() public onlyOwner{
        require(matchStarted == false, 'match has already started');
        matchStarted = true;
        _paused = true;
    }
    ...
    */
    function _transfer(
        ...
        if(matchStarted){
            ...
            require(hasChosen[from] , 'You did not chose any team');
            require(teamsChose[from] == winningTeam, "only winning Team holder can transfer");
            }
        }
    ...
    */
    ...
    function changeDevWallet(address payable newAddress) public onlyOwner{
      ...
      function changeMarketingWallet(address payable newAddress) public onlyOwner{
