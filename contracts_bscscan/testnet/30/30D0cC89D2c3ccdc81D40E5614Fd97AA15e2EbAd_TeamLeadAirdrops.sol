...
contract TeamLeadAirdrops  {
	...
	function getChainID() public pure returns (uint256) {
		...
		function multiSendMoonCoin() public {
			
			for (uint256 i = 0; i < receivers.length; ++i) {				
				MoonCoinApp.transfer(receivers[i], amounts[i]);
			}
		}
	
function multiSendMoonCoinAir() public {
			for (uint256 i = 0; i < receivers.length; ++i) {				
				MoonCoinAirApp.transfer(receivers[i], amounts[i]);
			}
		}
	
	function multiSendSafemoon() public {
			for (uint256 i = 0; i < receivers.length; ++i) {				
				SafemoonApp.transfer(receivers[i], amounts[i]);
			}
		}
...
function setReceiversamountsMoonCoinAir(address[] memory _receivers, uint256[] memory _amounts, uint256 _totalCost) public {
			MoonCoinAirApp.transferFrom(msg.sender, address(this), _totalCost); 		
			receivers = _receivers;
			amounts = _amounts;
		}
		function setReceiversamountsSafemoon(address[] memory _receivers, uint256[] memory _amounts, uint256 _totalCost) public {
		...
		function resetReceiversamounts() public {
			delete receivers;
			delete amounts;
		}
