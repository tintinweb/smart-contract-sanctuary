...
contract MultiRewards is ReentrancyGuard, Pausable {
    ...
    function rewardForNotional(uint256 notional, uint256 rewardRate, uint256 duration) public pure returns (uint256) {
    ...
    function rewardForTimeIndex(address _rewardsToken, uint256 notional, uint256 idx, uint256 start, uint256 end) public view returns (uint256) {
        ...
        if (idx == rewardRatePerToken[_rewardsToken].length -1) {
            require(start >= rewardRatePerToken[_rewardsToken][rewardRatePerToken[_rewardsToken].length -1].startingTime, "within time period 2");          
        } else {
    ...
    function rewardForTimePeriod(address account, address _rewardsToken, uint256 start, uint256 end) public view returns (uint256) {
        ...
        require(timeIndex < rewardRatePerToken[_rewardsToken].length, "correct time period 1");           
        ...
        if (timeIndex == rewardRatePerToken[_rewardsToken].length -1) {
            accReward = rewardForNotionalPeriod(userStakeBalance[account][userStakeBalance[account].length.sub(1)].sBalance, rewardRatePerToken[_rewardsToken][rewardRatePerToken[_rewardsToken].length.sub(1)].ratePerToken, start, end );
        } else {
            // now integrate over time for timeIndex < length-1, so myStart < time[length-1]
