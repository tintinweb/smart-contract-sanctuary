    ...
    **/
    ...
    function _claimByRound(uint256 roundIndex) internal{
        for (uint256 rewardLevel = 0; rewardLevel < 5; rewardLevel++) {
            if(_userChoosedTexts[roundIndex][msg.sender] == _luckyTexts[roundIndex][rewardLevel] &&
                _userChoosedNumbers[roundIndex][msg.sender] == _luckyNumbers[roundIndex][rewardLevel]){
                    if(rewardLevel == 0){
                        ...
                        uint256 winnerTotal = getWinnerCountByRoundAndRewardLevel(roundIndex, rewardLevel);
