...
contract zombieFactory is Ownable{
    ...
    uint  cooldownTime = 6000; //rare 5 cooldownTime
    ...
    function createZombieType(uint32 _typeId,uint32 _rare,string memory _typeName) public onlyOwner  {
    ...
    function _createBoss (uint32 _attributeId,uint32 _rare,uint32 _isready,uint32 _reward,uint32 _level, uint32 _energy,string memory _name) public  onlyOwner{
        boss.push(Boss(uint32(boss.length),_attributeId,_rare,_isready,_reward,_level,_energy,_name));

    }
    function _resetBoss(uint32 _Id,uint32 _isready,uint32 _attackReward) public onlyOwner{
        boss[_Id].isready = _isready;
        boss[_Id].attackReward = _attackReward;
    }
    ...
    function _getTypeId(uint32 _rare) internal   returns(uint32,string memory){
    ...
    function _createZombie(string memory _name) public  {
        ...
        zombies.push(Zombie(_zombieId,0,_typeID,_attributeID,_rare,_energy,1,0,0,0,0,0,_name,_typeName));
...
contract zombieHelper is zombieFactory{
    ...
    function _triggerPVECooldown(uint  _zombieId) public    {
        zombies[_zombieId].readyForPVETime = uint32(block.timestamp + (cooldownTime/zombies[_zombieId].rare)) ;
    }
    function _triggerPVPCooldown(uint  _zombieId) public  {
    ...
    function _isReadyForPVE(uint _zombieId)  public view returns (bool) {
      return (zombies[_zombieId].readyForPVETime <= block.timestamp);
    }

    function _isReadyForPVP(uint _zombieId)  public view returns (bool) {
    ...
    function fusionRare(uint32 _rareOne,uint32 _rareTwo) public returns(uint32){
        uint32 _Rare;
        _Rare = _rareOne > _rareTwo ? _rareOne :_rareTwo;
        uint32 _rare;
    ...
    function multiply(uint _zombieId, uint _targetId) public multiplyOnlyOwnerOf(_zombieId,_targetId) returns(uint32){
        ...
        uint32 _res;
        _res = _rare >myZombie.rare && _rare >targetZombie.rare ? _res = 1 :_res = 0;
        uint32 _typeID;
        uint32 _attributeID = uint32(_randMod(modules,5));
        uint32 _energy = uint32(_randMod(modules,_rare*100)+_rare*100);
        ...
        zombies.push(Zombie(_newZombieId,0,_typeID,_attributeID,_rare,_energy,1,0,0,0,0,0,"Noname",_typeName));
        zombieToOwner[_newZombieId] = _msgSender();
        ownerZombieCount[_msgSender()]--;
        zombieCount = zombieCount++;
        //ownerZombieCount[_msgSender()]--;
        emit Newzombie(_newZombieId,_rare,_energy,_typeID,_typeName);
        return _res;
    }
}
...
contract zombieBattlefiled is zombieOwnership {
    ...
    function getReward(uint _zombieId) public returns(uint){
        return zombies[_zombieId].reward=((block.timestamp - battleField[_zombieId].caculateTime)*battleReward*zombies[_zombieId].energy*battleField[_zombieId].rewardProportion) / 100;
    }
...
contract zombieAttack is zombieHelper,zombieBattlefiled{
    ...
    function attackPlayer(uint _zombieId,uint _targetId)external onlyOwnerOf(_zombieId) returns(bool){
        require(zombies[_zombieId].isBattling == 1 && zombies[_targetId].isBattling == 1,'zombie is not in the battlefield');
        require(_msgSender() != zombieToOwner[_targetId] && _isReadyForPVP(zombies[_zombieId].zombieId),'target zombie is yours');
        Zombie storage myZombie = zombies[_zombieId];
        ...
        if(_rand<=_attacPlayerkVictoryProbability){
            myZombie.level++;
            myZombie.winCount++;
            _triggerPVPCooldown(_zombieId);
            //payable(_msgSender()).transfer((pveReward *(100 - marketFee))/100);
            battleField[_zombieId].rewardProportion = battleField[_zombieId].rewardProportion+enemyZombie.rare;
            ...
            if(battleField[_zombieId].rewardProportion>50){
                battleField[_zombieId].rewardProportion = battleField[_zombieId].rewardProportion-myZombie.rare;
            }else{
                battleField[_zombieId].rewardProportion = 50;
            }
            enemyZombie.winCount++;
            return false;
        }
    }
    function attackBoss(uint _zombieId,uint _BossId) external onlyOwnerOf(_zombieId) returns(bool){
        require(boss[_BossId].isready == 1 && _isReadyForPVE(_zombieId));
        //Boss storage enemyBoss = boss[_BossId];
        //Zombie storage myZombie = zombies[_zombieId];
        uint _attacBosskVictoryProbability = gettAttackBossVictoryProbability(_zombieId,_BossId);
        uint _rand = _randMod(modules,10000);
        if(_rand<=_attacBosskVictoryProbability){
            zombies[_zombieId].level++;
            _triggerPVECooldown(_zombieId);
            //payable(_msgSender()).transfer((boss[_BossId].attackReward *(100 - marketFee))/100);
            return true;
        }else{
            _triggerPVECooldown(_zombieId);
            return false;
