...
contract InuloansToken is ERC20, ERC20Burnable, ERC20Snapshot, AccessControl, Pausable, ERC20Permit, ERC20VotesComp {
    ...
    modifier mintAllowedOnly(uint256 newTotalAmount) {
        require(isMintAllowed(totalSupply() + newTotalAmount), "Mint exceeds soft cap");
        _;
    }

    function snapshot()
        public onlyRole(SNAPSHOT_ROLE)
    {
        _snapshot();
    }
    ...
    function setReleaseBlock(uint256 _startReleaseBlock, uint256 _endReleaseBlock)
        external
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        IERC20(token).transfer(msg.sender, amount > totalAmount ? totalAmount : amount);
    }
    ...
    function lock(address _account, uint256 _amount) external onlyRole(MINTER_ROLE) {
    ...
    function upgradeCap(address _implementation) external onlyRole(DEFAULT_ADMIN_ROLE) {
