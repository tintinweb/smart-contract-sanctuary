...
// File: contracts/SwampWolfReferral.sol

pragma solidity 0.8.6;

contract SwampWolfReferral is ISwampWolfReferral, Ownable {
    ...
    mapping(address => References[]) public references; // referrer address => [references address]
    ...
    function recordReferral(address _user, address _referrer) public override onlyAdmin {
        if (_user != address(0)
        && _referrer != address(0)
        && _user != _referrer
            ...
            references[_referrer].push(References(_user));
    ...
    function recordReferralCommission(address _referrer, uint256 _commission) public override onlyAdmin {
    ...
    // Get all the references address from this referrer
    function getReferences(address _referrer) public  view returns (References[] memory) {
        return references[_referrer];
    }
