...
contract TestContract_02 is Context, IBEP20, Ownable {
    ...
    string internal constant _name = "CustomContract-02";
    string internal constant _symbol = "Custom-02";
    uint8 internal constant _decimals = 9;

    address public _mktWallet = owner();
    address public _devWallet = owner();
    ...
    uint256 public _maintenanceFee = _mktFee.add(_devFee);
    ...
    uint256 public maxTxAmount = 18 * 10**6 * 10**9;
    uint256 public swapAtAmount = 9 * 10**4 * 10**9;
    ...
    constructor() {
        ...
        //address payable _pancakeFactory = payable(
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        _takeOwned[account] = _takeOwned[account].sub(
            amount,
            "BEP20: burn amount exceeds balance"
        );
        _takeTotal = _takeTotal.sub(amount);
    ...
    function setDevFeePercent(uint256 taxFee) external onlyOwner {
        emit SetDevFeePercent(taxFee);
        _devFee = taxFee;
    }

    function setMktFeePercent(uint256 taxFee) external onlyOwner {
        emit SetMktFeePercent(taxFee);
        _mktFee = taxFee;
    }
    ...
    /*function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        ...
        _maximumValueOfTransaction = _takeTotal.mul(maxTxPercent).div(10**2);
    }*/
    //ELIMINATED
    ...
    function _getValues(
        uint256 tAmount /*uint256[] memory*/
    )
        internal
        view
        returns (
    ...
    {
        //uint256[] memory getValuesT = new uint256[](8);
        (
            uint256 takeAmountToTransfer,
            uint256 tFee,
            uint256 tLiquidity,
            uint256 tMktFee /*getValuesT[3], // takeAmountToTransfer
            getValuesT[4], // tFee
            getValuesT[5], // tLiquidity
            getValuesT[6], // tMktFee
            getValuesT[7] // tDevFee*/ //uint256 tDevFee
        ) = _getTValues(tAmount);
        (
            uint256 reflectAmount,
            uint256 reflectAmountToTransfer,
            uint256 rFee /*getValuesT[0], // reflectAmount
            getValuesT[1], // reflectAmountToTransfer
            getValuesT[2] // rFee*/
        ) = _getRValues(
                tAmount,
                tFee,
    ...
    function swapAndLiquify(uint256 contractTokenBalance) internal lockSwaping {
    ...
    function _transfer(
        ...
        /*if (contractTokenBalance >= _maximumValueOfTransaction) {
        ...
        if (
            ...
            uint256 tokensForLiquidity = calculateLiquidityFee(
            ...
            swapTokensForBNB(remainingTokens);
            uint256 BNBalance = address(this).balance;
            uint256 marketingFeeBNB = BNBalance.mul(_mktFee).div(
                _maintenanceFee
            );
            uint256 devFeeBNB = BNBalance.mul(_devFee).div(_maintenanceFee);
            // Send MktFee
