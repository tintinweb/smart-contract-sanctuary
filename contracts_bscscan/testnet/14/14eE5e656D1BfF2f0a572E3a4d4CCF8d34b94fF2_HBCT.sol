...
abstract contract AbstractDeflationaryAutoLPToken is AbstractDeflationaryToken {
    ...
    function inTAllowance(uint256 amount) private view returns (uint256) {
    ...
    function getLiquidity(uint256 _token1or2) private view returns (uint256) {
        address _pair = IUniswapV2Factory(_getRouter().factory()).getPair(address(this), _getWETH());
       (uint256 _tokenA, uint256 _tokenB,) = IUniswapV2Pair(_pair).getReserves();
...
contract FeeToAddrDeflAutoLPToken is DeflationaryAutoLPToken, FeeToAddress {
    ...
    // function _payFeesForEth(address sender, uint256 amount) internal {
    //     uint256 _feeSize = bnbPerc * amount / 100;
    //     require(balanceOf(sender) >= _feeSize, "Low Balance #1");
    //     _approve(sender, address(uniswapV2Router), _feeSize);
