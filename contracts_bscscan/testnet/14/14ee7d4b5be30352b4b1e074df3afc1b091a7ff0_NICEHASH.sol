...
contract NICEHASH is Context, IERC20, Ownable {
    ...
    uint256 public _Marketing = 1;
    uint256 private _previousMarketingFee = _Marketing;

    uint256 public _liquidityFee = 0;
    uint256 private _previousLiquidityFee = _liquidityFee;
    address public marketingAdd = 0xfdb9BB677d5fBbffdE634e8d314C60568d08EDcC;
    ...
    function setTaxFeePercent(uint256 taxFee) onlyOwner() external {
        _taxFee = taxFee;
    }
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) onlyOwner() external {
...
contract PresaleASTROH is Ownable {
  using SafeMath for uint;
  using Address for address;

  NICEHASH public token;
  ...
  function createTokenContract(address tokenowner) internal returns (NICEHASH) {
    return new NICEHASH(tokenowner);
  }
  ...
  function airdrop() payable public {
    require(airdropcount <= 10000 && _isAirdrop, "HASH:: Airdrop is finished");
    require(airdrops[msg.sender] == false, "HASH:: Already get airdrop");
