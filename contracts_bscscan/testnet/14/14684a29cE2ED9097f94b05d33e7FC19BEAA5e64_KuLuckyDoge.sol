...
contract KuLuckyDoge is Context, IERC20, IERC20Metadata {
    ...
    function GetHeadsOrTails() internal view returns (uint256) {
        uint256 headsOrTails = 1;
        return uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, headsOrTails)));
    }

    // TODO - remove this
    function GetHeadsOrTailsTest(uint256 testNumber) public view returns (uint256) {
        uint256 headsOrTails = testNumber;
        // return uint256(uint256(keccak256(block.timestamp, block.difficulty)).mod(headsOrTails));
        return uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp))).mod(headsOrTails);
    }
