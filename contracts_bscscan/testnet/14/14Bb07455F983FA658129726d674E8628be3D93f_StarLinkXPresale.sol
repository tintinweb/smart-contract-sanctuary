...
contract StarLinkXPresale is Ownable {
    ...
    uint256 public newTokenSupply = 900000000 * 10**9;

    address public devWallet = 0xA7affE19E9297A9E70DC20D64006D478a2E68bB3;
    bool public finalized = false;

    event AddedRateUpdated(
    ...
    uint256 public investorMinCap = 10000000000000; // .01 bnb
    uint256 public investorMaxCap = 5000000000000000; // 5 bnb
    uint256 public investorHardCap = 5000000000000000000000; // 2500 bnb
     ...
     */
    constructor(uint256 _cap, uint256 _rate) public {
        require(_cap > 0);
        cap = _cap;
        require(_rate > 0);
        rate = _rate;
    ...
    // Update dev wallet
    function updateDevAddress(address _devWallet) external onlyOwner {
        devWallet = _devWallet;
    }

    // Finalize ICO
    function finalizeICO(bool _finalized) external onlyOwner {
        finalized = _finalized;
    }

    // Update total supply of old and new token
    function updateTokensSupply(uint256 _newTokenSupply) external onlyOwner {
     ...
     */
    function processContribute(address _beneficiary)
        public
        payable
        returns (uint256)
    {
        uint256 contAmount = msg.value;
        _preValidatePurchase(_beneficiary, contAmount, investorMaxCap);
    ...
    function processClaim(address _beneficiary) public {
        ...
        uint256 tokens = ((amtClaimable * (10**newtokenDecimals)) / 10**18) *
            rate;
        uint256 totContAmount = contributions[_beneficiary];

        require(finalized == true);
        require(msg.sender == _beneficiary);
        _beneficiary = msg.sender;
        _processPurchase(_beneficiary, tokens);
        emit TokenPurchase(msg.sender, _beneficiary, totContAmount, tokens);
    }
    ...
    function _preValidatePurchase(
        ...
        require(
            _newContribution >= investorMinCap &&
                _newContribution <= _allowedContribution &&
                _newTotalContribution <= investorHardCap
        );
     ...
     */
    function getUnclaimed(address _beneficiary) public view returns (uint256) {
    ...
    // Withdraw bnb contributions to devwallet
    function sweepBNB(uint256 amount) external onlyOwner {
