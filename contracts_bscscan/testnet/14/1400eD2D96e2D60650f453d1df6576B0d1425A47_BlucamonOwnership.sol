...
contract BlucaDependency {
    ...
    modifier onlyWhitelistSetter() {
        require(msg.sender == whitelistSetterAddress, "S-PMS100");
        _;
    }

    modifier onlySpawner() {
        require(whitelistedSpawner[msg.sender], "S-PMS101");
        _;
    }

    modifier onlyAirdropSetter() {
        require(whitelistedAirdropSetter[msg.sender], "S-PMS102");
        _;
    }
...
contract BlucamonAirdrop is BlucamonFactory {
    ...
    modifier onlyAirdropWhitelist(address _address) {
        require(whitelistEggDetail[_address].id != 0, "S-ARD100");
        _;
    }
    ...
    function validateWhitelistParameter(
        ...
        require(addressCount > 0 && tokenUriCount > 0, "S-ARD101");
        require(addressCount == tokenUriCount, "S-ARD102");
    }
}
...
abstract contract BlucamonFactory is BlucaDependency {
    ...
    function spawnBlucamonEgg(uint256 _id, bool _isSummoned) internal {
        blucamons.push(Blucamon(_isSummoned, 5));
    ...
    BlucamonAirdrop
{
    ...
    modifier onlyOwnerOf(uint256 _id) {
        require(msg.sender == ownerOf(_id), "S-ONS100");
        _;
    }
    ...
    function claimBlucamonEgg() external onlyAirdropWhitelist(msg.sender) {
        require(!isClaimedMapping[msg.sender], "S-ARD103");
        AirdropEggDetail memory eggDetail = whitelistEggDetail[msg.sender];
        spawnBlucamonEgg(eggDetail.id, false);
    ...
    function summon(uint256 _id) external payable onlyOwnerOf(_id) {
        require(!blucamons[indexMapping[_id]].isSummoned, "S-SMN100");
        require(msg.value == summoningFee, "S-SMN101");
        summonBlucamon(_id);
    ...
    function mintBlucamon(
        ...
        blucamonId = blucamonId.add(1);
        spawnBlucamonEgg(blucamonId, _isSummoned);
        _safeMint(_address, blucamonId);
        _setTokenURI(blucamonId, _tokenUri);
    }
}
