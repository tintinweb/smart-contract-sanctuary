    ...
    function placeOrder(Order calldata order) external {
        ...
        while (i < orders.length && remaining > 0){
            ...
            if (order.price >= orderPrice - calculationSlippage && order.price <= orderPrice + calculationSlippage) {
                ...
                if (order.side == Side.Sell) {
                    require(TraderBalances[msg.sender][order.tokenA] >= order.amount, "Swapland: Amount of tokenA is lower than requested amount");
                    ...
                    require(TraderBalances[msg.sender][order.tokenB] >= order.amount, "Swapland: Amount of tokenB is lower than requested amount");
                    TraderBalances[msg.sender][order.tokenA] += matched;
                    TraderBalances[msg.sender][order.tokenB] -= matched * order.price;
        ...
        if (remaining > 0){
            orders.push(Order(msg.sender, order.tokenA, order.tokenB, order.amount, order.side, order.price, remaining, order.slippage));
        }
    }

    function getUserCreditForToken(address token) tokenExist(token) public view returns (uint) {
