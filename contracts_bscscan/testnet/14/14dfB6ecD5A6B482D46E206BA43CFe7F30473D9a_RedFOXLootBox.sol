...
contract RedFOXLootBox is VRFConsumerBase, Ownable {
    ...
    /// @notice Force push id to classes
    /// @dev Only Owner can push card id to each class. Duplicate ids is not allowed
    ...
    /// @param _class Class of new card
    /// @param _cardId Id of card
    function removeCardIdFromClass(Class _class, uint256 _cardId) public onlyOwner {
        uint256 index = uint(-1);
        for (uint256 i = 0; i < classToIds[_class].length; i++) {
            if (classToIds[_class][i] == _cardId) {
                index = i;
            }
        }
        require(index != uint(-1), "RedFOXLootbox: Token ID not exists in this class");
        delete classToIds[_class][index];
    }

    /// @notice Force push ids to classes
    /// @dev Only Owner can push card id to each class. Duplicate ids is not allowed
    /// @param _class Class of new card
    /// @param _cardIds Ids of card
    function pushBatchCardIdToClass(Class _class, uint256[] memory _cardIds) external {
        for (uint256 i = 0; i < _cardIds.length; i++) {
            pushCardIdToClass(_class, _cardIds[i]);
        }
    }
    ...
    function getClassByProbabilities(
        uint256 _random,
        uint16[NUM_CLASSES] memory _classProbabilities
    ) public pure returns (Class _class) {
