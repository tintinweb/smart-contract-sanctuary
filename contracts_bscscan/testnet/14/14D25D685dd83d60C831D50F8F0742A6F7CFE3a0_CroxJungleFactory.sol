...
import "./CroxJungle.sol";
import "./../libs/IBEP20.sol";

contract CroxJungleFactory is Ownable {
    event NewCroxJungleContract(address indexed croxJungle);
     ...
     * @param _rewardHarvestFee: the harvest fee of first rewardToken
     * @param _secRewardHarvestFee: the harvest fee of second rewardToken
     * @param _lockPeriod: the lock period
     * @param _admin: admin address with ownership
     * @return address of new crox jungle contract
     */
    function deployPool(
        ...
        uint256 _rewardHarvestFee,
        uint256 _secRewardHarvestFee,
        uint256 _lockPeriod,
        address _admin
    ) external onlyOwner {
        ...
        address croxJungleAddress = _jungleAddress(IBEP20(_stakedToken), IBEP20(_rewardToken), _startBlock);

        CroxJungleInitializable(croxJungleAddress).initialize(
            _stakedToken,
            _rewardToken,
            _secRewardToken,
            _feeAddress,
            ...
            _bonusEndBlock,
            _rewardHarvestFee,
            _secRewardHarvestFee,
            _lockPeriod,
            _admin
        );

        emit NewCroxJungleContract(croxJungleAddress);
    }

    function _jungleAddress(
        IBEP20 _stakedToken,
        IBEP20 _rewardToken,
        uint256 _startBlock
    ) internal returns (address) {
        bytes memory bytecode = type(CroxJungleInitializable).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address croxJungleAddress;

        assembly {
            croxJungleAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        return croxJungleAddress;
    }
}
...
contract CroxJungleInitializable is Ownable, ReentrancyGuard {
    using SafeMath for uint256;

    // The address of fee charge
    address public feeAddress;

    // The address of the crox jungle factory
    address public CROX_JUNGLE_FACTORY;
    ...
    // The transfer fee of reward token
    uint256 public rewardHarvestFee;
    uint256 public secRewardHarvestFee;

    // The deposit fee of main tokens
    ...
    event NewRewardTokenHarvestFee(uint256 transferFee, bool isFirstRewardToken);
    event NewWithdrawalInterval(uint256 interval);
    event NewDepositFee(uint256 fee);

    constructor() public {
        CROX_JUNGLE_FACTORY = msg.sender;
    }
     ...
     * @param _rewardHarvestFee: the tranfer fee of harvest first reward token
     * @param _secRewardHarvestFee: the tranfer fee of harvest second reward token
    ...
    function initialize(
        ...
        require(msg.sender == CROX_JUNGLE_FACTORY, "Not factory");
        ...
        rewardHarvestFee = _rewardHarvestFee;
        secRewardHarvestFee = _secRewardHarvestFee;
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                if (rewardHarvestFee > 0) {
                    uint256 harvestFee = pending.mul(rewardHarvestFee).div(10000);
                    rewardToken.transfer(feeAddress, harvestFee);
                    rewardToken.transfer(address(msg.sender), pending.sub(harvestFee));
                } else rewardToken.transfer(address(msg.sender), pending);
            ...
            if (secRewardToken != address(0)) {
                ...
                if (secPending > 0) {
                    if (secRewardHarvestFee > 0) {
                        uint256 harvestFee = secPending.mul(secRewardHarvestFee).div(10000);
                        IBEP20(secRewardToken).transfer(feeAddress, harvestFee);
                        IBEP20(secRewardToken).transfer(address(msg.sender), secPending.sub(harvestFee));
                    } else IBEP20(secRewardToken).transfer(address(msg.sender), secPending);
                    // user.withdrawFrom = block.timestamp.add(lockPeriod);
                }
            }
        }
    ...
    /*
     * @notice Update staked first reward token harvest fee
     * @dev Only callable by owner.
     * @param _harvestFee: the harvest fee of reward token
     */
    function updateFirstRewardTokenHarvestFee(uint256 _harvestFee, bool isFirst) external onlyOwner {
        require(_harvestFee < 10000, "Invalid transfer fee of reward token");
        if (isFirst) {
            rewardHarvestFee = _harvestFee;
        } else {
            secRewardHarvestFee = _harvestFee;
        }

        emit NewRewardTokenHarvestFee(_harvestFee, isFirst);
    }
