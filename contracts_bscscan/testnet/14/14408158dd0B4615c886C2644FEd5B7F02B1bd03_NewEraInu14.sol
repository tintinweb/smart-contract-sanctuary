...
contract NewEraInu14 is Context, IERC20, Ownable {
    ...
    /* Naming */
    string private _name = "New Era Inu 14";
    string private _symbol = "NewEraInu14";
    ...
    uint256 public _maxWalletAmount = 3 * (_tTotal / 100);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (to != getPairAddress() && to != DEAD)
            require(_isExcludedFromFee[to] || amount + balanceOf(to) <= _maxWalletAmount, "Transfer amount exceeds the maxWalletAmount.");
        ...
        // Don't take any fees if marketing address or BuyBack addresses aren't set.
        if (_marketingAddress == address(0) || _BuyBackAddress == address(0)) {
            takeFee = false;
        }
        ...
        if (
            ...
            uint256 totalFee = _BuyBackFee.add(_marketingFee).add(_liqFee);
    ...
    function _takeFees(uint256 amount, address from) private lockTakeFees {
        ...
        uint256 walletFee = _BuyBackFee.add(_marketingFee);
        ...
        // Calculate BuyBack and marketing portions of the swapped ETH (also remember to multiply this factor by 10)
        uint256 BuyBackETHPortion = swappedETH.div(totalFeeToBeSwappedForETHMul10).mul(_BuyBackFee.mul(10));
        ...
        uint256 totalETHPortionForWallets = BuyBackETHPortion.add(marketingETHPortion);
        uint256 liquidityPoolETHPortion = swappedETH.sub(totalETHPortionForWallets);

        // Transfer ETH to fee wallets
        (bool sent, bytes memory data) = _BuyBackAddress.call{value: BuyBackETHPortion}("");
        require(sent, 'ETH was not sent to BuyBack');
    ...
    function removeAllFees() private {
        if(_taxFee == 0 && _marketingFee == 0 && _BuyBackFee == 0 && _liqFee == 0) return;
    ...
    function showBuyBackAddress() external onlyOwner view returns(address payable) {
        return _BuyBackAddress;
    }
    
    function showMarketingAddress() external onlyOwner view returns(address payable) {
