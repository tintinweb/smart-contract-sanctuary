...
contract Windmill_Power is Base, Math, AccessControlEnumerable, With_DAORole, ERC20, With_FundContract, I_Windmill_Power{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    modifier withGuard() {
        require(!guarded, "Windmill_Power: guard token already false");
        guarded = true;
        _;
        guarded = false;
    }
    ...
    ////
    //////////////// Public functions ////////////////
    function mintTo(address to, uint256 amount) external onlyRole(MINTER_ROLE) withGuard{
        _mint(to, amount);
    }

    function burnFrom(address from, uint256 amount) external onlyRole(BURNER_ROLE) withGuard{
        _burn(from, amount);
    }

    function transferFromTo(address from, address to, uint256 amount) external onlyRole(MOVER_ROLE) withGuard{
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override{
        if (!guarded){
            fund.beforePWRTransfer(from, to, amount);
        }
    }
