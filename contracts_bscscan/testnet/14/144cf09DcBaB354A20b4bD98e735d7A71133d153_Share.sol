...
contract Share is ERC777OwnableUpgradeSafe, ContractGuard {
    using SafeMath for uint256;
    ...
    // TOTAL MAX SUPPLY = 1,000,000 SDS
    uint256 public constant COMMUNITY_FUND_POOL_ALLOCATION = 200000 ether;
    uint256 public constant DEV_FUND_POOL_ALLOCATION = 175000 ether;
    uint256 public constant IDO_ALLOCATION = 25000 ether;

    uint256 public constant VESTING_DURATION = 80 weeks;
    uint256 public constant startTime = 1623412800; // Friday, 11 June 2021 12:00:00 PM UTC
    uint256 public constant endTime = startTime + VESTING_DURATION; // Friday, 23 December 2022 12:00:00 PM UTC

    uint256 public constant communityFundRewardRate = COMMUNITY_FUND_POOL_ALLOCATION / VESTING_DURATION;
    uint256 public constant devFundRewardRate = DEV_FUND_POOL_ALLOCATION / VESTING_DURATION;
    ...
    function initialize(uint256 _cap, address _taxFund) public initializer {
        __ERC777_init("SafeDollar Share", "SDS", false);
        require(_taxFund != address(0), "zero");
        cap = _cap;
        taxFund = _taxFund;
        devFund = msg.sender;
        burnRate = 0; // 0%
        taxRate = 200; // 2% (1% to buyback SDO and burn, 1% to add liquidity)
        ...
        _mint(_msgSender(), IDO_ALLOCATION.add(1 ether), "", ""); // mint extra 1 SDS for initial pools deployment
    }
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        require(recipient != address(0), "SDO: transfer to the zero address");
    ...
    function transferFrom(
        ...
        require(recipient != address(0), "SDS: transfer to the zero address");
        require(holder != address(0), "SDS: transfer from the zero address");
    ...
    function _send(
        ...
        require(from != address(0), "SDS: send from the zero address");
        require(to != address(0), "SDS: send to the zero address");
    ...
    function _move(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedToFee[to]) {
            {
                ...
                if (_taxRate > 0) {
                    ...
                    _balances[from] = _balances[from].sub(_taxAmount, "Safe Dollar: transfer amount exceeds balance");
                    _balances[_taxFund] = _balances[_taxFund].add(_taxAmount);
                    _amount = _amount.sub(_taxAmount);
                    _totalTaxAdded = _totalTaxAdded.add(_taxAmount);
                    emit TaxAdded(from, _taxFund, _taxAmount);
                    emit Transfer(from, _taxFund, _taxAmount);
                }
            }
            {
        ...
        _balances[from] = _balances[from].sub(_amount, "SDS: transfer amount exceeds balance");
