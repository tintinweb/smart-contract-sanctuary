...
ReentrancyGuardUpgradeable {
    ...
    function calculateAdditionalReward(address user)
    public  
    returns (uint256) {
        ...
        if (t1 < 30 days) {
            ...
            if (t2 < 30 days) {
                ...
                uint256 secondPhaseReward = userBalance*(t2+(30 days))*(t2-(30 days))/(2*GOEN_CONSTANT);
                return firstPhaseDuration * (userBalance/(2*SECOND_IN_YEAR)) + secondPhaseReward;
            } else {
                uint256 secondPhaseReward = userBalance*((60 days)+(30 days))*((60 days)-(30 days))/(2*GOEN_CONSTANT);
        ...
        if (t1 < 60 days) {
            if (t2 < 60 days) {
                return userBalance*(t2+t1)*(t2-t1)/(2*GOEN_CONSTANT);
            } else {
                uint256 secondPhaseReward = userBalance*userBalance*((60 days)+t1)*((60 days)-t1)/(2*GOEN_CONSTANT);
                uint256 thirdPhaseDuration = t2 - (60 days);
                emit Debug('thirdPhaseDuration',thirdPhaseDuration);
                emit Debug('secondPhaseReward',secondPhaseReward);
                emit Debug('1',userBalance*thirdPhaseDuration/SECOND_IN_YEAR);
