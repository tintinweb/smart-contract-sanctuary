...
contract DaiSwap is SafeMath {
    ...
    address feeReceiver = 0x4aFAdd1E9B0a47E90A803A10A730CDD097447f5f;
    ...
    function swapForUSDC(uint dai) public {
        ...
        if ( !daiContract.transferFrom(msg.sender, address(busdReceiver), dai)) revert();
        if (!usdcContract.transfer(msg.sender, usdc)) revert();
    }
    ...
    function swapForDai(uint usdc) public {
        ...
        uint feeT = safeSub(usdc, dai);
        if (!usdcContract.transferFrom(msg.sender, address(feeReceiver), feeT)) revert();
        if (!usdcContract.transferFrom(msg.sender, address(busdReceiver), safeSub(usdc, feeT))) revert();
