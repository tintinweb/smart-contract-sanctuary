...
contract TokenTest3 is ERC20, SafeMath, Ownable {
    ...
    function createDeposit(address pool, address tokens, uint256 amount)
     public 
     returns (bool success) 
    {   
        balances[msg.sender] = safeSub(balances[msg.sender], amount);
        transfer(pool, amount);
        deposits[pool][tokens] = safeAdd(deposits[pool][tokens], amount);
        emit Transfer(msg.sender, pool, amount);
        return true;
    }
}