...
contract CardanoEvo is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    constructor (address payable _marketWallet, address payable _CEXWallet,address payable _buyBackWallet) {
    ...
    function setBuyBackWallet(address payable _buyBackWallet) external onlyOwner{
        require(_buyBackWallet != address(this),"Invalid address");
        buyBackWallet = _buyBackWallet;
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        //get transferBNB to wallet.
        getFeeAmount(amount, takeFee);
    ...
    function getFeeAmount(uint256 amount, bool takeFee)internal {
        
        uint256 balance = address(this).balance;
        uint256 tokenAmountToBeSwapped = amount.mul(_buyBackFee.add(_marketFee).add(_sellMarketFee)).div(1e3);
        
        if(takeFee){
            Utils.swapTokensForEth(address(pancakeRouter), tokenAmountToBeSwapped);
            
            uint256 rewardBNB = address(this).balance.sub(balance);
            if(_buyBackFee > 0){
                uint256 bnbAmount = rewardBNB.mul(_buyBackFee).div(_buyBackFee.add(_marketFee).add(_sellMarketFee));
                require(payable(buyBackWallet).send(bnbAmount),"Cardano :: transaction failed");
            }
            if(_marketFee > 0){
                uint256 bnbAmount = rewardBNB.mul(_marketFee).div(_buyBackFee.add(_marketFee).add(_sellMarketFee));
                require(payable(marketWallet).send(bnbAmount),"Cardano :: transaction failed");
            }
            if(_sellMarketFee > 0){
                uint256 bnbAmount = rewardBNB.mul(_sellMarketFee).div(_buyBackFee.add(_marketFee).add(_sellMarketFee));
                require(payable(marketWallet).send(bnbAmount),"Cardano :: transaction failed");
            }
        }
    }
