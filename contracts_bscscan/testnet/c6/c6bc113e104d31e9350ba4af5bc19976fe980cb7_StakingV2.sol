...
contract StakingV2 is Initializable, OwnableUpgradeable, Earning, Delegate {
    function initialize(
        address _token,
        address _minter,
        uint256 _startBlock
    ...
    function compound() external {
        ...
        uint256 _amount = _pendingReward(msg.sender);
...
abstract contract Earning is OwnableUpgradeable, IEarning {
    ...
    function deposited(address _user) external view virtual override returns (uint256) {
    ...
    function getNumOfBlocks(uint256 _from, uint256 _to) internal view virtual returns (uint256) {
    ...
    function _pendingReward(address _user) internal view virtual returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && stakedSupply != 0) {
            uint256 multiplier = getNumOfBlocks(pool.lastRewardBlock, block.number);
    ...
    function getToken() external view virtual override returns (address) {
...
contract MinterV2 is IMinter, Initializable, OwnableUpgradeable {
    ...
    function setRewardPerBlock(uint256 _newValue) external override onlyOwner {
        rewardPerBlock = _newValue;
    }

    function getStatus(address _farm, address _user) external view override returns (FarmInfo memory) {
        FarmInfo memory f;
    ...
    function getStatusStaking(address _user) external view override returns (FarmInfo[] memory) {
    ...
    function getStatusFarms(address _user) external view override returns (FarmInfo[] memory) {
    ...
    function registerFarm(address _farm, uint256 _multiplier) external override onlyOwner {
    ...
    function registerStaking(address _farm, uint256 _multiplier) external override onlyOwner {
        require(_multiplier > 0, "Multiplier can't be 0");
    ...
    function mint(address _to, uint256 _amount) external override {
        ...
        minted += (_amount * devPercentage) / 1000;
        require(token.mint(_to, _amount));
    }

    function devPull() external override {
        require(token.mint(dev, minted), "failed to transfer to dev");
        minted = 0;
    }
...
interface IMinter {
    ...
    function setRewardPerBlock(uint256 _newValue) external;
