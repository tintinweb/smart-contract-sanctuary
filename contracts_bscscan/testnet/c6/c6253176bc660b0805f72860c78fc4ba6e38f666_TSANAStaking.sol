...
contract TSANAStaking is Ownable, ReentrancyGuard {
  using SafeMath for uint256;
  ...
  // max Staking Tokens
  uint256 public maxStakingTokens;
   ...
   * @param _maxStakingTokens: max Staking Tokens
  ...
  function initialize(
    ...
    maxStakingTokens =_maxStakingTokens;
  ...
  function deposit(uint256 _amount) external nonReentrant {
    require(totalStakingTokens.add(_amount) <= maxStakingTokens, 'no more stake in this pool');
    require(isFrozen() == false, "deposit is frozen");
    require(isFull == false, "Staking is full");
    
    UserInfo storage user = userInfo[msg.sender];
    require(_amount.add(user.amount) >= minDepositAmount, "User amount below minimum");
    
    if(totalStakingTokens==maxStakingTokens){
         isFull = true;
      }
