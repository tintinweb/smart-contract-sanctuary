...
contract SamoyedVaultHelper {
    using LowGasSafeMath for uint256;
    using SafeERC20 for IERC20;
    using SafeERC20 for ISamoyedVault;
    ...
    function withdrawToTokens(address vault, uint256 withdrawAmount) external {
        (ISamoyedVault _vault, IUniswapV2Pair _pair) = _getVaultPair(vault);
    ...
    function _getVaultPair(address vault) private view returns (ISamoyedVault _vault, IUniswapV2Pair _pair) {
        _vault = ISamoyedVault(vault);

        address pairAddress = _vault.want();
