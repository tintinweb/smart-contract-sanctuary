...
interface IHachikoLottery {
    ...
    /**
     * @notice Draw the final number, calculate reward in HACHIKO per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in HACHIKO token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInHachiko: price of a ticket in HACHIKO
...
/** @title Hachiko Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract HachikoLottery is ReentrancyGuard, IHachikoLottery, Ownable {
    ...
    uint256 public maxPriceTicketInHachiko = 50 ether;
    uint256 public minPriceTicketInHachiko = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInHachiko;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] hachikoPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInHachiko;
        uint32 finalNumber;
    }
     ...
     * @param _hachikoTokenAddress: address of the HACHIKO token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _hachikoTokenAddress, address _randomGeneratorAddress) {
        hachikoToken = IERC20(_hachikoTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of HACHIKO to this contract
        uint256 amountHachikoToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInHachiko,
            _ticketNumbers.length
        );

        // Transfer hachiko tokens to this contract
        hachikoToken.safeTransferFrom(address(msg.sender), address(this), amountHachikoToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInHachiko += amountHachikoToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInHachikoToTransfer
        uint256 rewardInHachikoToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInHachikoToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        hachikoToken.safeTransfer(msg.sender, rewardInHachikoToTransfer);

        emit TicketsClaim(msg.sender, rewardInHachikoToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInHachiko) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in HACHIKO for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].hachikoPerBracket[j] =
                ...
                // A. No HACHIKO to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].hachikoPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInHachiko - amountToShareToWinners);

        // Transfer HACHIKO to treasury address
        hachikoToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwner {
        ...
        hachikoToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInHachiko += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInHachiko >= minPriceTicketInHachiko) && (_priceTicketInHachiko <= maxPriceTicketInHachiko),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInHachiko: _priceTicketInHachiko,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            hachikoPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInHachiko: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(hachikoToken), "Cannot be HACHIKO token");
    ...
    /**
     * @notice Set HACHIKO price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInHachiko: minimum price of a ticket in HACHIKO
     * @param _maxPriceTicketInHachiko: maximum price of a ticket in HACHIKO
     */
    function setMinAndMaxTicketPriceInHachiko(uint256 _minPriceTicketInHachiko, uint256 _maxPriceTicketInHachiko)
        external
        onlyOwner
    {
        require(_minPriceTicketInHachiko <= _maxPriceTicketInHachiko, "minPrice must be < maxPrice");

        minPriceTicketInHachiko = _minPriceTicketInHachiko;
        maxPriceTicketInHachiko = _maxPriceTicketInHachiko;
    }
     ...
     * @param _priceTicket price of a ticket (in HACHIKO)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].hachikoPerBracket[_bracket];
        } else {
            return 0;
        }
    }
