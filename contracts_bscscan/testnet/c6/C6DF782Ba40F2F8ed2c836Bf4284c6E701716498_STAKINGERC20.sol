...
contract BaseFarm {
        ...
        returns (uint256)
    {
        ...
        if (roundPassed == 0) {
            ...
            uint256 multiplier = rateConfig.reducingCycle *
                rateConfig.firstCycleRate *
                MAGIC_NUMBER;
            uint256 i = 0;
    ...
    ITokenFarm
{
    ...
    uint256 public accABURAPerShare;
    uint256 public lockRewardDuration;

    // Info of each user that stakes ABURA tokens.
    ...
    function initialize(
        ...
        require(
            _rateParameters[2] > 0 && _rateParameters[2] < 100,
        ...
        returns (uint256 amount)
    {
        ...
        uint256 _accABURAPerShare = accABURAPerShare;
        ...
        if (
            ...
            _accABURAPerShare =
                _accABURAPerShare +
                (_tokenReward / _tokenSupply);
        }
        amount =
            ((user.amount * _accABURAPerShare) / MAGIC_NUMBER) -
            user.rewardDebt;
    }
    ...
    function updatePool() public mustActive {
        ...
        uint256 _multiplier = getMultiplier(lastRewardBlock, block.number);
        uint256 _tokenReward = _multiplier * tokenPerBlock;
        accABURAPerShare = accABURAPerShare + (_tokenReward / _tokenSupply);
        lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _amount) external override mustActive {
        ...
        updatePool();
        if (user.amount > 0) {
            uint256 pending = ((user.amount * accABURAPerShare) /
                MAGIC_NUMBER) - user.rewardDebt;
            if (pending > 0) {
        ...
        user.rewardDebt = (user.amount * accABURAPerShare) / MAGIC_NUMBER;
        if (_amount > 0) {
    ...
    function withdraw(uint256 _amount) external override {
        ...
        uint256 pending = ((user.amount * accABURAPerShare) / MAGIC_NUMBER) -
            user.rewardDebt;
        if (pending > 0) {
            _calcTransferOfLock(msg.sender, pending);
        }
    ...
    function claimLockReward(uint256 _index) external nonReentrant {
        ...
        require(
            _index < userToLockRewardList[_user].length,
            "Farm: Invalid index"
        );
    ...
    function claimTotalReward(uint256 _start, uint256 _end)
        external
        nonReentrant
    {
        address _user = msg.sender;
    ...
    function getTotalClaimableAmount(
        ...
        for (uint256 _index = _start; _index <= _end; _index++) {
            totalClaimableAmount =
                totalClaimableAmount +
                _getClaimableAmount(_user, _index);
        }
    }
    ...
    function getLockRewardCountByUser(address _user)
        external
        view
    ...
    function updateReducingRate(uint256 _reducingRate)
        external
        onlyOwner
        mustActive
    {
        require(
            _reducingRate > 0 && _reducingRate <= 100,
            "Farm: Invalid reducing rate"
        );
        rateConfig.reducingRate = _reducingRate;
    }

    function updateLockDuration(uint256 _lockRewardDuration)
        external
        onlyOwner
        mustActive
    {
    ...
    function _claimLockReward(
        ...
        if (
            userToLockRewardList[_user][_index].amount ==
            userToLockRewardList[_user][_index].claimedAmount
        ) {
    ...
    function _calcTransferOfLock(address _user, uint256 _amount) internal {
        ...
        LockRewardInfo memory info = LockRewardInfo(
            _amount,
            block.number,
            lockRewardDuration,
            0,
