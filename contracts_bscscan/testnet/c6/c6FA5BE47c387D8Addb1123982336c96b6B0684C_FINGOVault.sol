...
contract FINGOVault is SafeOwnable {
    ...
    constructor(IFINGOToken _rewardToken) {
        rewardToken = _rewardToken;
    }

    function updateRooHash(bytes32 _rootHash, uint256 _releaseAmount) external onlyOwner {
        emit NewHash(rootHash, _rootHash);
        rootHash = _rootHash;
        rewardToken.mint(address(this), _releaseAmount);
        totalReleaseAmount = totalReleaseAmount.add(_releaseAmount);
    }

    function claim(address _to, uint _amount, bytes32[] memory _proof) external {
        uint totalAmount = _amount.add(userClaimed[_to]);
        bytes32 leaf = keccak256(abi.encodePacked(_to, totalAmount));
        require(MerkleProof.verify(_proof, rootHash, leaf), "illegal amount");
        userClaimed[_to] = userClaimed[_to].add(_amount);
        rewardToken.safeTransfer(_to, _amount);
        emit Claim(_to, _amount, totalAmount);
    }
}