...
contract FairTrade {
    ...
    function initTrade(address[] calldata items) public payable returns (bytes32){
        ...
        for (uint i = 0; i < items.length; i++) {
            require(IOwnable(items[i]).owner() == msg.sender, "An item ownership cannot be transferred");
            IOwnable(items[i]).transferOwnership(address(this));
            emit TransferOwnershipToContract(tradeId, items[i], msg.sender);
        }
...
/*
    function transferMyOwnerships(byte32 tradeId) public {
        Trade memory trade = _trades[tradeId];
