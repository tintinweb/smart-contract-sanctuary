...
library OasisStakeLib {
    ...
    returns (uint256) {
        ...
        require(remaining > 0, "OasisStaking: Staking cap is filled");
        // require((remaining + stakedTotal) <= stakingCap, "OasisStaking: this will increase staking amount pass the cap");
    ...
    external
    returns (bool) {
        require(rewardAmount > 0, "OasisStaking: reward must be positive");
        require(withdrawableAmount >= 0, "OasisStaking: withdrawable amount cannot be negative");
        require(withdrawableAmount <= rewardAmount, "OasisStaking: withdrawable amount must be less than or equal to the reward amount");
    ...
    returns (uint256) {
        require(amount <= state._stakes[from], "OasisStaking: not enough balance");
        if (now < withdrawEnds) {
    ...
    returns (uint256) {
        ...
        require(principalPaid && rewardPaid, "OasisStaking: error paying");
    ...
    modifier _realAddress(address addr) {
        require(addr != address(0), "OasisStaking: zero address");
        _;
    }

    modifier _positive(uint256 amount) {
        require(amount != 0, "OasisStaking: negative amount");
        _;
    }

    modifier _after(uint eventTime) {
        require(now >= eventTime, "OasisStaking: bad timing for the request");
        _;
    }
 ...
 * One can create several TraditionalOasisStaking over one
...
contract OasisStakeOptimized is IOasisStake {
    ...
    constructor (
        ...
        _name = name_;
        require(tokenAddress_ != address(0), "OasisStaking: 0 address");
        tokenAddress = tokenAddress_;

        require(stakingStarts_ > 0, "OasisStaking: zero staking start time");
        ...
        require(stakingEnds_ >= stakingStarts, "OasisStaking: staking end must be after staking starts");
        stakingEnds = stakingEnds_;

        require(withdrawStarts_ >= stakingEnds, "OasisStaking: withdrawStarts must be after staking ends");
        withdrawStarts = withdrawStarts_;

        require(withdrawEnds_ >= withdrawStarts, "OasisStaking: withdrawEnds must be after withdraw starts");
        withdrawEnds = withdrawEnds_;

        require(stakingCap_ >= 0, "OasisStaking: stakingCap cannot be negative");
...
abstract contract RewardAdder is IOasisStake {
    address  public rewardTokenAddress;
    OasisStakeLib.FestakeRewardState public rewardState;
    ...
    external returns (bool) {
        return OasisStakeLib.addReward(rewardAmount, withdrawableAmount,
            rewardTokenAddress, rewardState);
    }
}

contract OasisStakeWithReward is OasisStakeOptimized, RewardAdder {
    constructor (string memory name_,
        ...
        uint withdrawStarts_,
        uint withdrawEnds_,
        uint256 stakingCap_) OasisStakeOptimized (
            name_,
            tokenAddress_,
            stakingStarts_,
            stakingEnds_,
        ...
        require(rewardTokenAddress_ != address(0), "OasisStaking: 0 reward address");
    ...
    external {
        require(msg.sender == rewardSetter, "OasisStaking: Not allowed");
        rewardState.earlyWithdrawReward = withdrawableAmount;
        OasisStakeLib.addMarginalReward(rewardTokenAddress, tokenAddress,
