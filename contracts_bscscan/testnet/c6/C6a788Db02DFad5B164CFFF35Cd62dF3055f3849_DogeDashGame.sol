...
contract DogeDashGame is Ownable{
    ...
    function withdrawToPlayer(address player_wallet, uint256 amount, uint256 _version) checkVersion(_version) public isNotPaused onlyAdmin {
        ...
        require(block.timestamp - playerInfo.last_deposit_ts > tenmin_seconds, "You are not able to claim");

        // total withdraw rate per 10 mins
        uint256 total_withdraw_rate = TOTAL_WITHDRAW_RATE * tenmin_seconds / total_seconds;
        // we need to validate max withdraw per day.
        if(withdraw_counter >= total_withdraw_rate) {
            require(block.timestamp - withdraw_ts >= tenmin_seconds, "Exceed total withdraw rate");

        }

        // each player withdraw rate per 10 mins
        uint256 player_withdraw_rate = PLAYER_WITHDRAW_RATE * tenmin_seconds / total_seconds;
        ...
        if(player_withdraw_cnt >= player_withdraw_rate) {
            ...
            require(block.timestamp - ts >= tenmin_seconds, "Exceed player's withdraw rate");
        }
        ...
        // DogeDash.transferFrom(address(this), player_wallet, amount);
