...
contract CakePrizePool is PrizePool {
  using SafeMathUpgradeable for uint256;
  using SafeERC20Upgradeable for IERC20Upgradeable;

  event CakePrizePoolInitialized(address indexed vToken);
  ...
  initializer
  {
    ...
    emit CakePrizePoolInitialized(address(vToken));
  }

  /// @dev Gets the balance of the underlying assets held by the Yield Service
  /// @return The underlying balance of asset tokens
  function _balance() internal override returns (uint256) {
    (uint amount,) = vToken.userInfo(poolId, address(this));
    return amount;
  }
...
//    require(vToken.withdraw(amount) == 0, "BunnyPrizePool/withdraw-failed");
  ...
  function earned() external view returns (uint){
    uint256 earned = vToken.pendingCake(poolId,address(this));
    (, uint valueInUSD) = PRICE_CALCULATOR.valueOfAsset(address(_token()), earned);
    return valueInUSD;
  }

  /// compound
  function getReward() external returns (uint256){
    vToken.leaveStaking(0);
...
import "./CakePrizePool.sol";
import "../../external/openzeppelin/ProxyFactory.sol";

/// @title Cake Prize Pool Proxy Factory
/// @notice Minimal proxy pattern for creating new Cake Prize Pools
contract CakePrizePoolProxyFactory is ProxyFactory {

  /// @notice Contract template for deploying proxied Prize Pools
  CakePrizePool public instance;

  /// @notice Initializes the Factory with an instance of the Venus Prize Pool
  constructor () public {
    instance = new CakePrizePool();
  }

  /// @notice Creates a new Cake Prize Pool as a proxy of the template instance
  /// @return A reference to the new proxied Cake Prize Pool
  function create() external returns (CakePrizePool) {
    return CakePrizePool(deployMinimal(address(instance), ""));
  }
}
