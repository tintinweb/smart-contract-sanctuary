...
interface IManagableObject {
    function onlyEvolver (address sender) external view returns (bool);
    function onlySpawer (address sender) external view returns (bool);
}
    ...
    ReentrancyGuard
{
    ...
    uint256 public maxSupply    = 1000000000 * 10 ** 9; // Total supply: 1000000000
    uint256 public maxMarketing = 30000000   * 10 ** 9; // 3% for marketing
    uint256 public tokenForMarketing;

    // Tokens for rewards
    uint256 public availableRewards = 200000000  * 10 ** 9; // 20% for rewards
    
    // Tokens for airdrop
    uint256 public availableAirdrop = 20000000   * 10 ** 9; // 2% for airdrop
    ...
    constructor() ERC20("COLS", "COLS") {
        _mint(_msgSender(), maxSupply.sub(availableAirdrop).sub(availableRewards).sub(maxMarketing));
        _mint(address(this), availableAirdrop.add(maxMarketing).add(availableRewards));
    ...
    function setMinTokensBeforeSwap(uint256 _tokenForMarketing)
        ...
        onlyOwner
    {
        require(_tokenForMarketing < maxMarketing);
    ...
    function sweepTokenForMarketing(uint256 amount) public onlyOwner {
        ...
        if (contractTokenBalance >= amount && amount < maxMarketing) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (
            ...
            availableRewards = availableRewards.add(_fee.div(2));
        }
