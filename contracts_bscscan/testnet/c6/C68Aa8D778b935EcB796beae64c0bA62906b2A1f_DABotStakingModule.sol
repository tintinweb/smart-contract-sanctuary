...
contract DABotStakingModule is DABotModule, IDABotStakingModuleEvent {
    ...
    function governVaults(address certToken, address account) external view returns(VaultInfo[] memory result) {
    ...
    function upgradeVault(IDABotCertToken certToken) external {
        _upgradeVault(certToken, _msgSender());
    }
    ...
    function _upgradeVault(IDABotCertToken certToken, address account) private {
        ...
        if (amount == 0)
            return;
        vaultManager.delegateWidthdraw(vID0, account, amount);
