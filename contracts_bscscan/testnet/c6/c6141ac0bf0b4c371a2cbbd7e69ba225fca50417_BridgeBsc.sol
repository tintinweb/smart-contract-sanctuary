...
contract BridgeBsc is Initializable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  string public constant name = "BSC-EKTA Bridge";
  mapping(address => address) public BscEktaPairs; //BSC Token => EKTA Token
  mapping(address => address) public EktaBscPairs; //EKTA Token => BSC Token
  mapping(address => mapping(uint => bool)) public processedNonces;

  address public admin;
  address public tokenToSwapWithNative;

  event AddedTokenPair(address indexed bscToken, address indexed ektaToken);
  event UpdatedTokenPair(address indexed bscToken, address indexed ektaToken);
  event TokenDeposited(address indexed bscToken, address indexed ektaToken, uint256 amount, address indexed user);
  event TokenWithdrawn(address indexed bscToken, address indexed ektaToken, uint256 amount, address indexed user);
  event DepositedTokenToNative(address indexed bscToken, uint256 amount, address indexed user);
  event WithdrawnNativeToToken(address indexed bscToken, uint256 amount, address indexed user);
  event TokenFromContractTransferred(address externalAddress,address toAddress, uint amount);
  event BnbFromContractTransferred(address toAddress, uint amount);
  ...
  /**
   * @dev Creates a pair for bsc and ekta token by the caller with MAPPER_ROLE.
   *
   * Requirements:
   * - @param _bscToken cannot be the zero address.
   ...
   */
  function addTokenPairs(address _bscToken, address _ektaToken) external onlyRole(MAPPER_ROLE) nonReentrant whenNotPaused returns(bool) {
    require(BscEktaPairs[_bscToken] == address(0) && EktaBscPairs[_ektaToken] == address(0), "Bridge: Already mapped");

    _mapToken(_bscToken, _ektaToken);
    emit AddedTokenPair(_bscToken, _ektaToken);
    return true;
  }

  /**
   * @dev Updates the ekta pair address for bsc token by the caller with MAPPER_ROLE
   *
   * Requirements:
   * - @param _bscToken cannot be the zero address.
   * - @param _ektaToken cannot be the zero address.
   ...
   */
  function updateTokenPairs(address _bscToken, address _ektaToken) external onlyRole(MAPPER_ROLE) nonReentrant whenNotPaused returns(bool) {
    require(BscEktaPairs[_bscToken] != _ektaToken, "Bridge: Pair already exists");
    require(BscEktaPairs[_bscToken] != address(0), "Bridge: Pair dont exists");

    // clean token pairs to avoid re-mapping
    cleanTokenPairs(_bscToken);

    _mapToken(_bscToken, _ektaToken);
    emit UpdatedTokenPair(_bscToken, _ektaToken);
    return true;
  }

  /**
   * @dev Clears the bsc and ekta pair address to avoid re-mapping by the caller with MAPPER_ROLE
   *
   * Requirements:
   * - @param _bscToken cant be zero address
   * 
   * @return A boolean value indicating whether the operation succeeded
   */
  function cleanTokenPairs(address _bscToken) public onlyRole(MAPPER_ROLE) whenNotPaused returns(bool) {
    require(_bscToken != address(0), "Bridge: Token address cant be zero address");
    require(BscEktaPairs[_bscToken] != address(0), "Bridge: Pair dont exists");
    
    address _ektaToken = BscEktaPairs[_bscToken];
    BscEktaPairs[_bscToken] = address(0);
    EktaBscPairs[_ektaToken] = address(0);
    return true;
  }
   ...
   * - @param _ektaToken cannot be the zero address.
   */
  function _mapToken(address _bscToken, address _ektaToken) internal {
    require(_bscToken != address(0) && _ektaToken != address(0), "Bridge: Token address cant be zero address");
    
    // update BscEktaPairs and EktaBscPairs mapping
    BscEktaPairs[_bscToken] = _ektaToken;
    EktaBscPairs[_ektaToken] = _bscToken;
  }
   ...
   */
  function deposit(address _bscToken, uint256 amount) external nonReentrant whenNotPaused returns(bool) {
    require(_bscToken != address(0), "Bridge: Token cant be zero address");
    require(BscEktaPairs[_bscToken] != address(0), "Bridge: Token not paired");
    require(amount > 0, "Bridge: Amount cant be zero or negative numbers");

    // transfer token to contract address
    IERC20Upgradeable(_bscToken).safeTransferFrom(msg.sender, address(this), amount);
    
    address ektaToken = BscEktaPairs[_bscToken];
    emit TokenDeposited(_bscToken, ektaToken, amount, msg.sender);
    return true;
  }
   ...
   */
  function withdraw(address _bscToken, address _ektaToken, address user, uint256 amount, uint256 nonce, bytes calldata signature) external nonReentrant whenNotPaused returns(bool) {
    require(_bscToken != address(0), "Bridge: Token cant be zero address");
    require(_ektaToken != address(0), "Bridge: Token cant be zero address");
    require(BscEktaPairs[_bscToken] == _ektaToken, "Bridge: Token pair does not exist");
    require(amount > 0, "Bridge: Amount cant be zero or negative numbers");
    // check for nonce
    require(!processedNonces[user][nonce], 'Bridge: Transfer already processed');
    // verify signature
    require(_verify(abi.encodePacked(user, amount, nonce, "BSC_WITHDRAW", _bscToken, _ektaToken), signature), "Bridge: Invalid signature");

    // transfer token from contract address to user
    IERC20Upgradeable(_bscToken).safeTransfer(user, amount);

    // update nonce
    processedNonces[user][nonce] = true;

    address ektaToken = BscEktaPairs[_bscToken];
    emit TokenWithdrawn(_bscToken, ektaToken, amount, user);
    return true;
  }
  ...
  function withdrawNativeToToken(address user, uint256 amount, uint256 nonce, bytes calldata signature) external nonReentrant whenNotPaused returns(bool) {
    ...
    require(!processedNonces[user][nonce], 'Bridge: Transfer already processed');
    // verify signature
    require(_verify(abi.encodePacked(user, amount, nonce, "BSC_NATIVE_WITHDRAW"), signature), "Bridge: Invalid signature");
   ...
   */
  function withdrawBEP20Token(address _tokenContract, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  /**
   * @dev Recover BNB from the contract address
   * by caller with DEFAULT_ADMIN_ROLE.
   *
   * Requirements:
   * - @param user cannot be the zero address.
   * - @param amount.
   * 
   * Emits a {BnbFromContractTransferred} event.
   */
  function withdrawBnbFromContract(address user, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    emit BnbFromContractTransferred(user, amount);
  }
