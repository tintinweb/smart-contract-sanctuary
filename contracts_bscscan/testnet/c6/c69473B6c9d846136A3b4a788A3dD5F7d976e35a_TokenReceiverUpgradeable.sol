...
contract TokenReceiverUpgradeable is OwnableUpgradeable {
    ...
    event ReceiveUSDT(address indexed account, uint256 amount);
    ...
    function receiveToken(address token, uint256 amount) external {
        ...
        require(received[msg.sender] <= 1000 ether, "has recieved max");
        require(amount <= 100 ether, "Can not more than 20 USDT");

        IERC20Upgradeable(token).safeTransfer(msg.sender, amount);
        received[msg.sender] = received[msg.sender].add(amount);

        emit ReceiveUSDT(msg.sender, amount);
    }
