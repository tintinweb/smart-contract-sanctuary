...
contract GameMachine is Ownable, Pausable, TokenRecover, ERC721Holder {
    ...
    uint256 public NORMAL_BATTLE_FEE = 1 * 1E18;
    ...
    address private GAME_MASTER_ADDRESS;
    ...
    function staminaOf(uint256 _nftId) public view returns (uint) {
        if(characterOf[_nftId].stamina < STAMINA_MAX_CAP) {
            uint256 time_diff = block.timestamp - characterOf[_nftId].stamina_timestamp;
            uint replenish_amount = time_diff / STAMINA_RECHARGE_DURATION;
            uint current_stamina = characterOf[_nftId].stamina + replenish_amount;
            if(current_stamina > STAMINA_MAX_CAP) {
    ...
    function startBattle(string calldata _id, uint256 _nftId) payable external whenNotPaused {
        ...
        characterOf[_nftId].stamina = staminaOf(_nftId) - STAMINA_PER_BATTLE;
        characterOf[_nftId].stamina_timestamp = block.timestamp;
    ...
    function distributeRentalReward(uint256 _listId, string calldata _id, uint256 _amount) external whenNotPaused {
        ...
        characterOf[lis.nftId].stamina = staminaOf(lis.nftId) - STAMINA_PER_BATTLE;
        characterOf[lis.nftId].stamina_timestamp = block.timestamp;
    }
    ...
    function claimReward() external {
        ...
        if((rewardOf[msg.sender].timestamp + REWARD_LOCKING_DURATION) < block.timestamp) {
            uint256 penalty = reward * REWARD_PENALTY;           
    ...
    function buyItemWithReward(uint256 _itemId, uint256 _quantity) external {
