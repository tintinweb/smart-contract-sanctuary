...
contract ElpisIGO is Pausable, Ownable {
    ...
    // Info of each address in whitelist
    struct WhiteListInfo {
        uint256 allowances; //the number of nft that an address can buy in the pre-sale=
        bool prepaid; //is this address prepaid?
    }
    ...
    //number of public tokens
    uint256 public immutable nftsPublicSale;
    //the elpis-hero addres
    IElpisHero public immutable elpisHero;
    //amount of nft sold
    uint256 public amountOfNftSold;
    ...
    uint256 public immutable blockFinishSale;
    //List of addresses that can participate in presale
    ...
    constructor(
        ...
        require(
            _blockStartPublicSale > _blockStartPreSale,
            "constructor: invalid public sale time"
        );
        require(
            _blockFinishSale > _blockStartPublicSale,
            "constructor: invalid sale end time"
        );
        ...
        blockFinishSale = _blockFinishSale;
    }

    modifier onlyWhiteList() {
        require(
            whiteList[address(msg.sender)].allowances > 0,
            "ElpisIGO: caller is not whitelisted"
        );
        require(
            block.number >= blockStartPreSale,
            "ElpisIGO: it's not time to open for pre-sale yet"
        );
        require(
            block.number <= blockFinishSale,
            "ElpisIGO: opening times has ended"
        );
        _;
    }
    ...
    modifier onlyOpeningTimes() {
        require(
            ...
            "ElpisIGO: it's not time to open for public sale yet"
        );
        require(
            block.number <= blockFinishSale,
            "ElpisIGO: opening times has ended"
    ...
    modifier validTransaction(uint256 amount) {
        ...
        require(
            ...
            "ElpisIGO: max tokens per transaction number exceeded"
        );
        require(
            amountOfNftSold.add(amount) <= nftsPublicSale,
            "ElpisIGO: tokens number to mint exceeds number of public tokens"
        );
        _;
    }
    ...
    function setWhiteList(
        ...
        bool[] calldata _prepaids
    ) external onlyOwner {
        require(
            _addressWhiteList.length == _allowances.length,
            "setWhiteList: addressWhiteList and whiteListInfo length mismatch"
        );
        require(
            _addressWhiteList.length == _prepaids.length,
            "setWhiteList: addressWhiteList and prepaids length mismatch"
        );
        for (uint256 i = 0; i < _addressWhiteList.length; ++i) {
            whiteList[_addressWhiteList[i]] = WhiteListInfo({
                allowances: _allowances[i],
                prepaid: _prepaids[i]
            });
        }
    }
    ...
    {
        WhiteListInfo storage whiteListInfo = whiteList[address(msg.sender)];
        if (!whiteListInfo.prepaid) {
            require(
                nftPrice.mul(amount) <= msg.value,
                "buyHeroesPreSale: the ether value sent is not enough for payment"
            );
        }
        whiteListInfo.allowances = whiteListInfo.allowances.sub(
            amount,
            "buyHeroesPreSale: the amount of purchase exceeds allowance"
        );
        amountOfNftSold = amountOfNftSold.add(amount);
        for (uint256 i = 0; i < amount; i++) {
            elpisHero.mint(address(msg.sender));
        }
    }
        ...
        validTransaction(amount)
    {
        require(
            nftPrice.mul(amount) <= msg.value,
            "buyHeroesPublicSale: the ether value sent is not enough for payment"
        );
        amountOfNftSold = amountOfNftSold.add(amount);
        for (uint256 i = 0; i < amount; i++) {
            elpisHero.mint(address(msg.sender));
