...
contract PotLuck is Context, Ownable {
    ...
    function distributeFundsToWinners(address address1, address address2, uint256 amount1, uint256 amount2) internal {
        (bool success, ) = payable(address1).call{value: amount1}("");
        (success, ) = payable(address2).call{value: amount2}("");
        require(success == true);
    }
    function takeFee() internal {
        uint256 ownerFee = address(this).balance.div(100).div(2);
        (bool success, ) = payable(adminWallet).call{value: ownerFee}("");
    ...
    function pickWinner() public {
        ...
        distributeFundsToWinners(tickets[winner1Index].player, tickets[winner2Index].player, winner1Amount, winner2Amount);
