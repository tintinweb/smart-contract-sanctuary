...
contract TOKEN2NFT is  IERC721Receiver, Ownable {
    ...
    function stakeTokens(uint256 _multiple) public {
        ...
        require (userStaked[msg.sender] == 0, "Already Staked");
         ...
         require (maxInvestment >= totalstaked + stkamount, "maxInvestment reached");
         totalstaked = totalstaked.add(stkamount);
        
         uint256 platformFee = stkamount.mul(fees).div(100).div(100);
    ...
    function insertNFT(uint256[] calldata ids) external onlyOwner {
     ...
     function initialize(address _stakingtoken, address _nftredeemaddress, uint256 _claimtime, uint256 _redeemstarting, uint256 _redeemend, uint256 _stakingamount, uint256 _maxInvestment) public onlyOwner {
