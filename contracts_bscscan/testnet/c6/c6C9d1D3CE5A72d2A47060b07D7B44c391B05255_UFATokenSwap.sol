...
contract UFATokenSwap is Ownable {
    ...
    uint256 public min = 1515151515151515100; // 50 UFA
    uint256 public max = 15151515151515152000000; // 500000 UFA
    
    constructor(address _tokenA, address _tokenB
    ) {
        tokenA = IBEP20(_tokenA);  // BUSD
        tokenB = IBEP20(_tokenB);  // UFA
    }

    function swap(uint256 _amount, address payable _refaddr) public {
        require(_amount >= min, "amount too small"); 
        require(_amount <= max, "amount too ever"); 
        uint256 _payback = _amount.mul(rate);
        uint256 _payref = _payback.mul(refrate).div(100);
