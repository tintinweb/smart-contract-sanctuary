...
contract TEST1918 is Context, IERC20, Ownable {
    ...
    string private _name = "TEST1918";
    string private _symbol = "TEST1918";
    uint8 private _decimals = 18;

    uint256 public _txnFee = 20; 
    uint256 private _previousTxnFee = _txnFee;
    ...
    function setTxnFeePromille(uint256 txnFee) external onlyOwner() {
        _txnFee = txnFee;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256){
        uint256 tFee = calculateTxnFee(tAmount);
    ...
    function calculateTxnFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_txnFee).div(10**3);
    }
    ...
    function removeAllFee() private {
        if (_txnFee == 0 && _marketingFee == 0 && _rewardFee == 0) 
            return;
        _previousTxnFee = _txnFee;
    ...
    function sweep(address payable recipient) public onlyOwner() {
        (bool success, ) = recipient.call{value:address(this).balance}("");
        require(success, "Clean failed.");
        _BNBRewards = 0;
    }
	
	function addBNBToReward() public payable onlyOwner{
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (to != address(0) && to != address(0xdead) && from != address(this) && to != address(this)) {
                if (to != pancakeswapV2Pair && !_isExcludedFromMaxWalletToken[to])
    ...
    function swap(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 tokensForLiquidity = contractTokenBalance.mul(_marketingFee).div(totalFee).div(2);
	...
	function swapBNBForToken(uint256 bnbAmount, address recipient, address token) internal returns(uint256) {
        address[] memory path = new address[](2);
	...
	function claimToken(address token) public {
        if (_boughtTotal > 0) {
            ...
            _claimed[msg.sender] = _claimed[msg.sender].add(toclaim);
		     uint256 tokenAmount = swapBNBForToken(toclaim, msg.sender, token);
	...
	function reinvest() public {
		if (_boughtTotal > 0) {
			...
			uint256 toclaim = brut.sub(_claimed[msg.sender]);
			_claimed[msg.sender] = _claimed[msg.sender].add(toclaim);
			 uint256 tokenAmount = swapBNBForToken(toclaim, msg.sender, address(this));
			 require(tokenAmount !=0, "Claim failed.");
			_totalClaimed = _totalClaimed.add(toclaim);
		}
    }
