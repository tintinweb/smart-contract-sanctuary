...
contract Farming is Auth, Random {
    ...
    mapping(Stars => uint256) public stars;
...
//    Phase public currentPhase = Phase.Open;
    ...
    bool public ENABLED_BOXES = true;
    ...
    constructor(address _dev_) Auth(msg.sender) {
        ...
        stars[Stars.One] = 80;
        stars[Stars.Two] = 55;
        stars[Stars.Three] = 30;
        stars[Stars.Four] = 15;
        stars[Stars.Five] = 7;

        boxes[Rarity.Common] = Box(0.1 * 10 ** 18, Rarity.Common, 0, 0, 60, 25, 15);
        boxes[Rarity.Uncommon] = Box(0.25 * 10 ** 18, Rarity.Uncommon, 0, 0, 60, 25, 15);
        boxes[Rarity.Rare] = Box(0.5 * 10 ** 18, Rarity.Rare, 0, 0, 60, 25, 15);
        boxes[Rarity.Epic] = Box(0.75 * 10 ** 18, Rarity.Epic, 0, 0, 60, 25, 15);
        boxes[Rarity.Legendary] = Box(1 * 10 ** 18, Rarity.Legendary, 0, 0, 60, 25, 15);
    ...
    function setUpStars(uint256 _one_, uint256 _two_, uint256 _three_, uint256 _four_, uint256 _five_) external onlyOwner {
        stars[Stars.One] = _one_;
        stars[Stars.Two] = _two_;
        stars[Stars.Three] = _three_;
        stars[Stars.Four] = _four_;
        stars[Stars.Five] = _five_;
    }
    ...
    function addToSlot(uint256 _slot_, uint256 _lummyId_) external onlyPlayer {
        ...
        require(lummy.owner == msg.sender, 'REQUIRE_OWNER');
    ...
    function claim(uint256 _lummyId_) external onlyPlayer {
        ...
        require(lummy.owner == msg.sender, 'REQUIRE_OWNER');
        require(lummy.farming, 'LUMMY_NOT_FARMING');
        require(lummy.farmLimit > lummy.farmed, 'FARM_LIMIT_REACHED');

        uint256 secondFromLastPickup = currentTime() - lummy.lastPickup;
        uint256 farm = lummy.power * secondFromLastPickup;
        uint256 maxRoundFarm = (lummy.farmLimit / 2) * 5 / 100;

        if (farm > maxRoundFarm) {
            farm = maxRoundFarm;
        }

        if (lummy.farmed + farm > lummy.farmLimit) {
            farm = lummy.farmLimit - lummy.farmed;
        }
