...
// MasterChefCenDistr is the master of Centaurus (Cen) distribution. He can make Cen and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. Initially the ownership is
// transferred to TimeLock.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract MasterChefCenDistr is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // Initial emission rate: 0.0694 Centaurus per block.
    uint256 public constant INITIAL_EMISSION_RATE = 69444444444444444 wei;
	
    // Initial harvest time: 5 days.
    uint256 public constant INITIAL_HARVEST_TIME = 5*28;	
    ...
    constructor(
        ...
        cen = _cen;
        cenPerBlock = INITIAL_EMISSION_RATE;
    ...
    function pendingCen(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 cenReward = multiplier.mul(cenPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accCenPerShare = accCenPerShare.add(
                cenReward.mul(1e12).div(lpSupply)
            );
        }
        uint256 pending = user.amount.mul(accCenPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
	    ...
	    // Pay or lockup pending Centauruss.
    function payOrLockupPendingCen(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (block.number >= startBlockHarvest && block.number <= lastBlockHarvest) {
            if (pending > 0 || user.rewardLockedUp > 0) {        
                ...
                // send rewards
                safeCenTransfer(msg.sender, totalRewards);
            }
        } else if (pending > 0) {
