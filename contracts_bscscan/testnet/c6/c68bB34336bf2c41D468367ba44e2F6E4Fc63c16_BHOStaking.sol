...
contract BHOStaking is OwnableUpgradeable {
    ...
    StakingPackage[] public stakingPackages;
    mapping (address => UserInfo) public usersInfo;

    // Emergency unstake fee
    uint256 public unstakeFee = 75; // 25%
    ...
    function initialize(IERC20 _bho) public initializer {
        bho = _bho;
        stakingPackages.push(StakingPackage("Basic", 7, 10));
        stakingPackages.push(StakingPackage("Vip", 21, 30));
        __Ownable_init();
    }
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < _stakingPackages.length; i++) {
            require(
                _stakingPackages[i].apy <= 100,
                "APY must be less than 100"
            );
            stakingPackages.push(StakingPackage(
                _stakingPackages[i].id,
                _stakingPackages[i].numberOfDay,
    ...
    function stakeBHO(uint256 _amount, uint256 _pid) external returns (uint256 amount, uint256 duration) {
        ...
        userInfo.stakedAt = stakedAt;
        userInfo.lastAmoumt = lastAmount;
        userInfo.lastStakedAt = lastStakedAt;
    }
    ...
    function pendingBHO(address _user) external returns (uint256) {
        _pendingBHO(_user);
    }

    /******************************************* Internal function below *******************************************/
    function _pendingBHO(address _user) internal returns (uint256) {
        uint256 pending = 10;
        return pending;
    }
}
