...
contract AlmightyCoin is Context, IBEP20, Ownable {
    ...
    string private _name = "Almighty coin";
    string private _symbol = "ALMC";
    ...
    function setBurnFeePercent(uint256 marketeFee) external onlyOwner() {
        _burnFee = marketeFee;
    }
    ...
    function _takeBurnFee(uint256 tAmount) private {
		...
		_rOwned[burnWallet] = _rOwned[burnWallet].add(rBurnFee);
        if(_isExcluded[burnWallet])
            _tOwned[burnWallet] = _tOwned[burnWallet].add(tBurnFee);
        emit Transfer(_msgSender(), burnWallet, tBurnFee);
    }
