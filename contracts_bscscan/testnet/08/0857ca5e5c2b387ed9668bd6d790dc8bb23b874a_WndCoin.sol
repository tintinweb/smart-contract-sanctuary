...
contract WndCoin is IGP, ERC20, Ownable {
  ...
  function addTown(address addr) external onlyOwner{
      admins[addr] = true;
  }

  function addWnd(address addr) external onlyOwner{
      admins[addr] = true;
  }
  function reNameCoin(string calldata name_,string calldata symbol_) external onlyOwner{
      _reNameErc20(name_,symbol_);
  }
