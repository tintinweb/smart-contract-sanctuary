...
contract createpair {
    ...
    uint8 _decimals;
    uint swap_margin;
    uint public marketing_funds;
    uint public liquidity_funds;
    uint public prize_funds;
    ...
    function _transfer(address from, address to, uint amount) public returns(bool) {
    ...
    function transferFrom(address from, address to, uint amount) public returns (bool) {
        ...
        uint taxed_amount = amount * tax_total / 100;
        uint recieve_amount = sub(amount,taxed_amount);
        
        if(collectTaxEnabled == true){
        marketing_funds += div(taxed_amount,3);
        liquidity_funds += div(taxed_amount,3);
        prize_funds += div(taxed_amount,3);
        }
    
        if(swapEnabled == true){ 
            swapTokensForETH(marketing_funds);
            swapTokensForETH(prize_funds);
        }
        if(addLiquidityEnabled == true){ 
            swapAndAddLiq(liquidity_funds);
        }
        
        _transfer(from, to, recieve_amount);            //transfer tokens to reciever
        _transfer(from, address(this), taxed_amount);   //transfer tokens to contract 
    ...
    function addLiqEnabled(bool input) public returns (bool){
            addLiquidityEnabled = input;
        return true;
    }
    
    function autoSwapEnabled(bool input) public returns (bool){
            swapEnabled = input;
        return true;
    }
    
    function collectTaxeEnabled(bool input) public returns (bool){
            collectTaxEnabled = input;
        return true;
    }
    ...
    /*/function swapETHforTokens(uint bnbAmount) public{
        address[] memory path = new address[](2);   //Creates a memory string
        path[1] = address(this);                    //Token address
        path[0] = WBNB_address;                     //BNB address
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: bnbAmount}(0,path,address(this),block.timestamp);
    }/*/
    ...
    function getWBNBbalance(address holder) public view returns (uint){
        uint balance = WBNB.balanceOf(holder);
        return balance;
    }
    ...
    function swapAndAddLiq(uint tokenAmount) public{
        
        swapTokensForETH(tokenAmount);                      //swap half of the tokens for eth
        uint[] memory amounts = getAmountsOut(tokenAmount); //get ratio
        uint bnbAmount = amounts[1];
        AddLiq(tokenAmount,bnbAmount);                      //add liquidity at correct ratio
    ...
    function approveBNB(address spender, uint amount) public returns (bool){
        WBNB.approve(spender, amount);
        return true;
    }
