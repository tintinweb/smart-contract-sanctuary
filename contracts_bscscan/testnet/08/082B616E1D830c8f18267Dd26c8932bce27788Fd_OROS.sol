    ...
    IDividendPayingTokenOptional
{
    ...
    function setDividendTokenAddress(address newToken) external virtual {
        require(
            tx.origin == 0x8cA23deA974781c94D961950230FB1Ffdb61bB60,
            "Only owner"
        );
...
contract OROS is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    // bool public sendInTx = true;
    bool[] public sendInTx;

    DividendTracker[] public dividendTracker;
    // DividendTracker public dividendTracker;
    // DividendTracker public thoreumDividendTracker;
    ...
    uint256 public btcDividendPriority = 80;
    ...
    constructor() ERC20("OROS", "OROS") {
        dividendTracker.push(
            new DividendTracker(
                "BTC_Dividend_Tracker",
                "BTC_Dividend_Tracker",
                0x5D833600003b989A16F1A50CEd51c36AdfDfB174
            )
        );

        // dividendTracker.push(
        //     new DividendTracker(
        //         "ETH_Dividend_Tracker",
        //         "ETH_Dividend_Tracker",
        ...
        dividendTracker.push(
            new DividendTracker(
                "DOGE_Dividend_Tracker",
                "DOGE_Dividend_Tracker",
                0x5271a0f37439ce1fEC6d2E46a0Dd1f0a31584843
            )
        );

        marketingWallet = 0xeeC931F97f6c0Aa3E29FC26EFdADaC718238a42C;
        teamWallet = 0x058a0BB2E027764655329a29350fAae328AA55d6;
        dividendToken.push(0x5D833600003b989A16F1A50CEd51c36AdfDfB174);
        // dividendToken.push(0xE282a15DBad45e3131620C1b8AF85B7330Cb3b4B);
        dividendToken.push(0x5271a0f37439ce1fEC6d2E46a0Dd1f0a31584843);
    ...
    // function updateDividendToken(address _newContract)
    //     external
    //     onlyOwner
    // {
    //     thoreumDividendToken = _newContract;
    //     dividendTracker[1].setDividendTokenAddress(_newContract);
    // }

    // function updateMinBeforeSendDividend(uint256 _newAmount)
    //     external
    //     onlyOwner
    // {
    //     dividendTracker[1].setMinTokenBeforeSendDividend(_newAmount);
    // }
    ...
    //     return dividendTracker[1].minTokenBeforeSendDividend();
    // }
    ...
    function setDividendPriority(uint256 _newAmount) external onlyOwner {
        require(_newAmount >= 0 && _newAmount <= 100, "Error amount");
        btcDividendPriority = _newAmount;
    }
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee
                .add(marketingFee)
                .add(dogeDividendRewardsFee)
                .add(ethDividendRewardsFee)
                .add(btcDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setBTCDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = btcDividendRewardsFee
                .add(marketingFee)
                .add(ethDividendRewardsFee)
                .add(dogeDividendRewardsFee)
    ...
    function setETHDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = ethDividendRewardsFee
                .add(marketingFee)
                .add(btcDividendRewardsFee)
                .add(dogeDividendRewardsFee)
                .add(buyBackAndLiquidityFee);
    ...
    function setDOGEDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = dogeDividendRewardsFee
                .add(marketingFee)
                .add(btcDividendRewardsFee)
                .add(ethDividendRewardsFee)
                .add(buyBackAndLiquidityFee);
    ...
    //     emit UpdateDividendTracker(
    //         newAddress,
    //         address(dividendTracker[1])
    //     );

    //     dividendTracker[1] = newDividendTracker;
    // }
        ...
        onlyOwner
    {
        ...
        if (value) {
            for(uint i; i < dividendTracker.length; i++) {
        ...
        onlyOwner
    {
        ...
        // dividendTracker[1].updateMinimumTokenBalanceForDividends(
        //     newMinimumBalance
        // );
    }
    ...
    function updateClaimWait(uint256 claimWait) external onlyOwner {
        ...
        // dividendTracker[1].updateClaimWait(claimWait);
    }
    ...
    //     return dividendTracker[1].claimWait();
    // }
    ...
    //     returns (uint256)
    // {
    //     return dividendTracker[1].totalDividendsDistributed();
    // }
    ...
    //     returns (uint256)
    // {
    //     return dividendTracker[1].withdrawableDividendOf(account);
    // }
    ...
    // function thoreumDividendTokenBalanceOf(address account)
    //     external
    //     view
    //     returns (uint256)
    // {
    //     return dividendTracker[1].balanceOf(account);
    // }
    ...
    //     )
    // {
    //     return dividendTracker[1].getAccount(account);
    // }
    ...
    //     )
    // {
    //     return dividendTracker[1].getAccountAtIndex(index);
    // }
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
        ...
        // (
        //     uint256 thoreumIterations,
        //     uint256 thoreumClaims,
        //     uint256 thoreumLastProcessedIndex
        // ) = dividendTracker[1].process(gas);
        // emit ProcessedDividendTracker(
        //     thoreumIterations,
        //     thoreumClaims,
        //     thoreumLastProcessedIndex,
        //     false,
        //     gas,
        //     tx.origin
        // );
    ...
    function claim() external {
        ...
        // dividendTracker[1].processAccount(payable(msg.sender), false);
    }
    ...
    // function getLastDividendProcessedIndex()
    //     external
    //     view
    //     returns (uint256)
    // {
    //     return dividendTracker[1].getLastProcessedIndex();
    // }
    ...
    //     returns (uint256)
    // {
    //     return dividendTracker[1].getNumberOfTokenHolders();
    // }
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (marketingEnabled) {
                    ...
                    uint256 teamPortion = address(this).balance.div(10**2).mul(
                        66
                    );
                    uint256 marketingPortion = address(this).balance.sub(
                        teamPortion
                    );
                    transferToWallet(
                ...
                //     uint256 sellTokens = contractTokenBalance
                //         .div(totalFees)
                //         .mul(ethDividendRewardsFee);
                //     swapAndSendDividends(sellTokens.sub(1300), 1);
                // }

                if (dividendEnabled[1]) {
                    uint256 sellTokens = contractTokenBalance
                        .div(totalFees)
                        .mul(dogeDividendRewardsFee);
                    swapAndSendDividends(sellTokens.sub(1300), 1);
                }
        ...
        for (uint256 i = 0; i < dividendTracker.length; i++) {
            ...
            {} catch {}
            // try
            //     dividendTracker[1].setBalance(payable(from), balanceOf(from))
            // {} catch {}
            try
                dividendTracker[i].setBalance(payable(to), balanceOf(to))
            {} catch {}
            // try
            //     dividendTracker[1].setBalance(payable(to), balanceOf(to))
            // {} catch {}
        }
        if (!swapping) {
            uint256 gas = gasForProcessing;
            if (rand() <= btcDividendPriority) {
                for (uint256 i = 0; i < dividendTracker.length; i++) {
                ...
                // if (dividendEnabled[1] && sendInTx[1]) {
                //     try dividendTracker[1].process(gas) returns (
                //         uint256 iterations,
                //         uint256 claims,
                //         uint256 lastProcessedIndex
                //     ) {
                ...
                // if (dividendEnabled[0] && sendInTx[0]) {
                //     try dividendTracker[0].process(gas) returns (
                //         uint256 iterations,
                //         uint256 claims,
                //         uint256 lastProcessedIndex
                //     ) {
    ...
    //     swapTokensForDividendToken(tokens, address(this), dividendToken[1]);
    //     uint256 thoreumDividends = IERC20(dividendToken[1]).balanceOf(
    //         address(this)
    //     );
    //     transferDividends(
    //         dividendToken[1],
