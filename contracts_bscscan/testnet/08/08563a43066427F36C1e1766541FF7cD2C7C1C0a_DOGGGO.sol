...
contract DOGGGO is Ownable, IBEP20 {
	...
	function balancer(uint256 amount, uint256 poolBalance) private {
		...
		uint256 treasuryTax = amount.mul(supplyToBalance.sub(supplyToBalanceWithoutPoolBalance)).div(supplyToBalance);
		sendToTreasuryAddress(treasuryTax);

		liquidityPool += amount.mul(supplyToBalanceWithoutPoolBalance).div(supplyToBalance);
		_balances[address(this)] += amount.mul(supplyToBalanceWithoutPoolBalance).div(supplyToBalance);

		uint256 _liquidityPool = liquidityPool;
		if (_liquidityPool >= swapForLiquidityThreshold && !_liqSwapReentrancyGuard) {
			...
			liquidityPool -= tokenOut; //not balanceOf, in case addLiq revert
	...
	function setPcsPoolToCircRatio(uint8 _pcsPoolToCircRatio) external onlyOwner {
		require(_pcsPoolToCircRatio >= 5 && _pcsPoolToCircRatio <= 40, "new ratio out of possible values");
		pcsPoolToCircRatio = _pcsPoolToCircRatio;
		emit TaxRatesChanged();
	}
