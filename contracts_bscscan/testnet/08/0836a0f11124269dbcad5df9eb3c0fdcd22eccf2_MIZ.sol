...
contract MIZ is ERC20, Ownable {
    ...
    event DevwalltUpdated(address indexed newDevwallt, address indexed oldDevwallt);
    ...
    constructor() ERC20("MIZ", "HTC") {
        ...
        */
        _create(owner(), 1000000000000 * (10**18));
    }
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "MIZ: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
        emit MarketingWalletUpdated(marketingWallet, _newWallet);
  	    marketingWallet = _newWallet;
  	}
  	function updateDevWalet(address _newWallet) external onlyOwner {
  	    require(_newWallet != Devwallt, "MIZ: The Dev wallet is already this address");
        excludeFromFees(_newWallet, true);
        emit DevwalltUpdated(Devwallt, _newWallet);
  	    Devwallt = _newWallet;
  	}
    ...
    function updateShibDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(shibDividendTracker), "MIZ: The dividend tracker already has that address");

        ShibDividendTracker newShibDividendTracker = ShibDividendTracker(payable(newAddress));

        require(newShibDividendTracker.owner() == address(this), "MIZ: The new dividend tracker must be owned by the MIZ token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "MIZ: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "MIZ: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MIZ: The PanshibSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "MIZ: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "MIZ: Cannot update gasForProcessing to same value");
    ...
    function getAccountShibDividendsInfo(address account)
 	...
 	function getAccountShibDividendsInfoAtIndex(uint256 index)
  	...
  	function processDividendTracker(uint256 gas) external onlyOwner {
		(uint256 ethIterations, uint256 ethClaims, uint256 ethLastProcessedIndex) = shibDividendTracker.process(gas);
		emit ProcessedShibDividendTracker(ethIterations, ethClaims, ethLastProcessedIndex, false, gas, tx.origin);
    ...
    function _transfer(
        ...
        require(SwapAndLiquifyIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "MIZ: SwapAndLiquify has not started yet");
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (marketingEnabled) {
                    ...
                    uint256 DevwalltPortion = address(this).balance.div(10**2).mul(17);
                    uint256 marketingPortion = address(this).balance.sub(DevwalltPortion);
                    transferToWallet(payable(marketingWallet), marketingPortion);
                    transferToWallet(payable(Devwallt), DevwalltPortion);
                }
...
contract ShibDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MIZ_Shibereum_Dividend_Tracker", "MIZ_Shibereum_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "MIZ_Shibereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "MIZ_Shibereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MIZ contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "MIZ_Shibereum_Dividend_Tracker: Cannot update claimWait to same value");
