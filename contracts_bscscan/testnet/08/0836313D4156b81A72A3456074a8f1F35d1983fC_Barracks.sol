...
//        zombie = _zombie;
    ...
    function deposit(address _token, uint _barrackID, uint _amount) public payable {
        ...
        if (barracks[_barrackID].bnb) {
            ...
            barracks[_barrackID].totalDeposited += msg.value; // total deposited including fee.
            barracks[_barrackID].deposits[msg.sender].amount += depositedAmount; // deposited by user minus the fee.
            ...
            barracks[_barrackID].totalDeposited += _amount; // total deposited including fee.
            barracks[_barrackID].deposits[msg.sender].amount += depositedAmount; // deposited by user minus the fee
    ...
    // no nft in case of early refunding.
    function refundEarlier(uint _barrackID) public returns (bool) {
        require(!barracks[_barrackID].locked, "Barrack is locked");
        require(barracks[_barrackID].active, "Barrack is not active");
        require(barracks[_barrackID].deposits[msg.sender].amount > 0, "Not deposited yet.");
        if (barracks[_barrackID].bnb) {
            ...
            IBEP20(barracks[_barrackID].token).transfer(msg.sender, barracks[_barrackID].deposits[msg.sender].amount);
        }
        // subtracting the amount from total deposited.
        barracks[_barrackID].totalDeposited -= barracks[_barrackID].deposits[msg.sender].amount;
        // resetting the deposited amount of user
        barracks[_barrackID].deposits[msg.sender].amount = 0;
        return true;
    }
