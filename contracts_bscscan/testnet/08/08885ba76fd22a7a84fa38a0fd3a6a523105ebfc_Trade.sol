...
contract Trade {
    ...
    event CharityAddress(address indexed charityAddress);
    ...
    function getFees(uint paymentAmt, BuyingAssetType buyingAssetType, address buyingAssetAddress, uint tokenId, uint _charityFee) internal returns(Fee memory){
        ...
        uint charityFee = price.mul(_charityFee).div(1000);
        ...
        assetFee = price.sub(royaltyFee).sub(sellerFee).sub(charityFee);
