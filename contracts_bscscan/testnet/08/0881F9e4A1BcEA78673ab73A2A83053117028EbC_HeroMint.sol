...
contract HeroMint is Ownable {
    ...
    event Compose(bytes32 hash);
    constructor(address _funAddress){
        funAddress = _funAddress;
    }

    function compose(Data memory data) public {
        require(block.timestamp <= data.timestamp,"already expired");
        require(msg.sender == data.to,"to address is not owner");
        require(!hashList[data.hash],"the hash has used");
        address _funAddress = ecrecover(data.hash, data.v, data.r, data.s);
        require(funAddress == _funAddress,"data is wrong");
        
        string memory signDataStr = string(abi.encodePacked("Cybertron",
        string(abi.encodePacked("0x",addressToStr(data.heroAddress))),
        string(abi.encodePacked("0x",addressToStr(data.to))),
        data.name,
        uint2str(data.level),
        uint2str(data.rarity),
        uint2str(data.series),
        uint2str(data.race),
        uint2str(data.mineralLevel),
        uint2str(data.timestamp),
        string(abi.encodePacked("0x",addressToStr(funAddress)))));

        bytes32 signData  =keccak256(abi.encodePacked(signDataStr));

        require(signData == data.hash,"data is wrong");

        INFT(data.heroAddress).mint(data.to,data.name,data.level,data.rarity,data.series,data.race,data.mineralLevel);
        emit Compose(data.hash);
    }

    function getStr(Data memory data) public view returns(string memory){
        string memory signDataStr = string(abi.encodePacked("Cybertron",
        string(abi.encodePacked("0x",addressToStr(data.heroAddress))),
        string(abi.encodePacked("0x",addressToStr(data.to))),
        data.name,
    ...
    function getHash(Data memory data) public view returns(bytes32){
        string memory signDataStr = string(abi.encodePacked("Cybertron",
        string(abi.encodePacked("0x",addressToStr(data.heroAddress))),
        string(abi.encodePacked("0x",addressToStr(data.to))),
        data.name,
