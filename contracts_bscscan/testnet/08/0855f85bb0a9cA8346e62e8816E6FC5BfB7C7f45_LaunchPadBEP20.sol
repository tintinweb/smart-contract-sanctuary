...
contract LaunchPadBEP20 is Ownable, Initializable {
    // Contract libs
    ...
    /**
     * @dev Buy launch pad token with native token BNB.
     */
    ...
    function setlaunchPadTGE(uint256 _TGEDate) public onlyOwner {
        ...
        uint256 joinedCounter = vestingInfoAddresses.length;

        for (uint256 i = 0; i < joinedCounter; i++) {
            launchPadVestingInfo storage info = vestingInfo[
                vestingInfoAddresses[i]
            ];
            info.startDate = _TGEDate.add(info.lockDuration);
        }
    }
    ...
    function buylaunchPadTokens() public {
        ...
        // // launch pad has ended
        // require(
        //     launchPadTGE == 0 || launchPadTGE > block.timestamp,
        //     "LaunchPad: launch pad TGE has not set or has ended."
        // );

        // check lock pool conditions
        require(
            lockPool.validWhitelist(_msgSender()),
        ...
        require(
            vestingInfo[_msgSender()].bnbAmount == 0,
        ...
        uint256 startDate = launchPadTGE > 0
            ? launchPadTGE.add(launchPadLockDuration)
            : 0;
        ...
        returns (uint256, uint256)
    {
        ...
        if (info.vestDuration == 0) {
            return (info.amount, info.startDate);
        }
        ...
        // Vesting is done
        if (block.timestamp >= info.startDate.add(info.vestDuration)) {
            return (info.amount, info.startDate.add(info.vestDuration));
        }
    ...
    /**
     * @dev Set remaining tokens to contract owner with the same vesting schedule
     */
    function claimRemainingTokens() public onlyOwner {
        require(
            block.timestamp > launchPadTGE,
            "Token sale has not finished yet"
        );
        uint256 remainingBnb = maxCapAmount.sub(currentCapAmount);
        require(remainingBnb > 0, "No remaining token");
        uint256 remainingToken = remainingBnb.mul(outputTokenAmount).div(
            inputTokenAmount
        );

        // add launch pad information
        uint256 upfrontAmount = remainingToken.mul(launchPadUpfrontPercent).div(
            100
        );

        uint256 leftOverAmount = remainingToken.sub(upfrontAmount);
        ...
        vestingInfo[_msgSender()] = launchPadVestingInfo({
            ...
            released: upfrontAmount,
            upfrontAmount: upfrontAmount,
            startDate: launchPadTGE.add(launchPadLockDuration),
            vestInterval: launchPadVestInterval,
            lastReleasedAt: launchPadVestInterval,
            upfrontAmountClaimed: true
        });

        vestingInfoAddresses.push(_msgSender());
        if (upfrontAmount > 0) {
            // transfer upfront amount
            launchPadToken.safeTransfer(_msgSender(), upfrontAmount);
            // imit event
            emit LaunchPadTokenClaimed(_msgSender(), upfrontAmount);
        }
    }
}
...
interface ILockPool {
    function validWhitelist(address _stakeholder) external view returns (bool);
}
