...
interface IPolkaMonsterFT is IERC20 {
    function ownerOf(uint256 tokenId) external view returns (address);
    function isEvolved(uint256 tokenId) external view returns (bool);
    function safeTransferFrom(address from, address to, uint256 tokenId) external view;
    function evolved(uint256 _tokenId) external view returns (bool);
}

contract PolkaMarketPlace is 
    Initializable,
    ...
    IPolkaMonsterFT public monerNFT;
    ...
    function initialize(
       address _manager,
        ...
        monerNFT = IPolkaMonsterFT(_monerNFT);
    }
    ...
    function placeOrder(uint256 _tokenId, uint256 _price, address _nftAddress) payable public {
        require(monerNFT.ownerOf(_tokenId) == _msgSender(), "not own");
        require(_price > 0, "nothing is free");
        require(monerNFT.evolved(_tokenId), "require: evolved");
        ...
        if (tokenSales.contains(_tokenId)){
            if (sellerTokens[_msgSender()].contains(_tokenId)) {
                transferAndStore(_tokenId, _price, _nftAddress);
            }
            else{
    ...
    function transferAndStore(uint256 _tokenId, uint256 _price, address _nftAddress) internal {
        address(_nftAddress).call(
            abi.encodeWithSignature("transferFrom(address,address,uint256)", _msgSender(), address(this), _tokenId)
        );
    ...
    function cancelOrder(uint256 _tokenId, address _nftAddress) public {
        ItemSale storage itemSale = markets[_tokenId];
        require(itemSale.owner == _msgSender(), "not own");
        address(_nftAddress).call(
            abi.encodeWithSignature("transferFrom(address,address,uint256)", address(this), itemSale.owner, _tokenId)
        );
    ...
    function fillOrder(uint256 _tokenId, address _nftAddress) public {
        ...
        address(_nftAddress).call(
            abi.encodeWithSignature("transferFrom(address,address,uint256)", address(this), _msgSender(), _tokenId)
        );
        tokenSales.remove(_tokenId);
        sellerTokens[itemSale.owner].remove(_tokenId);
        markets[_tokenId] = ItemSale({
    ...
    function tokenOrder(
        ...
        if (_sell) {
            ...
            // monerNFT.transferFrom(address(this), _msgSender(), _tokenId);
            address(_nftAddress).call(
                abi.encodeWithSignature("transferFrom(address,address,uint256)", address(this), _msgSender(), _tokenId)
            );
            tokenSales.remove(_tokenId);
    ...
    function getSale(uint256 _tokenId) public view returns (ItemSale memory) {
        ...
        return ItemSale({tokenId: 0, owner: address(0), price: 0, orderBlockTime: 0, nftAddress: address(0)});
    }
