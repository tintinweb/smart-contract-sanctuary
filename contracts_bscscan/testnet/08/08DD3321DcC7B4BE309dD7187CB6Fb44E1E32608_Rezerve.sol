...
contract Rezerve is Context, IERC20, Ownable {
    ...
    bool public _stakingTaxToggle = true;
   ...
   function toggleStakingTax () public onlyOwner {
       ...
       _stakingTaxToggle = !_stakingTaxToggle; 
   }

    function swapAndLiquify(uint256 contractTokenBalance) public lockTheSwap {
       uint _forexchange = contractTokenBalance;
       if ( _stakingTaxToggle ) {
           _forexchange = (contractTokenBalance.mul(4)).div(5); 
           uint _forstaking  = (contractTokenBalance.mul(1)).div(5); 
           if ( _forstaking >0 ) transfer( reserveStaking, _forstaking );
       }
          swapTokensForDai(_forexchange); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
    }
