...
contract RandomNumberGenerator is VRFConsumerBase, IDeHubRand, Ownable {
    ...
    function getRandomNumber() external override returns (bytes32) {
        ...
        // uint256 seed = uint256(keccak256(abi.encode(block.timestamp, blockhash(block.number), _seed))); // Hash user seed and blockhash
        uint256 seed = uint256(keccak256(abi.encode(block.timestamp, blockhash(block.number)))); // Hash user 
        latestRequestId = requestRandomness(keyHash, fee, seed);
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        latestId = IDeHubRandConsumer(msg.sender).viewCurrentTaskId();
