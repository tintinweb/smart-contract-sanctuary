...
contract TestZilla is Context, IERC20, Ownable {
    ...
    string private _name = "TestZilla";
    string private _symbol = "TestZilla";
    ...
    uint256 public maxWallet = 20 * _tTotal / 1000;          // 2.0% of total supply, 138e19 tokens
    ...
    bool private inBurn;
    bool private accumulatingForBurn;
    uint256 burnAmount;
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_totalLPFee).div(
            10**2
        );
    }
    ...
    function setFees(uint256 marketingFee, uint256 liquidityFee, uint256 taxFee) public onlyOwner{
        require(0 <= taxFee && taxFee <= 5, "Requested tax fee out of acceptable range.");
        require(0 <= liquidityFee && liquidityFee <= 5 , "Requested liquidity fee out of acceptable range.");
        require(0 <= marketingFee && marketingFee <= 5, "Requested marketing fee out of acceptable range.");
        require(0 < marketingFee + liquidityFee, "Total fee amount must be strictly positive.");
    ...
    function shouldBurn() private view returns (bool){
        ...
        return tradingIsEnabled && canBurn && !inBurn &&
        accumulatingForBurn && !antiBotActive;
    }
    ...
    function swapTokens(uint256 tokens) inSwap private {
        ...
        uint256 LPtokens = tokens.mul(_liquidityFee).div(_totalLPFee);
        uint256 halfLPTokens = LPtokens.div(2);
        uint256 marketingtokens = tokens.sub(LPtokens);
        ...
        uint256 bnbForLP = newBalance.mul(_liquidityFee).div(_totalLPFee).div(2).
        div( 1e3 - 1e3 * _liquidityFee.div(_totalLPFee).div(2) ).mul(1e3);
    ...
    function planBurn(uint256 _burnPercentage) public onlyOwner{
        burnAmount = _tTotal * _burnPercentage / 1000;
        accumulatingForBurn = true;
    }
    
    function buybackStuckBNB(uint256 percent) public onlyOwner {
        uint256 amountToBuyBack = address(this).balance * percent / 1e6;
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
