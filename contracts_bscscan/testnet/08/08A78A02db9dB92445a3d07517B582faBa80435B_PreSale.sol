...
contract PreSale is ReentrancyGuard, Context, Ownable {
    ...
    function showbuyprocess1() public nonReentrant icoActive payable returns(uint256) {

        uint256 weiAmount = msg.value;
        return weiAmount;
    }
    function showbuyprocess2() public nonReentrant icoActive payable returns(uint256) {
    ...
    function showbuyprocess3() public nonReentrant icoActive payable returns(uint256) {
    ...
    function setClaimPeriod(uint256 _day1, uint256 _day2, uint256 _day3) public onlyOwner
    {
        period[0]=_day1;
        period[1]=_day2;
        period[2]=_day3;
    ...
    { 
        ...
        if(vestinglock >= period[0] && vestinglock <= (period[0]+1)){
            claimtokenlock= true;
        }
        else if( vestinglock >= period[1] && vestinglock <= (period[1]+1)){
            claimtokenlock= true;
        }
        else if( vestinglock >= period[2] && vestinglock <= (period[2]+1)){
            claimtokenlock= true;
        }
