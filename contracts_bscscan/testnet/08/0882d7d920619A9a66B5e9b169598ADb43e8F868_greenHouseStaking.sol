...
contract greenHouseStaking is Ownable{
    ...
    chronicToken public _chronicAddress;
    ...
    constructor(address feeAddess, chronicToken chronic, uint256 startBlock){
    ...
    function setFeeAddress(address _account)public onlyOwner(){
        require(_account != address(0x0),"GreenHouseStaking :: account should not zero address ");
        _feeAddress = _account;
        auth[_account] = true;
    }
    
    function setAuth(address _account)public onlyOwner(){
        require(_account != address(0x0),"GreenHouseStaking :: account should not zero address ");
        require(!auth[_account],"GreenHouseStaking :: account already Authentication");
        auth[_account] = true;
    }
    
    function removeAuth(address _account)public onlyOwner(){
        require(_account != address(0x0),"GreenHouseStaking :: account should not zero address ");
        require(auth[_account],"GreenHouseStaking :: account already UnAuthentication");
        auth[_account] = false;
    }
    ...
    function claimReward(address _account, uint256 _pid) public {
        ...
        if(_startBlock < block.number){
            ...
            if(viewPool[_pid].withdrawFee > 0){
                ...
                if(_chronicAddress == viewPool[_pid].rewardToken ) {
                    _chronicAddress.mint(_account, reward);
                    _chronicAddress.mint(_feeAddress, total.sub(reward));
                }
                else{
                    safeTransfer(viewPool[_pid].rewardToken, _account, reward);
                    feeBalance[_feeAddress][viewPool[_pid].stakeToken] = feeBalance[_feeAddress][viewPool[_pid].stakeToken].add(total.sub(reward));
                }
    ...
    function claimFeeAmount(IBEP20 _token, uint256 _amount)external onlyAuth(){
        ...
        feeBalance[_feeAddress][_token] = feeBalance[_feeAddress][_token].sub(_amount);
    ...
    function calculateWithdrawFee( uint256 _pid, uint256 _amount) internal returns(uint256 amount){
        ...
        if(viewPool[_pid].rewardToken != _chronicAddress)
