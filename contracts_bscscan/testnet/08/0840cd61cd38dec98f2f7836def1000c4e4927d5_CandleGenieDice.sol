...
contract CandleGenieDice is Ownable, ReentrancyGuard 
{
    ...
    function Roll(int position, bool useVRF) external payable nonReentrant notContract
    {
        ...
        _safeRoll(msg.sender,betAmount, position, useVRF);
    ...
    function _safeRoll(address user, uint256 betAmount, int position, bool useVRF) internal
    {
        ...
        bet.vrfUsed = useVRF;
        UserBets[msg.sender].push(rollID);
        ...
        // Emit Event
        emit RollEvent(msg.sender, bet.Index, bet.rollId, getRollToPosition(bet.guess), bet.rollTimestamp, bet.betAmount, bet.rewardMultiplier, bet.vrfUsed);
