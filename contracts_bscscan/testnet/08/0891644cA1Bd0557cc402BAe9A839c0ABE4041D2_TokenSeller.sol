...
contract TokenSeller is Ownable, AccessControl {
    ...
    receive() external payable {
        ...
        uint256 tokenAmount = SafeMath.mul(msg.value, sellRate);

        require(tokenAmount == phasePackage, "must send the exact amount for the package");

        list[msg.sender].phaseBought = true;
        list[msg.sender].unlockTimestamp = phaseTimestamp + phaseDuration + phaseSellLock;
        list[msg.sender].bought += tokenAmount;

        console.log("tokenAmount %s",tokenAmount);

        require(IERC20(token).transfer(msg.sender, tokenAmount / 10**18), "token transfer fail");
    }
    ...
    function changeSellRate(uint256 newSellRate) public onlyOperator {
