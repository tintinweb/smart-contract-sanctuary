...
contract HODL is Context, IBEP20, Ownable, ReentrancyGuard {
        ...
        initializer
    {
        ...
        selltax = Selltax(150, 150, 30, 20);
        buytax = Buytax(60, 20, 10, 10);
        transfertax = Transfertax(60, 20, 10, 10);
    ...
    function _transfer(
        ...
        if (
            ...
            _taxFee = selltax.reflection;
            _liquidityFee = selltax.bnbReward.add(selltax.bnbReserve).add(selltax.liquidity);
        ...
        // take buy fee
            ...
            if (!checkTaxAndLiquidityFees()) {
                _taxFee = buytax.reflection.div(2);
                _liquidityFee = (buytax.bnbReward.add(buytax.marketing).add(buytax.liquidity)).div(2);
                // buytax = 35;
            } else {
                _taxFee = buytax.reflection;
                _liquidityFee = buytax.bnbReward.add(buytax.marketing).add(buytax.liquidity);
                // buytax = 70;
            }
        ...
        // take transfer fee
            ...
            if (takeFee && from != owner() && from != address(this)) {
                _taxFee = transfertax.reflection;
                _liquidityFee = transfertax.bnbReward.add(transfertax.marketing).add(transfertax.liquidity);
                // uint256 transferFee = amount.mul(transfertax).div(100);
                // amount = amount.sub(calculateTaxFee(amount).sub(calculateLiquidityFee(amount)));
    ...
    Transfertax public transfertax;
    ...
    function changeselltax(uint256 bnbReserve, uint256 bnbReward, uint256 liquidity, uint256 reflection) public onlyOwner {
        selltax = Selltax(bnbReserve, bnbReward, liquidity, reflection);
    }

    function changebuytax(uint256 bnbReward, uint256 liquidity, uint256 marketing, uint256 reflection) public onlyOwner {
        buytax = Buytax(bnbReward, liquidity, marketing, reflection);
    }

    function changetransfertax(uint256 bnbReward, uint256 liquidity, uint256 marketing, uint256 reflection) public onlyOwner {
        transfertax = Transfertax(bnbReward, liquidity, marketing, reflection);
    }
