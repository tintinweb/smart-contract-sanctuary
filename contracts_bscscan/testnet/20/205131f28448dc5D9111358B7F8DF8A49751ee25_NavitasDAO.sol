...
contract NavitasDAO {
    ...
    function estimateReward(address staker) public view returns (uint256){
        ...
        uint256 rewardCalculation=((initialPercentFactor)/currentCO2Concentration)*((block.timestamp-lastClaimed[staker])/rewardLimiter);
        uint256 reward=((stake[staker]/100)*(rewardCalculation));
        return reward;  
    ...
    function getRewardLimiter() public view returns(uint256){
        return rewardLimiter;
    }
    ...
    function getUnlockDate(address staker) public view returns(uint256){
        return unlockedAt[staker];
    }
    ...
    function getReward() public {
        ...
        uint256 rewardCalculation=((initialPercentFactor)/currentCO2Concentration)*((block.timestamp-lastClaimed[msg.sender])/rewardLimiter);
        uint256 reward=((stake[msg.sender]/100)*(rewardCalculation));
    ...
    event voteProposal(address proposer,string id);

    function createProposal(string memory id) public {
        require(block.timestamp>expiryDate[id],"Voting not ended or ID already in use!");
    ...
    event requestChangeVoteFee(address requestor,uint256 proposedFee);
    function changeVoteFee(uint256 proposedFee) public {
        require(block.timestamp>expiryDate["k6HrUBZa"],"Vote already in progress!");
        expiryDate["k6HrUBZa"]=block.timestamp+stakeLockSeconds;
        vote_votingFeeInProgress=true;
        proposedVoteFee=proposedFee;
        emit requestChangeVoteFee(msg.sender,proposedFee);
    }

    function finish_changeVoteFee (string memory id) public {
        require(block.timestamp>expiryDate[id],"Voting not ended!");
        require((upVotes[id]+downVotes[id])>(stakers/2),"Not enough voters to proceed.");
        if (upVotes[id]>downVotes[id]){
