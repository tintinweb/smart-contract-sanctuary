...
contract EpicHeroNFT is ERC721Enumerable, EpicAuth {
    ...
    function bornHero(address user, uint heroId0, uint heroId1) external {
        authorizedFor(Permission.ManageAttributes);
    ...
    function killHero(uint heroId) external {
        authorizedFor(Permission.ManageAttributes);

        safeTransferFrom(ownerOf(heroId), deadAddress, heroId);

        emit KillHero(heroId);
    }

    function setRarity(uint heroId, uint rarity) external {
        authorizedFor(Permission.ManageAttributes);
    ...
    function userLevelUp(uint heroId) external {
    ...
    event KillHero(uint indexed heroId);
