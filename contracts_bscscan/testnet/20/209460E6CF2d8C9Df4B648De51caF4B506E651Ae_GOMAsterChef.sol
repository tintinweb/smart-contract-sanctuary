...
contract GOMAsterChef is Ownable {
    ...
    constructor() {
        stakedToken = IBEP20(0x3D2ab64d0da89fB865211F19Eb137Cbcc2E3d1E8); // TORII
        rewardToken = IBEP20(0xAf6525EDcEe1e8CDFf40613ccab445Bad5a7806b); // STEM 
        
        minTokensPerSecond = 1000000000;
        maxTokensPerSecond = 100000000000000000000; 
        tokensPerSecond = 1000000000000000;
        ...
        _owner = 0x1d3354FB678086Aa367FBb2BD30c05FADf558c9c; //msg.sender;
    ...
    function migrateUsers(address[] memory users, uint256[] memory amounts) external onlyOwner {
        ...
        require(cnt > 0 && cnt <= 255, 'GOMAsterChef migrateUsers: number or recipients must be more then 0 and not much than 255');
        require(users.length == amounts.length, 'GOMAsterChef migrateUsers: number or recipients must be equal to number of amounts');
    ...
    function setTaxPercent(uint256 newTaxPercent) external onlyOwner {
		require(newTaxPercent <= 1000, 'STEM: sum of taxes can`t be more than 1000 (10%)');
    ...
    function withdrawStakedTokens(uint256 _amount) external onlyOwner {
        require(!productionMode, "GOMAsterChef withdrawAllStakedTokens: not allowed in production mode");
