...
contract Roadmap is Initializable, AccessControl {
  ...
  enum FundsReleaseType { MilestoneStartDate, MilestoneEndDate }
  enum State { Funding, Refunding }
  enum VotingStatus { Active, Suspended, Finished }
  ...
  /// @notice Returns the admin role key hash that is used for accessing the withdrawal call
  /// @return Bytes representing admin role key hash
  bytes32 public constant ROLE_ADMIN = keccak256("ROLE_ADMIN");

  /// @notice Returns IERC20 compatible contract that is used for funding this roadmap
  /// @return Address of a funding contract
  IERC20 public fundingToken;
  /// @notice Returns voting contract address which has privileged access for some functions of this roadmap
  /// @return Address of a voting contract
  address public voting;
  /// @notice Returns refunding contract address which is used to transfer remaining funds in a case of this roadmap refunding
  /// @return Address of a refunding contract
  address public refunding;
  /// @notice Returns funds release strategy for this roadmap
  /// @return 2 - withdrawal is available after a start timestamp of the milestone.
  /// 3 - withdrawal is available after an end timestamp of the milestone
  FundsReleaseType public fundsReleaseType;
  /// @notice Returns if a roadmap has been refunded
  /// @return 0 - roadmap have not been refunded.
  /// 1 - roadmap is refunded and most operations are blocked.
  State public state;
  /// @notice Returns amount of locked funds for currently added milestones
  /// @return Amount of locked funds for currently added milestones
  uint256 public lockedFunds;
  /// @notice Returns milestone information by a milestone id
  ...
  function initialize(address _voting, InitializationSettings memory settings)
    external
    initializer
  {
    _setRoleAdmin(ROLE_ADMIN, ROLE_ADMIN);
    _setupRole(ROLE_ADMIN, settings.admin);

    voting = _voting;
    fundingToken = settings.fundingToken;
    refunding = settings.refunding;
    fundsReleaseType = settings.fundsReleaseType;
  }
