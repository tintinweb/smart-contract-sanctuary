...
import "./interfaces/LockNFT.sol";

interface ERC721Locker {
  function lock(uint256 tokenId, address locker) external returns (bool);

  function unlock(uint256 tokenId) external returns (bool);
}

contract TokenFarmingBank is OwnableUpgradeable, IERC721Locker {
  ...
  address public _nft;
  mapping(address => uint256) public nftLock;

  function initialize(address _token, uint256 _startTime) external initializer {
    __Ownable_init();
    token = _token;
    rewardPerSecond = 100000000000000000; // 0.1 token/seconds
  ...
  function setRewardPerSecond(uint256 _rewardPerSecond) external onlyOwner {
    require(_rewardPerSecond <= 10 ether, "insane high rate");
    _updatePools();
    ...
    returns (uint256)
  {
    ...
    if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
      ...
      if (totalAllocPoint > 0) {
        uint256 _tokenReward = (_seconds * rewardPerSecond * pool.allocPoint) /
          totalAllocPoint;
        accTokenPerShare += (_tokenReward * 1e18) / lpSupply;
      }
    }
    uint256 _totalReward = accTokenPerShare;
  ...
  function _getReward(uint256 _pid, address _account) internal {
  ...
  function _safeTokenTransfer(address _to, uint256 _amount) internal {
    ...
    if (_tokenBalance > 0) {
      if (_amount > _tokenBalance) {
        IERC20(token).safeTransfer(_to, _tokenBalance);
      } else {
  ...
  function unlock(uint256 tokenId) external override {
    require(nftLock[msg.sender] == tokenId, "!Locked");
    ERC721Locker(_nft).unlock(tokenId);
  }
  ...
  function onERC721Locked(
    ...
    require(nftLock[ownerNFT] == 0, "Locked!");
    require(msg.sender == _nft, "!NFT");
    require(locker == address(this), "!contract");
    nftLock[ownerNFT] = tokenId;
    return true;
  }
