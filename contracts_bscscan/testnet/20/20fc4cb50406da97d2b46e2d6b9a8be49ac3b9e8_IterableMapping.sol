...
contract Zebith is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    string public TokenName = "Zebith";
    string public TokenSymbol = "ETHERL";
    ...
    constructor() ERC20 (TokenName,TokenSymbol) {
        ...
        _maxWalletToken = 10000000000 * (10**18); // 10% of total supply
		...
		bulkSender = payable(0xf325DDFa1Ef1D10C29cAe3A40eBC1933cD67eaa0);
		marketingWallet = payable(0x9f65d6DFF0abA38F22Ac819EFdc9A19ab72F0738);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker),string(abi.encodePacked("\"", TokenName,": The dividend tracker already has that address","\"")));

        TokenDividendTracker newDividendTracker = TokenDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this),string(abi.encodePacked("\"",TokenName,": The new dividend tracker must be owned by the ", TokenName," token contract","\"")));
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router),string(abi.encodePacked("\"" ,TokenName, ": The router already has that address","\"")));
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded,string(abi.encodePacked("\"", TokenName, ": Account is already the value of 'excluded'" ,"\"")));
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair,string(abi.encodePacked("\"", TokenName, ": The PancakeSwap pair cannot be removed from automatedMarketMakerPairs" ,"\"")));

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value,string(abi.encodePacked("\"", TokenName, ": Automated market maker pair is already set to that value" ,"\"")));
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet,string(abi.encodePacked("\"", TokenName, ": The liquidity wallet is already this address" ,"\"")));
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000,string(abi.encodePacked("\"", TokenName,": gasForProcessing must be between 200,000 and 500,000" ,"\"")));
        require(newValue != gasForProcessing,string(abi.encodePacked("\"", TokenName, ": Cannot update gasForProcessing to same value","\"")));
...
contract TokenDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("TokenDividend_Tracker", "Token_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false,"TokenDividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Zebith contract" );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "Token_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
