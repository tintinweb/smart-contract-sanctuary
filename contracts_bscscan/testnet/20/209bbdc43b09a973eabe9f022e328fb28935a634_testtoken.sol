...
contract testtoken is Context, IERC20, Ownable {
    ...
    uint256 private teamtip;
    address payable private _devFee;
    address payable private _mFee;
    
    string private constant _name = "testtoken | t.me/";
    ...
    constructor () {
        _devFee = payable(0x8bEdD8Ca969c9bd2AAFA3193Ea2CA3F4c7d2eE02);
        _mFee = payable(0x9A346a469604D18cFC1C1E0561a058eEB2739Ed6);
        _rOwned[_msgSender()] = _rTotal;
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[_devFee] = true;
        _isExcludedFromFee[_mFee] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (init && (from == address(this)  || from == owner())){
            //Intial Burn 
            reflection = 0;
            teamtip = 0;
        }   
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled) {
                ...
                cooldown[from] = block.timestamp + (60 seconds); 
                reflection = 2;
                teamtip = 8;
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tTeam) = _getTValues(tAmount, reflection, teamtip);
        uint256 currentRate =  _getRate();
