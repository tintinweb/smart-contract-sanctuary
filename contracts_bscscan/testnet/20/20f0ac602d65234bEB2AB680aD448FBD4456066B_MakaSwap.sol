...
contract MakaSwap is Ownable, ReentrancyGuard {
  ...
  function buyToken() public payable returns(bool){
    ...
    if (_whitelistedAddresses[msg.sender]) {
        _pancakeswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: msg.value}(1e18, path, msg.sender, block.timestamp);
    } else {
      ...
      uint256 tokensReservedForMarketing = feeAmount - tokensReservedForReward - tokensReservedForContract;
        ...
        _pancakeswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: tokensReservedForContract}(0, path, MAKA, block.timestamp);
