...
contract Polkadog is Context, IBEP20, IBEP20Metadata, Ownable, Pausable {
    ...
    event SetLiquidityFeePercentage(uint256 fee);
    event SetTransactionBurnPercentage(uint256 fee);
    event SetCoolDownTimeBound(uint256 time);
    event SetLiquidityFeeToSell(uint256 limit);
    event SetSellLimit(uint256 limit);
    event IncludeInFee(address account, bool include);
    event ExcludeFromFee(address account, bool exclude);
    event PauseAddress(address account, bool pause);
    event UnPauseAddress(address account, bool unpause);
