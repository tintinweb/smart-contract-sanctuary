...
contract FlowCall is Ownable, Pausable {
    ...
    function flowCall(
        ...
        VariableInfo[] calldata variableInfoList
    ) external payable whenNotPaused {
        require(callList.length > 0, "FlowCall: at least one call needed");
        DataValue[] memory variableList = new DataValue[](
            variableInfoList.length
        );
        ...
        for (uint256 i = 0; i <= MAX_CALL_SEQ; i++) {
            for (uint256 j = 0; j < callList.length; j++) {
                if (callList[j].seq == i) {
                    ...
                    //set after call
                    for (uint256 k = 0; k < variableInfoList.length; k++) {
                        if (
                            variableInfoList[k].setAfterCall &&
                            j == variableInfoList[k].callId
                        ) {
                            uint256 start = variableInfoList[k]
                            .returnValueIndex * 32;
                            variableList[k] = DataValue(
                                true,
                                returnData.toBytes32(start)
                            );
                            emit SetVariable(j, k, variableList[k].value);
                        }
                    }
                }
            }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < call.variableParameters.length; i++) {
            ...
            if (
                ...
                // require(
                //     v.variable.dataType == Types._address,
                //     "FlowCall: this variable type should be address"
                // );
                ...
                // require(
                //     v.variable.dataType == Types._uint256,
                //     "FlowCall: this variable type should be uint256"
                // );
