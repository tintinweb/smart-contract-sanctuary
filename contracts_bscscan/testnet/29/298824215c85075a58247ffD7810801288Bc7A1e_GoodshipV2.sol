...
// File: contracts/GoodshipV2.sol



contract GoodshipV2 is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    string private _name = "Goodship V2";
    ...
    uint256 public rewardCycleBlock = 3 days;  //  standard claim cycle for claiming reward
    uint256 public threshHoldTopUpRate = 25; // 25 percent threshHold buy limit
    uint256 public _maxTxAmount = _tTotal.div(100); // max amount should be 1% percent per transaction
    uint256 public maxPerWallet = _tTotal.mul(3).div(100); // max holding per wallet should be less than 3%
    mapping(address => uint256) public nextAvailableClaimDate; // next claim for BNB reward
    bool public swapAndLiquifyEnabled = false; // should be true to strat the pool
    bool inSwapAndLiquify = false;

    uint256 public _taxFee = 15; // 1.5% will be distributed among holder as G2 divideneds
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee = 50; // 5% will be added pool
    uint256 private _previousLiquidityFee = _liquidityFee;
    
    uint256 public _poolFee = 5; // 0.5% will be added to pool for BNB reward
    uint256 private _previousPoolFee = _poolFee;
    
    uint256 public _charityFee = 30; // 3% will go to the charity wallet in BNB
    uint256 private _previousCharityFee = _charityFee;
    
    uint256 public _marketFee = 10;  // 1% will go to the market wallet in BNB
    uint256 private _previousMarketFee = _marketFee;
    
    uint256 public _burnFee = 10;  // 1% will go to the Dead address to deflate G2
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_taxFee.add(_liquidityFee).add(_poolFee).add(_charityFee).add(_marketFee).add(_burnFee)).div(1e3);
        return percentage;
    }
    ...
    function _takeMultipleFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tFee = tAmount.mul(_liquidityFee.add(_poolFee).add(_charityFee).add(_marketFee)).div(1e3);
    ...
    function _takeBurnFee(uint256 tAmount, uint256 currentRate, address sender) internal {
    ...
    function _transfer(
        ...
        require(amount > 0, "BEP20: amount can not zero");
        require(balanceOf(to).add(amount) <= maxPerWallet,"BEP20: exceeding limit");
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _takeMultipleFee(tAmount, currentRate);
        _takeBurnFee(tAmount, currentRate, sender);
        _reflectFee(tAmount);
    ...
    function calculateBNBReward(address ofAddress) public view returns (uint256) {
        ...
        .sub(balanceOf(burnAddress)) // exclude burned wallet
    ...
    function claimBNBReward() isHuman nonReentrant public {
        require(nextAvailableClaimDate[msg.sender] <= block.timestamp, "Bep20: next available not reached");
        require(balanceOf(msg.sender) >= 0, "Error: must own G2 to claim reward");

        uint256 reward = calculateBNBReward(msg.sender);
        require(reward >= 0.1 ether,"Bep20: reward is less than the limit");
        ...
        require(sent, 'Bep20: Cannot withdraw reward');
    }
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            uint256 totalPercent = _liquidityFee.add(_poolFee).add(_charityFee).add(_marketFee);
            uint256 lpPercent = _liquidityFee.mul(1e4).div(totalPercent).div(2);
            uint256 bpPercent = _poolFee.mul(1e4).div(totalPercent);
            uint256 cwPercent = _charityFee.mul(1e4).div(totalPercent);
            ...
            totalPercent = lpPercent.add(bpPercent).add(cwPercent).add(mwPercent);
            ...
            charityAddress.transfer(deltaBalance.mul(cwPercent).div(totalPercent));
            marketAddress.transfer(deltaBalance.mul(mwPercent).div(totalPercent));
