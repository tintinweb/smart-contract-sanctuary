...
import "../tokens/Seeder.sol";
import "../tokens/SeederAttribute.sol";
...
contract VendingMachine is OwnableUpgradeable, OracleClient {
    ...
    event AddSeeder(uint256 indexed packID, bytes32 name, uint256 birthRate);
    event RemoveSeeder(uint256 indexed packID, bytes32 name);
    ...
    function getGachaTicket(uint256 _packID) external payable {
        ...
        //Get random number from oracle, length (0-99) with 7 items return
        requestRandomNumber(ticketCounter.current(), 0, 99, 7, msg.value);
    ...
    function createPack(uint256 _tokenPrice, uint256 _bonusPoint)
        external
        onlyOwner
    {
    ...
    function addSeeder(
        ...
        require(
            ...
            "VendingMachine: seeder already added"
        );
        require(
            seeder.doesSeederExist(_name) == true,
            "VendingMachine: seeder is not existed"
        );

        packIDToNameToBirthRate[_packID][_name] = _birthRate;
        packIDToNames[_packID].push(_name);

        emit AddSeeder(_packID, _name, _birthRate);
    }

    function removeSeeder(uint256 _packID, bytes32 _name) external onlyOwner {
        require(
            packIDToPackInfo[_packID].tokenPrice > 0,
            "VendingMachine: invalid pack ID"
        );
        require(
            packIDToNameToBirthRate[_packID][_name] > 0,
            "VendingMachine: seeder is not added"
        );
        ...
        emit RemoveSeeder(_packID, _name);
    }

    function listSeeder(uint256 _packID)
        external
        view
    ...
    /// @notice after oracle finishing the randomness, start to call minting seeder (call from oracle only)
    function processCreation(uint256 ticketID, uint256[] memory randomNumbers)
        internal
        override
    {
        require(
            randomNumbers.length == 7,
            "VendingMachine: Invalid random number from oracle"
        );

        TicketInfo storage ticket = ticketIDToTicketInfo[ticketID];
        require(
            ticket.player != address(0),
            "VendingMachine: Ticket is not valid"
        );

        uint256 packID = ticket.packID;
        bytes32 name = getSeedMon(randomNumbers[6], packID);
        SeederAttribute.UniqueProfile memory unique = getUnique();
        SeederAttribute.Stat memory stat = getStat(randomNumbers, packID);
        seeder.mint(ticket.player, name, unique, stat);
        token.transfer(wallet, ticket.amount);

        delete ticketIDToTicketInfo[ticketID];
        delete playerToTicketID[ticket.player];
        emit CreateSeedmon(ticket.player, ticketID, name);
    }
    ...
    function ratioNumber(
        ...
        uint256 to1,
        uint256 from2
    ) public pure returns (uint256 to2) {
    ...
    function getUnique()
        internal
        view
        returns (SeederAttribute.UniqueProfile memory)
    {
        return
            SeederAttribute.UniqueProfile({
    ...
    function getStat(uint256[] memory randomNumbers, uint256 packID)
        internal
        view
        returns (SeederAttribute.Stat memory)
    {
        uint256 bonusPoint = packIDToPackInfo[packID].bonusPoint;
        uint256 sum = 0;
        for (uint256 i = 0; i < randomNumbers.length; i++) {
            sum = sum + randomNumbers[i];
        }
        return
            SeederAttribute.Stat({
                vitality: ratioNumber(sum, randomNumbers[0], bonusPoint),
                strength: ratioNumber(sum, randomNumbers[1], bonusPoint),
                smart: ratioNumber(sum, randomNumbers[2], bonusPoint),
                speed: ratioNumber(sum, randomNumbers[3], bonusPoint),
                dexterity: ratioNumber(sum, randomNumbers[4], bonusPoint),
                luck: ratioNumber(sum, randomNumbers[5], bonusPoint)
            });
    }

    function getSeedMon(uint256 randomNumber, uint256 packID)
        internal
        view
        returns (bytes32 name)
    {
        uint256 totalProbLength = 0;
        for (uint256 i = 0; i < packIDToNames[packID].length; i++) {
            totalProbLength =
                totalProbLength +
                packIDToNameToBirthRate[packID][packIDToNames[packID][i]];
        }

        uint256 targetIndex = ratioNumber(100, randomNumber, totalProbLength);
        totalProbLength = 0;
        name = bytes32(packIDToNames[packID][0]);
        for (uint256 i = 0; i < packIDToNames[packID].length; i++) {
            totalProbLength =
                totalProbLength +
                packIDToNameToBirthRate[packID][packIDToNames[packID][i]];
            if (totalProbLength > targetIndex) {
                break;
            }
            name = bytes32(packIDToNames[packID][i]);
        }
    }
}
...
abstract contract OracleClient is IClient {
    ...
    // =========== Process ====================
    function processCreation(uint256 ticketID, uint256[] memory randomNumbers) internal virtual {
    }
    ...
    function onRandomNumberReceived(uint256 requestId, uint256[] memory randomNumbers) override external onlyOracle returns (bool isCompleted) {
        processCreation(requestId, randomNumbers);
        isCompleted = true;
    }
...
import "../utils/SafeMath.sol";
import "./SeederAttribute.sol";

contract Seeder is ERC721Enumerable, TrustCaller {
    event SeederCreated(bytes32 name);
    event SeederChanged(bytes32 name);
    ...
    mapping(bytes32 => SeederAttribute.Profile) public nameToProfile;
    mapping(bytes32 => SeederAttribute.Stat) public nameToStat;
    mapping(bytes32 => SeederAttribute.BaseAction) public nameToBaseAction;
    mapping(bytes32 => string) public nameToTokenURI;

    mapping(uint256 => bytes32) public tokenIdToName;
    mapping(uint256 => SeederAttribute.UniqueProfile) public tokenIdToUniqueProfile;
    mapping(uint256 => SeederAttribute.Stat) public tokenIdToBonusStat;

    constructor() ERC721("Seeder", "SEEDER") {
    }

    function totalSeederData() external view returns (uint256 numberOfSeeder) {
        numberOfSeeder = names.length;
    }
    ...
    function mint(address to, bytes32 name_, SeederAttribute.UniqueProfile calldata uniqueProfile_, SeederAttribute.Stat calldata bonusStat_) external onlyTrustCaller {
        require(to != address(0), "Seeder: unsupport zero address");
        require(doesSeederExist(name_), "Seeder: does not exist");
    ...
    function createSeeder(bytes32 name, SeederAttribute.Profile calldata profile_, SeederAttribute.Stat calldata stat_, SeederAttribute.BaseAction calldata baseAction_, string calldata uri) external onlyOwner {
        require(!doesSeederExist(name), "Seeder: duplicated name");
        require(isProfileValid(profile_), "Seeder: Invalid profile");
        require(isBaseActionValid(baseAction_), "Seeder: Invalid action");
        ...
        emit SeederCreated(name);
    }

    function adjustSeeder(bytes32 name, SeederAttribute.Profile calldata profile_, SeederAttribute.Stat calldata stat_, SeederAttribute.BaseAction calldata baseAction_) external onlyOwner {
        require(doesSeederExist(name), "Seeder: does not exist");
        require(isProfileValid(profile_), "Seeder: Invalid profile");
        require(isBaseActionValid(baseAction_), "Seeder: Invalid action");
    ...
    function addSpecies(bytes32 species_) external onlyOwner {
        require(!speciesToActiveStatus[species_], "Seeder: duplicated species");
    ...
    function addElement(bytes32 element_) external onlyOwner {
        require(!elementToActiveStatus[element_], "Seeder: duplicated element");
    ...
    function addTarget(bytes32 target_) external onlyOwner {
        require(!targetToActiveStatus[target_], "Seeder: duplicated target");
    ...
    function addAim(bytes32 aim_) external onlyOwner {
        require(!aimToActiveStatus[aim_], "Seeder: duplicated aim");
    ...
    function addBaseAction(bytes32 action_) external onlyOwner {
        require(!baseActionToActiveStatus[action_], "Seeder: duplicated action");
    ...
    // ================== Internal method ============
    function doesSeederExist(bytes32 name_) public view returns (bool doesExist) {
        doesExist = nameToStat[name_].vitality > 0;
    }

    function isProfileValid(SeederAttribute.Profile calldata profile) internal view returns (bool) {
        return speciesToActiveStatus[profile.species] && 
                elementToActiveStatus[profile.element];
    }

    function isBaseActionValid(SeederAttribute.BaseAction calldata baseAction) internal view returns (bool) {
