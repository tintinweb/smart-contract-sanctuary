...
contract FOFF is Context, IERC20, Ownable {
    ...
    string private constant _name = "FOFF";
    string private constant _symbol = "FOFF1";
    ...
    uint256 private _maxWallet = 2000000000 * 10**9;
    ...
    function _maxWalletAmount() private view returns (uint256) {
        ...
        if (time_deployment < 1 days) {
            ...
            return _maxWallet.mul(8);
        } else {
            return _maxWallet.mul(10);
        }
    }
  ...
  /*  function setSwapAndLiquifyEnabled(bool _swapAndLiquifyEnabled) external onlyOwner() {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] && from == pancakePair && to != address(pancakeRouter)){
            takeFee = false;
        }
