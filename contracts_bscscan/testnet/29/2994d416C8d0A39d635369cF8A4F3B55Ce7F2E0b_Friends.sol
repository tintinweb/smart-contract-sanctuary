...
contract Friends is Ownable {
    ...
    function takeInvestment(uint256 _packetId) public{
        ...
        uint256 _lastUpdate = (lastUpdate[msg.sender][_packetId] > userPackets[msg.sender][_packetId].finishTime) ? userPackets[msg.sender][_packetId].finishTime : lastUpdate[msg.sender][_packetId];
    
        uint256 _earned = userPackets[msg.sender][_packetId].invested.mul(DAILY_REWARD).mul(_endTime.sub(_lastUpdate)).div(600).div(100);
    ...
    function totalClaimable(uint256 _packetId, address _user) public view returns(uint256) {
        ...
        uint256 _lastUpdate = (lastUpdate[msg.sender][_packetId] > userPackets[_user][_packetId].finishTime) ? userPackets[_user][_packetId].finishTime : lastUpdate[msg.sender][_packetId];

        uint256 _reward = userPackets[_user][_packetId].invested.mul(DAILY_REWARD).mul(_endTime.sub(_lastUpdate)).div(600).div(100);
    ...
    function getActivePackets(address _user) public view returns(Packet[] memory) {
        ...
        for(uint i=0; i<_allPackets.length; i++) {
            if(_allPackets[i].finishTime > block.timestamp || _allPackets[i].paid < (_allPackets[i].invested.mul(99).div(100))) {
                _size++;
            }
        }
