  ...
  Ownable
{
  ...
  function multiMint(address _to, uint256[] calldata token_id, string calldata _uri, string calldata _payload) external {
      for (uint i = 1; i <= token_id.length; i++) {
           
            require(idToOwner[token_id[i]] == address(0), NFT_ALREADY_EXISTS);

            super._mint(_to, token_id[i]);
            super._setTokenUri(token_id[i], _uri);
            super._setTokenPayload(token_id[i], _payload);
      }
  }
