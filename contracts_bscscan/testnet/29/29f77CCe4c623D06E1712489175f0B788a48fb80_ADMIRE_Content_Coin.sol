...
contract MultiSig is Context {
    ...
    // @dev Initializes the contract with _change_operator_stopped state control off.
    // @dev Initializes the contract with _taxrate_change_stopped state control off.
    // @dev Initializes the contract with _charityrate_change_stopped state control off.
    // @dev Initializes the contract with _blacklist_stopped state control off.
    // @dev Initializes the contract with _antiwhale_stopped state control off.
    // @dev Initializes the contract with _mint_stopped state control off.
    // @dev Initializes the contract with _approve_stopped state control off.
    constructor () internal {
    ...
    // @dev **************** Lock Approving Tokens To Contracts BREAKS THINGS   ***********************
...
contract ADMIRE_Content_Coin is BEP20('ADMIRE Content Coin', 'ADMIRE'){
    ...
    modifier blacklisted(address sender, address recipient) {
            ...
            // @dev WARNING! If this is any type of payable function or contract call, it's an EVIL CONTRACT!
    ...
    function Revoke_Change_Operator() public onlyOperator whenChangeOperatorOn {
        _stop_change_operator();
    }

    function Allow_Change_Operator() public onlyOperator whenChangeOperatorOff {
    ...
    function Revoke_Blacklist() public onlyOperator whenBlacklistOn {
        _stop_blacklist();
    }

    function Allow_Blacklist() public onlyOperator whenBlacklistOff {
        _start_blacklist();
    }
    ...
    function Revoke_Mint() public onlyOwner whenMintOn {
        _stop_mint();
    }

     function Allow_Mint() public onlyOwner whenMintOff {
        _start_mint();
    }

    // @dev WARNING! Things will break! The devs need to be able to increase allowances.
    function allowapprove() public onlyOwner whenApproveOn {
        _stop_approve();
    }

     function unallowapprove() public onlyOwner whenApproveOff {
        _start_approve();
    }

    
    /// @dev overrides transfer function to meet tokenomics of ADMIRE
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override blacklisted(sender, recipient) antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
   ...
   /// destroy the contract and reclaim the leftover funds. REMOVE FOR PRODUCTION USE
    ...
    function updateCharityRate(uint16 _charityRate) public onlyOwner whenCharityRateChangeOn {
        require(_charityRate <= 100, "ADMIRE::updateCharityRate: Charity rate must not exceed the maximum rate.");
