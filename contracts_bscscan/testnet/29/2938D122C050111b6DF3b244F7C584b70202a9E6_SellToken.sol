...
contract SellToken {
  ...
  constructor(address _idoAdmin, address _buyToken, address _idoToken, uint8 _f1_rate, uint8 _f2_rate, uint8 _f3_rate, uint256 _tokenRate) public {
    IDOAdmin = _idoAdmin;
    IDO_TOKEN = _idoToken;
    BUY_TOKEN = _buyToken;
    f1_rate = _f1_rate; f2_rate = _f2_rate; f3_rate = _f3_rate;
   ...
   */
  function updateRateRef(uint8 _f1_rate, uint8 _f2_rate, uint8 _f3_rate) public onlyIDOAdmin {
    f1_rate = _f1_rate;
    f2_rate = _f2_rate;
    f3_rate = _f3_rate;
  }
  ...
  function buyIDO(address recipient, uint256 buy_amount, address _referrer) public returns (uint256) {
   if (
        _referrer != address(0) //level 0
        && referrers[msg.sender] == address(0) //level 1
        && referrers[referrers[msg.sender]] == address(0) // level 2

        && msg.sender != _referrer //level 0
        && msg.sender != referrers[_referrer] //level 1
        && msg.sender != referrers[referrers[_referrer]] //level 2
    ) {
        ...
        if (referrers[referrers[_referrer]] != address(0)) {
            emit NewReferral(referrers[referrers[referrers[_referrer]]], msg.sender, 3);
        }
    }
    ...
    if (referrers[referrers[referrers[msg.sender]]] != address(0)){ 
      uint256 f3_reward = sold_amount.mul(f3_rate).div(100);
      IERC20(IDO_TOKEN).transfer(referrers[referrers[referrers[msg.sender]]], f3_reward);
      refAmount[referrers[referrers[referrers[ msg.sender]]]] += f3_reward;
      emit RefReward(referrers[referrers[referrers[ msg.sender]]], f3_reward, 3);
    }
    return sold_amount;
  }
}
