...
contract DevelopmentFeeVault is Ownable {
  ...
  constructor(
      ...
      public
    {
      ...
      devAddr = _devAddr;
      _totalLock = 35*(1e6)*(1e18);
      totalSupply = 35*(1e6)*(1e18);
      _locks[devAddr] = 35*(1e6)*(1e18);
      _lastUnlockBlock[_devAddr] = _startReleaseBlock;
    }
  ...
  function setReleaseBlock(uint256 _startReleaseBlock, uint256 _endReleaseBlock) public onlyOwner {
    ...
    startReleaseBlock = _startReleaseBlock;
    endReleaseBlock = _endReleaseBlock;
    _lastUnlockBlock[devAddr] = _startReleaseBlock;
  }
  ...
  function canUnlockAmount(address _account) public view returns (uint256) {
    // When block number less than startReleaseBlock, no CEA can be unlocked
    if (block.number < startReleaseBlock) {
      return 0;
    }
    // When block number more than endReleaseBlock, all locked CEA can be unlocked
    else if (block.number >= endReleaseBlock) {
      return _locks[_account];
    }
    // When block number is more than startReleaseBlock but less than endReleaseBlock,
    // some CEA can be released
    else
    {
  ...
  function unlock() public {
    require(_locks[msg.sender] > 0, "no locked CEA");
    ...
    cea.safeTransfer( msg.sender, amount);
  }

  function refundTokens(address recipient, address token) public onlyOwner {

    IBEP20 _token = IBEP20(token);
    uint256 _balance = _token.balanceOf(address(this));
    _token.transfer(recipient, _balance);
  }
