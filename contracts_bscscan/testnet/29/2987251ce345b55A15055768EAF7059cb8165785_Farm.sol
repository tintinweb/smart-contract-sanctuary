...
contract Farm is Ownable, ReentrancyGuard {
    ...
    // The block number when SHREW farming starts.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 shrewReward = multiplier.mul(shrewPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        shrew.transferFrom(farmWallet, address(this), shrewReward);
        pool.accShrewPerShare = pool.accShrewPerShare.add(shrewReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Withdraw shrew tokens after farming end
    function shrewWithdraw(IERC20 _shrew, address _to) onlyOwner external {
        uint256 amount = _shrew.balanceOf(address(this));
        _shrew.transfer(_to, amount);
    }

    // Deposit LP tokens to Farm for SHREW allocation.
    ...
    function safeShrewTransfer(address _to, uint256 _amount) internal {
        uint256 shrewBal = shrew.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > shrewBal) {
            transferSuccess = shrew.transfer(_to, shrewBal);
        } else {
            transferSuccess = shrew.transfer(_to, _amount);
        }
    ...
    //Shrew has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
