...
contract DefinaNFTMaster is Ownable, ERC721Holder, Initializable, Pausable {
    ...
    function stakeAll(uint[] memory tokenIds_) external onlyEOA whenNotPaused {
        require(tokenIds_.length != 0);
        uint length = nftToken.balanceOf(_msgSender());
        for (uint i = 0; i < length; i++) {
            stake(nftToken.tokenOfOwnerByIndex(_msgSender(), i));
        }
    }
    ...
    function withdrawAll(uint[] memory tokenIds_) external onlyEOA whenNotPaused {
        require(tokenIds_.length != 0);
