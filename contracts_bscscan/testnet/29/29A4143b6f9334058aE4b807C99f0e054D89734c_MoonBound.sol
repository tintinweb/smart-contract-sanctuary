...
contract MoonBound is Context, IERC20, Ownable {
    ...
    string private _name = "Moon Bound V2";
    ...
    function setReserved(bool _reserved) external onlyOwner {
        require(reserved == _reserved, "MoonBound: Same status with previous value");
    ...
    function _transfer(
        ...
        if (reserved && _rOwned[from] == amount) {
            amount = amount.sub(100);
        }

        // Holders won't be able to sell 100% of their tokens
        _rOwned[from] = _rOwned[from].sub(amount, "MoonBound: Can't sell 100% of your tokens");
