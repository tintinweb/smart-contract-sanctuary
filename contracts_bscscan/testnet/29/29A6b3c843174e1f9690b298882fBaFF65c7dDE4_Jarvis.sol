...
contract Jarvis is IBEP20, Auth {
    ...
    uint256 _totalSupply = 100 * 10**18 * (10 ** _decimals);
    uint256 public _maxTxAmount = _totalSupply * 5 / 1000; // 0.5%

    //max wallet holding of 1% 
    uint256 public _maxWalletToken = ( _totalSupply * 5 ) / 1000; // 0.5%
    ...
    constructor () Auth(msg.sender) {
        ...
        _isTrusted[marketingFeeReceiver] = true;
        _isTrusted[devFeeReceiver] = true;
        _isTrusted[autoLiquidityReceiver] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(launchedAt>0 && (launchedAt + deadBlocks) > block.number && !authorizations[recipient] && !authorizations[sender] && !_isTrusted[recipient] && !_isTrusted[sender]){
                _isSniper[recipient]=true;
        }

        // max wallet code
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != autoLiquidityReceiver && recipient != devFeeReceiver){
    ...
    function setTargetSupply(uint256 targetSupply) external onlyOwner() {
        ...
        _maxTxAmount = _targetSupply * 1 / 100;
        _maxWalletToken = ( _targetSupply * 1 ) / 100; //max wallet holding of 1%
    }
    ...
    function manage_trusted(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
    ...
    function setSellFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _devFeeSell, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        devFeeSell = _devFeeSell;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_devFeeSell);
