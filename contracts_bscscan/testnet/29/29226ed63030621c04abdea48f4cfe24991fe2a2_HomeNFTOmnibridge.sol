...
FailedMessagesProcessor
{
     ...
     * @param _royaltyReceivers royaltyReceivers for the bridged token instances.
     * @param _royaltyAmounts royaltyAmounts for the bridged token instances.
     */
    function deployAndHandleBridgedNFT(
        ...
        uint256[][] calldata _royaltyAmounts
        ...
        _setTokensURIAndRoyalties(bridgedToken, _tokenIds, _tokenURIs,_royaltyReceivers,_royaltyAmounts);
    }
    ...
    function handleBridgedNFT(
        ...
        _setTokensURIAndRoyalties(token, _tokenIds, _tokenURIs, _royaltyReceivers, _royaltyAmounts);
    }
    ...
    function _prepareMessage(
        ...
        if (nativeToken == address(0)) {
            ...
            address[][] memory royaltyReceiversMap = new address[][](_tokenIds.length);
            uint256[][] memory royaltyAmountsMap = new uint256[][](_tokenIds.length);
            ...
            if (_values.length > 0) {
                for (uint256 i = 0; i < _tokenIds.length; i++) {
                    ...
                    royaltyReceiversMap[i] = royaltyReceivers;
                    royaltyAmountsMap[i] = royaltyAmounts;
     ...
     * @param _royaltyAmounts royaltyAmounts for the bridged token instances.
     */
    function _setTokensURIAndRoyalties(
        address _token,
        uint256[] calldata _tokenIds,
        string[] calldata _tokenURIs,
        address[][] calldata _royaltyReceivers,
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            if (_royaltyReceivers.length > 0) {
                IBurnableMintableERC721Token(_token).setRoyaltyInfo(_tokenIds[i], _royaltyReceivers[i], _royaltyAmounts[i]);
            }
        }
    }
...
contract NFT is AccessControl, ERC721, IBurnableMintableERC721Token {
    ...
    constructor(address _bridgeContract, uint256 _mintMillionNumber) ERC721("NFTb_NFT", "NFTb_NFT") ERC165() {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(BRIDGE_ROLE, _bridgeContract);
    ...
    function mint(
        ...
        require(newTokenId >= mintLowerBound, "Invalid token id below lower bound in this chain");
        require(newTokenId <= mintHigherBound, "Invalid token id exceeds higher bound in this chain");
    ...
    function burn(uint256 tokenId) external override {
        require(
            _isApprovedOrOwner(_msgSender(), tokenId) || isBurner(_msgSender()) || isBridge(_msgSender()),
            "ERC721Burnable: caller is not owner nor approved, nor burner, nor bridge contract"
        );
     ...
     */
    function setBaseURI(string calldata _baseURI) external onlyBridgeOwner {
     ...
     */
    function setTokenURI(uint256 _tokenId, string calldata _tokenURI) external override onlyBridgeOwner {
        _setTokenURI(_tokenId, _tokenURI);
    }

    function setRoyaltyInfo(uint256 _tokenId, address[] calldata _royaltyReceivers, uint256[] calldata _royaltyAmounts) external override onlyBridge{
        require(
            _royaltyReceivers.length == _royaltyAmounts.length,
            "Invalid royalty data"
        );
        require(
            _royaltyReceivers.length <= ROYALTY_MAX_COUNT,
            "Royalty receivers max amount exceeded"
        );

        for (uint256 i = 0; i < _royaltyReceivers.length; i++) {
            royalties[_tokenId].push(
                RoyaltyInfo(_royaltyReceivers[i], _royaltyAmounts[i])
            );
        }
    }
...
contract NFTm is ERC1155Upgradeable, NFTBERC2981Enhanced, IBurnableMintableERC1155Token {
    ...
    function mint(
        address _to,
        uint256[] memory _tokenIds,
        uint256[] memory _values) override external  onlyBridge {
        if (_tokenIds.length == 1 && _values.length == 1) {
            _mint(_to, _tokenIds[0], _values[0], "");
        } else {
     ...
     */
    function burn(uint256[] memory _tokenIds, uint256[] memory _values) override external onlyBridge {
     ...
     * @param _tokenURI new token URI.
     */
    function setTokenURI(uint256 _tokenId, string calldata _tokenURI) external onlyBridgeOwner {
    ...
    function uri(uint256 _tokenId) external view override returns (string memory) {
        string memory currTokenURI = _tokenURIs[_tokenId];
...
interface IBurnableMintableERC721Token is IERC721 {
    ...
    function setRoyaltyInfo(uint256 _tokenId, address[] calldata _royaltyReceivers, uint256[] calldata _royaltyAmounts) external;
}
