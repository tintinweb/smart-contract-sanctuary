...
contract HowlsCastle is Ownable, ReentrancyGuard {
    ...
    uint256 public calciferHalvingInterval = 21600;
    ...
    //internal - call source
    string internal callSource = "internal";
    ...
    function pendingCALCIFER(uint256 _pid, address _user) public view returns (uint256) {
    ...
    function deposit(uint256 _pid, uint256 _amount, bool _boost, string memory _callSource, address _referrer) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (amount > 0) {
          ...
          if (address(calciferReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
        ...
        if (_boost) {
            ...
            require(keccak256(bytes(_callSource)) == keccak256(bytes(callSource)),'deposit: FORBIDDEN BOOST CALL');
        }

        uint256 boostedAmount = 0;
        uint256 totalRewards = 0;

        (boostedAmount, totalRewards) = calculateBoostedAmount(_pid);
        payOrLockupPendingCALCIFER(_pid, _boost, amount, boostedAmount, totalRewards);
        ...
        if(!_boost && amount > 0) {
            ...
            if(pool.depositFeeBP > 0){
                ...
                if (address(pool.lpToken) == address(CALCIFER)) {
                    totalCALCIFERInPools = totalCALCIFERInPools.add(amount).sub(depositFee);
                }
            } else {
                user.amount = user.amount.add(amount);

                if (address(pool.lpToken) == address(CALCIFER)) {
                    totalCALCIFERInPools = totalCALCIFERInPools.add(amount);
                }
            }
            user.rewardDebt = user.amount.mul(pool.accCALCIFERPerShare).div(1e12).add(boostedAmount);
        }
        else {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        (boostedAmount, totalRewards) = calculateBoostedAmount(_pid);
        payOrLockupPendingCALCIFER(_pid, false, amount, boostedAmount, totalRewards);
    ...
    function calculateBoostedAmount(uint256 _pid) internal view returns (uint256,uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (pending > 0 || user.rewardLockedUp > 0) {
            ...
            totalRewards = pending.add(user.rewardLockedUp);

            //Check if user has Boost and add to total rewards
             if (userBoost[msg.sender] > 0) {
                boostedAmount = totalRewards.mul(userBoost[msg.sender]).div(10000);
                totalRewards = totalRewards.add(boostedAmount);
             }
            //Check if pool boost is enabled and add to total rewards
            if (pool.isBoostEnabled && user.boost > 0) {
                boostedAmount = totalRewards.mul(user.boost).div(10000);
                totalRewards = totalRewards.add(boostedAmount);
            }
        }
        return (boostedAmount, totalRewards);
    }

    // Pay or lockup pending CALCIFER.
    function payOrLockupPendingCALCIFER(uint256 _pid, bool _boost, uint256 _amount, uint256 _boostedAmount, uint256 _totalRewards) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (_totalRewards > 0) {
            ...
            if (_boost || _amount == 0) { // User wanna boost or harvest
                ...
                CALCIFER.mint(address(this), _boostedAmount);
                ...
                if (_boost) {
                    ...
                    uint256 halfRewards = _totalRewards.mul(POOL_BOOST_FEE).div(100);
                    ...
                    uint256 taxRewards = _totalRewards.mul(harvestTaxAmount).div(100);
                    uint256 netRewards = _totalRewards.sub(taxRewards);
                ...
                uint256 pending = user.amount.mul(pool.accCALCIFERPerShare).div(1e12).add(_boostedAmount).sub(user.rewardDebt);
    ...
    // Update callSource by the owner
    function setCallSource(string memory _callSource) public onlyOwner {
