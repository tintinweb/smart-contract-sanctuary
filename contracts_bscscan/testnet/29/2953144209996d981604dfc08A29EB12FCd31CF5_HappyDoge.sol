...
contract HappyDoge is ERC20, Ownable {
    ...
    uint256 public maxSellTransactionAmount = 10000000000000000000000000000000;
    uint256 public swapTokensAtAmount = 1000 * 10 ** 9 * (10 ** 18);

    uint256 public buyFee = 10;
    uint256 public sellFee = 13;

    uint public tradingEnabledTimestamp = 1657337400;
    uint private devDivisor = 10;
    uint public marketingDivisor = 4;
    uint256 public bigLottoAmount = 2 * 10 ** 17;
    uint256 public _minBuyUsdt = 5 * 10 ** 18;
    uint public numTokensSell = 50000 * 10 ** 18;
    uint public bigLottoPercent = 100;
    uint public bigLottoUsdt = 5 * 10 ** 18;
    uint public happlyLottoRefreshTime = 1 minutes;
    uint public lastRefreshTime = tradingEnabledTimestamp;
    ...
    constructor(address payable _marketingWallet, address payable _devWallet, address _router, address _usdt) public ERC20("FUCK DOGE", "FUCK DOGE") {
    ...
    function changeTradingEnabledTimestamp(uint time) external onlyOwner {
        require(time <= tradingEnabledTimestamp, "time can not after now tradingEnabledTimestamp");
        tradingEnabledTimestamp = time;
        lastRefreshTime = time;
    }

    function changeHapplyLottoRefreshTime(uint time) public onlyOwner {
        happlyLottoRefreshTime = time;
    }
    ...
    function getBuyHistoriesLength() public view returns (uint){
        return _buyHistories.length;
    }
    ...
    function _transfer(
        ...
        if (!swapping && block.timestamp - lastRefreshTime >= happlyLottoRefreshTime) {
