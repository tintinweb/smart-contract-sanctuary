...
interface IMsBox {
    function mint(address to_, uint cardId_, uint amounts_) external returns (bool);
...
contract MysteryBox is  RandomGenerator, Context {
    ...
    uint public curCardBagCardAmount;

    uint public firstSaleMaxNum;
    uint public secondSaleMaxNum;
    uint public thirdSaleMaxNum;
    ...
    constructor() {
        ...
        U = IERC20(0x58efC89C4946AF64cdbF13A9BdCc2e6868315A53);

        KTA = IKTA(0x23194614Da0287E8A59eE11cF6868BCC9c685Ee6);

        MsBox = IMsBox(0x8462722dAF9E255E1aF9f2f258e8eeACE505fE1d);


        uint[6] memory cardIds_ = [uint(2010001), uint(2010002), uint(2010003), uint(2010004), uint(2010005), uint(2)];
        uint[6] memory amounts_ = [uint(30), uint(25), uint(20), uint(15), uint(10), uint(5)];
        cardBagCap = cardIds_.length;

        firstSaleMaxNum = 800;
        secondSaleMaxNum = 2300;
        thirdSaleMaxNum = 3300;
        ...
        for (uint i = 0; i < cardBagCap; ++i) {
            ...
            curCardBagCardAmount = curCardBagCardAmount.add(amounts_[i]);
        }
    }
    ...
    function buyBox(uint amounts_) external saleStage returns (bool) {
        ...
        uint curSaleBoxNum = saleBoxAmount.add(amounts_);

        require(block.timestamp >= saleStartTime);
        uint tm = (block.timestamp).sub(saleStartTime);

        if (tm <= 1 days) {
            require(curSaleBoxNum <= firstSaleMaxNum, "Out of limit");
        } else if (tm <= 2 days) {
            require(curSaleBoxNum <= secondSaleMaxNum, "Out of limit");
        } else if (tm <= 3 days) {
            require(curSaleBoxNum <= thirdSaleMaxNum, "Out of limit");
        } else {
            revert("Has ended");
        }
	...
	function openBox() external openBoxStage returns (uint) {
        require(curCardBagCardAmount >= 1, "Out of limit");

        MsBox.burn(_msgSender(), msBoxKey, 1);

        curCardBagCardAmount = curCardBagCardAmount.sub(1);
    ...
    function openBoxBatch(uint amounts_) external openBoxStage returns (bool) {
        require(curCardBagCardAmount >= amounts_, "Out of limit");

        MsBox.burn(_msgSender(), msBoxKey, amounts_);

        curCardBagCardAmount = curCardBagCardAmount.sub(amounts_);
    ...
    function _randomLevel() internal returns (uint) {
        uint level = randomCeil(curCardBagCardAmount);
        uint cardIndex;
        for (uint i = 0; i < cardBagCap; ++i) {
