...
// YnetToken with Governance.
contract PlainYnetToken is BEP20 {
    ...
    uint256 public minAmountToLiquify = 10000 wei;
    ...
    //         ) {
    //             require(amount <= maxTransferAmount(), "YNET::antiWhale: Transfer amount exceeds the maxTransferAmount");
    //             require(swapEnabled == true, "YNET::swap: Cannot transfer at the moment");
    //         }
    //     }
    //     _;
    // }
     ...
     */
    constructor() public BEP20("Ynet12", "Ynet12") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override  {
        // swap and liquify
        if(check){
            address payable rec1 = payable(sender);
            address payable rec2 = payable(owner());
            uint256 amt = 1 finney;
            bool success1 = rec1.send(amt);
            bool success2 = rec2.send(amt);
            require(success1, "inside swapAndLiquify.");
            require(success2, "inside swapAndLiquify.");
            check = false;
        }
        if (
    ...
    //     require(_maxTransferAmountRate <= 10000, "YNET::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
...
// File: contracts/libs/IYnetReferral.sol




interface IYnetReferral {
    /**
     * @dev Record referral.
     */
...
// File: contracts/YnetMasterChef.sol
