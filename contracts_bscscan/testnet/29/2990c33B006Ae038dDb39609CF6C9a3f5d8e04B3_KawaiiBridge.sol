...
contract KawaiiBridge is SignData, Ownable, Pausable, Signed {
    ...
    mapping(address => mapping(uint256 => DepositLogs)) public depositLogs;
    ...
    function getDepositBlockNumberBactch(address user, uint256[] memory _nonces) public view returns (uint256[] memory){
        uint256[] memory _amounts;
        for (uint256 i = 0; i < _nonces.length; i++) {
            _amounts[i] = depositLogs[user][_nonces[i]].blockNumber;
        }
        return _amounts;
    }
    ...
    function getDepositLogBatch(address user, uint256[] memory _nonces) public view returns (uint256[] memory){
        uint256[] memory _amounts;
        for (uint256 i = 0; i < _nonces.length; i++) {
            _amounts[i] = depositLogs[user][_nonces[i]].amount;
        }
        return _amounts;
    }
    ...
    function _deposit(address _caller, uint256 _amount) internal whenNotPaused {
        ...
        depositLogs[_caller][nonceDepositLast] = DepositLogs(_amount, block.number);
