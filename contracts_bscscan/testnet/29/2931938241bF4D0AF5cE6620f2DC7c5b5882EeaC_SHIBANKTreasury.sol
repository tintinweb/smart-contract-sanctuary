...
contract SHIBANKTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SSHBK }

    address public immutable SHBK;
    ...
    address public sSHBK;
    uint public sSHBKQueue; // Delays change to sTAC address
    ...
    /**
        @notice allow approved address to deposit an asset for SHBK
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint SHBK needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( SHBK ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn TAC for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = _amount;
        ITACERC20( SHBK ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sSHBK ).balanceOf( msg.sender ); // Can only borrow against sTAC held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with TAC
        @param _amount uint
     */
    function repayDebtWithTAC( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ITACERC20( SHBK ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, SHBK, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( SHBK ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( SHBK ).totalSupply().sub( totalDebt ) );
    }
    ...
    // function setReserveToken(address _token) external onlyManager()
    // {
    //     isreserveToken[_token] = true;
    // }

    /**
        @notice returns TAC valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match TAC decimals
            value_ = _amount.mul( 10 ** IERC20( SHBK ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SSHBK ) { // 9
            sSHBKQueue = block.number.add( blocksNeededForQueue );
        } else return false;
