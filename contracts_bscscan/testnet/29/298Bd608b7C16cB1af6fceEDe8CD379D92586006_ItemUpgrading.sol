...
contract ItemUpgrading is MarketAuth, EIP712Upgradeable {
  ...
  event UpgradeSuccess(address indexed _user, uint8 _gene, uint16 _itemClass, uint[] materials, uint _newTokenId, uint _fee, uint _timestamp);
  event UpgradeFailed(address indexed _user, uint8 _gene, uint16 _itemClass, uint[] materials, uint _fee, uint _timestamp);
  ...
  function upgradeItem(
    ...
    bool success = _upgradeItem(_itemClass, _materials, _acceptedRatio, _seed, ownPriceFeeWhenSuccess, fee);
    _finishUpgrade(success, ownPriceFeeWhenSuccess, _materials, willKeepMaterialIdIfFailed, _tokenIdFormula);
  }
  ...
  function _upgradeItem(
    ...
    if (success) {
      uint newTokenId = itemNft.mintItem(msg.sender, gene, _itemClass, _ownPrice, 0);
  ...
  function _finishUpgrade(
    ...
    if (!_success) {
      ...
      for(uint i = 0; i < _materials.length; i++) {
        if (_materials[i] == _willKeepMaterialIdIfFailed) {
