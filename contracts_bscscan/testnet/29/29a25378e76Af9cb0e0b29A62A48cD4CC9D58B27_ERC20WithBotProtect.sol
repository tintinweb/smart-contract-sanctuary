...
contract ERC20WithBotProtect is ERC20Capped, ERC20Burnable, Ownable, Pausable {
    ...
    function setOperator(address _operator, bool isOperator) public onlyOwner {
        ...
        (bool isOperatorBefore, uint256 indexInArr) = ArrayLib
            .checkExistsInArray(listOperators, _operator);

        if (isOperator && !isOperatorBefore) {
            listOperators.push(_operator);
        }
        if (!isOperator && isOperatorBefore) {
            ArrayLib.removeOutOfArray(listOperators, indexInArr);
        }
...
library ArrayLib {
    ...
    function checkExistsInArray(string[] memory _arr, string memory _str)
        internal
        pure
        returns (bool isExist, uint256 index)
    {
    ...
    function removeOutOfArray(string[] storage _arr, uint256 _index)
        internal
