...
contract FeeManagerBSC is FeeManager, Ownable {
    ...
    /// @notice address of LP mint destination
    ...
    /// @notice amount of tokens needed to mint LP
    uint256 public minTokensToLp;
    ...
    function _addTokensToLiquidity(uint256 tokensAmount, uint256 usdAmount) private {
        if (tokensAmount != 0 && usdAmount != 0) {
            address destination = (beneficiary != address(0)) ? beneficiary : address(this);
            // omit revert, let's use those tokens on the next swap
            try
    ...
    function setMinTokensToLp(uint256 newMinTokensToLp) public onlyOwner {
        require(newMinTokensToLp >= 2, "not less then 2");
        minTokensToLp = newMinTokensToLp;
    }
}
