...
contract IBGTOKEN is BEP20('IBGTOKEN', 'IBG') {
    
    uint256 public MAX_SUPPLY = 45000000 *10**18;
...
// MasterChef is the master of IBG. He can make IBG and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once IBG is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of IBGs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accIBGPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accIBGPerShare` (and `lastRewardBlock`) gets updated.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 ibgReward = multiplier.mul(ibgPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        ibg.mint(devaddr, ibgReward.div(10));
        ibg.mint(address(this), ibgReward);
