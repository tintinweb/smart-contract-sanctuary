...
contract Token is IERC20, Ownable, Pausable {
    ...
    uint256 constant public MAX_TX_LIMIT = 5000 * 1E9;
    ...
    function calculateDonationFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(donationFee).div(10**2);
    }
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tDonation, uint256 currentRate) private pure returns (uint256, uint256, uint256, uint256) {
    ...
    function burn(uint256 amount) external onlyOwner() {
        require(amount > 0, "Burn amount must larger than zero.");
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) private {
        require(!isBlacklisted[from], "Transfer from blacklisted address");
        require(!isBlacklisted[to], "Transfer to blacklisted address");
        require(!isBlacklisted[tx.origin], "Transfer called from blacklisted address");

        if (hasBotControl && !isWhitelistFromBotControl[tx.origin]) {
            require(amount <= MAX_TX_LIMIT, "Transfer amount exceeds limit");
            require(block.timestamp - lastTransferedAt[tx.origin] >= 10 seconds, "Transfer within cooldown");
