...
contract WedexChef is Ownable, ReentrancyGuard {
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            uint256 WedexReward = multiplier.mul(pool.fixedApr).div(BLOCKS_PER_YEAR.mul(100)).mul(getLPPrice(_pid)).div(getWexPrice()).mul(1e12);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 wedexReward = multiplier.mul(pool.fixedApr).div(BLOCKS_PER_YEAR.mul(100)).mul(getLPPrice(_pid)).div(getWexPrice()).mul(pool.totalAmount).mul(1e12);
    ...
    function payDirectCommission(uint256 _pid,uint256 _amount, bool vipBranch, uint256 leaderCommission, address referrer) internal {
        ...
        if(vipBranch){
            wedex.mint(
                address(referrer),
                _amount.mul(lpPrice).div(1e2).mul(uint256(10).sub(leaderCommission))
            );
            wedexReferral.recordReferralCommission(referrer,_amount.mul(lpPrice).div(1e2).mul(uint256(10).sub(leaderCommission)));
            wedex.mint(
                getLeader(msg.sender),
                _amount.mul(lpPrice).div(1e2).mul(leaderCommission)
            );
            ...
            wedex.mint(
                address(referrer),
                _amount.div(1e2).mul(7)
            );
            wedexReferral.recordReferralCommission(referrer,_amount.div(1e2).mul(7));
            address upperVip =  getUpperVip(msg.sender);
            if(upperVip!=address(0)){
                wedex.mint(
                    upperVip,
                    _amount.div(1e2).mul(3)
                );
                wedexReferral.recordReferralCommission(upperVip,_amount.div(1e2).mul(3));
            }
        }
    }
