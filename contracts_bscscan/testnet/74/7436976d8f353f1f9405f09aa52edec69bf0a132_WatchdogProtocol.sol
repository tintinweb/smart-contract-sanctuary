...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minPeriod = 1 seconds; // min 1 hour delay // debug
    uint256 public minDistribution = 1 * (10 ** 14); // 1 BUSD minimum auto send // debug
...
contract WatchdogProtocol is IBEP20, Auth {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        bool setShareEnabled_ = setShareEnabled;
        if (setShareEnabled_ && !isDividendExempt[sender]) { try distributor.setShare(sender, senderBal.sub(amount)) {} catch {} }
        if (setShareEnabled_ && !isDividendExempt[recipient]) { try distributor.setShare(recipient, recipientBal.add(amountReceived)) {} catch {} }
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _utilityFee, uint256 _feeDenominator, uint256 _totalSellFee) external {
        authorizedFor(Permission.AdjustVariables);
        ...
        totalBuyFee = _liquidityFee.add(_reflectionFee).add(_utilityFee);
    ...
    function setSetShareEnabled(bool enabled) external {
        authorizedFor(Permission.AdjustVariables);
