...
contract CheesePrivateStakePool is WhiteList, ReentrancyGuard {
    ...
    modifier updateRewardPerShare() {
        if (totalStake > 0 && block.timestamp > lastBlockTimeStamp) {
            ...
            //TODO:这个校验的原因
            //currentRewardShare里面会调用prePoolMint，会调用preMint
            //poolMint里面会调用_updateTotalAmount，会调用preMint
            //preMint是动态的，但是这两个函数中间有时间差，有可能不一致，所以要强制校验一下
            require(
    ...
    function stake(uint256 amount)
        ...
        onlyWhitelisted
        nonReentrant
        updateRewardPerShare() //更新当前总奖励，rewardPerShare
        updateUserReward(msg.sender) //根据rewardPerShare，更新user的奖励&负债
    {
        if (amount > 0) {
