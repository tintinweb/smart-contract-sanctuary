...
contract BoxNFT is ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable{
  ...
  function createBox(address owner,uint256 typeNFT, uint256 rank) public onlyOwner returns (uint256) {
        _tokenIds.increment();
        ...
        boxes[owner][newBoxId] = encode(Box(newBoxId,typeNFT,rank));
        return newBoxId;
    }
 ...
 function createRandomThreeBox(address owner) public {
        ...
        uint256 typeNFTRandom = (value << 30+i*2)%10+1;
        uint256 rankRandom = (value << (60+i*3))%2 +1;

        boxes[owner][newBoxId] = encode(Box(newBoxId,typeNFTRandom,rankRandom));
        }
    }
  ...
  function getBox(address owner, uint256 id) public view returns (Box memory _box) {
    ...
    _box.typeNFT = uint256(uint16(details>>148));
    _box.rank = uint256(uint16(details>>164));
  }
...
function transfer(uint256 _nftId, address _target)
        external
    {
        ...
        boxes[ownerOf(_nftId)][_nftId]= encode(Box(0,0,0));
...
function buyBox(uint256 rank) public payable returns (bool) {
        require(msg.value==prices[rank],'Not equal value to buy');
            uint256 value = uint256(block.timestamp);
            uint256 randomCharacterType = (value << 30)%10+1;
            characterNFT.createCharacter(msg.sender,randomCharacterType,rank,1,0);
            diceNFT.createDice(msg.sender,rank);
        return true;
    }
...
function openBox(uint256 tokenId) external {
        ...
        if(box.typeNFT==1){
            uint256 randomCharacterType = (value << 30)%10+1;
            characterNFT.createCharacter(owner,randomCharacterType,box.rank,1,0);
        }
        // else if(box.typeNFT==2){
        // }
        _burn(tokenId);
    }
