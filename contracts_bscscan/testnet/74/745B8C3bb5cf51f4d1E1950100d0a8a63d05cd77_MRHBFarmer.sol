...
import "./MRHBToken.sol";
import "hardhat/console.sol";

contract MRHBFarmer is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        if (block.number <= pool.lastRewardBlock || availableRewards == 0) {
            return;
        }
        uint256 lpSupply;
        if (address(pool.lpToken) != address(token)) {
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending = user
                ...
                .sub(user.rewardDebt);
            if (pending > 0) {
                rewardTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
...
contract MRHBToken is ERC20Burnable {
