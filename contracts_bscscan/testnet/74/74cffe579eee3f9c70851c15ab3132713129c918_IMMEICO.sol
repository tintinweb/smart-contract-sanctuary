...
contract IMMEICO is Context, Ownable {
    IBEP20 IMMEToken;
  ...
  constructor()  {
       IMMEToken = IBEP20(0x6Baa59eb9827C268b0EDB418A25f03292A24C314);
  }
   ...
   */
   function Inverst() public payable{
       ...
       userInfo[msg.sender].tokenWindrwal=(((msg.value*(10**18))/rate)).add(userInfo[msg.sender].tokenWindrwal);
   ...
   function Claim() public{
      ...
      if(finialzie){
          ...
          require(address(this).balance>softcap,"Minimum Amount Not Crossed");
          require(address(this).balance< hardcap,"Maximum amount Reached");
          require(block.timestamp>endDate,"ICO Is Not Completed Yet");
          bool found=false;
        for (uint i=0; i<_useraddress.length; i++) {
            if(_useraddress[i]==msg.sender){
                found=true;
                break;
            }
        }
        if(found){
            IMMEToken.transferFrom(owner(),msg.sender,userInfo[msg.sender].tokenWindrwal);
        }
        ...
        require(airdrop!=0, "No Airdrop started yet");
            ...
            for (uint8 i=0; i < _airaddress.length; i++) {
                ...
                require(_isExist==false, "Already Dropped");
                    IMMEToken.transferFrom(owner(), msg.sender, airdrop*(10**18));
                     IMMEToken.transferFrom(owner(), ref_address, ((airdrop*(10**18)*rewards)/100));
                    _airaddress.push(msg.sender);
                
    return true;
    }
    
    function ReStartICO(uint256 _softcap,uint256 _hardcap, uint256 _endDate,uint256 _rate) public onlyOwner returns(bool){
        softcap=_softcap;
        hardcap=_hardcap;
        startDate=block.timestamp;
        endDate=_endDate;
        rate=_rate;
        hasStart=true;
        for(uint256 index;index<=_useraddress.length;index++){
