...
contract elfinStaking is Ownable {
    ...
    struct PoolInfo {
        ...
        uint tokenApyFor30Day;
        uint tokenApyFor60Day;
        uint tokenApyFor90Day;
        uint tokenApyFor180Day;
        uint elfinApyFor30Day;
        uint elfinApyFor60Day;
        uint elfinApyFor90Day;
        uint elfinApyFor180Day;
    ...
    constructor(address elfinToken, address[] memory _token, uint256[] memory _totalLimit, uint256[] memory _minStakingAmount) public {
        _owner  = msg.sender;
        _cmo    = msg.sender;
        _elfinToken = IERC20(elfinToken);
    ...
    function staking(uint token20Index, uint256 amount, uint lockDay) external {
        PoolInfo storage pool = poolInfo[token20Index];
        require(amount >= pool.minStakingAmount, "Amount is too small");
        require(lockDay == 30 || lockDay == 60 || lockDay == 90 || lockDay == 180 ,"Lock day only can be 30/60/90/180");


        require(pool.stakingToken != IERC20(address(0)), "Token index is wrong");
    ...
    function withdraw(uint userStakingIndex) external {
        ...
        uint elfinApy = 0;
        if(info.lockDay == 30) {
            elfinApy = pool.elfinApyFor30Day;
        } else if(info.lockDay == 60) {
            elfinApy = pool.elfinApyFor60Day;
        }else if(info.lockDay == 90) {
            elfinApy = pool.elfinApyFor90Day;
        }else if(info.lockDay == 180) {
            elfinApy = pool.elfinApyFor180Day;
        }
        if(elfinApy > 0) {
            _elfinToken.safeTransfer(msg.sender,((info.stakingAmount).mul(elfinApy)).div(10000));
        }
        
        uint tokenApy = 0;
        if(info.lockDay == 30) {
            tokenApy = pool.tokenApyFor30Day;
        } else if(info.lockDay == 60) {
            tokenApy = pool.tokenApyFor60Day;
        }else if(info.lockDay == 90) {
            tokenApy = pool.tokenApyFor90Day;
        }else if(info.lockDay == 180) {
            tokenApy = pool.tokenApyFor180Day;
        }

        if(tokenApy > 0) {
            pool.stakingToken.safeTransfer(msg.sender,info.stakingAmount.add(((info.stakingAmount).mul(tokenApy)).div(10000)));
        } else {
    ...
    function setStakingPool(uint index, address _newToken, uint256 _newTotalLimit, uint256 _minStakingAmount
    , uint _tokenApyFor30Day, uint _tokenApyFor60Day, uint _tokenApyFor90Day, uint _tokenApyFor180Day
    , uint _elfinApyFor30Day, uint _elfinApyFor60Day, uint _elfinApyFor90Day, uint _elfinApyFor180Day) external onlyCmo {
        ...
        require(_tokenApyFor30Day <= 10000 && _tokenApyFor30Day >= 0);
        require(_tokenApyFor60Day <= 10000 && _tokenApyFor60Day >= 0);
        require(_tokenApyFor90Day <= 10000 && _tokenApyFor90Day >= 0);
        require(_tokenApyFor180Day <= 10000 && _tokenApyFor180Day >= 0);
        require(_elfinApyFor30Day <= 10000 && _elfinApyFor30Day >= 0);
        require(_elfinApyFor60Day <= 10000 && _elfinApyFor60Day >= 0);
        require(_elfinApyFor90Day <= 10000 && _elfinApyFor90Day >= 0);
        require(_elfinApyFor180Day <= 10000 && _elfinApyFor180Day >= 0);
        pool.tokenApyFor30Day = _tokenApyFor30Day;
        pool.tokenApyFor60Day = _tokenApyFor60Day;
        pool.tokenApyFor90Day = _tokenApyFor90Day;
        pool.tokenApyFor180Day = _tokenApyFor180Day;
        pool.elfinApyFor30Day = _elfinApyFor30Day;
        pool.elfinApyFor60Day = _elfinApyFor60Day;
        pool.elfinApyFor90Day = _elfinApyFor90Day;
        pool.elfinApyFor180Day = _elfinApyFor180Day;
    }


    function setDuration(uint _duration) external onlyCmo {
        require(duration > 0, "Duration must gather then 0");
        duration = _duration;
    }
    ...
    function setElfinToken(address elfintoken) public onlyCmo {
        _elfinToken = IERC20(elfintoken);
    }
