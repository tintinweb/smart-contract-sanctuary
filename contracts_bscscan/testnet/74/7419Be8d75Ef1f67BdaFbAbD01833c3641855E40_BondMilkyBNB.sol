...
interface IPriceData {
    ...
    function getPrice(string memory _base) external view returns (PriceData memory);
}
...
contract SignData {
    ...
    constructor() public {
        ...
        DEPOSIT_HASH = keccak256("Data(address depositor,uint256 nonce)");
    }
...
contract BondMilkyBNB is Pausable, SignData {
    ...
    function depositPermit(address _depositor, address sender, uint8 v, bytes32 r, bytes32 s) external payable whenNotPaused returns (uint){
        uint256 _amount = msg.value;
        IWBNB(principle).deposit{value : _amount}();
        verify(keccak256(abi.encode(DEPOSIT_HASH, _depositor, nonces[sender]++)), sender, v, r, s);
        require(_depositor != address(0), "Invalid address");
        //Calculate lp token to milky
        uint value = _amount.mul(10 ** 18).div(uint256(IPriceData(calculator).getPrice("MILKY").rate));
        //Apply discount
    ...
    function deposit(address _depositor) external payable whenNotPaused returns (uint){
        ...
        require(_depositor != address(0), "Invalid address");
        //Calculate lp token to milky 
        uint value = uint256(IPriceData(calculator).getPrice("MILKY").rate).mul(_amount).div(10 ** 18);
        //Apply discount
        uint payout = payoutFor(value);
        // must be > 0.01 milky ( underflow protection )
        require(payout <= maxPayout(), "Bond too large");
