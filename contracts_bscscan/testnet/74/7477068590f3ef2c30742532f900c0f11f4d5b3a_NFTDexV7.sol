...
contract NFTDexV7 is ERC721Full, ERC721MetadataMintable, Ownable {
  IERC20 ALIA;
  ...
  mapping (string=> uint256) tokenSellTimeBlindbox;
  ...
  function sellNFT(address nft_a,uint256 tokenId, address seller, uint256 price, uint256 priceDollar) public{
    ...
    // require(revenueAddressBlindBox[boxNameByToken[tokenId]] == address(0x0), "112");
...
function placeBid(uint256 _tokenId, uint256 _amount, bool awardType, address from)  public{
  ...
  function claimAuction(uint256 _tokenId, bool awardType, string memory ownerId, address from) public {
    ...
    if(revenueAddressBlindBox[boxNameByToken[_tokenId]] == address(0x0)){
          ...
          ALIA.transfer(temp.seller, (temp.bidAmount  / 1000) * mainPerecentage);
          ALIA.transfer( revenueAddressBlindBox[boxNameByToken[_tokenId]], (temp.bidAmount  / 1000) * authorPercentage);
    }
  ...
  function buyNFT(address nft_a,uint256 tokenId, string memory ownerId) public{
        ...
        if(revenueAddressBlindBox[boxNameByToken[tokenId]] == address(0x0)){
          ...
          authorPercentage =135;
          ALIA.transferFrom(msg.sender, temp.seller, (price  / 1000) * mainPerecentage);
          ALIA.transferFrom(msg.sender, revenueAddressBlindBox[boxNameByToken[tokenId]], (price  / 1000) * authorPercentage);
