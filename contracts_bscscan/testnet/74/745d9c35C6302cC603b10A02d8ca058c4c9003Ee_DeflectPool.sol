...
// boostedbalanceOf returning 0 (staking balance of the account)

// DEV NOTE: Add/edit documentation
// DEV NOTE: RE-ORGANISE FUNCTIONS FOR GAS EFFICINECY
...
contract DeflectPool is LPTokenWrapper {
    ...
    address public immutable devFund;
        ...
        //  Pool Array                          - stores all the reward pools, each pool has the following data stored in it:
        //      rewardTokenAddress
        //      duration
        //      periodFinish
        //      lastUpdateTime
        ...
        //      UserRewardInfo                      - stores the users personal info in relation to the they are in
        //          rewards
    ...
    function getRewardPoolAddress(uint _pid) internal view returns (address addr) {
        PoolInfo storage pool = poolInfo[_pid];
        address poolAddr = address(pool.rewardTokenAddress);
        return poolAddr;
    }

    // Adds a new reward pool with specified duration
    ...
    // Removes a specific pool in the array, leaving the pid slot empty
    ...
    // Removes all reward pools [NOT WORKING: Error: Fail]
    ...
    // Returns the amount of tokens needed to purchase the boost level input
    ...
    // Ejects any remaining tokens from the reward pool specified.
    ...
    // Ejects any remaining tokens from all reward pools [NOT WORKING: Error: Fail]
