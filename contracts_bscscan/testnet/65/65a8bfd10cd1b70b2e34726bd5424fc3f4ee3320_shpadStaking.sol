...
//sdfsd
    ...
    //Функция разлока
    function withdrow(uint timeInput) public returns(uint[] memory amt, uint[] memory time, uint[] memory typeStaked, uint[] memory activeStake, uint[] memory endTimeStake, uint length) {
        BEP20 token = BEP20(stakeTokenAddr);
