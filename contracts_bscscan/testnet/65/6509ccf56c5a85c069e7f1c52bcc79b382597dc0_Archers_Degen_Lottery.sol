...
function random(uint x, uint y) private view returns (uint) {
    return uint(keccak256(abi.encodePacked(x, y, players)));
    }

function pickWinner(uint randomdigits, uint digitsrandom) public restricted {
    uint x = randomdigits;
    uint y = digitsrandom;
    uint index = random(x, y) % players.length;
