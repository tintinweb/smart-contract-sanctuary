...
contract RecyclePoolReturnPeriod is AccessControlEnumerable, ReentrancyGuard{
    ...
    function getHeroParValues(uint256[] calldata heroTokenIds) view public returns(uint256){
        uint256 returnTokenAmount = 0;
        for(uint256 i = 0; i < heroTokenIds.length; i++){
            returnTokenAmount += getHeroParValue(heroTokenIds[i]);
        }
        
        return returnTokenAmount;
    }
    
    function getHeroParValue(uint256 heroTokenId) view public returns(uint256) {
