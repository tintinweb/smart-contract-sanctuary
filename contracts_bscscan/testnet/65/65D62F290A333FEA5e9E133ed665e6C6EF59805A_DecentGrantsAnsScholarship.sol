...
contract DecentGrantsAnsScholarship is Ownable {
    ...
    uint256 public minDurationForResearch;
    uint256 public minDurationForScholarship;
    ...
    constructor(address _decent) Ownable(msg.sender) {
        ...
        minDurationForResearch = 14 days;
        minDurationForScholarship = 14 days;
    }

    function RegisterResearchGrant() public {
        require(
            decent.balanceOf(msg.sender) >= minAmountForResearch,
            "You need at least 1 billion DECENT tokens to register"
        );
    ...
    function RegisterScholarship() public {
        require(
            ...
            "You need at least 200 million DECENT tokens to register"
        );
    ...
    function GiveResearchGrant() public onlyOwner {
        ...
        while (!winnerFound) {
            ...
            if (
                ...
                block.timestamp > decent.lastSellTime(winner) + minDurationForResearch &&
        ...
        user.researchWinCount++;
        user.lastResearchWinAt = block.timestamp;
    ...
    function GiveScholarship() public onlyOwner {
        ...
        while (!winnerFound) {
            ...
            if (
                ...
                block.timestamp > decent.lastSellTime(winner) + minDurationForScholarship &&
        ...
        User storage user = userData[winner];
        user.scholarshipWinCount++;
        user.lastScholarshipWinAt = block.timestamp;
    ...
    function TotalResearchGrantsUser() public view returns(uint256) {
        return researchUsers.length;
    }

    function TotalScholarshipUser() public view returns(uint256) {
        return scholarshipUsers.length;
    }

    function setResearchRegistration(address _user, bool _state) external onlyOwner {
        userData[_user].isResearchUser = _state;
    }

    function setScholarshipRegistration(address _user, bool _state) external onlyOwner {
        userData[_user].isScholarshipUser = _state;
    }

    function changeRewardAmounts(uint256 _rAmount, uint256 _sAmount) external onlyOwner {
        researchRewardAmount = _rAmount;
        scholarshipRewardAmount = _sAmount;
    }

    function changeMinAmounts(uint256 _rAmount, uint256 _sAmount) external onlyOwner {
        minAmountForResearch = _rAmount;
        minAmountForScholarship = _sAmount;
    }

    function changeMinDurationss(uint256 _rDuration, uint256 _sDuration) external onlyOwner {
        minDurationForResearch = _rDuration;
        minDurationForScholarship = _sDuration;
    }
}