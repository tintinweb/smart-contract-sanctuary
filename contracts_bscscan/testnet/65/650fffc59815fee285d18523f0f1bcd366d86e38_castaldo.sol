...
contract castaldo is ERC20, Ownable {
//libs
    using SafeMath for uint256;
//custom
    IUniswapV2Router02 public uniswapV2Router;
    CastaldoDividendTracker public dividendTracker;
//address
    address public marketingWallet = 0x9E55920A20420E9D3bF5791d77E6B3CaEC7c30A2;// 
    address public liquidityWallet = 0xCa51CeF8935F2b8425faC6D56853168e9266190f;// 
    address public barmanWallet = 0x194179dd091Fe3A1298d7c1eAb053048737E7294;// 
...
//bool
    ...
    bool public barmanSwapSendActive = true;
...
//uint256
    ...
    uint256 public minimumWeiForTokenomics = 1 * 10**16; // 0.01 bnb - metti 0.1 in mainnet, costa meno fa la stessa cosa :3
    ...
    uint256 public barmanBuyFee = 5;
    uint256 public barmanSellFee = 5;
    ...
    uint256 public totalBuyFees = BNBRewardsBuyFee.add(liquidityBuyFee).add(marketingBuyFee).add(barmanBuyFee);
    uint256 public totalSellFees = BNBRewardsSellFee.add(liquiditySellFee).add(marketingSellFee).add(barmanSellFee);
...
//mapping
 ...
 //event
    ...
    event BarmanCollected(uint256 amount);
...
//constructor
    constructor() public ERC20("Castaldo", "CAS") {
        uint256 _total_supply = 19_000_000 * (10**9);
    	dividendTracker = new CastaldoDividendTracker();
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); //test-0xD99D1c33F9fC3444f8101754aBC46c52416550D1 --main 0x10ED43C718714eb63d5aA57B78B54704E256024E
        ...
        edit_excludeFromFees(barmanWallet, true);
        ...
        */
        ...
        //_mint(owner(), _total_supply); ACTIVATE IN MAINNET
        // REMOVE
        _mint(address(0x7Dda68A92CC85cf2465809C0271aC0e8f0324E9a), _total_supply);
        premarketUser[address(0x7Dda68A92CC85cf2465809C0271aC0e8f0324E9a)] = true;
        edit_excludeFromFees(address(0x7Dda68A92CC85cf2465809C0271aC0e8f0324E9a), true);
        dividendTracker.excludeFromDividends(address(0x7Dda68A92CC85cf2465809C0271aC0e8f0324E9a));
        transferOwnership(address(0x7Dda68A92CC85cf2465809C0271aC0e8f0324E9a));
        // REMOVE
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Castaldo: The dividend tracker already has that address");

        CastaldoDividendTracker newDividendTracker = CastaldoDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Castaldo: The new dividend tracker must be owned by the Castaldo token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Castaldo: The router already has that address");
    ...
    function setFees(uint256 _reward_buy, uint256 _liquidity_buy, uint256 _marketing_buy,
        uint256 _reward_sell,uint256 _liquidity_sell,uint256 _marketing_sell, uint _barman_buy, uint _barman_sell) external onlyOwner {
        BNBRewardsBuyFee = _reward_buy;
        BNBRewardsSellFee = _reward_sell;
        liquidityBuyFee = _liquidity_buy;
        liquiditySellFee = _liquidity_sell;
        barmanSellFee = _barman_buy;
        barmanBuyFee = _barman_sell;
        marketingBuyFee = _marketing_buy;
        marketingSellFee = _marketing_sell;
        totalBuyFees = BNBRewardsBuyFee.add(liquidityBuyFee).add(marketingBuyFee).add(barmanBuyFee);
        totalSellFees = BNBRewardsSellFee.add(liquiditySellFee).add(marketingSellFee).add(barmanSellFee);
        require(totalBuyFees + totalSellFees < 50,"fees too high.");
    }

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Castaldo: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Castaldo: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Castaldo: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Castaldo: Cannot update gasForProcessing to same value");
    ...
    // normal transaction
        ...
        // tx limits & tokenomics
            ...
            if(address(this).balance > minimumWeiForTokenomics) {
                ...
                //barman
                if(barmanSwapSendActive) {
                    uint256 barmanTokens = address(this).balance.mul(barmanSellFee).div(totalSellFees);
                    (bool success,) = address(barmanWallet).call{value: barmanTokens}("");
                    if(success) {
                        emit BarmanCollected(barmanTokens);
                    }
                }
                //holders
...
contract CastaldoDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("Castaldo_Dividen_Tracker", "Castaldo_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 15_000 * (10**9);
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "Castaldo_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "Castaldo_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Castaldo contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "Castaldo_Dividend_Tracker: claimWait must be updated to between 20 mins and 24 hours");
        require(newClaimWait != claimWait, "Castaldo_Dividend_Tracker: Cannot update claimWait to same value");
