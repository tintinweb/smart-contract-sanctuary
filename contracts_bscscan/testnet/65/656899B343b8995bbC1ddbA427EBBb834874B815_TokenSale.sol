...
contract TokenSale is AccessControl, Whitelist, Finalizable, Initializable {
    ...
    function initialize(address _usdToken, address _wallet, address _priceFeedAddress, address _vestingVault) initializer public {
    ...
    function addToWhitelist(address account) public isInitialized onlyRole(ADMIN_ROLE) {
    ...
    function removeFromWhitelist(address account) public isInitialized onlyRole(ADMIN_ROLE) {
    ...
    function purchaseInUSD(address beneficiary, uint256 purchaseAmount) public isTokenSaleInitialized isOpen isWhitelisted(beneficiary) {
        ...
        _addPurchaser(beneficiary);



        _prePurchaseValidate(beneficiary, purchaseAmount, USDperTokens(tokens));



        USDRaised = USDRaised + purchaseAmount;

        _postPurchaseProcess(beneficiary, USDperTokens(tokens));



        usdToken.transferFrom(msg.sender, address(this), purchaseAmount);
        ...
        vestingVault.addVesting(beneficiary, totalTokens, address(this));
        ...
        emit TokensPurchased(msg.sender, beneficiary, purchaseAmount, totalTokens);

    }



    function purchaseInBNB(address beneficiary) public payable isTokenSaleInitialized isOpen isWhitelisted(beneficiary) {
    ...
    function withdrawUSD() external isTokenSaleInitialized canWithdrawFunds onlyRole(ADMIN_ROLE) { 

        withdrawTokens(address(usdToken));      
    ...
    function capRaisedUSD() public view returns (uint256) {
        ...
        return _amountRaisedInUSD / 10 ** 18;
    ...
    function capToRaiseUSD() public view returns (uint256) {
        ...
        return (hardCap - _amountRaisedInUSD) / 10 ** 18;
    ...
    function tokensPerBNB(uint256 _amount) public view returns (uint256) {
            ...
            return BNBPrice()* USDRate / 10 ** BNBPriceFeed.decimals() * _amount;
    ...
    function USDperTokens(uint256 _tokens) internal view returns (uint256) {
