...
contract TESTCAKE7 is ERC20, Ownable {
    ...
    RewManagerInterface public MDIV = RewManagerInterface(0xf281a09C81a2618Eea56C60CB9fA39763C3d1319); // TODO
    ...
    uint256 private _maxRewSellFee = 36;
    ...
    event FinalFeeCalc(uint256 totalFeeTokens, uint256 origAmount, uint256 amount);

    constructor() public ERC20("TEST CAKE 7", "TESTCAKE7") {
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[to]) {
                ...
                uint256 daysSinceLastBuy = block.timestamp.sub(lastBuy[from]).div(5 minutes);
        	...
        	emit FinalFeeCalc(totalFeeTokens, origAmount, amount);
        }
...
contract RewDividendTracker is Ownable, ERC20 {
    ...
    event RDT_AutoClaiming(address account);
    event RDT_ClaimSucceeded(address account);
    event RDT_ClaimFailed(address account);
    ...
    function process(uint256 gas) public returns (uint256, uint256, uint256) {
    	...
    	while(gasUsed < gas && iterations < numberOfTokenHolders) {
    		...
    		if(canAutoClaim(lastClaimTimes[account])) {
    		    emit RDT_AutoClaiming(account);
    			if(processAccount(payable(account), true)) {
    			    emit RDT_ClaimSucceeded(account);
    				claims++;
    			} else {
    			    RDT_ClaimFailed(account);
    			}
    		}
