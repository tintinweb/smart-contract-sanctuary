...
contract StakeEarn is Ownable, ReentrancyGuard {
    ...
    function initialize(IBEP20 _stakedToken, IBEP20 _rewardToken, uint256 _rewardPerStake, uint256 _startTime, uint256 _endTime, uint256 _minStakePerUser, uint256 _maxStakePerUser, uint256 _poolStakeLimit, uint256 _minTimeToStake) external onlyOwner{
		require(_endTime >= _startTime, "Inccorect end time");
		require(_maxStakePerUser >= _minStakePerUser, "Inccorect max stake per user");
		require(_poolStakeLimit >= _maxStakePerUser, "Inccorect max pool stake limit");
		require(_minStakePerUser >= 0, "Inccorect min stake per user");
		require(_startTime >= block.timestamp, "Inccorect start time");
    ...
    function deposit(uint256 _amount) external nonReentrant {
		...
		limitLeft = limitLeft.sub(_amount);
	...
	function pendingreward(address _user) public view returns (uint256) {
		...
		uint256 eTime = user.endTime;
		uint256 remaining = user.rewardRemaining;
		if(user.endTime > block.timestamp)
		{
		    eTime = block.timestamp;
		}
		uint256 treward = user.amount.div(user.decimal).mul(user.rewardPerStake).mul(eTime.sub(sTime)).div(TIME_STEP);
		return treward.add(remaining);
    }
	
    function withdraw() public nonReentrant {
	    UserInfo storage user = userInfo[msg.sender];
		require(user.amount > 0, "Stake not found");
		require(block.timestamp > user.endTime, "Staking time not complete");
	...
	function getUserInfo(address userAddress) public view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (user.amount, user.rewardRemaining, user.startTime, user.endTime, user.minTime, user.rewardPerStake, user.decimal);
    }
