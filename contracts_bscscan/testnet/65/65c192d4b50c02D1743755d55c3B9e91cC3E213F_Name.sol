...
contract Name is Context, IERC20, Ownable {
    ...
    uint256 public maxTxLimit = 1 * 10**8 * (10**18);
    uint256 public maxWalletLimit = 1 * 10**7 * (10**18);
    ...
    function _takeMarketingAndBurn(uint256 tMarketing, uint256 tBurn) private {
        ...
        emit Transfer(tx.origin, address(this), tMarketing);       
    }
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 total = buyFee.marketing + sellFee.marketing + buyFee.autoLp + sellFee.autoLp;
            uint256 forMarketing = contractTokenBalance.mul(buyFee.marketing + sellFee.marketing).div(total);
        ...
        if(!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]){
            ...
            if (recipient != uniswapV2Pair) {
                require(
                    ...
                    "Transfer amount exceeds hold limit"
                );
            }
        }
    ...
    function setWhaleLimit(uint256 amount) external onlyOwner {
        maxWalletLimit = amount;
    }

    function setTransferLimit(uint256 amount) external onlyOwner {
        maxTxLimit = amount;
    }
