...
abstract contract Merchant is MerchantInt{
    ...
    function getClaimable(address resellerAddress, address memberAddress) public view returns(uint code, uint256 unclaimed){
        ...
        uint span = _hashRates[resellerAddress][memberAddress].expiry > block.timestamp? block.timestamp - _hashRates[resellerAddress][memberAddress].startDate : _hashRates[resellerAddress][memberAddress].expiry - _hashRates[resellerAddress][memberAddress].startDate;
        return (0, span * (_hashRates[resellerAddress][memberAddress].hashRate));
    }
    
    function getHashRateInfo(address reseller, address memberAddress) public view returns(uint256 hashRate, uint expiry, uint startDate, string memory meta, bool locked, bool exist) {
        return (_hashRates[reseller][memberAddress].hashRate,
            ...
            _hashRates[reseller][memberAddress].exist
        );
    }
