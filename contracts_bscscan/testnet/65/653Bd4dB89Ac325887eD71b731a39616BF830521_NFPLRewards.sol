...
contract NFPLToken is ERC20("NiftyPlanet Token", "NFPL"), AdminContract {
    uint256 private maxSupplyPaper = 300000000e14;
...
contract NFPLRewards is Ownable {
    using SafeMath for uint256;
    ...
    event NewLock(uint256 ethAmount, uint256 nfplAmount);

    uint256 public totalTokens = 300000000e13;
    uint256 public totalReleased = 0;

    constructor(NFPLToken _nfpl, address _developers) public {
        nfpl = _nfpl;
    ...
    function getTokensEpoch() public view returns (uint256) {
        ...
        */
        if (now >= 1632247200) {
            return totalTokens.mul(60).div(100);
        }
        if (now >= 1632243600) {
            return totalTokens.mul(30).div(100);
        }
        if (now >= 1632240000) {
            return totalTokens.mul(10).div(100);
        }
        return 0;
    }

    function releaseTokens() external onlyOwner {
        uint256 nfplAmount = getTokensEpoch().sub(totalReleased);
        require(nfplAmount > 0, "Error: Nothing to release");

        nfpl.mintTokens(address(this), nfplAmount);
        nfpl.mintTokens(developers, nfplAmount.div(10));
        totalReleased = totalReleased.add(nfplAmount);
    }
    ...
    function withdraw() external {
        require(
            _balances[msg.sender] > 0 &&
                IERC20(nfpl).balanceOf(address(this)) >= _balances[msg.sender],
