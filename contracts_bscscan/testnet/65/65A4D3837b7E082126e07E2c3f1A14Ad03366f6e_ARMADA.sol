...
abstract contract DepreciatingFees {
    ...
    (uint256, uint256, uint256, uint256, uint256) {
        ...
        (uint256 _estimatedFleetRewardsFee, uint256 _estimatedWarChestFee, uint256 _estimatedMarketingChestFee, 
            uint256 _estimatedMutinyFee, uint256 estimatedAccruedTime) = getEstimatedFee(_userAddress);
    ...
    */
    ...
    function reAdjustFees(address userAddress, uint256 _amount, uint256 accruedTime, uint256 _FleetRewardsFee, uint256 _WarChestFee,  uint256 _MarketingChestFee, 
        ...
        updateFeeHolder( 
            _userAddress,
            _isBuying ? _newFleetRewardsFee : __FleetRewardsFee.add(_accruedTime.mul(FleetRewardsReductionPerSec)), 
            _isBuying ? _newWarChestFee : __WarChestFee.add(_accruedTime.mul(WarChestReductionPerSec)), 
            _isBuying ? _newMarketingChestFee : __MarketingChestFee.add(_accruedTime.mul(MarketingChestReductionPerSec)),  
            _isBuying ? _newMutinyFee : __MutinyFee.add(_accruedTime.mul(MutinyreductionPerSec)),
...
contract ARMADA is IERC20Metadata, DepreciatingFees, Ownable {
    ...
    function _transferTokens(address sender, address recipient, uint256 amount, bool takeFee, bool feeReduction, address userAddress) private {
        ...
        uint256 reduceMutinyFee, uint256 estimatedAccruedFees) = _getHolderFees(userAddress, feeReduction);
        ...
        if(feeReduction) {
            ...
            reAdjustFees(userAddress, amount, estimatedAccruedFees, reduceFleetRewardsFee, reduceWarChestFee, reduceMarketingChestFee, reduceMutinyFee, isBuying);
        }
