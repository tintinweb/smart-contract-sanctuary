...
contract TEST is IBEP20, Ownable {
    ...
    uint256 private _maxTxAmountBuy = _maxWalletAmount / 2; 
    ...
    function _transferTo(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(!isFeeExempt[recipient]){ enterLottery(recipient, amount); }
    ...
    function enterLottery(address recipient, uint256 amount) private {
        if (amount >= 10000) {
    ...
    function pickWinner() public restricted() {
        ...
        if (balanceOf(players[index]) >= 15000) {        
