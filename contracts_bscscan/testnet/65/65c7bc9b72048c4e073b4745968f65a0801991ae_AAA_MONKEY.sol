...
interface IDividendDistributor {
    ...
    function claimDividend(address claimer) external;
    function setRewardToken(address newToken, address oldReceiver) external;
...
contract DividendDistributor is IDividendDistributor {
    ...
    function claimDividend(address claimer) external override onlyToken {
        distributeDividend(claimer);
    }

    function setRewardToken(address newToken, address oldReceiver) external override onlyToken {
        uint256 balance = REWARD.balanceOf(address(this));
        REWARD.transfer(oldReceiver, balance);
...
contract AAA_MONKEY is IBEP20, Auth {
    ...
    string constant _name = "TS";
    string constant _symbol = "TS1.0A";
    ...
    constructor () Auth(msg.sender) {
        router = IDEXRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); // mainnet 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    function processAllBalances(uint256 gas)  external authorized {
        try distributor.process(gas) {} catch {}
    }

    function setRewardToken(address token, address oldReceiver) external authorized {
        distributor.setRewardToken(token, oldReceiver);
    }
