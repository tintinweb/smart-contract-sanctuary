...
contract JINDOINU  is Context, IERC20, Ownable {
    ...
    string private _name = "JINDO INU test";
    string private _symbol = "JINDT";
    ...
    function setextraAddress(address payable extra) public onlyOwner {
        _extraAddress = extra;
    }
    ...
    function showextraaddress() public view returns(address payable) {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateLiquidityPlusextraFee(tAmount);
    ...
    function calculateLiquidityPlusextraFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _extraFee + _marketingAndDevBudget).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingAndDevBudget == 0 && _extraFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalLiqFee = _marketingAndDevBudget + _liquidityFee + _extraFee;
        ...
        // calculate the portions of the liquidity to add to extra
        uint256 extraBalance = newBalance.div(totalLiqFee).mul(_extraFee);
        uint256 extraPortion = otherHalf.div(totalLiqFee).mul(_extraFee);
        ...
        uint256 finalBalance = newBalance.sub(extraBalance).sub(marketingBalance);
        uint256 finalHalf = otherHalf.sub(extraPortion).sub(marketingPortion);
        
        
        
        (bool sent, bytes memory data) = _extraAddress.call{value: extraBalance}("");
        if(sent){
            _tokenTransfer(address(this), 0x0000000000000000000000000000000000000001, extraPortion, false);
            emit Transfer(address(this), 0x0000000000000000000000000000000000000001, extraPortion);
        } else {
            addLiquidity(extraPortion, extraBalance, _extraAddress);
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount, address extra) private {
