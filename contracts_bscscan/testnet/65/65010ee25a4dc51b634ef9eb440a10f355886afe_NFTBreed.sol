...
contract NFTBDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("NFTBD_Dividend_Tracker", "NFTBD_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 20000000 * (10**9); //must hold 1000000+ tokens
    }
    
    function setMinimumTokenBalanceForDividends(uint256 amount) external onlyOwner() returns(bool) {
        minimumTokenBalanceForDividends = amount;
        return true;
    }

    function _transfer(address, address, uint256) internal override pure {
        require(false, "NFTBD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "NFTBD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NFT Breed contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "NFTBD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "NFTBD_Dividend_Tracker: Cannot update claimWait to same value");
...
contract NFTBreed is ERC20, Ownable {
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    NFTBDDividendTracker public dividendTracker;

    address public _marketingWallet = 0x9Ec376b4cE92aA459BF50C4eb96f8bf6F2505389;

    uint256 public maxTxAmount = 100 * 10**9 * (10**9);
    uint256 public swapTokensAtAmount = 50 * 10**9 * (10**9);
    ...
    constructor() ERC20("NFT Breed", "NFTBD") {

        dividendTracker = new NFTBDDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "NFT Breed: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "NFT Breed: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "NFT Breed: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "NFT Breed: Cannot update gasForProcessing to same value");
    ...
    function prepareForLaunch() external onlyOwner {
        updateBuyFees(3,2,6);
        updateSellFees(4,2,6);
        maxTxAmount = 1 * 10**9 * (10**9);
    }

    function prepareForPresale() external onlyOwner {
        updateBuyFees(0,0,0);
        updateSellFees(0,0,0);
        maxTxAmount = 100 * 10**9 * (10**9);
    }
