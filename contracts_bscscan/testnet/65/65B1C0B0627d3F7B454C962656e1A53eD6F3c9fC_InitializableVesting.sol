...
// modify from File: @openzeppelin\contracts\drafts\TokenVesting.sol
 ...
 * owner.
 */
contract InitializableVesting is InitializableOwnable {
    // website https://maskdogecoin.io/
    ...
    function releaseFixedAmount(uint256 _amount) public {
        uint256 unreleased = releasableAmount();
        require(unreleased > 0, "TokenVesting: no tokens are due");
        require(
            _amount > 0 && _amount <= unreleased,
            "TokenVesting: invalid amount"
        );

        _released[address(_token)] = _released[address(_token)].add(_amount);
        token().safeTransfer(_beneficiary, _amount);
        emit TokensReleased(address(_token), _amount);
    }
    ...
    function releasableAmount() public view returns (uint256) {
        return _vestedAmount().sub(_released[address(_token)]);
    }
    ...
    function _vestedAmount() private view returns (uint256) {
        ...
        if (block.timestamp < _cliff) {
            ...
            block.timestamp >= _start.add(_duration) ||
            _revoked[address(_token)]
        ) {
            return totalBalance;
        } else {
