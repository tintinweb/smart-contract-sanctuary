...
contract LuniSwap is VersionedInitializable{
  ...
  uint256 public constant REVISION = 9;
 ...
 function addNewEmploy(address[] calldata employList, string memory _employType) public onlyOwner {
  ...
  function swapLuni(uint256 amountLuni) public {
    ...
    uint256 realUSDTSwap = amountLuni > swapAmountValid ? swapAmountValid : amountLuni;
    uint256 fee = realUSDTSwap.mul(swapFee).div(100);

    IERC20(LUNI_TOKEN).transferFrom(msg.sender, address(this), realUSDTSwap);
    IERC20(USDT_TOKEN).transfer(msg.sender, realUSDTSwap.sub(fee).mul(usdtRate).div(1e18));
    if (isReset) {
      employ[msg.sender].amountSwap = realUSDTSwap;
    } else {
    ...
    emit SwapLuniProcess(msg.sender, realUSDTSwap, realUSDTSwap, usdtRate);
  }
  ...
  function swapUSDT(uint256 amountUSDT) public {
    ...
    uint amountLuni = amountUSDT.sub(fee).div(usdtRate).mul(1e18);
