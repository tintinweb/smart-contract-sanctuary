...
contract Punck is ERC1155, Ownable {
    ...
    event BuyNftWithBnb(uint256 numberOfNfts, uint256[] nftIds, uint256 coinType, uint256 invitorId, uint256 secId);
    event NameChange(uint256 nftId, string newName, address owner);
    event Transfer(uint256 nftId, address from, address to);
    ...
    function transferFrom(uint256 nftId, address to) public {
        ...
        emit Transfer(nftId, msg.sender, to);
    }
    ...
    function setNameLength(uint256 _min) public onlyOwner {
    ...
    function buyWithBnb(uint256 numberOfNfts, uint256 coinType, uint256 invitorId) public payable returns(uint256) {
        ...
        for (uint i = 0; i < numberOfNfts; i++) {
            ...
            if (_firstInvitors[nftId] > 0) {
                _bnbReword[_tokenOwner[_firstInvitors[nftId]]] = _bnbReword[_tokenOwner[_firstInvitors[nftId]]].add(msg.value.mul(35).div(100));
                if (_secInvitors[nftId] > 0) {
                    _bnbReword[_tokenOwner[_secInvitors[nftId]]] = _bnbReword[_tokenOwner[_secInvitors[nftId]]].add(msg.value.mul(15).div(100));
                }
            }
        }
        ...
        emit BuyNftWithBnb(numberOfNfts, nftIds, coinType, invitorId, _firstInvitors[invitorId]);
        return msg.value;
    }
