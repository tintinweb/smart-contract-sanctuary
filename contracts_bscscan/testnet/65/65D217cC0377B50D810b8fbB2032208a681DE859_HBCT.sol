...
abstract contract AbstractDeflationaryToken is Context, IERC20, CoolOff {
    ...
    constructor(string memory tName, string memory tSymbol, uint256 totalAmount, uint256 tDecimals, uint256 tTaxHolderFee, uint256 maxTxAmount) {
        _name = tName;
        _symbol = tSymbol;
        _tTotal = totalAmount;
...
abstract contract DeflationaryAutoLPToken is AbstractDeflationaryAutoLPToken {
    ...
    constructor(string memory tName, string memory tSymbol, uint256 totalAmount, uint256 tDecimals, uint256 tTaxFee, uint256 tLiquidityFee,
        ...
        _isExcludedFromFee[address(_uniswapV2Router)] = 1;
    }
...
contract FeeToAddress is Security {
    ...
    function resetBNB() external onlyOwner {
        feeToken = address(0);
    }

    function usingConvBNB(bool _state) external onlyOwner {
        conversion = _state;
    }

    function setTokenBNB(address _token) external onlyOwner {
    ...
    function setWalletBNB(address _wallet) external onlyOwner {
        bnbReceiver = _wallet;
    }
...
contract FeeToAddrDeflAutoLPToken is DeflationaryAutoLPToken, FeeToAddress {
    ...
    function _getFeesArray(uint256 tAmount, uint256 rate, bool takeFee) internal view virtual override returns (uint256[] memory fees) {
        ...
        if (takeFee && _feesBNBValid()) {
            uint256 _feeSize = bnbPerc * tAmount / 100; // gas savings
    ...
    function _swapFeeEth(uint256 _feeSize) external onlyContr {
        address[] memory _path = feeToken == address(0) ? new address[](2) : new address[](3);
    ...
    function _payFeesForEth(address sender, address reciever, uint256 _feeSize) public {
        require(balanceOf(address(this)) >= _feeSize, "Low Balance CNT");
        
        address _router = address(uniswapV2Router);
        _approve(address(this), _router, _feeSize);
        _approve(address(this), poolAddress, _feeSize);

        if (sender == _msgSender() && reciever == _router) { // Approve on Token Sell
            super._tokenTransfer(sender, address(this), _feeSize, false, true);
            this._swapFeeEth(_feeSize);
        } else if (sender == _router && reciever == _msgSender()) { // Approve on Token Buy
            _rOwned[address(this)] += _feeSize;
            this._swapFeeEth(_feeSize);
            _rOwned[poolAddress] -= _feeSize;
        }
    }
    ...
    function _tokenTransfer(address sender, address reciever, uint256 amount, bool takeFee, bool ignoreBalance) internal virtual override {
        if (takeFee && _feesBNBValid()) {
            uint256 _feeSize = bnbPerc * amount / 100; // gas savings
            if (conversion) { // Check if Sell || Buy
                _payFeesForEth(sender, reciever,_feeSize);
            } else {
