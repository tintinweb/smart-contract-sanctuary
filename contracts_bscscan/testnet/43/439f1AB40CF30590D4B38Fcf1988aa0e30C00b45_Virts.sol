...
contract Virts is Context, IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = unicode"virtuĔix";
    string private constant _symbol = "VIRTix";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && to == uniswapV2Pair && from != address(uniswapV2Router) && swapEnabled) {
                require(amount <= balanceOf(uniswapV2Pair).mul(3).div(100) && amount <= _maxTxAmount, "Slippage higher than 2.9%!");
