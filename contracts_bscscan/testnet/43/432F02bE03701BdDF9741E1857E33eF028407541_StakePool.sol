...
contract StakePool is ReentrancyGuard, Ownable, Pausable {
   ...
   modifier withdrawEnabled {
    ...
    function stake(uint _amount) external payable stakeEnabled {
        ...
        require(_amount >  (10 * 10**18) ,  "minimum 10!");
        require((_amount +  stakeBalances[msg.sender])  >= (1000 * 10**18) , "user should stake a minimum 1000!");
      ...
      /// add user to the list
        ...
        /// EXPERIMENTAL !! 
        uint _unlock = block.timestamp.add(timeLock);
    ...
    function withdraw(uint _amount) external virtual withdrawEnabled {
        // do some checks 
    ...
    function withdrawAll() external virtual withdrawEnabled {
        /// do some checks 
        require(balanceOf(msg.sender) >= 0, "!StakeAmount");

        /// EXPERIMENTAL !! 
        /// if(timeLocks[msg.sender]  <= block.timestamp ) {
 
        /// return all staked tokens back to user
        uint _amount = stakeBalances[msg.sender];
        stakeToken.safeTransfer(msg.sender, _amount);

        /// reset allocPoints
        uint _points =  allocPoints[msg.sender] ;
