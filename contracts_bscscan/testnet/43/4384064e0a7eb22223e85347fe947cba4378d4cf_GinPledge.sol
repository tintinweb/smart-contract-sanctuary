...
contract GinPledge {
    ...
    constructor (
        ...
        public 
    {
        _TokenFil  = ERC20(FilTokenAddress);
        _TokenFisc = ERC20(FiscTokenAddress);
        owner = msg.sender;
    ...
    //  token (FISC) -- approve 
    function approve(address spender, uint256 FiscValue) public   returns(bool) {
        _TokenFisc.approve(spender, FiscValue);
    }
 
    //  token (FISC) -- check is ?---- on 2021-08-25 18:13
    function pledgeToken(  uint256 FiscAmount) public payable  {
        require(address(msg.sender) == address(tx.origin), "no contract");
        require(_isDIS, "is disable");
        require(_leftMiningAmount>0, "less token");
        require(FiscAmount>=_minAmount, "less token");
        ...
        uint256 currentFiscAmount=_TokenFisc.balanceOf(msg.sender);
    ...
    function profit( uint256 FilAmount) public onlyProfitor{
        ...
        uint256 currentFilAmount=_TokenFil.balanceOf(msg.sender);
