...
contract PokerNut is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    mapping(address => uint256) referralId;
    ...
    function claim(address referralAddress) public payable nonReentrant {
        require(balanceOf(referralAddress) >= 1 ,'Error: referral address must own POKER');
        referralId[referralAddress]++;

        // unlimited supply
        uint256 tokenId = totalSupply();

        require(tokenId <= 99999, 'Error: supply limited');
