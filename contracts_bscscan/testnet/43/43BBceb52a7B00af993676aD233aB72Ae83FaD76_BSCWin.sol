...
contract BSCWin is Context, IERC20, Ownable {
    ...
    function nextLotteryTime() public view returns(uint256) {
        ...
        if (lotteriesToday == maxLotteriesPerDay) {
            ...
            if (block.timestamp.sub(lastLotteryTime) > timeBetweenLottery) {
                ...
                uint256 timeUntilNextLottery = timeBetweenLottery.sub(block.timestamp.sub(lastLotteryTime));
                return block.timestamp.add(timeUntilNextLottery);
            }
        }
    }
    ...
    function triggerLottery() public {
        ...
        //send caller token reward
         uint256 currentCallerReward = callerRewardHeld.div(maxTries + 1);
         if (callerRewardHeld > (callRewardLimit * 10**18)*(maxTries + 1)) {
             uint256 divider = (callRewardLimit * 10**18)*(callerRewardHeld);
             currentCallerReward = currentLotteryReward.div(divider);
         }
