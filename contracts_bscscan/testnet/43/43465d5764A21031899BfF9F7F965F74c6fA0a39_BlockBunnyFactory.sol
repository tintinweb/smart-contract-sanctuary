 ...
 */
contract BlockBunnyFactory is Initializable, ContextUpgradeable, AccessControlUpgradeable, ERC1155BurnableUpgradeable, ERC1155PausableUpgradeable, ERC1155SupplyUpgradeable {
     ...
     */
    function __BlockBunnyFactory_init(string memory uri) internal initializer {
        ...
        __BlockBunnyFactory_init_unchained();
    }

    function __BlockBunnyFactory_init_unchained() internal initializer {
	 ...
	 /**
     * @dev Creates `amount` new tokens for sender, of a new token type
     *
     * See {ERC1155-_mint}.
     *
     */
    function mint(uint256 amount, bytes memory data) external returns(uint256) {
        uint256 id = nextId++;
	...
	/**
     * @dev Creates `amount` new tokens for `to`, of a new token type
     *
     * See {ERC1155-_mint}.
     *
     * Requirements:
     ...
     */
    function mint(address to, uint256 amount, bytes memory data) external onlyRole(MINTER_ROLE) returns (uint256)  {
        uint256 id = nextId++;
    ...
    function mint(address to, uint256 id, uint256 amount,  bytes memory data) external onlyRole(MINTER_ROLE) {
		require(id < nextId, "BlockBunnyFactory: Type does not exist");
        _mint(to, id, amount, data);
    }
    ...
    function mintBatch(address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) external onlyRole(MINTER_ROLE) {
		for (uint256 i = 0; i < ids.length; ++i) {
            require(ids[i] < nextId, "BlockBunnyFactory: Type does not exist");
        }
    ...
    function _mintBatch(address account, uint256[] memory ids, uint256[] memory amounts, bytes memory data) internal virtual override(ERC1155Upgradeable, ERC1155SupplyUpgradeable) {
        for (uint256 i = 0; i < ids.length; ++i) {
            if (creators[ids[i]] == address(0)) {
                creators[ids[i]] = account;
            }
        }
    ...
    function _burn(address account, uint256 id, uint256 value) internal virtual override(ERC1155Upgradeable, ERC1155SupplyUpgradeable) {
        super._burn(account, id, value);
    }

    function _burnBatch(address account, uint256[] memory ids, uint256[] memory values) internal virtual override(ERC1155Upgradeable, ERC1155SupplyUpgradeable) {
