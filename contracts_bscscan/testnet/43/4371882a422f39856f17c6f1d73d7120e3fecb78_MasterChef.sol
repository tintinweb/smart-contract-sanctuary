...
contract HulkToken is BEP20('Hulk Token', 'Hulk') {
    ...
    */
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "HULK::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "HULK::_writeCheckpoint: block number exceeds 32 bits");
...
// StoneBar with Governance.
contract StoneBar is BEP20('StoneBar Token', 'STONE') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    function safeHulkTransfer(address _to, uint256 _amount) public onlyOwner {
...
interface IMigratorChef {
    // Perform LP token migration from legacy hulkSwap to HulkSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to hulkSwap LP tokens.
    // HulkSwap must mint EXACTLY the same amount of HulkSwap LP tokens or
    // else something bad will happen. Traditional hulkSwap does not
...
contract MasterChef is Ownable {
    ...
    function pendingHulk(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accHulkPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        hulk.mint(address(stone), HulkReward);
        pool.accHulkPerShare = pool.accHulkPerShare.add(HulkReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit HULK by staking');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw HULK by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    // Stake HULK tokens to MasterChef
    function enterStaking(uint256 _amount) public {
        ...
        stone.mint(msg.sender, _amount);
        emit Deposit(msg.sender, 0, _amount);
    }

    // Withdraw HULK tokens from STAKING.
    ...
    function safeHulkTransfer(address _to, uint256 _amount) internal {
        stone.safeHulkTransfer(_to, _amount);
    }
