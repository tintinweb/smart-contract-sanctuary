...
contract With_FundContract is AccessControlEnumerable, With_DAORole, I_With_FundContract{
    ...
    function setFund(address _fund) external onlyRole(DAO_ROLE){
...
contract With_DAOAddress is AccessControlEnumerable, With_DAORole, I_With_DAOAddress{
 ...
 */
interface I_Windmill_Royalty is I_Math, IAccessControlEnumerable, I_With_DAORole, I_With_UpdaterRole, IERC20, I_With_PWRToken{
 ...
 */
interface I_Windmill_Stacking is I_Math, IAccessControlEnumerable, I_With_DAORole, I_With_UpdaterRole, I_With_PWRToken{
...
contract With_UpdaterContract is AccessControlEnumerable, With_DAORole, I_With_UpdaterContract{
...
contract Windmill_Trade_Manager is Math, Adressable, AccessControlEnumerable, With_DAORole, With_UpdaterContract,
                                   With_FundContract, With_DAOAddress, With_BUSDToken, I_Windmill_Trade_Manager{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    constructor(){
        ...
        juniorTraderDefaultMaxEnergy = 10;
        seniorTraderDefaultMaxEnergy = 100;
    }
    ...
    function getEnergyValueBUSD() public view returns (uint256){
        return (fund.getFundBUSD() * maxLeverage.numerator) / (maxEnergyTotalSupply * maxLeverage.denominator);
    }
    
    function getMinimumEnergyToTrade() public view returns (uint256){
        return 1 + (minimumBUSDToTrade-1) / getEnergyValueBUSD();
    }
    ...
    function startTrade(uint256 deployerId, uint256 nbEnergy) external returns (uint256){
        ...
        uint256 nbBUSD = getEnergyValueBUSD() * nbEnergy;
        
        I_Windmill_Trade_Abstract trade = deployer.deployer.deployNewTrade(msg.sender, thisAddr, address(fund), address(updater));
        ...
        data.percentPWRLockedNumerator =  nbBUSD * 100000 / totalBUSD;
    ...
    function endTrade(uint256 tradeId) external{
        ...
        require(data.owner == msg.sender, "Windmill_Trade_Manager: Only the owner can end the trade.");
