...
contract PreTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SPRE }
    ...
    address public sPRE;
    uint public sPREQueue; // Delays change to sPRE address
    ...
    /**
        @notice allow approved address to deposit an asset for PRE
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint PRE needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( PRE ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn PRE for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IPREERC20( PRE ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sPRE ).balanceOf( msg.sender ); // Can only borrow against sPRE held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with PRE
        @param _amount uint
     */
    function repayDebtWithPRE( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IPREERC20( PRE ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, PRE, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( PRE ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( PRE ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns PRE valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match PRE decimals
            value_ = _amount.mul( 10 ** IERC20( PRE ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SPRE ) { // 9
            sPREQueue = block.number.add( blocksNeededForQueue );
        } else return false;
