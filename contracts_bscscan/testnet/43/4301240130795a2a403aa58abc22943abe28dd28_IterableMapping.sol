...
contract PesaDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Pesa_Dividend_Tracker", "Pesa_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Pesa_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Pesa_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Pesa contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Pesa_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "Pesa_Dividend_Tracker: Cannot update claimWait to same value");
...
contract PESACOIN is BEP20, Ownable, SafeToken, LockToken {
    ...
    PesaDividendTracker public dividendTracker;
...
10% of every transaction is split in to:
• 7% BNB is redistributed to holders
• 1.5% is used to fuel the liquidity pool exchange growth
• 1.5% is allocated to the marketing fund
*/

    constructor() BEP20("PESA", "PESA") {
        BNBRewardsFee = 7;
        liquidityFee = 2;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 2;
        marketingWallet = payable(0x9bcBE4Af33C43E12c82d2fcD77DF6AF7F82174A2); // please provide marketing address ok one second
     //   address pinkAntiBot_ ;
    	...
    	dividendTracker = new PesaDividendTracker();
 ...
 function setEnableAntiBot(bool _enable) external onlyOwner {
   antiBotEnabled = _enable;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Pesa: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Pesa: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Pesa: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Pesa: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Pesa: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Pesa: Cannot update gasForProcessing to same value");
