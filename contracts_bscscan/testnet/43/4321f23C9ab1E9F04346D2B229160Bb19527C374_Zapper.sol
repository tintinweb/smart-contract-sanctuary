...
contract Zapper is Ownable {
    ...
    function zapTokenToLP(address inputToken, address outputToken, uint256 amount) external {
        IBEP20(inputToken).transferFrom(msg.sender, address(this), amount);
        uint256 nativeAmount = swapTokenForNative(inputToken, amount, address(this));
        swapNativetoLP(outputToken, nativeAmount, msg.sender);
    }

    function zapTokenIntoFarm(address inputToken, uint256 amount, address farm, uint256 pool_index) external {        
        IBEP20(inputToken).transferFrom(msg.sender, address(this), amount);
        uint256 nativeAmount = swapTokenForNative(inputToken, amount, address(this));
