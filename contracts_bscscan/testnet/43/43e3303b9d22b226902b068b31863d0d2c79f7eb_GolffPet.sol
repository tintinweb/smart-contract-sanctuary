...
contract GolffPet is ERC721Enumerable, Ownable {
    ...
    uint256[] private initIndex = [2,1,1,0,0,0,0,0,0,0];

    event Claim(address indexed owner, uint256 tokenId);
    //bytes32 merkleRoot_
    constructor(address account,string memory _ipfsHash ) ERC721('Golff Pet', "Golff Pet") {
...
//        require(MerkleProof.verify(merkleProof, merkleRoot, node), 'MerkleDistributor: Invalid proof.');
    ...
    function initClaim(address account) onlyOwner private {
        require(totalSupply() + initIndex.length <= MAX_SPACE_SUPPLY, "GolffPet:claim would exceed max supply");
        for (uint256 i ;i<initIndex.length;i++){
            _tokenIds.increment();
            uint256 newTokenId = _tokenIds.current();
            _safeMint(account, newTokenId);
            setAttributeIndexInner(newTokenId,initIndex[i]);
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        if (keccak256(abi.encodePacked(revealedCollectionBaseURL)) == keccak256(abi.encodePacked(''))) {
            content = abi.encodePacked(content,
                ', ',
                '"description": "An unrevealed Golff Pet"',
                ', ',
            ...
            if (getBoardFactor(_tokenId) == boardFactor[2] || getBoardFactor(_tokenId) == boardFactor[3]) {
                content = abi.encodePacked(content,
                    ', ',
                    '"description": "To celebrate the first anniversary of Golff, \'Golff Pet NFT\' is hereby launched to bring exclusive rights and interests to holders. Golff users who meet specific criterias can generate different NFTs randomly. NFT Holders can join the Golff VIP community with the priority experience of Golff products, and get a long-term revenue bonus in the Golff ecology."',
                    ', ',
