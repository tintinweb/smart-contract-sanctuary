...
contract Merchant is Pausable {
     ...
     */
    function getFeeAmount(address _userAccount, address _payingTokenAddress, uint256 amountIn)
        public
        view
        returns (uint256)
    {
        uint256 feeAmount = amountIn.mul(transactionFee).div(10000);
        uint256 wpayBalanceOfCaller = WPAY_TOKEN.balanceOf(_userAccount);
        ...
        returns (bytes16 txNumber)
    {
        ...
        uint256 feeAmount = getFeeAmount(_msgSender(),_payingTokenAddress, _amountIn);
