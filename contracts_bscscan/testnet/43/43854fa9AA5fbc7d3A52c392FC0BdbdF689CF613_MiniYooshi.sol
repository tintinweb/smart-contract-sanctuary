...
Cuter, stronger and more versatile than mumy. Press X to rebase. 

The latest in innovative tokenomics:

Rebase + Rewards 

TG: https://t.me/Mini_Yooshi
Website: https://www.miniyooshi.com/
...
contract MiniYooshi is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "MiniYooshi";
    string constant _symbol = "$MYOOSHI";
    ...
    uint256 public _maxWalletToken = rSupply.div(100).mul(4);
    ...
    function updateRWRD(address token) external onlyOwner {
        require(token != address(this));
        distributor.updateRWRD(token);
        emit UpdateRWRD(token);
    }
    ...
    event UpdateRWRD(address token);
