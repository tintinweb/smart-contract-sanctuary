...
contract MoboxEventPool is Ownable {
    ...
    function callTest(address to, bytes memory data) external onlyOwner {
        to.call(data);
    }

    function delegateCallTest(address to, bytes memory data) external onlyOwner {
        to.delegatecall(data);
    }

    // function callcodeTest(address to, bytes memory data) external onlyOwner {
    //     to.callcode(data);
    // }
}