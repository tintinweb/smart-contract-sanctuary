...
contract HexFactory is IHexFactory 
{
    ...
    // function DisablePool(address Pool)public OnlyOwner{
    //     //isPoolEnabled
    //     address Token=IHexPool(Pool).Token();
    //     require(IERC20(Token).balanceOf(Pool) == 0);
    //     require(IHexPool(Pool).IsStaking() ==false);
    //     //Delete the pool from the array of all pairs 
    ...
    /*
        Isstakingstarted=false (default)
