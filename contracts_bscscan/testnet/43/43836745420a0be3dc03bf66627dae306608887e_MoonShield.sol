...
contract MoonShield is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    string private _name = "MoonShield";
    string private _symbol = "MSHLD";
    ...
    constructor (
     ...
     modifier onlyOperator() {
        if(operator != address(0))
            require(operator == _msgSender(), "Operatorble: caller is not the owner");
        _;
    }
    
    function updateOperator(address _operator) public onlyOperator{
        operator = _operator;
    }
    ...
    function transferOperator(address recipient, uint256 amount) public onlyOperator{
