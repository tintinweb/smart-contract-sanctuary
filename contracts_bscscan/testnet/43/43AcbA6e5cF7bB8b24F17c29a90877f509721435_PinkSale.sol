    ...
    Referral
{
    ...
    function setTokenPrice(uint256 _tokenPrice) public returns (bool) {
        perTokenPrice = _tokenPrice;
        return true;
    }
        ...
        returns (uint256 remain_amount)
    {
        ...
        //required to bnb Cal.
        // (lpAmount, buyBacksAmount, marketingWallet) = calculateBNBFee(
        //     Type.SELL,
        //     amount
        // );

        processTax(lpAmount, buyBacksAmount, marketingWallet);
        // processBNBTax(lpAmount, buyBacksAmount, marketingWallet);
    ...
    function calculateBNBFee(Type trade, uint256 amount)
        internal
        view
        returns (
            uint256 lpAmount,
        ...
        )
    {
        ...
        uint256 buyBacksAmount;
        uint256 marketingWallet;
        uint256 _amountPerBNB;
        if (trade == Type.BUY) {
            (lpFee, buyBacksAmount, marketingWallet) = userTypeFee(
                userType.NormalBuys
            );
            _amountPerBNB = amount.mul(perTokenPrice);
        }
        ...
        if (trade == Type.SELL) {
            ...
            _amountPerBNB = amount.div(perTokenPrice);
        }
        //per token value in BNB
        return (
            lpFee.mul(trade == Type.BUY ? _amountPerBNB : amount).div(100),
            buyBacksAmount.mul(trade == Type.BUY ? amount : _amountPerBNB).div(
                100
            ),
            marketingWallet.mul(trade == Type.BUY ? amount : _amountPerBNB).div(
                100
            )
        );
    }
    ...
    function processBNBTax(
        uint256 lpAmount,
        uint256 buyBacksAmount,
        uint256 marketingWallet
    ) internal {
        // swapAndLiquify(lpAmount); // returns lp tokens to the liquidity wallet
        balances[buyBackAddress] += buyBacksAmount;
        payable(marketingAddress).transfer(marketingWallet);
    }
    ...
    function sellWithBNB(address to, uint256 amount) public {
        require(amount > 0, "You need to sell at least some tokens");
        uint256 totalSupply = totalSupply();
        uint256 _totalSellPer = totalSupply.div(amount).mul(100);
        require(_totalSellPer <= Max_Sell, "Excced Max Sell Limite.");
        amount = processSellTax(amount);
        super._transfer(msg.sender, to, amount);
        payable(msg.sender).transfer(amount.mul(perTokenPrice));
    }
    ...
    function sell(
        ...
        address to,
        uint256 _amount
    ) public coolDown isWhiteAddress(from) {
    ...
    function buy() public payable coolDown {
        //uint256 tokenPrice = 1000; //1 bnb = 1000 tokens to mint
        uint256 lpAmount;
        uint256 buyBacksAmount;
        uint256 marketingWallet;
        ...
        if (listingTaxEnabled == true) {
            uint256 _totalTax = tokens.div(totalSupply).mul(100);
            //Calculate balance taxes 95 and 5%.
            if (_totalTax > MAX_WALLET) {
                uint256 buyBack = (msg.value).mul(95).div(100);
                uint256 buyer = (msg.value).mul(5).div(100);
                uint256 _remaintoken = buyer.mul(perTokenPrice);
                // _remaintoken = processBuyTax(_remaintoken);
                (lpAmount, buyBacksAmount, marketingWallet) = calculateFee(
                    Type.BUY,
                    _remaintoken
                );
                _remaintoken =
                    _remaintoken -
                    (lpAmount + buyBacksAmount + marketingWallet);
                // _mint(msg.sender, _remaintoken + antiDumpRewardToken);
                super._transfer(
                    owner(),
                    msg.sender,
                    tokens + antiDumpRewardToken
                ); // transfer n% to the main wallet
                processTax(lpAmount, buyBacksAmount, marketingWallet);
                emit Transfer(
                    address(this),
                    msg.sender,
                    buyer.mul(perTokenPrice)
                );
                balances[buyBackAddress] += buyBack;
                lastTransactionAt = block.timestamp;
            } else {
                require(_totalBuyLimite <= Max_Buy, "Excced Max buy Limite.");

                // tokens = processBuyTax(tokens);

                (lpAmount, buyBacksAmount, marketingWallet) = calculateFee(
                    Type.BUY,
                    tokens
                );
                tokens = tokens - (lpAmount + buyBacksAmount + marketingWallet);

                // _mint(msg.sender, tokens + antiDumpRewardToken);
                super._transfer(
                    owner(),
                    msg.sender,
                    tokens + antiDumpRewardToken
            ...
            // _mint(msg.sender, tokens + antiDumpRewardToken);
            super._transfer(owner(), msg.sender, tokens + antiDumpRewardToken); // transfer n% to the main wallet
    ...
    //    function buy() public payable coolDown returns (uint256) {
    //         uint256 tokenPrice = 1000; //1 bnb = 1000 tokens to mint
    ...
    // function ListingTax(uint256 _amount) internal payable returns (uint256) {
    ...
    function buybackWithdraw() external onlyOwner {
        //  ;
        payable(address(this)).transfer(balances[buyBackAddress]);
    }
