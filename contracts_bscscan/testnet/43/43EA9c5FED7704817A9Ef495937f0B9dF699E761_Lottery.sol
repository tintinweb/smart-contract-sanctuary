...
contract Lottery is Ownable, Initializable, Testable {
    ...
    // VIEW FUNCTIONS
    //-------------------------------------------------------------------------
    function getLotteryIdCounters() external view returns (uint256 ) {
        return lotteryIdCounter_;
    }

    function getLottoInfo(uint256 _lotteryId) external view returns (uint256, Status, LotteryType, Coefficient[] memory, uint256, uint256, uint256) {
        return (
            allLotteries_[_lotteryId].lotteryId,
            allLotteries_[_lotteryId].lotteryStatus,
            allLotteries_[_lotteryId].lotteryType,
            allLotteries_[_lotteryId].coefficients,
    ...
    function numbersDrawn(
        ...
        if (requestId_ == _requestId) {
            ...
            if (allLotteries_[_lotteryId].stakeholders.length > allLotteries_[_lotteryId].winningSize) {
    ...
    function _split(
        ...
        if (lotterySetting.stakeholders.length - paddedSize < lotterySetting.winningSize){
            sizeOfLottery = lotterySetting.stakeholders.length - paddedSize;
        }
        else {
            sizeOfLottery = lotterySetting.winningSize;
        }
