...
contract DOGGGO is Ownable, IBEP20 {
	...
	uint32 public dynamicFeeRatio = 10**4;

	bool public circuitBreaker;
	bool private _liqSwapReentrancyGuard;
	bool private _rewardSwapReentrancyGuard;
	...
	function _transfer(
		...
		// >1 day since last tx
		if (block.timestamp > lastTx[sender].lastTimestamp + 1 days) {
	...
	function balancer(uint256 amount, uint256 poolBalance) private {
		// TODO: capire se tenere sempre la total supply o se sottrarre il charity
		uint256 unwghtCircSupply = totalSupply().sub(_balances[charityWallet]);
		...
		if (_balancerBalances.liquidityPool >= swapForLiquidityThreshold && !_liqSwapReentrancyGuard) {
		...
		if (_balancerBalances.rewardPool >= swapForRewardThreshold && !_rewardSwapReentrancyGuard) {
			_rewardSwapReentrancyGuard = true;
			uint256 tokenOut = sendToRewardAddress(_balancerBalances.rewardPool);
			balancerBalances.rewardPool -= tokenOut;
			_rewardSwapReentrancyGuard = false;
		}
	...
	function endOfPeriod() external view returns (uint256) {
		return lastTx[msg.sender].lastClaim + resetRate;
	}
	...
	function setDynamicFeeRatio(uint8 _dynamicFeeRatio) external onlyOwner {
		require(_dynamicFeeRatio >= 10**2 && _dynamicFeeRatio <= 10**6, "new fee out of possible values");
		dynamicFeeRatio = _dynamicFeeRatio;
		emit TaxRatesChanged();
	}
