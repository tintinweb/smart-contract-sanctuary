...
contract PUSD is Context, IERC20, Ownable {
  ...
  mapping (address => bool) public permittedToken;
  ...
  uint256 public _collateralized;//5%+unstaked
  ...
  event CollectedUSD(uint BUSDBal, uint USDCBal, uint MinCollateral);
  ...
  constructor(address _uniswapV2Router, address _swapAddress) {
    ...
    permittedToken[BUSD] = true;
    permittedToken[USDC] = true;
    
    BUSD = 0xE0dFffc2E01A7f051069649aD4eb3F518430B6a4;
    USDC = 0xB8F5B50ed77596b5E638359d828000747bb3dd89;
  ...
  function getPUSD(address fromToken, uint amount) external {
    require(permittedToken[fromToken], "token not permitted");
    IERC20 tokenA = IERC20(fromToken);
    tokenA.transferFrom(msg.sender, address(this), amount);
    if(fromToken == BUSD || fromToken == USDC) {
        _mint(msg.sender, amount);
        emit PUSDMinted(amount);
    } else {
        uint pUSDToMint = handleSwap(fromToken, BUSD, amount.div(2));
        pUSDToMint = pUSDToMint.add(handleSwap(fromToken, USDC, amount.sub(amount.div(2))));
  ...
  function getPUSDForBNB() external payable {
    uint pUSDToMint = handleBNBSwap(BUSD, msg.value.div(2));
    pUSDToMint = pUSDToMint.add(handleBNBSwap(USDC, msg.value.sub(msg.value.div(2))));
    uint fees = calculateFees(pUSDToMint, _swapFee, _feeDenominator);
    _balances[swapAddress] = _balances[swapAddress].add(fees);
    _mint(msg.sender, pUSDToMint.sub(fees));
    emit PUSDMinted(pUSDToMint.sub(fees));
  ...
  function cashoutPUSD(uint amount, bool isUSDC) external {
      ...
      if(isUSDC) {
          ...
          require(token.balanceOf(address(this)) >= amount, "Contract does not have enough USDC");
          token.transfer(msg.sender,amount);
      } else{
          IERC20 token = IERC20(BUSD);
          require(token.balanceOf(address(this)) >= amount, "Contract does not have enough BUSD");
          token.transfer(msg.sender, amount);
      }
  ...
  function collect() external onlyOwner {
      ...
      if(BUSDBal > minCollateral) {
          IERC20 BUSDTok = IERC20(BUSD);
          BUSDTok.transfer(msg.sender, BUSDBal.sub(minCollateral));
      }
      
      if(USDCBal > minCollateral) {
          IERC20 USDCTok = IERC20(USDC);
          USDCTok.transfer(msg.sender, USDCBal.sub(minCollateral));
      }
      
      emit CollectedUSD(BUSDBal, USDCBal, minCollateral);
  }
  
  function getCollectionValues() internal view returns (uint,uint,uint) {
      IERC20 USDCTok = IERC20(USDC);
      IERC20 BUSDTok = IERC20(BUSD);
      uint BUSDBal = BUSDTok.balanceOf(address(this));
      uint USDCBal = USDCTok.balanceOf(address(this));
      uint pUSDStaked = _balances[stakingAddress];
      uint minCollateral = calculateFees(_totalSupply, _collateralized, _feeDenominator);
      minCollateral = _totalSupply.sub(pUSDStaked).add(minCollateral);
  ...
  function changeCollateralizedRatio(uint amount) external onlyOwner {
    _collateralized = amount;
  }
  ...
  function setPermitted(address token, bool permission) external onlyOwner{
      permittedToken[token] = permission;
  }
  ...
  function setUSDAddresses(address _USDC, address _BUSD) external onlyOwner {
      USDC = _USDC;
      BUSD = _BUSD;
  }
  /**
  ...
  function handleSwap(address fromToken, address toToken, uint amount) private returns (uint256) {
      address[] memory path = new address[](2);
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            amount,
            0, // accept any amount of BUSD or USDC
            path,
            address(this),
            block.timestamp
        );
