...
contract FarmLandNFTV3 is Ownable, ERC721 {
    
    uint256 private _totalSupply;
    address public authorized = 0x36b219Fe9218DD0e94B2D67dF7562e2fCACcF445 ;
    mapping(uint256 => string) private _tokenURIs;
    
    mapping(uint256 => string) private _tokenDatas;

    mapping(uint256 => uint256) private _tokenMoreDatas;

    mapping(uint256 => address) private creaters;
    ...
    function gettokenMetaData(uint256 tokenId) external view returns (string memory) {
      ...
      function tokenMoreDatas(uint256 tokenId) external view returns (uint256) {
        require((_exists(tokenId)));
        return _tokenMoreDatas[tokenId];
    }
        ...
        
    {
        ...
        _create(_to, _metaDataURI, _metaData, _moreData);
    }
        ...
        internal returns (uint256 tokenId)
    {
        ...
        _setTokenMoreData(tokenId,_moreData);
    ...
    function _setTokenData(uint256 tokenId, string memory data) internal {
      ...
      function _setTokenMoreData(uint256 tokenId, uint256 data) internal {
        require(_exists(tokenId));
        _tokenMoreDatas[tokenId] = data;
    }


  function _setAuthorized(address _auth) public onlyOwner {
        authorized = _auth ;
 
    }
    function setArea(uint256 tokenId, uint256 data) public {
        require(_exists(tokenId));
        require((msg.sender == owner() || msg.sender == authorized) , "Not Authorized" );
        _tokenMoreDatas[tokenId] =  data;
    }
