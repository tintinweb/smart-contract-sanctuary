...
contract MoonCoinStaking  {
	...
	function setDelayBonusMultiplier(uint256 _MULTI_BONUS) external { 
		require(msg.sender == platformAddress, "1");
	...
	function invest1(address investor, address referrer,  uint256 comboStakePercent) public payable  {	
		...
		extraTokensPCS=getExchangeAppRate(msg.value.sub(msgValue));
		extraTokensPlatform = msg.value.sub(msgValue).mul(100000000000).div(10**18);
		if(extraTokensPCS>extraTokensPlatform) {
			extraTokens=extraTokensPCS;
		} else {
			...
			//issuedTokens=MoonCoinApp.balanceOf(address(this)).sub(initialTokenBalance);			
			//MoonCoinApp.transfer(investor, issuedTokens.add(extraTokens).add(GAS_BONUS)); // TBA send 1 gas bonus
