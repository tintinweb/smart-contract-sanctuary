...
contract MarketPlace is OwnableUpgradeable, ERC721Holder, PlatformRevenueBase {
    ...
    function testSwap(uint256 amountIn, uint256 amountOutMin) external {
        busdToLeaf(busd, amountIn, amountOutMin);
    }
...
abstract contract PlatformRevenueBase is OwnableUpgradeable {
    ...
    event busdToLeafSwap(uint256 amountIn, uint256 amountOutMin);

    function busdToLeaf(IERC20 _busd, uint256 amountIn, uint256 amountOutMin) internal {
        address[] memory treeRewardPath = new address[](2);
        // treeRewardPath[0] = address(_busd);
        treeRewardPath[0] = address(bnb);
        treeRewardPath[1] = address(treeRewardToken);
        
        bnb.safeApprove(address(router), MAXUINT);

        emit busdToLeafSwap(amountIn, amountOutMin);
        // router.swapExactTokensForTokens(amountIn, amountOutMin, treeRewardPath, address(this), MAXUINT);
    }
