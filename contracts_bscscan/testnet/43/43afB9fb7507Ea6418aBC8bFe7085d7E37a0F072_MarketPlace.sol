...
contract MarketPlace{
    ...
    function changeSaleTokenStatus(uint256 tokenId,bool status)internal tokenNotSoldAlready(tokenId) returns(bool){
        require(nft.balanceOf(msg.sender)!=0 ,"You do not own the token so you cannot change the status");
    ...
    function putTokenOnSale(uint256 _tokenId, uint256 _amount, bool _purchaseWithToken)external  checkTokenOwner() payable returns(bool){
        require(nft.balanceOf(msg.sender)!=0,"You donot own this token");    
    ...
    function buyTokenWithBNB(uint256 tokenId, address buyer)external payable tokenNotSoldAlready(tokenId) returns(orderDetails memory) {
        ...
        require(saleItems[tokenId].timeOnsale!=0,"TOKEN_IS_NOT_BUYABLE");
        require(!saleItems[tokenId].sold,"TOKEN_ALREADY_SOLD");
        require(saleItems[tokenId].onSale == true,"TOKEN_NOT_ON_SALE");
        require(nft.isApprovedForAll(saleItems[tokenId].owner,address(this)),"TOKEN_NOT_APPROVED_FOR_TRANSFER");
        ...
        require(msg.value >= saleItems[tokenId].price, "Insufficient ether value");
    ...
    function buyTokenWithToken(uint256 tokenId, address buyer)external payable tokenNotSoldAlready(tokenId) returns(orderDetails memory) {
        ...
        address creator = nft.getCreator(tokenId);
        uint256 amount = saleItems[tokenId].price;
        
        if(saleItems[tokenId].owner == creator){
            // Seller is Creator : No Royalty
             gamartToken.transfer(creator, amount);
        }else{
            // Seller is Not Creator : Give Royalty
            uint256 royaltyAmount = amount.mul(nft.getTokenRoyaltyPercentage(tokenId)).div(100);
            amount = amount.sub(royaltyAmount);
            gamartToken.transfer(saleItems[tokenId].owner, amount);
            gamartToken.transfer(creator, royaltyAmount);
        }
