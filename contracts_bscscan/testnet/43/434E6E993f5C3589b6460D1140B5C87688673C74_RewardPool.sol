...
contract RewardPool is IRewardPool, Ownable, ReentrancyGuard {
    ...
    function claim() nonReentrant timedTransitions public returns (uint256 reward, uint256 refbonus, uint256 winbonus){
        UserInfo storage user = userInfo[msg.sender];
        ...
        pending = pending.add(refbonus);
       
        winbonus = user.bonusProfit;
        pending = pending.add(winbonus);
        ...
        if (!safeBUSDTransfer(msg.sender, pending)) {
			...
			refbonus = 0;      
        	winbonus = 0;           
            require(1 == 0, "safeTransfer failed");
        }
        else
        {
            user.dividendDebt = 0;
...

{
  ...
  "libraries": {
    "/contracts/Utils.sol": {
      "Utils": "0x56aF60B794F4AA5A1424c027E2940013db8a0130"
    }
  },
  "outputSelection": {
    "*": {
