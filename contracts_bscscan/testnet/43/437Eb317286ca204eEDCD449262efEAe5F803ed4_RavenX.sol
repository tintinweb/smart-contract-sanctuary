...
contract RavenX is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 0.001 * 10**3;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private _tBurnTotal;
    uint256 private _tDonationTotal;

    string private _name = "Raven X";
    string private _symbol = "RX";
    ...
    uint public withdrawable = _tTotal.sub(_rOwned[burnAddress]);
    uint public infund = address(this).balance / buyPrice;
    
    //Address where all the contract-bought PonziCoins are sent 
    address payable private burnAddress = 0x000000000000000000000000000000000000dEaD; // Address to burn coins with.  
    uint totalPonziBuys = 0;
    ...
    uint256 public _maxTxAmount = 0.001 * 10**3;
    uint256 private minimumTokensBeforeSwap = 50 * 10**3; 
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        //measuring eth before the buyback
        uint EthBeforePonziBuy = address(this).balance;
        
        //swaping contracts tokens for eth in DeX
        swapTokensForEth(contractTokenBalance); 
        
        //measuring eth after the buyback
        uint EthForPonziBuy = address(this).balance.sub(EthBeforePonziBuy);
        
        //adding the eth to the tally for ponzi buys
        totalPonziBuys = totalPonziBuys.add(EthForPonziBuy);
        
        //calculating how many tokens for the eth
        uint tokenCount = EthForPonziBuy / buyPrice;
        
        // if the token amount is larger than the tier, you buy the rest of the tier, and get refunded
        ...
        // sending tokens to the burn address
        _rOwned[burnAddress] += tokenCount;
        _tTotal += tokenCount;
      ...
      returns (bool)
    {
      ...
      if (tokenCount > tierBudget) {
        uint changeGiven = tokenCount - tierBudget;
        uint changeWithdrawEth = changeGiven * buyPrice;
        msg.sender.transfer(changeWithdrawEth);
        tokenCount = tierBudget;
      }
      
      uint investment = tokenCount * buyPrice;

      _rOwned[msg.sender] += tokenCount;
      _tTotal += tokenCount;
      tierBudget -= tokenCount;
      ...
      returns (bool)
    {
     ...
     if (withdrawable > infund){
        return false;
     }
     
     if (_rOwned[msg.sender] >= tokenCount) {
