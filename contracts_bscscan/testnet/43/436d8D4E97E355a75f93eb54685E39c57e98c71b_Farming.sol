...
contract Farming is Ownable, ReentrancyGuard {
    ...
    function _autoDeposit(uint256 _pid, uint256 _amount, address _from) private {
     ...
     * @param _amount: the amount of LP tokens that should be deposited
     */
    function autoDeposit(uint256 _pid, uint256 _amount, address _from) public poolExists(_pid) {
        updatePool(_pid);
        ...
        rewardToken.approve(ROUTER_ADDRESS, _amount/2);
        IPancakeRouter02(ROUTER_ADDRESS).swapExactTokensForETHSupportingFeeOnTransferTokens(_amount/2, 0, path, address(this), 20);
        uint256 balanceDifference = address(this).balance - contractbalance;
        (, ,uint256 liquidity) = IPancakeRouter02(ROUTER_ADDRESS).addLiquidityETH{value: balanceDifference}(address(rewardToken), _amount/2, 0, 0, address(this), 20);
        _autoDeposit(_pid, liquidity, _from);
    }
