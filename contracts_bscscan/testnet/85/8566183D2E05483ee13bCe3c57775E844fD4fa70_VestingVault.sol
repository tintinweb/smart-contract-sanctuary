...
contract VestingVault is Ownable {
    ...
    function claimAllByOwner(address[] memory _vestedAddresses) public onlyOwner {
        ...
        for (uint i = 0; i < _vestedAddresses.length; i++) {
            Grant storage grantInfo = grants[_vestedAddresses[i]];
            grantInfo.transferred = grantInfo.transferred.add(transferAmount[i]);

            token.transfer(_vestedAddresses[i], transferAmount[i]);
            emit NewRelease(_vestedAddresses[i], transferAmount[i]);
        }
    }
