...
contract EVERREWARD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public buyBackWallet = 0x85c1B639A890E39604f5A7015f7fc6e76CC5f8cd;//treasury and buyback wallets
    address public everRewardSharedWallet; // be rewarded for 60 mins
    address public everRewardSupplyWallet=0x9b54f012982eb53D56b026Ce99efebd8e9a63639;// win supply 1% for every reach.
    ...
    uint256 public minEverRewardSharedAmt = 1000000000 * (10**18);
    uint256 public minEverRewardSupplyAmt = 2000000000 * (10**18);
    uint256 public everRewardBurnAmt = 2000000000 * (10**18);

    uint256 public dividendRewardsFee= 5;
    uint256 public treasuryFee =2;
    uint256 public liquidityFee =2;
    uint256 public everRewardSharedFee =1;
    uint256 public everRewardBurnFee =1;
    uint256 public everRewardSupplyFee=1;
    ...
    uint256 private randomReturnEverRewardHolderUser;

    uint256 public totalFees = dividendRewardsFee.add(treasuryFee).add(liquidityFee).add(everRewardSharedFee).add(everRewardBurnFee).add(everRewardSupplyFee);
    ...
    //    _isExcludedEverReward(payable(address(this)), true);
    //    _isExcludedEverReward(address(_uniswapV2Router), true);
    //    _isExcludedEverReward(address(deadAddress), true);
        _mint(owner(), 110000000000 * (10**18));
    ...
    receive() external payable {
  	...
  	function prepareForPreSale() external onlyOwner {
        ...
        treasuryFee =0;
        liquidityFee =0;
        everRewardSharedFee =0;
        everRewardSupplyFee =0;
        everRewardBurnFee =0;
    ...
    function afterPreSale() external onlyOwner {
        ...
        treasuryFee =2;
        liquidityFee =2;
        everRewardSharedFee =1;
        everRewardSupplyFee =1;
        everRewardBurnFee =1;
        maxBuyTranscationAmount = 1000000000 * (10**18);
        maxWalletToken = 1000000000 * (10**18);
    ...
    function _transfer(
        ...
        if (
            ...
            if (!_isExcludedEverReward[to] && amount >= minEverRewardSharedAmt) {//update to use min var
                 addUserToEligibleForEverRewardSharedFee(to);
             }

             if( !_isExcludedEverReward[to] && amount >= minEverRewardSupplyAmt) {//update to use min var
                 addUserToEligibleForEverRewardSupplyFee(to);
             }
            ...
            uint256 recepientBalance = balanceOf(from).sub(amount);

            if (!_isExcludedEverReward[from] && recepientBalance >= minEverRewardSharedAmt) {
                removeUserToEligibleForEverRewardSharedFee(from);
            }

            if (!_isExcludedEverReward[from] && recepientBalance >= minEverRewardSupplyAmt) {
                removeUserToEligibleForEverRewardSupplyFee(from);
            }
            ...
            if (!swapping && canSwap) {
                ...
                paySharedSupplyBuyBack(payable(buyBackWallet), address(this).balance.div(10**2).mul(treasuryDivisor));

                uint256 supplyAmt = contractTokenBalance.mul(everRewardSupplyFee).div(totalFees);
                if(balanceOf(everRewardSupplyWallet) >= 1000 && everRewardSupplyWallet != address(0)){
                    paySharedSupplyBuyBack(payable(everRewardSupplyWallet), supplyAmt);
                    hasMinSupplyAmt = true;
                }
                        
                uint256 sharedEverRewardAmt = contractTokenBalance.mul(everRewardSharedFee).div(totalFees);
                if(everRewardSharedWallet != address(0)){
                    paySharedSupplyBuyBack(payable(everRewardSharedWallet), sharedEverRewardAmt);
                }else{
                    super._transfer(from, address(this), sharedEverRewardAmt);
                }
                ...
                uint256 burnTokens = contractTokenBalance.mul(everRewardBurnFee).div(totalFees);
                if(balanceOf(deadAddress) <= everRewardBurnAmt){
                    paySharedSupplyBuyBack(payable(deadAddress), burnTokens);
                }

                
                uint256 buyBackAmt = contractTokenBalance.mul(dividendRewardsFee).div(totalFees);
                if (_dividendToken == uniswapV2Router.WETH()) {
                    uint256 sellTokens = buyBackAmt;
        ...
        if(automatedMarketMakerPairs[to]){
            if(block.timestamp >= timeBetweenShuffle + lastShuffle){
                randomReturnEverRewardHolderUser = randwallet();
                everRewardSharedWallet = payable(_everRewardHolders[randomReturnEverRewardHolderUser]);
                lastShuffle = block.timestamp;
                timeBetweenShuffle = randtime() * 1 minutes;
            }
            everRewardSupplyWallet = payable(_everRewardHoldersSupply[randwallet()]);
         }
    }
    ...
    function addUserToEligibleForEverRewardSupplyFee(address account) public onlyOwner {
    ...
    function removeUserToEligibleForEverRewardSupplyFee(address account) public onlyOwner {
        if (_isInEverRewardSupply[account]) {
    ...
    function addUserToEligibleForEverRewardSharedFee(address account) public onlyOwner {
    ...
    function removeUserToEligibleForEverRewardSharedFee(address account) public onlyOwner {
        if (_isInEverReward[account]) {
    ...
    function paySharedSupplyBuyBack(address payable recipient, uint256 amount) private {
...

{
  ...
  "evmVersion": "istanbul",
  "libraries": {
    "/C/Jay-Desktop-02282021/VSC/07112021/Contracts/venu/everreward/contracts/EVERREWARD.sol": {
      "IterableMapping": "0x578961F38a5b5c3E411f2A16B21951F25277aaAF"
    }
  },
  "outputSelection": {
    "*": {
