...
contract DcfContract {
    ...
    function getUserInfo(address user) public view returns(uint256[] memory) {
        ...
        if (isJoinedUser(user)) {
            ...
            info[8] = _userMaps[user].withdrawableWelfareWei;
            info[9] = getUserSpecialBonusWei(user);
            info[10] = _userMaps[user].roiPercentPerMonth;
            
            if (_userMaps[user].closed == false) {
                info[11] = (_userMaps[user].depositTime + _timelock).subNoNegative(block.timestamp); // Time to finish time lock
            } else {
                info[11] = 0;
            }
    ...
    function depositOpenAccount(address referrerUser) public payable returns(bool) {
    ...
    function withdrawCloseAccount() public returns(bool) {
