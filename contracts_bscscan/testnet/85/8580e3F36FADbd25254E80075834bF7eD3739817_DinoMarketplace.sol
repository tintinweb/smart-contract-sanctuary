...
contract DinoMarketplace is Pausable, Ownable {
  ...
  uint256 public skipHatchCooldownPrice = 500 * 10e18;
  ...
  function getEggDetail(uint256 _eggId) external view returns(uint256 genes, address owner, uint256 createdAt, uint256 readyHatchAt, bool isAvaiable) {
      Egg storage egg = eggs[_eggId];
      return (egg.genes, egg.owner, egg.createdAt, egg.readyHatchAt, egg.isAvaiable);
  }
  
  function updateEggStatus(uint256 _eggId, bool _isAvaiable) external onlyAdminOrOwner returns(uint256) {
      require(_eggId <= eggs.length -1, "egg_not_exist");
      eggs[_eggId].isAvaiable = _isAvaiable;
      return _eggId;
  }
