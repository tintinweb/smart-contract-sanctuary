    ...
    INafterMarketAuction
{
    ...
    mapping(uint256 => mapping(address => ActiveBidRange))
        private activeBidRange;
    ...
    function __NafterMarketAuction_init(
        ...
        iERC1155CreatorRoyalty = INafterRoyaltyRegistry(
            _iERC1155CreatorRoyalty
        );
     ...
     * @param _owner address of the token owner
     */
    function isOwnerOfTheToken(uint256 _tokenId, address _owner)
        public
        view
        returns (bool)
    {
     ...
     * @param _owner address of the token owner
     */
    function getSalePrice(uint256 _tokenId, address _owner)
        external
        view
     ...
     * @param _owner address of the token owner
     */
    function hasTokenActiveBid(uint256 _tokenId, address _owner)
        external
        view
        override
        returns (bool)
    {
        if (
            activeBid[_tokenId][_owner].bidder == _owner ||
            activeBid[_tokenId][_owner].bidder == address(0)
        ) return false;
     ...
     * @param _tokenId uint256 ID of the token
     */
    function getActiveBidRange(uint256 _tokenId, address _owner)
        external
        view
        returns (uint256, uint256)
    {
        return (
            activeBidRange[_tokenId][_owner].startTime,
            activeBidRange[_tokenId][_owner].endTime
        );
    }
    ...
    /**
     * @dev set nafter token only by admin
     * @param _token address of the nafter token
     */
    function setNafterToken(address _token) external onlyOwner {
    ...
    function setSalePrice(
        ...
        salePrice[_tokenId][_owner] = SalePrice(
            payable(_owner),
            _amount,
            _paymentMode
        );
    ...
    function restore(
        ...
        for (uint256 i = _startIndex; i < _endIndex; i++) {
            ...
            for (uint256 j = 0; j < owners.length; j++) {
                ...
                (address payable sender, uint256 amount) = oldContract
                    .getSalePrice(tokenId, owner);
                ...
                (
                    ...
                    uint8 marketplaceFee,
                    uint256 bidAmount
                ) = oldContract.getActiveBid(tokenId, owner);
                activeBid[tokenId][owner] = ActiveBid(
                    bidder,
                    marketplaceFee,
                    bidAmount,
                    0
                ...
                bidBalance[bidder] = bidBalance[bidder].add(
                    bidAmount.add(serviceFee)
                );

                (uint256 startTime, uint256 endTime) = oldContract
                    .getActiveBidRange(tokenId, owner);
                activeBidRange[tokenId][owner] = ActiveBidRange(
                    startTime,
                    endTime
                );
            }
        ...
        setMinimumBidIncreasePercentage(
            oldContract.minimumBidIncreasePercentage()
        );
    }
    ...
    function _checkConditionBeforeBuy(uint256 _tokenId, address _owner)
        internal
        view
    {
    ...
    function buyWithNafter(
        uint256 _tokenId,
        address _owner,
        uint256 _amount
    ) external {
        _checkConditionBeforeBuy(_tokenId, _owner);
        SalePrice memory sp = salePrice[_tokenId][_owner];
        require(sp.paymentMode == 1, "only naft payment");
        require(sp.amount > 0, "not for sale.");
        // Check that enough ether was sent.
        require(
            tokenPriceFeeIncluded(_tokenId, _owner) == _amount,
            "no correct price"
        );
        ...
        //funds tranfer to contract
        Payments.safeTransferFrom(msg.sender, address(this), _amount);
        // Payout all parties.
        address payable marketOwner = payable(owner());
        Payments.payout(
            sp.amount,
    ...
    function buy(uint256 _tokenId, address _owner) public payable {
        ...
        require(sp.paymentMode == 0, "ETH payment");
        ...
        if (_priceSetterStillOwnsTheToken(_tokenId, _owner)) {
            return
                salePrice[_tokenId][_owner].amount.add(
                    salePrice[_tokenId][_owner]
                        .amount
                        .mul(nafter.getServiceFee(_tokenId))
                        .div(100)
                );
        }
        return 0;
    ...
    function setInitialBidPriceWithRange(
        ...
        require(_bidAmount > 0, "cant bid 0");
        senderMustBeTokenOwner(_tokenId);
        ...
        emit SetInitialBidPriceWithRange(
            _bidAmount,
            _startTime,
            _endTime,
            _owner,
    ...
    function _checkBidConditions(
        uint256 _newBidAmount,
        uint256 _tokenId,
        address _owner
    ) internal view {
        // Check that bid is greater than 0.
        require(_newBidAmount > 0, "cant bid 0");
        ...
        if (priceType == 1)
            require(
                range.startTime < block.timestamp &&
                    range.endTime > block.timestamp,
                "cant place bid'"
            );
     ...
     * @param _owner address of the token owner
     */
    function bidWithNafter(
        uint256 _newBidAmount,
        uint256 _amount,
        uint256 _tokenId,
        address _owner
    ) external {
        _checkBidConditions(_newBidAmount, _tokenId, _owner);
        require(activeBid[_tokenId][_owner].paymentMode == 1, "in NAFT");
        ...
        uint256 requiredCost = _newBidAmount.add(
            _newBidAmount.mul(nafter.getServiceFee(_tokenId)).div(100)
        );
        require(requiredCost <= _amount, "no correct price.");

        // Refund previous bidder.
        _refundNafterBid(_tokenId, _owner);

        // Set the new bid.
        _setBid(_newBidAmount, payable(msg.sender), 1, _tokenId, _owner);
    ...
    function bid(
        ...
        _checkBidConditions(_newBidAmount, _tokenId, _owner);
        require(activeBid[_tokenId][_owner].paymentMode == 0, "only ETH");
        // Check that bid is higher than previous bid
        uint256 currentBidAmount = activeBid[_tokenId][_owner].amount;
        require(
            _newBidAmount > currentBidAmount &&
        ...
        require(requiredCost <= msg.value, "no correct price");

        // Refund previous bidder.
        _refundBid(_tokenId, _owner);

        // Set the new bid.
        _setBid(_newBidAmount, payable(msg.sender), 0, _tokenId, _owner);
        nafter.setBid(_newBidAmount, msg.sender, _tokenId, _owner);
        emit Bid(msg.sender, _newBidAmount, _tokenId);
    }
    ...
    function acceptBid(uint256 _tokenId, address _owner) public {
        ...
        iMarketplaceSettings.markERC1155Token(_tokenId, true);
        uint256 serviceFee = currentBid
            .amount
            .mul(currentBid.marketplaceFee)
            .div(100);
        bidBalance[currentBid.bidder] = bidBalance[currentBid.bidder].sub(
            currentBid.amount.add(serviceFee)
        );
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Cancel the nafter bid on the token.
     * @param _tokenId uint256 ID of the token.
     * @param _owner address of the token owner
     */
    function cancelNafterBid(uint256 _tokenId, address _owner) external {
        // Check that sender has a current bid.
        require(
            _addressHasBidOnToken(msg.sender, _tokenId, _owner),
            "cant cancel"
     ...
     * @param _owner address of the token owner
     */
    function _priceSetterStillOwnsTheToken(uint256 _tokenId, address _owner)
        internal
        view
        returns (bool)
    {
        return _owner == salePrice[_tokenId][_owner].seller;
    }
    ...
    function _setBid(
        ...
        //add bid balance
        uint256 serviceFee = _amount.mul(nafter.getServiceFee(_tokenId)).div(
            100
        );
...
contract Payments is SendValueOrEscrow {
  ...
  function payout(
    ...
    require(_marketplacePercentage <= 100, "cant be above 100");
    require(_royaltyPercentage.add(_primarySalePercentage) <= 100, "cant go beyond 100");
    require(
      _payee != address(0) &&
    ...
    if (_paymentMode == 0) {
      ...
      if (payments[2] > 0) {
        // SendValueOrEscrow.sendValueOrEscrow(_primarySalePayee, payments[2]);
        nafterToken.safeTransfer(_primarySalePayee, payments[2]);
      }
      // payeePayment
      if (payments[3] > 0) {
        // SendValueOrEscrow.sendValueOrEscrow(_payee, payments[3]);
