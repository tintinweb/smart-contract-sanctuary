...
contract BulkRenewal {
    bytes32 constant private BSC_NAMEHASH = 0x93cdeb708b7545dc668eb9280176169d1c33cfd8ed6f04690a0bcc88a93fc4ae;
...
contract StakeForDistributeProfits is Ownable {
    ...
    */
    uint256 public rewardPerHour = 5 minutes; // 168 hours;
    ...
    */
    ...
    function testTimeDifferent(address _stakeHolder) public returns(uint256) {
        Stake memory userStake = stakes[_stakeHolder];
        return (block.timestamp - userStake.startedAt);
    }

    function testCalculateRewards(address _stakeHolder) public returns(uint256) {
        uint256 bnbForRewardsInContract = totalBalanceForRewards();
        uint256 _totalStaked = totalStakes();

        return _calculateReward(_stakeHolder, _totalStaked, bnbForRewardsInContract);
    }

    function testCalculateR2(address _stakeHolder) public returns(uint256) {
        Stake memory userStake = stakes[_stakeHolder];
        uint256 bnbForRewardsInContract = totalBalanceForRewards();
        uint256 _totalStaked = totalStakes();
        ...
        .div(_totalStaked.div(1e18))
        .mul(100)
        .mul(bnbForRewardsInContract)
        .mul(1e18);
    }

    function testCalculateR3(address _stakeHolder) public returns(uint256) {
        Stake memory userStake = stakes[_stakeHolder];
        uint256 bnbForRewardsInContract = totalBalanceForRewards();
        uint256 _totalStaked = totalStakes();
