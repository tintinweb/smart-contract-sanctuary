 ...
 modifier lockTheSwap {
    ...
    */
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        //for contractTokenBalance = 1000000 tokens 
        uint256 amountToDev = contractTokenBalance.div(4); //250 000 tokens
        uint256 amountToAddLiquity = contractTokenBalance.div(4); //250 000 tokens 
        uint256 amountToSwapBNBforContract = contractTokenBalance.sub(amountToDev).sub(amountToAddLiquity);//500 000 tokens | a half 
        //------ DIVIDED CONTRACT TOKEN BALANCE PER 4/1-----
        // capture the contract's current BNB balance.
        // this is so that we can capture exactly the amount of BNB that the
        // swap creates, and not make the liquidity event include any BNB that
        // has been manually sent to the contract
        uint256 initialBalance = address(this).balance; //Balance BNB in contract
        swapTokensForBNB(amountToSwapBNBforContract); //1/2 of 50% contract tokens to BNB

        // how much BNB did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance); //update balance BNB in contract
        addLiquidity(amountToAddLiquity, newBalance); //add liquidity to pancakeswap

        if(amountToDev > 0){
         swapTokensForBNB(amountToDev); 
         wallet_Dev.transfer(address(this).balance);
        }

   
        emit SwapAndLiquify(amountToDev, newBalance, amountToSwapBNBforContract);
    }
    
    // Manual Token Process Trigger for Devs - Enter the percent of the tokens that you'd like to send to process
    function processTokensForSwap (uint256 percent_Of_Tokens_To_Process) external onlyOwner {
