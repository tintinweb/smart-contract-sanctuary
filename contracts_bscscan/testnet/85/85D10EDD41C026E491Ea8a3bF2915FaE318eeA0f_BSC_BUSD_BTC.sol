...
contract BSC_BUSD_BTC{
	...
	function getRemainingTokensIC() public { 	// TBA	
		require(msg.sender == ceoAddress, "6");
		icApp.transfer(msg.sender, icApp.balanceOf(address(this)));		
	}
	...
	function getBalances(address adr) public view returns(uint256, uint256, uint256,uint256,uint256,uint256) {
		return ( icApp.balanceOf(address(adr)), ocApp.balanceOf(address(adr)), ic[adr], icc[adr], oc[adr], icApp.allowance(address(adr), address(this)));
	}
