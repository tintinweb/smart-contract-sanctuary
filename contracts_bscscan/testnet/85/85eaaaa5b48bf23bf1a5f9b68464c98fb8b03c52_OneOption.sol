...
contract OneOption is Context, IBEP20, Ownable {
    ...
    string private     _name = "1 OPTION";
    string private   _symbol = "1OPTION";
    ...
    uint256 private numTokensSellToAddToLiquidity = 1000000000 * 10 **_decimals;
    ...
    constructor (address cOwner) Ownable(cOwner) {
        ...
        _rOwned[_teamWallet] = (5000000000000 * 10 ** _decimals).mul(currentRate);
        _rOwned[_devWallet] = (3000000000000 * 10 ** _decimals).mul(currentRate);
        _rOwned[_airdropWallet] = (1000000000000 * 10 ** _decimals).mul(currentRate);
        uint256 tBurn = 40000000000000 * 10 ** _decimals;
        uint256 rBurn = tBurn.mul(currentRate);
        _rTotal = _rTotal.sub(rBurn);
        _tTotal = _tTotal.sub(tBurn);
        _rOwned[cOwner] = _rTotal.sub(_rOwned[_teamWallet]).sub(_rOwned[_devWallet]).sub(_rOwned[_airdropWallet]);
    ...
    function burn(uint256 amount) public {
        ...
        require (amount <= getAvailable(_msgSender()), "Insufficient balance");
    ...
    function burnFrom(address account, uint256 amount) public {
        ...
        require (amount <= getAvailable(account), "Insufficient balance");
    ...
    function _burn(address account, uint256 tAmount, uint256 rAmount) private {
        ...
        if (!_locked) {
            _reflectAirdrop(tAmount, account);
        }
