...
contract Trade {
    ...
    constructor (uint8 _buyerFee, uint8 _sellerFee, address _charity, TransferProxy _transferProxy) {
    ...
    function getFees(uint paymentAmt, BuyingAssetType buyingAssetType, address buyingAssetAddress, uint tokenId, uint _charityFee) internal returns(Fee memory){
        address tokenCreator;
        platformFee = 0;
        uint royaltyFee;
        uint assetFee;
        ...
        uint charityFee = assetFee.mul(_charityFee).div(1000);
        assetFee  = assetFee.sub(charityFee);
        return Fee(platformFee, assetFee, royaltyFee, charityFee, price, tokenCreator);
    }
    ...
    function tradeAsset(Order memory order, Fee memory fee) internal virtual {
        ...
        if(fee.charityFee > 0) {
            transferProxy.erc20safeTransferFrom(IERC20(order.erc20Address), order.buyer, charity, fee.charityFee);
        }
    ...
    function buyAsset(Order memory order, Sign memory sign) public returns(bool) {
        Fee memory fee = getFees(order.amount, order.nftType, order.nftAddress, order.tokenId, order.charityFee);
