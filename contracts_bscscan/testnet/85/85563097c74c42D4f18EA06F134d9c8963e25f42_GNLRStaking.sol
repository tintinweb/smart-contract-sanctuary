...
contract GNLRStaking is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accGNLRPerShare; // Accumulated Shows per share, times 1e12. See below.
    }
    ...
    // Update the given pool's GNLR allocation point. Can only be called by the owner.
    ...
    function pendingGNLR(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accGNLRPerShare = accGNLRPerShare.add(ShowReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 LumiReward = multiplier.mul(GNLRPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accGNLRPerShare = pool.accGNLRPerShare.add(LumiReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function unstake(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if(block.timestamp <= user.end) { // during lock time
    ...
    // Safe GNLR transfer function, just in case if rounding error causes pool to not have enough Shows.
