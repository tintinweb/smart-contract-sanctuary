...
contract Chainance is Context, Ownable{
    ...
    uint256 public MaxAccountCount;
    
    constructor() {
        zeroBlock = block.number;
    }
    
    function UpdateAccountCount(uint256 _accountcount) public onlyOwner {
        require(_accountcount >= AccountCount(), "err_code : 10" ); 
        MaxAccountCount = _accountcount;
    }
    ...
    event CreateChainEvent(string DataCategory, string Data, string UserName, string DataImageLink, address AccountAddress);
    event CreateClapEvent(address ChainAddress, uint256 Id, address AccountAddress);
    ...
    struct Chain {
        uint256 index;
        uint256 ChainTime;
        string  ChainCategory;
        string  Chain;
        string  ChainImageLink;
        string  ChainUserName;
        address Chain_Address;
    }
    mapping(address => Chain[]) private ChainStruct;
    
    struct ChainClap {
        uint256 index;
        address ChainAddress;
        uint256 ChainId;
        address Clap_Address;
    } 
    mapping(address => ChainClap[]) private ClapStruct;

    //- Count
    function AccountCount() public view returns(uint256) {
        return AccountIndex.length;
    }
    function ChainCount(address _address) public view returns(uint256) {
        return ChainStruct[_address].length;
    }
    ...
    function Login(address _address) public AccountControl view returns(string memory) {
        if ( AccountStruct[_address].Account_Address == msg.sender ) return(AccountStruct[_address].UserName);
        revert ("err_code : 1"); //- Failed to login.
    }
    ...
    function UpdateAccount(address _address, string memory _UserName, string memory _NameSurname, string memory _Birthday, string memory _Email, string memory _Description, string memory _WebSite) public returns(bool) {
        require(AccountStruct[_address].Account_Address == msg.sender, "err_code : 6");
    ...
    //- Create  Data
    function CreateChain(address _address, string memory _ChainCategory, string memory _Chain, string memory _ChainImageLink) public LoginControl(_address) {
        string memory _UserName = AccountStruct[_address].UserName;
        ChainStruct[_address].push(Chain(ChainStruct[_address].length,block.timestamp,_ChainCategory,_Chain,_ChainImageLink,_UserName,_address));
        emit CreateChainEvent(_ChainCategory, _Chain, _UserName, _ChainImageLink, _address);
    } 
    ...
    function getAccount(address _address) public view returns(uint256, string memory, string memory, string memory, string memory, string memory, string memory) {
        require(AccountStruct[_address].Account_Address == msg.sender, "err_code : 6"); //- No account found at Wallet address.
    ...
    function getAccountImage(address _address) public view returns(uint256, string memory, string memory, address) {
        require(ImageStruct[_address].Image_Address == msg.sender, "err_code : 6");
    ...
    function getUserName(address _address) public view returns(string memory) {
        require(AccountStruct[_address].Account_Address == msg.sender, "err_code : 6");
        return AccountStruct[_address].UserName;
    }
    
    //- Get My Chain's
    function getChain(address _address) public view returns(uint256[] memory, uint256[] memory, string[] memory, string[] memory, string[] memory) {
        uint IndexLen = ChainStruct[_address].length;
        uint256[] memory chainid = new uint256[](IndexLen);
        uint256[] memory chaintime = new uint256[](IndexLen);
        string[] memory chaindata = new string[](IndexLen);
        string[] memory chaincategory = new string[](IndexLen);
        string[] memory chainimagelink = new string[](IndexLen);
        for ( uint i = 0; i < ChainStruct[_address].length; i++ ) {
            chainid[i] = ChainStruct[_address][i].index;
            chaintime[i] = ChainStruct[_address][i].ChainTime;
            chaindata[i] = ChainStruct[_address][i].Chain;
            chaincategory[i] = ChainStruct[_address][i].ChainCategory;
            chainimagelink[i] = ChainStruct[_address][i].ChainImageLink;
        }
        return(chainid, chaintime, chaindata, chaincategory, chainimagelink);
    }
    
    //- Set Chain Clap
    function setChainClap(address _address, uint256 _id) public {
        ClapStruct[_address].push(ChainClap(ClapStruct[_address].length, _address, _id, msg.sender));
        emit CreateClapEvent(_address, _id, msg.sender);
    }
    
    //- Get Chain Clap
    function getChainClap(address _address) public view returns(uint256[] memory) {
        uint IndexLen = ClapStruct[_address].length;
        uint256[] memory chainid = new uint256[](IndexLen);
        for ( uint i = 0; i < ClapStruct[_address].length; i++ ) {
            chainid[i] = ClapStruct[_address][i].ChainId;
        }
        return (chainid);
    }
