...
contract TEST is ERC20, Ownable {
    ...
    uint256 public totalFees = tokenRewardsFee.add(liquidityFee).add(wallet1Fee).add(wallet2Fee);
    ...
    function updateBurnWallet(address newBurnWallet) public onlyOwner {
        require(newBurnWallet != burnAddress, "TEST: The burn wallet is already this address");
        excludeFromFees(newBurnWallet, true);
        emit BurnWalletUpdated(newBurnWallet, burnAddress);
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 finalBalance = newBalance.sub(wallet1feeBalance).sub(wallet2feeBalance);
        uint256 finalHalf = otherHalf.sub(wallet1feePortion).sub(wallet2feePortion);
