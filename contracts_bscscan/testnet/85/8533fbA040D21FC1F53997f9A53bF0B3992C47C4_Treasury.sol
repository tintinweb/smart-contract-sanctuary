...
contract Treasury is Ownable {
    ...
    address public team;
    address public nftFarming;
    address public jackpot;
    address private immutable busd;
    address private immutable weth;

    uint256 public teamPercent = 10;
    uint256 public nftFarmingPercent = 70;
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        // Checks
        require(
            token != busd && token != weth && token != bridge,
    ...
    function _convertStep(
        ...
        uint256 amount0,
        uint256 amount1
    ) internal returns (uint256 busdOut) {
        // Interactions
        if (token0 == token1) {
            ...
            if (token0 == busd) {
                ...
                busdOut = _toBUSD(weth, amount);
            } else {
                address bridge = bridgeFor(token0);
                amount = _swap(token0, bridge, amount);
                busdOut = _convertStep(bridge, bridge, amount, 0);
            }
        } else if (token0 == busd) {
            // eg. SUSHI - ETH
            busdOut = _toBUSD(token1, amount1).add(amount0);
        } else if (token1 == busd) {
            // eg. USDT - SUSHI
            busdOut = _toBUSD(token0, amount0).add(amount1);
        } else if (token0 == weth) {
            // eg. ETH - USDC
            busdOut = _toBUSD(
                weth,
    ...
    {
        // X1 - X5: OK
        amountOut = _swap(token, busd, amountIn);
    }


    function _distribute(uint256 _amount) internal {
        IERC20(busd).safeTransfer(team, _amount * teamPercent / PRECISION);
        IERC20(busd).safeTransfer(nftFarming, _amount * nftFarmingPercent / PRECISION);
        IERC20(busd).safeTransfer(jackpot, _amount * jackpotPercent / PRECISION);
    }

    function changeTargetAddress(address _team, address _nftFarming, address _jackpot) external onlyOwner {
        team = _team;
        nftFarming = _nftFarming;
        jackpot = _jackpot;
    }
