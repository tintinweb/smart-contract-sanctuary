...
contract Faucet is Ownable {
    ...
    //todo
    //NFT Mapping -- input nft Address, return -- id?
    mapping(address => uint256[]) public NFT_Contracts;
    uint256 public availableNFTContracts = 0;
...
//todo check hold requirements for nfts
    function updateNFTContracts(address NFTAddress, uint256[] ids) public onlyOwner {
        NFT_Contracts[NFTAddress] = ids;
        availableNFTContracts++;
    }

    //todo add in update/ add/remove nft requirements/rewards
    ...
    //todo check NFT LEVEL
    function NFTLevel(address _addr, address[] NFTAddresses) public view returns (uint8) {
        uint8 _level = 0;
        //uint256[] storage depth = NFT_Contracts[NFTAddresses[0]];
        for (uint8 i = 0; i < availableNFTContracts; i++) {
            IERC1155 nft = IERC1155(address(NFTAddresses[i]));
            uint256[] storage ids = NFT_Contracts[NFTAddresses[i]];

            for (uint x = 0; x < ids.length; x++){
            
            if (nft.balanceOf(_addr, ids[x]) >= nft_balance)
            _level = 15;
