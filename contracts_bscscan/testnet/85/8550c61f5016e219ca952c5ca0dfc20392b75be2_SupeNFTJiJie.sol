...
contract SupeNFTJiJie is Ownable {
    ...
    struct PoolInfo {
        ...
        mapping(address => mapping(address=>uint256)) wait_claimed;
    ...
    function stake_one(uint256 _asset_id) public checkPaused{
        ...
        emit StakeOne(msg.sender, _asset_id, block.timestamp, block.timestamp + interval, round_id);
    }
    ...
    function exit_one(uint256 _round_id, uint256 _asset_id) public{
        ...
        uint256 _tlp = pools[_round_id].wait_claimed[msg.sender][address(tlp_token)];
        uint256 _frog = pools[_round_id].wait_claimed[msg.sender][address(frog_token)];
        if(_tlp > 0){
            IERC20(tlp_token).mint(msg.sender, _tlp);
            pools[_round_id].wait_claimed[msg.sender][address(tlp_token)] = 0;
        ...
        if(_frog > 0){
            ...
            pools[_round_id].wait_claimed[msg.sender][address(frog_token)] = 0;
        ...
        if(pools[_round_id].balances[msg.sender] >= 1){
            pools[_round_id].balances[msg.sender] = pools[_round_id].balances[msg.sender].add(1);
        }
    ...
    function _get_reward(uint256 _round_id, uint256 _asset_id) internal{
        ...
        if(attend_count == 1){
            ...
            pools[_round_id].wait_claimed[account][address(tlp_token)] = pools[_round_id].wait_claimed[account][address(tlp_token)].add(total_tlp);
            pools[_round_id].wait_claimed[account][address(frog_token)] = pools[_round_id].wait_claimed[account][address(tlp_token)].add(total_frog);
            pools[_round_id].tlps.push(total_tlp);
            pools[_round_id].frogs.push(total_frog);
            emit CreateReward(_round_id, 0, account, _asset_id, total_tlp, total_frog);
            ...
            pools[_round_id].lucky_accounts.push(account);
            pools[_round_id].wait_claimed[account][address(tlp_token)] = pools[_round_id].wait_claimed[account][address(tlp_token)].add(total_tlp);
            pools[_round_id].wait_claimed[account][address(frog_token)] = pools[_round_id].wait_claimed[account][address(frog_token)].add(total_frog);
            pools[_round_id].tlps.push(total_tlp);
            pools[_round_id].frogs.push(total_frog);
            emit CreateReward(_round_id, 0, account, _asset_id, total_tlp, total_frog);
        }else{
            ...
            uint256 third_index;
            seed = uint256(keccak256(abi.encode(msg.sender, first_index, _round_id + 1, _asset_id, block.timestamp, blockhash(block.number))));
            if(first_index == 0){
                second_index = first_index + 1 + seed % (attend_count - 1);
                if(second_index != attend_count -1){
                    seed = uint256(keccak256(abi.encode(msg.sender, first_index, second_index, _round_id + 2, _asset_id, block.timestamp, blockhash(block.number))));
            ...
            uint256 _tlp_reward = total_tlp.mul(50).div(100);
            uint256 _frog_reward = total_frog.mul(50).div(100);
            {
            ...
            pools[_round_id].wait_claimed[first_account][address(tlp_token)] = pools[_round_id].wait_claimed[first_account][address(tlp_token)] + _tlp_reward;
            pools[_round_id].wait_claimed[first_account][address(frog_token)] = pools[_round_id].wait_claimed[first_account][address(frog_token)] + _frog_reward;
            pools[_round_id].tlps.push(_tlp_reward);
            pools[_round_id].frogs.push(_frog_reward);
            emit CreateReward(_round_id, 0, first_account, first_asset_id, _tlp_reward, _frog_reward);
            }
            {
            ...
            pools[_round_id].wait_claimed[second_account][address(tlp_token)] = pools[_round_id].wait_claimed[second_account][address(tlp_token)] + _tlp_reward;
            pools[_round_id].wait_claimed[second_account][address(frog_token)] = pools[_round_id].wait_claimed[second_account][address(frog_token)] + _frog_reward;
            pools[_round_id].tlps.push(_tlp_reward);
            pools[_round_id].frogs.push(_frog_reward);
            emit CreateReward(_round_id, 1, second_account, second_asset_id, _tlp_reward, _frog_reward);
            }
            {
            ...
            pools[_round_id].wait_claimed[third_account][address(tlp_token)] = pools[_round_id].wait_claimed[third_account][address(tlp_token)] + _tlp_reward;
            pools[_round_id].wait_claimed[third_account][address(frog_token)] = pools[_round_id].wait_claimed[third_account][address(frog_token)] + _frog_reward;
            pools[_round_id].tlps.push(_tlp_reward);
            pools[_round_id].frogs.push(_frog_reward);
            emit CreateReward(_round_id, 2, third_account, third_asset_id, _tlp_reward, _frog_reward);
            }
        }
    ...
    function exit_stake_one(uint256 _asset_id) public checkPaused{
        exit_one(0, _asset_id);
        stake_one(_asset_id);
    }
    function get_pool_1st_info(uint256 _round_id, address _account) external view returns 
        (
    ...
    function get_pool_2nd_info(uint256 _round_id, address _account) external view returns 
        (
            uint256 attend_asset_count, 
            uint256 _2asset,
            address _2account,
    ...
    function get_pool_3rd_info(uint256 _round_id, address _account) external view returns 
        (
            uint256 attend_asset_count, 
            uint256 _3asset,
            address _3account,
