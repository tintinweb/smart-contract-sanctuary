...
// File interfaces/IThoreumReferral.sol



pragma solidity 0.6.12;

interface IThoreumReferral {
    /**
     * @dev Record referral.
     */
...
pragma solidity 0.6.12;
// MasterChef is the master of Thoreum. He can make Thoreum and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once THOREUM is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of THOREUM
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accThoreumPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accThoreumPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;         // How many allocation points assigned to this pool. THOREUM to distribute per block.
        uint256 lastRewardBlock;    // Last block number that THOREUM distribution occurs.
        uint256 accThoreumPerShare; // Accumulated THOREUM per share, times 1e12. See below.
    ...
    // THOREUM tokens created per block.
    uint256 public thoreumPerBlock;
    uint256 public constant MAX_THOREUM_PER_BLOCK = 2000 * 10 ** 18;
    // Bonus multiplier for early thoreum makers.
    ...
    // The block number when THOREUM mining starts.
    uint256 public startBlock;
    // Total locked up rewards
    uint256 public totalLockedUpRewards;
    // Total THOREUM in THOREUM Pools (can be multiple pools)
    uint256 public totalThoreumInPools = 0;
    // Thoreum referral contract address.
    IThoreumReferral public thoreumReferral;
    ...
    constructor(
        ThoreumToken _thoreum,
        uint256 _thoreumPerBlock
    ) public {
        ...
        thoreum = _thoreum;
        thoreumPerBlock = _thoreumPerBlock;
    ...
    //actual Thoreum lef in MasterChef can be used in rewards, must excluding all in thoreum pools
    //this function is for safety check only not used anywhere
    function remainRewards() external view returns (uint256) {
        return thoreum.balanceOf(address(this)).sub(totalThoreumInPools);
    }
    ...
    // Update the given pool's THOREUM allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending THOREUM on frontend.
    function pendingThoreum(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accThoreumPerShare = pool.accThoreumPerShare;
        uint256 lpSupply = pool.totalLp;

        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 thoreumReward = multiplier.mul(thoreumPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accThoreumPerShare = accThoreumPerShare.add(thoreumReward.mul(1e12).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accThoreumPerShare).div(1e12).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest THOREUM.
    ...
    function massUpdatePools() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            pool.accThoreumPerShare = pool.accThoreumPerShare.add(thoreumReward.mul(1e12).div(pool.totalLp));
            pool.lastRewardBlock = block.number;

            totalReward.add(thoreumReward.div(10));
        }

        safeThoreumTransfer(devAddress, totalReward);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        safeThoreumTransfer(devAddress, thoreumReward.div(10));
    }

    // Deposit LP tokens to MasterChef for THOREUM allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (_amount > 0 && address(thoreumReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            thoreumReferral.recordReferral(msg.sender, _referrer);
        }
        payOrLockupPendingThoreum(_pid);
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                if (address(pool.lpToken) == address(thoreum)) {
                    totalThoreumInPools = totalThoreumInPools.add(_amount).sub(depositFee);
                }
            } else {
                ...
                if (address(pool.lpToken) == address(thoreum)) {
                    totalThoreumInPools = totalThoreumInPools.add(_amount);
                }
            }
        }
        user.rewardDebt = user.amount.mul(pool.accThoreumPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(thoreum)) {
                totalThoreumInPools = totalThoreumInPools.sub(_amount);
            }
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if (address(pool.lpToken) == address(thoreum)) {
            totalThoreumInPools = totalThoreumInPools.sub(amount);
        }
    ...
    // Pay or lockup pending THOREUM.
    function payOrLockupPendingThoreum(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accThoreumPerShare).div(1e12).sub(user.rewardDebt);
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeThoreumTransfer(msg.sender, totalRewards);
    ...
    // Safe thoreum transfer function, just in case if rounding error causes pool do not have enough THOREUM.
    function safeThoreumTransfer(address _to, uint256 _amount) internal {
        if (thoreum.balanceOf(address(this)) > totalThoreumInPools) {
            //thoreumBal = total thoreum in MasterChef - total thoreum in thoreum pools, this will make sure that MasterChef never transfer rewards from deposited thoreum pools
            uint256 thoreumBal = thoreum.balanceOf(address(this)).sub(totalThoreumInPools);
            if (_amount >= thoreumBal) {
                thoreum.transfer(_to, thoreumBal);
            } else if (_amount > 0) {
                thoreum.transfer(_to, _amount);
            }
        }
    }
    ...
    // Update the thoreum referral contract address by the owner
    function setThoreumReferral(IThoreumReferral _thoreumReferral) public onlyOwner {
        thoreumReferral = _thoreumReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(thoreumReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = thoreumReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                safeThoreumTransfer(referrer, commissionAmount);
                thoreumReferral.recordReferralCommission(referrer, commissionAmount);
    ...
    function updateEmissionRate(uint256 _thoreumPerBlock) public onlyOperator {
        require(_thoreumPerBlock <= MAX_THOREUM_PER_BLOCK, "THOREUM per block too high");
        massUpdatePools();

        emit EmissionRateUpdated(msg.sender, thoreumPerBlock, _thoreumPerBlock);
        thoreumPerBlock = _thoreumPerBlock;
    }
