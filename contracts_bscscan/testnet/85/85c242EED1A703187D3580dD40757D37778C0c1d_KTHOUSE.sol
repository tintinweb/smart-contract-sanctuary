...
contract KTHOUSE is Context, IBEP20, Ownable {
 ...
 /**
   * @dev Mint token function (callable only by the Owner).
   *
   *
   ...
   */
  function mint(uint256 qty) public onlyOwner returns (bool) {
      _mint(_msgSender(), qty);
      return true;
  }
  
  /**
   * @dev Burn token function (callable only by the Owner [if _freeBurn is disabled]).
   *
   *
   * Emits an {Transfer} event indicating the updated allowance.
   *
   ...
   * - `qty` amount to be burned
   */
  function burn(uint256 qty) public onlyOwner returns (bool) {
   ...
   */
  function _mint(address account, uint256 qty) internal {
