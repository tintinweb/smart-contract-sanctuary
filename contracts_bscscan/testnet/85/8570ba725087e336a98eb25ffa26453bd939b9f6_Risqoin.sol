...
contract Risqoin is Context, IERC20, Ownable {
    ...
    address private _operationWalletAddress = 0x2eD68868bb473D8E6b487525E9B025E357ef74eD;
    ...
    string private _name = "Risqoin";
    string private _symbol = "RISQ ";
    uint8 private _decimals = 9;
    
    uint256 public _taxFee = 5;
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _operationFee = 3;
    ...
    function setOperationsFeePercent(uint256 operationFee) external onlyOwner() {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tCharity = calculateOperationFee(tAmount);
    ...
    function _takeCharity(uint256 tCharity) private {
        uint256 currentRate =  _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
        _rOwned[_operationWalletAddress] = _rOwned[_operationWalletAddress].add(rCharity);
        if(_isExcluded[_operationWalletAddress])
            _tOwned[_operationWalletAddress] = _tOwned[_operationWalletAddress].add(tCharity);
    }
    ...
    function calculateOperationFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_operationFee).div(
            10**2
        );
    }
