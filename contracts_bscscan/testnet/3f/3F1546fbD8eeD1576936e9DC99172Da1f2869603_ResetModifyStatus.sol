...
contract ResetModifyStatus is Divestor {
    uint public fee;
    IERC20 bank;

    using SafeERC20 for IERC20;

    event Reset(address user);

    constructor() {
        fee = 1e8;
        bank = IERC20(0x715d4Ae3237Ba6447154fa4C2816D0D7dd776981);
    }

    function editBank(address bank_) external onlyOwner {
        require(bank_ != address(0) && bank_ != address(bank),"wrong bank");
        bank = IERC20(bank_);
    }

    function editFee(uint fee_) external onlyOwner {
        require(fee_ != 0 && fee_ != fee, "wrong value");
        fee = fee_;
    }

    function resetStatus() external {
        bank.safeTransferFrom(_msgSender(), address(this), fee);
        emit Reset(_msgSender());
    }
