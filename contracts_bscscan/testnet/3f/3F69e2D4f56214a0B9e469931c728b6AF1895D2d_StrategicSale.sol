...
contract StrategicSale is AccessControl {
    ...
    uint256 public lockPeriod = 2592000; // 30 days
    uint256 public lockAmount = 15000000000000000000000000; // 15,000,000 IMBA
    ...
    function getBalanceToken(address _address) public view returns( uint256 ) {
        ...
        if(block.timestamp > lockTime){
            ...
            uint256 remainAmount90Percent = remainAmount[address(_address)].mul(9).div(10);
            uint256 amount90Percent = remainAmount90Percent.mul(claimPeriod).div(vestingPeriod) > withdrawalAmount[address(_address)] ? (remainAmount90Percent.mul(claimPeriod).div(vestingPeriod) - withdrawalAmount[address(_address)]) : 0;
            amount = amount90Percent + remainAmount[address(_address)].div(10);
        }
        return amount;
    }
