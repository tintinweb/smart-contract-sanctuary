 ...
 function whitelistUser(address _user) public onlyOwner {
  ...
  function withdraw() public payable onlyOwner {
    require(payable(msg.sender).send(address(this).balance));
      ...
      function setPayFG(address _pay) public onlyOwner {
   PayTokenFG = _pay; 
         
     }
     
     function setPriceFG(uint _price) public onlyOwner {
    amountToken = _price;
    }
    
    // contract wallet to receive the partner/set token 
    function setTokenCollector(address _vault) public onlyOwner {
   vault = _vault;
    }
}
// File: FG-NFTSTAKE.sol
...
contract SmartNFTPoolRenewable is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // The staked token
    IDigitalaxNFT public stakedToken;
    ...
    */
        ...
        external
    {
        ...
        for (uint i = 0; i < balance; i++) {
            _stake(stakedToken.tokenOfOwnerByIndex(msg.sender,i));
        }
    }
    ...
    function _unstake(uint256 _tokenId) internal {
        ...
        user.tokenIds[user.tokenIndex[_tokenId]] = lastIndexKey;
