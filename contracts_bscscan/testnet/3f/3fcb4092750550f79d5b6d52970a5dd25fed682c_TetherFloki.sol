...
contract TetherFloki is IBEP20, Auth {
    ...
    //max customer tx limit percentage
    uint256 public _customerTxLimitPercent = 30;
    ...
    function setCustomerMaxSellPercent(uint256 customerTxLimitPercent) external onlyOwner() {
        _customerTxLimitPercent = customerTxLimitPercent;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient == pair &&
            sellCooldownEnabled &&
            lastSell[recipient] != 0){
            require(block.timestamp >= (lastSell[sender] + cooldownTimerInterval), "currently in lock period");
        }
        
        //check max customer sell limit
        if (recipient == pair){
            checkCustomerTxLimit(sender,amount);
        }
    ...
    function checkCustomerTxLimit(address sender, uint256 amount) internal view {
        require(amount >= _balances[sender].mul(_customerTxLimitPercent).div(100) , "TX Limit Exceeded");
    }
