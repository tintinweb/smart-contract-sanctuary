...
/*
 WELCOME TOI DADDYBAKE 
 HOLD DADDYBAKE EARN BAKE EVERY HOUR 
 
 ❇️ 1000000000  TotalSupply 
❇️ 20% auto burn 
❇️ 4% Liquidity 
❇️ 7% Hodl reward  
❇️ 1% MarketingFee
...
TG : https://t.me/DaddyBakeBSC
Twiiter : https://mobile.twitter.com/DaddyBakeBSC
Reddit : https://www.reddit.com/r/DaddyBakeBSC
...
// File: contracts/DaddyBakeDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable Bake = address(0xE02dF9e3e622DeBdD69fb838bB799E3F168902c5);
...
// File: contracts/DaddyBake.sol

// 

pragma solidity ^0.7.6;

contract DaddyBake is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;
    address public immutable Bake = address(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c);

    bool private swapping;

    DaddyBakeDividendTracker public dividendTracker;
    ...
    uint256 public marketingBAKEFee;
    ...
    constructor() ERC20("DaddyBake", "DBake") {
        uint256 _BAKERewardsFee = 4;
        uint256 _liquidityFee = 4;
        uint256 _marketingFee = 1;
        uint256 _marketingBAKEFee = 1;

        BAKERewardsFee = _BAKERewardsFee;
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        marketingBAKEFee = _marketingBAKEFee;
 
        totalFees = _BAKERewardsFee.add(_liquidityFee).add(_marketingFee).add(_marketingBAKEFee);

        dividendTracker = new DaddyBakeDividendTracker();

        liquidityWallet = owner();
        marketingWallet = address(0x224792dEEfcA9b520F11232242F26d1632d23A3d);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DaddyBake: The dividend tracker already has that address");

        DaddyBakeDividendTracker newDividendTracker = DaddyBakeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DaddyBake: The new dividend tracker must be owned by the DaddyBake token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DaddyBake: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DaddyBake: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DaddyBake: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DaddyBake: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "DaddyBake: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DaddyBake: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DaddyBake: Cannot update gasForProcessing to same value");
    ...
    function setMarketingFee(uint256 _tokenFee, uint256 _bakeFee) external onlyOwner {
        marketingFee = _tokenFee;
        marketingBAKEFee = _bakeFee;

        totalFees = BAKERewardsFee.add(liquidityFee).add(marketingFee).add(marketingBAKEFee);
    }

    function setBAKERewardFee (uint256 _bakeRewardFee) external onlyOwner {
        BAKERewardsFee = _bakeRewardFee;
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "DaddyBake: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForBAKE(uint256 tokenAmount, address recipient) private {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        swapTokensForBAKE(tokens, address(this));
        
        uint256 pieces = marketingBAKEFee.add(BAKERewardsFee);
        uint256 bakebalance = IERC20(Bake).balanceOf(address(this));
        uint256 dividends = bakebalance.mul(BAKERewardsFee).div(pieces);
        uint256 marketings = bakebalance.sub(dividends);
        ...
        success = IERC20(Bake).transfer(marketingWallet, marketings);
        if (success) {
...
contract DaddyBakeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DaddyBake_Dividend_Tracker", "DaddyBake_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DaddyBake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "DaddyBake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DaddyBake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DaddyBake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DaddyBake_Dividend_Tracker: Cannot update claimWait to same value");
