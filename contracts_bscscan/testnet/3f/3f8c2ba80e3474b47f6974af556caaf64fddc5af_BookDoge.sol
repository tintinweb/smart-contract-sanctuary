...
contract DividendDistributor is IDividendDistributor {
    ...
    function deposit(uint256 amount) external override onlyToken {
        // Reflectable doge is already in balanceOf(address(BookDoge))
...
contract BookDoge is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "BookDoge";
    string private _symbol = "$BookDoge";
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (newBalanceDev > 0) {
            IBEP20(DOGE).transfer(devAddress, newBalanceDev);
        }
