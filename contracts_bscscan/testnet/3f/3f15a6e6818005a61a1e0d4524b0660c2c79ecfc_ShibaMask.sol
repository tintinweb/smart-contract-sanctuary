...
contract ShibaMask is Context, IERC20, Ownable {
    ...
    address public marketingAddress = 0xD97517E109D6383808f525FbAC5EfAA5f82D1749;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "ShibaMask";
    string private constant _symbol = "SHBMA";
    ...
    Taxes public sellTaxes = Taxes(1,1,8,2);
    ...
    function serTradingStatus(bool state) external onlyOwner{
    ...
    function updateAntiDummp(uint256 _maxSellAmountPerCycle, uint256 timeInMinutes) external onlyOwner{
    ...
    function aidropTokensV2(address[] memory accounts, uint256[] memory amounts) external onlyOwner{
        require(accounts.length == amounts.length, "Arrays must have the same size");
        for(uint256 i= 0; i < accounts.length; i++){
            taxFreeTransfer(msg.sender, accounts[i], amounts[i]);
        }
    }
