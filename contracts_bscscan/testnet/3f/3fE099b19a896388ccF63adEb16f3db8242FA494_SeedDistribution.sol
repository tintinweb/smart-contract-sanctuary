...
contract SeedDistribution {
    ...
    IERC20 private immutable _returnToken = IERC20(0xDa44884bd71E1A22a30Ef5577d6271B90Cf2006D);
    ...
    function addAddresses(address[] memory _addresses, uint256[] memory _ammounts) public onlyOwner {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            inPresale[_addresses[i]].tgeCollect = (((_ammounts[i]*10**2)/seedPrice)*tgeToCollect)/(1*10**5);
            inPresale[_addresses[i]].nrmCollect = (((_ammounts[i]*10**2)/seedPrice)*normalCollect)/(1*10**5);
        }
