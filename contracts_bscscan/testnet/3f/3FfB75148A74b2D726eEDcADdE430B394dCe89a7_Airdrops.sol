...
*/
	...
	constructor( iMoonCoin _MoonCoin) public { 
	  ...
	  receive() external payable {}
		...
		function getAirdropDetails(address userAddress) public view returns( uint256, uint256, uint256, uint256,uint256,uint256, uint256, uint256) {
			User storage user = users[userAddress];			
			return (user.issued, user.claimed, user.joined, user.allocated, user.pastClaims,user.allocateTime, user.userId, MoonCoinApp.balanceOf(address(this)));	
