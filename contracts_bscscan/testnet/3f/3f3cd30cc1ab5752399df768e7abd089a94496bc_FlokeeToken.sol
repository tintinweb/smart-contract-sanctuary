...
// FlokeeToken with Governance.
contract FlokeeToken is KRC20 {
    ...
    event FlokeeSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "FLOKEE::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the FlokeeToken contract.
     */
    constructor() public KRC20("Flokee Token", "FLOKEE") {
    ...
    /// @dev overrides transfer KRC20 function to meet tokenomics of FLOKEE
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (recipient == BURN_ADDRESS || _excludedFromTrsfTax[sender] == true || _excludedFromTrsfTax[recipient] == true) {            
            ...
            require(amount == sendAmount + burnAmount, "FLOKEE::transfer: Burn value invalid");
    ...
    function maxTransferAmount() public view returns (uint256) {
        ...
        // if antiwhale maxTransferAmountRate is set too low, a minimum of 5000 FLOKEE is applied
    ...
    function updateBurnRate(uint16 _burnRateTax) public onlyOperator {
        require(_burnRateTax <= 200, "FLOKEE::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "FLOKEE::updateMaxTransferAmountRate: Max transfer amount rate must not exceed 100 percents.");
        require(_maxTransferAmountRate >= 1, "FLOKEE::updateMaxTransferAmountRate: Max transfer amount rate must not be less than 0.01 percents.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "FLOKEE::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "FLOKEE::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "FLOKEE::delegateBySig: invalid nonce");
        require(now <= expiry, "FLOKEE::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "FLOKEE::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying FLOKEEs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "FLOKEE::_writeCheckpoint: block number exceeds 32 bits");
    ...
    //a way to get back other KRC20 tokens sended by error into the FLOKEE token contract
    function inCaseTokensGetStuck(address _token, uint256 _amount) public onlyOperator {
        IKRC20(_token).transfer(msg.sender, _amount);
    }    
