...
interface Flottery{
    ...
    function drawFinalNumberAndMakeLotteryClaimable(uint256, bool, uint256) external;
...
contract FarmageddonLottoKeeper is KeeperCompatibleInterface, Ownable {
    ...
    function StartKeeper(uint256 _upKeepTime, uint256 _step) external onlyOwner {
        fcurrentLotteryId = LotteryAddress.viewCurrentLotteryId();
        require(fcurrentLotteryId >= 1, "Initial Lottery must be set");
    ...
    function quickUpkeep() external onlyOwner {
            LotteryAddress.closeLottery(fcurrentLotteryId);
            step = 2;
            LotteryAddress.drawFinalNumberAndMakeLotteryClaimable(fcurrentLotteryId, true, fburnDivisor);
            step = 3;
            upKeepTime += interval;
    ...
    function testOneEndLotteryUpkeep() external onlyOwner {
            LotteryAddress.closeLottery(fcurrentLotteryId);
            step = 2;
            LotteryAddress.drawFinalNumberAndMakeLotteryClaimable(fcurrentLotteryId, true, fburnDivisor);
            step = 3;
    ...
    function test2StartLottertUpkeep() external onlyOwner {
            upKeepTime += interval;
            LotteryAddress.startLottery(upKeepTime, fticketPrice, fdiscount, frewardsBreakdown, ftreasuryFee);           
            fcurrentLotteryId = LotteryAddress.viewCurrentLotteryId();
            step = 1;
