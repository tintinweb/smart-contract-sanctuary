...
contract EarnOtherFixedAPRLockReward is Ownable, ReentrancyGuard {
  ...
  // Ratio of stakedToken/rewardToken
  ...
  uint256 private constant REWARD_PER_BLOCK_PRECISION_FACTOR = 1e38;
  ...
  constructor(
    ...
    rewardPerBlock = apr.div(BLOCK_PER_DAY.mul(365)).mul(_tokenRatio);
  ...
  function deposit(uint256 _amount) external nonReentrant {
    require(block.number < endBlock, "EarnOther::deposit:: not allow after end period");
    ...
    rewardDebt = rewardDebt.add(
      _amount.mul((endBlock.sub(lastRewardBlock)).mul(rewardPerBlock)).div(REWARD_PER_BLOCK_PRECISION_FACTOR)
    );
  ...
  function _calculateReward(uint256 _amount, uint256 _fromBlock) internal view returns (uint256) {
    ...
    return _amount.mul(blocks.mul(rewardPerBlock)).div(REWARD_PER_BLOCK_PRECISION_FACTOR);
  }

  function pendingReward(address _for) external view returns (uint256) {
    require(_for != address(0), "EarnOther::pendingReward:: bad address");
  ...
  function withdraw() external nonReentrant {
    require(block.number > endBlock, "EarnOther::withdraw:: not allow before end period");
    UserInfo storage user = userInfo[msg.sender];
  ...
  function recoveryReward(address _for) external onlyOwner {
    require(block.number > endBlock, "EarnOther::recoveryReward:: not allow before end period");
  ...
  function emergencyWithdraw() external nonReentrant {
    require(block.number > endBlock, "EarnOther::emergencyWithdraw:: not allow before end period");
    UserInfo storage user = userInfo[msg.sender];
