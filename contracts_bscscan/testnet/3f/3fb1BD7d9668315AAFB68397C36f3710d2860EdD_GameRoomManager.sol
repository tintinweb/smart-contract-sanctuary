...
contract GameRoomManager is SafeOwnable, Random {
    ...
    constructor(IPandoraERC1155 _nftToken, address _receiver, address _rewardReceiver, address _linkAccessor) Random(_linkAccessor) {
    ...
    function claim(uint256 _rid, uint256 _loop, address _to) external {
        ...
        (uint totalBalance, uint[] memory balances) = nftToken.totalBalance(address(this), nftIDs[_rid][_loop]);
        require(totalBalance == 1, "illegal balance");
        uint resultNftId = uint(-1);
        for (uint i = 0; i < balances.length; i ++) {
            if (balances[i] > 0) {
                resultNftId = balances[i];
                break;
            }
        }
