...
contract Reincarnate is Context, IERC20, Ownable {
    ...
    string private constant SYMBOL = "RT2";
    ...
    uint8[2] public _marketingFee = [2, 1];
    uint8[2] public _rugMemoriamFee = [1, 0];
    uint8[2] public _productionsFee = [1, 1];
    uint8[] public _additionalSellFeeSteps = [0, 1, 2, 4, 7, 12];
    uint8 public _kothWinnerAdditionalPercent = 2;
    uint256 public _rugMemoriamTokens;
    uint256 public _marketingTokens;
    
    uint8 public _numConsecutiveSells;
    
    uint256 public _kothTriggerAmount = 2 * 10**15; //1_100 * 10**18; //CHANGEME
    uint256 public _kothWinnableBuyAmount = 1 * 10**15; //100 * 10**18; //CHANGEME
    ...
    address public _rugMemoriamAddress = 0x201B384e5353D97ab3a99cbf00D0cCB1776D65D5; // CHANGEME
    ...
    function _transfer(
        ...
        if (overMinTokenBalance && !inSwapAndLiquify && from != uniswapV2Pair && swapAndLiquifyEnabled)
            swapAndLiquify(contractTokenBalance.sub(_rugMemoriamTokens).sub(_marketingTokens));
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapTokensForEth(half.add(_rugMemoriamTokens).add(_marketingTokens)); 

        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);

        // add liquidity to uniswap
        uint256 feeEth = addLiquidity(otherHalf, newBalance);
        
        if (feeEth > 0) {
            uint256 rugMemoriamEth = feeEth.mul(_rugMemoriamTokens).div(_marketingTokens.add(_rugMemoriamTokens));
            uint256 marketingEth = feeEth.sub(rugMemoriamEth);
            _rugMemoriamTokens = 0;
            _marketingTokens = 0;
            
            if (!feesInBUSD) {
                (bool successRM, ) = payable(_rugMemoriamAddress).call{ value: rugMemoriamEth }("");
                (bool successMarketing, ) = payable(_marketingAddress).call{ value: marketingEth }("");
                require (successMarketing && successRM, "Failed to send fees to rugMemoriam and Marketing Wallets");
            } else {
                swapEthForBUSDAndTransfer (rugMemoriamEth, _rugMemoriamAddress);
    ...
    function _takeOtherFees (uint256 tOtherFees, address sender, uint8 feeType) private {
        ...
        _rugMemoriamTokens = _rugMemoriamTokens.add(tOtherFees.sub(tOtherFees.mul(_liquidityFee[feeType].add(_marketingFee[feeType]).add(_productionsFee[feeType])).div(otherFeesDivisor))); 
        _marketingTokens = _marketingTokens.add(tOtherFees.mul(_marketingFee[feeType]).div(otherFeesDivisor));
    }
    ...
    function makeKOTHChanges (address from, address to, uint256 amount) internal {
        if (to == uniswapV2Pair && from != address(this) && _numConsecutiveSells < _additionalSellFeeSteps.length - 1) { 
            // Sell, haven't had KOTH number of sells so increment taxes
            _numConsecutiveSells++;
            
            // If the sell is big enough to trigger KOTH then record this. KOTH will start on a large enough buy
            if (getTokenValue (amount) > _kothTriggerAmount && !_inKOTH) 
                _kothTriggerActive = true;
        } else if (to == uniswapV2Pair && from != address(this) && !_inKOTH) {
            // Sell whilst at maximum sell tax, and above the value that triggers KOTH (only if not already in KOTH)
            _kothTriggerActive = true;
