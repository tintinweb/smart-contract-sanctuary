...
contract Olympia is ERC20, Ownable {
    ...
    constructor(address distributor, address newRouter) ERC20("Olympia", "OLP") {
    ...
    function _distributeReflection(address sender, address recipient) private {
        if (_shouldSetShare(sender, recipient)) {
            try IReflectionDistributor(payable(_reflectionDistributor)).setShare(sender, balanceOf(sender)) {} catch {}
            try IReflectionDistributor(payable(_reflectionDistributor)).setShare(recipient, balanceOf(recipient)) {} catch {}
        }
        
        if (!_inSwap) {
	    	try IReflectionDistributor(_reflectionDistributor).process(_gasForProcessing) {
