...
contract FanDungeonAdventure is
    ...
    Pausable
{
    event NewHeroAndLandAndStageAndCrystal(
        address sender,
        address hero,
        address land,
        address stage,
    ...
    uint256 public crystalPoolSize = 10000 ether;
    ...
    function setCrystalPoolSize(uint256 _crystalPoolSize) external onlyRole(OPERATOR_ROLE) {
       crystalPoolSize =  _crystalPoolSize;
    } 

    function setHeroAndLandAndStageAndCrystalAddress(
        address _fanDungeonHero,
        address _fanDungeonLand,
        address _fanDungeonStage,
        address _crystalToken
        ...
        emit NewHeroAndLandAndStageAndCrystal(
            msg.sender,
            _fanDungeonHero,
            _fanDungeonLand,
            _fanDungeonStage,
    ...
    function deposit(
        ...
        require(fanDungeonLand.landOf(_dungeonId).zone == 1, "Adventure: Zone");
        require(
            _level <= fanDungeonStage.levelCapOf(_dungeonId) && _level > 0,
        ...
        uint64 _timestamp = uint32(block.timestamp);
        ...
        ) = _takeReservedRewards(
                _dungeonId,
                _level,
                fanDungeonStage.bonusOf(_dungeonId).duration,
                _timestamp
            );
    ...
    function withdraw(uint256 _teamId) external {
        ...
        _earnCrystal(
            ...
            _crystal,
            fanDungeonStage.feeOf(_teams[_teamId].dungeonId)
        );
    ...
    function calculateAllHeroPower(uint256[] memory _heroTokenIds)
        public
        view
        returns (uint32 power)
    {
        for (uint256 i; i < _heroTokenIds.length; i++) {
            power += fanDungeonHero.totalStatPointsOf(_heroTokenIds[i]);
        }
    }
    ...
    function _takeReservedRewards(
        uint256 _dungeonId,
        uint32 _dungeonLevel,
        uint64 reduceDuration,
        uint64 _timestamp
    )
        internal
        returns (
    ...
    {
        Dungeon memory dungeon = dungeonOf(_dungeonId);

        uint256 _remainingCrystal = crystalPoolSize - dungeon.reservedCrystal;
        uint256 _maxCrystalPerTurn = _dungeonLevelInfo[_dungeonLevel]
            .crystalPerSecond * maximumDuration;
        uint64 duration;
        if (_remainingCrystal < _maxCrystalPerTurn) {
            duration = uint64(
                _remainingCrystal /
                    _dungeonLevelInfo[_dungeonLevel].crystalPerSecond
            );
        } else {
            duration = uint64(
                _maxCrystalPerTurn /
        ...
        return _setDungeon(_dungeonId,duration,_dungeonLevelInfo[_dungeonLevel].xpPerSecond,_dungeonLevelInfo[_dungeonLevel].crystalPerSecond,reduceDuration,_timestamp);
    }

    function _setDungeon(uint256 _dungeonId,uint64 duration,uint64 xpPerSecond,uint256 crystalPerSecond,uint64 reduceDuration,uint64 _timestamp) internal returns (
            uint64 endTime,
            uint64 reservedXP,
            uint256 reservedCrystal
        ){
        uint256 _reservedCrystal = duration * crystalPerSecond;
        uint64 _reservedXP = duration * xpPerSecond;
        uint64 _endTime = _timestamp +
            duration -
            ((duration * reduceDuration) / 10000);

        _dungeons[_dungeonId].updatedAt = _timestamp;
        _dungeons[_dungeonId].reservedCrystal += _reservedCrystal;

        return (_endTime,_reservedXP, _reservedCrystal);
    }
    ...
    function _calculatePendingIncompleteRewards(
        Team memory team_,
        uint256 _timestamp
    ) internal pure returns (uint32 xp, uint256 crystal) {
        xp = uint32(
    ...
    function setLastRecoveryTime(address user,uint32 time) external {
        _userInfo[user].lastRecoveryTime = time;
    }
