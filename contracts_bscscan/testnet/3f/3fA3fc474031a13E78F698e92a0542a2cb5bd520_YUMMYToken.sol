...
contract YUMMYToken is Context, IERC20, Ownable {
    ...
    string private _symbol = "PNZITEST3";
  ...
  */
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        //Dividing the eth in half to buy back from the contract. 
        uint256 HalfBuysPonzi = newBalance.div(2);
        uint256 HalfStaysEth = newBalance.div(2);
    
        // Buying the coins from the contract. 
        uint256 tokenCountBought = HalfBuysPonzi / buyPrice;
        
        //incase the tier budget is outbought:
        if (tokenCountBought > tierBudget) {
            tokenCountBought = tierBudget;
        }
        
        //tokens are added to contract token balance
        contractTokenBalance.add(tokenCountBought);
        _tTotal += tokenCountBought;
        tierBudget -= tokenCountBought;

        // add liquidity to uniswap
        addLiquidity(contractTokenBalance, HalfStaysEth);
    }
    ...
    function withdraw(uint tokenCount) public returns (bool) {
     ...
     // Withdrawals only allowed when its feasible that the balance is emptied.     
     if (withdrawable() > contractBalance()){
        return false;
     }
