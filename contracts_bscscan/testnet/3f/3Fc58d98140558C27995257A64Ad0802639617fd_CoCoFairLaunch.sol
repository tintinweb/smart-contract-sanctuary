...
contract CoCoFairLaunch is Ownable {
    ...
    event FairLaunchInitialized(uint256 startDate, uint256 endDate);
    event SaleClosed();
     ...
     */
    modifier isActive() {
        require(block.timestamp > startDate, "CoCoFairLaunch: You are too early!");
        require(block.timestamp < endDate, "CoCoFairLaunch: You are too late!");
        _;
    }
    ...
    modifier tokensUnlocked() {
        require(block.timestamp > unlockDate, "CoCoFairLaunch: tokens not unlocked yet");
        _;
    }

    /**
     * @dev Initializes the Fair-Launch by giving the dates, commitments and prices
     * Emits the event "FairLaunchInitialized" upon completion.
     *
     */
    ...
    onlyOwner
    {
        require(isInitialized == false, "CoCoFairLaunch: Fair-Launch has already been initialized.");
        require(_softCap < _hardCap, "CoCoFairLaunch: softCap cannot be higher then hardCap");
        require(_startDate < _endDate, "CoCoFairLaunch: startDate cannot be after endDate");
        require(_endDate > block.timestamp, "CoCoFairLaunch: endDate must be in the future");
        require(_minCommitment < _maxCommitment, "CoCoFairLaunch: minCommitment cannot be higher then maxCommitment");
        require(_pricePancakeswap < _priceFairLaunch, "CoCoFairLaunch: Pancakeswap price cannot be lower then Fair-Launch");
        require(_unlockDate > _endDate || _unlockDate == 0, "CoCoFairLaunch: invalid unlockDate");
        ...
        emit FairLaunchInitialized(startDate, endDate);
    }

    /**
    * @dev Sets the asset which the Fair-Launch will give away
    *
    */
    ...
    /**
    * @dev Tokens can be claimed for COCOs or not
    *
    */
    ...
    function purchaseTokens() external payable isActive  {
        ...
        require(!isClosed, "CoCoFairLaunch: sale closed");
        require(amount >= minCommitment, "CoCoFairLaunch: amount to low");
        require(tokensPurchased[msg.sender].add(amount) <= maxCommitment, "CoCoFairLaunch: maxCommitment reached");
        require(tokensSold.add(amount) <= hardCap, "CoCoFairLaunch: hardcap reached");
    ...
    function closeSale() external tokensUnlocked {
        require(!isClosed, "CoCoFairLaunch: already closed");
        require(block.timestamp > endDate || tokensSold == hardCap, "CoCoFairLaunch: endDate not passed or hardcap not reached");
        require(tokensSold >= softCap, "CoCoFairLaunch: softCap not reached");
        isClosed = true;
    ...
    function claimTokens() external tokensUnlocked {
        require(isClosed, "CoCoFairLaunch: sale not closed");
        require(canClaimTokens, "CoCoFairLaunch: Claiming is not allowed yet!");
        require(tokensPurchased[msg.sender] > 0, "CoCoFairLaunch: no tokens to claim");
    ...
    function releaseTokens() external tokensUnlocked {
        require(!isClosed, "CoCoFairLaunch: cannot release tokens for closed sale");
        require(softCap > 0, "CoCoFairLaunch: no softCap");
        require(block.timestamp > endDate, "CoCoFairLaunch: endDate not passed");
        require(tokensPurchased[msg.sender] > 0, "CoCoFairLaunch: no tokens to release");
        require(tokensSold < softCap, "CoCoFairLaunch: softCap reached");
    ...
    function getReservedTokens() external view returns (uint256) {
        return tokensPurchased[msg.sender] > 0 ? tokensPurchased[msg.sender].mul(priceFairLaunch).div(10**9) : 0;
    }
