...
contract tradeBNB {
    ...
    constructor(address _aiContract, address _EACAggregatorProxyAddress)
    {
        terminal = msg.sender;
    ...
    function buy(address _referredBy,uint256 USDAmount) payable external returns(uint256)
    {
      ...
      //uint256 USDAmount = BNBToUSD(msg.value);
      ...
      payable(aiContract).transfer(USDToBNB(USDAmount));
      ...
      _AIpollDeposits(msg.sender, USDAmount, totalbus);
    ...
    function withdrawAll() external returns(bool)
    {
      ...
      require(rewardBalanceLedger_[_customerAddress] + totaldaily + TopSponsorGain + TopEarner >= minWithdraw, "beyond withdraw limit");
      ...
      if(pool_last_draw + 1 days < block.timestamp)
      {
        ...
        uint userbalance = USDToBNB(_balance * 95 /100);
        uint adminfee =USDToBNB(_balance * 5 /100);
        ...
        if(ai_pool_top.length > 0 )
        {
          ...
          for(uint8 i = 0; i < ai_pool_top.length; i++) {
              if(ai_pool_top[i] == address(0)) break;
                ...
                uint256 checkedamt= checklimit(ai_pool_top[i], draw_amount);
                if(checkedamt==draw_amount){
                  top5Earners[ai_pool_top[i]] +=draw_amount;
                  userInfo[ai_pool_top[i]].total_payouts += draw_amount;
                }
                else
                {
                  uint remamt = draw_amount - checkedamt;
                  top5Earners[ai_pool_top[i]] += checkedamt;
                  top5Earners[terminal] += remamt;
                  userInfo[ai_pool_top[i]].total_payouts += checkedamt;
                }
