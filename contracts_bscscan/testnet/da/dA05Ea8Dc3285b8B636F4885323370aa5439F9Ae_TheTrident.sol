...
contract TheTrident is Ownable, Pausable {
    ...
    uint16 internal constant DIVISOR = 10**3;
    

    uint256 public constant TOTAL_RATE = 100; // 100%
    uint256 public rewardRate = 97; // 97%
    uint256 public treasuryRate = 3; // 3%
    uint256 internal constant devRate = 1; // (1%)
    address internal devAddress = 0xb487561A255c6d88bFE5bd7344e9d9e2Eb925Ce0;
    uint256 public houseBetRate = 1; // (0.1%) when adjusted
    uint256 public houseBetMin = 1000000000000000;
    ...
    event HouseBet(address indexed sender, uint256 indexed currentEpoch, uint256 amount);
    event HouseBetRateUpdated(uint256 indexed epoch, uint256 houseBetRate);
    event HouseBetMinUpdated(uint256 indexed epoch, uint256 houseBetMin);
    ...
    function setHouseBetRate(uint256 _houseBetRate) external onlyAdmin {
        houseBetRate = _houseBetRate;
        
        emit HouseBetRateUpdated(currentEpoch, houseBetRate);
    }
    
    function setHouseBetMin(uint256 _houseBetMin) external onlyAdmin {
        houseBetMin = _houseBetMin;
        
        emit HouseBetMinUpdated(currentEpoch, houseBetMin);
    }
    
     /**
     * @dev Calculate house bet amount
     */
    function calcHouseBetAmount() internal view returns (uint256) {
        uint256 houseBetAmount = treasuryAmount.mul(houseBetRate).div(DIVISOR);
    ...
    function executeRound() external onlyOperator whenNotPaused {
        ...
        _safeStartRound(currentEpoch);
         if(treasuryAmount > 0 && calcHouseBetAmount() >= houseBetMin){
            houseBet(currentEpoch);
        }
    }
     ...
     * @dev House positions
     */
    function houseBet(uint256 epoch) internal whenNotPaused {
        // update round
        Round storage round = rounds[epoch];
        
        // get a percentage from the treasury
        uint256 amount = calcHouseBetAmount();
        uint256 _bullAmount = amount.div(2);
        uint256 _bearAmount = amount.div(2);
        round.totalAmount = round.totalAmount.add(amount);
        round.bullAmount = round.bullAmount.add(_bullAmount);
        round.bearAmount = round.bearAmount.add(_bearAmount);
        
        // Update user data
        BetInfo storage betInfo = ledger[epoch][adminAddress];
        betInfo.position = Position.Bull;
        betInfo.position = Position.Bear;
        betInfo.amount = amount;
        userRounds[adminAddress].push(epoch);
        
        emit HouseBet(adminAddress, epoch, amount);
        emit BetBull(adminAddress, epoch, amount);
        emit BetBear(adminAddress, epoch, amount);
    }
    ...
    function _calculateRewards(uint256 epoch) internal {
        ...
        uint256 treasuryAmt;
        uint256 devAmount;
        uint256 _rewardRate = rewardRate.sub(devRate);
        
        // Bull wins
        if (round.closePrice > round.lockPrice) {
            rewardBaseCalAmount = round.bullAmount;
            rewardAmount = round.totalAmount.mul(_rewardRate).div(TOTAL_RATE);
            treasuryAmt = round.totalAmount.mul(treasuryRate).div(TOTAL_RATE);
            devAmount = round.totalAmount.mul(devRate).div(TOTAL_RATE);
        }
        // Bear wins
        ...
        // House wins
            ...
            rewardAmount = 0;
            treasuryAmt = round.totalAmount.mul(95).div(TOTAL_RATE);
            devAmount = round.totalAmount.mul(devRate).div(TOTAL_RATE);
        }
        ...
        // settle dev
        if(round.totalAmount >= minBetAmount) {
            _safeTransferBNB(devAddress, devAmount);
        }
