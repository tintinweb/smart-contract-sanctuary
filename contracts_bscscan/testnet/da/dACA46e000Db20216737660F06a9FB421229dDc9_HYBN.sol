...
contract HYBN is Ownable {
    ...
    function _invest(address _addr, uint256 _planId, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        require(hbcToken.balanceOf(_addr)>=tokenAmount,"Low wallet balance");
        require(hbcToken.allowance(_addr,address(this))>=tokenAmount,"Allow token first");
        hbcToken.transferFrom(_addr,address(this),tokenAmount);
