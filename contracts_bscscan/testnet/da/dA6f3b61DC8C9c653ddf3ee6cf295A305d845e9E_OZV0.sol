...
contract OZV0 is OZV0Events, Ownable, ReentrancyGuard {
    ...
    uint256 internal constant dollarDecimals = 10**12; // Decimal Factor for dollar
    ...
    function calculateOZToUSD(uint256 ozt, uint64 tokenDecimal)
        public
        view
        returns (uint256)
    {
    ...
    function sellToken(
        ...
        // Convert OZT To USD
        dollars =
            (ozTokens * currentTokenPrice() * 72 * dollarDecimals) /
            (10**13);
        ...
        if ((soldInDollars + dollars) < maxInvestmentCap) {
            ...
            uint256 r = (soldInDollars + dollars) % maxInvestmentCap;
            for (
                uint256 i = 1;
                i <= (soldInDollars + dollars) / maxInvestmentCap;
                i++
            ) {
                publicSaleInDollars -= maxInvestmentCap;
                soldInDollars = 0;
        ...
        if (investmentCounter == uint256(0)) {
            //direct withdraw
            require(
                (OZT.balanceOf(_msgSender()) >= ozTokens),
                "OZV0 : Insufficient OZ Tokens"
            );
            TransferHelper.safeTransferFrom(
                address(OZT),
            ...
            // input ozt check availabel in contract drip or not
            uint256 drip = _dripwithdrawal(_msgSender(), investmentCounter);
            require((ozTokens <= drip), "OZV0 : Tokens are greater than drip");

            paymentToken = IERC20(tokenAddress);

            // register Token balance in contract
            require(
                (paymentToken.balanceOf(address(this)) >=
                    dollars / dollarDecimals),
                "OZV0 : Insufficient tokens in contract"
            );
            ...
            investment[_msgSender()][investmentCounter]
                .totalTokensWithdrawn += uint128(ozTokens);
            OZT.burn(ozTokens);
        }
    }
