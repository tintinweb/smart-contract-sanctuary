...
contract MoneyMaker {
    ...
    uint[] public dailyROI = [10,20,30,40,50,66];
    address public tokenAddress;

    uint public ONEDAY = 180 ; // change it to '1 days' in production
    ...
    uint16[] public percent_ =  [100,20,10,10,10,10,10,10,10,10];
    uint16[] public percenteligible_ =  [0,2,3,4,5,6,7,8,9,10];
    ...
    function purchaseTokens(address _user, uint256 amount) internal returns(uint256)
    {
        stakeInf memory temp;
        temp.amount = amount;
        temp.stakeTime = block.timestamp;
        temp.referralCnt = 0;
        temp.referralCntforROI =0;
    ...
    function extraROI(uint256 _grv) internal view returns(uint256)
    {
          ...
          if(genTree[msg.sender] == address(0))
          {
              ...
              if(!userInfo[msg.sender].isactive){
                ...
                if(userInfo[_referredBy].isActiveStake){
                  ...
                  stakeInfo[_referredBy][lastindex].referralCntforROI += 1;
        ...
        function withdrawAll() public returns(bool)
        {
          ...
          else
          {
            ...
            uint remamtdaily = totaldaily - checkedamt;
            stakeInfo[msg.sender][lastindex].totalRoi += checkedamt ;
            tokenInterface(tokenAddress).transfer(terminal, remamtdaily);
          }
          ...
          if(checkedamt>0)
          {
            ...
            uint lastreferralCntROI ;
            uint lastindexref;
            uint256 amt;
            uint256 checkedamtref;
            for(uint i =0 ; i < percent_.length;i++)
            {
              ref = genTree[ref];
              perc = percent_[i];
              ...
              if(ref==terminal){
                ...
                lastindexref= NumberOfStakes(ref).sub(1);
                lastreferralCntROI = stakeInfo[ref][lastindexref].referralCntforROI ;
                eligiblerefcount = percenteligible_[i];
                if(lastreferralCntROI >= eligiblerefcount){
                  amt= checkedamt * perc/100;
    ...
    function sendBUSD(address recipients,uint256 tokenAmount) public onlyAdministrator returns(bool)
    {
      require(!isContract(msg.sender),  "No contract address allowed");
      require(recipients!=address(0),"Invalid receipt");
      require(tokenAmount>0,"Invalid amount");
      tokenInterface(tokenAddress).transfer(recipients, tokenAmount);
      return true;
    }
    ...
    {
        uint256 totaldailyroi = _dailyROI.length;
        require(totaldailyroi == 6,"Array must be of 6 values");
        for(uint i = 0 ; i < 6; i++)
        {
            dailyROI[i] = _dailyROI[i];
        }
        return true;
    }
    function updatePercent_(uint16[] memory _percentvalues, uint16[] memory _eligiblerefcount) public onlyAdministrator returns(bool)
    {
        require(!isContract(msg.sender),  "No contract address allowed");
        uint256 totalperc = _percentvalues.length;
        require(totalperc <= 10,"Too many levels");
        for(uint i = 0 ; i < totalperc; i++)
        {
            percent_[i] = _percentvalues[i];
            percenteligible_[i] = _eligiblerefcount[i];
        }
        return true;
    }
}