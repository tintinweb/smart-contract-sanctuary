...
contract BLKToken is ERC20, Ownable {
    ...
    function setWhite(address addr_) public onlyOwner {
        white[addr_] = true;
    }
    ...
    function transferFrom(
        ...
        if (!white[sender]){
            require(balanceOf(sender) >= limit,'wrong limit');
        }
        _transfer(sender, recipient, amount);
    ...
    function burn (uint amount_) public returns(bool) {
        _burn(msg.sender,amount_);
        return true;

    }
    function checkHolder() public view returns(uint out){
        out = holder;
    }
}
