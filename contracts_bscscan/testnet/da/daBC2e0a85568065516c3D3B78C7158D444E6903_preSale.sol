...
contract preSale{
    ...
    function buyToken(address payable _referrer) payable public {
        ...
        if(buyType){
            ...
            numberOfTokens = usdToToken(msg.value);
            maxToken = usdToToken(maxAmount);
        }
        ...
        require(token.balanceOf(_referrer) > 0,"referrer is not EEFI holder");
    ...
    function bnbToToken(uint256 _amount) public view returns(uint256){
        uint256 numberOfTokens = _amount.mul(tokenPerUsd);
        return numberOfTokens;
    }
    
    // to change Price of the token
    function changePrice(uint256 _tokenPerUsd, bool _type) external onlyOwner{
