...
contract DevSalary {
    ...
    function addDeveloper(address _newDev, uint256 _monthlyAllowance) public onlyOwner {
        require(!msnDevelopers[_newDev].isDeveloper, "Error: Team member is already added");
        msnDevelopers[_newDev].isDeveloper = true;
        msnDevelopers[_newDev].monthlyAllowance = _monthlyAllowance;
        emit DeveloperAdded(_newDev, _monthlyAllowance);
    }

    function deleteDeveloper(address _address) public onlyOwner {
    ...
    function _returnMonthsSinceTGE() internal view returns (uint256) {
        ...
        uint256 monthSince = unixTimeSince/60; //number of minutes since
        return monthSince;
    }

    function claim() public {
        uint256 monthSinceTGE = _returnMonthsSinceTGE();
        uint256 canClaim = 1 + monthSinceTGE - msnDevelopers[msg.sender].txCount;
        if (canClaim > 0) {
    ...
    function emergencyWithdraw() public onlyOwner {
        _msnToken().safeTransfer(owner, _msnToken().balanceOf(address(this)));
    }
