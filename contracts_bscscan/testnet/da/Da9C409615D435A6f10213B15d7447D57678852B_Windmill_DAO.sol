...
contract Windmill_DAO is Ownable {
     ...
     * params -> parameters associated with the proposal (max 1 parameters)
     ...
     */
    function submitProposal(uint16 id, uint256[3] memory params) external{
    ...
    function applyProposal(Proposal storage proposal) internal{
        if (proposal.id == 0){
            nbProposalPerUserPerCycle = proposal.params[0];
        }
    }
}