...
contract lts is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    address public constant burnAddress = 0x000000000000000000000000000000000000dEaD; // DEAD ADDRESS
    ...
    uint256 public _maxTxAmount = 90000000 * 10**6 * 10**18;
    ...
    constructor() {
        ...
        totalBurns = 0; // REMAINING MANUAL BURNS
    ...
    function calculateBurnFee(uint256 _amount, bool _heldLessThan7Hours) private view returns (uint256) {
            if(_heldLessThan7Hours) {
                return _amount.mul(_burnFee + 3).div(10 ** 2); 
                } else {
    ...
    function calculateBuybackFee(uint256 _amount, bool _heldLessThan7Hours) private view returns (uint256) {
            if(_heldLessThan7Hours) {
                return _amount.mul(_buyBackFee + 2).div(10 ** 2); 
                } else {
                return _amount.mul(_buyBackFee).div(10 ** 2); 
                }
        }

         // SEND BURNED TOKENS TO DEAD WALLET AND SUBSTRACT THEM FROM TOTAL SUPPLY IN EACH TRANSACTION
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        bool takeFee = _feesStatus;
            // IF SENDER EXCLUDED REMOVE ALL FEES
        if (_isExcluded[from]) {
            takeFee = false;
        }
        ...
        //THIS METHOD IS RESPOSIBLE FOR TAKING ALL THE FEE IF TRUE
