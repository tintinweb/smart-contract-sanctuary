...
contract With_DAORole is I_With_DAORole{
 ...
 * markets can exist.
 */
interface I_Windmill_Power is IERC20, IAccessControlEnumerable, I_With_DAORole{
    /**
...
interface I_With_PWRToken is I_With_DAORole{
...
contract With_PWRToken is I_With_PWRToken, With_DAORole, AccessControlEnumerable{
 ...
 */
interface I_Windmill_Royalty is I_With_PWRToken, IERC20, IAccessControlEnumerable {
    function percentPWRRoyaltyMintNumerator() external view returns (uint256);
    function percentPWRRoyaltyMintDenominator() external view returns (uint256);
...
contract Windmill_Royalty is ERC20, AccessControlEnumerable, With_PWRToken, I_Windmill_Royalty {
    address payable immutable internal thisAddr;

    mapping(address=>uint256) public holderId;
    address[] public holders;

    uint256 public percentPWRRoyaltyMintNumerator;
    uint256 public percentPWRRoyaltyMintDenominator;


    constructor() ERC20("Windmill_Royalties", "ROY") {
        _setRoleAdmin(DAO_ROLE, DAO_ROLE);
        _setupRole(DAO_ROLE, msg.sender);
        thisAddr = payable(address(this));
        holders.push(address(0x0));
        percentPWRRoyaltyMintNumerator = 100;
        percentPWRRoyaltyMintDenominator = 10000;
    }
    ...
    function mintTo(address to, uint256 amount) external onlyRole(DAO_ROLE){
        require(totalSupply() + amount <= 100);
        _mint(to, amount);
    }

    function setRoyaltyRatio(uint256 numerator, uint256 denominator) external onlyRole(DAO_ROLE){
        require(denominator>0);
        percentPWRRoyaltyMintNumerator = numerator;
        percentPWRRoyaltyMintDenominator = denominator;
    }

    function _afterTokenTransfer(address from, address to, uint256 amount) internal override {
        if (holderId[to] == 0){
            holderId[to] = holders.length;
            holders.push(to);
        }

        if (from != address(0x0) && balanceOf(from) == 0){
            uint256 i = holderId[from];
            uint256 lastI = holders.length-1;
            if (i != lastI){
                address lastAddr = holders[lastI];
                holders[i] = lastAddr;
                holderId[lastAddr] = i;
            }
            holders.pop();
    ...
    function processRoyalties() external onlyRole(DAO_ROLE){
        uint256 ROYTotal = totalSupply();
        uint256 PWRTotal = PWRToken.totalSupply();
        for (uint256 i=1; i<holders.length; i++){
            address addr = holders[i];
            uint256 ROYBalance = balanceOf(addr);
            uint256 PWRToMint = (PWRTotal * percentPWRRoyaltyMintNumerator * ROYBalance) / (percentPWRRoyaltyMintDenominator * ROYTotal);
            PWRToken.mintTo(addr, PWRToMint);
        }
    }

    function migrateTo(I_Windmill_Royalty newRoyalty) external onlyRole(DAO_ROLE){
        for (uint256 i=1; i<holders.length; i++){
            address addr = holders[i];
            uint256 ROYBalance = balanceOf(addr);
            newRoyalty.mintTo(addr, ROYBalance);
        }
    }
