...
contract LiquidityAdder is Auth {
	...
	uint256 slippage = 0;
    uint256 slippageDenom = 1000;
    bool refund = true;
    
    event Refund(address indexed origin, uint value, bool indexed sent);
	...
	function setSlippage(uint _slippage, uint _denom) external authorized {
		slippage = _slippage;
		slippageDenom = _denom;
	}
	

	function setRefund(bool _refund) external authorized {
	    refund = _refund;
	}
	...
	function addLiquidity(uint256 bnb, uint256 tokens) internal {
		router.addLiquidityETH{value: bnb}(
			token,
			tokens,
			tokens * slippage / slippageDenom,
			bnb * slippage / slippageDenom,
			msg.sender,
			block.timestamp
		);
	}
	...
	receive() external payable {
        if (msg.value > 0 && refund) {
            (bool sent,) = payable(tx.origin).call{value: msg.value}("");
            emit Refund(tx.origin, msg.value, sent);
        }
    }
}