...
/**
METASANTA (New and Improved)

Old: 0xDadB0d51ce2bb41950e8ecBBfa4E80F853e5A3Ef
New: 0xDADb0DB3652fdb222A9ECE4Bf0273Bcc8D069c31

                   __________________
                  |____(Launch 1.0)_|
                  | | | | | | |/ .-.\   HANG IN THERE
                  |_|_|_|_|_|_/ /   `.      METASANTA FANS
                   |_|__|__|_; |      \          2.0 IS HERE
                   |___|__|_/| |     .'`}
                   |_|__|__/ | |   .'.'`\
                   |__|__|/  ; ;  / /    \.-"-.
                   ||__|_;   \ \  ||    /`___. \
...
contract MetaSanta is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MetaSantaDividendTracker public dividendTracker;
    ...
    constructor() ERC20("MetaSanta", "SLEIGH") {
        marketingWallet = payable(0x47CCa2240A62e2A8D8173Ce55Ebf2e71227E579a);
        ...
        dividendTracker = new MetaSantaDividendTracker(
            payable(this),
            router,
        ...
        maxSellTransactionAmount = totalTokenSupply / 400; // 0.25%
    ...
    function updateMaxAmount(uint256 newNum) public onlyOwner {
        require(newNum == 0 || newNum >= totalSupply() / 1000); // 0.1%
    ...
    function updateFees(
        ...
        require(
            liquidityBuy <= 8 && liquiditySell <= 8,
            "Liquidity fees can't exceed 8%"
        );
        require(deadBuy <= 8 && deadSell <= 8, "Burn fees can't exceed 8%");
        require(
            BNBRewardsBuy <= 25 && BNBRewardsSell <= 25,
            "Reward fees cannot exceed 25%"
        );
        require(
            marketingBuy <= 4 && marketingSell <= 4,
            "Marketing fees can't exceed 4%"
        );
        buyDeadFees = deadBuy;
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividendsFromBuy = address(this)
            ...
            .div(buyBNBRewardsFee.add(buyDeadFees + buyMarketingFees));
        ...
        uint256 dividendsFromSell = address(this)
            ...
            .div(sellBNBRewardsFee.add(sellDeadFees + sellMarketingFees));
...
contract MetaSantaDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    MetaSanta public MetaSantaContract;
    ...
    constructor(
        ...
        defaultToken = token;
        MetaSantaContract = MetaSanta(mainContract);
        minimumTokenBalanceForAutoDividends = 75_000_000000000000000000; // 100,000, 0,01% of supply
    ...
    function _transfer(
        ...
        require(false, "MetaSanta_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "MetaSanta_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MetaSanta contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(MetaSantaContract);

            uint256 prevBalance = MetaSantaContract.balanceOf(address(this));
            ...
            {
                uint256 received = MetaSantaContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MetaSantaContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
