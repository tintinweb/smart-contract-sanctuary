...
contract MPStaking is Ownable, ReentrancyGuard {
    ...
    uint256 public noOfBlocksPerDay = 20;
        ...
        returns (uint256[] memory milestones)
    {
        ...
        uint256 _gapBlocks = _noOfBlocksPerDay.mul(31);   // gap between milestone[0] & milestone[1], 31 days

        milestones[0] = _rewardStartBlock;
        milestones[1] = _rewardStartBlock.add(_gapBlocks);
        ...
        if (user.status[0] == false) {
            ...
            if (user.status[1] == false) {
                if (block.number > user.milestones[1]) {
                    ...
                    gapReward = user.amounts[0].mul(apy.mul(1e12).div(1e12)).mul(30).div(noOfDaysYear).div(100);
                }
            }
        }
