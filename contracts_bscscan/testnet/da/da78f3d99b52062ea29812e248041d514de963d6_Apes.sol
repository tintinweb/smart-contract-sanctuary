...
contract Apes is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  function setup() public onlyOwner {
        ...
        growPrice= 10;
        evolvePrice= 10000; 
        aGoldAddr= address(0x5B5214c6110a2B538C2541998b243ec1CC517E24);
        wAGoldAddr= address(0x5B5214c6110a2B538C2541998b243ec1CC517E24);
  ...
  function getTypeIdByNftId(uint256 tokenId) public view returns(uint256) {
    ...
    return uint(current.level);
  }

  function setApeUriByNftId(uint256 nftID, string memory jsonURL) public onlyOwner {
    _setTokenURI(nftID-1, jsonURL);
  }
  function setApesLandAddress(address add) public onlyOwner {
    apesLandAddr = add;
  }
  function setLevel(uint256 nftID, uint lvl) public {
    IApesLandV2 apesLand = IApesLandV2(apesLandAddr);
    require(address(msg.sender) == this.owner() || 
            msg.sender == address(apesLand), "Owner required");
    apes[nftID - 1].level = Level(lvl);
  }
  function getLevel(uint256 nftID) public view returns(uint) {
    return uint(apes[nftID - 1].level);
  }
  function setDeltaStack(uint256 nftID, uint delta) public {
    IApesLandV2 apesLand = IApesLandV2(apesLandAddr);
    require(address(msg.sender) == this.owner() || 
            msg.sender == address(apesLand), "Owner required");
