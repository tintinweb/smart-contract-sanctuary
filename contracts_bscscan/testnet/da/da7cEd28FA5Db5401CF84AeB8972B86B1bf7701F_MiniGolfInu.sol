...
//
// $MiniGolfInu Baby contract, thanks to Zooshi for tax functions.
//
// DIVIDEND YIELD PAID IN SUSHI! With the auto-claim feature,
// simply hold $MiniGolfInu and you'll receive SUSHI automatically in your wallet.
// 
// Hold MiniGolfInu and get rewarded in Sushi on every transaction!
//
// 📱 Telegram: https://t.me/MiniGolfInuCoin
// 🌎 Website: https://MiniGolfInu.io
// 🌐 Twitter: https://twitter.com/MiniGolfInucoin
...
import "./MiniGolfInuTransferHelper.sol";

contract MiniGolfInu is ERC20, Ownable {
	...
	MiniGolfInuDividendTracker public dividendTracker;
	MiniGolfInuTransferHelper private transferHelper;
	...
	constructor(address routerAddress, address payable marketingWalletAddress) public ERC20("MiniGolfInu", "MiniGolfInuTest") {
		dividendTracker = new MiniGolfInuDividendTracker();
		transferHelper = new MiniGolfInuTransferHelper(routerAddress);
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "MiniGolfInu: The dividend tracker already has that address");

		MiniGolfInuDividendTracker newDividendTracker = MiniGolfInuDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "MiniGolfInu: The new dividend tracker must be owned by the MiniGolfInu token contract");
	...
	function updateUniswapV2Router(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "MiniGolfInu: The router already has that address");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "MiniGolfInu: Account is already the value of 'excluded'");
	...
	function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
		require(pair != uniswapV2Pair, "MiniGolfInu: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
	...
	function setMaxWalletAmount(uint256 amount) public onlyOwner {
		require(amount <= totalSupply(), "MiniGolfInu: Amount cannot be over the total supply.");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "MiniGolfInu: Automated market maker pair is already set to that value");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "MiniGolfInu: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "MiniGolfInu: Cannot update gasForProcessing to same value");
...
contract MiniGolfInuDividendTracker is Ownable, DividendPayingToken {
	using SafeMath for uint256;
	...
	constructor() public DividendPayingToken("MiniGolfInu_Dividend_Tracker", "MiniGolfInu_Dividend_Tracker") {

	}

	function _transfer(address, address, uint256) internal override {
		require(false, "MiniGolfInu_Dividend_Tracker: No transfers allowed");
	}

	function withdrawDividend() public override {
		require(false, "MiniGolfInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MiniGolfInu contract.");
	}
	...
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "MiniGolfInu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "MiniGolfInu_Dividend_Tracker: Cannot update claimWait to same value");
