...
contract PokMonsterFactory is IPokMonsterFactory, OwnableUpgradeable, ReentrancyGuard {
    ...
    function totalBurned() external view override returns (uint256) {
        return IPokMonsterNFT(nft_).totalBurned();
    }
    ...
    function mintToken(
        ...
        IPokMonsterStats(stats_).setBody(tokenId,  body);
        ++mintedCount_;
    ...
    function mintMultiTokens(
        ...
        IPokMonsterStats.PokBody[] memory bodies
    ) external override onlyAdmin {
        uint256 length = tokenIds.length;
        require(length >= 1, "zero token");
        require(length == bodies.length, "Wrong array length");
        for (uint256 i = 0; i < length; i++) {
            mintToken(receiver, tokenIds[i],bodies[i]);
        }
    }

    function burnToken(uint256 tokenId) external override nonReentrant {
        IPokMonsterNFT _nft = IPokMonsterNFT(nft_);
        require(_nft.ownerOf(tokenId) == msg.sender, "PokMonsterFactory: burn token that is not own");
        _nft.burn(tokenId);
