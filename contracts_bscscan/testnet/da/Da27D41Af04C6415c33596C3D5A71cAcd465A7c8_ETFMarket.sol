...
contract ETFMarket is Ownable, Pausable {
    ...
    uint256 public sellFee = 25; // 0.2%
    uint256 public buyFee = 15; // 0.2%
    ...
    uint256 first = 1;
    uint256 last = 0;
    uint256 public pricePeriod = uint256(1 hours);
    ...
    function orderBuy(
        ...
        uint256 paid = _wantPrice.mul(_amount);
    ...
    function buy(uint256 _orderId, uint256 _amount) external whenNotPaused checkSellOrder(_orderId) {
        Order storage order = sellOrderMap[_orderId];
    ...
    function sell(uint256 _orderId, uint256 _amount) external whenNotPaused checkBuyOrder(_orderId) {
        Order storage order = buyOrderMap[_orderId];
        if (_amount > order.amount.sub(order.filled)) _amount = order.amount.sub(order.filled);
        etf.safeTransferFrom(msg.sender, address(this), _amount);

        // Transfer payment to seller
        uint256 paid = _amount.mul(order.price);
        uint256 sellFeeAmount = paid.mul(sellFee).div(MAX_FEE);
        if (sellFeeAmount > 0) {
            paid -= sellFeeAmount;
            currency.safeTransfer(feeRecipient, sellFeeAmount);
    ...
    function updateBuyOrder(uint256 _orderId, uint _price) external checkOwner(_orderId, true) {
        Order storage order = buyOrderMap[_orderId];
        require(order.locked == false, "in process");
        order.locked = true;
        order.price = _price;
    ...
    function marketPrice(uint _count, uint _period) external view returns (uint avgPrice, uint sumCount) {
        require(_count > 0 || _period > 0, "!parameter");

        uint basePrice = oraclePrice();
        if (last < first) return (basePrice, 0);
        ...
        if (_count == 0 && _period > 0) {
            for (uint i = last; i >= first; i--) {
                ...
                if (price.price > basePrice || price.price < basePrice) priceSum += basePrice;
            ...
            for (uint i = last; i >= first && sumCount <= _count; i--) {
                Price storage price = priceQueue[i];
                if (price.timestamp < block.timestamp.sub(_period)) break;
                if (price.price > basePrice.mul(MAX_FEE+priceMargin).div(MAX_FEE)) {
                    priceSum += basePrice.mul(MAX_FEE+priceMargin).div(MAX_FEE);
                } else if (price.price < basePrice.mul(MAX_FEE-priceMargin).div(MAX_FEE)) {
                    priceSum += basePrice.mul(MAX_FEE-priceMargin).div(MAX_FEE);
                } else {
                    priceSum += price.price;
                }
        ...
        if (sumCount == 0) return (basePrice, sumCount);

        return (priceSum.div(sumCount), sumCount);
    }
    ...
    function setPricePeriod(uint256 _period) external onlyOwner {
        require(_period > uint256(1 minutes), "!period");
        pricePeriod = _period;
    }
