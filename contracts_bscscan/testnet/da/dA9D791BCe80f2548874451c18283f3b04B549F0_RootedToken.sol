...
contract RootedToken is LiquidityLockedERC20("Hobbs Network Token", "HNW")
{
    ...
    {   
        ...
        if(!limitInAffect){
            ...
            {                        
                ...
                if(recipient != getPairAddress() || recipient != getElitePairAddress()){
                    uint256 totalAmount = transfersReceived[recipient];
                    require(totalAmount <= buyLimit, "Total Amount already received until time limit over.");
                }
    ...
    function getElitePairAddress() public view returns (address)
    {
        return factory.getPair(address(elite), address(baseToken));
    }
}