...
import "./MFCExchange.sol";

contract MFCBuyback is BackendAgent {
  ...
  function buybackOffer(uint256 offerId, address seller) external onlyBackendAgents {
    ...
    if (buybackAmount > MINIMUM_SWAP_IN_BUSD) {
    ...
    if (mfcBalance > 0) {
      _mfc.transfer(_mfcTreasuryAddress, mfcBalance);
    }
  }
}
...
contract MFCExchange is Context {
  ...
  constructor(address mfcAddress_, address busdAddress_, address mfcTreasuryAddress_, address busdComptrollerAddress_) {
    _busdAddress = busdAddress_;
    _tradingPairs[TradingPairs.MFC_BUSD] = TradingPair(mfcAddress_, busdAddress_, mfcTreasuryAddress_, busdComptrollerAddress_, MFC_FEE, BUSD_FEE);
    _tradingPairs[TradingPairs.BUSD_MFC] = TradingPair(busdAddress_, mfcAddress_, busdComptrollerAddress_, mfcTreasuryAddress_, BUSD_FEE, MFC_FEE);
  }
  ...
  function createOffer(TradingPairs tradingPair, uint256 quantity, uint256 price) external onlyValidCreateOffer(tradingPair, quantity, price) {
    _createOffer(tradingPair, quantity, price);
  }

  function createOffer(TradingPairs tradingPair, uint256 quantity, uint256 price, uint8 v, bytes32 r, bytes32 s) external onlyValidCreateOffer(tradingPair, quantity, price) {
  ...
  function tradeOffer(uint256 id, address seller, uint256 quantity) external onlyValidTradeOffer(id, seller, quantity) {
    _tradeOffer(id, seller, quantity);
  }

  function tradeOffer(uint256 id, address seller, uint256 quantity, uint8 v, bytes32 r, bytes32 s) external  onlyValidTradeOffer(id, seller, quantity) {
