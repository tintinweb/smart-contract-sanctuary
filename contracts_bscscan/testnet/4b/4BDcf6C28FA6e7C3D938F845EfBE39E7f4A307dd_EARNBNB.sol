...
contract EARNBNB is Context, IBEP20, Ownable { // change contract name
    ...
    address payable public _marketingAddress = payable(0x9D49EcEeAb6fb30bAE3027F7b5B9E768BE62da4f); // Marketing Address
    ...
    string private _name = "EARNBNB"; // token name
    string private _symbol = "EARNBNB"; // token symbol
    ...
    uint256 public _burnFee = 0; // burn fee 1%
    uint256 private _previousBurnFee = _burnFee; // burn fee
 
    uint256 public _reflectionFee = 0; //reflection fee 4% actual value set from transfer function
    uint256 private _previousReflectionFee = _reflectionFee; //reflection fee
 
    uint256 public _marketingFee = 110; // marketing fee 5% actual value set from transfer function
    uint256 private _previousMarketingFee = _marketingFee; // marketing fee
 
    uint256 public _liquidityFee = 40; // actual liquidity fee 5% actual value set from transfer function
    ...
    uint256 public _maxTxAmount = 50000 * 10**6 * 10**18; // max allowed tokens tranfer per transaction
    uint256 public _numTokensSellToAndTransfer = 500 * 10**6 * 10**18; // min token liquidity fee collected before swapandLiquify
    uint256 public _maxTokensPerAddress = 50000 * 10**6 * 10**18; // Max number of tokens that an address can hold
    ...
    constructor () {
		...
		setFirstMonth(30,0,120,0);
		setSecondMonth(30,0,120,0);
		setThirdMonth(30,0,120,0);
		setFourthMonth(30,0,120,0);
    ...
    /**  
     * @dev sets bnb reward address
     */
    function setBNBRewardAddress(address payable rewardAddress) external onlyOwner() {
        _marketingAddress = rewardAddress;
    }
    ...
    /**  
     * @dev Anti sniper bot: block single wallet
     */
    function blockSniperBot(address addresses) public onlyOwner(){
        if(_isBlacklisted[addresses] == true) return;
        _isBlacklisted[addresses] = true;
    }
 
    /**  
     * @dev Anti sniper bot: block multiple wallets
     */
    function blockMultipleSniperBots(address[] calldata addresses) public onlyOwner(){
    ...
    /**  
     * @dev Anti sniper bot: returns if is blocked
     */
    function isSniperBotBlocked(address addresses) public view returns (bool){
    ...
    /**  
     * @dev Anti sniper bot: unblock single wallet
     */
    function unblockSniperBot(address addresses) external onlyOwner(){
         if(_isBlacklisted[addresses] == false) return;
        _isBlacklisted[addresses] = false;
    }
 
    /**  
     * @dev Anti sniper bot: unblock multiple wallets
     */
    function unblockMultipleSniperBots(address[] calldata addresses) public onlyOwner(){
        for (uint256 i; i < addresses.length; ++i) {
