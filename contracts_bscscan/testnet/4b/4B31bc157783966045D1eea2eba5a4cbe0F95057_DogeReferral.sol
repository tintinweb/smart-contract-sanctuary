...
contract DogeReferral is Ownable {
    ...
    struct Account {
        address referrer;
        uint referredCount;
        mapping (address => bool) activeReferrals;
    ...
    function addReferrer(address referrer) external {
        ...
        uint256 balance = IBEP20(_token).balanceOf(msg.sender);
        ...
        parentAccount.activeReferrals[msg.sender] = true;
     ...
     */
    function updateReferralStatus(address referee) external onlyToken {
        uint256 balance = IBEP20(_token).balanceOf(address(referee));
        ...
        bool isRefereeActive = parentAccount.activeReferrals[referee];

        if (isRefereeActive && balance < MIN_TOKEN_THRESHOLD) {
            parentAccount.activeReferrals[referee] = false;
            parentAccount.referredCount.sub(1);
        } else if (!isRefereeActive && balance > MIN_TOKEN_THRESHOLD) {
            parentAccount.activeReferrals[referee] = true;
            parentAccount.referredCount.add(1);
        }
    }
