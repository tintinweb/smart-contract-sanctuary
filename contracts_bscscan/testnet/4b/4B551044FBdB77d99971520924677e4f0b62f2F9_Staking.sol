...
/** @title Staking
 * @notice It is a contract for Staking PlayToken
 */
...
contract Staking is Ownable, ReentrancyGuard, IStaking {
    ...
    function withdraw(
    ...
    ) external validatePoolByPid(pid) nonReentrant {
        PoolInfo storage pool = poolInfo[pid];
        UserInfo storage user = userInfo[pid][msg.sender];
        ...
        if (pending > 0) {
            ...
            if (_withdrawRewards) {
                ...
                emit Claim(msg.sender, pid, user.pendingRewards);
     ...
     * @param amount: Amount of token to deposit
     */
    function emergencyWithdraw(uint256 pid, uint256 amount) external validatePoolByPid(pid) nonReentrant {
        PoolInfo storage pool = poolInfo[pid];
        UserInfo storage user = userInfo[pid][msg.sender];
     ...
     * @param pid: Pool Id
     */
    function claim(uint256 pid) external validatePoolByPid(pid) nonReentrant {
        PoolInfo storage pool = poolInfo[pid];
        UserInfo storage user = userInfo[pid][msg.sender];
        updatePool(pid);
        uint256 pending = (user.amount * (pool.accTokenPerShare)) / (1e12) - (user.rewardDebt);
