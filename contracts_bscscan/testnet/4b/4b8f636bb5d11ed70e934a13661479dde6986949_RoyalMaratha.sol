...
contract RoyalMaratha is BEP20, BEP20Burnable, Pausable, Ownable {

    bool public airdropEnabled; 

    mapping(address => bool) public blacklist;
    mapping(address => uint256) public lockIn;
    uint256 public unlockBlock;
    uint256 public maxSupply;

    constructor(uint256 _unlockBlock) BEP20("Royal Maratha", "MARATHA") {
        airdropEnabled=true;
        ...
        override
    {
        ...
        if(block.number < unlockBlock){ 
            if(from != address(0)){
                require(lockIn[from] <= (balanceOf(from) - amount) ,"Airdrop LockIn");
            }
    ...
    {
        require(airdropEnabled == true,"Airdrop Disabled By Owner");
        uint256 count = users.length;
        for(uint256 i=0; i< count; i++)
        {
            _transfer(address(this), users[i], amounts[i] * (10**decimals()));
            lockIn[users[i]] = lockIn[users[i]] + (amounts[i] * (10**decimals()));
        }
    }
