...
contract ObjectUtil {
    ...
    function getLastXYParam(uint256 param_) public view returns (uint256[] memory, uint256, uint256, uint256){
        return (arr1, arr1[0], arr1[1], arr1[2]);
    }

    function setLastXYParam(uint256 param_, uint256[] memory param_2) public {
        if (param_ == 0) arr0 = param_2;
        if (param_ == 1) arr1 = param_2;
    }
...
interface ObjectPlot {
    function getLastXYParam(uint256) external view returns (uint256[] memory, uint256, uint256);
}