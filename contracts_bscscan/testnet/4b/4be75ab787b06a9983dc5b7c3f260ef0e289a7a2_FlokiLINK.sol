...
contract FlokiLINK is ERC20, Ownable {
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        ...
        totalFees = dividendRewardsFee.add(treasuryFee).add(liquidityFee);
    }
    ...
    function updateLPFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "FlokiLINK: Dividend LP must be between 0 and 10");
        liquidityFee = newFee;
        totalFees = dividendRewardsFee.add(treasuryFee).add(liquidityFee);
    }
    ...
    bool isSellFactorEnabled = false;
    
    function setisSellFactorEnabled(bool _isSellFactorEnabled) external onlyOwner{
        isSellFactorEnabled = _isSellFactorEnabled;
    }
    ...
    function _transfer(
        ...
        bool hasExcluded = _isExcludedFromFees[from] || _isExcludedFromFees[to];
        ...
        if(takeFee) {
            ...
            if(isSellFactorEnabled && automatedMarketMakerPairs[to]) {
