...
contract EpicMarket is AccessControl {
    ...
    event BuyItem(uint256 marketId, uint256 tokenId, uint256 price, address seller, address buyer);
    ...
    event MakeOffer(uint256 marketId, uint256 price, bool status);
    ...
    function buyItem(uint256 marketId) public onlyExistItem(marketId) {
        ...
        ERC20.transferFrom(msg.sender, itemData._owner, itemData._amount - fee);
        ...
        emit BuyItem(marketId, itemData._tokenId, itemData._price, itemData._owner, msg.sender);
    }
    function makeOffer(uint256 marketId, uint256 price, uint256 expiration) public onlyExistItem(marketId){
        ...
        (bool found, Item memory itemData) = _getItemInfo(marketId);
        require(found, "Item is not found");
        require(itemData._buyer == address(0), "Item is already sold");
        offers[marketId].push(
            Offer(
                msg.sender,
                price,
                marketId,
                expiration,
                offers[marketId].length,
                false,
                true
            )
        );
        emit MakeOffer(marketId, price, true);
    }
    function getItems() public view returns(Item[] memory){
        return items;
    }
    function getOfferLists(uint256 marketId) public view onlyExistItem(marketId) returns (Offer[] memory){
        return offers[marketId];
    }
    function getOfferById(uint256 marketId, uint256 offerId) public view returns(bool status, Offer memory){
        for(uint256 i = 0; i < offers[marketId].length; i++){
            if(i == offerId) return  (true, offers[marketId][i]);
        }
        return (false, Offer(address(0), 0, 0, 0, 0, false, false));
    }
    function acceptOffer(uint256 marketId, uint256 offerId) public onlyExistItem(marketId){
        (, Item memory itemData) = _getItemInfo(marketId);
        require(itemData._buyer == address(0), "Item is already sold");
        
        (bool status, Offer memory offerData) = getOfferById(marketId, offerId);
        ...
        ERC20.transferFrom(offerData._buyer, itemData._owner, itemData._amount - fee);
        ERC20.transferFrom(offerData._buyer, _wallet, fee);

        if(itemData._itemType == TokenType.ERC1155){
            IERC1155(itemData._item).safeTransferFrom(itemData._owner, offerData._buyer, itemData._tokenId, itemData._amount, "");
        ...
        offers[marketId][offerId]._isAccept = true;
        offers[marketId][offerId]._active = false;
    }
    function closeOffer(uint256 marketId, uint256 offerId) public onlyExistItem(marketId){
        require(offerId < offers[marketId].length, "Invalid offerId");
