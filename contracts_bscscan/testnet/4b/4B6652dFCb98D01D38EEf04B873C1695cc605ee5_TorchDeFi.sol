...
// -------------------------------------------User methods-----------------------------------------------------
    ...
    // user stakes Torch in the first 3 of 4 pools
...
contract TorchDeFi is LpVault{
    ...
    function claimStatic() public updateReward(msg.sender) notBlack{
        ...
        uint userActLev = getActLevel(_msgSender());
    ...
    function setActCond(uint[3] calldata _actCond) external onlyOwner{
        for(uint i=0;i<3;i++){
            actTokenCond[i] = _actCond[i];
        }
    }
...
//------------------------------------------AirDrop--------------------------------------------------
    ...
    mapping(uint=>mapping(uint=>uint)) public airdropNodeReg; // drop tiems => (node level => number of people)
    ...
    uint[6] public levelPortionAirdrop = [0,4,6,10,16,20];
    ...
    */
    ...
    function stakeAirdrop(uint amount) external{
        IERC20(Torch).transferFrom(_msgSender(), address(this), amount);
        userStakeAirdrop[_msgSender()] += amount;
    }
    
    function unstakeAirdrop()external{
        // not airdrop on or user has not registered current airdrop
        require(!isAirdrop || !userAirdrop[_msgSender()][airdropTimes].registered);
        IERC20(Torch).transfer(_msgSender(), userStakeAirdrop[_msgSender()]);
    ...
    */
    function registerAirdrop() external{
        ...
        require(userStakeAirdrop[_msgSender()] * 10000 >= dropInfo[airdropTimes].req * userInfo[_msgSender()].LPinPool, "airdrop stake requirement");
        
        userAirdrop[_msgSender()][airdropTimes].registered = true;
        userAirdrop[_msgSender()][airdropTimes].registeredAmount = userInfo[_msgSender()].LPinPool;
        dropInfo[airdropTimes].weightTotal += userInfo[_msgSender()].LPinPool;
        ...
        // iterate over levels
        for(uint i=1; i<=lev; i++){
            airdropNodeReg[airdropTimes][i] += 1;
        }
    ...
    function claimAirdrop() external{
        ...
        for(uint i=0;i<3;i++){
            if(temp_user == address(this)){break;}
                ...
                userAirdrop[temp_user][airdropTimes].rewards_d += shares * DirectAirdropDynamic[i] / 100;
                temp_user = userAccount[temp_user].invitor;
            }
        }
        
        uint temp_amount;
        uint node = dropInfo[airdropTimes]._amount * uint(4).div(100); // 4% each node
        
        //require(getActLevel(_msgSender())==3, "should comsume enough activative tokens");
        uint lev = getLevel(_msgSender());
        require(lev > 0, "no level");
        for(uint i=1; i<=lev;i++){
            temp_amount += node.div(airdropNodeReg[airdropTimes][i]);
        }
