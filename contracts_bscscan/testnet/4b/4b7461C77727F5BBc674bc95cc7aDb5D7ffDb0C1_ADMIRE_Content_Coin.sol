...
contract MultiSig is Context {
    ...
    // @dev Initializes the contract with MultiSig _mint_to_stopped state control off.
    // @dev Initializes the contract with MultiSig _change_operator_stopped state control off.
    // @dev Initializes the contract with MultiSig _taxrate_change_stopped state control off.
    // @dev Initializes the contract with MultiSig _charityrate_change_stopped state control off.
    ...
    // @dev Initializes the contract with MultiSig _approve_stopped state control off.
    // @dev Initializes the contract with MultiSig _nocontracts_stopped state control off.
    constructor () public {
        _mint_to_stopped = false;
        _change_operator_stopped = false;
        _taxrate_change_stopped = false;
        _charityrate_change_stopped = false;
        _blacklist_stopped = false;
        _antiwhale_stopped = false;
        _mint_stopped = false;
        _approve_stopped = false;
        _nocontracts_stopped = false;
    }

    // @dev **************** Lock the Mint_To   ***********************
    // @dev Emitted when Minting is paused by Operator.
    event Mint_To_Stopped(address account);

    // @dev Emitted when the Minting is allowed by Operator.
    event Started_Mint_To(address account);

    bool private _mint_to_stopped;

    // @dev Returns true if the contract is _mint_to_stopped, and false otherwise.
    function mint_to_stopped() public view returns (bool) {
        return _mint_to_stopped;
    }

    // @dev Modifier to make a function callable only when the contract is not _mint_to_stopped.
    modifier whenMintToOn() {
        require(!_mint_to_stopped, "MultiSig: Mint To Allowed");
        _;
    }

    // @dev Modifier to make a function callable only when the contract is _mint_to_stopped.
    modifier whenMintToOff() {
        require(_mint_to_stopped, "Multisig: Error: Mint To Permission Revoked");
        _;
    }

    // @dev Triggers stopped state. Call in function mint:
    // function mint(address _to, uint256 _amount) public onlyOwner whenMintToOn {
    function _stop_mint_to() internal virtual whenMintToOn {
        _mint_to_stopped = true;
        emit Mint_To_Stopped(_msgSender());
    }


    // @dev Returns to normal state.
    function _start_mint_to() internal virtual whenMintToOff {
        _mint_to_stopped = false;
        emit Started_Mint_To(_msgSender());
    }
    ...
    // @dev Triggers stopped state. Call in function transferOperator:
    // function transferOperator(address newOperator) public onlyOwner whenChangeOperatorOn {
    ...
    // @dev **************** Lock the TaxRate   ***********************
    // @dev Emitted when TaxRateChanging is paused by Operator.
    event TaxRate_Change_Stopped(address account);

    // @dev Emitted when the TaxRateChanging is allowed by Operator.
    event Started_TaxRate_Change(address account);

    bool private _taxrate_change_stopped;

    // @dev Returns true if the contract is _taxrate_change_stopped, and false otherwise.
    function taxrate_change_stopped() public view returns (bool) {
        return _taxrate_change_stopped;
    }

    // @dev Modifier to make a function callable only when the contract is not _taxrate_change_stopped.
    modifier whenTaxRateChangeOn() {
        require(!_taxrate_change_stopped, "MultiSig: TaxRateChange Allowed");
        _;
    }

    // @dev Modifier to make a function callable only when the contract is _taxrate_change_stopped.
    modifier whenTaxRateChangeOff() {
        require(_taxrate_change_stopped, "Multisig: Error: TaxRateChange Permission Revoked");
        _;
    }

    // @dev Triggers stopped state. Call in function updateTransferTaxRate:
    // function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOwner whenTaxRateChangeOn {
    function _stop_taxrate_change() internal virtual whenTaxRateChangeOn {
        _taxrate_change_stopped = true;
        emit TaxRate_Change_Stopped(_msgSender());
    }

    // @dev Returns to normal state.
    function _start_taxrate_change() internal virtual whenTaxRateChangeOff {
        _taxrate_change_stopped = false;
        emit Started_TaxRate_Change(_msgSender());
    }
    
    // @dev **************** Lock the CharityRate   ***********************
    // @dev Emitted when CharityRateChanging is paused by Operator.
    event CharityRate_Change_Stopped(address account);

    // @dev Emitted when the CharityRateChanging is allowed by Operator.
    event Started_CharityRate_Change(address account);

    bool private _charityrate_change_stopped;

    // @dev Returns true if the contract is _charityrate_change_stopped, and false otherwise.
    function charityrate_change_stopped() public view returns (bool) {
        return _charityrate_change_stopped;
    }

    // @dev Modifier to make a function callable only when the contract is not _charityrate_change_stopped.
    modifier whenCharityRateChangeOn() {
        require(!_charityrate_change_stopped, "MultiSig: CharityRateChange Allowed");
        _;
    }

    // @dev Modifier to make a function callable only when the contract is _charityrate_change_stopped.
    modifier whenCharityRateChangeOff() {
        require(_charityrate_change_stopped, "Multisig: Error: CharityRateChange Permission Revoked");
        _;
    }

    // @dev Triggers stopped state. Call in function updateCharityRate:
    //function updateCharityRate(uint16 _charityRate) public onlypOwner whenCharityRateChangeOn { 
    function _stop_charityrate_change() internal virtual whenCharityRateChangeOn {
        _charityrate_change_stopped = true;
        emit CharityRate_Change_Stopped(_msgSender());
    }

    // @dev Returns to normal state.
    function _start_charityrate_change() internal virtual whenCharityRateChangeOff {
        _charityrate_change_stopped = false;
        emit Started_CharityRate_Change(_msgSender());
    }
    ...
    // function setExcludedFromAntiWhale(address _account, bool _excluded) public onlyOwner whenAntiWhaleOn {
    ...
    // @dev **************** Lock the Mint_To   ***********************
        // @dev Emitted when Minting is paused by Operator.
    event Mint_Stopped(address account);

    // @dev Emitted when the Minting is allowed by Operator.
    event Started_Mint(address account);

    bool private _mint_stopped;

    // @dev Returns true if the contract is _mint_stopped, and false otherwise.
    function mint_stopped() public view returns (bool) {
        return _mint_stopped;
    }

    // @dev Modifier to make a function callable only when the contract is not _mint_stopped.
    modifier whenMintOn() {
        require(!_mint_stopped, "MultiSig: Mint Allowed");
        _;
    }

    // @dev Modifier to make a function callable only when the contract is _mint_stopped.
    modifier whenMintOff() {
        require(_mint_stopped, "MultiSig: Error: Mint Ro msg.sender Permission Revoked");
        _;
    }

    // @dev Triggers stopped state. Call in function mintUpdate:
    // function mint(uint256 amount) public onlyOwner  whenMintOff returns (bool) {
    function _stop_mint() internal virtual whenMintOn {
        _mint_stopped = true;
        emit Mint_Stopped(_msgSender());
    }

    // @dev Returns to normal state.
    function _start_mint() internal virtual whenMintOff {
        _mint_stopped = false;
        emit Started_Mint(_msgSender());
    }

    // @dev **************** Locking Approving Access by address can BREAKS THINGS   ***********************
        // @dev Emitted when Approveing is paused by Operator.
    event Approve_Stopped(address account);

    // @dev Emitted when the Approveing is allowed by Operator.
    event Started_Approve(address account);

    bool private _approve_stopped;

    // @dev Returns true if the contract is _approve_stopped, and false otherwise.
    function approve_stopped() public view returns (bool) {
        return _approve_stopped;
    }

    // @dev Modifier to make a function callable only when the contract is not _approve_stopped.
    modifier whenApproveOn() {
        require(!_approve_stopped, "MultiSig: Approve Allowed");
        _;
    }

    // @dev Modifier to make a function callable only when the contract is _approve_stopped.
    modifier whenApproveOff() {
        require(_approve_stopped, "MultiSig: Error: Approve Ro msg.sender Permission Revoked");
        _;
    }

    // @dev Triggers stopped state. Call in function approveUpdate:
    // function approve(address spender, uint256 amount) public override whenNotPaused  whenApproveOn returns (bool) {
    function _stop_approve() internal virtual whenApproveOn {
        _approve_stopped = true;
        emit Approve_Stopped(_msgSender());
    }

    // @dev Returns to normal state.
    function _start_approve() internal virtual whenApproveOff {
        _approve_stopped = false;
        emit Started_Approve(_msgSender());
    }
    ...
    // First whitelist all contracts that interact with ADMIRE, turn this on if under attack
        // @dev Emitted when NoContractsing is paused by Operator.
    // @dev Emitted when NoContractsing is paused by Operator.
    event NoContracts_Stopped(address account);

    // @dev Emitted when the NoContractsing is allowed by Operator.
    event Started_NoContracts(address account);

    bool public _nocontracts_stopped;
    ...
    modifier whenNoContractsOff() {
        require(_nocontracts_stopped, "MultiSig: Error: NoContracts Ro msg.sender Permission Revoked");
        _;
    }

    // @dev Triggers stopped state. Call in function nocontractsUpdate:
    // function nocontracts(address spender, uint256 amount) public override whenNotPaused  whenNoContractsOn returns (bool) {
...
contract BEP20 is Context, IBEP20, Ownable, Pausable, MultiSig {
     ...
     * - `spender` cannot be the zero address.
     */
    function increaseAllowance(address spender, uint256 addedValue) public whenApproveOn returns (bool) {
     ...
     */
    function mint(uint256 amount) public onlyOwner whenMintOn returns (bool) {
...
contract ADMIRE_Content_Coin is BEP20('ADMIRE Content Coin', 'ADMRSol6V54'){
    ...
    mapping(address => bool) private _excludedFromMultiSig;
    ...
    //bool private _nocontracts_stopped;
    modifier NoContractsOn(address sender, address recipient) {
            //require(isContract(sender), "Address: call to non-contract");
            if (
                _nocontracts_stopped = true 
                && _contractwhitelist[sender] == false
                && _contractwhitelist[recipient] == false
                && recipient.isContract() 
                && sender.isContract()
            ) {
                //require(isContract(sender), "Address: call to non-contract");
                revert("ADMIRE::Transfers to non-whitelisted contracts declined");
            } 
            _;
    }
    ...
    /**
     * @notice Constructs the AdmireToken contract.
     */
    constructor() public {
        _operator = _msgSender();
        _contractaddress = _msgSender();
    
        emit OperatorTransferred(address(0), _operator);
        emit ContractAddressTransferred(address(0), _contractaddress);
    ...
    function contractwhitelistUpdate(address user, bool value) public virtual onlyOwner whenNoContractsOn {
        // require(_owner == _msgSender(), "Only owner is allowed to modify blacklist.");
        _contractwhitelist[user] = value;
        emit ContractWhitelistUpdated(user, value);
    }
    
    function isBlackListed(address user) public view onlyOwner returns (bool) {
        return _blacklist[user];
        
    }
    
    function isContractWhiteListed(address user) public view onlyOwner returns (bool) {
        return _contractwhitelist[user];
    }
    
     function Allow_PausingTheChain() public onlyOperator whenNotPaused {
        _pause();
    }

    function Revoke_PausingTheChain() public onlyOperator whenPaused {
        _unpause();
    }
    
    function Revoke_Mint_To() public onlyOperator whenMintToOn {
        _stop_mint_to();
    }
  
     function Allow_Mint_To() public onlyOperator whenMintToOff {
        _start_mint_to();
    }
    
    function Revoke_Change_The_Operator() public onlyOperator whenChangeOperatorOn {
        _stop_change_operator();
    }

    function Allow_Change_The_Operator() public onlyOperator whenChangeOperatorOff {
        _start_change_operator();
    }

    function Revoke_TaxRate_Change() public onlyOperator whenTaxRateChangeOn {
        _stop_taxrate_change();
    }

    function Allow_TaxRate_Change() public onlyOperator whenTaxRateChangeOff {
        _start_taxrate_change();
    }
    
    function Revoke_CharityRate_Change() public onlyOperator whenCharityRateChangeOn {
        _stop_charityrate_change();
    }

    function Allow_CharityRate_Change() public onlyOperator whenCharityRateChangeOff {
        _start_charityrate_change();
    }

    function Revoke_BlacklistingAddresses() public onlyOperator whenBlacklistOn {
        _stop_blacklist();
    }

    function Allow_BlacklistingAdresses() public onlyOperator whenBlacklistOff {
        _start_blacklist();
    }

    function Revoke_AntiWhale() public onlyOperator whenAntiWhaleOn {
        _stop_antiwhale();
    }

     function Allow_AntiWhale() public onlyOperator whenAntiWhaleOff {
        _start_antiwhale();
    }

    // @dev WARNING! Things will break! Only for pausing during partner sites' and/or BSC network-wide hacks.
    function Revoke_Mint() public onlyOperator whenMintOn {
        _stop_mint();
    }

     function Allow_Mint() public onlyOperator whenMintOff {
        _start_mint();
    }

    // @dev WARNING! Things will break! The devs need to be able to increase allowances.
    function Allow_ApprovingSpendableAddresses() public onlyOperator whenApproveOn {
        _stop_approve();
    }

     function Revoke_ApprovingSpendableAddressess() public onlyOperator whenApproveOff {
        _start_approve();
    }
    // @dev WARNING! This will absolutely break things! Whitelist every known contract first!
    function Allow_TurnOnContractBlocking() public onlyOwner whenNoContractsOn {
        _stop_nocontracts();
    }

     function Allow_NonWhitelistedContracts() public onlyOperator whenNoContractsOn {
        _stop_nocontracts();
    }

     function Revoke_NonWhitelistedContracts() public onlyOperator whenNoContractsOff {
        _start_nocontracts();
    }
   ...
   /// destroy the contract and reclaim the leftover funds. REMOVE FOR PRODUCTION USE
     ...
     */
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOwner whenTaxRateChangeOn {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "ADMIRE::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateCharityRate(uint16 _charityRate) public onlyOwner whenCharityRateChangeOn {
        require(_charityRate <= 10000, "ADMIRE::updateCharityRate: Charity rate must not exceed the maximum rate.");
        emit CharityRateUpdated(msg.sender, charityRate, _charityRate);
        charityRate = _charityRate;
    }

    // owner can drain tokens that are sent here by mistake
    function emergencyBEP20Drain(BEP20 token, uint amount) public onlyOwner {
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOwner {
        require(_maxTransferAmountRate <= 10000, "ADMIRE::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     */
    function transferOperator(address newOperator) public onlyOwner whenChangeOperatorOn {
        require(newOperator != address(0), "ADMIRE::transferOperator: new operator is the zero address");
        emit OperatorTransferred(_operator, newOperator);
        _operator = newOperator;
    }

    /**
    * @dev Returns the address of the current trusted external contract address.
    */
  ...
  /**
    * @dev Extra security hook against future "multisig" implemenations.
    * Address this contract should trust can be queried from this contract itself
    * This can enable a "trusted" payable wallet for the operator with default "yes" from owner.
    * Change the address to remove operator's signing ability
    */
    function transferTrustedContractAddress(address newContractAddress) public onlyOwner {
        require(newContractAddress != address(0), "ADMIRE::transferTrustedContractAddress: contract cannot be the zero address");
        emit ContractAddressTransferred(_contractaddress, newContractAddress);
  ...
  /**
    ...
    */
    function transferCharitytAddress(address newCharityAddress) public whenCharityRateChangeOn onlyOwner {
    ...
    * Comment out this function if you want CHARITY_ADDRESS to be static
    */
    function transferTaxtAddress(address newTaxAddress) public whenTaxRateChangeOn onlyOwner {
        require(newTaxAddress != address(0), "ADMIRE::transferTaxAddress: contract cannot be the zero address");
