...
contract LCRtoken is ERC20 {
	...
	function getInterestInfo(address conta) public view returns (uint256 interest)
	{		
		require (stakeBalances[conta] > 0, "zero stake balance");	
		
		uint256 interestToPay = 0;
		uint256 deltaBlocks = block.number - stakeBlock[conta];
		
		if (deltaBlocks > intervalBlockCountYear)
		{
			interestToPay = stakeBalances[conta] * (1 / stakeInterest);			
	...
	function getStakeValue(address conta) public view returns (uint256 stakedBalance)
	{		
	    return stakeBalances[conta];
	}
