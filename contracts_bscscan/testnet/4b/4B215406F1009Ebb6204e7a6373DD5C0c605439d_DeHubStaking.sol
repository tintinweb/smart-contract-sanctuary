...
contract DeHubStaking is Ownable, AccessControl, Pausable, IDehubStaking {
  ...
  // Average block time in seconds. Will be used to calculate projected rewards.
  uint8 public avgBlockTime = 3;
  ...
  /**
   * @notice Update average block time, as it can increase with time.
   * @dev I know it's not very reasonable to believe we will need anyting more
   * than 3 sec., but still.
   */
  function setAverageBlockTime(uint8 _value) external onlyOwner {
    avgBlockTime = _value;
  }
  ...
  function harvestAndWithdraw() external canHarvest whenNotPaused {
    ...
    // Transfer total rewards all users staked tokens.
  ...
  /**
   * @notice Function to get the pending rewards for a 'msg.sender' based on the
   * current pool state and the user's staking amount. Must include the
   * reflections amount too.
   * @dev In this function we don't update any storage, just pure calculations
   * to be used for the viewing in the UI.
   */
    ...
    returns (uint256 claimableReflection, uint256 claimableHarvest)
  {
    ...
    // Pending rewards from last update to now
    (claimableReflection, claimableHarvest) = _calcRewards(
      user,
   ...
   * @dev We estimate close block number and then calcRewards without changing
   * any states or storage to keep it a view function.
   */
    ...
    returns (uint256 claimableReflection, uint256 claimableHarvest)
  {
    ...
    // Calc estimated close block number based on the average block time.
    uint256 timeDelta = pool.closeTimeStamp - pool.openTimeStamp;
    uint256 estCloseBlock = timeDelta / avgBlockTime;

    // Projected rewards from last update to close
    (claimableReflection, claimableHarvest) = _calcRewards(
      user,
      pool.lastUpdateBlock,
      estCloseBlock
   ...
   * "harvestFund" from the token balance of the contract.
  ...
  function getTotalReflectionsAccumulated() public view returns (uint256) {
    ...
    if (balance == 0) {
      ...
      uint256 totalReflectionsAccumulated = balance.sub(pool.harvestFund);
    ...
    returns (uint256)
  {
    ...
    uint256 valuePerBlock = pool.valuePerBlock.add(
  ...
  /**
   * @dev Helper function to calculate how many rewards will the user get based
   * on his current staking amount, other variables and arbitrary block distance.
   * Note: this is a view function just for getting data, not actually acting
   * and changing any states.
   */
  ...
  {
    //    require(_blockFrom < _blockTo, "from block must be bigger");
    if (_user.amount == 0) return (0, 0);

    uint256 vPerBlock = _getValuePerBlock(_blockFrom, _blockTo);
    uint256 totalReflections = getTotalReflectionsAccumulated();
    uint256 reflShare = _user.amount.mul(vPerBlock).div(1e30);
    uint256 harvShare = _user.amount.mul(vPerBlock).div(1e30);
    // Cache all user variables locally to avoid storage manipulations.
    uint256 reflectionPending = _user.reflectionPending;
    uint256 harvestPending = _user.harvestPending;

    if (_user.amount > 0) {
      // Adjust pending reflection based on the current block.
      uint256 newReflectionPending = reflShare.sub(_user.reflectionDebt);
      uint256 newHarvestPending = harvShare.sub(_user.harvestDebt);
      reflectionPending += newReflectionPending;
      harvestPending += newHarvestPending;
    }

    claimableReflection = reflectionPending.mul(totalReflections).div(1e20);
    claimableHarvest = harvestPending.mul(pool.harvestFund).div(1e20);
  }
