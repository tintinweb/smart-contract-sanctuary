...
contract EarnOtherFixedAPR is Ownable, ReentrancyGuard {
  ...
  */
  ...
  function pendingReward(address _for) external view returns (uint256 rewards) {
    require(_for != address(0), "EarnOther::pendingReward:: bad address");
    UserInfo storage user = userInfo[_for];
    rewards = _calculateReward(user.amount, user.lastRewardBlock);
  }
  ...
  */
  function _calculateReward(uint256 _amount, uint256 _calculateFromBlock) internal view returns (uint256 rewards) {
    ...
    // if (toBlock <= _calculateFromBlock || _amount == 0) {
    //   return 0;
    // }
    // if (toBlock > endBlock) {
    //   toBlock = endBlock;
    ...
    // rewards =_amount.mul(blocks.mul(rewardPerBlock)).div(REWARD_PER_BLOCK_PRECISION_FACTOR);
    rewards = 0;
  }
