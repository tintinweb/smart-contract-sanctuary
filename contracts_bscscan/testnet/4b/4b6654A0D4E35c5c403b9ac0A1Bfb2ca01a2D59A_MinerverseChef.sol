...
import "../library/bep20/IBEP20.sol";
...
import "./MinerverseToken.sol";

contract MinerverseChef is IMinerverseChef, OwnableUpgradeable {
    using SafeMath for uint;
    using SafeBEP20 for IBEP20;

    /* ========== CONSTANTS ============= */

    MinerverseToken public constant MVX = MinerverseToken(0xC9849E6fdB743d08fAeE3E34dd2D1bc69EA11a51);
    ...
    IMinerverseMinterV2 public minter;

    uint public startBlock;
    uint public override minerversePerBlock;
    uint public override totalAllocPoint;

    /* ========== MODIFIERS ========== */

    modifier onlyVaults {
        require(vaults[msg.sender].token != address(0), "MinerverseChef: caller is not on the vault");
        _;
    }
    ...
    modifier updateRewards(address vault) {
        ...
        if (block.number > vaultInfo.lastRewardBlock) {
            ...
            if (tokenSupply > 0) {
                ...
                uint rewards = multiplier.mul(minerversePerBlock).mul(vaultInfo.allocPoint).div(totalAllocPoint);
                vaultInfo.accMinerversePerShare = vaultInfo.accMinerversePerShare.add(rewards.mul(1e12).div(tokenSupply));
            }
    ...
    event MinerverseRewardPaid(address indexed user, address indexed vault, uint amount);

    /* ========== INITIALIZER ========== */

    function initialize(uint _startBlock, uint _minerversePerBlock) external initializer {
        __Ownable_init();

        startBlock = _startBlock;
        minerversePerBlock = _minerversePerBlock;
    }
    ...
    function pendingMinerverse(address vault, address user) public view override returns (uint) {
        UserInfo storage userInfo = vaultUsers[vault][user];
        VaultInfo storage vaultInfo = vaults[vault];

        uint accMinerversePerShare = vaultInfo.accMinerversePerShare;
        uint tokenSupply = tokenSupplyOf(vault);
        if (block.number > vaultInfo.lastRewardBlock && tokenSupply > 0) {
            uint multiplier = timeMultiplier(vaultInfo.lastRewardBlock, block.number);
            uint minerverseRewards = multiplier.mul(minerversePerBlock).mul(vaultInfo.allocPoint).div(totalAllocPoint);
            accMinerversePerShare = accMinerversePerShare.add(minerverseRewards.mul(1e12).div(tokenSupply));
        }
        return userInfo.pending.add(userInfo.balance.mul(accMinerversePerShare).div(1e12).sub(userInfo.rewardPaid));
    }
    ...
    function addVault(
        ...
        require(vaults[vault].token == address(0), "MinerverseChef: vault is already set");
        bulkUpdateRewards();
    ...
    function updateVault(address vault, uint allocPoint) public onlyOwner {
        require(vaults[vault].token != address(0), "MinerverseChef: vault must be set");
        bulkUpdateRewards();
    ...
    function setMinter(address _minter) external onlyOwner {
        require(address(minter) == address(0), "MinerverseChef: setMinter only once");
        minter = IMinerverseMinterV2(_minter);
    }

    function setMinerversePerBlock(uint _minerversePerBlock) external onlyOwner {
        bulkUpdateRewards();
        minerversePerBlock = _minerversePerBlock;
    }
    ...
    function notifyDeposited(address user, uint amount) external override onlyVaults updateRewards(msg.sender) {
        ...
        uint pending = userInfo.balance.mul(vaultInfo.accMinerversePerShare).div(1e12).sub(userInfo.rewardPaid);
        userInfo.pending = userInfo.pending.add(pending);
        userInfo.balance = userInfo.balance.add(amount);
        userInfo.rewardPaid = userInfo.balance.mul(vaultInfo.accMinerversePerShare).div(1e12);
    ...
    function safeMinerverseTransfer(address user) external override onlyVaults updateRewards(msg.sender) returns (uint) {
        UserInfo storage userInfo = vaultUsers[msg.sender][user];
        VaultInfo storage vaultInfo = vaults[msg.sender];
        ...
        minter.mint(amount);
        minter.safeMinerverseTransfer(user, amount);
        emit MinerverseRewardPaid(user, msg.sender, amount);
        return amount;
    }
    ...
    function recoverToken(address _token, uint amount) external virtual onlyOwner {
        require(_token != address(MVX), "MinerverseChef: cannot recover MVX token");
