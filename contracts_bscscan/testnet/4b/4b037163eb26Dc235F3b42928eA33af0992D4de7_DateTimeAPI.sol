...
contract DateTimeAPI {
  ...
  uint constant private MINUTE_IN_SECONDS = 60;
  ...
  function parseTimestamp(uint timestamp) public pure returns (DateTimeObject memory dt) {
    uint secondsAccountedFor = 0;
    uint buf;
    uint8 i;
    // Year
    ...
    // Day
    for (i = 1; i <= _getDaysInMonth(dt.month, dt.year); i++) {
    ...
    // Leap year
    dt.isLeapYear = _isLeapYear(dt.year);
  }
