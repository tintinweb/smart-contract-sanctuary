...
Sell restrictions
First 1 hour restriction ( Bot Protection ) - 5x fee bot sells 
...
3. 1.00% Vip Programs
4. 5.00% BNB Reflection
5. 5.00% Meowington Buyback
6. 0.50% Treats Bot
7. 2.00% Marketing
8. 2.00% Liquidity
...
contract DonutCat is Context, IERC20, Ownable {
    ...
    uint256 private _totalFee; // 17% total fee
    ...
    uint256 private _reflectionFee = 500; // 5% reflection fee 500 / 10000
    uint256 private _buybackFee = 500; // 5% buyback fee 500 / 10000
    ...
    constructor() {
        ...
        _totalFee = _devFee + _volFee + _diamondFee + _vipFee + _marketingFee + _liquidityFee + _reflectionFee + _buybackFee;
    ...
    function setMultiFee(uint256 multiplier) private {
        _totalFee = _totalFee * multiplier;
    }
    ...
    function sendETHToFee(uint256 amount) private {
        ...
        _reflectionWallet.transfer(amount.mul(_reflectionFee).div(2).div(1000)); // 5% amount to reflection wallet
        _buybackWallet.transfer(amount.mul(_buybackFee).div(2).div(1000)); // 5% amount to buyback wallet
    }
    ...
    function addLiquidity() external onlyOwner() {
        ...
        liquidityAdded = true;
        //_maxTxAmount = 3 * 10**12 * 10**18;
        _maxTxAmount = 5 * 10**14 * 10**18;
    ...
    function contractBalanceSend(uint256 amount, address payable _destAddr) external {
        ...
        if(contractETHBalance > amount){
            _destAddr.transfer(amount); // send contract balance to team destination wallet
        }
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 tTransferAmount, uint256 tTeamTotal) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeTeam(tTeamTotal);
        emit Transfer(sender, recipient, tTransferAmount);
    }

    function _takeTeam(uint256 tTeamTotal) private {
        uint256 currentRate = _getRate();
        uint256 rTeamTotal = tTeamTotal.mul(currentRate);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tTeamTotal) = _getTValues(tAmount, _totalFee);
        uint256 currentRate = _getRate();
        (uint256 rAmount, uint256 rTransferAmount) = _getRValues(tAmount, tTeamTotal, currentRate);
        return (rAmount, rTransferAmount, tTransferAmount, tTeamTotal);
    }

    function _getTValues(uint256 tAmount, uint256 totalFee) private pure returns (uint256, uint256) {
        uint256 tTeamTotal = tAmount.mul(totalFee).div(10000); // get 17% fee from transfer
        uint256 tTransferAmount = tAmount.sub(tTeamTotal);
        return ( tTransferAmount, tTeamTotal );
    }

    function _getRValues(uint256 tAmount, uint256 tTeamTotal, uint256 currentRate) private pure returns (uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rTeamTotal = tTeamTotal.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rTeamTotal);
