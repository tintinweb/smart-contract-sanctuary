...
contract MockMasterChef is IMasterChef {
	...
	function addPool(IERC20 _stakedToken) external {
		poolInfo.push(PoolInfo({stakedToken: _stakedToken}));
	}
	...
	function _deposit(uint256 _pid, uint256 _amount) internal {
		...
		if (user.amount > 0) {
			_sendRewards(address(msg.sender));
		}
		...
		emit Deposit(address(msg.sender), _pid, _amount);
	}
	...
	function _sendRewards(address _to) internal {
		if (rewardAmount > 0 && block.number > lastRedeemBlock) {
			IMintableToken(address(rewardToken)).mint(
				address(this),
				rewardAmount
			);
