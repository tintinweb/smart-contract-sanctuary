...
contract PrivateSale is Initializable , OwnableUpgradeable{
  ...
  function addInvester(address ref, uint256 totalInvest) public onlyOwner {
    ...
    uint256 lockAmount = totalInvest * 91 /100;
  ...
  function getClaimable(address buyerAddress) public view returns (uint256) {
    ...
    uint256 userStartUnlockBlock = SellLockUser[buyerAddress].firstReleaseBlock > startUnlockBlock ? SellLockUser[buyerAddress].firstReleaseBlock : startUnlockBlock + totalCiffBlock;
  ...
  function claim(address recipient) public unlockEnabled {
    require(whiteList[recipient].enable, "White list invalid");
  ...
  function buyTokenByUSD(address recipient, uint256 usdAmount, address usdToken) public sellEnabled returns (bool) {
    require(whiteList[recipient].enable, "Ref invalid");
    require(whiteListToken[usdToken], "USD Token invalid");
    require(msg.sender == recipient, "Buyer invalid");
    ...
    if (soldToken > 0) {
        uint256 lockAmount = soldToken * 91 / 100;
