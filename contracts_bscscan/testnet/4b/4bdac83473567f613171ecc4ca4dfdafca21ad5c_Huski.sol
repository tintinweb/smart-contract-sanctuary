...
abstract contract Lottery is VRFConsumerBase
{
    ...
    */
    ...
    {
        if(_ticketBalances[account].lotteryTime == _lotteryStart)
        {
            return _ticketBalances[account].tickets;
        }
...
contract Huski is IERC20, IERC20Metadata, Lottery, Context, Ownable
{
        ...
        )
    {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //TESTNET Pancakeswap router address https://pancake.kiemtienonline360.com/#/swap
    ...
    */
    ...
    {
        _lotteryStart = block.timestamp - 1 days;
    ...
    function buyTickets() public payable override returns (bool)
    {
        ...
        _ticketCount = _ticketCount + ticketAmount;

        //Remove previous lotteries tickets
        if(_ticketBalances[sender].lotteryTime == _lotteryStart)
        {
            _ticketBalances[sender].tickets = _ticketBalances[sender].tickets + ticketAmount;
        }
        else
        {
            _ticketBalances[sender].tickets = ticketAmount;
            _ticketBalances[sender].lotteryTime = _lotteryStart;
        }
    ...
    function _drawLottery(uint256 randomNumber) internal override
    {
        ...
        address winner = _tickets[drawnTicket];

        uint256 tLotteryPool = lotteryPool();

        _tRewards[winner] = _tRewards[winner] + tLotteryPool;

        _tUnclaimedRewards = _tUnclaimedRewards + tLotteryPool;
