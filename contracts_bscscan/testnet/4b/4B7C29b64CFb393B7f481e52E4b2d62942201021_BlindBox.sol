...
contract BlindBox  is Ownable {
   ...
   function setBox(
    ...
    function buyBox(uint8 boxId,uint256 num,address _user) public  returns (uint256){
        ...
        currency.safeTransferFrom(_user, address(config.getReceiveAddress()), amount);
        uint256 value=0;
        delete LastBoxOrderList[_user];
        for (uint256 i = 1; i <= num; i++) {
         uint256 _value=   getValue(detail.chacnce,i);
            value=value.add(_value);
            LastBoxOrderList[_user].push(LastBoxOrder(_value));
            BoxOrderList[_user].push(BoxOrder(boxId,detail.boxType,1,detail.amount,_value,detail.currency,block.timestamp));
        } 
        emit BuyBox(_user,boxId,detail.boxType,num, amount,currency);
        return value;
    }
