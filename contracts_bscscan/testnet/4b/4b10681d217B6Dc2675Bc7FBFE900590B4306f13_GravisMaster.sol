...
contract GravisMaster is ERC721Holder, Ownable, Pausable, ReentrancyGuard {
    ...
    function deposit(uint256 _pid, uint256 _amount) public whenNotPaused nonReentrant {
        ...
        // If bonus deadline for the pool is not set, set it to current time + bonusDeadline
        if (pool.bonusDeadlineTime == 0) {
            pool.bonusDeadlineTime = block.timestamp.add(pool.bonusDeadline);
        }
