...
contract LeagueBasedDogeSupreme is IBEP20, Auth, TokensRescuer {
    using SafeMath for uint256;
    ...
    uint256 constant _totalSupplyTokens = 1000000000; // 1 Billion tokens
    uint256 _totalSupply = _totalSupplyTokens * (10 ** _decimals); // Total
    ...
    uint256 totalFee = 1800; // 18%
    uint256 feeDenominator = 10000; // Example: 1800/10000 = 0.18 = 18%
    ...
    // Rules - Max transaction and wallet
    ...
    // Rules - Pause/Start transaction
    bool private _txPaused = true;

    constructor(
        string memory _token_name,
        string memory _token_symbol
    ) Auth(msg.sender) TokensRescuer(msg.sender) public { // Sets Auth wallet + TokensRescuer wallet 
        // Default Values
        address _router_address = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
        address _rewards_token = 0x5AF335c2a80e6F58A3807d7571aFb7A90387e8d7;
        ...
        // Allow router to transfer all tokens belonging to this contract
        ...
        // Liquidity + Marketing wallets setup
        autoLiquidityReceiver = address(this);
        marketingFeeReceiver = msg.sender;

        // Give total supply to the contract owner
    ...
    // msg.sender > person calling the function
    
    // Question: Why is this "public" and not "external"?
    // msg.sender > the user/wallet (in the pancakeswap "approve" button)
    // spender > router (in the pancakeswap "approve" button)
    // Runs when clicking "approve" button in pancakeswap
    ...
    // msg.sender > can be the user/wallet, or pancakeswap router, or something else
    // For example when you transfer from wallet to another wallet then msg.sender is the the "from" wallet
    // When buying tokens (eg. pancakeswap) the msg.sender is the pair contract (e.g. buy 0.04 BNB will be sent to pair address 
    // and then address pair address sends 10 millions tokens back using this transfer function)
    // Override: Transfer from msg.sender to "to" address a "value" amount of tokens
    function transfer(address recipient, uint256 amount) external override returns (bool) { // "Buy"
        return _transferFrom(msg.sender, recipient, amount);
    }

    // "uint256(-1)" > maximum number for a uint256
    // Override: Transfer from "from" to "to" address a "value" amount of tokens
    // Runs when clicking "swap" button in pancakeswap
    // msg.sender > router (in the pancakeswap "swap" button)
    // sender > the user/wallet (in the pancakeswap "swap" button)
    // recipient > owner (in the pancakeswap "swap" button)
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) { // "Sell"
        ...
        // this line is checking which of the two options the user used for the initial approval. 
        // If the user has not granted an "infinite" allowance, then subtract the current transfer amount from 
        // their allowance, otherwise don't reduce their allowance.
    ...
    // Pause/Start Transactions
    // TODO make sure the only person that can run this is Authorized
    function pauseTransactions(bool _paused) public {
        _txPaused =  _paused;
    }
}