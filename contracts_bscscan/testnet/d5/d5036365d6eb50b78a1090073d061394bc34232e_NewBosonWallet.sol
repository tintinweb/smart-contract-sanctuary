...
abstract contract Context {
 ...
 contract NewBosonWallet is Context, ReentrancyGuard{
    ...
    //transfer of admin
         function ownershipstransfer(address New) nonReentrant public onlyAdmin
         { admin = New;}   
   ...
   //function Vote to withdraw 50%
    ...
    //function Vote against withdrawal
     function voteagainst() nonReentrant public returns (bool)
     {
        ...
        require (B>0 && B<G);
         uint L = B.mul(10000000000);
         uint P = L.div(G);
        votepercentagainst=votepercentagainst.add(P);
   ...
   //withdraw 100% after successfully voted by more than 50% of New Boson holders 
    ...
    function withdrawoneyear (bool requirement) nonReentrant public onlyAdmin returns(bool){
        if (votepercentagainst<50)
    {
        require (requirement == true);
        require (block.timestamp>timestampA.add(31535462));
        timestampA=block.timestamp.add(31535462);
    ...
    function withdrawsixmonths (bool requirement) nonReentrant public onlyAdmin returns(bool){
        if (votepercentagainst<50)
    {
        require (requirement == true);
        require (block.timestamp>timestampA.add(15767731));
        timestampB=block.timestamp.add(15767731);
        for (uint i=0; i<Voteragainst.length; i++)
        voteragainst[Voteragainst[i]]=0;
        votepercentagainst=0;
        uint valuesixmonths = value.mul(50).div(50);
