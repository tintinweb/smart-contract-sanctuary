...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    mapping(address => mapping(uint256 => bool)) isCreator;
    ...
    function _setCreator(address creator, uint256 id) internal virtual{
        isCreator[creator][id] = true;
    }

    function _removeCreator(address creator, uint256 id) internal virtual{
        isCreator[creator][id] = false;
    }
...
contract ERC1155Token is ERC1155, Ownable, ReentrancyGuard {
    ...
    function setCreator(address creator, uint256 id) public nonReentrant onlyOwner{
        address[] storage creators;
        creators = idToCreator[id];
        creators.push(creator);
        idToCreator[id] = creators;
        _setCreator(creator,id);
    }
    
    function removeCreator(address creator, uint256 id) public nonReentrant onlyOwner{
        for(uint i = 0 ;  i < idToCreator[id].length ; i++ ){
            if(idToCreator[id][i] == creator){
                delete idToCreator[id][i];
                _removeIndex(idToCreator[id],i);
            }
        }
    ...
    function _removeIndex(address[] storage array,uint index) internal {
