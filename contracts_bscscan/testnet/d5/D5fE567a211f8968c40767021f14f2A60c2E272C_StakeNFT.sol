...
contract StakeNFT is ReentrancyGuard {
    ...
    Counters.Counter private _stakedItem;

    address payable owner;

    uint256 stakePrice = 0.025 ether;
    ...
    mapping(address => StakeItem) stakeHolderInfo;
    ...
    function getStakingPrice() public view returns (uint256) {
        return stakePrice;
    }
    ...
    function createMarketplaceSale(address nftContract, uint256 itemId)
        ...
        nonReentrant
    {
        uint256 price = idToStakeItem[itemId].price;
        ...
        idToStakeItem[itemId].seller.transfer(msg.value);
        IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId);
        idToStakeItem[itemId].owner = payable(msg.sender);
        idToStakeItem[itemId].sold = true;

        _stakedItem.increment();
    
        payable(owner).transfer(stakePrice);
    }

    // returns all unsold marketplace items
    function fetchStakeItems()
        public
        view
        returns (StakeItem[] memory)
    {
        uint256 itemCount = _items.current();
        uint256 unsoldItemCount = _items.current() - _stakedItem.current();
        ...
        for (uint256 i = 0; i < itemCount; i++) {
            if (idToStakeItem[i + 1].owner == address(0)) {
                ...
                StakeItem storage currentItem = idToStakeItem[
                    currentId
                ];
    ...
    function fetchMyNFTs() public view returns (StakeItem[] memory) {
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (idToStakeItem[i + 1].owner == msg.sender) {
                itemCount += 1;
            }
        }
        ...
        returns (StakeItem[] memory)
    {
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (idToStakeItem[i + 1].seller == msg.sender) {
                itemCount += 1;
            }
        }
