...
contract DirectSwap {
    ...
    // IPancakeRouter02 private constant ROUTER = IPancakeRouter02(0x10ED43C718714eb63d5aA57B78B54704E256024E); bscMainnet
    ...
    function swapThroughPancakeSwap(address _from, address _to, uint _amount, uint _minOutAmount) public {
        IERC20(_from).safeTransferFrom(msg.sender, address(this), _amount);
        return _zap(_from, _to, _amount, _minOutAmount);
    }

    function _zap(address _from, address _to, uint _amount, uint _minOutAmount) internal {
        uint amount;
        amount = _swapDirect(_from, _amount, _to);

        require(amount >= _minOutAmount, string(abi.encodePacked("slippage exceeds, can only receive ", _uint2str(amount))));
    }
