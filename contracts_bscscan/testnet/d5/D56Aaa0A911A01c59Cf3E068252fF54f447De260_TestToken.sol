...
contract TestToken is Context, IERC20, Ownable {
    ...
    uint256 private _preparationRateDivisor;
    uint256 private _burnBufferFactor;
    uint256 private _memeBurnAmount = 420000000000;
    uint256 private _memeBurnCost;
    uint256 private _maxBurnMatchingDivisor = 1000; //1 Trillion
    ...
    function _transfer(
        ...
        */
        ...
        if (justLaunched){
            if (block.timestamp <= launchTime.add(300)){          // minutes [1,5]
            ...
            else if (block.timestamp <= launchTime.add(600)){     // minutes (15,60]
            ...
            maxTxAmount = _useCustomMaxTx ? _maxTxAmount : currentSupply.div(3333);    // 300 billion at launch     
            maxWalletAmount = _useCustomMaxTk ? _maxWalletAmount : currentSupply.div(100); // Cap by circulating token supply or total?
        }
        
        if(from != owner() && to != owner() && 
        from != address(this) && to != uniswapV2Pair) {   //don't need dead address, doesn't go through transfer
            require(amount <= maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
            require(walletTokens.add(amount) <= maxWalletAmount, "Wallet amount exceeds the maxWalletAmount.");
        }
        
        if (_accumulatingForBurn && _automaticBigBurn && 
        balanceOf(address(this)) >= _bigBurnAmount.mul(_burnBufferFactor).div(10)){
            this.bigBurn();
        ...
        /*
        if (!inBuyBack && !inSwapAndLiquify && to == uniswapV2Pair &&
            balanceOf(uniswapV2Pair) > 0 && address(this).balance > 0){
            swapETHForExactTokens(memeBurnAmount);
        }
        */
        ...
        _isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(from == uniswapV2Pair){
                ...
                if (_burnModeActive){
                    ...
                    uint256 amountToBurn = amount > currentSupply.div(_maxBurnMatchingDivisor) ? 
                    currentSupply.div(_maxBurnMatchingDivisor) : amount; 
                    this.burn(amountToBurn); //Reward big buys or more smaller buys? *amount = currentBalance
                }
                else{
                    _taxFee = _buyTaxFee;
            ...
            if(to == uniswapV2Pair){
                ...
                removeAllFee();
                if (_burnModeActive){
                    _taxFee = _sellTaxFee.add(_sellBurnModeFeeIncrease);
    ...
    function swapTokens(uint256 tokenAmountCandidate) private lockTheSwap {
        ...
        uint256 tokensToSwap = _accumulatingForBurn ?
        (balanceOf(address(this)) > _bigBurnAmount.mul(_burnBufferFactor).div(10)  ? 
        balanceOf(address(this)).sub(_bigBurnAmount.mul(_burnBufferFactor).div(10)) : balanceOf(address(this)).div(_preparationRateDivisor) )  :
        tokenAmountCandidate;
        
        uint256 initialBalance = address(this).balance;
        _memeBurnCost = estimateEthCost(_memeBurnAmount);
        //Keep at least ten memeBurn bnb-buffer in contract, TODO: consider long burnMode
        
        if (initialBalance < _memeBurnCost.mul(10)){
            ...
            transferToAddressETH(marketingAddress, ethToTransfer - ethToLP);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            path,
            payable(address(this)), // The contract            //X
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        uniswapV2Router.addLiquidityETH{value: ethAmount}(
            ...
            0, // Slippage is unavoidable
            owner(),                        //Risk, LP tokens should be in contract https://certik-public-assets.s3.amazonaws.com/CertiK+Audit+Report+for+SafeMoon.pdf
            block.timestamp
        );
    }
    ...
    function estimateEthCost(uint256 tokenAmount) public view returns(uint256) {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(this);
    ...
    function burn(uint256 tAmount) external onlyAuthorized {
    ...
    function prepareForBurn(bool automaticBigBurn, uint256 burnAmount, 
     uint256 burnBufferFactor, uint256 preparationRateDivisor,  
     uint256 maxBurnMatchingDivisor) external onlyAuthorized {
        _automaticBigBurn = automaticBigBurn; 
        _bigBurnAmount = burnAmount;
        _burnBufferFactor = burnBufferFactor;
        _preparationRateDivisor = preparationRateDivisor;
        _maxBurnMatchingDivisor = maxBurnMatchingDivisor;
    ...
    function bigBurn() external onlyAuthorized {
        ...
        this.burn(_bigBurnAmount);
        this.burnMode(true);
    }
    
    function bigBurnReady() external view onlyAuthorized returns(bool){
        return balanceOf(address(this)) >= _bigBurnAmount.mul(_burnBufferFactor).div(10);
    }
    ...
    function burnMode(bool burnModeActive) external onlyAuthorized {
        ...
        uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, _maxBurnMatchingDivisor))) %
    ...
    function setRouter(address _router) public onlyOwner returns(address _pair) {
