...
contract SummitReferrals is ISummitReferrals {
    ...
    function enableSummit(SummitToken _trueSummit) external onlyCartographer() override {
        summit = _trueSummit;
    }
    ...
    event BurnUnclaimedRewards(uint256 indexed round, uint256 amount);
    ...
    function addReferralRewardsIfNecessary(address referee, uint256 amount) external onlyCartographer() override {
    ...
    function getReferralRound() external view override returns(uint256) {
        return roundIndex;
    }

    // Called after burn button pressed with correct code and tokens burned
    function burnUnclaimedReferralRewardsAndRolloverRound(address referee) external onlyCartographer() override {
        uint256 burnReward = summit.balanceOf(address(this)).div(100);
        summit.transfer(referee, burnReward);
        summit.burn(summit.balanceOf(address(this)));
        roundIndex += 1;
    }
