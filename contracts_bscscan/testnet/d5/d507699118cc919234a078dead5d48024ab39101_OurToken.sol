...
/**
 ...
 - Supply:            1.000.000
 - Max wallet:        30000  
 - Max TX:            max buy 20000 / max sell 10000
 - Sell cooldown:     120 SECONDS (!)
 - Buy cooldown:      30 SECONDS (!)
 - Fees:              8% reflections // 1% marketing // 1% liquidity //
 ...
 * COIN
 */
contract OurToken is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public _maxBuyTxAmount = 20000 * 10**9;
    ...
    uint256 private rewardFeeBuy = 8;
    uint256 private devFeeBuy = 1; 
    uint256 private totalFeeBuy = 10;

    // Sell tax in percentage
    uint256 private liquidityFeeSell = 1;
    uint256 private burnFeeSell = 0;
    uint256 private rewardFeeSell = 8;
    uint256 private devFeeSell = 1;
    ...
    // tax variables 
    uint256 private _tBNBrRewardsTotal;
    ...
    // Get BNBr rewards
    function totalBNBrRewards() public view returns (uint256) {
        return _tBNBrRewardsTotal;
    }
    ...
    */
    ...
    function deliver(uint256 tAmount) public {
        ...
        _tBNBrRewardsTotal = _tBNBrRewardsTotal.add(tAmount);
    }
    ...
    function _BNBrFee(uint256 rBNBrFee, uint256 tBNBrFee) private {
        _rTotal = _rTotal.sub(rBNBrFee);
        _tBNBrRewardsTotal = _tBNBrRewardsTotal.add(tBNBrFee);
    }
    ...
    {
        (uint256 tTransferAmount, uint256 tBNBrFee, uint256 tBurn, uint256 tLiquidity) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rBNBrFee) = _getRValues(tAmount, tBNBrFee, tBurn, _getRate(), tLiquidity);
        return (rAmount, rTransferAmount, rBNBrFee, tTransferAmount, tBNBrFee, tBurn, tLiquidity);
    }
    ...
    {
        uint256 tBNBrFee = calculateRewardFee(tAmount);
        uint256 tBurn = calculateBurnFee(tAmount);
        uint256 tLiquidity = calculateLiquidityFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tBNBrFee).sub(tBurn).sub(tLiquidity);
        return (tTransferAmount, tBNBrFee, tBurn, tLiquidity );
    }
        ...
        )
    {
        ...
        uint256 rBNBrFee = tBNBrFee.mul(currentRate);
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rBNBrFee).sub(rBurn).sub(rLiquidity);
        return (rAmount, rTransferAmount, rBNBrFee);
    }
    ...
    function _transferFromExcluded(
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rBNBrFee, uint256 tTransferAmount, uint256 tBNBrFee, uint256 tBurn, uint256 tLiquidity) =
            _getValues(tAmount);
