...
import "./IShinobiGame.sol";
import "./IBank.sol";
import "./ITraits.sol";
import "./IChakra.sol";
import "./ISeed.sol";

contract ShinobiGame2 is IShinobiGame, ERC721Enumerable, Ownable, Pausable {
  // mint price
  //uint256 public MINT_PRICE = 29 ether;
  uint256 public MINT_PRICE = 29 ether;
  uint256 public PRESALE_MINT_PRICE = 20 ether;
  ...
  uint8 public phase = 1; // 1 => presale | 2 => public, in addition of Pausable


  mapping(address => bool) public whitelist;
  mapping(address => uint) public giftedMints;
  // mapping from tokenId to a struct containing the token's traits
  mapping(uint256 => ShinobiGameTraits) public tokenTraits;
  // mapping from hashed(tokenTrait) to the tokenId it's associated with
  // used to ensure there are no duplicates
  mapping(uint256 => uint256) public existingCombinations;
  // reference to the Bank for choosing random scammer
  IBank public bank;
  // reference to $CHAKRA for burning on mint
  ICHAKRA public chakra;
  // reference to Traits
  ...
  constructor(
    address _chakra,
    address _traits
  ) ERC721("Shinobi Game", "SGAME") {
    chakra = ICHAKRA(_chakra);
  ...
  /**
   * mint a token - 90% Shinobi, 10% Ronins
   * The first 20% are free to claim, the remaining cost $CHAKRA
   */
    ...
    whenNotPaused
  {
	...
	require(phase == 2, "Public mint not started");
  ...
  function presale_mint(uint256 amount)
    external
    payable
    nonReentrant
    whenNotPaused
  {
	...
	require(minted + amount <= MAX_TOKENS, "All tokens minted");
	require(amount > 0 && amount <= 30, "Invalid mint amount");
	require(phase == 1 && whitelist[_msgSender()], "Not whitelisted"); 
	require((amount * PRESALE_MINT_PRICE) == msg.value, "Invalid payment amount");

   game_mint(amount);
  }

    function game_mint(uint256 amount) private {
        uint256 totalChakraCost = 0;
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            totalChakraCost += mintCost(minted);
            owners[i] = recipient;
        }

        if (totalChakraCost > 0) chakra.burn(_msgSender(), totalChakraCost);
    ...
    function gifted_mint(uint256 amount) external nonReentrant whenNotPaused {
        require(tx.origin == _msgSender(), "Only EOA");
        require(minted + amount <= MAX_TOKENS, "All tokens minted");
        require(giftedMints[_msgSender()] >= amount, "Not enough free mints"); 
		giftedMints[_msgSender()] -= amount;
        address[] memory owners = new address[](amount);
        uint256 seed;
        uint256 firstMinted = minted;
  ...
  /**
   * the first 20% are paid in FTM
   * the next 20% are 20000 $CHAKRA
   * the next 40% are 40000 $CHAKRA
   * the final 20% are 80000 $CHAKRA
  ...
  function generate(uint256 tokenId, uint256 seed)
    internal
    returns (ShinobiGameTraits memory t)
  {
    t = selectTraits(seed);
  ...
  /**
   * the first 20% (FTM purchases) go to the minter
   * the remaining 80% have a 10% chance to be given to a random staked Ronin
  ...
  function selectRecipient(uint256 seed) internal view returns (address) {
    ...
    address ronin = bank.randomRoninTheft(seed >> 144);
   ...
   */
  function structToHash(ShinobiGameTraits memory s) internal pure returns (uint256) {
    return
      uint256(
        keccak256(
          abi.encodePacked(
  ...
  function random(uint256 seed) internal view returns (uint256) {
	...
	function setGiftedMints(address[] memory _gifted, uint[] memory _amounts) external onlyOwner {
        for (uint256 i = 0; i < _gifted.length; i++) {
			giftedMints[_gifted[i]] += _amounts[i];
        }
    }
  ...
  function getTokenTraits(uint256 tokenId)
    ...
    view
    override
    returns (ShinobiGameTraits memory)
  {
  ...
  function setPaidTokens(uint256 _paidTokens) external onlyOwner {
	...
	function getGiftedMints(address account) view external returns(uint) {
    	return giftedMints[account];
  	}
  /**
   * updates the sale phase
   */
...
import "./ShinobiGame.sol";

interface IShinobiGame {
  // struct to store each token's traits
  struct ShinobiGameTraits {
    uint8 classType;
    uint8 uniform;
    uint8 hair;
    uint8 eyes;
...
interface IBank {
  ...
  function randomRoninTheft(uint256 seed) external view returns (address);
  ...
  function totalChakraEarned() external view returns (uint256);
...
contract ShinobiGame is IShinobiGame, ERC721Enumerable, Ownable, Pausable {
  // mint price
  //uint256 public MINT_PRICE = 29 ether;
  uint256 public MINT_PRICE = 29 ether;
  // max number of tokens that can be minted - 50000 in production
...
contract Bank is Ownable, IERC721Receiver, Pausable {
  ...
  ShinobiGame game;
  // reference to the $CHAKRA contract for minting $CHAKRA earnings
  CHAKRA chakra;
  ...
  // amount of $CHAKRA due for each alpha point staked
  uint256 public chakraPerAlpha = 0;

  // Shinobis earn DAILY_CHAKRA_RATE $CHAKRA per day
  uint256 public DAILY_CHAKRA_RATE = 2500 ether;
  // Shinobis earn DAILY_CHAKRA_RATE_SENSEI $CHAKRA per day
  uint256 public DAILY_CHAKRA_RATE_SENSEI = 3500 ether;
  // Ronins must have 2 days worth of $CHAKRA to unstake or else it's too soon
  uint256 public MINIMUM_TO_EXIT = 2 days;
  // Ronins take a 20% tax on all $CHAKRA claimed
  uint256 public constant CHAKRA_CLAIM_TAX_PERCENTAGE = 20;
  // there will only ever be (roughly) 2.4 billion $CHAKRA earned through staking
  uint256 public constant MAXIMUM_GLOBAL_chakra = 1000000000 ether;

  // amount of $CHAKRA earned so far
  uint256 public totalChakraEarned;
  // number of Thief staked in the Bank
  uint256 public totalShinobiStaked;
  // the last time $CHAKRA was claimed
  uint256 public lastClaimTimestamp;

  // emergency rescue to allow unstaking without any checks but without $CHAKRA
  ...
  /**
   * @param _game reference to the ShinobiGame NFT contract
   * @param _chakra reference to the $Chakra token
   */
  constructor(
    address _game,
    address _chakra
  ) {
    game = ShinobiGame(_game);
    chakra = CHAKRA(_chakra);
    _pause();
  }
    ...
    nonReentrant
  {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      ...
      if (_msgSender() != address(game)) {
        ...
        require(game.ownerOf(tokenIds[i]) == _msgSender(), "Token ownership issue");
        game.transferFrom(_msgSender(), address(this), tokenIds[i]);
      }

      if (isThief(tokenIds[i])) _addThiefToBank(account, tokenIds[i]);
      if (getClass(tokenIds[i]) == 20) _addSenseiToBank(account, tokenIds[i]);
  ...
  function _addThiefToBankWithTime(
    ...
    totalChakraEarned +=
      ((time - lastClaimTimestamp) * totalShinobiStaked * DAILY_CHAKRA_RATE) /
      1 days;
  ...
  function _addSenseiToBank(address account, uint256 tokenId)
    internal
    whenNotPaused
    _updateEarnings
  {
  ...
  function _addPoliceToPack(address account, uint256 tokenId) internal {
    ...
    pack[alpha].push(
      Stake({
        owner: account,
        tokenId: uint16(tokenId),
        value: uint80(chakraPerAlpha)
      })
    );
    // Add the police to the Pack
    emit TokenStaked(account, tokenId, chakraPerAlpha);
  }

  /***CLAIMING / UNSTAKING */

  /**
   * realize $CHAKRA earnings and optionally unstake tokens from the Bank / Pack
   * to unstake a Ronin it will require it has 2 days worth of $CHAKRA unclaimed
    ...
    _updateEarnings
  {
    ...
    uint256 owed = 0;
    for (uint256 i = 0; i < tokenIds.length; i++) {
      if (getClass(tokenIds[i]) == 0) {
        owed += _claimThiefFromBank(tokenIds[i], unstake);
      } else if(getClass(tokenIds[i]) == 20) {
        owed += _claimSenseiFromBank(tokenIds[i], unstake);
      } else {
        owed += _claimPoliceFromPack(tokenIds[i], unstake);
      }
    }
    if (owed == 0) return;
    chakra.mint(_msgSender(), owed);
  }

  function estimatedRevenuesOf(uint16[] calldata tokenIds) view external returns(uint) {
    uint owed = 0;
    for (uint256 i = 0; i < tokenIds.length; i++) {
      if (isThief(tokenIds[i]) || getClass(tokenIds[i]) == 20) {
        Stake memory stake = bank[tokenIds[i]];
        uint newOwed = 0;
        uint256 chakraRate;
        chakraRate = getClass(tokenIds[i]) == 20 ? DAILY_CHAKRA_RATE_SENSEI : DAILY_CHAKRA_RATE;
        if (totalChakraEarned < MAXIMUM_GLOBAL_chakra) {
          newOwed = ((block.timestamp - stake.value) * chakraRate) / 1 days;
        } else if (stake.value > lastClaimTimestamp) {
          newOwed = 0;
        } else {
          newOwed = ((lastClaimTimestamp - stake.value) * chakraRate) / 1 days;
        }
        owed += (newOwed * (100 - CHAKRA_CLAIM_TAX_PERCENTAGE)) / 100;
      } else {
        uint256 alpha = _alphaForPolice(tokenIds[i]);
        Stake memory stake = pack[alpha][packIndices[tokenIds[i]]];
        owed += (alpha) * (chakraPerAlpha - stake.value);
      }
    }
    return owed;
  }
  ...
  /**
   * realize $CHAKRA earnings for a single Thief and optionally unstake it
   * if not unstaking, pay a 20% tax to the staked Polices
   * if unstaking, there is a 50% chance all $CHAKRA is stolen
   * @param tokenId the ID of the Thief to claim earnings from
   * @param unstake whether or not to unstake the Thief
   * @return owed - the amount of $CHAKRA earned
   */
    ...
    returns (uint256 owed)
  {
    ...
    require(stake.owner == _msgSender(), "Must be owner to unstake");
    require(
      !(unstake && block.timestamp - stake.value < MINIMUM_TO_EXIT),
      "Must stay at least two days before unstaking"
    );
    if (totalChakraEarned < MAXIMUM_GLOBAL_chakra) {
      owed = ((block.timestamp - stake.value) * DAILY_CHAKRA_RATE) / 1 days;
    } else if (stake.value > lastClaimTimestamp) {
      owed = 0;
      // $CHAKRA production stopped already
    } else {
      owed = ((lastClaimTimestamp - stake.value) * DAILY_CHAKRA_RATE) / 1 days;
      // stop earning additional $CHAKRA if it's all been earned
    }
    if (unstake) {
      ...
      _payPoliceTax((owed * CHAKRA_CLAIM_TAX_PERCENTAGE) / 100);
      // percentage tax to staked wolves
      owed = (owed * (100 - CHAKRA_CLAIM_TAX_PERCENTAGE)) / 100;
  ...
  function _claimSenseiFromBank(uint256 tokenId, bool unstake)
    internal
    returns (uint256 owed)
  {
    Stake memory stake = bank[tokenId];
    ...
    if(unstake) {
      ...
      // Everything goes to our Senseis
      bank[tokenId] = Stake({
        owner: _msgSender(),
        tokenId: uint16(tokenId),
        value: uint80(block.timestamp)
  ...
  /**
   * realize $CHAKRA earnings for a single Police and optionally unstake it
   * Ronins earn $CHAKRA proportional to their Alpha rank
  ...
  /**
   * add $CHAKRA to claimable pot for the Pack
   * @param amount $CHAKRA to add to the pot
   */
  function _payPoliceTax(uint256 amount) internal {
    ...
    chakraPerAlpha += (amount + unaccountedRewards) / totalAlphaStaked;
    unaccountedRewards = 0;
  }

  /**
   * tracks $CHAKRA earnings to ensure it stops once 2.4 billion is eclipsed
   */
   ...
   * @return isShinobi - whether or not a token is a Thief
   */
  function isThief(uint256 tokenId) public view returns (bool isShinobi) {
    uint8 thief;
    (thief, , , , , , , , ) = game.tokenTraits(tokenId);
    isShinobi = (thief == 0) ? true : false;
  }

  function getClass(uint256 tokenId) public view returns (uint8 charClass) {
    (charClass, , , , , , , , ) = game.tokenTraits(tokenId);
  }
...
contract CHAKRA is ERC20, Ownable {
  ...
  constructor() ERC20("CHAKRA", "CHKRA") {
    _mint(msg.sender, 120000*10**18);
  }

  /**
   * mints $CHAKRA to a recipient
   * @param to the recipient of the $CHAKRA
   * @param amount the amount of $CHAKRA to mint
   */
  ...
  /**
   * burns $CHAKRA from a holder
   * @param from the holder of the $CHAKRA
   * @param amount the amount of $CHAKRA to burn
   */
  function burn(address from, uint256 amount) external {
