...
contract LP {
    ...
    uint public constant minDeposit = 1000000000000000000;
    ...
    function withdraw(uint256 amount) external paused() {
        ...
        totalLP = totalLP.sub(postTaxedAmount);
    ...
    // in case there is an issue and we need to relaunch the gRoyToken during the LP game
    function updateGroyToken(address tokenAddress) external onlyOwner() paused() {
            _gRoyToken = tokenAddress;
        }
    ...
    // return a 4 decimal percentage by comparing their balance with all LP
    function percentageOfPool(address wallet) view public returns(uint256) {
        if (balances[wallet] == 0) {
            return 0;
        }
        uint256 stakedAmount = balances[wallet];
        return stakedAmount.mul(10000).div(totalLP);
    }
}