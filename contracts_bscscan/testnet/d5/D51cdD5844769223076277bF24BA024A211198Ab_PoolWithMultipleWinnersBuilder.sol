...
import "../prize-pool/bunny/BunnyPrizePool.sol";
import "../prize-pool/bunny/BunnyPrizePoolProxyFactory.sol";
import "../external/pancake/IMasterChef.sol";
import "../prize-pool/cake/CakePrizePool.sol";
import "../prize-pool/cake/CakePrizePoolProxyFactory.sol";
...
contract PoolWithMultipleWinnersBuilder {
  ...
  event CakePrizePoolWithMultipleWinnersCreated(
    CakePrizePool indexed prizePool,
    MultipleWinners indexed prizeStrategy
  );

  event BunnyPrizePoolWithMultipleWinnersCreated(
    BunnyPrizePool indexed prizePool,
    MultipleWinners indexed prizeStrategy
  );
  ...
  struct BunnyPrizePoolConfig {
    BunnyPoolInterface vToken;
    uint256 maxExitFeeMantissa;
    uint256 maxTimelockDuration;
  }

  struct CakePrizePoolConfig {
    IMasterChef vToken;
    uint256 maxExitFeeMantissa;
    uint256 maxTimelockDuration;
  }
  ...
  BunnyPrizePoolProxyFactory public bunnyPrizePoolProxyFactory;
  CakePrizePoolProxyFactory public cakePrizePoolProxyFactory;
  ...
  constructor (
    ...
    BunnyPrizePoolProxyFactory _bunnyPrizePoolProxyFactory,
    CakePrizePoolProxyFactory _cakePrizePoolProxyFactory,
    ...
    require(address(_bunnyPrizePoolProxyFactory) != address(0), "GlobalBuilder/bunnyPrizePoolProxyFactory-not-zero");
    require(address(_cakePrizePoolProxyFactory) != address(0), "GlobalBuilder/cakePrizePoolProxyFactory-not-zero");
  ...
  function createBunnyMultipleWinners(
    BunnyPrizePoolConfig memory prizePoolConfig,
    MultipleWinnersBuilder.MultipleWinnersConfig memory prizeStrategyConfig,
    uint8 decimals
  ) external returns (BunnyPrizePool) {
    BunnyPrizePool prizePool = bunnyPrizePoolProxyFactory.create();
    MultipleWinners prizeStrategy = multipleWinnersBuilder.createMultipleWinners(
      prizePool,
      prizeStrategyConfig,
      decimals,
    ...
    prizePool.initialize(
      ...
      prizePoolConfig.maxExitFeeMantissa,
      prizePoolConfig.maxTimelockDuration,
      BunnyPoolInterface(prizePoolConfig.vToken)
    );
    prizePool.setPrizeStrategy(prizeStrategy);
    prizePool.setCreditPlanOf(
      address(prizeStrategy.ticket()),
    ...
    prizePool.transferOwnership(msg.sender);
    emit BunnyPrizePoolWithMultipleWinnersCreated(prizePool, prizeStrategy);
    return prizePool;
  }

  function createCakeMultipleWinners(
    CakePrizePoolConfig memory prizePoolConfig,
    MultipleWinnersBuilder.MultipleWinnersConfig memory prizeStrategyConfig,
    uint8 decimals
  ) external returns (CakePrizePool) {
    CakePrizePool prizePool = cakePrizePoolProxyFactory.create();
    MultipleWinners prizeStrategy = multipleWinnersBuilder.createMultipleWinners(
      prizePool,
      prizeStrategyConfig,
      decimals,
    ...
    prizePool.initialize(
      ...
      prizePoolConfig.maxExitFeeMantissa,
      prizePoolConfig.maxTimelockDuration,
      IMasterChef(prizePoolConfig.vToken)
    );
    prizePool.setPrizeStrategy(prizeStrategy);
    prizePool.setCreditPlanOf(
      address(prizeStrategy.ticket()),
    ...
    prizePool.transferOwnership(msg.sender);
    emit CakePrizePoolWithMultipleWinnersCreated(prizePool, prizeStrategy);
    return prizePool;
  }
...
/* solium-disable security/no-block-members */
contract CakePrizePoolHarness is CakePrizePool {
...
import "./CakePrizePoolHarness.sol";
import "../external/openzeppelin/ProxyFactory.sol";

/// @title Cake Prize Pool Proxy Factory
/// @notice Minimal proxy pattern for creating new Cake Prize Pools
contract CakePrizePoolHarnessProxyFactory is ProxyFactory {

  /// @notice Contract template for deploying proxied Prize Pools
  CakePrizePoolHarness public instance;

  /// @notice Initializes the Factory with an instance of the Cake Prize Pool
  constructor () public {
    instance = new CakePrizePoolHarness();
  }

  /// @notice Creates a new Cake Prize Pool as a proxy of the template instance
  /// @return A reference to the new proxied Cake Prize Pool
  function create() external returns (CakePrizePoolHarness) {
    return CakePrizePoolHarness(deployMinimal(address(instance), ""));
  }
}
...
/* solium-disable security/no-block-members */
contract BunnyPrizePoolHarness is BunnyPrizePool {
...
import "./BunnyPrizePoolHarness.sol";
import "../external/openzeppelin/ProxyFactory.sol";

/// @title Venus Prize Pool Proxy Factory
/// @notice Minimal proxy pattern for creating new Venus Prize Pools
contract BunnyPrizePoolHarnessProxyFactory is ProxyFactory {

  /// @notice Contract template for deploying proxied Prize Pools
  BunnyPrizePoolHarness public instance;

  /// @notice Initializes the Factory with an instance of the Venus Prize Pool
  constructor () public {
    instance = new BunnyPrizePoolHarness();
  }

  /// @notice Creates a new Venus Prize Pool as a proxy of the template instance
  /// @return A reference to the new proxied Venus Prize Pool
  function create() external returns (BunnyPrizePoolHarness) {
    return BunnyPrizePoolHarness(deployMinimal(address(instance), ""));
  }
}
