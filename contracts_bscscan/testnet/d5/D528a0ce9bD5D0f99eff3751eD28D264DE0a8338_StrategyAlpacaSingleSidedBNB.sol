...
import "../interfaces/AlpacaPancakeFarm/IStrategy.sol";
...
contract StrategyAlpacaSingleSidedBNB is
    Initializable,
    ...
    CoreRefUpgradeable
{
    ...
    function init(
        ...
        IERC20Upgradeable(vaultAddress).safeApprove(fairLaunchAddress, uint256(-1));
    }

    function calculateMinLP(uint256 _amountWant) public view returns (uint256 minLP) {
        address[] memory path = new address[](2);
        path[0] = wbnbAddress;
        path[1] = farmTokenAddress;
        ...
        require(amountIn <= _amountWant, "Not enough tokens");
        uint256 amountOut = IPancakeRouter02(uniRouterAddress).getAmountsOut(amountIn, path)[1];

        uint256 amountWantToInvest = _amountWant.sub(amountIn);
        uint256 totalSupply = _getLPTotalSupply(path[0], path[1]);
        minLP = MathUpgradeable.min(amountWantToInvest.mul(totalSupply) / rWant, amountOut.mul(totalSupply) / rFarm);
    }
    ...
    function _getPairReserves(address token0, address token1) internal view returns (uint256 rWant, uint256 rFarm) {
        ...
        rWant = lptoken.token0() == wbnbAddress ? r0 : r1;
    ...
    function deposit(uint256 minLPAmount) public payable onlyMultistrategy nonReentrant whenNotPaused {
        require(msg.value > 0, "WTF Strategy AlpacaPancakeBNB:: BNB amount is zero");
        _deposit(msg.value, minLPAmount);
    }

    function _deposit(uint256 _wantAmt, uint256 minLPAmount) internal {
        // Get Alpaca Farm position
        if (vaultPositionId != 0) {
            vaultPositionId = Vault(vaultAddress).nextPositionID();
        }
        bytes memory ext = abi.encode(uint256(minLPAmount));
        bytes memory data = abi.encode(strategyAddAllBaseToken, ext);
        Vault(vaultAddress).work{value: msg.value}(vaultPositionId, worker, _wantAmt, 0, 0, data);
    }

    function calculateMinBaseToken() public view returns (uint256 minBaseToken) {
    ...
    function _liquidate(uint256 minBaseToken) internal {
        ...
        Vault(vaultAddress).work(vaultPositionId, worker, 0, 0, uint256(-1), data); // check this
    }
    ...
    function withdraw(uint256 minBaseToken) public onlyMultistrategy nonReentrant {
        ...
        if (alpacaAddress != wbnbAddress && earnedAmt != 0) {
            IPancakeRouter02(uniRouterAddress).swapExactTokensForETH(
                earnedAmt,
                1,
                earnedToWantPath,
                address(this),
    ...
    function _pause() internal override {
        ...
        IERC20Upgradeable(vaultAddress).safeApprove(fairLaunchAddress, 0);
    }
...
interface IStrategy {
    ...
    function deposit(uint256 minLPAmount) external payable;
