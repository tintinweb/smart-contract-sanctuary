...
contract XPSToken is Context, IERC20, Ownable {
    ...
    uint256 public _liquidityTriggerAmount = 5 * 10 ** 9; // = 5,000 tokens
    uint256 public _marketingTriggerAmount = 5 * 10 ** 9; // = 5,000 tokens
    uint256 public _communityTriggerAmount = 5 * 10 ** 9; // = 5,000 tokens
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(!isExcludedFromFee(_msgSender())) {
            ...
            burnAmount = (feeAmount.mul(_burnFee)).div(100);
        ...
        _balances[recipient] = _balances[recipient].add(totalSendAmount.add(burnAmount));
        // Burn
        _burn(address(recipient), burnAmount);
