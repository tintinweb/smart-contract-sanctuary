    ...
    // UUPSUpgradeable
{
        ...
        internal whenNotPaused override
    {
        ...
        if (to.isContract()) {
            if (!isOwner(from) && (to == dexRouter || to == dexPair)) { 
                transferAmount -= amount * feeTrade / DENOMINATOR;
                _transfer(from, address(this), amount - transferAmount, TransferMethods.NONE);
            }
        }

        super._beforeTokenTransfer(from, to, transferAmount, method);
    }
}