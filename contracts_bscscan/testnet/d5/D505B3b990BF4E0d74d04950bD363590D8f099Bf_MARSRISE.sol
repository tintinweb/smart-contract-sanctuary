...
contract MARSRISE is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using EnumerableSet for EnumerableSet.AddressSet;
    
    address payable public marketingAddress = payable(0xf6c201637FcebF121A69e3311aE3b6745A232CF8); // Marketing Address
    ...
    string private _name = "MarsRise";
    string private _symbol = "MARSRISE";
    ...
    function _getCurrentSupply() private view returns(uint256, uint256) {
        ...
        for (uint256 i = 0; i < _isExcluded.length(); i++) {
            if (_rOwned[_isExcluded.at(i)] > rSupply || _tOwned[_isExcluded.at(i)] > tSupply) return (_rTotal, _tTotal);
            rSupply = rSupply.sub(_rOwned[_isExcluded.at(i)]);
            tSupply = tSupply.sub(_tOwned[_isExcluded.at(i)]);
        }
