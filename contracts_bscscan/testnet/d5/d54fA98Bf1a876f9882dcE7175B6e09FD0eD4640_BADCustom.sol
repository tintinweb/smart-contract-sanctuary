...
contract BADCustom is Ownable{
    ...
    event Custom(address indexed player, uint indexed cardId, uint indexed tokenId, uint coreNo);
    ...
    event Debug(bytes32 hash, address banker);
    function custom(address player_, uint cardId_, uint tokenId_,  uint coreNo_, bytes32 r, bytes32 s, uint8 v) public returns (bool) {
        bytes32 hash =  keccak256(abi.encodePacked(_msgSender(), cardId_, tokenId_, coreNo_));
