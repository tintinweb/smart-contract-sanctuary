...
contract IGO is Auth, PausableUpgradeable {
  ...
  event Claimed(address indexed user, uint id, IMarketPlace.PaymentCurrency paymentCurrency, uint[] nftClassIds);
  ...
  function claim(uint _id, uint _index, bytes32[] calldata _path, IMarketPlace.PaymentCurrency _paymentCurrency) external whenNotPaused {
    ...
    _takeFund(_id, ids, _paymentCurrency);
    uint[] memory nftClassIds = _mintNFT(_id, ids);
    emit Claimed(msg.sender, _id, _paymentCurrency, nftClassIds);
  }
  ...
  function _takeFund(uint _id, uint16[] memory _ids, IMarketPlace.PaymentCurrency _paymentCurrency) private {
    uint amount;
  ...
  function _takeFundFOTA(uint _amount) private {
    uint _fotaAmount = _amount * decimal3 / fotaPricer.fotaPrice();
    require(fotaToken.allowance(msg.sender, address(this)) >= _fotaAmount, "IGO: please approve fota first");
    require(fotaToken.balanceOf(msg.sender) >= _fotaAmount, "IGO: insufficient balance");
    require(fotaToken.transferFrom(msg.sender, address(this), _fotaAmount), "IGO: transfer fota failed");
  ...
  function _mintNFT(uint _id, uint16[] memory _ids) private returns (uint[] memory){
