...
contract CCL is Context, Ownable, ReentrancyGuard {
 ...
 //  uint256 private BNBCost; // Fees in BNB
    ...
    function newRequest (string memory _fullLegalName, string memory _email, bool payLTS, bool isContract, address _contract, string memory _Chain) external LTSHOLDER() returns (bool) {
    ...
    function _isSusAddress(address _addr) external view returns (bool) {
        require(ltsHolderAddress); 
        return isSuspected[_addr]; } // RETURNS SUSPECTED ADDRESSES/NEWLY DEPLOYED CONTRACTS
