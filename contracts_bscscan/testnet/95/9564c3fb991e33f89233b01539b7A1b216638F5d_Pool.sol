...
contract Pool is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(
            address(_rewardsPoolAddress) != address(0),
            "Token is a zero value"
        );
    ...
    function setDepositFee(uint256 _pid, uint16 _depositFee)
        external
        onlyOwner
    {
        massUpdatePools();
    ...
    function setLockupDuration(uint256 _pid, uint256 _lockupDuration)
        external
        onlyOwner
    {
        massUpdatePools();
    ...
    function pendingCWolf(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && depositedAmount != 0) {
            ...
            uint256 cWolfReward = multiplier
                ...
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accCWolfPerShare = accCWolfPerShare.add(
                cWolfReward.mul(1e12).div(depositedAmount)
            );
        }
        return
            user
                .amount
                .mul(accCWolfPerShare)
                .div(1e12)
                .sub(user.rewardDebt)
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(
            block.timestamp > (user.lastWithdraw + pool.lockupDuration),
            "You cannot withdraw yet!"
        );
        require(user.amount >= _amount, "withdraw: not good");

        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accCWolfPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            user.pendingRewards = user.pendingRewards.add(pending);
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBal = IERC20(cWOLFContractAddress).balanceOf(
    ...
    function setCWolfPerBlock(uint256 _cWolfPerBlock) external onlyOwner {
        require(
            _cWolfPerBlock > 0,
            "Token per block must be greather than zero"
        );
        massUpdatePools();
        cWolfPerBlock = _cWolfPerBlock;
    }
