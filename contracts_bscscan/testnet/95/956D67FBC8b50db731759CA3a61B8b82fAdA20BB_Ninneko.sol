...
contract Ninneko is Initializable, ERC721Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function setAllowBreed(bool _allowBreed) external onlyOwner {
        allowBreed = _allowBreed;
    }

    function setTimeBreedAfter(uint256 _timer) external onlyOwner {
        adulthoodTime = _timer;
    }

    function breed(uint256 _pet1Id, uint256 _pet2Id) external validateBreed(_pet1Id, _pet2Id) {
        ...
        NINOToken.safeTransferFrom(msg.sender, owner(), breedNINOFee);
        MATAToken.safeTransferFrom(msg.sender, owner(), breedMATAFee);
