...
contract LotteryToken is Context, IERC20, Ownable {
    ...
    uint256 public timeWeek = 20 * 60; //60 min
    uint256 public timeMonth = 30 * 60; //3 hour
    ...
    function _transfer(
        ...
        if(!inSwapAndLiquify ){
            if (lotteryStarted && block.timestamp.div(timeDay) > checkpointTier3  && dailyGatherAwardInTokenTier3 > 0){
    ...
    function sendLotteryAward(address winner, uint256 tokenAmount, uint tier) private{
        swapETHForTokensAndBurn(tokenAmount.mul(10).div(100));
        investIntoCakePool(tokenAmount.mul(10).div(100));
    ...
    function setTier1Threshold(uint256 _tier1Threshold) public onlyOwner {
            tier1Threshold = _tier1Threshold * 10**9;
    }

    function setTier2Threshold(uint256 _tier2Threshold) public onlyOwner {
            tier2Threshold = _tier2Threshold * 10**9;
    }

    function setTier3Threshold(uint256 _tier3Threshold) public onlyOwner {
            tier3Threshold = _tier3Threshold * 10**9;
    }
}