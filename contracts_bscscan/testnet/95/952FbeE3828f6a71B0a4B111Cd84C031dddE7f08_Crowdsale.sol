...
contract Crowdsale is Ownable {
        ...
        returns (bool) 
    {
        ...
        //require(IBEP20(tokenContract).approve(msg.sender, _quantity));
        require(IBEP20(tokenContract).transferFrom(msg.sender, address(this), _quantity));
        // require(IBEP20(tokenContract).transfer(address(this), _quantity));
        if(salesIndex != 0) {
            Sale storage previousSale = sales[salesIndex];
            require(block.timestamp > previousSale.endDate, "Previous sale is not closed yet.");
            return false;
        }
    ...
    {
        // TODO: add requirements
        Sale storage c = sales[_saleIndex];
