...
contract ShibaGalaxy is Context, IERC20, Ownable {
    ...
    function collectFee(address account, address to, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(buyBackFee != 0){
            ...
            _reflectionBalance[buybackWallet] = _reflectionBalance[buybackWallet].add(_buyBackFee.mul(rate));
        ...
        if(vaultFee != 0 && to == uniswapV2Pair){
            ...
            _reflectionBalance[vaultWallet] = _reflectionBalance[vaultWallet].add(_vaultFee.mul(rate));
            if (_isExcluded[vaultWallet]) {
                _tokenBalance[vaultWallet] = _tokenBalance[vaultWallet].add(_vaultFee);
            }
    ...
    function vaultSwap(uint256 swapAmount) private lockTheSwap {
        if(swapAmount > maxSwapAmount)
            swapAmount = maxSwapAmount;
        swapTokensForEth(swapAmount, bnbVaultWallet);
    }
    ...
    function setVaultWallet(address wallet) external onlyOwnerAndAdmin {
        vaultWallet = wallet;
    }
