...
contract Charge40Strategy is StratManager {
	...
	constructor(
		...
		dollar = dollar_;
		wrapped = wrapped_;
		dollarToWrappedRoute = [dollar, wrapped];
		dollarToShareRoute = [dollar, wrapped, share];
		wrappedToTshareRoute = [wrapped, share];
	...
	function deposit() public whenNotPaused {
		if (initiateTimelock) {
			...
			addLiquidity();
			uint256 wantBal = IERC20(share).balanceOf(address(this));
	...
	function chargeFees() internal {
		uint256 toWrapped = IERC20(dollar)
	...
	function set60Strategy(address strat) external {
	...
	function _giveAllowances() internal {
		IERC20(share).safeApprove(boardroom, type(uint256).max);
