...
contract Lions is ERC721Namable, Ownable {
	...
	constructor(string memory _name, string memory _symbol, address _yieldToken) ERC721Namable(_name, _symbol) {
		_setBaseURI("https://lions.youaremetaverse.com/lion/");
    ...
    function init() public onlyOwner{
        ...
        _mint(0x1D23a28C71b5daC1b0F3Af101666d7184b299032, 228);
        _mint(0x1D23a28C71b5daC1b0F3Af101666d7184b299032, 322);
        _mint(0x1D23a28C71b5daC1b0F3Af101666d7184b299032, 777);
        _mint(0x1D23a28C71b5daC1b0F3Af101666d7184b299032, 888);
        _mint(0x3cD51329FfFC6D433c28B4126933a284A5E5D8a1, 666);

        yieldToken.updateRewardOnMint(0x1D23a28C71b5daC1b0F3Af101666d7184b299032, 4);
        yieldToken.updateRewardOnMint(0x3cD51329FfFC6D433c28B4126933a284A5E5D8a1, 1);
    }

    function mint(uint256 _id) public payable {
        lionsCount < 500 ? require(PRESALE_PRICE == msg.value, "Value isn't correct") :
        require(PRICE == msg.value, "Value isn't correct");
    ...
    function mintTokens(uint256[] memory _ids) public payable {
...
contract YieldToken is ERC20("SexyToken", "SEXY") {
	...
	constructor() {
        ...
        _mint(owner, 25000000000000000000000000);
	}
	...
	function updateReward(address _from, address _to) external isValidAddress isContract{
        address _contract = msg.sender;
        updateReward(_contract, _from, _to);
    }

    function updateReward(address _contract, address _from, address _to) internal {
        uint256 _rewardTime = rewardsEnd[_contract];

        uint256 _time = min(block.timestamp, _rewardTime);
        uint256 _timerFrom = lastUpdates[_contract][_from];
        if(_rewardTime >= _timerFrom) {
            if (_timerFrom > 0)
                rewards[_contract][_from] += ILions(_contract).balanceOG(_from).mul(BASE_RATE.mul(_time.sub(_timerFrom))).div(REWARD_SEPARATOR);
            if (_timerFrom != _rewardTime)
    ...
    function testPending(address _contract, address _user) public view returns(uint256) {
        uint256 _rewardTime = rewardsEnd[_contract];

        uint256 _time = min(block.timestamp, _rewardTime);

        return ILions(msg.sender).balanceOG(_user).mul(BASE_RATE.mul((_time.sub(lastUpdates[_contract][_user])))).div(REWARD_SEPARATOR);
    }

    function testReward(address _contract, address _user) public view returns(uint256) {
        uint256 _rewardTime = rewardsEnd[_contract];

        uint256 _time = min(block.timestamp, _rewardTime);
        uint256 timerFrom = lastUpdates[_contract][_user];
            if (timerFrom > 0)
                return ILions(_contract).balanceOG(_user).mul(BASE_RATE.mul(_time.sub(timerFrom))).div(REWARD_SEPARATOR);

        return 0;        
    }

    function testRewardTime(address _contract) public view returns(uint256) {
        return rewardsEnd[_contract];
    }

    function testMinTime(address _contract) public view returns(uint256) {
        uint256 _rewardTime = rewardsEnd[_contract];

        return min(block.timestamp, _rewardTime);
    }

    function testSubTime(address _contract, address _user) public view returns(uint256) {
        uint256 _rewardTime = rewardsEnd[_contract];

        uint256 _time = min(block.timestamp, _rewardTime);
        uint256 _timerFrom = lastUpdates[_contract][_user];

        return _time.sub(_timerFrom);
    }

    function testLastUpdates(address _contract, address _user) public view returns(uint256) {
        return lastUpdates[_contract][_user];
    }
    ...
    function getAllRewards() public {
        for(uint i=0; i<contracts.length; i++) {
            updateReward(contracts[i], msg.sender, address(0));
