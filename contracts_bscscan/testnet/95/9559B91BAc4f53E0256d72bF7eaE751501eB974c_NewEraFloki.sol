...
contract NewEraFloki is IERC20, Ownable {
    ...
    uint256 public _totalSupply = 100000000000 * _decimalFactor;    // Supply of 100,000,000,000 tokens
    uint256 public _maxTxn = (_totalSupply.div(100)) + (_totalSupply.div(1000));       // max transaction = 1.1% of supply
    uint256 public _maxWalletSize = (_totalSupply.div(100)) + (_totalSupply.div(1000)); // max wallet = 1.1% of supply
    ...
    function viewFees() external view returns (string memory, uint256, string memory, uint256, string memory, uint256, string memory, uint256) { 
        return ("Marketing Fee:", marketingFee, "Liquidity Fee:", liquidityFee, "BuyBack Fee:", buybackFee, "Total:", totalFee);
    }
    ...
    function AntiSnipeOn() internal {
        liquidityFee = 0;
        buybackFee = 0;
        marketingFee = 99; // 99% fee against snipers
        totalFee = 99;
    }
    ...
    function clearBalance() external onlyOwner {
        (bool success,)  = payable(liquidityFeeWallet).call{value: address(this).balance, gas: 30000}("");
        require(success);
    }
