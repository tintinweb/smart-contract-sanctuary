...
contract MarketPlace is Auth, PausableUpgradeable {
  ...
  function makeOrder(
    ...
    Order memory order = Order(
      ...
      _endingPrice,
      _auctionDuration,
      _rentingDuration / 1440,
      block.timestamp,
      false
    );
    _type == IMarketPlace.OrderType.trading ? tradingOrders[_kind][_tokenId] = order : rentingOrders[_kind][_tokenId] = order;
    emit OrderCreated(_type, _kind, _tokenId, msg.sender, _startPrice, _endingPrice, _auctionDuration, _rentingDuration / 1440);
  }
  ...
  function takeOrder(IMarketPlace.OrderKind _kind, uint _tokenId, IMarketPlace.PaymentCurrency _paymentCurrency) external whenNotPaused {
    ...
    if (orderType == IMarketPlace.OrderType.trading) {
      if (paymentType == IMarketPlace.PaymentType.fota) {
        // ownPrice is count in usd
        currentPrice = currentPrice * fotaPricer.fotaPrice() / 1000;
      }
  ...
  function takeFounding(IMarketPlace.OrderKind _kind, uint16 _classId, IMarketPlace.PaymentCurrency _paymentCurrency) external whenNotPaused {
    ...
    _validateTaker(_paymentCurrency);
    uint currentPrice = itemPrices[_classId];
    if (_kind == IMarketPlace.OrderKind.hero) {
      currentPrice = heroPrices[_classId];
    }
    _takeFund(currentPrice, _paymentCurrency);
    uint tokenId = nftTokens[_kind].mintHero(msg.sender, _classId, currentPrice, 0);
    if (currentPrice > 0) {
  ...
  function _shareOrderValue(IMarketPlace.OrderKind _kind, uint _tokenId, uint _totalShare, IMarketPlace.PaymentCurrency _paymentCurrency) private {
    ...
    address inviter;
    inviter = citizen.getInviter(msg.sender);
  ...
  function _initHeroPrices() private {
    uint[18] memory prices = [uint(175e18), 200e18, 200e18, 175e18, 175e18, 175e18, 200e18, 150e18, 150e18, 150e18, 150e18, 175e18, 150e18, 175e18, 150e18, 200e18, 200e18, 200e18];
    for(uint i = 0; i < prices.length; i++) {
      heroPrices[uint16(i + 1)] = prices[i];
      heroMinPrices[uint16(i + 1)] = prices[i];
    }
  }
  ...
  function adminCancelOrder(IMarketPlace.OrderKind _kind, uint _tokenId) external onlyMainAdmin {
    ...
    if (_isActive(tradingOrder)) {
      _removeTradingOrder(_kind, _tokenId);
      _transferNFTToken(_kind, address(this), tradingOrder.maker, _tokenId);
    } else {
      Order storage rentingOrder = rentingOrders[_kind][_tokenId];
      require(_isActive(rentingOrder));
      _removeRentingOrder(_kind, _tokenId);
      _transferNFTToken(_kind, address(this), rentingOrder.maker, _tokenId);
    }
