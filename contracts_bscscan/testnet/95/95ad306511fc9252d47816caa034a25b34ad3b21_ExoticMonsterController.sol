...
contract ExoticMonsterController is Ownable {
    ...
    event SelectMonster(uint256 monsterID,uint256 userHeroId,bool playResult,uint256 profit);
    ...
    //用户所有英雄
    mapping(address => mapping(uint => Hero)) public userHeros;
    
    //用户所有投资池
    mapping(address => mapping(uint => Pool)) public userPools;
    ...
    constructor() {
        ...
        //初始化 五种英雄
        //初始化 五种英雄
        heros[1] = Hero(0,0,1,2,0,0,0,0,0,0,0,false);
        heros[2] = Hero(0,0,2,3,0,0,0,0,0,0,0,false);
        heros[3] = Hero(0,0,3,4,0,0,0,0,0,0,0,false);
        heros[4] = Hero(0,0,4,5,0,0,0,0,0,0,0,false);
        heros[5] = Hero(0,0,5,6,0,0,0,0,0,0,0,false);
              
        //初始化 五种矿池
        pools[1] = Pool(0,1,0,0,0,0,0,5);
        pools[2] = Pool(0,2,0,0,0,1 weeks,0,58);
        pools[3] = Pool(0,3,0,0,0,2 weeks,0,163);
        pools[4] = Pool(0,4,0,0,0,30 days,0,400);
        pools[5] = Pool(0,5,0,0,0,90 days,0,1500); 
    ...
    function selectBox(uint256 boxId) public {
        ...
        if(2 == boxId && box2MaxTimes > 0){
            ...
            _addHeroToUser(msg.sender, 0);
            _addHeroToUser(msg.sender, 3300);
            _addHeroToUser(msg.sender, 6600);
        ...
        if(3 == boxId && box3MaxTimes > 0){
            ...
            _addHeroToUser(msg.sender, 0);
            _addHeroToUser(msg.sender, 2000);
            _addHeroToUser(msg.sender, 4000);
            _addHeroToUser(msg.sender, 6000);
            _addHeroToUser(msg.sender, 8000);
    ...
    function selectMonster(uint256 monsterID,uint256 userHeroId) public {
        ...
        if(userHero.usedTimes < userHero.totalTimes){
            ...
            //英雄对战累计收益
            userHero.monsterProfit = userHero.monsterProfit + price;
            ...
            emit SelectMonster(monsterID,userHeroId,playResult,profit);

        }
    }

    //用户获得盒子英雄
    function _addHeroToUser(address user , uint256 num) internal virtual{
    
        //随机获取等级英雄
       uint256 heroLevel  =  _getHero(num);
    ...
    function _getHero(uint256 num) internal virtual returns (uint256) {
    
        uint256 number =  (uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty,msg.sender))).add(num)) % 10000 ;
