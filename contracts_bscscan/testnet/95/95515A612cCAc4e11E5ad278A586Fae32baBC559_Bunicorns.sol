...
contract Bunicorns is Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    */
    ...
    event Enhanced(address indexed owner, uint256 indexed enhanced, uint256 indexed burned, uint8 lowPoints, uint8 fourPoints, uint8 fivePoints);
    ...
    function mint(address minter, uint256 seed) public restricted returns(uint256) {
        ...
        else if(roll < 375) { // 4* at 3.5%
            stars = 3;
        }
        else if(roll < 1725) { // 3* at 13.5%
            stars = 2;
        }
        else if(roll < 5000) { // 2* at 32.75%
            stars = 1;
        }
        else {
            stars = 0; // 1* at 50%
    ...
    ) public restricted noEmergency returns(uint256) {
    ...
    function enhance(uint256 enhanceID, uint256 burnID) public restricted {
        uint8[] memory values = _calculateBurnValues(burnID);

        // Note: preexisting issue of applying burn points even if _burn fails.
        _applyBurnPoints(enhanceID, values[0], values[1], values[2]);
        _burn(burnID);

        BunicornBurnPoints storage wbp = burnPoints[enhanceID];
        emit Enhanced(
            ownerOf(enhanceID),
            enhanceID,
            burnID,
    ...
    function _applyBurnPoints(uint256 enhanceID, uint8 amountLB, uint8 amount4B, uint8 amount5B) private {
        BunicornBurnPoints storage wbp = burnPoints[enhanceID];
    ...
    function getBonusPower(uint256 id) public view noFreshLookup(id) returns (uint24) {
        ...
        return getBonusPowerForBattle(id, bunicorn.level);
    }

    function getBonusPowerForBattle(uint256 id, uint8 level) public view noFreshLookup(id) returns (uint24) {
    ...
    function getBattleData(uint256 id, uint8 trainerTrait) public view noFreshLookup(id) returns (int128, int128, uint24, uint8) {
        Bunicorn storage bunicorn = tokens[id];
        return (
            ...
            getBonusPowerForBattle(id, bunicorn.level),
            getTraitFromProperties(bunicorn.properties)
        );
    }

    function getBattleDataAndDrainDurability(uint256 id, uint8 trainerTrait, uint8 drainAmount) public
    ...
    // =========================== MIGRATOR ===========================
    function getTotalBunicorns() external view onlyMigratorContract returns(uint256) {
        return tokens.length;
    }

    function tokenExists(uint256 _tokenId) external view onlyMigratorContract returns(bool) {
        return _exists(_tokenId);
    }

    function burnByMigrator(uint256 _tokenId) external onlyMigratorContract {
        _burn(_tokenId);
    }
    // =========================== MIGRATOR ===========================
}
