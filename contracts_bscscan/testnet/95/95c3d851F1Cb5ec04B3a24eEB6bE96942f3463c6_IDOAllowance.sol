...
contract IDOAllowance is IDO {
    ...
    function initialize(
        ...
        address _admin,
        address _manager,
        address[] memory _allowances
    ) public {
        allowanceCount = _allowances.length;
        require((_totalAllocationAmount / allowanceCount) >= _maxDeposit);
        for (uint256 i = 0; i < allowanceCount; i++) {
            allowances[_allowances[i]] = true;
        }

        super.initialize(
            _minDeposit,
            _maxDeposit,
            _totalAllocationAmount,
            _startDepositTime,
            _endDepositTime,
            _pricePerCurrency,
            _token,
    ...
    function addAllowance(address _allowance) public onlyRole(MANAGER_ROLE) {
        allowances[_allowance] = true;
        allowanceCount++;
        _calculateUserAllocation();
    }

    function removeAllowance(address _allowance) public onlyRole(MANAGER_ROLE) {
    ...
    function _calculateUserAllocation() private {
        uint256 allocationPerUser = totalAllocationAmount / allowanceCount;
        maxDeposit = allocationPerUser;
        minDeposit = maxDeposit <= minDeposit ? maxDeposit : minDeposit;
    }
}
