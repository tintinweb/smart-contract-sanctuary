...
abstract contract Auth {
    ...
    modifier onlyOwner() {
        require(isOwner(msg.sender), "WebooFarming: !OWNER");
        _;
    }

    modifier authorized() {
        require(isAuthorized(msg.sender), "WebooFarming: !AUTHORIZED");
        _;
    }
...
contract WebooFarming is Context, Auth {
    ...
    event AddLiquidity(address account,uint256 amountTokenA, uint256 amountTokenB, uint256 amountLiquidity);
    event RemoveLiquidity(address account,uint256 amountLiquidity,address recipient,uint256 amountTokenA,uint256 amountTokenB);

    modifier onlyToken() {
        require(_msgSender()==tokenAddress,"WebooFarming: OnlyToken");
        _;
    }

    constructor(address _tokenAddress, address _routerAddress, address _tokenB, address _rewardAddress)  Auth(msg.sender) {
   ...
   function addLiquidity(uint256 amountTokenForFarming, address recipient) external payable {
      ...
      IERC20(tokenAddress).transferFrom(_msgSender(), address(this), amountForFarm);
      ...
      IERC20(tokenAddress).approve(routerAddress, balanceForFarming);
       ...
       emit AddLiquidity(recipient,amountTokenA,amountTokenB,liquidity);
   }

    function getAmountForFarm(address holder,uint256 amount) internal view returns(uint256){
        uint256 currentBalance = IERC20(tokenAddress).balanceOf(holder);
   ...
   function removeLiquidity(uint256 amountRemove, address recipient) external {
       require(amountLPToken[_msgSender()] >= amountRemove,"WebooFarming: Insufficient Amount");
       ...
       totalLPToken = totalLPToken.sub(amountRemove);
       setShare(_msgSender());
       calculateShare();
       emit RemoveLiquidity(_msgSender(),amountRemove,recipient,amountA,amountB);
   }

    function setRewardShare() public {
        IWebooRewardV2(rewardAddress).setShare(address(this),IERC20(tokenAddress).balanceOf(pairAddress));
    }
    ...
    function getFee(uint256 amount) internal returns(uint256){
        ...
        if(amountFee > 0){
            payable(addressReceiverTax).transfer(amountFee);
        }
    ...
    function setMinimumWebooOnHolderWallet(uint256 amount) external onlyOwner {
        minimumWebooLeftOnHolderWallet = amount;
    }
...
interface IWebooRewardV2 is IWebooReward {
    function dividendOf(address account) external returns(uint256);
