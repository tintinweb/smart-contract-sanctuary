...
contract HIREX is Context, IBEP20, Ownable {
    ...
    uint256 public _maxWalletAmount = 3000000 * 10**9;
   
    uint256 private numTokensSellToAddToLiquidity = 1500000 * 10**9; // this tx amount will trigger swap and add liquidity
    ...
    function setMaxTxPercent(uint256 maxTxPercent, uint256 maxWalletPercent) external onlyOwner() {
        require(maxTxPercent > 0.000, "% must be greater than 0.000");
        require(maxWalletPercent > 0.000, "% must be greater than 0.000");
    ...
    function setFeePercent(uint256 fee, uint256 LiquidityFee, uint256 MarketingFee, uint256 ResearchFee) external onlyOwner() {
    taxFee = fee;
    liquidityFee = LiquidityFee;
    marketingFee = MarketingFee;
    researchFee = ResearchFee;
    totalLiqFee = liquidityFee.add(marketingFee).add(researchFee);
    }

    function setBuyFeePercent(uint256 BuyTaxFee, uint256 BuyLiquidityFee, uint256 BuyMarketingFee, uint256 BuyResearchFee) external onlyOwner() {
    buytaxFee = BuyTaxFee;
    buyliquidityFee = BuyLiquidityFee;
    buymarketingFee = BuyMarketingFee;
    buyresearchFee = BuyResearchFee;
    }

    function setSellFeePercent(uint256 SellTaxFee, uint256 SellLiquidityFee, uint256 SellMarketingFee, uint256 SellResearchFee) external onlyOwner() {
    selltaxFee = SellTaxFee;
    sellliquidityFee = SellLiquidityFee;
    sellmarketingFee = SellMarketingFee;
    sellresearchFee = SellResearchFee;
    }
	
	function setWalletAddress(address payable marketingWallet, address payable researchWallet) external onlyOwner() {
         _marketingWallet = marketingWallet;
         _researchWallet = researchWallet;
    }
    ...
    function _transfer(
        ...
        if(!(_isExcludeAddressFromMaxTx[from] || _isExcludeAddressFromMaxTx[to])) {
            ...
            require(balanceOf(to).add(amount) <= _maxWalletAmount, "Already reach your maxWalletAmount, wait till check off");           
		}
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 ethForLiquidity = balanceDiff.sub(ethForMarketing).sub(ethForResearch);
