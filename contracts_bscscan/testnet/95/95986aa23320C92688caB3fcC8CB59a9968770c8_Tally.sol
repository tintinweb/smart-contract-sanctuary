...
contract Tally is Context, IERC20, Ownable {
    ...
    uint256 public _previousSellTaxFee = _taxSellFee;
    
    uint256 public _taxBuyFee = 300;
    uint256 public _previousBuyTaxFee = _taxBuyFee;
    ...
    uint256 public _previousRaffleFee = _raffleFee;

    uint256 public _poolFee = 100;
    uint256 public _previousPoolFee = _poolFee;
...
//      IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F);
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount, uint8 txKind) public {
        uint256[12] memory values = _getValues(tAmount, txKind);
    ...
    function _getValues(uint256 tAmount, uint8 txKind) public view returns (uint256[12] memory values) {
    ...
    function _getTValues(uint256 tAmount, uint8 txKind) public view returns (uint256[9] memory tValues) {
    ...
    function _getRValues(uint tAmount, uint256[9] memory tValues, uint256 currentRate) public pure returns (uint256, uint256, uint256) {
    ...
    function _takeBuyBack(uint256 tBuyBack) public {
        if(tBuyBack == 0) return;

        swapAndSendToWallet(tBuyBack, _buyBackWallet);
    }

    function _takeRaffle(uint256 tRaffle) public {
        if(tRaffle == 0) return;
        _transfer(_msgSender(), _charityWallet, tRaffle); 
    }

    function _takePool(uint256 tPool) public {
        if(tPool == 0) return;

        _transfer(_msgSender(), _poolCharityWallet, tPool);
    }

    function calculateTaxFee(uint256 _amount, uint8 txKind) public view returns (uint256) {
    ...
    function calculateBuyBackFee(uint256 _amount) public view returns (uint256) {
        return _amount.mul(_buyBackFee).div(
            10**4
        );
    }

    function calculateRaffleFee(uint256 _amount) public view returns (uint256) {
        return _amount.mul(_raffleFee).div(
            10**4
        );
    }

    function calculatePoolFee(uint256 _amount) public view returns (uint256) {
    ...
    function removeOtherFee() public {
    ...
    function removeRaffleFee() public {
    ...
    function restoreOtherFee() public {
    ...
    function restoreRaffleFee() public {
        _raffleFee = _previousRaffleFee;
    }

    function restorePoolFee() public {
...
//            swapAndLiquify(contractTokenBalance);
    ...
    function swapAndSendToWallet(uint256 tokens, address account) public {
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, uint8 txKind) public {
        uint256[12] memory values = _getValues(tAmount, txKind);
    ...
    function _transferToExcluded(address sender, address recipient, uint256 tAmount, uint8 txKind) public {
        uint256[12] memory values = _getValues(tAmount, txKind);
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount, uint8 txKind) public {
        uint256[12] memory values = _getValues(tAmount, txKind);
