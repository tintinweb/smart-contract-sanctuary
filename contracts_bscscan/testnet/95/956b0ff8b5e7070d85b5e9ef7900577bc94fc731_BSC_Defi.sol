...
contract BSC_Defi {
    ...
    function chooseWinner() public {
       require(((block.timestamp.sub(LOTTERY_START_TIME) >= LOTTERY_STEP) || participants >= MAX_LOTTERY_PARTICIPANTS),
        ...
        //if timestep is done, but participants = 0, reset lottery start time.
        if(participants != 0){
            ...
            for(uint256 i = 0; i < participants; i++){
                if((random >= init_range[i]) && (random <= end_range[i])){
                    ...
                    user.totalLotteryBonus = user.totalLotteryBonus.add(eggs);
                    totalLotteryBonus = totalLotteryBonus.add(eggs);
    ...
    //remove in main net, this will be used just to recover testnet bnb funds.
    ...
    /** setup **/
    function SET_PROJECT_ADDRESS(address _addr) external {
        require(msg.sender == owner, "Admin use only.");
