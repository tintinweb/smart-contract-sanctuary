...
contract BEP20Token is  IBEP20Fee{
    ...
    function getAllFees() external returns (feeStructure[] memory) {
       return allFees;
    }  
    function getDivider() external returns (uint256) {
       return divider;
    }  
    function checkCalculateTheHighwayRobberyTax(uint256 amount) external returns (uint256[] memory,uint256) {
        uint[] memory feesToPay;
        //emit Log("Divider",divider);
