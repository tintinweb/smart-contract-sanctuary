...
contract PresaleContract is Ownable {
    ...
    uint256 public presalerate = 1;
    bool public paused = false;

    constructor(address _baseToken, address _presaleToken) {
    ...
    function swapFor(address _recipient, uint256 _amount) external {
        TokenSwap(_recipient, _amount);
    }
    function TokenSwap(address _recipient, uint256 _amount) private {
        require(!paused, "PresaleSwap: Presale is paused");
        require(_amount > 0, "PresaleSwap: Invalid Base Token amount");
        require(baseToken.transferFrom(msg.sender, address(this), _amount), "PresaleSwap: Failed to transfer Base Token");
        require(presaleToken.transfer(_recipient, _amount.div(presalerate)), "PresaleSwap: Failed to transfer Presale Token");
    }
    function setSwapRate(uint256 _presalerate) external onlyOwner() {
        presalerate = _presalerate;
    }
    function withdrawBaseToken(address _recipient, uint256 _amount) external onlyOwner {
        require(baseToken.transfer(_recipient, _amount), "PresaleSwap: Failed to transfer Base Token");
    }
