...
contract JadeiteSale is ReentrancyGuard, Context, Ownable {
    ...
    mapping (address => bool) public _earlyInvestor;
    ...
    //Start Sale
    function startSale(uint endDate, uint _minPurchase, uint _maxPurchase, uint _hardCap, uint _procent, uint _bonusPurchase, uint _endBonus) external onlyOwner SaleNotActive() {
    ...
    function buyTokens() public SaleActive payable {
        ...
        //Bonus allocation
        if(weiAmount >= bonusPurchase  && block.timestamp <= endBonus && procent > 0) {
        ...
        else if (_earlyInvestor[beneficiary] == true) {
            uint earlybonus = 5;
            uint bonus = earlybonus.mul(weiAmount).div(100);
            uint256 bonusTokens = _getTokenAmount(bonus);
            uint256 standardTokens = _getTokenAmount(weiAmount);
            uint256 tokens = bonusTokens+standardTokens;
            availableTokensSale = availableTokensSale - tokens;
    ...
    function setMaxPurchase(uint256 value) external onlyOwner {
     ...
     function setBonusPurchase(uint256 value) external onlyOwner {
        bonusPurchase = value;
    }
