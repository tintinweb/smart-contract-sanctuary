...
contract EpicAuction is AccessControl {
    ...
    function _getItemInfo(uint256 marketId) public view returns(bool, Item memory) {
        ...
        return (false, Item(address(0), TokenType.CLOSED, address(0), 0, 0, 0, 0, 0, address(0), false, address(0), 0, 0));
    }
    ...
    function buyAuction(uint256 marketId) public returns(bool, Item memory){
    ...
    function _getBidWinner(uint256 marketId) internal view returns(Bid memory) {
        ...
        return Bid(address(0), 0, 0, false, false, false);
    }
    ...
    function winnerAcceptBid(uint256 marketId) public onlyExistItem(marketId) returns (bool, uint256) {
        ...
        require(items[marketId]._expiration >= block.timestamp, "Out of accept time");
    ...
    function nextBidWinner(uint256 marketId) public onlyItemOwner(marketId) {
        ...
        if(winnerIndex == 0) {
            ...
            if(items[marketId]._acceptTime <= block.timestamp){
                items[marketId]._acceptTime = block.timestamp + 1 hours;
            } else {
