...
contract TokenCarrier is BuildableContext {
    ...
    constructor(address carrierToken) {
        ...
        _carrierTokenDecimalFactor = 10**18;
    ...
    function _purchaseRate() internal view returns (uint256) {
        uint256 currentPurchaseRate = _carrierToken.totalSupply().mul(_liquidityTokenDecimalFactor).div(address(this).balance);
    ...
    // @dev function recieve() - public payable method used to inject liquidity without recieving tokens
    ...
    function _liquidationRate() internal view returns (uint256) {
        return address(this).balance.mul(_carrierTokenDecimalFactor).div(_carrierToken.totalSupply());
    }
...
contract TokenLab is OwnableContext {
    ...
    constructor() {
        ...
        _testToken = new CarriedToken("Test Token", "TsT", 100000000, 9);
    ...
    function updateCarrierMaxPurchaseRate(uint256 newMaxPurchaseRate) public onlyOwner returns (bool) {
        return _tokenCarrier.setPurchaseRateMax(newMaxPurchaseRate);
    }
}
