...
/* MortyToken:
*
* Telegram: https://t.me/MortyBSC
* Website: www.mortytoken.com
*
*
* Fees on transfer:
* 3% Reflections to be automatically redistributed amongst the holders 
* 3% Liquidity in order to maintain a healthy liquidity pool
* 3% Giveaway wallet which will be used for random giveaways as well as our milestone giveaways (is automatically converted to BNB)
* 1% Marketing wallet in order to pay for our ongoing marketing efforts (is automatically converted to BNB)
* 1% Burn in order to ensure that the price of each token will inflate with every buy and with every sell. 
*
* 10% of the total supply will be locked through DxLock with a vesting period with 5 unlock times.
* 2% of the supply will be burned each day for the first 5 days.
*
*
* For the first 10 minutes of the pancakeswap launch, there will be a maximum transaction percentage in order
* to give everyone a fair chance of getting in and in order to prevent the whales from buying in big at
* the start.
*/
...
contract LotteryDoge is Context, IERC20, Ownable {
    ...
    string private constant _name = "LotteryDoge";
    string private constant _symbol = "LDoge";

    uint256 _minLottoBalance = 20* 10**6 * 10**_decimals; //20M is the minimum to be eligible to win
    ...
    uint256 private _maxTxAmount = _tTotal*5/1000; //this will be set to the full supply manually by the owner after 10 minutes from launch
    ...
    constructor () {
        ...
        IUniswapV2Router02 _PancakeSwapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //BSC mainnet
        ...
        antiwhale_measures[0] = antiwhale({selling_treshold: _tTotal*25/10000, extra_tax: 25});//0.25% of initial supply
        antiwhale_measures[1] = antiwhale({selling_treshold: _tTotal*50/10000, extra_tax: 50});//0.5% of initial supply
        antiwhale_measures[2] = antiwhale({selling_treshold: _tTotal/100, extra_tax: 75});//1% of initial supply
...
(to_return.rAmount,to_return.rTransferAmount,to_return.rRfi,to_return.rLottery,to_return.rMarketing,to_return.rLiquidity,to_return.rBurn) = _getRValues(to_return, tAmount, takeFee, _getRate());
    ...
    function lotterize() private view returns(address) {
                ...
                if(numHolders>0)
                {
                randomNumber = block.number % numHolders;
                do
                {
                    chosenWinner = tokenHoldersEnumSet.at(randomNumber);
				    ownedAmount = _rOwned[chosenWinner];
                    ...
                    else
                    {
                        ...
                        { 
                            return address(0); //only one holder and he can't win
                        }
                    }
                    }
                }while(trial<2);
    ...
    function testCounts() external view returns(bool){
    return (contractBalance.lp_balance+contractBalance.marketing_balance) == balanceOf(address(this));
