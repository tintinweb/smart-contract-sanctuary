...
contract UnboundTokenSeller is IUnboundTokenSeller {
        ...
        _control_
    {
        ...
        _oracle = IBisharesUniswapV2Oracle(_pool.oracle());
        _uniswapV2router = IUniswapV2Router02(_pool.router());
    }
     ...
     */
    function setPremiumPercent(uint8 premiumPercent)
        external
        override
        _control_
    {
    ...
    function executeSwapTokensForExactTokens(
    ...
    ) external override _lock_ returns (uint256 premiumPaidToCaller) {
        ...
        IERC20(tokenIn).safeApprove(address(_uniswapV2router), maxAmountIn);
        ...
        // Execute the swap.
        uint256[] memory amounts = _uniswapV2router.swapTokensForExactTokens(
            amountOut,
            maxAmountIn,
            path,
            address(_pool),
        ...
        if (amountIn < maxAmountIn) {
            IERC20(tokenIn).safeApprove(address(_uniswapV2router), 0);
        ...
        _pool.gulp(tokenOut);
        emit SwappedTokens(tokenIn, tokenOut, amountIn, amountOut);
    }
    ...
    function executeSwapExactTokensForTokens(
        ...
        // Approve UniSwap to transfer the input tokens
        IERC20(tokenIn).safeApprove(address(_uniswapV2router), amountIn);
        // Verify that the first token in the path is the input token and that
        // the last is the output token.
        require(
            path[0] == tokenIn && path[path.length - 1] == tokenOut,
    ...
    function swapExactTokensForTokens(
    ...
    ) external override _lock_ returns (uint256 amountOut) {
        ...
        emit SwappedTokens(tokenOut, tokenIn, amountOut, amountIn);
    }
    ...
    function swapTokensForExactTokens(
        ...
        uint256 amountOut,
        uint256 maxAmountIn
    ) external override _lock_ returns (uint256 amountIn) {
    ...
    function getPremiumPercent() external override view returns (uint8) {
    ...
    function calcInGivenOut(
        ...
        address tokenOut,
        uint256 amountOut
    ) public override view _desired_(tokenIn) returns (uint256 amountIn) {
        require(
    ...
    function calcOutGivenIn(
        ...
        address tokenOut,
        uint256 amountIn
    ) public override view _desired_(tokenIn) returns (uint256 amountOut) {
        (
            PriceLibrary.TwoWayAveragePrice memory avgPriceIn,
            PriceLibrary.TwoWayAveragePrice memory avgPriceOut
        ) = _getAveragePrices(tokenIn, tokenOut);
        ...
        )
    {
        ...
        tokens[0] = token1;
        tokens[1] = token2;
        PriceLibrary.TwoWayAveragePrice[] memory prices = _oracle
