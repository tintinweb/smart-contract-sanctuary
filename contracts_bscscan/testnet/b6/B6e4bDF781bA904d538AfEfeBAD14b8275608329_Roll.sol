...
contract Roll is VRFConsumerBase {
    ...
    event ShowResult(bytes32 indexed requestId, uint _timestamp, address indexed _wallet, uint _rollUnder,  uint _roll, bool _win);
    ...
    function getRandomNumber(uint rollUnder) private returns (bytes32 requestId) {
        ...
        players[reqId] = msg.sender;
        bets[reqId] = rollUnder;
        return reqId;
    }

    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        address sender = players[requestId];
        uint rollUnder = bets[requestId];
        uint rollResult = (randomness % 101) + 1;
        bool win = rollResult < rollUnder;
        if (win) payout(rollUnder);
        emit ShowResult(requestId, block.timestamp, sender, rollUnder, rollResult, win);
    }
