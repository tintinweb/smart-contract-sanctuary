...
contract ShareV2 is ERC20UpgradeSafe, OwnableUpgradeSafe, ReentrancyGuard {
    uint256 public cap;
    mapping(address => bool) public minter;

    uint256 public migrationRate;
    uint256 public migrationCap;
    uint256 public totalMigrationAmount;

    uint256 public burnRate;
    uint256 public taxRate; // buy back SDO and add liquidity
    address public taxFund; // fund buy back SDO
    ...
    function initialize(address _taxFund) public initializer {
        __ERC20_init("pok.Fi", "P");
        ...
        _isExcludedFromFee[_taxFund] = true;
        _isExcludedToFee[_taxFund] = true;
        totalMigrationAmount = 0;
        _mint(msg.sender, 10 ether);

    }

    function setMigrationRate(uint256 _migrationRate) external onlyOwner {
        require(_migrationRate <= 10000, "max 1:1"); // <= 100%
    ...
    function setTaxRate(uint256 _taxRate) external onlyOwner {
        require(_taxRate <= 1000, "too high"); // <= 10%
        taxRate = _taxRate;
    }
        ...
        if (!_isExcludedFromFee[_sender] && !_isExcludedToFee[_recipient]) {
            {
                ...
                if (_taxRate > 0) {
                    uint256 _taxAmount = _amount.mul(_taxRate).div(10000);
                    address _taxFund = taxFund;
                    _sentAmount = _amount.sub(_taxAmount);
                    _totalTaxAdded = _totalTaxAdded.add(_taxAmount);
                    super._transfer(_sender, _taxFund, _taxAmount);
                    emit TaxAdded(_sender, _taxFund, _taxAmount);
                }
            }
            {
