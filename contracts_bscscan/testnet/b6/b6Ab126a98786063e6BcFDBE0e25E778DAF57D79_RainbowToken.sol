...
contract RainbowToken is Context, IERC20, Ownable {
    ...
    // General Info
    string private _name     = "R";   // TODO: Change back to 'RainbowToken'
    string private _symbol   = "R";   // TODO: Change back to 'RAINBOW'
    ...
    // Addresses
    address payable public _marketingAddress = payable(0xf1108a21078fdd47A67DdcC238c7919323332b04); // Marketing address used to pay for marketing
    ...
    constructor () {
        ...
        IUniswapV2Router02 pancakeswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 0xD99D1c33F9fC3444f8101754aBC46c52416550D1 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (_enableBuyback && !currentlySwapping && to == _pancakeswapV2LiquidityPair) {
	        ...
	        // Only execute a buyback when the contract has more than 0.5 BNB
        ...
        if (takeFee && to == _pancakeswapV2LiquidityPair) {
            ...
            uint delta = block.timestamp - _timeSinceFirstSell[from];
     ...
     */
    function _burnTokens(uint256 tFee, uint256 rFee) private {
    ...
    /**
     * @notice Increases the rate of how many each token is worth
     */
    function _reflectTokens(uint256 tFee, uint256 rFee) private {
     ...
     */
    function _takeTokens(uint256 tFee, uint256 rFee) private {
        _rOwned[address(this)] = _rOwned[address(this)].add(rFee.mul(5));
        if(_isExcluded[address(this)]) {
            _tOwned[address(this)] = _tOwned[address(this)].add(tFee.mul(5));
        }
        
        // Add a portion to the lottery pool to be rewarded to a lucky buyer
        _lotteryPool = _lotteryPool.add(rFee);
    }
