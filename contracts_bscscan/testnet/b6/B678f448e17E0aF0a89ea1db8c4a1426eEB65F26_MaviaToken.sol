...
import "./interfaces/arbitrum/IL1CustomGateway.sol";
import "./interfaces/arbitrum/IL1GatewayRouter.sol";

contract MaviaToken is ERC20Token, ICustomToken {
  uint256 private _arbitrumChain;
  bool private _shouldRegisterGateway;

  address public l1CustomGateway;
  address public l1GatewayRouter;

  /**
   * @dev Upgradable initializer
   */
  function __MaviaToken_init(address _l1CustomGateway, address _l1GatewayRouter) public initializer {
    __ERC20Token_init("MaviaToken", "MAVIA", 18, 1000000000);
    // 0xa4b1 = 42161 = chain id
    _arbitrumChain = 0xa4b1;
    l1CustomGateway = _l1CustomGateway;
    l1GatewayRouter = _l1GatewayRouter;
  }

  /**
   * @notice Normal ERC20 TransferFrom
   */
  ...
  /**
   * @notice Get balance ERC20 token
   */
  function balanceOf(address _account) public view override(ERC20Upgradeable, ICustomToken) returns (uint256) {
    return ERC20Upgradeable.balanceOf(_account);
  }

  /**
   * @dev We only set _shouldRegisterGateway to true when in `registerTokenOnL2`
   */
  function isArbitrumEnabled() external view override returns (uint8) {
    require(_shouldRegisterGateway, "NOT_EXPECTED_CALL");
    return uint8(_arbitrumChain);
  }

  /**
   * @notice We only set _shouldRegisterGateway to true when in `registerTokenOnL2`
   * @dev Remember send ETH to pay gas fee on L2
   */
  function registerTokenOnL2(
    address _l2CustomTokenAddress,
    uint256 _maxSubmissionCostForCustomBridge,
    uint256 _maxSubmissionCostForRouter,
    uint256 _maxGasForCustomBridge,
    ...
    IL1CustomGateway(l1CustomGateway).registerTokenToL2{ value: _valueForGateway }(
      _l2CustomTokenAddress,
      _maxGasForCustomBridge,
      _gasPriceBid,
      _maxSubmissionCostForCustomBridge,
      _creditBackAddress
    );

    IL1GatewayRouter(l1GatewayRouter).setGateway{ value: _valueForRouter }(
      l1CustomGateway,
      _maxGasForRouter,
      _gasPriceBid,
  ...
  /**
   * @dev Update arb config
   */
  function updateArbConfig(
    uint8 _arbChain,
    address _l1CustomGateway,
    address _l1GatewayRouter
  ) external onlyOwner {
    _arbitrumChain = _arbChain;
    l1CustomGateway = _l1CustomGateway;
    l1GatewayRouter = _l1GatewayRouter;
  }
