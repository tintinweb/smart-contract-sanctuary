...
abstract contract TSTBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
    ...
    address private _marketingWallet = 0x7c4E7534470001Eac40401080509140783D0E4b7;
    address private _devAddress = 0x7c4E7534470001Eac40401080509140783D0E4b7;
	...
	constructor (address routerAddress) {
		...
		// Initialize PancakeSwap V2 router and TST <-> BNB pair.
	...
	// This function swaps a {tokenAmount} of TST tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for TST -> WBNB
	...
	// Returns how many more $TST tokens are needed in the contract before triggering a swap
...
contract TST is TSTBase {
	...
	uint256 private _totalBNBAsTSTClaimed; // The total number of BNB that was converted to TST and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsTSTClaimed; // The amount of BNB converted to TST and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy TST automatically
	...
	bool private _reimburseAfterTSTClaimFailure; // If true, and TST reward claim portion fails, the portion will be given as BNB instead
	...
	constructor (address routerAddress) TSTBase(routerAddress) {
	...
	function onActivated() internal override {
		...
		setReimburseAfterTSTClaimFailure(true);
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough TST balance");
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim TST tokens
		if (!claimTST(user, claimBnbAsTokens)) {
			// If token claim fails for any reason, award whole portion as BNB
			if (_reimburseAfterTSTClaimFailure) {
	...
	function claimTST(address user, uint256 bnbAmount) private returns (bool) {
		if (bnbAmount == 0) {
			return true;
		}
		...
		_bnbAsTSTClaimed[user] += bnbAmount;
		_totalBNBAsTSTClaimed += bnbAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $TST on a zero balance address:
		...
		// Depending on the % of $TST tokens transferred, relative to the balance, we might need to extend the period
    ...
    function bnbRewardClaimedAsTST(address byAddress) public view returns (uint256) {
		return _bnbAsTSTClaimed[byAddress];
	}
    ...
    function totalBNBClaimedAsTST() public view returns (uint256) {
		return _totalBNBAsTSTClaimed;
	}
	...
	function reimburseAfterTSTClaimFailure() public view returns(bool) {
		return _reimburseAfterTSTClaimFailure;
	}


	function setReimburseAfterTSTClaimFailure(bool value) public onlyOwner {
		_reimburseAfterTSTClaimFailure = value;
	}
