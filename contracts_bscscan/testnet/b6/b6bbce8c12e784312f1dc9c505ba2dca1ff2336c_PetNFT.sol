...
contract PetNFT is ERC721{
    ...
    mapping(uint => GGPet) public ggPets;
    // Optional mapping for token URIs
    mapping (uint256 => string) private _tokenURIs;

    constructor() ERC721("GGPetNFT", "GGPETNFT") {
        owner = msg.sender;
    }
    ...
    function setGGPet(uint _tokenId,int16 _class,int16 _color,int32 _eyes,int32 _hair,int32 _horn,int32 _back,int32 _tail,int32 _mouth) external onlyOwner(){
        ggPets[_tokenId] = GGPet(_class,_color,_eyes,_hair,_horn,_back,_tail,_mouth,block.timestamp);
    }
