...
Initializable
{
    ...
    function refreshSeedA(uint seed_) onlyRole(ADMIN_ROLE) external {
        seedA = seed_ + block.timestamp + IDefinaCard(nftToken).totalSupply();
    }

    function refreshSeedB(uint seed_) onlyRole(ADMIN_ROLE) external {
        seedB = seed_ + block.timestamp + IDefinaCard(nftToken).totalSupply();
    }
