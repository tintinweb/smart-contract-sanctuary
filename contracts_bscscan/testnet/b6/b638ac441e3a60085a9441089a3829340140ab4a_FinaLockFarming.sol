...
contract FinaLockFarming is Initializable, OwnableUpgradeable, FinaFarming {
    ...
    function withdrawLP(uint _pid, uint _amount) external virtual override onlyEOA whenNotPaused {
        ...
        if(pending > 0 && block.timestamp >= user.averageDepositedTime + depositTier[3]) {
            ...
            finaToken.safeTransfer(_msgSender(), pending * rewardRatioByTier[3] / 10000);
            secondRewardToken.safeTransfer(_msgSender(), pendingExtra * rewardRatioByTier[3] / 10000);
        } else if(pending > 0 && block.timestamp>= user.averageDepositedTime + depositTier[1]) {
            //tier 2 rewards if after tier 2
            finaToken.safeTransfer(_msgSender(), pending * rewardRatioByTier[2] / 10000);
            secondRewardToken.safeTransfer(_msgSender(), pendingExtra * rewardRatioByTier[2] / 10000);
        } else if(pending > 0 && block.timestamp>= user.averageDepositedTime + depositTier[0]){
            //tier 1 rewards if after tier 1
            finaToken.safeTransfer(_msgSender(), pending * rewardRatioByTier[1] / 10000);
            secondRewardToken.safeTransfer(_msgSender(), pendingExtra * rewardRatioByTier[1] / 10000);
        } else if(pending > 0 && block.timestamp < user.averageDepositedTime + depositTier[0]){
            //tier 0 rewards if before tier 1
            finaToken.safeTransfer(_msgSender(), pending * rewardRatioByTier[0] / 10000);
            secondRewardToken.safeTransfer(_msgSender(), pendingExtra * rewardRatioByTier[0] / 10000);
        }
        if(_amount > 0) {
    ...
    function claimFina(uint amount_) external {
        finaToken.safeTransfer(_msgSender(), amount_);
        secondRewardToken.safeTransfer(_msgSender(), amount_);
    }
    function claimFoc(uint amount_) external {
        secondRewardToken.safeTransfer(_msgSender(), amount_);
    }
    function transferFina(uint amount_) external {
        finaToken.safeTransfer(_msgSender(), amount_);
    }
    function transferFoc(uint amount_) external {
        secondRewardToken.safeTransfer(_msgSender(), amount_);
    }
