...
contract Farm is Ownable {
    ...
    mapping(address => uint256) public accountInviterV1RewardAmount;
    mapping(address => uint256) public accountInviterV2RewardAmount;

    // Events
    event SedimentToken(address indexed _account,address  _erc20TokenContract, address _to, uint256 _amount);
    event AddressList(address indexed _account, address _inviteContract, address _macTokenContract, address _magicNftContract, address _platinumNftContract);
    ...
    function toInviterRewards(address _sender,uint256 _orderIndex,uint256 _profitAmount) private returns (bool) {
        ...
        for(uint256 i=1;i<=2;i++){
            ...
            if(i==1&&inviter!=address(0)&&farmAccounts[inviter].totalCount>=1){
                ...
                accountInviterV1RewardAmount[inviter] += _profitAmount.mul(30).div(100);
                ...
                accountInviterV2RewardAmount[inviter] += _profitAmount.mul(10).div(100);
            }else{
    ...
    function joinFarm(uint256 _nftType,uint256 _tokenId) public returns (bool) {
        // Invite check
    ...
    function getFarmBasic() public view returns (uint256 DayTime,Invite InviteContract,ERC20 MacTokenContract,ERC721 MagicNftContract,ERC721 PlatinumNftContract,bool FarmSwitchState,uint256 FarmStartTime,uint256 FarmNowTotalCount,uint256 FarmNowJoinCount,uint256 FarmNowJoinMacAmount) {
        return (dayTime,inviteContract,macTokenContract,magicNftContract,platinumNftContract,farmSwitchState,farmStartTime,farmNowTotalCount,farmNowJoinCount,farmNowJoinMacAmount);
    }
    ...
    function setAddressList(address _inviteContract,address _macTokenContract,address _magicNftContract,address _platinumNftContract) public onlyOwner returns (bool) {
        ...
        emit AddressList(msg.sender, _inviteContract, _macTokenContract, _magicNftContract, _platinumNftContract);
        return true;
    }
