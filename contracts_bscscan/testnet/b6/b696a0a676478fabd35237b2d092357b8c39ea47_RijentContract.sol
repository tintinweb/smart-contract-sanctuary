...
contract RijentContract {    
    ...
    // View details 
    function allDetails(address addr) public view returns(uint,uint,bool) {
        User storage dep  = user[addr];
        uint timeDiff     = block.timestamp- dep.investTime;
        ///calculate ROI
        uint roiAmt       = ( dep.investAmount * timeDiff*calculteValue*dep.roiPer)/( 10**18 * 10**calculteDecimal );

        return (timeDiff,roiAmt,dep.registered);
    }
    ///withdraw roiAmt
