...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    */
    ...
    // Update oasis transfer owner. Can only be called by existing oasisTransferOwner
    function setOasisTransferOwner(address _oasisTransferOwner) external {
        require(msg.sender == oasisTransferOwner);
        oasisTransferOwner = _oasisTransferOwner;
        emit SetOasisTransferOwner(msg.sender, _oasisTransferOwner);
    }
     ...
     */
    function acceptOasisOwnership() external {
        require(msg.sender == oasisTransferOwner);
        require(pendingOasisOwner != address(0));
      //  oasis.transferOwnership(pendingOasisOwner);
        pendingOasisOwner = address(0);
        emit AcceptOasisOwnership(msg.sender, pendingOasisOwner);
    }
    
    function setPendingOasisOwnership(address _pendingOwner) external {
        require(msg.sender == oasisTransferOwner);
        pendingOasisOwner = _pendingOwner;

        emit NewPendingOasisOwner(msg.sender, _pendingOwner);
    }
}
