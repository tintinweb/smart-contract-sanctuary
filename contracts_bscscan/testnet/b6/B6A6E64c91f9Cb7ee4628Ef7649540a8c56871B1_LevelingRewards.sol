...
contract LevelingRewards is StarlinkComponent {
    struct Reward {
        uint256 id;
        uint128 typeId;
        uint128 amount;
    ...
    function claimRewards(uint256 level) external notPaused nonReentrant notUnauthorizedContract process {
        doClaimRewards(msg.sender, level);
    }

    function claimRewards(address user, uint256 level) external onlyAdmins {
    ...
    function addReward(uint256 level, uint128 typeId, uint128 amount) public onlyAdmins {
        Reward memory reward = Reward(rewardsPerLevel[level].length, typeId, amount, block.timestamp);
        rewardsPerLevel[level].push(reward);
    }

    function unclaimedRewards(address user, uint256 level) public view returns(uint256) {
        uint256 rewardDate = rewardDatesPerLevel[user][level];
        Reward[] memory rewards = rewardsPerLevel[level];
