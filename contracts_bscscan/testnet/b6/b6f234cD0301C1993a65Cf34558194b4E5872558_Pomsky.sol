...
contract Pomsky is Ownable, PomskyMetaData {
 ...
 function burning(address receiver, uint amount) public {
  ...
  function _deductTaxes(uint256 amount, TransferType kindOfTransfer) private returns(uint256 totalTax) {
    ...
    // we may not sell tokens on PancakeSwap here. Let the tokens on this contract's
    ...
    if (kindOfTransfer != TransferType.BuyViaPcs) {
      ...
      if (tokensToSwap > 0) {
        ...
        _sendBnb(punishmentFund, swappedBnb * _accruedPunishmentTax / tokensToSwap);
      }
