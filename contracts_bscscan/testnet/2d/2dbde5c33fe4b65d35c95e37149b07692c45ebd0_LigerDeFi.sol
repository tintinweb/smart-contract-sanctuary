...
contract LigerDeFi is Context, ILigerBEP20, Ownable {
    ...
    uint256 public MaxTxAmount = 500 * 10**9 * 10**18;
    ...
    event holders(uint256 totalholders);
    ...
    constructor(address payable DEXaddress, address TreasuryAddress) public {
        ...
        //exclude ownerAddress, burnAddress and this contract from fee
    ...
    function updateDEX(address newDEXaddress) external onlyOwner() {
        ...
        // Check if liger pair already exists
    ...
    function setExcludeFromMaxTx(address account, bool value) public onlyOwner {
    ...
    function _tokenTransfer(
        ...
        if (TreasuryFee != 0) {
            //Temporarily remove fees to transfer to Treasury Vault
            distributeFee(false);
            //Send treasury fee to Treasury Vault
            _transferStandard(sender, TreasuryVault, _treasuryFee);
            //Restore HolderReward, LiquidityFee & TreasuryFee
            distributeFee(true);
        }
