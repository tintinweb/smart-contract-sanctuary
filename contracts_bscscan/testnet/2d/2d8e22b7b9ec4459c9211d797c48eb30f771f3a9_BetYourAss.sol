...
contract BetYourAss is Context, IERC20, IERC20Metadata,Ownable {
    ...
    string private _name = "Bet Your Ass";
    string private _symbol = "BYA";
    uint256 private _tSupply = 1000000000*10**18;
    
    
    
    uint256 public dexTaxFee = 1000;
    ...
    constructor() {
        ...
        IPancakeRouter _router = IPancakeRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
    ...
    function _transfer(
        ...
        bool takeFee = true;
        if (_isExcludedFromFee[sender] || reflectionFeesdiabled) {
            takeFee = false;
        }
        
        if(sender == pairAddress || !takeFee){ //checks if the sender is the pair address or if its excluded from fees, doesnt take any fees if one of these conditions are true.
            _balances[recipient] = _balances[recipient].add(amount);
            emit Transfer(sender, recipient, amount);
        }
        else { // takes the fees normally otherwise
            uint256 currentSupply =
            _tSupply.sub(balanceOf(0x000000000000000000000000000000000000dEaD));
