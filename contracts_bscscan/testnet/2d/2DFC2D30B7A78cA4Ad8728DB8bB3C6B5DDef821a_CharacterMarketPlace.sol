...
contract CharacterMarketPlace is OwnableUpgradeable {
    ...
    function closeAuctionSudden(uint256 _nftId) external {
        ...
        if (auctions[_nftId].originalOwner != auctions[_nftId].highestBidder) {
            (bool sent, ) = payable(auctions[_nftId].originalOwner).call{
                value: auctions[_nftId].highestBid - auctions[_nftId].highestBid*2/100
            }("");
            
            require(sent, "Transfer BNB failed");
            feeTotal=feeTotal+auctions[_nftId].highestBid*2/100;
        }
...
contract CharacterNFT is ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable{
 ...
 function createRandomThreeCharacter(address owner) public {
        ...
        uint256 rankRandom = (value << (60+i*3))%5 +1;
