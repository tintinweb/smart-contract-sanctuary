...
contract Ownership {
    ...
    function getTotalSupply(address contractAddress) internal view returns(uint256) {
        return deployedContract(contractAddress).totalSupply();
    }
    
    function getBalanceOf(address contractAddress, address owner) internal view returns(uint256) {
