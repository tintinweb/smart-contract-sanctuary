...
contract ApeHeroNFT is Ownable, ERC721, ERC721Enumerable {
    ...
    /**
     * @dev all tokens in wallet address
     */
    ...
    /**
     * @dev calculate earned reflection minus minus claimed reflections
     */
    function _allEarnedReflection(address tokenOwner) internal view returns (uint) {
        ...
        for (uint i = 0; i < _tokens.length; i++) {
            ...
            console.log(tokenOwner, _tokens[i], _tEarned, _claimedDividends[_tokens[i]]);
            if (_tEarned > 0) {
                _allEarned += _tEarned;
            }
        }
     ...
     * to all holders
     */
    function airdropMint(uint amount) public onlyOwner {
        require((_tokenIds.current()) + amount < _maxSupply, "Exceeds max supply");
        mintToken(owner(), amount, 0);
    }
    ...
    function claimMintRewards() public {
        uint _claimingTotal = _allEarnedReflection(_msgSender());
        require(_claimingTotal > 0, "Insufficient balance");
        _balanceReflections -= _claimingTotal;
        payable(_msgSender()).transfer(_claimingTotal);
    }
}
