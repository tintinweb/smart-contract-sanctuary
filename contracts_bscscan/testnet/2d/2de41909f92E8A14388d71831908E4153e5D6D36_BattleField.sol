...
contract BattleField is VRFConsumerBase, ITrait {

    bytes32 internal keyHash;
    bytes32 internal latestRequestID;
    ...
    struct ElementPair {
        ELEMENT_TYPES element1;
        ELEMENT_TYPES element2;
    }
    uint256 public POWER_BONUS = 10;
    uint256 public WIN_BONUS = 5;
    uint256 public ENTER_PRICE = 3 * 10 ** 15;
    ...
    enum STATES { NOTENTER, ENTERED, PICKED, FIGHT }
    event EnteredBattleField(address indexed _from, uint256 indexed _id);
    event StartedEnemyChoose(address indexed _from, uint256 indexed _id);
    event ChoosedRandomEnemy(address indexed _from, uint256 indexed _id, uint256 indexed _eid);
    event StartedBattleField(address indexed _from, uint256 indexed _id, uint256 indexed _eid);
    event EndedBattleField(address indexed _from, uint256 indexed _id, uint256 indexed _eid, bool result);
    ...
    function enterGame(uint256 tokenID) external {
        ...
        require(characters[tokenID].state == STATES.NOTENTER, "Already entered");
        ...
        shibatoken.transferFrom(msg.sender, address(this), ENTER_PRICE);
        emit EnteredBattleField(msg.sender, tokenID);
    }

    function startChooseEnemy(uint256 tokenID) public {
        require(characters[tokenID].state == STATES.ENTERED, "You did not enter the game");
        require(characters[tokenID].owner == msg.sender, "You are not the owner of this nft");
        bytes32 reqID = getRandomNumber();
        randomRequest[reqID] = tokenID;
        emit StartedEnemyChoose(msg.sender, tokenID);
    }

    function getChosenEnemy(bytes32 requestId, uint256 random) public returns (uint256) {
        ...
        emit ChoosedRandomEnemy(msg.sender, tokenID, characters[tokenID].enemyID);
        return index + 1;
    }

    function startFight(uint256 tokenID) public returns (bool) {
        require(characters[tokenID].state == STATES.PICKED, "Enemy not choosed yet");
        require(characters[tokenID].owner == msg.sender, "You are not the owner of this nft");
        emit StartedBattleField(msg.sender, tokenID, characters[tokenID].enemyID);
        ...
        characters[tokenID].state = STATES.NOTENTER;
        if(random >= winPercent) {
            emit EndedBattleField(msg.sender, tokenID, characters[tokenID].enemyID, false);
            return false;
        }
        uint256 rewardPercent = (characters[tokenID].random / 100000) % 100;
        rewards[msg.sender] += ENTER_PRICE * (100 + rewardPercent) / 100;
        emit EndedBattleField(msg.sender, tokenID, characters[tokenID].enemyID, true);
        return true;
    }
    ...
    function claimReward(address owner) external {
        require(rewards[owner] > 0, "No funds to claim");
        shibatoken.transfer(owner, rewards[owner]);
    }

    function getCurrentReward(address owner) external view returns (uint256) {
        return rewards[owner];
    }

    function getCurrentEnemy(uint256 tokenID) external view returns (uint256) {
        require(characters[tokenID].state == STATES.PICKED, "Enemy not choosed yet");
        return characters[tokenID].enemyID;
    }
