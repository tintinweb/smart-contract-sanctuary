...
contract NFTKeys is Context, ERC721Burnable {

    using SafeMath for uint256;
    
    uint256 public constant tokenPrice = 0.05 ether;
    uint public constant maxTokensNumber =  11;

    uint internal numTokens = 0;
    
    uint public constant referralPercent = 10;
    ...
    string public constant baseURL = "https://cdn.nftkeys.app/keys/";

    uint internal nonce = 0;
    uint[maxTokensNumber] internal indices;

    address payable _owner = address(0);
    address constant _ignoreReferral = 0x101e87a04e4E3d56F1D8bFBF0d66f8e331b1D36E;
    ...
    constructor() public ERC721("NFTKeys", "NFTK") {
        _owner = _msgSender();
        _setBaseURI(baseURL);
    }
    
    function randomIndex() internal returns (uint) {
        uint totalSize = maxTokensNumber - numTokens;
    ...
    function mint(address to, address payable referral) public payable{
        ...
        require(msg.value >= tokenPrice, "Not enought money");
        require(to != address(0), "Cannot mint to empty");
        require(numTokens < maxTokensNumber, "Token limit");
        ...
        referral.transfer(msg.value.mul(referralPercent).div(100));
        
        totalMint+=tokensMintedNow;
        
        uint ownerAmount = 0;
        ownerAmount += (tokenPrice * tokensMintedNow  * 90)/100;
        numTokens++;
    ...
    function setMintingStatus(bool _status) public _ownerOnly {
        mintEnabled = _status;
    }
