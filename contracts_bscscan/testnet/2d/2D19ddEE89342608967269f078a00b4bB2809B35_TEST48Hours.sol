...
contract TEST48Hours is IERC20 {
    ...
    uint256 private constant startingSupply = 48_000_000_000; // 48 Millions, underscores aid readability

    uint8 private _decimals = 8;
    uint256 private _decimalsMul = _decimals;
    uint256 private _tTotal = startingSupply * (10 ** _decimalsMul);

    string constant _name = "1Test48Hours";
    string constant _symbol = "1TEST48HOURS";
    ...
    uint256 private maxReflectionFee = 1500;
    ...
    // Max Buy TX amount is 0.48% of the total supply.
    uint256 private buyMaxTxPercent = 48; // Less fields to edit
    ...
    // Max Sell TX amount is 0.48% of the total supply.
    uint256 private sellMaxTxPercent = 48; // Less fields to edit
    ...
    uint256 public snipersCaught = 13;
    ...
    constructor (uint _snipeBlockAmt, uint256 _gasPriceLimit, address cInitializer) payable {
        ...
        _isFeeExcluded[_givewayWallet] = true;
        _isDividendExcluded[_marketingWallet] = true;
        _isDividendExcluded[_givewayWallet] = true;
    ...
    function transferOwner(address newOwner) external onlyOwner() {
        ...
        if (_givewayWallet == payable(_owner))
            _givewayWallet = payable(newOwner);
    ...
    function setDividendExcluded(address holder, bool enabled) private onlyOwner {
    ...
    function giveMeWelfarePlease() private {
    ...
    function setMaxTxPercents(uint256 buyPercent, uint256 buyDivisor, uint256 sellPercent, uint256 sellDivisor) external onlyOwner() {
        ...
        require(_sellMaxTxAmount >= (_tTotal / 1000) && _buyMaxTxAmount >= (_tTotal / 1000), "Max Transaction amts must be above 0.48% of total supply.");
    }
    ...
    function swapBack(uint256 numTokensToSwap) internal swapping {
        ...
        uint256 amountBNBMarketing = amountBNB - ((amountBNBLiquidity + amountBNBReflection)/2);
        uint256 amountBNBGiveway = amountBNB - ((amountBNBLiquidity + amountBNBReflection)/2);
    ...
    function manualDepost() private onlyOwner() {
