...
contract Faucet is Whitelist {
    ...
    constructor() public {
        faucetInfo.tokenAmount = 1000 * WEI_PRECISION;
    ...
    modifier checkAllowedToWithdraw(address _address, string memory _tokenSymbol) {
        require(_allowedToWithdraw(_address, _tokenSymbol), "withdrawal cooldown");
        _;
    }

    function _allowedToWithdraw(address _address, string memory _tokenSymbol)
        internal
        view
        returns (bool)
    {
        if (coolDown[_address][_tokenSymbol] == 0) {
            return true;
        } else if (block.timestamp >= coolDown[_address][_tokenSymbol]) {
            return true;
        }
        return false;
    }
    ...
    {
        usdt = faucetInfo.usdt.balanceOf(address(this));
        busd = faucetInfo.busd.balanceOf(address(this));
        return (usdt, busd);
    }
    ...
    function faucetSendBUSD(address payable _to)
        ...
        checkAllowedToWithdraw(_to, "busd")
    {
        ...
        coolDown[_to]["busd"] = block.timestamp + faucetInfo.waitTime;
    ...
    function faucetSendUSDT(address payable _to)
        ...
        payable
        avoidZeroAddress(_to)
        checkAllowedToWithdraw(_to, "usdt")
    {
        (uint256 usdtBalance, uint256 busdBalance) = getBalance();
        ...
        coolDown[_to]["usdt"] = block.timestamp + faucetInfo.waitTime;
