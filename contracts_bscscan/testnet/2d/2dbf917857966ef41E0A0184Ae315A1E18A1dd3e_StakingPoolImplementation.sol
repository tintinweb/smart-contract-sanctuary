...
contract StakingPoolImplementation is StakingPoolStorageStructure {
    ...
    */
    ...
    function getStakingReward(address _staker) 
        ...
        returns (uint256) 
    {
        uint256 reward = ClaimRewardLib.getStakingReward(
            predictions[_staker],
            lps
        );
    ...
    function getIndexedStakingReward(address _staker, uint256 _stakeIndex) 
        ...
        returns (uint256) 
    {
        uint256 reward = IndexedClaimRewardLib.getIndexedStakingReward(
            predictions[_staker], 
            _stakeIndex,
            lps
        );
     ...
     function getPrizeReward(address _staker)
        public
        view
        returns (uint256, uint256)
    {
        (uint256 reward, uint256 wrappedTokenReward) = ClaimRewardLib.getPrizeReward(
                predictions[_staker],
                lps,
                prizeRewardRates
            )
    ...
    function getIndexedPrizeReward(address _staker, uint256 _stakeIndex)
        public
        view
        returns (uint256, uint256)
    {
        (uint256 reward, uint256 wrappedTokenReward) = IndexedClaimRewardLib.getIndexedPrizeReward(
            predictions[_staker], 
            _stakeIndex,
            lps,
            prizeRewardRates
