...
contract BnbMiner {
	...
	uint256 constant public PROJECT_FEE = 5; // 5%; 
	...
	uint[6] public ref_bonuses = [25,5,5,5,5,10];
    
    
    uint256[7] public defaultPackages = [0.1 ether,0.25 ether,0.5 ether ,1 ether,2.5 ether,5 ether, 10 ether];
    
    
    mapping(uint256 => address payable) public singleLeg;
    uint256 public singleLegLength;
    uint[6] public requiredDirect = [1,2,3,4,5,6];

	address payable public admin;
    address payable public dev;
    address payable public marketing_charity;
  ...
  constructor(address payable _admin, address payable _dev, address payable _marketing_charity) public {
		require(!isContract(_admin));
		admin = _admin;
		dev = _dev;
		marketing_charity = _marketing_charity;
		singleLeg[0]=admin;
		singleLegLength++;
	}
    ...
    function invest(address referrer) public payable {
		...
		// 6 Level Referral
		   _refPayout(msg.sender,msgValue);
            ...
            uint256 _marketing_charity_fees = msg.value.mul(PROJECT_FEE).div(PERCENTS_DIVIDER);
           
            
            _safeTransfer(marketing_charity,_marketing_charity_fees);
  ...
  function withdrawal() external{
    ...
    _safeTransfer(dev,_fees.div(2));
    _safeTransfer(admin,_fees.div(2));
  ...
  function getEligibleWithdrawal(address _user) public view returns(uint8 reivest, uint8 withdrwal){
      ...
      uint256 TotalDeposit = users[_user].amount;
      if(users[_user].refs[0] >=4 && (TotalDeposit >=0.1 ether && TotalDeposit < 1 ether)){
          reivest = 60;
          withdrwal = 40;
          
      }else if(users[_user].refs[0] >=4 && (TotalDeposit >= 1 ether && TotalDeposit < 5 ether)){
          reivest = 50;
          withdrwal = 50;
      }
      else if(users[_user].refs[0] >=6 && (TotalDeposit >= 5 ether && TotalDeposit < 10 ether)){
          reivest = 40;
          withdrwal = 60;
