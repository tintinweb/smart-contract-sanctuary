...
contract Vault is Context, IBEP20, Ownable {
	...
	function lock(uint256 lockAmount, uint256 lockPeriod) external returns (bool) {
		...
		require( _balances[sender] >= lockAmount, "Vault: lockAmount exceeds balance");
		_balances[sender] = _balances[sender].sub(lockAmount, "BEP20: transfer amount exceeds balance");
		LockRecord memory lockRecord = LockRecord(lockAmount, block.timestamp, lockPeriod);
