 ...
 */
 contract Presale is Owned{
    ...
    // address of busd stablecoin
    ERC20 public busd;
    // buy rate
    uint public rate;
    // amount of wei raised
    uint public busdRaised;
    ...
    uint256 public minBusdLimit = 300 * (10 ** 18);
    uint256 public maxBusdLimit = 600 * (10 ** 18);
    ...
    constructor(uint256 _rate, address _wallet, ERC20 _busd, ERC20 _pToken, uint _closingTime, uint _vestedTime) {
        require(_rate > 0);
        require(_wallet != address(0));
        require(address(_busd) != address(0));
    ...
    function setVestedTime(uint256 _vestedTime) external onlyOwner {
        vestedTime = _vestedTime;    
    }

    // allows buyers to put their busd to get some pToken once the presale will closes
    function buy(uint _amount) public {
        require(isPresaleOpen, "Presale is not open yet");
        require(isList == false || whiteListed[msg.sender] == true);
        require(_amount >= minBusdLimit, "You need to sell at least some min amount");
        require(paidTotal[msg.sender] + _amount <= maxBusdLimit);
        ...
        uint256 pTokenAmount = _amount.mul(rate).div(RATE_DECIMALS);
        ...
        preBuys[msg.sender].busdAmount = preBuys[msg.sender].busdAmount.add(_amount);
        paidTotal[msg.sender] += _amount;
        emit TokenPurchase(
            msg.sender,
            _amount,
            pTokenAmount
    ...
    function getClaimableAmount(address _account) public view returns (uint) {
        ...
        uint totalpTokenToClaim = preBuys[_account].busdAmount.mul(rate).div(RATE_DECIMALS).mul(percentReleased).div(VESTING_TIME_DECIMALS);
        uint pTokenToClaim = totalpTokenToClaim.sub(preBuys[_account].pTokenClaimedAmount);
        return pTokenToClaim;
    }

    // allows pre-salers to redeem their pToken over time (vestedTime) once the presale is close
    function claim() public {
        require(!isPresaleOpen, "Presale is not over yet");
        require(isList == false || whiteListed[msg.sender] == true);
        uint percentReleased = getPercentReleased();

        uint totalpTokenToClaim = preBuys[msg.sender].busdAmount.mul(rate).div(RATE_DECIMALS).mul(percentReleased).div(VESTING_TIME_DECIMALS);
    ...
    // allows operator wallet to get the busd deposited in the contract
    function retreiveBUSD() public {
        require(msg.sender == wallet);
        require(!isPresaleOpen, "Presale is not over yet");
        busd.safeTransfer(wallet, busd.balanceOf(address(this)));
    }
