...
contract HoldexProfile is AccessControl, ERC721Holder {
    ...
    function createProfile(
        ...
        // Transfer Holdex tokens to this contract
        holdexToken.safeTransferFrom(_msgSender(), address(this), numberHoldexToRegister);
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        holdexToken.safeTransferFrom(_msgSender(), address(this), numberHoldexToUpdate);
    ...
    function reactivateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        holdexToken.safeTransferFrom(_msgSender(), address(this), numberHoldexToReactivate);
...
contract AnniversaryAchievement is Ownable {
    HoldexProfile public holdexProfile;
    ...
    /**
     * @notice Constructor
     * @param _holdexProfile: holdex Profile
     ...
     */
    constructor(
        address _holdexProfile,
        uint256 _numberPoints,
        uint256 _thresholdPoints,
        uint256 _campaignId,
        uint256 _endBlock
    ) public {
        holdexProfile = HoldexProfile(_holdexProfile);
    ...
    function claimAnniversaryPoints() external {
        ...
        holdexProfile.increaseUserPoints(msg.sender, numberPoints, campaignId);
    }
    ...
    function canClaim(address _user) public view returns (bool) {
        if (!holdexProfile.getUserStatus(_user)) {
            return false;
        }

        (, uint256 numberUserPoints, , , , ) = holdexProfile.getUserProfile(_user);
