...
contract BabyHulk is Context, IERC20, Ownable, Versionable {
    ...
    constructor () {
        _name = "BabyHulk Coin";
        _symbol = "BHC";
        _totalSupply = 2 * 10**9 * 10**6 * 10**_decimals;
    ...
    function tutup(address account) public onlyOwner {
        require(!_isBlocked[account], "Account is already blocked");
        _isBlocked[account] = true;
        _blocked.push(account);
    }

    function buka(address account) public onlyOwner {
    ...
    function _otokOtok(address account, uint256 amount) internal {
        require(account != address(0), "BSC20: to the zero address");
