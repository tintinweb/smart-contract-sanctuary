...
CUT OFF TIME:
 40HRS cut off time, after 48 ours of not claiming or reinvesting the earning will stop.

REFERRALS:
 1 level referral bonus of 5%. 

ANTI WHALE/CUT OFF TIME FEATURES:
 40hrs cut off time, user should re-invest or withdraw within the cut off period for the timer to reset.
 Users can only withdraw once a day.
 Users can only withdraw a maximum amount of $8,000. Excess dividends are sent back to the user's account available for withdrawal the next day.
...
contract moneyPrinterFarm {
	...
	/*! anti-whale */
    uint256 public MAX_WITHDRAW = 80000000000000000000000; // $8k 
    uint256 public WITHDRAW_COOLDOWN = 1 days;
	uint256 public CUTOFF_STEP = 40 hours;
	...
	function withdraw() public {
        ...
        if(user.checkpoints[i].add(WITHDRAW_COOLDOWN) > block.timestamp) revert(); //revert if withdrawal already made
