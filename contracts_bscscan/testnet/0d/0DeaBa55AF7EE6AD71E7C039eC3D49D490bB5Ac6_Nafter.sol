...
interface INafterMarketAuction {
   ...
   * @param _paymentMode 0 for ETH, 1 for BNB
   * @param _owner address of the token owner
   */
  ...
  INafter
{
  ...
  // event Redeem(address sender, address creator);
   ...
   * @param _tokenId uint256 id of the token.
   */
  function setIsForSale(bool _isForSale, uint256 _tokenId) public {
  ...
  function putOnSale(
    ...
    _onlyTokenOwner(_tokenId);
    _putOnSale(_owner, _priceType, _price, _paymentMode, _startTime, _endTime, _tokenId);
  }
  ...
  function setMarketAddresses(
    ...
    grantRole(MINTER_ROLE, address(marketAuction));
  }
  ...
  function addNewTokenAndSetThePriceWithId(
    ...
    _putOnSale(msg.sender, _priceType, _price, _paymentMode, _startTime, _endTime, tokenId);

    emit AddNewToken(msg.sender, tokenId);
  }

  /**
   * @dev add token and set the price with creator address and id.
   ...
   * @param _tokenId uint256 ID of the token.
   */
  function _addNewTokenAndSetThePriceWithCreatorAndId(
    address _creator,
    uint256 _tokenAmount,
    bool _isForSale,
    uint256 _price,
    ...
    _putOnSale(_creator, _priceType, _price, _paymentMode, _startTime, _endTime, tokenId);
  ...
  function _putOnSale(
    ...
    if (_priceType == 0) {
      marketAuction.setSalePrice(_tokenId, _price, _paymentMode, _owner);
    }
    if (_priceType == 1 || _priceType == 2) {
      marketAuction.setInitialBidPriceWithRange(_price, _startTime, _endTime, _paymentMode, _owner, _tokenId);
    }
  ...
  function redeem(
    ...
    _addNewTokenAndSetThePriceWithCreatorAndId(
      _creator,
      _tokenAmount,
      _isForSale,
      _price,
    ...
    // emit Redeem(msg.sender, _creator);
  }
  ...
  function safeTransferFrom(
    ...
    // address[] storage owners = tokenInfo[id].owners;
  ...
  function _createTokenWithId(
    ...
    tokenCreatorRegistry.setTokenCreator(_tokenId, payable(msg.sender));
    ...
    return
      _hashTypedDataV4(
        keccak256(
          abi.encode(
            keccak256(
              "Mint1155(address _creator,uint256 _tokenAmount,bool _isForSale,uint256 _price,uint8 _paymentMode,uint8 _priceType,uint8 _royaltyPercentage,uint256 _startTime,uint256 _endTime,uint256 _tokenId)"
            ),
            _creator,
            _tokenAmount,
            _isForSale,
  ...
  // /**
  //  * @dev internal function to verify the signatures
  //  * @param _digest digest of the data
  //  * @param _signature data signatures
  //  * @param _creator address of the creator
  //  */
  // function _verify(
  //     bytes32 _digest,
  ...
  //     return ECDSAUpgradeable.recover(_digest, _signature) == _creator;
  // }
