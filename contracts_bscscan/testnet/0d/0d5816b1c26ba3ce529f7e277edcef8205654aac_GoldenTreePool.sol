...
contract GoldenTreePool is UUPSUpgradeable, OwnableUpgradeable, IGoldenTreePool {
  ...
  /// @dev Stored previous BUSD balance
  ...
  // events 
  event Synced(uint256 oldBalance, uint256 newBalance, uint256 amount, address account);
  ...
  function sellSmtc(uint256 amount) external {
    ...
    syncRevenue(msg.sender);
  }

  /**
   * Sync total revenue or total swapped amount
   * This function should be called after transfer BUSD to this pool or
   * Swapped.
   */
  function syncRevenue(address account) public override onlyRewardsDistributor {
    IERC20 busdToken = comptroller.getBUSD();

    uint256 balance = busdToken.balanceOf(address(this));
    if(balance == storedBalance) {
      // didn't updated busd balance
      return;
    }
    
    uint256 amount = 0;
    if(balance > storedBalance) {
      amount = balance.sub(storedBalance);
      ...
      for(uint i = 0 ; i < growthShare.length; i++) {
        ...
        if(percent > 0 && ref != address(0x0)) {
          uint256 shareAmount = amount.mul(percent).div(10_000);
        ...
        ref = comptroller.getSmartLadder().sponsorOf(ref);
      }
    } 
    else {
      amount = storedBalance.sub(balance);
      
      totalSwapped = totalSwapped.add(amount);
      storedBalance = balance;
    }

    emit Synced(storedBalance, balance, amount, account);
  }
  ...
  function increaseGrowth(address account, uint256 amount) internal {
    ...
    ISmartAchievement arch = comptroller.getSmartAchievement();
    arch.notifyGrowth(account, old, newBalance);
  }
  ...
  function thresholdPrice() public view returns(uint256) {
    return storedBalance.mul(1e18).div(smtcTotalSupply());
  }
