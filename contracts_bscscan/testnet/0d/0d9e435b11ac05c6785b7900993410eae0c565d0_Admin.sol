...
contract Admin is Initializable {
    ...
    function set_jinr_ubi_tax(uint _ubi_tax, address _wallet) public onlyGovernor {
        jinr.setUbiTax(_ubi_tax, _wallet);
    }

    function set_jinr_transaction_tax(uint _tx_tax, address _wallet) public onlyGovernor {
        jinr.setUbiTax(_tx_tax, _wallet);
    }
    ...
    function set_jusd_ubi_tax(uint _ubi_tax, address _wallet) public onlyGovernor {
        jusd.setUbiTax(_ubi_tax, _wallet);
    }

    function set_jusd_transaction_tax(uint _tx_tax, address _wallet) public onlyGovernor {
        jusd.setUbiTax(_tx_tax, _wallet);
    }
...
interface IXBEP20 {
    ...
    function setTransactionTax(uint tx_tax, address wallet) external;
    ...
    function add_to_blacklist(address _addr) external view returns (uint256);

    function check_blacklist(address _addr) external view returns (uint256);

    function remove_from_blacklist(address _addr) external view returns (uint256);
    ...
    function set_active_status(uint flag) external view returns (uint256);
