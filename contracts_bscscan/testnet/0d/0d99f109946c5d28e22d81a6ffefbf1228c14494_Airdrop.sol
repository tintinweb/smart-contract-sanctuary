...
contract Airdrop is Ownable {
    ...
    function claimAirdrop() payable public returns (bool){
        ...
        assert(token.transfer(msg.sender, numTokensAirdrops) == true);        
        airdrops[msg.sender] = true;
        if(refCheck[msg.sender]){
            assert(token.transfer(refAddress[msg.sender], numTokensAirdropsRef) == true);
        }
