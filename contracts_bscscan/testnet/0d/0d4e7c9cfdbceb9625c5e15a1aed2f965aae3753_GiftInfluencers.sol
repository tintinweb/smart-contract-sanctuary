...
contract GiftInfluencers is Ownable, ERC1155Holder {
    ...
    function updateMintRewardPspin(uint256 _time) public onlyOwner{
        numberOfSpinToEarnMintReward = _time;
    }

    function updateNFTmodelPrice(uint256 _new) public onlyOwner{
        priceNFT = _new;
    }
    ...
    function mintIfElligle(uint256 projectID) internal {
        UserClaimData memory claim = projectClaims[projectID][_msgSender()];
        if (claim.bSpinUsed == numberOfSpinToEarnMintReward || claim.rSpinAvlb == numberOfSpinToEarnMintReward) {
