...
contract MoneyMaker {
    ...
    function withdrawble(uint stakeIndex) public view returns(uint256)
    {
        ...
        if(genTree[msg.sender] == address(0))
        {
            ...
            if(!userInfo[msg.sender].isactive){
              ...
              if(userInfo[_referredBy].isActiveStake){
                ...
                uint lastindex= NumberOfStakes(_referredBy);
      ...
      if(totaldaily>0)
      {
        ...
        for(uint i =1 ; i < percent_.length;i++)
        {
          ...
          if(refCount[ref] >= eligiblerefcount){
            amt=totaldaily * perc/100;
            checkedamt=checklimit(msg.sender,amt);
            if(checkedamt==amt){
