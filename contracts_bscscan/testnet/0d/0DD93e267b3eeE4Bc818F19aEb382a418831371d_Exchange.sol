...
contract Exchange is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function calculateLoanAmountAndExchangeRate(
        ...
        if (_col.collateralAddress == address(0)) {
            collateralToUSD =
                (uint256(RateBNBwithUSD()) * 10**10 * _pkg.loanToValue * _col.amount) / (100 * 10**5);
        } else {
            collateralToUSD =
                (uint256(getLatesPriceToUSD(_col.collateralAddress)) * 10**10 * _pkg.loanToValue * _col.amount) / (100 * 10**5);
        }
