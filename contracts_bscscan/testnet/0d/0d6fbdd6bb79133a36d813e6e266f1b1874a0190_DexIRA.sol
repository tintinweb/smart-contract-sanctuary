...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  // @note Router address for PancakeSwap v2 (LIVE) @note disabled
  // IUniswapV2Router02 public uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);

  // @note Router address for PancakeSwap (TESTNET) @note enabled
...
contract DexIRA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DexIRADividendTracker public dividendTracker;
    ...
    uint256 public maxSellTransactionAmount = 1000000000000000 * (10**18); // @note Updated to 1,000,000,000

    // uint256 public swapTokensAtAmount = 100 * 10 ** 9 * (10**9);
    uint256 public swapTokensAtAmount = 100000000000 * (10**18); // @note Updated to 1,000,000,000
    ...
    bool public transferDelayEnabled = false; // @note Disabled transfer delays
    ...
    constructor() ERC20("DexIRA_Testnet", "DEXIRA_TESTNET", 9) {

    	dividendTracker = new DexIRADividendTracker();
        ...
        */
        ...
        _mint(owner(), 1000000000 * (10**18)); // @note Updated to 1,000,000,000
    }
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
  	...
  	function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        DexIRADividendTracker newDividendTracker = DexIRADividendTracker(payable(newAddress));
...
contract DexIRADividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DEXIRA_Dividend_Tracker", "DEXIRA_Dividend_Tracker", 9) {
    	claimWait = 3600;
        // minimumTokenBalanceForDividends = 30 * 10 ** 9 * (10**9); // must hold 30,000,000,000+ tokens to get divs
        minimumTokenBalanceForDividends = 10000 * (10**18); // @note Updated to hold a minimum of 10K
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DEXIRA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "DEXIRA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DEXIRA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DEXIRA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DEXIRA_Dividend_Tracker: Cannot update claimWait to same value");
