...
contract Prediction is Pausable {
    ...
    constructor(
        ...
        addAdmin(address(0x4590F62A547188bA545Fcb7e8cf8F13556EFfD84));
    ...
    function claim(uint256 epoch) public {
        ...
        for (uint256 index = 0; index < round.listToken.length; index++) {
            ...
            round.endTimestamp + 100 * roundDuration <= block.timestamp) {
                info.claimed = true;
                _claimFund(amount, msg.sender, token);
                emit Claim(msg.sender, epoch, token, amount);
                return;
    ...
    (bool, Position, uint256, address[] memory, uint256[] memory, uint256[] memory, bool) {
        Round storage round = rounds[epoch];
        ...
        if (!info.participated) {
            return (false, Position.UP, 0, round.listToken, upAmount, downAmount, false);
        }
        ...
        for (uint256 index = 0; index < round.listToken.length; index++) {
            ...
            if (amount != 0) { // user predict with this token
                ...
                if ((info.position == Position.UP && round.result == Result.UP) ||
                    (info.position == Position.DOWN && round.result == Result.DOWN)) {
                    isWin = true;
                }
            }
        ...
        return (info.claimed, info.position, predictAmount, round.listToken, upAmount, downAmount, isWin);
    }
