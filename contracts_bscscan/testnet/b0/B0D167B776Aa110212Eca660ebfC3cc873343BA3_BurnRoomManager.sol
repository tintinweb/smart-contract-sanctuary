...
contract BurnRoomManager is SafeOwnable, Random {
    ...
    constructor(address _WETH, IPandoraERC1155 _nftToken, address _feeReceiver, address _rewardReceiver, address _linkAccessor) Random(_linkAccessor) {
        WETH = _WETH;
    ...
    function ownerClaim(uint _rid, uint256 _loop) external onlyOwner {
        ...
        SafeERC20.safeTransfer(room.token, rewardReceiver, amount); 
    }
