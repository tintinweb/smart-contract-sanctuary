...
contract RUSaleBox is Ownable, Pausable {
    ...
    function setBoxPlayer(string[] memory bronze, string[] memory silver, string[] memory gold) public {
        ...
        require(boxQuantity[2] * playerQuantity[2] == silver.length, "box quantity and player per bronze box not match");
        require(boxQuantity[3] * playerQuantity[3] == gold.length, "box quantity and player per bronze box not match");
    ...
    function buyBox(uint256 _type) external whenNotPaused {
        require(currentBoxIndex[1] != 99999999, "Box is not ready!");
        ...
        emit BoxSold(round, _type, currentBoxIndex[_type], msg.sender);
    }
