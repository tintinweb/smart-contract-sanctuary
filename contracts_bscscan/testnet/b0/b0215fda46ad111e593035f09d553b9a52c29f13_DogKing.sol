...
contract Ownable is Context {
    ...
    constructor () internal {
        ...
        _owner = 0x8aBF4b96A8eBA70353921CaefEE710Cfcd1e0Cd2;
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    ...
    contract DogKing is Context , IERC20, Ownable {
    ...
    string private _name = "DogKing";
    string private _symbol = "DogKing";
    ...
    address public MarketingWallet = 0x93fDE51A9043e47E2cEfBA51D555d3d9D07b718A; 
    uint256 private _previousMarketingFee = _marketingFee;

    uint256 public _partyFee = 1;
     address public Partywallet=  0xB83195e7d62936714d9e14D8396fac6509374eE0;
     uint256 private _previousPartyFee =  _partyFee ; 
    ...
    uint256 public _maxTxAmount = 1000000000000000 * 10**9; //1% of total supply   //ANTIDUMP
    uint256 private numTokensSellToAddToLiquidity = 2430  * 10**9;
    uint256 public _maxWalletToken = 1000000000000000 * 10**9;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 PartyAmt = amount.mul(_partyFee).div(100);

        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(MarketingAmt).sub(PartyAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(MarketingAmt).sub(PartyAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(MarketingAmt).sub(PartyAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(MarketingAmt).sub(PartyAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(MarketingAmt).sub(PartyAmt)));
        }
        ...
        _transferStandard (sender,Partywallet ,PartyAmt );
    ...
    function enableAllFees() external onlyOwner() {
        ...
        _partyFee = 2;
        _previousPartyFee =_partyFee;
    }

function setMarketingFeePercent (uint256 marketingFee)
    external onlyOwner ()  {
    ...
    function setPartyFeePercent (uint256 partyFee)
    external onlyOwner () {
        _partyFee = partyFee ;
    ...
    function setMarketingWallet(address newWallet) external onlyOwner() {
     ...
     function setPartywallet(address newWallet) external onlyOwner() {
        Partywallet = newWallet;
    }
