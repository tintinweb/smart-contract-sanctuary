...
contract FactoryMigrationHelper {
  ...
  mapping(address => bool) public migrationExistence;
	...
	function sendNewTokenToMigration(address migrationHelper, uint256 amount) external {
		MigrationHelper mh = MigrationHelper(migrationHelper);
		IERC20(mh.newTokenAddress())
				.safeTransferFrom(msg.sender, migrationHelper, amount);
...
contract MigrationHelper {
  using SafeERC20 for IERC20;
	...
	uint256 balance = IERC20(oldTokenAddress).balanceOf(msg.sender);
	IERC20(oldTokenAddress).safeTransferFrom(msg.sender, address(this), balance);
