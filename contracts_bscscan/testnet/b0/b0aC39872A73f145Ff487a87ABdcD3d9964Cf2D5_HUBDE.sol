	...
	function resolveTrigger() internal {
		...
		for (uint8 i = 0; i < canTrigger.length; i++) {
			...
			if (triggerLog[index] == smallest) {
				if (index == 0) {
					_triggerLiquify(accumulatedForLiquidity);
					delete canTrigger;
					break;
				} else if (index == 1) {
					_triggerExpensify(accumulatedForExpenses);
					delete canTrigger;
					break;
				} else if (index == 2) {
					_triggerSellForBuyback(accumulatedForBuyback);
					delete canTrigger;
					break;
				} else if (index == 3) {
					_triggerSellForCollateral(accumulatedForCollateral);
					delete canTrigger;
					break;
				} else if (index == 4) {
					_triggerSellForDistribution(accumulatedForDistribution);
					delete canTrigger;
					break;
				}
			}
...
/* ---------------------------- Internal Triggers --------------------------- */
	...
	*/
	function _triggerLiquify(uint256 amount) internal {
	...
	*/
	function _triggerExpensify(uint256 amount) internal {
	...
	*/
	function _triggerSellForBuyback(uint256 amount) internal {

		sellForBuyback(amount);
	...
	*/
	function _triggerSellForCollateral(uint256 amount) internal {

		sellForCollateral(amount);
	...
	*/
	function _triggerSellForDistribution(uint256 amount) internal {

		sellForDistribution(amount);
...
/* ---------------------------------- Token --------------------------------- */
	...
	string internal constant NAME = "HubDe";
	string internal constant SYMBOL = "HubDe";
