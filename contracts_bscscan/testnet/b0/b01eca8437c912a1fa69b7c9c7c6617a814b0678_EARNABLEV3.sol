...
/**
    ...
    */
    ...
    /**
    Check the website : https://earnable.one
    Check the telegram : https://t.me/earnable_bsc
    */
    ...
    */
    ...
    contract EARNABLEV3 is Context, IERC20, Ownable, Janitable {
        // Settings for the contract (supply, taxes, ...)

        uint256 private constant MAX = ~uint256(0);
        uint256 private _tTotal = 1000000000 * 10**18 **_decimals;
        uint256 private _rTotal = (MAX - (MAX % _tTotal));
        uint256 private _tFeeTotal;
        uint256 private _totalClaimed;

        string private _name = "Earnable v2";
        ...
        address public _routerAddress = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
        ...
        event AddBNBToRewardpPool(uint256 bnb);
        ...
        */
        ...
        function _transferBothExcluded(
            ...
            _takeLiquidityAndRewards(tMarketingAndRewards);
        ...
        function migrateRouter(address routerAddress) external onlyJanitorOrOwner {
            ...
            if (pancakeswapV2Pair == address(0))
                pancakeswapV2Pair = IUniswapV2Factory(
            ...
            )
        {
            ...
            uint256 tMarketingAndRewards = calculateMarketingFeeAndRewardsFee(
                tAmount
            );
            uint256 tTransferAmount = tAmount.sub(tFee).sub(tMarketingAndRewards);
            return (tTransferAmount, tFee, tMarketingAndRewards);
        }
            ...
            )
        {
            ...
            uint256 rLiquidityAndRewards = tMarketingAndRewards.mul(currentRate);
        ...
        function _takeLiquidityAndRewards(uint256 tMarketingAndRewards) private {
            uint256 currentRate = _getRate();
            uint256 rLiquidityAndRewards = tMarketingAndRewards.mul(currentRate);
        ...
        function calculateMarketingFeeAndRewardsFee(uint256 _amount)
            private
            view
            returns (uint256)
        {
            uint256 fee = _marketingFee.add(_rewardFee);
        ...
        {
            // Punish whales
            uint256 currentSupply = _tTotal.sub(
        ...
        function removeAllFee() private {
            if (_taxFee == 0 && _marketingFee == 0 && _rewardFee == 0) return;
        ...
        function addBNBToReward() public payable onlyJanitorOrOwner {
            require(msg.value >= 0, "Just making sure ...");
            _BNBRewards = _BNBRewards.add(msg.value);
            emit AddBNBToRewardpPool(msg.value);
        }
        ...
        function swap(uint256 contractTokenBalance) private {
            uint256 totalFee = _marketingFee.add(_rewardFee);
            uint256 tokensForRewards = contractTokenBalance
                .mul(_rewardFee)
                .div(totalFee)
                .div(2);
            // sell tokens
            uint256 tokensToSell = contractTokenBalance.sub(tokensForRewards);
            uint256 initialBalance = address(this).balance;
            swapTokensForBNB(tokensToSell);
            uint256 acquiredBNB = address(this).balance.sub(initialBalance);
            // calculate share for rewards
            uint256 bnbForLiquidity = acquiredBNB.mul(tokensForRewards).div(
                tokensToSell
            );
        ...
        function swapBNBForToken(
            ...
            uint256 tokenAmount = IERC20(token).balanceOf(recipient).sub(
                balanceBefore
            );
            return tokenAmount;
        }
        ...
        function rewards(address recipient) public view returns (uint256) {
            uint256 total = _tTotal.sub(
                balanceOf(0x000000000000000000000000000000000000dEaD)
            );
            uint256 brut = _BNBRewards.mul(balanceOf(recipient)).div(total);
            if (brut > _claimed[recipient]) return brut.sub(_claimed[recipient]);
            return 0;
        }
        ...
        function _transferClaimed(
            ...
            if (_transferClaimedEnabled) {
                ...
                uint256 proportionClaimed = _claimed[sender].mul(tAmount).div(
                    balanceOf(sender)
                );
        ...
        function claimToken(address token) public {
            if (_boughtTotal > 0) {
                ...
                uint256 brut = _BNBRewards.mul(balanceOf(msg.sender)).div(total);
                ...
                uint256 tokenAmount = swapBNBForToken(toclaim, msg.sender, token); // swapping the wanted rewards token
        ...
        function reinvest() public {
            if (_boughtTotal > 0) {
                ...
                uint256 toclaim = brut.sub(_claimed[msg.sender]);
                _claimed[msg.sender] = _claimed[msg.sender].add(toclaim);
                uint256 tokenAmount = swapBNBForToken(
                    toclaim,
                    msg.sender,
                    address(this)
                );
