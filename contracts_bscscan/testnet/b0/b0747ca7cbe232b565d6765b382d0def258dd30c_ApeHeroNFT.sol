...
contract ApeHeroNFT is Ownable, ERC721, ERC721Enumerable {
    ...
    ERC721(name, symbol)
    {
        ...
        mintToken(owner(), marketingMint, 0,1);
    }
    ...
    modifier whenMintingAllowed() {
        require(_mintingStarted, "Minting not started or sold-out");
        _;
    }
    ...
    function teamPayout() public onlyOwner {
        //payable(owner()).transfer(balanceOf(this));
    }
    ...
    function airdropMint(address recipient, uint amount) public onlyOwner {
        mintToken(recipient, amount, 0,1);
    }
     ...
     */
    function mintToken(address recipient, uint amount, uint payment, uint _nftIndex) internal {
        require(amount == 1);
     ...
     */
    function mint(uint amount, uint _nftIndex) public payable whenMintingAllowed {
        require(msg.value >= _actualMintPrice(), "Mint failed");
        mintToken(_msgSender(), amount, msg.value, _nftIndex);
    }
