...
contract UNWSwapAgentImpl is Context {
    ...
    event SwapFilled(address indexed erc20Addr, string indexed bscTxHash, address indexed toAddress, uint256 amount);
    ...
    function fillBSC2ETHSwap(string calldata bscTxHash, address erc20Addr, address payable toAddress, uint256 amount) onlyOwner external returns (bool) {
        ...
        if (erc20Addr == address(0)) {
            require(address(this).balance >= amount, "not enough UNW");
    ...
    function swapETH2BSC(address erc20Addr, uint256 amount) payable external notContract returns (bool) {
        ...
        if (erc20Addr == address(0)) {
            require(msg.value == amount + swapFee, "swap fee not equal");
