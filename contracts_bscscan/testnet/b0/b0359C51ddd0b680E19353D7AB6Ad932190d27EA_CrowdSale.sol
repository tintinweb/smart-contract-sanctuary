...
contract CrowdSale is Context, Ownable {
    ...
    event LaunchPhase(uint256 phaseActivated, uint256 tokenPrice, uint256 phaseTokenAllocation, uint256 timeActivation);
    event BuyToken(address buyer, uint256 amount);

    // FREETOKER : 0x642703fFfcD6F040A0889843C1c31756849eDe84
    constructor (IBEP20 _freeToker, address payable wallet) public {
        ...
        priceFeeder[0x0003358a89EdFFc08DeB4f2233F79E947422fe39] = 0x4046332373C24Aed1dC8bAd489A04E187833B28d;

        currentPhase = 0;

        Phase storage phase = soldTokenOnEachPhase[currentPhase];
        phase._startTime = block.timestamp;

        soldTokenOnEachPhase[currentPhase] = phase;

        launchSale(phaseWisePrice[currentPhase]);
    ...
    function buyToken(address _purchaseTokenAddress, uint256 _amount) public {
        ...
        uint256 priceInUSD = uint256(getLatestPrice(priceFeed)).mul(_amount).div(10**17);

        uint256 buyingAmount = priceInUSD.div(tokenPrice).mul(10**17);
        ...
        if(totalSoldToken >= phaseTokenAllocation[currentPhase] ) {
        ...
        emit BuyToken(_msgSender(), buyingAmount);
    }
    ...
    fallback() external payable { 
        ...
        uint256 priceInUSD = uint256(getLatestPrice(priceFeed)).mul(sendBNB).div(10**17);
    ...
    function launchSale(uint256 _tokenPrice) internal {
        ...
        emit LaunchPhase(currentPhase, tokenPrice, phaseTokenAllocation[currentPhase], soldTokenOnEachPhase[currentPhase]._startTime);
    }
    ...
    function getPhase(uint256 _phaseId) public view returns (uint256, uint256, uint256, uint256) {
        Phase storage phase = soldTokenOnEachPhase[_phaseId];
        return(phase._startTime, phase._endTime, phase._soldToken, phaseWisePrice[_phaseId]);
    }
