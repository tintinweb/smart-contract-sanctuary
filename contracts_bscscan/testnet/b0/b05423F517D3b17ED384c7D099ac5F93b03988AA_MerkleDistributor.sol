...
contract MerkleDistributor is IMerkleDistributor {
    ...
    function claimtest(uint256 index, address account, uint256 amount) public view returns (bytes32){
        bytes32 node = keccak256(abi.encode(index, account, amount));
        return node;
    }

    function claimtestproof(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof, bytes32 root) public view returns (bool){
        bytes32 node = keccak256(abi.encode(index, account, amount));
        return MerkleProof.verify(merkleProof, root, node);
    }

    function claimtestpacked(uint256 index, address account, uint256 amount) public view returns (bytes32){
        bytes32 node = keccak256(abi.encodePacked(index, account, amount));
        return node;
    }

    function claimtestpackedproof(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof, bytes32 root) public view returns (bool){
        bytes32 node = keccak256(abi.encodePacked(index, account, amount));
        return MerkleProof.verify(merkleProof, root, node);
    }
