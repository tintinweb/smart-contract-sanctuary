...
contract Swap is ISwap, SwapCore, AccessControlEnumerable, ReentrancyGuard {
    ...
    address payable public _paymentAddress;
    ...
    */
    constructor(address payable paymentSplitterAddress, uint lastSwapId) {
        _paymentAddress = paymentSplitterAddress;
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(ADMIN_ROLE, msg.sender);
        _setupRole(SWAP_ADMIN_ROLE, msg.sender);

        for (uint currentId; currentId <= lastSwapId; currentId++) {
    ...
    // See SwapCore.sol
    function addSwap(address itemToken_, uint256 fee_, uint256 supply_, uint256 tokenType_) external onlyRole(SWAP_ADMIN_ROLE) {
        SwapStorage.Core memory core = _addSwap(itemToken_, fee_, supply_, tokenType_);
        swapMetadata[_swapCounter.current()] = core;
        _swapCounter.increment();
    }
    
    // See SwapCore.sol
    function addERC20Swap(address currencyToken_, address itemToken_, uint256 fee_, uint256 supply_, uint256 tokenType_) external onlyRole(SWAP_ADMIN_ROLE) {
        SwapStorage.Core memory core = _addERC20Swap(currencyToken_, itemToken_, fee_, supply_, tokenType_);
        swapMetadata[_swapCounter.current()] = core;
        _swapCounter.increment();
    }

    // See SwapCore.sol
    function setSwapFee(uint256 swapId_, uint256 fee_) external onlyRole(SWAP_ADMIN_ROLE) {
        SwapStorage.Core storage core = _getSwap(swapId_);
        _setFee(core, fee_);
    }

    // See SwapCore.sol
    function addSupply(uint256 swapId_, uint256 amount_) external onlyRole(SWAP_ADMIN_ROLE) {
        SwapStorage.Core storage core = _getSwap(swapId_);
        _addSupply(core, amount_);
    }

    // See SwapCore.sol
    function enableSwap(uint256 swapId_) external onlyRole(SWAP_ADMIN_ROLE) {
        SwapStorage.Core storage core = _getSwap(swapId_);
        _enableSwap(core);
    }

    // See SwapCore.sol
    function disableSwap(uint256 swapId_) external onlyRole(SWAP_ADMIN_ROLE) {
        SwapStorage.Core storage core = _getSwap(swapId_);
        _disableSwap(core);
    }
    ...
    function swapTokens(uint256 swapId_, uint256 amount_) external payable nonReentrant {
        ...
        IERC721Swap itemToken = core.tokenAddress;
        uint256 tokenType = core.tokenType;
        ...
        itemToken.safeMint(msg.sender, address(this), swapId_, amount_, tokenType);
...
/**
 * @title ERC-721 Non-Fungible Token Standard, optional type enumeration extension
 * @dev See https://eips.ethereum.org/EIPS/eip-721
 */
interface IERC721EnumerableType {
    /**
     * @dev Returns the attached type on a token with id `tokenId_`
     * @param tokenId_ Token id with an attached type
     * @return tokenType
     */
    function tokenType(uint256 tokenId_) external view returns (uint256);

    /**
     * @dev Returns all tokens with an attached type of `tokenType_`
     * @param tokenType_ Type of the token to search for
     * @return tokens
     */
    function tokensByType(uint256 tokenType_)
        external
        view
...
import "./extensions/IERC721EnumerableType.sol";

interface IERC721Swap is IERC721, IERC721EnumerableType, IERC721Enumerable {
...
import "../token/ERC721Swap/IERC721Swap.sol";
import "./Shared/SwapStorage.sol";
...
contract SwapCore {
    ...
    */
    ...
    /** 
        ...
        @param tokenAddress_ Address of the ERC721 to be paired with ETHER
    ...
    */
    function _addSwap(address tokenAddress_, uint256 fee_, uint256 supply_, uint256 tokenType_) internal returns(SwapStorage.Core memory) {
        IERC20 currencyToken = IERC20(address(this));
        IERC721Swap tokenAddress = IERC721Swap(tokenAddress_);

        SwapStorage.Core memory swap = SwapStorage.Core(currencyToken, tokenAddress, true, fee_, supply_, 0, tokenType_, false, true);
        emit AddSwap(swap);
    ...
    /** 
        ...
        @param tokenAddress_ Address of the ERC721 to be paired with the ERC20 token
    ...
    */
    function _addERC20Swap(address currencyToken_, address tokenAddress_, uint256 fee_, uint256 supply_, uint256 tokenType_) internal returns(SwapStorage.Core memory) {
        IERC20 currencyToken = IERC20(currencyToken_);
        IERC721Swap tokenAddress = IERC721Swap(tokenAddress_);

        SwapStorage.Core memory swap = SwapStorage.Core(currencyToken, tokenAddress, false, fee_, supply_, 0, tokenType_, false, true);
        emit AddSwap(swap);
    ...
    */
    function _enableSwap(SwapStorage.Core storage coreData) internal {
        ...
        emit SwapEnable(address(coreData.currencyToken), address(coreData.tokenAddress), coreData.isEnabled);
    }
    ...
    */
    function _disableSwap(SwapStorage.Core storage coreData) internal {
        ...
        emit SwapDisable(address(coreData.currencyToken), address(coreData.tokenAddress), coreData.isEnabled);
    }
}
