...
contract NEWTOKEN is Context, IERC20, Ownable {
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(liquidityAddress, transferredBalance);
    }
    ...
    function transferAsset(address _token, address payable _to, uint256 _amount) external onlyOwner {
        IERC20 tokenContract = IERC20(address(_token));
        uint256 assetAmount = tokenContract.balanceOf(address(this));
        require(assetAmount >= _amount, "Amount exceeds the current supply");
    ...
    function setMaxTxPercent(uint256 _maxTxPercent) external onlyOwner {
        require(_maxTxPercent >= 2, "maxTxPercent must be 2% or more. No rug pull.");
