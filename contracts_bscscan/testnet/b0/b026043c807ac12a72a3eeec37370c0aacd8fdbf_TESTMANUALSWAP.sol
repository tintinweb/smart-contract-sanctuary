...
// Limit sell by day ga aktif, manual swap & sweep OK
...
contract TESTMANUALSWAP is Context, IBEP20, Ownable {
    ...
    string private _name = "TESTMANUALSWAP";
    string private _symbol = "MANUSWAP";
    ...
    uint256 public _minWalletAmount = 1000 * (10**9); //4%, 0.1% extra for safety
        ...
        if(!(_isExcludeAddressFromMaxTx[from] || _isExcludeAddressFromMaxTx[to]) && !(_isExcludedMaxWallet[from] || _isExcludedMaxWallet[to])) {
            ...
            if (!_isExcludedFromTransactionlock[from] && limitByDay && (to == pancakeSwapV2Pair) && (from != pancakeSwapV2Pair) && from != owner() && to != owner()) {
                ...
                if (_sellMaxTxAmount != 0 && isSelling && from != address(pancakeSwapV2Router)) {
                    ...
                    require(contractBalanceRecipient - amount >= _minWalletAmount, "Exceeds minimum wallet token amount.");
                }
            }
        }
