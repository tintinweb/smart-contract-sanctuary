...
contract HOURS48 is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 48000000 * 10**3 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public maxBuyAmount = 48000000 * 10**3 * 10**9;
    uint256 public maxSellAmount = 48000000 * 10**3 * 10**9;
    uint256 public swapTokensAtAmount = 1000 * 10**3 * 10**9;
    uint256 public buyBackUpperLimit = 1 * 10**18;

    address public giveawaysAddress = 0x5B6B5aeD6B0D1A294D11b829D68fDbaA389cD376; // Giveaway waller
    address public marketingAddress = 0x5B6B5aeD6B0D1A294D11b829D68fDbaA389cD376; // Marketing wallet
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[giveawaysAddress] = true;
    ...
    function setFeeRates(uint256 _rfi, uint256 _giveaways, uint256 _marketing, uint256 _liquidity, uint256 _buyback) external onlyOwner {
        feeRates.rfi = _rfi;
        feeRates.giveaways = _giveaways;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _giveaways, uint256 _marketing, uint256 _liquidity, uint256 _buyback) external onlyOwner{
        sellFeeRates.rfi = _rfi;
        sellFeeRates.giveaways = _giveaways;
    ...
    function _takeGiveaway(uint256 rGiveaway, uint256 tGiveaway) private {
        totFeesPaid.giveaways +=tGiveaway;
        if(_isExcluded[address(this)]){
             _tOwned[address(this)]+=tGiveaway;
        }
        _rOwned[address(this)] +=rGiveaway;
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rGiveaway,to_return.rMarketing, to_return.rLiquidity, to_return.rBuyback) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tGiveaway = tAmount*sellFeeRates.giveaways/1000;
            ...
            s.tTransferAmount = tAmount-s.tRfi-s.tGiveaway-s.tMarketing-s.tLiquidity-s.tBuyback;
        }
        else{
            s.tRfi = tAmount*feeRates.rfi/1000;
            s.tGiveaway = tAmount*feeRates.giveaways/1000;
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rGiveaway, uint256 rMarketing, uint256 rLiquidity, uint256 rBuyback) {
        ...
        rGiveaway = s.tGiveaway*currentRate;
        rMarketing = s.tMarketing*currentRate;
        rLiquidity = s.tLiquidity*currentRate;
        rBuyback = s.tBuyback*currentRate;
        rTransferAmount =  rAmount-rRfi-rGiveaway-rMarketing-rLiquidity-rBuyback;
        return (rAmount, rTransferAmount, rRfi,rGiveaway,rMarketing,rLiquidity, rBuyback);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        ...
        _takeGiveaway(s.rGiveaway,s.tGiveaway);
        _takeMarketing(s.rMarketing,s.tMarketing);
        _takeLiquidity(s.rLiquidity,s.tLiquidity);
        _takeBuyback(s.rBuyback, s.tBuyback);
        emit Transfer(sender, recipient, s.tTransferAmount);
        emit Transfer(sender, address(this), s.tLiquidity + s.tGiveaway + s.tMarketing + s.tBuyback);
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        uint256 denominator= (feeRates.liquidity + feeRates.buyback + feeRates.giveaways + feeRates.marketing) * 2;
        ...
        // Send BNB to giveawaysWallet
        uint256 giveawaysAmt = unitBalance * 2 * feeRates.giveaways;
        if(giveawaysAmt > 0){
          payable(giveawaysAddress).transfer(giveawaysAmt);
        }
    ...
    function updateGiveawayWallet(address newWallet) external onlyOwner{
        require(giveawaysAddress != newWallet ,'Wallet already set');
        giveawaysAddress = newWallet;
        _isExcludedFromFee[giveawaysAddress];
    }
