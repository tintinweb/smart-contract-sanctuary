...
contract ONEi is Context, IERC20, Ownable {
    ...
    string private _name = "One Island";
    string private _symbol = "ONEi";
    ...
    uint256 public islandDivisor = 5;
    address payable public marketingWallet = payable(0x787AAe22aeA1dCC22fb229d80Fda923508d0E891);
    address payable public islandWallet = payable(0x0f71b877BE22f28507700D7a6359ae4088775E4a);
    ...
    function _transfer(
        ...
        if (
            ...
            //add liquidity and send bnb to marketing address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 teamEthShare = transferredBalance.div(_marketingFee).mul(islandDivisor);
        uint256 marketingEthShare = transferredBalance.sub(teamEthShare);
        
        if(teamEthShare > 0) {
            transferToAddressETH(islandWallet, transferredBalance.div(_marketingFee).mul(islandDivisor));
        }
        
        if(marketingEthShare > 0) {
            transferToAddressETH(marketingWallet, transferredBalance.sub(teamEthShare));
        }
    ...
    function setislandWallet(address payable newWallet) external onlyOwner() {
        islandWallet = newWallet;
    }
    ...
    function setMarketingAndIslandFeePercent(uint256 marketingFee, uint256 _islandDivisor) external onlyOwner() {
        require(islandDivisor < marketingFee || islandDivisor == 0);
        _marketingFee = marketingFee;
        islandDivisor = _islandDivisor;
    }
