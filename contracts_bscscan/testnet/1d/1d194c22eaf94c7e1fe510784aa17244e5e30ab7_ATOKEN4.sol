...
contract ATOKEN4 is IBEP20, Auth {
    ...
    string constant _name = "ATOKEN4";
    string constant _symbol = "ATOKEN4";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 1000000000000000 * 10 ** 18;
    uint256 public _maxTxAmount = _totalSupply.div(200); // 0.5% of max supply
    uint256 public _maxWalletToken = _totalSupply.div(40); // 2.5% of max supply
    ...
    uint256 public totalFee = liquidityFee + buybackFee + gasWalletFee + reflectionFee + marketingFee;
    ...
    function getMultipliedFee() public view returns (uint256) {
        if (launchedAtTimestamp + 1 days > block.timestamp) {
            return totalFee.mul(12500).div(feeDenominator);
        } else if (
    ...
    function setFees(
        ...
        totalFee = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee).add(gasWalletFee);
