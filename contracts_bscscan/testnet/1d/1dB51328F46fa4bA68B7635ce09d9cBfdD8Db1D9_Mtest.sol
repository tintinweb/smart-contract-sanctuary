...
contract Mtest is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MtestDividendTracker public dividendTracker;

    address public liquidityWallet;

    address payable public operations1Address =
        payable(0xed52346d0556687e77861b21d4Cb44E1fb4A0Fc1);
    address payable public operations2Address =
        payable(0xc10607a269A1F3B9fB5Ad4c762c588a33EBDB200);
    ...
    constructor() ERC20("Mtest", "MTEST") {
        dividendTracker = new MtestDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        MtestDividendTracker newDividendTracker = MtestDividendTracker(
            payable(newAddress)
        );
...
contract MtestDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(mTest);

            uint256 prevBalance = mTest.balanceOf(address(this));
            ...
            {
                uint256 received = mTest.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    mTest.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
