...
contract WhySell is Context, IERC20, Ownable, Janitable {
    ...
    string private _name = "WhySelL";
    string private _symbol = "WSL";
    ...
    event AddeWSLReward(uint256 bnb);
    ...
    function swap(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (tokensForLiquidity < contractTokenBalance) {
            ...
            uint256 acquireWSL = address(this).balance.sub(initialBalance);
            // calculate share for liquidity or rewards 
            uint256 bnbForLiquidity = acquireWSL.mul(tokensForLiquidity).div(tokensToSell);
            uint256 bnbForRewards = acquireWSL.sub(bnbForLiquidity);
            // update rewards
