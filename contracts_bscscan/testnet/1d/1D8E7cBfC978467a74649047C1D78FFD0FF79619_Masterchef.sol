...
contract Masterchef is Ownable {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            fixedAPR.add(rwdReward.mul(1e12).div(lpSupply));
        }
        //return user.amount.mul(accRewardPerShare).div(1e12).sub(user.rewardDebt);
        return user.amount.mul(fixedAPR).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
       ...
       uint256 depositedAmount = pool.depositedAmount;
        ...
        fixedAPR.add(rwdReward.mul(1e12).div(depositedAmount));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
           // uint256 pending = user.amount.mul(pool.accRewardPerShare).div(1e12).sub(user.rewardDebt);
            uint256 pending = user.amount.mul(fixedAPR).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(fixedAPR).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        if(block.timestamp > user.userLockedPeriod){
        ...
        user.rewardDebt = user.amount.mul(fixedAPR).div(1e12);
         ...
         _amount = user.amount.div(2);
        pool.depositedAmount = pool.depositedAmount.sub(user.amount);
        pool.lpToken.safeTransfer(address(msg.sender), _amount);
        //burn
        pool.lpToken.safeTransfer(burnAddress, _amount);
