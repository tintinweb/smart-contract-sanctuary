...
contract StrayInu is Context, IERC20, Ownable {
    ...
    uint256 public _burnRate = 80;

    string private _name = "StrayInu";
    string private _symbol = "STRAYINU";
    ...
    event NumTokensSellToAddToLiquidityUpdated(uint256 amount);
    ...
    {
        require(
            tAmount <= _tTotal.sub(_tBurnTotal),
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = _tTotal.sub(_tBurnTotal).mul(maxTxPercent).div(10**2);
    }
    ...
    function setNumTokensSellToAddToLiquidity(uint256 amount)
        ...
        onlyOwner
    {
        require(amount >= 10**9, "amount too low");
        numTokensSellToAddToLiquidity = amount;
        emit NumTokensSellToAddToLiquidityUpdated(amount);
    }
    ...
    function _getCurrentSupply() private view returns (uint256, uint256) {
        uint256 tTotal = _tTotal.sub(_tBurnTotal);
    ...
    function calculateBurn(uint256 _amount) private view returns (uint256) {
        uint256 tBurn = _amount.mul(_burnFee).div(10**3);
        return Math.min(tBurn, burnAmount());
    }

    function burnAmount() private view returns (uint256) {
        return _tTotal.mul(_burnRate).div(10**2).sub(_tBurnTotal);
    }
