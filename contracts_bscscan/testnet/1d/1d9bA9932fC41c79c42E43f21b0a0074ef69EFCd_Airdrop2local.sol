...
contract Airdrop2local is Ownable {
    using SafeMath for uint256;
    ...
    address public wallet = 0xC72B225a8A14381811269a5DdD5C9aD17d925a25;
    ...
    function userBuy2LC(address _user, uint256 _amount) public onlyRouter {
        ...
        if (_amount > 0) {
            buyerInfo[_user] = _amount;
        }
    }

    function userSell2LC(address _user, uint256 _amount) public onlyRouter {
        if (isBuyer[_user]) {
            if (buyerInfo[_user] > _amount) {
                buyerInfo[_user] = buyerInfo[_user].sub(_amount);
            }
        }
    }
