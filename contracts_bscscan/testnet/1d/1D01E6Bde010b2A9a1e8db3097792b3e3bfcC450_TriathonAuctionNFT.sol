...
contract TriathonAuctionNFT is Ownable, IERC721Receiver {
    ...
    function create(uint tokenId, uint amount) public {
    ...
    function cancel(uint poolId) public {
        _cancel(poolId);
    }
    ...
    function bid(uint poolId) external isPoolOpen(poolId) {
        ...
        //BidInfo memory lastBid;
        
        uint bidderNumber = bids[poolId].length;
        /*if (bidderNumber != 0) {
            lastBid = bids[poolId][bidderNumber - 1];
        }*/
        
        // 1. transfer token0 to this
        address token1 = pool.token1;
        uint token1Amount = pst.currPrice.add(pst.currPrice.mod(10));
        ...
        // 5. transfer reward to every participator
        ...
        if (bidderNumber != 0) {
            IERC20(token1).transfer(pst.lastBidder, lastBidderReward);      // to last bidder 16%
        } else {
    ...
    modifier isPoolFinished(uint poolId) {
        require(poolId < pools.length && getPoolStatus(poolId) == POOL_CLOSED,
            "this pool is not finished");
        _;
    }
