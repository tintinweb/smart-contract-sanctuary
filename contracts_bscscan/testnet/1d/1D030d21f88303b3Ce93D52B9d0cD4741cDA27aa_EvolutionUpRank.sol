...
contract BPLUSToken is Context, IBEP20, Ownable , Pausable {
...
import "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";
import "./BPLUSToken.sol";
import "./CharacterNFT.sol";
import "./AcademyUpStar.sol";

contract EvolutionUpRank is OwnableUpgradeable  {
    // The TOKEN to upgrade NFT
    BPLUSToken public bplus;
    CharacterNFT public characterNFT;
    AcademyUpStar public academyUpStar;
    // uint256 minDefault = 1000 * 10 ** 18;
    uint256 minDefault;
    mapping(uint256 => uint256) multiples;
    mapping(uint256 => uint256) materials;

    function initialize() public initializer {
        minDefault = 1000 * 10 ** 18;
        multiples[1]=1; // RANK D
        multiples[2]=2; // RANK C
        multiples[3]=4; // RANK B
        multiples[4]=8; // RANK A
        multiples[5]=16; // RANK S
        multiples[6]=256; // RANK SS
        multiples[6]=65536; // RANK SSS

        materials[1]=2; // RANK D
        materials[2]=2; // RANK C
        materials[3]=2; // RANK B
        materials[4]=2; // RANK A
        materials[5]=2; // RANK S
        materials[6]=2; // RANK SS
        materials[6]=2; // RANK SSS
        __Ownable_init();
    }
    function initByOwner(BPLUSToken _bplus,CharacterNFT _characterNFT, AcademyUpStar _academyUpStar) public  onlyOwner {
        bplus = _bplus;
        characterNFT = _characterNFT;
        academyUpStar=_academyUpStar;
    }

    function upRank(uint256 _mainNftId, uint256 [] memory _materialNftIds) public  returns (bool) {
        
        (uint256 id,,uint256 rank, uint256 star,) = characterNFT.getCharacterPublic(msg.sender,_mainNftId);
        if(id==0){
        ...
        for(uint i = 0; i < _materialNftIds.length; i++){
            (uint256 materialId,,uint256 materialRank,,) = characterNFT.getCharacterPublic(msg.sender,_materialNftIds[i]);
            if(materialId==0){
        ...
        uint256 bplusUpRank = multiples[rank] * 1000;
        bplus.approve(address(this), bplusUpRank*10**18);
        bplus.transferFrom(msg.sender, address(this), bplusUpRank*10**18);
        return true;
    }
    ...
    function setConfigUpgrade (uint256 _rank, uint256 _multiple,uint256 _material) public onlyOwner {
        multiples[_rank]=_multiple;
        materials[_rank]=_material;
    } 

    function setBPLUSToken(BPLUSToken _bplus) public onlyOwner{
        bplus=_bplus;
    }
    ...
    function setMinDefault(uint256 _minDefault) public onlyOwner{
        minDefault=_minDefault;
    }
}
