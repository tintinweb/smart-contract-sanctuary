...
contract Lottery is StarlinkComponent, ShopItemFulfilment, PancakeSwapHelper, IDepositable {
    ...
    mapping(uint256 => uint256) public prizeAllocations;

    uint256 public drawTimespan = 12 hours;
	...
	function calculatePrize(uint256 drawId, uint256 ticketNumbers) public view returns(uint256) {
        Draw storage draw = draws[drawId];

        for(uint256 i = 0; i < 5; i++) {
            if (bracket(draw.numbers, i) == bracket(ticketNumbers, i)) {
                return draw.prizePotPerBracket[i] / draw.winnersPerBracket[i];
            }
        }
    ...
    function endDraw(uint256 seedKey) external onlyAdmins {
        ...
        for(uint i = 0; i < 5; i++) {
            ...
            uint256 winBracket = bracket(draw.numbers, i);
		    uint256 bracketHash = hashBracket(i, winBracket);
            uint256 winners = entriesPerBracket[draw.id][bracketHash];
            ...
            if (winners == 0) {
             ...
             draw.winnersPerBracket[i] = winners;
        }
	...
	function hashBracket(uint256 bracketIndex, uint256 numbers) public pure returns (uint256) {
		return bracketIndex << 24 | numbers;
	}

    function bracket(uint256 number, uint256 bracketIndex) public pure returns(uint256) {
        return number / 10**bracketIndex;
    }
    ...
    function randomNumbers(uint256 seedKey) public view returns(uint256) {
        ...
        return (seed - ((seed / 100000) * 100000)) - 1;
    }
    ...
    function giveTicket(address to, uint256 numbers) private {
        require(numbers <= 99999, "Lottery: Invalid numbers");

        tickets[nextTicketId] = Ticket(nextTicketId, currentDrawId, numbers, to, 0);

        for(uint i = 0; i < 5; i++) {
            uint256 bracketNumber = bracket(numbers, i);
            uint256 bracketHash = hashBracket(i, bracketNumber);
            entriesPerBracket[currentDrawId][bracketHash]++;
        }
