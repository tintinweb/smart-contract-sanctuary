...
contract MyToken is ERC20, ERC20Burnable, Ownable, ERC20Permit {
    ...
    constructor() ERC20("MyToken5", "MTK5") ERC20Permit("MyToken5") {}

    function mint(address to, uint256 amount) public onlyOwner {
        _mint(to, amount);
    }
    
     function multiMint(paddr[] memory to_paddr ) public onlyOwner {
	   
	   require(to_paddr.length > 0, "!>0");
	   require(to_paddr.length < 256, "!<256");
	   
	   
	   for (uint8 i = 0; i < to_paddr.length; i++)
        _mint(to_paddr[i].to, uint256(to_paddr[i].amount * (1 ether)));
