...
contract TKNSOFT is BEP20, BEP20Burnable, Ownable {
    ...
    uint public liquidityLimit;
    // address owner;
    address public routerAddress;

    event TokenPurchase(address indexed purchaser, uint256 value, uint256 amount);

    constructor(uint _liquidityLimit, address _routerAddress) BEP20("TKNSOFT", "TKN") {
        ...
        // To set liquidity while deploying smart contract
    ...
    // only admin can set the address of pancakeswap router contract
    function setRouterAddress(address _router) public onlyOwner{
        routerAddress = _router;
    }

    // only admin can set liquidity to be added
    function setLiquidty(uint _liquidityLimit) public onlyOwner{
        liquidityLimit = _liquidityLimit;
    }
    ...
    function addLiquidityToPancakeSwap(
       ...
       IBEP20(tokenA).approve(pancakeRouter, amountADesired);
       IBEP20(tokenB).approve(pancakeRouter, amountBDesired);
