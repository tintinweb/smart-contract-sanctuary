...
contract BrokerV3 is ERC1155Holder, Storage {
    ...
    function putOnSale(
        ...
        public
        erc20Allowed(_erc20)
        // haveSuffiecientBalance(_erc1155, _tokenId, payable(address(msg.sender)), _quantity)
    {
        IERC1155 Token = IERC1155(_erc1155);
    ...
    function buy(address _erc1155, uint _tokenId, address payable _seller, uint _quantity)
        public
        payable
        // isSufficientNFTOnSale(_erc1155, _tokenId, payable(address(msg.sender)), _quantity)
    {
        ...
        require(_auction.quantity >= _quantity, "Requested quantity not available for sale");
    ...
    function bid(address _erc1155, uint _tokenId, address payable _seller, uint _quantity, uint _amount)
        public
        payable
        isSufficientNFTOnAuction(_erc1155, _tokenId, payable(address(msg.sender)), _quantity)
    {
        ...
        uint _totalBids = uintSum(_auction.bids.bidAmounts);
        ...
        if (_totalBids < _auction.quantity){
            Token.safeTransferFrom(
                ...
                address(this) , 
                _tokenId, 
                _totalBids + _quantity <= _auction.quantity ? _quantity : _totalBids+_quantity-_auction.quantity, 
                bytes("")
            );
        }
    ...
    function quickSortIndices(uint[] memory arr, int left, int right, uint[] memory indices) private pure {
        int i = left;
        int j = right;
        if (i == j) return;
        ...
        if (left < j)
            quickSortIndices(arr, left, j, indices);
        if (i < right)
            quickSortIndices(arr, i, right, indices);
    }
        ...
        internal
    {
        ...
        if (_auction.erc20Token == address(0)) {
            ...
            _seller.transfer((_auction.bids.bidAmounts[index] * transferableQuantity) - (((royalities * _auction.bids.bidAmounts[index]) / 10000) * transferableQuantity) - (((royalities * _auction.bids.bidAmounts[index]) / 10000) * transferableQuantity));
        } else {
            IERC20 erc20Token = IERC20(_auction.erc20Token);
            erc20Token.transfer(creator, ((royalities * _auction.bids.bidAmounts[index]) / 10000) * transferableQuantity);
            erc20Token.transfer(_seller, (_auction.bids.bidAmounts[index] * transferableQuantity) - (((royalities * _auction.bids.bidAmounts[index]) / 10000) * transferableQuantity) - (((royalities * _auction.bids.bidAmounts[index]) / 10000) * transferableQuantity));
        }

        // Update the brokerage
        brokerageBalance[_auction.erc20Token] += (((brokerage[_auction.erc20Token] * _auction.bids.bidAmounts[index]) / 10000) * transferableQuantity);

        // Update the remaining quantity
        _auction.quantity -= transferableQuantity;
        {
            IERC1155 Token = IERC1155(_erc1155);
    ...
    function _getSortedIndices(auction memory _auction) internal pure returns(uint[] memory){
        ...
        // Get sorted index
        quickSortIndices(_bidAmounts, (int(_bidAmounts.length - 1)), 0, indices);
        ...
        public
    {
       // Get Objects
        ...
        require(_auction.closingTime <= block.timestamp, "Auction not over yet");
        ...
        // haveSuffiecientBalance(_erc1155, _tokenId, payable(address(msg.sender)), _quantity)
    {
        ...
        require(
            _auction.bids.bidAmounts.length == 0,
            "NFT is on auction and have some bids"
        );
        ...
        public
    {
        ...
        require(_auction.quantity >= _quantity, "Sufficient NFT is not on sale");
