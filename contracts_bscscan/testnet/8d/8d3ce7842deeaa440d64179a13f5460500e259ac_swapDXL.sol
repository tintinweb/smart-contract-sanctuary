...
contract swapDXL is Ownable {
    ...
    uint256 public swapRate = 10; // 10 DXL == 1 DEEDEE;
    uint256 public DXLLimit = 200e18;
    uint256 public amountDXL = 0; //amount of deposited DXL
    uint256 amountDeeDee = 0; //resulting DeeDee to transfer
    ...
    function swapDXLForDEEDEEToken(uint256 _amount) external {
    	...
    	require(DEEDEE.balanceOf(address(this)) >= amountDXL.div(swapRate),"insufficient DEEDEE amount on the contract");

        amountDeeDee = amountDXL.div(swapRate); 
        
        //contract should be allowed to spend DXL
      	require(DXL.allowance(msg.sender,address(this)) >= amountDXL,"swapDXLForDEEDEEToken :: not allowed to spend DXL");   
    ...
    function updateRate( uint _swapRate) external onlyOwner {
        swapRate = _swapRate;
    }
