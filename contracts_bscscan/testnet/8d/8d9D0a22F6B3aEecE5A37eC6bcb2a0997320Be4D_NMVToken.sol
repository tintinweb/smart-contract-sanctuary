...
interface backBNBHoldersInterface {
 ...
 function distributeDividends() external payable;
  ...
  function withdrawDividend() external;
   ...
   event backBNBHoldersDistributed(
    address indexed from,
    uint256 weiAmount
  );

   event backBNBHoldersWithdrawn(
    address indexed to,
    uint256 weiAmount
  );
}
interface backBNBHoldersOptionalInterface {
 function withdrawableDividendOf(address _owner) external view returns(uint256);
function withdrawnDividendOf(address _owner) external view returns(uint256);
 function accumulativeDividendOf(address _owner) external view returns(uint256);
}
contract backBNBHolders is ERC20, backBNBHoldersInterface, backBNBHoldersOptionalInterface {
  using SafeMath for uint256;
...
uint256 public totalbackBNBHoldersDistributed;
...
function distributeDividends() public override payable {
 ...
 if (msg.value > 0) {
      ...
      emit backBNBHoldersDistributed(msg.sender, msg.value);
 totalbackBNBHoldersDistributed = totalbackBNBHoldersDistributed.add(msg.value);
    }
  }
 ...
 function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      emit backBNBHoldersWithdrawn(user, _withdrawableDividend);
...
contract NMVTokenbackBNB is backBNBHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() backBNBHolders("NMVToken_backBNB_Tracker", "NMVToken_backBNB_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "NMVToken_backBNB_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "NMVToken_backBNB_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NMVToken contract.");
    }
...
contract NMVToken is ERC20, Ownable {
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("NFT METAVERSE", "NMV") {
 
        TotalFees = backBNBFee.add(liquidityFee).add(marketFee);
 ...
 DEXV2Router = _DEXV2Router;
        ...
        backBNB = new NMVTokenbackBNB();
 backBNB.excludeFromDividends(owner());
        backBNB.excludeFromDividends(address(this));
        backBNB.excludeFromDividends(address(DEXV2Pair));
        backBNB.excludeFromDividends(address(DEXV2Router));
        backBNB.excludeFromDividends(address(backBNB));
        backBNB.excludeFromDividends(0x000000000000000000000000000000000000dEaD);
        backBNB.excludeFromDividends(0x0000000000000000000000000000000000000000);
lp = _DEXV2Pair;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
        ...
        if(Limited){
            backBNB.excludeFromDividends(holder);
        }else{
             try backBNB.includeInDividends(payable(holder), balanceOf(holder)) {} catch {}
        }
    }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "NMVToken: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "NMVToken: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "NMVToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "NMVToken: Cannot update gasForProcessing to same value");
        emit GasForProcessingUpdated(newValue, gasForProcessing);
        gasForProcessing = newValue;
    }

    function updateClaimWait(uint256 claimWait) external masterWallet {
        backBNB.updateClaimWait(claimWait);
    }

    function getClaimWait() external view returns(uint256) {
        return backBNB.claimWait();
    }
  function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
 uint256 tokensTNMVToken = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTNMVToken);
...
uint256 bnbForLiquidity = bnbBalance.mul(liquidityFee).div(bnbFeeFactor).div(2);
        uint256 bnbForReflection = bnbBalance.mul(backBNBFee).div(bnbFeeFactor);
        uint256 bnbForDev = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection);
addLiquidity(tokensTNMVToken, bnbForLiquidity);
payable(marketFeeReciever).transfer(bnbForDev);
 (bool success,) = address(backBNB).call{value: bnbForReflection}("");
if(success) {
            emit transferDividends(tokensToLiquify.mul(backBNBFee).div(TotalFees), bnbForReflection);
        }
    }
 ...
 DEXV2Router.addLiquidityETH{value: ethAmount}(
   ...
   function getAccountDividendsInfo(address account)
        external view returns (
            ...
            uint256,
            uint256) {
        return backBNB.getAccount(account);
    }
function getAccountDividendsInfoAtIndex(uint256 index)
        external view returns (
            ...
            uint256,
            uint256) {
        return backBNB.getAccountAtIndex(index);
    }
  function processbackBNB(uint256 gas) external {
        (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) = backBNB.process(gas);
        emit ProcessedbackBNB(iterations, claims, lastProcessedIndex, false, gas, tx.origin);
    }
 function claim() external {
        backBNB.processAccount(payable(msg.sender), false);
    }
function getLastProcessedIndex() external view returns(uint256) {
        return backBNB.getLastProcessedIndex();
    }
function getNumberOfDividendTokenHolders() external view returns(uint256) {
        return backBNB.getNumberOfTokenHolders();
    }
...
try backBNB.setBalance(payable(from), balanceOf(from)) {} catch {}
        try backBNB.setBalance(payable(to), balanceOf(to)) {} catch {}
 if(!swapping) {
            uint256 gas = gasForProcessing;
try backBNB.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
                emit ProcessedbackBNB(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
            }
            catch {}
        }
    }
