...
contract Marketplace is IERC721Receiver, Auth {
    ...
    uint8 public minLevel = 2;
    uint8 public minRarity = 1;
    uint public tradeFee = 0;

    address public tokenAddress = 0x76CfDd9044Ca74a5B0887B311ECF630c587a91Aa;
    address public nftAddress = 0x76DB87E403Cc08090B44b6D8c213F6911b76242d;
    ...
    function openTrade(uint24 tokenId, uint200 price) external {
        (uint8 level, uint8 rarity) = nftContract.getHero(tokenId);

        require(level >= minLevel && rarity >= minLevel, "Token not meet required");
    ...
    function executeTrade(uint tradeId) external {
        ...
        uint fee = (trade.tokenPrice * tradeFee) / 100;

        if (fee > 0) {
            require(token.transferFrom(msg.sender, address(this), fee), "Fee transfer failed.");
        }
        require(token.transferFrom(msg.sender, trade.poster, trade.tokenPrice - fee), "Token transfer failed.");
    ...
    function setTradeFee(uint newFee) public authorized {
        require(newFee <= 10, "The trade fee can at most be 10%.");
        tradeFee = newFee;
    }

    function setMinLevel(uint8 amount) public authorized {
        minLevel = amount;
    }

    function setMinRarity(uint8 amount) public authorized {
        minRarity = amount;
    }
