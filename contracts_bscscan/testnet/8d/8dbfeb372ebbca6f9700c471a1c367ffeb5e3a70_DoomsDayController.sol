...
contract DoomsDayNFT is ERC721URIStorage, Ownable {
    ...
    function _rand() internal virtual returns(uint256) {
        ...
        uint256 number1 =  uint256(keccak256(abi.encodePacked(block.timestamp, randNum++, msg.sender))) % (4 * 10 ** 15) + 296874639854288;
...
contract DoomsDayController is Ownable {
    ...
    function setDOMCost1(uint256 levels, uint256 num) public onlyOwner {heroPublicAttributes[levels].DOMCost = num;}
    function setDOOCost1(uint256 levels, uint256 num) public onlyOwner {heroPublicAttributes[levels].DOOCost = num;}
    ...
    mapping(uint256 => BattleHistory) public battleHistorys;
    ...
    function setFightRewards(uint256 levels,uint256 monsterId, uint256 num1,uint256 num2,uint256 num3,uint256 num4,uint256 num5) public onlyOwner {fightRewards[levels][monsterId] = FightReward(num1, num2, num3, num4, num5);}
    ...
    //预售NFT转换英雄卡
    // function NFTToHeroCard(uint256 tokenId, uint256 types) public{
    ...
    //     (uint256 num1,uint256 num2,uint256 num3,uint256 num4,uint256 num5,uint256 num6) = getHeroAttr(types, rarity); 

    //     heros[tokenId] = Hero(num1, num3, num2, num4, num5, num6, tokenId, types, rarity, 1, 0, 0, msg.sender);

    //     userHeros[msg.sender][users[msg.sender].heroNumber] = tokenId;

    //     updateBattleAttribute(totalHeros);

    //     herosToNFTs[totalHeros] = tokenId;

    //     heroSource[totalHeros] = 3;
    ...
    function getHeroCard() public{
        ...
        herosToNFTs[totalHeros] = DDN.createNFT(msg.sender, types, totalHeros);

        emit GetHero(msg.sender, totalHeros, types, rarity);
    }

    //获取英雄卡
    function _getHeroAttr(uint256 types, uint256 rarity) internal returns (uint256 num1,uint256 num2,uint256 num3,uint256 num4,uint256 num5,uint256 num6) {

        num1 = uint256(_getRandomNum(herroAttrs[rarity].mainLow, herroAttrs[rarity].mainUp));

        num2 = uint256(_getRandomNum(herroAttrs[rarity].secondaryLow, herroAttrs[rarity].secondaryUp));

        //获取其他属性
        num3 = uint256(_getRandomNum(35, 100));

        num4 = uint256(_getRandomNum(35, 100));

        num5 = uint256(_getRandomNum(35, 100));

        num6 = uint256(_getRandomNum(35, 100));
    ...
    function executeTask(uint256 heroId, uint256 activeType) public {
        ...
        if(activeType < 10) {
            require(heros[heroId].level > 1 && heros[heroId].rarity > 5,"This hero's level or rarity is non-compliant");
        } 
    ...
    function taskRewardWithdrew(uint256 heroId) public {
        ...
        if (tasks[heroId].taskTypes < 10) {
            total = miningRatio * (10 + (mainAttr - 85) * 5) * number * heroPublicAttributes[heros[heroId].level].profitMagnification * overtimeRatio / 10000000 ;
        }

        if (tasks[heroId].taskTypes == 10) {
            total = miningRatio * overtimeRatio * number * ((mainAttr - 85) * 5 + 10) * heroPublicAttributes[heros[heroId].level].profitMagnification / 1000000000 ;
        }
    ...
    function heroCardTransfer(uint256 heroId, address recipient) public {
        ...
        DDN.approveToController(msg.sender, herosToNFTs[heroId]);

        DDN.transferFrom(msg.sender, recipient, herosToNFTs[heroId]);
    ...
    function buyHeroCard(uint256 heroId) public {
        ...
        DDN.approveToController(ownerAddress, herosToNFTs[heroId]);

        DDN.transferFrom(ownerAddress, msg.sender, herosToNFTs[heroId]);
    ...
    function fightMonster(uint256 areaId, uint256 heroId) public {
        ...
        battleHistorys[users[msg.sender].battleTimes].heroId = heroId;

        battleHistorys[users[msg.sender].battleTimes].areaId = areaId;

        battleHistorys[users[msg.sender].battleTimes].monsterNumber = monsterNum;

        if(DOOProfit > 0){
            // DOO.transfer(msg.sender, DOOProfit * 10 ** 15);

            battleHistorys[users[msg.sender].battleTimes].DOOProfit = DOOProfit;
        ...
        if(DOMProfit > 0){
            ...
            battleHistorys[users[msg.sender].battleTimes].DOMProfit = DOMProfit;

            users[msg.sender].DOMProfitOfBattle += DOMProfit;
        }

        if(result){
            battleHistorys[users[msg.sender].battleTimes].result = result;        
        }
    ...
    function withdrewBattleProfit() public {
        ...
        if(users[msg.sender].DOMProfitOfBattle > 0){
            DOO.transfer(msg.sender, users[msg.sender].DOMProfitOfBattle * 10 **18);
    ...
    function getUsedTimes(uint256 heroId) public view returns (uint256) {
        ...
        // if((block.timestamp - 4 hours) / 24 hours - (heros[heroId].lastUsedTime - 4 hours) / 24 hours >= 1){
        if((block.timestamp + 20 hours) / 24 hours - (heros[heroId].lastUsedTime + 20 hours) / 24 hours >= 1){
            // availableTimes = heroPublicAttributes[heros[heroId].level].totalTimes;
            return 0;
        }else{
            // availableTimes = heroPublicAttributes[heros[heroId].level].totalTimes - heros[heroId].usedTimes;
