...
contract ICliq {
 ...
 contract withdraw {
  ...
  uint256 public tokenEndTime;
  uint256 public userToken;
  uint256 public tokenReward;
  uint256 public tokenPenalty; 
  uint256 public totalStakedToken;
  uint256 public finalTokenStakeWithdraw;
...
function withdrawStakedToken(uint256 stakingId) public returns(bool) {
    ...
    stakingAddress = istaking.getTokenStakingAddressById(stakingId);
    totalDays = istaking.getTokenStakingTotalDaysById(stakingId);
    tokenStartTime = istaking.getTokenStakingStartTimeById(stakingId);
    tokenEndTime = istaking.getTokenStakingEndTimeById(stakingId);
    userToken = istaking.getStakingTokenById(stakingId);
    tokenReward = istaking.getTokenRewardDetailsByStakingId(stakingId);
    tokenPenalty = istaking.getTokenPenaltyDetailByStakingId(stakingId);
    totalStakedToken = istaking.getTotalTokenStakesInContract(stakingId);
    finalTokenStakeWithdraw = istaking.getFinalTokenStakeWithdraw(stakingId);
    transactionStatus = istaking.getTokenLockStatus(stakingId);
  ...
  function getvalues(uint256 stakingId) public view returns(uint256){
    return totalDays;
  }
