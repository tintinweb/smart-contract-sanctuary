...
import "./interfaces/IKuSwapRouter.sol";
//////////////////////////// DEX INTERFACES ////////////////////////////



// TODO - remove all instances of koffeeswap
...
contract ANKH is Context, IERC20, IERC20Metadata {
    ...
    //////////////////////////// DEX VARS ////////////////////////////  
    // address public routerAddressForDEX = 0xc0fFee0000C824D24E0F280f1e4D21152625742b;  // KOFFEESWAP      // CHANGEIT - make sure you have the right router
    // address public routerAddressForDEX = 0xA58350d6dEE8441aa42754346860E3545cc83cdA;     // KUSWAP
    address public routerAddressForDEX = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;

    IKuSwapRouter public kuSwapRouter;
    address public kuSwapPair;
    ...
    //////////////////////////// GOVERNANCE VARS ////////////////////////////
    address public governanceContractAddress = 0xe71F91e008B765dCb2B5c2f36De2c4f93C5fBEEa; // CHANGEIT - make sure you have the right one
    ...
    constructor () {
        ...
        reflectBalance[depositWallet] = reflectTokensTotalSupply.div(100).mul(20);          // sends 20% to Deposit Wallet
        ...
        randomNumberForGames = randomNumberForGames.add(54);
    ...
    function _transfer(address sender, address recipient, uint256 transferAmount) internal {
        ...
        if(kuSwapPair != recipient && kuSwapPair != sender){  
        ...
        if(!isInSwapAndLiquify){
            ...
            if(isSwapAndLiquifyEnabled){
                ...
                if(sender != kuSwapPair){      // do not allow on a buy
    ...
    function SwapAndLiquify(uint256 tokenAmountToSwapAndLiquifiy) private {        // this sells half the tokens when over a certain amount.
        ...
        _approve(address(this), address(kuSwapRouter), half2);
        kuSwapRouter.addLiquidityETH{value: newBalance}(address(this), half2, 0, 0, directorAccount, block.timestamp);     // adds the liquidity
    }
    ...
    function SwapTokensForEth(uint256 tokenAmount) private {
        ...
        path[1] = kuSwapRouter.WETH();     // Router Address
        
        _approve(address(this), address(kuSwapRouter), tokenAmount);

        // so when this is called in the code, it's using the CA as the "from"
        kuSwapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(tokenAmount, 0, path, address(this), block.timestamp);     // make the swap
    }
    //////////////////////////// LIQ FUNCTIONS ////////////////////////////
    ...
    function SetKuSwapRouterAddress(address newRouter) external OnlyDirector() {
        routerAddressForDEX = newRouter;
        kuSwapRouter = IKuSwapRouter(routerAddressForDEX); 
        kuSwapPair = IKuswapFactory(kuSwapRouter.factory()).createPair(address(this), kuSwapRouter.WETH());     // CHANGEIT - remove for live, WKCS needed
        isAddressExcludedFromMaxBuy[kuSwapPair] = true;
        isAddressExcludedFromMaxSell[kuSwapPair] = true;
        isAddressNotRobotPermanently[kuSwapPair] = true;
        isAddressExcludedFromMaxTransfer[kuSwapPair] = true;
    }
