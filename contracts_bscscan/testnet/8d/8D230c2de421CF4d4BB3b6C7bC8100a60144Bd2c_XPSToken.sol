...
contract XPSToken is Context, IERC20, Ownable {
    ...
    uint256 public _liquidityTriggerAmount = 5 * 10 ** 9; // = 500,000 tokens
    uint256 public _marketingTriggerAmount = 5 * 10 ** 9; // = 500,000 tokens
    uint256 public _communityTriggerAmount = 5 * 10 ** 9; // = 500,000 tokens
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(!isExcludedFromFee(_msgSender())) {
            ...
            if(
                ...
                if(_currentCommunityTriggerAmount >= _communityTriggerAmount && !inSwapAndCommunity) {
                    swapAndCommunity(_currentCommunityTriggerAmount);
                    _currentCommunityTriggerAmount = 0;
                }
                
                if(_currentMarketingTriggerAmount >= _marketingTriggerAmount && !inSwapAndMarketing) {
                    swapAndMarketing(_currentMarketingTriggerAmount);
                    _currentMarketingTriggerAmount = 0;                    
                }
                
                if(_currentLiquidityTriggerAmount >= _liquidityTriggerAmount && !inSwapAndLiquify) {
