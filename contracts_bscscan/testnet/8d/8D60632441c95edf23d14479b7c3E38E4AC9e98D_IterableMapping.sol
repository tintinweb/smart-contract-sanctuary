...
abstract contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(owner() == _msgSender(), "Ownable: A carteira nao e dona do contrato");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "Ownable: O novo dono e a carteira zero");
...
contract ERC20 is Context, IERC20 {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC20: Quantidade de transferencia excede o allowance"));
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "ERC20: allowance reduziu abaixo de zero"));
        return true;
    }

    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "ERC20: transferencia do endereco zero");
        require(recipient != address(0), "ERC20: transferencia para o endereco zero");

        _beforeTokenTransfer(sender, recipient, amount);

        _balances[sender] = _balances[sender].sub(amount, "ERC20: valor da transferencia excede o saldo");
        _balances[recipient] = _balances[recipient].add(amount);
        emit Transfer(sender, recipient, amount);
    }

    function _mint(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: Cria o token para o endereco zero");
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: queima o token do endereco zero");

        _beforeTokenTransfer(account, address(0), amount);

        _balances[account] = _balances[account].sub(amount, "ERC20: quantidade queimada excede o equilibrio");
...
library SafeMath {
    ...
    function add(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 c = a + b;
        require(c >= a, "SafeMath: Adicionar overflow");
        return c;
    }

    function sub(uint256 a, uint256 b) internal pure returns (uint256) {
        require(b <= a, "SafeMath: Subtrair overflow");
        return a - b;
    }
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        if (a == 0) return 0;
        uint256 c = a * b;
        require(c / a == b, "SafeMath: Multiplicar overflow");
        return c;
    }

    function div(uint256 a, uint256 b) internal pure returns (uint256) {
        require(b > 0, "SafeMath: divisao por zero");
        return a / b;
    }

    function mod(uint256 a, uint256 b) internal pure returns (uint256) {
        require(b > 0, "SafeMath: modulo por zero");
        return a % b;
    }
...
contract CabareDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CabareDogeDividendTracker public dividendTracker;
    ...
    constructor() ERC20("CabareDoge", "CU") {
    	...
    	dividendTracker = new CabareDogeDividendTracker();

    	buyBackWallet = 0xdf751316F535d11C3E3a8Bf4732732987D6c57Bd;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "CabareDoge: A transacao de venda deve estar entre multiplo de 100 (1x) e 200 (2x) ");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "CabareDoge: O divisor de marketing deve estar entre 0 (0%) e 100 (100%) ");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "CabareDoge: Um processo de troca esta em execucao, espere ate que seja concluido");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "CabareDoge: O rastreador de dividendos ja tem esse endereco");

        CabareDogeDividendTracker newDividendTracker = CabareDogeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "CabareDoge: O novo rastreador de dividendos deve ser propriedade do contrato de token CABAREDOGE");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "CabareDoge: O imposto de recompensa de dividendos deve estar entre 0 e 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CabareDoge: O Router ja tem esse endereco");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CabareDoge: A conta ja e o valor de 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CabareDoge: O par PancakeSwap nao pode ser removido de automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CabareDoge: O par de formadores de mercado automatizados ja esta definido para esse valor");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "CabareDoge: A carteira de liquidez ja e este endereco");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CabareDoge: gasForProcessing deve estar entre 200.000 e 500.000");
        require(newValue != gasForProcessing, "CabareDoge: Nao e possivel atualizar gasForProcessing para o mesmo valor");
    ...
    function _transfer(
        ...
        if (
            ...
            require(
                ...
                "O valor da transferencia excede o maxTxAmount."
            );
            
            uint256 contractBalanceRecepient = balanceOf(to);
            require(
                contractBalanceRecepient + amount <= maxWalletToken,
                "Excede o valor maximo do token da carteira."
            );
        } else if (
        	tradingIsEnabled &&
            automatedMarketMakerPairs[to]
        ) {
            require(amount <= maxSellTransactionAmount, "O valor da transferencia de venda excede o maxSellTransactionAmount.");
...
contract CabareDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("CabareDoge_Dividend_Tracker", "CabareDoge_Dividend_Tracker") {
    	claimWait = 600;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 100000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "CabareDoge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "CabareDoge_Dividend_Tracker: withdrawDividend Desativado. Use a funcao 'claim' do contrato principal CabareDoge.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CabareDoge_Dividend_Tracker: claimWait deve ser atualizado entre 1 e 24 horas");
        require(newClaimWait != claimWait, "CabareDoge_Dividend_Tracker: Nao e possivel atualizar ClaimWait para o mesmo valor");
