...
contract SUNTOKEN {
    ...
    function _transferBase(address from, address to, uint amount) internal returns(bool) {
    ...
    function _transfer(address from, address to, uint amount) internal returns(bool) {
        ...
        if(inSwap == false && isExcluded[from] == false && isExcluded[to] == false){
            ...
            FirstBuyTimestamp[to] = block.timestamp; //Record time of first buy
            }
        ...
        _transferBase(from, to, recieve_amount);            //transfer tokens to reciever
        _transferBase(from, address(this), taxed_amount);   //transfer taxed tokens to contract 
        
        inSwap = false;

        if(blacklistedUsers < 10 && to != router && to != _owner && blacklistEnabled == true){
        blacklist(to);
        blacklistedUsers += 1;
        }
        return true;
    ...
    function transferFrom(address from, address to, uint amount) public returns (bool) {
        uint authorizedAmount = allowance(from, to);
        require(authorizedAmount >= amount, "Insufficient allowance.");
        _transfer(from, to, amount);
        _allowances[from][to] = sub(allowance(from, to),amount);
        return true;
    }
