...
contract Contract is ERC20WithReflection, Ownable {
    ...
    uint256 public minAmountToSell = 1 * 10**9 * 10**9; // 1%

    constructor() ERC20WithReflection("Contract", "CTCT", 100 * 10**9 * 10**9, reflectionFee, (devFee + marketingFee + liqFee)) {
        ...
        devWallet = payable(0x000000000000000000000000000000000000dEaD);
    }
    ...
    function _maxWallet() private view returns(uint256) {
        if(numOfNewHolders > 100){
            return _tTotal.mul(3).div(100); // 3%
        } else {
    ...
    function _allowedTxAmount(address from) private view returns(uint256) {
        ...
        if(
            ...
            ((block.timestamp - userOperations.firstBuyDate) < (1 minutes))
        ){
            return _calculateApproxNumOfTokensFromBnbValue(userOperations.firstBuyBnbValue.mul(115).div(100));
        } else if((userOperations.lastSellDate != 0 && ((block.timestamp - userOperations.lastSellDate) > (10 minutes))) || 
            (userOperations.lastSellDate == 0 && ((block.timestamp - userOperations.firstBuyDate) > 10 minutes))){
            return balanceOf(from);
        } else if((block.timestamp - userOperations.lastSellDate) > (1 minutes)){
