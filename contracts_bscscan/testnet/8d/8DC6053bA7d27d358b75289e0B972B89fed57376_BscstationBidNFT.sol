...
interface IGnftBscsStartPools {
    function addGenesisNft(uint256 _nftId, uint256 _price, address _nftOwner,address _tokenAddress) external;
    function updateGenesisNft(uint256 _nftId, address _newOwner) external;
}
...
contract BscstationBidNFT is IBidNFT, ERC721Holder, Ownable, Pausable {
    using SafeMath for uint256;
    ...
    address public nftPoolAddr; // NFT pool contract address 
    address public genesisNftCreator;
    
    mapping(address => bool) public inoWhitelisted;
    uint256 public whitelistCount;
    mapping(address => uint256) public userBuyCount;
    uint256 public inoBuyMax = 1; // number of INO NFT item that allowed a user to buy
    address public inoNftCreator;
    mapping(uint256 => bool) public InoNftList;
    ...
    function addWhitelist(address _addr) external onlyOwner{
        ...
        require(!inoWhitelisted[_addr],'Already in whitelist');
        inoWhitelisted[_addr] = true;
        whitelistCount++;
    }
    
    function removeWhitelist(address _addr) external onlyOwner{
        require(_addr != address(0),'Invalid address');
        require(inoWhitelisted[_addr],'Not in whitelist');
        inoWhitelisted[_addr] = false;
        whitelistCount--;
    }
    
    function updateBuyMax(uint256 _buyCount) external onlyOwner{
        inoBuyMax = _buyCount;
    }
    
    function setNftPoolAddr(address _addr) public onlyOwner {
        nftPoolAddr = _addr;
    }
    
    function setGenesisNftCreator(address _addr) external onlyOwner {
        genesisNftCreator = _addr;
    }
    
    function setInoNftCreator(address _addr) external onlyOwner {
        inoNftCreator = _addr;
    }
    ...
    function buyTokenTo(uint256 _tokenId, address _to) public override whenNotPaused {
        ...
        // if INO NFT then require user to be whitelisted
        if(InoNftList[_tokenId]){
            require(inoWhitelisted[msg.sender],'You are not in whitelist');
            require(userBuyCount[msg.sender] < inoBuyMax, 'No more item allowed to buy');   
        }
        
        // updating genesis nft owner as now transferring to new owner
        if (nftPoolAddr != address(0)) {
            IGnftBscsStartPools(nftPoolAddr).updateGenesisNft(_tokenId, _to);
        }
    ...
    function readyToSellTokenTo(
        ...
        // bscs admin add genesis nft data to nft pool so later on user will able to stake it
        if (nftPoolAddr != address(0) && (address(_msgSender()) == genesisNftCreator)) {
            IGnftBscsStartPools(nftPoolAddr).addGenesisNft(_tokenId, _price, address(this), address(nft));
        }
        
        // bscs admin add INO nft data to nft pool so later on user will able to stake it
        if (nftPoolAddr != address(0) && (address(_msgSender()) == inoNftCreator)) {
            InoNftList[_tokenId] = true;
            IGnftBscsStartPools(nftPoolAddr).addGenesisNft(_tokenId, _price, address(this), address(nft));
        }
