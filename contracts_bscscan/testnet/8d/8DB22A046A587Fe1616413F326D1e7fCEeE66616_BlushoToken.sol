/**
   ...
   of the ecosystem. Wallet to Wallet transfers don't incur a fee. These are your tokens!
    ...
    Total Supply: 100.000.000.000 (100 Billion / Constant)
    Buy/Sell Transaction Limit: 1% of Total Supply (Max 1% of Circulating Supply)
    Developer Tax: 5% (Max 10%)
...
contract BlushoToken is IERC20Metadata, Ownable {
     ...
     * @dev Token name
     */
    string internal constant NAME = "blusho.finance";
     ...
     */
    uint256 internal constant TOTAL_SUPPLY = 100 * 10**9 * ZEROES;
    ...
    /**
     * @dev The temporary wallet address.
     * Nobody can get the private key to this wallet
     * Tokens sent to this address will just keep track of the soft-burned project tokens
     */
    address internal constant softBurnAddress =
     ...
     */
    uint256 internal _projectWalletsIndex = 1000;
     ...
     */
    uint256 internal _taxProjectsCycle = 0;

    /**
     * @dev Track tokens transferred to project wallets since last tax cycle
     */
    uint256 internal _tokensTransferedSinceLastTaxCycle;
     ...
     */
    uint256 internal _adjustBurnBalance;
     ...
     * tbd: add pcs wallets to taxablewallets
     */
    constructor() {
    ...
    function _transfer(
        ...
        //if it is an exchange transaction
            ...
            //tbd: adjust for taxes
            //tbd: deduct taxes and deduct from receivedAmount
        ...
        if (_isProjectWallet(recipient)) {
            _afterTokenTransferToProjectWallet(recipient, amount);
        }
    }
    ...
    function addProjectWallet(string memory projectName) public {
        ...
        address projectWalletAddress = address(
            uint160(uint256(_projectWalletsIndex))
        );
    ...
    /**
     * @dev Get Project wallet data for caller
     */
    ...
    /**
     * @dev Method to trigger project wallet tax
     * Taxes all project wallets
     */
    function triggerProjectWalletTax() external onlyOwner {
        require(
        ...
        if (_tokensTransferedSinceLastTaxCycle > 0) {
            //Increase temporary _adjustBurnBalance by the amount of tokens sent since the last taxProjectCycle
            uint256 increaseBurnBalance = (_tokensTransferedSinceLastTaxCycle *
                taxProjects) / 100;
            _adjustBurnBalance += increaseBurnBalance;

            //Reset _tokensTransferedSinceLastTaxCycle for the next _taxProjectsCycle
            _tokensTransferedSinceLastTaxCycle = 0;

            emit Transfer(softBurnAddress, burnAddress, increaseBurnBalance);
        }

        //Increase the _taxProjectsCycle
        _taxProjectsCycle += 1;
    }
    ...
    /**
     * @dev Emitted when `value` tokens are moved to the burnAddress
     *
     * Note that `value` may be zero.
     */
    event Burn(address indexed from, uint256 value);

    function _afterTokenTransferToProjectWallet(
        address projectWalletAddress,
        uint256 amount
    ) internal virtual {
        _tokensTransferedSinceLastTaxCycle += amount;
        ProjectWallet memory projectWallet = _projectWallets[
            projectWalletAddress
        ];
        //Check if the project had any burn events for the current _taxProjectsCycle
        if (
            projectWallet.lastTransferToBurnAddressProjectTaxCycle <
            _taxProjectsCycle
        ) {
            //Calculate the project wallet balance before this transfer
            uint256 balanceBeforeTransfer = _balances[projectWalletAddress] -
                amount;
            if (balanceBeforeTransfer > 0) {
                ...
                uint256 taxAmountToBurn = balanceBeforeTransfer -
                ...
                //Decrease the temporary _adjustBurnBalance by the taxAmount as it has actually been burned now
                _adjustBurnBalance -= taxAmountToBurn;

                emit Burn(projectWalletAddress, taxAmountToBurn);
                emit Transfer(
                    projectWalletAddress,
                    burnAddress,
                    taxAmountToBurn
                );
                emit Transfer(burnAddress, softBurnAddress, taxAmountToBurn);
            }
            //Update the project wallets taxProjectsCycle
            _projectWallets[projectWalletAddress]
                .lastTransferToBurnAddressProjectTaxCycle = _taxProjectsCycle;
        }
    }
    ...
    function _balanceOfBurnAddress() internal view virtual returns (uint256) {
        return _balances[burnAddress] - _adjustBurnBalance;
    }
