...
contract OLY10 is IBEP20, Auth {
    ...
    string constant _name = "OLY10";
    string constant _symbol = "OLY10";
...
//    }
    ...
    function getTotalFee(bool selling, address recipient) public view returns (uint256) { // added address recipient
        if (launchedAt + 2 >= block.number){ return feeDenominator.sub(1); }
        if (isSell(recipient) == false && !selling && totalBuyFee >= 500 && feeMultipliersTriggeredAt.add(feeMultipliersDuration) < block.timestamp) { return getbuyfee();} // greenwall
        if (isSell(recipient) == false && selling && totalSellFee <= 3300 && feeMultipliersTriggeredAt.add(feeMultipliersDuration) < block.timestamp) { return getsellfee();} // greenwall
    ...
    function getbuyfee() public view returns (uint256) {
        ...
        return totalFee.sub(feeDecrease.mul(remainingTime).div(buyMultipliersDuration));
    }
    //greenwall
    function getsellfee() public view returns (uint256) {
        ...
        return totalFee.add(feeIncrease.mul(remainingTime).div(buyMultipliersDuration));
    }
...
//        return amount.sub(feeAmount);
 //   }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(getTotalFee(isSell(recipient),recipient)).div(feeDenominator); // added recipient ori is  uint256 feeAmount = amount.mul(getTotalFee(isSell(recipient))).div(feeDenominator); 
        _balances[address(this)] = _balances[address(this)].add(feeAmount);
        emit Transfer(sender, address(this), feeAmount);
