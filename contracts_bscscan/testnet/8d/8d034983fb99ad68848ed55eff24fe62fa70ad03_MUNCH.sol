...
pragma solidity ^0.6.12;
    ...
    contract MUNCH is Context, IERC20, Ownable {
        ...
        address payable public devAddress = payable(0x6C63F1E5c892f9086DD1FdBb48615A7bB46E46A3);
        ...
        uint256 private _maxTxAmount = 1 * 10**7 * 10**9;
        // We will set a minimum amount of tokens to be swaped => 5M
        uint256 private _numOfTokensToExchangeForBuyback = 5 * 10**4 * 10**9;
        ...
        constructor (address payable buybackWalletAddress) public {
        ...
        function removeAllFee() private {
            if(_taxFee == 0 && _buybackFee == 0 && _devFee == 0) return;
        ...
        function _transfer(address sender, address recipient, uint256 amount) private {
            ...
            bool overMinTokenBalance = contractTokenBalance >= _numOfTokensToExchangeForBuyback;
        ...
        function manualSendBuyback() external onlyOwner() {
            uint256 contractETHBalance = address(this).balance;
            sendETHToBuyback(contractETHBalance);
        }
        
        function manualSendDev() external onlyOwner() {
            uint256 contractETHBalance = address(this).balance;
        ...
        function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
            (uint256 tTransferAmount, uint256 tFee, uint256 tBuyback) = _getTValues(tAmount, _taxFee, _buybackFee);
            uint256 currentRate =  _getRate();
            (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, currentRate);
            return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tBuyback);
        }

        function _getTValues(uint256 tAmount, uint256 taxFee, uint256 buybackFee) private pure returns (uint256, uint256, uint256) {
        ...
        function _setBuybackFee(uint256 buybackFee) external onlyOwner() {
            require(buybackFee >= 1 && buybackFee <= 9, 'buybackFee should be in 1 - 9');
       ...
       function _setBuybackWallet(address payable buybackWalletAddress) external onlyOwner() {
        ...
        function _setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
            require(maxTxAmount >= 1000000e9 , 'maxTxAmount should be greater than 1000000e9');
