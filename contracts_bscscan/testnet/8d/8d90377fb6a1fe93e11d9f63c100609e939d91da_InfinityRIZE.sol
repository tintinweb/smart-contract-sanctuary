...
/**
Infinity RIZE: $RIZE
- You buy, hold, & RIZE while the platform grows & provides infinite rewards to $RIZE holders.

Buy Fee: 7%
* 5% of each buy goes to existing $RIZE holders
* 2% of each buy goes to the RIZE Rewards Treasury
Sell Fee: 8%
* 1% of each sale goes to the RIZE Rewards Treasury
* 2% of each sale goes to the RIZE Network Expansion Treasury
* 5% of each sale goes to the INFINITY RIZEASAURUS unique buyback & LP system. RIZE/BUSD and RIZE/BNB LP tokens are created & staked/farmed for increased funding for RIZE Network scaling, development, major marketing, & maintenance! (as well as massive suprise BUYBACKS!)
*/
...
contract InfinityRIZE is Context, IERC20, Ownable {
    ...
    uint256[] public _infinityRizeasaurusFee;

    uint256 internal _feeTotal;
    uint256 internal _infinityRizeasaurusFeeCollected;
    ...
    address public infinityRizeasaurus = 0xDF7e60bb4E93BbEd3094e15640Ead18857020d4E;
    address public rewardsTreasury = 0x6811aA9D68bBf437D05CAcDBB5eB10D9c19c59F1;
    ...
    constructor() public {
        ...
        _isExcludedFromFee[rewardsTreasury] = true;
        _isExcludedFromFee[infinityRizeasaurus] = true;
        ...
        _rewardsFee.push(0);
        _rewardsFee.push(500);
        _rewardsFee.push(500);

        _infinityRizeasaurusFee.push(0);
        _infinityRizeasaurusFee.push(500);
        _infinityRizeasaurusFee.push(500);
    }
    ...
    function calculateFee(uint256 feeIndex, uint256 amount) internal returns (uint256, uint256){
        ...
        uint256 rewardsFee = amount.mul(_infinityRizeasaurusFee[feeIndex]).div(
            10**(_feeDecimal + 2)
        );
        ...
        _infinityRizeasaurusFeeCollected = _infinityRizeasaurusFeeCollected.add(rewardsFee);
    ...
    function swap() private lockTheSwap {
        uint256 totalFee = _rewardsFeeCollected.add(_infinityRizeasaurusFeeCollected);
        ...
        uint256 amountInfinityRizeasaurus = amountFee.mul(_infinityRizeasaurusFeeCollected).div(totalFee);

        if (amountInfinityRizeasaurus > 0){
            payable(infinityRizeasaurus).transfer(amountInfinityRizeasaurus);
        }

        uint256 amountRewards = address(this).balance;
        uint256 amountNetwork = amountRewards.div(5).mul(2);
        if (amountRewards > 0){
            payable(rewardsTreasury).transfer(amountRewards.sub(amountNetwork));
            payable(networkTreasury).transfer(address(this).balance);
        }

        _infinityRizeasaurusFeeCollected = 0;
    ...
    function setInfinityRizeasaurusFee(uint256 buy, uint256 sell, uint256 p2p) external onlyOwner {
        require(buy <= 2500 && sell <= 2500 && p2p <= 2500, "Invalid fee");
        _infinityRizeasaurusFee[0] = buy;
        _infinityRizeasaurusFee[1] = sell;
        _infinityRizeasaurusFee[2] = p2p;
    }

    function setInfinityRizeasaurus(address _address) external onlyOwner {
        infinityRizeasaurus = _address;
    }

    function setRewardsTreasury(address _address) external onlyOwner {
        rewardsTreasury = _address;
    }
