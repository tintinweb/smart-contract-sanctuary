...
contract RijentContract {    
    ...
    function checktimeDiff() public view returns (uint time){
        uint time            = block.timestamp;
        uint packageExpiry   = (time + 7 days); 
    ...
    function deposit(uint amount, uint lockingType) public {
        ...
        if(lockingType==0){
            packagePer      = 3;
            packageForMonth = 12;
            packageExpiry   = time + (1 days); 
        }
        else if(lockingType==1){
            packagePer      = 4;
            packageForMonth = 18;
            packageExpiry   = time + (2 days);
        }
        else if(lockingType==2){
            packagePer       = 5;
            packageForMonth  = 24;
            packageExpiry    = time + ( 3 days);
        }
        else if(lockingType==3){
            packagePer      = 7;
            packageForMonth = 36;
            packageExpiry   = time + (4 days);
        }
        else{
            packagePer      = 9;
            packageForMonth = 60;
            packageExpiry   = time + (5 days);
        }
        uint timeDiff       = time- packageExpiry;
        /*
    ...
    function packageDetails(address addr) public view returns(uint[] memory packageAmt, uint[] memory lockingType){
        ...
        for(uint i = 0; i < len; i++){
            Package storage dep = userData.packages[i];
            packageAmt[i]  = dep.packageAmt;
            lockingType[i] = dep.lockingType; 
        }
