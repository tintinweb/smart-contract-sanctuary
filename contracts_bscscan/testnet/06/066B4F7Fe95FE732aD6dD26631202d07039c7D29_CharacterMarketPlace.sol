...
contract CharacterMarketPlace is OwnableUpgradeable {
    struct Auction {
        uint256 highestBid;
        uint256 closingTime;
    ...
    // function initialize(address _characterNFT) external onlyOwner {
    //     require(_characterNFT != address(0), "Invalid address");
    ...
    function closeAuctionSudden(uint256 _nftId) external {
        require(auctions[_nftId].isActive == true, "Not active auction");
...
contract CharacterNFT is ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable{
    ...
    modifier onlyOwnerOrBoxNFT {
      require(msg.sender == boxNFT || msg.sender == owner());
      _;
    }

  function createCharacter(address owner,uint256 characterType, uint256 rank, uint256  star,uint256  exp) public onlyOwnerOrBoxNFT  returns (uint256) {
        _tokenIds.increment();
