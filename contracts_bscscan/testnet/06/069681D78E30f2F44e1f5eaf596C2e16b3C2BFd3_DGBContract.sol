...
contract DGBContract {
    ...
    event WithdrawDGB(address, uint);
    ...
    function initialize(address _admin, address _teamAddr, address _usdtAddr, address _dgbAddr) external {
        require(!initialized,"initialized");
        admin = _admin;
        teamAddr = _teamAddr;
        USDT = IERC20Upgradeable(_usdtAddr);
        DGB = IERC20Upgradeable(_dgbAddr);
        poolRate = 90;
        teamRate = 10;
        userRate = 97;
        team2Rate = 3;
    ...
    function withdrawDGB(address _addr, uint _amount) external onlyAdmin {
        require(_addr!=address(0),"Can not withdraw to Blackhole");
        DGB.transfer(_addr, _amount);
    }

    function batchAdminWithdrawDGB(address[] memory _userList, uint[] memory _amount) external onlyAdmin {
        for (uint i = 0; i < _userList.length; i++) {
            DGB.transfer(address(_userList[i]), uint(_amount[i]));
        }
    }

    function burnDGB(uint _amount) external onlyAdmin {
        DGB.transferFrom(msg.sender,address(0x000000000000000000000000000000000000dEaD), _amount);
