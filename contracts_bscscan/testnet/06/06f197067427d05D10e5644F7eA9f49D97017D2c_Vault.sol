...
contract Vault {
    ...
    function getRewardsEarned(address staker) public view returns (uint256) {
        return stakersRewards[staker] + (balance[staker] * rewardsPerSAFUPerBlock * (block.number - lastUpdate[staker])) / 10e9;
    }
    ...
    function stake(uint256 amount) public {
		...
		if (balance[msg.sender] > 0) {
			stakersRewards[msg.sender] += (balance[msg.sender] * rewardsPerSAFUPerBlock * (block.number - _lastUpdate)) / 10e9;
		} else {
    ...
    function withdraw(uint256 amount) public {
		...
		SAFU.transfer(msg.sender, amount.mul(101) /100 + rewards);
    }
	...
	function modifyRewards(uint256 amount) public onlyOwner {
		...
		for (uint256 i = 0; i < stakers.length; i++) {
			...
			stakersRewards[stakers[i]] += (balance[stakers[i]] * rewardsPerSAFUPerBlock * (block.number - _lastUpdate)) / 10e9;
		}
