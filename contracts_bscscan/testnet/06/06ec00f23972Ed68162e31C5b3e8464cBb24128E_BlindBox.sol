...
contract BlindBox  is Ownable {
    ...
    mapping(address => BoxOrder[]) private LastBoxOrderList;
   ...
   function setBox(
    ...
    function buyBox(uint8 boxId,uint256 num) public {
        ...
        for (uint256 i = 1; i <= num; i++) {
            value=value.add(getValue(detail.chacnce,i));
            LastBoxOrderList[msg.sender].push(BoxOrder(boxId,detail.boxType,num, amount,detail.currency,block.timestamp));
        } 
        BoxOrderList[msg.sender].push(BoxOrder(boxId,detail.boxType,num, amount,detail.currency,block.timestamp));
  ...
  function getLastBoxOrderList(address account) external view returns(BoxOrder[] memory){
