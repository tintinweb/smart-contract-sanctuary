...
contract MoonCoinFinance  {
	...
	event Evt_RefBonus(uint256 eventId, uint256 timestamp,  address indexed user, address indexed upline, uint256 i, uint256 amount, uint256 promoMultiplier, uint256 tokenBalance, uint256 tokenBalanceFactor);
	event Evt_RefBonus1(uint256 eventId, uint256 tmpEarn,uint256 tmpMiss,uint256 tokensEarned,uint256 tokensMissed);
	...
	function setPromoMultiplier(address referrer) public { 
		...
		uint256 tokenBalance = PlatformTokenApp.balanceOf(referrer);
		uint256 tokenBalanceFactor = tokenBalance.div(10**TOKEN_DECIMAL_FACTOR);
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 10; i++) {
				if (upline != address(0) && upline!= msg.sender) {				
					...
					uint256 tokenBalance = PlatformTokenApp.balanceOf(upline);
					uint256 tokenBalanceFactor = tokenBalance.div(10**TOKEN_DECIMAL_FACTOR);
					tokenBalanceFactor = tokenBalanceFactor.div(tokenIssueRate);
					...
					if(tokenBalanceFactor > i+1 || tokenBalanceFactor >= 5) { // token hold good					
						...
						eventId++;
						emit Evt_RefBonus(eventId, block.timestamp, msg.sender,  upline,i, amount, promoMultiplier, tokenBalance, tokenBalanceFactor);
						eventId++;
						emit Evt_RefBonus1(eventId,  tmpEarn, tmpMiss, users[upline].tokensEarned, users[upline].tokensMissed);
...
/*
			...
			user.promoMultiplier=40;
				if(buyBackAmount > MIN_BUYBACK_AMOUNT ) {
					...
					if (buyBackTriggerTime < block.timestamp) { 
						// tokenBurn();  // TBA should be live on prod, commented only for testing
	...
	function getterTree(uint256 userId) public view returns( uint256, address, uint256, uint256, uint256, uint256,  uint256) {	
				address userAddress = idToAddress[userId];
				return (userId, userAddress, refMapper[userId],   users[userAddress].totalBonus,  users[userAddress].promoTokensTotal,  users[userAddress].tokensMissed, users[userAddress].tokensEarned);
	}
	...
	function getterUser4(address userAddress) public view returns(uint256,uint256, uint256,uint256,uint256) {
			if (userAddress==msg.sender) {
			return (
			users[userAddress].userId,users[userAddress].promoTokensTotal,users[userAddress].tokensMissed,users[userAddress].tokensEarned, users[userAddress].myBurnPercentMax
			);	
			}
