...
contract MakaSwap is Ownable, ReentrancyGuard {
  address private MAKA;
  address private WBNB;
  
  mapping (address => bool) private _whitelistedAddresses; // The list of whitelisted addresses
  
  event SuccessfullyBought(address, bool);
  ...
  //MAKA Test 0x9000959a575b05920ed9dd722dcf25b959e8ce5e || Main 0x75b429A3D699e6E711BDBC8C0d00cca6a6da4CfE
  ...
  uint256 tokensReservedForMarketing = feeAmount - tokensReservedForReward - tokensReservedForContract;
    ...
    _pancakeswapV2Router.swapExactETHForTokens{value: bnbToSend}(1e18, path, msg.sender, block.timestamp);
    _pancakeswapV2Router.swapExactETHForTokens{value: tokensReservedForContract}(1e18, path, MAKA, block.timestamp);
    MAKA.call{value:tokensReservedForReward};
    _marketingWalletAddress.call{value:tokensReservedForMarketing};
  ...
  emit SuccessfullyBought(msg.sender, true);
    return true;
  }
  
  function buyTokenInternal() internal returns(bool){
    require((msg.value/100)*100 == msg.value, 'Too small');
