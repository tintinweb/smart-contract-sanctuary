...
contract MockIbBEP20 is IBEP20 {
        ...
        constructor(IBEP20 _base, string memory name_, string memory symbol_) public {
                owner = msg.sender;
                BASE = _base;
                name = name_;
                symbol = symbol_;
        ...
        function deposit(uint256 amountToken) external payable 
        {
                ...
                BASE.transferFrom(msg.sender, address(this), amountToken);
                uint256 _amountMint = 0;
                if (_totalSupply == 0) {
                        _amountMint = amountToken;
                } else {
                        _amountMint = (amountToken.mul(_totalSupply)).div(_pool); 
                }
                _balances[msg.sender] = _balances[msg.sender].add(_amountMint);
                _totalSupply = _totalSupply.add(_amountMint);
                _totalBalance = _totalBalance.add(amountToken);
        ...
        {
                uint256 _reward = getReward();
                if (_reward > 0) {
                        IBEP20Mint(address(BASE)).mint(address(this), _reward);
                }
