...
contract ForthBoxGameNFT is ERC721Enumerable,ReentrancyGuard  {
    ...
    constructor () ERC721("ForthBox Warplane NFT", "Warplane NFT") {
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721) returns (string memory){
        ...
        if (bytes(base).length != 0) {
            json1 = string(abi.encodePacked(
                '{"name":"ForthBox Warplane NFT",',
                '"description":"Warplane NFT",',
                '"image":"',imageAdress, '",',
                '"base":"',base, '",',
    ...
    function TransferNFTs(address[] calldata _to, uint256[] calldata _value) external returns (bool){
        require(_to.length > 0);
        for(uint256 i=0; i < _to.length ; i++){
            transNFT(_to[i], _value[i]);
        }
        return true;
    }

    //---write onlyOwner---//
    function mintNFTs(uint256 num,uint256 degree) public onlyOwner {
        require(num<=1000, "ForthBoxNFT: num exceed 100!");
        require(degree<=4, "ForthBoxNFT: degree exceed 4!");
        require(degree>=1, "ForthBoxNFT: degree less than 1!");

        for(uint256 i=0; i<num; ++i) {
            _mintNFT(degree,_msgSender());
        }
        return;
    }
    //---write onlyOwner---//
    ...
    function setImageAdress(string[] memory imageAdresses) external onlyOwner {
        require(imageAdresses.length == _imageAdress.length, "ERC721: Existent ID");
