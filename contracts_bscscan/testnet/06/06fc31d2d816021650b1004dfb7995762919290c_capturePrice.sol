...
contract capturePrice is VRFConsumerBase, Context{
    using SafeMathChainlink for uint256;
    IERC20 rigel;
...
constructor(
    ...
    function submitBid(uint256 _pid, uint256 _quantity) public returns(bool){
    ...
    function getChainlinkRandomNumber() public returns(bytes32 _requestID){
        return requestRandomness(keyHash, fee);
    }
    
    function fulfillRandomness( bytes32 requestId, uint256 randomness) internal override {
       
        randomResult = randomness.mod(projBidders[0].length);
    }
    
    function expand(uint256 _pid) internal returns (uint256[] memory expandedValues) {
        CreateBid storage bid = createBid[_pid];
        expandedValues = new uint256[](5);
    ...
    // // function randBidders(uint256 _pid) public returns(address[] memory) {
        
    // function getRandomBidders(uint256 _pid) public returns(bool) {
    //     CreateBid storage bid = createBid[_pid];
        
    //     if (projBidders[_pid].length > 2) {
    //         uint256 _amount = bid.totalBidding.mul(10E18).div(100E18);
    //         holderAccts.push(projBidders[_pid][1]);
    //         holderAccts.push(projBidders[_pid][3]);
    //         holderAccts.push(projBidders[_pid][2]);
    ...
    function WBNBSafeWithdraw(address _to, uint256 _amount, uint256 _pid) onlyOwner public {
        CreateBid storage bid = createBid[_pid];
