...
contract Lunicorns is ERC721Metadata {
    ...
    uint256 public MINT_PRICE1 = 0.07 ether;
	uint256 public MINT_PRICE2 = 0.1  ether;
	uint256 public MINT_PRICE3 = 0.12  ether;
    ...
    function calculatePrice() public view returns (uint count) {
    ...
    function purchase(uint256 numberOfNfts) external payable {
        ...
        require(msg.value >= calculatePrice() * (numberOfNfts), "BNB value sent is below the price");
        require(totalSupply() + (numberOfNfts) <= MAX_NFT_SUPPLY,"Lunicorns: sale already ended");
        if(tax_freeUsers[msg.sender] == 0) require(MINT_PRICE1 * (numberOfNfts) == msg.value, "Lunicorns: invalid ether value");
        else if (tax_freeUsers[msg.sender] < numberOfNfts){
            require(MINT_PRICE1 * (numberOfNfts -  tax_freeUsers[msg.sender]) == msg.value, "Lunicorns: invalid ether value");
    ...
    function reserveGiveaway(uint256 numberOfNfts) public onlyOwner {
        uint currentSupply = totalSupply();
        require(numberOfNfts > 0 && numberOfNfts <= 100, "Max 100 to reserve at once");
        require(totalSupply()+ (numberOfNfts) <= MAX_NFT_SUPPLY, "Exceeds MAX_APEBOYS"); 
        require(hasSaleStarted == false, "Sale has already started");
        uint256 index;
        for (index = 0; index < numberOfNfts; index++) {
