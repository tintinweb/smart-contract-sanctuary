...
contract PrivateSell is ExtendedAccessControl {
        ...
        returns (uint256)
    {
        ...
        uint256 whitelistedA = whitelistedAmount(_address);
        
        if(whitelistedA == 0 && block.timestamp <= PRESALE_START_TIMESTAMP + 12 hours + 10 minutes){
            return 0;
        }
        ...
        if (
            ...
            return whitelistedA - _tokenBuyed[_address];
        }
        

        return 1000 ether + whitelistedA - _tokenBuyed[_address];
    }
