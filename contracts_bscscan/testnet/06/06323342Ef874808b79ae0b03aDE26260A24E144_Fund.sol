...
import "./watcher/impl/PriceWatcher.sol";

contract Fund is PriceWatcher {
    ...
    function query() onlyOwner external {
        ...
        string memory reqId = string(abi.encodePacked("bsc-", requestCounter.current()));
        watcherQuery(reqId, QueryType.UpdatePrice, "");
    }
...
abstract contract PriceWatcher is Watcher {
    ...
    function getPriceOracle() public view returns (PriceOracle memory priceOracle) {
        require(encodedPriceOracle.length > 0, "WATCHER: invalid price data");
        ...
        require(decoder.finished(), "WATCHER: data decode not finished");
    } 

    /**
     * @dev An implementation of the IWatcher callback function will be called after Watcher finished external querying. 
     *
     * Requirements:
     *
     * - resolverAddress must be correct
     */
    function watcherCallback(string memory _reqId, QueryType _type, bytes calldata _calldata) external override {
        require(msg.sender == resolverAddress, "WATCHER: invalid resolver address");
...
abstract contract Watcher is IWatcher, Ownable {
    ...
    event WatcherQuery(string _reqId, address _caller, QueryType _type, bytes _calldata);
    ...
    function watcherQuery(string memory _reqId, QueryType _type, bytes memory _calldata) internal {
...
interface IWatcher {
    function watcherCallback(string memory _reqId, QueryType _type, bytes calldata _calldata) external;
}
