...
contract LastSurvivorCharacter is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, OwnableUpgradeable, ERC721BurnableUpgradeable {
    ...
    function initialize() initializer public {
        __ERC721_init("Last Survivor Character", "LSCC");
    ...
    function _updateTokenOwner(address from, address to, uint256 tokenId) internal {
        if (from == address(0)){
    ...
    function safeMint(address to, uint256 _charId, uint256 _typeID) public onlyWhitelistMinter returns (uint256){
        _safeMint(to, _tokenIdCounter.current());
        emit NewCharacter(_tokenIdCounter.current(), to);
        properties[_tokenIdCounter.current()].ascensionLevel = 1;
    ...
    function getTokenOwners(address _owner, uint256[] memory _selectedIdx) public view returns (CharacterInfo[] memory) {
        if (_selectedIdx.length > 0) {
            ...
            for (uint256 i =0; i < _selectedIdx.length; i ++) {
                nftInfos[i] = properties[tokensOfOwners[_owner][_selectedIdx[i]]];
            }
            return nftInfos;
        } else {
