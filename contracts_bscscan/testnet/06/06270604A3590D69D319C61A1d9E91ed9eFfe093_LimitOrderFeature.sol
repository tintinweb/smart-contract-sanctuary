    ...
    FCXAuth
{
    ...
    function createNativeOrder(
        LibNativeOrder.NativeOrder calldata order,
        LibSignature.Signature calldata signature
    )
    public
    override
    ...
    {    
        address takerAddress = msg.sender;
        if (order.makerToken == LibNativeOrder.NULL_ADDRESS) {
            require(msg.value == order.makerAmount);
        } else {
            require(msg.value == order.takerAmount);
        }

        orderInfo = getNativeOrderInfo(order);
        // Either our context is valid or we revert
        _assertFillableNativeOrder(
            order,
            orderInfo,
            takerAddress,
            signature
        ...
        if (order.makerToken == LibNativeOrder.NULL_ADDRESS) {
            IERC20TokenV06 makerAddressToken;
            makerAddressToken = IERC20TokenV06(order.makerToken);
            _transferERC20Tokens(
                makerAddressToken,
                order.maker,
                address(this),
                order.makerAmount.safeAdd(order.makerTokenFeeAmount)
            );
        }
        ...
        LibNativeOrdersStorage
        .getStorage()
        .orderLocked[orderInfo.orderHash] = order.makerAmount.safeAdd(order.makerTokenFeeAmount);
        
        LibNativeOrdersStorage
        .getStorage()
        .orderHashToFeeAmountRemaining[orderInfo.orderHash] = order.makerTokenFeeAmount;

        emit TransferDone(order.maker, address(this));
        emit LockedBalanceNativeOrder(
            orderInfo.orderHash,
            order.maker,
            order.taker,
            address(this)
    ...
    function _assertFillableNativeOrder(
        LibNativeOrder.NativeOrder memory order,
        LibNativeOrder.OrderInfo memory orderInfo,
        address takerAddress,
        LibSignature.Signature memory signature
    )
    ...
    function getNativeOrderInfo(LibNativeOrder.NativeOrder memory order)
    public
    view
    returns (LibNativeOrder.OrderInfo memory orderInfo)
    {
        // Compute the order hash and fetch the amount of takerAsset that has already been filled
        LibNativeOrder.OrderInfo memory orderInfo = getNativeOrderInfoV2(order, order.makerAmount);
    ...
    returns (bytes4 success)
    {
        ...
        _registerFeatureFunction(this.createNativeOrder.selector);
...
interface ILimitOrderFeature
{
  ...
  event LockedBalanceNativeOrder(
    bytes32 indexed orderHash,
    address indexed makerAddress,         // Address that created the order.
    address takerAddress,                 // Address that filled the order.
    address matchOrderAddress
  ...
  external payable returns (LibNativeOrder.OrderInfo memory orderInfo);
