...
contract ZmnBtcBank is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    struct Profile {
        uint16 feeBP; // Lending fee in basis points
        uint256 period; // Block period to payback
    ...
    struct LoanAgreement {
        ...
        uint256 amountFeeZmn; // Amount of zmn as fee
        uint256 amountBorrowBtcb; // Amount of borrowing BTCB
        uint16 status; // 0: Ongoing, 1: End without payback BTCB, 2: End with payback BTCB
    }
    ...
    // adjust period
    // when ongoing contract from zmine.com get payback or overdue
    bool public adjustPeriod;
    ...
    function initialize(IBEP20 _zmn, IBEP20 _btcb) public initializer {
        __Ownable_init();
        zmn = _zmn;
        btcb = _btcb;
        minBorrowAmount = 10000 ether;
        adjustPeriod = true;
    ...
    function manualAddRate(
        ...
        uint256 _accPaybackBtcb,
        uint256 _ongoingBorrowingBtcb,
        uint256 _accFeeZmn,
        uint256 _accOverdueCollateralZmn
    ) public onlyOwner {
        require(adjustPeriod, "Cannot manual add rate");
        RateHistory memory _r = RateHistory({
            rate: _rate,
            circulateZmn: _circulateZmn,
            accFeeZmn: _accFeeZmn,
            accOverdueCollateralZmn: _accOverdueCollateralZmn,
            accIncomingBtcb: _accIncomingBtcb,
            accBorrowingBtcb: _accBorrowingBtcb,
            accPaybackBtcb: _accPaybackBtcb,
            ongoingBorrowingBtcb: _ongoingBorrowingBtcb,
            epoch: block.timestamp
        });
        rates.push(_r);

        calcRate = RateHistory({
            rate: 0,
            circulateZmn: 0,
            accFeeZmn: _r.accFeeZmn,
            accOverdueCollateralZmn: _r.accOverdueCollateralZmn,
            accIncomingBtcb: 0,
            accBorrowingBtcb: _r.accBorrowingBtcb,
            accPaybackBtcb: _r.accPaybackBtcb,
            ongoingBorrowingBtcb: _r.ongoingBorrowingBtcb,
            epoch: 0
        });
    ...
    function setAdjustPeriod(bool _adjustPeriod) public onlyOwner {
        adjustPeriod = _adjustPeriod;
    }
    ...
    function addProfile(
        ...
        require(_feeBP <= 10000, "Invalid feeBP basis points");
        profiles.push(
            Profile({feeBP: _feeBP, period: _period, enable: _enable})
        );
    }
    ...
    function setMinBorrowAmount(uint256 _minBorrowAmount) public onlyOwner {
    ...
    {
        if (_amountCollateralZmn < minBorrowAmount) {
            return (0, 0, 0);
        }
        ...
        uint256 amountIn = _amountCollateralZmn;
        uint256 fee = 0;
        if (profile.feeBP > 0) {
            fee = _amountCollateralZmn.mul(profile.feeBP).div(10000);
    ...
    function borrow(uint256 _pid, uint256 _amountCollateralZmn) public {
        require(
            _amountCollateralZmn >= minBorrowAmount,
        ...
        // transfer zmn to contract
        zmn.safeTransferFrom(address(msg.sender), address(this), amountIn);
        ...
        if (fee > 0) {
            calcRate.accFeeZmn = calcRate.accFeeZmn.add(fee);
        }
        ...
        emit Borrow(address(msg.sender), _amountCollateralZmn);
    }
    ...
    function payback(uint256 _loanId) public {
        ...
        // update calc information
        calcRate.accPaybackBtcb = calcRate.accPaybackBtcb.add(
            _loanAgreement.amountBorrowBtcb
        );
    ...
    function update(uint256 _loanId) public {
        require(_loanId < loanAgreements.length, "No loan agreement");
        LoanAgreement storage _loanAgreement = loanAgreements[_loanId];
        if (_loanAgreement.status == 0) {
            if (block.timestamp > _loanAgreement.endTime) {
                // End without payback BTCB (overdue)
    ...
    function massUpdateOngoing() public {
