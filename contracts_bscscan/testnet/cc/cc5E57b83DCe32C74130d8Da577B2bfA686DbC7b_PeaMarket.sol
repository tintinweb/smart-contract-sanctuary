...
contract PeaMarket {
    ...
    constructor(
        ...
        peanNFT = PeaNFT(_peanNFT);
        router = IPeaManager(_manager);
    }

    modifier notEgg(uint256 _tokenId) {
        require(peanNFT.getPean(_tokenId).level != 0, "not evolved");
        _;
    }
    ...
    function placeOrder(uint256 _tokenId, uint256 _price)
        ...
        notEgg(_tokenId)
    {
        require(peanNFT.ownerOf(_tokenId) == msg.sender, "Only owner can sell");
        require(_price > 0, "Cannot sell without fee");
        peanNFT.transferFrom(msg.sender, address(this), _tokenId);
    ...
    function cancelOrder(uint256 _tokenId) public {
        ...
        peanNFT.transferFrom(address(this), msg.sender, _tokenId);
    ...
    function fillOrder(uint256 _tokenId) public {
        ...
        peanToken.transferFrom(msg.sender, router.feeAddress(), feeMarket);
