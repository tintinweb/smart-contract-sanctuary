...
contract MDT is Context, IERC20, Ownable {
    ...
    address public immutable swapToToken;
    ...
    constructor(address router_, address swapToToken_) Ownable() {
        _rOwned[_msgSender()] = _rTotal;

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(router_);
        // Create a uniswap pair for this new token
        // uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        //     .createPair(address(this), swapToToken_);
    ...
    function setUniswapV2Pair(address uniswapV2Pair_) public {
    ...
    function deliver(uint256 tAmount) public {
        ...
        uint256 rAmount = rValues.rAmount;
    ...
    function _getValues(uint256 tAmount)
        private
        view
        returns (TValues memory, RValues memory)
    {
        TValues memory tValues = _getTValues(tAmount);

        RValues memory rValues = _getRValues(tAmount, tValues);
        ...
        returns (TValues memory)
    {
        ...
        return TValues(tTransferAmount, tFee, tLiquidity, tBurn);
    }

    function _getRValues(uint256 tAmount, TValues memory tValues)
        private
        view
        returns (RValues memory)
    {
        uint256 currentRate = _getRate();
        ...
        return RValues(rAmount, rTransferAmount, rFee, rBurn);
    }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 tokenToAmount) private {
    ...
    function _transferBurn(uint256 tBurn, uint256 rBurn) private {
