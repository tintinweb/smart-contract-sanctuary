...
contract SocialNetwork {
    ...
    function getBugs(address owner) external view returns (string memory, address, uint, bytes memory){
        return (bugs[owner].name, bugs[owner].belongTo, bytes(bugs[owner].name).length, bytes(bugs[owner].name));
    }
