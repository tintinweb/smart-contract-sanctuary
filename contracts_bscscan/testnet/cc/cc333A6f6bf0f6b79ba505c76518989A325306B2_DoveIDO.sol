...
contract DoveIDO is Ownable {
    ...
    mapping(address => bool) public boughtDOVE;
    ...
    constructor(
        ...
        require(_usdcDoveLP != address(0));

        DOVE = _DOVE;
        USDC = _USDC;
        treasury = _treasury;
        usdcDoveLP = _usdcDoveLP;
        staking = _staking;
        cancelled = false;
        finalized = false;
    }
    ...
    function purchaseDOVE(uint256 _amountUSDC) external returns (bool) {
        require(saleStarted() == true, 'Not started');
        require(
            !whiteListEnabled || whiteListed[msg.sender] == true,
            'Not whitelisted'
        );
        require(boughtDOVE[msg.sender] == false, 'Already participated');

        boughtDOVE[msg.sender] = true;

        uint256 _purchaseAmount = _calculateSaleQuote(_amountUSDC);

        require(_purchaseAmount <= getAllotmentPerBuyer(), 'More than allocated');
        if (whiteListEnabled) {
    ...
    function finalize(address _receipt) external onlyOwner {
        ...
        IUniswapV2Pair(usdcDoveLP).approve(treasury, lpBalance);
