...
contract KeplerFactory is Ownable {
    ...
    event TokenInfo(address token, string name, string symbol, uint256 decimals);
    event PairCreated(address indexed token0, address indexed token1, address pair, uint);
    ...
    function createPair(address tokenA, address tokenB) public returns (address pair) {
        ...
        emit TokenInfo(token0, ERC20(token0).name(), ERC20(token0).symbol(), ERC20(token0).decimals());
        emit TokenInfo(token1, ERC20(token1).name(), ERC20(token1).symbol(), ERC20(token1).decimals());
...
contract KeplerPair is ERC20 {
    ...
    event Mint(address indexed sender, uint amount0, uint amount1, uint reserve0, uint reserve1, uint liquidity, uint totalSupply);
    event Burn(address indexed sender, uint amount0, uint amount1, address indexed to, uint reserve0, uint reserve1, uint liquidity, uint totalSupply);
    event Swap(
        address indexed sender,
        uint amount0In,
        uint amount1In,
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        emit Mint(msg.sender, amount0, amount1, balance0, balance1, liquidity, _totalSupply.add(liquidity));
    }
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        _update(balance0, balance1, _reserve0, _reserve1);
        emit Burn(msg.sender, amount0, amount1, to, balance0, balance1, liquidity, _totalSupply.sub(liquidity));
    }
