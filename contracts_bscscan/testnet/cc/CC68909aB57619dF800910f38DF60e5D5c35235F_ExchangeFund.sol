...
error ExchangeFundUnsufficientDeposits();
...
contract ExchangeFund is Governed, ReentrancyGuard {
    ...
    mapping(address => mapping(IPancakeSwapPair => uint256)) liquidity;

    event Deposit(address indexed depositor, IERC20 indexed token, uint256 amount);
    event Withdrawal(address indexed depositor, IERC20 indexed token, uint256 amount);
    ...
    function withdraw(IERC20 token, uint256 amount) external nonReentrant {
        ...
        if (normalizedAmount > deposits[msg.sender][token]) {
            revert ExchangeFundUnsufficientDeposits();
        }
