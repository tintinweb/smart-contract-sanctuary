...
contract NFT is ERC721, Ownable {
    ...
    // 用户的总魅力值
    mapping (address => uint256) public userCharm;
    ...
    // 添加新的卡牌系列
    // 参数1: 系列ID
    function addSeries(uint256 _seriesId) public onlyOwner {
        // 系列id必须不存在
    ...
    // 参数1: 系列id
    //
    function niopPublish(uint256 _seriesId) external onlyOwner {
        // 新增一个系列id
        addSeries(_seriesId);
        // 这21个token都给到管理员
    ...
    function _randomID64() private view returns (uint256 _tokenId) {
        uint64 _id = uint64(uint256(keccak256(abi.encodePacked(msg.sender, block.difficulty, now))));
        // uint256接受uint64, 很稳;
    ...
    function _mint(address _address, uint256 _seriesId, uint256 _tokenId,  uint256 _color, uint256 _charm) private {
        ...
        // 用户的魅力值
        userCharm[_address] = userCharm[_address].add(_charm);
    ...
    function _burn(address _address, uint256 _tokenId, uint256 _charm) private {
        ...
        // 用户的魅力值
        userCharm[_address] = userCharm[_address].sub(_charm);
        // 触发一个销毁事件
    ...
    function _transfer(address _from, address _to, uint256 _tokenId) private {
        ...
        // 发送方的魅力值
        userCharm[_from] = userCharm[_from].sub(tokenMapping[_tokenId].charm);
        // 接收方的魅力值
        userCharm[_to] = userCharm[_to].add(tokenMapping[_tokenId].charm);
        // 触发交易事件
    ...
    function openBlindBox(uint256 _seriesId) external {
        ...
        // 消耗hnft; 转给合约地址, 然后取不出来, 等于销毁了;
        (bool success, ) = hnftAddress.call(
            abi.encodeWithSelector(TRANSFERFROM, msg.sender, address(this), hnftNumber)
        );
        if(!success) {
    ...
    function userBuyOrder(uint256 _orderId) external {
        ...
        // 订单状态必须是已挂起
        require(o_.status == 1, "NFT: Order id is not push");
        // 转usdt, 99%给买方, 1%给合约用于分成
        uint256 _value99 = o_.price.mul(99).div(100);
        uint256 _value1 = o_.price.mul(1).div(100);
        (bool success1, ) = usdtAddress.call(
            abi.encodeWithSelector(TRANSFERFROM, msg.sender, o_.seller, _value99)
        );
        if(!success1) {
            revert("NFT: transfer fail 1");
        }
        (bool success2, ) = usdtAddress.call(
            abi.encodeWithSelector(TRANSFERFROM, msg.sender, address(this), _value1)
        );
        if(!success2) {
            revert("NFT: transfer fail 2");
        }
        // 全部的手续费增加
        allFee = allFee.add(_value1);
