...
contract PancakePredictionV2 is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public rewardRate; // reward rate (e.g. 9800 = 98%)
    uint256 public treasuryRate; // treasury rate (e.g. 200 = 2%, 150 = 1.50%)
    ...
    event Claim(address indexed sender, uint256 indexed claimedEpoch, uint256 amount);
    ...
    event NewRewardAndTreasuryRates(uint256 indexed epoch, uint256 rewardRate, uint256 treasuryRate);
     ...
     * @param _rewardRate: reward rate (9000 = 90%)
     * @param _treasuryRate: treasury rate (1000 = 10%)
     */
    constructor(
        ...
        require(_rewardRate + _treasuryRate == 10000, "Must equal 10000 (100 * 1e2)");
    ...
    function setOracle(address _oracle) external whenPaused onlyAdmin {
        ...
        // Add a dummy check to make sure the interface implements this function properly
    ...
    /**
     * @notice Set reward and treasury rates
     * @dev Callable by admin
     */
    function setRewardAndTreasuryRates(uint256 _rewardRate, uint256 _treasuryRate) external whenPaused onlyAdmin {
        require(_rewardRate + _treasuryRate == 10000, "Must equal 10000 (100 * 1e2)");

        rewardRate = _rewardRate;
        treasuryRate = _treasuryRate;

        emit NewRewardAndTreasuryRates(currentEpoch, rewardRate, treasuryRate);
    }
    ...
    /**
     * @notice Return round epochs and bet information for a user that has participated
    ...
    function _calculateRewards(uint256 epoch) internal {
        ...
        if (round.closePrice > round.lockPrice) {
            ...
            rewardAmount = (round.totalAmount * rewardRate) / 10000;
