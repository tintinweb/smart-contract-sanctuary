...
contract APIDO is ReentrancyGuard, OwnableUpgradeable {
    ...
    // address => amount
    mapping(address => uint256) public nextTimeWithdraw;
    // participators
    ...
    function setTime(uint256 _nextTime) public onlyOwner {
        nextTime = _nextTime;
    }


    function claim(address _account,uint256 _amount) external onlyActive onlyOperator nonReentrant {
        require(nextTimeWithdraw[msg.sender] <=  block.timestamp, "you can't withdraw 48h!");
        require(_amount <= maxAmount, "value must <= max");
        require(_amount>0,"!_amount");
        uint256 userAmount = _amount;
        _tokenTransfer(_account, userAmount);
        nextTimeWithdraw[msg.sender] = block.timestamp + nextTime; 
    }

    function _tokenTransfer(address _account, uint256 _amount) internal {
        uint256 _releaseAmount = _amount;
        IERC20(token_).safeTransfer(_account, _releaseAmount);
    }

    function nextTimeWdraw(address _user) external view returns (uint256) {
        return nextTimeWithdraw[_user];
    }
