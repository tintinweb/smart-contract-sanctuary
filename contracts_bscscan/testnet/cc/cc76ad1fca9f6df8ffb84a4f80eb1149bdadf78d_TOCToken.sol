...
contract TOCToken is BEP20Detailed, BEP20 {

    uint256 private totalTokens;

    constructor() public BEP20Detailed("TOC Token", "TOC", 18){
    ...
    function transfer(address _receiver, uint256 _amount) public returns (bool success) {
        require(_receiver!=address(0),"Not receiver");
    ...
    function transferFrom(address _from, address _receiver, uint256 _amount) public returns (bool) {
        require(_from != address(0),"From should not zero address");
        require(_receiver != address(0),"Receiver should not zero address");
        require(_amount <= allowance(_from, msg.sender),"Amount should not below allowance");

        return BEP20.transferFrom(_from, _receiver, _amount);
    }
    
    function () payable external {  
        revert("Error override");
    }
}