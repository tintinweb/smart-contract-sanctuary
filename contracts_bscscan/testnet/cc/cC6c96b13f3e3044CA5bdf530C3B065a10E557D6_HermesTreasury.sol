...
contract HermesTreasury is Ownable {
  ...
  address public sHRMS;
  uint256 public sHRMSQueue; // Delays change to sHRMS address
  ...
  constructor(
    ...
    // isLiquidityToken[ _HRMSBAI ] = true;
    // liquidityTokens.push( _HRMSBAI );

    blocksNeededForQueue = _blocksNeededForQueue;
  }

  /**
        @notice allow approved address to deposit an asset for HRMS
        @param _amount uint
  ...
  function deposit(
    ...
    (_token, _amount);
    // mint HRMS needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(HRMS).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn HRMS for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOfToken(_token, _amount);
    IHRMSERC20(HRMS).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sHRMS).balanceOf(msg.sender); // Can only borrow against sHRMS held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with HRMS
        @param _amount uint
     */
  function repayDebtWithHRMS(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");

    IHRMSERC20(HRMS).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, HRMS, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(HRMS).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(HRMS).totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns HRMS valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  {
    if (isReserveToken[_token]) {
      // convert amount to match HRMS decimals
      value_ = _amount.mul(10**IERC20(HRMS).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SHRMS) {
      // 9
      sHRMSQueue = block.number.add(blocksNeededForQueue);
    } else return false;
