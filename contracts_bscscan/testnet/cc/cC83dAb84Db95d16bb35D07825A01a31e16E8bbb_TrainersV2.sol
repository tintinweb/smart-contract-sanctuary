...
contract TrainersV2 is ITrainersV2, Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    function _onlyMigratorContract() internal view {
        require(hasRole(ROLE_MIGRATOR_CONTRACT, msg.sender), "Not migrator contract");
    }
    ...
    function getTrainerPower(uint256 id) public view noFreshLookup(id) returns (uint24) {
        return getTrainerPowerAtLevel(trainers[id].level);
    }  

    function getTrainerPowerAtLevel(uint8 level) public view returns (uint24) {
    ...
    function setElement(uint256 id, uint8 _element) public onlyGameContract {
        trainers[id].element = _element;
    }
    ...
    function getStaminaPoints(uint256 id) public view noFreshLookup(id) returns (uint8) {
        return getStaminaPointsFromTimestamp(staminaTimestamp[id]);
    }

    function getStaminaPointsFromTimestamp(uint64 _timestamp) public view returns (uint8) {
        if(_timestamp  > now) {
            return 0;
        }
    ...
    // =========================== MINT ===========================
    function mintOneTrainerBySpecs(address _minter, uint8 _element) public override onlyGameContract notInEmergencyPause {
        ...
        staminaTimestamp[tokenID] = uint64(now.sub(getMaxStaminaCooldown()));
    ...
    // =========================== BATTLE ===========================
    function getBattleDataAndDrainStamina(uint256 id, uint8 _stamina) public onlyGameContract returns(uint96) {
        Trainer storage trainer = trainers[id];
        uint8 staminaPoints = getStaminaPointsFromTimestamp(staminaTimestamp[id]);
        require(staminaPoints >= _stamina, "Not enough stamina!");
        ...
        if(staminaPoints >= MAXIMUM_STAMINA) {
            staminaTimestamp[id] = uint64(now - getMaxStaminaCooldown() + drainTime);
        }
        else {
            staminaTimestamp[id] = uint64(staminaTimestamp[id] + drainTime);
        }
        return uint96(trainer.element | (uint96(getTrainerPowerAtLevel(trainer.level)) << 8) | (uint96(preTimestamp) << 32));
    }
    // =========================== BATTLE ===========================
    ...
    function fuse(uint256 _fusionID, uint256 _burnID) public onlyGameContract {
        ...
        if (lastFusionLevel[_fusionID] != 0) {
            require(lastFusionLevel[_fusionID] < trainerLevel, "Reforged");
        }
