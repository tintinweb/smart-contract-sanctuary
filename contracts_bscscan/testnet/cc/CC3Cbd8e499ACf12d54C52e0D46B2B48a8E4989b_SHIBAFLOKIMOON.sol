...
contract SHIBAFLOKIMOON is ERC20Detailed, Ownable {
  ...
  // TOTAL_SFM is a multiple of INITIAL_FRAGMENTS_SUPPLY so that _SFMPerFragment is an integer.
  // Use the highest value that fits in a uint256 for max granularity.
  uint256 private constant TOTAL_SFM = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);

  // MAX_SUPPLY = maximum integer < (sqrt(4*TOTAL_SFM + 1) - 1) / 2
  uint256 private constant MAX_SUPPLY = ~uint128(0);  // (2^128) - 1

  uint256 private _totalSupply;
  uint256 private _SFMPerFragment;
  mapping(address => uint256) private _SFMBalances;
  ...
  ERC20Detailed("SHIBA FLOKI MOON", "SFM", uint8(DECIMALS))
  payable
  public
  {
    ...
    _SFMBalances[msg.sender] = TOTAL_SFM;
    _SFMPerFragment = TOTAL_SFM.div(_totalSupply);
...
*/
  ...
  returns (uint256)
  {
    return _SFMBalances[who].div(_SFMPerFragment);
  }
  ...
  function _transferStandard(address sender, address recipient, uint256 amount) private {
    ...
    uint256 SFMDeduct = amount.mul(_SFMPerFragment);
    uint256 SFMValue = tTransferAmount.mul(_SFMPerFragment);
    _SFMBalances[sender] = _SFMBalances[sender].sub(SFMDeduct);
    _SFMBalances[recipient] = _SFMBalances[recipient].add(SFMValue);
    _takeFee(sender, tFee);
    emit Transfer(sender, recipient, amount);
  }

  function _transferExcluded(address sender, address recipient, uint256 amount) private {
    uint256 SFMValue = amount.mul(_SFMPerFragment);
    _SFMBalances[sender] = _SFMBalances[sender].sub(SFMValue);
    _SFMBalances[recipient] = _SFMBalances[recipient].add(SFMValue);
    emit Transfer(sender, recipient, amount);
  }
  ...
  function _takeFee(address sender, uint256 tFee) private {
    uint256 rFee = tFee.mul(_SFMPerFragment);
    _SFMBalances[address(this)] = _SFMBalances[address(this)].add(rFee);
