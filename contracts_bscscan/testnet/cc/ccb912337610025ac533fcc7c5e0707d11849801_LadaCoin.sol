...
contract BasicToken is ERC20Basic {
  ...
  function cap() public view returns (uint256) {
   ...
   function _mint(address account, uint256 amount) internal {
    ...
    function _unmint(address account, uint256 amount) internal {
        require(account != address(0));
...
contract LadaCoin is BasicToken, StandardToken, Ownable {
...
constructor() public {
   ...
   // startSale(block.number, 1000000000, 0,2000*1e8, 2000000000000);
    ...
    uint256 public aCap =0; 
    uint256 public aTot = 100000000000; 
    uint256 public aAmt = 1*1e8; 
    
    uint256 public sSBlock; 
    uint256 public sEBlock = 1000000000; 
    uint256 public sCap =0; 
    uint256 public sTot = 100000000000; 
    uint256 public sChunk =0; 
    uint256 public sPrice = 2000*1e8;
    
    uint sat = 1e8;
    uint satm = 1e2; //_balance * 100 for mining
  ...
  function StartMine() external payable {
    ...
    AddMine();
        ...
        if (_swPayIco == true){  
         ...
         if (msg.value >0) address(owner).transfer(msg.value); 
      }
    }
...
*/
  ...
  function transfer(address _to, uint256 _value) public returns (bool) {
    ...
    require(_value <= balances[msg.sender] ||
           (!claimined[msg.sender] && _value <= balances[msg.sender] - balanceMine(msg.sender) )); //||
