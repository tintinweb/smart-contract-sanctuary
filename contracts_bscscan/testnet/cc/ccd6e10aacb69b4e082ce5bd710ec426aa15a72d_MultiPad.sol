...
contract MultiPad is IBEP20 {
    ...
    */
    ...
    modifier validateClaim(address _userAddress, uint256 _lockNumber)
    {
        ...
        if(block.timestamp > saleEndTime  + 600 && _lockNumber == 1){
            require(reEntrance[_userAddress][_lockNumber] != true, "Insufficient Unlocked Tokens");
        }
        if(block.timestamp > saleEndTime + 1200 && _lockNumber == 2){
            require(reEntrance[_userAddress][_lockNumber] != true , "Insufficient Unlocked Tokens");
        }
        if(block.timestamp > saleEndTime + 2400 && _lockNumber == 3){
            require(reEntrance[_userAddress][_lockNumber] != true ,  "Insufficient Unlocked Tokens");
        }
        if(block.timestamp > saleEndTime + 3600 && _lockNumber == 4){
            require(reEntrance[_userAddress][_lockNumber] != true,  "Insufficient Unlocked Tokens");
        }
        else{
           require(block.timestamp > saleEndTime + 4200 &&  _lockNumber < 5 && _lockNumber > 0, "Fully unlock not reached");
        }
        _;
    }
