...
contract PriceGameV3Upgradeable is OwnableUpgradeable {
    ...
    address public constant CDZ = 0x17Acc21Da1Cd31d273c3f54b7d5Dd556C8715b79;
    ...
    function _bet(address account, uint32 tokenIndex, uint256 tokenAmount, bool isBull) internal {
        ...
        if (tokenIndex != INDEX_CDZ) {
            amount = _getAmountOut(tokenIndex, tokenAmount);
        } else {
            amount = tokenAmount;
        }
        if (isBull) {
        ...
        for (uint32 i = 0; i < tokens.length; i ++) {
            if (i == INDEX_CDZ) {
                ...
                if (0 < round.bullTokenAmounts[i]) {
                    uint256 amountOutMin = round.bullAmounts[i] * 95 / 100; // 95% of slippage
                ...
                if (0 < round.bearTokenAmounts[i]) {
                    uint256 amountOutMin = round.bearAmounts[i] * 95 / 100; // 95% of slippage
    ...
    function getPlayerList(uint256 epoch) public view returns (address[] memory) {
    ...
    function getClaimableAmount(uint256 epoch, address user) public view returns (uint256) {
        BetInfo memory betInfo = ledger[epoch][user];
        if (0 == betInfo.tokenAmount) return 0;
        if (betInfo.claimed) return 0;

        RoundInfo memory roundInfo = rounds[epoch].base;
        if (0 == roundInfo.rewardAmount) return 0;
        if (roundInfo.stage != RoundStages.Ended) return 0;

        if (claimable(epoch, user) == false) return 0;

        return _getRewardForUser(epoch, user, betInfo.tokenIndex);
    }
    ...
    function _calculateRewards(uint256 epoch) internal {
        ...
        if (roundInfo.pseudoRound || roundInfo.closePrice == roundInfo.lockPrice) {
            ...
            round.base.rewardAmount = roundInfo.bullAmount + roundInfo.bearAmount;
        } else {
            // Bear wins
    ...
    function _getRewardForUser(
        ...
        if (tokenIndex == INDEX_CDZ) {
            ...
            if (betInfo.position == Position.Bull) {
                reward =  betInfo.tokenAmount
                    * round.bullAmounts[tokenIndex]
                    * roundInfo.rewardAmount
                    / round.bullTokenAmounts[tokenIndex]
                    / roundInfo.rewardBaseCalAmount;
            } else {
                reward =  betInfo.tokenAmount
                    * round.bearAmounts[tokenIndex]
                    * roundInfo.rewardAmount
                    / round.bearTokenAmounts[tokenIndex]
                    / roundInfo.rewardBaseCalAmount;
            }
        }
    }
    ...
    //         BetInfo memory betInfo = ledger[epoch][claimant];
    //         if (betInfo.claimed) continue;
    //         if (claimable(epoch, claimant) == false) continue;

    //         reward = _getRewardForUser(epoch, claimant, betInfo.tokenIndex);
    //         if (0 < reward) {
    //             _claim(epoch, claimant, reward, betInfo.feeRate);
    //         }
    //     }
    // }

    function _claim(uint256 epoch, address user, uint256 reward, uint32 feeRate) internal {
        uint256 fee = reward * feeRate / DENOMINATOR;
        reward -= fee;
        treasuryAmount += fee;
    ...
    function claim(uint256 epoch) external {
        ...
        uint256 reward = getClaimableAmount(epoch, claimant);
        require(0 < reward, "No reward");

        _claim(epoch, claimant, reward, ledger[epoch][claimant].feeRate);
    }
    ...
    function _getAmountOut(uint32 tokenIndex, uint256 amountIn) internal view returns (uint256) {
        address tokenIn = (tokenIndex == INDEX_BNB) ? WBNB : tokens[tokenIndex];
        return router.getAmountsOut(amountIn, _getPath(tokenIn, CDZ))[1];
    }
    ...
    function _swap(address tokenIn, address tokenOut, uint amountIn, uint amountOutMin) internal returns (uint){
        if(tokenIn == address(0)) {
            ...
            return router.swapExactTokensForTokens(amountIn, amountOutMin, path, address(this), block.timestamp)[1];
        }
    }
}
