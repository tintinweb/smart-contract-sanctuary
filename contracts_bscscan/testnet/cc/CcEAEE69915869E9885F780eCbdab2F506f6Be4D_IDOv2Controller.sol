...
import "../interfaces/IDOv2/IIDOv2Controller.sol";
...
contract IDOv2Controller is Ownable, IIDOv2Controller {
    ...
    function createIDO(
        ...
        IDOv2 ido = new IDOv2(this, saleTarget, fundToken, fundTarget);
...
contract IDOv2 is Ownable, ReentrancyGuard {
    ...
    IIDOv2Controller public controller;
    ...
    function getRoundTotalAllocation(uint256 currentRoundId) public view returns (uint256) {
        ...
        if (currentRoundId <= 4) {
            // tiers
            uint256 multiplierSum = controller.getTotalMultiplier();
            return (fundTarget * controller.getMultiplierAtIndex(4 - currentRoundId - 1)) / multiplierSum;
        }
        return fundTarget - fundRaised;
    }
    ...
    function getMaxAllocation(address addr) public view returns (uint256) {
        ...
        if (currentRoundId <= 4) {
            // tier period
            if (controller.getTierId(addr) == currentRoundId) {
    ...
    function getUnlockedTokenAmount(address addr, uint256 totalSaleToken) private view returns (uint256) {
    ...
    function getTierClaimableAmount(address addr) private view returns (uint256) {
        ...
        return getUnlockedTokenAmount(addr, tierFunders[addr].totalSaleToken) - tierFunders[addr].released;
    }

    function getWhitelistClaimableAmount(address addr) private view returns (uint256) {
        return getUnlockedTokenAmount(addr, whitelistFunders[addr].totalSaleToken) - whitelistFunders[addr].released;
    }

    function getFCFSClaimableAmount(address addr) private view returns (uint256) {
        return getUnlockedTokenAmount(addr, fcfsFunders[addr].totalSaleToken) - fcfsFunders[addr].released;
    }
    ...
    function _processBuy(address buyer, uint256 amount) private {
        ...
        if (currentRoundId <= 4) {
            ...
            tierFunders[buyer].totalSaleToken = tierFunders[buyer].totalSaleToken + saleTokenAmount;
            ...
            whitelistFunders[buyer].totalSaleToken = whitelistFunders[buyer].totalSaleToken + saleTokenAmount;
        } else if (currentRoundId == 6) {
            // fcfs
            if (tierFunders[buyer].totalFunded == 0) {
                funderAddresses.push(buyer);
            ...
            fcfsFunders[buyer].totalSaleToken = fcfsFunders[buyer].totalSaleToken + saleTokenAmount;
        }
