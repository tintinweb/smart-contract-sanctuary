...
contract TSHIBA is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _transfer(
        ...
        if(from == address(uniswapV2Pair) && !_isExcludedFromFee[to] && to != address(uniswapV2Router)) {
            _transferBuy(from, to, amount);
        }
        else if (!inSwap && !_isExcludedFromFee[from] && to == address(uniswapV2Pair)) {
