...
/** @title WateenSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract WateenSwapLottery is ReentrancyGuard, IWateenSwapLottery, Ownable {
    using SafeERC20 for IERC20;

    address public injectorAddress;
    address public operatorAddress;
    address public syrubAddress;
     ...
     * @param _wtnTokenAddress: address of the CAKE token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _wtnTokenAddress, address _randomGeneratorAddress) {
        wtnToken = IERC20(_wtnTokenAddress);
        ...
        nonReentrant
    {
        ...
        wtnToken.safeTransferFrom(address(msg.sender), address(this), amountCakeToTransfer);
    ...
    function claimTickets(
        ...
        wtnToken.safeTransfer(msg.sender, rewardInCakeToTransfer);
        ...
        nonReentrant
    {
        ...
        if (_autoInjection) {
            pendingInjectionNextLottery = amountForBurnandSyrub;
            amountForBurnandSyrub = 0;
        }
        

        amountForBurnandSyrub += (_lotteries[_lotteryId].amountCollectedInCake - amountToShareToWinners);
        

        // Transfer CAKE to treasury address
        wtnToken.safeTransfer(syrubAddress, amountForBurnandSyrub/2);
        wtnToken.transfer(burnAddress,amountForBurnandSyrub/2);
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(wtnToken), "Cannot be CAKE token");
    ...
    /**
     * @notice Set operator, _syrubAddress, and injector addresses
     * @dev Only callable by owner
     * @param _operatorAddress: address of the operator
     * @param _syrubAddress: address of the treasury
     * @param _injectorAddress: address of the injector
     */
    function setOperatorAndTreasuryAndInjectorAddresses(
        address _operatorAddress,
        address _syrubAddress,
        address _injectorAddress
    ) external onlyOwner {
        require(_operatorAddress != address(0), "Cannot be zero address");
        require(_syrubAddress != address(0), "Cannot be zero address");
        require(_injectorAddress != address(0), "Cannot be zero address");

        operatorAddress = _operatorAddress;
        syrubAddress = _syrubAddress;
        injectorAddress = _injectorAddress;

        emit NewOperatorAndTreasuryAndInjectorAddresses(_operatorAddress, _syrubAddress, _injectorAddress);
    }
