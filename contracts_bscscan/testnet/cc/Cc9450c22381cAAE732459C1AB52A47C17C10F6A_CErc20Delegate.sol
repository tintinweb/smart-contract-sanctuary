...
// File: PlanetLending/CTokenInterfaces.sol
...
contract CTokenInterface is CTokenStorage {
    ...
    /**
     * @notice Event emitted when the discounted reserves are reduced
     */
    event DiscountedReservesReduced(address admin, uint reduceAmount, uint newTotalReservesDiscount);
...
contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
        ...
        if(payer == borrower){
            (vars.mathErr, vars.accountBorrows) = borrowBalanceStoredInternal(borrower);
            PlanetDiscount(discountLevel).changeLastBorrowAmountDiscountGiven(borrower,vars.accountBorrows);
        }
        /***************************************************************************************************/
    ...
    function _reduceReservesFresh(uint reduceAmount) internal returns (uint) {
        ...
        require(totalReservesAfterDiscountNew <= totalReserves, "reduce discounted reserves unexpected underflow");

        // Store reserves[n+1] = reserves[n] - reduceAmount
        totalReserves = totalReservesNew;
        totalReservesAfterDiscount = totalReservesAfterDiscountNew;
        
        // doTransferOut reverts if anything goes wrong, since we can't be sure if side effects occurred.
        doTransferOut(admin, reduceAmount);

        emit ReservesReduced(admin, reduceAmount, totalReservesNew);
        emit DiscountedReservesReduced(admin,reduceAmount,totalReservesAfterDiscountNew);
...
contract CErc20 is CToken, CErc20Interface {
    ...
    /**
     * @param borrower address of the borrow whose discount will be calculated
     * @return given borrower current discount.
     */
    // function getUserAvaialaibleDiscount(address borrower) external view returns(uint total) {
        
    //     DiscountLocalVars memory discountVars;
        
    //     DiscountSnapshot storage _dis = discountSnap[borrower];
        
    //     uint discount = currentDiscountPercentage(borrower);
    //     (,discount) = divUInt(discount,2);
        
    //     (,uint accountBorrowsNew) = borrowBalanceStoredInternal(borrower);
        
    //     if(_dis.lastBorrowAmountDiscountGiven != 0)
    //     (,discountVars.borrowDifference) = subUInt(accountBorrowsNew,_dis.lastBorrowAmountDiscountGiven);
    //     else
    //     discountVars.borrowDifference = 0;
    ...
    //     (total) = (discountVars.newDiscount);
    // }
...
// File: PlanetLending/CErc20Delegate.sol
