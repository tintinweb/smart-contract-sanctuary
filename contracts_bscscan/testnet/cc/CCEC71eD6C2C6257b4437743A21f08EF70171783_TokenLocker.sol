...
contract TokenLocker is SafeOwnable {
    ...
    event NewReceiver(address receiver, uint totalReleaseAmount, uint releaseAmount, uint lastReleaseAt);
    ...
    ///@notice how many blocks must be passed before next release
    uint256 public immutable interval;
    ///@notice how many time a receiver can release
    uint256 public immutable releaseNum;
    ///@notice the info of a releaser
    struct ReleaseInfo {
        address receiver;               //who will receive the release token
        uint256 releaseAmount;          //the amount of the token the receiver wille receive one time
    ...
    constructor(
        address _token, uint256 _totalRelease, uint256 _intervalSeconds, uint256 _releaseNum
    ) {
        ...
        interval = interval_;
        require(_releaseNum > 0, "release number is zero");
    ...
    function addReceiver(address _receiver, uint256 _amount) external onlyOwner {
        ...
        require(remainRelease >= _amount, "release amount is bigger than reaminRelease");
        remainRelease = remainRelease.sub(_amount);
        ReleaseInfo storage receiver = receivers[_receiver];
        require(receiver.receiver == address(0), "receiver already exists");
        receiver.receiver = _receiver;
        receiver.releaseAmount = _amount.div(releaseNum);
        require(receiver.releaseAmount > 0, "release amount is zero");
        receiver.totalReleaseAmount = _amount;
        receiver.lastReleaseAt = block.number >= interval ? block.number.sub(interval) : 1;  //we want to release the first token immediately
        receiver.alreadyReleasedAmount = 0;
        emit NewReceiver(_receiver, _amount, receiver.releaseAmount, receiver.lastReleaseAt);
    }

    //response1: the block number for next release
    ...
    function getReleaseInfo(address _receiver) public view returns (uint256, uint256, uint256, uint256) {
        ReleaseInfo storage receiver = receivers[_receiver];
        require(receiver.receiver != address(0), "receiver not exist");
        uint nextReleaseBlockNum = receiver.lastReleaseAt.add(interval);
        uint nextReleaseAmount = 0;
        if (receiver.totalReleaseAmount > receiver.alreadyReleasedAmount) {
            if (receiver.totalReleaseAmount.sub(receiver.alreadyReleasedAmount) > receiver.releaseAmount) {
                nextReleaseAmount = receiver.releaseAmount;
            } else {
                nextReleaseAmount = receiver.totalReleaseAmount.sub(receiver.alreadyReleasedAmount);
            }
        }
        uint alreadyReleasedAmount = receiver.alreadyReleasedAmount;
        uint remainReleasedAmouont = receiver.totalReleaseAmount.sub(receiver.alreadyReleasedAmount);
        return (nextReleaseBlockNum, nextReleaseAmount, alreadyReleasedAmount, remainReleasedAmouont);
    }

    function claim(address _receiver) external {
        (uint nextReleaseBlockNum, uint nextReleaseAmount, , ) = getReleaseInfo(_receiver);
        require(block.number >= nextReleaseBlockNum, "not the right time");
        require(nextReleaseAmount > 0, "already released all");
        ReleaseInfo storage receiver = receivers[_receiver];
        receiver.lastReleaseAt = nextReleaseBlockNum;
        ...
        SafeERC20.safeTransfer(token, receiver.receiver, nextReleaseAmount);
        (uint nextNextReleaseBlockNum, uint nextNextReleaseAmount, , ) = getReleaseInfo(_receiver);
        emit ReleaseToken(_receiver, nextReleaseAmount, nextNextReleaseBlockNum, nextNextReleaseAmount);
    }
}