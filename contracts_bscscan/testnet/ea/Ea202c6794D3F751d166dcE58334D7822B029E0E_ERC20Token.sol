...
contract TWAPOracleUpdater is ERC20Permit, VaultOwned {
  ...
  function addTWAPSource( address newTWAPSourceDexPool_ ) external onlyOwner() {
    require( _dexPoolsTWAPSources.add( newTWAPSourceDexPool_ ), "ERC20Token: TWAP Source already stored." );
    emit TWAPSourceAdded( newTWAPSourceDexPool_ );
  }

  function removeTWAPSource( address twapSourceToRemove_ ) external onlyOwner() {
    require( _dexPoolsTWAPSources.remove( twapSourceToRemove_ ), "ERC20Token: TWAP source not present." );
...
contract ERC20Token is Divine {

  using SafeMath for uint256;

    constructor() Divine("Asgard", "ASD", 9) {
    }
