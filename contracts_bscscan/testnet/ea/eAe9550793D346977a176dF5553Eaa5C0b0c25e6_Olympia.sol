...
contract Olympia is ERC20, Ownable {
    ...
    uint256 private _swapThreshold = 5_000_000 * 10 ** decimals(); // 50M $OLP ( 0.05% )
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (_shouldTakeFees(sender, recipient)) {
            ...
            uint256 remainingFeesPerTenThousand = 10000 - totalFeesPerTenThousand;
            uint256 newAmount = amount.mul(remainingFeesPerTenThousand).div(10000);
            uint256 feesAmount = amount.mul(totalFeesPerTenThousand).div(10000);
    ...
    event _ABC_Debug(uint256 code, address sender, address recipient, uint256 amount);

    bool public _abc_transferFees = false;
    function _abc_shouldTransferFees(bool enabled) external {
        _abc_transferFees = enabled;
    }

    function _def_transferFees(address sender) external {
        super._transfer(sender, address(this), 100);
    }

    bool public _abc_1 = false;
    function _abc_should1(bool enabled) external {
        _abc_1 = enabled;
    }

    bool public _abc_2 = false;
    function _abc_should2(bool enabled) external {
        _abc_2 = enabled;
    }

    bool public _abc_3 = false;
    function _abc_should3(bool enabled) external {
        _abc_3 = enabled;
    }

    bool public _abc_4 = false;
    function _abc_should4(bool enabled) external {
        _abc_4 = enabled;
    }

    bool public _abc_5 = false;
    function _abc_should5(bool enabled) external {
        _abc_5 = enabled;
    }

    function _abc_foo() external {
    }
