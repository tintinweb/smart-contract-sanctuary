...
contract OrderBook is Initializable, Ownable, ReentrancyGuard, IOrderBook {
  ...
  function __fillOrder(uint8 position, uint256 price, uint256 securityAmt, uint256 currencyAmt) private {
    ...
    while(currNodeAddr != address(0) && (securityLeft > 0 || currencyLeft > 0)) {
  ...
  function __fillMatchedOrder(uint8 position, uint256 securityAmt, uint256 currencyAmt, address orderNodeOpAddr) private returns (uint256 securityLeft, uint256 currencyLeft) {
    ...
    address currItemAddr = _orderNodeMap[orderNodeOpAddr].headItem;

    while(currItemAddr != address(0) && (securityLeft > 0 || currencyLeft > 0)) {
      ...
      ITradeOrderV2 tradeItem = ITradeOrderV2(currItemAddr);
      ...
      emit OrderItemLogger(currItemAddr, securityInItem, currencyInItem, securityLeft, currencyLeft);
  ...
  function __makeTradeOrder(ITradeOrderV2 tradeOrder, uint8 position, uint256 securityAmt, uint256 currencyAmt, uint8 orderType) private {
    ...
    uint256 securityBalance = IERC20(_security).balanceOf(address(this));
    uint256 currencyBalance = IERC20(_currency).balanceOf(address(this));

    emit OrderComplete(orderType, address(token0), amount0, msg.sender, address(token1), amount1, transferTo, securityBalance, currencyBalance);
  }
...
interface IOrderBook {
  ...
  event OrderComplete(uint8 orderType, address token0, uint256 amount0, address account0, address token1, uint256 amount1, address account1, uint256 securityBalance, uint256 currencyBalance);
  event OrderNodeFilled(uint8 position, uint256 securityLeft, uint256 currencyLeft, address orderNodeAddr);
  event OrderItemLogger(address itemAddr, uint256 securityInItem, uint256 currencyInItem, uint256 securityLeft, uint256 currencyLeft);
