...
contract Master is Ownable {
    ...
    ISureMoon public suremoon;

    // Market oracle provides the SureMoon/USD exchange rate as an 18 decimal fixed point number.
    ...
    constructor(address _suremoon) public {
        deviationThreshold = 100 * 10 ** (DECIMALS-2);

        targetRate = (2 * 10**(DECIMALS-4));

        ceiling = 1337000 * 10 ** DECIMALS;
    ...
    function rebase() public {
        ...
        uint256 supplyAfterRebase = suremoon.rebase(epoch, supplyDelta);
    ...
    function adjustTargetRate() external
    {
      ...
      if (exchangeRate > targetRate){
        targetRate = exchangeRate.mul(8094131).div(8000000);
      }
    ...
    function incrementTargetRate() internal {
      ...
      uint256 newRate = targetRate.mul(8094131).div(8000000);
    ...
    function getRebaseValues() public view returns (uint256, int256) {
        ...
        if (supplyDelta > 0 && suremoon.totalSupply().add(uint256(supplyDelta)) > MAX_SUPPLY) {
            supplyDelta = (MAX_SUPPLY.sub(suremoon.totalSupply())).toInt256Safe();
        }
        ...
        returns (int256)
    {
        ...
        return suremoon.totalSupply().toInt256Safe()
