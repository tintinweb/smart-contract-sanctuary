...
contract DonutCat is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled) {
                ...
                uint256 contractETHBalance = address(this).balance - 5 * 10**17;  // Always need 0.5BNB in contract balance for swap and transfer fees.
                if (contractETHBalance >= 10**18) { // Send Fee to team when the contract BNB balance is over 1 BNB
    ...
    function contractBalanceSend(uint256 amount, address payable _destAddr) external {
        ...
        uint256 contractETHBalance = address(this).balance - 5 * 10**17;  // we need 0.5BNB in contract balance for swap and transfer fees.
