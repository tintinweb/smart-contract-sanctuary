...
contract OracleV3 is IOracleV3, OracleV2 { 
    ...
    event WithdrawSeedmonRequested(address requester, uint256 requestId, uint256 tokenId, uint256 providedGas);
    event DownloadSeedRequested(address requester, uint256 requestId, address downloader, uint256 providedGas);
    ...
    function requestDownloadSeed(uint256 requestId, address downloader)
        external
        payable
        override
        onlyOpen
        requireGas
    {
        ...
        emit DownloadSeedRequested(requester, requestId, downloader, providedGas);
    }
    ...
    function provideDownloadSeed(address requester, uint256 requestId, uint256 tokenBalance)
        external
        onlyProphet
    {
        require(isRequestPending(requester, requestId), "Oracle: Transaction was refunded or responded"); 
        ...
        IClientV3(requester).onDownloadSeedReceived(requestId, tokenBalance);
        payable(msg.sender).transfer(providedGas);
...
interface IClientV3 is IClientV2 {
    ...
    function onDownloadSeedReceived(uint256 requestId, uint256 tokenBalance) external;
