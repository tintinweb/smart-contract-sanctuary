...
contract Pazzive is ERC20, Ownable {
    ...
    constructor(address routerAddress) ERC20("Pazzive", "PAZ") {
        ...
        swapPair = ISwapFactory(swapRouter.factory()).createPair(
            address(this),
            swapRouter.WETH()
        );
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        PazDividendTracker newDividendTracker = PazDividendTracker(
            payable(newAddress)
        );
        ...
        newDividendTracker.excludeFromDividends(address(swapRouter));
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(swapRouter));
        emit UpdateUniswapV2Router(newAddress, address(swapRouter));
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (
                ...
                from != address(swapRouter) //router -> pair is removing liquidity which shouldn't have max
            ) {
                require(
...
contract PazDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(address payable pazziveAddress, address routerAddress)
    DividendPayingToken(
        "PazDividendTracker",
        "PazDividendTracker"
    )
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "PazDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main Pazzive contract."
        );
    }
    ...
    returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            {
                uint256 received = paz.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
