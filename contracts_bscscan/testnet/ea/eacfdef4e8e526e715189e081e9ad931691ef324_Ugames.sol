...
contract Ugames is Context, IBEP20, Ownable {
    ...
    address public _marketingWallet = 0xCeC6B69b43E7828195C182E5AD8344675CAb48D1;
    address public _freelancerWallet = 0xCeC6B69b43E7828195C182E5AD8344675CAb48D1;
    
    uint256 private _totalFeePermille = 100;
    uint256 private _liquidityFeePermille = 400;
    uint256 private _marketingFeePermille = 200;
    ...
    // We may also need to do this if CARR will be used to deposit in farms (ref. Cerberus etc.)
    ...
    function setFeePermilles (uint256 totalFeePermille, uint256 liquidityFeePermille, uint256 structureFeePermille, uint256 marketingFeePermille, uint256 freelancerFeePermille) external onlyFeeSetter {
        require (liquidityFeePermille.add(structureFeePermille).add(marketingFeePermille).add(freelancerFeePermille) == 1000, "Fee permilles must add up to 1000");
        _totalFeePermille = totalFeePermille;
        _liquidityFeePermille = liquidityFeePermille;
        _structureFeePermille = structureFeePermille;
        _marketingFeePermille = marketingFeePermille;
        _freelancerFeePermille = freelancerFeePermille;
        emit FeePercentagesUpdated (totalFeePermille, liquidityFeePermille, structureFeePermille, marketingFeePermille, freelancerFeePermille);
    }
    ...
    // Does not affect the proper running of the contract - CARR is specifically prevented from being withdrawn in this way
    function withdrawOtherTokens (address _token) external nonReentrant onlyFeeSetter {
        require (_token != address(this), "Can't withdraw CARR from contract");
    ...
    function _getTransferAmount (uint256 amount) private returns (uint256) {
        if (!_enableFees) {
            ...
            uint256 marketingFee = fee.mul(_marketingFeePermille).div(FEE_DIVISOR);
            uint256 freelancerFee = fee.mul(_freelancerFeePermille).div(FEE_DIVISOR);
            uint256 otherFees = fee.sub(marketingFee).sub(freelancerFee);
            _balance[_marketingWallet] = _balance[_marketingWallet].add(marketingFee);
            _balance[_freelancerWallet] = _balance[_freelancerWallet].add(freelancerFee);
            _balance[address(this)] = _balance[address(this)].add(otherFees);
    ...
    function _transfer (address from, address to, uint256 amount) private {
        ...
        // Check we're not already adding liquidity and don't take fees if sender is the pancake pair (i.e. someone is buying CARR).
    ...
    function _swapTokensForBNB (uint256 tokenAmount) private returns (uint256) {
        ...
        // Generate the pancake pair path of CARR -> WBNB
