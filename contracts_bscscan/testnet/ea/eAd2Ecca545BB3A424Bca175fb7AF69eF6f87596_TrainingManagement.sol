...
contract TrainingManagement is TrainingCenter {
    ...
    function registerCoaching(uint256 _monsterId, uint256 _spotId, uint256 _trainingId) external monstOwner(_monsterId){
        ...
        require(I.getMonsterLevel(_monsterId) >= t.coach.minLevelReq, "!Level");
    ...
    monstOwner(_monsterId)
    {
        ...
        if(t.coach.coachId != 0){
            uint256 coachPayment = ownerPayment.mul(t.coach.percentPayment).div(10000);
    ...
    {
        IBandZaiNFT I = IBandZaiNFT(gameAddresses.getMonsterAddress());
        uint256 monsterLevel = I.getMonsterLevel(_monsterId);
        if(_coachId != 0){
            uint256 coachLevel = I.getMonsterLevel(_coachId);
            if(coachLevel.sub(monsterLevel) > 0){
                _duration = _duration.mul(coachLevel.sub(monsterLevel));
            }
        }
