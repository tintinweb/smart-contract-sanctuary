...
/** 
    ...
    @notice Version v1.0.4  date: dec 03, 2021
*/
...
/** @notice What is StakePool
    StakePool provides a basic staking system and has build from scratch 23.09.2021 by AtomPad Dev Team for a clean and basic understanding 
    This contract works in conjuction with Presale.sol and PresaleTest.sol 
    This contract is inherited by 
*/

/** @notice How the contract works
    stakepool.sol is up and running and accounts stake the stakeTokens they are holding
    by calculating the maxium tiers that can be met, this contract holds the allocation points per user (allocPoints)
    the routine allocPercentageOf() dividies the allocPoints by the totalAllocPoints,  so that a percentacge of the total allocaton is calculated for each user.
    the contract presale.sol. reads the a allocPercentageOf() after the start of a presale (routine userConnect()), and distributes the allocations based on these percentages.
    the stake and withdraw routines can be frozen during the presale (variables stakeOn and withdrawOn
     
    @notice Tiers
    HAWKING     Requirements    1000    Pool _weight    15
    TESLA       Requirements    2000    Pool _weight    40
    NEWTON      Requirements    5000    Pool _weight    130
    EINSTEIN    Requirements    10000   Pool _weight    300
    GOD         Requirements    20000   Pool _weight    750
*/
...
contract StakePool is ReentrancyGuard, Ownable, Pausable {
   ...
   modifier withdrawEnabled {
    ...
    uint public timeLock; /// @dev  create a setter to be altered after deployment

    /// @dev control buttons for the admin
    bool stakeOn;   /// set by owner, turns off/on stake process
    bool withdrawOn;   /// set by owner, turns off/on withdraw process
    ...
    /// dev StakePoolFees
    uint32 public constant MAX_FEE = 10000;
    uint32 public withdrawalFee;
    ...
    /// @dev constructor
    /// @param _stakeToken The token that must be staked to participate in any presale.
    ...
    function depositTokens(uint _amount) external payable onlyOwner   { 
        /// do some checks 
    ...
    /// @param msg.sender
    /** @dev returnRewardTokens
    /// @notice admin can withdraw reward tokens
    
     **/
    function returnRewardTokens() external onlyOwner  { // 
        // do some checks 
        require (rewardToken.balanceOf(address(this))> 0,'!Amount');

        uint _remaining = rewardToken.balanceOf(address(this));

        rewardToken.safeTransfer(msg.sender, _remaining);
    ...
    /// @dev stake
    /// @dev called by msg.sender as participant
    /// @param _amount the amount of tokens to stake by this user
    function stake(uint _amount) external payable stakeEnabled {
        // do some checks 
        ...
        rewards[msg.sender] += _reward; // Storing reward before manuplating user balance and timestamp so reward doesn't get lost
      ...
      /// calculate and assing new allocPoints
      ...
      /// update other variables like unlockTime etc. 

      /// add user to the list
        _checkOrAddUser(msg.sender);

        /// EXPERIMENTAL !! 
        // uint _unlock = block.timestamp.add(uint(8 weeks));

        uint _unlock = block.timestamp;   // Staking time
        // timeLocks[msg.sender] = _unlock; 
        timeLocks[msg.sender] = _unlock; // Storing staking time in map

        emit Staked(msg.sender, _amount);
    }
    /// @dev withdraw
    /// @dev called by msg.sender as participant
    /// @param _amount the amount of tokens to withdraw from staking by this user
    function withdraw(uint _amount) public virtual withdrawEnabled {
        // do some checks 
        require(balanceOf(msg.sender) >= _amount, "!StakeAmount");

        /// new function calculate the fee in percentage
        // then takeof this from the _amount

        ///@dev Calculating fee based on _amount   

        uint _fee =  calculateWithdrawFees(_amount); 

        ///@dev Adding _fee to storage variable collected fee
        collectedFee += _fee;

        /// transfer tokens to user
        // stakeToken.safeTransfer(msg.sender, _newAmount);

        rewards[msg.sender] += earned(msg.sender);

        // Calculating withdraw amount by deducting _fee from _amount
        ...
        /// remove from userList if no Tier
        // if (allocPoints[msg.sender] == 0) _removeUser(msg.sender);
        ...
        /// remove users if stakes == 0
         /// allocPoints.remove(msg.sender);
         // /stakeBalances.remove(msg.sender);
    ...
    function calculateWithdrawFees(uint _amount) internal view returns (uint _fee){
        ...
        uint _stakingTime = timeLocks[msg.sender];  // Storing time when user staked to local variable
        // uint _stakingBalance = stakeBalances[msg.sender]; 

        //Comparing current time and staking time plus two weeks
        if(block.timestamp <= _stakingTime + uint(2 weeks)){
            //If current time smaller than staking time plus two weeks deduct 20 % from _amount
           _fee = (_amount * 20) / 100;
        }
    ...
    function claimReward() public  {
        ...
        rewards[msg.sender] += earned(msg.sender); // Updating rewards mapping
        uint _reward = rewards[msg.sender];

        require(_reward > 0, "Already Claimed");
        require (rewardToken.balanceOf(address(this))> _reward,'Not enough tokens, try later');
        
        rewards[msg.sender] = 0; // reseting rewards
        userTime[msg.sender] = block.timestamp;  //reseting user time
    ...
    /// @dev reBalance
    /// @param _balance is the number of tokens stakend
    ...
    function _reBalance(uint _balance) internal view returns (uint _points) {
        ...
        /// uint _smallest = 1000;    // tier 4 staked
        uint _smallest = tiers[tiers.length-1].stake; // use former routine if this fails

        /// take the biggest tier possible
        /// we can keep of numbers per tier in later stage like tier{god}++
        while (_balance >= _smallest) { 
            for (uint i = 0; i < tiers.length; i++) {   /// iterate over tiers, order by biggest tier
                if (_balance >= tiers[i].stake) {  /// check if we have enough stake left for this tier
    ...
    /// @dev removeUser
    /// @param _user address of user to evalute
    /// @dev check if the user still particates in this staking pool
    ...
    /// @dev addUser
    /// @param _user address of user to evalute
    /// @dev check if the user already existe for this staking pool, if not then create
...
/// @dev  subroutines
    ...
    function allocPercentageOf(address _sender) public view returns(uint) {
        ...
        /// calculated percentage of total Allowance in percPoints (100% = 1000000 points)
    ...
    /// @dev Returns no of tokens user earned based on staking balance
    /// @param _account address of user
    
    function earned(address _account) public view returns(uint){
        uint _balance = stakeBalances[_account]; // balance of user
         
        uint _tokens = _balance.mul(apy).div(100);  // how much tokens user will get in a year based on its balance
        
        uint _tokensPerSecond = _tokens.div(52 * 1 weeks); // how much tokens user will get every second
        
        uint _userTime = userTime[_account]; // time when user staked or claimed rewards
        
        uint _currentTime = block.timestamp; // current time on latest block
        
        uint _time = _currentTime.sub(_userTime); // subtracting current time with user staking time to get no of seconds user have staked
        
        uint _result = _tokensPerSecond * _time;  // multiply the tokens per second with not of seconds
    ...
    function resetApy(uint _apy) public onlyOwner {
        require(_apy > 0, "!apy");
         ...
         for(uint i = 0 ; i < userAdresses.length; i++) {
             ...
             uint _reward = earned(userAdresses[i]);
