...
/**
 * Test Pool is the irst ever token you will be rewarded in 2 decent coins $ETH and $ADA
 * 
 * 
 * 18% Tax
...
contract TestPool is ERC20, Ownable {
    ...
    constructor() ERC20("Test Pool", "TestPool") {
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "TestPool: The marketing wallet is already this address");
    ...
    function updateethDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(ethDividendTracker), "TestPool: The dividend tracker already has that address");
 
        ETHDividendTracker newethDividendTracker = ETHDividendTracker(payable(newAddress));
 
        require(newethDividendTracker.owner() == address(this), "TestPool: The new dividend tracker must be owned by the TestPool token contract");
    ...
    function updateadaDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(adaDividendTracker), "TestPool: The dividend tracker already has that address");
 
        ADADividendTracker newadaDividendTracker = ADADividendTracker(payable(newAddress));
 
        require(newadaDividendTracker.owner() == address(this), "TestPool: The new dividend tracker must be owned by the TestPool token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "TestPool: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "TestPool: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TestPool: The PanadaSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "TestPool: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "TestPool: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "TestPool: Trading has not started yet");
...
contract ETHDividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("TestPool_Eth_Dividend_Tracker", "TestPool_Eth_Dividend_Tracker", 0xBC537C876510083f7D2406C12fdd93cbe8Da4b8A) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "TestPool_Eth_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "TestPool_Eth_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TestPool contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TestPool_Eth_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TestPool_Eth_Dividend_Tracker: Cannot update claimWait to same value");
...
contract ADADividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("TestPool_Ada_Dividend_Tracker", "TestPool_Ada_Dividend_Tracker", 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "TestPool_Ada_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "TestPool_Ada_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TestPool contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TestPool_Ada_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TestPool_Ada_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
