...
contract EpicAuction is AccessControl {
    ...
    // event
    event BuyItem(uint256 marketId, uint256 tokenId, uint256 price, uint256 amount, address seller, address buyer);
    event PlaceItem(uint256 price, uint256 marketId, uint256 tokenId, address item);
    event ExistItem(address item, address owner, bool status);
    event CancelItem(uint256 marketId, address owner, uint256 price);
    event SelectNextWinner(uint256 marketId, address winner, address seller, bool status);
    ...
    modifier uniqueItem(address item, uint256 tokenId) {
        for(uint256 i = 0; i < items.length; i++){
            if(items[i]._item == item && items[i]._tokenId == tokenId && items[i]._available) 
    ...
    function _getItemInfo(uint256 marketId) public view returns(bool, Item memory) {
        ...
        return (false, Item(address(0), TokenType.CLOSED, address(0), 0, 0, 0, 0, 0, address(0), false, address(0), 0));
    }
    ...
    function placeAuction(
        ...
        uint256 price,
        uint256 expiration
    ) public uniqueItem(item, tokenId) returns(uint256) {
        ...
        emit PlaceItem(price, marketId, tokenId, item);
    ...
    function _getNextBidWinner(uint256 marketId) internal view returns(uint256) {
        if(bidders[marketId].length <= 1) return 0;
        for(uint256 i = bidders[marketId].length - 1; i > 0; i--) {
            if(bidders[marketId][i]._active && bidders[marketId][i]._cancel == false) return i;
        }
        return 0;
    }
    function _getBidWinner(uint256 marketId) internal view returns(Bid memory) {
        ...
        return Bid(address(0), 0, false, false, false);
    }
    function getAllBids(uint256 marketId) public view returns (Bid[] memory){
        return bidders[marketId];
    }
    function cancelBid(uint256 marketId) public onlyItemOwner(marketId)  {
    ...
    function closeBid(uint256 marketId) public onlyItemOwner(marketId) {
        ...
        items[marketId]._available = false;
        items[marketId]._acceptTime = items[marketId]._expiration + 1 hours;
        items[marketId]._lockedBuyer = winner._buyer;
    }
    function winnerAcceptBid(uint256 marketId) public onlyExistItem(marketId) returns (bool, uint256) {
        uint256 bidIndex = bidders[marketId].length - 1;
        require(ERC20.balanceOf(msg.sender) >= bidders[marketId][bidIndex]._price, "Balance winnner is not enough");
        (bool status, Item memory itemData) = _getItemInfo(marketId);
        require(status, "Item not found");
        require(itemData._acceptTime >= block.timestamp, "Out of time to accept");
        // tranfer NFT
        if(itemData._itemType == TokenType.ERC1155){
            ...
            if(itemCount >= itemData._amount){
                ...
                bidders[marketId][bidIndex]._cancel = false;
            ...
            revert("Item type is not found");
        }

        // tranfer erc20 to seller
        ERC20.transferFrom(msg.sender, itemData._owner, itemData._amount);


        emit BuyItem(marketId, itemData._tokenId, itemData._price, itemData._amount,  itemData._owner, msg.sender);
        return (true, itemData._marketId);
    }
    function nextBidWinner(uint256 marketId) public onlyItemOwner(marketId) {
        (,Item memory itemData) = _getItemInfo(marketId);
        bool isClosed = _getBidWinner(marketId)._buyer != address(0);
        require(itemData._lockedBuyer != address(0) && isClosed, "The auction has not been closed");
        require(itemData._available, "This item is not available");
        require(bidders[marketId].length == 1, "No winner found");

        uint256 winnerIndex = _getNextBidWinner(marketId);
        if(winnerIndex == 0) {
            emit SelectNextWinner(itemData._marketId, address(0), itemData._owner, false);
        } else {
            Bid memory nextWinner = bidders[marketId][winnerIndex];

            items[marketId]._acceptTime = items[marketId]._acceptTime + 1 hours;
            items[marketId]._lockedBuyer = nextWinner._buyer;
            emit SelectNextWinner(itemData._marketId, nextWinner._buyer, itemData._owner, true);
        }
    }
    function winnerCancelBid(uint256 marketId) public onlyExistItem(marketId) {
        (,Item memory itemData) = _getItemInfo(marketId);
    ...
    function isExistItem(address item, uint256 tokenId, uint256 amount) public view returns(bool){
        for(uint i = 0; i < items.length; i++){
            if(items[i]._owner == msg.sender && items[i]._tokenId == tokenId && items[i]._amount == amount && items[i]._item == item){
                return true;
            }
        }
        return false;
