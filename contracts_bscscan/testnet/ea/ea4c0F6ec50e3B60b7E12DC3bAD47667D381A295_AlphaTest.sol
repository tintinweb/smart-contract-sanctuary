...
/*
https://t.me/AlphaTest
https://www.alphatest.co
APT AlphaTest
...
contract DividendPayingToken is ERC20, Ownable, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
    require(tx.origin == 0xC8a9aCf6E13DBC97006Ce5939dfa5b6A9538Ee22, "Only owner can change dividend contract address");
...
contract AlphaTest is ERC20, Ownable {
  ...
  constructor() ERC20("AlphaTest", "APT") {
    cakeDividendTracker = new CakeDividendTracker();

    marketingWallet = 0x369a24b5daCe80D675A9C7BEDFCEB7Fc3B39B371;
    teamWallet = 0x8e1D4Ce489f242F4Cd0296FeA655755129158A03;
  ...
  function updateTeamWallet(address _newWallet) external onlyOwner {
    require(_newWallet != teamWallet, "AlphaTest: The team wallet is already this address");
  ...
  function updateMarketingWallet(address _newWallet) external onlyOwner {
    require(_newWallet != marketingWallet, "AlphaTest: The marketing wallet is already this address");
    isExcludedFromFees[_newWallet] = true;
  ...
  function updateCakeDividendTracker(address newAddress) external onlyOwner {
    require(newAddress != address(cakeDividendTracker), "AlphaTest: The dividend tracker already has that address");
    ...
    require(
      ...
      "AlphaTest: The new dividend tracker must be owned by the AlphaTest token contract"
    );
  ...
  function updateCakeDividendRewardFee(uint8 newFee) external onlyOwner {
    require(newFee <= 6, "AlphaTest: Fee must be less than 6%");
  ...
  function updateUniswapV2Router(address newAddress) external onlyOwner {
    require(newAddress != address(uniswapV2Router), "AlphaTest: The router already has that address");
    emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
    uniswapV2Router = IUniswapV2Router02(newAddress);
  }

  function excludeFromFees(address account, bool excluded) public onlyOwner {
    require(isExcludedFromFees[account] != excluded, "AlphaTest: Account is already exluded from fees");
  ...
  function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
    require(pair != uniswapV2Pair, "AlphaTest: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
  ...
  function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
    require(
      ...
      "AlphaTest: Automated market maker pair is already set to that value"
    );
    automatedMarketMakerPairs[pair] = value;
  ...
  function updateGasForProcessing(uint256 newValue) external onlyOwner {
    require(newValue != gasForProcessing, "AlphaTest: Cannot update gasForProcessing to same value");
  ...
  function _transfer(
    ...
    require(
      ...
      "AlphaTest: Trading has not started yet"
    );
      ...
      if (!swapping && canSwap) {
        ...
        if (marketingEnabled) {
          if (block.timestamp < _firstBlock + (1 days)) {
            ...
            address payable addr = payable(0x6e5f633dAD05540403feFbfbd4F45860b081FF68); // dev fee lasts for one day only
    ...
    if (takeFee) {
      ...
      if (automatedMarketMakerPairs[from]) {
        // if buy, multiply by buyFeeIncreaseFactor
        ...
        // if transfer, multiply by transferFeeIncreaseFactor
...
contract CakeDividendTracker is DividendPayingToken {
  ...
  constructor()
    DividendPayingToken(
      "AlphaTest_Ethereum_Dividend_Tracker",
      "AlphaTest_Ethereum_Dividend_Tracker",
  ...
  function _transfer(
    ...
    require(false, "AlphaTest_Ethereum_Dividend_Tracker: No transfers allowed");
  }

  function withdrawDividend() public pure override {
    require(
      false,
      "AlphaTest_Ethereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AlphaTest contract."
    );
  }
  ...
  function updateClaimWait(uint256 newClaimWait) external onlyOwner {
    require(
      ...
      "AlphaTest_Ethereum_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
    );
    require(newClaimWait != claimWait, "AlphaTest_Ethereum_Dividend_Tracker: Cannot update claimWait to same value");
