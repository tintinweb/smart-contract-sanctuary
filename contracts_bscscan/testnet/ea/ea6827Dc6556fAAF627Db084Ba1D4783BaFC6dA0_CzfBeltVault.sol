...
contract CzfBeltVault is ERC20, Ownable, ReentrancyGuard {
    ...
    constructor() ERC20("CzfBeltVault", "CzfBeltVault") {}
    ...
    function deposit(address _for, uint _wad) external {
        IERC20(beltBNB).approve(address(this), _wad);

        _safeTransferFrom(IERC20(beltBNB), msg.sender, address(this), _wad);
        IBeltFarm(beltFarm).deposit(BELT_POOL_ID, _wad);
    ...
    function withdraw(address _for, uint _wad) external {
        ...
        IBeltFarm(beltFarm).withdraw(BELT_POOL_ID, _wad);
        IERC20(beltBNB).transfer(_for, _wad);
    ...
    function harvest(address _pool) external onlyOwner {
        IBeltFarm(beltFarm).withdrawAll(BELT_POOL_ID);

        IERC20(beltBNB).transfer(_pool, IERC20(beltBNB).balanceOf(address(this)));
    }
