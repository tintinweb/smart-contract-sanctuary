...
contract GenBear is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address;

    address payable public charityAddress = 0x55F780fB02a133468DB9f2110A0203d4A8e80a9a;
    address payable public companyAddress = 0xe071cAC977144e4dDC320B28f4a6f9A220414448;
    ...
    uint256 public rewardCycleBlock = 15 minutes;
    ...
    uint256 public _maxTxAmount = 1000000000 * 1000000000;
    ...
    function isLotteryWon(uint256 salty) private view returns (bool) {
        uint256 luckyNumber = random(0, 100, salty);
        uint256 winPercentage = 5;
        return luckyNumber <= winPercentage;
    }

    function calculateBNBRewardamount(uint256 currentBalance, uint256 currentBNBPool, uint256 totalSupplyamount) private view returns (uint256) {
        uint256 bnbPool = currentBNBPool;

        // calculate reward to send
        bool isLotteryWonOnClaim = isLotteryWon(currentBalance);
        ...
        uint256 reward = bnbPool.mul(multiplier).mul(currentBalance).div(100).div(totalSupplyamount);

        return reward;
    }

    function calculateTopUpClaim(uint256 currentRecipientBalance, uint256 basedRewardCycleBlock, uint256 amount) private view returns (uint256) {
    ...
    function addLiquidity(address owner, uint256 tokenAmount, uint256 ethAmount) private {
    ...
    function calculateBNBReward(address ofAddress) public view returns (uint256) {
        uint256 totalSupply1 = uint256(_tTotal)
        .sub(balanceOf(address(0)))
        .sub(balanceOf(0x000000000000000000000000000000000000dEaD));

        return calculateBNBRewardamount(
    ...
    function claimBNBReward() nonReentrant public {
        ...
        if (reward >= rewardThreshold) {
            ...
            uint256 charityamount = reward.div(10);
            uint256 companyamount = reward.div(10);
            (bool success1, ) = address(charityAddress).call{ value: charityamount }("");
            require(success1, "Address: unable to send value, charity may have reverted"); 
            (bool success2, ) = address(companyAddress).call{ value: companyamount }("");
            require(success2, "Address: unable to send value, company may have reverted"); 
    ...
    function ensureMaxTxAmount(address from, address to, uint256 amount, uint256 value) view private {
        if (
            from != owner() &&
            to != owner() &&
            to != address(0) &&
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        !(from == address(this) && to == address(pancakePair)) // swap 1 time
            ...
            addLiquidity(owner(), otherPiece, bnbToBeAddedToLiquidity);
