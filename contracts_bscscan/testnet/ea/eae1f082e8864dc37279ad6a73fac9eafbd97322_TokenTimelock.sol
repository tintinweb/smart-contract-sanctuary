...
contract TokenTimelock is Ownable {
    ...
    address public lpToken = 0x2B34ded9E8D60BF1F11fEAA3a30dff79C2338FB2;

    /**
     * @notice Transfers tokens held by timelock to owner.
     */
    function releaseDpetToken() external onlyOwner {
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp >= releaseTime, "TokenTimelock: current time is before release time");

        uint256 amount = IERC20(dpetToken).balanceOf(address(this));
        require(amount > 0, "TokenTimelock: no tokens to release");

        IERC20(dpetToken).safeTransfer(owner(), amount);
    }
    ...
    function getDpetTokenBalance() external view returns(uint256) {
        return IERC20(dpetToken).balanceOf(address(this));
    }
    ...
    function setDpetToken(address _dpetToken) external onlyOwner {
        dpetToken = _dpetToken;
    }
