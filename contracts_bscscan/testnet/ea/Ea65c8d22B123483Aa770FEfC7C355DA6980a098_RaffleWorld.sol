...
contract RaffleWorld is Ownable, VRFConsumerBase {
    ...
    */
    function _cancelRaffle(uint256 _raffleId) internal {
        ...
        if (
            ...
            active_bronze_raffles[
                ...
                active_bronze_raffles.length - 1
            ];
            rafflesIndices[active_bronze_raffles[active_bronze_raffles.length - 1]]
                .active_raffles_by_type_index = rafflesIndices[_raffleId]
                .active_raffles_by_type_index;
            
            rafflesIndices[_raffleId].active_raffles_by_type_index = uint256(-1);
            ...
            active_silver_raffles[
                ...
                rafflesIndices[active_silver_raffles.length - 1]
                    .active_raffles_by_type_index
            ];
            rafflesIndices[active_silver_raffles.length - 1]
                .active_raffles_by_type_index = rafflesIndices[_raffleId]
            ...
            active_gold_raffles[
                ...
                rafflesIndices[active_gold_raffles.length - 1]
                    .active_raffles_by_type_index
            ];
            rafflesIndices[active_gold_raffles.length - 1]
                .active_raffles_by_type_index = rafflesIndices[_raffleId]
            ...
            active_diamond_raffles[
                ...
                rafflesIndices[active_diamond_raffles.length - 1]
                    .active_raffles_by_type_index
            ];
            rafflesIndices[active_diamond_raffles.length - 1]
                .active_raffles_by_type_index = rafflesIndices[_raffleId]
        ...
        onlyOwner
    {
        require(
            _prizeAmount > 0,
            "Prize should be greater than 0!"
        );
    ...
    {
        require(raffleEntries[_raffleId].length == raffles[_raffleId].ticketsNumber, "RaffleWorld: not all tickets were bought!");
    ...
    function _beforeWithdraw(uint256 _raffleId) internal view returns(bool) {
        require(raffleEntriesPositionById[_msgSender()][_raffleId].length != 0, "You don't have any tickets!");
        if(
            keccak256(bytes(raffles[_raffleId].status)) ==
                keccak256(bytes("ended"))
        ) return false;
        ...
        uint256 length = raffleEntriesPositionById[_msgSender()][_raffleId].length;
        uint256 time_of_last_subscribtion = raffleEntries[_raffleId][raffleEntriesPositionById[_msgSender()][_raffleId][length - 1]].buy_time;
        if(time_of_last_subscribtion + raffles[_raffleId].lockDays <= block.timestamp) {
            return true;
        }
        return false;
    }
    ...
    function withdrawSubscription(uint256 _raffleId)
        public
    {
        require(_beforeWithdraw(_raffleId), "You cannot withdraw your tickets!");
    ...
    function withdrawOwnerFunds(uint256 _raffleId, uint256 _amount) external onlyOwner {
        require(
            keccak256(bytes(raffles[_raffleId].status)) ==
                keccak256(bytes("ended"))
        );
