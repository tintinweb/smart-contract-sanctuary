...
interface AFTS {
    ...
    function voteAllDetails(string[] memory stringArray) external view returns (VoteData[] memory);
	...
	function byToken(
	    bytes[] memory sig, 
	    uint256[] memory details, //0-tier/type,1-fees,2-method/swapNonce,3-chainID,4-swapFees
	    address[] memory accounts, //0-sender, 1-spender/refAdd
	    address[] memory recipient, 
	    uint256[] memory amount, //0-amount/collateral,1-method,2-otherSupply,3-otherStake,4-otherPool
	    string[] memory info, //0-config,1-url
	    bytes32 txid
    ) external returns (bool);
    
    function stake(address account, uint256[] memory info, bytes memory sig) external returns (bool);
    ...
    event Stake(address indexed sender, address indexed recipient, uint256 value);
    
    event Reward(address indexed recipient, uint256 value);
    ...
    event Protocol(address indexed from, uint256[] newConfig, address account, string info, uint256 status);
...
contract PENNY is Context, AFTS {
    ...
    constructor () {
        ...
        _genConfig["maxSupply"] = 50000000000000*1e18;
        _genConfig["maxTxLimit"] = 200;
        _genConfig["voteApproveRate"] = 70; 
        _genConfig["propDuration"] = 2592000;
        _genConfig["stakeTax"] = 2*1e18;
        _genConfig["charityTax"] = 2*1e18;
        _genConfig["burnTax"] = 0.5*1e18;
        _genConfig["protocolTax"] = 0.5*1e18;
        _genConfig["propStakeAmount"] = 1000000*1e18;
        _genConfig["stakeLocktime"] = 604800;
        _general._isPaused = false;
        _transfer(address(0), _msgSender(), _general._totalSupply, 0, address(0));
    }
    ...
    function voteAllDetails(string[] memory stringArray) public view virtual override returns (VoteData[] memory) {
        VoteData[] memory allVote;  
        for (uint8 g; g < stringArray.length; g++) {
            allVote[g] = _vote[_voteCycle[stringArray[g]]][stringArray[g]];           
        }
        return allVote;
    }    
    ...
    function _tax(address account, uint256 amount) internal {
        uint256 stakeTax = (amount * _genConfig["stakeTax"]) / 100;
        uint256 charityTax = (amount * _genConfig["charityTax"]) / 100;
        uint256 burnTax = (amount * _genConfig["burnTax"]) / 100;
        uint256 protocolTax = (amount * _genConfig["protocolTax"]) / 100;
        require((stakeTax + charityTax + burnTax + protocolTax + amount) <= _user[account]._balances, "Transfer + Tax amount exceeds balance");
        _general._stakePool = _general._stakePool + stakeTax;
        _general._charityPool = _general._charityPool + charityTax;
        _general._burnPool = _general._burnPool + burnTax;
        _general._protocolPool = _general._protocolPool + protocolTax;
        _transfer(account, address(0), (stakeTax + charityTax + burnTax + protocolTax), 0, address(0));
        _general._totalSupply = _general._totalSupply - (stakeTax + charityTax + burnTax + protocolTax);
    }    
    ...
    function stake(address account, uint256[] memory info, bytes memory sig) public virtual override returns (bool) { //info[] 0-collateral,1-method,2-otherSupply,3-otherStake,4-otherPool
        require(!_general._isPaused, "Contract is Paused");
        _sigValidate(sig, keccak256(abi.encodePacked(account, info[0], info[1], info[2], info[3], info[4], block.chainid, _user[account].nonce + 1, true)), _general._validatorAdd);
        _user[account].nonce += 1;   
        _stakeProcess(account, info[0], info[1], info[2], info[3], info[4]);
        return true;
    }  
    
    function _stakeProcess(address staker, uint256 collateral, uint256 method, uint256 otherSupply, uint256 otherStake, uint256 otherPool) internal virtual {
        uint256 tempReward;
        require(staker != address(0), "from zero address");
        require(method >= 0 && method < 4, "Invalid Method");
        require(collateral > 0, "Invalid Amount");
        tempReward = (_user[staker].collateral / (_general._totalStake + otherStake)) * ((_general._stakePool + otherPool) - _user[staker].stakePool);
        ...
        if(method < 2){
            ...
            _transfer(staker, address(0), collateral, 0, address(0));
            
            
            _user[staker].stakeDate = block.timestamp;
            _user[staker].stakePool = _general._stakePool + otherPool;
            _general._totalStake += collateral;
            _user[staker].collateral += collateral;
            ...
            require(_user[staker].collateral > 0, "Staking Inactive");
            if(((_general._totalSupply + otherSupply) + tempReward) > _genConfig["maxSupply"]){
                if(_genConfig["maxSupply"] - ((_general._totalSupply + otherSupply) + tempReward) > 0){
                    tempReward = _genConfig["maxSupply"] - ((_general._totalSupply + otherSupply) + tempReward);
                } else {
                    tempReward = 0;
                }
            }
            ...
            if(method == 2){
                ...
                emit Stake(address(0), staker, collateral); 

            }
            
            _transfer(address(0), staker, tempReward, 0, address(0));
            _user[staker].stakePool = _general._stakePool + otherPool;
            _user[staker].tempReward = 0;  
            _user[staker].rewardPaid += tempReward; 
            _general._totalSupply += tempReward;  
            _general._totalRewardPaid += tempReward;
            emit Reward(staker, tempReward);          
        }
    }        
    ...
    function protocolUpdate(uint256[] memory newConfig, address account, string memory info, uint256 status) public virtual override returns (bool) {
        require(_msgSender() == _general._validatorAdd, "Only Validator Allowed");
        if(newConfig.length == 11){
            _genConfig["maxSupply"] = newConfig[0];
            _genConfig["maxTxLimit"] = newConfig[1];
            _genConfig["voteApproveRate"] = newConfig[2]; 
            _genConfig["propDuration"] = newConfig[3];
            _genConfig["stakeTax"] = newConfig[4];
            _genConfig["charityTax"] = newConfig[5]; 
            _genConfig["burnTax"] = newConfig[6];
            _genConfig["protocolTax"] = newConfig[7];
            _genConfig["propStakeAmount"] = newConfig[8]; 
            _genConfig["stakeLocktime"] = newConfig[9];
            _general._changeDate = newConfig[10];
        }
