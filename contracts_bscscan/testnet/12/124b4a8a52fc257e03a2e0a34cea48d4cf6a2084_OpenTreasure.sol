    ...
    ReentrancyGuard
{
    ...
    function openTreasureChest(uint256 _tokenId, uint256 _chestTokenId)
        public
        payable
        nonReentrant
        returns (bytes32 requestId)
    {
        ...
        if (validBundleId[_tokenId]) {
            require(
                ...
                "You Dont have a bundle set to open"
            );
    ...
    function openTreasureChestMulti(
        uint256 _tokenId,
        uint256 _chestTokenId,
        uint256 total
    ) public payable nonReentrant returns (bytes32 requestId) {
        require(
            LINK.balanceOf(address(this)) >= fee,
            "Not enough LINK - fill contract with faucet"
        );
