...
contract DragonSlayerWarrior is ERC721, ERC721Enumerable, ERC721Burnable, Ownable, Tradable {
    ...
    modifier onlyValidHash (bytes32 _hash) {
        require(manager.verifyHash(_hash), "Invalid token hash");
        _;
    }
    ...
    /** WARRIOR **/
    function createWarriors (address _receiver, Warrior[] memory _warriors, bytes32 _hash) external onlyValidHash(_hash) {
        require(_warriors.length > 0, "require: > 0");
        require(_receiver != address(0), "Not allow for address(0)");

        for (uint256 index = 0; index < _warriors.length; index++) {
            _createWarrior(_receiver, _warriors[index]);
        }
    }
    ...
    function createWarrior (
        ...
        bytes32 _hash
    ) public onlyValidHash(_hash) returns (Warrior memory) {
