...
contract LifeTimePrizePool is PrizePool {
  using SafeMathUpgradeable for uint256;
  using SafeERC20Upgradeable for IERC20Upgradeable;

  event LifeTimePrizePoolInitialized(address indexed chief);

  /// @notice Interface for the AMCStaking contract
  AMCStakingInterface public chief;

  /// @notice Address of the litetimelottery contract
  address public lottery;

  modifier onlyLottery {
    require( _msgSender() == lottery, "only-lottery");
    _;
  }

  function updateLottery (address newLottery) 
    public 
    onlyOwner
  {
    lottery = newLottery;
    ...
    initializer
  {
    ...
    emit LifeTimePrizePoolInitialized(address(chief));
  }
  ...
  /// @param amount The amount of assets to be awarded
  /// @param controlledToken The address of the asset token being awarded
  /// updated to reward AMC(underlying token) directly
  function award(
    address to,
    uint256 amount,
    address controlledToken
    ...
    onlyControlledToken(controlledToken)
  {
    ...
    // redeem directly from the staking contract
    uint256 redeemed = _redeem(amount);

    // take 10% of reward for managers
    uint256 rewardFee = redeemed.mul(10).div(100);
    uint256 reward = redeemed.sub(rewardFee);

    _token().safeTransfer(owner(), rewardFee);
    _token().safeTransfer(to, reward);
...
contract LifeTimePrizePoolProxyFactory is ProxyFactory {
  ...
  function create() external returns (LifeTimePrizePool) {
    return LifeTimePrizePool(deployMinimal(address(instance), ""));
  }
}
