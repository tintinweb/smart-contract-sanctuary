...
/**
Space Vikings
    One  of the first danish founded DeFi project. 
    A project that aims to create a renown brand in the cryptoworld.
    Our team seek to achieve our vision and create the biggest scandinavian token with multiple usage purposes in the real world.
 
    # Space Vikings Features:
 
    10% TRANSACTION FEE
    - 1% fee per transaction gets added to jackpot pool
    - 1% for the Raid wallet 
    - 2% to the marketing wallet
    - 2% fee per transaction gets added to the liquidity pool.
    - 4% fee per transaction gets distributed to holders.
*/

contract SpaceVikings is Context, IERC20, Ownable {
    ...
    string private _name = "Space Vikings";
    string private _symbol = "SVT";
    uint8 private _decimals = 9;

    uint256 public _taxFee = 400;  // 4% fee per transaction gets distributed to holders.
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _total3Fee = 500;  // 5% liquidity + raid + marketing fee
    ...
    address public _jackpotfeewallet = 0xAe29811E7cE9277bef2B930449819515C80168dC;
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tTotal3);
