...
contract PausableToken is StandardToken, Pausable {
  ...
  function decreaseApproval(address _spender, uint _subtractedValue) public override whenNotPaused returns (bool success) {
...
contract CoinToken is PausableToken {
    ...
    constructor(string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,address _FeeAddress,address tokenOwner) payable {
        name = _name;
        symbol = _symbol;
        decimals = _decimals;
	...
	function setTxTax(uint256 _txFee) onlyOwner public{
	    txFee = _txFee;
	}
	
	function setburnFee(uint256 _burnFee) onlyOwner public{
	    burnFee = _burnFee;
	}
	
	function setfeeAddress(address _FeeAddress) onlyOwner public{
	    FeeAddress = _FeeAddress;
	}

	function _burn(address _who, uint256 _value) internal {
		require(_value <= balances[_who]);
		balances[_who] = balances[_who] - (_value*10**decimals);
		totalSupply = totalSupply - (_value*10**decimals);
		emit Burn(_who, _value);
		emit Transfer(_who, address(0), _value*10**decimals);
	}

    function mint(address account, uint256 amount) onlyOwner public {

        totalSupply = totalSupply + (amount*10**decimals);
        balances[account] = balances[account] + (amount*10**decimals);
        emit Mint(address(0), account, amount*10**decimals);
...
contract Factory {
    ...
    function deployNewToken(string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,address _FeeAddress,address tokenOwner) 
    external returns (address) {
        CoinToken t = new CoinToken(_name, _symbol, _decimals, _supply, _txFee, _burnFee, _FeeAddress, tokenOwner);
