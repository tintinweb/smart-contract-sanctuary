...
contract Shibnobi is Context, IERC20, Ownable {
    ...
    uint256 public marketingFee = 38;
    ...
    function airdrop(address recipient, uint256 amount) external onlyOwner() {
        amount = amount * 10**9;
        _rOwned[msg.sender] = _rOwned[msg.sender].sub(amount , "Insufficient Balance");
        _rOwned[recipient] = _rOwned[recipient].add(amount);
        emit Transfer(msg.sender, recipient, amount);
    }
    
    function airdropInternal(address recipient, uint256 amount) internal {
        require(_rOwned[msg.sender] >= amount);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingshare = newBalance.mul(marketingFee).div(100);
