...
contract DogeDealer is ERC20, Ownable {
    ...
    mapping (address => bool) public privateSaleWallet;
    ...
    mapping(address => bool) public convertReferralRewards;
    address[] public referralAccounts;
    mapping(address => uint256) public referralEarnings;
    mapping(address => uint256) public referralEarningsDaily;
    mapping(address => uint256) public referralEarningsWeekly;
    mapping(address => uint256) public referralEarningsMonthly;
    mapping(address => uint256) public referralCount;
    mapping(address => uint256) public referralCountDaily;
    mapping(address => uint256) public referralCountWeekly;
    mapping(address => uint256) public referralCountMonthly;
    uint private dailyTimer = block.timestamp + 86400;
    uint private weeklyTimer = block.timestamp + 604800;
    uint private monthlyTimer = block.timestamp + 2629743;
    ...
    event ReferralRewards(address indexed by,address indexed to,uint256 indexed amount);
    event ReferredBy(address indexed by ,address indexed Referrer);
    
    event LeaderboardCompletion(uint8 leaderboardCase, uint256 entryCount);
    ...
    function _transfer(
        ...
        require(!privateSaleWallet[from], "Private sale wallet detected");
        ...
        if (amount == 0) {
            ...
            */
            ...
            bool referred = referrer != address(0x0000000000000000000000000000000000000000);
            
            if(referred){
                amount = amount.sub(referralFeeTxn);
                
                // DISABLED: if the referrer wants to convert referral rewards into an approved token
                // if the referrer wants to convert referral rewards into an approved token
                if(convertReferralRewards[address(referrer)]){
                    // DISABLED: super._transfer(from, address(this), referralFeeTxn);
                    // DISABLED: swapTokensForEthForReferral(referralFeeTxn, address(referrer));
                    super._transfer(from, address(referrer), referralFeeTxn);
                }else{
                    super._transfer(from, address(referrer), referralFeeTxn);
                }
                
                updateReferralEarnings(address(referrer), referralFeeTxn);
                emit ReferralRewards(from, address(referrer), referralFeeTxn);
            }else{
                amount = amount.sub(referralFeeTxn);
    ...
    function swapTokensForEthForReferral(uint256 tokenAmount, address toAccount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();

        _approve(address(this), address(msg.sender), tokenAmount);
    ...
    function setReferrer(address _referrer) public {
        ...
        require(referrers[msg.sender] == address(0),"Referrer cannot be changed!");
            
        referrers[msg.sender] = _referrer;
        referralAccounts.push(_referrer);
        emit ReferredBy(_referrer, msg.sender);
    }
    ...
    function setConvertReferralRewards(bool convert) public {
        convertReferralRewards[msg.sender] = convert;
    }
    
    function updateReferralEarnings(address account, uint256 amount) private {
        referralEarnings[account] += amount;
        referralEarningsDaily[account] += amount;
        referralEarningsWeekly[account] += amount;
        referralEarningsMonthly[account] += amount;
        referralCount[account]++;
        referralCountDaily[account]++;
        referralCountWeekly[account]++;
        referralCountMonthly[account]++;
    
        // check if the daily/weekly/monthly leaderboards should be reset
        
        if(block.timestamp >= dailyTimer){
            uint256 count = 0;
            for(uint256 i = 0; i < referralAccounts.length; i++){
                if(referralEarningsDaily[referralAccounts[i]] > 0){
                    count++;
                }
                referralEarningsDaily[referralAccounts[i]] = 0;
                referralCountDaily[referralAccounts[i]] = 0;
            }
            
            dailyTimer = block.timestamp + 8600;
            emit LeaderboardCompletion(1, count);
        }
        
        if(block.timestamp >= weeklyTimer){
            uint256 count = 0;
            for(uint256 i = 0; i < referralAccounts.length; i++){
                if(referralEarningsWeekly[referralAccounts[i]] > 0){
                    count++;
                }
                referralEarningsWeekly[referralAccounts[i]] = 0;
                referralCountWeekly[referralAccounts[i]] = 0;
            }
            
            weeklyTimer = block.timestamp + 604800;
            emit LeaderboardCompletion(2, count);
        }
        
        if(block.timestamp >= monthlyTimer){
            uint256 count = 0;
            for(uint256 i = 0; i < referralAccounts.length; i++){
                if(referralEarningsMonthly[referralAccounts[i]] > 0){
                    count++;
                }
                referralEarningsMonthly[referralAccounts[i]] = 0;
                referralCountMonthly[referralAccounts[i]] = 0;
            }
            
            monthlyTimer = block.timestamp + 2629743;
            emit LeaderboardCompletion(3, count);
        }
    }
    ...
    LEADERBOARD CASES:
        ...
        3 = Monthly
    */
    function getReferralEarnings(address account, uint8 leaderboardCase) public view returns (uint256) {
        if(leaderboardCase == 0){
            return referralEarnings[account];
        }else if(leaderboardCase == 1){
            return referralEarningsDaily[account];
        }else if(leaderboardCase == 2){
            return referralEarningsWeekly[account];
        }else if(leaderboardCase == 3){
            return referralEarningsMonthly[account];
        }
    ...
    function getReferralCounts(address account, uint8 leaderboardCase) public view returns (uint256) {
        if(leaderboardCase == 0){
            return referralCount[account];
        }else if(leaderboardCase == 1){
            return referralCountDaily[account];
        }else if(leaderboardCase == 2){
            return referralCountWeekly[account];
        }else if(leaderboardCase == 3){
            return referralCountMonthly[account];
        }
    ...
    function getReferralStatsAtUnsortedIndex(uint256 index, uint8 leaderboardCase) public view returns (address, uint256) {
        return (referralAccounts[index], getReferralEarnings(referralAccounts[index], leaderboardCase));
    }
    
    function getReferralCountsAtUnsortedIndex(uint256 index, uint8 leaderboardCase) public view returns (address, uint256) {
        return (referralAccounts[index], getReferralCounts(referralAccounts[index], leaderboardCase));
    }
    
    function getReferralStatsIndexLength() public view returns (uint256) {
        return referralAccounts.length;
    }
    ...
    function isPrivateSaleWallet(address account) public view returns (bool){
        return privateSaleWallet[account];
    }
    
    function setPrivateSaleWallet(address account, bool status) public onlyOwner {
        privateSaleWallet[account] = status;   
    }
