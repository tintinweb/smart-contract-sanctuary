...
/** 
 *  SourceUnit: d:\bscspace\TomJerry\contracts\Tom.sol
*/
...
contract TomToken is ERC20,Ownable{
    address payable public treasury;
    uint256 public iwoendtime;
    ...
    constructor(address payable _treasury) ERC20("Tom Token","TOM"){  
        treasury = _treasury;      
    ...
    function addWhiteList(address[] memory vips) external onlyOwner{
        ...
        for(uint256 i=0;i<vips.length;i++){
            ...
            if(whitelist[vips[i]] != uint8(0)) continue;
            whitelist[vips[i]] = uint8(1);
        }
    }

    function setIwoTime(uint256 timestamp) external onlyOwner {
        require(timestamp > block.timestamp, "ERROR TIME");
        require(iwoendtime == 0, "ALREADY TIME");
        iwoendtime = timestamp;

    }
    function iwo() payable external{
        require(iwoendtime>0 && block.timestamp < iwoendtime , "NOT ACTIVE");
    ...
    function setIdoTime(uint256 timestamp) external onlyOwner{
        require(timestamp > block.timestamp, "ERROR TIME");
        require(idostarttime == 0, "ALREADY TIME");
        idostarttime = timestamp;
    }

    function ido() payable external{        
        require(idostarttime > 0 && block.timestamp >= idostarttime, "WAITING...");
