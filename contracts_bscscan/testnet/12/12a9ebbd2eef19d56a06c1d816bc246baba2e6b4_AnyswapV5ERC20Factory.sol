...
contract AnyswapV5ERC20Factory {
    event TokenCreated(address indexed creator, address indexed srcToken, address indexed token, uint  index);
    ...
    function tokenFor(address creator, address srcToken) external view returns (address token) {
        token = address(uint160(uint256(keccak256(abi.encodePacked(
                hex'ff',
                address(this),
                keccak256(abi.encodePacked(creator, srcToken)),
                initCodeHash
            )))));
    }

    function createAnyswapERC20V1(
        string memory name, string memory symbol, uint8 decimals,
        address underlying, address vault, address srcToken
    ) external returns (address token) {
        token = _createAnyswapERC20(name, symbol, decimals, underlying, vault, false, srcToken);
    }

    function createAnyswapERC20(
        string memory name, string memory symbol, uint8 decimals,
        address underlying, address vault, address srcToken
    ) external returns (address token) {
        token = _createAnyswapERC20(name, symbol, decimals, underlying, vault, true, srcToken);
    }

    function _createAnyswapERC20(
        string memory name, string memory symbol, uint8 decimals,
        address underlying, address vault, bool vaultOnly, address srcToken
    ) internal returns (address token) {
        bytes memory bytecode = type(AnyswapV5ERC20).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(msg.sender, srcToken));
        assembly {
        ...
        emit TokenCreated(msg.sender, srcToken, token, allTokens.length);
    }
}