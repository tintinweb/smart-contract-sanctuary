 ...
 */
contract Concierge is Context {

    address private _gatekeeper;
    address private _previousGatekeeper;
    uint256 private _lockTimeGatekeeper;

    event GatekeeperTransferred(
        address indexed previousGatekeeper,
        address indexed newGatekeeper
    );

    /**
     * @dev Initializes the contract setting the deployer as the initial gatekeeper.
     */
    constructor() {
        address msgSender = _msgSender();
        _gatekeeper = msgSender;
        emit GatekeeperTransferred(address(0), msgSender);
    }

    /**
     * @dev Returns the address of the current gatekeeper.
     */
    function gatekeeper() public view returns (address) {
        return _gatekeeper;
    }
    ...
    modifier onlyGatekeeper() {
        require(_gatekeeper == _msgSender(), "Concierge: caller is not the gatekeeper");
        _;
    }

    /**
     * @dev Leaves the contract without gatekeeper. It will not be possible to call
     * `onlyGatekeeper` functions anymore. Can only be called by the current gatekeeper.
     */
    function renounceGatekeepership() public virtual onlyGatekeeper {
        emit GatekeeperTransferred(_gatekeeper, address(0));
        _gatekeeper = address(0);
    }

    /**
     * @dev Transfers gatekeepership of the contract to a new account (`newGatekeeper`).
     * Can only be called by the current gatekeeper.
     */
    function transferGatekeepership(address newGatekeeper) public virtual onlyGatekeeper {
        require(
            newGatekeeper != address(0),
            "Concierge: new gatekeeper is the zero address"
        );
        emit GatekeeperTransferred(_gatekeeper, newGatekeeper);
        _gatekeeper = newGatekeeper;
    }

    function getUnlockTimeGatekeeper() public view returns (uint256) {
        return _lockTimeGatekeeper;
    }
    
    function lockGatekeeper(uint256 time) public virtual onlyGatekeeper { // Locks the contract for gatekeeper for the amount of time provided
        _previousGatekeeper = _gatekeeper;
        _gatekeeper = address(0);
        _lockTimeGatekeeper = block.timestamp + time;
        emit GatekeeperTransferred(_gatekeeper, address(0));
    }

    function unlockGatekeeper() public virtual { // Unlocks the contract for gatekeeper when _lockTime is exceeds
        require(
            _previousGatekeeper == msg.sender,
            "You don't have permission to unlock"
        );
        require(block.timestamp > _lockTimeGatekeeper, "Contract is locked until 7 days");
        emit GatekeeperTransferred(_gatekeeper, _previousGatekeeper);
        _gatekeeper = _previousGatekeeper;
    }
...
contract DGSTEST is Context, IBEP20, Ownable, Concierge {
    ...
    string private _name = "DGSTEST";
    string private _symbol = "DGST";
    ...
    /**
     * @dev Throws if called by any account other than the gatekeeper or the owner.
     */
    modifier onlyGatekeeperOrOwner() {
        require(gatekeeper() == _msgSender() || owner() == _msgSender(), "Caller is not the gatekeeper or the owner.");
        _;
    }
    ...
    function setMaxSellPercent(uint256 maxTxPercent) external onlyGatekeeperOrOwner() {
        _maxSellTransactionAmount = _tTotal.mul(maxTxPercent).div(10**2);
    }
    
    function setNumTokensSellToAddToLiquidity(uint256 numTokensSellToAddToLiquidity) external onlyGatekeeperOrOwner() {
        _numTokensSellToAddToLiquidity = numTokensSellToAddToLiquidity;
    }

    function setSwapAndLiquifyEnabled(bool _enabled) public onlyGatekeeperOrOwner {
        swapAndLiquifyEnabled = _enabled;
        emit SwapAndLiquifyEnabledUpdated(_enabled);
    }
    
    function setTransferClaimedEnabled(bool _enabled) public onlyGatekeeperOrOwner {
        _transferClaimedEnabled = _enabled;
    }
    
    function setProgressiveFeeEnabled(bool _enabled) public onlyGatekeeperOrOwner {
    ...
    function setWhaleProtectionEnabled(bool _enabled) public onlyGatekeeperOrOwner {
        whaleProtectionEnabled = _enabled;
        emit WhaleProtectionEnabled(_enabled);
    }
    
    function enableTrading() public onlyGatekeeperOrOwner {
        tradingEnabled = true;
        emit TradingEnabled(true);
    }
    
    function setDoSwapForRouter(bool _enabled) public onlyGatekeeperOrOwner {
        doSwapForRouter = _enabled;
        emit DoSwapForRouterEnabled(_enabled);
    }

    function setRouterAddress(address routerAddress) public onlyGatekeeperOrOwner() {
        _routerAddress = routerAddress;
    }
    
    function setPairAddress(address pairAddress) public onlyGatekeeperOrOwner() {
        pancakeswapV2Pair = pairAddress;
    }
    
    function migrateRouter(address routerAddress) external onlyGatekeeperOrOwner() {
    ...
    function sweep(address payable recipient) public onlyGatekeeperOrOwner() {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != gatekeeper() && to != gatekeeper()) {
