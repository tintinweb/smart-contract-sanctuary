...
contract Calcifire is Context, IERC20, Ownable {
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        _reflectTotal = _reflectTotal.add(amount.mul(currentRate));

        _reflectOwned[account] = _reflectOwned[account].add(amount.mul(currentRate));
   ...
   function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
    ...
    function calcReflectSupply() public view returns (uint256) {
        return (MAX - (MAX % _tokenTotal));
    }
