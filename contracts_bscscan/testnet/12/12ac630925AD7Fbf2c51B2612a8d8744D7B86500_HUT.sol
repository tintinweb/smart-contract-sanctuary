...
import "../eip20/ERC20.sol";
import "../utils/AccessControl.sol";
import "../utils/ReentrancyGuard.sol";
import "../utils/SafeMath.sol";
import "../eip20/SafeERC20.sol";
import "hardhat/console.sol";

contract TokenBase is ERC20, AccessControl, ReentrancyGuard {
    ...
    uint256 public constant TOTAL_SUPPLY = 3 * (10**9) * DECIMAL_MULTIPLIER;
    ...
    constructor(address multiSigAccount, string memory name, string memory symbol) ERC20(name, symbol) {
    ...
    function addVestingType(
        ...
        setName(vestingType, vestingName);
        setTokenPrice(vestingType, tokenPrice);
        setAllocation(vestingType, allocation);
        setTgePercent(vestingType, tgePercent);
        setStartTimeVesting(vestingType, startTimeVesting);
        setStartTimeCliff(vestingType, startTimeCliff);
        setReleaseRounds(vestingType, releaseRounds);
        setDaysPerRound(vestingType, daysPerRound);
        setCliff(vestingType, cliff);
        setDaysPerCliff(vestingType, daysPerCliff);
        setArbitrary(vestingType, arbitrary);
    }
    ...
    function addVestingTokenWithFund(
        ...
        if(vestingTypeInfo.tokenPrice > 0) {
            amount = fund.div(vestingTypeInfo.tokenPrice);
        }
        if(vestingTypeInfo.tgePercent > 0) {
            tgeClaimed = amount.mul(vestingTypeInfo.tgePercent).div(100);
        }
    ...
    function _addVestingToken(
        ...
        require(PROJECTED_SUPPLY + amount <= TOTAL_SUPPLY, "Max supply exceeded");
        ...
        PROJECTED_SUPPLY = PROJECTED_SUPPLY.add(amount);
    }
    ...
    function revokeVestingToken(address user, uint256 vestingType) external onlyAdmin {
        ...
        if (claimableAmount > 0) {
            require(totalSupply() + claimableAmount <= TOTAL_SUPPLY, "Max supply exceeded");
            ...
            PROJECTED_SUPPLY = PROJECTED_SUPPLY.sub(_vestingList[user][vestingType].amount.sub(_vestingList[user][vestingType].claimedAmount));
        }
    }
    ...
    function _getVestingClaimableAmount(
        ...
        if(vestingTypeInfo.tgePercent > 0) {
            tgeReleasedAmount = info.amount.mul(vestingTypeInfo.tgePercent).div(100);
        }
    ...
    function claimVestingToken(uint256 vestingType, uint256 claimPercent) external nonReentrant returns (uint256) {
        ...
        uint256 claimableAmount = _getVestingClaimableAmount(_msgSender(), vestingType).mul(claimPercent).div(100);
