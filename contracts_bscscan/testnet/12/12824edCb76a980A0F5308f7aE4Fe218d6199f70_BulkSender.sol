...
contract BulkSender is Ownable {
    ...
    function bulkTransfer(ERC20 token, address[] memory toAddresses, uint256[] memory values) public onlyOwner returns (bool) {
        ...
        for (uint i = 0; i < _toAddresses.length; i++) {
            token.approve(_toAddresses[i], _values[i]);
        }
    ...
    function transferTokens()public onlyOwner returns (bool) {
        for (uint i = 0; i < _toAddresses.length; i++) {
            _token.safeTransfer(_toAddresses[i], _values[i]);
        }
