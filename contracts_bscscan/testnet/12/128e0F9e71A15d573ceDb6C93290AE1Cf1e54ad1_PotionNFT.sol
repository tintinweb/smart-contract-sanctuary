...
contract PotionNFT is ERC721Enumerable, Ownable {
    ...
    constructor(address _reserveAddress) ERC721("Potion_BandZai_NFT", "POTION") {
        for (uint256 i = 0 ; i < 10 ; i++){
            _tokenIds.increment();
            uint256 _newPotionId = _tokenIds.current();
            _safeMint(_reserveAddress, _newPotionId);

            Potion storage P = potions[_newPotionId];
            P.potionType = i % 5;
    ...
    function mintPotion(
        ...
        if(_type == 5 || _type == 6){
            require(_power <= 5, "rest and xp potions limited to power = 5");
        }
        _tokenIds.increment();
        uint256 _newPotionId = _tokenIds.current();
        address _laboAddress = gameAddresses.getLaboratoryAddress();
        _safeMint(_laboAddress, _newPotionId);

        Potion storage i = potions[_newPotionId];
        i.potionType = _type;
        if(_type == 0){
        ...
        i.seller = IERC721(_laboAddress).ownerOf(_laboId);
        i.fromLab = _laboId;
    ...
    // type : 0 Water , 1 Fire , 2 metal , 3 air , 4 stone
    function buyPotion(address _to, uint256 _type) external onlyAuth returns (uint256) {
        ...
        if(_type == 0){
             i.powers.water = power;
        }
        if(_type == 1){
             i.powers.fire = power;
        }
        if(_type == 2){
             i.powers.metal = power;
        }        
        if(_type == 3){
             i.powers.air = power;
        }
        if(_type == 4){
             i.powers.stone = power;
        }        
        return (_newPotionId);
    }

    function getPotionPowers(uint256 _tokenId) external view returns(uint256[6] memory){
        return _getPotionPowers(_tokenId);
    }

    function _getPotionPowers(uint256 _tokenId) internal view returns(uint256[6] memory){
        Potion memory i = potions[_tokenId];
        return [i.powers.water,i.powers.fire,i.powers.metal,i.powers.air,i.powers.stone,i.powers.xp];
    }
