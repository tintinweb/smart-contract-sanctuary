...
contract Order is Owner {
    ...
    function placeOrder(uint256 _tokenId, uint256 _price) external nonContract {
        ...
        require(!templar.getTokenIsStake(_tokenId), "is stake");
        ...
        templar.setTokenIsSell(_tokenId, true);
    ...
    function cancelOrder(uint256 _tokenId) public nonContract {
        ...
        templar.setTokenIsSell(_tokenId, false);
        ...
        returns (uint256[] memory available, uint256[] memory marketplace)
    {
        ...
        if (levelcode == 0) {
            ...
            for (uint256 i = 0; i < a.length; i++) {
                if (!templar.getTokenIsStake(a[i])) {
                    aa[acounter] = a[i];
                    acounter++;
                }
            }
        ...
        for (uint256 i = 0; i < a.length; i++) {
            ...
            if (levelCode == levelcode && !templar.getTokenIsStake(a[i])) {
    ...
    function allMarketplace(uint256 levelcode)
        external
        view
        returns (uint256[] memory)
    {
        ...
        uint256[] memory a = new uint256[](tokenSales.values().length);

        uint256 c = 0;
        for (uint256 i = 0; i < tokenSales.values().length; i++) {
            (, , , uint256 levelCode) = templar.getTokenInfo(
                tokenSales.values()[i]
            );
            if (levelCode == levelcode) {
                a[c] = tokenSales.values()[i];
                c++;
            }
        }
        return a;
