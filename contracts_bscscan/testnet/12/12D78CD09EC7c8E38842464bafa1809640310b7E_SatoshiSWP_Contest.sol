...
contract SatoshiSWP_Contest is ReentrancyGuard {
  ...
  constructor (address _SATOSHI, uint _finishTime) public {
  ...
  function draw (uint256 _externalRandomNumber) public onlyBy(owner) atState(ContestStates.COLLECTION_FINISHED) {
    ...
    for (i=0; i<winnerTotalNumber; i++) {
      if (i>=player_addresses.length) break;
      ...
      winner_addresses.push(cache[_randomNumber]);

      if( i<winner_numbers[0] ) {
        players[cache[_randomNumber]].winner_level = 1;
      } else if( i<winner_numbers[0] + winner_numbers[1] ) {
        players[cache[_randomNumber]].winner_level = 2;
      } else if( i<winner_numbers[0] + winner_numbers[1] + winner_numbers[2] ) {
        players[cache[_randomNumber]].winner_level = 3;
      }        

      cache[_randomNumber] = cache[cache.length-1];
      delete cache[cache.length-1];
    }

    for (i=0; i<winner_addresses.length; i++) {
      uint w_level = players[winner_addresses[i]].winner_level;
      SATOSHI.transfer(winner_addresses[i], allocation[w_level]);
    }
