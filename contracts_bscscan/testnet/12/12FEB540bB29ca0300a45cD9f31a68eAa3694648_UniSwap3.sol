...
interface EACAggregatorProxy {
    // this function gets the latest price for ETH
  function latestAnswer() external view returns (int256);
}

interface IUniswapRouter is ISwapRouter {
    // returns leftover eth from transactions
  function refundETH() external payable;
}

interface DepositableERC20 is IERC20 {
    // to work with the WETH version of ETH
...
contract UniSwap3 {
    ...
    // for getting price data from Uniswap
    IQuoter quoter = IQuoter(uinswapV3QuoterAddress);
    // to do swaps
    IUniswapRouter uniswapRouter = IUniswapRouter(uinswapV3RouterAddress);
    IUniswapV3Factory uniswapFactory;

    event myLog(string msg, uint ref);

    constructor() {
        console.log('Deploying UniSwap3 contract');
        // set the owner address to the address of the contract
        owner = msg.sender;
    }
    ...
    // will get total value in USD of the account which will require an ETH price oracle
    ...
    function updateEthPriceChainlink() public returns(uint) {
        ...
        ethPrice = uint(chainLinkEthPrice / 1000000000);
        return ethPrice;
    }
    ...
    function rebalance() public {
        ...
        if (usdBalancePercentage < usdTargetPercentage) {
            ...
            emit myLog('amountToBuy', amountToBuy);
            require (amountToBuy > 0, "Nothing to buy");
            buyWeth(amountToBuy);
        }
    }

    function swapExactInputSingle(uint256 amountIn, address tokenInput, address tokenOutput) public {
        TransferHelper.safeTransferFrom(tokenInput, msg.sender, address(this), amountIn);
        TransferHelper.safeApprove(tokenInput, address(uinswapV3RouterAddress), amountIn);
        uint amountOut;
    ...
    function swapExactOutputSingle(uint256 amountOut, uint256 amountInMaximum, address tokenInput, address tokenOutput) external returns (uint256 amountIn) {
        // Transfer the specified amount of DAI to this contract.
        TransferHelper.safeTransferFrom(tokenInput, msg.sender, address(this), amountInMaximum);

        // Approve the router to spend the specifed `amountInMaximum` of DAI.
        // In production, you should choose the maximum amount to spend based on oracles or other data sources to acheive a better swap.
        TransferHelper.safeApprove(tokenInput, address(uinswapV3RouterAddress), amountInMaximum);
        ...
        if (amountIn < amountInMaximum) {
            TransferHelper.safeApprove(tokenInput, address(uinswapV3RouterAddress), 0);
            TransferHelper.safeTransfer(tokenInput, msg.sender, amountInMaximum - amountIn);
        }
    }

    function createNewPool(address tokenA, address tokenB, uint24 fee) external payable {
        uniswapFactory.createPool(tokenA, tokenB, fee);
    }
