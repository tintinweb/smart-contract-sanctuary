...
contract DFLICO  {
    ...
    function claimAirdrop(address referrerAddress) public 
    {
	    ...
	    uint256 refIncome=(airDropToken.mul(3)).div(100).mul(tokenPrice);
    ...
    function _invest(uint256 walletUsedBusd,uint256 rewardUsedBusd,uint256 compoundUsedBusd,uint256 airdropToken,uint256 _planId,address referrer) public 
    {
        ...
        // airdrop balance
        require(users[msg.sender].airdropBonus>=airdropToken,"Low Airdrop balance");
        ...
        uint256 _amount=walletUsed+rewardUsed+compoundUsed+airdropToken;
        dflToken.transferFrom(msg.sender,address(this),walletUsed);
       
        uint256 _busdAmount=walletUsedBusd+rewardUsedBusd+compoundUsedBusd+((airdropToken/1e18)*tokenPrice);
    ...
    {
        require(msg.sender==owner,"Only Owner");
        require(airdropClaimed[_user],"Already Sended");
        airdropClaimed[_user]=true;
	    users[_user].airdropBonus=token;
    ...
    function sale_setting(uint8 _type) public payable
    {
     ...
     function swichStaking(uint8 _type) public payable
    {
        require(msg.sender==owner,"Only Owner");
            if(_type==1)
            stakingOn=true;
    ...
    function swichAirdrop(uint8 _type) public payable
    {
        require(msg.sender==owner,"Only Owner");
            if(_type==1)
            airdropOn=true;
