...
contract KawaiiGachaOffchain is Signed, Pausable, SignData {
    using SafeMath for uint256;
    ...
    event Gacha(address indexed sender);

    constructor(address _kawaiiToken) public {
    ...
    function setTokenKawaii(address _kawaiiToken) external onlyOwner {
        kawaiiToken = _kawaiiToken;
    }

    function gacha(bytes memory data) external whenNotPaused {
        (IERC1155 _nft1155Address,
        uint256[] memory _inputBurnNFTIds,
        uint256[] memory _inputBurnAmounts,
        ...
        );
        {
            ...
            address signer = getSigner(
                keccak256(
                    abi.encode(address(this), this.gacha.selector, sender, _inputBurnNFTIds, _inputBurnAmounts, _inputNFTIds, _inputAmounts, nonce)
                ), v, r, s);
            require(isSigner[signer], "Forbidden");
            require(_inputBurnNFTIds.length == _inputBurnAmounts.length, "token id burn must same length with amount");
            require(_inputNFTIds.length == _inputAmounts.length, "token id must same length with amount");

            IBEP20(kawaiiToken).safeTransferFrom(sender, address(this), _amountFee);

            for (uint256 i = 0; i < _inputBurnNFTIds.length; i++) {
                IERC1155(_nft1155Address).burn(sender, _inputBurnNFTIds[i], _inputBurnAmounts[i]);
            }

            for (uint256 i = 0; i < _inputNFTIds.length; i++) {
                IERC1155(_nft1155Address).mint(sender, _inputNFTIds[i], _inputAmounts[i]);
            }
            emit Gacha(sender);
        }
    }
