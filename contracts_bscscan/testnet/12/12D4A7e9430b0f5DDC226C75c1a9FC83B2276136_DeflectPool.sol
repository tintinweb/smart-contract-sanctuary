...
// DEV NOTE: Fix 'purchase' function

// boostedbalanceOf returning 0 (staking balance of the account)
// when looping functions called empty array pids are called breaking tx Fail with error 'Address: call to non-contract'
//      getReward, ejectAll, updateReward

// implement logic for can only remove rewardPool after duration is finished + emergencyRemove that drains reward tokens
...
contract DeflectPool is LPTokenWrapper {
    ...
    constructor(
        ...
        devFund = _devFund;
        super.setDefaultFeeReduceVariables;
    }
...
//     Testing notes 
//      addRewardPool prior
//      addRewardPool2 testing
//      addRewardPool1 testing

    
//     +addRewardPool2
// 	        Works, pool added with id "0" according to event log but no pool can be looked up
	
	
//     +addRewardPool1
//     +addRewardPool1
//     	    Works, pools added at id's 0 and 1 
//     +addRewardPool2
//     	    Error, { "code": -32000, "message": "gas required exceeds allowance (25000000)" }
//     +removepool (pid: 1)
//      	Works, pool id address set to 0x0
//     +addRewardPool2
//      	Works, new pool added in pool id 1

//      +addRewardPool1
//      +addRewardPool1
//      +removepool (pid: 1)
//      +addRewardPool1
//            Works, but newly added pool is assigned pid 2 rather than taking the empty slot at pid 1
    ...
    function addRewardPool(IERC20Metadata _rewardToken, uint256 _duration) public onlyOwner {
        require(!addedRewardTokens[address(_rewardToken)], "Reward token is already added");
    ...
    // // Adds a new reward pool with specified duration
    // function addRewardPool1(IERC20Metadata _rewardToken, uint256 _duration) public onlyOwner {
    //     require(!addedRewardTokens[address(_rewardToken)], "Reward token is already added");
    //     require(address(_rewardToken) != address(0), "Cannot add burn address");
    //     require(_duration != 0, "Must define valid duration length");
    ...
    //     // find empty reward pool pid slots if available
    //     for (uint i = 0; i < poolInfo.length; i++) {
    //             // if pool id is 0x address, then it is empty and can be used
    //            if (getRewardPoolAddress(i) == address(0)) {
    //             _rewardTokenID = i;
    //             i = poolInfo.length;
    //         } else {
    //             _rewardTokenID = totalPools++;
    //         }
    //     }
    ...
    // // Adds a new reward pool with specified duration
    // function addRewardPool2(IERC20Metadata _rewardToken, uint256 _duration) public onlyOwner {
    //     require(!addedRewardTokens[address(_rewardToken)], "Reward token is already added");
    //     require(address(_rewardToken) != address(0), "Cannot add burn address");
    //     require(_duration != 0, "Must define valid duration length");
    ...
    //             pool.rewardTokenAddress = _rewardToken;
    //             pool.rewardPoolID = _rewardTokenID;
    ...
    //             break; // i = poolInfo.length; 
            
    //             // if there are no empty pool ids create a new one
    //         } else {
    //             _rewardTokenID = totalPools++;
    ...
    // // Adds a new reward pool with specified duration
    // function addRewardPool3(IERC20Metadata _rewardToken, uint256 _duration) public onlyOwner {
    //     require(!addedRewardTokens[address(_rewardToken)], "Reward token is already added");
    //     require(address(_rewardToken) != address(0), "Cannot add burn address");
    //     require(_duration != 0, "Must define valid duration length");
...
abstract contract LPTokenWrapper is Ownable {
    ...
    function withdraw(uint256 amount) public virtual {
        ...
        uint256 prismBalance = IERC20(prism).balanceOf(msg.sender);
    ...
    // Sets the default values for the Fee Reduction on unstaking for holding PRISM
    function setDefaultFeeReduceVariables() public onlyOwner virtual {
        unstakeFeeReduceLvl1Amount = 25;
        unstakeFeeReduceLvl2Amount = 50;
        unstakeFeeReduceLvl3Amount = 100;
