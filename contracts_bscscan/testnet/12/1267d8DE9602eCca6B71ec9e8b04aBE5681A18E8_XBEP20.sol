...
contract XBEP20 is BEP20Mintable, BEP20Burnable, BEP20Fee {
    ...
    function set_active_status(uint flag) public onlyOwner { 
        _active_status = flag;
    }
...
abstract contract BEP20Fee is BEP20 {
    ...
    // transaction tax
    uint public _transaction_tax = 0;
    ...
    uint public _fee_decimal = 8;
    mapping (address => address) public _referrers;

    struct Colony {
         uint _transaction_tax;
         string _policy_link;
         string _policy_hash;
         uint _level;
    }

    mapping (address => address) public _colony_addresses;
     ...
     */
    function registerColony(address colony_public_key, uint tx_tax, string memory colony_policy_link, string memory colony_policy_hash, address mother_colony_public_key) public onlyOwner {
        ...
        if( bytes(_colonies[mother_colony_public_key]._policy_hash).length != 0 && _colonies[mother_colony_public_key]._level < _colonies[colony_public_key]._level) {
            _colony_addresses[colony_public_key] = mother_colony_public_key;
            _colonies[colony_public_key]._level = _colonies[mother_colony_public_key]._level + 1;
        } else if (bytes(_colonies[mother_colony_public_key]._policy_hash).length == 0) {
            _colony_addresses[colony_public_key] = address(0);
            _colonies[colony_public_key]._level = 1;
        }
    }

    function getColonyInfo(address addr) public view returns(Colony memory, address){
        return (_colonies[addr], _colony_addresses[addr]);
    }
    ...
    function setColonyAddress(address addr,address colony) public onlyOwner {
        // require(_colony_addresses[colony] != address(0), "Please set valid colony address");
        _colony_addresses[addr] = colony;
    }

    function setJaxCorpDAO(address jaxCorpDao_wallet, uint tx_tax, string memory policy_link, string memory policy_hash) public onlyOwner {
     ...
     * @dev Set cashback
     */
    function setCashback(uint _cashback_percent) public onlyOwner {
        _cashback = _cashback_percent;
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        uint tx_fee_amount = amount * _transaction_fee / (10 ** _fee_decimal);
        ...
        if( referrer != address(0xdEaD) || referrer != address(0)){
            super.transfer(referrer, 70 * tx_fee_amount * _referral_fee / (10 ** _fee_decimal) / 100);
            referrer = _referrers[referrer];
            total_referral_fees += 70 * tx_fee_amount * _referral_fee / (10 ** _fee_decimal) / 100;
            if( referrer != address(0xdEaD) || referrer != address(0)){
                super.transfer(referrer, 10 * tx_fee_amount * _referral_fee / (10 ** _fee_decimal) / 100);
                referrer = _referrers[referrer];
                total_referral_fees += 10 * tx_fee_amount * _referral_fee / (10 ** _fee_decimal) / 100;
                if( referrer != address(0xdEaD) || referrer != address(0)){
                    super.transfer(referrer, 10 * tx_fee_amount * _referral_fee / (10 ** _fee_decimal) / 100);
                    referrer = _referrers[referrer];
                    total_referral_fees += 10 * tx_fee_amount * _referral_fee / (10 ** _fee_decimal) / 100;
        ...
        super.transfer(_tx_fee_wallet, tx_fee_amount - total_referral_fees - (tx_fee_amount * _cashback / (10 ** _fee_decimal)));
        
        //Transfer of UBI Tax
        
        if( amount * _ubi_tax / (10 ** _fee_decimal) > 0 ){
            super.transfer(_ubi_tax_wallet, amount * _ubi_tax / (10 ** _fee_decimal));  // ubi tax
        }
        ...
        uint tx_tax_amount = amount * _colonies[_colony_addresses[recipient]]._transaction_tax / (10 ** _fee_decimal);     // Calculate transaction tax amount
        
        transferTransactionTax(_colony_addresses[recipient], tx_tax_amount, 1);          // Transfer tax to colonies and jaxCorp Dao
        
        // Transfer tokens to recipient. recipient will pay the fees.
        super.transfer(recipient, amount - tx_fee_amount - amount * _ubi_tax / (10 ** _fee_decimal) - amount * _transaction_tax / (10 ** _fee_decimal));
    ...
    /**
     * @dev Transfer transaction tax to colony wallet address
     *      50% of transaction tax will go to mothers of the colony.
     * @param colony_address colony address
     * @param amount transaction tax amount
     * @param level level will be limited to 5
     */
    function transferTransactionTax(address colony_address, uint amount, uint level) internal {

        if(colony_address == address(0) || level > 5  ) {
            super.transfer(_jaxcorp_dao_wallet, amount);
            return;
        }
        super.transfer(colony_address, amount / 2);
        transferTransactionTax(_colony_addresses[colony_address], amount / 2, level + 1);
    }
}
