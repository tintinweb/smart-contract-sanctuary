...
contract Exchange is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function calcLoanAmountAndExchangeRate(
        ...
        if (collateralAddress == address(0)) {
            ...
            (, uint256 coll) = SafeMathUpgradeable.tryMul(uint256(RateBNBwithUSD()), ltvAmount);
            ...
            (, uint256 ltvAmount) = SafeMathUpgradeable.tryMul(loanToValue, amount);
            (, uint256 coll) = SafeMathUpgradeable.tryMul(uint256(getLatesPriceToUSD(collateralAddress)), ltvAmount);
            (, uint256 collToUSD) = SafeMathUpgradeable.tryDiv(coll, 100);
        ...
        if (loanAsset == address(0)) {
            // (, uint256 rateLA) = SafeMathUpgradeable.tryMul(uint256(RateBNBwithUSD()), 10**5);
            // rateLoanAsset = uint256(RateBNBwithUSD()) * 10**10;
            rateLoanAsset = uint256(RateBNBwithUSD());
        } else {
            // (, uint256 rateLA) = SafeMathUpgradeable.tryMul(uint256(getLatesPriceToUSD(loanAsset)), 10**5);
