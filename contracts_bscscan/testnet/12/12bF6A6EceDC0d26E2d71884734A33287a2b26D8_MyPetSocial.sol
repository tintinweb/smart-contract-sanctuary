...
contract MyPetSocial  is ERC20Burnable, Ownable {
    ...
    constructor (address _factoryAddress) ERC20("My Pet Social", "MPS") {
        _mint(_msgSender(), 0x204FCE5E3E25026110000000); // 10.000.000.000 MPS
        blacklistTime = block.timestamp + 3 days;
        factoryAddress = _factoryAddress;
        pairAddress = IUniswapV2Factory(factoryAddress).createPair(address(this), factoryAddress);
    ...
    function multiBlacklist(address[] memory addresses) external onlyOwner {
        if(block.timestamp <= blacklistTime) {
