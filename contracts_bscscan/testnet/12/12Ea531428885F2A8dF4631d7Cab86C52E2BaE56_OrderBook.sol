...
contract OrderBook is Initializable, Ownable, ReentrancyGuard, IOrderBook {
  ...
  function viewOrderBalance(address orderNodeAddr) external view override returns (uint256 balance, uint256 capacity) {
    ...
    for (uint16 i = 0; i < orderNode.itemCount; i++) {
      ...
      (uint256 tmpB, uint256 tmpC) = __getOrderItemBalance(currAddr);
  ...
  function viewOrderItem(address orderNodeAddr) external view override returns (OrderItem[50] memory) {
    require(orderNodeAddr != address(0), "view address invalid");
  ...
  function __bookOrder(uint8 position, uint256 price, uint256 securityAmt, uint256 currencyAmt) private {
    ...
    if (inNode) {
      ...
      // (uint256 balance, uint256 capacity) = __getOrderItemBalance(currItemAddr);
      // currNode.balance = currNode.balance.add(balance);
      // currNode.capacity = currNode.capacity.add(capacity);
  ...
  function __fillMatchedOrder(uint8 position, uint256 securityAmt, uint256 currencyAmt, address orderNodeOpAddr) private returns (uint256 securityLeft, uint256 currencyLeft) {
    ...
    uint16 loopCount = currNodeOp.itemCount;
    ...
    for (uint16 i = 0; i < loopCount; i++) {
      ...
      if (securityLeft <= securityInItem && currencyLeft <= currencyInItem) {
        // (uint256 b1, uint256 c1) = __getOrderItemBalance(currItemAddr);
        __makeTradeOrder(tradeItem, position, securityLeft, currencyLeft, 2);
        // (uint256 b2, uint256 c2) = __getOrderItemBalance(currItemAddr);

        // currItem.balance = tradeItem.balance();
        // currItem.capacity = tradeItem.capacity();
        currItem.status = 2;

        // currNodeOp.balance = currNodeOp.balance.sub(b1 - b2);
        // currNodeOp.capacity = currNodeOp.capacity.sub(b1 - b2);
  ...
  function __createOrderNode(uint8 position, uint256 price, uint256 securityAmt, uint256 currencyAmt) private returns (address) {
    ...
    // (uint256 balance, uint256 capacity) = __getOrderItemBalance(orderItemAddr);
  ...
  function __cleanAndMoveOrderItem(address orderItemAddr) private returns (address) {
    ...
    // (uint256 balance, uint256 capacity) = __getOrderItemBalance(orderItemAddr);
    // currNode.balance = currNode.balance.sub(balance);
    // currNode.capacity = currNode.capacity.sub(capacity);
    currNode.itemCount -= 1;
  ...
  function __flushOrderItem(address orderItemAddr) private {
    ...
    // orderNode.balance = orderNode.balance.sub(balance);
    // orderNode.capacity = orderNode.capacity.sub(capacity);
  }
  ...
  function __getCurrencyBySecurity(uint256 price, uint256 amount) private pure returns (uint256) {
    require(amount > 0 && price > 0, "trade price and amount invalid");
    return amount.mul(price).div(1e18);
  }

  function __getSecurityByCurrency(uint256 price, uint256 amount) private pure returns (uint256) {
    require(amount > 0 && price > 0, "trade price and amount invalid");
  ...
  function __getOrderItemBalance(address orderItemAddr) internal view returns (uint256 balance, uint256 capacity) {
...
interface IOrderBook {
  ...
  function viewOrderItem(address orderNodeAddr) external view returns (OrderItem[50] memory);
