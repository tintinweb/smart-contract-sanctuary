...
contract asd {
    ...
    uint256 private _ideasAmount = 0;
    struct Idea {
        address creator;
        string title;
        string description;
    ...
    function createIdea(string memory title, string memory description) external {
        _ideas[_ideasAmount] = Idea(msg.sender, title, description, 0);
        _ideasAmount += _ideasAmount;
    }
    ...
    function pickAirdrop() external {
        ...
        uint256 amount = _airdropAmount * (10**_decimals);
        require(_balances[address(this)] > amount, "Address don't have balance");
