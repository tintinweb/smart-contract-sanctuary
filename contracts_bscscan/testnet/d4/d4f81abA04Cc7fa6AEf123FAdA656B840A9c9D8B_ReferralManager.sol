    ...
    UUPSUpgradeable
{
    ...
    // register using custom userID (MODERATOR_ROLE)
    function registerID(string memory createUserId, address newUserAcc)
        public
        override
    ...
    // Generate UserID
    function getUserId() internal view returns (string memory autoUserID) {
        bool userExist = true;
        uint8 num = 1;
        while (userExist) {
    ...
    function getReferrer(address account)
        public
        view
        override
        returns (address ReferBy)
    {
        UserInfo memory user = userInfo[userId[account]];
    ...
    {
        refersID = StringsUpgradeable.upperCase(refersID);
    ...
    // update referral reward status by operator (OPERATOR_ROLE)
    function setRewardActive(address account, bool activate)
        public
        override
        onlyRole(OPERATOR_ROLE)
        returns (bool activated)
    {
        if (!hasRole(MODERATOR_ROLE, account)) {
            UserInfo storage user = userInfo[userId[account]];
            user.rewardActive = activate;
            emit RewardActiveUpdated(account, activate);
            return true;
        }
    }

    // set revoke period for revoke change address (HANDLER_ROLE)
    function setRevokeTime(uint256 RevokeTime) public onlyRole(HANDLER_ROLE) {
        revokeTime = RevokeTime;
    }

    // set address fee for change address (HANDLER_ROLE)
    function setChangeAddrFee(uint256 newFee) public onlyRole(HANDLER_ROLE) {
        changeAddrFee = newFee;
    }

    // set address fee ability for change address (HANDLER_ROLE)
    function setChangeAddrPaused(bool status) public onlyRole(HANDLER_ROLE) {
        changeAddrPaused = status;
    }

    // restrict address account for refer new user (MODERATOR_ROLE)
...
interface IReferralManager {
    ...
    event RewardActiveUpdated(address userAddress, bool active);
