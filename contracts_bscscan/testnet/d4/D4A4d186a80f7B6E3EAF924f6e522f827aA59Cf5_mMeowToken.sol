...
contract mMeowToken is ERC20("mMeow", "mMEOW"), Ownable {
    /// @notice Events
    event SetOpen(address indexed user, bool open);
    ...
    IERC20 public meow;
    bool public open;
    uint256 public fourMonths = 5 minutes; // 4 months
    uint256 public endTime;

    constructor(IERC20 _meow) public {
        meow = _meow;
        endTime = block.timestamp.add(fourMonths);
    }
    ...
    function withdraw(uint256 _share) public {
        require(block.timestamp > endTime, "not within the specified period");
        uint256 totalShares = totalSupply();
