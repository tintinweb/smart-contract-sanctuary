...
contract PrivateSale {
  ...
  function buyAllocation(uint256 _busdAmount) public rightAmount(_busdAmount) {
    ...
    require(busdToken.allowance(msg.sender, address(this)) >= _busdAmount, "Insufficient funds");
    require(busdToken.balanceOf(msg.sender) >=_busdAmount);

    busdToken.transferFrom(msg.sender, saleAddress, _busdAmount);
    _allocatedTokens[msg.sender] = _allocatedTokens[msg.sender].add(tokenAmount);
    raised = raised.add(_busdAmount);

    emit TokensAllocated(msg.sender, tokenAmount);
  }

  function getAllowance(address _owner) public view returns (uint256) {
    return busdToken.allowance(_owner, address(this));
  }
