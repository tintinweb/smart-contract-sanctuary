...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWRD = IBEP20(0x8BaBbB98678facC7342735486C851ABD7A0d17Ca);
...
contract LTestToken is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "LTestToken";
    string constant _symbol = "LTT";
    uint8 constant _decimals = 8;

    uint256 _totalSupply = 2 * 10**13 * 10**_decimals;
    ...
    struct CustomFees {
        uint256 buyTaxMultiplier;
        uint256 sellTaxMultiplier;
        uint256 transferTaxMultiplier;
    }
    ...
    //Referral System
    uint256 public referrerReward  = 25; 
    uint256 public referrentReward = 25;
    ...
    function setMaxBuyTxPercent_base1000(uint256 maxBuyTXPercentage_base1000) external onlyOwner() {
        _maxBuyAmount = (_totalSupply * maxBuyTXPercentage_base1000 ) / 1000;
    }
    ...
    function setMaxSellTxPercent_base1000(uint256 maxSellTXPercentage_base1000) external onlyOwner() {
        _maxSellAmount = (_totalSupply * maxSellTXPercentage_base1000 ) / 1000;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // Blacklist
        if(LDXLOCKED){
            require(!isLDXLOCKED[sender] && !isLDXLOCKED[recipient],"Blacklisted");    
        }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isBuy) {
            if(userFees[recipient].buyTaxMultiplier !=0 ) {
                feeAmount = amount.mul(totalFee).mul(userFees[recipient].buyTaxMultiplier).div(feeDenominator * 100);
            }
            else {
                feeAmount = amount.mul(totalFee).div(feeDenominator);
            }
            if(isWhitelisted[recipient] && isFirstBuy[recipient]) {//referredbuy
            ...
            if(userFees[sender].sellTaxMultiplier != 0) {
                feeAmount = amount.mul(totalFee).mul(sellMultiplier).mul(userFees[sender].sellTaxMultiplier).div(feeDenominator * 100 * 100);
            } else {
            ...
            if(userFees[sender].transferTaxMultiplier != 0) {
                feeAmount = amount.mul(totalFee).mul(sellMultiplier).mul(userFees[sender].transferTaxMultiplier).div(feeDenominator * 100 * 100);
            } else {
                feeAmount = amount.mul(totalFee).mul(sellMultiplier).div(feeDenominator * 100);
            }
            _balances[address(this)] = _balances[address(this)].add(feeAmount);
    ...
    function cooldownEnabled(bool _buyStatus, uint8 _buyInterval, bool _sellStatus, uint8 _sellInterval) public onlyOwner {
    ...
    function LDXSRR(uint256 LDXRRP_1, uint256 LDXRRP_2) public onlyOwner {
    ...
    function LDXSU(address LDXSU_1, uint256 LDXSU_2, uint256 LDXSU_3, uint256 LDXSU_4) external authorized {
        require(LDXSU_2 != 0 , "Cant be set to 0. Use isFeeExempt instead");
        require(LDXSU_3 != 0 , "Cant be set to 0, Use isFeeExempt instead");
        require(LDXSU_4 != 0 , "Cant be set to 0, Use isFeeExempt instead");
        userFees[LDXSU_1].buyTaxMultiplier = LDXSU_2;
        userFees[LDXSU_1].sellTaxMultiplier = LDXSU_3;
        userFees[LDXSU_1].transferTaxMultiplier = LDXSU_4;
    }
