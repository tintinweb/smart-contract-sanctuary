...
contract Merchant is Pausable {
        ...
        require(
            _method != FeeMethod.AFLIQU ||
                (address(WEB3_TOKEN) != address(0) &&
                    ...
                    affiliatorWallet != address(0)), // address(affiliatorWallet)
    ...
    function getFeeAmount(
        ...
        if (address(WEB3_TOKEN) != address(0)) {
            uint256 web3BalanceOfCaller = WEB3_TOKEN.balanceOf(_msgSender());
            if (web3BalanceOfCaller >= web3BalanceForFreeTransaction) {
                return 0;
            }
        }
        ...
        uint256 feeAmount = amountIn.mul(uint256(transactionFee)).div(10000);
        if (address(stakingContract) != address(0)) {
            feeAmount = feeMaxPercent.sub(
