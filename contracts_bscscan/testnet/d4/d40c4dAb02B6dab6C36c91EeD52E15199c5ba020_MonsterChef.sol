...
contract MonsterChef is Ownable, ReentrancyGuard {
  ...
  function updateMultiplier(uint256 multiplierNumber) public onlyOwner {
    require(
      multiplierNumber <= MAXIMUM_BONUS_MULTIPLIER,
      "setMultiplier: invalid multiplier number"
    );
  ...
  function add(
    ...
    require(
      _harvestFeeBP <= MAXIMUM_HARVEST_FEE,
  ...
  function pendingMonster(uint256 _pid, address _user)
    external
    view
    returns (uint256)
  {
    ...
    if (block.number > pool.lastRewardBlock && lpSupply != 0) {
      ...
      uint256 MonsterReward = multiplier
        .mul(MonsterPerBlock)
        .mul(pool.allocPoint)
        .div(totalAllocPoint);
      accMonsterPerShare = accMonsterPerShare.add(
        MonsterReward.mul(1e12).div(lpSupply)
      );
    }
  ...
  function updatePool(uint256 _pid) public validatePool(_pid) {
    ...
    pool.accMonsterPerShare = pool.accMonsterPerShare.add(
      MonsterReward.mul(1e12).div(lpSupply)
    );
    pool.lastRewardBlock = block.number;
  }
  ...
  function deposit(
    ...
    if (user.amount > 0) {
      uint256 pending = user.amount.mul(pool.accMonsterPerShare).div(1e12).sub(
        user.rewardDebt
      );
      if (pending > 0) {
  ...
  function payReferralCommission(address _user, uint256 commissionAmount)
    internal
  {
    ...
    )
  {
    return (
      ...
      poolInfo[_pid].accMonsterPerShare
    );
  }
  ...
  function setMonsterReferral(IMonsterReferral _monsterReferral)
    public
    onlyOwner
  {
