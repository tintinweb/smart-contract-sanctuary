...
contract Swap {
    ...
    mapping (address => Whitelist) whitelistToBalance;
    ...
    function swap(uint256 quillAmount) external whenActive {
        // only if active
        require(whitelistToBalance[msg.sender].wallet  != address(0), "Caller is not whitelisted");
        require(quillAmount <= whitelistToBalance[msg.sender].qutBalance / 10, "Exceeds 10% QUT balance");
    ...
    function addToWhitelist(address wallet) external onlyOwner {
        Whitelist memory newWhitelist = whitelistToBalance[wallet];
        newWhitelist.wallet = wallet;
        newWhitelist.qutBalance = _quill.balanceOf(wallet);
