...
contract GET_SCHIFFY_GOLD is Context, IBEP20, Ownable {	
    ...
    uint256 private _tTotal = 100 * 10**10 * 10**9;  	   
    uint256 private _rTotal = (MAX - (MAX % _tTotal));	
    
    uint256 private _tFeeTotal;
    string private _name = "GET SCHIFFY GOLD";	
	...
	uint256 public _txgoldFee = 15;
    uint256 private _previousTxgoldFee = _txgoldFee;
    
	uint256 public _stakingFee = 10;
    ...
    uint256 public _antiWhaleThreshold = 1 * 10**8 * 10**9;
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {	
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tTxgold, uint256 tStaking) = _getValues(tAmount);	
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {	
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tTxgold, data.tStaking);	
    }	
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {	
        ...
        return (tTransferAmount, TData(0, tFee, tLiquidity, tTxgold, tStaking, 0));	
    }	
    function _getRValues(TData memory _data) private pure returns (uint256, uint256, uint256) {	
		...
		uint256 rStaking = _data.tStaking.mul(_data.currentRate);
    ...
    function _takeTxgold(uint256 tTxgold) private {
        uint256 currentRate =  _getRate();
    ...
    function _takeStaking(uint256 tStaking) private {
        ...
        if (_shouldSwapToBNB) {
            swapTokensForEth(rStaking);
            _stakingWallet.transfer(address(this).balance);
        } else {
            _rOwned[_stakingWallet] = _rOwned[_stakingWallet].add(rStaking);
            if(_isExcluded[_stakingWallet])
                _tOwned[_stakingWallet] = _tOwned[_stakingWallet].add(tStaking);
        }
    }
    ...
    function removeAllFee() private {	
        if(_rewardFee == 0 && _liquidityFee == 0 && _txgoldFee == 0 && _stakingFee == 0) return;	
