...
contract GAMCasino {
  ...
  uint256 public maximumBetsNr = 1;
  ...
  event Bets(bool _status, address _address, uint _amount);
  ...
  function bet(uint256 numberSelected) public payable {
    ...
    require(numberSelected <= 5 && numberSelected >= 1);
    ...
    totalBet += msg.value;
    emit Bets(true, msg.sender, msg.value);
  ...
  function generateNumberWinner() public {
    uint256 numberGenerated = block.number % 5 + 1;
  ...
  function distributePrizes(uint256 numberWin) public {
    ...
    if (countWin != 0) {
      uint256 winnerEtherAmount = totalBet*2;
