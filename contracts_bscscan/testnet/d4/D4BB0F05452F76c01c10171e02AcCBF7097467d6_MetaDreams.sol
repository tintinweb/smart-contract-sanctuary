...
contract MetaDreams is Context, IERC20, Ownable {
    ...
    string private _name = "MDXX5";
    string private _symbol = "MDXX5";
    ...
    address public marketingAndTeamFeeWallet = 0x0897b6f79Fb000Fa4BF6c7E8B972f09Fc95293f6;
    ...
    function _distributeFee(uint256 rFee, uint256 rBurn, uint256 rMarketingFee, uint256 tFee, uint256 tBurn, uint256 tMarketingFee) private {
        _rTotal = _rTotal.sub(rFee).sub(rBurn);
        _tFeeTotal = _tFeeTotal.add(tFee);
        _tTotal = _tTotal.sub(tBurn);
        _tBurnTotal = _tBurnTotal.add(tBurn);

        _rOwned[marketingAndTeamFeeWallet] = _rOwned[marketingAndTeamFeeWallet].add(rMarketingFee);
        if (_isExcluded[marketingAndTeamFeeWallet]) {
            _tOwned[marketingAndTeamFeeWallet] = _tOwned[marketingAndTeamFeeWallet].add(tMarketingFee);
        }
    }

    function _getValues(uint256 tAmount) private view returns (ValuesStruct memory) {
        TValuesStruct memory tvs = _getTValues(tAmount);
        RValuesStruct memory rvs = _getRValues(tAmount, tvs.tFee, tvs.tBurn, tvs.tMarketingFee, tvs.tLiquidity, _getRate());
    ...
    function _getTValues(uint256 tAmount) private view returns (TValuesStruct memory) {
        ...
        uint256 tMarketingFee = calculateMarketingAndTeamFee(tAmount);
        uint256 tLiquidity = calculateLiquidityFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tBurn).sub(tMarketingFee).sub(tLiquidity);
        return TValuesStruct(tTransferAmount, tFee, tBurn, tMarketingFee, tLiquidity);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tBurn, uint256 tMarketingFee, uint256 tLiquidity, uint256 currentRate) private pure returns (RValuesStruct memory) {
        uint256 rAmount = tAmount.mul(currentRate);
        ...
        return RValuesStruct(rAmount, rTransferAmount, rFee, rBurn, rMarketingFee, rLiquidity);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        _distributeFee(vs.rFee, vs.rBurn, vs.rMarketingFee, vs.tFee, vs.tBurn, vs.tMarketingFee);
