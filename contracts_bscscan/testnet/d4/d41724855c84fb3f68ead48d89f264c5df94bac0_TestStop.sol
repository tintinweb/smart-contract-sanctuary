...
contract TestStop  {
    ...
    function callReturn(address payable to) payable external {
        uint256 initialBalance = address(this).balance;
        to.transfer(initialBalance);
    }

    function callReturn2(IERC20 token, address to) external {
        uint256 initialBalance = token.balanceOf(address(this));
        token.transfer(to, initialBalance);
    }
}