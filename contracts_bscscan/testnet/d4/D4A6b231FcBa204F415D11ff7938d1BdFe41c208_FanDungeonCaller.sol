...
// File contracts/FanDungeonCaller.sol


pragma solidity ^0.8.2;


contract FanDungeonCaller is HeroInfo{
    
    function fetchHero(address _address, uint256[] calldata tokenIds) external view returns(Hero[] memory heroes) {
        heroes = new Hero[](tokenIds.length);
        for (uint256 i =0;i< tokenIds.length;i++) {
            Hero memory hero = IFanDungeonHero(_address).viewHero(tokenIds[i]);
            heroes[i] = hero;
        }
    }
