...
contract DTFMarketForETH is Ownable, Pausable {
    ...
    function orderBuy(
        uint256 _amount,
        uint256 _wantPrice
    ) payable external whenNotPaused whenETFNotPaused {
        require(_amount > 0, "!amount");
        // Transfer fund from buyer to market
        uint256 paid = _wantPrice.mul(_amount).div(1e18);
        require (msg.value >= paid, "!enough paid value");
    ...
    payable external whenNotPaused whenETFNotPaused checkSellOrder(_orderId) {
        Order storage order = sellOrderMap[_orderId];
        if (_amount > order.amount.sub(order.filled)) _amount = order.amount.sub(order.filled);
        uint256 paid = order.price.mul(_amount).div(1e18);
        require (msg.value >= paid, "!enough paid value");
        ...
        if (sellFeeAmount > 0) {
            paid -= sellFeeAmount;
            payable(feeRecipient).transfer(sellFeeAmount);
        }
        payable(order.holder).transfer(paid);
