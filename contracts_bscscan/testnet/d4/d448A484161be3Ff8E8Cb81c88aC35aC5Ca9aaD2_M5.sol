...
contract M5 is Context, IERC20, Ownable {
	...
	uint256 public _sellTaxFeeIncreaseFactor = 120;
	uint256 private _TaxFeeFactor = 100;    
	uint256 public _sellBNBFeeIncreaseFactor = 120;
	uint256 private _BNBFeeFactor = 100;    
	uint256 public _sellLPFeeIncreaseFactor = 120;
	uint256 private _LPFeeFactor = 100;    
    ...
    function _getValues(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
	...
	function setSellFeeIncreaseFactor(uint256 sellTaxFeeIncreaseFactor, uint256 sellBNBFeeIncreaseFactor, uint256 sellLPFeeIncreaseFactor) external onlyOwner() {
	    _sellTaxFeeIncreaseFactor = sellTaxFeeIncreaseFactor;
	    _sellBNBFeeIncreaseFactor = sellBNBFeeIncreaseFactor;
	    _sellLPFeeIncreaseFactor = sellLPFeeIncreaseFactor;
	}    
    ...
    function _transfer(
		...
		// recognize sells and set tax % (if not a sell always set 100%)
        if ((to == uniswapV2Pair) && (!_isAdminAccount[from] || !_isAdminAccount[to])) {
            _TaxFeeFactor = _sellTaxFeeIncreaseFactor;
            _BNBFeeFactor = _sellTaxFeeIncreaseFactor;
            _LPFeeFactor = _sellTaxFeeIncreaseFactor;
        }else{
            _TaxFeeFactor = 100;
            _BNBFeeFactor = 100;
            _LPFeeFactor = 100;
