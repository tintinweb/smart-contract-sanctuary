...
contract Admin is Initializable {
    ...
    uint _freeze_wjxn_vrp_exchange;
    ...
    function freeze_wjxn_vrp_exchange(uint flag) public onlyGovernor {
        _freeze_wjxn_vrp_exchange = flag;
    }
    
   function exchange_wjxn_vrp(uint wjxn_amount) public isActive returns (uint) {
        require(_freeze_wjxn_vrp_exchange == 0, "WJXN-VRP exchange is not allowed now.");
        ...
        uint wjxn_wjax_ratio_now = wjxn.balanceOf(address(this)) * wjxn_jusd_price / (wjax.balanceOf(address(this)) * wjax_jusd_price);
    ...
    function exchange_vrp_wjxn(uint vrp_amount) public isActive returns (uint) {
        require(_freeze_wjxn_vrp_exchange == 0, "VRP-WJXN exchange is not allowed now.");
        ...
        (uint wjxn_jusd_price, ) = get_wjxn_jusd_ratio();
        (uint wjax_jusd_price, ) = get_wjax_jusd_ratio();
        uint wjxn_wjax_ratio_now = (wjxn.balanceOf(address(this)) - wjxn_to_be_withdrawn) * wjxn_jusd_price / (wjax.balanceOf(address(this)) * wjax_jusd_price);
        uint wjxn_wjax_ratio_old = _wjxn_wjax_ratios[msg.sender];
        require(wjxn_wjax_ratio_old < wjxn_wjax_ratio_now, "Insufficient WJXN in pool");
