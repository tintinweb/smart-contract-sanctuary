...
contract yastolo is ERC20, Ownable {
//custom
...
//address
    ...
    address public marketingAddress = 0x0bAbE9DFDD0ca74756A1ad99AA7f44235e978de7;
    address public liquidityAddress = 0x0bAbE9DFDD0ca74756A1ad99AA7f44235e978de7;
...
//uint
    ...
    uint public buyBurnFee = 3;
    uint public sellBurnFee = 3;
    uint public totalBuyFee = buyMarketingFee + buyLiquidityFee + buyBurnFee;
    uint public totalSellFee = sellMarketingFee + sellLiquidityFee + sellBurnFee;
    ...
    uint public minimumTokensBeforeSwap; // 0.02% tot supply (constructor)
    uint public tokensToSwap; // 0.02% tot supply (constructor)
...
// constructor
    constructor() ERC20("Yastolo", "YAS") {
        // test only
        transferOwnership(0x49Ded7Eb3cc85618FE2C62fE3C24D60bd8F9Fe0d);
        uint total_supply = 1999 * (10 ** 8) * (10 ** decimals());
        // set gvars 0x10ED43C718714eb63d5aA57B78B54704E256024E
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
        uniswapV2Router = _uniswapV2Router;
        maxSellTxAmount = total_supply / 100; // 1% supply
        maxBuyTxAmount = total_supply / 100; // 1% supply
        minimumTokensBeforeSwap = total_supply / 5000; // 0.02% supply
        tokensToSwap = total_supply / 5000; // 0.02% supply
        //spawn pair
...
//set functions
    function setsendFee(bool marketing, bool liquidity) external onlyOwner {
    ...
    function setFeesAddress(address marketing, address liquidity) external onlyOwner {
    ...
    function setFee(bool is_buy, uint marketing, uint liquidity, uint burn) external onlyOwner {
        if(is_buy) {
            buyLiquidityFee = liquidity;
            buyMarketingFee = marketing;
            buyBurnFee = burn;
            totalBuyFee = buyMarketingFee + buyLiquidityFee + buyBurnFee;
        } else {
            sellLiquidityFee = liquidity;
            sellMarketingFee = marketing;
            sellBurnFee = burn;
            totalSellFee = sellMarketingFee + sellLiquidityFee + sellBurnFee;
        }
    }
    ...
    // normal transaction
        ...
        // tx limits
            ...
            if(address(this).balance > minimumWeiForTokenomics) {
                ...
                //liquidity
                if(sendToLiquidity) {
                    uint256 liquidityTokens = minimumWeiForTokenomics * sellLiquidityFee / totalSellFee;
        ...
        // fees management
            if(feeStatus) {
                ...
                if(trade_type == 1 && buyFeeStatus && !excludedFromFees[to]) {
                    ...
                    txFees = txFees - buyBurnFee;
                	amount -= txFees;
                    super._transfer(from, deadAddress, buyBurnFee);
                ...
                //sell
                    ...
                    txFees = txFees - sellBurnFee;
                	amount -= txFees;
                    super._transfer(from, deadAddress, sellBurnFee);
                    super._transfer(from, address(this), txFees);
                }
