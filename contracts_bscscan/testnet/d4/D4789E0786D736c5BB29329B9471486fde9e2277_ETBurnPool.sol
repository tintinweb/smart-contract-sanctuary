 ...
 */
contract ETBurnPool is AccessControlEnumerable {
    ...
    event CalculateETHRewards(
        uint256 indexed epoch,
        uint256 ethAmount,
        uint256 etBurnAmount,
        bool indexed calculated
    );
    event BurnET(
        uint256 indexed epoch,
     ...
     * @param operator Initialize operator role
     * @param calculator Initialize calculator role
     * @param _firstRoundStartBlock Initialize first round start block, should be at 17:00 on a certain day
     * @param _intervalBlocks Initialize interval blocks, 1 day = 28800 blocks
     */
    constructor(
        ...
        _setupRole(CALCULATOR_ROLE, calculator);
    }

    /**
     * @dev Refresh current epoch
     */
    function refreshCurrentEpoch() public {
        if (block.number >= firstRoundStartBlock)
            currentEpoch =
                (block.number - firstRoundStartBlock) /
                intervalBlocks +
                1;
    }
     ...
     */
    function transferETH(uint256 epoch, uint256 ethAmount)
        external
    ...
    /**
     * @dev Calculate ETH rewards of a past round, should run after 17:00 everyday
     */
    function calculateETHRewards(uint256 epoch)
        external
        onlyRole(CALCULATOR_ROLE)
    {
        ...
        Round storage round = rounds[epoch];
        if (round.calculated == true)
            revert("This round has already been calculated.");
        ...
        if (round.etBurnAmount > 0) {
            ...
            if (round.ethAmount > 0) {
                for (uint256 i = 0; i < users[epoch].length(); i++) {
                    ethRewards[users[epoch].at(i)] +=
                        (round.ethAmount *
                            round.userBurnET[users[epoch].at(i)]) /
                        round.etBurnAmount;
                }
            }
        }

        emit CalculateETHRewards(
            epoch,
            round.ethAmount,
            round.etBurnAmount,
            round.calculated
    ...
    function burnET(uint256 etBurnAmount) external {
        ...
        users[currentEpoch].add(msg.sender);
    ...
    function withdrawETH() external {
        uint256 ethRewardsAmount = ethRewards[msg.sender];
