...
contract VeldoraTest is Context, IERC20, Ownable {
    ...
    address payable public _marketing_address = 0x89ff795cd04e4AdFbf78C89189B6bB7c565Aa1aE;
    ...
    uint256 private _marketingTransferAmount = 1 * 10**_decimalsMul;
    
    uint256 public _burnFee = 2;
    uint256 private _previousBurnFee = _burnFee;
    
    // Taxes accumulated in this account until swap event
    uint256 public _tAccumulatedMarketing = 0;
    uint256 public _tAccumulatedLiquidity = 0;
    uint256 public _rAccumulatedMarketing = 0;
    uint256 public _rAccumulatedLiquidity = 0;
    ...
    uint256 public _purchaseLimitBlocks = 0;
    uint256 public _purchaseLimit = 0;
    uint256 public _maxTxAmount = 5e12 * 10**_decimalsMul;
    uint256 private numTokensSellToAddToLiquidity = 1 * 10 ** _decimalsMul; //5e11 * 10**_decimalsMul;
	...
	event SniperStatusChanged(address sniperAddress, bool isSniper);
    ...
    constructor () public {
        ...
        _isExcludedFromRewards[_burn_address] = true;
    ...
    function excludeFromReward(address account) public onlyOwner() {
        ...
        if(_rOwned[account] > 0) {
            ...
            if (account == address(this)) {
                _tAccumulatedLiquidity = tokenFromReflection(_rAccumulatedLiquidity);
                _tAccumulatedMarketing = tokenFromReflection(_rAccumulatedMarketing);
            }
        }
    ...
    function includeInReward(address account) external onlyOwner() {
        ...
        if (account == address(this)) {
            _tAccumulatedMarketing = 0;
            _tAccumulatedLiquidity = 0;
        }
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        _rAccumulatedLiquidity = _rAccumulatedLiquidity.add(rLiquidity);
        if(_isExcludedFromRewards[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tLiquidity);
            _tAccumulatedLiquidity = _tAccumulatedLiquidity.add(tLiquidity);
    }
	...
	function _reserveMarketing(uint256 tMarketing) private {
		uint256 currentRate = _getRate();
		uint256 rMarketing = tMarketing.mul(currentRate);
		_rOwned[address(this)] = _rOwned[address(this)].add(rMarketing);
		_rAccumulatedMarketing = _rAccumulatedMarketing.add(rMarketing);
        if(_isExcludedFromRewards[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tMarketing);
            _tAccumulatedMarketing = _tAccumulatedMarketing.add(tMarketing);
	}
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != uniswapV2Pair) {
            uint256 marketingAccumulation = _tAccumulatedMarketing > 0 ? _tAccumulatedMarketing : tokenFromReflection(_rAccumulatedMarketing);
            bool overMinTokenBalance = contractTokenBalance - marketingAccumulation >= numTokensSellToAddToLiquidity;
            ...
            if (marketingAccumulation >= _marketingTransferAmount) {
                // Swap and transfer accumulated marketing tax
                swapAndTransferMarketing(_marketingTransferAmount);
            }
        }
    ...
    function swapAndTransferMarketing(uint256 amount) private lockTheSwap {
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(amount);
        
        if (_tAccumulatedMarketing > 0) 
            _tAccumulatedMarketing = _tAccumulatedMarketing.sub(amount);
        else
            _rAccumulatedMarketing = amount.sub(amount * _getRate());
        
        _marketing_address.transfer(address(this).balance.sub(initialBalance));
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (_tAccumulatedLiquidity > 0) 
            _tAccumulatedLiquidity = _tAccumulatedLiquidity.sub(contractTokenBalance);
        else
            _rAccumulatedLiquidity = _rAccumulatedLiquidity.sub(contractTokenBalance * _getRate());
...
// 		_liqAddBlock = block.number;
	...
	function _checkSniper(address from, address to) private returns (bool) {
	...
	function addSniper(address sniperAddress) external onlyOwner() {
	    ...
	    emit SniperStatusChanged(sniperAddress, true);
	}
	...
	function removeSniper(address sniperAddress) external onlyOwner() {
	    ...
	    emit SniperStatusChanged(sniperAddress, false);
	}
    ...
    function _completeTransfer(address sender, address recipient, ExtraValues memory values) private {
        ...
        if (values.tBurn > 0) {
            ...
            emit Transfer(sender, _burn_address, values.tBurn);
        }
        if (values.tMarketing > 0) {
            _reserveMarketing(values.tMarketing);
