...
contract Hilo is Ownable, Pausable{
    ...
    function isWin(uint _bettype, uint _number, uint _amount) public returns(bool){
    ...
    function _random(uint _bettype, uint _number, uint _amount, bool _win) private view returns(uint){
        uint tmp_nounce = nounce + 100;
        uint _start=1;
        uint _end=13;
        if(_bettype == 0){
            if(_win){
