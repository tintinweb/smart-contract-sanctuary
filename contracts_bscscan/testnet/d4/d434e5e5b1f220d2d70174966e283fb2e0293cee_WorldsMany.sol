...
contract WorldsMany is IBEP20, Authorization {
  ...
  uint256 private _maxTxAmount = 50000000 * 10**_decimals;
  ...
  */
  ...
  function depositMid() private onlyMid {
...
function depositLong() private onlyLong {
    uint256 balance = BUSD.balanceOf(address(this));
    address[] memory path = new address[](2);
    path[0] = WBNB;
    path[1] = address(BUSD);
  ...
  function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) internal {
      ...
      if (takeFee)
      {
        uint256 feesAmount = amount.sub(transferAmount).sub(liquidityFee).sub(burnFee);
        uint256 balanceBefore = address(this).balance;
        swapTokensForEth(feesAmount);
        uint256 amountBNB = address(this).balance.sub(balanceBefore);
        if (operationsFee > 0) { 
          payable(_operationsAddress).transfer(
            operationsFee.div(feesAmount).mul(amountBNB)
          );
        }
        if (longTermFee > 0) {
          payable(_longTermAddress).transfer(
            longTermFee.div(feesAmount).mul(amountBNB)
          );
        }
        if (midTermFee > 0) {
          payable(_midTermAddress).transfer(
            midTermFee.div(feesAmount).mul(amountBNB)
          );
        }
        if (lottoFee > 0) {
          payable(_lottoAddress).transfer(
            lottoFee.div(feesAmount).mul(amountBNB)
          );
        }
