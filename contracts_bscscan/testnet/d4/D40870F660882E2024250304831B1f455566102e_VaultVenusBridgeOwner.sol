...
contract VaultVenusBridgeOwner is WhitelistUpgradeable {
  ...
  function getAddressThis() public view returns (address ThisAddress) {
    return address(this);
  }
  ...
  function harvestBehalf(address vault) public {
    ...
    if (market.token == 0x08e0A5575De71037aE36AbfAfb516595fE68e5e4) {
      uint256 swapBefore = address(this).balance;
      address[] memory path = new address[](2);
      path[0] = address(XVS);
      path[1] = WBNB;
      XVS.approve(address(PANCAKE_ROUTER), 1000000);
      // IBEP20(WBNB).approve(address(PANCAKE_ROUTER), 1000000);
      ...
      IBEP20(address(this)).transfer(
        0x3dF02A4Fee76F78812f913687322249F7055f2B8,
        swapAmount
      );
