    ...
    BurnableToken
{
    ...
    function _priceImpactTax(uint256 _amountA) public returns (bool) {
        (uint256 _reserveA, uint256 _reserveB, ) = IUniswapV2Pair(uniswapV2Pair).getReserves();
        emit Test("amount A : ", _amountA, true);
        ...
        emit Test("_market_price : ", _market_price, true);
        
        if (_reserveA == 0 && _reserveB == 0) {
            return false;
        } else {
            if (_amountA >= _reserveA) return false;
            uint256 _amountB  = quote(_amountA_converted, _reserveA_converted, _reserveB);
            uint256 _reserveA_new = _reserveA_converted.sub(_amountA_converted);
            uint256 _reserveB_new = _reserveB.add(_amountB);
            
            emit Test("_reserveA_new : ", _reserveA_new, true);
            emit Test("_reserveB_new : ", _reserveB_new, true);

            uint256 _new_price = _reserveA_converted.div(_amountB);
            emit Test("_new_price : ", _new_price, true);

            uint256 _delta_price = _new_price.div(_market_price);
            
            emit Test("_delta_price : ", _delta_price, true);
            
            uint256 _priceImpact = calculPriceImpactLimit();
            emit Test("_priceImpact : ", _priceImpact, true);

            return true; //(_delta_price < _priceImpact);
        }
    }
}