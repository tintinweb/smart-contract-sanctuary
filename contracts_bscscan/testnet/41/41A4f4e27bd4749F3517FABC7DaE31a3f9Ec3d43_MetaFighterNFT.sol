...
import {IMetaFighterNFT} from "../interfaces/IMetaFighterNFT.sol";

contract MetaFighterNFT is
    ERC721,
    ERC721Enumerable,
    Pausable,
    ERC721Burnable,
    ...
    IMetaFighterNFT
{
    ...
    string public override baseMetaFighterURI;
    ...
    event mintRecord(uint256 ID, uint256 amount, address to, uint256 unitPrice, uint256[]);
    ...
    modifier onlyGenesisMinter() {
        require(
            ...
            "MetaFighterERC721: caller is not genesis minter"
        );
        _;
    }
    ...
    modifier onlyForkMinter() {
        require(
            forkMinter == _msgSender(),
            "MetaFighterERC721: caller is not fork minter"
        );
        _;
    }
    ...
    function rollStartIndex() external override onlyGenesisMinter {
        require(
            randomizedStartIndex == 0,
            "MetaFighterERC721: start index is already rolled"
        );
    ...
    function mintGenesis(
        ...
        address to,
        uint256 unitPrice
    ) external  override onlyGenesisMinter{
        uint256 startId = currentId;
        require(
            startId + amount <= MAX_GENESIS_SUPPLY,
            "MetaFighterERC721: max genesis supply reached"
        );
        ...
        emit mintRecord(startId, amount, to, unitPrice,ids);
    }
    ...
    function mintFork(uint256 tokenId) external override onlyForkMinter {
        ...
        require(
            forkId > MAX_GENESIS_SUPPLY,
            "MetaFighterERC721: not mint all genesis yet"
        );

        address owner = ownerOf(tokenId);
        require(owner != address(0), "MetaFighterERC721: token does not exist");
        ...
        returns (address)
    {
        require(
            ...
            "MetaFighterERC721: invalid token id"
        );
...
interface IMetaFighterNFT is IERC721Enumerable {
    /**
     * @dev Call only by the Genesis Minter to roll the start index
     */
    ...
    /**
     * @dev Return the base MetaFighter URI for tokens
     */
    function baseMetaFighterURI() external view returns (string memory);
