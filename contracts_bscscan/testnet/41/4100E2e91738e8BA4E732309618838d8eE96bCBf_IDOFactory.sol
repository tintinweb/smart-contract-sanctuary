...
contract IDOFactory is Whitelist {
    ...
    function createIDOContract(
        ...
        if (_allowances.length > 0) {
            newIDO = address(new IDOProxy(implementations[1], owner()));
            ...
            newIDO = address(new IDOProxy(implementations[0], owner()));
    ...
    function getIDO(uint256 index) public view returns (address) {
        return IDOs[index];
    }
