...
// File: contracts/flokiETHDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable BUSD = address(0x18C7Cbf505519bd86438fA4Df8CFa1967Cd68b82);
...
contract DADDY_SUSHI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable BUSD = address(0x18C7Cbf505519bd86438fA4Df8CFa1967Cd68b82);
    address private _marketingWallet = address(0xa338E9E47bBAd592fF2D6D3573f3965AFe8B01FA);
    // address private _devWallet = address(0xcA374305971114B028E679c0B69e43Cc95c6A6f7);

    bool private swapping;
    bool public airdrop;

    DADDY_SUSHIDividendTracker public dividendTracker;

    address public liquidityWallet;
    address public marketingWallet;
    // address public devWallet = _devWallet;

    uint256 public maxBuyTranscationAmount = 200000 * (10**18); // 2% of total supply
    uint256 public maxSellTransactionAmount = 50000 * (10**18); // 0.5% of total supply
    uint256 public swapTokensAtAmount = 50000 * (10**18);
    uint256 public _maxWalletToken = 10000000 * (10**18); // No limit
    ...
    uint256 public immutable tradingEnabledTimestamp = 1627132800;
    ...
    constructor() public ERC20("Daddy_Sushi_Test", "DADDY_SUSHI") {
    	...
    	dividendTracker = new DADDY_SUSHIDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DADDY_SUSHI: The dividend tracker already has that address");

        DADDY_SUSHIDividendTracker newDividendTracker = DADDY_SUSHIDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DADDY_SUSHI: The new dividend tracker must be owned by the DADDY_SUSHI token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DADDY_SUSHI: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DADDY_SUSHI: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DADDY_SUSHI: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "DADDY_SUSHI: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DADDY_SUSHI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DADDY_SUSHI: Cannot update gasForProcessing to same value");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DADDY_SUSHIBSC: Account is already the value of 'excluded'");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "DADDY_SUSHI: This account cannot send tokens until trading is enabled");
        }
        ...
        if(
            ...
            uint256 marketingTokens = contractTokenBalance.mul(BNBRewardsFee).div(totalFees);
            swapTokensForBNB(marketingTokens);
            
            // uint256 devTokens = contractTokenBalance.mul(liquidityFee).div(totalFees);
...
contract DADDY_SUSHIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("DADDY_SUSHI_Dividend_Tracker", "DADDY_SUSHI_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "DADDY_SUSHI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "DADDY_SUSHI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DADDY_SUSHI contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DADDY_SUSHI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DADDY_SUSHI_Dividend_Tracker: Cannot update claimWait to same value");
