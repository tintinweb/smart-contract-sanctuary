...
contract charity{
    ...
    function getFundsIn(string memory _language) public view returns (fundsIn[] memory){
         require(keccak256(abi.encodePacked(_language)) == keccak256(abi.encodePacked("English")) || keccak256(abi.encodePacked(_language)) == keccak256(abi.encodePacked("Chinese")),"Invalid language provided");
        if(keccak256(abi.encodePacked(_language)) == keccak256(abi.encodePacked("English"))){
