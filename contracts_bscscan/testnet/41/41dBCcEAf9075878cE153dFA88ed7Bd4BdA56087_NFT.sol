...
contract NFT is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    // mint a new token
    function createToken(string memory uriOfToken) public onlyWhitelistedMinter returns (uint256) {
        _tokenIds.increment();
    ...
    function createTokens(string memory tokenBaseURI, uint256 count) public onlyWhitelistedMinter {
    ...
    function createTokenForUser(string memory uriOfToken, address ownerAddress) public onlyWhitelistedMinter returns (uint256) {
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
    ...
    function createTokensForUser(string memory tokenBaseURI, uint256 count, address ownerAddress) public onlyWhitelistedMinter {
        for (uint i = 0; i < count; i++) {
            _tokenIds.increment();
            uint256 newItemId = _tokenIds.current();
