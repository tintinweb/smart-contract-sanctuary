 ...
 * `Err18` Already join
 */
contract Racepool is Initializable, OwnableUpgradeable, IRacepool {
  ...
  {
    if (balances[id][msg.sender][0] > 0) {
      IERC20Upgradeable(erc20Addr_1).safeTransfer(msg.sender, balances[id][msg.sender][0]);
    }

    if (balances[id][msg.sender][1] > 0) {
      IERC20Upgradeable(erc20Addr_2).safeTransfer(msg.sender, balances[id][msg.sender][1]);
    }
