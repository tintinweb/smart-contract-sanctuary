...
contract UltimateDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    UltimateDogeDividendTracker public dividendTracker;

    address public liquidityWallet;

    address payable public operations1Address =
        payable(0x03AA1cf0097041Df7Ee901518697bEDde21CCaB5);
    address payable public operations2Address =
        payable(0x03AA1cf0097041Df7Ee901518697bEDde21CCaB5);

    uint256 public maxSellTransactionAmount = 500000000000 * (10**9);
    ...
    constructor() ERC20("UltimateDoge", "UDOGE") {
        dividendTracker = new UltimateDogeDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        UltimateDogeDividendTracker newDividendTracker = UltimateDogeDividendTracker(
            payable(newAddress)
        );
    ...
    function _transfer(
        ...
        require(
            tradingEnabled || canTransferBeforeTradingIsEnabled[from] || block.timestamp >= tradingEnabledTimestamp,
...
contract UltimateDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    UltimateDoge public ultimateDoge;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "UltimateDoge_Dividend_Tracker",
            "UltimateDoge_Dividend_Tracker"
        )
    {
        ultimateDoge = UltimateDoge(mainContract);
    ...
    function _transfer(
        ...
        require(false, "UltimateDoge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "UltimateDoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main UltimateDoge contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ultimateDoge);

            uint256 prevBalance = ultimateDoge.balanceOf(address(this));
            ...
            {
                uint256 received = ultimateDoge.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    ultimateDoge.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
