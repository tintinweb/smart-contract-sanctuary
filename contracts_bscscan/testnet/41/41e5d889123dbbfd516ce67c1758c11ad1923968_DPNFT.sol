...
contract DPNFT is Context, IERC20, Ownable {
    ...
    string private _name = " Test DP";
    string private _symbol = "TEST-DP";
    uint8 private _decimals = 9;
    
    uint256 public _taxFee = 5;//return holder
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 10;//deadwallet
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        uint256 currentRate =  _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        _rOwned[deadWallet] = _rOwned[deadWallet].add(rLiquidity);
        if(_isExcluded[deadWallet])
            _tOwned[deadWallet] = _tOwned[deadWallet].add(tLiquidity);
    }
