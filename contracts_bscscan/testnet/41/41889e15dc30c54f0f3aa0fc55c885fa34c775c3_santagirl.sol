...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minDistribution = 6 * (10 ** 13);
...
contract santagirl is IBEP20, Auth {
    ...
    string constant _name = "santa03";
    string constant _symbol = "santa03";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 1000000 * (10 ** _decimals);
    uint256 public _maxTxAmount = _totalSupply / 400; // 0.25%
    uint256 public maxWallet = _totalSupply / 100; // 1%
    uint256 public minRaffle = _totalSupply / 2000; // 0.05%
    ...
    uint256 lotteryFee = 100;
    uint256 devFee = 50;
    uint256 totalfixedBuyFee = liquidityFee.add(buybackFee).add(reflectionFee).add(marketingFee).add(lotteryFee).add(devFee);
    uint256 totalBuyFee = 1010;
    ...
    uint256 public PaperHandNumerator = 250;
    ...
    uint256 SlidingBuyNumerator = 10; 
    uint256 SlidingSellNumerator = 50; 
    uint256 SlidingBuyDuration = 5 minutes; 
    uint256 SlidingSellDuration = 20 minutes;
    uint256 SlidingBuyLimit = 600;
    uint256 SlidingSellLimit = 2000;
   ...
   function setMultiplierSettings(uint256 PaperHandNum, uint256 DiamondHandNum, uint256 denominator, uint256 length) external authorized{
    ...
    function setTxLimit(uint256 amount) external authorized {
        require(amount != _maxTxAmount, "santagirl:the max tx amount is already this amount");
        _maxTxAmount = amount;
    }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _devFeeReceiver, address _lotteryFeeReceiver) external authorized {
