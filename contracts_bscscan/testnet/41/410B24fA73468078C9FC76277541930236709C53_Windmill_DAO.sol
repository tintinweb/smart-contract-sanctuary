...
interface I_Windmill_Stacking is I_Math, IAccessControlEnumerable, I_With_DAORole, I_With_UpdaterRole, I_With_PWRToken, I_With_FundContract{
    ...
    function updateStackingNeeded() external returns (bool);
    
    function updateOneStackingNeeded(uint256 groupId) external returns (bool);
...
event GasLog_With_StackingContract(uint256 line, uint256 gas);
    I_Windmill_Stacking public stacking;

    function setStacking(I_Windmill_Stacking _stacking) external onlyRole(DAO_ROLE){
emit GasLog_With_StackingContract(15, gasleft());
        stacking = _stacking;
emit GasLog_With_StackingContract(16, gasleft());
    }
}// Version: 1.0.0
...
interface I_Windmill_Royalty is I_Math, IAccessControlEnumerable, I_With_DAORole, I_With_UpdaterRole, IERC20, I_With_PWRToken, I_With_FundContract{
    function percentPWRRoyaltyMint() external returns (uint256, uint256);
...
event GasLog_With_RoyaltyContract(uint256 line, uint256 gas);
    I_Windmill_Royalty public royalty;

    function setRoyalty(I_Windmill_Royalty _royalty) external onlyRole(DAO_ROLE){
emit GasLog_With_RoyaltyContract(15, gasleft());
        royalty = _royalty;
emit GasLog_With_RoyaltyContract(16, gasleft());
    }
}// Version: 1.0.0
...
interface I_Windmill_Trade_Manager is I_Math, IAccessControlEnumerable, I_With_DAORole, I_With_UpdaterContract,
    ...
    function getNbTradeDeployers() external returns (uint256);
    
    function getTrade(uint256 id) external returns (TradeData memory);
    
    function getNbOpenTrades() external returns (uint);
    
    function getOpenTrade(uint openId) external returns (TradeData memory, uint256);
    
    function getTradeDeployer(uint256 id) external returns (DeployerData memory);
}

contract With_TradeManagerContract is AccessControlEnumerable, With_DAORole, I_With_TradeManagerContract{
event GasLog_With_TradeManagerContract(uint256 line, uint256 gas);
    I_Windmill_Trade_Manager public tradeManager;

    function setTradeManager(address _tradeManager) public onlyRole(DAO_ROLE){
emit GasLog_With_TradeManagerContract(15, gasleft());
        tradeManager = I_Windmill_Trade_Manager(_tradeManager);
emit GasLog_With_TradeManagerContract(16, gasleft());
    }
}// Version: 1.0.0
...
interface I_Gas_Refundable is I_Math{
    ...
    function refundBNBBonusRatio() external returns (uint256, uint256);
...
event GasLog_With_UpdaterContract(uint256 line, uint256 gas);
    I_Windmill_Updater public updater;

    function setUpdater(address _updater) external onlyRole(DAO_ROLE){
emit GasLog_With_UpdaterContract(15, gasleft());
        updater = I_Windmill_Updater(payable(_updater));
emit GasLog_With_UpdaterContract(16, gasleft());
    }
}// Version: 1.0.0
...
interface I_Windmill_DAO is IAccessControlEnumerable, I_With_DAORole, I_With_UpdaterRole, I_With_FundContract,
    ...
    function getNbOpenProposalIds() external returns (uint256);
    
    function updateOpenProposalNeeded(uint256 i) external returns (bool);
...
contract Windmill_DAO is Math, Adressable, AccessControlEnumerable, With_DAORole, With_UpdaterRole,
                         With_FundContract, With_StackingContract, With_RoyaltyContract,
                         With_TradeManagerContract, With_UpdaterContract, With_PWRToken, I_Windmill_DAO{
event GasLog_Windmill_DAO(uint256 line, uint256 gas);
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    function getUserData(address addr) external returns (DAOLevel memory){
emit GasLog_Windmill_DAO(85, gasleft());
        return users[addr];
emit GasLog_Windmill_DAO(86, gasleft());
    }
...
emit GasLog_Windmill_DAO(136, gasleft());
        require(level <= 3, "Windmill_DAO: the max level is 3");
emit GasLog_Windmill_DAO(137, gasleft());

        DAOLevel storage user = users[addr];
emit GasLog_Windmill_DAO(139, gasleft());
        user.level = level;
emit GasLog_Windmill_DAO(140, gasleft());
        
        tradeManager.setTraderLevel(addr, level);
emit GasLog_Windmill_DAO(142, gasleft());
        
        updater.setUserGovernorStatus(addr, level == 3);
emit GasLog_Windmill_DAO(144, gasleft());
    }
     ...
     */
    function getVotes(address addr) public returns (uint8){
emit GasLog_Windmill_DAO(157, gasleft());
        if (users[addr].level < 3){
emit GasLog_Windmill_DAO(158, gasleft());
            return 0;
emit GasLog_Windmill_DAO(159, gasleft());
        }

        uint256 balance = PWRToken.balanceOf(addr);
emit GasLog_Windmill_DAO(162, gasleft());

        if (balance == 0){
emit GasLog_Windmill_DAO(164, gasleft());
            return 0;
emit GasLog_Windmill_DAO(165, gasleft());
        }

        uint256 PWRSupply = fund.getTotalPWR();
emit GasLog_Windmill_DAO(168, gasleft());
        uint256 fraction = ((10**3) * (balance)) / PWRSupply;
emit GasLog_Windmill_DAO(169, gasleft());
        uint8 nbVotes = numDigits(fraction);
emit GasLog_Windmill_DAO(170, gasleft());

        return nbVotes;
emit GasLog_Windmill_DAO(172, gasleft());
    }

    /**
     * @notice Get the number of remaining proposals for an address
     */
    function getRemainingProposals(address addr) public returns (uint256){
emit GasLog_Windmill_DAO(178, gasleft());
        DAOLevel storage user = users[addr];
emit GasLog_Windmill_DAO(179, gasleft());

        if (user.level < 3){
emit GasLog_Windmill_DAO(181, gasleft());
            return 0;
emit GasLog_Windmill_DAO(182, gasleft());
        }

        if (currentDAOCycle > user.lastProposalDAOCycle){
emit GasLog_Windmill_DAO(185, gasleft());
            return nbProposalPerUserPerDAOCycle;
emit GasLog_Windmill_DAO(186, gasleft());
        }

        return nbProposalPerUserPerDAOCycle - user.nbProposalsDone;
emit GasLog_Windmill_DAO(189, gasleft());
    }
...
emit GasLog_Windmill_DAO(196, gasleft());
        require(openProposalIds.length < maxNbOpenProposals, "Windmill_DAO: Max number of open proposals reached");
emit GasLog_Windmill_DAO(197, gasleft());
        require(id < nbProposals, "Windmill_DAO: This is not a valid proposal id");
emit GasLog_Windmill_DAO(198, gasleft());
        require(getRemainingProposals(msg.sender) > 0, "Windmill_DAO: No remaining proposals for this address");
emit GasLog_Windmill_DAO(199, gasleft());

        Proposal memory proposal;
emit GasLog_Windmill_DAO(201, gasleft());
        proposal.id = id;
emit GasLog_Windmill_DAO(202, gasleft());
        proposal.paramsUint256 = paramsUint256;
emit GasLog_Windmill_DAO(203, gasleft());
        proposal.paramsAddress = paramsAddress;
emit GasLog_Windmill_DAO(204, gasleft());
        proposal.startBlock = block.number;
emit GasLog_Windmill_DAO(205, gasleft());
        if (isProposalSuper[id]){
emit GasLog_Windmill_DAO(206, gasleft());
            proposal.endBlock = block.number + superVoteBlockDuration;
emit GasLog_Windmill_DAO(207, gasleft());
        }else{
emit GasLog_Windmill_DAO(208, gasleft());
            proposal.endBlock = block.number + voteBlockDuration;
emit GasLog_Windmill_DAO(209, gasleft());
        }
...
emit GasLog_Windmill_DAO(229, gasleft());
        uint8 nbVotes = getVotes(msg.sender);
emit GasLog_Windmill_DAO(230, gasleft());
        require(nbVotes>0, "Windmill_DAO: There is 0 vote for this address");
emit GasLog_Windmill_DAO(231, gasleft());


        require(id < proposals.length, "Windmill_DAO: Proposal does not exist");
emit GasLog_Windmill_DAO(234, gasleft());
        Proposal storage proposal = proposals[id];
emit GasLog_Windmill_DAO(235, gasleft());

        require(!hasVoted[msg.sender][id], "Windmill_DAO: address have already voted");
emit GasLog_Windmill_DAO(237, gasleft());

        require(block.number >= proposal.startBlock, "Windmill_DAO: Proposal is not opened to vote");
emit GasLog_Windmill_DAO(239, gasleft());
        require(block.number < proposal.endBlock, "Windmill_DAO: Proposal is closed to vote");
emit GasLog_Windmill_DAO(240, gasleft());

        hasVoted[msg.sender][id] = true;
emit GasLog_Windmill_DAO(242, gasleft());

        if (isYes){
emit GasLog_Windmill_DAO(244, gasleft());
            proposal.nbYesVotes += nbVotes;
emit GasLog_Windmill_DAO(245, gasleft());
        }else{
emit GasLog_Windmill_DAO(246, gasleft());
            proposal.nbNoVotes += nbVotes;
emit GasLog_Windmill_DAO(247, gasleft());
        }
    }
    
    function setDAOCycle(uint256 cycle) external onlyRole(UPDATER_ROLE){
emit GasLog_Windmill_DAO(251, gasleft());
        currentDAOCycle = cycle;
emit GasLog_Windmill_DAO(252, gasleft());
    }
    
    function getNbOpenProposalIds() external returns (uint256){
emit GasLog_Windmill_DAO(255, gasleft());
        return openProposalIds.length;
emit GasLog_Windmill_DAO(256, gasleft());
    }
    
    function updateOpenProposalNeeded(uint256 i) public returns (bool){
emit GasLog_Windmill_DAO(259, gasleft());
        uint256 l = openProposalIds.length;
emit GasLog_Windmill_DAO(260, gasleft());
        
        require(i < l, "Windmill_DAO: Open proposal ID not found.");
emit GasLog_Windmill_DAO(262, gasleft());
        
        Proposal storage proposal = proposals[openProposalIds[i]];
emit GasLog_Windmill_DAO(264, gasleft());
        
        if (block.number >= proposal.endBlock && !proposal.done){
emit GasLog_Windmill_DAO(266, gasleft());
            return true;
emit GasLog_Windmill_DAO(267, gasleft());
        }
        return false;
emit GasLog_Windmill_DAO(269, gasleft());
    }
    
    function updateOneOpenProposal(uint256 i) external onlyRole(UPDATER_ROLE){
emit GasLog_Windmill_DAO(272, gasleft());
        uint256 l = openProposalIds.length;
emit GasLog_Windmill_DAO(273, gasleft());
        
        require(i < l, "Windmill_DAO: Open proposal ID not found.");
emit GasLog_Windmill_DAO(275, gasleft());
        
        _updateOneDAO(i);
emit GasLog_Windmill_DAO(277, gasleft());
    }
...
emit GasLog_Windmill_DAO(284, gasleft());
        uint256 proposalI = openProposalIds[i];
emit GasLog_Windmill_DAO(285, gasleft());
        Proposal storage proposal = proposals[proposalI];
emit GasLog_Windmill_DAO(286, gasleft());
        bool isSuper = isProposalSuper[proposal.id];
emit GasLog_Windmill_DAO(287, gasleft());
        if (updateOpenProposalNeeded(i)){
emit GasLog_Windmill_DAO(288, gasleft());
            _updateProposal(proposal, isSuper);
emit GasLog_Windmill_DAO(289, gasleft());
            
            if (i < openProposalIds.length-1){
emit GasLog_Windmill_DAO(291, gasleft());
                openProposalIds[i] = openProposalIds[openProposalIds.length-1];
emit GasLog_Windmill_DAO(292, gasleft());
            }
            openProposalIds.pop();
emit GasLog_Windmill_DAO(294, gasleft());
        }
    }
    
    function _updateProposal(Proposal storage proposal, bool isSuper) internal{
emit GasLog_Windmill_DAO(298, gasleft());
        uint256 totalVotes = proposal.nbYesVotes+proposal.nbNoVotes;
emit GasLog_Windmill_DAO(299, gasleft());
        uint256 percentYes = (100*proposal.nbYesVotes)/totalVotes;
emit GasLog_Windmill_DAO(300, gasleft());

        if (isSuper){
emit GasLog_Windmill_DAO(302, gasleft());
            if (percentYes > superVoteMajorityPercent){
emit GasLog_Windmill_DAO(303, gasleft());
                if (totalVotes >= superQuorum){
emit GasLog_Windmill_DAO(304, gasleft());
    ...
    function _checkProposalParameters(Proposal memory proposal) internal returns (bool){
        if (proposal.id == 0){
    ...
    function _unpackDeployerProposalData(uint256 data) internal returns (uint256, uint256, uint256){
emit GasLog_Windmill_DAO(574, gasleft());
        uint256 param = data >> (8+16);
emit GasLog_Windmill_DAO(575, gasleft());
        uint256 deployerProposalId = (data << (256-8-16)) >> (256-8);
emit GasLog_Windmill_DAO(576, gasleft());
        uint256 deployerId = (data << (256-16)) >> (256-16);
emit GasLog_Windmill_DAO(577, gasleft());
        
        return (param, deployerProposalId, deployerId);
emit GasLog_Windmill_DAO(579, gasleft());
    }
}