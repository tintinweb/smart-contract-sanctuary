...
abstract contract RedPill is BaseRfiToken, Liquifier, Antiwhale {

    using SafeMath for uint256;

    // constructor(string memory _name, string memory _symbol, uint8 _decimals){
    constructor(string memory tokenName, string memory symbolName, Env _env) {
    ...
    function _takeTransactionFees(uint256 amount, uint256 currentRate) internal override {
        ...
        for (uint256 index = 0; index < feesCount; index++ ){
            (FeeType feeName, uint256 feeValue, address feeRecipient,) = _getFee(index);
            // no need to check value < 0 as the value is uint (i.e. from 0 to 2^256-1)
            if ( feeValue == 0 ) continue;

            if ( feeName == FeeType.Rfi ){
                _redistribute( amount, currentRate, feeValue, index );
            }
            else if ( feeName == FeeType.Burn ){
                _burn( amount, currentRate, feeValue, index );
            }
            // else if ( feeName == FeeType.Antiwhale){
            //     // TODO
            // }
            else if ( feeName == FeeType.ExternalToETH){
                _takeFeeToETH( amount, currentRate, feeValue, feeRecipient, index );
            }
            else {
                _takeFee( amount, currentRate, feeValue, feeRecipient, index );
            }
        }
    }
    ...
    function _approveDelegate(address tokenOwner, address spender, uint256 amount) internal override {
        _approve(tokenOwner, spender, amount);
    }
}

contract RedPillBeta2 is RedPill{
    constructor(string memory tokenName, string memory symbolName) RedPill(tokenName, symbolName, Env.Testnet){
...
abstract contract BaseRfiToken is IERC20, IERC20Metadata, Ownable, Presale, Tokenomics {
    ...
    function _approve(address tokenOwner, address spender, uint256 amount) internal {
        require(tokenOwner != address(0), "BaseRfiToken: approve from the zero address");
    ...
    function _transferTokens(address sender, address recipient, uint256 amount, bool takeFee) private {
         ...
         */
        uint256 txSumOfFees = _getSumOfFees(sender, amount);
        if ( !takeFee ){ txSumOfFees = 0; }

        (uint256 rAmount, uint256 rTransferAmount, uint256 tAmount, uint256 tTransferAmount, uint256 currentRate ) = _getValues(amount, txSumOfFees);
        ...
        _takeFees( amount, currentRate, txSumOfFees );
        emit Transfer(sender, recipient, tTransferAmount);
    }

    function _takeFees(uint256 amount, uint256 currentRate, uint256 tfSumOfFees ) private {
        if ( tfSumOfFees > 0 && !isInPresale ){
...
abstract contract Liquifier is Ownable, Manageable {
     ...
     */
    function _setRouterAddress(address router) internal {
...
abstract contract Tokenomics {
    ...
    string internal NAME = "RedPill";
    string internal SYMBOL = "REDPILL";
     ...
     */
    address internal burnAddress = 0x0000000000000000000000000000000000C0FFEE;
    ...
    function _addFees() private {
         ...
         */
        _addFee(FeeType.Rfi, 31, address(this) );
        _addFee(FeeType.Burn, 9, burnAddress );
...
abstract contract Presale is Manageable {
