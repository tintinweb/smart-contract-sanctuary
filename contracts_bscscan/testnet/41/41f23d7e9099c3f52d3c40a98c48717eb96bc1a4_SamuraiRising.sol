...
contract SamuraiRising is ERC721Enumerable, RSunAuth {
    ...
    constructor() ERC721("SamuraiRising", "Samurai Rising") RSunAuth(msg.sender) {
        ...
        addPack(15 * 10 ** 14, 1, true, 0);
        addPack(50 * 10 ** 14, 5, true, 0);
        addPack(100 * 10 ** 14, 10, true, 0);
        addPack(150 * 10 ** 14, 15, true, 0);
        addPack(200 * 10 ** 14, 25, true, 0);

        addPack(40 * 10 ** 14, 1, true, 1);
        addPack(350 * 10 ** 14, 10, true, 1);
    }
    ...
    function teamMintSingle(uint8 packId) external {
    ...
    function teamMintPack(uint8 packId) external {
        authorizedFor(Permission.Mint);
        Pack memory pack = packTypes[packId];
