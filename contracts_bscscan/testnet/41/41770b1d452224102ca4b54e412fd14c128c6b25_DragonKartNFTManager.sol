    ...
    ItemFactoryManager
{
    ...
    modifier onlySupportedBoxType(uint8 boxType_) {
        require(
    ...
    function _mint(address to_, uint8 boxType_) private {
        uint256 _tokenId = nftCore.mint(to_);
        (, uint256 _artifacts) = nftCore.tokenMetaData(_tokenId);

        // Add box type: size 1 byte
        _artifacts = _addArtifactValue(_artifacts, 0, 8, uint256(boxType_));
    ...
    function mint(address to_, uint8 boxType_)
        public
        ...
        override(RandomConsumerBase)
    {
        ...
        _artifacts = _addArtifactValue(_artifacts, 32, 8, _itemType); // add itemType

        // add artifact length
        uint256 _artifactsLength = itemFactory.artifactsLength(_itemType);
        _artifacts = _addArtifactValue(_artifacts, 40, 8, _artifactsLength);
