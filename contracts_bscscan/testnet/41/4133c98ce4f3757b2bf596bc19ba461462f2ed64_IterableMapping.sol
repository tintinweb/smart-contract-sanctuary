...
contract miniBABYDOGE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    minibabydogeDividendTracker public dividendTracker;

    bool public presaleOver = false;
    address public presaleWallet;
    address public presaleRouter;

    // more internal contract functions that govern the finalization and presale
    bool private firstFewBlocks = false;
    bool private tradingIsEnabled = false;
    uint256 private firstFewBlocksF;
    

    // liquidityWallet is the wallet that holds liquidity for the token, which is locked through dxsale
    address public liquidityWallet;
    
    address payable public marketingWallet = 0x568C69865Cf023112410C42b6A2a048e3556Bef4;

    uint256 public swapTokensAtAmount = 200000 * (10**18);
    uint256 public maxSellTransactionAmount = 750000 * (10**18);
    ...
    // addresses that can make transfers before presale is over -- such as presale wallet and router
    mapping (address => bool) private canTransferBeforeTradingIsEnabled;

    // store addresses that a automatic market maker pairs. Any transfer *to* these addresses
    // could be subject to the maximum transfer amount of 1bn tokens
    // this is stored as a map of addresses instead of an address in case we list on more exchanges in the future
    ...
    constructor() ERC20("miniBABYDOGE", "miniBABYDOGE") {
        
        totalFees = BNBRewardsFee.add(liquidityFee);


    	dividendTracker = new minibabydogeDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "miniBABYDOGE: The router already has that address");
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "miniBABYDOGE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "miniBABYDOGE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "miniBABYDOGE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 1000000, "miniBABYDOGE: gasForProcessing must be between 200,000 and 1,000,000");
        require(newValue != gasForProcessing, "miniBABYDOGE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "miniBABYDOGE: This account cannot send tokens until trading is enabled");
        }
...
contract minibabydogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("minibabydoge_Dividend_Tracker", "minibabydoge_Dividend_Tracker") {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "minibabydoge_Dividend_Tracker: No transfers allowed");
    }
    ...
    function withdrawDividend() public override {
        require(false, "minibabydoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main minibabydoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 900 && newClaimWait <= 86400, "minibabydoge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "minibabydoge_Dividend_Tracker: Cannot update claimWait to same value");
