...
library SafeCast {
    ...
    function toUint224(uint256 value) internal pure returns (uint224) {
        require(value <= type(uint224).max, "SafeCast: val not fit in 224 bits");
    ...
    function toUint128(uint256 value) internal pure returns (uint128) {
        require(value <= type(uint128).max, "SafeCt: val not fit in 128 bits");
...
contract QfourTokenNoUp is Once, Ownable, ERC20Permit {
...
contract Presale is Ownable, ReentrancyGuardNoUpgrade {
    using SafeMath for uint256;
    ...
    QfourTokenNoUp public tokenAddr;
    ...
    modifier isTimeRange() {
        require(block.timestamp >= startTime, "Not yet");
        require(block.timestamp <= stopTime || tokenAddr.balanceOf(address(this)) > 0, "Not yet");
        _;        
    }
    ...
    */
    ...
    function setTokenAddr(QfourTokenNoUp _tokenAddr) public onlyOwner {
    ...
    */
    ...
    function setStartTime(uint32 start) public onlyOwner {
        startTime = start > 0 ? block.timestamp.add(uint(start).mul(1 minutes)) : block.timestamp;
    }

    function setStopTime(uint32 stop) public onlyOwner {
        stopTime = stop > 0 ? block.timestamp.add(uint(stop).mul(1 minutes)) : block.timestamp;
    }
}