...
contract Farm is UnicornMint{
    ...
    function incr(address account, uint256 boost, uint256 salt) external onlyMinter {
        
        bytes32 _to = hash(msg.sender,account,salt);
        
        updateReward(_to);
        
        uint lastBalance = balances[_to].amount;
        
        uint currBalance = lastBalance.mul(100+boost)/100;
        
        balances[_to].amount = currBalance;
        
        uint incrAmount = currBalance.sub(lastBalance);
        
        totalSupply = totalSupply.add(incrAmount);
    }
    
    function decr(address account, uint256 boost, uint256 salt) external onlyMinter {
        ...
        uint currBalance = lastBalance.mul(100)/(100+boost);
        
        balances[_to].amount = currBalance;
        
        uint decrAmount = lastBalance.sub(currBalance);
        
        totalSupply = totalSupply.sub(decrAmount);
    }
    ...
    function autoMint() internal returns( uint macMint, uint macPure, uint macBurn ) {
        ...
        mac.mint(address(this),macMint);
        
        macPure = macMint.mul(curHash).div(THRESHOLD);
        
        macBurn = macMint.sub(macPure);
        
        if(macBurn>0) mac.safeTransfer(dead,macBurn);
    }
    ...
    function rewardPerToken(uint macPure) public view returns(uint256) {
        ...
        return rewardPerTokenStored.add(
            macPure.mul(1e18).div(totalSupply)
        );
    }
...
contract UnicornFarm is Ownable,IERC721Receiver {
    ...
    using PawnLib for mapping(uint=>mapping(address=>PawnLib.Pawn[]));
    ...
    mapping(uint=>mapping(address=>PawnLib.Pawn[])) public pawns;
    ...
    function mintGroup(address account,uint race,uint groupId) internal {
        ...
        if(groups[account][race][groupId].length==5) {
            for(uint i;i<5;i++) {
                farm.incr(account,amplifier[race],groups[account][race][groupId][i]);
            }
        }
    }
     ...
     function burnGroup(address account,uint race,uint groupId) internal {
        ...
        if(groups[account][race][groupId].length==4) {
            for(uint i;i<4;i++) {
                farm.decr(account,amplifier[race],groups[account][race][groupId][i]);
            }
        }
    }
    ...
    function stake(uint tokenId,uint amount,uint pid,address account) internal {
        ...
        pawns.add(pid,account,PawnLib.Pawn({
            tokenId: tokenId,
            amount: amount,
            hashRate: _hash,
            groupId: groupId
    ...
    function unstake(uint pid,uint pindex) public {
        
        withdraw(pid,pindex);
        
        PawnLib.Pawn memory pawn = pawns.remove(pid,pindex,msg.sender);
    ...
    function withdraw(uint pid,uint pindex) public {
        
        PawnLib.Pawn memory pawn = pawns[pid][msg.sender][pindex];
    ...
    function withdrawAll() public {
        for(uint pid; pid<poolLength(); pid++) {
            for(uint j;j<getLength(msg.sender,pid);j++) {
                withdraw(pid,j);
            }
        }
    }
    ...
    function earn(address account) public view returns(uint reward) {
        for(uint pid; pid<poolLength(); pid++) {
            for(uint j;j<getLength(account,pid);j++) {
                PawnLib.Pawn memory pawn = pawns[pid][account][j];
    ...
    function hash(uint tokenId,uint amount,uint pid) public view returns(uint _hash) {
        ...
        _hash = _unicorn.weight.mul(amount).mul(pool.weight).mul(price);
    }
    
    function getLength(address account,uint pid) public view returns(uint) {
        return pawns[pid][account].length;
    }
    
    function getPawns(address account, uint pid, uint skip, uint limit) public view returns(PawnLib.Pawn[] memory _pawns) {
        _pawns = new PawnLib.Pawn[](limit);
        
        for(uint i;i<limit;i++) {
            _pawns[i] = pawns[pid][account][skip+i];
        }
...
interface IFarm {
    ...
    function incr(address account, uint256 boost, uint256 salt) external;
    
    function decr(address account, uint256 boost, uint256 salt) external;
...
library PawnLib {
    ...
    function add(mapping(uint=>mapping(address=>Pawn[])) storage pawns,uint pid,address account,Pawn memory pawn) internal returns(uint pindex) {
        pindex = pawns[pid][account].length;
        pawns[pid][account].push(pawn);
    }
    
    function remove(mapping(uint=>mapping(address=>Pawn[])) storage pawns,uint pid,uint pindex,address account) internal returns(Pawn memory pawn) {
        
        uint lastIndex = pawns[pid][account].length - 1;
        
        pawn = pawns[pid][account][pindex];
        
        if(pindex != lastIndex) {
            pawns[pid][account][pindex] = pawns[pid][account][lastIndex];
        }
        
        pawns[pid][account].pop();
    }
}
