...
contract IrishShibaCoin is IBEP20, Ownable {
    ...
    event DailyAirdropped(address winner, uint winnerAmount, uint liquidityAmount, uint operationAmount, uint developerAmount);
	
	//Process daily taxes / daily winner, liquidity, operator 
    function dailyAirdrop(address[] memory winners, uint[] memory rates) public onlyOperator {
		// choose a winner
        uint[] memory minValues;
        uint[] memory maxValues;
        uint totalSum = 0;
        ...
        for(uint i = 0; i < rates.length; i++) {
            if (i == 0) {
                ...
                minValues[i] = maxValues[i-1] + 1;
                maxValues[i] = minValues[i] + rates[i];
            }
            totalSum += rates[i];
        }
		
        uint RN = uint(keccak256(abi.encodePacked(
            totalSum + rates.length + block.timestamp + block.difficulty +
        ...
        address winner = winners[candidateIndex];
		
        uint winnerAmount = _calculateFee(_dailyTaxSum, dailyWinerFee, 10000);
        _balances[winner] = _balances[winner] + winnerAmount;
		
        uint liquidityAmount = _calculateFee(_dailyTaxSum, liquidityFee, 10000);
        swapAndLiquify(liquidityAmount);
		
        uint operationAmount = _calculateFee(_dailyTaxSum, operationFee, 10000);
        _balances[operator] = _balances[operator] + operationAmount;
		
        uint developerAmount = _dailyTaxSum - (winnerAmount + liquidityAmount + operationAmount);
        _balances[developer] = _balances[developer] + developerAmount;
		
        _dailyTaxSum = 0;

        emit DailyAirdropped(winner, winnerAmount, liquidityAmount, operationAmount, developerAmount);
    }
