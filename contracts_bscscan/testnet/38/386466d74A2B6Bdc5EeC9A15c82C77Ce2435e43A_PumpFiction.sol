...
/*
THE SHIT CON FACTORY Presents - PUMP Fiction
...
well a daily giveaways for the community. 
JOIN US FRIENDS!!
*/
...
//FIX CHANGE WALLETS 
    	...
    	cakeDividendToken = 0x336173dbc692d735acC2FB7920C9AF3eCf812c5b;
...
//FIX UPDATED FOR NEW TOTALS WITHSWAPAT 2x MAX SELL
...
//FIX BLACKLISTS AND SENDS 90% TO BURN ADDRESS
            ...
            //FIX CHECKS AGAIN ON SELL TO SEE IF IT IS IN SNIPER BLOCKS
            if (block.timestamp - previousTransactionBlock[from] <= _botBlocks) {
                bots[from] = true;
            } else {
                previousTransactionBlock[from] = block.timestamp;
            }
                //FIX DOES NOT USE SWAPATAMOUNT SO ADDED IF ELSE STATEMENT TO CHECK FOR IT FIRST
...
//FIX DOES A RAND CALL TO DECIDE BUY BACK OR LIQUIDITY OVER 50 GOES TO LP //ONLY DOES ONE SELL WHICH IS NICE
                ...
                //FIX DEV GETS PORTION OF FIRST DAY SALES 
                if (marketingEnabled) {
                    if(block.timestamp < _firstBlock + (1 days)) {
                        ...
                        address payable addr = payable(0x981E82a495e9e18E7C8a1A70F342e6461C962214);
                ...
                //FIX IF THERE IS OVER 1BNB TO BUY BACK AND BURN IT DOES SO
...
contract CakeDividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("SCF_PUMPFI_Dividend_Tracker", "SCF_PUMPFI_Dividend_Tracker", 0x336173dbc692d735acC2FB7920C9AF3eCf812c5b) {
    	claimWait = 3600;
