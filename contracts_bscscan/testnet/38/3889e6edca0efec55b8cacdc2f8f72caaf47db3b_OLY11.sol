...
//        return amount.sub(feeAmount);
 //   }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        {greenwallTriggeredAt = block.timestamp;} //greenwall2
...
//    }
    ...
    function setFees(uint256 _totalBuyFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _feeDenominator, uint256 _totalSellFee) external authorized {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        totalBuyFee = _totalBuyFee; //liquidityFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee);
