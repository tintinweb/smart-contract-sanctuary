...
contract FLOKISKY is ERC20Detailed, Ownable {
  ...
  // TOTAL_FSKY is a multiple of INITIAL_FRAGMENTS_SUPPLY so that _FSKYPerFragment is an integer.
  // Use the highest value that fits in a uint256 for max granularity.
  uint256 private constant TOTAL_FSKY = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);

  // MAX_SUPPLY = maximum integer < (sqrt(4*TOTAL_FSKY + 1) - 1) / 2
  uint256 private constant MAX_SUPPLY = ~uint128(0);  // (2^128) - 1

  uint256 private _totalSupply;
  uint256 private _FSKYPerFragment;
  mapping(address => uint256) private _FSKYBalances;
  ...
  ERC20Detailed("FLOKISKY", "FSKY", uint8(DECIMALS))
  payable
  public
  {
    ...
    _FSKYBalances[msg.sender] = TOTAL_FSKY;
    _FSKYPerFragment = TOTAL_FSKY.div(_totalSupply);
...
*/
  ...
  returns (uint256)
  {
    return _FSKYBalances[who].div(_FSKYPerFragment);
  }
  ...
  function _transferStandard(address sender, address recipient, uint256 amount) private {
    ...
    uint256 FSKYDeduct = amount.mul(_FSKYPerFragment);
    uint256 FSKYValue = tTransferAmount.mul(_FSKYPerFragment);
    _FSKYBalances[sender] = _FSKYBalances[sender].sub(FSKYDeduct);
    _FSKYBalances[recipient] = _FSKYBalances[recipient].add(FSKYValue);
    _takeFee(sender, tFee);
    emit Transfer(sender, recipient, amount);
  }

  function _transferExcluded(address sender, address recipient, uint256 amount) private {
    uint256 FSKYValue = amount.mul(_FSKYPerFragment);
    _FSKYBalances[sender] = _FSKYBalances[sender].sub(FSKYValue);
    _FSKYBalances[recipient] = _FSKYBalances[recipient].add(FSKYValue);
    emit Transfer(sender, recipient, amount);
  }
  ...
  function _takeFee(address sender, uint256 tFee) private {
    uint256 rFee = tFee.mul(_FSKYPerFragment);
    _FSKYBalances[address(this)] = _FSKYBalances[address(this)].add(rFee);
    emit Transfer(sender, address(this), tFee);
  }

  function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
    // split the contract balance into quarters
    uint256 fourFifth = contractTokenBalance.mul(5).div(5);
    ...
    uint256 sharedETH = newBalance.div(5);

    // add liquidity to uniswap
    addLiquidity(oneFifth, sharedETH);

    // Transfer to marketing address
    transferToAddressETH(marketingAddress, sharedETH.mul(5));
