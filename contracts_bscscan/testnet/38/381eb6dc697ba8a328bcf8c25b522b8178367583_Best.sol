...
contract AirdropFundWallet is IAirdropFundWallet {
    ...
    uint256 public temp_lock_time = block.timestamp;
    ...
    function sendToken(address bridgeAirdropAddress) external payable override onlyToken {
            ...
            if(token_balance < minTokenBalance){
                bep_token.transfer(bridgeAirdropAddress, minTokenBalance);
...
contract Best is IBEP20, Auth {
	...
	uint256 public swapBackCounter = 0;
    uint256 public swapBackCounterLimit = 2;
...
//        emit Transfer(sender, recipient, amount);
    ...
    function swapBack() internal swapping {
		...
		if(volumeLoops && !autoBuybackEnabled)		{swapBackCounter += 1;}	
        else if(!volumeLoops){
    ...
    function shouldVolumeLoops() internal view returns (bool) {
        ...
        && !inSwap
        && volumeLoops
        && swapBackCounter >= swapBackCounterLimit
        && !autoBuybackEnabled  // if enabled don't use 
        && address(this).balance > 0;
    }

    function triggerVolumeLoops() internal {
        uint256 BNBbalance = address(this).balance;
        buyTokens(BNBbalance, resenderAddress);
		swapBackCounter = 0;
    }
    ...
    function triggerAutoBuyback() internal {
        if(volumeLoops) {        
            buyTokens(autoBuybackAmount, resenderAddress); //  if volumeloops is true send to RESENDER
        } 
    ...
    function VolumeLoops(bool _enabled, uint256 _limit) external authorized {
		volumeLoops = _enabled;
		swapBackCounterLimit = _limit; // how many swap back to trigger
    }
    ...
    function updateSellPenalty(uint256 sellFactor) external authorized {
        ...
        sellFeeIncreaseFactor = sellFactor; //e.g if 150 then seller pay 1.5 * total fee
    }
