...
contract ArteOriginMintableStore is Ownable, SignerRole, StoreBase {
    string public name;
    string public symbol;

    IERC20 public ArteToken;
    uint public mintFee;
    ...
    function setMintFee( uint fee) public onlyOwner {
        mintFee = fee;
    }
    ...
    function deductFee( address minter) private returns(bool) {
        require(ArteToken.balanceOf(minter) >= mintFee, "insufficient balance to pay mint fees");
        require(ArteToken.allowance(minter, address(this)) >= mintFee, "insufficient allowance to pay mint fees");

        require(ArteToken.transferFrom(minter, owner(), mintFee), "mint fee transfer failed");
        return true;
    } 
