...
contract IDOFactory is Whitelist {
    ...
    ) external onlyOwner whenNotPaused onlyWhitelist(_token) {
        address newIDO;
        if (_allowances.length > 0) {
            newIDO = address(
                new IDOProxy(implementations[idoAllowanceId], owner())
            );
            IIDOAllowance(newIDO).initialize(
                _minDeposit,
                _maxDeposit,
    ...
    function setIdoId(uint256 id) public onlyOwner {
        idoId = id;
    }

    function setIdoAllowanceId(uint256 id) public onlyOwner {
        idoAllowanceId = id;
    }

    function addImplementation(address _address) public onlyOwner {
...
contract IDOProxy is UpgradableProxy, Ownable {
    constructor(address _implementation, address owner)
        UpgradableProxy(_implementation)
    {
...
interface IIDO is IAccessControlUpgradeable {
