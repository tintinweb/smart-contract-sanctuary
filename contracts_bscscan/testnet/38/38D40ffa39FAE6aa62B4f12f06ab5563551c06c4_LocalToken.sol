...
contract Staking2local is Ownable {
    ...
    function pendingLocal(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (user.amount > 0) {
            ...
            if (currentTime > user.userRewardInfo[user.lastPaidIndex].canHarvestTimestamp) {
                for (uint256 i = user.lastPaidIndex; i < user.userRewardInfo.length; i++ ) {
                    ...
                    if (currentTime > rewardInfo.canHarvestTimestamp) {
                        pending = pending.add(rewardInfo.amount.add(rewardInfo.amount.mul(rewardInfo.bonusRate).div(100)));
                        // rewardInfo.isPaid = true;
                        // user.amount = user.amount.sub(rewardInfo.amount);
                        // user.lastPaidIndex = i;
                    }
                }
            }
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        // uint256 pending = pendingLocal(_pid, msg.sender);
        // if(pending > 0) {
        //     safeLocalTransfer(msg.sender, pending);
        // }
        // if(_amount > 0) {
         ...
         if (user.amount > 0) {
            ...
            for (uint256 i = user.lastPaidIndex; i < user.userRewardInfo.length; i++ ) {
                ...
                if (sumAmount < _amount){
                    if (currentTime > rewardInfo.canHarvestTimestamp) {
                        pending = pending.add(rewardInfo.amount.mul(rewardInfo.bonusRate).div(100));
                    }
                    rewardInfo.amount = 0;
                    rewardInfo.isPaid = true;
                    user.amount = user.amount.sub(rewardInfo.amount);
                    ...
                    if (currentTime > rewardInfo.canHarvestTimestamp) {
                        pending = pending.add(rewardInfo.amount.sub(restAmount).mul(rewardInfo.bonusRate).div(100));
                    }

                    user.amount = user.amount.sub(restAmount);
                    break;
                    // lastPaidIndex = i;
                }
            }
            user.lastPaidIndex = lastPaidIndex;
