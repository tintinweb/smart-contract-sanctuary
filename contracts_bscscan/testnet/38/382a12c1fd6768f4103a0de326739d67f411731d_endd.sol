...
contract DividendDistributor is IDividendDistributor {
    ...
    function lockTokens(address shareholder, uint256 lockTime) internal {
        ...
        else if(lockTime > localUpperLimiter ){ localUpperLimiter = upperLimiter; }
    ...
    function setShare(address shareholder, uint256 amount) external override onlyToken {       
        if(amount >= minTokens && shares[shareholder].amount == 0){
    ...
    function getTotalShares() internal view returns (uint256) {
        ...
        for (uint256 i = 0; i < shareholders.length; i++) {
            total += shares[shareholders[i]].amount;//sprawdz czy wzial dyvidende jak tak to nie licz to udzalu
        }
        return total;
    }
    ...
    function getShareholderInfo(address shareholder, bool hideDecimal) public view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        Share memory s = shares[shareholder];
        TokensLocker memory l = locker[shareholder]; 
        if(hideDecimal){ return (
            s.amount.div(10 ** 18), 
            s.totalRealised.div(10 ** 18), 
            forecastDividend(shareholder).div(10 ** 18), 
            l.tokensLocked.div(10 ** 18), 
            l.unlockTime, 
            l.tokensUnlocked.div(10 ** 18)
            );
        }
        else return (
            s.amount, 
...
//        emit Transfer(sender, recipient, amount);
    ...
    function getDistributorInfo(bool hideDecimal) public view returns (uint256 totalShares, uint256 totalLockedTokens, uint256 dividendPool, uint256 totalDistributed, uint256 dividendsPerShare, uint256 minTokens, uint256 annualMulipler, uint256 penaltyPercentage, uint256 shareholderCount) {
