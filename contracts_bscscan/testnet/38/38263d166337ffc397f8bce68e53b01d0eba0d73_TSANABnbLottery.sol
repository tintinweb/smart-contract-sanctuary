...
// File: contracts/BnbLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title TSANA Bnb Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract TSANABnbLottery is ReentrancyGuard, IBnbLottery, Ownable {
    ...
    event SaunaSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
     ...
     */
    constructor(address _saunaTokenAddress, address _randomGeneratorAddress) {
        ...
        nonReentrant
    {
        ...
        require(msg.value >= amountBnbToTransfer, "Insufficient BNB balance");
    ...
    function claimTickets(
        ...
        receiver.transfer(rewardInBnbToTransfer);
        ...
        nonReentrant
    {
        ...
        if (feeMethod == 0) {
            // Transfer BNB to marketing address
            payable(marketingAddress).transfer(amountToWithdrawToTreasury);    
        } else if (feeMethod == 1) {
            // Add to liquidity
            require(address(saunaSwapRouter) != address(0), "Invalid Sauna Swap Router");
            require(address(saunaBnbPair) != address(0), "Invalid SAUNA-BNB Pair");

            uint256 half = amountToWithdrawToTreasury.div(2);
            uint256 otherHalf = amountToWithdrawToTreasury.sub(half);

            uint256 initialSaunaBalance = saunaToken.balanceOf(address(this));

            // swap bnb to sauna
            swapBnbToSauna(half);
            uint256 newSaunaBalance = saunaToken.balanceOf(address(this)).sub(initialSaunaBalance);

            // add liquidity
            addLiquidity(newSaunaBalance, otherHalf);
        } 
     ...
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override payable onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");
        require(msg.value >= _amount, "Insufficient amount");

        _lotteries[_lotteryId].amountCollectedInBnb += _amount;
    ...
    /// @dev Swap bnb to sauna
    function swapBnbToSauna(uint256 bnbAmount) private {
        // generate the saunaSwap pair path of bnb -> sauna
        address[] memory path = new address[](2);
        path[0] = saunaSwapRouter.WETH();
        path[1] = address(saunaToken);

        // make the swap
        saunaSwapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: bnbAmount}(
            0, // accept any amount of SAUNA
            path,
            address(this),
            block.timestamp
        );
    ...
    /// @dev Add liquidity
    function addLiquidity(uint256 saunaAmount, uint256 bnbAmount) private {
        // approve token transfer to cover all possible scenarios
        saunaToken.approve(address(saunaSwapRouter), saunaAmount);
        
        // add the liquidity
        saunaSwapRouter.addLiquidityETH{value: bnbAmount}(
            address(saunaToken),
            saunaAmount,
    ...
    function updateSaunaSwapRouter(address _router) public onlyOwner {
        ...
        saunaBnbPair = IUniswapV2Factory(saunaSwapRouter.factory()).getPair(address(saunaToken), saunaSwapRouter.WETH());
        require(saunaBnbPair != address(0), "Invalid SAUNA-BNB pair address.");
        emit SaunaSwapRouterUpdated(msg.sender, address(saunaSwapRouter), saunaBnbPair);
    }
