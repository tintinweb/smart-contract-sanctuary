...
import "./access/GovernanceAgent.sol";
import "./token/MFCToken.sol";
import "./treasury/Treasury.sol";

contract MFCMembership is Treasury, AdminAgent, BackendAgent, WithdrawAgent, GovernanceAgent {
  ...
  constructor(
    ...
    BackendAgent(adminAgents, backendAgents)
    WithdrawAgent(busdContractAddress_, adminAgents, withdrawAgents)
    GovernanceAgent(governanceAddress_) {
  ...
  function setReferralLoanAddress(address destination) external onlyAdminGovAgents {
  ...
  function restoreMember(address account, address inviterAddress) external onlyBackendAgents {
    require(!_memberExist(account), "Member already exist");

    _createMember(account, inviterAddress);

    emit RestoreMember(account, inviterAddress, block.timestamp);
  }
...
contract BackendAgent is Context {
  ...
  constructor(
    ...
    for (uint i = 0; i < backendAgents_.length; i++) {
      _backendAgents[backendAgents_[i]] = true;
    }
  }
...
contract GovernanceAgent is Context {
...
contract MFCExchange is Context, ExchangeCheck {
  ...
  function tradeOffer(uint256 id, address seller, uint256 quantity) external onlyValidTradeOffer(id, seller, quantity) onlyValidMember(_msgSender()) {
    _tradeOffer(id, seller, quantity);
  }
  ...
  function _tradeOffer(uint256 id, address seller, uint256 quantity) private {
    TradingPair memory tradingPair = _tradingPairs[_offers[seller][id].tradingPair];
    ...
    uint256 buyQuantity = _executeTrade(tradingPair, seller, quantity, _offers[seller][id].price);
  ...
  function _executeTrade(TradingPair memory tradingPair, address seller, uint256 quantity, uint256 price) private returns (uint256) {
...
contract BUSDT is Treasury, AdminAgent, WithdrawAgent, GovernanceAgent {
  ...
  constructor(
    ...
    GovernanceAgent(governanceAddress) {
    for (uint i = 0; i < adminGovAgents.length; i++) {
      _adminGovAgents[adminGovAgents[i]] = true;
    }
  }
