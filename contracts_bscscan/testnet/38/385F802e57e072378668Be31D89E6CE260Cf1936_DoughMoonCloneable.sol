...
contract DoughMoonCloneable is DoughTokenCloneable  {
    ...
    uint256 private lastBurnAmount;
    uint256 private lastMarketingAmount;
    uint256 private lastContractAmount;
    ...
    function __DoughToken_init (SharedStructures.TokenInfo memory _tokenInfo) internal override { 
        ...
        lastBurnAmount = balance;

        address _marketingWallet = addressInfo.marketingWallet;

        // Record balance of marketing wallet (if not msg.sender)
        if (msg.sender != _marketingWallet) {
            balance = ERC20Upgradeable.balanceOf (_marketingWallet);
            tBalances[_marketingWallet] = balance;
            rBalances[_marketingWallet] = balance * currentRate;
            lastMarketingAmount = balance;
        }
    ...
    function postTransferActions (address from, address to, uint256 transferFromAmount, uint256 transferToAmount) internal override {
        ...
        // Increment fee wallet holdings based on change from last transfer
        uint256 balance = ERC20Upgradeable.balanceOf (BURN_ADDRESS);
        tBalances[BURN_ADDRESS] += (balance - lastBurnAmount);
        lastBurnAmount = balance;

        address _marketingWallet = addressInfo.marketingWallet;
        balance = ERC20Upgradeable.balanceOf (_marketingWallet);
        tBalances[_marketingWallet] += (balance - lastMarketingAmount);
        lastMarketingAmount = balance;

        balance = ERC20Upgradeable.balanceOf (address(this));
        tBalances[address(this)] += (balance - lastContractAmount);
        lastContractAmount = balance;

        // Calculate new reflective balance based on the changes above
        rBalances[BURN_ADDRESS] = tBalances[BURN_ADDRESS] * currentRate;
        rBalances[_marketingWallet] = tBalances[_marketingWallet] * currentRate;
        rBalances[address(this)] = tBalances[address(this)] * currentRate;
        ...
        // Calculate new balances based on reflection
        tBalances[BURN_ADDRESS] = tokenFromReflection (rBalances[BURN_ADDRESS]);
        tBalances[_marketingWallet] = tokenFromReflection (rBalances[_marketingWallet]);
        tBalances[address(this)] = tokenFromReflection (rBalances[address(this)]);
