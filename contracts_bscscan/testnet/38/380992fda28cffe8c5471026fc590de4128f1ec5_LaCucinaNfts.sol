...
contract LaCucinaNfts is BaseERC721WithRoyalties, ReentrancyGuardUpgradeable, ERC1155ReceiverUpgradeable {
	...
	*/
	...
	// itemId => item Hash => exists or not
	...
	function generateItem(
		...
		require(_itemId > 0 && _itemId <= traitFactory.getCurrentItemId(), 'ERR5');
		require(_seriesId > 0 && _seriesId <= traitFactory.currentSeries(_itemId), 'ERR6');
		require(traitFactory.isNftGenerationEnabled(_itemId, _seriesId), 'ERR4');
 ...
 */
		...
		returns (string memory Svg)
	{
		...
		for (uint8 slot = 0; slot < item.totalTraits; slot++) {
			...
			if (genSlottedValue > 0) {
				...
				Svg = TalienUtils.strConcat(Svg, variationSvg);
			}
		}
		...
		if (item.totalAccessoryTypes > 0) {
			for (
				...
				if (accessoryIds[_tokenId][accessorytType] != 0) {
					...
					Svg = TalienUtils.strConcat(Svg, _svg);
				}
			}
		}
		...
		if (isNumberedNFT) {
			Svg = TalienUtils.strConcat(Svg, traitFactory.getSvgNumber(_tokenId));
		}

		// show badge
		string memory badge = traitFactory.getSvgBadge(item.likes);

		if (bytes(badge).length > 0) Svg = TalienUtils.strConcat(Svg, badge);

		// show total likes
		if (item.likes > 0)
			Svg = TalienUtils.strConcat(Svg, traitFactory.getSvgLikes(item.likes));

		Svg = TalienUtils.strConcat(Svg, '</svg>');
	}
	...
	function _generateItem(
		...
		(, , , uint256 maxNfts, uint256 totalNftsMinted, , uint8 totalTraits) = traitFactory
			.seriesDetails(_itemId, _seriesId);

		require(totalNftsMinted <= maxNfts, 'ERR18');

		uint256 traitVariationHash = _getHash(_itemId, _seriesId, totalTraits);
	...
	function _getAccessories(
		...
		require(_itemId <= accessories.getCurrentItemId(), 'ERR20');
	...
	function _getHash(
		...
		for (uint8 i = 0; i < totalTraits; i++) {
			uint256 traitId = traitFactory.getSeriesTraitId(_itemId, _seriesId, i);
	...
	function _getRandomAccessory(
		...
		for (uint256 i = 1; i < _totalAccessories; ++i) {
			(, , , , , , probability) = accessories.accessories(
				accessories.getAccessoryId(_itemId, _seriesId, _accessoryTypeId, i)
			);
	...
	function _getRandomTraitVariation(
		...
		for (uint256 i = 1; i < _totalVariations; ++i) {
			(, , , , , probability) = traitFactory.traitVariations(
				traitFactory.getVariationsId(_itemId, _seriesId, _traitId, i)
			);
...
interface ITraitFactory {
	...
	function isNftGenerationEnabled(uint256 _itemId, uint256 _seriesId)
		external
		view
		returns (bool);
...
interface IAccessories is IERC1155 {
	...
	function totalAccessoryTypes(uint256 itemId)
		external
		view
