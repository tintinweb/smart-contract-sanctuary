...
contract TransferHelper is Ownable {
    ...
    event TransferFrom(address from, address to, uint256 amount, bool success);
    event BatchTransfer(address from, uint256 num, uint256 gasCost, uint256 gasFee);
    ...
    function batchTransfer(address _token, address _from, address[] calldata _tos, uint256[] calldata _amounts, bool _ignoreError) external payable {
        ...
        for(uint i = 0; i < _tos.length; i ++) {
            ...
            emit TransferFrom(_from, _tos[i], _amounts[i], success);
            if (!success) {
                if (_ignoreError) {
                    continue;
                } else {
        ...
        emit BatchTransfer(_from, _tos.length, gasBefore.sub(gasAfter).mul(gasPrice), fee);
    }
