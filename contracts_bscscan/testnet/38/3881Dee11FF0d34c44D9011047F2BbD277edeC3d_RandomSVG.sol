...
contract RandomSVG is ERC721URIStorage, VRFConsumerBase {
    ...
    function finishMint(uint256 _tokenId) public {
        // check to seef if it's been minted and a random number is returned
        // generate random SVG code
        // turn that into an imageURI
        // use that imageURI to format the tokenURI
        require(
            bytes(tokenURI(_tokenId)).length <= 0,
            "tokenURI is already all set!"
        );
        require(tokenCounter > _tokenId, "TokenId has nog been minted yet!");
        require(
