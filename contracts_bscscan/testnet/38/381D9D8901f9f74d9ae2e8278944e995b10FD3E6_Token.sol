...
import { Minter } from "./abstract/Minter.sol";
import { TransactionThrottler } from "./abstract/TransactionThrottler.sol";
import { Constants } from "./libraries/Constants.sol";

contract Token is IERC20, EIP712, Ownable, Minter, TransactionThrottler {
    ...
    function burnFrom(address account, uint256 amount) external onlyMinter returns (bool) {
    ...
    function changeFeeContract(address newContract, bool _isFeeManager) external onlyOwner {
...
abstract contract Minter is Ownable, MinterData {
    event MinterAdded(address indexed minterAdded);
    event MinterRevoked(address indexed minterRevoked);
    ...
    function initMinter(address _minter) external onlyOwner {
        require(_init, "init already done");
    ...
    function applyMinter() external onlyOwner {
        require(pendingMinter != address(0), "zero address");
        require(block.timestamp >= delayMinter, "soon");
        isMinter[pendingMinter] = true;
        minters.push(pendingMinter);
        emit MinterAdded(pendingMinter);
        pendingMinter = address(0);
    }

    // No time delay for revoke minter emergency function
    function revokeMinter(address _revoked) external onlyOwner {
        isMinter[_revoked] = false;
        emit MinterRevoked(_revoked);
    }
