...
contract LuckyStaking {
    ...
    function removeStake(uint256 percentage) public{
        ...
        uint256 amountToRemove=((percentage)*stake[msg.sender]);
        removeStakeHistory.push(amountToRemove);
        removeStakeHistoryAddress.push(msg.sender);
        removeStakeHistoryTS.push(block.number);
        uint256 amountToRemovePostFees=amountToRemove-((amountToRemove)*(ownerFee+dividendFee));
        uint256 devFee=((amountToRemove)*ownerFee);
        CNR.transfer(owner,devFee/100);
        CNR.transfer(msg.sender,amountToRemovePostFees/100);
