...
contract AlpacaTranchingController is Ownable {
    ...
    uint256 constant frozenBlockCountAfterDeposit = 20;
    ...
    struct UserWallet {
        ...
        int256 principal;
        int256 reward;
        uint256 farmingReward; // TGT farming reward.
    ...
    constructor(
        ...
        lastFarmingRewardBlock = _farmingStartBlock;
    ...
    function deposit(InvestorType _investorType, uint256 _amount) external {
        // distribute all rewards before deposit.
        distributeAllReward();
        ...
        userWallet.principal += int256(_amount);
        userWallet.lastDepositBlockNumber = block.number;

        compoundAll();
    }

    // Withdraw partial amount.
    function withdraw(InvestorType _investorType, uint256 _amount) external {
        require(
            userWalletExists(msg.sender, _investorType),
        ...
        // distribute all rewards before withdraw.
        distributeAllReward();

        withdrawFromWallet(userWallet, int256(_amount));
    }

    // Withdraw all principal and reward.
    function withdrawAll(InvestorType _investorType) external {
        ...
        // Compute all amount that belongs to this investor.
        int256 allAmount = userWallet.principal + userWallet.reward;
        withdrawFromWallet(userWallet, allAmount);
    }

    // Get total equity by investor type
    function getTotalEquity(InvestorType _investorType)
        external
        view
        returns (int256)
    {
        int256 totalAmount;
        int256 totalSuperiorAmount;
        int256 totalInferiorAmount;
        ...
        int256 newTotalReward = totalAmount -
            totalSuperiorAmount -
            totalInferiorAmount;

        // If newTotalReward is non-negative, we will distribute the profit according to pre-defined multipliers.
        // Otherwise we will let inferior investors to pay the loss.
        int256 effectiveSuperiorMultiplier = int256(superiorMultiplier);
        int256 effectiveInferiorMultiplier = int256(inferiorMultiplier);
        if (newTotalReward < 0) {
            effectiveSuperiorMultiplier = 0;
            effectiveInferiorMultiplier = 1;
        }

        int256 totalWeights = totalSuperiorAmount *
        ...
        if (_investorType == InvestorType.SUPERIOR) {
            return totalSuperiorAmount;
        } else {
            return totalInferiorAmount;
        }
    }
    ...
    function getAccountDetails(address _investor, InvestorType _investorType)
        ...
        returns (
            int256 principal,
            int256 reward,
            uint256 unblockDepositBlockNumber
        )
    {
        ...
        UserWallet storage userWallet = getUserWallet(_investor, _investorType);

        return (
            userWallet.principal,
            userWallet.reward,
            userWallet.lastDepositBlockNumber + frozenBlockCountAfterDeposit
        );
    }

    // Compound all asset by calling external contract ABI.
    // This is costly an anyone who wants to pay gas can execute it.
    ...
    // Distribute all rewards to each investor's wallets.
    function distributeAllReward() private {
        distributeInvestmentReward();
        distributeFarmingReward();
    }

    // Distribute investment rewards to each investor's wallets.
    function distributeInvestmentReward() private {
        int256 totalAmount;
        int256 totalSuperiorAmount;
        int256 totalInferiorAmount;
        ...
        if (totalWeights > 0) {
            // Distribute new total reward to all wallets according to superior and inferior multiplier.
            for (uint256 wid = 0; wid < userWallets.length; ++wid) {
                ...
                if (userWallet.investorType == InvestorType.SUPERIOR) {
                    userWallet.reward +=
                        ((userWallet.principal + userWallet.reward) *
                            effectiveSuperiorMultiplier *
                            newTotalReward) /
                        totalWeights;
                } else {
    ...
    function distributeFarmingReward() private {
        ...
        for (uint256 wid = 0; wid < userWallets.length; ++wid) {
            UserWallet storage userWallet = userWallets[wid];
            int256 userEquity = userWallet.principal + userWallet.reward;
            if (userEquity >= 0) {
                totalEquity += uint256(userEquity);
            }
        }
            ...
            for (uint256 wid = 0; wid < userWallets.length; ++wid) {
                ...
                if (userEquity >= 0) {
                    userWallet.farmingReward +=
                        (uint256(userEquity) * totalFarmingRewards) /
                        totalEquity;
                }
            }
        }
    ...
    // Validate if user can withdraw such amount out of the fund.
    function validateWithdrawal(UserWallet storage _userWallet, int256 _amount)
        private
        view
    {
        require(_amount > 0, "The amount to withdraw is not valid.");

        require(
            _userWallet.principal + _userWallet.reward >= _amount,
            "You have no enough amount in this fund to withdraw."
        );

        require(
            _userWallet.lastDepositBlockNumber + frozenBlockCountAfterDeposit <
                block.number,
            "You cannot withdraw right after deposit, please wait some time and try again."
        );

        //TODO: We should have many other validations.
    }

    function withdrawFromWallet(UserWallet storage _userWallet, int256 _amount)
        private
    {
        validateWithdrawal(_userWallet, _amount);

        withdrawExternalEquity(_amount);

        // Transfer the amount from current contract to this investor.
        principalToken.transfer(msg.sender, uint256(_amount));

        // Update our contract state for this user wallet.
        // Reset all principal and reward amount.
        _userWallet.principal =
            _userWallet.principal +
            _userWallet.reward -
            _amount;
        _userWallet.reward = 0;

        if (_userWallet.farmingReward > 0) {
            // Transfer TGT rewards from current contract to this investor.
            tgtToken.transfer(msg.sender, _userWallet.farmingReward);

            // Reset farming reward.
            _userWallet.farmingReward = 0;
        }
    ...
    function compoundAllAsset() private {
        //TODO: compound all asset into Alpaca.
        // 1. Get all principalToken in this contract.
        // 2. Invoke work method to farm (with loan).
    }

    function getEquityOverview()
        private
        view
        returns (
            int256 externalEquity,
            int256 distributedSuperiorEquity,
            int256 distributedInferiorEquity,
            uint256 remainingTgt
        )
    {
        // Get external equity for this fund.
        int256 totalAmount = getExternalEquity();

        // Calculate distributed equity.
        int256 totalSuperiorAmount = 0;
        int256 totalInferiorAmount = 0;
        for (uint256 wid = 0; wid < userWallets.length; ++wid) {
            UserWallet storage userWallet = userWallets[wid];
            if (userWallet.investorType == InvestorType.SUPERIOR) {
                totalSuperiorAmount += userWallet.principal + userWallet.reward;
            } else {
                totalInferiorAmount += userWallet.principal + userWallet.reward;
            }
        }

        return (totalAmount, totalSuperiorAmount, totalInferiorAmount, 0);
    }

    function getExternalEquity() private view returns (int256) {
        //TODO: get all equity from Alpaca. Note this should equal to asset - liability.
        // For testing purpose it's just as total amount of deposit.
        int256 totalAmount = 0;
        for (uint256 wid = 0; wid < userWallets.length; ++wid) {
            UserWallet storage userWallet = userWallets[wid];
            totalAmount += userWallet.principal + userWallet.reward;
        }
        return totalAmount;
    }

    function withdrawExternalEquity(int256 _amount) private {
        //TODO: withdraw corresponding amount of equity from Alpaca.
    }
     ...
     */
    function convertPrincipalAmountToLoanAmount(int256 _principalAmount)
        private
        view
        returns (int256)
    {
