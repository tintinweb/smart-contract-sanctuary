...
contract BetswampMVP is context {
    ...
    function _placeBet(
        ...
        if (bets[event_id][msg.sender] == true) {
            ...
            userBets[event_id][msg.sender].amount = userBets[event_id][
                msg.sender
            ].amount.add(_amount);
    ...
    function getUserEventHistory() public view returns (uint256[] memory) {
        ...
        _wallet_validation_points[msg.sender] = _wallet_validation_points[
            msg.sender
                ...
                (_validator_lock_amount[msg.sender] *
                    (block.timestamp -
                        _validator_wallet_lock_time[msg.sender])) / 100000
            ); // calculate point
    ...
    */
    function _reclaimBettingWager(uint256 event_id, address user_address) private {
        ...
        require(events[event_id].startTime < block.timestamp, "Event time not elapse.");
