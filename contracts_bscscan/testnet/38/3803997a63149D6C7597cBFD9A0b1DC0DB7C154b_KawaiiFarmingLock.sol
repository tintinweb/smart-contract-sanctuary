...
contract KawaiiFarmingLock is SignData {
    ...
    function _deposit(address _caller, uint256 _amount) internal {
        ...
        IBEP20(kawaiiToken).safeTransferFrom(_caller, address(this), _amount);
        ...
        if (userData.epoch != epochNow) {
            ...
            totalLock = totalLock.sub(userData.amount);
            userData.amount = 0;

        }

        userData.epoch = epochNow;
        userData.amount = userData.amount.add(_amount);
        totalLock = totalLock.add(_amount);

        emit Deposit(_caller, epochNow, _amount);
    }
    ...
    function withdrawAll(address _caller) public {
        ...
        uint256 totalAmount = reward.add(userData.amount);
