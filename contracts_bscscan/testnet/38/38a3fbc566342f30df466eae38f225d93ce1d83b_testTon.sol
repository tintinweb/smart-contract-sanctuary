...
contract testTon is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress;
    address payable public devAddress;
    address payable public communityPrizeAddress;
    ...
    uint256 private constant _tTotal = 1337 * 1e6 * 1e18;
    ...
    uint256 public _buyCommunityPrizeFee = 50;
    ...
    uint256 public _sellCommunityPrizeFee = 100;
    ...
    uint256 public _communityPrizeTokensToSwap;
    ...
    event UpdatedCommunityPrizeAddress(address communityPrize);
    ...
    constructor() {
        address newOwner = address(0x59a395CAAA08847a8631ECe8b6e2C0756a2a4199);
        ...
        marketingAddress = payable(0x59a395CAAA08847a8631ECe8b6e2C0756a2a4199);
        devAddress = payable(0x59a395CAAA08847a8631ECe8b6e2C0756a2a4199);
        communityPrizeAddress = payable(0x59a395CAAA08847a8631ECe8b6e2C0756a2a4199);
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            // Buy
                ...
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyDevFee + _buyCommunityPrizeFee;
                if(_liquidityFee > 0){
                    buyOrSellSwitch = BUY;
                }
            } 
            // Sell
                ...
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee + _sellDevFee + _sellCommunityPrizeFee;
                if(_liquidityFee > 0){
                    buyOrSellSwitch = SELL;
                }
            }
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _marketingTokensToSwap + _devTokensToSwap + _communityPrizeTokensToSwap;
        bool success;
        ...
        uint256 ethForCommunityPrize = ethBalance.mul(_communityPrizeTokensToSwap).div(totalTokensToSwap);
        ...
        (success,) = address(communityPrizeAddress).call{value: ethForCommunityPrize}("");
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(buyOrSellSwitch == BUY){
            ...
            _communityPrizeTokensToSwap += tLiquidity * _buyCommunityPrizeFee / _liquidityFee;
            ...
            _communityPrizeTokensToSwap += tLiquidity * _sellCommunityPrizeFee / _liquidityFee;
        }
        uint256 currentRate = _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyDevFee, uint256 buyCommunityPrizeFee)
        external
        onlyOwner
    {
        ...
        _buyCommunityPrizeFee = buyCommunityPrizeFee;
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyDevFee + _buyCommunityPrizeFee <= 1000, "Must keep buy taxes below 10%");
        emit SetBuyFee(buyMarketingFee, buyLiquidityFee, buyTaxFee);
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellDevFee, uint256 sellCommunityPrizeFee)
        external
        onlyOwner
    {
        ...
        _sellCommunityPrizeFee = sellCommunityPrizeFee;
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellDevFee + _sellCommunityPrizeFee <= 2000, "Must keep sell taxes below 20%");
    ...
    function setCommunityPrizeAddress(address _address) external onlyOwner {
        require(_address != address(0), "_marketingAddress address cannot be 0");
        _isExcludedFromFee[communityPrizeAddress] = false;
        communityPrizeAddress = payable(_address);
        _isExcludedFromFee[communityPrizeAddress] = true;
        emit UpdatedCommunityPrizeAddress(_address);
    }
