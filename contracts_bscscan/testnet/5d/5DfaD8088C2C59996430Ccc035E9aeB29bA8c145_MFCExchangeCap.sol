...
import "./treasury/MFCT.sol";
import "./MFCExchangeFloor.sol";

contract MFCExchangeCap is MFCT {
  ...
  event CreateOffer(uint256 id, uint256 quantity, uint256 price, uint256 expiresAt, uint256 timestamp);
  ...
  function createOffer(uint256 quantity, uint256 price) external onlyPrivilegedAgents {
    ...
    uint256 id = _nonce++;
    _offers[id] = Offer(id, quantity, price, expiresAt, true);
    _mfcAllocatedInOffers += quantity;
    emit CreateOffer(id, quantity, price, expiresAt, block.timestamp);
  }
  ...
  function closeOffer(uint256 id) external onlyPrivilegedAgents {
    require(_isOfferActive(id), "Invalid offer");
    _closeOffer(id);
  }

  function _isOfferActive(uint256 id) private view returns (bool) {
    return _offers[id].isOpen && _offers[id].expiresAt > block.timestamp;
  }
...
contract MFCExchangeFloor is Context {
  ...
  event TradeOffer(address buyer, uint256 sellerQuantity, uint256 buyerQuantity, uint256 timestamp);
  ...
  function getPrice() external view returns (uint256) {
    return _getAmountOut(MULTIPLIER);
  }
  ...
  function tradeOffer(uint256 quantity, uint256 minimumOut) external {
    ...
    _mfc.transferFrom(_msgSender(), _mfcTreasuryAddress, quantity);
    ...
    emit TradeOffer(_msgSender(), amountOut, quantity, block.timestamp);
  }
  ...
  function _getAmountOut(uint256 quantity) private view returns (uint256) {
    ...
    if (_mfcCirculation > 0) {
      ...
      amountOut = quantity * _initialPrice / MULTIPLIER;
    }
...
contract BUSDT is Treasury {

  constructor(
    address tokenContractAddress,
    address[] memory treasurers
  ) Treasury(tokenContractAddress, treasurers) {
  }

  function withdrawTo(address recipient, uint256 amount) external onlyPrivilegedAgents {
    require(getTreasuryToken().balanceOf(address(this)) >= amount, "Insufficient balance");
    getTreasuryToken().transfer(recipient, amount);
    emit Withdraw(recipient, amount, getTreasuryToken());
  }
}
...
contract Treasury is Context {
  address private _tokenContractAddress; // public address of the main asset of the treasury
  mapping(address => bool) private _treasurers;
  BEP20 private _token;

  event Withdraw(address recipient, uint256 amount, BEP20 token);

  constructor(address tokenContractAddress_, address[] memory treasurers_) {
    _tokenContractAddress = tokenContractAddress_;
    _token = BEP20(tokenContractAddress_);
    for (uint i = 0; i < treasurers_.length; i++) {
      _treasurers[treasurers_[i]] = true;
    }
  }

  modifier onlyPrivilegedAgents() {
    require(_treasurers[_msgSender()], "Unauthorized");
    _;
  }

  function withdraw(uint256 amount) external onlyPrivilegedAgents {
    require(_token.balanceOf(address(this)) >= amount, "Insufficient balance");
    _token.transfer(_msgSender(), amount);
    emit Withdraw(_msgSender(), amount, _token);
  }
