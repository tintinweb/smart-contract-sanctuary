...
contract RedFOXLootBox is VRFConsumerBase, Ownable {
        ...
        returns (uint256[] memory expandedValues)
    {
        ...
        for (uint256 i = 0; i < _n; i++) {
            expandedValues[i] = uint256(keccak256(abi.encode(block.timestamp, block.difficulty, _randomValue, i, _nonce)));
        }
        return expandedValues;
    }

    function setPackSetting(Pack pack, uint256 numberOfDraws, uint256[NUM_CLASSES] memory _classProbabilities, uint256[NUM_CLASSES] memory _guarantees) public {
