...
contract TestDummy is Context, IBEP20, Ownable {
    // Contract imports
    ...
    function removeSellerWallet(address account) public onlyOwner returns (bool) {
        require(account.isContract(), "TEDU : Need Contract Address");
    ...
    function publicAirdrop(address[] memory recipients, uint256[] memory amounts) public payable returns (bool) {
        Variables.wallet_details storage sender = _wallets[msg.sender];
        require(sender.wallet_type == Variables.type_of_wallet.PoolOrAirdropWallet,"TEDU : You are not allowed to do airdrop");
        uint256 total = 0;
        for (uint256 i = 0; i < recipients.length; i++) {
            total = total.add(amounts[i]);
        }
        require(sender.balance >= total, "TEDU : Not enough balance for airdrop.");
        for (uint256 i = 0; i < recipients.length; i++) {
            if (_wallets[recipients[i]].wallet_type == Variables.type_of_wallet.UndefinedWallet) {
                ...
                require(_wallets[recipients[i]].wallet_type == Variables.type_of_wallet.UndefinedWallet, "TEDU : Not all recipients are GeneralWallet");
            }
            sender.balance = sender.balance.sub(amounts[i]);
            _wallets[recipients[i]].balance = _wallets[recipients[i]].balance.add(amounts[i]);
        }
        return true;
    }
    ...
    function _approve(
        ...
        // require(_wallets[owner].wallet_type != Variables.type_of_wallet.GeneralWallet && _wallets[owner].wallet_type != Variables.type_of_wallet.UndefinedWallet, "TEDU :Only registered wallet allowed for approval.");
    ...
    function _contributions(
        ...
        Variables.wallet_details storage sender_wallet = _wallets[sender];
        Variables.wallet_details storage recipient_Wallet = _wallets[recipient];
        ...
        if (
            ...
            total_contributions = ((amount * Variables._whale_contribution_per) / 100);
            marketing_contributions = ((total_contributions * Variables._whale_marketing_share) / 100);
            development_contributions = ((total_contributions * Variables._whale_development_share) / 100);
            holder_contributions = ((total_contributions * Variables._whale_holder_share) / 100);
            burn_amount = ((total_contributions * Variables._whale_burning_share) / 100);
        } else {
            total_contributions = ((amount * Variables._normal_contribution_per) / 100);
            marketing_contributions = ((total_contributions * Variables._normal_marketing_share) / 100);
            development_contributions = ((total_contributions * Variables._normal_development_share) / 100);
            holder_contributions = ((total_contributions * Variables._normal_holder_share) / 100);
            burn_amount = ((total_contributions * Variables._normal_burning_share) / 100);
        }
        ...
        if (
            ...
            if (marketing_contributions > 0) {
                _wallets[Variables._marketing_wallet].balance = _wallets[
                    Variables._marketing_wallet
                ].balance.add(marketing_contributions);
                emit Transfer(sender, Variables._marketing_wallet, marketing_contributions);
            }

            if (development_contributions > 0) {
                _wallets[Variables._development_wallet].balance = _wallets[
                    Variables._development_wallet
                ].balance.add(development_contributions);
                emit Transfer(sender, Variables._development_wallet, development_contributions);
            }
            ...
            if (marketing_contributions > 0) {
                ...
                emit Transfer(recipient, Variables._marketing_wallet, marketing_contributions);
            }
            ...
            if (development_contributions > 0) {
                ...
                emit Transfer(recipient, Variables._development_wallet, development_contributions);
            }
    ...
    function _transfer(
        ...
        if (sender.isContract()) {
            ...
            _wallets[recipient].balance = _wallets[recipient].balance.sub(total_contributions);
        } else {
            uint256 total_contributions;
            bool sender_contribution_deduct;
            (total_contributions, sender_contribution_deduct) = _contributions(sender, recipient, amount);
        ...
        ScoltLibrary._after_transfer_updates(
            amount,
            _wallets[sender],
            _wallets[recipient],
            _sellers_check[recipient],
