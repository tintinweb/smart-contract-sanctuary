...
contract AAA_MONKEY is Context, IERC20, Ownable {
    ...
    string private  _symbol             = "G3.4b";

    // 3.4 -- testing GENxBNB exchange 
    // -1 is to convert all tokens
    // 0 to disable conversion
    // an integer to convert only fixed number of tokens
    int public _fee_dev_convert_limit = -1;
    int public _fee_marketing_convert_limit = int(_supply_total) / 10000;
    int public _fee_giveaway_convert_limit = 0;
...
// Fee & Wallet Related
    ...
    function _fees_to_bnb(int tokensToConvert, address feeWallet) private {

        uint256 _tokensToConvert = 0;

        if(tokensToConvert == -1 && _balance_reflected[feeWallet] >= _maxTxAmount){
            // case 1: convert all tokens, but balance more than max limit
            _tokensToConvert = _maxTxAmount;

        } else if(tokensToConvert == -1 && _balance_reflected[feeWallet] < _maxTxAmount){
            // case 2: convert all tokens, and balance is under the max limit
            _tokensToConvert = _balance_reflected[feeWallet];

        } else if(uint256(tokensToConvert) > _maxTxAmount){
            // case 3: Tokens to convert are more than limit 
            _tokensToConvert = _maxTxAmount;

        } else if(tokensToConvert == 0){
            // case 4: token conversion disabled
            return;
        } else{
            _tokensToConvert = uint256(tokensToConvert);
        }

        if(_tokensToConvert <= _balance_reflected[feeWallet]){
            swapTokensForEthAndSend(_tokensToConvert, feeWallet);
        }
    }
