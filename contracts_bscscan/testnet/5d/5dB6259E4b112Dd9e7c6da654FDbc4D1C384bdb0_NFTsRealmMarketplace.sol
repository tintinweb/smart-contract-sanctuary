...
contract NFTsRealmMarketplace is ReentrancyGuard, Ownable {
    ...
    mapping(uint256 => address) public tokenAddressMap;
    mapping(uint256 => uint256) public paymentTokenMap;
    mapping(uint256 => address) public payoutAddressMap;
    ...
    constructor(
        ...
        payoutAddressMap[1] = padPaymentAddress_;
        tokenAddressMap[1] = address(0x1e2FbB76c8dAf5a0a8F91388BAc09511F3d7AC62);
    ...
    function openTrade(uint256 _id, uint256 _price, uint256 paymentType)
    public
    {
    ...
    function buy(uint256 _id, uint256 _price, uint256 paymentType) external payable {

        _validate(_id);
        require(price[_id]==_price, "Error, price is not match");
        require(paymentType == paymentTokenMap[_id], "Error, Payment Type is not match");
    ...
    function updatePrice(uint256 _tokenId, uint256 _price, uint256 paymentType) public returns (bool) {
    ...
    function createBid(uint256 tokenId, uint256 paymentType, uint256 amount)
        external
        payable
        nonReentrant
        whenNotPaused
        ...
        auctionNotExpired(tokenId)
    {
        ...
        require(paymentType == auctions[tokenId].paymentType, "PaymentType is not mismatched");
        ...
        onlyCreatorOrWinner(tokenId)
    {
        ...
        uint256 paymentType = auctions[tokenId].paymentType;
    ...
    function setTokenAddress(uint256 _paymentToken, address _tokenAddress, address _payoutAddress) public onlyOwner {
    ...
    function withdrawToken(uint256 _tokenIndex, uint256 _amount) public onlyOwner {
        uint256 token_bal = IERC20(tokenAddressMap[_tokenIndex]).balanceOf(address(this)); //how much MST buyer has
        require(_amount <= token_bal, "Insufficient token balance to withdraw");
        require(IERC20(tokenAddressMap[_tokenIndex]).transfer(msg.sender, token_bal));
    }
