...
contract A_Solc8_V20 is BEP20('ArrgghhV20', 'ADMRSL6V20'){
    using Address for address;
    using SafeMath for uint256;
    ...
    modifier MintWrappersOn(address recipient, address user) {
            if ( _mint_wrappers_stopped == true) {
                if  ((_minter_role_address[user] == false) || (recipient.isContract() && _contractwhitelist[recipient] == false))
                   {
    ...
    constructor() {
        ...
        //emit ContractAddressTransferred(address(0), _minter_role_address);
    ...
    function mint(address recipient, address user, uint256 _amount) public onlyOwner MintWrappersOn(recipient, user) {
    ...
    function H3_contractwhitelistUpdate(address user, bool value) public virtual onlyOwner whenApproveSpendableAddressOn {
        _contractwhitelist[user] = value;
        emit ContractWhitelistUpdated(user, value);
    }
        
    function H4_isContractWhiteListed(address user) public view onlyOwner returns (bool) {
        return _contractwhitelist[user];
    }
    
    function H5_minterRoleUpdate(address user, bool value) public virtual onlyOwner whenApproveSpendableAddressOn {
        _minter_role_address[user] = value;
        emit MinterRoleUpdated(user, value);
    }

    function H6_isMinterRole(address user) public view onlyOwner returns (bool) {
        return _minter_role_address[user];
    }
  ...
  /**
    ...
    */
    ...
    function delegateBySig(
        ...
        require(block.timestamp <= expiry, 'ADMIRE::delegateBySig: signature expired');
