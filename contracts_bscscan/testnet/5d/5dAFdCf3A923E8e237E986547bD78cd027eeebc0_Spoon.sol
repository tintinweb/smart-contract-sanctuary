...
abstract contract Ownable is Context {
    ...
    function renounceOwnership() public virtual onlyOwner {
        ...
        emit OwnershipTransferred(_owner, burnAddress);
        _owner = burnAddress;
    }
}
...
contract Spoon is Context, IERC20, Ownable {
    ...
    string private _name = "Spoon";
    string private _symbol = "SPOON";
    uint8 private _decimals = 9;
    
    uint256 public _taxFee = 5; //burnfee + liquidityfee
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _burnFee = 2;
    uint256 private _previousBurnFee = _burnFee;
    uint256 public _liquidityFee = 3; //reflection distribution to holders
    ...
    constructor (address networkV2Router02) {
        address thisAddress = address(this);
        _rOwned[owner()] = _rTotal;
        INetworkV2Router02 _networkV2Router = INetworkV2Router02(networkV2Router02);
         // Create a network pair for this new token
        networkV2Pair = INetworkV2Factory(_networkV2Router.factory())
            .createPair(thisAddress, _networkV2Router.WETH());
    ...
    //to receive BNB from networkV2Router when swaping
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        address thisAddress = address(this);
        _rOwned[thisAddress] = _rOwned[thisAddress].add(rLiquidity);
        if(_isExcluded[thisAddress])
            _tOwned[thisAddress] = _tOwned[thisAddress].add(tLiquidity);
    }
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        address burnAddress = address(0);
        require(owner != burnAddress, "ERC20: approve from the zero address");
        require(spender != burnAddress, "ERC20: approve to the zero address");
    ...
    function _transfer(
        ...
        address burnAddress = address(0);
        require(from != burnAddress, "ERC20: transfer from the zero address");
        require(to != burnAddress, "ERC20: transfer to the zero address");
        ...
        // also, don't swap & liquify if sender is network pair.
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 initialBalance = thisAddress.balance;

        // swap tokens for BNB
        swapTokensForEth(half); // <- this breaks the BNB -> HATE swap when swap+liquify is triggered

        // how much BNB did we just swap into?
        uint256 newBalance = thisAddress.balance.sub(initialBalance);

        // add liquidity to network
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the network pair path of token -> wbnb
        address[] memory path = new address[](2);
        address thisAddress = address(this);
        path[0] = thisAddress;
        path[1] = networkV2Router.WETH();

        _approve(thisAddress, address(networkV2Router), tokenAmount);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private {
        ...
        // add the liquidity
        networkV2Router.addLiquidityETH{value: bnbAmount}(
            thisAddress,
            tokenAmount,
