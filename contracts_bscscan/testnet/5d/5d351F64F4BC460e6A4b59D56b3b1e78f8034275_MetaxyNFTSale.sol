...
contract MetaxyNFTSale is IChest, Ownable, VRFConsumerBase {
    ...
    uint256 private constant NOT_FOUND = 404;

    mapping(uint16 => uint16) rankLength;
    ...
    function mintAirdrop(address to, uint256 amount, uint16 rankId) external onlyOwner {
        require(rankLength[rankId] >= amount, "MetaxyNFTSale: Run out of id");
        ...
        rankLength[rankId] = uint16(rankLength[rankId] - amount);
        // If rankLength = 0, remove rank from chestRank
        if (rankLength[rankId] == 0) {
            uint indexCommon = getRankIndexByChestType(COMMON_CHEST, rankId);
            uint indexDeluxe = getRankIndexByChestType(DELUXE_CHEST, rankId);
            if (indexCommon != NOT_FOUND)
                removeRankFromPool(COMMON_CHEST, indexCommon);
            if (indexDeluxe != NOT_FOUND)
                removeRankFromPool(DELUXE_CHEST, indexDeluxe);
        }
    }

    function getRankIndexByChestType(uint16 chestType, uint16 rankId) private view returns(uint) {
        for (uint256 index = 0; index < arrLength[chestType]; index++) {
            if (rankId == chestRank[chestType][index])
                return index;
        }
        return NOT_FOUND;
    }
    ...
    function claimChest(uint16 chestType, bytes32 requestId) external override {
        ...
        for (uint256 k = 0; k < chestRank[chestType].length; k++) {
            virtualLengthTemp += rankLength[chestRank[chestType][k++]];
        }
        ...
        for (uint256 i = 1; i <= AMOUNT_NFT_PER_CHEST; i++) {
            ...
            for (uint256 l = 0; l < chestRank[chestType].length; l++) {
                concatLength += rankLength[chestRank[chestType][l]];
                if (concatLength >= selectedLength) {
                    chestRankIndex = l;
                    break;
                }
            }
            uint16 rankLengthIndex = chestRank[chestType][chestRankIndex];
            // Mapping id with rank
            rankOf[startId + i] = rankLengthIndex;
            // Minus rankLength
            if (rankLength[rankLengthIndex] > 0)
                rankLength[rankLengthIndex]--;
            // If rankLength = 0, remove rank from chestRank
            if (rankLength[rankLengthIndex] == 0)
                removeRankFromPool(chestType, rankLengthIndex);
