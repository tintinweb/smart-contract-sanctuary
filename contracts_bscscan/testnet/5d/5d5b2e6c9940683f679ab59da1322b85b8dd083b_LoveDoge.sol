...
contract LoveDoge is ERC20, Ownable {
    ...
    LoveDogeDividendTracker public dividendTracker;
    ...
    address payable public marketingAddress = payable(0xDD4E15a1abCF6259A5bDA713A28d2ff2c7994C0d); // Marketing Address
    address payable public buybackAddress = payable(owner()); // Buyback Address
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;

    uint256 public marketingDivisor;
    uint256 public buybackDivisor;

    uint256 private minimumTokensBeforeSwap = 2000 * 10**18; // 9
    ...
    constructor() public ERC20("LoveDoge", "LD") {
        uint256 _BNBRewardsFee = 6;
        uint256 _liquidityFee = 6;
        uint256 _marketingDivisor = 2;
        uint256 _buybackDivisor = 4;
    	...
    	dividendTracker = new LoveDogeDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        LoveDogeDividendTracker newDividendTracker = LoveDogeDividendTracker(payable(newAddress));
    ...
    function _transfer(
        ...
        // if(isFixedSaleBuy) {
        //     require(block.timestamp >= fixedSaleStartTimestamp, "LoveDoge: The fixed-sale has not started yet.");
        ...
        //     if(!openToEveryone) {
        //         require(fixedSaleEarlyParticipants[to], "LoveDoge: The fixed-sale is only available to certain participants at the start");
        //     }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(buybackAddress, transferredBalance.div(liquidityFee).mul(buybackDivisor));  
    }
...
contract LoveDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("LoveDoge_Dividend_Tracker", "LoveDoge_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "LoveDoge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "LoveDoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LoveDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "LoveDoge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "LoveDoge_Dividend_Tracker: Cannot update claimWait to same value");
