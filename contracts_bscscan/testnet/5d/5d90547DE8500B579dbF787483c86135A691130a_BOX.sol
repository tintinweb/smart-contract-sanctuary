...
contract BOX is Context, IERC20, Ownable {
    ...
    string public _name;
    bool private _setPresaleAddress = true;
    ...
    address payable private _marketingAddress =
        0x1dd38Ed37ACEae4Db1a178C11c886Ed753Fd3b24;
    ...
    uint256 public _launchTimeStamp = 0;
    ...
    event DonateToMarketing(uint256 ethDonated);
    ...
    constructor() public {
        ...
        _symbol = "BOX";
        _decimals = 18;
        _totalSupply = 10000000000 * 10**18; //10B
    ...
    function setMarketingWallet(address payable newWallet) external onlyOwner {
        require(_marketingAddress != newWallet, "Wallet already set!");
    ...
    function setNumTokensSellToAddToLiquidity(uint256 numTokensToLiquidity)
        external
        onlyOwner
    {
        numTokensSellToAddToLiquidity = numTokensToLiquidity;
    }
    ...
    function setPresaleAddress(address presaleAddress) external onlyOwner {
        require(
            _setPresaleAddress == true,
            "You can only set the presale addresses once!"
        );
        _setPresaleAddress = false;
        _isExcludedFromFee[presaleAddress] = true;
        _isExcludedFromMaxTxAmount[presaleAddress] = true;
        _liquidityHolders[presaleAddress] = true;
    }
    ...
    // This can remove from the blacklist if someone human somehow made it onto the list.
    // function removeSniper(address account) external override onlyOwner {
    ...
    function checkTxLimit(
        ...
        if (from == uniswapV2Pair) {
            require(
                amount <= _maxTxAmount || _isExcludedFromMaxTxAmount[to],
                "TX Limit Exceeded"
            );
        } else {
            require(
                amount <= _maxTxAmount || _isExcludedFromMaxTxAmount[from],
                "TX Limit Exceeded"
            );
        }
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // 75% of the total token balance in the contract will be converted into ETH
        uint256 tokensToSwapToETH = tokenBalanceToLiquifyAsETH.add(
            marketingTokenBalance
        );
        // swap tokens for ETH and store ETH in the contract
        ...
        // ETH to liquify is 25% of the total token balance on the contract or 33% of the ETH that has already been liquified
        uint256 ethToLiquify = ethSwapped.div(3);
        // add liquidity to uni swap
        ...
        // The remaining ETH balance is to be donated to the marketing wallet
        uint256 marketingETHToDonate = ethSwapped.sub(ethToLiquify);
        // Transfer the ETH to the marketing wallet
        _marketingAddress.transfer(marketingETHToDonate);
        emit DonateToMarketing(marketingETHToDonate);
    }
    ...
    function _tokenTransfer(
        ...
        if (_launchTimeStamp > 0 && sender == uniswapV2Pair && !_liquidityHolders[sender] && !_liquidityHolders[recipient]) {   
            if (block.timestamp <= _launchTimeStamp + BotTaxTime) { 
            //bot tax
            _transferBotProtection(sender, recipient, amount);
             }
        } else if (_isExcludedFromFee[sender]) {
    ...
    function _getBotTax(uint256 duration) private view returns (uint8) {
        ...
        return
            uint8(
                BotMaxTax -
                    (((BotMaxTax - _buyTax) * timeSinceLaunch) / duration)
            );
    }

    function _transferBotProtection(
        address sender,
        address recipient,
        uint256 amount
    ) private {
        ...
        uint256 taxPercentage = _getBotTax(BotTaxTime);
        uint256 totalTaxedToken = amount.mul(taxPercentage).div(10**2);
    ...
    function _transferStandard(
        ...
        uint256 totalTaxedLiquidityToken = calculateTaxFee(amount);
        uint256 totalTaxedMarketingToken = calculateLiquidityFee(amount);
        uint256 totalTax = totalTaxedLiquidityToken + totalTaxedMarketingToken;
        uint256 taxedAmount = amount - totalTax;
        _balances[address(this)] = _balances[address(this)].add(totalTax);
        _balances[recipient] = _balances[recipient].add(taxedAmount);
        emit Transfer(sender, recipient, taxedAmount);
    }
