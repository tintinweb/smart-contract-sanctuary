...
contract Trainers is Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    */
    ...
    function get(uint256 id) public view noFreshLookup(id) returns (uint16, uint8, uint8, uint64) {
    ...
    function getMaxGainXp(uint256 id) public view returns(uint256) {
        ...
        if (trainer.level < 255) {
            ...
            if (maxGainXp > trainer.xp && maxLevel != 254) {
    ...
    function getBattleDataAndDrainStamina(uint256 id, uint8 amount) public restricted returns(uint96) {
        Trainer storage trainer = tokens[id];
    ...
    function hasFused(uint256 id) public view returns(bool) {
        ...
        return _trainerReforgedLevel[id] == _fusionMaxLevel(trainerLevel);
    }

    function getFusedLevel(uint256 id) public view returns(uint8) {
        return _trainerReforgedLevel[id];
    }

    function fusion(uint256 fusionID, uint256 burnID) public restricted {
        uint8 trainerLevel = tokens[fusionID].level;
        ...
        require(
            _fusionMinLevel(trainerLevel) <= burnTrainerLevel && burnTrainerLevel <= trainerLevel,
            "Burn trainer not the same level range"
        );
        if (_trainerReforgedLevel[fusionID] != 0) {
            require(_trainerReforgedLevel[fusionID] < trainerLevel, "Reforged");
        }

        _trainerReforgedLevel[fusionID] = _fusionMaxLevel(trainerLevel);
        _burn(burnID);

        emit Fused(ownerOf(fusionID), fusionID, burnID);
    }

    function _fusionMinLevel(uint8 level) internal view returns(uint8 minLevel) {
    ...
    function _fusionMaxLevel(uint8 level) internal view returns(uint8 maxLevel) {
        for (uint8 i = 0; i < _trainerReforgeMilestones.length; i++) {
    ...
    function _levelMax(uint256 id) internal view returns(uint8) {
        ...
        uint8 maxLevel = _fusionMaxLevel(level);
