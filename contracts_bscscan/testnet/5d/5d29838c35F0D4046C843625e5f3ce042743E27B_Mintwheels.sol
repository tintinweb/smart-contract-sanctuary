...
contract Mintwheels is ERC721Metadata {
  ...
  // Public Sale: 0.5 BNB, Presale: 0.333 BNB
  uint256 public constant PUBLIC_SALE_PRICE = 0.5 ether;
  uint256 public constant PRESALE_PRICE = 0.333 ether;
  ...
  uint256 public startTime = 9999999600;

  // Minters
  mapping(uint256 => address) public minters;
  mapping(address => uint8) public _presalers;
  ...
  modifier periodStarted() {
    require(block.timestamp >= startTime, "Mintwheels: Period not started");
    _;
  }
  ...
  constructor(
    ...
    address admin_,
    address admin2_
  ) ERC721Metadata("Mintwheels", "MWHEELS", baseURI_) {
    _admin = admin_;
    _admin2 = admin2_;
  }

  function setStartTime(uint256 _startTime) external onlyOwner {
    require(_startTime > 0, "Mintwheels: invalid _startTime");
    ...
    onlyOwner
  {
    for (uint256 i = 0; i < addresses.length; i++) {
      _presalers[addresses[i]] = numAllowedToMint;
    }
  }

  function purchase(uint8 numberOfNfts) external payable periodStarted {
    require(pendingCount > 0, "Mintwheels: All minted");
    require(numberOfNfts > 0, "Mintwheels: numberOfNfts cannot be 0");
    require(
      numberOfNfts <= (isPresale ? _presalers[msg.sender] : 20),
      "Mintwheels: You may not buy more than 20 NFTs at once"
    );
    require(
      totalSupply().add(numberOfNfts) <= MAX_NFT_SUPPLY,
      "Mintwheels: sale already ended"
    );
    require(
      (isPresale ? PRESALE_PRICE : PUBLIC_SALE_PRICE).mul(numberOfNfts) ==
        msg.value,
      "Mintwheels: invalid ether value"
    );

    if (isPresale) {
      _presalers[msg.sender] = _presalers[msg.sender] - numberOfNfts;
    }

    for (uint8 i = 0; i < numberOfNfts; i++) {
  ...
  function _splitBalance(uint256 amount) internal {
    uint256 mintingShare1 = (amount * 85) / 100;
    uint256 mintingShare2 = amount - mintingShare1;
  ...
  function _randomMint(address _to) internal returns (uint256) {
    require(totalSupply() < MAX_NFT_SUPPLY, "Mintwheels: max supply reached");
