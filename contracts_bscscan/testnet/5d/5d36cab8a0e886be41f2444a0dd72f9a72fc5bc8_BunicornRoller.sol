...
contract BunicornRoller is IBunicornRoller, Initializable, AccessControlUpgradeable {
    ...
    function mintOneRandomBunicornWithStar(address _minter, uint8 _stars, uint256 _seed)
        ...
        returns(uint256)
    {
        uint8 _element = uint8(RandomUtil.randomSeededMinMax(0, 3, RandomUtil.combineSeeds(_seed, 1)));
        return _mintOneRandomBunicornWithStarAndElement(_minter, _stars, _element, _seed);
    }

    function mintOneRandomBunicornWithStarsProbSpecs(address _minter, uint256 _starsProbSpecs, uint256 _seed)
        external override
        onlyGameContract
        returns(uint256)
    {
        uint8 _stars = _getRandomStarsWithProbSpecs(_seed, _starsProbSpecs);
        uint8 _element = uint8(RandomUtil.randomSeededMinMax(0, 3, RandomUtil.combineSeeds(_seed, 1)));
        return _mintOneRandomBunicornWithStarAndElement(_minter, _stars, _element, _seed);
    }
    ...
    {
        uint8 _stars = _getRandomStarsWithProbSpecs(_seed, _starsProbSpecs);
        return _mintOneRandomEventBunicornWithStarAndElement(_minter, _stars, _element, _seed, _eventId);
    }
    ...
    function _mintOneRandomBunicornWithStarAndElement(
        address _minter,
        uint8 _stars,
        uint8 _element,
        uint256 _seed
        ...
        private returns(uint256)
    {
        ...
        uint8 _costume = _getRandomCostume(_stars, _element, _seed);
        uint16 properties = _getRandomProperties(_stars, _element, _costume, _seed);
        return bunicornsContract.mintOneBunicornBySpecs(_minter, properties, attr1, attr2, attr3);
    }

    function _mintOneRandomEventBunicornWithStarAndElement(
        address _minter,
        uint8 _stars,
        uint8 _element,
        uint256 _seed,
        ...
        private returns(uint256)
    {
        ...
        (uint16 attr1, uint16 attr2, uint16 attr3) = _getRandomAttributes(_stars, _seed);
        uint16 properties = _getRandomProperties(_stars, _element, 1, _seed); // event always returns costume as 1
        uint32 eventMultiplier = _getRandomEventMultiplier(_seed);
        return bunicornsContract.mintOneEventBunicornBySpecs(_minter, properties, attr1, attr2, attr3, eventMultiplier, _eventId);
    }

    function rollAttributesFromStars(uint8 _stars, uint256 _seed)
        external
        override
        onlyGameContract
        returns (uint16, uint16, uint16)
    {
        return _getRandomAttributes(_stars, _seed);
    }

    function _getRandomStarsWithProbSpecs(uint256 _seed, uint256 _starsProbSpecs) private pure returns (uint8) {
        uint256 roll = _seed % 10000;
        uint256 _starsRate = uint16((_starsProbSpecs >> 64) & 0xFFFF);
        if (roll < _starsRate) {
            return 4;
        }
        
        _starsRate += uint16((_starsProbSpecs >> 48) & 0xFFFF);
        if (roll < _starsRate) {
            return 3;
        }

        _starsRate += uint16((_starsProbSpecs >> 32) & 0xFFFF);
        if (roll < _starsRate) {
            return 2;
        }

        _starsRate += uint16((_starsProbSpecs >> 16) & 0xFFFF);
        if (roll < _starsRate) {
            return 1;
        }
    ...
    function _getRandomCostume(uint8 _stars, uint8 _element, uint256 _seed) private pure returns(uint8) {
        uint8 maxCostume;
        if (_element == 0) { // FIRE
            maxCostume = 2;
        }
        else if (_element == 1) { // EARTH
            if (_stars < 4) {
                maxCostume = 2;
            }
            else {
        ...
        return uint8(
            RandomUtil.randomSeededMinMax(1, maxCostume, RandomUtil.combineSeeds(_seed, 2))
        );
    }

    function _getRandomProperties(uint8 _stars, uint8 _element, uint8 _custume, uint256 _seed) private pure returns (uint16) {
    ...
    function _getRandomEventMultiplier(uint256 _seed) private pure returns (uint32) {
        uint256 roll = _seed % 10000;

        uint8 multiplier;
        if (roll >= 4500) { // low multiplier: 5500 / 10000 ~ 55%
            multiplier = 0;
        }
        else if (roll >= 1200) { // medium multiplier: (4500 - 1200) / 10000 ~ 33%
            multiplier = 1;
        }
        else {
            multiplier = 2; // high multiplier: 1200 / 10000 ~ 12%
        }

        uint32 minRoll =  _getMinEventMultiplier(multiplier);
        uint32 maxRoll = _getMaxEventMultiplier(multiplier);

        return uint32(RandomUtil.randomSeededMinMax(minRoll, maxRoll, RandomUtil.combineSeeds(_seed, 10)));
    }

    function _getMinEventMultiplier(uint8 _multiplier) private pure returns (uint32) {
        if (_multiplier == 2) {
            return uint32(30000); // boost 3.0
        }
        if (_multiplier == 1) {
            return uint32(25000); // boost 2.5
        }
        return uint32(20000); // boost 2.0
    }

    function _getMaxEventMultiplier(uint8 _multiplier) private pure returns (uint32) {
        if (_multiplier == 2) {
            return uint32(35000); // boost 3.5
        }
        if (_multiplier == 1) {
            return uint32(30000); // boost 3.0
        }
