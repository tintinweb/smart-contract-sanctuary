...
contract FlokiSimp is IERC20, Ownable {
    ...
    uint256 private _total = startingSupply * (10 ** _decimalsMul);
    ...
    function swapBack(uint256 numTokensToSwap) internal swapping {
        uint256 dynamicLiquidityFee = isOverLiquified(targetLiquidity, targetLiquidityDenominator) ? 0 : _liquidityFeeSell;
        uint256 amountToLiquify = numTokensToSwap * dynamicLiquidityFee / _totalFeeSell / 2;
        ...
        uint256 amountBNBReflection = amountBNB * _reflectionFeeSell / totalBNBFee;
        ...
        if (amountToLiquify > 0) {
            ...
            // If we are not liquifying we add the bnb to SIMP buys.
