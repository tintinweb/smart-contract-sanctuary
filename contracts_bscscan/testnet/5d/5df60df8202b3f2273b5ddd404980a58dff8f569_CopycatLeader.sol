...
contract CopycatLeader is ERC20('Copycat Leader', 'COPYLEADER'), Ownable, ICopycatLeader, CopycatEmergency {
  CopycatLeaderFactory public factory;
  bool public initialized = false;

  string public tokenName = "Copycat Leader";
  string public tokenSymbol = "COPYLEADER";
  ...
  uint256 public depositCopycatFee = 1e18; // Base fee
  uint256 public withdrawCopycatFee = 1e18; // Base fee
  ...
  function initialize(
    ...
    require(depositCopycatFee >= 1e18, "Not enough fee");
    require(withdrawCopycatFee >= 1e18, "Not enough fee");
  ...
  function getAadaptersBnb() override public view returns(uint256[] memory) {
  ...
  function getShareRatio() override public view returns(uint256) {
    return getShareRatioSaveGas(getAadaptersBnb()[adapters.length + 1]);
  }
  ...
  function addAdapter(ICopycatAdapter adapter) override public {
    require(factory.registeredAdapterFactory(msg.sender)/* || (factory.isUnsafeLeader(address(this)) && msg.sender == owner())*/, "Forbidden");
    require(adapter.getLeaderContract() == this, "Wrong contract");
    require(adapter.getLeaderAddress() == owner(), "Wrong leader");
    require(adapter.getBnbValue() == 0, "Not zero");
  ...
  function removeAdapter(uint256 adapterId) override public onlyOwner {
    require(
      ...
      CopycatAdapterFactoryBase(adapters[adapterId].factory()).allowEmergencyRemoveAdapter(adapters[adapterId].contractSignature()), 
      "Not zero");
  ...
  function depositTo(address to, uint256 shareMin) override public payable returns (uint256 totalShare) {
    ...
    uint256[] memory bnbValues = getAadaptersBnb();
    ...
    uint256 totalReceived = getAadaptersBnb()[adapters.length + 1] - totalBnb;
    ...
    require(totalShare >= shareMin, "CPC: Insufficient output share");
  ...
  function withdrawFromMigration(address to, uint256 amount, uint256 bnbMin) internal returns (uint256 outputAmount) {
    ...
    if (outputAmount <= address(this).balance) {
      require(outputAmount >= bnbMin, "CPC: Insufficient output BNB");
      payable(to).transfer(outputAmount);
    } else {
      require(address(this).balance >= bnbMin, "CPC: Insufficient output BNB");
      outputAmount = address(this).balance;
      payable(to).transfer(outputAmount);
    }
    
    emit Withdraw(msg.sender, amount, outputAmount, migratedTo.getShareRatio());
  ...
  function upgrade(uint256 amount) public override {
    require(address(migratedTo) != address(0), "No migration");
    _burn(msg.sender, amount);
...
interface ICopycatNameNft is IERC721 {
  function getTokenName(uint256 tokenId) external view returns(string memory);
  function getTokenSymbol(uint256 tokenId) external view returns(string memory);
...
interface ICopycatLeader is IERC20 {
  ...
  function getAadaptersBnb() external view returns(uint256[] memory);
...
contract CopycatLeaderFactory is CopycatEmergencyMaster {
  ...
  function emitDeposit(address depositer, uint256 level, uint256 amount) public {
    require(leaderStat[msg.sender].leaderId > 0, "Not Registered");
  ...
  function deployLeader(
    ...
    require(msg.value == FEE_LIST[4] + 0.0001 ether, "Insufficient lock");
...
abstract contract CopycatAdapterFactoryBase is CopycatEmergencyMaster {
  ...
  mapping(bytes32 => bool) public allowEmergencyRemoveAdapter;

  event SetAllowEmergencyRemoveAdapter(address indexed setter, bytes32 signature, bool allow);
  function setAllowEmergencyRemoveAdapter(bytes32 signature, bool allow) public onlyOwner {
    allowEmergencyRemoveAdapter[signature] = allow;
    emit SetAllowEmergencyRemoveAdapter(msg.sender, signature, allow);
  }

  modifier onlyLeader(address leaderContract) {
    require(Ownable(leaderContract).owner() == msg.sender, "Not leader");
    _;
  }
...
abstract contract CopycatEmergency {
    ...
    function queueTransaction(address target, uint value, string memory signature, bytes memory data, uint eta) public returns (bytes32) {
        ...
        require(allowEmergencyCall(txHash), "Not allowed");
...
contract CopycatEmergencyMaster is Ownable, ICopycatEmergencyMaster {
  ...
  event AllowEmergency(address indexed allower, bytes32 txHash);
  function allowEmergency(bytes32 txHash) public override onlyOwner {
    isAllowEmergency[txHash] = true;
    emit AllowEmergency(msg.sender, txHash);
  }
}
...
interface ICopycatEmergencyMaster {
  ...
  function allowEmergency(bytes32 txHash) external;
}