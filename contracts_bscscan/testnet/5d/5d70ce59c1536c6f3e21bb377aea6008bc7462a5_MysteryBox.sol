...
contract MysteryBox is AccessControlEnumerable, ERC721Enumerable, ERC721Burnable  {
    ...
    function mint(uint256 tokenId, bytes memory signature) public virtual {
        require(!nonces[tokenId], "MysteryBox: nonce was used");

        address msgSender = _msgSender();

        bytes32 message = keccak256(abi.encodePacked(tokenId, msgSender, _chainId, this)).prefixed();

        require(hasRole(SIGNER_ROLE, message.recoverSigner(signature)), "MysteryBox: signature is invalid");
    ...
    function mintBatch(uint256[] memory tokenIds, address[] memory accounts) public virtual {
        ...
        require(length > 0 && length == accounts.length, "MysteryBox: array length is invalid");
