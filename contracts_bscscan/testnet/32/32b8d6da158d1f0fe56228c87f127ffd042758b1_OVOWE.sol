...
contract Ownable is Context {
 ...
 contract Frozen is Context {
   ...
   */
  modifier notFrozen() {
    require(!_frozen, "Freezable: frozen");
    _;
  }
   ...
   * - The contract must not be frozen.
   */
  function _freeze() internal notFrozen {
    _frozen = true;
    emit Freeze(_msgSender());
  }
   ...
   * - The contract must be frozen.
   */
  function _unfreeze() internal frozen {
    _frozen = false;
    emit Unfreeze(_msgSender());
  }
}


contract OVOWE is Context, Frozen, IBEP20, Ownable {
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    require(!getFrozen(), "BEP20: token transfer while frozen");
