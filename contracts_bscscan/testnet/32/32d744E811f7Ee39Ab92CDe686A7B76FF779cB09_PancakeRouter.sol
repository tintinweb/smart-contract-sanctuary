...
library TransferHelper {
    function safeApprove(address token, address to, uint value) internal {
        bytes4 FUNC_SELECTOR =  bytes4(keccak256(bytes('approve(address,uint256)')));
    ...
    function safeTransfer(address token, address to, uint value) internal {
        bytes4 FUNC_SELECTOR = bytes4(keccak256(bytes('transfer(address,uint256)')));
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(FUNC_SELECTOR, to, value));
    ...
    function safeTransferFrom(address token, address from, address to, uint value) internal {
        bytes4 FUNC_SELECTOR = bytes4(keccak256(bytes('transferFrom(address,address,uint256)')));
//        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(0x23b872dd, from, to, value));
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(FUNC_SELECTOR, from, to, value));
...
library PancakeLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'8b19951d97f95e3b8a5afad4f30da99bc71922c09b6e62ae1c7cd728c7ce827d' // init code hash
            ))));
    }
...
contract PancakeRouter is IPancakeRouter02 {
    ...
    function pairaddress( address token) external view returns (address _pairaddress) {
        _pairaddress = PancakeLibrary.pairFor(factory, token, WETH);
    }
