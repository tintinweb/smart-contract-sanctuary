...
contract TokenVestingFactory is Ownable {
    ...
    // Get INP address
    function getINP() public view returns (IERC20) {
        return _inp;
    }

    // Modify INP address to create a lock
    function setINP(IERC20 inp_) public onlyOwner {
        _inp = inp_;
    }

    // Return all vested wallets for a specific user
    function getUserWallets(address _user) public view returns (address[] memory) {
        return wallets[_user];
    }

    // Return all vested wallets
    function getAllWallets() public view returns (address[] memory) {
        return _wallets;
    }

    function newTokenVesting(address beneficiary_, uint256 start_, uint256 periodDuration_, uint256 periodsAmount_)
        public
        payable
        returns (address wallet)
    {
        ...
        wallet = address(new TokenVesting(beneficiary_, start_, periodDuration_, periodsAmount_, _inp));        
        ...
        // If beneficiary is the different as sender then add wallet to sender's wallets too.
        if (msg.sender != beneficiary_) {
            wallets[beneficiary_].push(wallet);
        }

        // Emit event.
        emit Created(wallet, msg.sender, beneficiary_, block.timestamp, start_, periodDuration_, periodsAmount_);
    }
...
contract TokenVesting {
    ...
    /**
     * @dev Creates a vesting contract that vests its balance of INP ERC20 token to the
     ...
     * @param inp_ address of INP token
     */
    constructor (address beneficiary_, uint256 start_, uint256 periodDuration_, uint256 periodsAmount_, IERC20 inp_) {
     ...
     * @return INP address
     */
    function inp() public view returns (address) {
        return address(_inp);
    }

    /**
     * @return INP tokens already released
     */
    ...
    /**
     * @return returns amount of INP tokens available for release 
     */
    ...
    /**
     * @return returns the current balance of INP tokens
     */
    function currentBalance() public view returns (uint) {
        return _inp.balanceOf(address(this));
    }
    ...
    function release() public {
        ...
        _inp.safeTransfer(_beneficiary, unreleased);
