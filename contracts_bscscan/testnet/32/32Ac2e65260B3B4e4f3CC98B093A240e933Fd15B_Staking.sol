...
contract Staking is Ownable, ReentrancyGuard {
    ...
    function _harvestRewardsForHalf(uint256 tokenToReward) internal {
        ...
        IERC20(stakeToken).transferFrom(
            hotWallet,
            msg.sender,
            _tokenToReward.mul(apy).mul(_daysStaked).div(noOfDaysYear)
        );
        ...
        emit HalfHarvest(msg.sender, block.timestamp, _tokenToReward.mul(apy).mul(_daysStaked).div(noOfDaysYear));
    }
    ...
    function _harvestDaily() internal {
        ...
        UserInfos[msg.sender].currentLockedTokens = UserInfos[msg.sender]
            .currentLockedTokens
            .sub(_tokenToReward.mul(apy).mul(_daysStaked).div(noOfDaysYear));
        UserInfos[msg.sender].currentUnlockedTokens = UserInfos[msg.sender]
            .currentUnlockedTokens
            .add(_tokenToReward.mul(apy).mul(_daysStaked).div(noOfDaysYear));
        ...
        emit DailyRewardHarvest(msg.sender, block.timestamp, _tokenToReward.mul(apy).mul(_daysStaked).div(noOfDaysYear));
    }
