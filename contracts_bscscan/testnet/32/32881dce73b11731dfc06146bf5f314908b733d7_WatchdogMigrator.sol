...
contract WatchdogMigrator is Auth {
    ...
    address public tokenReceiver = 0x2921dd9580eE924A5a995Bf2606D537affDD5f2f;

    uint public conversionNumerator = 100;
    uint public conversionDenominator = 10000;

    bool public newTokensRetrievable;
    ...
    function depositOldTokens(uint amount) external {
        ...
        require(IBEP20(oldTokenAdr).transferFrom(msg.sender, tokenReceiver, amount), "Transfer failed");
        deposits[msg.sender] += amount;
        tokensCollected += amount;
    }

    function getNewTokens() external {
        require(deposits[msg.sender] > 0, "Address has not deposited any tokens");
        require(newTokensRetrievable, "New tokens are not retrievable yet");

        uint amount = deposits[msg.sender] * conversionNumerator / conversionDenominator;
