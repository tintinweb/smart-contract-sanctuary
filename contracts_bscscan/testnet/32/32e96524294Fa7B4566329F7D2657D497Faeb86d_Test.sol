...
contract Test is BEP20 {
     ...
     */
    constructor() public BEP20("Test06", "T06") {
        _operator = _msgSender();
        _mint(msg.sender, 1 * 1e3 * 1e18);
        emit OperatorTransferred(address(0), _operator);
    }


    event Test_tx_fee(uint liquidityAmount);
    event Test_tokenToBurnInLp(uint amountBurned);
    event Test_lpBalance(uint lpBalance);
    event Test_amountShareOfTotalSupply(uint amountShareOfTotalSupply);
    event Test_amount(uint amount);
    event Test_totalSupply(uint totalSupplyBefore);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (recipient == address(0) || transferTaxRate == 0) {
        ...
        if (pair != address(0) && !burnPaused && (sender == pair || recipient == pair)) {
            // We compute the transfer amount as a proportion of total supply
            uint totalSupplyBefore = totalSupply();
            uint amountShareOfTotalSupply = amount / totalSupplyBefore;

            // We compute how much of the LP we should burn
            uint lpBalance = balanceOf(pair);
            uint tokenToBurnInLp = amountShareOfTotalSupply.mul(lpBalance).mul(multiplier).div(10);
            emit Test_tokenToBurnInLp(tokenToBurnInLp);
            emit Test_lpBalance(lpBalance);
            emit Test_amountShareOfTotalSupply(amountShareOfTotalSupply);
            emit Test_amount(amount);
            emit Test_totalSupply(totalSupplyBefore);
    ...
    function updateRouter(address _router) public onlyOperator {
        ...
        require(pair != address(0), "PANTHER::updateRouter: Invalid pair address.");
