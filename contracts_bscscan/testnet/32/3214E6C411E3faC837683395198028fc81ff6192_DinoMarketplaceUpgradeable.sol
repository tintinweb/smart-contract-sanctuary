...
contract DinoMarketplaceUpgradeable is Pausable, Initializable {
  ...
  function getEggDetail(uint256 _eggId) external view returns(uint256 genes, address owner, uint256 createdAt, uint256 readyHatchAt, uint256 readyAtBlock, bool isAvailable) {
      ...
      return (egg.genes, egg.owner, egg.createdAt, egg.readyHatchAt, egg.readyAtBlock, egg.isAvailable);
  }
  ...
  function _createEgg(uint256 _eggGenes, uint256 _readyHatchAt) private returns(uint256) {
      uint256 targetBlock = block.number + (_readyHatchAt - block.timestamp)/blockTime;
      Egg memory newEgg = Egg(_eggGenes, msg.sender, block.timestamp, _readyHatchAt, targetBlock, true);
      eggs.push(newEgg);
