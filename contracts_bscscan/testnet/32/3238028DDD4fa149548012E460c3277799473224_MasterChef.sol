...
contract MasterChef is Ownable {
        ...
        returns (uint256,uint256,uint256)
    {
        ...
        if(chachaReward==0||lpSupply == 0){
            ...
            uint256 lpbalance = ChaCha.balanceOf(address(pool.lpToken)).mul(lpSupply).mul(1 ether).div(pool.lpToken.totalSupply()).mul(2);
            uint256 dayChaChaReward =  chachaReward.mul(1 ether).div(lpbalance);
            return (user.amount,lpSupply,dayChaChaReward.mul(365 * 24 * 6 ));
        }
    }  

    function getLp(uint256 _pid) external
        view
        ...
        uint256 chachaReward =  IChaChaLP(address(ChaCha)).getMultiplierForLp(block.timestamp - 600, block.timestamp);
        uint256 dayChaChaReward =  chachaReward.mul(1 ether).div(lpbalance);
        return(lpSupply,lpbalance,chachaReward,dayChaChaReward);
    }
