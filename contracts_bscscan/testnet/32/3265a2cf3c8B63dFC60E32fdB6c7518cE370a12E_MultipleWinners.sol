...
abstract contract PrizePool is PrizePoolInterface, OwnableUpgradeable, ReentrancyGuardUpgradeable, TokenControllerInterface, IERC721ReceiverUpgradeable {
    ...
    canAddLiquidity(amount)
  {
	...
	if (amount > 0 && address(pantherReferral) != address(0) && referrer != address(0) && referrer != msg.sender) {
		pantherReferral.recordReferral(msg.sender, referrer);
	}
    ...
    onlyControlledToken(controlledToken)
  {
	...
	if(sablierAwardStream.stream != address(0)){
		uint256 startTime = block.timestamp + sablierAwardStream.start;
		uint256 endTime = startTime + (amount / sablierAwardStream.maxRate);
		
		// _mint(address(this), amount, controlledToken, address(0));
		// IERC20Upgradeable(controlledToken).approve(sablierAwardStream.stream, amount);		
		// IERC1620(sablierAwardStream.stream).createStream(to, amount, controlledToken, startTime, endTime);
		
		IERC20Upgradeable(address(_token())).approve(sablierAwardStream.stream, amount);	
		IERC1620(sablierAwardStream.stream).createStream(to, amount, address(_token()), startTime, endTime);	
	} else {
		// _mint(to, amount, controlledToken, address(0));
		_token().transfer(to, amount);
	}
	
	payReferralCommission(to, amount, controlledToken);
	...
	emit Awarded(to, address(_token()), amount);
  }
  ...
  struct SablierAwardStream {
     address stream; 
	 uint256 start;
	 uint256 maxRate;
  }
  
  SablierAwardStream public sablierAwardStream;
  
 function setSablierAwardStream(
    address stream,
    uint256 start,
    uint256 maxRate
  )
    external onlyOwner
  {  
	sablierAwardStream = SablierAwardStream(stream, start, maxRate);
  }
  
  function cancelSablierStream(
	 uint256 streamId
  )
    external onlyOwner
  {
	require(sablierAwardStream.stream != address(0), "SablierAwardStream not set");	
	
	IERC1620(sablierAwardStream.stream).cancelStream(streamId);
  }
  ...
  function payReferralCommission(address _user, uint256 _pending, address controlledToken) internal {
      if (address(pantherReferral) != address(0) && referralCommissionRate > 0) {
          ...
          if (referrer != address(0) && commissionAmount > 0) {    
			  _mint(referrer, commissionAmount, controlledToken, address(0));			  
