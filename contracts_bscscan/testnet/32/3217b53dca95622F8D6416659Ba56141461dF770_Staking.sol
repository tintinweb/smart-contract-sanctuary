...
contract Staking is Ownable, ReentrancyGuard{
    ...
    * Sua utilidade é apenas para governança do sistema
    ...
    function unstake() external nonReentrant{
        ...
        require(recompensaPorUser() == 0, "seu saldo de recompensas deve ser zero");
    ...
    function unstakeReward() external  nonReentrant{
        require(_poolBalance > 0, "saldo igual a zero");
        require(recompensaPorUser() > 0, "saldo menor doque 0");
        uint balanceReward = SafeMath.sub(_poolBalance, stakingBalance[msg.sender]);
        _poolBalance = balanceReward;
        uint stakingTime = recompensaPorUser();
   ...
   function recompensaPorUser() public view returns(uint) {
       uint256 rewardAmount = (((block.timestamp - startBlock[msg.sender]) / rewardsPerHour) * stakingBalance[msg.sender]) / rewardPercent;
       return rewardAmount;
   }
