...
contract Mineable is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 private _updatedSwapPrice;
    uint256 private _largestLossAllowed;
    ...
    constructor (uint256 _tokenSupply, uint256 _tokenMinerFee, uint256 _tokenReflectionFee, uint256 _tokenMarketingFee, uint256 _maxTokenSwap) {
        ...
        _prevMarketingFee = _marketingFeeRate;
    ...
    function _swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _currentMinerFees = _currentMinerFees.add(transferredBalance.mul(_minerFeeRate).div(100));
        _currentMarketingFees = _currentMarketingFees.add(transferredBalance.mul(_marketingFeeRate).div(100));
    }
    ...
    function _swapTokensForETH(uint256 tokenAmount) private nonReentrant {
        ...
        /* Avoid Frontrunning */
        uint256 amountOutMin = _updatedSwapPrice.mul(tokenAmount).mul(hundered.sub(_largestLossAllowed)).div(100);
    ...
    function setLargestLossAllowed(uint256 largestLossPercentage) external onlyOwner {
        _largestLossAllowed = largestLossPercentage;
    }

    /* 
        @param swapPrice: current swap price multiplied by 10^18 (bnb decimals)
    */
    function setSwapPrice(uint256 swapPrice) external onlyOwner {
        _updatedSwapPrice = swapPrice;
    }
