...
contract OxygenManageNFT is ERC721Pausable, AccessControl, TransferOwnable {
    ...
    function setAllowAddress(address AllowAddress, bool isAllow) public {
        ...
        allowAddress[AllowAddress] = isAllow;
        
    }

    function mint(address to, string memory _tokenURI) public payable returns (uint256 tokenId)  {
        if(!isPublic){
            require(isPartner(msg.sender), 'mintes: require isPartner(msg.sender)');
        }
        require(allowAddress[msg.sender], 'mint: require allowAddress[msg.sender]');
    ...
    function mintes(address to, string memory _tokenURI, uint256 quantity) public payable returns (uint256[] memory) {
        ...
        require(allowAddress[msg.sender], 'mintes: require allowAddress[msg.sender]');
        require(block.number>=lastBlockNumber[msg.sender],'Attack_check: Too fast');
        lastBlockNumber[msg.sender] = block.number+1; 
