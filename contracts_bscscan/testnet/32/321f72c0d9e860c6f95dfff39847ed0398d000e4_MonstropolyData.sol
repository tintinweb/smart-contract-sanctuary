...
contract MonstropolyData is AccessControlProxyPausable, UUPSUpgradeableByRole { 
    ...
    uint public nStats = 4;
    uint public nAttributes = 2;
    ...
    function initialize() public /** TBD: initializer */{
        ...
        randLength = 6; //TBD: when f to modify calc this with loop, not manually
    ...
    function getRarityByGen(string calldata gen) public view returns(uint256) {
        ...
        _stringM   = slice(_bytes, randLength*5, randLength);
        return getValue(Value(parseInt(_string), parseInt(_stringM)));
    }
    ...
    function deconstructGen(string calldata gen) public view returns(DeconstructedGen memory){
        ...
        require(len >= 12, "Gen length is not correctly"); //TBD: not 12
        string memory _string;
        string memory _stringM;
        bytes memory _bytes = bytes(gen);
        ...
        // Weapon or Character RAND
        {
            _string   = slice(_bytes, 0, randLength);
            _stringM   = slice(_bytes, randLength*3, randLength);
            decGen._asset = Value(parseInt(_string), parseInt(_stringM));
        }

        {
            _string   = slice(_bytes, randLength, randLength);
            _stringM   = slice(_bytes, randLength*4, randLength);
            decGen._type = Value(parseInt(_string), parseInt(_stringM));
        }

        {
            _string   = slice(_bytes, randLength*2, randLength);
            _stringM   = slice(_bytes, randLength*5, randLength);
            decGen._rarity = Value(parseInt(_string), parseInt(_stringM));
        }
        
        // Stats  RAND
        {
            for(uint i = 0; i < nStats; i++) {
                _string = slice(_bytes, (randLength*6 + randLength*i), randLength);
                _stringM = slice(_bytes, (randLength*10 + randLength*i), randLength);
                decGen._stats[i] = Value(parseInt(_string), parseInt(_stringM));
            }
        }
        ...
        // Attributes  RAND
        {
            for(uint i = 0; i < nAttributes; i++) {
                _string = slice(_bytes, (randLength*14 + randLength*i), randLength);
                _stringM = slice(_bytes, (randLength*16 + randLength*i), randLength);
                decGen._attributes[i] = Value(parseInt(_string), parseInt(_stringM));
            }
        }
    ...
    function genExists(DeconstructedGen memory gen) public view returns (bool) {
        bytes memory _bytes = abi.encode(
            getValue(gen._asset),
            getValue(gen._type),
        ...
        bytes32 _hash = keccak256(_bytes);
