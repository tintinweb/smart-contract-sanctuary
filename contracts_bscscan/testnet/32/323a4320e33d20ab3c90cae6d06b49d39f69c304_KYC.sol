    ...
    function register(address _address, string memory uuidv4, uint8 level) public onlyAdmin {
     ...
     function registerMultiple(RecordsStruct[] memory _records) public onlyAdmin { 
        for(uint i=0; i< _records.length; i++){
           addressRecord[_records[i]._address] = Record(_records[i]._uuidv4, _records[i]._level, true);
           emit RecordAdded(_records[i]._address, _records[i]._uuidv4, _records[i]._level);
        }
    }
