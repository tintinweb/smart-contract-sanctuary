...
contract NewStaking_TestingV4 is Ownable {
    ...
    */
            ...
            if(ended){ // claim o withdraw cuando ha terminado
                if(!_withdraw){
                    ...
                    if(depositedTokens[account] == 0){
                        ...
                        toUnclaimed = pendingDivs.sub(toSend);
                    }
                ...
                if(_restake){ // deposit
                    ...
                    unclaimed[account] = unclaimed[account].sub(subUnclaimed);
                    
                    if(depositedTokens[account] > maxDepositedTokens[account]) maxDepositedTokens[account] = depositedTokens[account];
    ...
    function getAPY(address _staker) public view returns(uint){
        uint apy = rewardRate;
        if(block.timestamp.sub(firstTime[_staker]) > unstakeTime && alreadyProgUnstaked[msg.sender] == 0){
    ...
    function deposit(uint amountToStake) public  returns (bool){
        ...
        if(depositedTokens[msg.sender] > maxDepositedTokens[msg.sender]) maxDepositedTokens[msg.sender] = depositedTokens[msg.sender];
    ...
    */
    function getMaxAmountWithdrawable(address _staker) public view returns(uint){
        ...
        if(block.timestamp.sub(stakingTime[msg.sender]) < unstakeTime){ // esto esta mal. tiene que referirse al primer staking, para que no de 0 despues
            _res = 0;
        }else if(alreadyProgUnstaked[_staker] == 0){
            ...
            if(block.timestamp.sub(stakingTime[_staker].add(unstakeTime)) < duration_interval){
                _numIntervals = 0;
            }
