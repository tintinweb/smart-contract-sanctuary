...
contract StakingPool is AccessControl, ReentrancyGuard {
    ...
    event Deposit(address indexed account, uint256 indexed stakeId, uint256 sssAmount, uint256 vesssAmount, uint256 duration);
    ...
    function deposit(uint256 sssAmount, uint256 duration) external nonReentrant {
        ...
        uint256 vesssAmount = _sssToVesss(sssAmount, duration);
        ...
        emit Deposit(_msgSender(), asset.stakeId, sssAmount, vesssAmount, duration);
    }
    ...
    function harvestAll(address to) public {
        for(uint i = 0; i < rewardList.length; i++) {
            ...
            // console.log("token:%s, _pendingReward: %d", rewardList[i], _pendingReward);
