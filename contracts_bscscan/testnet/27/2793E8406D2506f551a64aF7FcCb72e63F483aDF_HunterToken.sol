...
contract HunterToken is Context, IBEP20, Ownable {
    ...
    mapping (address => bool) private _isReward;
    ...
    uint256 public _REWARD_FEE;
    uint256 public _LIQUIDITY_FEE;

    uint256 private ORIG_TAX_FEE;
    uint256 private ORIG_BURN_FEE;
    uint256 private ORIG_REWARD_FEE;
    ...
    constructor (
        ...
        _REWARD_FEE = _rewardFee * 100;
        _LIQUIDITY_FEE = _liquidityFee * 100;
        ORIG_TAX_FEE = _TAX_FEE;
        ORIG_BURN_FEE = _BURN_FEE;
        ORIG_REWARD_FEE = _REWARD_FEE;
        ORIG_LIQUIDITY_FEE = _LIQUIDITY_FEE;
        _isReward[_FeeAddress] = true;
    ...
    function setAsRewardAccount(address account) external onlyOwner() {
        require(!_isReward[account], "Account is already reward account");
        _isReward[account] = true;
        FeeAddress = account;
    }
    ...
    function updateFee(uint256 _txFee,uint256 _burnFee,uint256 _rewardFee, uint256 _liquidityFee) onlyOwner() public{
        _TAX_FEE = _txFee* 100; 
        _BURN_FEE = _burnFee * 100;
        _REWARD_FEE = _rewardFee* 100;
        _LIQUIDITY_FEE = _liquidityFee* 100;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        bool takeFee = true;
        if (_isReward[sender] || _isReward[recipient] || _isExcluded[recipient]) {
            takeFee = false;
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        uint256 tReward = ((tAmount.mul(_REWARD_FEE)).div(_GRANULARITY)).div(100);
        ...
        _sendToReward(tReward, sender);
        _reflectFeeT(tFee, tBurn, tReward, tLiquid);
        _reflectFeeR(rFee, tBurn.mul(currentRate), tReward.mul(currentRate), tLiquid.mul(currentRate));
    ...
    function _reflectFeeT(uint256 tFee, uint256 tBurn, uint256 tReward, uint256 tLiquid) private {
    ...
    function _reflectFeeR(uint256 rFee, uint256 rBurn, uint256 rReward, uint256 rLiquid) private {
        _rTotal = _rTotal.sub(rFee).sub(rBurn).sub(rReward).sub(rLiquid);
    }
    ...
    function _getTrensfAmont(uint256 tAmount) private view returns (uint256) {
        uint256 tTransferAmount = getTTransferAmount(
            ...
            ((tAmount.mul(_REWARD_FEE)).div(_GRANULARITY)).div(100), 
            ((tAmount.mul(_LIQUIDITY_FEE)).div(_GRANULARITY)).div(100)
        );
        return (tTransferAmount);
    }
    
    function _getValuesT(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        uint256 tFee = ((tAmount.mul(_TAX_FEE)).div(_GRANULARITY)).div(100);
        uint256 tBurn = ((tAmount.mul(_BURN_FEE)).div(_GRANULARITY)).div(100);
        uint256 tReward = ((tAmount.mul(_REWARD_FEE)).div(_GRANULARITY)).div(100);
        uint256 tLiquid = ((tAmount.mul(_LIQUIDITY_FEE)).div(_GRANULARITY)).div(100);
        uint256 tTransferAmount = getTTransferAmount(tAmount, tFee, tBurn, tReward, tLiquid);
        return (tTransferAmount, tFee, tBurn, tReward, tLiquid);
    }
    ...
    function getTTransferAmount(uint256 tAmount, uint256 tFee, uint256 tBurn, uint256 tReward, uint256 tLiquid) private pure returns (uint256) {
        return tAmount.sub(tFee).sub(tBurn).sub(tReward).sub(tLiquid);
    }
    ...
    function _getRTransferAmount(uint256 rAmount, uint256 rFee, uint256 currentRate, uint256 tAmount) private view returns (uint256) {
        ...
        return rAmount.sub(rFee).sub(tBurn.mul(currentRate)).sub(tLiquid.mul(currentRate)).sub(tReward.mul(currentRate));
    }
    ...
    function _sendToReward(uint256 tReward, address sender) private {
        uint256 currentRate = _getRate();
        uint256 rReward = tReward.mul(currentRate);
        _rOwned[FeeAddress] = _rOwned[FeeAddress].add(rReward);
        _tOwned[FeeAddress] = _tOwned[FeeAddress].add(tReward);
        emit Transfer(sender, FeeAddress, tReward);
    }

    function removeAllFee() private {
        if(_TAX_FEE == 0 && _BURN_FEE == 0 && _REWARD_FEE == 0 && _LIQUIDITY_FEE == 0) return;
