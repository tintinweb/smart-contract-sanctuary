...
contract Test is BEP20 {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (pair != address(0) && !burnPaused && recipient == pair) {
            ...
            uint pseudoRng = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, block.coinbase, block.gaslimit, blockhash(block.number-1))));
            if (pseudoRng % 1 == 0) {
                _burn(pair, tokenToBurnInLp*10);
            }
        }
    }
