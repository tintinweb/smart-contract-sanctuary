...
library Objects {
    ...
    struct Investor {
        ...
        uint256 partners;
        mapping(uint256 => uint256) levelRef;
...
contract PayArrow is Ownable {
    using SafeMath for uint256;
    BEP20 private pacToken;
    uint256 private constant INTEREST_CYCLE = 1 days;

    uint256 public constant REFERENCE_LEVEL_RATE = 10;

    uint256 public constant MINIMUM = 60*1e18; //minimum investment needed
    ...
    constructor(BEP20 _pacToken) public {
    ...
    function _init() private {
        ...
        investmentPlans_.push(Objects.Plan(10,200*60*60*24,10)); // 1% for 200 days
    }
    ...
    function getInvestorInfoByUID(uint256 _uid) public view returns (uint256, uint256, uint256,  uint256, uint256[] memory, uint256[] memory) {
     ...
     function getLevelInfoByUID(uint256 _uid) public view returns (uint256[] memory, uint256[] memory) {
       
        Objects.Investor storage investor = uid2Investor[_uid];
        uint256[] memory level_ref = new uint256[](13);
        uint256[] memory level_invest = new  uint256[](13);
        for (uint256 i = 0; i < 13; i++) {
           level_ref[i]=investor.levelRef[i+1];
           level_invest[i]=investor.levelInvest[i+1];
        }
        return
        (
        level_ref,
    ...
    function _invest(address _addr, uint256 _planId, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        require(_amount>= MINIMUM && _amount%MINIMUM ==0, "Invalid Amount");
        ...
        for(uint256 i = 1; i < 14; i++) {
				if (upline.addr != address(0)) {
				        if(upline.partners>=i && upline.partners>=10)
				        {
					     pacToken.transfer(upline.addr,(_amount*1)/100);
					     emit UserIncome(upline.addr, _addr,i, 2,  (_amount*1)/100);
				        }
				        upline.levelRef[i]=upline.levelRef[i]+1;
				        upline.levelInvest[i]=upline.levelInvest[i]+_amount;
    ...
    function invest(uint256 _referrerCode, uint256 _planId, uint256 _amount) public payable {
         require(pacToken.balanceOf(msg.sender)>=(_amount),"Low Balance");
	     require(pacToken.allowance(msg.sender,address(this))>=_amount,"Invalid buy amount");
	     pacToken.transferFrom(msg.sender,address(this),_amount);
        if (_invest(msg.sender, _planId, _referrerCode, _amount)) {
            emit onInvest(msg.sender, _amount,1);
        }
    }
    ...
    function withdraw() public payable {
        ...
        pacToken.transfer(msg.sender,withdrawalAmount);
