...
contract PM is IERC721Receiver, Ownable {
        ...
        returns(string[] memory,uint256[] memory,address[] memory,uint256)
    {
       ...
       uint256 _tokenLength = 0 ; 
       string[] memory _tokenSymbol  ; 
       uint256[] memory _tokenFee  ; 
       address[] memory _tokenAddress  ; 
       for(uint256 i = 0 ; i < tokenCount ; i++ ){
           address _token =  sellTokenArray[i] ;
           if(sellToken[_token]){
               _tokenSymbol[_tokenLength] =  IERC20(_token).symbol();
               _tokenFee[_tokenLength] = tokenfee[_token];
               _tokenAddress[_tokenLength] = _token;
               _tokenLength++ ; 
           }
       }
        
       return (_tokenSymbol,_tokenFee,_tokenAddress,_tokenLength) ;
