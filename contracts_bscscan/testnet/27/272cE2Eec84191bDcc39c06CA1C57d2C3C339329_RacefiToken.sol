...
contract RacefiToken is Initializable, ERC20, AccessControl, ReentrancyGuard {
    ...
    AddressesLib.Addresses private _vestingAddresses;
    ...
    function addVestingToken(
        ...
        require(projectedSupply + _amount <= TOTAL_SUPPLY, "Racefi: Max supply exceeded");
        ...
        _vestingAddresses.push(_beneficiary);
    ...
    function revokeVestingToken(address _beneficiary) external onlyAdmin {
        ...
        if (claimableAmount > 0) {
            require(netSupply + claimableAmount <= TOTAL_SUPPLY, "Racefi: Max supply exceeded");
        ...
        if (claimedAmount == 0) _vestingAddresses.remove(_beneficiary);
    }
    ...
    function getVestingList() external view returns (VestingInfo[] memory) {
        VestingInfo[] memory vestingList = new VestingInfo[](_vestingAddresses.size());

        for (uint256 i = 0; i < _vestingAddresses.size(); i++) {
            address lockerAddress = _vestingAddresses.at(i);
            vestingList[i] = _vestings[lockerAddress];
        }
