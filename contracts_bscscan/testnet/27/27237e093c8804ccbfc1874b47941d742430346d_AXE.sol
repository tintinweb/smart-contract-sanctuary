...
* 2% operation/dev/marketing wallet
* 1% research wallet
*
...
contract AXE is Context, IERC20, Ownable {
    ...
    string private constant _name = "AxenWhCr";
    string private constant _symbol = "AXE";
    uint8 private constant _decimals = 9;

    uint256 private constant MAX = ~uint256(0);
    uint256 private constant _tTotal = 25 * 10**7 * 10**_decimals;
    ...
    feeRatesStruct public feeRates =
        feeRatesStruct({rfi: 2, liquidity: 3, research: 1, dev: 2}); //32 bytes - perfect, as it should be
    ...
    uint256 public _maxTxAmount = 3 * 10**3 * 10**_decimals;
        ...
        )
    {
        ...
        rTransferAmount = rAmount.sub(rRfi).sub(rLiquidity).sub(rResearch).sub(
            rDev
        );
    ...
    function _transfer(
        ...
        require(
            ...
            "Insuf balance, check balance at SafeSale.finance if you have token lock"
        );
        //Exclude owner and presale wallets from maxTxAmount.
        if (
            (from != owner() && to != owner()) &&
            (!isPresaleWallet[from] && !isPresaleWallet[to])
        )
            require(
                amount <= _maxTxAmount,
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (swapTokensForBNB(half)) {
            //enough liquidity ? If not, no swapLiq
        ...
        returns (bool status)
    {
        ...
        if (
            allowance(address(this), address(PancakeSwapV2Router)) < tokenAmount
        ) {
