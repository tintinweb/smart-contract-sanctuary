...
contract TEST3 is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if(to != owner() && (from == uniswapV2Pair)) require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
        
        if(from != owner() && (to == uniswapV2Pair)) require(amount <= _maxSellAmount, "Transfer amount exceeds the maxTxAmount."); 
