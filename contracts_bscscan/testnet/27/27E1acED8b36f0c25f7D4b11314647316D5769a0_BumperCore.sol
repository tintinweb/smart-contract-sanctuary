...
contract BumperCore is Initializable, Ownable, ReentrancyGuard, IBumperCore {
  ...
  mapping(address => uint256) public minColRatioMap;
...
function createBToken1(
    ...
    bytes32 salt = keccak256(abi.encodePacked(_col, _paired, _expiry, _mintRatioStr));
    address proxyAddr = Clones.cloneDeterministic(bERC20Impl, salt);
    ...
    IBTokenProxy(proxyAddr).initialize("Bumper Protocol bToken", symbol, 18);
    tokenList.push(proxyAddr);
    emit BTokenCreated(proxyAddr, symbol);
  }

function createBToken2(
    address _col,
    address _paired,
    uint256 _expiry,
    string calldata _expiryStr,
    ...
    IBTokenProxy(proxyAddr).initialize("Bumper Protocol bToken", "WSSS2", 18);
    tokenList.push(proxyAddr);
    emit BTokenCreated(proxyAddr, "wsss");
  }
  ...
  function addPair(
    ...
    _createBToken(_col, _paired, _expiry, _expiryStr, _mintRatioStr, "BC_");
    _createBToken(_col, _paired, _expiry, _expiryStr, _mintRatioStr, "BR_");
  }

  function deposit(address token, uint256 colAmt) public {
    IBERC20(token).mint(msg.sender, colAmt);
  }
...
//     emit Deposit(msg.sender, _col, _paired, _expiry, _mintRatio, received, mintAmount);
//   }
...
interface IBumperCore {
  event BTokenCreated(address token, string symbol);
  ...
  event Deposit(address indexed user, address indexed collateral, address indexed paired, uint48 expiry, uint256 mintRatio, uint256 amount, uint256 tmp1);
