...
contract Escrow is IArbitrable, IEvidence, Ownable {
    ...
    function payeeClaimFunds(uint256 _txID) public {
        TX storage transaction = txs[_txID];
        ...
        if (
            msg.sender != transaction.payee && block.timestamp - transaction.createdAt <= transaction.reclamationPeriod
        ) {
            revert ReleasedTooEarly(); // Note that this will be ReleasedTooEarly() if anyone other than the payee calls this function
        }
        transaction.status = Status.Resolved;
        transaction.payee.transfer(transaction.value);
    }
    ...
    function payArbitrationCost(uint256 _txID) public payable {
        TX storage transaction = txs[_txID];
        uint256 requiredAmount = transaction.arbitrator.arbitrationCost(transaction.value); // Send arbitrator contract value of initial escrow contract to get arbitration cost
    ...
    function depositArbitrationFeeForPayee(uint256 _txID) public payable {
        ...
        transaction.disputeID = transaction.arbitrator.createDispute{value: msg.value}(numberOfRulingOptions, 0);
