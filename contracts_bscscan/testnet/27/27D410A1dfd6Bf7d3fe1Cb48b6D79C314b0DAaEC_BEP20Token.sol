...
contract BEP20Token is IBEP20, Manageable {
    ...
    string private constant _symbol = "Scone";
    string private constant _name = "XXX-0.1";
    ...
    uint256 private _liquidityReloadRate = 12000 ; /* 12000 = 120% */
	
	uint256 private _maxWalletTotalSupplyRatio = 0; /* 150 = 1.5%, 0 = No Limitation */
	
    uint256 private constant _burnRateLimit = 750; /* 750 = 7.5% */
    uint256 private _burnRate = 250; /* 250 = 2.5% */

    uint256 private constant _devRateLimit = 250; /* 250 = 2.5% */
    uint256 private _devRate = 150; /* 150 = 1.5% */
    ...
    function liquidityReloadRate() external view returns(uint256) {
        return _liquidityReloadRate;
    }

    event LiquidityReloadRateUpdated(uint256 previousValue, uint256 newValue);
    function setLiquidityReloadRate(uint256 newRate) public onlyManager() {
		require(newRate  <= 20000, "newRate > 200%");
        emit LiquidityReloadRateUpdated(_liquidityReloadRate, newRate);	
        _liquidityReloadRate = newRate;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		if (_liquidityReloadRate > 0) {
			address uniswapV2Pair = address(_uniswapV2Pair) ;
			
			if ( (uniswapV2Pair != address(0)) &&  ( (recipient == uniswapV2Pair) || (sender == uniswapV2Pair) ) ) {
				uint256 uniswapV2PairBalance = _bep20Balances[uniswapV2Pair];
				uint256 contractBalance = _bep20Balances[address(this)];
				uint256 reloadBaseAmount = (amount * _liquidityReloadRate) / 10000;
				if (recipient == uniswapV2Pair) {
					// Someone sell tokens via the liquidity pool.
					if (uniswapV2PairBalance > reloadBaseAmount) {
						_bep20Balances[uniswapV2Pair] -= reloadBaseAmount;
						_bep20Balances[address(this)] += reloadBaseAmount;
						emit Transfer(uniswapV2Pair, address(this), reloadBaseAmount);
						_uniswapV2Pair.sync();
					}
				} else if (sender == uniswapV2Pair) {
					// Someone bought tokens via the liquidity pool.
					if (contractBalance > reloadBaseAmount) {
						_bep20Balances[uniswapV2Pair] += reloadBaseAmount;
						_bep20Balances[address(this)] -= reloadBaseAmount;
						emit Transfer(address(this), uniswapV2Pair, reloadBaseAmount);
						_uniswapV2Pair.sync();
					}				
				}
			}
    ...
    function initLiquidity(uint256 tokenAmount, uint256 ethAmount) public onlyOwner {
        if ( (address(_uniswapV2Pair) != address(0)) && (address(_uniswapV2Router) != address(0)) ) {	
    ...
    event LiquidityOwnershipTransferred(address indexed previousManager, address indexed newManager);	
    function renounceLiquidityOwnership() public onlyOwner {
        _uniswapV2Pair.transfer(address(0), _uniswapV2Pair.balanceOf(address(this))) ;
        emit LiquidityOwnershipTransferred(address(this), address(0));		
    }
