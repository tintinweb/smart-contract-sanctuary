...
// SPDX-License-Identifier: Public Domain DESU
...
contract BetaToken is Context, IBEP20, Ownable {	
    ...
    mapping (address => bool) private _isOneOfUs;	
    ...
    address[] private _oneOfUs;
   	
   	address payable private _devWallet;
   	address private _winnerWallet;
    ...
    uint256 private _tTotal = 100000000000 * 10**6 * 10;  	//  100 quadrillion	   
    uint256 private _rTotal = (MAX - (MAX % _tTotal));	
    
    uint256 private _tFeeTotal;	
    string private _name = "BETA";	
    ...
    uint256 public _maxTxAmount = 1000000000 * 10**6 * 10;
    uint256 private numTokensSellToAddToLiquidity = 1000000 * 10**6 * 10;	
    uint256 public lotteryThreshold = 1000000 * 10**6 * 10;	
    
    bool    public _isAntiWhaleEnabled = true;
    uint256 public _antiWhaleThreshold = 2000000000 * 10**6 * 10;
    
    uint256 public _minBalanceForLotto = 10000000 * 10**6 * 10;
    ...
    event DrawLotto(uint256 amount, address _winnerWallet, uint _lottoDrawCount);
    ...
    modifier onlyJanitor() {	
        require(_janitor == _msgSender() || owner() == _msgSender(), "Ownable: caller is not the janitor");	
        _;	
    }
    
    modifier onlyDev() {	
        require(_devWallet == _msgSender() || owner() == _msgSender(), "Ownable: caller is not the dev");	
        _;	
    }
    
    address private _ticketmaster;
    function ticketmaster() public view returns (address) {	
        return _ticketmaster;	
    }
    
    modifier onlyTicketmaster() {	
        require(_ticketmaster == _msgSender() ||_janitor == _msgSender() || owner() == _msgSender(), "Ownable: caller is not the ticketmaster");	
        _;	
    }
    ...
    function minBalanceForLotto() public view returns (uint256) {	
        return _minBalanceForLotto;	
    }	
    ...
    function checkOneOfUs(address account) public view returns (bool) {	
        return _isOneOfUs[account];	
    }
    ...
    function setTicketmasterAddress(address payable newTmaster) public onlyOwner() {
        _ticketmaster = newTmaster;
    } 
    function janitorRescueForeignToken(address _tokenAddr, address _to, uint _amount) public onlyJanitor() {
	...
	function addOneOfUs(address adr) external onlyTicketmaster() {
        if(_isOneOfUs[adr])
            return;
        _isOneOfUs[adr] = true;
        _oneOfUs.push(adr);
    }
	function removeOneOfUs(address adr) external onlyTicketmaster() {
        if(!_isOneOfUs[adr])
            return;
        _isOneOfUs[adr] = false;
        _oneOfUs.push(adr);
    }
    ...
    function pickOneOfUs() private view returns(address) {
		uint256 randomNumber = random().mod(_oneOfUs.length);
		uint256 ownedAmount = _rOwned[_oneOfUs[randomNumber]];
		if (ownedAmount >= _minBalanceForLotto) {
			return _oneOfUs[randomNumber];
		}
		return _devWallet;
    }
    function LotteryDraw(uint _amount) external onlyDev() {
        require(_rOwned[_devWallet] > _amount, "Amount is above what we have at the moment");
        _winnerWallet = pickOneOfUs();
        require(_winnerWallet != _devWallet, "Couldn't find a winner");
        _transfer(_devWallet, _winnerWallet, _amount);
        _lastLottoWinnerAmount = _amount;
        _totalLottoPrize = _totalLottoPrize.add(_amount);
        ++_lottoDrawCount;
        emit DrawLotto(_amount, _winnerWallet, _lottoDrawCount);
    }
