...
contract VelaMarketplace is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => mapping(uint256 => uint256)) public historyLengths; // erc721 address => token id => length
    ...
    function listingPrice(address erc721, address erc20, uint256 tokenId, uint256 price) public whenNotPaused nonReentrant inWhitelist(erc721, erc20){
        ...
        // Add history
        uint256 length = historyLengths[erc721][tokenId];
        if (length == 0){
            length = 1;
        }else{
            length++;
        ...
        bytes32 _bockhash = blockhash(block.number);
        histories[erc721][tokenId][length] = History("listingPrice", _bockhash, erc721, msgSender, address(0), erc20, price, false, 0,0, block.timestamp);
    }

    // request: add buy history
