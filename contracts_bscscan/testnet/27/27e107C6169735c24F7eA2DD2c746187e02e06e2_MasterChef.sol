...
contract YerbamateToken is ERC20('Yerba Mate', 'YERBAMATE') {
    ...
    /// @dev Max holding rate in basis point. (default is 5% of total supply)
    ...
    function isExcludedFromBlacklist(address _account) public view returns (bool) {
        return blacklist[_account];
    }

    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        if (recipient == address(0) || sender == DEV_ADDRESS || recipient == FEE_ADDRESS || _excludedFromAntiWhale[sender]) {
