...
contract StakePool is ReentrancyGuard, Ownable, Pausable {
    ...
    constructor(address _stakingToken) public {
        ...
        // Add Tiers
        addTier(uint(20000 ether), 750, uint(1 days));
        addTier(uint(10000 ether), 300, uint(1 days));
        addTier(uint(5000 ether), 130, uint(1 days));
        addTier(uint(2000 ether), 40, uint(1 days));
        addTier(uint(1000 ether), 15, uint(1 days));
    }
