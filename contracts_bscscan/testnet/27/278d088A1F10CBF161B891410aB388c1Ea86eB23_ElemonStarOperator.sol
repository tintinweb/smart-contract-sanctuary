...
interface IElemonInfo{
    ...
    function _tokenInfos(uint256 tokenId) external returns(Info memory);
...
contract ElemonStarOperator is ReentrancyGuard, Runnable{
    ...
    function upgradeStar(uint256 tokenId, uint256 star) external nonReentrant whenRunning{
        ...
        IElemonInfo.Info memory info = _elemonInfo._tokenInfos(tokenId);
        require(info.baseCardId != 0, "Invalid tokenId");
        require(info.star + 1 == star, "Invalid star");
