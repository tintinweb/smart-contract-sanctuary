...
Pyro is built upon the fundamentals of Buyback and increasing the investor's value

Main features are

1) 2% tax is collected and distributed to holders for HODLing
2) 8% buyback and marketing tax is collected and 3% of it is sent for marketing and dev fund and othe 5% is used to buyback the tokens
3) Massive tax for sellers, be able to enable and disable tax                                                                    
*/
...
contract Pyro is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) private _rOwned; // reflection toal supply
    mapping (address => uint256) private _tOwned; // Actual total supply
    ...
    uint256 private _tTotal = 10**9 * 10**9;  // Total Supply 1,000,000,000
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Pyro(onPlanet.io)";
    string private _symbol = "PYRO";
    uint8 private _decimals = 9;  // why 18  or 9 web3.parseUnit(defulat( decimal 18 , 9))
    ...
    uint256 public _maxSellCount = 5;  // 1 normal tax 10% , 5 max tax 42% for 5 times more seller, after one day it recovers to 10%
    uint256 public _maxTxAmount = 5000000 * 10**9; // Limit transfer amount for block big whales
    uint256 private minimumTokensBeforeSwap = 500000 * 10**9; // Collect 500,000 tokens and swap
    uint256 private buyBackUpperLimit = 1 * 10**18;  // 1 bnb buyback ( In auto mode )
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && to == uniswapV2Pair) {
	        ...
	        uint256 balance = address(this).balance;  // Get contract BNB balance
            if (buyBackEnabled && balance > uint256(1 * 10**18)) {  // this is keep 1 BNB in contract for gas fee
                
                if (balance > buyBackUpperLimit)
                    balance = buyBackUpperLimit; // set Buyback amount to buyBackUpperLimit
                
                buyBackTokens(balance); // Buyback tokens
            }
    ...
    function manualBuyBackTokens(uint256 amount) public onlyOwner {
    	if (amount > 0) {
            uint256 bnbAmount = amount * 10**18;
