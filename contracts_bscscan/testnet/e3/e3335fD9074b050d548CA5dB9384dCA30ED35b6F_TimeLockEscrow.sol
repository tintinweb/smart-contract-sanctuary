...
contract TimeLockEscrow is Ownable {
    ...
    function claim( address account) external onlyAuth {
        (uint amount, uint claimTime) = this.getReward(account);

        if(amount == 0)
            return;
        
        user[account].lockTime += claimTime;
        user[account].totalRewards += amount;
