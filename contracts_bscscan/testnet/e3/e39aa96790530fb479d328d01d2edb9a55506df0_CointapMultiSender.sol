...
contract CointapMultiSender is Context, Ownable, SafeTransfer {
    ...
    uint256 public SERVICE_BASE_COST = 0.04 * 10**18;
    ...
    function _estFeeTransferBulk(uint256 noOfTxs, address token) internal view returns (uint256) {
        uint256 totalFee;
        ...
        totalFee = (SERVICE_BASE_COST * MULTIPLIER_PREMEUM) + (COST_PER_TX_DIAMOND * (noOfTxs - 50));
        return _feeAfterDiscount(totalFee, token);
    }
   ...
   function transferBulkForReflection(address token, address[] memory _tos, uint256[] memory _values) public payable virtual returns(bool) {
        require(!isDisabled, "MultiSend Bulk: Service stopped by owner");
    ...
    function estFeeTransferBulk(uint256 noOfTxs, address token) external view returns(uint256) {
    ...
    function removeManager(address manager) public virtual onlyOwner returns (bool) {
    ...
    // Handle discount for contract create by CoinTap
    /**
      * 1% => 100
      * 10% => 1000
      * 25,25% => 2525
      * 100% => 10000
     */
    function updateDiscountFor(address newContract, uint256 discountPercent) public virtual onlyManager returns (bool) {
       discounts[newContract] = discountPercent;
       return true;
    }

    function getDiscountPercent(address contractAddr) external view returns(uint256) {
        return discounts[contractAddr];
    }

    function _feeAfterDiscount(uint256 fee, address contractAddr) internal view returns (uint256) {
        return fee * (10000 - discounts[contractAddr]) / 10000;
    }

    function estFeeAfterDiscount(uint256 fee, address contractAddr) external view returns (uint256) {
        return fee * (10000 - discounts[contractAddr]) / 10000;
    }
}