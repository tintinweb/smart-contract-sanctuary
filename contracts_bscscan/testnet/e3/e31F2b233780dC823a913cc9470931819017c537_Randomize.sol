...
contract Randomize is VRFConsumerBase, Ownable {
    
    bytes32 internal keyHash;
    uint256 internal fee;
    mapping (bytes32 => uint256) randomResults;
    mapping (bytes32 => uint256) lotteryIds;
     ...
     * @param _linkToken LINK token address on the corresponding network
    ...
    {
        require(keyHash != bytes32(0), "RANDOMIZE: Must have valid key hash");
        require(
            LINK.balanceOf(address(this)) >= fee, 
            "RANDOMIZE: Not enough LINK"
        );
        requestId = requestRandomness(keyHash, fee);
        lotteryIds[requestId] = lotteryId;
    }
...
interface IPrediction {
    ...
    event NewTicket(address indexed buyer, bool label, uint256 indexed pid, uint256 indexed epoch, uint256 tid, uint256 price, uint256 timestamp);
