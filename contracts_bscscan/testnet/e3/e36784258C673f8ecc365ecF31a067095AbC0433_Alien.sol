...
contract Alien is ERC721Enumerable {
    ...
    function random1(uint256 rand) public  view returns (uint256) {
    ...
    function random(uint256 rand) internal view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(
        tx.origin,
        blockhash(block.number - 1),
        block.timestamp,
    ...
    function generate(uint256 _next_alien, uint256 rand, uint8 _alien) internal {
        ...
        generate(_next_alien, random(rand), _alien);
    }
