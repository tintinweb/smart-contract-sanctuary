...
contract MyDiamondTeam {
   ...
   function changeTeamName(uint256 teamId, string memory name) external {
    ...
    function _checkNickname(string memory name) private view returns (bool result){
    ...
    function checkNickname(string memory name) external view returns (bool result){
        return _checkNickname(name);
    }

    function getAddressToNickname(string memory name) private view returns (address){
        for( uint256 i = 0; i < total_users; i++){
            string memory nick = nicknames[id2Address[i]];
            if( strcmp(nick, name)){
                return id2Address[i];
