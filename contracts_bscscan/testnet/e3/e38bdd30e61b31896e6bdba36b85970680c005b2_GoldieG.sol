...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    ...
    contract GoldieG is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "GoldieG";
    string private _symbol = "GGOL";
    ...
    // this goes to a specific wallet
    uint256 public _devFee = 400;
    // this goes to a specific wallet
    uint256 public _operationFee = 100;
    // this goes to a specific wallets
    ...
    uint256 public _operationFeeTotal;
    uint256 public _marketingFeeTotal;

    //assigns wallet adresses
    ...
    constructor() public {
        ...
        devWallet = 0xd3cC017a3F6eA6a91725108692c9f5A644D28E67;
        operationWallet = 0xCb8dE84F5F3d8A172Bc71881E3BD06e0682627D7;
        marketingWallet = 0x2E7853E3C389f58f1423dD704e24c55Fa86f10a7;
        //paste the wallet ^^^^^^^^^^^^^^^^^^^^^ adress, that earns the marketingFee here
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        //@dev operation fee
         if(_operationFee != 0){
            uint256 operationFee = amount.mul(_operationFee).div(10**(_feeDecimal + 2));
            transferAmount = transferAmount.sub(operationFee);
            _reflectionBalance[operationWallet] = _reflectionBalance[operationWallet].add(operationFee.mul(rate));
            if (_isExcluded[operationWallet]) {
                _tokenBalance[operationWallet] = _tokenBalance[operationWallet].add(operationFee);
            }
            _operationFeeTotal = _operationFeeTotal.add(operationFee);
            emit Transfer(account,operationWallet,operationFee);
        }
    ...
    function setOperationFee(uint256 amount) external onlyOwner {
        _operationFee = amount;
    }
