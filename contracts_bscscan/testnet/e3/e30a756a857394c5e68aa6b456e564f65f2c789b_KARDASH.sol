...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minDistribution = 1 * (10 ** 18); //10 ** 14 = 0.0001 BNB
    ...
    function deposit() external payable override {
        ...
        //uint256 amount = IWBNB(WBNB).balanceOf(address(this));
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            //assert(IWBNB(WBNB).transfer(shareholder, amount));
...
contract KARDASH is IBEP20, Auth {
    ...
    uint256 swapThreshold = _totalSupply / 8000; // 0.025%
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        if(launchedAt + 1 >= block.number){ return feeDenominator.sub(1); } // Fuck the sniping bots :D:D:D:D:D
    ...
    function getFees() external view returns (bool,uint256,uint256,uint256,uint256) {
        return (
            feeEnabled,
            reflectionFee,
            fundingFee,
