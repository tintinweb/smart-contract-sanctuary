...
contract Auction is Context, Ownable {
	...
	function claims(uint256 _auctionId) public {
	    ...
	    require(claim[msg.sender][_auctionId] > 0,"No bid found");
	...
	function buy(uint256 _tokenId) public payable  {
	     ...
	     payable(nft_token.ownerOf(_tokenId)).transfer(msg.value);
	     emit Buy(msg.sender, _tokenId, nft_token.ownerOf(_tokenId), msg.value, block.timestamp);
    }
	...
	function cancelSell(uint256 _tokenId) public onlyOwner{
	     nft_token.transferFrom(address(this), msg.sender, _tokenId);
