...
contract AbstractSale is Ownable {
    ...
    function prepareMessage(
        ...
        if (sellerFee != 0) {
            return result.append(". sellerFee: ", sellerFee.toString());
        } else {
            return result;
        }
    }
...
contract AuctionERC721V2 is Ownable, IERC721Receiver, AbstractSale {
    using AddressLibrary for address;
    using UintLibrary for uint256;
    using StringLibrary for string;
    uint256 private constant maxBuyerFeePercentage = 10000;
    ...
    constructor(TransferProxy _transferProxy, AuctionERC20TransferProxy _erc20TransferProxy, AuctionERC721SaleNonceHolder _nonceHolder, address payable beneficiary, address _systemSigner) public AbstractSale(beneficiary) {
    ...
    function changeSystemSigner(address _newSystemSigner) public {
        require(owner() == msg.sender, "only owner can call");
        require(_newSystemSigner != address(0), "New signer can't be zero address");
        systemSigner = _newSystemSigner;
        emit SystemSignerChanged(systemSigner, _newSystemSigner);
    }
    ...
    function closeAuction(Asset memory asset, Trade memory trade, BuyerSig memory buyerSig, SystemSig memory systemSig) public payable {
        ...
        verifyBidderSign(asset, trade, buyerSig, nonce);
        verifySystemSign(asset, trade, systemSig, nonce);
        ...
        if(creatorFee != 0) {
            creator = getCreator(asset.token, asset.assetTokenId);
            creatorSlash = offeredPrice.mul(creatorFee).div(maxBuyerFeePercentage);
    ...
    function verifyBidderSign(Asset memory asset, Trade memory trade, BuyerSig memory buyerSig, uint256 nonce) internal pure {
        string memory messageSigned = prepareMessage(address(asset.token), asset.assetTokenId, trade.offeredPrice, trade.sellerFee, nonce);
        address isBidder = StringLibrary.recover(messageSigned, buyerSig.v, buyerSig.r, buyerSig.s);
        require(trade.buyer == isBidder, "owner should sign correct message");
    }
    
    function verifySystemSign(Asset memory asset, Trade memory trade, SystemSig memory systemSig, uint256 nonce) internal view {
        ...
        require(systemSigner == isSystemSigner, "system signer should sign correct message");
    }
