...
contract CompanyReserveGovernor is Governor, GovernorCompatibilityBravo , GovernorTimelockControl {
    ...
    IERC20 posi = IERC20(0xb228359B5D83974F47655Ee41f17F3822A1fD0DD); //TODO change in mainnet
    uint256 private _delay = 10;
    uint256 private _period = 20;
    uint256 private _threshold = 0;
    uint256 private _quorum = 1000;
    // Dev only has right to propose a proposal
    address public dev;
    ...
    constructor(TimelockController _timelock)
        Governor("CompanyReserveGovernor")
        ...
        returns (uint256)
    {
        ...
        proposalReward[proposalId] = ProposalReward(totalReward, totalCountedReward, 0);
        return proposalId;
    }
    ...
    ) internal virtual override(Governor) returns (uint256)
    {
        ...
        if(proposalReward[proposalId].totalDistributed < proposalReward[proposalId].totalCountedReward){
            uint256 myReward = _weight.mul(proposalReward[proposalId].totalReward).div(proposalReward[proposalId].totalCountedReward);
            try posi.transfer(_msgSender(), myReward) {
            }catch {
            }
