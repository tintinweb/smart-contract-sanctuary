...
contract VestingContract is ReentrancyGuard{ 
    ...
    RDetails R1Contract = RDetails(0xEf1CD9EF49E4e1d1451391f50D109F5543919389); // R1 address
    RDetails R2Contract = RDetails(0x53C03f5313C6Ab9FC80DAee3864B87578195cF38); // R2 address
    IERC20 OmniaToken = IERC20(0x78b9f99a712BE6824550e402fa83D8Eb2353b76a); //OMNIA address
    ...
    function totalOminaAllocatedToR1(address beneficiary) public view returns(uint256) {
        
        require(R1Contract.omniaAllocatedTo(beneficiary)>0, "XV1");
        
        return R1Contract.omniaAllocatedTo(beneficiary);
        
    }
    
    function totalOminaAllocatedToR2(address beneficiary) public view returns(uint256) {
        
        require(R2Contract.omniaAllocatedTo(beneficiary)>0, "XV1");
        
        return R2Contract.omniaAllocatedTo(beneficiary);
        
    }
    
    function totalPendingToR1(address beneficiary) public view returns(uint256) {
        
        require(R1Contract.omniaAllocatedTo(beneficiary)>0, "XV1");
        
        return R1Contract.omniaAllocatedTo(beneficiary)-totalClaimedR1[beneficiary];
        
    }
    
     function totalPendingToR2(address beneficiary) public view returns(uint256) {
        
        require(R2Contract.omniaAllocatedTo(beneficiary)>0, "XV1");
        
        return R2Contract.omniaAllocatedTo(beneficiary)-totalClaimedR2[beneficiary];
    ...
    function withdrawTokensR1(address beneficiary) nonReentrant() public {
        ...
        uint256 amount = (((block.timestamp-contractCreationTime)/monthTime)+1)*3*totalAllocation/100;
    ...
    function withdrawTokensR2(address beneficiary) nonReentrant() public {
        ...
        require(R2Contract.omniaAllocatedTo(beneficiary)>0, "XV3");
        
        uint256 totalAllocation = R2Contract.omniaAllocatedTo(beneficiary);
        uint256 amount = (((block.timestamp-contractCreationTime)/monthTime)+1)*5*totalAllocation/100;
