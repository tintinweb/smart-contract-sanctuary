...
contract FINAContract is ERC20('Defina Finance', 'FINA'){
...
import "./FINAContract.sol";

contract server_FINA {
    FINAContract public FINA;
    mapping(address => uint256) _depositedBalancesOf;
    ...
    constructor(FINAContract token) public {
        FINA=token;
    }

    function mint(address receiver,uint256 amount) public {
        FINA.mintTo(receiver,amount);
    }

    function deposit(uint256 amount) public {
        address sender = msg.sender;
        FINA.transferFrom(sender, address(this), amount);
        _depositedBalancesOf[sender] = _depositedBalancesOf[sender].add(amount);
        emit Deposit(amount);
    }

    function withdraw(uint256 amount) public {
        address sender = msg.sender;
        require(amount <= _depositedBalancesOf[sender], "Error: Not Enough Balance to withdraw");
        FINA.transferFrom(address(this),sender, amount);
        _depositedBalancesOf[sender] = _depositedBalancesOf[sender].sub(amount);
        emit Withdraw(amount);
    }

    function approve(uint256 amount) public{
        FINA.approve(msg.sender,amount);
        emit Approve(amount);
    }

    function depositedBalancesOf(address user) public  view returns (uint256) {
        return _depositedBalancesOf[user];
    }
}
