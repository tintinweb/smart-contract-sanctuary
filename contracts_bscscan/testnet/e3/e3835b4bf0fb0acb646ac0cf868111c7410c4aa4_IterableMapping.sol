...
contract APPLEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("APPLE_Dividend_Tracker", "APPLE_Dividend_Tracker") {
        claimWait = 120;
        minimumTokenBalanceForDividends = 10 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "APPLE_Dividend_Tracker: No transfers allowed");
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "APPLE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main APPLE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 60 && newClaimWait <= 86400, "APPLE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "APPLE_Dividend_Tracker: Cannot update claimWait to same value");
...
contract APPLE is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    APPLEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("AppleSwap", "APPLE") {
        BNBRewardsFee = 3;
        liquidityFee = 3;
        extraFeeOnSell = 9; // extra fee on sell
        MarketingFee = 3;
        MarketingWallet = payable(0xd6240cBb935e0364AfE8E976C652ad75FBc0c388);

        totalFees = BNBRewardsFee.add(liquidityFee).add(MarketingFee); // total fee transfer and buy

    	dividendTracker = new APPLEDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "APPLE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "APPLE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "APPLE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "APPLE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "APPLE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "APPLE: Cannot update gasForProcessing to same value");
