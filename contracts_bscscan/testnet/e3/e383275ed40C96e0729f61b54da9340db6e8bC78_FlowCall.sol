...
contract FlowCall is Ownable, Pausable {
            ...
            for (uint256 j = 0; j < callList.length; j++) {
                if (callList[j].seq == i) {
                    ...
                    if(callList[j].callCondition.length>0){
                        Equation.Node[] memory equation=Equation.init(callList[j].callCondition);
                        bool t=equation.calculateBool(toXValues(variableList));
                        if(!t)
                            continue;
                    }
    ...
    function toXValues(DataValue[] memory variableList) private pure returns(uint256[] memory){
