...
contract OmniCashStaking is Ownable, Pausable {
    ...
    constructor () { 
        ...
        // set token address via setTokenAddres
        //OCA$H =  0xb340F67E9Cc3927eBeEB04c2e03f74bd0543F4fc; // mainnet
        //OCA$H = 0x4065f9bb9085fc093682155a43de35e16e6ca07b; // testnet
    }
    ...
    {
        require(OCA$H!=address(0),"Address is not set");
        _idx = numberPools;
    ...
    {
        require(OCA$H!=address(0),"Address is not set");
        ERC20(OCA$H).safeTransferFrom(_staker, coldWallet, _amount);
        ...
        numberStakeTime[_msgSender()] = _sid + 1;
    }
    ...
    returns (uint256 _sid)
    {
        ...
        _sid = executeStake(_msgSender(), _pid, _amount);
    }
    ...
    {
        require(OCA$H!=address(0),"Address is not set");
        uint256 nStakeTime = numberStakeTime[_msgSender()];

        for (uint i = 0; i < nStakeTime; i++) {
            StakingData storage data = stakingData[_msgSender()][i];
            if (data.status == StakingStatus.AVAILABLE) {
                uint256 reward_ = reward(_msgSender(), i);
                IERC20(OCA$H).transfer(_msgSender(), reward_);
                emit Claim(_msgSender(), reward_, i);
                data.stakeFrom = block.timestamp;
                ...
                if (data.stakeTo <= block.timestamp) {
                    IERC20(OCA$H).transfer(_msgSender(), data.balance);
                    emit Withdraw(_msgSender(), i);
    ...
    {
        require(OCA$H!=address(0),"Address is not set");
        uint256 balance = IERC20(OCA$H).balanceOf(coldWallet);
        IERC20(OCA$H).transfer(owner(), balance);
    }
}