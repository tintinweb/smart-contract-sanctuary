...
contract DirectSwap is Ownable  {
    ...
    function addManager(address _sender) public onlyOwner {
    ...
    event RedeemAll(address indexed, uint256);

    function redeemAll(address _token) public onlyOwner {
        uint256 balance = IERC20(_token).balanceOf(address(this));
        require(balance == 0, "No amount");

        IERC20(_token).approve(msg.sender, balance);
        IERC20(_token).safeTransfer(msg.sender, balance);

        emit RedeemAll(_token, balance);
    }
    ...
    function swapDirect(address _from,address _to, uint256 _amount, uint256 _blockNumber) public onlyManager {
