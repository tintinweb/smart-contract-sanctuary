//contracts/mocks/TimeBasedDistributionV2.sol
// SPDX-License-Identifier: MIT

pragma solidity 0.7.6;

import "../wallet/TimeBasedDistribution.sol";

contract TimeBasedDistributionV2 is TimeBasedDistribution {
  function updatedToUpgradeable() external view returns (bool) {
    return true;
  }
}
...
contract TimeBasedDistribution is ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  uint256 public numberOfUnlockPeriod; // number of period to distribute
  ...
  function initialize(
    ...
    require(
      ...
      "TimebasedDistribution: minimum allocation cannot be greater than maximum allocation"
    );
    token = _token;
    ...
    numberOfUnlockPeriod = _numberOfUnlockPeriod;
    ...
    for (uint256 i; i < members.length; i++) {
      ...
      if (member.lockedAmount > 0) {
        ...
        if (member.releaseAmountPerPeriod == 0) {
          member.releaseAmountPerPeriod = member.lockedAmount.div(numberOfUnlockPeriod);
        }
