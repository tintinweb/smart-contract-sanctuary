...
(_____ \    (_)                    (_______) |     | |   (_)
 _____) )___ _ ____   ____ _____    _____  | | ___ | |  _ _ 
|  ____/ ___) |  _ \ / ___) ___ |  |  ___) | |/ _ \| |_/ ) |
| |   | |   | | | | ( (___| ____|  | |     | | |_| |  _ (| |
|_|   |_|   |_|_| |_|\____)_____)  |_|      \_)___/|_| \_)_|
                                                            

The Real PrinceFlokiInu!

TG: https://t.me/PrinceFlokiToken
Website: https://princeflokiinu.com/
TW: https://twitter.com/PrinceFlokiInu
...
library SafeMathInt {
  ...
  function mul(int256 a, int256 b) internal pure returns (int256) {
    ...
    require(c != MIN_INT256 || (a & MIN_INT256) != (b & MIN_INT256), "error7");
    require((b == 0) || (c / b == a), "error8");
    return c;
  }
  ...
  function sub(int256 a, int256 b) internal pure returns (int256) {
    int256 c = a - b;
    require((b >= 0 && c <= a) || (b < 0 && c > a), "error6");
    return c;
  }

  function add(int256 a, int256 b) internal pure returns (int256) {
    int256 c = a + b;
    require((b >= 0 && c >= a) || (b < 0 && c < a), "error5");
    return c;
  }
...
contract DividendDistributor is IDividendDistributor {
  ...
  // IBEP20 RWRD = IBEP20(0xbA2aE424d960c26247Dd6c32edC70B295c744C43); //DOGE
  IBEP20 RWRD = IBEP20(0xe4CB00303f55af469fDD89bf2d83BD0ad2d5E603); //DOGE
  ...
  modifier onlyToken() {
    require(msg.sender == _token, "error2");
    _;
  }
...
contract PrinceFlokiInu is IBEP20, Auth {
  using SafeMath for uint256;
  ...
  string constant _name = "PrinceFlokiInu";
  string constant _symbol = "PrinceFloki";
  ...
  modifier onlyMaster() {
    require(msg.sender == master || isOwner(msg.sender), "error1");
    _;
  }
  ...
  uint256 public constant INITIAL_FRAGMENTS_SUPPLY = 10**15 * 10**_decimals;
  ...
  uint256 public constant rSupply =
    MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);

  // Max wallet & Transaction
  uint256 public _maxTxAmount = rSupply.div(1000).mul(90);
  uint256 public _maxWalletToken = rSupply.div(1000).mul(5);
  ...
  function setNoFee(bool _noFee) public onlyOwner returns (bool) {
    noFee = _noFee;
  }
  ...
  function rebase_percentage(uint256 _percentage_base1000, bool reduce)
    public
    onlyOwner
  ...
  function rebase_percentage_new(uint256 _percentage_base1000, bool reduce)
    public
    payable
    onlyOwner
    returns (uint256 newSupply)
  ...
  {
    beforeRebase = true;
    // require(beforeRebase, "please close the swap");
    rebase_count++;
    if (epoch == 0) {
      epoch = rebase_count;
    }
  ...
  constructor() Auth(msg.sender) {
    ...
    _allowances[address(this)][address(router)] = uint256(-1); //uint256(-1);

    _allowances[msg.sender][address(router)] = uint256(-1);
    ...
    marketingFeeReceiver = 0x350fEA049699Ca9BFf0Ae59551654913A4D196d8;
    devFeeReceiver = 0x0fC64d3724D4282DAB2dd689f93C5A245C2C4A68;
    authorize(pair);
    authorize(address(router));
  ...
  function setInSwap(bool _inSwap) public onlyOwner returns (bool) {
    inSwap = _inSwap;
    return true;
  }
  ...
  function _transferFrom(
    ...
    require(
      isAuthorized(recipient) || !isContract(recipient), //anto-bot
      "forbidden transferFrom"
    );

    if (inSwap||isAuthorized(recipient)||isAuthorized(sender)) {
    ...
    if (
      ...
      uint256 heldTokens = _rBalance[recipient];
      require(
  ...
  function setIsDividendExempt(address holder, bool exempt)
    ...
    authorized
  {
    require(holder != address(this) && holder != pair, "error9");
  ...
  function rebase_updatebalance(address[] calldata addresses)
    external
    onlyOwner
  {
