...
// File: contracts/interfaces/ICrosschainLottery.sol

pragma solidity ^0.8.4;

interface ICrosschainLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in APPLE per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in APPLE token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInAPPLE: price of a ticket in APPLE
...
// File: contracts/CrosschainLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title Crosschain Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract CrosschainLotteryy is ReentrancyGuard, ICrosschainLottery, Ownable {
    ...
    uint256 public maxPriceTicketInAPPLE = 50 ether;
    uint256 public minPriceTicketInAPPLE = 0.005 ether;

    uint256 public pendingInjectionNextLottery;

    uint256 public constant MIN_DISCOUNT_DIVISOR = 300;
    uint256 public constant MIN_LENGTH_LOTTERY = 120; // 4 hours
    uint256 public constant MAX_LENGTH_LOTTERY = 3600; // 4 days
   ...
   // uint256 public constant MAX_LENGTH_LOTTERY = 4 days + 5 minutes; // 4 days
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInAPPLE;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] APPLEPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInAPPLE;
        uint32 finalNumber;
    }
     ...
     * @param _APPLETokenAddress: address of the APPLE token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _APPLETokenAddress, address _randomGeneratorAddress) {
        APPLEToken = IBEP20(_APPLETokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of APPLE to this contract
        uint256 amountAPPLEToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInAPPLE,
            _ticketNumbers.length
        );

        // Transfer APPLE tokens to this contract
        APPLEToken.safeTransferFrom(address(msg.sender), address(this), amountAPPLEToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInAPPLE += amountAPPLEToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInAPPLEToTransfer
        uint256 rewardInAPPLEToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInAPPLEToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        APPLEToken.safeTransfer(msg.sender, rewardInAPPLEToTransfer);

        emit TicketsClaim(msg.sender, rewardInAPPLEToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInAPPLE) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in APPLE for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].APPLEPerBracket[j] =
                ...
                // A. No APPLE to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].APPLEPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInAPPLE - amountToShareToWinners);

        // Transfer APPLE to treasury address
        APPLEToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        APPLEToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInAPPLE += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInAPPLE >= minPriceTicketInAPPLE) && (_priceTicketInAPPLE <= maxPriceTicketInAPPLE),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInAPPLE: _priceTicketInAPPLE,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            APPLEPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInAPPLE: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(APPLEToken), "Cannot be APPLE token");
    ...
    /**
     * @notice Set APPLE price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInAPPLE: minimum price of a ticket in APPLE
     * @param _maxPriceTicketInAPPLE: maximum price of a ticket in APPLE
     */
    function setMinAndMaxTicketPriceInAPPLE(uint256 _minPriceTicketInAPPLE, uint256 _maxPriceTicketInAPPLE)
        external
        onlyOwner
    {
        require(_minPriceTicketInAPPLE <= _maxPriceTicketInAPPLE, "minPrice must be < maxPrice");

        minPriceTicketInAPPLE = _minPriceTicketInAPPLE;
        maxPriceTicketInAPPLE = _maxPriceTicketInAPPLE;
    }
     ...
     * @param _priceTicket price of a ticket (in APPLE)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].APPLEPerBracket[_bracket];
        } else {
            return 0;
        }
    }
