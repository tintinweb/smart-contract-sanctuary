...
contract ElemonIDO is Ownable, ReentrancyGuard {
    ...
    function setUserSlots(address[] memory accounts, uint256[] memory quantities) external onlyOwner{
        require(accounts.length == accounts.length, "Invalid input");
        for(uint256 index = 0; index < accounts.length; index++){
            _userSlots[accounts[index]] = quantities[index];
        }
    }
