    ...
    function _transfer(
        ...
        if(launchTime == 0 && to == uniswapV2Pair){
            ...
            if(antiWhaleEnable && block.timestamp > launchTime && block.timestamp < launchTime + 30*60){
                (uint256 nToken, uint256 nBNB, ) = IUniswapV2Pair(uniswapV2Pair).getReserves();
                uint256 nTokenPerBNB = nToken / nBNB;
                require(amount <= nTokenPerBNB * 10**18, "In 60 min after launching, only number of token worth 1 BNB is allowed");
            }
        }
