...
contract StarlinkX is Context, IERC20, IERC20Extented, Ownable {
    ...
    uint256 private _teamPercent = 10;
    ...
    address payable constant private _projectMaintainence = payable(0x4B9E82d29a17De110494974Ad96b04dE32126480);
    address payable private _teamAddress = payable(0x4B9E82d29a17De110494974Ad96b04dE32126480);
    ...
    event PercentsUpdated(uint256 _marketingPercent, uint256 _buybackPercent, uint256 _teamPercent);
    event FeesUpdated(uint256 _buyBuybackFee, uint256 _buyMarketingFee, uint256 _buyLiquidityFee, uint256 _buyReflectionFee, uint256 _buyTeamFee, uint256 _sellBuyBackFee, uint256 _sellMarketingFee, uint256 _sellLiquidityFee, uint256 _sellReflectionFee, uint256 _sellTeamFee);
    ...
    function removeAllFee() private {
        if (_buyMarketingFee == 0 && _buyBuybackFee == 0 && _buyReflectionFee == 0 && _buyLiquidityFee == 0 && _buyTeamFee == 0 && _sellMarketingFee == 0 && _sellBuybackFee == 0 && _sellReflectionFee == 0 && _sellLiquidityFee == 0 && _sellTeamFee == 0) return;
            ...
            if (!inSwap && from != uniswapV2Pair) { //sells, transfers
                ...
                uint256 contractTokenBalance = balanceOf(address(this));
                    if (contractTokenBalance > 0) {
                        ...
                        uint256 autoLPamount = _sellLiquidityFee.mul(contractTokenBalance).div(_sellBuybackFee.add(_sellMarketingFee).add(_sellLiquidityFee).add(_sellTeamFee));
    ...
    function _getValuesSell(uint256 tAmount, DynamicTax memory currentTax) private view returns (FinalFees memory) {
        SellBreakdown memory sellFees = _getTValuesSell(tAmount, currentTax.sellBuybackFee, currentTax.sellMarketingFee, currentTax.sellReflectionFee, currentTax.sellLiquidityFee, currentTax.sellTeamFee);
        FinalFees memory finalFees;
        uint256 currentRate = _getRate();
        (finalFees.rAmount, finalFees.rTransferAmount, finalFees.rReflection) = _getRValuesSell(tAmount, sellFees.tBuyback, sellFees.tMarketing, sellFees.tReflection, sellFees.tLiquidity, sellFees.tTeam, currentRate);
        ...
        finalFees.tTeam = sellFees.tTeam;
        finalFees.tTransferAmount = sellFees.tTransferAmount;
        return (finalFees);
    }

    function _getTValuesSell(uint256 tAmount, uint256 buybackFee, uint256 marketingFee, uint256 reflectionFee, uint256 liquidityFee, uint256 teamFee) private pure returns (SellBreakdown memory) {
        ...
        tsellFees.tTeam = tAmount.mul(teamFee).div(100);
        ...
        tsellFees.tTransferAmount -= tsellFees.tTeam;
        return (tsellFees);
    }

    function _getRValuesSell(uint256 tAmount, uint256 tBuyback, uint256 tMarketing, uint256 tReflection, uint256 tLiquidity, uint256 tTeam, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function _getValuesBuy(uint256 tAmount, DynamicTax memory currentTax) private view returns (FinalFees memory) {
        BuyBreakdown memory buyFees = _getTValuesBuy(tAmount, currentTax.buyBuybackFee, currentTax.buyMarketingFee, currentTax.buyReflectionFee, currentTax.buyLiquidityFee, currentTax.buyTeamFee);
        FinalFees memory finalFees;
        uint256 currentRate = _getRate();
        (finalFees.rAmount, finalFees.rTransferAmount, finalFees.rReflection) = _getRValuesBuy(tAmount, buyFees.tBuyback, buyFees.tMarketing, buyFees.tReflection, buyFees.tLiquidity, buyFees.tTeam, currentRate);
        ...
        finalFees.tTeam = buyFees.tTeam;
        finalFees.tTransferAmount = buyFees.tTransferAmount;
        return (finalFees);
    }

    function _getTValuesBuy(uint256 tAmount, uint256 buybackFee, uint256 marketingFee, uint256 reflectionFee, uint256 liquidityFee, uint256 teamFee) private pure returns (BuyBreakdown memory) {
        ...
        tbuyFees.tTeam = tAmount.mul(teamFee).div(100);
        ...
        tbuyFees.tTransferAmount -= tbuyFees.tTeam;
        return (tbuyFees);
    }

    function _getRValuesBuy(uint256 tAmount, uint256 tBuyback, uint256 tMarketing, uint256 tReflection, uint256 tLiquidity, uint256 tTeam, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rBuyback = tBuyback.mul(currentRate);
        uint256 rMarketing = tMarketing.mul(currentRate);
        uint256 rReflection = tReflection.mul(currentRate);
    ...
    function setPercents(uint256 marketingPercent, uint256 buybackPercent, uint256 teamPercent) external onlyOwner() {
        require(marketingPercent.add(buybackPercent).add(teamPercent) == 95, "Sum of percents must equal 95");
        _marketingPercent = marketingPercent;
        _buybackPercent = buybackPercent;
        _teamPercent = teamPercent;
        emit PercentsUpdated(_marketingPercent, _buybackPercent, _teamPercent);
    }

    function setTaxes(uint256 buyMarketingFee, uint256 buyBuybackFee, uint256 buyReflectionFee, uint256 buyLiquidityFee, uint256 buyTeamFee, uint256 sellMarketingFee, uint256 sellBuybackFee, uint256 sellReflectionFee, uint256 sellLiquidityFee, uint256 sellTeamFee) external onlyOwner() {
        uint256 buyTax = buyMarketingFee.add(buyBuybackFee).add(buyReflectionFee);
        buyTax += buyLiquidityFee.add(buyTeamFee);
        uint256 sellTax = sellMarketingFee.add(sellBuybackFee).add(sellReflectionFee);
        sellTax += sellLiquidityFee.add(sellTeamFee);
        ...
        emit FeesUpdated(_buyBuybackFee, _buyMarketingFee, _buyLiquidityFee, _buyReflectionFee, _buyTeamFee, _sellBuybackFee, _sellMarketingFee, _sellLiquidityFee, _sellReflectionFee, _sellTeamFee);
    }
    ...
    function startIgnitionHour() external onlyOwner {
        _buyMarketingFee = 5;
        _buyBuybackFee = 3;
        _buyReflectionFee = 2;
        _buyLiquidityFee = 3;
...
function startCriticalEngineHour() external onlyOwner {
        _buyMarketingFee = 0;
        _buyBuybackFee = 0;
        _buyReflectionFee = 0;
        _buyLiquidityFee = 0;
