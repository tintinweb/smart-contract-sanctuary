...
ReentrancyGuardUpgradeable {
    ...
    function calculateAdditionalReward(address user) public returns (uint256) {
        uint256 userBalance = goenBalanceOf(user);
        uint256 enterPoolTimestamp = firstTimeDepositedUser[user];
        uint256 lasttimeUpdateReward = lasttimeUpdateRewardUser[user];
        emit Debug(userBalance, 'userBalance');
        emit Debug(enterPoolTimestamp, 'enterPoolTimestamp');
        emit Debug(lasttimeUpdateReward, 'lasttimeUpdateReward');
        if (enterPoolTimestamp == 0) {
            return 0;
        }
        ...
        if (t1 < 60 days) {
            ...
            if (t2 < 60 days) {
                ...
                emit Debug(secondPhaseReward + userBalance*thirdPhaseDuration/SECOND_IN_YEAR, 'secondPhaseReward + userBalance*thirdPhaseDuration/SECOND_IN_YEAR');
