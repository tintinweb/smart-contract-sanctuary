...
interface IERC20 {
    /**
     * @dev Returns the amount of DarkMatters in existence.
     */
    function totalSupply() external view returns (uint256);

    /**
     * @dev Returns the amount of DarkMatters owned by `account`.
     */
    function balanceOf(address account) external view returns (uint256);

    /**
     * @dev Moves `amount` DarkMatters from the caller's account to `recipient`.
     *
    ...
    /**
     * @dev Returns the remaining number of DarkMatters that `spender` will be
    ...
    /**
     * @dev Sets `amount` as the allowance of `spender` over the caller's DarkMatters.
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
    ...
    /**
     * @dev Moves `amount` DarkMatters from `sender` to `recipient` using the
    ...
    /**
     * @dev Emitted when `value` DarkMatters are moved from one account (`from`) to
     * another (`to`).
     *
 ...
 *
 * This implementation is agnostic to the way DarkMatters are created. This means
...
contract ERC20 is Context, IERC20 {
    ...
    /**
     * @dev Returns the name of the DarkMatter.
     */
    ...
    /**
     * @dev Returns the symbol of the DarkMatter, usually a shorter version of the
     * name.
     */
     ...
     * For example, if `decimals` equals `2`, a balance of `505` DarkMatters should
     * be displayed to a user as `5,05` (`505 / 10 ** 2`).
     *
     * DarkMatters usually opt for a value of 18, imitating the relationship between
     ...
     * - the caller must have allowance for ``sender``'s DarkMatters of at least
     * `amount`.
     */
    ...
    /**
     * @dev Moves DarkMatters `amount` from `sender` to `recipient`.
     *
     * This is internal function is equivalent to {transfer}, and can be used to
     * e.g. implement automatic DarkMatter fees, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        _beforeDarkMatterTransfer(sender, recipient, amount);
    ...
    /** @dev Creates `amount` DarkMatters and assigns them to `account`, increasing
     * the total supply.
     *
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        _beforeDarkMatterTransfer(address(0), account, amount);
    ...
    /**
     * @dev Destroys `amount` DarkMatters from `account`, reducing the
     * total supply.
     *
     ...
     * - `account` must have at least `amount` DarkMatters.
     */
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: burn from the zero address");

        _beforeDarkMatterTransfer(account, address(0), amount);
    ...
    /**
     * @dev Sets `amount` as the allowance of `spender` over the `owner`s DarkMatters.
     *
     ...
     * applications that interact with DarkMatter contracts will not expect
    ...
    /**
     * @dev Hook that is called before any transfer of DarkMatters. This includes
     * minting and burning.
     *
     * Calling conditions:
     *
     * - when `from` and `to` are both non-zero, `amount` of ``from``'s DarkMatters
     * will be to transferred to `to`.
     * - when `from` is zero, `amount` DarkMatters will be minted for `to`.
     * - when `to` is zero, `amount` of ``from``'s DarkMatters will be burned.
     * - `from` and `to` are never both zero.
     *
     * To learn more about hooks, head to xref:ROOT:extending-contracts.adoc#using-hooks[Using Hooks].
     */
    function _beforeDarkMatterTransfer(address from, address to, uint256 amount) internal virtual {}
}
...
/**
 * @title SafeERC20
 * @dev Wrappers around ERC20 operations that throw on failure (when the DarkMatter
 * contract returns false). DarkMatters that return no value (and instead revert or
 * throw on failure) are also supported, non-reverting calls are assumed to be
 * successful.
 * To use this library you can add a `using SafeERC20 for IERC20;` statement to your contract,
 * which allows you to call the safe operations as `DarkMatter.safeTransfer(...)`, etc.
 */
library SafeERC20 {
    using SafeMath for uint256;
    using Address for address;

    function safeTransfer(IERC20 DarkMatter, address to, uint256 value) internal {
        _callOptionalReturn(DarkMatter, abi.encodeWithSelector(DarkMatter.transfer.selector, to, value));
    }

    function safeTransferFrom(IERC20 DarkMatter, address from, address to, uint256 value) internal {
        _callOptionalReturn(DarkMatter, abi.encodeWithSelector(DarkMatter.transferFrom.selector, from, to, value));
    }
     ...
     */
    function safeApprove(IERC20 DarkMatter, address spender, uint256 value) internal {
        // safeApprove should only be called when setting an initial allowance,
        // or when resetting it to zero. To increase and decrease it, use
        // 'safeIncreaseAllowance' and 'safeDecreaseAllowance'
        // solhint-disable-next-line max-line-length
        require((value == 0) || (DarkMatter.allowance(address(this), spender) == 0),
            "SafeERC20: approve from non-zero to non-zero allowance"
        );
        _callOptionalReturn(DarkMatter, abi.encodeWithSelector(DarkMatter.approve.selector, spender, value));
    }

    function safeIncreaseAllowance(IERC20 DarkMatter, address spender, uint256 value) internal {
        uint256 newAllowance = DarkMatter.allowance(address(this), spender).add(value);
        _callOptionalReturn(DarkMatter, abi.encodeWithSelector(DarkMatter.approve.selector, spender, newAllowance));
    }

    function safeDecreaseAllowance(IERC20 DarkMatter, address spender, uint256 value) internal {
        uint256 newAllowance = DarkMatter.allowance(address(this), spender).sub(value, "SafeERC20: decreased allowance below zero");
        _callOptionalReturn(DarkMatter, abi.encodeWithSelector(DarkMatter.approve.selector, spender, newAllowance));
    }
     ...
     * @param DarkMatter The DarkMatter targeted by the call.
     * @param data The call data (encoded using abi.encode or one of its variants).
     */
    function _callOptionalReturn(IERC20 DarkMatter, bytes memory data) private {
        // We need to perform a low level call here, to bypass Solidity's return data size checking mechanism, since
        // we're implementing it ourselves. We use {Address.functionCall} to perform this call, which verifies that
        // the target address contains contract code and also asserts for success in the low-level call.

        bytes memory returndata = address(DarkMatter).functionCall(data, "SafeERC20: low-level call failed");
...
//File: contracts/Token/DarkMatter.sol
