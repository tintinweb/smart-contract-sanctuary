...
// File: contracts/AnpansProfile.sol
...
*/
contract AnpansProfile is AccessControl, ERC721Holder {
    ...
    uint256 public numberAnpanToReactivate;
    uint256 public numberAnpanToRegister;
    uint256 public numberAnpanToUpdate;
    ...
    constructor(
        IBEP20 _anpanToken,
        uint256 _numberAnpanToReactivate,
        uint256 _numberAnpanToRegister,
        uint256 _numberAnpanToUpdate
    ) public {
        anpanToken = _anpanToken;
        numberAnpanToReactivate = _numberAnpanToReactivate;
        numberAnpanToRegister = _numberAnpanToRegister;
        numberAnpanToUpdate = _numberAnpanToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends ANPAN to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer ANPAN tokens to this contract
        anpanToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberAnpanToRegister
        );
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer ANPAN token to this address
        anpanToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberAnpanToUpdate
    ...
    /**
     * @dev Update the number of ANPAN to register
     * Callable only by owner admins.
     */
    function updateNumberAnpan(
        uint256 _newNumberAnpanToReactivate,
        uint256 _newNumberAnpanToRegister,
        uint256 _newNumberAnpanToUpdate
    ) external onlyOwner {
        numberAnpanToReactivate = _newNumberAnpanToReactivate;
        numberAnpanToRegister = _newNumberAnpanToRegister;
        numberAnpanToUpdate = _newNumberAnpanToUpdate;
    }
...
// File: contracts/ClaimBackAnpan.sol

pragma solidity ^0.6.12;

contract ClaimBackAnpan is Ownable {
    using SafeBEP20 for IBEP20;

    IBEP20 public anpanToken;
    AnpansProfile anpansProfile;

    uint256 public numberAnpan;
    ...
    constructor(
        IBEP20 _anpanToken,
        address _anpansProfileAddress,
        uint256 _numberAnpan,
        uint256 _thresholdUser
    ) public {
        anpanToken = _anpanToken;
        anpansProfile = AnpansProfile(_anpansProfileAddress);
        numberAnpan = _numberAnpan;
        thresholdUser = _thresholdUser;
    }

    function getAnpanBack() external {
        // 1. Check if she has registered
        require(anpansProfile.hasRegistered(_msgSender()), "not active");

        // 2. Check if she has claimed
        require(!hasClaimed[_msgSender()], "has claimed ANPAN");

        // 3. Check if she is active
        uint256 userId;
        (userId, , , , , ) = anpansProfile.getUserProfile(_msgSender());
        ...
        // Transfer ANPAN tokens from this contract
        anpanToken.safeTransfer(_msgSender(), numberAnpan);
    }

    /**
     * @dev Claim ANPAN back.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        anpanToken.safeTransfer(_msgSender(), _amount);
    ...
    function canClaim(address _userAddress) external view returns (bool) {
        if (!anpansProfile.hasRegistered(_userAddress)) {
            return false;
        } else if (hasClaimed[_userAddress]) {
            return false;
        } else {
            uint256 userId;
            (userId, , , , , ) = anpansProfile.getUserProfile(_userAddress);
