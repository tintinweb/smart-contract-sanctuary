...
/**
   ...
   #Vault Swaps features for every transaction:
...
contract VaultSwaps is Context, IERC20, Ownable {
    ...
    string private _name = "Corgo";
    string private _symbol = "CORG";
    uint8 private _decimals = 9;
    
    uint256 public _taxFee = 2;  // devFee
    uint256 private _previousTaxFee = _taxFee;
    uint256 public _burnFee = 2;  // 2% burn fee
    uint256 private _previousBurnFee = _burnFee;
    
    uint256 public _liquidityFee = 2;  //2% auto liquidity
    ...
    function _reflectFee(uint256 rBurn, uint256 tFee, uint256 tBurn) private {
    ...
    function _sendToDev(address sender, uint256 rFee, uint256 tFee) private {
        if(sender != owner()){
            _rOwned[owner()] = _rOwned[owner()].add(rFee);
            _tOwned[owner()] = _tOwned[owner()].add(tFee);
