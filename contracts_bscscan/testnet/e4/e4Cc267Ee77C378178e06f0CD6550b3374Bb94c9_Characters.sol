...
contract Characters is Initializable, ERC721Upgradeable, AccessControlUpgradeable, ITransferCooldownable {
    ...
    function initialize () public initializer {
        __ERC721_init("HerosBattle character", "HEROC");
        __AccessControl_init_unchained();
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    function initCharExperienceTable() public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not admin");

        charExpTable = [
            10, 11, 11, 13, 13, 15, 15, 17, 17, 20, 20, 23, 23, 27, 28, 32, 
            33, 38, 39, 45, 46, 53, 55, 63, 66, 75, 79, 89, 94, 105, 111, 123, 
            130, 143, 151, 165, 174, 189, 199, 215, 226, 243, 255, 273, 286, 305, 
            319, 339, 354, 375, 391, 413, 430, 453, 471, 495, 514, 539, 559, 585, 606, 
            633, 655, 683, 706, 735, 759, 789, 814, 845, 871, 903, 930, 963, 991, 1025, 1054, 
            1089, 1119, 1155, 1186, 1223, 1255, 1293, 1326, 1365, 1399, 1439, 1474, 1515, 1551, 1593,
            1630, 1673, 1711, 1755, 1794, 1839, 1879, 1925, 1966, 2013, 2055, 2103, 2146, 2195, 2239, 2289, 
            2334, 2385, 2431, 2483, 2530, 2583, 2631, 2685, 2734, 2789, 2839, 2895, 2946, 3003, 3055, 3113, 3166, 
            3225, 3279, 3339, 3394, 3455, 3511, 3573, 3630, 3693, 3751, 3815, 3874, 3939, 3999, 4065, 4126, 4193, 4255,
            4323, 4386, 4455, 4519, 4589, 4654, 4725, 4791, 4863, 4930, 5003, 5071, 5145, 5214, 5289, 5359, 5435, 5506, 5583, 
            5655, 5733, 5806, 5885, 5959, 6039, 6114, 6195, 6271, 6353, 6430, 6513, 6591, 6675, 6754, 6839, 6919, 7005, 7086, 7173, 
            7255, 7343, 7426, 7515, 7599, 7689, 7774, 7865, 7951, 8043, 8130, 8223, 8311, 8405, 8494, 8589, 8679, 8775, 8866, 8963, 9055, 
            9153, 9246, 9345, 9439, 9539, 9634, 9735, 9831, 9933, 10030, 10133, 10231, 10335, 10434, 10539, 10639, 10745, 10846, 10953, 11055, 
            11163, 11266, 11375, 11479, 11589, 11694, 11805, 11911, 12023, 12130, 12243, 12351, 12465, 12574, 12689, 12799, 12915, 13026, 13143, 
            13255, 13373, 13486, 13605, 13719, 13839, 13954, 14075, 14191, 14313, 14430, 14553,14670
        ];
        charLimit = 5;
    ...
    /*
        ...
        levels: 1-256
        traits: 0-4 [0(wood) > 1(earth) > 2(water) > 3(fire) > 4(metal) > repeat]
        sex: 1: male, 0: female
        start: 0* -> 4*
    */
    ...
    uint256 public constant secondsPerSpin = 60; //3 * 60
    ...
    function _restricted() internal view {
        require(hasRole(GAME_ADMIN, msg.sender), "character: Not game admin");
    }
    ...
    function get(uint256 id) public view returns (uint16, uint8, uint8, uint64, uint16, uint16, uint16, uint16, uint16, uint16) {
        ...
        return (c.xp, c.level, c.trait, c.spinTimestamp,
    ...
    function mint(address minter, uint256 seed) public restricted {
        ...
        else if(roll < 4) { // 4* at 5%
            stars = 3;
        }
        else if(roll < 21) { // 3* at 15%
            stars = 2;
        }
        else if(roll < 51) { // 2* at 35%
            stars = 1;
        }
        else {
            stars = 0; // 1* at 44%
        ...
        uint16 xp = 0;
        uint8 level = 0; // 1
        uint8 trait = uint8(RandomUtil.randomSeededMinMax(0,4,seed));
        uint8 sex = uint8(RandomUtil.randomSeededMinMax(0,1,seed));
        uint64 staminaTimestamp = uint64(now.sub(getSpinMaxWait()));

        tokens.push(Character(xp, level, trait, sex, stars, staminaTimestamp));
    ...
    function getRequiredXpForNextLevel(uint8 currentLevel) public view returns (uint16) {
        return uint16(charExpTable[currentLevel]); // this is helpful to users as the array is private
    }

    function getPower(uint256 id) public view returns (uint24) {
        // add stars 
        uint24 basePower =  getPowerAtLevel(tokens[id].level);
        uint8 stars = tokens[id].stars;
        return uint24(uint256(basePower).mul(stars+100).div(100));
    }
    ...
    */
    function getCharacter(uint256 id) public view returns (uint16,uint8,uint8,uint8,uint8,uint64) {
        return (tokens[id].xp,tokens[id].level,tokens[id].trait,tokens[id].sex,tokens[id].stars,tokens[id].spinTimestamp);
    }
    ...
    function getSex(uint256 id) public view returns (uint32) {
        return tokens[id].sex;
    }

    function getStars(uint256 id) public view returns (uint32) {
    ...
    function gainXp(uint256 id, uint16 xp) public restricted {
        ...
        if(char.level < 255) {
            ...
            uint requiredToLevel = charExpTable[char.level]; // technically next level
            while(newXp >= requiredToLevel) {
                newXp = newXp - requiredToLevel;
                char.level += 1;
                emit LevelUp(ownerOf(id), id, char.level);
                if(char.level < 255)
                    requiredToLevel = charExpTable[char.level];
                else
                    newXp = 0;
            }
    ...
    function getSpinTimestamp(uint256 id) public view returns (uint64) {
        return tokens[id].spinTimestamp;
    }

    function setSpinTimestamp(uint256 id, uint64 timestamp) public restricted {
        tokens[id].spinTimestamp = timestamp;
    }

    function getSpinPoints(uint256 id) public view returns (uint8) {
        return getSpinPointsFromTimestamp(tokens[id].spinTimestamp);
    }

    function getSpinPointsFromTimestamp(uint64 timestamp) public view returns (uint8) {
        if(timestamp  > now)
            return 0;

        uint256 points = (now - timestamp) / secondsPerSpin;
        if(points > maxSpin) {
            points = maxSpin;
        }
        return uint8(points);
    ...
    function isSpinFull(uint256 id) public view returns (bool) {
        return getSpinPoints(id) >= maxSpin;
    }

    function getSpinMaxWait() public pure returns (uint64) {
        return uint64(maxSpin * secondsPerSpin);
    }

    function getFightDataAndDrainSpin(uint256 id, uint8 amount) public restricted returns(uint96) {
        Character storage char = tokens[id];
        uint8 staminaPoints = getSpinPointsFromTimestamp(char.spinTimestamp);
        require(staminaPoints >= amount, "Not enough stamina!");

        uint64 drainTime = uint64(amount * secondsPerSpin);
        uint64 preTimestamp = char.spinTimestamp;
        if(staminaPoints >= maxSpin) { // if stamina full, we reset timestamp and drain from that
            char.spinTimestamp = uint64(now - getSpinMaxWait() + drainTime);
        }
        else {
            char.spinTimestamp = uint64(char.spinTimestamp + drainTime);
        }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
        if(to != address(0) && to != address(0x000000000000000000000000000000000000dEaD) && !hasRole(NO_OWNED_LIMIT, to)) {
            require(balanceOf(to) < charLimit, "Recv has too many characters");
        }
