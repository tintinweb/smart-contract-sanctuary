...
// 
interface IOasisNFT1155 {
    function creators(uint256) external returns (address);
}

contract OasisMarketplace is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    /// @notice Oasis721 contract
    address public Oasis721;
    ...
    function buyItem(
        ...
        // require(feeTransferSuccess, "OasisMarketplace: Fee transfer failed");
        if (
            ...
            //     "OasisMarketplace: Royalty fee transfer failed"
            // );
        }
        listedItem.payToken.safeTransferFrom(
            _msgSender(),
        ...
        // }("");
        // require(ownerTransferSuccess, "OasisMarketplace: Owner transfer failed");
    ...
    function registerRoyalty(
        ...
        if (_supportERC721(_nftAddress)) {
            require(
                IERC721(Oasis721).ownerOf(_tokenId) == _msgSender(),
                "Not owning the item."
            );
        } else if (_supportERC1155(_nftAddress)) {
            require(
                IOasisNFT1155(_nftAddress).creators(_tokenId) == _msgSender(),
                "Not owning the item."
            );
        } else {
            revert("Invalid NFT address.");
    ...
    /**
     @notice Update Oasis721 contract
     @dev Only admin
     */
    function updateOasis721(address _Oasis721) external onlyOwner {
        require(
            IERC165(_Oasis721).supportsInterface(INTERFACE_ID_ERC721),
            "Not ERC721"
        );
        Oasis721 = _Oasis721;
    }
