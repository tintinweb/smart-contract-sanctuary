...
contract GenesisPool {
    ...
    mapping(address => uint256) public stakeid;
    ...
    constructor(IBEP20 token) public{
        DFC = token; // 0xbabee41b5ac5405777dd2ee4ee34047589dc111b
    }
    ...
    function stake(uint256 amount) external returns (bool) {
        ...
        stakes.stakeId = totalStakes;
        stakeid[msg.sender] = totalStakes;
        stakes.stakedAmount = amount;
        stakes.stakedTime = block.timestamp;
        stakings[msg.sender][totalStakes] =stakes;
    ...
    function unStake(uint256 stakeId) external returns (uint256) {
        ...
        // );
         // for testing
         require(
            block.timestamp >= stakes.stakedTime.add(1 hours),
            "UnStake not allowed"
        );
        totalStaked = totalStaked.sub(stakes.stakedAmount);
         userTotal[msg.sender] = userTotal[msg.sender].sub(stakes.stakedAmount);
