    ...
    ReentrancyGuardUpgradeable
{
        ...
        linearValidatePoolById(_poolId)
    {
        ...
        if (stakingData.joinTime != 0) {
            require(
                block.timestamp < stakingData.joinTime + lockDuration,
                "LinearStakingPool: no longer to deposit"
            );
        }
     ...
     * @param _poolId id of the pool
     */
    function linearWithdraw(uint256 _poolId)
        external
        nonReentrant
        linearValidatePoolById(_poolId)
    {
        ...
        require(
            stakingData.balance >= stakingData.balance,
            "LinearStakingPool: invalid withdraw amount"
        );
        ...
        if (pool.delayDuration == 0) {
            ...
            emit LinearWithdraw(_poolId, account, stakingData.balance);
            return;
        }
