...
contract FinaLockFarming is Initializable, OwnableUpgradeable, FinaFarming {
    ...
    //     depositTimeTier[0] = 30;
    //     rewardRatioByTier[0] = 2000;
    // }
    ...
    function pendingLPRewardByTier(uint _pid, address _user) public view returns (uint[] memory pending_) {
        ...
        if(pending > 0){
            for(uint i = depositTimeTier.length - 1; i >= 0 ; i--) {
                if(block.timestamp>= userLPInfo[_pid][_user].averageDepositedTime + depositTimeTier[i]){
                    pending = pending * rewardRatioByTier[i] / 10000;
                    pendingExtra = pendingExtra * rewardRatioByTier[i] / 10000;
                    break;
                } else {
                    //if not reaching 1st tier, return zero reward
                    pending = 0;
                    pendingExtra = 0;
                }
            }
    ...
    //front end function to return countdown time (in seconds) before the next tier reward kicks in
    function pendingTimeToNextTier(uint _pid, address _user) public view returns (bool validUser_, uint pendingTime) {
        uint[] memory p = pendingLPReward(_pid,_user);
        uint pending = p[0];
        uint tmpTime;
        bool validUser;
        //only valid if user exists and time has not passed final reward tier
        if(pending > 0 && block.timestamp < userLPInfo[_pid][_user].averageDepositedTime + depositTimeTier[depositTimeTier.length-1]){
            validUser = true;
            for(uint i = depositTimeTier.length - 2; i >= 0 ; i--) {
                if(block.timestamp>= userLPInfo[_pid][_user].averageDepositedTime + depositTimeTier[i]){
                    tmpTime = userLPInfo[_pid][_user].averageDepositedTime + depositTimeTier[i+1] - block.timestamp;
                    break;
                } else if(block.timestamp < userLPInfo[_pid][_user].averageDepositedTime + depositTimeTier[0]){
                    tmpTime = userLPInfo[_pid][_user].averageDepositedTime + depositTimeTier[0] - block.timestamp ;
                }
            }
        }
        return (validUser, tmpTime);
    }
    ...
    function withdrawLP(uint _pid, uint _amount) external virtual override onlyEOA whenNotPaused {
        ...
        if(pending > 0){
            for(uint i = depositTimeTier.length - 1; i >= 0 ; i--) {
                if(block.timestamp>= user.averageDepositedTime + depositTimeTier[i]){
                    pending = pending * rewardRatioByTier[i] / 10000;
                    pendingExtra = pendingExtra * rewardRatioByTier[i] / 10000;
                    finaToken.safeTransfer(_msgSender(), pending);
                    secondRewardToken.safeTransfer(_msgSender(), pendingExtra);
                    emit Rewards(_msgSender(), _pid, pending, pendingExtra);
                    break;
                } else {
                    //if not reaching 1st tier, return zero reward
                    pending = 0;
    ...
    function setdepositTimeTierAndRatio(uint[] calldata seconds_, uint[] calldata ratio_) onlyOwner public {
        ...
        for(uint i = 0; i < seconds_.length; i++){
            require(seconds_[i]<seconds_[i+1],"depositTimeTier array must be in ascending order");
            depositTimeTier[i] = seconds_[i];//uint in seconds
            rewardRatioByTier[i] = ratio_[i];//divide by 10000
            emit DepositTimeTier(i,seconds_[i] );
