...
// BTCC
  ...
  mapping(uint8 => uint256) public employTypeConfig;
 ...
 function addNewEmploy(address newEmploy, uint8 employType) public onlyOwner {
      employ[newEmploy] = UserData(employType, 0, 0);
  }

  function addEmployConfig(uint8 typeConfig, uint256 maxOut) public onlyOwner {
      employTypeConfig[typeConfig] = maxOut;
  }


  function swapLuni(uint256 amountLuni) public returns (uint256, uint256) {
    require(employ[msg.sender].employType == 0, "Address is not valid");
    uint256 amountUsdtAvailbale = IERC20(USDT_TOKEN).balanceOf(address(this));
    uint256 swapAmountValid = _getValidSwap();
    uint256 swapAmount = amountLuni.div(usdtRate).mul(1e18);
    if(swapAmount > swapAmountValid) {
      revert();
    } else {
      if (swapAmount > amountUsdtAvailbale) {
      revert();
      } else {
        uint256 fee = swapAmount.mul(swapFee).div(100);
        IERC20(USDT_TOKEN).transfer(msg.sender, swapAmount.sub(fee));
        employ[msg.sender].amountSwap = employ[msg.sender].amountSwap.add(swapAmount);
