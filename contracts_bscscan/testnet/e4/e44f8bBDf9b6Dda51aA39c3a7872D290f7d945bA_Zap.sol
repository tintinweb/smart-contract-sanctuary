...
contract Zap is Ownable {
    ...
    event FeeChange(address feeTo, uint16 rate, uint16 min);
    ...
    function zapInToken(
        ...
        return _swapTokenToLP(_from, amount, _to, _recipient, routerAddr);
    }
    ...
    function zapIn(address _to, address routerAddr, address _recipient) external payable returns (uint256) {
        // from Native to an LP token through the specified router
        return _swapNativeToLP(_to, msg.value, _recipient, routerAddr);
    }
    ...
    function zapAcross(
        ...
        ( , , liquidity) = IUniswapV2Router01(_toRouter).addLiquidity(
            pair.token0(),
            pair.token1(),
            amt0,
            amt1,
    ...
    function zapOutToken(
        ...
        amt = amt0.add(amt1);
        IERC20(_to).safeTransfer(_recipient, amt);
    }
    ...
    function swapToken(
        ...
        IERC20(_from).safeTransferFrom(msg.sender, address(this), amount);
        _approveTokenIfNeeded(_from, routerAddr);
        return _swap(_from, amount, _to, _recipient, routerAddr);
    }
    ...
    function swapToNative(
        ...
        IERC20(_from).safeTransferFrom(msg.sender, address(this), amount);
        _approveTokenIfNeeded(_from, routerAddr);
        amt = _swapTokenForNative(_from, amount, _recipient, routerAddr);
    }
    ...
    function setUseNativeRouter(address router) external onlyOwner returns (bool) {
        useNativeRouter[router] = true;
        return true;
    }

    function setFee(address addr, uint16 rate, uint16 min) external onlyOwner returns (bool) {
