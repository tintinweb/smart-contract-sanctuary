...
contract sTrapeza_v2 is IsFIDL, ERC20Permit {
    ...
    function prevCirculatingSupply() public view returns (uint256) {
        return balanceForGons(prevsFIDL.gonsForBalance((prevsFIDL.circulatingSupply())));
    }

    // Staking contract holds excess sOHM
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(balanceForGons(prevsFIDL.gonsForBalance((prevsFIDL.circulatingSupply())))).add(gFIDL.balanceFrom(IERC20(address(gFIDL)).totalSupply())).add(
