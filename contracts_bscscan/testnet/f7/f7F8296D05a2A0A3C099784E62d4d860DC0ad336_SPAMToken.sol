...
contract SPAMToken is Context, IERC20, Ownable {
    ...
    constructor (address account) {
        ...
        newAccountDeflationStartTime = block.timestamp;
        nextNewAccountDeflationTimePeriod = block.timestamp + NEW_ACCOUNT_DEFLATION_TIME;
    ...
    function _applyNewAccountDeflation(address sender, address receiver) internal {
        if (!skippedAccountFromDeflation[sender] && (accountToBeDeducted[sender] && block.timestamp >= deductedAfterTime[sender])) {
        ...
        if (_balances[receiver] == 0 && !skippedAccountFromDeflation[receiver]) {
            if (block.timestamp > nextNewAccountDeflationTimePeriod) {
                // calculate the next deflation timestamp.
                uint256 passedPeriods = (block.timestamp - newAccountDeflationStartTime).div(NEW_ACCOUNT_DEFLATION_TIME);
