...
contract ExoticMonster20 is ERC20, Ownable {
...
contract ExoticMonsterController is Ownable {
    ...
    constructor() {
        ...
        EMC20  = ExoticMonster20(0xb3438B2Da776631965E3AB6895cA202728bF9888);
        
        // EMC20.transfer(address(msg.sender),25000000 * 10 ** 18);
    ...
    function sellHeroCard(uint256 heroId, uint256 price) public {
        ...
        require(userHeros[msg.sender][heroId].id != 0 && userHeros[msg.sender][heroId].isPooled == false && userHeros[msg.sender][heroId].isOnsell == false && price > 0,"This hero is not available");
        
        totalSell++;

        sells[totalSell].onwerAddr = msg.sender;

        sells[totalSell].myHeroId = heroId;

        sells[totalSell].price = price;

        sells[totalSell].sold = false;
    ...
    function cancelSellHeroCard(uint256 sellId) public {
        ...
        require(sells[sellId].onwerAddr == msg.sender && userHeros[msg.sender][sells[sellId].myHeroId].isPooled == false && userHeros[msg.sender][sells[sellId].myHeroId].isOnsell == true,"This hero is not available");
    ...
    function buyHeroCard(uint256 sellId) public {
        ...
        uint256 price = sells[sellId].price * 10 ** 18;

        address ownerAddress = sells[sellId].onwerAddr;

        require(ownerAddress != msg.sender,"This buyer is owner");

        EMC20.approveToController(msg.sender, price);

        EMC20.transferFrom(msg.sender,ownerAddress,price);

        users[msg.sender].heroIds+= 1;

        userHeros[msg.sender][users[msg.sender].heroIds] = userHeros[ownerAddress][sells[sellId].myHeroId];

        delete userHeros[ownerAddress][sells[sellId].myHeroId];

        delete sells[sellId];

        userHeros[msg.sender][users[msg.sender].heroIds].isOnsell == false;
    ...
    function selectBox(uint256 boxId) public {
        ...
        if(1 == boxId && box1MaxTimes > 0){
            ...
            EMC20.approveToController(msg.sender, 3500 * 10 **18);
        ...
        if(2 == boxId && box2MaxTimes > 0){
            ...
            EMC20.approveToController(msg.sender, 10000 * 10 **18);
        ...
        if(3 == boxId && box3MaxTimes > 0){
            ...
            EMC20.approveToController(msg.sender, 15700 * 10 **18);
