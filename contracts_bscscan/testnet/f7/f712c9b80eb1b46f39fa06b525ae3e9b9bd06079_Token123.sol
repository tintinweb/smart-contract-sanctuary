...
contract Token123 is Context, IERC20, Ownable {
    ...
    string private _name = "Token123";
    string private _symbol = "T123";
    ...
    uint256 public _maxSellPerDay = 1000 * 10**9; // Max number of tokens that can be sold per day
    uint256 private transferTime = 10 * 60; // 24 hrs
    uint256 lastBuy;
    ...
    function setMaxSellPerDay(uint256 maxSellPerDay) external onlyOwner() {
        _maxSellPerDay = maxSellPerDay;
    }
    ...
    function _transfer(
        ...
        if(from != owner()) {
          require(amount <= _maxSellPerDay, "Transfer amount exceeds the maxSellPerDay.");
          require(block.timestamp > lastBuy + transferTime, "Time since last transfer must be greater then time to transfer.");
        }
