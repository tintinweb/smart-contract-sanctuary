...
abstract contract NftStakingV2 is ERC1155TokenReceiver, IERC721Receiver, Auth {
    ...
    function addRewardsForPeriodsNow(
        uint256 amountTime,
        uint256 rewardsPerCycle
    ) external onlyOwner {
        uint256 startPeriod = block.timestamp;
        uint256 endPeriod = startPeriod + amountTime;
        require(startPeriod != 0 && startPeriod <= endPeriod, "NftStaking: wrong period range");
...
interface tokenPower {
    function tokenPow(uint256 _id) external view returns(uint64);
}

contract NftGetYoureABitch is NftStakingV2 {
    ...
    function _validateAndGetNftWeight(uint256 tokenId) internal virtual override view returns (uint64) {
        return tokenPower(address(whitelistedNftContract)).tokenPow(tokenId);
    }
}