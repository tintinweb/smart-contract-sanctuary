...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        bool refPayments; // Ref Payments for the pool
    ...
    uint256 public toRefPercent = 100;
    ...
    event RefPaymentsChange(uint256 _pid, bool _value);
    event RefAddressChange(address _address);
    ...
    function setRefFee(uint256 _pid, bool _value) public onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        pool.refPayments = _value;
        emit RefPaymentsChange(_pid, _value);
    }

    function setRefAddress(address _address) public onlyOwner {
        refAddress = _address;
        emit RefAddressChange(_address);
    }

    function changeRefPercent(uint256 _value) public onlyOwner {
        require(_value > 0, "ref percent is 0. Consider just turning off ref reward");
        toRefPercent = _value;
    }
    ...
    function add(uint256 _totalReward, IERC20 _lpToken, bool _withUpdate,
        uint256 _startBlock, uint256 _endBlock, bool _refPayments, bool _lockEnabled, uint256 _vestingTime, uint256 _unlockPeriod, bool _withMaisonTransfer) external onlyOwner {
    ...
    function setPoolInfo(uint256 _pid, uint256 _rewardForEachBlock, bool _withUpdate, uint256 _startBlock, uint256 _endBlock, bool _refPayments, bool _lockEnabled) external validatePoolByPid(_pid) onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function deposit(uint256 _pid, uint256 _amount) external validatePoolByPid(_pid) payable {
        ...
        if (pool.lockEnabled) {
            ...
            user.withdrawTX = 0;
            user.withdrawablePerTime = _amount.div(50);
        }
    ...
    function getClaimCycle(uint256 _pid, address _address) public view returns (uint256) {
        ...
        uint256 _userCyclesToClaim = _timeShare - user.withdrawTX;
    ...
    function getUserWithdrawableAmount(uint256 _pid, address _address) public view returns (uint256, uint256) {
        ...
        _withdrawableAmount = _userCyclesToClaim.mul(user.withdrawablePerTime);
    ...
    function harvest(uint256 _pid, address _to) public nonReentrant payable validatePoolByPid(_pid) returns (bool success) {
        ...
        if (pending > ZERO) {
            success = true;
            if (pool.refPayments) {
                uint256 _toRefPending = pending.mul(toRefPercent).div(RATIO_BASE);
                pending -= _toRefPending;
                safeTransferTokenFromThis(maison, refAddress, _toRefPending);
