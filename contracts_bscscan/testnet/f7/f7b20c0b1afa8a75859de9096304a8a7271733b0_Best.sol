...
contract AirdropFundWallet is IAirdropFundWallet {
    ...
    function sendToken(address bridgeAirdropAddress) external payable override onlyToken {
            ...
            if(token_balance < minTokenBalance){
                ...
                temp_lock_time = block.timestamp + duration * 1 days; //temporary blocking of transfers
            }
    }
}
...
contract Best is IBEP20, Auth {
    ...
    string constant _name = "BESTv2";
    string constant _symbol = "BSTv2";    
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(setPriceTimestamp <= block.timestamp){ setPastPrice(); }
...
//        emit Transfer(sender, recipient, amount);
    ...
    function checkMarketCondition() internal {
          ...
          if (price <= pastPrice) { marketDowntrend = 1; } // downtrend or not uptrend
          else if (price.div(pastPrice).mul(100) <= 80) { marketDowntrend = 2; } // downtrend
          else if (price.div(pastPrice).mul(100) <= 60) { marketDowntrend = 3; } // strong downtrend
          else if (price.div(pastPrice).mul(100) <= 40) { marketDowntrend = 4; } // very strong downtrend
          else marketDowntrend = 0;
    }   
    ...
    function currentPrice() public returns (uint256) {
       ...
       return (router.getAmountsOut(oneBNB,path)).div(oneBNB);
    }

    function setPastPrice() internal {
        if (pastPrice < currentPrice()){
             pastPrice = currentPrice(); //force a price increase
        }
         setPriceTimestamp = block.timestamp + setPriceTimestampInterval * 1 days;
    }
    ...
    function UpdatePriceTimestampInterval(uint256 _setPriceTimestampInterval) external authorized {
        setPriceTimestampInterval = _setPriceTimestampInterval; //e.g if set to 10 then check price every 10 days
    }
    ...
    function manualSetPastPrice(uint256 _pastPrice) external authorized {
