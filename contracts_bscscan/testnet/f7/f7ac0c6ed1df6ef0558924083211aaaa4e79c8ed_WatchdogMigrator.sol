...
interface IPancakePair {
    ...
    event Sync(uint216 reserve0, uint216 reserve1);
...
contract WatchdogMigrator is Auth {
    ...
    struct Deposit {
        uint216 amount;
        uint32 timestamp; // for the decreasing bonus
        bool withdrawn;
    }
    ...
    function depositOldTokens(uint amount) external {
        ...
        require(oldTokensDepositable, "Tokens not depositable yet");
    ...
    function claimNewTokens() external {
        ...
        setDepositsToWithdrawn(msg.sender);
    ...
    function setDepositsToWithdrawn(address user) internal {
        Deposit[] memory deps = deposits[user];
        for (uint256 i = 0; i < deps.length; i++) {
            deposits[msg.sender][i].withdrawn = true;
        }
    }
    ...
    function editDeposit(address user, uint depositId, uint216 amount, uint32 timestamp, bool withdrawn) external authorizedFor(Permission.AdjustVariables) {
        deposits[user][depositId].amount = amount;
        deposits[user][depositId].timestamp = timestamp;
        deposits[user][depositId].withdrawn = withdrawn;
    }
