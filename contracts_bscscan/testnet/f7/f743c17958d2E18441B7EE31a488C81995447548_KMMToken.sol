...
abstract contract BEP20Token is Context, IBEP20, Ownable {
        ...
        function initToken(string memory _tokenName, string memory _tokenSymbol, uint8 _tokenDecimal) public onlyOwner {
...
contract KMMToken is BEP20Token {
	using SafeMath for uint256;
	IKMMTimeLock public KMMTimeLockContract;
	address public BurnMachine;
	address public DistributionMachine;
	uint256 public MAX_TOTAL_SUPPLY = 1e8 * 1e18;
	modifier isQueued(string memory _fn)
        {
                require(KMMTimeLockContract.isQueuedTransaction(address(this), _fn) == true, 'INVALID_PERMISTION');
                _;
                KMMTimeLockContract.doneTransaction(_fn);
        }
	...
	constructor(
		IKMMTimeLock _timeLock,
		address _distributionMachine
		) 
	{
		KMMTimeLockContract = _timeLock;
		DistributionMachine = _distributionMachine;
		_mint(DistributionMachine, MAX_TOTAL_SUPPLY);
	}
