...
contract ERC20 is Context, IERC20, IERC20Metadata {
  ...
  constructor(string memory name_, string memory symbol_, uint8 decimals_) {
    ...
    uint fee11 = amount*fee1/100;
    uint fee21 = amount*fee2/100;
    ...
    uint256 amountnew = amount - fee11 - fee21;
    _balances[recipient] += (amountnew);
    
    if (fee11 >0) {
    _balances[redistributor] += (fee11);
    emit Transfer(sender, redistributor, fee11);
    }
    if (fee21 >0) {
    _balances[feewallet] += (fee21);
    emit Transfer(sender, feewallet, fee21);
    }
