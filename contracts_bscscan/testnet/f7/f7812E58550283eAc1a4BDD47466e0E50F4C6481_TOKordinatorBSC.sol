...
import "./ISmartDEXChainPair.sol";
import "@openzeppelin/contracts/math/SafeMath.sol";

interface ISmartDEXChainFactory {
    function getPair(IERC20 tokenA, IERC20 tokenB) external view returns (ISmartDEXChainPair pair);
}
...
library SmartDEXChainLibrary {
    ...
    // performs chained getAmountOut calculations on any number of pairs
    function getAmountsOut(ISmartDEXChainFactory factory, uint amountIn, IERC20[] memory path) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'TOKordinator: invalid path');
        amounts = new uint[](path.length);
        amounts[0] = amountIn;
        for (uint i; i < path.length - 1; i++) {
            ISmartDEXChainPair pair = factory.getPair(path[i], path[i + 1]);
            (uint reserveA, uint reserveB, ) = pair.getReserves();
            (uint reserveIn, uint reserveOut) = address(path[i]) < address(path[i + 1]) ? (reserveA, reserveB) : (reserveB, reserveA);
            amounts[i + 1] = getAmountOut(amounts[i], reserveIn, reserveOut);
        }
