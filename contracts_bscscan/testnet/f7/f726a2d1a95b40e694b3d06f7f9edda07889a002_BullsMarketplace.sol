...
contract BullsMarketplace is ERC721Holder, Ownable, ReentrancyGuard {
  ...
  mapping(address => uint256) public totalHoldersDividentBalance;
  mapping(address => uint256) public mintersDividentBalance;
  mapping(address => uint256) public totalMintersDividentBalance;
  ...
  event CollectionNew(address indexed collection, address indexed creator, address indexed whitelistChecker, uint256 tradingFee, uint256 creatorFee, uint256 collectionSize);

  // Existing collection is updated
  event CollectionUpdate(address indexed collection, address indexed creator, address indexed whitelistChecker, uint256 tradingFee, uint256 creatorFee, uint256 collectionSize);
  ...
  function addCollection(
    ...
    emit CollectionNew(_collection, _creator, _whitelistChecker, _tradingFee, _creatorFee, _collectionSize);
  }
  ...
  function modifyCollection(
    ...
    emit CollectionUpdate(_collection, _creator, _whitelistChecker, _tradingFee, _creatorFee, _collectionSize);
  }
  ...
  function addDividendForHolders(address _collection, uint256 amount) private {
    ...
    totalHoldersDividentBalance[_collection] = totalHoldersDividentBalance[_collection] + (amount / _collections[_collection].collectionSize);
  }
  ...
  function currentHoldersDividentRate(address _collection) public view returns (uint256) {
  ...
  function addDividendForMinters(address _collection, uint256 amount) private {
    ...
    totalMintersDividentBalance[_collection] = totalMintersDividentBalance[_collection] + (amount / _collections[_collection].collectionSize);
  }
  ...
  function currentMintersDividentRate(address _collection) public view returns (uint256) {
    ...
    returns (uint256)
  {
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      holderBalance += (totalHoldersDividentBalance[_collection] - lastHolderDividendAt[_collection][bullId]);  
      if(_isClaiming){
        lastHolderDividendAt[_collection][bullId] = totalHoldersDividentBalance[_collection];
      }
    }
    
    uint256 mintBalance = (totalMintersDividentBalance[_collection] - lastMinterDividendAt[_collection][msg.sender]) * getMintCount(_collection, msg.sender);
    if(_isClaiming){
      lastMinterDividendAt[_collection][msg.sender] = totalMintersDividentBalance[_collection];
    }
