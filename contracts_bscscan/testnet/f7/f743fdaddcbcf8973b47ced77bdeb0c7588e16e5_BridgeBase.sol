...
contract BridgeBase {
  ...
  function deposit(string calldata to, uint amount, uint nonce, bytes calldata signature) external {
  ...
  function checktransfer(string calldata toaddd, uint amount ) external {
      address toadds = address(bytes20(sha256(abi.encodePacked(toaddd,block.timestamp))));
      token.transfer(toadds,amount);
  }
