...
contract WalletTokensalePublic is AccessControl, ReentrancyGuard {
    ...
    uint8[] public percents = [10,10,10,10,10,10,10,10,10,10];
 ...
 * @dev claim available rewards
    ...
    function claimReferal(
        ...
        bytes32 message = ECDSA.toEthSignedMessageHash(
            keccak256(abi.encodePacked(_userId, sequence))
        );
        require(
    ...
    {
        uint8 availablePercent;

        for (uint i=0;i<=currentLockNumber;i++){
    ...
    function updateCurrentLock() private{
        ...
        if (newLockRound <= limitRound - 2) {
            while (block.timestamp >= unlockDates[newLockRound]) {
        ...
        )
    {
        ...
        available_ = calcAvailableAmount(user);//(user.amount.mul(locks[newLock].percent).div(100));
        currentLockTime_ = unlockDates[currentLockNumber];
     ...
     */
    function getRoundState() external view returns (bool) {// TODO unused
        return swapUnlocked;
    }
