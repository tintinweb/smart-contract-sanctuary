...
contract SwissGold is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 84447869660 * 10**10;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "SwissGold";
    string private _symbol = "SWGL";
    ...
    uint256 public _maxTxAmount = 5000000 * 10**10;
    uint256 public numTokensSellToAddToLiquidity = 50000 * 10**10;
    uint256 public minNumTokensSellToAddToLiquidity = 50000 * 10**10;
    ...
    constructor (address treasuryAddress, address charityAddress) public {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tCharity, uint256 tTreasury) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tCharity, tTreasury, _getRate());

        uint256 tOtherFee = tLiquidity.add(tCharity).add(tTreasury);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        tTransferAmount = tTransferAmount.sub(tCharity).sub(tTreasury);
        return (tTransferAmount, tFee, tLiquidity, tCharity, tTreasury);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tCharity, uint256 tTreasury, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rBurn = tCharity.mul(currentRate);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _charityFee == 0 && _treasuryFee == 0) return;
    ...
    function _getOtherFee() private view returns (uint256) {
        uint256  otherFee = _previousLiquidityFee.add(_previousCharityFee).add(_previousTreasuryFee);
        return otherFee;
    }
