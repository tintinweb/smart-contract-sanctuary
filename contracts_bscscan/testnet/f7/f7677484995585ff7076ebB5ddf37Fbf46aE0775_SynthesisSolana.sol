...
import "./interfaces/ISyntFabricSolana.sol";
import "./interfaces/ISynthesisSolana.sol";
 ...
 */
contract SynthesisSolana is RelayRecipientUpgradeable {
  /// ** PUBLIC states **
  ...
  function mintSyntheticToken(
    ...
    ISyntFabricSolana(fabric).synthesize(
      _to,
      _amount,
      ISyntFabricSolana(fabric).getSyntRepresentation(_tokenReal, _chainID)
    );
  ...
  function burnSyntheticToken(
    ...
    require(msg.value == nativeTokenPrice, "Symb: Not enough money");
    ISyntFabricSolana(fabric).unsynthesize(_msgSender(), _amount, _stoken);
    bytes32 rtoken = ISyntFabricSolana(fabric).getRealRepresentation(_stoken);
...
interface ISyntFabricSolana {
  function getRealRepresentation(address _syntTokenAdr)
    external
    view
    returns (bytes32);

  function getSyntRepresentation(bytes32 _realTokenAdr, uint256 _chainID)
    external
    view
    returns (address);
...
library MetaRouteStructs {
  ...
  struct MetaBurnTransactionSolana {
    address syntCaller;
    address finalDexRouter;
    address sToken;
    bytes swapCallData;
  ...
  struct MetaRouteTransactionSolana {
    bytes32 to;
    address[] firstPath; // uni -> BUSD
    address[] secondPath; // BUSD -> sToken
    address[] finalPath; // rToken -> another token
