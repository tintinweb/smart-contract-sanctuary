...
import "./libs/LotteryOwnable.sol";
...
contract Lottery is LotteryOwnable, Initializable {
    ...
    // issueId => trickyNumber => historyTimestamp
    ...
    event BurnRemain(address indexed user, uint256 amount);
    ...
    function drawing(uint256 _externalRandomNumber) external onlyAdmin {
        ...
        winningNumbers[2]=0;
        winningNumbers[3]=0;
        historyTimestamp[issueIndex] = lastTimestamp;
    ...
    function generateNumberIndexKey(uint8[4] memory number) internal pure returns (uint64[keyLengthForEachBuy] memory) {
    ...
    // Anybody can topup (donate) to Lottery Pot
    function topupLottery(uint256 _amount) public {
    ...
    // Burn remain Lottery Pot
    function burnRemain(uint256 _amount) public onlyAdmin {
        token.safeTransfer(BURN_ADDRESS, _amount);
        emit BurnRemain(msg.sender, _amount);
    }
...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("Lottery Ticket", "LT") {}
