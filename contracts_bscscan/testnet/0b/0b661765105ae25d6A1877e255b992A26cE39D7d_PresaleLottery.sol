...
// File: contracts/Presale_Lottery.sol
...
contract PresaleLottery is Ownable {
    ...
    function calc(uint256 tickets) public pure returns(uint) {
        uint result;
        uint256 FinalMinReward;
        uint256 A = MIN_REWARD;
        
        uint256 B = tickets.div(300);
        uint256 C =  A.mul(B);
        
        FinalMinReward = C; 
        for(uint i = 0; i < 19; i++) {
            if(i >= 0) {
             return FinalMinReward;
            } if(i >= 1) {
                result = result.add(FinalMinReward.mul(i));
            return result;
        }
    }
