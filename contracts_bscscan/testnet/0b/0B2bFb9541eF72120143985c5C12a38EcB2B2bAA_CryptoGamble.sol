...
contract CryptoGamble is Ownable {
    ...
    function enterGamble(uint symbole, bool up) payable public{
        ...
        require(targetPool.enteredUp[msg.sender] == 0, "You are already gambling this pool");
    ...
    function getCurrentBnbPool(uint256 poolIndex) public view returns (PublicPool memory){
        ...
        PublicPool memory pool = PublicPool({
            ...
            enteredUpValue: bnbPool.enteredUpValue,
            enteredDownValue: bnbPool.enteredDownValue,
            enteredUpCount: bnbPool.enteredUpCount,
            enteredDownCount: bnbPool.enteredDownCount,
    ...
    function getCurrentBtcPool(uint256 poolIndex) public view returns (PublicPool memory){
        ...
        PublicPool memory pool = PublicPool({
            ...
            enteredUpValue: btcPool.enteredUpValue,
            enteredDownValue: btcPool.enteredDownValue,
            enteredUpCount: btcPool.enteredUpCount,
            enteredDownCount: btcPool.enteredDownCount,
    ...
    function getCurrentEthPool(uint256 poolIndex) public view returns (PublicPool memory){
        ...
        PublicPool memory pool = PublicPool({
            ...
            enteredUpValue: ethPool.enteredUpValue,
            enteredDownValue: ethPool.enteredDownValue,
            enteredUpCount: ethPool.enteredUpCount,
            enteredDownCount: ethPool.enteredDownCount,
            poolSize: ethPool.enteredUpValue + ethPool.enteredDownValue
        });
        return pool;
    }

    function isSenderGamblingInPool(uint256 poolIndex, uint256 symbole) public view returns (bool){
        require(symbole >= 0 && symbole < 3, "Pool dosen't exist");
        Pool storage targetPool = defaultPool;

        if(symbole == 0){
            targetPool = bnbPools[poolIndex];
        }
        else if(symbole == 1){
            targetPool = btcPools[poolIndex];
        }
        else if(symbole == 2){
            targetPool = ethPools[poolIndex];
        }
        return targetPool.enteredUp[msg.sender] != 0;
    }
