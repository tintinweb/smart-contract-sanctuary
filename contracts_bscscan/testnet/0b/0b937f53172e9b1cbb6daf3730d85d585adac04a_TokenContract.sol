...
contract TokenContract is Context, IERC20, Ownable {
    ...
    uint256 public _sellFeeTokenDistribution = 0;
    uint256 public _sellFeeBNBDistribution = 777; // 7.77%
    uint256 public _sellFeeBuyback = 1323; // 13.23%

    uint256 public _bbFeeLiquid = 1400; // 14%
    uint256 public _bbFeeTokenDistribution = 700; // 7%
    uint256 public _bbFeeMarketingBNB = 1400; // 14%
    uint256 public _bbFeeJackpot = 700; // 7%
    uint256 public _bbFeeBurn = 2320; //23.2%
    uint256 public _bbFeeMarketingToken= 3480; //34.8%
    ...
    uint256 public _buybackThreshold = 10**17; // 0.1 BNB
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSelling) private view returns (uint256, uint256, uint256) {
        ...
        if(takeFee){
            if(isSelling){
                tTotalFeeExceptTokenDistribution = (_sellFeeBNBDistribution.add(_sellFeeBuyback)).mul(tAmount).div(_PERCENR_NOMINATOR);
                tTokenDistributionFee = tAmount.mul(_sellFeeTokenDistribution).div(_PERCENR_NOMINATOR);
            }else {
    ...
    function _takeAllFeeExceptTokenDistribution(uint256 tTotalFeeExceptTokenDistribution, bool isSelling) private {
        if(tTotalFeeExceptTokenDistribution > 0){
            if(isSelling){
                uint256 numBnbDistr = tTotalFeeExceptTokenDistribution.mul(_sellFeeBNBDistribution).div(_sellFeeBNBDistribution.add(_sellFeeBuyback));
                accumulatedAmountTokenForBNBDistribution = accumulatedAmountTokenForBNBDistribution.add(numBnbDistr);
                accumulatedAmountTokenForBuyback = accumulatedAmountTokenForBuyback.add(tTotalFeeExceptTokenDistribution.sub(numBnbDistr));
                
                //Token for BNB distribution & buyback are kept in token contract
                ...
                uint256 numMarketing = tTotalFeeExceptTokenDistribution.mul(_buyFeeMarketing).div(_buyFeeLiquid.add(_buyFeeMarketing).add(_buyFeeJackpot));
                uint256 numJackpot = tTotalFeeExceptTokenDistribution.sub(numLiquid).sub(numMarketing);

                //Token for Liquidity, kept in token contract
                accumulatedAmountTokenForLiquidity = accumulatedAmountTokenForLiquidity.add(numLiquid);
                sendToken(address(this), numLiquid);

                //Send marketing fee
                sendToken(walletMarketing, numMarketing);

                //Send jackpot fee
                sendToken(walletJackpot, numJackpot);
            }
        }
    }
    ...
    function _transfer(
        ...
        if (
            ...
            } else if (address(this).balance >= _buybackThreshold){
                buyback();
            }
        }
    ...
    function buyback() private lockTheSwap{
        uint256 currentContractBNBBalance = address(this).balance;
        //Send buyback marketing BNB fee
        payable(walletMarketing).transfer(currentContractBNBBalance.mul(_bbFeeMarketingBNB).div(_PERCENR_NOMINATOR));

        currentContractBNBBalance = address(this).balance;

        uint contractTokenBalance = balanceOf(address(this));
        buyTokens(currentContractBNBBalance);
        uint totalBuyBackToken = balanceOf(address(this)).sub(contractTokenBalance);

        uint256 currentRate =  _getRate();
        //Take token from token contract --> reduce contract balance
        uint256 rtotalBuyBackToken = totalBuyBackToken.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].sub(rtotalBuyBackToken);
        if(isDividendExempt[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].sub(totalBuyBackToken);


        uint256 _totalBuybackFeeExceptMarketingBNB = _PERCENR_NOMINATOR.sub(_bbFeeMarketingBNB);
        uint256 tBbFeeLiquid = totalBuyBackToken.mul(_bbFeeLiquid).div(_totalBuybackFeeExceptMarketingBNB);
        uint256 tBbFeeTokenDistribution = totalBuyBackToken.mul(_bbFeeTokenDistribution).div(_totalBuybackFeeExceptMarketingBNB);
        uint256 tBbFeeJackpot = totalBuyBackToken.mul(_bbFeeJackpot).div(_totalBuybackFeeExceptMarketingBNB);
        uint256 tBbFeeBurn = totalBuyBackToken.mul(_bbFeeBurn).div(_totalBuybackFeeExceptMarketingBNB);
        uint256 tBbFeeMarketing = totalBuyBackToken.sub(tBbFeeLiquid + tBbFeeTokenDistribution + tBbFeeJackpot + tBbFeeBurn);
        
        //send marketing token
        sendToken(address(this), tBbFeeLiquid);
        accumulatedAmountTokenForLiquidity = accumulatedAmountTokenForLiquidity.add(tBbFeeLiquid);

        //Redistribute token for all holders
        _reflectTokenDistribution(tBbFeeTokenDistribution.mul(currentRate), tBbFeeTokenDistribution);

        //send jackport token
        sendToken(walletJackpot, tBbFeeJackpot);

        //Send marketing fee
        sendToken(walletMarketing, tBbFeeMarketing);

        //Burn
        sendToken(walletDEAD, tBbFeeBurn);
    }

    function swapBack(uint256 contractTokenBalance) private lockTheSwap {

        uint256 amountLiquid = contractTokenBalance.mul(accumulatedAmountTokenForLiquidity).div(accumulatedAmountTokenForBNBDistribution + accumulatedAmountTokenForBuyback + accumulatedAmountTokenForLiquidity);
        uint256 amountBNBDis = contractTokenBalance.mul(accumulatedAmountTokenForBNBDistribution).div(accumulatedAmountTokenForBNBDistribution + accumulatedAmountTokenForBuyback + accumulatedAmountTokenForLiquidity);
        uint256 amountBuyback = contractTokenBalance.sub(amountLiquid + amountBNBDis);
        ...
        // swap tokens for ETH
        swapTokensForEth(amountBNBDis + amountBuyback + halfLiquid); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        ...
        uint256 bnbLiqid = swapBalance.mul(halfLiquid).div(amountBNBDis + amountBuyback + halfLiquid);
        uint256 bnbDis = swapBalance.mul(amountBNBDis).div(amountBNBDis + amountBuyback + halfLiquid);

        try distributor.deposit{value: bnbDis}() {} catch {}
    ...
    function updateSellFee(uint256 tokenDistributionFee, uint256 bnbDistributionFee, uint256 buybackFee) public onlyOwner {
        _buyFeeTokenDistribution = tokenDistributionFee;
        _sellFeeBNBDistribution = bnbDistributionFee;
        _sellFeeBuyback = buybackFee;
    }

    function updateBuyBackFee(uint256 tokenDistributionFee, uint256 liquidFee, uint256 marketingBNBFee, uint256 jackpotFee, uint256 burnFee, uint256 marketingTokenFee) public onlyOwner {
        require(tokenDistributionFee + liquidFee + marketingBNBFee + jackpotFee + burnFee + marketingTokenFee == _PERCENR_NOMINATOR, "Total buyback fee must be 10000");
        _bbFeeTokenDistribution = tokenDistributionFee;
        _bbFeeLiquid = liquidFee;
