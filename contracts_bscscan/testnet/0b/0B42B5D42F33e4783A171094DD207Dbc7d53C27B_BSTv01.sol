...
contract BSTv01 is RedistributableToken {
  ...
  function patchSupplies() onlyOwner public {
    require(!suppliesPatched, "Supplies patched already");
    uint correctedActivatedSupply   = 102600970254;
    uint correctedDeactivatedSupply = 897399029746;
    _supply = (correctedDeactivatedSupply << 128) + correctedActivatedSupply;
    address addr = 0x3478854047377531366Ce4344b3E6E0634d349Ae;
    uint delta = 92125592;
    _book[addr] = Account.increaseBalance(_book[addr], delta);
    emit Transfer(address(0), addr, delta);
  }
...
contract RedistributableToken is Initializable, BEP20, Burnable, Pausable, Ownable, UUPS {
  ...
  function setAccountRules(
    ...
    if( holder.mustHideBalance() != hideBalance ) {
      uint supply = _supply;
      if(hideBalance) {
        supply = supply.decreaseActive(balance);
        supply = supply.increaseHidden(balance);
      } else {
        supply = supply.decreaseHidden(balance);
        supply = supply.increaseActive(balance);
      }
      _supply = supply;
    }
  ...
  function rules(address account) public view virtual returns (
    bool sendWithoutFee,
    bool sendWithoutBurn,
    bool receiveWithoutFee,
    bool receiveWithoutBurn,
