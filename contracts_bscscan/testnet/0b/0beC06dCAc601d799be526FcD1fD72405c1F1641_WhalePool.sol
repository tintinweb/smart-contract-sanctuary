...
contract WhalePool is Ownable, ReentrancyGuard, VRFConsumerBase {
    struct UserInfo {
    ...
    mapping(bytes32 => address)     public randomRequests;      // Mapping of random requsts to the user making it
    ...
    function nftMintTime(address _user) public view returns(uint) {
        return (userInfo[_user].lastNftMint + mintingTime) - block.timestamp;
    }
    ...
    function startMinting() public payable nonReentrant() returns (bytes32) {
        require(userInfo[msg.sender].isStaked, 'You are not staked in the pool');
        ...
        require(LINK.balanceOf(address(this)) >= linkFee, 'LINK balance failure');
    ...
    function fulfillRandomness(bytes32 _requestId, uint256 _randomNumber) internal override {
        uint randomNumber = (_randomNumber % (rewardNfts.length - 1));
