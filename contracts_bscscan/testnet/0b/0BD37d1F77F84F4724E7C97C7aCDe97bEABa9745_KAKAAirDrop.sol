...
contract KAKAAirDrop is Ownable, Divestor {
    ...
    function setAddr(uint addrId_, address addr_) public onlyOwner {
        addrList[addrId_] = addr_;
    }

    function setAirdroped(uint addrId_, bool b_) public onlyOwner {
        airdroped[addrId_] = b_;
    }
    ...
    function _claimERC721(uint airdropId_, uint fromAddrId_, uint cardId_, uint tokenId, uint amount_) private {
        IKTA KTA = IKTA(addrList[fromAddrId_]);
        if (tokenId == 0) {
            KTA.mintMulti(_msgSender(), cardId_, amount_);
        } else {
    ...
    function _claimERC1155(uint airdropId_, uint fromAddrId_, uint cardId_, uint  amouns_) private {
        IKTN(addrList[fromAddrId_]).mint(_msgSender(), cardId_, amouns_);
