...
contract GameItems is Ownable, ERC721URIStorage {
  ...
  constructor(address _olymAdd, uint256 _boxPrice) ERC721("The Olympus","OLYM"){
    olymAddress = _olymAdd;
    boxPrice = _boxPrice;
  }

  function setBoxPrice(uint256 newPrice) external  virtual onlyOwner {
    require(newPrice > 0, "Item: Price is zero");
    boxPrice = newPrice;
  }

  function mint(uint256 amount) public returns (uint256){
    require(amount >= boxPrice,"Item: Not enough token");
