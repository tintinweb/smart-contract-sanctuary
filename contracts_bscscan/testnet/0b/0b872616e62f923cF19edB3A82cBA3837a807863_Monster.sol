...
contract Monster is ERC721Enumerable {
    ...
    mapping(uint => string) public profession;//职业

    string[79] private monsters = [//怪物
        "Kidnapper",
        "Axe Gang",
        "Jackstraw",
        "Serpent",
    ...
    uint[79] private professions = [//职业
        1,
        2,
        3,
        4,
    ...
    uint public next_monster;//下一个怪物

    uint constant TOTAL = 30;//全部的
    uint constant POINT = 9;//观点

    mapping(uint => string) public monster;//怪物
    mapping(uint => string) public prefix;//前缀
    mapping(uint => uint) public suffix;//后缀

    mapping(uint => uint) public health_Point;//健康点
    mapping(uint => uint) public physical_damage_point;//物理伤害点
    mapping(uint => uint) public magical_damage_point;//魔法伤害点
    mapping(uint => uint) public physical_defence;//物理防御
    mapping(uint => uint) public magical_defence;//魔法防御
    mapping(uint => uint) public dodge;//躲闪
    mapping(uint => uint) public hit;//打
    mapping(uint => uint) public critical;//临界
    mapping(uint => uint) public parry;//招架

    uint[POINT] private basePoints;//基点
    ...
    function claim() public payable{
        require(msg.value == 1e18, "1FTM IS REQUIRED");
