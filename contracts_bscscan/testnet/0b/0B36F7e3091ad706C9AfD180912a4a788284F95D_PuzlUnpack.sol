...
contract PuzlUnpack is ERC1155 {
    ...
    constructor(address _puzl) ERC1155("") {
        puzl = IERC721(_puzl);
    }
    ...
    function onERC721Received(
        ...
        // only supports callback from the Puzl contract
    ...
    function open(address who, uint256 tokenId) private {
       ...
       _mintAPart(who, "male punk");
       _mintAPart(who, "red hair");
       _mintAPart(who, "beard shadow");
       
       // _mint(who, 'PUNK', 10**18, "");
     ...
     function _mintAPart(address _to, string memory _name) private {
         uint id = _mintedParts();
        _partIdTracker.increment();
        _mint(_to, id, 1, bytes(_name));
        emit CreatePart(id);
    }
