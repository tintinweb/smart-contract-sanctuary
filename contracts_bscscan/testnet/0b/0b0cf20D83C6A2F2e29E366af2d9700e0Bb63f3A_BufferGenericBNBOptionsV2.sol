...
interface IBufferOptions {
    ...
    event UpdateOptionCreationStopWindow(
        uint256 startHour,
        uint256 startMinute,
        uint256 endHour,
        uint256 endMinute
    ...
    struct OptionCreationStopWindow {
        uint256 startHour;
        uint256 startMinute;
        uint256 endHour;
        uint256 endMinute;
    ...
    ReentrancyGuard
{
    ...
    OptionCreationStopWindow public optionCreationStopWindow =
        OptionCreationStopWindow(0, 0, 0, 0);
     ...
     */
    function setOptionCreationStopWindow(
        uint256 startHour,
        uint256 startMinute,
        uint256 endHour,
        uint256 endMinute
    ) external onlyOwner {
        optionCreationStopWindow = OptionCreationStopWindow(
            startHour,
            startMinute,
            endHour,
            endMinute
        );
        emit UpdateOptionCreationStopWindow(
            startHour,
            startMinute,
            endHour,
            endMinute
    ...
    function canCreate() internal view returns (bool) {
        ...
        uint256 minute = (block.timestamp % 3600) / 60;

        if (
            ((hour == optionCreationStopWindow.startHour &&
                minute < optionCreationStopWindow.startMinute) ||
                hour < optionCreationStopWindow.startHour) ||
            (hour > optionCreationStopWindow.endHour ||
                (hour == optionCreationStopWindow.endHour &&
                    minute > optionCreationStopWindow.endMinute))
        ) {
            return true;
        }
        return false;
    ...
    function create(
        ...
        require(canCreate(), "Creation isn't allowed in this time window");
