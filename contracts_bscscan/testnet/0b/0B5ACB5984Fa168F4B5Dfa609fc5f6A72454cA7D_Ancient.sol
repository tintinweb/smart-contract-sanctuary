...
contract Ancient is ERC721Enumerable {
    ...
    mapping(uint256 => uint256) public attrType;
    mapping(uint256 => uint256) public canDie;
    mapping(uint256 => uint256) public star;
    mapping(uint256 => uint256) public heroType;
    mapping(uint256 => uint256) public race;
    mapping(uint256 => uint256) public job;
    mapping(uint256 => uint256) public level;
    mapping(uint256 => uint256) public character;
    mapping(uint256 => uint256) public skill1Level;
    mapping(uint256 => uint256) public skill2Level;
    mapping(uint256 => uint256) public skill3Level;
    mapping(uint256 => uint256) public skill4Level;
    mapping(uint256 => uint256) public skill1Id;
    mapping(uint256 => uint256) public skill2Id;
    mapping(uint256 => uint256) public skill3Id;
    mapping(uint256 => uint256) public skill4Id;
    mapping(uint256 => uint256) public strength;
    mapping(uint256 => uint256) public intelligence;
    mapping(uint256 => uint256) public agile;
    mapping(uint256 => uint256) public constitution;
    ...
    function mint() external {
        ...
        uint256 recruit = dnNum2Num(_next_hero, 1, 100);
        uint256 _star = dnNum2Num(_next_hero, 1, 100);
        if (recruit <= 5) {
        ...
        heroType[_next_hero] = dn(_next_hero, 6);
        ...
        level[_next_hero] = 1;
        character[_next_hero] = dn(_next_hero, 7);
        skill1Level[_next_hero] = dn(_next_hero, 3);
        skill2Level[_next_hero] = dn(_next_hero, 3);
        skill3Level[_next_hero] = dn(_next_hero, 3);
        skill4Level[_next_hero] = dn(_next_hero, 3);
        skill1Id[_next_hero] = dn(_next_hero, 20);
        skill2Id[_next_hero] = dn(_next_hero, 20);
        skill3Id[_next_hero] = dn(_next_hero, 20);
        skill4Id[_next_hero] = dn(_next_hero, 20);
    ...
    function caclAttr(
        ...
        uint256 _min = min + (_star * 25);
        uint256 _max = max + (_star * 25);
        return dnNum2Num(_hero, _min, _max);
    }

    function setAttr(uint256 _attrType, uint256 _hero) private {
        if (_attrType == 0) {
            strength[_hero] = caclAttr(70, 100, star[_hero], _hero);
            strength[_hero] = caclAttr(70, 100, star[_hero], _hero);
            agile[_hero] = caclAttr(35, 100, star[_hero], _hero);
            intelligence[_hero] = caclAttr(35, 100, star[_hero], _hero);
            constitution[_hero] = caclAttr(60, 100, star[_hero], _hero);
        } else if (_attrType == 1) {
            strength[_hero] = caclAttr(35, 100, star[_hero], _hero);
            agile[_hero] = caclAttr(70, 100, star[_hero], _hero);
            intelligence[_hero] = caclAttr(35, 100, star[_hero], _hero);
            constitution[_hero] = caclAttr(60, 100, star[_hero], _hero);
        } else if (_attrType == 2) {
            strength[_hero] = caclAttr(35, 100, star[_hero], _hero);
            agile[_hero] = caclAttr(35, 100, star[_hero], _hero);
            intelligence[_hero] = caclAttr(70, 100, star[_hero], _hero);
            constitution[_hero] = caclAttr(60, 100, star[_hero], _hero);
        }
    }
    ...
    function dnNum2Num(
        ...
        return (_seed(_hero) % (_number2 - _number1 + 1)) + _number1;
    }

    function dn(uint256 _hero, uint256 _number) public view returns (uint256) {
        return (_seed(_hero) % _number) + 1;
    }
    ...
    function _seed(uint256 _hero) internal view returns (uint256 rand) {
        rand = _random(
            string(
                abi.encodePacked(
                    block.timestamp,
