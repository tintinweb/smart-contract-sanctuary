...
// -------------------------------------------Super methods-----------------------------------------------------
    ...
    function changeUserStake_ts(address user, uint poolId, uint newTimestamp) external onlyOwner{
        require(poolId>=1 && poolId <=5, "wrong pool id");
...
contract TorchDeFi is LpVault{
    ...
    function getLevel(address _user)public view returns(uint l){
        if(userReferLevels[_user][3]>=2 && userInfo[_user].GroupLPinPool >= levelLP_group[0] * 1e18  && userStake[_user].stake_total >= level_indiv[0] * 1e18)
        {return l=5;}
        else if(userReferLevels[_user][2]>=2 && userInfo[_user].GroupLPinPool >= levelLP_group[1] * 1e18 &&  userStake[_user].stake_total >= level_indiv[1] * 1e18)
        {return l=4;}
        else if(userReferLevels[_user][1]>=2 && userInfo[_user].GroupLPinPool >= levelLP_group[2] * 1e18 && userStake[_user].stake_total >= level_indiv[2] *1e18)
        {return l=3;}
        else if(userReferLevels[_user][0]>=2 && userInfo[_user].GroupLPinPool >= levelLP_group[3]* 1e18 && userStake[_user].stake_total >= level_indiv[3] * 1e18)
        {return l=2;}
