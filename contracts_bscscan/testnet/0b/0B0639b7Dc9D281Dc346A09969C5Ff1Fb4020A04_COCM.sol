...
contract COCM is Context, Ownable {
    ...
    * @dev Emitted when `value` tokens are moved from one account (`from`) to
    ...
    struct LockInfo {
        ...
        uint256 swapped;
        uint256 blockNUnlock;
        bool lockedByAdmin;
        bool unlockManual;
    }
    mapping(address => LockInfo) _addressLockInfo;
//    uint256 _TIME_LOCKED = 17280000;
    uint256 _TIME_SIX_MONTH = 200;
    uint256 _TIME_ONE_MONTH = _TIME_SIX_MONTH.div(6);
    ...
    * Set rate unlock to 10% and unlock the token after 17280000 blocks than block transaction ( 6 month )
    */
    function deliverToAccountWithRate10(address to_, uint256 amount_) public onlyAdmin returns (bool) {
        ...
        lockInfoAddress.blockNUnlock = block.number + _TIME_SIX_MONTH;
    ...
    /**
    * @dev Send quantity of `amount_` in balance address `to_`
    * Set rate unlock to 5% and unlock the token after 17280000 blocks than block transaction ( 6 month )
    */
    ...
    */
    ...
    function getInfoLockedByAddress(address owner_) public view returns(uint256, uint256, bool) {
        uint256 amountUnlock = _addressLockInfo[owner_].amountUnlock;
        uint256 blockNUnlock = _addressLockInfo[owner_].blockNUnlock;
        bool lockedByAdmin = _addressLockInfo[owner_].lockedByAdmin;

        return (amountUnlock, blockNUnlock, lockedByAdmin);
    }
    ...
    function getTokenUnlock() public view returns(uint256) {
        ...
        if (lockInfoAddress.unlockManual) {
            return lockInfoAddress.amountUnlock - lockInfoAddress.swapped;
        }

        if (block.number < lockInfoAddress.blockNUnlock) {
            return 0;
        }
        uint256 rate = (block.number - lockInfoAddress.blockNUnlock) / _TIME_ONE_MONTH;
        if (rate == 0) {
            return 0;
        }
        if (((lockInfoAddress.amountUnlock * rate) - lockInfoAddress.swapped) > _balances[_msgSender()]) {
    ...
    function swapCOCMtoCOC() public returns(bool) {
        ...
        LockInfo memory lockInfoAddress = _addressLockInfo[_msgSender()];
        require(block.number > lockInfoAddress.blockNUnlock, "COCM: token is already locked");
        require(!lockInfoAddress.lockedByAdmin, "COCM: swap is already locked by admin");
        uint256 tokenToUnlock = getTokenUnlock();
