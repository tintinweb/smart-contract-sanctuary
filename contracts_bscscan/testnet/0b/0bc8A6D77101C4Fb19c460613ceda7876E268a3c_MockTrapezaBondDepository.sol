...
contract MockTrapezaBondDepository is Ownable {
  ...
  address public immutable FIDL; // token given as payment for bond
  address public immutable principle; // token used to create bond
  address public immutable treasury; // mints FIDL when receives principle
  ...
  struct Bond {
    uint256 payout; // FIDL remaining to be paid
  ...
  function deposit(
    ...
    require(payout >= 10000000, "Bond too small"); // must be > 0.01 FIDL ( underflow protection )
    ...
    /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) FIDL
         */
    ...
    if (fee != 0) {
      ...
      IERC20(FIDL).safeTransfer(DAO, fee);
    }
  ...
  function stakeOrSend(
    ...
    if (!_stake) {
      ...
      IERC20(FIDL).transfer(_recipient, _amount); // send payout
    } else {
      // if user wants to stake
      if (useHelper) {
        // use if staking warmup is 0
        IERC20(FIDL).approve(stakingHelper, _amount);
        IStakingHelper(stakingHelper).stake(_amount, _recipient);
      } else {
        IERC20(FIDL).approve(staking, _amount);
  ...
  function maxPayout() public view returns (uint256) {
    return IERC20(FIDL).totalSupply().mul(terms.maxPayout).div(100000);
  }
  ...
  /**
   *  @notice calculate current ratio of debt to FIDL supply
   *  @return debtRatio_ uint
   */
  function debtRatio() public view returns (uint256 debtRatio_) {
    uint256 supply = IERC20(FIDL).totalSupply();
  ...
  /**
   *  @notice calculate amount of FIDL available for claim by depositor
   *  @param _depositor address
  ...
  /**
   *  @notice allow anyone to send lost tokens (excluding principle or FIDL) to the DAO
   *  @return bool
   */
