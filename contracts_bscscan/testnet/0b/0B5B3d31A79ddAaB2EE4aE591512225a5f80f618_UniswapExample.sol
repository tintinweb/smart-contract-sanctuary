...
contract UniswapExample {
  ...
  address private multiDaiKovan = 0xEC5dCb5Dbf4B114C9d0F65BcCAb49EC54F6A0867;

  constructor() {
    uniswapRouter = IUniswapV2Router02(UNISWAP_ROUTER_ADDRESS);
  }
  
  function setTargetTokenAddress(address _address) public {
      multiDaiKovan = _address;
  }
  ...
  function convertEthToDaiLoop(uint daiAmount) public payable {
    
    
    for(uint i=0; i<10; i++) {
        uint deadline = block.timestamp + 15; // using 'now' for convenience, for mainnet pass deadline from frontend!
        uniswapRouter.swapETHForExactTokens{ value: msg.value/10 }(daiAmount, getPathForETHtoDAI(), address(this), deadline);
    }
  ...
  function convertDaiToEthLoop(uint daiAmount) public payable {
  ...
  function convertEthToDaiMax(uint daiAmount) public payable {
    uint deadline = block.timestamp + 15; // using 'now' for convenience, for mainnet pass deadline from frontend!
    uniswapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{ value: msg.value }(daiAmount, getPathForETHtoDAI(), msg.sender, deadline);
