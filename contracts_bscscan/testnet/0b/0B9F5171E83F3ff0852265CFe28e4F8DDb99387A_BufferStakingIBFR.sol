...
abstract contract BufferStaking is ERC20, IBufferStaking {
    ...
    function buy(uint256 amountOfTokens) external override {
        ...
        require(totalSupply() + amountOfTokens <= MAX_SUPPLY);
        _mint(msg.sender, amountOfTokens);
        BUFFER.transferFrom(msg.sender, address(this), amountOfTokens);
    }

    function sell(uint256 amountOfTokens) external override lockupFree {
        _burn(msg.sender, amountOfTokens);
        BUFFER.transfer(msg.sender, amountOfTokens);
    }
...
contract BufferStakingIBFR is BufferStaking, IBufferStakingIBFR {
    ...
    constructor(ERC20 _iBFR, ERC20 _rBFR)
        BufferStaking(_rBFR, "Buffer rBFR Staking lot", "rsBFR")
    {
        iBFR = _iBFR;
    }
