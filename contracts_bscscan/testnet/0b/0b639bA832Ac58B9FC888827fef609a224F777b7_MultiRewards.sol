...
contract MultiRewards is ReentrancyGuard, Pausable {
    ...
    function unlockedStakeAtTime(address account, uint256 thisTime, uint256 stakeI) public view returns (uint256) {
	    if (userStakes[account][stakeI].remainingBalance > 0 && userStakes[account][stakeI].stakeTime.add(lockDuration) <= thisTime)		
    ...
    function stake(uint256 amount) external nonReentrant notPaused updateReward(msg.sender) {
        ...
        userStakes[msg.sender].push(Stake(block.timestamp, amount));
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        ...
        require(actualAmount > 0 && actualAmount <= amount, "No amount available or internal error");        
