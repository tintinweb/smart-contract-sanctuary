...
contract DexUpdates is Ownable, DexStorage {
  ...
  function init() public {
    XToken = IERC20(0x3df50c7C5008230C92C75Db8CF4c6f5d903FCD64);
  }
  ...
  function buyNFT(address nft_a,uint256 tokenId, string memory ownerId, uint256 currencyType) isValid(nft_a) public{
        ...
        require(currencyType == 1 || (_allowedCurrencies[tokenId][nft_a][currencyType] && currencyType != 2), "123");
        require(msg.sender != nonCryptoNFTVault || currencyType == 1, "124" );
        
        uint256 price = calculatePrice(temp.price, temp.currencyType, temp.seller == nonCryptoNFTVault ? 1 :currencyType, tokenId, temp.seller, nft_a);
        ...
        if(temp.seller == nonCryptoNFTVault) {
          if(msg.sender != nonCryptoNFTVault){
              XToken.mint(temp.seller, (price  / 1000) * mainPerecentage);
          }
