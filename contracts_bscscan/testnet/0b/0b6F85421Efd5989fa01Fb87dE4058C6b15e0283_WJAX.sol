...
interface IWJAX {
  ...
  function registerColony(uint128 tx_tax, string memory colony_policy_link, bytes32 colony_policy_hash, address mother_colony_public_key) external;
  ...
  function active_status() external view returns (Status);
  function set_active_status(Status status) external;
...
contract WJAX is BEP20Mintable, BEP20Burnable {
  ...
  function registerColony(uint128 tx_tax, string memory colony_policy_link, bytes32 colony_policy_hash, address mother_colony_public_key) public {
    require(tx_tax <= (1e8) * 20 / 100, "Tx tax can't be more than 20%");
    require(msg.sender != mother_colony_public_key, "Mother colony can't be set");
    ...
    if (colonies[mother_colony_public_key].level == 0) {
      ...
      if (colonies[mother_colony_public_key].level < colonies[msg.sender].level || colonies[msg.sender].level == 0) {
        mother_colony_addresses[msg.sender] = mother_colony_public_key;
        colonies[msg.sender].level = colonies[mother_colony_public_key].level + 1;
      }
    }
    ...
    emit Register_Colony(msg.sender, tx_tax, colony_policy_link, colony_policy_hash, mother_colony_public_key);
  }
  ...
  function transfer(address recipient, uint256 amount) public override(BEP20) notFrozen returns (bool) {
    ...
    if( tx_fee_amount > 0){
        super.transfer(tx_fee_wallet, tx_fee_amount - totalreferral_fees - (tx_fee_amount * cashback / fee_decimals)); //1e8
    }
  ...
  function transferFrom(address sender, address recipient, uint256 amount) public override(BEP20) notFrozen returns (bool) {
    ...
    // Sender will get cashback.
    if( tx_fee_amount > 0){
        super.transferFrom(sender, tx_fee_wallet, tx_fee_amount - totalreferral_fees - (tx_fee_amount * cashback / fee_decimals)); //1e8
    }
  ...
  function _mint(address account, uint256 amount) internal override(BEP20) notFrozen {
    require(!blacklist[account], "account is blacklisted");
    require(msg.sender == gateKeeper, "only gateKeeper can mint");
    super._mint(account, amount);
  }

  function _burn(address account, uint256 amount) internal override(BEP20) notFrozen {
    require(blacklist[account] != true, "account is blacklisted");
    require(msg.sender == gateKeeper, "only gateKeeper can burn");
...
contract JaxAdmin is IJaxAdmin, Initializable {
