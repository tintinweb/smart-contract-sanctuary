...
contract HiReX is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public _taxFee = 1; // Fee that gets re-distributed to holders.
    ...
    uint256 private _totalLiqFee = _liquidityFee.add(_marketingFee);
    ...
    constructor (address payable LiquidityWallet, address payable MarketingWallet, address UniSwapRouter) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 ethForMarketing = balanceDiff.mul(_marketingFee).div(_totalLiqFee);
        uint256 ethForLiq = balanceDiff.mul(_liquidityFee).div(_totalLiqFee);
        uint256 ethForLiquidity = balanceDiff.sub(ethForMarketing).sub(ethForLiq);
        ...
        liquidityWallet.transfer(ethForLiq);
    }
    ...
    function setFees(uint256 taxFee, uint256 burnFee, uint256 liquidityFee, uint256 marketingFee) external onlyOwner {
        _taxFee = taxFee;
        _burnFee = burnFee;
    ...
    //buy
    function setBuyFees(uint256 buytaxFee, uint256 buyburnFee, uint256 buyliquidityFee, uint256 buymarketingFee) external onlyOwner {
    ...
    //sell
    function setSellFees(uint256 selltaxFee, uint256 sellburnFee, uint256 sellliquidityFee, uint256 sellmarketingFee) external onlyOwner {
    ...
    function changeUniSwapRouter(address _router) public onlyOwner returns(address _pair) {
