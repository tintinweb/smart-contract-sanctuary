...
interface IMigratorFarm {
    // Perform LP token migration from legacy PanluckySwap to LuckySwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to PanluckySwap LP tokens.
    // LuckySwap must mint EXACTLY the same amount of LuckySwap LP tokens or
    // else something bad will happen. Traditional PanluckySwap does not
    // do that so be careful!
    function migrate(IBEP20 token) external returns (IBEP20);
}

// Farm is the master of Lucky. He can make Lucky and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once LUCKY is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.

interface ISpaceHunter {
    function getRateReward(uint256 _tokenId) external view returns(uint8);
}
...
contract Farms is Ownable {
    ...
    address public constant SPACEHUNTER = 0xb3597830f51a9d57623318D389B38C70c94ADfa8;

    // Info of each user.
    struct UserInfo {
        uint256 amount;     // How many LP tokens the user has provided.
        uint256 rewardDebt; // Reward debt. See explanation below.
        uint256 nftBoosted; // Token ID NFT to boost reward
        //
    ...
    event SetNFTBoosted(address indexed user, uint256 indexed pid, uint256 tokenId);
    event UnsetNFTBoosted(address indexed user, uint256 indexed pid, uint256 tokenId);
    ...
    function setNftBoosted(uint256 _pid, uint256 _tokenId) external {
        UserInfo storage user = userInfo[_pid][msg.sender]; 
        PoolInfo storage pool = poolInfo[_pid];
        IERC721(SPACEHUNTER).safeTransferFrom(address(msg.sender), address(this), _tokenId);
        updatePool(_pid);

        uint256 pending = user.amount.mul(pool.accLuckyPerShare).div(1e12).sub(user.rewardDebt);
        if(pending > 0) {
            if(user.nftBoosted > 0) {
                uint8 rateReward = ISpaceHunter(SPACEHUNTER).getRateReward(user.nftBoosted);
                safeTransferReward(msg.sender, pending.mul(rateReward).div(100));
            }
            else 
            {
        ...
        user.nftBoosted = _tokenId;
        user.rewardDebt = user.amount.mul(pool.accLuckyPerShare).div(1e12);

        emit SetNFTBoosted(msg.sender, _pid, _tokenId);
    }

    function unsetNftBoosted(uint256 _pid) external {
        UserInfo storage user = userInfo[_pid][msg.sender]; 
        PoolInfo storage pool = poolInfo[_pid];
        uint256 nftBoosted = user.nftBoosted;
        updatePool(_pid);

        uint256 pending = user.amount.mul(pool.accLuckyPerShare).div(1e12).sub(user.rewardDebt);
        if(pending > 0) {
            if(nftBoosted > 0) {
                uint8 rateReward = ISpaceHunter(SPACEHUNTER).getRateReward(nftBoosted);
                safeTransferReward(msg.sender, pending.mul(rateReward).div(100));
            }
            else 
            {
        ...
        IERC721(SPACEHUNTER).safeTransferFrom(address(this), address(msg.sender), nftBoosted);

        user.nftBoosted = 0; 
        user.rewardDebt = user.amount.mul(pool.accLuckyPerShare).div(1e12);

        emit UnsetNFTBoosted(msg.sender, _pid, nftBoosted);
    }
    ...
    function setRewardPerBlock(uint256 _luckyPerBlock) public onlyOwner {
        luckyPerBlock = _luckyPerBlock;
    }
    ...
    function pendingLucky(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if(user.nftBoosted > 0) {
            uint8 rateReward = ISpaceHunter(SPACEHUNTER).getRateReward(user.nftBoosted);
            return user.amount.mul(accLuckyPerShare).div(1e12).sub(user.rewardDebt).mul(rateReward).div(100);
        }
        else 
        {
            return user.amount.mul(accLuckyPerShare).div(1e12).sub(user.rewardDebt);
        }
    }
    ...
    // Deposit LP tokens to Farm for LUCKY allocation.
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(pending > 0) {
            if(user.nftBoosted > 0) {
                uint8 rateReward = ISpaceHunter(SPACEHUNTER).getRateReward(user.nftBoosted);
                safeTransferReward(msg.sender, pending.mul(rateReward).div(100));
                IERC721(SPACEHUNTER).safeTransferFrom(address(this), address(msg.sender), user.nftBoosted);
                user.nftBoosted = 0; 
            }
            else 
            {
