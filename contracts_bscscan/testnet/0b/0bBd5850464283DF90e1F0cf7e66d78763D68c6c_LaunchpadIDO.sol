...
contract LaunchpadIDO is Adminable, ReentrancyGuard, Timed, GeneralIDO, Withdrawable, WithLimits, WithWhitelist, WithLevelsSale, WithClaimer {
...
abstract contract WithClaimer is Adminable {
    ...
    event ClaimerChanged(Claimer claimer);

    function setClaimer(string calldata _network, string calldata _address) external onlyOwnerOrAdmin {
        claimer = Claimer({network : _network, addr : _address});
