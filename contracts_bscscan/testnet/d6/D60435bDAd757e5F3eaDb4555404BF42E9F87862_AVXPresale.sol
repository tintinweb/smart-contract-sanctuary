...
contract AVXPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	// Price of presale AVX, 0.0001 BNB
	uint256 BNBPerToken = 10 ** 15;
	// Amount of BNB received in presale
	...
	function buy(uint256 _amount) payable public nonReentrant {
		...
		// require (BNBReceived + _amount <= maxHardCap, "Presale hardcap reached");
        // require(_amount >= minimumDepositEthAmount, "low amount than min");
        // require(_amount + (tokensOwned[msg.sender] * BNBPerToken) <= maximumDepositEthAmount, "high amount than max");        
        
		//address _buyer = msg.sender;
		uint256 tokens = _amount.div(BNBPerToken);		
		
		// BNB.safeTransferFrom(msg.sender, address(this), _amount);
        payable(msg.sender).transfer(_amount);

        AVX.safeTransfer( msg.sender, tokens );

		// tokensOwned[_buyer] = tokensOwned[_buyer].add(tokens);
		// tokensUnclaimed[_buyer] = tokensUnclaimed[_buyer].add(tokens);
		// totalTokensSold = totalTokensSold.add(tokens);
		BNBReceived = BNBReceived.add(_amount);
	...
	function getAVXTokensLeft() external view returns (uint256) {
		return AVX.balanceOf(address(this));
	}

	function withdrawFunds() external onlyOwner {
		BNB.safeTransfer(msg.sender, BNB.balanceOf(address(this)));
	}

	function withdrawUnsoldAVX() external onlyOwner {
		uint256 amount = AVX.balanceOf(address(this)) - totalTokensSold;
		AVX.safeTransfer(msg.sender, amount);
	}

	function withdrawAllAVX() external onlyOwner {
		AVX.safeTransfer(msg.sender, AVX.balanceOf(address(this)));
	}
