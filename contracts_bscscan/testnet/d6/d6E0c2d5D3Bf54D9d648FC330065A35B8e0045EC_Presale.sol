...
contract Presale is ReentrancyGuard {
    ...
    // Price of presale TKN, 5 BUSD
    uint256 public BUSDPerToken = 5;
    ...
    function buy(uint256 _amount, address beneficiary) public nonReentrant {
        ...
        require(
            busdReceived + _amount <= 1000 ether,
    ...
    function getTKNTokensLeft() external view returns (uint256) {
    ...
    function claimTokens(address claimer) external {
        ...
        require(tokensOwned[msg.sender] > 0, "User should own some TKN tokens");
        require(
            tokensUnclaimed[msg.sender] > 0,
            "User should have unclaimed TKN tokens"
        );
        require(
            TKN.balanceOf(address(this)) >= tokensUnclaimed[msg.sender],
            "There are not enough TKN tokens to transfer."
        );

        TKN.safeTransfer(msg.sender, tokensUnclaimed[msg.sender]);
    ...
    function withdrawUnsoldTKN() external onlyOwner {
        uint256 amount = TKN.balanceOf(address(this)) - totalTokensSold;
        TKN.safeTransfer(msg.sender, amount);
    }

    function withdrawAllTKN() external onlyOwner {
        TKN.safeTransfer(msg.sender, TKN.balanceOf(address(this)));
    }
}
