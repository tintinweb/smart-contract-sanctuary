...
/**
//Fair launched in SafeSatoshi. We have created a token so rewarding for everyone.
//SafeSatoshi - 
//Telegram: https://t.me/SafeSatoshigroup
*/
 ...
 * 
NOTE:
 `SafeMath` is no longer needed starting with Solidity 0.8. The compiler
...
return
 functionCallWithValue(target, data, 
0
, errorMessage);
    }
...
contract SafeSatoshi is Context, IERC20, Ownable {
    using SafeMath 
for
 uint256;
    using Address 
...
address addr1,address addr2
        ...
        _marketingWalletAddress = addr1;
        _teamWalletAddress = addr2;
...
{
        (uint256 rAmount, uint256 rTransferAmount, RFees memory rFees, uint256 tTransferAmount, TFees memory tFees) = _getValues(tAmount);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);        
        _takeFees(tFees.tLiquidity + tFees.tMarketing);
        _reflectFee(rFees.rFee, tFees.tFee, rFees.rBurn, tFees.tBurn);
...
updateMaxSellTransactionAmount
(
uint256 maxSell
) 
public
...
{
        maxSellTransactionAmount = maxSell * (
10
**
9
);
...
{
        IERC20 BEP20 = IERC20(bep20);
        uint256 balance = BEP20.balanceOf(address(
this
));
...
this
        ...
        payable(_marketingWalletAddress).transfer(amountBNB * amountPercentage / 
100
);
    }
...
_reflectFee
(
uint256 rFee, uint256 tFee, uint256 rBurn, uint256 tBurn
) 
private
...
returns
 (
uint256, uint256, RFees memory, uint256, TFees memory
) 
{
        (uint256 tTransferAmount, TFees memory tFees) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, RFees memory rFees) = _getRValues(tAmount, tFees, _getRate());
        
return
 (rAmount, rTransferAmount, rFees, tTransferAmount, tFees);
    }
...
returns
 (
uint256, TFees memory
) 
{
        TFees memory tFees;
        tFees.tFee = calculateFee(tAmount, _taxFee);
        tFees.tBurn = calculateFee(tAmount, _burnFee);
        tFees.tLiquidity = calculateFee(tAmount, _liquidityFee);
		tFees.tMarketing = calculateFee(tAmount, _marketingFee);
        
return
 (tAmount.sub(tFees.tFee).sub(tFees.tBurn).sub(tFees.tLiquidity).sub(tFees.tMarketing), tFees);
    }
...
_getRValues
(
uint256 tAmount, TFees memory tFees, uint256 currentRate
) 
private
...
returns
 (
uint256, uint256, RFees memory
) 
{
        uint256 rAmount = tAmount.mul(currentRate);
        RFees memory rFees;
        rFees.rFee = tFees.tFee.mul(currentRate);
        rFees.rBurn = tFees.tBurn.mul(currentRate);
        rFees.rLiquidity = tFees.tLiquidity.mul(currentRate);
		rFees.rMarketing = tFees.tMarketing.mul(currentRate);
        
return
 (rAmount, rAmount.sub(rFees.rFee).sub(rFees.rBurn).sub(rFees.rLiquidity).sub(rFees.rMarketing), rFees);
    }
...
{
        uint256 sumFees = _marketingFee.add(_liquidityFee);
        uint256 liquifyTokens = contractTokenBalance.mul(_liquidityFee).div(sumFees);
        uint256 marketingTokens = contractTokenBalance - liquifyTokens;
        
// split the contract balance into halves

        uint256 half = liquifyTokens.div(
2
);
...
// add liquidity to uniswap
        ...
        uint256 onethird = marketingTokens.div(
3
);
        uint256 leftover = marketingTokens.sub(onethird);
        swapTokensForEthAndSendEthToAddress(onethird, _marketingWalletAddress);
        swapTokensForEthAndSendEthToAddress(onethird, _teamWalletAddress);
    }
...
swapTokensForEthAndSendEthToAddress
(
uint256 tokenAmount, address to
) 
private
