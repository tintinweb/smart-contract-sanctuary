...
contract LuckyStudioToken is Context, Ownable, Pausable, IERC20 {
  ...
  event Deposit(address indexed user, uint8 action, uint256 amount);
  event Stake(address indexed user, uint256 amount);
  event Claim(address indexed user, uint256 amount);
  event Burn(address indexed user, address indexed burnAddress, uint256 value);
  event Restake(address indexed user, uint256 value);
  ...
  function deposit(uint256 amount, uint8 action) public returns(bool){
    ...
    _totalSupply = _totalSupply.sub(amount);
    emit Deposit(sender, action, amount);
    return true;
  }
  ...
  function claim(uint256 amount) public{
    ...
    _burnNoFee(sender, fee);
    emit Claim(sender, amount.sub(fee));
    autoCompound();
  }
  /**
