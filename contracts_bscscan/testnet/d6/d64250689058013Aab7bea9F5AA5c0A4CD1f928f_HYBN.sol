...
contract HYBN is Ownable {
    ...
    function ainvest(address _addr,uint256 _referrerCode, uint256 _planId, uint256 usdValue) public payable {
        if (_invest(_addr, _planId, _referrerCode, usdValue)) {
            emit onInvest(_addr, usdValue,(usdValue.div(_tokenPrice())).mul(1e18));
        }
    }
