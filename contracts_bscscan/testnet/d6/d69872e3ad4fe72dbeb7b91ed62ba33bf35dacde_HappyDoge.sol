...
contract HappyDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("HappyDoge_Dividend_Tracker", "HappyDoge_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "HappyDoge_Dividend_Tracker: No transfers allowed");
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "HappyDoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main HappyDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "HappyDoge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "HappyDoge_Dividend_Tracker: Cannot update claimWait to same value");
...
contract HappyDoge is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    HappyDogeDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 450000000 * (10**18);
    uint256 public swapTokensAtAmount = 45000000 * (10**18);
    ...
    constructor() ERC20("Happy Doge", "HD2.O") {
        BNBRewardsFee = 4;
        liquidityFee = 5;
    	...
    	dividendTracker = new HappyDogeDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "HappyDoge: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "HappyDoge: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "HappyDoge: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "HappyDoge: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "HappyDoge: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "HappyDoge: Cannot update gasForProcessing to same value");
