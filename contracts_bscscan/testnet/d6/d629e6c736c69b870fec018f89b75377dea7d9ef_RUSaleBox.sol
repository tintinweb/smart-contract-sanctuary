...
contract RUSaleBox is Ownable, Pausable {
    ...
    // ticket id
    mapping(uint256 => address) public saleBoxAdress;
    mapping(uint256 => uint256) public saleBoxType;
    ...
    function setOpener(address _account) public onlyOwner {
        opener = _account;
    }
    ...
    function saleBox(uint256 ticketId, uint256 _type) payable external {
        require(saleBoxAdress[ticketId] == address(0), "ticketId already sale");
        ...
        saleBoxAdress[ticketId] = msg.sender;
        saleBoxType[ticketId] = _type;
    }

    function openBox(uint256 ticketId, address buyer, string[] memory _inputMetadatas) external {
        require(saleBoxAdress[ticketId] == buyer, "buyer of ticketId is not correct");
        require(isOpenBox[ticketId] == false, "ticketId already open");
        require(opener == msg.sender, "caller is not opener");

        for (uint256 i = 0; i < _inputMetadatas.length; i++) {
            nftPlayer.mint(buyer, _inputMetadatas[i]);
        }

        isOpenBox[ticketId] = true;
    }
