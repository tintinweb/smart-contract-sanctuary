...
/**
 * @title AntiG price Oracle
 *      This Oracle calculates the average USD price of AntiG based on the BNB-AntiG and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private AntiGBnbPrice0CumulativeLast;
    uint private AntiGBnbPrice1CumulativeLast;
    uint32 private AntiGBnbBlockTimestampLast;
    ...
    // wbnb+busd LP on testnet => 0xe0e92035077c39594793e61802a350347c320cf2.
    //AntiG+wbnb LP on testnet => 0x466195092066141d76cDFd61537e34E0F213a41F.

    IUniswapV2Pair private _AntiG_bnb;
    ...
    constructor(
        address __AntiG_bnb,   // Address of the AntiG-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _AntiG_bnb = IUniswapV2Pair(__AntiG_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        AntiGBnbPrice0CumulativeLast = _AntiG_bnb.price0CumulativeLast();
        AntiGBnbPrice1CumulativeLast = _AntiG_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, AntiGBnbBlockTimestampLast) = _AntiG_bnb.getReserves();
    ...
    // Get the average price of 1 AntiG in the smallest BNB unit (18 decimals)
    function getAntiGBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_AntiG_bnb));

        require(_blockTimestamp != AntiGBnbBlockTimestampLast, "AntiG Last and current are equal");

        FixedPoint.uq112x112 memory AntiGBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - AntiGBnbPrice0CumulativeLast) / (_blockTimestamp - AntiGBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, AntiGBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        uint AntiGBnbAverage;
        uint busdBnbAverage;

        (AntiGBnbPrice0CumulativeLast, AntiGBnbPrice1CumulativeLast, AntiGBnbBlockTimestampLast, AntiGBnbAverage) = getAntiGBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, AntiGBnbAverage) = getAntiGBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = AntiGBnbAverage*(1e6) / busdBnbAverage;
