...
contract MysteriousCrates is ERC721Enumerable, RSunAuth, IMysteriousCrates {
    ...
    address public samuraiAdr = 0x14a3Ee3771845cee9EA2D49Fcca8DDA58f5D5D8b; // 0xC3c3B849ED5164Fb626c4a4F78e0675907B2C94E Mainnet
    ...
    event CrateMinted(address user, uint crateId);
    event CrateOpened(address user, uint crateId, uint newSamuraiId);

    constructor() ERC721("SamuraiRising", "Samurai Rising") RSunAuth(tx.origin) {
    ...
    function mintCrate(address user, uint[] memory cardIds) external override {
        ...
        emit CrateMinted(user, mintIndex);
    }

    function openCrate(uint crateId) external {
        Crate memory crate = crates[crateId];

        require(ownerOf(crateId) == msg.sender, "Can only open one's own crates");
        require(crate.opened == false, "Crate already opened");
    ...
    function getAllCratesOfUser(address user) external view returns(OutputCrate[] memory crates_) {
        ...
        for (uint256 i = 0; i < crateIds.length; i++) {
            ...
            crates_[i] = OutputCrate(crateIds[i], ids, crate.opened);
        }
    }
