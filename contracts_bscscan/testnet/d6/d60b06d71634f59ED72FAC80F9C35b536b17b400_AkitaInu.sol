...
contract AkitaInu is Context, IERC20, Ownable {
    ...
    uint256 constant private numTokensSellToAddToLiquidity = 5000000 * 10**9;
    ...
    function setMarketingDevFeePercent(uint256 marketingAndDevBudget) external onlyOwner {
        require(marketingAndDevBudget <= 15, "Marketing fee cannot be set higher than 15%");
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _marketingAndDevBudget).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingAndDevBudget == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalLiqFee = _marketingAndDevBudget + _liquidityFee;
        ...
        uint256 finalHalf = otherHalf.sub(marketingPortion);


        (bool sent, ) = _marketingDevAddress.call{value: marketingBalance.add(halfBalance)}("");
        if(sent){
            _tokenTransfer(address(this), _marketingDevAddress, marketingPortion, false);
            emit Transfer(address(this), _marketingDevAddress, marketingPortion);
        } else {
