...
contract MemeWarsToken is ERC20Token, IERC20Full {
  /**
   * @dev Upgradable initializer
   */
  function __MemeWarsToken_init() public initializer {
    __ERC20Token_init("MemeWarsToken", "WARS", 18, 10000000000);
  }
  ...
  function mint(address account, uint256 amount) external override {
    require(presenter == _msgSender(), "Token: presenter only");
  ...
  function burnFrom(address account, uint amount) external override {
    ...
    require(currentAllowance >= amount, "Token: exceeds allowance");
