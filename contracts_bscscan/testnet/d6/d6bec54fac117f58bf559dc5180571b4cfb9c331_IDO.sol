...
contract IDO is ReentrancyGuard, Context, Ownable {
    ...
    function setMultiWhitelist(address[] memory recipients) public onlyOwner{
        for(uint256 i = 0; i < recipients.length; i++){
            _whitelisted[recipients[i]] = true;
        }
    }

    function removeFromWhitelist(address[] memory recipients) public onlyOwner{
        for(uint256 i = 0; i < recipients.length; i++){
    ...
    //Start Pre-Sale
    function startICO(uint startDate, uint endDate, uint _minPurchase,uint _maxPurchase, uint256 _hardcap) external onlyOwner icoNotActive() {
    ...
    function _preValidatePurchase(address beneficiary, uint256 weiAmount) internal view {
        ...
        require(block.timestamp>= startTime, "IDO not Start yet");
        if(whitelistEnabled){
    ...
    function switchwhitelistEnabled(bool _turn) external onlyOwner {
        whitelistEnabled = _turn;
    }
    ...
    modifier icoActive() {
        require(endTime > 0 && block.timestamp < endTime && _weiRaised < hardcap, "ICO must be active");
        _;
    }
    
    modifier icoNotActive() {
        require(endTime < block.timestamp, 'ICO should not be active');
        _;
    }
