...
// Venus = Compound + MakerDAO
// Most codes are directly copied from Compound
contract VenusStrategy is BaseRhoStrategy {
    ...
    function withdrawUnderlyingInternal(uint256 _amount) internal override {
        require(vToken.redeemUnderlying(_amount) == 0, "VenusStrategy: Fail to withdraw from Venus");
        // Venus charges 0.01% upon withdrawal
        uint256 fee = _amount / 10000;
        underlying.safeTransfer(_msgSender(), _amount - fee);
    }

    function withdrawAllInternal() internal override {
        uint256 originalBal = underlying.balanceOf(address(this));
        // implement by redeem() but not redeemUnderlying() as vToken quantity is const
    ...
    function swapToken(
        ...
        require(xvs.balanceOf(address(this)) >= amountIn, "not enough balance");
        require(amountIn > 0, "amountIn cannot be 0");
        require(
            path[0] == address(xvs) && path[path.length - 1] == address(underlying),
            "path index 0 and index last should match stragegy award token and underlying token"
        );
        xvs.safeIncreaseAllowance(address(uniswap), amountIn);
...
interface IUnitroller {
    ...
    function venusSpeeds(address vTokenAddr) external view returns (uint256);
}
